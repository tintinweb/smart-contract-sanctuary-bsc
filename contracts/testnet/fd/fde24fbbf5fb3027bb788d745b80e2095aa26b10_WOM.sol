...
contract WOM is Context, IERC20, Ownable {
    ...
    uint8 public constant maxLiqFee = 3;
    uint8 public constant maxTaxFee = 3;
    uint8 public constant maxBurnFee = 3;
    uint8 public constant maxWalletFee = 2;
    uint8 public constant maxBuybackFee = 3;
    uint8 public constant minMxTxPercentage = 0; 
    uint8 public constant minMxWalletPercentage = 0;
    ...
    uint256 private constant MAX = ~uint256(0);  //Bitwise negation 2**265-1
    ...
    event maxWalletAmountUpdate(uint256 indexed maxWalletUpdate);
    event maxTxAmountUpdate(uint256 indexed maxTxAmountUpdate);
    event buyBackUpdate(uint256 indexed buyBackUpperLimitUpdate);
    event taxFeeUpdate(uint8 indexed _taxFeeUpdate, uint8 indexed _liquidityFeeUpdate, uint8 indexed _burnFeeUpdate);
    event burnWalletUpdate(uint8 indexed _buyBackFeeUpdate, uint8 indexed _walletFeeUpdate);
    ...
    constructor (address tokenOwner,string memory tokenName,
        ...
        _symbol = tokenSymbol;  //token symbol
        _decimals = decimal;  //token decimal point
        _tTotal = amountOfTokenWei;   //total amount of tokens in wei
        ...
        _maxTxAmount = _tTotal.mul(setMxTxPer).div(  //max amount of tokens someone can send, 1% - 10,000,000
            100
        );
         emit maxTxAmountUpdate(_maxTxAmount);  
        _maxWalletAmount = _tTotal.mul(setMxWalletPer).div(   //max amount of tokens someone can hold, 3% - 30,303,030
            33
           
        );
         emit maxWalletAmountUpdate(_maxWalletAmount);
    ...
    function setAllFeePercent(uint8 taxFee, uint8 liquidityFee, uint8 burnFee, uint8 walletFee, uint8 buybackFee) external onlyOwner() {
        require(taxFee >= 0 && taxFee <=maxTaxFee,"TF err"); //Sets max tax fee at 3
        require(liquidityFee >= 0 && liquidityFee <=maxLiqFee,"LF err"); //Sets max liquidity fee at 3
        require(burnFee >= 0 && burnFee <=maxBurnFee,"BF err"); //Sets max burn fee at 3
        require(walletFee >= 0 && walletFee <=maxWalletFee,"WF err"); //Sets max wallet fee at 2
        require(buybackFee >= 0 && buybackFee <=maxBuybackFee,"BBF err"); //Sets max buyback fee at 3
        _taxFee = taxFee;
        _liquidityFee = liquidityFee;
        _burnFee = burnFee;
        _buybackFee = buybackFee; 
        _walletFee = walletFee;
        emit taxFeeUpdate(taxFee, liquidityFee, burnFee);
        emit burnWalletUpdate(buybackFee, walletFee);
    ...
    function setBuybackUpperLimit(uint256 buyBackLimit) external onlyOwner() {
        ...
        emit buyBackUpdate (buyBackUpperLimit);
    }
 ...
 function updateRouter(address newAddress) external onlyOwner {
    ...
    pcsV2Router = IUniswapV2Router02(newAddress);
    address get_pair = IUniswapV2Factory(pcsV2Router.factory()).getPair(
      address(this),
      pcsV2Router.WETH()
    );
