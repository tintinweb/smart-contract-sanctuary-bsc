...
contract BSCswap2 {
    ...
    uint256 private constant dayPerCycle = 45 minutes; 
    uint256 private constant maxAddFreeze = 135 minutes;
    ...
    uint256[3] private balReached = [10000e18, 20000e18, 40000e18];
    uint256[3] private balFreezeStatic = [7000e18, 14000e18, 28000e18];
    uint256[3] private balFreezeDynamic = [4000e18, 8000e18, 6000e18];
    uint256[3] private balRecover = [7000e18, 14000e18, 28000e18];
    ...
    function _setReferral(address _user,address _referral, uint256 _refAmount, bool _isReDept) private {
        for(uint8 i = 0; i < level_bonuses.length; i++) {
            ...
            if(isok==1 && freezeDynamicReward==false){
    ...
    function unfreezeDepts(address _addr) private {
        ...
        for(uint i=0;i<userDepts[_addr].length;i++){
            ...
            if(pl.isUnfreezed==false && block.timestamp>=pl.unfreeze && isdone==0 && freezeStaticReward==false){
                pl.isUnfreezed=true;
    ...
    function withdraw(uint256 _amount) public{
        ...
        uint256 wamount=_amount.sub(tempSplit).sub(2e18);
