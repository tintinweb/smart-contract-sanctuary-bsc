    ...
    Ownable
{
    ...
    mapping(address => Order[]) public openOrders; // Mapping of User's Open trade
    mapping(bytes32 => uint256) public openOrderID;
    mapping(address => Order[]) public closeOrders; // mapping of User's closed trade
    mapping(bytes32 => orderInfo) public orderStatus; // order status
    ...
    constructor(address buyAndBurn) {
        ...
        jamonRouter = IJamonRouter(0xF300E478550a2e5eA17bff07B5D3E1E2f8c880a4);
        link = IERC20(0x84b9B910527Ad5C03A9Ca831909E21e236EA7b06);
    ...
    modifier onlyOnKeeperActive() {
        UpkeepInfo memory result = keeperRegistery.getUpkeep(keeperId);
        require(result.target == address(this), "Uninitialized keeper");
        _;
    }
    ...
    function _createOrder(
        ...
        openOrderID[orderId] = openOrders[user].length;
        openOrders[user].push(newOrder);
    }

    function removeOpenOrder(bytes32 _orderId) internal {

        uint256 index = openOrderID[_orderId];
        address user = orderBook[_orderId].user;
        openOrders[user][index] = openOrders[user][openOrders[user].length - 1];
        openOrderID[openOrders[user][openOrders[user].length - 1].orderID] = index;
        openOrders[user].pop();
        closeOrders[user].push(orderBook[_orderId]);
    }
    ...
    function _performSwap(
        ...
        if (tokenIn == WETH) {
            ...
            returns (uint256[] memory amounts) {
                orderStatus[orderId].message = string(("Order Filled Successfully"));
                  orderStatus[orderId].success = true;
                emit OrderFilled(orderId, amounts[path.length - 1]);
            } catch {
                _forceFail(amountIn, tokenIn, user);
                 orderStatus[orderId].message = "Order Force Failed ";
                  orderStatus[orderId].success = false;
            ...
            returns (uint256[] memory amounts) {
                orderStatus[orderId].message = string(("Order Filled at Successfully !"));
                 orderStatus[orderId].success = true;
                emit OrderFilled(orderId, amounts[path.length - 1]);
            } catch {
                 orderStatus[orderId].message = "Order Force Failed ";
    ...
    function getOpenOrderPerUser(address _user) external view returns (Order[] memory){
        return openOrders[_user];
    }

    function getCloseOrders(address _user) external view returns (Order[] memory){
        return closeOrders[_user];
    }
    ...
    function cancelOrder(bytes32 orderId) external nonReentrant {
        ...
        orderStatus[orderId].message =string(("Order Cancelled by the User "));
        orderStatus[orderId].success = false;
