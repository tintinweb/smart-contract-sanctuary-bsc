...
contract TSwap is Ownable {
    ...
    function swapExactTokensForTokens(
        ...
        IBEP20(path[0]).approve(address(pancakeSwap), amountIn);

        if (isTeamToken(path[0])){

            ( amounts ) = pancakeSwap.swapExactTokensForTokens(amountIn, amountOutMin, path, address(this), block.timestamp + 5 minutes);

            IBEP20(path[1]).transfer(prizeAddress, amounts[1] * prizeFee / 100);
            IBEP20(path[1]).transfer(pairAddress, amounts[1] * liquidityFee / 100);
            IBEP20(path[1]).transfer(marketingAddress, amounts[1] * marketingFee / 100);
            IBEP20(path[1]).transfer(treasuryAddress, amounts[1] * treasuryFee / 100);

            IBEP20(path[1]).transfer(msg.sender, amounts[1] * (100 - totalFee) / 100);

        } else {

            IBEP20(path[0]).transfer(prizeAddress, amountIn * prizeFee / 100);
            IBEP20(path[0]).transfer(pairAddress, amountIn * liquidityFee / 100);
            IBEP20(path[0]).transfer(marketingAddress, amountIn * marketingFee / 100);
            IBEP20(path[0]).transfer(treasuryAddress, amountIn * treasuryFee / 100);
