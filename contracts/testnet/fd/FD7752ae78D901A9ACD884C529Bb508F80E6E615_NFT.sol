...
contract NFT is ERC721, ERC721Enumerable, Pausable, AccessControl, ERC721Burnable {
    ...
    constructor(string memory name, string memory symbol, string memory uriBase) ERC721(name, symbol) {
    ...
    function tokenIdsOfOwner(address owner, uint256 startIdx)
        public
        view
        returns (uint256[] memory, uint256, uint256)
    {
        ...
        if(length == 0 || startIdx >= length){size = 0;}
        else
        {
            if(startIdx + size - 1 > length){size = length - startIdx;}
            last = startIdx + size;
            remain = length - last;
        }
