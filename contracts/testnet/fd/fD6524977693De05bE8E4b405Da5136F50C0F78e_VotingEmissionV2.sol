...
contract VotingEmissionV2 is IVotingEmissionV2, OwnableUpgradeable, CoreUtility {
    ...
    function initialize(address votingEscrow_, uint256 guardedLaunchStart_) public initializer {
        ...
        require(_endOfWeek(guardedLaunchStart_) == guardedLaunchStart_ + 1 weeks, "must be end of a week");
    }
    ...
    function initializeEmission(uint256 lastTimestamp_) external {
        ...
        for(uint256 i = 0; i < size; i++) {
            weights0 = weights0.add(weightsPerWeek[lastTimestamp_ + 1 weeks][_pools[i]]);
    ...
    function _updateWeight() private {
        uint256 weekTimestamp = _endOfWeek(block.timestamp).sub( 1 weeks);
        uint256 lastTimestamp_ = lastTimestamp; // gas saver
        require(weekTimestamp <= lastTimestamp_ + 1 weeks, "Previous week is empty");
        if (weekTimestamp > lastTimestamp_) {
            lastTimestamp = lastTimestamp_ + 1 weeks;
