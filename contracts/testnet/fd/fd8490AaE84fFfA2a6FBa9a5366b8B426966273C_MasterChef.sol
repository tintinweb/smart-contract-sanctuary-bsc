...
contract MasterChef is Ownable {
    ...
    function pendingBountie(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cakeReward = multiplier.mul(bountiePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accBountiePerShare = accBountiePerShare.add(cakeReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cakeReward = multiplier.mul(bountiePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        bountie.mint(devaddr, cakeReward.div(10));
        // Syrup reward ? 
        // bountie.mint(address(syrup), cakeReward);
        pool.accBountiePerShare = pool.accBountiePerShare.add(cakeReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit BOUNTIE by staking');
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0) {
                safeBountieTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            uint256 penalty = calculatePenalty(_pid, _amount);
    ...
    function calculatePenalty(uint256 _pid, uint256 _amount) public returns (uint256) {
        // Staking penalty
        ...
        uint256 durationDay = block.number.sub(user.depositBlock).div(28800);
        if(durationDay < 14) {
            return _amount.mul(25).div(1e2);
        } else if (durationDay < 28) {
            return _amount.mul(15).div(1e2);
        } else if (durationDay < 42) {
            return _amount.mul(10).div(1e2);
        } else if (durationDay < 56) {
            return _amount.mul(5).div(1e2);
        } else {
            return _amount;
        }
    }
    ...
    // function safeBountieTransfer(address _to, uint256 _amount) internal {
    //     syrup.safeBountieTransfer(_to, _amount);
    // }

    function safeBountieTransfer(address _to, uint256 _amount) public onlyOwner {
...
contract BountieHunter is ERC20, Ownable {
    ...
    constructor() public ERC20 ("Kuda Token", "KUDA")
    {
