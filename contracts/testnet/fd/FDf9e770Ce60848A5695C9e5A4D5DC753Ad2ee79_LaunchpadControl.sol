...
contract LaunchpadControl is OwnableUpgradeable, UUPSUpgradeable {
  ...
  function createProject(
    ...
    AlturaNFTFactoryV2(payable(_factory)).createCollection(_name, _uri, bPublic);
  }
  ...
  function buyNFT(uint256 _id, uint256 _amount) external payable {
    ...
    require(
      items[_id].currency != address(0x0) || items[_id].price.mul(_amount) == msg.value,
      "Invalid amount"
    );
    ...
    emit BuyNFT(msg.sender, item.owner, _id, items[_id].collection, items[_id].token_id, _amount);
  }
