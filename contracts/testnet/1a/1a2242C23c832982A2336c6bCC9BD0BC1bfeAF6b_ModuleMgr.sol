...
contract ModuleMgr is Ownable {
    ...
    address landBlindBoxDataAddress;
   ...
   function setRelationship(address _relationshipAddress) external onlyOwner {
    ...
    function setRewardAccount(address _rewardAccountAddress) external onlyOwner {
        require(_rewardAccountAddress != rewardAccountAddress, "the same");
        rewardAccountAddress = _rewardAccountAddress;
    }

    function getRewardAccount() external view returns (address res) {
        res = rewardAccountAddress;
    }
