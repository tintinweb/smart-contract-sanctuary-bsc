...
contract HeroNFT is BaseNFT {
  ...
  function syncFOTAOwnPrice(uint[] calldata _tokenIds, uint[] calldata _fotaOwnPrices) onlyMintAdmin external {
    for (uint i = 0; i < _tokenIds.length; i++) {
      ...
      emit AllOwnPriceUpdated(_tokenIds[i], heroes[_tokenIds[i]].ownPrice, _fotaOwnPrices[i], heroTotalProfitedInFota[_tokenIds[i]]);
    }
  }
  ...
  function updateAllOwnPrices(uint _tokenId, uint _ownPrice, uint _fotaOwnPrice) onlyMintAdmin external {
    ...
    emit AllOwnPriceUpdated(_tokenId, _ownPrice, _fotaOwnPrice, heroTotalProfitedInFota[_tokenId]);
  }
