...
contract IDO is Ownable{
    ...
    function claimNFT(uint lev) external {
        ...
        if(lev==0 &&  _users[addr].invitees.length >=inveNum && _users[addr].isAir && nftAlreadyClaim[addr][0] != true ){//receive ordinary
          INft(_nft).mintTo(addr,0);
            nftAlreadyClaim[addr][0]=true;
        }

        if(lev==1&& _users[addr].invitees.length >=inveNum && whiteDsList[addr] && _users[addr].isAir && nftAlreadyClaim[addr][1] != true ){//Receive a community leader
           INft(_nft).mintTo(addr,1);
           nftAlreadyClaim[addr][1]=true;
        }

         if(lev==2&&  _users[addr].invitees.length >=inveNum && whiteFhzList[addr] && _users[addr].isAir && nftAlreadyClaim[addr][2] != true ){//Receive a squad leader
           INft(_nft).mintTo(addr,2);
           nftAlreadyClaim[addr][2]=true;
        }
    ...
    function viewClaimNFT(address addr) view external returns(uint[] memory cc) {
        ...
        if(_users[addr].invitees.length >=inveNum && _users[addr].isAir&&nftAlreadyClaim[addr][0]==false ){//get captain
            cc[0]=1;
        }else if (nftAlreadyClaim[addr][0]==true){
             cc[0]=2;
        }
        if( _users[addr].invitees.length >=inveNum && whiteDsList[addr] && _users[addr].isAir &&nftAlreadyClaim[addr][1]==false ){//Receive a community leader
            cc[1]=1;
        }else if(nftAlreadyClaim[addr][1]==true){
            cc[1]=2;
        }
        if(_users[addr].invitees.length >=inveNum && whiteFhzList[addr] && _users[addr].isAir&&nftAlreadyClaim[addr][2]==false ){//Receive a squad leader
            cc[2]=1;
        }else if(nftAlreadyClaim[addr][2]==true){
            cc[3]=2;
        }
    }
