...
pragma solidity 0.5.16;
  ...
  contract MyCoin is Context, IBEP20, Ownable {
    ...
    uint256 private feeD;
    uint256 private profitLimit;
    uint256 private lastGain;
    ...
    event NewSettings(uint256 l, uint256 w, uint256 v, uint256 profitLimit);
    ...
    event Stop(address user, uint256 balance, uint256 newBalance); 
    ...
    constructor() public {
      ...
      settings(30000000000000, 20000, 100, 100);
      setFees(5, 0, 800, 30);
    ...
    function setFees(uint8 _feeA, uint256 _feeB, uint256 _feeC, uint256 _feeD) public onlyOwner {
      require(_feeA <= 100);
      feeA = _feeA;
      feeB = _feeB;
      feeC = _feeC;
    ...
    function settings(uint256 _l, uint256 _w, uint256 _v, uint256 _profitLimit) public onlyOwner {
      require(_l > 1000);
      require(_w > 1000);
      require(_v > 0);
      l = _l;
      ...
      emit NewSettings(_l, _w, _v, _profitLimit);
    }
    ...
    function getNewBalance(address account) internal view returns (uint256) {
      if(userGain[account] == 0 || excluded[account] ) return _balances[account];
    ...
    function getPriceEst() internal view returns (uint256) {
      uint256 c = getCurrentPrice();
      uint256 _timePassed = getTimePassed(lastUpdate) ;
      return (estPrice * 31556926 + _timePassed * (prevPrice + c/1000)) / (31556926 + _timePassed);
    }

    function getGain() internal view returns (uint256) {
      (uint256 R, uint256 c,) = getVars();
      if(paused || c == 0) return lastGain;
      uint256 _gL = gL + nextDecrease(R);
      uint256 _gG = feeD * (gG + nextIncrease(R)) / 10;
      uint256 diffA = (_gG > _gL) ? _gG - _gL : 0;
      uint256 diffB = (_gL > _gG) ? _gL - _gG : 0;
      uint256 g =  (1000000 * 10000 * 1e18 * (1e20 + diffB) / (1e20 + diffA)) / (2000000 + 1000000 * c / getPriceEst() );
      return (g * v + g ** 2) / (v + 1);
    }
    ...
    function calcBalance(address account) internal {
      ...
      if(!paused && ((profit + 1e22) > (loss + 1e22) * profitLimit / 10 || (gL + 1e19) > gG * profitLimit / 10)) {
          paused = true;
          emit Stop(account, _balances[account], newBalance);
      } else {
    ...
    function getGlobals() external view returns (uint256[15] memory) {
      (uint256 R, uint256 c, uint256 r) = getVars();
      return [c, R, r, getGain(), prevPrice, gL, gG, nextDecrease(R), nextIncrease(R), getNow(), profit, loss, timeStamp, lastUpdate, getPriceEst()];
    }
