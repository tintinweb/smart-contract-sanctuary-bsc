...
library PancakeLibrary {
    ...
    function getReserves(bytes32 initHash, address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IPancakePair(pairFor(initHash, factory, tokenA, tokenB)).getReserves();
...
contract AnjiRouter is IPancakeRouter02, Ownable {
    ...
    constructor(address _WETH, address _BUSD, address _anji) public {
    ...
    function addDex(string memory name, address _factory, address router, bytes32 _initHash, uint256 fee) external onlyOwner {
        dexList[dexCount++] = DEX({
        name: name,
        factory: _factory,
        router: router,
    ...
    function getLargestDEX(address tokenA, address tokenB) public {
        ...
        for(uint256 i=0; i<dexCount; i++){
            ...
            if(nextPair != address(0)){
                (uint256 reserve0, uint256 reserve1) = PancakeLibrary.getReserves(dexList[i].initHash, dexList[i].factory, tokenA, tokenB);
    ...
    function setSwapDex(uint256 dexId) internal {
        factory = dexList[dexId-1].factory;
        initHash = dexList[dexId-1].initHash;
        _fee = dexList[dexId-1].fee;
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? PancakeLibrary.pairFor(initHash, factory, output, path[i + 2]) : _to;
            IPancakePair(PancakeLibrary.pairFor(initHash, factory, input, output)).swap(
    ...
    //     amounts = PancakeLibrary.getAmountsOut(initHash, factory, amountIn, path, _fee);
    //     require(amounts[amounts.length - 1] >= amountOutMin, 'PCR: INSUF_OUTA');
    //     TransferHelper.safeTransferFrom(
    //         path[0], msg.sender, PancakeLibrary.pairFor(initHash, factory, path[0], path[1]), amounts[0]
    //     );
    ...
    // ) external virtual override ensure(deadline) returns (uint[] memory amounts) {
    //     getLargestDEX(path[0],path[1]);
    //     amounts = PancakeLibrary.getAmountsIn(initHash, factory, amountOut, path, _fee);
        ...
        returns (uint[] memory amounts)
    {
        ...
        uint fee = _feeAmount(msg.value, path[1], false, usd);
        ...
        amounts = PancakeLibrary.getAmountsOut(initHash, factory, amount, path, _fee);
        require(amounts[amounts.length - 1] >= amountOutMin, 'AnjiR: INSUF_OUTA');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(initHash, factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        ...
        if(IWETH(WETH).balanceOf(feeReceiver) > 3){
            //AnjiFees(feeReceiver).distributeDividend();
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(path[0] == WETH, 'AnjiR: INVALID_PATH');
        //uint amountIn = msg.value;
        uint fee = _feeAmount(msg.value, path[1] ,true, usd);
        //uint amount = msg.value - fee;
        if (dexId == 0) {
            getLargestDEX(path[0],path[1]);
        } else {
        ...
        amounts = PancakeLibrary.getAmountsOut(initHash, factory, msg.value - fee, path, _fee);
        require(amounts[amounts.length - 1] >= amountOutMin, 'AnjiR: INSUF_OUTA');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(initHash, factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = PancakeLibrary.sortTokens(input, output);
            IPancakePair pair = IPancakePair(PancakeLibrary.pairFor(initHash, factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = PancakeLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, _fee);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(initHash, factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(initHash, factory, path[0], path[1]), amount));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        if (fee > 0) {
            TransferHelper.safeTransferETH(feeReceiver, fee);
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(initHash, factory, path[0], path[1]), msg.value - fee));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        if (fee > 0) {
            TransferHelper.safeTransferETH(feeReceiver, fee/2);
    ...
    {
        return PancakeLibrary.getAmountOut(amountIn, reserveIn, reserveOut,9975);
    }
    ...
    {
        return PancakeLibrary.getAmountIn(amountOut, reserveIn, reserveOut, 9975);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return PancakeLibrary.getAmountsOut(initHash, factory, amountIn, path, _fee);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return PancakeLibrary.getAmountsIn(initHash, factory, amountOut, path, _fee);
    }
}