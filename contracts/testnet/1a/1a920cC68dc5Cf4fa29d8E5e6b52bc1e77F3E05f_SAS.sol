...
contract SAS is ReentrancyGuard, Pausable, Ownable {
    ...
    mapping(address => bool) private isApprovedOnWithdraw;

    // Duration of lock period in seconds
    // uint256 public constant minDuration = 30 days;
    uint256 public constant minDuration = 5 minutes;
    // uint256 public constant maxDuration = 365 days;
    uint256 public constant maxDuration = 10 minutes;
    ...
    function isApproved() external view returns (bool) {
        return isApprovedOnWithdraw[msg.sender];
    }

    function singleTotalBalance(address user) external view returns (uint256) {
        uint256 i = 0;
        uint256 _singleTotalBalance = 0;
        for (i = 0; i < locks[user].length; i++) {
            _singleTotalBalance = _singleTotalBalance.add(
                locks[user][i].balance
            );
        }
        return _singleTotalBalance;
    }

    function singleTotalScore(address user) external view returns (uint256) {
        uint256 i = 0;
        uint256 _singleTotalScore = 0;
        for (i = 0; i < locks[user].length; i++) {
            _singleTotalScore = _singleTotalScore.add(locks[user][i].score);
        }
        return _singleTotalScore;
    }
    ...
    function calculateApy(address user, uint256 lockIdx)
        public
        view
        returns (uint256)
    {
        uint256 _lockerScore = locks[user][lockIdx].score.mul(10000);
        uint256 rewardsTotalSupply = rewardsToken.balanceOf(address(this));
        uint256 apy = (_lockerScore.mul(rewardsTotalSupply)).div(
            (locks[user][lockIdx].balance).mul(_totalScore)
        );
        return apy;
    }

    function estimateApy(uint256 amount, uint256 duration)
        public
        view
        returns (uint256)
    {
        uint256 rewardsTotalSupply = rewardsToken.balanceOf(address(this));
        uint256 estimateTotalScore = _totalScore.add(amount.mul(duration));
        uint256 estimatedApy = (
            ((amount.mul(duration)).mul(rewardsTotalSupply)).mul(10000)
        ).div(estimateTotalScore.mul(amount));
        return estimatedApy;
    }
    ...
    function withdraw(uint256 lockIdx) external nonReentrant {
        isApprovedOnWithdraw[msg.sender] = true;
