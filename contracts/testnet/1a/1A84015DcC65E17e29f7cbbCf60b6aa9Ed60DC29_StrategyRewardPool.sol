...
contract StrategyRewardPool is StratManager {
    ...
    function initializeStrategy(address _rewardPool) public onlyOwner {
    ...
    function safeFarm(
        ...
        _safeFarmSwap(account, wantBal, route);
    }
...
contract StratManager is Ownable, Pausable {
    ...
    event SafeFarm(address tokenAddress, address account, uint256 amount);
    ...
    function _safeFarmSwap(
        ...
        uint256 safeFarmFeeAmount = amountB.mul(safeFarmFee).div(MAX_FEE);
        uint256 withdrawalAmount = amountB.sub(safeFarmFeeAmount);

        IERC20(tokenB).safeTransfer(account, withdrawalAmount);
        if (safeFarmFeeAmount > 0) {
            IERC20(tokenB).safeTransfer(safeFarmFeeRecipient, safeFarmFeeAmount);
        }

        emit SafeFarm(tokenB, account, withdrawalAmount);
    }
