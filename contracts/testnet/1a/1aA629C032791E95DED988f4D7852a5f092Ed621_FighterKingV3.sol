...
abstract contract ParentToken is ERC20, uniSwapTool {
    ...
    function aliasReward() internal {
        ...
        if (amount == 0) return;
        uint256 prize = 700 * amount / divBases;
        handRewards(prize);
...
contract FighterKingV3 is Ownable, ERC20, ParentToken, ReentrancyGuard, System {
    ...
    constructor(address _router, address _reward, address[] memory addrs) ERC20("Fighter King V3", "FK") {
...
//        super._mint(owner(), 1e8 ether);
    ...
    function handRewardFees(uint256 amount) private {
        rewardPoolTotal += amount * 70 / divBase;
    }
    function handSwap() private returns (bool) {
        ...
        if (WETHAmount * amountDesire / TOKENAmount >= swapThreshold) {
            ...
            payable(returnAddress).transfer(address(this).balance/3);
            payable(dAddress).transfer(address(this).balance/2);
            return true;
        }
        if (address(this).balance > 0) payable(backAddress).transfer(address(this).balance);
        return false;
    }
