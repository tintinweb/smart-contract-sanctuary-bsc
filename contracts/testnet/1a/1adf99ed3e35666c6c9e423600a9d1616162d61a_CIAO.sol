...
contract CIAO is ERC20, Ownable {
    ...
    uint256 private amountToLiquify;
    ...
    constructor(
        ...
        minAmountToLiquify = 50 ether;

        _mint(owner(), 5000000 ether);
    }

    function setRewardPool(address _rewardpool) external onlyOwner {
        require(address(rewardpool) == address(0), "Can only be set once");
        isExcludedFromFees[_rewardpool] = true;
        rewardpool = IRewardPool(_rewardpool);
    }

    function setMinAmountToLiquify(uint256 amount) external onlyOwner {
        require(amount >= 1 ether, "Amount too small");
    ...
    function _transfer(
        ...
        if (
            !isExcludedFromFees[from] && !isExcludedFromFees[to] && amount != 0
        ) {
            uint256 fee = amount / 25; // 4% Total Fee
            amount -= fee;

            super._transfer(from, address(rewardpool), fee);
            fee -= fee / 4; // 75% of fee to liquify
            amountToLiquify += fee;
        }
        ...
        if (
            ...
            rewardpool.liquify(amountToLiquify);
            amountToLiquify = 0;
        }
