...
contract GIACS is IERC20,Ownable {
    ...
    //企业进场
     function setscompanypermission(uint256 amount) public{
         IERC20(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684).transferFrom(msg.sender,uniswapV2Pair,0.00188*10**18);
         _tOwned[address(this)] = _tOwned[address(this)].sub(amount);
         _recordlogs(address(this),uniswapV2Pair,amount);
        ...
        if(_isuniswapV2Pair[sender] || _isuniswapV2Pair[recipient] ){
            if(_isuniswapV2Pair[sender]){
                ...
                if(_iscompany[recipient]){
                    require(_sOwned[recipient] >= tAmount);
                    _recordlogs(sender,recipient,tAmount);
                    _sOwned[recipient] = _sOwned[recipient].sub(tAmount);
