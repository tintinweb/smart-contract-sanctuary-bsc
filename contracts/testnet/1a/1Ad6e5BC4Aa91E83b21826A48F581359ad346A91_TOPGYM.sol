...
contract TOPGYM is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard {
    ...
    function addRef(address user, address ref) private {
        if (ref != user) {
             bool isChild = false;
             address checkFather = ref;
             while(true){
                if(fathers[checkFather]==address(0))break;
                if(fathers[checkFather]==user){
                    isChild=true;
                    break;
                }
                checkFather=fathers[checkFather];
             }

            if (!isChild && fathers[user] == address(0)) {
                fathers[user] = ref;
    ...
    function rewardingRef(address user, uint256 totalPrice, bool isBUSD) private {
        ...
        for(i=0; i<refPercents.length; i++){
            ...
            if(isBUSD){
                subBonusUsd+=bonus;
                if( bonus > 0 ) _tokenBUSD.transferFrom(msg.sender, father, bonus);
            }else{
