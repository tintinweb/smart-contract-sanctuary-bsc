...
import "hardhat/console.sol";
contract OptVaultAuto is AccessControl, Initializable {
    ...
    function purchase(address user, uint amount) external returns(uint){
    ...
    function sell(address user, uint amount) external returns(uint){
    ...
    function swapWantToBUSD() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        path[0] = address(want);
        path[1] = Admin.BUSD();
        uint256 wantBalance = IPancakeMaster(masterChef).userInfo(id, address(this)).amount - totalStaked;
