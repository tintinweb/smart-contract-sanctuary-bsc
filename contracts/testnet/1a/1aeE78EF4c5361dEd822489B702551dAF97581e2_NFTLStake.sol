...
contract NFTLStake is Ownable, ReentrancyGuard {
...
//end of variables for dapp
  ...
  mapping (address => uint256) private ownerToNFTokenCount;
    ...
    constructor() {
        ...
        nftCollection = IERC721(0xCa76f5E6c9469Dba1cC15414b6402d1B66867DA0);
        rewardsToken = IERC20(0x106FEFf621a395043b21a602D4b6a2039e16bBeC);
        } else if (block.chainid == 97) {
        nftCollection = IERC721(0x057D7E3f4a88efa75C436C5F4e0bB184a5196304);
        rewardsToken = IERC20(0x0aDb5Af66dE60A86Ad09adC7cd22CBF736aB6E4d);
        } else {
            revert();
        }
//Variables have been defined now we move on the rest of the configuration	
    ...
    //multisend nft
    function multisendNFT(uint256[] calldata _tokenIds, address[] calldata recipient) external nonReentrant {
        uint256 len = _tokenIds.length;
        for (uint256 i; i < len; ++i) {													   
            require(
                nftCollection.ownerOf(_tokenIds[i]) == msg.sender,
                "Can't send tokens you don't own!"											
            );
            nftCollection.transferFrom(msg.sender, recipient[i], _tokenIds[i]);							
        }
        stakers[msg.sender].amountStaked += len;
        stakers[msg.sender].timeOfLastUpdate = block.timestamp;
    }
