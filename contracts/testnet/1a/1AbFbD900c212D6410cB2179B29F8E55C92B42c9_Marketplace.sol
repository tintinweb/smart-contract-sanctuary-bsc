...
interface IWoolFactory {
    ...
    function priceOf(uint256 _tier) external pure returns (uint256);
    ...
    function countItemsOf(address _user) external;
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    // Mapping to indicate token URIs
    ...
    function _afterTokenTransfer(address from) internal virtual {}
}
...
contract Marketplace is IERC721Receiver, Pausable, Whitelist, ReentrancyGuard {
    ...
    IERC20 public sheep;  // SH33P token
    ...
    function maxItemsOfTier(uint256 _tier) public view returns (uint256) {
    ...
    function totalAvailableOfTier(uint256 _tier) public view returns (uint256) {
        ...
        uint256 _max = maxItemsOfTier(_tier);

        return (_max.sub(_minted));
    }
    ...
    function listItem(address _contract, uint256 _tokenId, uint256 _markup) whenNotPaused() public nonReentrant {
        ...
        uint256 _mintPrice = woolMinter.priceOf(_tier);
        ...
        woolMinter.countItemsOf(msg.sender);
    ...
    function buyItem(address _contract) whenNotPaused() public nonReentrant returns (uint256) {
    ...
    function buyItemFromMarket(uint256 _tierId, address _recipient) internal returns (uint256) {
        ...
        require(sheep.transferFrom(_recipient, address(this), price), 'Must pay item price');
        
        // Pay the seller for the item
        sheep.transfer(seller, price);
    ...
    function buyItemFromMint(uint256 _tierId, address _recipient) internal returns (uint256 _newItemID) {
        ...
        address _contract = getContractOf(_tierId);
        uint256 _mintPrice = woolMinter.priceOf(_tierId);

        tierData[_tierId].totalMinted += 1;

        // Collect Mint Payment
        require(sheep.transferFrom(_recipient, mintFeeSplitter, _mintPrice), 'Must pay minting fee');
    ...
    function setMintFeeSplitter(address _address) onlyOwner() whenNotPaused() public returns (bool _success) {
    ...
    function pause() onlyOwner() public returns (bool _success) {
        _pause();

        return true;
    }

    function unpause() onlyOwner() public returns (bool _success) {
        _unpause();
