...
contract TopStaking is ReentrancyGuard, DataStorage, Events, Ownable, Utils, Pausable {
    ...
    function _invest(uint256 _poolId, uint256 _amount) internal {
        ...
        user.deposits.push(Deposit(_poolId, block.timestamp, block.timestamp, _amount, pools[_poolId].rate, false));
        if(user.registerTime == 0) {
            user.registerTime = block.timestamp;
            totalUser.push(user);
            emit Newbie(_msgSender(), block.timestamp);
    ...
    function unStake(uint256 _poolId) external nonReentrant whenNotPaused {
        UserInfo storage user = userInfos[_poolId][_msgSender()];
        ...
        IERC20(pool.tokenAddress).transfer(_msgSender(), (totalCurrentDividend * (DENOMINATOR - _usageFee)/DENOMINATOR));
        IERC20(pool.tokenAddress).transfer(_coldWallet, (totalCurrentDividend * _usageFee/DENOMINATOR));
        emit UnStake(_msgSender(), _poolId, (totalCurrentDividend*(DENOMINATOR - _usageFee)/DENOMINATOR));
    }

    function harvest(uint256 _poolId) external nonReentrant whenNotPaused {
        UserInfo storage user = userInfos[_poolId][_msgSender()];
        Pool memory pool = pools[_poolId];
        ...
        returns (uint256)
    {
        ...
        for (uint256 index = 0; index < user.deposits.length; index++) {
            if(!user.deposits[index].isUnstake) {
                ...
                uint256 toTime = pool.toTime > block.timestamp ? block.timestamp : pool.toTime;
                uint256 totalTime = toTime.sub(fromTime);
                currentDividend = currentDividend + user.deposits[index].amount + user.deposits[index].amount.mul(totalTime).mul(user.deposits[index].rate).div(DENOMINATOR).div(365*SECONDS_IN_A_DAY);            }
        }
        ...
        returns (uint256)
    {
        ...
        for (uint256 index = 0; index < user.deposits.length; index++) {
            if(!user.deposits[index].isUnstake) {
                ...
                uint256 toTime = pool.toTime > block.timestamp ? block.timestamp : pool.toTime;
                uint256 totalTime = toTime.sub(fromTime);
                currentDividend = currentDividend + user.deposits[index].amount.mul(totalTime).mul(user.deposits[index].rate).div(DENOMINATOR).div(365*SECONDS_IN_A_DAY);
            }
        }
        return currentDividend;          
    }
