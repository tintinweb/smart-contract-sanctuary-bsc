...
abstract contract OracleReader {
     ...
     * @param source the token being traded from
     * @param target the token being traded to
     */
    function getFeeMinimumFromExchange(
        address factory,
        uint256 _amountIn,
        address source,
        address target
    ) public view returns(uint256) {
        uint256 minimum = getSingleFeeMinimumFromExchange(factory, _amountIn, source);
        if (minimum == 0) {
            minimum = getSingleFeeMinimumFromExchange(factory, _amountIn, target);
        }
        return minimum;
    }
     ...
     */
    function getSingleFeeMinimumFromExchange(
        address factory,
        uint256 _amountIn,
        address source
    ) public view returns(uint256 minimum) {
        (uint256 tokenAmount, uint256 wethAmount) = checkPairForValidPrice(factory, source);
        if (hasEnoughReserves(tokenAmount, wethAmount)) {
            return (((((_amountIn * wethAmount) / tokenAmount) * fee) / feeDenominator) * 9_000) / 10_000;
        }
    }

    function hasEnoughReserves(uint256 tokenAmount, uint256 wethAmount) public pure returns(bool) {
        return tokenAmount > 10_000_000 && wethAmount > 1 ether;
    }

    function checkPairForValidPrice(address factory, address token) public view returns(uint256 tokenA, uint256 tokenB) {
        IUniswapV2Factory uniFactory = IUniswapV2Factory(factory);
        address pair = uniFactory.getPair(token, wNative);
        if (pair.isContract()) {
            (tokenA, tokenB) = getSyncedPairReserves(pair);
        }
        if (!hasEnoughReserves(tokenA, tokenB)) {
            pair = uniFactory.getPair(wNative, token);
            if (pair.isContract()) {
                (uint256 _tokenB, uint256 _tokenA) = getSyncedPairReserves(pair);
                if (tokenA < _tokenA && tokenB < _tokenB) {
                    tokenA = _tokenA;
                    tokenB = _tokenB;
                }
            }
    ...
    function getSyncedPairReserves(address pair) public view returns(uint256, uint256) {
        uint256 tokenA;
        uint256 tokenB;
        IUniswapV2Pair uniPair = IUniswapV2Pair(pair);
        (tokenA, tokenB, ) = uniPair.getReserves();
 ...
 */
contract WalletSwap is Ownable, Migratable, Recoverable, Utils, OracleReader {
    ...
    function swapTokenV2(
        ...
        if (first == _wNative) {
            ...
            uint256 minimum = getFeeMinimumFromExchange(
