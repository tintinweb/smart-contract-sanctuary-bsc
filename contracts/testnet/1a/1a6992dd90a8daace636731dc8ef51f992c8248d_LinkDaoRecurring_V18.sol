...
contract LinkDaoRecurring_V18 is Ownable {
    ...
    uint256 public deposittedAmount;
    ...
    function depositLKD(uint256 _amount) external onlyOwner{
        deposittedAmount = deposittedAmount.add(_amount);
        IERC20(LINKDAO_TOKEN_ADDRESS).safeTransferFrom(msg.sender,address(this),_amount);
    }

    function withdrawLKD(uint256 _amount) external onlyOwner{
        require(deposittedAmount >= _amount,"Cannot withdraw more than depositted amount");
        deposittedAmount = deposittedAmount.sub(_amount);
