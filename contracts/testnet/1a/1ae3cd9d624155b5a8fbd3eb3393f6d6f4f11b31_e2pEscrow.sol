...
contract e2pEscrow is Stoppable, SafeMath {
  ...
  // fixed amount of percentage wei accrued to verifier with each transfer
  uint public commissionFeePercentage;
  ...
  event LogChangeFixedCommissionFeePercentage(
				    uint oldCommissionFeePercentage,
				    uint newCommissionFeePercentage
				    );
   ...
   * @param _commissionFeePercentage uint Verifier's fixed commission Percentage for each transfer
   */
  constructor(uint _commissionFeePercentage, address _verifier) public {
    commissionFeePercentage = _commissionFeePercentage;
    verifier = _verifier;
  }
    ...
    returns(bool)
  {
    ...
    require(msg.value > 0);
    uint commissionFee = (safeDiv(safeMul(msg.value,commissionFeePercentage),100));
   ...
   * 
   * @param _newCommissionFeePercentage uint New verifier's fixed commission percentage
   * @return True if success.
   */
  function LogChangeCommissionFeePercentage(uint _newCommissionFeePercentage)
                          public
                          whenNotPaused
                          whenNotStopped
                          onlyOwner
  ...
  {
    uint oldCommissionFeePercentage = commissionFeePercentage;
    commissionFeePercentage = _newCommissionFeePercentage;
    emit LogChangeFixedCommissionFeePercentage(oldCommissionFeePercentage, _newCommissionFeePercentage);
    return true;
  }
