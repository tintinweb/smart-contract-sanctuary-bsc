...
contract UkraineDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Ukraine_Dividend_Tracker", "Ukraine_Dividend_Tracker") {
        claimWait = 3600;
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "Ukraine_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Ukraine_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Ukraine contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Ukraine_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Ukraine_Dividend_Tracker: Cannot update claimWait to same value");
...
contract TESTUkraine is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    UkraineDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 1000000000 * (10**9);
    uint256 public maxBuyTransactionAmount = 1000000000 * (10**9);
    ...
    address payable public  developmentWallet1;
    address payable public  developmentWallet2;
    ...
    function setBuyFees(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _developmentFee, uint256 _buybackFee) 
        public onlyOwner {
        BNBRewardsBuyFee = _bnbRewardFee;
        liquidityBuyFee = _liquidityFee;
        marketingBuyFee = _marketingFee;
        developmentBuyFee = _developmentFee;
        buybackBuyFee = _buybackFee;
        totalBuyFees = BNBRewardsBuyFee.add(liquidityBuyFee).add(marketingBuyFee).add(developmentBuyFee).add(buybackBuyFee); // total buy fee
    }
    
    function setSellFees(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _developmentFee, uint256 _buybackFee) 
        public onlyOwner {
        BNBRewardsSellFee = _bnbRewardFee;
        liquiditySellFee = _liquidityFee;
        marketingSellFee = _marketingFee;
        developmentSellFee = _developmentFee;
        buybackSellFee = _buybackFee;
        totalSellFees = BNBRewardsSellFee.add(liquiditySellFee).add(marketingSellFee).add(developmentSellFee).add(buybackSellFee); // total sell fee
    }
    ...
    constructor() ERC20("test Save Ukraine", "testUKRAINE") 
    {
        BNBRewardsBuyFee = 1;
        liquidityBuyFee = 1;
        marketingBuyFee = 5;
        ...
        liquiditySellFee = 1;
        marketingSellFee = 5;
        developmentSellFee = 3;
        buybackSellFee = 2;
        totalSellFees = BNBRewardsSellFee.add(liquiditySellFee).add(marketingSellFee).add(developmentSellFee).add(buybackSellFee); // total Sell fee
        
        
        
        marketingWallet = payable(0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db);
        developmentWallet1 = payable(0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB);
        developmentWallet2 = payable(0x17F6AD8Ef982297579C203069C1DbfFE4348c372);
        

    	dividendTracker = new UkraineDividendTracker();
        ...
        _isExcludedFromMaxTx[developmentWallet1] = true;
        _isExcludedFromMaxTx[developmentWallet2] = true;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Ukraine: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Ukraine: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Ukraine: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Ukraine: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Ukraine: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Ukraine: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && automatedMarketMakerPairs[to] ||
            !_isExcludedFromFees[to] && automatedMarketMakerPairs[from])
        {
        	uint256 BuyTax = (amount*totalBuyFees)/100;
        	uint256 SellTax = (amount*totalSellFees)/100;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensToAddLiquidityWith = contractTokenBalance.mul(liquiditySellFee).div(totalSellFees).div(2);
        ...
        uint256 bnbToAddLiquidityWith = deltaBalance.mul(liquiditySellFee).div(totalSellFees).div(2);
        
        // add liquidity to uniswap
        addLiquidity(tokensToAddLiquidityWith, bnbToAddLiquidityWith);
        
        // worthy marketing fee
        uint256 marketingAmount = deltaBalance.mul(marketingSellFee).div(totalSellFees);
        if (marketingAmount > 0) {
    	    marketingWallet.transfer(marketingAmount);
	    }
        
        // worthy development fee
        uint256 developmentAmount = deltaBalance.mul(developmentSellFee).div(totalSellFees);
        if (developmentAmount > 0) {
    	    developmentWallet1.transfer(developmentAmount.div(2));
	    }

        if (developmentAmount > 0) {
    	    developmentWallet2.transfer(developmentAmount.div(2));
	    }


        

        uint256 dividends = deltaBalance.mul(BNBRewardsSellFee).div(totalSellFees);
    ...
    function setdevelopmentWallet(address payable wallet1, address payable wallet2) public onlyOwner {
        developmentWallet1 = wallet1;
        developmentWallet2 = wallet2;
    }
