...
contract NFTStakingPool is INFTStakingPool, Ownable, IERC721Receiver {
	...
	function claimVNS() public needPoolOpen {
		...
		if (user.ids.length > 0) {
			(uint256 pendingVNS, bool includDividendVns) = pendingVNSWithEdit(msg.sender);
			uint256 halfPending = pendingVNS / 2;
			uint256 otherHalf = pendingVNS - halfPending;
			if (pendingVNS > 0) {
				rewardVNSTransfer(msg.sender, halfPending);
				user.totalRewardVNS += pendingVNS;
				totalRewardVNS += pendingVNS;
				...
				user.lastVNSClaimTime = block.timestamp;
				if (includDividendVns) user.lastDividendClaimTime = block.timestamp;
			}
		}
	}
	...
	function claimCPD() public needPoolOpen {
		...
		if (user.ids.length > 0) {
			uint256 pending = pendingCPD(msg.sender);
			if (pending > 0) {
	...
	function pendingVNSWithEdit(address _user) internal returns (uint256 pendingVNS, bool includDividendVns) {
	...
	function pendingVNS(address _user) public view returns (uint256 pendingVNS) {
		NftStakedInfo storage user = stakers[_user];
		...
		if (user.ids.length > 0) {
			...
			//dividend vns
			if (block.timestamp > user.lastDividendClaimTime + dividendClaimColdTime && totalStaked != 0)
				pendingVNS += (user.ids.length * curDividendBalance) / totalStaked;
		}
	}
