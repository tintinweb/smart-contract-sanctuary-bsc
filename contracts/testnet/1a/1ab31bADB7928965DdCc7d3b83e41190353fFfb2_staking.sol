...
contract staking is  Context, IBEP20, IBEP20Metadata {
 ...
 //uint256 public REWARD_RATE = 13333333334e16; //                  1333333.33333
// uint256 public REWARD_RATE = 2222222223e16; // 80000000/(60*6) = 222222.222222
   uint256 public REWARD_RATE =  555555556e16;//  80000000/(60*24)=  55555.555
 // uint256 public REWARD_RATE =    444444444e16; // 80000000/(60*30) = 44444.4444444
  ...
  // uint256 private rewardEndtime = block.timestamp+62208000; //720 days
    ...
    function lockandmint(uint amount) public {
        ...
        if(rewardStartFlag==false) {
            ...
            rewardEndtime = block.timestamp + 60*24 ;   // 60  is after 60 second reward generation off.
        }
...
function amountToHarvest (address account) public view returns (uint256) {
      ...
      // if ((balance >= 1 ) && (block.timestamp <= rewardEndtime)) {
           if ((balance >= 1 ) && (block.timestamp <= rewardEndtime)) {
            ...
            amount = ((lap / _cap)* wrapped_balances[account])/1e32;
        }   
      ...
      //  else amount =0; 
            ...
            amount = ((laps / _cap)* wrapped_balances[account])/1e32;
         } 
