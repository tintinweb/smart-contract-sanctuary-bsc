...
contract HeroNFT is BaseNFT {
  ...
  IMarketPlace public marketPlace; // TODO remove on mainnet upgrading
  ...
  function _beforeTokenTransfer(
    ...
    if (_from == address(0)) {
      totalSupply++;
    } else if (!exclusives[_from] && !reachMaxProfit(_tokenId)) {
  ...
  function increaseTotalProfited(uint _tokenId, uint _amount) onlyMintAdmin external returns (uint) {
    ...
    (uint fotaTotalProfited, uint fotaProfitAble) = _increaseTotalFOTAProfited(_tokenId, _convertUsdToFota(_amount), maxFOTAProfitable);
    _amount = _convertFotaToUsd(fotaProfitAble);
  ...
  function _increaseTotalFOTAProfited(uint _tokenId, uint _fotaProfitable, uint _maxFOTAProfitable) private returns (uint, uint) {
    if (heroTotalProfitedInFota[_tokenId] + _fotaProfitable >= _maxFOTAProfitable) {
      _fotaProfitable = _maxFOTAProfitable - heroTotalProfitedInFota[_tokenId];
    }

    heroTotalProfitedInFota[_tokenId] += _fotaProfitable;
    return (heroTotalProfitedInFota[_tokenId], _fotaProfitable);
  }
