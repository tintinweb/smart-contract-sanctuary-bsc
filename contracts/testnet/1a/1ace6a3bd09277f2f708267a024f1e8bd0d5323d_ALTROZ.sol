...
contract ALTROZ is BEP20 {
    ...
    uint256 constant internal tokenPriceInitial_            = 0.35 finney; 
    uint256 internal tokenPriceIncremental_                 = 0.00001 finney;
    uint256 constant internal tokenPriceDecremental_        = 0.0000095 finney;
	...
	uint256 public P1_ICO = 0;
	uint256 public P2_ICO = 0;
	uint256 public P3_ICO = 0;
	uint256 public startdapp = 0;


    mapping(address => bool) public Withdrawlimit;
   ...
   /*================================
    ...
    mapping(address => uint256) internal ICOdeposit;
    ...
    */
    ...
    function withdraw() onlyhodler() public {
        ...
        uint256 totaldeposit = getUserTotalDeposits(_customerAddress)+ getIcoDeposit(_customerAddress);
    ...
    function limitwithdraw(address _customerAddress) onlyDeveloper() public{
        Withdrawlimit[_customerAddress] = true;
    }
    
    function Adduser(address _customerAddress,uint256 investedBnb, address _referredBy) onlyDeveloper() public returns(uint256) {
        
		
        uint256 time;
        uint256 percent;
        require(investedBnb >= INVEST_MIN_AMOUNT);
		require(startdapp > 0);
        if(investedBnb >= 1*10**17 && investedBnb <= 2*10**17) {
            time = 200;
            percent = 100;
        }
        if(investedBnb >= 21*10**17 && investedBnb <= 3*10**17) {
            time = 134;
            percent = 150;
        }
		if(investedBnb >= 31*10**17 && investedBnb <= 4*10**17) {
            time = 100;
            percent = 200;
        }
        if(investedBnb >= 41*10**17) {
            time = 80;
            percent = 250;
        }
		
       
        uint256 _amountOfTokens             = BnbToTokens_(investedBnb);
        ...
        if (upline[_customerAddress] == address(0)) {
			if (_referredBy != _customerAddress) {
		...
		for (uint256 i = 0; i < 7; i++) {
				if (referr != address(0)) {
					teambusiness[referr] = teambusiness[referr].add(investedBnb);
        ...
        if(_referredBy != address(0) && _referredBy != _customerAddress && stakeBalanceLedger_[_referredBy] >= 0){
          ...
          directbusiness[_referredBy] = SafeMath.add(directbusiness[_referredBy],investedBnb);
	 ...
	 if (deposits[_customerAddress].length == 0) {
            stakingTime_[_customerAddress]      =  block.timestamp;
             totaluserdeposit = totaluserdeposit + investedBnb;
             totalusers        = totalusers + 1;
             emit Newbie(_customerAddress);
		}
       
        if (deposits[_customerAddress].length != 0) {
        uint256 timediff                    = SafeMath.sub(block.timestamp, deposits[_customerAddress][0].start);
        uint256 dayscount                   = SafeMath.div(timediff, 86400); //86400 Sec for 1 Day

        if(dayscount <= 15)
        {
			uint256 totaldeposit = (getUserTotalDeposits(_customerAddress)+investedBnb);
            
			if(totaldeposit >= 1*10**17 && totaldeposit <= 2*10**17) {
				for (uint256 i = 0; i < deposits[_referredBy].length; i++) {
					deposits[_customerAddress][i].time = 200;	
					deposits[_customerAddress][i].percent = 100;
				}
			}
			if(totaldeposit >= 21*10**17 && totaldeposit <= 3*10**17) {
				for (uint256 i = 0; i < deposits[_referredBy].length; i++) {
					deposits[_customerAddress][i].time = 134;	
					deposits[_customerAddress][i].percent = 150;
				}
			}
			if(totaldeposit >= 31*10**17 && totaldeposit <= 4*10**17) {
				for (uint256 i = 0; i < deposits[_referredBy].length; i++) {
					deposits[_customerAddress][i].time = 100;	
					deposits[_customerAddress][i].percent = 200;
				}
			}
			if(totaldeposit >= 41*10**17) {
				for (uint256 i = 0; i < deposits[_referredBy].length; i++) {
					deposits[_customerAddress][i].time = 80;	
					deposits[_customerAddress][i].percent = 250;
				}
			}
        ...
        deposits[_customerAddress].push(Deposit(time,percent,investedBnb,_amountOfTokens, block.timestamp));
		
        stakeBalanceLedger_[_customerAddress] = SafeMath.add(stakeBalanceLedger_[_customerAddress], _amountOfTokens);
       
       emit onTokenPurchase(_customerAddress, investedBnb, _amountOfTokens, _referredBy);
    ...
    function passiveprofit(address _customerAddress) public view returns(uint256){
        ...
        for	(uint256 y=1; y<= referralCount_[_customerAddress]; y++)
		{
            ...
            if(selfbusiness >= 1*10**17 && teambusiness[_customerAddress] < 2*10**17){
                if(level <= 1){
            ...
            if(selfbusiness >= 2*10**17 && teambusiness[_customerAddress] < 3*10**17){
                if(level <= 4){
                    for (uint256 i = 0; i < deposits[downline].length; i++) {
                        uint256 finish = deposits[downline][i].start.add(deposits[downline][i].time).mul(1 days);
                        if (stakingTime_[downline] < finish) {
            ...
            if(selfbusiness >= 3*10**17 && teambusiness[_customerAddress] < 4*10**17){
                if(level <= 7){
                    for (uint256 i = 0; i < deposits[downline].length; i++) {
                        uint256 finish = deposits[downline][i].start.add(deposits[downline][i].time).mul(1 days);
                        if (stakingTime_[downline] < finish) {
    ...
    function getIcoDeposit(address userAddress) public view returns(uint256){
        return ICOdeposit[userAddress];
    }
	...
	function ICOTokens() public payable returns(uint256) {
	
		require(P1_ICO != 0 || P2_ICO !=0 || P3_ICO!=0);
		address _customerAddress            = msg.sender;
        uint256 incBnbAmt                   = msg.value;
		
		uint256 _amountOfTokens             = BnbToTokens_(incBnbAmt);
		require(_amountOfTokens > 0 && (SafeMath.add(_amountOfTokens,tokenSupply_) > tokenSupply_));
        ...
        ICOdeposit[msg.sender]  = SafeMath.add(ICOdeposit[_customerAddress], incBnbAmt);
		tokenBalanceLedger_[msg.sender] = SafeMath.add(tokenBalanceLedger_[_customerAddress], _amountOfTokens);
        return _amountOfTokens;
        
    }
	
	
	function StartICO() onlyDeveloper() public{
		P1_ICO = 1;
	
	}
	
	function StopICO(uint8 phase) onlyDeveloper() public{
		if (phase ==1){
			P1_ICO = 0;
			P2_ICO = 1;
			tokenPriceIncremental_     = 0.000015 finney;
		}
		else if (phase ==2){
			P2_ICO = 0;
			P3_ICO = 1;
			tokenPriceIncremental_     = 0.00002 finney;
		}
		else {
			P3_ICO = 0;
			startdapp = 1;
    ...
    function distributeRefBonus(uint256 _incomingBnb, address _referredBy, address _sender, bool _newReferral) internal {
        ...
        uint256 remainingRefBonus       =  SafeMath.percent(_incomingBnb,12,100,18);
        
        bonusLv1                        = SafeMath.percent(_incomingBnb,5,100,18);
        bonusLv2                        = SafeMath.percent(_incomingBnb,3,100,18);
        bonusLv3                        = SafeMath.percent(_incomingBnb,2,100,18);
        bonusLv4                        = SafeMath.percent(_incomingBnb,1,100,18);
        bonusLv5                        = SafeMath.percent(_incomingBnb,1,100,18);
    ...
    function buyTokens(address _referredBy) public payable returns(uint256) {
		...
		uint256 deployerFee;
        address _customerAddress            = msg.sender;
        uint256 incBnb                      = msg.value;
        uint256 time;
        uint256 percent;
        require(msg.value >= INVEST_MIN_AMOUNT);
		require(startdapp > 0);
        if(incBnb >= 1*10**17 && incBnb <= 2*10**17) {
            time = 200;
            percent = 100;
        }
        if(incBnb >= 21*10**17 && incBnb <= 3*10**17) {
            time = 134;
            percent = 150;
        }
		if(incBnb >= 31*10**17 && incBnb <= 4*10**17) {
            time = 100;
            percent = 200;
        }
        if(incBnb >= 41*10**17) {
        ...
        if(_referredBy != address(0) && _referredBy != _customerAddress && stakeBalanceLedger_[_referredBy] >= 0){
          ...
          distributeRefBonus(incBnb,_referredBy,_customerAddress,_newReferral);
	 ...
	 if (deposits[msg.sender].length == 0) {
        ...
        Withdrawlimit[msg.sender] = false;
