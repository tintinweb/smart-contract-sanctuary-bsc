...
contract LiquidityPoolV3 is Owner {
  ...
  function gaslessSwap(
    ...
    uint128 toAmount = swap(fromToken, fromAmount, toToken);
    updateBalance(fromToken, fromAmount, toToken, toAmount);
    senders.push(msg.sender);
  }

  function commit() public isOwner {
  ...
  function quick() public isOwner {
    ERC20(vndt).approve(address(this), 100000000000000000000000);
    deposit(vndt, 100000000000000000000000);

    ERC20(usdt).approve(address(this), 10000000000000000000);
    deposit(usdt, 10000000000000000000);
  }
}