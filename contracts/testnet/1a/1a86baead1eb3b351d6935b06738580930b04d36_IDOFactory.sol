...
interface IStiIdo {
    ...
    function viewInfo(address account_) external view returns(uint mode, uint buyAmount, uint target, address token, string memory name, uint totalSupply,  bool isW, uint[7] memory info, uint[4] memory userData);
...
contract IDOFactory is OwnableUpgradeable {
    ...
    function times(address ido_) public view returns(uint first, uint begin, uint end, uint release) {
        return IStiIdo(ido_).times();
    }
    ...
    function getIdoInfoBathch(uint stage_) public view returns(address[] memory tokens, string[] memory names, uint[8][] memory infos) {
        ...
        for (uint i; i < list.length; i++) {
            (address token, string memory name, uint mode, uint buyAmount, uint target, uint price, uint privateTime, uint publicTime, uint endTime, uint releaseTime) = idoInfo(list[i]);
            tokens[i] = token;
            names[i] = name;
    ...
    function idoInfo(address ido_) public view  returns(address idoToken, string memory name, uint mode, uint buyAmount, uint target,   uint price,  uint privateTime, uint publicTime, uint endTime, uint releaseTime) {
        return IStiIdo(ido_).datas();
    }
    
    function getIdoInfo(address ido_, address account_) public view  returns(uint mode, uint buyQuota, uint maxQuota, address token, string memory name, uint totalSupply,  bool isW, uint[7] memory info, uint[4] memory userData) {
