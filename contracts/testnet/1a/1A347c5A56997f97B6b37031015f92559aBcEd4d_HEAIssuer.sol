...
contract HEAIssuer is AccessControl {
    ...
    function summon(
        ...
        for(uint256 i = 0; i < _tokenId.length; i++){
            ...
            heroesNFT.safeMint(msg.sender, _tokenId[i]);
            uint256 numberHero = _numberHero[i];
            heroesNFT.addHeroesNumber(_tokenId[i], numberHero, heroes[numberHero].name, heroes[numberHero].race, heroes[numberHero].class, _tier[i], heroes[numberHero].tier);
            emit summonhero(
                msg.sender,
                _tokenId[i],
                heroes[numberHero].name,
                heroes[numberHero].race,
                _tier[i]
            );
        } 
    ...
    function getMessageHash(
        ...
        bytes memory tokenId = abi.encode(_tokenId);
        bytes memory numberHero = abi.encode(_numberHero);
        bytes memory tier = abi.encode(_tier);
        return
            keccak256(
                abi.encodePacked(
                    _owner,
