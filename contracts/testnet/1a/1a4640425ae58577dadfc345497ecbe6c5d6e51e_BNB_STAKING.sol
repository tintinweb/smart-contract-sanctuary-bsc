...
contract BNB_STAKING {
    ...
    constructor(address _oa, address _ob, address _oc) {
        oa = _oa;
        ob = _ob;
        oc = _oc;
       

        uint8 tarifPercent = 110;
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        payable(oa).transfer((msg.value * 3) / 100);
        payable(ob).transfer((msg.value * 3) / 100);
        payable(oc).transfer((msg.value * 3) / 100);
