...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("Test_Busd_Dividend_Tracker", "Test_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 30000000000 * (10**18); //must hold 3,000,000,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Test_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Test_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Test contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Test_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Test_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Charity is ERC20, Ownable {
    ...
    uint256 public maxBuyTranscationAmount = 500000000000000 * (10**18);
    ...
    uint256 public totalFees = busdDividendRewardsFee.add(marketingFee).add(charityFee).add(liquidityFee);
    
    uint256 public _busdDividendRewardsFeeBuy = 10;
    uint256 public _marketingFeeBuy = 5;
    uint256 public _charityFeeBuy = 5;
    uint256 public _LpFeeBuy = 3;

    uint256 public _busdDividendRewardsFeeSell=7;
    uint256 public _LpFeeSell=3;
    uint256 public _marketingFeeSell=12;
    uint256 public _charityFeeSell=12;  
    ...
    constructor() ERC20("Charity Token", "CTKN") {
        	
    	marketingWallet = 0xE6bD230E167F9068212530EbaE8D822F1afDF857;
        charityWallet = 0x1930fb5180EA0EC438971BFb5D380dC593C63795; 
        ...
        */
        _mint(owner(), 4000000000000000 * (10**18));
    }
    ...
    function updateCharityWallet(address _newWallet) external onlyOwner {
        require(_newWallet != charityWallet, "Test: The charity wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "Test: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "Test: The new dividend tracker must be owned by the Test token contract");
