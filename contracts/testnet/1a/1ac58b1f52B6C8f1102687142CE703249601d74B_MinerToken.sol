...
contract MinerToken is Ownable {
    ...
    constructor(address _dct, address _usdt) {
        ...
        _products[1] = Item(100, 100);
        _products[2] = Item(1000, 1000);
    }
    ...
    function deposit(uint256 option, uint256 amount) external {
        ...
        uint256 usdtAmount = _products[option].usdtAmount.mul(amount);
    ...
    // 新增产品
    function setAddProduct(uint256 option, uint256 dctAmount, uint256 usdtAmount) external onlyOwner {
        require(_products[option].dctAmount == 0, "Product serial number does exist");
        _products[option] = Item(dctAmount, usdtAmount);
    }
