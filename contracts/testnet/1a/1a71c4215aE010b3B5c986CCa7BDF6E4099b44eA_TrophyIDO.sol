...
contract TrophyIDO is Ownable {
    ...
    event Purchased(address indexed purchaser);
    event Claimed(address indexed purchaser, uint256 claimAmount);
    ...
    function calcCurrentRate() public view returns (uint256) {
        return currentCap.mul(RATE_PRECISION_FACTOR).div(tokenAmount);
    }

    function purchase() external occurring underHardCap {
        currency.transferFrom(msg.sender, address(this), price);

        currentCap = currentCap.add(price);
        purchasers.push(msg.sender);
        purchaseDetails[msg.sender].amount = purchaseDetails[msg.sender]
            .amount
            .add(price);
    ...
    function finalize(address _to) external onlyOwner {
        ...
        // for liquidity
        uint256 tokenLiqAmount = tokenAmount.mul(lpPercent).div(100);
        uint256 currencyLiqAmount = currentCap.mul(lpPercent).div(100);

        token.approve(address(ROUTER), tokenLiqAmount);
        currency.approve(address(ROUTER), currencyLiqAmount);
        ...
        // for project
        currency.transfer(_to, currentCap.sub(currencyLiqAmount));
