...
interface IERC20 {
    ...
    function treasuryOf(address account) external view returns (uint256);
...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    mapping(address => uint) private _fixedtreasuryAmount;
    ...
    function _transfer(
        ...
        _developmentFee = 3*(_tTotal/100000)*100;
        _liquidityFee = (_tTotal/100000)*100;
        CoinageDifficulty = _mintweight/10**18;
    }

    function SetUpAndJoinTreasury(address Mentor) external updateReward(msg.sender) {
        require(menteeStatus[msg.sender] < 1, "Has a mentor");
        require(menteeStatus[Mentor] > 0, "Has a mentor"); 
        uint256 MentorshipFee = (100000*10**18)/_tTotal;
    ...
    modifier updateReward(address _account) {
        ...
        if (block.timestamp < finishAt[_account]) {
            uint256 rewardearned = _treasuryAmount[msg.sender]*_treasuryRateOf[msg.sender]/(100*_treasurydurationOf[msg.sender]) * (block.timestamp - startAt[msg.sender]) - _treasuryWithdrawn[msg.sender];
        ...
        if (block.timestamp > finishAt[_account]) {
            ...
            Ttreasury = Ttreasury - _fixedtreasuryAmount[msg.sender];
        }
    ...
    function stakeTreasury(uint256 _amount) external updateReward(msg.sender) {
        ...
        uint256 Yield = Ttreasury*110/100 * treasuryRATE/100;
        ...
        _fixedtreasuryAmount[msg.sender] =  _fixedtreasuryAmount[msg.sender] + _treasuryRewardsOf[msg.sender] + remainingtreasuryAmount[msg.sender] + _amount*10**18;
        Ttreasury = Ttreasury + _treasuryRewardsOf[msg.sender] + remainingtreasuryAmount[msg.sender] + _amount*10**18;
    ...
    function treasuryOf(address account) public view override returns (uint256) {
        uint256 rewardearned = _treasuryAmount[account]*_treasuryRateOf[account]/(100*_treasurydurationOf[account]) * (block.timestamp - startAt[account]) - _treasuryWithdrawn[account];
    ...
    function withdrawTreasury(uint256 _amount) external updateReward(msg.sender) {
        ...
        if ( block.timestamp < finishAt[msg.sender]) {
            require(_amount*10**18 <= rewardearned);
        ...
        if (block.timestamp >= finishAt[msg.sender]) {
            ...
            Ttreasury = Ttreasury - _fixedtreasuryAmount[msg.sender];
            //reset fixedtreasury
            _fixedtreasuryAmount[msg.sender] = 0;
        }
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = _LiquidityWalletAddress;
