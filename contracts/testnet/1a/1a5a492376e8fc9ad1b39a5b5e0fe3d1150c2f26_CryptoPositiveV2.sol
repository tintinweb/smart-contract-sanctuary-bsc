...
contract CryptoPositiveV2 is IBEP20, UsingFlagsWithStorage, UsingInitializer, UsingUUPS {
    ...
    function initialize(address owner_, address router_) external initializer {
        ...
        _setFlags(msg.sender, _DEPLOYER_FLAG(), 0);
        _setFlags(owner_, _OWNER_FLAG() | _FEE_EXEMPT_FLAG() | _TX_LIMIT_EXEMPT_FLAG() | _MAX_HOLD_EXEMPT_FLAG() | _ADJUST_CONTRACT_VARIABLES_FLAG(), 0);
    ...
    /// @dev warning if this is address(0) it will renounce the contract
    function setOwner(address newOwner) external requires(msg.sender, _OWNER_FLAG(), 0) {
        _setFlags(msg.sender, 0, _OWNER_FLAG() | _FEE_EXEMPT_FLAG() | _TX_LIMIT_EXEMPT_FLAG() | _MAX_HOLD_EXEMPT_FLAG() | _ADJUST_CONTRACT_VARIABLES_FLAG()); // clear old owner permissions
        if (newOwner != address(0)) {
            _setFlags(newOwner,  _OWNER_FLAG() | _FEE_EXEMPT_FLAG() | _TX_LIMIT_EXEMPT_FLAG() | _MAX_HOLD_EXEMPT_FLAG() | _ADJUST_CONTRACT_VARIABLES_FLAG(), 0);
    ...
    function shouldSwapBack() internal view returns (bool) {
        ...
        && !inSwap
        && _getFlags(address(this)).all(_SWAP_ENABLED_FLAG())
    ...
    function setSwapBackSettings(bool _enabled, uint256 _amount) external requires(msg.sender, _ADJUST_CONTRACT_VARIABLES_FLAG(), 0) {
        _setOrClearFlagsIf(address(this), _SWAP_ENABLED_FLAG(), _enabled);
    ...
    function _authorizeUpgrade(address newImplementation) internal override requires(msg.sender, _DEPLOYER_FLAG(), 0) {}
    ...
    function _DEPLOYER_FLAG() internal pure returns (uint256) {
        return _INITIALIZED_FLAG() >> 1;
    }
