...
contract ERC20Deflationary is Context, IERC20, Ownable {
    ...
    address private constant burnAccount = 0xE7628a97fF7fBEf3277f96E19577b157440e5C98;

    address private constant marketingAddress = 0xE7628a97fF7fBEf3277f96E19577b157440e5C98;
    ...
    */
    ...
    bool private _enableTradeFee;
    bool private _tradeFeeEnabled;
    ...
    */
    ...
    event SetTaxTradeFee();
    event EnableTradeFee();
    ...
    function addLiquidity(uint256 ethAmount, uint256 tokenAmount) private {
        ...
        _pancakeRouter.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            marketingAddress, // the LP is sent to burnAccount. 
            block.timestamp + 60 * 1000
        );
    }
    ...
    */
    ...
    function enableTradeFee(uint8 taxTradeFee_, uint8 taxTradeFeeDecimals_) public onlyOwner {
        require(!_tradeFeeEnabled, "Trade fee feature is already enabled.");
        require(taxTradeFee_ > 0, "Tax must be greater than 0.");
        require(taxTradeFeeDecimals_ + 2  <= decimals(), "Tax decimals must be less than token decimals - 2");
        
        _tradeFeeEnabled = true;
        setTaxTradeFee(taxTradeFee_, taxTradeFeeDecimals_);
        
        emit EnableTradeFee();
    }

    function setTaxTradeFee(uint8 taxTradeFee_, uint8 taxTradeFeeDecimals_) public onlyOwner {
        require(taxTradeFee_ > 0, "Tax must be greater than 0.");
        require(taxTradeFeeDecimals_ + 2  <= decimals(), "Tax decimals must be less than token decimals - 2");
        
        taxTradeFee_ = taxTradeFee_;
        taxTradeFeeDecimals_ = taxTradeFeeDecimals_;
    ...
    */
    ...
    function enableAutoSwapAndLiquify(uint8 taxLiquify_, uint8 taxLiquifyDecimals_, uint256 minTokensBeforeSwap_) public onlyOwner {
        ...
        // init Router
        IPancakeRouter02 pancakeRouter02 = IPancakeRouter02(0x10ED43C718714eb63d5aA57B78B54704E256024E);

        if (_pancakePair == address(0)) {
            _pancakePair = IPancakeFactory(pancakeRouter02.factory())
                .createPair(address(this), pancakeRouter02.WETH());
        }
        
        _pancakeRouter = pancakeRouter02;

        // exclude pancakeRouter02 from receiving reward.
        _excludeAccountFromReward(address(pancakeRouter02));
        // exclude WETH and this Token Pair from receiving reward.
        _excludeAccountFromReward(_pancakePair);

        // exclude pancakeRouter02 from paying fees.
        excludeAccountFromFee(address(pancakeRouter02));
        // exclude WETH and this Token Pair from paying fees.
        excludeAccountFromFee(_pancakePair);
...
contract ExampleToken is Context, ERC20Deflationary {
    ...
    string private name_ = "Business Universe";
    string private symbol_ = "BUUN";
    uint8 private decimal_ = 18;
    uint8 private buyFee_ = 10;
    uint8 private sellFee_ = 10;
    uint256 private tokenSupply_ = 10 ** 9;
    ...
    constructor () ERC20Deflationary(name_, symbol_, decimal_, tokenSupply_) {
        ...
        enableTradeFee(buyFee_, sellFee_);
    }
