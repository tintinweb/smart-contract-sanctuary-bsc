...
library Address {
    ...
    function excludeFromFee(uint256 _trnxId) public onlyMultiOwner {
        ...
        executeTransaction(_trnxId, 1); // 2 for excludeFromFee
    }
   ...
   function updateIsBlacklisted(uint256 _trnxId, bool state) external onlyMultiOwner {
    ...
    function bulkIsBlacklisted(address[] memory accounts, bool state,uint _trnxId) external onlyMultiOwner {
 ...
 // ADD NEW TRANSACTION 
    ...
    function newTransaction(uint _methodID, address _data) external onlyMultiOwner returns(uint){
        ...
        require(_methodID<=13 && _methodID>0,"invalid method id");
