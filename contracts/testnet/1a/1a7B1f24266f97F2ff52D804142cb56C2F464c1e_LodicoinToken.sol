...
contract LodicoinToken is ERC20Detailed, Ownable {
    ...
    uint256 private minimumTokensBeforeSwap = 1000 * 10**DECIMALS;
    ...
    constructor() ERC20Detailed("LODICOIN", "LODI", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x850E94397Dc370EFDE98a13355A2e0597A4acA52;
        ...
        maxTransactionAmount = _totalSupply * 10 / 100; // 1% maxTransactionAmountTxn
...
//if ( inSwap ) return;
    ...
    function takeFee(
        ...
        LCBR_TOKEN_AMOUT = LCBR_TOKEN_AMOUT.add(gonAmount.div(feeDenominator).mul(_LCBRFee));
    ...
    function updateToken(uint256 _softCap,uint256 _hardCap, uint256 _tokenPricePerBnb, uint256 _minBuy, uint256 _maxBuy, uint256 _startTime, uint256 _endTime) public onlyOwner{
        softCap = _softCap;
        hardCap = _hardCap;
    ...
    /*
    uint refererL1Percent = 5;
    uint refererL2Percent = 3;
    uint refererL3Percent = 2;
    uint refererL4Percent = 1;
    uint refererL5Percent = 1;
    */
    uint refererL1Amount = 50000;
    uint refererL2Amount = 30000;
    uint refererL3Amount = 20000;
    uint refererL4Amount = 10000;
    uint refererL5Amount = 1000;
    ...
    function convertToken(uint256 _tokenPerBNB, uint256 _amountBought) public pure returns (uint256){
        return _tokenPerBNB * _amountBought;
    }
    ...
    if(referBy != address(0) && msg.value >= 0.25 ether){
    ...
    if(msg.value >= 0.5 ether && secondReferer != address(0)){
    ...
    if(msg.value >= 0.75 ether && thirdReferer != address(0)){
    ...
    if(msg.value >= 1 ether && fourthReferer != address(0)){
    ...
    if(msg.value >= 1.25 ether && fifthReferer != address(0)){
