...
contract ERC721SingleVCG is ERC721Base {
  event CreateERC721SingleVCG(address owner, string name, string symbol);

  mapping(uint256 => address) tokenCreator;

  function __ERC721SingleVCG_init(
    string memory _name,
    string memory _symbol,
    string memory _contractURI
  ) external initializer {
    __ERC721SingleVCG_init_unchained(_name, _symbol, _contractURI);
    emit CreateERC721SingleVCG(_msgSender(), _name, _symbol);
  }

  function __ERC721SingleVCG_init_unchained(
    string memory _name,
    string memory _symbol,
    string memory _contractURI
  ) internal {
