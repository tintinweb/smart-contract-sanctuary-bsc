...
contract NFTAuction {
    ...
    event Rented(address rentAddress, uint tokenId);
    ...
    **/
    ...
    function putOnRent(uint _time, uint _price, uint tokenId, address nftContractAddress) external {
    ...
    function removeFromRent(uint _tokenId, address nftContractAddress) external {
        require(rents[_tokenId].owner == msg.sender, "You are not the owner");
        require(rents[_tokenId].rentEndTime + rents[_tokenId].rentedAt < block.timestamp, "Already on rent by someone");
    ...
    function rent(uint _tokenId, address nftContractAddress) external payable {
        uint256 rentAmount = rents[_tokenId].rentPrice;
        require(msg.value >= rentAmount, "Price higher than amount sent");
        require(rents[_tokenId].rentEndTime + rents[_tokenId].rentedAt < block.timestamp, "Already on rent");
