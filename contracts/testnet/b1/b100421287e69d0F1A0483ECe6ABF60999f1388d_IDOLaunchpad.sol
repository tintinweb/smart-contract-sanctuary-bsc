...
contract IDOLaunchpad is Ownable, Pausable {
    ...
    function editStartTime(uint256 newsaleStart) public onlyOwner {
        require(block.timestamp < saleStart, "Sale already started");
        saleStart = newsaleStart;
    }

    function editEndTime(uint256 newSaleEnd) public onlyOwner {
        require(
    ...
    function editUsers(address[] memory _users, uint256[] memory _tiers)
        external
        onlyOwner
    {
