...
contract MileToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    // Multisig Protocol Wallets

    address payable public feeAddress = payable(0x2d3bacB4c2cC248F8b409EA75303618C556aa069); 
    address payable public usdtAddress = payable(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684);
    ...
    string private constant _name = "MILE";
    string private constant _symbol = "MILE";
    ...
    uint256 private minimumTokensBeforeSwap = 800 * 10**18; 
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) 
        {
            if (to == uniswapV2Pair) 
            {
                if (overMinimumTokenBalance) 
                {
                    ...
                    uint256 feeToken = contractTokenBalance.mul(_sellMarketingFee).div(_sellMarketingFee);
                    // Token Swapping in Token for usdt
                    SwapAndLiquifytoUsdt(feeToken); 
                }  
            }
        }
    ...
    {
        _tempAllSwapableFees = _buyMarketingFee;
    }

    function setSellFee() private
    {
        _tempAllSwapableFees = _sellMarketingFee;
    }
    ...
    function swapTokensForUSDT(uint256 tokenAmount) private {
        ...
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            ...
            path,
            address(_usdtReceiver), // The contract
            block.timestamp
        );
    ...
    function transferToAddressUSDT(address recipient, uint256 amount) private {
        IERC20(usdtAddress).transferFrom(address(_usdtReceiver),recipient, amount);
    }
    ...
    function manualSwapAndLiquifyBNBtoUsdt(uint256 bnbLiquifyAmount) external lockTheSwap onlyOwner {
        SwapAndLiquifytoUsdt(bnbLiquifyAmount);
    }
    ...
    function SwapAndLiquifytoUsdt(uint256 bnbLiquifyAmount) private lockTheSwap {
        // swap BNB for Tokens
        swapTokens(bnbLiquifyAmount); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered
        // how much Tokens did we just swap into?
        uint256 newTokenBalance = IERC20(usdtAddress).balanceOf(address(_usdtReceiver));

        uint256 marketingFunds = newTokenBalance.mul(_sellMarketingFee).div(_sellMarketingFee);

        transferToAddressUSDT(feeAddress,marketingFunds);
    ...
    function addLiquidityToToken(uint256 tokenLiquifyAmount) private lockTheSwap{
        ...
        // swap tokens for BNB
        swapTokens(half); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered
