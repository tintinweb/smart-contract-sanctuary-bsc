...
import "./interfaces/IBeastSwapLottery.sol";

/** @title BeastSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract BeastSwapLottery is ReentrancyGuard, IBeastSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInBeast = 50 ether;
    uint256 public minPriceTicketInBeast = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInBeast;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] beastPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInBeast;
        uint32 finalNumber;
    }
     ...
     * @param _beastTokenAddress: address of the BEAST token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _beastTokenAddress, address _randomGeneratorAddress) {
        beastToken = IERC20(_beastTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of BEAST to this contract
        uint256 amountBeastToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInBeast,
            _ticketNumbers.length
        );

        // Transfer beast tokens to this contract
        beastToken.safeTransferFrom(address(msg.sender), address(this), amountBeastToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInBeast += amountBeastToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInBeastToTransfer
        uint256 rewardInBeastToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInBeastToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        beastToken.safeTransfer(msg.sender, rewardInBeastToTransfer);

        emit TicketsClaim(msg.sender, rewardInBeastToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in BEAST per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInBeast) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in BEAST for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].beastPerBracket[j] =
                ...
                // A. No BEAST to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].beastPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInBeast - amountToShareToWinners);

        // Transfer BEAST to treasury address
        beastToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in BEAST token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        beastToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInBeast += _amount;
     ...
     * @param _priceTicketInBeast: price of a ticket in BEAST
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInBeast >= minPriceTicketInBeast) && (_priceTicketInBeast <= maxPriceTicketInBeast),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInBeast: _priceTicketInBeast,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            beastPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInBeast: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(beastToken), "Cannot be BEAST token");
    ...
    /**
     * @notice Set BEAST price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInBeast: minimum price of a ticket in BEAST
     * @param _maxPriceTicketInBeast: maximum price of a ticket in BEAST
     */
    function setMinAndMaxTicketPriceInBeast(uint256 _minPriceTicketInBeast, uint256 _maxPriceTicketInBeast)
        external
        onlyOwner
    {
        require(_minPriceTicketInBeast <= _maxPriceTicketInBeast, "minPrice must be < maxPrice");

        minPriceTicketInBeast = _minPriceTicketInBeast;
        maxPriceTicketInBeast = _maxPriceTicketInBeast;
    }
     ...
     * @param _priceTicket price of a ticket (in BEAST)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].beastPerBracket[_bracket];
        } else {
            return 0;
        }
    }
