...
contract ProxyContract is OwnableUpgradeable {
    ...
    mapping (uint256 => bool) public isMovableMapping;
    // Slot number for mapping is hash of "Budaghyan"
    uint256 public constant MAPPING_SLOT_NUMBER = 102;
    ...
    validSlot(_slotNumber, _lastSlotNumber)
    {
        while(_slotNumber != _lastSlotNumber) {
            _swapValues(_slotNumber, _slotNumber + 1);
            _slotNumber++;
        }
        _resetSlot(_lastSlotNumber);
    }

    function updateSlotMovability(uint256 _slotNumber, bool _isMovable) external {
        _updateSlot(_slotNumber, _isMovable);
    }
    ...
    function isMovable(uint256 _slotNumber) external view returns(bool) {
        return _movable(_slotNumber);
    }
    ...
    function _updateSlot(uint256 _slotNumber, bool _isMovable) private {
        uint256 realSlot = _firstSlot() + _slotNumber;
        bytes32 slot = _getSlotInMapping(realSlot);
        assembly{
            sstore(slot, _isMovable)
        }
    }
    ...
    function _movable(uint256 _slotNumber) private view returns(bool movable) {
        uint256 realSlot = _firstSlot() + _slotNumber;
        bytes32 slot = _getSlotInMapping(realSlot);
        assembly {
            movable := sload(slot)
