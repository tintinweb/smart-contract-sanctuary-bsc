...
contract BNBRagnarokOracle is AccessControl, IPriceOracle {
    ...
    constructor(address _tokenAddress) {
        ...
        //mainnet characterPriceInBNB = 20 * 10 ** 16; //0.2BNB
        characterPriceInBNB = 20 * 10 ** 16; //testnet
        upgradesPriceInBNB = [0, 75 * 10 ** 15, 75 * 10 ** 15, 125 * 10 ** 15, 250 * 10 ** 15, 250 * 10 ** 15]; //0.075
        
        ZENYprice = 3750 * 10 ** 5; //37500 ZENYBNB
        basePriceToUnlockInBNB = 8 * 10 ** 15; //0.008 BNB
        unlockRate = 4;
        isStarted = false;
    ...
    function getZenyBNBPrice() internal view returns(uint256) {
    ...
    function _getCharacterPrice() internal view returns (uint256) {
        if (isStarted) {
            return characterPriceInBNB * ZENYprice / (10 ** 18);
        }
        else
            return 500 * 10 ** 5;
    }
    ...
    function getUnlockLevelPrice(uint256 level) external override view returns (uint256) {
        return ((basePriceToUnlockInBNB + (basePriceToUnlockInBNB * unlockRate * level) / 100) * ZENYprice) / (10 ** 18);
    }

    function getWarpPrice() external override view returns (uint256) {
        return _getCharacterPrice() / 10;
    }

    function getVipUpgradePrice(uint256 level) external override view returns (uint256) {
        require(level < upgradesPriceInBNB.length, "Invalid Vip Level");
        return upgradesPriceInBNB[level] * ZENYprice / (10 ** 18);
    }

    function getVipUpgradePrices() external view returns (uint256[] memory) {
        uint256[] memory prices = new uint256[](upgradesPriceInBNB.length);
        for(uint i = 0; i < upgradesPriceInBNB.length; i++){
            prices[i] = upgradesPriceInBNB[i] * ZENYprice / (10 ** 18);
        }
        return prices;
    }

    function fetchZENYPrice() external {
        require(hasRole(PRICE_UPDATER, msg.sender), "Sender cannot update price");
        ZENYprice = getZenyBNBPrice();
        if (!isStarted &&  (5000 * 10 ** 18) >= characterPriceInBNB * ZENYprice / (10 ** 18) ) {
            isStarted = true;
        }
    }
