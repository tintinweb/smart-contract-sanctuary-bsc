...
contract WarcraftUniverseNFT is ERC721URIStorage, Ownable {
    ...
    function setBMV(address BMVAddress) public onlyOwner {
        BMV = ERC20(BMVAddress);
    }
...
contract WarcraftUniverseController is Ownable {
    ...
    uint256[4] private poolMonths = [1,3,6,12];

    bool[4] public poolClose = [false,false,false,false];

    uint256[5][4] public poolRate;
    ...
    uint256[3] public boxPrices = [1500,4500,7500];
    ...
    constructor() {
        ...
        BMV = ERC20(0x864AF7b3551935341Eeb92F1DC25af003E1DB0Ca);

        SHE = ERC20(0x32CD801ef1dA2b3B278e1A57822aB6871C29F69D);

        WUN = WarcraftUniverseNFT(0xF0431e86DDF76995147076790d41e05E98A06176);
        
        monsters[1] = Monster(1,72,126,12,15,6,1,1);
        monsters[2] = Monster(2,52,138,30,18,7,3,1);
        monsters[3] = Monster(3,43,168,40,23,8,5,2);
        monsters[4] = Monster(4,25,208,50,28,11,8,3);
        monsters[5] = Monster(5,11,258,130,35,13,12,5);
        ...
        poolRate[0] = [20,30,40,50,200];
        poolRate[1] = [25,35,45,55,300];
        poolRate[2] = [30,40,50,60,400];
        poolRate[3] = [40,45,55,65,500];
    ...
    function ownerDeposit(uint256 amount) public onlyOwner {
        ...
        uint256 dexBalance = SHE.balanceOf(msg.sender);
        
        require(amount > 0, "You need to send some token");
        
        require(amount <= dexBalance, "Dont hava enough token");
        
        SHE.transferFrom(msg.sender, address(this), amount);
    }
    ...
    function setSHE(address SHEAddress) public onlyOwner {
        SHE = ERC20(SHEAddress);
    }

    function setWUN(address WUNAddress) public onlyOwner {
        WUN = WarcraftUniverseNFT(WUNAddress);      
    }

    function setMonsterReward(uint256 index, uint256 rewardLow, uint256 rewardUp) public onlyOwner { 
        monsters[index].basePrice = rewardLow;
        monsters[index].stepPrice = rewardUp - rewardLow;
    }
    ...
    function buyHeroCard(uint256 sellId) public {
        ...
        SHE.transferFrom(msg.sender,ownerAddress,price);
    ...
    function inPool(uint256 poolType, uint256 heroId) public {
        ...
        userPools[msg.sender][users[msg.sender].poolIds] = Pool(
            ...
            heroId,
            amount,
            amount.mul(poolRate[poolType - 1][userHero.id - 1]).mul(poolMonths[poolType - 1]).div(100),
    ...
    function outPool(uint256 poolId) public {
        ...
        if(userPoolRewardTime[msg.sender][myPool.id] < time){
            ...
            uint256 reward = myPool.amount.mul(rewardTime * 100 / 24 hours).mul(myPool.rate).div(myPool.circleTime.div(24 hours) * 10000);
            ...
            SHE.transfer(msg.sender , reward + myPool.amount);
                
            userPoolRewardTotal[msg.sender][myPool.id] += reward;
        }
    ...
    function userWithdrewReward (uint256 poolType) public {
        ...
        SHE.transfer(msg.sender , poolReward);
        ...
        for(uint256 i = 1; i <= users[user].poolIds; i ++ ){
            ...
            if(myPool.id == poolType && myPool.amount > 0){
                ...
                if(time > startTime){
                    ...
                    uint256 _reward = myPool.amount.mul(time.sub(startTime) * 100 / 24 hours).mul(myPool.rate).div(myPool.circleTime.div(24 hours) * 10000);
    ...
    function NFTsToHeros() public {
        ...
        for(uint256 i = 1; i <= WUN.userNFTNumber(msg.sender); i ++){
            ...
            bool suc = _NFTToHero(tokenId);

            if(suc){
                count ++;
            }
        }
       emit NFTToHeros(msg.sender, count);
    }

    function _NFTToHero(uint256 tokenId) private returns (bool) {
    ...
    function selectBox(uint256 boxId) public {
        ...
        if(1 == boxId && box1MaxTimes > 0){
            ...
            BMV.transferFrom(msg.sender,deadWallet,boxPrices[boxId - 1] * 10 **6);
            
            todayBurn += boxPrices[boxId - 1];
            totalBurn += boxPrices[boxId - 1];
            todayBox1Sales += 1;
            totalBox1Sales += 1;
            todayBoxSales += 1;
            totalBoxSales += 1;
        ...
        if(2 == boxId && box2MaxTimes > 0){
            ...
            emit SelectBox(msg.sender,boxId,boxPrices[boxId - 1],3);
        ...
        if(3 == boxId && box3MaxTimes > 0){
            ...
            emit SelectBox(msg.sender,boxId,boxPrices[boxId - 1],5);
    ...
    function selectMonster(uint256 monsterID,uint256 heroId) public {
        ...
        if(playResult){
            ...
            profit =  userSelectMonster.basePrice + _getMonsteRadom(userSelectMonster.stepPrice + 1);
        ...
        SHE.transfer(msg.sender, profit);
...
contract WarBet is Ownable {
    ...
    address public WundWallet = 0x336080B83df5Bb6E9e20CfB0048Ea918a8C89458;
    ...
    function selectMonster() public {
                
        require(isOpen == true,"Copy limit");

        SHE.transferFrom(msg.sender, address(WundWallet), amount* 10 **3);
    ...
    function setOpen(bool num) public onlyOwner {      
        isOpen = num;       
    }
}