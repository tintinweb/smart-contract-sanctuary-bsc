...
contract HBTLending is Ownable {
    ...
    uint256 public interestFeeDays = ONE_DAY * 21;
    ...
    function takeLoan() external {
        ...
        loans[msg.sender] = Loan({
            ...
            interestFeeDays: interestFeeDays,
    ...
    function getUserInterest(address user) public view returns (uint256) {
        ...
        Loan storage loan = loans[user];
        bool isInterestFee = (loan.loanTime + loan.interestFeeDays) >
            block.timestamp;
        if (loan.amount > 0 && !isInterestFee) {
            uint256 interestAnnually = (loan.amount * loan.interestRate) / 1000;
            uint256 interestPerDay = interestAnnually / 365;
            uint256 daysPassed = ((block.timestamp -
                loan.loanTime -
                loan.interestFeeDays) / ONE_DAY);
            uint256 totalInterestSoFar = interestPerDay * daysPassed;
            interestAmount = totalInterestSoFar;
        }
    ...
    function repayLoan() external {
        ...
        if (interestAmount > 0) {
            stakingContract.chargeInterest(msg.sender, interestAmount);
        }
