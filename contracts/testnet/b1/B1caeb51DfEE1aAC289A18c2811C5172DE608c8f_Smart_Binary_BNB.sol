...
contract Smart_Binary_BNB is Context {
    ...
    constructor() {
        ...
        _listingNetwork = 1 ether;
        _lotteryNetwork = 25 * 10**18;
        Max_Point = 50;
        Max_Lottery_Price = 0.25 ether;
    ...
    function Reward_24() public {
        ...
        All_Payment += address(this).balance;
        ...
        for (uint256 i = 0; i <= _userId; i = unsafe_inc(i)) {
            ...
            if (Result > 0) {
                ...
                if (Point * valuePoint > address(this).balance) {
                    Address.sendValue(
                        payable(_allUsersAddress[i]),
                        address(this).balance
                    );
                } else {
                    Address.sendValue(payable(_allUsersAddress[i]), Point * valuePoint);
                }
        ...
        if (RewardClick <= address(this).balance) {
            Address.sendValue(payable(_msgSender()), RewardClick);
        }
    ...
    function Register(address uplineAddress) public payable {
        require(msg.value == 1 ether, "Please Pay 1 BNB To Register!");
        ...
        if (Numer_Win != 0 && _count_Lottery_Candidate != 0) {
            if (_count_Lottery_Candidate > Numer_Win) {
                ...
                for (uint256 i = 1; i <= Numer_Win; i = unsafe_inc(i)) {
                    ...
                    Address.sendValue(payable(Lottery_candida[resultNumber - 1]), Max_Lottery_Price);
                }
                ...
                for (
                    ...
                    Address.sendValue(payable(Lottery_candida[i]), Max_Lottery_Price);
                }
            }
        }
