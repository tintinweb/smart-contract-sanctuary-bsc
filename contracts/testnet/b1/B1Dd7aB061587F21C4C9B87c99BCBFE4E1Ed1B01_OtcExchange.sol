...
contract OtcExchange is IOrderEscrow, IOrderFixed, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => OrderEscrow) private marketsEscrow;
    mapping(uint256 => OrderFixed) private marketsFixed;
 ...
 //   );
    ...
    modifier canUpdateOrder(
        ...
        if( msg.sender != getMaker(orderID, orderClass))
            revert InvalidOwner(orderID, orderClass);
        if( amountBase <= 0 )
            revert InvalidAmount(orderID, orderClass);
    ...
    function createOrderFixed(
       ...
       nonReentrant canCreateOrder(tokenBase, tokenQuote) 
       returns (bool success) {

        uint256 thisOrderID = lastOrderID + 1;

        // take into account the possibility of trading tokens with different decimals than 18
        uint256 price =  (amountQuote * 10 ** (18 - ERC20(tokenQuote).decimals() ) ) * 1e18 /
                         (amountBase * 10 ** (18 - ERC20(tokenBase).decimals() ) )  ;        
    ...
    function takeOrderFixed(
      ...
      returns (bool success) {
        ...
        if (orderSelected.orderType) {
            ...
            emit OrderFixedFulFilled(bytes32(orderID), orderSelected.tokenBase, orderSelected.tokenQuote);
    ...
    function updateOrderFixed(
      ...
      returns (bool success) {
        ...
        uint256 price =  (amountQuote * 10 ** (18 - ERC20(oldOrder.tokenQuote).decimals() ) )  * 1e18  /
    ...
    function takeOrderEscrow(
      ...
      returns (bool success) {
        ...
        if (orderSelected.orderType) {
            ...
            uint256 amountQuote = ( amountToken * orderSelected.price) / 1e18 ;


            if(
                amountToken > tokenBaseERC.balanceOf(msg.sender)
            )
                revert InsufficientFunds(
                    orderSelected.tokenBase,
                    amountToken,
            ...
            if(
                amountToken > tokenQuoteERC.balanceOf(msg.sender)
            )
                revert InsufficientFunds( 
                    orderSelected.tokenQuote, 
                    amountToken,   
    ...
    function cancelOrderEscrow(
      ...
      nonReentrant
      returns (bool success) {
        OrderEscrow memory _orderToBeDeleted = marketsEscrow[
            orderID
        ];
     ...
     */
    function setAvailableTokenBase(address newTokenBase, bool state) external onlyOwner {
     ...
     */
    function setAvailableTokenQuote(address newTokenQuote, bool state) external onlyOwner {
    ...
    function isActive(
        ...
        if (orderClass) {
            // fixed order
            return (marketsFixed[orderID].expirationTime >
                block.timestamp);
        } else {
     ...
     */
 //   0function emergencySweep() external onlyOwner {
