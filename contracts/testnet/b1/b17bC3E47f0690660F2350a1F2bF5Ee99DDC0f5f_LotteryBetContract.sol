...
contract LotteryBetContract is Ownable{
    ...
    //竞猜平台竞猜NFT
    address public _LotteryNFTTokenContract= 0x1b2c6dbaEdFD50a7a1De67737801FBD8f1ea054D;
    ...
    //游戏LP池子
    address public gameLpAddress= 0x88B133054B35889b1Db7F5237279d6E492606b54;
    //加游戏LP比例
    uint256 public gameLpFee=2;
    ...
    //Nft Kol持有者获取比例
    uint256 public nftKolFee=10;

    //返佣比例
    uint256 public inviteAwardFee=6;

    //足球游戏代币
    address public _LotteryGameTokenContract= 0x1486dC1ADF9121280A2024b198Cfc0409772C274;
    ...
    //////NFT竞猜//////
    constructor(){
        _LotteryToken=IERC20(_LotteryToken);
    ...
    function setLotteryGameTokenContract(address token) external onlyOwner{
        _LotteryGameTokenContract=token;
    }
    ...
    mapping (uint256 =>mapping(uint8=>uint256))   _BWP;
    //key: tokenId  key2:ways key3=choose value:price  每场比赛每个玩法每个选项的投注总金额
    mapping (uint256 =>mapping(uint8=>mapping(string=>uint256)))  _BWCM;
    //key: tokenId  key2:use  value:price  每场比赛每个用户的投注总金额
    mapping (uint256 =>mapping(address=>uint256))  _BUOM;
    //key: tokenId  key2:ways key3=choose  key4:use value:price  每场比赛每个玩法每个选项的每个用户的投注总金额
    mapping (uint256 =>mapping(uint8=>mapping(string=>mapping(address=>uint256))))  _BWCUM;
    //key: tokenId  key2:ways key3:use value:[]  每场比赛每个玩法每个用户的投注选项汇总
    mapping (uint256 =>mapping(uint8=>mapping(address=>string[])))  _BWCVUM;
    //key: user  value:tokenId  用户参与的投注NftToken集合
    mapping (address=>uint256[]) _UBTM;
    //key: orderId   value:order  bet订单明细
    mapping (uint256=>NFTBet) _BOM;
    //订单Id
    Counters.Counter public _Bid;

    //邀请人的地址 key是用户  value是被谁邀请的
    mapping(address=>address) _IA;
    //邀请人返利总金额
    mapping(address=>uint256) _IAR;
    //邀请人提取金额
    mapping(address=>uint256) _IARE;
    //邀请人的返佣列表
    mapping(address=>InviteReturn[]) _IALM;
    struct InviteReturn {
        address use;//投注人
        uint256 money;//投注金额
    ...
    mapping(uint256=>mapping(uint8=>uint256)) _BP;
    //Kol返利金额 已提取金额
    // key1=tokenId key=way value=金额
    mapping(uint256=>mapping(uint8=>uint256)) _BPE;
    //key: tokenId  key2:ways  value:price  每场比赛每个玩法的归属用户的代币金额  考虑税率的变动 因此将金额单独存储
    mapping(uint256 =>mapping(uint8=>uint256))  _BWTUM;
    //key: tokenId  key2:ways key3:user value:price  每场比赛每个玩法用户领取的金额
    mapping(uint256 =>mapping(uint8=>mapping(address=>uint256)))  _BWEM;
    //////////////////////////下注接口//////////////////////////
    //key：user value：返现的足球游戏代币
    mapping(address =>uint256) public _BGRBM;
    //返佣出的游戏币总数
    ...
    function betOrder(uint256[] memory _tokenId,uint8[] memory ways,string[] memory wayValue,uint256[] memory betMoney,address inviteAddress)external returns(bool){
        ...
        for(uint index=0;index<_tokenId.length;index++){
            _Bid.increment();
            //只有已经创建的比赛可以投注
            require(_LotteryNFTToken.getTokenStatus(_tokenId[index])==1, "bet status is error!");
            require(betMoney[index]>0, "bet money is non!");
            NFTBet memory betInfo=NFTBet(_Bid._value,_tokenId[index],ways[index],wayValue[index],betMoney[index],_msgSender());
            _BOM[betInfo.orderId]=betInfo;
            ...
            if(_BUOM[_tokenId[index]][_msgSender()]<=0){
                _UBTM[_msgSender()].push(betInfo.tokenId);
            }
            _BUOM[_tokenId[index]][_msgSender()]+=betMoney[index];
    ...
    function bonusPay(uint256 tokenId,uint8 way,uint256 sumMoney) internal{
        _BP[tokenId][way]+=sumMoney.mul(nftKolFee).div(100);
        _BWTUM[tokenId][way]+=sumMoney.mul(prizePool()).div(100);
        _BGRBM[_msgSender()]+=sumMoney.mul(gameRakeBackRatio).div(100);
        _gameTokenTotal+=sumMoney.mul(gameRakeBackRatio).div(100);
    }
    ...
    function queryInviteList(uint256 _lastIndex,uint256 _limit) external view returns(address[] memory use,uint256[] memory sumMoney,uint256[] memory returnMoney,uint256 lastIndex,uint256 inviteReceiveValue,uint256 completeInviteReceive){
        ...
        if(ir.length<=0){
            return (use,sumMoney,returnMoney,0, _IAR[_msgSender()], _IARE[_msgSender()]);
        }
        if(_lastIndex==0){
        ...
        return (use,sumMoney,returnMoney,_lastIndex, _IAR[_msgSender()], _IARE[_msgSender()]);
    }
    ...
    function inviteReceive() external{
        ...
        _LotteryToken.transferFrom(address(this),_msgSender(),maxReceive);
    }
    ...
    function queryMyLotteryList(uint256 lastIndex,uint256 limit,address user) external view returns(MyLotteryItem[] memory,uint256){
        ...
        MyLotteryItem[] memory _myLottery=queryLotteryItemChoose(_tokenId,user);
    ...
    //获取我的所有投注项、投注金额和中奖金额
    function queryLotteryItemChoose(uint256[] memory _tokenId,address user)private view returns(MyLotteryItem[] memory _myLottery){
        (uint8[][] memory _ways,string[][] memory _itemValue) = _LotteryNFTToken.getLotteryResultListByTokenId(_tokenId);
        uint256 calcLengthValue = calcItemChooseLength(_tokenId,_ways,user);
        _myLottery=new MyLotteryItem[](calcLengthValue);
        uint vIndex=0;
        for(uint256 index =0;index< _tokenId.length;index++){
            for(uint256 wayIndex=0;wayIndex<_ways[index].length;wayIndex++){
                //获取所有的下注
                string[] memory chooseItem= _BWCVUM[_tokenId[index]][_ways[index][wayIndex]][_msgSender()];
                for(uint256 chooseIndex=0;chooseIndex<chooseItem.length;chooseIndex++){
                    MyLotteryItem memory item=MyLotteryItem(_tokenId[index],_ways[index][wayIndex],chooseItem[chooseIndex],_BWCUM[_tokenId[index]][_ways[index][wayIndex]][chooseItem[chooseIndex]][user],0);
                    if(bytes(chooseItem[chooseIndex]).length>0&&_itemValue[index][wayIndex].isEqual(chooseItem[chooseIndex])){
                        item.award+=calcBetAward(_tokenId[index],_ways[index][wayIndex],chooseItem[chooseIndex],item.itemMoney);
                    }
                    _myLottery[vIndex++];
                }
            }
    ...
    //计算可选项的总长度
    function calcItemChooseLength(uint256[] memory tokenId,uint8[][] memory ways,address user) internal view returns(uint256 length){
        for(uint256 index =0;index< tokenId.length;index++)
          for(uint256 wayIndex=0;wayIndex<ways[index].length;wayIndex++){
              string[] memory chooseItem= _BWCVUM[tokenId[index]][ways[index][wayIndex]][user];
    ...
    function queryLotteryOdds(TokenOdds[] memory odds) external view returns(TokenOdds[] memory _odds){
        ...
        for(uint index=0;index<odds.length;index++){
            ...
            if(itemSumMoney>0 && item.jackpot>0){
                item.odds=item.jackpot.div(itemSumMoney);
            }
            _odds[index]=item;
        }
    }
