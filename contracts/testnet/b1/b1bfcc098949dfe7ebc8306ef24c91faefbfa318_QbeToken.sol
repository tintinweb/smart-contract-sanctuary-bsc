...
contract QbeToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    QbeTokenDividendTracker public dividendTracker;
    ...
    constructor() ERC20("QBE Ecosystem", "QBE") {
        marketingWallet = payable(0x564fb798015f20b47a20Ec89F7EC2A84734E48A6);
        devWallet = payable(0xA8a4Eb65d30736248c574804F7125BF4E48A3058);
        ...
        dividendTracker = new QbeTokenDividendTracker(
            payable(this),
            router,
...
contract QbeTokenDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    QbeToken public QbeTokenContract;
    ...
    constructor(
        ...
        defaultToken = token;
        QbeTokenContract = QbeToken(mainContract);
    ...
    function _transfer(
        ...
        require(false, "QbeToken_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "QbeToken_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main QbeToken contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(QbeTokenContract);

            uint256 prevBalance = QbeTokenContract.balanceOf(address(this));
            ...
            {
                uint256 received = QbeTokenContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    QbeTokenContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
