...
contract AirdropSystem is Whitelist, Pausable {
    ...
    function resetUpline(address _user) public onlyWhitelisted() {
    ...
    function unbanUser(address _user, string memory _reason) public onlyWhitelisted() returns (bool _success) {
        _users[_user].banned = false;

        emit onUnbanUser(msg.sender, _user, _reason, block.timestamp);
        return true;
    }
}