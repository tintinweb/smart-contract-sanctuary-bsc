...
contract BinaryPlan is Base, IBinaryPlan, Initializable {
    ...
    uint256 public leftHeight;
    uint256 public rightHeight;
    ...
    function addReferrer(
        ...
        bool isLeftChildOfRoot = isLeftBranch(leaf, root());
        bool updateHeight = (isLeftChildOfRoot && (leafLevel > leftHeight)) ||
            (!isLeftChildOfRoot && (leafLevel > rightHeight));
        if (updateHeight) {
            if (isLeftChildOfRoot) leftHeight = leafLevel;
            else rightHeight = leafLevel;
        }
        Account memory rootAccount = accounts[root_];
        while (root_ != address(0)) {
            ...
            if (
                ...
                (1 << ((leafLevel - __levelOf(indices[root_])) + 1)) - 2
