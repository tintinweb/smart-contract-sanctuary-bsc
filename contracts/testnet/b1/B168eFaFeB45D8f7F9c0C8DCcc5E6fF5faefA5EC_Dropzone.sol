...
contract Dropzone is Initializable,Ownable {
    ...
    struct Airdrop{
        ...
        uint from;
        uint to;
        mapping(uint8=>string)_socialMediaId;
        mapping(uint8=>uint) _socialMediaAmount;
    ...
    function applyforAirdrop(address _token,uint _amount,uint _from ,uint _to ,string[] calldata _socialMedia ,uint[] calldata amount) external  {
        ...
        require(_to>block.timestamp && _from>=(block.timestamp-100),"Invalide from to timestamp");
