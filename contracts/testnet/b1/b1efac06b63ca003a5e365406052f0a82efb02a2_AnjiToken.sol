...
abstract contract Ownable is Context {
    ...
    event OwnershipRenounced(bool renounced);
...
contract BNBDistributor is IDividendDistributor {
    ...
    uint256 public constant minAmount = 0;

    event RewardsClaimed(address indexed account, uint256 amount);
    event RewardsDeposited(uint256 rewardAmount, uint256 oldAmount, uint256 newAmount);
    event RewardsDonated(address indexed account, address indexed charityAccount, uint256 amount);
    event RewardsBoughtToken(address indexed account, uint256 amount, uint256 recieved);
    event ShareUpdated(address indexed account, uint256 newShare);
    ...
    function setShare(address shareholder, uint256 amount) external override onlyToken {
        uint256 oldShare = _shareAmount[shareholder];
    ...
    function claimDividend(address shareholder) external override onlyToken {
        ...
        emit RewardsClaimed(shareholder, amount);
    }

    function setCharityFeeReceiver(address _receiver) external onlyToken {
        address oldAddr = charityReceiver;
    ...
    function setMarketingFeeReceiver(address _receiver) external onlyToken {
        address oldAddr = marketingReceiver;
    ...
    function setLiquidityFeeReceiver(address _receiver) external onlyToken {
        address oldAddr = liquidityReceiver;
    ...
    function donate(address shareholder) onlyToken external {
        ...
        emit RewardsDonated(shareholder, charityReceiver, amount);
    }
    ...
    function buyToken(address shareholder) external onlyToken {
        ...
        emit RewardsBoughtToken(shareholder, amount, amountTokensToShareholder);
    }
...
contract AnjiToken is Context, IBEP20, Ownable {
    ...
    uint256 constant _totalSupply = 10 * 10 ** (9 + _decimals); // 10 Billion
    ...
    event AddPairing(address indexed pairAddress);
    event AutoLiquify(uint256 amountBNB, uint256 amountToken);
    event ExcludeFromFees(address indexed account, bool isExcluded);
    event ExcludeFromRewards(address indexed account, bool isExcluded);
    event FeesUpdated(uint256 indexed newCharityFee, uint256 newLiquidityFee, uint256 newMarketingFee, uint256 newBnbReflectionFee);
    event ManualLiquify(uint256 amountBNB, uint256 amountToken);
    event RecoveredExcess(uint256 amount);
    event RecoverTokens(address indexed account, uint256 amount);
    event RemoveLastPairing(address indexed pairAddress);
    event UpdateAutoLiquifyEnabled(bool enabled);
    event UpdateCharityAddress(address indexed newAddr, address indexed oldAddr);
    event UpdateFeesOnNormalTransfer(bool enabled);
    event UpdateIsRestrictedAddress(address indexed account, bool isRestricted);
    event UpdateLiquidityAddress(address indexed newAddr, address indexed oldAddr);
    event UpdateMarketingAddress(address indexed newAddr, address indexed oldAddr);
    event UpdateSwapThreshold(uint256 newThreshold, uint256 oldThreshold);
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(shouldSwapBack()) { swapBack(true); }
    ...
    function swapBack(bool _auto) internal swapping {
        ...
        if (_auto) {
            emit AutoLiquify(swapedBNBAmount, amountToSwap);
        } else {
            emit ManualLiquify(swapedBNBAmount, amountToSwap);
        }
    }
    ...
    function setIsDividendExempt(address holder, bool exempt) external onlyOwner {
        ...
        emit ExcludeFromRewards(holder, exempt);
    }
    ...
    function setFees(
        ...
        emit FeesUpdated(charityFee, liquidityFee, marketingFee, bnbReflectionFee);
    }
    ...
    function setSwapThreshold(uint256 threshold) external onlyOwner {
        ...
        emit UpdateSwapThreshold(threshold, oldThreshold);
    }
    ...
    function setCharityFeeReceiver(address _receiver) external onlyOwner {
        ...
        emit ExcludeFromRewards(_receiver, true);
        isFeeExempt[_receiver] = true;
        emit ExcludeFromFees(_receiver, true);
        emit UpdateCharityAddress(oldAddr, _receiver);
    }
    ...
    function addPair(address pair) external onlyOwner {
        ...
        emit AddPairing(pair);
    }

    function removeLastPair() external onlyOwner {
        address _pair = pairs[pairs.length - 1];
        pairs.pop();

        emit RemoveLastPairing(_pair);
    }
    ...
    function setisRestricted(address adr, bool restricted) external onlyOwner {
        ...
        emit UpdateIsRestrictedAddress(adr, restricted);
    }
    ...
    function recoverTokens(address tokenaddr) external onlyOwner {
        ...
        emit RecoverTokens(tokenaddr, tokenBal);
    }
