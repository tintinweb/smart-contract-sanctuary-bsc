...
import "./interface/ISssStake.sol";
...
interface IBranchV4Account {
    function accountAdd(address addr, address parent) external;
    ...
    function accountFollow(address addr) external view returns (address[] memory);

    function accountParent(address addr) external view returns (address);

    function accountLevel(address addr) external view returns (uint8);
}
...
contract BranchV4 is ReentrancyGuard, AccessControlEnumerable {
    ...
    address sss = 0xADC660e078F274844025c2159A5b9f6cA143B2B5;
    address sssTreasury = 0x4c596c6595c11c5B11E837b68D84aA63EC5d8D99;
    address sssStake = 0xB252D0a8863D8b509Be1269681e498D9C021675B;
    address branchRandom = 0x68B87f27E39A3B9EB7f7b0d8874aE57f6F3Fa788; // 0x82Df23Cde0031Baa9B975F18c2Df7eA99066DC11
    address branchAccount = 0xba18d155F9c13D23A0d16c74C41874C396528856; //0xa8b2d04D5Fba858D59af6a815Eb8367Eb155BB1a

    function initAddress(
        address _usdt,
        address _sss,
        address _sssTreasury,
        address _sssStake,
        address _branchRandom,
        address _branchAccount
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        usdt = _usdt;
        sss = _sss;
        sssTreasury = _sssTreasury;
        sssStake = _sssStake;
        branchRandom = _branchRandom;
        branchAccount = _branchAccount;
    }

    IBranchV4Random private iBranchV4Random;
    IBranchV4Account private iBranchV4Account;
    // 管理员配置
    ...
    uint256 public sssTreasuryTotal; // 燃料消耗统计
    ...
    mapping(address => uint256) public bounsDrawable;
    ...
    constructor() {
        ...
        iBranchV4Account = IBranchV4Account(branchAccount);
        iBranchV4Random = IBranchV4Random(branchRandom);
    ...
    function accountAdd(address addr, address parent) private {
        ...
        iBranchV4Account.accountAdd(addr, parent);
        IERC20(sss).safeTransferFrom(msg.sender, sssTreasury, 1e18);
        sssTreasuryTotal += 1e18;
        brokers[addr].currentVersionTimestamp = versionTimestamps[versionTimes];
    }

    function levelUp(uint8 target) external {
        iBranchV4Account.levelUp(msg.sender, target, brokers[msg.sender].depositTotal);
    }

    function followList(address addr) external view returns (address[] memory addrs, uint256[] memory levels) {
        addrs = iBranchV4Account.accountFollow(addr);
        levels = new uint256[](addrs.length);
        for (uint256 i = 0; i < addrs.length; i++) {
            (, levels[i], ) = iBranchV4Account.accountInfo(addr);
        }
    }
    ...
    function userPreDeposit(uint256 amount, bool useCommentReward) external nonReentrant {
        ...
        (, uint8 level, uint256 id) = iBranchV4Account.accountInfo(msg.sender);
        require(id != 0, "account have not actived.");
        require(amount <= brokerMaxDepositAmountEach[level], "amount limit of your level");
        require(deposits[msg.sender].length == 0 || block.timestamp - deposits[msg.sender][deposits[msg.sender].length - 1].createTimestamp >= depositInterval[level], "deposit interval limit of your level.");
        ...
        uint256 days_ = iBranchV4Random.randomDaysStartFinalPay(depositWithdrawRatePercent(), brokers[msg.sender].depositTotal == 0);
    ...
    function userFinalDeposit(uint256 _index) external nonReentrant {
        ...
        uint256 days_ = iBranchV4Random.randomDaysStartWithdraw(depositWithdrawRatePercent(), iBranchV4Account.accountLevel(msg.sender));
    ...
    function userWithdraw(uint256 _index) external nonReentrant {
        ...
        IERC20(sss).safeTransferFrom(msg.sender, sssTreasury, fuelAmount);
        sssTreasuryTotal += fuelAmount;
    ...
    function allocAmount(uint256 amount) private {
        ...
        IERC20(usdt).safeIncreaseAllowance(sssStake, (amount * 5) / 1000); // 处理援助池
        ISssStake(sssStake).sssRescue((amount * 5) / 1000); //
    }
    ...
    function processBrokerBonus(address addr, uint256 amount) private {
        ...
        for (uint256 i = 0; i < 30; i++) {
            address parent = iBranchV4Account.accountParent(addr);
            if (parent == address(0)) {
                break;
            }
            uint8 parentLevel = iBranchV4Account.accountLevel(parent);
    ...
    function brokerBonusWithdraw() external nonReentrant {
        ...
        if (bounsDrawable[msg.sender] > 0) {
            amount += bounsDrawable[msg.sender];
            bounsDrawable[msg.sender] = 0;
        }
        if (amount > 0) {
            IERC20(sss).safeTransferFrom(msg.sender, sssTreasury, amount / 100);
            sssTreasuryTotal += amount / 100;
    ...
    function userDealBlacklist(uint256 index) external nonReentrant {
        ...
        deposit.status = 40;
        IERC20(sss).transferFrom(msg.sender, address(this), 100e18 * brokers[msg.sender].punishTimes);
        sssTreasuryTotal += 100e18 * brokers[msg.sender].punishTimes;
    }
    ...
    function fomoRewardWithdraw() external nonReentrant {
        ...
        IERC20(sss).safeTransferFrom(msg.sender, sssTreasury, fomoRewardDrawables[msg.sender] / 100);
        sssTreasuryTotal += fomoRewardDrawables[msg.sender] / 100;
    ...
    function sprintRewardWithdraw() external nonReentrant {
        ...
        IERC20(sss).safeTransferFrom(msg.sender, sssTreasury, sprintRewardDrawables[msg.sender] / 100);
        sssTreasuryTotal += sprintRewardDrawables[msg.sender] / 100;
    ...
    function lossCompensationWithdraw(uint32 _versionTimestamp) external nonReentrant {
        ...
        IERC20(sss).safeTransfer(msg.sender, drawable);
...
interface ISssStake {
    function sssRescue(uint256 amount) external;
}
