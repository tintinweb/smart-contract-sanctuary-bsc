...
contract SoulCore is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable {
    ...
    **/
    ...
    function setTeamsFrom(address communityAddr) public{
        //必须未绑定骑士
        address userKnight = teams[_msgSender()];
        require(userKnight == address(0), "already bound");

        (address targetKnight,,) = meta.SOUL_TEAM.teams(communityAddr);
        require(targetKnight != address(0), "community does not exist");

        teams[_msgSender()] = communityAddr;
        meta.SOUL_TOKEN.transfer(_msgSender(), 0.01 ether);
    }
