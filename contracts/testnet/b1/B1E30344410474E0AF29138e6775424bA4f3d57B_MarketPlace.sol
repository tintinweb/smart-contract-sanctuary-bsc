...
contract MarketPlace is ReentrancyGuard, ERC721Holder {
    ...
    event WITHDRAWAL(address indexed userAddress, address indexed nftContract, uint256 indexed tokenId);
    ...
    function buy(
        ...
        require(!isOwnerNFTOnMarket(nftContract, msg.sender, _tokenId), "Error, you are the owner");
        require(msg.value == price, "Please submit the asking price in order to complete purchase");
        //pay the seller the amount
        uint tax = msg.value * _fees / 100;
        order[nftContract][_tokenId].seller.transfer(msg.value - tax);
    ...
    function swap(uint _ver, address _ct, uint256 _tokenId, address _buyer, address _seller, uint256 _matchPrice, uint _fees, string memory _data) public payable {
        uint tax = _matchPrice * _fees / 100;
        order[_ct][_tokenId].seller.transfer(_matchPrice - tax);
        payable(ownerMartket).transfer(tax);
        order[_ct][_tokenId].seller.transfer(_matchPrice);
        if (_ver == 721) IERC721(_ct).transferFrom(address(this), msg.sender, _tokenId);
    ...
    function accept(uint _ver, address _ct, uint256 _tokenId, address _buyer, uint _fees) public {
        ...
        swap(_ver, _ct, _tokenId, _buyer, msg.sender, offers[_buyer][_ct][_tokenId].offersPrice, _fees, "accept");
    }
    ...
    function userWithdrawNFT(uint _ver, address _ct, uint256 _tokenId) public {
        require(isOwnerNFTOnMarket(_ct, msg.sender, _tokenId), "Error, you are not the owner");
        if (_ver == 721) {
            IERC721(_ct).transferFrom(address(this), msg.sender, _tokenId);
            emit WITHDRAWAL(msg.sender, _ct, _tokenId);
        }
    }
}
