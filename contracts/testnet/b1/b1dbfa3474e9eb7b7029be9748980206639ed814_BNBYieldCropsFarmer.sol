...
/**
    ...
    plan 4 - 3.5% daily, 140% for  40 days (actual APR 1.00% ,  40% ROI)

    Withdrawals are subject to 10% Sustainability tax.
    *sustainability tax stays in the contract.
    
    ~ PLAN REQUIREMENTS: 
    plan 1 - min. 0.1 BNB, max. of 25 BNB
    plan 2 - min. 0.2 BNB, max. of 30 BNB
    plan 3 - min. 0.3 BNB, max. of 35 BNB
    plan 4 - min. 0.4 BNB, max. of 40 BNB
    ...
    Referral Bonus will be 5% for the first week. 
    Sustainability Tax will be 5% for the first week.
...
contract BNBYieldCropsFarmer {
    ...
    uint public startTime = 1654264800; // Fri Jun 03 2022 14:00:00 GMT+0000 https://www.unixtimestamp.com/
	...
	constructor(address payable _dev1, address payable _dev2, address payable _dev3, address payable _prtnr1, address payable _prtnr2, address payable _mkt) {
        ...
        plans.push(Plan(200, 20,  1 * 1e17, 25 ether, 0, 0, 0, 0, true));
        plans.push(Plan( 90, 25,  2 * 1e17, 30 ether, 0, 0, 0, 0, true));
        plans.push(Plan( 60, 30,  3 * 1e17, 35 ether, 0, 0, 0, 0, true));
        plans.push(Plan( 40, 35,  4 * 1e17, 40 ether, 0, 0, 0, 0, true));
	}
