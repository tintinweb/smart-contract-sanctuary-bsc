...
contract TestContract is ERC721, NonblockingReceiver, ILayerZeroUserApplicationConfig {
    ...
    /// set max mint mount per tx TestContract npx hardhat --network rinkeby setMaxMintAmountPerTx
    function setMaxMintAmountPerTx(uint256 _maxMintAmountPerTx) public onlyOwner {
        maxMintAmountPerTx = _maxMintAmountPerTx;
    }

    /// Mint TestContract npx hardhat --network rinkeby setPublicSaleStartTime
    ...
    function mint(uint8 numTokens) external payable {
        ...
        require(numTokens <= maxMintAmountPerTx, "Max NFTs per transaction");
...
contract WhiteList {
    function _whitelist() internal view returns (string memory) {
        return "test";
    }
}
