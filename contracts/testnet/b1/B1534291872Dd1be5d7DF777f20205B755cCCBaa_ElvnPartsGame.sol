...
contract ElvnPartsGame is ERC1155, Ownable, Pausable, ERC1155Burnable, ERC1155Supply {
    ...
    */
    ...
    function addPartBatch(uint[] memory _partId, uint[] memory _class, uint[] memory _stakingPower, uint[] memory _setId) external onlySetter{
    ...
    function getRandomBase() public returns(uint[2] memory _baseParts,uint[2] memory _baseClass){
        require(!charactersCreated,"ELVN Parts: Characters were already created");
        _baseParts[0] = getRandomNft();
        _baseClass[0] = class[_baseParts[0]];
        classSupply[_baseClass[0]] += 1;
        for(bool i; i=false;){
            _baseParts[1] = getRandomNft();
            i = (_baseClass[0] == class[_baseParts[1]]);
        }
        _baseClass[1] = class[_baseParts[1]];
        classSupply[_baseClass[1]] += 1;
        return (_baseParts, _baseClass);
    }
    ...
    function openWoodChest(bool _useKey) public {
        ...
        {
            if(_randomBase >5 && _randomBase <= 20){
            getRandomStakingBoost(1);
            }
            else
            {
                if(_randomBase >20 && _randomBase <= 75){
                getRandomExpBoost(1);
                }
                else
                {
    ...
    function openBronzeChest(bool _useKey) public {
        ...
        {
            if(_randomBase >10 && _randomBase <= 35){
            getRandomStakingBoost(2);
            }
            else
            {
                if(_randomBase >35 && _randomBase <= 85){
                getRandomExpBoost(2);
                }
                else{
                    mint(msg.sender,keyId,1,"");
    ...
    function openSilverChest(bool _useKey) public {
        ...
        {
            if(_randomBase >20 && _randomBase <= 50){
            getRandomStakingBoost(3);
            }
            else
            {
                if(_randomBase >50 && _randomBase <= 95){
                getRandomExpBoost(3);
                }
                else
                {
    ...
    function openGoldChest(bool _useKey) public {
        ...
        {
            if(_randomBase >30 && _randomBase <= 70){
    ...
    function openLegendaryChest(bool _useKey) public {
       ...
       if(_useKey){
        ...
        if(_randomBase <= 40){
            ...
            if(_randomBase >40 && _randomBase <= 90){
    ...
    function getRandomNft() internal returns (uint){
        ...
        randomizer = _randomClass;
        uint _randomPartResult = random(10000);
        randomizer = _randomPartResult;
        uint _partId;
        ...
        {
            if(_randomPartResult > 2500 && _randomPartResult <= 4000){
            _partId = 2;
            } 
            else
            {
                if(_randomPartResult > 4000 && _randomPartResult <= 4750){
                _partId = 3;
                } 
                else
                {
                    if(_randomPartResult > 4750 && _randomPartResult <= 5000){
                    _partId = 4;
                    } 
                    else
                    {
                        if(_randomPartResult > 5000 && _randomPartResult <= 6500){
                        _partId = 5;
                        }
                        else
                        {
                            if(_randomPartResult > 6500 && _randomPartResult <= 7400){
                            _partId = 6;
                            } 
                            else
                            {
                                if(_randomPartResult > 7400 && _randomPartResult <= 7850){
                                _partId = 7;
                                }
                                else
                                {
                                    if(_randomPartResult > 7850 && _randomPartResult <= 8000){
                                    _partId = 8;
                                    } 
                                    else
                                    {
                                        if(_randomPartResult > 8000 && _randomPartResult <= 8750){
                                        _partId = 9;
                                        } 
                                        else
                                        {
                                            if(_randomPartResult > 8750 && _randomPartResult <= 9200){
                                            _partId = 10;
                                            } 
                                            else
                                            {
                                                if(_randomPartResult > 9200 && _randomPartResult <= 9425){
                                                _partId = 11;
                                                }
                                                else
                                                {
                                                    if(_randomPartResult > 9425 && _randomPartResult <= 9500){
                                                    _partId = 12;
                                                    } 
                                                    else
                                                    {
                                                        if(_randomPartResult > 9500 && _randomPartResult <= 9750){
                                                        _partId = 13;
                                                        }
                                                        else
                                                        {
                                                            if(_randomPartResult > 9750 && _randomPartResult <= 9900){
                                                            _partId = 14;
                                                            }
                                                            else
                                                            {
                                                                if(_randomPartResult > 9900 && _randomPartResult <= 9975){
                                                                _partId = 15;
                                                                }
                                                                else
                                                                {
    ...
    function getRandomBoostNumber(uint _chestId) internal returns(uint){
        ...
        if(_chestId == 1){
            ...
            {
                if(_randomBoostResult > 5 && _randomBoostResult <= 20){
                _boostLevel = 3;
                }
                else
                {
                    if(_randomBoostResult > 20 && _randomBoostResult <= 50){
                    _boostLevel = 2;
                    }
                    else
                    {
        ...
        else
        {
            if(_chestId == 2)
            {
                ...
                {
                    if(_randomBoostResult > 10 && _randomBoostResult <= 40){
                    _boostLevel = 3;
                    }
                    else
                    {
                        if(_randomBoostResult > 40 && _randomBoostResult <= 70){
                        _boostLevel = 2;
                        }
                        else
                        {
            ...
            else
            {
                if(_chestId == 3){
                    if(_randomBoostResult <= 20){
                        ...
                        if(_randomBoostResult > 20 && _randomBoostResult <= 60){
                        _boostLevel = 3;
                        }
                        else
                        {
                ...
                else
                {
                    if(_chestId == 4){
                        ...
                        {
                            if(_randomBoostResult > 40 && _randomBoostResult <= 80){
                            _boostLevel = 3;
                            }
                            else
                            {
