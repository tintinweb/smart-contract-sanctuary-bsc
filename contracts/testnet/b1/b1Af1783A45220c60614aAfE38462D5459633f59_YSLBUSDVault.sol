    ...
    */
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused {
        ...
        if(IERC20(Admin.YSL()).balanceOf(address(this)) >= YSLRequire){
            ...
            path[0] = Admin.BUSD();
            path[1] = Admin.YSL();
            IERC20(Admin.BUSD()).safeApprove(router, (amount * percentYSL/100));
            swap(
            ...
            if(IERC20(Admin.YSL()).balanceOf(address(this)) <= (reserveYSL * amount * percentYSL)/(reservesBUSD * 100)){
                    convertBUSDToLP(reservesBUSD * IERC20(Admin.YSL()).balanceOf(address(this)) / reserveYSL, 
                    IERC20(Admin.YSL()).balanceOf(address(this)),
                    Admin.BUSD(), Admin.YSL(), address(yslBUSD));
                    

            }else{
                convertBUSDToLP(amount * percentYSL/100, 
                    (reserveYSL * amount * percentYSL)/(reservesBUSD * 100),
                    Admin.BUSD(), Admin.YSL(), address(yslBUSD));
                    IReceipt(Admin.YSL()).burn(address(this),IERC20(Admin.YSL()).balanceOf(address(this)));
