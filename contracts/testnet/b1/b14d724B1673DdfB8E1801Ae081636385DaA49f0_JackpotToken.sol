...
contract Jackpot_SettingsMixin is Ownable {
  ...
  JackpotSettings public jackpotSettings = JackpotSettings({
    ...
    minBuy: 1 * 10**(BNB_DECIMALS - 5), // 0.1 BNB
    overflowPercentage: 5000, // 50%
    overflowBNB: 50 * 10**(BNB_DECIMALS-5), // 50 BNB
    activeThresholdBNB: 25 * 10**(BNB_DECIMALS-5) // 25 BNB
  });
  ...
  uint256 private constant JACKPOT_OVERFLOW_BNB_MIN = 10 * 10**(BNB_DECIMALS-5); // 10 BNB
  uint256 private constant JACKPOT_OVERFLOW_BNB_MAX = 250 * 10**(BNB_DECIMALS-5); // 250 BNB

  // this is the amount above which the jackpot is active
  uint256 private constant JACKPOT_ACTIVE_THRESHOLD_MIN = 5 * 10**(BNB_DECIMALS-5); // minimum 5 BNB
  uint256 private constant JACKPOT_ACTIVE_THRESHOLD_MAX = 100 * 10**(BNB_DECIMALS-5); // maximum 100 BNB
