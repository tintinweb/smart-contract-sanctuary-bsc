...
contract HiVPN is IHiVPN, Payable, Tokenized, Planed {
  ...
  function findPlan(uint256 value) public view override returns (uint8 plan) {
    ...
    uint moreValue = value.mul(100).div(more.add(100));

    for (uint8 i = 1; i < planPrices.length; i++) {
      if (lessValue >= planPrices[i] && moreValue <= planPrices[i]) plan = i;
    }
  }
  ...
  function payment(
    ...
    require(plan > 0 && plan < planPrices.length, "SLP");

    if (token == address(0)) {
      require(msg.value == amount, "BNB");
    } else {
      require(tokenAllowance(_msgSender(), token) >= amount, "APR");
    }
...
abstract contract Planed is IPlaned, Secured {
  ...
  uint256[8] public planIds = [0, 51, 5, 31, 32, 47];

  // View functions --------------------------------------------------------
  function allPlanPrices() external view override returns (uint256[8] memory) {
    return planPrices;
  }

  function allPlanIds() external view override returns (uint256[8] memory) {
    return planIds;
  }

  function allOptions() external view override returns (uint256, uint256, uint256) {
    return (fee, more, less);
  }
...
interface IPlaned {
  function allPlanIds() external view returns (uint256[8] memory);

  function allPlanPrices() external view returns (uint256[8] memory);

  function allOptions() external view returns (uint256, uint256, uint256);
}
