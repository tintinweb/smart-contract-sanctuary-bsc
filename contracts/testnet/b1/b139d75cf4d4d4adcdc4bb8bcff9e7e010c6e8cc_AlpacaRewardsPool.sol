...
contract AlpacaRewardsPool is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    /**
     * @notice Info of Pool
     * @param want: Address of reward token
    ...
    /// VaultBank contract address
    address public vaultBankAddress;
    ...
    event AddNewPool(address indexed _want);

    modifier onlyWhitelistedContract() {
        require(
            whitelist_contract[msg.sender] || msg.sender == owner(),
            "AlpacaRewardsPool: not whitelisted or owner"
        );
        _;
    }
    ...
    function initialize(
        address _vaultBankAddress
    ) external initializer {
        vaultBankAddress = _vaultBankAddress; // address of VaultBank Contract
        isPoolActive = true;
        __Ownable_init();
        __ReentrancyGuard_init();
    }

    function setVaultBankAddress(address _bankAddress) external onlyOwner {
        vaultBankAddress = _bankAddress;
    }
    ...
    function setUserInfo(uint256 _pid, address _user, uint256 _shares, uint256 _rewardsClaimt, uint256 _rewardDebt) external nonReentrant onlyWhitelistedContract {
    ...
    function setPoolInfo(uint256 _pid, uint256 _lastChangeId, uint256 _accRewardPerShare) external nonReentrant onlyWhitelistedContract {
        PoolInfo storage pool = poolInfo[_pid];
        pool.lastChangeId = _lastChangeId;
        pool.accRewardPerShare = _accRewardPerShare;
    }
    ...
    */
    function updateUser(uint256 _pid, uint256 _shares, address _user) external nonReentrant onlyWhitelistedContract {
        _claim(_pid, _user);
        UserInfo storage user = userInfo[_pid][_user];
    ...
    function setUserShares(uint256 _pid, address _user, uint256 _shares) external nonReentrant onlyWhitelistedContract {
        UserInfo storage user = userInfo[_pid][_user];
        user.shares = _shares;
    }

    function updatePool(uint256 _pid, uint256 _receivedAmount) external nonReentrant onlyWhitelistedContract {
        _updatePool(_pid, _receivedAmount);
    }

    function claim(uint256 _pid, address _user) external nonReentrant onlyWhitelistedContract {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        _claim(_pid, _user);
        user.rewardDebt = (user.shares * pool.accRewardPerShare) / (1e18);
    ...
    /// @notice Function for deposit in pool
    function _updatePool(uint256 _pid, uint256 _amount) private {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 lastChangeId = pool.lastChangeId++;

        uint256 sharesTotal = IERC20Upgradeable(pool.want).balanceOf(address(this));
        if (sharesTotal == 0) {
    ...
    function _claim(uint256 _pid, address _user) private {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        IERC20Upgradeable rewardToken = pool.want;
