...
contract GKT is ERC721, ERC721Enumerable, Ownable {
    bool public IsActive = true;
    string private _baseURIextended;
    address payable public immutable contract_owner;
    uint256 public _price = 0;

    constructor(address payable _owner) ERC721("GKTEST", "GKT") {
    ...
    function mint(address _addr, uint256 _amount, uint num) public payable onlyOwner{
        require(IsActive, "Sale must be active to mint Tokens");
        require(_addr != address(0));
        require(num > 0);
        require(_amount >= 0);
        require(_price * num <= msg.value, "Price is not correct");

        for(uint i = 0; i < num; i++) 
        {
            uint mintIndex = totalSupply();
            _safeMint(_addr, mintIndex);// 給指定地址
        }
    }
