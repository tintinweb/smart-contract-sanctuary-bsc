...
contract AmexDao is Context, IERC20, Ownable {
    ...
    mapping (address => bool) public isWalletLimitExempt;                                                      // 是否无钱包个数限制
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amountBNBLiquidity = initialBalance.mul(_liquidityShare).div(totalBNBFee).div(2);
        uint256 amountBNBTeam = initialBalance.mul(_teamShare).div(totalBNBFee);
        uint256 amountBNBMarketing = initialBalance.sub(amountBNBLiquidity).sub(amountBNBTeam);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            ...
            MFee = amount.mul(_buyMarketingFee).div(100);
            TFee = amount.mul(_buyTeamFee).div(100);
            OFee = amount.mul(_buyOperationFee).div(100);
            LFee = amount.mul(_buyLiquidityFee).div(100);
            feeAmount = MFee.add(TFee).add(OFee).add(LFee);
        }
        else if(isMarketPair[recipient]) {
            // feeAmount = amount.mul(_totalTaxIfSelling).div(100);
            MFee = amount.mul(_sellMarketingFee).div(100);
            TFee = amount.mul(_sellTeamFee).div(100);
            OFee = amount.mul(_sellOperationFee).div(100);
        ...
        if(feeAmount > 0) {
            ...
            _balances[teamWalletAddress] = _balances[teamWalletAddress].add(TFee);
            emit Transfer(sender, teamWalletAddress, TFee);
            _balances[operationWalletAddress] = _balances[operationWalletAddress].add(OFee);
            emit Transfer(sender, operationWalletAddress, OFee);
