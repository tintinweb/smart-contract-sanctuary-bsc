...
contract Loan is ERC721Holder, AccessControlEnumerable, ReentrancyGuard {
    ...
    address public gasStake;

    uint256 public creatorRatio;
    uint256 public gasStakeRatio;
    uint256 public maxLevel;
    uint256 public repayTimes = 1;     //TODO 上线改成 6次
    uint256 public INTERVAL = 2 * 60;  //TODO 上线改成 7 * 24 * 60 * 60
    ...
    mapping(address => mapping(uint256 => LoanInfo)) private loanInfos;
    ...
    function setRepayConfig(uint256 _creatorRatio, uint256 _gasStakeRatio) external onlyRole(OPERATER_ROLE) {
        creatorRatio = _creatorRatio;
        gasStakeRatio = _gasStakeRatio;
    }

    function setMedalId(uint256 _medalId) external onlyRole(OPERATER_ROLE) {
        medalIds.add(_medalId);
    }

    function rescue(IERC20 _token, uint256 amount) external onlyRole(OPERATER_ROLE) {
    ...
    function redeem(uint256 _index, uint256 _tokenId) external nonReentrant {
        require(_countLoanRecord(_index) == 0, "Loan: loan records exist!");
        Group storage group = groups[msg.sender][_index];
        require(group.activate == 1, "Loan: active status error!");
        require(group.creator == msg.sender, "Loan: no permission!");
        group.tokenId = 0;
        group.activate = 0;
        IERC721(medals).safeTransferFrom(address(this), msg.sender, _tokenId);
    }

    function levelUp(uint256 _index) external {
        require(_checkStatus(_index), "Loan: status error!");
        Group storage group = groups[msg.sender][_index];
    ...
    function reset(uint256 _index) external nonReentrant {
        require(_checkStatus(_index), "Loan: status error!");
        Group storage group = groups[msg.sender][_index];
        require(group.activate == 1, "Loan: not activate!");
        require(group.creator == msg.sender, "Loan: no permission!");
    ...
    function borrow(uint256 _index, address _address) external nonReentrant {
        require(_checkLoanable(_index), "Loan: loan unable!");
        LoanInfo storage loanInfo = loanInfos[_address][_index];
        require(loanInfo.loanAmount == 0, "Loan: repeated loan!");
        Group storage group = groups[msg.sender][_index];
        require(group.creator == msg.sender, "Loan: address error!");
        require(group.activate == 1, "Loan: not activate!");
        require(group.creator == msg.sender, "Loan: no permission!");
        _borrowUpdateMember(_index, _address);
        ...
        IERC20(token).safeTransfer(_address, (getTokenPrice() * config.loanAmount) / 1e18);
    }

    function repay(uint256 _groupIndex, uint256 _index) external nonReentrant {
        LoanInfo storage loanInfo = loanInfos[msg.sender][_groupIndex];
        require(loanInfo.loanAmount > 0, "Loan: no Record!");
        require(loanInfo.alreadyRepayPeriod == _index, "Loan: incorrect index!");
        uint256 current = _index + 1;
        ...
        _repayUpdateMember(loanInfo, _groupIndex, current);
        ...
        IERC20(usdt).safeTransfer(groups[msg.sender][_groupIndex].creator, (interest * creatorRatio) / 1e18);
    }

    function _checkActivated() private view returns (bool) {
        Group[] memory arr = groups[msg.sender];
    ...
    function _checkGroupExist(address[] memory addrs) private view returns (bool) {
        for (uint256 i = 0; i < addrs.length; i++) {
            if (groups[addrs[i]].length > 0) {
                return true;
            }
        }
        return false;
    ...
    function _checkLoanable(uint256 _index) public view returns (bool) {
        Member[] storage members = groups[msg.sender][_index].members;
        uint256 count = _countLoan(members);
        if (count < 2) {
            return true;
        } else if (count < 4) {
            return _countRepay(members) >= 2;
        } else {
            return _countRepay(members) >= 4;
        }
    }

    function _countLoan(Member[] memory members) private pure returns (uint256) {
        uint256 count = 0;
    ...
    function _countRepay(Member[] memory members) private view returns (uint256) {
        uint256 count = 0;
        for (uint256 i = 0; i < members.length; i++) {
    ...
    function _checkStatus(uint256 _index) public view returns (bool) {
        Member[] storage members = groups[msg.sender][_index].members;
        for (uint256 i = 0; i < members.length; i++) {
    ...
    function _reset(uint256 _index) private {
        Member[] storage members = groups[msg.sender][_index].members;
        for (uint256 i = 0; i < members.length; i++) {
            members[i].loanStatus = 1;
            members[i].repayCount = 0;
        }
    }

    function _addGroupInfo(
        address[] memory addrs,
        uint256 i,
        uint256 _tokenId
    ) private {
        Group storage group = groups[addrs[i]].push();
        group.level = 1;
        group.tokenId = _tokenId;
        group.activate = 1;
        ...
        for (uint256 j = 0; j < addrs.length; j++) {
            group.members[j].account = addrs[j];
            group.members[j].loanStatus = 1;
        }
    }

    function _borrowUpdateMember(uint256 _index, address _address) private {
        Member[] storage members = groups[msg.sender][_index].members;
        for (uint256 i = 0; i < members.length; i++) {
            Member storage member = members[i];
            if (member.account != _address) {
                continue;
            }
    ...
    function _repayUpdateMember(
        ...
        for (uint256 i = 0; i < members.length; i++) {
            ...
            if (_repayCount == loanInfo.period) {
                member.loanStatus = 3;
            }
        }
    }
    ...
    function getGroups(address _addr) external view returns (Group[] memory) {
        return groups[_addr];
    }

    function getBrorrowInfo(address _address, uint256 _index) external view returns (LoanInfo memory) {
        return loanInfos[_address][_index];
    }

    function getMedalsInfo(address _address, uint256 _medalsId) external view returns (uint256[] memory) {
        return IMedals(medals).accountMedalTokenIds(_address, _medalsId);
    }

    function getMedalIds() external view returns (uint256[] memory) {
        return medalIds.values();
    }
}
