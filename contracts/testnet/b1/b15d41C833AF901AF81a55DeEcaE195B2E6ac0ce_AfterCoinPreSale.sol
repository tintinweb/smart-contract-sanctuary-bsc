...
contract AfterCoinPreSale is Crowdsale, FinalizableCrowdsale, AccessControlUpgradeable {
    ...
    // fee(%) in token sold fee to afterCoin owner
    uint256 public tokenSoldFee;

    // afterCoin owner account
    address public afterCoinOwner;
    ...
    function initialize(
      ...
      address _afterCoinOwner,         // afterCoin owner account
      ...
      afterCoinOwner = _afterCoinOwner;
  ...
  function dexListing() private {
    ...
    // (%) of bnb raised to afterCoin owner
    uint256 fee = (bnbBalanceInContract * bnbRaisedFee) / 10**2;
    address payable _owner = payable(afterCoinOwner);   
    _owner.transfer(fee);

    // (%) of token sold to afterCoin owner
    require(token.balanceOf(address(this)) > 0, "Finalize: Insufficient token balance");
    uint256 tokensForAfterCoinOwner = (tokensSold * tokenSoldFee) / 10 ** 4;
    token.transfer(afterCoinOwner, tokensForAfterCoinOwner);
    ...
    uint256 refundBalance = tokenBalanceInContract - tokensForAfterCoinOwner - tokenForLiquidity - tokensSold;
    if(refundType == 1){
