...
contract Staking is IStaking, Ownable, ReentrancyGuard, Pausable {
	...
	function getAPY() external view virtual override returns (uint256 APY) {
		if (totalStaked > 0)
			APY = rewardPerBlock * (BLOCKS_PER_YEAR) * (100) * (PRECISION_5) / (totalStaked);
	}
	...
	function recoverNonLockedRewardTokens() external override onlyOwner {
		uint256 nonLockedTokens = rewardsToken.balanceOf(address(this)) - rewardTokensLocked;
		...
		updateReward(_msgSender())
	{
		...
		usersInfo[_msgSender()].staked = usersInfo[_msgSender()].staked + _amount;
	...
	function withdraw(uint256 _amount) public override nonReentrant updateReward(_msgSender()) {
		...
		require(staked - locked >= _amount, "Withdraw: cooldown not reached");

		totalStaked = totalStaked - _amount;
		usersInfo[_msgSender()].staked = staked - _amount;
	...
	function rewardPerToken() public view override returns (uint256) {
		...
		uint256 accumulatedReward = blocksWithRewardsPassed() * rewardPerBlock * PRECISION / totalPoolStaked;
		return rewardPerTokenStored + accumulatedReward;
	}

	function earned(address _account) public view override returns (uint256) {
		uint256 rewardsDifference = rewardPerToken() - usersInfo[_account].rewardPerTokenPaid;
		uint256 newlyAccumulated = usersInfo[_account].staked * rewardsDifference / PRECISION;

		return usersInfo[_account].rewards + newlyAccumulated;
	}
	...
	function _getFutureRewardTokens() internal view returns (uint256) {
		...
		return blocksLeft * rewardPerBlock;
	}
	...
	function _updateCooldown(uint256 _amount) internal {
		if (cooldownPeriod > 0) {
			uint256 timePassed = block.timestamp - usersInfo[_msgSender()].lastUpdate;

			if (timePassed >= cooldownPeriod) {
				usersInfo[_msgSender()].locked = _amount;
			} else {
				uint256 share = timePassed * PRECISION / cooldownPeriod;
				uint256 locked = usersInfo[_msgSender()].locked;

				usersInfo[_msgSender()].locked = locked + _amount  - (
					locked * share  / PRECISION
				);
			}
