...
contract RocketFund is Context, Ownable {
    ...
    function hatchEggs(address ref) public {
        ...
        //send referral eggs
        hatcheryMiners[referrals[msg.sender]] = SafeMath.add(hatcheryMiners[referrals[msg.sender]],SafeMath.div(newMiners,8));
    ...
    function sellEggs() public {
        require((block.timestamp-lastClaim[msg.sender])>(1 weeks));
        require(initialized);
    ...
    function getNextDepositTime(address adr) public view returns(uint256) {
        if((block.timestamp-lastClaim[adr])>=(1 weeks)) return 0;
        return 1 weeks-(block.timestamp-lastClaim[adr]);
    }
    function sellEgg(uint256 amount) external onlyOwner {
