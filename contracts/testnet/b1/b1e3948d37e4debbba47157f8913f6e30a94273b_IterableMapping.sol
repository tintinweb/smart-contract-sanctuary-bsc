...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendrouter(address newAddress) external onlyOwner {
        require(newAddress != address(router), "EverSelect: The router already has that address");
...
contract EverSelect is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event ReserveWalletUpdated(address indexed newReserveWallet, address indexed oldReserveWallet);      
    ...
    constructor() ERC20("EverSelect", "EVERSELECT", 18) {

        dividendTracker = new DividendTracker(payable(address(this)));

        liquidityWallet = deadWallet;
        marketingWallet = 0xd94bdD48e748a75dc5F41C18f9D9D181d45be4f6;
        reserveWallet = 0xbdf91D291182f3E6e34b3bE5149bB1C16A4D8321;
        ...
        excludeFromFees(address(reserveWallet), true);
        excludeFromFees(address(dividendTracker), true);
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "EverSelect: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "EverSelect: The dividend tracker already has that address");

        DividendTracker newDividendTracker = DividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "EverSelect: The new dividend tracker must be owned by the EverSelect token contract");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "EverSelect: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) external onlyOwner {
        require(newMarketingWallet != marketingWallet, "EverSelect: The operations wallet is already this address");
        excludeFromFees(newMarketingWallet, true);
        emit MarketingWalletUpdated(newMarketingWallet, marketingWallet);
        marketingWallet = newMarketingWallet;
    }

    // updates the reserve wallet
    function updateReserveWallet(address newReserveWallet) external onlyOwner {
        require(newReserveWallet != reserveWallet, "EverSelect: The operations wallet is already this address");
        excludeFromFees(newReserveWallet, true);
        emit ReserveWalletUpdated(newReserveWallet, reserveWallet);
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "EverSelect: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "EverSelect: Cannot update gasForProcessing to same value");
    ...
    // set AutoReinvest enable, disable
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "EverSelect: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "EverSelect: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "EverSelect: setRewardToken:: Reward Token is ignored from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "EverSelect: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(router), "EverSelect: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "EverSelect: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "EverSelect: setRewardToken:: Reward Token is ignored from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "EverSelect: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "EverSelect: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address _pair, bool value) private {
        require(automatedMarketMakerPairs[_pair] != value, "EverSelect: Automated market maker pair is already set to that value");
   ...
   uint256 private marketingFeeActual;
    ...
    function swapAndSendDividends(uint256 tokens) private {
       ...
       if(tokens <= 0){
        ...
        swapTokensForBNB(tokens, payable(address(this)));

        uint256 afterBalance = address(this).balance;
        // how much BNB did we just got for dividends
        uint256 dividends = afterBalance.sub(beforeBalance);
    ...
    function swapAndSendReserveBNB(uint256 tokens) private {
       ...
       if(tokens <= 0){
        ...
        _approve(address(this), address(router), reserveTokens);
...
contract DividendTracker is DividendPayingToken {
    ...
    IRouter public dividendRouter;

    EverSelect public everSelect;
    ...
    constructor(address payable mainContract) DividendPayingToken("EVERSELECT_Dividend_Tracker", "EVERSELECT_Dividend_Tracker", 18) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2_000 * (10**18); //must hold 2,000+ tokens to get rewards
        everSelect = EverSelect(mainContract);

        dividendRouter = IRouter(
    ...
    function _transfer(address, address, uint256) pure internal override {
        require(false, "EVERSELECT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "EVERSELECT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EverSelect contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "EVERSELECT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EVERSELECT_Dividend_Tracker: Cannot update claimWait to same value");
    ...
    function getAccount(address holder)
        public view returns (
            address account,
            int256 index,
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            address[] memory path = new address[](2);
            path[0] = dividendRouter.WETH();
            path[1] = address(everSelect);

            uint256 prevBalance = everSelect.balanceOf(address(this));
            ...
            {
                uint256 received = everSelect.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    everSelect.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
