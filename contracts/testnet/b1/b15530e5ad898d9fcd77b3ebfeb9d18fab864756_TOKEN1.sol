...
contract TOKEN1 is Context, IERC20, Ownable {
    ...
    uint256 public maxWalletSize = 2000000000 * 10**_decimals;
    ...
    uint256 public swapTokenThreshold = 5000 * 10**_decimals;
    uint256 public currentMarketingToken = 0;
    uint256 public currentBuybackToken = 0;
    

  
    address public rewardsPoolAddress = 0xbCE70CF5DFC639e819456483EbF355DcABd1056C;
   ...
   function setAntibot(address account, bool _bot) external onlyOwner{
    ...
    //input in BNB
    function fireBuyback(uint256 bnbAmount) external onlyOwner {
        uint256 amount = bnbAmount * 10**18;
    	if (amount > 0) {
            uint256 balance = address(this).balance;
            require(balance >= amount, "Buyback: amount exceeds balance");
            require(buyBackUpperLimit >= amount, "Buyback: amount exceeds upper limit");
    ...
    function adjustAmountsPostLaunch() external  {
        maxWalletSize = 40000000000 * 10**_decimals;
        maxBuyAmount  = 40000000000 * 10**_decimals;
        maxSellAmount = 10000000000 * 10**_decimals;
    }
   ...
   function setBuyFeeRates(uint256 marketing, uint256 rewardsPool, uint256 buyback) external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if( to != pair &&
            from != pair &&
            to != address(0xdead)) isTransfer = true; 
            
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to] || isTransfer) {
            unchecked {
   ...
   function _swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        //tokens should = currentMarketingToken + currentBuybackToken;
        uint256 denominator= (currentMarketingToken + currentBuybackToken);
        uint256 marketingTokenRate = currentMarketingToken / denominator;
        
        //reset fee token count
        ...
        uint256 marketingAmt = deltaBalance * marketingTokenRate;
        if(marketingAmt > 0){
          payable(marketingAddress).transfer(marketingAmt);
        }
        //Buyback amount in BNB will be left in the contract  
    }
