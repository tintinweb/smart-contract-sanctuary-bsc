    ...
    * @param user address of the user than will now have to pay transaction fees.
    ...
    function validateLaunchFirstBlocks(address from, address to, uint256 amount) internal view {
        ...
        if (block.timestamp < launchLockTime && !whitelistedAtLaunch[to]) {
            require(!pairAddresses[from], "You are not whitelisted at launch. Wait for the launch to be open to public");
        }
        //if launched AND recipient IS whitelisted, recipient can buy up to 2000 Tokens in the first minute(s)
        if (block.timestamp < launchLockTime && whitelistedAtLaunch[to]) {
            if (pairAddresses[from] && !excludedFromFees[from] && !excludedFromFees[to]) {
                require(amount < 2000000000000000000000, "You can't buy more than 2000 Lama Tokens at launch.");
            }
        }
    }
