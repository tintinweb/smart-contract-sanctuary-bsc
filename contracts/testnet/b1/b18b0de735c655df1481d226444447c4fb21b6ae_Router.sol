...
library Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
                ...
                hex'EEE4AD7FCB09C5358EEB00EF0AD91D8C3CB94B4DD3628F2C076CB613E4C53512' // init code hash
            )))));
    }
...
contract Router is IRouter {
    ...
    constructor(address _factory, address _WBNB, address _lpRewards) {
        require(_factory != address(0) && _WBNB != address(0) && _lpRewards != address(0), "Router: Zero address(es)");
        factory = _factory;
        WBNB = _WBNB;
    ...
    function addLiquidityBNB(
        ...
        address pair = Library.pairFor(factory, token, WBNB);
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWBNB(WBNB).transfer(Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactBNB(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        ensure(deadline)
    {
        ...
        assert(IWBNB(WBNB).transfer(Library.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IBEP20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IBEP20(path[path.length - 1]).balanceOf(to) - balanceBefore >= amountOutMin,
