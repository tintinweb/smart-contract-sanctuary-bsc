...
contract BineVerse is Ownable, IBEP20 {
    using SafeMath for uint256;

    address WBNB;
    address Reserve = 0x4a500B3f9C0d39EB87C6F6F645Ad14257D4dD780;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "BineVerse";
    string constant _symbol = "$BINE";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 2.1 * 10**6 * 10**_decimals;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(Reserve) && recipient != pair && recipient != stakingFeeReceiver && recipient != marketingFeeReceiver && !isTxLimitExempt[recipient]){
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return _totalSupply.sub(balanceOf(Reserve)).sub(balanceOf(ZERO));
    }
