...
contract MyContract {
    ...
    function withdrawCapital (uint256 ammount) public returns(uint256){
        ...
        User storage user = users[msg.sender];
        uint256 avaiableAmmount = availableWithdraw(msg.sender);
    	require( (avaiableAmmount >0) , "Withdraw condition not met");
    ...
    function availableWithdraw(address usr) public view returns(uint256){
        ...
        uint256 referralTotal = user.referralEggRewards;
        if (referralTotal < 1 * 1e17) {
            return 0;
        }else if (referralTotal < 5 * 1e17){
            return referralTotal;
        }else{
            return SafeMath.mul(referralTotal,2);
        }
    }
