...
// File: contracts/PodNFT.sol

pragma solidity 0.8.7;


contract PodNFT is LaunchpadNFT {

    constructor(string memory name_, string memory symbol_, string memory baseURI_, address launchpad) LaunchpadNFT(name_, symbol_, baseURI_, "", launchpad, 2**256 - 1) {                           
        baseURI = baseURI_;
        suffix = "";
        LAUNCHPAD = launchpad;
        LAUNCH_MAX_SUPPLY = 2**256 - 1;
    }

    function mintByOwner(address to, uint size) external onlyOwner {
        require(to != address(0), "can't mint to empty address");
        require(size > 0, "size must greater than zero");
        require(LAUNCH_SUPPLY + size <= LAUNCH_MAX_SUPPLY, "max supply reached");
