...
contract MetaEdgeTech is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _rewardFee,uint256 _marketingFee,address _FeeAddress,address tokenOwner) {
		_NAME = _name;
		_SYMBOL = _symbol;
		_DECIMALS = _decimals;
	...
	function updateFee(uint256 _txFee,uint256 _rewardFee,uint256 _marketingFee) onlyOwner() public{
        _LIQUIDITY_FEE = _txFee* 100; 
        _REWARD_FEE = _rewardFee * 100;
		_MARKETING_FEE = _marketingFee* 100;
		ORIG_LIQUIDITY_FEE = _LIQUIDITY_FEE;
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tFee, uint256 tBurn, uint256 tCharity) = _getTBasics(tAmount, _LIQUIDITY_FEE, _REWARD_FEE, _MARKETING_FEE);
    ...
    function _getTBasics(uint256 tAmount, uint256 liquidityFee, uint256 burnFee, uint256 charityFee) private view returns (uint256, uint256, uint256) {
        uint256 tFee = ((tAmount.mul(liquidityFee)).div(_GRANULARITY)).div(100);
    ...
    function removeAllFee() private {
        if(_LIQUIDITY_FEE == 3 && _REWARD_FEE == 4 && _MARKETING_FEE == 5) return;
