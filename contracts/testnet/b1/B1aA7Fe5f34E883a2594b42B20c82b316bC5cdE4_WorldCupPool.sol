    ...
    PausableUpgradeable
{
    ...
    mapping(uint256 => string) public heroRepresentCountry;

    //Each Hero reward ratio based on FIFA ranking
    mapping(uint256 => uint256) public heroRewardRatio;
    ...
    mapping(address => uint256[]) public userStakedTokenList;

    //Each Hero's owner
    mapping(uint256 => address) public heroOwnedBy;

    //Each hero initial stake time
    mapping(uint256 => uint256) public heroStakedTimeBlock;

    //Each hero total stake time
    mapping(uint256 => uint256[]) public heroTotalStakedTimeBlock;

    //Each hero total staking amount by all players
    mapping(uint256 => uint256) public heroTotalStakedAmount;
    ...
    NewDefinaCardV2 public definaCard;
    ...
    uint256 public constant totalMatches = 8; //TODO: change to 48
    ...
    uint256 public leaderboardTotalCredits;
    ...
    function setCountryInfo(
        ...
        require(
            _heroId.length == 16, //TODO: Change to 38
        ...
        for (uint256 i = 0; i < _heroId.length; i++) {
            ...
            groupMatchCountry.heroId.push(_heroId[i]);
            groupMatchCountry.heroRarity.push(_heroRarity[i]);
        }
    }
    ...
    function setGroupMatchCountries(
        string[] calldata _groupMatchCountries,
        GroupMatchCountryInfo[] calldata _groupMatchCountryInfos
    ) external onlyOwner {
        require(
            _groupMatchCountries.length == _groupMatchCountryInfos.length,
            "Array length must be same"
        );
        for (uint256 i = 0; i < _groupMatchCountries.length; i++) {
            groupMatchCountries[
                _groupMatchCountries[i]
            ] = _groupMatchCountryInfos[i];
        }
    }
    ...
    function _bet(uint256 _tokenId, uint256 _matchId, bool _isDraw, UserBetInfo memory userBet) view private {
        require(
            userBet.credits >= consumeCreditEachBet,
    ...
    function getUserBetInfo() public view returns(UserBetInfo memory) {
        return userBets[_msgSender()];
    }
    ...
    function claimBetCreditsByMatchId(uint256 _matchId) public onlyEOA {
        require(groupMatches[_matchId].result != 0, "Group Match Result is not revealed");
        GroupMatchInfo storage groupMatch = groupMatches[_matchId];
        UserBetInfo storage userBet = userBets[_msgSender()];
    ...
    function claimBetCreditsAll() external onlyEOA {
