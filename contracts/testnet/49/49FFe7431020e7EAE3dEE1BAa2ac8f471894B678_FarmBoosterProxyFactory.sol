...
// File: contracts/FarmBoosterFactory.sol
...
contract FarmBoosterProxy is ReentrancyGuard {
     ...
     * @param _floatieToken: the address of the floatie token
     */
    function initialize(
        address _admin,
        address _farmBooster,
        address _masterchefV2,
        address _floatieToken
    ) external {
        ...
        floatieToken = IERC20(_floatieToken);
    }
    ...
    function harvestFloatie() internal {
        uint256 floatieBalance = floatieToken.balanceOf(address(this));
        if (floatieBalance > 0) {
            floatieToken.safeTransfer(msg.sender, floatieBalance);
        }
    }
}
...
contract FarmBoosterProxyFactory {
    ...
    address public immutable floatieToken;
    ...
    function createFarmBoosterProxy() external {
        ...
        FarmBoosterProxy(farmBoosterProxyAddress).initialize(msg.sender, Farm_Booster, masterchefV2, floatieToken);
