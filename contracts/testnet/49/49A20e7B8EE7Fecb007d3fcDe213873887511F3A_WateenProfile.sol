 ...
 */
contract WateenProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public WTNToken;
    ...
    uint256 public numberWTNToReactivate;
    uint256 public numberWTNToRegister;
    uint256 public numberWTNToUpdate;
    ...
    constructor(
        IBEP20 _WTNToken,
        uint256 _numberWTNToReactivate,
        uint256 _numberWTNToRegister,
        uint256 _numberWTNToUpdate
    ) {
        WTNToken = _WTNToken;
        numberWTNToReactivate = _numberWTNToReactivate;
        numberWTNToRegister = _numberWTNToRegister;
        numberWTNToUpdate = _numberWTNToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends WTN to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer WTN tokens to this contract
        WTNToken.safeTransferFrom(_msgSender(), address(this), numberWTNToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer WTN token to this address
        WTNToken.safeTransferFrom(_msgSender(), address(this), numberWTNToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        WTNToken.safeTransferFrom(_msgSender(), address(this), numberWTNToReactivate);
    ...
    /**
     * @dev Claim WTN to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        WTNToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of WTN to register
     * Callable only by owner admins.
     */
    function updateNumberWTN(
        uint256 _newNumberWTNToReactivate,
        uint256 _newNumberWTNToRegister,
        uint256 _newNumberWTNToUpdate
    ) external onlyOwner {
        numberWTNToReactivate = _newNumberWTNToReactivate;
        numberWTNToRegister = _newNumberWTNToRegister;
        numberWTNToUpdate = _newNumberWTNToUpdate;
    }
 ...
 * other WateenProfile requirements.
 */
contract IFOV2 is IIFOV2, ReentrancyGuard, Ownable {
    ...
    // WateenProfile
    WateenProfile public wateenProfile;
     ...
     * @param _WateenProfileAddress: the address of the WateenProfile
    ...
    constructor(
        ...
        wateenProfile = WateenProfile(_WateenProfileAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        require(wateenProfile.getUserStatus(msg.sender), "Must have an active profile");
    ...
    function _claimPoints(address _user) internal {
        if (!_hasClaimedPoints[_user]) {
            ...
            if (sumPools > thresholdPoints) {
                ...
                wateenProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
            }
        }
    }
