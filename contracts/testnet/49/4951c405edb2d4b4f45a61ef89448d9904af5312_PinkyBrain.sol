...
Liquidity
0x9548615A816156569f2b70B3E42Bf35d9269c7Ae
...
contract PinkyBrain is Context, IERC20, Ownable {
    ...
    uint256 public _poolRewards = 3;
    uint256 private _previousPoolRewards = _poolRewards;
    ...
    uint256 public totalFeesTax = _taxFee.add(_poolRewards).add(_liquidityFee).add(_marketingFee);
    ...
    function calculateLiquidity(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_poolRewards.add(_liquidityFee)).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(_taxFee == 0 && _poolRewards == 0 && _liquidityFee ==0 && _marketingFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousPoolRewards = _poolRewards;
    ...
    function restoreAllFee() private {
       ...
       _poolRewards = _previousPoolRewards;
    ...
    function _transfer(
        ...
        if (
            ...
            //add liquidity
            uint256 totalTaxForSwap = _poolRewards.add(_marketingFee).add(_liquidityFee);
    ...
    function transferPoolRewards(address wallet) public onlyOwner {
        uint256 totalTaxForSwap = _poolRewards.add(_marketingFee).add(_liquidityFee);
        uint tokens = balanceOf(address(this)).mul(_poolRewards).div(totalTaxForSwap);
        IERC20(address(this)).transfer(wallet, tokens);
    }
    ...
    function setBotFees() private {
        ...
        _poolRewards = _previousPoolRewards * _botIncreaseFee;
    ...
    function setliquidityFee(uint256 liquidityFee) external onlyOwner() {
        ...
        totalFeesTax = _taxFee.add(_liquidityFee).add(_marketingFee).add(_poolRewards);
        require(totalFeesTax <= 25, "max 25%");
    }
