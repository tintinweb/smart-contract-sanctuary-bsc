...
interface TdexMining {
    ...
    function getExperience(address _tokenContract, address _sender) external view returns (uint256);
}
...
contract Robot {
    ...
    function mining() internal view returns (TdexMining)
    {
        return TdexMining(tdex().mining());
    }
    ...
    function getRevenue() external view returns (uint256 releasedRevenueOf, uint256 totalExtract)
    {
        releasedRevenueOf = mining().getReleasedRevenueOf(address(this));
        totalExtract = mining().totalExtract(address(this));
    }
    ...
    {
        return mining().getUserReceiveRecordLength(address(this));
    }

    function getUserReceiveRecords(uint[] memory __indexs) external view returns (ReceiveRecord[20] memory)
    {
        return mining().getUserReceiveRecords(address(this), __indexs);
    }
    ...
    function getTdexStatistics() external view returns (uint256 entrustAmount, uint256 volume)
    {
        uint length = tdex().getOrderUnmatchedListLength(_tokenContract, address(this));
        if (length > 0)
        {
            uint256[] memory list = tdex().getOrderUnmatchedList(_tokenContract, address(this), 0, length);
            for (uint i=0; i<list.length; i++)
            {
                if (list[i] > 0)
                {
                    (uint256 price,,uint256 tokenSurplus,uint256 tokenFee,uint256 usdtSurplus,uint256 usdtFee,,,uint8 orderType,,) = tdex().getOrder(_tokenContract, list[i]);
                    if (OrderType(orderType) == OrderType.Buy)
                    {
                        entrustAmount += (tokenSurplus + tokenFee) * price / 1e8;
                    }
                    else if (OrderType(orderType) == OrderType.Sell)
                    {
                        entrustAmount += (usdtSurplus + usdtFee);
                    }
                }
            }
        }
        volume = mining().getExperience(_tokenContract, address(this));
    }
    ...
    function entrustOrders(bool[] memory _isBuys, uint256[] memory _prices, uint256[] memory _amount, bool _isUStandard) external onlyTrader
    {
        gasRecharge();
        Approve();
        ...
        for (uint i=0; i<_prices.length; i++)
        {
            if (_isUStandard)
            {
                _amount[i] = _amount[i] / _prices[i] * 1e8;
            }
    ...
    function RunLiquidity(uint256 _amount, bool _isUStandard) external onlyTrader
    {
        Approve();
        uint256 price = GetLiquidityPrice();
        if (_isUStandard)
        {
             _amount = _amount / price * 1e8;
        }
        if (_tokenContract != constant_ETH)
        {
