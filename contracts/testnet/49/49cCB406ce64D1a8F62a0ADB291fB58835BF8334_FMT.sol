...
contract FuMeta is Context, IERC20, Ownable,ReentrancyGuard {
    ...
    IPair mainPairC;
    address mainPair;
    address[2] public path_FMTtoUSDT;
    ...
    ) 
    {
        ...
        USDT = IERC20(0xB6407C5542E7aAB6aB84D560245F1151f966b8B9);
        ...
        path_FMTtoUSDT[0]=address(this);
        path_FMTtoUSDT[1]=address(USDT);
    ...
    function setContract(address _stake,address _surpriseAddr,address _pp,address _U,address _manager)public onlyOwner{
        stakeAddress=_stake;
        surpriseAddress=_surpriseAddr;
        ppAddress=_pp;
        USDT = IERC20(_U);
        decimalsU = USDT.decimals();
    ...
    function setChildContract(address _addr,bool _state)public onlyOwner{
        ...
        _state?childList.push(_addr):getIndexAndDelOfAddr(childList,_addr);
    }
    ...
    function getUserTeamr(address _user,uint8 _layer,uint _begin,uint _end)public view returns(address[] memory _teamr){
        if(getUserTeam[_user][_layer].length>=_begin+1){
            for(uint i=0;i<=(_end-_begin+1);i++){
                if(getUserTeam[_user][_layer][i+_begin]!=address(0)){
                    _teamr[i]=getUserTeam[_user][_layer][i];
                }else{
                    break;
                }
            }
    ...
    function safeRegisterUser(address _userAddress,address _inviter) external onlyChild returns(bool) {
    ...
    function fmtBurn(uint _amount)public onlyChild{
        ...
        if(unburn>0){
            ...
            unchecked {
                _balances[_sender] = _senderBalance - _burnAmount;
            }
            _totalSupply -= _burnAmount;
            unburn-=_burnAmount;
            
            emit Transfer(_sender, address(0), _burnAmount);
        }
        if(_amount>_burnAmount){
            _basicTransfer(_sender,fundAddr,_amount-_burnAmount);
        }
    }
    function addBuyAmount(address _user,uint _amount)external onlyChild{
        getUserData[_user].buyAmount += _amount;
    }
    function basicTransfer(address from,address to,uint256 amount) external onlyChild{
        _basicTransfer(from,to,amount);
    }
    function pTransfer(address to,uint256 amount) external onlyChild{
        _pTransfer(to,amount);
    }

    
    modifier onlyChild() {
        require(isChild[msg.sender],"Only Child Contract");
        _;
    }
    ...
    function setReleaseCycle(uint _rCycle) public onlyOwner {
        releaseCycle = _rCycle;
    }
    ...
    function _safeRegisterUser(address _userAddress,address _inviter) private{
        ...
        for(uint8 i=1;i<=8;i++){
            address _addr=getUserData[_inviter].inviterAddr[i];
            if(_addr!=address(0)){
...
contract FMT is FuMeta {
    constructor() FuMeta(
        ...
        0xe5132B78f963415e7762f79F41f2a32889bf732F,
        300
    ){
