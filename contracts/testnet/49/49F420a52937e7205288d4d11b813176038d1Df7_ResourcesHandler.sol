...
contract ResourcesHandler is AccessControl, ReentrancyGuard, Pausable {
    ...
    function _regenItemEnergy(uint _recipeId, uint _itemId) internal {
        require(items.ownerOf(_itemId) == msg.sender || IMarketplace(marketplace).getSellerFor(address(items), _itemId) == msg.sender, "Address not owner of item");
    ...
    function updateTaxAccount(address _MoDTaxAccount) external onlyRole(GAME_CONTROL){
