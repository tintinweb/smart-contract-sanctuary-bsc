     ...
     constructor(
      // address _manager
        ...
        **/
        ...
        function ADMIN_referansKoduIledanCuzdaniBul(uint _referansKodu) external view sahip() returns(address){
            return referansKodundanCuzdaniBul[_referansKodu];
        }
        function ADMIN_cuzdanAdresindenReferansKodunuBul (address _cuzdan) external view sahip() returns(uint){
            return cuzdandanReferansKodunuBul[_cuzdan];
        }
        function ADMIN_buAdresinREFERANSIKIM(address _bu) external view sahip() returns(address) {
            return buAdresinREFERANSI[_bu];
        }
        function ADMIN_buAdresinBakiyesiniSorgula(address _bu) external view sahip() returns(uint){
            return bakiye[_bu];
        }
        function ADMIN_kayitSorgula(address _cuzdanadresi) external view sahip() returns(bool) {
        ...
        function KAYDIMI_SORGULA() external view returns(bool){
            return kayit[msg.sender];
        }
        function bakiyemiSorgula() external view returns(uint){
            return bakiye[msg.sender];
        }

        function REFERANS_KODUMU_SORGULA() external view returns(uint){
            return cuzdandanReferansKodunuBul[msg.sender];
        }

        function KiM_TARAFINDAN_DAVET_EDiLDiM() external view returns(address){
