...
contract GFi is Context, IBEP20, Ownable, ReentrancyGuard{ 
    using SafeMath for uint256;
    ...
    address private devReceiver             = 0x5F45ac78d52233b463fF48A75160d0129035e16a;
    address private autoLiquidityReceiver;
    address public ecosystemReceiver;
    address private reserveReceiver;

    string constant _name                           = "GFi";
    ...
    constructor () {
        ...
        totalFeeBuy 				                = buyLiquidityFee + buyRewardFee + buyEcosystemFee + buyDevFee + buyReserveFee;
        totalFeeSell 				                = sellLiquidityFee + sellRewardFee + sellEcosystemFee + devFee + sellReserveFee;
    ...
    function setBuyFees(uint256 _buyLiquidityFee, uint256 _buyEcosystemFee, uint256 _buyReserveFee, uint256 _feeDenominator) external onlyOwner {
        ...
        feeDenominator          = _buyLiquidityFee + buyRewardFee + _buyEcosystemFee + buyDevFee + _buyReserveFee;
        require(totalFeeBuy < _feeDenominator/10, "Fees cannot be lesser than 10%");
        require(totalFeeBuy < _feeDenominator/6, "Fees cannot be more than 16,66%");
    }

    function setSellFees(uint256 _sellLiquidityFee, uint256 _sellEcosystemFee, uint256 _sellReserveFee, uint256 _feeDenominator) external onlyOwner {
 	    sellLiquidityFee 	    = _sellLiquidityFee;
 	    sellRewardFee 		    = 500;
 	    sellEcosystemFee 	    = _sellEcosystemFee;
 	    sellDevFee 	            = 100;
 	    sellReserveFee          = _sellReserveFee;
 	    totalFeeSell 		    = _sellLiquidityFee + sellRewardFee + _sellEcosystemFee + sellDevFee + _sellReserveFee;
        feeDenominator          = _feeDenominator;
        require(totalFeeSell < _feeDenominator/10, "Fees cannot be lesser than 10");
        require(totalFeeSell < _feeDenominator/6, "Fees cannot be more than 16,66%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _ecosystemReceiver, address _reserveReceiver) external onlyOwner {
