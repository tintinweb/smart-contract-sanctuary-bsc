...
contract PancakeSwapOperator is IPancakeSwapOperator {
    ...
    function swapExactETHForTokens(IERC20 sellToken, IERC20 buyToken)
        external
        payable
        override
        returns (uint256[] memory amounts, address[] memory tokens)
    {
        ...
        console.log("Router: %s", operatorStorage.router());
        
        address[] memory path = new address[](2);
        path[0] = address(sellToken);
        path[1] = address(buyToken);

        IPancakeRouter02(operatorStorage.router()).swapExactETHForTokens{
            value: msg.value
