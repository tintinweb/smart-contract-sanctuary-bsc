...
contract Santa is IERC20, Ownable {
    ...
    uint256 public buyDonationFee = 50;
    uint256 public buyDevFee = 50;
    uint256 public buyTotalFees = 500;
    // sell fees
    uint256 public sellRewardFee = 300;
    uint256 public sellMarketingFee = 100;
    uint256 public sellDonationFee = 50;
    uint256 public sellDevFee = 50;
    uint256 public sellTotalFees = 500;
    ...
    uint256 public marketingSwap = 100;
    ...
    function updateBuyFees(
        ...
        buyDonationFee = donation;
        buyDevFee = dev;

        buyTotalFees = reward.add(marketing).add(donation).add(dev);

        require(
            buyTotalFees.add(sellTotalFees) <= 2500,
            "Fees can not be greater than 25%"
        );
    }
    ...
    function updateSellFees(
        ...
        sellDonationFee = donation;
        sellDevFee = dev;

        sellTotalFees = reward.add(marketing).add(donation).add(dev);
    ...
    function updateSwapPercentages(
        ...
        donationSwap = donation;
        devSwap = dev;

        totalSwap = reward.add(marketing).add(donation).add(dev);
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 busdForReward = swappedTokensAmount.mul(rewardSwap).div(
            totalSwap
        );

        uint256 busdForMarketing = swappedTokensAmount.mul(marketingSwap).div(
            totalSwap
        );

        uint256 busdForDonation = swappedTokensAmount.mul(donationSwap).div(
            totalSwap
        );

        uint256 busdForDev = swappedTokensAmount
            .sub(busdForDonation)
            .sub(busdForMarketing)
            .sub(busdForReward);
    ...
    function setMaxWalletToken(uint256 amount) external onlyOwner {
        require(amount <= 1000000000, "max wallet token should grater than 1%");
