...
//contract BatchAuction {
  ...
  uint128 private CLAIMABLE_PERIOD;

  /// @dev to check commit currency is BNB or BEP20 token
  ...
  // TODO:
  // BSC Testnet
  /// @dev The placeholder BNB address.
  address private constant ETH_ADDRESS = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE; // BNB
  address private constant WETH = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd; // WBNB
  address private constant HOTBODY = 0xA1F92046c84Bf0067cD2c7aCdaDe986Ec81FC0D6;
  address private constant USDT = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;

  IPancakeFactory public pancakeFactory;
  address private constant PancakeFactoryAddress = 0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc;
  address public PANCAKE_FACTORY_POOL_ADDRESS;

  IPancakeRouterV2 public pancakeRouterV2;
  address private constant PancakeRouterV2Address = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
  ...
  event BNBCommitted(address indexed _user, uint256 _amount);
  ...
  event BNBWithdrawan(address indexed _user, uint256 _amount);
  ...
  event PancakeSwapPoolCreated(address, uint256, uint256, uint256);

  /* ========== TEMPORARY FUNCTIONS FOR DEV ========== */
  /* ========== TEMPORARY FUNCTIONS FOR DEV ========== */
  /* ========== TEMPORARY FUNCTIONS FOR DEV ========== */
  function changetotalOfferingTokens(uint256 _totalOfferingTokens) external {
  ...
  /* ========== CONSTRUCTOR ========== */
  constructor() {
    WITHDRAW_CAP_MIN_THRESHOLD = 1e18; // 1 
    WITHDRAW_CAP_MAX_THRESHOLD = 15e18; // 15
    WITHDRAW_CAP_LIMIT = 1.5e18; // 1.5 
    WITHDRAW_CAP_INTERCEPT = 285714290000000000000;
    WITHDRAW_CAP_INTERCEPT_PLUS = 714285710000000000000;
    ...
    IERC20(HOTBODY).safeApprove(PancakeRouterV2Address, 0);
    IERC20(HOTBODY).safeApprove(PancakeRouterV2Address, type(uint256).max);
   ...
   * @notice Commit BNB
   */
  function commitBNB() external payable {
    if (!DEBUG) {
    ...
    emit BNBCommitted(msg.sender, msg.value);
  }
  ...
  function commitERC20Token(uint256 _amount) public {
    require(COMMIT_CURRENCY != ETH_ADDRESS, "INVALID_COMMIT_CURRENCY");
  ...
  //function withdrawBNB(uint256 _amount) public nonReentrant {
  function withdrawBNB(uint256 _amount) external {
    ...
    emit BNBWithdrawan(msg.sender, _amount);
  }
  ...
  function claimVestedLPToken(bool _stake) external {
    require(auctionData.endTime < block.timestamp, "AUCTION_NOT_ENDED");
    require(isValidClaimablePeriod(), "CLAIMABLE_PERIOD_ENDED");
    require(COMMITMENTS[msg.sender] > 0, "NO_COMMITMENTS");
    require(isAuctionSuccessful(), "AUCTION_SHOULD_BE_SUCCESSFUL");
    ...
    if (_stake) {
      require(isStakingAvailable(), 'STAKING_SHOULD_BE_AVAILABLE');
  ...
  function withdrawUnclaimedLPToken() external onlyOwner {
    ...
    //IERC20(BALANCER_BPT).safeTransfer(AUCTION_TREASURY, amount);
  ...
  function getCommitmentBack() external {
    ...
    if(COMMIT_CURRENCY == ETH_ADDRESS) {
      safeTransferBNB(payable(msg.sender), userCommited);
    } else { // ERC20 Token
  ...
  function finalizeAuction() public onlyOwner {
    ...
    if (isAuctionSuccessful()) {
      ...
      if(COMMIT_CURRENCY == ETH_ADDRESS) {
        safeTransferBNB(AUCTION_WALLET, transferAmount);
      } else {
        IERC20(COMMIT_CURRENCY).safeTransfer(AUCTION_WALLET, transferAmount);
      }

      // 50% of funds & 50% of AUCTION_TOKEN instantly goes to the POOL
      uint256 token1Amount = auctionData.totalOfferingTokens.div(2); // 50%
      uint256 token2Amount = auctionData.totalCommitments.div(2); // 50%

      (PANCAKE_FACTORY_POOL_ADDRESS, auctionData.totalLPTokenAmount) =
      ...
      require(PANCAKE_FACTORY_POOL_ADDRESS != address(0), "INVALID_PANCAKE_POOL_ADDRESS");
  ...
  function safeTransferBNB(address payable to, uint value) internal {
  ...
  function setupPancakeSwapPool(
    ...
    if(!DEBUG) {
      require(_token1Amount> 0, "INVALID_TOKENA_AMOUNT");
      require(_token2Amount> 0, "INVALID_TOKENB_AMOUNT");
      require(auctionData.endTime < block.timestamp, "AUCTION_NOT_ENDED");
      require(isAuctionSuccessful(), "AUCTION_SHOULD_BE_SUCCESSFUL");
    }
    ...
    if(_token2 == ETH_ADDRESS) {
      // addLiquidity will create a pool pair if it doesn't exist
      ...
      // token2 should be WETH(WBNB)
      poolAddress = IPancakeFactory(pancakeFactory).getPair(_token1, WETH);
    }

    emit PancakeSwapPoolCreated(poolAddress, token1Result, token2Result, LPTokenAmount);

    return (poolAddress, LPTokenAmount);
  }

  /// @notice Returns lp token amount
  // TODO::
  ...
  // TODO: 
  function isValidClaimablePeriod() internal view returns (bool) {
  ...
  function getTotalVestedLPTokenAmount(address _user) public view returns (uint256) {
    ...
    uint256 userShare = COMMITMENTS[_user].mul(4).div(5); // 80%
