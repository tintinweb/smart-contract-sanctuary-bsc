...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - see https://github.com/BEPereum/solidity/issues/2691
        return msg.data;
    }
}
...
library ECDSA {
     ...
     * be too long), and then calling {toBEPSignedMessageHash} on it.
     */
    function recover(bytes32 hash, bytes memory signature) internal pure returns (address) {
        ...
        // unique. Appendix F in the BEPereum Yellow paper (https://BEPereum.github.io/yellowpaper/paper.pdf), defines
    ...
    /**
     * @dev Returns an BEPereum Signed Message, created from a `hash`. This
     * replicates the behavior of the
     * https://github.com/BEPereum/wiki/wiki/JSON-RPC#BEP_sign[`BEP_sign`]
     * JSON-RPC mBEPod.
     *
     * See {recover}.
     */
    function toBEPSignedMessageHash(bytes32 hash) internal pure returns (bytes32) {
...
contract EliteBridge is Ownable {
    ...
    */
    ...
    function claim(address _user,address _tokenAddress,uint amount, bytes calldata signature,uint _time) public isLock isApprove(_tokenAddress) {
        ...
        IBEP20(_tokenAddress).transfer(_user,amount);
        msgHash[messageHash] = true;
        emit Claim(_user,_tokenAddress,amount,block.timestamp);
    }
    
    /**
    * @dev BEPereum Signed Message, created from `hash`
    ...
    */
    ...
    {
        bytes32 hash = ECDSA.toBEPSignedMessageHash(_messageHash);
    ...
    */
    ...
    function failsafe(address user,address tokenAddress,uint amount)public onlyOwner{
        if(tokenAddress == address(0x0)){
            ...
            IBEP20(tokenAddress).transfer(user,amount);
        }
