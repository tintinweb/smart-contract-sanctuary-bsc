...
contract MMM is Ownable{
    ...
    function withdraw() external {
        ...
        if(total_wd < user.withdrawable + user.refReward){
            if(user.withdrawable <= total_wd){
                totalWithdraw += user.withdrawable;
                user.recWithdrawable -= user.withdrawable;
                total_wd -= user.withdrawable;
                user.withdrawable = 0;
                
                user.refReward -= total_wd;
                user.recRefReward += total_wd;
                totalReferWithdraw += total_wd;
            }else{
                user.withdrawable -= total_wd;
                user.recWithdrawable += total_wd;
                totalWithdraw += total_wd;
            }
        }else{
            user.recWithdrawable += user.withdrawable;
            totalWithdraw += user.withdrawable;
