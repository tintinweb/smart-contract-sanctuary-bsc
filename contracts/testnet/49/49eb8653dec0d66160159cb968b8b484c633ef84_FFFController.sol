...
contract FFFController is Ownable {
    ...
    constructor() {
        ...
        //teamLeader
        // teamLeader[1] = address(0xf8ED7d92d6209ec070223DCaD57F98E6C9feE7a5);
        // teamLeader[2] = address(0xC5a89985D57FdddEFCE3E9f26faB99EbF471516A);
        // teamLeader[3] = address(0xf34a7238cF9423A5FC6852c8882d774BF08446DE);
    ...
    function queryNodeProfit(uint256 nodeType) public view returns(uint256 profit, uint256 profitToday) {
        profit = nodeProfit[nodeType];
        profitToday = nodeProfitToday[nodeType];
    }
    ...
    function queryNodeTokenReward(address user) public view returns(uint256 total, uint256 draw, uint256 notDraw, uint256 canDraw) {
        ...
        total = nodeTokenReward[nodeType];
        draw = tokenForTimeRelease[nodeId] + tokenForPriceRelease[nodeId];
        notDraw = total - draw;
        canDraw = _queryNodeTokenTotalRelease(msg.sender);
    }
    ...
    function queryMyNodeReward(address user) public view returns(uint256 total, uint256 expected, uint256 draw, uint256 canDraw) {
        ...
        total = userNodeProfits[user];
        expected = total.mul(3 * 365 days).div(block.timestamp - nodeOpenTime[nodeId]);
        draw = userDrawNodeProfits[user];
        canDraw = total - draw;
    }

    function queryNodeRecommendReward(address user) public view returns(uint256 cumulative, uint256 draw, uint256 canDraw) {
        cumulative = userNodeRecommendeProfits[user];
        draw = userDrawNodeRecommendeProfits[user];
    ...
    function queryMyInviteesData(address user) public view returns(uint256[] memory levels, address[] memory inviteess, uint256[] memory powers, uint256[] memory underPowers) {
