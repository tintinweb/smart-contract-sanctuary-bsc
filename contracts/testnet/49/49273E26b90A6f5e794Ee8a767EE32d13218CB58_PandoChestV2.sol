...
contract PandoChestV2 is Ownable, Pausable {
        ...
        for(uint256 i = 0; i < _tokens.length; i++){
            ...
            if(pools.length > 0){
                ...
                for(uint256 i = 0; i < pools.length; i++){
                    ...
                    emit RewardAllocated(pools[i].pool, _token, _amount, _allocateDay, _allocateDay * (1 days));
                }
            }
    ...
    function directAllocate(address _pool, address _token, uint256 _allocationAmount, uint256 _second, bool _isSecond) external onlyOwner{
        IERC20(_token).safeTransferFrom(msg.sender,address(this), _allocationAmount);
        if(_isSecond) {
            IPandoAssembly(_pool).allocateForSecond(_allocationAmount , _second);
        } else {
            IPandoAssembly(_pool).allocateMoreRewards(_allocationAmount , _second / (1 days));
        }
        emit RewardAllocated(_pool, _token, _allocationAmount, _second / (1 days), _second);
    }
    ...
    event RewardAllocated(address _pool, address _token,uint256 _amount, uint256 _days, uint256 _second);
