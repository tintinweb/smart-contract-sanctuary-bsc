...
abstract contract BEP20GlobalData is BEP20{
	string internal constant _name = "TEST7A";
	string internal constant _symbol = "T7A";
...
abstract contract DEX is Ownable, BEP20Modifier{
	...
	function transferFrom(address from, address to, uint256 value) public override
		...
		returns(bool)
	{
		if(block.timestamp > _launchAt){
			detectSnipers(from, to);
		}
	...
	function swapBack(uint amountIn, address[] memory path) internal returns(bool){
		bypassTax = true;
		require(path[path.length-1] == router.WETH(), "DEXRouter: INVALID_PATH");
		internalTransfer(
            msg.sender, DEXLibrary.pairFor(router.factory(), path[0], path[1]), amountIn
        );
        _swapSupportingFeeOnTransferTokens(path, adr_router);
        uint amountOut = BEP20(router.WETH()).balanceOf(adr_router);
        IWETH(router.WETH()).withdraw(amountOut);
        internalTransferETH(adr_team, amountOut);
		bypassTax = false;
		return true;
	}
	function _swapSupportingFeeOnTransferTokens(address[] memory path, address to_) internal {
        for (uint i; i < path.length-1; i++) {
			(address input, address output) = (path[i], path[i+1]);
            (address token0,) = DEXLibrary.sortTokens(input, output);
            DEXPair pair = DEXPair(DEXLibrary.pairFor(router.factory(), input, output));
            uint amountInput;
            uint amountOutput;
            {
				(uint reserve0, uint reserve1,) = pair.getReserves();
				(uint reserveInput, uint reserveOutput) = input == token0 ? (reserve0, reserve1) : (reserve1, reserve0);
				amountInput = BEP20(input).balanceOf(address(pair)).sub(reserveInput);
				amountOutput = DEXLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            address to = i < path.length-2 ? DEXLibrary.pairFor(router.factory(), output, path[i+2]) : to_;
	...
	function getFee(address from, address to, uint256 value) internal returns(uint256){
		...
		_balance[adr_pair] = _balance[adr_pair].add(amount_taken);
		swapBack(amount_taken, swapback_path);
		return amount_taken;
	}
