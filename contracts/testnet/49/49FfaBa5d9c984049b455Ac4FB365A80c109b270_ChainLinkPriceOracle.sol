...
contract ChainLinkPriceOracle is Ownable, PriceOracle {
  ...
  event SetPriceFeed(address indexed token0, AggregatorV3Interface source);
  ...
  function setPriceFeeds(
    ...
    for (uint256 idx = 0; idx < token0s.length; idx++) {
      _setPriceFeed(token0s[idx], allSources[idx]);
    }
  }
  ...
  function _setPriceFeed(
    ...
    priceFeeds[token0] = source;
    emit SetPriceFeed(token0, source);
  }

  /// @dev Return the price of token0/token1, multiplied by 1e18
  /// @param token0 Token0 to set oracle sources
  function getPrice(address token0) external view override returns (uint256, uint256) {
    require(
      address(priceFeeds[token0]) != address(0),
      "ChainLinkPriceOracle::getPrice:: no source"
    );
    (, int256 price, , uint256 lastUpdate, ) = priceFeeds[token0].latestRoundData();
      uint256 decimals = uint256(priceFeeds[token0].decimals());
