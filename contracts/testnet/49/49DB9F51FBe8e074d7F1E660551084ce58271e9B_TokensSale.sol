...
contract TokensSale is AccessControlEnumerable {
    ...
    function _getListCurrentBatchSale(uint256 time)
        private
        view
        returns (uint256[] memory)
    {
        uint[] memory currentBatchSale;
        uint j = 0;
    ...
    function activeBatchSalesByTime(uint256 time)
        public
        view
        returns (uint256[] memory currentBatchSales)
    {
        return _getListCurrentBatchSale(time);
    }
}
