...
contract TheDotPayBank is ITheDotPayBank,Context {
   ...
   mapping(address=>MerchantDetails) private _merchants;
   ...
   event AddMerchant(address indexed merchantOwnerAddress_,address indexed verifierAddress_,address indexed merchantContractAddress,uint256  currentShare_,uint256  sharingCliffValue_);
   ...
   event AddVerifier(address indexed verifierAddress_,address indexed callerAddress_);
   event RemoveVerifier(address indexed verifierAddress_,address indexed callerAddress_);
   event WithdrawBNB(uint indexed amount_,address indexed callerAddress_);
   event WithdrawTokens(address indexed tokenAddress_, uint256 indexed amount_,address indexed callerAddress_);
   ...
   function getMerchantOwner(address merchantContractAddress_) external view returns(address){
   ...
   function getCurrentShare(address merchantContractAddress_) external view returns(uint256){
       IMerchant merchant=IMerchant(merchantContractAddress_);
       return(merchant.getCurrentShare());
   }

   function getSharingCliffValue(address merchantContractAddress_) external view returns(uint256){
       IMerchant merchant=IMerchant(merchantContractAddress_);
       return(merchant.getSharingCliffValue());
   }

   function getMerchantStatus(address merchantContractAddress_) external view returns(bool){
       IMerchant merchant=IMerchant(merchantContractAddress_);
   ...
   function getMerchantContractAddress(address merchantOwnerAddress_) external view returns(address){
      require(_merchants[merchantOwnerAddress_].status==true,"Address does not ownes a contract address");
      return(_merchants[merchantOwnerAddress_].contractAddress);
   }
   ...
   function addMerchant(address merchantOwnerAddress_,uint256 currentShare_,uint256 sharingCliffValue_) external isVerifier returns(address){
      require(_merchants[merchantOwnerAddress_].status!=true,"Already owner of a merchant contract address");
      Merchant merchant=new Merchant(merchantOwnerAddress_,address(this),currentShare_,sharingCliffValue_);
      _merchants[merchantOwnerAddress_].status=true;
      _merchants[merchantOwnerAddress_].contractAddress=address(merchant);
      emit AddMerchant( merchantOwnerAddress_, _msgSender(), address(merchant),  currentShare_,  sharingCliffValue_);
   ...
   function addVerifier(address verifierAddress_) external isContractOwner returns(bool){
      ...
      emit AddVerifier(verifierAddress_,_msgSender());
      return(true);
   }
   ...
   function removeVerifier(address verifierAddress_) external isContractOwner returns(bool){
     ...
     emit RemoveVerifier(verifierAddress_,_msgSender());
     return(true);
   }
   ...
   function withdrawBNB(uint amount_) external isContractOwner returns(bool){
      ...
      emit WithdrawBNB(amount_,_msgSender());
      return(true);
   }
   ...
   function withdrawTokens(address tokenAddress_, uint256 amount_) external isContractOwner returns(bool){
      ...
      emit WithdrawTokens(tokenAddress_, amount_,_msgSender());
      return(true);
   }
