...
contract DOGEINU2 is Context, IERC20, Ownable {
    ...
    // tax fees on buying
    uint256 private liquidityFeeBuy = 1;
    uint256 private MarketingFeeBuy = 5;
    uint256 private BuyBackFeeBuy = 4;
    // tax fees on selling
    uint256 private liquidityFeeSell = 4;
    uint256 private MarketingFeeSell = 12;
    uint256 private BuyBackFeeSell = 4;
    // Max buy and sell  amount in percentage
    uint256 private MaxBuyAmt = 1;
    uint256 private MaxSellAmt = 1;
    // Max wallet amount
    uint256 private MaxWalletAmt = 3;
    ...
    mapping(address => bool) private _isWalletFreezed;
    mapping(address => bool) private _specialAddress;
    ...
    constructor(
        ...
        BuyBackAddress = payable(BuyBackAddress_);
    ...
    function initContract() internal {
        ...
        _isExcludedFromFee[address(BuyBackAddress)] = true;
        _isExcludedFromFee[address(marketingAddress)] = true;
        _isExcludedFromFee[address(liquidityAddress)] = true;
        _isExcludedFromFee[address(uniswapV2Router)] = true;

        _specialAddress[owner()] = true;
        _specialAddress[address(BuyBackAddress)] = true;
        _specialAddress[address(marketingAddress)] = true;
        _specialAddress[address(liquidityAddress)] = true;
    }
    ...
    function setMaxBuyAmount(uint256 _MaxBuyAmt) public onlyOwner {
        MaxBuyAmt = _MaxBuyAmt;
    }

    function setMaxSellAmount(uint256 _MaxSellAmt) public onlyOwner {
        MaxSellAmt = _MaxSellAmt;
    }

    function setMaxWalletAmt(uint256 MxWalletSize) public onlyOwner {
        MaxWalletAmt = MxWalletSize;
    }
    ...
    function setBuyingFee(
        ...
        MarketingFeeBuy = MarketingFee;
        BuyBackFeeBuy = BuyBackFee;
    }

    function setSellingFee(
        uint256 liquidityFee,
        uint256 MarketingFee,
        uint256 BuyBackFee
    ) public onlyOwner {
        liquidityFeeSell = liquidityFee;
        MarketingFeeSell = MarketingFee;
        BuyBackFeeSell = BuyBackFee;
    }
    ...
    function _transfer(
        ...
        if (_specialAddress[from] || _specialAddress[to]) {
            ...
            setCorrectFees(isSell(from, to));

            uint256 BuytxnLt = (totalSupply() / 100) * MaxBuyAmt;
            uint256 SelltxnLt = (totalSupply() / 100) * MaxSellAmt;
            uint256 walletLt = (totalSupply() / 100) * MaxWalletAmt;

            if (isBuy(from, to) && !isExcludedFromFee(to)) {
                require(amount <= BuytxnLt, "txn amount exceeds 1% limit");
                require(
                ...
                require(
                    !_isWalletFreezed[from],
                    " your wallet is currently freezed "
                );
                require(amount <= SelltxnLt, "txn amount exceeds 1% limit");
                _tokenTransferWithTax(from, to, amount);
            } else {
                _transferStandard(from, to, amount);
            }
    ...
    function _tokenTransferWithTax(
        ...
        _tOwned[BuyBackAddress] += BuyBackTax;
        ...
        emit Transfer(sender, BuyBackAddress, BuyBackTax);
        ...
        )
    {
        return (
            ...
            _amount.mul(_BuyBackFee).div(10**2),
    ...
    function FreezeWallet(address wallet) public onlyOwner {
        _isWalletFreezed[wallet] = true;
    }

    function UnFreezeWallet(address wallet) public onlyOwner {
        _isWalletFreezed[wallet] = false;
    }
    ...
    function addSpecialAddr(address addr) public onlyOwner {
        _specialAddress[addr] = true;
    }

    function removeSpecialAddr(address addr) public onlyOwner {
        _specialAddress[addr] = false;
    }
}