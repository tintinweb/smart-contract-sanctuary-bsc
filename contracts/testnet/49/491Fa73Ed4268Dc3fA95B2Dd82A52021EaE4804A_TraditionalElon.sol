...
contract TraditionalElon is Context, IBEP20, Ownable, ReentrancyGuard{ 
    using SafeMath for uint256;
    ...
    address public externalAddress       = 0x02Ef67124D03d90d74B63989d110A8fd7718C26c;
    address public autoLiquidityReceiver = 0x02Ef67124D03d90d74B63989d110A8fd7718C26c;

    string constant _name    = "Traditional Elon";
    ...
    uint256 public externalFee       = 5;
    uint256 public sellExternalFee   = 5;

    uint256 public liquidityFee      = 2;
    uint256 public sellLiquidityFee  = 2;

    uint256 public totalBNBFee        = reflectionFee + externalFee + liquidityFee;
    uint256 public totalSellFee       = sellReflectionFee + sellExternalFee + sellLiquidityFee;
    ...
    function setFees(uint256 _reflectionFee, uint256 _sellReflectionFee, uint256 _externalFee, uint256 _sellExternalFee, uint256 _liquidityFee, uint256 _sellLiquidityFee, uint256 _feeDenominator) external onlyOwner {

        reflectionFee     = _reflectionFee;
        sellReflectionFee = _sellReflectionFee;
        externalFee       = _externalFee;
        sellExternalFee   = _sellExternalFee;
        liquidityFee      = _liquidityFee;
        sellLiquidityFee  = _sellLiquidityFee;

        totalBNBFee       = _reflectionFee + _externalFee + _liquidityFee;
        totalSellFee      = _sellReflectionFee + _sellExternalFee + _sellLiquidityFee;

        feeDenominator    = _feeDenominator;
        require(totalBNBFee < feeDenominator, "Fees cannot be more than 99%");
        require(totalSellFee < feeDenominator, "Fees cannot be more than 25%");
    }
    ...
    function setBUSDDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) external onlyOwner {
        distributor.setDistributionCriteria(_minPeriod, _minDistribution);
    }

    function setBUSDDistributorSettings(uint256 gas) external onlyOwner {
        require(gas < 750000);
        distributorGas = gas;
    }
