...
contract Marketplace1155 is ERC1155HolderUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getCollectionBoost(address collection) public view returns (uint256) {
        uint256 boostSum = 0;
        for (uint256 i = 1; i <= _lastListingId; i++) {
            if ( listings[i].collection == collection ) {
                boostSum += listingsAdditional[i].boost;
            }
        }
        return boostSum;
    }

    function getCollectionBoostBatch(address[] memory collections) public view returns (uint256[] memory) {
        uint256[] memory list = new uint256[](collections.length);
        for (uint i=0; i< collections.length; i++) {
            list[i] = getCollectionBoost(collections[i]);
        }
        return list;
    }
