...
contract ARYZE_LimitOrdersModule is
    IModule,
    Order,
    Initializable,
    PausableUpgradeable,
    ...
    UUPSUpgradeable
{
    ...
    function execute(
        ...
        (address outputToken, uint256 minReturn) = abi.decode(_data, (address, uint256));

        IHandler handler = abi.decode(_auxData, (IHandler));

        // Do not trust on _inputToken, it can mismatch the real balance
        uint256 inputAmount = ARYZE_Utils.balanceOf(_inputToken, address(this));
        _transferAmount(_inputToken, payable(address(handler)), inputAmount);

        handler.handle(_inputToken, outputToken, inputAmount, minReturn, _auxData);

        bought = ARYZE_Utils.balanceOf(outputToken, address(this));
    ...
    function canExecute(
        ...
        return handler.canHandle(_inputToken, outputToken, _inputAmount, minReturn, _auxData);
    }
...
library ARYZE_Utils {
    ...
    function sortTokens(address _tokenA, address _tokenB) internal pure returns (address token0, address token1) {
        require(_tokenA != _tokenB, "Utils#sortTokens: IDENTICAL_ADDRESSES");
        (token0, token1) = _tokenA < _tokenB ? (_tokenA, _tokenB) : (_tokenB, _tokenA);
        require(token0 != address(0), "Utils#sortTokens: ZERO_ADDRESS");
    }

    /**
     * @notice Calculates the CREATE2 address for a pair without making any external calls
     * @param _factory - Address of the ARYZE factory contract
     * @param _tokenA - Address of the token A
     * @param _tokenB - Address of the token B
     * @param _initCodeHash - Bytes32 of the pair contract unit code hash
    ...
    function getAmountOut(
        ...
        require(_amountIn > 0, "Utils#getAmountOut: INSUFFICIENT_INPUT_AMOUNT");
        require(_reserveIn > 0 && _reserveOut > 0, "Utils#getAmountOut: INSUFFICIENT_LIQUIDITY");
