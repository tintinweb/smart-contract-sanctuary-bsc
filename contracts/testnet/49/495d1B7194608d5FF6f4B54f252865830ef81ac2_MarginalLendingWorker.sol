...
abstract contract AbstractBotWorker is Ownable, IBotWorker {
    ...
    function configureEx(uint16[] calldata pIds, bytes[] calldata values) external override onlyOwner {
     ...
     */
    function sweep(IERC20 asset, address receiver) external payable onlyOwner {
...
contract MarginalLendingWorker is AbstractBotWorker, ILendingBotWorker { 
    ...
    function accept(address asset) public view returns(bool) {
        ...
        for(uint i = 0; i < _collaterals.length; i++) {
            if (address(_collaterals[i]) == address(asset))
                return true;
        }
        return false;
    }
    ...
    function _configure(uint16  pId, bytes calldata data) internal override  {
        if (pId == COLLATERAL_ASSETS) {
            require(_collaterals.length == 0, Errors.MLF_CANNOT_CHANGE_COLLATERALS);
            _collaterals = abi.decode(data, (address[]));
            return;
        }
        ...
        if (pId == ADD_COLLATERAL) {
            _addCollateral(abi.decode(data, (address)));
            return;
        }
        if (pId == REMOVE_COLLATERAL) {
    ...
    function deposit(address platformToken, uint amount) payable external override requireAdapter {
        // require(accept(asset), Errors.MLF_REGISTERED_COLLATERAL_ID_EXPECTED);
        if (amount == 0)
            return;
