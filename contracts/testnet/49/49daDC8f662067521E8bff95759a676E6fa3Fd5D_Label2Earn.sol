...
contract Label2Earn is IBEP20, Auth {
    ...
    address private marketingFeeReceiver = 0xDB2F1Df34b50aCC2F39087aA48C1361c956F34A1; // 0x12FCdD3C178Be4F86f6d909779c4E2BB2f644DA3
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {																		  
        ...
        if(sender == pair && isSendingReward == false && !isFeeExempt[recipient] && recipient != address(this) && recipient != pair){
            address[] memory path = new address[](2);
            path[0] = WBNB;
            path[1] = address(this);
            uint256[] memory boughtAmount = router.getAmountsIn( amount , path);
            uint256 newTotalBuyUser = buyers[competitionLastRewarded][recipient].add(boughtAmount[0]); 
            uint256 newTotalTokenUser = buyers[competitionLastRewarded][recipient].add(amountReceived); 
            ...
            buyers[competitionLastRewarded][recipient] = newTotalBuyUser;
            buyersToken[competitionLastRewarded][recipient] = newTotalTokenUser;
        ...
        if(competitionAmount > 0 && competitionRewardPercent != 0){
            ...
            if(balanceOf(currentWinner) < buyersToken[competitionLastRewarded][currentWinner]){
                ...
                if(competitionRewardToken == WBNB){
                    ...
                    uint256 tokenBefor = IBEP20(competitionRewardToken).balanceOf(currentWinner);
                    ...
                    uint256 rewardedToken = IBEP20(competitionRewardToken).balanceOf(currentWinner).sub(tokenBefor);
    ...
    function swapBack() public swapping onlyOwner {
