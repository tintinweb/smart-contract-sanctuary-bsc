...
contract SmartChefFactory is Ownable {
     ...
     * @param _dutyProfile: Duty Profile address
     * @param _dutyProfileIsRequested: Duty Profile is requested
     * @param _dutyProfileThresholdPoints: Duty Profile need threshold points
    ...
    function deployPool(
        ...
        bool _dutyProfileIsRequested,
        uint256 _dutyProfileThresholdPoints,
        address _admin
    ) external onlyOwner {
        ...
        bytecode = abi.encodePacked(
            bytecode,
            abi.encode(_dutyProfile, _dutyProfileIsRequested, _dutyProfileThresholdPoints)
        );
...
import {IDutyProfile} from "./interfaces/IDutyProfile.sol";
...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // Duty profile
    IDutyProfile public immutable dutyProfile;

    // Duty Profile is requested
    bool public dutyProfileIsRequested;

    // Duty Profile points threshold
    uint256 public dutyProfileThresholdPoints;
    ...
    constructor(
        ...
        IDutyProfile(_dutyProfile).getTeamProfile(1);
        dutyProfile = IDutyProfile(_dutyProfile);

        // if dutyProfile is requested
        dutyProfileIsRequested = _dutyProfileIsRequested;

        // dutyProfile threshold points when profile & points are requested
        dutyProfileThresholdPoints = _dutyProfileThresholdPoints;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            (!dutyProfileIsRequested && dutyProfileThresholdPoints == 0) ||
                dutyProfile.getUserStatus(msg.sender),
            "Deposit: Must have an active profile"
        );

        uint256 numberUserPoints = 0;

        if (dutyProfileThresholdPoints > 0) {
            (, numberUserPoints, , , , ) = dutyProfile.getUserProfile(msg.sender);
        }

        require(
            dutyProfileThresholdPoints == 0 || numberUserPoints >= dutyProfileThresholdPoints,
        ...
        onlyOwner
    {
        ...
        dutyProfileIsRequested = _isRequested;
        dutyProfileThresholdPoints = _thresholdPoints;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 dutyReward = multiplier * rewardPerBlock;
            uint256 adjustedTokenPerShare = accTokenPerShare + (dutyReward * PRECISION_FACTOR) / stakedTokenSupply;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 dutyReward = multiplier * rewardPerBlock;
        accTokenPerShare = accTokenPerShare + (dutyReward * PRECISION_FACTOR) / stakedTokenSupply;
        lastRewardBlock = block.number;
    }
