...
interface IAuctionTreasury {
    function requestFund(address _token, address _to, uint256 _amount) external;
    function cancelAllowance(address _token) external;
}
...
contract LgoDutchAuction is ReentrancyGuard {
    ...
    constructor(
        ...
        emit AuctionDeployed(
            auctionToken, payToken, _totalTokens, _startTime, _endTime, _startPrice, _minimumPrice, _admin, _treasury
            );
    }
     ...
     */
    function tokensClaimable(address _user) public view returns (uint256 _claimerCommitment) {
    ...
    function cancelAuction() public nonReentrant {
        ...
        IAuctionTreasury(auctionTreasury).cancelAllowance(auctionToken);
        finalized = true;
    ...
    function finalize() public nonReentrant {
        ...
        if (auctionSuccessful()) {
            ...
            IExtendedERC20(payToken).burn(commitmentsTotal);
            IERC20(auctionToken).safeTransferFrom(auctionTreasury, address(this), totalTokens);
        } else {
            /// @dev Failed auction
    ...
    event TokensWithdrawn(address _token, address _to, uint256 _amount);
