...
contract CakePool is IERC20, Ownable {
    ...
    string constant _name = "CakePool";
    ...
    uint256 public buyStakePoolFee = 1;
    ...
    function takeFee(
        ...
        if (to == pair) {
            ...
            if (sellStakePoolFee > 0) {
                stakeAmount = feeAmount.mul(sellStakePoolFee).div(
                    sellTotalFees
                );
            }
        } else {
            ...
            if (buyStakePoolFee > 0) {
                stakeAmount = feeAmount.mul(buyStakePoolFee).div(buyTotalFees);
            }
        }
        _balances[address(this)] = _balances[address(this)].add(
            feeAmount.sub(stakeAmount)
        );
        emit Transfer(sender, address(this), feeAmount.sub(stakeAmount));

        if (stakeAmount > 0) {
            _balances[address(stakePoolAddress)] = _balances[
                address(stakePoolAddress)
            ].add(stakeAmount);
            emit Transfer(sender, address(stakePoolAddress), stakeAmount);
        }
    ...
    function updateBuyFees(
        ...
        buyTotalFees = reward.add(marketing).add(liquidity).add(staking);

        require(buyTotalFees <= 25, "Fees can not be grater than 25%");
    }
    ...
    function updateSellFees(
        ...
        sellTotalFees = reward.add(marketing).add(liquidity).add(staking);

        require(sellTotalFees <= 25, "Fees can not be grater than 25%");
    }
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 swapFeeForOtherTokens = totalSwap.sub(liquiditySwap);
        uint256 tokensToSwapOtherTokens = contractTokenBalance.sub(
            tokensToLiquidity
        );
        ...
        if (tokensToSwapOtherTokens > 0) {
            ...
            uint256 tokensToMarketing = swappedTokensAmount.sub(tokensToReward);
