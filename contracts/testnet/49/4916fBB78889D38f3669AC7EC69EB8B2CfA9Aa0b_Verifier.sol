...
contract Verifier is Ownable{
    ...
    function setCliffInfo(uint256[] memory _timeCliff, uint256[] memory _percentage, uint256[] memory _proofs) external onlyOperator isSetting {
    ...
    function updateProof(uint256 _proof, uint256 _cliffIndex) external onlyOperator returns(bool){
        require(_cliffIndex < cliffInfo.length, "Verifier: cliff not exist");
        cliffInfo[_cliffIndex].proof = _proof;
        return true;
    }

    function approveClaim(address[] memory _users, uint256[] memory _data, uint256 _claimTimes) external onlyOperator {
    ...
    function removeApprove(address[] memory _users, string memory _reason,  uint256 _claimTimes) external onlyOperator {
        require(_claimTimes < cliffInfo.length, "Verifier: times overflow");
        for(uint i = 0; i < _users.length; i++) {
    ...
    function verify(address _user, uint256 _totalToken, uint8 _claimTimes) public view returns (uint amountClaim, bool finish, bool claimable, uint totalClaim, uint8 lastCliff) {
        require(_claimTimes < cliffInfo.length, "Verifier: times overflow");
        ...
        for(uint i = _claimTimes;i < cliffInfo.length; i++) {
            if(cliffInfo[i].timeCliff <= block.timestamp && (status[_user][i] || i == 0)) {
                totalPercentage += cliffInfo[i].percentage;
                lastCliff += 1;
            }
        }
