...
contract MasterShifu is ERC721 {
    ...
    uint256 public constant NFT_TotalSupply = 200;
    ...
    constructor() ERC721("Master Shifu", "MSF") {
    ...
    function trunOffairdrop() public onlyDev {
        airdropSwitch = false;
    }

    function airdrop(uint256[] memory _id, string[] memory _hash, address[] memory _to, address[] memory _origin) public onlyDev {
        require(_id.length == _hash.length && _hash.length == _to.length && _to.length == _origin.length,'length wrong');
        require(airdropSwitch, "air drop not allow");
        for (uint256 i = 0; i < _id.length; i++){
            NFT_Id++;
            require(NFT_Id == _id[i], "not in order");
            starAttributes[NFT_Id].origin = _origin[i];
            starAttributes[NFT_Id].stampFee = 50;
