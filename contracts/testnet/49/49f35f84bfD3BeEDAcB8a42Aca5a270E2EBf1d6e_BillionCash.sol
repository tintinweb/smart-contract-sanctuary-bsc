...
contract BillionCash is Context, IERC20, IERC20Metadata, Ownable {
    ...
    address public marketAddress = 0x13fa87bDeF0735c5794d5aCaF2e13F9589227C89;
    address public developmentAddress =
        0x30580573240cf9530424DbFCf8F9687f218223eC;
    address public ownerAddress = 0x378B826ab5639B3094cE6b0A58da097CC44b1961;
    ...
    constructor(address _pancakeRouter) {
        ...
        //bacMainnet: 0x10ED43C718714eb63d5aA57B78B54704E256024E
        IPancakeRouter02 _pancakeV2Router = IPancakeRouter02(_pancakeRouter);
        ...
        _isAutomaticMarketMaker[pancakeV2Pair] = true;

        _mint(ownerAddress, 50_000);
    }
    ...
    function transferFrom(
        ...
        if (from != msg.sender) {
            ...
            if (allowed != type(uint256).max) {
                require(allowed >= value, "BEP20: request exceeds allowance");
    ...
    function _transfer(
        ...
        uint256 _developmentAmt;
        uint256 _liquidityAmt;
        uint256 _marketingAmt;
        if (
            ...
            if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
                ...
                _developmentAmt = amount.mul(_developmentFee).div(10000);
                _liquidityAmt = amount.mul(_liquidityFee).div(10000);
                _marketingAmt = amount.mul(_marketingFee).div(10000);
            }
        } else {
            _developmentAmt = 0;
            _liquidityAmt = 0;
        ...
        _balances[marketAddress] += _marketingAmt;
        _balances[developmentAddress] += _developmentAmt;
        ...
        if (_marketingAmt > 0) {
            emit Transfer(sender, marketAddress, _marketingAmt);
        }
        _balances[recipient] += amount
            .sub(_developmentAmt)
            .sub(_liquidityAmt)
            .sub(_marketingAmt);
        if (_developmentAmt > 0) {
            emit Transfer(sender, developmentAddress, _developmentAmt);
        }
        emit Transfer(
            sender,
            recipient,
            amount.sub(_developmentAmt).sub(_liquidityAmt).sub(_marketingAmt)
        );
    }
    ...
    function updateFees(
        ...
        require(
            liquidityFee.add(marketingFee).add(developmentFee) <= 2500,
        ...
        emit TaxFeeUpdated(liquidityFee.add(marketingFee).add(developmentFee));
    }
