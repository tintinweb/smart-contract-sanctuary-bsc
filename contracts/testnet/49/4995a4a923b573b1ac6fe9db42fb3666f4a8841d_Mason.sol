...
contract Mason is Ownable{
    ...
    uint256 private MARKETING_FEE = 900;
    uint256 private DEV_FEE = 300;
	uint256 private DIVIDENDS_PERCENT = 200;
    ...
    function createUserIfNotExist(address _address, address refer) private {
        ...
        if (user.registeredAt == 0) {
            ...
            if (initializedAt > 0 && refer != address(0)) {
    ...
    function lastPayGift() public {
		if (lastPayTime > 0 && (block.timestamp - 1 minutes) > lastPayTime) {
