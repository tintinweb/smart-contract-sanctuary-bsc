 ...
 */
contract CrazyTramps is Context,  AccessControlEnumerable, ERC721Enumerable, ERC721URIStorage{
  ...
  function setBaseURI(string memory baseURI) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "CrazyTramps: must have admin role to change base URI");
    _baseTokenURI = baseURI;
  }

  function setTokenURI(uint256 tokenId, string memory _tokenURI) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "CrazyTramps: must have admin role to change token URI");
    _setTokenURI(tokenId, _tokenURI);
  }

  function setPrice(uint mintPrice) external {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "CrazyTramps: must have admin role to change price");
    _price = mintPrice;
  }
  ...
  function mint() public payable {
    require(msg.value == _price, "CrazyTramps: must send correct price");
    require(_tokenIdTracker.current() < _max, "CrazyTramps: all CrazyTramps have been minted");
  ...
  function trampCreator(uint256 tokenId) public view returns(address){
  ...
  function claimReward(uint256 tokenId) public {
    require(ownerOf(tokenId) == _msgSender(), "CrazyTramps: Only owner can claim rewards");
