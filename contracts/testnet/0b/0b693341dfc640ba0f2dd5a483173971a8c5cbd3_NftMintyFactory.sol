...
contract NftMintyFactory is Ownable {
   ...
   MyCollection[] public theCollections;
    ...
    function mintNFTFromCollection(address _collection, uint256 _mintAmount) public payable {
        require(_mintAmount != 0);
        NftSmartContract nsc = NftSmartContract(_collection);
        if (msg.sender != nsc.caller()) {
            require(msg.value == nsc.fee() * _mintAmount, "Pay the fee for mint!");
            payable(nsc.caller()).transfer(msg.value);
        }
        nsc.mintNft(msg.sender, _mintAmount);
    }
...
contract NftSmartContract is ERC721Enumerable, Ownable {
    ...
    // MINT FUNCTION
    function mintNft(address _minter, uint256 _mintAmount) public payable {
        require(_mintAmount != 0);

        uint256 supply = totalSupply();
        require(pause != true, "The smart contract is paused!");
        require(blacklist[_minter] != true, "You are blacklisted!");
