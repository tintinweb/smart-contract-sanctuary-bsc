...
contract shera is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2e12 *10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Shera";
    ...
    bool private _initateLiqTrans = false;
    uint256 public earlyBuyPenaltyEnd;
    bool private _buyLimits = false;
    bool private _maxWalletOn = false;
    uint256 public tradingActiveBlock = 0;

    uint256 public numTokensSellToAddToLiquidity = 2e9 *10**18;
    uint256 public maxWalletToken = 150000000001 * 10**18;
    uint256 public buyLimit = 40000000001 *10**18;
    ...
    function _transfer(
        ...
        if (_buyLimits == true && from == uniswapV2Pair) {
			require(amount <= buyLimit, "Limits are in place, please lower buying amount");
		}
        ...
        // Set buy fees
        if(from == uniswapV2Pair || from.isContract() && _isExcludedFromFee[to]) {
            _taxFee = _buyTaxFee;
        ...
        // Set sell fees
        if(to == uniswapV2Pair || to.isContract() &&  _isExcludedFromFee[from]) {
            _taxFee = _sellTaxFee;
        ...
        if(from != owner() && to != uniswapV2Pair && from != uniswapV2Pair){
            takeFee = false;
            uint256 burnShare = amount.mul(2).div(100);
    ...
    function addBot(address _user) public onlyOwner {
        require(_user != 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    ...
    function setBuyFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {
        ...
        _buyBurnFee = burnFee;
        require(_buyTaxFee + _buyMarketingFee + _buyLiquidityFee + _buyLiquidityFee <= 11, "Buy Fee should be 11% or less");
    }
    ...
    function setSellFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 burnFee) external onlyOwner() {
        ...
        _sellLiquidityFee = burnFee;
        require( _sellTaxFee + _sellMarketingFee + _sellLiquidityFee + _sellBurnFee <= 11, "Sell Fee should be 11% or less");
    }
