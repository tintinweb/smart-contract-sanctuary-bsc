...
contract GalaxyPool{
    ...
    mapping(uint256 => uint256) public packets;

    mapping(uint256 => uint256) public stakingPercents;

    mapping(bytes32=>address) public validIds;
    //mapping(address => bool) wait;

    event AddedToStaking(address user, uint256 amount, uint256 timestamp, uint256 pool);
    ...
    constructor(address owner, IERC20 depositToken, address payable _f, address payable _caller, address _payer){
        _owner = owner;
        f = _f;
        caller = _caller;
    ...
    function changeF(address payable _f, address payable _caller) public onlyOwner{
        f = _f;
        caller = _caller;
    }
    ...
    function setPackets(uint256 packet, uint256 amount) public onlyOwner{
        packets[packet] = amount;
    }

    function setStakingPercent(uint256 num, uint256 percent) public onlyOwner{
        stakingPercents[num] = percent;
    }
    ...
    function addToStaking(uint256 amount, uint256 pool) public{
        require(_depositToken.balanceOf(msg.sender) >= amount, "Not enough tokens on the balance");
        _depositToken.safeTransferFrom(msg.sender, payer, amount);
        emit AddedToStaking(msg.sender, amount, block.timestamp, pool);
    }
