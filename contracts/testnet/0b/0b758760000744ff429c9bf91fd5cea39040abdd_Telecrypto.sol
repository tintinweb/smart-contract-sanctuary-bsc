...
contract Telecrypto {
    ...
    uint256 public nrgame;    
    string public luckynumbers;    
    ...
    function buyTicket(
        ...
        uint256 ShareDev = totalwei.div(5); // 20% of `totalwei`
        uint256 SharePool = totalwei.div(5).mul(4); // 80%

        address WalletPool = games[_nrgame].wallet;

        Busd.transferFrom(msg.sender, WalletDev, ShareDev);
        Busd.transferFrom(msg.sender, WalletPool, SharePool);
    ...
    function createGame(
        ...
        emit UpdateGameState(_nrgame, _ticketprice, "open");
    }

    //GameStates: 'open','closed','raffle','completed','payed'
    function updateGame(uint256 _nrgame, string memory _gamestate) public {
        require(msg.sender == Owner, "Only Owner can update games!");
        games[_nrgame].gamestate = _gamestate;

        emit UpdateGameState(_nrgame, games[_nrgame].ticketprice, _gamestate);
    }
    ...
    function getGameBalance(uint256 _nrgame) public view returns (uint256) {
        return Busd.balanceOf(address(games[_nrgame].wallet));
    }

    function deposit(uint256 _tokenamount) public {
        uint256 totalwei = _tokenamount * (10**18);
        Busd.transferFrom(msg.sender, address(this), totalwei);

        emit Deposit(msg.sender, _tokenamount);
    }

    function withdraw(address _Busd, uint256 _amount) public {
