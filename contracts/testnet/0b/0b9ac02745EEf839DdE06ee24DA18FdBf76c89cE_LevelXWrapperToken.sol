...
contract LevelXWrapperToken is Initializable, ReentrancyGuard, ERC20
{
	...
	function claim(uint256 _i, address _account) external nonReentrant returns (uint256 _rewardAmount)
	{
		require(_i < LevelXToken(token).rewardIndexLength(), "invalid index");
		_claimRewards();
		_updateAccount(_account, 0);
		if (_i == 0) {
			return _compound(_account, _i);
		} else {
			return _claim(_account, _i);
		}
	}
	...
	function _compound(address _account, uint256 _i) internal returns (uint256 _rewardAmount)
	{
		...
		if (_rewardAmount > 0) {
			...
			if (_i == 0) {
				...
				super._transfer(address(this), msg.sender, _shares);
			} else {
				...
				IERC20(_rewardToken).safeTransfer(masterChef, _rewardAmount);
			}
		}
