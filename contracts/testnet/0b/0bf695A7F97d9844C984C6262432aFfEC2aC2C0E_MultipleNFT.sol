...
contract MultipleNFT is ERC1155 {
    ...
    mapping(uint256 => uint256) public _idToidx;
    uint256 []  salenft;
    uint256 [] auctionnft;
    ...
    function getApproves() public view returns(address[] memory){
        return approvers;
    }
    function setApprover(address _approver) public onlyOwner {
     ...
     function createcsdoge(uint256 collectionid,address to,string memory _uri, string memory _mintname,string memory _nftowner,string memory description,uint256 copies) external 
    {
     ...
     function  csdogeinfo(uint256 tokenid) external view returns(bool,uint256,uint256)
    {
      ...
      function nftInformation(uint256 id) external view returns(uint256,string memory,uint256,string memory,uint256,string memory,string memory,uint256,address, uint)
    {
        require(id <= tokenidmint, "unvalid NFT" );
        uint256 _idx =  id;
        return (id,nftInfo[_idx].mintName,nftInfo[_idx].timeofmint,nftInfo[_idx].nftowner,nftInfo[_idx].copies,nftInfo[_idx].description,nftInfo[_idx].uri,nftcollectionid[_idx], originalowner[_idx],nftInfo[_idx].price);
    }
     ...
     function burncopies(uint256 copiesnumber,uint256 tokenid) external 
    {
        ...
        totalcsdogecopies[tokenidmint]=copiesnumber;
    }
    ...
    function burnorinalnft(uint256 _collectionid,uint256 tokenid) external
    {
        ...
        uint256 _idx =  _idToidx[tokenid];
        stopduplicate[nftInfo[_idx].uri]=false;
        uint256 amount = nftInfo[_idx].reserves;
     ...
     function startauction(uint256 tokenid,uint256 price,uint256 endday,uint256 endhours) external
    {
        ...
        require(originalowner[tokenid] == msg.sender,"You are not owner");
        ...
        safeTransferFrom(firstowner,address(this), tokenid,1,"");
    }
    ...
    {
        uint256 _idx =  _idToidx[tokenid]; 
        require(nftInfo[_idx].reserves!=0,"copies finish");
        ...
        copycsdoge(collectionid,_to,nftInfo[_idx].uri, nftInfo[_idx].mintName,nftInfo[_idx].nftowner,1,nftInfo[_idx].description);
     ...
     function copycsdoge(uint256 collectionid,address to,string memory _tokenURI,string memory _mintname,string memory _nftowner,uint256 _copies,string memory description) internal 
    {
        ...
        nftcollectionid[tokenidmint]=collectionid;
        originalowner[tokenidmint]=msg.sender;
        uint256 _idx =  _idToidx[collectionid]; 
        nftInfo[_idx].reserves += _copies;
    ...
    function mintBatchNFT(
       ...
       for(uint i =0; i< _ids.length; i++) {
        ...
        require(msg.value >= mintVariables.totalPrice, "invalid transaction");
        for(uint i =0; i< _ids.length; i++) {
        mintVariables.id = _ids[i];
        uint256 _idx =  _idToidx[_ids[i]];
        mintVariables.price = nftInfo[_idx].price;
        mintVariables.reserves = nftInfo[_idx].reserves;
        mintVariables.category = _categories[i];
        mintVariables.amount = _amounts[i];
        require(mintVariables.reserves >= mintVariables.amount,
        "ERC1155: Not enough NFTs of this tokenId in stock!");
        require(mintVariables.amount <= maxMintAmount 
         && balanceOf(msg.sender,mintVariables.id) <= maxMintAmount,
         "There's a limit to minting per address");

        _updateReserves(_idx,mintVariables.amount);

        mintNFT(to, mintVariables.id, mintVariables.category, mintVariables.amount, data[i]);
