...
contract MercuryMultiNetworkRouter is Initializable {
    IMercuryMultiNetworkRouter private mercuryMultiNetworkRouter; // router instance for do swaps
    ...
    function setDexRouter(address _mercuryMultiNetworkRouterAddress) external {
        mercuryMultiNetworkRouter = IMercuryMultiNetworkRouter(
            _mercuryMultiNetworkRouterAddress
        );
    }
    ...
    function getDexRouterAddress() external view returns (address) {
        return address(mercuryMultiNetworkRouter);
    }
    ...
    function swapTokensForStableCoin(
        ...
        IERC20(stableCoin).approve(
            address(mercuryMultiNetworkRouter),
            type(uint256).max
        );
    ...
    function addLiquidity(
        ...
        // add the liquidity
        mercuryMultiNetworkRouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
        ...
        returns (uint256)
    {
        ...
        uint256[] memory amountsOut = mercuryMultiNetworkRouter.getAmountsOut(
            _amount,
            path
        );
...
contract AletheiaTruthPriceOracle  {
    ...
    function getTokensValueInNativeNetworkCurrency(
        IMercuryMultiNetworkRouter dexRouter,
        uint256 chainId,
        ...
        address[] memory path = new address[](2);
        path[0] = _tokenAddress;
        //path[1] = dexRouter.getNativeTokenAddress(chainId);
        //uint256[] memory amountsOut = dexRouter.getDexRouter().getAmountsOut(_amount, path);
        //uint256 tokenAmount = amountsOut[1];

        //return (nativeNetworkCurrencyPrice * tokenAmount) / 1000000000000000000;
         return (nativeNetworkCurrencyPrice * 1) / 1000000000000000000;
    }
