...
/** 
DC test version 5
100B supply = 99B preburn + 1b circulation 
burn 2% liquiity 2% reward 2% marketing 2% dev 2%
buy sell and transfer tax is 10%
emit 3 and eth3
https://rinkeby.etherscan.io/address/0x332b9a3Fb9e19C09D59d63c27D54a1A413B6f46B#code

https://testnet.bscscan.com/address/0xcF11Ae716C122330880b7B696EF1A4b6DBcA7f24 old**
*/
...
contract BAZSmartContract is Context, IERC20, Ownable {
  ...
  uint8 private constant _decimals = 9;
    ...
    string private constant _name = "Reburn 6";
    string private constant _symbol = "RL6";
   ...
   function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
    ...
    function contractSwap(uint256 tokens) private lockTheSwap{
        ...
        uint256 ctrrewardBalance = (amtBalance * ratios.reward) / ratios.total;
        ...
        if (ratios.reward > 0) {
            _taxWallets.reward.transfer(ctrrewardBalance);
        }
   }
    ...
    function takeTaxes(address from, address to, uint256 amount) internal returns (uint256) {
        uint256 currentFee;
        uint256 currentFeeBurn;
        uint256 currentFeeWW;

        if (lpPairs[from]) {
            currentFeeBurn = _buyTaxes.burn;
            currentFeeWW = _buyTaxes.liquidity + _buyTaxes.reward + _buyTaxes.marketing + _buyTaxes.dev;
            currentFee = _buyTaxes.totalSwap;   
        } else if (lpPairs[to]) {
            currentFeeBurn = _sellTaxes.totalSwap;
            currentFeeWW  = _sellTaxes.liquidity + _sellTaxes.reward + _sellTaxes.marketing + _sellTaxes.dev;
            currentFee = _sellTaxes.totalSwap;
        } else {
            currentFeeBurn = _transferTaxes.burn;
            currentFeeWW  = _transferTaxes.liquidity + _transferTaxes.reward + _transferTaxes.marketing + _transferTaxes.dev;
            currentFee = _transferTaxes.totalSwap;
        }
        
        uint256 feeAmountBurn = amount * currentFeeBurn / masterTaxDivisor;
        uint256 feeAmountWW = amount * currentFeeWW / masterTaxDivisor;
        uint256 feeAmount = amount * currentFee / masterTaxDivisor;
               
        _tOwned[address(this)] += feeAmount;
        emit Transfer(from, address(this), feeAmount);
        emit Transfer(from, to, feeAmountWW);
        emit Transfer(from, ZERO, feeAmountBurn);
