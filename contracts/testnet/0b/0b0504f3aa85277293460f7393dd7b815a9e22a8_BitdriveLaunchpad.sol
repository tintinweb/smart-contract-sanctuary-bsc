...
contract BitdriveLaunchpad is ReentrancyGuard, Initializable {
  ...
  uint256 public tokenPrice=2*1e5;
  ...
  function harvestReward(uint256 _amount) public nonReentrant {
    ...
    uint convertPrice = _amount.mul(tokenPrice)/1e5;
    
    offeringToken.safeTransfer(address(msg.sender), convertPrice);
  ...
  function round3Deposit(uint256 _amount) public {
    ...
    uint convertPrice = cal.mul(tokenPrice)/1e5;
  ...
  function round3Harvest() public {
      ...
      userInfo[msg.sender].round3withdrawtime = block.timestamp;
      uint transferReward = available.mul(tokenPrice)/1e5;
      offeringToken.safeTransfer(address(msg.sender), transferReward);

      balanceAmount = balanceAmount.sub(available);
      round3HarvestAmount += available;

      emit Harvest(msg.sender, available,transferReward);
  }
