...
contract Crowdfunding is AccessControlEnumerable {
    ...
    //设置地址参数
    function setAddresses(address _fee, address _donate, address _stake) external /*onlyRole(DEFAULT_ADMIN_ROLE)*/ {
        fee = _fee;
        donate = _donate;
        stake = _stake;
    }
    ...
    //设置分区参数
    function setRegionParams(uint256 _regionIndex, uint256 _period, uint16 _multiplier, uint16 _restartType) external /*onlyRole(DEFAULT_ADMIN_ROLE)*/ {
    ...
    //添加分区
    function addRegion(uint256 period, uint16 poolType, uint256 initAmount, uint16 multiplier, uint256 startTime, uint16 restartType) external /*onlyRole(DEFAULT_ADMIN_ROLE)*/ {
    ...
    function recordMetaPoint(address _account) internal returns (bool) {
        ...
        if (!accounts[_account].pointFlag) {
            ...
            //记录45层上级
            for (uint256 i = 0; i < 45; i++) {
                if (accounts[head].head == address(0)) {
                    account = IMetaPoint(metaPoint).accounts(head);
                    accounts[head].head = account.head;
                    head = account.head;
                    if (head == address(0)) break;
                } else {
                    head = accounts[head].head;
                }
            }
        }
        return true;
    ...
    function harvestAll(uint256 _regionIndex, uint256[] calldata _roundIds) external {
        ...
        for (uint256 i = 0; i < _roundIds.length; i++) {
            if (harvestableIds[msg.sender][_regionIndex].contains(_roundIds[i]) ||
                reserveIds[msg.sender][_regionIndex].contains(_roundIds[i])
            ) {
                ...
                if (state > 3) {
                    ...
                    reserveIds[msg.sender][_regionIndex].remove(_roundIds[i]);
                } else {
    ...
    function getProjectIds(address _account, uint256 _regionIndex, uint256 _type) public view returns (uint256[] memory pids) {
        if (_type == 1)
            pids = projectIds[_account][_regionIndex];
        else if (_type == 2)
            pids = harvestableIds[_account][_regionIndex].values();
        else if (_type == 3)
            pids = reserveIds[_account][_regionIndex].values();
        else if (_type == 4)
            pids = brokerRewardIds[_account][_regionIndex].values();
    }
