...
// File: contracts/interfaces/IBiscuit.sol


pragma solidity ^0.8.4;

interface IBiscuit {
    function burnBiscuit(address _staker, uint256 _amount) external;
    function mintBiscuit(address _depositor, uint256 amount) external;
}
...
contract PYEStakingContract is ReentrancyGuard, Ownable {
    ...
    IBiscuit public BiscuitInterface;
    address public BiscuitAddress;
    address public PYE;
    ...
    modifier addressCheck {
        require(BiscuitAddress != address(0), "PYEStakeTokenAddress hasn't been set yet!");
        _;
    }
    ...
    function setBiscuitToken(address _address) external onlyOwner {
        BiscuitAddress = _address;
        BiscuitInterface = IBiscuit(_address);
    }
    ...
    function deposit(uint256 _amount) external addressCheck nonReentrant notContract() {
        ...
        if (currentStakedBalance == 0) {
            ...
            BiscuitInterface.mintBiscuit(msg.sender, 1);
    ...
    function withdraw(uint256 _amount) external addressCheck nonReentrant notContract() {
        ...
        uint256 currentStakedBalance = StakedBalance[msg.sender];
        uint256 currentBiscuitBalance = IERC20(BiscuitAddress).balanceOf(msg.sender);

        if (currentStakedBalance.sub(_amount) == 0) {
            BiscuitInterface.burnBiscuit(msg.sender, currentBiscuitBalance);
