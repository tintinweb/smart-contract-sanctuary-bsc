...
contract NftMarketPlace is Ownable, Pausable, IMarketplace, ERC721Holder {
    ...
    receive() external payable {
        assert(msg.sender == WETH); // only accept ETH via fallback from the WETH and dfynWETH contract
    }
    ...
    function acceptBid(
        ...
        IWETH(WETH).transfer(order.seller, (bid.price));
...
interface IMarketplace {
  ...
  event OrderSuccessful(bytes32 id, address indexed buyer, uint256 priceInWei);
