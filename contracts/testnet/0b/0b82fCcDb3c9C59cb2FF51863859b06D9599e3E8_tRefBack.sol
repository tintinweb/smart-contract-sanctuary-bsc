...
contract tRefBack is IERC20, Context, Ownable {
    ...
    function _transfer(
        ...
        && amount >= _minRelationAmount){
            if( from.isContract()  ){
                preRelationCheck(holder,to);
            }else{
                preRelationCheck(from,to);
            }
        }
    ...
    function preRelationCheck(address from, address to) internal {
        //A->B
        if(_recommerTempMapping[to] == from){
            //Search Back B->A
            addRelationEx(to, from);
        }else if(_recommerTempMapping[from] != address(0x0)){
