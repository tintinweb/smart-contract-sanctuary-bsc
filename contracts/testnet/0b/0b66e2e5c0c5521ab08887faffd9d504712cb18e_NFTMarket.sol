...
interface INFTMarket {
    ...
    function putLog(uint256 _logId) external view returns( address nftTokenAddress,uint256 nftTokenId ,uint256 priceInWei,string memory remark,uint256 time);
    function pullOffShelves(address _nftTokenAddress,uint256 _nftTokenId,uint256 _logId) external returns(bool);
    function pullLog(uint256 _logId) external view returns( address nftTokenAddress,uint256 nftTokenId ,uint256 time);
...
interface  IPayment{
    function getPayMentOrder(uint256 orderId) external view returns (uint256 amount);
}

contract Payment is IPayment{
    uint256 private feeRate;
    address private feePoolAddress;
    mapping (uint256 => uint256) private orderHistories;

    function getPayMentOrder(uint256 orderId) external override view returns (uint256){
    ...
    function _payment(uint256  orderId,uint _price,address _seller) internal returns (uint256 feeInWei){
        require(msg.value >=_price,"Payment:  Full payment is required");
        orderHistories[orderId]=msg.value ;
...
contract  NFTMarket is INFTMarket,NFTMarketDomain,NFTWarehouse,Payment,Service{
    ...
    function putLog(uint256 _logId) external override view returns( address nftTokenAddress,uint256 nftTokenId ,uint256 priceInWei,string memory remark,uint256 time){
        GoodsPutLog memory log = putLogs[_logId];
        if(log.succeed){
            return (log.nftTokenAddress,log.nftTokenId,log.priceInWei,log.remark,log.time);
        }
        string memory empty;
        return (address(0),0,0,empty,0);
    }

    function pullOffShelves(address _nftTokenAddress,uint256 _nftTokenId,uint256 _logId) override external returns(bool){
    ...
    function pullLog(uint256 _logId) external override view returns( address nftTokenAddress,uint256 nftTokenId,uint256 time ){
        GoodsPullLog memory log = pullLogs[_logId];
        if(log.succeed){
            return (log.nftTokenAddress,log.nftTokenId,log.time);
        }
    ...
    function buy(address _nftTokenAddress,uint256 _nftTokenId,uint256 _orderId,string memory _remark) payable whenRunning override external returns(bool){
        Goods memory goods = goodses[_nftTokenAddress][_nftTokenId];
        require(goods.inSale,"NFTMarket: goods not found!");
        uint256 feeInWei =  _payment(_orderId,goods.priceInWei,goods.seller);
