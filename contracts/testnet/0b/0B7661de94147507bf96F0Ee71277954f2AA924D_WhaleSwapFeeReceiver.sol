...
contract WhaleSwapFeeReceiver {
    using SafeMath for uint;

    address public owner;
    IWhaleSwapFactory public factory;
    address public WETH;
    ...
    constructor(
        ...
        owner = _owner;
        factory = IWhaleSwapFactory(_factory);
        WETH = _WETH;
    ...
    function transferOwnership(address newOwner) external {
        require(msg.sender == owner, 'WhaleSwapFeeReceiver: FORBIDDEN');
        owner = newOwner;
    }
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'WhaleSwapFeeReceiver: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'WhaleSwapFeeReceiver: ZERO_ADDRESS');
    }
    ...
    // Taken from WhaleSwapLibrary, removed the factory parameter
    ...
    // Done with code form WhaleSwapRouter and WhaleSwapLibrary, removed the deadline argument
    function _swapTokensForETH(uint amountIn, address fromToken)
        internal
    {
        IWhaleSwapPair pairToUse = IWhaleSwapPair(pairFor(fromToken, WETH));
        ...
        require(reserveIn > 0 && reserveOut > 0, 'WhaleSwapFeeReceiver: INSUFFICIENT_LIQUIDITY');
    ...
    // Take what was charged as protocol fee from the WhaleSwap pair liquidity
    function takeProtocolFee(IWhaleSwapPair[] calldata pairs) external {
