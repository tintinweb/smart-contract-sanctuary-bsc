...
contract StakePool is Ownable {
    ...
    function _addstake(address _staker, uint256 _stakeNum) internal returns(uint256) {
        ...
        require(mainPool != address(0),"mainPool error");
        require(aPancakePair != address(0), " do getPancakeLPAddress() first ... ");
        uint256 hasLP = this.queryLP(_staker);
    ...
    function addStake(uint256 _stakeNum) external returns(uint256) {
        ...
        return _addstake(_staker, _stakeNum);
    }

    //查询用户可用于质押的LP余额
    function queryLP(address _staker) external view onlyOwner returns(uint256) {
    ...
    function subStake(address _staker, uint256 _stakeNum) external onlyOwner returns(uint256) {
        require(mainPool != address(0),"mainPool error ...");
    ...
    //提现 (提现人 提现金额)
    function withdraw(address _sender, uint256 _amount) external onlyOwner returns(bool) {
        require(mainPool != address(0),"mainPool error ...");
        require(stake.length > 0, "Empty staker ...");
    ...
    function getPancakeLPAddress() external onlyOwner returns (address) {
        ...
        address _PancakePair = IPancakeFactory(address(0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc)).getPair(address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd),mainToken);
