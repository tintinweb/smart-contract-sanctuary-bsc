...
contract Demountain is ERC20, Ownable {
    ...
    address payable public paramountTeamAddr1 = payable(0xFfaf5770e3859935fC4A44ff397B0645232325D1); // 1%
    address payable public paramountTeamAddr2 = payable(0xE08B6015B0870d0E3711962c11D1144e019f7482); // 1%
    address payable public paramountTeamAddr3 = payable(0xde08C84C9005dcE3BD82FaD861Aa191f85CF14e0); // 1%
    address payable public paramountTeamAddr4 = payable(0x2AAD5D68bfDC3210674ec9a8Ea254265E1c72209); // 1%
    address payable internal Mount_buyback_addr = payable(0x57f52F4a0A1a749F38f61Dc5858c78bD82ebd020); // 3.5%
    address payable internal inventive_addr = payable(0xb7a0D2Ba085F8CBB4d93cd9717C93E397b6cAB7c); // 1%
    address payable internal buyBack_addr = payable(0x1706765C9751368682d30B8b7039E0d5E4E29F13); // 1%
    ...
    function ExitLobby(uint256 targetDay) external {
        require(mapMemberLobby[msg.sender][targetDay].hasCollected == false, "Already collected");
        _updateDaily();
    ...
    function calcBonusToken (uint256 StakeDuration, uint256 StakeAmount) public pure returns (uint256) {
        require(StakeDuration <= max_stake_days, 'Days greater than max duration');
    ...
    function cancelSellStakeRequest(uint256 _stakeId) internal {
        require(mapMemberStake[msg.sender][_stakeId].userAddress == msg.sender,"Unauthorized");
        require(mapMemberStake[msg.sender][_stakeId].stake_forSell == true,"Stake not for sell");
        require(mapMemberStake[msg.sender][_stakeId].stake_hasSold == false,"Stake sold");
    ...
    function lendOnStake(address loanerAddress , uint256 stakeId) external payable nonReentrant {
        ...
        require(mapMemberStake[loanerAddress][stakeId].stake_hasLoan == false, 'Stake has an active loan on it');
        require(mapMemberStake[loanerAddress][stakeId].stake_forLoan == true, 'Stake is not requesting a loan');
        require(mapMemberStake[loanerAddress][stakeId].stake_hasSold == false, 'Stake is sold');
        require(mapMemberStake[loanerAddress][stakeId].endDay > currentDay, 'Stake duration is finished');
    ...
    */
    ...
    function updateFinishedLoan(address lenderAddress, address loanerAddress, uint256 lenderLendId, uint256 stakeId) internal {
        ...
        require(mapMemberStake[loanerAddress][stakeId].stake_hasLoan == true, 'Stake does not have an active loan on it');
        require(currentDay >= mapRequestingLoans[loanerAddress][stakeId].lend_endDay, 'Due date not yet reached');
        require(mapLenderInfo[lenderAddress][lenderLendId].loanIsPaid == false,"Loan has been paid");
        require(mapRequestingLoans[loanerAddress][stakeId].loanIsPaid == false,"Loan has been paid");
        require(mapRequestingLoans[loanerAddress][stakeId].hasLoan == true, "Stake doesn't have active loan");
        require(mapRequestingLoans[loanerAddress][stakeId].lenderAddress == lenderAddress,"Unauthorized");        
        require(mapRequestingLoans[loanerAddress][stakeId].lenderLendId == lenderLendId,"Id not matched");
