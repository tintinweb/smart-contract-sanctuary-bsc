...
interface IStakingRewards {
    ...
    function withdrawTime(address account) external  view returns (uint256[2] memory);

    function getRewardTime(address account) external  view returns (uint256[2] memory);

    function userRewarded(address account) external  view returns (uint256[2] memory);
...
contract SeutStakingRewards is RewardsDistributionRecipient {
    ...
    uint[2] withdrawCycle = [1 days, 7 days];
    mapping(address => uint256[2]) public withdrawTime;

    uint[] getRewardCycle = [0, 7 days];
    ...
    constructor(
        ...
        _totalSupply = stakeReward.totalSupply();
    ...
    function migrate() external  {
        ...
        withdrawTime[sender] = stakeReward.withdrawTime(sender);
        getRewardTime[sender] = stakeReward.getRewardTime(sender);
        userRewarded[sender] = stakeReward.userRewarded(sender);




    //   uint pledgeAmount = tbaReward.getAddressPledgeTotal(sender);
    //   require(0 < pledgeAmount, 'not pledged');

    //   recommender[sender] = tbaReward.getRecommender(sender);
    //   _balances[sender] = pledgeAmount;
    //   _totalSupply = _totalSupply.add(pledgeAmount);
    //   rewards[sender] = tbaReward.getReward(sender);
    //   isWhite[sender] = tbaReward.getReleaseWhite(sender);

    //   PledgeOrder[] memory _addressPledgeOrders = tbaReward.getAddressPledgeOrders(sender);
    //   for(uint i = _addressPledgeOrders.length - 1; i >= 0; i--){
    //     PledgeOrder memory order = _addressPledgeOrders[i];
    //     if(!order.releaseStatus){
    //       withdrawTime[sender] = order.releaseTime;
    //       break;
    //     }
    //   }
