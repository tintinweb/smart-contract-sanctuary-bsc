...
contract BaseNFT is ERC721, ERC721Enumerable, ERC721URIStorage, base {
    ...
    function transable() public view returns (bool) {
        return Transable;
    }

    function setTransable(bool enable) public onlyAddrOrOwner(daoAddress()) {
        Transable = enable;
    }
    ...
    uint256 private _itemIdCursor = 0;

    /*
     * 由逻辑合约调用，入口验证了是不是逻辑合约，必须在初始化中设置逻辑合约地址或者通过setLogicAddress 设置合约地址
     */
    function _increaseMint(address to, string memory uri)
        internal
        virtual
        returns (uint256)
    {
        uint256 itemId = _itemIdCursor;
        _safeMint(to, itemId);
        bytes memory URI = bytes(uri);
        if (URI.length > 0) _setTokenURI(itemId, uri);
        _itemIdCursor = _itemIdCursor + 1;
        return itemId;
    }
    ...
    function transferFrom(
        ...
        require(Transable, "NFT trans is disabled");
    ...
    function _beforeTokenTransfer(
        ...
        // require(Transable || from == address(0), "NFT trans is disabled");
        super._beforeTokenTransfer(from, to, tokenId);
    }
...
//logiccontract 主管铸造=minnerlogic
//finlogicContract 管分红和交易
contract FinNFT is BaseNFT {
    constructor(
        string memory name_,
        string memory symbol_,
        string memory baseUrl
    ) BaseNFT(name_, symbol_, baseUrl) {}

    // 每个tokenID的算力
    mapping(uint256 => uint256) private _tokenPower;
    //每个token的铸造时间
    mapping(uint256 => uint256) private _tokenMintTime;
    //每个FINNFT的来源minnerNFT
    mapping(uint256 => uint256) private _tokenFromMiner;
    //已经领取的奖励
    mapping(uint256 => uint256) private _tokenSettledProfit;
    //本金
    ...
    function tokenPower(uint256 tokenId) public view returns (uint256) {
        return _tokenPower[tokenId];
    }

    function tokenMintTime(uint256 tokenId) public view returns (uint256) {
        return _tokenMintTime[tokenId];
    }

    function tokenFromMinner(uint256 tokenId) public view returns (uint256) {
        return _tokenFromMiner[tokenId];
    }

    function tokenSettledProfit(uint256 tokenId) public view returns (uint256) {
        return _tokenSettledProfit[tokenId];
    }

    function profitSettled(uint256 tokenId, uint256 profit)
        public
        onlyAddr(IConfig(CONFIG_CONTRACT).finLogicAddress())
        returns (uint256)
    {
        _requireMinted(tokenId);
        _tokenSettledProfit[tokenId] = _tokenSettledProfit[tokenId] + profit;
        return _tokenSettledProfit[tokenId];
    }
    ...
    function logicTrans(
        ...
        address to,
        uint256 tokenId
    ) public onlyAddr(IConfig(CONFIG_CONTRACT).finLogicAddress()) {
        _requireMinted(tokenId);
    ...
    function logicMint(
        ...
        uint256 minerItemId,
        uint256 price,
        string memory minerItemUrl
    )
        public
        onlyAddr(IConfig(CONFIG_CONTRACT).minerLogicAddress())
        returns (uint256)
    {
        uint256 tokenId = BaseNFT._increaseMint(to, minerItemUrl);
        _tokenPower[tokenId] = power;
        _tokenMintTime[tokenId] = block.timestamp;
        _tokenFromMiner[tokenId] = minerItemId;
        _tokenSettledProfit[tokenId] = 0;
        _tokenPrice[tokenId] = price;
        return tokenId;
    }

    function logicBurn(uint256 tokenId)
        public
        onlyAddr(IConfig(CONFIG_CONTRACT).finLogicAddress())
    {
        _isApprovedOrOwner(_msgSender(), tokenId);
        BaseNFT._burn(tokenId);
        delete _tokenPower[tokenId];
        delete _tokenMintTime[tokenId];
        delete _tokenFromMiner[tokenId];
        delete _tokenSettledProfit[tokenId];
