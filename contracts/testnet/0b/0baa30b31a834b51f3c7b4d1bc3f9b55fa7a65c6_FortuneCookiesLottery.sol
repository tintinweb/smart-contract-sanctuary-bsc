...
contract FortuneCookiesLottery is ReentrancyGuard, IFortuneCookiesLottery, Ownable {
    ...
    struct Lottery {
        ...
        uint256 amountDistributedToWinners;
        uint32 finalNumber;
    }
        ...
        nonReentrant
    {
        ...
        // Initializes final amount to be distributed to winners
        uint256 amountDistributedToWinners = 0;
        ...
        for (uint32 i = 0; i < 5; i++) {
            ...
            if ((_numberTicketsPerLotteryId[_lotteryId][transformedWinningNumber] - numberAddressesInPreviousBracket) != 0) {
              ...
              amountDistributedToWinners += (_lotteries[_lotteryId].rewardsBreakdown[j] * amountToShareToWinners) /10000;

            }
            else {
              // No CALCIFIRE to distribute
        ...
        if (_autoInjection == false) {
          amountToWithdrawToTreasury = (_lotteries[_lotteryId].amountCollectedInCalcifire - (amountToBurn + amountDistributedToWinners));
        }
        else {
          pendingInjectionNextLottery =  (_lotteries[_lotteryId].amountCollectedInCalcifire - (amountToWithdrawToTreasury + amountToBurn + amountDistributedToWinners));
        }

        _lotteries[_lotteryId].burnAmount = amountToBurn;
        _lotteries[_lotteryId].treasuryAmount = amountToWithdrawToTreasury;
        _lotteries[_lotteryId].amountToShareToWinners = amountToShareToWinners;
        _lotteries[_lotteryId].amountDistributedToWinners = amountDistributedToWinners;
    ...
    function startLottery(
        ...
        uint256 _burnAmount = (pendingInjectionNextLottery * _burnFee) /10000;
        uint256 _treasuryAmount = (pendingInjectionNextLottery * _treasuryFee) /10000;
        uint256 _amountToShareToWinners = pendingInjectionNextLottery - (_treasuryAmount + _burnAmount);
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            amountToShareToWinners: _amountToShareToWinners,
            amountDistributedToWinners: 0,
            finalNumber: 0
        });
