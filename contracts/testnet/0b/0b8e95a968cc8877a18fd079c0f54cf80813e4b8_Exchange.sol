...
contract Exchange is Initializable, UUPSUpgradeable, EIP712Upgradeable, OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address public constant WEKTA = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    ...
    bytes32 constant ORDER_TYPEHASH = keccak256(
        "Order(bytes4 tokenType,address seller,bytes4 saleType,address tokenAddress,uint256 tokenId,uint256 tokenValue,uint256 amount,uint256 startTime,uint256 endTime,uint256 nonce)"
    );
    bytes32 constant BID_ORDER_TYPEHASH = keccak256(
        "BidOrder(bytes4 tokenType,address seller,bytes4 saleType,address tokenAddress,uint256 tokenId,uint256 tokenValue,uint256 amount,uint256 startTime,uint256 endTime,uint256 nonce,address buyer,uint256 bidAmount)"
    );
    ...
    function completeFixedSale(Order memory order) external payable whenNotPaused nonReentrant {
        ...
        if(order.tokenType == ASSET_TYPE_ERC721) IERC721Upgradeable(order.tokenAddress).transferFrom(order.seller, msg.sender, order.tokenId);
        if(order.tokenType == ASSET_TYPE_ERC1155) IERC1155Upgradeable(order.tokenAddress).safeTransferFrom(order.seller, msg.sender, order.tokenId, order.tokenValue, "");
    }
    ...
    function completeBidding(BidOrder memory bidOrder, bytes calldata signature) external whenNotPaused nonReentrant {
        ...
        // get unique key
        Order memory order = Order(bidOrder.tokenType, bidOrder.seller, bidOrder.saleType, bidOrder.tokenAddress, bidOrder.tokenId, bidOrder.tokenValue, bidOrder.amount, bidOrder.startTime, bidOrder.endTime, bidOrder.nonce);
        bytes32 hashKey = genHashKey(order);
        ...
        (bool success) = IWETH(WEKTA).transferFrom(bidOrder.buyer, msg.sender, bidOrder.bidAmount);
        require(success, "Exchange: WEKTA transfer failed");

        // transfer token from seller to buyer
        if(order.tokenType == ASSET_TYPE_ERC721) IERC721Upgradeable(bidOrder.tokenAddress).transferFrom(msg.sender, bidOrder.buyer, bidOrder.tokenId);
        if(order.tokenType == ASSET_TYPE_ERC1155) IERC1155Upgradeable(bidOrder.tokenAddress).safeTransferFrom(msg.sender, bidOrder.buyer, bidOrder.tokenId, bidOrder.tokenValue, "");
    }
    ...
    function genOrderHash(Order memory order) internal pure returns(bytes32) {
        return keccak256(
            abi.encode(ORDER_TYPEHASH, order.tokenType, order.seller, order.saleType, order.tokenAddress, order.tokenId, order.tokenValue, order.amount, order.startTime, order.endTime, order.nonce)
        );
    }
    ...
    function genBidOrderHash(BidOrder memory order) internal pure returns(bytes32) {
        return keccak256(
            abi.encode(BID_ORDER_TYPEHASH, order.tokenType, order.seller, order.saleType, order.tokenAddress, order.tokenId, order.tokenValue, order.amount, order.startTime, order.endTime, order.nonce, order.buyer, order.bidAmount)
        );
    }
    ...
    function genHashKey(Order memory order) internal pure returns(bytes32) {
        bytes32 hashKey = keccak256(
            abi.encode(order.tokenType, order.seller, order.saleType, order.tokenAddress, order.tokenId, order.tokenValue, order.amount, order.startTime, order.endTime, order.nonce)
        );
        return hashKey;
    }
