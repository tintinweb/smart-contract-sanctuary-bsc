 ...
 */
    ...
    function purchaseBYSL(address givenToken, uint256 amount) external {
        ...
        if (
            ...
            console.log(IEarlyAccessNFT(Admin.EarlyAccess()).Won(msg.sender));
            console.log("eligibleTime",(startTime +
                    (IEarlyAccessNFT(Admin.EarlyAccess()).Won(msg.sender) *
                        1 hours)),"block.timestamp",block.timestamp);
            require(
                (startTime +
                    ((IEarlyAccessNFT(Admin.EarlyAccess()).Won(msg.sender) - 1) *
                        1 hours) <=
                    block.timestamp) &&
    ...
    function sellBYSLviaSWAP(address wantToken, uint256 amount) external {
        ...
        (BUSDAmount, leftAmount) = IReferralNFT(Admin.Refferal())
            .rewardDistribution(
                msg.sender,
                ((BUSDAmount * 1) / 10) - ((BUSDAmount * 1) / 100),
                BUSDAmount
            );
        if (amount != 0) {
            IERC20(Admin.BUSD()).safeTransfer(
