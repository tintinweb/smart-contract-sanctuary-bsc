...
contract OVLSpend is AccessControl {
    ...
    IBEP20 public token = IBEP20(address(0x38E76197B89679C18CbB5bC88baBe9058301ca4b));
    bytes32 public constant CREATOR_ADMIN_SERVER  = keccak256("CREATOR_ADMIN_SERVER");
    address public feeReceiver = 0x2808A9D1359C8be1a354d53DC496eE34c9ed1437;
    uint256 public unitTimeDeposit = 86400; // seconds (60 * 60 * 24)
    uint256 public limitDepositPerDay = 500 * 1e18;
    uint256 public startTimeDeposit = 1646118000;
    ...
    function changeBEP20Token(address _token) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, address(msg.sender)), "Caller is not an owner");
        token = IBEP20(_token);
    }
    ...
    function deposit(uint256 _amount) public {
        require(_amount > 0, "amount > 0");
        uint256 unitTime = (block.timestamp - startTimeDeposit).div(unitTimeDeposit);
        uint256 expectDepositPerDay = amountDeposit[unitTime][msg.sender].add(_amount);
        require(expectDepositPerDay <= limitDepositPerDay, "The limit has been exceeded");
        token.safeTransferFrom(address(msg.sender), address(feeReceiver), _amount);
        amountDeposit[unitTime][msg.sender] = expectDepositPerDay;
        emit Deposit(
            address(msg.sender),
            _amount,
            block.timestamp
