...
contract Marketplace is Ownable{
    ...
    function addProfit(uint256[] memory tokenID) external onlyOwner {
        require(tokenID.length > 0, "empty list");
        require(tokenID.length + profits.length <= 0, "exceed maximum");
        bool exist;
        uint256[] memory _profits = profits;
        for (uint256 i; i < tokenID.length; i ++) {
            require(flexNFT.ownerOf(tokenID[i]) != address(0), 'no nft created');
            for (uint256 j; j < _profits.length; i ++) {
                if (tokenID[i] == _profits[j]) exist = true;
            }
            if (!exist) _profits[_profits.length] = tokenID[i];
        }
        profits = _profits;
    }

    function removeProfit(uint256[] memory tokenID) external onlyOwner {
        require(tokenID.length > 0, "empty list");
        uint256[] memory _profits = new uint256[](profits.length);
        _profits = profits;
        uint256 idx = _profits.length;
        for (uint256 i; i < tokenID.length; i ++) {
            require(flexNFT.ownerOf(tokenID[i]) != address(0), 'no nft created');
            for (uint256 j = 0; j < _profits.length; j ++) {
                if (_profits[j] == tokenID[i]) idx = j;
            }
            if (idx < _profits.length) delete _profits[idx];
            idx = _profits.length;
        }
        profits = _profits;
    }
