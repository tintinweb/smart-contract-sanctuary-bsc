...
contract SmartWalletFactory is ISmartWalletFactory, OwnableUpgradeable {
     ...
     */
    function spawn(address _sponsor) external returns (address smartWallet) {
        require(
        ...
        require(
            _sponsor == address(0) ||
                getSmartWallet[ISmartWallet(_sponsor).owner()] == _sponsor,
            "invalid sponsor"
        );
        ...
        if (_sponsor != address(0)) {
            IClearingHouse(clearingHouse).registerSponsor(
                _sponsor,
                smartWallet
            );
        }
...
interface IClearingHouse {
    ...
    function registerSponsor(address _sponsor, address _referral) external;
