...
contract Fairlaunch is Ownable
{
    ...
    function SetWorker(address dev,address utilityToken,address stakeToken, uint256 workerId) public onlyOwner
    {
    ...
    function StakeUtility(uint256 amount, uint256 poolId) public onlyRegisteredUsers
    {
        ...
        if (user.Amount > 0) {
            ...
            if(pending > 0) {
                IERC20(RewardToken).safeTransfer(address(msg.sender), amount);
    ...
    function UpdatePool(uint256 poolId)  public onlyRegisteredUsers
    {
        ...
        uint256 reward = 0;
        {
            (uint256 price,) = pool.PriceOracle.getPrice(IUtilityToken(pool.StakeToken).BaseToken());
        emit GetPrice(price);
        uint256 totalReward = 
    ...
    function TransferUtilityOwnership(address newOwner) public onlyOwner{
        for(uint i = 0;i< Pools.length; i++){
            Ownable(Pools[i].StakeToken).transferOwnership(newOwner);
        }
    }
