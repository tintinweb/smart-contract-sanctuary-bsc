...
contract NFTFractional is Upgradable {
    ...
    * @param _datas poolId(0), fnftId(1), poolBalance(2), exchangeRate(3), active(4)
    * @param _configs registrationStartTime(0), registrationEndTime(1), purchaseStartTime(2), purchaseEndTime(3)
    * @param _tiers list alloction for tiers
    */
    function createFNFTPool(address[] memory _addrs, uint256[] memory _datas, uint256[] memory _configs, uint256[] memory _tiers) external onlyAdmins {
        require(_addrs[0] != address(0) || _addrs[1] != address(0), "NFTFractional: Address is not the zero address");
        // exchange rate > 0
        require(_datas[3] > 0, "NFTFractional: Exchange Rate must greater then zero");
        // pool balance > 0
        ...
        // check fnt exists
        require(fnft.tokenNFT != address(0), "NFTFractional: FNFT does not exist");
        ...
        FNFTPool memory pool = FNFTPool(_addrs[0], _addrs[1], _datas[0], _datas[1], _datas[2], _datas[2],_datas[3], _datas[4], _configs, _tiers);
        fnftPools[_datas[0]] = pool;
        emit CreateFNFTPool(_addrs[0], _addrs[1], _datas[0], _datas[1], _datas[2], _datas[3], _datas[4]);
    }
    ...
    * @param _datas poolID(0), lockDuration(1), withdrawDelayDuration(2)
    */
    function createTieringPool(address _addr, uint256[] memory _datas) external onlyAdmins {
        ...
        if (pool.stakingToken == address(0)) {
            tierPools[_datas[0]] = TierPool(_addr, 0, 0, _datas[1], _datas[2]);
        } else {
    ...
    * @param _datas poolID(0), fnftPoolId(1), totalRewardAmount(2), poolOpenTime(3), exchangeRate(4), active(5)
    */
    function createRewardPool(address _addr, uint256[] memory _datas) external onlyAdmins {
        ...
        // purchase end time of f-nft pool < current time
        // require(fnftPool.configs[3] < block.timestamp, "NFTFractional: Current Time must greater than purchase end time of f-nft pool");
        // totalRewardAmount > 0
        require(_datas[2] > 0, "NFTFractional: Total Reward Amount must greater than zero");
        // poolOpenTime >= current time
        require(_datas[3] >= block.timestamp && _datas[3] >= fnftPool.configs[3], "NFTFractional: Pool Open Time must greater than current time and purchase end time");
        // exchangeRate > 0
        require(_datas[4] > 0, "NFTFractional: Exchange rate must greater than zero");
        RewardPool memory pool = RewardPool(_addr, _datas[1], _datas[2], _datas[3], _datas[4], _datas[5]);
        rewardPools[_datas[0]] = pool;
        emit CreateRewardPool(_addr, _datas[0], _datas[1], _datas[2], _datas[3], _datas[4], _datas[5]);
    }
    ...
    */
    function withdrawFund(address _tokenAddress, address _account, uint256 _amount) external onlyAdmins {
        require(IERC20(_tokenAddress).balanceOf(address(this)) >= _amount, "NFTFractional: Pool not has enough balance");
    ...
    * @param _datas poolId(0), amount(1), alloction(2), purchaseFNFT(3)
    ...
    */
    function purchaseFNFT(uint256[] memory _datas, string memory _purchaseId,bytes memory _signature) external {
        require(
            ...
            "NFTFractional: Signature is invalid"
        );
        ...
        require(_datas[3] <= pool.availableBalance, "NFTFractional: Amount must less then availaible balance pool");
    ...
    */
    function stakeTierPool(uint256[] memory _datas) external {
        TierPool storage pool = tierPools[_datas[0]];
        // check pool exists
        require(pool.stakingToken != address(0), "NFTFractional: Pool does not exits");
        // check amount > 0
    ...
    */
    function unStakeTierPool(uint256[] memory _datas) external {
        ...
        require(userInfo.unStakeLastTime + pool.withdrawDelayDuration * AQUATER_MINUTE < block.timestamp, "NFTFractional: User is in withdraw delay duration");
        ...
        if (pool.withdrawDelayDuration > 0) {
            ...
            IERC20(pool.stakingToken).transferFrom(address(this), msg.sender, _datas[1]);
        }
    ...
    */
    function withdrawDelayToken(uint256 poolId) external {
        ...
        require(tierPool.stakingToken != address(0), "Tier Pool doesnt exist");
        ...
        require(block.timestamp - userInfo.unStakeLastTime >= tierPool.withdrawDelayDuration, "Pool doesnt finish withraw delay duration");
    ...
    * @param _datas poolId(0), amountFNFT(1), alloction(2), rewardUSDT(3)
    ...
    */
    function claimReward(uint256[] memory _datas, string memory _claimId,bytes memory _signature) external {
        ...
        require(rewardPool.rewardToken != address(0) || fnftPool.acceptToken != address(0), "NFTFractional: Pool does not exist");
        require(rewardPool.poolOpenTime >= block.timestamp, "NFTFractional: Pool does not open");
        require(IERC20(rewardPool.rewardToken).balanceOf(address(this)) >= rewardPool.totalRewardAmount, "NFTFractional: Pool is not enought balance");
        IERC20(fnftPool.acceptToken).transferFrom(msg.sender, address(this), _datas[1]);
        IERC20(rewardPool.rewardToken).transferFrom(address(this), msg.sender, _datas[3]);
        // can suy nghi them
        ERC20Token(fnftPool.acceptToken).burnFrom(address(this), _datas[1]);
        rewardPool.totalRewardAmount -= _datas[3];
        // emit PurchaseFNFT(_datas[0], userInfo.purchased, userInfo.alloction - userInfo.purchased, _datas[3], msg.sender, _purchaseId);
    }
...
contract Upgradable {
    ...
    mapping(address => uint256) public adminList; // 1: admin, 2: user, 3: controller
    ...
    modifier onlyAdmins() {
        require(msg.sender == controller || adminList[msg.sender] == 1, "NFTFractional: Only controller and admins");
        _;
    }
...
contract ERC20Token is ERC20Upgradeable {
  ...
  function mintERC20(address _curator, uint256 _supply) external {
...
interface ISignatureUtils {
    function getMessageHash(uint256 poolId,uint256 amount,uint256 allocation,uint256 purchaseFNFT, address sender) external returns (bytes32);
    function getEthSignedMessageHash(bytes32 _messageHash) external returns (bytes32);
    function verify(uint256 poolId,uint256 amount,uint256 allocation,uint256 purchaseFNFT,address sender,address signer,bytes memory signature) external returns (bool);
