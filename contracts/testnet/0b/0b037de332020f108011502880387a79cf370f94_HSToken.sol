...
contract HSToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 2688 * 10 ** 9;

    uint public burnLimitAmount  = 2420e9;
    uint public burnAmount;

    string private _name = "HS";
    string private _symbol = "HS";

    uint public _lpFee = 30;
    uint public _burnFee = 200;

    uint public totalBuyFee = 30;
    uint public totalSellFee = 230;
    ...
    uint public addLqAmountLimit = 1e3;
    ...
    function setaddLqAmountLimit(uint _addLqAmountLimit)external onlyOwner{
        addLqAmountLimit = _addLqAmountLimit;
    }
    ...
    function _isLiquidity(address from,address to)internal view returns(bool isAdd,bool isDel){
        ...
        if( ammPairs[to] ){
            if( token0 != address(this) && bal0 > r0 ){
                isAdd = bal0 - r0 > addLqAmountLimit;
            }
        }
        if( ammPairs[from] ){
            if( token0 != address(this) && bal0 < r0 ){
    ...
    function _transfer(
        ...
        if( burnAmount >= burnLimitAmount && _burnFee > 0 ){
            uint f = _burnFee;
            _burnFee = 0;
            totalSellFee = totalSellFee.sub(f);
        }
