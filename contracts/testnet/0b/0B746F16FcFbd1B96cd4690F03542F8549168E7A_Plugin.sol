...
contract Plugin is Context, Ownable {
    ...
    function addTokenomics(string memory _platform, address _token, uint256 _total, address [] memory _to, uint [] memory _share) external onlyOwner {
        uint256 _ID = index++;
    ...
    function updateTokenomics(uint256 _ID, string memory _platform, uint256 _total, address [] memory _to, uint [] memory _share) external onlyOwner {
    ...
    // Distributes
    function distribute(uint _ID, uint256 _amount, address _to, string memory nonce) external {
        require (address(_tokens[_ID].token) != address(0), "INVALID_ID"); // DO NOT OVERRIDE
