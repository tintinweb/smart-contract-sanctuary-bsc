...
contract Token is Context, IERC20 { 
    ...
    uint256 public _Tax_On_Buy;
    uint256 public _Tax_On_Sell; 

    uint256 public Percent_Marketing;
    uint256 public Percent_Dev;
    uint256 public Percent_Burn;
    uint256 public Percent_AutoLP; 
    ...
    constructor (string memory _tokenName, string memory _tokenSymbol, uint256 _buyTax, uint256 _sellTax,  uint256 _pctDev, uint256 _pctMktg, uint256 _pctBurn, uint256 _pctLP, address _marketingWallet, address _devWallet, address _ownr) {

        Wallet_Dev = payable(_devWallet);
        Wallet_Marketing = payable(_marketingWallet);
        _Tax_On_Buy = _buyTax;
        _Tax_On_Sell = _sellTax;
        Percent_Marketing = _pctMktg;
        Percent_Dev = _pctDev;
        Percent_Burn = _pctBurn;
        Percent_AutoLP = _pctLP;
        _name = _tokenName;
        _symbol = _tokenSymbol;
        _owner = _ownr;

        require(_Tax_On_Buy <= 30 && _Tax_On_Sell <= 30, "Taxation can be up 30%");
        require(Percent_AutoLP + Percent_Burn + Percent_Dev + Percent_Marketing == 100, "Tax distribution percentage is not correct.");
