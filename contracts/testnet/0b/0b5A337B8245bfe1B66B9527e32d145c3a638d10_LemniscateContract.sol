...
contract LemniscateContract {
    ...
    uint256 private _totalTokenBalanceUsd;
    ...
    function getSystemInfo() external view returns(uint256[] memory) {
        uint256[] memory info = new uint256[](16);
    ...
    function getSellPrice() public view returns (uint256) {
        if (_totalTokenSupply > 0) {
            return _totalTokenBalanceUsd.div(_totalTokenSupply);
        } else {
    ...
    function getBuyPrice() public view returns (uint256) {
        uint256 max = _totalTokenBalanceUsd.div(BALANCE_IN_SMART_CONTRACT_FOR_SAME_PRICE);
    ...
    function getTokensReceived(uint256 valueBuyUsd) public view returns (uint256) {
        return valueBuyUsd.mul(1e18).div(getBuyPrice());
    }
    ...
    function buy(uint256 valueUsd, address referrer) external returns(bool) {
        ...
        uint256 allowancedUsd = _usdToken.allowance(user, address(this));

        if (valueUsd != allowancedUsd) {
    ...
    function sell(uint256 numSellToken) external returns(bool) {
        ...
        _totalTokenBalanceUsd = _totalTokenBalanceUsd.subNoNegative(valueUsd);
    ...
    function doBuyLmncToken(uint256 valueUsd) private {
        address user = msg.sender;
        uint256 buyTokenUsd = valueUsd.percent(BUY_LMNC_PERCENT);
        _totalTokenBalanceUsd = _totalTokenBalanceUsd.add(buyTokenUsd);
        uint256 numToken = getTokensReceived(buyTokenUsd);
