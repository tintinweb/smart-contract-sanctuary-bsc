...
contract BeastTokenV1 is TokenBasic {
    ...
    uint16 public rewardPercentage; // Using 2 decimals
    uint16 public marketingPercentage; // Using 2 decimals
    uint16 public burnPercentage; // Using 2 decimals

    event RewardUpdated(address newRewardAddress);
    event MarketingUpdated(address newMarketingAddress);
    event PercentageUpdated(uint16 rewardPercentage, uint16 marketingPercentage, uint16 burnPercentage);

    function setReward(address newAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        reward = newAddress;
        emit RewardUpdated(newAddress);
    }
    ...
    function setMarketing(address newAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
     ...
     function setPercentage(uint16 rewardPrctg, uint16 marketingPrctg, uint16 burnPrctg) external onlyRole(DEFAULT_ADMIN_ROLE) {            
        rewardPercentage = rewardPrctg;
        marketingPercentage = marketingPrctg;
        burnPercentage = burnPrctg;
        emit PercentageUpdated(rewardPercentage, marketingPercentage, burnPercentage);
    }
    ...
    */
    ...
    function _taxDeduction(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if(!(_isExcludedFromFees[sender] || _isExcludedFromFees[recipient])) {        
            ...
            require(reward != address(0) && marketing != address(0), "Tax : Address not set correctly");
            require(rewardPercentage + marketingPercentage + burnPercentage != 0, "Tax : Percentage not set correctly");

            // Calculate amount
            uint256 rewardAmount = uint256(rewardPercentage) * amount / 100_00;
            uint256 marketingAmount = uint256(marketingPercentage) * amount / 100_00;
            uint256 burnAmount = uint256(burnPercentage) * amount / 100_00;

            // Transfer token        
            _transfer(recipient, reward, rewardAmount);
