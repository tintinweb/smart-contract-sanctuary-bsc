...
contract LuckySpin is Ownable, Initializable, Pausable, ERC1155Holder {
    ...
    function updateTotalSlot(uint256 _roundId, uint256 _totalSlots) public onlyOwner {
    ...
    function updateTicketPrice(uint256 _roundId, uint256 _ticketPrice) public onlyOwner {
        RoundInfo storage info = roundInfos[_roundId];
        require(info.status == 1, "Invalid round");
    ...
    function spinReward(
        ...
        nextRoundInfo.roundId = nextRoundId;
    ...
    function getJoins(address _user) public
    view
    returns
    (uint256[] memory, uint256[] memory, address[] memory, uint256[] memory) {
        uint256 count = numOfJoins[_user];
        uint256[] memory roundIds = new uint256[](count);
        uint256[] memory stakedTimes = new uint256[](count);
        address[] memory winners = new address[](count);
        uint256[] memory status = new uint256[](count);
        ...
        if (count > 0) {
            ...
            for (uint256 i = 1; i <= currentRoundId; i++) {
                ...
                if (info.userStakedTokens[_user] != 0 && (info.status == 3 || info.status == 1)) {
                    roundIds[idx] = info.roundId;
                    stakedTimes[idx] = info.userStakedTimes[_user];
                    winners[idx] = info.winner;
                    status[idx] = info.status;
                    idx++;
                }

            }
        }
        return (roundIds, stakedTimes, winners, status);
    }
