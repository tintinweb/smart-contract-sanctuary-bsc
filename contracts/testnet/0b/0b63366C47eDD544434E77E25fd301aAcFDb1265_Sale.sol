...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    function creates(address _caller, bool _isCreateNewToken, bool _isSetRoyalty, ProjectInfo memory _projectInfo, SaleInput[] memory _saleInputs) external onlyProject returns(uint256) {
    ...
    function _createSale(address _caller, uint256 _saleId, bool _isCreateNewToken, bool _isSetRoyalty, ProjectInfo memory _project, SaleInput memory _saleInput) private returns (SaleInfo memory) {
        ...
        if (_project.isSingle) {
            if (_isCreateNewToken) {
                sale.tokenId = _isSetRoyalty ? IOSB721(_project.token).mintWithRoyalty(address(this), _saleInput.royaltyReceiver, _saleInput.royaltyFeeNumerator) : IOSB721(_project.token).mint(address(this));
            } else {
                IOSB721(_project.token).safeTransferFrom(_caller, address(this), _saleInput.tokenId);
            }
        } else {
            if (_isCreateNewToken) {
                sale.tokenId = _isSetRoyalty ? IOSB1155(_project.token).mintWithRoyalty(address(this), _saleInput.amount, _saleInput.royaltyReceiver, _saleInput.royaltyFeeNumerator) : IOSB1155(_project.token).mint(address(this), _saleInput.amount);
            } else {
