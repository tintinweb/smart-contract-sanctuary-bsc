...
contract Unisynths is Ownable, ReentrancyGuard {
    ...
    event MintToken(address indexed user, string token,uint256 amount);
    event BurnToken(address indexed user, string token,uint256 amount);
   ...
   function bytes32ToString(bytes32 _bytes32) internal pure returns (string memory) {
    ...
    function getBoldPrice(string memory _kBTC, string memory _kGOLD) public view returns (uint256) {
    ...
    function getSdrPrice(string memory _kCNY, string memory _kYEN , string memory _kGBP , string memory _kEUR) public view returns (uint256) {
    ...
    function mintSynth(string memory _ticker,uint256 _amount) tokenExist(_ticker) nonReentrant public {
        ...
        if(keccak256(abi.encodePacked(_ticker)) == keccak256(abi.encodePacked(kBOLD))){
            ...
            string memory kBTC = "kBTC";
            string memory kGOLD = "kGOLD";

            synthPrice = getBoldPrice(kBTC,kGOLD);

        } else if (keccak256(abi.encodePacked(_ticker)) == keccak256(abi.encodePacked(kSDR))){
            
            string memory kCNY = "kCNY";
            string memory kYEN = "kYEN";
            string memory kGBP = "kGBP";
            string memory kEUR = "kEUR";
        ...
        // Return reserve balance in usd
          Token[] memory _tokens = new Token[](tokenList.length);
            for (uint i = 0; i < tokenList.length; i++) {
                ...
                uint256 tokenprice = getLatestPrice(convertedTicker);
