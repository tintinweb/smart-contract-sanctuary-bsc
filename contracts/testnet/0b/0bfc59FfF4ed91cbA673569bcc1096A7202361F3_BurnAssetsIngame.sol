...
contract BurnAssetsIngame is AccessControl {
    ...
    function burnHero(address asset, uint256[] memory tokenId) external {
        require(hasRole(BURNER, address(msg.sender)), "Caller is not a buner");
        IERC721 Asset = IERC721(asset);
        for(uint256 i = 0; i < tokenId.length; i++){
            Asset.burn(msg.sender, tokenId[i]);
            emit burnhero(
                msg.sender,
                tokenId[i],
                block.timestamp
