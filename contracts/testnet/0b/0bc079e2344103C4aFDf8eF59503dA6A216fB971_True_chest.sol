...
mapping (address => mapping (uint256 => mapping (uint256 => mapping (uint256=>uint256)))) public LevelPlayer;
...
public virtual override view  returns (uint256) {
    return  LevelPlayer[adr][num_track][num_chest][Num];
}

function Get_LevelPlayer_num (address adr,uint256 num_track, uint256 num_chest) 
public virtual override view  returns (uint256) {
     return  LevelPlayer[adr][num_track][num_chest][Get_chest(adr, num_track, num_chest)];
}
...
function Take_chest (uint256 num_track, uint256 num_chest,address adr) public virtual override {
    ZoomLoupe=dis.getZoomLoupe();
    require(msg.sender == ZoomLoupe);
    LevelPlayer[adr][num_track][num_chest][Get_chest(adr, num_track, num_chest)]=0;
    if (num_chest == 1) {
...
function T_chest (address adr, uint256 num_track, uint256 num_chest, uint256 LevelPlayer_ ) external onlyOwner{
    ZoomLoupe=dis.getZoomLoupe();
    if (num_chest == 1) {
        chest1[adr][num_track]++;
    }
    ...
    LevelPlayer[adr][num_track][num_chest][Get_chest(adr, num_track, num_chest)]=LevelPlayer_;
}
}
