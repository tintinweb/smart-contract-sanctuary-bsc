...
contract FleaNFT is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    // Mapping to check if token id number is has type "Common or Rare or Legendary" Rartity Enum
    ...
    function userMint(uint256 qty, uint256 amount, Rarity rarity) external {
        ...
        require(
            rarity == Rarity.Common &&
                _totalCommonMinted <
                MAX_SUPPLY - (_rareNumbers.length + _legendaryNumbers.length),
            "All Common Rarity has been minted"
        );
        require(
            rarity == Rarity.Rare && _totalRareMinted < _rareNumbers.length,
    ...
    */
    ...
    function _getMintTokenId(
        uint nextCommonNumber,
        Rarity rarity
    ) internal view returns (uint) {
        if (rarity == Rarity.Common) {
            while (tokenIdRarity[nextCommonNumber] != Rarity.Common) {
                nextCommonNumber++;
            }
            return nextCommonNumber;
    ...
    function _safeMint(address to, Rarity rarity) internal {
        uint getUpcomingMintTokenId = _getMintTokenId(_totalSupply, rarity);
