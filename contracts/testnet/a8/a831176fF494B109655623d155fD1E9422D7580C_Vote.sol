...
contract Vote is Auth {
    ...
    function test() public view returns (string[] memory, uint[] memory) {
        string[] memory names = new string[](voting_materials.length);
        uint[]    memory counts = new uint[](voting_materials.length);

        for (uint i = 0; i < voting_materials.length; i++) {
            names[i] = voting_materials[i].name;
            counts[i] = voting_materials[i].options.length;
        }
