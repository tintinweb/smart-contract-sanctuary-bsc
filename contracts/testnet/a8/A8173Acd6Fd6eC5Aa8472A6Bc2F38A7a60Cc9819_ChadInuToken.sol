...
contract ChadInuToken is Context, IBEP20, Ownable {
    ...
    uint256 public unlockDevTokenDelay = 0;
    uint256 public unlockCexTokenDelay = 0;
    ...
    constructor () {
        ...
        _balances[_msgSender()] = _tTotal.mul(51).div(100);
        emit Transfer(address(0), owner(), _tTotal.mul(50).div(100));
        _balances[_msgSender()] = _tTotal.mul(10).div(100);
        emit Transfer(owner(), address(0), _tTotal.mul(41).div(100));
    }
    ...
    function extendCexLockTime(uint256 duration) external onlyOwner {
        require(unlockCexTokenDelay + duration < 1 * 183);
        unlockCexTokenDelay = unlockCexTokenDelay + duration;
    }

    function extendDevTokenLockTime(uint256 duration) external onlyOwner {
        require(unlockDevTokenDelay + duration < 1 * 91);
        unlockDevTokenDelay = unlockDevTokenDelay + duration;
    }

    function releaseLockedCex(address account) external onlyOwner {
        require(block.timestamp > lockCexTime + unlockCexTokenDelay, "You have to wait to unlock cex.");
        ...
        lockCexTime = lockCexTime + 1 * 183;
        unlockCexTokenDelay = 0;
        emit Transfer(address(0), account, _tTotal.mul(10).div(100));
    }

    function releaseLockedDevTokens(address account) external onlyOwner {
        require(block.timestamp > lockDevTime + unlockDevTokenDelay, "You have to wait to unlock dev tokens.");
        ...
        lockDevTime = lockDevTime.add(1 * 91);
        unlockDevTokenDelay = 0;
