...
contract SEESAW is Context, IERC20, Ownable {
    ...
    uint8 public maxBuyTax = 15;
    uint8 public maxSellTax = 20;
    ...
    function updateTaxes(bool isBuy) private {
        if (isBuy) {
            if (buyTax < maxBuyTax) {
                buyTax++;
                sellTax--;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        require(_balances[_msgSender()] >= amount, "SEESAW: transfer amount exceeds balance");
        updateTaxes(false);
