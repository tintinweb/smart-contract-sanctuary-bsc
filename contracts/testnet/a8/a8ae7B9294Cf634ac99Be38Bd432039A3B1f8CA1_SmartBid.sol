...
contract SmartBid is  Ownable{
    ...
    function _fundOthers(uint256 pid, bool status, uint256 rem, uint256 num) internal {
        ...
        tokenInfo memory info = request_token_info[pid];
        if (status) {
            address[] memory others = new address[](num);
            uint256 projLength = projBidders[pid].length;
            uint256 all = info.multiplier;
            ...
            uint256 individual = rem.div(others.length);
            uint256 share = (individual.mul(all));
            // uint256 remnt;
            for (uint256 x; x < tempLength; x++) {
                // address wallet = others[x];
            ...
            for (uint256 j = 0; j < Have.length; j++) {
                address wallet = Have[j];
                info.token.transfer(wallet, share);
    ...
    function fundRand(uint256 pid, uint256 remt) internal {
        ...
        uint256 share = (individual.mul(info.multiplier));
        if (iHave.length > 0 ) {
            for (uint256 i = 0; i < iHave.length; i++) {
                if (iHave[i] != address(0)) {
                    info.token.transfer(iHave[i], share);
                    emit luckyWinnerWithReward(iHave[i], pid, share, block.timestamp);
                }
            }
        }
        if (idHave.length > 0) {
            for (uint256 i = 0; i < iHave.length; i++) {
                if(idHave[i] != address(0)) {
                    info.token.transfer(idHave[i], individual);
                    emit luckyWinnerWithoutReward(iHave[i], pid, individual, block.timestamp);
                }
            }
        }
    }
    ...
    function ownedNFTForRand(uint256 pid) public view returns(address[] memory iHave, address[] memory idHave, uint256 wtWAllGet) {
        ...
        uint256 numOwned;
        uint256 numNotOwned;
        iHave = new address[](lent);
        idHave = new address[](lent);

        for (uint256 i; i < lent; i++) {
            if(bidders[pid][ratedAddress[i]].tokenOwned != 0) {
                numOwned ++;
        ...
        uint256 multi = info.multiplier;
        wtWAllGet = ((numOwned).mul(multi)).add(numNotOwned);
    ...
    function b4TForRand(uint256 pid, address _user, uint256 _remnt) internal view returns(uint256 wUGet, bool status) {
        ...
        uint256 multi = info.multiplier.div(1E18);
        biddersInfo memory bidder = bidders[pid][_user];
        if(bidder.tokenOwned != 0) {
            wUGet = RandUser.mul(multi);
            status = true;
    ...
    function fundTopBidders(uint256 pid) internal returns (bool, uint256 rem) {
        ...
        tokenInfo memory info = request_token_info[pid]; 
        (address FirstTBidder, address secondTBidder, address thirdTBidder) = Top3Bidders(pid);
        (uint256 nPOne, uint256 nPTwo,  uint256 nPThree) = bTransfer(pid, FirstTBidder, secondTBidder, thirdTBidder);
        (, ,, , uint256 devShare, ) = position(pid);
        uint256 gtRem;
    ...
    function bTransfer(
        ...
        view
        returns
        (uint256 nPOne, uint256 nPTwo,  uint256 nPThree) {
        tokenInfo memory info = request_token_info[pid];
        (uint256 positionOne, uint256 positionTwo, uint256 positionThree, , , ) = position(pid);
        uint256 share = (info.multiplier);

        if(bidders[pid][FirstTBidder].tokenOwned != 0) {
            nPOne = positionOne.mul(share);
        } else {nPOne = positionOne;}
        if(bidders[pid][secondTBidder].tokenOwned != 0) {
            nPTwo = positionTwo.mul(share);
        } else {nPTwo = positionTwo;}
        if(bidders[pid][thirdTBidder].tokenOwned != 0) {
            nPThree = positionThree.mul(share);
