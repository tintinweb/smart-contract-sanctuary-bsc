...
contract HMTLaunchpadFactoryMasterV1 is HMTLaunchpadFactoryBaseV1 {
    ...
    function setLaunchpadDetail(address owner,address presaleAddress,uint8 launchpadType) public override onlyAllowedFactory {
        require(!presaleOwnerToPresaleToHasLaunchpad[owner][presaleAddress],"Presale already exists");
        presaleOwnerAddressToLaunchpadList[owner].push(Launchpad(presaleAddress,launchpadType,PresaleOwnerDetail(false,false,false,false)));
        launchpadArray.push(Launchpad(presaleAddress,launchpadType,PresaleOwnerDetail(false,false,false,false)));
    ...
    function setPresaleOwnerDetail(
        ...
        unchecked{        
            for(uint256 i=0;i<launchpadArray.length;++i){
                if(launchpadArray[i].presaleAddress==_presaleAddress){
        ...
        unchecked{        
            for(uint256 i=0;i<presaleOwnerAddressToLaunchpadList[_presaleOwnerAddress].length;++i){
                if(presaleOwnerAddressToLaunchpadList[_presaleOwnerAddress][i].presaleAddress==_presaleAddress){
    ...
    function getAllLaunchpadByOwner(address owner)
        ...
        returns (Launchpad[] memory)
       {
        ...
        Launchpad[] memory launchpad = new Launchpad[](
            presaleOwnerAddressToLaunchpadList[owner].length
        );
        unchecked {
            for (
                uint256 i = 0;
                ...
                launchpad[currentIndex] = presaleOwnerAddressToLaunchpadList[owner][i];
                currentIndex.add(1);
            }
        }
        return launchpad;
    ...
    function getLaunchpadByOwnerAndType(address owner, uint8 launchpadType)
      ...
      function getAllLaunchpadsByType(uint8 launchpadType)
        external
        view
        returns ( Launchpad[] memory)
    {
