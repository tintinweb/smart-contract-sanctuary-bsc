...
contract IDO is Ownable {
    ...
    function startUnlock(uint _pid, uint _firstTimePercent) external onlyOwner {
        ...
        require(idoToken.transferFrom(_msgSender(), address(this), _pool.totalAmount - _pool.remainAmount));
        _pool.idoToken2Buy.transfer(_msgSender(), totalFundRaised[address(_pool.idoToken2Buy)]);
    ...
    function addPool(IERC20 _idoToken2buy, uint _tokenBuy2IDOtoken, uint _amount, uint _totalAmount, uint _startTime, uint _endTime) external onlyOwner {
        poolInfo.push(PoolInfo(_idoToken2buy, _tokenBuy2IDOtoken, _amount, _totalAmount, _totalAmount, _startTime, _endTime, 0, false));
    }
