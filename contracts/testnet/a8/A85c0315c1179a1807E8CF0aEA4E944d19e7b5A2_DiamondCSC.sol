...
contract DiamondCSC is Context, BEPC20 {
    ...
    function price() public view virtual returns (uint256) {
        ...
        if(plata == 0 || tokens == 0){
            ...
            return (plata*(10**decimals()))/( tokens );
    ...
    function buyToken(uint256 amount) public {
        ...
        uint256 valor = (amount*( 10 ** decimals() ))/(price());

        if(!contractPool.transferFrom(_msgSender(), address(this), valor))return();
        _print(_msgSender(), amount*95/100);
        burn();
    ...
    function sellToken(uint256 amount) public {
        ...
        uint256 pago = (amount*price())/(10 ** decimals());

        if(!contractPool.transfer(_msgSender(), (pago*90)/100))return();
