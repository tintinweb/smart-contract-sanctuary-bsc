...
contract Router is Context, IAdminHolder, IRouter {
    ...
    function setFactory(ITopLevelDomainsFactory _factory)
        external
        onlyAdminAccess
    {
        factory = _factory;
    ...
    function registerTopLevelDomain(ITopLevelDomain tld)
        external
        onlyFactoryAccess
        onlyIfTLDNotRegistered(tld.name())
    {
        tlds[tld.name()] = tld;
        emit TopLevelDomainRegistered(tld);
    }

    function _getTLD(string calldata _tld)
        internal
        view
        returns (ITopLevelDomain)
    {
    ...
    function domainInfo(string calldata _domain, string calldata _tld)
        external
        view
        returns (DomainInformation memory)
    {
    ...
    function subdomainInfo(
        ...
        return
            DomainInformation({
                tld: tld,
                tokenId: subdomainId,
                owner: tld.ownerOf(subdomainId)
            });
    }

    function topLevelDomain(string calldata _tld)
        external
        view
        returns (ITopLevelDomain)
    {
