  ...
  function Reduceproduction()public view  returns(uint256)  {
    ...
    function withdraw() external {
      ...
      //  userRewards.split = userRewards.split.add(splitAmt);
        ...
        uint256 withdrawable = userRewards.statics.add(userRewards.level4Released).add(userRewards.level5Released).add(userRewards.top).add(userRewards.luck);
    ...
    function _calLevelNow(address _user) private view returns(uint256) { 
        ...
        if(total >= 100e18){
            ...
            if(total >= 2000e18 && user.teamNum >= 30 && maxTeam >= 50000e18 && otherTeam >= 50000e18){// 等级5 200人 
                levelNow = 5;
            }else if(user.teamNum >= 20 && maxTeam >= 10000e18 && otherTeam >= 10000e18){// 等级4 50人 
                levelNow = 4;
            }else if(user.teamNum >= 10 && maxTeam >= 5000e18 && otherTeam >= 5000e18){//等级3 10人
                levelNow = 3;
        }else if(user.teamNum >= 20 && maxTeam >= 2000e18 && otherTeam >= 2000e18 ){//等级2 5人
            levelNow = 2;
        }else if(total >= 50e18){
            levelNow = 1;
        }
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < orderInfos[_user].length; i++){
            ...
            if(block.timestamp > order.unfreeze  && order.isUnfreezed == false && _amount >= order.amount){
                ...
                uint256 splitStaticReward = staticReward.mul(freezeIncomePercents).div(baseDivider);
                uint256 trueStaticReward = staticReward.sub(splitStaticReward);
                order.profit=order.profit.add(trueStaticReward);
                if(isFreezeReward){
                    if(user.totalFreezed > user.totalRevenue){
                        uint256 leftCapital = user.totalFreezed.sub(user.totalRevenue);

                        if(trueStaticReward > leftCapital){
                            trueStaticReward = leftCapital;
                        }
                    }else{
                        trueStaticReward = 0;
                    }
                ...
                rewardInfo[_user].statics = rewardInfo[_user].statics.add(trueStaticReward);
                rewardInfo[_user].split = rewardInfo[_user].split.add(splitStaticReward);
                ...
                if(i > 4){
                    if(userInfo[upline].level > 4){
                        reward = newAmount.mul(level5Percents[i - 5]).div(baseDivider).mul(Reduceproduction()).div(10000);
                          reward.mul(freezeIncomePercents).div(baseDivider);
                          reward.sub(splitReward);
                  ...
                  reward = newAmount.mul(level4Percents[i - 1]).div(baseDivider).mul(Reduceproduction()).div(10000);
                    splitReward =  reward.mul(freezeIncomePercents).div(baseDivider);
                    trueReward = reward.sub(splitReward);
                  upRewards.split = upRewards.split.add(splitReward);
    ...
    function _releaseUpRewards(address _user, uint256 _amount) private {
        ...
        uint256 splitLevel4Reward;
        uint256 trueLevel4Reward;
        uint256 level5Reward ;
        uint256 splitLevel5Reward ;
        uint256 trueLevel5Reward ;
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                uint256 newAmount = _amount;
                if(upline != defaultRefer){
                ...
                if(i > 0 && i < 5 && userInfo[upline].level > 3){
                    if(upRewards.level4Freezed > 0){
                         level4Reward = newAmount.mul(level4Percents[i - 1]).div(baseDivider);
                         splitLevel4Reward = level4Reward.mul(freezeIncomePercents).div(baseDivider);
                         trueLevel4Reward =level4Reward.sub(splitLevel4Reward);
                        if(trueLevel4Reward > upRewards.level4Freezed){
                            trueLevel4Reward = upRewards.level4Freezed;
                        }
                        upRewards.level4Freezed = upRewards.level4Freezed.sub(trueLevel4Reward); 
                        upRewards.level4Released = upRewards.level4Released.add(trueLevel4Reward);
                        upRewards.split = upRewards.split.add(splitLevel4Reward);
                        userInfo[upline].totalRevenue = userInfo[upline].totalRevenue.add(trueLevel4Reward);
                    }
                }

                if(i >= 5 && userInfo[upline].level > 4){
                    if(upRewards.level5Left > 0){
                         level5Reward = newAmount.mul(level5Percents[i - 5]).div(baseDivider);
                         splitLevel5Reward =level5Reward.mul(freezeIncomePercents).div(baseDivider);
                         trueLevel5Reward =level5Reward.sub(splitLevel5Reward);
                        if(trueLevel5Reward > upRewards.level5Left){
                            trueLevel5Reward = upRewards.level5Left;
                        }
                        upRewards.split =  upRewards.split.add(splitLevel5Reward);
                        upRewards.level5Left = upRewards.level5Left.sub(trueLevel5Reward); 
                        upRewards.level5Freezed = upRewards.level5Freezed.add(trueLevel5Reward);
                    }
                }
                upline = userInfo[upline].referrer;
            }else{
