...
contract StakingSet is StakingSetStorage, IBEP721, IBEP721Metadata {
   ...
   // uint RewardDebt;
    ...
    function getNFTfields(uint tokenId) external view returns (NFTFields[] memory) {
        NFTFields[] memory nftFields = new NFTFields[](3);
        (uint256 nbuReward, uint256 gnbuReward, uint256 cakeReward) = getTokenRewardsAmounts(tokenId);
        nftFields[0].pool = address(NbuStaking);
        nftFields[0].rewardToken = address(nbuToken);
        nftFields[0].rewardAmount = nbuReward;
        nftFields[0].percentage = 35 ether;
        nftFields[1].pool = address(GnbuStaking);
        nftFields[1].rewardToken = address(nbuToken);
        nftFields[1].rewardAmount = gnbuReward;
        nftFields[1].percentage = 35 ether;
        nftFields[2].pool = address(CakeStaking);
        nftFields[2].rewardToken = address(cakeToken);
        nftFields[2].rewardAmount = cakeReward;
        nftFields[2].percentage = 30 ether;
        return nftFields;
    }
    ...
    function burnStakingSet(uint tokenId, address tokenOwner) external nonReentrant {
      //require(msg.sender == hubRouting, "StakingSet:: Caller is not the HubRouting contract");
        ...
        pancakeRouter.removeLiquidityBNB(address(cakeToken), userSupply.CakeBnbAmount, 0, 0, tokenOwner, block.timestamp);
    ...
    function getUserCakeRewards(uint256 tokenId) public view returns (uint256) {
        ...
        IMasterChef.UserInfo memory userInfo = CakeStaking.userInfo(CAKE_PID, address(this));
        ...
        return boostedAmount * accCakePerShare / ACC_CAKE_PRECISION - userInfo.rewardDebt - userSupply.RewardDebt;
    }
