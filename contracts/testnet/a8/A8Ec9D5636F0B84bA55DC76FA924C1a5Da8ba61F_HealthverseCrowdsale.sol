...
contract HealthverseCrowdsale is Context {
  ...
  uint256 vestingPeriodPercentage;
  ...
  function vesting(uint256 _vestingPeriod, uint256 _releaseTime, uint256 _vestingPeriodPercentage, uint256 _vestingPeriodTime) public
  {
    require(isAdmin[msg.sender],"only-admin");
  ...
  public { 
   ...
   founderMinPercentageTokenAmount[founderThree] = founderThreePercentage *(10**decimals);  
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount) internal onlyWhileOpen {
        ...
        require(isWhitelisted[beneficiary], "WhitelistCrowdsale: beneficiary  tis not in the Whitelisted List");
  ...
  //Founder Vesting////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //@return the Founder alreadey withdrawn Percentage.
    function showWithdrawnPercentage() public view returns (uint){
        require(founder[msg.sender],"You are no Founder");
        return founderShareWithdrawn[msg.sender];
    }

    //@return the Founders left withdrawn Percentage.
    function showAvailableWithdrawPercentage() public view returns (uint){
        require(founder[msg.sender],"You are no Founder");
        uint availablePercentage = (vestingPeriodPercentage*((block.timestamp - releaseTime)/vestingPeriod));
        if(availablePercentage >= 100){
            availablePercentage = 100;
            return  availablePercentage - founderShareWithdrawn[msg.sender];
        } else return availablePercentage - founderShareWithdrawn[msg.sender];
    }

    // withdraws the available Percentage of Founder Share.
    function withdrawFounderShareAvailable(address payable beneficiaryFounderAddress) public payable {

        require(founder[msg.sender],"Vesting: You are no Founder");
        require(founderShareWithdrawn[msg.sender] < 100,"Vesting: You have already withdrawn 100%");
        vestingPeriodCount[msg.sender] = ((block.timestamp - releaseTime)/vestingPeriod);

        if(vestingPeriodCount[msg.sender] >= vestingPeriodTime && founderShareWithdrawn[msg.sender] > 0) 
        { 
        vestingPeriodCount[msg.sender] = vestingPeriodTime - (founderShareWithdrawn[msg.sender].div(2));
        }  else if(vestingPeriodCount[msg.sender] >= vestingPeriodTime) {
            vestingPeriodCount[msg.sender] = vestingPeriodTime;
        } else if(founderShareWithdrawn[msg.sender] > 0) {
            vestingPeriodCount[msg.sender] = vestingPeriodCount[msg.sender] - (founderShareWithdrawn[msg.sender].div(2));
        } 

        transferFounderShare(beneficiaryFounderAddress, founderMinPercentageTokenAmount[msg.sender].mul(vestingPeriodCount[msg.sender]));
        founderShareWithdrawn[msg.sender] = founderShareWithdrawn[msg.sender] + vestingPeriodCount[msg.sender].mul(vestingPeriodPercentage);
