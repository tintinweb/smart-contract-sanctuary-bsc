 ...
 * @notice It distributes GLR tokens with rolling Merkle.
 */
contract TradingRewardsDistributor is Pausable, ReentrancyGuard, Ownable {
    ...
    function claim(uint256 amount, bytes32[] calldata merkleProof) external whenNotPaused nonReentrant {
        ...
        require(!hasUserClaimedForRewardRound[currentRewardRound][msg.sender], "Claim: Already claimed");

        (bool claimStatus, uint256 adjustedAmount) = _canClaim(msg.sender, amount, merkleProof);

        require(claimStatus, "Claim: Invalid proof");
        require(maximumAmountPerUserInCurrentTree >= amount, "Claim: Amount higher than max");
