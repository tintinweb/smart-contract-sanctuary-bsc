 ...
 */
contract USDyVault is AccessControl, Initializable {
    ...
    uint256 public rebaseRate = 25; //cofficient by 100
    uint256 public epochTime = 8 hours;//setting epochtime as uint
    ...
    @param _admin Admin contract address
           Admin.USDs(); USDs token address
           Admin.USDy(); USDy token address
           Admin.BUSD(); BUSD token address
     */
    function initialize(
        address _admin,
        address owner
        ...
        INTT(USDs).initialize(address(this), "USDs", "USDs");
    ...
    function Deposit(uint256 _amount) external {
        ...
        IUSDY(Admin.USDy()).transferFrom(msg.sender, address(this), _amount);
        console.log("ratio before mint",exchangeRatio);
        exchangeRatio = exchangeRatio == 0 ? 10 ** 18 : exchangeRatio;
        INTT(USDs).mint(address(this), (_amount * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100));
        console.log("mint",(_amount * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100));
        UserDeposit[msg.sender] += (_amount * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100);
        console.log("UserDeposit",UserDeposit[msg.sender]);
        console.log(" INTT(Admin.USDs()).totalSupply()",INTT(USDs).totalSupply());
        exchangeRatio = (IERC20(Admin.USDy()).balanceOf(address(this)) * (10 ** 18)) / INTT(USDs).totalSupply();  
        console.log("ratio after mint",exchangeRatio);
    }
    ...
    function Withdraw(address user,uint256 amount) external {
        require(amount <= UserDeposit[user],"USDyVault : Your withdraw amount exceeds deposit");
        uint balance = (amount * exchangeRatio)/10 ** 18;
        IERC20(Admin.USDy()).transfer(msg.sender, balance);
        UserDeposit[user] -= amount;
        INTT(USDs).burn(address(this), amount);
    }
    ...
    */
    ...
    function reBaseRewards() external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(block.timestamp >= 1 hours + currentTimestamp,"USDyVault : Rebase happens only after this epoch");
        uint mintReward = ((IERC20(USDs).totalSupply() * exchangeRatio) * rebaseRate)/100;
     ...
     */
    function setRebaseRateUSDY(uint256 rate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(rate > 0, "USDyVault: Rate must be greater than zero");
        rebaseRate = rate;
    }
