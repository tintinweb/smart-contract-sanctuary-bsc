...
contract MonthlyLottery is Initializable, IMonthlyLottery, OwnableUpgradeable {
    ...
    event UserWon(address user, uint256 reward);
    ...
    function addParticipant(address _participant) external override {
    ...
    function startLottery() onlyOwner external {
        require(participants.length >= 10, "Lottery:Not enough participants");
        ...
        address[10] memory winners = _getWinners();
        nextLotteryTimestamp = block.timestamp + lotteryPeriod;
        _payProcess(winners);
        lastDrawBlock = block.number;
    }

    function _getWinners() internal returns (address[10] memory winners) {
    ...
    function _findFirstWinner() internal returns (address) {
        lotteryCounter++;
    ...
    function _payProcess(address[10] memory winners) internal {
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            emit UserWon(winners[i], prize);
        }
        // IStorage(storageAddress).recordMonthlyLotteryPayment(
        //     winnersAddresses,
        //     prizesAmounts
        // );
        delete participants;
