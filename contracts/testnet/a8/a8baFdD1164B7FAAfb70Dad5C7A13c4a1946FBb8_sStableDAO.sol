...
interface IgSTABLEToken is IERC20 {
  ...
  function migrate( address _staking, address _sSTABLEToken ) external;
}
...
contract sStableDAO is IsSTABLEToken, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgSTABLEToken public gSTABLEToken; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked STABLE Token", "sSTABLE Token", 18) ERC20Permit("Staked STABLE Token") {
    ...
    function setIndex(uint256 _index) external {
        require(msg.sender == owner, "Initializer:  caller is not initializer");
        require(INDEX == 0, "Cannot set INDEX again");
        INDEX = gonsForBalance(_index);
    }

    function setgSTABLEToken(address _gSTABLEToken) external {
        require(msg.sender == owner, "Initializer:  caller is not initializer");
        require(address(gSTABLEToken) == address(0), "gSTABLEToken:  gSTABLEToken already set");
        require(_gSTABLEToken != address(0), "gSTABLEToken:  gSTABLEToken is not a valid contract");
        gSTABLEToken = IgSTABLEToken(_gSTABLEToken);
    }
    ...
    /**
        @notice increases rSTABLEToken supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sSTABLEToken of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sSTABLEToken
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sSTABLEToken: insufficient balance");
    }
    ...
    // toG converts an sSTABLEToken balance to gSTABLEToken terms. gSTABLEToken is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gSTABLEToken.balanceTo(amount);
    }

    // fromG converts a gSTABLEToken balance to sSTABLEToken terms.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gSTABLEToken.balanceFrom(amount);
    }

    // Staking contract holds excess sSTABLEToken
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gSTABLEToken.balanceFrom(IERC20(address(gSTABLEToken)).totalSupply())).add(
