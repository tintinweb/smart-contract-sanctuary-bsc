...
contract EmporiaWorldMoreLand is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  string public ipfsGateway = "https://ipfs.warena.io/ipfs/";
  ...
  string[] areas = ["Eshuya", "Moonlit Glades", "Forlorn Woods", "Deadthorn Lands", "Terminus Veil"];

  struct LandData {
    int256[2] coordinates; // X,Y
    uint16[3] elements; // earth, tree, water
    uint8 area; // 
  }

  mapping(uint => string) public tokenMetadataIPFSHashes;
  mapping(string => uint) private ipfsHashTokenIds;
  mapping(uint => LandData) private tokenLandData;
  ...
  ) ERC721("Emporia World", "Emporia World") {
  ...
  function updateMetadataIPFSHash(uint _tokenId, string calldata _tokenMetadataIPFSHash) tokenExists(_tokenId) external onlyOwner {
  ...
  function getCoordinate(uint _tokenId) tokenExists(_tokenId) external view returns (int256[2] memory) {
    return tokenLandData[_tokenId].coordinates;
  }

  function getElements(uint _tokenId) tokenExists(_tokenId) external view returns (uint16[3] memory) {
    return tokenLandData[_tokenId].elements;
  }

  function getArea(uint _tokenId) tokenExists(_tokenId) external view returns (string memory) {
    return areas[tokenLandData[_tokenId].area];
  }

  function getTotalMinted() external view returns (uint) {
    return totalMinted;
  }
  function getAddressMintCount( address _addressUser) external view returns (uint) {
    return addressMintCount[_addressUser];
  }
  ...
  function mintLand(
    ...
    _safeMint(_to, _mintData._tokenId);
    addressMintCount[_to]++;
  }
  ...
  function mintLandBatch(
    MintData[] calldata _mintDatas,
    address[] calldata _toAddresses
  ) external onlyOwner {
    uint256 lengthData = _mintDatas.length;
    for (uint256 i = 0; i < lengthData; i++) {
      mintLand(_mintDatas[i], _toAddresses[i]);
    }
  }
