...
// File: contracts/SimpleERC721A.sol
    ...
    ReentrancyGuard
{
    ...
    uint256 public constant WHITELIST_MINT_LIMIT = 1500;
    uint256 public constant WHITELIST_MAX_PER_WALLET = 20;
    ...
    constructor(
        ...
        address _token
    ) ERC721A("LEGO NFT", "LEGONFT", MAX_PER_WALLET, MAX_SUPPLY) { 
        token = _token;
    ...
    modifier callerIsAdmin() {
        require(admins[msg.sender], "not enough permissions to perform this action");
        _;
    }
    ...
    function refundIfOver(uint256 _price) private {
        TransferHelper.safeTransferFrom(token, msg.sender, address(this), _price);
    }
    ...
    function addToWhitelist(address[] calldata _addresses) external callerIsAdmin {
        for (uint256 i = 0; i < _addresses.length; i++) {
    ...
    function removeFromWhitelist(address[] calldata _addresses) external callerIsAdmin {
        for (uint256 i = 0; i < _addresses.length; i++) {
            require(_addresses[i] != address(0), "cannot add the null address");
    ...
    function setPublicPrice(uint256 _price) external callerIsAdmin {
        publicPrice = _price;
    }

    function setWhitelistPrice(uint256 _price) external callerIsAdmin {
        whitelistPrice = _price;
    }

    function setPaused(bool _status) external callerIsAdmin {
        paused = _status;
    }

    function setWhitelistPaused(bool _status) external callerIsAdmin {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        require(_exists(_tokenId), "Metadata URI query for nonexistent token");
    ...
    function setBaseURI(string memory _newBaseURI) public callerIsAdmin {
        baseURI = _newBaseURI;
    }

    function setBaseExtension(string calldata _newNaseExtension) external callerIsAdmin {
        baseExtension = _newNaseExtension;
    }
