...
contract MysteryBox is Ownable{
    ...
    function extract(uint index) public {
        ...
        // 3600 * 24 * _player[index].getRandomDay
        require(block.timestamp >= _player[index].createTimestamp + 3600 * 24 * _player[index].getRandomDay, "its not time yet");
        ...
        if(_box.capitalPool <= withdrawAmount){
            ...
            IERC20(nxxToken).transfer(msg.sender, withdrawAmount);
        }
    ...
    // 用户参与盲盒抽取
    function setUser(address userAddress, uint256 _amount, uint256 _extractAmount, uint _randomDay, uint _timestamp, uint32 _boxID) public onlyOwner{

        users[userAddress].push(Player(_amount, _extractAmount, _randomDay, _timestamp, _boxID));
