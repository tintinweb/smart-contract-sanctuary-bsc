...
import "./ICrushHeartManger.sol";

contract CrushHeartBox is ERC721Enumerable, Ownable, ReentrancyGuard {
    constructor(address _crushHeartBoxManagerAddress)
        ERC721("Crush Heart Box", "DINOCHBOX")
    {
        crushHeartBoxManagerAddress = _crushHeartBoxManagerAddress;
        boxOpenableAt = block.timestamp;
        round3StartAt = block.timestamp;
        round3EndAt =  block.timestamp + 7 days;
        maxAllocationPerWallet = 1;
        totalSellingBoxes = 750;
        totalSoldBoxes = 0;
        boxPrice = 15 * 10**2 * 10**18;
    }
    ...
    function setBoxManagerAddress(address _crushHeartBoxManagerAddress)
        external
        onlyOwner
    {
        crushHeartBoxManagerAddress = _crushHeartBoxManagerAddress;
    ...
    function setMaxAllocationPerWallet(uint256 _maxAllocationPerWallet) external onlyOwner {
    ...
    function buyBox(uint256 _amount) external noContract nonReentrant {
      uint256 currentTimestamp = block.timestamp;
      require(_amount > 0, "Amount must be greater than 0");
      uint256 curentTotalBoughtBoxesByAddress = _amount.add(addressToTotalBoughtBoxes[msg.sender]);
      require(boxPrice > 0, "Box price does not set yet");
      require(curentTotalBoughtBoxesByAddress <= maxAllocationPerWallet, "You have reached your allocation");
      require(
        currentTimestamp >= round3StartAt && currentTimestamp <= round3EndAt,
        "Buy box is not available at this time"
      );
      require(totalSoldBoxes.add(_amount) <=  totalSellingBoxes, "No more boxes available");
      require(IERC20(tokenContractAddress).transferFrom(msg.sender, address(this), boxPrice.mul(_amount)), "Token transfer failed");
      addressToTotalBoughtBoxes[msg.sender] = _amount.add(addressToTotalBoughtBoxes[msg.sender]);
      totalSoldBoxes += _amount;
      Box memory newBox = Box(block.number, block.timestamp, true);
      boxes.push(newBox);
      uint256 newBoxId = boxes.length - 1;
    ...
    function openBox(uint256 _boxId) external noContract nonReentrant {
        ...
        _burn(_boxId);
         (
            uint256 dinoGenes
        ) = ICrushHeartManager(crushHeartBoxManagerAddress).calculateRewards(
                _boxId,
                box.createdAtBlock
            );
        uint256 dinoId = 0;
        ...
        emit BoxOpened(_boxId, msg.sender, dinoId);
    }
}
