...
contract CollieSTAKE is CollieContext, Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 lockPeriod, uint256 apy, uint256 tier, uint256 minTxAmount,uint256 maxTxAmount, uint256 penalty, uint256 maxallowed, uint256 maxAllowedWallets) public onlyOwner{
        Pool storage pool = pools[tier];
        ...
        pool.maxallowedTokens = maxallowed;
        pool.maxAllowedWallets = maxAllowedWallets;
        emit TierListUpdated(tier, lockPeriod, apy, minTxAmount, maxTxAmount, penalty, maxallowed, 0);
    }
