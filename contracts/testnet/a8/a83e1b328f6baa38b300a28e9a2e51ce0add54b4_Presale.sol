...
contract Presale is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 withdrawStep; // withdraw available block number 
    }
    ...
    uint256 public vestingBlock; // besting period of first public sale: block number
    ...
    uint256 public saleStartBlock; // presale status
    ...
    mapping(address => uint256) public vestinwithdrawBlock; // withdraw time of address
    ...
    constructor(
        ...
        DAO = _DAO;
        require(_saleStartBlock > block.number);
    ...
    // set vestingBlock: block number
    function setvestingBlock(uint256 _vestingBlock) external onlyOwner {
        require(!started, "Sale has already started");
    ...
    // set presale phase 
    function toggleIsPublic() external onlyOwner returns (bool) {
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        if(PrivatePhasewhitelisted[msg.sender] && isPublic == true) {
        ...
        uint256 payout = _amount.mul(1e18).div(price).div(1e9); // pBct to mint for _amount
    ...
    function withdraw(uint256 _amount) external checkIfPaused {
        ...
        require(claimable, "Bct is not yet claimable");
        require(_amount > 0, "_amount must be greater than zero");
        uint _vestingwithdrawBlock = block.number - saleStartBlock;

        UserInfo storage user = userInfo[msg.sender];
        
        if (user.withdrawStep < 5) {
        ...
        emit Withdraw(address(Bct), msg.sender, _amount);
            } else if (_vestingwithdrawBlock > vestingBlock * (30 + 10 * user.withdrawStep )) {
