...
contract bitcityzLaunchpool is Ownable {
    ...
    function keyAvailable(uint _pid, address _user) public returns(uint n) {
        uint timeSpend = block.timestamp - userInfo[_pid][_user].timeForClaim;
        uint n = timeSpend / poolInfo[_pid].lockingTime;
    }
    ...
    function claimKey(uint _pid) external {
        ...
        user.timeForClaim += timeSpend % pool.lockingTime;
