...
contract DepositBox is Ownable {
    ...
    constructor(address payable _paymentWallet) Ownable(msg.sender) {
    ...
    function _invest(uint256 _ethAmount) internal {
        ...
        paymentWallet.transfer(_ethAmount);
        ...
        uint256 numberOfTokens = _ethAmount
            ...
            .mul(10**IERC20(token[currentSale]).decimals())
            .div(1 ether);
    ...
    function setCurrentSaleIndex(uint256 _saleIndex) public onlyOwner {
        currentSale = _saleIndex;
    }

    function setPaymentWallet(address payable _newWallet) public onlyOwner {
        paymentWallet = _newWallet;
    }

    function removeStuckEth() public onlyOwner {
        paymentWallet.transfer(address(this).balance);
    }

    function removeStuckTokens(address _token) public onlyOwner {
        IERC20(_token).transfer(paymentWallet, IERC20(_token).balanceOf(address(this)));
    }
