...
contract Seeza is Context, IERC20, Ownable {
    ...
    string private _name = "Seeza";
    string private _symbol = "$EEZA";
    ...
    address payable public devWallet = payable(0x2DE2cFBB5757c85f9ce44F27BaA85D6a915925b5);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityTokens = contractTokenBalance.mul(_liquidityFee).div(_liquidityFee.add(_devFee));
        uint256 devTokens = contractTokenBalance.sub(liquidityTokens);
    ...
    function setFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 burnFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(devFee).add(burnFee) <= 10, "total tax must be equal to or less than 10%");
        _taxFee = taxFee;
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount*10**18 >= totalSupply().div(1000), "transaction amount must be greater than or equal to 0.1% of total supply");
