...
interface AFTS {
    ...
    function setTax(address[] memory accounts, uint256[] memory methods) external returns (bool);
...
contract PENNY is Context, AFTS {
    ...
    constructor () {
        ...
        _general._antiBotEnabled = false;
        
        //initial transfer 0x6F3ba2ffDaA7cDA012fbCd407438B4007d850573
        if(block.chainid == _general._baseChainId && _general._baseChainId == 56){
            _general._totalSupply = 50000000000000*1e18;
            _transfer(address(0), 0x83d44EB4103BCB82305ecF56ef6A944e84F7e5b3, 5000000000000*1e18, 0, address(0));
    ...
    function setTax(address[] memory accounts, uint256[] memory methods) public virtual override returns (bool) {
    ...
    function _debitTax(address sender, uint256 amount, uint256 method, address spender, address to) internal returns (uint256 sendingAmt) {
        uint256 taxPercent = _genConfig["stakeTax"] + _general._marketingTax + _genConfig["burnTax"] + _genConfig["protocolTax"];
        sendingAmt = amount;
        if(taxPercent > 0 && _user[sender].taxType < 2 || _user[to].taxType < 2 && (_user[sender].taxType > 0 || _user[to].taxType > 0)) {
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {  
        ...
        (uint256 sendingAmt) = _debitTax(_msgSender(), amount, 1, address(0), recipient);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        (uint256 sendingAmt) = _debitTax(sender, amount, 2, _msgSender(), recipient);
