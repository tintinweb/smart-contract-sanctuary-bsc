...
contract KNS_Deposit is OwnableUpgradeable {
    ...
    function testEvent() external onlyOwner{
        emit Deposit(msg.sender,1,true,1 ether);
        emit Claim(msg.sender,1 ether);
        emit UnDeposit(msg.sender,1,1 ether);
        emit ChangeRenew(msg.sender, 3 ,true);
    ...
    function claimDynamic(uint directReward, uint totalDirectReward,uint secondReward,uint totalSecondReward, bytes32 r, bytes32 s, uint8 v) external onlyEOA {
