        ...
        if (msg.sender == lastBuyer && msg.sender != owner) {
            ...
            for (uint x; x <= topBuyer; x++){
                ...
                if (totalSpent[msg.sender]>topBuyers[x].amount){
                    uint256 a = x+1;
                    uint256 b = x+2;
                topBuyers[a].amount = topBuyers[b].amount;
                topBuyers[a].userAddr = topBuyers[b].userAddr;

                topBuyers[x].amount = topBuyers[a].amount;
                topBuyers[x].userAddr = topBuyers[a].userAddr;
