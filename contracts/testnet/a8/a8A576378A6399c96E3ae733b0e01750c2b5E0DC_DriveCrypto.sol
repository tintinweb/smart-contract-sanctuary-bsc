...
contract DriveCrypto is GasHelper, ERC20 {
  ...
  address constant private BUSD = 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee; // BSC BUSD TESTNET
  ...
  uint constant private maxTotalFee = 1250; // fee will never ever be higher than 25% (12.5% Buy/ 12.5% sell) 
  ...
  constructor()ERC20(_nameToken, _symbolToken) {
    ...
    administrativeWallet = 0x008f01baEc225938E44c9eF850D65Cb93F02314E;
    _permissions[administrativeWallet] = 15; // exempt fee, fee receiver, tx limit and wallet limit
    internalFundWallet = 0x6e1D421b70545D2eDF2E2f8Aa8E5CdC76AA9F545;
  ...
  function setRewardWithdrawWaitTime(uint valueInSeconds) external isAdmin { 
    rewardWithdrawWaitTime = valueInSeconds; 
     if (valueInSeconds > 604800) // 1 week, Maximum Permitted
  ...
  function setContractFees(uint administrative, uint internalFund, uint rewardFee, uint burnFee) public isFinancial {
    ...
    feeReward = rewardFee;
    feeBurn = burnFee;
    require(getFeeTotal() <= maxTotalFee, "All rates and fee together must be equal or lower than 12.5%");
  }
  ...
  function setSpecialWallet(address target, bool isSender, uint administrative, uint internalFund, uint reward, uint burnFee) internal isFinancial {
    require(administrative + internalFund + reward + burnFee <= maxTotalFee, "All rates and fee together must be equal or lower than 12.5%");
  ...
  function setMaxTxAmount(uint256 maxTxAmount) public isFinancial {
    require(maxTxAmount >= maxSupply / 1000, "Amount must be bigger then 0.1% tokens"); // 1000 tokens
  ...
  function _transfer( address sender, address receiver,uint256 amount) internal override {
    ...
    if ( !isExemptSwapMaker(senderAttributes)) autoSwap(sender);
    ...
    executeRewardOperations(sender, receiver, newSenderBalance, newRecipientBalance, senderAttributes, receiverAttributes);
