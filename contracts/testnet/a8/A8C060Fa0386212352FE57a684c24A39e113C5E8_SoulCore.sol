...
contract SoulCore is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable{
    ...
    mapping(address => bool) public knights;          //判断是否为骑士
    mapping(address => address) public referrers;     //绑定推荐人
    mapping(address => address) public bindKnights;   //绑定骑士


    mapping(address => uint) public wallet;           //钱包

    function initialize(address USDT_, address SOUL_1155_, address SOUL_721_, address SOUL_TOKEN_, address platform_, address jackpot_, address shareHolder_, address receiveAddr_) public initializer {
        __Divestor_init();
        coreInfo = coreStruct({
    ...
    function createToken(uint8 gender, uint amount, uint8 soulCount, address recommender) public returns(uint){
        ...
        //往当前合约地址付款4U
        coreInfo.USDT.safeTransferFrom(_msgSender(), address(this), (amount / 100) * 4 ether);    //当前合约获得金额
        ...
        coreInfo.SOUL_721.mint(_msgSender(), tokenId);
        coreInfo.SOUL_1155.mint(address(this), tokenId, amount);
        ...
        if(soulCount > 0){
            ...
            //销毁soul
            coreInfo.SOUL_TOKEN.burnFrom(_msgSender(), uint(soulCount) * 1 ether);
        }
    ...
    function extractToken(uint8 gender, address recommender) public returns(uint){
        ...
        //往当前合约地址付款4U
        coreInfo.USDT.safeTransferFrom(_msgSender(), address(this), 4 ether);    //当前合约获得金额

        //给抽取者1枚soul========================================
        coreInfo.SOUL_TOKEN.transfer(_msgSender(), 1 ether);
        ...
        if(row == 1){
            //免费池
            tokenId = _getFree(gender, recommender, 0);
        }else{
            //氪金池
            tokenId = _getCharge(gender, recommender, 0);
        }


        require(tokenId != 0, "Unable to extract");

        coreInfo.SOUL_1155.safeTransferFrom(address(this), _msgSender(), tokenId, 1, '');
     ...
     **/
    function _getFree(uint8 gender_, address recommender_, uint8 isJump_) private returns(uint){
        uint tokenId;
        uint length = publicPool[gender_].length;   //免费池的长度
        if(length == 0){
            if(isJump_ == 0){
                //跳转到收费池
                tokenId = _getCharge(gender_, recommender_, 1);
                return tokenId;
            }else{
                return tokenId;
            }
        ...
        address caster = coreInfo.SOUL_721.ownerOf(tokenId);
        //分成规则
        _divide(recommender_, caster, 100);

        //池子数量维护
        _countMaintain(false, gender_,  0, tokenIndex, length);           //tokenId数量维护
    ...
    //氪金池子的抽取
    function _getCharge(uint8 gender_, address recommender_, uint8 isJump_) private returns(uint){
        uint tokenId;
        uint8 proIndex = _getIndex();                //获取概率索引
        if(gender_ == 1){
            //女
            ...
            if(length > 0){
                ...
                _countMaintain(true, gender_,  proIndex, tokenIndex, length);           //tokenId数量维护
                ...
                if(isJump_ == 0 ){
                    //是，跳转到免费池子
                    tokenId = _getFree(gender_, recommender_, 1);
                }
            }
        }else if (gender_ == 2){
            //男
    ...
    **/
    function _divide(address recommender_, address caster_, uint amount_) private {
        ...
        //直接给固定奖池转账 ==> soul奖池
        coreInfo.USDT.safeTransferFrom(address(this), coreInfo.Jackpot, share * 1 ether);


        //通过铸造者地址区分铸造或者开启
        if(caster_ == address(0)){
            //铸造者地址为空，为铸造操作
            coreInfo.USDT.safeTransferFrom(address(this), coreInfo.Platform, share * 0.8 ether);    //平台 0.8
            coreInfo.USDT.safeTransferFrom(address(this), coreInfo.ShareHolder, share * 1.2 ether);    //股东会  1.2
        }else{
            //开启，有铸造者地址
            wallet[caster_] += 1;   //铸造者获得1u
            coreInfo.USDT.safeTransferFrom(address(this), coreInfo.Platform, share * 0.2 ether);    //平台 0.8
            coreInfo.USDT.safeTransferFrom(address(this), coreInfo.ShareHolder, share * 0.8 ether);    //股东会  1.2
        }
        ...
        if(recommender_ == address(0)){
            //没有推荐人，设定默认推荐人地址
            recommender_ = coreInfo.ReceiveAddr;
        }

        //设定初始分成金额
        uint receiveCount = share;

        //查询用户是否绑定骑士
        if(bindKnights[_msgSender()] != address(0)){
            //已绑定骑士，骑士的钱包获得0.1
            wallet[bindKnights[_msgSender()]] += (share / 10);
            //初始分成金额减少
            receiveCount -= (share / 10);
        }
        ...
        if(referrers[_msgSender()] != address(0)){
            //之前就存在推荐人，推荐人获得0.4
            wallet[referrers[_msgSender()]] += (share * 4 / 10);
            //初始分成金额减少
            receiveCount -= (share * 4 / 10);
            ...
            if(recommender_ != address(0)){
                //存在推荐人，推荐人获得0.4
                wallet[recommender_] += (share * 4 / 10);
                //初始分成金额减少
                receiveCount -= (share * 4 / 10);
        ...
        //二级推荐判断
        if(recommender_sec != address(0)){
            //存在二级推荐人，推荐人获得0.3
            wallet[recommender_sec] += (share * 3 / 10);

            //初始分成金额减少
            receiveCount -= (share * 3 / 10);

            //判断是否有三级推荐人
            if(referrers[recommender_sec] != address(0)){
                //存在三级推荐人
        ...
        //三级推荐人判断
        if(recommender_ter != address(0)){
            //存在三级推荐人，三级推荐人获得0.2
            wallet[recommender_ter] += (share * 2 / 10);

            //初始分成金额减少
            receiveCount -= (share * 2 / 10);
        }
        ...
        if(receiveCount > 0){
            //将剩余的金额往默认推荐人地址转
            coreInfo.USDT.safeTransferFrom(address(this), coreInfo.ReceiveAddr, receiveCount * 1 ether);
        }
    }
    ...
    function withdrawal() public {
        require(wallet[_msgSender()] > 0, "Sorry, your credit is running low");
        coreInfo.USDT.safeTransferFrom(address(this), _msgSender(), wallet[_msgSender()] * 1 ether);
        wallet[_msgSender()] = 0;
    }

    // ------------------------ onlyOwner start--------------------
    function setKnights(address addr_, bool bool_) public onlyOwner{
        knights[addr_] = bool_;
    }
    ...
    function setbindKnights(address addr_, address knight_) public onlyOwner{
        bindKnights[addr_] = knight_;
    }
    ...
    function setSoul1155(address com_) public onlyOwner {
        coreInfo.SOUL_1155 = ISoul1155(com_);
    }

    function setSoul721(address com_) public onlyOwner {
        coreInfo.SOUL_721 = ISoul721(com_);
    }

    function setSoulToken(address com_) public onlyOwner {
        coreInfo.SOUL_TOKEN = ISoulToken(com_);
    }

    function setPlatform(address com_) public onlyOwner {
        coreInfo.Platform = com_;
    }

    function setJackpot(address com_) public onlyOwner {
        coreInfo.Jackpot = com_;
    }

    function setShareholder(address com_) public onlyOwner{
        coreInfo.ShareHolder = com_;
    }

    function setReceive(address com_) public onlyOwner {
        coreInfo.ReceiveAddr = com_;
    }
