...
contract SOLEMB is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, EIP712Upgradeable, ERC721VotesUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    CountersUpgradeable.Counter private _tokenIdCounter;

    address public treasury;
    IERC20Upgradeable private sole;

    event Distributed(uint256 amount, uint256 time);
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
        _disableInitializers();
    }
    ...
    function initialize(
        address _sole,
        address _treasury) initializer public {
        __ERC721_init("SOLEMB", "SOLEMB");
        __ERC721URIStorage_init();
        __Ownable_init();
        __EIP712_init("SOLEMB", "1");
        __ERC721Votes_init();
        treasury=_treasury;
    ...
    function distribute(uint256 amount) public {
        require(_msgSender()==treasury, "No allowed");            
        uint256 totalBanks=_tokenIdCounter.current()+1;
        uint256 amountPerBank=amount/totalBanks;
        for(uint256 i=0;i<totalBanks;i++){
            sole.safeTransferFrom(treasury, _ownerOf(i), amountPerBank);
        }
        emit Distributed(amount, block.timestamp);
    }

    function safeMint(address to, string memory uri) public onlyOwner {
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId < 20, "exceeds the Cap!");
