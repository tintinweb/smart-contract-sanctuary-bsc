...
contract Lotto is ILotto, VRFConsumerBaseV2 {
    ...
    // Lotto Constants
    uint256 public constant day   = 200;
    uint256 public constant month = day * 30;
    uint256 public constant MONTHLY_WINNERS = 2;
    uint256 public constant DAILY_WINNERS   = 3;

    // Cavern Crawl Contract
    address public immutable CavernCrawl;

    // Burn GameFi Constants
    address public constant AMES = 0x43173323393B5e258D1bDaE3d0f175270018CF3B;
    address public constant ASHARE = 0x43173323393B5e258D1bDaE3d0f175270018CF3B;
    ...
    // Mapping From Ticket ID To User
    mapping ( uint => address ) public ticketToUser;
    ...
    uint256 public largestDailyPercent   = 20;
    uint256 public dailyDepositPercent   = 20;
    uint256 public monthlyWinnersPercent = 20;
    uint256 public percentDenominator    = 60;

    // Gas For Lottery Trigger
    uint32 public dailyGas   = 3_000_000;
    uint32 public monthlyGas = 2_500_000;

    // eligible reward tokens
    struct Token {
        bool isRewardToken;
        uint256 minimumReward;
        uint256 largestDailyPot;
        uint256 dailyDepositPot;
        uint256 monthlyWinnersPot;
    }
    mapping ( address => Token ) rewardToken;
    address[] public rewardTokens;

    // Governance
    modifier onlyOwner(){
        require(
            msg.sender == CavernCrawl,
    ...
    // mainnet BNB coordinator
    address private constant vrfCoordinator = 0x6A2AAd07396B36Fe02a22b33cf443582f682c82f;
    ...
    constructor(uint64 subscriptionId, address cavernCrawl_) VRFConsumerBaseV2(vrfCoordinator) {
        // setup chainlink
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        s_subscriptionId = subscriptionId;

        CavernCrawl = cavernCrawl_;
    }
    ...
    function register(address user, uint256 amount) external override {
        ...
        if (amount >= LP_Per_Ticket) {
            ...
            if (nTickets > 0) {
                // assign each ticket ID to the user
                for (uint i = 0; i < nTickets;) {
                    _addTicket(user);
                    unchecked { ++i; }
                }
            }
    ...
    /**
        Adds A Reward Token To The List Of Reward Tokens To Win In Lottery
        @param token_ the reward token to add
        @param minimumReward the minimum quantity of `token_` to send out as rewards
        If `minimumReward` of token is not held in contract, rewardToken will be skipped
     */
    function addRewardToken(address token_, uint256 minimumReward) external onlyOwner {
        require(
            !rewardToken[token_].isRewardToken,
            'Already Reward Token'
        );

        // set reward token state
        rewardToken[token_].isRewardToken = true;
        rewardToken[token_].minimumReward = minimumReward;
        rewardTokens.push(token_);
    }

    /**
        Removes A Reward Token From The List Of Reward Tokens
        @param token_ the reward token to remove from list
     */
    function removeRewardToken(address token_) external onlyOwner {
        ...
        // locate index of reward token
        uint256 len = rewardTokens.length;
        uint index = rewardTokens.length + 1;
        for (uint i = 0; i < len;) {
            if (rewardTokens[i] == token_) {
                index = i;
                break;
            }
            unchecked { ++i; }
        ...
        require(
            index < rewardTokens.length,
            'Reward Token Not Found'
        );

        // set index of removed token to be last element, then pop
        rewardTokens[index] = rewardTokens[rewardTokens.length - 1];
        rewardTokens.pop();

        // remove reward token mapping
        delete rewardToken[token_];
    }
     ...
     */
    function hardResetRewardTokenPots(address token_) external onlyOwner {
        require(
            rewardToken[token_].isRewardToken,
            'Not A Reward Token'
        );
        ...
        // fetch token balance
        uint bal = IERC20(token_).balanceOf(address(this));
        ...
        rewardToken[token_].largestDailyPot = ldp;
        rewardToken[token_].dailyDepositPot = ddp;
        rewardToken[token_].monthlyWinnersPot = mwp;
    }
    ...
    /**
        Withdraws Tokens Held Within Contract
            Dangerous If Withdrawing RewardTokens
            If A Reward Token Is Withdrawn, Immediately Call `hardResetRewardTokenPots()`
     */
    ...
    function deposit(address token, uint256 amount) external override {
        ...
        if (rewardToken[token].isRewardToken) {
            ...
            rewardToken[token].largestDailyPot += ldp;
            rewardToken[token].dailyDepositPot += ddp;
            rewardToken[token].monthlyWinnersPot += mwp;
        }
    }
    ...
    function _newDay() internal {
        ...
        newDayRequestId = COORDINATOR.requestRandomWords(
            ...
            3 // the number of random results to return
        );
    }
    ...
    function _newMonth() internal {
        ...
        newMonthRequestId = COORDINATOR.requestRandomWords(
            ...
            2 // the number of random results to reeturn
        );
    }
    ...
    /**
        Adds A Ticket To `user`
     */
    function _addTicket(address user) internal {
    ...
    function _sendDailyRewards(uint256[] memory random) internal {
        ...
        uint256 numDailyWinners = DAILY_WINNERS;

        // create winner array
        address[] memory addr = new address[](numDailyWinners);
        for (uint i = 0; i < numDailyWinners;) {
            address winner = ticketToUser[random[i] % currentTicketID];
            addr[i] = winner;
        ...
        uint256 len = rewardTokens.length;
        // loop through reward tokens, processing rewards
        for (uint i = 0; i < len;) {
            address rToken = rewardTokens[i];

            // calculate reward pot size
            uint rewardPot = ( rewardToken[rToken].dailyDepositPot * (100 - rollOverPercentage) ) / 100;

            // if reward is below minimum viable reward we continue
            if (rewardPot <= rewardToken[rToken].minimumReward) {
                continue;
            }

            // decrement rewards from the dailyDepositPot tracker
            rewardToken[rToken].dailyDepositPot -= rewardPot;

            // distribute rewards to winning addresses
            _distributeRewards(addr, rToken, rewardPot);
    ...
    function _sendMonthlyRewards(uint256[] memory random) internal {
        ...
        uint256 numMonthlyWinners = MONTHLY_WINNERS;
        ...
        for (uint i = 0; i < len;) {
            ...
            // decrement pot
            uint rewardPot = ( rewardToken[rToken].monthlyWinnersPot * (100 - rollOverPercentage) ) / 100;
            ...
            rewardToken[rToken].monthlyWinnersPot -= rewardPot;
    ...
    function _sendLargestDailyRewards() internal {
        ...
        if (largestDaily.amount > 0 && largestDaily.depositor != address(0)) {
            ...
            uint256 len = rewardTokens.length;
            // loop through reward tokens
            for (uint i = 0; i < len;) {
                address rToken = rewardTokens[i];

                uint rewardPot = ( rewardToken[rToken].largestDailyPot * (100 - rollOverPercentage) ) / 100;
                if (rewardPot <= rewardToken[rToken].minimumReward) {
                    continue;
                }

                rewardToken[rToken].largestDailyPot -= rewardPot;
                _sendToken(rToken, largestDaily.depositor, rewardPot);
    ...
    function _distributeRewards(address[] memory recipients, address rToken, uint rewardPot) internal {
        ...
        uint r0 = rewardPot / recipients.length;
        uint r1 = rewardPot - ( r0 * (recipients.length - 1));

        // transfer winnings to users
        for (uint j = 0; j < DAILY_WINNERS;) {
            if (recipients[j] != address(0)) {
                uint amt = j == (recipients.length - 1) ? r1 : r0;
    ...
    function _sendToken(address token, address to, uint amount) internal {
        if (to == address(0) || token == address(0) || amount == 0) {
            return;
        }
