    ...
    UUPSUpgradeable
{
    ...
    mapping(address => mapping(uint256 => uint256)) private _totalNFTsUserPurchasedPreSale; 
    uint256 public constant MAXIMUM_PRESALE_LIMIT = 60;
    mapping(address => mapping(string => uint256)) private _totalNFTsUserPurchasedThisPreSale;
    ...
    function batchTransfer(address from, address to, uint256[] calldata tokenIds, string calldata preSaleId) public {
        require(_totalNFTsUserPurchasedThisPreSale[to][preSaleId] + tokenIds.length < MAXIMUM_PRESALE_LIMIT, "NFTs purchased excceed limit!");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            safeTransferFrom(from, to, tokenIds[i]);
        }
        _totalNFTsUserPurchasedThisPreSale[to][preSaleId] += tokenIds.length;
