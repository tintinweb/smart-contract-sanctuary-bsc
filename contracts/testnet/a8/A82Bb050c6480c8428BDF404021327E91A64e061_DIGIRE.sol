...
contract DIGIRE is BEP20Upgradeable {
    ...
    uint loanRequestCount;
    LoanRequest[] loanRequests;

    enum OpexLoanRequestStatus { Init, Approved, Rejected }

    struct OpexLoanRequest {
        uint amount;
        OpexLoanRequestStatus status;
    }

    uint opexLoanRequestCount;
    OpexLoanRequest[] opexLoanRequests;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        require(sender == admin.gate_keeper() && isWhitelisted(recipient), "Invalid transfer");
...
contract BEP20Upgradeable is OwnableUpgradeable {
   ...
   function _setup(string memory name_, string memory symbol_, uint8 decimals_) internal {
        name = name_;
        symbol = symbol_;
        decimals = decimals_;
    }
