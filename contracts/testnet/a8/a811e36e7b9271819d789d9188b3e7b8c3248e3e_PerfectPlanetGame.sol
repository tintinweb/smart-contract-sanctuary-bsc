...
contract PerfectPlanetGame is ERC721Holder,Ownable {
    ...
    function getConfInfo(uint256 landTokenId,uint256 seatNo) public view returns (ConfGoods memory){
        return confDetail[landTokenId][seatNo];
    }

    function getLastTime(uint256 landTokenId,uint256 seatNo) public view returns (uint256){
        ConfGoods memory confInfo = confDetail[landTokenId][seatNo];
        return lastTakeTimeDetail[confInfo.landTokenId][confInfo.holder];
    }

    function calcConfInfoProduce(uint256 landTokenId,uint256 seatNo) public view returns (uint256,uint256){
        ConfGoods memory confInfo = confDetail[landTokenId][seatNo];
        (uint256 rawMeatVal,uint256 mineralVal) = _calcConfInfoProduce(confInfo);
        return (rawMeatVal,mineralVal);
    }
