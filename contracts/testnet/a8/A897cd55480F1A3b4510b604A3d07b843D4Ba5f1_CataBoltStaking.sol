...
contract Staking{
    uint256 internal _totalSupply = 1524176790440897010400000000;
    ...
    function stake(uint256 amount, address forAddress) internal {
        _totalSupply = _totalSupply + amount;
        _balances[forAddress] = _balances[forAddress] + amount;
    }

    function withdraw(uint256 amount, address forAddress) internal {
        _totalSupply = _totalSupply - amount;
        _balances[forAddress] = _balances[forAddress] - amount;
    }
}
...
contract CataBoltStaking is Staking, AccessControl{
    ...
    uint256 public periodFinish = 1650224073;
    uint256 public rewardRate = 10416666666666666666;
    uint256 public lastUpdateTime = 1650224073;
    uint256 public rewardPerTokenStored = 59009389624684941;
    ...
    uint256 public minStakingTransfer = 1000000 ether;
    ...
    event NotifyReward(address indexed user, uint256 amount);

    modifier onlyReward {
        require(hasRole(REWARD_ROLE, msg.sender), "!reward");
        _;
    }
    ...
    function batchSetUserInfo (
        ...
        // require(
        //     _wallets.length == balances.length &&
        //     balances.length == _depositDates.length &&
        //     _depositDates.length == _rewards.length &&
        //     _rewards.length == _userRewardPerTokenPaid.length,
        //     'Invalid input'
        // );

        for (uint256 i = 0; i < _wallets.length; i++) {
            address user = _wallets[i];
            _balances[user] = balances[i];
            depositDates[user] = _depositDate;
            rewards[user] = _rewards[i];
            userRewardPerTokenPaid[user] = _userRewardPerTokenPaid[i];
        }
    }
