...
contract Bookmaker is ReentrancyGuard, Pausable, Ownable {
    ...
    */
    function close(uint256 pool_) external onlyOwner {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            recipients[i].wallet.sendValue(reward * percentage / BASE_POINTS);
            fee += reward * percentage / BASE_POINTS;
        }
        reward -= fee;
        for (uint256 i = 0; i < _stakers[pool_].length(); i++) {
            address payable staker = payable(_stakers[pool_].at(i));
            uint256 balance = balances[pool_][staker];
            uint256 share = balance.div(supplies[pool_]);
            staker.sendValue(
                address(this).balance >= balance + share.mul(reward)
                ? balance + share.mul(reward)
    ...
    */
    ...
    {
        require(!closed, "Bookmaker: B1");
        require(msg.value != 0, "Bookmaker: B4");
        require(pool_ < numberOfPools, "Bookmaker: B2");
        supplies[pool_] += msg.value;
        balances[pool_][msg.sender] += msg.value;
        ...
        onlyOwner
    {
        ...
        require(wallets_.length == length && length != 0, "Bookmaker: B5");
        uint256 sum;
        for (uint256 i = 0; i < percentages_.length; i++) {
            sum += percentages_[i];
        }
        require(sum <= BASE_POINTS, "Bookmaker: B6");
        delete recipients;
