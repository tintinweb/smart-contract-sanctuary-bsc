...
contract PresaleDatabase is IPresaleDatabase {
    ...
    // Early Access NFT
    IEarlyAccessNFT public earlyAccess;
    ...
    // Early Access Time Boost In Seconds
    uint256 public override earlyAccessBoost = 5 minutes;
    ...
    function createSale(
        ...
        uint256 maxContribution,
        uint256 presaleFee
    ) external returns (address payable sale) {
        require(
            saleOwner.isContract() == false,
            'Owner Cannot Be Contract'
        );
    ...
    function setEarlyAccessBoost(uint newBoost) external onlyOwner {
        earlyAccessBoost = newBoost;
    }

    function setEarlyAccessNFT(address newNFT) external onlyOwner {
        earlyAccess = IEarlyAccessNFT(newNFT);
    }
    ...
    function removePending(address sale) external onlyOwner {
        _removePending(sale);
    }

    function removeLive(address sale) external onlyOwner {
        _removeLive(sale);
    }
    ...
    function userHasAccessPass(address user) public view override returns (bool) {
        if (address(earlyAccess) == address(0)) {
            return true;
        }
        return earlyAccess.balanceOf(user) > 0;
    }

    /**
        Checks the time compared to the early start time boost
        If boost has ended, returns true, else returns true only if user owns an early access NFT
     */
    function earlyAccessCheck(uint256 presaleStartTime, address user) external view override returns (bool) {
        if (address(earlyAccess) == address(0)) {
            return true;
        }
        if (presaleStartTime + earlyAccessBoost >= block.timestamp) {
            return userHasAccessPass(user);
        } else return true;
    }
