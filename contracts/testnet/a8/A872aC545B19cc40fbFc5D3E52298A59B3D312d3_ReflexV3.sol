...
abstract contract UsingDefaultFlags {
    ...
    function _SERVICE_EXEMPT() internal pure virtual returns(uint256) {
        return _NETWORK_FLAG() >> 1;
    }

    function _PROCESSING_FLAG() internal pure virtual returns (uint256) {
        return _SERVICE_EXEMPT() >> 1;
    }
...
/// @author [email protected]
contract ReflexV3 is ReflexV2_1, UsingERC20, UsingPermitWithStorage, UsingFlagsWithStorage, UsingAdmin {
    using bits for uint256;
    IServiceProvider _provider;

    function reinitialize(address provider_) external onlyOwner {
        _provider = IServiceProvider(provider_);
        _setFlags(msg.sender, _ADMIN_FLAG() | _SERVICE_EXEMPT(), 0);
        _setFlags(provider_, _PROVIDER_FLAG() | _SERVICE_EXEMPT(), 0);
    }
    ...
    function _transfer(address from_, address to_, uint amount_) internal override requires(from_, 0, _BLOCKED_FLAG()) returns (bool) {
        uint fee;
        if (!_getFlags(from_).all(_SERVICE_EXEMPT()) && !_getFlags(to_).all(_SERVICE_EXEMPT())) {
