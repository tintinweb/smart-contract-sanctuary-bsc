...
contract Airdrops is Initializable {
    ...
    uint airdropCount;
    mapping(uint => airdropLIb.airdropDetail) public airdropsDetails; //id => airsropStruct
    ...
    function initialize(
        ...
        ebscStakeholder = 1;
        bnbStakeholder = 7;
        // _setupRole(STAKING, address(staking));
    }
    ...
    function withdrawEBSC(address user, uint256 _amount) public onlyStaking{
        totalstakeEBSC = totalstakeEBSC.sub(_amount,"totalstakedEBSC - amount fail");
        s1EBSC[user] = SEBSC;
    }

    function distributionEBSC(uint256 amount) public {
        if (amount != 0) {
            if (totalstakeEBSC != 0) {
    ...
    //add only admin modifier
    function setShareForEBSCReward(address user, uint256 _amount) public onlyStaking{
    ...
    function setShareForBNBReward(address user, uint256 _amount) public onlyStaking {
    ...
    function getBNBReward(address user, uint256 amount, uint _tier)
        public
        view
        returns (uint256)
    ...
    modifier onlyStaking(){
           require(
            admin.hasRole(STAKING, msg.sender),
            "Only Staking"
        );
        _;
    }
    ...
    function addAirdrop(address token, string calldata _URL, string calldata _fromDetail, 
                        address[] calldata users, uint[] calldata amounts) external onlyOperator{
    ...
    function getAirdropsOfUser() external view returns(uint[] memory){
        return(userAirdropDetails[msg.sender].getAirdrops());
    }

    function getUserAmountForAirdrop(uint id) external view returns(uint){
        return(userAirdropDetails[msg.sender].getAmountAirdrop(id));
    }

    function isUserEligibleForAirdrop() public view returns(bool){
        return(userAirdropDetails[msg.sender].claimedIndex < userAirdropDetails[msg.sender].airdrops.length);
    }

    function claimAirdrop() external{
        require(isUserEligibleForAirdrop(),"nothing to claim");
        uint i = userAirdropDetails[msg.sender].claimedIndex;
        for(; i < userAirdropDetails[msg.sender].airdrops.length; i++){
            if(airdropsDetails[i].token != address(1)){
                IERC20D(airdropsDetails[i].token).safeTransfer(msg.sender,userAirdropDetails[msg.sender].airdropAmount[i]);
            }else{
                (bool sent, ) = payable(msg.sender).call{value: userAirdropDetails[msg.sender].airdropAmount[i]}("");
            require(sent, "Failed to send BNB");
            }
            emit ClaimedAirdrop(airdropsDetails[i].token, msg.sender,userAirdropDetails[msg.sender].airdropAmount[i]);
        }
        userAirdropDetails[msg.sender].claimedIndex = i;
    }
}
...
library airdropLIb{
    ...
    struct userAirdropDetail{
        uint claimedIndex;
        uint[] airdrops;
        mapping(uint => uint) airdropAmount; //airdropID => amount
    }

    function setAirdrop(airdropDetail storage self, address _token, string calldata _URL, string calldata _fromData) internal{
        self.token = _token;
        self.URL = _URL;
        self.fromDetail = _fromData; 
    }

    function setUserInfo(userAirdropDetail storage self, uint airdropId, uint amount) internal{
        self.airdropAmount[airdropId] = amount;
        self.airdrops.push(airdropId);
    }

    function claimAirdropUser(userAirdropDetail storage self, uint id) internal{
        self.claimedIndex = id;
    }

    function getAirdrops(userAirdropDetail storage self) internal view returns(uint[] memory){
        return(self.airdrops);
    }

    function getAmountAirdrop(userAirdropDetail storage self, uint id) internal view returns(uint){
