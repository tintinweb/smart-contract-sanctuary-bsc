...
contract SantaClubStaking is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    uint256 public DEFAULT_DAILY_REWARD = 10;
    ...
    function getIsStakingAvailable() external view returns (bool) {
        return isStakingAvailable;
    }

    function enableIsStakingAvailable() external {
        bool stakingAvailable = rewardToken.balanceOf(address(this)) > 0;
        require(
            stakingAvailable,
            "Enable Staking: There are no more rewards in the staking vault."
        );
        if (stakingAvailable) {
            isStakingAvailable = true;
            emit EnableIsStakingAvailable();
        }
    }
    ...
    function _claimRewards(
        ...
        require(
            isStakingAvailable,
            "Claim Rewards: There are no more rewards in the staking vault."
        );
