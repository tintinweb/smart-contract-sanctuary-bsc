...
contract DEX is ReentrancyGuardUpgradeable{
    ...
    uint256 public constant MAX_INT = 2**256 - 1;
        ...
        uint256[] memory swapedAmounts=  IPancakeV2Router(ROUTER).swapExactTokensForTokens(
            _amount,
            1,
            path,
            msg.sender,
        ...
        uint256 _pid
    ) external nonReentrant returns (uint256 lpTokenReceived) {
...
IERC20Upgradeable(_lpToken).safeTransfer(msg.sender,lpTokenReceived);
  ...
  function _zapOut(
        ...
        uint256[] memory tokenAmountOutMin =    IPancakeV2Router(ROUTER).getAmountsOut(swapAmountIn, path);
        // Swap tokens
        uint256 balBef =  IERC20Upgradeable(_tokenToReceive).balanceOf(address(this));

        IPancakeV2Router(ROUTER).swapExactTokensForTokens(swapAmountIn, tokenAmountOutMin[1], path, address(this), block.timestamp);
        uint256 balAft = IERC20Upgradeable(_tokenToReceive).balanceOf(address(this));
       ...
       IERC20Upgradeable(_tokenToReceive).safeTransfer(msg.sender, amountReceived);
    ...
    function _calculateAmountToSwap(
       ...
       function sqrt(uint256 y) internal pure returns (uint256 z) {
        ...
        private
    {
        if (IERC20Upgradeable(_token).allowance(address(this), ROUTER) < _swapAmountIn) {
            // Reset to 0
