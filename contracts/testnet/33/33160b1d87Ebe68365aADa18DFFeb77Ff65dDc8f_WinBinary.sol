...
contract WinBinary is ERC20, Ownable, BaseToken {
    ...
    function swapTokensForBUSD(uint256 tokenAmount) public {
        /// Since we created pair with BUSD and pancake not allow to swap
        /// token to BUSD and send directly to our contract, so we will swap token for WBNB first
        /// and then swap WBNB for BUSD
        address[] memory path = new address[](3);
        path[0] = address(this);
        path[1] = BUSD;
