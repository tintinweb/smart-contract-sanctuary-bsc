...
contract Referrals is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => Structs.ReferralFatherDataInternal)
        public childReferralPerFather;
    mapping(address => address) public fatherReferralByChild;
    mapping(address => uint32) public refferalLevelForFather;
    mapping(address => uint256) public referralFatherProfit;
    mapping(uint32 => uint32) public referralLevelsAndPercents;
    mapping(address => uint32) public referralPercentsForAdmins;
    ...
    function initialize(
        ...
        prepare_and_update_referral_levels_and_percents(
    ...
    function storage_referral_deposit() public payable nonReentrant {
        require(
        ...
        require(
            !childReferralPerFather[fatherReferral].isPresent,
            "Father Referral already exists"
        );
        childReferralPerFather[fatherReferral].isPresent = true;
        ...
        refferalLevelForFather[fatherReferral] = referralLevel;
        // emit StorageReferralDeposit(msg.sender, referralLevel);
        emit StorageReferralDeposit(msg.sender, referralLevel);
    }

    function add_new_child_referral_to_father(
        address childReferral,
        address fatherReferral,
        string memory code
        ...
        childReferralPerFather[fatherReferral].children.push(
    ...
    function update_level_referral_father(
        address fatherReferral,
        uint32 newLevel
    ) public payable onlyOwnerOrHelperAccount {
        assert_one_yocto();
        ...
        refferalLevelForFather[fatherReferral] = newLevel;
        emit UpdateLevelReferralFather(fatherReferral, newLevel);
    }

    function update_percent_referral_admin(
        address fatherReferral,
        uint32 newPercent
    ) public payable onlyOwnerOrHelperAccount {
        assert_one_yocto();
        ...
        referralPercentsForAdmins[fatherReferral] = newPercent;
        emit UpdatePercentReferralAdmin(fatherReferral, newPercent);
    }

    function add_calculated_father_fee(address _childReferral)
        public
        payable
        onlyOwnerOrProxyRouterAccount
    {
        ...
        referralFatherProfit[fatherReferral] += msg.value;
        emit NewReferralPayout(fatherReferral, _childReferral, msg.value);
    }

    function withdraw_referral_father(uint256 _amount) public payable nonReentrant {
        uint256 _newAmount = referralFatherProfit[msg.sender];
        (bool _bool, ) = SafeMathUpgradeable.trySub(_newAmount, _amount);
        require(_bool, "Amount to withdraw is bigger than account balance");
        referralFatherProfit[msg.sender] -= _amount;
    ...
    function set_new_root_caller(address _newRootCaller)
        public
        payable
        onlyOwner
    {
    ...
    function set_new_helper_account(address _newHelperAccount)
        public
        payable
        onlyOwner
    {
    ...
    function calculate_referral_father_fee(
        uint256 _amount,
        ...
        uint32 fatherReferralLevel = refferalLevelForFather[fatherReferral];
        uint32 levelPercent = referralLevelsAndPercents[fatherReferralLevel];
        uint32 levelPercent2 = referralPercentsForAdmins[fatherReferral];
    ...
    function prepare_and_update_referral_levels_and_percents(
        Structs.ReferralLevelsAndPercentsStruct[]
    ...
    function assert_one_yocto() internal {
