...
contract Project is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function setSaleAddr(address _sale) external {
    ...
    function checkManager(uint256 _projectId, address _account) public {
        if (projects[_projectId].isCreatedByAdmin) {
            require(_account == owner() || admins[_account] || _account == projects[_projectId].manager, "Caller is not the manager");
        } else {
