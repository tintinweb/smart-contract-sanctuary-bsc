...
contract AlphaStrategy is Pausable {
   ...
   function updateAdminData() internal {
      ...
      _totalSupply = alphaToken.totalSupply();
      _treasury = admin.treasury();
       netDepositInd =  admin.netDepositInd(); 
      netAmountEvent = admin.netAmountEvent();
    }

    function calculateMaxDepositAmount( ) external 
        whenNotPaused onlyManager {
    if (netDepositInd == 1) {
         maxDepositAmount = (netAmountEvent + ((withdrawAmountTotal * _ALPHA_PRICE) 
    ...
    function finalizeDeposits( address[] memory _users) external 
      ...
      if (_feeStableTotal >0){
       stableToken.transfer(_treasury, _feeStableTotal/AMOUNT_SCALE_DECIMALS);
      }
    }
...
function calculateMaxWithdrawAmount( )external 
        whenNotPaused onlyManager
          ...
          * _COEFF_SCALE_DECIMALS_P) /(_ALPHA_PRICE * withdrawAmountTotal);
    }
...
function calculateAcceptedWithdrawRequests(address[] memory _users) 
    ...
    function depositRequest(uint256 _amount) external whenNotPaused {
        ...
        stableToken.transferFrom(msg.sender, address(this), _amount/AMOUNT_SCALE_DECIMALS);
    }

    function cancelDepositRequest(uint256 _amount) external whenNotPaused canCancel {
        uint256 _index;
        (  ,  , _index )= depositNFT.pendingDepositPerAddress(msg.sender);
        uint256 _tokenIdDeposit = depositNFT.getTokenId(msg.sender);
        depositNFT.updateDepositData(msg.sender,  _tokenIdDeposit, _amount, false);
        depositAmountTotal = depositAmountTotal - _amount; 
        stableToken.transfer(msg.sender, _amount/AMOUNT_SCALE_DECIMALS);
    ...
    function cancelWithdrawalRequest( uint256 _amount) external whenNotPaused {
        ...
        require(block.timestamp - TIME_WITHDRAW_MANAGER>= admin.LOCKUP_PERIOD_MANAGER(), 
         "Formation Fi: Manager Position locked");
        require(
            stableToken.balanceOf(address(this)) >= (_amount/AMOUNT_SCALE_DECIMALS),
            "Formation Fi: requested amount exceeds contract balance"
        );
        TIME_WITHDRAW_MANAGER = block.timestamp;
        stableToken.transfer(admin.manager(), _amount/AMOUNT_SCALE_DECIMALS);
    }
...
contract AdminInterface is Pausable {
  ...
  // fees
    ...
    function protectAgainstSlippage(uint256 _withdrawAmount) public onlyManager 
      ...
      require(netDepositInd == 0, "it is not a slippage case");
      require(_withdrawAmount != 0, "amount is zero");
       uint256 _amount = 0; 
       uint256 _deltaAmount =0;
       uint256 _slippage = 0;
       uint256  _alphaAmount = 0;
       uint256 _balanceAlphaTreasury = alphaToken.balanceOf(treasury) * AMOUNT_SCALE_DECIMALS;
      ...
      if (_withdrawAmount< netAmountEvent){
         ...
         if ((_slippage * COEFF_SCALE_DECIMALS_F) >= SLIPPAGE_TOLERANCE) {
            return netAmountEvent;
             ...
             if ( _deltaAmount  > 0){
                stableToken.transferFrom(treasury, alphaStrategy, _deltaAmount/AMOUNT_SCALE_DECIMALS);
          ...
          if (_alphaAmount >0) {
            ...
            stableToken.transfer(treasury, _deltaAmount/AMOUNT_SCALE_DECIMALS);   
            alphaToken.burn( treasury, _alphaAmount);
          }
          if ((_amount - _deltaAmount)>0) {
              stableToken.transfer(manager, (_amount - _deltaAmount)/AMOUNT_SCALE_DECIMALS); 
          }
        }
        return 0;
    ...
    function sendStableFromcontract() external 
      ...
      stableToken.transfer(alphaStrategy, alphaToken.balanceOf(address(this)));
      }
 ...
 function deleteDepositData(address _account) internal {
       require(
        ...
        for (uint256 i = _ind; i< size; i++){
         ...
         pendingDepositPerAddress[usersOnPendingDeposit[i]].listPointer = i;
         }
         nn = nn+1;
       ...
       // else {
           // delete usersOnPendingDeposit;
         // usersOnPendingDeposit = new address[](0);
        //}
         //delete usersOnPendingDeposit[size];
...
function deleteWithdrawData(address _account) internal {
        ...
        for (uint256 i = _ind; i< size; i++){
            ...
            pendingWithdrawPerAddress[usersOnPendingWithdraw[i]].listPointer = i;
        }
        if ( size != 0){
