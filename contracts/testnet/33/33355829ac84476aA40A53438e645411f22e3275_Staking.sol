...
contract Staking is Ownable {
    ...
    uint32 public maturationPeriod;
    ...
    */
    ...
    function depositStake(uint256 amount) public {
        ...
        uint256 timeSinceDeployment = block.timestamp - deploymentDate;
        uint256 adjustedRewardRate = rewardRate - Math.mulDiv(rewardRate, timeSinceDeployment, maturationPeriod, Math.Rounding.Up);
        uint256 reward = Math.mulDiv(amount, adjustedRewardRate, PRECISION * 100, Math.Rounding.Up);
        Stake memory stake = Stake(amount, reward, address(msg.sender), block.timestamp, 0, 0, 0);
    ...
    function previewInvestorClaimReward(address investor, uint256 index) public view returns (uint256) {
        ...
        if (maturationDate > block.timestamp) {      
            ...
            rewardDueNow = stake.reward - Math.mulDiv(stake.reward, timeRemaining, maturationPeriod, Math.Rounding.Up);
        } 
    ...
    function previewInvestorWithdrawStake(address investor, uint256 index) public view returns (uint256, uint256) {
        ...
        if (maturationDate > block.timestamp) {
            uint256 timeRemaining = maturationDate - block.timestamp;
            uint256 currentBurnRate = Math.mulDiv(startingBurnRate, timeRemaining, maturationPeriod);
