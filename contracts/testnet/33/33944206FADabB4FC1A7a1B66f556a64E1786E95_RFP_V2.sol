...
contract RFP_V2 is IBEP20, Ownable {
    ...
    uint256 public maxTxAmount = (_totalSupply * 100) / 10000; // 0.1% of supply
    ...
    function totalMintSupply() public view override returns(uint256){
    ...
    function updateBuyFees(uint256 reward, uint256 marketing, uint256 liquidity) public onlyOwner {
        buyRewardFee = reward;
    ...
    function updateSwapPercentages(uint256 reward,uint256 marketing,uint256 liquidity) public onlyOwner {
        rewardSwap = reward;
    ...
    function swapBackInBnb() internal swapping {
        ...
        uint256 swapFeeForOtherTokens = rewardSwap; //.add(swapForStake).add(burnSwap);
        uint256 tokensToSwapOtherTokens = contractTokenBalance.mul(swapFeeForOtherTokens).div(totalSwap);

        // calculate tokens amount to swap
        uint256 tokensToMarketing = contractTokenBalance.sub(tokensToLiquidity) .sub(tokensToSwapOtherTokens);
        ...
        if (tokensToSwapOtherTokens > 0) {
            ...
            uint256 tokensToReward = swappedTokensAmount.mul(rewardSwap).div(swapFeeForOtherTokens);
                    
            if (tokensToReward > 0 && rewardSwap > 0) {
                // send token to reward
                IBEP20(REWARD).transfer(address(dividendTracker),tokensToReward);
