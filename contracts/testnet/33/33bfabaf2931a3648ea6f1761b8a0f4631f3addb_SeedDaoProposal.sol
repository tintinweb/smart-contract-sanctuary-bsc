...
contract SeedDaoProposal is Modifier, Util {

    using SafeMath for uint256;

    uint256 [] proposalIds;
    mapping(uint256 => uint256) private proposalVote;
    ...
    function vote(uint256 proposalId, uint256 voteCount) public isRunning returns (bool) {
        proposalVote[proposalId] = proposalVote[proposalId].add(voteCount);
        return true;
    }
    ...
    function getProposalVote(uint256 proposalId) public view returns(uint256) {
       return proposalVote[proposalId];
    }
