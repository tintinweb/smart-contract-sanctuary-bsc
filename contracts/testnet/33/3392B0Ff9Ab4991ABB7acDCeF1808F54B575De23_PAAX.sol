...
contract PAAX is  Context, ERC20 {
    ...
    uint256 public liquidFee = 4;
    uint256 public holdersFee = 4;
    uint256 public exchangeFee = 4;
    uint256 public sellingTaxFee = 16;
    ...
    constructor() ERC20("Paax Coin", "PAXX") {
        owner = msg.sender;
        marketing = 0xe5221AEAAaE99385dEeadab9FfF2292e9F922Cf0;
        exchange  = 0x4a6Fc915cfD6475C4FA110819c064Fee49d077Eb;
        ...
        _excluded.push(marketing);
         _isExcluded[exchange] = true;
        _excluded.push(exchange);
    }

    modifier onlyOwner() {
        require(msg.sender == owner, "Platform owner only");
        _;
    }
    ...
    function setSellingTax(uint256 _taxFee) public onlyOwner {
        sellingTaxFee = _taxFee;
    }


    function setMarketing(address wallet) public onlyOwner {
        _isExcluded[marketing] = false;
        marketing = wallet;
        _isExcluded[marketing] = true;
        emit MarketingSet(wallet);
    }
 
    function setExchange(address wallet) public onlyOwner {
        _isExcluded[exchange] = false;
        exchange = wallet;
        _isExcluded[exchange] = true;
    ...
    function airdrop(address[] memory wallet, uint256[] memory amount) public onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount)
        ...
        if(_isExcluded[sender] && recipient == pairAddress){
            super._transfer(sender,recipient, amount);
         }else if(_isExcluded[recipient] && _isExcluded[sender]){
            super._transfer(sender,recipient, amount);
         }else if(recipient == pairAddress && !_isExcluded[sender]){
         marketingFee = 4;
         liquidFee = 4;
         holdersFee = 4;
         exchangeFee = 4;
         uint256 ftFee = 16-(marketingFee+liquidFee+exchangeFee);
           uint256 tAmount = amount - ((amount * ftFee) / 100);
           uint256 marketingAmount = amount - ((amount * marketingFee) / 100);
           super._transfer(sender,marketing, marketingAmount);
           //uint256 liquidAmount = amount - ((amount * liquidFee) / 100);
           uint256 exchangeAmount = amount - ((amount * exchangeFee) / 100);
           super._transfer(sender,exchange, exchangeAmount);
