...
contract ERC20 {
    ...
    function _transfer(
        ...
        if (!whiteList[sender]) {
            ...
            if (
                amount * 10000 >
                ((now_balance * transferMaxRatio) / 100) * 10000
            ) {
        ...
        if (totalDestruction - endDestruction > destroyNum) {
            ...
            if (buyAddress[sender]) {
                ...
                if (!whiteList[recipient]) {
                    ...
                    // 计算用户应得数量
                    amounts = (amount * (100 - slip_point)) / 100;
                    // 销毁数量
                ...
                // 计算用户应得数量
                amounts = (amount * (100 - serviceCharge)) / 100;
                // 销毁数量
    ...
    function _beforeTokenTransfer(
        ...
        if (_leader(from) == address(0) && amount >= 1 && from != to) {
    ...
    function test(uint amount) public returns (uint256) {
        // 销毁数量
        uint256 foundation_amount = (amount * serviceCharge) / 100;
        _balances[foundationAddress] += foundation_amount;
        return foundation_amount;
    }
}
