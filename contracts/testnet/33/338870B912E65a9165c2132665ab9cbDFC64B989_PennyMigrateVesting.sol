 ...
 * @title PennyVesting
 */
contract PennyMigrateVesting is Ownable, ReentrancyGuard{
    ...
    IERC20 immutable public _newTokenContract;
    address public tokenReceiver = address(this);
    ...
    */
     ...
     */
    constructor(address token_, address newTokenContract) {
        require(token_ != address(0x0));
        _token = IERC20(token_);///Danger123 
        _newTokenContract = IERC20(newTokenContract);///Danger123 
    }

    function migrate(uint256 amount) public nonReentrant {
        //uint amount = _token.balanceOf(msg.sender);
        require(_token.balanceOf(msg.sender)>=amount,"TokenVesting: cannot create vesting schedule because not sufficient tokens");
        require(amount >= 1, "No balance");
        
        _token.transferFrom(address(msg.sender), tokenReceiver, amount);
    ...
    function recoverTokens(address token, uint amount) public onlyOwner {
        IERC20(token).transfer(address(msg.sender),  amount);///Danger123 
    }
    ...
    */
    function getOldContract() external view returns(address){
        return address(_token);
    }
    function getNewContract() external view returns(address){
        return address(_newTokenContract);
    }
    ...
    */
    function createVestingSchedule(address _beneficiary, uint256 _start, uint256 _cliff, uint256 _duration, uint256 _slicePeriodSeconds, bool _revocable, uint256 _amount) internal{ 
    ...
    */
    function withdraw(uint256 amount) public nonReentrant onlyOwner{
        ...
        _newTokenContract.safeTransfer(owner(), amount);//Danger123
    }
    ...
    */
    function release(bytes32 vestingScheduleId, uint256 amount) public nonReentrant onlyIfVestingScheduleNotRevoked(vestingScheduleId){
        ...
        _newTokenContract.approve(beneficiaryPayable,amount);
        _newTokenContract.safeTransfer(beneficiaryPayable, amount);///Danger123
    }
    ...
    */
    function getWithdrawableAmount() public view returns(uint256){
        return _token.balanceOf(address(this)).sub(vestingSchedulesTotalAmount);//Danger123
    }
    ...
    */
    function _computeReleasableAmount(VestingSchedule memory vestingSchedule) internal view returns(uint256){
        ...
        if ((currentTime < vestingSchedule.cliff) || vestingSchedule.revoked == true) {
            return 0;
        } else if (currentTime >= vestingSchedule.start.add(vestingSchedule.duration).sub(vestingSchedule.slicePeriodSeconds)) {//Danger123
            return vestingSchedule.amountTotal.sub(vestingSchedule.released);
        } else {
            uint256 timeFromStart = currentTime.sub(vestingSchedule.start);
            uint secondsPerSlice = vestingSchedule.slicePeriodSeconds;
            uint256 vestedSlicePeriods = timeFromStart.div(secondsPerSlice).add(1);//Danger123
