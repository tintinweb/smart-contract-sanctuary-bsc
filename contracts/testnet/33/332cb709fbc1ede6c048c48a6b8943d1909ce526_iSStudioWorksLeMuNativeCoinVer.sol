...
contract iSStudioWorksLeMuNativeCoinVer is BankSettingsFunctions {
    ...
    function internalCompoundingFunction(address adr, uint256 amtComp, bool isBWC, bool isAuto) internal {
        ...
        require(amtComp <= getMyLeXC(adr), "You CANNOT compound more than what you have!");
        require(getMyLeXC(adr) != 0, "You CANNOT compound with nothing in your wallet! Buy first please!");
        
        uint256 compoundingLeXC = amtComp;

        user.Bonds += compoundLogic(adr, compoundingLeXC, isBWC, isAuto);
    ...
    function compoundLogic(address adr, uint256 amt, bool isBWC, bool isAuto) internal returns(uint256) {
    ...
    function startAutoCompounding(uint256 intrvlHrs, uint256 runDays) external nonReentrant {
        ...
        internalCompoundingFunction(msg.sender, getMyLeXC(msg.sender), false, true);
    }
