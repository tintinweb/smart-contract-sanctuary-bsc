...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => mapping(address => Bill)) public bills;

    event Creates(uint256 indexed projectId, SaleInfo[] sales);
    event CloseAuction(uint256 indexed projectId, bool isGive);
    ...
    modifier onlyAdmin() {
        require(_msgSender() == project.getSuperAdmin() || project.isAdmin(_msgSender()), "Invalid admin");
        _;
    }
    ...
    modifier onlyManagerWithSaleId(uint256 saleId) {
        if (project.getProject(sales[saleId].projectId).isCreatedByAdmin) {
            require(_msgSender() == project.getSuperAdmin() || project.isAdmin(_msgSender()) || _msgSender() == project.getManager(sales[saleId].projectId), "Invalid manager");
        } else {
            require(_msgSender() == project.getManager(sales[saleId].projectId), "Invalid manager");
        }
        _;
    }
    ...
    modifier onlyManagerWithProjectId(uint256 projectId) {
        if (project.getProject(projectId).isCreatedByAdmin) {
            require(_msgSender() == project.getSuperAdmin() || project.isAdmin(_msgSender()) || _msgSender() == project.getManager(projectId), "Invalid manager");
        } else {
            require(_msgSender() == project.getManager(projectId), "Invalid manager");
        }
        _;
    }
    ...
    modifier onSale(uint256 _saleId) {
        ...
        require(!sale.isSoldOut, "Sold out");
    ...
    function setContracts(address _project) external {
        require(_project != address(0), "Invalid param");
        require(address(project) == address(0), "Project existed");
    ...
    function setControllers(address[] memory _accounts, bool _allow) external onlyAdmin accountsNotEmpty(_accounts) {
        for (uint256 i; i < _accounts.length; i++) {
            ...
            require(_allow != controllers[_accounts[i]], "Controller existed");
    ...
    function setWinners(uint256 _saleId, address[] memory _accounts, bool _isWinner) external onlyController accountsNotEmpty(_accounts) {
        ...
        for (uint256 i; i < _accounts.length; i++) {
            require(_accounts[i] != address(0), "Invalid account");
            require(_isWinner != winners[_saleId][_accounts[i]], "Winner existed");
    ...
    function createRaiseSingle(uint256 _projectId, uint256[] memory _tokenIds, uint256[] memory _raisePrices) external payable onlyManagerWithProjectId(_projectId) {
        ...
        require(_project.isRaise, "Invalid project");
    ...
    function bidSingle(uint256 _saleId) external payable onlyWinner(_saleId) onSale(_saleId) {
        ...
        sale.isSoldOut = true;
        project.updateSold(sale.projectId, _project.sold += 1);
        ...
        _sharing(_project, sale, 0, _price, _project.minSales);
        if (_project.minSales == 0) IOSB721(_project.token).safeTransferFrom(address(this), _msgSender(), sale.tokenId);
    }
    ...
    function bidMulti(uint256 _saleId, uint256 _amount) external payable onlyWinner(_saleId) onSale(_saleId) {
        ...
        _sharing(_project, sale, _amount, _price, _project.minSales);
        if (_project.minSales == 0) IOSB1155(_project.token).safeTransferFrom(address(this), _msgSender(), sale.tokenId, _amount, "");
    }

    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, uint256 _amount, uint256 _price, uint256 _minSales) private {
    ...
    function closeProject(uint256 _projectId, bool _isGive) external onlyManagerWithProjectId(_projectId) nonReentrant {
        ProjectInfo memory _project = project.getProject(_projectId); 
        if (_project.minSales == 0) require(!_isGive, "Invalid soft cap");
        require(block.timestamp > _project.saleEnd, "Sale live");
        uint256[] memory saleIds = saleIdsOfProject[_projectId];
         
        if (_project.sold < saleIds.length) {
            for (uint256 i; i < saleIds.length; i++) {
                SaleInfo memory saleInfo = sales[saleIds[i]];     
                address[] memory accountBuyers = buyers[saleIds[i]];
                if (_project.isSingle && !saleInfo.isSoldOut) {
                    IOSB721(_project.token).safeTransferFrom(address(this), _project.manager, saleInfo.tokenId);
                    continue;
                }
                else if (!_project.isSingle && accountBuyers.length == 0) {
        ...
        emit CloseAuction(_projectId, _isGive);
    }
    ...
    function _close(ProjectInfo memory _project, SaleInfo memory _sale, address[] memory _accountBuyers, bool _isGive) private {
        for (uint256 i; i < _accountBuyers.length; i++) {
            if (bills[_sale.id][_accountBuyers[i]].account == _accountBuyers[i]) {
                ...
                uint256 moneyPaid = _isGive || _project.sold < _project.minSales ? billInfo.royaltyFee + billInfo.supperAdminFee + billInfo.sellerFee : 0;
                if (moneyPaid > 0) Helper.safeTransferNative(billInfo.account, moneyPaid);
                else if (_project.sold >= _project.minSales) {
                ...
                address receiver = _isGive || _project.sold >= _project.minSales ? billInfo.account : _project.manager;
    ...
    function currentDutchPrice(uint256 _saleId) public view returns (uint256) {
        ...
        if (currentTimestamp <= _project.saleStart) return sale.dutchMaxPrice;
