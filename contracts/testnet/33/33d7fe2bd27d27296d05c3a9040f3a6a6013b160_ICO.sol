...
contract ICO is Ownable {
    ...
    uint public adminMultiplier = 1;
    uint public adminDivider = 50;
    Token public token;
    ...
    function start()
        ...
        icoNotActive()
        onlyOwner {
        require(token.balanceOf(address(this)) >= availableTokens ,"Please transfer >= availiableTokens for ICO to this smart contract ");
    ...
    function buy(uint daiAmount)
        ...
        //uint tokenInContract = token.balanceOf(address(this));
        require(
    ...
    function withdrawTokens()
        ...
        require(token.balanceOf(address(this)) >= sale.amount,"Not enough Tokens in Smart Contract");
    ...
    function withdraw(uint amount)
        ...
        icoEnded() 
        onlyOwner {
        uint clientRatio = adminDivider-adminMultiplier;
        uint clientAmount = (amount*clientRatio)/adminDivider;
        uint adminAmount = (amount*adminMultiplier)/adminDivider;
        dai.transfer(admin, adminAmount);
        dai.transfer(clientAdmin, clientAmount);
    }

    function clientWithdrawToken(uint amount)
        external
        icoEnded()
        onlyOwner {
        require(amount <= availableTokens, "Please make sure the amount of Token withdraw is less than availableTokens left");
        availableTokens = availableTokens.sub(amount);
        token.transfer(clientAdmin, amount);
    }
    ...
    function setPriceMultiplier(uint _setPriceMultiplier) external onlyOwner {
        priceMultiplier = _setPriceMultiplier;
    } 

    function setPriceDivider(uint _setPriceDivider) external onlyOwner {
        priceDivider = _setPriceDivider;
    } 

    function setAdminMultiplier(uint _shareMultiplier) external onlyOwner {
        adminMultiplier = _shareMultiplier;
    } 

    function setAdminDivider(uint _shareDivider) external onlyOwner {
        adminDivider = _shareDivider;
    }

    function setMinPurchase(uint _minPurchase) external onlyOwner {
        minPurchase = _minPurchase;
    } 
    ...
    function setClientAddress(address _clientAdmin) external onlyOwner {
        clientAdmin = _clientAdmin;
    }

    function checkIfEnd() external view returns (bool){
    ...
    modifier onlyClient() {
        require(msg.sender == clientAdmin, "Only Client");
        _;
    }
}