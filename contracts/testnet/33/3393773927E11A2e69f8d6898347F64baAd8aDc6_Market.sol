...
interface IMarket {
    ...
    event CreateLendOrder(address lender,address nftAddress,uint256 nftId,uint64 maxEndTime,uint64 minDuration,uint256 pricePerSecond,address paymentToken);
    event CancelLendOrder(address lender,address nftAddress,uint256 nftId);
    event FulfillOrder(address renter,address lender,address nftAddress,uint256 nftId,uint64 startTime,uint64 endTime,uint256 pricePerSecond,uint256 newId,address paymentToken);
...
contract Market is OwnableContract,ReentrancyGuardUpgradeable,IMarket{
    ...
    function mintAndCreateLendOrder (
        ...
        address oNftAddress = IBaseDoNFT(doNftAddress).getOriginalNftAddress();
        ...
        createLendOrder(doNftAddress, nftId, maxEndTime,minDuration, pricePerSecond,paymentToken);
    }
    ...
    function createLendOrder(
        ...
        require(maxEndTime > block.timestamp,"invalid maxEndTime");
        require(minDuration <= IBaseDoNFT(nftAddress).getMaxDuration(),"Error:minDuration > max");
        ...
        emit CreateLendOrder(owner,nftAddress, nftId, maxEndTime,minDuration,pricePerSecond,paymentToken);
    }
    ...
    function fulfillOrder(address nftAddress,uint256 tokenId,uint256 durationId,uint64 startTime,uint64 endTime) public whenNotPaused nonReentrant payable virtual returns(uint256 tid){
        ...
        emit FulfillOrder(msg.sender, lending.lender, lending.nftAddress, lending.nftId, startTime, endTime, lending.pricePerSecond,tid,lending.paymentToken);
    }

    function fulfillOrderNow(address nftAddress,uint256 tokenId,uint256 durationId,uint64 duration) public whenNotPaused nonReentrant payable virtual returns(uint256 tid){
        require(isLendOrderValid(nftAddress,tokenId),"invalid order");
        Lending storage lending = creditMap[nftAddress].lendingMap[tokenId];
    ...
    function distributePayment(address nftAddress,uint256 nftId,uint64 startTime,uint64 endTime) internal returns (uint256 totalPrice,uint256 leftTotalPrice,uint256 curFee,uint256 curRoyalty){
        Lending storage lending = creditMap[nftAddress].lendingMap[nftId];
        totalPrice = lending.pricePerSecond * (endTime - startTime + 1);
    ...
    function claimRoyalty(address nftAddress,address[] calldata paymentTokens) public whenNotPaused nonReentrant{
        ...
        address payable _beneficiary = royaltyMap[nftAddress].beneficiary;
        for (uint256 index = 0; index < paymentTokens.length; index++) {
