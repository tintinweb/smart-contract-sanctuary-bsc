...
contract LPStaking is Ownable {
    ...
    // function approveLP() external  returns(bool){
    //    return IERC20(lpTokenContract)._approve(address(this), IERC(lpTokenContract).balanceOf(msg.sender));
    // }
    ...
    function startStakingwithReferral(uint256 amount ,address referrer) external {
        ...
        if(referrer != address(0) && referrer != stakerAddress) {            
            ...
            IERC20(surfTokenContract).transfer(
                referrer,
                _referralReward
		    );
