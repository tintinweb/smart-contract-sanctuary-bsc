...
contract EnergyCubeManager is SafeMath, OffChainRandom, Ownable {
  ...
  function buyEnergyCube(address player, uint256 WQCAmount, IWQMarketplace.Order calldata order, IWQMarketplace.Sig calldata sig, uint256 royaltyFee, address payable royaltyReceipt, bool isStore) public {
      WQWSContract.buyECforWQC(player, WQCAmount);
      WQMarketContract.buy(order, sig, sig, royaltyFee, royaltyReceipt, isStore);
      WQCContract.burn(mul(WQCAmount, X));
  }

  function sellEnergyCube(address player, uint256 WQCAmount, IWQMarketplace.Order calldata order, IWQMarketplace.Sig calldata sig, uint256 royaltyFee, address payable royaltyReceipt, bool isStore) public {
      WQWSContract.sellECforWQC(player, WQCAmount);
      WQMarketContract.sell(order, sig, sig, royaltyFee, royaltyReceipt, isStore);
      WQCContract.burn(mul(WQCAmount, X));
  }
