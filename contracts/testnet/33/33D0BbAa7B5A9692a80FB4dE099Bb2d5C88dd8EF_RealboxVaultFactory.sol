...
import './interfaces/IPrivatePriceCalculator.sol';
...
contract RealboxVault is Ownable, ERC721Holder, ReentrancyGuard {
    ...
    address public privatePriceCalculator;
    ...
    mapping(address => uint256) private _privateAmount;
    ...
    event BuyVaultToken(address indexed buyer, uint256 amount, uint256 price);
    ...
    event CrowdFundingRefund(address indexed buyer, uint256 amount, uint256 price);
     ...
     * @param _privatePriceCalculator: price calculator for the private sale (must be 0x00 if none)
     ...
     * @param _privateStartBlock: the start block for the private sale (must be _publicStartBlock if _privatePriceCalculator is 0x00)
    ...
    function initialize(
        ...
        IERC721 _realx,
        IERC20 _baseToken,
        address _privatePriceCalculator,
        ...
        privatePriceCalculator = _privatePriceCalculator;
    ...
    function claimOrRefund(uint256 _size) external nonReentrant onlyState(CrowdFundingState.Finalized) {
        ...
        for (uint256 i = 0; i < _size; i++) {
            ...
            if (refundToken > 0) {
                ...
                emit CrowdFundingRefund(txInfo.sender, refundToken, txInfo.unitPrice);
            }
            if (claimToken > 0) {
    ...
    /**
     * @notice Buy vault token using base token (in private sale)
     * @param _amount: amount of vault token to buy
     * @dev Caller must have allowance for this contract of at least `_amount`.
     */
    function buyPrivateToken(uint256 _amount) external nonReentrant onlyState(CrowdFundingState.PrivateStarted) {
        (uint256 price, uint256 allowance) = IPrivatePriceCalculator(privatePriceCalculator).calculatePriceAndAllowance(
            msg.sender
        );
        require(price > 0, 'RealboxVault: Invalid price');
        require(_amount <= allowance.sub(_privateAmount[msg.sender]), 'RealboxVault: Buy more than allowance');
        _privateAmount[msg.sender] = _privateAmount[msg.sender].add(_amount);
        _buyToken(_amount, price);
    }
    ...
    function _buyToken(uint256 _amount, uint256 _price) internal {
        require(_amount > 0);
        ...
        _txInfo[txIds] = TxInfo(msg.sender, _amount, _price);
        txIds += 1;
        currentSupply = currentSupply.add(_amount);
        emit BuyVaultToken(msg.sender, _amount, _price);
    }
...
contract RealboxVaultFactory is AccessControlEnumerable {
    ...
    constructor(
        ...
        require(_treasuryFee <= 10000, 'RealboxVaultFactory: treasuryFee must not be higher than 10000 (100%)');
     ...
     * @param _treasuryFee: crowdfunding fee (100 = 1%, 500 = 5%, 5 = 0.05%)
     */
    function setTreasury(address _treasuryAddress, uint256 _treasuryFee) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            'RealboxVaultFactory: must have admin role to set new treasury'
        );
    ...
    function deployVault(
        ...
        require(block.number < _privateStartBlock, 'RealboxVaultFactory: startBlock must be higher than current block');
        require(
            (_privatePriceCalculator == address(0) && _privateStartBlock == _publicStartBlock) ||
                (_privatePriceCalculator != address(0) && _privateStartBlock < _publicStartBlock),
            'RealboxVaultFactory: Private sale parameters incorrect'
        );
        require(_publicStartBlock < _endBlock, 'RealboxVaultFactory: publicStartBlock must be lower than endBlock');
        require(_minSupply <= _maxSupply, 'RealboxVaultFactory: minSupply must not be higher than maxSupply');
        ...
        RealboxVault(vaultAddress).setTreasury(treasuryAddress, treasuryFee);
        RealboxVault(vaultAddress).initialize(
            _vaultIds.current(),
            realx,
            _baseToken,
            _privatePriceCalculator,
            _publicUnitPrice,
            _minSupply,
            _maxSupply,
            _privateStartBlock,
...
interface IPrivatePriceCalculator {
    function calculatePriceAndAllowance(address _buyer) external view returns (uint256 price, uint256 allowance);
}