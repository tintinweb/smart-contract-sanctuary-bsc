...
contract LGOStakingView is Initializable, ILGOStakingView {
    struct EmissionInfo {
    ...
    function initialize(address _lgo) external initializer {
        require(_lgo != address(0), "LGOStakingView::initialize: invalid address");
        LGO = IERC20(_lgo);

        emissions.push(EmissionInfo({rewardsPerSecond: 11574074074074074, startTimestamp: 1671592195, endTimestamp: 0}));
    }
    ...
    function estimatedLGOCirculatingSupply() external view override returns (uint256) {
        ...
        uint256 _lgoBurnedAmount = LGO_MAX_SUPPLY > _lgoTotalSupply ? LGO_MAX_SUPPLY - _lgoTotalSupply : 0;
