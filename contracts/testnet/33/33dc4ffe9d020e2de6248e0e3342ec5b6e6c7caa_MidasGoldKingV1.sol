...
contract MidasGoldKingV1 is Initializable {
    uint256 private oboloWeigth;
    address private _owner;
    AggregatorV3Interface private goldPriceOracle;
    ...
    function initialize(
        ...
        owner = msg.sender;
        oboloWeigth = 570000000000000000; // 0.57 Grams
        goldPriceOracle = AggregatorV3Interface(goldOracleAddress);
    ...
    function isPayingWithNativeCurrency(address tokenAddress)
        ...
        returns (bool)
    {
        return tokenAddress == this.getNativeTokenAddress();
    }

    function setOracles(address goldOracleAddress, address kindomTokenAddress)
        public
        onlyOwner
    {
        goldPriceOracle = AggregatorV3Interface(goldOracleAddress);
    ...
    function getGoldUsdPrice(uint256 grams) public view returns (uint256) {
        return uint256(askOracle(goldPriceOracle)) * grams;
    }

    function getOboloWeigth() public view returns (uint256) {
        return oboloWeigth;
    }

    function getOboloUsdPrice() public view returns (uint256) {
        uint256 _goldGramPrice = getGoldUsdPrice(1 ether); // 1 gold gram
        return oboloWeigth * _goldGramPrice;
    }
        ...
        returns (uint256)
    {
        return
            estimatedTokensForTokens( //cambiando el orden de los address me devuelve lo que coincide con pancake en testnet
                this.getNativeTokenAddress(), //segundo
    ...
    function swapBnbForUSD(address to) external payable {
        ...
        uint256 amountOutIn = midasMultinetworkRouter.getAmountsOut(
            msg.value,
            path
        )[1];

        // make the swap
        midasMultinetworkRouter.swapExactETHForTokens{value: msg.value}(
            amountOutIn,
            path,
            to,
    ...
    function swapTokensForStableCoin(
        ...
        path[0] = tokenAddress;
        path[1] = this.getNativeTokenAddress();
        path[2] = this.getStableCoinAddress();

        // Do approve for router spend swap token amount
        IERC20(this.getNativeTokenAddress()).approve(
            address(midasMultinetworkRouter),
            amount
        );

        IERC20(tokenAddress).approve(address(midasMultinetworkRouter), amount);
