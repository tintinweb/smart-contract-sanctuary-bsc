...
contract Collections is Ownable,Pausable,ICollections {
    ...
    mapping (uint256 => uint256) public What_Collection;
    ...
    constructor () {
         MITI=IMiticoin(0x1c1AA27A86DC3739BA339C229B3ffB584c24e36A);
    }

    function AddNFTA (uint256 amount) public onlyOwner {
        require(Total_ColNFTA+amount<=Max_Col_NFTA);
        uint256 Col;
        while(Col<amount){
        ADD_Collection_NFTA(NFTA_id_array[Total_ColNFTA], 0xBB8127c845F01d6cCf8dE7a26Dc83aE6cd596Fa6, One_Col_NFTA);
        Total_ColNFTA=Total_ColNFTA+amount;
        Col++;
        Total_Collection=Total_Collection+amount;
        }
    }
    function ADD_Collection_NFTA (uint256 ID, address Collection_address,uint256 Max_world) internal {
        require(Primary==false);
            require(Total_ColNFTA<Max_Col_NFTA);
          Infinity(ID, Collection_address, Max_world);
        
          Total_world_NFTA=Total_world_NFTA+Max_world;
          Total_Collection++;
          What_Collection[ID]=1;
          MITI.BypassMINT(Max_world);
    }
