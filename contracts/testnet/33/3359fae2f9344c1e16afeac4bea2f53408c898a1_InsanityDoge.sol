...
contract InsanityDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("InsanityDoge_Dividend_Tracker", "InsanityDoge_Dividend_Tracker") {
        claimWait = 86400;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "InsanityDoge_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "InsanityDoge_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main InsanityDoge contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "InsanityDoge_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "InsanityDoge: Cannot update claimWait to same value");
...
contract InsanityDoge is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    InsanityDogeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Insanity Doge", "B3") {
        BNBRewardsFee = 25;
        liquidityFee = 1;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 0;
        marketingWallet = payable(0x8f4065758Dd3aa21c434EeE730249Fc397d5e77b);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new InsanityDogeDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "InsanityDoge: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "InsanityDoge: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "InsanityDoge: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "InsanityDoge: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "InsanityDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "InsanityDoge: Cannot update gasForProcessing to same value");
