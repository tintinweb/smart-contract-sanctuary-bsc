...
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";
import "./interfaces/IABep20Interface.sol";
import "./interfaces/IAnnexRouter02.sol";
import "./libraries/IdToAddressBiMap.sol";
import "./libraries/SafeCast.sol";

contract AnnexIronNFTVault is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable  {
  ...
  IERC721Upgradeable public annexNFT;
  IERC20Upgradeable public aNN;
  IABep20Interface public aANN;
  IERC20Upgradeable public tusd;
  ...
  uint256 private _daySecond;
  uint256 private _monthSecond;
  ...
  function initialize() public initializer {
    owner = msg.sender;
    _daySecond = 86400;
    _monthSecond = _daySecond.mul(30);
    uint256 rewardsAllocationTUSD = 1000 * 1e18;
    rewardsPerSecond = (rewardsAllocationTUSD).div(_daySecond);
    annexNFT = IERC721Upgradeable(0xB103D5166CA024Aa167A93Ca88f713ec6998B38A);           // ANNEX NFT
    aNN = IERC20Upgradeable(0xB8d4DEBc77fE2D412f9bA5B22B33A8f6c4d9aE1e);                 // ANNEX TOKEN
    aANN = IABep20Interface(0x15EdC067884b969a6BE23605499f5e6fc114017e);                 // ANNEX A-TOKEN
    tusd = IERC20Upgradeable(0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47);                // TUSD
  ...
  function _stake(uint256 _tokenId) internal {
    ...
    require(annexNFT.ownerOf(_tokenId) == _user, "NOT OWNER OF NFT");
  ...
  function _updateReward() internal returns (uint256) {
    Staker storage staker = stakers[msg.sender];
    uint256 bonusAmount = calculateRewards(msg.sender);
    uint256 bounsDiff = 0;
    if (bonusAmount == 0) {
      ...
      bounsDiff = bonusAmount.sub(staker.rewardsEarned);
      totalRewards = totalRewards.add(bounsDiff);
      staker.rewardsEarned = staker.rewardsEarned.add(bounsDiff);
      if (staker.isAutoCompound) {
        totalRewardsForCompound = totalRewardsForCompound.add(bounsDiff);
      }
    } else {
      bounsDiff = staker.rewardsEarned.sub(bonusAmount);
      totalRewards = totalRewards.sub(bounsDiff);
      staker.rewardsEarned = staker.rewardsEarned.sub(bounsDiff);
      if (staker.isAutoCompound) {
        totalRewardsForCompound = totalRewardsForCompound.sub(bounsDiff);
      }
    }
    return bounsDiff;
  }
  ...
  function unStakeBatch(uint256[] memory tokenIds) external nonReentrant  whenNotPaused {
    for (uint256 i = 0; i < tokenIds.length; i++) {
      _unStake(tokenIds[i]);
    }
  }
...
//    uint64 userId = getUserId(_user);
    ...
    require(tokenOwner[_tokenId] == _user, "NOT OWNER OF NFT");
    ...
    uint256 unstakedRewardClaim = _updateReward();
  ...
  function updateAutoCompound(bool isAutoCompound) public nonReentrant whenNotPaused {
...
//    uint256 deltaSeconds    = (block.timestamp).sub(staker.lastClaimedAt);
    uint256 _userStakeToken = (staker.tokenIds.length).div(totalStakedTokens);
    return rewardsPerSecond.mul((block.timestamp).sub(staker.lastClaimedAt)).mul(_userStakeToken <= 0 ? 1 : _userStakeToken );
  }
  ...
  function claim() public nonReentrant whenNotPaused {
    ...
    staker.aANNrewardsEarned += staker.rewardsEarned;
    staker.rewardsEarned = 0;
    staker.lastClaimedAt = block.timestamp;
  ...
  function swapAutoCompound() public onlyOwner nonReentrant whenNotPaused  {
  ...
  function _swapTokens(uint256 _amount) internal returns (uint256) {
  ...
  function _lendingTokens(uint256 _amount) internal {
  ...
  function setRewardsAllocationTUSD(uint256 _amount) public onlyOwner nonReentrant  {
    rewardsPerSecond = (_amount).div(_daySecond);
  }

  function setAnnexNFT(address _new) public onlyOwner nonReentrant  {
    annexNFT = IERC721Upgradeable(_new);
  }

  function setAnn(address _new) public onlyOwner nonReentrant  {
    aNN = IERC20Upgradeable(_new);
  }

  function setaAnn(address _new) public onlyOwner nonReentrant  {
    aANN = IABep20Interface(_new);
  }

  function setTusdAddress(address _new) public onlyOwner nonReentrant  {
    tusd = IERC20Upgradeable(_new);
  }

  function setAnnexRouter(address _new) public onlyOwner nonReentrant  {
    annexRouter = IAnnexRouter02(_new);
  }

  function withdrawTokens(address token) public onlyOwner nonReentrant  {
    uint256 balanceOfTokens = IERC20Upgradeable(token).balanceOf(address(this));
    require(balanceOfTokens > 0, "INSUFFICIENT BALANCE");
    IERC20Upgradeable(token).transfer(owner, balanceOfTokens);
  }
  /**
