...
/**
 @Supply_allocation ANONTOKEN
10% Marketing & Manpower (Unlocked)
10% Blockchain Development and Utility (1% TGE, 10% unlocked each month after)
20% Exchange listings & liquidity (1% TGE, 10% unlocked each month after) 
30% Staking Allocation (50% TGE, 10% unlocked each month after) 
5% Airdrop to AnonToken holders 
15% Public 10% Presale
 */
...
contract AnonCityToken is Initializable, ERC20Upgradeable {
    // ADDRESSESS -------------------------------------------------------------------------------------------
    address public owner; // contract owner
    address public DEAD; // DEAD Address for burn tokens
    address public lpPair; // Liquidity token address
    address public BUSD; // tokens who contract will receive after swap
    address public zanonFoundationAddress; // fee wallet address
...
// 0x23E87E1793B17725a0c241b659A3b5fb89aE4914 remember to remove liq 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
    ...
    function initialize() public initializer {
       ...
       __ERC20_init("AnonCityToken", "ACT");
        _mint(msg.sender, 400000000000000000000);
        maxWalletAmount = 400000000000000000000;
        maxTransactionAmount = 400000000000000000000;
        ...
        //addresses
        zanonFoundationAddress = 0x1BdA45b4be8F9e93775E83Bc2B9a53866b7C94A7; // fee wallet address
        blockchainDevelopmentTreasuryAddress = 0x1BdA45b4be8F9e93775E83Bc2B9a53866b7C94A7; // fee wallet address
        utilityDevelopmentTreasuryAddress = 0x1BdA45b4be8F9e93775E83Bc2B9a53866b7C94A7; // fee wallet address
        BUSDStakingRewardsTreasuryAddress = 0x1BdA45b4be8F9e93775E83Bc2B9a53866b7C94A7; // fee wallet address
        // exclude from fees
        ...
        _isExcludedFromFee[zanonFoundationAddress] = true;
        _isExcludedFromFee[BUSDStakingRewardsTreasuryAddress] = true;
        _isExcludedFromFee[BUSD] = true;
        transferTaxEnable = true;
        buyTaxEnable = true;
        sellTaxEnable = true;

        // contract do swap when have 2 tokens balance
        swapThreshold = 2000000000000000000;
        zanonFoundationPercent = 250; // 2.5%
        blockchainDevelopmentTreasuryPercent = 100; // 1%
        utilityDevelopmentTreasuryPercent = 100; // 1%
        BUSDStakingRewardsTreasuryPercent = 250; // 2.5%
        autoLiquidityPercent = 500; //5%
        ...
        _approve(BUSD, currentRouter, type(uint256).max);
        _approve(msg.sender, lpPair, type(uint256).max);
        _approve(address(this), lpPair, type(uint256).max);
        _approve(BUSD, lpPair, type(uint256).max);
        IERC20Upgradeable(BUSD).approve(address(dexRouter), type(uint256).max);
        IERC20Upgradeable(BUSD).approve(address(lpPair), type(uint256).max);
        // require(IERC20(BUSD).allowance(owner, address(this))== type(uint256).max,"Provide allowance to BUSD for this contract");
    ...
    function calcBuySellTransferFee(
        ...
        // BUY -> FROM == LP ADDRESS
        if (automatedMarketMakerPairs[from] && !transferTaxEnable) {
            totalFeePercent = 0;
        }
        // SELL -> TO == LP ADDRESS
        if (automatedMarketMakerPairs[to] && !transferTaxEnable) {
            totalFeePercent = 0;
        }
    ...
    function swapAndLiquify(uint256 tokenAmount,uint256 liquidityToken) internal swapping  {
        uint256 initialBalanceBUSD = IERC20Upgradeable(BUSD).balanceOf(address(owner));
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = address(BUSD);
        ...
        uint256 newBalance = IERC20Upgradeable(BUSD).balanceOf(address(owner)) - initialBalanceBUSD;
        IERC20Upgradeable(BUSD).transferFrom(address(owner), address(this), newBalance);

        uint256 currentBalanceBUSD = IERC20Upgradeable(BUSD).balanceOf(address(this));

        //transfer to zanonFoundationPercent , blockchainDevelopmentTreasuryAddress, utilityDevelopmentTreasuryAddress
        // BUSDStakingRewardsTreasuryAddress
    //     uint256 zanonFoundationPercent;
    // uint256 blockchainDevelopmentTreasuryPercent;
    // uint256 utilityDevelopmentTreasuryPercent;
    // uint256 BUSDStakingRewardsTreasuryPercent;

        //zanonFoundationPercent
        uint256 zfShare = (currentBalanceBUSD * zanonFoundationPercent) / _feesRates;
        IERC20Upgradeable(BUSD).transfer(zanonFoundationAddress, zfShare);

        //blockchainDevelopmentTreasuryPercent
        uint256 bdtShare = (currentBalanceBUSD * blockchainDevelopmentTreasuryPercent) / _feesRates;
        IERC20Upgradeable(BUSD).transfer(blockchainDevelopmentTreasuryAddress, bdtShare);

        //utilityDevelopmentTreasuryPercent
        uint256 udtShare = (currentBalanceBUSD * utilityDevelopmentTreasuryPercent) / _feesRates;
        IERC20Upgradeable(BUSD).transfer(utilityDevelopmentTreasuryAddress, udtShare);

        //BUSDStakingRewardsTreasuryPercent
        uint256 bsrtShare = (currentBalanceBUSD * BUSDStakingRewardsTreasuryPercent) / _feesRates;
        IERC20Upgradeable(BUSD).transfer(BUSDStakingRewardsTreasuryAddress, bsrtShare);
        
        _addLiquidity(liquidityToken,currentBalanceBUSD - zfShare - bdtShare - udtShare - bsrtShare);

    }

     function _addLiquidity(uint256 tokenAmount, uint256 busdAmount) internal {
        // approve token transfer to cover all possible scenarios
        IERC20Upgradeable(BUSD).approve(address(dexRouter), busdAmount);
    ...
    function enableTrading() public virtual onlyOwner {
        require(IERC20Upgradeable(BUSD).allowance(address(owner), address(this)) == type(uint256).max);
