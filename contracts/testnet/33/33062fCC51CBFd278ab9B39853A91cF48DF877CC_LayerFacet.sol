...
contract LayerFacet is Helper {
    ...
    function withdraw(uint256 liquidity, uint32[] calldata crates, uint256[] calldata amounts) external nonReentrant {
        LibDiamond.enforceIsContractOwner(); // Only the owner
        (uint256 balance) = checkLiq(liquidity, amounts); // calc amount DLP for burn
...
contract Helper is ReentrancyGuard {
    ...
    function DLPtoBNB(uint256 liqDLP) public view returns (uint256) {
        uint256 amountLP = calcBalance(liqDLP, IDLP(s.c.dlp).totalSupply(), s.reserveLP);
        ...
        (uint256[] memory countTokens, , ) = getCountTokenFromCorn(getCORN);
        return getBNB + countTokens[1]; 
        
/*
        uint256 amountLP = calcBalance(liqDLP, IDLP(s.c.dlp).totalSupply(), s.reserveLP);
        uint256 supplyLP = IERC20(s.c.pair).totalSupply();
        (uint256[] memory countTokens, , ) = getCountTokenFromCorn((amountLP * (IERC20(s.c.topcorn).balanceOf(s.c.pair)) / supplyLP));
        return (amountLP * (IERC20(s.c.wbnb).balanceOf(s.c.pair)) / supplyLP) + countTokens[1]; 
*/
    }
}
