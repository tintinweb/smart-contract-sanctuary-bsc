...
contract Referral{
   ...
   * @param _memberRate The bonus rate for each level, which will divide by decimals too. The max depth is MAX_REFER_DEPTH.
   */
  constructor(
    uint256 _decimals,
    uint256[] memory _levelRate,
    uint256[] memory _memberRate
  )
  {
  ...
  function addReferrer(address payable referrer) internal returns(bool){
    ...
    //     return false;
    //   }else{
    //   accounts[referrer].referrer=payable(secondrefer);
    //    if( accounts[secondrefer].referrer==address(0)){
    //   address thirdsecond=referralvault.getReferrer(secondrefer);
    //   accounts[secondrefer].referrer=payable(thirdsecond);
    //   }
    //   }
  ...
  function payReferral(uint256 value,address token) internal returns(uint){
    ...
    for (uint256 i; i < levelRate.length; i++) {
      ...
      if(depositamount[parent]==0){
         c = value.mul(memberRate[i]).div(decimals);
      }else{
         c = value.mul(levelRate[i]).div(decimals);
      }
 ...
 function changeReferrer(address payable _user,address payable referrer)public onlyOperator{
  ...
  function setMemberrate(uint256[] memory _memberRate) public onlyOperator{
    require(_memberRate.length > 0, "Referral level should be at least one");
    require(_memberRate.length <= MAX_REFER_DEPTH, "Exceeded max referral level depth");
    memberRate = _memberRate;
  }
...
contract AgentContribution is ReentrancyGuard,IAgentContribution,Referral,Pausable{
  ...
  constructor(
    ...
    uint256[] memory _levelRate,
    uint256[] memory _memberRate
    ) Referral (_decimals,_levelRate,_memberRate) {
  ...
  function depositAgent(address token,address payable referrer,uint256 amount)public override payable {
    ...
    require(depositamount[msg.sender]==0,'Already Purchased');
    ...
    if(amount==0){
    require(!memberdetect[msg.sender],'Already Member');
    memberdetect[msg.sender]=true;
    }else{
