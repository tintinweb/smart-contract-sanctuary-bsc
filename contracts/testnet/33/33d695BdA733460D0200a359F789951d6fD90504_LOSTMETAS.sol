...
contract LOSTMETAS is Context, IERC20, Ownable {
    ...
    uint256 public marketingAndBuybackShare = 30;
    uint256 public devShare                 = 10;

    address public marketingWallet = 0xB828b4B5dE5ca744688d6F7fe27310Ac621aEa0b;
    address public devWallet       = 0x8D1b7Be66DE1893F13b8230f899e0C27c2b3a7FD;
    ...
    event SwapFeeSharesChanged(uint256 liquidityShare, uint256 marketingAndBuybackShare, uint256 devShare);
    ...
    {   
        ...
        antiBot = ILostMetasAntiBot(0x0952f734e4a7cCEb8901626265F66DdA30667e4F);
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && launchTime == 0) {
        ...
        if (
            ...
            uint256 marketingTokens = contractTokenBalance * marketingAndBuybackShare / 100;
            uint256 devTokens       = contractTokenBalance * devShare / 100;

            uint256 initialBalance = address(this).balance;
            swapTokensForBNB(marketingTokens + devTokens);
            uint256 bnbReceived    = address(this).balance - initialBalance;

            if (devTokens > 0) {
                uint256 devBNB = bnbReceived * devTokens / (devTokens + marketingTokens);
                sendBNB(payable(devWallet), devBNB);
            }

            if (marketingTokens > 0) {
                uint256 marketingBNB = bnbReceived * marketingTokens / (marketingTokens + devTokens);
    ...
    function setSwapFeeShares(uint256 _liquidityShare, uint256 _marketingAndBuybackShare, uint256 _devShare) external onlyOwner {
        require(_liquidityShare + _marketingAndBuybackShare + _devShare == 100, "Shares must add up to 100%");
        liquidityShare = _liquidityShare;
        marketingAndBuybackShare = _marketingAndBuybackShare;
        devShare = _devShare;
        emit SwapFeeSharesChanged(liquidityShare, marketingAndBuybackShare, devShare);
    }
