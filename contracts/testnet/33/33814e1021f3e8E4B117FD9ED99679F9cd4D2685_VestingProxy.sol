...
contract VestingProxy is IVesting2, Initializable {
    ...
    VestingSchedule public settings;
    ...
    function initialize(
        ...
        require(
            settings_.firstUnlockPerc +
                settings_.linearUnlockSpeed *
                settings_.linearUnlockPeriod <=
                1 ether,
            'vesting unlock percentage exceeds 100%'
        );
    }

    function changeSettings(VestingSchedule calldata settings_) external afterInit {
        require(msg.sender == manager, 'not manager');
        settings = settings_;
    }

    function unlocked(address user) public view returns (uint256) {
        uint256 userTotal = launchpad.userTotal(round, user);
        VestingSchedule memory sched = settings;
        uint256 startTime = launchpad.startTime() + launchpad.duration(); // TODO: store starttime there
        console.log('unlocked', user);

        // [0, pause)
        if (startTime + sched.pausePeriod > block.timestamp) {
            console.log('pause period');
            return 0;
        }

        // [pause, cliff)
        if (startTime + sched.pausePeriod + sched.cliffPeriod > block.timestamp) {
            console.log('cliff period');
            return (userTotal * sched.firstUnlockPerc) / 1 ether;
        }

        // [cliff, linearUnlock)
        if (
            startTime + sched.pausePeriod + sched.cliffPeriod + sched.linearUnlockPeriod >
            block.timestamp
        ) {
            ...
            uint256 vestingPeriod = block.timestamp -
                ...
                sched.pausePeriod -
                sched.cliffPeriod;
            uint256 percentage = sched.firstUnlockPerc +
                (sched.linearUnlockSpeed * vestingPeriod);

            return (userTotal * percentage) / 1 ether;
        }

        console.log('vesting over');
        // [linearUnlock, inf)
        return userTotal;
    }
    ...
    function claim(address investor) external afterInit {
        ...
        uint256 stableAmount = (projectAmountClaimed * launchpad.raisedTotalCapped()) /
            launchpad.projectTokensAmount();
