...
contract OrbitPlay is Ownable, ReentrancyGuard {
    ...
    function placeDiceRollBet(uint256 amount, uint256 betNumber) public {
        ...
        require(
            amount >= minBetAmount,
            "OrbitPlay: Your amount should be equal or greater than min bet!"
        );
        require(
            amount <= maxBetAmount,
            "OrbitPlay: Your amount should be equal or less than max bet!"
        );
    ...
    function placeRoshamboBet(uint256 amount, uint256 betGesture) public { // 1: Rock 2: Paper 3: Scissors
		require(
            roshamboPlayable,
            "OrbitPlay: Rock, Paper, Scissors can not be played!"
        );
        require(
            amount >= minBetAmount,
            "OrbitPlay: Your amount should be equal or greater than min bet!"
    ...
    function diceRandom() private returns (uint) {       	        
        uint rnd = 0;
        randDiceNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randDiceNonce)));        
        rnd = rnd / 43;
        return rnd + 1;
    }
    ...
    function coinFlipRandom() private returns (uint) {       	        
        uint rnd = 0;
        randCoinFlipNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randCoinFlipNonce)));
        rnd = rnd / 128;
        return rnd + 1;
    }
    ...
    function spinRandom() private returns (uint) {       	        
        uint rnd = 0;
        randSpinNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randSpinNonce)));
        rnd = rnd / 64;
        return rnd + 1;
    }

    function roshamboRandom() private returns (uint) {       	        
        uint rnd = 0;
        randRoshamboNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randRoshamboNonce)));
        rnd = rnd / 86;
        return rnd + 1;
    }
