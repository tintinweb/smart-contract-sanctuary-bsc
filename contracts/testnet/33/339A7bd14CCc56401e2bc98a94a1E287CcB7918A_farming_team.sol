...
contract farming_team is owned {
    ...
    constructor(address ownerAddress) public {
        ...
        for (uint8 i = 1; i <= LAST_LEVEL; i++) {
            users[ownerAddress].activeUserLevels[i] = true;
        }
    ...
    function updateUserReferrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        else if(users[referrerAddress].UserMatrix[level].referrals.length == 3) {
            sendDividends(referrerAddress, userAddress,level);
        }
