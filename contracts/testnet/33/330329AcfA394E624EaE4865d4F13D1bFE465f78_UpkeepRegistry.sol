...
contract UpkeepRegistry is UUPSUpgradeable, AccessControlEnumerableUpgradeable, PausableUpgradeable, KeeperCompatibleInterface {
    address[] public _upkeeps; // contracts that need upkeep
    uint256 public _lastUpkeepPerformedIndex; // the last upkeep index in the array that performed an upkeep

    uint256 public _maxUpkeeps; // max number of upkeeps
     ...
     */
    function addUpkeep(address upkeep) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 upkeepsCount = _upkeeps.length;
        require(upkeepsCount < _maxUpkeeps, "UpkeepRegistry: Cannot add more than max up keeps");
        for (uint256 i=0; i<upkeepsCount; i++) {
            if (_upkeeps[i] == upkeep)
                return;
        }
        
        _upkeeps.push(upkeep);
     ...
     */
    function removeUpkeep(address upkeep) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || upkeep == msg.sender, "UpkeepRegistry: No access to remove upkeep");
        
        uint256 upkeepsCount = _upkeeps.length;
        for (uint256 i=0; i<upkeepsCount; i++) {
            if (_upkeeps[i] == upkeep) {
                if (i != upkeepsCount - 1)
                    _upkeeps[i] = _upkeeps[upkeepsCount - 1];
                _upkeeps.pop();
    ...
    function checkUpkeep(bytes calldata checkData) external override view returns (bool, bytes memory) {
        ...
        uint256 upkeepsCount = _upkeeps.length;
        for (uint256 i=_lastUpkeepPerformedIndex+1; i<upkeepsCount; i++) {
            KeeperCompatibleViewInterface upkeep = KeeperCompatibleViewInterface(_upkeeps[i]);
        ...
        for (uint256 i=0; (i<_lastUpkeepPerformedIndex+1) && (i<upkeepsCount); i++) {
            KeeperCompatibleViewInterface upkeep = KeeperCompatibleViewInterface(_upkeeps[i]);
            try upkeep.checkUpkeep(checkData) returns (bool upkeepNeeded, bytes memory performData) {
                if (upkeepNeeded)
                    return (upkeepNeeded, abi.encode(upkeep, performData));
