...
contract StoX1 is AccessControlEnumerable {
    using ECDSAOffsetRecovery for bytes32;

    uint8 public constant SIGNATURE_LENGTH = 65;
    bytes32 public constant OWNER_ROLE = keccak256("OWNER_ROLE");
    address public immutable stoXCC;
    ...
    function switchTokenHalving(
        ...
        IStoXCC(stoXCC).switchTokenHalving(
            _token,
            _enableStatus,
            _tokenToUsdPath
        );
    ...
    function getRewards() public {
        ...
        if (initBalance > 0) {
            ...
            for (uint256 i = 0; i < ownersCount; i++) {
                ...
                uint256 reward = (initBalance * (ownersWeight[owner])) /
                    (currentTotalWeight);
                payable(owner).transfer(reward);
            }

            if (IStoXCC(stoXCC).isTokenHalvingEnabled()) {
                address tokenAddress = IStoXCC(stoXCC).activeToken();
                uint256 tokenBalance = IPancakeERC20(tokenAddress).balanceOf(
                    address(this)
                );
                if (tokenBalance > 0) {
                    for (uint256 i = 0; i < ownersCount; i++) {
                        address owner = getRoleMember(OWNER_ROLE, i);
                        IPancakeERC20(tokenAddress).transfer(
                            owner,
                            (ownersWeight[owner] * tokenBalance) /
                                currentTotalWeight
                        );
                    }
                }
