...
contract d is IBEP20, Auth {
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingWallet = 0xeB86B7e6A80C1F170e85e9D6A56b6B4BD7145346;  // Marketing wallet
    ...
    function changeTxLimit(uint256 newLimit) external authorized {
        ...
        require(newLimit >= 1000000 * 10 ** _decimals); // Max Tx amount shouldn't be lower or equal to 0.1% of Total Supply
    }
    ...
    function changeWalletLimit(uint256 newLimit) external authorized {
        _walletMax  = newLimit;
        require(newLimit >= 10000000 * 10 ** _decimals); // Max Wallet amount shouldn't be bigger or equal to 1% of Total Supply
    }
    ...
    function changeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee) external authorized {
        ...
        totalFee = liquidityFee.add(marketingFee).add(rewardsFee);
        totalFeeIfSelling = totalFee;
        require(totalFee <= 20); // Total fee should be lower or equal to 10%
    }
    ...
    function rescueBNB(address payable recipient, uint256 amount) external authorized {
        recipient.transfer(amount);
    }

    function changeMarketingFeeReceiver(address newMarketingWallet) external authorized {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        {
            require(_balances[recipient].add(amount) <= _walletMax || isMaxWalletExempt[recipient], "Receiver's max wallet amount exceeded");
        }
