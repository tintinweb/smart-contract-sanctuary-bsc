...
contract Sale is Initializable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    function bidSingle(uint256 _saleId) external payable onlyWinner(_saleId) isValidSale(_saleId) {
        ...
        require(_project.isRaise ? msg.value == sale.raisePrice : msg.value >= currentPriceToSale(_saleId), "bid: invalid value");
    ...
    function bidMulti(uint256 _saleId, uint256 _amount) external payable onlyWinner(_saleId) isValidSale(_saleId) {
        ...
        require(timestamp >= _project.saleStart, "bid: sale is not start");
        require(timestamp <= _project.saleEnd, "bid: sale end");
        require(_project.isRaise ? msg.value == sale.raisePrice * _amount : msg.value >= currentPriceToSale(_saleId) * _amount, "bid: invalid value");
    ...
    function currentPriceToSale(uint256 _saleId) public view returns (uint256) {
