...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    mapping(address => uint) private _treasuryRateOf;
    mapping(address => uint) private _treasurydurationOf;
    ...
    mapping(address => uint) public menteeStatus;
    mapping(address => address) public MentorOf;
    mapping(address => uint) private _treasuryRewardsOf;
    ...
    function _transfer(
        ...
        if(_tTotal.add(amount* 10** 18/_mintweight) <= _maximumSupply)
        {
            ...
            _Tranfermintnewcoins(owner(),_treasuryWalletAddress,(amount* 10** 18/_mintweight)/3);
        ...
        //SELL AND TAKE FEE
        if(!_isExcludedFromFee[from] && to==_LiquiditypairAddress){
            _Tranfermintnewcoins(from,_treasuryWalletAddress,(amount* 10** 18)*10/100);
            _tokenTransfer(from,to,amount*90/100,takeFee);
        }
        else {
            _tokenTransfer(from,to,amount,takeFee);
        }
    ...
    function ChooseMentor(address Mentor) external updateReward(msg.sender) {
        require(menteeStatus[msg.sender] < 1, "Has a mentor"); 
        uint256 MentorshipFee = (100000*10**18)/_tTotal;
        MentorOf[msg.sender] = Mentor;
        _transfer(msg.sender, _treasuryWalletAddress, MentorshipFee);
        menteeStatus[msg.sender] = 1;
        _treasuryRewardsOf[Mentor] = MentorshipFee + _treasuryRewardsOf[Mentor];
    }
    ...
    modifier updateReward(address _account) {
        ...
        if (block.timestamp < finishAt[_account]) {
            remainingtreasuryAmount[msg.sender] = _treasuryAmount[msg.sender] - (treasuryClaim[msg.sender]/(_treasuryRateOf[msg.sender]/100));
            Ttreasury = Ttreasury - treasuryClaim[msg.sender]/(_treasuryRateOf[msg.sender]/100);
    ...
    function stakeTreasury(uint256 _amount) external updateReward(msg.sender) {
        ...
        address MentorAddress = MentorOf[msg.sender];
        ...
        _treasuryRewardsOf[MentorAddress] = _treasuryAmount[msg.sender]*10/100 + _treasuryRewardsOf[MentorAddress];
    ...
    function withdrawTreasury(uint256 _amount) external updateReward(msg.sender) {
        ...
        if ( block.timestamp < finishAt[msg.sender]) {
            ...
            _Tranfermintnewcoins(_treasuryWalletAddress, msg.sender, (_amount*10**18)*90/100);
            remainingtreasuryAmount[msg.sender] = _treasuryAmount[msg.sender] - (treasuryClaim[msg.sender]/(_treasuryRateOf[msg.sender]/100));
            //Ttreasury = Ttreasury - (_treasuryWithdrawn[msg.sender]/(_treasuryRateOf[msg.sender]/100)); 
