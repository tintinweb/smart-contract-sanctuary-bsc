...
contract Auction is Factory {
    ...
    function placeBid(uint256 auctionItemIndex, uint256 price) external payable  nonReentrant {
        require(_auctionItems.length > auctionItemIndex, AuctionError.AUCTION_NOT_FOUND);
        ...
        require(msg.value >= payExtra, AuctionError.BALANCE_NOT_ENOUGH);

        (bool sent, ) = address(this).call{value: payExtra}("");

        require(sent, AuctionError.PAYMENT_TOKEN_TRANSFER_FAILED);
    ...
    function withdraw(uint256 auctionItemIndex) external nonReentrant {
        ...
        if (_lastBidHistoryIndex[auctionItemIndex][_msgSender()] < _bidHistories[auctionItemIndex].length - 2) {
            ...
            uint _value = _bidHistories[auctionItemIndex][_lastBidHistoryIndex[auctionItemIndex][_msgSender()]].price;
            (bool sent, ) = payable(_msgSender()).call{value: _value}("");
        ...
        if (auctionItem.owner == _msgSender()) {
            ...
            if (feePercent == 0) {
                (bool success, ) = payable(auctionItem.owner).call{value: highestBid}("");
                require(success, AuctionError.PAYMENT_TOKEN_TRANSFER_TO_OWNER_ERROR);
            } else {
                uint256 beneficiaryReceivable = (highestBid * feePercent) / (1 ether);
                uint256 ownerReceivable = highestBid - beneficiaryReceivable;

                (bool result, ) = payable(beneficiary).call{value: beneficiaryReceivable}("");
                require(result, AuctionError.PAYMENT_TOKEN_TRANSFER_TO_BENEFICIARY_ERROR);

                (bool sentowner, ) = payable(auctionItem.owner).call{value: ownerReceivable}("");
                require(sentowner, AuctionError.PAYMENT_TOKEN_TRANSFER_TO_OWNER_ERROR);
...
contract Exchange is Factory {
    ...
    function createExchangeSale(uint256 exchangeItemIndex, uint256 amount) external payable  nonReentrant {
        ...
        require(msg.value >= price, ExchangeError.BALANCE_NOT_ENOUGH);

        if (feePercent == 0) {
            payable(exchangeItem.seller).transfer(price);
            (bool sent, ) = payable(exchangeItem.seller).call{value: price}("");
            require(
                sent,
            ...
            (bool sent, ) = payable(beneficiary).call{value: beneficiaryReceivable}("");
            require(sent, ExchangeError.PAYMENT_TOKEN_TRANSFER_TO_BENEFICIARY_ERROR);

            (bool success, ) = payable(exchangeItem.seller).call{value: sellerReceivable}("");
            require(success, ExchangeError.PAYMENT_TOKEN_TRANSFER_TO_SELLER_ERROR);
        }
...
contract Factory is AccessControlEnumerable, ReentrancyGuard {
    ...
    function _transferAsset(
        ...
        if (_checkContractIsERC1155(nftContractAddress)) {
...
library AuctionError {
    string public constant BALANCE_NOT_ENOUGH = "Error: Not enough fund to send";
