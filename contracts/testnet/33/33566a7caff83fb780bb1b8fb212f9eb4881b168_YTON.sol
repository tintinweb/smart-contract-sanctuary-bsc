...
contract YTON is Ownable{
	...
	event InvestUsdtAndHds(address indexed user, uint256 totalAAmount, uint256 totalBAmount, uint256 number);
    ...
    //IDO
	function investUsdtAndHds(uint256 tokenAAmount,uint256 tokenBAmount,uint256 number) public returns (bool) {

        usdt.transferFrom(msg.sender, address(this),tokenAAmount);
        //90%
        token.transferFrom(msg.sender,address(this),tokenBAmount);
		emit InvestUsdtAndHds(msg.sender, tokenAAmount, tokenBAmount,number);
