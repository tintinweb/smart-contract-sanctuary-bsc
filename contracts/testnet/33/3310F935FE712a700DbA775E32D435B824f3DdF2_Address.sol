...
contract NFT_Minting is ERC721Enumerable, Ownable {
    using Strings for uint256;
    string private unRevealedURL="ipfs://QmZjMEP3ZjbmvbwKp1RxaFTFGFaJDP9CEzzyZBixridpki/1.jpg";
    ...
    mapping(address => bool) public buy_before;

    constructor(

    ) ERC721("Saviour Society", "SS") {}
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) 
        {
            ...
            if(buy_before[msg.sender]!=true && supply<800)
            {
                //preSale
                if (supply<1800) 
                {
                    if(supply+_mintAmount> 1800)
                    {
                        uint pub=(supply+_mintAmount)-1800;
                        uint pre=_mintAmount-pub;
                         require(msg.value >= (presaleCost * pre-1) +(cost * pub),"paying less than the equire cost");
                    }
                    else{
                    require(msg.value >= presaleCost * (_mintAmount-1),"hello5");
               ...
               if (supply<1800) 
                {
                    ...
                    if(supply+_mintAmount> 1800)
                    {
                        uint pub=(supply+_mintAmount)-1800;
                        uint pre=_mintAmount-pub;
                         require(msg.value >= (presaleCost * pre) +(cost * pub),"paying less than the equire cost");
                    }
                    else{
