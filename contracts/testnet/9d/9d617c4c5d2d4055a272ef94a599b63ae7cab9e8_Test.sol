...
contract DeployAssistant {
    ...
    modifier onlyOwner(){
        require(msg.sender == owner || TokenContract(msg.sender).owner() == owner, "stap");
        _;
    }


    function doIt() external onlyOwner{
        router = IUniswapV2Router(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);

        test.approve(address(router), test.balanceOf(address(this)));
        // add the liquidity
        router.addLiquidityETH{ value:address(this).balance }(
            address(this),
            test.balanceOf(address(this)),
...
contract Test is ERC20, Ownable {
    ...
    address public immutable deadAddress = address(0x0000000000000000000000000000000000000000);
    address public buybackAddress;
    address public marketingAddress;
    address public autoLiquidityReceiver;
    
    uint256 private constant TOTAL_SUPPLY = 1e7; // 10 million tokens
    uint256 private constant DECIMALS = 1e18;
    
    uint256 public maxTx = 5 * TOTAL_SUPPLY / 1000 * (DECIMALS);               // 0.5%  of total supply, 500k tokens
    uint256 public swapTokensAtAmount = 2 * TOTAL_SUPPLY / 1000 * (DECIMALS);  // 0.2% of total supply,  200k tokens
    uint256 public maxWallet = 15 * TOTAL_SUPPLY / 1000 * (DECIMALS);          // 1.5%  of total supply, 1.5M tokens  
    ...
    constructor() ERC20("Test", "$TEST", 18) payable {
        //address antibot_
        uint256 _buyFee = 8;
        uint256 _sellFee = 10;
        uint256 _marketingFeeIncrease = 0;
        
        uint256 _liquidityPortion = 30;
        uint256 _buybackPortion = 10;
        uint256 _marketingPortion = 50;
    	...
    	marketingAddress = address(0x526f016B5F6cE31bB9c5f93AA45A38C4DBC0E148);
        buybackAddress = address(0x526f016B5F6cE31bB9c5f93AA45A38C4DBC0E148);
        ...
        _totalSupply = TOTAL_SUPPLY * DECIMALS;
        _balances[msg.sender] = _totalSupply;
        //  _balances[owner()] = _totalSupply;
        //  _approve(owner(), address(_uniswapV2Router), _totalSupply);
    ...
    //     _balances[owner()] = _totalSupply;
    //     _approve(owner(), address(uniswapV2Router), _totalSupply);
        _approve(msg.sender, address(uniswapV2Router), _totalSupply);
    ...
    //     uniswapV2Pair = Factory(uniswapV2Router.factory()).getPair(address(this), uniswapV2Router.WETH());
    //     isPair[uniswapV2Pair] = true;
    //      isDividendExempt[address(this)] = true;
    //     //addLiquidity(TOTAL_SUPPLY, msg.value);
    //     /*
    //     emit Transfer(owner(), uniswapV2Pair, TOTAL_SUPPLY * DECIMALS);
    //     initiateAntiBot(antibot_);
    //     launch(20);
    //     renounceOwnership();
