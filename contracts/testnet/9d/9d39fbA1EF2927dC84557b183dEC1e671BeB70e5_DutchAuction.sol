...
contract AuctionTime is Ownable, Random{
    ...
    uint public auctionStartTime = 1643544000;// 2022/1/30 20:00
    uint public auctionEndTime = 9999999999;
    uint public dutchAuctionTime = 1643544000;
...
contract DutchAuction is Pausable, AuctionTime, IMomoBear, ERC721{
    ...
    constructor() public ERC721("MomoBear", "DutchAuction") {
    }

    function getPriceForUSDT() public view returns(uint){
        uint newtime = now - dutchAuctionTime;
        uint offPrice = newtime / 60;
        if(offPrice > 20){
            offPrice = 20;
        }
        uint newPrice = _priceForUSDT - offPrice*10e18;
        if (newPrice < 100e18){
            newPrice = 100e18;
        }
        if (newPrice > 200e18){
    ...
    function winTheBidByUsdt(
        ...
        require(totalAuctionAmount() < 19, "Auction has ended");
        require(now >= dutchAuctionTime, "Auction has not started");
...
//onlyOwner
    ...
    function setDutchAuctionTime() private {
        dutchAuctionTime = now + 60;
    }
