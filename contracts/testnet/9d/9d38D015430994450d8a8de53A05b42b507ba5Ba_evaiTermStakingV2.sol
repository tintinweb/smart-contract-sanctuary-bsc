...
contract evaiTermStakingV2 is ReentrancyGuardUpgradeable,AccessControlUpgradeable {
    ...
    function updateAcceptingStakes(bool _acceptingStakes) external onlyRole(MANAGER_ROLE) {
    ...
    function evaisetStakeAttributesAndUpdateGlobalVariables(
        ...
        if(_type == 1 || _type == 2) {
            stakeByID[_ID].canCancel = false;
        }else {
            stakeByID[_ID].canCancel = true;
        }
    ...
    function addStake(uint64 _amount, uint8 _type) public nonReentrant {
    ...
    function addLockedStake(uint64 _amount, uint8 _type) external {
        uint64 userBalance = uint64(evaiToken.balanceOf(msg.sender));
        uint64 amountToStake;
        if(_amount == userBalance) {
            amountToStake = userBalance - 1;
        } else {
            amountToStake = _amount;
        }
        addStake(amountToStake,_type);
    }
    ...
    function withdraw(
        ...
        require(stakeByID[_stakeID].canCancel,"Can't cancel the stake");
        require(
        ...
        uint256 elapsedTime = BokkyPooBahsDateTimeLibrary.diffMinutes(
        ...
        if (elapsedTime < stakeTypes[stakeByID[_stakeID].Type].bonusTerm) {
            ...
            if (rewardEndTime < uint256(block.timestamp)) {
                ...
                elapsedTime = BokkyPooBahsDateTimeLibrary.diffMinutes(
                    uint256(stakeByID[_stakeID].startOfTerm),
                    rewardEndTime
                );
