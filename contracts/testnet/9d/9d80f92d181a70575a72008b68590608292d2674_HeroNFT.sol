...
contract HeroNFT is BaseNFT {
  ...
  bool public beforeTokenTransferLogicEnabled; // TODO remove next deployment or mainnet
  ...
  function updateTotalProfited(uint _tokenId, uint _totalProfited) onlyMintAdmin external {
    ...
    if (_totalProfited > maxProfitable) {
  ...
  function increaseTotalProfited(uint _tokenId, uint _amount) onlyMintAdmin external {
    Hero storage hero = heroes[_tokenId];
    uint maxProfitable = _getMaxProfitable(hero);
    if (hero.totalProfited + _amount >= maxProfitable) {
      hero.totalProfited = maxProfitable;
    } else {
      hero.totalProfited += _amount;
    }
    emit TotalProfitedUpdated(_tokenId, hero.totalProfited);
  }
  ...
  function resetLevel(uint _tokenId) onlyMintAdmin external {
    heroes[_tokenId].level = 0;
  }
}
