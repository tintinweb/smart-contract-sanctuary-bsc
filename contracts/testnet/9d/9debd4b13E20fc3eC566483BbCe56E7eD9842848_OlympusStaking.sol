...
contract OlympusStaking is OlympusAccessControlled {
    ...
    uint256 public earlyUnstakeTime = 7 days;
    /* ========== CONSTRUCTOR ========== */
    ...
    function stake(
        ...
        stakeTimes[_to] = block.timestamp;

        if (_claim && warmupPeriod == 0) {
            return _send(_to, newAmount, _rebasing);
            emit Staked(_to, newAmount);
        } else {
    ...
    function setEarlyUnstakeTime(uint256 _time) external onlyGovernor {
        earlyUnstakeTime = _time;
    }
}
