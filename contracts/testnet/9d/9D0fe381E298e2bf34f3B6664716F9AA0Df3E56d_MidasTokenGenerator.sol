...
import "./IMidasTokenGeneratorManager.sol";

contract MidasTokenGenerator is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function createNewToken(
        ...
        if (
            ...
            if (_isPayingWithNativeCurrency) {
                uint256 requiredEthAmount = uint256(getRequiredEthAmount());
    ...
    function getRequiredEthAmount() public view returns (uint256) {
        int256 nativeTokenPrice = getLatestPrice();

        return (creationTokenPrice / uint256(nativeTokenPrice)) * 1e18;
    }
    ...
    */
    ...
    function isPayingWithNativeCurrency(address tokenAddress)
        ...
        returns (bool)
    {
        return tokenAddress == pcsV2Router.WETH();
    }
    ...
    function swapBnbForUSD(uint256 _amount) private {
        ...
        uint256 amountOutIn = pcsV2Router.getAmountsOut(_amount, path)[1];

        // make the swap
        pcsV2Router.swapExactETHForTokens{value: _amount}(
            amountOutIn,
            path,
            address(this),
