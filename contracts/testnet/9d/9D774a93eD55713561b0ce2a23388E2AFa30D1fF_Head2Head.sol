...
contract Head2Head is AccessControl, Pausable {
        ...
        whenNotPaused
    {
        ...
        if (totalBetsPooledInUsd > curUpdateMulAtAmountInWei) {
            ...
            curUpdateMulAtAmountInWei = ((totalBetsPooledInUsd / updateMulAfterAmountInWei) + 1) *
                updateMulAfterAmountInWei;
            head2headGames[_gameId].curUpdateMulAtAmountInWei = curUpdateMulAtAmountInWei;
                
        }
        // EMIT BET PLACED EVENT FOR GAME ID, BETTER, STOCK ID, BET AMOUNT
        uint256 index = bets[_gameId][_msgSender()].length - 1; 
        emit Head2HeadBetPlaced(_gameId, _msgSender(), index, _stockId, betAmount, curUpdateMulAtAmountInWei);
    }
            ...
            for (uint256 j = 0; j < numOfBetsPlaced; ++j) {
                if (
                    ...
                    if(win){
                        ...
                        if(temp_winningBetsIndexes.length < temp_winners.length)
                        temp_winningBetsIndexes.push([j]);
                        else
                        temp_winningBetsIndexes[temp_winners.length-1].push(j);
