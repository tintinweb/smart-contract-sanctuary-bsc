...
contract DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("TUSA_Dividend_Tracker", "TUSA_Dividend_Tracker") {
        claimWait = 86400;
        minimumTokenBalanceForDividends = totalSupply() / 10000000; //must hold some tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "TUSA_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "TUSA_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SP contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "TUSA_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
...
contract TestUSA is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    function setbuySellTax(uint256 _buySellTax) public onlyOwner {
        buySellTax = _buySellTax; // extra fee on sell
    }
    ...
    constructor(uint256 initialSupply, address _owner, address _charityWallet, address _router) ERC20("TestUSA", "TUSA") {
        BNBRewardsFee = 2;
        liquidityFee = 1;
        buySellTax = 5; // extra fee on sell
        charityFee = 2;
        charityWallet = payable(_charityWallet);
	    maxSellTransactionAmount = initialSupply * (10**9) / 100000;
    	swapTokensAtAmount = initialSupply * (10**9) / 50000;
    ...
    function setcharityWallet(address payable newcharityWallet) public onlyOwner {
         if (_isExcludedFromMaxTx[charityWallet] = true) _isExcludedFromMaxTx[charityWallet] = false;
         if (_isExcludedFromFees[charityWallet] = true) excludeFromFees(charityWallet, false);
        charityWallet = newcharityWallet;
         if (_isExcludedFromMaxTx[charityWallet] = false) _isExcludedFromMaxTx[charityWallet] = true;
         if (_isExcludedFromFees[charityWallet] = false) excludeFromFees(charityWallet, true);
    }
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(automatedMarketMakerPairs[to]) {
                extraFee =(amount*buySellTax)/100;
                fees=fees+extraFee;
            }
        	amount = amount-fees;
