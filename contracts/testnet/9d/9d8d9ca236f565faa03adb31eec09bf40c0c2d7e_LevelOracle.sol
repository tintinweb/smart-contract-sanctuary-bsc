...
contract LevelOracle is Ownable, ILevelOracle {
    ...
    function _postPrice(address token, uint256 price) internal {
        ...
        uint256 selectedPrice = normalizedPrice;
        (, uint256 lowerBound, uint256 upperBound,,) = _getReferencePrice(token);

        if (normalizedPrice < lowerBound) {
            selectedPrice = lowerBound;
        } else if (normalizedPrice > upperBound) {
            selectedPrice = upperBound;
        }
        lastAnswers[token] = selectedPrice;
        lastAnswerTimestamp[token] = block.timestamp;
        lastAnswerBlock[token] = block.number;
        emit PricePosted(token, selectedPrice);
    }
