...
contract Efatah33 is IERC20 {
    ...
    string private _name = "EFATAH33_5";
    string private _symbol =  "EFALOCK_5";
 ...
 */
    ...
    //     else { //its BUSD
    //        // try 
    //         _router.addLiquidity(address(this), busdToken, tokenAmount, bnbUsdAmount,  0, 0, liquidityReceiver, block.timestamp);// { 
    //             isSuccess = true;
    //        // } catch (bytes memory) {   } 
    //     }
    //     return isSuccess;
   ...
   function addLiquidity() internal {
    ...
    function _addLiquidityBUSD() internal swapping {
        ...
        if(efaLiquid == 0) {
            emit evtLiqError(false, "efaLiquid == 0");
            return;
        }
        ...
        if(efaToSwap == 0 || efaToLiquify == 0) {
            emit evtLiqError(false, "efaToSwap/efaToLiquify == 0");
            return;
        }
        ...
        if(busdToLiquify > 0){
            bool success = _addLiquidityOfBUSD(efaToLiquify, busdToLiquify, address(this));
            if(success){ 
                emit evtLiqError(success, "passed address(this)");
            }
            else {
                success = _addLiquidityOfBUSD(efaToLiquify, busdToLiquify, liquidityReceiver);
                emit evtLiqError(success, "passed liquidityReceiver");
            } 
        }
        else{
             emit evtLiqError(true, "no error, busdToSwap is Zero");
        }
       /* get balance from liquidreceiver and sum to contract efa
          split into 2 parts
          use first part and swaptousd, 
          provide liquidity
        */ 
    }

    function _addLiquidityOfBUSD(uint256 tokenAmount, uint256 busdAmount, address toAddress) private returns (bool) { 
        bool issuccess = false;
