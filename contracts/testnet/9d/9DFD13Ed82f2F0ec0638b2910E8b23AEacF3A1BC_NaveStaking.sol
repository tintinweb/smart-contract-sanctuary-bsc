...
contract NaveStaking is Ownable, ReentrancyGuard {
    ...
    */
    ...
    /**
    * @notice The last time of reward distribution across all stakeholders.
    */
    ...
    */
    function distributeRewards(uint256 _amount) external onlyOwner {
        require(_amount > 0, "Cannot distribute 0");
        require(totalStaked > 0, "No stakeholders");
        lastRewardUpdateTime = block.timestamp;
        for (uint256 i = 0; i < stakeholders.length; i += 1){
            address stakeholder = stakeholders[i];
            // calculate reward
            uint256 reward = _amount.mul(balances[stakeholder]).div(totalStaked);
