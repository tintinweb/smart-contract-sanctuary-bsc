...
contract BusinessParcel is Parcel, IBusinessParcel {
    ...
    function assignNFT(
        ...
        require(
            ...
            "BusinessParcel:  Parcel slots areis occupied"
        );
        if (_publicBuildingType == PublicBuildings.Aesthetic) {
            parcelStakes[parcelId].aestheticNFTTokenIds.push(_tokenId);
        } else {
            require(
                parcelStakes[parcelId].branch == 0,
                "BusinessParcel: Slot of the building  occupied"
            );
            parcelStakes[parcelId].branch = _tokenId;
            parcelStakes[parcelId]._type = _publicBuildingType;
        }
        parcelStakes[parcelId].occupied = true;
        buildingTypeAndTokenIdToParcelId[_publicBuildingType][
            _tokenId
        ] = parcelId;
    ...
    function unassignNFT(
        ...
        require(
            buildingTypeAndTokenIdToParcelId[_publicBuildingType][_tokenId] ==
                parcelId,
            "BusinessParcel: Buildng  not found"
        );
        if (_publicBuildingType == PublicBuildings.Aesthetic) {
            uint256 aestheticNFTsCount = parcelStakes[parcelId]
                .aestheticNFTTokenIds
                .length;
            require(
                aestheticNFTsCount != 0,
                "BusinessParcel: Aesthetic NFTs not found on this parcel"
            );
            for (uint256 i; i < aestheticNFTsCount; i++) {
                if (
                    parcelStakes[parcelId].aestheticNFTTokenIds[i] == _tokenId
                ) {
                    parcelStakes[parcelId].aestheticNFTTokenIds[
                        i
                    ] = parcelStakes[parcelId].aestheticNFTTokenIds[
                        aestheticNFTsCount - 1
                    ];
                    parcelStakes[parcelId].aestheticNFTTokenIds.pop();
                    break;
                }
            }
        } else {
            require(
                parcelStakes[parcelId].branch == _tokenId,
                "BusinessParcel: This building NFT is not assigned to this parcel"
            );
            parcelStakes[parcelId].branch = 0;
        }
        parcelStakes[parcelId].occupied = false;
    ...
    function isEmpty(uint256 _parcelId) private view returns (bool) {
        PublicBuildingsBranches memory parcel = parcelStakes[_parcelId];
        return (parcel.branch == 0 && parcel.aestheticNFTTokenIds.length == 0);
    }
}
...
contract Parcel is ERC721Base, OwnableUpgradeable, IParcel {
    ...
    function mintAdjacentQuadraticParcels(
        ...
        for (uint256 i; i <= _size**2; i++) {
            if (i % _size == 0) {
                currentId += firstId + maxCoordinate * i / _size;
            }
            _mintFor(currentId, _beneficiary);
            allParcels[currentId] = _parcel;
            currentId++;
            emit ParcelMinted(currentId, _beneficiary);
        }
    }
