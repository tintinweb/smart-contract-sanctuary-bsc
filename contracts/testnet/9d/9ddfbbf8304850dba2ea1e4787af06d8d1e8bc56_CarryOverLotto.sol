...
interface INormalLotto {
    ...
    function getAsset(string calldata symbol) external view returns (string memory, address);
}

contract CarryOverLotto is Ownable {
    ...
    function openSeri(
        ...
        uint256 _postPrice,
        uint256 _max2sale,
        address[] memory _initAssets,
        uint256[] memory _initPrizes
    ) public onlyOwner {
        ...
        for (uint256 i = 0; i < series[_seri].assetIndex.length; i++) {
            if (series[_seri].seriAssetRemain[series[_seri].assetIndex[i]] > 0) {
                ...
                if (series[_seri].assetIndex[i] == 0) carryOver.transfer(takeAmount);
                    ...
                    (, address _asset) = INormalLotto(normalLotto).getAsset(_symbol);
    ...
    function config(uint256 _expiredPeriod) external onlyOwner {
        expiredPeriod = _expiredPeriod;
    }

    function configAffiliate(address payable[] calldata shareAddress_, uint256[] calldata sharePercents_) external onlyOwner {
