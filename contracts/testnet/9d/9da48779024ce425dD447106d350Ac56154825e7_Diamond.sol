...
contract Diamond is ERC20, Ownable {
    ...
    uint256 public totalEthRewardBalance;
    ...
    function takeFees(
        ...
        if (automatedMarketMakerPairs[_from]) {
            ...
            if (tokensToSwap > minContractTokensToSwap) {
                ...
                distributeTokensEth(
                    tokensToSwap,
                    rewardTokens.add(pendingTokensForReward)
                );
            } else if (totalSellTax > baseSellTax) {
                pendingTokensForReward = pendingTokensForReward.add(
                    rewardTokens
                );
            }
    ...
    function distributeTokensEth(uint256 _tokenAmount, uint256 _rewardTokens)
        private
    {
        uint256 totalPercentage = 100;
        uint256 rewardPercentage = 0;
        if (_rewardTokens > 0) {
            rewardPercentage = _rewardTokens.div(_tokenAmount).mul(100);
            totalPercentage = totalPercentage.add(totalPercentage);
        }

        uint256 tokensForLiquidity = _tokenAmount.mul(autoLP).div(
            totalPercentage
        );
        ...
        uint256 ethForAddLP = totalEth.mul(autoLP).div(totalPercentage);
        uint256 devFeesToSend = totalEth.mul(devFee).div(totalPercentage);
        uint256 marketingFeesToSend = totalEth.mul(marketingFee).div(
            totalPercentage
        );

        uint256 rewardEth = totalEth.mul(rewardPercentage).div(totalPercentage);
        ...
        if (rewardEth > 0) {
            totalEthRewardBalance = totalEthRewardBalance.add(rewardEth);
            emit AddRewardPool(rewardEth);
        }
    }
