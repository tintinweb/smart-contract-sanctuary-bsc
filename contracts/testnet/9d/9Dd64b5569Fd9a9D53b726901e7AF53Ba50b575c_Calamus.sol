...
contract Calamus is Initializable, OwnableUpgradeable, ICalamus, IPlatformFee, ReentrancyGuardUpgradeable, CarefulMath, PausableUpgradeable {
    ...
    function _validateRecipient(
        ...
        require(recipient.recipient != addressZero, "stream to the zero address");
        require(recipient.recipient != addressThis, "stream to the contract itself");
        require(recipient.recipient != sender, "stream to the caller");
        require(recipient.releaseAmount > 0, "deposit is zero");
        require(
            (userTokenBalance[msg.sender][correctTokenAddress] - lockedUserTokenBalance[msg.sender][correctTokenAddress]) >= recipient.releaseAmount,
            "Available token balance < release amount"
        );
    }

    function _validateSingleStream(
        Types.StreamRequest memory streamRequest,
        ...
        Types.StreamGeneral memory generalInfo = Types.StreamGeneral(
            streamRequest.tokenAddress,
            streamRequest.startTime,
            streamRequest.stopTime,
            streamRequest.vestingRelease,
            streamRequest.releaseFrequency,
            streamRequest.transferPrivilege,
            streamRequest.cancelPrivilege
        );
        ...
        Types.Recipient memory recipient = Types.Recipient(
            streamRequest.recipient,
            streamRequest.releaseAmount
        );
        _validateRecipient(
            recipient,
            correctTokenAddress,
    ...
    function _createSingleStream(
        Types.StreamRequest memory streamRequest,
        address correctTokenAddress,
        CreateStreamLocalVars memory vars
    ) internal returns(Types.Stream memory) {
        uint256 releaseAmountAfterFee = _getReleaseAmountAfterFee(
            streamRequest.tokenAddress,
            streamRequest.releaseAmount
        );
        (vars.mathErr, vars.vestingAmount) = mulUInt(releaseAmountAfterFee, streamRequest.vestingRelease);

        assert(vars.mathErr == MathError.NO_ERROR);

        vars.vestingAmount /= DENOMINATOR;

        contractFees[correctTokenAddress] += streamRequest.releaseAmount - releaseAmountAfterFee;
        ...
        recipientToStreams[streamRequest.recipient] +=1;
        /* Increment the next stream id. */
        nextStreamId += 1;
        lockedUserTokenBalance[msg.sender][correctTokenAddress] += streamRequest.releaseAmount;
        return stream;
    }
    ...
    function _createBatchStreams(Types.StreamGeneral memory generalInfo, Types.Recipient[] memory recipients) internal {
        ...
        for (uint i=0; i < recipients.length; i++) {
            recipient = recipients[i];
            releaseAmountAfterFee = _getReleaseAmountAfterFee(
                generalInfo.tokenAddress,
                recipient.releaseAmount
            );
            (vars.mathErr, vars.vestingAmount) = mulUInt(releaseAmountAfterFee, generalInfo.vestingRelease);

            assert(vars.mathErr == MathError.NO_ERROR);

            vars.vestingAmount /= DENOMINATOR;

            contractFees[correctTokenAddress] += recipient.releaseAmount - releaseAmountAfterFee;
            stream = Types.Stream(
                nextStreamId + i,
                sender,
                releaseAmountAfterFee - vars.vestingAmount,
            ...
            lockedUserTokenBalance[sender][correctTokenAddress] += recipient.releaseAmount;
    ...
    function _getReleaseAmountAfterFee(address tokenAddress, uint256 releaseAmount) internal view returns (uint256) {
    ...
    function createStream(
        ...
        address correctTokenAddress = (streamRequest.tokenAddress == address(this)) ? address(0) : streamRequest.tokenAddress;
        CreateStreamLocalVars memory vars;
        uint256 blockTimestamp = block.timestamp;
        address addressZero = address(0);
        address addressThis = address(this);
        address sender = msg.sender;
        _validateSingleStream(
            streamRequest,
            vars,
            correctTokenAddress,
            blockTimestamp,
        ...
        Types.Stream memory stream  = _createSingleStream(streamRequest, correctTokenAddress, vars);
        emit CreateStream(
            stream.streamId,
            stream.sender,
            stream.recipient,
    ...
    streamExists(streamId)
    {
        ...
        uint256 amountIncludedFee = _getAmountIncludedFee(stream.sender, stream.tokenAddress, amount);
        userTokenBalance[stream.sender][stream.tokenAddress] -= amountIncludedFee;
        lockedUserTokenBalance[stream.sender][stream.tokenAddress] -= amountIncludedFee;
    ...
    streamExists(streamId)
    {
        ...
        if (recipientBalance > 0) {
            ...
            uint256 recipientBalanceIncludedFee = _getAmountIncludedFee(stream.sender, stream.tokenAddress, recipientBalance);
            uint256 remainingBalanceIncludeFee = _getAmountIncludedFee(stream.sender, stream.tokenAddress, stream.remainingBalance);

            MathError mathErr;
            (mathErr, streams[streamId].remainingBalance) = subUInt(stream.remainingBalance, recipientBalance);
            assert(mathErr == MathError.NO_ERROR);

            userTokenBalance[stream.sender][stream.tokenAddress] -= recipientBalanceIncludedFee;
            lockedUserTokenBalance[stream.sender][stream.tokenAddress] -= remainingBalanceIncludeFee;
    ...
    streamExists(streamId) {
        ...
        if (recipientBalance > 0) {
            ...
            uint256 amountIncludedFee = _getAmountIncludedFee(stream.sender, stream.tokenAddress, recipientBalance);
            userTokenBalance[stream.sender][stream.tokenAddress] -= amountIncludedFee;
            lockedUserTokenBalance[stream.sender][stream.tokenAddress] -= amountIncludedFee;
    ...
    function withdrawFromBalance(address tokenAddress, uint256 amount) external whenNotPaused nonReentrant {
        address correctTokenAddress = (tokenAddress == address(this)) ? address(0) : tokenAddress;
        require(userTokenBalance[msg.sender][correctTokenAddress] > 0, "user token balance = 0");
    ...
    function _getDueDateStreams() internal view returns (Types.Stream[] memory) {
        uint256 countDueStreams = 0;
        for (uint i=0; i < nextStreamId; i++) {
            if (streams[i].status == 1 && streams[i].stopTime < block.timestamp && streams[i].remainingBalance > 0 ) {
                countDueStreams += 1;
            }
        }
        Types.Stream[] memory dueStreams = new Types.Stream[](countDueStreams);
        uint256 count = 0;
        for (uint i=0; i < nextStreamId; i++) {
            if (streams[i].status == 1 && streams[i].stopTime < block.timestamp && streams[i].remainingBalance > 0 ) {
                dueStreams[count] = streams[i];
                count += 1;
            }
        }
    ...
    function doAutoWithdraw() external {
        require(msg.sender == systemAddress, "!system address");
        Types.Stream[] memory dueStreams = _getDueDateStreams();
        if (dueStreams.length > 0) {
            uint256[] memory streamIds = new uint256[](dueStreams.length);
            Types.Stream memory stream;
            uint256 amountIncludedFee;
            uint256 count = 0;
            for(uint i=0; i < dueStreams.length; i++) {
                stream = dueStreams[i];
                amountIncludedFee = _getAmountIncludedFee(stream.sender, stream.tokenAddress, stream.remainingBalance);
                if (userTokenBalance[stream.sender][stream.tokenAddress] >= amountIncludedFee && lockedUserTokenBalance[stream.sender][stream.tokenAddress] >= amountIncludedFee) {
                    userTokenBalance[stream.sender][stream.tokenAddress] -= amountIncludedFee;
                    lockedUserTokenBalance[stream.sender][stream.tokenAddress] -= amountIncludedFee;
                    streams[stream.streamId].remainingBalance = 0;
                    streams[stream.streamId].status = 3;
                    streamIds[count] = stream.streamId;
                    count += 1;
                    if (stream.tokenAddress != address(0x00)) {
    ...
    function setSystemAddress(address newSystemAddress) external onlyOwner {
        require(systemAddress != newSystemAddress, "no different system address");
