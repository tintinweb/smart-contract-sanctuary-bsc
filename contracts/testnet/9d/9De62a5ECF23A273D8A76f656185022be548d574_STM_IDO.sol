...
contract STM_IDO {
    ...
    address  feeReceivers1 ; //

    address  feeReceivers2 ; // 暂时这样
    ...
    constructor(address _fee1,address _fee2) public {//0xa3346b35fef87829d13922de39fa89254adcdd8d   ,0xF3559Bd49B5D8de03a07342110F4E22994f514aE
        feeReceivers1  =  _fee1 ;
        feeReceivers2  = _fee2 ;
        _owner = msg.sender;
    }
  ...
  function buyShare(address _user,uint256 _amount) external{
        ...
        usdt.transferFrom(msg.sender,feeReceivers1,_amount.sub(supAmount));
  ...
  function buyNode(address _user,uint256 _amount) external{
       ...
       // usdt.transferFrom(msg.sender,address(this),_amount);
        ...
        user.nodeTime = block.timestamp; 
         (uint256 supAmount) =    _distributeReward(_user,_amount,3);
          usdt.transferFrom(msg.sender,feeReceivers2,_amount.sub(supAmount));
         ...
         for(uint256 i = 0; i < _referDepth; i++){
           ...
           RewardInfo storage upReward = rewardInfo[upline];   
              uint256 profit =   _amount.mul(5).div(100);
                supAmount += profit;
                if(_referDepth==2 && i==0){ //直推
                   upUser.totalRevenue += profit;
                   upReward.directs += profit; 
                }else if(_referDepth==2 && i==1){//间推
                   upUser.totalRevenue += profit;
                   upReward.interval += profit; 
                }else{//节点
                    upUser.totalRevenue += profit;
                    upReward.node += profit; 
                }
               // usdt.approve(address(this),profit);  
                usdt.transferFrom(msg.sender,upline,profit);          
