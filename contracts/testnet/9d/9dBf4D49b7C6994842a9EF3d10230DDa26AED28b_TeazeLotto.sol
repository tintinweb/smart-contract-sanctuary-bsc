...
contract TeazeLotto is Ownable, Authorized, ReentrancyGuard {
    ...
    mapping(address => uint256) public lastSpinValue; 
    mapping(address => uint256) public lastSpinSBX; 
    mapping(address => bool) public lastSpinJackpot; 
    mapping(address => uint256) public lastSpinJackpotAmt; 
    ...
    event SpinResult(uint roll, uint userReward, bool jackpotWinner);
    ...
    function SpinSimpWheel() external payable nonReentrant {
       ...
       // require(staked, "User must be staked to spin the SimpWheel of Fortune");
        ...
        lastSpinValue[_msgSender()] = roll;

        uint256 userReward = 0;
        bool jackpotWinner = false;
        lastSpinJackpot[_msgSender()] = false;
        ...
        if(roll != winningNumber && !adminWinner) {
            ...
            if (roll > 499) { //winning of some kind
                ...
                lastSpinSBX[_msgSender()] = userReward;
            ...
            lastSpinJackpot[_msgSender()] = true;
            lastSpinJackpotAmt[_msgSender()] = netamount;
        ...
        emit SpinResult(roll, userReward, jackpotWinner);
    ...
    function getLastSpinResult(address _recipient) external view returns (uint, uint, bool) {
        return (lastSpinValue[_recipient], lastSpinSBX[_recipient],lastSpinJackpot[_recipient]);
    }
