...
contract THTTokenSale is ReentrancyGuard, Context, Ownable {
    ...
    //Start Presale
    function startPresaleICO(address token) external onlyOwner {
        availableTokensICO = _token.balanceOf(token);
        PreSaleActive = true;
    }
    ...
    function buyTokens(uint256 tokensAmount) public nonReentrant payable {
        ...
        availableTokensICO = availableTokensICO - tokensAmount * (uint256(10) ** _tokenDecimals);
    ...
    function otherPayments(address account) public onlyOwner {
        ...
        _unlockTime[account] = block.timestamp + 2629743;
    }

    function getPart(address account) private {
        _part[account] = _rewards[account]/16;
    }
