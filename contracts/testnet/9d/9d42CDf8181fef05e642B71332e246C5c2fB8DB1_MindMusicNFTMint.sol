...
contract MindMusicNFTMint is ERC721Enumerable, Ownable {
    ...
    uint256 public MINT_PRICE = 0.0003 ether; // Public mint price
    uint256 public MAX_TOTAL_SUPPLY = 10000; // The maximum total supply of tokens
    uint256 public rand_seed = 1000;
    ...
    constructor(string memory _baseUri, address _token) ERC721("MindMusic NFT", "MindMusicNFT") {
    ...
    function changeRandSeed(uint256 _seed) public onlyOwner {
        rand_seed = _seed;
    }
    ...
    function createItem(uint256 _amount) external payable {
        ...
        if (flag == true && externalTokenNeeded == true) {
        ...
        if (supply >= 667) {
            MINT_PRICE = 0.0005 ether;
        } else if (supply >= 333 && supply < 667) {
            MINT_PRICE = 0.0004 ether;
        }
        ...
        uint256 i = 0; 
        do  {
            id = uint256(keccak256(abi.encodePacked(block.timestamp, i))) % rand_seed;
            id = id +  1;
            if (isMint[id] == false) {
