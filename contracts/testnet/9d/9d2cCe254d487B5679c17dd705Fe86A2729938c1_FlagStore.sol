...
contract FlagStore is ReentrancyGuard {
    TetCTFToken public immutable token;

    uint public flagPrice  = 133737 * 1e18;
    mapping (address => bool) public flagClaimed;
    ...
    function claimFlag() external returns (bool) {
        require(token.balanceOf(msg.sender) >= flagPrice, "Insufficient balance");

        token.burn(msg.sender, token.balanceOf(msg.sender));
        flagClaimed[msg.sender] = true;
