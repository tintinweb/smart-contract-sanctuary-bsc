...
contract BurnReward9c is Ownable, ReentrancyGuard {
    ...
    constructor(
        FeedRegistryInterface _ref,
        IERC20 _crank ) {
        ref = _ref;
        crank = _crank;
        crank.balanceOf( address(this));
    }
    ...
    function execute() external payable nonReentrant returns (uint256 amount) {
        require(crank.balanceOf(address(msg.sender)) >= 100 * 1000 * 10 ** 18 , 'CRANK token balance is too low');
        uint256 _amount = msg.value;
        require(_amount > 0, "Amount must be greater than zero");

        //determine amount of tokens
        int256 _rate  = ref.latestAnswerByName(
            "BNB",
            "USD"
        );
