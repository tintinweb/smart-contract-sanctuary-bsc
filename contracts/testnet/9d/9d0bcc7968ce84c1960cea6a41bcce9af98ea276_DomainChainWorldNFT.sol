...
contract DomainChainWorldNFT is ERC721URIStorage, Ownable {
    ...
    event GetNFT(address indexed user, uint256[] tokenIds, uint256[] types);
    ...
    mapping(address => uint256[]) public userNFTTypeNumber;

    mapping(uint256 => uint256) public NFTTypeNumber;

    mapping(uint256 => bool) public isUnlock;

    // uint256 public boxMaxTimes = 10000;

    uint256 public boxPrice = 1000;

    uint256[6] public rate = [250,500,750,900,975,999];
    ...
    address USDT = 0xFd2464406Cc3d9585e08DF545e793838DF697470;
    ...
    function queryUserNFTs(address user) public view returns(uint256[] memory) {
        return userNFTs[user];
    }

    function queryUserNFTTypeNumber(address user) public view returns(uint256[] memory) {
        return userNFTTypeNumber[user];
    }

    function setBoxsRate(uint256 num1, uint256 num2, uint256 num3, uint256 num4, uint256 num5, uint256 num6) public onlyOwner {
        rate[0] = num1;
        rate[1] = num1 + num2;
        rate[2] = num1 + num2 + num3;
        rate[3] = num1 + num2 + num3 + num4;
        rate[4] = num1 + num2 + num3 + num4 + num5;
        rate[5] = num1 + num2 + num3 + num4 + num5 + num6;
    }
    ...
    function checkTotalLeft() public view returns(uint256 totalLeft) {
        for(uint256 i = 0; i < numberLimit.length; i ++){       
            totalLeft += numberLimit[i];
        }
    }
    ...
    function openBlindBox(uint256 times) public {
        ...
        uint256 cost = boxPrice * times * 10 ** decimals;

        //钱转钱包地址,不然咋提取?
        ERC20(USDT).transferFrom(msg.sender, address(this), cost); //USDT精度????

        // boxMaxTimes -= 1;//是否需要限制次数

        uint256[] memory tokenIds;
        uint256[] memory typess;

        // require(times <= checkTotalLeft(),"Presale is not open yet");
        ...
        for(uint256 i = 0; i < times; i ++){
            ...
            // do  {
            //         types = _getNFT();
            //     } while (NFTTypeNumber[types] == numberLimit[types - 1]);

            typess[i] = types;

            tokenIds[i] = _createNFT(types);
            
        }

        // emit GetNFT(msg.sender, tokenIds, typess);
    }

    function _createNFT(uint256 NFTType) private returns (uint256){      
        
        if(numberLimit[NFTType] > 0){
            require(NFTTypeNumber[NFTType] < numberLimit[NFTType - 1], "Number limit exceeded");
        }
    ...
    //     if(numberLimit[NFTType] > 0){
    //         require(NFTTypeNumber[NFTType] < numberLimit[NFTType], "Number limit exceeded");
    //     }
