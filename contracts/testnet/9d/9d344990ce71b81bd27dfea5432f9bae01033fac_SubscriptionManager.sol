...
contract SubscriptionManager is ERC721Upgradeable, OwnableUpgradeable {
    ...
    )
  {
    ...
    referrerFee =
      (authorFee * userRefDataByAuthor[subscriber][author].referrerShare) /
      DENOMINATOR;
  ...
  function distributeFees(
    ...
    if (author != owner() && authorFee > 0) {
      token.transferFrom(subscriber, author, authorFee);
    }

    if (author == owner()) adminFee += authorFee;

    if (adminFee > 0) token.transferFrom(subscriber, owner(), adminFee);
  ...
  function upgradeSubscription(
    ...
    if (userSubscriptionsList.length <= upgradeLevel) {
      for (
    ...
    require(
      upgradeLevel < subscriptionLevels[author].length &&
    ...
    uint256 newCost = (subscriptionLevels[author][upgradeLevel].price *
      upgradePeriod *
      upgradeAmount);
    uint256 upgradeCost = costTillExpiration > newCost
      ? 0
      : newCost - costTillExpiration;
    ...
    userSubscriptionsList[subscription.level].remove(subscription.id);
    ...
    userSubscriptionsList[upgradeLevel].insertSorted(
  ...
  function setSubscriptionLevel(
    ...
    if (
      authorInvitedBy != address(0) &&
      userRefDataByAuthor[msg.sender][owner()].invitedBy == address(0) &&
      _authorSubscriptionLevels.length == 0
    ) {
      userRefDataByAuthor[msg.sender][owner()].invitedBy = authorInvitedBy;
    }
  ...
  function _beforeTokenTransfer(
    ...
    if (from != address(0)) {
      ...
      if (to != address(0)) require(author != owner(), "Forbidden");
