...
contract Bet is Ownable {
  ...
  address public immutable coliseums;
  ...
  constructor(IERC20 _token, address _coliseums) {
    token = _token;
   ...
   * present (address = 0x0), fee goes to team and coliseum owners.
   */
  function depositPayment(uint256 amount, address referrer) external {
    uint256 fee = amount / 100; // 1% fee
    referrerBalance[referrer] += fee;
    token.transferFrom(msg.sender, address(this), amount);
    emit NewDeposit(msg.sender, amount - fee);
  }

  /**
   * @dev Lets users claim a payment using an authorized `signature`.
   *
   ...
   */
  function claimPayment(uint256 amount, uint256 nonce, bytes calldata signature) external {
  ...
  /**
   * @dev Sends collected fees to contract owner and Coliseums contract, including fight fees (`teamFees` and `coliseumFees`) passed from server (caller).
   *
   * See {depositPayment}.
   */
  function withdrawFees(uint256 teamFees, uint256 coliseumFees) external onlyOwner {
    uint256 coliseumAndTeamBalance = referrerBalance[address(0)];
    uint256 coliseumShares = coliseumAndTeamBalance / 2 + coliseumFees;
    uint256 teamShares = coliseumAndTeamBalance / 2 + teamFees;
