...
abstract contract baseToken is IERC20, Ownable {
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint amountBNBreward = amountBNB.mul(allot.liquidity).div(totalBNBFee).div(2);
        uint fundAmount = amountBNB.sub(amountBNBLiquidity).sub(amountBNBreward);
        bool tmpSuccess;
        if(fundAmount>0){
