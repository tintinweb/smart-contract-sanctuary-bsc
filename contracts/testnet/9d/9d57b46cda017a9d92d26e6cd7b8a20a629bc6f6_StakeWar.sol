...
interface IMasterChef {
    function StakeWarDistribute(address winerUser, address loserUser, uint256 profit) external;
    function userStakeTokenId(address owner) external view returns(uint256 tokenId);
    function minimalPower() external view returns(uint256);
    ...
    function poolInfo(uint256 idx) external view returns(PoolInfo memory);
}
...
contract StakeWar is Initializable, OwnableUpgradeable, Member {
    ...
    event Fight(address attcker, uint256 attckerTokenId, uint256 ultimatePowerA,  uint256 attckerOriginPower, address defender, uint256 defenderTokenId, uint256 ultimatePowerB, uint256 defenderOriginPower, bool result, uint256 rate, uint256 profit, uint256 chance);
    ...
    function pushEthnicityNFT(uint256 tokenId) external onlyNftPool {
        ...
        if(nftAttr.level != 0){
            ...
            // 總 total
            totalPower = totalPower.add(nftAttr.power);
            totalLevel = totalLevel.add(nftAttr.level);
        }
    }
    ...
    function removeEthnicityNFT(uint256 tokenId) external onlyNftPool {
        ...
        if(nftAttr.level != 0){
            ...
            // 總 total
            totalPower = totalPower.sub(nftAttr.power);
            totalLevel = totalLevel.sub(nftAttr.level);
        }
    }

    function compareRace(uint8 ethnicityA,uint8 ethnicityB) public pure returns(bool) {
    ...
    // rate 傳入時已算到小數掉 2 位
    function calcProfit(uint256 rate) internal pure returns(uint256) {
        require(rate >= 500 && rate <= 9500, "calcProfit rate notcorrect");
        uint256 winrate_min = 500;
        uint256 winrate_max = 9500;
        uint256 profit_min = 500;
        uint256 profit_max = 2000;

        uint256 _rangeWinrate = winrate_max.sub(winrate_min); // 90 
        uint256 _rangeProfit = profit_max.sub(profit_min); // 15
        uint256 _profitRatio = _rangeWinrate.div(_rangeProfit); // 6
        
        uint256 _realRange = rate.sub(winrate_min);
        uint256 _payout = profit_max.mul(100).sub(_realRange.mul(100).div(_profitRatio));
    ...
    function fight() public {
        require(manager.members("NFTStakePool") != address(0), "NFTStakePool address need to set");
        bool isOverMinimalPower = IMasterChef(manager.members("NFTStakePool")).poolInfo(0).lpSupply > IMasterChef(manager.members("NFTStakePool")).minimalPower();
        require(isOverMinimalPower || IMasterChef(manager.members("NFTStakePool")).poolInfo(0).accSushiPerShare > 0, "war not start");
        ...
        // 尋找其他種族
        uint8 findEthnicity = findOpponent(ethnicity);
        ...
        // 計算平均算力（含加成）
        (uint256 originPowerA, uint256 ultimatePowerA, uint256 originPowerB, uint256 ultimatePowerB) = calculatePlayerPower(nftAttr_1, nftAttr_2);

        // 判斷輸贏
        address originB = tokenIdToStaker[tokenIdB];        
        uint256 _tokenIdB = tokenIdB;
        (bool isWinner, uint256 profit, uint256 randomSeed, uint256 chance) = referee(ultimatePowerA, ultimatePowerB, originB);
        
        // randomSeed 1- 10000
        // profit 50000 - 200000(5%-20%)
        emit Fight(msg.sender, IMasterChef(manager.members("NFTStakePool")).userStakeTokenId(msg.sender), ultimatePowerA, originPowerA, originB, _tokenIdB, ultimatePowerB, originPowerB, isWinner, randomSeed, profit, chance);
    }
    ...
    // 尋找對手
    function findOpponent(uint8 ethnicity) internal returns(uint8 findEthnicity) {
        ...
        return findEthnicity;
    }


    function calcBattle(uint256 attacker, uint256 defender) internal pure returns(uint256) {
        uint256 check_battle = (attacker.sub(defender)).mul(10000).div(defender).mul(50).div(100);
        check_battle = check_battle.add(5000);
        if (check_battle >= 9500) return 9500;
        return check_battle;
    }

    function realBattle(uint256 attacker, uint256 defender) internal returns(bool, uint256, uint256) {
        uint256 battleChance = calcBattle(attacker, defender);
        uint256 _randomNum = _random();
        // p 隨機數 1-100.00%(一樣到小數點後兩位)
        uint256 randomSeed = _randomNum % (10000 + 1);
        if(randomSeed < battleChance) {
            return (true, battleChance, randomSeed);
        } else {
            uint256 _all = 10000;
            return (false, _all.sub(battleChance), randomSeed);
        }
    }

    function referee(uint256 ultimatePowerA, uint256 ultimatePowerB, address originB) internal returns(bool isWinner, uint256 profit, uint256 randomSeed, uint256 chance) { 
        if(ultimatePowerA >= ultimatePowerB) {
            (isWinner, chance, randomSeed) = realBattle(ultimatePowerA, ultimatePowerB);
        } else {
            (isWinner, chance, randomSeed) = realBattle(ultimatePowerB, ultimatePowerA);
            isWinner = !isWinner;
        }

        profit = calcProfit(chance);

        // 贏的話，使用贏家的勝率下去看損益區間
        if (isWinner) {
            // 贏的話使用贏家損益比
            // 呼叫 NFT質押池合約，獎賞分配
            IMasterChef(manager.members("NFTStakePool")).StakeWarDistribute(msg.sender, originB, profit);
        } else {
            // 輸的話使用輸家損益比
            // 呼叫 NFT質押池合約，獎賞分配
            IMasterChef(manager.members("NFTStakePool")).StakeWarDistribute(originB, msg.sender, profit);
        }
    ...
    // 計算雙方平均戰力
    function calculatePlayerPower(INFT.starAttributesStruct memory userA, INFT.starAttributesStruct memory userB) internal pure returns(uint256, uint256, uint256, uint256) {
        // 計算到小數點後兩位
        userA.power = userA.power.mul(100);
        userB.power = userB.power.mul(100);

        // 平均戰力
        uint256 ultimatePowerA = userA.power.div(syntheticCount(userA.level));
        uint256 ultimatePowerB = userB.power.div(syntheticCount(userB.level));

        // 等級加成戰力
        // 等級差加成  10% * 等差
        uint256 additionA = 100;
        uint256 additionB = 100;
        if (userA.level > userB.level) {
            // x = x + x*10/100*(等差)
            additionA += 10*(userA.level - userB.level);
        } else if (userA.level < userB.level) {
            additionB += 10*(userB.level - userA.level);
        }

        // 對手克制表 15%
        bool checkRace = compareRace(userA.ethnicity, userB.ethnicity);
        if (checkRace) {
            additionA = additionA+15;
        } else {
            additionB = additionB+15;
        }

        ultimatePowerA = ultimatePowerA.mul(additionA).div(100);
        ultimatePowerB = ultimatePowerB.mul(additionB).div(100);

        // 計算到小數點後兩位
        return (userA.power, ultimatePowerA, userB.power, ultimatePowerB);
    }
    
    // 合成數 2^(n-1)
    function syntheticCount(uint256 level) internal pure returns(uint256) {
        return 2**(level-1);
    }
}
