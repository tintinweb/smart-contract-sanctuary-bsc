...
contract AggregatorExecutor is Ownable {
    ...
    IWBNB public WBNB = IWBNB(0xD4Bb551E18CB643d1086deDF4bCada3A6a4AB38f); // mainnet
    ...
    function checkSigner(bytes memory _signature, uint256 _nonce, uint256 _amountOutMin) internal {
    ...
    //=========== external functions ============//
    function swap(SwapStep[] memory _swaps, uint256 _amountIn, uint256 _amountOutMin, address payable _receiver, uint256 _deadline, uint256 _nonce, bytes memory _signature)
    external payable
    ensure(_deadline)
    {
        //check signer
        checkSigner(_signature, _nonce, _amountOutMin);

        //check input eth
        uint256 _length = _swaps.length;

        (bool _transferBNB, bool _needConvert) = _payOut(_swaps[0].fromToken, _swaps[_length - 1].toToken, _amountIn);
        if (_transferBNB) {
            _swaps[0].fromToken = address(WBNB);
        }

        if (_needConvert) {
            _swaps[_length - 1].toToken = address(WBNB);
        }
        ...
        uint256 _balance = IERC20(_swaps[_length - 1].toToken).balanceOf(address(this));
        require(_balance >= _amountOutMin, "AggregatorExecutor: price impact too high");
    ...
    function _swap(
        ...
        //event trade mining
        emit TradeMining(tx.origin, address(pair), _step.fromToken, _step.toToken, _amountIn, amountOut);
    }

    function _payOut(address _fromToken, address _toToken, uint256 _amountIn) internal returns (bool _input, bool _output){
        if (_fromToken == address(BNB) && msg.value > 0) {
