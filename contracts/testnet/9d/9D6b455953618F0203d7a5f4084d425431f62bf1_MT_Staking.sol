...
contract MT_Staking is OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    modifier countingDebt(){
        ...
        if (tempBalance > lastU) {
            ...
            if (superNodeAmount > 0) {
                rewardInfo.superDebt += nodeRew * normalRate[2] / 100 / superNodeAmount;
            }
        }
        _;
        ...
        if (isAdd) {
            ...
            if (newLevel != oldLevel) {
                ...
                if (oldLevel != 0) {
                    ...
                    if (tempReward > 0) {
                        claimInfo[msg.sender].levelClaimed += tempReward;
    ...
    function setUserLevel(address addr, uint Amount) external onlyManager {
    ...
    function setUserSuperNode(address addr, bool b) external onlyManager {
        if (b) {
