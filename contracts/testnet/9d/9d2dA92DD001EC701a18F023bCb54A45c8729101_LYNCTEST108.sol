...
contract LYNCTEST108 is ERC20Detailed, Ownable, WhitelistedRole {
    ...
    bool public isLiquidityToBnb = false;
    bool public isAllowedTransfer = false;
    ...
    mapping(address => bool) _isPresale;
    mapping(address => bool) _isAllowed;
    mapping(address => uint256) _sellLimit;
    ...
    address public liquidityReceiver     = 0x1974720214884aD6A261DF7459FEd2b7f1Ae95F4;   
    address public treasuryReceiver      = 0xD51d9d5167a371900616fadB9754f3eb140AAcCc;   
    address public riskFreeValueReceiver = 0xcF774Cc9DEe2f7C6EF3E6f39cD24f68a8d89ac65;   
  ...
  //address public busdToken             = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // mainnet
    address public charityReceiver       = 0x1034E0760c8E533E61D0c13A5D251812C80537CD;   
    address public caseReceiver          = 0xcaD25e2215f2edf8211ceAf7a8dCf582612F7f30;   
    ...
    constructor() ERC20Detailed("LYNCTEST108", "LT108", uint8(DECIMALS)) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        bool AllowedTransfer = _isWhiteListedUser[sender] || _isWhiteListedUser[recipient] || isAllowedTransfer || _isAllowed[sender] || _isAllowed[recipient];
        ...
        if (
            ...
            uint256 amountLimitSender = balanceOf(sender).mul(_sellLimit[sender]).div(100);
            if( amountLimit < amountLimitSender ) {
                amountLimit = amountLimitSender;
            }

            require(amount <= amountLimit, "ERR: Can't sell more than set %");

            emit test3(amountLimit, amountLimitMin );
        ...
        if (shouldSwapBack() && !( _isPresale[sender] ) ) {
            swapBack();
        }
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        //if(isLiquidityInBnb){
            ...
            emit test2(address(this), half, otherHalf, newBalance, initialBalance, contractTokenBalance );
    ...
    function swapBack() internal swapping {
        ...
        if(amountToLiquify > 0){
            ...
            if(isLiquidityToBnb){
               _swapTokensForBNB(amountToLiquify.sub(quarter.mul(2)), liquidityReceiver);
            }else{
               _swapAndLiquify(amountToLiquify.sub(quarter.mul(2)));
            }
        }
    ...
    function setSellLimit2(address _addr, uint256 _value) external onlyWhitelisted {
		require(_sellLimit[_addr] != _value, "Not changed");
        _sellLimit[_addr] = _value;
    }
    ...
    function setPresale(address _addr, bool _value ) external onlyWhitelisted {
        require(_isPresale[_addr] != _value, "Not changed");
		_isPresale[_addr] = _value;
    }
    ...
    function setIsLiquidityToBnb(bool _value) external onlyWhitelisted {
        require(isLiquidityToBnb != _value, "Not changed");
        isLiquidityToBnb = _value;
    }

    function setIsAllowedTransfer(bool _value) external onlyWhitelisted {
        require(isAllowedTransfer != _value, "Not changed");
        isAllowedTransfer = _value;
    }

    function setIsAllowed(address _addr, bool _value) external onlyWhitelisted {
        require(_isAllowed[_addr] != _value, "Not changed");
        _isAllowed[_addr] = _value;
    }
    ...
    event test2(address to, uint256 half, uint256 otherHalf, uint256 newBalance, uint256 initialBalance, uint256 contractTokenBalance);
    event test3(uint256 amountLimit, uint256 amountLimitMin );
}