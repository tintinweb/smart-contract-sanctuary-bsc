...
contract ReedBricks is Context, IERC20, Ownable {
    ...
    // Treasury Currency
    address private TRS_BANK_TOKEN;
    ...
    function setRewardFeePercent(uint256 RewardFee) external onlyOwner {
        uint256 feeCheck = _rewardFee.add(_liquidityFee).add(_treasuryFee);
        require(feeCheck <= feeCap, "REED: All fees must not exceed 10%");
    ...
    function getERC20Tokens(address _token, address recipient)
        external
        onlyOwner
    {
        ...
        returns (uint256)
    {
        ...
        emit ExtractERC20Token(tokenAddr, amount);
        return amount;
    }
    ...
    // Set the Token to use for Treasury Bill
    // TreasuryBill converts token to any Asset
    function setTreasuryFeePercent(address tokenAddr) external onlyOwner {
        require(tokenAddr != TRS_BANK_TOKEN, "REED: TOKEN is the same");
        TRS_BANK_TOKEN = tokenAddr;
    }
    ...
    // Convert REED from contract to Bill
    function treasuryBill(uint256 tokens) external returns (bool) {
        uint256 initialTRS_BANK_TOKEN_Balance = IERC20(TRS_BANK_TOKEN)
            .balanceOf(address(this));
        swapTokensForEth(tokens);
        uint256 newBalance = (IERC20(TRS_BANK_TOKEN).balanceOf(address(this)))
            .sub(initialTRS_BANK_TOKEN_Balance);
        IERC20(TRS_BANK_TOKEN).transfer(TREASURY_BANK_WALLET, newBalance);
        return true;
    }
}