...
contract BTCxPresale {
    ...
    uint256 public totalTokensToSell = 30000000 ;          // 81000000000 BTCx tokens for sell
    uint256 public BTCxPerBnb = 14330 ;             // 1 BNB = 30000000 BTCx
    uint256 public minPerTransaction = 1433;         // min amount per transaction (0.1BNB)
    uint256 public maxPerUser = 143300;                // max amount per user (10BNB)
    ...
    mapping(address => uint256) public BTCxPerAddresses;
    ...
    function buyWithBNB(uint256 buyAmount) public payable checkSaleRequirements(buyAmount) {
        ...
        uint256 sumSoFar = BTCxPerAddresses[msg.sender].add(buyAmount);
        require(sumSoFar <= maxPerUser, 'Greater than the maximum purchase limit');

        BTCxPerAddresses[msg.sender] = sumSoFar;
        totalSold = totalSold.add(buyAmount);
        
        BTCx.transfer(msg.sender, buyAmount);
    ...
    function setTokenPricePerBNB(uint256 _BTCxPerBnb) public {
        require(msg.sender == owner);
        require(_BTCxPerBnb > 0, "Invalid BTCx price per BNB");
        BTCxPerBnb = _BTCxPerBnb;
    }
    ...
    function endSale() public {
        ...
        uint256 tokenAmount = BTCx.balanceOf(address(this));
    ...
    function withdrawUnsoldTokens() public {
        ...
        BTCx.transfer(owner, remainedTokens);
    }
    ...
    function unsoldTokens() public view returns (uint256) {
        ...
        return BTCx.balanceOf(address(this));
    }
    ...
    function calculateBTCxAmount(uint256 bnbAmount) public view returns (uint256) {
        uint256 BTCxAmount = BTCxPerBnb.mul(bnbAmount);
        return BTCxAmount;
    }

    function calculateBNBAmount(uint256 BTCxAmount) public view returns (uint256) {
        require(BTCxPerBnb > 0, "BTCx price per BNB should be greater than 0");
        uint256 bnbAmount = BTCxAmount.div(BTCxPerBnb);
        return bnbAmount;
    }
    ...
    function setRouter(address _router) public {
        ...
        if (IUniswapV2Factory(_newPancakeRouter.factory()).getPair(address(BTCx), _newPancakeRouter.WETH()) == address(0)) {
            uniswapV2Pair = IUniswapV2Factory(_newPancakeRouter.factory()).createPair(address(BTCx), _newPancakeRouter.WETH());
        } else {
            uniswapV2Pair = IUniswapV2Factory(_newPancakeRouter.factory()).getPair(address(BTCx), _newPancakeRouter.WETH());
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        BTCx.approve(address(uniswapV2Router), tokenAmount);
        uint256 allowance = BTCx.allowance(address(this), address(uniswapV2Router));
