    ...
    function cancelOrder(FixedPriceOrder[] calldata _orders) external {
        ...
        for (uint256 i = 0; i < _orders.length; i++) {
            ...
            if (maker != order.maker) {
                revert MakerNotMatch({maker:maker});
            }

            (, , bytes32 orderHash, ) = deriveOrder(order);

            if (ordersStatus[orderHash].matched) {
                revert OrderIsDealt({isDealt:ordersStatus[orderHash].matched});
            }

            if (ordersStatus[orderHash].cancelled) {
                revert OrderIsCancelled({isCanceled:ordersStatus[orderHash].cancelled});
            }
...
abstract contract OrderValidator is NonceManager, OrderDeriver {
        ...
        )
    {
        ...
        if (_makerOrder.makerNonce != _nonce(maker)) {
            revert MakerNonceNotMatch({nonce: _makerOrder.makerNonce});
        }
        // 校验订单开启时间
        if (_makerOrder.startAt > block.timestamp) {
            revert OrderNotReady({startAt:_makerOrder.startAt});
        }
        // 校验订单过期时间
        if (_makerOrder.expireAt <= block.timestamp) {
            revert OrderIsExpired({expireAt:_makerOrder.expireAt});
        }


        if (maker == _taker) {
            revert TakerEqualsMaker({maker:maker});
        }
        // 校验手续费
        if (_makerOrder.royaltyRate > _maxRoyaltyRate()) {
            revert TooHighRoyaltyRate({royaltyRate:_makerOrder.royaltyRate });
        }
    }
