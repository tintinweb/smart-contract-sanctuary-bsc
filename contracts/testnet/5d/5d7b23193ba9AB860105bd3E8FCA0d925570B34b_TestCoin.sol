...
contract TestCoin is Context, IERC20, Ownable {
    ...
    uint256 public constant _maxTxAmount = 10**5 * 10**9 * 10**18; 
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 10 * 10**9 * 10**18; // 0.001 %

    string private constant _name = "TestCoin"; 
    string private constant _symbol = "TC"; 
    uint8  private constant _decimals = 18; 
    uint256 public constant _marketingFee = 2;
    uint256 public constant _liquidityFee = 4;
    address public constant _marketingWallet = 0xDDbCB24Ec53fa96c1C7c244763B6A05877d1d84a;
    ...
    function _transfer(
        ...
        if (contractTokenBalance >= SWAP_TOKENS_AT_AMOUNT && !swapping && from != uniswapV2Pair && from != owner() && to != owner()) {
            swapping = true;
    ...
    function swapAndLiquify() private {
        ...
        // split the contract liquidityTokens into halves
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        if (ethAmount - ethFromLiquidity > 0)
            sendValue(payable(_marketingWallet), ethAmount - ethFromLiquidity);
    }
