 ...
 */
contract OsizProfile is AccessControl, ERC721Holder {
    using Counters for Counters.Counter;
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    IBEP20 public osizToken;
    ...
    uint256 public numberosizToReactivate;
    uint256 public numberosizToRegister;
    uint256 public numberosizToUpdate;
    ...
    constructor(
        IBEP20 _osizToken,
        uint256 _numberosizToReactivate,
        uint256 _numberosizToRegister,
        uint256 _numberosizToUpdate
    ) {
        osizToken = _osizToken;
        numberosizToReactivate = _numberosizToReactivate;
        numberosizToRegister = _numberosizToRegister;
        numberosizToUpdate = _numberosizToUpdate;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @dev To create a user profile. It sends the NFT to the contract
     * and sends osiz to burn address. Requires 2 token approvals.
     */
    function createProfile(
        ...
        // Transfer osiz tokens to this contract
        osizToken.safeTransferFrom(_msgSender(), address(this), numberosizToRegister);
    ...
    function updateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        // Transfer osiz token to this address
        osizToken.safeTransferFrom(_msgSender(), address(this), numberosizToUpdate);
    ...
    function reactivateProfile(address _nftAddress, uint256 _tokenId) external {
        ...
        osizToken.safeTransferFrom(_msgSender(), address(this), numberosizToReactivate);
    ...
    /**
     * @dev Claim osiz to burn later.
     * Callable only by owner admins.
     */
    function claimFee(uint256 _amount) external onlyOwner {
        osizToken.safeTransfer(_msgSender(), _amount);
    }
    ...
    /**
     * @dev Update the number of osiz to register
     * Callable only by owner admins.
     */
    function updateNumberosiz(
        uint256 _newNumberosizToReactivate,
        uint256 _newNumberosizToRegister,
        uint256 _newNumberosizToUpdate
    ) external onlyOwner {
        numberosizToReactivate = _newNumberosizToReactivate;
        numberosizToRegister = _newNumberosizToRegister;
        numberosizToUpdate = _newNumberosizToUpdate;
    }
 ...
 * other osizProfile requirements.
 */
contract IFOV2 is IIFOV2, ReentrancyGuard, Ownable {
    ...
    // osizProfile
    OsizProfile public osizProfile;
     ...
     * @param _osizProfileAddress: the address of the osizProfile
    ...
    constructor(
        ...
        osizProfile = OsizProfile(_osizProfileAddress);
    ...
    function depositPool(uint256 _amount, uint8 _pid) external override nonReentrant notContract {
        ...
        require(osizProfile.getUserStatus(msg.sender), "Must have an active profile");
    ...
    function _claimPoints(address _user) internal {
        if (!_hasClaimedPoints[_user]) {
            ...
            if (sumPools > thresholdPoints) {
                ...
                osizProfile.increaseUserPoints(msg.sender, numberPoints, campaignId);
            }
        }
    }
    ...
    function updateOfferingToken(IBEP20 token) public onlyOwner {
        offeringToken = token;
    }

    function updateBlock(uint256 _start,uint256 _end)public onlyOwner{
        startBlock = _start;
        endBlock = _end;
    }
