...
contract TifiRouter is IPancakeRouter02 {
    ...
    function _feeAmount(uint amount) public view returns (uint) {   
        ...
        uint fee = amount.mul(totalFee)/10000;
        return fee;        
    }
    ...
    function swapExactTokensForTokens(
        ...
        if (path[0] == tifi){
            ...
            TransferHelper.safeApprove(path[0], PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]);        
        ...
        if (path[0] == tifi){
            ...
            if (fee > 0) {
                TransferHelper.safeApprove(path[0], feeReceiver, fee/2);  
                TransferHelper.safeTransfer(path[0], feeReceiver, fee/2);
            }
        } 
        if (path[1] == tifi){
            _swap(amounts, path, address(this));
            fee = _feeAmount(amounts[amounts.length - 1]);
            uint sendingAmount = amounts[amounts.length - 1].sub(fee);
            if (fee > 0){
                TransferHelper.safeApprove(path[path.length - 1], feeReceiver, fee/2);  
                TransferHelper.safeTransfer(path[path.length - 1], feeReceiver, fee/2);
            }
            TransferHelper.safeApprove(path[path.length - 1], to, sendingAmount);  
            TransferHelper.safeTransfer(path[path.length - 1], to, sendingAmount);
        }
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if (path[0] == tifi){
            ...
            TransferHelper.safeApprove(path[0], PancakeLibrary.pairFor(factory, path[0], path[1]), amountIn-fee);        
            TransferHelper.safeTransfer(path[0], PancakeLibrary.pairFor(factory, path[0], path[1]), amountIn-fee);         
        } else {
