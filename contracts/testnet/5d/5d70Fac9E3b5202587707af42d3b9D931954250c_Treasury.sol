...
contract MarketPlace is Auth, PausableUpgradeable {
  ...
  function makeOrder(
    ...
    if (_startPrice != _endingPrice) {
      require(_auctionDuration >= 1 days && _auctionDuration <= 365 days, "duration is invalid");
    }
    if (_type == IMarketPlace.OrderType.renting) {
      require(_rentingDuration >= 7 days && _rentingDuration <= 365 days, "renting duration is invalid");
    }
...
contract Treasury is Auth {
  ...
  function initialize(address _mainAdmin, address _heroToken, address _itemToken) public initializer {
    ...
    fotaToken = IFOTAToken(0x69c3FF5F466757BF99feF916aE9a191d7c13696c);
