...
contract TSXNBase is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public constant _name = "Tra Sua Xi Nghiep Base";
    string public constant _symbol = "TSXN";
    ...
    uint256 public constant tsxnRiskFreeValueFee = 40;
    uint256 public constant firePitFee = 20;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(tsxnRiskFreeValueFee).add(
            firePitFee
        );
    ...
    address public tsxnRiskFreeValueReceiver;
    ...
    constructor()
        ERC20Detailed("Tra Sua Xi Nghiep Base", "TSXN", uint8(DECIMALS))
        Ownable()
    {
        ...
        tsxnRiskFreeValueReceiver = 0xd25e40DDEDF7D93A1626b577C21b2540781358F9;
    ...
    function takeFee(
        ...
        uint256 _tsxnRiskFreeValueFee = tsxnRiskFreeValueFee;

        if (recipient == pair) {
            _totalFee = 160;
            _liquidityFee = 50;
            _treasuryFee = 20;
            _tsxnRiskFreeValueFee = 70;
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.mul(_treasuryFee.add(_tsxnRiskFreeValueFee)).div(
                feeDenominator
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndRFV.mul(treasuryFee).div(
                treasuryFee.add(tsxnRiskFreeValueFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(tsxnRiskFreeValueReceiver).call{
            value: amountETHToTreasuryAndRFV
                .mul(tsxnRiskFreeValueFee)
                .div(treasuryFee.add(tsxnRiskFreeValueFee)),
            gas: 30000
        }("");
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        require(
            amountToSwap > 0,
            "There is no TSXN token deposited in token contract"
        );
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
    ...
    function setFeeReceivers(
        ...
        address _tsxnRiskFreeValueReceiver
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        tsxnRiskFreeValueReceiver = _tsxnRiskFreeValueReceiver;
    }
