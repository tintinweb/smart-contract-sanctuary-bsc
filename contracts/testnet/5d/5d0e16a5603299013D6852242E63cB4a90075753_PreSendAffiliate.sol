...
contract PreSendAffiliate is Initializable, AccessControlUpgradeable {
    ...
    // Event to emit whenever the percentage for an affiliate tier is updated.
    event affiliateTierAddedOrUpdated(uint256 indexed affiliateTier, uint256 newTierPercentage);
    ...
    */
    function addUpdateOrRemoveAffiliate(address affiliate, uint256 tier) external onlyRole(AFFILIATE_ADMIN) {
        ...
        require(tier == 0 || affiliateTierToPercentage[tier] > 0, "This affiliate tier does not exist. To remove an affiliate, set their tier to 0. Otherwise, choose a tier already created.");
        affiliateToTier[affiliate] = tier;
        emit affiliateAddedUpdatedOrRemoved(affiliate, tier);
    }

    /**
    @dev Affiliate admin only function to add or update a tier.
    @param tier the index for the tier mapping that is being added or updated
    @param tierPercentage the percentage for the tier being added or updated
    */
    function addOrUpdateTier(uint256 tier, uint256 tierPercentage) external onlyRole(AFFILIATE_ADMIN) {
        ...
        emit affiliateTierAddedOrUpdated(tier, tierPercentage);
    }
