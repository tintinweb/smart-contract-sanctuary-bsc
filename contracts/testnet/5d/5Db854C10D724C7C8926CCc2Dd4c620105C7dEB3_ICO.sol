...
Pause
{
    ...
    initializer
    {
        ...
        tokenProject = IERC20(_tokenProject);
        isWhitelist = false;
        ref_directSale = 20;
        ref_leader = 20;
        price = 18;
    ...
    function execute(
        ...
        IERC20 tokenRaise_ = IERC20(tokenRaiseAddress_);
        ...
        createOrder(tokenRaiseAmount_, referral_);
    ...
    function paySystem(IERC20 tokenRaise_, uint256 tokenRaiseAmount_) internal {
        SafeERC20.safeTransferFrom(tokenRaise_, _msgSender(), address(this), tokenRaiseAmount_);
    }

    function payReferral(IERC20 tokenRaise_, address referral_, uint256 tokenRaiseAmount_) internal {
        ...
        SafeERC20.safeTransfer(tokenRaise_, referral_, amount);
    }

    function payLeader(IERC20 tokenRaise_, address referral_, uint256 tokenRaiseAmount_) internal {
        address leader = getLeader(referral_);
        uint256 amount = SafeMathUpgradeable.mul(tokenRaiseAmount_, ref_leader).div(100);
        tokenRaise_.approve(address(this), amount);
        SafeERC20.safeTransfer(tokenRaise_, leader, amount);
    }

    function createOrder(uint256 tokenRaiseAmount_, address referral_) internal {
        uint256 tokenProjectAmount = SafeMathUpgradeable.div(tokenRaiseAmount_, price).mul(10000);
        address manager = getManager(referral_, managers);
        calculateRevenue(manager, tokenRaiseAmount_);
        for (uint i = 0; i < lengthVesting; i++) {
            ...
            Order memory order = Order(tokenRaiseAmount_, tokenProjectAmountVesting, timeVesting, manager, referral_);
    ...
    function getOrder(address buyer) public view returns (Order[] memory) {
        return orders[_msgSender()];
    }

    function claimOrder() public {
        if (timeLock <= block.timestamp) {
            for (uint i = 0; i < lengthVesting; i++) {
                Order memory order = orders[_msgSender()][i];
                if (order.claimDate_ <= block.timestamp) {
                    tokenProject.approve(address(this), order.tokenProjectAmount_);
                    SafeERC20.safeTransfer(tokenProject, _msgSender(), order.tokenProjectAmount_);
                    delete orders[_msgSender()][i];
                }
            }
        }
    }
    ...
    function validate(IERC20 tokenRaise_, uint256 amount_) internal {
        require(
    ...
    function withdrawToken(address token_) external onlyOwner isWithdrawWallet {
        IERC20 tokenERC20_ = IERC20(token_);
        uint256 balance_ = tokenERC20_.balanceOf(address(this));
        tokenERC20_.approve(address(this), balance_);
        SafeERC20.safeTransfer(tokenERC20_, getWithdrawWallet(), balance_);
    }
    ...
    function isManager(address manager_) internal view returns (bool){
        return checkExists(manager_, managers);
    }

    function getManager(address referral_, address[] memory managers) internal view returns (address){
        address manager_ = purchasers[referral_];
        address refDefault_ = getRefWalletDefault();

        if (checkExists(_msgSender(), managers)) {
            return _msgSender();
        }

        if (checkExists(referral_, managers)) {
            return referral_;
        }

        if (manager_ == address(0) || referral_ == address(0) || referral_ == refDefault_ || manager_ == refDefault_) {
            return refDefault_;
        }
    ...
    function getRevenue(address manager) public view returns (uint256) {
        return revenue[manager];
    }

    // ########### Start leader ###########
    function addLeader(address leader_) public {
        require(hasRole(MOD_ROLE, _msgSender()));
        leaders.push(leader_);
    }

    function removeLeader(uint256 index_) public {
        require(hasRole(MOD_ROLE, _msgSender()));
        delete leaders[index_];
    }
    ...
    function getLeader(address referral_) internal view returns (address){
        address leader_ = purchasers[referral_];
        address refDefault_ = getRefWalletDefault();

        if (checkExists(_msgSender(), leaders)) {
            return _msgSender();
        }

        if (checkExists(referral_, leaders)) {
            return referral_;
        }
    ...
    function isLeader(address leader_) internal view returns (bool){
