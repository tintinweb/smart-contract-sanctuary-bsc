...
Ownable
{
  ...
  mapping(address => mapping(address => address)) public lpMap;
  ...
  function buyNFT(address _nft, address _token) public {
    ...
    require(lpMap[token0][token1] != address(0), "buy: _token not good");
    uint _price = price;
    if (_token != usdtAddress) {
      _price = price.mul(times).div(getPrice(times, _token));
      if (_token == threeAddress) {
        require(ERC20(_token).balanceOf(msg.sender) >= fee.add(_price), "buy: balance not good");
      } else {
        require(ERC20(_token).balanceOf(msg.sender) >= _price, "buy: balance not good");
      }
    } else {
