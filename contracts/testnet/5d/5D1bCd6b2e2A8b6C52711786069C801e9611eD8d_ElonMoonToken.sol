...
import "../dependencies/open-zeppelin/utils/AddressUpgradeable.sol";

contract ElonMoonToken is
    Initializable,
    ERC20Upgradeable,
    OwnableUpgradeable,
    ...
    ERC20VotesUpgradeable
{
    ...
    //     __ERC20_init("ELONMOON", "MOON");
    //     __Ownable_init();
    //     _mint(msg.sender, 3_000_000_000_000 * 1e18);
    // }
    constructor() initializer {}
    ...
    uint256 private TIME_UNIT = 3600;

    address public LOCK_CONTRACT;
    address public LP_CONTRACT;
    bool public bpEnabled;
    uint8 public swapRate;
    uint16 public timeLock; //don vi hours
    ...
    function _afterTokenTransfer(
        ...
        if (AddressUpgradeable.isContract(to)) {
            lockedTime[from] = block.timestamp + timeLock;
        } else if (lockedTime[from] > lockedTime[to]) {
            lockedTime[to] = lockedTime[from];
        }
    ...
    function setLockContract(address lockContract)
        public
        onlyOwner
        returns (bool)
    {
    ...
    // function setLPContract(address lpContract) public onlyOwner {
    //     LP_CONTRACT = lpContract;
    // }

    function configLock(uint8 _swapRate, uint16 _timeLock) public onlyOwner {
        swapRate = _swapRate;
        timeLock = _timeLock;
    }
    ...
    function _transfer(
        ...
        if (LOCK_CONTRACT != address(0)) {
            lockedAmount = ILockContract(LOCK_CONTRACT).lockedAmount(sender);
            if ((lockedAmount + amount) > balanceOf(sender)) {
                revert();
            }
        }
        //HANDLE swap token to other
        uint256 availableBalance = balanceOf(sender) - lockedAmount;
        if (AddressUpgradeable.isContract(recipient)) {
            if(amount > (availableBalance * swapRate / 1000)
                || lockedTime[sender] > block.timestamp){
                revert("Amount Invalid or timelock");
            }
        }
    ...
    function isContract(address contractAddress) public view returns (bool){
        return AddressUpgradeable.isContract(contractAddress);
    }
    /**
