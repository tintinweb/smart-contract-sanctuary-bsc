...
abstract contract MCYMarketplaceErrorCode {
    // Define Error Code

    string public constant NotList = "Not List";
...
abstract contract MCYMarketplaceFee is Ownable {
...
abstract contract MCYMarketplaceOfferer is MCYMarketplaceFee {
    ...
    {
        require(
            offerNfts[_nft][_tokenId][_offerer].nft != address(0),
            "not offerred nft"
        );
        require(
            IERC721Enumerable(_nft).ownerOf(_tokenId) == msg.sender,
            "not nft owner"
        );
        ...
        if(offer.payToken != address(0)) {
            ...
            payToken.transfer(address(this), platformFeeTotal);
            // Transfer to seller
            payToken.transfer(msg.sender, totalPrice - platformFeeTotal);
        } else {
        ...
        IERC721Enumerable(offer.nft).safeTransferFrom(
            address(msg.sender),
            offer.offerer,
            offer.tokenId
        );
...
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "./core/MCYMarketplaceAuctions.sol";
// import "./interfaces/IMegacyNFT.sol";
// import "./interfaces/IMegacyNFTFactory.sol";
import "./core/MCYMarketplaceFee.sol";
import "./core/MCYMarketplaceOfferer.sol";
import "./core/MCYMarketplaceErrorCode.sol";
...
/* Megacy NFT Marketplace
    ...
    & support Royalty
*/
contract MegacyNFTMarketplace is 
    Ownable,
    MCYMarketplaceErrorCode,
    MCYMarketplaceFee, 
    MCYMarketplacePaymentTokens, 
    MCYMarketplaceOfferer, 
    IERC721Receiver, 
    ReentrancyGuard {//
    ...
    modifier isListedNFT(address _nft, uint256 _tokenId) {
        ...
        require(
            ...
            MCYMarketplaceErrorCode.NotList
        );
        _;
    }
    ...
    function listNft(
        ...
        IERC721Enumerable(_nft).safeTransferFrom(msg.sender, address(this), _tokenId);
    ...
    function updateListedNft(
        ...
        listingNFTs[_nft][_tokenId] = ListNFT({
            ...
            price: _price,
            sold: false,
            listingDate: listedNFT.listingDate,
            expiredDate: _expiredDate
        });
        emit UpdateListedNFT(_nft, _tokenId, msg.sender, address(this), _price);
    }
    ...
    function buyNFT(
        ...
        require(_payPrice >= (listedNft.price + calculatePlatformFee(listedNft.price)), "Please submit the asking price in order to complete the purchase");
        listedNft.sold = true;
        //uint256 totalPrice = msg.value;
        ...
        // uint256 totalPrice = msg.value;
        owner.transfer(listedNft.price);//Owner received from buyer // thu phí người bán ở đây
        payable(address(this)).transfer(_payPrice - listedNft.price);//Platform received from buyer
    ...
    function buyNFTPayToken(
        ...
        payToken.approve(address(this), _price);
        ...
        payable(address(this)).transfer(totalPrice-listedNft.price);
    ...
    // function withdrawFullNFT()
    //     public onlyOwner
    // {
    //     IERC721Enumerable nft = IERC721Enumerable(_nft);
    //     require(nft.ownerOf(_tokenId) == address(this), "not nft owner");
    ...
    function withdrawToken(address _token, uint256 amount) public onlyOwner {
        require(IERC20(_token).balanceOf(address(this)) >= amount, "Insufficient account balance");
