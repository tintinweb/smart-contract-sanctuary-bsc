...
contract EVTMaster is Ownable {
    ...
    uint256[] public LEVEL_YIELD = [93, 78, 64, 50, 37, 24, 11, 0];
    ...
    event Operation(string _type, address indexed _user, address indexed _referrer, uint256 amount, uint256 timestamp);

    event LevelChanged(uint256 newLevel, uint256 timestamp);
    event NewReferral(address indexed _user, address referral, uint256 timestamp);
    event ReferralReward(address indexed _referrer, address indexed _user, uint256 amount, uint256 timestamp);
    event ClaimStaked(address indexed _user, address indexed _referrer, uint256 amount, uint256 timestamp);
    ...
    constructor(EVT _rewardToken) {
        ...
        user.totalReferrals = uint256(1);
        user.investment = 10000000000000000000; // ?
    ...
    function _refund() private {
        _mintTokens();
        safeSendValue(payable(msg.sender), msg.value);
    }
    ...
    function _stake(
        ...
        if (!user.exists) {
            ...
            _users[referrer].totalReferrals += 1;
        ...
        _platformFees += (_amount * UNSTAKE_FEE) / 100;
    ...
    function unstake(uint256 _amount) public {
        ...
        User storage user = _users[msg.sender];
        require(user.investment >= _amount, "EVTMaster::unstake: Insufficient funds");
        _mintTokens();
        ...
        _totalStaked -= _amount;
        user.investment = user.investment - _amount;
        _amount = ((uint256(100) - UNSTAKE_FEE) * _amount) / 100;

        safeSendValue(payable(msg.sender), _amount);
    }
    ...
    function _mintTokens() internal returns (uint256 mintedAmount) {
        uint256 timePassed = block.timestamp - _lastMintTime;
        if (timePassed == 0) {
            return 0;
        }
        if (_totalStaked != 0) {
            uint256 toMint = (((_totalStaked * timePassed) / REWARD_INTERVAL) * LEVEL_YIELD[_currentLevel]) / 100;
            // Add ref percent
            toMint += (toMint * REF_REWARD_PERCENT) / 100;
            if (_mintedByStaking + toMint > MAX_MINT_BY_STAKING) {
                toMint = MAX_MINT_BY_STAKING - _mintedByStaking;
            }
            mintedAmount = safeMint(address(this), toMint);
            _mintedByStaking += mintedAmount;
        }
        _lastMintTime = block.timestamp;
    }
    ...
    function _claimStaked(address _address) internal {
        ...
        uint256 referralReward = (reward * REF_REWARD_PERCENT) / 100;

        safeTokenTransfer(user.addr, reward);

        _users[user.referrer].referralReward += referralReward;
    ...
    function pendingReward(address _address) public view returns (uint256 reward) {
        User memory user = _users[_address];
        uint256 lastClaim = user.lastClaim;
        for (uint256 lvl = 0; lvl <= _currentLevel; ++lvl) {
            uint256 time = (levelChangeTime[lvl] == 0) ? block.timestamp : levelChangeTime[lvl];
            if (_users[_address].lastClaim >= time) {
                continue;
            }
            reward += (((user.investment * (time - lastClaim)) / REWARD_INTERVAL) * LEVEL_YIELD[lvl]) / 100;
    ...
    // Function for owner to withdraw staking fees after minting ends
    function withdrawFees() public onlyOwner returns (uint256) {
        return withdrawFees(payable(owner()), _platformFees);
    }

    // Function for owner to withdraw staking fees after minting ends
    function withdrawFees(address payable _address, uint256 _amount) public onlyOwner returns (uint256) {
        require(_currentLevel == LEVEL_LIMIT.length - 1, "Mining didn't end");
        _platformFees -= _amount;
    ...
    // Since this breaks contract stats only owner (governance contract can call) can call this function
    function emergencyWithdraw(address payable _user, uint256 _amount) public onlyOwner returns (uint256) {
        _users[msg.sender].investment -= _amount;

        uint256 fee = (_amount * UNSTAKE_FEE) / 100;
        return safeSendValue(_user, _amount - fee);
    }

    // Don't care about rewards and stats (emergency only, in case if unexpected error occurs)
    function emergencyWithdraw(address payable _user) public onlyOwner returns (uint256) {
    ...
    function mintFromLP(uint256 _amount) public returns (uint256 mintedAmount) {
        ...
        _mintTokens();
        if (_amount >= MAX_MINT_BY_LP - _mintedByLP) {
            _amount = MAX_MINT_BY_LP - _mintedByLP;
        }
        mintedAmount = safeMint(msg.sender, _amount);
        _mintedByLP += mintedAmount;
    }
    ...
    function safeMint(address _to, uint256 _amount) internal returns (uint256) {
        uint256 canBeMinted = rewardToken.maxSupply() - rewardToken.totalSupply();
        if (_amount > canBeMinted) {
            _amount = canBeMinted;
        }

        _mintedCurrentLevel += _amount;

        rewardToken.mint(_to, _amount);

        if (_mintedCurrentLevel >= LEVEL_LIMIT[_currentLevel] && _currentLevel < (LEVEL_LIMIT.length - 1)) {
...
contract EVT is Ownable, ERC20("EVT", "Evereth"), ERC20Permit("Evereth") {



    uint256 public maxSupply = 2800000 ether; 
     ...
     */
    function _ourMint(address _to, uint256 _amount) internal {
        require(_to != address(0), "EVT::_ourMint: mint to the zero address");
        _mint(_to, _amount);
    ...
    function mint(address _to, uint256 amount) public onlyOwner returns (bool) {
        require(totalSupply() + amount <= maxSupply, "EVT::mint: max supply exceeded");
        _ourMint(_to, amount);
        return true;
    }
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "EVT::delegateBySig: invalid signature");
        require(nonce == nonces(signatory) + 1, "EVT::delegateBySig: invalid nonce");
    ...
    function getPriorVotes(address account, uint256 blockNumber) public view returns (uint256) {
        require(blockNumber < block.number, "EVT::getPriorVotes: not yet determined");
    ...
    function _transferTokens(
        ...
        require(src != address(0), "EVT::_transferTokens: cannot transfer from the zero address");
        require(dst != address(0), "EVT::_transferTokens: cannot transfer to the zero address");
