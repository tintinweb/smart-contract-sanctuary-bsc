...
interface IFMT {
    ...
    function _checkReleasedAmount(address _userAddr) external view returns (uint256);
    function stakeTransfer(address from,address to,uint amount) external;
    function safeRegisterUser(address _userAddress,address _inviter) external;
...
contract Stake is Context, Ownable,ReentrancyGuard {
    ...
    //lp用户列表
    address[] public lpUserList;
    uint public lpAmount;
    mapping(address=>uint) public userLpAmount;
    ...
    mapping(address=>StakeData) getStakeData;
    uint computeCycle;
    // mapping(uint=>uint) stakeRate;
    ...
    // mapping(address=>uint[]) public userAllNode;
    // mapping(address=>uint8) public userLevel;

    constructor() 
    {
        defaultInviter = 0xC87C8fFF5b8c4d8C87A03401279A073A87BCE668;

        mainPairC = IPair(0x609D9E6554939B40A02DfC650b31Fe97D878afE8);

        lpRequire = 10;
        stakeRate=[110,150,250];

        FMT = IFMT(0x854Bd94E860256e51EF3Bb1c3c93A756F6a1128c);

        computeCycle = 60;
    }

    function setData(address _defaultInviter,address mainPair,address _fmtAddress,uint _computeCycle)public onlyOwner{
    ...
    //质押模块
    function lpStake(uint _stakeAmount)public {
        ...
        FMT.safeRegisterUser(_sender, defaultInviter);

        if(isLpStaker[_sender]){
            lpStakeAmount[_sender]+=_stakeAmount;
        }else{
            lpStaker.push(_sender);
            isLpStaker[_sender]=true;
            lpStakeAmount[_sender]=_stakeAmount;
        }
    }

    function withdrawLpStaked()public nonReentrant{
        address _sender = msg.sender;
        require(isLpStaker[_sender],"user not pledged.");
        getIndexAndDelOfAddr(lpStaker,_sender);
        isLpStaker[_sender] = false;
        uint _stakeAmount = lpStakeAmount[_sender];
        delete lpStakeAmount[_sender];
        mainPairC.transfer(_sender, _stakeAmount);
    }

    function stake(uint _stakeAmount,StakeOptions _options) public {
        address _sender = msg.sender;
        require(!isStaker[_sender],"Already staked");
        
        uint256 unfreezeBalance;
        FMT.includedUser(_sender)?unfreezeBalance = FMT._checkReleasedAmount(_sender):unfreezeBalance = FMT.balanceOf(_sender);
        require(unfreezeBalance >= _stakeAmount);
        FMT.basicTransfer(_sender,address(this),_stakeAmount);

        StakeData storage _newStakeData = getStakeData[_sender];
        _newStakeData.stakeAmount=_stakeAmount;
        _newStakeData.stakeOptions=_options;
        _newStakeData.stakeTime=block.timestamp;
        isStaker[_sender]=true;
        staker.push(_sender);
    }
    ...
    function withdrawStaked() public nonReentrant {
        address _sender = msg.sender;
        require(isStaker[_sender],"UnStaked");
        StakeData memory _data = getStakeData[_sender];
        ...
        //从质押者列表中移除msg.sender
        getIndexAndDelOfAddr(staker,_sender);
        isStaker[_sender]=false;
        uint _profit=_data.stakeAmount/100*(stakeRate[uint8(_data.stakeOptions)]-100);


        //开始结算转账
        poolBalance -= _profit;
        FMT.pTransfer(_sender,_profit);
        FMT.basicTransfer(address(this),_sender,_data.stakeAmount);
        delete getStakeData[_sender];
    }
