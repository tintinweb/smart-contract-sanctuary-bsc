...
// File: contracts/SvgcNFTMarketV1.sol


pragma solidity ^0.8.0;
pragma abicoder v2;

contract SvgcNFTMarketV1 is ERC721Holder, Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function buyTokenUsingBNB(address _collection, uint256 _tokenId) external payable nonReentrant {
        // Wrap BNB
        IWETH(SVGC).deposit{value: msg.value}();

        _buyToken(_collection, _tokenId, msg.value, true);
    }
     */
    /**
     * @notice Buy token with SVGC by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     * @param _price: price (must be equal to the askPrice set by the seller)
     */
    function buyTokenUsingSVGC(
        address _collection,
        uint256 _tokenId,
        uint256 _price
    ) external nonReentrant {
        IERC20(SVGC).safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function claimPendingRevenue() external nonReentrant {
        ...
        IERC20(SVGC).safeTransfer(address(msg.sender), revenueToClaim);
    ...
    function recoverFungibleTokens(address _token) external onlyOwner {
      //  require(_token != SVGC, "Operations: Cannot recover SVGC");
        uint256 amountToRecover = IERC20(_token).balanceOf(address(this));
        require(amountToRecover != 0, "Operations: No token to recover");
     ...
     * @param _withBNB: whether the token is bought with BNB (true) or SVGC (false)
     */
    function _buyToken(
        address _collection,
        uint256 _tokenId,
        ...
        (uint256 netPrice,  uint256 creatorFee) = _calculatePriceAndFeesForCollection(
            _collection,
            _price
        );
        ...
        // Transfer SVGC
        IERC20(SVGC).safeTransfer(askOrder.seller, netPrice);
