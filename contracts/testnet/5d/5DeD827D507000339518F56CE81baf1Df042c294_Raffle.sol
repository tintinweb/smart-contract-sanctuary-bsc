// Open Front end to enter lottery
// Pick a random winner
// Select a winner every 'X' time
// Random number from chain link oracle
...
contract Raffle is VRFConsumerBaseV2, KeeperCompatible {
    ...
    /* Events */
    event RaffleEntered(address indexed rafflePlayer);
    event RequestRaffleWinner(uint256 requestId);
    event RaffleWinnerAnnounced(address indexed winner);
    ...
    function enterRaffle() public payable {
        if (msg.value < i_minRaffleFee) revert Raffle__NotEnoughEthSent();
    ...
    // will be called externally and not by contract
    function performUpkeep(
        bytes calldata /* performData */
    ) external override {
        (bool upkeepNeeded, ) = checkUpkeep("");
        //bool timePassed = (block.timestamp - s_lastTimeStamp) > i_interval;
        //if (!timePassed) revert Raffle_UpkeepNotNeeded();
        if (!upkeepNeeded) revert Raffle_UpkeepNotNeeded();
        s_raffleState = RaffleState.CALCULATING;

        uint256 requestId = i_vrfCoordinatorAddress.requestRandomWords(
            i_gasLaneAddress,
            i_subsId,
    ...
    function fulfillRandomWords(
        ...
        if (success) emit RaffleWinnerAnnounced(winner);
        else revert Raffle_TransferredFailed();
    }

    function cancelKeeper() public payable {
        i_vrfCoordinatorAddress.cancelSubscription(i_subsId, i_creator);
        s_raffleState = RaffleState.CLOSE;
        emit RaffleCancelled();
    }

    function getMinRaffleFee() public view returns (uint256) {
        return i_minRaffleFee;
    }
