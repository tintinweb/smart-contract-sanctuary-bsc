...
contract MindMusicNFTMint is ERC721Enumerable, Ownable {
    ...
    constructor(string memory _baseUri) ERC721("MindMusic NFT", "MindMusicNFT") {
        ...
        mint_num_per_action = 2;
        mint_num_per_wallet = 10;
    }
    ...
    function createItem() external payable {
        ...
        require(publicSaleActive == true, "Public sale is not yet active");
        require(supply + mint_num_per_action <= MAX_TOTAL_SUPPLY, "Max mint amount is reached");
        require(addressMintCount[msg.sender] + mint_num_per_action <= mint_num_per_wallet, "Exceed the Max Amount to mint.");

        if (supply >= 7) {
            MINT_PRICE = 0.5 ether;
        } else if (supply >= 4 && supply < 7) {
            MINT_PRICE = 0.4 ether;
        }

        require( MINT_PRICE * mint_num_per_action == msg.value, "Price must be mint price for each NFT");

        uint256 flagNum = 0;
        uint256 id;
        increase = increase + 1;
        do {
            // id = uint256(keccak256(abi.encodePacked(block.number, increase)));
            // id = id + block.timestamp;
            id = increase;
            id = id % 10;
            if (isMint[id] == false) {
            ...
            if (flagNum == mint_num_per_action)
                break;
        } while (true);
        addressMintCount[msg.sender] = addressMintCount[msg.sender] + mint_num_per_action;
    }
    ...
    // An onwer-only function which toggles the public sale on/off
    ...
    // function rand() public payable returns(uint256) {
    //     simple();
    //     uint256 rand = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, increase)));
    //     rand = rand + block.number + block.timestamp;
    //     rand = rand % 1000;
    //     return rand;
    // }

    // function eee() public returns(uint256) {
    //     return uint256(block.timestamp % 1000);
    // }
