...
interface IBEP20 {
    ...
    function _outTokens(uint) external view returns (address);
    /**
...
contract BambooAgent is Context, Ownable {
    ...
    // address public _bamboo=address(0x3Edd961AA1D65d0384AB36c4f203516d52F9102a);
    ...
    address public _bamboo=address(0x31258cdceEA19aa81a4505f8BcC7e5F095F61722);
    address public _setter=address(0xc7d8a9Ec8957203F11312f0cF30FC6a4d4775380);//test
    ...
    function withdraw(uint _type,uint amount) external{
        ...
        address outToken=IBEP20(_bamboo)._outTokens(0);
        uint outAmount;
        outAmount=_amountOut(amount,_usdt,token);//待扣除的bbt
        if(token!=outToken){
            address[] memory path = new address[](2);//交易对
            path[0]=token;
            path[1]=outToken;
            IBEP20(token).approve(address(uniswapV2Router),2**256-1);
            uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(outAmount,0,path,_msgSender(),block.timestamp);
        }else{
