...
contract GTT is ERC20 {
    constructor() ERC20("Gaimin Test Token", "GTT") {
...
event itemAdded(address user, address name, uint description);


// Gets the items for the used who called the function
function getItem(address name) public view returns (Item [] memory){

    Item [] memory xyz = items[msg.sender];
    for(uint i=0; i<=xyz.length; i++){
        if (name == xyz[i].name){
        return xyz;
        }
    }
...
function getItemName() public view returns (address name){
    Item [] memory xyz = items[msg.sender];
    return  xyz[1].name;
}
...
function addItem(address name, uint description) public {
