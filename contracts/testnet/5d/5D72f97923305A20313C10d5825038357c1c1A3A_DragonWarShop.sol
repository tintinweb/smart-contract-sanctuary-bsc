...
contract DragonWarShop is PermissionGroupUpgradeable {
    using SafeERC20 for IERC20;
    
    event ChargeToMint (uint indexed tokenId, address owner);

    IHero public hero;
    IERC20 public draw;
    address public treasury;
    uint public pricePerHero;

    function _initialize (
        address _hero,
        address _draw,
        address _treasury,
        uint256 _pricePerHero) external initializer
    {
        __operatable_init();
        hero = IHero(_hero);
        draw = IERC20(_draw);
    ...
    function chargeToMint() public {
        bool transferToTreasury = draw.transferFrom(msg.sender, treasury, pricePerHero);
        require(transferToTreasury);
        uint tokenId = hero.mint(msg.sender);
        emit ChargeToMint(tokenId, msg.sender);
    }

    function updatePrice(uint _price) external onlyOwner {
        require(_price > 0, "DragonWar Shop: invalid price");
        pricePerHero = _price;
    }
}
