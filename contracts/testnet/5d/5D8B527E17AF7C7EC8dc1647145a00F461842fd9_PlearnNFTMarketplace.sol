...
contract PlearnNFTMarketplace is IERC721Receiver, Ownable, ReentrancyGuard, Pausable {
    ...
    //Collection (ERC721)
    mapping(address => Collection) public collections; // Details about the collections
    ...
    function closeCollection(address collection) external collectionExists(collection) onlyOwner {
        collections[collection].status = Status.Close;
    ...
    function openCollection(address collection) external collectionExists(collection) onlyOwner {
        collections[collection].status = Status.Open;
    ...
    {
        if (collections[collection].status != Status.Open) {
            return (0, 0, 0);
        }
    ...
    {
        tradingFee = (price * collections[collection].tradingFee) / RATE_BASE;
        creatorFee = (price * collections[collection].creatorFee) / RATE_BASE;
    ...
    function _distributePayment(
        ...
        // Tranfer creator fee
        if (creatorFee != 0) {
            _transferERC20(dealToken, collections[collection].creatorAddress, creatorFee);
        }
    }
    ...
    modifier collectionExists(address collection) {
        require(collections[collection].status != Status.Pending, "Marketplace: Collection not listed");
        _;
    }

    modifier collectionOpen(address collection) {
        require(collections[collection].status == Status.Open, "Marketplace: Collection not listed or opened");
        _;
    }
