...
contract TradingPool is Ownable {
    ...
    uint256 public rebaseDuration = 200; // 1 days
    ...
    function userHashRate(address _pair, address _account) external view returns(uint256 _userHashRate) {
        ...
        _userHashRate = getCurrentHashRate(_user.hashRate, _user.lastRebased);
    }
    ...
    function enter(address _account, address _input, address _output, uint256 _amount) public onlySwapRouter returns(bool) {
        ...
        _amount = _amount.add(getBonus(_account, _amount));
        uint256 _addedHashRate = 0;
        if (addedPairs[_pair]) {
            ...
            if (_amount > 0) {
                ...
                if (totalAllocPoint > 0) {
                    _addedHashRate = _amount.mul(_pool.allocPoint) / totalAllocPoint;
                }
                _user.hashRate = _userHashRate.add(_addedHashRate);
                _user.lastRebased = rebaseDuration.mul(block.number / rebaseDuration);
                _user.rewardDebt = _user.rewardDebt.add(int256(_addedHashRate.mul(_pool.accPANPerHashRate) / ACC_PAN_PRECISION));
                _pool.totalHashRate = _pool.totalHashRate.add(_addedHashRate);
            }
            emit Deposit(_account, _pair, _addedHashRate);
            return true;
        }
        return false;
    }
