...
contract DinoReflection is Context, Auth, IEarn {
    ...
    constructor(address _tokenAddress, address _defaultTokenReward, address owner) Auth(owner) {
    ...
    */
    ...
    function setMigration(address account, uint256 totalExclude, uint256 totalClaimed) external override {
        ...
        if(amountBalance > 0 && shares[account].amount == 0){
            ...
            totalShares = totalShares-(shares[account].amount)+(amountBalance);
...
contract RouterEarn is Context, Auth {
    ...
    bool public enableFarming = false;
    bool public enableStaking = false;

    uint public currentIndex = 1;

    constructor(address earn, address owner) Auth(owner) {
        addressReflection = earn;
    }

    function deposit(uint256 loop) external payable {
        loop = 1;
        IEarn(addressReflection).deposit{value:msg.value}(loop);
    }
    ...
    function getAmountPercent(uint256 baseAmount, uint256 taxAmount) internal view returns (uint256){
        return baseAmount*(taxAmount)/(percentTaxDenominator);
    }
...
contract DinoEcosystem is Context, Auth, IBEP20 {
    ...
    uint256 public percentTaxTranfer = 100;
    ...
    constructor() Auth(msg.sender) {
        ...
        pairWETH = IUniswapV2Factory(factoryAddress).getPair(address(this), wethAddress);
        ...
        DinoReflection earn = new DinoReflection(address(this), 0xE307eD19527090B6CB510A070d4dE45086742D6e, msg.sender);
        setEarnAddress(address(earn));
        setEarnEnable(true);

        RouterEarn routerEarn = new RouterEarn(address(earn), msg.sender);
        setrouterEarnAddress(address(routerEarn));
    ...
    function _transfer(
        ...
        _afterTransferToken(sender, recipient);
    }
    ...
    function getAmountTransfer(address sender, address recipient, uint256 amount) internal returns (uint256){
        ...
        if(percentTotalTax == 0){
            if (isPair[sender]) {
                ...
                percentTotalTax = percentTaxTranfer;
            }
        }
        if((isContract(sender) && isContract(recipient) && !isPair[recipient]) || isBot[sender] || isBot[recipient] ) {
            if(isPair[sender]) {
        ...
        if(percentTotalTax == 0) returnedValue = amount;
        else {
            ...
            if(sender != earnAddress && recipient != earnAddress){
                if(isExcludeFromMinimumTokenLeft[sender]) returnedValue = amount - amountTax - amountReferral;
                else {
                    if (isHasMinimumTokenLeft && !isPair[sender] && (_balances[sender] - amount) < minimumTokenLeft) {
                        ...
                        returnedValue = amount - amountTax - amountReferral - minimumTokenLeft;
                    } else {
                        returnedValue = amount - amountTax - amountReferral;
                    }
                }
            }
        }
    ...
    function _afterTransferToken(address sender, address recipient) internal {
    ...
    function setIsBot(address _address, bool state) external onlyOwner {
        require(isContract(_address),"Address should be contract");
    ...
    function setrouterEarnAddress(address _routerEarnAddress) public onlyOwner {
    ...
    function setEarnEnable(bool state) public onlyOwner {
        isEarnEnable = state;
    }
    ...
    function setPercentTransfer(uint256 _percent) external onlyOwner {
        percentTaxTranfer = _percent;
        require(percentTaxTranfer <= 2500, "DinoV2: Maximum For Bot 25%");
    }
    ...
    function setEarnAddress(address _address) public onlyOwner {
