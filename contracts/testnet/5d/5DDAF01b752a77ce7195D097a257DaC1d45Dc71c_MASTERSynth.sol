...
contract MASTERSynth is Ownable, ReentrancyGuard {
        ...
        returns (uint256)
    {
        ...
        uint256 userWrapped = IERC20(pool.wtoken).balanceOf(_user);
        uint256 userWant = calculateSwapSTtoTokens(_pid, userWrapped);

        uint256 profitCheck = 0;
        uint256 RefComission = 0;
        ...
        if (RefComission > 0) {
            userFinalWant = userWant.sub(RefComission);
        } else {
    ...
    function deposit(
        ...
        if (_wantAmt > 0) {
            ...
            uint256 wrappedCreated = IStrategy(poolInfo[_pid].strat).deposit(
                msg.sender,
                _wantAmt
            );
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) public nonReentrant {
        ...
        if (unWrappedTokens > user.deposited) {
            ...
            if (user.referrer != address(0)) {
                ...
                RefComission = (RefComission.mul(totalSupply)).div(
                    wantLockedTotal
                );
            }
        }
        ...
        uint256 wanTokens = IStrategy(poolInfo[_pid].strat).withdraw(
            msg.sender,
            _wantAmt
        );
        ...
        if (wTokenBal > 0) {
            ...
            userInfo[_pid][user.referrer].totalBonus = userInfo[_pid][
                user.referrer
            ].totalBonus.add(wTokenBal);
        }
    ...
    function depositAll(address _referral, uint256 _pid) external {
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function changeStartBlock(uint256 _newstartBlock) public {
