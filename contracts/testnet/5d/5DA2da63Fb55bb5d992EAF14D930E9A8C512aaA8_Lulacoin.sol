 ...
 * the owner.
 *
 * The ONG acronym stands for "organização não governamental" (non-governmental organization)
 */
abstract contract Ownable is Context {
    ...
    address private _ongWalletAddress;
    address private _previousOngWalletAddress;

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    event OngWalletTransferred(address indexed previousOngWalletAddress, address indexed newOngWalletAddress);
    ...
    constructor() {
        ...
        _owner = msgSender;
        _ongWalletAddress = msgSender;
        _renouncedOwnership = false;

        emit OwnershipTransferred(address(0), msgSender);
        emit OngWalletTransferred(address(0), msgSender);
    }
    ...
    function ong() public view returns (address) {
        return _ongWalletAddress;
    }
    ...
    function setOngWalletAddress(address newOngWalletAddress) public virtual onlyOwner() {
        require(newOngWalletAddress != _ongWalletAddress, "Ownable: new ONG wallet address is the current ONG wallet address");
        emit OngWalletTransferred(_ongWalletAddress, newOngWalletAddress);
        _ongWalletAddress = newOngWalletAddress;
    }
...
contract Lulacoin is Context, IBEP20, Ownable {
    ...
    uint256 private constant _tTotal = 1 * (10**7) * (10**18);
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private constant _name = "Lulacoin";
    ...
    uint256 public _ongFee = 6;
    uint256 private _previousOngFee = _ongFee;
    ...
    event OngTaxUpdate(uint256 newOngTaxValue);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // transfer amount, it will take tax, burn, liquidity fee and ong fee
    ...
    */
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tOngFee) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        
        _takeFee(tLiquidity, tOngFee, rFee, tFee);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tOngFee) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tOngFee, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tOngFee);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tOngFee = calculateFee(_ongFee, tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tOngFee);
        return (tTransferAmount, tFee, tLiquidity, tOngFee);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tOngFee, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rOngFee = tOngFee.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rOngFee);
    ...
    // Set the ONG wallet fee percentage
    function setOngFeePercent(uint256 ongFee) external onlyOwner() {
        _ongFee = ongFee;
        emit OngTaxUpdate(ongFee);
    }
    ...
    function removeAllFee() private {
        if (_reflectionTaxFee == 0 && _liquidityFee == 0 && _ongFee == 0) return;
        
        _previousReflectionTaxFee = _reflectionTaxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousOngFee = _ongFee;
    ...
    */
    ...
    // Handles liquidity fee, ong fee and reflection fee. Code was reused, so merged into one function. 
    function _takeFee(uint256 tLiquidity, uint256 tCharityFee, uint256 rFee, uint256 tFee) private {
        _takeLiquidity(tLiquidity);
        _takeOngFee(tCharityFee);
    ...
    // Takes ong fee from transfer
    function _takeOngFee(uint256 tCharityFee) private {
        uint256 currentRate =  _getRate();
        uint256 rCharityFee = tCharityFee.mul(currentRate);
        
        _rOwned[ong()] = _rOwned[ong()].add(rCharityFee);
        
        if (_isExcluded[ong()])
            _tOwned[ong()] = _tOwned[ong()].add(rCharityFee);
    }
