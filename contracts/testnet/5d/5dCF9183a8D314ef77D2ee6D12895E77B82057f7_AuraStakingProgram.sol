...
contract AuraStakingProgram  {
    ...
    uint256 public tokenBusdPrice=4e16;
    uint256 public tokenBnbPrice=4e14;
    ...
    function start_staking(uint256 walletUsedBusd,uint256 _programId,address referrer) public 
    {
        ...
        // wallet balance 
        uint256 walletUsed=(walletUsedBusd.mul(1e9)).div(tokenBusdPrice);
    ...
    function admin_staking(uint256 walletUsedBusd,uint256 _programId,address user,address referrer) public 
    {
        require(msg.sender==owner,"Only Owner...!");
        ...
        if(users[user].totalStakingBusd<100*1e18 && (users[user].totalStakingBusd+_busdAmount)>=100*1e18)
        {
            users[users[user].referrer].qualifiedDirect+=1;
        }
        users[users[user].referrer].directInvestment+=_busdAmount;
        ...
        users[user].programs[programCount].staking = _busdAmount;
        ...
        users[user].programs[programCount].stakingToken = walletUsed;
        users[user].programCount = users[user].programCount.add(1);
        
        users[user].totalStakingToken = users[user].totalStakingToken.add(walletUsed);
        users[user].totalStakingBusd = users[user].totalStakingBusd.add(_busdAmount);
        users[user].currentPercent=getStakingPercent(user);
        address referrerAddress=users[user].referrer;
        
        uint256 refBonus=(walletUsed.mul(REFERRAL_PERCENTS[0])).div(100000);
        AuraToken.transfer(referrerAddress,refBonus);
        emit ReferralReward(referrerAddress,user,refBonus,1,1);

        uint256 passiveIncome=(walletUsed.mul(users[user].currentPercent)).div(1000000);
        ...
        if(referrerAddress!=address(0))
        {
            for(uint8 i=1;i<10;i++)
            {
                ...
                {
                    refBonus=(walletUsed.mul(REFERRAL_PERCENTS[i])).div(100000);
                    emit ReferralReward(referrerAddress,user,refBonus,i+1,1);
	    ...
	    emit CycleStarted(user,users[user].programCount, walletUsedBusd,walletUsed);
    }


    function BuyFromBUSD(uint256 tokenQty) public payable
	{
	     ...
	     uint256 buy_amt=(tokenQty/1e9)*tokenBusdPrice;	  
	...
	function withdraw() public payable 
	{
        ...
        if(withdrawalAmount>0)
        {
            ...
            total_withdraw_busd=total_withdraw_busd+(withdrawalAmount.mul(tokenBusdPrice.div(1e18)));
    ...
    function updateRewards(address user) private
	{
        uint256 uid = users[user].id;
        require(uid != 0, "Can not withdraw because no any stakings");
        
        for (uint256 i = 0; i < users[user].programCount; i++) 
        {
            if (users[user].programs[i].isExpired) {
                continue;
            }

            Program storage program = stakingPrograms_[users[user].programs[i].programId];

            bool isExpired = false;
            uint256 withdrawalDate = block.timestamp;
            if (program.term > 0) {
                uint256 endTime = users[user].programs[i].stakingDate.add(program.term);
                if (withdrawalDate >= endTime) {
                    withdrawalDate = endTime;
                }
            }
            ...
            uint256 stakingPercent;
            if(users[user].programs[i].programId==0)
            stakingPercent=users[user].currentPercent;
            else
            stakingPercent=stakingPrograms_[users[user].programs[i].programId].dailyInterest;
            
            uint256 amount = _calculateRewards(users[user].programs[i].stakingToken , stakingPercent , withdrawalDate , users[user].programs[i].lastWithdrawalDate , stakingPercent);

            users[user].programs[i].lastWithdrawalDate = withdrawalDate;
            users[user].programs[i].isExpired = isExpired;
            users[user].programs[i].currentRewards += amount;
            users[user].programs[i].genRewards += amount;
        }
    }
    ...
    function updatePrice(uint256 busdprice,uint256 bnbprice) public payable
    {
       require(msg.sender==owner,"Only Owner");
       tokenBusdPrice=busdprice;
       tokenBnbPrice=bnbprice;
    }
