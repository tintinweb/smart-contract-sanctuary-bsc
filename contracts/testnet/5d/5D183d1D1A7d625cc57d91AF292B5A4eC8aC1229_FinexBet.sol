...
contract FinexBet is Ownable {
    ...
    function getPoolAmount(uint256 _eventId) external view returns (uint256) {
        return (((100 - events[_eventId].platformFeePercent) * events[_eventId].meta.poolAmount) / 100);
    }
     ...
     */
    function placeBet(uint256 _eventId, uint256 _prediction) external {
       ...
       // require(currEvent.meta.ticketCost >= msg.value, "Not received ticket cost");
        ...
        require(IERC20(tokenContract).approve(address(this), currEvent.meta.ticketCost), "Kindly appove us to transfer tokens on your behalf.");
        require(IERC20(tokenContract).transferFrom(msg.sender, address(this), currEvent.meta.ticketCost), "Not received ticket Cost");

        currEvent.participants.push(msg.sender);
        currEvent.prediction[msg.sender].prediction = _prediction;
        currEvent.prediction[msg.sender].betAmount = currEvent.meta.ticketCost;
        emit Prediction(_eventId, msg.sender, _prediction);

        currEvent.meta.poolAmount += currEvent.meta.ticketCost;
    }
    ...
    function endEvent(
        ...
        uint256 platformFeeTokens = ((currEvent.platformFeePercent * currEvent.meta.poolAmount) / 100);
