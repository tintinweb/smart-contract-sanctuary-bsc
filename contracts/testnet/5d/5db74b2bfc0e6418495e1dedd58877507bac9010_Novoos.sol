...
// Dependency file: contracts/NOVODividendTracker.sol
...
contract NOVODividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NOVO contract."
        );
    }
...
// Root file: contracts/AntiBotNovo.sol
...
// import "contracts/NOVODividendTracker.sol";
// import "contracts/BaseToken.sol";

contract Novoos is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    NOVODividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = NOVODividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "NOVO: The dividend tracker already has that address"
        );

        NOVODividendTracker newDividendTracker = NOVODividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "NOVO: The new dividend tracker must be owned by the NOVO token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "NOVO: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "NOVO: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "NOVO: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "NOVO: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "NOVO: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "NOVO: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialBUSDBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );
