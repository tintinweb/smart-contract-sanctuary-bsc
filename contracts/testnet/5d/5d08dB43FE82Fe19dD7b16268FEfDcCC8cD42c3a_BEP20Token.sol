...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  uint256 public purchaseAmount = maxSell; // Limita a compra minima de moedas por transação.
  ...
  event SetpurchaseAmount(uint256 purchaseAmount);

  constructor() public {
    _name = "tested";
    _symbol = "tst";
    _decimals = 18;
    _totalSupply = 600000000e18;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(amount > purchaseAmount, "Transfer amount must be less than the purchaseAmount!");
    require(amount < maxSell, "Transfer amount must be greater than the maxSell!");
    ...
    function setMaxSell (uint256 _maxSell) external onlyOwner {
        maxSell = _maxSell * 10**8;
        emit SetMaxSell(_maxSell);
    }

    function setpurchaseAmount (uint256 _purchaseAmount) external onlyOwner {
        purchaseAmount = _purchaseAmount * 10**8;
        emit SetpurchaseAmount(_purchaseAmount);
    }
