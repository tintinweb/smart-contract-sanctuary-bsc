...
contract GEMUNILeasing is PermissionGroupUpgradeable, IGEMUNILeasing {
    ...
    mapping(address => bool) public gemuniItems;
    ...
    function addGemuniItems(address _gemuniItem) external onlyOperator {
        require(_gemuniItem != address(0) && !gemuniItems[_gemuniItem], "Leasing: invalid address");
    ...
    function setInput(address _geniPass, address _treasury, address _exchange) external onlyOperator {
        require(_treasury != address(0) && _geniPass != address(0) && _exchange != address(0), "Leasing: invalid address");
        treasury = _treasury;
        geniPass = _geniPass;
        exchange = _exchange;
    ...
    function putOnLeasing(LeasingCreate[] memory inputs) public override {
        for (uint i = 0; i < inputs.length; i++) {
            ...
            require(IERC721(nftToken).ownerOf(tokenId) == msg.sender, "Leasing: not token owner");
            require(minDuration > 0 && maxDuration > minDuration, "Leasing: invalid input");
            require(pricePerWeek > 0, "Leasing: invalid price");
            if (nftToken != geniPass && !gemuniItems[nftToken]) {
                ...
                require(lenderAddress == address(0) && guildAddress == address(0),"Leasing: already put on leasing");
    ...
    function registerLeasing(LeasingRegister[] memory inputs, uint rentalPrice) external override {
        ...
        require(inputs.length >= minNFT, "Leasing: invalid number of NFT");
        uint totalRental;
        for( uint i = 0; i < inputs.length; i++) {
            address nftToken = inputs[i].nftToken;
            uint tokenId = inputs[i].tokenId;
    ...
    function register(address nftToken, uint tokenId, uint duration, uint minDuration, uint maxDuration, uint pricePerWeek) internal returns (uint rentalAmount){
        ...
        require(leasing.lender == treasury, "Leasing: invalid token");
    ...
    function updateLeasing(address nftToken, uint tokenId, uint newMinDuration, uint newMaxDuration, uint newPricePerWeek) external override {
        ...
        require(IERC721(nftToken).ownerOf(tokenId) == address(this) && renter.renter == address(0), "Leasing: already rented");
    ...
    function cancelLeasing(address nftToken, uint tokenId) external override {
        ...
        if (msg.sender == guild.gameGuild && leasing.lender == address(0)) {
            IERC721(nftToken).transferFrom(address(this), guild.gameGuild, tokenId);
            delete leasingGuildInfo[nftToken][tokenId];
        } else if (msg.sender == guild.gameGuild && leasing.lender != address(0)) {
            delete leasingGuildInfo[nftToken][tokenId];
         }else if (operators[msg.sender] || msg.sender == leasing.lender && leasing.lender != treasury) {
            require(leasing.price > 0, "Leasing: not on lending");
            if (leasing.lender == treasury) {
    ...
    function rentFromLeaser(LeasingInfo storage leasing, uint duration, uint price) internal returns(uint rentalPriceGeni) {
        require(msg.sender != leasing.lender, "Leasing: Lender can not rent");
    ...
    function rentMore(address nftToken, uint tokenId, uint addedDuration, uint addedRentalPrice) external override {
        ...
        RenterInfo storage renter = renterInfo[nftToken][tokenId];
        LeasingGuildInfo storage guild = leasingGuildInfo[nftToken][tokenId];
        require(addedDuration > 0,"Leasing: invalid addedDuration");
        uint addedAmount;
        address lender;
    ...
    function finishRenting(address nftToken, uint tokenId) external override {
        ...
        //renter return pass to guild + admin/user
            ...
            //renter return pass to leaser(admin/user) 
            } else {
                if (nftToken == geniPass) {
                    IGENIPass(geniPass).unLockPass(tokenId);
                    IGENIPass(geniPass).setActive(tokenId, false);
    ...
    function finishLeasing(address nftToken, uint[] memory tokenIds) external override {
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            require(msg.sender == guild.gameGuild || operators[msg.sender] || msg.sender == leasing.lender && leasing.lender != treasury, "Leasing: invalid ");
            require(renter.renter != address(0) || guild.gameGuild != address(0),"Leasing: not found lending info");
            if (msg.sender == guild.gameGuild) {
                ...
                require(leasing.lender != address(0), "Leasing: not put on leasing");
                leaserRetrieveNFT(nftToken, tokenIds[i], renter, leasing);
            }
        }
    }
    ...
    function withdrawNFT(address nftToken, uint tokenId, address to) external onlyOperator {
        require(IERC721(nftToken).ownerOf(tokenId) == address(this), "Leasing: not found token in contract");
