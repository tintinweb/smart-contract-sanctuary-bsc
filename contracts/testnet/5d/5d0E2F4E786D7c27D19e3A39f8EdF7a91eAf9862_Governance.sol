...
contract Governance is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function accept(address _loan, uint256 _borrowAmount) public onlyWhiteListedBorrower notBlackListed nonReentrant {
    ...
    function getAllAvailableLoans() public view returns (CreatedLoanStruct[] memory loans){

        uint256 i = 0;
        uint256 aI = 0;
        while(i < loanArray.length){
            ILoan loan = ILoan(loanArray[i]);
            uint256 availableAmount = IERC20(loan.token()).balanceOf(address(loan));
            if(loan.status() > 0 && availableAmount > 0){
                aI++;
            }
            i++;
        }

        loans = new CreatedLoanStruct[](aI);
        i = 0;
        aI = 0;
        ...
        while(i < loanArray.length){
            ...
            if(loan.status() > 0 && availableAmount > 0){
                loans[aI].loan = address(loan);
                loans[aI].token = loan.token();
                loans[aI].availableAmount = availableAmount;
                loans[aI].tokenAmount = loan.tokenAmount();
                loans[aI].duration = loan.duration();
                loans[aI].paymentPeriod = loan.paymentPeriod();
                loans[aI].aPRInerestRate = loan.aPRInerestRate();
    ...
    function getTotalAvailablefunds() public view returns (uint256 totalAmount){
...
interface ILoan {
    ...
    function paymentPeriod() external view returns (uint64);
