...
contract ShimaCryptoFantasy is Ownable, AccessControl {
    ...
    {
        uint256[] memory allPools = new uint256[](poolCounter);
        uint256 activeCount = 0;
        for (uint256 i = 0; i < poolCounter; i++) {
            allPools[activeCount] = i;
            if (pools[i].endTime > block.timestamp) {
                activeCount++;
            }
        }
        return (allPools, activeCount);
    }
    ...
    function getWinnerInfo(uint256 _poolID)
    external
    view
    returns (Winners memory)
    {
        return winnerDetails[_poolID];
    }
}
