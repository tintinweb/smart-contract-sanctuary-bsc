...
contract Yyy is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = "Yyy Token";
    string public _symbol = "YYY";
    ...
    uint256 public yyyInsuranceFundFee = 50;
    uint256 public sellFee = 20;
    uint256 public firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(yyyInsuranceFundFee).add(firePitFee);
    ...
    address public yyyInsuranceFundReceiver;
    ...
    constructor() ERC20Detailed("Yyy", "YYY", uint8(DECIMALS)) Ownable() {
        //testnet
        ...
        autoLiquidityReceiver = 0x98c1C148dA80115f9BA366E2327bDF1D391c4626;
        treasuryReceiver = 0x427fD19b611eA34CE731B88d7e9f6C11BAE2EB7D;
        yyyInsuranceFundReceiver = 0x68Eae235Cf46Ec46c90cEc89DF0550ed2C5e1b80;
        ...
        _initRebaseStartTime = block.timestamp + 3600000;
        _lastRebasedTime = block.timestamp;
        _autoRebase = true;
        _autoAddLiquidity = true;
        _isFeeExempt[treasuryReceiver] = true;
        //_isFeeExempt[yyyInsuranceFundReceiver] = true;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(
                _treasuryFee.add(yyyInsuranceFundFee)
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndXIF.mul(treasuryFee).div(
                treasuryFee.add(yyyInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(yyyInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndXIF.mul(yyyInsuranceFundFee).div(
                treasuryFee.add(yyyInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    //     require(
    //         amountToSwap > 0,
    //         "There is no yyy token deposited in token contract"
    //     );
    //     address[] memory path = new address[](2);
    //     path[0] = address(this);
    //     path[1] = router.WETH();
    ...
    //     address _yyyInsuranceFundReceiver,
    //     address _firePit
    // ) external onlyOwner {
    //     autoLiquidityReceiver = _autoLiquidityReceiver;
    //     treasuryReceiver = _treasuryReceiver;
    //     yyyInsuranceFundReceiver = _yyyInsuranceFundReceiver;
    //     firePit = _firePit;
    // }
        ...
        onlyOwner
    {
        ...
        require(
            ...
            "cannot blacklist yyy Token, Pair and Router PancakeSwap"
        );
