...
contract metaminerengine is Context, IBEP20, Ownable {
  ...
  function claimRedPack(address packcontract) public returns (uint256) {
    require( _ready == true,"BEP20: contract does not setup");
    require( _chest[msg.sender] > 0,"BEP20 : require to mint chest first");
    IBEP20 a = IBEP20(packcontract);
    a.transfer(msg.sender,0);
  ...
  function claimWhitePack(address packcontract) public returns (uint256) {
    require( _ready == true,"BEP20: contract does not setup");
    require( _chest[msg.sender] > 0,"BEP20 : require to mint chest first");
    IBEP20 a = IBEP20(packcontract);
  ...
  function mining() public returns (bool) {
    ...
    require( mine > ran,"BEP20: coral are too low");
    mine = mine.div(ran);
  ...
  function updateGameEcosystem(uint256 chest,uint256 pickaxe,uint256 miner,uint256 uplv,uint256 marketfee,uint256 equip,uint256 lv,uint256 rw,uint256 rate,bool mintable,uint256 cap,bool market,uint256 minclaim) public onlyOwner returns (bool) {
    _mintfee_chest = chest;
    _mintfee_pickaxe = pickaxe;
    _mintfee_miner = miner;
    _mintfee_uplevel = uplv;
    _mintfee_market = marketfee;
    _mintfee_TokenEquip = equip;
    _maxlevel = lv;
    _rewardrate = rw;
    _goldmintrate = rate;
