...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    function bid(uint256 _saleId, bytes32[] memory _merkleProof, uint256 _amount) external payable nonReentrant saleValid(_saleId) {
        ...
        uint256[] memory tokenIdsBought; // use to pack
        ...
        if (!_project.isSaleWithPacks && _project.isInstantPayment) {
            ...
            tokenIdsBought = _bidPack(_project, sale, _amount);
        }

        project.updateSold(sale.projectId, soldAmount);
        _sharing(_project, sale, tokenIdsBought, _amount, _price); 
    }

    function _bidPack(ProjectInfo memory _project, SaleInfo storage _sale, uint256 _amount) private returns(uint256[] memory) {
        ...
        uint256[] memory tokenIdsBought;

        if (_project.isSingle) {
            tokenIdsBought = new uint256[](_amount);
            for (uint256 i; i < amountSent; i++) {
                ...
                tokenIdsBought[i] = (_sale.warePackTokenIds[sentByIndex - i]);
                _sale.warePackTokenIds.pop();
            }
        } else {
            tokenIdsBought = new uint256[](_sale.warePackTokenIds.length);
            for (uint256 i; i < tokenIdsBought.length; i++) {
        ...
        return tokenIdsBought;
    }

    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, uint256[] memory _tokenIdsBought, uint256 _amount, uint256 _price) private {
        ...
        if (_project.minSales == 0 && _project.isInstantPayment) {
            ...
            if (_project.isSaleWithPacks) {
                if (_project.isSingle) {
                    for (uint256 i; i < _tokenIdsBought.length; i++) {
                        billInfo.packIds.push(_tokenIdsBought[i]);
                    }
                } else {
                    for (uint256 i; i < _tokenIdsBought.length; i++) {
                        if (!tokenMultiIsBoughts[_sale.id][_msgSender()][_tokenIdsBought[i]]) {
                            billInfo.packIds.push(_tokenIdsBought[i]);
                            tokenMultiIsBoughts[_sale.id][_msgSender()][_tokenIdsBought[i]] = true;
                        } 
                    }
                }
            }
        ...
        bidEvent.tokenIds = _tokenIdsBought;
