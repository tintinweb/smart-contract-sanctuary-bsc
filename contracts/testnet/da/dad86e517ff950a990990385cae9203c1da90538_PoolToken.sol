...
contract PoolToken is Ownable {
    ...
    /// `lockTime` The seconds of lockTime.
    ...
    event LogSetXTokenAddress(address);
    ...
    /// @param _lockTime The seconds of lockTime.
    /// @param _depositLimit The minimum amount to deposit.
    function addPool(IERC20 _lpToken, uint256 _lockTime, uint256 _depositLimit, uint256 _firstDepositLimit) public onlyOwner {
        ...
        emit LogAddPool(lpToken.length.sub(1), _lpToken, _lockTime, _depositLimit, _firstDepositLimit);
    }
    ...
    function setXTokenAddress(address _xTokenAddress) public onlyOwner {
        xTokenAddress = _xTokenAddress;
        emit LogSetXTokenAddress(_xTokenAddress);
    }
    ...
    function deposit(uint256 _poolId, uint256 _depositLpAmount) public {
        ...
        userDepositRecordList[msg.sender][_poolId].push(
            DepositRecord({
                ...
                startBlockTimestamp: lastBlockTimestamp,
                endBlockTimestamp: lastBlockTimestamp.add(pool.lockTime),
        ...
        emit LogDepositLp(_poolId, msg.sender, _depositLpAmount, lastBlockTimestamp, lpPrice, userDepositPrice);
    }
    ...
    function withdraw(uint256 _poolId, uint256 _depositRecordId) public {
        ...
        require(userDepositRecord.endBlockTimestamp <= block.timestamp, "Time has not arrived");
