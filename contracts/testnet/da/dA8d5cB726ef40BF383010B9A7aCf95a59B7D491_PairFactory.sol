...
contract PairFactory is Ownable{
    ...
    function createSale(
        ...
        uint256 totalToken = _feesCount(_rateSettings[0] , _rateSettings[1] , _capSettings[1] , _liquidityPercent[0] , _feeSettings[0] , IERC20(token).decimals());
        ...
        IPoolManager(poolManager).payAmaPartner( 
            _partnerAddress,
            pair
        );
    ...
    function _feesCount(uint256 _rate , uint256 _Lrate , uint256 _hardcap , uint256 _liquidityPercent , uint256 _fees , uint8 _decimal )  internal pure returns (uint256){
        uint256 totalToken = ((_rate * _hardcap / 10**_decimal)).add(((_hardcap * _Lrate / 10**_decimal) * _liquidityPercent) / 100);
        uint256 totalFees = (((_rate * _hardcap / 10**_decimal)) * _fees / 100);
