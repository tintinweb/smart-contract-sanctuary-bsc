...
contract MidasTokenGenerator is Initializable {
    ...
    function setFunctionAvailable(string memory functionName, bool value)
        public
        onlyOwner
    {
        require(
            keccak256(abi.encodePacked(functionName)) !=
                keccak256(abi.encodePacked("setFunctionAvailable")),
            "Cant disabled this function to prevent heart attacks!"
        );
        require(
    ...
    function enableDisableNetwork(string memory name, bool status)
        public
        onlyOwner
    {
    ...
    function updateCreationPrice(uint256 amount) public onlyOwner {
        require(
            !availableFunctions["updateCreationPrice"],
            "Function disabled"
        );
        creationTokenPrice = uint256(
            ((amount**uint256(decimalsDataFeed())) /
                uint256(getLatestPrice())) * 1e18
        );
    }

    function updatePriceDataFeed(address newValue) public onlyOwner {
        require(
            !availableFunctions["updatePriceDataFeed"],
            "Function disabled"
        );
        priceFeed = AggregatorV3Interface(address(newValue));
    }

    function getPathForTokenETH(address tokenAddress)
        private
        view
    ...
    function createNewToken(
        ...
        if (
            ...
            if (paymentTokenAddress != pcsV2Router.WETH()) {
                uint256 requiredTokenAmount = getEstimatedTokensForETH(
                    paymentTokenAddress,
                    creationTokenPrice
                );
    ...
    function getEstimatedTokensForETH(address tokenAddress, uint256 ethAmount)
        public
        view
        returns (uint256)
    {
        return
            pcsV2Router.getAmountsIn(
                ethAmount,
...
contract Token is Context, IERC20, Ownable {
    ...
    function setMaxWalletPercent(uint256 maxWalletPercent) external onlyOwner {
        require(
            maxWalletPercent >= minMxWalletPer && maxWalletPercent <= 100,
            "err"
        );
