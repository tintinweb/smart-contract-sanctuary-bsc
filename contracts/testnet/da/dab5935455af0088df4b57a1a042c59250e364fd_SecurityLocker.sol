...
contract SecurityLocker is Ownable, ReentrancyGuard {
  ...
  modifier modifierCreateLocker(
    ...
    require(address(personallockers[_wallet1][key1_address][key1_Id]) == address(0), "already created locker");
    _;
  }
  ...
  // event AddPersonalLockerWhiteList(address lockerAddr, address receiver);
  // event RemovePersonalLockerWhiteList(address lockerAddr, address receiver);
  ...
  ) external nonReentrant modifierCreateLocker(msg.sender, key1_address, key1_Id, _wallet2, key2_address, key2_Id) {
  ...
  function withdrawUtility(
    ...
    uint256 _fee = amounts.mul(_feeRate).div(standardization);
    PersonalLocker locker = lockers[lockerAddr];
    // locker.withdrawUtility(msg.sender, amounts.sub(_fee));
    locker.withdrawUtility(receiver, amounts.sub(_fee));
    // locker.withdrawUtility{ value: _fee }(payable(feeWallet));
    locker.withdrawUtility(address(this), _fee);
  ...
  //   lockers[lockerAddr].addWhiteList(receiver);
  //   emit AddPersonalLockerWhiteList(lockerAddr, receiver);
  // }

  // function removePersonalWhiteList(address lockerAddr, address receiver) external isAuth(lockerAddr) {
  //   lockers[lockerAddr].removeWhiteList(receiver);
  //   emit RemovePersonalLockerWhiteList(lockerAddr, receiver);
  // }
  ...
  View Function
   */
  function lockerInfoByLocker(address lockerAddr) external view returns (VerifyProps memory) {
    return lockers[lockerAddr].showInfo();
  }

  function lockersInfoByWallet(address account, uint8 isMain) external view returns (VerifyProps[] memory) {
    //0:backup 1:main
  ...
  function mainLockerCounts(address account) external view returns (uint256) {
    return mainAccount[account].length;
  }

  function backupLockerCounts(address account) external view returns (uint256) {
