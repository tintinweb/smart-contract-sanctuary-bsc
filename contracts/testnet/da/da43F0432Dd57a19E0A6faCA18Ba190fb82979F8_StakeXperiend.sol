	...
	ReentrancyGuardUpgradeable
{
	...
	function createStake(
		...
		currentIdStake++;
		StakeInfo memory s = StakeInfo(_APR, _maxCupo, _endTime, _timeLock, 0);
		stakes[currentIdStake] = s;
		emit StakeCreated(_APR, _maxCupo, _endTime, _timeLock, currentIdStake);
	}

	function infoStakeByUser(uint256 _stakeId, address _user)
		public
		view
	...
	function updateStake(
		...
		stakes[id] = s;
		emit StakeUpdated(_APR, _maxCupo, _endTime, _timeLock, id);
	}

	function getRewards(uint256 _stakeId)
		public
		isInStake(msg.sender, _stakeId)
		nonReentrant
	{
		require(
			block.timestamp > stakes[_stakeId].timeLock,
	...
	function updatePendingReward(address _user, uint256 _stakeId) internal {
		if (userStakes[_user].length > 0) {
			uint256 i = indexForUserInfoStakeById(_user, _stakeId);
			if (block.timestamp > stakes[_stakeId].endTime) {
	...
	function exit(uint256 _stakeId)
		public
		isInStake(msg.sender, _stakeId)
		nonReentrant
	{
		...
		UserStakeInfo memory usi = UserStakeInfo(0, 0, 0, 0, 0);
	...
	function withdrawTokens(uint256 _amount, address _recipient)
		public
		onlyOwner
	{
	...
	function indexForUserInfoStakeById(address _user, uint256 _stakeId)
		internal
		view
	...
	function isUserInStake(address _user, uint256 _stakeId)
		public
		view
		returns (bool itIs)
	{
	...
	function userStake(uint256 _balance, uint256 _stakeId) public {
		...
		require(
			stakes[_stakeId].currentCupo < stakes[_stakeId].maxCupo,
