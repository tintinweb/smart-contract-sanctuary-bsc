...
// DreamIdol Contracts (DIExpedition_New_V3.sol)
 ...
 */
contract DIExpedition_New_V3 is
    Initializable,
    ContextUpgradeable,
    PausableUpgradeable,
    ...
    ERC1967UpgradeUpgradeable
{
    ...
    DIExpedition_V2_Config _config;
    ...
    function setConfigAddress(address configaddr) external {
        require(hasRole(MANAGER_ROLE, _msgSender()), "DI1");

        _config = DIExpedition_V2_Config(configaddr);
    }
    ...
    function _setupEquipedCharacterTransfer(
        ...
        DIStarNFT _starNft = _config._starNft();
    ...
    function setupEquipedCharacter(
        ...
        (nftData,fansData,fansNFTId,nftCodec) = _setupEquipedCharacterTransfer(
            starNftCharId,
        ...
        DIStar1155 _star1155 = _config._star1155();
    ...
    function dismissEquipedCharacter(uint8 equipedCharIndex) external {
        ...
        //_config._deleteEquipedCharacter(equipedCharIndex);
        delete ecs[equipedCharIndex];
    }

    function recalcEqpChaCP(uint64 fansNFTId, uint8 equipedCharIndex) external {
        EquipedCharacter[] storage ecs = _equipedChars[_msgSender()];
        require(equipedCharIndex < ecs.length, "DIg");
        EquipedCharacter storage ec = ecs[equipedCharIndex];
        require(ec.role > 0, "DIh");

        _recalcEqpChaCP(fansNFTId, ec);
    }

    function _recalcEqpChaCP(uint64 fansNFTId, EquipedCharacter storage ec) internal {
        DIStarNFT _starNft = _config._starNft();
        DIStar1155 _star1155 = _config._star1155();
        IDIStarNFTCodec nftCodec = IDIStarNFTCodec(_starNft.getCodec());
        IDIStar1155Codec codec = IDIStar1155Codec(_star1155.getCodec());

        DIStarNFTDataBase memory nftData = _starNft.getNftData(ec.starNftCharId);
        DIStarCharacter memory cd = nftCodec.getCharacterData(nftData);
        ...
        DIFansNFTData memory fansData = _config._fansNFT().getNftData(fansNFTId);
        //console.log("[sol]setupEquipedCharacter totalCombatPower fansData",ec.totalCombatPower,fansData.sp);
        //require(ec.totalCombatPower <= fansData.sp, "DIExpedition: support power not enough");
        if (ec.totalCombatPower > fansData.sp) {
            ec.totalCombatPower = fansData.sp;
    ...
    function addEquipToCharacter(
        ...
        // get fans nft data
        DIStar star = DIStar(_config._starFactory().getDIStarAddress(ec.role));
    ...
    function startExpedition(uint8 equipedCharIndex, uint32 levelId) external {
        ...
        ExpeditionLevelConfig memory conf = _config.getExpeditionLevelConfig(
            levelId
        );
    ...
    function fillDreamPoint(uint8 equipedCharIndex, uint256 dpValue) external {
        ...
        DITokens1155 _token1155 = _config._token1155();
        require(
    ...
    function finishUserRound(uint32 levelId, uint64 round) external {
        ...
        if (conf.role > 0) {
            address starAddr = _config._starFactory().getDIStarAddress(
                conf.role
            );
        ...
        userStatus.dreamCoinGained =
            ...
            //console.log("dreamCoinGained,userStatus.dreamCoinGained",dreamCoinGained,userStatus.dreamCoinGained);
        ...
        // _config._starPool().giveDreamCoins(
        //     _msgSender(),
        //     userStatus.dreamCoinGained
        // );
        _config._starPool().mintDreamCoins(
            _msgSender(),
            userStatus.dreamCoinGained,
            "expedition"
        );
        ...
        returns (uint8 index, EquipedCharacter storage ec)
    {
        ...
        require(
            ecs.length < 128 &&
                (ecs.length <
                    _config.getMaxEquipedCharactersByFansSP(fansData.sp) ||
        ...
        )
    {
        ...
        //console.log("levelStatus.endBlock <= block.number",levelStatus.endBlock , block.number);
        // new round
