...
contract BostonBNB is Ownable {   
  ...
  uint public bostonPrice = 20000; // 20000 Boston per bnb
 ...
 // mapping (address => InvestorReferral) public investorreferrals;
  ...
  constructor() {
    tariffs.push(Tariff(182 * oneDay, 315, 0.0002*(10**18)));
    tariffs.push(Tariff(182 * oneDay, 378, 0.031*(10**18)));
    tariffs.push(Tariff(182 * oneDay, 630, 0.061*(10**18)));

    // tariffs.push(Tariff(182 * oneDay, 315, 0.02*(10**18)));
    // tariffs.push(Tariff(182 * oneDay, 378, 3.1*(10**18)));
    // tariffs.push(Tariff(182 * oneDay, 630, 6.1*(10**18)));
  ...
  function updateLevelBusiness(uint amount) internal {
    ...
    for (uint i = 0; i < refRewards.length; i++) {
      ...
      investors[rec].levelBusiness += amount;
      if(investors[rec].levelBusiness >= 100*(10**15)) { // minimum 100 bnb for rank bonus
        if(investors[rec].levelBusiness >= 500*(10**15) && investors_income[rec].kingRankBonus == 0){
          uint bonus = (500*(10**15) * 15)/(100*10); //1.5 percent 
        ...
        else if(investors[rec].levelBusiness >= 300*(10**15) && investors_income[rec].ambassadorRankBonus == 0){
          uint bonus = (300*(10**15) * 20)/(100*10);//2 percent 
          investors_income[rec].rankBonus += bonus; 
          investors_income[rec].ambassadorRankBonus = bonus;
          investors_time[rec].ambassadorClubAchieveAt = block.timestamp;
          
        }
        else if(investors[rec].levelBusiness >= 200*(10**15) && investors_income[rec].diamondRankBonus == 0){
          uint bonus = (200*(10**15) * 30)/(100*10); //3 percent 
          investors_income[rec].rankBonus += bonus;
          investors_income[rec].diamondRankBonus = bonus;
          investors_time[rec].diamondClubAchieveAt = block.timestamp;
        }
        else if(investors[rec].levelBusiness >= 100*(10**15) && investors_income[rec].eagleRankBonus == 0){
          uint bonus = (100*(10**15) * 35)/(100*10); //3.5 percent
          investors_income[rec].rankBonus += bonus;
  ...
  function invest(uint tariff, address referer) external payable {
      ...
      uint bostonToken = msg.value * bostonPrice;
  ...
  function upgradePackage() external payable {
      ...
      require(bnbAmount >= 5 *(10**15),"Minimum Investment");
      ...
      uint cashbackAmt = msg.value*cashback()/100;
  ...
  function compound() external  {
    ...
    if(investors[msg.sender].oneWeekcompoundCount==7){
      ...
      investors_income[msg.sender].compoundBonus = 0;
 ...
 function compoundData(address user) public view returns (uint blockTime,bool timeCondition,bool sameDayCondition,bool elseCondtion) {
    
    if((investors_time[user].nextCompoundAt > block.timestamp)){
      timeCondition = true;
    }
    else if((investors_time[user].nextCompoundAt - block.timestamp) <= oneDay){
      sameDayCondition = true;
    }
    else{
      elseCondtion = true;
    }
    return (block.timestamp,timeCondition,sameDayCondition,elseCondtion);
  ...
  function roi(address user) public view returns (uint amount) {
  ...
  function unstake(uint index) external {
    ...
    if((block.timestamp - dep.stakeAt) > (365*oneDay)){
      ...
      amount =  dep.stakeAt*80/100;
      // dev Fee
      uint DevFeeAmount = dep.stakeAt*20/100; 
  ...
  function myData() public view returns (uint,bool,bool){
      ...
      uint withdrawableBerry = roi(msg.sender);
      ...
      return (withdrawableBerry,withdrawBtn,compoundBonusWillCount);
  }
  ...
  function withdrawal() external {
    ...
    uint remainingAmt = amount - DevFeeAmount - referralFeeAmt;
  ...
  function emergencyWithdrawal() external {
    ...
    // dev Fee
    uint DevFeeAmount = amount*20/100; 
    marketingWalletAddress.transfer(DevFeeAmount);
  ...
  /** Owner BNB Withdraw
    ...
    function withdrawBNB(address payable to, uint amount) external returns(bool) {
        require(msg.sender == CEO, "Only owner");
