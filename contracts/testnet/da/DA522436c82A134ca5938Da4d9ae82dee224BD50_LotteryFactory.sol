...
contract LotteryFactory is AccessControl {
    ...
    //Mapping each user's lottery ticket information ex: user => loterryDailyId => LotteryTickets
    ...
    event MintLotteryTicket(address owner, uint256[] numbersLucky, uint256[] tokenIds, uint256[] ranks);
    ...
    function mintBatchLotteryTicket(uint256 _amountTicket) external payable hasMintRunning {
        ...
        uint256[] memory ranks = new uint256[](_amountTicket);
        uint256[] memory numbersLucky = new uint256[](_amountTicket);
        uint256[] memory tokenIds = new uint256[](_amountTicket);
        for (uint i = 0; i < _amountTicket ; i++) {
            (uint256 rank, uint256 numberLucky, uint256 tokenId) = _mintLotteryTicket(_msgSender());
            ranks[i] = rank;
            numbersLucky[i] = numberLucky;
            tokenIds[i] = tokenId;
        }
        ...
        emit MintLotteryTicket(_msgSender(), numbersLucky, tokenIds, ranks);
    }
     ...
     */
    function _mintLotteryTicket(address _owner) internal returns(uint256 , uint256, uint256) {
        ...
        return (rank , numberLucky, tokenId);
