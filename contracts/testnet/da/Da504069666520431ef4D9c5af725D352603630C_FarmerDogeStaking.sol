...
contract FarmerDogeStaking is Ownable, ReentrancyGuard {
    ...
    address public buyBackWallet = 0x257a88FB0fC481a5316C8C8001F76aB3Ed2Cac97;
    ...
    function _updateDividendDebt(UserInfo storage user) internal {
        if(user.dividendDebt.length != dividendTokens.length) {
            for(uint i = 0; i < user.dividendDebt.length - dividendTokens.length; i++){
                uint pos = i + dividendTokens.length;
                user.dividendDebt.push(user.amount * accDividendPerShare[pos] / PRECISION_FACTOR_DIVIDEND[pos]);
            }
            for(uint i = 0; i < dividendTokens.length; i++) {
    ...
    function pendingDividends(address _account) external view returns (uint256[] memory data) {
        ...
        for(uint i = 0; i < dividendTokens.length; i++) {
            ...
            if(user.dividendDebt.length >= i + 1){
                user.amount * adjustedDividendPerShare / PRECISION_FACTOR_DIVIDEND[i] - user.dividendDebt[i];
            }
