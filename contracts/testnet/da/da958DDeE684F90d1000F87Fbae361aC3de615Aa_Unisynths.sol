...
abstract contract EIP20Interface {
   ...
   function mint(address _recipient, uint256 _amount) virtual public returns (bool);

   function burnFrom(address from, uint256 _amount) virtual public returns (bool);
...
abstract contract EIP20 is EIP20Interface {
    ...
    */
    ...
    function mint(address _recipient,uint256 _amount) override public returns (bool) {
...
contract Unisynths is Ownable, ReentrancyGuard {
    ...
    address ethTrading = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca;
    address maticTrading = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca;
    ...
    // Keep track of total minted supply
    uint256 public totalMinted = 0;

    // Keep track of total reserve supply
    uint256 public totalReserve = 0;
    ...
    constructor() {
        ...
        EIP20 _btc = EIP20(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca);

        btc = _btc;

        EIP20 _eth = EIP20(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca);

        eth = _eth;

        EIP20 _matic = EIP20(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca);
    ...
    function getBoldPrice() public view returns (uint256) {
        uint256 btcprice = getLatestPrice("BTC");
        uint256 goldprice = getLatestPrice("GOLD");

        uint256 sharebtc = btcprice.mul(60).div(100);
        uint256 sharegold = goldprice.mul(40).div(100);

        
        uint256 synthPrice = sharebtc + sharegold;
    ...
    function getSdrPrice() public view returns (uint256) {
        ...
        uint256 cnyprice = getLatestPrice("CNY");
        uint256 yenprice = getLatestPrice("YEN");
        uint256 gbpprice = getLatestPrice("GBP");
        uint256 eurprice = getLatestPrice("EUR");

        uint256 cnyamountprice = sdrCNY / cnyprice;
        uint256 yenamountprice = sdrYEN / yenprice;  
        uint256 gbpamountprice = sdrGBP / gbpprice; 
        uint256 euramountprice = sdrEUR / eurprice;
    ...
    function betMATIC() public {
        ...
        luckblocksM.BuyTicket(address(this),amountNecessary,3,5,18,23);
    }
    ...
    function mintSynth(bytes32 _ticker,uint256 _amount) tokenExist(_ticker) nonReentrant public {
        ...
        bytes32 boldHash = keccak256(abi.encodePacked("BOLD"));
        bytes32 sdrHash = keccak256(abi.encodePacked("SDR"));
        ...
        if(tickerHash == boldHash){
        
            synthPrice = getBoldPrice();

        } else if (tickerHash == sdrHash){
        ...
        uint256 amountMint = (_amount * synthPrice) / 10**18;
    ...
    function burnSynth(bytes32 _ticker,uint256 _amount) tokenExist(_ticker) nonReentrant public {
        ...
        require(getTotalSupply() >= getTotalMinted().mul(80).div(100), "Supplied reserve tokens is less than 80% overcollaterized.");
        ...
        uint256 amountToSend = (_amount * synthPrice) / 10**18;
        ...
        if (usd.balanceOf(address(this)) <= amountToSend.mul(60).div(100)){
          ...
          if (usd.balanceOf(address(this)) < amountToSend.mul(30).div(100)){
            ...
            if(krstm.balanceOf(address(this)) >= amountToSend / krstmprice){
            ...
            uint256 krstmtosell = amountToSend.mul(70).div(100);
            krstmtosell = krstmtosell / krstmprice;
            if(krstm.balanceOf(address(this)) >= krstmtosell){
                
                swapTokensToUSD(krstmtosell);
            
            }
          }

        } else if(usd.balanceOf(address(this)) > getTotalMinted().mul(5)){
           ...
           betMATIC();
            ...
            uint256 krstmtosell = amountToSend.mul(40).div(100);
            krstmtosell = krstmtosell / krstmprice;
            if(krstm.balanceOf(address(this)) >= krstmtosell){
        ...
        if(usd.balanceOf(address(this)) >= amountToSend){
    ...
    function getTotalSupply() public view returns (uint256) {
        ...
        uint256 krstmUsdReserve = krstmReserve.mul(krstmprice);
        ...
        // Return reserve balance in usd
          Token[] memory _tokens = new Token[](tokenList.length);
            for (uint i = 0; i < tokenList.length; i++) {
                ...
                uint256 mkcap = totalsupply.mul(tokenprice);
