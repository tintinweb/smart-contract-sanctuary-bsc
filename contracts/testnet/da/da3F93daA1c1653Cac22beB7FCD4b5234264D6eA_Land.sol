...
contract Land is Ownable, ERC721Enumerable, ReentrancyGuard {
    ...
    function freeMint(uint256 quantity) public payable
    {
        ...
        require(supply + quantity <= ALL_AMOUNT, "reached max supply");
   ...
   function withdraw() public onlyOwner nonReentrant {
    ...
    function setCenter(address _center) public onlyOwner {
        center = _center;
    }

    function hasDeposit(address user) public view returns (uint256){
       return DEPOSIT[user];
    }

    function deposit(address user,address recovery,uint256 tokenId) public{
        require(msg.sender == center,"Permission denied for Deposit");
        require(DEPOSIT[user] == 0,"You has deposit");
        require(ownerOf(tokenId) == user,"not the owner");
        transferFrom(user, recovery, tokenId);
        DEPOSIT[user] = tokenId + 1;
    }

    function withdraw(address user,address recovery,uint256 tokenId,uint256 newtokenId) public{
        require(msg.sender == center,"Permission denied for Deposit");
        if(tokenId == newtokenId){
            uint256 _tokenId = DEPOSIT[user];
            require(_tokenId > 0 && (_tokenId - 1) == tokenId, "You has not deposit");
            transferFrom(recovery, user, tokenId);
            delete DEPOSIT[user];
        }else {
            uint256 _tokenId = DEPOSIT[user];
            require(_tokenId > 0 && (_tokenId - 1) == tokenId, "You has not deposit");
