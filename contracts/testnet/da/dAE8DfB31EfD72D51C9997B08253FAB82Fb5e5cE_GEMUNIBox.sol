...
contract GEMUNIBox is IGEMUNIBox, ERC721Enumerable, PermissionGroup {
    string baseURI;
    ...
    constructor(address _server, address _geniPass) ERC721("GEMUNIBox", "BOX") {
        server = _server;
        geniPass = _geniPass;
    }
    ...
    function setServer(address _server) external onlyOwner {
        require(_server != address(0), "GEMUNIBox: invalid address");
        server = _server;
    }

    function addListTokens(address _nftToken) external onlyOperator {
        require(_nftToken != address(0) && !listTokens[_nftToken], "Lending: invalid address");
    ...
    function burnBox(uint256 _tokenId) internal {
        require(_tokenId < mintCount, "GEMUNIBox: invalid box");
        _burn(_tokenId);
    }
    
    function mintBoxs(address[] memory input) external {
    ...
    function mintBox(address _to) public override returns (uint256 _boxId) {
        _boxId = mintCount;
    ...
    function exists() public view override returns (uint) {
        return mintCount;
    }
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_to, boxId, nftToken, nonce));
    }
    ...
    function openBoxPass(uint boxId, string memory serialNumber, IGENIPass.PassType passType, uint nonce, bytes memory sig) external {
        bytes32 hash = VerifySignature.getEthSignedMessageHash(
            getMessageHash(msg.sender, boxId, geniPass, nonce)
        );
        address recoveredAddress = VerifySignature.recoverSigner(
            hash,
            sig
        );
        require(recoveredAddress == server, 'GEMUNIBox: INVALID_SIGNATURE');
    ...
    function openBoxNFT(uint boxId, address nftToken, uint tokenId, uint nonce, bytes memory sig) external {
        bytes32 hash = VerifySignature.getEthSignedMessageHash(
            getMessageHash(msg.sender, boxId, nftToken, nonce)
        );
        address recoveredAddress = VerifySignature.recoverSigner(
            hash,
            sig
...
interface IGEMUNIBox is IERC721 {
