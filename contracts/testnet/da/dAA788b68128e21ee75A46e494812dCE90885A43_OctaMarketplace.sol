...
interface IOctaTokenRegistry {
    function enabled(address) external view returns (bool);
}

contract OctaMarketplace is Ownable, ReentrancyGuard {
    ...
    // 0 -> offer
    // 1 -> royalty
    mapping(uint => bool) public isAllowed;

    /// @notice Address registry
    IOctaAddressRegistry public addressRegistry;

    bool public isBNBEnabled;

    modifier bnbEnabled(address _paytoken) {
        if (!isBNBEnabled) {
            require(_paytoken != address(0), "BNB is not enabled");
        }
        _;
    }
    ...
    ) external notListed(_nftAddress, _tokenId, _msgSender()) bnbEnabled(_payToken) {
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC721)) {
            IERC721 nft = IERC721(_nftAddress);
    ...
    function updateListing(
        ...
        address _payToken,
        uint256 _newPrice
    ) external nonReentrant isListed(_nftAddress, _tokenId, _msgSender()) bnbEnabled(_payToken) {
    ...
    function createOffer(
        ...
        uint256 _pricePerItem,
        uint256 _deadline
    ) external offerNotExists(_nftAddress, _tokenId, _msgSender()) bnbEnabled(address(_payToken)) {
        require(isAllowed[0], "Not allowed");
        require(
    ...
    function registerRoyalty(
        ...
        require(isAllowed[1], "Not allowed");
    ...
    function modifyIsAllowed(uint index, bool _isAllowed) external onlyOwner {
        isAllowed[index] = _isAllowed;
    }

    /**
     @notice Update OctaAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IOctaAddressRegistry(_registry);
    }

    function toggleIsBNBEnabled() external onlyOwner {
        isBNBEnabled = !isBNBEnabled;
    }
