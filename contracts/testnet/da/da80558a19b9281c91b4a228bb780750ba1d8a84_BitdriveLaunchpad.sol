...
contract BitdriveLaunchpad is ReentrancyGuard, Initializable {
  ...
  function deposit(uint256 _amount) public {
    ...
    _amount == STAKING_PRICE[3] || _amount == STAKING_PRICE[4], 'need _amount > 0');
  ...
  function harvestReward(uint256 _amount) public nonReentrant {
    ...
    require (block.number > endBlock && currentRound==1, 'not harvest time');
  ...
  function round3Deposit(uint256 _amount) public {
    ...
    require (currentRound==3, 'not harvest time');
  ...
  function availableMcar(address _user) public view returns (uint256) {
      uint diff = block.timestamp - userInfo[_user].round3withdrawtime;
      ...
      if( block.timestamp > userInfo[_user].round3withdrawtime && diff > PERIOD_LENGTH ){
            ...
            available = uint256(calcMcar) * balance.mul(20)/100;
            available = calcMcar.mul(tokenPrice);
      }else{
          available = 0;
      }
      return available;
