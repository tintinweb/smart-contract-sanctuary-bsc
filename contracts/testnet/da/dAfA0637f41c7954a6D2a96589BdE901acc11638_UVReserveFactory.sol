...
contract UVReserveFactory is Ownable, AccessControl, IUVReverseFactory {
    ...
    function createReserve(
        ...
        // );
        address poolStake = address(new UVStake());
        IUVStake(poolStake).initialize(orderNumber, _pool, _fundWallet);

        // bytes32 saltReserve = keccak256(
        //     abi.encodePacked(orderNumber, poolStake)
        // );
        address poolReserve = address(new UVReserve());
        IUVReserve(poolReserve).initialize(orderNumber, poolStake);
        ...
        onlyOwner
    {
        ...
        IUVStake(getPoolStake[_orderNumber]).addManager(_manager);
        ...
        onlyOwner
    {
        ...
        IUVStake(getPoolStake[_orderNumber]).removeManager(_manager);
        // (bool success, ) = getPoolReserve[_orderNumber].call(
