...
contract AvionCollection is ERC721EnumerableUpgradeable, OwnableUpgradeable, VRFConsumerBaseUpgradable {
    ...
    function getRandomNumber() internal returns (bytes32 requestId) {
        return requestRandomness(KEY_HASH, CHAINLINK_FEE);
    }
    ...
    function mintInternal(uint256 _tier, address _recipient) public  {
        currentSupply += 1;
    ...
    function _mint(uint256 _amount, uint256 _price) internal {
        ...
        require(LINK.balanceOf(address(this)) >= CHAINLINK_FEE * _amount, "AC: not enough LINK");
    ...
    function mintForFree(uint256 _tier) external {
        mintInternal(_tier, msg.sender);
    }

    function mintToDevWallet(uint256[] calldata _tier) external onlyOwner {
        require(!allowedToMint && !whitelistPresalePeriod && !publicPresalePeriod, "AC: cannot call when minting is allowed");
