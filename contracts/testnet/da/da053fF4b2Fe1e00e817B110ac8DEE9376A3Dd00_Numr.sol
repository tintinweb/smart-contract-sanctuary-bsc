...
contract Numr is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint public auctionPeriod = 21600;
    uint public swapPoint = 5;
    ...
    function pointMint(address to, uint[] memory tokenId, address ref) external nonReentrant {
        require(isStartTimePaidMint);
        require(isSwap);
    ...
    function bid(uint tokenId, uint _bid, address ref) external nonReentrant payable {
        ...
        require(totalBid > 0);
        require(msg.value == _bid && _bid >= priceBid && _bid >= minBid[tokenId] && _bid > maxBid[tokenId] && _bid.mod(stepPrice) == 0);
        require(isTimeBid);
    ...
    function ascend(uint[] memory tokens) external payable {
        require(isAscend, "combining not active");
        uint sum;
        uint totalPaidBurn;
        uint totalFreeBurn;
    ...
    function basePrice(uint tokenId) external view returns(uint){
        uint _price =  (tokenId < 1000) ? priceBidThreeNumer : priceBidFourNumer;
