...
contract IDO is Ownable{
    ...
    address constant public _usdt = 0x9060df9E5153A29B03FEC91a14b87E9E77dC809d;
    ...
    function invest1() external {
        ...
        require(_totalAmount + 100e6 <= 100e4 * 1e6,"overflow");
        ...
        IERC20(_usdt).transferFrom(msg.sender, address(this), 95e6);
        IERC20(_usdt).transferFrom(msg.sender, inviter, 5e6);
        user.invest += 100e6;
        ...
        if (_users[inviter].isPartner == true) {
            ...
            if (length == 10) {
                IERC20(_usdt).transfer(inviter, 300e6);
    ...
    function invest2() external {
        ...
        require(_stop == false, "already stop");
        require(_totalAmount + 200e6 <= 100e4 * 1e6);

        User storage user = _users[msg.sender];
        require(user.invest > 0, "invest1 first");
        require(user.invitees.length >= 10, "not enough invitees");

        IERC20(_usdt).transferFrom(msg.sender, address(this), 200e6);
        user.invest += 200e6;
    ...
    function stakeUsdt() external {
        ...
        IERC20(_usdt).transferFrom(msg.sender, address(this), 300e6);
    ...
    function claim(address recipient) external {
        ...
        uint amount = user.invest * 100 * 1e12;
    ...
    function userInfo(address account) external view returns(uint invest, address[] memory invitees, bool isPartner, bool isClaimed, uint totalAmount, uint totalCount, uint totalCount2){

        User memory user = _users[account];
        return (user.invest, user.invitees, user.isPartner, user.isClaimed, _totalAmount, _totalCount, _totalCount2);
    }
}