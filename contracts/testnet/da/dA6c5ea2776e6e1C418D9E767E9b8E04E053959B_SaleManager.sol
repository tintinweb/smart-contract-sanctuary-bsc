...
contract SaleManager is ReentrancyGuard, Ownable {
    ...
    uint256[] public bitmonToSale = [4000, 2500, 1500, 1000, 700, 300];
    ...
    /**
     * @dev Set Bitmon To Sale
     * This function is only used for testing
     */
    function setBitmonToSale(uint256[] calldata _newQuantity) external onlyOwner {
        require(_newQuantity.length == bitmonToSale.length, "qty data not valid");
        if (saleStage == SaleStage.Closed) {
            bitmonToSale = _newQuantity;
        }
    }
    ...
    function getCurrentStageBitmonPrice() public view returns (uint256[] memory) {
        if (uint256(saleStage) > 0) {
            return bitmonStagePrice[uint256(saleStage) - 1];
        }
    ...
    function getCurrentStageSuperchargerPrice() public view returns (uint256[] memory) {
        if (uint256(saleStage) > 0) {
            return superchargerStagePrice[uint256(saleStage) - 1];
        }
    ...
    function buyEgg(
        ...
        require(_type > 0 && _type < 9, "invalid type");
        ...
        // Check if all Ancients were sold
        if (!bitmonAncientsSold) {
            if (
                bitmonToSale[0] == 0 &&
                bitmonToSale[1] == 0 &&
                bitmonToSale[2] == 0 &&
