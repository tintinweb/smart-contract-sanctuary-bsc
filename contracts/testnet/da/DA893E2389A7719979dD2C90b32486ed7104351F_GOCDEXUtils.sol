...
contract GOCDEXUtils is GOCAuth, IGOCDEXUtils {
    ...
    constructor() GOCAuth(msg.sender) {
        dexList[dexCount++] = DEX({
        name: "PancakeSwap V1",
          factory: 0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc,
         router: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3,
         initHash: hex'ecba335299a6693cb2ebc4782e74669b84290b6378ea3a3873c7231a8d7d1074',
    ...
    function sortTokens(address tokenA, address tokenB) public pure override returns (address token0, address token1) {
        require(tokenA != tokenB, 'GOCDEXUtils: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'GOCDEXUtils: ZERO_ADDRESS');
    }
    ...
    function getAmountOut(uint256 dexID, uint amountIn, uint reserveIn, uint reserveOut) public view override returns (uint amountOut) {
        require(amountIn > 0, 'GOCDEXUtils: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'GOCDEXUtils: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(uint256 dexID, uint amountOut, uint reserveIn, uint reserveOut) public view override returns (uint amountIn) {
        require(amountOut > 0, 'GOCDEXUtils: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'GOCDEXUtils: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(uint amountIn, address[] memory path, uint256[] memory dexPath) public view override returns (uint[] memory amounts) {
        require(path.length >= 2 && dexPath.length == path.length-1, 'GOCDEXUtils: INVALID_PATH');
        amounts = new uint[](path.length);
        amounts[0] = amountIn;
        for (uint i; i < path.length - 1; i++) {
            (uint reserveIn, uint reserveOut) = getReserves(dexPath[i], path[i], path[i + 1]);
            amounts[i + 1] = getAmountOut(dexPath[i], amounts[i], reserveIn, reserveOut);
        }
    }

    // performs chained getAmountIn calculations on any number of pairs
    function getAmountsIn(uint amountOut, address[] memory path, uint256[] memory dexPath) public view override returns (uint[] memory amounts) {
        require(path.length >= 2 && dexPath.length == path.length-1, 'GOCDEXUtils: INVALID_PATH');
        amounts = new uint[](path.length);
        amounts[amounts.length - 1] = amountOut;
        for (uint i = path.length - 1; i > 0; i--) {
            (uint reserveIn, uint reserveOut) = getReserves(dexPath[i - 1], path[i - 1], path[i]);
            amounts[i - 1] = getAmountIn(dexPath[i - 1], amounts[i], reserveIn, reserveOut);
        }
    }

    function getPathReserves(address[] memory path, uint256[] memory dexPath) public view override returns (uint256[] memory reservesIn, uint256[] memory reservesOut) {
        require(path.length >= 2 && dexPath.length == path.length-1, 'GOCDEXUtils: INVALID_PATH');
        reservesIn = new uint256[](path.length - 1);
        reservesOut = new uint256[](path.length - 1);
        for (uint i; i < path.length - 1; i++) {
            (reservesIn[i], reservesOut[i]) = getReserves(dexPath[i], path[i], path[i + 1]);
        }
    }
