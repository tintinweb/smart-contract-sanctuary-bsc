...
interface INewAms {
    ...
    function veryRpRwCall(address usr, uint256 lpnum) external;
...
contract AmsSwap is Ownable {
    ...
    bool public removeLpState = false;//是否可以移除lp了？以交易日开始算起200天后可以移除lp。到200天了管理员设置可移除状态即可
    bool public upCallState = false;//是否调用up交易合约？

    address public noReceiveAdmin;//没有满足条件的奖励，发送给某人接收地址
    uint256 public rpRewardState = 1;//代数奖励业务逻辑的开关，关闭则不再调用层
    ...
    function setRpRate(uint256 _num, uint256[] memory _rate, uint256[] memory _urte, bool _removeLpState, bool _upCallState,
        address _noReceiveAdmin, uint256 _rpRewardState) public onlyOwner() {
        fanum = _num;
        swaprate = _rate;
        uprate = _urte;
    ...
    //交易的九层分润
    function rpNinReward(address _user, uint256 _amount) public returns (uint256){
        if (rpRewardState == 0) return 0;
        ...
        IERC20(newAms).transfer(noReceiveAdmin, _orwsum - _orw);
    ...
    function buy(uint256 _amount) public {
        ...
        uint256 _rmon = rpNinReward(msg.sender, IERC20(newAms).balanceOf(address(this)));
    ...
    function sell(uint256 _amount) public {
        ...
        if (INewAms(newAms).swapWriteList(msg.sender) == false) {
            ...
            _rateadd = IERC20(newAms).balanceOf(address(this)) * uprate[1] / 100; //上一步剩下后的ams，算出来交易应抽多少ams
            sellUpRsUp(_rateadd, 2);//卖成usdt，回流到up池
        }
        _amount = _amount - _rmon - _rateadd;//卖出的ams数量应该，同时减少分走的ams数量 + 回流至up池的ams数量
    ...
    function addL(uint256 _amountADesired, uint256 _amountBDesired) public {
        ...
        router02.addLiquidity(usdtZhen, newAms, _amountADesired, _amountBDesired, 0, 0, msg.sender, block.timestamp);

        //添加流动性，去调用绑定关系。转账ams即绑定关系，但是否参与九代分润？需要上线在交易之前添加lp，才能得到分润的效果
        INewAms(newAms).veryRpRwCall(msg.sender, _amountBDesired);
    ...
    function remL(uint256 _liquidity) public {
        require(removeLpState, 'remove lp data 200 day,stop!');
    ...
    function sellUpRsUp(uint256 _amount, uint256 _type) internal {
        ...
        path[1] = usdtZhen;
        path[2] = usdtJia;
        router02.swapExactTokensForTokens(_amount, 0, path, UP, block.timestamp);
        INewAms(UPSwap).upPutUsdRsUp(IERC20(usdtJia).balanceOf(address(this)), _type);
    }
