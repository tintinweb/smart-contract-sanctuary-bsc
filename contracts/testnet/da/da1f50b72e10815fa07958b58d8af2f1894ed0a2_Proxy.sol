...
contract BitdriveLaunchpad is Owned {
    ...
    function getTokenPerUser() public view returns (uint256) {
        ...
        uint256 remainingTokens = (totalSupply >= soldTokens)
            ? hardCap - soldTokens
            : 0;
        uint256 eachTier = remainingTokens / round2Users;
    ...
    function buyTokenRound1(uint256 amount) public {
        ...
        require(
            IToken(_crypto).transferFrom(msg.sender, address(this), amount),
    ...
    function claimToken() public {
        ...
        if (!isVest) {
            ...
            require(
                currentTime >
                    userInfo[msg.sender].lastClaimed * vestDays * 1 days,
            ...
            (uint256 available,uint256 Percentage,) = availableVesting(msg.sender);
            ...
            userInfo[msg.sender].claimableTokens = userInfo[msg.sender].claimableTokens.sub(available);
            userInfo[msg.sender].lastClaimed = block.timestamp;
        }
    }

    function availableVesting(address _user) public view returns (uint256,uint256,uint256) {
        
        uint256 vestingPeriod = 86400 * vestDays;
        uint256 diff = (end> block.timestamp)?end-block.timestamp:0;

        if (currentTime>0 && userInfo[_user].isExits && userInfo[_user].lastClaimed>0 && userInfo[_user].lastClaimed < currentTime) {
            diff = currentTime-userInfo[_user].lastClaimed;
        }
        
        // if (userInfo[_user].lastClaimed>0 && userInfo[_user].lastClaimed < block.timestamp) {
        //     diff = block.timestamp-userInfo[_user].lastClaimed;
        // }
        ...
        uint256 Percentage= 0; 
         if(userInfo[_user].claimableTokens>0){
             uint totalVesting = userInfo[msg.sender].totalVesting+vestPercent;
             ...
             if(totalVesting>10000){
                ...
                Percentage = 10000-userInfo[msg.sender].totalVesting;
                 ...
                 available =(uint256(calcVesting) * invest.mul(vestPercent))/10000;
    ...
    function withdrawtimeTest(address _user, uint256 numberOfdays)
        public
        onlyOwner
    {
        uint256 Days = numberOfdays.mul(1 days);
        uint256 changeTime = block.timestamp.add(Days);
        userInfo[_user].lastClaimed = changeTime;
    }

    function withdrawtimeTest1(uint256 numberOfdays)
        public
        onlyOwner
    {
        uint256 Days = numberOfdays.mul(1 days);
...
contract Proxy {
        ...
        returns (uint256)
    {
        ...
        uint256 eachTier = (getallocationToken>0)?(maxSupply*1e18)/getallocationToken:0;
