...
contract Pricer {
    ...
    uint basis = 1e8; // 100,000,000 = 100%
    ...
    */
    ...
    function getLpShare(uint amount, address _pair) external view returns(uint lpShare_){
        require(amount > basis, "not enough LP");
        IUniswapV2Pair pair = IUniswapV2Pair(_pair); // define pair

        uint lpSupply = pair.totalSupply(); // get pair total supply of LP tokens
        uint lpShare = (((amount).mul(1e16)).div(lpSupply)).div(basis); // get the lp share for amountLp input / % in basis
        lpShare_ = lpShare;
        if (lpShare == 0){
            revert();
        }
