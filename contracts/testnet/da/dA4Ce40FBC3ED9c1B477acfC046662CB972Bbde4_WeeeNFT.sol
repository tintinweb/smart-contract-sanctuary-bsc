...
contract WeeeNFT is ERC721URIStorage, AccessControl, ReentrancyGuard {
    ...
    address public WEE_TOKEN_ADDRESS;
    uint256 public defaultLevelUpgradeRequirement;
    
    mapping(address => bool) public beneficiaries; //Admin can set beneficiarie for minting by user
    mapping(address => uint256) public tokenHolders; //Token owned by user mapping
    mapping(uint256 => uint256) public tokenLevels; //Token level mapping
    mapping(uint256 => uint256) public nextCheckinTimes;
    mapping(uint256 => uint256) public tokenActives;
    mapping(uint256 => uint256) public levelUpgradeRequirements;
    ...
    constructor(address _weeTokenAddress, uint256 _defaultLevelUpgradeRequirement) ERC721("WeeeNFT", "NWEEE") {
        require(_defaultLevelUpgradeRequirement > 0, "Default level upgrade requirement must not be zero");
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        WEE_TOKEN_ADDRESS = _weeTokenAddress;
        WEE_TOKEN = IERC20(WEE_TOKEN_ADDRESS);
    ...
    function checkin(uint256 tokenId) external nonReentrant {
        ...
        _validateCheckinTime(currentTime, nextCheckinTimes[tokenId]);
        tokenActives[tokenId] += 1;
        nextCheckinTimes[tokenId] = currentTime + DateTime.DAY_IN_SECONDS;
    }

    function _validateCheckinTime(uint256 currentTime, uint256 nextCheckinTime) internal pure {
        if (nextCheckinTime > 0) {
            DateTime._DateTime memory currentDate = DateTime.parseTimestamp(nextCheckinTime);
            uint256 startDay = DateTime.toTimestamp(currentDate.year, currentDate.month, currentDate.day);
            uint256 endDay = startDay + DateTime.DAY_IN_SECONDS - 1;
            
            if (!(currentTime >= startDay && currentTime <= endDay || currentTime > endDay)) {
    ...
    function levelUp(uint256 tokenId) external nonReentrant {
        ...
        require(WEE_TOKEN.balanceOf(_msgSender()) >= fee, "Insufficient balance to level up");
        require(WEE_TOKEN.allowance(_msgSender(), address(this)) >= fee, "Insufficient allowance approval to level up");
        bool transferRes = WEE_TOKEN.transferFrom(_msgSender(), address(this), fee);

        if (!transferRes) {
            revert("Can not level up, transfer error");
        }
    ...
    function mintAndSetTokenURI(address to, string memory _tokenURI) external onlyRole(MINTER_ROLE) {
        uint256 _tokenId = _mint(to);
        _setTokenURI(_tokenId, _tokenURI);
    }

    function _mint(address to) internal returns(uint256) {
        require(balanceOf(to) == 0, "This address has already minted");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        tokenLevels[tokenId] = 1;
        _mint(to, tokenId);
        tokenHolders[to] = tokenId;
        return tokenId;
    }
    ...
    function setBeneficiary(address _caller, bool res) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function rescueERC20(address erc20Address, address recipient, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        bool transferRes = erc20.transfer(recipient, amount);

        if (!transferRes) {
            revert("Can not rescue ERC20, transfer error");
        }
    ...
    //Test function, must remove before deploying to mainnet
    function setNextCheckinTimeToday(uint256 tokenId) external onlyRole(DEFAULT_ADMIN_ROLE) {
        nextCheckinTimes[tokenId] = block.timestamp;
    }
}
