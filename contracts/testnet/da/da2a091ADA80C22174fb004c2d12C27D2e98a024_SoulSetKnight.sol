...
contract SoulSetKnight is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable {
    ...
    function setContracts(address soulCore_, address soulToken_) public onlyOwner {
        dataAddr.soulCore = ISoulCore(soulCore_);
        dataAddr.soulToken = ISoulToken(soulToken_);
    }

    function setProductMap(uint id_, bool isOpen_, address addr_, uint tokenVolume_, uint fixedAmount_, uint surplusAmount_) public onlyOwner {
    ...
    function setTeamsFrom(address communityAddr_) public{
        dataAddr.soulCore.setTeamsFrom(_msgSender(), communityAddr_);
    ...
    function receiveProduct(address communityAddr_, uint productId_) public{
        ...
        if(communityAddr_ == address(0)){
            //soul大于0，可以领取
            require(amount > 0 && userReceiveDate[productId_][_msgSender()] != day, "Conditions not met");
            ...
            require(productMap[productId_].isOpen && productMap[productId_].surplusAmount > 1, "Insufficient remaining quantity");

            IERC20Upgradeable(productMap[productId_].tokenAddr).transfer(_msgSender(), productMap[productId_].tokenVolume * 1 ether);
            ...
            require(userReceiveDate[productId_][_msgSender()] != day, "Time not reached");
...
interface ISoulCore {
    ...
    function extractBox(uint partIndex, uint8 soulCount) external returns(uint);
