...
contract CustomTokenFactory {
    ...
    function createToken(
        ...
        string memory tokenName = string(
            abi.encodePacked(name, " ", contractName)
        );
        string memory tokenSymbol = string(
            abi.encodePacked(symbol, contractName)
        );

        ERC20MintableBurnable token = new ERC20MintableBurnable{
            salt: keccak256(abi.encodePacked(tokenSymbol))
        }(tokenName, tokenSymbol, decimals, initialSupply, true);
