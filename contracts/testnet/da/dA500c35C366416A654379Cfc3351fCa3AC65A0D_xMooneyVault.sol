...
contract xMooneyVault is Ownable, ReentrancyGuard {
    ...
    function releaseTokens() external nonReentrant {
        ...
        if (contractBalance > amountOfTokens) {
            ...
            require(addLP(amountToBakeintoLP), "Add LP Failed");
    ...
    function addLP(uint256 addLPTokenAmount) private returns(bool){
        ...
        (uint amountToken, uint amountETH, uint liquidity) = uniswapLPRouter.addLiquidityETH{value: amountOfEth[0]}(
    ...
    function addLPExternal(uint256 addLPTokenAmount) external nonReentrant returns (bool) {        
        require(addLP(addLPTokenAmount), "Add LP Failed");

        return true;
    }

    function addLPAndTransfer(uint256 addLPTokenAmount) external nonReentrant returns(bool) {
        uint256 deadline = block.timestamp + 15;
    ...
    function convertEthToContractToken(
        ...
        uint[] memory amounts = uniswapRouter.swapETHForExactTokens{value: ethAMount }(
            exchangeTokenAmount,
    ...
    function convertEthToContractToken2(
        ...
        require(convertEthToContractToken(exchangeToken, exchangeTokenAmount, msg.value), "Swap for Token");
    ...
    function swapTokenForEth(uint256 amount) private returns (bool) {
        ...
        uint256[] memory EthAmount = getEstimatedEthforContractToken(uniswapRouter.WETH(), amount);
        uniswapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            amount,
            EthAmount[0],
            getPathForSourceTokentoContractToken(uniswapRouter.WETH()),
            address(this),
            deadline
        );
    ...
    function swapTokenForEthPub(uint256 amount) public nonReentrant returns (bool) {
