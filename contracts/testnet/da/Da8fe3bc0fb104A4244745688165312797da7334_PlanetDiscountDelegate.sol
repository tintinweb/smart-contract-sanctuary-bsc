...
contract PlanetStorage {
    ...
    */
    event BorrowDiscountAccrued(address market,address borrower,uint discountGiven,uint lastUpdated,uint marketTotalBorrows);
...
contract PlanetDiscountDelegate is PlanetStorage,ExponentialNoError{
    ...
    function changeUserBorrowDiscount(address borrower) external returns(uint,uint,uint,uint){
        ...
        if( _dis.exist && discount > 0) {
            ...
            emit BorrowDiscountAccrued(_market,borrower,vars.newDiscount,_dis.lastUpdated,marketTotalBorrows);
