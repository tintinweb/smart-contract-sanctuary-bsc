...
contract Warrior721 is ERC721Enumerable, ERC721URIStorage {
    ...
    IEternalStorage private immutable eternalStorage;
    ...
    // "Arena of Warriors", "WARRIOR", "https://hero-api.arenaofwarriors.io/metadata/"
    constructor(address eternalStorageAddr_, string memory baseURI_) ERC721("WARRIOR721", "WARRIOR721") {
        ...
        owner_ = IEternalStorage(eternalStorageAddr_).owner();
        baseURI = baseURI_;
    }

// *** MINT ***
    // MINT (OWNER OR WHITELIST)
    function WhitelistMint(address to_, uint256 timestamp_, uint256 hero_, uint256 rarity_, uint256 level_) external onlyOwnerOrWhitelisted {
        Mint(to_, timestamp_, hero_, rarity_, level_);
    }
    function MultiWhitelistMint(address[] calldata to_, uint256[] calldata hero_, uint256[] calldata rarity_) external onlyOwnerOrWhitelisted { 
    ...
    // MINT (PRIVATE)
    function Mint(address to_, uint256 createdTime_, uint256 hero_, uint256 rarity_, uint256 level_) private {
        uint256 warriorId_ = warriorId.current();
        warriorId.increment();
        _mint(to_, warriorId_);
        ...
        actions.A_AfterHeroMinted(to_, warriorId_, createdTime_, hero_, rarity_, level_);
...
// *** GETTER ***
    ...
    function G_AllOwnerTokenIds(address tokenOwner_) external view returns (uint256[] memory _ownedTokenIds) {
        uint256 userBalance_ = balanceOf(tokenOwner_);
        uint256[] memory ownedTokenIds_ = new uint256[](userBalance_);
        if (userBalance_ > 0) {
            for (uint256 i = 0; i < userBalance_; ++i) {
                ownedTokenIds_[i] = tokenOfOwnerByIndex(tokenOwner_, i);
            }
        }
