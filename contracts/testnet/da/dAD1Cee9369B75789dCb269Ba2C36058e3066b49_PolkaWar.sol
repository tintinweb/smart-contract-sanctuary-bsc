...
contract PolkaWar is Ownable, ReentrancyGuard {
    ...
    uint256[] private poolIdsForPlayer;
    ...
    function getPoolInfoById(uint256 _pid) external view returns (
        ...
        address winner
            ...
            GamePool storage pool = pools[_pid];
            return (pool.state, pool.id, pool.numberOfPlayers, pool.tokenAmount, pool.drawStatus, pool.players, pool.winner);
    }

    //get pool id by player address
    function getPoolIdsContainingPlayer(address _player) external returns (uint256[] memory)
    {        
        // uint256[] memory pids;
        for(uint256 i=0; i<pools.length; i++) {
            if(checkPlayerExist(_player, pools[i].players) >= 0 && pools[i].state != GameState.Finished)                
            {
                // pid = int(i);
                // break;
                // pids.push(i);
                // pids[pids.length] = 
                poolIdsForPlayer.push(i);
            }
        }
        return poolIdsForPlayer;
        // return pid;
    }
    //check if player in players[]
    function checkPlayerExist(address _player, address[] memory _players) internal pure returns (int256) {
    ...
    function bet(uint256 _pid) external {        
        ...
        GamePool storage pool = pools[_pid];
        // check game status
        require(pool.state != GameState.Running, "game is running");
        // add user
        if(pool.state == GameState.Opening) {
            pool.players.push(msg.sender);
            pool.state = GameState.Waiting;
        } else if(pool.state == GameState.Waiting) {
            require(checkPlayerExist(msg.sender, pool.players) < 0, "already existing player");
            pool.players.push(msg.sender);
            if (pool.players.length == pool.numberOfPlayers)
                pool.state = GameState.Running;
        }
        // deposit token
    ...
    function revoke(uint256 _pid) external {
        ...
        // check balance
        require(polkaWarToken.balanceOf(address(this)) >= pool.tokenAmount, "insufficient funds");
        // check permission
        // require(players[0] == msg.sender, "invalid permission");
        require(pool.state == GameState.Waiting || pool.state == GameState.Running, "invalid state");

        int256 playerIndex = checkPlayerExist(msg.sender, players);
        require(playerIndex >= 0, "player doesn't exist in the pool");
        // withdraw token
        ...
        for (uint256 i=uint256(playerIndex); i<players.length - 1; i++)
            players[i] = players[i + 1];
        players.pop();
        // delete players[uint256(playerIndex)];
    ...
    function updateGameStatus(uint256 _pid, address _winnerAddress, bool drawStatus) external onlyOwner {
        ...
        if(drawStatus == true) {
            ...
            address[] memory players = pool.players;
            require(checkPlayerExist(_winnerAddress, players) >= 0, "winner not found");
    ...
    function claimAward(uint256 _pid) external nonReentrant {
        ...
        require(pool.state == GameState.Finished, "no valid time");
        address[] memory players = pool.players;
        require(checkPlayerExist(msg.sender, players) >= 0, "player not found");
        if(pool.drawStatus == true) {
            uint256 refund = pool.tokenAmount * rewardMultiplier / 100;
            for(uint256 i=0; i<players.length; i++)
                polkaWarToken.transfer(players[i], refund);
            uint256 fee = pool.tokenAmount * pool.numberOfPlayers * (100 - rewardMultiplier) / 100;
        ...
        } else 
        {
            ...
            // send award
            uint256 award = pool.tokenAmount * pool.numberOfPlayers * rewardMultiplier / 100;
            uint256 fee = pool.tokenAmount * pool.numberOfPlayers * (100 - rewardMultiplier) / 100;
    ...
    function reset(uint256 _pid) internal {
        GamePool storage pool = pools[_pid];
        // initialize game
        pool.state = GameState.Opening;
        pool.winner = address(0);
