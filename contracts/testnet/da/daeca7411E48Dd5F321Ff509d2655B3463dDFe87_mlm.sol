...
contract mlm is Initializable {
    ...
    // function setRate(uint[] memory _rate) external onlyWhitelist {
    //     rate = _rate;
    // }

    // function setGradeRate(uint[] memory _rate) external onlyWhitelist {
    //     gradeRate = _rate;
    // }

    // function setJoinAmount(
    //     uint[] memory _amount
    // ) external virtual onlyWhitelist {
    //     joinAmount = _amount;
    // }
        ...
        returns (bool)
    {
        ...
        if (amount > 0) {
            ...
            if (parentInfo.grade > _grade && _grade < gradeRate.length - 1) {
                grade = _grade + 1;
            }
        }
        User memory _user = User({
    ...
    function upgrade(
        ...
        userData[msg.sender].expireTime = block.timestamp + 365 days;
