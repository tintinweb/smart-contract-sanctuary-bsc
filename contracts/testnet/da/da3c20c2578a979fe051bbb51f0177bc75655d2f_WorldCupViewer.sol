...
interface IWorldCup {
    ...
    function mapMatch(uint index_) external view returns(string memory roller);
    ...
    function mapPlayerChampionLen(address player_) external view returns(uint);
    function mapPlayerRecordLen(address player_) external view returns(uint);
...
contract WorldCupViewer is DivestorUpgradeable {
    ...
    function getRandom(uint index_) public view returns(uint[] memory) {
        uint[] memory randomNums = random.getRandom(worldCup.mapMatch(index_));
        return randomNums;
    }
    ...
    // 获取用户记录
    function getPlayerRecord(address player) public view returns(IWorldCup.Instant[] memory) {
        uint recordlen = worldCup.mapPlayerRecordLen(player);
        IWorldCup.Instant[] memory record = new IWorldCup.Instant[](recordlen);
        // uint index = record[0].matchId;
        return record;
    }
    ...
    function getEarningsData(address player) public view returns(IWorldCup.PlayerEarningInfo memory) {
        ...
        uint len = worldCup.mapPlayerChampionLen(player);
        if (worldCup.mapPlayerRecordLen(player) != 0) {
            
            uint recordlen = worldCup.mapPlayerRecordLen(player);
            IWorldCup.Instant[] memory record = new IWorldCup.Instant[](recordlen);
            uint index = record[0].matchId;

            uint[] memory randomNums = random.getRandom(worldCup.mapMatch(index));
            uint winner;
            ...
            uint totalBonus;
            // 参赛记录
            if (winner == record[0].team) {
                // 赢
                uint oldChampion = (record[0].matchId - record[0].matchId % 127) * 1 gwei / 127;
                if (oldChampion < (worldCup.championIndex() - 1) * 1 gwei && oldChampion != 0) {
                    // 首次投注新周期 重置杠杆
                ...
                // (投注数量 / 赢者池子) * 池子总数量 
                if (winner == 1) {
                    totalBonus = record[0].leverageAmount * 1 gwei / worldCup.mapInstantTotal(index).team1Amount * worldCup.mapInstantTotal(index).total / 1 gwei;
                } else {
                    totalBonus = record[0].leverageAmount * 1 gwei / worldCup.mapInstantTotal(index).team2Amount * worldCup.mapInstantTotal(index).total / 1 gwei;
                } 
                // 可领收益
                earningInfo.instantEarnings += totalBonus;
            } else {
                earningInfo.leverage = 0;
                earningInfo.ssoulAmount += record[0].initAmount * 1000;
            }

        } 
        // uint nowTimestamp = block.timestamp % 1 days;
        // 计算收益 - 冠军赛
        uint nowChampion = worldCup.matchId() * 1 gwei / 127;
        IWorldCup.Champion[] memory championRecord = new IWorldCup.Champion[](len);
        for (uint i = 0; i < len; i++) {
            if (nowChampion > championRecord[i].championIndex * 1 gwei) {
                // 冠军收益 = 当天 ssoul 积分 / 当天冠军池 ssoul 总量 * 当天冠军池 soul 总量
                earningInfo.championEarnings += earningInfo.ssoulAmount * 1 gwei / worldCup.mapChampionTotal(championRecord[i].championIndex).ssoulAmount * worldCup.mapChampionTotal(championRecord[i].championIndex).soulAmount / 1 gwei;
            }
        }
