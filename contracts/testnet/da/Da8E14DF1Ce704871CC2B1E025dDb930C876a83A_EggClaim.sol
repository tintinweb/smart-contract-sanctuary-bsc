...
// File: contracts\NEO\EggClaim.sol



contract EggClaim is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // set token nft
    function setAllowedNftList(address[] memory _tokenList) public onlyAdmin nonReentrant {
    ...
    event ClaimToken(address _user, address _payToken, uint256 _payPrice, address _token, uint256[] resultTokenId, uint256 nftNum, uint256 _at);
    ...
    function addTokenBalance(address _token, address _user, uint256 _num, uint256 _orderId) external nonReentrant onlyAdmin {
        ...
        require(tokenIsAllowed(_token), "NFT_NOT_ALLOWED");
