...
contract ApolloxExchangeTreasury is Ownable, Pausable {
    using Address for address payable;
    using SafeERC20 for IERC20;
    using SignatureChecker for address;

    uint8 constant public USD_DECIMALS = 8;
    address constant public ETH = address(bytes20(keccak256("ETH")));

    event Deposit(address indexed account, address indexed currency, bool isETH, uint256 amount, uint256 broker);
    event ClaimPaused(address indexed trigger, address indexed currency, uint256 amount, uint256 amountUsd);
    event Withdraw(uint256 indexed id, address indexed to, address indexed currency, bool isETH, uint256 amount, uint256 broker);
    event Claim(uint256 indexed id, address indexed to, address indexed currency, bool isETH, uint256 amount, uint256 broker);
    ...
    event TransferToCounterParty(address indexed currency, bool isETH, uint256 amount);
    ...
    mapping(uint256 => uint256) public chaimPerHours;
    ...
    constructor (address truthHolder_, address operator_, address payable counterParty_, uint256 hourlyLimit_) {
        truthHolder = truthHolder_;
        operator = operator_;
        counterParty = counterParty_;
        hourlyLimit = hourlyLimit_;
    }
    ...
    function changeTruthHolder(address newTruthHolder) external onlyOwner {
    ...
    function changeCounterParty(address payable newCounterParty) external onlyOwner {
        address payable oldCounterParty = counterParty;
        counterParty = newCounterParty;
        emit NewCounterParty(oldCounterParty, newCounterParty);
    }

    function updateHourlyLimit(uint256 newHourlyLimit) external onlyOwner {
    ...
    function addToken(
        ...
        require(currency != address(0), "currency cannot be zero address");
        require(!fixedPrice && priceFeed != address(0), "when the price is not fixed, the priceFeed cannot be zero address");
    ...
    function removeToken(address currency) external onlyOwner {
        delete supportToken[currency];
        emit RemoveToken(currency);
    }

    function transferToCounterParty(bool isETH, address currency, uint256 amount) external onlyOperator {
        _transfer(counterParty, isETH, currency, amount);
        emit TransferToCounterParty(currency, isETH, amount);
    }

    function _transfer(address payable to, bool isETH, address currency, uint256 amount) private {
        if (isETH) {
            to.sendValue(amount);
        } else {
    ...
    function depositEth(uint256 broker) external payable {
        require(_supportCurrency(ETH), "currency not support");
        uint256 amount = msg.value;
        require(amount > 0, "msg.value must be greater than 0");
        emit Deposit(msg.sender, ETH, true, amount, broker);
    }

    function withdraw(bytes calldata message, bytes calldata signature) external whenNotPaused {
        require(truthHolder.isValidSignatureNow(ECDSA.toEthSignedMessageHash(keccak256(message)), signature), "only accept truthHolder signed message");
        (uint256 id, address payable to, bool isETH, address currency, uint256 amount, uint256 broker, uint256 deadline) =
        abi.decode(message, (uint256, address, bool, address, uint256, uint256, uint256));

        bool result = _claim(id, to, currency, isETH, amount, deadline);
        if (result) {
            emit Withdraw(id, to, currency, isETH, amount, broker);
        }
    }

    function claim(bytes calldata message, bytes calldata signature) external whenNotPaused {
        require(truthHolder.isValidSignatureNow(ECDSA.toEthSignedMessageHash(keccak256(message)), signature), "only accept truthHolder signed message");
        (uint256 id, address payable to, bool isETH, address currency, uint256 amount, uint256 broker, uint256 deadline) =
        abi.decode(message, (uint256, address, bool, address, uint256, uint256, uint256));
        ...
        bool result = _claim(id, to, currency, isETH, amount, deadline);
        if (result) {
            emit Claim(id, to, currency, isETH, amount, broker);
        }
    }

    function _claim(uint256 id, address payable to, address currency, bool isETH, uint256 amount, uint256 deadline) private returns (bool) {
        if (isETH) {
            currency = ETH;
        }
        ...
        if (chaimPerHours[cursor] + amountUsd > hourlyLimit) {
            _pause();
            emit ClaimPaused(msg.sender, currency, amount, amountUsd);
            return false;
        } else {
            claimHistory[id] = block.number;
            chaimPerHours[cursor] += amountUsd;
