...
interface IMultilevelRoyalty {
    ...
    function addRoyalty( address _nftContract, uint256 _tokenID, uint32 _royaltyFee ) external returns (bool);

    function setBuyoutPrice( address _nftContract, uint256 _tokenID, uint256 _buyoutPrice ) external returns (bool);
...
contract DVC is ERC20, Ownable{
    ...
    constructor( address _protectedMarketplace, address _mlrFactory ) ERC20( "LPToken", "LPT" ){
    ...
    function checkBuyCriteriaStatus(uint256 _orderID) internal returns(bool) {
        ...
        require( isCollection[ipo.tokenAddress],"NFT address are not matched" );
    ...
    function sellNftOnCriteria( uint256 _orderID ) public returns ( bool ) {
        ...
        uint256 endSellOrderTime = block.timestamp + ipo.protectionTime - 86400;

        require( block.timestamp < endSellOrderTime, "Insufficient offer time" );
        uint256 newProtectionTime = endSellOrderTime - block.timestamp;
        
        marketplace.createOrder( ipo.tokenAddress, ipo.nftTokenId, sellPrice, sellNFT.protectionRate, newProtectionTime, sellNFT.isAuction, endSellOrderTime );
    ...
    function getFundsFromDownsideProtectionAsBuyer( uint256 _orderId ) public returns ( bool ) {
        ...
        require( ipo.buyerAddress == payable(address(this)), "This order should be done by buyer" );
        require( block.timestamp <= ipo.soldTime + ipo.protectionTime, "Protection time expired" );
    ...
    function getFundsFromDownsideProtectionAsSeller( uint256 _orderId ) public returns ( bool ) {
        ...
        require( ipo.sellerAddress == payable(address(this)), "This order should be done by buyer" );
        require( block.timestamp > ipo.soldTime + ipo.protectionTime, "Protection time is not expired" );
...
// Buy or Sell via MLR contract
    ...
    function checkBuyCriteriaStatusFromMLR (
        ...
        require( isCollection[_nftContract],"NFT address are not matched" );
        ...
        require( imo.protectionTime >= buyNFT.protectionPeriod,"Protection time is different" );
    ...
    returns ( bool ) {
        ...
        if ( sellNFT.royalty > 0 ) {
            mlr.addRoyalty(_nftContract, _tokenID, sellNFT.royalty);
        }
        if ( sellNFT.buyoutPrice > 0 ) {
            mlr.setBuyoutPrice(_nftContract, _tokenID, sellNFT.buyoutPrice);
        }
        
        uint256 priceChange = imo.tokenPrice.mul( sellNFT.abovePriceRate ).div( 100 );
        uint256 sellPrice = imo.tokenPrice.add( priceChange );

        uint256 endSellOrderTime = block.timestamp + imo.protectionTime - 86400;
    ...
    function getFundsFromDownsideProtectionAsBuyerFromMLR(
        ...
        require( imo.buyerAddress == payable(address(this)), "Only Buyer can claim" );
        require( block.timestamp <= imo.soldTime + imo.protectionTime, "Protection time expired" );
    ...
    function getFundsFromDownsideProtectionAsSellerFromMLR(
        ...
        require( imo.sellerAddress == payable(address(this)), "Only Seller can claim" );
        require( block.timestamp > imo.soldTime + imo.protectionTime, "Protection time is not expired" );
