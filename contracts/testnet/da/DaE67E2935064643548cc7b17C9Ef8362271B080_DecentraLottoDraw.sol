...
contract DecentraLottoDraw is Context, Ownable, RandomNumberConsumer, DrawInterface {
    ...
    //taken on each ticket purchase
    uint8 public marketingDivisor = 2; //50%
    uint8 public hedgeDivisor = 5; //20%
    uint8 public stakingDivisor = 6; //16.66%

    //taken on draw end
    uint8 public liquidityDivisor = 20; //5%
    uint8 public megadrawDivisor = 20; //5%
    uint8 public stakersJackpotRewardDivisor = 2; //50%
    ...
    function getCurrentPot() public view returns(uint256){
        ...
        return deloBal - deloBal/liquidityDivisor - deloBal/megadrawDivisor;
    }
    ...
    function createNextDraw() private returns(bool){
        ...
        draw.id = currentDraw;
        draw.numTickets = 0;
        draw.createdOn = block.timestamp;
    ...
    function getNumberWinners() public view returns(uint){
        ...
        //divide by two to remove the stakers rewards as a winner
        uint256 bal = delo.balanceOf(address(this))/2;
        while (bal/2 >= deloCost && numWinners <= maxWinners){
            bal = bal-(bal/2);
            numWinners++;
        }
        return numWinners;
    ...
    function drawWinners() public isState(LotteryState.Ready) returns(bool){
        ...
        //send half the pot to stakers
        uint256 stakersRewards = draw.totalPot/stakersJackpotRewardDivisor;
        //edge case to handle a zero pot
        if (stakersRewards > 0){
            delo.approve(deloStakingAddress, stakersRewards);
            deloStaking.ADDFUNDS(stakersRewards);
        }
        
        while (delo.balanceOf(address(this))/2 >= deloCost && seed <= maxWinners){
            //pick a random winner
            address winner = draw.tickets[(uint256(keccak256(abi.encode(randomResult, seed)))%draw.numTickets)];
            //add them to the winners array
            draw.winners.push(winner);
            //increment their wins
            walletTotalWins[winner]++;
            //add their win value
            uint256 amt = delo.balanceOf(address(this))/2;
    ...
    */
    ...
    function endDrawAndGetRandom() external isState(LotteryState.Open) returns(bool){
        NewDraw storage draw = draws[currentDraw];
        require (block.timestamp > draw.drawDeadline, 'Draw deadline not yet reached');
        ...
        if (takeLiquidity == true && inSwapAndLiquify == false){
            //% to liquidity
            swapAndLiquify(jackpotTotal/liquidityDivisor);
        }
        
        //take the megadraw allotment
        if (takeMegadraw == true){
            //take megadraw % to be accumulated for megadraws
            delo.transfer(megadrawWallet, jackpotTotal/megadrawDivisor);
        }
    ...
    function getPriceForTickets(address tokenAddress, uint numTickets) public view returns(uint256){
        ...
        uint256 price;
        if (numTickets >= 20){
            price = priceOneTicket - priceOneTicket*discountTwentyTickets/100;
        }else if(numTickets >= 10){
            price = priceOneTicket - priceOneTicket*discountTenTickets/100;
        }else if(numTickets >= 5){
            price = priceOneTicket - priceOneTicket*discountFiveTickets/100;
        }else{
    ...
    function buyTicketsBNB(uint16 numTickets, address recipient, address airDropRecipient) payable external isState(LotteryState.Open) returns(bool){
        NewDraw storage draw = draws[currentDraw];
        require (block.timestamp < draw.drawDeadline, 'Ticket purchases have ended for this draw');
    ...
    function buyTicketsStable(address tokenAddress, uint16 numTickets, address recipient, address airdropRecipient) isState(LotteryState.Open) external returns(bool){
        ...
        uint256 tokenAmount = token.balanceOf(address(this))-initialTokenBal;
    ...
    function processTransaction(uint256 bnbValue, uint16 numTickets, address recipient, address airdropRecipient) isState(LotteryState.Open) private returns(bool){
        ...
        if (takeMarketing == true){
            bnbValue = bnbValue-bnbValue/marketingDivisor;
        }
        
        //swap the bnb from the ticket sale for DELO
        swapEthForDelo(bnbValue);
        uint256 tokenAmount = delo.balanceOf(address(this))-initialTokenBal;
        ...
        if (takeHedge == true){
            ...
            if (airdropRecipient == msg.sender || airdropRecipient == recipient || charityRecipients[airdropRecipient] == true){
                totalAirdropsReceived[airdropRecipient] += tokenAmount/hedgeDivisor;
                delo.transfer(airdropRecipient, tokenAmount/hedgeDivisor);
            }
        ...
        if (takeStaking == true){
            ...
            uint256 amt = tokenAmount/stakingDivisor;
