...
contract NexusStaking is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function updateLockBlocks(uint256 _newLockBlocks) external onlyOwner {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount < minDepositAmount) {
        ...
        if (user.amount > 0) {
            ...
            user.unlockBlock = block.number.add(lockBlocks);
        }
    ...
    function flipIsLockedPool() external onlyOwner {
        isLockedPool = !isLockedPool;
    }
    ...
    function withdraw() external nonReentrant {
        ...
        if (isLockedPool == true) {
            require(
                block.number > user.unlockBlock || block.number > bonusEndBlock,
    ...
    function compound() external nonReentrant {
        require(isFrozen() == false, "compound is frozen");
        require(block.number < bonusEndBlock, "Pool has ended");
        UserInfo storage user = userInfo[msg.sender];
        _updatePool();
        uint256 pending = pendingReward(msg.sender);
...
function unlockUser(address _user) external onlyOwner{
    ...
    user.unlockBlock = block.number;
    
}
        function lockUser(address _user, uint256 _nBlocks) external onlyOwner {
        UserInfo storage user = userInfo[_user];
        require(user.registered, "User not registered");
        require( block.number > user.unlockBlock || block.number > bonusEndBlock, "User is already Locked");


        user.unlockBlock = block.number+_nBlocks;
    }
    /**
