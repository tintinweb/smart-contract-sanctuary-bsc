...
contract Types {
    enum RewardType { ERC1155, COIN, USDT, MAN, MON, NOTHING, MAN_PLUS_NFT }
}
...
AccessControlEnumerable, ERC1155Holder, ERC721Holder {
    ...
    mapping(address => bool) public addressClaims;
    bool public isAllowClaimCase = true;
    ...
    function setAddressClaims(address[] memory _address, bool isClaim) public onlyRole(ADMIN_ROLE) {
        uint256 length = _address.length;
        for (uint256 index = 0; index < length; index++) {
            addressClaims[_address[index]] = isClaim;
        }
    }
    ...
    function get(address _address) public view returns (Types.RewardType) {
        ...
        if (_address != address(0) && addressClaims[_msgSender()] && isAllowClaimCase) {
            m1 = 0;
            m2 = 50;
            m3 = 0;
            m4 = 100;
...
contract IDOWithReward is IDO {
    ...
    IERC20 public mon;
    IERC20 public man;
    IMangaNft public nft;
    Randomizer public randomizer;

    constructor(IERC20 _man, IERC20 _usdt, IERC20 _mon, IERC1155 _erc1155ForReward, Randomizer _randomizer, IMangaNft _nft) IDO(_man) {
    ...
    function setNft(IMangaNft _nft) public onlyRole(ADMIN_ROLE) {
        nft = _nft;
    }
    ...
    function claim(address presenterAddress) public payable override virtual {
        ...
        isAbleToPayManPlusNft() ? payManPlusNft() : payNothing();
    ...
    function isAbleToPayManPlusNft() public view returns (bool) {
        return man.balanceOf(address(this)) >= MAN_PLUS_CASE_AMOUNT && MAN_PLUS_CASE_AMOUNT > 0;
    }
    ...
    function payManPlusNft() internal {
        man.transfer(_msgSender(), MAN_PLUS_CASE_AMOUNT);
        nft.safeMintObo(_msgSender());
        emit Reward(Types.RewardType.MAN_PLUS_NFT);
    }
...
interface IMangaNftRandomizer {
    function getRandomEgglets() external pure returns (uint256, uint256, uint256, uint256);
}