...
contract TokenV1 is BaseContract, ERC20Upgradeable {
    ...
    function _transfer(address from_, address to_, uint256 amount_) internal override {
        if (from_ == _properties.lpAddress)
            require(_canSwap[to_], "FUR: No swaps from external contracts");
        if (to_ == _properties.lpAddress)
            require(_canSwap[from_], "FUR: No swaps from external contracts");
        uint256 _taxes_ = 0;
