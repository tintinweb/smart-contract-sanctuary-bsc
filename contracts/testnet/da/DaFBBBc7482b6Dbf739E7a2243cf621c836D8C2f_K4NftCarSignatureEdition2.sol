...
contract K4NftCarSignatureEdition2 is ERC721, Ownable, ReentrancyGuard {
    ...
    function safeMintFromHotWallet(
        uint256 tokenId,
        address tokenAddress,
        uint256 amount,
        address to
        ...
        require(isSaleActive, "Sale Inactive");
        require(tokenAddress != address(0), "Address cannot be zero");
        require(whitelist[msg.sender], "Only whitelisted users allowed");
        IERC20 token;
        token = IERC20(tokenAddress);
        require(
            token.allowance(msg.sender, address(this)) >= amount,
        ...
        if (tokenId <= NFTTOTALSUPPLY && !_exists(tokenId)) {
            _safeMint(to, tokenId);
            emit NFTMinted(to, tokenId, 1, true, _CONTRACTID);
        } else {
            emit NFTMinted(msg.sender, tokenId, 0, false, _CONTRACTID);
        }
        SafeERC20.safeTransferFrom(token, msg.sender, address(this), amount);
        emit TokenTransfered(tokenAddress, msg.sender, address(this), amount);
    }
    ...
    function setHotwalletAddress(address user) public onlyOwner {
        require(user != address(0), "Address cannot be 0");
        require(!whitelist[user], "User is already on the whitelist.");
        whitelist[user] = true;
    ...
    function getHotWalletAddresses()
        public
        view
        onlyOwner
