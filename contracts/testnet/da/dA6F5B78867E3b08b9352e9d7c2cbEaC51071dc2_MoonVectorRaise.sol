...
contract VectorShards is IERC20, Ownable {
    ...
    uint256 public totalSupply = 350_000;
    ...
    function distribute(uint256 amount) external {
        ...
        accRewardsPerShare += (amount * 1 ether) / allocatedShards;
...
contract MoonVectorRaise is ICrowdFundBasic, Ownable {
    ...
    function _pledge(uint256 amount, address _ref) private {
        ...
        if (pledger.total == 0)
            require(minDeposit <= amount, "Min deposit not reached");
    ...
    function _distributeShares(address user) private {
        ...
        uint256 shares = (bonus + pledger.refAmount) / 1 ether;
        shares -= claimed[user];
