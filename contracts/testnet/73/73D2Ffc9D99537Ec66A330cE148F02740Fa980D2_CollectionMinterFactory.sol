...
contract CollectionMinterFactory is Ownable {
    // events
    event NewCollectionMinter(address indexed collection, address indexed owner, uint indexed deployIndex);

    // deployed marketplaces
    mapping (uint => address) public collectionsMinter;
    uint public collectionsMinterCounter;
    ...
    /**
     * @notice Deploy new Collection minter
     ...
     * @param _suffixURI Suffix of the token URI
     * @param _maxSupply max supply for token, set 0 means no maxSupply
     * @param _mintPrice price for mint in ETH, 18 decimals
    */
    function deployCollectionMinter(address _owner, string memory _name, string memory _symbol, string memory _baseURI,
        string memory _suffixURI, uint _maxSupply, uint _mintPrice, bool _vipTokenMode, uint _discountPercentage) external onlyOwner {

        CollectionMinter collectionMinter = new CollectionMinter(_name, _symbol, _maxSupply, _mintPrice,
            _vipTokenMode, _discountPercentage);

        collectionMinter.setBaseURI(_baseURI);
        collectionMinter.setSuffixURI(_suffixURI);
        collectionMinter.transferOwnership(_owner);

        collectionsMinter[collectionsMinterCounter] = address(collectionMinter);
        collectionsMinterCounter += 1;

        emit NewCollectionMinter(address(collectionMinter), _owner, collectionsMinterCounter - 1);
    }
}
...
contract CollectionMinter is ERC721Enumerable,ERC2981, Ownable {
  ...
  uint public totalValueWithdraw;


  constructor(string memory _name, string memory _symbol, uint _maxSupply, uint _mintPrice,
                bool _vipTokenMode, uint _discountPercentage) ERC721(_name, _symbol) {
   ...
   * This function expect a payment in ETH
   */
  function mint(address _to,address royaltyReceiver, uint96 feeNumerator) public payable {
    require(msg.value >= mintPrice, "CollectionMinter: not enough ETH");
    require(totalSupply() < maxSupply || maxSupply == 0, "CollectionMinter: max supply reached");
 ...
 * @param _to address to mint NFT
 */
  function mintBatch(address _to,uint _amount,address royaltyReceiver, uint96 feeNumerator) public payable  {
    require (totalSupply() + _amount <= maxSupply || maxSupply == 0, "CollectionMinter: max supply reached");
  ...
  */
  function tokenURI(uint _tokenID) public view virtual override returns (string memory) {
    require(_exists(_tokenID), "CollectionMinter: tokenID not exist");

    return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, suffixURI)) : "";
  }
  ...
  */
  ...
  function processRefund(address _to, uint _amount) internal returns(uint) {
    ...
    require(address(collectionVip) != address(0) || !vipTokenMode, "CollectionMinter: collection vip not set");
  ...
  /**
   * @notice Set vip token for discount
   * @param _collection vip token collection
   */
