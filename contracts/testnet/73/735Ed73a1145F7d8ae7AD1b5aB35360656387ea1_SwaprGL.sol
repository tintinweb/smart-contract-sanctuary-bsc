...
contract SwaprGL is BaseGovernanceUpgradeable {
    //SWAPR ONCHAIN vs OFFCHAIN
    //Its the Swapr Admin Contract
    //Responsible to manage swapr wallet
    //Can only interact with wallet after verifying the signature from backend
    //For back-end dev reference -> https://github.com/OpenZeppelin/merkle-tree
    //Use merkleProof for data verification
    ...
    uint public constant timeOffset = 15*60;//15minutes
    ...
    function dockNewMarketplace(address marketplace) external onlyAdmin {
        theMarketplace = marketplace;
    }
    function payNow(address acceptedToken, address from, address to, uint amount) internal returns(bool paid) {
        paid = swaprWallet.payNow(acceptedToken, from, to, amount);
    }


    /////////////////////////////////////////////////////////////////////////////////
    //START HERE on FRIDAY - and finish INSHAALLAH
    //ALL FUNCTIONS SHOULD RECEIVE MSG SENDER FROM SWAPR CONTRACT - IMPORTANT
    ...
    function createListing(uint listingType, bytes calldata data) external onlyMarketplace(data) {
        ...
        //INPUT:
        //1. uint listingType : 1=> auction, 2=> order->direct sale
        //2. bytes data: {
        //     bytes listing: {
        //      bytes message,
    ...
    function isListable(bytes memory listingData) public view returns(bool isValid, string memory res) {
    ...
    function _validateListing(uint listingType, bytes memory listingData) internal view returns(bool isValid, address lock, uint nftId, bytes memory sig, string memory res) {
        ...
        //INPUT:
        //1. uint listingType : 1=> auction, 2=> order
        //2. bytes listingData: {
        //      bytes message,
        //      bytes sellerSignature,
        //   }
    ...
    returns(bool isValid, string memory res, Bid memory validatedBid){
        ...
        //(,bytes memory listingSig) = abi.decode(listingData, (bytes, bytes));
        // (
        //     uint depositType,,,
        //     bool isEscrow, address acceptedToken
        //     ,,,,,,, 
        // ) = _extractAuctionInfo(listingData);


        Auction memory auction = _extractAuctionInfo(listingData);

        // (
        //     uint proposedOfferPrice, 
        //     uint proposedLockedBalance,, 
        //     address proposedBidder,
        // ) = _extractBid(proposedBid);
        ...
        if(auction.depositType == 1){
            ...
            if(_verify(_msgSender(), proposedBidData)){
                ...
                if(auction.isEscrow){
                    ...
                    if(bidderTotalBalance.sub(proposedBid.lockedBalance) >= proposedBid.offerPrice){
                        isValid = true;
                    }else{isValid = false;}
                }
            }
        ...
        if(isValid){
            ...
            // validatedBid = abi.encode(
            //     proposedOfferPrice,
            //     proposedLockedBalance,
            //     validatedListingEndTime,
            //     proposedBidder,
            //     listingSig
            //     );
        }
    ...
    function buyNow(bytes memory listingData, uint split) external returns(bool isSplit, Order memory newListing) {

        Order memory order = _extractOrderInfo(listingData);
        require(_verify(getMarketplace(), listingData), "INVALID_LISTING_SIG");
        ...
        //MakePayment
        uint price = split/EXP * order.fixedPrice;
        require(payNow(order.acceptedToken, buyer, order.seller, price), "ERROR: PAYMENT_FAILED");
    ...
    function claim(bytes calldata data) external onlyMarketplace(data) returns(bool success, string memory res) {
        ...
        // bytes data: {
            // bytes message: {
                ...
                // }
                // bytes lastBid: {
                //     encoded bytes message => listing signature,
                //     bytes bidderSignature
                // }
            // },
            // bytes marketPlaceSignature
        ...
        //(,address lock, uint nftId,, address acceptedToken,,,,,,,) = _extractAuctionInfo(listingData);
        Auction memory auction = _extractAuctionInfo(listingData);
        ...
        if(success){
            if(_verify(claimant, listingData)){
                ...
                if(auction.acceptedToken == address(0)){
                    swaprWallet.swapNative(bid.bidder, claimant, bid.offerPrice);
                    swaprWallet.releaseFunds(claimant, bid.offerPrice);
                }else{
                    swaprWallet.swapERC(auction.acceptedToken, bid.bidder, claimant, bid.offerPrice);
                    swaprWallet.releaseFunds(auction.acceptedToken, claimant, bid.offerPrice);
                }
    ...
    function isClaimable(bytes memory listingData, bytes memory lastBid) public view returns(bool isValid, string memory res) {
    ...
    function _validateClaim(address claimant, bytes memory listingData, bytes memory bidData) internal view returns(bool isValid, string memory res) {
        ...
        // (,address lock, uint nftId, 
        // bool isEscrow, address acceptedToken,,
        // ,,,, address seller,) = _extractAuctionInfo(listingData);
        Auction memory auction = _extractAuctionInfo(listingData);
        
        // (uint bidOfferPrice, 
        // uint bidLockedBalance, uint listingEndTime, 
        // address bidder, bytes memory bidListingSig) = _extractBid(lastBid);
                    ...
                    if(block.timestamp >= lastBid.listingEndTime){
                        ...
                        if(auction.isEscrow){
                            if(_verify(claimant, listingData)){
                                ...
                                if(bidderTotalBalance.sub(lastBid.lockedBalance) >= lastBid.offerPrice){
                                    isValid = true;
                                }else{
    ...
    //TODO: to be done
    function withdraw(bytes memory data) external onlyMarketplace(data) {// For Assets
        //INPUT:
        // bytes data: {
            // bytes message: {
                // address lock,
    ...
    function withdraw(address token) public {// For Funds
    ...
    function _withdraw(address claimant, address token) internal {
        //TODO: can only withdraw unlocked funds
    ...
    //INTERNAL FUNCTIONS
    /////////////////////////////////////////////////////////////////////////////////
    function _validateAuctionCapability(bytes memory _listingData) internal view 
    returns(
        ...
        // (
        //     uint depositType, address lock, uint nftId,,, uint startingPrice,
        //     uint buyNowPrice, uint startTime, uint endTime, uint timeOffset, 
        //     address seller, uint activeDepositType
        // ) = _extractAuctionInfo(_listingData);
        ...
        if(isValid){err = "OK"; lockAddr = auction.lock; tokenId = auction.nftId;}

    }

    function _validateOrderCapability(bytes memory _orderData) internal view 
    returns(
        bool isValid, address lockAddr, 
        uint tokenId, string memory err
    ){
        ...
        // (
        //     uint depositType, address lock, uint nftId, 
        //     ,,uint fixedPrice,
        //     ,uint maxBuyPerWallet, address seller,
        //     , uint activeDepositType
        // ) = _extractOrderInfo(_orderData);
            ...
            if(order.depositType == 2){
                ...
                if(order.seller != address(0)){
                    ...
                    if(order.fixedPrice > 0){
                        err = "INCORRECT_MAX_BUY_PER_WALLET";
        ...
        if(isValid){err = "OK"; lockAddr = order.lock; tokenId = order.nftId;}
    ...
    function _extractAuctionInfo(bytes memory auctionData) internal pure returns(Auction memory auction) {
        ...
        // (
        //     auction.depositType, auction.lock, auction.nftId, 
        //     auction.isEscrow, auction.acceptedToken, auction.startingPrice,
        //     auction.buyNowPrice, auction.startTime, auction.endTime, auction.seller,
        //     auction.activeDepositType
        // ) = abi.decode(
        //     auctionInfo, 
        //     (
        //         uint, address, uint,
    ...
    function _extractOrderInfo(bytes memory orderData) internal pure returns(Order memory order) {
        ...
        // (
        //     depositType, lock, nftId, 
        //     isEscrow, acceptedToken, fixedPrice,
        //     maxTokenSell, maxBuyPerWallet, seller,
        //     remainingPart, activeDepositType
        // ) = abi.decode(
        //     orderInfo, 
        //     (
        //         uint, address, uint,
    ...
    function _extractBid(bytes memory bidData) internal pure returns(Bid memory bid) {
        ...
        // (offerPrice, lockedBalance, listingEndTime, bidder, listingSig) 
        // = abi.decode(bidInfo, (uint, uint, uint, address, bytes));
    ...
    returns(
        ...
        (bytes memory listingInfo,) = abi.decode(listingData, (bytes, bytes));
        // (,,,,, uint startingPrice,,, uint endTime, uint timeOffset, address seller,) = _extractAuctionInfo(listingData);
        //(uint proposedOfferPrice,,,, bytes memory proposedListingSig) = _extractBid(proposedBid);
        Auction memory auction = _extractAuctionInfo(listingData);
        //Bid memory bid = _extractBid(proposedBid);
    ...
    returns(
        ...
        // (
        //     uint activeOfferPrice,, uint activeEndTime, 
        //     address activeBidder, bytes memory activeListingSig
        // ) = _extractBid(activeBid);
        // (uint proposedOfferPrice,,,,) = _extractBid(proposedBid);
        //Bid memory activeBid = _extractBid(activeBidData);
        //Bid memory proposedBid = _extractBid(proposedBidData);
