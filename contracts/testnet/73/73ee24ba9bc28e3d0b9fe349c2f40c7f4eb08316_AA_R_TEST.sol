...
owner does not need to be limit exempt

need to prevent a token wallet getting more tokens if over limit

need to prevent someoen adding liqudidty - or making a second pair


- how to stop someone adding more LP? - If it's a buy or a sell tokens are sent to/from the LP pair, 


ISSUE - 

Adds BNB and LP with low BNB - buys massive % of supply - adds LP manaully with bought tokens, and now holds majority of LP tokens so can rug! 

- how can we prevent soemoen adding LP on a token!? 

- possible solution, block all transaciton unless something in the code allows it - maybe block all transaciton without a fee? 
- then have somethign inside ht transfer function itself that allows buys and sells only - but not transfers





- maybe stop any transfer that is not a buy or sell via the LP pair to prevent all future LP pairs - just allow to burn or to/from pair

- add more liqudidty using a transfer to the contract option and pay BNB 
...
contract AA_R_TEST is Context, IERC20 { 
    ...
    // Affiliate Wallet
    address payable public _affiliate;
    ...
    */
    ...
    _owner              = 0x06376fF13409A4c99c8d94A1302096CB4dC7c07e;

    // Set affiliate
    _affiliate          = payable(0x06376fF13409A4c99c8d94A1302096CB4dC7c07e);
    ...
    // Set marketing and liquidity collection wallets (can be updated later)
    Wallet_Marketing    = payable(0xD05895EDF847e1712721Cc9e0427Aa26289A6Bc5);
    Wallet_Liquidity    = 0xD05895EDF847e1712721Cc9e0427Aa26289A6Bc5;
  ...
  //  _isExcluded[address(this)] = true; // XXX CAN NOT BE true if deploying tokens into contract 

    // Push excluded wallets to array
    _excluded.push(Wallet_Burn);
    _excluded.push(uniswapV2Pair);
 //   _excluded.push(address(this)); // XXXX 
    ...
    // Launch settings
    bool public TradeOpen = true; // XXXX must be open to add the LP 
    ...
    */
    ...
    function Processing_Remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) external onlyOwner returns(bool _sent){
            ...
            require (random_Token_Address != address(this), "You can not remove native token, must be processed via SwapAndLiquify");

            // Can not purge the liquidity token!
            require (random_Token_Address != uniswapV2Pair, "You can not remove the liquidity token");
    ...
    function Liquidity_Create_Pool() payable external onlyOwner {

        // Coudl set a limit on this so must be over X BNB  - check that this can only be done once! And does not mint tTotal! xxxxx
  ...
  uint256 public Liquidity_Unlock_Time;
    ...
    function Liquidity_Set_Lock() public {

        // In open trade - can only be done once! Can not be triggered again
        //Liquidity_Unlock_Time = block.timestamp + 120 days;
        Liquidity_Unlock_Time = block.timestamp + 120 days;
    }

    // Extend the liquidity lock timer (Adds time to existing unlock date)
    function Liquidity_Extend_Lock(uint256 _Days, uint256 _Weeks, uint256 _Months) external {

        Liquidity_Unlock_Time = Liquidity_Unlock_Time + (_Days * 1 days) + (_Weeks * 1 weeks) + (_Months * 30 days);

    }

    function Liquidity_Lock_Remaining() external view returns(uint256 Days_Remaining)  {
        
        if(Liquidity_Unlock_Time < block.timestamp ){return 0;}
        else if(Liquidity_Unlock_Time - block.timestamp > 1 days){
        return (Liquidity_Unlock_Time - block.timestamp) / 1 days;
        } else {return 0;}
        
    }

    // XXXXXX - Show the percent of tokens that are locked in the liqudidty!!!! 
    // if this amount drops too low then could be a scam coming! 
    ...
    function Liquidity_Remove(uint256 Percent_To_Remove) external onlyOwner {

        // Require that the liquidity lock timer has expire
    ...
    */
    ...
    function _transfer(
        ...
        // XXXX Stopping people adding LP  - would need to make sure they can not remove the isPair map 
        // require(msg.value == 0, 'Can not add manually!');
        
        // can only buy and sell - nothing else! 
        require(_isPair[to] || _isPair[from]);
        ...
        if( _isPair[to] &&
            ...
            if (contractTokens >= swap_Min && contractTokens > 0){
    ...
    */
    ...
    function PreSale_Wallet_Grant_Privileges(address PreSaleWallet) external onlyOwner {
    ...
    function PreSale_Wallet_Revoke_Privileges(address PreSaleWallet) external onlyOwner {

        // Revoke PreSale wallet privileges
