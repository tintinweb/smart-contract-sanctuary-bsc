...
contract PensionSeed is Ownable {
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;
    
    IBEP20 public newtoken;
    uint8 public newtokenDecimals = 18;



    uint256 public newTokenSupply = 550 * 10**18;

    address public devWallet = 0xdc976a15e3929759df943544c918B4E2A68C3F9e;
    ...
    event NewTokenTransfered(address indexed operator, uint256 sendAmount);
        ...
        internal view returns (uint256)
      {
        ...
        uint256 public investorMinCap = 1000000000000000000; // 1 bnb
        uint256 public investorMaxCap = 2000000000000000000; // 2 bnb
    ...
    // Update tokens
    function updateTokens(address _newtoken, uint8 _newtokenDecimals) external onlyOwner {
        newtoken = IBEP20(_newtoken);
        newtokenDecimals = _newtokenDecimals;
    }

    // Update Caps
    function updateCap(uint256 _investorMinCap, uint256 _investorMaxCap, uint256 _investorHardCap, uint256 _cap) external onlyOwner {
    ...
    mapping(address => bool) public whitelist;
      ...
      function processContribute(address _beneficiary) public payable returns (uint256 _allowCont) 
        {
         if( whitelist[_beneficiary] = true ) {
                 _allowCont = investorMaxCap;
            }
            else {
                 _allowCont = 0;
            }
    ...
    function processClaim(address _beneficiary) public {
        ...
        isWhitelisted(_beneficiary)
      {
        ...
        require(_newContribution >= investorMinCap && _newContribution <= _allowedContribution && _newTotalContribution <= investorHardCap);
    ...
    */
    ...
    function getAllowedContribution(address _beneficiary) 
