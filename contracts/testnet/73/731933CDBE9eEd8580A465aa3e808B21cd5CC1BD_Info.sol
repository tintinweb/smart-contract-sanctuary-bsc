...
contract Info is OwnableUpgradeable{
    ...
    function cowInfoes(uint tokenId) external view returns(uint[22] memory info1,bool[3] memory info2, uint[2] memory parents){
    ...
    function getUserProfilePhoto(address addr_) public view returns(string[] memory) {
        ...
        uint index;
        // 1.bovine hero
        uint []memory bovineHeroPhoto = cattle.checkUserCowListType(addr_, true);
