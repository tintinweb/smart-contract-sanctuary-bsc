...
contract SoccerGold is Context, IBEP20, LockToken 
{
    ...
    string private _name = "Soccer Gold";
    string private _symbol = "SGOLD";
    ...
    {
        uint256 _totalFee = _liquidityFee.add(_marketingFee).add(_buybackFee);
                _totalFee =  _totalFee.add(_sellLiquidityFee).add(_sellMarketingFee).add(_sellBuybackFee);
    ...
    {
        return _amount.mul(_liquidityFee+_marketingFee+_buybackFee).div(100);
    }
    ...
    function setBuyFeePercents(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee) 
    external onlyOwner() 
    {
        _taxFee = taxFee;
        ...
        uint256 _totalFee = taxFee.add(liquidityFee).add(marketingFee).add(buybackFee);
        require(_totalFee<=15, "Too High Fee");
    }

    function setSellFeePercents(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 buybackFee) 
    external onlyOwner() 
    {
        _sellTaxFee = taxFee;
