...
contract Staking is Ownable, ReentrancyGuard {
    ...
    *   @notice Return staked token with interest
    *   @param _index The ID of the stake to be returned
     */
    function unStakeToken(uint _index) external nonReentrant returns (bool){
        Stake memory selectedStake = stake[msg.sender][_index];
        ...
        uint totalInterest = calculateInterest(_index);
        ...
        stake[msg.sender][_index].returned = true;

        if(busdContract.transfer(msg.sender, total_amount)){
            emit UnStake(_index, total_amount);
        }else{
            revert("Unable to transfer funds");
        }
    ...
    * @param _index The ID of the stake of the caller
     */
    function getCurrentStakeAmount(uint _index) external view returns (uint256)  {
        return stake[msg.sender][_index].deposit_amount;
    }
    ...
    * @return Amount of stake
    ...
    function getStakeInfo(uint _index) external view returns(
        uint,   // Returns the user deposited amount
        bool,   // current status of the stake
        uint,   // Stake Creation time
        uint,   // Stake Ending time
        uint,   // Earnerd Intrest
        uint    // penalty on withdraw
        ){

        Stake memory selectedStake = stake[msg.sender][_index];

        uint earnedIntrest = calculateInterest(_index);
        uint amountWithIntrest = earnedIntrest + selectedStake.deposit_amount;
        uint penalty;
        if(block.timestamp <= selectedStake.stake_end_time){
            penalty = calculatePenalty(amountWithIntrest);
        }
        return (
    ...
    // Returns all stakes of the msg.sender
    ...
    * @return Number of active stake
     ...
     * param uint _index The stake where the reward should be calculated
     * @return The reward value
     */
    function calculateInterest(uint _index) internal view returns (uint){
        Stake memory _stake = stake[msg.sender][_index];

        uint amount_staked = _stake.deposit_amount;
        uint periods = calculatePeriods(_index);  //Periods for interest calculation
    ...
    function calculatePeriods(uint _index) public view returns (uint){
        Stake memory _stake = stake[msg.sender][_index];
    ...
    function calculateRewardToWithdraw(address _account, uint _index) internal view returns (uint){
        Stake memory _stake = stake[_account][_index];
        ...
        total_interest = interestPerDay.mul(periods);  //IntrestPerDay * noOfDays
        }
        return total_interest;
    }
