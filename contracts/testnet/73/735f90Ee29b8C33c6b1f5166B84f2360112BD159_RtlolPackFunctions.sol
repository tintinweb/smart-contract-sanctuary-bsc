...
contract RtlolPackFunctions is AccessControl, ReentrancyGuard {
    ...
    IRainiCardPacks public packsContact;
    ...
    constructor(
        ...
        packsContact = IRainiCardPacks(_packsContract);
        verifier = _verifier;
    }
    ...
    function runLottery(uint256 id, bytes32 lotteryHash, uint256 prizeCount) external onlyEditor {
    ...
    function claimPacks(
        ...
        uint256[] memory _amount) external {

        require (!sigUsed.get(transactionId), "packs claimed");
        sigUsed.set(transactionId);

        bytes memory _hashString = abi.encode(transactionId, "|claimPacks|");       
