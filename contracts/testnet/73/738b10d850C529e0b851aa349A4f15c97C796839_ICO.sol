...
contract ICO {      
    ...
    function update_ICO(IERC20 _stakingToken, IERC20 _stakingFromToken, uint40 _startTime, uint40 _endTime, uint256 _ratePerToken, bool _ICO_status) public onlyOwner returns(bool){        
    ...
    */    
    ...
    function _buy(address sender, uint256 amount) internal {
        ...
        uint256 tokens = amount * ratePerToken;
    ...
    function claim (uint256 invest_id,uint256 days_cnt) public returns(bool){
        ...
        if(investments[invest_id].upto > uint256(block.timestamp)){
           // uint256 days_cnt = uint256((uint40(block.timestamp) - investments[invest_id].timestamp)/ 1 days);
            uint256 ttl_income = ((investments[invest_id].amount * 50)/100 * days_cnt)/100;
