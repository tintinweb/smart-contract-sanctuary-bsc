...
contract KbaCoinSC is Context, IERC20, Ownable {  
    ...
    function updateBuyFee(uint buyTaxFee,uint marketingBuyPer, uint developmentBuyPer) onlyOwner public {
    ...
    function updateSellFee(uint sellTaxFee,uint marketingSellPer,uint developmentSellPer,uint autoBurnSellPer) onlyOwner public {
    ...
    function _transfer(
        ...
        if(from!=address(this)){
         require (_rOwned[from] > amount,"Transfer amount must be less than available balance");
        }
   ...
   */
    ...
    function swapAndLiquify(uint256 contractTokenBalance,uint TaxType) private lockTheSwap {
        ...
        //swap tokens for ETH
        if(forMarketing.add(forDevelopment)>0)
        swapTokensForEth(forDevelopment.add(forMarketing)); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        ...
        if(TaxType==1){
            EachPer=100/(_developmentBuyPer.add(_marketingBuyPer));
            MarketingPer=EachPer*_marketingBuyPer;
            DevelopmentPer=EachPer*_developmentBuyPer;
        }
        else{
            EachPer=100/(_developmentSellPer.add(_marketingSellPer));
        ...
        emit SwapAndLiquify(forMarketing,FullExp, Balance,forDevelopment);
    }
