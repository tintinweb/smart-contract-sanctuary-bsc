...
contract FarmPair is FarmHashrate, Initializable, Ownable {
    ...
    function init(IAppConf _appConf, IPancake _pancake, IFarmUser _farmUser) public onlyOwner {
        appConf = _appConf;
        pancake = _pancake;
        farmUser = _farmUser;
    ...
    function stakePair(uint256 usdtAmount) public needInit {
        ...
        // calc hashrate
        uint256 pairUsdtAmount = SafeMath.div(usdtAmount * 1000, appConf.getPairUsdtRate());
        ...
        _burnToken(quoteToken, quoteAmount);

        // handle usdt token
        if (hc.tokenRebate == 0) {
            IERC20Ex(usdtToken).safeTransferFrom(userAddr, appConf.getCoolAddr(), usdtAmount);
        }

        // swap & burn 
        uint256 swapUsdtAmount = SafeMath.div(SafeMath.mul(usdtAmount, appConf.getPairUsdtSwapRate()), 1000);
...

        {
            ...
            uint256[2] storage amounts = pairPoolMap[quoteToken][usdtToken][userAddr];
            amounts[0] = SafeMath.add(amounts[0], quoteAmount);
            amounts[1] = SafeMath.add(amounts[1], usdtAmount);

            // update user hashrate
            pairHashrateMap[userAddr][quoteToken][usdtToken] = SafeMath.add(pairHashrateMap[userAddr][quoteToken][usdtToken], hashrate);
        ...
        // to rebate
        {
            if (hc.rebate == 1 || hc.tokenRebate == 1) {
                uint256 rebateUsdtAmount = SafeMath.sub(usdtAmount, swapUsdtAmount);
                ...
                for (uint8 gen = 1; gen <= appConf.getMaxGen(); gen++) {
                    ...
                    if (hc.tokenRebate == 1) {
                        uint256 usdtCommission = SafeMath.div(SafeMath.mul(rebateUsdtAmount, commissionRate), 1000);
                        IERC20Ex(usdtTokenInner).safeTransferFrom(userAddr, genUser.addr, usdtCommission);
                    }

                    if (hc.rebate == 1) {
                        uint256 hashrateCommission = SafeMath.div(SafeMath.mul(hashrate, commissionRate), 1000);
                        userHashrateMap[genUser.addr] = SafeMath.add(userHashrateMap[genUser.addr], hashrateCommission);
        ...
        emit PairStaked(userAddr, quoteToken, quoteAmount, usdtToken, usdtAmount, hashrate);
    }

    function calcTokenHashrate(uint256 usdtAmount, uint256 multiple) public view returns (uint256) {
        // hashrate is calculated by usdt
        uint256 hashrate = SafeMath.div(SafeMath.mul(usdtAmount, multiple), 1000 * (10**pancake.getUsdtDecimals()));
        return hashrate;
    }
    ...
    function clearHashrate(address userAddr) external override onlyFarm {
        // clear user hashrate
        totalHashrate = SafeMath.sub(totalHashrate, userHashrateMap[userAddr]);
        ...
        for (uint8 gen = 1; gen <= appConf.getMaxGen(); gen++) {
            ...
            userHashrateMap[genUser.addr] = SafeMath.sub(userHashrateMap[genUser.addr],commission);
            totalHashrate = SafeMath.sub(totalHashrate, commission);
