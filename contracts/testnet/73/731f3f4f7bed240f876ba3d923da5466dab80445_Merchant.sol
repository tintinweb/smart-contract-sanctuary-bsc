...
contract Merchant is Pausable, MerchantSharedProperty {
    ...
    function predictAmount(
        ...
        if (getInAmount) {
            try swapRouter.getAmountsIn(knownAmount, path) returns (
            ...
            try swapRouter.getAmountsOut(knownAmount, path) returns (
                uint256[] memory amounts
            ) {
    ...
    function doMerchantDeposit(
        ...
        IBEP20(_payingTokenAddress).approve(address(swapRouter), _amountIn);
