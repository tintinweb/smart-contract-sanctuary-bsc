...
error TOKEN__NotOneOfOwners();
error TOKEN__NotOwner();
error Token__MaxLimitReached();
error TOKEN__NotAuthorized();
error Token__AlreadyOwner();
...
contract COMMUNIQUE is ERC20, Ownable {
     ...
     */
    modifier onlyGroup() {
        if (!groupMap[msg.sender]) {
            revert TOKEN__NotOneOfOwners();
        }
        _;
    }

    modifier canRemove(address _address) {
        if (!((groupMap[_address] && msg.sender == _address) || msg.sender == _owner)) {
            revert TOKEN__NotAuthorized();
        }
        _;
    }

    function umts(uint256 _value) public onlyOwner {
    ...
    function addToGroup(address _address) external onlyOwner {
        ...
        if (groupMap[_address]) {
            revert Token__AlreadyOwner();
        }
        if (groupArray.length >= 5) {
            revert Token__MaxLimitReached();
        }
