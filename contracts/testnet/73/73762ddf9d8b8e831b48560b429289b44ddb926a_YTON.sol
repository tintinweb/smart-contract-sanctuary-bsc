...
contract YTON is Ownable{
	...
	constructor(IERC20 _usdt,IERC20 _token,IERC20 _lp,address payable marketingAddr) public {
	...
	function investNFT(uint256 investAmount,address to) public returns (bool) {
        ...
        //LP本金 充进合约
        uint256 fee = investAmount.mul(5).div(100);
        usdt.transferFrom(msg.sender, address(this), fee);
        //LP本金 充进合约
        usdt.transferFrom(msg.sender, to, investAmount.sub(fee));
		emit FeePayed(msg.sender, investAmount);
    ...
    //提现
	function withdraw(address from,address to,uint256 amount) public onlyMarket {
