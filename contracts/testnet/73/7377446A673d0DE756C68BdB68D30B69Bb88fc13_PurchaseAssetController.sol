    ...
    UUPSUpgradeable
{
    ...
    // Guinea Pig Purchase scheme: price, likelihood and races
    uint256 public priceOne;
    uint256 public priceTwo;
    uint256 public priceThree;
    uint256[] public likelihoodRacePriceOne;
    uint256[] public likelihoodRacePriceTwo;
    uint256[] public likelihoodRacePriceThree;
    ...
    event SoFarSoGood(uint256 _ix, uint256 _guineaPigId, string _raceAndGender);
    ...
    function fulfillRandomness(
        ...
        if (_compareStrings(_tx.transactionType, "PURCHASE_GUINEA_PIG")) {
            _finishPurchaseGuineaPig(_tx.ix, _account, _randomNumbers);
    ...
    function _finishPurchaseGuineaPig(
        ...
        (
            ...
            string memory _raceAndGender
                ...
                _ix,
                (_randomNumbers[0] % 100) + 1,
                _randomNumbers[1] % 2
            );
        // Mint Guinea Pigs
        emit SoFarSoGood(_ix, _guineaPigId, _raceAndGender);
    ...
    function _setLikelihoodAndPrices() internal {
        likelihoodData[1] = LikelihoodData(5 * 1e6, new uint256[](4));
        likelihoodData[1].likelihood = [70, 85, 95, 100];

        likelihoodData[2] = LikelihoodData(10 * 1e6, new uint256[](4));
        likelihoodData[2].likelihood = [20, 50, 80, 100];

        likelihoodData[3] = LikelihoodData(15 * 1e6, new uint256[](4));
    ...
    function _getLikelihood(uint256 _ix)
        internal
        view
        returns (uint256[] memory)
    {
        if (_ix == 1) {
            return likelihoodRacePriceOne;
        } else if (_ix == 2) {
            return likelihoodRacePriceTwo;
        } else {
            return likelihoodRacePriceThree;
        }
    }
    ...
    function setLikelihoodPerPrice(
        uint256 _ix,
        uint256[] memory _likelihoodRaceArr
    ) public onlyRole(GAME_MANAGER) {
        if (_ix == 1) {
            likelihoodRacePriceOne = _likelihoodRaceArr;
        } else if (_ix == 2) {
            likelihoodRacePriceTwo = _likelihoodRaceArr;
        } else if (_ix == 3) {
            likelihoodRacePriceThree = _likelihoodRaceArr;
        }
    }

    function setPriceForScheme(uint256 _ix, uint256 _newPrice)
        public
