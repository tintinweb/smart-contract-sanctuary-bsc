...
contract OPTXLMS is OwnableUpgradeable {
    ...
    function getLMSAvailable() external view returns(uint256, uint256, uint256){
        
        uint256 bnbPrice = uint256(getBNBPrice());
        (uint256 optxRes, uint256 bnbRes, ) = commandBNBPair.getReserves();
        ...
        if(optxRes > optxResBuy) return (nowPrice, optxResBuy, 0);
        if(optxRes < optxResSell) return (nowPrice, 0, optxResSell);
        return (nowPrice, 0, 0);
    }
    ...
    function executeBNBLMS() external onlyDev returns(uint256){
        ...
        if(nowPrice + priceDecThreshold < priceDownThreshold) {
            uint256 optxResBuy = bnbRes * bnbPrice / priceDownThreshold;
            require(optxRes > optxResBuy && address(this).balance > (optxRes - optxResBuy), "Not enough BNB");

            uint256 buyAmount = optxRes - optxResBuy;
    ...
    function executeBUSDLMS() external onlyDev returns(uint256){
        ...
        (uint256 optxRes, uint256 busdRes, ) = commandBUSDPair.getReserves();
