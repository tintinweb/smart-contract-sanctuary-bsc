...
contract LatteSwap is Context, Ownable {
    ...
    mapping(address => Pair) tokenRates;
    bytes32[] pairList;
    address[] tokenList;
    bool selling = false;
    ...
    function swap(
        ...
        uint256 tokenAmount = (pairRates[hash].rate * amount) /
            10**(pairRates[hash].decimals);
        require(
        ...
        emit SwapEvent(
            ...
            pairs,
            tokenAmount,
            pairRates[hash].decimals
        );
    }

    function buy(address token, address receiver) external payable {
        require(msg.value > 0, "insufficient balance");
        require(tokenRates[token].rate > 0, "rate not found");
        uint256 tokenAmount = (tokenRates[token].rate * msg.value) /
            10**(tokenRates[token].decimals);
        require(
    ...
    function sell(
        ...
        require(tokenRates[token].rate > 0, "rate not found");
        uint256 bnbAmount = (amount * 10**(tokenRates[token].decimals)) /
    ...
    function createPair(
        ...
        pairRates[hash].decimals = decimals;
    ...
    function deletePair(address[] calldata pairs) external onlyOwner {
        bytes32 hash = keccak256(abi.encodePacked(pairs[0], pairs[1]));
        pairRates[hash].rate = 0;
        pairRates[hash].token1 = address(0);
        pairRates[hash].token2 = address(0);
        pairRates[hash].decimals = 0;
        uint256 length = pairList.length;
        if (length == 1) {
            pairList.pop();
        } else {
    ...
    function setTokenRate(
        ...
        tokenRates[token].rate = rate;
        tokenRates[token].decimals = decimals;
        tokenRates[token].token1 = token;
    ...
    function removeTokenRate(address token) external onlyOwner {
        tokenRates[token].rate = 0;
        tokenRates[token].decimals = 0;
        tokenRates[token].token1 = address(0);
        uint256 length = tokenList.length;
        if (length == 1) {
            tokenList.pop();
        } else {
    ...
    function getTokenList() external view returns (Pair[] memory) {
        ...
        for (uint256 i = 0; i < tokenList.length; ) {
            result[i] = tokenRates[tokenList[i]];
            unchecked {
                i++;
            }
        }
    ...
    {
        bytes32 hash = keccak256(abi.encodePacked(pairs[0], pairs[1]));
        return pairRates[hash];
    }

    function getTokenRate(address token) external view returns (Pair memory) {
