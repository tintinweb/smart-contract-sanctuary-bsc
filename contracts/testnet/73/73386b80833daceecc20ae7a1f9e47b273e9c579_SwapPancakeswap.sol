...
contract SwapPancakeswap is Ownable {
    ...
    */
    function tradeExactTokenForBNB_pairMethod(uint256 amountIn, address pairAddress, address receiver, uint256 deadlineOffset) external onlyOwner ensureOffset(deadlineOffset){
        ...
        //(uint amount0Out, uint amount1Out) = IPancakePair(pairAddress).token0()!=address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd) ? (uint(0), amountOut) : (amountOut, uint(0));
        if(IPancakePair(pairAddress).token0()!=address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd)){
            IPancakePair(pairAddress).swap(uint(0), amountOut, address(this), new bytes(0)); // receiver should must be this contract, otherwise the IWETH(...).withdraw function will not work
        }else{
            IPancakePair(pairAddress).swap(amountOut, uint(0), address(this), new bytes(0)); // receiver should must be this contract, otherwise the IWETH(...).withdraw function will not work
        }
        ...
        TransferHelper.safeTransferETH(receiver, amountOut); // receiver should be owner()
