...
contract DBSNFTCast  is Ownable{
    ...
    function GetCast() public  returns (uint256, uint256){
        ...
        require(block.timestamp >= oldTime.add(7*24*3600), "DBSNFTTechnology: GetCast Time error");
        // Transfer the token
        uint256 tableid = techAddress.GetRandomShip(NFTCopper);
    ...
    function GetEndCastTime() public view returns (uint256){
        return CastShip[msg.sender].add(7*24*3600);
    }
