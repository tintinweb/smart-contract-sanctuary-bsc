...
contract IKONICVesting is Ownable,ReentrancyGuard{
    ...
    uint[] public intermediateVestingTimePeriod=[0,0,0,0,0,0,0,0,30 minutes,0,0];
    uint[] public linearVestingTimePeriod=[0,660 minutes,540 minutes,365 minutes,180 minutes,900 minutes ,1080 minutes ,1050 minutes,720 minutes,1350 minutes,480 minutes];
    ...
    function setDates (uint startTime) external onlyOwner {
        ...
        linearVestingAmountWithdrawThresholdTime= [0,startTime+ 30 minutes ,startTime + 1 minutes ,startTime + 1 minutes,startTime +  1 minutes ,startTime + 20 minutes,startTime + 20 minutes,startTime + 30 minutes,startTime + 40 minutes, startTime + 20 minutes,startTime + 1 minutes];
    ...
    function intermediateVestStatus(address _userAddress) public view returns (uint,uint) {
        ...
        uint timeDifference;
        uint[11] memory intermediateArray = [0,0,0,0,0,0,0,0,globalStartTime + 14 minutes,0,0];// give Intermediate time
    ...
    function linearVestingDetails(address _userAddress) public view returns (uint) {
        uint lastClaimTime = InvestorDetails[_userAddress].lastClaimTime;
        uint timeDifference;
        uint[11] memory linearTime =[0,globalStartTime+ 30 minutes ,globalStartTime + 1 minutes ,globalStartTime + 1 minutes,globalStartTime +  1 minutes ,globalStartTime + 20 minutes,globalStartTime + 20 minutes,globalStartTime + 30 minutes,globalStartTime + 40 minutes, globalStartTime + 20 minutes,globalStartTime + 1 minutes];
