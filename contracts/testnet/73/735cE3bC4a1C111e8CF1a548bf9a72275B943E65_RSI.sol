...
contract RSI is ERC20, Ownable {
    ...
    uint256 public _maxTxAmountBuy = 1000000 * 10**18;
    uint256 public _maxTxAmountSell = 500000 * 10**18;
    ...
    constructor() ERC20("RSI", "RSI") {

        _owner = 0x3056dbEfDfc73Ba92968616458c03049D7732e3C;
        ...
        marketingWallet = address(0x31ec3153F576bc43F25689C8cB15a6e481523CC2); // set as marketing wallet
    ...
    function checkTxLimitBuy(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmountBuy || _isExcludedFromFees[sender], "TX Limit Exceeded");
    }

    function checkTxLimitSell(address sender, uint256 amount) internal view {
        require(amount <= _maxTxAmountSell || _isExcludedFromFees[sender], "TX Limit Exceeded");
    }

    function setTxLimitBuy(uint256 amount) public onlyOwner {
        _maxTxAmountBuy = amount * 10**18;
    }

    function setTxLimitSell(uint256 amount) public onlyOwner {
        _maxTxAmountSell = amount * 10**18;
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0){
                checkTxLimitSell(from, amount);
                fees = amount.mul(sellTotalFees).div(100);
                tokensForLiquidity += fees * sellLiquidityFee / sellTotalFees;
                tokensForMarketing += fees * sellMarketingFee / sellTotalFees;
                require(lastTrade[from] < (block.timestamp - tradeCooldown), string("No consecutive sells allowed. Please wait."));
                lastTrade[from] = block.timestamp;
            }
            // on buy
            else if(automatedMarketMakerPairs[from] && buyTotalFees > 0) {
                checkTxLimitBuy(to, amount);
