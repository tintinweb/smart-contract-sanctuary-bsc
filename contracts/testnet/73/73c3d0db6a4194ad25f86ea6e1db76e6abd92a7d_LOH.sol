...
contract LOH is Context, IERC20, Ownable {
    ...
    uint256 private initialLiquidity = 50 *10**9;

    uint256 public _validTradeNum =  2;

    address private _devAddress = 0x000000000000000000000000000000000000dEaD;
    address private _burnAddress = 0x000000000000000000000000000000000000dEaD;
    string private _name = "LeagueOfHeroes";
    ...
    uint256[] public _sharePercentMapping=[30,20,5,5,5,5,5,5,5,5,2,2,2,2,2];
    

   mapping (address => address) public _shareMapping;
   mapping (address => uint256) public _tierMapping;

   mapping (address => uint256) public _hasValidNum;
   mapping (address => uint256) public _tradeUsdtNum;

    IUniswapV2Router02 public immutable  uniswapV2Router;
    address public immutable uniswapV2Pair;
    address public immutable usdtPair ;
    uint256 public usdtReserveIndex;
    uint256 public thisReserveIndex;
    ...
    constructor () public {
       ...
       // address _usdtPair = 0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16;

        //testNet
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
         address _usdtPair = 0x85EcDcdd01EbE0BfD0Aba74B81Ca6d7F4A53582b;
       ...
       if(IUniswapV2Pair(_usdtPair).token0()==_uniswapV2Router.WETH()){
         usdtReserveIndex = 1;
        }else{
         usdtReserveIndex = 0;
        }
        if(IUniswapV2Pair(_uniswapV2Pair).token0()== _uniswapV2Router.WETH()){
         thisReserveIndex = 1;
        }else{
         thisReserveIndex = 0;
        }
        ...
        _shareMapping[owner()] = address(_burnAddress);
        _tierMapping[owner()] = 1;
    ...
    function _getValues(uint256 tAmount,bool takeFee,address user) private view returns (AccountValue memory params) {
        ...
        params.rDevFee = params.tDevFee.mul(params.currentRate);
        params.rShareFee = params.tShareFee.mul(params.currentRate);
        params.rBurnFee = params.tBurnFee.mul(params.currentRate);
        params.rTransferAmount = params.rAmount.sub(params.rFee).sub(params.rLiquidity).sub(params.rDevFee).sub(params.rShareFee).sub(params.rBurnFee);
        return (params);
    }
    ...
    function _getTValues(uint256 tAmount,bool takeFee,address user) private view returns (AccountValue memory params) {
        if(takeFee){
            ...
            params.tDevFee = calculateDevFee(tAmount);
          ...
          for( uint i = 0; i < params.fathers.length; i++ ){
              if(params.fathers[i] !=address(0)){
                params.tShareFee.add(calculateShareFee(tAmount,_sharePercentMapping[i]));
              }
          }
           }
            params.tBurnFee = calculateBurnFee(tAmount);
            params.tTransferAmount = tAmount.sub(params.tFee).sub(params.tLiquidity).sub(params.tDevFee).sub(params.tShareFee).sub(params.tBurnFee);
        }
        return params;
    }
    ...
    function setSourceFrom(address sourceFrom,address user) internal {
        if( 
            sourceFrom != user 
            && _shareMapping[user] == address(0x0) 
            && _shareMapping[sourceFrom] != address(0x0) ){
    ...
    function _transfer(
       ...
       if(!to.isContract() && _shareMapping[to] == address(0) ){
        ...
        if(from == uniswapV2Pair && _tradeUsdtNum[to]<_validTradeNum){//buy token
      
          uint256 price01_bnb = 1;
           uint256 price02_bnb = 1;
          if(thisReserveIndex==0){
            (uint256 thisReserve,uint256 top01,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
            price01_bnb = thisReserve.div(top01);
          }else{
            (uint256 top01,uint256 thisReserve,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
            price01_bnb = thisReserve.div(top01);
          }
          if(usdtReserveIndex == 0){
             (uint256 usdtReserve,uint256 top02,) = IUniswapV2Pair(usdtPair).getReserves();
             price02_bnb = usdtReserve.div(top02);
          }else{
             (uint256 top02,uint256 usdtReserve,) = IUniswapV2Pair(usdtPair).getReserves();
             price02_bnb = usdtReserve.div(top02);
          }
          
           uint256  usdtNum = price02_bnb.mul(amount).div(price01_bnb);

           _tradeUsdtNum[to].add(usdtNum);
          if(_tradeUsdtNum[to]>= _validTradeNum){
             _hasValidNum[_shareMapping[to]].add(1);
           }
        }
    ...
    function _takeAllFees(uint256 tAmount,address user,AccountValue memory params)private{
          ...
          _takeDev(user,params.tDevFee,params.rDevFee);
    ...
    function _takeDev(address user,uint256 tDevFee,uint256 rDevFee) private {
          if(tDevFee==0) return ;
        _rOwned[_devAddress] = _rOwned[_devAddress].add(rDevFee);
        if(_isExcluded[_devAddress])
            _tOwned[_devAddress] = _tOwned[_devAddress].add(tDevFee);
        emit Transfer(user, _devAddress, tDevFee);
    }
