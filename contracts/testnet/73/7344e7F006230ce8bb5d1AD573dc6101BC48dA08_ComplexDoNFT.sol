...
contract Royalty is IRoyalty {
    ...
    mapping(address => bool) private supportTokens;
    ...
    function setSupportTokens(address[] calldata tokens,bool isSupport) virtual onlyRoyaltyAdmin external{
        for(uint index = 0;index < tokens.length; index++){
            supportTokens[tokens[index]] = isSupport;
        }
    }

    function isSupportToken(address token) external virtual onlyRoyaltyAdmin returns(bool){
...
interface IRoyalty {
    ...
    function setSupportTokens(address[] calldata tokens,bool isSupport) external;

    function isSupportToken(address token) external returns(bool);
}
