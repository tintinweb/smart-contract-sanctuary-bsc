...
contract SimpleTimelock {
    ...
    mapping(address => uint256) public lastTimestampWithdrawn;
    ...
    /// @param _startTime Time allowed to start receive. Unit: second
    ...
    function setTimestamp(uint256 _startTime, uint256 _timePeriodInSeconds) public onlyOwner timestampNotSet  {
        timestampSet = true;
        startTime = _startTime;
        timePeriod = startTime.add(_timePeriodInSeconds);
    }
    ...
    function depositTokens(address recipient, uint256 amount) public onlyOwner timestampIsSet incomingDepositsStillAllowed {
        ...
        lastTimestampWithdrawn[recipient] = startTime;
    ...
    function bulkDepositTokens(address[] calldata recipients, uint256[] calldata amounts) external onlyOwner timestampIsSet incomingDepositsStillAllowed {
        ...
        for(uint256 i = 0; i < recipients.length; i++) {
            depositTokens(recipients[i], amounts[i]);
        }
    }
    ...
    function transferTimeLockedTokensAfterTimePeriod(IERC20 token, address to, uint256 amount) public timestampIsSet noReentrant {
        ...
        require(balances[to] > 0, "Insufficient token balance");
        ...
        uint256 recipientTime = safeRecipientTime();

        if (recipientTime >= startTime) {
            uint256 maxAmount = calculateAmount(to, recipientTime);
            if (amount == 0) {
                amount = maxAmount;
            }
            require(amount <= maxAmount && balances[to] >= amount, "Insufficient token balance, try lesser amount");
            alreadyWithdrawn[to] = alreadyWithdrawn[to].add(amount);
            balances[to] = balances[to].sub(amount);
            token.safeTransfer(to, amount);
            lastTimestampWithdrawn[to] = recipientTime;
    ...
    /// @dev Calculate the amount received
    /// @param to - received address
    /// @param recipientTime block.timestamp or timePeriod
    function calculateAmount(address to, uint256 recipientTime) public view returns(uint256) {
        
        // The last time to receive
        if (timePeriod == recipientTime) {
            return balances[to];
        }

        // Length of time remaining to receive
        uint256 leftoverTime = timePeriod.sub(lastTimestampWithdrawn[to]);
        require(leftoverTime > 0, "Insufficient leftover timestamp");

        // Quantity that can be claimed for each length of time
        uint256 amountRate = balances[to].div(leftoverTime);

        // The length of time that can be received this time
        uint256 canTime = recipientTime.sub(lastTimestampWithdrawn[to]);

        // Amount available this time
        return amountRate.mul(canTime);
    }

    /// @dev Safely get pick up recipient time
    /// @return min time
    function safeRecipientTime() public view returns (uint256) {
        return Math.min(block.timestamp, timePeriod);
    }
