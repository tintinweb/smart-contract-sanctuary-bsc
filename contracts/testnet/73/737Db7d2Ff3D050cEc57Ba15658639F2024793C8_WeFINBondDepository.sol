...
contract WeFINBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
        ...
        address public immutable WeFIN; // token given as payment for bond
    ...
    uint256 public basePrice = 1000000000;
    ...
    struct Bond {
        uint256 payout; // weFIN remaining to be paid
    ...
    function setAdjustment(
        ...
        require(
            _increment <= terms.controlVariable.mul(25).div(100),
            "Increment too large"
        );
    ...
    function setBasePrice(uint256 _basePrice) external onlyPolicy {
    ...
    function deposit(
        ...
        if (fee != 0) {
            ...
            IERC20(WeFIN).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(WeFIN).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(WeFIN).approve(stakingHelper, _amount);
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(WeFIN).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(WeFIN).totalSupply();
