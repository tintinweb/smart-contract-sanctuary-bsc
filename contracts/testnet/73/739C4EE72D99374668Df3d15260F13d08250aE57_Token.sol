...
contract Token is Context, IERC20, Ownable {
    ...
    mapping(address => uint256) private _vipAmount;
    ...
    uint256 public _vipLimit = 0.01 * 10**18;
    uint256 public _swapLimit = 0.05 * 10**18;
    uint256 public _lotteryAmount = 0.02 * 10**18;
    ...
    function _transfer(
        ...
        bool isVipTime = activateTime > 0 &&
            timestamp < activateTime + 10 minutes;
        ...
        if (activateTime == 0) {
            ...
            require(
                _isExcludedFromFee[from] ||
                    ...
                    // _isVip[from] ||
                    (isSwapPair(from) && _isVip[to]),
                "Ban bot for 10 minutes"
            );
            takeFee = false;
        }
        ...
        if (isSwapPair(to)) {
            ...
            //     uint256 newUsdtAmount = getSellAmount(amount)[1].add(
            //         _vipAmount[from]
            //     );
            //     require(
            //         getSellAmount(amount)[1].add(_vipAmount[from]) <=
            //             _vipLimit,
            //         "Limit Swap Amount"
            //     );
            //     _vipAmount[from] = newUsdtAmount;
            // }

            // max sell 3 times
            if (takeFee) {
                require(getSellAmount(amount)[1] <= _swapLimit, 'Max swap amount limit');
            ...
            if (isVipTime && _isVip[to]) {
                uint256 newUsdtAmount = getBuyAmount(amount)[0].add(
                    _vipAmount[to]
                );
                require(newUsdtAmount <= _vipLimit, "Limit Swap Amount");
                _vipAmount[to] = newUsdtAmount;
            }

            if (takeFee) {
                require(getBuyAmount(amount)[0] <= _swapLimit, 'Max swap amount limit');
            }
