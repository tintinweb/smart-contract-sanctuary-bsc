    ...
    ReentrancyGuard
{
    ...
    function earn(
        ...
        uint256 increaseAccSharesPerStakedToken = currentShares.add(missingShares).sub(previousShares).mul(1e18).div(totalStake());
        accSharesPerStakedToken = accSharesPerStakedToken.add(
            increaseAccSharesPerStakedToken
        );

        missingShares = currentShares + missingShares - previousShares - (increaseAccSharesPerStakedToken * totalStake() / 1e18);
    }
