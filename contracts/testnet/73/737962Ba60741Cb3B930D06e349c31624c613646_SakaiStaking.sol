...
contract SakaiStaking is ERC20, Ownable {
        ...
        returns (uint256)
    {
        ...
        uint256 startCountTime = _stakers[account].checkPoint;
        uint256 rewardPerDay = getRewardPerDay(balance);
        uint256 availableDay = (block.timestamp - startCountTime) / 3 minutes;
        uint256 availableClaimReward = rewardPerDay * availableDay;
        ...
        returns (uint256)
    {
        ...
        uint256 availableDay = (timeEnd - block.timestamp) / 1 minutes;
    ...
    function claimReward(address account) public payable {
        ...
        require(
            msg.value >= bnbFee,
            "msg.value must be equal or greater than bnbFee"
        );
