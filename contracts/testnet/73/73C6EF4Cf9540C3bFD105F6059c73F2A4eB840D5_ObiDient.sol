...
contract ObiDient is Context, IERC20, Ownable {
    ...
    function purchase(uint256 bnbAmount) internal returns (bool) {
        ...
        IERC20(_busd).transfer(
            publicCampaignFundWallet,
            totalFeesBUSD.mul(_bPublicCampaignFee).div(100)
        );
        IERC20(_busd).transfer(
            developmentFundWallet,
            totalFeesBUSD.mul(_developmentFee).div(100)
        );
        if (referrals[msg.sender] != address(0)) {
            IERC20(_busd).transfer(
                referrals[msg.sender],
                totalFeesBUSD.mul(_referralFee).div(100)
            );
        }
    ...
    function sell(uint256 tokenAmount) public returns (bool) {
        ...
        _transfer(
            msg.sender,
            publicCampaignFundWallet,
            totalFeesTokens.mul(_sPublicCampaignFee).div(100)
        );
        _transfer(
            msg.sender,
            developmentFundWallet,
            totalFeesTokens.mul(_developmentFee).div(100)
        );
        if (referrals[msg.sender] != address(0)) {
            _transfer(
                msg.sender,
                referrals[msg.sender],
                totalFeesTokens.mul(_referralFee).div(100)
            );
        }
        uint256 taxTokens = totalFeesTokens.mul(_sTaxFee).div(100);
