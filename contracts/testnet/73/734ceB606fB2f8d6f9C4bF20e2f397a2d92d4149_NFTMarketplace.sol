...
contract NFTMarketplace is Ownable {
  ...
  event OfferFilled(uint offerId, uint id, address oldOwner, address newOwner, uint256 timestamp, uint price);
  event OfferCancelled(uint offerId, uint id, address owner, uint256 timestamp);
  ...
  function makeOffer(uint _id, uint _price) public {
    ...
    emit Offer(offerCount, _id, msg.sender, _price, false, false, block.timestamp);
  }
  ...
  function fillOffer(uint _offerId) public payable {
    ...
    emit OfferFilled(_offerId, _offer.id, _offer.user, msg.sender, block.timestamp, _offer.price);
  }
  ...
  function cancelOffer(uint _offerId) public {
    ...
    emit OfferCancelled(_offerId, _offer.id, msg.sender, block.timestamp);
  }
