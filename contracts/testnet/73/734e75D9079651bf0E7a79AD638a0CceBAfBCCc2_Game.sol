...
contract Game is AccessControl, ReentrancyGuard {
    ...
    bytes32 public constant BUY_TYPEHASH = keccak256("mintWithGame(uint256 nonce, bytes memory signature)");
    bytes32 public constant DEV_ROLE = keccak256("DEV_ROLE");

    address public addressMonster;
    constructor(address monster) {
        addressMonster = monster;
        _setOracleSigner(msg.sender);
    }


    function _random(uint256 nonce, uint256 max) internal view returns(uint256) {
        return uint256(
            keccak256(
                abi.encodePacked(
                    block.timestamp,
    ...
    function mintWithGame(uint256 nonce, bytes memory signature) external nonReentrant onlySigner(BUY_TYPEHASH, nonce, signature) {
        IMonster(addressMonster).safeMint(msg.sender,1);
    }
}