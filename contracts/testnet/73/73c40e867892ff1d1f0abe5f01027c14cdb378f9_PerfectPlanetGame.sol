...
contract PerfectPlanetGame is ERC721Holder,Ownable {
    ...
    IERC721 public wep1Nft = IERC721(0xE9935Ef1a0b806361483018399364a5dE8Dd44A8);
    IERC721 public wep2Nft = IERC721(0xE9935Ef1a0b806361483018399364a5dE8Dd44A8);
    IERC721 public wep3Nft = IERC721(0xE9935Ef1a0b806361483018399364a5dE8Dd44A8);
    IERC721 public dev1Nft = IERC721(0xE9935Ef1a0b806361483018399364a5dE8Dd44A8);
    IERC721 public dev2Nft = IERC721(0xE9935Ef1a0b806361483018399364a5dE8Dd44A8);
    IERC721 public dev3Nft = IERC721(0xE9935Ef1a0b806361483018399364a5dE8Dd44A8);
    IERC721 public landNft = IERC721(0x68567aeecB7B85CaB710e84F5E46158219eB93b6);
    ...
    function setMineralExchangeRate(uint256 _rate) external onlyOwner {
        mineralExchangeRate = _rate;
    }

    function setRawMeatExchangeRate(uint256 _rate) external onlyOwner {
    ...
    function getInitDurable(uint256 typeVal) public pure returns(uint256){
        if(typeVal == 1){
            return 100;
        }else if(typeVal == 2){
            return 300;
    ...
    function getInitEnergy(uint256 typeVal) public pure returns(uint256){
        if(typeVal == 1){
            return 100;
        }else if(typeVal == 2){
            return 300;
    ...
    function getLastTakeTime(uint256 landTokenId) public view returns (uint256){
        return lastTakeTimeDetail[landTokenId];
    }

    function getGoodsInfo(uint256 goodsType,uint256 goodsTokenId) public view returns (uint256 durable,uint256 energy){
        durable = durableDetail[goodsType][goodsTokenId];
        energy = energyDetail[goodsType][goodsTokenId];
    }

    function calcConfInfoRemain(uint256 landTokenId,uint256 seatNo) public view returns (uint256 remainDu,uint256 remainEn){
        if(confDetail[landTokenId][seatNo].isConf){
            ConfGoods memory confInfo = confDetail[landTokenId][seatNo];
            (remainDu,remainEn) = _calcConfInfoRemain(confInfo);
        }
    }
    ...
    function addGoodsToLand(uint256 landTokenId,uint256 seatNo,uint256 goodsType,uint256 goodsTokenId) public {
        ...
        if(!tokenIdsOfType[goodsType].contains(goodsTokenId)){
            durableDetail[goodsType][goodsTokenId] = getInitDurable(goodsType);
            energyDetail[goodsType][goodsTokenId] = getInitEnergy(goodsType);
        }
    ...
    function removeGoodsFromLand(uint256 landTokenId,uint256 seatNo) public {
        ...
        //结算单个装备的收益
        (uint256 rawMeatVal,uint256 mineralVal) = _calcConfInfoPro(confInfo);
        if(rawMeatVal > 0){
    ...
    function removeAllFromLand(uint256 landTokenId) public {
        ...
        //根据领地位置编号集合循环删除领地配置数据
        EnumerableSet.UintSet storage setNos = seatNosOfLandId[landTokenId];
        if (setNos.length() > 0) {
            for (uint256 i = 0; i < setNos.length(); i++) {
                delete confDetail[landTokenId][setNos.at(i)];
            }
        }
        //删除领地配置的位置编号
    ...
    function calcConfInfoPro(uint256 landTokenId,uint256 seatNo) public view returns (uint256 rawMeatVal,uint256 mineralVal){
        if(confDetail[landTokenId][seatNo].isConf){
            ConfGoods memory confInfo = confDetail[landTokenId][seatNo];
            (rawMeatVal,mineralVal) = _calcConfInfoPro(confInfo);
        }
    }

    //计算领地配置的装备的实际产出
    function _calcConfInfoPro(ConfGoods memory confInfo) internal view returns (uint256 rawMeatVal,uint256 mineralVal){
        //领地的上次领取时间
    ...
    function _calcConfInfoRemain(ConfGoods memory confInfo) internal view returns (uint256 remainDu,uint256 remainEn){
        ...
        remainDu = durableDetail[goodsType][goodsTokenId] - lossDu;
        remainEn = energyDetail[goodsType][goodsTokenId] - lossEn;
    }
    ...
    function takeAll(uint256 landTokenId) public {
        ...
        //获取领地总产出
        (uint256 rawMeatTotal,uint256 mineralTotal) = calcTotalPro(landTokenId);
        if(rawMeatTotal > 0){
    ...
    //计算领地的全部产出
    function calcTotalPro(uint256 landTokenId) public view returns (uint256 rawMeatTotal,uint256 mineralTotal){
        ConfGoods[] memory confList = getConfList(landTokenId);
        if(confList.length == 0){
            return (0,0);
        }
        ...
        for (uint256 i = 0; i < confList.length; i++) {
            ConfGoods memory confInfo = confList[i];
            (uint256 rawMeatVal,uint256 mineralVal) = _calcConfInfoPro(confInfo);
            totalVal1 = totalVal1 + rawMeatVal;
            totalVal2 = totalVal2 + mineralVal;
        }
        return (totalVal1,totalVal2);
    }
    ...
    function repairAll(uint256 landTokenId) public {
        ...
        for (uint256 i = 0; i < confList.length; i++) {
            ConfGoods memory confInfo = confList[i];
            confDetail[confInfo.landTokenId][confInfo.seatNo].confTime = block.timestamp;
            durableDetail[confInfo.goodsType][confInfo.goodsTokenId] = getInitDurable(confInfo.goodsType);
            energyDetail[confInfo.goodsType][confInfo.goodsTokenId] = getInitEnergy(confInfo.goodsType);
        }
    }
    ...
    function calcRepairCost(uint256 landTokenId) public view returns (uint256 ppgCost,uint256 ppfCost){
        ...
        for (uint256 i = 0; i < confList.length; i++) {
            ConfGoods memory confInfo = confList[i];
            (uint256 remainDu,uint256 remainEn) = _calcConfInfoRemain(confInfo);
            needDurableVal = needDurableVal + (getInitDurable(confInfo.goodsType) - remainDu);
            needEnergyVal = needEnergyVal + (getInitEnergy(confInfo.goodsType) - remainEn);
        }

        //计算得出所需要的金币和肉币
        if(needDurableVal > 0){
            ppgCost = needDurableVal * 1000000000000000000 / 5;
        }
        if(needEnergyVal > 0){
            ppfCost = needEnergyVal * 1000000000000000000 / 5;
        }
    }

    //计算领地配置的装备是否还有产能效果
    function _canProduce(ConfGoods memory confInfo) internal view returns (bool){
        uint256 goodsType = confInfo.goodsType;
        uint256 goodsTokenId = confInfo.goodsTokenId;
        //计算实际损耗时长，不满一小时按0小时计算
        uint256 diffHour = (block.timestamp - confInfo.confTime) / 3600;
    ...
    function calcLandProOneHour(uint256 landTokenId) public view returns  (uint256 rawMeatTotal,uint256 mineralTotal){
        ...
        if(confList.length > 0){
            ...
            rawMeatTotal = rawMeatTotalVal;
            mineralTotal = mineralTotalVal;
        }
    }
    //================领地 end===============
