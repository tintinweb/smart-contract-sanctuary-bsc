...
// File: contracts/MonkeyToken.sol
...
contract Admin is Ownable {
    ...
    function renounceAdmin() public onlyAdmin onlyOwner {
        bool _delAdmin = false;
        uint256 index;
        for (uint i=0; i<admin.length; i++) {
            if (admin[i] == msg.sender) {
...
contract MonkeyToken is ERC721, ERC721Enumerable, Pausable, Admin {
  ...
  function balanceOfUser (address user) public view returns (uint) {
      require(user != address(0), "ERC721: address zero is not a valid owner");
  ...
  function mintWhitelisted() public payable whenNotPaused isWlAddress {
    require(1 <= addressWhitelist[msg.sender], "Tu n'as plus de Token WL a achete");
    ...
    function addAddressWhitelist (address _wlAddress) public onlyOwner onlyAdmin {
        require(_wlAddress != address(0), "l'adresse ne peut pas etre l'adresse 0");
        wlAddress.push(_wlAddress);
    }

    function delAddressWhitelist (address _wlAddress) public onlyOwner onlyAdmin isWlAddress {
        uint256 index;
        for (uint i=0; i<wlAddress.length; i++) {
  ...
  function addTokenWhitelist(address[] memory _addresses) external onlyOwner onlyAdmin {
    for (uint256 i = 0; i < _addresses.length; i++) {
      require(_addresses[i] != address(0), "L'adresse ne peut pas etre 0");
      require(addressWhitelist[_addresses[i]] == 0, "La balance doit etre 0");
