...
contract DefiStaking is AccessControl, IDefi, Ownable {
    ...
    uint256 private constant TIME_UNIT = 1 days;// Time Unit in days
    ...
    function checkNativeStakeReward(address _address, uint256 stakeIndex)
        public
        view
    ...
    function checkOtherStakeReward(address _address, uint256 stakeIndex)
        public
        view
        returns (uint256 yieldReward, uint256 timeDiff)
    {
    ...
    function unstake(uint256 stakeIndex) public {
        ...
        if (stakes[msg.sender][stakeIndex]._stakeRewardType == 0) {
            (uint256 reward, uint256 daysSpent) =
                checkNativeStakeReward(msg.sender, stakeIndex);
            ...
            (uint256 DefiReward, uint256 daysSpent) =
                checkOtherStakeReward(msg.sender, stakeIndex);
            ...
            Defi.transfer(msg.sender, DefiReward*1e9);
    ...
    function addDefiTokenReward(uint256 _amount)
        public
        onlyRewardProvider
        returns (bool)
    {
        uint256 amount = _amount * 1e9;
        Defi.transferFrom(msg.sender, address(this), amount);
        return true;
    }
    ...
    function initialize(address _token0, address _token1,address _admin) external override{
...
contract DefiFactory is Ownable{
    ...
    function createPair(address tokenA, address tokenB) external onlyOwnerAndAdmin returns (address pair)  {
        ...
        IDefi(pair).initialize(token0, token1, _msgSender());
        getPair[token0][token1] = pair;
        getPair[token1][token0] = pair; // populate mapping in the reverse direction
        allPairs.push(pair);
        getPairInfo[pair] = poolInfo(pair,token0, token1);
        pool.push(poolInfo(pair,token0, token1));
        emit PairCreated(token0, token1, pair, allPairs.length);
    }

    function setAdmin(address _admin) public onlyOwnerAndAdmin{
        admin  = _admin;
    }
}
