...
contract NFTFractional is Upgradable {
    ...
    * @param _datas poolId(0), fnftId(1), poolBalance(2), exchangeRate(3), active(4), poolType(5)
    ...
    */
    function createFNFTPool(address[] memory _addrs, uint256[] memory _datas, uint256[] memory _configs) external onlyAdmins {
        ...
        if(_datas[5] == 2) {
            pool = FNFTPool(_addrs[0], _addrs[1], _datas[0], _datas[1], _datas[2], _datas[2],_datas[3], _datas[4], _datas[5], _configs);
        } else {
            pool = FNFTPool(_addrs[0], _addrs[1], _datas[0], _datas[1], _datas[2], _datas[2],_datas[3], _datas[4], 1, _configs);
        }
    ...
    */
    function withdrawFund(address _tokenAddress, address _account, uint256 _amount, uint256 _poolId) external onlyAdmins {
        FNFTPool storage pool = fnftPools[_poolId];
        require(pool.acceptToken != address(0), "Pool does not exist");
        require(IERC20(_tokenAddress).balanceOf(address(this)) >= _amount && pool.availableBalance >= _amount, "NFTFractional: Pool not has enough balance");
        // Transfer fund back to account
        IERC20(_tokenAddress).safeTransfer(_account, _amount);
        pool.availableBalance -= _amount;
        emit WithdrawFun(_poolId, _amount, _tokenAddress, _account);
    }
    ...
    */
    function claimReward(uint256[] memory _datas, string memory _claimId,bytes memory _signature) external {
        ...
        require(rewardPool.totalRewardAmount >= _datas[3], "NFTFractional: insuffcient total reward amount");
        IERC20(fnftInfo.tokenFNFT).transferFrom(msg.sender, address(this), _datas[1]);
        IERC20(rewardPool.rewardToken).transfer(msg.sender, _datas[3]);
        ERC20Token(fnftInfo.tokenFNFT).burnFrom(address(this), _datas[1]);
        rewardPool.totalRewardAmount -= _datas[3];
        emit ClaimReward(_datas[0], _datas[1], balanceSender - _datas[1] , _datas[3], msg.sender, _claimId);
    }

    function setAdminList(address[] memory _admins) external onlyController {
        for (uint256 i = 0; i < _admins.length; i++) {
            adminList[_admins[i]] = 1;
        }
    }
}
