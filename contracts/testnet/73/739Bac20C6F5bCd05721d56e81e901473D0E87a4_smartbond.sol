...
abstract contract Guardian {
    ...
    function pause(uint _paused) external onlyGuardians {
        if (_paused == paused)
            revert PauseNotChanged(msg.sender, _paused);
    ...
    function revokeApproval(uint _txIndex) external onlyGuardians nonReentrant txExists(_txIndex) notExecuted(_txIndex) {
        if (isApproved[_txIndex][msg.sender] != _TRUE)
            revert NotApproved(msg.sender, _txIndex);
    ...
    function executeTransaction(uint _txIndex) external onlyGuardians nonReentrant txExists(_txIndex) notExecuted(_txIndex) {
        ...
        if (_transaction.approvals < minApprovals) revert NotApproved(msg.sender, _txIndex);
    ...
    function _nonZero(uint _amount) private view {
        if (_amount <= 0)
            revert ZeroNotAllowed(msg.sender);
    }
    function _notNobody(address _address) private view {
        if (_address == address(0))
            revert NobodyNotAllowed(msg.sender);
    }
    ...
    function _txExists(uint _txIndex) private view {
        if (_txIndex >= transactions.length)
            revert InvalidTransaction(msg.sender, _txIndex);
    }
    function _notExecuted(uint _txIndex) private view {
        if (transactions[_txIndex].executed == _TRUE)
            revert PreviouslyExecuted(msg.sender, _txIndex);
    }
    function _notApproved(uint _txIndex) private view {
        if (isApproved[_txIndex][msg.sender] == _TRUE)
            revert PreviouslyApproved(msg.sender, _txIndex);
    }
    ...
    error ZeroNotAllowed(address caller);
    error NobodyNotAllowed(address caller);
    error ReentrancyRejected(address caller);
    error AccessDenied(address caller);
    error InvalidTransaction(address caller, uint txIndex);
    error PreviouslyExecuted(address caller, uint txIndex);
    error PreviouslyApproved(address caller, uint txIndex);
    error Paused(address caller);
    error PauseNotChanged(address caller, uint isPaused);
    ...
    error NotApproved(address caller, uint txIndex);
...
contract smartbond is botConfig, SelfInsured{
    ...
    function deposit(uint _amount, address _referrer) public pausible nonReentrant {
        if (_amount < MIN_DEPOSIT)
            revert DepositBelowMinimum(msg.sender, _amount);
    ...
    function compoundRewards() public pausible {
        if (ownedDeposits[msg.sender].length == 0)
            revert NonParticipant(msg.sender);

        // Calculate the total rewards payable
        uint _rewardAmount = _getClaimableRewards(msg.sender);
        if (_rewardAmount < MIN_DEPOSIT)
            revert DepositBelowMinimum(msg.sender, _rewardAmount);
    ...
    function withdrawPrincipal(uint id) public nonReentrant pausible {
        ...
        if (_deposit.depositor != msg.sender)
            revert UnauthorizedWithdrawl(msg.sender, _deposit.depositor);
    ...
    function createInsuranceClaim(address _depositorAddress) external insuranceFundActive notNobody(_depositorAddress) returns(uint) {
        ...
        if( enhancedInsurance == _TRUE ){
            ...
            depositors[_depositorAddress].claimableRewards = _myRewards;
    ...
    function getInsuranceEstimate(address _depositorAddress) external insuranceFundActive notNobody(_depositorAddress) view returns(uint) {
        uint _myInsurable = depositors[_depositorAddress].lockedAmount;
        if( enhancedInsurance == _TRUE ){
    ...
    function totalInsurableAssets() public onlyGuardians override view returns(uint _insurable) {
    ...
    // depositments individually and cross check that they are immediately
    ...
    function _makeNewDeposit(uint _amount, address _referrer) private {
        ...
        _depositor.claimableRewards = _getClaimableRewards(msg.sender);
    ...
    function _getClaimableRewards(address _depositorAddress) private returns (uint) {
        ...
        return _depositor.claimableRewards.add(_newRewards);
    }
    ...
    error DepositBelowMinimum(address caller, uint amount);
    error NonParticipant(address caller);
    error NoRewardsAvailable(address caller);
    error UnauthorizedWithdrawl(address taker, address target);
    ...
    error PayoutUnavailable(address caller);
