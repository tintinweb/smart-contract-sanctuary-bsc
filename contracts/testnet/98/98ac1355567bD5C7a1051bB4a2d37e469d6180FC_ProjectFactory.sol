...
contract ProjectFactory is Ownable {
  ...
  event ProjectCreated(address projectAddress, ProjectInfo data);
  ...
  function createProject(ProjectInfo calldata data) external payable {
    require(msg.value >= serviceFee, "Service Fee is not enough");
    refundExcessiveFee();

    Project project = new Project(msg.sender, data);
    ...
    emit ProjectCreated(projectAddress, data);
  }
...
import "./structs/ProjectInfo.sol";
...
contract Project is Ownable {
  ...
  event ProjectUpdated(ProjectInfo data);
  event ProjectUpdatedByFactoryAdmin(
    ProjectInfo data,
  ...
  constructor(address _owner, ProjectInfo memory _data) {
  ...
  modifier onlyFactoryAdmin() {
    require(
      IProjectFactory(factory).owner() == msg.sender || IProjectFactory(factory).isAdmin(msg.sender),
  ...
  modifier onlyFactoryAdminOrAdmin() {
    require(
      IProjectFactory(factory).owner() == msg.sender ||
        IProjectFactory(factory).isAdmin(msg.sender) ||
        isAdmin[msg.sender],
  ...
  function contribute(uint256 _amount) external payable {
    require(approvalStatus == ApprovalStatus.APPROVED, "Project is not Approved");
    if (address(data.paymentToken) == address(0)) {
      require(msg.value >= _amount, "Insufficient contribution amount");
    } else {
      require(data.paymentToken.balanceOf(msg.sender) >= _amount, "Insufficient contribution amount");
    }
    require(_amount >= data.minContribution, "Amount should be greater than minimum contribution");
    require(block.timestamp >= data.startTimestamp, "You can contribute only after start time");
    require(block.timestamp <= data.endTimestamp, "You can contribute only before end time");

    totalContribution += _amount;

    if (address(data.paymentToken) != address(0)) {
      data.paymentToken.transferFrom(msg.sender, address(this), _amount);
  ...
  function setProjectDescription(string memory _projectDescription) external onlyFactoryAdminOrAdmin {
    ...
    data.projectDescription = _projectDescription;
  ...
  function setRewardDescription(string memory _rewardDescription) external onlyFactoryAdminOrAdmin {
    ...
    data.rewardDescription = _rewardDescription;
  ...
  function setMinContribution(uint256 _minContribution) external onlyFactoryAdminOrAdmin {
    data.minContribution = _minContribution;
  ...
  function setProjectGoals(uint256 _projectGoals) external onlyFactoryAdminOrAdmin {
    ...
    data.projectGoals = _projectGoals;
  ...
  function setRewardDistributionTimestamp(uint256 _rewardDistributionTimestamp) external onlyFactoryAdminOrAdmin {
    data.rewardDistributionTimestamp = _rewardDistributionTimestamp;
  ...
  function setStartTimestamp(uint256 _startTimestamp) external onlyFactoryAdminOrAdmin {
    require(_startTimestamp < data.endTimestamp, "Start timestamp must be before end timestamp");
    data.startTimestamp = _startTimestamp;
  ...
  function setEndTimestamp(uint256 _endTimestamp) external onlyFactoryAdminOrAdmin {
    require(_endTimestamp > data.startTimestamp, "End timestamp must be after start timestamp");
    data.endTimestamp = _endTimestamp;

    emit EndTimestampUpdated(_endTimestamp);
  }

  function configProjectInfo(ProjectInfo calldata _data) external onlyFactoryAdminOrAdmin {
    require(_data.startTimestamp < _data.endTimestamp, "Start timestamp must be before end timestamp");
    require(
      approvalStatus == ApprovalStatus.PENDING || _data.paymentToken == data.paymentToken,
      "You can't change payment token after Approval"
    );

    data = _data;

    emit ProjectUpdated(_data);
  }
  ...
  function withdrawToken(uint256 _amount, address _receiver) private onlyOwner {
    require(data.paymentToken.balanceOf(address(this)) >= _amount, "You cannot withdraw more than you have");

    uint256 withdrawalFee = getWithdrawalFee(_amount, fixFeeAmount, percentageFeeAmount);
    require(data.paymentToken.balanceOf(address(this)) > withdrawalFee, "You cannot withraw less than widrawal fee");

    uint256 receiverAmount = _amount - withdrawalFee;

    data.paymentToken.transfer(_receiver, receiverAmount);
    data.paymentToken.transfer(factory, withdrawalFee);
  }
  ...
  function configProjectInfo(
    ...
    require(
      getWithdrawalFee(data.projectGoals, _fixFeeAmount, _percentageFeeAmount) <= data.projectGoals,
    ...
    emit ProjectUpdatedByFactoryAdmin(_data, _approvalStatus, _percentageFeeAmount, _fixFeeAmount);
  }
  ...
  function setPercentageFeeAmount(uint256 _percentageFeeAmount) external onlyFactoryAdmin {
    require(_percentageFeeAmount <= FEE_DENOMINATOR, "percentageFeeAmount should be less than FEE_DENOMINATOR");
    require(
      getWithdrawalFee(data.projectGoals, fixFeeAmount, _percentageFeeAmount) <= data.projectGoals,
      "Withdrawal fee should not more than project goals"
    );
  ...
  function setFixFeeAmount(uint256 _fixFeeAmount) external onlyFactoryAdmin {
    require(
      getWithdrawalFee(data.projectGoals, _fixFeeAmount, percentageFeeAmount) <= data.projectGoals,
      "Withdrawal fee should not more than project goals"
    );
