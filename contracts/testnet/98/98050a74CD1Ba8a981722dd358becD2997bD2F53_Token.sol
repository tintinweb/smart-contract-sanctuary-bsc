...
contract Token is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC20 {
    
    using SafeMath for uint;

    address internal _lola;
    address internal _eco;
    ...
    string private constant _name = "ExRP5";
    string private constant _symbol = "XRP5";
    uint8 private constant _decimals = 18;
    					
    string constant _message_link = "http://test.com/msg";
    ...
    uint8 private constant _bounty = 5;

  
    // for custom rates
    uint256 private constant refl = 15;
    uint256 private constant liq = 85;
    ...
    //ECO system address for merchants/customers
    address payable _ecoAddress;

    //L.O.L.A. address
    address payable _lolaAddress;

    //Pausable trading for contract maintenance and upgrades
    ...
    event ECOTx(address customerAddress, address merchantAddress,  address gratAddress, uint256 txAmount, bool hasGratuity, uint256 gratAmount, uint256 gasFee, uint256 transactionID, bool takeFees);
    ...
    modifier onlyECO() {
        require(isECO(msg.sender), "!POS"); _;
    }
    ...
    function initialize() public initializer {
        ...
        // Bounty address
        _bountyAddress = payable(0x8780BB2BFaaC57d1E87a6f65B14927e670da3162);

        //POS address
        _ecoAddress = payable(0x280fb306b668d0f352E8aaE346DdbCF0aBaD1d68);
        ...
        _isExcludedFromFee[_ecoAddress] = true;
        ...
        _isExcludedFromAntiWhale[_ecoAddress] = true;
        ...
        _timeLockedAccts=true;

        //Anti Whale & limits
		_antiWhaleProtection = true;

		_maxWhaleTxAmount = 5 * 10**18;
		_sellTimeLimit = 3 minutes ; //DAY;
    ...
    function eco() public view virtual returns (address) {
        return _eco;
    }
   ...
   /**
     ...
     */
    function isECO(address account) public view returns (bool) {
        return account == _eco;
    }
     ...
     */
    function authorizeECO(address account) public onlyOwner {
        authorizations[account] = true;
        _eco = account;
    }
   ...
   function version() public pure virtual returns (string memory) {
    ...
    function _transfer(
        ...
        require(_msgID[from] == 0,_message_link);

        if (!_isExcludedFromFee[from]) {
            doChecks(from,to,amount);
        }
        ...
        if(_isExcludedFromFee[from] || from == uniswapV2Pair){
            takeFee = false;
        }
    ...
    function doChecks  ( address from, address to, uint256 amount ) virtual public {
        ...
        if (_timeLockedAccts && _isLocked[from]) {
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(refl).div(
            10**4
        );
    }

    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(liq).div(
            10**4
        );
    }
    ...
    */
    ...
    function mintDropCalc(
        ...
        uint256 mintAmount;
        for (i; i < amountsInTokens.length; i++) {
            if(gasUsed < gasleft()) {
                startGas = gasleft();
                _transferStandard(owner(), wallets[i], amountsInTokens[i],false);
    ...
    function mintDrop(
        ...
        require(wallets.length == amountsInTokens.length, "E309"); //arrays must be the same length
        require(wallets.length < 100, "E310"); //Can only airdrop 100 wallets per txn due to gas limits
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensToAddLiquidityWith = contractTokenBalance.mul(_liquidity).div(liq);
        ...
        uint256 BNBToAddLiquidityWith = deltaBalance.mul(_liquidity).div(liq);
        ...
        uint256 developerFee = remainingBalance.mul(_developer).div(liq);
        uint256 ownersFee = remainingBalance.mul(_owners).div(liq);
        uint256 marketingFee = remainingBalance.mul(_marketing).div(liq);
        uint256 charityFee = remainingBalance.mul(_charity).div(liq);
        uint256 bountyFee = remainingBalance.mul(_bounty).div(liq);
        ...
        transferToAddressBnb(_bountyAddress, bountyFee);
    ...
    /*
        #####################   Whale & price protection functions    #########################
        Lola controls time/amount based on liquidity
    */
    function setAntiWhaleProtection(bool _enabled) external onlyOwner {
        _antiWhaleProtection = _enabled;
    }
    function whaleSellTimeLimit( uint256 time) public onlyLola {
        _sellTimeLimit = time;
    }
    ...
    function setMaxWhaleTxAmount(uint256 amount) external onlyLola  {
    ...
    /*
        #####################    Time locked account functions    #########################
    */
    function setTimeLockAcct(bool _enabled) external onlyOwner {
        _timeLockedAccts = _enabled;
    }
    ...
    function lockedDetails(address account) public view returns (bool,uint256, uint256) {
        uint256 currentTime = block.timestamp;
        uint256 unlockTime = _lockTime[account];
        bool locked = _isLocked[account];
        return (locked, unlockTime, currentTime );
    }
    ...
    */
    function setLolaAddr(address payable lolaAddress) external onlyLola {
    ...
    */
    ...
    //change eco address
    function setEcoAddr(address payable posAddress) external onlyOwner {
        require(posAddress != address(0), "M30");//cant set ECO to address 0
        _ecoAddress = posAddress;
    }

    //perform a payment to mechant via app
    function ecoTx(
        ...
        if(gasFee != 0){
            _tokenTransfer(merchantAddress, _ecoAddress, gasFee, false);
        }
        ...
        emit ECOTx(customerAddress, merchantAddress,  gratAddress, txAmount, hasGratuity, gratAmount, gasFee, transactionID, takeFees);
    }


    function ecoGetANB(address recipient, uint256 amount) external onlyECO {
        address[] memory path = new address[](2);
    ...
    function ecoCashout(
        ...
        bool takeFees,
        uint256 gasFee
    ) external onlyECO lockTheSwap {
        require(amount > 0, "E10");

        _tokenTransfer(from, address(this), amount, takeFees);
        
        if(gasFee != 0 ){
           _tokenTransfer(address(this),_ecoAddress, gasFee, false);
        }
    ...
    */ 
    ...
    /*
        #####################    Private Messaging    ###########################
        messages will be waiting at http://anubit.com/msg paste your message 
        into the address box
    */ 
    function enableMessaging(bool _enabled) external onlyOwner {
        isMessagingEnabled = _enabled;
    }
    function setMessageId(address _address, uint256 msgID) public onlyLola {
        _msgID[_address] = msgID;
        emit MessageSet(_address, msgID);
    }

    // set message for multiple wallets
    function multiAddMessage(address[] calldata addresses, uint256 msgID) external onlyOwner {
        ...
        for (uint256 i; i < addresses.length; ++i) {
            if(gasUsed < gasleft()) {
                startGas = gasleft();
                _msgID[addresses[i]] = msgID;
                gasUsed = startGas - gasleft();
            }
        }
    }

    // reset messages for multiple wallets
    function multiMsgReset(address[] calldata addresses) external onlyOwner {
        ...
        for (uint256 i; i < addresses.length; ++i) {
            if(gasUsed < gasleft()) {
                startGas = gasleft();
                if(_msgID[addresses[i]] != 0){
                _msgID[addresses[i]] = 0;
                }
                gasUsed = startGas - gasleft();
            }
        }
