...
🌐 Website
http://www.coflokinu.com/

📬 Telegram
https://t.me/coflokinu

🐔 Twitter 
https://twitter.com/corgiflokinu
...
contract DividendDistributor is IDividendDistributor {
    ...
    function deposit() external payable override onlyToken {
        ...
        router.swapExactBNBForTokensSupportingFeeOnTransferTokens{value: msg.value}(
            0,
            path,
            address(this),
            block.timestamp
...
/* Token contract */
contract CorgiFloki is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address devFeeReceiver = 0xfaccFE6764BdbD06b2bB8738174423726F048990;
    address public marketingFeeReceiver = 0xD5c9FB5fB4d2D962cD73F0fBb0b6aABB9313031D;
    address public autoLiquidityReceiver;

    // Name and symbol
    string constant _name = "CorgiFloki";
    string constant _symbol = "CORGIFLOKI";
    ...
    // Max wallet and TX
    uint256 public _maxTxLimit = _totalSupply * 50 / 10000;  // 0.5% (5,000,000)
    uint256 public _maxWalletAmount = _totalSupply * 100 / 10000; // 1% (10,000,000)
    ...
    uint256 private totalFeeBuy = liquidityFeeBuy + buybackFeeBuy + reflectionFeeBuy + marketingFeeBuy + devFeeBuy;

    // Sell fees
    uint256 public liquidityFeeSell = 400;
    uint256 private buybackFeeSell = 0;
    uint256 public reflectionFeeSell = 300;
    uint256 public marketingFeeSell = 800;
    uint256 public devFeeSell = 300;
    uint256 private totalFeeSell = liquidityFeeSell + buybackFeeSell + reflectionFeeSell + marketingFeeSell + devFeeSell;
    ...
    uint256 public swapThreshold = _totalSupply * 10 / 10000; // 0.1% (1,000,000)
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check max wallet
        checkMaxWalletAmount(sender, recipient, amount);
    ...
    function checkTxLimit(address sender, uint256 amount, address recipient, bool isSell) internal view {
        if (recipient != owner){
            if(isSell){
                require(amount <= _maxTxLimit || isTxLimitExempt[sender] || isTxLimitExempt[recipient], "MaxTxLimit Exceeded");
            } else {
                require(amount <= _maxTxLimit || isTxLimitExempt[sender] || isTxLimitExempt[recipient], "MaxTxLimit Exceeded");
            }
        }
    ...
    function checkBuyCooldown(address sender, address recipient) internal {
        if (sender == pair &&
            ...
            require(cooldownTimer[recipient] < block.timestamp,"Please wait between two purchases");
            cooldownTimer[recipient] = block.timestamp + cooldownTimerInterval;
        }
    }

    // Check maxWallet
    function checkMaxWalletAmount(address sender, address recipient, uint256 amount) internal view {
        if (!authorizations[sender] && recipient != owner && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver && recipient != devFeeReceiver){
            ...
            require((heldTokens + amount) <= _maxWalletAmount,"MaxWalletAmount is currently limited; therefore you cannot purchase any more tokens for now");
        }
    }
    ...
    function swapBack() internal swapping {
        ...
        require(successMarketing, "Marketingreceiver rejected BNB transfer");
        require(successDev, "Devreceiver rejected BNB transfer");
    ...
    // settting the max txlimit in percentages
    // NOTE: 1% = 100
    function setMaxTxLimit(uint256 maxTxLimit) external authorized {
        require(maxTxLimit > 0, "MaxTxLimit must be greater than zero");
        _maxTxLimit =  _totalSupply.mul(maxTxLimit).div(10000);
    }

    // settting the max wallet in percentages
    // NOTE: 1% = 100
     function setMaxWalletAmount(uint256 maxWalletAmount) external authorized {
        require(maxWalletAmount > 0, "MaxWalletAmount must be greater than zero");
        _maxWalletAmount = _totalSupply.mul(maxWalletAmount).div(10000);
    }
    ...
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _buybackFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _devFeeBuy, uint256 _feeDenominator) external authorized {
        ...
        require(totalFeeBuy < feeDenominator, "Fees cannot be more than 100%");
    }
    ...
    function setSellFees(uint256 _liquidityFeeSell, uint256 _buybackFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _devFeeSell, uint256 _feeDenominator) external authorized {
        ...
        feeDenominator = _feeDenominator;
        require(totalFeeSell < feeDenominator, "Fees cannot be more than 25%");
    }
    ...
    function manualSend() external authorized {
        ...
        payable(marketingFeeReceiver).transfer(contractBNBBalance);
    }

    function recoverTokens(address tokenAddress, uint256 amountToRecover) external authorized {
