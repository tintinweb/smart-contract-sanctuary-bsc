...
contract Upgradeable is Ownable, ReentrancyGuard, IERC721Receiver{
    ...
    event DepositNFTEvent(address sender, address nftAddr, uint256 tokenId, uint256 tokenType, uint256 tokenAmount, bytes fractorId, bytes internalTxId, uint256 index);
    ...
    event MintNFTEvent(uint256 nftId, uint256 chainId, uint256 [] tokenIds, uint256 [] nftTypes, uint256 [] tokenAmounts, bytes internalTxId);
    event MintFNFTEvent(uint256 []tokenIds, uint256 amount, address fracTokenAddr, bytes fractorId, string name, string symbol, bytes internalTxId);
