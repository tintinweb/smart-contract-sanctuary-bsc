...
contract HTYDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("HTY_Dividend_Tracker", "HTY_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "HTY_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "HTY_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main HTY contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "HTY_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "HTY_Dividend_Tracker: Cannot update claimWait to same value");
...
contract HTY is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    HTYDividendTracker public dividendTracker;

    uint256 public maxSellTransactionAmount = 10**9 * (10**18);
    uint256 public maxBuyTransactionAmount = 10**8 * (10**18);
    ...
    function setSellFee(uint256 _onSellbnbRewardFee, uint256 _onSellliuidityFee, uint256 _onSellMarketingFee, uint256 _onSellDevFee) public onlyOwner {
        ...
        onSelltotalFees = onSellBNBRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee).add(onSellDevFee);
    }
    ...
    function setdevelopmentWallet(address payable _newBuybakWallet) public onlyOwner {
        developmentWallet = _newBuybakWallet;
    }

    constructor() ERC20("Humanity Resistance", "HTY") {
        BNBRewardsFee = 3;
        liquidityFee = 3;
        marketingFee = 2;
        devFee = 2;

        onSellBNBRewardsFee = 3;
        onSellliquidityFee = 4;
        onSellmarketingFee = 2;
        onSellDevFee = 2;

        marketingWallet = payable(0x99eE90244ea5aAE8aC8AD9f2D4ea3C84eA0247d1);
        developmentWallet = payable(0x5279DB18ae43E59d4414E5E7F6aD0db400299733);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(devFee); // total fee transfer and buy

        onSelltotalFees = onSellBNBRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee).add(onSellDevFee);

    	dividendTracker = new HTYDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "HTY: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "HTY: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "HTY: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "HTY: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "HTY: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "HTY: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        developmentWallet.transfer(buybackAmount);
