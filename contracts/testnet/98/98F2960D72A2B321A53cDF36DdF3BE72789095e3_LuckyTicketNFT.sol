    ...
    VRFConsumerBase
{
    ...
    function getOwnerCrownNFT() external view returns (uint256) {
        return ownerWinningCrownNFT[msg.sender].length;
    }
    ...
    /**
     @param nftType: 0: Crown, 1 Scepter
     */
    function claimNFT(uint8 nftType) external {
        if (nftType == 0) {
            require(
                ownerWinningCrownNFT[msg.sender].length > 0,
                "Not enough quantity"
            );
            for (
                uint256 i = 0;
                i < ownerWinningCrownNFT[msg.sender].length;
                i++
            ) {
                if (ownerWinningCrownNFT[msg.sender][i] != 0) {
                    _CrownToken.transferFrom(
                        address(this),
                        msg.sender,
                        ownerWinningCrownNFT[msg.sender][i]
                    );
                }
            }
            ownerWinningCrownNFT[msg.sender] = new uint256[](0);
        } else {
    ...
    function burnCrown() external onlyValidTarget {
        ...
        emit TicketNFTWin(address(this), todayTokenId);
    }
