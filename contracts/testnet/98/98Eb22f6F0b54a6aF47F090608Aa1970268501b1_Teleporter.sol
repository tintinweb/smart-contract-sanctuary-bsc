...
contract Teleporter is OwnedAndParsed{

    Archives public archive;

    constructor(address Owner, address Parser, Archives arch) OwnedAndParsed(Owner, Parser){
        archive = arch;
    }
    ...
    mapping(uint => record) teleportRecords;
    function getRecord(uint innerId) public view returns (record memory){
        return teleportRecords[innerId];
    }

    IERC20 public feeToken;
    function changeToken(IERC20 token) public onlyOwner returns (bool){
    	feeToken = token;
    	return true;
    }
    mapping(uint => uint) fees;
    function changeFee(uint chain, uint price) public onlyOwnerOrParser returns (bool){
    	fees[chain] = price;
    	return true;
    }
    function getFee(uint chain) public view returns (uint){
        return fees[chain];
    }


    function teleport(uint chainId, uint internalId, uint futureChain, address futureWallet) public currentOwner(chainId, internalId)  returns (bool){

    	require(fees[futureChain] != 0, "Chain is not available");
    	require(archive.getCurrentStatus(chainId, internalId) == 2, "Please move it to archive");

    	record storage ptr = teleportRecords[lastOne];
    	ptr.chainId = chainId;
    	ptr.internalId = internalId;
    	ptr.futureChain = futureChain;
    	ptr.futureWallet = futureWallet;
    	...
    	// pay fee
    	toArch[1].a = fees[futureChain];
        toArch[1].b = block.chainid;
        toArch[1].z = msg.sender;
        toArch[1].x = archive.getOwner();
        toArch[1].token = feeToken;
    ...
    function report(uint innerId) public onlyOwnerOrParser returns (bool){
        teleportRecords[innerId].status = true;
        return true;
    }

    function release(uint chainId, uint internalId, address newOwner) public onlyOwnerOrParser returns (bool){

    	require(archive.getCurrentStatus(chainId, internalId) == archive.showActionStatus(address(this)));
