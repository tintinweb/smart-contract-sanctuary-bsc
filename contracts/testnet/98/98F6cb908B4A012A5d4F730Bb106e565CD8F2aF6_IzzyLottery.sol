...
contract IzzyLottery is ILottery, ReentrancyGuard, Ownable {
    ...
    uint256 internal constant MAX_LENGTH_LOTTERY = 4 days + 5 minutes; // 2 hours
    ...
    address public affiliateReceiverAddress;
        ...
        nonReentrant
    {
        ...
        if (affiliateReward > 0) {
            _sendAffiliateReward(_lotteryId, msg.sender, affiliateReward);
        }

        emit TicketsClaim(msg.sender, rewardToTransfer, _lotteryId, _ticketIds.length);
    }

    function _sendAffiliateReward(
        uint256 _lotteryId,
        address winner,
        uint256 amount
    ) internal {
        if (lotteries[_lotteryId].affiliatePrize == 0) {
            return;
        }

        (address receiver, ) = referralProgram.getReferralAccount(winner);

        if (receiver == address(0)) {
            require(affiliateReceiverAddress != address(0));
            receiver = affiliateReceiverAddress;
        }
     ...
     * @param _affiliateAddress: address of the affiliate receiver
     */
    function setAdminAddresses(
        ...
        affiliateReceiverAddress = _affiliateAddress;
    }
