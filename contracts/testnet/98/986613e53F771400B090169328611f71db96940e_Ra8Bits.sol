...
contract Ra8Bits is Context, Ownable, ERC1155Holder {
    ...
    uint256 public amountForCarrotTier = 100;
    ...
    event setGen0NFTAddressEvent(
        address callerAddress,
        address Gen0NFTAddress
    );
    ...
    function superLikeNFT(
        ...
        require(receivedAddress == authAddress, "Invalid signature");
         require(
            (IERC1155(formNftAddress).balanceOf(_msgSender(), toNftId) > 0),
    ...
    function acceptSuperLike(
        ...
        require(
            IERC20(ratBitTokens).transfer(
                _msgSender(),
                isSuperliked[superLikeNFTId].div(50)
            )
        );
        require(
            IERC20(ratBitTokens).transfer(
                tokenBurnAddress,
                isSuperliked[superLikeNFTId].div(25)
            )
        );
        require(
            IERC20(ratBitTokens).transfer(
    ...
    function rejectSuperLike(
        ...
        require(
            IERC20(ratBitTokens).transfer(fromOwnerAddress, costForSuperLike)
        );
        emit SuperLikeResponse(
            fromNftId,
            fromOwnerAddress,
    ...
    function isBreedingStart(uint256 _breedId,address userAddress) public view returns (bool) {
        if (
            breedInfoList[_breedId].ownerOfParentA == userAddress ||
            breedInfoList[_breedId].ownerOfParentB == userAddress
        ) return true;
    ...
    function withdrawalNft(uint256 _breedId) public {
        require(checkValidBid(_breedId,_msgSender()), "Invalid id");
        require(!isBreedingStart(_breedId,_msgSender()), "Breeding started cant withrawal NFT");
    ...
    function addTokenListForBreeding(uint256 _tokenAmount) external onlyOwner {
        ...
        emit addTokenListForBreedingEvent(breedingTokenListId,_tokenAmount);
    }
    ...
    function setGen0NFTAddress(address _address) external onlyOwner {
        ...
        emit setGen0NFTAddressEvent( _msgSender(),_address );
    }
    ...
    function setNftAddress(address _address) external onlyOwner {
        require(_address != address(0));
        nftAddress = _address;
        emit setNftAddressEvent( _msgSender(),_address );
    }
}
