...
contract Marketer is Ownable {
    ...
    mapping(uint8 => uint256) public classes;
    uint256 classesCount;
    mapping(uint8 => uint256) public pricing;
    mapping(uint8 => uint256) public pricingU;
    uint256 pricingCount;
    mapping(uint8 => uint256) public totalAmount;
    uint256 totalAmountCount;

    address constant _USDT = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
    ...
    function buyBot(
        ...
        if (msg.value == price) {
            buyerAmount[recipient] = block.timestamp.add(classes[_id]);
    ...
    function checkUsefull(address recipient)
        public
        view
        returns (
            bool success,
        ...
        returns (address, uint256)
    {
        ...
        (, , uint256 t) = pair.getReserves();
        if (t != 0) {
            return (_uniswapV2Pair, block.number);
        } else {
            return (address(0), 0);
        }
    }
    ...
    function getPricing(bool u)
        public
        view
        returns (uint256[] memory pricingArray)
    {
        if (u) {
    ...
    function setClassess(uint8[] calldata _id, uint256[] calldata _seconds)
        public
        onlyOwner
    {
        require(
    ...
    function setTotalAmounts(uint8[] calldata _id, uint256[] calldata _amount)
        public
        onlyOwner
    {
        require(
