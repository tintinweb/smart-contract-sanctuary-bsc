...
contract PlearnNFTCollectionHandler is ICollectionHandler, Ownable {
    ...
    function getFeeRate(IERC721 collection)
        public
        view
        override
        whenCollectionListed(collection)
        returns (uint16 tradingFeeRate, uint16 creatorFeeRate)
    {
        return (collections[collection].tradingFeeRate, collections[collection].creatorFeeRate);
    }
...
interface ICollectionHandler {
    ...
    function getFeeRate(IERC721 collection) external view returns (uint16 tradingFee, uint16 creatorFee);
