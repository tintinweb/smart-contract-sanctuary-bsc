...
contract NDBPoolFactory is Ownable {
    event NewNDBPoolContract(address indexed ndbPool);

    constructor() public {}

    /*
     * @notice Deploy the pool (NDB Pool, Enterprise Pool , Consumption Pool, Electoral Pool)
     * @return address of new NDB Pool contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(NDBPool).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(_stakedToken, _rewardToken, _startBlock));
        address ndbPoolAddress;

        assembly {
            ndbPoolAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        NDBPool(ndbPoolAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewNDBPoolContract(ndbPoolAddress);
    }
}
...
contract NDBPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of the NDBPool factory
    address public NDB_POOL_FACTORY;
    ...
    uint256 public constant MAXIMUM_WITHDRAWAL_INTERVAL = 7 days;
    ...
    function initialize(
        ...
        require(msg.sender == NDB_POOL_FACTORY, "Not factory");
