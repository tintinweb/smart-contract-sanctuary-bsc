...
contract CLG is ERC165, ReentrancyGuard, Context, IERC721TokenReceiver {
    ...
    function getExchangeRates(address payment_token) public view returns (ExchangeRate memory) {
        ...
        for (i=0; i<payment_tokens.length; i++) {
            ...
            pairAddress = IUniswapV2Factory(swapFactory).getPair(payment_token, address(payment_tokens[i]));
    ...
    function sell(uint256 tokenId, uint256 priceValue, address pricePaymentToken, address payable wallet, bytes memory ceekUserIdParam, address[] memory paymentTokenNft, string[] memory _allowedToBuyCeekUserId, address[] memory _allowedToBuyWallet) public {	
    ...
    // Buy option
    function buy(uint256 tokenId, uint256 priceValue, address pricePaymentToken, bytes memory ceekUserIdParam, EIP20 payment_token) public payable nonReentrant {        
       ...
       // uint256 amount4owner = token_price.sub(amount4admin);
        ...
        emit Sale(tokenId, _wallets[tokenId], address(payment_token), sellBidPrice[tokenId], saleBidPriceCurrency, token_price.sub(amount4admin));
        emit Commission(tokenId, admin, address(payment_token), sellBidPrice[tokenId], saleBidPriceCurrency, commissionRate, amount4admin);
