...
library RandomHelper {
        ...
        returns (address resourceAddress)
    {
        if (_randomNumber >= 4400 && _randomNumber <= 7519) {
            return 0x56aa289Ad03Ee8960EC7c38aE30736816e8e4748;
        }
        if (_randomNumber > 7519 && _randomNumber <= 10639) {
            return 0x74a2b206FFFb3Ed5f6b7C55225C7F227BB063fb5;
        }
        if (_randomNumber > 10639 && _randomNumber <= 13759) {
            return 0x741248d2ddac239003c8fE575A91c2F4940ecB10;
        }
        if (_randomNumber > 13759 && _randomNumber <= 16099) {
            return 0xFabAEdA673050420487B99d1919231eA3705eD76;
        }
        if (_randomNumber > 16099 && _randomNumber <= 18439) {
            return 0x0B6D9fD43325AEF12f7DC8994312527925Fcdf82;
        }
        if (_randomNumber > 18439) {
            return 0xdc61e9F838489AAD3CC2d012e9dbCeD1e967e7Cb;
        }
    }
    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    event BuyMisteryBox(uint256 timestamp);
    ...
    function buyMisteryBox() external whenNotPaused {
        ...
        supply = supply - 1;
        emit BuyMisteryBox(block.timestamp);
    }
    ...
    function _generateObject(address _to, uint16 _random) internal {
        ...
        if (itemType == 0) {
            ...
            SharedStructs.WeaponTierConfig
                memory weaponConfig = weaponTierConfigMap[_range - 1];
    ...
    function getWeaponConfig(uint8 _tier)
        external
        view
        returns (SharedStructs.WeaponTierConfig memory)
    {
        return weaponTierConfigMap[_tier];
    }

    function getArmorConfig()
        external
        view
        returns (SharedStructs.ArmorTierConfig memory)
    {
        return armorTierConfigMap[0];
    }
}
