    ...
    ReentrancyGuard
{
    ...
    // Fish token id minted 
    mapping(uint256 => bool) public mintedFishTokens;

    /**
     * @notice config external SC address for this SC
     * @dev only call by owner
     * @param _fishAddress: address of NFT721 in fish game
        ...
        onlyOwner
    {
        ...
        require(address(_fishAddress) != address(0), "Invalid address");
        ...
        isInitialized = true;
        fishContract = IFISH721(_fishAddress);
    ...
    /**
     * @notice set new fish address
     * @dev only call by owner
     * @param _fishAddress: address of NFT721 in fish game
     */
    function setNewFishAddress(address _fishAddress) public onlyOwner {
        fishContract = IFISH721(_fishAddress);
    }
    ...
    /**
     * @notice set fee for minting NFT
     * @dev only call by owner
    ...
    function mintNFT(uint256 _boxNumber) external payable nonReentrant {
        ...
        if (fee > 0) {
            payable(fishTreasuryWallet).transfer(
                bnbToTransfer - (bnbToTransfer * (100 - fee)).div(100)
            );
        }

        uint256 _var = (bnbToTransfer * (100 - fee)).div(100);
        payable(rewardSC).transfer(_var);
    ...
    function _mint(address _receipient, uint256 tokenId) internal {
        fishContract.mintBox(_receipient, tokenId);
        mintedFishTokens[tokenId] = true;
    }
...
interface IFISH721 is IERC721Upgradeable {
