...
import "./OpenZepplin/token/ERC721/ERC721Holder.sol";
import "./OpenZepplin/token/ERC721/IERC721.sol";
...
contract MarketplaceHero is Pausable, Ownable, ERC721Holder, IBidNFT {
    ...
    event ReadyBidToken(uint256 tokenId, uint256 startPrice, uint64 startTime, uint64 endTime);
    event Bid(address indexed bidder, uint256 indexed tokenId, uint256 price);
    event UpdateBidStartPrice(uint256 tokenId, uint256 oldPrice, uint256 newPrice);
    ...
    event BuyToken(address buyer, address seller, uint256 tokenId, uint256 price, uint256 time);
    event UpdatePriceSellToken(uint256 tokenId, uint256 oldPrice, uint256 newPrice);
    ...
    function getTokenNFTDetail(uint256 _tokenId) public view returns (TokenNFTEntry memory) {
        return _tokenNFTEntry[_tokenId];
    }

    function getTokenSellDetail(uint256 _tokenId) public view returns(TokenSellEntry memory) {
    ...
    function isFinisher (uint256 _tokenId, address _address) public view returns (bool) {
        if (_tokenBids[_tokenId].length != 0) {
            return _tokenBidder[_tokenId] == _address || _tokenBids[_tokenId][_tokenBids[_tokenId].length - 1].bidder == _address;
        } else {
            return _tokenBidder[_tokenId] == _address;
        }
    }

    function config(address _quoteErcAddress, address _feeAddr, uint256 _fee) external onlyOwner {
    ...
    function readyToSellToken(uint256 _tokenId, uint256 _price) public override whenNotPaused {
        ...
        _tokeSellEntry[_tokenId] = TokenSellEntry({ tokenId: _tokenId, price: _price });
    ...
    function updateSellToken(uint256 _tokenId, uint256 _price) public override whenNotPaused {
        address _seller = _tokenSeller[_tokenId];
        require(_msgSender() == _seller, "Only the seller can change the price");
        require(_price != 0, "Price must be greater than zero");
    ...
    function buyToken(uint256 _tokenId) public override whenNotPaused {
        ...
        quoteErc20.safeTransferFrom(_msgSender(), _seller, _price.sub(feeAmount));
        if (feeAmount != 0) {
    ...
    function cancelToSellToken(uint256 _tokenId) public override whenNotPaused {
        require(_tokenIsSell[_tokenId] == true, "Token not in sell");
        address _seller = _tokenSeller[_tokenId];
        require(_msgSender() == _seller, "Only the seller can cancel sell token");
        nft.safeTransferFrom(address(this), _msgSender(), _tokenId);
        _deleteSell(_tokenId);
        emit CancelSellToken(_tokenId);
    }

    function readyBidToken(uint256 _tokenId, uint256 _startPrice, uint64 _startTime, uint64 _endTime) public override whenNotPaused {
        require(_msgSender() == nft.ownerOf(_tokenId), "Only Token Owner can sell token");
        ...
        require(_endTime > _startTime, "Auction: end time must be older than start time");
        nft.safeTransferFrom(address(_msgSender()), address(this), _tokenId);
        _tokenNFTEntry[_tokenId] = TokenNFTEntry({ tokenId: _tokenId, startPrice: _startPrice, startTime: _startTime, endTime: _endTime });
    ...
    function bidToken(uint256 _tokenId, uint256 _price) public override whenNotPaused {
        require(block.timestamp >= _tokenNFTEntry[_tokenId].startTime && block.timestamp <= _tokenNFTEntry[_tokenId].endTime, "Auction: outside auction hours");
        require(_msgSender() != address(0) && _msgSender() != address(this), "Wrong msg sender");
        require(_price != 0, "Price must be granter than zero");
        require(_price > _tokenNFTEntry[_tokenId].startPrice, "The price must be higher than the starting price");
        if (_tokenBids[_tokenId].length > 0) {
            require(_price > _tokenBids[_tokenId][_tokenBids[_tokenId].length - 1].price, "Bid price must be greater than current bidder");
        }
        ...
        if (_tokenBids[_tokenId].length > 0) {
            quoteErc20.safeTransfer(_tokenBids[_tokenId][_tokenBids[_tokenId].length - 1].bidder, _tokenBids[_tokenId][_tokenBids[_tokenId].length - 1].price);
        }
        _tokenBids[_tokenId].push(BidEntry({bidder: _to, price: _price}));
        emit Bid(_msgSender(), _tokenId, _price);
    }

    function updateBidStartPrice(uint256 _tokenId, uint256 _price) public override whenNotPaused {
        address _seller = _tokenBidder[_tokenId];
        require(_msgSender() == _seller, "Only the seller can change the starting price");
        require(block.timestamp < _tokenNFTEntry[_tokenId].startTime, "Auction has started or ended");
        require(_price != 0, "Price must be granter than zero");
        require(_msgSender() != address(0) && _msgSender() != address(this), "Wrong msg sender");
    ...
    function cancelBidToken(uint256 _tokenId) public override whenNotPaused {
        address _seller = _tokenBidder[_tokenId];
        require(_msgSender() == _seller, "Only the seller can cancel sell token");
        require(block.timestamp < _tokenNFTEntry[_tokenId].endTime, "Auction has ended");
        require(_msgSender() != address(0) && _msgSender() != address(this), "Wrong msg sender");
        if (_tokenBids[_tokenId].length > 0) {
            quoteErc20.safeTransfer(_tokenBids[_tokenId][_tokenBids[_tokenId].length - 1].bidder, _tokenBids[_tokenId][_tokenBids[_tokenId].length - 1].price);
        }
    ...
    function finalized (uint256 _tokenId) public override whenNotPaused {
        require(block.timestamp > _tokenNFTEntry[_tokenId].endTime, "Auction is not over yet");
        require(_msgSender() != address(0) && _msgSender() != address(this), "Wrong msg sender");
        address _seller = _tokenBidder[_tokenId];
        require(_msgSender() == _tokenBids[_tokenId][_tokenBids[_tokenId].length - 1].bidder || _msgSender() == _seller, "Bidder does not exist");
        if (_tokenBids[_tokenId].length > 0) {
            address _to = _tokenBids[_tokenId][_tokenBids[_tokenId].length - 1].bidder;
            uint256 _price = _tokenBids[_tokenId][_tokenBids[_tokenId].length - 1].price;
