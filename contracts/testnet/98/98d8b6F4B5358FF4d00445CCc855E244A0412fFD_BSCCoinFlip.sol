...
contract BSCCoinFlip is VRFConsumerBaseV2, Ownable {
    ...
    address public _devFeeReceiver = address(0x2c3DE508c770a44F2902259f1800aA798f25ee06);
    uint8 public _devFeePercentage = 5; // In 0.1% increments

    mapping (address => bool) _team;

    // Game Details
    mapping (uint256 => Game) public _games; // Game ID -> Game
    mapping (uint256 => uint256) public _responses; // Queue index -> VRF Request ID
    mapping (uint256 => Flips) public _vrfResponses; // VRF Request ID -> Numbers returned
    Queue public _queuedGames;
    Queue public _queuedResponses;
    bool _gameEnabled = true; // If we want to pause the flip game
    uint32 public _queueResetSize = 20; // How many games we want to queue before requesting a random number for the flip
    uint256 public _globalQueueSize;
    uint256 public _responseQueueSize;
    uint256 public _lastUpdateTime;
    uint256 public _updateInterval = 30 seconds;
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        for(uint256 i = 0; i < randomWords.length; i++) {
            flips |= (randomWords[i] % 2) << i; // Place this game `i` digits from the right
        }

        _vrfResponses[requestId].numFlips = uint8(randomWords.length);
        _vrfResponses[requestId].flips = uint56(flips);
        _responses[_queuedResponses.end++] = requestId;
        _responseQueueSize++;
        emit RNGReturned(requestId, randomWords.length, uint56(flips));
    }

    function updateGames() public {
        if (_responseQueueSize > 0) {
            finishGames();
        }

        if (_globalQueueSize > 0 &&
            (_globalQueueSize >= _queueResetSize || block.timestamp.sub(_lastUpdateTime) >= _updateInterval)) {
            callChainlinkVrf();
        }
    }

    function finishGames() internal {
        uint256 requestId = _responses[_queuedResponses.start++];
        _responseQueueSize--;
        Flips storage vrfFlips = _vrfResponses[requestId];
        uint256 vrfQueueStart = vrfFlips.queueStart;

        for (uint256 i = 0; i < vrfFlips.numFlips; i++) {
            Game storage game = _games[vrfQueueStart++];
            if (game.finished) continue; // Chainlink VRF resiliency; don't allow someone to get paid who requested a refund.
            game.winner = (vrfFlips.flips >> i & 1) == game.predictedOutcome;
            ...
            game.finished = true;
            emit GameFinished(game.better, game.id, game.winner);
        }
    }

    function callChainlinkVrf() internal {
        uint256 numGamesToFulfill = _globalQueueSize > _queueResetSize ? _queueResetSize : _globalQueueSize;
        ...
        emit RNGRequested(requestId, numGamesToFulfill, _queuedGames.start);

        _vrfResponses[requestId] = Flips({ flips: 0, numFlips: 0, queueStart: _queuedGames.start });
        _queuedGames.start += uint32(numGamesToFulfill);
        _globalQueueSize -= numGamesToFulfill;
    }
    ...
    function enterGame(uint256 wager, uint8 outcome, address token) external {
        ...
        emit GameStarted(_msgSender(), _queuedGames.end);
        _games[_queuedGames.end] = Game({better: _msgSender(), token: token, id: _queuedGames.end++, predictedOutcome: outcome, finished: false, winner: false, wager: wager});
        _globalQueueSize++;
    ...
    function setQueueSize(uint32 newSize) external onlyTeam {
        require(newSize < 56, "Queue size cannot be larger than 56");
        require(newSize != _queueResetSize, "This is already the queue size");

        _queueResetSize = newSize;
    }

    function setUpdateInterval(uint32 numSeconds) external onlyTeam {
        uint256 newInterval = numSeconds * 1 seconds;
    ...
    function requestRefund(uint32 gameId) external {
        Game storage game = _games[gameId];
        require(!game.finished, "Can't request a refund for a game that's finished");
        require(_msgSender() == game.better, "Can't request a refund for a game you didn't play");

        game.finished = true; // Don't let the user get paid out if for some reason VRF does return after this refund.
        IERC20 gameToken = IERC20(game.token);
        gameToken.transfer(_msgSender(), game.wager);

        updateGames(); 
        emit RefundedGame(_msgSender(), gameId, game.token, game.wager);
    }
