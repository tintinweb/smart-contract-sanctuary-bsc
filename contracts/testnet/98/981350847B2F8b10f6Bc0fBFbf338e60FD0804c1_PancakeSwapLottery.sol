...
contract PancakeSwapLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    mapping(uint256 => uint32[]) private _ticketsSold;

    // Keep track of ticket ids for a given lotteryId
    mapping(uint256 => uint256[]) private _ticketsIds;
        ...
        nonReentrant
    {
        ...
        uint256 referralFees = (_lotteries[_lotteryId].ticketsSold.mul(_lotteries[_lotteryId].priceTicketInCake))
            .mul(_lotteries[_lotteryId].referralReward)
            .div(1e4);
        amountToWithdrawToTreasury = _lotteries[_lotteryId].amountCollectedInCake - _totalPrizeAmount - referralFees;
