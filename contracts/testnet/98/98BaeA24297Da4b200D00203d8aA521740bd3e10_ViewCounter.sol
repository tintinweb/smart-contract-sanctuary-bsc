...
contract ViewCounter{
    ...
    function word_matched(string[] memory words, uint num_types, uint[] memory arr_codes) public returns (bool){
        ...
        uint[] memory code_types = new uint[](num_types);
        for(i=0; i<num_types; i++){
            code_types[i] = i;
        }
        uint[] memory cds = new uint[](num_types);
        uint[] memory ids = new uint[](num_types);
        ...
        for(i=0; i<words.length; i++){
            if(word_id[words[i]] == 0){
                ...
                for(j=0; j<num_types; j++){
                    ids[j] = last_ind;
                    cds[j] = arr_codes[i*num_types+j];
                }
                setCodes(code_types, cds, ids);
