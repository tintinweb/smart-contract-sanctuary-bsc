...
contract BUSD_STAKING is Ownable {
    ...
    // uint256 private LOCKING_PERIOD=180 days;
    mapping(uint8=>uint256) private locking_period;
    uint256 private TIME_STEP=60;
    ...
    event Stake(address user, uint256 amount,uint8 planid ,uint256 _lockduration);
    ...
    constructor(IERC20 _busd) {
        BUSD = _busd;
        referShare = 10;
        // locking_period[1]=160 days;
        // locking_period[2]=356 days;
    ...
    function stake(uint256 _amount, address _refrral,uint8 _lockId) external {
        ...
        require(_lockId==1 || _lockId==2,"Staking :: Invalid Locking period");
        require(BUSD.balanceOf(_msgSender())>= _amount,"Staking:: user has low balance!");
        require(BUSD.allowance(_msgSender(),address(this))>= _amount,"Staking:: allowance exceed!");
        ...
        if(referral!=address(0)){
            users[referral].referralIncome = users[referral].referralIncome.add(_amount.mul(referShare).div(PERCENT_DIVIDER));
            BUSD.transfer(referral,_amount.mul(referShare).div(PERCENT_DIVIDER));
            emit ReferralIncome(user ,referral,_amount.mul(referShare).div(PERCENT_DIVIDER));  
        }
      
        uint8 planid = getPlanIdByAmount(_amount);
        require(planid!=0,"Invalid Amount");
        users[user].roi.push(Roi(planid,_amount,plans[planid].monthlyShare,false,locking_period[_lockId],block.timestamp));
        ttlStaked+=_amount;
        emit Stake( user,  _amount, planid ,locking_period[_lockId]);
    }
  ...
  function getAvilableForHarvest(address user) public view returns (uint256 amount) {
      for(uint256 i = 0; i<users[user].roi.length; i++) {
          ...
          if(_roi.timestamp.add(_roi.locking_time)<block.timestamp && _roi.isUnstake!=true){
  ...
  function harvest () external payable {
    ...
    BUSD.transfer(msg.sender , total);
    ttlUnstaked += total;
  ...
  function getUserDividends(address user) public view returns (uint256) {
      ...
      Roi memory _roi = _user.roi[i];
			// uint256 finish = _roi.timestamp.add(365 days);
			uint256 finish = _roi.timestamp.add(720);
