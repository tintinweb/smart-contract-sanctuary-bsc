...
contract RandomTierNFT is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event TokenMint(address indexed to, uint indexed tokenId, uint rank);

    function initialize(
        string memory baseURI
    ) public initializer {
        __ERC721_init("Random Tier NFT", "RandomTierNFT");
    ...
    function mint(address to, uint256 rank) public onlyRole(TOKEN_MINTER_ROLE) nonReentrant {
        require(to != address(0), "Address can not be zero");
        _lastTokenId += 1;
        uint tokenId = _lastTokenId;
        _tokens[tokenId].createTimestamp = block.timestamp;
        _tokens[tokenId].rank = rank; //start from 1 level
        _safeMint(to, tokenId);
    }
    ...
    function _safeMint(address to, uint256 tokenId) internal override {
        ...
        emit TokenMint(to, tokenId, _tokens[tokenId].rank);
    }
