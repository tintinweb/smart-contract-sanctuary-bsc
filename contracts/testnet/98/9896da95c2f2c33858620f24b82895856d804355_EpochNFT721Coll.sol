...

{
    ...
    EpochNFT721 public EpochNFT721M;


     constructor(uint256 id , address _whitelistAdmin, address originalContract,string memory type1,address  address1,string memory type2,address address2,string memory contractname,string memory contractSymbol) 
    ...
    {   
        ...
        EpochNFT721 _EpochNFT721M = EpochNFT721(originalContract);
        // set the rest of the contract variables
        EpochNFT721M = _EpochNFT721M;
    ...
    function getServiceFeeESC() public view returns (uint256) {
        return  EpochNFT721M.getServiceFeeESC();
    }

    function getServiceFee() public view returns (uint256) {
        return  EpochNFT721M.getServiceFee();
    }
    ...
    function newtokenasbid(
        ...
        uint256 serviceVal = EpochNFT721M.getServiceFee();
        if(keccak256(bytes(tokenAss)) == keccak256(bytes("ESC")))
        {
             serviceVal = EpochNFT721M.getServiceFeeESC();
        }

        address payable admin =  payable(EpochNFT721M.getOwner());
