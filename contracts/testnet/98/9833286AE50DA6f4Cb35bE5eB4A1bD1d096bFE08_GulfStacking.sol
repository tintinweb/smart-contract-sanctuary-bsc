...
contract GulfStacking is ReentrancyGuard, Ownable{
    ...
    function calculateRewards(address wallet, uint256 pendingMonth)internal view returns(uint256){
        ...
        //uint256 percentage  = ((rewardPer * 1000) / amount ); 
        ...
        console.log("stackingPlan =======>>", stackingPlan);
        console.log("rewardPer =======>>", rewardPer);
        console.log("percentage =======>>", percentage);
        console.log("totalRewards =======>>", totalRewards);
        console.log("convertIntoGewi(totalRewards) =======>>", convertIntoGewi(totalRewards));
    ...
    function calculatePercentageAmount(address wallet, uint256 _percentage) internal view returns(uint256){
        uint256 amount = convertIntoOrignal(stackingUserDeatil[wallet].amountStaked); 
        //uint256 percentage  = (_percentage * 100) / amount ;
        ...
        console.log("amount==========>>>", amount);
        console.log("percentage==========>>>", percentage);
        console.log("totalPanality==========>>>", totalPanality);
        console.log("convertIntoGewi(totalPanality) ====>", convertIntoGewi(totalPanality));
    ...
    function renewPackage(uint32 newPackage) public {
        require(stackingStatus == true , "Stacking is off please connect to admin!");
        require(stackingUserDeatil[msg.sender].status = true, "You can not renew your package");
        console.log("stackingUserDeatil[msg.sender].amountStaked =======>>>>", stackingUserDeatil[msg.sender].amountStaked );
        console.log("(minimumStackAmount + stackingPrice) ========>>>>", (minimumStackAmount + stackingPrice) );
        ...
        require(newPackage == 1 || newPackage == 2 || newPackage == 3, "Stacking package is not valid!");
        require( (stackingUserDeatil[msg.sender].amountStaked - stackingPrice) >= minimumStackAmount, "You can not renew your package because after subtracting the stacking charges your remaining amount are less then minimum amount of stacking!");
    ...
    function stackToken(uint256 _amount, uint32 _stackingPlan) external nonReentrant{
        ...
        require(amount >= minimumStackAmount  , "You can not stack this amount because your amount is less than minimum amount");
    ...
    function claimRewards() external nonReentrant{
        ...
        uint256 stackedTime = 6;//getMonth(block.timestamp , stackingUserDeatil[msg.sender].timeOfLastUpdate); // check stacked time
        require(stackedTime > 0, "Month is not completed you have to wait!");
        console.log("stacking plan ======>", stackingUserDeatil[msg.sender].stackingPlan );
        console.log("claimedRewardsMonthCount[msg.sender] =====>>>", claimedRewardsMonthCount[msg.sender]);
        ...
        require(stackingUserDeatil[msg.sender].amountStaked > 0 , "Pending reward and stacked amount both are zero");
    ...
    function claimRewardNewStacking() internal {
        uint256 pendingMonths = getPendingMonths();
        console.log("=====>> claimedRewardsMonthCount[msg.sender] in new function", claimedRewardsMonthCount[msg.sender]);
    ...
    function submitRequestForUnstack() external nonReentrant {
        ...
        require(unstackingRequests[msg.sender] == false, "Your request is already submittied!");
        require(stackingUserDeatil[msg.sender].amountStaked > 0, "You don not have any Stacked Tokens for unstack");
    ...
    function approvedStackingRequest(address walletAddress) external onlyOwner{
        require(stackingUserDeatil[walletAddress].amountStaked > 0, "You don not have any Stacked tokens for withdraw");
    ...
    function unStackToken(address walletAddress) internal nonReentrant {
        ...
        if(stackingUserDeatil[walletAddress].stackingPlan == 1 && claimedRewardsMonthCount[walletAddress] >= 6 || stackingUserDeatil[walletAddress].stackingPlan == 2 && claimedRewardsMonthCount[walletAddress] >= 12 || stackingUserDeatil[walletAddress].stackingPlan == 3 && claimedRewardsMonthCount[walletAddress] >= 24 ){
            ...
            console.log("remaining amount if =====>", stackingUserDeatil[walletAddress].amountStaked);
        }else{
            ...
            console.log("remaining amount else =====>", remainingAmount);
