...
contract BnbGrapes is Ownable {
    ...
    // uint256 private REWARD_EPOCH_SECONDS = 86400; // 1 day
    uint256 public REWARD_EPOCH_SECONDS = 360; // 1 hour

    uint256 public GRAPES_TO_HATCH_1MINERS = SafeMath.mul(SafeMath.div(REWARD_EPOCH_SECONDS, PERCENT), 100);
    ...
    uint256[] public REF_LEVEL_PERCENT = [5, 2, 1, 1, 1]; // 10% in total
    ...
    function deposit(address payable inviter) external payable whenInitialized {
        require(msg.value >= MIN_DEPOSIT, "Deposit at least 1 BNB");
    ...
    function seedMarket() external payable onlyOwner {
        ...
        initialized = true;
        marketGrapes = SafeMath.mul(GRAPES_TO_HATCH_1MINERS, 100000);
    }
    ...
    function bnbRewards(address _address) external view returns(uint256) {
        ...
        require(hasGrapes > 0, "NOTHING TO SELL");
        uint256 bnbValue = calculateGrapeSell(hasGrapes);
        return bnbValue;
    }
