    ...
    ERC2981PerTokenRoyalties
{
    ...
    mapping(uint256 => bool) public iswinningId;
    mapping(address => uint256) private addressMints;
    mapping(uint256 => address) public winnerLog;
    mapping(uint256 => uint256) public winningIds;
    ...
    constructor(
        ...
        string memory _initpendingURI
    ) ERC721(_name, _symbol) {
        setBaseURI(_initBaseURI);
        setwinnerURI(_initwinnerURI);
        setpendingURI(_initpendingURI);
        drawNumber++;
    }
  ...
  */
    ...
    function _winnerURI() internal view virtual returns (string memory) {
        return winnerURI;
    }

    function _pendingURI() internal view virtual returns (string memory) {
        return pendingURI;
    }
  ...
  */
    function mint(uint256 _mintAmount) public payable {
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            ...
            // activate lottery once 100 minted
            if (tokenId - lotteryref >= 100) {
                lotteryref = tokenId;
                selectWinner();
                address _winner = getLastWinnerForDraw();
  ...
  */
        ...
        returns (string memory)
    {
        ...
        if (iswinningId[tokenId])
        return winnerURI;
        else if (tokenId >= lotteryref)
        return pendingURI;
        else return baseURI;
    }
  ...
  */
    ...
    function setpendingURI(string memory _newpendingURI) public onlyOwner {
        pendingURI = _newpendingURI;
    }
  ...
  */
    ...
    /*
    @description - Sends contract balance to wallet. Kept manual to prevent hacks.
    */
    function send_to_wallet() public payable onlyOwner {
    ...
    */
    function selectAWinner(
        ...
        } else if (it>=5 && winningToken >= lotteryref) {
            winnerLog[drawNumber] = winnerAddress;
            winningIds[drawNumber] = winningToken;
        ...
        returns (uint256)
    {
        ...
        return (seed - ((seed / _supply) * _supply)) + 1;
    }

    function getLastWinnerForDraw() public view returns (address) {
        return LastWinner;
    }
    ...
    */
    ...
    @description - Gets the winner for a given draw
    */
    function getWinnerForDraw(uint256 drawNo)
        public
        view
        returns (address, uint256)
    {
        require(drawNo <= drawNumber);
        return (winnerLog[drawNo], winningIds[drawNo]);
    }
}