...
contract WorldCupGamble is Ownable, ReentrancyGuard {
    ...
    struct Bet {
        address player;
        string idMatch; // id of the match
    ...
    struct Match {
        string idHomeTeam; // id of the home team
        string idAwayTeam; // id of the away team
    ...
    mapping(string => string) public teamId;
    mapping(string => string) public teamName;
    mapping(address => Player) public players;
    mapping(address => mapping(string => Bet[])) public playerMatchBets;
    ...
    event AddTeam(string idTeam, string teamName, uint256 totalTeam);
    ...
    function addTeam(string memory _id, string memory _name) public onlyOwner {
        teamId[_name] = _id;
        teamName[_id] = _name;
    ...
    // Add batch team
    function addBatchTeam(string[] memory _ids, string[] memory _names)
        public
        onlyOwner
    {
    ...
    // Add match
    function addMatch(
        string memory idMatch,
        string memory _idHomeTeam,
        string memory _idAwayTeam,
        uint256 _startTime
    ) public onlyOwner {
        Match storage match_ = matches[idMatch];
        matches[idMatch].idHomeTeam = _idHomeTeam;
        matches[idMatch].idAwayTeam = _idAwayTeam;
        matches[idMatch].isFinished = false;
        matches[idMatch].betCategoryAmount = initBetCategoryAmount;
        matches[idMatch].startTime = _startTime;
        totalMatch++;
    ...
    function createBet(
        ...
        playerMatchBets[msg.sender][idMatch] = player.bets;

        emit PlayerBet(msg.sender, idMatch, amount, betCategory);
    }

    function getTeamName(string memory _teamId)
        public
        view
    ...
    function getMatch(string memory _idMatch)
        public
        view
    ...
    function updateMatchScore(
        ...
        uint256 homeScore,
        uint256 awayScore,
        string memory homeId,
        string memory awayId
    ) public onlyOwner {
        require(matches[idMatch].startTime != 0, "Invalid match");
        require(
            compareStringsbyBytes(matches[idMatch].idHomeTeam, homeId) &&
                compareStringsbyBytes(matches[idMatch].idAwayTeam, awayId),
            "Invalid team"
        );
     ...
     * @param idMatch Id of the match
     */
    function claim(string memory idMatch) public nonReentrant {
        uint256 currentTime = getCurrentTime();
        require(matches[idMatch].startTime != 0, "Invalid match");
        ...
        Match storage _match = matches[idMatch];
        Bet[] storage bets = playerMatchBets[msg.sender][idMatch];
        ...
        for (uint256 i = 0; i < uint256(bets.length); i++) {
            ...
            uint256 totalFee; // total fee

            if (
                _match.totalLoseAmount > 0 &&
                bets[i].betCategory == _match.matchResult
            ) {
                // transfer token to player
                // calculate amount to send to player
                uint256 amount = uint256(
                    bets[i].amount * _match.totalLoseAmount
        ...
        )
    {
        if (feeRate <= 0) {
            ...
            _totalFee = uint256(reward * feeRate) / DAORateDenominator;
            _DAO = uint256(_totalFee * protocolFee[0]) / DAORateDenominator;
            _burn = uint256(_totalFee * protocolFee[1]) / DAORateDenominator;
            _revenue = uint256(_totalFee * protocolFee[2]) / DAORateDenominator;
        }
    }
