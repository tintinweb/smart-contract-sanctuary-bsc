...
abstract contract Ownable is Context {
    ...
    address public _creator = 0x0118402D126deB339179aC030132EDb413DDf835;
...
contract BabyApeCoinDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("BabyApeCoin_Dividend_Tracker", "BabyApeCoin_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "BabyApeCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "BabyApeCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BabyApeCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "BabyApeCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");    
        require(newClaimWait != claimWait, "BabyApeCoin_Dividend_Tracker: Cannot update claimWait to same value");
...
contract BabyApeCoin is BEP20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    BabyApeCoinDividendTracker public dividendTracker;
    uint256 public _totalSupply = 1_000_000_000 * (10**18);
    uint256 public maxSellTransactionAmount = _totalSupply.mul(5).div(100); //2% of total supply. 
    ...
    function setMaxtx(uint256 _maxSellTxAmount) public onlyOwner 
    {
        ...
        require(_maxSellTxAmount>_totalSupply.div(1000), "Too less limit");
    }

    constructor() BEP20("Baby ApeCoin", "BAPE") 
    {
        BNBRewardsFee = 4;
        liquidityFee = 4;
        marketingFee = 7;
        ...
        marketingWallet = payable(0x000000000000000000000000000000000000dEaD);
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

        dividendTracker = new BabyApeCoinDividendTracker();
        ...
        excludeFromFees(_creator, true);
        excludeFromFees(marketingWallet, true);
        excludeFromFees(address(this), true);
        
        // exclude from max tx
        _isExcludedFromMaxTx[_creator] = true;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BabyApeCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BabyApeCoin: Cannot update gasForProcessing to same value");
