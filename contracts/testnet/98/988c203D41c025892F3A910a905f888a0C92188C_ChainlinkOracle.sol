...
contract ChainlinkOracle is ChainlinkClient, ConfirmedOwner {
     ...
     */
    function requestTokenIds(address wallet) public {
        require(wallet != address(0),"Invalid Address");
        Chainlink.Request memory req = buildChainlinkRequest(jobId, address(this), this.fulfillBytes.selector);
        string memory _url = joinString(base_url,toAsciiString(wallet));        
        req.add(
            'get',
            _url
        );
     ...
     */
    function fulfillBytes(bytes32 requestId, string memory bytesData) public recordChainlinkFulfillment(requestId) {
    ...
    function joinString(string memory _a, string memory _b) internal pure returns (string memory)  {
