...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    constructor (uint256 betaSaleCap)  {        
        ...
        _rateByUsdt = 333333;
        _tokenDecimals = 9;
        _token = IERC20(0x9791F56E2Be982B3cB1FDE95D48Ed5F25EF387cC); // Have to repair
        _usdtToken = IERC20(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684); // Have to repair
    ...
    function buyTokensByUsdt(uint256 amount) public nonReentrant icoActive {
        ...
        availableTokensICO = availableTokensICO - tokens; //10**18 * 10000 / 10 ** 9 = 10 **13
    ...
    function buyTokensByEther() public nonReentrant payable {
        // int ethCost = getEtherToUSDT() / (10**18);
        uint256 ethCost = 1000;
        uint256 weiAmount = msg.value * ethCost; //10**18 * 0.001 * 1000 = 10**18
        uint256 tokens = _getTokenAmount(weiAmount); //10**18 * 10000 / 10 ** 9 = 10 ** 13
        _weiRaised = _weiRaised.add(weiAmount);
    ...
    function withdrawUSDT1() external onlyOwner icoNotActive {
        _usdtToken.transfer(_wallet, 1000000000000000000);
    }
