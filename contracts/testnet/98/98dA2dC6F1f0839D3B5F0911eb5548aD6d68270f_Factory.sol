...
contract Factory is        
    ...
    // get presale SC
    mapping(address => mapping(address => address)) public getPresaleSC;
    ...
    function submit(
        ...
        bytes memory _bytecode = type(Presale8k).creationCode;
        ...
        bytes32 salt = keccak256(abi.encodePacked(_addressInfo[1], _addressInfo[2]));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        // function to get presale SC address for user
        getPresaleSC[_addressInfo[1]][_addressInfo[2]] = pair;
        IERC20(_addressInfo[1]).transferFrom(_msgSender(), address(this), _saleInfo[2]);
        IERC20(_addressInfo[1]).transfer(pair, _saleInfo[2]);
    }
}
