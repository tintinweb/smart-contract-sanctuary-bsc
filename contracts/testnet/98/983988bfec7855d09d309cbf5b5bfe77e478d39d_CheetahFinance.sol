...
library IterableMapping {
    ...
    function isDeleted(Map storage map, address key) public view returns (bool) {
    ...
    function insert(Map storage map, address key) public view returns (bool) {
...
contract CheetahFinance is ERC20, Ownable {
    ...
    constructor() ERC20("Cheetah Finance", "$CHEETAH") {
        ...
        CheetahFinanceNFTs = IERC721(0xe8404d84a2A7E37aA1cfE137d6bD59612CF4417F);
        buyFee.liquidityFee = 20;
        buyFee.marketingFee = 10;
        buyFee.angelInvestorFee = 10;
        totalBuyFee = 40;   
        swapEnabled = false;

        sellFee.liquidityFee = 30;
        sellFee.marketingFee = 15;
        sellFee.angelInvestorFee = 15;
        totalSellFee = 60;
    ...
    function ownerSetTokenHolder(address user, uint256 userBalance) external onlyOwner {
    ...
    function getIllMappingForUser(address user) external view returns(address _user, uint256 _values, int _indexOf, bool _inserted, bool _isDelete ) {
        _user = user;
    ...
    function process(uint256 _supplyDelta) internal {        
        uint256 len = getNumbersOfTokenHolders();
        uint256 baseDelta = _supplyDelta;
        for (uint256 i; i < len; i++) {
            address userInd = tokenHoldersMap.getKeyAtIndex(i);
            uint256 getBal = balanceOf(userInd);
            uint256 userNFT = CheetahFinanceNFTs.balanceOf(userInd);
            uint256 userAddedFunds = getBal.mul(baseDelta).div(1000E18);
            if (userNFT == 0 ) {
                _gonBalances[userInd] = _gonBalances[userInd].add(userAddedFunds.mul(_gonsPerFragment));
            }
            if (userNFT >= 1) {                
                if (userNFT < 5) {
                    uint256 newAPY = userAddedFunds.mul(userNFT);
                    _gonBalances[userInd] = _gonBalances[userInd].add(newAPY.mul(_gonsPerFragment));
                } else if (userNFT > 5) {
                    uint256 newAPY = userAddedFunds.mul(5);
                    _gonBalances[userInd] = _gonBalances[userInd].add(newAPY.mul(_gonsPerFragment));
                }
            }
        }
    ...
    function takeFee(
        ...
        if (sender == address(pair)) {
            ...
            require(gonAmount <= maxBuyAmount, "Buy exceeds limit");
        } else if (recipient == address(pair)) {
            _totalFee = totalSellFee;
            require(gonAmount <= maxSellAmount, "Sell exceeds limit");
        }

        if (recipient != address(pair)) {
                require(
                    balanceOf(recipient) + gonAmount <= maxWalletAmount,
