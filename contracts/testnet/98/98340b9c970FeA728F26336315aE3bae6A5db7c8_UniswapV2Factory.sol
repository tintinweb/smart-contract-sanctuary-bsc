    ...
    OwnableUpgradeable
{
    ...
    function createPair(address tokenA, address tokenB) external override returns (address pairProxy) {
        ...
        bytecode = abi.encodePacked(bytecode, address(pairImpl), "");
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pairProxy := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        UniswapV2Pair(pairProxy).initialize(token0, token1);
...
contract UniswapV2Pair is UniswapV2ERC20, Initializable {
    using SafeMath  for uint;
