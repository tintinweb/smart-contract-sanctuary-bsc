...
contract BurnTheSupply is Context, IERC20 { 
    ...
    uint256 private OriginalSupply = _totalSupply;
    string private constant _symbol = "BLUNA"; 
    string private constant _name = "Burn The Supply"; 
    ...
    uint256 public Percent_Terra_Luna_Burn = 70;
    uint256 public Percent_Dev = 20;
    uint256 public Percent_AutoLP = 10;

     // Max Wallet amount (3%)
    uint256 public _maxWalletToken = _totalSupply * 3 / 100;
    // Maximum transaction amount (4% at launch) (If from or to wallet is only owner wallet, available to send over than this value)
    uint256 public _maxTxAmount  = _totalSupply * 4 / 100;

    // Blacklist wallet state(prevent _isBlacklisted wallets from transfer/selling tokens if noBlackList is true)
    ...
    // Exclude from limit (_maxWalletToken limit)
    ...
    function openTrade() external onlyOwner() {
        ...
        require(Percent_Terra_Luna_Burn == 70 && Percent_Dev == 20 && Percent_AutoLP == 10, "Trading open conditions are not met!");
        TradeOpen = true;
        emit OpenedTrade();
    } 
    ...
    function walletUpdateDev(address wallet) external onlyOwner() {
        ...
        _isExcludedFromFee[wallet] = true;  // Update mapping on new wallet        
    }
    ...
    function setMaxWalletHoldingPercent(uint256 max_Wallet_Holding_Percent) external onlyOwner() {
        ...
        emit UpdatedMaxHoldPercent(_maxWalletToken);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != owner() && txCount >= swapTrigger && !inSwapAndLiquify && !_isPair[from] && swapAndLiquifyEnabled && block.number > swapBlock){  
