    ...
    PausableUpgradeable
{
    ...
    event LinearPoolCreated(
        ...
        address linearAcceptedToken,
        address linearRewardToken,
        address linearRewardDistributor,
        uint64 APR,
        uint8 exchangeRate,
        uint256 capacity,
    ...
    struct LinearPoolInfo {
        ...
        uint256 totalStaked;
        uint64 APR;
        uint8 exchangeRate; // default: REWARD_TOKEN =  exchangeRate * ACCEPTED_TOKEN
        uint256 lockDuration;
        uint256 startJoinTime;
        uint256 endJoinTime;
    }
    ...
    // mapping(uint256 => mapping(address => uint256)) public linearRemainReward;
    ...
    event LinearSetDelayDuration(uint256 poolId, uint256 duration);
     ...
     */
    function __LinearPool_init(address _treasury) public initializer {
        __Ownable_init();
        __Pausable_init();
        pauseContract();
    ...
    function linearAddPool(
        address _linearAcceptedToken,
        address _linearRewardToken,
        ...
        linearPoolInfo.push(
            LinearPoolInfo({
                linearAcceptedToken: _linearAcceptedToken,
                linearRewardToken: _linearRewardToken,
    ...
    function linearSetPool(
        ...
        linearPoolInfo[_poolId].linearAcceptedToken = _linearAcceptedToken;
        linearPoolInfo[_poolId].linearRewardToken = _linearRewardToken;
        linearPoolInfo[_poolId]
            .linearRewardDistributor = _linearRewardDistributor;
        linearPoolInfo[_poolId].cap = _cap;
        linearPoolInfo[_poolId].APR = _APR;
        linearPoolInfo[_poolId].exchangeRate = _exchangeRate;
        linearPoolInfo[_poolId].endJoinTime = _endJoinTime;

        emit LinearUpdatePool(
            _poolId,
            _linearAcceptedToken,
            _linearRewardToken,
            _linearRewardDistributor,
        ...
        linearValidatePoolById(_poolId)
    {
        ...
        require(treasury != address(0), "LinearStakingPool: treasury is not set");
        IERC20(linearPoolInfo[_poolId].linearAcceptedToken).safeTransferFrom(
            account,
            treasury,
            _amount
        );
        ...
        linearValidatePoolById(_poolId)
    {
        ...
        address linearRewardDistributor = pool.linearRewardDistributor;
        ...
        if (stakingData.reward > 0) {
            ...
            IERC20(linearPoolInfo[_poolId].linearRewardToken).safeTransferFrom(
        ...
        stakingData.balance = 0;
        IERC20(linearPoolInfo[_poolId].linearAcceptedToken).safeTransfer(
            account,
            withdrawBalance
        );
    ...
    function linearSetDelayDuration(uint256 _poolId, uint256 _duration)
        external
        onlyOwner
        linearValidatePoolById(_poolId)
    {
        ...
        linearValidatePoolById(_poolId)
    {
        ...
        uint256 reward;
        uint256 totalReward = stakingData.reward;
        if (totalReward > 0) {
        ...
        returns (uint256 reward)
    {
        ...
        uint256 pendingReward = ((stakingData.balance *
            stakedTimeInSeconds *
            pool.APR *
            pool.exchangeRate) / ONE_YEAR_IN_SECONDS) / 100;
    ...
    // TODO: Is the deposited token send to treasury?
    ...
    function _linearClaimPendingWithdraw(uint256 _poolId) private {
        ...
        IERC20(linearPoolInfo[_poolId].linearAcceptedToken).safeTransferFrom(
            linearPoolInfo[_poolId].linearRewardDistributor,
            account,
            reward
        );
