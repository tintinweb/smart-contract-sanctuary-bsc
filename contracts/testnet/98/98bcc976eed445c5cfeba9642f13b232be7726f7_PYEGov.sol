...
contract PYEGov{
    ...
    function vote(bool vote, uint256 prop) public {
    ...
    function initprop(string memory propmsg, uint256 _endtime) public {
        ...
        if(snap){
            ...
            uint256 tempsnap = IGov(tokenaddress).snapshot();

            require(weight[msg.sender].votes > IGov(tokenaddress).totalSupplyAt(tempsnap)/100, "Need atleast 1% weight to propose");

        } else {

            require(weight[msg.sender].votes > IGov(tokenaddress).totalSupply()/100, "Need atleast 1% weight to propose");
