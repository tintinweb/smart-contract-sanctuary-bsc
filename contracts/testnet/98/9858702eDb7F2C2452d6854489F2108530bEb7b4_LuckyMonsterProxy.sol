...
// File contracts/LuckyMonsterProxy.sol

pragma solidity ^0.8.6;



contract LuckyMonsterProxy is ERC721Enumerable, LuckyMonsterStorage, Proxy {

    constructor (address logicAddr_, string memory baseURI_) ERC721("LM-TEST", "LM-TEST") {
        _logicAddr = logicAddr_;
        baseURI = baseURI_;
    }
    ...
    function setLogicAddr(address logicAddr_) external onlyOwner {
        _logicAddr = logicAddr_;
    }
    ...
    function gameInfo() external view returns(GameInfo memory info) {
        ...
        info.aliveMonsters = balanceOf(address(this));
        ...
        info.nextBombZones = _nextBombZones;
        info.aliveZones = _aliveZones;
    }

    function monsterInfo(uint256 tokenId) external view returns(MonsterInfo memory info) {
