...
contract CloudaxLaunchpad is Ownable, ReentrancyGuard {
    ...
    event UserSalesCapUpdated(address buyer, uint256 amount, Presale saleId);
    ...
    function _buyLaunchpadSale(
        ...
        require(
                saleParticipantsCap[sale][msg.sender].add(busdAmount) <= launchpadSale.saleMax,
                "You current purchase will exceed the maximum presale purchase cap"
            );
        require(
        ...
        if (saleParticipantsCap[sale][msg.sender] != 0) {
            saleParticipantsCap[sale][msg.sender] = saleParticipantsCap[sale][
             ...
             emit UserSalesCapUpdated(msg.sender, busdAmount, sale);
        } else {
