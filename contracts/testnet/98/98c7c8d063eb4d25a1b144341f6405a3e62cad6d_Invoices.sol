...
contract Invoices is Ownable {
    ...
    function create_invoice(
        ...
        invoices[invoice_id] = Invoice(_amount, _seller, 0, block.timestamp.add(_untilTime), _token);
    ...
    function pay(uint256 _invoice_id) external onlyAvailable(_invoice_id) {
        IERC20 _token = IERC20(invoices[_invoice_id].token);
        uint256 allowance = _token.allowance(msg.sender, address(this));
        require(invoices[_invoice_id].amount <= allowance, "Not enough token");
        _token.transferFrom(
            msg.sender,
            address(this),
        ...
        _token.transfer(invoices[_invoice_id].seller, weiAmount);
        _token.transfer(treasury, invoices[_invoice_id].amount.sub(weiAmount));
