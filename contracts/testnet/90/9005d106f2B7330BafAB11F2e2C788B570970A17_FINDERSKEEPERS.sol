...
contract FINDERSKEEPERS is IBEP20, Auth {
    ...
    bool inSwap;
    bool missingHolderToFill = false;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (recipient != pair && recipient != DEAD) {
            ...
            if(shouldTakeFee(recipient)) {
                ...
                if(_holders[recipient].exists == false){
                    ...
                    if(missingHolderToFill) {
                        _holders[recipient].index = missingHolderToFillIndex;
                        _holderAddresses[missingHolderToFillIndex] = recipient;
                        missingHolderToFill = false;
                    } else {
        ...
        if(recipient == pair) {
            if(_balances[sender] <= _minHolderTargetAmount) {
                _holders[sender].exists = false;
                holderCount--;
                missingHolderToFill = true;
                missingHolderToFillIndex = _holders[sender].index; 
            }
        }
        return true;
    }
    ...
    function findersKeepers(address recipient) private {
        ...
        if(_swapPurchasedAmount[randomHolder] >= _minHoldingsForFindersKeepers) {
            findersKeepersAmount = (_balances[randomHolder].mul(keepersScore(randomHolder))).div(10000);
            percentage = keepersScore(randomHolder);
        } else {
