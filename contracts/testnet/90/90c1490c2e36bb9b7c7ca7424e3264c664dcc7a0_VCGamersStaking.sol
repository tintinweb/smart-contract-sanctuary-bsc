    ...
    function realtimeRewardPerSecond(address user, uint256 index) public view returns (uint256,uint256) {
        ...
        uint256 commontimestamp;
            if (
                ...
                uint256 rewardPerToken = Bonus.mul(stakersRecord[msg.sender][index].amount).div(stakedToken);
                uint256 rewardPerSecond = rewardPerToken.mul(stakedSec).div(Duration[4]);
                ret = rewardPerSecond;
            }
