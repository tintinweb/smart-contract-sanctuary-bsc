...
contract Comptroller is ComptrollerV5Storage, ComptrollerInterfaceG2, ComptrollerErrorReporter, ExponentialNoError {
    ...
    event VenusSpeedUpdated(EToken indexed eToken, uint newSpeed);
    ...
    modifier onlyListedMarket(EToken eToken) {
        require(markets[address(eToken)].isListed, "venus market is not listed");
        _;
    }
     ...
     * @param vTokens The list of addresses of the eToken markets to be enabled
    ...
    function enterMarkets(address[] calldata vTokens) external override returns (uint[] memory) {
        ...
        for (uint i = 0; i < len; i++) {
            results[i] = uint(addToMarketInternal(EToken(vTokens[i]), msg.sender));
        }
    ...
    function exitMarket(address vTokenAddress) external override returns (uint) {
        EToken eToken = EToken(vTokenAddress);
        ...
        // In order to delete eToken, copy last item in list to location of item to be removed, reduce length by 1
        EToken[] storage userAssetList = accountAssets[msg.sender];
     ...
     */
    function mintAllowed(address eToken, address minter, uint mintAmount) external onlyProtocolAllowed override returns (uint) {
     ...
     */
    function redeemAllowed(address eToken, address redeemer, uint redeemTokens) external override onlyProtocolAllowed returns (uint) {
     ...
     */
    function redeemVerify(address eToken, address redeemer, uint redeemAmount, uint redeemTokens) external override {
        // Shh - currently unused
        eToken;
        redeemer;
     ...
     */
    function borrowAllowed(address eToken, address borrower, uint borrowAmount) external override onlyProtocolAllowed returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        ...
        if (!markets[eToken].accountMembership[borrower]) {
            ...
            Error err = addToMarketInternal(EToken(eToken), borrower);
            if (err != Error.NO_ERROR) {
                return uint(err);
            }
        }
    ...
    function repayBorrowAllowed(
        ...
        address payer,
        address borrower,
        uint repayAmount) external override onlyProtocolAllowed returns (uint) {
        // Shh - currently unused
        payer;
        borrower;
        repayAmount;
    ...
    function liquidateBorrowAllowed(
        ...
        (Error err, , uint shortfall) = getHypotheticalAccountLiquidityInternal(borrower, EToken(address(0)), 0, 0);
        if (err != Error.NO_ERROR) {
            return uint(err);
        }
        if (shortfall == 0) {
        ...
        borrowBalance = EToken(vTokenBorrowed).borrowBalanceStored(borrower);
    ...
    function seizeAllowed(
        ...
        address liquidator,
        address borrower,
        uint seizeTokens) external override onlyProtocolAllowed returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        ...
        if (EToken(vTokenCollateral).comptroller() != EToken(vTokenBorrowed).comptroller()) {
     ...
     */
    function transferAllowed(address eToken, address src, address dst, uint transferTokens) external override onlyProtocolAllowed returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
    ...
    function getHypotheticalAccountLiquidity(
        ...
        (Error err, uint liquidity, uint shortfall) = getHypotheticalAccountLiquidityInternal(account, EToken(vTokenModify), redeemTokens, borrowAmount);
        return (uint(err), liquidity, shortfall);
    }
     ...
     * @param vTokenBorrowed The address of the borrowed eToken
     * @param vTokenCollateral The address of the collateral eToken
     * @param actualRepayAmount The amount of vTokenBorrowed underlying to convert into vTokenCollateral tokens
     * @return (errorCode, number of vTokenCollateral tokens to be seized in a liquidation)
     */
    function liquidateCalculateSeizeTokens(address vTokenBorrowed, address vTokenCollateral, uint actualRepayAmount) external view override returns (uint, uint) {
        /* Read oracle prices for borrowed and collateral markets */
        uint priceBorrowedMantissa = oracle.getUnderlyingPrice(EToken(vTokenBorrowed));
        uint priceCollateralMantissa = oracle.getUnderlyingPrice(EToken(vTokenCollateral));
         ...
         */
        uint exchangeRateMantissa = EToken(vTokenCollateral).exchangeRateStored(); // Note: reverts on error
        uint seizeTokens;
    ...
    function _supportMarket(EToken eToken) external returns (uint) {
        ...
        Market storage mk = markets[address(eToken)];
        mk.isListed = true;
        mk.isVenus = false;
        mk.collateralFactorMantissa = 0;

        //markets[address(eToken)] = Market({isListed: true, isVenus: false, collateralFactorMantissa: 0});

        _addMarketInternal(eToken);
    ...
    function _addMarketInternal(EToken eToken) internal {
        for (uint i = 0; i < allMarkets.length; i ++) {
            require(allMarkets[i] != eToken, "market already added");
        }
        allMarkets.push(eToken);
    }
    ...
    /**
      ...
      */
    function _setMarketBorrowCaps(EToken[] calldata vTokens, uint[] calldata newBorrowCaps) external {
 ...
 * ETokens should reference this contract as their comptroller.
 */
