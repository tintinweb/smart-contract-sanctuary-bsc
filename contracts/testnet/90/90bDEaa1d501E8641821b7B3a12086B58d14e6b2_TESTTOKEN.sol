...
contract TESTTOKEN is ERC20, Ownable {
    ...
    address public RewardToken =
        address(0x8a9424745056Eb399FD19a0EC26A14316684e274); //DAI Testnet RewardToken

    uint256 public swapTokensAtAmount = 22 * 10**3 * (10**18); //for testing 22000 tokens
    uint256 public maxBuyAmount = 9 * 10**4 * 10**18; // 90000 (0.1% of the supply per buy)
    uint256 public maxSellAmount = 9 * 10**4 * 10**18; // 90000 (0.1% of the supply per sell)
    uint256 public maxWalletAmount = 9 * 10**5 * 10**18; // 900 000 (antiwhale)

    address public marketingWallet = address(0x123);
    address public buybackWallet = address(0x456);
    ...
    constructor() ERC20("TestPoken", "TESTTOKEN") {
        dividendTracker = new TESTTOKENDividendTracker(RewardToken);
        ...
        _mint(owner(), 9 * 10**7 * (10**18));
    }

    receive() external payable {}

    function updateRewardToken(address newToken) public onlyOwner {
        TESTTOKENDividendTracker newDividendTracker = new TESTTOKENDividendTracker(
            newToken
        );
    ...
    function withdrawForeignTokens(address _token, uint256 _amount) external onlyOwner {
    ...
    function setBuyFees(
        ...
        totalBuyFee =
            ...
            require (totalBuyFee <= 300, " max buy fees limit is 30%");
    }
    ...
    function setSellFees(
        ...
        totalSellFee =
            ...
            sellFee.marketing +
            sellFee.buyback;
            require (totalSellFee <= 300, " max buy fees limit is 30%");
    }
    ...
    function claimOldDividend(address tracker) external {
        TESTTOKENDividendTracker oldTracker = TESTTOKENDividendTracker(tracker);
    ...
    function swapAndSendToFee(uint256 tokens, uint16 fees) private {
        ...
        uint256 marketingShare = newBalance
            ...
            .div(fees);
        uint256 buybackShare = newBalance.mul(buyFee.marketing + sellFee.marketing).div(fees);
...
contract TESTTOKENDividendTracker is Ownable, DividendPayingToken {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "TESTTOKEN_Dividend_Tracker: claimWait must be updated to between 8 and 24 hours"
        );
