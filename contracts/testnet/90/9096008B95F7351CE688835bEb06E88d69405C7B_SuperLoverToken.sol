...
contract UsdtWrap {
    ...
    constructor (IERC20 _token520)  {
        token520 = _token520;
        usdt = IERC20(0x6cd2Bf22B3CeaDfF6B8C226487265d81164396C5);
    }
...
contract SuperLoverToken is ERC20, Ownable {
    ...
    uint256 private totalSupply_ = 10E25 * (10**2);
    uint256 public swapTokensAtAmount = totalSupply_.mul(2).div(10**21);

    uint256 public USDTBonusFee = 2;
    ...
    uint256 public minimumTokenBalanceForDividends = 10E22 * (10**2);



    address public USDT = 0x6cd2Bf22B3CeaDfF6B8C226487265d81164396C5;
    ...
    function setUSDTBonusFee(uint256 amount) public onlyOwner {
        USDTBonusFee = amount;
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(ammPairs[from]){
                ...
                finalAmount = finalAmount.sub(_LiquidityFee).sub(_USDTBonusFee).sub(_MarketingFee).sub(_DeadFee);
                ...
                if((!ammPairs[from] && !ammPairs[to])){
                    if(_isExcludedFromFees[from] || _isExcludedFromFees[to]){
                    burnAmount = amount.mul(A).div(100);
