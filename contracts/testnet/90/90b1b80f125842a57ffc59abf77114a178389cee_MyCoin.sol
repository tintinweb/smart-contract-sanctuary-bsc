...
pragma solidity 0.5.16;
  ...
  contract MyCoin is Context, IBEP20, Ownable {
    ...
    uint256 private decreaseRate = 15000;
    uint256 private w = 20000;
    uint256 private v = 6;
    ...
    function getGain() internal view returns (uint256) {
      ...
      uint256 diffA = (_globalGain > _globalLoss) ? _globalGain - _globalLoss : 0;
      uint256 diffB = (_globalLoss > _globalGain) ? _globalLoss - _globalGain : 0;
    ...
    function nextDecrease(uint256 R) internal view returns (uint256) {
      ...
      uint256 diffA = (globalGain > globalLoss) ? globalGain - globalLoss : 0;
      uint256 diffB = (globalLoss > globalGain) ? globalLoss - globalGain : 0;
      uint256 temp = (c - R); 
      if(temp > c) temp = c;
    ...
    function getGlobals() external view returns (uint256[14] memory) {
      (uint256 R, uint256 c, uint256 r) = getVars();
      return [c, R, r, getGain(), prevPrice, globalLoss, globalGain, v, w, l, a, nextDecrease(R), nextIncrease(R), getNow()];
    }
