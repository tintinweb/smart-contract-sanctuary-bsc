...
contract RandomNumberGenerator is VRFConsumerBaseV2, Ownable {
  uint256 constant fee = 0.005 * 10 ** 18; // fee to get random number
  ...
  string public lastPotId; // last Pot ID
  ...
  function fulfillRandomWords(
    ...
    require(lastRequestId == requestId, "request not found");
    randomResult = randomWords;
    ILottery(lottery).setWinningNumber(lastPotId, randomResult, requestId);
  }
  ...
  function updateLottery(address contractLottery) external onlyOwner {
    lottery = contractLottery;
  }

  function updateSubId(uint64 subId) external onlyOwner {
    s_subscriptionId = subId;
  }
