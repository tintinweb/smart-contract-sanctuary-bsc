...
contract Arbitration {
   ...
   //金额相关
    ...
    uint256                                             public minFee = 10*1E18;              //申请仲裁员要求的最低保证金
   //时间相关
   ...
   ///仲裁员资格
    //通过抵押代币成为仲裁员
    function arbApply(string memory data,uint256 arbFee) external { 
        UserInfo storage user = userInfo[msg.sender];
        if(bytes(data).length != 0) commun(data);
        require(user.idForArb == 0 ,"arb/already-apply");
        require(arbFee >= minFee,"arb/Less-minimum");
        arbId +=1;
    ...
    function exitArb(address usr, uint256 wad) internal { 
        ...
        if (user.idForArb != 0 && user.balanceMar < minFee) {
            uint256 id =  user.idForArb;
            cover(id);
        }
    }
    ...
    function arbTwoApply(uint256 _order) external  {
        ...
        require(arbOrder.time[2] == 0, "arb/Apply-already-end");
        ...
        people += 1;
        if (people >= quorum && secondRegistrationWaitingTime < block.timestamp-arbOrder.time[1]) arbOrder.time[2] = block.timestamp;
        if (people == maxPeople)  arbOrder.time[2] = block.timestamp; 
    ...
    //投票
    function vote(uint256 _order, uint256 schemeId) external returns(uint256){
       OrderInfo storage arbOrder = orderInfo[_order];
       require(voteCycle > block.timestamp - arbOrder.time[2] && arbOrder.time[2] != 0 , "arb/arbitration-not-end");
    ...
    function cancelVote(uint256 _order, uint256 schemeId) external {
      ...
      // emit Notvote(i,schemeId,weight[i][timc][schemeId],msg.sender);
    }
    ...
    function executeFroTwo(uint256 _order, uint256 schemeId) public {
        ...
        require(weight[_order][arbOrder.time[1]][schemeId] >= minVote, "arbtwo/The number of votes did not meet the statutory requirements");
        require(arbOrder.time[3] != 0 && communityVoteCycle < block.timestamp - arbOrder.time[3], "arbtwo/not-accept or Community appeal not over");
    ...
    //第一轮仲裁员保证金分配
     function marAllocationForOne(uint256 _order,uint256 schemeId) internal{
        ...
        address arbiterA = arbOrder.arbiters[0];
        address arbiterB = arbOrder.arbiters[1];
  ...
  ///前端接口
    ...
    function applySuceed(uint256 _order) external view returns (address[] memory,uint256[] memory) {
        ...
        for (uint j = 0; j < length ; ++j) {
            ...
            addr[j] = arbitrators;
            _pau[j] = voteMark[_order][orderInfo[_order].time[1]][arbitrators];
        }
        return (addr, _pau);
    }
    //订单方案对应的投票者
    function voteSuceed(uint256 _order,uint256 pau) external view returns (address[] memory) {
        uint256 timc = orderInfo[_order].time[1];
        uint256 k;
