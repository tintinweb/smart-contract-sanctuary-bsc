...
contract ObiDient is ERC20, Ownable, BaseToken {
    ...
    function purchase(uint256 bnbAmount, address ref) internal returns (bool) {
        ...
        uint256 totalFeesBUSD = difference.mul(bTotalFees).div(FEE_DENOMINATOR);
        IERC20(BUSD).transfer(
            publicCampaignWallet,
            totalFeesBUSD.mul(bPublicCampaignFee).div(bTotalFees)
        );
        IERC20(BUSD).transfer(
            developmentWallet,
            totalFeesBUSD.mul(bDevelopmentFee).div(bTotalFees)
        );
        if (refs[msg.sender] != address(0)) {
            IERC20(BUSD).transfer(
                refs[msg.sender],
                totalFeesBUSD.mul(bReferralFee).div(bTotalFees)
            );
        } else {
            IERC20(BUSD).transfer(
                msg.sender,
        ...
        uint256 tokensToSend = nShouldPurchase.mul(spreadDivisor).div(FEE_DENOMINATOR);
    ...
    function sell(uint256 tokenAmount) public returns (bool) {
        ...
        uint256 totalFeesTokens = tokenAmount.mul(sTotalFees).div(FEE_DENOMINATOR);
