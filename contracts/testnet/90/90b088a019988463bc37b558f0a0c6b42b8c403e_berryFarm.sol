...
contract berryFarm is Ownable {   
  ...
  constructor() {
    tariffs.push(Tariff(365 * 86400, 3650));
  ...
  function Buy(address referer) external payable {
    ...
    investors[msg.sender].roiAmountTill = block.timestamp+oneDay;
 ...
 function reinvest() external  {
  ...
  function withdraw(address payable addressAt) external {
      require (msg.sender==addressAt,"address should be same");
    uint amount = profit();
    require(amount >= 1 );
    if (addressAt.send(amount)) {
