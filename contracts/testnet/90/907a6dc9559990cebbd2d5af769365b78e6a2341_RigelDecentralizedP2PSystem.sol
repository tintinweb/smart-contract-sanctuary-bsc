...
abstract contract Rigelp2pPriceSet is Ownable, events {    
    ...
    function resetUnlistedTokenPercent(uint256 _new) external onlyOwner {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
        ds.unlistedTokenFees = _new;
    }
    ...
    function getBatchTokenFeeForMerchant(address[] memory _asset) external view returns(uint256[] memory _fee) {
        ...
        for ( uint256 i; i < _asset.length; i++) {
            _fee[i] = getTokenFeeFormerchant(_asset[i]);
        }
        return _fee;
    }
...
contract RigelDecentralizedP2PSystem is Rigelp2pPriceSet {
     ...
     * @param unlistedTokenFee the percentage reward to be receive when an unlisted tokens is to be traded;
    ...
    */
    constructor (
        ...
        ds.devAddress = dev;
        ds.unlistedTokenFees = unlistedTokenFee;
    }
    ...
    function makeSellOrder(IERC20 purchaseToken, uint256 amount, address from, address to, address referral) external returns(uint256){   
        ...
        _settleReferralRewards(_s_seller, to, referral);
    ...
    function _settleReferralRewards(uint256 s_seller, address to,  address referral) internal {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();   
        if (referral != address(0)) {
            ...
            if(!ds.refWhitelist[referral]) {
                ...
                if(specialReferral != 0) {
                    ...
                    _contractTransfer(address(ds.RGP), referral, refWShare); 
                } else {
    ...
    //  */ 
    function makeBuyPurchase(IERC20 purchaseToken, address from, uint256 productID) external onlyAdmin {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage(); 
        ...
        uint256 gas = getTokenFeeFormerchant(address(purchaseToken));
        uint256 remnant;
        if (gas == 0) {
            uint256 tMount = (purchaseToken.decimals()); // ds.unlistedTokenFees
            subGas = amount * (ds.unlistedTokenFees * 10 ** (tMount - 2)) / 100 * 10 ** tMount;
        ...
        _contractTransfer(s_token, s_buyer, subGas);
    ...
    function _merchantChargesDibited(address from, uint256 amount) internal {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
        if (!IERC20(ds.RGP).transferFrom(from, ds.devAddress, amount))  revert Unable_To_Withdraw(); 
    }
