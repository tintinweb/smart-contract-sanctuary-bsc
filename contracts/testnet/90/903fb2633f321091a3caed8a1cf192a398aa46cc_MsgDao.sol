...
// File contracts/core/MsgDao.sol
...
contract MsgDao {
    ...
    function setDaoReward(uint256 _amount) external onlyGov {
        daoReward = _amount;
    }
    ...
    function addNode(address _address) public onlyGov {
        if (Address.isContract(_address) || Address.isContract(tx.origin)) {
            return;
        }
    ...
    function addEdge(address father, address child) public onlyGov {
        addNode(father);
        addNode(child);
        if (
            !hasFatherByAddress(child) &&
