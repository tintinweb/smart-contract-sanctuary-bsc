...
contract NFT_1155 is ERC1155, ERC1155URIStorage, ERC1155Supply, Ownable {
    ...
    // Array of NFT IDs
    struct Nft { uint256 id; }

    // Array of Active NFT IDs
    Nft[] public _active; // TODO Test it

    // Mapping for NFT from <token_id> to <owner_address>
    mapping(uint256 => address) private _owners;

    // Mapping from Owner to owned NFT IDs
    mapping(address => Nft[]) private _owned; // TODO Test it
    ...
    function getOwnedIDs(address owner) public view virtual returns (Nft[] memory) {
        return _owned[owner];
    }
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if ( from == address(0) ) {
                ...
                // Add ID into _active structure
                _active.push( Nft( ids[i] ) ); // TODO Test it

                // Add ID into _owned structure
                _owned[ to ].push( Nft( ids[i] ) ); // TODO Test it
            ...
            if ( to == address(0) ){
                ...
                // Remove ID from _active structure
                delete _active[ ids[i] ];  // TODO Test it _active[ Nft( ids[i] ) ]

                // Remove ID from _owned structure
                delete _owned[ to ][ ids[i] ];  // TODO Test it _owned[ to ][ Nft( ids[i] ) ]
            ...
            if ( from != address(0) && to != address(0) ) {
                ...
                // Update ID into _owned structure
                _owned[ to ].push( _owned[ from ][ ids[i] ] );  // TODO Test it
                delete _owned[ from ][ ids[i] ]; // TODO Test it
