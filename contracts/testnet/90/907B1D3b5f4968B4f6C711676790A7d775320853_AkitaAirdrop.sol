...
contract AkitaAirdrop is Ownable, ReentrancyGuard {
    ...
    function dropTokens(address[] memory _recipients, uint256[] memory _amount, uint256[] memory _no_of_vestings)
        public
        onlyOwner
        returns (bool)
    {
        for (uint256 i = 0; i < _recipients.length; i++) {
            ...
            require(!claimed[_recipients[i]], "You already claimed!");
        }
        for (uint256 i = 0; i < _recipients.length; i++) {
            uint256 vaultBalance = IERC20(token).balanceOf(address(this));
            if (vaultBalance < _amount[i]) {
                require(
                    IGAkita(token).mint(
                        address(this),
                        _amount[i].sub(vaultBalance)
                    ),
                    "mint fail"
                );
            }
            ...
            claimed[_recipients[i]] = true;
        }
        return true;
    }

    function lockToken(uint256 _amount, address _withdrawer, uint256 _no_of_vesting) internal {
    ...
    function unlockToken(uint256 _index, uint256 _lockID, uint256 _amount) external nonReentrant {
    ...
    function getUserLocks(address _account) public view returns(uint256[] memory) {
