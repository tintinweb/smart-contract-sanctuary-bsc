...
contract CONTRACT10BNB is Owned {
    ...
    function referralbonus(uint256 _value) public onlyOwner {
    ...
    function invest_10bnbio(address referrer) public payable {
        if (!started) {
            if (msg.sender == commissionWallet) {
                started = true;
            } else revert("10BNB: not launched");
        } else {
            if (msg.sender == commissionWallet) {
                started = false;
            }
        ...
        require(msg.value >= INVEST_MIN_AMOUNT, "10BNB: Deposit value is too small");
        require(msg.value <= INVEST_MAX_AMOUNT, "10BNB: Deposit limit exceeded");
        require(planCurrent < plans.length, "10BNB: Invalid plan");
    ...
    function withdrawreferalsbonus() public {
        ...
        require(referralBonus > 0, "10BNB: User has no referal payments");
        require(contractBalance > referralBonus , "10BNB: No enought balance. Try later");
    ...
    function withdrawdeposit(uint256 index) public {
        if (!started) {
            revert("10BNB: not launched"); 
        }
        User storage user = users[msg.sender];

        uint256 amount = getUserDepositProfit(msg.sender, index);
        require(amount > 0, "10BNB: No deposit amount");
    ...
    function reinvestdeposit(uint256 index) public {
        ...
        // uint256 minamount = user.deposits[index].amount.mul(plans[user.deposits[index].plan].percent).div(PERCENTS_DIVIDER);
        // require(amount > minamount, "10BNB: Lower than min");
