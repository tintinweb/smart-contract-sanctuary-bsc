...
contract PresaleGenerator is Ownable {
    ...
    constructor() public {
        PRESALE_FACTORY = IPresaleFactory(0xb2a57F476b804D74Ea58f206C2dF492Ad055010B);
        PRESALE_SETTINGS = IPresaleSettings(0xD471b1e6b7E98B6809F6381588c55dD1baC7922a);
    }
...
interface IPresaleSettings {
    ...
    function getCommandersRoundLength () external view returns (uint256);
    function getPilotRoundLength () external view returns (uint256);
    function getSpecialistRoundLength () external view returns (uint256);
    function getRound4Length () external view returns (uint256);
...
contract Presale is ReentrancyGuard {
  ...
  struct PresaleStatus {
    ...
    uint256 COMMANDERS_ROUND_LENGTH; // length of round commanders in blocks
    uint256 PILOT_ROUND_LENGTH; // length of round pilot in blocks
    uint256 SPECIALIST_ROUND_LENGTH; // length of round mission specialist in blocks
    uint256 ROUND4_LENGTH; // length of round 4 in blocks
  ...
  mapping (address => uint256) private WHITELIST_LEVEL;

  constructor(address _presaleGenerator) public {
    PRESALE_GENERATOR = _presaleGenerator;
    MOON_FORCE_FACTORY = IMoonForceSwapFactory(0x0D5094b86922a80a98B93293D21c0cE34390f965);
    WETH = IWETH(0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F);
    PRESALE_SETTINGS = IPresaleSettings(0xD471b1e6b7E98B6809F6381588c55dD1baC7922a);
    PRESALE_LOCK_FORWARDER = IPresaleLockForwarder(0x97328C0F780fbda430eD0511f156B22d541B9D43);
    MOON_FORCE_FEE_ADDRESS = 0x2E241fF5bee46DBf7E3b984D15388989b912F2A6;
  }
  ...
  function init2 (
    ...
    address payable _referralAddress
      ...
      STATUS.COMMANDERS_ROUND_LENGTH = PRESALE_SETTINGS.getCommandersRoundLength();
      STATUS.PILOT_ROUND_LENGTH = PRESALE_SETTINGS.getPilotRoundLength();
      STATUS.SPECIALIST_ROUND_LENGTH = PRESALE_SETTINGS.getSpecialistRoundLength();
      STATUS.ROUND4_LENGTH = PRESALE_SETTINGS.getRound4Length();
  }
  ...
  function presaleStatus () public view returns (uint256) {
    ...
    if ((block.number >= PRESALE_INFO.START_BLOCK - STATUS.COMMANDERS_ROUND_LENGTH) && (block.number <= PRESALE_INFO.END_BLOCK)) {
      return 1; // ACTIVE - deposits enabled
    }
    return 0; // QUED - awaiting start block
  }

  function checkPresaleRound() internal {
    if (block.number >= PRESALE_INFO.START_BLOCK - STATUS.COMMANDERS_ROUND_LENGTH) { // 3 hour
      require(WHITELIST.contains(msg.sender) && WHITELIST_LEVEL[msg.sender] == 1, 'MoonForce: ALLOWED ONLY FOR COMMANDERS');
    }

    if (block.number >= PRESALE_INFO.START_BLOCK - STATUS.PILOT_ROUND_LENGTH) { // 2 hour
      require(WHITELIST.contains(msg.sender) && WHITELIST_LEVEL[msg.sender] == 2, 'MoonForce: ALLOWED ONLY FOR PILOTS');
    }

    if (block.number >= PRESALE_INFO.START_BLOCK - STATUS.SPECIALIST_ROUND_LENGTH) { // 1 hour
      require(WHITELIST.contains(msg.sender) && WHITELIST_LEVEL[msg.sender] == 3, 'MoonForce: ALLOWED ONLY FOR MISSION SPECIALISTS');
    }

    if (block.number >= PRESALE_INFO.START_BLOCK - STATUS.ROUND4_LENGTH) { // 30 minutes
      require(WHITELIST.contains(msg.sender) && WHITELIST_LEVEL[msg.sender] == 4, 'MoonForce: ALLOWED ONLY FOR WHITELISTED USERS');
    }
  ...
  // if something goes wrong in LP generation
  function forceFailByOwner () external {
      require(msg.sender == PRESALE_INFO.PRESALE_OWNER && presaleStatus() == 0);
      STATUS.FORCE_FAILED = true;
  }
  ...
  // editable at any stage of the presale
  function editWhitelist(address[] memory _users, uint256 _levels, bool _add) external onlyPresaleOwner {
    if (_add) {
        for (uint i = 0; i < _users.length; i++) {
          WHITELIST.add(_users[i]);
          WHITELIST_LEVEL[_users[i]] = _levels;
        }
    } else {
        for (uint i = 0; i < _users.length; i++) {
          WHITELIST.remove(_users[i]);
          WHITELIST_LEVEL[_users[i]] = 0;
        }
    }
  }
  ...
  function getUserWhitelistLevel (address _user) external view returns (uint256) {
    return WHITELIST_LEVEL[_user];
  }
}