...
contract POOL1155MKPVBG is ERC1155Holder,Ownable {
    address[] public supportedNFTList;
    mapping(address => bool) public supportedNFTMapping;
    event NFTSupported(address nft, bool val);
    event NFTSetApprovalForAllPool(address nft, address operator, bool val);
    event NFTSendNFTFromPool(address nft, address from, address to, uint256 tokenId, uint256 amount);
    ...
    */
    function clearToken(address recipient, address token, uint256 amount ) public onlyOwner {
        ...
        require(_token.balanceOf(address(this)) >= amount , "INVALID balance");
    ...
    */
    ...
    function setApprovalForAllPool(address _nft,address _operator, bool val ) external onlyOwner onlySupportedNFT(_nft) {
        IERC1155(_nft).setApprovalForAll(_operator , val);
        emit NFTSetApprovalForAllPool(_nft,_operator,val);
    }

    function sendNFTFromPool(
        address _nft,
        address _to,
        uint256 _tokenId,
        uint256 _amount
    ) public onlyOwner onlySupportedNFT(_nft) {
        ...
        emit NFTSendNFTFromPool(_nft,  address(this),_to,_tokenId,_amount);
    }
}
