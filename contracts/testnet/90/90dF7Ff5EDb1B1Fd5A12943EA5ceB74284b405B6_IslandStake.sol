...
contract IslandStake is OwnableUpgradeable {
    ...
    function unStake(uint id, uint idx, uint amount) external {
        ...
        if (block.timestamp - stakeInfo.stakeTime >= 1 minutes) {
    ...
    function claim(uint islandId, uint index, bool check) internal {
        ...
        if (check) {
            require(block.timestamp - info.claimTime >= 1 minutes, "time limit");
        }else if (block.timestamp - info.claimTime < 1 minutes) {
            return;
        }
