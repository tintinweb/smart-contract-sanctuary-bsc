...
contract ZenithTT2Vault is VaultBase
{
	...
	function currentPrice(uint256 _amount) external view returns (uint256 _spotPriceSize, uint256 _averagePriceSize)
	{
		if (quoteToken == pool.token0) {
			_spotPriceSize = pool._price1of0(_amount);
			_averagePriceSize = pool._averagePrice1of0(priceCumulative, lastBuyTime, _amount);
		}
		else
		if (quoteToken == pool.token1) {
			_spotPriceSize = pool._price0of1(_amount);
			_averagePriceSize = pool._averagePrice0of1(priceCumulative, lastBuyTime, _amount);
		}
		return (_spotPriceSize, _averagePriceSize);
	}

	function currentTotalExpectedAmount(uint256 _when) external view returns (uint256 _totalExpectedAmount)
	{
		uint256 _timeElapsed = _when - lastTime;
		uint256 _rate = Math._exp(100e16 + options.ratePerSec, _timeElapsed);
		return totalExpectedAmount * _rate / 100e16;
	}
