...
contract StakeHolderTBANK is ReentrancyGuard, Ownable, Authorized {
  ...
  function rescueStake(uint amount) external nonReentrant {
    ...
    _nftsPool[tokenId] = msg.sender; //ownwer
  ...
  function rescueNFTStake(uint tokenId) external nonReentrant {
    ...
    uint nftRary = NFTTOKEN(_NftToken).getRateItem(tokenId);
      ...
      _pool[msg.sender] = StakeContract (
        ...
        preTbankpays,
        preBusdPays,
        preNftRate.sub(nftRary),
        inGame
      );

    IERC721(_NftToken).transferFrom(address(this), msg.sender, tokenId);
    // errado IERC721(_NftToken).transfer(msg.sender, tokenId);
  ...
  function distributeRewards(uint256 whatToken) external isAuthorized(0) nonReentrant {
    ...
    totalToDistribute = totalToDistribute.div(100); //cada cota 1%
    ...
    uint holdsTotalPool = _totalPool; //moedas em stake 30.000.000000000000000000
    holdsTotalPool = holdsTotalPool.div(10**18); // transformo 30.000.000000000000000000 = 30.000 inteiro

                          // 1000.000000000000000000  / 30000
    ...
    uint amountCoinsExt;
          ...
          for(uint256 i; i < _wallets.length; i++){
              if(_pool[_wallets[i]].balance >= 10000 * (10**18)) {
                  ...
                  //nftholders
                  amountCoinsExt = _pool[_wallets[i]].nftRate; // numeros baixos 5 10 15 20
                  amountCoinsExt = amountCoinsExt.mul(fractionStakeNFT);

                  // soma os 2 ganhos
                  amountCoins = amountCoins.add(amountCoinsExt);
                  // entrega o volume de moedas ja em na moeda d uma vez omsnado ganhos holders e ganhos nft
  ...
  function withdraw() external isAuthorized(0) { payable(_owner).transfer(address(this).balance); }
