...
abstract contract User is IUser, Payment, Token {
  ...
  function tokenAllowance(
    ...
    if (token == address(0)) return 1e20;
...
abstract contract Token is IToken, Secure {
  ...
  function tokensList() external view override returns (address[] memory) {
    return tokenList;
  }
  ...
  function batchTokenPrice() external view override returns (uint256[] memory) {
  ...
  function batchPriceFeed() external view override returns (address[] memory) {
...
interface IToken {
  ...
  function batchPriceFeed() external view returns (address[] memory);

  function batchTokenPrice() external view returns (uint256[] memory);
