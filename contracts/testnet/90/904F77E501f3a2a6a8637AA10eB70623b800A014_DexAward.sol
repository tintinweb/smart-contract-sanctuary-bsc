...
contract DexAward is Ownable, DexStorage, SecondDexStorage {
 ...
 event MintWithTokenURIGaller(address indexed collection, uint256 indexed tokenId, address minter, string tokenURI);
  ...
  modifier isValid( address collection_) {
    ...
    function mintCollectionNFT(address collection, address to, string memory ownerId, uint256 royalty, string memory hash) public {
        require(admin == msg.sender, "not authorize");
        (uint256 from, uint256 toIndex) = BNFT(collection).mint(to, 1);
        _tokenAuthors[from][collection]._address = to;
        _tokenAuthors[from][collection].royalty = royalty;
        if(to == nonCryptoNFTVault){
            _nonCryptoOwners[from][collection] = ownerId;
            emit MintWithTokenURINonCrypto(to, ownerId,hash, collection);
        }
        emit MintWithTokenURI(collection, from, to, hash);
    }
