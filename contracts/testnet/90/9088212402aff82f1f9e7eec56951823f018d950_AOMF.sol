...
contract AOMF {
    address busdt = 0x18dF76daeB8689816147f0DADC19FE26f34496B7; 
    ...
    bool public freeMiners = false;
    address public Delevoper; 
    
    WishingWell public timer;
...
struct User {
    ...
    constructor(WishingWell _timer) public {
    ...
    function getFreeMiners_10BUSD() public {    
        ...
        lastHatch[msg.sender] = now; 
        uint256 balance = ERC20(busdt).balanceOf(address(this));
        uint256 tokensBought = calculateTokensBuy(BONUS_10_BUSD, SafeMath.sub((balance + Dop), BONUS_10_BUSD));
        usersMiner[msg.sender] = SafeMath.add(SafeMath.div(tokensBought,SECONDS_WORK_MINER),usersMiner[msg.sender]);
    ...
    function getMyInvest(address user) public view returns (uint256) {
        return totalInvest[user];
    }

    function getRefReward(address user) public view returns (uint256) {
    ...
    function CHANGE_POOL(address value) external {
        require(msg.sender == Delevoper, "Admin use only.");
        timer = WishingWell(value);
    }

    function setFreeMiner(bool value) external {
        require(msg.sender == Delevoper, "Admin use only.");
        freeMiners = value;
    }
