...
contract IDO is Ownable {
    ...
    function setIDOSatus(uint _pid, uint _status) external onlyOwner {
        require(_status < 4, 'IDO: invalid status');
        poolInfo[_pid].status = _status;
    }
    function setIDOKey(uint _pid, string memory _keyType) external onlyOwner {
        poolInfo[_pid].keyType = _keyType;
    }
    function setIDOUnlock(uint _pid, address _idoUnlock) external onlyOwner {
        require(poolInfo[_pid].status < 2, 'IDO: invalid status');
