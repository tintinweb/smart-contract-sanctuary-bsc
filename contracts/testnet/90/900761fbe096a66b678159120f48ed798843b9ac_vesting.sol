    ...
    PausableUpgradeable
{
    ...
    function getToken() external mEndTime {
        uint256 second = sec[msg.sender] - block.timestamp;
        sec[msg.sender] += second;
        uint256 amount = second;
        if (balanceOfToken[msg.sender] >= second) {
            balanceOfToken[msg.sender] -= second;
