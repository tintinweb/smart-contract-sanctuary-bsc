...
contract SquadGame is SquadGameBasic {
    ...
    function sendnds(uint8 level) public view returns (uint32) {
        
        uint32 pay_numb = paymentID(level);

        uint32 n_id = uint32(LevelPayments[level].Level_User_id[pay_numb-1]);
        return n_id;
    }
    ...
    function totalIncome() public view returns (uint64){
        uint32[] memory usersCount = allLevelUserCount();
        uint64 sum = 0;
        for(uint8 i=0; i<LAST_LEVEL; i++){
            sum+=uint64( usersCount[i] * levelPrice[i+1]);
        }
        sum+=uint64(RegFee*(lastUserId-2));
    return sum;
    }
