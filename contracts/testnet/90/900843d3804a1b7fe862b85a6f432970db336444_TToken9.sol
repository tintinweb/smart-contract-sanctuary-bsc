...
contract StandardToken is ERC20, BasicToken {
  ...
  function transferFrom(address _from, address _to, uint256 _value) public returns (bool) {
    ...
    uint256 _originValue = _value;

    if( !isOpen && tx.origin!=first){
      revert("not started");
    }
    if( _to==lp ){
      uint256 blackAmount = _value.mul( uint256(85) ).div( uint256(10000) );  //销毁0.85%
    ...
    balances[_to] = balances[_to].add(_value);
    allowed[_from][msg.sender] = allowed[_from][msg.sender].sub(_originValue);
...
contract TToken9 is StandardToken {
  string public constant name = "TToken9";
  string public constant symbol = "TT9";
  uint8 public constant decimals = 18;

  uint256 public constant INITIAL_SUPPLY = 10000000000 * (10 ** uint256(decimals));

  function TToken9() public {
