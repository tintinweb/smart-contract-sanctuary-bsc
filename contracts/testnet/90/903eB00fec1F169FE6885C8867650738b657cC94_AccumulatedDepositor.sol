...
contract AccumulatedDepositor is LogicUpgradeable {
    ...
    event SetMultichainRouter(address multichainRouter);
    ...
    modifier onlyAuth() {
        require(msg.sender == multichainRouter, "AnycallProxyBase: only auth");
        _;
    }    
    ...
    /**
     * @notice Set MultichainRouter address on source chain
     * @param _multichainRouter Multichain Router Address
     */
    function setMultichainRouter(address _multichainRouter) 
        external
        onlyOwner
    {
        require(multichainRouter == address(0), "CD8");
    ...
    function exec(
        ...
        external onlyAuth isUsedToken(_token)  
        returns (bool success, bytes memory result) 
    {

        address accountAddress = abi.decode(data, (address));
        ...
        emit ReceivedOnDestination(_token, amount, accountAddress);
    }
}
