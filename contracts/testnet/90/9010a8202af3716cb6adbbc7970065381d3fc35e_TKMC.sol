...
contract TKMC is ERC721, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 _amount, uint8 _tokenCount) external payable callerIsUser {
        if (!publicSaleActive) revert PublicSaleNotActive();
        if (tokenCount + _tokenCount > currentSupply) revert MaxSupplyExceeded();
        if (addressMintedBalance[msg.sender] + _tokenCount > maxMintAmount)
            revert MaxPerWalletExceeded();
        if (_amount != publicCost * _tokenCount) revert InsufficientValue();

        for (uint256 i = 1; i <= _tokenCount; i++) {
            addressMintedBalance[msg.sender]++;
            _safeMint(msg.sender, ++tokenCount);
        }
        uint256 cost = publicCost * _tokenCount;
        USDTR.transferFrom(msg.sender, address(this), cost);
    }
