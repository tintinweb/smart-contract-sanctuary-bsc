...
contract GymMLM is OwnableUpgradeable {
    ...
    address public turnoverPoolAddress;
    ...
    function setTurnoverPoolAddress(address _address) external onlyOwner {
        turnoverPoolAddress = _address;
    }

    function setLevelPoolAddress(address _address) external onlyOwner {
        levelPoolAddress = _address;
    }
    ...
    function distributeRewards(
        ...
        _distributeRewards(token, _distributionRewards);
        ...
        while (index < directReferralBonuses.length && addressToId[userToReferrer[_user]] != 1) {
            ...
            if (index <= _level && hasInvestment[referrer]) {
                ...
                if (
                    _borrowedAmount > 0 && _wantAddr == 0x3a0d9d7764FAE860A659eb96A500F1323b411e68
                ) {
    ...
    /**
     * @notice  Function to distribure rewards
     * @param _amount: amount
     */
    function _distributeRewards(IERC20 _token, uint256 _amount) private { 
        if (turnoverPoolAddress != address(0)) {  
            require(
                turnoverPoolAddress != address(0),
                "GymMLM:: Turnover Pool contract address is zero address"
            );
        }
        if (levelPoolAddress != address(0)) { 
            require(
                levelPoolAddress != address(0),
                "GymMLM:: Level Pool contract address is zero address"
            );
        }

        require(_token.transfer(turnoverPoolAddress, _amount / 2), "GymMLM:: Transfer failed");
        require(_token.transfer(levelPoolAddress, _amount / 2), "GymMLM:: Transfer failed");
    }
