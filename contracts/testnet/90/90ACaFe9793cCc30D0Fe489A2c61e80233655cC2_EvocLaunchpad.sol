...
contract EvocLaunchpad is Ownable, ReentrancyGuard {
        ...
        if (isFeeExempt[contractAdd]) {
            require(
                ContractToken.transferFrom(
                    msg.sender,
                    item._contractAddReceive,
                    amount.add(amount.mul(item._bonusPercent).div(100))
                ),
        ...
        isPausable
    {
        ...
        uint256 newAmount = (oldBalance.div(item._tokenValueBNB)).add(tokenAmount.mul(item._bonusPercent).div(100));
        ...
        ContractToken.transfer(msg.sender, newAmount);

        emit Received(msg.sender, tokenAmount, newAmount, contractAdd);
    }
