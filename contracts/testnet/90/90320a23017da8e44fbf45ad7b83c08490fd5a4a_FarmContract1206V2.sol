...
contract FarmContract1206V2 is Initializable, OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(uint256 => mapping(address => uint256)) public userCalculatedReferralRewards;
    mapping(uint256 => mapping(address => uint256)) public userCalculatedNodeRewards;
    ...
    mapping(address => uint256) public userTotalReferralRewards;
    mapping(address => uint256) public userTotalNodeRewards;
    ...
    function pendingRefferalReward(uint256 _pid, address _user) public view validatePid(_pid) returns(uint256 totalMintedAmount){
        ...
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 mintedAmount = pendingMiningReward(_pid, myInviteeAddress[i]).add(mintAmount[_pid][_user]).sub(userCalculatedReferralRewards[i][_msgSender()]);
            totalMintedAmount = totalMintedAmount.add(mintedAmount);
        }
        return totalMintedAmount;
    }

    function pendingNodeReward(uint256 _pid, address _user) public view validatePid(_pid) returns(uint256 totalMintedAmount){
        address[] memory myInviteeAddress = IInviteContract(inviteContract).getInvitees(_user);
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 mintedAmount = pendingMiningReward(_pid, myInviteeAddress[i]).add(mintAmount[_pid][_user]).sub(userCalculatedNodeRewards[i][_msgSender()]);
            totalMintedAmount = totalMintedAmount.add(mintedAmount);
        }
        return totalMintedAmount;
    }
    ...
    function getAllPendingReferralReward(address _user) public view returns(uint256 totalReferralBonus){
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 referralBonus = pendingRefferalReward(i,_user).mul(getRefferalRewardPercent(i, _user)).div(rateDenominator);
    ...
    function getAllPendingNodeReward(address _user) public view returns(uint256 totalNodeBonus){
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 nodeBonus = pendingNodeReward(i,_user).mul(getNodeRewardPercent(i, _user)).div(rateDenominator);
    ...
    function claimAllReferralReward() public {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            uint256 refferalReward = pendingRefferalReward(i, _msgSender());
            refferalReward = refferalReward.sub(userCalculatedReferralRewards[i][_msgSender()]);
            userCalculatedReferralRewards[i][_msgSender()] = userCalculatedReferralRewards[i][_msgSender()].add(refferalReward);
        }
        safeMiningTokenTransfer(_msgSender(), totalPendingRefferalReward);
        userTotalReferralRewards[_msgSender()] = userTotalReferralRewards[_msgSender()].add(totalPendingRefferalReward);
    }
    ...
    function claimAllNodeReward() public {
        ...
        for (uint256 i = 0; i < poolInfo.length; i++) {
            ...
            nodeReward = nodeReward.sub(userCalculatedNodeRewards[i][_msgSender()]);
            userCalculatedNodeRewards[i][_msgSender()] = userCalculatedNodeRewards[i][_msgSender()].add(nodeReward);
        }
        safeMiningTokenTransfer(_msgSender(), totalPendingNodeReward);
        userTotalNodeRewards[_msgSender()] = userTotalNodeRewards[_msgSender()].add(totalPendingNodeReward);
    }
    ...
    function getOneStarRequiredLP(uint256 pid) public view returns(uint256) {
        ...
        uint256 amountIn = IPancakeRouter02(pancakeRouter02).getAmountIn(oneStarUserpledgeThreshold, _reserve0, _reserve1);
        return Math.sqrt(oneStarUserpledgeThreshold.mul(amountIn));
    }

    function getTwoStarRequiredLP(uint256 pid) public view returns(uint256) {
        address pair = address(poolInfo[pid].lpToken);
        (uint112 _reserve0, uint112 _reserve1, ) = IPancakePair(pair).getReserves();
        uint256 amountIn = IPancakeRouter02(pancakeRouter02).getAmountIn(twoStarUserpledgeThreshold, _reserve0, _reserve1);
        return Math.sqrt(twoStarUserpledgeThreshold.mul(amountIn));
    }

    function getThreeStarRequiredLP(uint256 pid) public view returns(uint256) {
        address pair = address(poolInfo[pid].lpToken);
        (uint112 _reserve0, uint112 _reserve1, ) = IPancakePair(pair).getReserves();
        uint256 amountIn = IPancakeRouter02(pancakeRouter02).getAmountIn(threeStarUserpledgeThreshold, _reserve0, _reserve1);
        return Math.sqrt(threeStarUserpledgeThreshold.mul(amountIn));
    }

    function getLevel(uint256 pid, address user) public view returns(uint256) {
        if(depositAmount[pid][user] >= getThreeStarRequiredLP(pid)) {
            return 3;
        } else if(depositAmount[pid][user] >= getTwoStarRequiredLP(pid)) {
            return 2;
        } else if(depositAmount[pid][user] >= getOneStarRequiredLP(pid)) {
            return 1;
        } else {
            return 0;
        } 
