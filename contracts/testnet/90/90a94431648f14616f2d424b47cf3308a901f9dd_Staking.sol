...
contract Staking {
    ...
    function getAllStakesByAddress(address _address) public view returns(uint256[] memory, uint256[] memory, uint256[] memory, bool[] memory){
        uint256[] memory a;
        uint256[] memory b;
        uint256[] memory c;
        bool[] memory d;
        ...
        for(uint256 i=1; i <= numberOfStakesByAdd[_address]; i ++){
             stakeIdNow = allStakeIdByAddress[_address][i];
            a[i-1] = stakeIdNow;
            b[i-1] = amountByStakeId[stakeIdNow];
            c[i-1] = timeByStakeId[stakeIdNow];
            d[i-1] = flagByStakeId[stakeIdNow];
        }
        return (a,b,c,d);
    }
}
