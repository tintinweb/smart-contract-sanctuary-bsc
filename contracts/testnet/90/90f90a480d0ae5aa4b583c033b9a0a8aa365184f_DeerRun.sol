...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public xmasDate = 1668229217; //1671997113;
    ...
    function deposit() external payable override onlyToken {
        ...
        if (shouldDistributeXmas()) {
            ...
            dividendsPerShare = dividendsPerShare.add(
                dividendsPerShareAccuracyFactor.mul(totalDividends).div(
                    totalShares
                )
            );
        } else {
            ...
            dividendsPerShareXmas = dividendsPerShare.add(
                dividendsPerShareAccuracyFactor.mul(totalXmasDividends).div(
                    totalShares
                )
            );
        }
    ...
    function distributeXmaxDividend(address shareholder) internal {
        ...
        uint256 xmaxAmount = getCumulativeXmasDividends(
            shares[shareholder].amountForXmas
        );
        if (xmaxAmount > 0) {
        ...
        returns (uint256)
    {
        ...
        uint256 shareholderTotalXmasDividends = getCumulativeXmasDividends(
            shares[shareholder].amountForXmas
        );
        uint256 shareholderTotalExcluded = shares[shareholder].totalExcluded;

        if (shouldDistributeXmas())
            shareholderTotalDividends += shareholderTotalXmasDividends;
    ...
    function getUnpaidXmasEarnings(address shareholder)
        public
        view
        returns (uint256)
    {
    ...
    function getCumulativeXmasDividends(uint256 share)
        internal
        view
        returns (uint256)
    {
        return
            share.mul(dividendsPerShareXmas).div(
