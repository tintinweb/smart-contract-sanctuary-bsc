...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
        ...
        notZeroAddress
    {
        ...
        uint256 purchasePrice = _mintingSignature.parcels.length * 100 * 1e18;//_getPriceForParcels(_mintingSignature.parcels);
    ...
    function applyVoucher(uint256 numRequestedVouchers, uint256[] memory minerIds) external notZeroAddress {
        for(uint256 i = 0; i < minerIds.length; ++i) {
            ...
            IMining(miningAddress).applyVoucher(msg.sender,minerId, numRequestedVouchers);
    ...
    function _mintMiners(uint256 _count) private returns(uint256, uint256) {
...
interface IMining {
    ...
    function applyVoucher(address _user,uint256 _miner, uint256 _vouchers) external;
