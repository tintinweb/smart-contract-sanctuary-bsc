...
contract TestNFT is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    constructor() {
        aggregatorContract = IAggregator(address(0x143db3CEEfbdfe5631aDD3E50f7614B6ba708BA7));
    }
    ...
    function mint(uint256 numberOfMints) payable external {
        ...
        uint cost = (costUSD * numberOfMints) / (convertedPrice / 1e8);
    ...
    function getCurrentCost() external view returns (uint256) {
        int256 currentPrice = aggregatorContract.latestAnswer();
        uint256 convertedPrice = uint(currentPrice);
        return costUSD / (convertedPrice / 1e8) * 1e18;
    }
