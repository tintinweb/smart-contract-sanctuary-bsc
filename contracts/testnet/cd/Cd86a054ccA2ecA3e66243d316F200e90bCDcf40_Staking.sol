...
Owner – the account permitted to perform administrative functions on the contract. At this stage no administrative functions have been specified. 
*/

contract Staking is Ownable {
    address public immutable token;
    uint32 public immutable maturationDays;
    uint32 public immutable startingBurnRate;
    uint32 public immutable rewardRate;
    ...
    struct Stake {
        ...
        bool exists; // Used to test existence of stake
    }
    ...
    */
    ...
    function depositStake(uint256 amount) public {
        ...
        Stake memory stake = Stake(amount, Math.mulDiv(amount, rewardRate, PRECISION * 100), address(msg.sender), block.timestamp + (maturationDays * DAY), 0, 0, true);
        // Add the new Stake to the stakes array
        stakes[address(msg.sender)].push(stake);
    }

    // Withdraw all rewards due
    function claimReward(uint256 index) public {
        ...
        uint256 rewardAmount = rewardDueNow - stake.totalRewardClaimed;
        if (rewardAmount > 0) {
    ...
    function withdrawStake(uint256 index) public {
        ...
        // Set the withdrawn flag to true
        ...
        IERC20(token).transfer(stake.staker, stake.amount - burnAmount);

        emit StakeWithdrawn(stake.staker, index, stake.amount, burnAmount);
    }
}
