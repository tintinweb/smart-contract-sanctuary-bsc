...
contract MasterChef is Ownable {
    ...
    // Bonus muliplier for early reward markers.
    ...
    function safeRewardTokenTransfer(address _to, uint256 _amount) internal {
        rewardToken.safeTransferFrom(rewardAddr, _to, _amount);
    }

    function updateRewardToken(IERC20 _rewardToken) public onlyOwner {
        rewardToken = _rewardToken;
    }

    // Update dev address by the previous dev.
    function updateRewardAddr(address _rewardAddr) public onlyOwner{
        rewardAddr = _rewardAddr;
    }
}