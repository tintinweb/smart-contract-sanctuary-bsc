...
contract Kepler22bToken is ERC20Burnable, Ownable, IERC721Receiver, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.UintSet; 
    
    //addresses 
    address nullAddress = 0x0000000000000000000000000000000000000000;
    address public keplerFarmAddress;

    //uint256's 
    uint256 public expiration; 
    //rate governs how often you receive $KEPLER
    uint256 public rate; 
    ...
    constructor(
      ...
      uint256 _rate,
      uint256 _expiration
    ) ERC20("Kepler22b", "KEPLER") 
    {
        keplerFarmAddress = _keplerFarm; 
        rate = _rate;
  ...
  /* Tokens */
    ...
    function claimRewards(uint256[] calldata tokenIds) public {
      uint256 reward; 
      uint256 _block = Math.min(block.number, expiration);

      uint256[] memory rewards = calculateRewards(msg.sender, tokenIds);

      for (uint256 i; i < tokenIds.length; i++) {
        reward += rewards[i];
        _depositBlocks[msg.sender][tokenIds[i]] = _block;
      }
    ...
    function deposit(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            IERC721(keplerFarmAddress).safeTransferFrom(
                msg.sender,
                address(this),
                tokenIds[i],
                ''
