...
/**
 * @title XOracle.sol
 * @dev Th updater is responsible to provide interfaces and methods
 * for its owner to update asset balances and unrealized pnl
 * @author Lawson Cheng
 */
contract XOracle is SignatureWriteAccessController, XAssetInterface, XOracleReadableInterface {
  ...
  */
  function updateAssets(
    ...
    require(assets.length == balances.length && balances.length == assetTypes.length, "Unequal array lengths");

    // do update one by one
    for(uint8 i = 0; i < assets.length; i++) {
      require(checkAssetExistence(assets[i], assetTypes[i]), "Asset does not exists");
  ...
  ************************************/
  ...
  function getRecordsAtIndex(uint256 _index) external view override returns (BatchAssetRecord memory) {
    AssetDetailRecord[] memory records = new AssetDetailRecord[](xAssetList.length);
    uint256 blockNumber;
    for(uint256 i = 0; i < xAssetList.length; i++) {
      (AssetDetailRecord memory _record, uint256 _blockNumber) = xAssets[xAssetList[i]].getRecordAtIndex(_index);
      if(i == 0) {
        blockNumber = _blockNumber;
      } else {
        require(blockNumber == _blockNumber, "BlockNumber does not consist");
      }
      records[i] = _record;
    }
  ...
  */
  function addAsset(
    ...
    require(!checkAssetExistence(tokenAddress, assetType), "Asset already exists");
  ...
  */
  function removeAsset(
    ...
    require(checkAssetExistence(tokenAddress, assetType), "Asset does not exists");
    address assetKey = getKey(tokenAddress, assetType);
    delete xAssets[assetKey];
    // remove it from list
    for(uint192 i = 0; i < xAssetList.length; i++) {
      // found asset at index i
      if(assetKey == xAssetList[i]) {
        // remove asset key
        delete xAssetList[i];
        // shift all element towards left to remove the gap
        for(uint192 j = i; j < xAssetList.length; i++) {
  ...
  */
  ...
  )
  {
    ...
    (
      ...
      message_,
      (uint64, uint256, address[], int192[], AssetType[])
    );
...
interface XAssetReadWriteInterface is XAssetInterface {
  ...
  function getRecordAtIndex(uint256 index) external view returns (AssetDetailRecord memory, uint256);

  function update(uint256 timestamp, int192 newPrice, uint64 _observationTimestamp) external;
