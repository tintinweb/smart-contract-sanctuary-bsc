...
contract LevelStake {
	...
	function setComissionWallet(address payable _address) public onlyAdmin{
		commissionWallet = _address;
	}
  ...
  function updatePlanStatus(uint256 _planId,bool _active) public onlyAdmin {
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 _plan,uint256 planDetailId, uint256 percent, uint256 amount, bool isWithdraw, uint256 start, uint256 finish) {
	    User storage user = users[userAddress];
		planInformation storage planDetails = planInfo[user.deposits[index].plan];
