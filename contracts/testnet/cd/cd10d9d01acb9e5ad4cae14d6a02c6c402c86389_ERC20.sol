...
contract Bep20Test3 is ERC20, Ownable {
    ...
    uint256 public devFee = 4; // dev fee
    address public  devWallet = 0xDC8074b7323eb5119F959697B4C8CFa316a135CC; // dev wallet address
    ...
    constructor() ERC20("TestToken3", "TT3", 9) {
    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xdc4904b5f716Ff30d8495e35dC99c109bb5eCf81); // pancakeswap v2 router address
        ...
        */
        _createInitialSupply(owner(), 4700000000 * (10**9));
    }
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        ...
        require(_isExcludedFromFees[account] != excluded, "TestToken3: Account is already the value of 'excluded'");
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFromFees(account, excluded);
    }

    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "TestToken3: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "TestToken3: Automated market maker pair is already set to that value");
    ...
    // owner can only decrease tax, he has no right to increase tax more than 25%
    function setFee(uint256 _devFee) public onlyOwner {
        require(_devFee <= 25, "fees must be less than or equal to 25%");
        devFee = _devFee;
    }
    ...
    function setMaxTxAmount(uint256 newAmount) public onlyOwner {
        maxTxAmount = newAmount * 10**9;
        require(newAmount * 10**9 >= totalSupply().div(50000), "transaction amount must be greater than or equal to 0.01% of supply");
    }

     // enable cooldown between trades
    function changeCooldownSettings(bool newStatus, uint256 newInterval) external onlyOwner {
        require(newInterval < 10 seconds , "Allow maximum coolDown interval 10 seconds!!");
