...
contract Crowdsale is Ownable {
    ...
    function claimStakedInterest() external {
        ...
        for(uint i=0; i< _length; i++) {
            ...
            if(stakedData[msg.sender][i].claimTimestamp == 0) {
                eachStakedInt = calculateInterestAmount(stakedData[msg.sender][i].stakedAmount, stakedData[msg.sender][i].stakedTimestamp);
            } else {
                eachStakedInt = calculateInterestAmount(stakedData[msg.sender][i].stakedAmount, stakedData[msg.sender][i].claimTimestamp);
            }
     ...
     * @param _stakedTimestamp Investment timestamp
     */
    function calculateInterestAmount(uint _stakedAmount, uint _stakedTimestamp) public view returns(uint) {
        uint timeDiffinSecs = block.timestamp - _stakedTimestamp;
