...
contract PlearnNFTCollectionHandler is ICollectionHandler, Ownable {
    ...
    modifier whenCollectionListed(IERC721 collection) {
        require(_collectionIsListed(collection), "Marketplace: Collection not listed");
        _;
    }
    ...
    function addCollection(
        ...
        emit AddOrUpdateCollection(collection, creator, creatorFee, tradingFee, auctionEnable);
    }
    ...
    function updateCollection(
        ...
        uint256 tradingFee,
        bool auctionEnable
    ) external onlyOwner whenCollectionListed(collection) {
        require(
            (creatorFee == 0 && creator == address(0)) || (creatorFee != 0 && creator != address(0)),
            "Marketplace: Creator parameters incorrect"
        );
     ...
     * @dev Callable by admin
     */
    function removeCollection(IERC721 collection) external onlyOwner whenCollectionListed(collection) {
    ...
    function auctionEnabled(IERC721 collection) external view override returns (bool) {
        return collections[collection].auctionEnable;
    }

    function collectionListed(IERC721 collection) external view override returns (bool) {
        return _collectionIsListed(collection);
    }

    function collectionOpened(IERC721 collection) external view override returns (bool) {
        return _collectionIsListed(collection) && _collectionIsOpened(collection);
    }
    ...
    {
        return (collections[collection].tradingFee, collections[collection].creatorFee);
    }
    ...
    function _collectionIsListed(IERC721 collection) internal view returns (bool) {
        return collections[collection].status != Status.Pending;
    }

    function _collectionIsOpened(IERC721 collection) internal view returns (bool) {
