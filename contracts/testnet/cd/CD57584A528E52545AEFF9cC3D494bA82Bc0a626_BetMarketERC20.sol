...
contract BetMarketERC20 {
    ...
    IERC20 public token;
    uint256 public devTax = 500;
    uint256 constant public TAX_DIVISOR = 10000;
    address public devAddress = 0xb40eA1DA6A55bDf2b5BFbd054cC7dDB53F8ac615;
        ...
        public
    {
        ...
        Markets storage market = markets[_marketId];
        require(market.eventCompleted == false, "Event already completed");
        if (eventOutcome) {
            ...
            for (uint256 i = 0; i < winningAddresses.length; i++) {
                ...
                token.transfer(_address, winningAmount[_address] * (TAX_DIVISOR - devTax) / TAX_DIVISOR);
                taxAmount += winningAmount[_address] * devTax / TAX_DIVISOR;
                delete winningAmount[_address];
            }
            if(taxAmount > 0) {
                token.transfer(devAddress, taxAmount);
            }
