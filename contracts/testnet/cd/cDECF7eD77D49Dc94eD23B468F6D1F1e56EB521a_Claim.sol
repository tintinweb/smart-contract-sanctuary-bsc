...
contract Claim is AccessControl {
    ...
    constructor(address minter, address _tokenHeroes, uint256 _startTime){
    ...
    function claim() external {
        ...
        safeHETransfer(msg.sender, amount);
        emit ClaimToken(
            msg.sender,
            amount,
            block.timestamp
    ...
    function getAvaiableAmount(address _member) external view returns( uint256 ){
        return totalAmount[address(_member)].sub(withdrawnAmount[address(_member)]);
    }
