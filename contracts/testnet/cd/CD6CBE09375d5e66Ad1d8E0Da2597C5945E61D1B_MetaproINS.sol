...
contract MetaproINS is Ownable, ERC1155Holder, ReentrancyGuard {
    ...
    mapping(uint256 => INSConfiguration[]) public tokenIns;
    ...
    function deposit(
        ...
        INSConfiguration memory configuration = availableIns[insId];
    ...
    function withdraw(uint256 insId, bytes memory _data) external nonReentrant {
        ...
        uint256[] memory createdArray = createdInsIds;
        ...
        bool allInsForTokenIdFinalized = false;

        for (uint256 i = 0; i < tokenConfiguration.length; i++) {
            if (!tokenConfiguration[i].valid) {
                allInsForTokenIdFinalized = true;
            }
        }

        if (allInsForTokenIdFinalized) {
            for (uint256 i = 0; i < createdInsIds.length; i++) {
                if (createdInsIds[i] == configuration.tokenId) {
                    delete createdInsIds[i];
                }
            }
        }

        createdInsIds = createdArray;
        delete availableIns[insId];
