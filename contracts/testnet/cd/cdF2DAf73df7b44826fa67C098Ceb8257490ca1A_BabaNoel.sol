...
contract BabaNoel is IBEP20, Auth {
    ...
    string constant _name = "Noel BABA";
    string constant _symbol = "$NoelBABA";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 100_000_000_000_000 * (10**_decimals);

    uint256 public _maxWalletHoldingAmount = _totalSupply.mul(5).div(1000);
    ...
    function getTotalFee(bool selling) internal returns (uint256) {
        ...
        if (!selling) {
            totalFee = liquidityFee.add(MarketingFee).add(DonationFee);
            return totalFee;
        }
        return liquidityFee.add(MarketingFee).add(DonationFee);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBDonation = amountBNB.mul(sellDonationFee).div(
            totalBNBFee
        );
    ...
    function setBuyFees(
        ...
        uint256 _buyDonationFee,
        uint256 _feeDenominator
    ) external authorized {
        liquidityFee = _buyliquidityFee;
        MarketingFee = _buyMarketingFee;
        DonationFee = _buyDonationFee;
        feeDenominator = _feeDenominator;
        require(_buyDonationFee.add(_buyliquidityFee).add(_buyMarketingFee) <= feeDenominator.div(5),"You can't set more than 20%");
    ...
    function setSellFees(
        ...
        feeDenominator = _feeDenominator;
        require(_SellliquidityFee.add(_SellMarketingFee).add(_SellDonationFee) <= feeDenominator.div(5),"You can't set more than 20%");
