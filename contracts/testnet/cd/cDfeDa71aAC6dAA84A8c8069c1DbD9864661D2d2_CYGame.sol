...
contract CYGame is EIP712, ReentrancyGuard {
    string private constant names = "CY-Game";
    ...
    event ClaimRewards(address claimer, uint256 rewards);
    event Pausable(bool state);
    event UpdateOwner(address oldOwner, address newOwner);

    constructor(address _owner) EIP712(names, version) {
        owner = _owner;
    }
    ...
    function emergencyWithDrawFunds(uint256 _amount)
        external
        nonReentrant
        onlyOwner
    {
    ...
    function redeem(
        ...
        _transferETH(voucher.claimerAddress, voucher.rewardAmount);

        emit ClaimRewards(voucher.claimerAddress, voucher.rewardAmount);
    }
    ...
    function executeSetIfSignatureMatch(
        ...
        bytes32 hashStruct = keccak256(
            abi.encode(
                keccak256(
                    "Permit(address claimerAddress,uint256 rewardAmount)"
                ),
                voucher.claimerAddress,
