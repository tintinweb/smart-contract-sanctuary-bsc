...
contract GymVaultsStrategyAlpacaBUSD is Ownable, ReentrancyGuard, Pausable {
    ...
    function withdraw(address, uint256 _wantAmt) public onlyOwner nonReentrant returns (uint256) {
        ...
        if (isAutoComp && !strategyStopped) {
            ...
            if (
                vault.token() == IVaultConfig(0xbC6d2dfe97A557Bd793d07ebB0df3ea80cc990Fc).getWrappedNativeAddr()
                // address(this).balance > 0
            ) {
                IWETH(0x0266693F9Df932aD7dA8a9b44C2129Ce8a87E81f).deposit{value: _wantAmt}();
            }
        }
