...
interface ISafeswapFactory {
    ...
    function getTotalFee() view external returns (uint256, uint256);
}
...
library SafeswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'e0cfc87c73f95425639fbcc7b1160474a9da7feda49c17224ff9159b0de7373c' // init code hash
            ))));
    }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, address factory) internal view returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.mul(precision.sub(totalFee));
        uint numerator = amountInWithFee.mul(reserveOut);
        uint denominator = reserveIn.mul(precision).add(amountInWithFee);
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, address factory) internal view returns (uint amountIn) {
        ...
        require(reserveIn > 0 && reserveOut > 0, 'SafeswapLibrary: INSUFFICIENT_LIQUIDITY');
        (uint256 totalFee, uint256 precision) = getConfigFee(factory);
        uint numerator = reserveIn.mul(amountOut).mul(precision);
        uint denominator = reserveOut.sub(amountOut).mul(precision.sub(totalFee));
