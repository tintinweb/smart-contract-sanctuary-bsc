...
contract Solekah is Context, IBEP20, Ownable {
    ...
    constructor (address cOwner, address marketingWallet, address stakingWallet) Ownable(cOwner) {
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            (, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tStaking)  = _getTValues(tAmount);
            uint256 currentRate = _getRate();
            (uint256 rAmount,,) = _getRValues(tAmount, tFee, tLiquidity, tMarketing, tStaking, currentRate);
            ...
            (, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tStaking) = _getTValues(tAmount);
            uint256 currentRate = _getRate();
            (, uint256 rTransferAmount,) = _getRValues(tAmount, tFee, tLiquidity, tMarketing, tStaking, currentRate);
    ...
    function setTransferFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 stakingFee) external onlyOwner {
        require(
            taxFee + liquidityFee + marketingFee + stakingFee <= 15,
            "Fees must be less than 25%"
        );
        _taxFeeTransfer       = taxFee;
        _liquidityFeeTransfer = liquidityFee;
        _marketingFeeTransfer = marketingFee;
        _stakingFeeTransfer    = stakingFee;
    }
    function setBuyFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 stakingFee) external onlyOwner {
        require(
            taxFee + liquidityFee + marketingFee + stakingFee <= 15,
            "Fees must be less than 25%"
        );
        ...
        _stakingFeeBuy    = stakingFee;
    }
    function setSellFee(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 stakingFee) external onlyOwner {
        require(
            taxFee + liquidityFee + marketingFee + stakingFee <= 15,
            "Fees must be less than 25%"
        );
        ...
        _stakingFeeSell    = stakingFee;
    }
    function setStakingPercent(uint256 stakingPercent) external onlyOwner {
        _sellPercentageOfLiquidityForStaking = stakingPercent;
    }
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner {
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(25);
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        */
        ...
        uint256 bnbForStaking = newBalance.mul(_sellPercentageOfLiquidityForStaking).div(100);
        uint256 bnbForLiquidity = newBalance.sub(bnbForStaking);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        uint256 previousStakingFee    = _stakingFeeTransfer;
        ...
        if (!takeFee || isBuy || isSell) {
            ...
            _stakingFeeTransfer    = previousStakingFee;
        }
    }
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tStaking) = _getTValues(tAmount);
        uint256 currentRate = _getRate();
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidity, tMarketing, tStaking,currentRate);
        ...
        takeTransactionFee(sender, address(_stakingWallet), tStaking, currentRate);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tFee       = tAmount.mul(_taxFeeTransfer).div(0);
        uint256 tLiquidity = tAmount.mul(_liquidityFeeTransfer).div(100);
        uint256 tMarketing = tAmount.mul(_marketingFeeTransfer).div(200);
        uint256 tStaking    = tAmount.mul(_stakingFeeTransfer).div(100);
        uint256 tTransferAmount = tAmount.sub(tFee);
        tTransferAmount = tTransferAmount.sub(tLiquidity);
        tTransferAmount = tTransferAmount.sub(tMarketing);
        tTransferAmount = tTransferAmount.sub(tStaking);
        return (tTransferAmount, tFee, tLiquidity, tMarketing, tStaking);
    }
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint tStaking, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
