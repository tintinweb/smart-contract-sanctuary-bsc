...
contract INFLIV is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, UUPSUpgradeable {
    ...
    function initialize() initializer public {
        ...
        super._mint(msg.sender, 15000000 * (10**18)); // Partners & Investors Initial Supply
		super._mint(msg.sender, 1000 * (10**18)); // Bounty & Bonuses Initial Supply
		super._mint(msg.sender, 6000 * (10**18)); // Listings & Liquidity Initial Supply
		...
		listingsLiquidityPool.claimInterval = 15 minutes;
		...
		communityPool.claimInterval = 2 days;
	...
	function availablePartnersInvestorsPoolTokens() public view returns (uint256) {
	    if(partnersInvestorsPool.startTime >= block.timestamp){
	...
	function availableListingsLiquidityPoolTokens() public view returns (uint256) {
	    if(listingsLiquidityPool.startTime >= block.timestamp){
		
		    uint256 available = ((block.timestamp - listingsLiquidityPool.startTime) / listingsLiquidityPool.timeInterval) * listingsLiquidityPool.unlockAmount + 
			                    listingsLiquidityPool.supplyFromGovernance;
	...
	function availableBountyBonusesPoolTokens() public view returns (uint256) {
	    if(bountyBonusesPool.startTime >= block.timestamp)
		{
	...
	function availableTreasuryPoolTokens() public view returns (uint256) {
  	    uint256 available = (block.number - treasuryPool.startingBlock) * treasuryPool.supplyPerBlock + 
	                        treasuryPool.supplyFromGovernance;
	...
	function availableCommunityPoolTokens() public view returns (uint256) {
  	    uint256 available = (block.number - communityPool.startingBlock) * communityPool.supplyPerBlock + 
	                        communityPool.supplyFromGovernance;
	...
	function availableTeamPoolTokens() public view returns (uint256) {
  	    uint256 available = (block.number - teamPool.startingBlock) * teamPool.supplyPerBlock + 
	                        teamPool.supplyFromGovernance;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20Upgradeable){      
		...
		if(block.timestamp >= listingsLiquidityPool.lastClaimTime + listingsLiquidityPool.claimInterval && swapEnable && !swapping && automatedMarketMakerPairs[recipient]) {
			...
			if(LLPAmount > 0) 
			{  
			    super._mint(address(this), LLPAmount);
	...
	function _updatePools() public
	{
	   ...
	   if(block.timestamp >= (teamPool.lastClaimTime.add(teamPool.claimInterval)) && address(teamAddress) != address(0) && TAmount > 0) 
	   {
	      _approve(address(this), address(teamAddress), TAmount);
		  super._mint(address(this), TAmount);
	   ...
	   if(block.timestamp >= (communityPool.lastClaimTime.add(communityPool.claimInterval)) && address(stakingAddress) != address(0) && address(farmAddress) != address(0) && CPAmount > 0) 
	   {
		   uint256 stakingPoolShare = CPAmount * 20 / 100;
		   uint256 farmPoolShare = CPAmount - stakingPoolShare;
		   
           _approve(address(this), address(stakingAddress), stakingPoolShare);
		   _approve(address(this), address(farmAddress), farmPoolShare); 
		   
		   super._mint(address(this), CPAmount);
	    ...
	    {
		   _approve(address(this), address(insuranceAddress), IAmount);
	       super._mint(address(this), IAmount);
	    ...
	    { 
		   _approve(address(this), address(insuranceAddress), BAmount);
	       super._mint(address(this), BAmount);
	...
	function _distributeFee(uint256 amount) private {
	   ...
	   uint256 communityPoolShare = amount.mul(governanceFeeShare.communityPoolShare).div(10000);
	   uint256 liquidityPoolShare = amount.mul(governanceFeeShare.liquidityPoolShare).div(10000);
	   uint256 treasuryPoolShare = amount.mul(governanceFeeShare.treasuryPoolShare).div(10000);
	   uint256 bountyBonusesPoolShare = amount.mul(governanceFeeShare.bountyBonusesPool).div(10000);
	   uint256 insurancePoolShare = amount.mul(governanceFeeShare.insurancePoolShare).div(10000);
	   uint256 charityPoolShare = amount.mul(governanceFeeShare.charityPoolShare).div(10000);
	   uint256 teamPoolShare = amount.mul(governanceFeeShare.teamPoolShare).div(10000);
	   uint256 reservePoolShare = amount.mul(governanceFeeShare.reservePoolShare).div(10000);
