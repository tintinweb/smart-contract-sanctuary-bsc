...
contract TradingReaderFacet is ITradingReader {
    ...
    function getPairQty(address pairBase) external view override returns (ITradingCore.PairQty memory) {
        ITradingCore.PairPositionInfo memory ppi = LibTradingCore.tradingCoreStorage().pairPositionInfos[pairBase];
        return ITradingCore.PairQty(ppi.longQty, ppi.shortQty);
    }
...
library LibTradingCore {
    ...
    // todo: 后面改回 apollox.trading.core.storage
    bytes32 constant TRADING_CORE_POSITION = keccak256("apollox.trading.core.storage.20230306");
    ...
    function updatePairQtyAndAvgPrice(
        ITradingCore.PairPositionInfo storage ppi, uint256 qty, uint256 marketPrice, bool isOpen, bool isLong
    ) internal {
        if (isOpen) {
            if (isLong) {
                // LP Increase position
                if (ppi.longQty >= ppi.shortQty) {
                    ppi.lpAveragePrice = uint64((ppi.lpAveragePrice * (ppi.longQty - ppi.shortQty) + marketPrice * qty) / (ppi.longQty + qty - ppi.shortQty));
                }
                // LP Reverse open position
                else if (ppi.longQty < ppi.shortQty && ppi.longQty + qty > ppi.shortQty) {
                    ppi.lpAveragePrice = uint64(marketPrice);
                }
                // LP position == 0
                else if (ppi.longQty < ppi.shortQty && ppi.longQty + qty == ppi.shortQty) {
                ...
                if (ppi.shortQty >= ppi.longQty) {
                    ppi.lpAveragePrice = uint64((ppi.lpAveragePrice * (ppi.shortQty - ppi.longQty) + marketPrice * qty) / (ppi.shortQty + qty - ppi.longQty));
                }
                // LP Reverse open position
            ...
            if (isLong) {
                ...
                // LP Increase position
                    ...
                    ppi.lpAveragePrice = uint64((ppi.lpAveragePrice * (ppi.shortQty - ppi.longQty) + marketPrice * qty) / (ppi.shortQty - ppi.longQty + qty));
                }
                ppi.longQty -= qty;
            } else {
                // LP Reduce position, No change in average price
                ...
                // LP Increase position
                    ...
                    ppi.lpAveragePrice = uint64((ppi.lpAveragePrice * (ppi.longQty - ppi.shortQty) + marketPrice * qty) / (ppi.longQty - ppi.shortQty + qty));
                }
                ppi.shortQty -= qty;
            }
        }
...
library LibLimitOrder {
    ...
    // todo: 后面改回 apollox.limit.book.storage
    bytes32 constant LIMIT_ORDER_POSITION = keccak256("apollox.limit.order.storage.20230306");
    ...
    function executeLimitOrder(
        bytes32 orderHash, bool result, ITradingChecker.Refund refund
    ) internal {
        LimitOrderStorage storage los = limitOrderStorage();
        ...
        if (!result) {
            ...
            // openFee = notionalUsd * openFeeP / tokenPrice
            uint openFee = notionalUsd * IPairsManager(address(this)).getPairFeeConfig(order.pairBase).openFeeP * (10 ** token.decimals) / (1e4 * 1e10 * token.price);
            // executionFee = executionFeeUsd / tokenPrice;
            uint executionFee = ITradingConfig(address(this)).getTradingConfig().executionFeeUsd * (10 ** token.decimals) / (token.price * 1e18);
            IERC20(order.tokenIn).safeTransfer(msg.sender, executionFee);
            ITradingTrigger(address(this)).limitOrderDeal(
                ITradingTrigger.LimitOrder(
                    orderHash, order.user, order.limitPrice, order.pairBase, order.tokenIn,
                    uint96(order.amountIn - openFee - executionFee), order.stopLoss, order.takeProfit,
                    order.broker, order.isLong, uint96(openFee), uint96(executionFee), order.qty
                )
            );
        }
        // remove open order
        removeOrder(los, order, orderHash);
    }
