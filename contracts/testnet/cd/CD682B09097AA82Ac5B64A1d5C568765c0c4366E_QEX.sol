...
contract QEX is Ownable {
	using SafeMath for uint256;
	
	IERC20 public token = IERC20(0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee); //TESTNET BSC
	//IERC20 public token = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //PRODUCTION NET BSC BUSD
	

	//uint256 private EGGS_TO_HATCH_1MINERS = 1080000;
	uint256 private EGGS_TO_HATCH_1MINERS = 1380000;
	uint256 private PSN = 10000;
	uint256 private PSNH = 5000;
	uint256 private investFee = 3;
	uint256 private withdrawnFee = 2;
    ...
    //uint256 constant internal TIME_STEP = 2 minutes;
	

	constructor(address _dev, address _mark, address _ceo) {		
		devAddress = payable(_dev);
		marketingAddress = payable(_mark);
		ceoAddress = payable(_ceo);
	}
	...
	function reinvest(address ref) public initializer {		
	...
	function withdraw() external initializer checkUser_ {
		...
		transferHandler(msg.sender, SafeMath.sub(eggValue,fee));
	}
	...
	function invest(uint _amount, address ref) external initializer {
	...
	function devFee(uint256 _amount) private view returns(uint256) {
		return SafeMath.div(SafeMath.mul(_amount,investFee),100);
	}
	...
	function payFeesInv(uint _amount) internal {

		uint toOwners = _amount.div(4);
		transferHandler(devAddress, toOwners);
		transferHandler(marketingAddress, toOwners);
		transferHandler(ceoAddress, toOwners.mul(2));
		
	}

	function payFeesWith(uint _amount) internal {
