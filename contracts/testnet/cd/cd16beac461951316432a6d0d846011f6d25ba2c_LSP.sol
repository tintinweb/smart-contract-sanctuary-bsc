...
abstract contract AbsToken is IERC20, Ownable {
                ...
                if (_swapPairList[to]) {
                    if (!inSwap) {
                        ...
                        if (contractTokenBalance > 0) {
                            uint256 swapFee =  _buyLPDividendFee + _sellLPDividendFee + _sellLPFee;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap {
        ...
        USDT.transferFrom(address(_tokenDistributor), address(this), fistBalance);

        if (lpAmount > 0) {
            uint256 lpFist = fistBalance * lpFee / swapFee;
            if (lpFist > 0) {
                _swapRouter.addLiquidity(
                    address(this), _usdt, lpAmount, lpFist, 0, 0, address(0), block.timestamp
                );
            }
        }
    }
