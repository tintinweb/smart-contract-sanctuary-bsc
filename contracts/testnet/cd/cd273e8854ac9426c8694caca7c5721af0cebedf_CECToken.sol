...
contract CECToken is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    uint256 public transactionBurn;
    uint256 private previousTransactionBurn;
    ...
    event UpdateBurnPercent(uint256 burnPercent);
    ...
    function updateBurnPercent(uint256 burn_percentage) external onlyOwner whenNotPaused{
        transactionBurn = burn_percentage;
        emit UpdateBurnPercent(burn_percentage);
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) internal {
        (uint256 tTransferAmount, uint256 tFee, uint256 tSwapFee, uint256 tLiquidityFee, uint256 tBurn) = getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rSwapFee, uint256 rLiquidityFee) = getRValues(tAmount, tFee, tSwapFee, tLiquidityFee, tBurn);
        {
            address from = sender;
        ...
        takeTransactionBurnFee(sender, tBurn);
    ...
    function getTValues(uint256 amount) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount - tFee - tSwapFee - tLiquidityFee - tBurn;
        return (tTransferAmount, tFee, tSwapFee, tLiquidityFee, tBurn);
    }

    function getRValues(uint256 amount, uint256 tFee, uint256 tSwapFee, uint256 tLiquidityFee, uint256 tBurn) internal view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount - rFee - rSwapFee - rLiquidityFee - rBurn;
    ...
    function calculateTransactionBurn(uint256 _amount) internal view returns (uint256) {
        return (_amount * transactionBurn) / 10**2;
    }
    ...
    function takeTransactionBurnFee(address sender, uint256 tBurn) internal {
        if(tBurn > 0) {
            _amount_burnt += tBurn;
