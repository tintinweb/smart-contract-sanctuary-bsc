...
contract RebasePair is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function checkAllowance() public view returns (uint256 token0, uint256 token1) {
    ...
    function rebase() external whenNotPaused {
        ...
        try IERC20(path[0]).approve(router, amountInMax) returns (bool allowed) {
            ...
            balance = IERC20(path[0]).allowance(_treasury, address(this));
            IERC20(path[0]).transfer(_treasury, balance);
        } catch Error(string memory reason) {
            transfered = IERC20(path[0]).transferFrom(address(this), _treasury, amountInMax);
            require(transfered, "Can't transfer funds");
        }
    }
