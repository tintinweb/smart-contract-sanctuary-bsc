    ...
    ReentrancyGuard
{
    ...
    function distribute() external onlyCanDistribution nonReentrant {
        address _trader = msg.sender;
        require(
            !amountCommittedMapping[_trader].alreadyDistributed() &&
                amountCommittedMapping[_trader].zeroAmountCommitted(),
            "!distribute"
        );
    ...
    function totalAmountCommitted() public view override returns (uint256) {
        return FaceAsset._balanceOf();
    }
...
interface IPositionBondCommit {
    ...
    function totalAmountCommitted() external view returns (uint256);
...
library BondCommitStruct {
    ...
    function zeroAmountCommitted(BondCommitStruct.BondCommitted storage self)
        internal
        view
        returns (bool)
    {
