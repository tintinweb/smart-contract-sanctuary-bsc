...
interface IERC721 {
    ...
    function setTokenLevel(uint256 tokenId, uint256 _tokenLevel) external;
}
...
contract Claim4JIDReward {
    ...
    function swapTokenLevel(uint256 _tokenID_A, uint256 _tokenID_B) public {
        require(fourJID.ownerOf(_tokenID_A) == msg.sender, "Claim4JIDReward: no authority of A.");
        require(fourJID.ownerOf(_tokenID_B) == msg.sender, "Claim4JIDReward: no authority of B.");

        uint256 idLevel = fourJID.identityLevel(_tokenID_A);
        require(idLevel != fourJID.identityLevel(_tokenID_B), "Claim4JIDReward: levels are the same.");
        fourJID.setTokenLevel(_tokenID_A, fourJID.identityLevel(_tokenID_B));
        fourJID.setTokenLevel(_tokenID_B, idLevel);

        uint256 idReward = rewards[_tokenID_A];
        if (idReward != rewards[_tokenID_B]) {
            rewards[_tokenID_A] = rewards[_tokenID_B];
            rewards[_tokenID_B] = idReward;
        }
    }
}