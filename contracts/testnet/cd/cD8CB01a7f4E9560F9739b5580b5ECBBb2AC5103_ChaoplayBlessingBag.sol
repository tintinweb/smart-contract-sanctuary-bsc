...
contract ChaoplayBlessingBag is Context, IERC20, IERC20Metadata, AccessControl {
    // 福袋
    struct FuBag{
        uint number;           //福袋数量
        bool voted;            //福袋转卖
    ...
    constructor (address _router, address _usdt, address _liquidityAddress) {
    ...
    // 领预约券
    function BuyTickets(address blessingBag) public {
        require(BlessingBags[blessingBag].number <= 5, "Tickets over 5 ");
        BlessingBags[blessingBag].number += 5;
        // number += 5;
    }
    // 购买福袋
    function BuyFuBag(uint256 amount) public {
        require(amount >= min, "token allowance insufficient");
        require(amount >= IERC20(USDT_ADDRESS).allowance(_msgSender(), address(this)), "token allowance insufficient");
        IERC20(USDT_ADDRESS).transferFrom(_msgSender(),address(this),amount);
    }

    function get(address blessingBag) public view returns (uint) {
