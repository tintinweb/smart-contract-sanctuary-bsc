...
contract LuckyMoney is Ownable {
    ...
    uint256[] private _secondPrizes;
    uint256[] private _thirdPrizes;

    uint256 public nTicket;

    uint256 public MAX_REDRAW = 20;
    uint256 public MIN_TICKET_NUMBER = 9;
    ...
    mapping(uint256 => bool) public secondPrizeWinner;
    mapping(uint256 => bool) public thirdPrizeWinner;
    mapping(uint256 => bool) public isWinner;
    uint256 public maxSecondPrizeNumber = 2;
    uint256 public maxThirdPrizeNumber = 5;
    ...
    function register(string memory _name) external {
        registerFresh(_name, _msgSender());
    }

    function registerByOperator(string memory _name, address _user) external onlyOperator {
        registerFresh(_name, _user);
    }

    function registerFresh(string memory _name, address _user) internal {
        require(tickets[_user] == 0, "LuckyMoney: registered user");
        require(!_isUsedName[_name], "LuckyMoney: nickname is used");
        nTicket = nTicket + 1;
        uint256 tid = nTicket;
        tickets[_user] = tid;
        users[tid] = UserInfo({
        ...
        _isUsedName[_name] = true;

        emit Registered(_name, _user, tid);
    }

    function thirdPrizes(uint256 index) public view returns(uint256 winner) {
        return (index < _thirdPrizes.length) ? _thirdPrizes[index] : 0;
    }

    function secondPrizes(uint256 index) public view returns(uint256 winner) {
        return (index < _secondPrizes.length) ? _secondPrizes[index] : 0;
    }

    function getThirdPrizes() public view returns(uint256[] memory) {
        return _thirdPrizes;
    }

    function getSecondPrizes() public view returns(uint256[] memory) {
        return _secondPrizes;
    }

    function thirdPrizeDraw() external onlyOperator {
        require(allowRedraw == true || _thirdPrizes.length < maxThirdPrizeNumber, "LuckyMoney: all third prizes has a winner");
        uint256 seed;
        uint256 count;
        ...
        for (uint256 i = 0; i < MAX_REDRAW; i++) {
            ...
            if (!(isWinner[seed] || blacklist[seed])) {
                break;
            }
        }

        while (isWinner[seed] || blacklist[seed]) {
            require(count++ < _nTicket, "LuckyMoney: blacklist block too many tickets");
            seed = (seed != _nTicket) ? seed + 1 : 1;
        }
        _thirdPrizes.push(seed);
        thirdPrizeWinner[seed] = true;
        isWinner[seed] = true;

        emit ThirdPrizeDrawed(0, seed);
    }

    function secondPrizeDraw() external onlyOperator {
        require(allowRedraw == true || _secondPrizes.length < maxSecondPrizeNumber, "LuckyMoney: all second prizes has a winner");
        uint256 seed;
        uint256 count;
        uint256 _nTicket = nTicket;
        uint256 _n = (_nTicket % 10 == 0) ? _nTicket + 1 : _nTicket;
        ...
        _secondPrizes.push(seed);
        secondPrizeWinner[seed] = true;
        isWinner[seed] = true;

        emit SecondPrizeDrawed(0, seed);
    }
    ...
    function firstPrizeDraw() external onlyOperator {
        ...
        emit FirstPrizeDrawed(first, seed);
    }
