...
library Magic {
    ...
    function infoBG(uint256 id) external pure returns (string memory){
        string memory des = ', "description": "one of 10k cc0 Omnichain Onchain Crazy Snails minted out across 7 different chains"';
    ...
    function infoChain(uint256 id) external pure returns(string memory){
        string memory s = '"},{"trait_type": "Birth_Network","value": "';
        string memory c;
        if(id>=1 && id<=3300){
            c = "ETH";
        }
