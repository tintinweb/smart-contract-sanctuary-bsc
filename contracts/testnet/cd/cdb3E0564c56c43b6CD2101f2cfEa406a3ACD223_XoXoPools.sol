...
contract XoXoPools is Ownable {
    ...
    function buyGlobalSlot(uint _pool) public {
        ...
        if(ps.prelaunch == true) {
            ...
            if(poolID == 1) {
                ...
                (,,_refCount,,) = XoXoContract.getUserData(_buyer);
                uint prevPool = poolID - 1;
                (,,,,,uint[] memory _prevSlots) = getUserPoolData(_buyer, prevPool);
                require((p.whitelisted[_buyer] == true || _prevSlots.length >= ps.needPrevSlots || _refCount >= 10), 'Not whitelisted or not match entrance condition');
            }
        }
    ...
    function setPoolParams(uint _pool, bool _status, bool _prelaunch, bool _countLoseAmount, uint _needSlots, bool _payRef, uint _payRefAmount, uint _paySlotAmount, uint _cycleAt)  public onlyOwner  {
        Settings storage ps = PoolSettings[_pool];
        ps.status = _status;
