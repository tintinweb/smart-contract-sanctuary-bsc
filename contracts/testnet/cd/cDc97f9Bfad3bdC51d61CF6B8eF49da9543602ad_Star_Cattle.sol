...
contract Star_Cattle is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    using StringsUpgradeable for uint256;
    ...
    event UpGradeStar(uint indexed tokenId);
    ...
    function upGradeStar(uint tokenId) external {
        ...
        if(cowInfoes[tokenId].star < 3){
            cowInfoes[tokenId].star ++;
            emit UpGradeStar(tokenId);
        }
    }
