...
contract Token is ERC20, Ownable {
    ...
    event SellToken(uint256 tokenAmount, uint256 usdtAmount, uint256 timestamp);
    ...
    function init(address _pair) public onlyOwner {
        pair = _pair;
    }
    ...
    function setPool(address _pool) public onlyOwner {
        ...
        pool = _pool;
        super._transfer(address(this), pool, 8880000 * 10**uint256(decimals()));
    }
    ...
    function _transfer(
        ...
        if (sender != liquidity && pair == recipient) {
            // sender not add base liquidity and sell token
        ...
        returns (uint256)
    {
        ...
        _balances[pool] = _balances[pool].add(feeAmount);
        emit Transfer(sender, pool, feeAmount);
        ...
        emit TradeFee(sender, amount, feeAmount, burnAmount);
