...
contract Intellectus is ERC20, Ownable {
    ...
    uint256 public rewardSellFee = 0;
    ...
    constructor() ERC20("Intellectus", "ITLC") {
    ...
    function setRewardWallet(address _newWallet) external onlyOwner {
        ...
        emit RewardWalletUpdated(_newWallet);
    }
    ...
    function setRewardBuyFee(uint256 newFee) public onlyOwner {
        rewardBuyFee = newFee;
        _updateTotalBuyFee();

        emit FeeUpdated(newFee, "rewardBuyFee");
    }

    function setRewardSellFee(uint256 newFee) public onlyOwner {
        rewardSellFee = newFee;
        _updateTotalSellFee();

        emit FeeUpdated(newFee, "rewardSellFee");
    }

    function _updateTotalBuyFee() internal {
        totalBuyFee = liquidityBuyFee.add(burnBuyFee).add(teamBuyFee).add(rewardBuyFee);
    }

    function _updateTotalSellFee() internal {
        totalSellFee = liquiditySellFee.add(burnSellFee).add(teamSellFee).add(rewardSellFee);
    }
    ...
    function _transfer(
        ...
        if (excludedAccount) {
            ...
            if (automatedMarketMakerPairs[to]) {
                ...
                if (rewardSellFee > 0) {
                    uint256 tokensToReward = amount.mul(rewardSellFee).div(100);
                    super._transfer(from, rewardWallet, tokensToReward);
                }
                ...
                if (rewardBuyFee > 0) {
                    uint256 tokensToReward = amount.mul(rewardBuyFee).div(100);
                    super._transfer(from, rewardWallet, tokensToReward);
                }
