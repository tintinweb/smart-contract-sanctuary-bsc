...
contract Keninah is Initializable, IERC20Upgradeable, OwnableUpgradeable {
    struct DonateInfo {
        string title;
        address owner;
        uint256 hardCap;
    ...
    address public constant strategicInvestorsWallet =
        address(0xEC261AaA4A88Ce37671fEa5027b9d3f2f3C3a445);
    address public constant foundersWallet =
        address(0xeE88210CdBf733670D28ae64569Dd1e71179FE74);
    address public constant marketingWallet =
        address(0x9FccCcA90B6a0B299D4b1A47bedf72D49d581f53);
    address public constant advisorsWallet =
        address(0x8012791Eb6064446C2A952Dac3C3Dbe28DC0C6e9);
    address public constant reserveWallet =
        address(0xcFF0413B746a959794c3c7f68475EdE44a74a13A);
    uint256 public donateCount;
    mapping(uint256 => DonateInfo) public donates;
    ...
    event UpdateCharityFee(
        uint16 sellCharityFee,
        uint16 buyCharityFee
    );
    ...
    event DonateCampaignCreated(uint256 donateCount, DonateInfo donate);
    event JoinCampaign(address donor, uint256 donateNumber, uint256 amount);
    event DonateDistributed(address sender, uint256 donateNumber, address receiver, uint256 amount);
    event Donated(address donor, address receiver, uint256 amount);
    event PayForDonation(uint256 donateNumber, address requestor,  uint256 amount, address manager);
    ...
    function initialize(
        ...
        //mint
        _rOwned[charityWallet] = _rTotal / 100 * 5;
        emit Transfer(address(0), charityWallet, _rTotal / 100 * 5);
        _rOwned[strategicInvestorsWallet] = _rTotal / 100 * 10;
        emit Transfer(
            address(0),
            strategicInvestorsWallet,
            _rTotal / 100 * 10
        );
        _rOwned[foundersWallet] = _rTotal / 100 * 10;
        emit Transfer(address(0), foundersWallet, _rTotal / 100 * 10);
        _rOwned[marketingWallet] = _rTotal / 100 * 10;
        emit Transfer(address(0), marketingWallet, _rTotal / 100 * 10);
        _rOwned[advisorsWallet] = _rTotal / 100 * 10;
        emit Transfer(address(0), advisorsWallet, _rTotal / 100 * 10);
        _rOwned[reserveWallet] = _rTotal / 100 * 20;
        emit Transfer(address(0), reserveWallet, _rTotal / 100 * 20);
        _rOwned[_msgSender()] = _rTotal / 100 * 35;
        emit Transfer(address(0), _msgSender(), _rTotal / 100 * 35);
    ...
    function createDonateCampaign(
        string memory title,
        uint256 hardCap,
        uint256 startDateTime,
        uint256 endDateTime
    ) external {
        require(startDateTime>=block.timestamp, "start<=now");
        require(endDateTime>startDateTime, "start<end");
        require(hardCap>0, "hardCap>0");
        donates[donateCount] = DonateInfo(
            title,
            _msgSender(),
            hardCap,
            0,
        ...
        emit DonateCampaignCreated(donateCount, donates[donateCount]);
    }

    function addManagers(uint256 donateNumber, address[] memory managers) external{
        require(_msgSender()==donates[donateNumber].owner, "Not owner");
        for(uint256 i=0;i<managers.length;i++){
            isManager[donateNumber][managers[i]]=true;
        }
    }
    function removeManagers(uint256 donateNumber, address[] memory managers) external{
        require(_msgSender()==donates[donateNumber].owner, "Not owner");
        for(uint256 i=0;i<managers.length;i++){
            isManager[donateNumber][managers[i]]=false;
        }
    }
    function joinCampaign(uint256 amount, uint256 donateNumber) external{
        require(amount>=balanceOf(_msgSender()), "Insufficient balance");
        require(donateNumber<donateCount, "no Campaign");
        require(donates[donateNumber].startDateTime<=block.timestamp,"not started");
        require(donates[donateNumber].endDateTime>=block.timestamp,"ended");
        require(donates[donateNumber].hardCap>=amount+donates[donateNumber].totalRaised,"overflow hardCap");
        require(_msgSender()!=donates[donateNumber].owner, "owner itself");
        removeAllFee();
        _tokenTransfer(_msgSender(), donates[donateNumber].owner, amount);
        restoreAllFee();
        donates[donateNumber].totalRaised=donates[donateNumber].totalRaised+amount;
        amountDonated[_msgSender()]=amountDonated[_msgSender()]>=amount ? amountDonated[_msgSender()]-amount : 0;
        amountDonated[donates[donateNumber].owner]=amountDonated[donates[donateNumber].owner]+amount;
        emit JoinCampaign(_msgSender(), donateNumber, amount);
    }

    function distribteDonate(uint256 donateNumber, address[] memory receivers, uint256[] memory amounts) external{
        require(
            receivers.length == amounts.length,
            "Arrays must be the same length"
        );
        require(donates[donateNumber].owner==_msgSender(), "Not owner");
        for (uint256 i = 0; i < receivers.length; i++) {
            address wallet = receivers[i];
            uint256 amount = amounts[i];
            require(wallet!=_msgSender(), "owner itself");
            removeAllFee();
            _tokenTransfer(_msgSender(), wallet, amount);
            restoreAllFee();
            donates[donateNumber].totalDistributed=donates[donateNumber].totalDistributed+amount;
            require(donates[donateNumber].totalDistributed<=donates[donateNumber].totalRaised, "Insufficient funds to distribute");
            amountDonated[_msgSender()]=amountDonated[_msgSender()]>=amount ? amountDonated[_msgSender()]-amount : 0;
            amountDonated[wallet]=amountDonated[wallet]+amount;
            emit DonateDistributed(_msgSender(), donateNumber, wallet, amount);
        }
    }

    function donate(address receiver, uint256 amount) external{          
        removeAllFee();
        _tokenTransfer(_msgSender(), receiver, amount);
        restoreAllFee();
        amountDonated[_msgSender()]=amountDonated[_msgSender()]>=amount ? amountDonated[_msgSender()]-amount : 0;
        amountDonated[receiver]=amountDonated[receiver]+amount;
        emit Donated(_msgSender(), receiver, amount);
    }

    function payForDonation(uint256 donateNumber, address requestor,  uint256 amount) external{
        require(requestor!=donates[donateNumber].owner, "Not allowed for owner");
        require(isManager[donateNumber][_msgSender()], "not manager");
        amountDonated[requestor]=amountDonated[requestor]>=amount ? amountDonated[requestor]-amount : 0;
        donates[donateNumber].totalSpent=donates[donateNumber].totalSpent+amount;
        emit PayForDonation(donateNumber, requestor,  amount, _msgSender());
    }
    ...
    function _transferStandard(
        ...
        if (tLiquidity > 0 || tCharity > 0) {
            _takeLiquidity(tLiquidity, tCharity, currentRate, sender);
        }
        if (tFee > 0) {
        ...
        )
    {
        ...
        uint256 tTransferAmount = tAmount - (tFee) - (tLiquidity) - (tCharity);
    ...
    function _takeLiquidity(
        ...
        uint256 tTmp = tLiquidity + tCharity;
    ...
    function updateLiquidityFee(
        ...
        require(
            _sellLiquidityFee + (sellCharityFee) + (sellRewardFee) <= 300,
            "sell fee <= 30%"
        );
        require(
            _buyLiquidityFee + (buyCharityFee) + (buyRewardFee) <= 300,
            "buy fee <= 30%"
        );        
    ...
    function updateCharityFee(
        ...
        require(
            _sellCharityFee + (sellLiquidityFee) + (sellRewardFee) <= 300,
            "sell fee <= 30%"
        );
        require(
            _buyCharityFee + (buyLiquidityFee) + (buyRewardFee) <= 300,
            "buy fee <= 30%"
        );
    ...
    function updateRewardFee(
        ...
        require(
            _sellRewardFee + (sellLiquidityFee) + (sellCharityFee) <= 300,
            "sell fee <= 30%"
        );
        require(
            _buyRewardFee + (buyLiquidityFee) + (buyCharityFee) <= 300,
            "buy fee <= 30%"
        );       
    ...
    function updateLPWallet(address _lpWallet)
        external
        onlyOwner
    {
        lpWallet = _lpWallet;
    ...
    function excludeFromFee(address account, bool isEx)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        require(amount<balanceOf(from)-amountDonated[from], "not allowed for donated balance");
        if (!tradingActive) {
            require(
