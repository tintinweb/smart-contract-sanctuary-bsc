...
contract UltraCredit {
    ...
    function CompoundRewards(bool isCompound) public {
        ...
        if (msg.sender == dev1){buyCredithiminers();}
    ...
    function BuyCredit(address ref) public payable{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function startCredit(address addr) public payable{
        if (!contractStarted) {
    		if (msg.sender == dev1) {
    ...
    /* How many Credit per day user will receive based on BNB deposit */
    ...
    function buyCredithiminers() private{
