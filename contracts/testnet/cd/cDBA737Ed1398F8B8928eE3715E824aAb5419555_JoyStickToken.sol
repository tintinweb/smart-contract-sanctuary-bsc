...
contract JoyStickToken is ERC20, Pausable, AccessControl, GetStuck {
    ...
    constructor(address _feeReceiver) ERC20("JoyStick Token", "JSK") {
        _taxReceiver = _feeReceiver;
        isExcludedFromFee[_feeReceiver] = true;
        isExcludedFromFee[msg.sender] = true;

        emit ExcludeFromFees(_feeReceiver);
        emit ExcludeFromFees(msg.sender);
    ...
    function _transfer(
        ...
        if (!isExcludedFromFee[sender]) {
            ...
            amount = amount - fee;
            super._transfer(sender, _taxReceiver, fee);
        }
    ...
    function excludeMultipleAccountsFromFees(address[] memory accounts)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < accounts.length; i++) {
            if (!isExcludedFromFee[accounts[i]]) {
    ...
    function includeMultipleAccountsInFees(address[] memory accounts)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < accounts.length; i++) {
