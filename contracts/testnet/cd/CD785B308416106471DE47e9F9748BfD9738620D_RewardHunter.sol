...
library Counters {
 ...
 abstract contract Unbox is Ownable {
  ...
  struct UnboxUser {
    ...
    function randMod(uint _modulus) internal returns(uint){
        randNonce++;
        return uint(keccak256(abi.encodePacked(block.difficulty, msg.sender, randNonce))) % _modulus;
    }
  ...
  function _unboxClass(address _usr) internal{
      ...
      require(msg.value == box1 || msg.value == box2 || msg.value == box3, "Precisa definir um valor da box");
      if (msg.value == box1) {
...
library CounterNFT {
 ...
 contract RewardHunter is ERC1155, Ownable, Unbox, ReentrancyGuard {
    ...
    using Address for address;
    // Id de cada NFT
    CounterNFT.Counter private _tokenIds;
    // Id para Tokens Vendidos
    CounterNFT.Counter private _itemsSold;

    string private _uri;

    uint256 public percentBuy;
    uint256 public priceRareOne;
    uint256 public priceRareTwo;
    uint256 public priceRareThree;
    uint256 public priceRareFor;

 
    uint256 public priceRareOneBNB;
    uint256 public priceRareTwoBNB;
    uint256 public priceRareThreeBNB;
    uint256 public priceRareForBNB;

    uint256 private listingPrice;
    uint256 private priceInRHT;
    IBEP20 public token;
    address public wallet;

  event MintNfts(address indexed from, uint256 id, uint256 _amountBnb);
  ...
  struct NftUsers {
    ...
    function setListingBNB(uint256 _listing) public onlyOwner {
      listingPrice = _listing;
    }

    function setListingRHT(uint256 _rht) public onlyOwner {
      priceInRHT = _rht;
    }

  function priceRHTNfts(uint256 _priceRareOne, uint256 _priceRareTwo, uint256 _priceRareThree, uint256 _priceRareFor) external onlyOwner {
    priceRareOne = _priceRareOne;
    priceRareTwo = _priceRareTwo;
    priceRareThree = _priceRareThree;
    priceRareFor = _priceRareFor;
  }

    function priceBNBNfts(uint256 _priceRareOneBNB, uint256 _priceRareTwoBNB, uint256 _priceRareThreeBNB, uint256 _priceRareForBNB) external onlyOwner {
    priceRareOneBNB = _priceRareOneBNB;
    priceRareTwoBNB = _priceRareTwoBNB;
    priceRareThreeBNB = _priceRareThreeBNB;
    priceRareForBNB = _priceRareForBNB;
  }




  function _forwardFundsRht(uint256 _amount) internal {
    ...
    function _unboxUser(address _usr) internal returns(uint256) {
      ...
      emit MintNfts(msg.sender, newTokenId, msg.value);
  ...
  function _forwardFunds() internal {
    ...
    function mintNFT(uint _amount) external payable nonReentrant returns(uint256) {
              // Gera Ids de cada NFT
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
        NftUsers storage ids = idToMarketItem[newTokenId];
        ...
        if (priceInRHT != 0) {
          require(_amount == priceRareOne || _amount == priceRareTwo || _amount == priceRareThree || _amount == priceRareFor, "Voce precisa definir um preco identico ao preco de venda");
          require(listingPrice == 0, "RHT: Preco em BNB Precisa ser definido para 0");
          require(msg.value == 0, "RHT:Saldo de BNB precisa ser 0");
          if(_amount == priceRareOne) {
            _forwardFundsRht(priceRareOne);
            ids.rareType = 1;
          }
          else if(_amount == priceRareTwo) {
            _forwardFundsRht(priceRareTwo);
            ids.rareType = 2;
          }
          else if(_amount == priceRareThree) {
            _forwardFundsRht(priceRareThree);
            ids.rareType = 3;
          }
          else if(_amount == priceRareFor) {
            _forwardFundsRht(priceRareFor);
            ids.rareType = 4;
          }
          ...
          require(msg.value == priceRareOneBNB || msg.value == priceRareTwoBNB || msg.value == priceRareThreeBNB || msg.value == priceRareForBNB, "Voce precisa definir um preco identico ao preco de venda");
          require(priceInRHT == 0, "ListingPrice BNB Precisa ser false");
          require(_amount == 0, "RHT: Amount precisa ser 0");
          if(msg.value == priceRareOneBNB) {
            _forwardFunds();
            ids.rareType = 1;
          }
          else if(msg.value == priceRareTwoBNB) {
            _forwardFunds();
            ids.rareType = 2;
          }
          else if(msg.value == priceRareThreeBNB) {
            _forwardFunds();
            ids.rareType = 3;
          }
          else if(msg.value == priceRareForBNB) {
            _forwardFunds();
            ids.rareType = 4;
          }   
          ...
          require(priceInRHT == 1 || listingPrice == 1, "Precisa definir a forma de pagamento");
        }
    ...
    function _safeTransferFrom(uint _tokenId, address _receiver) virtual public nonReentrant  {
          NftUsers storage ids = idToMarketItem[_tokenId];
    ...
    function buyNfts(uint _tokenId, uint _price) public nonReentrant  {
      ...
      NftUsers storage ids = idToMarketItem[_tokenId];
          ...
          uint256 divisorTax = (_price * percentBuy) / 100;
           
          uint256 transferSeller = (_price - divisorTax);
          ...
          // Salvo o saldo da taxa para o Owner retirar depois
           // Contrato envio a % de desconto para a wallet do Owner
           IBEP20(token).transfer(wallet, divisorTax);
    ...
    function removeMarket(uint _tokenId) external nonReentrant  {
