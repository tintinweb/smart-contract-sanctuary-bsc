...
//    uint public rentingTime = 30 days;
    ...
    event AskNew(address indexed collection, address indexed seller, uint256 indexed tokenId, uint256 askPrice, address _assetAccept, uint _time2Rent);
    ...
    function createAskOrder(address _collection, uint256 _tokenId, uint256 _askPrice, address _assetAccept, address _to, uint _time2Rent) external nonReentrant {
        ...
        _askDetails[_collection][_tokenId] = Ask({seller: msg.sender, price: _askPrice, acceptAsset: _assetAccept, to: _to, time2Rent: _time2Rent});

        // Add tokenId to the askTokenIds set
        _askTokenIds[_collection].add(_tokenId);
        totalOrderCollection[_collection]++;
        // Emit event
        emit AskNew(_collection, msg.sender, _tokenId, _askPrice, _assetAccept, _time2Rent);
    }
...
//            payable(askOrder.seller).transfer(netPrice);
            _rentingDetails[_collection][_tokenId] = Renting({renter: msg.sender, lessor: askOrder.seller, startTime: block.timestamp, endTime: block.timestamp + askOrder.time2Rent, requestRevoke: 0, asset: address(0), assetAmount: netPrice});
        }
        else {
//            IERC20(asset2Buy).safeTransfer(askOrder.seller, netPrice);
            _rentingDetails[_collection][_tokenId] = Renting({renter: msg.sender, lessor: askOrder.seller, startTime: block.timestamp, endTime: block.timestamp + askOrder.time2Rent, requestRevoke: 0, asset: asset2Buy, assetAmount: netPrice});
        }
...
//        IERC20(WBNB).safeTransfer(askOrder.seller, netPrice);
        _rentingDetails[_collection][_tokenId] = Renting({renter: msg.sender, lessor: askOrder.seller, startTime: block.timestamp, endTime: block.timestamp + askOrder.time2Rent, requestRevoke: 0, asset: WBNB, assetAmount: netPrice});
    ...
    function processRentCost(address _to, uint _amount, address _asset) internal {
        if(_asset == address(0)) payable(_to).transfer(_amount);
        else IERC20(_asset).transfer(_to, _amount);
    }
    function revokeNFT(
        address _collection,
        uint256 _tokenId
        ...
        if(renting.requestRevoke > 0) processRentCost(renting.renter, renting.assetAmount, renting.asset);
        else processRentCost(renting.lessor, renting.assetAmount, renting.asset);
        // Update storage information
