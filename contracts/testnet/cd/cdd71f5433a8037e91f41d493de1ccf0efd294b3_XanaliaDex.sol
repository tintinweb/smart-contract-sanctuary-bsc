...
contract Manager is Initializable, OwnableUpgradeable, PausableUpgradeable {
	...
	address xanaliaAddressesStorageAddress;
	...
	event ArtistAddressSet(address indexed admin, uint256 indexed numberAddress);
...
contract XanaliaDex is Manager, ReentrancyGuardUpgradeable {
	...
	*/
	function acceptBidAuction(uint256 _bidAuctionId) external whenNotPaused {
		...
		(totalPaymentAmount, collectionAddress, paymentToken, tokenId, auctionOwner) = auctionDex.acceptBidAuction(
			_bidAuctionId,
			msg.sender
		);

		bool sent = _match(msg.sender, paymentToken, auctionOwner, collectionAddress, tokenId, totalPaymentAmount);
		require(sent, "FAILED_NFT_TRANSFER");
	...
	*/
	function setWhitelistAddress(address[] calldata _user, bool[] calldata _status) external onlyOwner {
...
interface IAuctionDex {
	...
	function acceptBidAuction(uint256 _bidAuctionId, address _auctionOwner) external returns (uint256, address, address, uint256, address);
}
