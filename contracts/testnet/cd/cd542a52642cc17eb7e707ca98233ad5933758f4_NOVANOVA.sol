...
/////////////////////////////////////////////////////////////////////////////////////////
   ...
   function  novuxa_Balance_Token1() external view returns(uint) {
    ...
    function  nouvaxa_price() external view returns(uint) {
      ...
      return (token.balanceOf(address(this))/tkdA) / (token_.balanceOf(address(this))/tkdB);
    ...
    function SwapAB(uint amountA) public swap_checkA(amountA) {
      require(token.balanceOf(msg.sender) >= amountA*tkdA, "Not Enough USDT Balance For this SWAP");
        token.transferFrom(msg.sender, address(this), amountA*tkdA);
        uint256 fee = amountA*tkdA / 100; // 0.1% swap fee
        uint256 amountq = (amountA*tkdA) - fee;
        uint256 price = (token.balanceOf(address(this))/tkdA) / (token_.balanceOf(address(this))/tkdB);
        require(token_.balanceOf(address(this)) >= amountq/tkdD, "Not Enough Liquidity For this SWAP");
         token_.transfer(msg.sender, (amountq/tkdD)*price);
      //  token_.transferFrom(reserve_address, msg.sender, amountq);     
       ...
       // token.approve(TokenA, amountA);
        emit SWAPABa(now, msg.sender, address(this), amountA*tkdA, token.balanceOf(address(this))/tkdA, token_.balanceOf(address(this))/tkdB);
        emit SWAPABb(now, address(this), msg.sender,  amountq/tkdD, token.balanceOf(address(this))/tkdA, token_.balanceOf(address(this))/tkdB);
      ...
      function SwapBA(uint amountB) public swap_checkB(amountB) {
        require(token_.balanceOf(msg.sender) >= amountB*tkdB, "Not Enough Deelian Balance For this SWAP");
           token_.transferFrom(msg.sender, address(this), amountB*tkdB);
           uint256 fee = amountB*tkdB / 100; // 1% swap fee
           uint256 amountq = (amountB*tkdB) - fee;
           uint256 price = (token.balanceOf(address(this))/tkdA) / (token_.balanceOf(address(this))/tkdB);
           require(token.balanceOf(address(this)) >= (amountq*tkdD), "Not Enough Liquidity For this SWAP");
           token.transfer(msg.sender, (amountq*tkdD)*price);
        //   token_.approve(TokenB, amountB);
           emit SWAPBAa(now, msg.sender, address(this), amountB*tkdB, token.balanceOf(address(this))/tkdA, token_.balanceOf(address(this))/tkdB);
           emit SWAPBAb(now, address(this), msg.sender,  amountq*tkdD, token.balanceOf(address(this))/tkdA, token_.balanceOf(address(this))/tkdB);
