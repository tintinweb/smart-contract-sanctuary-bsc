...
contract DTokenRegistry is Initializable, Adminable {
    ...
    function registerMintingDToken(
      ...
      vault_.initialize(address(appCtrl), feeConf, address(token_));

      uint vtype = 2;  // vtype 1 : for deposit vault 2: for mint vault
      appCtrl.setVault(address(token_), address(vault_), vtype);
      appCtrl.setVaultStates(address(vault_), vstate_);
      appCtrl.setOracles(address(token_), address(oracle_), collateralFactor_, creditBurn_);
      oracle_.setAggregator(address(token_), aggrator_);
      token_.addMiner(address(vault_));

      dTokens.push(address(token_));
      emit DTokenCreated(address(token_));
    }
