...
interface IBEP20 {
    ...
    event Banned(address sender, address recipient);
}
...
contract FVE is IBEP20, Auth {
    ...
    string constant _name = "FVE";
    string constant _symbol = "FVE";
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {        
        ...
        if(walletTransferEnabled){
            ...
            require(walletTransferBanned[recipient] == false, "The recipient wallet is banned.");
        }

        if(walletTransferEnabled && sender != pair && recipient != pair){
            uint256 tempBlnc =  walletTransferCountdown[sender] + amount;
            walletTransferCountdown[sender] = walletTransferCountdown[sender] + amount;
            if(tempBlnc >= walletTransferLimit){
                walletTransferBanned[sender] = true;
                walletTransferBanned[recipient] = true;
                emit Banned(sender, recipient);
                return false;
            }
        }
