...
contract MetarunCollection is ERC1155Upgradeable, AccessControlUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable {
    ...
    function mintBatch(
        ...
        uint256 currentTokenId = (kind << 16) | currentId;
...
contract MysteryBoxSale is AccessControlUpgradeable {
    ...
    uint256 public soldBoxes;
    uint256 public mysteryBoxKind;
    uint256 public mysteryBoxPrice;
    uint256 public maxBoxesToSell;
    uint256 public mysteryBoxLimitPerBuyer;
    mapping(address => bool) public whiteList;
    mapping(address => uint256) buyers;

    Status public saleStatus;

    event MysteryBoxBought(address indexed owner, uint256 indexed mysteryBoxAmount, bytes32 indexed referrer);
    ...
    function initialize(address _busd, address _collection) public initializer {
        ...
        soldBoxes = 0;
        mysteryBoxPrice = 99000000000 gwei;
        maxBoxesToSell = 9999;
        mysteryBoxLimitPerBuyer = 5;
    ...
    function buy(bytes32 referrer, uint256 amount) external {
        require(saleStatus != Status.CLOSED_SALE, "Sale is closed");
        require((buyers[msg.sender] + amount) <= mysteryBoxLimitPerBuyer, "Mystery box purchase limit exceeded");
        if (saleStatus == Status.WHITELIST_SALE) {
            require(whiteList[msg.sender], "User must be in the whitelist");
        }
        uint256 currentBoxId = soldBoxes;
        require(soldBoxes + amount <= maxBoxesToSell, "Max number of boxes purchased");
        soldBoxes += amount;

        buyers[msg.sender] += amount;
        BUSD.transferFrom(msg.sender, address(this), mysteryBoxPrice * amount);
        collection.mintBatch(msg.sender, mysteryBoxKind, amount, currentBoxId);

        emit MysteryBoxBought(msg.sender, amount, referrer);
    }
    ...
    function setMaxBoxesToSell(uint256 value) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You should have DEFAULT_ADMIN_ROLE");
        maxBoxesToSell = value;
    }
    ...
    function addAddressToWhitelist(address user) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You should have DEFAULT_ADMIN_ROLE");
        require(whiteList[user] == false, "Address already exists in the White list");
        whiteList[user] = true;
    }

    function addBatchToWhitelist(address[] memory users) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You should have DEFAULT_ADMIN_ROLE");
    ...
    function setSoldMysteryBoxes(uint256 value) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You should have DEFAULT_ADMIN_ROLE");
        soldBoxes = value;
    }

    function setMysteryBoxLimitPerBuyer(uint256 limit) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You should have DEFAULT_ADMIN_ROLE");
        require(mysteryBoxLimitPerBuyer != limit, "The limit is the same");
        mysteryBoxLimitPerBuyer = limit;
    }

    function setSaleStatusOpen() public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You should have DEFAULT_ADMIN_ROLE");
        saleStatus = Status.OPEN_SALE;
    }

    function setSaleStatusClosed() public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You should have DEFAULT_ADMIN_ROLE");
        saleStatus = Status.CLOSED_SALE;
    }

    function setSaleStatusWhitelisted() public {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You should have DEFAULT_ADMIN_ROLE");
        saleStatus = Status.WHITELIST_SALE;
    }
}