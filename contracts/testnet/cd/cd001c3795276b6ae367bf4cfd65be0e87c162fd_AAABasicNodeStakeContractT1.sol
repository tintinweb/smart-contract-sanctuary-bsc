 ...
 */
contract AAABasicNodeStakeContractT1 is Ownable {
    ...
    function add(uint256 _allocPoint,address _lpToken,uint256 _threshold) public onlyOwner nonDuplicated(_lpToken) {
    ...
    function getBoostMultiplier(uint256 _indexed) public view returns (uint256 boostmultiplier) {
        ...
        if (depositInfoByIndex.depositDays == duration * 86400) {
    ...
    function pendingReward(uint256 _pid, address _user) public view returns (uint256 totalPending) {
        ...
        for (uint256 i = 0; i < depositInfo.length; i++) {
            if (depositInfo[i].lpToken == pool.lpToken && depositInfo[i].user == _user) {
                ...
                uint256 userClaimAmountPerBlock=  poolClaimAmountPerDay.mul(depositInfo[i].amount).mul(getBoostMultiplier(i)).div(IERC20(pool.lpToken).balanceOf(address(this))).div(28800);
    ...
    function judgmentThreshold(uint256 _pid) public view returns(bool isReached) {
        PoolInfo storage pool = poolInfo[_pid];
        uint256 lpTokenSupply = IERC20(pool.lpToken).balanceOf(pool.lpToken);
        uint256 mylpTokenSupply = IERC20(pool.lpToken).balanceOf(msg.sender);
        uint256 rate = mylpTokenSupply.div(lpTokenSupply);
        if(pool.threshold.div(100) >= rate) {
            return true;
        } else {
            return false;
        }
    ...
    function deposit(uint256 _pid,uint256 _amount,uint256 _depositDays) public {
        ...
        require(judgmentThreshold(_pid), "deposite: Threshold amount must be met");
        require(_depositDays == duration,"deposite: depositDays amount must be met");
        if(_amount > 0) {
    ...
    function withdraw(uint256 _pid) public {
        ...
        if (getAllowedWithdrawAmount(_pid, msg.sender) > 0) {
            IERC20(pool.lpToken).safeTransfer(address(msg.sender),getAllowedWithdrawAmount(_pid, msg.sender));
        }
    ...
    function emergencyWithdraw(uint256 _pid) public {
        ...
        require(user.totalDepositAmount > 0,"emergencyWithdraw: user's amount must be greater than 0");
        if(user.totalDepositAmount > 0 && OpenEmergencyWithdraw == true) {
            IERC20(pool.lpToken).transfer(address(msg.sender), user.totalDepositAmount);
