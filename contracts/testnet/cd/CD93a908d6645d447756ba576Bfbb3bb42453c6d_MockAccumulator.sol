	...
	AccessControlEnumerable
{
	...
	function deposit(
		...
		IMockToken(blockTokenInfos[0].tokenLp).mint(_amount);
	...
	function _withdraw(
		...
		require(
			address(_outputTokens[0]) ==
				...
				address(_outputTokens[0]) == address(blockTokenInfos[0].token1),
		...
		if (_outputTokens.length == 2)
			require(
				address(_outputTokens[1]) ==
					address(blockTokenInfos[_blockIndex].token0) ||
					address(_outputTokens[1]) ==
					address(blockTokenInfos[0].token1),
				'Invalid output token'
			);
		...
		if (_blockIndex == 0) {
			...
			_sendOutputTokens(_outputTokens, _amount);
		} else {
			require(
				_amount == 0,
				'accum blocks do not have partial withdrawals'
			);
			_sendOutputTokens(_outputTokens, getUserBalances(msg.sender)[1]);
			userInfo[msg.sender].rewardsStartBlock = block.number;
		}
	}

	function _sendOutputTokens(IERC20[] calldata _outputTokens, uint256 _amount)
		internal
	{
		if (_outputTokens.length == 2) {
			uint256 amountToSend = _amount.div(2);
			...
			IMockToken(address(_outputTokens[1])).mintTo(
				msg.sender,
				_amount.sub(amountToSend)
			);
		} else {
			IMockToken(address(_outputTokens[0])).mintTo(msg.sender, _amount);
		}
	}
