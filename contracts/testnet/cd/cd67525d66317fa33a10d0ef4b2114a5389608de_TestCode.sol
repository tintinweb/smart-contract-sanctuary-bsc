...
contract TestCode is Context, Ownable {
    ...
    uint256 public EGGS_TO_HATCH_1MINERS = 1080000; // 86400/1080000 = 8% APY
	...
	//Tools to influence Selling Pressure. Could be used individually or in combination. ADJUSTED_REWARD_PERCENT, WHALE_TAX_PERCENT & CAPPED_DAILY_REWARD_AMOUNT
	uint256 public ADJUSTED_REWARD_PERCENT = 100;

	uint256 public WHALE_TAX_PERCENT = 100;                    // 100 = No tax. E.g. 75 = 0.75 or 25% tax
	uint256 public WHALE_TAX_CUTOFF = 200 * 1e16;              // 2 BNB/Day == 2 * 1e18 ==  200 * 1e16.  IF Reward is above 2 BNB/day, then get charged a whale tax
	// testing => uint256 public WHALE_TAX_CUTOFF = 2 * 1e15;  // IF Reward is above 2 BNB/day, then get charged a whale tax

	uint256 public CAPPED_DAILY_REWARD_AMOUNT = 500 * 1e16;    // 5 BNB/Day == 5 * 1e18 ==  500 * 1e16 . ~ $1.5k/day Max withdrawal. Update value to keep at $1.5k  **/
	// testing =>  uint256 public CAPPED_DAILY_REWARD_AMOUNT = 5 * 1e15; //testing

	uint256 public REFERRAL_PERCENT = 12;

    uint256 public marketEggs;

	uint256 public MARKETEGGS_BUY_INFLATION = 20;
	uint256 public MARKETEGGS_HATCH_INFLATION = 20;
	uint256 public MARKETEGGS_SELL_INFLATION = 100;

	uint256 public WITHDRAW_COOLDOWN = 24 * 60 * 60;    //  24 Hours. Withdraw cooldown
    ...
    function buyEggs(address ref) public payable {
		...
		eggsBought = applyBuyerBonus(msg.sender, eggsBought, msg.value);
		...
		uint256 totalEggs = SafeMath.add(eggsBought, eggsProducedByExistingMiners);
    ...
    function sellEggs() public {
		...
		//if whale tax is applicable
		if (WHALE_TAX_PERCENT < 100) {
			uint256 whaleEggValue = calculateTrade(myEggs, marketEggs, address(this).balance);  // Daily Reward value in BNB

			if (whaleEggValue > WHALE_TAX_CUTOFF) {  // check if classified as whale
					myEggs = SafeMath.div(SafeMath.mul(myEggs, WHALE_TAX_PERCENT), 100);  //adjust the eggs to apply whale tax
			}
		}
		...
		if (eggValue == CAPPED_DAILY_REWARD_AMOUNT) {  // exceeds daily withdrawal limit
				...
				myEggs= SafeMath.div(SafeMath.mul(eggValue,SafeMath.mul(PSN,marketEggs)), SafeMath.sub(SafeMath.mul(PSN,address(this).balance),PSN));
    	}
	...
	// getAvailableEarnings. Will give you current accumulated reward right now
    ...
    function calculateEggBuyWithBonus(uint256 eth) public view returns(uint256) {
		uint256 eggsBought = calculateEggBuy(eth, address(this).balance);
        return applyBuyerBonus(msg.sender, eggsBought, eth);
    }
	...
	/** How many miners and earningsPerDay user will recieve based on 1 BNB deposit **/
    function getEggsYield(uint256 amount) public view returns(uint256, uint256) {
        uint256 eggsAmount = calculateEggBuy(amount, getBalance().add(amount).sub(amount));
        uint256 miners = eggsAmount.div(EGGS_TO_HATCH_1MINERS);
        uint256 day = 1 days;
	...
	function Set_Whale_Tax(uint256 whale_tax) external onlyOwner {
        require(whale_tax >= 0 && whale_tax <= 100);
        // 85=> 0.85x of Reward. 75 => 0.75x of Reward. 100  = No Tax
        WHALE_TAX_PERCENT = whale_tax;
    }

	function Set_WhaleTax_Cutoff(uint256 whale_tax_cutoff) external onlyOwner {
		// 2 BNB/Day == 2 * 1e18  ==  200 * 1e16.
        require(whale_tax_cutoff >= 0);
        WHALE_TAX_CUTOFF = whale_tax_cutoff * 1e16;
    }
