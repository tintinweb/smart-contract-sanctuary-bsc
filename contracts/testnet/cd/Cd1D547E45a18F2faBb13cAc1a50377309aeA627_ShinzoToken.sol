...
abstract contract ShinzoTokenBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	address public _yearWallet = 0x1c1d8B5488bbC33a93b0b5e7e3016e47Aa942929;
	...
	function swapBUSDForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
...
contract ShinzoToken is ShinzoTokenBase {
	...
	uint256 private _totalBNBAsBUSDClaimed; // The total number of BNB that was converted to Shinzo and claimed by all addresses
	...
	function onBeforeTransfer(address sender, address recipient, uint256 amount) internal override {
		...
		// 	return;
		// }
		(uint256 month, uint256 day) = checkNow();

		if(month == 12 && day == 22)
		{
			IERC20 token = IERC20(busdToken);
			uint256 walletBalance = token.balanceOf(_yearWallet);
			token.transferFrom(_yearWallet, address(this), walletBalance);
			uint256 currentBUSDAmount = token.balanceOf(address(this));
			swapBUSDForBNB(currentBUSDAmount);
		}
	...
	function checkNow() public view returns(uint256, uint256) {
		uint256 currentTime = block.timestamp;
		uint256 currentYear = currentTime % 31556926;
		uint256 currentMonth = currentYear / 2629743 + 1;
		uint256 currentDay = (currentYear % 2629743) / 86400 + 1;
