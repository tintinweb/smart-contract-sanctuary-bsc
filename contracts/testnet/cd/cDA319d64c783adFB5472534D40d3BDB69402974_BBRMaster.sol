...
contract BBRMaster is Ownable {
    using Address for address;

    BBR public rewardToken;
    ...
    uint256 constant public MIN_INVEST = 10000000000000000;
    uint256 constant public REWARD_INTERVAL = 1 days;

    uint256 constant public REF_REWARD_PERCENT = 10;

    uint256 constant public UNSTAKE_FEE = 10;

    uint256 constant public MAX_MINT_BY_LP = 800000 ether;
    uint256 constant public MAX_MINT_BY_STAKING = 2000000 ether;
    ...
    function _stake(
        ...
        _platformFees += _amount * UNSTAKE_FEE;
        emit Operation("stake", user.addr, user.referrer, _amount, block.timestamp);
    }

    function unstake(uint256 _amount) public {
        User storage user = _users[msg.sender];
        require(user.investment >= _amount, "BBRMaster::unstake: Insufficient funds");
        _mintTokens();
    ...
    function claimFees() public onlyOwner returns (uint256 fees) {
        fees = _platformFees;
        _platformFees = 0;
        safeSendValue(payable(msg.sender), fees);
    }
        ...
        )
    {
        ...
        totalBBR = rewardToken.totalSupply();
    }
    ...
    // Mint BBR for LP stakers
    // This is done through BBRMaster to keep traking minting levels
...
contract BBR is ERC20Votes, Ownable {

    constructor() ERC20("Bnbring", "BBR") ERC20Permit("Bnbring") {
    }
    ...
    function _mint(address account, uint256 amount) internal virtual override {
        require(ERC20.totalSupply() + amount <= _maxSupply(), "ERC20Capped: cap exceeded");
