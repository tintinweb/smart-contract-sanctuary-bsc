...
contract ChainpalPresale is ReentrancyGuard, Ownable, Sale {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    IBEP20 public CHPToken; // the token being sold
    ...
    uint256 public day = 300; // day in seconds(for testing only)
    ...
    /**
     * This values(_CHPToken,_USDTToken,_BUSDToken) is immutable: it can only be set once during
     * construction.
     */
    constructor(
        IBEP20 _CHPToken,
        ...
        uint256 _minimumPurchaseForReferralReward,
        ...
        minimumPurchaseForReferralReward = _minimumPurchaseForReferralReward;
    }
        ...
        returns (bool)
    {
        ...
        if (storeUserDetails(numberOfTokens)) {
            CHPToken.safeTransfer(address(msg.sender), transaferableAmount);
    ...
    /**
     * function to get available CHP Tokens for pre-sale.
     **/
    ...
    function withdrawFunds() external onlyOwner returns (bool) {
        ...
        CHPToken.safeTransfer(
            address(treasuryWallet),
            CHPToken.balanceOf(address(this))
        );
