...
contract Cheddar is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   DividendTracker public dividendTracker;
    ...
    uint256 private constant DefaultTime = 1 weeks;
    ...
    constructor() ERC20 ("Cheddar", "GOUDA") {
        _buyRewardsFee = 6;
        _buyLiquidityFee = 6;
		_buyMarketingFee = 6;
        _buyDevFee = 1;
		...
		marketingWallet = payable(0x1417d1aa0568E511fae5701788aAEFe431De9DAC);
        devWallet1 = payable(0x8DB2da1Fd6AdC5a92e4e27aDFAaca102a1a28db0);
        devWallet2 = payable(0xC6EeE21b5474766F9671C17eE8435Bc02CaDAC1B);
    ...
    function updatedividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "Cheddar The dividend tracker already has that address");

        DividendTracker newdividendTracker = DividendTracker(payable(newAddress));

        require(newdividendTracker.owner() == address(this), "Cheddar: The new dividend tracker must be owned by the Cheddar token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "Cheddar: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Cheddar: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Cheddar: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Cheddar: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Cheddar: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "Cheddar: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "Cheddar: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Cheddar: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cheddar: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
		...
		if(canSwap && !swapping && !automatedMarketMakerPairs[from] && from != liquidityWallet && to != liquidityWallet && from != marketingWallet && to != marketingWallet && !_isExcludedFromFees[to] && !_isExcludedFromFees[from]) {
		    ...
		    uint256 swapTokens;
			uint256 liquidityAmount = contractTokenBalance.mul(_sellLiquidityFee).div(totalSellFees);
			uint256 half = liquidityAmount.div(2);
			uint256 otherHalf = liquidityAmount.sub(half);
            
            swapping = true;

            if (totalSellFees > 0 && _sellLiquidityFee > 0) {
            swapTokens = contractTokenBalance.sub(half);
            swapTokensForEth(swapTokens);
            }

            else if(totalSellFees > 0 && _sellLiquidityFee == 0) {
            ...
            uint256 devAmount = address(this).balance.mul(_sellDevFee).div(totalSellFees);
            uint256 devSplit = devAmount.div(2);
            devAmount = devAmount.sub(devSplit);
            (bool success, ) = devWallet1.call{value: devAmount}("");
            (success, ) = devWallet2.call{value: devSplit}("");
            require(success, "Failed to send dev amount");
            }

			if (_sellLiquidityFee > 0) {
			    
		    uint256 newBalance = address(this).balance.mul(_sellLiquidityFee).div(totalSellFees);
            ...
            uint256 sellTokens = address(this).balance.mul(_sellRewardsFee).div(totalSellFees);
	...
	function letsGoLive() external onlyOwner() {
        updateBuyFees(6,6,6,1);
        updateSellFees(6,6,6,1);
    ...
    function burnRemainingToken() public onlyOwner {
        uint256 balance = balanceOf(address(this));
        super._transfer(address(this), deadAddress, balance);
    }
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("Cheddar_Dividend_Tracker", "Cheddar_Dividend_Tracker", DividendToken) {
    	claimWait = 0;
