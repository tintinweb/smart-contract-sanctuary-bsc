...
contract StakingRouter is Ownable {
    ...
    function transferRewards(
        ...
        IERC20(_r).transfer(user, rewards);
    ...
    {
        uint256 count = users[msg.sender].stakedCollections.length();
        collections = new address[](count);
        rewards = new uint256[](count);
        tokens = new uint256[](count);
        for (uint256 i = 0; i < count; i++) {
            address c = users[msg.sender].stakedCollections.at(i);
            collections[i] = c;
            rewards[i] = users[msg.sender].earnedRewards[c];
            tokens[i] = users[msg.sender].stakedTokens[c];
        }
    }
