...
contract PharaohFarmPoTFull is Ownable , ReentrancyGuard {
    ...
    IERC20 CAFToken = IERC20(0xC071C73Fae079aA7116A39E785B0c30d4130c0b5);
    struct User {
        address userAddress;
    ...
    function deposit(uint256 amount, address referAddress) public nonReentrant {
        ...
        uint256 max = user_data[referAddress].maxPayout;
        uint256 claimed = user_data[referAddress].claimed;
        uint256 available = availableRewards(referAddress);
        uint256 refRew =   (amount * refRewardLevels[0]) /
        1000;
        ...
        for (uint256 i = 0; i <= 6; i++) {
            if (uplineUserAddress != address(0)) {
                if (currentDistributionLevel < 7) {
                    ...
                    uint256 max = user_data[uplineUserAddress].maxPayout;
                    uint256 claimed = user_data[uplineUserAddress].claimed;
                    uint256 available = availableRewards(uplineUserAddress);
        ...
        for (uint256 i = 0; i <= 6; i++) {
            if (uplineUserAddress != address(0)) {
                if (currentDistributionLevel < 7) {
                    ...
                    uint256 claimed = user_data[uplineUserAddress].claimed;
                    uint256 available = availableRewards(uplineUserAddress);
                    if (available + claimed < max || uplineUserAddress == firstAddress) {
                        emit ReferralRewarded(msg.sender, uplineUserAddress,rew);
                        user_data[uplineUserAddress].referralRewards += rew;
                    }
                    currentDistributionLevel++;
    ...
    function claimRewardsFirstAddress() public nonReentrant{
    ...
    uint256 toClaim;
        ...
        //Changed for Testing
        require(
            block.timestamp - lastClaim / 1 minutes >= 1 minutes,
 ...
 if (interest + claimed > maximumInterest) {
    ...
    function doubleUp() public nonReentrant {
        ...
        uint256 _max = user_data[oldRef].maxPayout;
        uint256 _claimed = user_data[oldRef].claimed;
        uint256 available = availableRewards(oldRef);
        if (available + _claimed < _max || oldRef == firstAddress) {
            uint256 _refRew = (finalAmount * refRewardLevels[0]) /
            1000;
            user_data[oldRef].referralRewards +=_refRew;
                emit ReferralRewarded(msg.sender, oldRef,_refRew); 
    ...
    function calculatePayout(address caller) public view returns (uint256) {
        ...
        uint256 durationInMinutes = (block.timestamp - deposit_time) / 1 minutes;
        ...
        if (contractBalance >= 10000000 * 1e18) {
            finalValue = (_deposit.mul(1e16).mul(durationInMinutes).div(1e18)).div(1440);
            return finalValue;
        } else if (contractBalance >= 5000000 * 1e18) {
            finalValue = (_deposit.mul(75e14).mul(durationInMinutes).div(1e18)).div(1440);
            return finalValue;
        } else if (contractBalance >= 2500000 * 1e18) {
            finalValue = (_deposit.mul(5e15).mul(durationInMinutes).div(1e18)).div(1440);
            return finalValue;
        } else {
            finalValue =( _deposit.mul(3e15).mul(durationInMinutes).div(1e18)).div(1440);
            return finalValue;
        }
    }
