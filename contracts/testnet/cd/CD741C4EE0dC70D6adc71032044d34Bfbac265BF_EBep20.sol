...
contract EBep20 is EToken, EBep20Interface {
    ...
    //  */
    function start(address underlying_,
        ...
        // startInit(comptroller_, interestRateModel_, initialExchangeRateMantissa_, name_, symbol_, decimals_, depositAllowed_, borrowAllowed_, onlyWhiteListAllowToDeposit_, whiteListDepositAccount_);
        ...
        //i initialExchangeRateMantissa = initialExchangeRateMantissa_;
        //i require(initialExchangeRateMantissa > 0, "initial exchange rate must be greater than zero.");

        // Set the comptroller
        //k uint err = _setComptroller(comptroller_);
        //k require(err == uint(Error.NO_ERROR), "setting comptroller failed");

        // Initialize block number and borrow index (block number mocks depend on comptroller being set)
        //k accrualBlockNumber = getBlockNumber();
        borrowIndex = mantissaOne;

        // Set the interest rate model (depends on block number / borrow index)
        //k err = _setInterestRateModelFresh(interestRateModel_);
        //k require(err == uint(Error.NO_ERROR), "setting interest rate model failed");
...
abstract contract EToken is ETokenInterface, Exponential, TokenErrorReporter {
    ...
    //  */
    function startInit(ComptrollerInterface comptroller_,
                        InterestRateModel interestRateModel_,
                        uint initialExchangeRateMantissa_,
                        string memory name_,
                        string memory symbol_,
