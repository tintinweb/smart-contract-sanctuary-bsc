...
contract Busdclub {
    ...
    uint[5] club_level2 = [60e18, 720e18, 8640e18, 103680e18, 3732450e18];
    modifier onlyAdmin(){
    ...
    function _setClub(uint256 poolNo,address _addr) private{
        ...
        if(players[up].b5_level[poolNo][0]==3 && players[up].clubIncome[poolNo][0]==0){
        ...
        if(up1 != address(0)){
            ...
            if(up2 != address(0)){
                ...
                if(players[up2].b5_level[poolNo][2]>=27 && players[up2].clubIncome[poolNo][1]==0){
    ...
    function incomeDetails(address _addr) view external returns(uint256 totalInc,uint256 wdl,uint256[20] memory lvinc,uint256[5] memory clubinc1,uint256[5] memory clubinc2) {
        for(uint8 i=0;i<20;i++){
