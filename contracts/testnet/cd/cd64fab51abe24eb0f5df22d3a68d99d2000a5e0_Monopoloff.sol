...
contract Monopoloff is Context, IERC20, Ownable {
    ...
    string private _name = "Monopoloff";
    ...
    uint256 private _previousLiquidityFeeBuy = _liqFeeBuy;
    ...
    address public marketingWallet = 0x59249FE2E748EDED65131a7941e4d21AAdDCC7f2;
    ...
    uint256 public numTokensSellToAddToLiquidity = 1 * 10**1 * 10**9;
    ...
    constructor() {
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xBBe737384C2A26B15E23a181BDfBd9Ec49E00248);//0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function _transfer(
        ...
        if (
            ...
            //distinguish buys and sells
        if(from == uniswapV2Pair) {
            _taxFee = _taxFeeBuy;
            _liquidityFee = _liqFeeBuy;
    ...
    function prepareForPresale() external onlyOwner() {
        ...
        _previousmarketingFeeBuy = 0;
        _previousBurnFeeBuy = 0;

        _previousTaxFeeSell = 0;
        _previousLiquidityFeeSell = 0;
        _previousmarketingFeeSell = 0;
        _previousBurnFeeSell = 0;
    ...
    function setBuyFees(uint taxFee, uint liquidityFee, uint marketingFee, uint burnFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(marketingFee).add(burnFee)<=25, "Buy fees can't be above 25 percent.");
        _taxFeeBuy = taxFee;
        _liqFeeBuy = liquidityFee;
        _marketingFeeBuy = marketingFee;
        _burnFeeBuy = burnFee;
    }

     function setSellFees(uint taxFee, uint liquidityFee, uint marketingFee, uint burnFee) external onlyOwner() {
        require(taxFee.add(liquidityFee).add(marketingFee).add(burnFee)<=25, "Sell fees can't be above 25 percent.");
        _taxFeeSell = taxFee;
        _liqFeeSell = liquidityFee;
