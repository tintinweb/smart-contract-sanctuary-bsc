...
import "../Staking/IMITStaking.sol" ;
...
contract UpgradeManager is BaseManager, IERC721Receiver {
    ...
    IMITStaking public iMITStaking ;
    ...
    // quanlity rate
    uint256 [] public upgradeRate = [10, 20, 30, 40, 50] ;
    ...
    constructor(address sign, address iMITStakingAddr, address mitAddr, address spaceshipAddr,
        ...
        iMITStaking = IMITStaking(iMITStakingAddr) ;
    ...
    event ClaimEvent(uint256 upgradeId, bool success) ;
    ///////////////////////////////////////////
    //          config manager
    ///////////////////////////////////////////
    ...
    function setMITStaking(address mitStakingAddr) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        iMITStaking = IMITStaking(mitStakingAddr) ;
        return true;
    }
    ...
    function upgrade(uint256 mainTokenId, uint256 extraTokenId, KIND kind) external nonReentrant returns(bool) {
        ...
        if(kind == KIND.SPACESHIP) {
            ...
            require(mainTokenDecodeGens[0] < 5, "Your Spaceship NFT quality is already top!") ;
            ...
            require(mainTokenDecodeGens[0] < 5, "Your DefensiveFacility NFT quality is already top!") ;
            ...
            require(mainTokenDecodeGens[0] < 5, "Your Hero NFT quality is already top!") ;
    ...
    function claim(uint256 [] memory upgradeIds) external nonReentrant returns(bool) {
        require(upgradeIds.length > 0, "The NFT to be upgraded cannot be empty!") ;
        for(uint256 i = 0; i < upgradeIds.length; i++) {
            UpgradeRecord memory upgradeRecord = upgradeRecords[upgradeIds[i]] ;

            require(upgradeRecord.claimBlockNo <= block.number, "Your NFT is being upgraded, please try again later!") ;
            require(upgradeRecord.account == _msgSender() && upgradeRecord.isClaimed == false, "Your upgraded NFT has been claimed!") ;
            uint256 blockHash = uint256(blockhash(upgradeRecord.claimBlockNo)) ;
            uint256 upgradeRateVal = blockHash % 100 ;
            ...
            upgradeRecords[upgradeIds[i]].isClaimed = true ;

            // emit
            emit ClaimEvent(upgradeIds[i], isSuccess) ;
        }
