...
contract CheckPlz is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(autoBuyback){
            if(address(this).balance > 0 && buybackAmount <= address(this).balance){
                buyBack();
            }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // add liquidity
        uint256 balanceForLiquidity = newBalance.sub(oldBalance);

        MarketingTokens = addLiquidity(otherHalf, balanceForLiquidity);
    ...
    function buyBack() internal {
        ...
        require(buybackAmount <= balance, "BuyBack: Contract has not enought BNB.");

        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();

        // swap bnbValue of eth/bnb to token (contract buy itself and reserve the tokens in contract)
    ...
    function triggerAutoByback(bool status) public onlyOwner {
        autoBuyback = status;
    }
    ...
    function setBuybackAmount(uint256 amount) public onlyOwner {
