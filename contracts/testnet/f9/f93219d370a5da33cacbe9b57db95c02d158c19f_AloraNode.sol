...
interface IUniswapV2Factory {
...
} contract AloraNode is ERC20Upgradeable {
  using SafeMath for uint256;
  function initialize() public initializer {
    __ERC20_init("AloraNode", "ALORA");
    owner = msg.sender;
    operator = msg.sender;
  ...
  uint256 public toDepositAmount;
  ...
  function setTransferTaxRate(uint32 _transferTaxRate) public onlyOwner {transferTaxRate = _transferTaxRate;}
  function buyFee() public view returns (uint32) {return buyBackFee;}
  function setBuyFee(uint32 value) public onlyOwner {buyBackFee = value;}
  function sellFee() public view returns (uint32) {return sellBackFee;}
  function setSellFee(uint32 value) public onlyOwner {sellBackFee = value;}
  function setOperator(address account) public onlyOwner {operator = account;}
  function setOperatorFee(uint32 value) public onlyOwner {operatorFee = value;}
  function setLiquidityFee(uint32 value) public onlyOwner {liquidityFee = value;}
  function setExcludedFromFee(address account) public onlyOwner {isExcludedFromFee[account] = true;}
  function removeExcludedFromFee(address account) public onlyOwner {isExcludedFromFee[account] = false;}
  function setMinAmountToLiquify(uint256 value) public onlyOwner {minAmountToLiquify = value;}
  function setMaxTransferAmount(uint256 value) public onlyOwner {maxTransferAmount = value;}
  function setMaxWalletAmount(uint256 value) public onlyOwner {maxWalletAmount = value;}
  function setNodeManagerAddress(address _nodeManagerAddress) public onlyOwner {nodeManagerAddress = _nodeManagerAddress;}
  function setCheckNodeBeforeSell(bool check) public onlyOwner {checkNodeBeforeSell = check;}
  function _transfer(
    address from,
    address to,
    uint256 amount
    ...
    bool _isSwappable = address(uniswapV2Router) != address(0) &&
      uniswapV2Pair != address(0);
    bool _isBuying = _isSwappable &&
      msg.sender == address(uniswapV2Pair) &&
      from == address(uniswapV2Pair);
    bool _isSelling = _isSwappable &&
    ...
    //     require(
    //       address(mgr.feeManager()) == from || mgr.countOfUser(from) > 0,
    ...
    if (
      ...
      require(balanceOf(to).add(_amount) <= maxWalletAmount,"Overflow maxWallet amount");
    }
  ...
  function swapAndLiquify() private lockTheSwap transferTaxFree {
    ...
    if (contractTokenBalance >= accumulatedOperatorTokensAmount) {
      contractTokenBalance = contractTokenBalance.sub(
        accumulatedOperatorTokensAmount
      );
  ...
  function claimOtherTokens(address anyToken, address recipient)
    external
    onlyOwner
  {
  ...
  function mintReserveAllocation(address _reserveAllocation, uint256 _amount)
    public
    onlyOwner
  {
    ...
    require(
      _maxCapReserveAllocation >= reserveAllocationAmount,
      "RA:Minted exceed limite"
    );
  ...
  function mintTeamAllocation(address _teamAllocation, uint256 _amount) public onlyOwner {
    ...
    require(
      _maxCapTeamAllocation >= teamAllocationAmount,
      "TA:Minted exceed limite"
    );
  ...
  function setStableToken(address _stableToken) public onlyOwner {stableToken = _stableToken;}
  function setDepositAmount(uint256 _toDepositAmount) public onlyOwner {toDepositAmount = _toDepositAmount;}
  function whiteLister(bytes32[] calldata merkleProof) public view {require(MerkleProof.verify(merkleProof,merkleRoot,keccak256(abi.encodePacked(msg.sender))),"Invalid WhiteLister");}
  function setWhiteListerfund(address _whiteListerfund) public onlyOwner {whiteListerfund = _whiteListerfund;}
  function depositToken(bytes32[] calldata merkleProof) public {
    require(
      MerkleProof.verify(
        merkleProof,
        merkleRoot,
    ...
    require(
      IERC20(stableToken).balanceOf(msg.sender) >= toDepositAmount,
    ...
    depositAmount[msg.sender] += toDepositAmount;
    _totalDeposit += toDepositAmount;
  }
  //
}