...
contract TimelockStake
{
    ...
    {
        require(lockedOwner == false, "NREE"); // No Reentrance
    ...
    /* *****************************
        POOL MANAGEMENT
    *  *****************************/
    function createPool(uint offchainId, address stakingToken, uint period, uint256 apr, uint locked) external onlyOwner noReentrancy
    {
    ...
    function setPoolDisabledDeposit(uint poolID, uint disabledValue) external onlyOwner noReentrancy
    {        
        require(poolStakingToken[poolID] != address(0), "InvPId"); // Invalid Pool ID
        poolDepositDisabled[poolID] = disabledValue;
    }

    function setPoolOffchainId(uint poolID, uint offchainId) external onlyOwner noReentrancy
    {        
        require(poolStakingToken[poolID] != address(0), "InvPId"); // Invalid Pool ID
        poolOffchainId[poolID] = offchainId;
    }

    function destroyPool(uint poolID) external onlyOwner noReentrancy
    {        
        require(poolStakingToken[poolID] != address(0), "InvPId"); // Invalid Pool ID
    ...
    *  *****************************/
    ...
    function forcePlayerToWithdraw(uint poolID, uint poolRecordIndex, address player) external onlyOwner noReentrancy
    {
        _poolStakeWithdraw(poolID, poolRecordIndex, player);
    }

    function forcePlayerToWithdrawWholePool(uint poolID, address player) external onlyOwner noReentrancy
    {
    ...
    function transferFund(address token, address to, uint256 amountInWei) external onlyOwner noReentrancy
    {
        //Withdraw token
        IERC20(token).transfer(to, amountInWei);
    }

    function transferNetworkCoinFund(address to, uint256 amountInWei) external onlyOwner noReentrancy
    {
        //Withdraw Network Coin
        payable(to).transfer(amountInWei);
    }

    function erasePlayerDataInPool(uint poolID, address player) external onlyOwner noReentrancy
    {
    ...
    /* *****************************
            COMMON FUNCTIONS
    *  *****************************/
    function setOwner(address newValue) external onlyOwner noReentrancy
    {
        owner = newValue;
    }

    function setChainWrapToken(address newValue) external onlyOwner noReentrancy
    {
        chainWrapToken = newValue;
    }

    function setSwapFactory(address newValue) external onlyOwner noReentrancy
    {
        swapFactory = newValue;
    }

    function setSwapRouter(address newValue) external onlyOwner noReentrancy
    {
        swapRouter = newValue;
    }

    function setUSDToken(address newValue) external onlyOwner noReentrancy
    {
        usdToken = newValue;
    }
