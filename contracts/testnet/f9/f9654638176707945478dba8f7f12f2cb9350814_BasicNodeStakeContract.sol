...
contract BasicNodeStakeContract is Ownable {
    ...
    function myReferralReward(uint256 _pid,address _user) public view returns(uint256 totalReferralBonus){
        ...
        for (uint256 i = 0; i < myInviteeAddress.length; i++) {
            uint256 referralBonus = pendingReward(_pid,myInviteeAddress[i]).mul(bonusPerReferral).div(referralRateDenominator).add(userInfo[_pid][myInviteeAddress[i]].totalMining);
    ...
    function receiveMyReferralReward(uint256 _pid) public {
        ...
        if(pendingReferralReward > 0){
            ...
            userInfo[_pid][msg.sender].receivedReferralReward = pendingReferralReward;
        }
    }
    ...
    function claim(uint256 _pid) public {
        ...
        user.totalMining = user.totalMining.add(deservedReward);
    ...
    function getMyDepositInfo(uint256 _pid, address _user,uint256 _index) public view returns (DepositInfo memory){
        return userInfo[_pid][_user].depositInfo[_index];
    }

    function getMyDepositInfoArrayLength(uint256 _pid, address _user) public view returns (uint256){
        return userInfo[_pid][_user].depositInfo.length;
    }
