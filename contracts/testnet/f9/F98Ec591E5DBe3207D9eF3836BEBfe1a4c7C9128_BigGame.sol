...
contract BigGame is Ownable {
    ...
    // change scope to private in production
    mapping (address => User) public _users; // mapping of users

    // change scope to private in production
    mapping (uint256 => uint256) public _fractalBonus; // bonus amount collected for each fractal
    ...
    //testnet
    address private BUSD = 0xdB1Cc97ada0D2A0bCE7325699A9F1081C95F0ac9;
    ...
    function registerUser(address _account, address _ref) internal {
        if (_ref == address(0)) _ref = _idToAddress[1];
        _lastUserId += 1;
    ...
    // change scope to internal in production
    function userRegistered(address _account) public view returns (bool) {
        return _users[_account].id > 0;
    }

    // change scope to internal in production
    function hasFractal(address _account, uint256 _fractal, uint256 _level) public view returns (bool) {
        return _users[_account].fractals[_fractal][_level].timestamp > 0;
    }

    // change scope to internal in production
    function isInArray(address _account, address[] memory _array) public pure returns(bool) {
    ...
    // change scope to internal in production
    function getMatrixLength(address _account, uint256 _fractal, uint256 _level) public view returns(uint256) {
        return _users[_account].fractals[_fractal][_level].firstLevelUsers.length;
    }


    // change scope to internal in production
    function findFreePlace(address _account, uint256 _fractal, uint256 _level) public view returns(address) {
        address upline;
    ...
    function buyFractal(uint256 _fractal) public {
        if (userRegistered(_msgSender())) {
            buyFractal(_fractal, _users[_msgSender()].partner);
        } else {
            buyFractal(_fractal, address(0));
        }
    }


    function buyFractal(uint256 _fractal, address _ref) public {
        if (!userRegistered(_msgSender())) {
            registerUser(_msgSender(), _ref);
        } else {
            _ref = _users[_msgSender()].partner;
        }
    ...
    // change scope to internal in production
    function getAmounts(uint256 _fractal, uint256 _level, uint256 _amount) public pure returns(uint256, uint256, uint256) {
        uint256 bonus;
        uint256 lOnePayment;
        if (_level == 1) {
            bonus = _amount / 4;
    ...
    // change scope to internal in production
    function getIdMatrix(address _account, uint256 _fractal, uint256 _level) public view returns(uint256[] memory) {
        uint256 l1Length = getMatrixLength(_account, _fractal, _level);
        uint256 l2Length = _users[_account].fractals[_fractal][_level].secondLevelUserCount;
        uint256[] memory m = new uint256[](l1Length + l2Length);
        for (uint i = 0; i < l1Length; i++) {
            m[i] = _users[_users[_account].fractals[_fractal][_level].firstLevelUsers[i]].id;
        }
        uint total = l1Length;
        address user;
        for (uint i = 0; i < l1Length; i++) {
            user = _users[_account].fractals[_fractal][_level].firstLevelUsers[i];
            for (uint j = 0; j < getMatrixLength(user, _fractal, _level); j++) {
                m[total] = _users[_users[user].fractals[_fractal][_level].firstLevelUsers[j]].id;
                total++;
            }
        }
        return m;
