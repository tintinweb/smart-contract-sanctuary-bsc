 ...
 */
contract NFTExchange is ILooksRareExchange, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                0x28a6a464620699b212626df3adedba9bb3fc311bfb3dece6448c8c9c73b2f2fb, // keccak256("NFTExchange")
    ...
    function _transferFeesAndFunds(
        ...
        // 2. Royalty fee
        {
            (address[] memory royaltyFeeRecipient, uint256[] memory royaltyFeeAmount) = royaltyFeeManager
                .calculateRoyaltyFeeAndGetRecipient(collection, tokenId, amount);

            // Check if there is a royalty fee and that it is different to 0
            for (uint i = 0; i < royaltyFeeRecipient.length; i++) {
                if ((royaltyFeeRecipient[i] != address(0) && (royaltyFeeAmount[i] != 0))) {
                    IERC20(currency).safeTransferFrom(from, royaltyFeeRecipient[i], royaltyFeeAmount[i]);
                    finalSellerAmount -= royaltyFeeAmount[i];

                    emit RoyaltyPayment(collection, tokenId, royaltyFeeRecipient[i], currency, royaltyFeeAmount[i]);
                }
            }
        }
    ...
    function _transferFeesAndFundsWithWETH(
        ...
        // 2. Royalty fee
        {
            ...
            for (uint i = 0; i < royaltyFeeRecipient.length; i++) {
                // Check if there is a royalty fee and that it is different to 0
                if ((royaltyFeeRecipient[i] != address(0) && (royaltyFeeAmount[i] != 0))) {
                    IERC20(WETH).safeTransfer(royaltyFeeRecipient[i], royaltyFeeAmount[i]);
                    finalSellerAmount -= royaltyFeeAmount[i];

                    emit RoyaltyPayment(collection, tokenId, royaltyFeeRecipient[i], address(WETH), royaltyFeeAmount[i]);
                }
            }
        }
