...
import "./Create2ContractsFactoryv2.sol";
import "../interfaces/factory/ICreate2BlueprintContractsFactoryv2.sol";

contract Create2BlueprintContractsFactoryv2 is
    Create2ContractsFactoryv2,
    ICreate2BlueprintContractsFactoryv2
{
    mapping(string => uint16) internal latestBlueprintVersion;

    mapping(string => mapping(uint16 => address)) internal blueprints; //name => version => blueprint address

    mapping(string => UpgradeableBeacon) internal beacons; //name -> beacon

    mapping(string => bool) internal blueprintNative;
    ...
    function registerNewBlueprint(
        ...
        string memory blueprintName // bool isNative
    ) external virtual override {
        _ensureHasRole(
            systemContext.omniteAccessControl().CONTROL_LIST_ADMIN_ROLE(),
            msg.sender
        );
        if (blueprintExists(blueprintName)) {
    ...
    function _deployProxy(
        ...
        if (!success) {
            revert InitializeFailed(newContract);
        }
    ...
    function deployFromBlueprint(
        ...
        return (newContract, latestBlueprintVersion[blueprintName]);
    }
