...
contract BNBDistributor is IDividendDistributor {
    ...
    constructor (address _wbnb, address _router, address _marketingReceiver, address _liquidityReceiver) {
        ...
        _stakingMagnitude = 1 * 10 ** (9 + 18); // 1 Billion
    }
    ...
    function setLpAcquifeeReceiver(address _receiver) external onlyToken {
        liquidityReceiver = _receiver;
    }

    function donate(address shareholder) onlyToken external {
        distributeDividend(shareholder, DEAD);
    }
...
contract MetaBull is Context, IBEP20, Ownable {
    using SafeMath for uint256;

    address WBNB = 0x6a1262783EfADcbCf9905B149F78765db06bD7a0;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEAD = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "MetaBull";
    string constant _symbol = "MBULL";
    uint8 constant _decimals = 18;

    uint256 _totalSupply = 1 * 10 ** (9 + _decimals); // 1 Billion
    ...
    constructor() {
        ...
        address upgradeContract = 0xA2fbe713FCD49d038eF348E5bA23a9549d072A8a;
        isFeeExempt[upgradeContract] = true;
        isDividendExempt[upgradeContract] = true;

        
        
        autoLiquidityReceiver = 0x6a1262783EfADcbCf9905B149F78765db06bD7a0;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (shouldTakeFee(sender, recipient)) {
            ...
            uint256 _bnbFee = amount.mul(reward).div(feeDenominator);
            uint256 _LpAcquifee = amount.mul(LpAcquifee).div(feeDenominator);

            uint256 _totalFee = _marketingFee + _burnFee + _bnbFee + _LpAcquifee; // Total token fee
    ...
    function swapBack() internal swapping {
        ...
        uint256 denom = LpAcquifee + burnFee + marketingFee + reward;

        uint256 marketingSwap = totalAmount.mul(marketingFee).div(denom);
        uint256 charitySwap = totalAmount.mul(burnFee).div(denom);
        uint256 bnbSwap = totalAmount.mul(reward).div(denom);
        uint256 liquiditySwap = totalAmount.mul(LpAcquifee).div(denom);
        ...
        if (swapedBNBAmount > 0) {
            uint256 bnbDenom = LpAcquifee.div(2) + burnFee + marketingFee + reward;
            ...
            uint256 bnbSwapBnbAmount = swapedBNBAmount.mul(reward).div(bnbDenom); // BNB for BNB Rewards
            uint256 bnbLiquidify = swapedBNBAmount.mul(LpAcquifee.div(2)).div(bnbDenom); // BNB for Liqudity
    ...
    function setLpAcquifeeReceiver(address _receiver) external onlyOwner {
        autoLiquidityReceiver = _receiver;
        bnbDistributor.setLpAcquifeeReceiver(_receiver);
    ...
    function buyMetaBullWithReward() external {
    ...
    // only for recovering tokens that are NOT Metabull tokens sent in error by wallets
