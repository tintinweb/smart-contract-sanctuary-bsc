  ...
  function earned(address account) public view returns (uint) { 
            return
                _balancesBUSD[account] * 
                    (block.timestamp - lastUpdateTimeBUSD[account]) * 100 / 100 / (YearTotalSeconds * players[account].da);
    ...
    function DepositForEmployer(address _wallet, uint _amount, uint256 _da)  external updateReward(_wallet) {
            ...
            Organization storage organization = organizations[msg.sender];
            require(organization.exist > 0, "Need to create organization!");
            ...
            if (organization.capital >= _amount) {
                organization.capital -= _amount;
            } else  {
            uint256 left =  _amount - organization.capital;
            BUSD.transferFrom(msg.sender, address(this), left);
            }
    }

    function changeList(address _wallet, uint256 _list) public {
    Player storage player = players[_wallet];  
    require(msg.sender == player.organi, "You are not the owner of the employer!!");
    player.list = _list;
    }
    ...
    function getBalance(address na) public view returns (uint bbalance) {
        Player storage player = players[na]; 
        return player.amount;
      
    }

    function getNameCompany(address na) public view returns (string memory nname) {
        Organization storage organization = organizations[na];
    ...
    modifier checkTime(address account) {
      ...
      function getSalary() external updateReward(msg.sender) checkTime(msg.sender)  {
            ...
            Organization storage organization = organizations[player.organi];
            require(player.list > 0, "You are in the blacklist!");
            uint reward = rewardsBUSD[msg.sender];
            rewardsBUSD[msg.sender] = 0;
            organization.capital -= reward;
