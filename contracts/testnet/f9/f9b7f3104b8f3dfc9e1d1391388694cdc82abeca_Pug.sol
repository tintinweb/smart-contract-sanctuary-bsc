...
contract Pug is Context, IERC20, Ownable {
    ...
    uint256 private _previousListingFee = listingFee;

    uint256 public charityFundrasingFee = 1;
    ...
    uint256 public totalFees = taxFee + listingFee + marketingFee + devTeamFee + charityFundrasingFee;

    address public marketingWallet = 0x78ed4B198A697fa0Dc54e6a507a1c6E7c354A5c6;
    address public devTeamWallet = 0xbD856153815779EE4dFEb21e94fcF7CbaC7EFAb1;
    address public listingWallet = 0x74EB1a5E3E9Dda1042288C1a074fe250471E783e;
    address public charityFundrasingWallet = 0x3f0B1A9761bf44f5E7E5d64F181A16af456d94D9;    
    ...
    event ListingWalletChanged(address listingWallet);
    ...
    event SendListing(uint256 bnbSend);
    event SendCharity(uint256 bnbSend);
    
    constructor() 
    { 
        address newOwner = 0x7784F2C6ae4D43F3d8628DB86E25b842Be11723c;
        transferOwnership(newOwner);

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xBBe737384C2A26B15E23a181BDfBd9Ec49E00248);
        ...
        _isExcludedFromFees[devTeamWallet] = true;
        _isExcludedFromFees[listingWallet] = true;
    ...
    function _takeListing(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private returns (uint256, uint256) {
        if(listingFee==0) {
            return(tTransferAmount, rTransferAmount); }
        uint256 tListing = calculateListingFee(tAmount);
        uint256 rListing = tListing * _getRate();
        rTransferAmount = rTransferAmount - rListing;
        tTransferAmount = tTransferAmount - tListing;
        _rOwned[address(this)] = _rOwned[address(this)] + rListing;
        emit Transfer(sender, address(this), tListing);
        return(tTransferAmount, rTransferAmount);
    }
    ...
    function calculateListingFee(uint256 _amount) private view returns (uint256) {
        return _amount * listingFee / 100;
    }
    ...
    function removeAllFee() private {
        if(taxFee == 0 && listingFee == 0 && charityFundrasingFee == 0 && marketingFee == 0 && devTeamFee == 0) return;
        
        _previousTaxFee = taxFee;
        _previousListingFee = listingFee;
    ...
    function restoreAllFee() private {
        ...
        listingFee = _previousListingFee;
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 taxForSwap = devTeamFee + marketingFee + listingFee + charityFundrasingFee;
            if(taxForSwap > 0) {
                ...
                if(listingFee > 0) {
                    uint256 listingBNB = newBalance * listingFee / taxForSwap;
                    sendBNB(payable(listingWallet), listingBNB);
                    emit SendListing(listingBNB);
                }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        (tTransferAmount, rTransferAmount) = _takeListing(sender, tTransferAmount, rTransferAmount, tAmount);
    ...
    function changeListingWallet(address _listingWallet) external onlyOwner {
        require(_listingWallet != listingWallet, "Listing wallet is already that address");
        require(!isContract(_listingWallet), "Listing wallet cannot be a contract");
        listingWallet = _listingWallet;
        emit ListingWalletChanged(listingWallet);
    }
    ...
    function setTaxFeePercent(uint256 _taxFee) external onlyOwner() {
        taxFee = _taxFee;
        totalFees = taxFee + marketingFee + listingFee + devTeamFee + charityFundrasingFee;
    ...
    function setListingFeePercent(uint256 _listingFee) external onlyOwner() {
