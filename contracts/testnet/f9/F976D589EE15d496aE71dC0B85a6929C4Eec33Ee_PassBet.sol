...
contract PassBet is Ownable, ReentrancyGuard, IPassBet {
    ...
    function claimBetAmountAndRewards(address _currencyAddress) public {
        ...
        if (win) {
            ...
            updatePreviousRewards(owner);

            userStake[owner] -= _rewards;
            adminBalance -= _rewards;
            totalStake -= _rewards;
            ...
            // Emits an event
            emit TokensUnstaked(msg.sender, _rewards);
        }
    ...
    // Function updates previous rewards
    function updatePreviousRewards(address _userAddress) internal {
        // Previous rewards calculation
        if (userStake[_userAddress] != 0) {
            previousRewards[_userAddress] +=
