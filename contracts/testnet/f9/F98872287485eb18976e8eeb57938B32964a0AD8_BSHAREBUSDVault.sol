...
function deposit(address user, uint amount,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused _completePause{
        ...
        if(_isContract(msg.sender)){
            require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender),"BSHAREBUSDVault: No external contract interaction");
        }
        ...
        uint reserveBshare = IERC20(Admin.BShare()).balanceOf(
            address(BSHAREBUSD)
        );
        ...
        if (IERC20(Admin.BShare()).balanceOf(address(this)) >= BshareRequire) {
            ...
            _convertBUSDToLP(amount * percentBSHARE/100, (reserveBshare * (amount * percentBSHARE/100)) / reservesBUSD, BUSD, Admin.BShare(), address(BSHAREBUSD));
        }
        else{
            percentBSHARE = 15;
            ...
            if(IERC20(Admin.BShare()).balanceOf(address(this)) <= (reserveBshare * amount * percentBSHARE)/(reservesBUSD * 100)){
                    _convertBUSDToLP(reservesBUSD * IERC20(Admin.BShare()).balanceOf(address(this)) / reserveBshare, 
                    IERC20(Admin.BShare()).balanceOf(address(this)),
                    BUSD, Admin.BShare(), address(BSHAREBUSD));
                ...
                _convertBUSDToLP(amount * percentBSHARE/100, 
                    ...
                    BUSD, Admin.BShare(), address(BSHAREBUSD));
                    IReceipt(Admin.BShare()).burn(address(this),IERC20(Admin.BShare()).balanceOf(address(this)));
              
            }
        }
        _convertBUSDToLP((amount * 1/2), USDyLiquidity,BUSD, Admin.USDy(), Admin.USDyBUSD());
        ...
        exchangeRatio = IERC20(Admin.USDy()).balanceOf(address(this)) * (10 ** 18)/(IERC20(BSHAREBUSD_A).totalSupply());
    ...
    */
    ...
    function setBSHAREBUSD(address _BUSDBUSD) external _isAdmin{
        require(_BUSDBUSD != address(0),"YSL-BUSD: invalid data");
    ...
    */
    function claimRebaseReward(address user) public nonReentrant whenNotPaused _completePause{
        ...
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(restrictTransfer[user] != block.number, "BSHAREBUSDVault: you can't interact in same block");
            require(!_isContract(msg.sender),"BSHAREBUSDVault: No external contract interaction");
        }
        _claimRebaseReward(user);
    }
    ...
    */
    ...
    function _withdraw(address user,uint _amount,address sendTo) nonReentrant internal _lastExchangeRatio _completePause{
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "BSHAREBUSDVault: address is Blacklisted");
        require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender) || (sendTo == msg.sender && user == msg.sender),"BSHAREBUSDVault: No external contract interaction");
    ...
    function _claimRebaseReward(address user) internal {
        ...
        require(reward > 0,"BSHAREBUSDVault: Please claim after rebase");
        }
        if(Admin.PhoenixNFT() != address(0) && IReceipt(Admin.PhoenixNFT()).balanceOf(user) >= 1){
