...
contract LuckyMoney is Ownable {
    ...
    function register(string memory name) external {
        _registerFresh(name, _msgSender());
    }

     function registerByOperator(string memory name, address user) external onlyOperator {
        _registerFresh(name, user);
    }

    function _registerFresh(string memory name, address user) internal {
        require(bytes(name).length > 0, "LuckyMoney: name is empty");
        require(tickets[user] == 0, "LuckyMoney: registered user");
        require(!_isUsedName[name], "LuckyMoney: nickname is used");
        nTicket = nTicket + 1;
        uint256 tid = nTicket;
        tickets[user] = tid;
        ticketToUser[tid] = user;
        users[tid] = UserInfo({
            name: name, 
            addr: user
        });
    ...
    function thirdPrizeDraw() external onlyOperator {
        ...
        for (uint256 i = 1; i < 100; i++) {
            ...
            if (!(seed == special || seed == first || seed == second || blacklist[ticketToUser[seed]] == true  )) {
                break;
            }
        }
        for (uint256 i = 0; i < 3; i++) {
            if (seed == special || seed == first || seed == second || blacklist[ticketToUser[seed]] == true ) seed = seed + 1;
        }
    ...
    function secondPrizeDraw() external onlyOperator {
        ...
        for (uint256 i = 1; i < 100; i++) {
            seed = Random.computerSeed(i) / (i + 1) % _n + 1;
            if (!(seed == special || seed == first || seed == third || blacklist[ticketToUser[seed]] == true )) {
                break;
            }
        }
        for (uint256 i = 0; i < 3; i++) {
            if (seed == special || seed == first || seed == third || blacklist[ticketToUser[seed]] == true) seed = seed + 1;
        }
    ...
    function firstPrizeDraw() external onlyOperator {
        ...
        for (uint256 i = 1; i < 100; i++) {
            seed = Random.computerSeed(i) / (i + 1) % _n + 1;
            if (!(seed == special || seed == second || seed == third || blacklist[ticketToUser[seed]] == true )) {
                break;
            }
        }
        for (uint256 i = 0; i < 3; i++) {
            if (seed == special || seed == second || seed == third || blacklist[ticketToUser[seed]] == true) seed = seed + 1;
        }
    ...
    function specialPrizeDraw() external onlyOperator {
        ...
        for (uint256 i = 1; i < 100; i++) {
            seed = Random.computerSeed(i) / (i + 1) % _n + 1;
            if (!(seed == first || seed == second || seed == third || blacklist[ticketToUser[seed]] == true )){
                break;
            }
        }
        for (uint256 i = 0; i < 3; i++) {
            if (seed == first || seed == second || seed == third || blacklist[ticketToUser[seed]] == true) seed = seed + 1;
        }
    ...
    function setMinTicketNumber(uint256 _num) external onlyOperator {
        MIN_TICKET_NUMBER = _num;
    }

    function setAllowRedraw(bool _allow) external onlyOperator {
