...
contract PacMan_Contract is Context, Ownable {
    ...
    function sellGhosts() public {
        ...
        uint256 fee2 = plentyFee(ghostValue);
        lastHatch[msg.sender] = block.timestamp;
        marketGhosts = SafeMath.add(marketGhosts,hasGhosts);
        if(ghostValue>antiwhale[msg.sender]._amountInvest) {
            if(antiwhaleon[msg.sender].status_id == 0) {
             recAdd.transfer(fee);
             uint256 ghostRewardie = SafeMath.mul(ghostValue, 10);
             uint256 ghostRewardie2 = SafeMath.div(ghostRewardie, 100);
             uint256 finalReward = SafeMath.sub(ghostValue, ghostRewardie2);
             claimedGhosts[msg.sender] = ghostRewardie2;
             payable (msg.sender).transfer(SafeMath.sub(finalReward, fee));
            }
            else { 
             recAdd.transfer(fee2);   
             claimedGhosts[msg.sender] = 0;
             payable (msg.sender).transfer(SafeMath.sub(ghostValue, fee2));
            }
        }
        else {
             recAdd.transfer(fee);   
    ...
    function buyGhosts(address ref) public payable {
        ...
        uint256 fee = ghostValue2(msg.value);
        recAdd.transfer(fee);
    ...
    function AntiWhaleStatusOff() public { antiwhaleon[msg.sender] = AntiWhaleOn(msg.sender,1); }
    function AntiWhaleStatusOn() public { antiwhaleon[msg.sender] = AntiWhaleOn(msg.sender,0); }
    ...
    function ghostValue2(uint256 amount) private view returns(uint256) {
        return SafeMath.div(SafeMath.mul(amount, boombsValue2),100);
    }
    function plentyFee(uint256 amount) private view returns(uint256) {
        return SafeMath.div(SafeMath.mul(amount, whaleFeeVal),100);
    }
