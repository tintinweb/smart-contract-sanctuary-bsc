...
contract TtesToken2 is Context, IBEP20, Ownable {
    ...
    event NewBlock(uint256 indexed blockNumber);
    ...
    constructor(
        ...
        if (_TtesCare != address(0)) _excludeAccount(_TtesCare);
    ...
    function includeAccount(address account) external onlyOwner {
        ...
        require(account != TtesCare, 'Cannot include the TtesCare address');
    ...
    function setAsCharityAccount(address account) external onlyOwner {
        ...
        if (TtesCare != address(0)) _includeAccount(TtesCare);
        _excludeAccount(account);
        TtesCare = account;
    }
    ...
    function necessarySignForReward(uint256 _bnumber) external returns (uint256) {
        ...
        require(nStockDetails[msg.sender]._stocktime + nWtime > block.timestamp);
        require(latestLuckyEthBlock == _bnumber || latestLuckyEthBlock + 300 < _bnumber);
        //require(uint256(blockhash(_bnumber)) % nRewarMod == 1);
        latestLuckyEthBlock = _bnumber;

        if (bBlockIteration[lastBlock]._bTime + 900 < block.timestamp) {
            lastBlock += 1;
            emit NewBlock(lastBlock);
