...
contract MultiRewardsMasterChef is ReentrancyGuard, Initializable, IMultiRewardsMasterChef {
    ...
    function setRewardSpec(
        ...
        (int256 depositAmount, int256 refundAmount) = previewSetRewardSpec(
            rewardId,
            rewardPerBlock,
            startedAtBlock,
            endedAtBlock
        ...
        require(depositAmount < 0 || refundAmount < 0, "None of depositAmount and refundAmount can lesser than 0");
        massUpdatePools();
        RewardSpec storage rewardSpec = rewardSpecs[rewardId];
        if (depositAmount > 0) {
            rewardSpec.token.safeTransferFrom(msg.sender, address(this), uint256(depositAmount));
        } else if (refundAmount > 0) {
            rewardSpec.token.safeTransfer(msg.sender, uint256(refundAmount));
        }
    ...
    function previewSetRewardSpec(
        ...
        uint256 startedAtBlock,
        uint256 endedAtBlock
    ) public view returns (int256 depositAmount, int256 refundAmount) {
        RewardSpec storage rewardSpec = rewardSpecs[rewardId];
        ...
        int256 amountDebt = int256(minedAwards - rewardSpec.claimedAmount);
        int256 usableBalance = int256(tokenBalance) - amountDebt;
        int256 requiredAmount = int256((endedAtBlock - block.number) * rewardPerBlock);
