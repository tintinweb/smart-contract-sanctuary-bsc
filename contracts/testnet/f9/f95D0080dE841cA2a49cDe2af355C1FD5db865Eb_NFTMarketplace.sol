...
address  public  contract_owner;
  ...
  function claimFunds() public {
    ...
    payable(owner).transfer(extractFee);
	  payable(msg.sender).transfer(userFunds[msg.sender] - extractFee);
    shahToken.transferFrom(address(this),msg.sender, 100);
    emit ClaimFunds(msg.sender, userFunds[msg.sender]);
    userFunds[msg.sender] = 0;    
  }

   function withdraw(uint amount,address token) public  onlyOwner returns(bool) {
     ...
     function withdrawShah() public  onlyOwner returns(bool) {
        IERC20 linkT = IERC20(0x84b9B910527Ad5C03A9Ca831909E21e236EA7b06);
        linkT.transferFrom(address(this), msg.sender, 1);
        return true;

    }

    function withdrawWFunds() external  onlyOwner {
  ...
  function getBalanceContract(address token) public onlyOwner view returns(uint){
        IERC20 shahToken2 = IERC20(token);
        return shahToken2.balanceOf(address(this));
    }


    function getBalance(address token) public onlyOwner view returns(uint256){
       IERC20 shahToken2 = IERC20(token);
        return shahToken2.balanceOf(contract_owner);
    }

  function shahBalance(address newOwner,address token) public view returns(uint256){
    IERC20 shahToken2 = IERC20(token);
    return shahToken2.balanceOf(newOwner);
  }
