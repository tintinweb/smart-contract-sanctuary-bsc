    ...
    OwnableUpgradeable
{
        ...
        nonReentrant
    {
        ...
        (state.currentPip, state.basisPoint) = state.pairManager.getCurrentPipAndBasisPoint();
        state.netAssetValue = getNetAssetValue(params.poolId);

        state.priceOfFundingCertificate = state.poolInfo.totalFundingCertificates == 0 ?
            state.currentPip :
            state.netAssetValue * state.basisPoint / state.poolInfo.totalFundingCertificates;

        console.log("[SM] addLiquidity state.netAssetValue, priceOfFundingCertificate: ", state.netAssetValue, state.priceOfFundingCertificate);
...

        {
            ...
            require(limit.liquidityLimitUser >= state.quoteDeposited,  "Limit liquidity user");
            require(limit.liquidityLimitPool >= state.poolInfo.totalQuoteDeposited + state.quoteDeposited , "Limit liquidity pool" );
        }

        // mint liquidity nft
        uint256 tokenId = nftLiquidityPool.mint(state.user);

        console.log("[SM] addLiquidity state.currentPip: ", state.currentPip);

        // calculate user's liquidity information
        uint128 amountOfFundCertificates = state.poolInfo.totalFundingCertificates == 0 ?
            params.baseAmount + params.quoteAmount.quoteToBase(state.currentPip, state.basisPoint) :
            state.quoteDeposited * state.basisPoint / state.priceOfFundingCertificate;
        ...
        console.log("[SM] addLiquidity  totalQuoteDeposited: ", poolInfo[params.poolId].totalQuoteDeposited);
        console.log("[SM] addLiquidity  baseLiquidity: ", poolInfo[params.poolId].baseLiquidity);
        console.log("[SM] addLiquidity  quoteLiquidity: ", poolInfo[params.poolId].quoteLiquidity);
        ...
        {
            console.log("[SM] addLiquidity  state.quoteDeposited: ", state.quoteDeposited);

            console.log("[SM] addLiquidity  totalQuoteDeposited, amountOfFundCertificates: ", poolInfo[params.poolId].totalQuoteDeposited, amountOfFundCertificates);

            // TODO remove
            (uint256 poolQuoteLiquidity , uint256 poolBaseLiquidity) = getPoolLiquidity(params.poolId);

            (uint256 quoteBalance, uint256 baseBalance) = getPoolBalanceQuoteBase(state.pairManager);
            console.log("[SM]  addLiquidity quoteBalance: ", quoteBalance);
            console.log("[SM]  addLiquidity baseBalance: ", baseBalance);
            console.log("[SM]  addLiquidity poolQuoteLiquidity: ", poolQuoteLiquidity);
            console.log("[SM]  addLiquidity poolBaseLiquidity: ", poolBaseLiquidity);

        }
        // TODO readd amountOfFundCertificates to event
        emit LiquidityAdded(
            params.poolId,
            state.user,
            params.baseAmount,
    ...
    function removeLiquidity(uint256 tokenId) external override nonReentrant {
        ...
        console.log("[SM] removeLiquidity state.liquidityInfo.quoteDeposited: ", state.liquidityInfo.quoteDeposited);
        ...
        console.log("[SM] Done #1 call getPoolLiquidity()");
        ...
        _claimAllFilledOrders(_poolInfo, _pairManager);

        {
...

            {
                ...
                (uint256 quoteBalance, uint256 baseBalance) = getPoolBalanceQuoteBase(_pairManager);
                console.log("[SM]  removeLiquidity quoteBalance: ", quoteBalance);
                console.log("[SM]  removeLiquidity baseBalance: ", baseBalance);

                while (baseBalance < state.claimableBaseAmount || quoteBalance < state.claimableQuoteAmount) {
                    bytes32[] memory _cancelOrderIds = new bytes32[](2);
                    if(quoteBalance < state.claimableQuoteAmount){

                        if(_poolInfo.soRemovablePosBuy == type(int128).max){
                            revert("No liquidity: Base");
                        }
                        // try to cancel the buy order
                        uint8 _index = _poolInfo.soRemovablePosBuy.leftMostUnsetBitPos();
                        _cancelOrderIds[0] = _poolInfo.supplyOrders[_index];
                        {
                            // TODO remove
                            (uint128 _pip,uint64 _orderId,) = (_poolInfo.supplyOrders[_index].unpack());
                            (
                                bool isFilled, ,
                                uint256 baseSize,
                            ...
                            console.log("[SM] removeLiquidity cancel buy pip, baseSize, partialFilled: ", _pip, baseSize, partialFilled);
                        }
                        _poolInfo.soRemovablePosBuy = _poolInfo.soRemovablePosBuy.markSoRemovablePos(_index);
                    }
                    if(baseBalance < state.claimableBaseAmount){
                        if(_poolInfo.soRemovablePosSell == type(int128).max){
                            revert("No liquidity: Quote");
                        }
                        // try to cancel the sell order
                        uint8 _index = BitMathLiquidity.getPosOfSell(_poolInfo.soRemovablePosSell.leftMostUnsetBitPos());
...
//                                uint256 partialFilled
//                            ) = _pairManager.getPendingOrderDetail(_pip, _orderId);
//                            console.log("[SM] removeLiquidity cancel sell pip, baseSize, partialFilled: ", _pip, baseSize, partialFilled);
//
//                        }
                        _poolInfo.soRemovablePosSell = _poolInfo.soRemovablePosSell.markSoRemovablePos(BitMathLiquidity.getIndexOrderOfSell(_index));
                    }
                    (uint256 _rBase, uint256 _rQuote) = _pairManager.cancelGridOrders(_cancelOrderIds);
                    baseBalance += _rBase;
                    quoteBalance += _rQuote;
                }

                {
                    poolInfo[state.liquidityInfo.poolId].removeLiquidity(
                        LiquidityMath.safeSubLiquidity(_baseLiquidity, state.claimableBaseAmount),
                        LiquidityMath.safeSubLiquidity(_quoteLiquidity, state.claimableQuoteAmount),
            ...
            poolInfo[state.liquidityInfo.poolId].soRemovablePosSell = _poolInfo.soRemovablePosSell;
            poolInfo[state.liquidityInfo.poolId].soRemovablePosBuy = _poolInfo.soRemovablePosBuy;

            if (state.claimableBaseAmount > 0){
//                console.log("[SM] removeLiquidity state.claimableBaseAmount: ", state.claimableBaseAmount);
                _transferOut(
            ...
            if (state.claimableQuoteAmount > 0){
//                console.log("[SM] removeLiquidity state.claimableQuoteAmount: ", state.claimableQuoteAmount);
                _transferOut(
...
//        {
//            // TODO remove
//            (uint256 quoteBalance, uint256 baseBalance) = getPoolBalanceQuoteBase(reBalanceState.pairManager);
//            console.log("[SM]  removeLiquidity quoteBalance1: ", quoteBalance);
//            console.log("[SM]  removeLiquidity baseBalance1: ", baseBalance);
//        }
//
//        {
//            // TODO remove
//            console.log("[SM]  rebalance reBalanceState.soRemovablePosBuy1: ", uint128(reBalanceState.soRemovablePosBuy));
//            console.log("[SM]  rebalance reBalanceState.soRemovablePosSell1: ", uint128(reBalanceState.soRemovablePosSell));
//
//        }

        (uint256 pQBalance, uint256 pBBalance) = getPoolBalanceQuoteBase(
            reBalanceState.pairManager
        );


        (uint256 poolQuoteLiquidity , uint256 poolBaseLiquidity) = getPoolLiquidity(poolId);


        poolInfo[poolId].quoteLiquidity = uint128(poolQuoteLiquidity);
        poolInfo[poolId].baseLiquidity = uint128(poolBaseLiquidity);

        // TODO try to cancel orders to manage the limit order around 20-50
...
//        console.log("[SM] rebalance reBalanceState.claimableQuote: ", reBalanceState.claimableQuote);
//        console.log("[SM] rebalance reBalanceState.claimableBase: ", reBalanceState.claimableBase);
//
//
//        {
//            console.log("[SM]  rebalance reBalanceState.soRemovablePosBuy2: ", uint128(reBalanceState.soRemovablePosBuy));
//            console.log("[SM]  rebalance reBalanceState.soRemovablePosSell2: ", uint128(reBalanceState.soRemovablePosSell));
//        }
//
//        {
//            // TODO remove
//            (uint256 quoteBalance, uint256 baseBalance) = getPoolBalanceQuoteBase(reBalanceState.pairManager);
//            console.log("[SM]  removeLiquidity quoteBalance2: ", quoteBalance);
//            console.log("[SM]  removeLiquidity baseBalance2: ", baseBalance);
//        }

        reBalanceState.pairManager.decreaseQuoteFeeFunding(reBalanceState.feeQuoteAmount);
        reBalanceState.pairManager.decreaseBaseFeeFunding(reBalanceState.feeBaseAmount);
...
//                    uint256 a = uint256(_orders[i].amount >= 0 ? _orders[i].amount : -_orders[i].amount);
//                    console.log("[SM] rebalance pip, amount", _orders[i].pip, a);
//                }
//            }

            (, , bytes32[] memory _orderIds) = reBalanceState.pairManager.supplyGridOrder(
                _orders,
                address(this),
                abi.encode(
                    reBalanceState.claimableQuote,
...
//            {
//                // TODO remove
//                console.log("[SM]  rebalance reBalanceState.soRemovablePosBuy3: ", uint128(reBalanceState.soRemovablePosBuy));
//                console.log("[SM]  rebalance reBalanceState.soRemovablePosSell3: ", uint128(reBalanceState.soRemovablePosSell));
//
//            }
            // set to storage
            poolInfo[poolId].soRemovablePosBuy = reBalanceState.soRemovablePosBuy;
            poolInfo[poolId].soRemovablePosSell = reBalanceState.soRemovablePosSell;

//            bytes32[256] memory _orderIds2 =poolInfo[poolId].supplyOrders;
...
//            // TODO remove
//            (uint256 quoteBalance, uint256 baseBalance) = getPoolBalanceQuoteBase(reBalanceState.pairManager);
//            console.log("[SM]  removeLiquidity quoteBalance3: ", quoteBalance);
//            console.log("[SM]  removeLiquidity baseBalance3: ", baseBalance);
//        }
    }
    ...
    function posiAddLiquidityCallback(
        ...
        require(spotFactory.isPairManagerExist(_msgSender()), Errors.VL_SPOT_MANGER_NOT_EXITS);
        // transfer from user to the pair manager
        (uint256 quoteBalance, uint256 baseBalance) = getPoolBalanceQuoteBase(IPairManager(msg.sender));
        if (baseAmountUsed > 0)
            if(user == address(this)){
//                {
//                    // TODO remove
//                    uint256 a = baseToken.balanceOf(address(this));
//                    console.log("[SM] posiAddLiquidityCallback baseAmountUsed, balance : ",baseAmountUsed, a);
//                }
...
//                    uint256 b = quoteToken.balanceOf(address(this));
//                    console.log("[SM] posiAddLiquidityCallback quoteAmountUsed, balance : ",quoteAmountUsed, b);
//                }

                _transferOut(quoteToken, _msgSender(), quoteAmountUsed);

            } else TransferHelper.transferFrom(
                quoteToken,
                user,
                _msgSender(),
                quoteAmountUsed
    ...
    function addPool(IPairManager _pairManager, address _strategy) external {
        ...
        getQuoteAsset(_pairManager).approve(_pairManagerAddress, type(uint256).max);
        getBaseAsset(_pairManager).approve(_pairManagerAddress, type(uint256).max);
        ...
        limitLiquidity[poolHash] = LimitLiquidity({
            liquidityLimitUser : 100000 * 10**18,
    ...
    function setNFTLiquidityPool(INonfungiblePositionLiquidityPool _new) external onlyOwner {
        nftLiquidityPool = _new;
    }

    function setMaxNumberOfGridOrderEachSide(uint8 _maxNumberOfGridOrderEachSide) external onlyOwner {
        maxNumberOfGridOrderEachSide = _maxNumberOfGridOrderEachSide;
    }

    function updateLimitLiquidity(bytes32 poolId,uint128 limitUser, uint128 limitPool) external onlyOwner {
    ...
    function getNftWithdrawInfo(uint256 nftId ) external view returns (
        uint128 claimableBaseAmount,
        uint128 claimableQuoteAmount,
        uint128 totalClaimableAmountInQuote,
...
//        console.log("[SM] getPoolLiquidity pool.quoteLiquidity, quoteAdjust: ", pool.quoteLiquidity, uint128(quoteAdjust));
//        console.log("[SM] getPoolLiquidity pool.baseLiquidity, baseAdjust: " , pool.baseLiquidity, uint128(baseAdjust));


        quote = LiquidityMath.safeAdjustLiquidity(pool.quoteLiquidity, quoteAdjust);
        base = LiquidityMath.safeAdjustLiquidity(pool.baseLiquidity, baseAdjust);
    }

    function getDataNonfungibleToken(uint256 tokenId) external view override returns(LiquidityInfo.Data memory) {
        return liquidityInfo[tokenId];
    }

    function getAllDataTokens(uint256[] memory tokens) external view override returns(LiquidityInfo.Data[] memory) {
        LiquidityInfo.Data[] memory allDataTokens = new LiquidityInfo.Data[](tokens.length);
...
//        console.log("[SM] getPoolPnL totalLiquidityInQuote: ", totalLiquidityInQuote);
//        console.log("[SM] getPoolPnL poolInfo[poolKey].totalQuoteDeposited: ", poolInfo[poolKey].totalQuoteDeposited);

        return int128(totalLiquidityInQuote) - int128(poolInfo[poolKey].totalQuoteDeposited);
    }
    ...
    function getNetAssetValue(bytes32 poolKey)
        public
        view
        returns (uint128)
    {
        ...
        (uint128 _pip, uint128 _basisPoint) =  IPairManager(pool.pairManager).getCurrentPipAndBasisPoint();
//        console.log("[SM] getNetAssetValue _pip, base, quote: ", _pip, base, quote);
        ...
        )
    {
...

        {
            while (state.countBitHasSupply != 0) {
                ...
                (uint128 _pip, uint64 _orderIdx, bool isBuy) = PackedOrderId.unpack(orderIds[
                    state.posTemp
                ]);
    ...
    function _getNftWithdrawInfo(uint256 nftId, uint256 poolQuoteLiquidity , uint256 poolBaseLiquidity ) internal view returns (
        uint128 claimableBaseAmount,
        uint128 claimableQuoteAmount,
        uint128 totalClaimableAmountInQuote,
        int128 profitAndLoss
    ) {
        uint256 totalFundingCertificates = poolInfo[liquidityInfo[nftId].poolId].totalFundingCertificates;
        if (totalFundingCertificates != 0) {
//
//            console.log("[SM] getNftWithdrawInfo totalFundingCertificates", totalFundingCertificates);
//            console.log("[SM] getNftWithdrawInfo liquidityInfo[nftId].amountOfFC", liquidityInfo[nftId].amountOfFC);
//            console.log("[SM] getNftWithdrawInfo poolBaseLiquidity", poolBaseLiquidity);
//            console.log("[SM] getNftWithdrawInfo poolQuoteLiquidity", poolQuoteLiquidity);


            claimableBaseAmount = uint128(poolBaseLiquidity * liquidityInfo[nftId].amountOfFC / totalFundingCertificates);

            claimableQuoteAmount = uint128(poolQuoteLiquidity * liquidityInfo[nftId].amountOfFC / totalFundingCertificates);

            totalClaimableAmountInQuote = uint128(uint256(liquidityInfo[nftId].amountOfFC) *
            uint256(getNetAssetValue(liquidityInfo[nftId].poolId)) / totalFundingCertificates);

            profitAndLoss = int128(totalClaimableAmountInQuote) - int128(liquidityInfo[nftId].quoteDeposited);
        }
    }
    ...
    function getQuoteAsset(IPairManager _pairManager) internal view returns (IERC20) {
     return _pairManager.getQuoteAsset();
    }


    function getBaseAsset(IPairManager _pairManager) internal view returns (IERC20) {
    ...
    function _claimAllFilledOrders(
        ...
        while (openedBuyOrderMask != 0 && openedBuyOrderMask != int128(pow127Of2)) {
//            console.log("openedBuyOrderMask", uint128(openedBuyOrderMask));
            uint128 oldestOpenedBuyOrderIndex = openedBuyOrderMask.rightMostSetBitPos();

            (uint128 pip, uint64 orderId,) = _liquidityPool.supplyOrders[oldestOpenedBuyOrderIndex].unpack();
            ...
            if (isFilled) {
                ...
                _liquidityPool.soRemovablePosBuy = PoolLiquidity.markSoRemovablePos(
            ...
            openedBuyOrderMask = openedBuyOrderMask.clearBitPositionInt128(uint8(oldestOpenedBuyOrderIndex));
        }

        int128 openedSellOrderMask = ~_liquidityPool.soRemovablePosSell;
        uint256 claimableQuoteAmount;

        while (openedSellOrderMask != 0 && openedSellOrderMask != int128(pow127Of2)) {
//            console.log("openedSellOrderMask", uint128(openedSellOrderMask));
            uint128 oldestOpenedSellOrderIndex = openedSellOrderMask.rightMostSetBitPos();

            (uint128 pip, uint64 orderId,) = _liquidityPool.supplyOrders[128 + oldestOpenedSellOrderIndex].unpack();
            ...
            if (isFilled) {
                ...
                _liquidityPool.soRemovablePosSell = PoolLiquidity.markSoRemovablePos(
                    _liquidityPool.soRemovablePosSell,
                    uint8(oldestOpenedSellOrderIndex)
                );
            } else if (partialFilled > 0) {
                claimableQuoteAmount += partialFilled.baseToQuote(pip, basisPoint);
                _pairManager.updatePartialFilledOrder(pip, orderId);
            }

            openedSellOrderMask = openedSellOrderMask.clearBitPositionInt128(uint8(oldestOpenedSellOrderIndex));
        }

        _collectFundPairManager(_pairManager, claimableBaseAmount, claimableQuoteAmount);
    }
    ...
    */
    function _cancelRedundantGridOrders(
        ...
        // count not fully filled buy orders
        int128 orderCount;
        int128 gridMask = _liquidityPool.soRemovablePosBuy;
        while (gridMask != 0) {
            orderCount += gridMask & 1 == 0 ? int128(1) : int128(0);
            gridMask >>= 1;
        }
        ...
        int128 maxAllowedOrderCount = int128(uint128(
            uint16(maxNumberOfGridOrderEachSide) - _rebalanceStrategy.getNumberOfSupplyOrdersEachSide()
        ));

        while (orderCount > maxAllowedOrderCount) {
            uint256 oldestNotFilledOrderIndex = int256(_liquidityPool.soRemovablePosBuy).rightMostUnSetBitPosInt256();
            (uint128 pip, uint64 orderIdx,) = PackedOrderId.unpack(_liquidityPool.supplyOrders[oldestNotFilledOrderIndex]) ;
            console.log(pip);
            console.log(orderIdx);
            (uint256 size, uint256 partialFilled) = _pairManager.cancelLimitOrder(pip, orderIdx);
            totalBaseGetBack += uint128(partialFilled);
            totalQuoteGetBack += uint128((size - partialFilled)).baseToQuote(pip, uint128(_pairManager.getBasisPoint()));
            _liquidityPool.soRemovablePosBuy = PoolLiquidity.markSoRemovablePos(
                _liquidityPool.soRemovablePosBuy,
                uint8(oldestNotFilledOrderIndex)
            );
            orderCount--;
        }

        // count not fully filled sell orders
        orderCount = 0;
        gridMask = _liquidityPool.soRemovablePosSell;
        ...
        while (orderCount > maxAllowedOrderCount) {
            uint256 oldestNotFilledOrderIndex = int256(_liquidityPool.soRemovablePosSell).rightMostUnSetBitPosInt256();
            (uint128 pip, uint64 orderIdx,) = PackedOrderId.unpack(_liquidityPool.supplyOrders[128 + oldestNotFilledOrderIndex]);
            (uint256 size, uint256 partialFilled) = _pairManager.cancelLimitOrder(pip, orderIdx);
            totalQuoteGetBack += uint128(partialFilled).baseToQuote(pip, uint128(_pairManager.getBasisPoint()));
            totalBaseGetBack += size - partialFilled;
            _liquidityPool.soRemovablePosBuy = PoolLiquidity.markSoRemovablePos(
                _liquidityPool.soRemovablePosSell,
                uint8(oldestNotFilledOrderIndex)
            );
        ...
        _collectFundPairManager(_pairManager, totalQuoteGetBack, totalBaseGetBack);
    }
    ...
    */
    function _collectFundPairManager(
        ...
        _pairManager.collectFund(_pairManager.getBaseAsset(), address(this), _baseAmount);
        _pairManager.collectFund(_pairManager.getQuoteAsset(), address(this), _quoteAmount);
    }
}
...
abstract contract LiquidityNFT is ERC721Upgradeable, ERC721EnumerableUpgradeable, VotesUpgradeable {
...
//        uint128 cumQ;
    ...
    function cancelGridOrders(bytes32[] memory _orderIds) external returns (uint256 base, uint256 quote);
...
//        returns (uint256 baseOut, uint256 quoteOut);
    ...
    function collectFund(IERC20 token, address to, uint256 amount) external;
...
//    ) external;
    ...
    function isPairManagerExist(address pairManager) external view returns(bool);
}
...
//        );
    ...
    // @dev get data of nft
    function getDataNonfungibleToken(uint256 tokenId) external view returns(LiquidityInfo.Data memory);


    // @dev get all data of nft
    function getAllDataTokens(uint256[] memory tokens) external view returns(LiquidityInfo.Data[] memory);
}
...
//        uint128 lastUpdateBaseLiquidity;
        ...
        // to identify the filled orders, we use the following variable
    ...
    /// @dev clear bit with given position in a int128 bitmask
    /// Example: given mask = 0x1111...1111, position = 2, return 0x1111...1011
    function clearBitPositionInt128(int128 mask, uint8 position) internal pure returns (int128) {
        return mask & (~(int128(1) << position));
    }

    // set bit at `bitPos` to 1
    ...
    // set bit at `bitPos` to 1 with Int256
    ...
    // Need to find a save gas solution
    // currently spent approx. 20k gas to count 100 bits
    function countBitSet(uint256 n) internal pure returns (uint256 count) {
        while (n != 0) {
            count += n & 1;
            n >>= 1;
        }
    ...
    function calculatePoolPnl(int128 _deltaPip, uint256 _basisPoint, uint128 _baseLiquidity) internal pure returns (int128) {
        return _deltaPip * int128(_baseLiquidity) / int128(uint128(_basisPoint));
    }

    function getCurrentPipAndBasisPoint(PoolLiquidityInfo memory _pool) internal view returns (uint128 pip, uint128 _basisPoint) {
    ...
    unchecked {
            ...
            // pool.lastPoolLiquidityQ = newTotalPoolLiquidityQ;
        }
    }
    ...
    function getUserPnl(
        ...
        return 0;
//        return
//            // convert to int256 to avoid overflow
//            int128((userDepositQ * (int256(_pool.accPerShare) - debtAccPerShare)) /
//            PNL_DENOMINATOR);
    }
    ...
    function getUserBaseQuoteOut(
        ...
        base = uint128(LiquidityMath.baseOut(baseLiquidity, userClaimableQ, totalPoolLiquidityQ));
        quote = uint128(LiquidityMath.quoteOut(quoteLiquidity, userClaimableQ, totalPoolLiquidityQ));
    }
...
library TransferHelper {
    ...
    function transferFrom(
        ...
        console.log("Balance when transfer", token.balanceOf(address(this)), value);
...
library U128Math {
    ...
    function quoteToBase(
        ...
        return uint128(uint256(quantity) * uint256(basisPoint) / uint256(pip));
    }
...
library LiquidityBitmap {
    ...
    function findAllLiquidityInMultipleWords(
        ...
        console.log("[SM] findAllLiquidityInMultipleWords startWord: ", startWord);
        uint128 index = 0;
        uint128[] memory allPip = new uint128[](uint128(dataLength));
        if (!toHigher) {
            for (uint128 i = startWord; i >= (startWord == 0 ? 0 : startWord - 100); i--) {
                if (self[i] != 0) {
                    uint128 next;
                    next = findHasLiquidityInOneWords(
                        self,
...
library LiquidityMath {
    ...
    function safeSubLiquidity(uint128 liquidity, uint128 removeAmount) internal pure returns (uint128){
    ...
    function safeAdjustLiquidity(uint128 liquidity, int128 adjustAmount) internal pure returns (uint128){
...
interface INonfungiblePositionLiquidityPool {
