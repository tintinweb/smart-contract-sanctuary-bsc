...
interface IFamily {
     ...
     */
    function fBalanceOf(address account) external view returns (uint256);
     ...
     * Emits a {Transfer} event.
     */
    function fTransfer(address recipient, uint256 amount) external returns (bool);
...
contract FamilyPartyNFT is OwnableContract, ERC721("Infinite Force", "IF") {
    ...
    function setFamilyToken(address tokenAddress) public onlyOwner {
        fpc = tokenAddress;
    }
    ...
    function withdrawToken() public onlyOwner {
        ...
        uint256 balance = IFamily(fpc).fBalanceOf(address(this));

        IFamily(fpc).fTransfer(msg.sender, balance);
    }
