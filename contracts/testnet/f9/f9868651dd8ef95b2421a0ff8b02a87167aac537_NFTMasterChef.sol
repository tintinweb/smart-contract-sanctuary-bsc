...
contract NFTMasterChef is Ownable, ReentrancyGuard {
    ...
    NFTsInfo[] public nftsInfo; // Array of NFT can mint
    ...
    address public NFTStakeAddress; // Address of NFTStake can compund
    address public DDTMasterchefAddress; // Address of DDTFarm can compund

    mapping(address => uint256) public powerBalance; // Because power is not token store powerBalance here
    ...
    function claimNFTReward(uint256 level, uint256 _pid) public nonReentrant {
        ...
        uint256 pending = user.amount.mul(accPowerPerShare).div(1e12).sub(user.rewardDebt); // calc pending power
        NFTsInfo storage nft = nftsInfo[level];
        uint256 total = pending.add(powerBalance[msg.sender]); // calc user total power pending + balance
        if(total >= nft.power){
            updatePool(_pid);
        }
        require(nft.power <= powerBalance[msg.sender], 'Power is not enough'); // NFT level user selected power is > total power
    ...
    function enterStakingCompund(uint256 _amount, address _account) public nonReentrant returns(bool){
        require(_amount > 0, "Amount is zero");
