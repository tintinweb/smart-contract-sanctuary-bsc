...
contract USDT_PizzaTower {
    ...
    function addCoins(address ref, uint256 value) initialized external {
        ...
        (,uint256 refCrystal, uint256 money) = getMarket(marketId);	

        towers[ref].coins += (coins * refCrystal) / 100;	
        towers[ref].money += (coins * 100 * money) / 100;	
    ...
    function upgradeTower(uint256 floorId) initialized external {
        require(floorId < 8, "Max 8 floors");
        address user = msg.sender;
        if (floorId > 0) {
            require(towers[user].chefs[floorId-1] == 6, "Prev Tower not upgraded");
        }
