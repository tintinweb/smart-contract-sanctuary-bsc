...
contract LMSTracker is Initializable, OwnableUpgradeable {
    ...
    function getAllSummary() public view returns (Summary[] memory) {
        if(weekIndex > 0) {
            Summary[] memory summaries = new Summary[](weekIndex - 1);
            ...
            Summary[] memory summaries = new Summary[](0);
            return summaries;
        }
    }
    ...
    function getUserHistory(address user) public view returns (UserHistory[] memory) {
        if(weekIndex > 0) {
            UserHistory[] memory history = new UserHistory[](weekIndex - 1);

            for (uint256 i = 1; i < (weekIndex); i++) {
                if (weeklyTrackers[i].eligibleUser[user]) {
            ...
            UserHistory[] memory history = new UserHistory[](0);
            return (history);
        }
    }
    ...
    function _closeWeek() private {
        ...
        for (uint256 i = 0; i < _eligibleBalanceHolders.length(); i++) {
            ...
            if (weeklyTrackers[week].soldOrNewUser[user] == false) {
                ...
                weeklyTrackers[week].eligibleUser[user] = true;
            }
        }
