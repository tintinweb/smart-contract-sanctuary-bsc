...
contract LaunchToken is Context, IERC20, Ownable {  
    ...
    //   function _getMaxHolding() public  (uint256) {
    //     return  (_tTotal * holdingPercent) / 100;
    // }
    ...
    function _takeMarketingFee(uint256 tAmount,uint256 tFee,uint TaxType) private {
      ...
      if(TaxType==1){
          ...
          if(tFee<(MarketingShare.add(RewardShare).add(BurningShare).add(DevelopmentShare))){
            RewardShare=RewardShare.sub((MarketingShare.add(RewardShare).add(BurningShare).add(DevelopmentShare)).sub(tFee));
        }
        uint256 FeeMarkRewLiqu=MarketingShare+DevelopmentShare+RewardShare;
