...
contract CORK is IBEP20, Ownable {
    ...
    string private constant _name = "CORKY";
    string private constant _symbol = "CORK";
...
// Adds Liquidity directly to the contract where LP are locked(unlike safemoon forks, that transfer it to the owner)
function _addLiquidityManual(uint256 tokenamount, uint256 bnbamount) public onlyOwner {
    totalLPBNB+=bnbamount;
    _approve(address(this), address(_pancakeRouter), tokenamount);
    _pancakeRouter.addLiquidityETH{value: bnbamount}(
        address(this),
