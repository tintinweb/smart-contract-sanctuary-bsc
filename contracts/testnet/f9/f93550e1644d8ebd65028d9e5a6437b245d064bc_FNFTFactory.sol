...
contract FNFTFactory {
    ...
    event MintFNFT(address to, uint8 index, uint256 initialBNB);
    ...
    // function setMainWallet(address addr) external onlyAdmin {
    //     _mainWallet = addr;
    // }
    ...
    function setWhitelist(address addr, bool flag) external onlyAdmin {
    ...
    function mintFNFT(string memory uri) external payable isWhiteListed {
        ...
        _founderNFT.mintFNFT(msg.sender, uri);

        FounderInfo memory founderInfo = FounderInfo(msg.sender, _indexOfFNFT, msg.value, block.timestamp, block.timestamp, 0, 0, false);
        _founderInfo[msg.sender] = founderInfo;
        _founderAddress[_indexOfFNFT] = msg.sender;

        emit MintFNFT(msg.sender, _indexOfFNFT, msg.value);
        _indexOfFNFT++;
    }
