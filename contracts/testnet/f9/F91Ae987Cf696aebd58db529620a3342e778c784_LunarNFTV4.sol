...
*/
contract LunarNFTV4 is LunarNFT {
  /**
...
contract LunarNFT is Initializable, ERC721Upgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  * @dev Custom Event | MintCompleted
  * @param toAddress  - The wallet address from which the event comes from.
  * @param tokenId    - An integer representing the token that were airdropped.
  * @param success    - An boolean whether or not the token were airdropped.
  ...
  */
  function plexusMint(address to, uint256[] memory tokenIds) external whenNotPaused onlyPlexusRoles
  {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++)
    {
      ...
      if(!hasRole(DEFAULT_ADMIN_ROLE, msg.sender))
      {
        ...
        if (!canUserResult)
        {
          ...
          mintResult = false;
          //exit loop if limit threshold has failed
          break;
        }
    ...
    //SHEPARD: If we could not issue all the tokens requested, Lunar Plexus(TM) will send another mint() request on another block. 
  ...
  */
  function _canUserMintToken(address to) internal view returns(bool success, string memory message)
  {
    ...
    //SHEPARD: Check if user has not exceeded allowed amounts based on the mint state for all limited minting MintStates.
    // DISABLED =  0, PRIVATE =   1, WHITELIST = 2, PUBLIC = 3, INTERNAL =  4
