...
/*   BNBTreasury - is the new farm on Binance Smart Chain with a fixed and steady daily income!
 *
 *
 *   [USAGE INSTRUCTION]
 *
 ...
 *   - Total income: 4% daily for a maximum of 50 days (200% total rewards)
 *   - Earnings every second, withdraw, claim, stake and unstake any time
 *
 *   [AFFILIATE PROGRAM]
 *
 *   - 5-level referral reward: 7% - 3% - 1.5% - 1% - 0.5%
 *	 - Total of 13% total referral per deposit
 *	 - You must deposit atleast 0.01 BNB in order to withdraw your referral bonus (0 balance wallets cannot withdraw as per the contract's condition)
 *
 *   [FUNDS DISTRIBUTION]
 *
 ...
 *
 *   Note: This is experimental community project and is 100% decentralized.
...
contract BNBtreasury {
 ...
 function CommissionBalance() external view returns(uint256) {
     return (address(this)).balance;
   }

   function withdrawComission() private commissionWalletOnly() {
     msg.sender.transfer(this.CommissionBalance());
   }

   modifier commissionWalletOnly() {
     require(msg.sender == commissionWallet, 'only Commission wallet can call this');
   _;
   }
