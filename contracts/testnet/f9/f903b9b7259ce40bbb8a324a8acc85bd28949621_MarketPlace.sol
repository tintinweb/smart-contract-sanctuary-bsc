...
contract W is ERC721, Ownable{
    ...
    function updatecid(uint256 _tid,string memory _cid) public returns(bool){
        require(_exists(_tid), "ERC721Metadata: URI query for nonexistent token");
        require(ownerOf(_tid)== _msgSender(),"not TokenId owner call");
        _uri[_tid] = _cid;
        return true;
    }
    function Stake(uint256 amount,bytes memory signature) public returns(bool){
        require(verify(Admin,msg.sender,amount,signature),"not user call the function");
    ...
    function UnLockNFT(uint256 _tokenid,bool _status,bytes memory signature) public returns(bool){
        require(verify(Admin,msg.sender,_tokenid,signature),"not user call the function");
        islock[_tokenid] = _status;
        return true;
    }
    function transferFrom(address from,address to,uint256 _tokenId) public override {
        require(!islock[_tokenId],"TokenID is Lock");
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(from, _amount));
    }
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(from, _amount);
