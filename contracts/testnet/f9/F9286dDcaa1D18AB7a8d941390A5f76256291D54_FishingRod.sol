...
contract FishingRod is ERC20, Ownable {
    ...
    uint totalUsers;
    uint totalStakedActual;
    uint totalStakedEffective;
...
//        stakeToken.transfer(MARKET, actualAmount);

        UserData storage userData = users[msg.sender];
        referralSystem.trySetReferrer(msg.sender, referrer == address(0) ? DEV_WALLET : referrer);
