...
contract SwapBingoPoint {
    ...
    function buyPoint(address _airdropToken, uint amount) payable external {
        require(IERC20(_airdropToken).balanceOf(msg.sender) >= amount, "Not enough balance");
        require(amount >= 1 ether, "Minimun 1 ether");

        IERC20(mainToken).transferFrom(msg.sender, treasuryAddress, amount);

        emit Deposit(msg.sender, _airdropToken, amount);
    }
