...
contract BridgeBase {
    ...
    mapping(address => mapping(uint => bridgeData)) public processedData;
    ...
    function burn(address to, uint amount, uint nonce, bytes calldata signature) external {
        ...
        processedData[msg.sender][nonce] = bridgeData(msg.sender, to, amount, block.timestamp, nonce, signature, Step.Burn);
    }
    ...
    function getNonce(address sender) public view returns (uint256) {
        return addressNonce[sender];
    }

    function getBridgeData(address sender, uint nonce) public view returns (bridgeData memory) {
        return processedData[sender][nonce];
    }
