...
// File: contracts/interfaces/ILootBoxVendor.sol
...
// File: contracts/LootBoxVendor.sol
...
contract LootBoxVendor is Ownable, ReentrancyGuard {
  ...
  mapping (uint8 => uint256) public saleTime;

  event BuyBox(address indexed buyer, uint8 boxType, uint price, uint[] tokenIds);
  
  constructor(address _lootBox, address _kabyToken) Ownable(){
    lootBox = ILootBox(_lootBox);
    kabyToken = IERC20(_kabyToken);
  }
  ...
  function setCurrency(address _currency) external onlyOwner {
    kabyToken = IERC20(_currency);
  }

  function putOnSale(uint8[] memory boxTypes, uint256[] memory listPrice, uint256[] memory timestamps) external onlyOwner{
    require(boxTypes.length == listPrice.length && boxTypes.length == timestamps.length, "length invalid");
    for (uint i = 0; i < boxTypes.length; i++) {
      require(boxTypes[i] <= lootBox.boxTypesCount(), "unknown box type");
      saleTime[boxTypes[i]] = timestamps[i];
      prices[boxTypes[i]] = listPrice[i];
    }
  }

  function endSale(uint8[] memory boxTypes) external onlyOwner {
    for (uint i = 0; i < boxTypes.length; i++) {
      saleTime[boxTypes[i]] = 0;
    }
  }

  function buyBox(uint8 boxTypes, uint256 allowPrice) external nonReentrant {
    require(saleTime[boxTypes] != 0 && saleTime[boxTypes] < block.timestamp, "Not on sale");
    uint256 payOut = prices[boxTypes];
    require(payOut <= allowPrice, "prive over");
    kabyToken.safeTransferFrom(msg.sender, owner(), payOut);
    uint[] memory tokenIds = lootBox.unbox(msg.sender, boxTypes);
    emit BuyBox(msg.sender, boxTypes, payOut, tokenIds);
  }
}