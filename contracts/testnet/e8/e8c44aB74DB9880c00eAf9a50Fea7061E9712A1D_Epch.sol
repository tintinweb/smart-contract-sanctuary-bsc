...
abstract contract Ownable is Context {
    ...
    function lock(uint256 time) public virtual onlyOwner {
        ...
        require((_lockTime - block.timestamp) > 30, "Can not set lock for more than 30 days");
...
contract DividendTracker is DividendPayingToken {
    ...
    function withdrawDividend() pure external override {
        require(false, "EUS_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EUS contract.");
    }
...
contract Epch is ERC20, Ownable {
    ...
    event ecosystemWalletUpdated(address indexed newLiquidityWallet, address indexed oldLiquidityWallet);
    
    event FeesUpdated(uint256 indexed newBNBRewardsFee, uint256 indexed newLiquidityFee, uint256 newEcosystemFee, uint256 newBuyBackFee);
    ...
    constructor() ERC20("Epch", "EPCH", 18) {
        ...
        sellBuybackFee = 0;
        sellMarketingFee = 2;
        sellTotalFees = sellRewardsFee + sellLiquidityFee + sellEcosystemFee + sellBuybackFee + sellMarketingFee ; // 15%
        ...
        buyBuybackFee = 0;
        buyMarketingFee = 2;
        buyTotalFees = buyRewardsFee + buyLiquidityFee + buyEcosystemFee + buyBuybackFee + buyMarketingFee; // 15%
    ...
    function addToBlackList(address updAds) public onlyOwner {
        _isBlacklisted[updAds] = true;
        emit WalletBlacklisted(updAds, true);
    }    

    function removeFromBlackList(address updAds) public onlyOwner {
    ...
    function updateSwapTokensAtAmount(uint256 newAmount) external onlyOwner returns (bool){
  	...
  	function updateDividendTracker(address newAddress) external onlyOwner {
        ...
        require(newDividendTracker.owner() == address(this), "EPOCH: The new dividend tracker must be owned by the EUS token contract");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "EPOCH: The liquidity wallet is already this address");
        excludeFromFees(newLiquidityWallet, true);
        emit LiquidityWalletUpdated(newLiquidityWallet, liquidityWallet);
        liquidityWallet = newLiquidityWallet;
    }
    
    // updates the ecosystem wallet (marketing, ecosystem etc.)
    function updateecosystemWallet(address newecosystemWallet) external onlyOwner {
        require(newecosystemWallet != ecosystemWallet, "EPOCH: The ecosystem wallet is already this address");
    ...
    function updateBuyFees(uint256 _EcosystemFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _marketingFee) external onlyOwner {
        buyEcosystemFee = _EcosystemFee;
        buyRewardsFee = _rewardsFee;
        buyLiquidityFee = _liquidityFee;
        buyBuybackFee = _buybackFee;
        buyMarketingFee = _marketingFee;
        buyTotalFees = buyRewardsFee + buyLiquidityFee + buyEcosystemFee + buyBuybackFee  + buyMarketingFee;
        require(buyTotalFees <= 20, "Total fee should not exceed 20%");
    }
    
    function updateSellFees(uint256 _EcosystemFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _buybackFee, uint256 _marketingFee) external onlyOwner {
        sellEcosystemFee = _EcosystemFee;
        sellRewardsFee = _rewardsFee;
        sellLiquidityFee = _liquidityFee;
        sellBuybackFee = _buybackFee;
        sellMarketingFee = _marketingFee;
        sellTotalFees = sellRewardsFee + sellLiquidityFee + sellEcosystemFee + sellBuybackFee + sellMarketingFee;
        require(sellTotalFees <= 20, "Total fee should not exceed 20%");
    }
    ...
    // @dev User Callable Functions start here!
  	...
  	function setRewardToken(address rewardTokenAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "EPOCH: setRewardToken:: Address is a wallet, not a contract.");
  	    require(rewardTokenAddress != address(this), "EPOCH: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "EPOCH: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	...
  	function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) external returns (bool) {
  	    require(isContract(rewardTokenAddress), "EPOCH: setRewardToken:: Address is a wallet, not a contract.");
  	    require(ammContractAddress != address(uniswapV2Router), "EPOCH: setRewardToken:: Use setRewardToken to use default Router");
  	    require(rewardTokenAddress != address(this), "EPOCH: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
  	    require(isWhitelistedToken(rewardTokenAddress), "EPOCH: setRewardToken:: Reward Token must be whitelisted to be used as rewards.");
  	    require(isAMMWhitelisted(ammContractAddress) == true, "EPOCH: setRewardToken:: AMM is not whitelisted!");
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _buyBackTokensToSwap + _marketingTokensToSwap + _rewardTokensToSwap + _ecosystemTokensToSwap;
        ...
        uint256 bnbForLiquidity = bnbBalance - bnbForMarketing - bnbForBuyback  - bnbForRewards - bnbForEcosystem;
