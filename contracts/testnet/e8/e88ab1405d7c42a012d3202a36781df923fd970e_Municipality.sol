...
contract Municipality is IMunicipality, OwnableUpgradeable, EIP712Upgradeable {
    ...
    IActivatableNFT public minnerContractAddress;
    IStandardParcel public standardParcelContract;

    uint256[4] public parcelMintingPricesInBUSD;

    uint256 public upgradePriceInBUSD;
    uint256 brokerBrunchPrice;
    uint256[5] public minerPrices;

    uint256 scarcityFactorMul1000000;

    uint256 buyAndBurnAmountVBTC;
    uint256 nftRankRewards;
    uint256 buyAndBurnAmountGYM;
    uint256 commissionWallet;

    address buyAndBurn;
    address vbtcAddress;
    address gymAddress;
    IERC20Upgradeable busdAddress;
    IMinerPublicBuilding minerContractAddress;
    IBrokerPublicBuilding brokerPublicBuilding;
    Bundle[6] bundles;

    address payable ambassadorAddress; // 5% , 5%, 5%
    address payable reserveWalletAddress; // 0.7%, 0.7%, 0.7%
    address payable companyWallet; // 20%, 20%, 20%, 20%
    ...
    modifier initialized() {
        require(
            address(minnerContractAddress) != address(0) &&
    ...
    function mintMultipleParcelsFor(
        ...
        evaluateParcelPrice(_parcels.length);
    }
    ...
    function mintMiner(MinerTypes _minerType, uint256 _value) external {
        ...
        busdAddress.safeTransferFrom(msg.sender, address(this), _value);
        uint256 tokenId = minerContractAddress.mintMiner(msg.sender);
    ...
    function buyBundles(
        ...
        for (uint256 k; k < _parcel.length; k++) {
            IParcel.Parcel memory parcel = IParcel.Parcel(
                _parcel[k]._x,
                _parcel[k]._y
            );
            require(
                _value >= _calculateBundlePrice(_parcel[k], _type),
                "Municipality: incorrect money amount"
            );
            require(
                _checkParcelIsFree(
                    _parcel[k],
                    bundles[uint8(_type)].parcelCount
                ),
                "Municipality: Some parcels are already minted"
            );
            busdAddress.safeTransferFrom(msg.sender, address(this), _value);

            brokerPublicBuilding.mintAdjacentQuadraticParcels(
                msg.sender,
                parcel,
                sqrt(bundles[uint8(_type)].parcelCount)
            );
            uint256 _tokenId;

            for (uint256 i = 0; i < bundles[uint8(_type)].minerCount; i++) {
                _tokenId = (minnerContractAddress.mintNFTTo(msg.sender));
                brokerPublicBuilding.assignOnStandardParcelByMunicipality(
                    parcel,
                    _tokenId,
                    msg.sender
                );
    ...
    function _calculateParcelsPrice(
        ...
        for (uint256 i = 1; i <= _parcels.length; i++) {
            uint256 price = (
                parcelMintingPricesInBUSD[uint8(_parcels[i]._type)]
            );
            if (uint256(_parcels[i]._type) == uint256(ParcelTypes.standard)) {
                finalPrice += (price / 1000000) * (scarcityFactorMul1000000**i);
            } else {
                finalPrice += parcelMintingPricesInBUSD[
                    uint8(ParcelTypes.Business)
                ];
            }
        }
    }
    ...
    function _calculateBundlePrice(
        ...
        for (
            uint256 i = 1;
            i <= bundles[uint256(_bundleType)].parcelCount;
            i++
        ) {
            uint256 price = (parcelMintingPricesInBUSD[uint8(_parcel._type)]);

            finalPrice += (price / 1000000) * (scarcityFactorMul1000000**i);
            (finalPrice / 100) * bundles[uint256(_bundleType)].discount;
        }
    }
    ...
    function _hash(MintParcelSignature calldata mintParcelSignature) private view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("SignatureMessage(uint value,uint256 x,uint256 y,uint256 type)"),
    ...
    function evaluateParcelPrice(uint256 _count) internal {
        ...
        emit EvaluateStandardParcelPrice(
            parcelMintingPricesInBUSD[uint8(ParcelTypes.standard)]
        );
    }
...
interface IMunicipality {
    ...
    function setBackendAddress(address _backend) external;
