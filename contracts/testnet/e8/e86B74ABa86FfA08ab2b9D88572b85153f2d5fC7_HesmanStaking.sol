...
contract HesmanStaking is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function claimToken() public nonReentrant isRun {
        ...
        emit EventUserClaim(msg.sender, amountClaim[msg.sender], block.timestamp);
    }
    ...
    function getInterest(address user) public view returns(uint256 nftTotalGain){ 
        nftTotalGain = (getCurrentBlock() - stakeBlock[user]).mul(nftIdList[msg.sender].length).mul(minInterest);
    }
