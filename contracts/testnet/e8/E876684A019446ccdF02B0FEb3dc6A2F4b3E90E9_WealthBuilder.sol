...
contract WealthBuilder {
    ...
    constructor() {
            owner = address(0x1FD6690a815E319c4f7dCcB51f3F79390d556e28);
            dev = address(0x1FD6690a815E319c4f7dCcB51f3F79390d556e28);
        ...
        for (uint256 i = 0; i < user.depoList.length; i++){	
            ...
            if (user.depoList[i].initialWithdrawn == false){
                ...
                if (elapsedTime > PercsKey[120].daysInSeconds && elapsedTime <= PercsKey[150].daysInSeconds){ // 120days-150days
                    uint256 dailyReturn = amount.mul(PercsKey[120].divsPercentage).div(percentdiv);
                    uint256 currentReturn = dailyReturn.mul(elapsedTime).div(1 days);
                    with += currentReturn;
                }
                if (elapsedTime > PercsKey[150].daysInSeconds){ // 150days-
                    uint256 dailyReturn = amount.mul(PercsKey[150].divsPercentage).div(percentdiv);
                    uint256 currentReturn = dailyReturn.mul(min(elapsedTime, 250 days)).div(1 days);
                    with += currentReturn;
                }
            } 
        }
