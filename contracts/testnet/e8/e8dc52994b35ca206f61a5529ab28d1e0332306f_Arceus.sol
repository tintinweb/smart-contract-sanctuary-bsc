...
contract Arceus is ERC20Detailed, Ownable {
    ...
    function burnArceus(uint256 amount) external onlyOwner virtual returns(uint amountburnt){
        ...
        _gonBalances[account] -= amountToBurn;
    ...
    function swapBack(uint256 _swapback) internal swapping {
        ...
        uint256 amountToSwap = _swapback.div(5e5).mul(totalFee);
        uint256 balanceBefore = address(this).balance;
    ...
    function claimTokens() external returns (string memory State) {
        ...
        if (TOTAL_TOKENS_CLAIMED >= REWARDS_ALLOCATION){
    ...
    function getRebaseIndexes() view external onlyOwner 
    ...
    returns(uint RebaseRate, uint RebaseTime, bool RebaseState)
    {
        return( REBASE_RATE, lastRebaseTime(), _autoRebase);
    }
