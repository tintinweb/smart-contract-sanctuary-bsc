...
contract BoardroomContractEF is ReentrancyGuard, AccessControl {
    ...
    function getGeneratedReward(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (snapshots[_pid].length > 0) {
            ...
            for (uint256 i = snapshots[_pid][snapshots[_pid].length - 1].blockNumber; i >= user.lastClaimedEpoch; i--) {
                Snapshot storage snapshot = snapshots[_pid][i];
    ...
    function getGeneratedRewardsBalance() external view returns (uint256) {
