...
contract BoxNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, PausableUpgradeable{
  ...
  function initialize() public initializer {  
      __ERC721_init("Box NFT NBASE", "BONN");
      openBoxActive=true;
      __Ownable_init();
    }

    function initByOwner(CharacterNFT _characterNFT,address _boxNFTRound, address _operator,Utils _utils) public  onlyOwner {
...
function openBox(uint256 tokenId) external whenNotPaused returns (uint256 characterId){
        require(openBoxActive==true, "openBox not active");
        require(_exists(tokenId),"Not existed");
        require(msg.sender == ownerOf(tokenId),"You are not owner");
        boxes[msg.sender][tokenId]= Box(0,0);
        // uint256 typeCharacter = utils.random(1,3);
        characterId=characterNFT.createCharacter(msg.sender,1,1,1,1,0); 
        // uint256 numberAmor= 1 + utils.random(5,10)%3;
        // uint256 numberLeftHand= 1 + utils.random(15,30)%3;
        // uint256 numberRightHand= 1 + utils.random(31,40)%3;
        // uint256 numberLegs= 1 + utils.random(41,50)%3;
        // characterNFT.createSuit(msg.sender,characterId,numberAmor,numberLeftHand,numberRightHand,numberLegs); 
        _burn(tokenId);
    }
...
function createSuit(address owner,
  ...
  uint256 legs) public onlyBoxNFTOrOwnerOrCyborgLabOrBattery {
        suits[owner][newCharacterId] = encodeSuit(Suit(newCharacterId,amor,leftHand,rightHand,legs));
    }
  ...
  function updateSuit(address owner,uint256 nftId, uint256 id, uint256 amor, uint256 leftHand, uint256  rightHand,uint256 legs) public onlyOwner returns (uint256) {
        suits[owner][nftId] = encodeSuit(Suit(id, amor, leftHand,rightHand, legs));
        return nftId;
    }
  ...
  function getSuit(address owner, uint256 id) public view returns (Suit memory _suit) {
    ...
    _suit.amor = uint256(uint8(details>>132));
    _suit.leftHand = uint256(uint8(details>>140));
    _suit.rightHand =uint256(uint8(details>>148));
...
function getSuitPublic(address _owner, uint256 _id) public view returns (
    ...
    id = _suit.id;
    amor =  _suit.amor;
    leftHand =  _suit.leftHand;
    rightHand = _suit.rightHand;
    legs = _suit.legs;
  ...
  // function encode(Character memory character)  external view returns  (uint256) {
   ...
   function encodeSuit(Suit memory suit) public pure returns (uint256){
    ...
    value |= suit.id << 100;
    value |= suit.amor << 132;
    value |= suit.leftHand << 140;
    value |= suit.rightHand << 148;
    value |= suit.legs << 156;
    return value;
  }
