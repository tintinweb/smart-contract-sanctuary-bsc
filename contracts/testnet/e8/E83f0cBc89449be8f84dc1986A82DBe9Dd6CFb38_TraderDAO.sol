...
contract TraderDAO is Ownable{
    ...
    function Insert(address userAddress, uint amount) public{
        ...
        emit UserDeposit(msg.sender, PoolIdCounter, amount);
    }
    ...
    function DistributeReward(uint _poolid, uint[] memory _ids, uint[] memory _amounts, uint _poolRealizedAmount) external onlyOwner{
        ...
        for(uint i = 0; i< _ids.length; i++) {
            ...
            if(_poolRealizedAmount > PoolTradeAmount[_poolid]){
                
            }

            UserReliazedAmount[_ids[i]] = UserReliazedAmount[_ids[i]] + _amounts[i];

            if(UserReinvestStatus[UserList[_ids[i]]] == true){
                Insert(UserList[i], _amounts[_ids[i]]);
            }
            else{
                UserAmountToClaim[UserList[_ids[i]]] = UserAmountToClaim[UserList[_ids[i]]] + _amounts[i];
            }
        }

        PoolRealizedAmount[_poolid] = _poolRealizedAmount;
    }
