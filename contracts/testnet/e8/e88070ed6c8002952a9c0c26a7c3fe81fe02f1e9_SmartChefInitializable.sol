...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when eMonster mining ends.
    uint256 public bonusEndBlock;

    // The block number when eMonster mining starts.
    ...
    // eMonster tokens created per block.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 eMonsterReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(eMonsterReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 eMonsterReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(eMonsterReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
...
contract SmartChefFactory is Ownable {
    ...
    function deployPool(
        ...
        poolAddress.push(smartChefAddress);

        emit NewSmartChefContract(smartChefAddress);
    }
    
    function igoLength() public view returns (uint256) {
