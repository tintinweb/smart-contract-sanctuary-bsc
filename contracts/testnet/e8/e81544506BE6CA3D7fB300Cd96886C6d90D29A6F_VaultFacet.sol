...
contract VaultFacet is IVault {
    ...
    bytes32 public constant TRANSFER_TO_EXCHANGE_TREASURY_ROLE = keccak256("TRANSFER_TO_EXCHANGE_TREASURY_ROLE");
    ...
    function itemValue(address token) external view override returns (LpItem memory lpItem) {
        ...
        return _itemValue(at);
    }

    function _itemValue(LibVault.AvailableToken storage at) internal view returns (IVault.LpItem memory lpItem) {
    ...
    function totalValue() external view override returns (LpItem[] memory lpItems) {
        ...
        for (uint256 i; i < tokenAddresses.length; i++) {
            ...
            lpItems[i] = _itemValue(at);
        }
    }

    function transferToExchangeTreasury(address tokenAddress, uint256 amount) external override {
        LibAccessControlEnumerable.checkRole(TRANSFER_TO_EXCHANGE_TREASURY_ROLE);
        require(amount > 0, "VaultFacet: Amount must be greater than 0");
        LibVault.VaultStorage storage vs = LibVault.vaultStorage();
        LibVault.AvailableToken storage at = vs.tokens[tokenAddress];
        require(at.weight > 0, "VaultFacet: Token does not exist");
        IERC20 token = IERC20(tokenAddress);
        require(token.balanceOf(address(this)) >= amount, "VaultFacet: Insufficient balance");
        token.safeTransfer(EXCHANGE_TREASURY_ADDRESS, amount);
        emit TransferToExchangeTreasury(tokenAddress, amount);
    }
...
interface IVault {
    ...
    event TransferToExchangeTreasury(address indexed token, uint256 amount);
    ...
    function transferToExchangeTreasury(address token, uint256 amount) external;
}
