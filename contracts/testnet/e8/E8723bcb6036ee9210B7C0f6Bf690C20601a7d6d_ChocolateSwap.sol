...
contract ChocolateSwap is ERC20, Ownable {
    ...
    uint256 public totalBuyFee = buyRewardFee + buyMarketingFee + buyLiquidityFee;
    uint256 public totalSellFee = sellRewardFee + sellMarketingFee + sellLiquidityFee;

    uint256 public rewardShare = 40;
    ...
    address public operatorWallet = 0x29224fC97BcB2Df0ceCB56BC1E7b579708fFa3ea;
    ...
    modifier onlyOperator() {
        require(_isOperator(msg.sender), "Caller not Operator");
        _;
    }

    function _isOperator(address addr) private view returns (bool){
        return addr==operatorWallet;
    }
    ...
    //baby doge => deploying
    address public immutable rewardToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;//BUSD
    uint256 public gasForProcessing = 300000;

    event ExcludeFromFees(address indexed account, bool isExcluded);
    event ExcludedFromMaxTransactionLimit(address indexed account, bool isExcluded);
    event ExcludedFromMaxWalletLimit(address indexed account, bool isExcluded);
    event FeesBuyUpdated(uint256 buyRewardFee, uint256 buyMarketingFee, uint256 buyLiquidityFee);
    event FeesSellUpdated(uint256 sellRewardFee, uint256 sellMarketingFee, uint256 sellLiquidityFee);
    ...
    constructor() payable ERC20("ChocolateSwap", "Chocolate") {
        
        require(maxTransactionRateBuy >= 1 && maxTransactionRateSell >= 1, "Max transfer rates must be greater than 0.1%");

    	dividendTracker = new DividendTracker(totalSupply() / 50000, rewardToken);
        ...
        //Supply 990 000 000
        swapTokensAtAmount = 990_000_000 * (10 ** 18) / 5000;
        _mint(owner(), 990_000_000 * (10 ** 18));
    }
    ...
    function updateBuyFees(uint256 _buyRewardFee, uint256 _buyMarketingFee, uint256 _buyLiquidityFee) external onlyOwner {
        require(_buyRewardFee + _buyMarketingFee + _buyLiquidityFee <= 30, "Fees must be less than 30%");
        buyRewardFee = _buyRewardFee;
        buyMarketingFee = _buyMarketingFee;
        buyLiquidityFee = _buyLiquidityFee;
        totalBuyFee = buyRewardFee + buyMarketingFee + buyLiquidityFee;
        emit FeesBuyUpdated(buyRewardFee, buyMarketingFee, buyLiquidityFee);
    }

    function updateSellFees(uint256 _sellRewardFee, uint256 _sellMarketingFee, uint256 _sellLiquidityFee) external onlyOwner {
        require(_sellRewardFee + _sellMarketingFee + _sellLiquidityFee <= 30, "Fees must be less than 30%");
        sellRewardFee = _sellRewardFee;
        sellMarketingFee = _sellMarketingFee;
        sellLiquidityFee = _sellLiquidityFee;
        totalSellFee = sellRewardFee + sellMarketingFee + sellLiquidityFee;
        emit FeesSellUpdated(sellRewardFee, sellMarketingFee, sellLiquidityFee);
    }
    ...
    function updateShare(uint256 _rewardShare, uint256 _marketingShare, uint256 _LiquidityShare) external onlyOwner{
        require(_rewardShare + _marketingShare + _LiquidityShare == 100,"Sum Of All Share Values should be 100");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 bnbShare = marketingShare + rewardShare + LiquidityShare;
            
            if(contractTokenBalance > 0 && bnbShare > 0) {
               
                if(LiquidityShare > 0){
                    uint256 LiquidityToken = contractTokenBalance * LiquidityShare / bnbShare;
                    contractTokenBalance -= LiquidityToken;
