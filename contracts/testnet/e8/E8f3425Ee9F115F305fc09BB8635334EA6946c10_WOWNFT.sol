...
contract WOWNFT is ERC721, Pausable, Ownable {
    ...
    // Event that logs every buy operation
    event BoughtNFT(address _buyer, uint256 _price, uint256 _tokenId);
    event SoldNFT(address _seller, uint256 _price, uint256 _tokenId);

    constructor(
      address _adminAddress,
      address _WCoinTokenAddress) ERC721("WOWNFT", "WOW") {
        totalSupply = 0;
        feeEnabled = true;
        transferable = true;
        adminEnabled = true;
        ...
        priceFeed = AggregatorV3Interface(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE); // Chainlink BNB/USD Price Feed on BSC
    }
    ...
    function enableAdmin() public onlyOwner {
        adminEnabled = true;
    }

    function disableAdmin() public onlyOwner {
        adminEnabled = false;
    }

    function disableWhitelist() public {
        if (adminEnabled) {
            require(msg.sender == adminAddress || msg.sender == owner(), "Only callable by an approved admin or owner!");
        } else {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://nft.cryptowao.io/metadata/";
    }
    ...
    /**
     * @notice Add bought NFT to the listings
     */
    function addListing(uint256 _price, uint256 _tokenId) private {
        listings[_tokenId] = Listing(_price, block.timestamp);
    }

    /**
     * @notice Mints NFT to the smart contract
     */
    function safeMint(address _minter, uint256 _price) external returns(uint256) {
        if (adminEnabled) {
            require(msg.sender == adminAddress || msg.sender == owner(), "Only callable by an approved admin or owner!");
        } else {
            require(msg.sender == owner(), "Only callable by the owner!");
        ...
        _safeMint(_minter, nftId);
        totalSupply = totalSupply + 1;
        tokens[nftId] = _price;
        return nftId;
    }

    function buy(address _receiver, uint256 _tokenId, uint256 _price) payable public whenNotPaused {

        if (whitelistEnabled) {
            require(contains(msg.sender), "Only whitelisted users can buy");
        }

        uint256 totalTokensAmount = 10000e18 / WCoinToken.getTokenPrice(); // $10000 Transaction limit
        require(_price <= totalTokensAmount, "Price is over transaction limit!");

        uint256 finalPrice = tokens[_tokenId];
        if (feeEnabled) {
          finalPrice = finalPrice + getBNBFee(); // NFTPrice + 0.95 USD
        }

        require(_price >= finalPrice, "Insufficient funds sent");

        uint256 limitSupply = totalSupply / 500; // 0.02% of total supply
        require(balanceOf(msg.sender) <= limitSupply, "Unable to buy. Holding limit achieved");

        safeTransferFrom(msg.sender, _receiver, _tokenId);

        WCoinToken.transfer(msg.sender, msg.value);
        addListing(_price, _tokenId);
        emit BoughtNFT(msg.sender, _price, _tokenId);
    }

    function sell(uint256 _tokenId, uint256 _price, address _buyer) payable public whenNotPaused {
        Listing memory item = listings[_tokenId];
        require(item.timestamp >= item.timestamp + 60 seconds, "You have to wait at least 60 seconds to sell the NFT");

        require(_price <= 10000e18, "Price is over transaction limit!");

        safeTransferFrom(msg.sender, _buyer, _tokenId);

        WCoinToken.transferFrom(_buyer, msg.sender, msg.value);
        emit SoldNFT(msg.sender, _price, _tokenId);
    }
        ...
        override
    {
        ...
        require(transferable, "Transfers are paused!");
     ...
     */
    function getCurrentWCoinBalance() external view returns(uint256) {
        return WCoinToken.balanceOf(address(this));
    }

    /**
     * @notice Get current TokenID
     */
