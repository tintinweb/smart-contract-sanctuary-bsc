...
contract AVVR is Context, IERC20, Ownable{
    ...
    string private _name = "ADULTVR";
    string private _symbol = "AVVR";
    ...
    function burn(uint256 tAmount) external {
        ...
        _rOwned[BURN_ADDRESS] = _rOwned[BURN_ADDRESS].add(rAmount);
        if(_isExcluded[_msgSender()]){
            _tOwned[_msgSender()] = _tOwned[_msgSender()].sub(tAmount);}
        if(_isExcluded[BURN_ADDRESS]){
            _tOwned[BURN_ADDRESS] = _tOwned[BURN_ADDRESS].add(tAmount);}
        emit Transfer(_msgSender(), BURN_ADDRESS, tAmount);
    }
