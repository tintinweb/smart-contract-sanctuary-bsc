...
contract PlearnLucky is IPlearnLucky, ReentrancyGuard, Ownable {
    ...
    function confirmTicket(
        ...
        emit TicketsConfirm(msg.sender, _luckyId, ticketId, _bracket, _isHaveFollower, _name);
    }
     ...
     * @dev Callable by operator
     */
    function drawLuckyNumbersAndMakeTicketConfirmable(uint256 _luckyId) external onlyOperator nonReentrant {
        Lucky storage luckyInfo = _lucks[_luckyId];
    ...
    function setConfirmBracket(address _confrimBracketAddress) external onlyOwner {
        require(_lucks[currentLuckyId].status != Status.Claimable, 'Lucky not in claimable');

        IConfirmBracket(_confrimBracketAddress);
        confirmBracket = IConfirmBracket(_confrimBracketAddress);

        emit NewConfirmBracket(_confrimBracketAddress);
    }
