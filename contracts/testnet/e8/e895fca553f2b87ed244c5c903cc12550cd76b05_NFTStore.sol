...
contract NFTStore is Ownable, SignerRole, StoreBase {
    ...
    event SetMintPrice(address owner, uint256 amount);

    //modifier
    modifier isOperator() {
        require(operators[msg.sender] == true || msg.value >= mintPrice, "Minting amount exceeds");
        _;
    }

    constructor(string memory _name,string memory _symbol,address _signer) public {
        name = _name;
        symbol = _symbol;
    ...
    function setMintPrice(uint256 amount)public onlyOwner{
        mintPrice = amount;
        emit SetMintPrice(msg.sender, amount);
    }
    ...
    function mint(
        ...
        uint256 supply,
        string memory uri
    ) public payable isOperator returns (bool) {
        require(
    ...
    function withdraw(address to)external onlyOwner returns(bool){
        ...
        require(payable(to).send(balance),"withdraw fail");
        return true;
    }
}