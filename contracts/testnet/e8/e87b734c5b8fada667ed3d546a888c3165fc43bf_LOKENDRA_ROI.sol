...
contract LOKENDRA_ROI {
    ...
    uint public tokenRewardTeam;
    uint public topupReward;
    ...
    constructor(address _token, address stableCoin_) public {
        ...
        level_income = tokenReward / 100 * 3;
    ...
    function Registration(uint _referrerID, uint256 _amount) public {
        ...
        require(_referrerID > 0 && _referrerID <= currUserID,"Incorrect referral ID");
            ...
            tokenRewardTeam = tokenReward/10;
  
        UserStruct memory userStruct;
        currUserID++;
        
        LEVEL_PRICE[1] = tokenReward / 10*7;
        LEVEL_PRICE[2] = tokenReward / 10 * 3;
        level_income = tokenReward / 10* 3;
        userStruct = UserStruct({
            isExist: true,
            id: currUserID,
        ...
        token.freezeToken(msg.sender, tokenReward/2); // Transfer Rewarded Token
...
function topUp(uint256 _amount) public{
     ...
     */
     ...
     uint256 numDays = (block.timestamp - startTime) / 60 / 60 / 24;
    if (numDays>1){
