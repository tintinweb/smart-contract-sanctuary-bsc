...
contract SigmaGlobal is SigmaGlobal_Status {
			...
			for(uint256 i; i < MAX_REFERRAL_LEVEL; i++) {
				if(upline != address(0)) {
					if(i <= users[upline].referrerCount[0]) {
						...
						users[upline].machineDeposits[machineLength].amount = amount;
	...
	function withdraw_t() external whenNotPaused checkUser_ returns(bool) {
		...
		for(uint i; i < user.machineLength; i++) {
			...
			if(machineBonus.withdrawn < machineBonus.initAmount) {
				dividends = calculateMachineDividents(machineBonus, userPercentRate, user);
				if(dividends > 0) {
					user.deposits[i].withdrawn = user.deposits[i].withdrawn.add(dividends); /// changing of storage data
					totalAmount = totalAmount.add(dividends);
				}
	...
	function calculateMachineDividents(MachineBonus memory deposit, uint256 userPercentRate, User memory user) internal view returns (uint256) {
		uint256 dividends;
		uint256	depositPercentRate = userPercentRate.add(MACHINE_ROI);
