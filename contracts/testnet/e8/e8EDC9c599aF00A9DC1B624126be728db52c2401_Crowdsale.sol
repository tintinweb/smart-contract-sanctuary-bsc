...
contract Crowdsale is Vesting {
    ...
    constructor(address token_) Vesting(token_) {
        require(token_ != address(0x0), "Crowdsale: invalid token address!");
    ...
    function init(
        ...
        require(presaleStatus() != 0, "Crowdsale: presale is already active!"); // QUED
    ...
    function updateTime(uint256 _startTime, uint256 _endTime)
        ...
        onlyOwner
    {
        require(presaleStatus() == 0, "Crowdsale: presale is already started!");
        INFO.START_TIME = _startTime;
        INFO.END_TIME = _endTime;
    }
    ...
    function userDeposit(uint256 _amount) external payable nonReentrant {
        ...
        require(presaleStatus() == 1, "Crowdsale: presale is not active!"); // ACTIVE
        ...
        require(tokensSold > 0, "Crowdsale: zero tokens!");
    ...
    function userWithdrawTokens() external nonReentrant {
        require(
            ...
            "Crowdsale: awaiting LP generation!"
        );
        ...
        require(tokensOwed > 0, "Crowdsale: nothing to withdraw!");
    ...
    function userWithdrawBaseTokens() external nonReentrant {
        require(presaleStatus() == 3, "Crowdsale: not failed!"); // FAILED

        BuyerInfo storage buyer = BUYERS[_msgSender()];
        require(buyer.baseDeposited > 0, "Crowdsale: nothing to withdraw!");
    ...
    function forceFailByOwner() external onlyOwner {
        require(!STATUS.FORCE_FAILED, "Crowdsale: sale is force failed!");
    ...
    function addLiquidity(address _pair) external onlyOwner {
        require(STATUS.LP_GENERATION_COMPLETE, "Crowdsale: pool exists!");
        require(presaleStatus() == 2, "Crowdsale: sale is not success!");
        ...
        require(totalLPTokensMinted != 0, "Crowdsale: LP creation failed!");
