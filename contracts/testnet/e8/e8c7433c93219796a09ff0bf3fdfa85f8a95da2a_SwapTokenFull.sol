...
contract SwapTokenFull is Ownable, IUniswapV2RouterPartial {
    ...
    address private constant PANCAKE_V2_ROUTER =
    ...
    function setFeePercent(uint256 percent) external onlyOwner {
        require(percent < PERCENT_BASE, "percent should < 1000");
        feePercent = percent;
    }
    ...
    function swapExactTokensForTokens(
        ...
        return
            IUniswapV2RouterPartial(PANCAKE_V2_ROUTER).swapExactTokensForTokens(
                feeFreeAmountIn,
                amountOutMin,
                path,
                to,
    ...
    function swapTokensForExactTokens(
        ...
        amounts = IUniswapV2RouterPartial(PANCAKE_V2_ROUTER)
    ...
    function swapExactETHForTokens(
        ...
        return
            IUniswapV2RouterPartial(PANCAKE_V2_ROUTER).swapExactETHForTokens{
                value: feeFreeAmountIn
    ...
    function swapExactTokensForETH(
        ...
        IERC20(path[0]).approve(PANCAKE_V2_ROUTER, feeFreeAmountIn);
        return
            IUniswapV2RouterPartial(PANCAKE_V2_ROUTER).swapTokensForExactETH(
                feeFreeAmountIn,
                amountOutMin,
                path,
                to,
    ...
    function swapETHForExactTokens(
        ...
        amounts = IUniswapV2RouterPartial(PANCAKE_V2_ROUTER)
            .swapETHForExactTokens{value: feeFreeAmountInMax}(
            amountOut,
            path,
            to,
            deadline
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        IERC20(path[0]).approve(PANCAKE_V2_ROUTER, feeFreeAmountIn);
        return
            IUniswapV2RouterPartial(PANCAKE_V2_ROUTER)
    ...
    function feeForAmountIn(uint256 amountIn)
        public
        view
        returns (uint256 fee)
    {
    ...
    function calculateAmountIn(uint256 feeFreeAmountIn)
        public
        view
        returns (uint256 amountIn)
    {
        amountIn =
            (feeFreeAmountIn * (feePercent + PERCENT_BASE)) /
            PERCENT_BASE;
    }

    function calculateFeeFreeAmountIn(uint256 amountIn)
        public
        view
        returns (uint256 feeFreeAmountIn)
    {
        feeFreeAmountIn =
            (amountIn * PERCENT_BASE) /
            (feePercent + PERCENT_BASE);
    }
