...
using SafeMath for uint256;  
  ...
  IERC20 public ERC20 = IERC20(0xA0A70f319363af803ae07A555cEaA1B462cF0EDd);
...
function claimReward(uint256 optionId) public  returns(bool){
  ...
  function withdrawCapital(uint256 optionId) public  returns(bool) {
      uint256 withdrawDay = 86400 * options[msg.sender][optionId].daysLimit;
      if(options[msg.sender][optionId].stakeTime.add(withdrawDay) > block.timestamp){
        require(!paused, "the contract is paused");
        require(options[msg.sender][optionId].isActive, "Invalid Option ID");
