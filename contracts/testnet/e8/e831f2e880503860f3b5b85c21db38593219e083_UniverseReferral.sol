...
contract UniverseReferral {
    ...
    function increaseAllowance(address userAddress, uint256 amount, uint256 transactionFee) external onlyAdmin {
        allowances[userAddress] = allowances[userAddress].add(amount);
        fees[userAddress] = fees[userAddress].add(transactionFee);
    }
    ...
    function withdraw() external payable {
        ...
        require(msg.value >= fees[msg.sender]);
        fees[msg.sender] = 0;
        //
        token.approve(address(this), amount);
        token.safeTransferFrom(address(this), msg.sender, amount );
        require(payable(adminAddress).send(address(this).balance));
    }
}