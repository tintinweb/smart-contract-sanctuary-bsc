...
////////////////////////////////////////// StakingStorage - AmountInBase view methods
    ...
    function getCurrentAmountInBaseWithAllBoostsAndCurrentTimeRewardPercent(
        ...
        public
        view
        returns (uint256 amountInBase, uint256 currentTimedRewardInPercent)
    {
        ...
        uint256 _timeRewardPeriodBegin;
        if (accountLastClaimTime[_account] == 0) {
            _timeRewardPeriodBegin = accountPeriodDataByPeriodNumber[_account][0].timeBegin;
        } else {
            _timeRewardPeriodBegin = accountLastClaimTime[_account];
        }

        if (_timeRewardPeriodBegin == 0) {
            currentTimedRewardInPercent = 0;
        } else {
            currentTimedRewardInPercent = block.timestamp - accountLastClaimTime[_account];

            uint256 _currentTimeRewardPeriod = block.timestamp - _timeRewardPeriodBegin;
            if (timedRewardPeriod <= _currentTimeRewardPeriod) {
                currentTimedRewardInPercent = timedRewardInPercent * _currentTimeRewardPeriod / timedRewardPeriod;
            } else {
                currentTimedRewardInPercent = timedRewardInPercent;
            }
        }
        return (amountInBase, currentTimedRewardInPercent);
    }
...
////////////////////////////////////////// fullBalance DevOnly todo rm in prod
    ...
    function devOnlyHardResetApproximateFullBalanceInBase()
        public
    {
        approximateFullBalanceInBase = __getFullAmountInBase();
    }
...
////////////////////////////////////////// methods for view
        ...
        returns (uint256 _rewardAllowedToClaim, uint256 _rewardInDelay)
    {
        ...
        uint256 _period = block.timestamp - stakingStartDate;
        ...
        returns (uint256 _rewardAllowedToClaim, uint256 _rewardInDelay)
    {
        ...
        _medianOfBalance = (_cumulativeFullBalance - _cumulativeDelayBalance) / _period;
        _medianOfPercent = (_cumulativeFullPercent - _cumulativeDelayPercent) / _period;
        ...
        )
    {
        ...
        for (uint256 i = accountLastPeriodNumber[_account]; i > 0; --i)
        {
...

            {
                ...
                if (_periodDelayEnd < _periodBegin) {
                    ...
                    _cumulativeDelayBalance += (_periodEnd - _periodDelayEnd) * _periodAmountInBase;
        ...
        uint256 _calculatedCumulativeTimePercentInEnd = __calculateCumulativeTimePercent(_periodBegin, _timedRewardPeriodEnd);
        _cumulativeFullPercent += _calculatedCumulativeTimePercentInEnd;
        if (_periodDelayEnd >= _periodBegin) {
            _cumulativeDelayPercent += _calculatedCumulativeTimePercentInEnd;
        } else if (_calculationOfCumulativeDelayPercentInProcess) {
            _cumulativeDelayPercent += _calculatedCumulativeTimePercentInEnd - __calculateCumulativeTimePercent(_periodBegin, _periodDelayEnd);
        }
    }
...
////////////////////////////////////////// methods for claim
        ...
        )
    {
        ...
        _cumulativeAllowedPercent += __calculateCumulativeTimePercent(_periodEnd, _timedRewardPeriodEnd);
    }
}
...
////////////////////////////////////////// methods for claim
        ...
        internal
    {
        ...
        IStablecoin(rewardCurrency).mint(_account, _rewardAllowedToClaim);
    }
}
