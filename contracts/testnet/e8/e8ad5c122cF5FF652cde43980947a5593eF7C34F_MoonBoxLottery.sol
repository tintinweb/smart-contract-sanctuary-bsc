...
contract MoonBoxLottery is Ownable, ReentrancyGuard {
    ...
    function buyTicket(uint256 numberOfTicket) external nonReentrant {
        ...
        require(token.transfer(treasuryWallet, amountToTreasury), "Failure");

        emit BuyTicket(periodNumber, msg.sender, numberOfTicket, ticketPrice);
    }
...
contract MoonBoxCashBack is Ownable, ReentrancyGuard {
    ...
    mapping(Package => CashBack) public cashBacks;
    mapping(address => uint256) public unClaimAmount;
    ...
    function isPackageEnable(Package _package) external view returns (bool) {
        CashBack memory cashBack = cashBacks[_package];
        uint256 remainingAmount = address(this).balance.sub(
            totalUnClaim.sub(totalClaim)
        );
        return cashBack.enable && cashBack.returnAmountBnb <= remainingAmount;
    }
