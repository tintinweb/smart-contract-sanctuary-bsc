...
contract CloudaxLaunchpad is Ownable, ReentrancyGuard {
    ...
    event ReferralEarned(address referrer, address refereed, uint256 amountPurchased, uint256 refferalEarnings, uint256 time);

    uint256 public vestedTokens;
    uint256 referrerPercent = 100; // % times 100 = 1%;
    // uint256 referrerPercent = 500; // % times 100 = 5%;
    ...
    mapping(Presale => mapping(address => uint256)) saleParticipantsCap;
    ...
    mapping(Presale => mapping(address => uint256)) public referralEarning;
    enum Presale {
        Angel,
        Private,
        Public
    ...
    constructor(
        ...
        uint256 _privateSaleMin,
        uint256 _privateSaleMax,
        ...
        require(
            _privateSaleMin <= _privateSaleMax,
            "Private sale min must be less than or equal to private sale max"
        );
        require(
            _privateSaleCap > _privateSaleMax,
            "Private sale cap must be greater than private sale max"
        );
        require(
        ...
        privateSale.saleMin = _privateSaleMin;
        privateSale.saleMax = _privateSaleMax;
    ...
    function stopLaunchpadSale(Presale sale) external onlyOwner {
        ...
        launchpadSale.saleEndDate = currentTime;
        emit LaunchpadSaleStopped(currentTime, sale);
    }
    ...
    function _createVestingSchedule(
        ...
        if (sale == Presale.Angel) {
            ...
            cliffPeriod = 10 seconds;
            vestingPeriod = 10 seconds;
        }
        if (sale == Presale.Private) {
            // cliffPeriod = 120 days;
            cliffPeriod = 10 seconds; // For test/demo purposes
            // vestingPeriod = 12 * 30 days;
            vestingPeriod = 10 seconds; // For test/demo purposes
        } else {
            cliffPeriod = 0;
        ...
        newSchedule.totalAmount = newSchedule.totalAmount.add(amount);
    }

    function buyLaunchpadSale(
        Presale sale,
        uint256 busdAmount,
        address referrer
    ) external nonReentrant {
        _buyLaunchpadSale(sale, busdAmount, referrer);
    }

    // Test
    function _buyLaunchpadSale(
        Presale sale,
        uint256 busdAmount,
        address referrer
    ) internal {
        ...
        bool referrerIsValid = referrer != address(0) && referrer != msg.sender;
        require(launchpadSale.isActive == true, "Sale is not active");
        ...
        require(
                saleParticipantsCap[sale][msg.sender] <= launchpadSale.saleMax,
                "You have reached your maximum presale purchase cap"
            );
        require(
        ...
        if (referrerIsValid) {
            referralEarning[sale][referrer] += tokenAmount
                .mul(referrerPercent)
                .div(10000);
           emit ReferralEarned(referrer, msg.sender, tokenAmount, tokenAmount
                .mul(referrerPercent)
                .div(10000), getCurrentTime());
        }
        if (sale == Presale.Private) {
        ...
        if (saleParticipantsCap[sale][msg.sender] != 0) {
            saleParticipantsCap[sale][msg.sender] = saleParticipantsCap[sale][
                msg.sender
            ].add(busdAmount);
        } else {
            saleParticipantsCap[sale][msg.sender] = busdAmount;
        }

        launchpadSale.sold = launchpadSale.sold.add(busdAmount);
        emit LaunchpadSaleBought(msg.sender, busdAmount, sale);
    }
    // test the referral bonus fix
    // that users can only buy their salescap
    ...
    function _release(
        ...
        uint256 saleEndDate = launchpadSales[uint256(sale)].saleEndDate;
        require(
            saleEndDate > 0 && getCurrentTime() >= saleEndDate,
            "Should receive token after presale"
        );
        require(
            currentTime >=
    ...
    function getReferralEarnings(Presale sale, address user)
        public
        view
        returns (uint256)
    {
        return referralEarning[sale][user];
    }

    function claimReferralEarnings(Presale sale) external nonReentrant {
        uint256 _earnings = getReferralEarnings(sale, msg.sender);
        uint256 saleEndDate = launchpadSales[uint256(sale)].saleEndDate;
        require(_earnings > 0, "No referral earnings");
        require(
            saleEndDate > 0 && getCurrentTime() >= saleEndDate,
            "Should claim after presale"
        );
        referralEarning[sale][msg.sender] = 0;
        _token.transfer(msg.sender, _earnings);
    }

    function _withdrawResidualBalance(address wallet) internal {
        require(!launchpadSales[0].isActive, "angel sale still active");
        require(!launchpadSales[1].isActive, "private sale still active");
        require(!launchpadSales[2].isActive, "public sale still active");
    ...
    function addAllocation(
        ...
        // uint256 newAllocationVolume = allocation;
        // Allocation increment alone
    ...
    // test increment of sale allocation
    ...
    function getParticipantsSaleCap(Presale sale)
        external
        view
        returns (uint256)
    {
        return saleParticipantsCap[sale][msg.sender];
    }
}
