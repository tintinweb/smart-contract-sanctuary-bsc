...
contract Crowdsale is Context, ReentrancyGuard {
    ...
    uint256 public minBuy = 0.01 ether;              // 100   ether;
    uint256 public maxBuy = 0.05 ether;              // 750   ether;
    uint256 public sale_price  = 0.0043 ether;      // 0.043 ether
    ...
    constructor (uint256 buyTime_, uint256 lockTime, uint256 claimTime_, address payable manager_, 
        ...
        USDT = IERC20(0xD500E3CF845CC41ABa54bB1dD426bD3485bD8Cd5);      // BUSD address on BSC testnet
    }
...
contract Manager is Context, Ownable {
    ...
    event Delivered(uint256 amountPair);
    event Finalized(uint256 amountToken, uint256 amountETH);
    ...
    constructor(address token_) {
        ...
        address _uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .getPair(token_, _uniswapV2Router.WETH());
    ...
    function setInitialTokenSupply(uint256 amount) public onlyOwner {
    ...
    function deliverLiquidityToReservesManager(address _reservesManager) public onlyOwner {
        require(!delivered, "IDO: ALREADY_DELIVERED");
        require(finalized, "IDO: NOT_FINALIZED");
        
        require(block.timestamp >= unlockTimestamp, "IDO: STILL_LOCKED");
        
        uint256 _amountPair = IERC20(uniswapV2Pair).balanceOf(address(this));
        IERC20(uniswapV2Pair).safeTransfer(_reservesManager, _amountPair);
        
        delivered = true;
        emit Delivered(_amountPair);
    }

    function _swapUSDTtoETH() internal {
        address sale_addr = rounds[rounds.length - 1];
        Crowdsale sale = Crowdsale(payable(sale_addr));
        IERC20 USDT = sale.USDT();

        uint tokenBalance = USDT.balanceOf(address(this));
        if (tokenBalance > 0) {
            // TODO: need to adjust `token0AmountOutMin` ?
            uint allowance = USDT.allowance(address(this), address(uniswapV2Router));
            if (allowance < tokenBalance) {
                USDT.safeApprove(address(uniswapV2Router), UNLIMITED_APPROVAL_AMOUNT);
            }
    ...
    function finalize() public onlyOwner {
        require(!finalized, "IDO: FINALIZED");

        bool status = isSaleFinalized();
        require(status == true, "Sale in progress");

        uint256 balanceOfToken = IERC20(address(token)).balanceOf(address(this));
        uint256 _amountToken = balanceOfToken > initialSupply ? initialSupply : balanceOfToken;
        ...
        IERC20(address(token)).safeApprove(address(uniswapV2Router), _amountToken);
        IUniswapV2Router01(uniswapV2Router).addLiquidityETH{value: _amountETH}(
            address(token),
            _amountToken,
            0,
            0,
        ...
        unlockTimestamp = block.timestamp + lockedPeriod;
        
        finalized = true;
        emit Finalized(_amountToken, _amountETH);
    }
