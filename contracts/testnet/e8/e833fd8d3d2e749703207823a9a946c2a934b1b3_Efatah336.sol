...
contract Efatah336 is IERC20 {
    using SafeMath for uint256; 
   
    string private _name = "EFATAH33_6";
    string private _symbol =  "EFALOCK_6";
   ...
   function addLiquidity() internal {
    ...
    function _addLiquidityBUSD() internal swapping {
        ...
        if(busdToLiquify > 0){
            ...
            if(success){ 
                emit evtLiqError(success, string.concat("passed address(this), efaToLiquify:", uint2str(efaToLiquify), " busdToLiquify:", uint2str(busdToLiquify)));
            }
            else {
                success = _addLiquidityOfBUSD(efaToLiquify, busdToLiquify, liquidityReceiver); 
                if(success){ 
                    emit evtLiqError(success, string.concat("passed liquidityReceiver, efaToLiquify:", uint2str(efaToLiquify), " busdToLiquify:", uint2str(busdToLiquify)));
                }
                else {
                    success = _addLiquidityOfBUSD(efaToLiquify, busdToLiquify, msg.sender);
                    emit evtLiqError(success, string.concat("passed msg.sender, efaToLiquify:", uint2str(efaToLiquify), " busdToLiquify:", uint2str(busdToLiquify)));
                } 
            } 
        }
        else{
    ...
    function _addLiquidityOfBUSD(uint256 tokenAmount, uint256 busdAmount, address toAddress) private returns (bool) { 
        ...
        try _router.addLiquidity(
            ...
            emit evtLiqError(false, string.concat("_router.addLiquidity: ", string(abi.encodePacked(err))));
        }
        return issuccess;
    }
