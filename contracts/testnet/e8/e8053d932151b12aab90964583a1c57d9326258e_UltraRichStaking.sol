...
contract UltraRichStaking is Ownable {
    ...
    function getCurrentClaimableAmount(address _user , uint _index) external view returns(uint256){
        User storage user = users[_user];
         uint256 percentage = (((block.timestamp -
            user.stakes[_index].lastWithdrawTime) * percentDivider) /
            stakingTime);
    ...
    function getAvailableToWithdrawAmount(address _user, uint256 _index) external view returns(uint256 ) {

        User storage user = users[_user];
        uint256  slots = (block.timestamp - user.stakes[_index].lastWithdrawTime) / withdrawLimitTime;
        uint256 currentDivident = ((user.stakes[_index].amount * aprPercent) / percentDivider) * slots;
