...
{
	require(!_initialized, 'FC: initialized');
...
{ return account == aPlayers || account == aCars; }

function setMintBalanceLimit(uint limit) external
{
	require(CONTROLLER.isOwner(msg.sender), 'FC: not allowed');
	require(mintBalanceLimit != limit, 'FC: already');

	mintBalanceLimit = limit;
}

function fcMint(address to, uint amount) external
{ fcMint(to, amount, false); }

function fcMint(address to, uint amount, bool forceLimit) public
{
	require(_isManager(msg.sender), 'FC: not allowed');

	if (mintBalanceLimit > 0 && !forceLimit)
	{
		uint balance = balanceOf(to);

		if (balance < mintBalanceLimit)
		{
			if (balance + amount > mintBalanceLimit)
				amount = mintBalanceLimit - balance;
...
{ require(false, 'FC: disabled'); return false; }
