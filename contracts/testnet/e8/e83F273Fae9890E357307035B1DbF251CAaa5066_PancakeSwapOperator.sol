...
import "./IPancakeSwapOperator.sol";
import "./PancakeSwapStorage.sol";
import "../../libraries/ExchangeHelpers.sol";

/// @title The PancakeSwap protocol operator to execute swap with the aggregator
contract PancakeSwapOperator is Context, IPancakeSwapOperator {
    PancakeSwapStorage public immutable operatorStorage;

    /// @dev Deploy with the storage contract
    constructor(address _router) {
        operatorStorage = new PancakeSwapStorage();
        PancakeSwapStorage(operatorStorage).updateRouter(_router);
        PancakeSwapStorage(operatorStorage).transferOwnership(msg.sender);
    }

    /// @inheritdoc IPancakeSwapOperator
    ...
    {
        console.log(
            "PancakeSwap perform swap from %s to %s",
            address(sellToken),
            address(buyToken)
        );
        ...
        console.log("Buy balance before purchase %s", buyBalanceBeforePurchase);
        console.log(
            "Sell balance before purchase %s",
            sellBalanceBeforePurchase
        );

        bool success = ExchangeHelpers.fillQuote(
            sellToken,
            operatorStorage.router(),
            swapCallData
        );
        require(success, "PSO: SWAP_FAILED");

        uint256 amountBought = buyToken.balanceOf(address(this)) -
            buyBalanceBeforePurchase;
        uint256 amountSold = sellBalanceBeforePurchase -
        ...
        console.log("PancakeSwap performed swap");
        console.log("Bought: %s (%s)", amountBought, address(buyToken));
        console.log("Sell: %s (%s)", amountSold, address(sellToken));
...
interface IPancakeSwapOperator {
    /// @notice Execute a swap via PancakeSwap
    /// @param sellToken The token sold
    /// @param buyToken The token bought
    /// @param swapCallData Pancake calldata from the API
...
/// @title PancakeSwapOperator storage contract
contract PancakeSwapStorage is Ownable {
    address public router;

    /// @notice Update the address of PancakeSwap router
...
library ExchangeHelpers {
    ...
    function fillQuote(
        ...
        console.log(
            "ExchangeHelpers fill quote for token %s with swap target %s",
            address(_sellToken),
            _swapTarget
        );
        ...
        console.log("%s call", _swapTarget);
    ...
    function setMaxAllowance(IERC20 _token, address _spender) internal {
        ...
        console.log("Set max allowance. Current: %s", _currentAllowance);
