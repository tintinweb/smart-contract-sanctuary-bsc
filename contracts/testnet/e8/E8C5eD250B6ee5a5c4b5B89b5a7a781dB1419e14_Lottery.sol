...
contract Lottery is LotteryOwnable, Initializable {
    ...
    function getNumbersTicket(uint256 _lotteryId) public view returns(uint8[4] memory){
    ...
    function drawing(uint256 _nextTime) external onlyAdmin {
        ...
        winningNumbers[0]= 2;
        //random(1 + block.timestamp);

        
        winningNumbers[1]= 3;
        //random(2 + block.timestamp);

        
        winningNumbers[2]= 4;
        //random(3 + block.timestamp);

        
        winningNumbers[3] = 5;
        //random(4 + block.timestamp);
    ...
    function internalBuy(uint256 _price, uint8[4] memory _numbers) internal {
        ...
        for (uint i = 0; i < 4; i++) {
            ...
            // lotteryBuyInfo[lotteryId].numbers[i] = _numbers[i];
    ...
    function  multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        for (uint i = 0; i < _numbers.length; i++) {
            for (uint j = 0; j < 4; j++) {
                ...
                // lotteryBuyInfo[lotteryId].numbers[i] = _numbers[i][j];
            }
