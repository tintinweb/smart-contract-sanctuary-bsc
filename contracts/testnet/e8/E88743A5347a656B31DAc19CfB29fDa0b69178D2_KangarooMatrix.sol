...
contract KangarooMatrix {
    ...
    address public immutable distrContrAddress;
    address public levelsAdmin;
    bool public openSalePeriod = false;

    KangarooStake public immutable stakePool;
    IBEP20 public matrixesToken;
    IBEP20 public immutable openSaleToken;
    ...
    uint256 public immutable periodDuration; //86400 * 30 = 2592000 seconds
    ...
    constructor(
        ...
        openSaleToken = _stakePool.rooToken();
        //IBEP20(_openSaleToken);
        matrixesToken = _stakePool.usdtToken();
        //IBEP20(_matrixesToken);
    ...
    function setRefBlocked(uint32 _userID, bool _status) external onlyLevelsAdmin {
        require(_userID > 0 && isUserExists(users[_userID].userAddress), "user not exists");
    ...
    function _updateMatrixesTokenRate() internal {
        if (openSalePeriod) {
            ...
            (uint112 reserveIn, uint112 reserveOut) = pancakePair.token0() == address(openSaleToken)
            ? (reserves0, reserves1)
            : (reserves1, reserves0);
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external {
        ...
        require(
            userID > 0 && users[userID].activeX3Levels >= 1 && users[userID].activeX6Levels >= 1,
        ...
        require(
            matrixesToken.allowance(address(msg.sender), address(this)) >= regPayAmount,
            "Increase the allowance first,call the approve method"
        );

        matrixesToken.safeTransferFrom(address(msg.sender), address(this), regPayAmount);

        if (matrix == 1) {
            require(
                users[userID].activeX3Levels == (level - 1) && users[userID].activeX6Levels == (level - 1),
                "wrong next X3 level"
            );
            ...
            require(
                users[userID].activeX3Levels == level && users[userID].activeX6Levels == (level - 1),
                "wrong next X6 level"
            );
    ...
    function registrationX3(uint32 referrerID) external {
        ...
        require(users[referrerID].activeX6Levels > 0, "referrer without X6 registration");
        ...
        require(
            matrixesToken.allowance(address(userAddress), address(this)) >= regPayAmount,
            "Increase the allowance first,call the approve method"
        );

        matrixesToken.safeTransferFrom(userAddress, address(this), regPayAmount);
    ...
    function updateX3Referrer(
        ...
        if (users[referrerID].x3Matrix[level].referralsID.length < 3) {
            emit NewUserPlace(
                ...
                1,
                level,
                uint8(users[referrerID].x3Matrix[level].referralsID.length)
            );
    ...
    function updateX6Referrer(
        ...
        if (users[referrerID].x6Matrix[level].firstLevelReferralsID.length < 2) {
            ...
            emit NewUserPlace(
                ...
                2,
                level,
                uint8(users[referrerID].x6Matrix[level].firstLevelReferralsID.length)
            );
            ...
            if (referrerID > 1) {
                ...
                if (
                    ...
                    (len == 1 || len == 2) && users[ref].x6Matrix[level].firstLevelReferralsID[0] == referrerID
                ) {
                    if (users[referrerID].x6Matrix[level].firstLevelReferralsID.length == 1) {
        ...
        if (users[referrerID].x6Matrix[level].closedPart != 0) {
            if (
                users[referrerID].x6Matrix[level].firstLevelReferralsID[0] ==
                users[referrerID].x6Matrix[level].closedPart
            ) {
        ...
        if (
            users[users[referrerID].x6Matrix[level].firstLevelReferralsID[0]]
            .x6Matrix[level]
            .firstLevelReferralsID
            .length <=
            users[users[referrerID].x6Matrix[level].firstLevelReferralsID[1]]
            .x6Matrix[level]
            .firstLevelReferralsID
            .length
        ) {
    ...
    function updateX6(
        ...
        if (!x2) {
            ...
            //set current level
            users[userID].x6Matrix[level].currentReferrerID = users[referrerID].x6Matrix[level].firstLevelReferralsID[
            0
            ];
        } else {
            users[users[referrerID].x6Matrix[level].firstLevelReferralsID[1]]
            ...
            emit NewUserPlace(
                userID,
                users[referrerID].x6Matrix[level].firstLevelReferralsID[1],
                2,
                level,
                uint8(
                    users[users[referrerID].x6Matrix[level].firstLevelReferralsID[1]]
    ...
    function updateX6ReferrerSecondLevel(
        ...
        uint32[] memory x6 = users[users[referrerID].x6Matrix[level].currentReferrerID]
        .x6Matrix[level]
        .firstLevelReferralsID;

        if (x6.length == 2) {
            if (x6[0] == referrerID || x6[1] == referrerID) {
    ...
    function usersX3Matrix(uint32 userID, uint8 level)
    external
    view
    returns (
        uint32,
    ...
    )
    {
        ...
        return (
        users[userID].x3Matrix[level].currentReferrerID,
        users[userID].x3Matrix[level].referralsID,
        users[userID].x3Matrix[level].blocked,
        users[userID].x3Matrix[level].reinvestCount
        );
    }

    function usersX6Matrix(uint32 userID, uint8 level)
    external
    view
    returns (
        uint32,
    ...
    )
    {
        require(userID < lastUserId && level <= LAST_LEVEL, "wrong arguments");
        return (
        users[userID].x6Matrix[level].currentReferrerID,
    ...
    function sendDividends(
        ...
        matrixesToken.safeTransfer(users[receiverID].userAddress, amount80);

        matrixesToken.safeTransfer(distrContrAddress, amount20);
...
contract KangarooStake is Ownable {
    ...
    IBEP20 public immutable lpKangarooToken;
    IBEP20 public immutable rooToken;
    IBEP20 public immutable usdtToken;
    address public immutable poolInitiator;
    ...
    function firstStaking(address _user, uint256 _amount) external {
        ...
        usdtToken.safeTransferFrom(_user, address(this), _amount);

        uint256 token0amount = usdtToken.balanceOf(address(this)).div(2);

        usdtToken.safeIncreaseAllowance(pancakeRouter, token0amount);

        uint256[] memory amounts = IPancakeRouter02(pancakeRouter).swapExactTokensForTokens(
            token0amount,
            0,
            tokenPath,
            address(this),
        ...
        uint256 token1Amt = amounts[amounts.length - 1];
        //rooToken.balanceOf(address(this));

        usdtToken.safeIncreaseAllowance(pancakeRouter, token0Amt);
        rooToken.safeIncreaseAllowance(pancakeRouter, token1Amt);
    ...
    function chargePool(uint256 amount) external returns (bool) {
        ...
        rooToken.safeTransferFrom(address(msg.sender), address(this), amount);
        ...
        pool[2].currentRewardPerShare = pool[2]
        ...
        pool[1].currentRewardPerShare = pool[1]
        .currentRewardPerShare
        .add(chargedAmount30.mul(1e12).div(pool[1].sharesTotal))
        .add(chargedAmount50.mul(1e12).div(pool[0].sharesTotal));

        pool[0].currentRewardPerShare = pool[0].currentRewardPerShare.add(
            chargedAmount50.mul(1e12).div(pool[0].sharesTotal)
        );
    ...
    function dividendsTransfer(
        ...
        pool[_id].usersInfo[_to].initialDepositAmount = pool[_id]
        .usersInfo[_to]
        .sharesAmount
    ...
    function stake(uint256 _id, uint256 _amount) external notForPoolInitiator poolExist(_id) {
        ...
        require(
            lpKangarooToken.allowance(address(msg.sender), address(this)) >= _amount,
            "Increase the allowance first,call the approve method"
        );
        ...
        lpKangarooToken.safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function emergencyWithdraw(uint256 _id) external notForPoolInitiator poolExist(_id) {
        ...
        require(unstaked_shares > 0, "you do not have staked tokens, stake first");
    ...
    function unstake(uint256 _id, uint256 _amount) external notForPoolInitiator poolExist(_id) {
        ...
        require(_amount > 0 && _amount <= user.sharesAmount, "bad _amount");
        require(isTokensFrozen(_id, msg.sender) == false, "tokens are frozen");
    ...
    function getDividends(uint256 _id) external poolExist(_id) {
        require(pool[_id].usersInfo[msg.sender].sharesAmount > 0, "you do not have staked tokens, stake first");
        uint256 dividends = calculateDividends(_id, msg.sender);
        if (dividends > 0) {
            dividendsTransfer(_id, msg.sender, dividends);
        }
    ...
    function calculateDividends(uint256 _id, address userAddress) public view returns (uint256) {
        return
        pool[_id].usersInfo[userAddress].sharesAmount.mul(pool[_id].currentRewardPerShare).div(1e12).sub(
            pool[_id].usersInfo[userAddress].initialDepositAmount
        );
    }
    ...
    function getPoolSharesTotal(uint256 _id) external view returns (uint256) {
        return pool[_id].sharesTotal;
    }

    function getUser(uint256 _id, address userAddress) external view returns (UserInfo memory) {
...
library TransferHelper {
    ...
    function safeIncreaseAllowance(
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), "INCREASE_ALLOWANCE_FAILED");
    }
}
