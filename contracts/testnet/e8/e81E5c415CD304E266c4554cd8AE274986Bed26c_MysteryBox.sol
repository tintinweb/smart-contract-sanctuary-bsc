...
contract MysteryBox is Ownable{
    ...
    // 系统配置
     function setting(uint256 _minAmount, uint256 _maxAmount, uint32 _NxxExchangeRate, uint256 _profitRatio, uint256 _bonusPool) public onlyOwner{
    ...
    function game(uint32 boxID, uint256 _amount) public returns (uint ) {
        ...
        // 初始化用户数据
        users[msg.sender].push(Player(_amount, _amount, _randomDay, block.timestamp, boxID));

        // 操作调用者合约地址划转到当前合约地址中
        IERC20(nxxToken).transferFrom(msg.sender, address(this), _amount);

        // 实际资金池的可用金额
        uint256 reality_amount = _amount * bonusPool / 100;

        // 增加到对应的资金池中
        boxMap[boxID].capitalPool += _amount - reality_amount;
