...
contract Vault is ReentrancyGuard {
    ...
    address public masterChef; // address of farm to stake (MasterChef)
    uint256 public pid; // pid of pool in farmContractAddress
    address public wantAddress; //tokens we want to stake & compound

    address public masterCompounder;
    ...
    function init(address _newMChef, uint256 _newPID, address _newMCompounder) external onlyAllowGov {
      require(!initalized, "already initialized");

      masterChef = _newMChef;
      pid = _newPID;
      masterCompounder = _newMCompounder;
    ...
    modifier onlyMasterCompounder() {
        require(msg.sender == masterCompounder, "!masterCompounder");
        _;
    }
        ...
        returns (uint256)
    {
        ...
        _wantAmt = IERC20(wantAddress).balanceOf(address(this)) - beforeAmount;
    ...
    function _farm() internal virtual {
        ...
        IERC20(wantAddress).safeIncreaseAllowance(masterChef, wantAmt);

        IMasterChef(masterChef).deposit(pid, wantAmt);
    }
    ...
    function _unfarm(uint256 _wantAmt) internal virtual {
        if (_wantAmt == 0){
          ...
          IMasterChef(masterChef).withdraw(pid, _wantAmt);
        }
    }
        ...
        returns (uint256)
    {
        ...
        IERC20(wantAddress).safeTransfer(masterCompounder, _wantAmt);
    ...
    function inCaseTokensGetStuck(
        ...
        require(_token != address(wantAddress), "!safe");
