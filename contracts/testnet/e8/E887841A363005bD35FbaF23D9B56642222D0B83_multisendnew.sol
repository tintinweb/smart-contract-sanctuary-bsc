...
contract multisendnew {
    ...
    function multiSendDiffToken(IERC20 token, address payable[] memory recipients, uint256[] memory amounts) public payable{
    ...
    function disperseToken(IERC20 token, address[] memory recipients, uint256[] memory values) public {
        uint256 total = 0;
        for (uint256 i = 0; i < recipients.length; i++)
            total += values[i];
