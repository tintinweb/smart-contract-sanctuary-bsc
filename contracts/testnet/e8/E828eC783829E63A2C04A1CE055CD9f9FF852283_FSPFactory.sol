...
interface IRematic is IERC20 {
    ...
    function transferTokenFromPool(address from, address to, uint value) external returns (bool);
...
contract FSPPool is Ownable, ReentrancyGuard {
    ...
    // contract address of RFTX token
    ...
    function deposit(uint256 _amount) external payable nonReentrant isPoolActive {
        ...
        if (_amount > 0) {
            ...
            if(address(stakedToken) == RFTX_ADDRESS){
    ...
    function withdrawAll() external payable nonReentrant {
        ...
        if (isReflectionToken && address(stakedToken) != RFTX_ADDRESS) {
...
contract FSPFactory is Ownable {
    ...
    function deployPool(
        ...
        if(address(_stakedToken) == RFTX_ADDRESS){
    ...
    function withdrawToken(
        ...
        if(_tokenContract == RFTX_ADDRESS){
