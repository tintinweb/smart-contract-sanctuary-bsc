...
contract MyNameIsZkToken is ERC20, Bot {
    ...
    uint256 public _buyBackFee = 4;  //回购
    uint256 private _previousBuyBackFee = _buyBackFee;
    address payable public buyBackAddress = payable(0xBda5fc5A5f6C24a73dA53c54107213A82c3d3099); // Liquidity Address
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        uint256 totalTokensToSwap = _buyBackFee + _marketingFee;

        if (
            overMinTokenBalance &&
            !inSwapAndLiquidity &&
            totalTokensToSwap > 0 &&
            from != uniswapV2Pair && //BUY:if(from == pair) SELL:if(to==pair)
    ...
    function swapUsdAndSendToken(uint256 bnbBalance) private {
        ...
        usdToken.transfer(buyBackAddress, tokenReceived);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (sender != uniswapV2Pair) { //BUY:if(from == pair) SELL:if(to==pair) is SELL
            burnAmt = 0;
        }
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount); //(amount.sub(burnAmt).sub(charityAmt))
    ...
    function setConfigFeeInfo(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 buyBackFee, uint256 burnFee, uint256 charityFee) external onlyOwner() {
        _taxFee = taxFee;
    ...
    function setNumTokenPercent(uint256 minimumToken) external onlyOwner() {
