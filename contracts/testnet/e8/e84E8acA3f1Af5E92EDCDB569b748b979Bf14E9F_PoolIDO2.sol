...
contract PoolIDO2 is Ownable, ReentrancyGuard {
...
//   }
  ...
  function urgentWithdrawAllToken() public onlyOwner {
    uint256 stakeTokenInContract = claimToken.balanceOf(address(this));
    if (stakeTokenInContract > 0) {
      claimToken.safeTransfer(idoOwner, stakeTokenInContract);
    }

    uint256 stakeToken1InContract = stakeToken.balanceOf(address(this));
    if (stakeToken1InContract > 0) {
      stakeToken.safeTransfer(idoOwner, stakeToken1InContract);
    }
  }
