...
contract TDOTC is Configurable {
...
/*  bytes32 internal constant _rebaseTime_  = "rebaseTime";
    ...
    mapping(bytes32 =>uint) public  fiatFeeMap; //currency=>feeRate
	...
	function __DOTC_init_reward2() public governance {
      ...
      //  _setConfig(_assetFreeLimit_, 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d, 1e18);    // USDC
        ...
        config[_rewardToken_] = uint(0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0);      // pear
   ...
   /* function migrate(address vault_) external governance {
    ...
    function setCustomFiatFee(bytes32[] memory fiatList,uint[] memory feeList) external governance {
        customFiatFeeKeys = fiatList;

        for(uint i=0; i<fiatList.length; i++){
            fiatFeeMap[fiatList[i]]=feeList[i];
        }
    }
    function isCustomFiat(bytes32 currency) public returns(bool include){
        for(uint i=0; i<customFiatFeeKeys.length; i++){
    ...
    function deliver(uint takeID) virtual external nonReentrant returns(uint vol) {
        ...
        vol = takes[takeID].vol;
        // if(isFreeCurrency(makes[makeID].currency)){
        //      IERC20(makes[makeID].asset).safeTransfer(buyer, vol);
        // }else{
            uint256 fee = _payFee(takeID, makes[makeID].asset, vol,makes[makeID].currency);
...
//    event Arbitrate(uint indexed takeID, uint indexed makeID, address indexed arbiter, uint vol, Status status);
    ...
    function payFee(uint takeID, address asset, uint vol,uint makeID) public returns(uint fee) {
    ...
    function _payFee(uint takeID, address asset, uint vol,bytes32 currency) internal returns(uint fee) {
        if(isCustomFiat(currency)){
             fee = vol.mul(fiatFeeMap[currency]).div(1e6);
            emit PayFee(currency,fiatFeeMap[currency],1);
        }else{
             fee = vol.mul(config[_feeRate_]).div(1e18);
             emit PayFee(currency,config[_feeRate_],2);
        }
    ...
    function arbitrate(mapping (uint => SMake) storage makes,mapping(uint =>SMakeEx) storage makeExs,mapping (uint => STake) storage takes,mapping (bytes32 => uint) storage config,mapping(uint =>AppealInfo) storage ais, mapping (address => uint) storage biddingN,ArbiterPara memory ap/*uint takeID, Status winner,Status assetTo,Status appealFeeTo,Status punishSide,uint punishVol*/) virtual external /*nonReentrant*/ returns(uint vol) {
        ...
        if (take.status == Status.Appeal){
            ...
            if( ap.winner == Status.Buyer) {
                uint fee = dotc.payFee(takeID, make.asset, vol,makeID);
