...
// File: Factory/NeedsTesting/LPFactory.sol
...
// File: Contracts/Factory/LPToken-Factory-TimedRelease.sol
...
contract FarmageddonPair is FarmageddonERC20, Ownable {
    ...
    function transfer(address to, uint value) external returns (bool) {
        ...
        if (FarmageddonFactory(factory).isMasterChef(to)) {
            ...
            try dividendTracker.setBalance(payable(msg.sender), balanceOf[msg.sender] + stakedAmount[msg.sender]) {} catch {}
    ...
    function updateGasForProcessing(uint256 newValue) public {
        require(FarmageddonFactory(factory)._isOperator(msg.sender), "must be an operator");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function sendDividends(address user, uint256 amountToFeeTo1, uint256 amountToUser) external returns (bool) {
        ...
        // address to2 = FarmageddonFactory(factory).feeTo2();
        
        IERC20(payToken).transfer(to1, amountToFeeTo1);
        // IERC20(payToken).transfer(to2, amountToFeeTo2);
...
contract FarmageddonFactory is Ownable {
    ...
    uint256 public minPercentOfSupplyForDiv = 10; // 100 = 1 %

    mapping (address => bool) public _isListed;
    address[] public payOutTokens;

    constructor(address _DividendFactory) {
        _IDFactory = _DividendFactory;
        setFeeTo1(0x5CdDC17F39222B6C9ED3D27E09C297DaA55EB17E);
        addOperator(0x5CdDC17F39222B6C9ED3D27E09C297DaA55EB17E);
        addMasterChef(0x467B2A6C7D9100509F144D257D62D629d39f7C28);  // random for testing
    }

    function changeDividendFactory (address DividendFactory) external {
        require(_isOperator[msg.sender] || msg.sender == owner(), "NO ACCESS");
        _IDFactory = DividendFactory;
    }
   
    function addPayoutToken(address newPayoutToken) external {
        require(_isOperator[msg.sender] || msg.sender == owner(), "NO ACCESS");
    ...
    function removePayOutToken(address _token) external {
        require(_isOperator[msg.sender] || msg.sender == owner(), "NO ACCESS");
    ...
    function addOperator(address _operator) public {
        require(_isOperator[msg.sender] || msg.sender == owner(), "NO ACCESS");
        require(!_isOperator[_operator], "already an operator");
         _isOperator[_operator] = true;
    }
    function removeOperator(address _operator) public {
        require(_isOperator[msg.sender] || msg.sender == owner(), "NO ACCESS");
        require(_isOperator[_operator],"not an operator");
        _isOperator[_operator] = false;
    }

    function setFeeTo1(address _feeTo1) public {
        require(_isOperator[msg.sender] || msg.sender == owner(), "NO ACCESS");
    ...
    function addMasterChef(address _mc) public {
        require(_isOperator[msg.sender] || msg.sender == owner(), "NO ACCESS");
        require(!isMasterChef[_mc], "already added");
         isMasterChef[_mc] = true;
    }
    function removeMasterChef(address _mc) public {
        require(_isOperator[msg.sender] || msg.sender == owner(), "NO ACCESS");
        require(isMasterChef[_mc],"not listed");
        isMasterChef[_mc] = false;
    }

    function setFactoryFee(uint256 _FactoryFee) external {
        require(_isOperator[msg.sender] || msg.sender == owner(), "NO ACCESS");
        require(_FactoryFee > 15 && _FactoryFee < 500 ," 15 < Fee > 500");
        FactoryFee = _FactoryFee;
    }
    function setMinimimPercentOfsuppyForDividends(uint256 percentTimesHundred) external {
        require(_isOperator[msg.sender] || msg.sender == owner(), "NO ACCESS");
