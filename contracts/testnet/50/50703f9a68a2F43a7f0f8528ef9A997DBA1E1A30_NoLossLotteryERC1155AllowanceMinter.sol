...
import "../nft/IERC1155AllowanceMintController.sol";

/**
 * No loss lottery for awarding ERC1155 tokens from an ERC1155AllowanceMintController.
 */
contract NoLossLotteryERC1155AllowanceMinter is NoLossLotteryERC1155 {
    IERC1155AllowanceMintController private _mintController;

    event AwardsSet(address awardToken, uint256 awardTokenId, uint256 awardsPerTicket, address mintController);
    ...
    function initialize(
        ...
        __NoLossLotteryERC1155_init(
            ...
            tieredTickets_, tieredAwards_, paymentTarget_, rng_, owner_
        );
        
        _mintController = IERC1155AllowanceMintController(address(0));
    }

    function mintController() public view returns (IERC1155AllowanceMintController) {
    ...
    function setAwards(
        ...
        IERC1155AllowanceMintController mintController_
    ) external onlyOwner {
        _setAwards(awardToken_, awardTokenId_, awardsPerTicket_);

        require(address(mintController_) != address(0), "NoLossLotteryERC1155AllowanceMinter: Mint controller cannot be zero address");
        _mintController = mintController_;

        emit AwardsSet(address(awardToken_), awardTokenId_, awardsPerTicket_, address(mintController_));
    }
    ...
    function _award(address player, uint256 ticketCount) internal virtual override {
        ...
        _mintController.mint(player, _awardTokenId, awardAmount, "");
    }
...
import "./NoLossLotteryLogicHooks.sol";
 ...
 */
abstract contract NoLossLotteryLogic is UUPSUpgradeable, LotteryConfig, LotteryStorage, NoLossLotteryLogicHooks, PausableUpgradeable, KeeperCompatibleViewInterface {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    function __NoLossLotteryLogic_init(
        ...
        __LotteryConfig_init(
            name_, endTime_, paymentToken_, pricePerTicket_, maxTicketsPerPlayer_,
            tieredTickets_, tieredAwards_, paymentTarget_, rng_, owner_
        );
        __LotteryStorage_init();
        __Pausable_init();
    ...
    function _setState(LotteryState newState) private {
        _state = newState;
    ...
    function transitState() public whenNotPaused {
        ...
        if (_state == LotteryState.CREATED) {
            ...
            _randomNumber = randomNumberGenerator().randomNumbers(_randomNumberRequestId)[0];
    ...
    function shouldRemoveUpkeep() external view override returns (bool) {
     ...
     */
    function winningTicketCountOf(address player) public view returns (uint256) {
...
abstract contract LotteryConfig is TwoStageOwnableUpgradeable {
    ...
    function __LotteryConfig_init_unchained(
        ...
        require(endTime_ > block.timestamp, "LotteryConfig: End time must be later than now");
        require(address(paymentToken_) != address(0), "LotteryConfig: Payment token is needed");
        require(pricePerTicket_ > 0, "LotteryConfig: Price per ticket must not be zero");
        require(paymentTarget_ != address(0), "LotteryConfig: Payment target is needed");
        require(rng_ != address(0), "LotteryConfig: Random number generator must not be zero address");
        require(tieredTickets_.length > 0, "LotteryConfig: Must have at least one tier");
        require(tieredTickets_.length == tieredAwards_.length, "LotteryConfig: Ticket and award tiers must match");
        require(tieredTickets_[0] == 0, "LotteryConfig: First tier tickets must be zero");

        for (uint256 i=1; i<tieredTickets_.length; i++) {
            require(tieredTickets_[i] > tieredTickets_[i-1], "LotteryConfig: Incorrect tiered tickets config");
            require(tieredAwards_[i] > tieredAwards_[i-1], "LotteryConfig: Incorrect tiered awards config");
        }
    ...
    function setPaymentTarget(address target) public onlyOwner {
        require(target != address(0), "LotteryConfig: Payment target must not be zero address");
...
/**
 * Logic contract hooks to keep file clean.
 *
 * @dev Must implement _awardsInitialized(), _secureAwards(), _awardsAvailable(), and _award().
 */
abstract contract NoLossLotteryLogicHooks {
...
interface KeeperCompatibleViewInterface {
  ...
  function shouldRemoveUpkeep() external view returns (bool);
}
