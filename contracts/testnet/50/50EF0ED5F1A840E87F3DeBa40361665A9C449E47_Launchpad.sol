...
contract BatchAuction is Ownable, ReentrancyGuard {
  ...
  event AllowlistSignerChanged(address indexed _addr);
   ...
   */
  function claimInstantToken() external isAuctionCompleted isClaimablePeriod {
   ...
   */
  function claimVestedLPToken() external isAuctionCompleted canClaimVestedLPToken(msg.sender) {
  ...
  function setAllowlistSigner(address _signer) external onlyOwner {
    ...
    emit AllowlistSignerChanged(_signer);
  }
  ...
  function getActualVestedLPTokenAmount(address _user)
    public view isAuctionCompleted canClaimVestedLPToken(_user) returns (uint256) {
...
contract Launchpad is Ownable {
   ...
   */
  function changeOperator(address _address) external onlyOwner {
    OPERATOR = _address;
  ...
  function setAllowlistSigner(uint256 _pid, address _signer) external isInitialized(_pid) onlyOperator {
    PROJECTS[_pid].auction.setAllowlistSigner(_signer);
    emit AllowlistSignerChanged(_pid, _signer);
  }
  
  function cancelAuctionBeforeStart(uint256 _pid) external isInitialized(_pid) onlyOperator {
    PROJECTS[_pid].auction.cancelAuctionBeforeStart();
    emit ProjectCancelledBeforeStart(_pid);
  }

  function transferAuctionTokenFromVault(uint256 _pid) external isInitialized(_pid) onlyOperator {
    PROJECTS[_pid].auction.transferAuctionTokenFromVault();
    emit AuctionTokenTransferredFromVault(_pid);
  }

  function finalizeProject(uint256 _pid) external isInitialized(_pid) onlyOperator {
    PROJECTS[_pid].auction.finalizeAuction();
    emit ProjectFinalized(_pid);
  }

  function withdrawUnclaimedAuctionToken(uint256 _pid) external isInitialized(_pid) onlyOperator {
    PROJECTS[_pid].auction.withdrawUnclaimedAuctionToken();
    emit UnclaimedAuctionTokenWithdrawn(_pid);
  }

  function withdrawUnclaimedLPToken(uint256 _pid) external isInitialized(_pid) onlyOperator {
  ...
  function getAuctionData(uint256 _pid) external view isInitialized(_pid) returns (AuctionData memory) {
    return PROJECTS[_pid].auction.getAuctionData();
  }

  function getAuctionAddress(uint256 _pid) external view isInitialized(_pid) returns (address) {
    return address(PROJECTS[_pid].auction);
  }
  
  function getAuctionTokenBalance(uint256 _pid) external view isInitialized(_pid) returns (uint256) {
