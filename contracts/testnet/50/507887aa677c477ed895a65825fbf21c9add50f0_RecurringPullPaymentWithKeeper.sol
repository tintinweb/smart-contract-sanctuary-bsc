...
contract RecurringPullPaymentWithKeeper is
	...
	KeeperCompatibleInterface
{
	...
	// bm unique reference => bmId
	mapping(string => uint256) private _bmReferences;
	// subscription unique reference => bmId
	 ...
	 */
	function initialize(address _registryAddress, address _keeperRegistry)
		external
		virtual
		initializer
	{
		...
		require(_keeperRegistry != address(0), 'RecurringPullPayment: INVALID_KEEPER');
	 ...
	 * @param _reference 				 - unique reference for the subscription.
	 */
	...
	{
		uint256[] memory subscriptionList = new uint256[](_subscriptionIDs.current());
		uint256 count = 0;

		for (uint256 subID = 1; subID <= _subscriptionIDs.current(); subID++) {
			if (isPullPayment(subID)) subscriptionList[count] = subID;
			count++;
		}

		upkeepNeeded = count > 0;
		performData = abi.encode(subscriptionList);

		//TODO-need to check mstore
		if (count != _subscriptionIDs.current()) {
			assembly {
				mstore(subscriptionList, count)
			}
		}
	...
	function performUpkeep(bytes calldata performData) external override onlyKeeperRegistry {
		...
		for (uint256 i = 0; i < subscriptionIds.length; i++) {
			_executePullPayment(subscriptionIds[i]);
		}
	}
	...
	function setKeeperRegistryAddress(address keeperRegistryAddress) public onlyOwner {
		...
		emit KeeperRegistryAddressUpdated(KeeperRegistryAddress, keeperRegistryAddress);
	...
	function isPullPayment(uint256 _subscriponId) public view returns (bool isPP) {
		BillingModel storage bm = _billingModels[_subscriptionToBillingModel[_subscriponId]];
		Subscription storage subscription = bm.subscriptions[_subscriponId];
		if (
