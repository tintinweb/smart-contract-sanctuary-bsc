...
contract Lottery {
    ...
    function lottery() public onlyManagerCanCall {
        ...
        for( uint i = 0; i < bettors.length; i++ ){
            uint256 index = random(bettors.length - 1 - count);
            if (count <= config.first.num) {
                firstWinners.push(bettors[index]);
            } else if (count <= config.first.num + config.second.num) {
                secondWinners.push(bettors[index]);
            } else if (count <= config.first.num + config.second.num + config.third.num) {
                thirdWinners.push(bettors[index]);
            }
            bettors[index] = bettors[bettors.length - 1 - count];
            count++;
        }
        ...
        if (round == 1) {
            ...
            jackpot = config.current.stake + config.total.stake * 20 / 100;
            config.total.stake = config.total.stake - config.total.stake * 20 / 100;
        }
