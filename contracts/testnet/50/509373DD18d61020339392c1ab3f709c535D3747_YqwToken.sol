...
contract YqwToken is Context, IERC20, Ownable {
    ...
    string private _name = "Yqw Finance Community Token";
    string private _symbol = "Yqw";
    ...
    address public oldYqw = address(0x1676BDdf162A800cCb5f9e81b0F402A33c51e06c);
    ...
    function mappingInviter(address[] calldata _sender) public {
        ...
        for(uint i=0;i<_sender.length;i++){
            address account=IYqw(oldYqw).inviter(_sender[i]);
    ...
    function transferAddress(address[] calldata _newAddress) public {
        ...
        for(uint i=0;i<_newAddress.length;i++){
          uint256 value=IERC20(oldYqw).balanceOf(_newAddress[i]);
