...
interface IBufferOptions is IOptions {
    struct Option {
        State state;
        uint256 strike;
        uint256 amount;
...
interface IBufferRangeOptions is IOptions {
    struct ConvertibleOption {
        State state;
        uint256 upperBound;
        uint256 lowerBound;
        uint256 amount;
...
interface IRangeOptionsConfig is IOptionsConfig {
    event UpdateDiscountFactor(uint256 value);
    event UpdateFaceValue(uint256 value);
    event UpdateRange(uint256 lowerBound, uint256 upperBound);
    event Initialize(
...
interface ISlidingWindowOracleV1 {
    function consult(address tokenIn, address tokenOut)
        external
        view
    ...
    function getTimeWeightedAverage(
        ...
        address tokenB,
        uint256 roundId
    ) external view returns (uint256 roundTimestamp, uint256 price);

    function update(address tokenIn, address tokenOut) external view;
}
    ...
    function checkParams(
        ...
        require(
            optionType == IOptions.OptionType.Call ||
                optionType == IOptions.OptionType.Put,
            "Wrong option type"
        );
...
// File: BufferPhysicallySettledAmericanOptions.sol
 ...
 */
contract BufferPhysicallySettledAmericanOptions is
    IBufferOptions,
    Ownable,
    ReentrancyGuard,
    BufferNFTCore
{
    ...
    mapping(uint256 => string) private _tokenURIs;
    IPriceProvider public priceProvider; // Gives pool's tokenX price in stable coin
     ...
     */
    function create(uint256 amount, string memory metadata)
        external
        nonReentrant
    ...
    function exercise(uint256 optionID) external returns (uint256 profit) {
        ...
        require(msg.sender == tokenOwner, "O12");
        ...
        uint256 tokenXDecimals = 10**(ERC20(pool.tokenX()).decimals());
        uint256 amountToPay = (option.strike *
            option.amount *
            (10**stableCoin.decimals())) / (tokenXDecimals * 1e8);
        if (msg.sender != pool.projectOwner()) {
            // User has to approve first inorder to execute this function
            bool success = stableCoin.transferFrom(
                msg.sender,
            ...
            require(success, "O3");
            stableCoin.transfer(pool.projectOwner(), amountToPay);
        }
        pool.send(optionID, ownerOf(optionID), option.lockedAmount);
    ...
    /**
     * @notice Used for getting the actual options prices in pool's tokenX
     * @param period Option period in seconds (1 days <= period <= 4 weeks)
     * @param amount Option amount
     * @param strike Strike price of the option
