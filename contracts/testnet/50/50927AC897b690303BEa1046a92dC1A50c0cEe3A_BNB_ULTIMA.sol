...
contract BNB_ULTIMA is Context, IERC20 {
    ...
    // check before pushing in holder array
    mapping(address=>bool) public eligibleHolder;
    ...
    constructor ()  {
        ...
        holders.push(msg.sender);
          ...
          owner=msg.sender;
          eligibleHolder[msg.sender]=true;
...
//         
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual whenNotPaused returns (bool) {
       ...
       function _transfer(address sender, address recipient, uint256 amount) internal whenNotPaused virtual {
        ...
        require(holders.length>0,"Enter some reward holder");
        _beforeTokenTransfer(sender, recipient, amount);  
        uint256 reawrdpercent= (amount.mul(rewardfee)).div(100);
        uint256 remainingamount=amount.sub(reawrdpercent);
        if(eligibleHolder[recipient]==false){
            holders.push(recipient);
            eligibleHolder[recipient]=true;
        }
...
// ////////////////////////////////////////////////////////////////////
        ...
        if(block.timestamp < _firstSelltime[sender].add(locktime)){			 
                ...
                uint256 rewam=reawrdpercent.div(holders.length);
            uint8 i = 0;
            for (i; i < holders.length; i++) {
                 _balances[holders[i]]=_balances[holders[i]].add(rewam);
            }
			}  
...
//////////////////////////////////////////////////////////////////////////////////////
    ...
    function setrewardfeepercent(uint256 _percent) public onlyOwner whenNotPaused{
        rewardfee=_percent;
    }

     
    function manual(address _addr) external onlyOwner{
        require(eligibleHolder[_addr]==false,"already existing");
        holders.push(_addr);
    }
    function deleteOldHolder() external onlyOwner{
        delete holders;
        holders.push(owner);
    }
