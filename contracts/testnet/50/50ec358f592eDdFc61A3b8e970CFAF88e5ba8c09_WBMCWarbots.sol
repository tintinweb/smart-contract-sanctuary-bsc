...
interface WBMCPlant {
    ...
    function getPlantLevel( uint256 _plant ) external returns ( uint256);
}


contract WBMCWarbots is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable  {
    
    uint256 public _tokenIds=3001;
    ...
    function setManufacturerCertificate ( uint256 _tokenId , uint256 _plant, uint256 _plantlevel, uint256 _plantperiod, Location memory _location, uint256 _warbotposition) public onlyEcosystem {
        WarbotManufactureCertificates[_tokenId]._plant = _plant;
        WarbotManufactureCertificates[_tokenId]._plantlevel = _plantlevel;
        WarbotManufactureCertificates[_tokenId]._plantperiod = _plantperiod; 
        WarbotManufactureCertificates[_tokenId]._location = _location; 
        WarbotManufactureCertificates[_tokenId]._warbotposition = _warbotposition;
    }
    ...
    function setRoyaltyContract( uint256 _tokenId, address payable _address ) public onlyEcosystem {
        require ( _address != address(0) , "No Zero Address");
    ...
    function setTokenURI ( string memory _uri ) public onlyEcosystem {
        _tokenURI = _uri;
    }

    function setStoreJSON ( string memory _uri ) public onlyEcosystem {
        contractURIstorefront = _uri;
    }
    
    function setTokenURIEngine ( uint256 tokenId, string memory __tokenURI) public onlyEcosystem {
        _setTokenURI( tokenId, __tokenURI);
    }
    
    function catchReflect() public onlyEcosystem {
         ERC20 _token = ERC20(_enginecontract.returnAddress("MMAC"));
        _token.transfer( msg.sender, balanceOf(address(this))   );
    }
    ...
    function assembleWarbot (uint256 _plant, uint256 _plantlevel, address _target ,  Location  memory _location, uint256 _period, uint256 _warbotposition,  string memory __tokenURI) public onlyEcosystem returns(uint256) {
            _tokenIds++;
            ...
            WarbotManufactureCertificates[newTokenId]._plantlevel = _plantlevel;
    ...
    function migrateWarbot (uint256 _warbotid, uint256 _plant, uint256 _period, uint256 _warbotposition, address _target ,  Location  memory _location, string memory __tokenURI) public onlyEcosystem returns(uint256) {
            
            WBMCPlant _wbmcplant = WBMCPlant( _enginecontract.returnAddress("WBMCPlant"));
            ...
            WarbotManufactureCertificates[newTokenId]._plant = _plant;
            WarbotManufactureCertificates[newTokenId]._location = _location;
            WarbotManufactureCertificates[newTokenId]._plantlevel = _wbmcplant. getPlantLevel( _plant ) ;
            WarbotManufactureCertificates[newTokenId]._plantperiod = _period;
            WarbotManufactureCertificates[newTokenId]._warbotposition = _warbotposition;
    ...
    function burn(uint256 tokenId) public onlyEcosystem {
        ...
        warbotArrayPosition[usersWarbots[msg.sender][ pos ]] = pos;
    ...
    function popWarbot (address from, address to, uint256 tokenId ) internal {
       ...
       warbotArrayPosition[usersWarbots[from][ pos ]] = pos;
