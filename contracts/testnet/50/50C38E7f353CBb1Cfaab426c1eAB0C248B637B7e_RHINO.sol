...
contract RHINO is ERC20Detailed, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    event LogRebase(uint256 indexed epoch, uint256 totalSupply);

    string public _name = 'Rhinoinu';
    string public _symbol = 'RHINO';
    ...
    uint256 public RHINOInsuranceFundFee = 50;
    uint256 public firePitFee = 10;
    uint256 public totalFee = liquidityFee.add(gameTreasuryFee).add(RHINOInsuranceFundFee).add(firePitFee);
    ...
    address public RHINOInsuranceFundReceiver;
    ...
    constructor(
        ...
        RHINOInsuranceFundReceiver = msg.sender;
        infernoPit = ZERO;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_gameTreasuryFee.add(RHINOInsuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(gameTreasuryReceiver).call{
            value: amountETHTogameTreasuryAndSIF.mul(gameTreasuryFee).div(gameTreasuryFee.add(RHINOInsuranceFundFee)),
            gas: 30000
        }('');
        (success, ) = payable(RHINOInsuranceFundReceiver).call{
            value: amountETHTogameTreasuryAndSIF.mul(RHINOInsuranceFundFee).div(
                gameTreasuryFee.add(RHINOInsuranceFundFee)
            ),
            gas: 30000
        }('');
    }
    ...
    function setFeeReceivers(
        ...
        address _RHINOInsuranceFundReceiver,
        address _infernoPit
    ) external onlyOwner {
        gameTreasuryReceiver = _gameTreasuryReceiver;
        RHINOInsuranceFundReceiver = _RHINOInsuranceFundReceiver;
    ...
    function setFee(
        ...
        uint256 _RHINOInsuranceFundFee,
        uint256 _firePitFee,
        uint256 _sellFee
    ) external onlyOwner {
        liquidityFee = _liquidityFee;
        gameTreasuryFee = _gameTreasuryFee;
        RHINOInsuranceFundFee = _RHINOInsuranceFundFee;
        firePitFee = _firePitFee;
        totalFee = liquidityFee.add(gameTreasuryFee).add(RHINOInsuranceFundFee).add(firePitFee);
        sellFee = _sellFee;
