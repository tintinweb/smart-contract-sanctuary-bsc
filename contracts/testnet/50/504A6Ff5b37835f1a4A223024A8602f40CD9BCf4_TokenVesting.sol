...
contract TokenVesting is Ownable, ReentrancyGuard {
  ...
  constructor (IDigichainAdmin _digixAdmins) {
    DIGIX_ADMINS = _digixAdmins;
    FEES.tokenFee = 35;
    FEES.feeAddress = payable(0x14f1d0b47ABB50FB9D5c49B3cE5b2c318d14c621);
  ...
  // Callable by DIGIX_ADMINS or whitelisted contracts (such as presale contracts)
  function editZeroFeeWhitelist (address _token, bool _add) external {
    require(DIGIX_ADMINS.userIsAdmin(msg.sender) || TOKEN_WHITELISTERS.contains(msg.sender), 'ADMIN');
    if (_add) {
      ZERO_FEE_WHITELIST.add(_token);
    } else {
