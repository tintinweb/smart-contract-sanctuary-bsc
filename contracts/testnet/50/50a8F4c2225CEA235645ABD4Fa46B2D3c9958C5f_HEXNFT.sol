...
contract HEXNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function buyNFT(buyNFTData memory _buyData) external payable {
        ...
        if(_buyData.tokenId == 0) {
            ...
            uint256 tokenId = mintNFT(mintData);
            createNftData memory createData = createNftData(
                _buyData.nftId,
                tokenId,
                payable(_buyData.owner)
        ...
        if(_buyData.nft == address(this)) {
            _transfer(_buyData.owner, msg.sender, _buyData.tokenId);
        }
        else {
            transferNFT(_buyData.nft, _buyData.owner, msg.sender, _buyData.tokenId);
        }
        emit NftTransferred(_buyData.tokenId, msg.value, _buyData.owner, msg.sender, royaltyPercent);
    }
