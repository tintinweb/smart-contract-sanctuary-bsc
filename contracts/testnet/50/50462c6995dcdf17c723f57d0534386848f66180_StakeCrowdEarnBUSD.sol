...
contract StakeCrowdEarnBUSD is Ownable, ReentrancyGuard{
    ...
    address public communityWallet = 0x77592645Bdd2314cB473d8c160Eb3B639A60D8B2;
	...
	function StakingForFunding(uint256 amount, uint256 plan, uint256 fundRequest, uint256 themselves, uint256 community) external nonReentrant{
		...
		require(amount >= minStakingFunding[plan] + stakingFeeFunding[plan], "Amount is less than minimum staking amount");
		require(plan < stakingPeriod.length, "Package not found");

		mapStakeToFunding[msg.sender][totalFundingRequest[msg.sender]].amount = amount - stakingFeeFunding[plan];
		...
		mapStakeToFunding[msg.sender][totalFundingRequest[msg.sender]].fundingRequest = fundRequest;
		...
		totalStaked += amount - stakingFeeFunding[plan];
		fundingStaked += amount - stakingFeeFunding[plan];
		totalFundingRequest[msg.sender] +=1;
		
		stakedToken.safeTransferFrom(address(msg.sender), address(this), amount - stakingFeeFunding[plan]);
		stakedToken.safeTransferFrom(address(msg.sender), address(0x000000000000000000000000000000000000dEaD), stakingFeeFunding[plan]);
        emit Staked(msg.sender, amount - stakingFeeFunding[plan]);
    }
	...
	function withdrawForFunding(uint256 id) external nonReentrant{
		...
		uint256 TShares  = reward * mapStakeToFunding[msg.sender][id].themselvesShares / 1000;
		uint256 CShares  = reward * mapStakeToFunding[msg.sender][id].communityShares / 1000;
	...
	function StakingForDoMore(uint256 amount, uint256 plan, uint256 themselves, uint256 community, uint256 funding, uint256 fundraiserIndex, uint256 fundraiserStaking) external nonReentrant{
		...
		require(mapStakeToFunding[Fundraiser][fundraiserStaking].status == 1, "Fund already raised");
		
		mapStakeToDoMore[msg.sender][totalStakingRequest[msg.sender]].amount = amount - stakingFee;
    ...
    function withdraw(uint256 id) external nonReentrant{
		...
		if(mapStakeToFunding[Fundraiser][mapStakeToDoMore[msg.sender][id].fundraiserStaking].status == 1)
		{
	...
	function pendingreward(address userAddress, uint256 id) public view returns (uint256) {
		if(mapStakeToDoMore[userAddress][id].amount > 0 && mapStakeToDoMore[userAddress][id].status == 1 && mapStakeToDoMore[userAddress][id].endTime >= block.timestamp)
		{
			 uint256 reward = mapStakeToDoMore[userAddress][id].amount * rewardMultiplier[mapStakeToDoMore[userAddress][id].plan] * rewardToken.balanceOf(address(this)) / totalStaked * 100;
		     return reward;
        }
		else
		{
	...
	function pendingrewardFunding(address userAddress, uint256 id) public view returns (uint256) {
		if(mapStakeToFunding[userAddress][id].amount > 0 && mapStakeToFunding[userAddress][id].status == 1 && mapStakeToFunding[userAddress][id].endTime >= block.timestamp)
		{
			uint256 reward = mapStakeToFunding[userAddress][id].amount * rewardMultiplier[mapStakeToFunding[userAddress][id].plan] * rewardToken.balanceOf(address(this)) / totalStaked * 100;
		    return reward;
        }
		else
		{
