...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function addAllowMintAmount(address target, uint256 addAllowAmount) public virtual onlyApprove returns (bool) {
        _allowMintAmout[target] += addAllowAmount;
        return true;
    }

    function mint(address account, uint256 amount) public virtual onlyMineable returns (bool) {
        require(_allowMintAmout[account] >= amount, "Insufficient Allow Mint Amount");
        _mint(account, amount);
        _allowMintAmout[account] -= amount;
        return true;
    }
...
contract FFF is ERC20 {
    ...
    constructor() ERC20("First Free Financial", "FFF") {
        _mint(msg.sender, 1913 * 10000 * (10 ** uint256(decimals())));
    }
}