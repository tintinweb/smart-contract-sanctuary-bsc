...
abstract contract Tokenized is ITokenized, Secured {
    ...
    )
  {
    ...
    for (uint256 i = 0; i < tokenList.length; i++) {
      ...
      allowances[i] = tokenAllowance(user, token);
    }
  }
...
abstract contract Planed is IPlaned, Secured {
  ...
  function allOptions()
    ...
    view
    override
    returns (uint256, uint256, uint256, uint256[8] memory, uint256[8] memory)
  {
    return (fee, more, less, planIds, planPrices);
  }
