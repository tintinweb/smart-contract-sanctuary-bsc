...
contract MarketDistribution is TokensRecoverable, IMarketDistribution {
    ...
    uint256 public rootedAndEliteAmountForLiquidity;
    ...
    function init(
        ...
        uint256 _vestingDuration) public onlyOwner
    {
        rootedToken = _rootedToken;
    ...
    function setupEliteRooted() public onlyOwner {
    ...
    function setupBaseRooted() public onlyOwner {
    ...
    function setLiquidityController(address _liquidityController) public onlyOwner {
        liquidityController = _liquidityController;
    }

    // baseToken = WBNB
    function distribute(uint256 _baseAmountForLiquidity, uint256 _rootedAndEliteAmountForLiquidity, uint256 _lockTimeInSeconds, uint256 _maxTransactionAmount) public override {
        ...
        rootedAndEliteAmountForLiquidity = _rootedAndEliteAmountForLiquidity;
   
        vestingPeriodStartTime = block.timestamp;
        vestingPeriodEndTime = block.timestamp + (vestingDuration * 1 minutes);
        distributionComplete = true;

        uint256 totalBaseTokenCollected = baseToken.balanceOf(address(marketGeneration));
        ...
        // WCRO
        uint256 totalBase = totalBaseTokenCollected.sub(baseAmountForLiquidity);

        baseToken.transfer(projectAddress, totalBase);
    ...
    function createRootedEliteLiquidity() private {
        eliteToken.depositTokens(baseToken.balanceOf(address(this)));
        uniswapV2Router.addLiquidity(address(eliteToken), address(rootedToken), rootedAndEliteAmountForLiquidity, rootedAndEliteAmountForLiquidity, 0, 0, address(this), block.timestamp);
    }

    function createRootedBaseLiquidity() private {
        uniswapV2Router.addLiquidity(address(baseToken), address(rootedToken), baseAmountForLiquidity, rootedAndEliteAmountForLiquidity, 0, 0, liquidityController, block.timestamp);
