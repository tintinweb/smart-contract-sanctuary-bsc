...
contract Empires {
    ...
    function upgradeTower(uint256 towerId) external {
        ...
        towers[user].crystals -= getUpgradePrice(towerId, workers) / denominator;
    ...
    function upgradeTreasury() external {
     ...
     function sellTower() external {
        collectMoney();
        address user = msg.sender;
        uint8[5] memory workers = towers[user].workers;
        totalWorkers -= workers[0] + workers[1] + workers[2] + workers[3] + workers[4];
        towers[user].money += towers[user].yield * 24 * 5;
        towers[user].workers = [0, 0, 0, 0, 0];
        towers[user].yield = 0;
        towers[user].treasury = 0;
    }
