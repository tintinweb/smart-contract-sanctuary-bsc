...
contract ShadowFiDonationPortal is Ownable, ReentrancyGuard {
    ...
    function donateBNB() payable public nonReentrant {
        ...
        uint8 tier = getTierByCost((msg.value / 1e18) * (convertedPrice / 1e8));
    ...
    function getTierByCost(uint256 _cost) public view returns (uint8 tier) {
        if (_cost >= tierCosts[0] && _cost < tierCosts[1]) {
            return 0;
        }
        if (_cost >= tierCosts[1] && _cost < tierCosts[2]) {
            return 1;
        }
        if (_cost >= tierCosts[2] && _cost < tierCosts[3]) {
            return 2;
        }
        if (_cost >= tierCosts[3]) {
            return 3;
        }
        else {
            return 4;
