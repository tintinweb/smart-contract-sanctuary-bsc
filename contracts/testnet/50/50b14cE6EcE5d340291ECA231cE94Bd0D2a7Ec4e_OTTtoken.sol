...
contract OTTtoken  is Context, IERC20, Ownable {
    ...
    string private _name = "ONE TRADE TOKEN";
    string private _symbol = "OTT";
    ...
    uint256 private numTokensSellToAddToLiquidity = 10000000000 * 10 **8;
    ...
    constructor () public {
        ...
        _marketingWallet = 0x72eD190Eb0b3e603Ae3fbC6748322e9EbeF92e3A;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        //(uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tBurn, uint256 tMarketing) = _getValues(tAmount);
        uint256[8] memory values = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256[8] memory values) {
        ...
        // @return rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidity, tBurn, tMarketing, 
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256[5] memory tValues) {
        ...
        uint256 tTransferAmount = tempTransferAmount.sub(tBurn).sub(tMarketing);
    ...
    function _getRValues(uint256[5] memory tValues, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        //function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, tBurn, tMarketing,uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = tempTransferAmount.sub(rBurn).sub(rMarketing);
    ...
    function restoreTax() public onlyOwner{
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, burn, liquidity, marketing
