...
contract NanoMinions {
    ...
    // Cleam Robot
    function cleamRobot() external onlyOpen onlyInvasion {
    ...
    function participateNanottery(uint _ticketCount) external onlyNanoMinionsPlayer {
        ...
        if (cleamedRobot[msg.sender] > 0) {
            nowInviteRebatePrice = calculateRobotCleam(cleamedRobot[msg.sender]);
        }

        uint nowRobotCleamPrice = calculateRobotCleam(getRobotSinceLastClone(msg.sender));
        uint buyPrice = _ticketCount * nanotteryPrice;
        require(nowInviteRebatePrice + nowRobotCleamPrice >= buyPrice, "not enough BUSD");
        require(_ticketCount >= 1, "at least one");

        uint haveRobot = marketRobot * 5 / 100;
        uint robotPrice = calculateRobotCleam(haveRobot);
        uint robot = buyPrice * haveRobot / robotPrice;
        marketRobot += robot;

        if(cleamedRobot[msg.sender] >= robot) {
            cleamedRobot[msg.sender] -= robot;
        } else {
            robot -= cleamedRobot[msg.sender];
            cleamedRobot[msg.sender] = 0;

            uint remainingRobotPrice = calculateRobotCleam(robot);
            uint nowWorksTime = min(ROBOT_TO_WORKING_WORKER, block.timestamp - lastWorking[msg.sender]);
            uint useTime = remainingRobotPrice * nowWorksTime / nowRobotCleamPrice;
