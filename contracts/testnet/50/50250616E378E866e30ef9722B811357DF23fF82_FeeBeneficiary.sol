...
// File: cocobolo/FeeBeneficiary.sol
...
contract FeeBeneficiary is Ownable {
    ...
    function chargeFeeFixedPrice(
        ...
        if (feeTo!= address(this)) {
            _usdt.transferFrom(_from,feeTo, fee);
        }

        if (liquidity!= address(this)) {
            _usdt.transferFrom(_from, liquidity, feeLQ);
        }
    ...
    function tranferFoundOfMint(
        ...
        if(address(poolLiquidityAddress)!= address(0)) {
            _usdt.transferFrom(_from, poolLiquidityAddress, _amount * poolLiquidityMintPercentage / 100);
        }
        if(address(houseFounds)!= address(0)) {
            _usdt.transferFrom(_from,houseFounds, _amount * houseFoundsPercentage / 100);
        }
    }
