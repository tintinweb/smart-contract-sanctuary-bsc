...
contract FarmPotTest01 is Ownable {
        ...
        returns (uint256)
    {
        ...
        uint256 avail = ((initial * time) / 100) + refRew + prev;
        return avail;
    }
    ...
    function claimRewards() public {
        ...
        user_data[msg.sender].lastUpdated = block.timestamp + setDays;
        user_data[msg.sender].referralRewards = 0;
    }
    ...
    function doubleUp() public {
        ...
        if (_availableRewards < doubleUpAmount) {
            CAFToken.safeTransferFrom(
                msg.sender,
                address(this),
                doubleUpAmount - _availableRewards
            );
            ...
            user_data[msg.sender] = User(
                ...
                0,
                0,
                _availableRewards - doubleUpAmount,
                oldRef,
                block.timestamp,
                block.timestamp
            );
    ...
    function calculatePayout() public view returns (uint256) {
      ...
      uint256 finalValue;
        if (contractBalance >= 10000000 * 1e18) {
            finalValue = contractBalance.div(100);
            return finalValue;
        } else if (contractBalance >= 5000000 * 1e18) {
           finalValue = contractBalance.mul(75).div(10000);
           return finalValue;
        } else if (contractBalance >= 2500000 * 1e18) {
            finalValue = contractBalance.mul(50).div(10000);
            return finalValue;
        } else {
            finalValue =  contractBalance.mul(30).div(10000);
            return finalValue;
        }
    }
