...
import "ERC20.sol";
import "RangeOptionConfigUnintialized.sol";
import "BufferNFTCore.sol";
import "BufferEpocPoolV2Unintialized.sol";

// import "OptionFeeCalculator.sol";
 ...
 */
contract BufferConvertibleOptions is
    IBufferRangeOptions,
    Ownable,
    ReentrancyGuard,
    BufferNFTCore
{
    ...
    mapping(uint256 => ConvertibleOption) public options;
    BufferEpocPoolV2Unintialized public pool;
    RangeOptionConfigUnintialized public config;
    mapping(uint256 => uint256) public expiryToRoundID;
    mapping(uint256 => ConvertibleSlotDetail) public slotDetails;
    ...
    constructor(
        ERC20 _stableCoin,
        ITwapPriceProvider pp,
        BufferEpocPoolV2Unintialized _pool,
        RangeOptionConfigUnintialized _config
    ) {
     ...
     * @param amount amount of tokenX
     * @param metadata ConvertibleOption metadata
        ...
        returns (uint256 optionID)
    {
        ...
        uint256 lockedAmount = (config.faceValue() * amount) /
            config.lowerBound();

        ConvertibleOption memory option = ConvertibleOption(
            State.Active,
            config.upperBound(),
            config.lowerBound(),
            amount,
            lockedAmount,
            (premium * tokenXDecimals * 1e8) /
                ((10**stableCoin.decimals()) * currentPrice),
            poolExpiration,
            fixedOptionType
        );
        ...
        if (msg.sender != pool.projectOwner()) {
            stableCoin.transfer(pool.projectOwner(), premium);
            stableCoin.transfer(owner(), settlementFee);
    ...
    function _modifyOption(
        uint256 optionID,
        ConvertibleOption memory option,
        uint256 lockedAmount,
        uint256 amount,
        uint256 premium
    ) internal returns (ConvertibleOption memory modifiedOption) {
     ...
     * @param expiryRoundId the expected roundId for expiryTimestamp
     */
        ...
        returns (bool isCorrectRoundId)
    {
        ...
        isCorrectRoundId = priceProvider.isRoundIdValid(
            expiryRoundId,
            expiryTimestamp
        );
        if (isCorrectRoundId) {
    ...
    function getPayout(uint256 optionID) public returns (uint256 payout) {
        ConvertibleOption storage option = options[optionID];
        require(option.state == State.Active, "O14");

        uint256 roundID = expiryToRoundID[option.expiration];
        require(roundID > 0, "O20");

        (, uint256 priceAtExpiration) = priceProvider.getRoundData(roundID);

        if (
            priceAtExpiration > config.lowerBound() &&
            priceAtExpiration < config.upperBound()
        ) {
            payout = (config.faceValue() * option.amount) / priceAtExpiration;
        } else if (priceAtExpiration < config.lowerBound()) {
            payout = (config.faceValue() * option.amount) / config.lowerBound();
        } else {
            payout = (config.faceValue() * option.amount) / config.upperBound();
        }

        if (payout > option.lockedAmount) payout = option.lockedAmount;
    }
     ...
     */
    function exercise(uint256 optionID) public returns (uint256 payout) {
        require(exists(optionID), "O10");
        require(pool.fixedExpiry() <= block.timestamp, "O4");

        payout = getPayout(optionID);
        pool.send(optionID, ownerOf(optionID), payout);
        ...
        emit Exercise(optionID, payout);
    }

    /**
     * @notice Used for getting the actual options prices in pool's tokenX
     * @param amount ConvertibleOption amount
    ...
    {
        uint256 tokenXDecimals = 10**(ERC20(pool.tokenX()).decimals());
        uint256 discountValue = (config.faceValue() * config.discountFactor()) /
            100;
        uint256 fee = (config.faceValue() - discountValue) * amount;
        total = (fee * (10**stableCoin.decimals())) / (tokenXDecimals * 1e8);
        settlementFee = (total * config.settlementFeePercentage()) / 100;
    ...
    function _getOption(uint256 optionID)
        internal
        view
        returns (ConvertibleOption memory)
    {
        return options[optionID];
    }

    function _setOption(uint256 optionID, ConvertibleOption memory option)
        internal
    {
        ...
        returns (uint256[] memory newOptionIDs)
    {
        ...
        ConvertibleOption memory option = _getOption(optionID);
        ...
        for (uint256 i = 0; i < splitUnits_.length; i++) {
            ...
            ConvertibleOption memory newOption = ConvertibleOption(
                option.state,
                option.upperBound,
                option.lowerBound,
                childAmount,
        ...
        for (uint256 i = 0; i < splitUnits_.length; i++) {
            ConvertibleOption memory newOption = _getOption(newOptionIDs[i]);
        ...
        external
    {
        ...
        ConvertibleOption memory targetOption = _getOption(targetOptionID);
        ...
        for (uint256 i = 0; i < optionIDs.length; i++) {
            ConvertibleOption memory option = _getOption(optionIDs[i]);
    ...
    function createSlot(
        ...
        slotDetails[slot] = ConvertibleSlotDetail(
            upperBound,
            lowerBound,
            expiration,
            optionType,
...
import "OptionConfigUnintialized.sol";
import "Interfaces.sol";
import "BufferEpocPoolV2Unintialized.sol";
