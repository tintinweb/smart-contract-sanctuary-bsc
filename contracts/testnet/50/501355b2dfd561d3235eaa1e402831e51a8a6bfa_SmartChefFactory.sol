...
// File: IEvocProfile.sol

/**
 * @title IEvocProfile
 */
interface IEvocProfile {
...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when EVOC mining ends.
    uint256 public bonusEndBlock;

    // The block number when EVOC mining starts.
    ...
    // Evoc profile
    IEvocProfile public immutable evocProfile;

    // Evoc Profile is requested
    bool public evocProfileIsRequested;

    // Evoc Profile points threshold
    uint256 public evocProfileThresholdPoints;

    // EVOC tokens created per block.
    ...
    /**
     * @notice Constructor
     * @param _evocProfile: Evoc Profile address
     * @param _evocProfileIsRequested: Evoc Profile is requested
     * @param _evocProfileThresholdPoints: Evoc Profile need threshold points
     */
    constructor(
        address _evocProfile,
        bool _evocProfileIsRequested,
        uint256 _evocProfileThresholdPoints
    ) {
        SMART_CHEF_FACTORY = msg.sender;

        // Call to verify the address is correct
        IEvocProfile(_evocProfile).getTeamProfile(1);
        evocProfile = IEvocProfile(_evocProfile);

        // if evocProfile is requested
        evocProfileIsRequested = _evocProfileIsRequested;

        // evocProfile threshold points when profile & points are requested
        evocProfileThresholdPoints = _evocProfileThresholdPoints;
    }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        require(
            (!evocProfileIsRequested && evocProfileThresholdPoints == 0) ||
            evocProfile.getUserStatus(msg.sender),
            "Deposit: Must have an active profile"
        );

        uint256 numberUserPoints = 0;

        if (evocProfileThresholdPoints > 0) {
            (, numberUserPoints, , , , ) = evocProfile.getUserProfile(msg.sender);
        }

        require(
            evocProfileThresholdPoints == 0 || numberUserPoints >= evocProfileThresholdPoints,
    ...
    function updateProfileAndThresholdPointsRequirement(bool _isRequested, uint256 _thresholdPoints) external onlyOwner {
        ...
        evocProfileIsRequested = _isRequested;
        evocProfileThresholdPoints = _thresholdPoints;
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 _evocReward = multiplier * rewardPerBlock;
            uint256 adjustedTokenPerShare = accTokenPerShare + (_evocReward * PRECISION_FACTOR) / stakedTokenSupply;
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 _evocReward = multiplier * rewardPerBlock;
        accTokenPerShare = accTokenPerShare + (_evocReward * PRECISION_FACTOR) / stakedTokenSupply;
        lastRewardBlock = block.number;
    }
...
contract SmartChefFactory is Ownable {
    ...
    function deployPool(
        ...
        bytecode = abi.encodePacked(
            bytecode,
            abi.encode(_evocProfile, _evocProfileIsRequested, _evocProfileThresholdPoints)
        );
