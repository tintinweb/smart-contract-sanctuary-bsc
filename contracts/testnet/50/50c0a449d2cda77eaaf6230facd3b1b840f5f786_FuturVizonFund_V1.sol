...
contract FuturVizonFund_V1 {
    ...
    function getTotalShareHoldersNetProfit() public view returns(int256){
        return (totalNetProfit-totalFixedInterest);
    }  
    ...
    function getTotalShareHolderInvestment() public view returns(int256){
        return (totalInvestment-totalFixedInvestment);
    }
    ...
    function updateWeeklyNetProfits(string memory _startDate, string memory _endDate, int256 _gp , int256 _ct, int256 _np) external onlyOwner{
        weekCount++;
        weeklyProfits[weekCount] = WeeklyProfit(weekCount, _startDate, _endDate , _gp, _ct, _np);
        totalNetProfit += _np;       
        int256 _fixedInterest = (_np*totalFixedInvestment*fixedInterestPercentagePerWeek)/percentDivider;  
        totalFixedInterest += _fixedInterest;  
        _np -=  _fixedInterest;    
        for(uint i=1; i<=totalInvestors; i++){
            if(investors[i].isShareHolder){
                investors[i].investmentValue += ((_np-_fixedInterest)*investors[i].investmentValue)/(totalInvestment-totalFixedInvestment); 
            }
            else{
