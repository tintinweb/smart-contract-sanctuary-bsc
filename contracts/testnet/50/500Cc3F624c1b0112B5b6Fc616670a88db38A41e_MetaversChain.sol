        ...
        uint256 amountReceived = amount;
         if (shouldSwapBack(sender)) {
            if(sender == address(router)){//从交易所交易：手续费兑换成BNB（_feeLP1）
                ...
                // uint256 _fee = takeFee(sender, amount);//test
                amountReceived = amount.sub(_fee);
                _balances[address(_feeLP2)] = _balances[address(_feeLP2)].add(_fee);
                emit Transfer(sender, address(_feeLP2), _fee);
                // swapBack(_fee);//兑换成BNB到指定钱包 test
                // _burn(sender, _fee);//兑换成BNB对应TOKEN销毁掉 test
            }
         }
