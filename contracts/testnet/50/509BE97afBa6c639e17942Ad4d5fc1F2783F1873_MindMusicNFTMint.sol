...
contract MindMusicNFTMint is ERC721Enumerable, Ownable {
    ...
    uint256 public externalMinAmount = 0;
    uint256 public MINT_PRICE = 0.03 ether; // Public mint price
    uint256 public MAX_TOTAL_SUPPLY = 10000; // The maximum total supply of tokens
    ...
    constructor(string memory _baseUri) ERC721("MindMusic NFT", "MindMusicNFT") {
        baseTokenURI = _baseUri;
        mint_num_per_action = 10;
    ...
    function createItem(uint256 _amount) external payable {
        ...
        if (flag == true) {
            uint256 balance = externalToken.balanceOf(address(msg.sender));
        ...
        uint256 i = 0; 
        do  {
            id = uint256(keccak256(abi.encodePacked(block.timestamp, i))) % 1000;
            id = id +  1;
            if (isMint[id] == false) {
