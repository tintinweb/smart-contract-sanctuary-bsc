...
contract Reservoir {
    ...
    ISwap private swap;
    bool _reentrancyLock;

    /*=======================================
    =            PUBLIC FUNCTIONS           =
    =======================================*/

    constructor(address _swapAddress, address _collateralAddress) {
    ...
    function buy() external payable noReentrancy returns (uint256){
    ...
    function reinvest() external onlyStronghands noReentrancy returns (uint) {
        // fetch dividends
    ...
    function withdraw() external onlyStronghands noReentrancy returns (uint) {
        // setup data
        address _customerAddress = msg.sender;
        ...
        payable(_customerAddress).transfer(bnbAmount);
        ...
        // fire event
        emit onWithdraw(_customerAddress, bnbAmount, block.timestamp);
    ...
    function sell(uint256 _amountOfTokens) external onlyStronghands noReentrancy {
        // setup data
        address _customerAddress = msg.sender;
