...
contract CHT is ERC20, Ownable {
    ...
    address public marketingWallet = 0xe7d4863894903A6f45a2d3e823d926210dFd71e2;
    address public devWallet = 0x9fADc507e3324CD4ADD56ba4069d86aa057FC8D3;
    address public buybackWallet = 0xa751A48e69E78cFC7adfb1021ef5f41A780B76e3;
    ...
    Taxes public taxes = Taxes(2, 2, 2, 0);
    Taxes public sellTaxes = Taxes(2, 2, 2, 0);
    ...
    constructor(address routerAdd, address _liquidityPairToken) ERC20("Community Hedge Token", "CHT") {
        _tokengeneration(msg.sender, 98 * 1e7 * 10**decimals());
        _tokengeneration(address(this), 1e7 * 10**decimals());
        _tokengeneration(devWallet, 1e7 * 10**decimals());
    ...
    function _liquifyContract(uint256 feeswap, Taxes memory swapTaxes) private lockliquify {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 initialBalance = IERCliquidityPairToken.balanceOf(address(this));

            //swapTokensForETH(toSwap);

            // uint256 deltaBalance = IERCliquidityPairToken.balanceOf(address(this)) - initialBalance;
            ...
            // if (marketingAmt > 0) {
            //     IERCliquidityPairToken.transferFrom(address(this), marketingWallet, marketingAmt);
            // }

            // uint256 buybackAmt = unitBalance * 2 * swapTaxes.buyback;
            // if (buybackAmt > 0) {
            //     IERCliquidityPairToken.transferFrom(address(this), buybackWallet, buybackAmt);
            // }

            // uint256 devAmt = unitBalance * 2 * swapTaxes.dev;
            // if (devAmt > 0) {
            //     IERCliquidityPairToken.transferFrom(address(this), devWallet, devAmt);
            // }
    ...
    function SetBuyTaxes(
        ...
        require((_marketing + _liquidity + _buyback + _dev) <= 15, "Must keep fees at 15% or less");
    }
