...
contract Distribution is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, ContextUpgradeable {
    struct Buyer {
        uint256 saleId;
        address account;
        uint256 amount;
    ...
    function initialize(address _project, address _sale) external initializer {
    ...
    function claim(uint256 _saleId) external nonReentrant {
        SaleInfo memory sale = ISale(sale).getSale(_saleId);
        require(sale.id > 0, "Invalid sale id");

        Buyer storage buyer = buyers[_saleId][_msgSender()];

        ProjectInfo memory project = IProject(project).getProject(sale.projectId);
        require(block.timestamp >= project.distributionStart, "claim: distribution is not start");
        require(buyer.account == _msgSender(), "claim: caller is not the buyer");
        if (project.isSingle) {
            IOSB721(project.token).safeTransferFrom(address(this), _msgSender(), sale.tokenId);
        }
        else {
            uint256 amount = buyer.amount;
            buyer.amount = 0;
            IOSB1155(project.token).safeTransferFrom(address(this), _msgSender(), sale.tokenId, amount, "");
        }
    }

    function setSale(uint256 _saleId, address _buyer, uint256 _amount) external {
...
contract $Distribution is Distribution {
    constructor() {}
