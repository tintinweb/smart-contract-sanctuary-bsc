...
contract Lottery1 is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInRush = 50 ether;
    uint256 public minPriceTicketInRush = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInRush;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] rushPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInRush;
        uint32 finalNumber;
    }
    ...
    constructor(address _rushTokenAddress, address _randomGeneratorAddress) {
        rushToken = IERC20(_rushTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of rush to this contract
        uint256 amountRushToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInRush,
            _ticketNumbers.length
        );

        // Transfer rushT tokens to this contract 
        rushToken.safeTransferFrom(address(msg.sender), address(this), amountRushToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInRush += amountRushToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInRushToTransfer
        uint256 rewardInRushToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInRushToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        rushToken.safeTransfer(msg.sender, rewardInRushToTransfer);

        emit TicketsClaim(msg.sender, rewardInRushToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInRush) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury 
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in rushT for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].rushPerBracket[j] =
                ...
                // A. No RUSH to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].rushPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInRush - amountToShareToWinners);

        // Transfer RUSH to treasury address
        rushToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        rushToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInRush += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInRush >= minPriceTicketInRush) && (_priceTicketInRush <= maxPriceTicketInRush),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInRush: _priceTicketInRush,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            rushPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInRush: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(rushToken), "Cannot be RUSH token");
    ...
    function setMinAndMaxTicketPriceInRush(uint256 _minPriceTicketInRush, uint256 _maxPriceTicketInRush)
        external
        onlyOwner
    {
        require(_minPriceTicketInRush <= _maxPriceTicketInRush, "minPrice must be < maxPrice");

        minPriceTicketInRush = _minPriceTicketInRush;
        maxPriceTicketInRush = _maxPriceTicketInRush;
    }
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].rushPerBracket[_bracket];
        } else {
            return 0;
        }
    }
