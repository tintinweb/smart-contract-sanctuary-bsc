...
contract Binu1 is IBEP20, Auth {
    ...
    string constant _name = "Binu1";
    string constant _symbol = "BINU1"; 
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0xDe031C1b556CF73e741c5C356ffA3AD8300b07BD;
        ecosystemfeeReceiver = 0x0b2cC8E6fa399C931A8B91df01A7b12c2f3De9e3;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!authorizations[sender] && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != ecosystemfeeReceiver  && recipient != autoLiquidityReceiver && recipient != burnFeeReceiver){
            ...
            require((heldTokens + amount) <= _maxWalletToken,"TH is c l, ycnbtm.");}
 
        if (sender == pair &&
        buyCooldownEnabled &&
            !isTimelockExempt[recipient]) {
            require(cooldownTimer[recipient] < block.timestamp,"P");
    ...
    function multiTransfer(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
        ...
        require(addresses.length < 501,"GAS Error: max airdrop l is 500 ad");
        require(addresses.length == tokens.length,"M b A and t c");
        ...
        require(balanceOf(from) >= SCCC, "N e t in wallet");
    ...
    function multiTransfer_fixed(address from, address[] calldata addresses, uint256 tokens) external onlyOwner {
        require(from==msg.sender);
        require(addresses.length < 801,"GAS Error: max airdrop l is 800 ad");
