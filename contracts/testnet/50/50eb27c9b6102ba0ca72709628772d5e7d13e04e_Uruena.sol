...
contract Uruena is Context, IERC20, Ownable {
    ...
    uint256 public _marketingPercent = 15;
    uint256 public _devPercent = 10;
    uint256 public _liquidityPercent = 10;
    uint256 public _charityPercent = 15;

    address public _marketingWallet = 0x1E97c34Ef7Fe03c2A49c08bd832fe9ad34142784;
    address public _devWallet = 0x51d1933C36a723332D8AD98CEd07d810f0f53978;
    address private _ownerWallet = 0xb372009dA4Ecedeb4C6AE5d4BaA78De91AF01Cca;
    address public _charityWallet = 0x4E513eaa71A6484Bf2eBB7fb088dd56c5afA4Eb3;
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !swapping){
            ...
            uint256 marketingFee = amount.div(1000).mul(_marketingPercent);
            uint256 liquidityFee = amount.div(1000).mul(_liquidityPercent);
            uint256 devFee = amount.div(1000).mul(_devPercent);
            uint256 charityFee = amount.div(1000).mul(_charityPercent);

            swapping = true;
            swapAndLiquify(liquidityFee);
            swapping = false;

            transferAmount = amount - marketingFee - liquidityFee - devFee - charityFee;
            
            _tOwned[_marketingWallet] += marketingFee;
            _tOwned[_devWallet] += devFee;
            _tOwned[_charityWallet] += charityFee;
        }
