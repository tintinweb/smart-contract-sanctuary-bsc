...
contract BamiRefundIDO is Ownable, ReentrancyGuard {
    ...
    // Refund
    uint256 public amountRefundPool1 = 75 * generalDec;
    uint256 public amountRefundPool2 = 75 * generalDec;
    uint256 public amountRefundPool3 = 75 * generalDec;
    mapping(uint256 => mapping(address => bool)) public refundedMap;
    ...
    function refund() public nonReentrant isRun validTimeRefund {
        ...
        if (hesmanPool1.whitelistMap(msg.sender)) {
            require(refundedMap[1][msg.sender] == false, "Already refunded");
            require(
                hesmanPool1.vestingMap(msg.sender) >= 1,
                "You didnt't claim first"
            );
            refundedMap[1][msg.sender] = true;
        ...
        if (hesmanPool2.whitelistMap(msg.sender)) {
            require(refundedMap[2][msg.sender] == false, "Already refunded");
            require(
                hesmanPool2.vestingMap(msg.sender) >= 1,
                "You didnt't claim first"
            );
            refundedMap[2][msg.sender] = true;
        ...
        if (hesmanPool3.whitelistMap(msg.sender)) {
            require(refundedMap[3][msg.sender] == false, "Already refunded");
            require(
                hesmanPool3.vestingMap(msg.sender) >= 1,
                "You didnt't claim first"
            );
            refundedMap[3][msg.sender] = true;
