...
contract WeedFarmer {
    ...
    uint256 public CUTOFF_STEP = 60;  //testnet
	uint256 public WITHDRAW_COOLDOWN = 60;  //testnet
	uint256 public COMPOUND_COOLDOWN = 60;  //testnet
    ...
    uint256 public TOP_DEPOSIT_STEP = 120; // testnet
    ...
    //TopDeposit[] public topDepositPool;
    mapping(uint8 => TopDeposit) public topDepositPool; 
	mapping (address => User) internal users;

    address payable private treasury;
	address public owner;
   // uint public startTime = 1671476400; // Monday, 19 December 2022 20:00:00 UTC https://www.unixtimestamp.com/ //mainnet
    uint public startTime = 1664820000; // Monday, 10 October 2022 18:00:00 UTC https://www.unixtimestamp.com/ //testnet
	...
	constructor(address payable _treasury) {
        ...
        plans.push(Plan( 90, 15,  2 * 1e16, 40 ether, 0, 0, 0, 0, true));
	...
	function withdraw() public {
        ...
        emit FeePayed(msg.sender, payFees(totalAmount));
 ...
 /**   function addTopDepositRewardToPlan(address userAddress, uint8 plan, uint256 win) private {
        User storage user = users[userAddress];
        uint8 safety = 0;
        for (uint256 i = 0; i < user.deposits.length; i++) {
            
            if(user.deposits[i].plan == plan && safety == 0){ //add reward to first matching plan
                user.deposits[i].amoun = user.deposits[i].amount.add(win);
                safety = 1;
            }
         }
    } **/
	...
	function _drawPool() private {
        ...
        for(uint8 i = 0; i < pool_bonuses.length; i++) {
		    ...
		    //addTopDepositRewardToPlan(topDepositPool[i].pool_top_addr, plan, win);
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 percent, uint256 amount, uint256 userTotalInvested, uint256 start, uint256 finish, bool reinvested) {
	    User storage user = users[userAddress];
