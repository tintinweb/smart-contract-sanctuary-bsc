...
contract PackageSale {
    ...
    function withdrawMonthly(uint id) external {
        ...
        // require(block.timestamp < currentPurchase.expirationTimestamp, "TIME_EXPIRED");
        // would prevent any withdraws after 365 days of purchase. Uncomment if needed
        require(block.timestamp > currentPurchase.nextPayoutTimestamp, "NOT_TIME_YET");
        require(currentPurchase.payouts < 12, "NO_PAYOUT_LEFT");

        currentPurchase.nextPayoutTimestamp += currentPurchase.payoutPeriodInSeconds;
        currentPurchase.totalBUSDreceived += currentPurchase.payoutAmount;
        currentPurchase.payouts += 1;

        BUSD.transfer(currentPurchase.user, currentPurchase.payoutAmount);
