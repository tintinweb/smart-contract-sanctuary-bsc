...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  function _giveReward(uint _mission, bool _firstConsumeInDay) private {
    ...
    rewardManager.addPVEReward(_mission, msg.sender, reward);
  }
  ...
  function _validateFinishGame(uint _mission, uint[] calldata _heroIds, bytes memory _signature) private {
    ...
    if (_mission == biggestFinishMission[msg.sender] + 1) {
  ...
  function _validateFinishGameBypassSignature(uint _mission, uint[] calldata _heroIds) private view {
    require(_heroIds.length >= 3 && _heroIds.length <= 5, "PVE: HeroIds is invalid");
    require(_mission <= totalMissions, "PVE: Mission invalid");
  }
  ...
  function _initData() private {
    ...
    for(uint i = 1; i <= totalMissions; i++) {
      missions[i] = Mission(i, i * 1e18, uint32(i), address(0));
    }
  }
...
interface IRewardManager {
  function addPVEReward(uint _mission, address _user, uint _amount) external;
}
