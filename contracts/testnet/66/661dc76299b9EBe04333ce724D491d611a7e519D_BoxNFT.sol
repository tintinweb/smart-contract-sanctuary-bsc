...
contract BoxNFT is ERC721Upgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable, PausableUpgradeable{
    ...
    ShoeNFT public shoeNFT;
    mapping (uint256 => uint256) public minQuality;
    mapping (uint256 => uint256) public maxQuality;
    ...
    modifier onlyBoxShopOrOwner {
      require(msg.sender == boxShop || msg.sender == owner());
      _;
    }

    //EVENT
    event openBoxEvent(uint256 shoeId);

    function initialize() public initializer {
      __ERC721_init("Box NFT MOVERSE", "BONM");
      openBoxActive=true;
      minQuality[1]=4;
      minQuality[2]=8;
      minQuality[3]=15;
    ...
    function initByOwner(ShoeNFT _shoeNFT,address _boxShop, address _boxMarketPlace, address _operator) public  onlyOwner {
        shoeNFT = _shoeNFT;
        boxShop=_boxShop;
  ...
  function createBox(address owner,uint256 boxType,uint256 rare) public onlyOperatorOrOwner returns (uint256) {
        _tokenIds.increment();
        uint256 newBoxId = _tokenIds.current();
        _safeMint(owner, newBoxId);
        boxes[owner][newBoxId] = Box(newBoxId,boxType,rare);
        return newBoxId;
    }
...
function transfer(uint256 _nftId, address _target)
        external whenNotPaused
    {
        ...
        boxes[ownerOf(_nftId)][_nftId]= Box(0,0,0);
...
function buyBox(address buyer,uint256 rare, uint256 boxType) external whenNotPaused onlyBoxShopOrOwner returns (uint256) {
        _tokenIds.increment();
        uint256 newBoxId = _tokenIds.current();
        _safeMint(buyer, newBoxId);
        boxes[buyer][newBoxId] =Box(newBoxId,boxType,rare);
        return newBoxId;
    }

function openBox(uint256 tokenId) external whenNotPaused returns (uint256 shoeId){
        require(openBoxActive==true, "openBox not active");
        Box memory box = boxes[msg.sender][tokenId];
        boxes[msg.sender][tokenId]= Box(0,0,0);
        uint256 rare=box.rare;
        uint256 shoeType = 1;
              shoeId=shoeNFT.createShoe(msg.sender,
              shoeType,
              0,
              1,
              utils.random1(minQuality[rare],maxQuality[rare]),
              utils.random2(minQuality[rare],maxQuality[rare]),
              utils.random3(minQuality[rare],maxQuality[rare]),
              utils.random4(minQuality[rare],maxQuality[rare]),
              0);
        emit openBoxEvent(shoeId);
        _burn(tokenId);
    }
  ...
  function setBoxShop(address _boxShop) public onlyOwner{
    boxShop=_boxShop;
  }
  ...
  function setShoeNFT(ShoeNFT _shoeNFT) public onlyOwner{
    shoeNFT=_shoeNFT;
  }
  ...
  function updateBox(address owner,uint256 nftId, uint256 id, uint256 boxType, uint256 rare) public onlyOperatorOrOwner returns (uint256) {
        boxes[owner][nftId] =Box(id,boxType,rare);
        return nftId;
    }
  ...
  /**
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI,box.rare,"_",box.boxType,".json"))  : "";
    }
...
function getShoePublic(address _owner, uint256 _id) public view returns (
  ...
  function initByOwner(address _shoeMarketPlace,address _shoeShop, address _boxNFT) public onlyOwner{
...
contract Utils is OwnableUpgradeable, PausableUpgradeable  {
    ...
    function random(uint256 from, uint256 to) public whenNotPaused view returns (uint256 number) {
        ...
        uint256 tmp3  = uint(keccak256(abi.encodePacked(block.timestamp,randomNumber,msg.sender)))%123;
     ...
     function random1(uint256 from, uint256 to) public whenNotPaused view returns (uint256 number) {
        ...
        uint256 tmp3  = uint(keccak256(abi.encodePacked(block.timestamp,randomNumber,msg.sender)))%333;
        uint256 tmp4= uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randomNumber))) % randomNumber;
        number = from + ((tmp1 +tmp2 +tmp3 + tmp4 )  % (to-from+1));
    }
    ...
    function random2(uint256 from, uint256 to) public whenNotPaused view returns (uint256 number) {
        ...
        uint256 tmp3  = uint(keccak256(abi.encodePacked(block.timestamp,randomNumber,msg.sender)))%555;
        uint256 tmp4= uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randomNumber))) % randomNumber;
        number = from + ((tmp1 +tmp2 +tmp3 + tmp4 )  % (to-from+1));
    }
     ...
     function random3(uint256 from, uint256 to) public whenNotPaused view returns (uint256 number) {
        require(to > from, "Not correct input");
        uint256 tmp1  = block.timestamp<<10 % 15;
        uint256 tmp2  = block.timestamp<<20 % 50;
        uint256 tmp3  = uint(keccak256(abi.encodePacked(block.timestamp,randomNumber,msg.sender)))%99;
        uint256 tmp4= uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randomNumber))) % randomNumber;
        number = from + ((tmp1 +tmp2 +tmp3 + tmp4 )  % (to-from+1));
    }
      ...
      function random4(uint256 from, uint256 to) public whenNotPaused view returns (uint256 number) {
        ...
        uint256 tmp3  = uint(keccak256(abi.encodePacked(block.timestamp,randomNumber,msg.sender)))%50;
        uint256 tmp4= uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randomNumber))) % randomNumber;
        number = from + ((tmp1 +tmp2 +tmp3 + tmp4 )  % (to-from+1));
    }
