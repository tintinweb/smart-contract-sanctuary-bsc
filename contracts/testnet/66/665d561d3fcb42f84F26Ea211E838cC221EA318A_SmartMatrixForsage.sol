...
contract SmartMatrixForsage {
    ...
    mapping(uint => address[]) public users_by_level;    
    ...
    constructor() public {
        ...
        users_by_level[LAST_LEVEL].push(ownerAddress) ;
    }
    ...
    function buyNewLevel( uint8 level) external payable {
        ...
        require(level > 1 && level <= LAST_LEVEL, "invalid level");
            ...
            payable(marketing_address).transfer(msg.value * 75 / 100);
            payable(lottery_address).transfer(msg.value * 20 / 100);

            address random_ref = findRandomX3Referrer(level);
    ...
    function findFreeX3Referrer(address userAddress, uint8 level) public view returns(address) {
     ...
     function findRandomX3Referrer(uint8 level) public view returns(address) {
       while (true) {
            if (users_by_level[level].length>0) {
                uint index=random()%users_by_level[level].length;

                return users_by_level[level][index];
            }
            level= level+1;
       }
    }
