...
contract Invoices is Base {
    ...
    address private subscribersAddr;

    event paid(uint256 indexed invoice_id);
    ...
    modifier onlyContract(address _contractAddr) {
        require(_contractAddr.isContract(), "Address: call non-contract");
        _;
    }
    ...
    function pay(uint256 _invoice_id) external onlyAvailable(_invoice_id) {
        ...
        require(invoices[_invoice_id].amount <= allowance, "Not enough USDT");
        usdt.transferFrom(
            msg.sender,
            address(this),
            invoices[_invoice_id].amount
        );
        uint256 weiAmount = invoices[_invoice_id].amount.mul(
            SafeMath.sub(1, fee.div(10000))
        );
        usdt.transfer(invoices[_invoice_id].seller, weiAmount);
        usdt.transfer(treasury, invoices[_invoice_id].amount.sub(weiAmount));
        invoices[_invoice_id].state = 1;
        emit paid(_invoice_id);
    }
    ...
    function set_subscribersAddr(address _subscribersAddr)
        external
        onlyOwner
        onlyContract(_subscribersAddr)
    {
