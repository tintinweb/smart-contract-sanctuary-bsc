...
// File: CIC/Contracts/Token-V8.sol
...
contract masterChef {
    ...
    event OperatorAdded(address indexed newMasterChef);
    event OperatorRemoved(address indexed oldMasterChef);
    constructor()  {
        _addOperator(msg.sender);
    }
    
    modifier onlyOp() {
        require(isMasterChef[msg.sender], "Operators: caller is not an Operator, or MasterChef");
        _;
    }

    function addMasterChef(address newMasterChef) public virtual onlyOp {
        require(!isMasterChef[newMasterChef]);
        _addOperator(newMasterChef);
    }

     function _addOperator(address newOperator) internal virtual {
        isMasterChef[newOperator] = true;
        emit OperatorAdded(newOperator);
    }
    
    function removemasterChef(address oldMasterChef) public virtual onlyOp {
        require(isMasterChef[oldMasterChef], "Not an Operator");
        isMasterChef[oldMasterChef] = false;
        
        emit OperatorRemoved(oldMasterChef);
    }
}
...
// RanchToken.
contract FertToken is Ownable, masterChef, ERC20('Fertilizer', 'FRTc') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterChef).
    function mint(address _to, uint256 _amount) public onlyOp {
        _mint(_to, _amount);      
    }
    ...
    constructor(address _MarketingWallet, uint256 initialSupply)  {
       sellFarmFee = 500;  // percent * 100
     ...
     function setExtraFeeOnSell(uint256 _newSellFarmFee) public onlyOwner {
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(automatedMarketMakerPairs[to] && sellFarmFee > 0) {
                uint256 extraFee =(amount * sellFarmFee)/10000;
                if (extraFee > 0) super._transfer(from, address(this), extraFee);
               
                if (balanceOf(address(this)) > swapAtAmount) {
