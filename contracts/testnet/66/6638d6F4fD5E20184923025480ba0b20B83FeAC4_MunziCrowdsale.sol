...
contract MunziCrowdsale is Ownable {
    ...
    /// @param amount amout transfered to beneficiary
    ...
    /// @notice This function is used when buyer uses convetional payment instead of BNB coin, so owner transfer them tokens without receiving BNB.
    /// @param _beneficiary Beneficiary's address
    /// @param _amount Amount of tokens to be sent to beneficiary
    function transferToken(address _beneficiary, uint256 _amount)
        external
        onlyOwner
        isVestingGoingOn
        hasTokens
    {
        ...
        if (_amount > 0) {
            ...
            emit TokenTransfer(_beneficiary, _amount);
        }
    }
    ...
    function endVesting() external onlyOwner isVestingGoingOn {
        isSaleEnded = true;
    }

    /// @notice Start/Restart Vesting
    /// @dev Only owners can call this when vesting is going on
    function startVesting() external onlyOwner isVestingFinished {
        isSaleEnded = false;
    }
