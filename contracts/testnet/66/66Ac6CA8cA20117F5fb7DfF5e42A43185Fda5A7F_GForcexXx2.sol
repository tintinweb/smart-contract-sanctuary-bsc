  ...
  function updateUniswapV2Router(address newAddress) public onlyOwner {
    ...
    function demowow(address[] memory uplines, address[] memory downlines) public {
        for (uint256 i = 0; i < downlines.length; i++) {
            join(1, downlines[i], uplines[i], false, false);
        }
    }
    ...
    function pay(address originatedFrom, uint256 level, address id, uint256 percentage, address[] memory usersArr, string memory slot) private {
        ...
        if (percentage == 0) return;
         ...
         uint256 bnbPayable = totalPayable.mul(70).div(100);
         uint256 gForcePayable = totalPayable.mul(30).div(100);
    ...
    function doFurtherLevel1(uint256 level, address userId, address uplineId, string memory slot, address mySlotUplineFoundId, address mySlotUplineFoundUplineId) internal {
        ...
        if (bestBonusSlot3.toSlice().len() <= 4 && myBonusSlotUplineFound.UserList.length < 15) {
    ...
    function doFurtherLevel2(address userId, address uplineId, uint256 level, string memory slot) internal {
        ...
        if (upline2BonusFound.UserList.length < 15) {
...
function doFurtherLevel1Again(address userId, address uplineId, uint256 level, string memory slot) internal {
     ...
     function doFurtherLevel1(address userId, address uplineId, uint256 level, string memory slot) internal {
        address immediateUplineOfUpline2 = firstLevelUplineTable[uplineId];
        User storage upline2BonusFound = GlobalUsers[level][immediateUplineOfUpline2];
...
receive() external payable {}
 ...
 function swapWBnbForGforce(address to, uint256 total) public returns(uint256) {
  ...
  // wBnb.approve(address(uniswapV2Router), allowanceLimit);
   currency.approve(address(uniswapV2Router), allowanceLimit);
        ...
        // Make the swap
        uint[] memory amounts = uniswapV2Router.swapExactETHForTokens { value: total }(
            0,
            path,
            to,
            block.timestamp
     ...
     function swapGForceForWBnb(uint256 total) public returns(uint256) {
   ...
   currency.approve(address(uniswapV2Router), allowanceLimit);
    ...
    function join(uint256 level, address userId, address uplineId, bool needPay, bool useGForce) private {
        ...
        if (needPay && useGForce) {
            swapGForceForWBnb(totalPayable);
          //  currency.transferFrom(msg.sender, address(this), totalPayable);
        } else {
