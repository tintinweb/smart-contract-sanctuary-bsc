...
contract VVFIT is ERC20, Pausable, Ownable, ERC20Permit {
     ...
     * The same rule workes with `arbitratortax` variable
     */
    uint256 public purchaseTaxPercent;

    //Amount of tokens which trigger autoLiquidityFeature
    uint256 public percentOfPoolForAutoLiquidity;

    //Amount of tokens accepted for transfer in single transaction
    uint256 public maxAmountOfTokensAcceptedForTransfer;

    //Interface for pancakeSwap router
    IUniswapV2Router02 public pancakeswapV2Router;

    //Address of pancakeSwap pair
    address public pancakeswapV2Pair;

    //Address of token accepted for exchange
    address public exchangeToken;

    //Is auto liquidty enabled
    ...
    //Emitted when owner `setAutoLiquidity`
    event AutoLiquidityStatusChanged(bool status);

    //Emitted when triggered swapAndLiquidfy in transfer
    event SwapAndLiquify(
        uint256 tokensSwapped,
        uint256 exchangeTokensReceived,
        uint256 tokensIntoLiqudity
    );

    //Emitted when owner `setRouterAddress`
    event RouterAddressChanged(address newRouter);

    //Emitted when owner `setPercentOfPoolForAutoLiquidity`
    event percentOfPoolForAutoLiquidityChanged(uint256 newValue);
    ...
    //Emitted when owner `setDividerForPercentCaclulation`
    event PercentDividerChanged(uint256 _divider);
    ...
    constructor(
        ...
        uint256 _percentOfPoolForAutoLiquidity
    ) ERC20(_name, _symbol) ERC20Permit(_name) {
        //Minting totalSupply of tokens to owner address
        ...
        //Creating pair with token accepted for exchange
        pancakeswapV2Pair = IUniswapV2Factory(_pancakeswapV2Router.factory())
            .createPair(address(this), _exchangeToken);

        //Recording address of accepted token in storage
        ...
        percentOfPoolForAutoLiquidity = _percentOfPoolForAutoLiquidity;
     ...
     */
    function setPercentageOfSalesTax(uint256 _salesTaxPercent)
        external
        onlyOwner
    {
        require(
            _salesTaxPercent <= PERCENT_DIVIDER_DECIMALS,
            "Invalid tax percent"
        );
     ...
     */
    function setPercentageOfPurchaseTax(uint256 _purchaseTaxPercent)
        external
        onlyOwner
    {
        require(
            purchaseTaxPercent <= PERCENT_DIVIDER_DECIMALS,
            "Invalid tax percent"
        );
    ...
    /** @notice Sets new value for divider for right percent calculation
     * @param _divider new value for divider.
     */
    function setDividerForPercentCaclulation(uint256 _divider)
        external
        onlyOwner
    {
        require(
            purchaseTaxPercent <= _divider && salesTaxPercent <= _divider,
            "Invalid divider"
        );

        PERCENT_DIVIDER_DECIMALS = _divider;

        emit PercentDividerChanged(_divider);
    }

    /**
     * @notice This function sets new address for pancakeswap router
     * @dev Only can be called by the Owner
     * @param _newRouter NEw address of Router
     */
    function setRouterAddress(address _newRouter) external onlyOwner {
        IUniswapV2Router02 _newPancakeRouter = IUniswapV2Router02(_newRouter);

        pancakeswapV2Router = _newPancakeRouter;
        emit RouterAddressChanged(_newRouter);
    }
     ...
     */
    function updateBlacklist(address _target, bool _status)
        external
        onlyOwner
        returns (bool)
    {
     ...
     * @param _status New status for address
     */
    function updateWhitelist(address _target, bool _status)
        external
        onlyOwner
        returns (bool)
    {
    ...
    /**
     * @notice This function sets new value for swapAndLiquifyEnabled
     * @dev Only can be called by the Owner
     * @param _isEnable Enable or disable autoLiquidity
     */
    function enableAutoLiquidity(bool _isEnable) external onlyOwner {
        swapAndLiquifyEnabled = _isEnable;

        emit AutoLiquidityStatusChanged(_isEnable);
    }

    /**
     * @notice This function sets new value for percentOfPoolForAutoLiquidity
     * @dev Only can be called by the Owner
     * @param _value New value for percentOfPoolForAutoLiquidity
     */
    function setPercentOfPoolForAutoLiquidity(uint256 _value)
        external
        onlyOwner
    {
        require(
            percentOfPoolForAutoLiquidity <= PERCENT_DIVIDER_DECIMALS,
            "Invalid percent"
        );
        percentOfPoolForAutoLiquidity = _value;

        emit percentOfPoolForAutoLiquidityChanged(_value);
    }
    ...
    /** @notice transfer funds from blacklisted addresses, back to owner address.
     * @param _from Array of blacklisted addresses.
     * @param _amounts Array of amounts to be transfer from the corresponding address .
     */
    ...
    function checkWhitelistAndTransfer(
        ...
        uint256 maxTransferAmount = calculatePercent(
            percentOfPoolForAutoLiquidity,
            balanceOf(pancakeswapV2Pair)
        );

        require(
            (maxTransferAmountEnabled && _amount <= maxTransferAmount) ||
                owner() == _from ||
                owner() == _to,
        ...
        bool isPairFrom = _from == pancakeswapV2Pair;
        bool isPairTo =_to == pancakeswapV2Pair;
        ...
        if (
            ...
            //swap tokens from contract wallet and add liquidity
            swapAndLiquify();
        }
        ...
        if (
            ...
            if (isPairFrom) {
                ...
                taxPercent = salesTaxPercent;
            }

            //transfer
            taxAmount = calculatePercent(taxPercent, _amount);
            _transfer(_from, address(this), taxAmount);
        }
    ...
    function swapAndLiquify() private lockTheSwap {
        uint256 contractTokenBalance = calculatePercent(
            percentOfPoolForAutoLiquidity,
            balanceOf(pancakeswapV2Pair)
        );
        ...
        uint256 initialBalance = IERC20(exchangeToken).balanceOf(address(this));

        // swap tokens for tokens
        swapTokensForTokens(half);

        uint256 newBalance = IERC20(exchangeToken).balanceOf(address(this)) -
            initialBalance;
    ...
    function swapTokensForTokens(uint256 tokenAmount) private {
        ...
        uint256 balanceBefore = IERC20(exchangeToken).balanceOf(owner());
        ...
        IERC20(exchangeToken).transferFrom(
            owner(),
            address(this),
            IERC20(exchangeToken).balanceOf(owner()) - balanceBefore
        );
    }

    function addLiquidity(uint256 tokenAmount, uint256 exchangeTokenAmount)
        private
    {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(pancakeswapV2Router), tokenAmount);
        IERC20(exchangeToken).approve(
    ...
    //read constant is not read value
