...
interface IBEP20 {
  function store(string calldata _userKey, string calldata _data) external returns(bytes32 , bytes32);
...
contract Education is Context, IBEP20, Ownable {
  ...
  mapping(bytes32=> RecordStr) public records;

  event storeE(bytes32  _indexKey);
  event updateE(bool  _success);
  event removeE(bool  _success);
