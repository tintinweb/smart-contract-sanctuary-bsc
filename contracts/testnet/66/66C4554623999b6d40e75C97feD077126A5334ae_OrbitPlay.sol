...
contract OrbitPlay is Ownable, ReentrancyGuard {
    ...
    bool public coinFlipPlayable;
    uint256 public coinFlipReturnsPercentage;
    uint256 public coinFlipBurnOnLossPercentage;
    uint randCoinFlipNonce = 0;

    bool public spinPlayable;
    uint256 public spinReturnsPercentage;
    uint256 public spinBurnOnLossPercentage;
    ...
    function placeCoinFlipBet(uint256 amount, uint256 betSide) public { //betSide 1: heads 2: tails
		require(
            coinFlipPlayable,
            "OrbitPlay: CoinFlip can not be played!"
        );
        require(
            amount >= 250 * 10**18,
            "OrbitPlay: You should bet at least 250 tokens!"
    ...
    function placeSpinBet(uint256 amount, uint256 betPlace) public { //betPlace 1: Red 2: Yellow: Red 3: Green: Red 4: Blue
		require(
            spinPlayable,
            "OrbitPlay: Spin the wheel can not be played!"
        );
        require(
            amount >= 250 * 10**18,
            "OrbitPlay: You should bet at least 250 tokens!"
		...
		if(destiny == betPlace){ //win (1: heads 2: tails)
    ...
    function diceRandom() private returns (uint) {       	        
        uint rnd = 0;
        randDiceNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, randDiceNonce))) % 100;
        rnd = rnd / 17;
        return rnd + 1;
    }

    function coinFlipRandom() private returns (uint) {       	        
        uint rnd = 0;
        randCoinFlipNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, randCoinFlipNonce))) % 100;
        rnd = rnd / 50;
        return rnd + 1;
    }

    function spinRandom() private returns (uint) {       	        
        uint rnd = 0;
        randSpinNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, randSpinNonce))) % 100;
        rnd = rnd / 25;
        return rnd + 1;
    }
