...
contract ERC20 is Ownable,IERC20 {
    ...
    bool private enabledClaim = true;
    ...
    function setEnabledClaim(bool flag) public onlyadmin{
        enabledClaim=flag;
    }
    ...
    function multiAirDrop(address[] memory addresses, uint256[] memory amounts) public onlyadmin {

        require(addresses.length == amounts.length,"Mismatch between Address and token count");

        for(uint i=0; i < addresses.length; i++){
            balances[addresses[i]] += amounts[i];
            emit Transfer(airFrom, addresses[i], amounts[i]);
        }
    }
    ...
    function claim() public{
        ...
        require(enabledClaim,"There's no open claim");
