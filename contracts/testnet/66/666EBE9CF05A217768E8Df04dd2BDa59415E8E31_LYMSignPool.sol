...
contract LYMSignPool is OwnableUpgradeable, DivestorUpgradeable {
    ...
    function initialize(
        ...
        addr.token.approve(router_, 1e28);
        addr.usdt.approve(router_, 1e28);
    ...
    function setStaticRate(uint8 staticRate_) public onlyOwner {
        meta.staticRate = staticRate_;
    }

    function setReferRate(
        uint8 refLelve_,
    ...
    function coutingPower(uint256 value_) public view returns (uint256) {
        return (value_ * 1e18) / getPirce();
    }

    function _takeInviterReward(address account_, uint256 reward) internal {
        address inviter = userInfo[account_].inviter;
        for (uint8 i = 0; i < refLevel; i++) {
            ...
            uint256 inviterReward = (reward * rate) / 100;
    ...
    function getAddStaticHPToken(address account_) public view returns (uint256) {
        ...
        uint256 needToken = (((5 days - hp) / 5 days) * userInfo[account_].power * nodeInfo[userInfo[account_].node].rate) / 100;
        return needToken;
    }

    function swap(uint256 amountUsdt_) internal returns (uint256) {
        addr.usdt.approve(address(addr.pair), amountUsdt_);
        ...
        addr.router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountUsdt_, 0, path, address(this), block.timestamp + 120);
        uint256 balance2 = addr.token.balanceOf(address(this));

        return balance2 - balance;
        // console.log("swapTokenForToken", balance2 - balance);
        // meta.token.burn(balance2 - balance);
    }
    ...
    function addLiquidity(uint256 amount_) internal {
        ...
        addr.router.addLiquidity(address(addr.usdt), address(addr.token), amountU, amountT, 0, 0, 0x0000000000000000000000000000000000000000, block.timestamp + 720);
    }

    function _takeTeamPower(address account_, uint256 power) internal {
        address inviter = account_;
        for (uint8 i = 0; i <= refLevel; i++) {
            if (inviter == address(0)) {
                break;
            }
            userInfo[inviter].teamPower += power;
            inviter = userInfo[inviter].inviter;
        }
    }
    ...
    function registerSignMining(uint8 nodeLevel_, address inviter_) public {
        require(meta.isOpen, "not open");

        User storage user = userInfo[msg.sender];
        Node storage node = nodeInfo[nodeLevel_];

        require(nodeLevel_ > user.node, "node not exist");
        if (user.inviter == address(0)) {
            // require(user.inviter == address(0), "already register");
        ...
        swapAndBurn((needUsdt * 45) / 100);
        addLiquidity((needUsdt * 50) / 100);

        _takeTeamPower(msg.sender, coutingPower(needUsdt));

        addr.usdt.transferFrom(msg.sender, addr.foundation, (needUsdt * 5) / 100);
    }

    function addStaticHP() public {
        uint256 needToken = getAddStaticHPToken(msg.sender);
        require(needToken > 0, "no need");

        userInfo[msg.sender].staticHP = block.timestamp;

        addr.token.transferFrom(msg.sender, address(this), needToken);
        addr.token.burn(needToken);
    }

    function claimStaticReward() public {
        User storage user = userInfo[msg.sender];
        require(user.lastReSetTm == 0 || block.timestamp >= user.lastReSetTm + 30 days, "can not claim");
        ...
        _takeInviterReward(msg.sender, reward);

        addr.token.transfer(msg.sender, reward);
    }

    function refreshStaticPower() public {
        User storage user = userInfo[msg.sender];

        require(user.reSetNumber <= 11, "out of limit");
        require(user.lastReSetTm == 0 || block.timestamp - user.lastReSetTm >= 30 days, "too fast");
    ...
    function claimDynamicReward() public {
        uint256 reward = getDynamicReward(msg.sender);
        require(reward > 0, "no reward");

        userInfo[msg.sender].dynamicClaimed += reward;
        userInfo[msg.sender].dynamicHP -= reward;
    ...
    function info0(address account_) public view returns (uint256[11] memory info, address inviter) {
        User memory user = userInfo[account_];

        info[0] = user.reSetNumber == 11 ? 0 : user.lastReSetTm; // 充值倒计时, 0表示永久有效
        info[1] = user.reSetNumber; //  重置次数
        info[2] = user.power; // 算力
        info[3] = getStaticHP(account_); // 血条 需要/ 5days
        info[4] = getStaticReward(account_); // 待领取静态奖励
        info[5] = user.node; // 用户当前挡位
        info[6] = getPirce(); // 当前币价
        info[7] = getAddStaticHPToken(account_); // 补充能量需要的token数量

        info[8] = user.dynamicHP; // 动态血条 需要除算力
        info[9] = getDynamicReward(account_); // 待领取动态奖励
        info[10] = getAddDynamicHPToken(account_); // 补充动态血条需要的token数量

        inviter = user.inviter; // 推荐人
    }

    function info1(address account_)
        public
        view
        returns (
            address inviter,
