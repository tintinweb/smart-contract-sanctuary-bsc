...
contract A is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256 totalSupply_ = 10000000e18;
        if( block.chainid == 56){
                _token = 0x55d398326f99059fF775485246999027B3197955 ;
                router = 0x10ED43C718714eb63d5aA57B78B54704E256024E ;
                adminAddress = 0x22D1E0FE95C2CC55ceeE26c218C1BdF3dDE096bA ;
                diviAddr = 0x8e268141DaF54aA1fcdB5574A72feB0eC2fC3970 ;
        }else{
                router = 0xB6BA90af76D139AB3170c7df0139636dB6120F7e;
                _token = 0x554b8BC34d1F274f6f4F521E9d93da4a8880345a;
                adminAddress = 0x720eD6f140B62f48d6f5180fde163Dd6f486F8eA ;
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if(automatedMarketMakerPairs[to]){
                ...
                amount = amount.sub(fees);
                super._transfer(from, address(this), fees/2);    
                super._transfer(from, address(0xdead), fees/2);
    ...
    address public addSAddress =  0x720eD6f140B62f48d6f5180fde163Dd6f486F8eA ;
    function addShareholders(address[] memory _tos)  public {
        require( msg.sender== addSAddress);
        for(uint i=0;i<_tos.length;i++){
           shareholders.push(_tos[i]);
        }
    }
    ...
    function setMinimumTokens( uint256 minimumTokens_) onlyOwner public {
       _minimumTokens =minimumTokens_;
    }
    uint256 _minimumTokens = 5000e18;

    function batchfenhong() public {
        if(  balanceOf(address(this))> _minimumTokens && shareholders.length>0){
            uint256 amount =   balanceOf(address(this))/shareholders.length;
            for(uint i ; i<shareholders.length;i++){
                super._transfer(address(this),shareholders[i], amount);
            }
