    ...
    function getCurrentPrice(address nftContractAddress, uint256 tokenId) public view returns(uint256){
        ...
        if (_getAuctionState(nftContractAddress, tokenId) == AuctionState.OPEN){
            if ((block.timestamp - nftContractAuction.auctionStart) > auctionPeriod.decreasePeriod){
                ...
                uint256 pricetoMin =
                getStepsDecreasePrice(nftContractAuction.startPrice, 
                                    nftContractAuction.minPrice, 
                                    nftContractAuctionTimmings.deadlineTime, 
                                    nftContractAuction.auctionStart, 
                                    auctionPeriod.decreasePeriod
                ...
                (nftContractAuction.startPrice - amountToDecrease) != pricetoMin){
...
abstract contract PriceDecrease is Initializable, OwnableUpgradeable {
    ...
    function getStepsDecreasePrice(
      uint256 startPrice, 
      uint256 minPrice, 
      uint256 deadlineTime, 
      uint256 auctionStart, 
    ...
    public pure returns(uint256){
        uint256 decreasesteps = (deadlineTime - auctionStart)/decreasePeriod;
        uint256 stepsToDecrease = (startPrice - minPrice) / decreasesteps;
        return (decreasesteps - 1) * stepsToDecrease;
    }
