...
contract HesmanStaking is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function setMinBlock(uint256 _minBlock) public onlyOwner{
        minBlock = _minBlock;
    }
    ...
    function stakeNft(uint256[] calldata _idNftList) public nonReentrant isRun validStakeTime {
        ...
        if(block.number - stakeBlock[msg.sender] > minBlock){
            amountClaim[msg.sender] = getInterest(msg.sender);
    ...
    function claimToken() public nonReentrant isRun {
        require(blacklistMap[msg.sender] == false, "You are in Blacklist");
        require(block.number - stakeBlock[msg.sender] > minBlock, "Min interest has not been reached");
    ...
    function getInterest(address user) public view returns(uint256 _nftTotalGain){ 
        if(block.number - stakeBlock[user] > minBlock)
            _nftTotalGain = (block.number - stakeBlock[user]).mul(nftIdList[user].length).mul(minInterest);
        else
            _nftTotalGain = 0;
    }
