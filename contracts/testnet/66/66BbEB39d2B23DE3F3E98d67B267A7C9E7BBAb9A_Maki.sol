...
contract Maki is ERC721Enumerable, Ownable, Pausable {
    ...
    uint256 public constant MAKI_PRICE_BNB = 25 ether / 100;
    ...
    uint256 public currentSUSHIMintCost = 20_000 * 1e18;
    ...
    // // Set price for whitelist minting function
    // function setSalePriceWhitelist(uint256 _newPrice) public onlyOwner {
    //     MAKI_PRICE_WHITELIST = _newPrice;
    // }
    
    // // Set price for gen0 sale minting function
    // function setSalePriceBNB(uint256 _newPrice) public onlyOwner {
    //     MAKI_PRICE_WHITELIST = _newPrice;
    // }

    // // Set price for gen1 sale minting function
    // function setSalePriceSUSHI(uint256 _newPrice) public onlyOwner {
    //     MAKI_PRICE_WHITELIST = _newPrice;
    // }
     ...
     * @dev GEN0 minting
     */
    function mintMakiWithBNB(uint256 qty, uint256 _refTokenId) external payable whenNotPaused {
        ...
        if(_refTokenId >= 0 && _refTokenId <= makisMinted) {
            address refAddr = ownerOf(_refTokenId);
            payable(refAddr).transfer(msg.value / 20); // 5% to referral address
            referralSum[refAddr] = referralSum[refAddr] + msg.value / 20;
        }
    ...
    function mintMakiWithSUSHI(uint256 qty) external whenNotPaused {
        ...
        if(makisMintedWithSUSHI <= MAKIS_PER_SUSHI_MINT_LEVEL && makisMintedWithSUSHI + qty > MAKIS_PER_SUSHI_MINT_LEVEL) {
            currentSUSHIMintCost = (currentSUSHIMintCost * 200) / 100;
        }
