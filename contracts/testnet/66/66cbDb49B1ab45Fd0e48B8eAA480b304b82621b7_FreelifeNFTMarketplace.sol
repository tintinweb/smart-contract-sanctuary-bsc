...
contract FreelifeNFTMarketplace is
    Initializable,
    OwnableUpgradeable,
    ...
    ERC1155HolderUpgradeable
{
    ...
    CountersUpgradeable.Counter private itemsSelling;

    IERC20Upgradeable private currencyContract;
    ...
    struct Item {
        ...
        CountersUpgradeable.Counter ownerInfoCount;
        uint256 price;
        uint256 amount;
        bool isSoldOut;
        bool isUnlisted;
    ...
    function initialize(address _nftAddress, address _currencyAddress, uint256 _listingFee)
        public
        initializer
    {
        uint256 listingFee = _listingFee.mul(100).div(FEE_DENOMINATOR);
        require(address(_currencyAddress) != address(0), "Address must not be zero");
        ...
        currencyContract = IERC20Upgradeable(_currencyAddress);
    ...
    function sell(
        ...
        // Transfer NFT
        IERC1155Burnable(nftContract).safeTransferFrom(
            msg.sender,
            address(this),
            tokenId,
            amount,
    ...
    function buy(uint256 itemId, uint256 amount)
        external
        nonReentrant
    {
        ...
        require(amount > 0, "Amount must be more than 0");
        require(
            idToItem[itemId].amount >= amount,
            "Insufficient item amount"
        );
        require(idToItem[itemId].isSoldOut != true, "This item is sold");
        require(
            idToItem[itemId].isUnlisted != true,
            "This item is unlisted"
        );

        uint256 cost = price.mul(amount);
        require(currencyContract.balanceOf(msg.sender) >= cost, "Insufficient balance");

        // Transfer currency to seller
        currencyContract.safeTransferFrom(
            msg.sender,
            idToItem[itemId].seller,
        ...
        idToItem[itemId].ownerInfoCount.increment();
        idToItem[itemId].ownerInfo[
            idToItem[itemId].ownerInfoCount.current().sub(1)
        ] = ownerInfo;

        // Transfer fee to contract owner
        currencyContract.safeTransferFrom(msg.sender, owner(), calculatedFee);

        idToItem[itemId].amount = idToItem[itemId].amount.sub(
            amount
        );
        bool sold = idToItem[itemId].amount == 0;
        if (sold) {
            idToItem[itemId].isSoldOut = true;
    ...
    function unlist(uint256 itemId) external nonReentrant {
        ...
        require(
            msg.sender == address(idToItem[itemId].seller),
            "You are not seller of this item"
        );

        uint256 remainingAmount = idToItem[itemId].amount;
    ...
    {
        ItemView memory itemView = ItemView({
            itemId: idToItem[itemId].itemId,
            tokenId: idToItem[itemId].tokenId,
            seller: idToItem[itemId].seller,
            price: idToItem[itemId].price,
            amount: idToItem[itemId].amount,
            isSoldOut: idToItem[itemId].isSoldOut,
            isUnlisted: idToItem[itemId].isUnlisted
        });
        return itemView;
    }

    function getOwnerInfo(uint256 itemId)
        public
        view
        returns (OwnerInfo[] memory)
    {
        require(itemId <= itemIds.current(), "Item ID not found");
        uint256 ownerInfoCount = idToItem[itemId]
            .ownerInfoCount
            .current();
        OwnerInfo[] memory ownerInfo = new OwnerInfo[](ownerInfoCount);
        for (uint256 i = 0; i < ownerInfoCount; i++) {
            ownerInfo[i] = idToItem[itemId].ownerInfo[i];
        }
        return ownerInfo;
    }
}
