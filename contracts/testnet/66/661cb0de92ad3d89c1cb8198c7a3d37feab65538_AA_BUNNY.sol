...
contract AA_BUNNY is Context, IERC20 { 
    ...
    */
    ...
    The distribution of transactions over buys, sells and transfers is tracked, so that it is allocated correctly when the fees are processed. 
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // Using the 'sell fees' to determine fee split distribution
        uint256 LP_Tokens = contractTokenBalance * _FeeSellLiquidity / _Fee_on_Sells / 2;
        ...
        // Double fees instead of halving LP to increase accuracy and reduce roudning errors if LP fee is an odd number
        uint256 fee_Split = _Fee_on_Sells * 2 - _FeeSellLiquidity;
        uint256 BNB_L = returned_BNB * _FeeSellLiquidity / fee_Split;
        uint256 BNB_D = returned_BNB * _FeeSellDevelopment * 2 / fee_Split;
        uint256 BNB_S = returned_BNB * 2 / fee_Split; // 1% fee to solidity developer
        ...
        // Send solidity developer BNB
        if (BNB_S > 0){
        sendToWallet(Wallet_SolidityDev, BNB_S);}


        // Send development and charity BNB
        if (BNB_D > 0){
