...
contract balahap {
    ...
    address public BUSD; 
    uint256 public period  = 120 ; //2mins
    ...
    function areAllBidderClaimed(uint256 _offerNo) public view returns (bool){
        ...
        for(uint256 i = 0 ; i < _num; i++){
            if(!biddersList[_offerNo][_num].isBidderClaimed){
                _allClaimed = false;
                break;
            }
        }
    ...
    function claimPrize(uint256 _offerNo) external {
        ...
        /* No dispute claim yet*/
            ...
            if(biddersList[_offerNo][bidderNum(msg.sender,_offerNo)].bidderResult == result.WIN){
                /* Bidder Win */
