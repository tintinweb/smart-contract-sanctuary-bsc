...
contract Kakoli is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100 * 10**2;  // Total supply * Decimals
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _totalBal;

    string private _name = "Kakoli" ;
    string private _symbol = "KK";
    uint8 private _decimals = 2 ;
    
    uint256 public _taxFee ;      // Total Transfer Amount / %2 auto holder rewards
    uint256 private _previousTaxFee = _taxFee;     
    
    uint256 public _liquidityFee ; // Total Transfer Amount / %2 auto add liquidity
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _marketingFee ; // Total Transfer Amount / %2 for devolopment
    ...
    uint256 public ownMaxRate ; // max owned amount %5
    
    // stops all transfers in a possible attack
    bool public isHack = false;
   

    uint256 private numTokensSellToAddToLiquidity = 100 * 10**2;
    ...
    constructor (uint256 TaxFee, uint256 LPFee, uint256 MarketingFee, uint256 MaxRate, address RouterAdrss, address TeamW, address CompanyW, address MarketingW, address BurnW, address AdvisorW, address LikidityW) {
        _marketingFee = MarketingFee;
        _liquidityFee = LPFee;
        _taxFee = TaxFee ;
        ownMaxRate = MaxRate;
        routerAddress = RouterAdrss;
        teamWallet = TeamW ;
        companyWallet = CompanyW;
        marketingWallet = MarketingW;
        burnWallet = BurnW;
        advisorWallet = AdvisorW;
        likidityWallet = LikidityW;
        ...
        _isExcludedFromFee[likidityWallet] = true;
