...
contract Monster is Ownable  {
    ...
    function fighting(uint256 tkId,uint256 enemyId,address addr) public  view isFullHp(tkId) returns(bool,uint256,uint256,uint256,uint256){
        ...
        if (rand(100)>=_figInfo.totalSuc){
            // 修改血量 basicHp
            _carDetail.hp = 0;
            _carDetail.unLockTime = block.timestamp + 600;
    ...
    function initEney() internal{
        if(enemys.length ==0 ){
            ...
            addEnemy(65,3*10**16,20,200*10**8,"Horace","");
            addEnemy(60,4*10**16,20,200*10**8,"Rufio","");
            addEnemy(55,5*10**16,20,200*10**8,"Hadrea","");
            addEnemy(50,6*10**16,20,200*10**8,"Sirius","");
        }
    }
...
contract Game is AccessControl,Ownable {
    ...
    event Fighting(bool isSuccess,uint256 indexed fightType,uint256 indexed sHp,uint256  addXp,uint256 indexed reward,address sender);
    ...
    function fighting(uint256 tokenId,uint256 enemyId) public isUnlock(tokenId) isteam(tokenId,msg.sender) returns(FightingEndInfo memory fig){
        ...
        if(fig.suc == true){
            ...
            _tokenDetail[tokenId].rgTime = block.timestamp + 600;
        }
        fig.fgType = 1;
        emit Fighting(fig.suc,1,fig.hp,fig.xp,fig.reward,msg.sender);
    ...
    function DoTask(uint256 tokenId,uint256 enemyId) public isUnlock(tokenId) isteam(tokenId,msg.sender) returns(FightingEndInfo memory fig) {
        ...
        fig.fgType = 2;
        emit Fighting(fig.suc,2,fig.hp,0,fig.reward,msg.sender);
        return fig;
    }
