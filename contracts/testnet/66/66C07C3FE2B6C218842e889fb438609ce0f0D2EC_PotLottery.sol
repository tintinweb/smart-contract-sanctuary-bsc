...
contract PotLottery is Initializable, ReentrancyGuardUpgradeable {
    /*
    ...
    modifier validBNBP() {
        require(BNBP_Address != address(0), 'BNBP Address invalid');
        _;
    }
    ...
    /**   @dev returns the contract balance of a particular token
        ...
        */
    function getContractBalance(string memory _tokenName) public view returns (uint256 balance){
    ...
    /**   @dev attempt to transfer token from user address to contract address
        ...
        */
    function attemptTransferFrom(string memory _tokenName, address _userAddress, uint256 _amount) public returns (bool success){
        IPRC20 token = IPRC20(tokenWhiteList[_tokenName].tokenAddress);
        success = token.transferFrom(_userAddress, address(this), _amount) ;
    }
    ...
    */
    ...
    function EnterPot(
        ...
        for (uint256 index = 0; index < _tokenNames.length; index++) {
            ...
            if( getContractBalance(_tokenNames[index]) <= ( _amounts[index] + adminFeeTokenValues[_tokenNames[index]]) + tokenTotalEntry[_tokenNames[index]] ){
        ...
        if (participantCount == 0) {
            ...
            for (uint256 index = 0; index < tokenWhiteListNames.length; index++) {
                IUniswapV2Pair pair = IUniswapV2Pair(
                    //tokenWhiteList[_tokenName].tokenAddress
    ...
    */
    ...
    /**   @dev refund token to an address
          @notice should be removed on launch on bsc
    */
    ...
    */
    ...
    function burnAccumulatedBNBP() external validBNBP {
        ...
        require(BNBP_Balance > 0, 'No BNBP balance');
        require(burnPool > 0, 'No burn amount');
        require(burnPool <= BNBP_Balance, 'Wrong BNBP Fee Value');
    ...
    function lotteryAccumulatedBNBP() external validBNBP returns (address) {
        ...
        require(lotteryPool <= BNBP_Balance, 'Wrong BNBP Fee Value');
