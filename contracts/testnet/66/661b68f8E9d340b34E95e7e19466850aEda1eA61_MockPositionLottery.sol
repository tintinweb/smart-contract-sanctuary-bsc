    ...
    PositionLotteryStorage
{
        ...
        returns (TicketNumberOfUser[] memory, uint256 )
    {
        ...
        for (uint i = 0; i < _ticketsOfUserWithDistinct.length; i++) {
            uint32 _ticketNumber = _ticketsOfUserWithDistinct[i];
            (, bool _hasPrize) = getPrizeOfTicket(
                _ticketNumber,
                _finalNumber
            );
            ticketNumberOfUser[i] = TicketNumberOfUser({
                ticketNumber: _ticketNumber,
                amount: amountTicketsOfUserWithDistinct[lotteryId][user][_ticketNumber],
                    hasPrize :_hasPrize
    ...
    public
    view
    returns (PrizeOfUser[] memory, uint256, uint256){

        (PrizeOfUser[] memory prizes, uint256 totalRewards) = _getPrizeOfUser(user, lotteryId);
        return (prizes, totalRewards, _lotteries[lotteryId].endTime);
    }
