...
contract BRealEstate is Ownable {
    ...
    struct Stock {
        ...
        uint256 maxBuyIdo; //minimun stockIdo
    ...
    uint256 public totalClaimValue;
    ...
    function updateStableCoin(address _stableCoin) external onlyOwner {
        ...
        BREToken = IERC20(_stableCoin);
        emit UpdateStableCoin(_stableCoin, _msgSender(), block.timestamp);
    }
        ...
        isWhitelist
    {
        ...
        require(
            BREToken.balanceOf(_msgSender()) >= _amount,
        ...
        //payment
        BREToken.safeTransferFrom(_msgSender(), address(this), tokenAmount);
    ...
    function voteAcceptOffer(uint256 offerIndex) external {
        ...
        require(stocks[buyerOffer.symbol].balances[_msgSender()] > 0, "Error: No vote permission");
        ...
        buyerOffer.accepted = SafeMath.add(stocks[buyerOffer.symbol].balances[_msgSender()], buyerOffer.accepted);
        if(stocks[buyerOffer.symbol].lockTransfers[_msgSender()] < buyerOffer.end){
            stocks[buyerOffer.symbol].lockTransfers[_msgSender()] = buyerOffer.end;
        }
    ...
    function voteRejectedOffer(uint256 offerIndex) external {
        ...
        buyerOffer.rejected = SafeMath.add(stocks[buyerOffer.symbol].balances[_msgSender()], buyerOffer.rejected);
    ...
    function cancelVoteOffer(uint256 offerIndex) external {
        ...
        if(buyerOffer.voters[_msgSender()] == VoteOption.Rejected){
            buyerOffer.rejected = SafeMath.sub(buyerOffer.rejected, stocks[buyerOffer.symbol].balances[_msgSender()]);
        }else{
            buyerOffer.accepted = SafeMath.sub(buyerOffer.accepted, stocks[buyerOffer.symbol].balances[_msgSender()]);
        }
    ...
    function distributeProject(string memory _symbol) external payable {
        require(
            owner() == _msgSender() || admin[_msgSender()],
            "Error: Not owner or admin"
        );
        require(_projects[_symbol], "Error: Project not exist");
        require(
        ...
        if (projects[_symbol].isSold) {
            projects[_symbol].totalClaimValue = buyerOffers[
                projects[_symbol].acceptOffer
            ].offerValue;
        } else {
        ...
        payable
    {
        ...
        require(
            projects[_symbol].isDistributed,
            "Error: project is not distribute"
        );
        if(_stockAmount == 0){
            _stockAmount = stocks[_symbol].balances[_msgSender()];
        }
        require(
            stocks[_symbol].balances[_msgSender()] >= _stockAmount && _stockAmount > 0,
        ...
        totalClaimed = SafeMath.add(totalClaimed, tokenAmount);
    ...
    function exchangeStock(
        ...
        require(stocks[_symbol].lockTransfers[_seller] < block.timestamp || projects[_symbol].isSold, "Error: Can't transfer. Your stocks was locked");
