...
contract MarketPlace is  AccessControlEnumerable, Allowances {
    ...
    uint64 private _maxUserAutionDays=365;
   ...
   //Max days of Auction
    ...
    function _updateUserAuction(
        uint256  onionId,
        uint256 pricePerCard,
        uint256 duration
        ) external  {
        require(_marketPlace[onionId].isOnAuction(), "Onion:The current card is invalid");
        address owner = _marketPlace[onionId].ownerAuction();
        require(owner == msg.sender, "Auction: Not owner of the auction");
        require(block.timestamp>=_marketPlace[onionId].deadlineOfAuction(),"Auction: Not Expired Yet");
        require(duration<= UserAuctionDaysLimitsDetails(),"UserAuction: Duration limit exceed form maxLimit");
        uint256 deadline = block.timestamp + (duration * 1 days); // you can set any of them " hours , minutes , weeks , months , days "
        _marketPlace[onionId].updateSet(
            pricePerCard,
            duration,
            deadline
        );
    ...
    */
    ...
    ) external 
    {
        ...
        for (uint256 index = 0; index < cardIds.length; ++index) {
            require(duration[index]<= UserAuctionDaysLimitsDetails(),"UserAuction: Duration limit exceed form maxLimit");
            _createUserAuction(
                owner,
                cardIds[index],
                currency,
    ...
    */
    ...
    function validAuctionDetails(uint256 onionId) public view returns(address owner, bytes32 currency,uint256 NFTid, uint256 quantity, uint256 price, uint256 duration,uint256 deadline) {
    ...
    function expireAuctionDetails(uint256 onionId) public view returns(address owner, bytes32 currency,uint256 NFTid, uint256 quantity, uint256 price, uint256 duration,uint256 deadline) {
        if(block.timestamp>=_marketPlace[onionId].deadlineOfAuction())
        {
        return _marketPlace[onionId].userAuctionDetails();
        }
    ...
    */
    ...
    function batchValidAuctionDetails(uint256[] memory onionIds) public view returns(MarketStorage .cardOnSale[] memory) {
        MarketStorage .cardOnSale[] memory detailsUserAuction = new MarketStorage .cardOnSale[](onionIds.length);
    ...
    function batchExpireAuctionDetails(uint256[] memory onionIds) public view returns(MarketStorage .cardOnSale[] memory) {
        MarketStorage .cardOnSale[] memory detailsUserAuction = new MarketStorage .cardOnSale[](onionIds.length);
