...
contract USDyVault is Initializable, ReentrancyGuard, Pausable, IEvents {
    ...
    modifier _isAdminOrOperator() {
        ...
        console.log(Admin.hasRole(OPERATOR_ROLE, msg.sender),Admin.hasRole(DEFAULT_ADMIN_ROLE, msg.sender));
        require(
    ...
    */
        ...
        initializer
    {
        ...
        withdrawTax = 10;
        rebaseRate = 70;
        epochTime = 10 minutes; // note For testing 8 Hours = 10 seconds
        rebase_Reward = [10, 10, 20, 10, 20];
    }
    ...
    function deposit(
        ...
        uint32 level,
        bool isBUSD
    ) external nonReentrant _lastExchangeRatio whenNotPaused {
        ...
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            ...
            require(!isContract(msg.sender),"USDyVault: No external contract interaction");
        }
    ...
    */
    ...
    function reBaseRewards() external nonReentrant _isAdminOrOperator {
        require(
        ...
        uint256 mintReward = ((IERC20(USDs).totalSupply() * exchangeRatio) *
            rebaseRate) / 10**20;
        ...
        getAPR[Admin.YSLBUSDVault()] = (mintReward * rebase_Reward[0]) / rebaseRate;
        IERC20(Admin.USDy()).safeTransfer(
            Admin.xYSLBUSDVault(),
            (mintReward * rebase_Reward[1]) / rebaseRate
        );
        getAPR[Admin.xYSLBUSDVault()] = (mintReward * rebase_Reward[1]) / rebaseRate;
        IERC20(Admin.USDy()).safeTransfer(
            Admin.USDyBUSDVault(),
            (mintReward * rebase_Reward[2]) / rebaseRate
        );
        getAPR[Admin.USDyBUSDVault()] = (mintReward * rebase_Reward[2]) / rebaseRate;
        IERC20(Admin.USDy()).safeTransfer(
            Admin.BShareBUSDVault(),
            (mintReward * rebase_Reward[3]) / rebaseRate
        );
        getAPR[Admin.BShareBUSDVault()] = (mintReward * rebase_Reward[3]) / rebaseRate;
        // IERC20(Admin.USDy()).safeTransfer(
        //     Admin.YSLVault(),
        ...
        IxYSLBUSDVault(Admin.xYSLBUSDVault()).distribute(
            (mintReward * rebase_Reward[1]) / rebaseRate
        );
        IxYSLBUSDVault(Admin.USDyBUSDVault()).distribute(
            (mintReward * rebase_Reward[2]) / rebaseRate
        );
        IBshareBUSDVault(Admin.BShareBUSDVault()).distribute(
            (mintReward * rebase_Reward[3]) / rebaseRate
        );
        distribute((mintReward * rebase_Reward[4]) / rebaseRate);
        getAPR[address(this)] = (mintReward * rebase_Reward[4]) / rebaseRate;
    ...
    */
    ...
    function rewards(address user) public view returns(uint _reward){
       _reward = ((UserDeposit[user] * (currentReward - ss[user])) / 10 ** 18);
         if(Admin.PhoenixNFT() != address(0) && IReceipt(Admin.PhoenixNFT()).balanceOf(user) >= 1){
        return(_reward + (_reward / 4));
         }
         else{
             return _reward;
         }
    ...
    function distribute(uint256 _rebaseReward) internal {
        if(IERC20(USDs).totalSupply() != 0){
        currentReward = currentReward + ((_rebaseReward * 10 ** 18)/(IERC20(USDs).totalSupply()));
        }
    }
