...
contract DexGoStorage is Ownable {
    ...
    function setApprovedPathOrMovie(uint tokenId, bool isApproved) public {
        require(msg.sender == gameServer || msg.sender == owner() || msg.sender == accountTeam1 || msg.sender == accountTeam2,'SRV');
    ...
    returns (string memory)
    {
        ...
        if (IDexGoRentAndKm(getRentAndKm()).getKmLeavesForId(tokenId) * 100 / getPriceInitialForType(typeNft) < 25) image = getImageForType25PercentKm(typeNft);
        if (IDexGoRentAndKm(getRentAndKm()).getKmLeavesForId(tokenId) * 100 / getPriceInitialForType(typeNft) < 50) image = getImageForType50PercentKm(typeNft);
        if (IDexGoRentAndKm(getRentAndKm()).getKmLeavesForId(tokenId) * 100 / getPriceInitialForType(typeNft) < 75) image = getImageForType75PercentKm(typeNft);
