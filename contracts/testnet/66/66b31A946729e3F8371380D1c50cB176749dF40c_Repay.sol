...
contract Repay is Node, Ownable, SessionManager {
    ...
    function enterStaking(uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ICrssToken(nodes.token).transferDirectSafe(nodes.xToken, msg.sender, pending);
            }
        }
        if (_amount > 0) {
    ...
    function harvestRepay(uint256 amount) public {
        ...
        ICrssToken(nodes.token).transferDirectSafe(nodes.xToken, userAddress, amount);
...
contract CrssToken is Node, Ownable, ICrssToken, SessionRegistrar, SessionFees, SessionManager {
    ...
    function payFeeCrssLogic(
        ...
        if (principal != 0) {
            if (rates.accountant != 0) {
                feesPaid += _payFeeImplementation(payer, principal, rates.accountant, feeStores.accountant, fromAllowance);
            }
            // if (rates.buyback != 0) {
    ...
    function _limitTransferPerSession(
        ...
        if ((sender == owner() || sender == address(this)) && pairs[recipient].token0 != address(0)) {
            ...
            // console.log("Tx amount control: OK", accTransferAmountSession[sender].sent.abs(accTransferAmountSession[sender].received),
