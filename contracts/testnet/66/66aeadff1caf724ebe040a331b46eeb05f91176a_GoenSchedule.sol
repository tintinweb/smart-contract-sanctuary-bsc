...
contract GoenSchedule is IGoenSchedule, OwnableUpgradeable, CoreUtility {
    ...
    function initialize(address token_, address votingEmission_, uint256 rate_, address operator_) public initializer {
        __Ownable_init();
    ...
    function rates(address pool, uint256 timestamp) public override view returns (uint256) {
        uint256 guardedLaunchStart = votingEmission.guardedLaunchStart();
        if (timestamp < guardedLaunchStart) {
            return votingEmission.weightsPerWeek(guardedLaunchStart - System.SYSTEM_INTERVAL, pool).mul(rate).div(1e18);
        }
