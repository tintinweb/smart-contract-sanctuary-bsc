...
contract RamJi is Context, IERC20, Ownable {
    ...
    uint256 public _totalReflection = 0;
    uint256 public _totalMarketing = 0;
    ...
    uint256 public _ReflectionPer = 2;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(sender != pancakePair || recipient == pancakePair) {
            UserLastSellDetails[sender]=block.timestamp;
        }
    }
    ...
    function _takeMarketingFee(uint256 tMarketing) private {
        ...
        uint256 Marketing=rMarketing.mul(40).div(100);
        uint256 Burn=rMarketing.mul(20).div(100);
        uint256 Reflection=rMarketing.mul(40).div(100);
        _rOwned[address(this)] = _rOwned[address(this)].add(Marketing);
        // if(_isExcludedFromReward[address(this)]){
        //     _tOwned[address(this)] = _tOwned[address(this)].add(tMarketing);
        // }
        _totalMarketing+=Marketing;
        _totalBurnt+=Burn;
        _totalReflection+=Reflection;
      //_takeAutoBurn(Burn);
      //_reflection(Reflection);
    }

    function _takeAutoBurn() public onlyOwner {
         _rOwned[address(this)].sub(_totalBurnt);
        _tTotal = _tTotal.sub(_totalBurnt);
    }

    function _reflection() public onlyOwner {
      _rOwned[address(this)].sub(_totalReflection);
