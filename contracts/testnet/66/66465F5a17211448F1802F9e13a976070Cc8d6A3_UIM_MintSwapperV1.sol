...
contract UIM_MintSwapperV1 is Ownable {
    ...
    function getExchangeRateBUSD_UIM(uint256 amount)
        external
        view
        returns (uint256[] memory rates)
    {
        ...
        uint256[] memory _rates = Router.getAmountsOut(amount, path);
        return _rates;
    }

    function getExchangeRateUIM_BUSD(uint256 amount)
        public
        view
        returns (uint256[] memory rates)
    {
