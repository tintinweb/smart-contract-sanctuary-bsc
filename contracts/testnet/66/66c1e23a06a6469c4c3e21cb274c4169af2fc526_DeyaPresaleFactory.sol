...
contract DeyaPresale is Ownable {
    using SafeMath for uint256;
    ...
    function _validatePurchase(address beneficiary, uint256 weiAmount)
        ...
        view
    {
        require(isOpen(), "DeyaPresale: sale is not open");
        require(!hasEnded(), "DeyaPresale: sale is over");
        require(
            weiAmount >= minContribution,
            "DeyaPresale: min contribution criteria not met"
        );
        require(
    ...
    function claimTokens() external {
        require(hasEnded(), "DeyaPresale: presale is not over");
        require(softCapReached(), "DeyaPresale: soft cap not reached, refund is available");
        require(finalized, "DeyaPresale: not yet finalized, wait for liquidity");
        require(
            contributions[msg.sender] > 0,
            "DeyaPresale: nothing to claim"
        );
    ...
    function claimRefund() external {
        require(hasEnded(), "DeyaPresale: presale is not over");
        require(!softCapReached(), "DeyaPresale: soft cap not reached");
        require(
            contributions[msg.sender] > 0,
            "DeyaPresale: nothing to claim"
        );
    ...
    function endPresale() external onlyOwner {
        require(!finalized, "DeyaPresale: presale is already over");
        finalized = true;
    ...
    function withdrawTokens() public onlyOwner {
        require(finalized, "DeyaPresale: presale is not over");
        // uint256 unsoldTokens = _getTokenAmount(hardCap.sub(weiRaised));
...
contract DeyaPresaleFactory is Ownable {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    mapping(IBEP20 => DeyaPresale) public presales;

    event PresaleCreated(address indexed presaleAddress, address indexed token);
    ...
    onlyOwner
    {
        DeyaPresale presale = new DeyaPresale(
            _token,
            _softCap,
            _hardCap,
            _tokensPerBnb,
        ...
        emit PresaleCreated(address(presale), address(_token));
    }
}