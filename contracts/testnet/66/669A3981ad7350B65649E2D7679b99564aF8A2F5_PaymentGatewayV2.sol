...
contract PaymentGatewayV2 is Owner {
  ...
  event PayEvent(bytes32 merchantId, bytes32 sessionId, uint256 amount);
  event SendEvent(bytes32 merchantId, bytes32 sessionId, uint256 amount);
  event BatchSendEvent(string merchantId, string sessionId);
  ...
  function pay(
    ...
    emit PayEvent(merchantId, sessionId, amount);
  }
  ...
  function send(
    ...
    emit SendEvent(merchantId, sessionId, amount);
  }
  ...
  function batchSend(uint256[] calldata amounts, address[] calldata recipients)
    public
  {
    uint256 total = 0;
