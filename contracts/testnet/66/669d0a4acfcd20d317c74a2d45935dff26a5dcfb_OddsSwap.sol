...
contract MarketService is MarketDomain,Sequence,WalletService,ConfigCenter{
 ...
 function _swap(
  ...
  function _draw(
   uint256 poolId,
   MarketPoolDrawDto [] memory drawOptions
 ) internal {
  ...
  function _reward(
   uint256 poolId,
   MarketPoolDrawDto  memory drawOption,
   uint256 count
 ) internal {
...
interface IOddsSwap{
  ...
  function draw(
    ...
    bool [] memory  winArr,
    uint256 [] memory  rewardRatioArr,
    uint256 [] memory  refundRatioArr
  )external;
  function reward(
    uint256 poolId,
    uint256  option,
  ...
  event AddMarketBetOption(uint256 indexed poolId,uint256 option,uint256 initOdds,uint256 betTotalAmount,uint256 [] totalArr,uint256 handicap,uint256 handicapBy);
  event Draw(uint256 indexed poolId,uint256 []  optionArr,bool []   winArr,uint256 []   rewardRatioArr,uint256 []   refundRatioArr);
  event Reward(uint256 indexed poolId,uint256 option,bool win,uint256 rewardRatio,uint256 refundRatio,uint256 count);
...
contract OddsSwap is IOddsSwap,Ownable,MarketService{
  ...
  function addMarketPoolBetOption(
    ...
    MarketBetOptionAddDto memory optionAddDto = _toOptionAddDto(poolId,option,initOdds,betTotalAmount,totalArr,handicap,handicapBy);
    _addMarketOptionEntity(optionAddDto);
    emit AddMarketBetOption(poolId,option,initOdds,betTotalAmount,totalArr,handicap,handicapBy);
  }
  ...
  function draw(
    ...
    bool [] memory  winArr,
    uint256 [] memory  rewardRatioArr,
    uint256 [] memory  refundRatioArr)external override onlyOwner{
    MarketPoolDrawDto [] memory drawDtoArr = _toDrawOptionDtos(optionArr,winArr,rewardRatioArr,refundRatioArr);
    _draw(poolId,drawDtoArr);
    emit Draw(poolId,optionArr,winArr,rewardRatioArr,refundRatioArr);
  }

  function _toDrawOptionDtos(
    uint256 [] memory  optionArr,
    bool [] memory  winArr,
    uint256 [] memory  rewardRatioArr,
    uint256 [] memory  refundRatioArr)
    internal pure returns(MarketPoolDrawDto [] memory drawDtoArr ){
      drawDtoArr = new MarketPoolDrawDto [](optionArr.length);
      for(uint256 i =0;i<optionArr.length; i++){
        drawDtoArr[i] = (MarketPoolDrawDto({
          option:optionArr[i],
          win:winArr[i],
          rewardRatio:rewardRatioArr[i],
          refundRatio:refundRatioArr[i]
        }));
      }
  }

    function _toDrawOptionDto(
    uint256  option, 
    bool  win,
    uint256  rewardRatio,
    uint256  refundRatio)
    internal pure returns(MarketPoolDrawDto memory drawDto){
      drawDto =  MarketPoolDrawDto({
          option:option,
          win:win,
  ...
  function reward(
    ...
    uint256  rewardRatio,
    uint256  refundRatio,
    uint256 count)external override onlyOwner{
      MarketPoolDrawDto memory drawOptionDto = _toDrawOptionDto(option,win,rewardRatio,refundRatio);
    _reward(poolId,drawOptionDto,count);
    emit Reward(poolId,option,win,rewardRatio,refundRatio,count);
  }
