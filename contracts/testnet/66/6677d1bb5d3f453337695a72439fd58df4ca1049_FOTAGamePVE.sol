...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  function finishGame(uint _mission, uint[] calldata _heroIds, uint[] calldata _itemIds, bytes memory _signature, uint _timestamp) external {
    ...
    require(energy >= energyConsumptions[_mission], "PVE: out of energy");
  ...
  function _loadEnergyAndFirstConsumeInDay(uint _mission) private returns (uint, bool) {
    ...
    uint from;
    uint to;
    (energy, from, to,,) = energyManager.energies(msg.sender);
    bool firstConsumeInDay;
    if (energy >= energyConsumptions[_mission] || from == 0 || to < block.timestamp) {
      firstConsumeInDay = energyManager.updateEnergy(msg.sender, energyConsumptions[_mission]);
      if (from == 0 || to < block.timestamp) {
        (energy,,,,) = energyManager.energies(msg.sender);
      }
    }
