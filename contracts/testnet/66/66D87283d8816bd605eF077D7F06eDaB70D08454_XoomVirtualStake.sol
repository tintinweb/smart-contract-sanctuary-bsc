...
contract XoomVirtualStake {
    ...
    struct StakeRecord {
        ...
        address token;
        address rewardToken;
        uint256 minRequiredBalanceToKeep;
        uint checkpointFrequency;
        uint checkpointWaitTime;
        uint lockTime;
        uint active;
    }
    ...
    function getStake(uint stakeID) external view returns (StakeRecord memory result)
    {
    ...
    function getPlayerStakeData(uint stakeID, address player) external view returns (PlayerStakeDataRecord memory result)
    {
        PlayerStakeDataRecord memory record = PlayerStakeDataRecord(
