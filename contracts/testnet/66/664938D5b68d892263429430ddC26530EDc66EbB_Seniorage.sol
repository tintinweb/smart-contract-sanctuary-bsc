...
contract Seniorage is Ownable {
    ...
    uint256 constant private RESERVES_WALLET_PERCENT = 3500;
    ...
    */
        ...
        onlyOwner
    {
        ...
        if (IERC20(btc).allowance(address(this), btcSnacks) == 0) {
            IERC20(btc).approve(btcSnacks, type(uint256).max);
        }
        if (IERC20(eth).allowance(address(this), ethSnacks) == 0) {
            IERC20(eth).approve(ethSnacks, type(uint256).max);
        }
    }
    ...
    */
        ...
        onlyOwner
    {
        ...
        reservesWallet = reservesWallet_;
    ...
    function distributeNonBusdCurrencies() external onlyAuthority {
        for (uint256 i = 0; i < _nonBusdCurrencies.length(); i++) {
            ...
            if (balance != 0) {
                ...
                IERC20(currency).safeTransfer(
                    reservesWallet,
                    balance * RESERVES_WALLET_PERCENT / BASE_PERCENT
                );
                IERC20(currency).safeTransfer(
                ...
                if (IERC20(currency).balanceOf(address(this)) != 0) {
                    uint256 remainingBalance = IERC20(currency).balanceOf(address(this));
                    IERC20(currency).safeTransfer(devWallet, remainingBalance);
                }
            }
        }
    }
    ...
    * токенов к получению после обмена 7.5% от общего баланса BUSD токенов
    * @param zoinksSwapSlippagePercent_ Процент проскальзывания при обмене BUSD токенов
    * на ZOINKS токены. Чем выше процент, тем меньше проскальзывание
    ...
    * ZOINKS и BUSD токенах. Чем выше процент,
    * тем меньше проскальзывание (минимальный процент - 1000, максимальный процент - 10000).
    */
    function distributeBusd(
        uint256 btcAmountOutMin_,
        uint256 ethAmountOutMin_,
        uint256 zoinksSwapSlippagePercent_,
        ...
        onlyAuthority
    {
        ...
        require(
            balance > 0,
            "Seniorage: insufficient amount of BUSD on the contract balance"
        );
        require(
            btcAmountOutMin_ > 0 &&
            ethAmountOutMin_ > 0,
            "Seniorage: invalid BTC or ETH amountOutMin"
        );
        require(
            zoinksSwapSlippagePercent_ >= LOWER_SLIPPAGE_PERCENT &&
            zoinksSwapSlippagePercent_ <= UPPER_SLIPPAGE_PERCENT &&
            addLiquiditySlippagePercent_ >= LOWER_SLIPPAGE_PERCENT &&
            addLiquiditySlippagePercent_ <= UPPER_SLIPPAGE_PERCENT,
            "Seniorage: one of the slippage percentages is too low or too high"
        );
        // Обмен 7.5% от общего баланса BUSD токенов на BTC токены
        uint256 amountToSwapOnBTCorETH = balance * BTC_ETH_PERCENT / BASE_PERCENT;
        ...
        // Покупка BTCSNACKS токенов на полученные BTC токены
        uint256 balanceBefore = IERC20(btcSnacks).balanceOf(address(this));
        ISnacksBase(btcSnacks).buyTotal(amounts[1]);
        uint256 difference = IERC20(btcSnacks).balanceOf(address(this)) - balanceBefore;
        // Отправка BTCSNACKS токенов на seniorage wallet и контракт Pulse
        IERC20(btcSnacks).safeTransfer(
            seniorageWallet,
            difference * BTC_SNACKS_SENIORAGE_PERCENT / BASE_PERCENT
        );
        IERC20(btcSnacks).safeTransfer(
            pulse,
            difference * BTC_SNACKS_PULSE_PERCENT / BASE_PERCENT
        );
        ...
        // Покупка ETHSNACKS токенов на полученные ETH токены
        balanceBefore = IERC20(ethSnacks).balanceOf(address(this));
        ISnacksBase(ethSnacks).buyTotal(amounts[1]);
        difference = IERC20(ethSnacks).balanceOf(address(this)) - balanceBefore;
        // Отправка ETHSNACKS токенов на seniorage wallet и контракт Pulse
        IERC20(ethSnacks).safeTransfer(
            seniorageWallet,
            difference * ETH_SNACKS_SENIORAGE_PERCENT / BASE_PERCENT
        );
        IERC20(ethSnacks).safeTransfer(
            pulse,
            difference * ETH_SNACKS_PULSE_PERCENT / BASE_PERCENT
        );
        ...
        amounts = IRouter(router).swapExactTokensForTokens(
            ...
            amountToSwapOnZoinks * zoinksSwapSlippagePercent_ / BASE_PERCENT,
            path,
            address(this),
            block.timestamp + 15
        );
        // Покупка SNACKS токенов на полученные ZOINKS токены
        balanceBefore = IERC20(snacks).balanceOf(address(this));
        ISnacksBase(snacks).buyTotal(amounts[1]);
        difference = IERC20(snacks).balanceOf(address(this)) - balanceBefore;
        // Отправка SNACKS токенов на seniorage wallet и контракт Pulse
        IERC20(snacks).safeTransfer(
            seniorageWallet,
            difference * SNACKS_SENIORAGE_PERCENT / BASE_PERCENT
        );
        IERC20(snacks).safeTransfer(
            pulse,
            difference * SNACKS_PULSE_PERCENT / BASE_PERCENT
        );
