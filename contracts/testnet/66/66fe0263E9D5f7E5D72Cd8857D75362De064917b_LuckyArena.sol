...
contract LuckyArena is OwnableUpgradeable {
    ...
    // last bomb time
    uint256 internal _lastBombTime;
    ...
    function _joinGame(uint256 tokenId, uint256 zone) internal {
        ...
        if (_joinedMonster[tokenId]) {
            revert("LuckyArena: cannot join twice");
        }
    ...
    function _nextBombTime() internal view returns (uint256) {
        if (gameOver()) {
            return 0;
        }
        if (_lastBombTime == 0) {
        ...
        return _lastBombTime + _bombPeriod;
    }
...
contract LuckyMonster is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    // mint with invite code
    function mintMonster(string memory inviteCode) external payable {
        uint256 tokenId = _mint();
        emit MintMonster(_arena.season(), msg.sender, tokenId, msg.value, inviteCode);
    }
