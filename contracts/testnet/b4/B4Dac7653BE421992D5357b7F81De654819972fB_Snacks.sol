...
contract Snacks is ISnacks, SnacksBase {
    ...
    Snapshots private _holdersSupplySnapshots;
    ...
    */
    ...
    /// @notice Retrieves the balance and deposit of `account_`.
    /// @param account_ Account address.
    function balanceAndDepositOf(address account_) public view returns (uint256) {
        return 
            IMultipleRewardPool(snacksPool).getBalance(account_)
            + balanceOf(account_);
    }

    /// @notice Retrieves the balance and deposit of `account_` at the time `snapshotId_` was created.
    /// @param account_ Account address.
    ...
    function holdersSupplyAt(
        ...
        returns (uint256) 
    {
        (bool snapshotted, uint256 value) = _valueAt(snapshotId_, _holdersSupplySnapshots);
    ...
    function getExcludedBalance() public override view returns (uint256) {
        ...
        for (uint256 i = 0; i < _excludedHolders.length(); i++) {
            excludedBalance += balanceAndDepositOf(_excludedHolders.at(i));
        }
    ...
    */
    ...
    /// @notice Функция, реализующая логику обновления данных о total supply холдеров.
    function _updateHoldersSupply() private {
        _updateSnapshot(_holdersSupplySnapshots, _totalSupply - getExcludedBalance());
    }
    ...
    function _updateSnapshot(Snapshots storage snapshots_, uint256 currentValue_) private {
        ...
        } else if (_lastSnapshotId(snapshots_.ids) == currentId && currentId != 0) {
    ...
    */
    ...
    /**
    * @notice Функция, реализующая логику подсчета заработанной пользователем комиссии
    * за `endIndex_` - `startIndex_` пропущенных снятий комиссий.
    * @param startIndex_ Стартовый индекс.
    * @param endIndex_ Конечный индекс.
    * @param isBtcSnacks_ Определяет то, для какого токена вести подсчет заработанной комиссии.
    */
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 cumulativeHoldersSupply;
        ...
        if (isBtcSnacks_) {
            for (i = startIndex_; i < endIndex_; i++) {
                ...
                cumulativeHoldersSupply += holdersSupplyAt(feeSnapshotId);
        ...
        if (i == startIndex_) {
            ...
            return (i, cumulativeFeeAmount.mul(cumulativeBalanceAndDeposit).div(cumulativeHoldersSupply));
        }
    }
}
...
abstract contract SnacksBase is ISnacksBase, IERC20Metadata, Ownable, ReentrancyGuard, Pausable {
    ...
    * @param correlationFactor_ Коэффициент, при делении на который осуществляется переход от
    * посчитанного количества SNACKS/BTCSNACKS/ETHSNACKS токенов к ZOINKS/BTC/ETH токенам.
    * @param totalSupplyFactor_ Коэффициент, на который делится `_totalSupply`, чтобы получить
    * стоимость последнего купленного снэка.
    ...
    * @dev При подсчете используется следующая формула: `S(n, m) = (n + m) * (m - n + 1) / 2 * d`, где 
    * n = `next`, m = `last`. После подстановки значений получаем, что 
    ...
    * @dev При подсчете используется следующая формула: `S = (2 * a + d * (n - 1)) / 2 * n`, где
    * a = `nextSnackPrice`. Из этой формулы нам необходимо найти значение n, поэтому, после преобразований
    * получаем, что нам необходимо решить квадратное уравнение `d * n ^ 2 + (2 * a - d) * n - 2 * S = 0`.
    * @param payTokenAmount_ Количество ZOINKS/BTC/ETH токенов, которое покупатель
    * желает потратить в обмен на SNACKS/BTCSNACKS/ETHSNACKS токены. Не может быть
    * меньше текущей суммы покупки одного SNACKS/BTCSNACKS/ETHSNACKS токена.
    */
    ...
    * @dev При подсчете используется следующая формула: `S(n, m) = (n + m) * (m - n + 1) / 2 * d`, где
    * n = `start`, m = `_totalSupply`. После подстановки значений получаем, что 
