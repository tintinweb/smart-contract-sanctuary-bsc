...
contract BalloonGame is BaseGame {
    ...
    function getOdds(uint8 amountOfPumps)
        ...
        returns (uint256)
    {
        uint24[6] memory pumpProbabilities = [90000, 72000, 50250, 30120, 15060, 6024];
        uint256 odds = _rtp * 100000 / pumpProbabilities[amountOfPumps-1];
    ...
    function revealSeed(
        ...
        uint8 balloonLimitNumber = uint8(SeedUtility.getHashNumberUsingAsciiNumber(seed) % 6 + 1);
        ...
        IBetSlips(_betSlipsAddr).completeBet(
            ...
            SeedUtility.uintToStr(balloonLimitNumber),
            returnAmount,
            betSlip.odds
        );
    }
    ...
    function getReturnAmount(
        ...
        uint256 wagerAmount,
        uint256 odds,
        uint8 balloonLimitNumber
    ) private view returns (uint256) {
        uint8 amountOfPumps = _playerChoices[seedHash];
        uint256 returnAmount;

        if (amountOfPumps < balloonLimitNumber) {
