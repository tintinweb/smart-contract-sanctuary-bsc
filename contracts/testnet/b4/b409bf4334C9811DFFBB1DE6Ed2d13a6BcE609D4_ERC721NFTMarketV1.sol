...
contract ERC721NFTMarketV1 is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    address public immutable KRC;
    uint256 public awardKRC = 10 * 10**18;// in wei
    ...
    uint256 public minimumAskPrice = 1; // in wei
    uint256 public maximumAskPrice = ~uint256(0); // in wei

    mapping(address => bool) public feeWhitelist;

    uint256 public feeWhitelistRate = 100;//(100 = 1%, 500 = 5%, 5 = 0.05%)
    ...
    event SetAwardKRC(uint256 oldAwardKRC, uint256 newAwardKRC);
    event SetFeeWhitelist(address user, bool feeWhitelist);
    ...
    constructor() {
        ...
        treasuryAddress = 0xF27c96cf40Ade578dc6fe2c103415A7171d818A3;
        WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
        KRC = 0x413C34BF2633edd6f1F1a3B88807Fb3b629C1E2E;
    }
    ...
    function setFeeWhitelist(address[] calldata _addresses,bool[] calldata _bool) external onlyAdmin {
        require(_addresses.length>0,"error");
        require(_addresses.length<500,"error");
        require(_addresses.length == _bool.length,"error");
        for (uint256 i = 0; i < _addresses.length; i++) {
            if(feeWhitelist[_addresses[i]]!=_bool[i]){
                feeWhitelist[_addresses[i]] = _bool[i];
                emit SetFeeWhitelist(_addresses[i], _bool[i]);
            }
        }
    }
    /**
    ...
    function setAwardKRC(uint256 _awardKRC) external onlyOwner {
        uint256 _oldAwardKRC = awardKRC;
        awardKRC = _awardKRC;
        emit SetAwardKRC(_oldAwardKRC, _awardKRC);
    }
     ...
     */
    function calculatePriceAndFeesForCollection(address collection, uint256 price, address seller)
        external
        view
        returns (
            uint256 netPrice,
        ...
        )
    {
        ...
        return (_calculatePriceAndFeesForCollection(collection, price,seller));
    }
...
//        IERC20(WBNB).safeTransfer(askOrder.seller, netPrice);
        safeTransferETH(askOrder.seller, netPrice);
...
//        }
        ...
        //给作者转费用
        safeTransferETH(IERC721Base(_collection).authorOf(_tokenId), creatorFee);
        //给平台转费用
        safeTransferETH(treasuryAddress, tradingFee);
        //给买方转KRC
        if(awardKRC>0){
            IERC20(KRC).safeTransfer(address(msg.sender), awardKRC);
        }
     ...
     */
    function _calculatePriceAndFeesForCollection(address _collection, uint256 _askPrice,address seller)
        internal
        view
        returns (
            uint256 netPrice,
        ...
        )
    {
        ...
        if(feeWhitelist[seller]){
            tradingFee = (_askPrice * feeWhitelistRate) / 10000;
        }
...
interface IERC721Base{
    /**
     * @dev Returns the author of the NFT specified by `tokenId`.
     */
