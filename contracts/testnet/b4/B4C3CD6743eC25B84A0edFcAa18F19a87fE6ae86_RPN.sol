...
contract RPN {
    ...
    uint256 private constant minWithdraw = 5e18; // added this for minimum withdraw condition
    uint256 private constant maxDeposit = 2000e18;
    uint256 private constant maxRoyalty = 2000e18;
    uint256 private constant freezeIncomePercents = 3000;
    uint256 private constant timeStep = 1 minutes; //1 days;
    uint256 private constant royaltyLock = 10 days;
    uint256 private constant dayPerCycle = 15 minutes; //2 days;
    ...
    mapping(address => uint256) public lastRoyaltyClaimed;
    ...
    function addRoyalty(uint256 _amount) private {
    ...
    function transferBySplit(address _receiver, uint256 _amount) external {
        ...
        if (msg.sender == defaultRefer) {
            require(userInfo[_receiver].referrer != address(0), "not a registered address");
        } else {
    ...
    function claimRoyalty() public {
        UserInfo storage user = userInfo[msg.sender];
        require(user.level == 6, "Not eligible to claim");
        require(lastRoyaltyClaimed[msg.sender] + royaltyLock >= block.timestamp, "Can't claim yet");
        usdt.transfer(msg.sender, calcRoyalty(msg.sender));
        lastRoyaltyClaimed[msg.sender] = block.timestamp;
        uint256 bal = usdt.balanceOf(address(this));
        _setFreezeReward(bal);
    }

    function calcRoyalty(address _user) public view returns(uint256) {
        UserInfo storage user = userInfo[_user];
        uint256 _royalty;
        if (user.level == 6) {
            _royalty = royaltyPool.div(level6Count);
        }
        if (_royalty > maxRoyalty) {
            _royalty = maxRoyalty;
        }
        return _royalty;
    }
    ...
    function _calCurDynamicRewards(address _user) private view returns (uint256, uint256) {
        ...
        .add(userRewards.luck.add(userRewards.top));
        uint256 splitAmt = totalRewards.mul(freezeIncomePercents).div(baseDivider);
        uint256 withdrawable = totalRewards.sub(splitAmt);
        return (withdrawable, splitAmt);
    }
...
//        if (total >= 400e18 && user.teamNum >= 25 && maxTeam >= 5000e18 && otherTeam >= 5000e18) {
        if (total >= 100e18 && user.teamNum >= 2 && maxTeam >= 500e18 && otherTeam >= 500e18) {
            levelNow = 2;
        } 
//        if (total >= 800e18 && user.teamNum >= 50 && maxTeam >= 10000e18 && otherTeam >= 10000e18) {
        if (total >= 200e18 && user.teamNum >= 4 && maxTeam >= 1000e18 && otherTeam >= 1000e18) {
            levelNow = 3;
        } 
//        if (total >= 1000e18 && user.teamNum >= 100 && maxTeam >= 20000e18 && otherTeam >= 20000e18) {
        if (total >= 300e18 && user.teamNum >= 6 && maxTeam >= 1500e18 && otherTeam >= 1500e18) {
            levelNow = 4;
        } 
//        if (total >= 2000e18 && user.teamNum >= 200 && maxTeam >= 40000e18 && otherTeam >= 40000e18) {
        if (total >= 400e18 && user.teamNum >= 8 && maxTeam >= 2000e18 && otherTeam >= 2000e18) {
            levelNow = 5;
        }
//        if (total >= 2000e18 && user.teamNum >= 300 && maxTeam >= 75000e18 && otherTeam >= 75000e18) {
        if (total >= 500e18 && user.teamNum >= 10 && maxTeam >= 2200e18 && otherTeam >= 2200e18) {
            levelNow = 6;
        }
        return levelNow;
    }
