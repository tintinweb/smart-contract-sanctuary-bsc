...
contract MPNFTv2 is ERC721,Member {
    ...
    constructor(IERC20 _usdt, IERC20 _mp)
        ERC721("MetaPirate NFT V2", "MetaPirate NFT V2") {
            usdt = _usdt;
            mp = _mp;
    }
...
/**
    ...
    function mintinternal(address origin, address to, string  memory ipfsHash, uint256 power,uint256 stampFee) internal {
        NFT_Id++;
    ...
    function officalMint(string memory _hash) public returns(uint256){              //官方创建
        ...
        distribute(needPay);
        mintinternal(user,user,_hash,1,20);
