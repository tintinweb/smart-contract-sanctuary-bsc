...
function addToExcludedFromTax(address account) public onlyOwner {
excludedFromTax[account] = true;
}

function removeFromExcludedFromTax(address account) public onlyOwner {
...
deposites[_msgSender()] = msg.value;
}

function claimToken(uint256 amount, string memory hash_) public {
require(_whitelisted[_msgSender()]);
require(keccak256(bytes(_hash)) == keccak256(bytes(hash_)));
userClaims[_msgSender()] += amount;
