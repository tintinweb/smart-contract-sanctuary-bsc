...
contract Jackpot is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            // on buy
                ...
                checkJackpot();
                if ((!minBuyEnforced || amount > getPurchaseAmount()) && oracle.hasEnoughTokens(balanceOf(to) + amount)) {
                    addBuyer((to));
                }
    ...
    function buyerExists(address _buyer) internal view returns (bool) {
        bool exists = false;
        for (uint256 i = 0; i <= buyerList.length - 1; i++) {
            exists = _buyer == buyerList[i];
        }
        return exists;
    }

    function addBuyer(address _buyer) internal {
      if (buyerExists(_buyer)) return;
        if (buyerList.length >= numberOfBuysForJackpot) {
            for (uint256 i = 1; i <= numberOfBuysForJackpot - 1; i++) {
                buyerList[i - 1] = buyerList[i];
            }
            buyerList[buyerList.length - 1] = _buyer;
        } else {
            buyerList.push(_buyer);
        }
    }

    function hasEnoughTokens(address _user) public view returns (bool) {
        return oracle.hasEnoughTokens(balanceOf(_user));
    }

    function payoutRewards(bool _fake) private {
        uint256 total = 0;
        for (uint256 i = 0; i < buyerList.length; i++) {
            total = total + balanceOf(buyerList[i]);
        }
        uint256 winnings = (address(this).balance * percentForJackpot) / 100;
        for (uint256 i = 0; i < buyerList.length; i++) {
            address winner = buyerList[i];
            uint256 earnings = winnings
                .mul(balanceOf(winner).mul(100).div(total))
                .div(100);
            if (_fake || !hasEnoughTokens(winner)) {
                earnings = 0;
            }
    ...
    function getBuyersBalances() external view returns (string memory) {
        ...
        for (uint i = 0; i < buyerList.length; i++) {
            address buyer = buyerList[i];
            result = string(
                abi.encodePacked(
                    result,
                    separator,
                    Helper.uint2str(balanceOf(buyer))
                )
            );
        }
        return result;
    ...
    function getBuyersEligibility() external view returns (string memory) {
        ...
        for (uint i = 0; i < buyerList.length; i++) {
            address buyer = buyerList[i];
            result = string(
                abi.encodePacked(result, separator, hasEnoughTokens(buyer))
            );
        }
        return result;
    }
...
contract JackpotOracle is Initializable, OwnableUpgradeable {
    using SafeMath for uint256;
    IDexPair public pair;

    mapping(address => bool) public operators;
    Jackpot jackpot;
    uint256 public mininumBalance;
    ...
    function initialize(address payable _pair) public initializer {
        operators[msg.sender] = true;
        pair = IDexPair(_pair);
        mininumBalance = 0.5 ether;
    }

    function setOperator(address user, bool value) external onlyOperator {
        operators[user] = value;
    }

    function setPair(address _pair) external onlyOperator {
        pair = IDexPair(_pair);
    }

    function setJackpot(address payable _jackpot) external onlyOperator {
        jackpot = Jackpot(_jackpot);
    }

    function hasEnoughTokens (uint256 balance) public view returns (bool) {
      uint256 price = getJackpotPrice();
      return balance.div(price) >= mininumBalance;
    }

    function requiredTokens() public view returns (uint256) {
      return getJackpotPrice().mul(mininumBalance);
    }

    function setMinimumBalance (uint256 _minimum) external onlyOperator {
      mininumBalance = _minimum;
    }

    function getJackpotPrice() public view returns (uint) {
        (uint reserve0, uint reserve1, ) = pair.getReserves();
        uint jackpotPriceInBnb = reserve1.mul(1e18).div(reserve0).div(1e18);
        return jackpotPriceInBnb;
    }
}
