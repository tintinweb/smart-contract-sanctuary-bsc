...
contract Statistics {
    ...
    function listPresales(uint256 _type, uint256 _start, uint256 _limit) external view returns (address[] memory presales, string[] memory status, uint256 lastId) {
        //Types
        //1 - active
        //2 - launched
        //3 - cancelled
        ...
        if (_limit > 0) {
            ...
            for (uint256 i=_start; gasUsed < 5000000 && i < searchLength && i - _start < _limit ; i++) {
                ...
                status[i-_start] = getPresaleStatus(presaleAddress);
                lastId = i;
    ...
    function getPresaleInfo(address _presale) external view returns(address tokenAddress,string memory tokenName, uint8 tokenDecimals, string memory tokenSymbol, string memory presaleStatus, uint256 tokenSupply, uint256 presaleBalance, address presaleOwner, uint256[16] memory inputs, string[10] memory socials) {
        ...
        presaleBalance = ps.currentBalance();
...
contract Tiers {
    ...
    IERC20 constant sod = IERC20(0xe8c0F7486e33EDcDbf110F66bbC38F82d2937f77);//0xCDB943908DE5Ee37998a53f23467017d1A307E60);
...
contract Presale is Whitelist {
    ...
    function _buy(address _for, uint256 _value) internal {
        ...
        require(!filled(), "Hardcap reached");
