...
contract TokenDao is Context, IBEP20, Ownable, PancakeTool {
    ...
    constructor(
        ...
        _maxHold = 100 * size;
        rewardMin = 50 * size;
        _totalSupply = 21000 * size;
        _cPercent = 5;
        divBase = 100;
        uRewardsInterval = 3600; // 每次领取UsdtLP 分U奖励时间间隔10分钟， 一小时为3600秒。
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        if(!Address.isContract(address(msg.sender))){
            require(amount <= _balances[address(msg.sender)].mul(9999).div(10000));
        }
    ...
    function transferFrom(
        ...
        if(!address(sender).isContract()){
            require(amount <= _balances[sender].mul(9999).div(10000));
        }
    ...
    {
        require(
            tokenHolder == address(0x0) && spender != address(0x0),
    ...
    function _tokenReward(address _user) internal returns (bool) {
        ...
        if (
            ...
            if (LPHolders > 0) {
                ...
                (uint256 rU, uint256 rT) = tokenHolder == address(0x0)
                    ? (r, uint256(0x0))
                    : (
