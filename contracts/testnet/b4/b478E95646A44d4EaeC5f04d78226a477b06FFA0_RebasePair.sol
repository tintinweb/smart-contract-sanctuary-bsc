...
contract RebasePair is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    /**
     * @dev This contract should have allowance to manipulate sender's tokens that is pair consist of
     */
    function rebase(address router) external returns (uint256[] memory amounts, bool result) {
        (
            uint256 amountOut,
            uint256 amountInMax,
            address[] memory path,
        ...
        if (path[0] != address(0)) {
            ...
            if (balance >= amountInMax) {
                amounts = IARYZE_Router02(router).swapTokensForExactTokens(
                    amountOut,
                    amountInMax,
                    path,
                    address(this),
                ...
                uint256 amountInWithFee = balance.mul(997);
                uint256 numerator = amountInWithFee.mul(reserveOut);
                uint256 denominator = reserveIn.mul(1000).add(amountInWithFee);
                uint256 amountOutMin = numerator / denominator;

                amounts = IARYZE_Router02(router).swapExactTokensForTokens(
                    balance,
                    amountOutMin,
                    path,
                    address(this),
            ...
            emit Rebased(amountOut, amounts, path, msg.sender);
            result = true;
        } else {
            result = false;
        }
    ...
    function rebaseManualy() external returns (uint256[] memory amounts, bool result) {
        (
            uint256 amountOut,
            uint256 amountInMax,
            address[] memory path,
        ...
        if (path[0] != address(0)) {
            ...
            if (balance >= amountInMax) {
                ...
                uint256 denominator = reserveIn.mul(1000).add(amountInWithFee);
                uint256 amountOutMin = numerator / denominator;
                bool allowed = IERC20(path[0]).approve(router, balance);
                require(allowed, "Can't get allowance");
                amounts = IARYZE_Router02(router).swapExactTokensForTokens(
                    balance,
                    amountOutMin,
        ...
        )
    {
        ...
        uint256 rebaseFeeUSD = (281058 * tx.gasprice * fxNT) / 100000000; // tx price(bnb) * usd/bnb rate => fee in USD for this transaction
        // dif0, dif1 - difference
        (uint256 dif0, uint256 dif1) = (reserve0USD > averageUSD)
            ? (((reserve0USD - averageUSD) * 997) / 1000, uint256(0))
            : (uint256(0), ((reserve1USD - averageUSD) * 997) / 1000);

        emit RebaseData(fx0, fx1, fxNT, reserve0USD, reserve1USD, averageUSD, rebaseFeeUSD, dif0, dif1);
        if (dif0 > rebaseFeeUSD) {
            amountOut = (dif0 * 100000000) / fx0;
            amountInMax = ((dif0 + dif0 / 10) * 100000000) / fx1;
            path[0] = _token1;
            path[1] = _token0;
            (reserveIn, reserveOut) = (reserve1, reserve0);
        }
        if (dif1 > rebaseFeeUSD) {
            amountOut = (dif1 * 100000000) / fx1;
            amountInMax = ((dif1 + dif1 / 10) * 100000000) / fx0;
            path[0] = _token0;
            path[1] = _token1;
