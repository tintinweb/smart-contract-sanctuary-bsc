...
contract UberTaxiNFT is ERC721URIStorage, Ownable {
    ...
    uint256[6] public NFTPrices = [1000,2000,3000,4000,5000,10000];

    uint256[6] public dayRates = [50,52,54,56,58,60];
    ...
    function _transfer(
        ...
        require(!islock[tokenId], "ERC20: the NFT is locked");
    ...
    function setNFTPrice(uint256 types, uint256 price) public onlyOwner {
        NFTPrices[types - 1] = price;  
    }

    //设置日收益率
    function setdayRate(uint256 types, uint256 rate) public onlyOwner {
        dayRates[types - 1] = rate;  
    }
    ...
    function createNFT(address user, uint256 NFTType) public onlyController returns (uint256){
        ...
        // userNFTs[user][userNFTNumber[user]] = tokenId;
...
// File: UberTaxi/UberTaxiController.sol
...
contract UberTaxiController is Ownable {
    ...
    struct User {
        ...
        uint256[6] vehicleTypeNum; //车辆类型数量

        uint256 vehicleNum; //车辆总数量

        address welMember; //邀请人地址

        uint256 bindNum; //绑定数量

        uint256 UTAXINum; //UTaix数量

        uint256 totalDeposit; //推荐用户总入金

        uint256 UTAXIReward; //UTaix推荐奖励

        bool isEffectiveUser; //有效用户：true=有效；false=无效
    ...
    struct Vehicle {
        ...
        uint256 price; //车辆TAXI价格

        uint256 uPirce; //UTAXI价格

        uint256 rate; //收益率

        uint256 purchaseLimit;//限购数量
    ...
    struct MyVehicle {
        ...
        uint256 buyDays;//购买时间

        uint256 vehicleState; //车辆状态：0=未使用；1=出租中; 2=上架中; 3=报废

        // bool isVehicleOnShelf; //车辆是否上架

        uint256 expireTime; //出租到期时间

        uint256 durability; //车辆耐久度

        // bool isScrap; //是否报废

        uint256 tokenId; //车辆标识

        address hold; //汽车持有者

        uint256 buyPrice; //购买价格

        uint256 profit; //收益

        uint256 totalProfit; //总收益

        uint256 sellPrice; //出售价格
    }

    struct Sell{

        address onwerAddr; //用户地址

        uint256 myHeroId; //用户的车辆Id

        uint256 price; //车辆出售价格
    ...
    struct Commission {
        ...
        uint256 buyTime; //购买的时间

        uint256 mineType; //购买的车辆类型
    ...
    uint256 public usersTotal; //用户总量

    uint256 public effectiveUsersNum; //有效用户总量1

    mapping(uint256 => uint256) public utaxiProduceTotal; //UTAXI产出总量1

    uint256 public uExchangeTTotal; //U兑换T的总量1

    mapping(uint256 => uint256) public utaxiTotal; //utaxi的余量总和

    mapping(uint256 => uint256) public vehicleTotals; //车辆类型的总量1

    uint256 public marketTransactionTotal; //市场交易总量1

    uint256 public marketDealTotal;  //市场成交总量1
    ...
    uint256 serviceCharge = 10; //兑换手续费

    //uint256 transferCharge = 10; //转账手续费

    uint256 sellFee = 10;//商城购买手续费

    uint256 commissionFee = 10;//返佣比例

    address depositAddress; //入金地址

    address withDrawAddress; //出金地址
    ...
    mapping(uint256 => uint256) public idtoIndex; //车辆id对应的索引

    mapping(address => uint[]) public myVehiclesIds; //我的全部汽车id

    mapping(uint256 => MyVehicle) public allVehicles; //所有车辆, 通过id 指向具体的车辆

    mapping(address => bool) public contractUsers; //合约有效用户

    mapping(address => uint[]) public myCommissionsIds; //返佣列表

    mapping(uint256 => Commission) public commissions;

    mapping(uint256 => uint[]) public vehiclesTypeSells;  //车辆租售列表带分类

    mapping(uint256 => uint) public idToIndexByTypeSells; //汽车出售列表在分类列表中的索引

    mapping(uint256 => address) public effectiveUsers;

    uint256 public effectiveUserNum; //有效用户的数量

    uint[]  public vehicleSells; //总列表

    mapping(uint256 => uint256) idToIndexBySells; //汽车出售列表在总列表中的索引

    event BuyVehicle(address addr, uint256 types); //购买车辆事件

    event VehicleRent(uint256 index); //车辆出租事件

    event VehicleGetReward(address user, uint256 index, uint256 reward); //车辆返佣事件
    ...
    constructor() {
        ...
        UTN = UberTaxiNFT(0xaBa844a4b4571b007732919ebc6C2D51b78Cafdb);

        ids = 10000;

        myaddress = msg.sender;

        TAXI20 = ERC20(0x077E62e7F5e7B92b9ce51A2eC871556bf1f68447);

        // UTAXI20 = ERC20(0x551B5291b194d99d5ABb33FF6F6a422B757A5F7E);

        withDrawAddress = 0x80658D4064A189C7FB9369872E351aD1c87952a5;

        depositAddress = 0x80658D4064A189C7FB9369872E351aD1c87952a5;

        contractUsers[0xEd703D611df2fef3D0c626B1f688edFeeA8CfcF7] = true;

        //初始化车辆
        vehicles[1] = Vehicle("Volkswagen",   1, 1000*10**6, 900*10**6, 3.8*10, 1000, true);
        vehicles[2] = Vehicle("Tesla",        2, 2000*10**6, 1800*10**6, 4*10, 1000, true);
        vehicles[3] = Vehicle("Audi",         3, 4000*10**6, 2700*10**6, 4.2*10, 1000, true);
        vehicles[4] = Vehicle("BMW",          4, 6000*10**6, 3600*10**6, 4.4*10, 1000, false);
        vehicles[5] = Vehicle("MercedesBenz", 5, 8000*10**6, 4500*10**6, 4.6*10, 1000, false);
        vehicles[6] = Vehicle("RollsRoyce",   6, 10000*10**6, 9000*10**6, 4.8*10, 1000, false);
    }
    ...
    //修改车辆属性
    function updateVehicle(uint256 index, uint256 price, uint256 rate) public onlyOwner {
    ...
    //修改车辆发行数量
    function openVehicleNum(uint256 index, uint256 purchaseLimitNum) public onlyOwner {
    ...
    function buyVehicle(uint256 index, uint8 tokenType) public {
        ...
        if (users[msg.sender].isEffectiveUser == false){ //判断是否为有效用户

            users[msg.sender].isEffectiveUser = true;

            effectiveUserNum++; //有效用户的数量增加
        ...
        if (tokenType == 1) {//taix
            ...
            vehicles[index].purchaseLimit --; //车辆限购数量
                
            vehicleTotals[index] ++; //车辆类型的总量
            ...
            require(users[msg.sender].UTAXINum >= vehicles[index].uPirce, "You don't have enough Utaix");

            vehicleTotals[index - 1] ++; //车辆类型的总量

            //扣除用户的Utaix
            users[msg.sender].UTAXINum -= vehicles[index].uPirce;
        ...
        // allVehicles[ids] = myVehicles(index, block.timestamp, 1, false, 0, 100, false, tokenId, msg.sender, 0);
    ...
    function _vehicleRent(uint256 index) private {
        ...
        //allVehicles[index].expireTime = block.timestamp + 1 days; //更新汽车领取时间

        allVehicles[index].expireTime = block.timestamp + 120; //更新汽车领取时间
    ...
    function vehicleGetReward(uint256 index) public isUsers(index) {
        ...
        allVehicles[index].durability -= 1; //扣除耐久度
        ...
        if(allVehicles[index].durability > 0) {
            ...
            emit VehicleGetReward(msg.sender, index, 0);
    ...
    function exchange(uint256 exchangeNum) public {
        ...
        require(users[msg.sender].UTAXINum >= exchangeNum, "Your UTaixNum is insufficient");

        //扣除用户的UTaix
        users[msg.sender].UTAXINum -= exchangeNum;

        //记录U兑换T的总量
        uExchangeTTotal += exchangeNum;
        
        TAXI20.transferFrom(depositAddress, msg.sender, exchangeNum * (100 - serviceCharge) / 100);

    }

    //向用户转UTaxi
    function setUserUtaix(address addr, uint256 inputNum) public onlyOwner {

        users[addr].UTAXINum = inputNum.mul(1000000);
    ...
    function playerTransfer(address adds, uint256 transferNum) public {
        ...
        require(users[msg.sender].UTAXINum >= transferNum, "Your UTaixNum is insufficient");

        //扣除用户的UTaix
        users[msg.sender].UTAXINum -= transferNum;

        //收款用户实际到账数量
        //users[adds].UTAXINum += transferNum * (100 - transferCharge) / 100;
        users[adds].UTAXINum += transferNum;
    ...
    function vehicleOnShelf(uint256 index, uint256 price) public isUsers(index){
        ...
        allVehicles[index].sellPrice = price;

        UTN.setNFTLock(allVehicles[index].tokenId, true);

        idToIndexByTypeSells[index] = vehiclesTypeSells[allVehicles[index].types].length; // 将index push到分类列表

        vehiclesTypeSells[allVehicles[index].types].push(index);

        idToIndexBySells[index] = vehicleSells.length;  //将index push到总列表中
    ...
    function vehicleOffShelf(uint256 index) public isUsers(index){
        ...
        allVehicles[index].vehicleState = 0; //修改车辆状态

        allVehicles[index].sellPrice = 0; //修改车辆出售价格

        UTN.setNFTLock(allVehicles[index].tokenId, false);

        delete vehiclesTypeSells[allVehicles[index].types][idToIndexByTypeSells[index]]; //删除索引
    ...
    function buyMemberRender(uint256 index) public {
        ...
        TAXI20.transferFrom(msg.sender, deadWallet, fee);  //销毁手续费

        TAXI20.transferFrom(msg.sender, allVehicles[index].hold, allVehicles[index].sellPrice - fee);

        marketTransactionTotal ++; //统计市场交易总量

        marketDealTotal += allVehicles[index].sellPrice; //统计成交总量
        ...
        if (users[msg.sender].welMember != address(0)) {
            ...
            users[users[msg.sender].welMember].UTAXIReward += allVehicles[index].sellPrice.mul(commissionFee).div(100);

            users[users[msg.sender].welMember].UTAXINum += allVehicles[index].sellPrice.mul(commissionFee).div(100);
        }
    ...
    function vehicleTransfer(address addr, uint256 index) public isUsers(index){
        ...
        //有效用户校验
        require(contractUsers[addr], "this address no use in this contract");
    ...
    //查询有效用户的总数量
    function querryEffectiveUser() public view returns(uint256 totalUtaxiOfEffectiveUser){

        for(uint256 i; i < effectiveUserNum; i++){
