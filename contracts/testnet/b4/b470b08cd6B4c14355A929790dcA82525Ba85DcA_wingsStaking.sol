...
contract wingsStaking is Ownable {
    ...
    uint256 public minTokenPerShare = 2;
    ...
    function setShare(
        ...
        uint256 totalShareAmount = (amount.mul(time)).mul(
            (time.div(bonusShareFactor)).add(1)
        );
    ...
    function withdrawAndExit() public {
        ...
        uint256 rewardByApr = calculateRewardByApy(
            amountToSend,
    ...
    function emergencyWithdraw() public {
        ...
        // calculate reward token
        uint256 rewardByShare = rewardPerShare.mul(shares[msg.sender].amount);
        uint256 rewardByApr = calculateExitRewardByApy(
            amountToSend,
            stakeDetails[msg.sender].duration,
            stakeDetails[msg.sender].startTime
        );
    ...
    function calculateRewardByApy(
        ...
        if (time == 7) {
            rewardAmount = (((_7daysApr.div(time)).add(1)).mul(time.sub(1)))
                .mul(amount);
        } else if (time == 14) {
            rewardAmount = (((_14DaysApr.div(time)).add(1)).mul(time.sub(1)))
                .mul(amount);
        } else if (time == 30) {
            rewardAmount = (((_30DaysApr.div(time)).add(1)).mul(time.sub(1)))
                .mul(amount);
        } else if (time == 60) {
            rewardAmount = (((_60daysApr.div(time)).add(1)).mul(time.sub(1)))
                .mul(amount);
        } else {
            uint256 stakedDays = (block.timestamp.sub(startTime)).div(
                secondsForDay
            );
            rewardAmount = (
                ((_unlimitedDaysApr.div(stakedDays)).add(1)).mul(
                    stakedDays.sub(1)
                )
            ).mul(amount);
        }
    ...
    function calculateExitRewardByApy(
        uint256 amount,
        uint256 time,
        uint256 startTime
    ) internal returns (uint256) {
        ...
        if (time == 7) {
            rewardAmount = (
                ((_7daysApr.div(stakedDays)).add(1)).mul(stakedDays.sub(1))
            ).mul(amount);
        } else if (time == 14) {
            rewardAmount = (
                ((_14DaysApr.div(stakedDays)).add(1)).mul(stakedDays.sub(1))
            ).mul(amount);
        } else if (time == 30) {
            rewardAmount = (
                ((_30DaysApr.div(stakedDays)).add(1)).mul(stakedDays.sub(1))
            ).mul(amount);
        } else if (time == 60) {
            rewardAmount = (
                ((_60daysApr.div(stakedDays)).add(1)).mul(stakedDays.sub(1))
            ).mul(amount);
        }
