...
/*
A vault that helps users stake in PiXEL farms and pools more simply.
Supporting auto compound in Single Staking Pool.
*/

contract BNBPixelVault is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    IPixelStakingManager public pixelStakingManager;
    IPixelReferral public pixelReferral;
    uint256 public constant PIXEL_BNB_PID = 3;
    //
    //    // TESTNET
    //    IERC20 public pixel = IERC20(0xCcEc5aDE6C99D1a24a195B223B7B9F6169568A58);
    ...
    //    IPixelStakingManager public pixelStakingManager = IPixelStakingManager(0x8fE6e415ae167dDefD0e9ad4ECF9F03A2CB88946);
    //    IPixelReferral public pixelReferral;
    //    uint256 public constant PIXEL_BNB_PID = 2;
    ...
    function initialize(address _pixel, address _router, address _factory, address _pixelStakingManager, address _vaultReferralTreasury) public initializer {
        __ReentrancyGuard_init();
        __Ownable_init();
        /*
        MAINNET
        pixel = IERC20(0x5CA42204cDaa70d5c773946e69dE942b85CA6706);
        busd = IERC20(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);
        router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        factory = IUniswapV2Factory(0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73);
        pixelStakingManager = IPixelStakingManager(0x0C54B0b7d61De871dB47c3aD3F69FEB0F2C8db0B);
        */
        pixel = IERC20(_pixel);
        router = IUniswapV2Router02(_router);
        factory = IUniswapV2Factory(_factory);
        weth = IERC20(router.WETH());
        pixelStakingManager = IPixelStakingManager(_pixelStakingManager);
    ...
    function canCompound() public view returns (bool) {
        return pixelStakingManager.canHarvest(PIXEL_BNB_PID, address(this));
    }

    function nearestCompoundingTime() public view returns (uint256 time) {
        (,,,time) = pixelStakingManager.userInfo(PIXEL_BNB_PID, address(this));
    }
    ...
    function totalPoolPendingRewards() public view returns (uint256) {
        ...
        return pixelStakingManager.pendingPosition(PIXEL_BNB_PID, address(this)).mul(99).div(100);
    }
    ...
    function getSwappingPair() internal view returns (IUniswapV2Pair) {
        return IUniswapV2Pair(
            factory.getPair(address(pixel), address(weth))
        );
    }
    ...
    function approve() public {
        ...
        pixel.approve(address(router), MAX_INT);
        getSwappingPair().approve(address(pixelStakingManager), MAX_INT);
    ...
    function getReserveInAmount1ByLP(uint256 lp) public view returns (uint256 amount) {
        ...
        uint256 balance0 = pixel.balanceOf(address(pair));
    ...
    * lpNeeded returns 10
    * once remove liquidity, 10 LP will get back 500 BUSD and an amount in PIXEL corresponding to 500 BUSD
    */
    ...
    * exp. amount = 1000 PIXEL
    * lpNeeded returns 10
    * once remove liquidity, 10 LP will get back 500 PIXEL and an amount in BUSD corresponding to 500 PIXEL
    */
    ...
    function deposit(address referrer) external payable updateReward(msg.sender) nonReentrant noCallFromContract {
        uint256 amount = msg.value;
        (, uint256 res1,) = getSwappingPair().getReserves();
        uint256 amountToSwap = SwapMath.calculateSwapInAmount(res1, amount);
        uint256 pixelOut = swapBnbToPixel(amountToSwap);
        uint256 amountLeft = amount.sub(amountToSwap);
        (,uint256 amountBNB,uint256 liquidityAmount) = _addLiquidity(pixelOut, amountLeft);
    ...
    function depositTokenPair(uint256 amountPixel, address referrer) external payable updateReward(msg.sender) nonReentrant noCallFromContract {
        uint256 amountBNB = msg.value;
        uint256 balanceOfPixelBeforeTrasnfer = pixel.balanceOf(address(this));
        pixel.transferFrom(msg.sender, address(this), amountPixel);
        uint256 amountPixelReceived = pixel.balanceOf(address(this)) - balanceOfPixelBeforeTrasnfer;
        // note pixelAdded is might reduced by ~1%
        (uint256 pixelAdded, uint256 bnbAdded, uint256 liquidityAmount) = _addLiquidity(amountPixelReceived, amountBNB);
        // transfer back amount that didn't add to the pool
        if(amountPixelReceived.mul(99).div(100) > pixelAdded){
            uint256 amountLeft = amountPixelReceived.mul(99).div(100) - pixelAdded;
            if(pixel.balanceOf(address(this)) >= amountLeft)
    ...
    function _depositLP(address account, uint256 liquidityAmount, address referrer) internal {
        if (
            address(pixelReferral) != address(0) &&
            referrer != address(0) &&
            referrer != account
        ) {
            pixelReferral.recordReferral(account, referrer);
        }
        //stake in farms
    ...
    function withdraw(uint256 amount, bool isReceiveBnb) external updateReward(msg.sender) nonReentrant noCallFromContract {
        ...
        pixelStakingManager.withdraw(PIXEL_BNB_PID, lpAmountNeeded);
        (uint256 amountA,uint256 amountB) = removeLiquidity(lpAmountNeeded);
        if(isReceiveBnb){
            // send as much as we can
            // doesn't guarantee enough $amount
            payable(msg.sender).transfer(swapPixelToBnb(amountA).add(amountB));
        }else{
            pixel.transfer(msg.sender, amountA);
    ...
    function withdrawLP(uint256 lpAmount) external updateReward(msg.sender) nonReentrant {
        ...
        pixelStakingManager.withdraw(PIXEL_BNB_PID, lpAmount);
    ...
    function harvest(bool isReceiveBnb) external updateReward(msg.sender) nonReentrant noCallFromContract {
        ...
        if (reward > 0) {
            ...
            pixelStakingManager.withdraw(PIXEL_BNB_PID, lpNeeded);
            (uint256 amountPixel,uint256 amountBNB) = removeLiquidity(lpNeeded);
            if(isReceiveBnb) {
                payable(msg.sender).transfer(swapPixelToBnb(amountPixel).add(amountBNB));
            }else{
                pixel.transfer(msg.sender, swapBnbToPixel(amountBNB).add(amountPixel));
            }
    ...
    function compound() external updateReward(address(0)) nonReentrant {
        ...
        if (_canCompound) {
            ...
            uint256 amountCollected = pixel.balanceOf(address(this));
            ...
            (uint256 res0,,) = getSwappingPair().getReserves();
            uint256 pixelAmountToSwap = SwapMath.calculateSwapInAmount(res0, rewardForPool);
            uint256 busdOut = swapPixelToBnb(pixelAmountToSwap);
            (,, uint256 liquidityAmount) = _addLiquidity(rewardForPool.sub(pixelAmountToSwap), busdOut);
            depositStakingPool(liquidityAmount);
    ...
    function pendingPositionNextCompound() public view returns (uint256){
        return pixelStakingManager.pendingPosition(PIXEL_BNB_PID, address(this));
    }
    ...
    function swapBnbToPixel(uint256 amountToSwap) internal returns (uint256 amountOut) {
        uint256 pixelBalanceBefore = pixel.balanceOf(address(this));
        ...
        amountOut = pixel.balanceOf(address(this)).sub(pixelBalanceBefore);
    }

    function swapPixelToBnb(uint256 amountToSwap) internal returns (uint256 amountOut) {
    ...
    function removeLiquidity(uint256 lpAmount) internal returns (uint256 amountPixel, uint256 amountBNB){
        uint256 pixelBalanceBefore = pixel.balanceOf(address(this));
        (,amountBNB) = router.removeLiquidityETH(
            address(pixel),
            lpAmount,
            0,
            0,
        ...
        amountPixel = pixel.balanceOf(address(this)).sub(pixelBalanceBefore);
    }

    function depositStakingPool(uint256 amount) internal {
        pixelStakingManager.deposit(PIXEL_BNB_PID, amount, address(vaultReferralTreasury));
    }


    function getBnbToPixelRoute() private view returns (address[] memory paths) {
        paths = new address[](2);
        paths[0] = address(weth);
        paths[1] = address(pixel);
    }

    function getPixelToBnbRoute() private view returns (address[] memory paths) {
        paths = new address[](2);
        paths[0] = address(pixel);
