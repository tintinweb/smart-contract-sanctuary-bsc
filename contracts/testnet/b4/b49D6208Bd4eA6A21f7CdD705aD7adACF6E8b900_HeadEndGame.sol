...
contract HeadEndGame is Ownable, Pausable, ReentrancyGuard {
    ...
    struct Round {
        ...
        uint256 headAmount;
        uint256 endAmount;
        bool valid; // Already calculated
        bool invalid; // Round invalid. Make it refundable
        uint256 resultBlock;
        bytes32 resultHash;
        uint8 lastDigit;
    ...
    event BetClaimed(uint256 id, uint256 round, address user, uint256 amount);
    ...
    constructor(
        ...
        address _keeperAddress, // address of the keeper
        address _currency,
    ...
    function placeBet(
        ...
        require(genesisBlock != 0, "Bet can run after genesis trigger");
        require(
            roundID == _getRoundID(block.number),
            "Bet not active for this round"
        );
        ...
        // Update bet data
        uint256 id = ++betIdCounter;
        bets[id] = Bet(id, _msgSender(), amount, roundID, false, head);
        userBets[_msgSender()].push(id);

        emit PlaceBet(id, roundID, _msgSender(), amount, head);
    }

    function claimReward(uint256[] calldata betIds)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < betIds.length; i++) {
            ...
            require(
                block.number > _getEndBlock(bet.roundID),
                "Round has not ended"
            );
            ...
            // Round valid, rewards
            if (rounds[bet.roundID].valid) {
                require(_claimable(bet, msg.sender), "Not eligible for claim");

                profit =
                    bet.amount +
                    (bet.amount * rounds[bet.roundID].rewardAmount) /
                    rounds[bet.roundID].rewardBaseCalAmount;

                // Round invalid, refund bet amount
            } else {
                require(
                    _refundable(bet, msg.sender),
            ...
            emit BetClaimed(bet.id, bet.roundID, _msgSender(), profit);
        }

        if (reward > 0) {
            IERC20(currency).safeTransfer(_msgSender(), reward);
        }
    }
     ...
     */
    function executeRound(uint256 roundID)
        external
        whenNotPaused
        onlyKeeperOrOperator
    {
        require(
            !rounds[roundID].valid && rounds[roundID].invalid,
            "Round executed"
        );
        uint256 resultBlock = _getEndBlock(roundID) + resultGap;
        ...
        if (_hash == 0) {
            ...
            emit RoundInvalid(roundID, resultBlock);
        } else {
            // Round valid
    ...
    function _endRound(Round storage round, bytes32 _hash) internal {
        ...
        if (round.head) {
            ...
            rewardAmount = round.endAmount - treasuryAmt;
            // End wins
        } else {
            ...
            rewardAmount = round.headAmount - treasuryAmt;
        }
    ...
    function _getRoundID(uint256 number) internal view returns (uint256) {
    ...
    function currentRound() external view returns (uint256) {
        ...
        return _getRoundID(block.number);
    }
        ...
        onlyAdmin
    {
        ...
        emit NewMinBetAmount(_getRoundID(block.number), minBetAmount);
    }
        ...
        onlyAdmin
    {
        ...
        emit NewTreasuryRate(_getRoundID(block.number), treasuryRate);
    }
    ...
    function getLastUserBets(
        ...
        uint256[] memory userBetsIds = userBets[user];
        uint256 length = dataLength;

        if (length > userBetsIds.length - cursor) {
            length = userBetsIds.length - cursor;
        }

        Bet[] memory _userBets = new Bet[](dataLength);
        if (length > 0) {
            for (uint256 i = 0; i < length; i++) {
                _userBets[i] = bets[cursor + i];
            }
        }
    ...
    function _claimable(Bet memory bet, address user)
        internal
        view
        returns (bool)
    {
        Round memory round = rounds[bet.roundID];
        return
            bet.user == user &&
            round.valid &&
            bet.amount != 0 &&
    ...
    function claimable(uint256 betId, address user) public view returns (bool) {
        return _claimable(bets[betId], user);
    }
    ...
    function _refundable(Bet memory bet, address user)
        internal
        view
        returns (bool)
    {
        return
            bet.user == user &&
            !rounds[bet.roundID].valid &&
            !bet.claimed &&
            rounds[bet.roundID].invalid &&
            bet.amount != 0;
    }

    /**
     * @notice Get the refundable stats of specific bet and user account
     * @param betId: Id of bet
     * @param user: user address
     */
    function refundable(uint256 betId, address user)
        public
        view
        returns (bool)
    {
        return _refundable(bets[betId], user);
    }
}
