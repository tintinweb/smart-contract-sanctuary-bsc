...
contract PandoPotV3 is Ownable, ReentrancyGuard, Pausable {
    ...
    function _getTickets(uint256 _number) internal view returns(uint, uint) {
    ...
    function _claim(PRIZE_TYPE _type, uint256 _roundId, uint256 _ticketNumber, address _receiver) internal {
        ...
        if (_type != PRIZE_TYPE.LEADERBOARD) {
            ...
            uint256 _number = _ticketNumber % typePrize[_type].digit;
    ...
    function buyTickets(address _receiver, uint256[] memory _tickets, uint256 _roundId) external availableBuyTicket(_receiver, _roundId) onlyEOA whenNotPaused nonReentrant{
        ...
        for (uint256 i = 0; i < _tickets.length; i++) {
            ...
            _addTicket(_receiver, _ticketNumber, _ticketMinor, _ticketMini, _roundId);
        }
    ...
    function _calculateNumbersForQC(uint256 _number1, uint256 _number2, uint256 _number3, PRIZE_TYPE _type, uint256 _quantity, EnumerableSet.UintSet storage _set) internal {
        ...
        for(uint256 i = 0; i < _quantity; i++) {
            if(numberWinner.length > i){
                _value = numberWinner[i] % typePrize[_type].digit;
            }else{
                _value = uint256(keccak256(abi.encodePacked(_rand, i))) % typePrize[_type].digit;
            }
