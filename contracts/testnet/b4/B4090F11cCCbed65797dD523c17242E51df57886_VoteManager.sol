...
contract VoteManager {
    ...
    function getProposalAtIndex(uint256 proposalAtIndex) public view returns (ProposalProperties memory) {
        return proposalArray[proposalAtIndex];
    }

    function getUserInfo(uint256 proposalAtIndex, address account) public view returns(bool, uint256) {
        proposalAtIndex -= 1;
        return Proposal(proposalArray[proposalAtIndex].contractAddress).getUserInfo(account);
    }

    function getVotingPower(uint256 proposalAtIndex, address account) public view returns (uint256) {
        proposalAtIndex -= 1;
        return Proposal(proposalArray[proposalAtIndex].contractAddress).getVotingPower(account);
    }

    function getUserDecision(uint256 proposalAtIndex, address account) public view returns (bool) {
        proposalAtIndex -= 1;
        return Proposal(proposalArray[proposalAtIndex].contractAddress).getUserDecision(account);
    }

    function getResultRaw(uint256 proposalAtIndex) public view returns (uint256) {
        proposalAtIndex -= 1;
        return Proposal(proposalArray[proposalAtIndex].contractAddress).getResultRaw();
    }

    function getResultString(uint256 proposalAtIndex) public view returns (string memory) {
        proposalAtIndex -= 1;
        return Proposal(proposalArray[proposalAtIndex].contractAddress).getResultString();
    }
    ...
    function createNewProposal(string calldata exchange, address withdrawer, uint32 epochStart, uint32 epochEnd, uint256 amountTokens) external onlyOwner {
        require(address(currentToken) != address(0), "Token must be set first.");
    ...
    function vote(uint256 proposalAtIndex, bool answer) external {
        ...
        Proposal _contract = Proposal(proposalArray[proposalAtIndex].contractAddress);
