...
library Constants {
    /* Chain */
    uint256 private constant CHAIN_ID = 25; // Cronos Mainnet

    /* Bootstrapping */
    uint256 private constant BOOTSTRAPPING_PERIOD = 28; // 7 days
    uint256 private constant BOOTSTRAPPING_PRICE = 102e16; // 1.02 WCRO
    uint256 private constant BOOTSTRAPPING_SPEEDUP_FACTOR = 3; // 30 days @ 8 hours

    /* Oracle */
    address private constant WCRO = address(0x0f69035cBf6Bbbd605093B7FBa3749A2B47DD3Bd);
    uint256 private constant ORACLE_RESERVE_MINIMUM = 100e18; // 100 WCRO

    /* Bonding */
    uint256 private constant INITIAL_STAKE_MULTIPLE = 1e6; // 100 ESC -> 100M ESCS
    ...
    uint256 private constant EPOCH_START = 1651118400; // Tuesday, 8 February 2022 00:00:00 (GMT+0)
    uint256 private constant EPOCH_PERIOD = 1800; // 30 minutes

    /* Governance */
    uint256 private constant GOVERNANCE_PERIOD = 12; // 12 epochs
    ...
    /* DAO */
    uint256 private constant SEED_INITIAL_LIQUIDITY = 1000000e18; // 1,000,000 ESC for launchpad and initial liquidity
    uint256 private constant ADVANCE_INCENTIVE = 1e18; // 1 ESC
    ...
    /* Regulator */
    uint256 private constant SUPPLY_CHANGE_LIMIT_0 = 2e16; // 2.0%: supply < 500k
    uint256 private constant SUPPLY_CHANGE_LIMIT_1 = 15e15; // 1.5%: supply < 50M
    uint256 private constant SUPPLY_CHANGE_LIMIT_2 = 1e16; // 1.0%: supply = 50M+
    uint256 private constant COUPON_SUPPLY_CHANGE_LIMIT_0 = 4e16; // 4%: supply < 500k
    uint256 private constant COUPON_SUPPLY_CHANGE_LIMIT_1 = 3e16; // 3%: supply < 50M
    uint256 private constant COUPON_SUPPLY_CHANGE_LIMIT_2 = 2e16; // 2%: supply = 50M+
    // - 45%: LPs and Bonded holders
    // - 20%: DAO
    // - 15%: SAFE
    // - 10%: DiamondRoom
    // - 10%: Development/Marketing fund
    uint256 private constant ORACLE_POOL_RATIO = 2250; // 22.5%
    uint256 private constant TREASURY_RATIO = 1000; // 10%
    uint256 private constant DAO_FUND_RATIO = 2000; // 20%
    uint256 private constant SAFE_RATIO = 1500; // 15%
    uint256 private constant DIAMOND_ROOM_RATIO = 1000; // 10%

    /* Deployed */
    address private constant DAO_ADDRESS = address(0x5F1EFd37ab1e4B6a1785e6A9603783837B9875C7);
    address private constant DOLLAR_ADDRESS = address(0x095939207EC875Bb5b6FfCde218f627442e58777);
    address private constant PAIR_ADDRESS = address(0x55D9bD3bD836456f35DF542aF6bA0cDa47fcb0E9);
    address private constant TREASURY_ADDRESS = address(0x991fC16489e672246Be81cb134F5fd57eC507885);
    address private constant DAO_FUND_ADDRESS = address(0x991fC16489e672246Be81cb134F5fd57eC507885);
    address private constant SAFE_ADDRESS = address(0x991fC16489e672246Be81cb134F5fd57eC507885);
    address private constant DIAMOND_HAND_ADDRESS = address(0xe9Eb4fF558D9c14b7A11214518D9AaddbF2Eb79F);

    /**
     * Getters
     */

    function getWftmAddress() internal pure returns (address) {
        return WCRO;
    }
    ...
    function getSeedAndInitialLiquidity() internal pure returns (uint256) {
        return SEED_INITIAL_LIQUIDITY;
    }
    ...
    function getSupplyChangeLimit(uint256 _totalNet) internal pure returns (Decimal.D256 memory) {
        return Decimal.D256({value: (_totalNet < 5e23) ? SUPPLY_CHANGE_LIMIT_0 : (_totalNet < 5e24) ? SUPPLY_CHANGE_LIMIT_1 : SUPPLY_CHANGE_LIMIT_2});
    }

    function getCouponSupplyChangeLimit(uint256 _totalNet) internal pure returns (Decimal.D256 memory) {
        return Decimal.D256({value: (_totalNet < 5e23) ? COUPON_SUPPLY_CHANGE_LIMIT_0 : (_totalNet < 5e24) ? COUPON_SUPPLY_CHANGE_LIMIT_1 : COUPON_SUPPLY_CHANGE_LIMIT_2});
    }
    ...
    function getDaoFundRatio() internal pure returns (uint256) {
        return DAO_FUND_RATIO;
    }

    function getSafeRatio() internal pure returns (uint256) {
        return SAFE_RATIO;
    }

    function getDiamondRoomRatio() internal pure returns (uint256) {
        return DIAMOND_ROOM_RATIO;
    }
    ...
    function getDaoFundAddress() internal pure returns (address) {
        return DAO_FUND_ADDRESS;
    }

    function getSafeAddress() internal pure returns (address) {
        return SAFE_ADDRESS;
    }

    function getDiamondHandAddress() internal pure returns (address) {
        return DIAMOND_HAND_ADDRESS;
    }
}
...
contract Getters is State {
    ...
    function epochTime() public view returns (uint256) {
        Constants.EpochStrategy memory strategy = Constants.getEpochStrategy();

        return epochTimeWithStrategy(strategy);
    }
    ...
    function nextEpochTime() external view returns (uint256) {
        Constants.EpochStrategy memory strategy = Constants.getEpochStrategy();
        return strategy.start.add((epoch() + 1).mul(strategy.period));
    }
...
/*
    Copyright 2020 Empty Set Squad <[email protected]l.com>
...
contract Comptroller is Setters {
    ...
    function increaseSupply(uint256 newSupply) internal returns (uint256, uint256) {
        ...
        // 0-c. Pay out to DAO Fund
        uint256 daoFundReward = newSupply.mul(Constants.getDaoFundRatio()).div(10000);
        mintToDaoFund(daoFundReward);

        // 0-d. Pay out to SAFE
        uint256 safeReward = newSupply.mul(Constants.getSafeRatio()).div(10000);
        mintToSafe(safeReward);

        // 0-e. Pay out to Diamond Room
        uint256 diamondRoomReward = newSupply.mul(Constants.getDiamondRoomRatio()).div(10000);
        allocateToDiamondRoom(diamondRoomReward);

        uint256 rewards = poolReward.add(treasuryReward).add(daoFundReward).add(safeReward).add(diamondRoomReward);
    ...
    function mintToDaoFund(uint256 amount) private {
        if (amount > 0) {
            dollar().mint(Constants.getDaoFundAddress(), amount);
        }
    }

    function mintToSafe(uint256 amount) private {
        if (amount > 0) {
            dollar().mint(Constants.getSafeAddress(), amount);
        }
    }

    function allocateToDiamondRoom(uint256 amount) private {
        if (amount > 0) {
            address _droom = Constants.getDiamondHandAddress();
            dollar().approve(_droom, 0);
            dollar().approve(_droom, amount);
            dollar().mint(address(this), amount);
            IDiamondRoom(_droom).allocateSeigniorage(amount);
        }
    }
...
contract Regulator is Comptroller {
    ...
    function limit(Decimal.D256 memory delta, Decimal.D256 memory price) private view returns (Decimal.D256 memory) {
        ...
        Decimal.D256 memory supplyChangeLimit = Constants.getSupplyChangeLimit(_totalNet);
        
        uint256 totalRedeemable = totalRedeemable();
        uint256 totalCoupons = totalCoupons();
        if (price.greaterThan(Decimal.one()) && (totalRedeemable < totalCoupons)) {
            supplyChangeLimit = Constants.getCouponSupplyChangeLimit(_totalNet);
        }
...
contract Implementation is State, Bonding, Market, Regulator, Govern {
    ...
    function initialize() initializer public {
        ...
        // For seed pool (run for 4 weeks)
        mintToAccount(msg.sender, Constants.getSeedAndInitialLiquidity());
    }
