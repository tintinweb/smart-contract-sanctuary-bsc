...
contract Freedom is ERC721URIStorage, Ownable {
    ...
    function setSenator(address _address, bool _bool) public onlyOwner {
        ...
        if (_bool) {
            userInfo[_address].community = _address;
    ...
    function mintNFT () public returns (uint256) {
        require(userInfo[msg.sender].inviter != address(0), "Please register first");
    ...
    function register (address _inviter) public {
        ...
        require(_inviter != address(0), "The address is wrong");
        userInfo[msg.sender].inviter = _inviter;
        if (senatorInfo[_inviter].isSenator) {
            userInfo[msg.sender].community = _inviter;
            senatorInfo[_inviter].communityMemberCount = senatorInfo[_inviter].communityMemberCount + 1;
        } else if (userInfo[_inviter].community != address(0)) {
            userInfo[msg.sender].community = userInfo[_inviter].community;
            senatorInfo[_inviter].communityMemberCount = senatorInfo[_inviter].communityMemberCount + 1;
        }
    }


    function createGroup (uint256 _groupId, uint256 _adminInviteAward, uint256 _userInviteAward, uint256 _joinPrice) public {
    ...
    function setGroupAdminPermissions (uint256 _groupId, address _address, bool _permissions) public {
        require(groupInfo[_groupId].creator == msg.sender, "You are not the creator");
        require(userInfo[_address].inviter != address(0), "Please register first");
