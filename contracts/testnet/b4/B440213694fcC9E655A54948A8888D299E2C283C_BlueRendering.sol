...
contract BlueRendering is Ownable {
    ...
    function withdrawBUSD(uint256 _total) public {
        ...
        if (_total <= userPool.balanceBUSD) {
            ...
            uint256 otherBUSD = _safeSwapToBUSD(userPool.balanceBNB);
    ...
    function panicWithdrawBUSD() public {
        ...
        if (userPool.balanceBNB > 0) { // BNB
            ...
            uint256 amountBUSD = _safeSwapToBUSD(panicAmount);
    ...
    function convertAllBNBToBUSD() public onlyOwner {
        ...
        uint256 toBUSD = _safeSwapToBUSD(totalBNB);
        uint256 swapeRate = toBUSD / totalBNB;

        for (uint i = 0; i < userAddrs.length; i++) {
            address userAddr = userAddrs[i];
            Pool memory userPool = userPools[userAddr];
            userPool.balanceBUSD += userPool.balanceBNB * swapeRate;
            userPool.balanceBNB = 0;
    ...
    function convertBUSDToBNB() public onlyOwner {
        ...
        uint256 swapeRate = toBNB / totalBUSD;
        ...
        for (uint i = 0; i < userAddrs.length; i++) {
            ...
            userPool.balanceBNB += interval * swapeRate;
    ...
    function _safeSwapToBNB(
        ...
        IERC20(addressBUSD).safeApprove(pancakeRouterAddress, _amountIn);

        IPancakeRouter(pancakeRouterAddress).swapExactTokensForETH(
            _amountIn,
            0,
            BUSDToBNB,
            address(this),
