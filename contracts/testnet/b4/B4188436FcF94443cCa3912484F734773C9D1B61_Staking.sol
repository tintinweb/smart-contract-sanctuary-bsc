    ...
    ERC20Upgradeable
{
    ...
    function enter(uint256 _amount) external isRewardSet {
    ...
    function leave(uint256 _share) external isRewardSet {
        IRewardPool(rewardPoolAddress).wave();
        uint256 totalShares = totalSupply();
        ...
        totalStaked[msg.sender] -= _share * balanceOf(msg.sender)/totalStaked[msg.sender];
        emit UnstakeECT(msg.sender, what);
    }

    function calculateReward() external view returns (uint){  
        return balanceOf(msg.sender).mul(ectAddress.balanceOf(address(this))).div(
            totalSupply()
        ).sub(totalStaked[msg.sender]);
    }
