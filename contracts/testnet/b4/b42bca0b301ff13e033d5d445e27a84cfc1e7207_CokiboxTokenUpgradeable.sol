...
contract CokiboxTokenUpgradeable is
    OwnableUpgradeable,
    ...
    IERC721ReceiverUpgradeable
{
    ...
    mapping(uint256 => bool) public nftOrderIds;
    ...
    event Buycard(address indexed account, uint256 count);
     ...
     * buy room card
     */
    function buycard(uint256 count) external {
        uint256 amount = count.mul(1e18);
        USDT.transferFrom(_msgSender(), address(this), amount);

        emit Buycard(_msgSender(), count);
    }

    function withdrawBalance(address payable account, uint256 amount) external onlyOwner {
