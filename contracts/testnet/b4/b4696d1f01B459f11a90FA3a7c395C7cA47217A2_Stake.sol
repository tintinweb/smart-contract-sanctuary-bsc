...
contract Stake is Ownable {
    ...
    function stake(uint256 _amount, address _referral) public returns(bool) {
        ...
        require((_amount / 10**20) * 10 **20 == _amount, "Amount must be multiply by 100");
    ...
    function claim(bytes32 _hash, uint8 _v, bytes32 _r, bytes32 _s, string memory message) public returns (bool){      
        ...
        uint256 amount = st2num(parts[1]);  
        if(ecrecover(prefixedHashMessage, _v, _r, _s) == signer && hashedMessage == _hash){        
            require(keccak256(abi.encodePacked(msg.sender)) == keccak256(abi.encodePacked(parts[0])),"You are not a staker");  
            token.transfer(msg.sender, amount -  userDetails[msg.sender].totalClaimed);
