...
contract DCApp{
    ...
    */
    ...
    event occurenceExecuted(uint timestamp, address userAddress, uint totalOccurence, uint currentOccurence);
    ...
    function addNewDCAToUser(uint _period, uint _totalOccurences, uint _amountPerOccurrence, address _tokenIn, address _tokenOut)
        ...
        mapDCA[msg.sender][timestamp] = userDCAData(_period, _totalOccurences, 0, _amountPerOccurrence, _tokenIn, _tokenOut,
                                                    _totalOccurences * _amountPerOccurrence, buildSwapPath(_tokenIn, _tokenOut));
    ...
    function executeSingleUserDCA(address user, uint DCAStartDate ) public {
        ...
        /*  Execute a single occurence of a single DCA for the user
            ...
            Steps :
             ...
             TODO: 
                - Check the amount of _tokenOut before and after the swap to get the exact obtained amount
                    Might not be possible because of security risks 
                    Other solution is to directly send the OUT token to the user instead of stocking it here
        */
        ...
        swap(userDCA.tokenIn, userDCA.tokenOut, userDCA.swapPath, userDCA.amountPerOccurence, amountOut, user);
