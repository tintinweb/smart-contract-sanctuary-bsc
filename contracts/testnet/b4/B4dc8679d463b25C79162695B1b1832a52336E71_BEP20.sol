...
contract ENTERNAME is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    ENTERNAMEDividendTracker public dividendTracker;
    ...
    uint256 public totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee).add(burnFee);
    ...
    constructor() BEP20("ENTERNAME", "ENTERSYMBOL") 
    {
        dividendTracker = new ENTERNAMEDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "ENTERNAME: The dividend tracker already has that address");
        ENTERNAMEDividendTracker newDividendTracker = ENTERNAMEDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "ENTERNAME: The new dividend tracker must be owned by the ENTERNAME token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ENTERNAME: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ENTERNAME: Account is already the value of 'excluded'");
    ...
    function setBUSDRewardsFee(uint256 value) external onlyOwner{
        ...
        totalFees = BUSDRewardsFee.add(liquidityFee).add(marketingFee).add(buybackFee).add(burnFee);
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ENTERNAME: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ENTERNAME: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "ENTERNAME: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ENTERNAME: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override 
    {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) 
        {
            ...
            if (marketingFee > 0 || buybackFee > 0 || liquidityFee > 0) {
                uint256 swapableFees = marketingFee.add(buybackFee).add(liquidityFee);
    ...
    function swapAndSendDividends(uint256 tokens) private{
        ...
        if (success) {
            try dividendTracker.distributeBUSDDividends(dividends) {} catch {}
...
contract ENTERNAMEDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ENTERNAME_Dividend_Tracker", "ENTERNAME_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2_000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ENTERNAME_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ENTERNAME_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ENTERNAME contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ENTERNAME_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ENTERNAME_Dividend_Tracker: Cannot update claimWait to same value");
