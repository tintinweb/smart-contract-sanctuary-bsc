...
contract VideoNFT is ERC721, ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("videonft.sg", "videonft.sg"){
        commissionRate=10;
        creatorRate=80;
   ...
   function _setTokenURI(uint256 tokenId, string memory _tokenURI)
    ...
    function buyNFT(BuyNFTStruct calldata data) public payable {
        ...
        address nftCreator = data.nftCreator;

        if (data.refAddress != address(0)) {

            refAmount = (data.price * commissionRate) / 100;
            creatorAmount = (data.price * creatorRate) / 100;
            price = data.price - refAmount-creatorAmount ;
        }

         if (data.refAddress == address(0)) {
            creatorAmount = (data.price * creatorRate) / 100;
            price = data.price - creatorAmount;
        }
