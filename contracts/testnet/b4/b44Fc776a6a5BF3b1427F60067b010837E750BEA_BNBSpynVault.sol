...
contract BNBSpynVault is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function deposit(address referrer) external payable updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if (amount > 0) {
            uint256 depositFeeAmount = amount.mul(depositFee).div(10000);
            if (depositFeeAmount > 0) {
                payable(treasury).transfer(depositFeeAmount);
            }
            uint256 feeTaken = payDepositReferralCommission(msg.sender, amount, address(0));
            amount = amount.sub(feeTaken).sub(depositFeeAmount);
        }
    ...
    function depositTokenPair(uint256 amountSpyn, address referrer) external payable updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if (amountBNB > 0) {
            uint256 depositFeeAmount = amountBNB.mul(depositFee).div(10000);
            if (depositFeeAmount > 0) {
                payable(treasury).transfer(depositFeeAmount);
            }
            uint256 feeTaken = payDepositReferralCommission(msg.sender, amountBNB, address(0));
            amountBNB = amountBNB.sub(feeTaken).sub(depositFeeAmount);
        }
    ...
    function depositLP(uint256 amount, address referrer) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        _depositLP(msg.sender, amountPairReceived, referrer);
    }
    ...
    function _depositLP(address account, uint256 liquidityAmount, address referrer) internal {
        if (
            ...
            spynReferral.recordReferral(account, referrer);
        }
        //stake in farms
    ...
    function harvest(bool isReceiveBnb) external updateReward(msg.sender) nonReentrant noCallFromContract waitForCompound {
        ...
        if (reward > 0) {
            ...
            if(isReceiveBnb) {
                ...
                uint256 harvestFeeAmount = bnbAmount.mul(harvestFee).div(10000);
                if (harvestFeeAmount > 0) {
                    payable(treasury).transfer(harvestFeeAmount);
                }
                uint256 feeTaken = payHarvestReferralCommission(msg.sender, bnbAmount, address(0));
                bnbAmount = bnbAmount.sub(feeTaken).sub(harvestFeeAmount);
                payable(msg.sender).transfer(bnbAmount);
            }else{
                uint256 spynAmount = swapBnbToSpyn(amountBNB).add(amountSpyn);
                uint256 harvestFeeAmount = spynAmount.mul(harvestFee).div(10000);
                if (harvestFeeAmount > 0) {
                    spyn.transfer(treasury, harvestFeeAmount);
                }
                uint256 feeTaken = payHarvestReferralCommission(msg.sender, spynAmount, address(spyn));
                spynAmount = spynAmount.sub(feeTaken).sub(harvestFeeAmount);
            ...
            for (uint256 i = 0; i < harvestReferralCommissionRates.length; i ++) {
                ...
                if (commissionAmount > 0 && referrersByLevel[i] != address(0)) {
                    ...
                    if (_token == address(0)) {
                        ...
                        spynReferral.recordReferralCommission(referrersByLevel[i], commissionAmount, address(0), 0, i);
                    } else {
                        IERC20(_token).transfer(referrersByLevel[i], commissionAmount);
                        spynReferral.recordReferralCommission(referrersByLevel[i], commissionAmount, _token, 0, i);
                    }
        ...
        if (address(spynReferral) != address(0)) {
            ...
            for (uint256 i = 0; i < depositReferralCommissionRates.length; i ++) {
                ...
                if (commissionAmount > 0 && referrersByLevel[i] != address(0)) {
                    ...
                    if (_token == address(0)) {
                        payable(referrersByLevel[i]).transfer(commissionAmount);
                        spynReferral.recordReferralCommission(referrersByLevel[i], commissionAmount, address(0), 1, i);
                    } else {
                        IERC20(_token).transfer(referrersByLevel[i], commissionAmount);
                        spynReferral.recordReferralCommission(referrersByLevel[i], commissionAmount, _token, 1, i);
                    }
