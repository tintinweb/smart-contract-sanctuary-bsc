...
contract StakingRobert {
    ...
    function changeAPY(uint _value) public {
        require(msg.sender == owner, "You are not allowed to do this!");
        rewardAPYRate = _value;
    }
    ...
    function stake30Days(address _upline, uint _amount) external updateReward(msg.sender) {
            require(_amount > 39999 ether, "MIN.  40,000 POTENT TOKEN");
    ...
    function stake60Days(address _upline, uint _amount) external updateReward(msg.sender) {
           require(_amount > 39999 ether, "MIN.  40,000 POTENT TOKEN");
            Player storage player = players[msg.sender];
            if (player.deposits == 0 && _upline != address(0)) {
            uint rewardOne = _amount * feeOne / 100;
    ...
    function stake90Days(address _upline, uint _amount) external updateReward(msg.sender) {
            require(_amount > 39999 ether, "MIN.  40,000 POTENT TOKEN");
            Player storage player = players[msg.sender];
            if (player.deposits == 0 && _upline != address(0)) {
            uint rewardOne = _amount * feeOne / 100;
