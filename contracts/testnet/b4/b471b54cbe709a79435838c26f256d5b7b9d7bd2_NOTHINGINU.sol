...
contract NOTHINGINU is IBEP20, Auth { 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, launchMode) : amount;
    ...
    function takeFee(address sender, uint256 amount, bool isSell, bool LAUNCHMode) internal returns (uint256) {
        uint256 feeAmount;             
        if (LAUNCHMode && islaunchExempt[sender]== false){
            if(isSell){
               feeAmount = amount.mul(totalFee).mul(5).div(feeDenominator); //set sell multiplier
            } else {
               feeAmount = amount.mul(totalFee).mul(8).div(feeDenominator); //set Buy multiplier
            }
        } else {
