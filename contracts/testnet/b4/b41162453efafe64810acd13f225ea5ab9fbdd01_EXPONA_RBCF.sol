...
contract EXPONA_RBCF {
    ...
    function Registration(uint _referrerID, uint _coreferrerID,uint256 _amountStable, uint256 _amountBep) public payable {
        ...
        if(msg.value >0){
            ...
            tokenReward =  _amountStable * 1e18 / tokenPrice();
            tokenIsAccpting = false;
        }else {
            tokenReward =  REGESTRATION_FESS / tokenPrice() * 1e18;
            require(_amountBep == tokenReward, "Incorrect_Fee_Should_Be_Equal_TO_CURRENT_REWARD");
            require(token.allowance(msg.sender, address(this)) >= tokenReward,"NEED_TO_APPROVE_TOKEN_EQUAL_TO_CURRENT_REWARD");
            token.transferFrom(msg.sender, address(this), tokenReward);
            tokenIsAccpting = true;
        }
        ...
        autoPool( msg.sender, msg.value, tokenIsAccpting );
        payReferral(1, msg.sender, msg.value, tokenIsAccpting);
        paycoReferral(1, msg.sender, msg.value, tokenIsAccpting);
    ...
    function heightPayment(
        ...
        if (
            ...
            heightPayment(_user, batch, id, h + 1,_value, isToken);
        } else {
            if (
                (h > 2 && h <= height) &&
    ...
    function autoPool(address _user, uint256 _value,bool isItToken) internal {
        bool sent = false;
        ownerPaid = false;
        uint i;
        for (i = 1; i < currUserID; i++) {
            if (users[userList[i]].batchPaid < batchSize) {
                ...
                heightPayment(_user, i, temp, heightCounter,_value, isItToken);
                i = currUserID;
            }
        }
    }
