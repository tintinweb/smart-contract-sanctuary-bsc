...
contract DEXINFT is ERC721URIStorage, ERC721Enumerable, IERC721TokenCreator, Ownable {
    ...
    /**
     * @notice Toggle minter by the address
     *
     * @param _address The minter wallet address.
     */
    function toggleMinter(address _address) external onlyOwner {
        minters[_address] = !minters[_address];
    }
    ...
    function mint(address to, string memory _tokenURI, uint256 _royalty) public returns (uint256) {
        require(msg.sender == owner() || minters[msg.sender] == true, "Caller is not a minter");
        require(marketContractAddress != address(0), "Market contract is not set");
