...
contract BnqNodeMining is OwnableUpgradeable {
    ...
    function setNodeInfo(
        uint[12] memory gear_,
        uint[12] memory out_
    ) public onlyOwner {
        // require(gear_.length == out_.length, "len worry");
    ...
    function setStaticFeeRate(uint u_) external onlyOwner {
        staticFeeRate = u_;
    }
    ...
    function checkNodeNumber(address addr) public view returns (uint) {
        uint newN = userReferLists[addr].length;
        uint oldN = checkUserReferList(addr).length;
        return (newN + oldN);
    }
    ...
    function checkOldAmount(
        ...
        (uint _amount, uint _total, uint _claimed, , , , , , , , ) = IMining(
            BNQSTAKE
        ).userInfo(user_);
        return (_amount, _total, _claimed);
    }
    ...
    function claimLink() external onlyEOA returns (uint) {
        UserInfos storage info = userInfos[msg.sender];
        (uint rew, uint bnqOut) = calculateLink(msg.sender);
    ...
    function claimV5Dividend() external onlyEOA {
        UserInfos storage info = userInfos[msg.sender];
    ...
    function _calculateRew(
        ...
        if (info.buyAmount == 0) {
            ...
            uint rate = ((info.oldAmount / 2) + info.buyAmount) / 100 / 86400;
            rew =
    ...
    function buyNode(address inv_, uint lv_) external onlyEOA {
        ...
        require(lv_ > 0 && lv_ > info.nodeLevel, "lowLv");
        // inherit
        bool isA;
        ...
        uint i = lv_ - 1;
        // uint diff = nodeGear[i] - info.buyAmount;
        ...
        emit BuyNode(msg.sender, inv_, diff);
    }

    function claimStaticRew() external onlyEOA {
        UserInfos storage info = userInfos[msg.sender];
        require(info.buyAmount > 0, "Invalid node");
        require(
        ...
        uint tempRew = (rew * (100 - staticFeeRate)) / 100;
        //updata
        info.claimedTime = block.timestamp;
        info.claimedQuota += rew;
        info.toClaimQuota = 0;
        rewardStruct[msg.sender].staticRewrd += rew;
        ...
        // v5
        uint _v = (tempRew * 10) / 100;
        _processV5(_v);

        // link
        _processLink(msg.sender, tempRew);

        if (bnqFee > 100 ether) {
            dividendsSP();
        }

        emit ClaimStaticRew(msg.sender, temp, rew);
    }
    ...
    function claimDynamicRew(
        ...
        require(_total > info.claimedQuota, "Quota exhaustion");

        bytes32 _hash = keccak256(abi.encodePacked(rew_, time_, msg.sender));
        address a = ecrecover(_hash, v, r, s);
        require(a == banker, "no banker");
        require(block.timestamp < time_, "out of time");
        ...
        if ((info.claimedQuota + info.toClaimQuota + rew) > _total) {
            rew = _total - info.claimedQuota;
        }
    ...
    function dividendsSP() public {
        if (bnqFee > 100 ether) {
            // uint a = bnqFee / 2;
            // uint oldBa = IERC20(SP).balanceOf(BNQSHARE);
            // reBuy(a, BNQ, SP, BNQSHARE);
            // reBuy(a, BNQ, SP, burnAddress);
            // uint newBa = IERC20(SP).balanceOf(BNQSHARE);
            // uint ba = newBa - oldBa;
    ...
    function checkOne(
        address addr
    ) public view returns (uint[7] memory _nodeList, uint[2] memory _linkList) {
        UserInfos storage info = userInfos[addr];

        _linkList[0] = info.linkToClaim;

        _linkList[1] = rewardStruct[addr].linkReward;
        _nodeList[0] = info.nodeLevel;
        uint total = info.thisQuota + info.oldQuota;
        if (total != 0) {
            uint a = total - info.claimedQuota;
            _nodeList[1] = a;
        }

        _nodeList[2] = info.claimedQuota;
        _nodeList[3] = info.claimedBNQAmount;

        _nodeList[4] = TVL;
        _nodeList[5] = BNQBURN;
        (, _nodeList[6]) = calculateReward(addr);
    }
