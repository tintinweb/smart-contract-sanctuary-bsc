...
contract NFTMarketplace is ERC721URIStorage {
    ...
    function getTotalFees(uint256 _marketId) public view returns (uint256) {
        return
            (marketItems[_marketId].price * listingPercentageBasisPoints) /
            10000;
    }
    ...
    function createMarketItem(
        ...
        if (checkRoyalties(address(_nft))) {
            ...
            (_royaltyReceiver, royaltyValue_) = nftRoyalty.royaltyInfo(
                _tokenId,
                _price
            );
        } else {
    ...
    function purchaseMarketItem(uint256 _marketId) external payable {
        uint256 totalFees = getTotalFees(_marketId);

        MarketItem storage item = marketItems[_marketId];
        ...
        require(
            msg.value >= totalFees + item.price,
            "Purchase Market Item: Not enough balance to cover item price and market fee!"
        );
        require(!item.sold, "Purchase Market Item: Item is already sold!");

        if (item.royaltyValue > 0) {
            payable(item.royaltyReceiver).transfer(item.royaltyValue);
        }

        payable(OWNER).transfer(totalFees);

        payable(item.seller).transfer(
            item.price - totalFees - item.royaltyValue
        );
    ...
    function fetchItemsListed() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (marketItems[i + 1].seller == _msgSender()) {
                itemCount += 1;
            }
        }
