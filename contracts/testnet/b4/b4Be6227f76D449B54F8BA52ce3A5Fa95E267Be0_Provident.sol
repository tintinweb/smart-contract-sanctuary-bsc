...
contract ERC20 is Context, IERC20 {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        require(account != address(0x000000000000000000000000000000000000dEaD), "ERC20: burn from the dead address");
...
contract Provident is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_LAUNCHPAD_DISTRIBUTION = 300000 ether;
    // Initial distribution for adding liquidity using deposit fee from genesis pool
    uint256 public constant INITIAL_PVN_FOR_LP_DISTRIBUTION = 150000 ether;
    // Initial distribution for community fund
    uint256 public constant INITIAL_PVNFUND_DISTRIBUTION = 550000 ether;
    ...
    uint256[] public taxTiersRates = [3000, 3000, 3000, 3000, 3000, 2000, 1500, 0, 0, 0, 0, 0, 0, 0];
    ...
    /**
     * @notice Constructs the PVN ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("Provident", "PVN") {
        // Mints 1 PVN to contract creator for initial pool setup
    ...
    function _getPvnPrice() internal view returns (uint256 _pvnPrice) {
        try IOracle(pvnOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Pvn: failed to fetch PVN price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _pvnPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_pvnPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPvnOracle(address _pvnOracle) public onlyOperatorOrTaxOffice {
        require(_pvnOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints PVN to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PVN to mint to
    ...
    function _transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPvnPrice = _getPvnPrice();
            currentTaxRate = _updateTaxRate(currentPvnPrice);
            if (currentPvnPrice < burnThreshold) {
                burnTax = true;
            }
            if(recipient != pair)
                currentTaxRate = 0;
    ...
    function distributeReward(
        ...
        require(_launchPad != address(0), "!_launchPad");
        require(_pvnBusd != address(0), "!_pvnBusd");
        require(_pvnFund != address(0), "!_pvnFund");
        rewardPoolDistributed = true;
        _mint(_launchPad, INITIAL_LAUNCHPAD_DISTRIBUTION);
        _mint(_pvnBusd, INITIAL_PVN_FOR_LP_DISTRIBUTION);
        _mint(_pvnFund, INITIAL_PVNFUND_DISTRIBUTION);
    }
