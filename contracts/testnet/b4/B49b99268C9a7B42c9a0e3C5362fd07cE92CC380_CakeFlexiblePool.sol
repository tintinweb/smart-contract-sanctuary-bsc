...
contract CakeFlexiblePool is Ownable, Pausable {
    ...
    function withdraw(uint256 _shares) public {
        ...
        if (earnAmount > 0) {
            ...
            totalShares -= performanceFeeShares;
            /**
                //Should run this code now , but in order to avoid running 'cakePool.withdrawByAmount' twice which will cost more gas,just put it below.
                cakePool.withdrawByAmount(currentPerformanceFee);
                currentPerformanceFee = available() >= currentPerformanceFee ? currentPerformanceFee : available();
                token.safeTransfer(treasury, currentPerformanceFee);
            */
        }
        ...
        if (currentWithdrawFee > 0) {
            currentWithdrawFee = available() >= currentWithdrawFee ? currentWithdrawFee : available();
