...
contract BingoV2 {
     ...
     */
    function placeBingoCard(uint16 cardId) external {
        require(cardId <= cardAmount, "This card is not exits");
        /// check round is not in locking time
        /// add round to user round
        if (!userEnrolledRounds[msg.sender][currentEpoch]) {
            userRounds[msg.sender].push(currentEpoch);
            userEnrolledRounds[msg.sender][currentEpoch] = true;
        }

        /// add card to ledger
        ledgers[currentEpoch][msg.sender].push(
            BetInfo(currentEpoch, cardId, false)
        );

        /// update round's total card
        rounds[currentEpoch].players += 1;
