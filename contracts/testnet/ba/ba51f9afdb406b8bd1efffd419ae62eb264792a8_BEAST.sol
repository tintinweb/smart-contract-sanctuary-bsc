...
contract BEAST is Context, IBEP20, Ownable{
    ...
    function excludeFromFee(address account) public onlyOwner {
	   require(_isExcludedFromFee[account] != true, "Account is already exclude");
       _isExcludedFromFee[account] = true;
    }
    
    function includeInFee(address account) public onlyOwner {
	     require(_isExcludedFromFee[account] != false, "Account is already include");
    ...
    function setFeePercent(uint256 _reflectionFee, uint256 _marketingFee, uint256 LiquidityFee) external onlyOwner() {
        require(_reflectionFee.add(_marketingFee).add(LiquidityFee)  <= 2000 , "Max fee limit reached for fee");
    ...
    function _transfer(address from, address to, uint256 amount) private 
	{
		...
		if(automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && sponsor[to] != address(0))
		{
			...
			{
			    _basicTokenTransfer(address(referralRewardWallet), sponsor[to], rewardAmountSponsor, false);
			}
		}
