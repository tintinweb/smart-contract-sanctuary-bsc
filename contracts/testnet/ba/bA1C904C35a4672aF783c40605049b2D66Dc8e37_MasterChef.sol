...
//                 margin += router.calcMargin(_assets[i], _borrowedAsset, _amounts[i]);
        ...
        token0Amount = borrowedAmount *
            ...
            10**((token0 == BNB_ADDRESS) ? 8 : IBEP20Metadata(token0).decimals()) /
            (2 * 10**vault.getAssetDecimals(_borrowedAsset) * priceOracle.getPrice(token0));

        token1Amount = borrowedAmount *
            priceOracle.getPrice(_borrowedAsset) *
            10**((token1 == BNB_ADDRESS) ? 8 : IBEP20Metadata(token1).decimals()) /
...
//      TODO:   uint256 withdrawalAmount = router.withdrawal(vault.getLPToken(_lpPositionId), _lpTokenAmount, borrowedAsset);
        ...
        if (currentLPPositionDebt >= _lpTokenAmount) {
            vault.updateMarginBorrowBalance(_lpPositionId, currentLPPositionDebt - _lpTokenAmount);
            router.transferToVault(borrowedAsset, _lpTokenAmount);
        } else {
            ...
            if (borrowedAsset == _withdrawableAsset) {
                withdrawalAmount = _lpTokenAmount - currentLPPositionDebt;
            } else {
                withdrawalAmount = (_lpTokenAmount - currentLPPositionDebt) *
            ...
            IIToken(vault.getAssetITokenAddress(_withdrawableAsset)).
                mint(msg.sender, withdrawalAmount); // iToken minting to depositor
            vault.updateTotalCollateralBalance(_withdrawableAsset);
            router.transferToVault(_withdrawableAsset, withdrawalAmount);
        }
