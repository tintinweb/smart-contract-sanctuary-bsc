...
contract Jackpot is ReentrancyGuard {
    ...
    uint256[] public jackpotAllocation = [2, 3, 4, 5];
    uint256[] public grandJackpotAllocation = [200, 300, 400, 500];
    ...
    function claimJackpot(uint256 _gid) external nonReentrant lockTheJackpot {
        ...
        uint256 rewardAmount = calculateJackpotReward(_gid);
    ...
    function claimGrandJackpot(uint256 _gid) external nonReentrant lockTheJackpot {
        ...
        uint256 rewardAmount = calculateGrandJackpotReward(_gid);
        require(BUSD.balanceOf(address(this)) >= rewardAmount, "Insufficient BUSD in contract");
    ...
    function calculateJackpotReward(uint256 _gid) internal view returns(uint256) {
    ...
    function calculateGrandJackpotReward(uint256 _gid) internal view returns(uint256) {
        uint256 contractBalance = BUSD.balanceOf(address(this));
