...
contract UbkiNFT is ERC721, ERC721URIStorage, ERC721Enumerable, Ownable {
    ...
    struct MintingInfos {
        uint256 ridesCount;
        uint256 sharesTotal;
        mapping(uint256 => uint256) sharesCount;
    }
    mapping(address => MintingInfos) private passengerRegistry;

    // TODO: delete?
    mapping(uint => uint) public tokenShares;

    uint256 public totalShares;

    constructor() ERC721("UbkiNFT", "ANFT") {}

    event NFTMinted(address passenger, uint256 shares);
    ...
    function sharesOf(address wallet) public view returns (uint) {
        return passengerRegistry[wallet].sharesTotal;
    }

    function sharesDetailOf(
        address wallet,
        uint256 shareValue
    ) public view returns (uint256) {
        return passengerRegistry[wallet].sharesCount[shareValue];
    }

    function addRide(address passenger) public {
        passengerRegistry[passenger].ridesCount += 1;
    }

    function ridesOf(address wallet) public view returns (uint256) {
        return passengerRegistry[wallet].ridesCount;
    }

    /**
     * @dev Safemint NFT and updates the variables tokenShares,
     *      sharesMinted and totalShares. Only callable from owner.
     * @param to The address to mint NFT for.
     * @param rank NFT share amount.
     */
    function safeMint(address to, uint rank) public onlyOwner {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _setTokenURI(
            tokenId,
            (string(abi.encodePacked(Strings.toString(rank), "", ".json")))
        );

        totalShares += rank;

        // with storing only shares:
        tokenShares[tokenId] = rank;

        // with MintingInfos struct:
        addWalletDetails(to, rank);
        emit NFTMinted(to, rank);
    }

    function addWalletDetails(address to, uint256 shares) internal {
        passengerRegistry[to].sharesTotal += shares;
        passengerRegistry[to].sharesCount[shares] += 1;
    }

    /*
     * @dev Destroys tokenId.
     * @param tokenId Id of token to destroy.
     */
    function _burn(
        uint256 tokenId
     ...
     * @param interfaceId If od interface.
     */
...
contract UbkiReserve is Ownable {
    UbkiToken public ubkiToken;
    UbkiNFT public ubkiNFT;
    ...
    constructor (
        ...
        ubkiNFT = UbkiNFT(_NFTFactory);
        deployDate = block.timestamp;
        counterDistribution = 365;
    }

    /*
     * @dev Distributes 30 percent of the UbkiToken it contains to all UbkiNFT holders in proportion to their share.
     * Can only be called after 365 days. Then once every 30 days after that date.
     */
    function distribute() public onlyOwner {
        uint oneYear = deployDate + 365 days;
        uint lastMonth =  deployDate + counterDistribution * 1 days;
        require(block.timestamp  >= oneYear , "Need to wait 365 days before initial distribution");
        require(block.timestamp  >= lastMonth , "Need to wait 30 days after last distribution");


        uint total = ubkiToken.balanceOf(address(this)) * 30 / 100; // 30 percent of the UbkiTokens in the pool
        uint totalSupply = ubkiNFT.totalSupply();
        uint totalShares =  ubkiNFT.totalShares();
        ubkiToken.approve(address(this), 9*10**50);
        //Send UbkiTokens in proportion to the share of each NFT in the collection.
        for (uint i = 0; i < totalSupply; i++) {
            //address holder of the i-th NFT
            address nftOwner = ubkiNFT.ownerOf(i);
            //share of the ith NFT
            uint shares = ubkiNFT.tokenShares(i);
            //Transfer of the UbkiubkiTokens to the ith NFT holder
            uint256 amount = FMaths.div(
                FMaths.mul(shares, total, 0, ubkiToken.decimals(), ubkiToken.decimals()),
                totalShares,
                ubkiToken.decimals(), 0, ubkiToken.decimals()
            );
            ubkiToken.transferFrom(address(this), nftOwner, amount);
        }
