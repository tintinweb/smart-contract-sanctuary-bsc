...
contract NetGymStreet is OwnableUpgradeable {
    ...
    function setLevelOfUser(uint256 _userLevel) external {
        level = _userLevel;
    }

    function setHasNft(bool _hasNft) external {
        hasNft = _hasNft;
    }

    function setIsPurchasedThisMonth(bool _isPurchasedThisMonth) external {
        isPurchasedThisMonth = _isPurchasedThisMonth;
    }

    function setLevel(uint256[] calldata _levels) external {
        for (uint256 i = 0; i < _levels.length; i++) {
            levels.push(_levels[i]);
        }
    }

    function getLengthlevels() public view returns(uint256) {
        return levels.length;
    }

    function deleteLevels() public {
    ...
    function distributeRewards(
        ...
        while (
            ...
            if (index <= _level && hasNft && referrerId != 1) {
    ...
    function _getUserLevel(address _user) private view returns (uint256 _levelNFT) {
        ...
        // bool isSpecial = IGymFarming(0x03ac9DE519e006E0f9e173392B4b8657E57fc683)
            // .isSpecialOfferParticipant(_user);
        // uint256 _level = IGymMLMQualifications(mlmQualificationsAddress).getUserCurrentLevel(_user);
        // if (isSpecial && _level >= 8 && block.timestamp < 1665403249) {
        //     return 24;
        // }
