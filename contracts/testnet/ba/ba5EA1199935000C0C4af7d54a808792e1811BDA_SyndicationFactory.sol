...
contract SyndicationFactory is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private allSyndications;
    address public operator;    //i.e. backend operator
...
contract Syndication is ERC20, Ownable{
    ...
    function withdraw( uint256 _lpamount ) external {
        ...
        if ( terms.statusTargetNFT == TargetNFTStatus.Auction ) {    // auction
            require(getBidStatus() != IProtectedMarketplace.BidStatus.Pending, "Syndication already bidded");
            if ( getBidStatus() == IProtectedMarketplace.BidStatus.Executed ) { // when accepted
                totalValue = totalValue.add(amount);
                _lpamount = 0;
            }
        } else if ( terms.statusTargetNFT == TargetNFTStatus.NotListed ) {    // not listed
            if ( getOfferStatus() == IProtectedMarketplace.OfferStatus.UnderDownsideProtectionPhase ) { // when accepted
                totalValue = totalValue.add(amount);
                _lpamount = 0;
            }
    ...
    function _buyNftOnMarketplace() internal returns (bool) {
        ...
        if (terms.statusTargetNFT == TargetNFTStatus.Fixed) {
            ...
            if ( getBuyOrderStatus() != IProtectedMarketplace.OrderStatus.Active && getBuyOrderStatus() != IProtectedMarketplace.OrderStatus.Bidded ) {
                _refundToInvestors();
                return false;
            }

            if ( getBidStatus() == IProtectedMarketplace.BidStatus.Pending ) {
                marketplace.updateBid{value: address(this).balance}(terms.buyOrderID); // balance will be totalValue
    ...
    function cancelSyndication() external returns (bool) {

        require( operatorAddr == msg.sender, "Only operator can cancel" );
        require( statusSyndication == SyndicationStatus.Active, "Invalid syndication status" );

        if ( terms.statusTargetNFT == TargetNFTStatus.Fixed ) { // fixed
            require( getBuyOrderStatus() != IProtectedMarketplace.OrderStatus.Active, "Cannot cancel" );
        } else if ( terms.statusTargetNFT == TargetNFTStatus.Auction ) {    // auction
            require( getBuyOrderStatus() != IProtectedMarketplace.OrderStatus.Active && getBuyOrderStatus() != IProtectedMarketplace.OrderStatus.Bidded, "Cannot cancel" );
        } else {
            require( getOfferStatus() == IProtectedMarketplace.OfferStatus.Active || getOfferStatus() == IProtectedMarketplace.OfferStatus.Cancelled, "Cannot cancel" );
            if ( getOfferStatus() == IProtectedMarketplace.OfferStatus.Active ) {
                marketplace.cancelOffer(offerID);
            }
        }
