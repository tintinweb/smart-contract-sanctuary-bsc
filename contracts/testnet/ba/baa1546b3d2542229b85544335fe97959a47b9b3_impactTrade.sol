...
contract impactTrade is Initializable, OwnableUpgradeable {
    ...
    function getServiceFee() public view returns (uint256, uint256, uint256) {
...
// ids[0] - tokenId, ids[1] - amount, ids[2] -  nooftoken, ids[3] - nftType
    function _saleToken(
        ...
        address create;
        if (ids[3] == 721) {
            (create, _royal) = ImpactCollective721(impact721)
                .getCreatorsAndRoyalty(ids[0]);
        } else {
            (create, _royal) = ImpactCollective1155(impact1155)
                .getCreatorsAndRoyalty(ids[0]);
        }
        if (
