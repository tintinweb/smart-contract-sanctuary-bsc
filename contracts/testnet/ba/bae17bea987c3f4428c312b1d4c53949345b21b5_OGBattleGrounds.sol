...
library OGBGDatasets {
    ...
    struct PlayerRounds {
        mapping (uint256 => uint256) team_tikt;   
        uint256 spet; 
        uint256 reve; 
    }
    struct Round {
        ...
        uint256 tmid;                               
        uint256 sum_tikt;                           
        mapping (uint256 => bool) team_fall;        
        mapping (uint256 => uint256) team_tikt;     
        uint256 tsend;                              
        bool ended;                                 
        uint256 tiktp0;                             
...
contract OGBattleGrounds is ReentrancyGuard, OGOperators {
    ...
    function endRound() external isActivated onlyOwnerOrOperator nonReentrant {
        ...
        if (validTickets > 0) {
            round_[rID_].tiktp1 = round_[rID_].reve.div(validTickets);
        } else {
    ...
    function buyTickets(uint256 _teamIndex, uint256 _votes) external isActivated notContract nonReentrant {
        ...
        player.lrnd = rID_;
        player.spet = player.spet.add(tollAmount);
    ...
    function getRoundCurrentPrice() external view returns (uint256) {
        return _calcRoundCurrentPrice();
    }

    function setPlayerBookAddr(address _plyrBookAddress) external onlyOwner {
        require(plyrBookAddr_ == address(0x0), "Has been set");
    ...
    function setAgentBookAddr(address _agtBookAddress) external onlyOwner {
        require(agtBookAddr_ == address(0x0), "Has been set");
    ...
    function setPlayerBankAddr(address _plyrBankAddress) external onlyOwner {
        require(plyrBankAddr_ == address(0x0), "Has been set");
    ...
    function setToken(address _tokenAddress) external onlyOwner {
        require(tokenAddr_ == address(0x0), "Has been set");
