...
contract MasterChef is Initializable, Ownable, ReentrancyGuard
{
	...
	constructor(string memory _name, string memory _symbol)
	{
		initialize(msg.sender, _name, _symbol);
	}
	...
	function initialize(address _owner, string memory _name, string memory _symbol) public initializer
	{
		...
		rewardToken = address(new MintableToken(_name, _symbol));
	}
	...
	function addPool(address _token, uint256 _allocPoint, uint256 _startTime) external onlyOwner
	{
		massUpdatePools();
	...
	function updateAllocPoints(uint256 _pid, uint256 _allocPoint) external onlyOwner
	{
		massUpdatePools();
	...
	function pendingReward(uint256 _pid, address _account) external view returns (uint256 _pendingReward)
	{
		...
		if (block.timestamp > _poolInfo.lastRewardTime) {
			if (_poolInfo.amount > 0 && _poolInfo.allocPoint > 0) {
				uint256 _reward = (block.timestamp - _poolInfo.lastRewardTime) * rewardPerSec * _poolInfo.allocPoint / totalAllocPoint;
				if (_reward > 0) {
					_accRewardPerShare += _reward * 1e18 / _poolInfo.amount;
				}
			}
		}
		return _userInfo.amount * _accRewardPerShare / 1e18  - _userInfo.rewardDebt;
	}
	...
	function updatePool(uint256 _pid) public
	{
		PoolInfo storage _poolInfo = poolInfo[_pid];
		if (block.timestamp > _poolInfo.lastRewardTime) {
			_poolInfo.lastRewardTime = block.timestamp;
			if (_poolInfo.amount > 0 && _poolInfo.allocPoint > 0) {
				uint256 _reward = (block.timestamp - _poolInfo.lastRewardTime) * rewardPerSec * _poolInfo.allocPoint / totalAllocPoint;
				if (_reward > 0) {
	...
	function deposit(uint256 _pid, uint256 _amount) external nonReentrant
	{
		...
		if (_userInfo.amount > 0) {
			...
			if (_reward > 0) {
				allocReward -= _reward;
				MintableToken(rewardToken).mint(msg.sender, _reward);
			}
		}
		if (_amount > 0) {
			_userInfo.amount += _amount;
			_poolInfo.amount += _amount;
			IERC20(_poolInfo.token).safeTransferFrom(msg.sender, address(this), _amount);
		}
	...
	function withdraw(uint256 _pid, uint256 _amount) external nonReentrant
	{
		...
		if (_amount > 0) {
			_userInfo.amount -= _amount;
			_poolInfo.amount -= _amount;
			IERC20(_poolInfo.token).safeTransfer(msg.sender, _amount);
		}
		_userInfo.rewardDebt = _userInfo.amount * _poolInfo.accRewardPerShare / 1e18;
	...
	function disburse(address[] memory _accounts, uint256[] memory _amounts) external onlyOwner
	{
	...
	{
		uint256 _reward = credits[msg.sender];
		if (_reward > 0) {
