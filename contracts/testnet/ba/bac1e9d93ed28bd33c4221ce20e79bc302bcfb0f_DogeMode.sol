...
contract DogeMode is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DogeModeDividendTracker public dividendTracker;

    address payable public marketingWallet;
    address payable public dappWallet;
    ...
    constructor() ERC20("DogeMode", "$DGM") {
        marketingWallet = payable(0x91Fc80e0E550aC4717C321e1ef48C95490513515);
        dappWallet = payable(0x91Fc80e0E550aC4717C321e1ef48C95490513515);
        ...
        dividendTracker = new DogeModeDividendTracker(
            payable(this),
            router,
...
contract DogeModeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DogeMode public DogeModeContract;
    ...
    constructor(
        ...
        defaultToken = token;
        DogeModeContract = DogeMode(mainContract);
    ...
    function _transfer(
        ...
        require(false, "DogeMode_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DogeMode_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BitBurnReflect contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(DogeModeContract);

            uint256 prevBalance = DogeModeContract.balanceOf(address(this));
            ...
            {
                uint256 received = DogeModeContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    DogeModeContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
