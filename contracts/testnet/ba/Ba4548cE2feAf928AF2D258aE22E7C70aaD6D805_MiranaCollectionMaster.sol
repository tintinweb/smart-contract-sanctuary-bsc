 ...
 *
 */
contract MiranaCollectionMaster is Ownable {
  ...
  function createToken(
    ...
    address beaconProxy = deployProxy(_getData(_name, _symbol, baseTokenURI), salt);
  ...
  function getAddress(
    ...
    bytes memory bytecode = getCreationBytecode(
      _getData(_name, _symbol, baseTokenURI)
    );
  ...
  function _getData(string memory _name, string memory _symbol, string memory baseTokenURI)
    internal
    pure
    returns (bytes memory)
  {
    return
      abi.encodeWithSignature('__ERC721Mirana_init(string,string,string)', _name, _symbol, baseTokenURI);
  }
}
