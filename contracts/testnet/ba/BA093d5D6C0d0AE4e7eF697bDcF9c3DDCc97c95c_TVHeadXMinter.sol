...
contract TVHeadXMinter is Ownable {
    ...
    mapping(address => bool) isMinted; // for whitelist
    ...
    function whitelistMint(bytes calldata sig) external {
        require(whitelistMinted < devLimit, "Over Whitelist Limit");

        address user = msg.sender;
        require(!isMinted[user], "aleady minted");
        require(verify(user, sig), "signature unmatch");
    ...
    function verify(address sender, bytes memory sig) internal view returns (bool) {
        bytes32 hashStruct = keccak256(abi.encode(sender, chainId, address(this)));
