...
/*
    ReflectFeeCoin template token tokenomics:
        ...
        - 1% fee goes to a "treasury address"
*/
...
contract ReflectFeeCoin is Context, IERC20, Ownable {
    ...
    uint256 public _previoustreasuryFee = _treasuryFee;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        // Don't take any fees if marketing address or treasury addresses aren't set.
        if (_marketingAddress == address(0) || _treasuryAddress == address(0)) {
            takeFee = false;
        }
        ...
        if (
            ...
            uint256 totalFee = _treasuryFee.add(_marketingFee).add(_liqFee);
    ...
    function _takeFees(uint256 amount, address from) private lockTakeFees {
        ...
        uint256 walletFee = _treasuryFee.add(_marketingFee);
        ...
        // Calculate treasury and marketing portions of the swapped ETH (also remember to multiply this factor by 10)
        uint256 treasuryETHPortion = swappedETH.div(totalFeeToBeSwappedForETHMul10).mul(_treasuryFee.mul(10));
        ...
        uint256 totalETHPortionForWallets = treasuryETHPortion.add(marketingETHPortion);
        uint256 liquidityPoolETHPortion = swappedETH.sub(totalETHPortionForWallets);

        // Transfer ETH to fee wallets
        (bool sent, bytes memory data) = _treasuryAddress.call{value: treasuryETHPortion}("");
        require(sent, 'ETH was not sent to treasury');
    ...
    function removeAllFees() private {
        if(_taxFee == 0 && _marketingFee == 0 && _treasuryFee == 0 && _liqFee == 0) return;
