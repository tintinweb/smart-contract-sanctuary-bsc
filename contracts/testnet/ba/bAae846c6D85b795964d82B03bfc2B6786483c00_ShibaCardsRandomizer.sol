...
import "./abstracts/ShibaCardsRandomizerBase.sol";
...
contract ShibaCardsRandomizer is ShibaCardsRandomizerBase, VRFConsumerBaseV2 {
  ...
  constructor(uint64 subscriptionId, address link, address vrfCoordinator, bytes32 keyHash) VRFConsumerBaseV2(vrfCoordinator) {
  ...
  function fulfillRandomWords(
    ...
    _forwardRandomness(requestId, randomWords[0]);
  }
...
abstract contract ShibaCardsRandomizerBase is
  IRandomizer,
  ShibaCardsAccessible
{
  function _forwardRandomness(uint256 requestId, uint256 randomNumber)
    internal
  {
    IRandomConsumer(_msgSender()).fulfillRandomness(requestId, randomNumber);
  }

  /**
   * @dev Request randomness.
   */
  function requestRandomness() external virtual override returns (uint256);
}
