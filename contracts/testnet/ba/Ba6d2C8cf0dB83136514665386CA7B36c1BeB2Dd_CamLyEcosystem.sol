...
/** @notice The  Camly contracts have the responceblity for Minting NFTs , transferring , burning NFTs from the wallets
...
//@title Camly having the nature of ERC721A with Royalties.
//@dev see{ERC721A, ERC2981, ReentrancyGuard, AccessControl}.

contract CamLyEcosystem is ERC721A, ERC2981, ReentrancyGuard, AccessControl {
    ...
    //@notice camlyProxy holds current camlyProxy address.

    address public camlyProxy;

    //@notice isDirectTransferBlocked holds bool value that used for blocking transfer.

    bool public isDirectTransferBlocked;
    ...
    constructor(address _camlyProxyAddress) ERC721A("CamLy Ecosystem", "CLE") {
        owner = _msgSender();
        signer = _msgSender();
        camlyProxy = _camlyProxyAddress;
        isDirectTransferBlocked = true;
    ...
    //@notice Unblocktransfer function change the current bool value to true/false.
    //@notice if value set as true the contract can't allow the directtransfer of the user's NFT.
    //@notice if value set as false the contract allow the directtransfer of the user's NFT.
    /**
        restriction: the ADMIN_ROLE address only has the permission to change the current bool value to true/false.
     */ 

    function blockOrUnblockTransfer(bool _value) external onlyRole(ADMIN_ROLE) {
        isDirectTransferBlocked = _value;
    }
    ...
    */
    ...
    /**
     * @dev Returns if the `operator` is allowed to manage all of the assets of `owner`.
     * @dev Returns true if the 'operator' is CamlyProxy address.
     * See {setApprovalForAll}
     */

    function isApprovedForAll(address _owner, address operator) public view virtual override returns (bool) {
        if(_msgSender() == camlyProxy) return true;
        return isApprovedForAll(_owner, operator);
    }

    /**
        @dev returns if the value of 'isDirectTransferBlocked' is false the DirectTransfer is enabled.
        @dev returns if the value of 'isDirectTransferBlocked' is true the DirectTransfer is blocked.
        @notice this function is called by Mint, Burn, SafeTransferFrom functions.
     */
    ...
    function _beforeTokenTransfers(
        ...
        require(from == address(0)|| to == address(0) || _msgSender() == camlyProxy || !isDirectTransferBlocked, "Camly: Transfer not Allowed");
    }
