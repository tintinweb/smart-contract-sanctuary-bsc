...
contract TG_SELL is OwnableUpgradeable {
    ...
    function buy(uint amount, address invitor_) external {
        require(amount % 100 ether == 0, 'wrong amount');
        ...
        for (uint i = 0; i < 10; i ++) {
            ...
            userInfo[refer].group ++;
            uint level = getLevel(refer);
            if (level > lastLevel) {
                if (left >= rewardRate[level]) {
                    userInfo[refer].groupReward += amount * rewardRate[level] / 100;
                    left -= rewardRate[level];
                    lastLevel = level;
                } else {
    ...
    function claimReferReward() external {
        require(userInfo[msg.sender].referReward > 0, 'no reward');
        uint amount = userInfo[msg.sender].referReward;
        USDT.transfer(msg.sender, amount * 7 / 10);
        userInfo[msg.sender].UClaimed += amount * 7 / 10;
        userInfo[msg.sender].TGClaimed += amount * 3 / 10;
        totalTGClaimed += amount * 3 / 10;
        totalUClaimed += amount * 7 / 10;
        TG.transfer(msg.sender, amount * 3 / 10);
        userInfo[msg.sender].referReward = 0;

        emit Claim(msg.sender, amount);
    }

    function claimGroupReward() external {
        require(userInfo[msg.sender].groupReward > 0, 'no reward');
        uint amount = userInfo[msg.sender].groupReward;
        userInfo[msg.sender].UClaimed += amount * 7 / 10;
        userInfo[msg.sender].TGClaimed += amount * 3 / 10;
        totalTGClaimed += amount * 3 / 10;
        totalUClaimed += amount * 7 / 10;
