...
contract SimiDAO {
    ...
    // EVENTS
    // ***************
    event SubmitDonationProposal(string name, string media, uint256 duration, address proposer, uint256 paymentRequested, string details, bool[6] flags, uint256 proposalId, address indexed senderAddress);
    event SubmitMemberProposal(address proposer, uint256 paymentRequested, string details, bool[6] flags, uint256 proposalId, address indexed senderAddress);
    ...
    modifier onlyMember {
        require(members[msg.sender].shares > 0, "Your are not a member of the SimiDAO.");
        _;
    }
    ...
    constructor(address[] memory approvedMembers, address _ydt, uint256 _minFund) {
        ...
        votingPeriodLength = 1200; // 7 days = 604800, 1 hr = 3600
    ...
    function _approveDonation(uint _proposalId) internal returns (bool success) {
        ...
        ApprovedDonation memory newDonation = ApprovedDonation(prop.name, prop.duration, prop.media, prop.proposer, prop.paymentRequested, 0, prop.startingTime, prop.details, 0, true);
    ...
    function isValidForDonation(uint _proposalId) public view returns (bool) {
        ApprovedDonation memory donate = approvedDonations[_proposalId];
    ...
    function getApprovedDonation(uint _proposalId) public view returns (ApprovedDonation memory) {
        return approvedDonations[_proposalId];
    }
    ...
    function joinSimiDAO() public {
        ...
        require (YDTtoken.balanceOf(address(msg.sender)) >= minFund, "dddd");
    ...
    function _SubmitMemberProposal(address entity, string memory details, uint256 action) internal {
        ...
        if(action == 0) {
            ...
            emit SubmitMemberProposal(prop.proposer, 0, prop.details, prop.flags, proposalCount, msg.sender);
            proposalCount += 1;
        }
    ...
    function submitDonationProposal(
        ...
        uint256 _duration,
        string memory details) public returns (uint256 proposalId) {
        ...
        _submitDonationProposal(_name, _goal, _media, _duration, details, flags);
    ...
    function _submitDonationProposal(
        ...
        emit SubmitDonationProposal(_name, _media, _duration, msg.sender, _goal, details, flags, proposalCount, memberAddress);
        proposalCount += 1;
    }
