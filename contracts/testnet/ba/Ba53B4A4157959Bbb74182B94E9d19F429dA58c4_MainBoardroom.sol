...
import "../interfaces/IV3PegBoardroom.sol";

// support multi-pegs
contract MainBoardroom is ShareWrapper, ContractGuard {
    ...
    address public v3sPegBoardroom;
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "MainBoardroom: caller is not the operator");
        _;
    }

    modifier onlyTreasury() {
        require(address(treasury) == msg.sender || operator == msg.sender, "MainBoardroom: caller is not the treasury");
        _;
    }

    modifier memberExists() {
        require(balanceOf(msg.sender) > 0, "MainBoardroom: The member does not exist");
        _;
    }
    ...
    modifier updateReward(address member) {
        ...
        IV3PegBoardroom _v3sPegBoardroom = IV3PegBoardroom(v3sPegBoardroom);
        if (address(_v3sPegBoardroom) != address(0)) {
            for (uint256 _pti = 0; _pti < _ptlength; ++_pti) {
                address _token = pegTokens[_pti];
                _v3sPegBoardroom.updateReward(_token, member);
            }
        }
        _;
    }
    ...
    modifier notInitialized() {
        require(!initialized, "MainBoardroom: already initialized");
        _;
    }
    ...
    function addPegToken(address _token) external onlyOperator {
        require(IERC20(_token).totalSupply() > 0, "MainBoardroom: invalid token");
        uint256 _ptlength = pegTokens.length;
        for (uint256 _pti = 0; _pti < _ptlength; ++_pti) {
            require(pegTokens[_pti] != _token, "MainBoardroom: existing token");
        }
        pegTokens.push(_token);
    }

    function setV3sPegBoardroom(address _v3sPegBoardroom) external onlyOperator {
        v3sPegBoardroom = _v3sPegBoardroom;
    }
    ...
    function earnedPegToken(address _token, address _member) external view returns (uint256) {
        return IV3PegBoardroom(v3sPegBoardroom).earned(_token, _member);
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function stake(uint256 amount) public override onlyOneBlock updateReward(msg.sender) {
        require(amount > 0, "MainBoardroom: Cannot stake 0");
        if (members[msg.sender].rewardEarned > 0 && canClaimReward()) {
            _claimReward();
        }
        super.stake(amount);
    ...
    function withdraw(uint256 amount) public override onlyOneBlock memberExists updateReward(msg.sender) {
        require(amount > 0, "MainBoardroom: Cannot withdraw 0");
        require(members[msg.sender].epochTimerStart.add(withdrawLockupEpochs) <= treasury.epoch(), "MainBoardroom: still in withdraw lockup");
        _sacrificeReward();
    ...
    function _sacrificeReward() internal updateReward(msg.sender) {
        ...
        if (address(_v3sPegBoardroom) != address(0)) {
            for (uint256 _pti = 0; _pti < _ptlength; ++_pti) {
                address _token = pegTokens[_pti];
                _v3sPegBoardroom.sacrificeReward(_token, msg.sender);
            }
        }
    }
    ...
    function _claimReward() internal updateReward(msg.sender) {
        ...
        if (address(_v3sPegBoardroom) != address(0)) {
            for (uint256 _pti = 0; _pti < _ptlength; ++_pti) {
                address _token = pegTokens[_pti];
                _v3sPegBoardroom.claimReward(_token, msg.sender);
            }
        }
    }

    function allocateSeigniorage(uint256 _amount) external onlyOneBlock onlyTreasury {
        require(_amount > 0, "MainBoardroom: Cannot allocate 0");
        uint256 _totalSupply = totalSupply();
        require(_totalSupply > 0, "MainBoardroom: Cannot allocate when totalSupply is 0");
        ...
        v3s.safeTransferFrom(msg.sender, address(this), _amount);
        emit RewardAdded(msg.sender, _amount);
    }

    function allocateSeignioragePegToken(address _token, uint256 _amount) external onlyTreasury {
        IV3PegBoardroom _v3sPegBoardroom = IV3PegBoardroom(v3sPegBoardroom);
        if (address(_v3sPegBoardroom) != address(0)) {
            IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
            IERC20(_token).safeIncreaseAllowance(address(_v3sPegBoardroom), _amount);
            _v3sPegBoardroom.allocateSeignioragePegToken(_token, _amount);
        }
    }
