...
contract Artifacts is Initializable, OwnableUpgradeable, ERC1155Upgradeable {
    ...
    function initialize(string memory _baseUrl, address _blackListContractAddress) initializer internal {
        baseUrl = _baseUrl;
    ...
    // function addNewArtifact(uint256 artifactId, uint256 _level, string calldata name) onlyOwner external {
    //     require(level[artifactId] == 0 && keccak256(abi.encodePacked(artifactName[artifactId])) == keccak256(abi.encodePacked("")), "This artifact already exist");
    //     level[artifactId] = _level;
