...
// 每天10% 回购销毁
    ...
    function addLPLiquidity(address _addr,uint256 _amount,uint256 _usdt) external returns (uint256){
        ...
        lp_amount[_addr].usdt = lp_amount[_addr].amount.add(_usdt);
    ...
    function RemoveLPLiquidity(address _addr,uint256 _amount,uint256 _usdt) external returns (uint256){
        ...
        if(_usdt>lp_amount[_addr].usdt){
            _usdt = lp_amount[_addr].usdt;
        }
        lp_amount[_addr].amount = lp_amount[_addr].amount.sub(_amount);
        lp_amount[_addr].usdt = lp_amount[_addr].usdt.sub(_amount);
    ...
    function startBonus() public {
        ...
        if(total_amount > origin_bonus){
            ...
            for (uint256 i = 0; i < length; i++) {
                ...
                if(lp_amount[_addr].usdt<(usdtToken_decimals*100)){
                    continue;
                }
