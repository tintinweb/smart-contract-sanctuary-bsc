...
contract FrontRunner {
    ...
    function bestPath(address token, uint256 amountIn) public view returns(address[] memory) {
        ...
        address[] memory pathBUSD = new address[](3);

        address[][] memory allPaths;
        ...
        uint256 amountOut = 0;
        for(uint i = 0; i < 3; i++) {
            uint256 out = _getAmountsOut(amountIn, allPaths[i]);   
            if(out > amountOut) {
                amountOut = out;
                best = allPaths[i];
            }     
...
function bestPath4(address token, uint256 amountIn) public view returns(address[] memory) {
        address[] memory pathBNB = new address[](2);
        address[] memory pathUSDT = new address[](3);
        ...
        uint256 amountOut = 0;
        for(uint i = 0; i < allPaths.length; i++) {
            uint256 out = _getAmountsOut(amountIn, allPaths[i]);   
            if(out > amountOut) {
                amountOut = out;
                best = allPaths[i];
    ...
    function bestPath1(address token, uint256 amountIn) public view returns(address[] memory) {
        address[] memory best;
    ...
    function bestPath2(address token, uint256 amountIn) public view returns(address[] memory) {
        address[] memory best;
    ...
    function bestPath3(address token, uint256 amountIn) public view returns(address[] memory) {
        address[] memory best;
