...
contract TokenStake is Member {
    ...
    modifier validSender{
        require(msg.sender == address(manager.members("LPAddress")) || msg.sender == manager.members("nft") || msg.sender == manager.members("updatecard") || msg.sender == manager.members("owner"));
        _;
    }
    ...
    function getReward() public {
        ...
        IERC20(mp).transfer(msg.sender, reward.add(userInfo[msg.sender].pendingReward));
    ...
    function withdraw() public {
        ...
        IERC20(stakeToken).transfer(msg.sender, pendingWithdraw);
...
contract Token is ERC20, Ownable, Member {
    ...
    function redeem(uint256 amount) public onlyOwner {
        require(totalSupply() >= amount, "Token: totalSupply not enough");
        require(balanceOf(msg.sender) >= amount, "Token: token not enough");
        _burn(msg.sender, amount);
    }

    function transferFrom(address from, address to, uint256 amount) public virtual override returns (bool) {
        if (manager.members("LPAddress") != address(0) && manager.members("swapPair") != address(0)) {
            address lpaddr = manager.members("LPAddress");
            address swapPair = manager.members("swapPair");
            if (to == swapPair) {
                uint256 reward = amount.mul(2).div(100);
                require(balanceOf(from) >= (amount + reward), "Token Swap: token not enough");
...
contract Promote is Member {
    ...
    struct DaliyInfo {
        ...
        uint256[] perNodeNum;
        // 全網各等級有效用戶數(直推)
        uint256[] perValidNumSS;
        // 全網各等級有效用戶數(兩代)
        // uint256[] perValidNumGS;
    ...
    // MD 值（兩代內）
    uint256[] internal numThresholdGS = [15,30,60,120];
    // 質押 MP 等級
    ...
    function init() internal {
        ...
        daliyInfo[0].perNodeNum.push(0);
        daliyInfo[0].perNodeNum.push(0);
        daliyInfo[0].perValidNumSS.push(0);
        daliyInfo[0].perValidNumSS.push(0);
        daliyInfo[0].perValidNumSS.push(0);
        daliyInfo[0].perValidNumSS.push(0);
    ...
    function getDaliyValidNum(uint256 _round) public view returns(uint256[] memory) {
        return daliyInfo[_round].perValidNumSS;
    }
    ...
    function redem(address sender, uint256 weight, uint256 amount) external onlyPool {
        ...
        if(user.isValid && weight.sub(amount) < 10) {
            userDown(sender);
            evoDown(f);
            if(ff != address(0)) {
                evoDown(ff);
        ...
        // 自己
        userInfo[sender].down8GenWeight = userInfo[sender].down8GenWeight.sub(amount);
        userInfo[sender].down3GenWeight = userInfo[sender].down3GenWeight.sub(amount);

        uint256[4] memory subTotalUpdateWeight;
        uint256 _level = userInfo[sender].level;
        if(_level > 0){
            subTotalUpdateWeight[_level-1] = subTotalUpdateWeight[_level-1].add(amount);
        }
        ...
        for(uint8 i=0; i < 8; i++) {
            ...
            // 更新上線8代的權重加總
            userInfo[_user].down8GenWeight = userInfo[_user].down8GenWeight.sub(amount);
            bool _isPreacher = IPreacher(manager.members("PreacherAddress")).checkIsPreacher(_user);
            if(_isPreacher){
                bool isAdd = false;
                IPreacher(manager.members("PreacherAddress")).updateWeight(_user, amount, isAdd);
            }

            // 更新上線3代權重加總
            if(i < 3){
                userInfo[_user].down3GenWeight = userInfo[_user].down3GenWeight.sub(amount);
    ...
    function userDown(address sender) internal {
        if (userInfo[sender].level > 0) {
            ...
            daliyInfo[round].perValidNumSS[userInfo[sender].level-1]-= userInfo[sender].numSS;
    ...
    function evoDown(address _user) internal {
        ...
        if (userInfo[_user].level > 0) {
            daliyInfo[round].perValidNumSS[level - 1]--;
        }
        
        // 更新兩代上線 MD 值
        address ff = userInfo[_user].f;
        if (ff != address(0)) {
            userInfo[ff].numGS--;
        }
        ...
        if ( level > 0 && (userInfo[_user].numSS < numThresholdSS[level - 1] || userInfo[_user].numGS < numThresholdGS[level - 1])) {
            ...
            daliyInfo[round].perValidNumSS[level - 1] -= userInfo[_user].numSS;
            daliyInfo[round].perValidWeight[level - 1] -= weight;
            if (userInfo[_user].level > 0) {
                daliyInfo[round].perNodeNum[userInfo[_user].level - 1]++;
                daliyInfo[round].perValidNumSS[userInfo[_user].level - 1] += userInfo[_user].numSS;
            ...
            // 如果上線降級影響兩代上線也因此降級了
            if (ff != address(0)) {
                uint8 levelFF = userInfo[ff].level;
                uint256 weightFF = userInfo[ff].weight;
                if ( levelFF > 0 && (userInfo[ff].numSS < numThresholdSS[levelFF - 1] || userInfo[ff].numGS < numThresholdGS[levelFF - 1])) {
                    daliyInfo[round].perNodeNum[levelFF - 1]--;
                    userInfo[ff].level--;
                    daliyInfo[round].perValidNumSS[levelFF - 1] -= userInfo[ff].numSS;
                    daliyInfo[round].perValidWeight[levelFF - 1] -= weightFF;
                    if (userInfo[ff].level > 0) {
                        daliyInfo[round].perNodeNum[userInfo[ff].level - 1]++;
                        daliyInfo[round].perValidNumSS[userInfo[ff].level - 1] += userInfo[ff].numSS;
                        daliyInfo[round].perValidWeight[userInfo[ff].level - 1] += weightFF;
                    }
                    emit LevelChange(ff, levelFF, levelFF - 1);
                    // 當有人降級成Lv0，檢查佈道者
                    if(userInfo[ff].level == 0) {
                        IPreacher(manager.members("PreacherAddress")).upgradePreacher(ff);
                    }

                    // 更新該等級全網權重
                    uint256 _down3GenWeight = userInfo[ff].down3GenWeight;
                    daliyInfo[round].totalDown3GenWeight[levelFF-1] = daliyInfo[round].totalDown3GenWeight[levelFF-1].sub(_down3GenWeight);
                    if(levelFF > 1){
                        daliyInfo[round].totalDown3GenWeight[levelFF-2] = daliyInfo[round].totalDown3GenWeight[levelFF-2].add(_down3GenWeight);
                    }
                }
            }
        }
    ...
    function newDeposit(address sender,uint256 weight, uint256 amount) external onlyPool {
        ...
        // 質押後，該用戶變為有效用戶
        if(!user.isValid && weight.add(amount) >= 10) {
            userUp(sender);
            evo(f);
            if (ff != address(0)) {
                evo(ff);
        ...
        uint256 _level = userInfo[sender].level;
        if(_level > 0){
            addTotalUpdateWeight[_level-1] = addTotalUpdateWeight[_level-1].add(amount);
        }
        for(uint8 i=0; i < 8; i++) {
            address _user =user.upline8Gen[i];
            if(_user == address(0)){
            ...
            // 更新上線8代的權重加總
            userInfo[_user].down8GenWeight = userInfo[_user].down8GenWeight.add(amount);
            bool _isPreacher = IPreacher(manager.members("PreacherAddress")).checkIsPreacher(_user);
            if(_isPreacher){
                bool isAdd = true;
                IPreacher(manager.members("PreacherAddress")).updateWeight(_user, amount, isAdd);
            ...
            // 更新上線3代權重加總
            if(i < 3){
                userInfo[_user].down3GenWeight = userInfo[_user].down3GenWeight.add(amount);
    ...
    function userUp(address sender) internal {
        ...
        if (userInfo[sender].isValid == false && level2 > 0) {
            ...
            daliyInfo[round].perValidNumSS[level2 -1] += userInfo[sender].numSS;
        ...
        if ( 
            ...
            if (ff != address(0)) {
                ...
                if ( 
                    userInfo[ff].isValid &&
                    levelFF <= 3 && 
                    userInfo[ff].numSS >= numThresholdSS[levelFF] && 
                    userInfo[ff].numGS >= numThresholdGS[levelFF] && 
                    userInfo[ff].depositAsUsdt >= amountThreshould[levelFF]) {
                        if (levelFF > 0) {
                            daliyInfo[round].perNodeNum[levelFF - 1]--;
                            daliyInfo[round].perValidNumSS[levelFF - 1] -= userInfo[ff].numSS;
                            daliyInfo[round].perValidWeight[levelFF - 1] -= weightFF;
                        }
                        daliyInfo[round].perNodeNum[levelFF]++;
                        userInfo[ff].level++;
                        daliyInfo[round].perValidNumSS[levelFF] += userInfo[ff].numSS;
                        daliyInfo[round].perValidWeight[levelFF] += weightFF;
                        emit LevelChange(ff, levelFF, userInfo[ff].level);
                        // 當有人升級成Lv1，檢查佈道者
                        if(levelFF == 0 && userInfo[ff].level == 1) {
                            IPreacher(manager.members("PreacherAddress")).upgradePreacher(ff);
                        }
                        ...
                        daliyInfo[round].totalDown3GenWeight[levelFF] = daliyInfo[round].totalDown3GenWeight[levelFF].add(_down3GenWeight);
                }
            }
        }
    }
    ...
    function update(uint256 amount) external validSender {
        if(block.timestamp >= roundTime[round] + 24 hours) {
            ...
            if (round > 0) {
                ...
                daliyInfo[round].perValidNumSS = daliyInfo[round -1].perValidNumSS;
    ...
    function deposit(uint256 amount) public {
        ...
        if (old != newlevel && userInfo[msg.sender].isValid) {
            ...
            daliyInfo[round].perValidNumSS[newlevel -1] += userInfo[msg.sender].numSS;
            daliyInfo[round].perValidWeight[newlevel -1] += userInfo[msg.sender].weight;
            if (old > 0) {
                daliyInfo[round].perNodeNum[old -1]--;
                daliyInfo[round].perValidNumSS[old -1] -= userInfo[msg.sender].numSS;
    ...
    function withdrawRequest() public {
        ...
        if (userInfo[msg.sender].level > 0) {
            ...
            daliyInfo[round].perValidNumSS[userInfo[msg.sender].level-1]-= userInfo[msg.sender].numSS;
...
contract Preacher is Member {
    ...
    using SafeMath for uint256;
    uint256 public totalWeightAmount;
    uint256 public round;
    uint256 public totalRewards;
    ...
    function getReward() public {
        uint256 reward = settleRewards(msg.sender);
        IERC20(rewardToken).transfer(msg.sender, reward.add(userInfo[msg.sender].pendingReward));
        userInfo[msg.sender].pendingReward = 0;
        userInfo[msg.sender].lastRewardRound = round;
        emit GetReward(msg.sender, reward);
    }
    ...
    function pendingRewards(address _user) public view returns (uint256 reward){
        ...
        uint8 i = round.sub(userInfo[_user].lastRewardRound) >= 15 ? 15: uint8(round.sub(userInfo[_user].lastRewardRound));
        for(i; i >0; i--) {
            reward = reward.add(daliyInfo[round-i].poolAmount.mul(promoteUserInfo.weight).div(daliyInfo[round-i].totalWeight));
        }
        reward = reward.add(userInfo[_user].pendingReward);
    }
    ...
    function upgradePreacher(address _user) external {
        ...
        if(isPreacher[_user]){
            // 檢查降級
            if(promoteUserInfo.level == 0 || promoteUserInfo.numDown6Gen < 150){
...
contract MPNFT is ERC721,Member {
    ...
    function distribute(uint256 needpay) internal{
        uint256 OfficalAmount = needpay.mul(5).div(105); 
        uint256 PromoteAmount = needpay.mul(25).div(105);
        uint256 LPAmount = needpay.mul(5).div(105);
        uint256 burnAmount = needpay.sub(OfficalAmount).sub(PromoteAmount).sub(LPAmount);
        IERC20(mp).transfer(address(manager.members("OfficalAddress")),OfficalAmount);  // 5% 项目方
        IERC20(mp).transfer(address(manager.members("PromoteAddress")),PromoteAmount);  // 25% 动态分配（MP 质押池奖励）
        IPromote(manager.members("PromoteAddress")).update(PromoteAmount);
        IERC20(mp).transfer(address(manager.members("LPAddress")),LPAmount);  // 5% 回流 LP 池奖励 
        IERC20(mp).burn(burnAmount);    // 70% 销毁
