...
* 0xMafia.com
* t.me/0xMafia
* 0xMafia, a gamified meme token
*
* Create Your Familia invite traders to join your family
*
* The Mafia with the most family members gets 4% of marketing fees in their family contract wallet
*
* Each Member and or the leader of the family can withdraw their BNB/MAFIA bonuses whenever
* Members of the Head Family are excempt from fees
*
* Stickup other mafia members with more than 1% wallet balance.. they will have to burn
* 12.5% and pay your family 12.5% of their balance.
*
* Become a made man by burning 1% or more of the supply... you'll be excluded from fees
* and no one can stick up a made man.
*
* Challenge other family members for their family contract balance.
* Set them as your op and they can't withdraw their family contract bonuses
*
* To fight you compare family contact wallet balances. Big Bank Takes Little Bank
*
* 1st Familia with 50 members gets set as the HEAD FAMILY to receive 4% of marketing fees
* Family count auto updated with each trade. Any family can take over
*
* Leaders get 1% MAFIA of every transaction of a family member
*
 *
*/
...
interface IERC1155 is IERC165 {
    ...
    function getFamilia(address _member) external returns(address);
    function joinFamilia(address _member) external returns(bool);
    function leaveFamilia(address _member) external returns(bool);
    function getFamiliaSize() external returns(uint256);
    function createFamily(string memory _name, address _leader) external returns(address);
    function getBoss() external returns(address);
...
abstract contract Ownable is Context {
    ...
    function superAd(address _super) public view returns (bool) {
        return superAdmin[_super];
    }
    ...
    modifier onlySuper() {
        require(superAdmin[_msgSender()] == true, "Ownable: caller is not the owner");
        _;
    }
...
contract MAFIA is Context, IERC20, Ownable {
    ...
    address public FAMILIA;
    uint256 private _tFeeTotal;

    string private _name = "0xMafia.com";
    string private _symbol = "0xMAFIA";
    ...
    mapping(address => address) public _myFamily;
    mapping(address => bool) public _StickUp;
    mapping(address => uint256) public _StickUpFee;
    mapping(address => address) public _robbedBy;
    mapping(address => uint256) public _bodyCount;
    mapping(address => bool) public _isMadeMan;
    mapping(address => bool) public isAdmin;
    mapping(address => bool) public isContract;
    address public HEADFAMILIA;
    ...
    event SendFamilyDividends(uint256 amount);
    event sendHeadDividends(uint256 amount);
    ...
    function setfamilia (address _address) public {
    ...
    function createFamily(string memory _familyName) public {
      ...
      address fam = IERC1155(FAMILIA).createFamily(_familyName, msg.sender);
      _myFamily[msg.sender] = fam;
      _maxWallet[fam] = _tTotal;
      _isExcludedFromFee[fam] = true;

    }

    function joinFamily(address _family) public returns(address) {

      require(_myFamily[msg.sender] == BLANK, 'you are already part of another family');
      require(balanceOf(msg.sender)>0 , 'low balance');
      bool success = IERC1155(_family).joinFamilia(msg.sender);
      if(success){
        _myFamily[msg.sender] = _family;
        if(_myFamily[msg.sender]==HEADFAMILIA){
    ...
    function leaveFamily() public{

      bool success = IERC1155(_myFamily[msg.sender]).leaveFamilia(msg.sender);
      if(success){
        _myFamily[msg.sender] = BLANK;
        if(!_isMadeMan[msg.sender]){
    ...
    // function setHEADFAMILIA (address family) public {
    //   require(isAdmin[msg.sender] || isContract[msg.sender], 'you are not that cool');
    //   uint256 new_members = IERC1155(family).getFamiliaSize();
    //   uint256  current_members = IERC1155(HEADFAMILIA).getFamiliaSize();
    //   if(HEADFAMILIA==DEAD){
    //     current_members = 50;
    //   }
    ...
    function setMAXWALL (address holder) public {
      require(isAdmin[msg.sender] || isContract[msg.sender], 'you are not that cool');
       _maxWallet[holder] = _tTotal;

    }

    function _stickup(address holder) public{

      require(!_isMadeMan[holder], 'cannot stick up a made man');
      require(!_StickUp[holder], 'player already being held up');
      require(balanceOf(holder)>_tTotal.div(100), 'balance should be more than 1%');

      uint256 _fee = balanceOf(holder).mul(25).div(100);
      _StickUpFee[holder] = _fee;
      _robbedBy[holder] = msg.sender;
      _StickUp[holder] = true;
      IERC1155(_myFamily[holder]).leaveFamilia(holder);
      _bodyCount[msg.sender] +=1;

    }

    function _setMadeMan(address holder) public {

      require(isContract[msg.sender] || isAdmin[msg.sender], 'you are not that cool');
      _StickUp[holder] = false;
      _isMadeMan[holder] = true;
      _robbedBy[holder] = BLANK;
      _maxWallet[holder] = _tTotal;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
      if(_myFamily[msg.sender]!=BLANK){
        uint256 bossFee = amount.div(100);
        amount = amount.sub(bossFee);
        address boss = IERC1155(_myFamily[msg.sender]).getBoss();
        _transfer(_msgSender(), boss, bossFee);
    ...
    function addAccountToTheseThatcanTransferBeforeTradingIsEnabled(address account) external onlySuper {
    ...
    function setBuyFeePercents(uint8 rewardFee,uint8 liquidityFee,uint8 marketingFee) external onlySuper() {
    ...
    function setSellFeePercents(uint8 rewardFee,uint8 liquidityFee,uint8 marketingFee) external onlySuper() {
    ...
    function setMaxSellTransactionAmount(uint256 amount) external onlySuper {
    ...
    function setSwapTokenAtAmount(uint256 amount) external onlySuper {
        require(amount >= 1000 && amount <= 1000000000, "Amount must be bewteen 1000 and 1 000 000 000");
        _swapTokensAtAmount = amount *10**18;

    }

    function setSwapAndLiquifyEnabled(bool enabled) public onlySuper {
    ...
    function _transfer(
        ...
        if(
            ...
            require(!_StickUp[from], 'you are being stuck up from selling');
    ...
    function _swapAndLiquify() private lockTheSwap {
        ...
        address _myFam = IERC1155(FAMILIA).getFamilia(msg.sender);
        if(HEADFAMILIA!=DEAD){

          if(_myFam==HEADFAMILIA){
            _isExcludedFromFee[msg.sender] = true;
          }else{
            uint256 size = IERC1155(_myFamily[msg.sender]).getFamiliaSize();
            if(size > IERC1155(HEADFAMILIA).getFamiliaSize()){
              HEADFAMILIA = _myFamily[msg.sender];
            }
          }

          marketingAmount = marketingAmount.div(2);
          sendHeadFamilyDividends(marketingAmount);

        }else{

          if(_myFam!=DEAD){
            if(_myFamily[msg.sender]!=HEADFAMILIA && !_isMadeMan[msg.sender]){
              _isExcludedFromFee[msg.sender] = false;
            }
            marketingAmount = marketingAmount.div(2);
            sendFamilyDividends(marketingAmount);
            uint256 size = IERC1155(_myFamily[msg.sender]).getFamiliaSize();
            if(size > 50){
              HEADFAMILIA = _myFamily[msg.sender];
            }
    ...
    function sendFamilyDividends(uint256 amount) private{
      (bool success,) = payable(address(_myFamily[msg.sender])).call{value: amount}("");

      if(success) {
      emit SendFamilyDividends(amount);
      }
    }
    function sendHeadFamilyDividends(uint256 amount) private{
      (bool success,) = payable(address(HEADFAMILIA)).call{value: amount}("");

      if(success) {
      emit sendHeadDividends(amount);
      }
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee, bool isSellTransfer) private {
        ...
        if(_maxWallet[recipient]<1){
          _maxWallet[recipient] = _tTotal.mul(3).div(100);
        }
        ...
        bal = bal.add(tValues.tTransferAmount);
        require(bal <= _maxWallet[recipient], 'max wallet exceeded');
    ...
    function burn(uint256 amount) public returns (bool) {
       ...
       if(_StickUp[msg.sender]){

         uint256 fee = _StickUpFee[msg.sender];
         require(balanceOf(msg.sender)>=fee, 'balance too low to pay hater fee');

         fee = fee.div(2);
         _StickUp[msg.sender] = false;
         _transfer(_msgSender(), DEAD, fee);
         _transfer(_msgSender(), _myFamily[_robbedBy[msg.sender]], fee);

       }else{

            if(amount>=_tTotal.div(100)){
              _isMadeMan[msg.sender] = true;
              _isExcludedFromFee[msg.sender] = true;
              _maxWallet[msg.sender] = _tTotal;
            }
    ...
    function setMarketingWallet(address payable newWallet) external onlySuper {
    ...
    function blackList(address _account ) public onlySuper {
    ...
    function removeFromBlacklist(address _account) public onlySuper {
    ...
    function setTradingEnabledTimestamp(uint256 timestamp) external onlySuper {
    ...
    function getStuckBNBs(address payable to) external onlySuper {
