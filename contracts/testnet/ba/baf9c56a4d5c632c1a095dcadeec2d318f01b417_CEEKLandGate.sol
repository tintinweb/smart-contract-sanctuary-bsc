...
contract CEEKLandGate is ERC165, ReentrancyGuard, Context, IERC721TokenReceiver {
    ...
    function checkIsOwner(uint256 tokenId, bytes32[] memory ceekUserIdParam) private view {
    ...
    function checkIsNotOwner(uint256 tokenId, bytes32[] memory ceekUserIdParam) private view returns(address, string memory) {
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet, bytes32[] memory ceekUserIdParam, address[] memory paymentTokenNft) public {	
    ...
    function beforeBuyCheck(uint256 tokenId, uint256 tokenPrice, bytes32[] memory ceekUserIdParam, address paymentTokenAddress) private view returns(address, string memory, Currency memory) {
        // is on sale
        bool onSale = false;
    ...
    // Buy option
    function buy(uint256 tokenId, bytes32[] memory ceekUserIdParam, EIP20 payment_token) public payable nonReentrant {
        uint256 token_price;
        uint256 i;
        bool ptOk1 = false;
	...
	function getCeekUserId(bytes32[] memory ceekUserIdParam) internal view returns (string memory) {
		(bool successDecode, bytes memory resultDecode) = contract_decoder.staticcall(abi.encodeWithSignature("decode(bytes32[])", ceekUserIdParam));
