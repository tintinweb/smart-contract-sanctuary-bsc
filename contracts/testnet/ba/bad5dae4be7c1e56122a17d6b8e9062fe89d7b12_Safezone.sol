...
contract Safezone is ISafezone, Ownable, ReentrancyGuard, IERC721Receiver, Pausable {
  ...
  mapping(address => uint256[]) private safezoneBuyer;
  uint256[] private tokenStaking;
  ...
  function _addHumanToSafezone(address account, uint256 tokenId) internal whenNotPaused _updateEarnings {
    ...
    safezoneBuyer[account].push(tokenId); 
    numHumansStaked += 1;
  ...
  function claimManyFromSafezoneAndFlight(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings nonReentrant {
    ...
    // clean safezoneBuyer
    _cleanSafezoneBuyerWhenUnstake(tokenIds, unstake);
  ...
  function _cleanSafezoneBuyerWhenUnstake(uint16[] calldata tokenIds, bool unstake) internal {

        if (unstake) {
           
            delete tokenStaking;
            for (uint j = 0; j < safezoneBuyer[_msgSender()].length; j++) {
                bool find = false;    
                for (uint k = 0; k < tokenIds.length; k++) {
                    
                    if (tokenIds[k] == safezoneBuyer[_msgSender()][j]) {
                        find = true;
                    }
                }
    
                if (find == false) {
                    tokenStaking.push(safezoneBuyer[_msgSender()][j]);
                }
            }

            safezoneBuyer[_msgSender()] = tokenStaking;
  ...
  function ownerInitTokenByAddress(address buyer) external view returns (uint256[] memory) {
      return safezoneBuyer[buyer];
  }
  ...
  function rescue(uint16[] calldata tokenIds) external nonReentrant {
    ...
    _cleanSafezoneBuyerWhenUnstake(tokenIds, true);
  }
  ...
  function _rankForVariant(uint256 tokenId) internal view returns (uint8) {
    ...
    return MAX_RANK - 2; // rank index is 0-3
  }
