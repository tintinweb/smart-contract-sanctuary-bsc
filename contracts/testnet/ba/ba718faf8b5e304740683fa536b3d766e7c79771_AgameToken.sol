...
contract AgameToken {
    ...
    uint partnerSalePrice = 20000000000000; // $0.00002
    ...
    /// @notice Max token minted through airdrop
    uint public maxInvestors; // The total that can be sold to investors
    uint public maxPartners; // The total that can be sold to partners
    ...
    uint public partnerAccumulated = 0;
    ...
    constructor(address admin_, uint level_1_, uint level_2_, uint level_3_, uint level_4_, uint level_5_, uint level_6_) {
        ...
        maxInvestors = (max_token_Supply / 100) * 7;

        maxPartners = (max_token_Supply / 100) * 13;
        ...
        uint MintAmount = reserve_mint_amount;
    ...
    function buyToken_Investors() payable public {
        ...
        require(InvestorAccumulated <= maxInvestors, "All tokens for investors have been sold");
        require(totalSupply < max_token_Supply, "Minting has stoped");
    ...
    function buyToken_Partners() payable public {
        int _bnbPrice = getBNBPrice();
        uint bnb_price = uint(_bnbPrice);
        uint lowerLimit = (6000 * (10 ** 18)) / bnb_price;
        uint upperLimit = (20000 * (10 ** 18)) / bnb_price;

        require(partnerAccumulated <= maxPartners, "All tokens for investors have been sold");
        require(totalSupply < max_token_Supply, "Minting has stoped");
        require(msg.value >= lowerLimit && msg.value < upperLimit, "Amount should be more than 16 bnb but less than 3,333 bnb");

        uint price_ = msg.value * bnb_price;
        uint _amount = price_ / partnerSalePrice;
        uint amount_ = (_amount * (10 ** 18)) / 4;
        ...
        partnerAccumulated = partnerAccumulated + amount_;
