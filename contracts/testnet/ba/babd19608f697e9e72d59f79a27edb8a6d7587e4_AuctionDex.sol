...
contract AuctionDex is ManagerAuction {
	...
	function bidAuction(
		...
		uint256 royaltyFee = _getRoyaltyFee(_tokenAddress, _tokenId);
		require(
			_price > (currentAuction.startPrice * (BASE_DENOMINATOR + royaltyFee + platformFee)) / BASE_DENOMINATOR,
		...
		newBidAuction.isBidderClaim = false;
	...
	function editBidAuction(uint256 _bidAuctionId, uint256 _price) external payable whenNotPaused returns (uint256) {
		...
		require(
			_price > bidAuctions[auctionHighestBidId[editPreviousBidAuction.auctionId]].bidPrice,
			"Price-bid-less-than-max-price"
		);
	...
	function cancelBidAuction(uint256 _bidAuctionId) external whenNotPaused returns (uint256) {
		...
		if (currentBid.paymentToken == address(0)) {
			...
			IERC20Upgradeable(currentBid.paymentToken).safeTransfer(
				currentBid.bidder,
				currentBid.bidPrice
			);
		}
	...
	function reclaimAuction(uint256 _auctionId) external whenNotPaused {
		...
		require(auctionBidCount[_auctionId] == 0, "Bid-placed");
		require(tokenOnAuction[currentAuction.tokenAddress][currentAuction.tokenId], "Auction-cancelled");
	...
	function claimWinnerAuction(uint256 _bidAuctionId) external whenNotPaused {
		...
		require(msg.sender == bidAuctions[highestBidId].bidder, "Not-bid-winner"); // make sure the sender is the winner

		uint256 royaltyFee = _getRoyaltyFee(currentAuction.tokenAddress, currentAuction.tokenId);
		require(
			currentBid.bidPrice >=
				(currentAuction.startPrice * (BASE_DENOMINATOR + royaltyFee + platformFee)) / BASE_DENOMINATOR,
			"Bid-not-valid"
		);

		require(!currentBid.isBidderClaim, "Bid-claimed");

		_payBidAuction(_bidAuctionId);
		_transferBidAuction(_bidAuctionId);

		currentBid.isBidderClaim = true;
