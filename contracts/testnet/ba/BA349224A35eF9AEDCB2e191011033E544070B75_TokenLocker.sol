...
contract TokenLocker is Ownable, ReentrancyGuard {
    ...
    IUniswapV2Factory private swapFactory;
    IFeesCalculator private feesCalculator;
    IMigrator private migrator;

    IERC20 private feeToken;
    address payable private feeReceiver = 0x3bC6a422A409AcfB79Fdb3b9f37f8A38C723Eef4;
    ...
    modifier onlyLockOwner(uint lockId) {
        ...
        require(lock.owner == address(msg.sender) || owner() == address(msg.sender), "NO ACTIVE LOCK OR NOT OWNER");
        _;
    }

    constructor(address _swapFactory, address _feesCalculator) {
    ...
    */
    function lockTokens(address lpToken, uint256 amount, uint256 unlockTime,
        ...
        feesCalculator.calculateFees(lpToken, amount, unlockTime, 0);
    ...
    function checkLpTokenIsPancake(address lpToken) private view returns (bool){
        ...
        address factoryPair = swapFactory.getPair(pair.token0(), pair.token1());
    ...
    */
    ...
    external payable nonReentrant onlyLockOwner(lockId) {
        ...
        (uint256 ethFee, uint256 tokenFee, uint256 lpTokenFee) =
        feesCalculator.calculateIncreaseAmountFees(lock.lpToken, amountToIncrement, lock.unlockTime, 0);
