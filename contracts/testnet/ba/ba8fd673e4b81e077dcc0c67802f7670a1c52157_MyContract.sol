...
contract MyContract is Initializable{
    ...
    function lock( address _owner, address token, bool isLpToken, uint256 amount, uint256 unlockDate, string memory description ) payable public returns(uint256 id) {
        ...
        require (msg.value >= Secondaryfee, "You have to pay fee for locking"); //TAKE FEE IF NOT AUTHORIZED
    ...
    function vestingLock(address _owner,address token,bool isLpToken,uint256 amount,uint256 tgeDate,uint256 tgeBps,uint256 cycle,uint256 cycleBps,string memory description) external returns (uint256 id) {
        require(token != address(0), "Invalid token");
        require(amount > 0, "Amount should be greater than 0");
