...
contract Pack is Context {
    ...
    function createPack(
        ...
        uint8 dropCount,    
        uint256 price,
        uint256[] calldata templates,
        uint256[] calldata chances
    ) external returns (uint256) {        
        // check chances and tempalte
        require(chances.length == templates.length , "INVALID TEMPLATE AND CHANCES");
        // check empty
        ...
        require(totalChance == 1e6, "CHANES IS NOT VALID");

        // get all templates by id
        (uint256[] memory templateIds,,) = template.getTemplatesByUser(msg.sender);
        // check template owner
        for (uint256 i = 0; i < templates.length; i ++) {
            bool isExist = false;
            for (uint256 j = 0; i < templateIds.length; j ++) {
                if (i == j) {
                    isExist = true;
                    break;
                }
            }
            require(isExist, "UNAUTHORIZED ACCESS TO TEMPLATE");
        }
        // keeping index
    ...
    function openPack(uint256 tokenId) payable public {
        // get nft from caller
        ...
        // random for each capacity of the pack
        uint256 packId = tokenPacks[tokenId];
        PackCollection storage pack = packCollection[packId];
        
        // uint256[] memory mintedTokens = new uint256[](pack.dropCount);
        
        // get co-relate template, address and chances
        uint256[] memory chances = pack.chances;
        uint256[] memory templates = pack.templates;
        address[] memory nftsAddress = template.getAddressesByTemplates(templates);
        string[] memory URIs = template.getTemplateURIs(templates);

        for (uint256 i = 0; i < pack.dropCount; i++) {
            // FIXME mock random
            uint256 randomHash = uint256(keccak256(abi.encodePacked(block.timestamp)));
            uint256 rand = randomHash * (i**2); 
            uint256 finalRand = rand % 1e6;

            uint256 rewardIndex;

            // for them chance, mint the appropriate one
            for (uint256 j = 0; j < chances.length; j ++) {
            ...
            uint256 totalSupply = IAsset(nftsAddress[rewardIndex]).totalSupply() - 1;
            
            // save what has been mint and distributed
            Asset[] memory assets = openPackHistories[packId];
            assets[assets.length + 1] = Asset(nftsAddress[rewardIndex], totalSupply);
        }
