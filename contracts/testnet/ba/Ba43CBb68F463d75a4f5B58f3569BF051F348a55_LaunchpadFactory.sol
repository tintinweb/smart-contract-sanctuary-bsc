...
import "./proxies/LaunchPoolProxy.sol";
    ...
    SharedConstants
{
    ...
    address public _tierManager;
    address public _launchpadLogic;
    address public _proxyAdmin;

    function initialize(
        address poolOwner_,
        address tierManager_,
        address launchpadLogic_,
        address proxyAdmin_
    ) external override initializer {
        _poolOwner = poolOwner_;
        _tierManager = tierManager_;
        _launchpadLogic = launchpadLogic_;
    ...
    function createLaunchpad(
        ...
        VestingTimeline memory _vestingTimeline_,
        ProjectInfo memory information_
    ) external override returns (address) {
        bytes memory data = abi.encodeWithSelector(
            ILaunchPool(_launchpadLogic).initialize.selector,
            poolInfo_,
            _vestingTimeline_,
            information_,
            _poolOwner,
        ...
        LaunchPoolProxy newPool = new LaunchPoolProxy(
            _launchpadLogic,
            _proxyAdmin,
            data
        );
        ...
        emit NewLaunchpad(newPoolAddress);
    ...
    function setTierAddress(address newTier_) public override onlyOwner {
        _tierManager = newTier_;
    }

    function setPoolOwnerAddress(address newPoolOwner)
        public
        override
        onlyOwner
    {
    ...
    function _transferTokenSupply(
        ...
        uint256 tokenAmount = salePrice_.mul(hardCap_).div(
            10**token_.decimals()
        );
        token_.safeTransferFrom(_msgSender(), poolAddress, tokenAmount);
    }
}
...
interface ILaunchpadFactory {
    event NewLaunchpad(address indexed launchpadAddr);
    ...
    function createLaunchpad(
        SharedConstants.LaunchPadInfo memory _launchpadData,
        SharedConstants.VestingTimeline memory _vestingTimeline,
        SharedConstants.ProjectInfo memory _information
    ) external returns (address);

    function collectBNB() external;

    function setTierAddress(address newTier_) external;

    function setPoolOwnerAddress(address newPoolOwner) external;
}
...
interface ILaunchPool {
    function initialize(
        ...
        SharedConstants.ProjectInfo memory information_,
        address poolOwner_,
        address tierManager_
    ) external;
