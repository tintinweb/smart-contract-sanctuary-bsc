...
contract DIGIRE is BEP20Upgradeable {
    ...
    event Swap_Jaxre_Digire(uint amountIn, address to);
    event Swap_Digire_Jaxre(uint amountIn, address to);
    ...
    function swap_jaxre_digire(uint amountIn) external onlyGatekeeper {
        JAXRE.transferFrom(msg.sender, address(this), amountIn);
        uint amountOut = amountIn * get_jaxre_digire_ratio() / 1e8;
        super._mint(msg.sender, amountOut);
        emit Swap_Jaxre_Digire(amountIn, msg.sender);
    }

    function _swap_digire_jaxre(uint amountIn, address to) internal returns (uint amountOut){
        ...
        emit Swap_Digire_Jaxre(amountIn, to);
    }

    function create_digire_busd_swap_request(uint amountIn) external returns (uint requestId){
        require(admin.isWhitelisted(msg.sender), "Not whitelisted");
        uint jaxreAmount = _swap_digire_jaxre(amountIn, admin.exchanger());
    ...
    function get_jaxre_digire_ratio() public view returns (uint) {
        if(JAXRE.balanceOf(address(this)) == 0) 
            return max_jaxre_digire_ratio;
        uint jaxre_digire_ratio = 1e8 * totalSupply / JAXRE.balanceOf(address(this));
        if(jaxre_digire_ratio > max_jaxre_digire_ratio)
            jaxre_digire_ratio = max_jaxre_digire_ratio;
        return jaxre_digire_ratio;
    }
