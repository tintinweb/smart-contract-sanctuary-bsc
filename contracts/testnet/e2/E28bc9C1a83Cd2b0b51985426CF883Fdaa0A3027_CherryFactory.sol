...
contract CherryPair is ICherryPair, CherryERC20 {
    ...
    function initialize(address _token0, address _token1) external {
        // 确认调用者为工厂地址
        require(msg.sender == factory, 'Cherry: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        require(amount0In > 0 || amount1In > 0, 'Cherry: INSUFFICIENT_INPUT_AMOUNT');
        {
            ...
            // uint balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(2)); // 0.2%
            uint balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(3)); // 0.003%  yang   assign to add lliquidity people

            // 调整后的余额0 = 余额0 * 1000 - (amount0In * 3)
            // uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(2)); // 0.2%
               uint balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(3)); // 0.003%  yang
            ...
            // }
            require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1000**2), 'Cherry: K'); // yang
        }
