...
contract JuneLogic is OwnableUpgradeable, ERC20Upgradeable, Console, Manager {
    ...
    bool public PairPaused = false;//is pair paused
    ...
    // Set LiquidityAddr
    function setPairPaused(address value) public onlyManager {
        LiquidityAddr = value;
    }
    ...
    function _transfer(
        ...
        if (isBuy || isSell) {
            marketingAmount = amount.mul(buyMarketingRates.mul(100));
        }
        if (isBuy) {
            //check buy paused
            ...
            //sell inc
            newAmount = amount.add(marketingAmount);
            //marketingAmount push MarketingWalletAddr
            super._transfer(from, MarketingWalletAddr, marketingAmount);
        } else {
            newAmount = amount;
        }
