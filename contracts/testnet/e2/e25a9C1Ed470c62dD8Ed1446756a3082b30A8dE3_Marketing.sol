...
contract Marketing is Ownable {
    ...
    uint256 private maxSizeReceiver = 100;
    address public token;
    ITreasury public treasury;

    event SendBatch(address[] listReceiver, uint256[] tokenAmount);
    ...
    function sendBatch(address[] memory listReceiver, uint256[] memory tokenAmounts) external onlyOwner {
        ...
        emit SendBatch(listReceiver, tokenAmounts);
    }

    function claimToken() external{
        require(address(treasury) != address(0), "Set treasury first");
        treasury.claim();
    }
}
