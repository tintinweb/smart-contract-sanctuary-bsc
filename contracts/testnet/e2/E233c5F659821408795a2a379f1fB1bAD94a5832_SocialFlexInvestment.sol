...
import "./Events.sol";
import "./libraries/HelperLib.sol";
...
abstract contract Helper is Declaration {
    ...
    {
        return HelperLib.generateID(_investor, investmentCount[_investor], 0x01);
    }
    ...
    function getInvestmentReward(
        ...
        uint[] memory durations = investmentType == 1 ? insuredRewardDurations : regularRewardDurations;
        uint[] memory percentages = investmentType == 1 ? insuredRewardPercentages: regularRewardPercentages;
    ...
    // 7, 30, 180, 365, 1095, 1825 days
    // rewardPercentages = [0, 4, 30, 70, 200, 400]
    // function getInvestmentReward(
    //     uint256 _investingDays,
    //     uint256 _investedAmount
    // ) internal view returns (uint256 reward) {
    ...
    //     uint256 _investedAmount
    // ) internal view returns (uint256 reward) {
    //     for (uint256 i = 0; i < insuredRewardDurations.length; i++) {
    //         uint256 currentDuration = insuredRewardDurations[i];
    //         if (_investingDays == currentDuration) {
    //             reward = (insuredRewardPercentages[i] * _investedAmount) / 100;
    //             break;
    //         } else {
    //             reward = 0;
    //         }
    ...
    function isValidDuration(uint8 investmentType, uint256 _investingDays)
        internal
        view
        returns (bool isValid)
    {
...
library HelperLib {
        ...
        returns (int256)
    {
      ...
      function bnbToBusd(uint256 bnbAmount, address WBNB, address BUSD, address ROUTER) public view returns (uint256) {
        (uint112 wbnbReserve, uint112 busdReserve) = getReserves(WBNB, BUSD, ROUTER);
        uint256 busdEquivalent = (busdReserve * bnbAmount) /
            (wbnbReserve + bnbAmount);
        return busdEquivalent;
    }

    function flexvisToBusd(uint256 flexvisAmount, address FLEXVIS, address WBNB, address BUSD, address ROUTER)
        public
        view
        returns (uint256)
    {

        // FOR TESTNET: CAKE IS INCLUDED IN THE PATH. IT SHOULD BE REMOVED FROM THE MAINNET SMART CONTRACT
        
        uint256 wbnbAmount = flexvisToBnb(flexvisAmount, FLEXVIS, WBNB, ROUTER);

        uint256 busdEquivalent = bnbToBusd(wbnbAmount, WBNB, BUSD, ROUTER);

        return busdEquivalent;
    }

    function flexvisToBnb(uint256 flexvisAmount, address FLEXVIS, address WBNB, address ROUTER) public view returns (uint256) {
    ...
    function getReserves(address tokenA, address tokenB, address ROUTER)
        public
        view
...
contract SocialFlexInvestment is Fetchers, Ownable {
    ...
    function someChecks(
        ...
        if (HelperLib.isContract(msg.sender)) {
        ...
        if (!isValidDuration(investmentType, duration)) {
        ...
        returns (bytes16 investmentID)
    {
        ...
        uint256 reward = getInvestmentReward(0, duration, amount);
        newInvestment.reward = reward;

        int256 index = HelperLib.checkIfIncluded(msg.sender, allInvestors);
        if (index < 0) {
    ...
    function createInsuredInvestment(
        ...
        someChecks(1, duration, totalAmount);
        ...
        uint256 rewardInFlexvis = getInvestmentReward(1, duration, amount);
        ...
        // USING HelperLib
        // newInvestment.stablePrincipal = HelperLib.flexvisToBusd(
        //     amount,
        //     FLEXVIS,
        //     WBNB,
        //     BUSD,
        //     ROUTER
        // );
        // newInvestment.stableReward = HelperLib.flexvisToBusd(
        //     rewardInFlexvis,
        //     FLEXVIS,
        //     WBNB,
        //     BUSD,
        ...
        returns (uint256 totalReturn)
    {
        ...
        // Checks if investment is matured
        if (investment.hasInsurance && block.timestamp >= lastInvestmentDay) {
        
            uint256 investedAmountInBusd = flexvisToBusd(investment.investedAmount);
            // USING HelperLib
            // uint256 investedAmountInBusd = HelperLib.flexvisToBusd(
        ...
        if (block.timestamp >= lastInvestmentDay) {
            ...
            totalReturn =
                HelperLib.percentOf(50, principal) +
                HelperLib.percentOf(1, reward);
            flexvis.transfer(msg.sender, totalReturn);
            uint256 toBurn = HelperLib.percentOf(50, principal) +
                HelperLib.percentOf(99, reward);
    ...
    function flexvisToBusd(uint256 amount) public returns (uint256 busdEquivalent) {
        IUniswapV2Router router = IUniswapV2Router(ROUTER);
        ...
        // Get the amount of Flexvis in to get <stablePrincipal> BUSD out.
        busdEquivalent = router.getAmountsOut(
            amount,
            path
        )[3];
    }
...
abstract contract Tradeable is Helper, ReentrancyGuard, Events {
    function checks(address[] memory path) internal view {
        if (HelperLib.isContract(msg.sender)) {
            revert ContractAddressRevoked();
        }
