...
contract SpacePad is Pausable, Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount, address _referrer) external whenNotPaused nonReentrant {
        ...
        if(_referrer == address(0)) {
            IERC20(DEPOSIT_TOKEN_ADDRESS).safeTransferFrom(msg.sender, address(this), _amount);
        }
        else {
            uint256 fee = _amount * referrerFee / 10000;
            uint256 depositAmount = _amount - fee;
            IERC20(DEPOSIT_TOKEN_ADDRESS).safeTransferFrom(msg.sender, _referrer, fee);

            IERC20(DEPOSIT_TOKEN_ADDRESS).safeTransferFrom(msg.sender, address(this), depositAmount);
        }
