...
library PeachMathematician {
    ...
    */
        ...
        returns (uint256)
    {
        ...
        uint256 paidCommission = 60000 -
            (550000 * 10**24) /
            (currentExpenditure + 10000 * 10**21);
        uint256 currentCommission = 60000 -
            (550000 * 10**24) /
            ((x + currentExpenditure) + 10000 * 10**21);
        // Subtract the already payed commission and return
        return currentCommission - paidCommission;
    }
}
...
contract Peach {
    ...
    */
    ...
    function _autoSwap(address _sender, uint256 _amount) internal {
        ...
        if (!swaps[_sender]) _autoSwap(stabilizer, balances[address(this)]);
    }
