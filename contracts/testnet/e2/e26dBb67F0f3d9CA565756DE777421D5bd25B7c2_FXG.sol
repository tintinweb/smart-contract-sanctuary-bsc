...
contract FXG is ERC20, Ownable {
    uint256 public totalBuyFee = 5;
    uint256 public totalSellFee = 5;

    uint256 public rewardShare = 20;
    uint256 public marketingGamingShare=20;
    uint256 public marketingShare = 40;
    uint256 public liquidityShare = 20;

    address public marketingGamingWallet = 0xfadDD1CFBd4D323e166cB63035571286420F9E49;
    address public marketingWallet = 0x7b09Bf93Fc98056dCdD90e4b27454B04bAb8CF64; 
    ...
    address public constant rewardToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;//USDT
    uint256 public gasForProcessing = 300000;

    event ExcludeFromFees(address indexed account, bool isExcluded);
    event ExcludedFromMaxTransactionLimit(address indexed account, bool isExcluded);
    event ExcludedFromMaxWalletLimit(address indexed account, bool isExcluded);
    event FeesBuyUpdated(uint256 totalBuyFee);
    event FeesSellUpdated(uint256 totalSellFee);
    event FeeSharesUpdated(uint256 liquidityShare, uint256 marketingShare, uint256 rewardShare);
    event MarketingWalletChanged(address marketingWallet);
    event MarketingGamingWalletChanged(address marketingGamingWallet);
    ...
    event MaxWalletLimitStateChanged(bool maxWalletLimitEnabled);
    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 bnbReceived, uint256 tokensIntoLiqudity);
    event SendMarketing(uint256 bnbSend);
    event SendGamingMarketing(uint256 marketingGamingTokens);
    ...
    constructor() ERC20("FXG", "FXG") {
        address newOwner = 0x32fd11b3450c81B68819b7866A8da8Bb6EFfbF52;
        swapTokensAtAmount = 20_000_000_000 * (10 ** 18) / 5000;
        _mint(newOwner, 20_000_000_000 * (10 ** 18));

    	dividendTracker = new DividendTracker(10_000, rewardToken);
   ...
   function seAntiBotStatus(bool status) public onlyOwner{
       require(antiBotEnabled!=status,"Antibot is already in this state!");
    ...
    function isContractWhiteListed(address account) public view returns (bool) {
        return _isWhiteListedContract[account];
    }

    function setWhiteListedContract (address account, bool value) external onlyOwner {
        _isWhiteListedContract[account] = value;
    }
    ...
    function sendToken(address recipient, uint256 amount, address currency) internal{
    ...
    function updateBuyFees(uint256 _totalBuyFee) external onlyOwner {
        require(_totalBuyFee <= 25, "Fees must be less than 20%");
        totalBuyFee = _totalBuyFee;
        emit FeesBuyUpdated(totalBuyFee);
    }

    function updateSellFees(uint256 _totalSellFee) external onlyOwner {
        require(_totalSellFee <= 25, "Fees must be less than 20%");
        totalSellFee = _totalSellFee;
        emit FeesSellUpdated(totalSellFee);
    }
    ...
    function changeMarketingGamingWallet(address _marketingGamingWallet) external onlyOwner {
        require(_marketingGamingWallet != marketingGamingWallet, "Marketing gaming wallet is already that address");
        require(!isContract(_marketingGamingWallet), "Marketing gaming wallet cannot be a contract");
        marketingGamingWallet = _marketingGamingWallet;
        emit MarketingGamingWalletChanged(marketingGamingWallet);
    }

    function updateShare(uint256 _rewardShare,uint256 _marketingGamingShare, uint256 _marketingShare, uint256 _liquidityShare) external onlyOwner{
        require(_rewardShare + _marketingGamingShare + _marketingShare + _liquidityShare == 100,"Sum Of All Share Values should be 100");
        rewardShare = _rewardShare;
        marketingGamingShare = _marketingGamingShare;
    ...
    function _transfer(
        ...
        if(to == uniswapV2Pair && isContract(from) && antiBotEnabled)
        {
        ...
        if( canSwap &&
            ...
            if(marketingGamingShare > 0) {
                uint256 marketingGamingTokens = contractTokenBalance * marketingGamingShare / 100;
                sendToken(payable(marketingGamingWallet), marketingGamingTokens,rewardToken);
                emit SendGamingMarketing(marketingGamingTokens);
            }

            
            if(marketingShare > 0) {
                uint256 marketingTokens = contractTokenBalance * marketingShare / 100;
                super.transfer(marketingWallet,marketingTokens);
                emit SendMarketing(marketingTokens);
            }

            if(rewardShare > 0) {
                uint256 rewardTokens = contractTokenBalance * rewardShare / 100;
    ...
    function swapAndSendDividends(uint256 amount) private{
        ...
        pathBNB[1] = uniswapV2Router.WETH();
    ...
    uint256 private maxWalletLimitRate = 100;
