...
contract GDCCBscCrossChain is Ownable, Pausable, ReentrancyGuard {
    ...
    event DepositBNB (address indexed user, uint256 indexed amount, uint time);
    event WithdrawBNB (address indexed user, uint256 indexed amount, uint time);
    ...
    event FailSafeForBNB(address indexed user, uint256 indexed amount, uint time);
    ...
    function bnbDeposit() external payable whenNotPaused isContractCheck(_msgSender()) nonReentrant{
        require(msg.value > 0, "INVALID BNB");

        emit DepositBNB(_msgSender(), msg.value, block.timestamp);
    }

    function bnbWithdraw(Sig memory _sig, address _user, uint256 _amount, uint _expiry) external whenNotPaused isContractCheck(_user) nonReentrant{
        require(block.timestamp <= _expiry,"EXPIRY_TIME_OUT");
        require(address(this).balance >= _amount, "INSUFFICIENT_BNB_IN_CONTRACT");
    ...
    function failSafeForBNB(address _user, uint256 _amount) external onlyOwner {
    ...
    function checkBNBBalance() external view returns(uint){
