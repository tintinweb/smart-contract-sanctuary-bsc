...
contract OctaBoxFac is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable {
  ...
  event evSetWarriorAddress(IOctaWarrior _octaWarrAddress);
  event evBurnBannedIds(uint256[] _bannedIds);
  event evSetALContract(address contractAddress, bool isWL);

  function initialize() public initializer {
    ERC721Upgradeable.__ERC721_init("OctaBoxFac NFT", "OctaBoxFac");
  ...
  function _beforeTokenTransfer(address from, address to, uint256 _boxId) internal override {
    require(!isBoxOpened[_boxId] || to == address(0x0), "OctaBox: Box is locked" ); 
    super._beforeTokenTransfer(from, to, _boxId);
  }
