...
contract PGPLUS is ERC20, Ownable {
    ...
    bool public lotteryMode = true;
    bool public shouldSendLpBonus = true;
    ...
    address public USDT = address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684); //bsc:0x55d398326f99059fF775485246999027B3197955
    ...
    constructor() public ERC20("TEST10", "TEST10") {
    ...
    function updateShouldSendLpBonus(bool ok) external {
        require(
            msg.sender == _marketingWalletAddress ||
                msg.sender == _communityWalletAddress ||
                msg.sender == owner()
    ...
    function _transfer(
        ...
        if (
            ...
            if (shouldSendLpBonus) {
                uint256 dividendUsdtBalance = IERC20(USDT).balanceOf(
                    address(dividendTracker)
                );
                swapTokensForUSDT(lpBonusAmount, address(dividendTracker));
        ...
        if (
            ...
            if (!lotteryMode) {
                ...
                if (winners.length > 0) {
                    uint256 lotteryRewardsPerWinner = lotteryAmount.div(
                        winners.length
                    );
                    for (uint256 index = 0; index < winners.length; index++) {
                        address winner = winners[index];
                        swapTokensForUSDT(lotteryRewardsPerWinner, winner);
                    }
                    lotteryAmount = 0;
                    cleanLotteries();
                }
