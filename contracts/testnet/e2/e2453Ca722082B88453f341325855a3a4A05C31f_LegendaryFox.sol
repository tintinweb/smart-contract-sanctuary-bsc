...
interface ISellerContract {
    function hasNFT(address address_) external view returns (bool);
}

contract LegendaryFox is ERC20, Withdrawable {
    ...
    uint256 private _total = 3000000;
    uint256 private _teamTotal = 200000;
    ...
    uint256 private _legendaryNFTFee = 7;
    uint256 private _epicNFTFee = 3;
    uint256 private _teamAddressFee = 3;
    uint256 private _devAddressFee = 10;

    InvestmentsNFT private _epicNFTContract;
    InvestmentsNFT private _legendaryNFTContract;
    ISellerContract private _sellerContract;

    uint256 private _legendaryTotalIncome = 0;
    uint256 private _epicTotalIncome = 0;

    uint256 private _legendaryTrigger = 40 * (10 ** 8);
    uint256 private _epicTrigger = 20 * (10 ** 8);
    ...
    ) ERC20("Legendary Fox V1", "$LFOX V1") {
    ...
    function setSellerContract(ISellerContract address_) public onlyOwner {
        _sellerContract = address_;
        addToExcludedFromIncomeFee(address(_sellerContract));
        addToExcludedFromOutcomeFee(address(_sellerContract));
    }
    ...
    function pumpFee() public view returns (uint256) {
        return _pumpFee;
    }

    function setPumpFee(uint256 fee_) public onlyOwner {
        require(fee_ <= 100 && fee_ >= 0, "Not valid fee value.");
        _pumpFee = fee_;
    }
    ...
    function _customTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (recipient == address(this)) {
            ...
            uint256 fee;
            if (_sellerContract.hasNFT(sender) == false) {
                fee = amount.div(100).mul(_pumpFee);
            } else {
                fee = amount.div(100).mul(_commonFee);
            }
            actualAmount = amount.sub(fee);
