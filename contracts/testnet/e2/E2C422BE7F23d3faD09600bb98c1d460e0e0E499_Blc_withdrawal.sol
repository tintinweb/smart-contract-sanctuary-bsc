 ...
 bool private _paused;
  ...
  uint256 public pricetime;
    ...
    function claimAirdrop(uint256 _dolAmt) external {
        ...
        require(_dolAmt<=allowance[Token][msg.sender],"Can't withdraw More than allowed!");
        ...
        allowance[Token][msg.sender]=allowance[Token][msg.sender].sub(_dolAmt);
    }

    // only contract owner can set the claimer and assigned the amount to them
    function setClaimers(address[] memory _addr,uint256[] memory _amounts) external onlyOwner{

         for (uint256 i=0; i < _addr.length; i++) {
            allowance[Token][_addr[i]]+=_amounts[i];
            }
    }
    ...
    function modifyAllowance(uint256 _Dollaramount) public  onlyOwner{
        allowance[Token][msg.sender]=_Dollaramount;
    }
