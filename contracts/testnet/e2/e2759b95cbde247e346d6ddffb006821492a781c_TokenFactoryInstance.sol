...
contract TokenFactory is Ownable {
    ...
    // rewardsToHolders flag
       ...
       function addBlacklist(address _addr) public onlyOwner{
        require(isBlacklist, "Blacklist is not enabled.");
        require(!isBlacklisted[_addr], "Address is already blacklisted.");
    ...
    function removeBlacklist(address _addr) public onlyOwner{
        require(isBlacklist, "Blacklist is not enabled.");
        require(isBlacklisted[_addr], "Address is not blacklisted.");
    ...
    function setBlacklist(bool _isBlacklist) public onlyOwner{
...
contract TokenFactoryInstance is Ownable {
    ...
    function createToken(string memory _name, uint256 _totalSupply, string memory _symbol, bool _mintable, bool _burnable) public payable returns(address) {
