...
contract FiveElements is Permission {
    ...
    IERC20 public usdtContract = IERC20(0xDF6bB0Cd22673Bd58eBA5Dab61EAa6e2603Fd0D6);

    uint public ticketPrice = 20 ether;
    address public clubAddress = 0x3becee29f89c7248648236A8cd522a44e067fC75; 
    address public maintainAddress = 0x3becee29f89c7248648236A8cd522a44e067fC75;
    IHash private hashAddress = IHash(0x94f137bF3C13378f905Aa159c2b61c9433e75ABd);
    ...
    function getAllComps(uint8 _comps) public view returns (address[] memory){
        ...
        for(uint16 i = 0; i < fiveCamps[_comps].length; i++){
            playerList[i] = fiveCamps[_comps][i];
        }
        return playerList;
    }

    function getMarquee() public view returns (Order[] memory){
        uint8 e = 0;
        if(marquee.length >= 10){
            Order[] memory newMarquee = new Order[](10);
            for(uint i = marquee.length - 1; i > marquee.length - 11; i--){
                newMarquee[e] = marquee[i];
                e = e + 1;
            }
            return newMarquee;
        }else if(marquee.length < 10 && marquee.length > 0){
            Order[] memory newMarquee = new Order[](marquee.length);
            for(uint i = marquee.length; i > 0; i--){
                newMarquee[e] = marquee[i - 1];
                e = e + 1;
            }
            return newMarquee;
        }
    ...
    function getAllOrderList(address _sender) public view returns (Order[] memory){
        ...
        for(uint i = orderList[_sender].length; i > 0; i--){
            newOrderList[a] = orderList[_sender][i - 1];
            a = a + 1;
        }
        return newOrderList;
    }

    function buyTicket(uint8 camp, uint16 amount) public {
    ...
    function buyTicketRecommender(uint8 camp, uint16 amount, address recommender) public {
        require(amount < 1001, "Amount is too many");
        require(!isContract(msg.sender), "It is contracts");
        require(endTime > block.timestamp && startTime <= block.timestamp, "The game is end or not start");
        require(camp > 0 && camp < 6,"The camp is not exist");
        require(usdtContract.balanceOf(msg.sender) >= amount * ticketPrice,"Your coin is not enough");
        require(msg.sender != recommender,"The recommender should not equal sender!");
        usdtContract.transferFrom(
            msg.sender,
            address(this),
            amount * ticketPrice
    ...
    function _buyTicket(uint8 camp, uint16 amount) private {
        if(endTime + 10 * amount > block.timestamp + 180){
            endTime = block.timestamp + 180;
        }else{
            endTime = endTime + 10 * amount;
        }
    ...
    function _buyTicketRecommender(uint8 camp, uint16 amount, address recommender) private {
        if(endTime + 10 * amount > block.timestamp + 180){
            endTime = block.timestamp + 180;
        }else{
            endTime = endTime + 10 * amount;
    ...
    function allocation() private{
        if(fiveCamps[0].length == 0){
            ...
            transferUsdt([clubAddress, maintainAddress], [(ticketPrice * 25 / 1000 ), (ticketPrice * 70 / 1000 )]);
        }else{
    ...
    function allocationRecommender(address recommender) private{
        if(fiveCamps[0].length == 0){
            ...
            transferUsdt([clubAddress, maintainAddress], [(ticketPrice * 25 / 1000 ), (ticketPrice * 50 / 1000 )]);
        }else{
            pool = pool + (ticketPrice * 380 / 1000 );
            recordBouns(getFrontAddress(), (ticketPrice * 525 / 1000 ));
            recordBouns(recommender, (ticketPrice * 20 / 1000 ));         
    ...
    function transferUsdt(address[2] memory toAddress, uint[2] memory _usdrAmount) private {
    ...
    function luckFive() private {
        ...
        uint winBouns = 0;  
        if(count < 8){
            pushOrderList(newHash, 0, 0);
        }else if(count == 8){
            winBouns = pool * 15 / 1000;
            pushOrderList(newHash, winBouns, 0);
        }else if(count == 9){
            winBouns = pool * 25 / 1000;
            pushOrderList(newHash, winBouns, 0);
        }else if(count == 10){
            winBouns = pool * 40 / 1000;
            pushOrderList(newHash, winBouns, 0);
        }else if(count == 11){
            winBouns = pool * 60 / 1000;
            pushOrderList(newHash, winBouns, 0);
        }else if(count == 12){
            winBouns = pool * 100 / 1000;
            pushOrderList(newHash, winBouns , 30);
        }else if(count == 13){
            winBouns = pool * 150 / 1000;
            pushOrderList(newHash, winBouns, 60);
        }else if(count == 14){
            winBouns = pool * 200 / 1000;
            pushOrderList(newHash, winBouns, 90);
        }else if(count == 15){
            winBouns = pool * 250 / 1000;
            pushOrderList(newHash, winBouns, 120);
        }else if(count >= 16){
            winBouns = pool * 500 / 1000;
            pushOrderList(newHash, winBouns, 200);
        }
        if(count >= 9){
            marquee.push(Order({orderAddress : msg.sender,orderTime : block.timestamp, orderHash: string(newHash), winBouns: winBouns}));
        }
    }

    function pushOrderList(bytes memory newHash, uint winBouns, uint redTime) private {
        pool = pool - winBouns;
        endTime = endTime - redTime;
        bouns[msg.sender] = bouns[msg.sender] + winBouns;
        allBouns[msg.sender] = allBouns[msg.sender] + winBouns;
        orderList[msg.sender].push(Order({orderAddress : msg.sender, orderTime : block.timestamp, orderHash: string(newHash), winBouns: winBouns}));
    }
    ...
    function endGame() public {
        ...
        if(allTicket == 0){
            uint newPool = pool;
            pool = 0;
            usdtContract.safeTransfer(clubAddress, newPool);
        }else{
            uint newPool = pool;
            pool = 0;
            onlyBouns = newPool * 500 / 1000;
            recordBouns(getAllComps(uint8(0))[allTicket - 1], onlyBouns);
            
            if(allTicket < 2){
                seatBouns = newPool * 100 / 1000;
                usdtContract.safeTransfer(clubAddress, seatBouns);
            }else if(allTicket >= 2 && allTicket < 20){
                seatBouns = newPool * 100 / 1000 / (getAllComps(uint8(0)).length - 1);
                for(uint16 i = 2; i < (getAllComps(uint8(0)).length); i++){
                    recordBouns(getAllComps(uint8(0))[allTicket - i], seatBouns);
                }
            }else{
                seatBouns = newPool * 5 / 1000;
                for(uint16 i = 2; i < 22; i++){
                    recordBouns(getAllComps(uint8(0))[allTicket - i], seatBouns);
                }
            }
            
            campBouns = newPool * 400 / 1000 / getAllComps(lastCamp).length;
            for(uint16 j = 0; j < getAllComps(lastCamp).length; j++){
    ...
    function setTime(uint _startTime, uint _pool) external onlyRole(MANAGER_ROLE){
        require(block.timestamp > endTime,"Game is not end!");
        isEnd = uint8(1);
        startTime = _startTime;
        endTime = _startTime + 180;
        usdtContract.transferFrom(
            msg.sender,
            address(this),
            _pool * 1e18
