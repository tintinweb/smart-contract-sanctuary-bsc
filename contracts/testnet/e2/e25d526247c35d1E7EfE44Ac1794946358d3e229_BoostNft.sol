    ...
    ERC1155SupplyUpgradeable
{
    ...
    function burn(
        ...
        IFarm(farm).withdrawNFT(account, address(this), id, value);
    ...
    function burnBatch(
        ...
        for (uint i = 0; i < ids.length; i++) {
            IFarm(farm).withdrawNFT(account, address(this), ids[i], values[i]);
        }
        _burnBatch(account, ids, values);
    }

    function adjustUserBoost(
        address _user,
        uint256 _id,
        uint256 _newAmount
    ) public onlyRole(OPERATOR_ROLE) {
        userInfo[_user][_id] -= _newAmount;
        IFarm(farm).withdrawNFT(_user, address(this), _id, _newAmount);
    }
    ...
    function _beforeTokenTransfer(
        ...
        for (uint i = 0; i < ids.length; i++) {
            require(
                (userInfo[to][ids[i]] += amounts[i]) <= maxStackable,
                "You reached your limit"
            );
            if (userInfo[from][ids[i]] > 0) {
                IFarm(farm).withdrawNFT(
                    from,
                    address(this),
                    ids[i],
                    amounts[i]
            ...
            IFarm(farm).depositNFT(to, address(this), ids[i], amounts[i]);
