...
contract LaunchpadFeature is Ownable, ReentrancyGuard, FixinTokenSpender {
    ...
    function launchpadBuy(
        ...
        _callLaunchpadBuy(slot, quantity, currentPrice);
    ...
    function _callLaunchpadBuy(DataType.LaunchpadSlot memory slot, uint256 buyQty, uint256 currentPrice) internal {
        uint256 price;
        ...
        // mintParams
        //      0: mint(address to)
        //      1: mint(address to, uint256 quantity)
        if (slot.mintParams == 0) {
            assembly {
                ...
                for { let i } lt(i, buyQty) { i := add(i, 1) } {
                    if iszero(call(gas(), targetContract, price, ptr, 0x24, ptr, 0)) {
            ...
            assembly {
                ...
                if iszero(call(gas(), targetContract, mul(buyQty, price), ptr, 0x44, ptr, 0)) {
                    // Failed, copy the returned data and revert.
                    returndatacopy(0, 0, returndatasize())
                    revert(0, returndatasize())
                }
