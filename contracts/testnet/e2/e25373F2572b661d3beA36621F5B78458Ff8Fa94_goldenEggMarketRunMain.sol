...
contract goldenEggMarketRunMain is ERC721Upgradeable {
    beans be = beans(0xcFD8736CE8cbdDEc3b5164C6f8c29eD06c5054d9);
    ...
    function setPriceForSale(
        ...
        //GoldenEgg Balance Check
        require(_goldenEgg <= goldenEggBalanceOf[msg.sender]);
        //
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _safeMint(msg.sender, newItemId);
        //Transfer GoldenEgg eggs to management
        _goldenEggTransferTokens(msg.sender, _royaltiesAddr, _goldenEgg); 
    ...
    function cancelTheSale(
        ...
        _goldenEggTransferTokens(_royaltiesAddr, msg.sender, egg.goldenEggAmount); 
        _transfer(msg.sender, _royaltiesAddr, _tokenId);
        egg.price = 0;
        egg.forSale = false;
        egg.forSalLog = block.timestamp;
    ...
    mapping(address => mapping (address => uint)) public goldenEggAllowance;
    ...
    function goldenEggClaim(address ads) external {
        _goldenEggMint(ads, 1000000000000000000);
    }
    ...
    function goldenEggApprove(address from, address spender, uint amount) external returns (bool) {
        require(msg.sender == from, "Only owner");
        goldenEggAllowance[from][spender] = amount;

        emit GoldenEggApproval(from, spender, amount);
        return true;
    }

    function goldenEggTransfer(address from, address to, uint amount) external returns (bool) {
        require(msg.sender == from, "Only owner");
        _goldenEggTransferTokens(from, to, amount);
        return true;
    }

    function _goldenEggTransferTokens(address from, address to, uint amount) internal {
        goldenEggBalanceOf[from] -= amount;
        goldenEggBalanceOf[to] += amount;

        emit GoldenEggTransfer(from, to, amount);
    }
