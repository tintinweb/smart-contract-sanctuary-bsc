...
contract TMEToken is ERC20, SafeOwnable {
    ...
    uint256 public constant BASE_RATIO = 10**18; //基础计算单位 1=1
    uint256 public constant MAX_FEE = (20 * BASE_RATIO) / 100; //最大费用 20%
    uint256 public constant SPY = (0 * BASE_RATIO) / 10000 / 1 days; //SPY 0.00% 取消持币复利
    uint256 public immutable rewardEndTime; //收益结束日子
    mapping(address => bool) private minner; //minter 
    mapping(address => bool) public whitelist; //白名单
    mapping(address => uint256) public lastUpdateTime; //上次更新时间

    mapping(address => bool) public rewardBlacklist; //收益黑名单
    uint256 public fundFeePercent = (1 * BASE_RATIO) / 100; //基金 1%
    uint256 public marketFeePercent = (1 * BASE_RATIO) / 100; //营销 1%
    uint256 public devFeePercent = (1 * BASE_RATIO) / 100; //技术 1%
    uint256 public liquidityFeePercent = (1 * BASE_RATIO) / 100; //流动性 1%
    uint256 public nftPoolFeePercent = (4 * BASE_RATIO) / 100; //NFT池子 4%
    uint256 public currentAllFee =
        fundFeePercent + marketFeePercent + devFeePercent + liquidityFeePercent + nftPoolFeePercent; //当前总费用 8%

    bool private inSwapAndLiquify; //是否开放LP池操作，默认是False
    uint256 public minSwapAndLiquifyLimit = 100 * 10**18; //最小加池限制 100
    uint256 public minSwapLimit = 100 * 10**18; //最小swap限制 100
    IReferral public referralHandle;
    IERC20 public usdtToken;
    address public liquidity; //lp地址
    address public fund; //基金地址
    address public market; //营销地址
    address public dev; //开发地址
    address public lpReceiver; //lp代币接收地址
    address public nftPool; //NFT池子地址
    address private smartVault; //金库地址
    IRouter public router; //路由合约
    uint256 public extraSupply; //额外供应
    bool public canTransfer; //能否转账
    bool public canSwap = true; //能否Swap,默认开启
    ...
    ); //交易事件：swap的token，收到的eth， 进lp的token
    event AddWhitelist(address account); //白名单添加事件

    event DelWhitelist(address account); //白名单删除事件
    //构造函数（代币名，代币缩写，精度，基金地址，lp接收地址，Usdt地址，工厂合约地址，路由合约地址
    constructor(
        string memory _name,
        ...
        liquidity = _factory.createPair(_usdtToken, address(this)); //创建池子
        router = _router;

        rewardEndTime = block.timestamp.add(730 days); //收益截至时间到730天
        setRewardBlacklist(liquidity, true); //把lp池加进收益黑名单
        setRewardBlacklist(address(this), true); //把合约自身地址加进收益黑名单
    ...
    //设置Minner中address的bool值,或者新增Minner
    ...
    function calculateFee(
        ...
        //如果是转账，把金库的代币全部转到本合约地址换成u发给基金地址
    ...
    function addLiquidity(uint256 usdtAmount, uint256 tokenAmount) private {
        // 本Token合约地址给router合约授权Token额度
        _approve(address(this), address(router), tokenAmount);
        // 把对应代币数量的U从金库转回至token合约地址
        usdtToken.transferFrom(smartVault, address(this), usdtAmount);
        // 本Token合约地址给router合约授权Usdt额度
    ...
    //改为true才能对池子操作，执行完加池之后就关闭池子操作权限
