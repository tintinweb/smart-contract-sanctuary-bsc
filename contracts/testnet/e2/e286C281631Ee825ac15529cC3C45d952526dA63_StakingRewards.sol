...
import { AssetData } from "../interfaces/interfaces.sol";
...
contract StakingRewards is Initializable {
  ...
  function configEmissionPerSecond(
    ...
    // udpate asset state before changing emission per second
  ...
  function stake(uint256 _amount) external {
    ...
    if (accruedRewards != 0) {
      rewardsToClaim[msg.sender] += accruedRewards;
  ...
  function withdraw(uint256 _amount, bool _isClaimAllRewards) external returns (bool) {
    ...
    uint256 cooldownEndTimestamp = cooldownStartTimestamp + COOLDOWN_SECONDS;
    
    require(block.timestamp > cooldownEndTimestamp, 'INSUFFICIENT_COOLDOWN');    
    require((block.timestamp - cooldownEndTimestamp) <= UNSTAKE_WINDOW, 'UNSTAKE_WINDOW_FINISHED');
  ...
  function getNextCooldownTimestamp(
    ...
    if (minimalValidCooldownTimestamp > toCooldownTimestamp) {
      ...
      if (timestamp < toCooldownTimestamp) {
        ...
        toCooldownTimestamp =
          ((_amountToReceive * timestamp) + (_toBalance * toCooldownTimestamp)) / (_amountToReceive + _toBalance);
      }
    }
    return toCooldownTimestamp;
  }   
  ...
  function claimTokenRewards(uint256 _amount) public {
    ...
    require(newTotalRewards >= amountToClaim, 'INVALID_AMOUNT');

    rewardsToClaim[msg.sender] = newTotalRewards - amountToClaim;
  ...
  function updateCurrentUnclaimedRewards(
    ...
    uint256 unclaimedRewards = rewardsToClaim[_user] + accruedRewards;
  ...
  function getAssetIndex(
    ...
    return (((_emissionPerSecond * timeDelta) * (10**uint256(PRECISION))) / totalSupply) + _currentIndex;
  }
  ...
  function getRewards(
    ...
    return (_principalUserBalance * (_reserveIndex - _userIndex)) / (10**uint256(PRECISION));
  }
  ...
  function getUnclaimedRewards(
    ...
    accruedRewards += getRewards(balances[msg.sender], assetIndex, assetData.users[_user]);
    return accruedRewards;
  }
  ...
  function earnedTokenRewards(address _user) external view returns (uint256) {
    return rewardsToClaim[_user] + getUnclaimedRewards(_user);
  }
  ...
  function getCooldownEndTimestamp(address _user) external view returns (uint256) {    
    return cooldowns[_user] + COOLDOWN_SECONDS;
  }
  ...
  function getWithdrawEndTimestamp(address _user) external view returns (uint256) {    
    return (cooldowns[_user] + COOLDOWN_SECONDS) + UNSTAKE_WINDOW;
  }
}
