/**
    ...
    SPECS:
    - 3-6% daily
    - 200 Tokens Max Wallet TVL
    - 5 Tokens Max Reward Accumulation 
...
contract BakedBeansBusd is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function initialize() public initializer {
        ...
        TOKEN_PER_BEAN = 1000000000000000;
        ...
        MIN_BAKE = 10000000000000000000; // 10 Tokens
        MAX_WALLET_TVL = 20000000000000000000000; // 20000 Tokens
        MAX_DAILY_REWARDS = 500000000000000000000; // 500 Tokens
        MIN_REF_DEPOSIT_FOR_BONUS = 50000000000000000000; // 50 Tokens
    ...
    function buyBeans(address ref, uint256 tokens) public {
        ...
        _token.transferFrom(address(msg.sender), address(this), tokenValue);
    ...
    function sendFees(uint256 totalFee, uint256 giveAway) private {
        ...
        _token.transfer(_dev, dev);
        _token.transfer(_marketing, marketing);
        _token.transfer(_ceo, ceo);

        if (giveAway > 0) {
            _token.transfer(_giveAway, giveAway);
        }
    }
     ...
     function handleBake(bool onlyRebaking) private {
        ...
        if (onlyRebaking == true) {
            require(
                beansToToken(rewardedBeans(baker.adr)) >= MIN_BAKE,
                "Rewards must be equal or higher than 0.01 tokens to bake"
            );
        }
    ...
    function eat() public {
        ...
        uint256 tokensToEat = Math.sub(beansInTokensBeforeFee, totalFee);
        uint256 forGiveAway = calcGiveAwayAmount(baker.adr, tokensToEat);
        tokensToEat = addWithdrawalTaxes(baker.adr, tokensToEat);

        if (
            Math.add(beansInTokensBeforeFee, baker.totalPayout) >=
            maxPayout(baker.adr)
        ) {
            tokensToEat = Math.sub(maxPayout(baker.adr), baker.totalPayout);
    ...
    function calcGiveAwayAmount(address adr, uint256 withdrawalAmount)
        private
        view
        returns (uint256)
    {
        return (percentFromAmount(withdrawalAmount, hasBeanTaxed(adr)) / 2);
    }
    ...
    function dailyReward(address adr) public view returns (uint256) {
        ...
        if (referralsCount < 10) return 30000000;
        if (referralsCount < 25) return (35000000);
        if (referralsCount < 50) return (40000000);
        if (referralsCount < 100) return (45000000);
        if (referralsCount < 150) return (50000000);
        if (referralsCount < 250) return (55000000);
        return 60000000;
    }
    ...
    function calcBeansReward(
        ...
        uint256 rewardsPerDay = percentFromAmount(
            Math.mul(bakers[adr].beans, 100000000000),
            dailyRewardFactor
        );
        ...
        beansRewarded = Math.div(beansRewarded, 1000000000000000);
        return beansRewarded;
    }
}
