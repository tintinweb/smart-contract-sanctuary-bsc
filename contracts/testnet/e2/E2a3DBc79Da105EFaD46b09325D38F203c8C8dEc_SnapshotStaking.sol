...
contract SnapshotStaking is ISnapshotStaking, AccessControl {
    ...
    function _add(
        ...
        uint256 dayRemainder = (_endTime - _startTime) %
            DAY_IN_SECONDS;
        if (dayRemainder > 0) {
        ...
        returns (uint256 _reward, uint256 _rewardTime)
    {
        ...
        uint256 lastRewardTime = user.lastRewardTime > pool.startTime ? user.lastRewardTime : pool.startTime;
        ...
        _reward =
            (lpSupply * multiplier * pool.apr) /
            (APR_DENOMINATION * 365);
    }
    ...
    function stake(uint256 _pid, uint256 _amount) external _positive(_amount) {
        ...
        console.log(stakedAmount[address(pool.stakeToken)]);
        ...
        _positive(_amount)
    {
        ...
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][sender];
        require(user.amount >= _amount, "Cannot withdraw more then staked amount");
        ...
        stakedAmount[address(pool.stakeToken)] -= _amount;
        console.log("unstake:", stakedAmount[address(pool.stakeToken)]);
    ...
    function withdrawRewards(address _token) external onlyRole(STAKING_ADMIN_ROLE) {
        uint256 totalBal = IERC20(_token).balanceOf(address(this));
        console.log("totalBal:", totalBal);
        console.log(stakedAmount[_token]);
