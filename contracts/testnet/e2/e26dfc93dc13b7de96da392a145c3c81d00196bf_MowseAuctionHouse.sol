...
// File: contracts/MowseAuctionHouse.sol
...
contract MowseAuctionHouse is Initializable, Payments {
    ...
    */
    ...
    function createScheduledAuction(
        ...
        require(
            auctions[_nftAddress][_tokenId].auctionType == NO_AUCTION ||
                (msg.sender !=
                    auctions[_nftAddress][_tokenId].auctionCreator),
    ...
    function bid(
        ...
        require(
         ...
         require(_amount > auction.Bids[_bidId].amount , "Bid must be higher than previous bid");

        (bool present , uint256 bidId) = getUserBid(_contractAddress, _tokenId, msg.sender);
    ...
    function updateBid(
        ...
        for (uint256 i = 1; i <= auction._bidId.current(); i++) {
            if(msg.sender == auction.Bids[i].bidder){
                isBidPlaced = true;
                bidderIndex = i;
                break;
            }
    ...
    function acceptBid(
        ...
        require(auction.Bids[_bidId].bidder != address(0), "Bid does not exist.");

        require(auction.auctionCreator == msg.sender, "Caller is not the auction creator.");
    ...
    function auctionPayout(address _contractAddress, uint256 _tokenId , uint256 auctionId)
        external
     {
        require(
        ...
        require(
            auction.auctionType != NO_AUCTION &&
                auction.startingTimeInSeconds != 0,
            'settleAuction::Must have a current auction that has started' //@TODO compare with block.timestamp
        );
        require(
            block.timestamp >=
        ...
        uint256 acceptedBidId = acceptedBids[auctionId];
        ...
        bool _isPrimarySale = !iMarketSettings.hasERC721TokenSold(_contractAddress, _tokenId);
        uint8 _royaltyPercentage = iERC721CreatorRoyalty.getERC721TokenRoyaltyPercentage(_contractAddress,_tokenId);
        uint8 _buyerRewardPercentage = iMarketSettings.getBuyerRewardFeePercentage();
        // address _royaltyPayee = iERC721CreatorRoyalty.tokenCreator(_contractAddress, _tokenId);

        // address owner = owner();

        address _payee; address _buyer; address _owner; address _royaltyPayee;

       (_payee,  _buyer,  _owner,  _royaltyPayee ) = getAllAddress(_contractAddress, _tokenId, auctionId);
     ...
     function getAllAddress(address _contractAddress, uint256 _tokenId, uint256 auctionId) public view returns(address, address, address, address){
          address _royaltyPayee = iERC721CreatorRoyalty.tokenCreator(_contractAddress, _tokenId);
            Auction storage auction = auctions[_contractAddress][_tokenId];
             ...
             return(auction.auctionCreator, currentBid.bidder, owner, _royaltyPayee);
     }
     ...
     * @param _tokenId uint256 id of the token.
     */
    function getAuctionDetails(address _contractAddress, uint256 _tokenId) external view
        returns (
            bytes32,
            uint256,
            address,
        ...
        )
    {
        ...
        return (auction.auctionType,auction.creationTimeInSeconds,auction.auctionCreator,auction.lengthOfAuctionInSeconds,auction.startingTimeInSeconds,auction.minimumBid,auction.reservePrice,auction.endTimeInEpochSeconds,auction.auctionId);
    }
     ...
     * @param _tokenId uint256 id of the token.
     */
    function getAllBids(address _contractAddress, uint256 _tokenId)
        public
        view
        returns (Bid [] memory)
    {
    uint256 totalItems = auctions[_contractAddress][_tokenId]._bidId.current();
       
      mapping(uint256 => Bid) storage bidMap = auctions[_contractAddress][_tokenId].Bids;
...
//     }
     ...
     */
    function getUserBid(address _contractAddress, uint256 _tokenId , address _bidderAddress)
        public
        view
    ...
    function getBidbyId(address _contractAddress, uint256 _tokenId,uint256 _id)
        public
        view
        returns (
            address,
    ...
    {
      mapping(uint256 => Bid) storage bidMap = auctions[_contractAddress][_tokenId].Bids;
       return (bidMap[_id].bidder,bidMap[_id].marketplaceFee, bidMap[_id].amount);
          }
    /////////////////////////////////////////////////////////////////////////
