...
contract RobinHoodStacking {
    ...
    // SHA256 encoded for privacy
    mapping(string => bool) public telegram_check;
    mapping(address => string) public wallet_telegram;
    ...
    // Events
    event tokensStaked(address from, uint256 amount, string telegramHandle);
    event TokensUnstaked(address to, uint256 amount, string telegramHandle);
    ...
    constructor(IERC20 _erc20_contract_address) {
        ...
        timePeriod = 432000; // 3600 * 24 *30
        amountToLock = 100 * 10**8;
        locked = false;
    }
    ...
    function stakeTokens(IERC20 token, string calldata telegramHandle) public noReentrant {
        
        require(balances[msg.sender] == 0, "You already staked");
        ...
        require(amountToLock <= token.balanceOf(msg.sender), "Not enough STATE tokens in your wallet, please try lesser amount");
        token.safeTransferFrom(msg.sender, address(this), amountToLock);
        balances[msg.sender] = balances[msg.sender].add(amountToLock);
        entryTimestamps[msg.sender] = block.timestamp;
        wallet_telegram[msg.sender] = telegramHandle;
        telegram_check[telegramHandle] = true;
        emit tokensStaked(msg.sender, amountToLock, telegramHandle);
    }
    ...
    /// Unstakes all tokens
    function unstakeTokens(IERC20 token, string memory telegramHandle) public noReentrant {
        ...
        if (block.timestamp >= entryTimestamps[msg.sender] + timePeriod) {
            telegramHandle = wallet_telegram[msg.sender];
            telegram_check[telegramHandle] = false;
            balances[msg.sender] = 0;
            token.safeTransfer(msg.sender, balances[msg.sender]);
            emit TokensUnstaked(msg.sender, balances[msg.sender], telegramHandle);
        } else {
            revert("Tokens are only available after correct time period has elapsed");
        }
    }

    function lockEnds(address account) external view returns (uint256) {
        return entryTimestamps[account] + timePeriod;
    }

    function changeAccess(string calldata telegramHandle, bool status) public onlyOwner{
        telegram_check[telegramHandle] = status;
    }

    function setAmountToLock(uint256 amount) public onlyOwner{
        amountToLock = amount;
    }
