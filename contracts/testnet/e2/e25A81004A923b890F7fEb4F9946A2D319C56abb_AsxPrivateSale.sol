...
contract AsxPrivateSale is Ownable, Whitelist {
    struct PaymentToken {
    ...
    uint256 private MIN_DEPOSIT = 100 * 10 ** 18;
    uint256 private MAX_DEPOSIT = 2000000 * 10 ** 18;
    uint256 private LISTING_PRICE = 0.02 * 10 ** 18;
    uint256 private MAX_TOTAL_SALE = 10000000 * 10 ** 18;
    ...
    event Deposit(address sender, uint amount, address token, uint asxAmount,  uint vestingId);
    ...
    function depositETH() public payable {
        ...
        uint256 totalSales = SafeMath.add(TOTAL_SALES, asxAmount);

        require(IERC20(ASX_TOKEN).balanceOf(address(this)) > asxAmount, "No available token.");
        require(totalSales <= MAX_TOTAL_SALE, "Insufficient ASX token.");
        ...
        IERC20(ASX_TOKEN).approve(LOCKER, asxAmount);
        uint256 vestingId = PinkLock02(LOCKER).vestingLock(
            msg.sender,
            ASX_TOKEN, 
            false, 
            asxAmount, 
        ...
        emit Deposit(msg.sender, msg.value, address(0), asxAmount, vestingId);
    }
    ...
    function depositToken(address token, uint256 amount) public {
        ...
        emit Deposit(msg.sender, amount, token, asxAmount, vestingId);
    }
