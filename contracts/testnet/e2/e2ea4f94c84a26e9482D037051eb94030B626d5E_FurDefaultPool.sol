...
/*
 * The Default Pool holds the FUR and FURUSD debt (but not FURUSD tokens) from liquidations that have been redistributed
 * to active vaults but not yet "applied", i.e. not yet recorded on a recipient active vault's struct.
 *
 * When a vault makes an operation that applies its pending FUR and FURUSD debt, its pending FUR and FURUSD debt is moved
 * from the Default Pool to the Active Pool.
 */
contract FurDefaultPool is BaseContract, IFurDefaultPool {
    ...
    event DefaultPoolFURBalanceUpdated(uint _FUR);
    ...
    function sendFURToActivePool(uint _amount) external override whenNotPaused {
        _requireCallerIsVaultManager();
        furToken.safeTransfer(activePoolAddress, _amount);

        uint256 FUR = getFUR();
        emit DefaultPoolFURBalanceUpdated(FUR);
        emit FURSent(activePoolAddress, _amount);
