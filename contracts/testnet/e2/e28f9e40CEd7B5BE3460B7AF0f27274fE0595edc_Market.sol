    ...
    EIP712Upgradeable
{
    ...
    mapping(address => bool) override public currencies;
    ...
    function whitelistCurrencies(address[] calldata _currencies, bool whitelist)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _currencies.length; i++) {
            currencies[_currencies[i]] = whitelist;
        }
    }
    ...
    function bulkPriceSet(
        ...
        require(
            tokenIds.length == erc20Addresses.length &&
                erc20Addresses.length == amounts.length,
            "Data missing"
        );

        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(prices[tokenIds[i]].amount == 0, "Only for new token");
            require(currencies[erc20Addresses[i]], "Unsupported currency");

            if (amounts[i] == 0) {
                continue;
            }
            prices[tokenIds[i]] = Shared.Offer(
                tokenIds[i],
                erc20Addresses[i],
                amounts[i],
                block.timestamp,
        ...
        onlyNewSignature(offer.signature)
    {
        ...
        require(currencies[offer.erc20Address], "Invalid currency");
