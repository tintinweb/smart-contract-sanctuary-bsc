...
contract HODL is Context, IBEP20, Ownable, ReentrancyGuard {
        ...
        initializer
    {
        ...
        triggerwallet = 0x02E6f7198C7F8036b681f2ABa9b74640ee596A83;

        _liquidityFee = 20;
        _previousLiquidityFee = _liquidityFee;

        minTokenNumberToSell = 10000000000000000; //_tTotal.mul(1).div(10000).div(10);
        minTokenNumberUpperlimit = 200000000000000000000; //_tTotal.mul(2).div(100).div(10);

        rewardHardcap = 1 ether; // 20 ether;
    ...
    {      
        ...
        */
        return (tAmount.sub(tFee).sub(tLiquidity), tFee, tLiquidity);
    }
        ...
        )
    {
        ...
        return (rAmount, rAmount.sub(rFee).sub(rLiquidity), rFee);
    }
    ...
    function swapAndLiquify(
        ...
        if (contractTokenBalance >= minTokenNumberUpperlimit &&
            ...
            !(from == address(this) && to == address(pancakePair))
                ...
                (bool sent, ) = marketingwallet.call{value: deltaBalance.mul(tokenomics.marketing).div(100)}("");
                require(sent, "Error: Cannot send reward");

                // send resere rewards
                (bool succ, ) = reservewallet.call{value: deltaBalance.mul(tokenomics.reserve).div(100)}("");
    ...
    function triggerSwapAndLiquify() public lockTheSwap {
        require(_msgSender() == triggerwallet, "Caller is not the triggerwallet");
        require(swapAndLiquifyEnabled, "Swap and liquify not enabled");
        //check triggerwallet balance
        if (triggerwallet.balance < 0.1 ether) {
            (bool sent, ) = triggerwallet.call{value: 0.1 ether}("");
        ...
        // send marketing rewards
        (bool sentm, ) = marketingwallet.call{value: deltaBalance.mul(tokenomics.marketing).div(100)}("");
        require(sentm, "Error: Cannot send reward");

        // send resere rewards
        (bool sentr, ) = reservewallet.call{value: deltaBalance.mul(tokenomics.reserve).div(100)}("");
    ...
    function activateContract() public onlyOwner {
        ...
        // approve contract
        _approve(address(this), address(pancakeRouter), 2**256 - 1);
        IWBNB(payable(address(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd))).approve(
            address(pancakeRouter),
            2**256 - 1
        );
    }
