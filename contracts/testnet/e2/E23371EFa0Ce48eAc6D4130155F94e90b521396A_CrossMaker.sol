...
contract CrossMaker is Node, IMaker, Ownable, SessionManager {
    ...
    function _payTransactionFeeLP(address lp, uint256 principal) internal virtual returns (uint256 feesPaid) {
        if (actionParams.isUserAction) {
            feesPaid = CrossLibrary.transferFeesFrom(lp, address(this), principal, feeRates[actionParams.actionType], feeStores, nodes.token);
        }
    }
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        CrossLibrary.lightTransferFrom(_token, address(this), to, IERC20(_token).balanceOf(address(this)) - balance0, nodes.token);
        IWETH(WETH).withdraw(amountETH);
        TransferHelper.safeTransferETH(to, amountETH);
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"bb3186be78404b3f6dc82f2c640cd653aa831860a67d0da48a3757c9f64ed44f" // init code hash
                        )
                    )
                )
            )
