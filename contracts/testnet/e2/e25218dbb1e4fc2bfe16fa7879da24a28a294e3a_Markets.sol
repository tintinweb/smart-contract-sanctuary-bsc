...
contract Markets {
    ...
    function setMoneyWhite(
        ...
        bool enable,
        uint256 priceMin,
        uint256 feeRatio ) external CheckPermit() {
    ...
    function getMyPurchasedOrders(address owner,uint256 startIndex,uint256 endIndex,uint256 resultLength) external view returns (Order[] memory) {
        if (endIndex == 0) {
    ...
    function _addOrder(address owner, address nft, uint256 nftId, address money, uint256 price) internal {
    ...
    function buyForETH(address nft, uint256 nftId) external payable {
        ...
        // Order memory order = orders[nft][idx];
        // if (order.money == address(0)) {
        //     require(msg.value == order.price,"invalid amount");
        // }  
        _buy(msg.sender, nft, nftId); 
    }
