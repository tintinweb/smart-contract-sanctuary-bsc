...
contract TEST is IBEP20, Auth {
    ...
    address routerAddress = 0xD99D1c33F9fC3444f8101754aBC46c52416550D1; //WETHDex router
    ...
    uint256 public WETHRewardsFee = 50;
    uint256 public developmentFee    = 40; 
    uint256 public liquidityFee  = 10;
    uint256 public totalFees     = WETHRewardsFee + developmentFee + liquidityFee;
    uint256 public feeDenominator = 1000;
    uint256 public reducedFeePercentage = 0;

    address public devWallet = 0x1F178dDf53E194da7A6d5303BE8D4d589344dF12;
    ...
    function setFeeRates(uint256 _WETHRewardsFee, uint256 _liquidityFee, uint256 _developmentFee, uint256 _feeDenominator) public onlyOwner {
            WETHRewardsFee = _WETHRewardsFee;
            liquidityFee = _liquidityFee;
            developmentFee = _developmentFee;
            totalFees = _WETHRewardsFee + _liquidityFee + _developmentFee;
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 totalWETHFee = totalFees.sub(liquidityFee.div(2));
        
        uint256 amountWETHLiquidity = amountWETH.mul(liquidityFee).div(totalWETHFee).div(2);
        uint256 amountWETHReflection = amountWETH.mul(WETHRewardsFee).div(totalWETHFee);
        uint256 amountWETHTeam = amountWETH.sub(amountWETHLiquidity).sub(amountWETHReflection);

        try dividendDistributor.deposit{value: amountWETHReflection}() {} catch {}
                
        if(developmentFee > 0){
            payable(devWallet).transfer(amountWETHTeam);
        }
