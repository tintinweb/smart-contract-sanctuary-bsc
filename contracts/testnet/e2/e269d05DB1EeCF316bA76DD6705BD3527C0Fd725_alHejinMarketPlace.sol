...
error InvalidFee(uint64 _fee);
error InvalidCurrency(string  _currency);
error InvalidOwner(address _address);
error InvalidAuctionId(uint _auctionId);

contract alHejinMarketPlace is  AccessControlEnumerable, Allowances, IERC721Receiver {
    using Counters for Counters.Counter;
    Counters.Counter private camelTracker;

    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    
    using Al_HejinStorage  for Al_HejinStorage.camelOnSale;
    
    using EnumerableSet for EnumerableSet.UintSet;
    using SafeERC20 for IERC20;

    using EnumerableSet for EnumerableSet.UintSet;
    EnumerableSet.UintSet private camelIndexes;
    
    IERC20 private ERC20;
    IERC721 private ERC721;

    uint64 private platformfee = 369;

    address private platformWallet;

    event NewAuction(
       uint indexed camelId,
       uint indexed price
    );
    ...
    event CamelBuyed(
        uint indexed onionId,
        uint indexed cardId,
        uint indexed priceOfCamel
    );
    event FeeRoyaltyUpdated(
        uint64 indexed fee
    );
    event FeePlatformUpdated(
        uint64 indexed fee
    );

    constructor(address ERC721ContractAddress,address platFormWalletAddress)  {
        _setupRole(DEFAULT_ADMIN_ROLE,_msgSender());
        _setupRole(ADMIN_ROLE, _msgSender());
        ERC721 = IERC721(ERC721ContractAddress);
        platformWallet = platFormWalletAddress;
    }
    mapping(uint => Al_HejinStorage.camelOnSale) private alHejinMarket;
    ...
    function updatePlatformAddress(address _address) external onlyRole(ADMIN_ROLE) {
    ...
    */
    function updatePlatformFee(uint64 fee) external onlyRole(ADMIN_ROLE) {
        if( fee > 10000 ){
            revert InvalidFee(fee);
        }
        platformfee = fee;
        emit FeePlatformUpdated(fee);
    }

    function platformfeeValue()public view returns(uint64){
        return platformfee;
    }

    function _createCamelAuction(
        address _owner,
        uint _camelId,
        uint _pricePerCard,
        string memory _currency
        ) private {
        camelTracker.increment();
        uint camelId = camelTracker.current();
        alHejinMarket[camelId].set(
            _owner,
            _camelId,
            _pricePerCard,
            _currency
        );
        camelIndexes.add(camelId);
    }

    
    function createCamelAuction(
        uint camelId,
        address owner,
    ...
    {          
        if( !_allowedCurrency(currency )){ revert InvalidCurrency(currency); }
        ERC721.safeTransferFrom(msg.sender, address(this), camelId, '' );
            _createCamelAuction(
                owner,
                camelId,
                pricePerCard,
        ...
        emit NewAuction(camelId, pricePerCard);
    }
   
    function _removeCamelAuction(
        uint  _auctionId
        ) private{
        
        delete alHejinMarket[_auctionId]; 
        camelIndexes.remove(_auctionId); 

    }

    function removeCamelAuction(
        uint auctionId
    ) external 
    {
        if( !alHejinMarket[auctionId].isCamelOnSale()){ revert InvalidAuctionId(auctionId); }
        address owner = alHejinMarket[auctionId].camelOwner();
        uint camelId = alHejinMarket[auctionId].auctionCamelId();
        if( owner != msg.sender){ revert InvalidOwner(msg.sender); }
        _removeCamelAuction(auctionId);
        ERC721.safeTransferFrom(address(this), owner, camelId, '');

        emit CancelAuction(auctionId);
    }

    function feeValue(uint price, uint64 _fee) internal pure returns(uint) {
        uint fee = (price * _fee) / 10000;
        return fee;
    }
    ...
    function buyCamel(
        ...
        uint camelId = alHejinMarket[auctionId].auctionCamelId();
        address owner = alHejinMarket[auctionId].camelOwner();
        string memory currency = alHejinMarket[auctionId].auctionCurrency();
    
        if( !alHejinMarket[auctionId].isCamelOnSale()){ revert InvalidAuctionId(auctionId); }
        if( !_allowedCurrency(currency )){ revert InvalidCurrency(currency); }

        uint priceOfCamel = alHejinMarket[auctionId].priceOfcamel();
        address contractCurrency = addressCurrency(currency);

        uint camelPlatformFee = feeValue(priceOfCamel, platformfee);
        
        IERC20(contractCurrency).transferFrom(msg.sender, platformWallet, camelPlatformFee); 

        uint remaining_price = priceOfCamel - camelPlatformFee;

        IERC20(contractCurrency).transferFrom(msg.sender, owner, remaining_price);

        delete alHejinMarket[auctionId]; 
        camelIndexes.remove(auctionId);  

        ERC721.safeTransferFrom(address(this), to, camelId, '');

        emit CamelBuyed(auctionId, camelId, priceOfCamel);

    } 

    function getAllCamelAuctionId() public view returns(uint[] memory){
        uint indexMax = camelAuctionIndex();
        uint[] memory camelAuctionIds = new uint[](indexMax);
        for (uint index = 0; index < indexMax; ++index) {
            uint onionId= getCamelAuctionByIndex(index);
            camelAuctionIds[index] = onionId;
        }
        return camelAuctionIds;
    }

    function batchDetailsCamelAuction(uint[] memory camelIds) public view returns(Al_HejinStorage.camelOnSale[] memory){
        Al_HejinStorage.camelOnSale[] memory detailsCamelAuction = new Al_HejinStorage.camelOnSale[](camelIds.length);
        for (uint index = 0; index < camelIds.length; ++index) {
            detailsCamelAuction[index] = alHejinMarket[camelIds[index]];
        }
        return detailsCamelAuction;
    }

    function camelAuctionIndex() public view returns(uint){
        return camelIndexes.length();
    }

    function getCamelAuctionByIndex(uint index) public view returns(uint){
        return camelIndexes.at(index);
    }  
...
library Al_HejinStorage{
    ...
    function set(
        ...
        self.owner = _owner;
        self.camelId = _camelId;
        self.pricePerCamel = _pricePerCamel;
        self.currency = _currency;
    }

    function userAuctionDetails(
        camelOnSale storage self
    ) internal view returns(address, string memory, uint, uint) 
    {
        return (self.owner, self.currency, self.camelId, self.pricePerCamel);
    }

    function isCamelOnSale(
        camelOnSale storage self
    )internal view returns(bool)
    {
        return self.camelId != 0;
    }
    ...
    function auctionCamelId(
        camelOnSale storage self
    ) internal view returns(uint){
        return self.camelId;
    }

    function priceOfcamel(
        camelOnSale storage self
        ) internal view returns(uint){
        return self.pricePerCamel;
    }

    function auctionCurrency(
        camelOnSale storage self
