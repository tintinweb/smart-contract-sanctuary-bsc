...
contract Safemoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function _getCurrentSupply() public view returns(uint256, uint256) {
       ...
       // t1Supply = tSupply.add(totReflection);
        ...
        r1Supply = r1Supply.sub(_rTotalSupply);
    ...
    function getTotalReflection() public view returns(uint256){
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            a = a.add(_rOwned[_excluded[i]]);
        }
        return a;
    }

    function refelectionOwened(uint i) public view returns(uint256){
        return _rOwned[_excluded[i]];
    }
    function refelectionOwenedAcc(uint i) public view returns(address){
        return _excluded[i];
    }
    ...
    // function values(uint256 tAmount, uint256 index) public view returns(uint256 memory){
    //     FeeValues memory _values = _getValues(tAmount, index);
    //     return (_values);
    // }
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount, uint256 tierIndex) private {
        ...
        _rTotalSupply = _rTotalSupply.add(_values.rFee * 2);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, uint256 tierIndex) private {
        ...
        _rOwned[recipient] = _rOwned[recipient].add(_values.rTransferAmount);
        //totReflection = totReflection.add(_values.tFee);
        //_rTotalSupply = _rTotalSupply.add(_values.rFee);
        _takeFees(sender, _values, tierIndex);
        _reflectFee(_values.rFee, _values.tFee);
        emit Transfer(sender, recipient, _values.tTransferAmount);
    }
