    ...
    UUPSUpgradeable
{
    ...
    function setTokenContractAddressAdmin(
        address _address
    ) external onlyOwner returns (bool) {
    ...
    function _BuyWithETH(
        ...
        IERC20Upgradeable(tokenContract).transfer(_address, _tokenValue);
    }
    ...
    function BuyWithETH(
        ...
        if (isBuyNStake) {
            _stake(_userAddress, _tokenValue, _duration);
        } else {
            _BuyWithETH(_userAddress, _tokenValue, _msgValue);
        }

        if (isPayReferral) {
            payable(referralContract).transfer(
                _msgValue.div(
                    IReferral(referralContract).getTotalReferralBonus()
                )
            );
        ...
        _updateTokenSale(_userAddress, _tokenValue, _msgValue, "ETH");
    }
    ...
    function BuyWithUSD(
        ...
        if (isBuyNStake) {
            ...
            _BuyWithUSD(_userAddress, _tokenValue, _value);
        }

        if (isPayReferral) {
            IERC20Upgradeable(USDContract).transfer(
                referralContract,
                _value.div(IReferral(referralContract).getTotalReferralBonus())
            );
        ...
        _updateTokenSale(_userAddress, _tokenValue, _value, "USDT");
    }
