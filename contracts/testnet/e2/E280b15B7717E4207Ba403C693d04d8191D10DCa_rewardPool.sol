...
contract Ownable is OwnableData {
    ...
    function setCmlAddress(address _cml) public {
    ...
    /// @notice Only allows the `owner` to execute the function.
    modifier onlyCml() {
        require(msg.sender == cml, "Ownable: caller is not the cml");
        _;
    }
...
contract rewardPool is Ownable {
    ...
    uint public rewardDuration = 86400;
    constructor(address _cmlAddress) {     
    ...
    function pendingRewardFor(uint _nftId) public view returns (uint256 _reward) {
        ...
        uint _daysSinceLastClaim = ((block.timestamp - _lastClaim).mul(1e9)) / rewardDuration;
    ...
    function claimReward(uint _nftId) public returns (bool success) {
        ...
        nftInfo[_nftId].lastClaim = block.timestamp;
        IERC20(cml).transfer(nftInfo[_nftId].owner, _reward);
        return true;
    }

    function setRewardDuration(uint duration) external onlyOwner {
