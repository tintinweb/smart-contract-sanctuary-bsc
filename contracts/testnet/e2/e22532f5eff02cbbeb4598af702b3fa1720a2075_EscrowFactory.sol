...
contract EscrowFactory is OwnableUpgradeable {
    ...
    mapping(string => mapping(string => address))
        private userIdToDealIdToEscrowProxyAddr;
    ...
    /**
     * @dev Sets the address of initial implementation.
     */
    function initialize(address _vLogic) public initializer {
        __Ownable_init();
        beacon = new EscrowBeacon();
        beacon.initialize(_vLogic);
    }

    /**
     * @dev Creates a beacon proxy and sets user's details.
     *
    ...
    function createEscrowProxy(
        ...
        BeaconProxy proxy = new BeaconProxy(
            ...
            abi.encodeWithSelector(
                Escrow(address(0)).initializeDeal.selector,
                _commissionWallet,
    ...
    function setUserDealDetails(
        ...
        userIdToDealIdToEscrowProxyAddr[_userId][_dealId] = _escrowAddress;
    }

    function updateBeaconImpl(address _vLogic) external {
        beacon.update(_vLogic);
    }
...
contract EscrowBeacon is OwnableUpgradeable {
