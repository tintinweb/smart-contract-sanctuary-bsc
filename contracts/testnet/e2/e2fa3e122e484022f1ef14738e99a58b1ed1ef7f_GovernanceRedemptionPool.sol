...
contract GovernanceRedemptionPool is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public constant MINIMAL_REDEEM_DURATION = 1 days;
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _tokens.length;) {
            ...
            if (_tokens[i] != _tokenOut) {
                ...
                pool.swap(_tokens[i], _tokenOut, 0, address(this), "");
                _amountOut += IERC20(_tokenOut).balanceOf(address(this)) - beforeBalance;
            } else {
    ...
    function setRedeemDuration(uint256 _duration) external onlyRole(ADMIN_ROLE) {
        require(
            _duration >= MINIMAL_REDEEM_DURATION,
            "GovernanceRedemptionPool::setRedeemDuration: < MINIMAL_REDEEM_DURATION"
        );
    ...
    function stopRedemption() external onlyRedemptionActive onlyRole(CONTROLLER_ROLE) {
        snapshot.startTimestamp = 0;
        snapshot.endTimestamp = 0;
