...
contract SelfServiceEditionCurationV4 is Whitelist, Pausable {
  ...
  address public optionalSplitAddress = 0x45CbCBf16E1251d2019bEdb940f70Cb6F12068b0;

  uint256 private randNum = 0;

  uint256 public boxPrice = 20000000000000000;

  address public blindBoxReceiver = 0x45CbCBf16E1251d2019bEdb940f70Cb6F12068b0;

  mapping(uint256 => mapping(uint256 => uint256)) public blindBoxNFTEditionNumber;

  mapping(uint256 => mapping(uint256 => uint256)) public blindBoxNFTRarityNumber;

  mapping(uint256 => uint256) public indexToTypeId;
  mapping(uint256 => uint256) public typeIdToIndex;

  uint256[4] public blindBoxNFTRarity = [0,4000,7000,9000];

  uint256 public NFTTypes = 0;

  bool public blindBoxOpen = true;

  uint256 public presaleStartTime = 1641909600;
  uint256 public presaleEndTime = 1648468800;
  ...
  function setNFTTypes(uint256 types) public {
    require(msg.sender == owner, "Must be owner");
    NFTTypes = types;
  }

  function setOptionalSplitAddress(address receiver) public {
    require(msg.sender == owner, "Must be owner");
    optionalSplitAddress = receiver;
  }

  function setblindBoxReceiver(address receiver) public {
    require(msg.sender == owner, "Must be owner");
    blindBoxReceiver = receiver;
  }

  function setBlindBoxOpen(bool isOpen) public {
    require(msg.sender == owner, "Must be owner");
    blindBoxOpen = isOpen;
  }

  function setBoxPrice(uint256 price) public {
    require(msg.sender == owner, "Must be owner");
    boxPrice = price;  
  }

  function setNFTRates(uint256 num1, uint256 num2, uint256 num3, uint256 num4) public {
    require(msg.sender == owner, "Must be owner");
    require(num1 + num2 + num3 + num4 == 10000, "Wrong input");

    blindBoxNFTRarity[1] = num1;
    blindBoxNFTRarity[2] = num1 + num2;
    blindBoxNFTRarity[3] = num1 + num2 + num3;
  }
  ...
  {
    require(msg.sender == owner, "Must be owner");
    require(_totalAvailable > 0, "Quantity must be greater than 0");
    require(rarityId >= 1 && rarityId <= 4, "Rarity must be 1 to 4");

    if(typeIdToIndex[typeId] == 0){
      NFTTypes++;
      typeIdToIndex[typeId] = NFTTypes;
      indexToTypeId[NFTTypes] = typeId;
    }

    blindBoxNFTRarityNumber[typeId][rarityId] = _totalAvailable;

    uint256 editionNumber = _createEdition(
      msg.sender,
      false,
      [_totalAvailable, 0, _startDate, _endDate, 0, _editionType],
      _tokenUri
    );

    blindBoxNFTEditionNumber[typeId][rarityId] = editionNumber;
  ...
  function openBlindBox(uint256 times) public payable returns(uint256[] memory) {
    ...
    // require(times <= 100, "The number of openings is more than 100");
    ...
    for(uint256 i = 0; i < times; i++){
      uint256 index = uint256(keccak256(abi.encodePacked(block.timestamp, (randNum++) * block.number, msg.sender))) % NFTTypes + 1;

      uint256 typeId = indexToTypeId[index];

      uint256 rarity = getNFTType(typeId);

      do  {
            index = uint256(keccak256(abi.encodePacked(block.timestamp, (randNum++) * block.number, msg.sender))) % NFTTypes + 1;

            typeId = indexToTypeId[index];
      ...
      uint256 _tokenId = kodaV2.mintTo(msg.sender, _editionNumber);
  ...
  returns (uint256 _editionNumber)
  {
    ...
    if (_optionalSplitRate > 0 && _optionalSplitAddress != address(0)) {
      kodaV2.updateOptionalCommission(editionNumber, _optionalSplitRate, optionalSplitAddress);
    }
...
interface IKODAV2SelfServiceEditionCuration {
  ...
  function mintTo(address _to, uint256 _editionNumber) external returns (uint256);
}
