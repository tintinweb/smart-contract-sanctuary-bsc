...
// File: contracts/Me/MeConsensus.sol
...
contract MeConsensus is Ownable {
  struct ConsensusPool {
    uint8     star; //星级
    uint256   amount; //奖励金额
    uint256   claimed; //领取
    uint256   unclaimed; //已确认但未领取的奖励
    uint256   birthday; //奖励开始时间
  }

  IERC20Ext public meToken;
  MyNftEX public myNftEX;
  ConsensusPoo public consensusPool;
  mapping(address => uint256) public receivedReward;//已提现收益
  mapping(uint8 => uint256) public poolPrices;
  mapping(address => address[]) public recommendedData;//被推荐人数组
  ...
  constructor(address _meToken_address, address _myNftEX_address, address _consensus_pool_address, address security_fund_) {
    ...
    _security_fund = security_fund_;//保障基金合约
    _dao_reward = address(0xf92DD02Dfd9577Cedcd62E9291261B862E74f0bc);//DAO奖励
  ...
  function setMyNftEX(address nftAddr) public onlyOwner {
    _myNftEX = nftAddr;
    myNftEX = MyNftEX(nftAddr);
  }

  function setConsensusPool(address consensusPoolAddr) public onlyOwner {
    _consensus_pool = consensusPoolAddr;
    consensusPool = ConsensusPoo(_consensus_pool);
  }
  ...
  function getUsdtAmountForMe(uint256 meAmount) public view returns(uint256) {
    IUniswapV2Pair pair = IUniswapV2Pair(meToken.pair());
    (uint256 amountMeInPair, uint256 amountUsdtInPair) = getLiquidityPairAmount(pair);
    return (amountUsdtInPair * meAmount) / amountMeInPair;
  }
  ...
  function availStarToPurchase(address account) public view returns(uint8) {
    ...
    if (residual == 0)
      ...
      star = 6;
    else if (residual > 3000 ether && residual <= 4200 ether)
      star = 7;
    return star;
  }
  ...
  function consensus() public {
    ...
    require(referrer != address(0), "[MeConsensus] Cannot purchase because you have no referrer!");
    uint8 star = availStarToPurchase(msg.sender);
    require(star != 0, "[MeConsensus] Star maximum reached");

    uint256 meAmount = getMeAmountForUsdt(poolPrices[star]);
    require(meToken.balanceOf(msg.sender) >= meAmount, "[MeConsensus] Insufficient ME balance to purchase consensus pool!");
    meToken.transferFrom(msg.sender, address(this), meAmount);

    //共识池
    uint256 consensusPoolAmount = (meAmount*50)/100;
    meToken.transfer(_consensus_pool, consensusPoolAmount);

    //保障基金
    uint256 securityFundAmount = (meAmount*10)/100;
    meToken.transfer(_security_fund, securityFundAmount);
    ...
    for(uint256 i = 0; i < referralRewards.length; i ++) {
      ...
      ConsensusPool[] storage cpArray = cpools[upline];
      for(uint256 j = 0; j < cpArray.length; j ++) {
        ConsensusPool memory c = cpArray[j];
        uint rRate = getRewardRate(upline);
        uint256 dayPassed = (block.timestamp - c.birthday) / rewardDuration; //几天
        uint256 reward = (dayPassed * c.amount * rRate) / 10000 + c.unclaimed;
    ...
    uint recmLen = recommendedData[referrer].length;
    bool haveAddress = false;
    if(recmLen > 0){
      for(uint256 i = recmLen - 1; i >= 0; ) {
        if(recommendedData[referrer][i] == msg.sender){
          haveAddress = true;
          break;
        }
        if (i > 0) {
    ...
    if(haveAddress == false){
      recommendedData[referrer].push(msg.sender);
    }
    ...
    //DAO
    uint256 daoRewardAmount = (meAmount*5)/100;
    meToken.transfer(_dao_reward, daoRewardAmount);
  ...
  function getRewardRate(address account) public view returns(uint) {
    uint recmCount;
    uint recmLen = recommendedData[account].length;
    if(recmLen > 0){
      for(uint256 i = recmLen - 1; i >= 0; ) {
        if(getPersonalEffectiveConsensus(recommendedData[account][i]) > 0){
          recmCount++;
        }
        if(recmCount > 9){
          break;
  ...
  //获取共识参与
  function getPersonalEffectiveConsensus(address account) public view returns(uint256 personalEffectiveConsensus) {
    ConsensusPool[] storage cpArray = cpools[account];
    for(uint8 i = 0; i < cpArray.length; i ++) {
      ConsensusPool memory c = cpArray[i];
      personalEffectiveConsensus += c.amount;
    }
  }

  //获取累计收益
  function getTotalReward(address account) public view returns(uint256 totalReward) {
    totalReward = getReward(account) + receivedReward[account];
  }
  ...
  function claimReward() public {
    ...
    require(tReward > 0, "[MeConsensus] No reward to claim!");
    receivedReward[account] += tReward; //统计已提现收益
    uint256 meAmount = getMeAmountForUsdt(tReward);
    require(meToken.balanceOf(_consensus_pool) >= meAmount, "[MeConsensus] Insufficient ME token balance in the contract!");
  ...
  function withdrawByOwner() public onlyOwner {
    ...
    require(amount > 0, "[MeConsensus] No ME token to withdraw!");
