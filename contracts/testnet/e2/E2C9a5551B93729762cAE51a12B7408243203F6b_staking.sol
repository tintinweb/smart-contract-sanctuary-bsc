...
contract staking is Ownable {
    ...
    function invest(uint256 investment,address reffer) public payable {
        ...
        if(msg.value==0 && investment>0){
            ...
            users.deposites.push(depoite(amount.sub(tax).add(refferalFee), block.timestamp,true,0,block.timestamp));
            }else{
            ...
            if(reffer==address(0) || reffer==msg.sender||reffer==address(this)){
            users.deposites.push(depoite(amount.sub(tax).add(refferalFee), block.timestamp,false,block.timestamp,0));
            }else{
                users.refferAddress=reffer;
