...
contract GEMUNIFarming is PermissionGroupUpgradeable, ReentrancyGuardUpgradeable, IGEMUNIFarming {
    ...
    function addPoolToken(
        ...
        for (uint256 i = 0; i < poolInfoTokens.length; i++) {
            ...
            uint endTime = pool.timePoolInfo.startTime + pool.rewardBalance / pool.rewardPerSecond;
            if (address(pool.stakedToken) == stakedToken && address(pool.rewardToken) == rewardToken && pool.timePoolInfo.stakingPeriod == stakingPeriod && block.timestamp < endTime && pool.isActive) {
                foundToken = true;
                break;
            }
            foundToken = false;
