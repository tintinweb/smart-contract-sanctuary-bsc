...
contract ReflectionToken is IERC20, Ownable {
    ...
    constructor(
        ...
        require(_fees[0]+(_fees[3])+(_fees[6]) <= 300);
        require(_fees[1]+(_fees[4])+(_fees[7]) <= 300);
        require(_fees[2]+(_fees[5])+(_fees[8]) <= 300);
    ...
    function updateMaxWalletAmount(uint256 _maxWalletAmount)
        ...
        onlyOwner
    {
        require(_maxWalletAmount <= _tTotal, "max wallet amount <= total supply");
