...
contract RichMonkey is Context, Ownable{
    ...
    event SwapLog(uint256 _usdt_fee,uint256 _valut,uint256 _usdt,uint256 rmt,uint256 usdt,uint256 _fefe,uint256 f,uint256 e);
    ...
    function swapss(uint256 _amount,uint256 min_amount,bool _Is) public workstatus{
        ...
        if(_Is){
            ...
            emit SwapLog(usdt_fee,_valut,_usdt,rmt,usdt,_rmt_w,_usdt_w,_fee);
        }else{
            path[0] = usdtToken;
            path[1] = rmtToken;
    ...
    function swaps(uint256 _amount,uint256 min_amount,bool _Is) public workstatus{
        require(_amount>0, "EE: amount not enough");
        address[] memory path;
        path = new address[](2);
        bool _isc_b ;
