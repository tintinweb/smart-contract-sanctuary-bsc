...
contract EMET_LpMining is ERC721Holder, Ownable, RandomGenerator{
    ...
    uint[4] private _bonusCoe = [50, 25, 15, 10]; // /1000
    ...
    struct User_NumberOfDraws {
        uint[5] depositSlot;
        uint claimedSweepstakes;
        uint usedSweepstakes;
        uint boxAmount;
    }
    // mapping (uint => mapping(address => uint[5])) public depositSlot_BoxTime;
    mapping (uint256 => PoolInfo_Box) public poolInfo_Box;
    //  pool ID - slot - user
    mapping (uint256 => mapping(uint => mapping(address => UserInfo_Box))) public userInfo_Box;
    //  pool ID - user
    mapping (uint256 => mapping(address => User_NumberOfDraws)) public numberOfDraws;
    ...
    function teamBonus(address outputToken, uint bonus) internal {
        ...
        for (uint i=0; i<10; i++) {
            if (_invitor[i] == address(0)) {
                ...
                if(i == 0) {
                    ERC20(outputToken).transfer(_invitor[i], _bonus * _bonusCoe[0] / 1000);
                    bonus -= _bonus * _bonusCoe[0] / 1000;
                } else if (1 <= i && i < 4) {
                    ERC20(outputToken).transfer(_invitor[i], _bonus * _bonusCoe[1] / 3000);     
                    bonus -= _bonus * _bonusCoe[1] / 3000;
                } else if (4 <= i && i < 7) {
                    ERC20(outputToken).transfer(_invitor[i], _bonus * _bonusCoe[2] / 3000);     
                    bonus -= _bonus * _bonusCoe[2] / 3000; 
                } else if (7<= i && i< 9) {
                    ERC20(outputToken).transfer(_invitor[i], _bonus * _bonusCoe[3] / 3000);
                    bonus -= _bonus * _bonusCoe[3] / 3000;  
                } else if (i == 9) {
    ...
    function deposit_Share(uint256 pid_, uint256 amountIn_)public returns(bool){
        ...
        require(amountIn_ >= pool.share, '0 is not good');
        require (pool.status && block.timestamp >= pool.startTime && block.timestamp < pool.endTime, 'deposit no good, status');
    ...
    function updataUserReward_Box(uint pid_, uint slot_, address user_)public view returns(uint _reward) {
        UserInfo_Box storage user = userInfo_Box[pid_][slot_][user_];
    ...
    function updataUserReward_BoxForAll(uint pid_, address user_)public view returns(uint _reward) {
        User_NumberOfDraws storage number = numberOfDraws[pid_][user_];
        
        uint len = number.depositSlot.length;
        uint slot;
        for(uint i = 0; i<len; i++) {
            if (number.depositSlot[i] != 0) {
                slot = number.depositSlot[i];
                _reward += updataUserReward_Box(pid_, slot, user_);
            }
        }
    }

    function updataUserRewardTime_Box(uint pid_, uint slot_,address user_) internal {
        UserInfo_Box storage user = userInfo_Box[pid_][slot_][user_];
        PoolInfo_Box storage pool = poolInfo_Box[pid_];
        while ((block.timestamp - user.rewrdTime) > pool.updataTime) {
            user.rewrdTime += pool.updataTime;
    ...
    function preDeposit_Box(uint pid_) internal returns(uint slot) {
        User_NumberOfDraws storage number = numberOfDraws[pid_][msg.sender];
        uint len = number.depositSlot.length;
        for(uint i=0; i<len; i++){
            if(number.depositSlot[i] == 0) {
                number.depositSlot[i] = block.timestamp;
                slot = number.depositSlot[i];
                break;
            }
        }
        require(slot != 0, "slot is full");
    }

    function deposit_Box(uint pid_ , uint256 amountIn_)public returns(uint){
        uint slot_ = preDeposit_Box(pid_);
        require(slot_ != 0, "slot is full");
        UserInfo_Box storage user = userInfo_Box[pid_][slot_][msg.sender];
        PoolInfo_Box storage pool = poolInfo_Box[pid_];
        require(amountIn_ >= pool.share, '0 is not good');
        require(user.amount == 0, "used slot");
        require(pool.status && block.timestamp >= pool.startTime && block.timestamp < pool.endTime, 'deposit no good, status');

        user.rewrdTime = slot_;
        // user
        user.amount += amountIn_;
        user.depositTime = slot_;
        user.lastClaimtTime = slot_;
    ...
    function claim_Box(uint256 pid_, uint slot_)public returns(uint){
        UserInfo_Box storage user = userInfo_Box[pid_][slot_][msg.sender];
        PoolInfo_Box storage pool = poolInfo_Box[pid_];
        User_NumberOfDraws storage number = numberOfDraws[pid_][msg.sender];
        ...
        if (pool.lastRewardTime > user.rewrdTime) {
            _reward = updataUserReward_Box(pid_, slot_, msg.sender);
            reward = _reward;
        }

        if (reward > 0) {
            // user
            number.claimedSweepstakes += reward;

            user.lastClaimtTime = block.timestamp;
            updataUserRewardTime_Box(pid_, slot_, msg.sender);
            // pool
    ...
    function claim_BoxAll(uint256 pid_)public returns(uint reward){
        User_NumberOfDraws storage number = numberOfDraws[pid_][msg.sender];
        uint len = number.depositSlot.length;
        uint _re;
        uint[5] memory list = number.depositSlot;
        for(uint i=0; i<len; i++) {
           if(list[i] != 0) {
               _re = claim_Box(pid_, list[i]);
               reward += _re;
           }
        }
    }
    // Withdraw LP tokens from Contract.
    function withdraw_Box(uint256 pid_, uint slot_) public returns(bool){
        UserInfo_Box storage user = userInfo_Box[pid_][slot_][msg.sender];
        PoolInfo_Box storage pool = poolInfo_Box[pid_]; 
        require(user.amount >= 0, "withdraw: amount not good");
        uint amountOut = user.amount;

        claim_Box(pid_, slot_);
        if (amountOut > 0 ){
            ERC20(pool.lpToken).transfer(address(msg.sender), amountOut);
        }
        user.amount = 0;
        pool.lpTVL -= amountOut;

        emit Withdraw(msg.sender, pid_ , amountOut);
        userInfo_Box[pid_][slot_][msg.sender] = UserInfo_Box({
            amount:0,
            depositTime:0,
            rewrdTime:0,
            lastClaimtTime:0
        });
        afterWithdraw(pid_, slot_);
        return true;
    }
    function afterWithdraw(uint pid_, uint slot_) internal {
        User_NumberOfDraws storage number = numberOfDraws[pid_][msg.sender];
        uint len = number.depositSlot.length;
        for(uint i=0; i<len; i++){
            if(number.depositSlot[i] == slot_) {
                number.depositSlot[i] = 0;
                break;
            }
        }
    }
    ...
    function randomBox(uint pid_, uint time_) public returns(uint boxs) {
        ...
        require(time_ <= (number.claimedSweepstakes - number.usedSweepstakes));
    ...
    // ----------------------------------------  Front  --------------------------------------------
    function checkBalance_Allowance(address lp, address user_) public view returns(uint256 ba, uint256 all){
        ba = ICnnPair(lp).balanceOf(user_);
        all = ICnnPair(lp).allowance(user_, address(this));
    }
    ...
    function checkAblePool() public view returns(uint[] memory pools, bool[] memory bools){
        ...
        for(uint b=100; b<lenShare;b++) {
            pools[x] = b;
            bools[x] = poolInfo_Share[b].status;
            x+=1;
        }
        for(uint c=1000; c<lenBox;c++) {
            pools[x] = c;
            bools[x] = poolInfo_Box[c].status;
            x+=1;
        }
        return(pools, bools);
    }

    function checkInfoTypePosShare (uint[] memory pid_, address user_)
    public view returns (address[] memory lpToken, uint[] memory depositTimes, 
    uint[7][] memory uintList, string[2][] memory symbels) {
        uint len = pid_.length;
        lpToken = new address[](len);
        depositTimes = new uint[](len);
        uintList =new uint[7][](len);
        symbels = new string[2][](len);

        // uint x = 0;                                           
        for (uint i=0; i<len; i++) {
            if(pid_[i]>=10 && pid_[i]<100) {
                // (lpToken[i], depositTimes[i], amounts[i], toClaims[i], claimeds[i], uintList[i], symbels[i]) = checkPosInfo(pid_[i], user_);
                (lpToken[i], depositTimes[i], uintList[i], symbels[i]) = checkPosInfo(pid_[i], user_);
            } else if (pid_[i]>=100 ) {
                // (lpToken[i], depositTimes[i], amounts[i], toClaims[i], claimeds[i], uintList[i], symbels[i]) = checkShareInfo(pid_[i], user_);
                (lpToken[i], depositTimes[i], uintList[i], symbels[i]) = checkShareInfo(pid_[i], user_);
            } 
        }
    }

    function checkInfoTypeBox (uint[] memory pid_, address user_)
    public view returns (address[] memory lpToken, uint[] memory depositTimes, uint[7][] memory uintList, string[2][] memory symbels) {
        // User_NumberOfDraws storage number = numberOfDraws[pid_][msg.sender];
        uint len = pid_.length;
        uint listlen = pid_.length * 5;
        lpToken = new address[](listlen);
        depositTimes = new uint[](listlen);
        uintList =new uint[7][](listlen);
        symbels = new string[2][](listlen);
        uint slot;
        uint _index;
        for (uint i=0; i<len; i++){
            if (i >0){
            ...
            for(uint x=0; x<5; x++) {
                slot = numberOfDraws[pid_[i]][user_].depositSlot[x];
                (lpToken[_index + x], depositTimes[_index + x], uintList[_index + x], symbels[_index + x]) = checkBoxInfo(pid_[i], slot, user_);
                // (lpToken[index], depositTimes[index], amounts[index], toClaims[index], claimeds[index], uintList[index], symbels[index]) = checkBoxInfo(pid_, slot, user_);
            }
        }
    }

    function checkPosInfo(uint pid_, address user_) public view 
    returns(address _lpToken, uint _depositTimes, uint[7] memory uintList, string[2] memory _symbel) {
        PoolInfo_Pos storage pool = poolInfo_Pos[pid_];
        UserInfo_Pos storage user = userInfo_Pos[pid_][user_];
        _symbel = ["null", "null"];
        ...
        if(_lpToken != address(0)){
            ...
            (uintList[0],uintList[1]) = checkBalance_Allowance(_lpToken, user_);
            uintList[2] = 0;
            uintList[3] = 0;
        }

        _depositTimes = user.depositTime;
        uintList[4] = user.amount;
        uintList[5] = user.claimed;
        uint temp1 =  updataUserReward_Pos(pid_, user_);
        uint temp2=0;
        if(user.lockReward > 0) {
            temp2 =  updataLockReward_Share(pid_, msg.sender);
        }
        uintList[6] = user.toClaim + temp1 + temp2;
    }
    ...
    returns(address _lpToken, uint _depositTimes, uint[7] memory uintList, string[2] memory _symbel ) {
        ...
        if(_lpToken != address(0)){
            ...
            _symbel[1] = ERC20(_t1).symbol();
            (uintList[0],uintList[1]) = checkBalance_Allowance(_lpToken, user_);
            uintList[2] = user.rewrdTime;
            uintList[3] = pool.updataTime;
        }
    ...
    function checkBoxInfo(uint pid_, uint slot_, address user_) public view 
    returns( address _lpToken, uint _depositTimes, uint[7] memory uintList, string[2] memory _symbel) {
        PoolInfo_Box storage pool = poolInfo_Box[pid_];
        UserInfo_Box storage user = userInfo_Box[pid_][slot_][user_];
        User_NumberOfDraws storage number = numberOfDraws[pid_][user_];
        ...
        _depositTimes = user.depositTime;
        uintList[4] = user.amount;
        uintList[5] = number.claimedSweepstakes;
        uint temp1 =  updataUserReward_Box(pid_, slot_, user_);
        uintList[6] = temp1;
    }

    function checkUserNftTime(uint pid_, address user_) public view returns (uint usableTimes, uint usedTimes, uint allTimes){
        User_NumberOfDraws storage number = numberOfDraws[pid_][user_];
        allTimes = number.claimedSweepstakes;
        usedTimes = number.usedSweepstakes;
        usableTimes = allTimes - usedTimes;
    }

    function checkUserSlot_Box(uint pid_, address addr_) public view returns(uint[5] memory list) {
        list = numberOfDraws[pid_][addr_].depositSlot;
    }

    function bonusCoe() public view returns(uint[4] memory) {
        return _bonusCoe;
    }
}
