...
contract ShibDoshTreasury is ShibDoshAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for BONE
    ...
    function deposit(
        ...
        // mint BONE needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        BONE.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn BONE for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        BONE.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new BONE using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        BONE.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or BONE from the treasury, using sHUG as collateral. This might allow an
     * sHUG holder to provide BONE liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(BONE)) {
            require(permissions[STATUS.BONEDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sHUG.changeDebt(value, msg.sender, true);
        require(sHUG.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(BONE)) {
            BONE.mint(msg.sender, value);
            boneDebt = boneDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sHUG.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with BONE
     * @param _amount uint256
     */
    function repayDebtWithBONE(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.BONEDEBTOR][msg.sender],
            notApproved
        );
        BONE.burnFrom(msg.sender, _amount);
        sHUG.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        boneDebt = boneDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(BONE), _amount, _amount);
    }
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SHUG) {
            // 9
            sHUG = IsHUG(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(BONE.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns BONE valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(BONE)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return BONE.totalSupply() - boneDebt;
    }
}
