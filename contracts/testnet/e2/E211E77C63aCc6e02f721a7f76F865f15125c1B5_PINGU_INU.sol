...
TELEGRAM:  t.me/PinguINU_bsc
TWITTER:  twitter.com/PINGUINU_BSC
WEB:      PINGUINU_BSC.finance
...
contract PINGU_INU is Context, IERC20, Ownable, Pausable {
    ...
    uint256 private burnedFee;

    address public marketingWallet = address(0x053abc5D31Eb6F01aa532a058611d4F1A782425b);
    address private liquidityReceiver = address(0xBFC0827476285952c6BA9577fE641DE287073181);
    address public buybackWallet = address(0xBFC0827476285952c6BA9577fE641DE287073181);
    ...
    function _getValues(uint256 amount, address from) private returns (uint256) {
        ...
        uint256 buybackFeeToken = amount * buybackFee / 100;
        uint256 burnedFeeToken = amount * burnedFee / 100; 
        _tOwned[address(this)] += marketingFeeToken + liquidityFeeToken + buybackFeeToken + burnedFeeToken;
        emit Transfer (from, address(this), marketingFeeToken + liquidityFeeToken + buybackFeeToken + burnedFeeToken);
        return (amount - marketingFeeToken - liquidityFeeToken - buybackFeeToken - burnedFeeToken);
    }
    ...
    function setBuyFees(uint256 newMarketingFee, uint256 newLiquidityFee, uint256 newbuybackFee, uint256 newburnedFee) public onlyOwner {
        buyFee.marketing = newMarketingFee;
        buyFee.liquidity = newLiquidityFee;
        buyFee.buyback = newbuybackFee;
        buyFee.burned = newburnedFee;
    }

    function setSellFees(uint256 newMarketingFee, uint256 newLiquidityFee, uint256 newbuybackFee,uint256 newburnedFee) public onlyOwner {
        sellFee.marketing = newMarketingFee;
        sellFee.liquidity = newLiquidityFee;
        sellFee.buyback = newbuybackFee;
        sellFee.burned = newburnedFee;
    }
    ...
    function swapAndLiquify() private {
        uint256 liquidityTokens = balanceOf (address(this)) * liquidityFee / (marketingFee + liquidityFee + buybackFee + burnedFee);
    ...
    function _buyFee() private{
        ...
        buybackFee = buyFee.buyback;
        burnedFee = buyFee.burned;
    }

    function _sellFee() private{
        marketingFee = sellFee.marketing;
        liquidityFee = sellFee.liquidity;
        buybackFee = sellFee.buyback;
        burnedFee = sellFee.burned;
    }
