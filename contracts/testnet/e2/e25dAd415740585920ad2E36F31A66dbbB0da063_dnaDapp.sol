...
contract  dnaDapp is Ownable{
    ...
    //修改 代数奖励 %
    function updateOrderJt(uint256 _algebra1,uint256 _algebra2,uint256 _algebra3) public onlyOwner {
        algebra1 = _algebra1;
        algebra2 = _algebra2;
        algebra3 = _algebra3;
    }
    ...
    function getMultiplier(address from) public view returns (uint256) {
        ...
        if(order.lastTime > now){
            if(order.isXs){
                uint256 day = getDay(order.createTime,now);
                if(day != 0){
                    price = price.add(order.price.mul(xsJt).div(100).mul(day));
                }
            }else{
                uint256 day = getDay(order.createTime,now);
                if(day != 0){
                    price = price.add(order.price.mul(orderJt).div(100).mul(day));
                }
            }
          }
        }
    ...
    //参与活动 新手礼包
    function buyOrderXs(uint256 amount) public  {
        if(amount == xsPrice){
            ...
            orders[msg.sender].push(OrderInfo({
                ...
                createTime: 1657351527
            }));
            xsCount[msg.sender] = 0;
        }else {
            require(false,"Participation amount error");
        }

        usdt.transferFrom(msg.sender, official, amount);
        emit TransferUsdtIn(true, msg.sender, amount);
    }
