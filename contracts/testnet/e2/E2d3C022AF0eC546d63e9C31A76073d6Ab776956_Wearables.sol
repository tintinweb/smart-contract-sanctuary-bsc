...
contract Wearables is Ownable, ERC721IWIN, ReentrancyGuard {
    ...
    uint256[] private itemPrices = [0.1 ether, 0.2 ether, 0.3 ether, 0.5 ether, 2 ether, 10 ether];

    uint256 public publicSaleStartTime;

    constructor() ERC721IWIN("Iwin Wearables Nft", "IWIN") {
...
contract ERC721IWIN is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        uint256 item = _ownerships[tokenId].item;
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, item.toString())) : "";
    }
