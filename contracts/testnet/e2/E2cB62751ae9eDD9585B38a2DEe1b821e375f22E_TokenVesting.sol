...
contract TokenVesting is Ownable {
    ...
    // Declare the contract as an ERC20 token
    using SafeERC20 for IERC20;

    // Declare the ERC20 token interface
    IERC20 public token;

    // Declare a constructor function to set the initial owner, token contract, and release date
    ...
    function releaseTokens(address vestingScheduleId, address _to, uint256 relase_amount) public onlyOwner{
       ...
       VestingSchedule storage vestingSchedule = vestingSchedules[vestingScheduleId];
        ...
        require(vestingSchedule.amountTotal > relase_amount + total_amount, "Exceeds the monthly limit.");

        // address payable beneficiaryPayable = payable(_to);

        total_amount += relase_amount;
        // Transfer the tokens to the specified address
        token.safeTransfer(_to, relase_amount);
    }
