...
contract TeazeLotto is Ownable, Authorized, ReentrancyGuard {
    ...
    uint public spinFee = 0.001 ether; //Fee the contract takes for each attempt at spinning the SimpWheel
    uint public overrideFee = 5;  //override fee in whole USD
    ...
    bool public adminWinner = false; //to test winning jackpot roll
    ...
    function SpinSimpWheel(bool _override) external payable nonReentrant {
       ...
       // require(staked, "User must be staked to spin the SimpWheel of Fortune");
        ...
        if(!_override) {
            ...
            require(msg.value >= (oracle.getbnbusdequivalent(overrideFee)).mul(99).div(100), "Please include the spin fee to spin the SimpWheel");
        }
