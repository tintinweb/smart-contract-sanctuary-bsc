...
contract Crowdfunding is AccessControlEnumerable {
    ...
    uint16 public /*constant*/ MIN_FUND_VIPS = 1; //最小等级限制
    ...
    function fund(uint256 _regionIndex, uint256 _amount) external {
        ...
        require(accounts[msg.sender].vips >= MIN_FUND_VIPS, "account level limited");
    ...
    function finishProject(uint256 _regionIndex) external {
        uint16 _round = regions[_regionIndex].currentRound;
        Project storage p = projects[_regionIndex][_round];
    ...
    function processBrokerReward(address account, uint256 brokerReward) internal {
        (address[] memory _addresses, uint16[] memory _equities) = brokerEquitys(account);
        uint256 used = 0;
        //brokerReward已经乘了经纪人奖比例rates[1] 这里须除以rates[1]再按极差算
        uint256 fundAmount = (brokerReward * 1000) / rates[1];
        for (uint256 i = 0; i < 4; i++) {
            if (_equities[i] > 0) {
                uint256 _amount = (fundAmount * _equities[i]) / 1000;
                IERC20(lp).transfer(_addresses[i], (_amount * (1000 - rates[6])) / 1000);
                accounts[_addresses[i]].lockedWithdrawAmount += (_amount * rates[6]) / 1000;
                used += _amount;
            }
        }
        if (brokerReward > used) {
            IERC20(lp).transfer(fee, brokerReward - used);
        }
    }
    ...
    function setTestParams(uint16 minFundVips, uint256 maxFundAmount, uint16 oneHour) public {
        MIN_FUND_VIPS = minFundVips;
