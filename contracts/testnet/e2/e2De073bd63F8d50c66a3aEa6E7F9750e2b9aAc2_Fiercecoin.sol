...
/*
FFFFFFFFFFFFFF     CCCCCCCCCCCCCCCCC
FFFFFFFFFFFFFF     CCCCCCCCCCCCCCCCC
FFFF               CCCCC        CCCC
FFFF               CCCCC
FFFFFFFFFFFFFF     CCCCC                RRRRRRRR       VVV          VVV
FFFFFFFFFFFFFF     CCCCC                RRRRRRRRR       VVV        VVV 
FFFFF              CCCCC                RRR   RR         VVV      VVV 
FFFFF              CCCCC        CCCC    RRRRRRRRR         VVV    VVV
FFFFF              CCCCCCCCCCCCCCCCC    RRR   RRRR         VVV  VVV
FFFFF              CCCCCCCCCCCCCCCCC    RRR    RRRR REAL    VVVVVV  VERSION

 The official token of FierceCoin. Real version

To Know more about the token from the website:-

 https://www.fiercecoin.net
...
contract Fiercecoin is Context, IERC20, Ownable {
    ...
    //_isBlacklisted = Block scammers or bots. Can not buy or sell or transfer tokens at all.
    ...
    constructor () {
        ...
        _maxWalletToken = 150000000 * 10**18;
        _maxTxAmount = 15000000 * 10**18;
        _previousMaxTxAmount = _maxTxAmount;
        minimumTokensBeforeSwap = 1800000 * 10**18;
        buyBackUpperLimit = 1 * 10**18;
        _rOwned[_msgSender()] = _rTotal;
        marketingAddress = payable(0xfA1aAA55d3D3964F6d4724859083c61353eF3b52);
    ...
    function _takeDevelopment(uint256 tDevelopment) private {
        ...
        _rOwned[marketingAddress] = _rOwned[marketingAddress].add(rDevelopment);
        if(_isExcluded[marketingAddress])
            _tOwned[marketingAddress] = _tOwned[marketingAddress].add(tDevelopment);
    }
 ...
 //adding multiple addresses to the blacklist - Used to manually block known bots and scammers
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 tDevelopment = calculateMarketingFee(tAmount);
  ...
  //blacklisted addreses can not buy! for scammers and bots
        ...
        require((recipientBalance + amount) <= (_maxWalletToken), "Sorry, at the moment nobody can own more than 0.5% of the total supply.");
