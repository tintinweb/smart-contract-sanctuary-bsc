...
contract GymSinglePool is ReentrancyGuardUpgradeable, OwnableUpgradeable {
   ...
   /**
    ...
    //address of contract's reward token
    ...
    modifier onlyOnGymMLM() {
        require(IGymMLM(relationship).isOnGymMLM(msg.sender), "SinglePool: Don't have relationship");
        _;
    }
    ...
    function initialize(
        ...
        levels = [0, 0, 200, 500, 2000, 4000, 10000, 20000, 40000, 45000, 50000, 60000, 65000, 70000, 75000, 80000];
    ...
    function deposit(
        ...
        require(_depositAmount > 0, "Value is not specified");

        _deposit(_depositAmount,_periodId);
    }

/**

Should approve allowance before initing
 */
function _deposit(
        uint256 _depositAmount,
        uint8 _periodId
        ...
        user.totalDepositDollarValue += ((_depositAmount * getPrice())/1e18) / 1e18;
        user.depositId += 1;
        ...
        UserDeposits memory depositDetails = UserDeposits(
            ...
            depositDollarValue: ((_depositAmount * getPrice())/1e18) / 1e18,
            stakePeriod: months[_periodId],
            depositTimestamp: block.timestamp,
            withdrawalTimestamp: DateTime.addMonths(block.timestamp,months[_periodId])
            }
            );
