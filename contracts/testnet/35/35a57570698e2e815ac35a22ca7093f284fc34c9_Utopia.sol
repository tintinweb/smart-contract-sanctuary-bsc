...
contract Utopia is  Ownable {
    ...
    address public dynamicAllocation;
    uint256 public dynamicAllocationRatio;

    constructor(address utopiaToken_, address utopiaNFT_, address utopiaInsurance_, address utopiaRalationship_, address operatingFund_, address dynamicAllocation_) {
        ...
        utopia1DataNum = 1;
        setDynamicAllocation(dynamicAllocation_);
        setDynamicAllocationRatio(1,13,8,4,2,2,50);
    }

    function setDynamicAllocation(address dynamicAllocation_) public onlyOwner {
        dynamicAllocation = dynamicAllocation_;
    }

    function setDynamicAllocationRatio(uint8 v0, uint8 v1, uint8 v2, uint8 v3, uint8 v4, uint8 v5, uint256 ratio_) public onlyOwner {
        rewardLadder = [v0,v1,v2,v3,v4,v5];
    ...
    function addTotalBonusFromContract(uint256 bonus, bool addOrSub) public onlyDerivation {
        if(addOrSub){
            totalBonus += bonus;
        }else{
            require(totalBonus >= bonus,"[Utopia] Insufficient amount");
            totalBonus -= bonus;
        }
    }
    ...
    function dataTransfer() public onlyOwner {
        ...
        address recommender_address;
        address current_address;
        uint8 remain_vip;
        uint8 current_vip;
        uint256 i;
        uint256 jNum = utopia1DataNum;
        for(uint256 j = jNum; j < jNum+100; j++) {
            if(j > totalNftCount){
                break;
            }
            utopia1DataNum++;
            account = utopiaNFT.ownerOf(j);
            recommender_address = getRecommender(account);
    ...
    function join() public {
        ...
        amount = (join_price*dynamicAllocationRatio)/100;
        usdt.transfer(dynamicAllocation,amount);
        totalBonus += join_price;

        invitePermissionNum[msg.sender] += 3;
        invitePermissionNum[recommender_address] += 3;
    ...
    function updateReferralGrade(address account, bool addOrSub, uint8 _vip, uint256 num) public onlyOwner {
        if(addOrSub){
            referral_grade[account][_vip] += num;
        }else{
            referral_grade[account][_vip] -= num;
        }
    }
    ...
    function statisticsReward() public onlyDerivation{
        ...
        if(totalNftCount > 0){
            ...
            bonus = totalBonus*(20+dynamicAllocationRatio)/100;
            rAmount -= bonus;
            if(rAmount > 0){
