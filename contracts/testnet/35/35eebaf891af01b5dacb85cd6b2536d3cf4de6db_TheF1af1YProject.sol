...
contract TheF1af1YProject is Context, Ownable {
    ...
    uint256 private SHARES_TO_HATCH_1MINERS = 720000;//for final version should be seconds in a day
    ...
    function investShares(address ref) public {
        require(initialized);
        ...
        uint256 newInvestor = SafeMath.div(sharesUsed,SHARES_TO_HATCH_1MINERS);
    ...
    function getSharesSinceLastInvest(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(SHARES_TO_HATCH_1MINERS,SafeMath.sub(block.timestamp,lastInvest[adr]));
