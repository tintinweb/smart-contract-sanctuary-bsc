...
contract Name is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    address payable public projectWalletAddress = payable(0x761E70C4317dD1a16592aB5c44D95aF03F4abFca);
    ...
    mapping (address => uint) public sellOptionPerAddress;
    mapping (address => uint256) public lastSellTimeAddress;
    ...
    function _transfer(
        ...
        {              
            ...
            if((!_isExcludedFromFees[from] && !_isExcludedFromFees[to])) {
                ...
                if (to == uniswapV2Pair && block.timestamp < launchTime + 72 hours){
                    if (sellOptionPerAddress[from] == 0){
                        projectFee_tmp = 20;
                        lastSellTimeAddress[from] = block.timestamp;
                        sellOptionPerAddress[from] ++;
                    }
                    else if (sellOptionPerAddress[from] == 1){
                        projectFee_tmp = 22;
                        require(block.timestamp > lastSellTimeAddress[from] + 1 hours, "Sell option 1 needs 1 hour cooldown!");
                        lastSellTimeAddress[from] = block.timestamp;
                        sellOptionPerAddress[from] ++;
                    }
                    else if (sellOptionPerAddress[from] == 2){
                        projectFee_tmp = 25;
                        require(block.timestamp > lastSellTimeAddress[from] + 2 hours, "Sell option 2 needs 2 hours cooldown!");
                        lastSellTimeAddress[from] = block.timestamp;
                        sellOptionPerAddress[from] ++;
                    }
                    else if (sellOptionPerAddress[from] == 3){
                        projectFee_tmp = 26;
                        require(block.timestamp > lastSellTimeAddress[from] + 6 hours, "Sell option 3 needs 6 hours cooldown!");
                        lastSellTimeAddress[from] = block.timestamp;
                        sellOptionPerAddress[from] ++;
                    }   
                    else if (sellOptionPerAddress[from] == 4){                        
                        require(block.timestamp > lastSellTimeAddress[from] + 24 hours, "Sell option 4 needs 24 hours cooldown!");
                        lastSellTimeAddress[from] = block.timestamp;
                        sellOptionPerAddress[from] ++;
                    }
                }
                if (from != uniswapV2Pair && to != uniswapV2Pair && block.timestamp < launchTime + 72 hours){
    ...
    function swapAndDistribute() private {
        if (address(this).balance > 0){
            address(projectWalletAddress).call{value: address(this).balance}("");            
        }
        ...
        address(projectWalletAddress).call{value: ethBalance}("");   
    }    
    ...
    function _blacklistBot(address account) external onlyOwner() {
        require(account != 0xD99D1c33F9fC3444f8101754aBC46c52416550D1, 'We cannot blacklist Uniswap');
