...
library Address{
 ...
 contract GEN is Context, IERC20, Ownable { //97 : 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 || 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    address public project = 0x324D9eA3F7403C2e07ee833080834E3Bfc8958c2;
    ...
    Taxes public buytaxes = Taxes(3, 3, 3, 2);
    Taxes public sellTaxes = Taxes(3, 3, 3, 2);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[project] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[marketingWallet] = true;
        allowedTransfer[project] = true;
    ...
    function _takeProject(uint256 rProfit, uint256 tproject) private {
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rprofit, to_return.rMarketing, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
        (to_return.rProfit) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tRfi = tAmount*temp.profit/100;
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rProfit = s.tproject*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rProfit;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rProfit) {

        if(!takeFee) {
          return(0);
        }

        rProfit = s.tproject*currentRate;
        return (rProfit);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rprofit > 0 || s.tRfi > 0) _reflectRfi(s.rprofit, s.tRfi);
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            _takeLiquidity(s.rLiquidity,s.tLiquidity);
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tproject);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rProfit > 0 || s.tproject > 0) _takeProject(s.rProfit, s.tproject);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(projectAmt > 0){
            payable(project).sendValue(projectAmt);
        }
    }
