...
//            rew = (rew + user.toClaim) - countingToken(quota - user.totalPower, price);
    ...
    function _calculateReward(address addr) internal view returns(uint){
        ...
        if(quota >= user.totalPower){
            rew = rew - countingToken(quota - user.totalPower, price);
        }
        return rew;
    }
    ...
    function _processRefer(address addr, uint price,uint debt_) internal {
        uint amount = countingToken(70 ether, price);
        uint left = amount;
        uint tempRew = amount / 10;
        address temp = userInfo[addr].invitor;
        uint tempQuota = 7 ether;
        ...
        for (uint i = 0; i < 10; i++) {
            ...
            uint rew = _calculateReward(temp);
            uint quota = countingQuota(rew, sbtcPrice);
            if (tempQuota + quota + userInfo[temp].toClaimQuota >= userInfo[temp].totalPower) {
                uint power = userInfo[temp].totalPower;
                uint finalRew = countingToken(userInfo[temp].totalPower - userInfo[temp].toClaimQuota,sbtcPrice - tempQuota) + userInfo[temp].toClaim;
                ...
                userInfo[temp].referReward += tempQuota;
                sbtc.transfer(temp, finalRew);
                ...
                userInfo[temp].referReward += tempQuota;
                userInfo[temp].claimed += countingToken(tempQuota,sbtcPrice);
                totalOut += tempQuota;
                sgy.transfer(temp,tempRew);
            }
            temp = userInfo[temp].invitor;
    ...
    function reBuySbtc(uint amount) internal {
    ...
    function stake(uint amount, address invitor) external {
        ...
        _processReferAmount(msg.sender,userInfo[msg.sender].referCost);
        userInfo[msg.sender].referCost = 0;
    ...
    function claimReward() external {
        ...
        if (quota >= user.totalPower) {
            ...
            user.totalPower = 0;
            user.debt = _debt;
            sbtc.transfer(msg.sender, finalsRew);
            user.toClaimQuota = 0;
            user.toClaim = 0;
