...
contract PredictionReferral is IReferralC, Ownable {
...
contract CPrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    function provideLiquidity(uint256 epoch, uint256 bullAmount, uint256 bearAmount) external whenNotPaused nonReentrant onlyOwner {
        require(epoch == currentEpoch, "Bet earlylate");
        require(_bettable(epoch), "not bettable");
        uint256 totalAmount = bullAmount + bearAmount;
        if(address(betToken) == address(0)){
            require(address(this).balance >= totalAmount, "balance<bet");
        }
        else{
            require(betToken.balanceOf(address(this)) >= totalAmount, "balance<bet");
        }
        ...
        if (bullAmount > 0)
        {
            ...
            emit PredictionsBet(address(this), epoch, bullAmount, 0, 0, uint8(Position.Bull));
        }

        if (bearAmount > 0)
        {
            round.bearAmount += bearAmount;
            emit PredictionsBet(address(this), epoch, bullAmount, 0, 0, uint8(Position.Bear));
        }

        ledger[currentEpoch][address(this)].amount = totalAmount;
    ...
    function claim(address user, uint256[] calldata epochs) external nonReentrant onlyOwner {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            if (rounds[epochs[i]].completed && !rounds[epochs[i]].cancelled) {
    ...
    function injectDonation() external payable {}
    ...
    function claimable(uint256 epoch, address user) public view returns (bool) {
        ...
        return
            round.completed && !round.cancelled && betInfo.amount > 0 && !betInfo.claimed &&
    ...
    function refundable(uint256 epoch, address user) public view returns (bool) {
        BetInfo memory betInfo = ledger[epoch][user];
        Round memory round = rounds[epoch];
        return (!round.completed || round.cancelled) && block.timestamp > (timestamps[epoch].closeTimestamp + intervalSeconds) && betInfo.amount != 0 && !betInfo.claimed;
    }
