    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant MINIMUM_SIZE = 0.01 ether;
    ...
    function initialize(
        ...
        __EIP712_init("Bet2Win", "1");
        __Base_init(authority_, Roles.TREASURER_ROLE);
    }

    function withdraw() external onlyRole(Roles.OPERATOR_ROLE) {
    ...
    function placeBet(
        ...
        ///@dev get rid of stack too deep
        {
            ...
            if (address(paymentToken_) != address(0)) {
                ...
                if (msg.value != 0) {
                    (bool ok, ) = user.call{value: msg.value}("REFUND");
                    require(ok, "BET2WIN: REFUND_FAILED");
                }
            }
            _safeTransferFrom(paymentToken_, user, address(_treasury), amount_);
            if (msg.value > amount_) {
                unchecked {
                    (bool ok, ) = user.call{value: msg.value - amount_}(
                        "REFUND"
                    );
                    require(ok, "BET2WIN: REFUND_FAILED");
                }
