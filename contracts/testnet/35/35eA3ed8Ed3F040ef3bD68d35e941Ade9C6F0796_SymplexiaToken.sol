...
//   ====================================== 
    ...
    function _cleanseBonusList () external  onlyRole(Contract_Manager) {
        for (uint256 entry=0; entry < Inventory.noBonusList.length; entry++) {
            ...
            for (uint256 i=entry+1; i < Inventory.noBonusList.length; i++) {
               ...
               if (Inventory.noBonusList[i] == Inventory.noBonusList[entry]) {
                  Inventory.includeInBonus(_msgSender(), Inventory.noBonusList[entry]);
               }
            }
        }
    }

    function _bonusBalances () internal view returns (uint256) {
        uint256 expurgedBalance;
        for (uint256 i=0; i < Inventory.noBonusList.length; i++){
            expurgedBalance += Inventory.Basis[Inventory.noBonusList[i]].balance;
        }
        return  expurgedBalance;                 
    }

    function BonusInfo () external view returns (uint256 length, uint256 noBonus, uint256 totalSupply) {
        length = Inventory.noBonusList.length;
        noBonus = _bonusBalances();
        totalSupply = Inventory.tokensSupply;
    }
    function BonusAddresses () external view returns (address[] memory noBonusList) {
