...
contract SSDOT is Ownable{
    ...
    uint256 public algebra = 8;

    constructor() public  {
        usdt = IERC20(0xe38a71627E3289D8154da871F9ecF25Cd41EB483);
        sdt = IERC20(0x86D0898Fd83FE6fe090E9405b2a63a85246d590c);
    ...
    event BuyOrder(address sender, uint amount, string uuid);
    event UpPrice(address sender, address upAddress, uint amount);

    function updateOfficial(address newOfficial) public onlyOwner {
        official = newOfficial;
    }

    function updateAlgebra(uint256 _algebra) public onlyOwner {
        algebra = _algebra;
    }

    function withdrawUsdt(address toAddr, uint256 amount) onlyOwner public  {
      sdt.transfer(toAddr, amount);
    }

    function withdrawUsdtNew(address toAddr, uint256 amount) onlyOwner public  {
    ...
    function buyOrder(string memory uuid,uint256 price) public  {
        usdt.transferFrom(msg.sender, official, price);
        isOrder[msg.sender] = true;
        if(isUpUser[msg.sender]){
            address up = users[msg.sender];
            if(up != address(0) && isOrder[up]){
                usdt.transfer(up, price.mul(algebra).div(100));
                emit UpPrice(msg.sender,up,price.mul(algebra).div(100));
            }
        }

        emit BuyOrder(msg.sender, price , uuid);
    }
