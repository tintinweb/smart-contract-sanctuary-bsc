...
contract SpartaToken is ERC20, Blacklist, Whitelist {
    ...
    event LPAwardEvent(address indexed from, address indexed to, uint amount);

    event NFTAwardEvent(address indexed from, address indexed to, uint amount);
    ...
    constructor() ERC20("Sparta Token", "SPA") {
        ...
        _buyFeeConfig.otherBurnPer = 6000;
    ...
    function _transferToBuy(address from, address to, uint amount) internal {
        ...
        if (lpAwardAmount > 0) {
            ...
            emit LPAwardEvent(from, _serAddress, lpAwardAmount);
        }

        uint ohterAmount = (amount * _buyFeeConfig.otherPer) / 10000;
        if (ohterAmount > 0) {
            uint otherBurnAmount = (ohterAmount * _buyFeeConfig.otherBurnPer) / 1000;
            super._transfer(from, BURN_ADDRESS, otherBurnAmount);

            uint otherDaoAmount = (ohterAmount * _buyFeeConfig.otherDaoPer) / 1000;
            super._transfer(from, _daoAddress, otherDaoAmount);

            uint otherEcoAmount = (ohterAmount * _buyFeeConfig.otherEcoPer) / 1000;
            super._transfer(from, _ecoAddress, otherEcoAmount);

            uint otherNFTAwardAmount = (ohterAmount * _buyFeeConfig.otherNFTAwardPer) / 1000;
            super._transfer(from, _serAddress, otherNFTAwardAmount);
            emit LPAwardEvent(from, _serAddress, otherNFTAwardAmount);
        }
    ...
    function _transferToSell(address from, address to, uint amount) internal {
        ...
        uint swapAmount = (amount * _sellFeeConfig.swapPer) / 10000;
        if (swapAmount > 0) {
        ...
        if (otherAmount > 0) {
            uint otherBurnAmount = (otherAmount * _sellFeeConfig.otherBurnPer) / 1000;
            super._transfer(from, BURN_ADDRESS, otherBurnAmount);

            uint otherDaoAmount = (otherAmount * _sellFeeConfig.otherDaoPer) / 1000;
            super._transfer(from, _daoAddress, otherDaoAmount);

            uint otherEcoAmount = (otherAmount * _sellFeeConfig.otherEcoPer) / 1000;
            super._transfer(from, _ecoAddress, otherEcoAmount);
        }

        amount = amount - daoAmount - ecoAmount - swapAmount - otherAmount;
        require(amount > 0, "SPA: Invalid amount");
    ...
    function addLiquidity(uint256 amountIn) public {
        ...
        uint burnAmount = (amountIn * 300) / 10000;
    ...
    function swapAndLiquify(uint256 amount) public {
        ...
        uint256 initialBalance = IERC20(USDT).balanceOf(_swapAddress);

        _swapTokensForUSDT(half, _swapAddress);

        uint256 newBalance = IERC20(USDT).balanceOf(_swapAddress) - initialBalance;

        _addLiquidity(otherHalf, newBalance);
    }

    function _swapTokensForUSDT(uint256 tokenAmount, address to) internal {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = USDT;
        ...
        IUniswapV2Router02(uniswapV2Router).swapExactTokensForTokensSupportingFeeOnTransferTokens(tokenAmount, 0, path, to, block.timestamp);
    }

    function _addLiquidity(uint256 tokenAmount, uint256 usdtAmount) internal {
        IERC20(USDT).transferFrom(_swapAddress, address(this), usdtAmount);
    ...
    function setAddress(address dao, address eco, address ser, address swap) public {
    ...
    function setConfig(uint minHold, uint transferFeeToBurnPer) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "SPA: Must have admin role to set config");
