...
contract Ownable is Context {
    ...
    address constant private multiSigWallet  = address(0x6DCb1FC5300a03bdB741c4F11D525E0943a01C13);
    constructor () {
...
contract TestCoin is Context, IERC20, Ownable {
    ...
    uint256 public constant buyTaxMax = 15;
    uint256 public sellTax = 9;
    uint256 public constant sellTaxMax = 15;
    uint256 public tax = 0;
    uint256 public constant taxTaxMax = 15;
    ...
    constructor (address payable _add1, address payable _add2) { 
        ...
        // Fix the approval of the contract and the owner
        // _approve(address(this), owner(), _tTotal);
        // emit Approval(address(this), owner(), type(uint256).max);
    ...
    function changeBuyTax(uint256 _newBuyTax) external onlyOwner{
        require(_newBuyTax <= buyTaxMax,"New Buy tax have to be under 15");
        buyTax = _newBuyTax;
        emit buyTaxUpdated(_newBuyTax);
    }

    // event sellTaxUpdated(uint256 _newTaxAmount);   
    // function changeSellTax(uint256 _newSellTax) external onlyOwner{
    //     require(_newSellTax <= sellTaxMax,"New sell tax have to be under 15");
    //     sellTax = _newSellTax;
    //     emit sellTaxUpdated(_newSellTax);
    // }

    // event taxTaxUpdated(uint256 _newTaxAmount);   
    // function changeTaxTax(uint256 _newTaxTax) external onlyOwner{
    //     require(_newTaxTax <= taxTaxMax,"New transfer tax have to be under 15");
    //     tax = _newTaxTax;
    //     emit sellTaxUpdated(_newTaxTax);
    // }
