...
contract NodeType is INodeType, Owners, NodeRewards {
    ...
    uint256 public maxLevelUpUser;
    uint256 public maxLevelUpTotal;
    uint256 public maxCreationPendingUser;
    uint256 public maxCreationPendingTotal;
    ...
    constructor(
        ...
        require(values.length == 11, "NodeType: Values.length mismatch");
        maxCount = values[0];
        price = values[1];
        claimTime = values[2];
    ...
    function setPlotAdditionalLifetime(
        ...
        node.plotAdditionalLifetime = amountOfGRP.times(_timeToGRP());
    }
    ...
    function addPlotAdditionalLifetime(
        ...
        require(u.inserted[tokenId], "NodeType: User doesnt own this node");
        Node storage node = u.values[tokenId];
        node.plotAdditionalLifetime += amountOfGRP.times(_timeToGRP()) * amount;
    }
    ...
    function setMax(uint256 _maxUser, uint256 _maxCount) external onlyOwners {
        maxUser = _maxUser;
        maxCount = _maxCount;
    }

    function setMaxLevelUp(uint256 _maxLevelUpUser, uint256 _maxLevelUpTotal)
        external
        onlyOwners
    {
    ...
    function setMaxCreationPending(
        uint256 _maxCreationPendingUser,
        uint256 _maxCreationPendingTotal
    ) external onlyOwners {
    ...
    function getTimeGRPOfBetweenIndexes(
        ...
        uint256[] memory grps = new uint256[](iEnd - iStart);
        User storage u = userOf[user];
        for (uint256 i = iStart; i < iEnd; i++)
            grps[i - iStart] =
                (price * claimTime) /
                rewardAmount +
                u.values[u.keys[i]].creationTime;
        return grps;
    }
        ...
        returns (uint256, uint256)
    {
        ...
        if (baseRewards < price && claimTaxGRP > 0) {
            fees += (rewardsTotal * claimTaxGRP) / 10000;
        }
...
abstract contract NodeRewards {
    ...
    function _extendLifetime(
        ...
        node.lastLifetime += ratioOfGRPExtended.times(_timeToGRP()) * amount;
    }
        ...
        returns (uint256)
    {
        ...
        // console.log("NodeRewards: Now: ", block.timestamp);
        // console.log("NodeRewards: Base rewards: ", baseRewards);
        // console.log("NodeRewards: Accumulated: ", node.accumulatedRewards);
        // console.log("NodeRewards: Last known lifetime: ", node.lastLifetime + node.plotAdditionalLifetime);
        // console.log("NodeRewards: Time since last reward update: ", Math.subOrZero(block.timestamp, node.lastRewardUpdateTime));
...
library GRPDependantRewardProfile {
    ...
    function integrateFertilizerAdditionalRewards(
        uint256 baseRewardsPerSecond,
        UPercentage percentageOfBaseRewards,
        ...
        returns (uint256)
    {
        return percentageOfBaseRewards.times(baseRewardsPerSecond) *
