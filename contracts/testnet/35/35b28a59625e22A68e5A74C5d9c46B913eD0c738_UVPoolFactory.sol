...
contract UVPoolFactory is Ownable, AccessControl {
    ...
    mapping(uint8 => address) public getPoolReserved;
    address[] public allPoolsReserved;
    mapping(uint8 => address) public getPoolStake;
    address[] public allPoolsStake;
    uint8 orderNumber = 0;
    address public factoryReverse;
    ...
    function createPool(
        ...
        //     abi.encodePacked(orderNumber, pool, _fundWallet)
        // );
        // address poolStake = address(
        //     new UVStake{salt: saltStake}(orderNumber, pool, _fundWallet)
        // );
        ...
        allPools.push(pool);
        // getPoolReserved[orderNumber] = poolReserve;
        // allPoolsReserved.push(poolReserve);
    ...
    //         "createReserve(address,address,uint8)",
    //         _fundWallet,
    //         _pool,
    //         _orderNumber
    //     );
    //     (bool success, bytes memory returnData) = factoryReverse.delegatecall(payload);
    //     require(success);
    //     (address poolReserve, address poolStake) = abi.decode(
    //         returnData,
    //         (address, address)
    //     );
    //     getPoolReserved[orderNumber] = poolReserve;
    ...
    // Set factory reverse address
    function setFactoryReverse(address _factoryReverse) public onlyRole(ADMIN_ROLE) {
        factoryReverse = _factoryReverse;
    }
        ...
        onlyRole(ADMIN_ROLE)
    {
        ...
        (bool success, ) = getPool[_orderNumber].call(
...
contract UVPool is ERC20, ERC20Burnable, Ownable, AccessControl {
    ...
    uint256 feeCreator = 1 ether;
    uint64 public voteCount = 0;
    mapping(uint8 => Vote) public allVotes;
    mapping(uint8 => VoteInfo[]) public allVoters;
    ...
    function transferToVesting(address _tokenAddress, uint256 _amount)
        external
    ...
    function deposit(uint256 amount) public {
        ...
        require(maxSizePool >= currentSizePool, "Pool is full");
    ...
    function setPercentFee(uint8 _percentFee) public onlyRole(MANAGER_ROLE) {
        require(_percentFee > 10 && _percentFee < 300, "Percent fee is invalid");
        percentFee = _percentFee;
    }

    function getEstimatedBNBforToken(uint256 _amount, address[] memory _path)
        public
        view
        returns (uint256[] memory)
    {
        return pancakeRouter.getAmountsIn(_amount, _path);
    }

    // Get pool reserved
    function getReserved() public returns (address _poolReserved) {
        bytes memory payload = abi.encodeWithSignature(
            "getPoolReserved(uint8)",
            orderNumber
        );
        (bool success, bytes memory returnData) = factoryReverse.call(payload);
        require(success);
        _poolReserved = abi.decode(returnData, (address));
    }
    ...
    function buyNativeToken(
        ...
        address _poolReserved = getReserved();
    ...
    function buyToken(
        ...
        emit BuyToken(_poolReserved, _amountOut);
    }
    ...
    function setMinimumDeposit(uint256 _minimumDeposit) public onlyRole(MANAGER_ROLE) {
        minimumDeposit = _minimumDeposit;
    }

    // Set Max size pool
    function setMaxSizePool(uint256 _maxSizePool) public onlyRole(MANAGER_ROLE) {
    ...
    function getVoteInfo(uint8 _orderNumber)
        public
        view
        returns (VoteInfo[] memory)
    {
        return allVoters[_orderNumber];
    }

    // get length of VoteInfo
    function getVoteInfoLength(uint8 _orderNumber)
        public
        view
        returns (uint256)
    {
        return allVoters[_orderNumber].length;
    }
    ...
    function createVote(
        ...
        // require(isClose, "This Pool is not closed");
        require(msg.value >= feeCreator, "You need to pay fee to create vote");
    ...
    function voting(uint8 _orderNumber, uint8 _optionId) public {
        ...
        require(
            allVoters[_orderNumber][_optionId].timestamp == 0,
            "You have voted"
        );
        require(
            allVoters[_orderNumber][_optionId].amount == 0,
            "You have voted"
        );
        uint256 _amountBalance = address(msg.sender).balance;

        transferFrom(msg.sender, address(this), _amountBalance);
        voteCount += 1;

        allVoters[_orderNumber].push(
            VoteInfo({
                voter: msg.sender,
    ...
    function _releaseToken(uint8 _orderNumber) internal {
        // require(isClose, "This Pool is not closed");
        require(allVotes[_orderNumber].isActive, "This Vote is closed");
        uint256 iBatch = 100;
        for (uint8 i = 0; i < allVoters[_orderNumber].length; i++) {
            if (allVoters[_orderNumber][i].isClaimed == false && iBatch > 0) {
                transfer(
                    allVoters[_orderNumber][i].voter,
                    allVoters[_orderNumber][i].amount
                );
                iBatch -= 1;
                allVoters[_orderNumber][i].isClaimed = true;
            }
        }
    }
}
