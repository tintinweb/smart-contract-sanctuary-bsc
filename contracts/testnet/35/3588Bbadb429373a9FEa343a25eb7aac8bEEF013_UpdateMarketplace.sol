...
contract UpdateMarketplace {
    ...
    constructor(address payable newBeneficiary, uint256 newFee) {
        ...
        payable
    {
        ...
        for (uint256 i = 0; i < nft.length; i++) {
            ...
            emit AcceptAsk({
                ...
                amount: asks[nftAddress][tokenID[i]].amount
            });
            ...
            bool success = nft[i].safeTransferFrom_(
                ...
                tokenID[i],
                new bytes(0),
                asks[nftAddress][tokenID[i]].amount
            );
        ...
        payable(beneficiary).sendValue((escrow[msg.sender] * baseFee) / 100);
        payable(address(msg.sender)).sendValue(
            100 - (escrow[msg.sender] * baseFee) / 100
        );
    }
        ...
        external
    {
        ...
        payable(address(msg.sender)).sendValue(escrow[msg.sender]);
    }
    ...
    /// @dev Used to change the marketplace fee
    function changeBaseFee(uint256 newFee) external {
        require(msg.sender == admin, "");
        baseFee = newFee;
    }
