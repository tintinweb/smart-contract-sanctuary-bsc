...
contract NFT is ERC721, Ownable {
    ...
    event MintedWithBNB(address indexed minter, uint price, uint nftID, string uri);
    event MintedWithToekn(address indexed minter, uint price, uint nftID, string uri);
    ...
    function mintWithBNB(string memory _tokenURI, address _toAddress, uint count) public payable returns (bool) {
        ...
        for(uint i=0;i<length;i++){
            ...
            emit MintedWithBNB(_toAddress, _price, _tokenId, _tokenURI);
        }
        

        return true;
    }
      function mintWithToken(string memory _tokenURI, address _toAddress, uint count , address _paytoken) public  returns (bool) {
        uint _price = ayraPrice[count];
        if(_paytoken == ITHD){
        ...
        for(uint i=0;i<length;i++){
            ...
            emit MintedWithToekn(_toAddress, _price, _tokenId, _tokenURI);
        }
