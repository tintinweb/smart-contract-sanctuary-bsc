...
contract BetWC is Context, IERC20, IERC20Metadata, Ownable {
    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _totalSupply;
    string private _name = "BetWC";
    string private _symbol = "BetWC";

    uint256 public insuranceTax = 25;
    uint256 public burnTax = 25;
    uint256 public totalTax = insuranceTax + burnTax;
    ...
    constructor(address _insuranceReceiver, address _burnReceiver) {
    ...
    function _transfer(
        ...
        if (!excludedAccount && !inSwap && to == pair) {
            uint256 _totalTax = totalTax;
            if (_totalTax > 0) {
                uint256 totalTaxAmount = (amount * _totalTax) / denominator;
                uint256 insuranceTaxAmount = (totalTaxAmount * insuranceTax) /
                    _totalTax;
                uint256 burnTaxAmount = (totalTaxAmount * burnTax) / _totalTax;
                _balances[address(this)] += insuranceTaxAmount;
                emit Transfer(from, address(this), insuranceTaxAmount);
                emit Transfer(from, burnReceiver, burnTaxAmount);
            }
    ...
    function swapBack() internal swapping {
        ...
        payable(insuranceReceiver).call{value: amountEthPayout, gas: 30000}("");
    }
        ...
        returns (uint256 amountEthPayout)
    {
        ...
        amountEthPayout = address(this).balance - balanceBefore;
    }
    ...
    function setInsuranceReceiver(address _insuranceReceiver)
        external
        onlyOwner
    {
        require(_insuranceReceiver != address(0), "0");
        insuranceReceiver = _insuranceReceiver;
    }

    function setTax(uint256 _insuranceTax, uint256 _burnTax)
        external
        onlyOwner
    {
        insuranceTax = _insuranceTax;
        burnTax = _burnTax;
        totalTax = _insuranceTax + _burnTax;
        require(totalTax <= 100, "Max 10%");
    }
