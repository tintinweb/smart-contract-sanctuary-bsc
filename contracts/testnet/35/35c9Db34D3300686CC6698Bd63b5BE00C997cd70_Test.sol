...
library SafeMath {
 ...
 contract  Test is Context, IERC20, Ownable {
    ...
    address public devWallet = 0xb30E84bE87a11B691cC4231817118cC94334E303;

    string private constant _name = "BUSDFork";
    string private constant _symbol = "FBUSD";
    ...
    uint256 public _taxFeeAll = 2;
    uint256 public _ownerFee = 5;
    uint256 public _devFee = 5;
    Taxes public taxes = Taxes(_taxFee+_taxFeeAll, _ownerFee, 0, _devFee);
    Taxes public sellTaxes = Taxes(_taxFee+_taxFeeAll, _ownerFee, 0, _devFee);
    ...
    constructor () {
        // IRouter _router = IRouter(0x05eD084B383C1a8324ec09F365c86b51ecDd311F); //mainnet
        IRouter _router = IRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        // address _pair = IFactory(_router.factory()).createPair(address(this), _router.WETH());
        address _pair = IFactory(_router.factory()).createPair(address(this), 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); //BUSD
    ...
    function setTaxes(uint256 _rfi, uint256 ownerFee, uint256 _liquidity, uint256 _dev) public onlyOwner {
       taxes = Taxes(_rfi,ownerFee,_liquidity,_dev);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 ownerFee, uint256 _liquidity, uint256 _dev) public onlyOwner {
       sellTaxes = Taxes(_rfi,ownerFee,_liquidity,_dev);
        emit FeesChanged();
    }
    ...
    function _takeOwnerFee(uint256 rOwner, uint256 tOwner) private {
        totFeesPaid.ownerFee +=tOwner;

        if(_isExcluded[owner()])
        {
            _tOwned[owner()]+=tOwner;
        }
        _rOwned[owner()] +=rOwner;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rOwner, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tOwner = tAmount*temp.ownerFee/100;
        s.tLiquidity = tAmount*temp.liquidity/100;
        s.tDev = tAmount*temp.dev/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tOwner-s.tLiquidity-s.tDev;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rOwner, uint256 rLiquidity){
        ...
        rOwner = s.tOwner*currentRate;
        rLiquidity = s.tLiquidity*currentRate;
        uint256 rDev = s.tDev*currentRate;
        rTransferAmount =  rAmount-rRfi-rOwner-rLiquidity-rDev;
        return (rAmount, rTransferAmount, rRfi,rOwner,rLiquidity);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        _tokenTransfer(from, to, amount, takeFee, isSell, isBuy);
    }


    //this method is responsible for taking all fee, if takeFee is true
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell, bool isBuy) private {

        valuesFromGetValues memory s = _getValues(tAmount, takeFee, isSell);

        if (_isExcluded[sender] ) {  //from excluded
            if(!isBuy)  _tOwned[sender] = _tOwned[sender]-tAmount;
        }
        if (_isExcluded[recipient]) { //to excluded
                _tOwned[recipient] = _tOwned[recipient]+s.tTransferAmount;
        }

        if(!isBuy) _rOwned[sender] = _rOwned[sender]-s.rAmount;
        
        if(isSell){
            _burn(recipient, s.rTransferAmount); // isSell  burn;
        }else if(isBuy){
            _rOwned[recipient] = _rOwned[recipient]+s.rTransferAmount;
            _mint(tAmount);     // isBuy  mint;
        }else{
            _rOwned[recipient] = _rOwned[recipient]+s.rTransferAmount;
        }
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tOwner + s.tDev);
        }
        if(s.rOwner > 0 || s.tOwner > 0) _takeOwnerFee(s.rOwner, s.tOwner);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.ownerFee + temp.dev) * 2;
