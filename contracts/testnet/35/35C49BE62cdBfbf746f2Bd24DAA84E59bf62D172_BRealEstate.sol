...
contract BRealEstate is Ownable {
    ...
    constructor(address _usdr, address _breToken) {
        buyerOfferIndex = 0;
        usdbAddress = _usdr;
        breAddress = _breToken;
        REXToken = IERC20(_breToken);
        USDB = IERC20(_usdr);
        fiatTypes["VND"] = true;
        fiatTypes["USD"] = true;
    ...
    // function updateStableCoin(address _usdr) external onlyOwner {
    //     usdbAddress = _usdr;
    //     USDB = IERC20(_usdr);
    // }
        ...
        isWhitelist
    {
        ...
        require(
            USDB.balanceOf(_msgSender()) >= tokenAmount,
        ...
        isAdmin
    {
        ...
        //calculate reward
        uint256 totalRewardMax = SafeMath.div(SafeMath.mul(
            SafeMath.mul(
        ...
        require(
            REXToken.balanceOf(_msgSender()) >= totalReward,
            "Error: not enough REX Token"
        );
        ...
        external
    {
        ...
        if (projects[_symbol].isDistributed) {
            REXToken.safeTransferFrom(address(this), _msgSender(), tokenAmount);
            emit ClaimReward(
                _symbol,
                _stockAmount,
                tokenAmount,
            ...
            USDB.safeTransferFrom(address(this), _msgSender(), tokenAmount);
            emit ClaimRefundIdo(
                _symbol,
                _stockAmount,
                tokenAmount,
    ...
    function exchangeStock(
        ...
        require(
            REXToken.balanceOf(_msgSender()) >= _tokenAmount,
            "Error: not enough REXToken"
        );
        require(
            block.timestamp > projects[_symbol].startTime,
        ...
        REXToken.safeTransfer(_seller, _tokenAmount);
    ...
    function withdrawIdo(string calldata _symbol) external isAdmin {
        ...
        USDB.safeTransferFrom(address(this), _msgSender(), totalUsdb);
