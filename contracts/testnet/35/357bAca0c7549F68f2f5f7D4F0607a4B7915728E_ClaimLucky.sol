...
contract ClaimLucky is Ownable {
    ...
    mapping(uint256 => uint256) public roundIdAmount;
    uint256 public lastRoundId;
    uint256 public preClaimAmount;
    address public signAddress;
    address public basicToken;
    address public adminAddress;
    constructor(address _signAddress,address _basicToken,address _adminAddress) public {
    ...
    function claim( uint256 _claimId,uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public {
        ...
        preClaimAmount = preClaimAmount.sub(amount);
        emit Claim(msg.sender, basicToken, amount,_claimId);
    }

    function addClaim(uint256 amount,uint256 roundId) public onlyAdmin {
        require(amount > 0,"Zero amount!");
        preClaimAmount = preClaimAmount.add(amount);
        roundIdAmount[roundId] = roundIdAmount[roundId].add(amount);
    }
    ...
    function getRealAmount() public view returns (uint256 realAmount,uint256 lastRId) {
        realAmount = (IERC20(basicToken).balanceOf(address(this))).sub(preClaimAmount);
        lastRId = lastRoundId;
    }
