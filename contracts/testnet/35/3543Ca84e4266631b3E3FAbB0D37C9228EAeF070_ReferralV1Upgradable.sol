    ...
    UUPSUpgradeable
{
    ...
    event StakingReferralRewardPaid(
        address indexed referee,
        address indexed referrer,
        uint256 indexed amount,
        uint256 level,
    ...
    function _getTokenName() private view returns (string memory) {
        return IERC20_EXTENDED(tokenAddress).name();
    }

    function getTokenName() external view returns (string memory) {
        return _getTokenName();
    }

    function _getTokenDecimals() private view returns (uint256) {
        return IERC20_EXTENDED(tokenAddress).decimals();
    }

    function getTokenDecimals() public view returns (uint256) {
        return _getTokenDecimals();
    }
    ...
    function getUSDContractName() external view returns (string memory) {
        return IERC20_EXTENDED(USDAddress).name();
    }

    function _getUSDContractDecimals() private view returns (uint256) {
        return IERC20_EXTENDED(USDAddress).decimals();
    }

    function getUSDContractDecimals() private view returns (uint256) {
        return _getUSDContractDecimals();
    }
    ...
    function _payReferralInUSD(
        ...
        for (uint256 i; i < levelRateReferral.length; i++) {
            ...
            emit ReferralRewardPaid(_userAddress, referrer, c, i + 1, "USD");
            userAccount = referrerAccount;
        }
    ...
    function _payReferralInTokenStaking(
        ...
        for (uint256 i; i < levelRateStaking.length; i++) {
            ...
            emit StakingReferralRewardPaid(
                _referee,
                referrer,
                c,
                i + 1,
