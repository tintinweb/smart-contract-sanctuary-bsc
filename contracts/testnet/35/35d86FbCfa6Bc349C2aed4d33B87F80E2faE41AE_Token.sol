...
contract Token is ReentrancyGuard, IToken{
    ...
    function claimPrincipal() external nonReentrant returns(uint256) {
        uint256 amountToClaim = getPrincipalToClaim(msg.sender);
        require(amountToClaim > 0, 'No tokens available to claim');
    ...
    /**
        @notice Returns the number of tokens purchased minus the number that have been repaid
                and the amount that is available to claim.
        @param _address Address of the holder.
        @return uint256 Balance of Principal
     */
    function getBalanceOfPrincipal(address _address) public view returns(uint256){
        return balances[_address].principal - balances[_address].principalRepaid - balances[_address].principalToClaim;
    }

    /**
        @notice Returns the number of tokens the ther holder has in principal.
                ...
                principal amount (getBalanceOfPrincipal).
        @param _address Address of the holder.
    ...
    function getTotalOutstandingPrincipal() public view returns(uint256) {
        ...
        for (uint256 i=0; i<arrayLength;) {
            totalOutstandingPrincipal += balances[_holders[i]].principal - balances[_holders[i]].principalRepaid - balances[_holders[i]].principalToClaim;
            unchecked { ++i; }
        }
    ...
    function getTotalOutstandingEarnedToClaim() public view returns(uint256) {
        ...
        // Calculate the total amounts of earned and principal to claim for all holders.
        for (uint256 i=0; i<arrayLength;) {
            totalOutstandingEarnedToClaim += getBalanceOfEarned(_holders[i]);
            unchecked { ++i; }
        }
    ...
    function recalculateDistributionAmountsForHolders() internal {
        ...
        // Calculate the total amounts of earned and principal to claim for all holders.
        uint256 totalPriorDistribution = getTotalOutstandingEarnedToClaim() + getTotalOutstandingPrincipalToClaim();

        for (uint256 i=0; i<arrayLength;) {
            if(getBalanceOfEarned(_holders[i]) > 0){
            ...
            if(balances[_holders[i]].principalToClaim > 0){
                ...
                balances[holders[i]].principalToClaim = amountForDistribution;
                balanceToDistribute -= amountForDistribution;
                newTotalSupply += getBalanceOfPrincipal(holders[i]);
            }
            unchecked { ++i; }
        }
     ...
     */
    function updateAssetAddress(address newAssetAddress, uint256 minNewAssetAmount) external override onlyOwner {
    ...
    function distributeEarnedAmountToHolder(uint256 amount, address holder) external onlyOwner returns(bool){
        bool success = depositAmountForDistribution(amount, DistributionType.EARNED, holder);
        return success;
    }
    ...
    function distributePrincipalAmountToHolder(uint256 amount, address holder) external onlyOwner returns(bool){
        ...
        bool success = depositAmountForDistribution(amount, DistributionType.PRINCIPAL, holder);
        return success;
    }
...
interface IToken is IERC20 {
    ...
    function updateAssetAddress(address newAssetAddress, uint256 minNewAssetAmount) external;
