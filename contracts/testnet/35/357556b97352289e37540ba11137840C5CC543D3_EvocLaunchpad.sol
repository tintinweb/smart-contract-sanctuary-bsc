...
contract EvocLaunchpad is Ownable, ReentrancyGuard {
    ...
    {
        BEP20 ContractToken = BEP20(contractAdd);
        Partner_register storage item = PartnerRegister[PartnerId];
        return (
           ContractToken.name(),
           ContractToken.symbol(),
           item._bonusPercent,
           item._contractAdd,
           item._OwnerAdd,
           item._maxUserAmount,
        ...
        )
    {
        ...
        return (
            ...
            item.PartnerValues._totalOnPreSale,
    ...
    function buyWithToken(
        ...
        item.Balance[tkName].amount += tkAmount;
        item.UserConfig[msg.sender].payOut[tkName].amount += tkAmount;
    ...
    function createPartner(
        ...
        uint256 dexBalance = ContractToken.balanceOf(msg.sender);
        uint256 LiquifyBalance = _totalOnPreSale.add((_totalOnPreSale*_liquidityPercentage).div(100));
        require(
            _totalOnPreSale > 0 && _totalOnPreSale <= dexBalance,
        ...
        PartnerRegister[partnerId_]._tokenValueDollar = _tokenValueDollar;
        PartnerRegister[partnerId_].PartnerValues._totalOnPreSale = _totalOnPreSale;
        PartnerRegister[partnerId_].PartnerValues._totalRemaining = _totalOnPreSale;
    ...
    function partnerStatus(uint PartnerId) public view returns (uint256) {
        ...
        if (item.PartnerValues._totalRemaining <= 0 || item.PartnerValues._totalTokenSold >=  item.PartnerValues._totalOnPreSale) {
        ...
        if (
            ...
            item.PartnerValues._totalTokenSold < item.PartnerValues._totalOnPreSale
        ) {
     ...
     * ----------------------
     */
    function setClaimingPartner(uint PartnerId) public isPausable {
        Partner_register storage item = PartnerRegister[PartnerId];
        require(
        ...
        if(
            item.PartnerValues._totalRemaining <= 0 &&
            item.PartnerValues._totalTokenSold >= item.PartnerValues._totalOnPreSale
        ){
            ...
            /* uint256 tokenBalance = (item.PartnerValues._totalOnPreSale*item.PartnerStatus._liquidityPercentage).div(100);
            swapAndLiquify(tokenBalance, PartnerId); */
        }
        
    }

    function swapAndLiquify(uint256 tokenBalance, uint PartnerId) private lockTheSwap {
        ...
        // swap tokens for BNB
        swapTokensForBNB(half, PartnerId); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered

        // how much BNB did we just swap into?
        uint256 newBalance = address(this).balance.sub(initialBalance);

        // add liquidity to uniswap
        addLiquidity(otherHalf, newBalance, PartnerId);

        emit SwapAndLiquify(half, newBalance, otherHalf);
    }

    function swapTokensForBNB(uint256 tokenAmount, uint PartnerId) private {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, uint PartnerId) private {
        IUniswapV2Router02 routerAdress = IUniswapV2Router02(PartnerRegister[PartnerId].PartnerStatus._tokenPair);
        IBEP20 ContractToken = IBEP20(PartnerRegister[PartnerId]._contractAdd);
        ...
        // add the liquidity
        routerAdress.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    function setPckRouterAddress(address newRouter) external onlyOwner {
    ...
    /* Function     : Set a new router if released  */
    /* Parameters   : New router Address */
    function setEvcRouterAddress(address newRouter) external onlyOwner {
        require(newRouter != address(0), "Should not be address 0");
     ...
     * @param addr of the contract address
     */
    function setUsdtAdress(address addr) external virtual onlyOwner {
        require(addr.isContract(), "The address entered is not valid");
        usdtAddress_ = addr;
    }
