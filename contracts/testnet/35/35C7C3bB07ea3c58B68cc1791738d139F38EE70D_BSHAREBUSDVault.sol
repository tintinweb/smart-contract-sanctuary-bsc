...
function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused {
        ...
        if(IERC20(Admin.BShare()).balanceOf(address(this)) >= BshareRequire){
            ...
            convertBUSDToLP(amount * percentBSHARE/100, (reserveBshare * (amount * percentBSHARE/100)) / reservesBUSD, Admin.BUSD(), Admin.BShare(), address(BSHAREBUSD));
        }
        else{
            percentBSHARE = 15;
            ...
            path[0] = Admin.BUSD();
            path[1] = Admin.BShare();
            IERC20(Admin.BUSD()).safeApprove(router, (amount * percentBSHARE/100));
            swap(
            ...
            if(IERC20(Admin.BShare()).balanceOf(address(this)) <= (reserveBshare * amount * percentBSHARE)/(reservesBUSD * 100)){
                    convertBUSDToLP(reservesBUSD * IERC20(Admin.BShare()).balanceOf(address(this)) / reserveBshare, 
                    IERC20(Admin.BShare()).balanceOf(address(this)),
                    Admin.BUSD(), Admin.BShare(), address(BSHAREBUSD));
                    

            }else{
                convertBUSDToLP(amount * percentBSHARE/100, 
    ...
    */
    ...
    function distribute(uint256 _rebaseReward) external nonReentrant{
        ...
        console.log(IERC20(BSHAREBUSD_A).totalSupply(),"(IERC20(BSHAREBUSD_A).totalSupply()");
