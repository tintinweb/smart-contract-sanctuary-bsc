    ...
    IERC721ReceiverUpgradeable
{
    ...
    function getCurrentPrice(address nftContractAddress, uint256 tokenId) public view returns(uint256){
        ...
        if ((block.timestamp - nftContractAuction.auctionStart) > auctionPeriod.decreasePeriod){
            uint256 totalPrice = (nftContractAuction.startPrice - nftContractAuction.minPrice);
            uint256 minutesPassed = ((nftContractAuctionTimmings.deadlineTime - block.timestamp)/(nftContractAuctionTimmings.deadlineTime - nftContractAuction.auctionStart)) * 1000000;
            uint256 amountToDecrease =(minutesPassed * totalPrice)/1000000;
