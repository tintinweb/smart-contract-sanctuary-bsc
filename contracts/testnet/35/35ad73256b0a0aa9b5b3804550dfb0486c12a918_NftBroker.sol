...
contract NftBroker is IERC721Receiver, IERC1155Receiver, ReentrancyGuard {
  ...
  function brokeNft(address _address, uint256 _id, uint256 _packageId) public payable nonReentrant {
    require(_listings[_address][_id].seller != address(0), "5");
    require(packages[_packageId].ethPrice > 0 || packages[_packageId].ercPrice > 0, "6");
    if(msg.value > 0) require(msg.value >= packages[_packageId].ethPrice, "7");
    else require(IERC20(token).transferFrom(msg.sender, address(this), packages[_packageId].ercPrice), "8");
    ...
    _brokerages[_address][_id].expiry = block.timestamp + (_packageId * 1 * 60 * 60);
    emit _brokeNFT(msg.sender, _address, _id);
  }
  function cancelBrokeNft(address _address, uint256 _id) public {
    require(_brokerages[_address][_id].broker == msg.sender, "9");
    delete _brokerages[_address][_id];
  ...
  function buyNft(address _address, uint256 _id, bool isERC721) public payable nonReentrant {
    require(_brokerages[_address][_id].expiry > block.timestamp, "10");
    if(_listings[_address][_id].isErc20) {
      ...
      require(IERC20(token).transferFrom(msg.sender, _listings[_address][_id].seller, priceAfterFee), "11");
      require(IERC20(token).transferFrom(msg.sender, _brokerages[_address][_id].broker, fee), "12");
    } 
    else {
      require(msg.value >= _listings[_address][_id].price, "13");
  ...
  function manageErc(address _to) public onlyOwner {
    ...
    require(IERC20(token).transfer(_to, ercBal), "17");
  }
}