...
contract BNBRegistrarControllerV6 is Ownable {
    ...
    uint public usedQuota;
    uint public invdividualQuota;
    ...
    constructor(
        ...
        uint _totalQuota,
        uint _individualQuota
    ) public {
        require(_maxCommitmentAge > _minCommitmentAge);
        require(_totalQuota > 0);
        require(_individualQuota > 0);
        totalQuota = _totalQuota;
        invdividualQuota = _individualQuota;
        usedQuota = 0;
    ...
    function hasSBT(address addr) public view returns (bool) {
        return babToken.balanceOf(addr) > 0 || galxePassport.balanceOf(addr) > 0;
    }

    function individualQuotaUsed(address addr) public view returns (uint) {
        return claimed[addr];
    }
    ...
    function getCurrentUsage() public view returns (uint) {
        return usedQuota;
    }
    ...
    function registerWithConfig(
        ...
        //qualification check
        require(hasSBT(addr), "has no BABT or GALXE Passport");
        require(claimed[addr] < invdividualQuota, "individual quota used up");
