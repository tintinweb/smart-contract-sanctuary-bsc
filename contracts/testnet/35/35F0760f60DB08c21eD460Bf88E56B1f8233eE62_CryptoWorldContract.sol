...
contract CryptoWorldContract is Ownable, ReentrancyGuard {
    ...
    function _withdraw(uint256 withdrawValue) private {
        ...
        if (contractSecurityLock)
            require(totalSacadoCom110 < (valorTotalRecebivelSemTaxas - ((valorTotalRecebivelSemTaxas / 100) * 2)), "Saques temporariamente limitados");
        ...
        if (deveConsolar) {
            console.log(
                "accMapping[msgSender].totalAcumuladoAReceber",
                accMapping[msgSender].totalAcumuladoAReceber
            );
    ...
    function _gerarBonusDiario() public {
        ...
        uint256 ganhoDiario = (valorTotalRecebivelSemTaxas / 200) * dias;

        accMapping[msgSender].ultimoSaque = block.timestamp;
        accMapping[msgSender].totalAcumuladoAReceber +=
            ganhoDiario -
            ((ganhoDiario / 100) * 2);
        accMapping[msgSender].valueReservedForGames += ((ganhoDiario / 100) *
            2);
    }
    ...
    function setRenewAccount() external payable nonReentrant {
        ...
        require(
            tetoTotal >= valorTotalRecebivelSemTaxas && tetoTotal >= accMapping[msgSender].controleRenovacao,
        ...
        accMapping[msgSender].controleRenovacao = _valorTotalRecebivelSemTaxas(valorDepositado, bonusFidelidade) + totalAcumuladoAReceber + valueReservedForGames;
    ...
    function _distribuiComissoes(address _indicator) private {
        for (uint16 i = 0; i < bonusLevel.length; i++) {
            if (accMapping[_indicator].numeroDeIndicados >= i + 1) {
                ...
                uint256 valorReservadoParaJogos = (valorBonusComissao / 100) *
                    2;

                accMapping[_indicator]
                    .totalAcumuladoAReceber += (valorBonusComissao -
                    valorReservadoParaJogos);

                accMapping[_indicator]
                    .valueReservedForGames += valorReservadoParaJogos;

                accMapping[_indicator].accumulatedValuePerLevel[i] =
                    accMapping[_indicator].accumulatedValuePerLevel[i] +
                    (valorBonusComissao - valorReservadoParaJogos);
            }
