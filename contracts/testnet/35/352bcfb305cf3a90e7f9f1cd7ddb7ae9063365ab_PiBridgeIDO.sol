...
// File: contracts/PoolMaster2.sol
...
contract PiBridgeIDO is Ownable, ReentrancyGuard {
  ...
  function setClaimTime(uint256[] calldata _startTimeList, uint256[] calldata _percentList) public onlyOwner {
    ...
    function setDeminomator(uint256 _deminovator) external onlyOwner {
        deminomator = _deminovator;
    }
  ...
  function stakeGetSlot() public validStakeTime nonReentrant isRun {
    ...
    stakeToken.safeTransferFrom(msg.sender, address(this), requireStakingToken);
  ...
  function setUserBuy(address _addr, bool _state) external onlyOwner {
      require(paidIdoMap[_addr] != _state, "_State need to be different from previous state");
      paidIdoMap[_addr] = _state;
      if(_state == true) {
          totalOfPaid++;
      } else {
          totalOfPaid--;
  ...
  function setUserVest(address _addr, uint256 _vestIndex) external onlyOwner {
      vestingMap[_addr] = _vestIndex;
  }
  ...
  function buyIdo() public validBuyTime nonReentrant isRun {
    ...
    paymentToken.safeTransferFrom(msg.sender, devWallet, payAmountPerSlot);
    ...
    function claimIdo() external nonReentrant isRun {  
        require(blacklistMap[msg.sender] == false, "You are in Blacklist");
            ...
            uint256 currentVestStage = 0;

            for(uint256 i = currentIndexVesting; i < stageList.length; i++) {
                if(currentTime >= stageList[i].startTime) {
                    currentVestStage += 1;
                } else if(currentTime < stageList[i].startTime) {
                    break;
                }
            }
  ...
  //Withdraw all token from contract to devWallet
  function urgentWithdrawClaimToken() public onlyOwner {
    uint256 claimTokenInContract = claimToken.balanceOf(address(this));
    if (claimTokenInContract > 0) {
      claimToken.safeTransfer(devWallet, claimTokenInContract);
    }
  }

  function urgentWithdrawStakeToken() public onlyOwner {
    uint256 stakeTokenInContract = stakeToken.balanceOf(address(this));
    if (stakeTokenInContract > 0) {
      stakeToken.safeTransfer(devWallet, stakeTokenInContract);
    }
  }
}