...
contract MYPublicRow is Modifier, StringUtil {

    using SafeMath for uint256;

    mapping(address => bool) private joinStatus;
    ...
    function setTokenContract(address _myToken) public onlyOwner {
    ...
    function join(address inviterAddress, string memory amountToStr, string memory seed, bytes32 _r, bytes32 _s, uint8 _v) external isRunning nonReentrant {

        if(joinStatus[msg.sender]) {
            _status = _NOT_ENTERED;
            revert("MY: Invalid error");
        }
        ...
        string memory _inviterAddress = toString(inviterAddress);

        bytes32 msgHash = keccak256(abi.encodePacked(_inviterAddress, amountToStr, seed));
    ...
    function getJoinStatus(address _address) public view returns(bool) {
        return joinStatus[_address];
    }
