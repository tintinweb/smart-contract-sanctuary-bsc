...
import "./INftCatalog.sol";
import "./NftCatalogStorage.sol";

contract NftCatalog is
  Initializable,
  UUPSUpgradeable,
  ...
  NftCatalogStorage
{
  ...
  function add(
    ...
    uint128 unitPrice_,
    uint128 fee_
  ) external virtual override onlyRole(MANAGER_ROLE) {
    require(fee_ <= 100, "NftCatalog_001");
  ...
  function remove(bytes32 id_)
    external
    virtual
    override
    onlyRole(MANAGER_ROLE)
  {
    Item memory item = catalog[id_];
    item.isListed = false;

    _update(id_, item);
  }

  function get(bytes32 id_) external view override returns (Item memory) {
    return catalog[id_];
  }

  event UpdateNftCatalog(
    bytes32 indexed id,
  ...
  function _update(bytes32 id_, Item memory item_) internal virtual {
    catalog[id_] = item_;
    emit UpdateNftCatalog(
      id_,
      item_.nftAddress,
      item_.tokenAddress,
      item_.feeAddress,
      item_.rewardsAddress,
      item_.unitPrice,
      item_.fee,
      item_.isListed
    );
