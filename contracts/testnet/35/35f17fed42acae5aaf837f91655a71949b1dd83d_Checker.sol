...
contract Checker is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public SWAPTOKEN = 0x094616F0BdFB0b526bD735Bf66Eca0Ad254ca81F;
    address public REWARD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;

    string constant _name = "checker";
    string constant _symbol = "chk";
    ...
    address public marketingFeeReceiver = 0x7f30174D3Ca49052eE11dAbFE709E7E87E9F0227;
    address public devFeeReceiver = 0x21f4f9117a9dE35ecA20ed940C4d2a9a4c4E9c33;
    ...
    uint256 public swapThreshold = (_totalSupply * 10) / 100000;//0.01%
    bool inSwap;
    modifier swapping() {
        inSwap = true;
        _;
    ...
    function takeFee(address sender, uint256 amount, address to) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            uint256 feesToContract = feeAmount;
    ...
    event UpdateBuyFees(uint256 reward, uint256 marketing, uint256 dev);
    function updateBuyFees(uint256 reward, uint256 marketing, uint256 dev) external onlyOwner {
        buyDividendRewardsFee = reward;
        buyMarketingFee = marketing;
        buyDevFee = dev;
        buyTotalFees = reward.add(marketing).add(dev);
        require(buyTotalFees <= 11, "Total Fee must be less than 11%");
        emit UpdateBuyFees(reward, marketing,dev);
    }

    event UpdateSellFees(uint256 reward, uint256 marketing, uint256 dev);
    function updateSellFees(uint256 reward, uint256 marketing, uint256 dev) external onlyOwner {
        sellDividendRewardsFee = reward;
        sellMarketingFee = marketing;
        sellDevFee = dev;
        
        sellTotalFees = reward.add(marketing).add(dev);
        require(sellTotalFees <= 11, "Total Fee must be less than 11%");
        emit UpdateSellFees(reward, marketing, dev);
    }
