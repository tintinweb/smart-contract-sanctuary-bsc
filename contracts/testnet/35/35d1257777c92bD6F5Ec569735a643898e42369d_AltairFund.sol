    ...
    ReentrancyGuardUpgradeable
{
    ...
    IAltairSwap internal altairSwap;
    ...
    function initialize(
        ...
        BaseTokenName = "BNB";
        WETH = address(IAltairFactory(_factoryOwner).NATIVE()); //testnet
        pause = false;
    ...
    function getTransferAmt(address _underyingAdd, uint256 _redeemratio)
        public
        view
        returns (AltairLib.transferData memory transData)
    {
        return _getTransferAmt(_underyingAdd, _redeemratio);
    }
    ...
    function getBalance(address _fromAdd) external view returns (uint256) {
        return _getBalance(_fromAdd);
    }

    function getBalanceInUSD(address _fromAdd) external view returns (uint256) {
        return _getBalanceInUSD(_fromAdd);
    }

    /// @dev return weight of each token in the fund
    function getTargetWeight(address _addr) public view returns (uint256) {
        return TargetWeight[_addr];
    }
    ...
    function getLatestPrice(address _targetAdd)
        external
        view
        returns (uint256)
    {
    ...
    function getTokenValues(address _tokenaddress)
        external
        view
        returns (uint256)
    {
        return _getTokenValues(_tokenaddress);
    }
        ...
        returns (uint256 finalSwapOutput)
    {
        ...
        uint256 platformUnit = _swapOutput.mul(platformFee).div(10000);
        platformWallet = IAltairFactory(factory).treasury();
        ...
        uint256 managerUnit = _swapOutput.mul(managerFeeBps).div(10000);
    ...
    function _handleFeeTransferRedeem(uint256 _swapOutput)
        internal
        returns (uint256 finalSwapOutput)
    {
        platformFee = IAltairFactory(factory).redeemFee();
        uint256 platformUnit = _swapOutput.mul(platformFee).div(10000);
        platformWallet = IAltairFactory(factory).treasury();
    ...
    function setCopyTrading(bool _isCopy, address _fundToCopy)
        public
        returns (bool)
    {
        onlyManager();
        if (_isCopy == false) {
            ...
            fundToCopy = IAltairFund(_fundToCopy);
    ...
    function setDCA(bool _dca) public returns (bool) {
        onlyManager();
        dca = _dca;
        return true;
    }
    ...
    function setManagerMonthlyFee(uint256 _fee) external returns (bool) {
        onlyManager();
        require(_fee <= managerMaxMonthlyFee, "FeeTooHigh");

        managerMonthlyFee = _fee;

        // managerSetMonthlyFeeTimestamp = block.timestamp + 30 days;

        managerSetMonthlyFeeTimestamp = block.timestamp + 1 minutes;

        managerMonthlyTimestamp = block.timestamp + 1 minutes;
    ...
    function updateRebalancePeriod(uint256 _newCycle) external {
        onlyManager();

        rebalanceCycle = _newCycle;
        emit RebalanceCycleUpdate(rebalanceCycle, _newCycle, block.timestamp);
    }

    /// @dev update manager fee and wallet
    function updateManagerProperty(address _newManager, bool _community)
        external
        payable
    {
        onlyManager();
        managerOwner = _newManager;
        community = _community;
        managerFeeBps = IAltairFactory(factory).managerFee();
    }
    ...
    function updateNonBMNames(
        ...
        for (uint256 i; i < _nonBMaddress.length; i++) {
            //make sure it is not in bm target
            require(TargetWeight[_nonBMaddress[i]] == 0, "InBalanceManager");
    ...
    function subscribe(
        ...
        _tradeParams.amount = finalSwapOutput;

        (uint256 mintQty, uint256 fundvalue) = _getMintQty(finalSwapOutput);

        factory.mint(address(this), _investorAddress, mintQty, EMPTY_DATA);

        uint256 priceImpactTollerance = _tradeParams.priceImpactTolerance;
        uint256 deadline = _tradeParams.deadline;
    ...
    function redeem(
        ...
        if (msg.sender != address(this)) {
            factory.burn(address(this), _investorAddress, _tradeParams.amount);
        }
        ...
        uint256 priceImpactTollerance = _tradeParams.priceImpactTolerance;
        uint256 amount = _tradeParams.amount;
        uint256 deadline = _tradeParams.deadline;
        ...
        uint256 totalValue = _getFundValues();
        address investor = _investorAddress;
    ...
    function emergencyRedeem(uint256 _redeemUnit, address _investorAddress)
        external
        payable
    {
        uint256 totalValueB4 = _getFundValues();

        uint256 redeemratio = _redeemUnit.mul(1e18).div(totalSupply());
        require(redeemratio > 0, "MustBeZero");
        factory.burn(address(this), msg.sender, _redeemUnit);
        uint256 totalBaseBal = address(this).balance;
    ...
    function _getDeleteNames(address[] memory _toAddresses)
        internal
        view
        returns (AltairLib.DeletedNames[] memory deletedNames)
    {
        ...
        for (uint256 i; i < targetNamesAddress.length; i++) {
            ...
            for (uint256 x; x < _toAddresses.length; x++) {
                if (targetNamesAddress[i] == _toAddresses[x]) {
                    break;
                } else if (
                    targetNamesAddress[i] != _toAddresses[x] &&
                    _toAddresses.length == matchtotal
                ) {
    ...
    function _getBalance(address _fromAdd) internal view returns (uint256) {
        if (IERC20Upgradeable(_fromAdd) == IERC20Upgradeable(BaseToken))
            return address(this).balance;
        return IERC20Upgradeable(_fromAdd).balanceOf(address(this));
    }

    function _getBalanceInUSD(address _fromAdd)
        internal
        view
        returns (uint256)
    {
        ...
        return
            IERC20Upgradeable(_fromAdd).balanceOf(address(this)).mul(unit).div(
                1e18
            );
    }
    ...
    function _getUnitPriceInUSD() internal view returns (uint256) {
        ...
        uint256 toBasePrice = uint256(fundMaster.useChainlinkOracle());

        return totalValue.mul(toBasePrice).div(1e18);
    }

    function _isSmallSubs(uint256 _fundvalue, uint256 _subsAmt)
        internal
        pure
        returns (bool)
    {
        if (_fundvalue == 0) return false;
        uint256 percentage = _subsAmt.mul(10000).div(_fundvalue);
    ...
    function _getTokenValues(address _tokenaddress)
        internal
        view
        returns (uint256)
    {
        uint256 tokenBalance = _getBalance(_tokenaddress);
        uint256 price = _getLatestPrice(_tokenaddress); //price from token to BNB
        return tokenBalance.mul(uint256(price)).div(1e18);
    }

    function _getLatestPrice(address _targetAdd)
        internal
        view
        returns (uint256)
    {
        ...
        return fundMaster.getPriceByAddress(_targetAdd, BaseTokenName);
    }

    /// @dev Calc return balance during redemption
    function _getTransferAmt(address _underyingAdd, uint256 _redeemratio)
        internal
        view
        returns (AltairLib.transferData memory transData)
    {
        AltairLib.transferData memory _transferData;
        _transferData.totalUnderlying = _getBalance(_underyingAdd);
        uint256 qtyToTrf = _redeemratio.mul(_transferData.totalUnderlying).div(
            1e18
        );
    ...
    function _getMintQty(uint256 _srcQty)
        internal
        view
    ...
    function _getActiveOverWeight(address _destAddress, uint256 _totalfundvalue)
        internal
        view
        returns (
            uint256 destRebQty,
    ...
    {
        destRebQty = 0;
        uint256 destTargetWeight = TargetWeight[_destAddress];
        uint256 destValue = _getTokenValues(_destAddress);
        fundWeight = destValue.mul(10000).div(_totalfundvalue);
        ...
        if (overweight) {
            uint256 price = _getLatestPrice(_destAddress);
    ...
    function _rebalance(uint256 _deadline, uint256 _priceImpactTolerance)
        internal
        returns (uint256 baseccyBal)
    {
        ...
        ) = _sellOverWeightNames(_deadline, _priceImpactTolerance);
    ...
    function _buyUnderWeightNames(
        uint256 _deadline,
        uint256 _priceImpactTolerance,
        AltairLib.UnderWeightData[] memory _underweightNames,
        uint256 _totalunderActiveweight
        ...
        for (uint256 i; i < _underweightNames.length; i++) {
            if (_underweightNames[i].token != address(0)) {
                uint256 rebaseActiveWgt = _underweightNames[i]
                    .activeWeight
                    .mul(10000)
                    .div(_totalunderActiveweight);
    ...
    function _transferNonBM(uint256 _redeemratio, address _investorAddress)
        internal
    {
        ...
        for (uint256 i; i < nonBMNamesAddress.length; i++) {
            ...
            if (TargetWeight[nonBMNamesAddress[i]] == 0) {
                ...
                uint256 trfOutput = _redeemratio.mul(tokenBalance).div(1e18);
                if (trfOutput > 0) {
                    TransferHelper.safeTransfer(
                        nonBMNamesAddress[i],
                        _investorAddress,
