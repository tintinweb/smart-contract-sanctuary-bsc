...
contract Flashloan is Ownable {
    ...
    IPancakeFactory constant pancakeFactory =
        IPancakeFactory(0x6725F303b657a9451d8BA641348b6761A6CC7a17);
    ...
    function startArbitrage(address _tokenBorrow, uint256 _amount)
        ...
        onlyOwner
    {
        address tokenOther = _tokenBorrow == WBNB ? BUSD : WBNB;
        permissionedPairAddress = pancakeFactory.getPair(
            _tokenBorrow,
            tokenOther
    ...
    function execute(
        ...
        (address tokenBorrow, uint256 _amount) = abi.decode(
            _data,
            (address, uint256)
        );
        ...
        address pair = pancakeFactory.getPair(token0, token1);

        uint256 fee = ((_amount * 3) / 998) + 1;
        uint256 amountToRepay = _amount + fee;
        IWBNB(WBNB).deposit{ value: amountToRepay }();
        ...
        IERC20(WBNB).transfer(pair, amountToRepay);
    }
