...
contract PredictionGame is Ownable, Pausable, ReentrancyGuard {
    ...
    /// @notice When any one place a bet
    /// @param user owner of the bet
    /// @param amount user bet amount
    /// @param head true: HEAD, false: END
    event PlaceBet(uint256 epoch, address user, uint256 amount, bool head);
    ...
    event NewMinBetAmount(uint256 round, uint256 minBetAmount);
    ...
    function placeBet(
        ...
        require(
            epoch == _getEpoch(block.number),
            "Bet not active for this epoch"
        );
        ...
        emit PlaceBet(epoch, msg.sender, amount, head);
    }

    function claimReward(uint256[] calldata epochs)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        if (reward > 0) {
            IERC20(currency).safeTransfer(msg.sender, reward);
        }
    }
     ...
     */
    function executeRound(uint256 epoch) external whenNotPaused onlyOperator {
    ...
    function _endRound(Round storage round, bytes32 _hash) internal {
        ...
        if (round.head) {
            ...
            if (rewardBaseCalAmount == 0) {
                ...
                treasuryAmt = (round.endAmount * treasuryRate) / 10000;
            }
            ...
            if (rewardBaseCalAmount == 0) {
                ...
                treasuryAmt = (round.headAmount * treasuryRate) / 10000;
            }
        ...
        onlyAdmin
    {
        ...
        emit NewMinBetAmount(_getEpoch(block.number), minBetAmount);
    }
