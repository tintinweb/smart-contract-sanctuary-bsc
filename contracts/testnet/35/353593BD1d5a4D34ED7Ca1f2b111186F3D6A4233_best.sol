...
contract best is ERC20Detailed, Ownable {
    ...
    string public _name = "BEST STAKING";
    string public _symbol = "BST";
    ...
    uint256 public bstInsuranceFundFee = 10;
    ...
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(bstInsuranceFundFee).add(
            firePitFee
        );
    ...
    address public bstInsuranceFundReceiver;
    ...
    constructor() ERC20Detailed("BST", "BST", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0xdf14055167B6946dB6671fC9Ca71B3dBAcC2A364;
        treasuryReceiver = 0xDC1EAD0C79b646E403d3d8F0dD18E2D9669D04F3;
        bstInsuranceFundReceiver = 0xfD7EB1fc6c660adF7b9727209f7349aF7615922F;
        firePit = 0x412C441B4aB77D20499AeBC1F362AC23d61F97c4;
    ...
    function takeFee(
        ...
        uint256 _bstInsuranceFundFee = bstInsuranceFundFee;
        ...
        if (recipient == pair) {
            ...
            _bstInsuranceFundFee = bstInsuranceFundFee.add(sellFees);
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(bstInsuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(bstInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(bstInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(bstInsuranceFundFee).div(
                treasuryFee.add(bstInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function withdrawAllToTreasury() external swapping {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no BST token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
    ...
    function withdrawAll(address receiver) external swapping {
        require(managers[msg.sender] == true, "This address is not allowed to interact with the contract");
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no BST token deposited in token contract");
        address[] memory path = new address[](2);
    ...
    function setFeeReceivers(
        ...
        address _bstInsuranceFundReceiver,
        address _firePit
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        bstInsuranceFundReceiver = _bstInsuranceFundReceiver;
        firePit = _firePit;
    }
