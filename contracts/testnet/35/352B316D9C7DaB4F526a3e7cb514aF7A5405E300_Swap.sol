...
contract Swap is BEP20, Whitelist {
    ...
    string public constant name = "Staker Liquidity Token";
    string public constant symbol = "Stakers";
    ...
    uint256 public maxTokensPeriodicSell = 1000 ether;
    uint256 public periodSellLimitation = 1 days;
    ...
    function setMaxTokensPeriodicSell(uint256 _newAmount) public onlyOwner {
        maxTokensPeriodicSell = _newAmount;
    }

    function setPeriodSellLimitation(uint256 _newPeriod) public onlyOwner {
        periodSellLimitation = _newPeriod;
    }
    ...
    function tokenToBnbSwapInput(uint256 tokens_sold, uint256 min_bnb) public isNotPaused returns (uint256) {
        uint256 lastSellPeriod = block.timestamp - lastSells[msg.sender].time;
        uint256 amount = lastSells[msg.sender].amount + tokens_sold;
        if (lastSellPeriod <= periodSellLimitation) {
            require(amount <= maxTokensPeriodicSell, 'Sell Limitation');
            lastSells[msg.sender].amount = amount;
        } else {
            lastSells[msg.sender].amount = 0;
        }
        lastSells[msg.sender].time = block.timestamp;
