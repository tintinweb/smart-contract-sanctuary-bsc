...
constructor(address _ICO){
   ...
   function updateICOPrice()public {
      currentPrice = ICO.tokenPrice();
      currentRegistrationFee = ICO.REGESTRATION_FESS();
      emit UpdateIcoPrice(msg.sender, currentPrice, currentRegistrationFee);
      
   }

// Get Price of Token and Time Stamp
   function getPricesDiff(address _account) public view returns(uint256 _times, uint256 _prices, uint256 _registrationfee, uint256 _currentPrice, uint256 _currentResistaionFee) {
      return (register[_account].timeStamp, register[_account].price, register[_account].registrationfee, currentPrice,currentRegistrationFee);
   }
