...
contract RBL20 is Context, IERC20, Ownable {
  ...
  function getSoldPrice() public view returns(uint256) {
    if(_totalSupply>0){
      return usdToken.balanceOf(address(this)).div(_totalSupply);
    }else{
      return 0;
    }
  }
  ...
  function SellToken(uint256 _amountToken) public returns (bool) {
    uint256 sold_price = getSoldPrice();
    require(sold_price>0,"ERROR: EMTY TOTALSUPPLY");
    usdToken.transfer(msg.sender,_amountToken.mul(sold_price));
