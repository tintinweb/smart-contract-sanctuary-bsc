...
contract STC_Staking_Platform is Ownable{
    ...
    struct userDetails{
        ...
        mapping(Pack => bool) packages;
        mapping(Pack => uint) stakedAmount; // EG = pack => level => stakedAmount
        Pack pack;
        address[] refferred;
    ...
    event UpdatedLevel(uint level,uint refCount);
    ...
    function buyPackage(Pack pack) public{
        ...
        require(userdetails.level !=0, "buy level one");
        require(userdetails.packageBoughtTime + package.lifeSpan <= block.timestamp, "wait till package expires");
        require(userdetails.stakedAmount[pack] == 0, "Claim before purchasing package");
        ...
        userdetails.lastRewardClaimedTime =  userdetails.packageBoughtTime = block.timestamp;

        if(!userdetails.packages[pack]) 
            userdetails.packages[pack] = true; 
    ...
    function buyLevel(Pack pack, address reffererAddr, uint stcPrice, uint expiry,uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(userReferringdetails.level == 0,"Already refferal bought this package!");
        require(!userReferringdetails.packages[pack], "refferer already bought the package!");    
        require((reffererAddr == _Owner) || (userRefererdetails.packages[pack]),"User must buy the package 1st");

        if ((reffererAddr != _Owner) && (userRefererdetails.packageBoughtTime + package.lifeSpan <= block.timestamp)){
            userReferringdetails.packageBoughtTime = 0;
            userReferringdetails.packages[pack] = false;
            revert("!pack expired"); 
        }
        userRefererdetails.refferred.push(msg.sender);
        if ((reffererAddr != _Owner) && (userRefererdetails.refferred.length == levels[userRefererdetails.level])){            

            userRefererdetails.affiliateRewardAmount +=  calculateReward(
                pack, 
                userRefererdetails.level, 
                stcPrice,
                userRefererdetails.stakedAmount[pack]
            );
            userRefererdetails.level++;
        }
        uint reward = ((package.busdPrice * rewardTwoPercent) / diviser);
        transferAsset(reffererAddr, reward, package.busdPrice);
        userReferringdetails.level = 1;
        userReferringdetails.reffererAddr = reffererAddr;
        userReferringdetails.packageBoughtTime = block.timestamp;
        userReferringdetails.packages[pack] = true;
    ...
    function stake(Pack pack,uint stcPrice,uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        require(userdetails.packages[pack],"User doesn't bought the mentioned package!");
        if (userdetails.packageBoughtTime + package.lifeSpan <= block.timestamp){
            revert("!pack expired");    
        }
        ...
        userdetails.stakedAmount[pack] += stcAmount;
        setHashCompleted(prepareHash(msg.sender, stcPrice, pack, expiry), true);
        emit Staked(msg.sender, userdetails.level, userdetails.stakedAmount[pack], userdetails.reffererAddr);
    }
    ...
    function topUp(Pack pack,uint stcPrice,uint expiry ,uint8 v, bytes32 r, bytes32 s) public {
        ...
        uint totalStakedAmount = userdetails.stakedAmount[pack];
        if (topUpAmount > totalStakedAmount) {
            ...
            userdetails.stakedAmount[pack] += fAmount;
            setHashCompleted(prepareHash(msg.sender, stcPrice, pack, expiry), true);
    ...
    function claimAffiliate() public {
        userDetails storage userdetails = UserDetails[msg.sender];
        require(userdetails.affiliateRewardAmount > 0, "Aff r0");
        uint afRewAmnt = userdetails.affiliateRewardAmount;
        userdetails.affiliateRewardAmount = 0;
    ...
    function claimRewardAmount(Pack pack,uint busdPrice,uint expiry, uint8 v, bytes32 r, bytes32 s) public {
        ...
        userDetails storage userdetails = UserDetails[msg.sender];
        require(userdetails.stakedAmount[pack] > 0,"No staked amount!");
        require(userdetails.packages[pack],"not found in this package!");
        ...
        uint _lastRewardTime = ((userdetails.lastRewardClaimedTime == 0) ? userdetails.packageBoughtTime : userdetails.lastRewardClaimedTime);
        ...
        userdetails.lastRewardClaimedTime = _lastRewardTime + (rewardUpdatePerHour *rhrs);
        userdetails.totalRewardClaimed +=  totalRewardAmount;
    ...
    function getRewards(Pack pack,address userAddr, uint busdPrice) public view returns (uint rCalculate, uint rhrs, uint rPerHr) {
        ...
        Packages memory packk = packages[pack];
        uint lcReward = userdetails.lastRewardClaimedTime;
        uint reward = userdetails.stakedAmount[pack] * rewardTwoPercent / diviser;
        rPerHr = reward * busdPrice/1e18;
        lcReward = (lcReward == 0) ? userdetails.packageBoughtTime : userdetails.lastRewardClaimedTime;
        if (currentTime > packk.lifeSpan){
    ...
    function claimCapital(Pack pack) public {
        userDetails storage userdetails = UserDetails[msg.sender];
        Packages storage package = packages[pack];
        require(userdetails.packages[pack], "User not found");
        uint stkAmnt = userdetails.stakedAmount[pack];
        userdetails.stakedAmount[pack] = 0;
    ...
    function viewIsPackExpired(address userAddr, Pack pack) public view returns(bool r, string memory res){
        userDetails storage userdetails = UserDetails[userAddr];
        if (!userdetails.packages[pack]) {
            return (true, "not found in this pack!");
        }
        Packages memory package = packages[pack];
        r = userdetails.packageBoughtTime + package.lifeSpan < block.timestamp;
        res = r ? "Expired!" : "Not yet Expired";
    }

    function showRefferredAddressess(address user) public view returns(address[] memory) {
        userDetails storage userdetails = UserDetails[user];
        return userdetails.refferred;
    }

    function getStakedAmount(Pack pack, address user) public view returns(uint) {
        return UserDetails[user].stakedAmount[pack];
    }
    ...
    function updateLevel(uint level,uint refCount) public OnlyOwner {
        require(levels[level - 1] < refCount, "Invalid refCount");
        levels[level] = refCount;
        emit UpdatedLevel(level, refCount);
    }
