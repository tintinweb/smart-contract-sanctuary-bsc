...
contract TaskManager is Validatable, ITaskManager, ReentrancyGuardUpgradeable {
    ...
    function createTask(
        ...
        uint256 endTime,
        uint256 budget
    ) external onlyAdmin notZeroAddress(collection) notZero(budget) {
        require(startTime > block.timestamp, "Invalid startTime");
        require(startTime < endTime, "Invalid endTime");
        require(project.isCollectionActive(collection), "Collection inactive");
    ...
    function updateTask(
        ...
        uint256 startTime,
        uint256 endTime
    ) external onlyAdmin validTaskId(taskId) notZeroAddress(collection) notZero(budget) {
        require(startTime < endTime, "Invalid endTime");
     ...
     *  @dev    Only admin can call this function.
     */
    function cancelTask(uint256 taskId) external onlyAdmin validTaskId(taskId) {
        TaskInfo storage taskInfo = tasks[taskId];
        require(taskInfo.status == StatusTask.ACTIVE, "Task was done or cancel");
    ...
    function completeTask(
    ...
    ) external onlyAdmin validTaskId(taskId) validAddressArray(users) {
        TaskInfo storage taskInfo = tasks[taskId];
        ...
        IClaimPool(project.getClaimPoolOf(taskInfo.collection)).withdrawCollection(taskInfo.collection, project.rewardAddress(), spentBudget);
    ...
    function isValidTaskOf(address collection) public view returns (bool) {
        for (uint256 i = 0; i < _taskOfCollection[collection].length(); ++i) {
            if (tasks[_taskOfCollection[collection].at(i)].status == StatusTask.ACTIVE) {
                return true;
            }
        }
        return false;
