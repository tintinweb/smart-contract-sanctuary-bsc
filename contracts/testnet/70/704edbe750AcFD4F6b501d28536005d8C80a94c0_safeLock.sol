    ...
    */
    function lockTokens(address[] memory _tokenAddress, uint256[] memory _amount, uint256[] memory _unlockTime) public returns (uint256 _id) {
        for(uint i = 0; i < _tokenAddress.length; i++) {
            ...
            require(Token(_tokenAddress[i]).transferFrom(msg.sender, address(this), amount), 'Transfer tokens failed');
     ...
     * linear release
    */
    function release(address _tokenAddress, uint256 _amount) onlyOwner public {
     ...
     *withdraw tokens
    */
    function withdrawTokens(uint256 _id, uint256 _amount) nonReentrant public {
        ...
        require(Token(lockedToken[_id].tokenAddress).transfer(msg.sender, _amount), 'Transfer of tokens failed');
