 ...
 */
     ...
     */
    function purchaseBYSL(address givenToken, uint256 amount) external {
        require(amount > 0, "amount is greater than zero.");
        require(startTime <= block.timestamp, "purchase yet to start");
        if (
            startTime +
                (IEarlyAccessNFT(Admin.EarlyAccess()).top() * 1 hours) >=
            block.timestamp
        ) {
            require(
                (startTime +
                    ((IEarlyAccessNFT(Admin.EarlyAccess()).Won(msg.sender) -
                        1) * 1 hours) <=
                    block.timestamp) &&
                    (IEarlyAccessNFT(Admin.EarlyAccess()).Won(msg.sender) != 0),
        ...
        (uint256 BUSDAmount, uint256 leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution
                        (msg.sender,((amount * 1) / 10) - ((amount * 1) / 100),amount);
        if (BUSDAmount != 0) {
            console.log("entered");
        ...
        console.log(IBYSL(bYSL).protocolPrice(), "IBYSL(bYSL).protocolPrice()");
        uint256 ProtocolPricebYSL = ((((amount * 9) / 10) * (10**18)) /
            (IBYSL(bYSL).calculateProtocolPrice())); //Divide by 10**18 for testing
        console.log(ProtocolPricebYSL, "ProtocolPricebYSL");
        console.log(IBYSL(bYSL).protocolPrice(), "IBYSL(bYSL).protocolPrice()");
    ...
    function sellBYSLviaSWAP(address wantToken, uint256 amount) external {
        ...
        uint256 leftAmount;
        console.log(amount,"amount busd");
        (BUSDAmount, leftAmount) = IReferralNFT(Admin.Refferal())
            .rewardDistribution(
                msg.sender,
                ((BUSDAmount * 9) / 100),
                BUSDAmount
