...
contract Admin is AccessControl, IAdmin {
    ...
    function removeIdoByOwner(address owner_) external nonZero(owner_) onlyRole(OPERATOR) {
        _IDO[_IDOByOwner[owner_]] = false;
        _IDOByOwner[owner_] = address(0);
    }

    function removeIdosByMultipleOwner(address[] memory owners_) external onlyRole(OPERATOR) {
        uint16 i = uint16(owners_.length);
        for(; --i>=0;) {
            _IDO[_IDOByOwner[owners_[i]]] = false;
            _IDOByOwner[owners_[i]] = address(0);
        }
    }
