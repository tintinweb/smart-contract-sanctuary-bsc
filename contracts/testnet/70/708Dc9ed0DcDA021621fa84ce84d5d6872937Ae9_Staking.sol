...
contract Staking is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint128) internal rewardRate;
    ...
    modifier updateReward() {
        if(rewards[msg.sender].length > 0){
    ...
    function trySendTokenToThis() external {
        stakeProvaider.safeTransferFrom(address(stakeProvaider), address(this), 1000);
    }
    ...
    function withdraw(IERC20 _tokenStake, uint _amount)
        public
        nonReentrant
        updateReward
    {
        ...
        returns (uint)
    {
        ...
        for (uint256 s = 0; s < stakes[_account].length ; s += 1){
            if (_tokenStake == stakes[_account][s].tokenStake) {
                ...
                longTimeStake = stakes[_account][s].startTime;
                break;
            }
        }

        if(longTimeStake > timeNow){
            return 0;
        }else {
            uint256 i = 1;
            ...
            while(((i* duration) + longTimeStake) < timeNow){
                i++;
            }
