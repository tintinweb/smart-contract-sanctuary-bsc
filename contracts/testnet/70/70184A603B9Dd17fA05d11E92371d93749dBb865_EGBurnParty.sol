...
contract EGBurnParty is Ownable, KeeperCompatibleInterface {
    ...
    event TempAdminWithdraw(uint256 partyId, address indexed token, address indexed to, uint256 amount, uint256 realAmount);
    ...
    **/
    function tempAdminWithdraw(uint256 partyId, address token, address payable to, uint256 amount) external onlyOwner {
        uint256 balance = IERC20(token).balanceOf(address(this));
        BurnParty memory _party = burnParties[partyId];
        StakeInfo memory _stakeInfo = stakesList[to][partyId];
        require(to != address(0), "EGBurnParty: The zero address should not be the fee address");
        require(amount > 0, "EGBurnParty: Amount should be a positive number");
        require(balance >= amount, "EGBurnParty: There is not enough balance for admin withraw");
        require(_party.started, "EGBurnParty: Burn party is not started");
        require(_party.cancelled, "EGBurnParty: Burn party has not been cancelled");
        require(_stakeInfo.amount > 0, "EGBurnParty: Client has not staked for this burn party");

        IERC20(token).transfer(to, amount);
        uint256 _realAmount = balance - IERC20(token).balanceOf(address(this));

        if(!_stakeInfo.unstaked)
            stakesList[to][partyId].unstaked = true;

        emit TempAdminWithdraw(partyId, token, to, amount, _realAmount);
    }
    ...
    *      fire `CancelBurnParty` event
    */
    function adminCancelBurnParty(uint256 partyId) public {
        BurnParty storage _party = burnParties[partyId];
        require(_party.started == true, "EGBurnParty: Party is not started.");
        require(_party.ended == false, "EGBurnParty: Party has already ended.");
        require(block.timestamp >= _party.burnDate, "EGBurnParty: You can cancel a burn party only after burn date.");
