...
contract FarmPair is FarmHashrate, Initializable, Ownable {
    ...
    // userAddr -> token0 -> amount0 -> token1 -> amount1 -> hashrate
    event PairStaked(address, address, uint256, address, uint256, uint256);
    ...
    function stakePair(uint256 usdtAmount) public needInit {
        ...
        // check usdt balance & allowance
        uint256 usdtBalance = IERC20(usdtToken).balanceOf(_msgSender());
        uint256 usdtAllowance = IERC20(usdtToken).allowance(_msgSender(), address(this));
        require(usdtBalance >= usdtAmount && usdtAllowance >= usdtAmount, "USDT balance or allowance insufficient");
        ...
        uint256 usdtAmountInner = usdtAmount;

        {
            address quoteTokenInner = quoteToken;
            address usdtTokenInner = usdtToken;

            // 检查quote授权额度及余额
            uint256 quoteBalance = IERC20(quoteTokenInner).balanceOf(_msgSender());
            uint256 quoteAllowance = IERC20(quoteTokenInner).allowance(_msgSender(), address(this));
            require(quoteBalance >= quoteAmount && quoteAllowance >= quoteAmount, "Quote token balance or allowance insufficient");

            _burnTokenFrom(_msgSender(), quoteTokenInner, quoteAmount);

            // update staking amount
            uint256[2] storage amounts = pairPoolMap[quoteTokenInner][usdtTokenInner][_msgSender()];
            amounts[0] = amounts[0] + quoteAmount;
            amounts[1] = amounts[1] + usdtAmountInner;

            // update user hashrate
            pairHashrateMap[_msgSender()][quoteTokenInner][usdtTokenInner] = pairHashrateMap[_msgSender()][quoteTokenInner][usdtTokenInner] + hashrate;
        ...
        // to rebate
        {
            if (hc.rebate == 1 || hc.tokenRebate == 1) {
                address usdtTokenInner = usdtToken;
                for (uint8 gen = 1; gen <= appConf.getMaxGen(); gen++) {
                    ...
                    if (hc.tokenRebate == 1) {
                        ...
                        if (usdtCommission > 0) {
                            IERC20Ex(usdtTokenInner).safeTransfer(genUser.addr, usdtCommission);
                        }
                    }
        ...
        emit PairStaked(_msgSender(), quoteToken, quoteAmount, usdtToken, usdtAmountInner, hashrate);
    }
