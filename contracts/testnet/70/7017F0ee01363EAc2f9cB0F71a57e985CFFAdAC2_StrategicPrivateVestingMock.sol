...
contract StrategicPrivateVestingMock is KeeperCompatibleInterface, ReentrancyGuard {
  /*==================================================== State Variables ====================================================*/
  VestingMock vesting;
  ...
  function checkUpkeep(
    ...
    address[] memory _users = vesting.getUsers(4);
    bool outBreak = false;
    for (uint256 i = 0; i < _users.length; i++) {
      uint256[] memory _indexes = vesting.getInvestorIndexes(_users[i], 4);
      for (uint256 k = 0; k < _indexes.length; k++) {
        uint256 withdrawableAmount = vesting.withdrawableTokens(_users[i],vesting.intToEnum(4),_indexes[k]);
  ...
  function performUpkeep(bytes calldata performData) external override nonReentrant {
    ...
    vesting.withdrawTokensAutomatically(_user, vesting.intToEnum(4), _index);
  }
}
