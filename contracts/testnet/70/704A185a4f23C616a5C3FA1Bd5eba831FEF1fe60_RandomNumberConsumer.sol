...
contract RandomNumberConsumer is VRFConsumerBaseV2 {
  ...
  mapping(uint256 => uint256[]) private _arrayRandom;
  ...
  function getRandom(uint256 _lotteryId) public view returns(uint256[] memory) {
    return _arrayRandom[_lotteryId];
  }
  ...
  function setNumber() external {
    for (uint256 i = 0; i < 6; i++) {
      _arrayRandom[lotteryId].push(((_randomWords[i] % 20) + 1));
    }
  }
