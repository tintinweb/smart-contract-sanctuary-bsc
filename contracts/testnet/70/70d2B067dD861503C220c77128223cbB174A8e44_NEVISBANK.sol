...
contract NEVISBANK  is ERC20 {
  ...
  function swapAndLiquify(uint256 contractTokenBalance) private  {
    ...
    function withdraw() public {
        ...
        //totalAmount = totalAmount.sub(user.divident);
    ...
    function reinvest() public{
        ...
        uint256 fee = totalAmount.sub(totalAmount.mul(PROJECT_FEE).div(PERCENTS_DIVIDER));
     
        user.deposits.push(Deposit(0, fee, block.timestamp));

	    totalInvested = totalInvested.add(fee);
	
		emit NewDeposit(msg.sender, 0, fee);
    ...
    uint256 divident,uint256 withdrawnbonus ) {
		...
		uint256 withdrawn= user.withdrawn;

        uint256 divident = user.divident;
        uint256 withdrawnbonus = user.withdrawnbonus;
    return(checkpoint,referrer,bonus,totalBonus,withdrawn,divident,withdrawnbonus);
