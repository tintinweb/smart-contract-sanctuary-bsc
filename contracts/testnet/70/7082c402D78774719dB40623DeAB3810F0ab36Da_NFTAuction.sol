...
contract NFTAuction is Ownable, ReentrancyGuard {
    ...
    uint256 public settlePenalty= 5;
    ...
    modifier biddingPeriodMinimum(
        uint256 _auctionBidPeriod
            ...
            // change to 600
        require(_auctionBidPeriod> 0,"Minimum bidding beriod is 10 minutes");
        _;
    }
    ...
    function initialize(address _platformNormalNFT, address _platformLazyNFT, address _platformERC1155NFT ,address payable _adminAddress) public onlyOwner {
        require(setNFTContract==false, "NFT contract already set");
    ...
    function setPlatformPercentage(uint256 _platformPercentage) public onlyOwner{
    ...
    */
        ...
        increasePercentageAboveMinimum(_bidIncreasePercentage)
    {
        ...
        if(_nftContractAddress== address(0) && _tokenId==0){
            IClubVirtualERC721(platformNormalNFT).mint(address(this), 1, _uri); // mint to this contract
            uint[] memory tokens= IClubVirtualERC721(platformNormalNFT).tokensOwned(address(this));
            uint tokenId = tokens[tokens.length-1];
            Auction storage auction=  nftContractAuctions[platformNormalNFT][tokenId];
            nftContractAuctions[platformNormalNFT][tokenId].auctionBidPeriod = _auctionBidPeriod;
            auction.bidIncreasePercentage = _bidIncreasePercentage;
            nftRoyalty[platformNormalNFT][tokenId].royaltyOwner= msg.sender;
            nftRoyalty[platformNormalNFT][tokenId].royaltyPercentage= _royaltyPercentage;
            _setupAuction(
                platformNormalNFT,
                tokenId,
                _erc20Token,
    ...
    { 
        ...
        nftContractAuctions[_nftContractAddress][_tokenId].auctionEnd  = nftContractAuctions[_nftContractAddress][_tokenId].auctionBidPeriod.add(block.timestamp);  
        nftContractAuctions[_nftContractAddress][_tokenId].auctionStartTime    = _auctionStartTime;
    }
    ...
    function _createNewNftAuction(
        ...
        if(_nftContractAddress == platformNormalNFT || _nftContractAddress== platformLazyNFT || _nftContractAddress == platformERC1155NFT){
    ...
    function createBatchResale(
        ...
        external
        batchWithinLimits(_tokenId.length)
    {
    ...
    function createBatchSale(
        ...
        external
        batchWithinLimits(_batchTokenPrice.length)
    {
        for(uint i=0; i< _batchTokenPrice.length; i++){
            require(_batchTokenPrice[i]>0, "price cannot be 0 or less");
            IClubVirtualERC721(platformNormalNFT).mint(address(this), 1, _uri[i]); // mint to this contract
            // last token minted
            uint[] memory tokens= IClubVirtualERC721(platformNormalNFT).tokensOwned(address(this));
            uint _tokenId = tokens[tokens.length-1];
    ...
    {   
        ...
        if(_isERC20Auction(erc20Token)){    // if sale is ERC20
            ...
            uint256 platformFees= (totalAmount.mul(platformPercentage)).div(10000);
            ...
            uint sellerAmount= totalAmount;
            address owner= owner();
            IERC20(erc20Token).transferFrom(msg.sender, adminAddress, platformFees);
            ...
            adminAddress.transfer(platformFees);
    ...
    function acceptBuyProposal(
        ...
        nftContractSale[_nftContractAddress][_tokenId].buyNowPrice= _proposedPrice;
        buyNFT(_nftContractAddress, _tokenId, _ownerPercentage);
    }
        ...
        auctionOngoing(_nftContractAddress, _tokenId)
    {
        ...
        require(nftContractAuctions[_nftContractAddress][_tokenId].auctionEnd != 0, "Auction Invalid");
...
*/
    ...
    function _purchaseAndTransfer(
        ...
        if(_isERC20Auction(erc20Token)){    // if sale is ERC20
            ...
            IERC20(erc20Token).transferFrom(msg.sender, adminAddress, platformFees);
            // IERC20(erc20Token).transferFrom(msg.sender, owner, ownerAmount);
            IERC20(erc20Token).transferFrom(msg.sender, _nftSeller, sellerAmount);
        }
        else{
            uint totalAmount= msg.value;
            // 1. Cut platform fees from the buyer
            ...
            // payable(owner).transfer(ownerAmount);
            adminAddress.transfer(platformFees);
            (bool success, ) = payable(_nftSeller).call{value: sellerAmount}("");
            // if it failed, update their credit balance so they can pull it later
            if (!success) {
                failedTransferCredits[_nftSeller] =
                    failedTransferCredits[_nftSeller].add(sellerAmount);
            }
        }
    }
    /*
