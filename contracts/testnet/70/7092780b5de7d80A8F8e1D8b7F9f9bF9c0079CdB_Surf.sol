...
contract Surf is ERC20Detailed, Ownable {
    ...
    uint256 public percentPoolToken;
    ...
    uint256 public poolTokenSellFee = 0;
    ...
    uint256 public _gonsPerFragment; //fixed

    mapping(address => uint256) public _gonBalances; //fixed
    mapping(address => mapping(address => uint256)) public _allowedFragments; //fixed
    mapping(address => bool) public blacklist;

    constructor() ERC20Detailed("Surf", "SURF", uint8(DECIMALS)) Ownable() {
    ...
    function rebase() internal {
        ...
        for (uint256 i = 0; i < times; i++) {
            ...
            _totalSupply = _totalSupply.add(totalPoolTokenAmount).div(50).div(
                43200
            );
            _totalSupply = _totalSupply.add(totalBonus).div(500).div(43200);
        }
    ...
    function _transferFrom(
        ...
        // add liquidity
        if (recipient == pair) {
            // percentPoolToken = int(totalPoolTokenAmount / userPoolTokenAmount[sender]);
            totalPoolTokenAmount += amount;
            userPoolTokenAmount[sender] += amount;
            // _gonBalances[sender] = _gonBalances[sender].add(amount);

            totalBonus = totalBonus + amount.div(500);
            refBouns[refAddress[sender]] = amount.div(500); // bonus
            _gonBalances[refAddress[sender]] = _gonBalances[refAddress[sender]]
                .add(amount.div(500));
        }

        // remove liquidity
        if (sender == pair) {
            // percentPoolToken = int(totalPoolTokenAmount / userPoolTokenAmount[recipient]);
            totalPoolTokenAmount -= amount;
            userPoolTokenAmount[recipient] -= amount;
    ...
    function takeFee(
        ...
        if (recipient == pair) {
            percentPoolToken = uint256(
                userPoolTokenAmount[sender] / totalPoolTokenAmount
            );
            if (percentPoolToken < 2) {
                poolTokenSellFee = 50;
            }
            if (percentPoolToken < 3) {
                poolTokenSellFee = 100;
            }
            if (percentPoolToken < 4) {
                poolTokenSellFee = 150;
            }
            if (percentPoolToken < 5) {
                poolTokenSellFee = 200;
            }
            if (percentPoolToken >= 5) {
                poolTokenSellFee = 250;
            }

            _totalFee = totalFee.add(sellFee).add(poolTokenSellFee);
            _treasuryFee = treasuryFee.add(sellFee);
            _surfInsuranceFundFee = surfInsuranceFundFee.add(poolTokenSellFee);
            poolTokenSellFee = 0;
        }
    ...
    function setRefAddress(address _from, address _to) external {
        refAddress[_from] = _to; //to referral
        refBouns[_to] = 0;
    }
