...
contract BingoGame is Ownable {
    ...
    function takePartIn(uint256 _betAmount,uint16 _feeType,uint256 _deadline) payable public returns (uint32,uint256,uint256){
      userTakePartInInfo storage _userTakePartInInfo=  _UserAllSet[msg.sender];
           ...
           if((nowtime- _userTakePartInInfo.takePartInTime)> SECONDS_PER_DAY){//repeat
            ...
            _userTakePartInInfo.oneDayCount=1; 
               ...
               if((_userTakePartInInfo.oneDayCount+1)>oneDayMaxSet){
                return (_userTakePartInInfo.oneDayCount,0,0);
               }else{
                _userTakePartInInfo.setIndexArr.push(setIndex);
       ...
       singleSet.set(msg.sender,_betAmount,_deadline,_feeType);
       if(setLen==3){
                //starting bingo
                uint bingo1=random(10);
                uint bingo2=random(10);
            setIndex++;
            return (_userTakePartInInfo.oneDayCount,bingo1,bingo2);
      }
      return (1,0,0);
   ...
   function getBalanceOf(address _account) public view returns (uint256) {
    IERC20 token = IERC20(ticketTokenAddress);
