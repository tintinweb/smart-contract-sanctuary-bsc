...
contract NFTBoxShop is NFTHelp {
    ...
    function sellBox(uint16 _num, uint256 _price, address _coinType) external lock haveBoxAddress haveMainPool returns(uint256 _groupID) {
    ...
    function tradeBox(uint64 _GroupID) external lock haveBoxAddress returns (uint256 _num) {
    ...
    function unSell(uint64 _GroupID) external lock haveBoxAddress {
        // (uint256[] memory Time,) = YiBoxBase.GetParam(YiBoxType.ShopTime,makeAParam(_GroupID));
        // require(block.timestamp > Time[0] && Time[0] != 0,"not in time");
        (,address[] memory  Seller) = YiBoxBase.GetParam(YiBoxType.ShopSeller,makeAParam(_GroupID));
