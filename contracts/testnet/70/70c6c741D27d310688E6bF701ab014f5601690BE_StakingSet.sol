...
contract StakingSet is StakingSetStorage {
    ...
    function initialize(
        ...
        require(IERC20Upgradeable(_nbuToken).approve(_nimbusRouter, type(uint256).max), "ERC20: Approve failed");
        require(IERC20Upgradeable(_gnbuToken).approve(_nimbusRouter, type(uint256).max), "ERC20: Approve failed");
        require(IERC20Upgradeable(_nbuToken).approve(_NbuStaking, type(uint256).max), "ERC20: Approve failed");
        require(IERC20Upgradeable(_gnbuToken).approve(_GnbuStaking, type(uint256).max), "ERC20: Approve failed");

        require(IERC20Upgradeable(_lpBnbCake).approve(_CakeStaking, type(uint256).max), "ERC20: Approve failed");
        require(IERC20Upgradeable(_lpBnbCake).approve(_pancakeRouter, type(uint256).max), "ERC20: Approve failed");
        require(IERC20Upgradeable(CakeStaking.CAKE()).approve(_pancakeRouter, type(uint256).max), "ERC20: Approve failed");
    ...
    function rescue(address to, address tokenAddress, uint256 amount) external onlyOwner {
        ...
        require(IERC20Upgradeable(tokenAddress).transfer(to, amount), "ERC20: TRANSFER_FAILED");
    ...
    function updateTokenAllowance(address token, address spender, int amount) external onlyOwner {
        ...
        require(IERC20Upgradeable(token).approve(spender, allowance), "ERC20: Approve failed");
    }
