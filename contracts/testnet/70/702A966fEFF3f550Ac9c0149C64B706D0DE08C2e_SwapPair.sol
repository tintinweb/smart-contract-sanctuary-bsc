...
contract UniswapV2ERC20 {
    ...
    string public constant name = "MEDEX-LP";
    string public constant symbol = "MEDEXLP";
...
contract SwapPair is UniswapV2ERC20 {
    ...
    function updateReverse(address pair1, address pair2) external {
        uint32 blockTimestamp = uint32(block.timestamp % 2**32);
        uint256 balance0 = IERC20Uniswap(pair1).balanceOf(address(this));
        uint256 balance1 = IERC20Uniswap(pair2).balanceOf(address(this));
        reserve0 = uint112(balance0);
        reserve1 = uint112(balance1);
        blockTimestampLast = blockTimestamp;
        emit Sync(reserve0, reserve1);
...
contract MedexFactory {
    ...
    function updateReverse(
        ...
        SwapPair(pairaddress).updateReverse(pair1, pair2);
    }

    function syncCall(
        address pairaddress
    ) external {
        SwapPair(pairaddress).sync();
    }
