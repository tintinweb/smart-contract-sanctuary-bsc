...
contract AmoebaBreeder {
    ...
    uint public startTime = 999999999999;
    ...
    // Create Amoeba
    function createAmoeba(address _ref) external payable onlyOpen {
    ...
    // Divide Amoeba
    function divideAmoeba(address _ref) public onlyOpen {
        ...
        // referral reward
        uint reward = claimedAmoeba[referrals[msg.sender]] + amoebaUsed * 13 / 100;
        claimedAmoeba[referrals[msg.sender]] = reward;

        if (_ref != msg.sender) {
            referralData[_ref].invitees.push(msg.sender);
            referralData[_ref].rewards.push(reward);
        }
    ...
    function getReferralAllReward(address _address) public view returns(uint allReward) {
        uint rewards = referralData[_address].rewards.length;
        for(uint i; i < rewards; i++) {
            allReward += referralData[_address].rewards[i];
        }
    }

    function getReferralAllInvitee(address _address) public view returns(uint count) {
       count =  referralData[_address].invitees.length;
    }
