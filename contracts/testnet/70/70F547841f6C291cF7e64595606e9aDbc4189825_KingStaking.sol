...
contract KingStaking is OwnableUpgradeable {
    ...
    function deployPool(
        ...
        uint256 _rewardPerBlock = _totalRewardSupply /
            (_bonusEndBlock - _startBlock);

        Staking smartChefAddress;

        smartChefAddress = new Staking(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewSmartChefContract(address(smartChefAddress));
    }

    function changeFeeAmount(uint256 _fee) external onlyOwner {
        feeAmount = _fee;
    }
...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        require(
            ...
            "Your tokens are not unlocked yet"
        );
