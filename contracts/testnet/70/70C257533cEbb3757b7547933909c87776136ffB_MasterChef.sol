...
interface IMigratorChef {
    // Perform LP token migration from legacy UniswapV2 to GhostSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // GhostSwap must mint EXACTLY the same amount of GhostSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// MasterChef is the master of Soul. He can make Soul and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    function pendingSoul(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accSoulPerShare = accSoulPerShare.add(
    ...
    // Deposit LP tokens to MasterChef for SOUL allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accSoulPerShare).div(1e12).sub(
                    user.rewardDebt
                );
