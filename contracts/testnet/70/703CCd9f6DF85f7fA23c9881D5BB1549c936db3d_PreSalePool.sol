...
contract PreSalePool is Initializable, Whitelist, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function deployNewSalePhase(
        ...
        SalePhase storage newSalePhase = salePhaseStatistics[totalSalePhase + 1];
        ...
        totalSalePhase++;
        currentSalePhase = totalSalePhase;
    ...
    function canBuyMoreToken(address _candidate, uint256 _BUSDAmount) private view returns (bool) {
        ...
        if (totalBUSDUserSpent + _BUSDAmount > maxBUSDUserCanSpend) return false;
    ...
    {
        require(1 <= _salePhase && _salePhase <= totalSalePhase, "POOL: INVALID SALE PHASE");
        SalePhase storage salePhase = salePhaseStatistics[_salePhase];
        startTime = salePhase.startTime;
        endTime = salePhase.endTime;
        claimTime = salePhase.claimTime;
        claimRate = salePhase.claimRate;
        isNoBuyLimit = salePhase.isNoBuyLimit;
        maxBUSDUserCanSpend = salePhase.maxBUSDUserCanSpend;
        ...
        treasuryAddress = salePhase.treasuryAddress;
    }
}
