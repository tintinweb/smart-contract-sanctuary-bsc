...
contract NFTComposite is ERC721, INFTComposite, Caller, Fallback {
    ...
    function batchMintNFT(
        ...
        require(0 != propertyScore.length, "");
        PropertyScore memory propertyScore_;
        for (uint256 i = 0; i < propertyScore.length; i++) {
            propertyScore_ = propertyScore[i];
            property[startTokenId] = propertyScore_.property;
            score[startTokenId] = propertyScore_.score;
            _safeMint(to, startTokenId);
            startTokenId = startTokenId.add(1, "");
        }
        maxId = startTokenId;
        if (address(0) != address(token)) {
            token.mint(to, propertyScore.length);
        }
    }

    function getCurrentMaxId() external view override returns (uint256) {
        return maxId;
    }
    ...
    {
        return (property[tokenId], score[tokenId]);
    }

    // 销毁NFT
    function _destroyNFT(address from, uint256 tokenId) internal {
        require(address(0) != ownerOf(tokenId), "");
        transferFrom(from, address(0), tokenId);
        delete property[tokenId];
        delete score[tokenId];
    }

    // 拆解
    function splitNFT(INFTElement elementToken, uint256 tokenId) external {
        require(msg.sender == ownerOf(tokenId), "");
        INFTElement.ElementScore[] memory elementScore_;

        uint64[] memory property_ = property[tokenId];
        uint32[] memory score_ = score[tokenId];

        for (uint128 i = 0; i < property_.length; i++) {
            elementScore_[i].property = property_[i];
            elementScore_[i].score = score_[i];
        }
        elementToken.batchMintNFT(msg.sender, elementScore_);

        _destroyNFT(msg.sender, tokenId);
    }
}
