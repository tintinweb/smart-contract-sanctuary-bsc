...
// Root file: contracts/mocks/MetaPool.sol
...
// Mock Curve Stablepool, support token with 18 decimals
contract MockCurveMetaPool is ERC20 {
    using SafeMath for uint256;

    address public token0;
    address public token1;

    constructor(address _token0, address _token1) ERC20("MetaPool", "META") {
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function removeLiquidity(address token, uint256 amountLp) external {
        require(token == token0 || token == token1, "unsupported token");

        IERC20(token).transfer(msg.sender, amountLp);
