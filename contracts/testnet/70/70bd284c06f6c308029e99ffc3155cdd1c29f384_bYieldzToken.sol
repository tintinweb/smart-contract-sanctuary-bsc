...
contract bYieldzToken is ERC20, Ownable {
    ...
    */
    ...
    function getPreWithdrawPercent(address staker, uint256 _count) public view returns(uint256) {
        ...
        uint256 _period;
        if (data.rate == rateForTwo) _period = 10;
        else _period = 20; 
        uint256 _preWithdrawPercent = delta / 1 days * 250 / _period + 50;
    ...
    */
    function claim(uint256 count) external {
        ...
        if (block.timestamp >= data.stakeTime) data.stakeTime = data.lockTime;
