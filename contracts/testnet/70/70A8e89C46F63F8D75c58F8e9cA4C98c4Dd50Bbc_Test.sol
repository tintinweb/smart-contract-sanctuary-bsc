...
// Main Contract
contract Test is IERC20Metadata, Ownable {
   ...
   uint256 public maxWalletSize = 50;
   uint256 public maxTxPercent = 25;
   ...
   uint256 public liqTaxS = 800;
   uint256 public liqTaxB = 400;
   ...
   // Swap
   uint256 public treasurySwapAt  = 388800000000;
   uint256 public liqSwapAt  = 377700000000;
   ...
   function changeLiqOnSell(uint256 newLiqTax) public virtual onlyOwner {
       require (newLiqTax <= 800, "Liquidity tax needs to be 8 and below");
       liqTaxS = newLiqTax;
   }
   function changeLiqOnBuy(uint256 newLiqTax) public virtual onlyOwner {
       require (newLiqTax <= 400, "Liquidity tax needs to be 4 and below");
       liqTaxB = newLiqTax;
   }
   function changeBurnOnSell(uint256 newBurnTax) public virtual onlyOwner {
       require (newBurnTax <= 500,"Burn tax needs to be 5 and below");
       burnTaxS = newBurnTax;
   }
   ...
   function _transfer(address sender, address recipient, uint256 amount) internal virtual {
       ...
       if (doTaxes){
           ...
           // BURN
           if (burnTax > 0){
               uint256 burnAmount = amount.div(taxDenominator).mul(burnTax);
           ...
           // TREASURY TAX
           if (treasuryTax > 0){
               uint256 taxAmount = amount.div(taxDenominator).mul(treasuryTax);
             
               _walletBalance[address(this)] = _walletBalance[address(this)].add(taxAmount);
               treasuryCount = treasuryCount.add(taxAmount);
