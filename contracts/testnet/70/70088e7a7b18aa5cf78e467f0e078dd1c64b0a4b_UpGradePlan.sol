...
contract UpGradePlan is Context {
    ...
    address public BUSDaddress = 0xC88887bCa276Af4D577a54f4F5376875d628c4a7 ;
    modifier onlyOwner() {
    ...
    event UserRegister(address user,uint256 amount,address referaddress,string Position);

    function Register(address reffer,string memory position) public returns(bool){
        ...
        emit UserRegister(_msgSender(),upgradeFee,reffer,position);
        return true;
    }
    function UpgradePlan() public returns(bool){
        ...
        IERC20(BUSDaddress).transfer(contarctaddress,upgradeFee);
        emit UserRegister(_msgSender(),upgradeFee,address(0x0),"x");
        return true;
    }
