...
contract CustomTreasury is Initializable, Policy {
    ...
    event BillContractToggled(address billContract, bool approved);
    event Withdraw(address token, address destination, uint256 amount);

    /* ======== CONSTRUCTOR ======== */

    function initialize(address _payoutToken, address _initialOwner, address _payoutAddress) public initializer {
        require(_payoutToken != address(0), "CustomTreasury: Payout token cannot address zero");
        payoutToken = _payoutToken;
        require(_initialOwner != address(0), "CustomTreasury: Payout token cannot address zero");
        _policy = _initialOwner;
        require(_payoutAddress != address(0), "CustomTreasury: Receiver address cannot address zero");
    ...
    function deposit(
        ...
        require(billContract[msg.sender], "msg.sender is not a bond contract");
    ...
    /**
     *  @notice bill contract receives payout tokens
     *  @param _amountPayoutToken uint
     */
    function sendPayoutTokens(uint _amountPayoutToken) external {
        require(billContract[msg.sender], "msg.sender is not a bill contract");
        IERC20(payoutToken).safeTransfer(msg.sender, _amountPayoutToken);
    }
