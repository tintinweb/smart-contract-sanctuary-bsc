...
contract Steven is Context, IBEP20, Ownable {
    ...
    // buy tax fee
    uint256 public redistributionFeeOnBuying = 60; // 2% will be distributed among holder as token divideneds
    ...
    // sell tax fee
    uint256 public redistributionFeeOnSelling = 60; // 1% will be distributed among holder as token divideneds
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns (uint256) {
        uint256 percentage = (tAmount *
            (_currentRedistributionFee +
                ...
                (_currentwheelWalletFee))) / (1e3);
        //  +
        // (_currentinstantRewardwalletFee)
        return percentage;
    }
        ...
        onlyOwner
    {
        ...
        require(
            redistributionFee + (liquidityFee) + (wheelWalletFee) <=
                //  +
    ...
    function getburnableamount() public view returns (uint256) {
        ...
        if (burnAmount > _tOwned[msg.sender]) {
            burnAmount = _tOwned[msg.sender];
        }
