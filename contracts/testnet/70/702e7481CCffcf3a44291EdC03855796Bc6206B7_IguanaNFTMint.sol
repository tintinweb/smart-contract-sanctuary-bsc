...
contract IguanaNFTMint is Ownable, ERC721Enumerable {
    using Address for address;

    // @notice total minted NFT number
    uint256 public totalMintedNumber;
    ...
    event mintIguana(uint256 indexed nftId, address indexed owner);

    /**
     * @notice Constructor inherits IguanaNFTMint
     * @param _baseURIParam ERC721 token baseURI
     */
    constructor(string memory _baseURIParam) ERC721("IGUANA", "IGUANA") {
        totalMintedNumber = 0;
        mintableNFTNumber = 250;
     ...
     * @dev Mint Iguana.
     */
    function MintIguanaNft() external payable {
        require(msg.value >= nftPrice, "Not enough BNB");
        ...
        uint256 id = _randomness % (mintableNFTNumber - totalMintedNumber);
        uint256 curTokenId = 0;

        for (uint256 i = 0; i < mintableNFTNumber; i++) {
            if (curTokenId == id) {
                _mint(msg.sender, i);
                emit mintIguana(i, msg.sender);
     ...
     * @dev Mint Iguana.
     */
    function MintIguanaNftByOwner(address _buyer) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            ...
            require(
                msg.sender == tradeCenterAddress,
                "Can transfer with only trade center"
            );
            super._beforeTokenTransfer(from, to, tokenId);
        }
    }
