...
contract HackedGameV1 is HackedGameBase, StakingTierProvider, VRFConsumerBaseV2, Recoverable, Pausable {
    ...
    function drawRound(uint256 tokenId) external whenNotPaused {
        ...
        require(tokenId == 0 
            || IERC721(hackedStaked).ownerOf(tokenId) == msg.sender 
            || IERC721(hackedStaked).ownerOf(tokenId) == msg.sender,
            "Not owner of token");
    ...
    function completeGame() external whenNotPaused {
        ...
        address winner = IERC721(hackedStaked).ownerOf(tokenId);
        if (winner == stakingContract) winner = IERC721(hackedStaked).ownerOf(tokenId);
    ...
    function _getFees(address owner, uint256 tokenId, uint256 tier) internal view returns (uint256) {
        if (IERC721(hackedStaked).ownerOf(tokenId) == owner) {
            return tier == 4 ? feeReenterStakedTier4 : feeReenterStaked;
        } else if (IERC721(hackedStaked).ownerOf(tokenId) == owner) {
