...
contract ValidVestingVault is Authorization, ERC721Enumerable {
    ...
    uint256 public merkleIdCount;
    uint256 public vestingIdCount;
    uint256 public nftIdCount;
    address internal newProfileOwner;
    mapping(uint256 => uint256) public vestingClaimedAmount; //vestingClaimedAmount[vestingId] = claimedAmount
    mapping(uint256 => MerkleInfo) public merkleInfo; //merkleInfo[merkleId] = MerkleInfo
    mapping(address => mapping(uint256 => bool)) public isVestingIdVerified; //isVestingIdVerified[recipient][vestingId] = bool
    mapping(address => mapping(uint256 => bool)) public isMerkleIdVerified; //isMerkleIdVerified[recipient][merkleId] = bool
    mapping(uint256 => uint256[]) public campaignMerkleIds; //campaignMerkleIds[campaignId] = uint256[]
    mapping(uint256 => uint256[]) public campaignDirectLockIds; //campaignDirectLockIds[campaignId] = uint256[]
    ...
    event CreateMerkleRoot(uint256 campaignId, uint256 merkleId, bytes32 merkleRoot, string dataUri);
    ...
    constructor(
        ...
        newProfileOwner = _newOwner;
    }  

    function initProfileOwner() external {
        require(address(newProfileOwner) != address(0), "null address");
        owner = newProfileOwner;
        newProfileOwner = address(0);
    }
    ...
    function campaignMerkleIdsLength(uint256 campaignId) external view returns (uint256 length) {
        length = campaignMerkleIds[campaignId].length;
    } 

    function campaignDirectLockIdsLength(uint256 campaignId) external view returns (uint256 length) {
        length = campaignDirectLockIds[campaignId].length;
    } 
    ...
    function merkleLock(
        ...
        bytes32 merkleRoot,
        string calldata dataUri
    ) external auth returns (uint256 merkleId) {
        merkleId = merkleIdCount;
        merkleIdCount += 1;
        merkleInfo[merkleId] = MerkleInfo({
            dataUri: dataUri,
            root: merkleRoot
        });
        campaignMerkleIds[campaignId].push(merkleId);
        emit CreateMerkleRoot(campaignId, merkleId, merkleRoot, dataUri);
    ...
    function directLock(
        ...
        uint256 startDate, 
        uint256 endDate
    ) external auth returns (uint256 vestingId) {
        IERC20 token = campaignInfo[campaignId].token;
        vestingId = writeVestingData(campaignId, recipient, amount, startDate, endDate); 
        campaignDirectLockIds[campaignId].push(vestingId);
        amount = _transferAssetFrom(token, msg.sender, amount);
    }
    ...
    function verifyMerkleLock(
        ...
        require(
            MerkleProof.verify(proof, merkleInfo[merkleId].root, keccak256(
                abi.encodePacked(
                    msg.sender, 
                    campaignId, 
                    amount, 
        ...
        , "merkle proof failed");
        require(!isMerkleIdVerified[msg.sender][merkleId], "merkleId already verified");
        uint256 vestingId = writeVestingData(campaignId, msg.sender, amount, startDate, endDate);
        nftId = nftIdCount;
        nftIdCount += 1; 
        nftVestingId[nftId] = vestingId;
        isVestingIdVerified[msg.sender][vestingId] = true;
        isMerkleIdVerified[msg.sender][merkleId] = true;
    ...
    function verifyDirectLock(
        ...
        VestingInfo memory info = vestingInfo[vestingId];
        require(!isVestingIdVerified[msg.sender][vestingId], "vestingId already verified");
...
contract ValidVestingVaultFactory {
    ...
    function newProfile(address[] calldata admins) external returns (uint256 profileId) {
        ...
        ValidVestingVault vestingVault = new ValidVestingVault(msg.sender, "Valid Vesting Vault", "Valid-Vesting-Vault");
        for (uint i = 0; i < admins.length; i++) {
            vestingVault.permit(admins[i]);
        }
        vestingVault.initProfileOwner();
