...
contract GymVaultsBank is ReentrancyGuardUpgradeable, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function claimAndDepositSinglePool(
        ...
        if (pending > 0) {
            ...
            uint256 _poolsRewards = (pending * POOLS_REWARD) / 100;
            ...
            IGymSinglePool(singlePoolAddress).depositFromOtherContract(
                (pending - distributeRewardTokenAmt - _poolsRewards),
                _periodId,
                _isUnlocked,
                msg.sender
            );
    ...
    function _withdraw(uint256 _pid) private {
        ...
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        uint256 deposit = userInvestment[_pid][msg.sender];
        require(deposit > 0, "GymVaultsBank: user amount is 0");

        _claim(_pid, msg.sender);

        IStrategy(poolInfo[_pid].strategy).withdraw(msg.sender, deposit);
        IERC20Upgradeable(address(pool.want)).safeTransfer(msg.sender, deposit);
        user.rewardDebt = (user.shares * (pool.accRewardPerShare)) / (1e18);
        user.dollarValue = 0;

        emit Withdraw(msg.sender, _pid, deposit);
    }
