...
contract MintHandler is Upgradable {
    ...
    /**
     * @dev Handle logic of mint function
     * @param _data (0) saleOrderId, (1) price, (2) NFT id
    ...
    function mintNFT(
    ...
    ) public payable virtual notBlocked reentrancyGuard {
        require(
            !soldSaleOrder[_signatures[0]],
            "MintHandler: Sale order has been sold"
        );

        require(!isAdmin(msg.sender), "MintHandler: You cannot buy NFT if you are admin");
        require(_data[2] > 0, "MintHandler: NFT ID must be greater than 0");
        require(_data[1] > 0, "MintHandler: Price must be greater than 0");

        require(signer == _addr[1], "MintHandler: Only signer signature");

        require(
            !invalidSaleOrder[_signatures[0]],
            "MintHandler: Sale order has been canceled"
        );
        ...
        require(
            SignatureUtils(signatureUtilsAddress).verifySaleOrder(saleOrder, _signatures[0], _addr[1]),
            "MintHandler: Verify buy request failed"
        );
        
        if (msg.value == 0) {
            require(
                IERC20(_addr[2]).transferFrom(msg.sender, superAdmin, _data[1]),
            ...
            require(msg.value == _data[1], "MintHandler: Sent value is not equal to the total price wanted to be transferred");
            payable(superAdmin).transfer(_data[1]);
        }

        uint256 tokenId = mint721(msg.sender, _data[2]);
        soldSaleOrder[_signatures[0]] = true;
...
import {SaleOrder, BundleSaleOrder} from "../common/Structs.sol";
...
import {BundleSaleOrder, SaleOrder} from "../common/Structs.sol";
