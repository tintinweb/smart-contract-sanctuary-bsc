...
contract PYESwap_Base is IPYE, Context, Ownable {
    ...
    function buyAndLiquify() internal {
        ...
        buyTokens(amountToSwap, address(this));
    ...
    /**    address[] memory path = new address[](2);
        ...
        uint256 amountTokenToAdd = balanceOf(address(this)).sub(initialTokenBalance);
        addLiquidity(amountTokenToAdd, amountWBNBtoAdd);

    /**    IERC20(address(this)).approve(address(pyeSwapRouter), amountTokenToAdd);
    ...
    function addLiquidity(uint256 amountTokenToAdd, uint256 amountWBNBtoAdd) internal {
        IERC20(address(this)).approve(address(pyeSwapRouter), amountTokenToAdd);
        IERC20(WBNB).approve(address(pyeSwapRouter), amountWBNBtoAdd);
