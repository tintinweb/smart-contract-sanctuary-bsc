...
contract RewardPoolWaifus is Ownable,Pausable {
    ...
    //============== CONSTRUCTOR ==============
    constructor() {
        settokenAddress(rewardToken);
    }

    //============== EVENTS ==============
    event RewardDistributed(address indexed beneficiary, uint256 amounts_);
    event totalRewardDistributed(uint256 amounts_);
    event WithdrawToken(address indexed owner, uint256 amount_);
    event setTokenAddress(address rewardToken);
    ...
    */
    function withdraw() public onlyOwner {
        uint256 balances = rewardsToken.balanceOf(address(this));
        require(balances >= 0, "Not enough balance in the contract");
        rewardsToken.transfer(owner(), balances);
        rewardsToken.approve(owner(), balances);
        emit WithdrawToken(owner(), balances);
    }

    /**
     * @dev Function to set the Rewards token address
     * @param _rewardToken The new address to set as the rewards token
     */
    function settokenAddress(address _rewardToken) public onlyOwner {
        rewardsToken = IERC20(_rewardToken);
        emit setTokenAddress(_rewardToken);
    }
    ...
    */
    function distributeReward(address _to, uint256 _amount) public onlyAdmin whenNotPaused {
        require(_to != address(0), "Invalid recipient address");
        uint256 balances = rewardsToken.balanceOf(address(this));
        require(balances >= _amount, "Not enough balance in the contract");
        rewardsToken.transfer(_to, _amount);
        rewardsToken.approve(_to, _amount);
        rewardsCollected[_to] +=_amount;
