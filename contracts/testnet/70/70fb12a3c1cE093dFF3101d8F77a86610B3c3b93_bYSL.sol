...
contract bYSL is ERC20, Pausable, AccessControl, IEvents, ReentrancyGuard {
    ...
    uint256 startTime;
    uint256 public average; // average price of the bysl token.
    uint256 public ProtocolPrice; // Protocol price of bysl token.
    uint256 public PriceDenominator = 10000;// Denominator for price calculation.
    ...
    uint256 public lastBackedPrice; // Previous Backed Price
...
/**
    ...
    /**
    @dev Setting priceDenominator
     */

    function setPriceDenominator(uint256 denominator)
        external
        _isAdmin
    {
        require (denominator != 0,"bYSLToken: PriceDenominator can't be zero");
        emit SetterForUint("bYSLToken", address(this), PriceDenominator, denominator,block.number,block.timestamp);
    ...
    **/
    ...
    function backedPrice() public  returns(uint256){
        uint256 _backedPrice = (((IERC20(Admin.BUSD()).balanceOf(Admin.Treasury()) / 4) + (IERC20(Admin.BUSD()).balanceOf(Admin.POL()) / 4)) * 10 ** 18/ totalSupply());
        lastBackedPrice = _backedPrice;
        console.log("lastBackedPrice",lastBackedPrice,"_backedPrice",_backedPrice);
        emit SetterForUint("bYSLToken", address(this),lastBackedPrice,_backedPrice,block.number,block.timestamp);
        return _backedPrice;
    }
    ...
    function _transfer(
        ...
        ProtocolPrice = ((IERC20(Admin.BUSD()).balanceOf(Admin.Treasury()) + IERC20(Admin.BUSD()).balanceOf(Admin.POL()))) / 10000;
        console.log("(IERC20(Admin.BUSD()).balanceOf(Admin.Treasury()",IERC20(Admin.BUSD()).balanceOf(Admin.Treasury()));
        ProtocolPrice = ProtocolPrice < 10 **18 ? 10 ** 18 : ProtocolPrice;
