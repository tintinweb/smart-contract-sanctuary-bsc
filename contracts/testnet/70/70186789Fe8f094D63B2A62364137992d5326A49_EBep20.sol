 ...
 * @author Evry.Finance
 */
contract EBep20 is EToken, EBep20Interface, EBep20Storage {
    ...
    function init(address underlying_,
         ...
         address whiteListDepositAccount_,
         address admin_) external {

        require(msg.sender == admin, "only1 admin may initialize the market");
    ...
    // function _setIsEProxyStrorage(bool _allow) public {
        
    //     isEProxyStrorage = _allow;
    // }
}
 ...
 * @author Evry.Finance
 */
abstract contract EToken is ETokenInterfaceStorage, ETokenInterface, Exponential, TokenErrorReporter {
    ...
    function mintInternal(uint mintAmount, bool activateCollateral) internal nonReentrant returns (uint, uint) {
        if (!depositAllowed) {
            return (fail(Error.UNAUTHORIZED, FailureInfo.MINT_IS_NOT_ALLOWED), 0);
        }

        if (onlyWhiteListAllowToDeposit && !depositWhiteListAccounts[msg.sender]) {
            return (fail(Error.UNAUTHORIZED, FailureInfo.MINT_IS_NOT_ALLOWED), 0);
        }
    ...
    //     if (onlyWhiteListAllowToDeposit && !depositWhiteListAccounts[msg.sender] && !depositWhiteListAccounts[receiver]) {
    //         return (fail(Error.UNAUTHORIZED, FailureInfo.MINT_IS_NOT_ALLOWED), 0);
    //     }
    ...
    //     if (!borrowAllowed) {
    //         return fail(Error.UNAUTHORIZED, FailureInfo.BORROW_IS_NOT_ALLOWED);
    //     }
...
abstract contract ETokenInterface {
...
abstract contract EBep20Interface {
