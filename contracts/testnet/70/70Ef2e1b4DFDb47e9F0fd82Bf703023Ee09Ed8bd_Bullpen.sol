...
// File: contracts/interfaces/IGenesis.sol
...
contract Bullpen is Ownable {
    ...
    function bullCount() public view returns (uint16) {
        return uint16(GenesisInterface.walletOfOwner(address(this)).length);
    }
    ...
    // returns Bull ids in this contract
    function viewBullpen() public view returns (uint256[] memory) {
        return GenesisInterface.walletOfOwner(address(this));
    }
    ...
    function selectRandomBullOwnerToReceiveStolenRunner(uint16 _stolenRunnerID) external onlyBullRunGame() returns (address) {
        uint256[] memory bullsInPen = viewBullpen();
        uint256 length = bullsInPen.length;

        uint256 randomIndex = _random(_stolenRunnerID) % length;
        uint256 selectedBullID = bullsInPen[randomIndex];
        address thief = OriginalOwner[uint16(selectedBullID)];
        emit RunnerThiefSelected(thief);
        return thief;
    }

    // pRNG generator
    function _random(uint16 _stolenRunnerID) internal view returns (uint256) {
        uint256 seed = uint256(keccak256(abi.encodePacked(
            block.timestamp + block.difficulty +
            ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (block.timestamp)) +
            block.gaslimit + 
            ((uint256(keccak256(abi.encodePacked(_stolenRunnerID)))) / (block.timestamp)) +
            block.number
        )));
