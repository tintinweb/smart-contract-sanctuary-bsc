...
contract Implementation is Upgradeable {
    ...
    function setAdmin(address _account) public notZeroAddress(_account) onlyOwner {
        adminList[_account] = true;

        emit SetAdminEvent(_account, true);
    }
    
    function revokeAdmin(address _account) public notZeroAddress(_account) onlyOwner {
        adminList[_account] = false;

        emit SetAdminEvent(_account, false);
    }
    
    function setAdmins(address[] memory _account) public onlyOwner {
        bool[] memory values = new bool[](_account.length);
        
        for(uint256 i = 0; i < _account.length; i++ ){
            if(!adminList[_account[i]] || _account[i] != address(0)) {
                adminList[_account[i]] = true;
                values[i] = true;
            }
        }

        emit SetAdminsEvent(_account, values);
    }
    
    function revokeAdmins(address[] memory _account) public onlyOwner {
        bool[] memory values = new bool[](_account.length);
        
        for(uint256 i = 0; i < _account.length; i++ ){
            if(adminList[_account[i]]) {
                adminList[_account[i]] = false;
                values[i] = false;
            }
        }
    ...
    function setViewAdmin(address _account) public notZeroAddress(_account) onlyOwner {
        viewAdmins[_account] = true;

        emit SetViewAdminEvent(_account, true);
    }
    
    function revokeViewAdmin(address _account) public notZeroAddress(_account) onlyOwner {
        viewAdmins[_account] = false;

        emit SetViewAdminEvent(_account, false);
    }
    
    function setViewAdmins(address[] memory _account) public onlyOwner {
        bool[] memory values = new bool[](_account.length);
        
        for(uint256 i = 0; i < _account.length; i++ ){
            if(!viewAdmins[_account[i]] || _account[i] != address(0)) {
                viewAdmins[_account[i]] = true;
                values[i] = true;
            }
        }

        emit SetViewAdminsEvent(_account, values);
    }
    
    function revokeViewAdmins(address[] memory _account) public onlyOwner {
        bool[] memory values = new bool[](_account.length);
        
        for(uint256 i = 0; i < _account.length; i++ ){
            if(viewAdmins[_account[i]]) {
                viewAdmins[_account[i]] = false;
                values[i] = false;
            }
        }
    ...
    function isViewAdmin(address _account) public view returns (bool) {
        return viewAdmins[_account];
    }
...
contract Upgradeable is Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) viewAdmins;
    ...
    event SetAdminsEvent(address[] accounts, bool[] values);
    
    event SetViewAdminEvent(address indexed account, bool value);
    
    event SetViewAdminsEvent(address[] accounts, bool[] values);
