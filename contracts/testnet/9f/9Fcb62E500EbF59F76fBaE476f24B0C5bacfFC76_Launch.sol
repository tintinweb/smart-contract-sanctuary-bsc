...
contract ICO is Initializable{
    ...
    function initialize (ICOparam memory _data) external initializer{
        icoInfo = _data;
        ...
        pancakeRouterAddress = 0x701d734A7AcA88429a516862c09eE8fF7893B145;
        token = IERC20Upgradeable(icoInfo.data.tokenAddress);
        dataStorage = Idatabase(0xb52BE54e153113fEE47B8893032D740101aa7888);

        setPairAddress();

    }

    function setPairAddress() internal{
        require(msg.sender == icoInfo.factory,"NF");//Not Factory

        IPancakeRouter02 _uniswapV2Router = IPancakeRouter02(pancakeRouterAddress);
        address _uniswapV2Pair = IPancakeFactory(_uniswapV2Router.factory()).createPair(icoInfo.data.tokenAddress, _uniswapV2Router.WETH());
...
contract Launch is Initializable{
    ...
    function initialize() external initializer{
        ...
        SSN = 0x5D78cde7106732BC0B1CEF6C63637595C28DfCEb;
    ...
    function createNewICO(DataParam calldata newICOData) external{
        ...
        icoContract.initialize(_icoBase);

        dataStorage.setPresaleDetails(address(icoContract), _icoBase);
        

        
        IERC20Upgradeable ssn = IERC20Upgradeable(SSN);

        ssn.transferFrom(msg.sender, admin, dataStorage.getLunchAdminWalletFee());
    ...
    function _totalFees(uint256 _liquiditySupply, uint256 _exchangeListingRateBNB, uint256 _hardCap, uint256 _presaleSupply) internal view returns(uint256){
