...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("Bd_Tracker", "Bd_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 30000 * (10**18); //must hold 3,000,000,000+ tokens
    }
 
    function _transfer(address, address, uint256) pure internal override {
        require(false, "Bd: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "Bd: withdrawDividend disabled. Use the 'claim' function on the main Boda contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Bd: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Bd: Cannot update claimWait to same value");
...
contract bd is ERC20, Ownable {
    ...
    uint256 public _burnFeeBuy = 1;

    uint256 public totalBuyFees = _busdDividendRewardsFeeBuy.add(_marketingFeeBuy).add(_lpFeeBuy).add(_burnFeeBuy);
    ...
    uint256 public totalSellFees = _busdDividendRewardsFeeSell.add(_lpFeeSell).add(_marketingFeeSell).add(_burnFeeSell);
    ...
    constructor() ERC20("testSon", "tsSon") {
        	
    	marketingWallet = 0x3D84ae452071A018b42F1128B8f35968A1a625A0; 
    ...
    function updateBuyFees(uint256 busdDividendRewardsFeeBuy, uint256 marketingFeeBuy, uint256 lpFeeBuy, uint256 burnFeeBuy) external onlyOwner{
        _busdDividendRewardsFeeBuy = busdDividendRewardsFeeBuy;
        _marketingFeeBuy = marketingFeeBuy;
        _lpFeeBuy = lpFeeBuy;
        _burnFeeBuy = burnFeeBuy;
        totalBuyFees = _busdDividendRewardsFeeBuy.add(_marketingFeeBuy).add(_lpFeeBuy).add(burnFeeBuy);
        require(totalBuyFees <= 20, "CGAME: Total tax must be less than or equal to 15%");
    }

        function updateSellFees(uint256 busdDividendRewardsFeeSell, uint256 marketingFeeSell, uint256 lpFeeSell, uint256 burnFeeSell) external onlyOwner{
        ...
        totalSellFees = _busdDividendRewardsFeeSell.add(_marketingFeeSell).add(_lpFeeSell).add(_burnFeeSell);
        require(totalSellFees <= 20, "CGAME: Total tax must be less than or equal to 15%");
    }
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            tmpBurnShare = amount.mul(burnFee).div(100);
            ...
            if(tmpBurnShare > 0){
            super._transfer(from, deadAddress, tmpBurnShare);
            }
