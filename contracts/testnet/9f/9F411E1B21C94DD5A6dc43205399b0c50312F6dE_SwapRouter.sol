...
library SwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'f228f8edbfde9a6cab9c44d7af17679f479afcfe55c8c6f227a5f41336041a9a' // init code hash
            ))));
    }
...
interface IWETH {
    function mintTokens(address _recipient, uint256 _amount) external returns (bool success);
    function transfer(address to, uint value) external returns (bool);
}
...
contract SwapRouter is ISwapRouter02 {
    ...
    function addLiquidityETH(
        ...
        IWETH(WETH).mintTokens(address(this), amountETH);
    ...
    function removeLiquidityETH(
        ...
        IWETH(WETH).transfer(address(WETH), amountETH);
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'SwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).mintTokens(address(this), amounts[0]);
    ...
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        IWETH(WETH).transfer(address(WETH), amounts[amounts.length - 1]);
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        IWETH(WETH).mintTokens(address(this), amountIn);
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        IWETH(WETH).transfer(address(WETH), amountOut);
