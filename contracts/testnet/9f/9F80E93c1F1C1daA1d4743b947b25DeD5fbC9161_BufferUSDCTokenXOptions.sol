...
interface ILiquidityPoolV2 is ILiquidityPool {
    ...
    ILiquidityPoolV2
{
    ...
    function setProjectOwner(address account) external {
        require(
            ...
            "msg.sender is not allowed to set this role"
        );
     ...
     */
    function setPoolState(bool _hasPoolEnded) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "msg.sender is not allowed change pool state"
        );
     ...
     */
    function rollOver(uint256 expiry) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "msg.sender is not allowed change pool state"
        );
        ...
        emit PoolRolledOver(currentRound);
    }
        ...
        returns (uint256 burn)
    {
        ...
        uint256 maxUserWithdrawal = (balanceOf(account) * balance) /
            totalSupply;

        uint256 amountToWithdraw = maxUserWithdrawal < tokenXAmount
            ? maxUserWithdrawal
            : tokenXAmount;

        burn = divCeil((amountToWithdraw * totalSupply), balance);
        ...
        bool success = tokenX.transfer(account, amountToWithdraw);
        require(success, "The Withdrawal didn't go through");
        emit Withdraw(account, amountToWithdraw, burn);
    }

    /*
     * @nonce Initiates withdraw requests for users
     * @param tokenX Amount Amount of X to receive
     * @return account User address for which the withdrawal has to be initiated
     */
    function _initiateWithdraw(uint256 tokenXAmount, address account) internal {
        require(balanceOf(account) > 0, "Nothing to withdraw");
        ...
        if (addressToWithdrawRequest.exists) {
            ...
            withdrawRequest.withdraw_amount =
                withdrawRequest.withdraw_amount +
                tokenXAmount;
        } else {
        ...
        emit WithdrawInitiated(tokenXAmount, account);
    }

    /*
     * @nonce withdraw burns rBFR-X and receives X from the pool
     * @param tokenX Amount Amount of X to receive
     */
    function withdraw(uint256 tokenXAmount) external {
        if (hasPoolEnded) {
            ...
            require(
                ...
                "Pool is not accepting withdraw requests currently"
            );
    ...
    /*
     * @nonce Processes all the withdraw requests once the round ends
     * @return requestsToProcess Number of requests to process
     */
    function processWithdrawRequests(uint256 requestsToProcess) external {
        require(queueStart != queueEnd, "Request queue is empty");
        ...
        for (uint256 i = queueStart; i < endIndex; i++) {
            ...
            _withdraw(withdrawRequest.withdraw_amount, withdrawRequest.account);
            emit WithdrawRequestProcessed(
                withdrawRequest.withdraw_amount,
                withdrawRequest.account
            );
    ...
    /*
     * @nonce calls by BufferCallOptions to change the locked funds
     * @param tokenXAmount Amount of funds that should be locked in an option
     */
    function lockChange(
        uint256 id,
...
// Part: OpenZeppelin/[email protected]3.2/ERC721URIStorage
    ...
    ERC721URIStorage
{
    ...
    function _mint(
        ...
        _mintUnits(minter_, optionID, slot_, config.maxUnits());
    }
        ...
        returns (uint256[] memory newOptionIDs)
    {
        ...
        uint256 amountUnits = option.amount / _units[optionID];
        uint256 premiumUnits = option.premium / _units[optionID];
        uint256 lockedAmountUnits = option.lockedAmount / _units[optionID];
        for (uint256 i = 0; i < splitUnits_.length; i++) {
            ...
            _lock(newOptionID, newOption.lockedAmount, newOption.premium);
        }
    }
        ...
        virtual
    {
        ...
        require(_slotOf(optionId_) == _slotOf(targetOptionId_), "N7");
    ...
    function transferFrom(
        ...
        uint256 newAmount = (option.amount / _units[optionID]) * transferUnits_;
        uint256 newPremium = (option.premium / _units[optionID]) *
            transferUnits_;
        uint256 newLockedAmount = (option.lockedAmount / _units[optionID]) *
            transferUnits_;
        Option memory newOption = Option(
            option.state,
            option.strike,
    ...
    function transferFrom(
        ...
        require(_exists(targetOptionID), "N12");
        Option memory option = _getOption(optionID);
        uint256 newAmount = (option.amount / _units[optionID]) * transferUnits_;
        uint256 newPremium = (option.premium / _units[optionID]) *
            transferUnits_;
...
// File: BufferUSDCTokenXOptions.sol
 ...
 */
contract BufferUSDCTokenXOptions is OptionsCore {
    ERC20 public USDC;
    ERC20 public immutable tokenX;
    mapping(uint256 => string) private _tokenURIs;
    ...
    function create(
        ...
        require(pool.getExpiry() > block.timestamp, "O1");
        ...
        require(totalFee > amount / 1000, "O2");
        ...
        BufferNFTCore._mint(
            optionID,
            msg.sender,
            BufferNFTCore.createSlot(optionID)
        );
        _setTokenURI(optionID, metadata);
        uint256 stakingAmount = ((settlementFee *
            config.stakingFeePercentage()) / 100);
    ...
    function _lock(
        ...
        tokenX.approve(address(pool), premium);
        ...
        returns (uint256 utilization)
    {
        ...
        utilization = ((pool.getLockedAmount() + amount) * 100e8) / poolBalance;
    }
    ...
    function burn(uint256 optionID) external virtual {
        require(_msgSender() == ownerOf(optionID), "O9");
        _burnToken(optionID);
    }
    ...
    function canExercise(uint256 optionID) internal view returns (bool) {
        ...
        require(optionBlocks[optionID] != block.number, "O11");
