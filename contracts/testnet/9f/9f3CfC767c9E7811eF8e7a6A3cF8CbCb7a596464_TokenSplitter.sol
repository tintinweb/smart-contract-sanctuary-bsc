...
contract TokenSplitter {
   ...
   function addAddress(address addr, uint taux) public onlyOwner {
    ...
    function removeAddress(address addr, uint taux) public onlyOwner {
    ...
    function splitTokens() public onlyOwner {
        Token erc20Token = Token(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);//testnet BUSD
        uint amount = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7).balanceOf(address(this)); //testnet BUSD
        //uint amount = getBalance();
        //_amountForEach = 30;
        bool success;
        for (uint i = 0; i < _recipients.length; i++) {
            success = erc20Token.transfer(_recipients[i], amount / 100 * _percent[i]);
        }
    }
}