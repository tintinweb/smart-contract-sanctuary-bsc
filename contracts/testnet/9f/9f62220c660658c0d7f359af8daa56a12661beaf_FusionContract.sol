...
import "./lib/Bytes32ArrayLib.sol";
...
contract FusionContract is ChainlinkClientUpgradeable, PausableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    // requestId => fusionInfo
    mapping(bytes32 => FusionInfo) private _fusions;

    bytes32[] private _itemIds;
    // itemId => ipfsHash
    mapping(bytes32 => string) private _ipfsHash;

    uint256 public amountNFTPerFusion;

    address[] public whitelistNFTContracts;
    // nftContract => isWhitelist
    ...
    function setIpfsHash(bytes32 _itemId, string memory _hash) public onlyRole(DEFAULT_ADMIN_ROLE) {
        if (!_itemIds.exists(_itemId)) {
            _itemIds.push(_itemId);
        }

        _ipfsHash[_itemId] = _hash;
    }

    function getIpfsHash() public view returns (bytes32[] memory itemIds, string[] memory ipfsHash) {
        itemIds = _itemIds;
        ipfsHash = new string[](_itemIds.length);
        for (uint256 i = 0; i < _itemIds.length; i++) {
            ipfsHash[i] = _ipfsHash[itemIds[i]];
        }
    }

    function getFustionInfo(bytes32 _requestId) public view returns (FusionInfo memory) {
        return _fusions[_requestId];
    }
        ...
        returns (bytes32 requestId)
    {
        ...
        _fusions[requestId] = FusionInfo(_msgSender(), _nftContract, _tokenIds);
    ...
    function reFusion(bytes32 _requestId) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bytes32 requestId) {
        FusionInfo memory fusionInfo = _fusions[_requestId];
        require(fusionInfo.user != address(0), "Error: fusion not found");
        delete _fusions[_requestId];

        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.handleFusion.selector);
        requestId = sendChainlinkRequestTo(oracle, request, 0.1 * 1 ether);
        _fusions[requestId] = fusionInfo;

        emit ReRequest(_requestId, requestId);
    }

    function handleFusion(bytes32 _requestId, bytes32 _itemIdBytes32) public recordChainlinkFulfillment(_requestId) {
        require(hasRole(ORACLE_ROLE, _msgSender()), "Error: ORACLE role required");

        string memory ipfsHash = _ipfsHash[_itemIdBytes32];
        require(bytes(ipfsHash).length != 0, "Error: itemId not set value");
        FusionInfo memory fusionInfo = _fusions[_requestId];
        require(fusionInfo.user != address(0), "Error: fusion not found");
        delete _fusions[_requestId];
        ...
        uint256 returnId = nftContract.mint(
            fusionInfo.user,
            string(abi.encodePacked("https://planetsandbox.infura-ipfs.io/ipfs/", ipfsHash)),
            1
        )[0];

        emit HandleFusion(_requestId, returnId);
    }
...
library Bytes32ArrayLib {
    function exists(bytes32[] storage self, bytes32 element) internal view returns (bool) {
        for (uint256 i = 0; i < self.length; i++) {
            if (self[i] == element) return true;
        }
        return false;
