...
contract BuddyDao is Ownable, Pausable, ReentrancyGuard, SafeTransfer, AutomationCompatibleInterface {
    ...
    function NewTrust(address _approveAddress, string memory _alias, address _token, uint256 _fixedRate, uint256 _amount) external nonReentrant whenNotPaused {
        ...
        LenderIndex[msg.sender][_approveAddress] = lendInfoLength;
        ...
        BorrowerIndex[_approveAddress][msg.sender] = borrowerInfoLength;
        // 3. 输出 log
    ...
    function RemoveTrust(address _approveAddress, uint256 _index, uint256 _cancelAmount) external nonReentrant whenNotPaused {
        ...
        if (_cancelAmount == personalLenderInfo.CreditLine) {
            // 全部取消，清空数据
            delete LenderData[msg.sender];
            delete BorrowerData[_approveAddress];
            delete LenderIndex[msg.sender][_approveAddress];
            delete BorrowerIndex[_approveAddress][msg.sender];
        } else {
            // 取消部分实际操作
    ...
    function Pay(address _lendAddress, uint256 _index, uint256 _payAmount) external nonReentrant whenNotPaused {
        ...
        require(_payAmount <= personalBorrowerInfo.Amount, "The returned quantity must be less than or equal to the borrowed quantity.");
         ...
         // interest 利息计算
        uint256 borrowerInterest = calculatingInterest(_lendAddress,  _index,  _payAmount);
        ...
        // 判断是全部归还还是归还部分操作
        if (_payAmount == personalBorrowerInfo.Amount) {
            // 全部归还，清空数组信息，直接删除信息
            delete LenderData[_lendAddress];
            delete BorrowerData[msg.sender];
            delete LenderIndex[_lendAddress][msg.sender];
            delete BorrowerIndex[msg.sender][_lendAddress];
        } else {
            // borrower记录归还信息
    ...
    // 根据归还数量计算归还利息函数
    function calculatingInterest(address _lendAddress, uint256 _index, uint256 _payAmount) public view returns(uint256){
        // 参数边界条件判断
        require(_lendAddress != address(0), "_lendAddress is not a zero address");
        require(_payAmount != 0, "The number of payment amount cannot be equal to 0");
        // 参数逻辑判断
        ...
        uint256 timeRatio = ((block.timestamp.sub(personalBorrowerInfo.BorrowStartTime)).mul(baseDecimal));
        uint256 perInterest = (((personalBorrowerInfo.FixedRate).mul(baseDecimal)).div(baseYear));
        uint256 interest = (timeRatio * perInterest * personalBorrowerInfo.Amount).div(baseDecimal);
        return interest;
    }
