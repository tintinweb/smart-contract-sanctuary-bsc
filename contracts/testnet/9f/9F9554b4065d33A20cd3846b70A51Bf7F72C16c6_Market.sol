...
contract Market is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function setOrganizer(address to, bool value) public onlyOwner{
        organizer[to] = value;
    ...
    function createEvent(
        ...
        idToEvent[idEvent] = Event(
            ...
            0,
            false,
            Token(tokenAddress).uri(idEvent)
        );
        openEvent ++;
        organizerEvent[msg.sender] ++;
        emit createEventEvent(idEvent, msg.sender, price, totalTicket, false, Token(tokenAddress).uri(idEvent));
    }

    function buyTicket(uint256 id, uint256 number) notBlacklisted public payable {
        require(!idToEvent[id].isClosed, "Event closed");
        require(idToEvent[id].ticketRemain >= number, "Not enough tickets");
        require(msg.value == idToEvent[id].ticketPrice * number, "Sent value is not equal to the total price");
        idToEvent[id].revenue += msg.value;
        idToEvent[id].ticketRemain -= number;
        Token(tokenAddress).safeTransferFrom(idToEvent[id].organizer, msg.sender, id, number, "");
        emit sellTicketEvent(id, idToEvent[id].ticketRemain, msg.sender, idToEvent[id].revenue);
    }

    function returnTicket(uint256 id, uint256 number) public payable notBlacklisted{
        require(!idToEvent[id].isClosed, "Event closed");
        require(Token(tokenAddress).balanceOf(msg.sender, id) >= number, "You cannot return tickets that are larger than the number of tickets you have ");
        Token(tokenAddress).burn(msg.sender, id, number);
        payable(msg.sender).transfer(idToEvent[id].ticketPrice * number * 70 / 100);
        idToEvent[id].revenue -= idToEvent[id].ticketPrice * number * 70 / 100;
        idToEvent[id].ticketRemain += number;
...
contract Token is ERC1155, Ownable, ERC1155Burnable {
    ...
    constructor(address _controller) ERC1155("https://wkmrsjag3nvm.usemoralis.com/{id}.json") {
