...
contract MyTokenPruebaEEEE is ERC20, Ownable
{    
    ...
    constructor(
        ...
        pancakePair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
     
        // marketing
        //marketing_address = 0xaBE6f6122A3a1A86E151a10Bb29A943327b6f8b4;
        ...
        fee_percentages.push(500);  // P2P  fee is  5.00%*/
        fee_percentages.push(buyFee_); // Buy  fee is 10.00%
        fee_percentages.push(buySell_); // Sell fee is 15.00%
        fee_percentages.push(buyP2P_);  // P2P  fee is  5.00%
        // End marketing

        is_taxless[msg.sender] = true;
        is_taxless[marketing_address] = true;
    ...
    ) internal virtual override(ERC20)
    {
        ...
        if(!is_in_fee_transfer)
        {
            ...
            if (!is_taxless[from] && !is_taxless[to]) {
                bool sell = to == pancakePair;
                bool p2p = from != pancakePair && to != pancakePair;
            ...
            if(fees_collected > 0)
            {
                ...
                _transfer(to, marketing_address, fees_collected);
