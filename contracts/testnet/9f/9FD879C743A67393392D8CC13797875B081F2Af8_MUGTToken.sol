...
contract MUGTToken {
    ...
    string private _name = "Music Government Token";
    string private _symbol = "MUGT";
    uint8 private _decimals = 18;

    uint256 public __reflectFee = 1;
    uint256 public __marketingFee = 2;
    uint256 public __developmentFee = 2;
    uint256 public __liquidityFee = 3;
    uint256 public __prev_liquidityFee = 2;
    ...
    constructor(address payable _marketing, address payable _development, address router) {
        _owner = msg.sender;
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(__reflectFee).div(10**2);
    }
    ...
    function calculateLiquidityFee(uint256 _amount)
        ...
        returns (uint256)
    {
        return _amount.mul(__liquidityFee).div(10**2);
    }
        ...
        returns (uint256)
    {
        ...
        return _amount.mul(operations_fee).div(10**2);
    }
    ...
    function payDistribution(uint256 distrib) private {
        uint256 marketingPart = distrib.mul(__marketingFee).div(
            __marketingFee.add(__developmentFee)
        );
