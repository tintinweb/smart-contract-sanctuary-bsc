    ...
    ReentrancyGuardUpgradeable
{
    ...
    function testingThis(address _user) external view returns(uint256) {
    uint256[] memory tests; //= new uint256[](this.tokensOf(_user).length);
    //    for (uint256 i = 0; i < this.tokensOf(_user).length; i++) {
    //     tests[i] = this.tokensOf(_user)[i];
    //    }
    tests = this.tokensOf(_user);
        // tests[1] = this.tokensOf(_user)[1];
        // upgradedParcelsMapping[this.tokensOf(_user)[1]];
    return  tests.length;//(tests, upgradedParcelsMapping[this.tokensOf(_user)[1]]);
    }
    ...
    function getUpgradedParcelIds(address _user) external view returns(uint256[] memory) {
       ...
       uint256[] memory upgradedParcesIds; //= new uint256[](this.tokensOf(_user).length);
       for (uint256 i = 0; i < tokensOf.length; i++) {
            if (upgradedParcelsMapping[tokensOf[i]]) {
                upgradedParcesIds[i] = tokensOf[i];
            }
       }
