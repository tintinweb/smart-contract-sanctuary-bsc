...
contract MRBTCStakingContract {
    ...
    ERC20 public token;
    uint256 public minTxAmount = 18866500;
    ...
    function stake(uint256 amount, uint256 level) public returns (bool) {
    ...
    function setLevel(uint256 level) internal {
        if (level == 1) {
            ...
            user[msg.sender][level].endTime = block.timestamp + 180 days;
        }
    }
    ...
    function Claim(uint256 level) public returns (bool) {
        ...
        user[msg.sender][level].claimAmount += amount;
    ...
    function emergencyUnstake(uint256 level) public returns (uint256) {
        require(user[msg.sender][level].isActive, "user not exist");
        uint256 _penalty;
        ...
        returns (userDetails memory, uint256)
    {
        ...
        return (
            userDetails(
                ...
                user[account][level].claimAmount,
