...
contract BnbSmartChain is ContractData {
  ...
  // --------------------- PUBLIC METHODS ---------------------------
    function newDonation() external payable {
        accountsInfo[msg.sender].donationAmount += msg.value;
    }
    ...
    function withdrawalTotal() external {
        ...
        accountsEarnings[sender].comulativePassiveAmount = comulativePercent;
    ...
    function withdrawalPercent(uint _amount) external {
        ...
        networkWithdraw += 1;
        distributeWithdrawalAmount(sender, _amount);
        accountsEarnings[firstUser].comulativePassiveAmount += _amount;
    ...
    function upgradeDeposit() external payable {
        ...
        networkDeposits += 1;
        emit updateNetwork(networkSize, maxBalance, address(this).balance);
        _registerDeposit(sender, msg.value, 2);
    ...
    function registerAccount(address ref) external payable {
        ...
        accountsInfo[ref].directCount += 1;
  ...
  // --------------------- PRIVATE METHODS ---------------------------
    ...
    function _registerDeposit(address sender, uint amount, uint8 _type) private {
        ...
        if (_type == 0 || _type == 2) {
            ...
            emit updateUser(referral, accountsInfo[referral].directCount, accountsInfo[referral].unlockedLevel, accountsInfo[referral].depositCounter, accountsInfo[referral].depositTotal);
        }
        ...
        if (_type == 0) {
            ...
            accountsEarnings[sender].comulativePassiveAmount = 0;
        ...
        emit updateUser(sender, accountsInfo[sender].directCount, accountsInfo[sender].unlockedLevel, accountsInfo[sender].depositCounter, accountsInfo[sender].depositTotal);
...
contract ContractData is Authorized {
  ...
  function projectInfo() public view returns(uint, uint, uint, uint) {
    return(address(this).balance, networkSize, networkDeposits, maxBalance);
  }
  ...
  function passiveEarning(address _address) public view returns(uint, uint) {
    ...
    uint available =  accountsEarnings[_address].receivedDirectBonusAmount + accountsEarnings[_address].withdrawDirectBonusAmount + accountsEarnings[_address].receivedLevelBonusAmount + accountsEarnings[_address].withdrawLevelBonusAmount;  
    uint maximunReceived = maximunEarning(_address);

    uint comulativeDay =  accountsEarnings[_address].comulativePassiveAmount;
    uint comulativeEarnings = (comulativeDay - accountsEarnings[_address].withdrawPassiveAmount) < 0 ? 0 : (comulativeDay - accountsEarnings[_address].withdrawPassiveAmount);

    uint pasiveAvailable = (((dailyRentability  * accountsEarnings[_address].depositValue) / 1000) /  10 seconds) * secondsRunning;
  ...
  function waitingToReceive(address _address) public view returns(uint) {
    ...
    uint available = comulativeDay + accountsEarnings[_address].receivedDirectBonusAmount + accountsEarnings[_address].withdrawDirectBonusAmount + accountsEarnings[_address].receivedLevelBonusAmount + accountsEarnings[_address].withdrawLevelBonusAmount;  
    uint maximunReceived = maximunEarning(_address);
