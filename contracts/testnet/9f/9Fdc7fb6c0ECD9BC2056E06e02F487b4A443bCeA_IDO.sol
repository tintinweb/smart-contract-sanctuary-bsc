...
// File: contracts/iwo.sol

interface MasterchefIDO {
...
contract IDO is ReentrancyGuard, Initializable {
    using SafeERC20 for IERC20;

    uint256 public constant HARVEST_PERIODS = 4;
    MasterchefIDO public masterchefIDO;
    ...
    mapping(address => uint256) public rankRegisters;
    mapping(address => bool) public whitelist;

    // Allocation amount;
    uint256[] public allocationLimits;
    ...
    event Register(address indexed user, uint256 rank);
    ...
    function initialize(
        ...
        // Allocation rates
        allocationLimits = [100 ether, 200 ether, 500 ether, 1000 ether];

        // Setup vesting release blocks
        for (uint256 i = 0; i < HARVEST_PERIODS; i++) {
            harvestReleaseTimestamps[i] = endTime + (_vestingOffset * i);
        }
    }

    // Register a user.
    function register() external {
        require(!registers[msg.sender], "User is already registered");
        require(block.timestamp < startTime, "End time register");
        uint256 rank = masterchefIDO.getRank(msg.sender);
        require(rank > 0, "Not eligible");
        registers[msg.sender] = true;
        rankRegisters[msg.sender] = rank;

        emit Register(msg.sender, rank);
    }
    ...
    function updateAllocationLimits(uint256[] memory _allocationLimits)
        public
        onlyOwner
    {
        allocationLimits = _allocationLimits;
    }
    ...
    function setOfferingAmount(uint256 _offerAmount) public onlyOwner {
        require(block.timestamp < startTime, "cannot update during active iao");
        offeringAmount = _offerAmount;
    }

    function setRaisingAmount(uint256 _raisingAmount) public onlyOwner {
        require(block.number < startTime, "cannot update during active iao");
        raisingAmount = _raisingAmount;
    }
    ...
    function getLimitDeposit(address _user) public view returns (uint256) {
        ...
        if (isRegistered(_user)) {
            uint256 rank = rankRegisters[_user];
            if (rank > 0) {
                uint256 limitAmount = allocationLimits[rank - 1];

                uint256 userLimit = limitAmount - userInfo[_user].amount;
                limit = userLimit > raisingAmount - totalAmount
    ...
    function deposit(uint256 _amount) external nonReentrant onlyActiveIAO {
        ...
        require(
            isWhitelist(msg.sender) || isRegistered(msg.sender),
            "User is not whitelisted or registered"
        );

        require(
            _amount <= getLimitDeposit(msg.sender),
        ...
        returns (uint256)
    {
        if (_period == 0) {
            // 10% firsttime
            return (getOfferingAmount(_user) * 10) / 100;
        } else {
            // 30% for next time
            return (getOfferingAmount(_user) * 30) / 100;
        }
    }
