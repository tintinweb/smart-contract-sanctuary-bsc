...
import "./OKGManagement.sol";

contract KABToken is ERC20, IERC20MintableBurnable {
	OKGManagement public gov;

	bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
	bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
	bytes32 public constant MARKETPLACE = keccak256("MARKETPLACE");

	constructor(OKGManagement _gov) ERC20("KAB Token", "KAB") {
		gov = _gov;
	}
	...
	modifier onlyMinter() {
		require(gov.hasRole(MINTER_ROLE, _msgSender()), "Caller is not Minter");
		_;
	}
...
/**
    @title OKGManagement contract
    @dev This contract is being used as Governance of Okeenga Game
        + Set additional roles - DEFAULT_ADMIN_ROLE, MANAGER_ROLE, MINTER_ROLE and MARKETPLACE ....
        + Register address (Treasury) to receive Commission Fee 
        + Set Commission Fee rate
       
*/
contract OKGManagement is AccessControlEnumerable {
    ...
    */
	function setCommissionFee(uint256 _newRate) external onlyRole(MANAGER_ROLE) {
    ...
    */
	function addToList(address _nftContr) external onlyRole(MANAGER_ROLE) {
		...
		require(!listOfNFTs[_nftContr], "Contract already added");
    ...
    */
	function removeFromList(address _nftContr) external onlyRole(MANAGER_ROLE) {
		require(listOfNFTs[_nftContr], "Contract not found");
    ...
    */
	...
	/**
       ...
       @param _token         Address of Token contract
    */
	function removePaymentToken(address _token) external onlyRole(MANAGER_ROLE) {
