...
contract SyndicationFactory is Ownable {
    ...
    struct CreateArgs {
        ...
        uint256 tokenId;
        bool useDepository;
        Syndication.TargetNFTStatus statusNFT;
        uint256 orderId;
    ...
    function createSyndication(
        ...
        if (_amountForLP > 0) {
            require(_createArgs.length == 1, "only one syndication");
        }
        if (_vaultAddress != address(0) && _initAmount > 0) {
            require(_createArgs.length == 1, "only one syndication");
            require( IVault(_vaultAddress).owner() == msg.sender, "Invalid owner" );
            _amountForLP = _initAmount;
            IVault(_vaultAddress).withdraw(address(this), _amountForLP);
        }

        address[] memory _syndications = new address[](_createArgs.length);

        for (uint256 i = 0; i < _createArgs.length; i ++) {
            uint256 _gasLeft = gasleft();
            uint256 _gasPrice = tx.gasprice;
            
            address syndication = address(
                new Syndication(
                    _createArgs[i].nftAddress,
                    _createArgs[i].tokenId,
                    _createArgs[i].useDepository,
                    _createArgs[i].statusNFT,
                    _createArgs[i].orderId,
                    _createArgs[i].protectedMarketplace,
                    msg.sender,
                    operator
                )
            );
            ...
            if (_amountForLP > 0) {
                ISyndication(syndication).addLiquidity{value:_amountForLP}(msg.sender, address(0), 0);
            }
        }
...
interface ISyndication {
    ...
    function addLiquidity(address _lpOwner, address _vaultAddress, uint256 _depositAmount) external payable;
...
contract Syndication is ERC20, Ownable{
    ...
    function addLiquidity(address _lpOwner, address _vaultAddress, uint256 _depositAmount) external payable {
        ...
        if (inputAmount == 0) {
            require( _vaultAddress != address(0) && _depositAmount > 0, "NA" );
            require( IVault(_vaultAddress).owner() == msg.sender, "Invalid owner" );
            inputAmount = _depositAmount;
            IVault(_vaultAddress).withdraw(address(this), _depositAmount);
        }
    ...
    function _buyNftOnMarketplace() internal returns (bool) {
        ...
        if (terms.statusTargetNFT == TargetNFTStatus.Fixed) {
            ...
            marketplace.buyFixedPayOrder{value:ipo.tokenPrice}(_orderIds, address(0));
            ...
            if ( getBidStatus() == IProtectedMarketplace.BidStatus.Pending ) {
                ...
                if (totalValue > ipo.tokenPrice) {
                    marketplace.createBid{value: totalValue}(terms.buyOrderID, address(0), 0);   //FIXME
    ...
    function _sellNftOnMarketplace() internal returns (bool) {
        ...
        IProtectedMarketplace.OrderArgs[] memory _orderArgs = new IProtectedMarketplace.OrderArgs[](1);
        _orderArgs[0] = IProtectedMarketplace.OrderArgs(
            terms.nftAddress, 
            terms.tokenId,
            tokenPrice,
            terms.useDepository,
            ...
            protectionTime,
            true,
            block.timestamp.add(auctionTime)
        );

        marketplace.createOrder( _orderArgs );
