...
contract MFTStaking {
    ...
    function cBalance3(address account) external view returns (uint256) {
        uint256 balance = customStakingBalance3[account];
        uint256 Duration = (block.timestamp - startTime3[account]) / 86400;
        uint256 cBalance = balance * (1 + customAPY3 / 100000) ** Duration;
        return cBalance;
    }
    ...
    function cBalance6(address account) external view returns (uint256) {
        ...
        uint256 cBalance = balance * (1 + customAPY6 / 100000) ** Duration;
        return cBalance;
    }
    ...
    function cBalance12(address account) external view returns (uint256) {
        ...
        uint256 Duration = (block.timestamp - startTime3[account]) / 86400;
        uint256 cBalance = balance * (1 + customAPY12 / 100000) ** Duration;
        return cBalance;
    }
