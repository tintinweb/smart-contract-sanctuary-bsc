...
contract LevelXToken is Initializable, Ownable, ERC20
{
	...
	uint256 constant WEEK_OFFSET = 2 days - 11 hours - 5 hours; // week offset for level bumps (Tuesdays 11AM EST)
...
*/
	...
	function timeToNextWeek() external view returns (uint256 _timeToNextWeek)
	{
		return time() % WEEK_DURATION;
	}

	function timeToNextEpoch() external view returns (uint256 _timeToNextEpoch)
	{
		return time() % EPOCH_DURATION;
	}
