...
contract test is IERC20, Auth {
    ...
    uint256 public developmentFeeSell = 1;

    uint256 public totalFee = 9;
    uint256 public totalFeeSell = 9;

    address public autoLiquidityReceiver = address(this);
    address public marketingWallet = 0xcF84523391178a496Af7C713F11DAa91D70F6212;
    address public developmentWallet = 0xcF84523391178a496Af7C713F11DAa91D70F6212;
    ...
    constructor () Auth(msg.sender) {
        ...
        IUniRouterV2 _router = IUniRouterV2(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        router = _router;
    ...
    function changeFees(uint256 newLiqFee, uint256 newRewardFee, uint256 newMarketingFee, uint256 newdevelopmentFee) external authorized {
        ...
        require(newdevelopmentFee <= 2, "development Tax on buys can not be higher than 2%");
        require(newRewardFee >= 0, "Rewards Tax on buys can not be lower than 0%");
        ...
        totalFee = liquidityFee.add(marketingFee).add(developmentFee).add(rewardsFee);
    }

    function changeFeesSell(uint256 newLiqFeeSell, uint256 newRewardFeeSell, uint256 newMarketingFeeSell, uint256 newdevelopmentFeeSell) external authorized {
        ...
        require(newdevelopmentFeeSell <= 2, "development Tax on sells can not be higher than 2%");
        require(newRewardFeeSell >= 0, "Rewards Tax on sells can not be lower than 0%");

        liquidityFeeSell = newLiqFeeSell;
        rewardsFeeSell = newRewardFeeSell;
        marketingFeeSell = newMarketingFeeSell;
        developmentFeeSell = newdevelopmentFeeSell;

        totalFeeSell = liquidityFeeSell.add(marketingFeeSell).add(developmentFeeSell).add(rewardsFeeSell);
    }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 developmentShare = amountCRO.mul(developmentFee).div(totalCROFee);
        uint256 marketingShare = amountCROMarketing.sub(developmentShare);
    ...
    //Use this in case CRO are sent to the contract by mistake
    function rescueCRO(uint256 weiAmount) external onlyOwner{
        require(address(this).balance >= weiAmount, "insufficient CRO balance");
