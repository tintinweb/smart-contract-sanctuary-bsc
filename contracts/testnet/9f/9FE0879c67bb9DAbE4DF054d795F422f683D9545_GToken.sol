...
import '../Helpers/ContextUpgradeable.sol';
...
import "../Libraries/MathUpgradeable.sol";
import '../Helpers/Initializable.sol';
...
// SPDX-License-Identifier: MIT
import '../Helpers/ERC20Upgradeable.sol';
import '../Helpers/ERC4626Upgradeable.sol';
import '../Libraries/MathUpgradeable.sol';
...
contract GToken is ERC20Upgradeable, ERC4626Upgradeable, OwnableUpgradeable, IGToken {
    ...
    function updateLockedDepositNftOnce(address _lockedDepositNft) external onlyManager{
        if (!lockedDepositNftOnce){
            lockedDepositNft = INft(_lockedDepositNft);
            lockedDepositNftOnce = !lockedDepositNftOnce;
        }
    }
    ...
    function gnsTokenToAssetsPrice1() public returns(uint price) { // GNS_PRECISION
        (bool success, bytes memory result) = gnsPriceProvider.addr.call(
    ...
    function gnsTokenToAssetsPrice() public returns(uint price) { // GNS_PRECISION
        price = (AggregatorInterfaceV6_2(gnsPriceProvider.addr)).tokenPriceDai();
    ...
    // Give USD to the vault, receive token
