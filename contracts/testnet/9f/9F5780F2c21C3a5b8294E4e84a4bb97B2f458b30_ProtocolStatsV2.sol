...
import "../interfaces/ITvlStats.sol";
import "../interfaces/IBoardroom.sol";
import "../interfaces/IRewardPool.sol";
import "../interfaces/IUniswapV2Pair.sol";
import "../interfaces/IBasisAsset.sol";

contract ProtocolStatsV2 is OwnableUpgradeSafe {
    ...
    address public tvlStats;

    address[] public v3PegTokens;
    address[] public v3PegOriginalTokens;
    address[] public v3PegPools;
    ...
    function _accountTotalDollarValue(address _account) internal view returns (uint256) {
        IPriceChecker _priceChecker = IPriceChecker(priceChecker);
        uint256 _v3sBalance = IERC20(v3s).balanceOf(_account);
        uint256 _vshareBalance = IERC20(vshare).balanceOf(_account);
        uint256 _wcroBalance = IERC20(wcro).balanceOf(_account);
        uint256 _vvsBalance = IERC20(vvs).balanceOf(_account);
        uint256 _usdcBalance = IERC20(usdc).balanceOf(_account);
        uint256 _v3sUsdValue = (_v3sBalance > 0) ? _priceChecker.getTokenPriceToUsd(v3s).mul(_v3sBalance).div(1e18) : 0;
        uint256 _vshareUsdValue = (_vshareBalance > 0) ? _priceChecker.getTokenPriceToUsd(vshare).mul(_vshareBalance).div(1e18) : 0;
        uint256 _wcroUsdValue = (_wcroBalance > 0) ? _priceChecker.getTokenPriceToUsd(wcro).mul(_wcroBalance).div(1e18) : 0;
        uint256 _vvsUsdValue = (_vvsBalance > 0) ? _priceChecker.getTokenPriceToUsd(vvs).mul(_vvsBalance).div(1e18) : 0;
        return _v3sUsdValue.add(_vshareUsdValue).add(_wcroUsdValue).add(_vvsUsdValue).add(_usdcBalance);
    }
    ...
    function tokenStats() external view returns (
        ...
        uint256 _vbondPremiumRate;
        {
            ...
            v3sSafePrice = _accountTotalDollarValue(_treasury.insuranceFund()).mul(1e18).div(v3sCirculation);
        }
        vbondPrice = v3sPrice;
    ...
    function v3PegTokenStats() external view returns (
        uint256 numOfPegTokens,
        address[] memory v3PegTokenAddresses,
        address[] memory v3PegOriginalTokenAddresses,
        address[] memory v3PegPoolAddresses,
        uint256[] memory v3PegTokenPrices,
        uint256[] memory v3PegOriginalTokenPrices,
        uint256[] memory v3PegTokenSupplyInfos
    ) {
        IPriceChecker _priceChecker = IPriceChecker(priceChecker);
        numOfPegTokens = v3PegPools.length;
        v3PegTokenAddresses = new address[](numOfPegTokens);
        v3PegOriginalTokenAddresses = new address[](numOfPegTokens);
        v3PegPoolAddresses = new address[](numOfPegTokens);
        v3PegTokenPrices = new uint256[](numOfPegTokens);
        v3PegOriginalTokenPrices = new uint256[](numOfPegTokens);
        v3PegTokenSupplyInfos = new uint256[](numOfPegTokens * 3);
        for (uint256 i = 0; i < numOfPegTokens; i++) {
            address _tokenAddress = v3PegTokens[i];
            address _originalTokenAddress = v3PegOriginalTokens[i];
            address _poolAddress = v3PegPools[i];
            v3PegTokenAddresses[i] = _tokenAddress;
            v3PegOriginalTokenAddresses[i] = _originalTokenAddress;
            v3PegPoolAddresses[i] = _poolAddress;
            v3PegTokenPrices[i] = _priceChecker.getTokenPriceToUsd(_tokenAddress);
            v3PegOriginalTokenPrices[i] = _priceChecker.getTokenPriceToUsd(_originalTokenAddress);
            v3PegTokenSupplyInfos[i * 3] = IERC20(_tokenAddress).totalSupply();
            v3PegTokenSupplyInfos[i * 3 + 1] = v3PegTokenSupplyInfos[i * 3].sub(IERC20(_tokenAddress).balanceOf(_poolAddress));
            v3PegTokenSupplyInfos[i * 3 + 2] = (_tokenAddress == v3s) ? 0 : IBasisAsset(_tokenAddress).totalBurned();
        }
    }
    ...
    function poolStats(address _pool) public view returns (uint256 poolLength, uint256 totalTvl, address reward, uint256 rewardPrice,
        ...
        for (uint i = 0; i < poolLength; i++) {
            ...
            {
                if (_pool == v3sPool) reward = v3s;
                else if (_pool == vsharePool) reward = vshare;
                else reward = IRewardPool(_pool).reward();
                ...
                if (_totalAllocPoint > 0) {
                    ...
                    if (tvls[i] > 0) aprs[i] = _poolEarnedPerYear.mul(1e18).div(tvls[i]);
                }
            }
        }
    }
    ...
    function v3PegPoolStats(uint256 _index) public view returns (uint256 poolLength, uint256 totalTvl, address reward, uint256 rewardPrice,
        address[] memory lps, uint256[] memory allocPoints, uint256[] memory tvls, uint256[] memory aprs, uint256[] memory lpPrices) {
        return poolStats(v3PegPools[_index]);
    }
    ...
    // uint256[] memory v3PegRewardInfos,
    // =
    // uint256[] memory nextExpansionRates,
    // uint256[] memory nextExpansionAmounts,
    // uint256[] memory rewardEarnPerShares,
    // uint256[] memory rewardApres,
    // uint256[] memory v3PegTokenPrices)
    function mainBoardroomStats() external view returns (
        uint256 epoch, uint256 nextEpochPoint, uint256 twap,
        address share, uint256 sharePrice, uint256 totalShare, uint256 tvl, uint256 totalApr,
        uint256 numOfPegTokens, address[] memory v3PegTokenAddresses, uint256[] memory v3PegRewardInfos) {
        ITreasury _treasury = ITreasury(treasury);
        IPriceChecker _priceChecker = IPriceChecker(priceChecker);
        epoch = _treasury.epoch();
        nextEpochPoint = _treasury.nextEpochPoint();
        ...
        numOfPegTokens = v3PegPools.length;
        v3PegTokenAddresses = new address[](numOfPegTokens);
        v3PegRewardInfos = new uint256[](numOfPegTokens * 5);
        uint256 _treasurySharedPercent = _treasury.boardroomSharedPercent();
        if (_treasurySharedPercent > 0) {
            address _boardroom = _treasury.boardroom();
            share = IBoardroom(_boardroom).share();
            ...
            tvl = totalShare.mul(sharePrice).div(1e18);
            for (uint256 i = 0; i < numOfPegTokens; i++) {
                address _pegToken = v3PegTokens[i];
                v3PegTokenAddresses[i] = _pegToken;
                v3PegRewardInfos[i * 5] = _treasury.getPegTokenExpansionRate(_pegToken);
                v3PegRewardInfos[i * 5 + 1] = _treasury.getPegTokenExpansionAmount(_pegToken);
                if (totalShare > 0) {
                    {
                        uint256 _boardroomAmount = v3PegRewardInfos[i * 5 + 1].mul(_treasurySharedPercent).div(10000);
                        v3PegRewardInfos[i * 5 + 2] = _boardroomAmount.mul(1e18).div(totalShare);
                    }
                    if (sharePrice > 0) {
                        uint256 _pegTokenPrice = _priceChecker.getTokenPriceToUsd(_pegToken);
                        // = (v3sEarnPerShare * totalShare * v3sPrice) / TVL
                        // = (v3sEarnPerShare * v3sPrice * 1460) / sharePrice
                        v3PegRewardInfos[i * 5 + 3] = v3PegRewardInfos[i * 5 + 2].mul(1460).mul(_pegTokenPrice).div(sharePrice);
                        totalApr = totalApr.add(v3PegRewardInfos[i * 5 + 3]);
                        v3PegRewardInfos[i * 5 + 4] = _pegTokenPrice;
                    }
                }
            }
        }
    ...
    function tvls() external view returns (uint256 v3sPoolTvl, uint256 vsharePoolTvl, uint256 boardroomTvl,
        uint256 daoTvl, uint256 insuranceTvl, uint256 lendingTvl, uint256 totalTvl, uint256[] memory v3PegPoolTvls) {
        ITreasury _treasury = ITreasury(treasury);
        IPriceChecker _priceChecker = IPriceChecker(priceChecker);
        ITvlStats _tvlStats = ITvlStats(tvlStats);

        (, v3sPoolTvl, , , , , , ,) = poolStats(v3sPool);
        (, vsharePoolTvl, , , , , , ,) = poolStats(vsharePool);
        totalTvl = v3sPoolTvl.add(vsharePoolTvl);
        ...
        totalTvl = totalTvl.add(boardroomTvl);

        if (address(_tvlStats) == address(0)) {
            daoTvl = _accountTotalDollarValue(_treasury.daoFund());
            insuranceTvl = _accountTotalDollarValue(_treasury.insuranceFund());
        } else {
            daoTvl = _tvlStats.daoTotalDollarValue();
            insuranceTvl = _tvlStats.insuranceTotalDollarValue();
            lendingTvl = _tvlStats.lendingTotalDollarValue();
        }
        totalTvl = totalTvl.add(daoTvl).add(insuranceTvl).add(lendingTvl);

        uint256 _length = v3PegPools.length;
        v3PegPoolTvls = new uint256[](_length);
        for (uint i = 0; i < _length; i++) {
            if (v3PegPools[i] != v3sPool) {
                (, uint256 _v3PegPoolTvl, , , , , , ,) = v3PegPoolStats(i);
                v3PegPoolTvls[i] = _v3PegPoolTvl;
                totalTvl = totalTvl.add(_v3PegPoolTvl);
            }
        }
    }
    ...
    function initialize(
        ...
        v3PegTokens.push(_v3s);
        v3PegOriginalTokens.push(_vvs);
        v3PegPools.push(_v3sPool);
    }
    ...
    function setTvlStats(address _tvlStats) external onlyOwner {
        tvlStats = _tvlStats;
    }

    function setV3PegInfo(address[] memory _v3PegTokens, address[] memory _v3PegOriginalTokens, address[] memory _v3PegPools) external onlyOwner {
        delete v3PegTokens;
        delete v3PegOriginalTokens;
        delete v3PegPools;
        require(_v3PegTokens.length == _v3PegOriginalTokens.length && _v3PegTokens.length == _v3PegPools.length, "array lengths mismatched");
        uint256 _length = _v3PegTokens.length;
        for (uint i = 0; i < _length; i++) {
            v3PegTokens.push(_v3PegTokens[i]);
            v3PegOriginalTokens.push(_v3PegOriginalTokens[i]);
            v3PegPools.push(_v3PegPools[i]);
        }
    }

    function addV3PegInfo(address _v3PegToken, address _v3PegOriginalToken, address _v3PegPool) external onlyOwner {
        v3PegTokens.push(_v3PegToken);
        v3PegOriginalTokens.push(_v3PegOriginalToken);
        v3PegPools.push(_v3PegPool);
    }
