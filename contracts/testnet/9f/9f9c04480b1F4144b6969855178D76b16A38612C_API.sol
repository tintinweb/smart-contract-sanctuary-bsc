...
contract API is Ownable2Step {
	...
	function PirceOfLevelUpgradeManually(
		uint _AccountID,
		uint _XPro,
		uint _LevelTo
	) public view returns (uint pirceOfLevelUpgradeManually_, uint LevelFrom_, uint LevelTo_) {
		return Tuktus.PirceOfLevelUpgradeManually(_AccountID, _XPro, _LevelTo);
	}
...
interface ITuktu {
	...
	function PirceOfLevelUpgradeManually(
		...
		uint _XPro,
		uint _LTo
	) external view returns (uint pirceOfLevelUpgradeManually_, uint LFrom_, uint LTo_);
