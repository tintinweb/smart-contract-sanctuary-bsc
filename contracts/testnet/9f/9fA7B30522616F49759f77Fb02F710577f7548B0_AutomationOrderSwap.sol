...
contract AutomationOrderSwap is AutomationCompatible, Ownable {
    ...
    function _validateOrder(address[] memory _path, uint256 _triggerRatio)
        private
        view
        returns (bool)
    {
        ...
        returns (bool, bytes memory)
    {
        ...
        for (uint256 i = start; i < end; i++) {
            ...
            if ((triggerAboveThreshold)) {
                ...
                if (_validateOrder(path, triggerRatio)) {
        ...
        onlyKeeper
    {
        ...
        require(path0 != address(0) && path1 != address(0), "Invalid order");
        if ((triggerAboveThreshold)) {
            address[] memory path = _getPath(path0, path1, path2);
            require(_validateOrder(path, triggerRatio), "Not executable");
            orderBook.executeSwapOrder(orderId, keeperBalancer);
        }
    }
