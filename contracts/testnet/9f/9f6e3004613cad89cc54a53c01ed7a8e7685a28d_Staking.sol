    ...
    ERC20Upgradeable
{
    ...
    uint public constant decimalPrecision = 100;
    ...
    function initialize(IERC20MetadataUpgradeable _ectAddress, uint256 _unstakeFee)
        public
        initializer
    {
    ...
    function calculateReward(address _account) external view returns (uint){  
        // return uint(balanceOf(_account).mul(totalSupply())).div(
        //     ectAddress.balanceOf(address(this)
        // )).sub(totalStaked[_account]);
        return (balanceOf(_account).mul(ectAddress.balanceOf(address(this))).div(
            totalSupply()
        )).sub(totalStaked[_account]);
    }
    ...
    function unstake(uint256 _share) external isRewardSet {
       ...
       require(what<=(this.calculateReward(msg.sender).add(totalStaked[msg.sender])),"ECT-Staking: Insufficient Balance");
    
        uint unstakeCharges = ((what * unstakeFee) / (100 * decimalPrecision));

        
        ectAddress.transfer(msg.sender, what - unstakeCharges);
        ectAddress.transfer(rewardPoolAddress, unstakeCharges);

        totalStaked[msg.sender] -= _share.mul( totalStaked[msg.sender]).div(balanceOf(msg.sender));
