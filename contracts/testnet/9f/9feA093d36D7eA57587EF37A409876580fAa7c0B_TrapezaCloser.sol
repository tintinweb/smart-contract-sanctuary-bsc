...
contract TrapezaCloser is Ownable {
  ...
  function withdraw(
    ...
    IERC20(_token).safeTransferFrom(address(this), _recipient, _amount);
  }
  ...
  function exchange(uint256 _amount, address _recipient) external {
    ...
    uint256 exchangedCurrency = exchangeAmount(_amount);

    IERC20(principle).approve(_recipient, exchangedCurrency);
    IERC20(principle).safeTransferFrom(address(this), _recipient, exchangedCurrency);
  ...
  function exchangeAmount(uint256 _amount) public view returns (uint256) {
    return _amount.mul(exchangeRate).mul(1e9).div(100);
    // _amount.mul(exchangeRate).mul(10**IERC20(principle).decimals()).div(10**IERC20(FIDL).decimals()).div(100);
  }
}
