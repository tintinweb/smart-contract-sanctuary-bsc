...
contract FeeWizard is ContextUpgradeable {
    ...
    struct TokenInfo {
        IERC20 token; // Address of peg token contract.
        IOracle oracle; // Address of Oracle contract.
        uint256 maxSwap;
        uint256 minSwap;
        uint256 pegPrice;
    ...
    mapping(IERC20 => address[]) public swapRoutes;
    ...
    function checkTokenDuplicate(IERC20 _token) internal view {
    ...
    function addToken(
        ...
        checkTokenDuplicate(token);
        address pegToken = _token;
        address _oracle = _getOracle(pegToken);
        tokenInfo.push(
            TokenInfo({
                token: IERC20(_token),
