...
contract InsuranceNami is ChainlinkClient, ConfirmedOwner {
    ...
    constructor(address payable addressNain) ConfirmedOwner(msg.sender) {
        setChainlinkToken(0xa36085F69e2889c224210F603D836748e7dC0088);
        setChainlinkOracle(0x74EcC8Bdeb76F2C6760eD2dc8A46ca5e581fA656);
        jobId = "ca98366cc7314957b8c012c72f05aeeb";
        fee = (1 * LINK_DIVISIBILITY) / 10;
        _owner = payable(msg.sender);
        _token = IERC20(addressNain);
    }
    ...
    **/
    ...
    event ERequestVolume(bytes32 indexed requestId, uint256 volume);
    event EUpdateStateInsurance();

    /*
     @function
    **/
    function requestVolumeData() public _onlyAdmin returns (bytes32 requestId) {
    ...
    {
        emit ERequestVolume(_requestId, _volume);
        volume = _volume;
    }
    ...
    function buyInsurance(
        ...
        uint256 _expire
    ) public payable returns (InsuranceStruct memory) {
        ...
        // transfer bnb
        if (compareString(_asset, "NAIN")) {
            require(
                _token.balanceOf(msg.sender) >= _value,
            ...
            require(
                _value == msg.value,
                "Value does's not enough, please check again!"
            );
    ...
    function getInsuranceById(uint256 _idInsurance)
        public
        view
        ...
        returns (InsuranceStruct memory)
    {
        if (
            ...
            compareString(insurances[_idInsurance].state, "Payout_Waiting")
        ) {
            revert("State insurance can't not update, please check again!");
        }

        insurances[_idInsurance - 1].state = state;
        insurances[_idInsurance - 1].recognition_date = block.timestamp;

        return insurances[_idInsurance - 1];
    }

    function getTotalInsurance() public view returns (uint256) {
