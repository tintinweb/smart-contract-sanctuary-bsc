...
contract XtudoCoin is ERC20, Ownable {
    ...
    address public teamWallet = 0x2795D92C556208bC5Ce46d3741ce1F88Fe13216a;
    address public playToEarnWalletFee = 0xB22E674b3E5916B659B551a9D8982498efA92A48;
    ...
    uint256 public playToEarnBuyFee = 0;
    uint256 public playToEarnSellFee = 0;
    ...
    constructor() ERC20("XtudoCoin", "XTUD") {
    ...
    function afterPreSale() external onlyOwner {
        ...
        setPlayToEarnBuyFee(1);
        setPlayToEarnSellFee(1);
    ...
    function setPlayToEarnBuyFee(uint256 newValueFee) public onlyOwner {
        playToEarnBuyFee = newValueFee;
        _updateTotalBuyFee();
    }   

    function setPlayToEarnSellFee(uint256 newValueFee) public onlyOwner {
        playToEarnSellFee = newValueFee;
        _updateTotalSellFee();
    }

    function _updateTotalBuyFee() internal {
        totalBuyFee = liquidityFee.add(burnBuyFee).add(teamBuyFee).add(playToEarnBuyFee);
    }

    function _updateTotalSellFee() internal {
        totalSellFee = liquidityFee.add(burnSellFee).add(teamSellFee).add(playToEarnSellFee);
    }
        ...
        if (!swapping || !automatedMarketMakerPairs[to] || !automatedMarketMakerPairs[from]) {
            ...
            if (excludedAccount) {
                ...
                if (burnedTokens >= tSupply.div(2)) {
                    setBurnBuyFee(0);
                    emit BurnFeeBuyStopped(burnedTokens, burnBuyFee);
                    
                    setBurnSellFee(0);
                    emit BurnFeeSellStopped(burnedTokens, burnSellFee);
                }
                ...
                if (automatedMarketMakerPairs[to]) {
                    ...
                    if (playToEarnSellFee > 0) {
                        uint256 tokensToPlayToEarn = amount.mul(playToEarnSellFee).div(100);
                        super._transfer(from, playToEarnWalletFee, tokensToPlayToEarn);
                    }
                } else {
                    if (teamBuyFee > 0) {
                    ...
                    if (playToEarnBuyFee > 0) {
                        uint256 tokensToPlayToEarn = amount.mul(playToEarnBuyFee).div(100);
                        super._transfer(from, playToEarnWalletFee, tokensToPlayToEarn);
                    }
                }
    ...
    event BurnFeeBuyStopped(uint256 burnedTokens, uint256 burnBuyFee);
    event BurnFeeSellStopped(uint256 burnedTokens, uint256 burnSellFee);
}