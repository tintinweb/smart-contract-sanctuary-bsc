...
contract ReedBricks is Context, IBEP20, Ownable, ReedLock {
    ...
    uint256 private _tTotal = 160 * 10**9 * 10**9;
    ...
    constructor() {
        ...
        _moveDelegates(address(0), _delegates[_msgSender()], _rTotal);
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "REED::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "REED::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "REED::delegateBySig: signature expired"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "REED::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "REED::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    function balanceOf(address account) public view override returns (uint256) {
        ...
        if (isLockEnabled == true) {
            return getFreeFund(account);
        }
        return _balanceOf(account);
    }

    function frozenOf(address account) external view returns (uint256) {
        uint256 allBalance = _balanceOf(account);
        uint256 freeBalance = getFreeFund(account);
        return allBalance.sub(freeBalance);
    }
    ...
    function setRewardFeePercent(uint256 RewardFee) external onlyOwner {
        require(
            _liquidityFee.add(RewardFee) <= 20,
            "REED: Ensure Fee Protection 20%"
        );
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner {
        require(
            _rewardFee.add(liquidityFee) <= 20,
            "REED: Ensure Fee Protection 20%"
        );
    ...
    function freeze(address _account, uint256 _ends) external onlyAdmin {
        _freeze(_account, _ends);
    }

    function unfreez(address _account) external onlyAdmin {
        _unfreeze(_account);
    }

    function getFreeFund(address _account) private view returns (uint256) {
        ...
        // Check if freez still Enabled for the account //
        if (frozen.status == false) {
            return balance;
        }

        //Get the interval that has passed //
        uint256 _tPassed = _now.sub(frozen.starts);
        uint256 _intvPassed = _tPassed.div(frozen.intervalPec);

        // Get the total freeable funds from PercentaeInterval//
        uint256 _cent_burst = (frozen.intervalPec).mul(balance).div(10**2);
        uint256 _totalBurst = _cent_burst.mul(_intvPassed);

        if (_totalBurst > balance) {
            return balance;
        }
    ...
    function buyTokens(uint256 tokenAmount, address to) external lockTheSwap {
        require(tokenAmount > 0, "REED: Amount is less than 0");
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniswapV2Router.WETH();
    ...
    uint256 public totalBNBSold = 0; // WEI is the smallest unit of ETH
    uint256 public unitsOneBNBCanBuy = 100000; // How many units of your coin can be bought by 1 ETH?
    bool private onPresale = true;

    //to recieve ETH from uniswapV2Router when swaping
    receive() external payable {
        if (onPresale) {
            totalBNBSold = totalBNBSold + msg.value;
            uint256 amount = msg.value * unitsOneBNBCanBuy;
            require(_rOwned[address(this)] >= amount);
...
contract ReedLock is Ownable {
    ...
    uint256 internal _intervalPec; // in %x of funds locked //
    uint256 internal _intervalHrs;

    struct Frozen {
        uint256 id;
        uint256 starts;
        uint256 ends;
        uint256 intervalPec; // in %x of locked //
        uint256 intervalHrs; // in Hours
    ...
    function setupFreezer(uint256 intervalPec, uint256 intervalHrs)
        external
        onlyAdmin
    {
        if (_intervalHrs != intervalHrs) _intervalHrs = intervalHrs;
        if (_intervalPec != intervalPec) _intervalPec = intervalPec;
    }

    function toggleFreezer() external onlyAdmin {
        if (isLockEnabled == true) isLockEnabled = false;
        if (isLockEnabled == false) isLockEnabled = true;
    }

    function _freeze(address _account, uint256 _ends)
        internal
        canFreez
        onlyAdmin
    {
