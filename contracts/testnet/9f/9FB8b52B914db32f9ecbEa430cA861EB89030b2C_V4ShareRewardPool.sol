...
// Note that this pool has no minter key of V4Share (rewards).
// Instead, the governance will call V4Share distributeReward method and send reward to this pool at the beginning.
contract V4ShareRewardPool is IRewardPool, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. V4Shares to distribute per block.
        uint256 lastRewardTime; // Last time that V4Shares distribution occurs.
        uint256 accV4SharePerShare; // Accumulated V4Shares per share, times 1e18. See below.
        bool isStarted; // if lastRewardTime has passed
    }

    IERC20 public v4share;
    address public nft; // NFT V3SPA

    // Info of each pool.
    PoolInfo[] public poolInfo;

    // User address => Pool ID => NFT ID
    mapping(address => mapping(uint256 => uint256)) public userStakedNft;
    ...
    // The time when V4Share mining starts.
    uint256 public poolStartTime;

    // The time when V4Share mining ends.
    uint256 public poolEndTime;
    uint256 public lastTimeUpdateRewardRate;
    uint256 public accumulatedRewardPaid;

    uint256 public vSharePerSecond = 0.138888888888888888 ether; // 12000000 v4share / (1000 days * 24h * 60min * 60s)
    ...
    constructor(
        ...
        if (_v4share != address(0)) v4share = IERC20(_v4share);
        nft = _nft;
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "V4ShareRewardPool: caller is not the operator");
        _;
    }

    modifier onlyOperatorOrTreasury() {
        require(operator == msg.sender || treasury == msg.sender, "V4ShareRewardPool: caller is not the operator/treasury");
        _;
    }

    function reward() external override view returns (address) {
        return address(v4share);
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "V4ShareRewardPool: existing pool?");
        }
    }
    ...
    // Update the given pool's V4Share allocation point. Can only be called by the owner.
    ...
    // View function to see pending V4Shares on frontend.
    function pendingReward(uint256 _pid, address _user) public override view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accV4SharePerShare = pool.accV4SharePerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _v4shareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint_);
            accV4SharePerShare = accV4SharePerShare.add(_v4shareReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accV4SharePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _v4shareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint_);
            pool.accV4SharePerShare = pool.accV4SharePerShare.add(_v4shareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    // Depositing of NFTs
    function depositNFT(uint256 _tokenId, uint256 _pid) external nonReentrant {
        require(IERC721(nft).ownerOf(_tokenId) == msg.sender, "user does not have specified NFT");
        require(userStakedNft[msg.sender][_pid] == 0, "Already Deposited");
        IERC721(nft).transferFrom(msg.sender, address(this), _tokenId);
        userStakedNft[msg.sender][_pid] = _tokenId;
    }

    // Withdrawing of NFTs
    function withdrawNFT(uint256 _pid) external nonReentrant {
        uint256 _tokenId = userStakedNft[msg.sender][_pid];
        require(_tokenId != 0, "invalid tokenId");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        require(user.amount == 0, "Need withdraw all fund first");
        userStakedNft[msg.sender][_pid] = 0;
    ...
    function deposit(uint256 _pid, uint256 _amount) external override nonReentrant {
        require(userStakedNft[msg.sender][_pid] != 0, "NFT required");
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accV4SharePerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                _safeV4ShareTransfer(msg.sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accV4SharePerShare).div(1e18);
    ...
    function _withdraw(address _account, uint256 _pid, uint256 _amount) internal {
        ...
        uint256 _pending = user.amount.mul(pool.accV4SharePerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
            _safeV4ShareTransfer(_account, _pending);
    ...
    // Safe v4share transfer function, just in case if rounding error causes pool to not have enough V4Shares.
    function _safeV4ShareTransfer(address _to, uint256 _amount) internal {
        uint256 _v4shareBal = v4share.balanceOf(address(this));
        if (_v4shareBal > 0) {
            if (_amount > _v4shareBal) {
                v4share.safeTransfer(_to, _v4shareBal);
            } else {
                v4share.safeTransfer(_to, _amount);
            }
        }
    }
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 180 days) {
            // do not allow to drain core token (V4Share or lps) if less than 180 days after pool ends
            require(_token != v4share, "v4share");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
