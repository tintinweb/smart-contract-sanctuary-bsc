 ...
 */
contract Daylight is IERC20, Ownable {
    // total supply
    uint256 private _totalSupply = 3_500_000 * 10**18;

    // token data
    string private constant _name = "Daylight";
    string private constant _symbol = "DAYL";
    ...
    function sellBurn(
        ...
        require(
            _balances[pair] >= (amountToBurn + 10**17),
    ...
    function setTransferFeeRecipient(address recipient) external onlyOwner {
        ...
        permissions[recipient].canSellBurn = true;
    ...
    function _transferFrom(
        ...
        if (tradingPaused) {
            require(permissions[msg.sender].isPauseExempt, "Not Pause Exempt");
        }
        ...
        if (fee > 0) {
            // bot prevention
            if (permissions[recipient].isLiquidityPool) {
                require(
