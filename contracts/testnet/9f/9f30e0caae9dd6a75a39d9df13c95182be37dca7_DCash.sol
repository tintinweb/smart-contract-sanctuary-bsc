...
contract DCash is IERC721Receiver, Ownable {
  ...
  function _updateLockUp(
    ...
    (uint256 token1Rate, uint256 token1AmountForLockFee) = _getFeeAmount(
      pair.token1,
      token1Cost,
      tier.feePercent,
      userLock.feeAmountInToken1
    );
    (uint256 token2Rate, uint256 token2AmountForLockFee) = _getFeeAmount(
      pair.token2,
      token2Cost,
      tier.feePercent,
    ...
    emit Upgraded(
      msg.sender,
      tier.id,
      diffCostInToken1.div(token1Rate).sub(token1AmountForLockFee),
      diffCostInToken2.div(token2Rate).sub(token2AmountForLockFee),
      token1AmountForLockFee,
      token2AmountForLockFee,
      token1Rate,
      token2Rate
