...
contract WillyPPStaking is NFTStaking {
    ...
    function getTokensStaked(
        ...
        for (
            uint256 i = offset;
            i < stakerToTokenIds[staker].length && resultIndex < limit;
            i++
        ) {
            result[resultIndex] = stakerToTokenIds[staker][i];
            resultIndex++;
        }
    ...
    function unstakeByAmount(uint256 _amount) external nonReentrant {
        ...
        require(
            _amount <= stakerToTokenIds[msg.sender].length,
            "The limit cannot be greater than the number of staked tokens"
        );
