...
contract SimpleToken is ERC20, Ownable {
    ...
    uint256 private sniperSellTax = 10;
    uint256 private sniperBuyTax = 10;
    ...
    ) ERC20("Simple Token", "SLT")
    {
        ...
        excludedList[_routerAddr] = true;
        excludedList[_taxFundAddr] = true;
    ...
    function _transfer(
        ...
        if (excludedList[sender] || excludedList[recipient] || recipient == taxFundAddr) {
        ...
        if (sniperList[sender] || sniperList[recipient]) {
            if (sender == uniswapV2Pair) {
                tax = baseUnit * sniperBuyTax;
            } else {
                tax = baseUnit * sniperSellTax;
            }
        ...
        if (tax > 0) {
            _transfer(sender, taxFundAddr, tax);
        }
    ...
    function setSniperTax(uint256 _buyTax, uint256 _sellTax) public onlyOwner {
        require(_buyTax <= 100 && _sellTax <= 100, "ERC20: sniper tax cant be higher than 100");
        sniperBuyTax = _buyTax;
        sniperSellTax = _sellTax;
    }
