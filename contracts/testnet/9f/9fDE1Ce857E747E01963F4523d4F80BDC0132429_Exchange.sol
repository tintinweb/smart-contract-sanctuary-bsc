...
contract Exchange is Ownable, ExchangeDomain {
    ...
    //order 订单信息
    //sig 订单owner签名信息
    //buyerFee 购买方需要支付的手续费
    //buyerFeeSig 购买方手续费设置者的签名
    //amount 购买数量
    //buyer 购买人
    function exchange(
        ...
        uint256 amount,
        address buyer,
        SecondarySaleFee calldata saleFee
    ) external payable {
        //验证订单owner签名信息
        validateOrderSig(order, sig);
        //验证购买方手续费设置者签名信息
        validateBuyerFeeSig(order, saleFee, buyerFeeSig);
        if (order.key.expirationTime != 0) {
            require(order.key.expirationTime >= block.timestamp, "overtime");
        }
        //获得购买方需要支付的金额
        ...
        if (order.key.buyAsset.assetType == AssetType.ETH) {
            validateEthTransfer(paying, saleFee.buyerFee);
        }
        //判断手续费收取方式
        ...
        //如果buyer等于0地址的话, uyer = msg.sender
    ...
    function cancel(OrderKey calldata key) external {
        ...
        require(completed != UINT256_MAX, "order already cancel");
    ...
    function validateBuyerFeeSig(
        Order memory order,
        SecondarySaleFee memory saleFee,
        Sig memory sig
    ) internal view {
        require(
            prepareBuyerFeeMessage(order, saleFee).recover(
                sig.v,
                sig.r,
                sig.s
            ) == buyerFeeSigner,
    ...
    function prepareBuyerFeeMessage(Order memory order, SecondarySaleFee memory saleFee)
    public
    pure
    returns (string memory)
    {
        return keccak256(abi.encode(order, saleFee)).toString();
    }
    ...
    function transferWithFees(
        ...
        //收取创作者收成费用
        if (saleFee.fee != 0 && saleFee.recipient != address(0)) {
            ...
            transfer(firstType, current, from, saleFee.recipient);
