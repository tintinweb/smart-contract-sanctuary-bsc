...
contract AiMine is Ownable {
    ...
    event BuyMine2(uint256 num ,uint256 endNum);
    ...
    function buyMine(uint256 id,uint256 num)  external {
        ...
        emit BuyMine(msg.sender,detail.currency,id,num,detail.amount.mul(num));
    }

    function buyMine2(uint256 id,uint256 num)  external {
         MineConfig storage detail =MineDetail[id];
      emit BuyMine2(mineNum[id]+num,detail.num);
        require(mineNum[id]+num<=detail.num,"Mine: num error !");
      
        mineNum[id]=mineNum[id]+num;
        emit BuyMine2(mineNum[id],num);
    }
