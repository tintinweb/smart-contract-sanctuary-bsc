...
library Address{
 ...
 contract Shibasheriff is Context, IERC20, Ownable { 
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _staking) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_staking);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _staking) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_staking);
        emit FeesChanged();
    }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(stakingAmt > 0){
            payable(staking).sendValue(stakingAmt);
        }
    }
