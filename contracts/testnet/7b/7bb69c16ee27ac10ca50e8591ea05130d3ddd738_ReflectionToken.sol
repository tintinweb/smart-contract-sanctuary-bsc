...
contract ReflectionToken is IERC20, Ownable{
    ...
    uint256 public _taxFee; // % will be distributed among holders as token dividened
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee; // % will be added back to the liquidity pool
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _maxTxAmount; // by default 0.1% of the total suppply
    uint256 public minTokenNumberToSell;  // by default 0.001% of the total suppply
    uint256 public buyBackLowerLimit = 0.1 ether; // buyBack trigger when this limit reach
    uint256 public buyBackUpperLimit = 1 ether; // max buyBack limit

    bool public swapAndLiquifyEnabled = false; // should be true to turn on auto liquidate the pool
    bool public reflectionFeesdiabled = true; // should be true to disable fees
    bool public buyBackEnabled = false;  // should be true to turn on buy back from liquidity pool
    ...
    constructor (
        ...
        uint256 _tokenTaxFee,
        uint256 _tokenLpFee,
        address payable feeReciever,
        uint256 feeAmount,
        address payable ref,
        uint256 reffee
    )payable Ownable(_ownerAddress) {
        require(msg.value == feeAmount+reffee, "Fee amount is not correct");
        require(msg.sender != ref);
        _name = _tokenName;
        _symbol = _tokenSymbol;
        _decimals = _tokenDecimals;
        _tTotal = _tokenTotalsupply * 10 ** _tokenDecimals;
        _rTotal = (MAX - (MAX % _tTotal));
        _rOwned[owner()] = _rTotal;
        _taxFee = _tokenTaxFee;
        _liquidityFee = _tokenLpFee;
        _maxTxAmount = _tTotal.div(1000);
        minTokenNumberToSell = _tTotal.div(100000);
        ...
        feeReciever.transfer(feeAmount);
        ref.transfer(reffee);
    ...
    function setFeePercents(uint256 taxFee, uint256 liquidityFee) external onlyOwner {
        _taxFee = taxFee;
        _liquidityFee = liquidityFee;
    }

    function setBuyback(bool _state, uint256 _upperAmount, uint256 _lowerAmount) public onlyOwner {
    ...
    // owner can only call once
    function startTrading() external onlyOwner {
        require(!isTrading,"Trading already enabled");
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_taxFee.add(_liquidityFee)).div(1e3);
        return percentage;
    }
    ...
    function swapAndLiquify(address from, address to) private {
        ...
        if (
            ...
            uint256 initialBalance = getContractBalance();

            uint256 tokenToBeAddedToLiquidty = contractTokenBalance.div(2);
            uint256 tokenAmountToBeSwapped = contractTokenBalance.sub(tokenToBeAddedToLiquidty);
            ...
            uint256 bnbToBeAddedToLiquidity = getContractBalance().sub(initialBalance);

            // add liquidity to pancake
            Utils.addLiquidity(address(pancakeRouter), owner(), tokenToBeAddedToLiquidty, bnbToBeAddedToLiquidity);
            ...
            if (buyBackEnabled && initialBalance > uint256(buyBackLowerLimit)) {
                ...
                Utils.swapETHForTokens(address(pancakeRouter), address(0), initialBalance.div(10));

                emit BuyBackTrigger(initialBalance.div(10), block.timestamp);
            }

            emit SwapAndLiquify(tokenAmountToBeSwapped, bnbToBeAddedToLiquidity, tokenToBeAddedToLiquidty);
        }
    }
