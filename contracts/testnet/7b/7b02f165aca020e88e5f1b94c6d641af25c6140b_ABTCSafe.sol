...
contract ABTCSafe is Ownable {
    ...
    function safeABTCTransfer(address _to, uint256 _amount) public onlyOperator {
        uint256 abtcBal = abtc.balanceOf(address(this));
        if (_amount > abtcBal) {
            abtc.transfer(_to, abtcBal);
        } else {
            abtc.transfer(_to, _amount);
        }
    }
