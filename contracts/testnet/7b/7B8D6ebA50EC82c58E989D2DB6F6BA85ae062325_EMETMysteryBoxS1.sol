...
contract EMETMysteryBoxS1 is ERC721Holder, Ownable, RandomGenerator {
    ...
    uint private directLockTime = 60 * 60;
    uint[2] private syntheticLockTime = [60 * 60 * 2, 60 * 60];
    
    uint[8] public cardIds ;
    uint[8] public amounts ;
    uint[8] public directRewards;
    uint[8] public syntheticRewards;
    uint[8] public costs;
    uint[3] public idOfMedal;
    struct BoxBag {
        uint cardId;
        uint amount;
    }
    ...
    mapping (address => UserInfo) userTimeInfo;
    ...
    // user - tokenIdList
    mapping(address => uint[]) public userTokenIdList;
    ...
    function openBox(uint igoBoxId_) external returns (uint) {
        ...
        // IBox721.safeTransferFrom(_msgSender(), blackHole, igoBoxId_);
        emetNFT.burn(igoBoxId_);
        ...
        uint tokenId = emetNFT.mint(msg.sender, cardId);
    ...
    function claimAll() public returns(uint) {
        UserInfo storage user = userTimeInfo[msg.sender]; 
        uint _t;
        uint temp;
    ...
    function directDepositIt(uint tokenId_) public returns(bool) {
        ...
        require(pending != 0, "invalid level");
    ...
    function _synthesisA(uint[] memory tokenIds_) internal returns(uint) {
        ...
        for(uint u=0; u<len; u++) {
            ...
            require(cardId == cardIds[u], "not ABCDE card");
        }
    ...
    function _synthesisB(uint[] memory tokenIds_) internal returns(uint) {
        ...
        for (uint u=0; u<len; u++) {
            cardId = emetNFT.cardIdMap(tokenIds_[u]);
            require(cardId == cardIds[u+1], "not BCDE card");
        }
    ...
    function _synthesisC(uint[] memory tokenIds_) internal returns(uint) {
        ...
        for (uint u=0; u<len; u++) {
            cardId = emetNFT.cardIdMap(tokenIds_[u]);
            require(cardId == cardIds[u+2], "not CDE card");
        }
    ...
    function userCardInBank(address user_) public view returns(uint[] memory, uint[] memory, string[] memory){
        ...
        uint[] memory tokenIdsList = new uint[](len);
        uint[] memory cardIdsList = new uint[](len);
        string[] memory tokenUrlsList = new string[](len);
        uint _tokenId;
        for(uint i=0; i<len; i++) {
            _tokenId = userTokenIdList[user_][i];
            tokenIdsList[i] =  _tokenId;
            tokenUrlsList[i] = emetNFT.tokenURI(_tokenId);
            cardIdsList[i] = emetNFT.cardIdMap(_tokenId);
        }

        return(tokenIdsList, cardIdsList, tokenUrlsList);
    }
    
    function userTimestamp(address user_) public view returns(uint[] memory directList, uint[] memory synthesisList) {
        UserInfo storage user = userTimeInfo[user_];    
        ...
        for(uint x=0; x<len1; x++) {
            synthesisList[x] = user.synTimestamp[x]; 
        }
    }

    function checkUserSynthesisRecord(address user_) public view returns(
        uint[] memory synthesisList,
        uint[] memory level, 
        uint[] memory lastClaimTime,
        uint[] memory lockTime, 
        uint[] memory lockReward, 
        uint[] memory claimed,
        uint[] memory toClaim ) {
        UserInfo storage user = userTimeInfo[user_];    
        ...
        uint _time;
        synthesisList = new uint[](len1);
        level = new uint[](len1);
        lastClaimTime = new uint[](len1);
        lockTime = new uint[](len1);
        lockReward = new uint[](len1);
        claimed = new uint[](len1);
        toClaim = new uint[](len1);

        for(uint i=0; i<len1; i++) {
            _time = user.synTimestamp[i];
            synthesisList[i] = _time;

            level[i] = userSynthesisInfo[user_][_time].level;
            lastClaimTime[i] = userSynthesisInfo[user_][_time].lastClaimTime;
            lockTime[i] = userSynthesisInfo[user_][_time].lockTime;
            lockReward[i] = userSynthesisInfo[user_][_time].lockReward;
            claimed[i] = userSynthesisInfo[user_][_time].claimed;
            toClaim[i] = updataUserSynthesisRewrd(user_, _time);
        }
        return(synthesisList, level, lastClaimTime, lockTime, lockReward, claimed, toClaim);
    }

    function checkUserDirectRecord(address user_) public view returns(
    uint[] memory directList, 
    uint[] memory level, 
    uint[] memory lastClaimTime,
    uint[] memory lockTime, 
    uint[] memory lockReward, 
    ...
    uint[] memory toClaim ) {
        ...
        for(uint i=0; i<len1; i++) {
            ...
            level[i] = userDirectInfo[user_][_time].level;
            lastClaimTime[i] = userDirectInfo[user_][_time].lastClaimTime;
            lockTime[i] = userDirectInfo[user_][_time].lockTime;
            lockReward[i] = userDirectInfo[user_][_time].lockReward;
            claimed[i] = userDirectInfo[user_][_time].claimed;
            toClaim[i] = updataUserDirectRewrd(user_, _time);
        }

        return(directList, level, lastClaimTime, lockTime, lockReward, claimed, toClaim);
    }
