...
contract MarsColony {
  ...
  uint[] public upgradeDuration = [1 days, 2 days , 3 days];
  uint256 public spinPerDayForParcel0 = 1;
  
  uint feePercentage = 0;
  uint spinConvertor = 1e18; // 1 busd = 1 spin
  ...
  function buySpin(uint amount) public {
    uint spin = amount / spinConvertor;
    uint fee = amount * feePercentage / 100;


    BUSD_TOKEN.transferFrom(msg.sender, address(this), fee);
