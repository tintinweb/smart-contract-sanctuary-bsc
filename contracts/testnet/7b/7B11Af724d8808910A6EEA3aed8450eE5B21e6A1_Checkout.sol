...
contract Checkout is DeHubUpgradeable, ICheckout {
  using SafeERC20Upgradeable for IERC20Upgradeable;
  using ERC165CheckerUpgradeable for address;

  uint256 public serviceFee; // 100% in 10000
  address public serviceFeeWallet;
  address public shopWallet;
  IERC20Upgradeable public deHubToken;
  IERC20Upgradeable public busdToken;
  IReceipt public receipt;

  mapping(bytes32 => uint256) private orderId2ReceiptId;
  ...
  function __Checkout_init(
    IERC20Upgradeable _deHubToken,
    IERC20Upgradeable _busdToken,
    uint256 _fee,
    address _treasury,
    address _shopWallet
  ) public initializer {
    ...
    serviceFee = _fee;
    serviceFeeWallet = _treasury;
    shopWallet = _shopWallet;
  }
  ...
  function setDeHubToken(IERC20Upgradeable _token) external override onlyOwner {
    deHubToken = _token;
  }

  function setBUSDToken(IERC20Upgradeable _token) external override onlyOwner {
    busdToken = _token;
  }

  function setTreasuryFee(uint256 _fee) external override onlyOwner {
    serviceFee = _fee;
  }

  function setTreasuryWallet(address _treasury) external override onlyOwner {
    serviceFeeWallet = _treasury;
  }

  function setReceipt(IReceipt _receipt) external override onlyOwner {
    receipt = _receipt;
  }

  /**
   * @notice Purchase product by BNB, transfer BNB and purchase
   *         Caller should call this function with transfering BNB
   * @param _orderId Order id from Moralis
   * @param _metadataURI IPFS hash which stores the information of metadata
   ...
   */
  function purchaseByBNB(string calldata _orderId, string calldata _metadataURI)
    external
    payable
    override
    returns (uint256)
  {
    return _purchaseByBNBFor(msg.sender, _orderId, _metadataURI);
  }

  /**
   * @notice Purchase product by BNB for another wallet, Refer above comments
   * @param _forWhom Destination wallet address who get received
   * @param _orderId Order id from Moralis
  ...
  function purchaseByBNBFor(
    ...
    return _purchaseByBNBFor(_forWhom, _orderId, _metadataURI);
  }

  /**
   * @notice Purchase product by $DeHub, transfer $DeHub and purchase
   *         Prior to call this function, caller should allow Checkout contract
   *         transfer $DeHub tokens.
   * @param _priceInDeHub Product price in $DeHub
   *         Product price can be fake by attacker, but those transactions can
   *         be filtered by DeHub team
   * @param _orderId Order id from Moralis
   * @param _metadataURI IPFS hash of metadata
   * @return Receipt Id minted
   */
  function purchaseByDeHub(
    ...
    return
      _purchaseByDeHubFor(msg.sender, _priceInDeHub, _orderId, _metadataURI);
  }

  /**
   * @notice Purchase product by $DeHub for another wallet
   * @param _forWhom Destination wallet address who get received
   * @param _priceInDeHub Product price in $DeHub
   * @param _orderId Order id from Moralis
   * @param _metadataURI IPFS hash of metadata
  ...
  function purchaseByDeHubFor(
    ...
    return _purchaseByDeHubFor(_forWhom, _priceInDeHub, _orderId, _metadataURI);
  }

  /**
   * @notice Purchase product by $BUSD, transfer $BUSD and purchase
   *         Prior to call this function, caller should allow Checkout contract
   *         transfer $BUSD tokens.
   * @param _priceInBUSD Product price in $BUSD
   *         Product price can be fake by attacker, but those transactions can
   *         be filtered by DeHub team
   * @param _orderId Order id from Moralis
   * @param _metadataURI IPFS hash of metadata
  ...
  function purchaseByBUSD(
    ...
    return _purchaseByBUSDFor(msg.sender, _priceInBUSD, _orderId, _metadataURI);
  }

  /**
   * @notice Purchase product by $BUSD for another wallet
   * @param _forWhom Destination wallet address who get received
   * @param _priceInBUSD Product price in $BUSD
   * @param _orderId Order id from Moralis
   * @param _metadataURI IPFS hash of metadata
  ...
  function purchaseByBUSDFor(
    ...
    return _purchaseByBUSDFor(_forWhom, _priceInBUSD, _orderId, _metadataURI);
  }
  ...
  function _purchaseByDeHubFor(
    address _forWhom,
    uint256 _priceInDeHub,
    string memory _orderId,
    string memory _metadataURI
  ) internal virtual returns (uint256 receiptId) {
    uint256 commission = (_priceInDeHub * serviceFee) / 10000;

    deHubToken.safeTransferFrom(_forWhom, serviceFeeWallet, commission);

    deHubToken.safeTransferFrom(
      _forWhom,
      shopWallet,
      _priceInDeHub - commission
    );

    receiptId = receipt.mint(_forWhom, _priceInDeHub, 0, _metadataURI);

    orderId2ReceiptId[_stringHash(_orderId)] = receiptId;

    emit Purchased(
      _forWhom,
      receiptId,
      uint256(PurchaseTokenType.DeHub),
      _priceInDeHub,
      _orderId,
      _metadataURI
    );
  ...
  function _purchaseByBUSDFor(
    address _forWhom,
    uint256 _priceInBUSD,
    string memory _orderId,
    string memory _metadataURI
  ) internal virtual returns (uint256 receiptId) {
    uint256 commission = (_priceInBUSD * serviceFee) / 10000;

    busdToken.safeTransferFrom(_forWhom, serviceFeeWallet, commission);

    busdToken.safeTransferFrom(_forWhom, shopWallet, _priceInBUSD - commission);

    receiptId = receipt.mint(_forWhom, _priceInBUSD, 0, _metadataURI);
    ...
    emit Purchased(
      _forWhom,
      receiptId,
      uint256(PurchaseTokenType.BUSD),
      _priceInBUSD,
      _orderId,
      _metadataURI
    );
  ...
  function _purchaseByBNBFor(
    address _forWhom,
    string memory _orderId,
    string memory _metadataURI
  ) internal virtual returns (uint256 receiptId) {
    require(msg.value > 0, "Should transfer BNB");
    uint256 commission = (msg.value * serviceFee) / 10000;

    (bool success, ) = serviceFeeWallet.call{value: commission}("");
    require(success);

    (bool success2, ) = shopWallet.call{value: msg.value - commission}("");
    require(success2);

    receiptId = receipt.mint(_forWhom, 0, msg.value, _metadataURI);
  ...
  function _stringHash(string memory _orderId) internal pure returns (bytes32) {
    return keccak256(abi.encodePacked(_orderId));
  }
  ...
  IReceipt
{
  ...
  function mint(
    ...
    return _mintFor(_to, _priceInDeHub, _priceInBNB, _metadataURI);
  }
  ...
  function priceInDeHub(uint256 _receiptId)
    external
    view
    override
    returns (uint256)
  {
    return _receipts[_receiptId].priceInDeHub;
  }

  function priceInBNB(uint256 _receiptId)
    external
    view
    override
    returns (uint256)
  {
    return _receipts[_receiptId].priceInBNB;
  }
...
interface IReceipt {
  ...
  function priceInDeHub(uint256 _receiptId) external view returns (uint256);

  function priceInBNB(uint256 _receiptId) external view returns (uint256);
