...
abstract contract DeSpace_Auction_721 is DeSpace_Container_721 {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    uint256 public aucIds;
    mapping(uint256 => Auction) private _auctions;
    event AuctionCreated(address indexed seller, uint256 indexed aucId);
    event CancelledAuction(uint256 indexed tokenID);
    ...
    function listAuction(
        ...
        if (_duration == 0 || _floorPrice == 0)
            revert DeSpace_Marketplace_DurationAndPriceCheck();
    ...
    function closeAuction(uint256 _aucId) external nonReentrant {
        ...
        (
            ...
            uint256 timeLeft,
            bool isFixedTime
        ) = _timeLeftForBid(_aucId);
        if (isFixedTime && startTime != 0 && timeLeft != 0)
            revert DeSpace_Marketplace_WrongCallPeriod();

        IERC721Upgradeable nft = IERC721Upgradeable(auction.token);
        if (auction.topBid == 0) {
            //not sold
...
abstract contract DeSpace_InstantTrade_721 is DeSpace_Container_721 {
    ...
    event CancelledInstant(uint256 indexed tokenID);
        ...
        nonReentrant
    {
        ...
        if (instant.money == MoneyType.DES) {
            ...
            des_.safeTransfer(wallet, fee);
            des_.safeTransfer(instant.seller, amount - fee);
...
error DeSpace_Marketplace_DurationAndPriceCheck();
