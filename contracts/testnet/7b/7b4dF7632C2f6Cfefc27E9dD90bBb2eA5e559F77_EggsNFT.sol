...
contract EggsNFT is ERC721Enumerable, Ownable {
    ...
    string[4] _states = ["Bronze", "Silver", "Gold", "Platinum"];

    event EggScratched(uint256 indexed state, uint256 zaiId, address owner);
    
    mapping(uint256 => uint256) _maturityTimestamp;
    mapping(uint256 => uint256) _stateIndex;

    mapping(uint256 => bool) _chickenBusy;
    mapping(uint256 => bool) _isCover;
    mapping(uint256 => uint256) _isCoverBy;
    ...
    function mintEgg(
        ...
        _stateIndex[_newItemId] = _state;
        _maturityTimestamp[_newItemId] = block.timestamp + _maturityDuration;
        return (_newItemId);
    }
    ...
    function _isMature(uint256 _tokenId) internal view returns(bool){
        if(_maturityTimestamp[_tokenId] > 0){
            return block.timestamp >= _maturityTimestamp[_tokenId];
        } else{
            return false;
        }
    }

    function updateMaturity(uint256 _tokenId, uint256 _maturityEnd) external onlyAuth {
        _maturityTimestamp[_tokenId] = _maturityEnd;
    } 

    function claimMatureZai(
        uint256 _tokenId,
        string memory _name
        ...
        if(_isCover[_tokenId]){
            _chickenBusy[_isCoverBy[_tokenId]] = false;
        }

        _burn(_tokenId);
        uint256 _zaiId = _generateRandomZai(_name, msg.sender, _stateIndex[_tokenId]);
        
        emit EggScratched(_stateIndex[_tokenId], _zaiId, msg.sender);
        return (_zaiId);
    }
    ...
    function _getPriceToClaim(uint256 _tokenId) internal view returns(uint256){
        if(_maturityTimestamp[_tokenId] > block.timestamp){
            uint256 nbOfHours = ((_maturityTimestamp[_tokenId] - block.timestamp) / 3600) + 1;
    ...
    function burnBzaiForClaimZai(uint256 _tokenId,string memory _name) external returns (uint256) {
        require(msg.sender == ownerOf(_tokenId), "Not yours");
        uint256 _toBurn = _getPriceToClaim(_tokenId);
        IBZAI IBzai = IBZAI(gameAddresses.getBZAIAddress());
        IBzai.transferFrom(msg.sender, address(this), _toBurn);
        require(IBzai.burn(_toBurn),'You need to burn BZAI !');
        if(_isCover[_tokenId]){
            _chickenBusy[_isCoverBy[_tokenId]] = false;
        }
        _burn(_tokenId);
    ...
    function getEggMaturityEnd(uint256 _tokenId) external view returns(uint256){
        return _maturityTimestamp[_tokenId];
    }

    function getEggState(uint256 _tokenId) external view returns (string memory) {
        return _states[_stateIndex[_tokenId]];
    }

    function getStateIndex(uint256 _tokenId) external view returns (uint256){
            return _stateIndex[_tokenId];
    } 

    function coverEggWithChicken(uint256 _eggId,uint256 _chickenId) external {
        require(IERC721(gameAddresses.getChickenAddress()).ownerOf(_chickenId) == msg.sender, "not your chicken");
        require(!_chickenBusy[_chickenId], "Chicken is busy");
            
        _chickenBusy[_chickenId] = true;
        _isCover[_eggId] = true;
        _isCoverBy[_eggId] = _chickenId;

        uint256 _originMaturity = _maturityTimestamp[_eggId];

        _maturityTimestamp[_eggId] = block.timestamp + 1 days;
        require(_originMaturity > _maturityTimestamp[_eggId], "Doesn't need to cover this egg");
    }
