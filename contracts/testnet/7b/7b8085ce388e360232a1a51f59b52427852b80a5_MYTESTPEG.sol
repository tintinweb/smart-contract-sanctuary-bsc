...
interface IMYTESTPEG is IERC20 {
...
contract MYTESTPEG is ReentrancyGuard, IMYTESTPEG {
    
    using SafeMath for uint256;
    using Address for address;

    // token data
    string constant _name = "myTestPeg";
    string constant _symbol = "MTP";
    uint8 constant _decimals = 18;
    uint256 constant precision = 10**18;
    
    // 10 myTestPeg Starting Supply
    ...
    // Underlying Asset
    address public constant _token = 0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47;
    ...
    constructor () {
        ...
        path = new address[](2);
        path[0] = 0x5B3E2Bc1da86ff6235D9eAd4504d598caE77DBCB;
        path[1] = _token;
        
        // Fund
        // _fund = 0x1ebc32b8edc9ce7ab9405c5766c1071994611571;
        _fundingFeeDenominator = 5;
        
        // fee exempt fund + owner + router for LP injection
        isFeeExempt[msg.sender] = true;
        isFeeExempt[0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3] = true;
    ...
    /** Stake Tokens and Deposits myTestPeg in Sender's Address, Must Have Prior Approval */
    ...
    /** Stake Underlying Asset Tokens and Deposits myTestPeg in Recipient's Address, Must Have Prior Approval */
    ...
    /** Sells myTestPeg Tokens And Deposits Underlying Asset Tokens into Seller's Address */
    function sell(uint256 tokenAmount) external override nonReentrant {
        _sell(tokenAmount, msg.sender);
    }
    
    /** Sells myTestPeg Tokens And Deposits Underlying Asset Tokens into Recipients's Address */
    function sell(address recipient, uint256 tokenAmount) external nonReentrant {
        _sell(tokenAmount, recipient);
    }
    
    /** Sells All myTestPeg Tokens And Deposits Underlying Asset Tokens into Seller's Address */
    ...
    /** Deletes myTestPeg Tokens Sent To Contract */
    ...
    /** Purchases myTestPeg Token and Deposits Them in Recipient's Address */
    ...
    function _stakeUnderlyingAsset(uint256 numTokens, address recipient) internal returns (bool) {
        ...
        uint256 prevTokenAmount = IERC20(_token).balanceOf(address(this));
        // move asset into myTestPeg Token
    ...
    function _handleMinting(address recipient, uint256 received, uint256 prevTokenAmount, uint256 oldPrice) private returns(bool) {
        ...
        // revert if under 1
        require(tokensToMint > 0, 'Must Purchase At Least One myTestPeg');
    ...
    /** Price Of myTestPeg in BUSD With 18 Points Of Precision */
    ...
    /** Volume in myTestPeg For A Particular Wallet */
    ...
    event TokenSold(uint256 amountmyTestPeg, uint256 assetsRedeemed, address recipient);
