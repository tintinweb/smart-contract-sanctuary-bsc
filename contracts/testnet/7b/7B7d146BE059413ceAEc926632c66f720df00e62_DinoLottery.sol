...
contract DinoLottery is Context, IBEP20, Ownable {
  ...
  uint256 private _rewardintravel;
  ...
  //Dino Token
  function setintrevel(uint256 amount) public onlyOwner returns (bool) {
    _rewardintravel = amount;
    return true;
  }

  function getintravel() external view returns (uint256) {
    return _rewardintravel;
  }
  ...
  function withdrawfund(uint256 index) external noReentrant() returns (bool) {
    ...
    if ( a.balanceOf(address(this)) < reward ) {
    reward = a.balanceOf(address(this));
    }
  ...
  function claimowner() external returns (bool) {
    ...
    _inmatch = true;
    _lastpaid = msg.sender; 
    _currentpool = block.timestamp.sub(_timer).div(_rewardintravel);
    _timer = block.timestamp.add(300);
  ...
  function getcurrentreward() external view returns (uint256) {
    ...
    result = block.timestamp.sub(_timer).div(_rewardintravel);
    result = result * 10 ** 18;
    }
    return result;
  }
