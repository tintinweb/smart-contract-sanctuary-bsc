...
contract DividendDistributor is IDividendDistributor {
    ...
    address WBNB = 0xdfEA3031511Fb5a5379C3165Af2cC1eeFd815BF4;
    IDEXRouter router;
...
contract Star is IBEP20, Auth {
    ...
    string constant _name = "Star";
    string constant _symbol = "$STAR";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1_000_000_000 * 10**6 * 10**_decimals;
    ...
    constructor () Auth(msg.sender) { 
        ...
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(); // Testnet
        router = IDEXRouter(_dexRouter);
        pair = IDEXFactory(router.factory()).createPair(WBNB, address(this));
        _allowances[address(this)][address(router)] = uint256(-1);

        distributor = new DividendDistributor(address(_dexRouter));
