...
contract ManagerAuction is Initializable, AccessControlUpgradeable, PausableUpgradeable, ERC721HolderUpgradeable {
	...
	function initialize(uint256 _platformFee, address _treasuryAddress) public virtual initializer {
	...
	modifier onlyAdmin() {
        require(
            ...
            "Xanalia: caller is not the admin"
        );
        _;
    }
	...
	function setPaymentMethod(address _token, bool _status) external onlyAdmin returns (bool) {
	 ...
	 */
	function withdrawFunds(address payable _beneficiary, address _tokenAddress) external onlyAdmin whenPaused {
	...
	function _payBidAuction(uint256 _bidAuctionId) internal {
		...
		uint256 royaltyFee = _getRoyaltyFee(bidAuction.tokenAddress, bidAuction.tokenId);
		...
		if (royaltyFee > 0 || platformFee > 0) {
			_totalEarnings = (_bidPrice * (BASE_DENOMINATOR - royaltyFee - platformFee)) / BASE_DENOMINATOR;
		}

		if (creator != address(0) && royaltyFee > 0) {
			_paid(_paymentToken, creator, (_bidPrice * royaltyFee) / BASE_DENOMINATOR);
		}
		if (platformFee > 0) {
			_paid(_paymentToken, treasuryAddress, (_bidPrice * platformFee) / BASE_DENOMINATOR);
		}
	...
	function _getCreator(address _tokenAddress, uint256 _tokenId) internal view returns (address) {
		try IXanaliaNFT(_tokenAddress).getCreator(_tokenId) returns (address _creator) {
			return _creator;
		} catch {}
		return address(0);
	}

	function _getRoyaltyFee(address _tokenAddress, uint256 _tokenId) internal view returns (uint256) {
		try IXanaliaNFT(_tokenAddress).getRoyaltyFee(_tokenId) returns (uint256 _royaltyFee) {
			return _royaltyFee;
		} catch {}
		return 0;
	}
...
contract AuctionDex is ManagerAuction {
	using SafeERC20Upgradeable for IERC20Upgradeable;
	using AddressUpgradeable for address payable;

	function initialize(uint256 _platformFee, address _treasuryAddress) public virtual override initializer {
		ManagerAuction.initialize(_platformFee, _treasuryAddress);
	}
	...
	function createAuction(
		...
		address itemOwner = hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ? _itemOwner : msg.sender;

		require(IERC721Upgradeable(_tokenAddress).ownerOf(_tokenId) == itemOwner || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Not-item-owner");

		_auctionId = totalAuctions;

		tokenOnAuction[_tokenAddress][_tokenId] = true;
		if (!IERC721Upgradeable(_tokenAddress).isApprovedForAll(itemOwner, address(this))) {
			IXanaliaNFT(_tokenAddress).setApprovalForAll(itemOwner, address(this), true);
		}
		IERC721Upgradeable(_tokenAddress).safeTransferFrom(itemOwner, address(this), _tokenId);

		Auction storage newAuction = auctions[_auctionId];

		newAuction.owner = itemOwner;
	...
	function bidAuction(
		...
		if (auctionBidCount[_auctionId] == 0) {
	...
	function editBidAuction(uint256 _bidAuctionId, uint256 _price, uint256 _expireTime) external payable whenNotPaused returns (uint256) {
		BidAuction storage editPreviousBidAuction = bidAuctions[_bidAuctionId];
		Auction storage currentAuction = auctions[editPreviousBidAuction.auctionId];
		require(auctionBidCount[editPreviousBidAuction.auctionId] > 0, "Invalid-bid");
		require(editPreviousBidAuction.bidPrice > 0, "Bid-not-exist");
		require(msg.sender == editPreviousBidAuction.bidder, "Not-owner-of-bid-auction");
		require(
			block.timestamp >= currentAuction.startTime && block.timestamp <= currentAuction.endTime,
			"Not-in-auction-time"
		);
		require(block.timestamp < _expireTime, "Invalid-expire-time");
		require(editPreviousBidAuction.status, "Bid-cancelled");

		require(
			_price >= (bidAuctions[auctionHighestBidId[_bidAuctionId]].bidPrice * MINIMUM_BID_RATE) / BASE_DENOMINATOR,
			"Price-bid-less-than-max-price"
		);

		auctionBidCount[editPreviousBidAuction.auctionId] += 1;

		if (editPreviousBidAuction.paymentToken == address(0)) {
			require(msg.value >= _price - editPreviousBidAuction.bidPrice, "Invalid-amount");
		} else {
			IERC20Upgradeable(editPreviousBidAuction.paymentToken).safeTransferFrom(
				editPreviousBidAuction.bidder,
				address(this),
				_price - editPreviousBidAuction.bidPrice
			);
		}

		editPreviousBidAuction.status = false;
		uint256 oldBidAuctionId = _bidAuctionId;

		bidAuctions[totalBidAuctions] = editPreviousBidAuction;
		bidAuctions[totalBidAuctions].status = true;
		bidAuctions[totalBidAuctions].bidPrice = _price;
		bidAuctions[totalBidAuctions].expireTime = _expireTime;
		_bidAuctionId = totalBidAuctions;

		currentAuction.listBidId.push(totalBidAuctions);

		auctionHighestBidId[editPreviousBidAuction.auctionId] = totalBidAuctions;
	...
	function cancelAuction(uint256 _auctionId) external whenNotPaused returns (uint256) {
		...
		Auction storage currentAuction = auctions[_auctionId];
		require(tokenOnAuction[currentAuction.tokenAddress][currentAuction.tokenId], "Auction-cancelled");
	...
	function cancelBidAuction(uint256 _bidAuctionId) external whenNotPaused returns (uint256) {
		...
		require(msg.sender == currentBid.bidder, "Not-owner-of-bid-auction");
		...
		if (currentBid.paymentToken == address(0)) {
			...
			IERC20Upgradeable(currentBid.paymentToken).safeTransfer(currentBid.bidder, currentBid.bidPrice);
		}
	...
	function reclaimAuction(uint256 _auctionId) external whenNotPaused {
		...
		require(currentAuction.owner == msg.sender, "Not-auction-owner");

		require(!tokenOnAuction[currentAuction.tokenAddress][currentAuction.tokenId], "Auction-not-cancelled");
	...
	function adminMigratePushNFT(
		...
		require(startId < totalAuctions, "StartId-more-than-maxid");
		if (endId > totalAuctions - 1) {
			endId = totalAuctions - 1;
		}
		for (uint256 i = startId; i <= endId; i++) {
			Auction memory auction = auctions[i];
			if (IERC721Upgradeable(auction.tokenAddress).ownerOf(auction.tokenId) == address(this)) {
				IERC721Upgradeable(auction.tokenAddress).safeTransferFrom(address(this), newMarket, auction.tokenId);
			}
		}
	}

	function sendTokenToNewContract(address _token, address _newContract) external onlyAdmin whenPaused {
