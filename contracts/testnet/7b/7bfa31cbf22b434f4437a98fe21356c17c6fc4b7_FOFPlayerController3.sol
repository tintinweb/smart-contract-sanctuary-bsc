...
contract FOFPlayerController2 is Ownable {
    ...
    function mergeAdvancedVehicle(uint256[] memory tokenIds) public returns (uint256) {
        ...
        // require(FPN.ownerOf(tokenIds[0]) == msg.sender && vehicles[tokenIds[0]].holder == msg.sender , "You are not the owner of the vehicle");
        require(FPN.ownerOf(tokenIds[0]) == msg.sender && FPN.ownerOf(tokenIds[1]) == msg.sender , "You are not the owner of the vehicle");

        require(FPN.NFTTypes(tokenIds[0]) == FPN.NFTTypes(tokenIds[1]), "The vehicle types don't match");

        uint256 types = vehicles[tokenIds[0]].types + 1;
...
contract FOFPlayerController3 is Ownable {
    ...
    constructor() {
        ...
        FPC2 = FOFPlayerController2(payable(0xD1A9C1355b731D96C7b0f3825a1155cc9c657624));
    ...
    function sellMyNFT(uint256 tokenId, uint256 price) public {
        ...
        require(tokenIdToStatus[tokenId] == 0,"This user dont hava a hero ");
    ...
    function cancelSellNFT(uint256 tokenId) public {
        ...
        require(tokenIdToStatus[tokenId] == 1 && tokenIdToPrice[tokenId] > 0,"This user dont hava a hero ");

        FPN.setNFTLock(tokenId, false);

        tokenIdToPrice[tokenId] = 0;

        tokenIdToStatus[tokenId] = 0;
    ...
    // function getSells() public view returns(uint256[] memory sellNFTIDs, uint256[] memory prices, address[] memory owners) {
    ...
    //     return (sellNFTIDs,prices,owners);
	// }
    ...
    function getSells(address user, uint256 NFTType) public view returns(uint256[] memory sellNFTIDs, uint256[] memory indexs, uint256[] memory prices, uint256[] memory types, uint256[] memory attributes, address[] memory owners) {
        ...
        for(i = 0; i < sellTokenIds.length; i++){
            if(user == address(0)){
                ...
                if(FPN.ownerOf(sellTokenIds[i]) == user){
        ...
        for(i = 0; i < counter; i++){
            ...
            indexs[i] = tokenIdToSellIndexs[sellNFTIDs0[i]];
    ...
    // function getSellsByType(uint256 NFTType) public view returns(uint256[] memory sellNFTIDs, uint256[] memory prices, address[] memory owners) {
    ...
    // function getUserSells(address user) public view returns(uint256[] memory sellNFTIDs, uint256[] memory prices) {
    ...
    //     return (sellNFTIDs,prices);
	// }

    // function getUserSellsByType(address user, uint256 NFTType) public view returns(uint256[] memory sellNFTIDs, uint256[] memory prices) {
