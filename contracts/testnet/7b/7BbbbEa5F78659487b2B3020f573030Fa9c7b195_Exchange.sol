...
contract Exchange is EIP712, Ownable, Pausable {
    using ECDSA for bytes32;

    string public constant name = "Exchange";
    address public constant MFB = 0xFe7cB8c9B98075cd3bc958559cF563398F7d78D5;
     ...
     */
    function createOrder(Order memory order, bytes calldata signature) external whenNotPaused {
        // verify signature
     ...
     */
    function completeFixedSale(Order memory order) external payable whenNotPaused {
        // get unique key
        bytes32 hashKey = genHashKey(order);
     ...
     * Emits a {Buy} event, indicating the seller, token address and token id.
     */
    function completeBidding(BidOrder memory bidOrder, bytes calldata signature) external whenNotPaused {
        ...
        // transfer MFB from buyer to seller
        IWETH(MFB).transferFrom(bidOrder.buyer, msg.sender, bidOrder.bidAmount);

        // transfer token from seller to buyer
        IERC721(bidOrder.tokenAddress).transferFrom(msg.sender, bidOrder.buyer, bidOrder.tokenId);
    ...
    /**
     * @dev Unpause the contract (normal state) by onlyOwner.
     *
     * Requirements:
