...
contract VCGamersStaking is ReentrancyGuard {
    ...
    function harvest(uint256 index) public nonReentrant {
        ...
        if(commontimestamp == stakersRecord[msg.sender][index].unstaketime){
    ...
    function realtimeRewardPerSecond(address user, uint256 index) public view returns (uint256,uint256) {
        ...
        uint256 commontimestamp;
            if (
                ...
                uint256 rewardPerToken = Bonus.div(stakedToken);
                uint256 rewardPerSecond = rewardPerToken.div(Duration[4]);
                ...
                if(block.timestamp < stakersRecord[user][index].unstaketime) {
                    commontimestamp = block.timestamp;
                } else {
                    commontimestamp = stakersRecord[user][index].unstaketime;
                }
                stakedSec = commontimestamp - lastharvesttime;

                ret = rewardPerSecond.mul(stakedSec).mul(stakersRecord[msg.sender][index].amount);
            }
