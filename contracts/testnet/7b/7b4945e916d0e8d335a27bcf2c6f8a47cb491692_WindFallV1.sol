...
contract WindFallV1 is Context, IBEP20, Ownable {
    ...
    uint16[BONUS_LINES_COUNT] private ref_bonuses = [150, 100, 50, 30, 20, 10]; 
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        address payable addr1 = address(uint160(ceo));
        address payable addr2 = address(uint160(dev1));
        address payable addr3 = address(uint160(dev2));
    ...
    function setCEO(address newCEO)  public onlyOwner returns (bool) {       
        ceo = newCEO;
        return true;
    }

   	function setDev1(address newDev)  public onlyOwner returns (bool) {       
        dev1 = newDev;
        return true;
    }

	function setDev2(address newDev)  public onlyOwner returns (bool) {       
        dev2 = newDev;
        return true;
    }
