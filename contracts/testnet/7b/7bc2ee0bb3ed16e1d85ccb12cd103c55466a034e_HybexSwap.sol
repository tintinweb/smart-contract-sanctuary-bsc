    ...
    ReentrancyGuardUpgradeable
{
    ...
    // preCappedAmount - amount of token already used/capped
    uint256 public preCappedAmount;
    ...
    event UpdateStakeAmount(address[] indexed _userAddress, uint256[] stakedAmount);
  ...
  */
    ...
    function swap(uint256 _capAmount) external nonReentrant whenNotPaused {
        ...
        uint256 totalCappedAmount = preCappedAmount + _capAmount;

        //calculating tier4 requirements
        if (
            _stakedAmount < tier3StakeAmount ||
            preCappedAmount > (tier1 + tier2 + tier3)
        ) {
            preCappedAmount += _capAmount;
            emit SwapEmit(msg.sender, 0, 0, 0, _capAmount);
        }
        ...
        if (
            ...
            if (totalCappedAmount < tier3) {
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, 0, 0, _capAmount, 0);
            } else if (preCappedAmount <= tier3 && totalCappedAmount > tier3) {
                uint256 tier3Cap = tier3 - preCappedAmount;
                uint256 tier4Cap = totalCappedAmount -
                    tier3Cap -
                    preCappedAmount;
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, 0, 0, tier3Cap, tier4Cap);
            } else if (preCappedAmount > tier3) {
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, 0, 0, 0, _capAmount);
            }
        }
        ...
        if (
            ...
            if (preCappedAmount > (tier2 + tier3)) {
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, 0, 0, 0, _capAmount);
            } else if (totalCappedAmount <= tier2) {
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, 0, _capAmount, 0, 0);
            } else if (
                preCappedAmount <= tier2 && totalCappedAmount <= (tier2 + tier3)
            ) {
                uint256 tier2Cap = tier2 - preCappedAmount;
                uint256 tier3Cap = totalCappedAmount -
                    tier2Cap -
                    preCappedAmount;
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, 0, tier2Cap, tier3Cap, 0);
            } else if (
                preCappedAmount <= tier2 && totalCappedAmount > (tier2 + tier3)
            ) {
                uint256 tier2Cap = tier2 - preCappedAmount;
                uint256 tier3Cap = tier3;
                uint256 tier4Cap = totalCappedAmount -
                    tier3Cap -
                    tier2Cap -
                    preCappedAmount;
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, 0, tier2Cap, tier3Cap, tier4Cap);
            } else if (
                preCappedAmount > tier2 && totalCappedAmount <= (tier2 + tier3)
            ) {
                preCappedAmount += _capAmount;
                ...
                uint256 tier3Cap = (tier2 + tier3) - preCappedAmount;
                uint256 tier4Cap = totalCappedAmount -
                    tier3Cap -
                    preCappedAmount;
                preCappedAmount += _capAmount;
        ...
        if (_stakedAmount >= tier1StakeAmount) {
            if (preCappedAmount > (tier1 + tier2 + tier3)) {
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, 0, 0, 0, _capAmount);
            } else if (totalCappedAmount <= tier1) {
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, _capAmount, 0, 0, 0);
            } else if (
                preCappedAmount <= tier1 && totalCappedAmount <= (tier1 + tier2)
            ) {
                uint256 tier1Cap = tier1 - preCappedAmount;
                uint256 tier2Cap = totalCappedAmount -
                    tier1Cap -
                    preCappedAmount;
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, tier1Cap, tier2Cap, 0, 0);
            } else if (
                preCappedAmount > tier1 && totalCappedAmount <= (tier1 + tier2)
            ) {
                preCappedAmount += _capAmount;
                ...
                preCappedAmount <= tier1 &&
                totalCappedAmount > (tier1 + tier2) &&
                totalCappedAmount <= (tier1 + tier2 + tier3)
            ) {
                uint256 tier1Cap = tier1 - preCappedAmount;
                uint256 tier2Cap = tier2;
                uint256 tier3Cap = totalCappedAmount -
                    tier2Cap -
                    tier1Cap -
                    preCappedAmount;
                preCappedAmount += _capAmount;
                emit SwapEmit(msg.sender, tier1Cap, tier2Cap, tier3Cap, 0);
            } else if (
                preCappedAmount <= tier1 &&
                totalCappedAmount > (tier1 + tier2 + tier3)
            ) {
                ...
                uint256 tier3Cap = (tier1 + tier2 + tier3) - preCappedAmount;
                uint256 tier4Cap = totalCappedAmount -
                    tier3Cap -
                    preCappedAmount;
                preCappedAmount += _capAmount;
                ...
                uint256 tier2Cap = (tier1 + tier2) - preCappedAmount;
                uint256 tier3Cap = totalCappedAmount -
                    tier2Cap -
                    preCappedAmount;
                preCappedAmount += _capAmount;
  ...
  */
    function updateStakeAmount(address[] memory _userAddress, uint256[] memory stakedAmount)
        public
        onlyOwner
    {
        require(_userAddress.length == stakedAmount.length,"Unequal paramaters length");
  ...
  */
    ...
    function reduceCap(uint256 _amount)public{
        preCappedAmount -= _amount;
    }
}
