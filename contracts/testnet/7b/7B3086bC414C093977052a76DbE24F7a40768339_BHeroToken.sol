...
// MINTABLE TOKEN https://testnet.bscscan.com/address/0x73A59d7b4D7d7316F0D933FA9739d9aAaAdEf98B#code
    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => mapping(uint256 => uint256[]))
        private tokenRandomRequests;

    //---------------------------------------------------//
    function initialize() public initializer {
        __ERC721_init("MYSTERY BOX", "MYSBOX");
        __AccessControl_init();
        __Pausable_init();
        __UUPSUpgradeable_init();
        coinToken = IERC20(0x73A59d7b4D7d7316F0D933FA9739d9aAaAdEf98B);
    ...
    function updateToken(uint256 id, uint256 details) internal {
        ...
        require(
            BHeroDetails.decodeId(details) == id &&
                BHeroDetails.decodeIndex(details) ==
                BHeroDetails.decodeIndex(currentDetails),
            "Invalid details"
        );
        tokenDetails[id] = details;
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            result[i] =
                tokenDetails[ids[i]] |
                (uint256(hasPendingRandomization(ids[i]) ? 1 : 0) << 250);
        }
        return result;
    }
    ...
    //? SET TOKEN DETAILS
    function setTokenDetails(uint256 id, uint256 details)
        external
    ...
    function mint(uint256 count, uint256 category) external {
        ...
        if (category == 0 || category == 1) {
            if (category == 0) {
                ...
                coinToken.transferFrom(to, address(this), cost); //? coin token transfer
            } else {
                uint256 cost = design.getSenMintCost() * count;
                senToken.transferFrom(to, address(this), cost); //? sen token transfer
            }
            heroCount = count;
            requestCategory = 0;
        } else if (category == 2) {
            ...
            senToken.transferFrom(to, address(this), cost); //? superbox with sen token transfer
            heroCount = 3;
            requestCategory = 1;
        }
    ...
    function _createTokenRequest(
        ...
        tokenRequests[to].push(
            CreateTokenRequest(details, uint16(count), uint8(rarity))
        );
        emit TokenCreateRequested(
            to,
            targetBlock,
            uint16(count),
            uint8(rarity),
    ...
    /** Gets the number of tokens that can be processed at the moment. */
    function _getProcessableTokens(address to, uint256 currentBlock)
        internal
        view
        returns (uint256)
    {
    ...
    function processTokenRequests() external {
        ...
        for (uint256 i = requests.length; i > 0; --i) {
            ...
            uint256 amount = available < request.count
                ? available
                : request.count;
    ...
    function upgrade(uint256 baseId, uint256 materialId) external {
        ...
        uint256 cost = getHeroCostByDetails(
            baseDetails,
            design.getUpgradeCost(rarity, baseLevel - 1)
        );
    ...
    function randomizeAbilities(uint256 id) external {
        ...
        uint256 cost = getHeroCostByDetails(
            details,
            design.getRandomizeAbilityCost(rarity, times)
        );

        coinToken.transferFrom(to, address(this), cost);

        // Increase counter;
        tokenDetails[id] = BHeroDetails.increaseRandomizeAbilityCounter(
            details
        );
    ...
    function processRandomizeAbilities(uint256 id) external {
        ...
        uint256 requestDetails = tokenRandomRequests[id][
            RANDOMIZE_ABILITY_TOPIC
        ][0];
    ...
    function getHeroCostByDetails(uint256 details, uint256 cost)
        public
        pure
        returns (uint256)
    {
