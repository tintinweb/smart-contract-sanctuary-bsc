 ...
 */
contract BNBRegistrarControllerV2 is Ownable, EIP712 {
    using StringUtils for *;

    uint constant public MIN_REGISTRATION_DURATION = 28 days;
    address public sid_signer;
    ...
    event NameRegistered(string label0, bytes32 indexed label, address indexed owner, uint cost, uint expires);
    event NameRenewed(string label0, bytes32 indexed label, uint cost, uint expires);
    ...
    constructor(
                ...
                uint _maxCommitmentAge) EIP712("SpaceID", "1.0.0") {
        require(_maxCommitmentAge > _minCommitmentAge);
        sid_signer = _sid_signer;
        base = _base;
        prices = _prices;
    ...
    function rentPrice(string memory name, bytes32 label, uint256 duration)
        public
        view
    ...
    function rentPrice(uint name_len, bytes32 label, uint256 duration)
        public
        view
        returns (IPriceOracle.Price memory price)
    {
        price = prices.price(name_len, base.nameExpires(uint256(label)), duration);
    }
    ...
    function register(uint name_len, 
                      bytes32 label, 
                      address owner, 
                      uint duration, 
                      bytes32 secret, 
                      bytes calldata signature) external payable {
      registerWithConfig(name_len, label, owner, duration, secret, address(0), address(0), signature);
    }
	...
	require(
            _verify(
                _hash(name_len, label, owner, duration, secret, resolver, addr),
                signature
            ),
            "Invalid signature"
        );

	IPriceOracle.Price memory price = rentPrice(name_len, label, duration);
        ...
        emit NameRegistered(bytes32ToString(label), label, owner, cost, expires);
    }

    function renew(bytes32 label, uint name_len, uint duration) external payable {
        IPriceOracle.Price memory price = rentPrice(name_len, label, duration);
        ...
        emit NameRenewed(bytes32ToString(label), label, cost, expires);
    }
    ...
    function _hash(
        ...
        return
            _hashTypedDataV4(
        		...
        		abi.encode(
        		    keccak256(
        			"SID(uint name_len,bytes32 label,address owner,uint duration,bytes32 secret,address resolver,address addr)"
        			),
			    name_len,
			    label,
			    owner,
    ...
    function _verify(bytes32 hash, bytes calldata signature)
        ...
        returns (bool)
    {
        return ECDSA.recover(hash, signature) == sid_signer;
    }
