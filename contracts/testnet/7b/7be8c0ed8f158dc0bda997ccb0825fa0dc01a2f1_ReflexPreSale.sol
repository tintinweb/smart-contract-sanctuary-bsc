...
abstract contract Crowdsale {
   ...
   * @param _minimumBuyLimit Address of the token being sold
   * @param _maximumBuyLimit Address of the token being sold
   */
  constructor(uint256 _rate, address _wallet, IERC20 _token, uint256 _minimumBuyLimit, uint256 _maximumBuyLimit) {
    ...
    wallet = _wallet;
    token = _token;
    minimumBuyLimit = _minimumBuyLimit;
    maximumBuyLimit = _maximumBuyLimit;
  }
  ...
  function buyTokens(address _beneficiary) internal {
    ...
    // require(tokens >= minimumBuyLimit && tokens <= maximumBuyLimit , "Buy Token: Token amount exceeds minimum and maximum limit.");
...
contract ReflexPreSale is Crowdsale, Pausable, FinalizableCrowdsale {
    ...
    constructor(
      ...
      uint256 minimumBuyLimit, // minimum buy limit 
      uint256 maximumBuyLimit, // maximum buy limit
      ...
      Crowdsale(rate, wallet, token, minimumBuyLimit, maximumBuyLimit)
    {
  ...
  function dexListing() external onlyOwner whenNotPaused {
    uint256 bnbForLiquidity = (address(this).balance * liquidityPercentage) / 10 ** 2;
    uint256 tokenForLiquidity = bnbForLiquidity * listingRate;
    // add liquidity to uniswap
    addLiquidity(tokenForLiquidity, bnbForLiquidity, address(this));
  ...
  function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address account) private {
    ...
    token.transferFrom(address(this), address(uniswapV2Router), tokenAmount);
