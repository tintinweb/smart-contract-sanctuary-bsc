...
interface YiMainPool {
    function allocAirDrop(uint256 _stake) external returns (uint256);
}
...
contract StakePool is Ownable {
    ...
    function recordStake(uint256 _laststake) internal {
        uint _now = now;
        ...
        stakeTimeStamp[_now].stakeNum = _laststake;
    ...
    function airDrop() external lock isOpen {
        ...
        lastAirDrop = YiMainPool(mainPool).allocAirDrop(_stake);
    ...
    //收益计算
    //质押收益计算方式：（两次质押操作时间间隔 / 1天） * （本次操作前的质押量 / 质押池总量） *  （上一次空投量）
    function calcIncome(uint256 _stakePrice, uint256 _delay, uint256 StakeCount, uint _lastAirDrop) internal pure returns(uint256) {
        // uint256 StakeCount = this.getLPOnStake();
        if (_lastAirDrop == 0 || _stakePrice == 0 || StakeCount == 0) {
            return 0;
        }
        // return (_delay / 1 days) * (_stakePrice / StakeCount) * lastAirDrop;
        return (_delay * _stakePrice * _lastAirDrop) / (86400 * StakeCount);
    }
    ...
    function getIncome(uint _idx, uint256 _time) internal  view returns (uint256 _income) {
        ...
        if (_time - stake[_idx].lastTime > 0 && _time - stakeTimeStampList[stakeTimeStampList.length - 1] > 0 && index > 0) {
    ...
    //质押更新
    function updateStaker(bool isadd, uint _idx, uint256 _stakeNum, uint256 _time) internal returns (uint256 _income){
        ...
        if (ownerStake[_staker] == 0) {
            if (stake.length == 0) {
                ...
                if (_staker == stake[0].staker) {
                    _income = updateStaker(true, 0, _stakeNum, now);
                } else {
                    stake.push(Stake(_staker, _stakeNum, now, now, 0, 0));
                    ownerStake[_staker] = stake.length - 1;
                }
            ...
            if (_staker == stake[_idx].staker) {
                _income = updateStaker(true, _idx, _stakeNum, now);
            } else {
    ...
    function subStake(uint256 _stakeNum) external lock isOpen returns(uint256 _income) {
        address _staker = _msgSender();
        ...
        _income = updateStaker(false, _idx, _stakeNum, _now);
        // settlement(_staker);
    ...
    function queryIncome() external view isOpen returns(uint256,uint256,uint256,uint256){
        ...
        uint256 _incomePer = calcIncome(stake[_idx].Amount, 1,this.getLPOnStake(), lastAirDrop); //每秒收益
    ...
    function withdraw(address _sender, uint256 _amount) external lock isOpen returns(bool) {
        ...
        uint _time = now;
        uint256 _incomeNow = getIncome(_idx, _time);//calcIncome(stake[_idx].Amount, now - stake[_idx].lastTime);
        _balance += _incomeNow;
        require(_amount <= _balance, "S2020: Person Insufficient balance ...");
        stake[_idx].balance = _balance - _amount;
        stake[_idx].allIncome += _incomeNow;
        stake[_idx].lastTime = _time;
