...
import "../tokens/ERC721OnmeUpgradeable.sol";

/** Baseon Rarible with some modification */
contract ERC721OnmeFactoryC2 is Ownable {
	address public transferProxy;
	address public beacon;

	event CreateERC721OnmeProxy(address deployed);
	...
	function createToken(
		...
		bytes memory code = getCreationBytecode(getData(name, symbol, baseURI));
		address deployed = Create2.deploy(0, salt, code);
		ERC721OnmeUpgradeable(deployed).transferOwnership(_msgSender());

		emit CreateERC721OnmeProxy(deployed);
	}
	...
	function getData(
		...
		return
			abi.encodeWithSelector(
				ERC721OnmeUpgradeable(address(0)).__ERC721Onme_init.selector,
				_name,
				_symbol,
				baseURI,
				transferProxy
