...
contract MetaPortal is MetaVerifier, Ownable {

    address public tokenMts;
    address public tokenMtt;
    address public tokenBox;
    ...
    event TokenBoxUpdated(address newBox);
    ...
    event TokenDeposit(address indexed tokenAddress, address indexed user, uint256 amount);
    event BoxClaimed(address indexed user, uint256 boxId, uint256 amount, uint256 nonce);

    constructor(address _signer, address _tokenMts, address _tokenMtt, address _tokenBox) MetaVerifier(_signer) {
        tokenMts = _tokenMts;
        tokenMtt = _tokenMtt;
        tokenBox = _tokenBox;
    }
    ...
    function updateTokenBox(address _newBox) external onlyOwner {
        tokenBox = _newBox;
        emit TokenBoxUpdated(_newBox);
    }
     ...
     * @dev Function to return the message hash that will be signed by the signer
     */
    function getIdAmountMessageHash(
        address account,
        uint256 nftid,
        uint256 amount,
        uint256 nonce
    ...
    function claimBox(uint256 nftId, uint256 amount, bytes calldata signature) external {
        uint256 currentNonce = nonces[msg.sender]++;
        bytes32 messageHash = getIdAmountMessageHash(msg.sender, nftId, amount, currentNonce);
        require(verify(messageHash, signature), "Invalid Signature");
        IERC1155Mintable(tokenBox).mint(msg.sender, nftId, amount, "0x00");
        emit BoxClaimed(msg.sender, nftId, amount, currentNonce);
    }
}
