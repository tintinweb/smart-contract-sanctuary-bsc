...
contract FamPredition is Ownable {
    ...
    function betBatch(
        ...
        uint256[3][] calldata infos // uint256[] memory betNumbers_, // uint256[] memory betAmounts_,  // uint256[] memory choices_,
    ) public {
        ...
        for (uint256 i; i < positions_.length; i++) {
            ...
            bet(epoch_, positions_[i], infos[i][0], infos[i][1], infos[i][2]);
        }
    ...
    function bet(
        ...
        userBets[epoch_][msg.sender].push(BetInfo({ position: position_, choice: choice_, betAmount: betAmount_, betNumber: betNumber_ }));
    }
    ...
    function coutingUserBetReward(
        ...
        if (uBetInfo.position == BetType.ONE) {
            ...
            isWin = isMatchModeFive(round.luckNumber, uBetInfo.choice);
        } else {
    ...
    // 1： 小， 2: 大， 3: 单， 4: 双
    function isMatchModeFive(uint256 luckNumber_, uint256 choice_) public pure returns (bool) {
        require(choice_ >= 111 && choice_ <= 144, "mode error");
        bool matchResult;
        for (uint256 i; i < 2; i++) {
