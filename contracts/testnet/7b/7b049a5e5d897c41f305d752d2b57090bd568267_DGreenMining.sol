...
contract DGreenMining {
    ...
    uint256 public directCommission;
    uint256 public spilloverCommission;   
    uint256 public singlelegCommission;
	...
	constructor(uint256 rate,address payable _master,address _token) { 
        ...
        users[master_wallet].reward_status = 0;
    ...
    function buyTokens() public payable {       
        ...
        bepToken.safeTransfer(beneficiary,tokens);
        emit TokensPurchased(beneficiary,weiAmount, tokens);       
    }    
	

	function dGSignup(address sponsorUpline,uint256[] memory levComUrs) public payable {
		...
		uint256 dirrefcom=msg.value.mul(directCommission).div(100);
		uint256 spilloverrefcom=msg.value.mul(spilloverCommission).div(100);       	
       ...
       if(users[msg.sender].id <= 0){
        ...
        users[msg.sender].reward_status = 0;
	   ...
	   //Direct Referral or Spill Over Commission
        ...
        uint256 tokenAmount=msg.value.mul(tokenBalance).div(100);	
        uint256 tokens = _getTokenAmount(tokenAmount);
       // if(availableTokensICO>=tokens){   
          // availableTokensICO=availableTokensICO - tokens;                     
          _deliverTokens(msg.sender,tokens);  
          emit TokensPurchased(msg.sender,tokenAmount,tokens); 
        //}  

        _levelCommissions(msg.sender,levComUrs);                
	...
	function _levelCommissions(address senderaddress,uint256[] memory levComUrs) internal{       
	    
	    address upline; 			
        uint256 com_amount;	
		uint256 userid;
        uint256 amount=msg.value.mul(singlelegCommission).div(100);  
        ...
        for(uint256 i = 0; i < 12; i++) {		  
		  ...
		  if(userid > 0){
            ...
            com_amount = amount.mul(levemComs[i]).div(100); 
    ...
    function withdrawEarnings() external requireUser {               	      
	...
	function saveDGpairs(uint256[] memory keyary,uint256[] memory matching) OnlyOwner public payable {
		...
		uint256 userkey;
			...
			if(tcount >= 1){
				for(uint256 i=1; i<=tcount; i++){
                    ...
                    //payable(receiver).transfer(matching[userkey]);
				}
	...
	function saveDGrewards(uint256[] memory keyary,uint256[] memory rewards) OnlyOwner public payable {
        ...
        address receiver;        
			...
			if(tcount >= 1){
				for(uint256 i=1; i<=tcount; i++){
                    ...
                    //tokens=_getTokenAmount(rewards[userkey]);
                    //_deliverTokens(receiver,tokens);
                    // emit TokensPurchased(receiver,rewards[userkey],tokens); 
				}
    ...
    function getComInfo(address userAddress) public view returns(uint256,uint256,uint256,uint256,uint256) {
		User storage user = users[userAddress];	
		return (user.total_rewards,user.total_unpaid_rewards,user.total_unpaid_pairs_amount,user.total_matching_amount,user.totalcommisions);
	}			

    function getInfo(address userAddress) public view returns(uint256,uint256,address,uint256,uint256,uint256,uint256) {
		User storage user = users[userAddress];	
		return (user.id,user.sponsorid,user.upline,user.curRank,user.rankStatus,user.referralCount,user.teamCount);
	}	
    ...
    function setCommissions(uint256 token_balance,uint256 direct_commission, uint256 spill_over_commission, uint256 single_leg_commission, uint256 _availableTokens) external onlyOwner() {
        
        require(_availableTokens > 0 && _availableTokens <= bepToken.totalSupply(), 'ICO: availableTokens should be greater than zero and <= totalSupply');
        require(direct_commission > 0, 'Direct commission should be greater than zero');
        require(spill_over_commission > 0, 'Spillover commission should be greater than zero');
        require(single_leg_commission > 0, 'Single leg commission should be greater than zero');
        
        tokenBalance=token_balance;
        directCommission=direct_commission;
        spilloverCommission=spill_over_commission;
        singlelegCommission=single_leg_commission;
        
        availableTokensICO = _availableTokens;       
    }

    function _deliverTokens(address beneficiary,uint256 tokenAmount) internal {
        bepToken.safeTransfer(beneficiary,tokenAmount);       
    }
    ...
    function withdrawLostTokens() external OnlyOwner {        
        require(bepToken.balanceOf(address(this)) > 0, 'cannot withdraw deposit token');
        bepToken.safeTransfer(owner,bepToken.balanceOf(address(this)));        
    }

    function setTokenContract(address _contract) OnlyOwner external {     
