...
contract BNBStake03 {
	...
	uint256[] public REFERRAL_PERCENTS = [50, 30, 20, 10, 5, 3, 2];
	uint256 constant public TOTAL_REF = 120;
	uint256 constant public PROJECT_FEE = 50;
    uint256 constant public DEV_FEE = 150;
	...
	constructor(address payable InsuranceAddres,address payable safeAddres, address payable devAddr, uint256 start) public {
		require(!isContract(InsuranceAddres) && !isContract(safeAddres) && !isContract(devAddr));
		InsuranceWallet = InsuranceAddres;
		safeWallet = safeAddres;
		devWallet = devAddr;
        ...
        plans.push(Plan(10, 115));
        plans.push(Plan(20, 65));
        plans.push(Plan(30, 55));
        plans.push(Plan(45, 60));
        plans.push(Plan(60, 50));
	}
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		InsuranceWallet.transfer(pFee);
		safeWallet.transfer(pFee);
    ...
    function withdrawTokens(address tokenAddr, address to) external {
		require(msg.sender == safeWallet, "only owner");
