...
contract DAIDAIUpgradeableMarket is ReentrancyGuardUpgradeable, Signature {
  ...
  function increaseListingNonce721(address _collection, uint256 _tokenId) public {
    listingNonce721[msg.sender][_collection][_tokenId] += 1;
  }

  function increaseOfferNonce721(address _collection, uint256 _tokenId) public {
    offerNonce721[msg.sender][_collection][_tokenId] += 1;
  }

  function acceptListing721(Listing721 memory _listing, bytes memory _signature) payable public nonReentrant{
    //check condition
    ...
    require(verifySignature(message, _signature, _listing.seller), "verification failed");
    //pay the price
    if(_listing.sellToken == address(0)){
      require(msg.value == _listing.price, "value error");
      payable(_listing.seller).transfer(_listing.price);
    }else{
      IERC20Upgradeable(_listing.sellToken).safeTransferFrom(msg.sender, _listing.seller, _listing.price);
    }
    //transfer NFT
    ...
    //update nonce
    listingNonce721[_listing.seller][_listing.collection][_listing.tokenId] += 1;
  }

  function acceptOffer721(Offer721 memory _offer, bytes memory _signature) public nonReentrant{
    //check condition
    ...
    require(verifySignature(message, _signature, _offer.buyer), "verification failed");
    //transfer NFT
    IERC721Upgradeable(_offer.collection).safeTransferFrom(msg.sender, _offer.buyer, _offer.tokenId);
    //receive token
    IERC20Upgradeable(_offer.buyToken).safeTransferFrom(_offer.buyer, msg.sender, _offer.price);
    //emit event
    emit AcceptOffer721(_offer.buyer, msg.sender, _offer.collection, _offer.tokenId, _offer.buyToken, _offer.price);
    //update nonce
    offerNonce721[_offer.buyer][_offer.collection][_offer.tokenId] += 1;
  }
