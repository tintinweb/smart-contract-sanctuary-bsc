...
contract DefiKings is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    DefiKingsDividendTracker public dividendTracker;

    address public liquidityWallet;

    address payable public marketingAddress = payable(0xF57AE29F46b2Df050E7F4d445B7eC4dCD1902cfF);
    ...
    uint256 public BNBRewardsBuyFee = 13;
    uint256 public BNBRewardsSellFee = 13;
    ...
    constructor() ERC20("Defi Kings", "DFK") {
        dividendTracker = new DefiKingsDividendTracker(payable(this));

        liquidityWallet = 0x49A61ba8E25FBd58cE9B30E1276c4Eb41dD80a80;

        uniswapV2Router = IUniswapV2Router02(
            // 0x10ED43C718714eb63d5aA57B78B54704E256024E      // mainnet
            0x75966f3A20966D4dEF259c96Ed0fcfAD309A429F   // testnet
        );
        ...
        dividendTracker.excludeFromDividends(0x49A61ba8E25FBd58cE9B30E1276c4Eb41dD80a80);
        dividendTracker.excludeFromDividends(address(uniswapV2Router));

        // exclude from paying fees or having max transaction amount
        _isExcludedFromFees[liquidityWallet] = true;
        _isExcludedFromFees[address(this)] = true;
        _isExcludedFromFees[0x49A61ba8E25FBd58cE9B30E1276c4Eb41dD80a80] = true;
        ...
        canTransferBeforeTradingIsEnabled[0x49A61ba8E25FBd58cE9B30E1276c4Eb41dD80a80] = true;
        /*
            _mint is an internal function in ERC20.sol that is only called here,
            and CANNOT be called ever again
        */

        _mint(0x49A61ba8E25FBd58cE9B30E1276c4Eb41dD80a80, 1000000000000 * (10**9));
    }
        ...
        onlyOwner
    {
        ...
        require(bonus <= 600, "Staking bonus can't exceed 600%");
    ...
    function updateMaxAmount(uint256 newNum) public onlyOwner {
        ...
        require(
            maxSellTransactionAmount == 0 ||
                maxSellTransactionAmount >= 500000 * (10**9),
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        DefiKingsDividendTracker newDividendTracker = DefiKingsDividendTracker(
            payable(newAddress)
        );
    ...
    function updateFees(
        ...
        require(marketing <= 10, "operation fees can't exceed 10%");
        ...
        emit UpdateFees(marketing, liquidity, BNBRewardsBuy, BNBRewardsSell);
    }
    ...
    function setMinimumTokenBalanceForDividends(
        ...
        dividendTracker.setMinimumTokenBalanceForDividends(
            _minimumTokenBalanceForAutoDividends,
    ...
    function _transfer(
        ...
        bool isSelling = _isSell(from,to);
        bool isBuying = _isBuy(from);

        uint256 totalFee = 0; 

        if (isBuying){
            totalFee = BNBRewardsBuyFee.add(liquidityFee).add(marketingFees);
        }
        else if (isSelling) {
            totalFee = BNBRewardsSellFee.add(liquidityFee).add(marketingFees).add(burnFee);
        }
        ...
        if(
            ...
            uint256 liquidityBuyTokens = swapAmountBought
                .mul(liquidityFee)
                .div(totalBuyFees);

            uint256 liquiditySellTokens = swapAmountSold
                .mul(liquidityFee)
                .div(totalSellFees);

            uint256 liquidityTokens = liquidityBuyTokens.add(liquiditySellTokens);

            swapAndLiquify(liquidityTokens);

            uint256 burnTokensSell = swapAmountSold
                .mul(burnFee)
                .div(totalSellFees);

            uint256 burnTokens = burnTokensSell;

            super._transfer(address(this), deadAddress, burnTokens);

            uint256 marketingTokensSell = swapAmountSold
                .mul(marketingFees)
                .div(totalSellFees);

            uint256 marketingTokensBuy = swapAmountBought
                .mul(marketingFees).
                div(totalBuyFees);

            uint256 marketingTokens = marketingTokensSell.add(marketingTokensBuy);
        ...
        if(_isExcludedFromFees[from] || _isExcludedFromFees[to] || !isBuying || !isSelling) {
            takeFee = false;
        }
        ...
        try dividendTracker.setBalance(payable(from), getStakingBalance(from)) {} catch {}
        try dividendTracker.setBalance(payable(to), getStakingBalance(to)) {} catch {}
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if(success) {
   	 		emit SendDividends(dividends, success);
        }
    }
}

contract DefiKingsDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    DefiKings public defiKings;

    mapping(address => uint256) public lastClaimTimes;

    uint256 public minimumTokenBalanceForAutoDividends;
    ...
    constructor(address payable mainContract)
        DividendPayingToken(
            "DefiKings_Dividend_Tracker",
            "DefiKings_Dividend_Tracker"
        )
    {
        defiKings = DefiKings(mainContract);
        minimumTokenBalanceForAutoDividends = 172500000 * (10**9);
        minimumTokenBalanceForDividends = 115000000 * (10**9);
    ...
    function _transfer(
        ...
        require(false, "DefiKings_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "DefiKings_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main DefiKings contract."
        );
    }
    ...
    function setMinimumTokenBalanceForDividends(
        ...
        minimumTokenBalanceForAutoDividends = _minimumTokenBalanceForAutoDividends * (10 ** 9);
        minimumTokenBalanceForDividends = _minimumTokenBalanceForDividends * (10 ** 9);
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(defiKings);

            uint256 prevBalance = defiKings.balanceOf(address(this));
            ...
            {
                uint256 received = defiKings.balanceOf(address(this)).sub(
                    prevBalance
                );
                if (received > 0) {
                    success = true;
                    defiKings.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
