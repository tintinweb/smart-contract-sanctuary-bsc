...
contract ERC20_Factory is Ownable {
    ...
    address public erc20Instance;

    address public gateway;

    string public rChainContract;

    event createERC20( string sender ,  address _contractAddress , string _name , string _symbol , address _owner );
    ...
    constructor(string memory _rChainContract ,address _gateway ) Ownable(){
        rChainContract = _rChainContract;
        gateway = _gateway;
    }

    function setRChainContract(string memory _rChainContract) external onlyOwner{
        rChainContract = _rChainContract;
    }
    ...
    //Factory Fx
    function createRemoteAll(
        string memory sender,
        bytes memory payload
    ) internal{
        ( string memory _name , string memory _symbol , address _owner ) = abi.decode( payload , ( string ,string , address  ));
        name = _name;
        symbol = _symbol;
        ERC20_Standard C1 = new ERC20_Standard( _name , _symbol , _owner , address(gateway)  );
        erc20Instance = address(C1);
        emit createERC20( sender ,address(C1) , _name , _symbol ,_owner );
    }

//    function setRequest2Router( address _addr , string memory _name , string memory _symbol ) external onlyOwner {
//        bytes memory p2 = abi.encode( chainID , _addr , _name , _symbol );
//        gateway.requestToRouter(p2, rChainContract);
//    }
    ...
    function handleRequestFromRouter(string memory sender, bytes memory payload) external {
        ...
        createRemoteAll(sender ,payload);
    }
}
...
contract ERC20_Standard is AccessControl, ERC20Snapshot {
    using SafeMath for uint256;

    address public gateway;

    mapping ( bytes32 => chainTypeStruct ) public crossChainAddress;

    mapping ( uint64 => ackStatus ) public crossChainAcks;

    enum ackStatus { inactive , active , settled , failed }

    struct chainTypeStruct {
        uint64 chainType;
        string chainId;
        address _addr;
    }

    struct crossChainStruct {
        address recipent;
        uint256 amt;
    }

    constructor( string memory _name , string memory _symbol  , address _owner , address _gateway  ) AccessControl() ERC20Snapshot() ERC20(_name , _symbol) {
    ...
    function createSnapshot() external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
         _snapshot();
        return true;
    }
    ...
    function burn(address _to, uint256 _value) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        _burn(_to, _value);
        return true;
    }
    ...
    function setCrossChainERC(
        uint64 _chainType,
        string memory _chainId,
        address _addr
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        bytes32 hash = keccak256( abi.encodePacked(_chainType , _chainId));
        crossChainAddress[hash] = chainTypeStruct( _chainType , _chainId , _addr);
    }

    function genPayload(  address[] memory _addrs , uint256[] memory _amt ) internal returns ( bytes memory payload ){
        crossChainStruct[] memory crossChainRequest = new crossChainStruct[](_addrs.length);
        for(uint i=0; i < _addrs.length; i++){
            crossChainStruct memory req;
            req.recipent = _addrs[i];
            req.amt = _amt[i];
            crossChainRequest[i] = req;

            // Add Burning Logic during gen payload only
            _burn( msg.sender , _amt[i]);

        }
        payload = abi.encode(crossChainRequest );
        return payload;
    }

    function decodePayload( bytes memory payload ) internal pure returns ( crossChainStruct[] memory req ){
        ( req ) = abi.decode(payload, ( crossChainStruct[] ));
        return req;
    }

    // use this is used for simple crosschain erc20 transfer
    function crossChainTransfer(
        ...
        bytes32 hash = keccak256( abi.encodePacked(_chainType , _chainId));
        require( crossChainAddress[hash]._addr != address(0), "Dest Address not mapped");
        address[] memory _addrs = new address[](1);
        uint256[] memory _amts = new uint256[](1);
        _addrs[1] = msg.sender;
        _amts[1] = _amt;
        bytes memory payload = genPayload(  _addrs , _amts );
        bytes memory destAddrs = CrossTalkUtils.toBytes(crossChainAddress[hash]._addr);

        Utils.DestinationChainParams memory chainParams = Utils.DestinationChainParams(_gasLimit, _gasPrice, _chainType, _chainId);
        Utils.AckType ackParam = Utils.AckType.ACK_ON_SUCCESS;
        Utils.AckGasParams memory gasParams = Utils.AckGasParams( _gasLimit, _gasPrice );
        ...
        crossChainAcks[nonce] = ackStatus.active;
    ...
    function handleRequestFromSource(
        ...
        bytes32 hash = keccak256( abi.encodePacked(srcChainType , srcChainId));
        address srcAddr = CrossTalkUtils.toAddress(srcContractAddress);
        require(crossChainAddress[hash]._addr == srcAddr, "Address Mismatch");

        // decoding the payload
        ackStatus status;
        crossChainStruct[] memory req = decodePayload(payload);
        // Array length cant be 0 which means decode didnt happen properly
        if ( req.length == 0 ){
            status = ackStatus.failed;
        }else {
            for(uint i=0; i < req.length; i++){
                // Mint tokens to recipents
                _mint(req[i].recipent , req[i].amt);
            }
            status = ackStatus.settled;
        }
    ...
    function handleCrossTalkAck(
        ...
        require(msg.sender == gateway, "Only gateway can call this function");
        //  eventIdentifier must be in active state
        ackStatus ack = crossChainAcks[eventIdentifier];
        require(ack == ackStatus.active ,"Status must be active");

        if ( execFlags[0] == true ){
            // decoders for status
            bytes memory _execData = abi.decode(execData[0], (bytes));
            (ackStatus newAck ) = abi.decode( _execData, (ackStatus) );
            crossChainAcks[eventIdentifier] = newAck;

        }else {
            crossChainAcks[eventIdentifier] = ackStatus.failed;
        }
    }

    function crossChainDisperse(
        uint64 _chainType,
        string memory _chainId,
