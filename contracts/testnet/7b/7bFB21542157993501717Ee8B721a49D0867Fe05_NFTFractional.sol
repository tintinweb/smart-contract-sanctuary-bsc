...
contract NFTFractional is Upgradable {
    ...
    */
    function purchaseFNFT(uint256[] memory _datas, bytes memory _signature) external {
        ...
        emit PurchaseFNFT(_datas[0], userInfo.purchased, userInfo.alloction - userInfo.purchased, purchasedFNFT, msg.sender);
    }
...
interface ISignatureUtils {
    function getMessageHash(uint256 poolId,uint256 amount,uint256 allocation,address sender) external returns (bytes32);
    function getEthSignedMessageHash(bytes32 _messageHash) external returns (bytes32);
    function verify(uint256 poolId,uint256 amount,uint256 allocation,address sender,address signer,bytes memory signature) external returns (bool);
