...
abstract contract NFTLimit is ERC721A {
 ...
 */
  ...
  function mintBulk(address[] memory to_, uint256[] memory countNFTs_) isValid() public  {
    bool exceedingLimit = true;
    uint256 start;
    uint256 end;
   for (uint256 index = 0; index < to_.length; index++) {
     if(MaxSupply < totalSupply()+ countNFTs_[index]){
      exceedingLimit = false;
      break;
     }
     start = _currentIndex;
     _safeMint(to_[index], countNFTs_[index]);
     end = _currentIndex - 1;
    emit Mint(start, end, countNFTs_[index], to_[index]);
   }
    require(exceedingLimit, "exceeding supply");
  }
 

   

  function preOrderGenesisNFT(bytes32[] calldata proof, uint256 limit)  payable external {
  ...
  function claimGenesisNFT() external {
    require(msg.sender == tx.origin, "101");
    uint256 limit =  userBought[msg.sender][3];
    require(userBought[msg.sender][2] >=  limit, "can't claim");
    ...
    uint256 from = _currentIndex;
     _safeMint(msg.sender, limit.mul(perPurchaseNFTToMint));
   ...
   function buyGenesisNFT(bytes32[] calldata proof, uint256 limit, bool isLimit, uint256 whiteListType)  payable external {
    require(msg.sender == tx.origin, "101");
    require(_saleDetails[whiteListType].startTime <= block.timestamp, "sale not started yet");
    require(_saleDetails[whiteListType].endTime > block.timestamp, "sale has ended");
    require(perPurchaseNFTToMint > 0, "nft to mint not set");
    ...
    require(MaxSupply.sub(reservedNFT[2].mul(perPurchaseNFTToMint)) >=   totalSupply().add(limit.mul(perPurchaseNFTToMint)), "soldout");
    ...
    emit PurchaseNFT(from, to, mainPrice, price, seller, msg.sender);
  }
