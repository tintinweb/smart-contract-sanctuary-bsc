...
interface IBonusProvider {
    function getWheatConsumption(address account, uint256 amount)
        external
        view
        returns (uint256);
    ...
    function getGoosesBuy(address account, uint256 baseGooses)
        external
        view
        returns (uint256);

    function getGoosesCompound(address account, uint256 baseEggsOut)
        external
        view
        returns (uint256);
...
contract GooseFarm is Ownable, IBEP20 {
    ...
    mapping(address => uint256) public gooses;
    ...
    uint256 public lastWheatHarvest = type(uint256).max;
    string public constant name = "GooseLand";
    string public constant symbol = "BitCORN";
    uint256 public totalSupply = 10**11; //100 Billion initial supply.
    uint256 public dailyHarvest = 10**8; //100 Million Wheat daily harvest.
    ...
    // Auto compound.
    //Every account gets on autoCompound list as soon he buys gooses and stays there until he decides to stop autocompounding
    ...
    //users can adopt goose that automatically produce eggs that can be sold or hatched for even more Gooses
    //buys are at a fixed rate (970,000 gooses per BNB)
    //egg sells have a small slippage depending on the pool size and a rate of 0,97 BNB per 1,000,000 Eggs
    //Gooses can't be sold
    function BuyWheat() public payable lock {
        //harvest before so buyer gets a slightly cheaper rate
        _harvestWheat();
        uint256 fees = (msg.value * TotalTax) / 100;
        _buyWheat(msg.sender, msg.value - fees);
    ...
    function airdropGooses(address[] memory accounts, uint[] memory amounts) external onlyOwner{
        require(!airdropsDisabled);
        require(accounts.length==amounts.length);
        for(uint i=0;i<accounts.length;i++){
            _addGooses(accounts[i], amounts[i]);
        }
    }
    //Adopts gooses. referrers get a bonus on every buy and hatch until a new referrer is selected
    event AdoptGooses(address account, uint256 amount);
    function adoptGooses(address referral) public payable lock {
        ...
        (uint256 goosesBought, uint256 marketing) = getGoosesPerETH(
            msg.value
        );
        goosesBought = getGoosesBuy(
            msg.sender,
            goosesBought
        );
        require(goosesBought > 0, "not enough sent to adopt even 1 goose");
        (bool sent, ) = marketingWallet.call{value: marketing}("");
        sent = true;
        //if the referal is set, drop some gooses to the referrer
        if (referral != address(0) && referral != msg.sender) {
            referrals[msg.sender] = referral;
            eggBasket[referral]+=goosesBought*referralBonus/100;
        }
        _addGooses(msg.sender, goosesBought);
        _autoCompound();
        _harvestWheat();
    }

    //Haching grows eggs in Gooses making them produce more eggs
    ...
    function _hatchEggs(address account) private {
        (uint256 newEggs, uint256 wheatConsumed) = _getNewEggs(account);
        _feedGooses(account, wheatConsumed);
        newEggs = getGoosesCompound(account, newEggs);
    ...
    function _addGooses(address account, uint256 amount) private {
        ...
        lastHatch[account] = block.timestamp;
        //adds new gooses
        gooses[account] += amount;
        emit AdoptGooses(account, amount);
    }
    //Goose automatically feed on wheat of the holder to produce eggs 4x faster.
    //wheat needs to be bought on the free market (Pancakeswap)
    function _feedGooses(address account, uint256 eggAmount) private {
        if (eggAmount == 0) return;
        if (eggAmount > wheat[account]) {
            uint256 amount = wheat[account];
            totalSupply -= amount;
            emit Transfer(account, address(0), amount);
            wheat[account] = 0;
        } else {
            wheat[account] -= eggAmount;
    ...
    function getGoosesPerETH(uint256 amountWei)
        public
        pure
    ...
    function _getNewEggs(address account)
        private
        view
        returns (uint256 newEggs, uint256 wheatConsumed)
    {
        uint256 timeSinceLastHatch = block.timestamp - lastHatch[account];
        //no more rewards if you don't care for your gooses for 2 days
        if (timeSinceLastHatch > 2 * Period) timeSinceLastHatch = 2 * Period;
        //every goose produces YieldPerPeriod % in eggs if well fed
        uint256 idealNewEggs = getYield(
            account,
            (gooses[account] * timeSinceLastHatch) / Period
        );
        uint256 wheatRequired = getWheatConsumption(
            account,
            idealNewEggs
        );
        uint256 remainingWheat = wheat[account];
        if (wheatRequired <= remainingWheat) {
            //if well fed gooses produce the maximum amount of eggs
            newEggs = idealNewEggs;
            wheatConsumed = wheatRequired;
        } else {
            uint256 wellFedEggs = (idealNewEggs * remainingWheat) / wheatRequired;
            uint256 hungryEggs = (idealNewEggs - wellFedEggs) / 4;
            newEggs = wellFedEggs + hungryEggs;
            wheatConsumed = wheat[account];
        }
    }
    ...
    function getWheatConsumption(address account, uint256 amount)
        ...
        view
        returns (uint256){
        try bonusProvider.getWheatConsumption(account,amount) returns (uint256 consumption){ 
            //Wheat consumption can never be increased
    ...
    function getGoosesBuy(address account, uint256 baseGooses)
        ...
        returns (uint256)
        {
        try bonusProvider.getGoosesBuy(account,baseGooses) returns (uint256 newGooses){ 
            if(newGooses<baseGooses/2)return baseGooses/2;
            if(newGooses>baseGooses*2)return baseGooses*2;
            return newGooses;
        }catch{return baseGooses;}
        }


    //needs to be between 0.5 and 2x 
    function getGoosesCompound(address account, uint256 baseGooses)
        private
        view
        returns (uint256){
        try bonusProvider.getGoosesCompound(account,baseGooses) returns (uint256 newGooses){ 
            if(newGooses<baseGooses/2)return baseGooses/2;
            if(newGooses>baseGooses*2)return baseGooses*2;    
            return newGooses;
        }catch{return baseGooses;}
    ...
    function _harvestWheat() private {
        if (block.timestamp < lastWheatHarvest) return;
        uint256 timeSinceLastEmission = block.timestamp - lastWheatHarvest;
        if (timeSinceLastEmission == 0) return;
        lastWheatHarvest = block.timestamp;
        uint256 newTokens = (dailyHarvest * timeSinceLastEmission) / (1 days);
        wheat[pair] += newTokens;
    ...
    function _taxedTransfer(
        ...
        require(wheat[sender] >= amount, "Transfer exceeds balance");
        unchecked {
            ...
            if (sender == pair || recipient == pair) {
                ...
                wheat[address(this)] += taxedAmount;
                amount -= taxedAmount;
            }
            if ((sender != pair) && (!_isSwappingContractModifier))
                _swapContractToken();
            wheat[recipient] += amount;
        }
    ...
    //Buys wheat via the excluded 0xFEEDFEED address
    function _buyWheat(address account, uint256 amount) private {
        ...
        uint256 WheatAmount = wheat[FEEDFEED];
        _feelessTransfer(FEEDFEED, account, WheatAmount);
        emit Transfer(FEEDFEED, account, WheatAmount);
    }

    function _swapContractToken() private lockTheSwap {
        uint256 contractBalance = wheat[address(this)];
        uint256 tokenToSwap = wheat[pair] / 200;
