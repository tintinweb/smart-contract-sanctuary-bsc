...
function getrandom(uint256 seed) internal view returns(uint16[15] memory){
    uint16[15] memory temp_bl;
    uint8 len = 0;
    while(len < 15) 
    {
        uint8 key = uint8(uint256(keccak256(abi.encodePacked(block.timestamp,seed ))) % 15);
        bool exists = false;
        uint16 value = belv[key];
...
function betm(uint8 yz,uint8 ma,uint256 seed) public view returns(uint16 key,uint32 won,string memory ff){
  uint16[15] memory temp = getrandom(seed); 
  uint16 po ;
  uint16 total;
  uint8 index;
