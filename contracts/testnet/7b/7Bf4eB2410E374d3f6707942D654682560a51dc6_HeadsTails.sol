...
contract HeadsTails is Ownable {
    ...
    /**
    * @param _period answering period (unit is second)
    * @param _period 1 ~ 60 * 60 * 24 * 7
    **/
    function setAnsweringPeriod(uint256 _period) external onlyOwner {
        require(_period > 0, "HeadsTails: Period should be a positive number.");
        require(_period <= 60 * 60 * 24 * 7, "HeadsTails: Period should not be bigger than 7 days.");
        answeringPeriod = _period;
        emit SetAnsweringPeriod(_period);
    }
    ...
    **/
    function setContractFee(uint256 _contractFee) external onlyOwner {
        ...
        require(_contractFee <= 3000, "HeadsTails: Contract fee should not be bigger than 30%.");
    ...
    **/
    function setSubmitterFee(uint256 _submitterFee) external onlyOwner {
        ...
        require(_submitterFee <= 200, "HeadsTails: Submitter fee should not be bigger than 2%.");
    ...
    **/
        ...
        external
    {
        ...
        IERC20(question.token).safeTransferFrom(msg.sender, address(this), stakeAmount);
    ...
    **/
    function unstake(uint256 questionId) external {
        ...
        if(question.token == address(this)){
            ...
            IERC20(question.token).safeTransfer(msg.sender, stakeInfo.amount);
        }
    ...
    **/
    function harvest(uint256 questionId) external {
        ...
        if(question.token == address(this)){
            ...
            IERC20(question.token).safeTransfer(msg.sender, _amount);
        }
        ...
        onlyOwner
    {
        ...
        if(question.token == address(this)){
            ...
            if (_submitterAmount > 0) {
                ...
                IERC20(question.token).safeTransfer(_submitter, _submitterAmount);
            }

            if (_commissionAmount > 0) {
                require(IERC20(question.token).balanceOf(address(this)) >= _submitterAmount, 
                    "HeadsTails: Out of token balance for contract fee.");
                IERC20(question.token).safeTransfer(owner(), _commissionAmount);
            }
        }
