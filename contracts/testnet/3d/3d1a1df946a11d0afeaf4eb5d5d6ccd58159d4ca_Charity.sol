...
contract Charity is Ownable {
    ...
    function buyProduct(address _sponsorAddress, uint256 amount) external payable {
        ...
        uint256 corporateAmount = msg.value - sponsorAmount - swapAmount;// - (uint256(2).mul(10 ** 18) / uint256(price).div(priceDivisor));
    ...
    function upgradeProduct(address _sponsorAddress, uint256 amount) external payable {
        ...
        uint256 amountInEth = (msg.value).mul(10 ** 18) / uint256(price).div(priceDivisor);
        
        uint256 utilityDBME = amount.mul(10 ** 18) / utilityPrice;
        uint256 tokensToBurn = utilityDBME.mul(burnPercentage).div(divisor);
        uint256 tokensToCharity = utilityDBME.mul(charityPercentage).div(divisor);

        uint256 sponsorAmount = amountInEth.mul(sponsorPercentage + swapPercentage).div(divisor);
        uint256 corporateAmount = amountInEth - sponsorAmount;
