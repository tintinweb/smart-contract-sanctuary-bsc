...
contract BerserkerGovernance is GovernorInterface, GovernorEvents {
    ...
    address private constant PANCAKESWAP_ROUTER = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    constructor() {
        ...
        admin = 0xe524bA33Ef3f26d8Ce347B04968f11324ebeF0C5;  // admin's wallet address
        ...
        WETH = IPancakeRouter(PANCAKESWAP_ROUTER).WETH();
    }
    ...
    function swap(address _tokenIn, address _tokenOut, uint256 _amountIn, uint256 _amountOutMin, address _to) internal {
      ...
      IERC20(_tokenIn).approve(PANCAKESWAP_ROUTER, _amountIn);
      ...
      IPancakeRouter(PANCAKESWAP_ROUTER).swapExactTokensForTokens(_amountIn, _amountOutMin, path, _to, block.timestamp);
    }
    ...
    function getAmountOutMin(address _tokenIn, address _tokenOut, uint256 _amountIn) internal view returns (uint256) {
      ...
      uint256[] memory amountOutMins = IPancakeRouter(PANCAKESWAP_ROUTER).getAmountsOut(_amountIn, path);
    ...
    function canVote(address user, uint proposalId) public view returns (bool) {
        if(getNFTBalance(user) > 0 && user != proposals[proposalId].proposer && proposals[proposalId].receipts[user].hasVoted == false) {
          return true;
        } else {
          return false;
        }
