//SPDX-License-Identifier: Unlicense
    ...
    contract FIFAL1C is Ownable {
        ...
        */
        function placeGuess(uint256 _matchUpIndex,uint256 _guessScore, uint256 _guessAmount, uint256 _level) external {
            ...
            require((_level == 1 && _guessAmount>=(100e18) && _guessAmount<(999e18)) || 
                (_level == 2 && _guessAmount>=(1000e18) && _guessAmount<(9999e18)) ||
                (_level == 3 && _guessAmount>=(10000e18)), "Invalid level or guess amount");
        ...
        */
        ...
        function claim(uint256 _matchIndex, uint256 _level) external{
            ...
            if ((!scoreInfo[_level][_matchIndex][resultScore].hasGuessed && matchStatus == MatchStatus.COMPLETED) || matchStatus == MatchStatus.DELAYED || matchStatus == MatchStatus.CANCELLED){
                IERC20(tokenAddress).safeTransfer(msg.sender, (userInfo[msg.sender][_level][_matchIndex].personalMatchTotal * refundInterest) / 1000);
                uint256 devFee = ((userInfo[msg.sender][_level][_matchIndex].personalMatchTotal * refundDevInterest ) / 1000);
            ...
            if (guessInfo[msg.sender][_level][_matchIndex][resultScore].hasUserGuessed && matchStatus == MatchStatus.COMPLETED) {
                ...
                uint256 devFee = (matchTotalPool[_level][_matchIndex]* devRewardInterest *  guessStorage.guessAmount )/ (1000 * scoreInfo[_level][_matchIndex][resultScore].scoreTotal);
                IERC20(tokenAddress).safeTransfer(devAddress, devFee);
                totalFeesCharged += devFee;
                userInfo[msg.sender][_level][_matchIndex].hasUserClaimed = true;
            }
        ...
        function viewGuessStatus(address _address, uint256 _matchIndex, uint256 _level) public view returns (string memory) {
            uint256 resultScore = resultList[_matchIndex];
        ...
        function getPersonalMatchTotal(uint256 _matchIndex, uint256 _level) public view returns (uint256){
            return userInfo[msg.sender][_level][_matchIndex].personalMatchTotal;
        }

        function getGuess(uint256 _matchUpIndex, uint256 result, uint256 _level) public view returns (Guess memory){
