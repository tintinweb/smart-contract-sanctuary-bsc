...
/**
 * #InvestToEarn
   - 0% of every transaction is redistributed to all holders
   ...
   - 2% of every transaction is converted to BNB and sent to the charity wallet address
   - 3% of every transaction is converted to BNB and sent to the dev wallet address
   - 3% of every transaction is converted to BNB and sent to the Marketing wallet address
   - 1% maximum for sell from total supply
   - blacklist wallet. dev can blacklist and unblacklist wallet address.
   - 1,000,000,000 total supply
   - 10,000,000 tokens limitation for sell, which is 1% of the total supply.
   - Maximum tokens per wallet is only 5% from total supply
*/
...
contract InvestToEarn is Context, IERC20, Ownable {
    ...
    address private _charityWalletAddress = 0x651A177aBFf554990eaEC40237Fdb1f04e169871;
    address private _devWalletAddress = 0x651A177aBFf554990eaEC40237Fdb1f04e169871;
    address private _MarkWalletAddress = 0x651A177aBFf554990eaEC40237Fdb1f04e169871;
    ...
    string private _name = "Invest To Earn";
    string private _symbol = "I2E";
    ...
    uint256 public _MarkFee = 3;
    uint256 private _previousMarkFee = _MarkFee;
    ...
    uint256 public _maxTxPercent = 1; //user can sell maximum 1 percent of _tTotal
    uint256 public _maxWalletPercent = 5; //user can keep maximum 5 percent in his wallet
    ...
    function setMarFeePercent(uint256 markFee) external onlyOwner {
        _MarkFee = markFee;
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 charityBNB = newBalance.div(_charityFee + _devFee + _liquidityFee + _MarkFee).mul(_charityFee);
        uint256 devBNB = newBalance.div(_charityFee + _devFee + _liquidityFee + _MarkFee).mul(_devFee);
        uint256 markBNB = newBalance.div(_charityFee + _devFee + _liquidityFee + _MarkFee).mul(_MarkFee);
        uint256 liquidityBNB = newBalance - charityBNB - devBNB - markBNB;

        TransferBnbToExternalAddress(_charityWalletAddress, newBalance.div(_charityFee + _devFee + _liquidityFee + _MarkFee).mul(_charityFee));
        TransferBnbToExternalAddress(_devWalletAddress, newBalance.div(_charityFee + _devFee + _liquidityFee + _MarkFee).mul(_devFee));
        TransferBnbToExternalAddress(_MarkWalletAddress, newBalance.div(_charityFee + _devFee + _liquidityFee + _MarkFee).mul(_MarkFee));
