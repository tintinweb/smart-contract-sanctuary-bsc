...
contract ScaTokenStaking is Ownable {
    ...
    function setPayWallet(address wallet) external {
        _wallet = wallet;
    }
     ...
     *
     */
    function stake(uint8 _pid, uint256 _amount) external payable returns (bool) {
     ...
     *
     */
    function unStake(uint8 _pid, uint256 _amount) external payable returns (bool) {
        ...
        uint256 fee;
        if(!_canClaim){
          fee = _amount.mul(pool.withdrawalFee).div(10000);
          _claimToFeeWallet(_pid, msg.sender);
        } else {
          _claim(_pid, msg.sender);
        }
    ...
    function _claimToFeeWallet(uint8 _pid, address _addr) internal {
        uint256 amount = _payout(_pid, _addr);

        if (amount > 0) {
            safeTransfer(_wallet, amount);
        }
