...
contract DogeRoulette is Ownable, ERC20 {
	...
	string private constant _symbol = "DogeRoulette";
	...
	CustomTaxPeriod private _default = CustomTaxPeriod('default',0,0,3,3,3,3,8,8);
	CustomTaxPeriod private _base = CustomTaxPeriod('base',0,0,3,3,3,3,8,8);
	...
	event UpdateFees(string indexed identifier, uint256 liquidityFee, uint256 marketingFee, uint256 holdersFee);

	event UpdateBlockedAcct(address indexed holder, bool indexed status);

	event UpdateMaxTXN(uint256 indexed newValue, uint256 indexed oldValue);
	event UpdateMaxWallet(uint256 indexed newValue, uint256 indexed oldValue);
	event UpdateExcludeFromFees(address indexed account, bool isExcluded);
	event UpdateMaxTransfer(address indexed account, bool isExcluded);
	event NewExcludeFromMaxWallet(address indexed account, bool isExcluded);
	event NewExcludeFromDividends(address indexed account, bool isExcluded);
	event AllowedWhenTradingDisabledChange(address indexed account, bool isExcluded);
	event UpdateMinTokenAmountBeforeSwap(uint256 indexed newValue, uint256 indexed oldValue);
	event UpdateMinTokenAmountForDividends(uint256 indexed newValue, uint256 indexed oldValue);
	...
	event NewDividendTokenCycle(uint256 newValue, uint256 oldValue);
	...
	constructor() ERC20(_name, _symbol) {
		dividendTokens.push(address(0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684)); //USDT
		dividendTokens.push(address(0x8BaBbB98678facC7342735486C851ABD7A0d17Ca)); //WETH
		dividendTokens.push(address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7)); //BUSD
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) external onlyOwner {
		...
		emit UpdateMaxTransfer(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) external onlyOwner {
		...
		emit NewExcludeFromMaxWallet(account, excluded);
	}
	...
	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		...
		emit UpdateMinTokenAmountBeforeSwap(newValue, minimumTokensBeforeSwap);
	...
	function setDividendTokenCycleInHours(uint256 newValue) external onlyOwner {
		emit NewDividendTokenCycle(newValue, dividendTokenCycleInHours);
