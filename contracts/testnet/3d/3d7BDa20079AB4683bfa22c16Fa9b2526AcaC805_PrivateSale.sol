...
contract PrivateSale is Auth {
  ...
  function joinPool() public payable {
    ...
    require(investors[msg.sender] == false, 'already enrolled');
  ...
  function withdrawPerMonth() external {
    ...
    uint256 notReceivedMamps = _claimMamp(amountToReceivePerMonth);
    if (notReceivedMamps > 0) {
      investorInfos[msg.sender].notReceivedAmount = investorInfos[msg.sender].notReceivedAmount.add(notReceivedMamps);
    } else {
      uint256 recevingLeft = _claimMamp(investorInfos[msg.sender].notReceivedAmount);
      if (recevingLeft > 0) investorInfos[msg.sender].notReceivedAmount = recevingLeft;
      else investorInfos[msg.sender].notReceivedAmount = 0;
    }
