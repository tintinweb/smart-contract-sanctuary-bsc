...
interface IMonstropolyDeployer {
    ...
    function deployProxyWithImplementation(bytes32 id, address implementation, bytes memory initializeCalldata) external;

    /// @notice Deploys a proxy and its implementation
    /// @param id The hash of the string identifier
    /// @param bytecode Bytecode of the implementation to be deployed
    /// @param initializeCalldata Calldata to initialize the proxy
    function deploy(bytes32 id, bytes memory bytecode, bytes memory initializeCalldata) external returns(address implementation);
}
...
contract CoinCharger {
    ...
    function _chargeAndtransferETH(address from_, address to_, uint amount_) internal {
...
interface IMonstropolyData {
    ...
    /// @return Asset
    function getValueFromGen(string calldata gen, uint index) external view returns(uint256);
