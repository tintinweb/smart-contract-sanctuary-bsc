...
contract YiBoxSetting is Ownable {
    ...
    uint16 maxHeroType;
    mapping(uint16 => uint32) public m_u32Level4Count;
    mapping(uint16 => uint32) public m_u32Level5Count;
    mapping(uint16 => uint32) public m_u32Level4Used;
    mapping(uint16 => uint32) public m_u32Level5Used;
    ...
    constructor() public {
        ...
        for (uint16 i = 0; i < maxHeroType; i++){
            m_u32Level4Count[i] = 200;
            m_u32Level5Count[i] = 50;
        }
    ...
    function calcHashrate4() external onlyGuardian returns (uint32 _hr) {
        ...
        for (uint i = 0; i < m_u32Hashrate4Per.length; i++) {
            ...
            if (_seed <= _t) {
                if (i == 0) {
                    _hr = _seed % m_u32Hashrate4[i];
                } else {
                    _hr = (_seed %(m_u32Hashrate4[i] - m_u32Hashrate4[i-1])) + m_u32Hashrate4[i-1];
                }
                break;
            }
        }
    ...
    function calcHashrate5() external onlyGuardian returns (uint32 _hr) {
        uint32 _seed;
        bytes memory payload = abi.encodeWithSignature("Hash(uint256)", 10000);
        (, bytes memory returnData) = address(seedAddress).call(payload);
        _seed =  SafeMathExt.safe32(abi.decode(returnData,(uint256))) ;
        ...
        for (uint i = 0; i < m_u32Hashrate5Per.length; i++) {
           ...
           if (_seed <= _t) {
               if (i == 0) {
                   _hr = _seed % m_u32Hashrate5[i];
               } else {
                   _hr = (_seed % (m_u32Hashrate5[i] - m_u32Hashrate5[i-1])) + m_u32Hashrate5[i-1];
               }
               break;
           }
        }
    ...
    function calcOpenBox() external onlyGuardian returns (uint256 _lv,uint256 _ty, uint32 _useIdx,uint256 _seed) {
        require(seedAddress!=address(0), "seedaddr error");
        
        bytes memory payload = abi.encodeWithSignature("Hash(uint256)", 10000);
        (, bytes memory returnData) = address(seedAddress).call(payload);
        _seed = abi.decode(returnData,(uint256));
        
        _ty = (_seed + now) % maxHeroType;
        _lv = 1;
        if (_seed <= m_u32ProbabilityL1) {
            _lv = 1;
        } else if (_seed <= m_u32ProbabilityL1 + m_u32ProbabilityL2) {
            _lv = 2;
        } else if (_seed <= m_u32ProbabilityL1 + m_u32ProbabilityL2) {
            _lv = 2;
        } else if (_seed <= m_u32ProbabilityL1 + m_u32ProbabilityL2 + m_u32ProbabilityL3) {
            _lv = 3;
        } else if (_seed <= m_u32ProbabilityL1 + m_u32ProbabilityL2 + m_u32ProbabilityL3 + m_u32ProbabilityL4) {
            bool isfind = false;
            for (uint i = 0; i < maxHeroType; i++) {
                if (m_u32Level4Used[uint16(_ty)] >= m_u32Level4Count[uint16(_ty)]) {
                    _ty++;
                    if (_ty >= maxHeroType) _ty = 0;
                } else {
                    isfind = true;
                }
            }
        ...
        }  else if (_seed <= m_u32ProbabilityL1 + m_u32ProbabilityL2 + m_u32ProbabilityL3 + m_u32ProbabilityL4 + m_u32ProbabilityL5) {
            bool isfind = false;
            for (uint i = 0; i < maxHeroType; i++) {
                if (m_u32Level5Used[uint16(_ty)] >= m_u32Level5Count[uint16(_ty)]) {
                    _ty++;
                    if (_ty >= maxHeroType) _ty = 0;
                } else {
                    isfind = true;
        ...
        if (_lv == 4) {
            m_u32Level4Used[uint16(_ty)]++;
            _useIdx = m_u32Level4Used[uint16(_ty)];
        } else if (_lv == 5) {
            m_u32Level5Used[uint16(_ty)]++;
            _useIdx = m_u32Level5Used[uint16(_ty)];
        }
    }
