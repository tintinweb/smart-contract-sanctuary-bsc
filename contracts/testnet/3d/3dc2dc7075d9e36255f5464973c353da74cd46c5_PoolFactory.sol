...
contract Pool is OwnableUpgradeable {
    ...
    function extendLockTime(uint256 _id,
        Tokentype _tokenType,
        address _tokenAddress,
        uint256 _extendTime) external {
        require(factory == msg.sender, "Not allowed");
        liquidityLockDays += _extendTime;
        IKronosLock(lock).extendUnlockTime(
            _id,
            _tokenType,
            _tokenAddress,
            _extendTime
...
contract PoolFactory is Ownable {
    ...
    function setExtendFee(uint256 extendFee_, uint256 extendUpto_, uint256 extendLockFees_)
        external
        onlyOwner
    {
    ...
    function extendLockTime(address payable presale, uint256 _id,
        Tokentype _tokenType,
        address _tokenAddress,
        uint256 _extendTime)
        external
        payable
    {
        ...
        Pool(presale).extendLockTime(_id, _tokenType, _tokenAddress,  _extendTime);
    }
