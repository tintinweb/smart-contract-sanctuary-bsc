...
contract Marketplace is ReentrancyGuard, Operator, OrderFulfiller {
    ...
    /**
     * @dev Emitted when seller cancelled his order
     *
     * @param orderHash  Hash of the fulfilled order.
     * @param seller     Seller of order.
     */
    ...
    /**
     * @notice Retrieve an order nonce for an order.
     *
     ...
     * @param dealSign      orders signed by backend
     * @param ordersHashes  list of order hashes
     *
     * @dev Should provide valid sign from order creator and operator
     */
    function buy(
    ...
    /**
     * @dev checks signs validity, handle ask and bid transfers to counterparties, increments nonce
     *
     * @param order  order struct
     */
    function proceedOrder(Order calldata order, bytes32 orderHash) internal {
        ...
        if (
            ...
            fulfillOrderPart(order.ask, order.amount, _msgSender(), order.signer);
        }
        ...
        if (
            ...
            fulfillOrderPart(order.bid, order.amount, order.signer, _msgSender());
        }
    ...
    /**
     * @dev generates unique hash for an order
     *
     * @param order  order struct
     */
    function getOrderHash(
        Order calldata order // ToDo: глянуть ещё раз на эту функцию
...
contract Operator is Ownable {
    ...
    /**
     * @dev check that an address exists in operators map
     */
...
contract FeeProvider is Ownable {
    /**
     * @dev Emitted when changing `MarketplaceFee` for an `collection`.
     */
    ...
    /**
     * @dev Emitted when changing `customCollectionRoyalties` for an `collection`.
     */
    ...
    function changeMarketplaceCollectionFee(
        ...
        require(sellerFee < 10000 && buyerFee < 10000, "Wrong fee amount");
    ...
    function changeCollectionRoyalties(
        ...
        require(collection != address(0), "Wrong collection");
        require(recipients.length == amounts.length, "Wrong params length");
...
contract TransferHelper is Ownable, TokenTransferrer {
    ...
    /**
     * @dev internal function to transfer ETH to a given recipient
     *
     * @param to      the recipient of the transfer.
     * @param amount  the amount to transfer.
     */
    function _performETHTransfer(address to, uint256 amount) internal {
     ...
     * @param amount      amount of tokens only applied if assetType is ERC1155 or ERC20
     */
    function _transfer(
        address from,
        address to,
        ...
        return _performETHTransfer(to, amount);
    }
    ...
    function transfer(
        ...
        require(from == msg.sender, "Transfer: wrong sender");
        ...
        emit Transfer(from, to, assets.length);
    }
    ...
    function burn(Asset[] calldata assets) external {
        ...
        emit Burn(msg.sender, assets.length);
    }
    ...
    function withdrawFunds() external onlyOwner {
        _performETHTransfer(msg.sender, address(this).balance);
    }
}
