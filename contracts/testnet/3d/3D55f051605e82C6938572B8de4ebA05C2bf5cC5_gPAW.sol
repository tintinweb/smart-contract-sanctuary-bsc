...
import "../interfaces/IsHUG.sol";
import "../interfaces/IgPAW.sol";
import "../types/ERC20.sol";

contract gPAW is IgPAW, ERC20 {
    /* ========== DEPENDENCIES ========== */
    ...
    constructor(address _migrator, address _sHUG) ERC20("Governance ShibDAO", "PAW", 18) {
        require(_migrator != address(0), "Zero address: Migrator");
        approved = _migrator;
        require(_sHUG != address(0), "Zero address: sHUG");
        sHUG = IsHUG(_sHUG);
    }
     ...
     * @param _sHUG address
     */
    function migrate(address _staking, address _sHUG) external override onlyApproved {
        ...
        require(_sHUG != address(0), "Zero address found");
        sHUG = IsHUG(_sHUG);
    }
    ...
    /**
     * @notice pull index from sHUG token
     */
    ...
    /**
        @notice converts gPAW amount to BONE
        @param _amount uint
        @return uint
     */
    ...
    /**
        @notice converts BONE amount to gPAW
        @param _amount uint
        @return uint
     */
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "gPAW::getPriorVotes: not yet determined");
...
interface IsHUG is IERC20 {
    function rebase(uint256 boneProfit_, uint256 epoch_) external returns (uint256);
...
interface IgPAW is IERC20 {
    ...
    function migrate(address _staking, address _sHUG) external;
}
