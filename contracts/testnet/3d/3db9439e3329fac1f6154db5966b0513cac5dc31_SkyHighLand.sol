...
contract SkyHighLand is Context, IERC20, Ownable {
    ...
    string private _name = "Sky High Land";
    string private _symbol = "Sky High Land";
    ...
    address payable public developmentWallet = payable(0x787AAe22aeA1dCC22fb229d80Fda923508d0E891);
    uint256 private _previousdevelopmentFee = _developmentFee;

    uint256 public _charityFee = 10;
    address payable public charityWallet = payable(0x787AAe22aeA1dCC22fb229d80Fda923508d0E891);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee==0 && _developmentFee==0 && _charityFee==0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 combineFees = _liquidityFee.add(_marketingFee).add(_charityFee).add(_developmentFee); 
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(combineFees);
        uint256 tokensFordevelopment = contractTokenBalance.mul(_developmentFee).div(combineFees);
        uint256 tokensForcharity = contractTokenBalance.mul(_charityFee).div(combineFees); 
        
        uint256 tokensForMarketing = contractTokenBalance.sub(tokensForLiquidity).sub(tokensForcharity).sub(tokensFordevelopment);
        ...
        uint256 contract1Balance = address(this).balance;
        swapTokensForEth(tokensForcharity);
        uint256 transferred1Balance = address(this).balance.sub(contract1Balance);
        
        //Send to charity address
        transferToAddressETH(charityWallet, transferred1Balance);

        uint256 contract2Balance = address(this).balance;
        swapTokensForEth(tokensFordevelopment);
        uint256 transferred2Balance = address(this).balance.sub(contract2Balance);
        
        //Send to development address
        transferToAddressETH(developmentWallet, transferred2Balance);
    }
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tDevelopment = tAmount.div(1000).mul(_developmentFee);
        ...
        emit Transfer(sender, address(this), tDevelopment);
        return(tTransferAmount, rTransferAmount); 
    }
