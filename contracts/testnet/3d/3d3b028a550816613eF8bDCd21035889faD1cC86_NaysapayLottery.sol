...
// File: contracts/interfaces/INaysapayLottery.sol

pragma solidity ^0.8.4;

interface INaysapayLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in Knigtex per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in Knigtex token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInKnigtex: price of a ticket in Knigtex
...
// File: contracts/NaysapayLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title Naysapay Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract NaysapayLottery is ReentrancyGuard, INaysapayLottery, Ownable {
    ...
    uint256 public maxPriceTicketInKnigtex = 50 ether;
    uint256 public minPriceTicketInKnigtex = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInKnigtex;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] knigtexPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInKnigtex;
        uint32 finalNumber;
    }
     ...
     * @param _knigtexTokenAddress: address of the Knigtex token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _knigtexTokenAddress, address _randomGeneratorAddress) {
        knigtexToken = IERC20(_knigtexTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of Knigtex to this contract
        uint256 amountKnigtexToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInKnigtex,
            _ticketNumbers.length
        );

        // Transfer knigtex tokens to this contract
        knigtexToken.safeTransferFrom(address(msg.sender), address(this), amountKnigtexToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInKnigtex += amountKnigtexToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInKnigtexToTransfer
        uint256 rewardInKnigtexToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInKnigtexToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        knigtexToken.safeTransfer(msg.sender, rewardInKnigtexToTransfer);

        emit TicketsClaim(msg.sender, rewardInKnigtexToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInKnigtex) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in Knigtex for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].knigtexPerBracket[j] =
                ...
                // A. No Knigtex to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].knigtexPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInKnigtex - amountToShareToWinners);

        // Transfer Knigtex to treasury address
        knigtexToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        knigtexToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInKnigtex += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInKnigtex >= minPriceTicketInKnigtex) && (_priceTicketInKnigtex <= maxPriceTicketInKnigtex),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInKnigtex: _priceTicketInKnigtex,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            knigtexPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInKnigtex: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(knigtexToken), "Cannot be Knigtex token");
    ...
    /**
     * @notice Set Knigtex price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInKnigtex: minimum price of a ticket in Knigtex
     * @param _maxPriceTicketInKnigtex: maximum price of a ticket in Knigtex
     */
    function setMinAndMaxTicketPriceInKnigtex(uint256 _minPriceTicketInKnigtex, uint256 _maxPriceTicketInKnigtex)
        external
        onlyOwner
    {
        require(_minPriceTicketInKnigtex <= _maxPriceTicketInKnigtex, "minPrice must be < maxPrice");

        minPriceTicketInKnigtex = _minPriceTicketInKnigtex;
        maxPriceTicketInKnigtex = _maxPriceTicketInKnigtex;
    }
     ...
     * @param _priceTicket price of a ticket (in Knigtex)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].knigtexPerBracket[_bracket];
        } else {
            return 0;
        }
    }
