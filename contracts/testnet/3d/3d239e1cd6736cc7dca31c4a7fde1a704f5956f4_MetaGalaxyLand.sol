...
// File: contracts/MetaGalaxyLand.sol
pragma solidity ^0.8.1;

contract MetaGalaxyLand is VRFConsumerBase, ERC721Enumerable, Ownable {
    ...
    uint256 public MAX_TOKENS = 40000;
    ...
    ) VRFConsumerBase(_coordinator, _link) ERC721(name, symbol) {
        // Chainlink setters
        keyHash = _keyhash;
        fee = _fee;
    ...
    function mintTokens(uint256 numberOfTokens) public payable {
        ...
        if (msg.sender != owner()) {
            if(onlyWhitelisted == true) {
                ...
                require(ownerMintedCount + numberOfTokens <= MAX_MINTS, "max NFT per address exceeded");
            }
            require(msg.value >= TOKEN_PRICE * numberOfTokens, "insufficient funds");
        }
        ...
        returns (string memory)
    {
	    ...
	    return bytes(notRevealedUri).length > 0
            ? string(abi.encodePacked(notRevealedUri))
            : "";
        }else {
        return bytes(_baseTokenURI).length > 0
                ? string(abi.encodePacked(_baseTokenURI, randomNumber.toString(), ".json"))
                : "";
        }
    }
