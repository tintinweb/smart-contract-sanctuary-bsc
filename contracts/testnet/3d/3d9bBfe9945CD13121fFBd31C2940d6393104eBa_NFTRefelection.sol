...
contract NFTRefelection is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    */
    function updateUser(
        ...
        user.rewardDebt = (user.totalGgymnetAmt * pool.accRewardPerShare) / (1e18) - (user.rewardDebt);
        // updatePool();
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        uint256 sharesTotal = 20000000000000000000000;

        return (user.totalGgymnetAmt * _accRewardPerShare) / (1e18) - (pool.accRewardPerShare);
    }

    function updatePool(uint256 lastChangeId,uint256 receivedAmount) public {
        PoolInfo storage pool = poolInfo;

        if (lastChangeId <= pool.lastChangeId) {
            return;
        }
        ...
        uint256 multiplier = lastChangeId - pool.lastChangeId;
        if (multiplier <= 0) {
            return;
        }
        
        uint256 _reward = (multiplier * receivedAmount ) / 100 ;
