...
contract SelfClaim is Ownable, Pausable, ReentrancyGuard {
    ...
    constructor(address _laroToken) {
    ...
    function setSelfClaims(
        address[] memory addresses,
        uint256[] memory amounts
    ) external onlyOperators {
        require(addresses.length == amounts.length, "index mismatch");
        for (uint256 i = 0; i < addresses.length; i++) {
            claimables[addresses[i]] += amounts[i];
        }
    }

    function topupFunds(uint256 amount) external onlyOperators {
        laroToken.safeTransferFrom(msg.sender, address(this), amount);
    }

    function claimTokens() external nonReentrant {
        require(claimables[msg.sender] > 0, "No claimable");
        laroToken.safeTransfer(msg.sender, claimables[msg.sender]);
