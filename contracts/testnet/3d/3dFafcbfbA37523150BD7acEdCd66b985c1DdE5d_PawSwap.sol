...
contract PawSwap is Ownable {
    ...
    function sellOnPawSwap (
        ...
        if (_taxes.liquidityTax != 0) {
            ...
            liquidityTokens = tokensToSwap * _taxes.liquidityTax / 10**(_taxes.feeDecimal + 2);
        ...
        uint256 minEthInLpSwap = minEthToReceive;
        if (!isExactIn) {
            minEthInLpSwap += _taxes.liquidityTax + _taxes.burnTax + _taxes.tokenTax;
        }
        ...
        if (_taxes.liquidityTax != 0) {
            // add to the LP
            uint256 liquidityEth = newEthBalance * _taxes.liquidityTax / 10**(_taxes.feeDecimal + 2);
            addLiquidity(liquidityTokens, liquidityEth, tokenAddress);
    ...
    function swapEthForTokens(uint256 ethToSwap, uint256 minTokensToReceive, address tokenAddress, bool isExactIn) private {
        ...
        if (isExactIn) {
            ...
            uniswapV2Router.swapETHForExactTokens{value: ethToSwap}(
                minTokensToReceive + _taxes.liquidityTax + _taxes.burnTax + _taxes.tokenTax,
                path,
                address(this),
                block.timestamp
            );
