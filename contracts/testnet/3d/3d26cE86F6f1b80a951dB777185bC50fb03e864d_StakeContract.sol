...
contract StakeContract is Owner, ReentrancyGuard, Params {
    ...
    // MAKE PRIVATE IN PROD
    uint256 public feeMul; // example x5 = 500 // = 100 if the tokens are the same as the price
    ...
    // minimum stake time in seconds
    uint256 public constant minimumStakeTime_7 = 60; // 1 min in seconds
    uint256 public constant minimumStakeTime_15 = 120; // 2 min in seconds
    uint256 public constant minimumStakeTime_30 = 180; // 3 min in seconds
    ...
    constructor(uint8 _apy, ERC20 _token, ERC20 _feeToken, uint256 _feeMul, uint256 _tokenPrice) {
        APY = _apy;
        token = _token;
        feeToken = _feeToken;
        feeMul = _feeMul;
    ...
    function calculateInterest(uint256 _stakeAmount, uint256 _mstType) private view returns (uint256) {
