...
contract ICO is Ownable {
    ...
    uint256 public minContribution = 5000 * 10**18;      
    uint256 public maxContribution = 50000 * 10**18;   
    ...
    constructor(address _usdt, uint256 _startTimestamp, uint256 _endTimestamp, address _recipientAddress) {
        USDT = IERC20(_usdt);
    ...
    function contribute(uint256 _contribution) external {
        require(block.timestamp >= startTimestamp, "Presale not started yet");
		require(block.timestamp <= endTimestamp, "Presale ended");
        require(USDT.balanceOf(msg.sender) > _contribution, "Insufficient USDT");

        uint256 _totalContribution = userContribution[_msgSender()].add(_contribution);
        ...
        if(userContribution[_msgSender()] == 0)
            participantAddressList.push(_msgSender());

        //Forward funds to treasury
        forwardFunds(msg.sender, recipientAddress, _contribution);

        //Add contribution amount for selected address
        userContribution[_msgSender()] = userContribution[_msgSender()].add(_contribution);

        //Add contribution amount to totalBnbPool
        totalRaised = totalRaised.add(_contribution);

        emit Contributed(_msgSender(), _contribution);
    }

    function forwardFunds(address from, address to, uint256 amount) public {
