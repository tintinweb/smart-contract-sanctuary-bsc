...
interface IWalletBridge {
    ...
    function send(address erc20, address receiver) external;
}

interface IGetdoneDisputation {
    function create(uint256 merchantId, uint256 appId, uint256 contractId, address customer, address talent, uint256 price, address erc20, address sender) external;
}
...
contract GetdoneEscrow is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, AccessControlEnumerableUpgradeable, GetdoneApp{
    ...
    event ContractDisputationSent(uint256 merchantId, uint256 appId, uint256 contractId, uint256 amount, address sender, address bridge, bool isTalentVoted, bool isCustomerVoted);
    ...
    event WalletBridgeUpdated(address wallet);
    ...
    address public walletBridgeAddress;
    address public disputationContractAddress;
    ...
    function updateWalletBridge(address wallet)
        external
    {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) || owner() == _msgSender(), "GetdoneEscrow: caller is not the admin");
        require(wallet != address(0), "GetdoneEscrow: address is invalid");

        walletBridgeAddress = wallet;

        emit WalletBridgeUpdated(wallet);
    }

    function updateDisputationContract(address addr)
        external
    {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) || owner() == _msgSender(), "GetdoneEscrow: caller is not the admin");
        require(addr != address(0), "GetdoneEscrow: address is invalid");

        disputationContractAddress = addr;

        emit DisputationContractUpdated(addr);
    }
    ...
    function createDisputation(uint256 merchantId, uint256 appId, uint256 contractId, address customer) 
        ...
        nonReentrant
    {
        require(walletBridgeAddress != address(0), "GetdoneEscrow: Wallet Bridge Address is invalid");
        require(disputationContractAddress != address(0), "GetdoneEscrow: Disputation Address is invalid");
        require(validApp(merchantId, appId), "GetdoneEscrow: App is invalid");
        address sender = _msgSender();
        ContractInfo storage contractInfo = contractInfos[merchantId][appId][customer][contractId];
        ...
        IGetdoneDisputation(disputationContractAddress).create(merchantId, appId, contractId, customer, contractInfo.talent, amountRefund, contractInfo.erc20, sender);
        if (amountRefund > 0 && amountRefund <= contractBalance) {
            address bridge = walletBridgeAddress.clone();
            IWalletBridge(bridge).initialize();
            if (contractInfo.erc20 == address(0)) {
                payable(bridge).transfer(amountRefund);
            } else {
                IERC20Upgradeable(contractInfo.erc20).safeTransfer(bridge, amountRefund);
            }
            IWalletBridge(bridge).send(contractInfo.erc20, disputationContractAddress);
            contractDisputation.bridge = bridge;
            contractInfo.refund = contractInfo.refund + amountRefund;
        }
        ...
        emit ContractDisputationSent(merchantId, appId, contractId, amountRefund, sender, contractDisputation.bridge, contractInfo.talent == sender, contractInfo.customer == sender);
    }
        ...
        internal
    {
        ...
        if (erc20 == address(0)) {
            if (fee > 0) {
                payable(apps[merchantId][appId].treasury).transfer(fee);
            }
            ...
            if (fee > 0) {
                IERC20Upgradeable(erc20).safeTransfer(apps[merchantId][appId].treasury, fee);
            }
            if (amount > 0) {
