...
import {IDesonPair} from "./interfaces/IDesonPair.sol";
import {IDesonRouter02} from "./interfaces/IDesonRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by DesonSwap teams.
 */
contract DesonZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // DesonRouter interface
    IDesonRouter02 public dasunRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from DesonSwap)
    ...
    // Address DesonRouter
    address private dasunRouterAddress;
     ...
     * @param _dasunRouter: address of the DesonRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _dasunRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        dasunRouterAddress = _dasunRouter;
        dasunRouter = IDesonRouter02(_dasunRouter);
    ...
    {
        address token0 = IDesonPair(_lpToken).token0();
        address token1 = IDesonPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IDesonPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = dasunRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = dasunRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IDesonPair(_lpToken).token0() || _token0ToZap == IDesonPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IDesonPair(_lpToken).token0() || _token1ToZap == IDesonPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IDesonPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IDesonPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IDesonPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = dasunRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = dasunRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IDesonPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "DesonRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "DesonRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        dasunRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), dasunRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(dasunRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = dasunRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = dasunRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = dasunRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = dasunRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = dasunRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
...
import "./IDesonRouter01.sol";

interface IDesonRouter02 is IDesonRouter01 {
