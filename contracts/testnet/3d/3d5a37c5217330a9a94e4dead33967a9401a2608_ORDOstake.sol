...
contract ORDOstake {
    ...
    uint256[6] public Duration = [30 days, 60 days, 90 days, 180 days, 360 days, 545 days];
    uint256[6] public Bonus = [30, 40, 70, 110, 160, 240];
    ...
    function stake(uint256 amount1, uint256 timeperiod) public {
        ...
        uint256 ORDOVAL = getPriceinUSD();
        uint256 amount = amount1.sub((amount1.mul(totalFee)).div(percentDivider));
        uint256 rewardtokenPrice = (amount.mul(ORDOVAL)).div(1e9);
    ...
    function getPriceinUSD() public view returns (uint256){
        ...
        address BUSD_WBNB = 0x5126C1B8b4368c6F07292932451230Ba53a6eB7A; //BUSD_WBNB pancake pool address
        ...
        address ORDO_WBNB = 0xd0d7d613DD9722D4B94AF6c31E95EE380dfCFF3F; // ORDO_WBNB pancake pool address
        IERC20 ORDOTOKEN = IERC20(0x445da459E18A4D54717664ae329913173d3920bf); // ORDO Token address 

        uint256 WBNBSUPPLYINORDO_WBNB =(WBNBTOKEN.balanceOf(ORDO_WBNB));
        uint256 ORDOSUPPLYINORDO_WBNB = (ORDOTOKEN.balanceOf(ORDO_WBNB));

        uint256 ORDOUSDVAL = (((WBNBSUPPLYINORDO_WBNB.mul(1e9)).div((ORDOSUPPLYINORDO_WBNB))).mul(BNBPRICE)).div(1e18);
        return ORDOUSDVAL;
    }
