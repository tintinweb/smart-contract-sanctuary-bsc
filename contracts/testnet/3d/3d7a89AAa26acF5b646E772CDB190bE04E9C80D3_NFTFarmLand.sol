...
contract NFTFarmLand is ERC721, Callerable {
    ...
    mapping(uint256 => MaxNumberData) max_land_number_list;
    ...
    constructor(string memory name_, string memory symbol_, address _auth) Callerable(_auth) ERC721(name_, symbol_) {
        ...
        max_land_number_list[0] = MaxNumberData(0, 30000);
        max_land_number_list[1] = MaxNumberData(0, 20000);
        max_land_number_list[2] = MaxNumberData(0, 15000);
        max_land_number_list[3] = MaxNumberData(0, 12000);
        max_land_number_list[4] = MaxNumberData(0, 10000);
        max_land_number_list[5] = MaxNumberData(0, 5000);
        max_land_number_list[6] = MaxNumberData(0, 2000);
    }
    ...
    function mintLand(uint256 price, address toAccount) external onlyCaller returns (uint256 tokenId){
        (bool b, uint256 area, string memory picture, uint256 index) = _getLand(price);
    ...
    function getLand(uint256 price) external view returns (bool res, uint256 area, string memory picture, uint256 index) {
        (res, area, picture, index) = _getLand(price);
    }

    function _getLand(uint256 price) internal view returns (bool res, uint256 area, string memory picture, uint256 index) {
