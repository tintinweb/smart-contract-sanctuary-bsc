...
contract HardStakingNFTAuctionCudstodialV2 is ReentrancyGuard, Ownable, IERC721Receiver {
    ...
    event Withdraw(address indexed user, uint auctionId, uint amount);
    ...
    function processSuccesfullAuction(uint auctionId) external { 
        ...
        if (winner == address(0)) {
            ...
            if (royaltyAmount > 0 && royaltyAmount < bidFinalAmount && author != address(0)) {
                if (!isCustodial) {
                      purchaseToken.safeTransfer(author, royaltyAmount);
    ...
    function startNewAuction(uint tokenId, uint startBidAmount, string memory description, bool isCustodial) public {
        ...
        emit NewAuction(auctionId, tokenId, collectionId, isCustodial, isDigitalObject);
    }
    ...
    function _stake(uint auctionId, uint amount, address user) internal virtual nonReentrant {
        ...
        Auction storage auction = auctions[auctionId];
        require(auction.owner != user, "HardStakingNFTAuctionCudstodial: You cannot bid on your auction");
    ...
    function _withdraw(uint auctionId) internal virtual nonReentrant {
        ...
        emit Withdraw(user, auctionId, amount);
    }
