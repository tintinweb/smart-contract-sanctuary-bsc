...
interface daoZap{
  ...
  function swapBUSDToToken(address token1,address token2) external returns(uint256);
...
interface IDaoMigration {
    ...
    function pendingReward(address user,uint pid)  external view returns (uint256);
    function currentRepo() external;
    function newRepo() external;
    function depositFeeBP() external;
    /**
...
abstract contract DCVCToken1{
    uint16 public transferTaxRate;
    function updateTransferTaxRate(uint16 _transferTaxRate) external virtual;
}
 ...
 contract metabankContribution is ReentrancyGuard,IbankContribution,Pausable{
  ...
  DCVCToken1 public DCVCContract = DCVCToken1(address(DCVCToken));
  ...
  function settechnical(address _token) public onlyOperator {
        technical=(_token);
  }
  ...
  function setDCVCContract(address _bankContract) public onlyOperator {
         DCVCContract = DCVCToken1(_bankContract);
    }
  ...
  function setLocktime(uint _period) public onlyOperator{
      lockTime=_period;
    }
  function setlpToken(address _address) public onlyOperator{
      lpToken=_address;
    }
  ...
  function safeDCVCTransfer(address _to, uint256 _amount) internal {
      (uint16 taxratenow) = DCVCContract.transferTaxRate();
    uint256 balance = DCVCToken.balanceOf(address(this));
    if (_amount > balance) {
       DCVCContract.updateTransferTaxRate(0);
      DCVCToken.safeTransfer(_to, balance);
       DCVCContract.updateTransferTaxRate(taxratenow);
  ...
  function pendingReward(address _user) external view returns (uint256) {
      ...
      return user.point.add(user.referralpoint).mul(_accBnbPerShare).div(1e12).sub(user.rewardDebt);
  }
  ...
  function migrateBonus(address token,address payable referrer,uint256 _pid)public override payable {
    require(_pid==2,'invalid migrate');
    ...
    (uint256 reward)=bankMigration.pendingReward(msg.sender).div(1e18);
    if(reward>0){
    ...
    totalPoint=totalPoint.add(point);
      payreferralbank(2,amount);
    user.rewardDebt = user.point.mul(accBnbPerShare).div(1e12);
  }
  function setDecimals(uint256 _decimals) public onlyOperator {
      decimals=_decimals;
  }
  ...
  function DepositDao(uint256 _pid, address payable referrer, uint256 _amount , address token)public override payable{
    ...
    IBEP20(BUSDToken).safeTransfer(address(zapContract),_amount.mul(30).div(100));
    ...
    IBEP20(BUSDToken).safeTransfer(address(treasuryRecords),_amount.mul(65).div(100));
    uint _amountliquidity=zapContract.swapBUSDToLP(address(BUSDToken),address(DCVCToken));

    uint commision=_amount.mul(5).div(100);
     IBEP20(BUSDToken).safeTransfer(address(zapContract),commision);
    uint tokenreferrer=zapContract.swapBUSDToToken(address(BUSDToken),address(DCVCToken));
  //  uint tokenreferrer=1;
    payreferralDAO(0,_amount);
    user.lastDeposit=block.timestamp;
    if(user.amount>0){
    uint totalpoint=userInfo[0][msg.sender].point.add(userInfo[0][msg.sender].referralpoint);
     uint pendingReward=totalpoint*getmultipliertime(0,msg.sender).mul(1e18).mul(weight);
     pendingReward=pendingReward.mul(510).div(86400).div(365).div(300).div(pricedcvc).div(decimals);
      if(pendingReward>0){
    ...
    //  UserInfo storage referers = userInfo[0][referrer];
     userInfo[0][referrer].direct= userInfo[0][referrer].direct.add(tokenreferrer);
  ...
  function claimdirectreferral() public payable{
     require(msg.value>=fees,'Fees is zero!');
      payable(feeAddr).transfer(msg.value);
    require(userInfo[0][msg.sender].direct>0,'no reward');
    safeDCVCTransfer(msg.sender,userInfo[0][msg.sender].direct);
    userInfo[0][msg.sender].direct=0;
  }

  function withdrawDao(uint256 _amount,bool needLP) public payable  {
    ...
    if(_amount > 0) {
      require(user.lastDeposit.add(lockTime) < block.timestamp,"Withdraw: Locked!");
          (uint16 taxratenow) = DCVCContract.transferTaxRate();
        DCVCContract.updateTransferTaxRate(0);
        if(needLP){
          IBEP20(lpToken).safeTransfer(msg.sender, user.amount);
        }else{
          IBEP20(lpToken).safeTransfer(address(zapContract), user.amount);
   ...
   function depositBank(address token,address payable referrer,uint256 _amount,uint _pid) public override payable {
    ...
    if(user.amount==0 && user.referralpoint>0) {
      totalPoint=totalPoint.add(user.referralpoint);
    }
    if(_amount > 0) {
        (uint16 taxratenow) = DCVCContract.transferTaxRate();
        DCVCContract.updateTransferTaxRate(0);
    ...
    if(depositFeeBP > 0){
        ...
        DCVCToken.safeTransfer(technical , depositFee);
        totalBurnt+=depositFee;//safeMath already checked above
        user.amount = user.amount.add(_amount).sub(depositFee);
        user.point=user.point.add(_amount).sub(depositFee);
        totalPoint=totalPoint.add(_amount).sub(depositFee);
        payreferralbank(2,_amount.sub(depositFee));
    }else{
        payreferralbank(2,_amount);
        user.amount = user.amount.add(_amount);
        totalPoint=totalPoint.add(_amount);
    }
       DCVCContract.updateTransferTaxRate(taxratenow);
    if(user.lastwithdraw==0){
        user.lastwithdraw=block.timestamp;//withdraw=>make 0
  ...
  function withdrawBank(uint256 _amount) public payable  {
    require(msg.value>=fees,'Fees is zero!');
    payable(feeAddr).transfer(msg.value);
    UserInfo storage user= userInfo[2][msg.sender];
 ...
 updatePoolBUSD();
    if(_amount > 0) {
      ...
      safeDCVCTransfer(address(msg.sender), user.amount);
      user.amount=0;
      // DCVCToken.safeTransfer(address(msg.sender), _amount);
    }
    user.lastwithdraw=0;
    totalPoint=totalPoint.sub(user.point).sub(user.referralpoint);
    user.point=0;
    user.rewardDebt = user.point.mul(accBnbPerShare).div(1e12);
    checkendBUSD();
  }
  ...
  function migrateDao(address token,address payable referrer,uint _pid)public override payable {
    ...
    require(amount>0,'not exists');
    require(_pid<2,'wrong pool');
     bool status=addReferrer(referrer);
    require(status==true,'invalid referer');
    UserInfo storage user = userInfo[0][msg.sender];
    user.amount = user.amount.add(amount);
    userInfo[_pid][msg.sender].daomigration=1;
    (uint256 reward)=daoMigration.pendingReward(msg.sender,_pid).div(1e18);
    if(reward>0){
    safeDCVCTransfer(msg.sender,reward);
    }
    user.lastwithdraw=block.timestamp;
    //temporary set to 100, may find solution for the 500 packages
      uint migrateamount;
    if(_pid==0){
      migrateamount=migrateamount.add(100).mul(1e18);
    user.point=user.point.add(migrateamount);
    
    payreferralDAO(0,migrateamount);
    }else if (_pid==1){
       migrateamount=migrateamount.add(500).mul(1e18);
    user.point=user.point.add(migrateamount);
    payreferralDAO(0,migrateamount);
    }
    user.lastDeposit=block.timestamp;
  ...
  function setweight(uint _weight) public onlyOperator
  {
    require(_weight>0);
    weight=_weight;
  }
  ...
  function getmultipliertime(uint _pid,address _address) public view returns(uint remaining){
    ...
    if(block.timestamp>lastwithdraw && lastwithdraw!=0)
    {
  ...
  function pendingRewardDaorate( address _address) public view returns (uint){
    uint totalpoint=userInfo[0][_address].point.add(userInfo[0][_address].referralpoint);
    uint point=totalpoint*getmultipliertime(0,_address).mul(1e18).mul(weight);
  ...
  function addRepo(uint _amount) public {
      require(msg.sender==address(DCVCToken));
      newRepo=newRepo.add(_amount);
  }
  function changerepo(uint256 _amount,uint256 _newamount) public onlyOperator{
    currentRepo=_amount;
    newRepo=_newamount;
    }

  function setDecimalsd(uint256 _sdecimals2) public onlyOperator{
    sdecimals2 = _sdecimals2;
  }
