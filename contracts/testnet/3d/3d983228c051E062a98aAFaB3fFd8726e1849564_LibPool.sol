...
import "../interfaces/IRouterPoolFunctionality.sol";
...
library LibPool {
    ...
    ) public returns(int112 amountA, int112 amountB) { 

        // AmountADesired and AmountBDesired are now actually the real amounts (Saving space and gas)
        if(assetAddressA == address(0)){
            (amountBDesired, amountADesired, ) = IRouterPoolFunctionality(_orionpoolRouter).addLiquidityETHFromExchange{ value: amountADesired }(assetAddressB, amountBDesired, amountBMin, amountAMin, msg.sender, deadline);
        } else {
            (amountADesired, amountBDesired, ) = IRouterPoolFunctionality(_orionpoolRouter).addLiquidityFromExchange(assetAddressA, assetAddressB, amountADesired, amountBDesired, amountAMin, amountBMin, msg.sender, deadline);
        }

        // AmountADesired and AmountBDesired are now converted real amounts (Saving space and gas)
        int112 amountA = LibUnitConverter.baseUnitToDecimal(assetAddressA,amountADesired);
        int112 amountB = LibUnitConverter.baseUnitToDecimal(assetAddressB,amountBDesired);
        
        assetBalances[msg.sender][assetAddressA] -= amountA;
        require(assetBalances[msg.sender][assetAddressA] >= 0, "E1w1"); //TODO
        assetBalances[msg.sender][assetAddressB] -= amountB;
        require(assetBalances[msg.sender][assetAddressB] >= 0, "E1w1"); //TODO
...
interface IRouterPoolFunctionality {
...
function addLiquidityETHFromExchange(
        address token,
