...
contract MiningPools is AccessControl {
    ...
    IProxy public proxy;
    uint256 private materialDecimal = 1e36;
    uint256 private blockDecimal = 1e9;
    ...
    struct MineInfo {
        ...
        uint256 startBlock;
        uint256[16] materialsOutput;
        uint256[16] exchanged;
        uint256[16] oldOutput;
    }
    ...
    event Exchange(uint256 mid, address user, uint256[16] amount);
    ...
    event NewMine(uint256 indexed mid, uint256[16] materialsOutput);
    event MineOutputChange(uint256 indexed mid, uint256[16] materialsOutput);
    event TransferPoint(
        uint256 mid,
        address sender,
        address receiver,
    ...
    function getMineExchanged(uint256 _mid)
        public
        view
        returns (uint256[16] memory)
    {
        return mineInfo[_mid].exchanged;
    }

    function getMineOldOutput(uint256 _mid)
        public
        view
        returns (uint256[16] memory)
    {
        return mineInfo[_mid].oldOutput;
    }

    function exchange(uint256 _mid, uint256[16] memory amount) public {
        MineInfo storage mine = mineInfo[_mid];
        ...
        point = point * materialDecimal;
        require(user.balance >= point, "Must be have point to exchange");
        uint256[16] memory available = getAvailableMaterials(_mid);
        ...
        for (uint256 j = 0; j < 16; j++) {
            if (amount[j] > 0) {
                ...
                proxy.mint(j, msg.sender, amount[j]);
            }
        }
        emit Exchange(_mid, msg.sender, amount);
    }

    function addMine(uint256 startBlock, uint256[16] memory materialsOutput)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
        returns (uint256)
    {
    ...
    function transferPoint(
        ...
        emit TransferPoint(_mid, msg.sender, _receiver, _amount);
    }

    // Unstake tokens from Mine.
    function unstake(uint256 _mid, uint256 _amount) external returns (uint256) {
        MineInfo storage mine = mineInfo[_mid];
        UserInfo storage user = userInfo[_mid][msg.sender];
        ...
        returns (uint256)
    {
        ...
        if (block.number > mine.lastRewardBlock && mine.totalSupply != 0) {
            uint256 pointAdded = ((block.number).sub(mine.lastRewardBlock)).mul(
                getTotalPointPerBlock(_mid)
            );
            pointPerShare = pointPerShare.add(
                pointAdded.div(mine.totalSupply).div(blockDecimal)
            );
        }
        return user.amount.mul(pointPerShare).sub(user.rewardDebt);
    }

    function updateMineOutput(uint256 _mid, uint256[16] memory materialsOutput)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        MineInfo storage mine = mineInfo[_mid];
        updateMine(_mid);
        uint256[16] memory oldOutput = getAvailableMaterials(_mid);
        for (uint256 i = 0; i < 16; i++) {
    ...
    //Get total point per block mul 1e9;mul 1e36 for point div
    function getTotalPointPerBlock(uint256 _mid) public view returns (uint256) {
        uint256 totalPoint = 0;
        uint256[16] memory materialsOutput = mineInfo[_mid].materialsOutput;
        for (uint256 i = 0; i < 16; i++) {
            totalPoint = totalPoint.add(materialsOutput[i].mul(material[i]));
        }
        return totalPoint.mul(materialDecimal);
    }
