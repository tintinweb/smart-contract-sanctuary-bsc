 ...
 */
contract NFTMarket_ is NFTMarketFees_, NFTMarketOffer {
    /**
    ...
    function initialize(
        address admin,
        INFTMarket nftMarket,
        IMoDApproveProxy moDApproveProxy
    ) external initializer {
        AccessControlUpgradeable.__AccessControl_init();
        _setupRole(DEFAULT_ADMIN_ROLE, admin);

        NFTMarketOffer._initializeNFTMarketOffer();
        NFTMarketFees_._initializeNFTMarketFees(nftMarket, moDApproveProxy);
    }
}
 ...
 */
abstract contract NFTMarketFees_ is Initializable, AccessControlUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using AddressUpgradeable for address;

    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;

    INFTMarket public INM;
    IMoDApproveProxy public IMODA;

    uint256 private nextOfferId;
    uint256 private nextListingId;

    function _initializeNFTMarketFees(INFTMarket _INM, IMoDApproveProxy _IMODA)
        internal
        onlyInitializing
    {
        INM = _INM;
    ...
    {
        return INM._getFees(nftContract, tokenId, seller, price);
    }
...
abstract contract NFTMarketOffer is NFTMarketFees_ {
    using AddressUpgradeable for address;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    ...
    modifier onlyContract(address addr) {
        require(addr.isContract(), "NMO: not contract address");
        _;
    }
    ...
    modifier onlyAdminOrNonPrimaryMarket(address nftContract) {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ||
                INM.getIsAllowedContract(nftContract),
            "NMO: Caller does not have the admin role and this is a primary market for this contract"
        );
        _;
    }
    ...
    function createListing(
        ...
        uint256 listingId = _getNextAndIncrementListingId();

        require(
            endTime >= block.timestamp + _minOfferTime,
            "NMO: Too short offer time."
        );
        require(price > 0, "NMO: Offer price must be greater than zero.");
        require(
            IERC721Upgradeable(nftContract).ownerOf(tokenId) != address(0),
            "NMO: NFT does not exist."
        );
    ...
    function buyListing(uint listingId) public {
        ...
        IMODA.DAR_transferFrom(msg.sender, listing.seller, listing.price);
    ...
    function createOffer(
        ...
        uint256 offerId = _getNextAndIncrementOfferId();
        ...
        IMODA.DAR_transferFrom(msg.sender, address(INM), price);
    ...
    function cancelOffer(uint256 offerId) external {
        ...
        require(msg.sender == offer.buyer, "NMO: Not your offer");

        delete offerIdToOffer[offerId];

        INM.refund(msg.sender, offer.price);

        emit OfferCanceled(offerId);
    }

    function finalizeOffer(uint256 offerId) external {
        Offer memory offer = offerIdToOffer[offerId];

        require(offer.endTime > block.timestamp, "NMO: Offer time has expired");
    ...
    function updateMinOfferTime(uint256 minOfferTime)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            minOfferTime >= 1 minutes,
            "NMO: can not be less than 1 minute"
        );
...
interface INFTMarket {
    function _primarySalesWallet() external view returns (address);

    function _secondarySalesWallet() external view returns (address);
    ...
    function getIsAllowedContract(address nftContract) external returns (bool);

    function getSecondaryMarketCreatorFee() external view returns (uint256);
    ...
    function refund(address to, uint amount) external;
}
