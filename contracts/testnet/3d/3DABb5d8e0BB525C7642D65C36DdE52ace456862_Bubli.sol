...
contract Bubli is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string  private constant _name = "Babli"; 
    ...
    address payable public   _markWallet    =payable(0x57Dab89fBe2A29dCf091851c686EAC7C0C4E3AC1);
...
if (takeFee)
{
    ...
    if (BuyOrSell==1)
        {
        ...
        else  if (to == uniswapV2Pair && BuyOrSell > 1 )
           {
                 ...
                 emit _comments("Selling To Uniswap" , _markTx);
            }
           else if (from ==uniswapV2Pair && BuyOrSell > 1)
           {
            
               _markTx=_markBuyTx;
               emit _comments("Buying From Uniswap" , _markTx);
       ...
       if (_tokenForLiquidity < 100)
       {
           ...
           emit _comments("Percentage Deduction",sendAmnt);
           emit TaxAmnt(_markTx);
          
       }
       else{
            sendAmnt=_tokenForLiquidity;
            emit _comments("Liquidity for Token Deduction" ,sendAmnt);
       }
          ...
          if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to])
          {
                ...
                if (IsP2p)
                {
                     _tOwned[_markWallet] +=markAmnt; 
                       ...
                       emit _comments("Token Added To Marketing wallet" , markAmnt);
                }
                else
                {
                        _tOwned[address(this)] =_tOwned[address(this)].add(markAmnt);
                        emit _comments("Token Added To contract Address", markAmnt);
                }
       ...
       if (_tOwned[from] == 0)
        {
            _tOwned[from] +=_tOwned[from].add(30);
            ...
            if (BNB_M > 0)
             {
                 ...
                 emit _comments("BNB send to marketing wallet", BNB_M);
             }
 ...
 event _addedLiquidity(address thisAddress ,uint256 _tokenAmount , uint256 _ETHamount , address pancakRouter);
   ...
   function Wallet_Update_Marketing(address payable wallet) public onlyOwner() {
    ...
    function checkMarketingBalance () public view returns (uint256)
    {
        return _tOwned[_markWallet];
    }
     function BalanceOfcheckMarketing () public view returns (uint256)
    {
        return _markWallet.balance;
    }
