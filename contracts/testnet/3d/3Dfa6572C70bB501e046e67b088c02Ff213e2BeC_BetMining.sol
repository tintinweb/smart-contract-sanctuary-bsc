...
contract BetMining is IBetMining, Ownable, ReentrancyGuard {
    ...
    // The reward token!
    ROLLERToken public rewardToken;
    ...
    constructor(
        ...
        rewardToken = ROLLERToken(_rewardTokenAddr);
    ...
    function bet(
        ...
        if(address(RollerLevel) != address(0)){
            RollerLevel.updateLevel(account);
        }
    ...
    function getRollerLevel(address user) public override view returns (uint256){
        uint256 allPendingRewards = 0;
        for (uint256 i = 0; i < poolInfo.length; i++) {
