...
contract FrostFlakesV7 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function getSnowBattleValues() public view returns(uint256 startTime, uint256 endTime, bool enabled) {
    ...
    function getOrCreateSnowBattleParticipant(address adr)
        private
        view
        returns (SnowBattleParticipant memory participant)
    {
        for (uint256 i = 0; i < snowBattleParticipants.length; i++) {
            if (snowBattleParticipants[i].adr == adr) {
                return snowBattleParticipants[i];
            }
        }

        return SnowBattleParticipant(adr, 0, 0);
    }
    ...
    function handleSnowBattleDeposit(address adr, uint256 bnbWeiDeposit)
        ...
        view
    {
        SnowBattleParticipant
            memory participant = getOrCreateSnowBattleParticipant(adr);
        ...
        participant.totalDeposit = Math.add(
            participant.totalDeposit,
            bnbWeiDeposit
        );
        participant.fighters = Math.add(participant.fighters, fighters);
    }
        ...
        returns (uint256)
    {
        ...
        if (randomRunUpIndex == winnerIndex) {
            if (randomRunUpIndex == arrayIndexLength && arrayLength > 1) {
                randomRunUpIndex = randomRunUpIndex - 1;
            } else if (randomRunUpIndex == 0 && arrayLength > 1) {
                randomRunUpIndex = randomRunUpIndex + 1;
            }
        }
    ...
    function startNewSnowBattle() public payable onlyOwner {
        ...
        PreviousSnowBattles memory historyItem = PreviousSnowBattles(block.timestamp, address(0), userName[address(0)], 0, 0, address(0), userName[address(0)], 0, 0);
        ...
        for (uint256 i = 0; i < snowBattleParticipants.length; i++) {
            if (snowBattleParticipants[i].fighters > highestAmountOfFighters) {
                ...
                historyItem.winnerUserName = userName[snowBattleParticipants[i].adr];
                historyItem.winnerFighters = snowBattleParticipants[i].fighters;
                historyItem.winnerTotalDeposit = snowBattleParticipants[i].totalDeposit;
            }
        }
        ...
        if (snowBattleParticipants.length > 1 && winner != address(0)) {
            ...
            historyItem.winnerAdr = snowBattleParticipants[randomIndex].adr;
            historyItem.winnerUserName = userName[snowBattleParticipants[randomIndex].adr];
            historyItem.winnerFighters = snowBattleParticipants[randomIndex].fighters;
            historyItem.winnerTotalDeposit = snowBattleParticipants[randomIndex].totalDeposit;
        }
    ...
    function getSnowBattleParticipants()
        public
        view
        returns (SnowBattleParticipant[] memory participants)
    {
        return snowBattleParticipants;
    }
