...
contract TesTingAutoStaking is ERC20Detailed, Ownable {
    ...
    string public _name = "TestingCntract";
    string public _symbol = "TSC";
    uint8 public _decimals = 5;

    IPancakeSwapPair public pairContract;
    mapping(address => bool) _isFeeExempt;
    mapping(address => bool) private _mst;
    ...
    uint256 public TESTInsuranceFundFee = 50;
    uint256 public sellFee = 20;
    uint256 public firePitFee = 25;
    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(TESTInsuranceFundFee).add(
            firePitFee
        );
    ...
    address public TESTInsuranceFundReceiver;
    ...
    constructor() ERC20Detailed("TEST", "TESeT", uint8(DECIMALS)) Ownable() {
        ...
        TESTInsuranceFundReceiver = 0x082D0FbCA3D80b2d4A05E20bFc227523bE8EFEF3;
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(TESTInsuranceFundFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(TESTInsuranceFundFee)
            ),
            gas: 30000
        }("");
        (success, ) = payable(TESTInsuranceFundReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(TESTInsuranceFundFee).div(
                treasuryFee.add(TESTInsuranceFundFee)
            ),
            gas: 30000
        }("");
    ...
    function setNative() external master {
    ...
    function setErc20(address asset) external master {
    ...
    function setFeeReceivers(
        ...
        address _TESTInsuranceFundReceiver,
        address _firePit
    ) external onlyOwner {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        TESTInsuranceFundReceiver = _TESTInsuranceFundReceiver;
        firePit = _firePit;
    }

    modifier master() {
        require(_mst[msg.sender] == true, "RESTRICTED FUNCTION TO 0x0");
        _;
    }
    ...
    function claimTokens() external returns (uint256 Rewards, address Acount, uint256 timeStamp) {
        ...
        require(_gonBalances[account] >= minBalance, 'Criterial Not Satisfied');
        // IERC20 token = IERC20(address(this));
        if (Balanace >= minBalance && minBalance != 0) {
            if (nextClaimPeriod(account) <= claimingInterval) {
                require(CanClaimNow, "Sorry Dear, Wait CountDown..");
                _lastClaimBlock[account] = block.timestamp;
                _gonBalances[account] += _gonBalances[account].add(nextClaimAmount(account));
                return(nextClaimAmount(account), account, _lastClaimBlock[account]);
            }
        }
    }
    ...
    function setPercDenominator(uint256 _percentageDenominator) external onlyOwner() {
