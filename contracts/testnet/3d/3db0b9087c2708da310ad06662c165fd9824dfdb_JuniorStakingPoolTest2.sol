...
contract JuniorStakingPoolTest2 is Ownable {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256 totalDeservedReward,uint256 referralBonus, uint256 finalyPendingReward) {
		...
		for (uint i = 0; i < transactionInfos.length; i++) {
			if(transactionInfos[i].lpToken == pool.lpToken && transactionInfos[i].user == _user){
				uint256 deservedReward = transactionInfos[i].amount.mul(getBoostMultiplier(i).mul(accYWTPerShare)).div(1e12);
		...
		finalyPendingReward = totalDeservedReward.add(referralBonus).sub(user.claimedReward);
   ...
   // Update reward variables of the given pool to be up-to-date.
    ...
    function getPendingReward(uint256 _pid, address _user) private returns (uint256 totalDeservedReward,uint256 finalyPendingReward) {
        ...
        for (uint i = 0; i < transactionInfos.length; i++) {
            if(transactionInfos[i].lpToken == pool.lpToken && transactionInfos[i].user == _user){
				transactionInfos[i].deservedReward = transactionInfos[i].amount.mul(getBoostMultiplier(i).mul(pool.accYWTPerShare)).div(1e12);
		...
		finalyPendingReward = totalDeservedReward.add(user.referralBonus).sub(user.claimedReward);
    ...
    function withdraw(uint256 _pid) public {
        ...
        if(allowedWithdrawLPAmount == user.totalDepositAmount) {
        ...
        for (uint i = 0; i < transactionInfos.length; i++) {
            if(transactionInfos[i].lpToken == pool.lpToken && transactionInfos[i].user == msg.sender) {
                if(block.timestamp > (transactionInfos[i].lastDepositTime + transactionInfos[i].depositDays)) {
                    if(i == transactionInfos.length.sub(1)){
                        transactionInfos.pop();
                        return;
                    }
                    for(uint j = i; j < transactionInfos.length - 1; j++) {
                        transactionInfos[j] = transactionInfos[j + 1];
                    }
                    transactionInfos.pop();
                }
            }
    ...
    function deleteAllWithdrawTX(uint256 _pid) private {  //0   2         
        PoolInfo storage pool = poolInfo[_pid];
        for (uint i = 0; i <= transactionInfos.length; i++) {
            if(transactionInfos[i].lpToken == pool.lpToken && transactionInfos[i].user == msg.sender) {
                if(i == transactionInfos.length.sub(1)){
                ...
                for (uint j = i; j < transactionInfos.length; j++) {                              
                    transactionInfos[i] = transactionInfos[i+1];
                }                
                transactionInfos.pop();
            }
        }
