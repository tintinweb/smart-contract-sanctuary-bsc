...
contract KochiInvest is Initializable, ContextUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, IKochiInvest, Verifiable {
  ...
  function _setReferrer(uint256 id, address user, address referrer) private {
    require(userReferrer[id][user] != referrer, "you have already set your referrer to this address.");
    require(referrer != user, "you cannot refer to yourself.");

    // not worried for overflow, since the number of referrals is small.

    // referrer isn't set
    bool replace = userReferrer[id][user] == address(0);
    if (replace) {
    ...
    // referrer is already set
      ...
      userMetadata[id][userReferrer[id][user]][EUserMetadata.referrals]--;
      userMetadata[id][referrer][EUserMetadata.referrals]++;
    }

    // set the referrer
    userReferrer[id][user] = referrer;
    emit UserReferral(id, user, referrer, userMetadata[id][referrer][EUserMetadata.referrals], !replace);
  }
