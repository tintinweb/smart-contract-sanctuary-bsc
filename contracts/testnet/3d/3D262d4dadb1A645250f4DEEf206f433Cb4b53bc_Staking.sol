...
contract Staking is OwnableUpgradeable, IERC721ReceiverUpgradeable {
    ...
    function _earned(address _account) private view returns (uint256 _rewardLTC, uint256 _rewardDOGE, uint256 _rewardAnto) {
        ...
        uint256 _miningPower = userInfoOf[_account].stakedNFTNum;
    ...
    function stake(uint256 _num) external updateReward(msg.sender) {
        ...
        totalSupplyNFT += _num;
        userInfoOf[msg.sender].stakedNFTNum += _num;
        ...
        for (uint256 _index; _index < _num; _index++) {
            uint256 _tokenId = IERC721EnumerableUpgradeable(CGKDAO_ADDRESS).tokenOfOwnerByIndex(msg.sender, 0);
    ...
    function unstake(uint256 _num) external updateReward(msg.sender) {
        uint256 _stakedNFTNum = userInfoOf[msg.sender].stakedNFTNum;
        require(userInfoOf[msg.sender].lastStakedTime >= block.timestamp + stakeThresholdTime, 'HeySheepStaking: The staking time is too short.');
        ...
        uint256 _unstakingNFTIndex = userInfoOf[msg.sender].stakedNFTIds.length - 1;
        for (uint256 _index; _index < _num; _index++) {
            uint256 _tokenId = userInfoOf[msg.sender].stakedNFTIds[_unstakingNFTIndex];
    ...
    (
        ...
        _stakedNFTNum = userInfoOf[_account].stakedNFTNum;
