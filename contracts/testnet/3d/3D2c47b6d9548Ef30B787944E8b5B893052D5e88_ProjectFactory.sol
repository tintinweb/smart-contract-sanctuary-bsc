...
contract IdoProject is ReentrancyGuard, Initializable {
    ...
    bool public isWithdrawAdmin = false;
    ...
    modifier checkSigner(bytes memory _signature, uint256 _slot) {
        require(_slot > 0, "_slot minimum is 1");
        bytes32 _hash = keccak256(abi.encodePacked(projectId, msg.sender, address(this), _slot)).toEthSignedMessageHash();
    ...
    modifier isWithdrawableAdmin() {
        bool check = block.timestamp > calculationTime && poolEnd && !isWithdrawAdmin;
        require(check, "!check");
        _;
    }

    //===================EVENTS===================
    event Setting(uint indexed projectId, bool indexed update, uint _registerStartTime, uint _calculationTime, uint _rate, address _currency,address _tokenSale, address _admin);
    event ProjectCreated(uint indexed projectId, address indexed contractAddress);
    event Register(address indexed user, uint indexed projectId);
    ...
    event WinnerMember(address indexed user, uint indexed projectId);
    event Slot(uint indexed _slot, uint indexed projectId);
    ...
    //=================== External ===================== //
    function registerProject(bytes memory _signature, uint256 _slot) external onPoolSale checkSigner(_signature, _slot) {
        currency.safeTransferFrom(msg.sender, address(this), maxAllocation * _slot);
        commit[msg.sender] = maxAllocation * _slot;
        users.add(msg.sender);
        emit Register(msg.sender, projectId);
    }

    function createRandomNumber() external onlyOperator afterSale {
        if (randomSeed == 0) {
            randomSeed = Random.computerSeed(block.number);
            emit CreateRandom(randomSeed , projectId);
        }
    }
    ...
    function withdrawAdmin() external onlyOperator isWithdrawableAdmin() nonReentrant {
        isWithdrawAdmin = true;
        currency.safeTransfer(msg.sender, totalReceive);
        emit Withdraw(msg.sender, totalReceive, projectId);
    }
    ...
    function setting(
        ...
        require(_dataUint.length == 4, "Not enough ");
        ...
        settingProject = true;
        emit Setting(projectId, _update, registerStartTime, calculationTime, rate, address(currency), address(tokenSale), admin);
    }

    function setSlot(uint _slot) external onlyOperator {
        require(block.timestamp > calculationTime, "Before calculation time");
        require(_slot > 0, "Slot greater than 0");
        slot = _slot;
        emit Slot(slot, projectId);
    }

    function submitWinners(address[] memory _users) external onlyOperator {
        require(block.timestamp > calculationTime, "Before calculation time");
        require(winners.length() + _users.length <= slot, "Slot over flow");
        for(uint i = 0; i < _users.length; i++ ){
            address _user = _users[i];
            require(users.contains(_user), "User not register");
            uint _allocation = commit[_user];
            // set claim info
            ClaimInfo storage _data = claimInfo[_users[i]];
            _data.totalToken = _allocation / rate;
            ...
            emit WinnerMember(_user, projectId);
        }
    }
    ...
    function checkUserInPool(address _user) external view returns (bool, bool) {
        return (users.contains(_user), winners.contains(_user));
    }
    ...
    function getProgress() external view returns (uint _numerator, uint _denominator) {
        ...
        _denominator = 1;
        if (rate > 0) {
            _numerator = winners.length() * maxAllocation / rate; //
            _denominator = slot * maxAllocation / rate; //total token receive
        }
    }

    function verifySignature(bytes memory _signature, address _sampleAddress, uint _slot) external view returns (bool) {
        bytes32 _hash = keccak256(abi.encodePacked(projectId, _sampleAddress, address(this), _slot)).toEthSignedMessageHash();
        return _hash.recover(_signature) == admin;
    }

    function getRandomNumberRound(uint round) external view returns(uint) {
        return uint(keccak256(abi.encodePacked(block.number, randomSeed))) / (block.timestamp) *
                uint(keccak256(abi.encodePacked(round, randomSeed))) / uint(keccak256(abi.encodePacked((blockhash(block.number)))));
    }
