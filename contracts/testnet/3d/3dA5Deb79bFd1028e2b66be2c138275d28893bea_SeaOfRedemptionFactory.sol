...
contract SeaOfRedemptionFactory is Ownable {
    ...
    function createSeaOfRedemption(uint256 _rewardRate, address _BUSD) external onlyOwner {
    ...
    function removeSeaOfRedemption(address _seaOfRedemption) external onlyOwner {
        require(
            ...
            "SeaOfRedemptionFactory: sea of redemption not found"
        );
...
contract SeaOfRedemption is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        mapping (uint256 => EnumerableSet.UintSet) tokensPerLottery;
        uint256 rewards;
    ...
    bool private firstLottery = true;
    
    mapping(uint256 => uint256) public amountOfStakedTokensPerLottery;
    mapping(uint256 => EnumerableSet.AddressSet) private stakersPerLottery;
    mapping(uint256 => uint256) private excludedTokenPerLottery;
    ...
    function updatePool(uint256 _tokenId, uint256 _lhs, uint256 _rhs) external onlyLottery {
        if (!firstLottery) {
            nonce++;
        } else {
            firstLottery = false;
        }
        excludedTokenPerLottery[nonce] = _tokenId;
        lhs = _lhs;
        rhs = _rhs;
    }
    ...
    function stake(uint256[] memory _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(
                excludedTokenPerLottery[nonce] != _tokenIds[i],
                "SeaOfRedemption: contains excluded token"
            );
            require(
                _tokenIds[i] >= lhs && _tokenIds[i] < rhs,
                "SeaOfRedemption: contains outdated token or too early staking"
            );
            ILottery(lottery).burn(_tokenIds[i]);
        }
        UserInfo storage user = userInfo[msg.sender];
        if (user.tokensPerLottery[nonce].length() != 0) {
            claim();
        } else {
            user.lastTimeRewardClaimed = block.timestamp;
        }
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            user.tokensPerLottery[nonce].add(_tokenIds[i]);
        }
        if (!stakersPerLottery[nonce].contains(msg.sender)) {
            stakersPerLottery[nonce].add(msg.sender);
        }
        amountOfStakedTokensPerLottery[nonce] += _tokenIds.length;
    ...
    function viewUserInfoPerLottery(
        uint256 _nonce,
        address _user
    )
        external
        ...
        )
    {
        ...
        uint256 amountOfTokens = user.tokensPerLottery[_nonce].length();
        if (amountOfTokens == 0) {
            tokens = new uint256[](0);
        } else {
            tokens = new uint256[](amountOfTokens);
            for (uint256 i = 0; i < amountOfTokens; i++) {
                tokens[i] = user.tokensPerLottery[_nonce].at(i);
            }
        }
    }
    ...
    function claim() public nonReentrant {
        require(
            stakersPerLottery[nonce].contains(msg.sender),
            "SeaOfRedemption: caller is not a staker of current lottery"
        );
        UserInfo storage user = userInfo[msg.sender];
    ...
    function pendingRewards(address _user) public view returns (uint256) {
        ...
        uint256 amount =
        timeBetween
        * userStakedNFTCountPerLottery(nonce, _user)
        * rewardRate
        * PRECISION
        / amountOfStakedTokensPerLottery[nonce];
        return user.rewards + amount / PRECISION;
    }
    
    function userStakedNFTCountPerLottery(
        uint256 _nonce,
        address _user
    )
        public
        ...
        returns (uint256)
    {
        require(
            _nonce <= nonce,
            "SeaOfRedemption: invalid nonce"
        );
        return userInfo[_user].tokensPerLottery[_nonce].length();
    }
    ...
    function safeRewardTransfer(address _to, uint256 _amount) internal returns (uint256) {
        ...
        if (balance > 0) {
            IERC20(BUSD).safeTransfer(_to, balance);
        }
