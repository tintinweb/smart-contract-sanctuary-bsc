...
contract cobyswap is IBSCswapRouter02 {
    using SafeMathBSCswap for uint;
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'COBYSWAPRouter: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'COBYSWAPRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = BSCswapLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'COBYSWAPRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, 'COBYSWAPRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'COBYSWAPRouter: INSUFFICIENT_B_AMOUNT');
    }
    function removeLiquidityBNB(
        address token,
        uint liquidity,
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'COBYSWAPRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'COBYSWAPRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, BSCswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    function swapExactBNBForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WBNB, 'COBYSWAPRouter: INVALID_PATH');
    ...
    function swapTokensForExactBNB(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WBNB, 'COBYSWAPRouter: INVALID_PATH');
        amounts = BSCswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'COBYSWAPRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, BSCswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WBNB, 'COBYSWAPRouter: INVALID_PATH');
        amounts = BSCswapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'COBYSWAPRouter: EXCESSIVE_INPUT_AMOUNT');
        IWBNB(WBNB).deposit{value: amounts[0]}();
        assert(IWBNB(WBNB).transfer(BSCswapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust BNB, if any
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'COBYSWAPRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactBNBForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'COBYSWAPRouter: INSUFFICIENT_OUTPUT_AMOUNT');
