        ...
        ERC20("LFW Join IDO Pool", "LFW-IDO-Join") 
{
    ...
    // Total amount of BUSD has been transfered to SC
     ...
     * @param _amount: max allocation for each person
     */
    function setConfig(
        address _busdToken,
        ...
        require(_joinStartAt > 0, "Invalid time");
    ...
    function join() external {
        ...
        require(
            joinStartAt > 0 && block.timestamp >= joinStartAt,
            'Join pool has not started yet'
        );

        // Setting 
        isJoined[_msgSender()] = true;
        totalJoined += allocationAmount;
        joinedCount += 1;

        // Transfer token from user to fundReceiver
        IERC20(busdToken).transferFrom(_msgSender(), fundReceiver, allocationAmount);
