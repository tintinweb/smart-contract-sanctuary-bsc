...
contract QuickLiquidityProvider is DexBase, IQuickLiquidityProvider, Initializable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address owner_, address babToken_, bool babSwitch_, address vault_) public payable initializer {
        _init(owner_, babToken_, babSwitch_, vault_);
    }

    function _init(address owner_, address babToken_, bool babSwitch_, address vault_) internal onlyInitializing {
        __ReentrancyGuard_init_unchained();
        _init_unchained(owner_, babToken_, babSwitch_, vault_);
    }

    function _init_unchained(address owner_, address babToken_, bool babSwitch_, address vault_) internal onlyInitializing {
    ...
    function _transferInVault(address from, address fromAsset, uint256 fromAmount) internal {
        if (fromAsset == BNB) {
            vault.addVault{value: msg.value}(fromAsset, fromAmount);
        } else {
            IERC20Upgradeable(fromAsset).safeTransferFrom(from, address(this), fromAmount); // user -> dex
            IERC20Upgradeable(fromAsset).safeIncreaseAllowance(address(vault), fromAmount); // dex -> vault approve
            vault.addVault(fromAsset, fromAmount);                                          // dex -> vault
        }
    }

    function _transferOutVault(address to, address toAsset, uint256 toAmount) internal {
        require(vault.query(toAsset) >= toAmount, "QuickLiquidityProvider: vault does not have enough asset for swap. Please go to DexLiquidityProvider");
        if (toAsset == BNB) {
            vault.removeVault(toAsset, toAmount);
            _safeTransferETH(to, toAmount);
        } else {
            vault.removeVault(toAsset, toAmount);
            IERC20Upgradeable(toAsset).safeTransfer(to, toAmount);
        }
    }
