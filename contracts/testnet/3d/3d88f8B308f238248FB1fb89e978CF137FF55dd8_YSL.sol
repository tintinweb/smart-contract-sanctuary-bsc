...
@dev this contract is for YSL token.
     ...
     Admin.ApeswapRouter(); address of APE_SWAP_ROUTER.
     _admin address of Admin contract.
 */
contract YSL is ERC20, AccessControl, Pausable {
    ...
    uint public lockTransactionTime = 10 seconds; 

    //For testing 1 day = 10s; for deployment 1 day = 86400s;
    uint public DAYS_IN_SECONDS = 10;             
     ...
     */
    function mint(address account, uint amount) external _isMinter whenNotPaused {
    ...
    Note only minter role can call 
     */
    function burn(address account, uint amount) external _isMinter whenNotPaused {
    ...
    Note allocation point should be equal to tax
        ...
        only admin can call 
     */
    function setYSLAndAllocationTax(uint tax, uint[] memory allocationTax) external onlyRole(ADMIN_ROLE) {
        require(tax > 0, 'YSL: Tax should be greater than zero');
        require(allocationTax.length > 0, 'YSL: AllocationTax should not be empty');
        uint total;
        for(uint i; i < allocationTax.length; i++) {
            total += allocationTax[i];
        }     
        require(total == tax, 'YSL: Tax not equal to sum of allocation tax');
        YSL_Tax = tax;
     ...
     */
    function setYSLThresholdLimit(uint _YSL_ThresholdLimit) external onlyRole(ADMIN_ROLE) {
        require(_YSL_ThresholdLimit > 0, 'YSL: Threshold limit for YSL can not be zero');
     ...
     */
    function setYSLCompensationAmount(address[] memory userAddresses, uint[] memory amounts) external onlyRole(ADMIN_ROLE) {
        require(userAddresses.length > 0, 'YSL: User addresses length can not be zero');
        require(amounts.length > 0, 'YSL: Amounts length can not be zero');
        require(userAddresses.length == amounts.length, 'YSL: User addresses length must be equals to amounts');
    ...
    function setDaysCompensationLimit(uint _DaysCompensationLimit) external onlyRole(ADMIN_ROLE){
        require(_DaysCompensationLimit!=0,"YSL: DaysCompensationLimit cannot be zero");
     ...
     */
    function claimYSL() external {
        require(compensationUsers[msg.sender] != 0, 'YSL: User has zero YSL');
        uint day = (block.timestamp - deploymentTime)/DAYS_IN_SECONDS;
        require(day <= DaysCompensationsLimit, 'YSL: Compensation period over');
        require(totalSupply() < compensationLimit, 'YSL: Total supply reaches compensation limit');
     ...
     */
    function setCompensationLimit(uint _compensationLimit) external onlyRole(ADMIN_ROLE) {
        require(_compensationLimit > 0, 'YSL: CompensationLimit must be greater than zero');
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        require(restrictTransfer[sender] != block.number,"YSL: you can't interact in same block");
        ...
        if(isContract(sender) || isContract(recipient)){
            ...
            "YSL: No external contract interact with YSL");
        }
        if(sender == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.BUSD()) || recipient == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.BUSD())){
            ...
            if(IWhitelist(Admin.whitelist()).getAddresses(user)){
                ...
                require(transactionTimeLimit[sender] + lockTransactionTime <= block.timestamp,"YSL: transactionTimeLimit is greater than current time");
                uint taxAmount = (amount * YSL_Tax)/10000;
                super._transfer(sender, address(this), taxAmount);
                // console.log("getAMountsOut",IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(balanceOf(address(this)),path)[1]);
                if(IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(balanceOf(address(this)),path)[1] >= YSL_ThresholdLimit){
                    ...
                    IERC20(Admin.BUSD()).transfer(Admin.temporaryHolding(), (convertedBUSD * YSL_Tax_Allocation[0])/YSL_Tax);
                    IERC20(Admin.BUSD()).transfer(Admin.Treasury(), ((convertedBUSD * YSL_Tax_Allocation[1])/YSL_Tax));
                }
                amount -= taxAmount;
            ...
            require(exitRateAmount >= amount, "YSL: Amount is greater than the exitRateAmount");
            ...
            require(prevAmount + ((prevAmount * priceImpactProtection)/1000) >= currentAmount && prevAmount - ((prevAmount * priceImpactProtection)/1000) <= currentAmount,
                "YSL: The difference b/w prev and current is greater than or less than the priceImpactProtection");
            }
     ...
     */
    function setPriceImpactProtection(uint value) public  {
