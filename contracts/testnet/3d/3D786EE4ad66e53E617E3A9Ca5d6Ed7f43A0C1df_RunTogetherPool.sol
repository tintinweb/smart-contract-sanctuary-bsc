...
contract RunTogetherPool is Ownable, ReentrancyGuard {
     ...
     * @param _amount: amount to withdraw (in rewardToken)
     */
    function withdraw(address _userAddress, uint256 _amount) public nonReentrant isRun returns(bool) {
        require(blacklistMap[_userAddress] == false, "You are in Blacklist");
        ...
        uint256 pending = pendingReward(_userAddress);
        ...
        if (_amount > 0) {
            ...
            stakedToken.safeTransfer(_userAddress, amountWithdraw);
        }

        if (pending > 0) {
            rewardToken.safeTransfer(_userAddress, pending);
            user.stakeBlock = block.number;
            emit Reward(_userAddress, pending);
        }
