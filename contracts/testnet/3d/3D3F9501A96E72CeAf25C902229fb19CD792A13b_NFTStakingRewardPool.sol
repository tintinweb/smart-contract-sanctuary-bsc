...
contract NFTStakingRewardPool is Ownable {
        ...
        onlyOwner
    {
     ...
     // Setup minter
    function setupMinter(address user) public onlyOwner {
        _minter = user;
    }
    ...
    // Setup minimum claim
    function setupMinimumClaim(uint256 amount) public onlyOwner {
        MINIMUM_CLAIM = amount;
    }

    // Get user paid fee 
    function getUserPaidFee(address user) public view returns (bool) {
        return userPaidFee[user];
    }

    // Pay fee
    function payFee() external payable {
        require(msg.value >= FEE, "Failed to send fee");
        (bool success, ) = _minter.call{value: FEE}("");
    ...
    function claimRewardFromPool(address claimAddress, uint256 _amount) public onlyOwner {
        require(userPaidFee[claimAddress], "User has not pay fee!");
        require(_amount >= MINIMUM_CLAIM * 1e18);
        IERC20(ecioContractAddress).transfer(claimAddress, _amount);

        userPaidFee[claimAddress] = false;
    }
