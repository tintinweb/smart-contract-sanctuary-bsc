...
contract MowaTokenAirdrop is Ownable, ReentrancyGuard {
    ...
    function setTimeClaim(uint256 _startClaim, uint256 _endClaim) public onlyOwner {
        startClaim = _startClaim;
        endClaim = _endClaim;
    }

    function addWhiteList(address user, uint256 amount) public onlyDev {
        amount = amount.mul(1e18);
        whiteList[user] = amount;
        claims[user].timeClaim = 0;
        claims[user].amount = 0;
        claims[user].claim = false;
    }

    // 1 not white list, 2 has claim
    function checkBeforeClaim(address _user) public view returns (uint256 _cl) {
    ...
    function claimToken() public payable nonReentrant {
        uint checkClaim = checkBeforeClaim(_msgSender());
        require(checkClaim == 5, "Wallet address is not eligible to claim tokens");
        ...
        claims[_msgSender()].timeClaim = block.timestamp;
        claims[_msgSender()].amount = amount;
        claims[_msgSender()].claim = true;
    }
