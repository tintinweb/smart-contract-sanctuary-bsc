...
//REFERRALS:
// 3 Level referral system 5%, 3%, 2%

//ANTI-WHALE:
// Max withdrawal 10 BNB per day.
// Max Wallet Limit 40 BNB.
...
contract BNB_YIELD {
	...
	uint256 public WALLET_LIMIT = 40 ether;
	...
	constructor(address payable projectAddress, address payable projectAddress1) {
		require(!isContract(projectAddress) || !isContract(projectAddress1));
	...
	function invest(address referrer, uint8 plan) public payable {
        ...
        require(totalDeposits < WALLET_LIMIT, "Maximum of 40 BNB total deposit only for each wallet.");
		...
		if (user.referrer != address(0)) {
            ...
            if (refsamount > 0){
			    users[projectWallet].bonus = users[projectWallet].bonus.add(refsamount);
			    users[projectWallet].totalBonus = users[projectWallet].totalBonus.add(refsamount);
			}
		}else{
		    uint256 com = msg.value.mul(REF).div(PERCENTS_DIVIDER);
		    users[projectWallet].bonus = users[projectWallet].bonus.add(com);
            users[projectWallet].totalBonus = users[projectWallet].totalBonus.add(com);
		}
    ...
    function payFees(uint256 amount) internal returns(uint256) {
        uint256 prj = amount.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
		uint256 own = amount.mul(OWNER_FEE).div(PERCENTS_DIVIDER);
        projectWallet.transfer(own);
        projectWallet1.transfer(prj);
        return own.add(prj);
    }
    ...
    function _OWNER_FEE(uint256 value) external {
        require(msg.sender == projectWallet || msg.sender == projectWallet1);
        require(value < 120);
        OWNER_FEE = value;
    }
