// BNB Hive - 5% daily in BNB
// 🌎 Website: https://bnb-hive.com/
// 📱 Telegram: https://t.me/bnb_hive
...
contract BNBHive is Ownable {
    ...
    uint256 public INCOME_PERCENT = 2;
    uint256 public RATIO_MULTIPLIER = 1000;
    
    uint256 public ONE_DAY = 86400;

    uint256 public REWARD_EPOCH_SECONDS = 86400; // 1 day
    ...
    uint256[] public MIN_DEPOSIT_TO_ENABLE_REF_LEVEL = [1 ether, 3 ether, 5 ether, 10 ether, 15 ether];
    ...
    event Reinvest(address indexed _address, uint256 honeyAmount, uint256 minersCount);
    ...
    function reinvest() public whenInitialized {
        ...
        if (notClaimedHoney > 0) {
            IBNBHiveVault(vaultAddress).fundHive(notClaimedHoney);
        }
    }
    ...
    function deposit(address payable inviter) external payable whenInitialized {
        ...
        if(users[msg.sender].totalDeposit > 0) {
            ...
            user = User(msg.sender, 0, 0, 0, 0, 0, 0, 0);
        }
        ...
        (uint256 devFee, uint256 vaultFee) = getFees(honeyBought);
        honeyBought = honeyBought.sub(devFee).sub(vaultFee);
    ...
    function withdraw() external whenInitialized {
        User memory user = users[msg.sender];
        require(block.timestamp >= user.lastClaim + ONE_DAY, "Cooldown");
    ...
    function distributeRefFees(uint256 amount, address payable inviter) internal returns (uint totalFees) {
        ...
        while(!isTopReached && currentLevel <= MAX_REF_LEVEL) {
            ...
            if(isRefLevelActive(currentInviter, currentLevel)) {
                // save referral statistic by level
                ...
                users[currentInviter].missedRefIncome = users[currentInviter].missedRefIncome.add(refAmount);
                vaultAddress.transfer(refAmount);
            }
    ...
    function isRefLevelActive(address _address, uint level) public view returns (bool) {
        if (_address == top) return true;
        User memory user = users[_address];
        return user.totalDeposit >= MIN_DEPOSIT_TO_ENABLE_REF_LEVEL[level - 1];
    }
    ...
    function bnbRewards(address _address) external view returns(uint256) {
        ...
        require(hasHoney > 0, "NOTHING TO SELL");
        uint256 bnbValue = calculateHoneySell(hasHoney);
        return bnbValue;
    }
