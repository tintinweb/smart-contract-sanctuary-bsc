...
contract GGStaking is Ownable {
    ...
    uint256 private _unusedRewardPot;
    ...
    event WithdrawUnusedRewardPot(uint256 _unusedRewardPot);
    ...
    function withdrawUnusedRewardPot() external onlyOwner {
        ...
        egToken.transfer(owner(), _unusedRewardPot);
        _unusedRewardPot = 0;
        emit WithdrawUnusedRewardPot(_unusedRewardPot);
    }
    ...
    function depositReward(uint256 _amount) external onlyOwner {
        ...
        if (totalLegendaryStakers > 0) {
            ...
            _unusedRewardPot = _unusedRewardPot + legendaryRewards;
        }
        if (totalAllSquadHolders > 0) {
            ...
            _unusedRewardPot = _unusedRewardPot + allSquadRewards;
        }
        if (totalCommonNFTsStaked > 0) {
            ...
            _unusedRewardPot = _unusedRewardPot + commonNFTRewards;
        }
        totalDepositCount++;
        emit DepositReward(msg.sender, _amount);
    }
    function userStakedNFTs(address _user) external view onlyOwner returns (uint256[] memory) {
    ...
    function unusedRewardPot() external view onlyOwner returns (uint256) {
        return _unusedRewardPot;
    }
