...
interface IWhitelistNFT {
    ...
    function consumePriority(uint256 tokenId) external;
}
...
contract PreSale is Validatable {
    ...
    /**
     *  @notice maximum is limit NFT at this round
     */
    ...
    /**
     *  @notice whitelistNFT is address of whitelist NFT
     */
    IWhitelistNFT public whitelistNFT;
    ...
    event SetWhitelistNFT(IWhitelistNFT indexed oldValue, IWhitelistNFT indexed newValue);
     ...
     * @param _whitelistNFT Address of whitelistNFT contract
     * @param _paymentToken Address of payment token
     * @param _maximum Maximum of NFT can be sold
   ...
   /**
    ...
    /**
     * @notice Set whiteList NFT address
     * @dev    Only admin can call this function
     * @param _whitelistNFT Address of whitelistNFT
     * 
     * emit {SetGenesisNFT} events
     */
    function setWhitelistNFT(address _whitelistNFT) external onlyAdmin notZeroAddress(_whitelistNFT) {
        IWhitelistNFT oldValue = whitelistNFT;
        whitelistNFT = IWhitelistNFT(_whitelistNFT);
        emit SetWhitelistNFT(oldValue, whitelistNFT);
    }
   ...
   /**
    ...
    function buy(uint256 times) external payable notZero(times) {
        require(whitelistNFT.balanceOf(_msgSender()) > 0, "Require own whitelist NFT");
        require(startTime <= block.timestamp && block.timestamp <= endTime, "Can not buy at this time");
        require(_counter.current() < maximum, "Sold out");
