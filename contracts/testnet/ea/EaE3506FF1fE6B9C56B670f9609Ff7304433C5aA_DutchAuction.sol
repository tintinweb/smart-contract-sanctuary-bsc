    ...
    IERC721ReceiverUpgradeable
{
    ...
    struct AuctionDecreasePeriod {        
        address nftContractAddress;
        uint256 tokenId;
        bool active;
        uint256 decreasePeriod;
        uint256 price;
        uint256 decreaseControl;
    }
    ...
    mapping(uint256 => AuctionDecreasePeriod) public mapAuctionDecreasePeriod;
    mapping(address => mapping(uint256 => AuctionDecreasePeriod)) public mapAuctionPeriod;
    ...
    uint32 public defaultTimeToReducePrice;
    uint32 public defaultDecreaseControl;
    ...
    event ChangeddefaultminPricePercentatge(uint32 defaultminPricePercentatge);
    event ChangedefaultDecreaseControl(uint32 defaultDecreaseControl);
    ...
    modifier miniumPriceMeetRequirements(
        ...
        uint256 pricePercetatge = (startPrice * (defaultminPricePercentatge)) / 10000;
        require((startPrice - pricePercetatge) >= minPrice &&
        minPrice > 0, "Set minium Price in Range");
        _;
    }
    ...
    function initialize(
        ...
        uint32 defaultDecreaseControl_,
        ...
        isPercentageInBounds(defaultDecreaseRate_)
        isPercentageInBounds(firstOwnerFeePercentage_)
        isPercentageInBounds(defaultminPricePercentatge_)
        isPercentageInBounds(defaultDecreaseControl_)
    {
        __Ownable_init();
        ...
        defaultDecreaseControl = defaultDecreaseControl_;
        initialized = true;
    }
    ...
    /// Change default Decrease Control
    /// @param newdefaultDecreaseControl New default Decrease Control
    function changeDefaultDecreaseControl(uint32 newdefaultDecreaseControl)
    external
        onlyOwner
        isPercentageInBounds(newdefaultDecreaseControl)
    {
        defaultDecreaseControl = newdefaultDecreaseControl;
        emit ChangedefaultDecreaseControl(defaultDecreaseControl);
    }
    ...
    function changeAuctionTimeMap(
        ...
        AuctionTime storage auctionTime = mapAuctionTime[endTime];
        auctionTime.active = active;
        auctionTime.price = price;
    }

    /// Change or add timmings for auction
    /// @param decreasePeriod Time Period allowed to decrease to end the Auction
    /// @param active Status of the auction
    /// @param price Fee price for aditional period
    function changeAuctionDecreasePeriod(
        uint256 decreasePeriod,
        bool active,
        uint256 price
    ) external onlyOwner {
        AuctionDecreasePeriod storage auctionDecreasePeriod = mapAuctionDecreasePeriod[decreasePeriod];
        auctionDecreasePeriod.decreasePeriod = decreasePeriod;
        auctionDecreasePeriod.active = active;
        auctionDecreasePeriod.price = price;
    }
    ...
    /// @param nftContractAddress Contract address that will be put on auction
    /// @param tokenId Identifier of the NFT
    function getminPrice(address nftContractAddress, uint256 tokenId) external view returns(uint){
    ...
    /// @param nftContractAddress Contract address that will be put on auction
    /// @param tokenId Identifier of the NFT
    function getDueAuctionTime(address nftContractAddress, uint256 tokenId) external view returns(uint){
    ...
    function getCurrentPrice(address nftContractAddress, uint256 tokenId) public view returns(uint256){
        ...
        AuctionTimming memory nftContractAuctionTimmings = nftContractAuctionTimming
        [nftContractAddress][tokenId];
        AuctionDecreasePeriod memory auctionPeriod = mapAuctionPeriod[
            nftContractAddress][tokenId];
        if (_getAuctionState(nftContractAddress, tokenId) == AuctionState.OPEN){
            if ((block.timestamp - nftContractAuction.auctionStart) > auctionPeriod.decreasePeriod){
                ...
                getDecreasePrice(nftContractAuction.startPrice, 
                                ...
                                auctionPeriod.decreasePeriod
                );
                if ((nftContractAuction.startPrice - amountToDecrease) > nftContractAuction.minPrice){
    ...
    function getBCurrentPrice(address nftContractAddress, uint256 tokenId) public view returns(uint256){
        Auction memory nftContractAuction = nftContractAuctions
        [nftContractAddress][tokenId];
        AuctionDecreasePeriod memory auctionPeriod = mapAuctionPeriod[
            nftContractAddress][tokenId];
        if (_getAuctionState(nftContractAddress, tokenId) == AuctionState.OPEN){
            if ((block.timestamp - nftContractAuction.auctionStart) > auctionPeriod.decreasePeriod){
                uint256 amountToDecrease = 
                getBDecreasePrice( 
                    nftContractAuction.startPrice,
                    nftContractAuction.auctionStart,
                    auctionPeriod.decreasePeriod,
                    auctionPeriod.decreaseControl
                );
                if ((nftContractAuction.startPrice - amountToDecrease) > nftContractAuction.minPrice){
                    return nftContractAuction.startPrice - amountToDecrease;
                }else{
...
/*     function getDecreasePrice(address nftContractAddress, uint256 tokenId) public view returns(uint256){
        Auction memory nftContractAuction = nftContractAuctions
        [nftContractAddress][tokenId];
        AuctionTimming memory nftContractAuctionTimmings = nftContractAuctionTimming
        [nftContractAddress][tokenId];
        AuctionDecreasePeriod memory auctionPeriod = mapAuctionPeriod[
            nftContractAddress][tokenId];
        uint256 decreasesteps = (nftContractAuctionTimmings.deadlineTime - nftContractAuction.auctionStart)/auctionPeriod.decreasePeriod; ////172800/600 = 288
        uint256 stepsToDecrease = (nftContractAuction.startPrice - nftContractAuction.minPrice) / (decreasesteps - 1); //10000/287 = 34,84
        uint256 timer = (block.timestamp - nftContractAuction.auctionStart)/ auctionPeriod.decreasePeriod; // 172800/600 = 288
    ...
    function _setupAuction(
        ...
        AuctionDecreasePeriod storage auctionPeriod = mapAuctionPeriod[
            nftContractAddress][tokenId];
        ...
        auctionPeriod.decreasePeriod = decreasePeriod;
        auctionPeriod.nftContractAddress = nftContractAddress;
        auctionPeriod.tokenId = tokenId;
        IERC721Upgradeable(nftContractAddress).safeTransferFrom(
            msg.sender,
            address(this),
            tokenId
    ...
    {
        AuctionTime memory auctionTime = mapAuctionTime[endTime];
        AuctionDecreasePeriod memory auctionDecreasePeriod = mapAuctionDecreasePeriod[decreasePeriod];
        require(auctionTime.active, "Not a valid bid period");
        require(auctionDecreasePeriod.active, "Not a valid decrease period");
        _auctionTimePayment(auctionTime.price);
        _auctionTimePayment(auctionDecreasePeriod.price);
    }
        ...
        internal
    {
        ...
        AuctionTimming storage nftContractAuctionTimmings = nftContractAuctionTimming[
            nftContractAddress
        ][tokenId];
        AuctionDecreasePeriod storage auctionPeriod = mapAuctionPeriod[
            nftContractAddress][tokenId];
        ...
        nftContractAuctionTimmings.deadlineTime = 0;
        auctionPeriod.decreasePeriod = 0;
        auctionPeriod.nftContractAddress = address(0);
        auctionPeriod.tokenId = 0;
    }
...
abstract contract PriceDecrease is Initializable, OwnableUpgradeable {
    ...
    public view returns(uint256){
        uint256 timer = (block.timestamp - auctionStart)/60/60/24; // 172800/60/60/24 = 2880
        return (timer * startPrice)/decreaseControl; // 2880*10000 = 28800000
    }
