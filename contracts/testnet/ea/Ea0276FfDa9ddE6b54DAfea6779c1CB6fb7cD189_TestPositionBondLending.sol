...
contract TestPositionBondLending is PositionBondLending {
    ...
    constructor(
        ...
        address issuer_
    ) PositionBondLending(bondInformation, assetInformation, issuer_) {
        mockTime = 0;
    }
    ...
    ReentrancyGuard
{
    ...
    function setPositionBondFunction(address positionBondFunction_) external {
        require(address(positionBondFunction) == address(0), "already set");
        ...
        if (isLiquidate()) {
            if (UnderlyingAssetLending.getUnderlyingAssetType() == 1) {
                address posiNftFactory = address(
                    positionBondFunction._posiNFTFactory()
                );
...
contract PositionBondFunction is
    ...
    OwnableUpgradeable
{
    IPosiNFTFactory public _posiNFTFactory =
        IPosiNFTFactory(0x9D95b5eA6C8f678B7486Be7a6331ec10A54156BD);


    IERC721 public _posiNFT = IERC721(0xecA16dF8D11d3a160Ff7A835A8DD91e0AE296489);
    ...
    // TODO change modifier to only owner
    function setWhileListAsset(address asset, bool isWhileList) public {
    ...
    function verifyRequire(
        ...
        if (_getLengthString(bondInformation.bondName) > 30 || _getLengthString(bondInformation.bondName) == 0)
            return false;
        if (
        ...
        if ( !whileListAsset[assetInformation.underlyingAsset] || !whileListAsset[assetInformation.faceAsset])
            return false;

//        if (bondInformation.startSale < block.timestamp + 1 hours) return false;

        if ( bondInformation.issuePrice == 0 || assetInformation.faceValue == 0 || bondInformation.totalSupply == 0 || bondInformation.duration < 30) return false;

        if (assetInformation.underlyingAssetType == 0) {
            return
                verifyRequireToken(assetInformation.collateralAmount, collateralAmountTransfer);
        }
        if (assetInformation.underlyingAssetType == 1) {
            return verifyRequireNFT(assetInformation.nftIds, assetInformation.collateralAmount);
        }
