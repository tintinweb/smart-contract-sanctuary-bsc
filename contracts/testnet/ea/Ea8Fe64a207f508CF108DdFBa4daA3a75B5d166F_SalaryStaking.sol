...
contract SalaryStaking is Ownable, ReentrancyGuard {
    ...
    constructor(IERC20 _usdtReward,IERC20 _salary,address _feeCollector,uint256 _depositFeePercent) {
        ...
        rewardTokens.push(_usdtReward);
        rewardTokens.push(_salary);
    ...
    function deposit(uint256 _amount) external nonReentrant {
       ...
       require(_amount>=MAX_DEPOSIT || user.amount>=200,"Stake must be at least 200 SLR.");
       ...
       for (uint256 i; i < _len; i++) {
        ...
        TotalStaked = TotalStaked.add(_amountMinusFee);
   ...
   function Restake() external nonReentrant{
        ...
        TotalStaked = TotalStaked.add(_salaryPending);
        for (uint256 i; i < _len; i++) {
            IERC20 _token = rewardTokens[i];
            updateReward(_token);
        }
   ...
   function pendingReward(address _user, IERC20 _token) public view returns (uint256) {
       ...
       function withdraw() external nonReentrant {
        ...
        TotalStaked = TotalStaked.sub(_previousAmount);
    ...
    function addRewardsSalaryByNumberDay(uint _amount , uint _numberOfDay) public onlyOwner {
     ...
     function getNumberDaysCanShared() public view returns(uint) {
   ...
   function updateReward(IERC20 _token) public {
        ...
        if(numberOfSalaryReward() > 0 && reawrdsInfo[_token].numberOfDay>0 && _token == salary){
            ...
            reawrdsInfo[salary].numberOfDayDistribut = reawrdsInfo[salary].numberOfDayDistribut.add(getNumberDaysCanShared());
            reawrdsInfo[salary].lastDistribution=block.timestamp;
        }
        uint256 _totalSLR = TotalStaked;
        uint256 _currRewardBalance = _token.balanceOf(address(this));
    ...
    function safeTokenTransfer(IERC20 _token,address _to, uint256 _amount) internal {
        uint256 _currRewardBalance = _token.balanceOf(address(this));
        uint256 _rewardBalance = _token == salary ? _currRewardBalance.sub(TotalStaked) : _currRewardBalance;
