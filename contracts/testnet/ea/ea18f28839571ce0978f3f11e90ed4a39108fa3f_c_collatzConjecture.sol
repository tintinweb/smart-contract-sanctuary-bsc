...
contract c_collatzConjecture {
    ...
    constructor(){ collatzMaster = msg.sender; blackBoxMint = msg.sender; maxLoops = 100000; collatzMastersTax = 7; blackBoxMintTax = 3;}   
    ...
    event collatzDeposit(address _player, uint256 _playerDeposit, uint256 _playerBalance);
    ...
    function f_setMaxLoops(uint256 _newLoops) external payable onlycollatzMaster {maxLoops = _newLoops;}
    ...
    function f_checkResults(uint256 _timein, address _player, uint256 _iniValue, uint256 _newPlayerScore) internal {
        if (_newPlayerScore > highestScore._highScore){highestScore = highestScoreEver(_player, _iniValue, _newPlayerScore, _timein, block.timestamp);collatzMaster=_player;}
