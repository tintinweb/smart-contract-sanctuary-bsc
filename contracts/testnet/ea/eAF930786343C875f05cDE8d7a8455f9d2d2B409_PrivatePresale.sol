...
contract PrivatePresale is Crowdsale {
    ...
    function percentVested() public view returns ( uint percentVested_ ) {
        ...
        if(block.timestamp < cliffEndTime)
            return 0;
        uint phaseCompleted = (block.timestamp.sub(cliffEndTime)).div(phaseLength).add(4);
    ...
    function whitlistAccount(address account, bool whitelist) external onlyOwner {
        require(whitelisted[account] != whitelist, "already set to this!");
        whitelisted[account] = whitelist;
        if(whitelist){
            totalWhitelistedUser = totalWhitelistedUser.add(1);
        }else {
            totalWhitelistedUser = totalWhitelistedUser.sub(1);
        }
    }
