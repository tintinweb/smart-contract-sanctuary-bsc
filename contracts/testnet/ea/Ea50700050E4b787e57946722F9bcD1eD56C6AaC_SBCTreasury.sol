...
contract SBCTreasury is ERC721Holder, AccessControlEnumerable {
    ...
    uint256 public acc;
    address public sbc = 0x87b903535c983Bd9f1f1fD5d64c55e02A5c4b18e;
    address public nft = 0xdbde4CB32e7138EB1B2F4EC53488C2d390f765a6;
    ...
    mapping(address => EnumerableSet.UintSet) private depositRecords; //用户质押记录
    mapping(address => mapping(uint256 => uint32)) public userMedalCount;

    uint8 public constant PAGE_SIZE = 2;
    ...
    function deposit(uint256[] calldata tokenIds) external {
        require(tokenIds.length <= 9, "Exceeded quantity");
        (uint256[] memory medals, uint256[] memory weights) = INFT(nft).getByTokenIdList(tokenIds);
        uint256 depositEffect = 0;
        for(uint256 i = 0; i < tokenIds.length; i++) {
            address owner = ERC721(nft).ownerOf(tokenIds[i]);
            if(msg.sender == owner) {
                depositEffect += weights[i];
                depositTokenIds[msg.sender][tokenIds[i]] = true;
                depositRecords[msg.sender].add(tokenIds[i]);
                userMedalCount[msg.sender][medals[i]] += 1;
    ...
    function bonusRescue(address account,uint256 amount) external {
        ...
        IERC20(sbc).transferFrom(account, address(this), amount);
    ...
    function harvest() external {
        ...
        if(totalBonusPending > 0) {
            IERC20(sbc).safeTransfer(msg.sender, totalBonusPending);
        }
    }
    ...
    function withdraw(uint256[] calldata tokenIds) external {
        ...
        (uint256 withdrawEffect, uint256[] memory medals) = calcWeight(tokenIds);
    ...
    function queryNftByPage(address addr, uint256 offset, uint256 size) external view returns(uint256[] memory tokenIds) {
        uint256 totalNftAmount = depositRecords[addr].length();
        require(offset + size <= totalNftAmount, "Size out of bound.");
    ...
    function queryTotalNftCount(address addr) external view returns(uint256) {
