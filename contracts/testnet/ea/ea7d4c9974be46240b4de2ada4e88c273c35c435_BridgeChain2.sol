...
// File: contracts\interfaces\GolffV1ERC20.sol
interface GolffV1ERC20 {
    function mint(address to, uint256 amount) external returns (bool);
    function burn(address from, uint256 amount) external returns (bool);
}

// File: contracts\BridgeChain2.sol
contract BridgeChain2 is Ownable, Pausable {
    ...
    function bridgeIn(address _receiver, address _token, uint256 _toChain, uint256 _amount) external payable whenNotPaused checkBridgeIn(_receiver, _token, _toChain, _amount) {
        ...
        // 销毁
        GolffV1ERC20(_token).burn(msg.sender, _amount);
        // 记录费用
