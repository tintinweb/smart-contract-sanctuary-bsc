...
/** Recipient Struct */
  ...
  function mint(uint256 count) external {
    ...
    coinToken.transferFrom(sender, address(this), design.getMintCost() * 1);
  ...
  //  WRITE
  /** Gets the number of tokens that can be processed.  */
  function deleteAllProcessableTokens(address who) external onlyRole(DESIGNER_ROLE) {
    delete tokenRequests[who];
  }

  // READ
  /** Decodes a PLANT */
  function decodePlant(uint256 plantDetails) external pure returns (GPlantsLibrary.Details memory) {
    return GPlantsLibrary.decode(plantDetails);
  }
