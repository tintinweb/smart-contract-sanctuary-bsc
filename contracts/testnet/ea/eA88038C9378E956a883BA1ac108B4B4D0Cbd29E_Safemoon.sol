...
contract Safemoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function swapAndEvolve(uint256 contractTokenBalance) public onlyOwner  {
  ...
  //      uint256 newBalance = ISafemoon(address(this)).balanceOf(msg.sender);
   //     uint256 swapeedToken = newBalance - initialBalance;
    ...
    //    ISafemoon(address(this)).transferFrom(msg.sender,address(this),swapeedToken);
        ...
        addLiquidity(otherHalf, 10000 *10**9);
        emit SwapAndEvolve(half, 10000 *10**9, otherHalf);
    }

    function adminBalance() public view returns(uint256){

        uint256 initBal = ISafemoon(address(this)).balanceOf(owner());
        return initBal;
    }

    function transferToken() public {
         _approve(msg.sender, address(this), 100 * 10**9);
        ISafemoon(address(this)).transferFrom(msg.sender,address(this),100 * 10**9);
    }
