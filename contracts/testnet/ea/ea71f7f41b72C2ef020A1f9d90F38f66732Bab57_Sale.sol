	...
	ReentrancyGuardUpgradeable
{
	...
	event PurchaseWithStableToken(
		address _buyer,
		uint256 _id,
		uint256 _amount,
		address _tokenStable
	);
	event WithdrawStableToken(
		address _recipient,
		uint256 _amount,
	...
	///@notice sell tokens and pay with others tokens
	function buyTokenWithStableToken(
		uint256 _tokenAmount,
		address _stableTokenAddress
	) external isPublicSale nonReentrant {
		uint256 finalPrice;
		require(
			_stableTokenAddress == usdtAddress ||
				_stableTokenAddress == usdcAddress ||
				_stableTokenAddress == busdAddress ||
				_stableTokenAddress == daiAddress,
			"This token is not a stableToken"
		);
		require(
		...
		finalPrice = _tokenAmount.mul(phases[currentPhase].price).div(10**18);
		require(
			ERC20(_stableTokenAddress).transferFrom(
				msg.sender,
				address(this),
				finalPrice
			),
			"Failed transfer stable token"
		);
		id++;
	...
	/// @notice sell tokens, require send ETH/BNB
	...
	function withdrawStableToken(
		...
		require(
			ERC20(_stableTokenAddress).transfer(_account, _amount),
			"Failed busd transfer"
		);
		emit WithdrawStableToken(_account, _amount, _stableTokenAddress);
	}
