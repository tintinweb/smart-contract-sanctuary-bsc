...
contract TresLechesSpecialPool is Ownable {
    ...
    function stake(uint256 _quantity) public {
        ...
        require(isWhitelist[_msgSender()], "TresLeches: Address Not whitelisted");
    ...
    function unStake(uint256 _amount) public {
        ...
        UserData storage _userData = userData[_msgSender()];
        require(_userData.tokenQuantity >= _amount, "TresLeches: Insufficient amount");
        
        uint256 pendingReward = calculateRewards(userData[_msgSender()].user);
        
        if(_amount == 0) {
            require(_userData.tokenQuantity > 0, "TresLeches: NO REWARD YET.");
        ...
        if(_amount > 0) {
            ...
            require( _amount <= _userData.tokenQuantity, "TresLeches: AMOUNT IS GREATER THAN USER STAKED TOKEN");
