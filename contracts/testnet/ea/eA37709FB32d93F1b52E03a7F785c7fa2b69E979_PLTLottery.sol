...
contract PLTLottery is ReentrancyGuard, IPLTLottery, Ownable, ERC1155Holder {
    ...
    event NewMlmManager(address indexed mlmManager);
    event NewPltNFT(address indexed pltNFT);
    event TicketsPurchase(address indexed buyer, uint256 indexed lotteryId, uint256 numberTickets, uint256 amountPLTToTransfer, uint256 amountNftToTransfer, string affiliateId, uint256 _refId);
     ...
     * @param _pltNFTaddress: The address of the NFT
     * @param _mlmManagerAddress: The address of the MLM manager
     */
    constructor(address _pltTokenAddress, address _randomGeneratorAddress, address _pltNFTaddress, address _mlmManagerAddress) {
        pltToken = IERC20(_pltTokenAddress);
        randomGenerator = IPLTRandomNumberGenerator(_randomGeneratorAddress);
        pltNFT = IERC1155(_pltNFTaddress);
        pltMlmManager = IPLTmlm(_mlmManagerAddress);
    ...
    function buyTickets(uint256 _lotteryId, uint32[] calldata _ticketNumbers, bool _useFreeTickets, string memory _affiliateId, uint256 _refId)
        external
        override
        notContract
        nonReentrant {
        ...
        pltMlmManager.distributeRevenue(amountPLTToTransfer, _refId);

        emit TicketsPurchase(msg.sender, _lotteryId, _ticketNumbers.length, amountPLTToTransfer, amountNftToTransfer, _affiliateId, _refId);
    }
    ...
    function changeMlmManager(address _mlmManagerAddress) external onlyOwner {
        require(_mlmManagerAddress != address(0), "Address can not be zero");
        pltMlmManager = IPLTmlm(_mlmManagerAddress);
        emit NewMlmManager(_mlmManagerAddress);
    }
