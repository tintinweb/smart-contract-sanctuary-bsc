...
contract Staking is SetParams {
    ...
    mapping(address => mapping(uint256 => bool)) public withdrawRequestList;
    ...
    uint256 internal totalRewardsAdded;
    ...
    event Approve(address from, address to, uint256 tokenId);
    ...
    constructor(address _tokenAddrUSDC, address _tokenAddrBRBC) {
        ...
        startTimeWhitelisted = uint32(block.timestamp);
        endTime = startTime + 500 days;
        endWhitelistTime = startTime + 1 days;
        // set up pool size
        minUSDCAmount = 100 * 10**decimals;
        maxUSDCAmount = 100000 * 10**decimals;
        maxPoolUSDC = 2930000 * 10**decimals;
        maxPoolBRBC = 2930000 * 10**decimals;
        USDC = IERC20(_tokenAddrUSDC);
        BRBC = IERC20(_tokenAddrBRBC);
        penaltyReciver = msg.sender;
        tokensLP.push(TokenLP(0, 0, 0, 0, 0, false, 0));
    }
    ...
    /// @param _tokenId token id of a token
    modifier ApprovedOf(uint256 _tokenId) {
        require(
    ...
    modifier ArrayLengthEquals(
        ...
        require(_array1.length == _array2.length, "Arrays length mismath");
        _;
    }
    ...
    modifier OnlyWhitelisted() {
        require(
            whitelist.contains(msg.sender) == true,
            "You are not in whitelist"
        );
        _;
    }

    function findRateForTime(uint32 _period)
        internal
        pure
        returns (uint32 rateForTime)
    {
        if (_period < uint32(90 days)) {
            return uint32(30 days);
        } else if (_period >= 90 days && _period < 180 days) {
            return uint32(90 days);
        } else {
            return uint32(180 days);
        }
    }
    ...
    function whitelistStake(uint256 _amountUSDC, uint32 _period)
        external
        MaxStakeAmount(msg.sender, _amountUSDC, 1000 * decimals)
        OnlyWhitelisted
        returns (bool StakeSuccess)
    {
        require(
            block.timestamp >= startTimeWhitelisted,
            "Staking period hasn't started"
        );
        require(
            block.timestamp <= startTimeWhitelisted + 1 days,
        ...
        bool _mintedTokens = _mint(
            _amountUSDC,
            (_amountUSDC * rate[findRateForTime(_period)]) / 100,
            _period
        );
        require(_mintedTokens == true, "failed to mint");
        return true;
    }
    ...
    /// TODO: can we withdraw USDC by mistake?
    function withdrawToOwner(
        ...
        if (_tokenAddresses.length > 0) {
            for (uint256 i = 0; i < _tokenAddresses.length; i++) {
                ...
                IERC20(_tokenAddresses[i]).transferFrom(
                    address(this),
                    _to,
                    _tokenAmounts[i]
                );
    ...
    /// returns true if success
    function _burn(uint256 _tokenId) internal returns (bool burntToken) {
    ...
    function _transfer(
        ...
        address _to,
        uint256 _tokenId
    ) private IsInStake(_tokenId) returns (bool internalTransferSuccess) {
    ...
    {
        bool transferedTokens = _transfer(msg.sender, _to, _tokenId);
        require(transferedTokens == true, "failed to transfer tokens");
        return true;
    }
    ...
    function transferFrom(
        ...
        TransferCheck(_to)
        ApprovedOf(_tokenId)
        returns (bool transferFromSuccess)
    {
        _transfer(_from, _to, _tokenId);
        bool transferedTokensFrom = _transfer(msg.sender, _to, _tokenId);
        require(transferedTokensFrom == true, "failed to transfer from tokens");
        return true;
    }
    ...
    function claimRewards(uint256 _tokenId)
        ...
        returns (bool claimRewardsSuccess)
    {
    ...
    /// Added param rewardAmount for reentrancy protection
    /// returns true if success
    function fromClaimRewards(uint256 _tokenId)
        external
        IsInStake(_tokenId)
        ApprovedOf(_tokenId)
        PositiveRewards(_tokenId)
        returns (bool fromClaimRewardsSuccess)
    {
        uint256 rewardAmount = viewRewards(_tokenId);
        tokensLP[_tokenId].lastRewardGrowth = rewardGrowth;
        USDC.transfer(msg.sender, rewardAmount);
        emit FromClaimRewards(
            address(this),
            msg.sender,
            _tokenId,
            rewardAmount
    ...
    {
        require(
            withdrawRequestList[msg.sender][_tokenId] == false,
            "You request is already sent"
        );
        withdrawRequestList[msg.sender][_tokenId] = true;
        requestArray.add(msg.sender);

        if (viewRewards(_tokenId) > 0) {
            bool claimedRewards = claimRewards(_tokenId);
            require(claimedRewards == true, "failed to claim rewards");
        }
        tokensLP[_tokenId].isStaked = false;

        if (tokensLP[_tokenId].deadline > uint32(block.timestamp + 1 days)) {
            bool penaltySuccess = penalizeAddress(_tokenId);
            require(penaltySuccess == true, "failed to penalize");
        }
    ...
    /// @param _tokenId the token id
    /// returns true if success
    function penalizeAddress(uint256 _tokenId)
        internal
        returns (bool penalized)
    {
        poolBRBC -= (tokensLP[_tokenId].BRBCAmount * penalty) / 100;
        uint256 beforePenaltyBRBC = tokensLP[_tokenId].BRBCAmount;
        tokensLP[_tokenId].BRBCAmount =
            (tokensLP[_tokenId].BRBCAmount * (100 - penalty)) /
            100;
        BRBC.transfer(penaltyReciver, (beforePenaltyBRBC * penalty) / 100);
        poolUSDC -= (tokensLP[_tokenId].USDCAmount * penalty) / 100;
        tokensLP[_tokenId].USDCAmount =
            (tokensLP[_tokenId].USDCAmount * (100 - penalty)) /
            100;
        return true;
    }
        ...
        returns (uint256 rewardAmount)
    {
        if (_tokenId > tokensLP.length - 1) {
            return 0;
        } else if (tokensLP[_tokenId].isStaked == false) {
            return 0;
        } else {
            return
                (tokensLP[_tokenId].USDCAmount *
    ...
    /// @param _tokenId the token id
    function viewRequestTokenId(address _from, uint256 _tokenId)
        public
        view
        returns (bool isRequested)
    {
        return withdrawRequestList[_from][_tokenId];
    }
    ...
    /// @dev calculates current apr for 90 days
    /// returns current apr
    function apr() public view returns (uint256 aprNum) {
        if (poolUSDC == 0) {
            return 0;
        } else {
            return ((FullMath.mulDiv(totalRewardsAdded, 10**29, poolUSDC) /
                90) *
                365 *
                100);
        }
    ...
    function fundRequestsForWithdraw(
        ...
        for (uint256 i = 0; i < _withdrawAddress.length; i++) {
            require(
                withdrawRequestList[_withdrawAddress[i]][_tokenIds[i]] ==
                    true &&
            ...
            delete (withdrawRequestList[_withdrawAddress[i]][_tokenIds[i]]);
        }
        ...
        returns (bool withdrawSuccess)
    {
        ...
        bool burnt = _burn(_tokenId);
        require(burnt == true, "failed to burn token");
        emit Withdraw(
            address(this),
            msg.sender,
            _tokenId,
...
contract SetParams is AccessControl {
    ...
    uint32 public startTimeWhitelisted;
    // End time of stacking
    uint32 public endTime;
    uint32 public endWhitelistTime;
    ...
    function setPenaltyAddress(address _penaltyAddress) external OnlyManager {
        require(penaltyReciver != _penaltyAddress, "Address already set");
    ...
    function setRates(
        ...
        rate[30 days] = _rate1Month;
        rate[90 days] = _rate3Month;
        rate[180 days] = _rate6Month;
    }
