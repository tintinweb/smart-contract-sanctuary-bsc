...
contract Game is Ownable, ReentrancyGuard {
    ...
    mapping (address => uint256) public userPackageCount;
    ...
    function subscribe(uint256 _pid) external nonReentrant {
        ...
        UserInfo storage _userPackageInfo = userPackageInfo[msg.sender][userPackageCount[msg.sender]];
    ...
    function renew(uint256 _sid) external nonReentrant {
        ...
        if(_sid != 0)
            require(_sid < userPackageCount[msg.sender], "Invalid package ID");
    ...
    function isCycleEnded(address _userAddress, uint256 _sid) internal view returns(bool) {
        UserInfo storage _userPackageInfo = userPackageInfo[_userAddress][_sid];
        uint256 _cycleInSeconds = _userPackageInfo.daysPerCycle * secondsPerDay;

        return block.timestamp > _userPackageInfo.renewTimestamp + _cycleInSeconds;
    }
    ...
    function calculateDaysPerCycle(address _userAddress, uint256 _sid) public view returns(uint256) {
        UserInfo storage _userPackageInfo = userPackageInfo[_userAddress][_sid];
