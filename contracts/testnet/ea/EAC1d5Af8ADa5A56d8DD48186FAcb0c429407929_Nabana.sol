...
contract Nabana is ERC20, Ownable {
    ...
    // Own : 0x294d0487fdf7acecf342ae70AFc5549A6E90f3e0
    // Arg : 0x9F66f039E924011A11AfB45bC758eC34F589Ea11
    // Inv : 0x82cAac35D5eFa57dBC962482a45415935fc76BF8
    // Inv : 0x8E80D48978bECBf34C3454AAa82138c31A697B45
    // TODO testnet
    // addresses for minting/vesting
    address private constant pancakeSwapLaunchWallet = 0x7Ef8E5643424bed763dD1BdE66d4b2f79F9EDcd8;
    address private constant pinkSaleWallet = 0xb6596252872e20256f2D802F698dE341d21331Ee;
    address private constant privateInvestorsWallet = 0x45faf7923BAb5A5380515E055CA700519B3e4705;
    ...
    constructor(address _nabanaVesting) ERC20("Nabana", "BANA") {
        ...
        // TODO
        // owner and treasury wallet should be the same
        // if owner deploys do nothing
        // if we deploy, hard code the owner's wallet address here
    ...
    function updateTreasuryWallet(address newTreasuryWallet) external onlyOwner {
        require(treasuryWallet != newTreasuryWallet, "Nabana: Treasury Wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 150000 && newValue <= 900000, "Nabana: gasForProcessing must be between 150,000 and 900,000");
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 gasUsed = gasForProcessing / 3;
            gas -= gasForProcessing;

            try dividendTracker.processAccount(gas, from) returns (
                uint256 iterations,
                uint256 claims,
                uint256 lastProcIndex,
                uint256 lastProcInvestment
            ) {
                emit ProcessedDividendTracker(iterations, claims, lastProcIndex, lastProcInvestment, false, gasUsed, tx.origin);
            } catch {}
        }
        ...
        if (
            ...
            try dividendTracker.processAccount(gas, to) returns (
                uint256 iterations,
                uint256 claims,
                uint256 lastProcIndex,
                uint256 lastProcInvestment
...
contract NabanaDividendTracker is Ownable {
    ...
    function process(uint256 gas) public onlyOwner returns (uint256, uint256, uint256, uint256) {
        ...
        while (gasUsed < gas && iterations < numberOfInvestors) {
            ...
            if (result == 3 && _lastProcessedIndex >= investorsMap.keys.length.safeSub(1)) {
    ...
    function processAccount(uint256 gas, address account) public onlyOwner returns (uint256, uint256, uint256, uint256) {
        ...
        while (gasUsed < gas && iterations < numberOfInvestments) {
            ...
            else if (index == lastProcessedIndex && _lastProcessedInvestment < lastProcessedInvestment) {
                if (lastProcessedInvestment >= privateInvestments[account].length) {
                    lastProcessedInvestment = lastProcessedInvestment.safeSub(1);
                }
            }

            iterations++;

            if (result == 3 && index >= lastProcessedIndex) {
                lastProcessedIndex = lastProcessedIndex >= investorsMap.keys.length ? 0 : lastProcessedIndex + 1;
            }
        }
    ...
    function processInvestment(address account, uint256 index, bool manual) public onlyOwner returns (uint8) {
        ...
        if (
            ...
            uint256 amountToUnlock =
                investment.currentInterval + numberOfIntervals >= totalIntervals ?
                investment.amountToUnlock - investment.amountUnlocked :
    ...
    // TODO remove
    function zzz(uint256 x, uint256 y) external {
        lastProcessedIndex = x;
        lastProcessedInvestment = y;
    }
}