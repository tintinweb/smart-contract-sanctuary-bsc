...
//    mapping (uint256 => uint256) ;
    ...
    function powerToLevel(uint256 _power) internal pure returns(uint256) {
        for(uint256 i = 0; i <= MAX_LEVEL; i++) {
            if(_power < BASE_POWER_LEVEL_CEILING * 2 ** i) {
                return i;
            }
        }
        return MAX_LEVEL;
