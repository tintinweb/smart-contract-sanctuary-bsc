...
contract RagdollCatNFT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Ragdoll Cat NFT", "RDC") {
        ...
        transferStakingFee = 4;
        transferTeamFee = 0;
        transferTotalFees = transferMarketingFee + transferLiquidityFee + transferStakingFee + transferTeamFee;

        marketingWallet = address(0x38B65985Fb2F71fCE33Ce321f426efea24Cc8a45); // set as marketing wallet
    	stakingWallet = address(_owner); // set as dev wallet
    	teamWallet = address(0x0152072afc3F6BeA428ACfB9718e762ADa9dc17D); // set as teamWallet
    ...
    function updateTransferFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _teamFee) external onlyOwner {
        ...
        transferTotalFees = transferMarketingFee + transferLiquidityFee + transferStakingFee + transferTeamFee;
        require(transferTotalFees <= 5, "Must keep fees at 5% or less");
    }
    ...
    function swapBack() private {
        ...
        if(contractBalance > 0){
            ...
            try st.deposit (1, (contractBalance* 15)/ 100) {} catch {}
            try st.deposit (1, (contractBalance* 20)/ 100) {} catch {}
            try st.deposit (1, (contractBalance* 25)/ 100) {} catch {}
            try st.deposit (1, (contractBalance* 30)/ 100) {} catch {}
        }
