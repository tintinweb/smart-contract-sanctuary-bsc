...
contract RandomGenerator is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function getRandomNumbers(
        ...
        uint256[] memory numbers = new uint256[](request.randomWords.length);
        // if (request.fulfilled) {
        //     for (uint256 i; i < request.randomWords.length; i++) {
        //         numbers[i] = request.randomWords[i] * (_rangeEnd - _rangeStart) + _rangeStart;
        //     }
        //     requestToNumbers[lastRequestId] = numbers;
        // }
        return numbers;
    }
}