...
contract UGCStaking {
    ...
    function buy(
        ...
        MONTHLY_REWARD_POOL_SILVER +=
            ...
            PERCENTS_DIVIDER;
        MONTHLY_REWARD_POOL_GOLD += (_amount * MONTHLY_REWARD_POOL_PERCENT) /
            PERCENTS_DIVIDER;
        MONTHLY_REWARD_POOL_DIAMOND +=
            (_amount * MONTHLY_REWARD_POOL_PERCENT) /
            PERCENTS_DIVIDER;
        MONTHLY_REWARD_POOL_CROWN +=    (_amount * MONTHLY_REWARD_POOL_PERCENT) /
            PERCENTS_DIVIDER;
    ...
    function seteligibility(
        ...
        users[upline].teamturnover_directs += _amount;
            if (
                ...
                TEAM_TURNOVER_AMOUNTS_MONTHLY[0] &&
            ...
            if (
                users[upline].teamturnover >=
                TEAM_TURNOVER_AMOUNTS_MONTHLY[1] &&
            ...
            if (
                users[upline].teamturnover >=
                TEAM_TURNOVER_AMOUNTS_MONTHLY[2] &&
            ...
            if (
                users[upline].teamturnover >=
                TEAM_TURNOVER_AMOUNTS_MONTHLY[3] &&
    ...
    function withdraw_monthly_pool_silver() external {
        ...
        require(block.timestamp > MONTHLY_REWARD_POOL_SILVER_TRIGGERD_AT + ONE_MONTH);
        {
            uint256 amount = MONTHLY_REWARD_POOL_SILVER /
                MONTHLY_REWARD_POOL_SILVER_ELIGIBLES.length;
            for (uint256 i = 0; i < MONTHLY_REWARD_POOL_SILVER_ELIGIBLES.length; i++) {
    ...
    function withdraw_monthly_pool_gold() external{
        ...
        require(block.timestamp > MONTHLY_REWARD_POOL_GOLD_TRIGGERD_AT + ONE_MONTH);
        {
            uint256 amount = MONTHLY_REWARD_POOL_GOLD /
                MONTHLY_REWARD_POOL_GOLD_ELIGIBLES.length;
            for (uint256 i = 0; i < MONTHLY_REWARD_POOL_GOLD_ELIGIBLES.length; i++) {
    ...
    function withdraw_monthly_pool_diamond() external{
        ...
        require(block.timestamp > MONTHLY_REWARD_POOL_DIAMOND_TRIGGERD_AT + ONE_MONTH);
        {
            uint256 amount = MONTHLY_REWARD_POOL_DIAMOND /
                MONTHLY_REWARD_POOL_DIAMOND_ELIGIBLES.length;
            for (uint256 i = 0; i < MONTHLY_REWARD_POOL_DIAMOND_ELIGIBLES.length; i++) {
    ...
    function withdraw_monthly_pool_crown() external {
        ...
        require(block.timestamp > MONTHLY_REWARD_POOL_CROWN_TRIGGERD_AT + ONE_MONTH);
        {
            uint256 amount = MONTHLY_REWARD_POOL_CROWN /
                MONTHLY_REWARD_POOL_CROWN_ELIGIBLES.length;
            for (uint256 i = 0; i < MONTHLY_REWARD_POOL_CROWN_ELIGIBLES.length; i++) {
