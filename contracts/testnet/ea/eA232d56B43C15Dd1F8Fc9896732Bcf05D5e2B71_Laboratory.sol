...
// File: interfaces/ILaboratory.sol
...
contract MadToken is ERC20, Ownable {
    /**
     * @dev Mint 10kk tokens to deployer
     */
    constructor(address owner) ERC20('MAD Token', 'MAD') {
    }
...
contract NanoCell is ERC721, Ownable {
...
contract Laboratory is ILaboratory, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    ...
    uint256 BOOST_PER_BLOCK_PRICE = 50000000000000 wei; // 0,00005 ether
    uint256 private NUM_OPTIONS = 1000;

    modifier isAllowedToBoost(uint256 _tokenID, address owner) {
        CellData.Cell memory cell = ICellRepository(repository).getMetaCell(_tokenID, owner);
    ...
    /**
     * @dev Sets storage address
     * can be used only for owner
     */
    function setRepository(address _repository) external onlyOwner {
     ...
     * can be used only for owner
     */
    function setNanoCell(address _token) external onlyOwner {
        nanoCell = _token;
    }
    ...
    function evolve(uint256 _tokenID, uint256 _enhancerID) external override {
        ...
        if (_enhancerID != 0) {
            enhancer = IEnhancerRepository(repository).getEnhancerInfo(_enhancerID);
            require(enhancer.id != type(uint256).max, 'Non-existent enhancer');
            require(
                IEnhancerRepository(repository).getEnhancersAmount(msg.sender, _enhancerID) > 0,
        ...
        if (_enhancerID != 0) {
            IEnhancerRepository(repository).decreaseEnhancersAmount(msg.sender, _enhancerID, 1);
        }
    ...
    function _processSplit(CellData.Cell memory cell) private {
         if (CellData.Class(cell.class) == CellData.Class.SPLITTABLE_NANO) {
            ...
            NanoCell(nanoCell).mintTo(msg.sender, _nanoCells.current());
        } else if (CellData.Class(cell.class) == CellData.Class.SPLITTABLE_MAD) {
            MadToken(madToken).mint(msg.sender, 1);
        }
    }

    function _evolve(CellData.Cell memory cell, CellEnhancer.Enhancer memory _enhancer)
        private
        view
        returns (CellData.Cell memory)
    {
        require(cell.user == msg.sender, 'You are not an owner of token');
        require(cell.stage < MAX_LEVEL_OF_EVOLUTION, 'You are not able to evolve more');
        require(cell.class != CellData.Class.FINISHED, 'You are not able to evolve more');
        require(cell.nextEvolutionBlock <= block.number, "You can't evolve right now");
        ...
        if (
            ...
            cell.stage = IRandom(random).getRandomStage(cell.stage, probabilityChanceIncreased);
        } else {
        ...
        if (
            ...
            cell.class = CellData.Class(
                IRandom(random).getSplittableWithIncreaseChance(probabilityChanceIncreased)
            );
        } else {
        ...
        if (cell.class == CellData.Class.FINISHED || cell.stage == MAX_LEVEL_OF_EVOLUTION) {
        ...
        isAllowedToBoost(_tokenID, msg.sender)
    {
        ...
        if (block.number >= cell.nextEvolutionBlock.sub(_blocksAmount)) {
            ...
            cell.nextEvolutionBlock = cell.nextEvolutionBlock.sub(_blocksAmount);
        }
        ICellRepository(repository).updateMetaCell(cell, msg.sender);
    ...
    function mutate(uint256 _cellId, uint256 _nftId) external override {
        CellData.Cell memory oldCell = ICellRepository(repository).getMetaCell(_cellId, msg.sender);
        ExternalNft.Nft memory nft = IExternalNftRepository(repository).getNft(_nftId, msg.sender);
        require(oldCell.tokenId >= 0 && nft.tokenId >= 0, 'Non-existent cell or nft');

        CellData.Cell memory newCell = _mergeWithNft(oldCell, nft, NUM_OPTIONS);
        IExternalNftRepository(repository).markNftAsUsed(_nftId, msg.sender);
        bytes[] memory data = new bytes[](2);
        data[0] = abi.encodeWithSignature('removeMetaCell(uint256,address)', _cellId, msg.sender);
        data[1] = abi.encodeWithSignature(
            'addMetaCell((uint256,address,uint8,uint256,uint256,uint256,string,bool,uint256))',
            newCell
        );
        ICellRepository(repository).multicall(data);
    ...
    function _mergeWithNft(
        ...
        if (newCell.stage == MAX_LEVEL_OF_EVOLUTION || newCell.class == CellData.Class.FINISHED) {
    ...
    function _createNewToken(address tokenOwner, uint256 _numOptions)
        ...
        returns (CellData.Cell memory)
    {
        CellData.Class newClass = CellData.Class(IRandom(random).getRandomClass());
    ...
    function setBoostPerBlockPrice(uint256 _price) external override onlyOwner {
    ...
    function getBoostedBlocks(uint256 _price) external view override returns (uint256) {
     ...
     * run evolve
     */
    function getAmountForNextEvolution(uint256 _tokenID) external view returns (uint256) {
        CellData.Cell memory cell = ICellRepository(repository).getMetaCell(_tokenID, msg.sender);
        require(cell.tokenId >= 0, 'Non-existent cell');
        require(cell.nextEvolutionBlock > block.number, 'Boost is not needed.');
        return cell.nextEvolutionBlock.sub(block.number).mul(getBoostPerBlockPrice());
    }
}