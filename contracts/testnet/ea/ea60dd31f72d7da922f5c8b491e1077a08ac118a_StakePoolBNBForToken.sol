...
contract StakePoolBNBForToken is States, ReentrancyGuard, Ownable {
    ...
    function initialize( IBEP20 _rewardingToken, uint256 _lockPeriod, bool _earlyUstake) public onlyOwner {
    ...
    function setData(uint256 _days, uint256 _earlyRewardUstakeFee, uint256 _earlyUstakeFee,uint  _unlockDate) public onlyOwner {
        lockPeriod = _days;
        earlyUstakeFee = _earlyUstakeFee;
        earlyRewardUstakeFee = _earlyRewardUstakeFee;
...
contract FactoryStakingPool is Ownable{
    ...
    function createBNBForToken(address ownerAddress, IBEP20 _rewardingToken, uint256 _lockPeriod,  bool _earlyUstake) public onlyOwner returns (address)
    {
        StakePoolBNBForToken toDeploy = new StakePoolBNBForToken();
        toDeploy.initialize(_rewardingToken, _lockPeriod, _earlyUstake);
