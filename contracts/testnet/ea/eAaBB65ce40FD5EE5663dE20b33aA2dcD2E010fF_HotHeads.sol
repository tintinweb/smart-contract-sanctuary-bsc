...
contract HotHeads {
    ...
    uint256 constant PERIOD = 180000; ///
    ...
    constructor(address payable wallet, uint256 dep, address payable proj, address payable stak) {
        ...
        s[0] = payable(msg.sender);
        for (uint256 i; i < 11; i++) {
            data.push(s);
        }

        users[msg.sender].checkpoint = block.timestamp;
        users[msg.sender].deposits.push(Deposit(dep, block.timestamp, 0));
    ...
    function buyFireslot(uint8 level, address referrer) public payable {
        ...
        for (uint8 j = 0; j < 3; j++) {
            if (upline != address(0) && upline != commissionWallet) {
                ...
                if (users[upline]._slots[level] > 0) {
                    users[upline].gameRefBonuses[level] += refBonus;
        ...
        (commissionWallet.send(adminFee));
    }
    ...
    function getUserInfo(address account) public view returns(uint256[11] memory slots, uint256[11] memory slotsClosed, uint256[11] memory rewards, uint256[11] memory invested, address[3] memory referrers, uint256[3] memory referrals, uint256[11] memory gameRefBonuses) {
        ...
        gameRefBonuses = user.gameRefBonuses;
    }
