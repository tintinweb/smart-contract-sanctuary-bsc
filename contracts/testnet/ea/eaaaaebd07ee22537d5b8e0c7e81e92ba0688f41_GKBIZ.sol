...
contract GKBIZ is Ownable {
    ...
    uint256 public dayProfit = 1369 * 10**9;
    bool public enableDistribute = true;
    ...
    constructor(address gksAddress_, address gkAddress_, address liquidityReceiveAddress_) {
        ...
        gksLPPair = uniswapV2Factory.getPair(gksAddress, bep20usdt);
    ...
    function mint(uint256 amount) public {
        ...
        ITokenHelper tokenHelper = ITokenHelper(gkTokenHelperAddress);
        require(tokenHelper.claimProfit(_msgSender(), amount));

        // swap gk to gks
        IERC20 gksContract = IERC20(gksAddress);
        uint256 initGKSBlance = gksContract.balanceOf(address(this));
        _swapTokensForGks(amount, address(this));
        uint256 gksOut = gksContract.balanceOf(address(this)).sub(initGKSBlance);

        gksContract.transfer(_msgSender(), gksOut);

        _updateLpReward(address(0), address(0));
    }
    ...
    function addGKSLiquidity(uint256 usdtAmount, uint256 gksAmount) lockTheSwap public {
        ...
        // add the liquidity
        (uint256 amountA, uint256 amountB, ) = uniswapV2Router.addLiquidity(
            gksAddress,
            bep20usdt,
            gksAmount,
            usdtAmount,
        ...
        _updateLpReward(_msgSender(), gksLPPair);
    ...
    function _feeLiquidity(uint256 gksFeeAmount) private {
        ...
        IERC20 gksInstance = IERC20(gksAddress);
        gksInstance.approve(address(uniswapV2Router), gksFeeAmount);
    ...
    function removeGKSLiquidity(uint256 lpAmount) lockTheSwap public {
        ...
        _updateLpReward(gksLPPair, _msgSender());
    }
    ...
    function setDayProfit(uint256 amount) public onlyOwner {
        dayProfit = amount;
    }

    function setEnableDistribute(bool enable) public onlyOwner {
        enableDistribute = enable;
    }
    ...
    function _updateLpReward(address from, address to) private {
        if(fromAddress == address(0) )fromAddress = from;
        if(toAddress == address(0) )toAddress = to;  
        if(!isDividendExempt[fromAddress] && fromAddress != gksLPPair ) setShare(fromAddress);
        if(!isDividendExempt[toAddress] && toAddress != gksLPPair ) setShare(toAddress);
    ...
    function process(uint256 gas) private {
        ...
        uint256 nowbanance = dayProfit;
        uint256 gasUsed = 0;
        uint256 gasLeft = gasleft();

        uint256 iterations = 0;

        IERC20 gkContract = IERC20(gkAddress);
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            ...
            uint256 amount = nowbanance.mul(IERC20(gksLPPair).balanceOf(shareholders[currentIndex])).div(IERC20(gksLPPair).totalSupply());
            if( amount == 0) {
                currentIndex++;
                iterations++;
                continue;
            ...
            if(gkContract.balanceOf(address(this)) < amount) return;
            
            gkContract.transfer(shareholders[currentIndex], amount);
    ...
    function setShare(address shareholder) private {
        if(_updated[shareholder] ){      
            if(IERC20(gksLPPair).balanceOf(shareholder) == 0) quitShare(shareholder);              
            return;  
        }
        if(IERC20(gksLPPair).balanceOf(shareholder) == 0) return;  
