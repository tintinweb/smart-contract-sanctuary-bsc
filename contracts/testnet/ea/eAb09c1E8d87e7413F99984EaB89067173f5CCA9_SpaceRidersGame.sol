...
contract SpaceRidersGame is Ownable {
    ...
    */
    ...
    function stakingRequest(
        ...
        sp.transferFrom(msg.sender, feeWallet, fee);
        sp.transferFrom(msg.sender, address(this), stakedAmount);
        feeWallet.transfer(bnbFee);
    ...
    function withdrawStaking(string calldata planetId) external payable {
        require(
        ...
        uint256 amount = userStaking[planetId].amount;
        ...
        getSpaceRiders().transfer(msg.sender, amount);
        feeWallet.transfer(bnbFee);
    }
...
contract SpaceRiders is Ownable, IERC20 {
    ...
    function _recalculateNewBalance(uint256 totalSprBack, UserAddLiquidity memory userAddLiquidity) private pure returns(uint256 newTradeableBalance, uint256 newPlayableBalance) {
    ...
    /**
     * Calculate how many tokens the user can buy/hold (in usd value) depending on how many planet NFT's he has.
     */
