...
contract Storage is events, Ownable {
    ...
    function _cancelTrx(uint96 productID, bool release) internal {
        ...
        if (msg.sender == _buyer) {
            ...
            if(hasBuyerTick[productID].hasBuyerTick) {
                _contractTransfer(s_token, seller, s_amount);
                allProducts[productID].isCompleted = true;
            } else if (block.timestamp > (allProducts[productID].startSales + 600)) {
                _contractTransfer(s_token, seller, s_amount);
                allProducts[productID].isCompleted = true;
            } else {
...
abstract contract Rigelp2pPriceSet is Storage {
    ...
    function setDeploy(
        ...
        _setDeployment(
            sellersFeeInRGP,
            unWhitelistedAddressReferralFeeInPercent,
            unListedTokenRewardPercent,
...
contract RigelDecentralizedP2PSystem is Rigelp2pPriceSet {
    ...
    */
     ...
     */ 
    function makeSellOrder(IERC20 purchaseToken, uint256 amount, address from, address to, address referral) external checkBlacklist(from) returns(uint256){   
