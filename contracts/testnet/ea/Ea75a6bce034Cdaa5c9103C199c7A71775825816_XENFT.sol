...
contract XENFT is ERC721Enumerable {
  ...
  mapping(uint256 => XENFTHoder) public holders;

  XENCrypto public immutable xen;

  constructor(XENCrypto xen_, Traits traits_) ERC721('the NFT of XENCrypto', 'XENFT') {
    xen = xen_;
    traits = traits_;
  }
  ...
  function mint(uint256 term) public {
    _mint(msg.sender, id);
    holders[id] = new XENFTHoder(this, id, term, xen);
    id++;
  }

  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    _requireMinted(tokenId);
    return traits.tokenURI(tokenId, holders[tokenId]);
  }
}
...
contract XENFTHoder is Context {
  ERC721 public immutable XENFTContract;
  XENCrypto public immutable xen;
...
contract Traits {
  ...
  function tokenURI(uint256 tokenId, XENFTHoder owner) public view returns (string memory) {
