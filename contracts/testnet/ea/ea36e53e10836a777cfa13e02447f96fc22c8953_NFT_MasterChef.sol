...
interface IPromote{
    ...
    function newDeposit(address sender,uint256, uint256 amount) external;
    function redem(address sender, uint256, uint256 amount) external;
}
...
contract NFT_MasterChef is Initializable, OwnableUpgradeable, Member {
    ...
    uint256 constant public CUT_PERIOD = 45 days;
    uint256 constant public ADD_PERIOD = 1000;
    uint256 constant public MAX_PERIOD = 10000;
    uint256 minimalPower;  // 全網最低派獎啟動算力
    ...
    event StakeWarDistributeLoser(address loserUser, uint256 amount);
    ...
    // 可以 mint 的總量上限 76% 380 万 
    uint256 MAX_MINT_MCA;
    uint256 mintedMCA;
    ...
    function pendingSushi(uint256 _pid, address _user) public view returns (uint256) {
        ...
        uint256 pending = user.pendingReward.add(user.amount.mul(accSushiPerShare).div(1e18).sub(user.rewardDebt));
        pending = pending.sub(userStakeWarProfit[_user].loseAmount).add(userStakeWarProfit[_user].winAmount);
        return pending;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (CUT_FLAG && CUT_TIME < CUT_MAX_TIME) {
            // 減產 20%
            if(block.timestamp >= lastCutBlock.add(CUT_PERIOD)){
                ...
                CUT_TIME = CUT_TIME + 1;
                lastCutBlock = block.timestamp;
            }
        } else if (!CUT_FLAG) {
            // 算力每高過一階增產 25%，且算力小於 10 階
            if (lpSupply > lastCutPower.add(ADD_PERIOD)) {
                lastCutPower = lastCutPower.add(ADD_PERIOD);
                // 增產 25%
                uint256 newSushiPerBlock = sushiPerBlock.mul(125).div(100);
                emit PerShareChanged(sushiPerBlock, newSushiPerBlock, block.number);
                sushiPerBlock = newSushiPerBlock;

                // 滿產觸發後就會停止增產了
                if (lpSupply > MAX_PERIOD) {
                    CUT_FLAG = true;
                    lastCutBlock = block.timestamp;
                }
            }
    ...
    function deposit_In(uint256 _pid, uint256 _power) internal {
        IPromote(manager.members("MCAStakePool")).newDeposit(msg.sender,0, _power);

        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];

        user.amount = user.amount.add(_power);
    ...
    function _stake(uint256 _pid,uint256 tokenID) internal returns(uint256 power){
        ...
        // 每次質押。更新 startRewordTime
        user.startRewordTime = block.timestamp;

        pool.totalStakingNFTAmount = pool.totalStakingNFTAmount.add(1);           //更新总抵押nft数量
        pool.lpSupply = pool.lpSupply.add(power);
        INFT(pool.lpToken).transferFrom(msg.sender,address(this),tokenID);

        // 質押時 transfter 給值推鑄造時的獎勵分配
        // 先檢查值推是否為有效
        address f = IPromote(manager.members("MCAStakePool")).getUser(msg.sender).f;
        UserInfo storage uplineUserInfo = userInfo[0][f];

        if (nftAttr.price > 0 && !UserNFTMintUplineReword[tokenID]) {
            uint256 uplineReword = nftAttr.price.mul(5).div(100);
            uint256 burnReword;
            if (uplineUserInfo._nftBalances.length == 0) {
                burnReword = uplineReword.div(2);
                uplineReword = uplineReword.sub(burnReword);

                // burn
                IERC20(mca).burn(burnReword);
            }
            UserNFTMintUplineReword[tokenID] = true;
            safeSushiTransfer(msg.sender, uplineReword);
        }
    ...
    function withdraw_In(uint256 _pid, uint256 _power) internal {
        IPromote(manager.members("MCAStakePool")).redem(msg.sender , 0, _power);
        UserInfo storage user = userInfo[_pid][msg.sender];
        
        user.amount = user.amount.sub(_power);
        emit Withdraw(msg.sender, 0, _power);
    }
    ...
    function getReward(uint256 _pid) public {
        ...
        updatePool(_pid);
        uint256 pending = user.pendingReward.add(user.amount.mul(pool.accSushiPerShare).div(1e18).sub(user.rewardDebt));
        pending = pending.sub(userStakeWarProfit[msg.sender].loseAmount).add(userStakeWarProfit[msg.sender].winAmount);
        if(pending>0){
            // 提領獎勵 7 天為一週期（開始質押後開始算），其他時間領獎，放棄 50% 獎勵，往彩票池
            if ((block.timestamp.sub(user.startRewordTime)).mod(sevenDays+1 days) < sevenDays) {
                uint256 giveUp = pending.div(2);
                pending = pending.sub(giveUp);
                safeMint(manager.members("lottery"), giveUp);
            }
            safeMint(msg.sender, pending);
        }
    ...
    function withdrawNFT(uint256 _pid,uint256 tokenID) internal returns(uint256 power){
        ...
        pool.lpSupply = pool.lpSupply.sub(power);
    ...
    function safeMint(address _to, uint256 _amount) internal {
        // 剩餘可以鑄造出的量
        uint256 remaining = MAX_MINT_MCA.sub(mintedMCA);
        require(remaining > 0, "not enough reward");

        uint256 toAmount = _amount;
        if (remaining < toAmount) {
            toAmount = remaining;
        }

        mintedMCA = mintedMCA.add(toAmount);
        mca.mint(_to, toAmount);
    }
    ...
    function StakeWarDistribute(address winerUser, address loserUser, uint256 bonus) external {
        ...
        uint256 result = loserPending.mul(bonus).div(100);
        ...
        if (ff != address(0)) {
            if (userInfo[0][ff]._nftBalances.length > 0) {
                ...
                emit StakeWarDistributeWinnerUpline(f, 2, dist2);
            }
        }
        ...
        emit StakeWarDistributeLoser(loserUser, result);
    }
