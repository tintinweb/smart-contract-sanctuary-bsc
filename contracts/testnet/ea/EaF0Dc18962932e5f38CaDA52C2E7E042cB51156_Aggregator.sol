...
contract Aggregator is AccessControl, Multicall {
    ...
    function stakeAnkrBnb() external payable {
        ...
        address(ankrBnb).delegatecall(abi.encodeWithSelector(IBinancePool_R4(ankrBnb).stakeAndClaimCerts{value: payableAmount}.selector));
        (bool success, bytes memory result) = address(ankrBnb).delegatecall(abi.encodeWithSignature("stakeAndClaimCerts()"));
        require(success, "Failure in address(ankrBnb).delegatecall(abi.encodeWithSignature(\"stakeAndClaimCerts()\"))");
