...
interface IPoolFunctionality {
    ...
    function doSwapThroughOrionPoolSupportingFee(
        address user,
        address to,
        IPoolFunctionality.SwapData calldata swapData
    ) external returns (uint amountOut, uint amountIn);
...
library LibPool {
    ...
    function doSwapThroughOrionPool(
        ...
        uint amountIn;
        if (d.supportingFee) {
        (amountOut, amountIn) = IPoolFunctionality(d.orionpool_router).doSwapThroughOrionPoolSupportingFee(
            d.isInContractTrade || d.isSentETHEnough ? address(this) : msg.sender,
            d.isInContractTrade ? address(this) : msg.sender,
            d
        );
        } else {
        (amountOut, amountIn) = IPoolFunctionality(d.orionpool_router).doSwapThroughOrionPool(
            d.isInContractTrade || d.isSentETHEnough ? address(this) : msg.sender,
            d.isInContractTrade ? address(this) : msg.sender,
            d
        );
