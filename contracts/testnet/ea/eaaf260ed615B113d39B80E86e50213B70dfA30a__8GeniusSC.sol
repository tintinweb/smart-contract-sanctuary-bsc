...
contract _8GeniusSC is Context, IERC20, Ownable,Initializable {   
    ...
    uint256 public _maxWhaleLimits;
    uint256 public _minWhaleLimits;
   ...
   function pauseTransaction() onlyOwner public {
    ...
    function update_WhaleLimits(uint256 maxWhaleLimits,uint256 minWhaleLimits) public onlyOwner {
       _maxWhaleLimits=maxWhaleLimits;
       _minWhaleLimits=minWhaleLimits;
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        require(paused != true, "BEP20: Transaction Is Paused now");  
         if(to == pancakePair && from!=address(this)) {
            require(amount <= _maxWhaleLimits, "BEP20: Sell Qty Exceed !");
            require(amount >= _minWhaleLimits, "BEP20: Sell Qty Does Not Match !"); 
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance,uint TaxType) private lockTheSwap {
        ...
        uint MarketingPer=0;
        if(TaxType==1){
            EachPer=100/(_LiquidityBuyPer.add(_marketingBuyPer));
            MarketingPer=EachPer*_marketingBuyPer;
        }
        else{
            EachPer=100/(_LiquiditySellPer.add(_marketingSellPer));
    ...
    function _takeMarketingFee(uint256 tAmount,uint256 tFee,uint TaxType) private {
        ...
        if(tFee<(MarketingShare.add(RewardShare).add(LiquidityShare))){
            RewardShare=RewardShare.sub((MarketingShare.add(LiquidityShare)).sub(tFee));
        }
        uint256 FeeMarkRewLiqu=MarketingShare+LiquidityShare;
