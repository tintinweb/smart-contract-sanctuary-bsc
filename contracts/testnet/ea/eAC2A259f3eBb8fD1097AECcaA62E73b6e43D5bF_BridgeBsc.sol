...
abstract contract BridgeBase is Ownable {
    ...
    function receiveTokens(address account, uint256 amount, uint256 fee) public onlyOwner {
        ...
        require(_feesPaid[account] == fee, "BridgeBase: Incorrect fee paid");

        _avoidReentrancy = true;
        _tokensBridged[account] += amount;
        _feesPaid[account] -= fee;

        uint256 amountToSend = _tokensBridged[account] - _tokensReceived[account];

        if (ERC20(_tokenAddress).transfer(account, amountToSend)) {
            _tokensReceived[account] += amountToSend;
        }
