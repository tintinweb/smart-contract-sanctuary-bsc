...
contract USDyBUSDVault is Initializable, ReentrancyGuard, Pausable, IEvents {
    ...
    */
    ...
    function deposit(address user, uint amount,bool isBUSD) nonReentrant external _lastExchangeRatio _completePause whenNotPaused {
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "USDyBUSD: address is Blacklisted");
        require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender) || user == msg.sender);
        require(amount > 0,"USDyBUSD: LPamount can't be 0");
        require(isBUSD,"USDyBUSD: Only Deposit BUSD");
        ...
        if(_isContract(msg.sender)){
            require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender),"USDyBUSD: No external contract interaction");
        }
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(restrictTransfer[user] != block.number, "USDyBUSD: you can't interact in same block");
        }
        IERC20(BUSD).safeTransferFrom(user,address(this),amount);
         console.log(IERC20(Admin.USDy()).balanceOf(Admin.USDyBUSD()),"pair bal");
         console.log(amount,"amt deposit");
         console.log(IERC20(BUSD).balanceOf(Admin.USDyBUSD()),"vault bal");
        ...
        uint USDyAmount = (( (amount * 4 / 5 ) * (10 ** 18))/USDyPrice);
        IReceipt(Admin.USDy()).mint(address(this), USDyLiquidity + USDyAmount);
        _convertBUSDToLP((amount * 4/5), USDyLiquidity + USDyAmount,BUSD, Admin.USDy(), Admin.USDyBUSD());
        _tax(amount);
    ...
    */
    ...
    function _withdraw(address user,uint _amount,address sendTo) nonReentrant internal _completePause _lastExchangeRatio{
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "USDyBUSD: address is Blacklisted");
        require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender) || (sendTo == msg.sender && user == msg.sender),"USDyBUSD: No external contract interaction");
        require(_amount <= UserDeposit[user],"USDyBUSD: Invalid Amount");
        if(UserDeposit[user] > 0){
            _claimRebaseReward(user);
        }
        if(_isContract(msg.sender)){
    ...
    function _convertBUSDToLP(uint amountBUSD,uint amountToken2,address path0,address path1,address lp) internal{
     ...
     function _claimRebaseReward(address user) internal {
        ...
        require(reward > 0,"USDyBUSD: Please claim after rebase");
        }
        if(Admin.PhoenixNFT() != address(0) && IReceipt(Admin.PhoenixNFT()).balanceOf(user) >= 1){
