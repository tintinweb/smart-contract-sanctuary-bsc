...
contract XillionIDOMaster is XillionIDOCommon {
    ...
    function finish() external onlyOwner nonReentrant {
        ...
        if (totalInvestedAmount >= idoValueCap * minInvestmentPercentForIDOCompletion / 100) {
            ...
            uint256 finalSupply = numberOfPoolTokens * totalInvestedAmount / idoValueCap;
...
contract XillionIDO is XillionIDOCommon {
    ...
    function joinOrRecalculateAllocation() external {
        (bool success, bytes memory result) = getMasterIDOAddress().delegatecall(abi.encodeWithSignature("joinOrRecalculateAllocation()"));
    ...
    function invest() external payable {
        (bool success, bytes memory result) = getMasterIDOAddress().delegatecall(abi.encodeWithSignature("invest()"));
        revertOnDelegateCallFailure(success, result);
    }
    ...
    function finish() external {
        (bool success, bytes memory result) = getMasterIDOAddress().delegatecall(abi.encodeWithSignature("finish()"));
        revertOnDelegateCallFailure(success, result);
    }
    ...
    function kill() external {
        (bool success, bytes memory result) = getMasterIDOAddress().delegatecall(abi.encodeWithSignature("kill()"));
        revertOnDelegateCallFailure(success, result);
    }
    ...
    function claimPoolTokens() external {
        (bool success, bytes memory result) = getMasterIDOAddress().delegatecall(abi.encodeWithSignature("claimPoolTokens()"));
        revertOnDelegateCallFailure(success, result);
    }
    ...
    function claimRefundedInvestment() external {
        (bool success, bytes memory result) = getMasterIDOAddress().delegatecall(abi.encodeWithSignature("claimRefundedInvestment()"));
        revertOnDelegateCallFailure(success, result);
    }
    ...
    function claimTotalInvestedAmount() external {
        (bool success, bytes memory result) = getMasterIDOAddress().delegatecall(abi.encodeWithSignature("claimTotalInvestedAmount()"));
        revertOnDelegateCallFailure(success, result);
    }
