...
contract GulfStacking is ReentrancyGuard, Ownable {
    ...
    mapping(address => uint256) public claimedRewardsTransfered;
    //mapping(address => StackingUserDeatils) public stackingUserDeatil;
    mapping(address => StackingUserDeatils) public stackingUserDeatil;
    struct unstackRequest {
        uint256 amount;
        bool status; //0 for not approved 1 for approved
    }
    mapping(address => unstackRequest) public unstackingRequests;
    ...
    constructor() {
        gulfTokenAddress        =   0xD5C3C4B4F80fFfd8E7a130F4846496BDa6035728;//_gulfTokenAddress;
        stackingPrice           =   10*10**18;
        minimumStackAmount      =   500*10*18;   
    ...
    function updatePanalityPercentage(uint256 pakageNumber, uint256 newPercentage) public onlyOwner{
        require(newPercentage > 0, "you can not update zero percentage");
        require(pakageNumber == 1 || pakageNumber == 2 || pakageNumber == 3, "Package not exists!");
        panelityPercentage[pakageNumber] = newPercentage;
    }

    function updateStackingPrice(uint256 newPrice) public onlyOwner {
        require(newPrice > 0, "You can not set zero Price!");
        stackingPrice = newPrice * 10**18;
    }

    function calculateRewards(address wallet, uint256 pendingMonth)internal view returns(uint256){
        return  pendingMonth*1000000000000000000;  //(pendingMonth  * calculatePercentageAmount(wallet, rewardPercentage[stackingUserDeatil[wallet].stackingPlan]) );
    }
    ...
    function calculatePercentageAmount(address wallet, uint256 _percentage) internal view returns(uint256){
        ...
        //uint256 percentage  = ( amount / _percentage) * 100 ;
        //return convertIntoGewi(percentage);
        return 1000000000000000;
    }

    function renewPackage(uint32 newPackage) public {
        require(stackingUserDeatil[msg.sender].status = true, "You can not renew your package");
        require(stackingUserDeatil[msg.sender].amountStaked >= (minimumStackAmount + stackingPrice), "Your stacking amount it not valid please unstack and stack again");
        require(newPackage == 1 || newPackage == 2 || newPackage == 3, "Stacking plan is not valid!");
        uint256 userBalance = getBalance();
        require(userBalance >= stackingUserDeatil[msg.sender].amountStaked, "You don't have sufficient balance for stacking!");
        stackingUserDeatil[msg.sender].amountStaked += (stackingUserDeatil[msg.sender].amountStaked - stackingPrice);
        ...
        unstackingRequests[msg.sender].status = false;
        unstackingRequests[msg.sender].amount = 0;
    }
    ...
    function stackToken(uint256 _amount, uint32 _stackingPlan) external nonReentrant{
        require(_stackingPlan == 1 || _stackingPlan == 2 || _stackingPlan == 3, "Stacking plan is not valid!");
        require(stackingUserDeatil[msg.sender].stackingPlan == _stackingPlan || stackingUserDeatil[msg.sender].stackingPlan == 0, "Your stacking package is mismatched please check and try again!");
        require(stackingUserDeatil[msg.sender].status == false, "Your package is finish please renew your package!");
        uint256 amount = convertIntoGewi(_amount);
        require(amount >= (minimumStackAmount + stackingPrice) , "You can not stack this amount because your amount is less than minimum amount");
        uint256 userBalance = getBalance();
        require(userBalance >= _amount, "You don't have sufficient balance for stacking!");
        if(stackingUserDeatil[msg.sender].amountStaked > 0){
            claimRewardNewStacking();
        }
        IERC20(gulfTokenAddress).approve(address(this), amount  );
        IERC20(gulfTokenAddress).transferFrom(msg.sender, address(this), amount );
        stackingUserDeatil[msg.sender].amountStaked += (amount - stackingPrice);
        stackingUserDeatil[msg.sender].timeOfLastUpdate = block.timestamp;
        ...
        stackingChargesAmount += stackingPrice;
        totalstackedAmount += (amount - stackingPrice);
    }

    event claimReward(uint256 balance, string value); 
    function claimRewards() external {
        ...
        require( (stackedTime - claimedRewardsMonthCount[msg.sender]) > 0 , "Your package is completed!");
        require(stackingUserDeatil[msg.sender].stackingPlan == 1 && claimedRewardsMonthCount[msg.sender] < 6 || stackingUserDeatil[msg.sender].stackingPlan == 2 && claimedRewardsMonthCount[msg.sender] < 12 || stackingUserDeatil[msg.sender].stackingPlan == 3 && claimedRewardsMonthCount[msg.sender] < 24, "You can not claimed you already done!");
        claimedRewardsMonthCount[msg.sender] +=  stackedTime;
        ...
        uint256 totalRewards = calculateRewards(msg.sender, (stackedTime - claimedRewardsMonthCount[msg.sender]) ) + stackingUserDeatil[msg.sender].unclaimedRewards;
        emit claimReward(totalRewards, "Total Reward");
        ...
        claimedRewardsTransfered[msg.sender] += totalRewards;
    ...
    function claimRewardNewStacking() internal {
        uint256 stackedTime = getMonth(block.timestamp , stackingUserDeatil[msg.sender].startTime);
        if(stackedTime > 0 && stackingUserDeatil[msg.sender].stackingPlan == 1 && claimedRewardsMonthCount[msg.sender] < 6 || stackingUserDeatil[msg.sender].stackingPlan == 2 && claimedRewardsMonthCount[msg.sender] < 12 || stackingUserDeatil[msg.sender].stackingPlan == 3 && claimedRewardsMonthCount[msg.sender] < 24){
            claimedRewardsMonthCount[msg.sender] +=  stackedTime;
            uint256 totalRewards = calculateRewards(msg.sender, (stackedTime - claimedRewardsMonthCount[msg.sender]) );
    ...
    function submitRequestForUnstack() external nonReentrant {
        ...
        require(unstackingRequests[msg.sender].amount == 0, "Your request is already submittied!");
        require(stackingUserDeatil[msg.sender].amountStaked > 0, "You don not have any Stacked Tokens for withdraw");
        require(getBalance() > 0, "You don't have suffient amount!"); 
        require( stackingUserDeatil[msg.sender].amountStaked   > 0, "Your stack amount is less then your withdraw amount");
        unstackingRequests[msg.sender].amount = stackingUserDeatil[msg.sender].amountStaked ;
    ...
    function approvedStackingRequest(address walletAddress) external onlyOwner{
        ...
        require(unstackingRequests[msg.sender].amount > 0, "We donot have any request against this address!");
        unStackToken(walletAddress); //unstackingRequests[walletAddress].amount, panelityPercentage[stackingUserDeatil[msg.sender].stackingPlan]
        unstackingRequests[walletAddress].status = true;
    }

    function unStackToken(address walletAddress) internal nonReentrant {
        require(unstackingRequests[walletAddress].status == true, "Your request not approved!");
        require(stackingUserDeatil[walletAddress].amountStaked > 0, "You don not have any Stacked NFT for withdraw");
        require(getBalance() > unstackingRequests[walletAddress].amount , "You don't have suffient amount!");
        require(stackingUserDeatil[walletAddress].amountStaked  >= unstackingRequests[walletAddress].amount, "Your stack amount is less then your withdraw amount");
        uint256 stackedTime = getMonth(block.timestamp , stackingUserDeatil[walletAddress].startTime); //check stacked time
        if(stackingUserDeatil[walletAddress].stackingPlan == 1 && claimedRewardsMonthCount[walletAddress] >= 6 || stackingUserDeatil[walletAddress].stackingPlan == 2 && claimedRewardsMonthCount[walletAddress] >= 12 || stackingUserDeatil[walletAddress].stackingPlan == 3 && claimedRewardsMonthCount[walletAddress] >= 24 ){
            stackingUserDeatil[walletAddress].unclaimedRewards += (stackedTime - claimedRewardsMonthCount[walletAddress] > 0) ? calculateRewards(walletAddress, (stackedTime - claimedRewardsMonthCount[walletAddress])) : 0 ;
            IERC20(gulfTokenAddress).transferFrom(address(this), walletAddress, stackingUserDeatil[walletAddress].amountStaked);
        }else{
            uint256 totalPanalityAmount = claimedRewardsTransfered[walletAddress] + calculatePercentageAmount(walletAddress, panelityPercentage[stackingUserDeatil[walletAddress].stackingPlan] );
        ...
        unstackingRequests[walletAddress].amount = 0;
        unstackingRequests[walletAddress].status = false;
    }
    ...
    function myPendingRewards()public view returns(uint256){
        require(stackingUserDeatil[msg.sender].unclaimedRewards > 0  || stackingUserDeatil[msg.sender].amountStaked > 0 , "Pending reward and stacked amount both are zero");
        uint256 stackedTime = getMonth(block.timestamp , stackingUserDeatil[msg.sender].startTime); //check stacked time
        return (calculateRewards(msg.sender, stackedTime - claimedRewardsMonthCount[msg.sender]) + stackingUserDeatil[msg.sender].unclaimedRewards);
    }
    ...
    function updateStartTime(uint256 newTime) external onlyOwner {
        ...
        stackingUserDeatil[msg.sender].startTime = newTime;
    }
}
