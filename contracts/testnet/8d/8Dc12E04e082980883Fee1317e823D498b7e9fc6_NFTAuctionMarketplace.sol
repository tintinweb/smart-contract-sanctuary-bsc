...
contract NFTAuctionMarketplace is Ownable, ReentrancyGuard {
    ...
    function createAuction(
        ...
        require(_startTime > block.timestamp, "Please set start after 5min");
        require(_endTime > _startTime, "invalid end time");

        nft.safeTransferFrom(msg.sender, address(this), _tokenId);

        auctionNfts[_nft][_tokenId] = AuctionNFT({
            id: _numCounter.current(),
            nft: _nft,
            tokenId: _tokenId,
            creator: msg.sender,
            payToken: _payToken,
    ...
    function setAcceptPayableToken(address _token) external onlyOwner {
        require(_token != address(0), "invalid token");
        payableToken[_token] = true;
    }

    function setDenicePayableToken(address _token) external onlyOwner {
        require(_token != address(0), "invalid token");
