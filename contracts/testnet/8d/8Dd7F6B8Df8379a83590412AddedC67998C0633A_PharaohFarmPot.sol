...
contract PharaohFarmPot is Ownable {
    ...
    function getMaxPayout() public view returns (uint256) {
        uint256 payout = user_data[msg.sender].maxPayout;
        return payout;
    }
    ...
    function availableRewards() public view returns (uint256) {
        ...
        uint256 time = (block.timestamp - deposit_time) / 1 days;
    ...
    function claimRewards() public {
        ...
        require(claimed + myRew <= myPayout, "Can't claim if already claimed");
        ...
        if (myRew > myPayout) {
            CAFToken.safeTransfer(
                msg.sender,
                myPayout - ((myPayout * withdrawTax + 10) / 100)
            );
            user_data[msg.sender].claimed += myPayout;
        } else {
            CAFToken.safeTransfer(
                msg.sender,
                myRew - ((myRew * withdrawTax + 10) / 100)
            );
    ...
    function doubleUp() public {
        ...
        if (doubleUpAmount - availableRewards > 0) {
