...
contract BinZaiNewToken is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    bool private swapping;
    bool public stopEnable = true;
    uint256 public openBlock;
    uint256 public kb = 9;

    uint256 public swapTokensAtAmount; 
    address public marketingWalletAddress = 0x2A5B05da93acfD6c54859833DC8a1695C1271C4D; 
    address public deadAddress = 0x000000000000000000000000000000000000dEaD; 
    address public lpRewardToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;  
    ...
    constructor() payable ERC20("BinZaiNew", "BinZaiNew")  {
        uint256 totalSupply = 10240 * (10**25);//
        swapTokensAtAmount = 5 * (10**25);//
    ...
    function open() external onlyOwner {
        require(0 == openBlock, "trading");
        openBlock = block.number;
        stopEnable = false;
    } 

    function stop(bool enable) external onlyOwner{
        stopEnable = enable;
    } 
    ...
    function _transfer(
        ...
        require(0 == openBlock, "not open");

        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            require(!stopEnable, "not open");
        }
        ...
        require(!_blackList[from] || !_blackList[to], "bot address");
        
        if (block.number < openBlock + kb) {
    ...
    function selltakeAllFee(address from,uint256 amount) private returns(uint256 amountAfter) {
        ...
        uint256 LFee = amount.mul(10).div(100);
    ...
    function _swapAndLiquid() private {
        ...
        uint256 usdtval = IERC20(lpRewardToken).balanceOf(address(this));
        if(usdtval > 0){
            uint256 yx = usdtval.div(10).mul(7);
            uint256 hl = usdtval.sub(yx);
            IERC20(lpRewardToken).transfer(address(dividendTracker), hl);
            IERC20(lpRewardToken).transfer(marketingWalletAddress, yx);
            return;
        }
        ...
        if (balances > swapTokensAtAmount) {
            ...
            _swapTokenForTokenUSDT(balances);
        }
    }

    function _swapTokenForTokenUSDT(uint256 tokenAmount) private {
