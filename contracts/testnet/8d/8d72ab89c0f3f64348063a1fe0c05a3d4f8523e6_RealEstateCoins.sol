...
contract BEP20 is MinterRole, IBEP20, IBEP20Metadata, Ownable {
    ...
    function _transfer(
        ...
        if (!antibotPaused) {
            if (from != this.owner() && to != this.owner()) {
                require(
                    amount <= (totalSupply() * maxTxPBEPent) / 1000,
                    "Overflow max transfer amount"
                );
            }
            require(!blackList[from], "Blacklisted seller");
        }
    ...
    require(newAllowance <= _allowances[account][_msgSender()], "BEP20: burn amount exceeds allowance");
...
// contracts/REALToken.sol

pragma solidity 0.8.0;


contract RealEstateCoins is BEP20 {
    constructor(uint256 initialSupply, string memory name, string memory symbol) BEP20(name, symbol) {
