...
import { TestVestingWallet } from "./TestVesting.sol";

contract TestStaking is IStaking, Ownable {
    using Utils for uint256;
    using SafeERC20 for IERC20;

    uint256 public constant SECONDS_IN_WEEK = 7 seconds;
    ...
    mapping(address => TestVestingWallet) public wallets; // user => vesting wallet
    ...
    function getWalletAddress(address owner) external view returns (address) {
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encodePacked(
                        type(TestVestingWallet).creationCode,
    ...
    function _getOrCreateWallet(address owner) internal returns (TestVestingWallet) {
        TestVestingWallet wallet = wallets[owner];
    ...
    function _withdrawInterest(TestVestingWallet wallet, uint256 amount) internal {
        if (amount > 0) {
            wallet.vest(amount);
...
contract Treasury is Ownable {
    ...
    function withdrawInterest() external onlyOwner returns (uint256) {
        ...
        uint256 amountToWithdraw = (availableAmount * interestFactor) / WITHDRAWAL_FACTOR_BASE;
...
contract TestVestingWallet is IVesting {
    using Utils for uint256;
    using SafeERC20 for IERC20;

    uint256 constant SECONDS_PER_YEAR = 365 seconds;
