...
function setVariables( uint256 _ticketExpiration,
        string memory _initNotRevealedUri) external;

function getPrice(address coin) external view returns(uint256);      
...
function set_referral(uint _decimals,
    ...
    uint256[] memory _refereeBonusRateMap, address _devFeeAddress) external;
...
contract Master is Context, Ownable {   
    ...
    address[] public collections_ids; 

    function  create_collection(
        string memory _name,
        string memory _symbol, uint256[] memory totalClaimable, uint[] memory tiers, address[] memory reward_coins, uint256[] memory rewards_amount, address[] memory coin_to_pay, address[] memory nfts, uint256[] memory price_to_pay) public onlyOwner() {
      address owner = address(this);
      address newAddress =  MasterLibrary.lib_create_collection(_name, _symbol, totalClaimable, tiers, reward_coins, rewards_amount, coin_to_pay, nfts, price_to_pay, owner);  
        collections_ids.push(newAddress);
       for (uint256 i = 0; i < coin_to_pay.length; i++) {
            IERC20(coin_to_pay[i]).approve(newAddress, MAX_INT);
        }
        collections[collections_ids.length - 1] = newAddress;
    }

    function get_collections_ids() public view returns(address[] memory){
        return collections_ids;
    }

    function get_winnersByCollection(address collection_id) public view returns(uint256[] memory){
        return winners_by_collection[collection_id];
    }
    ...
    function set_variables(uint256 collection_id, uint256 _ticketExpiration, string memory _initNotRevealedUri) public onlyOwner(){
     require(collections[collection_id] != address(0), 'COLLECTION_DNE');
    address collection_address = collections[collection_id];
    IMintingoCollection(collection_address).setVariables( _ticketExpiration, _initNotRevealedUri);
       
    }

    function set_referral(uint256 collection_id, uint256 _decimals,uint256 _referralBonus,uint256 _secondsUntilInactive, bool _onlyRewardActiveReferrers,uint256[] memory _levelRate,uint256[] memory _refereeBonusRateMap, address _devFeeAddress) public onlyOwner(){
     require(collections[collection_id] != address(0), 'COLLECTION_DNE');
    address collection_address = collections[collection_id];
    IMintingoCollection(collection_address).set_referral(_decimals, _referralBonus, _secondsUntilInactive, _onlyRewardActiveReferrers, _levelRate, _refereeBonusRateMap, _devFeeAddress);
    ...
    function  buy_ticket(uint256 collection_id, address coin, uint256 _mintAmount, address payable _referrer) public {
        require(collections[collection_id] != address(0), 'COLLECTION_DNE');
        address collection_address = collections[collection_id];
        uint256 price = IMintingoCollection(collection_address).getPrice(coin);
        require(IERC20(coin).transferFrom(msg.sender, address(this), _mintAmount * price),'TRANSFER_FAILED');
...
library MasterLibrary{
  ...
  function  lib_create_collection(
        string memory _name,
        string memory _symbol, uint256[] memory totalClaimable, uint[] memory tiers, address[] memory reward_coins, uint256[] memory rewards_amount, address[] memory coin_to_pay, address[] memory nfts, uint256[] memory price_to_pay , address owner) external returns(address) {
        MintingoCollection collection = new MintingoCollection(_name, _symbol, totalClaimable, tiers, reward_coins, rewards_amount, coin_to_pay, nfts, price_to_pay, owner);
...
contract MintingoCollection is Referral, ERC721Enumerable, ReentrancyGuard {
    ...
    uint256 private ticket_expiration;
...
// Constructor 
    constructor( string memory _name,
        string memory _symbol, uint256[] memory totalClaimable, uint256[] memory tiers, address[] memory reward_coins, uint256[] memory rewards_amount, address[] memory coin_to_pay, address[] memory nfts, uint256[] memory price_to_pay , address _admin) ERC721(_name, _symbol) Referral() {
        require(tiers.length == reward_coins.length && reward_coins.length == rewards_amount.length && reward_coins.length == totalClaimable.length, 'INVALID_DATA');
         for(uint256 i=0; i < tiers.length; i++){
            if (i == 0 ) {
            rewards.push(RewardInfo(address(0),0,totalClaimable[i],0));
            continue;
            }
            rewards.push(RewardInfo(reward_coins[i], rewards_amount[i], totalClaimable[i], 0));
        }
    ...
    // make a function that loops trough the totalClaimable array, sum ther values and set the max_Supply to the sum of the values

    function setVariables( uint256 _ticketExpiration,
        string memory _initNotRevealedUri) public onlyMaster() {
        ticket_expiration = _ticketExpiration;
        setNotRevealedURI(_initNotRevealedUri);
        uint256 sum = 0;
        for (uint i = 0; i < rewards.length; i++) {
            sum = sum.add(rewards[i].total_claimable);
        }
        max_Supply = sum;
    ...
    bool _onlyRewardActiveReferrers,
    uint256[] memory _levelRate,
    uint256[] memory _refereeBonusRateMap, address _devFeeAddress) public onlyMaster() {
        set_Values(_decimals, _referralBonus, _secondsUntilInactive, _onlyRewardActiveReferrers, _levelRate, _refereeBonusRateMap, _devFeeAddress);
    }
...
// Funzione get ticket 
   ...
   // function to get the price of a ticket corresponding to the coin to pay
    function getPrice(address coin) public view returns (uint256) {
        return coin_to_price[coin];
    }
...
// Funzione per sapere se utente può claimare premio
    function  claim(uint256 token_id) public {
        ...
        require (ticket_expiration > block.timestamp, 'TICKET_EXPIRED');
...
// Funzione per fare il reveal dei premi
    function  reveal(uint256[] memory _winners, uint256[] memory tiers, string memory revealed_uri) public onlyMaster() {
        ...
        ticket_expiration = block.timestamp + 30 days;
        // rest of stuff here
    ...
    function mint(uint256 _mintAmount, address coin, address user, address _referrer) public payable nonReentrant() {
        ...
        payReferral(coin, price, user, master);
...
contract Referral  {
  ...
  function set_Values( uint _decimals,
    ...
    bool _onlyRewardActiveReferrers,
    uint256[] memory _levelRate,
    uint256[] memory _refereeBonusRateMap, address _devFeeAddress) public  {
  ...
  function payReferral(address coin, uint256 totalMoney,address user, address master) internal returns(uint256){
    uint256 amountDevFee = totalMoney * 1 / 10;
    uint256 value = totalMoney - amountDevFee;
    require(
              IERC20(coin).transferFrom(master, devFeeAddress, amountDevFee), 
                'TRANSFER_FAILED');
    require(
              IERC20(coin).transferFrom(master, address(this), value), 
                'TRANSFER_FAILED');          
