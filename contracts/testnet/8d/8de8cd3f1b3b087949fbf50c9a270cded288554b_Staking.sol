...
contract Staking is Ownable {
    ...
    uint256 public feesMultiplier;
    mapping (address=>uint256) private time;
    uint256 public totalStake = 0;
    uint256 public mintime = 1000;// in seconds
    uint256 public minStakingAmount = 100 * 10**18;//min staking amount
    ...
    constructor(
        ...
        feesMultiplier = _feesMultiplier;
    ...
    function updatePool(uint256 _pid) private {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 rewardReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        pool.accRewardPerShare = pool.accRewardPerShare.add(rewardReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        if(user.amount == 0){
            ...
            if (fees > 0) {
                pool.lpToken.safeTransferFrom(address(msg.sender), address(feeReceiver), fees);
            }
    ...
    // Withdraw BaseToken tokens from STAKING.
    function withdraw(uint256 _amount) public {
        PoolInfo storage pool = poolInfo[0];
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount >= _amount, "withdraw: not good");
        require(time[msg.sender] <= block.timestamp, "Cant claim reward");
        updatePool(0);
        uint256 pending = user.amount.mul(pool.accRewardPerShare).div(1e18).sub(user.rewardDebt);
        if(pending > 0) {
            rewardToken.safeTransfer(address(msg.sender), pending);
