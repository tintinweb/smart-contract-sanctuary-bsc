...
contract Wallet is Ownable  {
    ...
    event Recharge( address indexed account, address  token, uint amount);
    event Withdrawal(address indexed account, address  token,uint amount);

    constructor(){

    }

    function addReserve(address _token) external onlyManager {
        isReserveToken[_token] = true;
        reserveTokens.push(_token);
    }

    function listReserve() external view returns(address[] memory) {
    ...
    function recharge(uint _amount, address _token)  external {
    ...
    function withdraw(uint _amount, address _token) external {
         require( isReserveToken[ _token ], "Not accepted" );
         require( balance[msg.sender][_token] >= _amount, "not enough");
