...
contract MebLPStake is Ownable {
    ...
    // 基础配置模块
    address public constant meb = 0xdA0B47eD306F2bF6b128e5a84389b1f270932Cb6;
    address public constant lp = 0x5EDb19b75B8c8Dc9b1651945a283EABA5bbA7aE9;
    ...
    function deposit(uint256 lockSecond, uint256 amount) external {
        ...
        for (uint256 i = 0; i < rewardTokens.length(); i++) {
            depositInfo.debts[rewardTokens.at(i)] = effect * accs[rewardTokens.at(i)];
        }
        totalEffect += effect;
        totalAmount += amount;
    }

    function withdraw(uint256 _index, address token) external {
    ...
    function pending(address _addr) external view returns (address[] memory addrs, uint256[] memory pendings) {
        addrs = new address[](rewardTokens.length());
        pendings = new uint256[](rewardTokens.length());
        DepositInfo[] storage userDepositInfosPool = userDeposits[_addr];
        for (uint256 i = 0; i < rewardTokens.length(); i++) {
            addrs[i] = rewardTokens.at(i);
            for (uint256 j = 0; j < userDepositInfosPool.length; j++) {
                pendings[i] += pending(_addr, j, addrs[i]);
            }
        }
    }

    function pending(address _addr, uint256 _index) external view returns (address[] memory addrs, uint256[] memory pendings) {
        addrs = new address[](rewardTokens.length());
        pendings = new uint256[](rewardTokens.length());
        for (uint256 i = 0; i < rewardTokens.length(); i++) {
            addrs[i] = rewardTokens.at(i);
            pendings[i] = pending(_addr, _index, addrs[i]);
        }
    }
    ...
    function pending(
        ...
        DepositInfo storage depositInfo = userDeposits[_addr][_index];
    ...
    function userAllDeposits(address _addr)
        external
        view
        returns (
    ...
    {
        DepositInfo[] storage depositInfos = userDeposits[_addr];
