...
contract SpringV2 is ERC20, Owners, ITokenForLiquidityManager {
    ...
    uint256 public sellTax = 500; // 4 decimals. 500 = 0.05 = 5%
    uint256 public liquidatePercentOfSellTax = 0; // 4 decimals. 1000 = 0.1 = 10%
    ...
    function setLiquidityManager(address _liquidityManager) external onlyOwners {
    ...
    function _transferFrom(
        ...
        if (from == liquidityPair || to == liquidityPair) {
            // interacting with LP
            if (liquidityPair == to && !whitelist[from] && from != address(this)) {
                // apply sell tax
                uint256 feeAmount = (amount * sellTax) / 1e4;
                amount -= feeAmount;
                if (feeAmount > 0) {
                    // send fee SPR to LM
                    uint256 liquidateFeeAmount = feeAmount * liquidatePercentOfSellTax / 1e4;
                    if (liquidateFeeAmount > 0) {
                        _transfer(from, address(this), liquidateFeeAmount);
                        liquidateSellFee(liquidateFeeAmount);
                    }
                    _transfer(from, address(liquidityManager), feeAmount - liquidateFeeAmount);
                }
            }
    ...
    function approveForLiquidityManger(address _liquidityPair, uint256 _amount) external {
        require(msg.sender == address(liquidityManager), "Only LM is allowed to call this");
...
interface ITokenForLiquidityManager is IERC20 {
    function approveForLiquidityManger(address liquidityPair, uint256 amount) external;
}
