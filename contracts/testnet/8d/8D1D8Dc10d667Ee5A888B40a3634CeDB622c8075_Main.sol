...
AccessControl
{
  ...
  ) public
  {
    ...
    typeTotalSupply[0] = 10000;
    typeTotalSupply[1] = 10000;
    typeTotalSupply[2] = 10000;
    typeTotalSupply[3] = 10000;
    typeTotalSupply[4] = 10000;

    typeUri[0] = "http://img.org/1.jpg";
    typeUri[1] = "http://img.org/2.jpg";
    typeUri[2] = "http://img.org/3.jpg";
    typeUri[3] = "http://img.org/4.jpg";
    typeUri[4] = "http://img.org/5.jpg";
  }
  ...
  external
  {
    ...
    require(tokens.length < maxSupply, "mint: exceeds maxSupply");
...
Ownable
{
  ...
  address public threeLpAddress;
  uint256 public price;
  uint256 public fee = 10 ether;
  uint256 public constant times = 10 ** 12;
  NFTokenMetadataEnumerableMock public nft;

  event Withdraw(address indexed user, uint256 amount);

  constructor(
    address _tbAddress,
    address _threeAddress,
    address _threeLpAddress,
    uint256 _price,
    uint256 _fee,
    NFTokenMetadataEnumerableMock _nft
  ) public
  {
    tbAddress = _tbAddress;
    threeAddress = _threeAddress;
    _threeLpAddress = _threeLpAddress;
    price = _price;
    fee = _fee;
    nft = _nft;
  }
  ...
  function buyNFT(uint256 _flag, uint256 _coin) payable public {
    require(_coin == 0 || _coin == 1, "buy: _coin not good");
    require(_flag <= 4, "buy: _flag not good");
    require(msg.value == fee, "buy: fee not good");
    uint _price = price;
    address _token = tbAddress;
    if (_coin == 1) {
      _price = price.mul(times).div(getThreePrice()).mul(uint(95)).div(uint(100));
      _token = threeAddress;
    }
    require(ERC20(_token).balanceOf(msg.sender) >= _price, "buy: balance not sufficient");
    ERC20(_token).safeTransferFrom(
      address(msg.sender),
      address(this),
      _price
    );
    nft.mint(address(msg.sender), _flag);
  }

  function buyNFTBatch(uint256 _flag, uint256 _num) payable public {
    require(_num >= 1, "buy: _num not good");
    for (uint256 i = 0; i < _num; i++) {
      buyNFT(_flag, 0);
    }
  }

  function setPrice(uint256 _price, uint256 _fee) public onlyOwner {
    price = _price;
    fee = _fee;
  }


  function getThreePrice() public returns (uint256 _price) {
    uint amountTb = ERC20(tbAddress).balanceOf(threeLpAddress);
    uint amountThree = ERC20(threeAddress).balanceOf(threeLpAddress);
    _price = amountThree.mul(times).div(amountTb);
  }
