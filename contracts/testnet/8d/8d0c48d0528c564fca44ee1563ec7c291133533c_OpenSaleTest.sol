...
/**
Token name: OpenSaleTest
Token symbol: OSFT
Total supply: 50 million 50,000,000
12% transaction tax
-4% team
-2% marketing
-3% auto liquidity
...
Marketing and team token fees will be reflected in BNB not in native token.

auto liquidity, black list function, white list dev wallet, adjustable anti whale, and bnb reflection.


*/

contract OpenSaleTest is Context, IERC20, Ownable {
    ...
    string private _name = "OpensaleTest";
    string private _symbol = "OSFT";
    ...
    address payable public teamWallet = payable(0x8d75f195FDaAB8e5cf8993E3e4C54dDd2a60870B); 
    address payable public marketingWallet = payable(0xfCb2B4B23ec50473F705Da2a6aC34a035A66fCD3); 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(_TeamFee.add(_liquidityFee).add(_marketingFee)); 

        uint256 tokensForDev = contractTokenBalance.mul(_TeamFee).div(_liquidityFee.add(_marketingFee).add(_TeamFee));
        
        uint256 tokensForMarketing = contractTokenBalance.mul(_marketingFee).div(_liquidityFee.add(_marketingFee).add(_TeamFee));
        ...
        swapTokensForWallets(tokensForDev, teamWallet);
    }
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tDev = tAmount.div(100).mul(_TeamFee);
    ...
    function setBuyFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 TeamFee) external onlyOwner() {
        _buyTaxFee = taxFee;
        _buyMarketingFee = MarketingFee;
        _buyLiquidityFee = liquidityFee;
        _buyTeamFee = TeamFee;
        require(_buyTaxFee + _buyTeamFee + _buyLiquidityFee + _buyMarketingFee <= 12, "Must keep at 12% or less");
    }

    function setSellFeePercent(uint256 taxFee, uint256 MarketingFee, uint256 liquidityFee, uint256 TeamFee) external onlyOwner() {
        _sellTaxFee = taxFee;
        _sellMarketingFee = MarketingFee;
        _sellLiquidityFee = liquidityFee;
        _sellTeamFee = TeamFee;
        require(_sellTaxFee + _sellTeamFee + _sellLiquidityFee + _sellMarketingFee<= 20, "Must keep at 20% or less");
    }
