...
contract FLASH is Context, IBEP20, Ownable 
{
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private 
    {
        ...
        uint256 stakeTokens = amount.mul(_stakeFee).div(100);
        _burn(sender, stakeTokens);

        uint256 marketingTokens = amount.mul(_marketingFee).div(100);
        amount = amount.sub(marketingTokens).sub(stakeTokens);
    ...
    function setSaleFee() private 
    {
        ...
        _stakeFee = _saleStakeFee;
    }
    

    event updateBuyFee(uint256 totalFee, uint256 timestamp);
    function setAllBuyFeePercentages(uint256 liquidityFee, uint256 marketingFee, uint256 stakeFee) 
    external onlyOwner()
    {
        ...
        uint256 totalFee = _liquidityFee.add(_marketingFee).add(_stakeFee);
    ...
    function setAllSaleFeePercentages(uint256 liquidityFee, uint256 marketingFee, uint256 stakeFee) 
    external onlyOwner()
    {
        _saleLiquidityFee = liquidityFee;
        _saleMarketingFee = marketingFee;
        _saleStakeFee = stakeFee;

        uint256 totalFee = liquidityFee.add(marketingFee).add(_saleStakeFee);
        require(totalFee<=13, "Too High Fee");
   ...
   // ---------- STAKES ----------
    ...
    uint256 public _unstakingFee = 8;
    ...
    {
        packages[90*uintTime] = 10;     
        packages[180*uintTime] = 25;   
        packages[360*uintTime] = 50;  
    }

    function setStakingAndUnstakingFees(uint256 stakingFee, uint256 unstakingFee) external onlyOwner
    {
        _stakingFee = stakingFee;
        _unstakingFee = unstakingFee;
        require(_stakingFee<8, "Too High Fee");
        require(_unstakingFee<15, "Too High Fee");
    }
    ...
    function createStake(uint256 _stake, uint256 _stakingPackage) external
    {
        ...
        uint256 stakingFeeTokens = _stake.mul(_stakingFee).div(100);
    ...
    {
        require(packages[_stakingPackage]>0, "Wrong Package");
        require(_stake<=balanceOf(account), "Insufficient Balance");
        require(stakes[account] == 0, "Already Staked");
        require(_stake>0, "Too Less Stake");
        return true;
    }
    ...
    function removeStake(uint256 _stake) public
    {
        ...
        uint256 unstakingFeeTokens = _stake.mul(_unstakingFee).div(100);
        _stake = _stake.sub(unstakingFeeTokens);        
    ...
    function _withdrawReward(address _stakeholder)  internal 
    {
        ...
        swapAndSendToFee(reward, _stakeholder);
    ...
    function withdrawReward() external 
    {
        ...
        require(reward>0, "No reward is available.");
    ...
    uint256 private busdRewardDistributed = 0;

    function getBusdRewardDistributed() external view returns (uint256)
    {
        return busdRewardDistributed;
    }
    ...
    function swapAndSendToFee(uint256 tokens, address account) private  
    {
        ...
        IBEP20(BUSD).transfer(account, newBalance);
        busdRewardDistributed = busdRewardDistributed.add(newBalance);
    }
