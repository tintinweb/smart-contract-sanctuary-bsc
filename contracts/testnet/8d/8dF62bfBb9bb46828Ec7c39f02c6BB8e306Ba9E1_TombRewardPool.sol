...
contract TombRewardPool {
   ...
   uint256[] public epochTotalRewards = [5000 ether, 5000 ether];
   ...
   constructor(
      ...
      _runningTime = _runningTime.div(2);
      epochEndTimes[0] = poolStartTime + _runningTime; // + 4 days; // Day 2-5
      epochEndTimes[1] = epochEndTimes[0] + _runningTime; // + 5 days; // Day 6-10

      epochTombPerSecond[0] = epochTotalRewards[0].div(_runningTime); // 4 days
      epochTombPerSecond[1] = epochTotalRewards[1].div(_runningTime); // 5 days
   ...
   function initialize(
      ...
      poolStartTime = _poolStartTime;
      _runningTime = _runningTime.div(2);
      epochEndTimes[0] = poolStartTime + _runningTime;
      epochEndTimes[1] = epochEndTimes[0] + _runningTime;
      epochTombPerSecond[0] = epochTotalRewards[0].div(_runningTime);
      epochTombPerSecond[1] = epochTotalRewards[1].div(_runningTime);
      epochTombPerSecond[2] = 0;
   }
         ...
         if (_toTime >= epochEndTimes[epochId - 1]) {
            ...
            for (epochId = epochId - 1; epochId >= 1; --epochId) {
               ...
               _generatedReward = _generatedReward.add(
                  epochEndTimes[epochId].sub(epochEndTimes[epochId - 1]).mul(epochTombPerSecond[epochId])
               );
            }
            return _generatedReward.add(epochEndTimes[0].sub(_fromTime).mul(epochTombPerSecond[0]));
         }
   ...
   function withdraw(uint256 _pid, uint256 _amount) public {
      ...
      if (_amount > 0) {
         ...
         uint256 hv = _amount.mul(99).div(100);
         pool.token.safeTransfer(_sender, hv);
         pool.token.safeTransfer(operator, _amount.sub(hv));
      }
      user.rewardDebt = user.amount.mul(pool.accTombPerShare).div(1e18);
