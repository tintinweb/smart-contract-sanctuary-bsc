...
contract Moments is ERC20, Ownable {
    ...
    function multiSend(address[] calldata addrs, uint256[] calldata amounts) public {
        ...
        for (uint256 i = 0; i < addrs.length; i++) {
            super._transfer(msg.sender, addrs[i], amounts[i]*(1 ether));
        }
    }

    function multiSendSame(address[] calldata addrs, uint256 amount) public {
        require(!_isBlacklisted[msg.sender], "Blacklisted address");
        for (uint256 i = 0; i < addrs.length; i++) {
            super._transfer(msg.sender, addrs[i], amount*(1 ether));
        }
    }
