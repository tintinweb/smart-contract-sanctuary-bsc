...
contract WorkFarm is Ownable, ReentrancyGuard {
    ...
    uint256 public maxSupply = 10000000000e18;
    uint256 public rewardPerBlock = 1e17; // reward tokens created per block
    ...
    function add(address want, uint256 allocPoint, bool isLp) public onlyOwner {
    ...
    function setAllocPoint(uint256 allocPoint) public onlyOwner {
        updatePool();
        poolInfo.allocPoint = allocPoint;
    }
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (IERC20(rewardTokenAddress).totalSupply() >= maxSupply) {
            return 0;
        }
