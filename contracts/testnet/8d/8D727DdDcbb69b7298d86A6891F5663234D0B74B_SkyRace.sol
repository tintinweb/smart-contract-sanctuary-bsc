...
contract SkyRace is ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("SkyRace", "SkyRace") {
        ...
        threeLevelTax = 10;
        maxRoomTax = 60;
        QuizURI = "https://web-nft-dapp-test.bljcoco.com/dapp/prod/NFT?tokenId=";
    }
    ...
    function serTopAddress(address _address, bool _bool) public onlyOwner {
    ...
    function createQuizProject (uint256 _quizID, uint256 _beginTime, uint256 _endTime) public {
    ...
    function editQuizProject (uint256 _quizID, uint256 _beginTime, uint256 _endTime, uint256 _result) public {
        require(operators[msg.sender], "Operators only");
    ...
    function createQuizRoom (uint256 _roomId, uint256 _quizID, uint256 _nftID, uint256 _oddsType, uint256 _maxRoomNumber, uint256 _roomTax, uint256 _minUserJoinAmount, uint256 _resultOneFixedOdds, uint256 _resultTwoFixedOdds) public {
        ...
        require(block.timestamp >= quizList[_quizID].beginTime && block.timestamp < quizList[_quizID].endTime, "Out of time");
    ...
    function userJoin (uint256 _roomId, uint256 _amount, uint256 _result) public {
        ...
        require(block.timestamp >= quizList[_quizID].beginTime && block.timestamp < quizList[_quizID].endTime, "Abnormal state");
        ...
        if (roomList[_roomId].oddsType == 2) {
            if (_result == 1) {
                ...
                require((_oneEexpected / _twoEexpected) < (roomList[_roomId].resultOneFixedOdds / roomList[_roomId].resultTwoFixedOdds), "The amount exceeded the odds limit [1]");
            }
            else if (_result == 2) {
                ...
                require((_twoEexpected / _oneEexpected) < (roomList[_roomId].resultTwoFixedOdds / roomList[_roomId].resultOneFixedOdds), "The amount exceeded the odds limit [2]");
            }
        }
    ...
    function userWithdrawal (address _address, uint256[] memory _indexList) public {
        ...
        for (uint256 _index = 0; _index < _indexList.length; _index = _index + 1) {
            ...
            if (roomList[_roomID].resultOneTotal == 0 || roomList[_roomID].resultTwoTotal == 0) {
                ...
                if (quizList[_quizID].result == 1) {
                    _odds = roomList[_roomID].resultTwoTotal / roomList[_roomID].resultOneTotal;
                }
                if (quizList[_quizID].result == 2) {
                    _odds = roomList[_roomID].resultOneTotal / roomList[_roomID].resultTwoTotal;
                }

                uint256 _winAmount = _amount * _odds;
