...
contract FarmerDoge is Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    FarmerDogeDividendTrackerFactory private _trackerFactory;
    ...
    function initialize(FarmerDogeDividendTrackerFactory trackerFactory) public initializer {
       ...
       // liquidityWallet = 0xb914fF525be446A21751343193b7366DEE3BC253;
        ...
        minimumTokensBeforeSwap = 1000000000000000;
        ...
        // MAINNET IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        ...
        dividendTracker = _trackerFactory.createDividendTracker(
            address(this),
            msg.sender,
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        FarmerDogeDividendTracker newDividendTracker = _trackerFactory.createDividendTracker(address(this), msg.sender, newAddress, uniswapV2Router);
        _isExcludedFromFee[address(newDividendTracker)] = true;
    ...
    function setMaxTransactionAmount(uint256 newValue) public onlyOwner {
        require((newValue >= (totalSupply().div(100))), "Error: max tx lower than 1%");
        emit MaxTransactionAmountChange(newValue, maxTxAmount);
        maxTxAmount = newValue;
    }

    function setMaxWalletAmount(uint256 newValue) public onlyOwner {
        require((newValue >= (totalSupply().div(100))), "Error: max wallet lower than 1%");
...
contract FarmerDogeDividendTrackerFactory {
    function createDividendTracker(address ca, address owner, address rewardToken, IUniswapV2Router02 uniswapV2Router) public returns (FarmerDogeDividendTracker) {
        FarmerDogeDividendTracker dividendTracker = new FarmerDogeDividendTracker(ca);

        dividendTracker.excludeFromDividends(address(dividendTracker), true);
        dividendTracker.excludeFromDividends(ca, true);
        ...
        dividendTracker.setTokenAdminRole(owner);
        dividendTracker.setUniswapRouter(uniswapV2Router);
...
contract FarmerDogeDividendTracker is ERC20, ERC20Burnable, AccessControl, IDividendPayingToken {
    ...
    constructor(address ca) ERC20("FarmerDoge_Dividend_Tracker", "FarmerDoge_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1 * (10 ** 18);
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(TOKEN_ADMIN_ROLE, msg.sender);
        _grantRole(TOKEN_ADMIN_ROLE, ca);
    }
    receive() external payable {
        distributeDividends();
    }
    ...
    function setRewardToken(address token) external onlyRole(TOKEN_ADMIN_ROLE) {
        _setRewardToken(token);
    }

    function setUniswapRouter(IUniswapV2Router02 router) external onlyRole(TOKEN_ADMIN_ROLE) {
    ...
    function excludeFromDividends(address account, bool value) external onlyRole(TOKEN_ADMIN_ROLE) {
    ...
    function process(uint256 gas) public onlyRole(TOKEN_ADMIN_ROLE) returns (uint256, uint256, uint256) {
    ...
    function _setRewardToken(address token) internal onlyRole(TOKEN_ADMIN_ROLE) {
        rewardToken = token;
    }

    function _setUniswapRouter(IUniswapV2Router02 router) internal onlyRole(TOKEN_ADMIN_ROLE) {
