...
contract NFTClaim is Ownable, ReentrancyGuard {
    ...
    event SetNftBalance(address _token, address _user, uint256 _num, uint256 _at);
    ...
    function setNftBalance(address _token, address _user, uint256 _num) external nonReentrant onlyAdmin {
        require(_token != address(0), "NONEMPTY_TOKEN");
        require(checkNftIsAllowed(_token), "NFT_NOT_ALLOWED");
        require(_user != address(0), "NONEMPTY_ADDRESS");

        nftBalanceList[_token][_user] = _num;

        emit SetNftBalance(_token, _user, _num, block.timestamp);
    }
