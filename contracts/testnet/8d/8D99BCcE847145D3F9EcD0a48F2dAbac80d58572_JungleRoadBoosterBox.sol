...
import "./BoosterVRF.sol";

/**
 * @title JungleRoadBoosterBox
 * JungleRoadBoosterBox - a randomized and openable lootbox of Booster
 * NFT.
 */
contract JungleRoadBoosterBox is ERC1155Tradable, ReentrancyGuard {
    using BoosterVRF for BoosterVRF.VRFState;

    BoosterVRF.VRFState state;

    constructor() ERC1155Tradable("Jungle Road Booster Box", "JGRD-BB", "") {}
    ...
    function setState(
        ...
        BoosterVRF.initState(state, _factoryAddress, _numOptions, _numClasses, _seed);
    }

    function setTokenIdsForClass(uint256 _classId, uint256[] memory _tokenIds) public onlyRole(DEFAULT_ADMIN_ROLE) {
        BoosterVRF.setTokenIdsForClass(state, _classId, _tokenIds);
    }
    ...
    function setOptionSettings(
        ...
        BoosterVRF.setOptionSettings(state, _option, _maxQuantityPerOpen, _classProbabilities);
    }

    function setSeed(uint256 _seed) public onlyRole(DEFAULT_ADMIN_ROLE) {
        BoosterVRF.setSeed(state, _seed);
    }
    ...
    function unbox(uint256 _optionId, address _toAddress) external {
        ...
        BoosterVRF._mint(state, _optionId, _toAddress);
    }
...
/*
  ...
  - We also cannot call onlyOwner or whenNotPaused. Users of this library should
    ...
    JungleRoadBoosterFactory.
 */

abstract contract Factory {
    function mintBooster(uint256 _optionId, address _toAddress) external virtual;
...
library BoosterVRF {
    ...
    function _sendTokenWithClass(
        ...
        factory.mintBooster(tokenId, _toAddress);
        return tokenId;
    }
