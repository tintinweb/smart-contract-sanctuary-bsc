...
/**
 This contract was created by 0r61. 
 https://github.com/0R61
*/
...
contract BattleX is Context, IBEP20, Ownable {
  ...
  address public devWallet=0x1B03aF2a493641402B9a934aE09810d1e545b706;

  address public oracleWallet=0x5889d48d6ec9Db8abc30628D80C247395676ee88;

  uint256 public pool=0;

  uint256 public claimInterval=86400; //24 hours 86400
...
*/
  ...
  function calculatePoolReward(address user) public view returns(uint256){
    uint256 thisReward=(_balances[user]*pool)/_totalSupply;
    return thisReward;
  }
  ...
  function maxTx() public view returns(uint256){
    return _totalSupply-((_totalSupply/100)*maxTxPercentage);
  }

  function maxWallet() public view returns(uint256){
    return _totalSupply-((_totalSupply/100)*maxWalletPercentage);
  }

  function calculateAmountPostTax(uint256 amountIn) public view returns(uint256){
    uint256 thisLiqTax=(amountIn/100)*liqTax;
    uint256 thisPoolTax=(amountIn/100)*poolTax;
    uint256 thisDevTax=(amountIn/100)*devTax;
  ...
  function startPVP(address user) public {
    ...
    require(inBattle[user]==true,"Requested user not in battle");
        require(_balances[user]<(_balances[msg.sender]-(_balances[msg.sender]/divisor)),"You have to high balance to battle this user.");
        require(_balances[msg.sender]<(_balances[user]-(_balances[user]/divisor)),"You have to small balance to battle this user.");
    inBattle[msg.sender]=true;
    remove(index[user]);
    uint256 batteVal=reward+battleReward[user];
        nextClaim[msg.sender]=block.timestamp+claimInterval;
        nextClaim[user]=block.timestamp+claimInterval;

    emit BigBattle(msg.sender,_balances[msg.sender],_balances[user],user,batteVal,keccak256(abi.encodePacked(msg.sender,user,block.timestamp)));
    }
...
//CONTRACT SPECIFIC FUNCTIONS END
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    require(amount<=currentMaxTxAmt,"BattleX: Transfer amount higher than allowed for this transaction");
    require(_balances[recipient]<=currentMaxWalletAmt,"BattleX: Wallet cannot hold more tokens at the moment.");
