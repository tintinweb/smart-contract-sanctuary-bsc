...
library SafeMath {
 ...
 contract hl is IERC20, Ownable {
    ...
    constructor (
        ...
        //改钱包
        MarketingAddress = 0x8fDbCfB00d97B5CeDe2110f09845f31c631C431B;
        ...
        _totalBuyFee = _buyMarketingFee.add(_buyLiquidityFee);
        _totalSellFee =_sellMarketingFee.add(_sellLiquidityFee);
    ...
    function _checkLiquidityAdd(address from, address to) private {
        ...
        //require(_liquidityHolders[from], "NO _liquidityHolders[from]");
        //require(_swapPairList[to], "NO _swapPairList[to]");
        //_liquidityHolders[from] &&
        if (_swapPairList[to]) {
            _hasLiqBeenAdded = true;
            startTradeBlock = block.number;//赋值当前区块号
        }
    }
    function _transfer(
        address from,
   ...
   function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        //计算营销费用并转账
        uint256 MarketingAmount = tAmount.mul(_buyMarketingFee).div(_totalBuyFee);
        _transfer(address(this), MarketingAddress, MarketingAmount);

       /* uint256 LiquidityAmount = tAmount.mul(_buyLiquidityFee).div(_totalBuyFee);

        //计算流动性一半的代币
        uint256 tokensForLP = LiquidityAmount.div(2);
        //将剩下的一半代币换成bnb
        uint256 tokensForSwap = LiquidityAmount.sub(tokensForLP);
        swapTokenForETH(tokensForSwap);
        //获取当前合约的bnb余额
        uint256 blanceETH = address(this).balance;
      
        addLiquidity(tokensForLP, blanceETH);*/
    }
