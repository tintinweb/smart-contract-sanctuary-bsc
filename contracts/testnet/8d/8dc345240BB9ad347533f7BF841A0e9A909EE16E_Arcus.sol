...
contract Arcus is Initializable, AccessControlUpgradeable {
    ...
    function _oncePerBlock(address user) internal {
        require(lastBlockNumberCalled[user] < block.number, "E1003");
    ...
    function _mintNft(
        ...
        if (_index == 1) {
            arcusWarrior.mintN(_minter, _edition, _amount);
        } else if (_index == 2) {
            arcusEther.mintN(_minter, _edition, _amount);
        } else if (_index == 3) {
            arcusWeapon.mintN(_minter, _edition, _amount);
        }
    }
    ...
    function mintNft(
        ...
        uint256 _index,
        uint256 _amount
    ) public oncePerBlock(msg.sender) {
        ...
        _mintNft(msg.sender, roundInfo.edition, _index, _amount);
    ...
    ERC721URIStorageUpgradeable
{
    ...
    mapping(uint256 => mapping(uint256 => uint256)) tokenVars;
    ...
    function initialize(string memory baseURI_) public initializer {
        __ERC721_init("Arcus Ether", "ARCETHER");
        __Pausable_init();
        __AccessControl_init();
    ...
    function mintN(
        ...
        for (uint256 i = 0; i < _amount; i++) mint(_minter, _edition);
    }

    function mint(address _minter, uint256 _edition)
        public
        gameMasterOnly
        whenNotPaused
    {
        ...
        emit EtherMinted(tokenId, _minter, _edition, true, uint64(block.timestamp));
    }
    ...
    function mintFromPack(
        ...
        emit EtherMinted(tokenId, _minter, _edition, (_tradeable == 1), uint64(block.timestamp));
        return tokenId;
    }
    ...
    function getEtherEdition(uint256 tokenId) public view returns (uint256) {
        return tokenVars[tokenId][EDITON];
    }

    function getUserEthers(address user)
        public
        view
        returns (uint256[] memory)
    {
    ...
    ERC721URIStorageUpgradeable
{
        ...
        whenNotPaused
    {
        ...
        emit WarriorMinted(tokenId, _minter, _edition, true, uint64(block.timestamp));
    }
    ...
    function mintFromPack(
        ...
        emit WarriorMinted(tokenId, _minter, _edition, (_tradeable == 1), uint64(block.timestamp));
        return tokenId;
    }
    ...
    function getWarriorEdition(uint256 tokenId) public view returns (uint256) {
        return tokenVars[tokenId][EDITON];
    }

    function getUserWarriors(address user)
        public
        view
        returns (uint256[] memory)
    {
    ...
    ERC721URIStorageUpgradeable
{
    ...
    function initialize(string memory baseURI_) public initializer {
        __ERC721_init("Arcus Weapon", "ARCWEAPON");
        __Pausable_init();
        __AccessControl_init();
        __ERC721Burnable_init();
        ...
        whenNotPaused
    {
        ...
        emit WeaponMinted(tokenId, _minter, _edition, true, uint64(block.timestamp));
    }
    ...
    function mintFromPack(
        ...
        emit WeaponMinted(tokenId, _minter, _edition, (_tradeable == 1), uint64(block.timestamp));
        return tokenId;
    }
    ...
    function getWeaponEdition(uint256 tokenId) public view returns (uint256) {
        return tokenVars[tokenId][EDITON];
    }

    function getUserWeapons(address user)
        public
        view
        returns (uint256[] memory)
    {
...
contract RoundManager is Initializable, AccessControlUpgradeable {
    ...
    function _updateRound(
        ...
        if (roundInfo.startDate != _startDate) {
            require(_startDate > 0, "E3004");
            rounds[_round].startDate = _startDate;
        }
        if (roundInfo.tokenId != _tokenId) {
            rounds[_round].tokenId = _tokenId;
        }
        if (roundInfo.edition != _edition) {
            require(_edition > 0, "E3005");
            rounds[_round].edition = _edition;
        }
        if (roundInfo.limit != _limit) {
            rounds[_round].limit = _limit;
        }
        if (roundInfo.userLimit != _userLimit) {
            rounds[_round].userLimit = _userLimit;
        }
        if (roundInfo.paymentToken != _paymentToken) {
            rounds[_round].paymentToken = _paymentToken;
        }
        ...
        if (roundInfo.whitelistOnly != _whitelistOnly) {
            rounds[_round].whitelistOnly = _whitelistOnly;
        }
    }
