...
/*
 * EverHODL is a new generation of APY protocols which was carefully crafted for a sustainable experience to all holders.
 *
 * EverHODL (eHODL) is a cryptocurrency launched in Q4 2022 built on BNB Smart Chain aiming to create the first decentralized marketplace for tokenized and NFT assets enabling exchanges 
 * between peers, and eliminating the risks from price slippage during trades.
 *
 * EverHODL (eHODL) is an innovative form of funds that serves as a store of value and a payment method for services and products which will be added into the ecosystem gradually.
 *
 * Website : https://www.everhodl.com
 * Twitter : https://twitter.com/EverHODLCoin
 * Telegram : https://t.me/EverHODLCoin
 */
...
contract TESTDividendDistributor is IDividendDistributor {
...
contract EverTEST is ERC20Detailed, Ownable {
    ...
    uint256 public USDTDividendFee = 25;
    uint256 public liquidityFee = 70;
    uint256 public totalFee = treasuryFee.add(USDTDividendFee).add(liquidityFee);
    ...
    TESTDividendDistributor distributor;
    ...
    //events
    event LogRebase(uint256 indexed epoch, uint256 _totalSupply, uint256 _maxHold);
    event FeesChanged(uint256 treasuryFee, uint256 USDTDividendFee, uint256 liquidityFee);
    ...
    constructor() ERC20Detailed("EverTEST Coin", "eTEST", uint8(DECIMALS)) Ownable() {
        ...
        liquidityReceiver = 0x1ED0d8bDe02089953C6C9FEEA600A0AAe9240137;

         _allowedFragments[address(this)][address(router)] = type(uint128).max;
     
        pairContract = IPancakeSwapPair(pair);

        distributor = new TESTDividendDistributor(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        ...
        isDividendExempt[liquidityReceiver] = true;
        ...
        isMaxHoldExempt[liquidityReceiver] = true;
    ...
    function rebase() internal {
        ...
        if (deltaTimeFromInit < (60 days)) {
            rebaseRate = 2084;
        } else if (deltaTimeFromInit >= (60 days)) {
            rebaseRate = 520;
        }
    ...
    function _transferFrom(
        ...
        require((_gonBalances[recipient].div(_gonsPerFragment) + amount) <= _maxHold, "Wallet cannot hold more than 2% from total supply");
        }
    ...
    function takeFee(
        ...
        uint256 _USDTDividendFee = USDTDividendFee;

        if (recipient == pair) {
            _totalFee = totalFee;
            _USDTDividendFee = USDTDividendFee;
        }

        uint256 feeAmount = gonAmount.div(feeDenominator).mul(_totalFee);
       
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(treasuryFee.add(USDTDividendFee))
        );
 ...
 function swapBack() internal swapping {
        ...
        uint256 _currentFee = treasuryFee.add(liquidityFee).add(USDTDividendFee);
        ...
        try distributor.deposit {
            value: currentBalance.mul(USDTDividendFee).div(treasuryFee.add(liquidityFee).add(USDTDividendFee))
        } () {} catch {}      
    ...
    //setters
    function setAPY(bool _flag) external onlyOwner {
        if (_flag) {
            _autoRebase = _flag;
            _lastRebasedTime = block.timestamp;
        } else {
    ...
    function setNormalFeesMode() external onlyOwner {
        ...
        USDTDividendFee = 40;
        liquidityFee = 20;
        totalFee = treasuryFee.add(liquidityFee).add(USDTDividendFee);
        
    }

    function setLowFeesMode() external onlyOwner {
        treasuryFee = 20;
        USDTDividendFee = 15;
        liquidityFee = 15;
        totalFee = treasuryFee.add(liquidityFee).add(USDTDividendFee);
    ...
    function setFeeReceivers(
        ...
        emit FeeReceiversChanged(_treasuryReceiver, _liquidityReceiver);      
    }
    ...
    function setMaxHoldPercentage(uint256 percent) external onlyOwner {
        _maxHold = _totalSupply / 200 * percent;  // *minimum of 2% max hold*
        require(percent >= 1, "Must keep at 4% or more"); // 1% is 0.5%.
    }    
