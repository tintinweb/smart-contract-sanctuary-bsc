...
contract EXPLiquify is ILiquify {
    ...
    function buyAndLiquify() external override onlyToken {
    ...
    function addLiquidity(uint256 amountWBNBtoAdd) internal {
        uint256 amountTokenToAdd = IERC20(_token).balanceOf(address(this));
        // BEGIN PHONE CALL CHANGES

        //IWETH.withdraw(amountWBNBtoAdd);
        //IERC20(_token).approve(address(pyeSwapRouter), amountTokenToAdd);

        // pyeSwapRouter.addLiquidityETH(
        //     _token,
        //     amountTokenToAdd,
        //     0, // Slippage is unavoidable
        //     0, // Slippage is unavoidable
...
// File: contracts/EXPPYESwap_Base.sol
...
contract PYESwap_Base is IPYE, Context, Ownable {
    ...
    string constant _name = "EXPPYESwap_Base";
    ...
    EXPLiquify expliquify;
    address public expliquifyAddress;
    ...
    constructor(address _marketing, uint256 _marketingFee, uint256 _liquidityFee) {
        ...
        expliquify = new EXPLiquify();
        expliquifyAddress = address(expliquify);
        ...
        _isExcludedFromFee[expliquifyAddress] = true;
    ...
    function shouldBuyAndLiquify() internal view returns (bool) {
        ...
        && !inSwap
        && swapEnabled
        && IERC20(WBNB).balanceOf(expliquifyAddress) >= swapThreshold;
    ...
    function _takeFees(FeeValues memory values) private {
        ...
        _takeFee(values.liquidity, expliquifyAddress);
    }
    ...
    function depositLPFee(uint256 amount, address token) public onlyExchange {
        ...
        if(tokenIndex < pairsLength) {
            ...
            if(allowanceT >= amount) {
                ...
                IERC20(token).transfer(expliquifyAddress, liquidityFeeAmount);
            }
        }
    }
    ...
    function addPair(address _pair, address _token) public {
        ...
        require(msg.sender == factory || msg.sender == address(pyeSwapRouter) || msg.sender == admin || msg.sender == address(this), "THE PROBLEM IS ADD PAIR");
    ...
    function getAddresses() public view returns (address, address, address) {
        address factory = pyeSwapRouter.factory();
        return (factory, address(this), address(pyeSwapRouter));
    }
