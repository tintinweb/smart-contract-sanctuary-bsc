...
contract Alien is ERC721URIStorage, Ownable {
    ...
    uint256 public price = 20000000000000000; //0.02 ether
    ...
    function mint(uint256 quantity) payable external {
        ...
        if(allowListEnabled) {
            ...
            _allowList[msg.sender] = maxPerWallet;
        }

        require(maxSupply >= _tokenIds.current() + quantity, "Max supply reached!");
        require(_allowList[msg.sender] >= _minters[to] + quantity, "Reached max count of mint");

        uint256 total = price * quantity;
        require(total <= msg.value, "Ether value sent is not correct");
        ...
        while ( i < quantity ) {
            ...
            super._setTokenURI(_tokenId, string(abi.encodePacked(baseURI, '/', _tokenId.toString(), '.json')));
    ...
    function multipleWhitelist(address[] calldata _addresses) public onlyOwner {
        uint i = 0;
        while ( i < _addresses.length) {
            _allowList[_addresses[i]] = maxPerWallet;
            i++;
        }
    }
    ...
    function deleteAddressFromAllowlist(address _address) public onlyOwner {
    ...
    function setAllowListEnabled(bool _status) public onlyOwner {
        allowListEnabled = _status;
    }
