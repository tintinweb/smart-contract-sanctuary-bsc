...
contract OPTXLMS is OwnableUpgradeable {
    ...
    function getLMSAvailable() external view returns(uint256, uint256, uint256, uint256, uint256){
        ...
        if(optxRes > optxResBuy) return (nowPrice, optxRes - optxResBuy, 0, optxRes, bnbRes);
        if(optxRes < optxResSell) return (nowPrice, 0, optxResSell - optxRes, optxRes, bnbRes);
