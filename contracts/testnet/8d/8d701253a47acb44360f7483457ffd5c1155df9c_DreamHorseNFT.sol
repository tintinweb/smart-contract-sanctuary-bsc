...
contract DreamHorseNFT is ERC721URIStorage, Ownable {
    ...
    uint256[6] public imageNumber;//NFT图片编号
    ...
    uint256 public recommendRewardRate = 8;
    ...
    //设置预售结束时间
    function setRecommendRewardRate(uint256 rate) public onlyOwner {
        recommendRewardRate = rate;
    }
    ...
    function openBlindBox(uint256 boxId) public returns (uint256){
        ...
        if(recommender != address(0)){
            ...
            uint256 reward = price * recommendRewardRate / 100;

            price = price - reward;

            ERC20(USDT).transferFrom(msg.sender, recommender, reward);

            recommendReward[recommender] += reward;
        }
