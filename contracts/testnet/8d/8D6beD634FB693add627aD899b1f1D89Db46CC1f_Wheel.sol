...
contract Adventure is Ownable {
  ...
  mapping(uint256 => uint256) public usedCounts;

  function increaseUsedCounts(uint256 petId) internal {
    usedCounts[petId] = usedCounts[petId] + 1;
  }

  function getUsedCount(uint256 petId) public view returns(uint256) {
    return usedCounts[petId];
  }
  ...
  uint256 public usedCountLimit = 0;

  function setUsedCountLimit(uint256 _usedCountLimit) public onlyOwner {
    usedCountLimit = _usedCountLimit;
  }
  ...
  function ensurePetIsFightable(AdventureTypes.Pet memory pet) public virtual view {
    ...
    require(usedCountLimit == 0 || usedCountLimit > getUsedCount(pet.id), "USED_COUNT_LIMITED");
  }
  ...
  function getPrize(AdventureTypes.Monster memory monster) public view returns(uint256) {
    ...
    return monster.power * getRateByFightCount(fightCount) * PRIZE_RATE / 10000 / 100000;
  }
  ...
  */
  ...
  uint256 public WITHDRAW_PERCENTAGE_EACH_TIME = 100;
...
contract Wheel is IWheel, Ownable {
  uint256 count = 0;
  
    
  QuickPair[] public pairs;
  Blacklister public blacklister;
  ...
  function setPairs(QuickPair[] memory _pairs) public onlyOwner {
    pairs = _pairs;
  }
  ...
  function getRandom(uint256 a, uint256 b, uint256 c) public returns(uint256) {
    return uint256(keccak256(abi.encodePacked(count++, block.timestamp, tx.origin, a, b, c, getOutsideValue())));
  }

  uint256 public POINT_FACTOR = 714;

  function setPointFactor(uint256 newPointFactor) public onlyOwner {
    POINT_FACTOR = newPointFactor;
  }

  function getPetPower(AdventureTypes.Pet memory pet, AdventureTypes.Monster memory monster, uint256 exp, uint256 price) public virtual view returns(uint256) {
    uint256 basePoint = price + price * (pet.hp + pet.mp + pet.st + pet.ag + pet.it) / POINT_FACTOR;
    uint256 pointWithMonster = pet.cl == monster.cl ? basePoint * 115 / 100 : basePoint * 85 / 100;
    return pointWithMonster + exp / 10;
  }

  uint256 public MIN_WIN_RATE = 20;
  uint256 public MAX_WIN_RATE = 100;

  function setWinRates(uint256 minRate, uint256 maxRate) public onlyOwner {
    MIN_WIN_RATE = minRate;
    MAX_WIN_RATE = maxRate;
  }

  function isWin(AdventureTypes.Pet memory pet, AdventureTypes.Monster memory monster, uint256 exp, IPetNft petNft) public override returns(bool) {
    blacklister.ensurePetNotInBlacklist(pet.id);
    uint256 price = petNft.PET_PRICES(pet.level);
    uint256 petPower = getPetPower(pet, monster, exp, price);
    if (petPower >= monster.power) return true;
    uint256 winRate = petPower * 100 / monster.power;
    if (winRate < MIN_WIN_RATE) return false;
    uint256 random = getRandom(pet.id, monster.id, petPower) % 101;
    uint256 normalizedRandom = random < MAX_WIN_RATE ? random : MAX_WIN_RATE;
    return normalizedRandom <= winRate;
  }
}