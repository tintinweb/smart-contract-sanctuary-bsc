...
contract TestToken is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "TSTTKN";
    ...
    constructor () {
        _DevelopmentWallet = payable(0x85BE49cbC4E3a5173eA9C938bC4EC558B73b3253);
        _LiquidityWallet = payable(address(0xdead));
        _BuyBackWallet = payable(0xC0e92af07565095da1a71328844cf63D1Babd91f);
        ...
        emit Transfer(address(0x71C31C2e23FBA2F8217A596916e5df2021760BC5), _msgSender(), _tTotal);
    }
    ...
    function swapBack() private {
        ...
        uint256 ethForLiquidity = ethBalance - ethForBuyBack - ethForDevelopment;
    ...
    function openTrading() external onlyOwner() {
        ...
        _maxBuyAmount = 1e10 * 10**12;
        _maxSellAmount = 1e10 * 10**12;
        _maxWalletAmount = 1e10 * 10**12;
        swapTokensAtAmount = 5e8 * 10**12;
        tradingOpen = true;
    ...
    function setSwapTokensAtAmount(uint256 newAmount) public onlyOwner {
        require(newAmount >= 1e7 * 10**12, "Swap amount cannot be lower than 0.001% of total supply.");
        require(newAmount <= 1e10 * 10**12, "Swap amount cannot be higher than 1% of total supply.");
    ...
    function setBuyFee(uint256 buyDevelopmentFee, uint256 buyLiquidityFee, uint256 buyBuyBackFee) external onlyOwner {
        require(buyDevelopmentFee + buyLiquidityFee + buyBuyBackFee <= 12, "Must keep buy taxes below 12%");
        _buyDevelopmentFee = buyDevelopmentFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyBuyBackFee = buyBuyBackFee;
    }

    function setSellFee(uint256 sellDevelopmentFee, uint256 sellLiquidityFee, uint256 sellBuyBackFee) external onlyOwner {
        require(sellDevelopmentFee + sellLiquidityFee + sellBuyBackFee <= 12, "Must keep sell taxes below 12%");
    ...
    function removeAllFee() private {
        if(_buyDevelopmentFee == 0 && _buyLiquidityFee == 0 && _buyBuyBackFee == 0 && _sellDevelopmentFee == 0 && _sellLiquidityFee == 0 && _sellBuyBackFee == 0) return;
    ...
    function _getTotalFees(bool isSell) private view returns(uint256) {
        if (isSell) {
            return _sellDevelopmentFee + _sellLiquidityFee + _sellBuyBackFee;
        }
        return _buyDevelopmentFee + _buyLiquidityFee + _buyBuyBackFee;
    }
}