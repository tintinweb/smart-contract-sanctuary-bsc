...
contract PresaleData {
    ...
    // Early Access Timer Ended
    uint256 public earlyAccessTimerEnded;
...
contract Presale is PresaleData, Cloneable {
    ...
    function _register(address user, uint256 value) internal {
        ...
        require(
            database.earlyAccessCheck(presaleStartTime, user),
            'Early Access Denied'
        );
    ...
    function masterRefund() external onlyMaster {
        refundEnabled = true;
        database.endPresale(totalValueRegistered);
    }
