...
// File contracts/VestingLeaf.sol
...
/*
 * @title Vesting
 * @dev Add vest function to specified address
 * @dev Multiple addresses can be added, and multiple lock functions can be added to each address
 */
contract VestingLeaf {
    ...
    //Schedule for each address
    mapping(address => mapping(uint256 => Schedule)) public schedules;
    //Record how many Schedule there are in the address
    mapping(address => uint256) public numberOfSchedules;
    //Record the number of tokens currently locked
    mapping(address => uint256) public locked;

    event Release(address indexed beneficiary, uint256 amount);
    event AddBeneficiary(address indexed to, uint256 amount);
    event Revoke(address indexed beneficiary, uint256 amount);
    ...
    function addBeneficiary(
        ...
        uint256 amount,
        address tokenAddr,
        bool revokable, //The default is false
        uint256 cliffTime,
        uint256 durationTime,
        uint256 startTime
    ) public onlyOwner {
        require(
            durationTime > 0 && durationTime >= cliffTime && amount > 0,
            "invalid vesting params"
        );

        //Obtain locked token
        uint256 currentLocked = locked[tokenAddr];

        require(
            LeafFT(tokenAddr).balanceOf(address(this)) >=
                currentLocked + amount,
            "Not enough tokens"
        );

        //Record the number of current address schedules
        uint256 currentNumSchedules = numberOfSchedules[beneficiary];

        //Adds the schedule to the current address（The schedule array starts at 0, so currentNumSchedules is the latest location）
        ...
        //Update the data
        numberOfSchedules[beneficiary] = currentNumSchedules.add(1);
        locked[tokenAddr] = currentLocked.add(amount);
        emit AddBeneficiary(beneficiary, amount);
    }

    function getschedules(address ben,uint256 num) view external returns(Schedule memory){
        return schedules[ben][num];
    }

    /*
     * @notice Allows users to release vested tokens if the cliff time has passed.
     * @dev The caller need to give which schedule number
     */
    function release(uint256 scheduleNumber) external {
        //Gets the schedule given by the current caller
        Schedule storage schedules = schedules[msg.sender][scheduleNumber];

        require(schedules.cliffTime <= block.timestamp, "cliff not reached");

        require(schedules.totalAmount > 0, "not releasable");

        //Get the amount to be released
        uint256 amount = calcDistribution(
            schedules.totalAmount,
            block.timestamp,
            schedules.startTime,
            schedules.endTime,
            schedules.cliffTime
        ...
        amount = amount > schedules.totalAmount
            ? schedules.totalAmount
            : amount;

        uint256 amountToTransfer = amount.sub(schedules.releasedAmount);
        schedules.releasedAmount = amount;
        locked[schedules.tokenAddr] = locked[schedules.tokenAddr].sub(
            amountToTransfer
        );
        LeafFT(schedules.tokenAddr).transfer(msg.sender, amountToTransfer);
     ...
     */
    function revoke(address account, uint256 scheduleId) external onlyOwner {
        Schedule storage schedule = schedules[account][scheduleId];
        require(!schedule.revokable, "Account is revoked");

        //Gets the remaining tokens in the current schedule
        uint256 outstandingAmount = schedule.totalAmount -
            schedule.releasedAmount;
        require(outstandingAmount != 0, "no outstanding tokens");
        schedule.totalAmount = 0;
        locked[schedule.tokenAddr] = locked[schedule.tokenAddr].sub(
            outstandingAmount
        );

        //Return the token to the sender
        LeafFT(schedule.tokenAddr).transfer(owner, outstandingAmount);
        emit Revoke(account, outstandingAmount);
    }

    /*
     * @notice calculates the amount of tokens to distribute to an account at any instance in time, based off some total claimable amount.
     */
    ...
    /*
     * @notice Withdraw the remaining tokens from the contract, except for the locked tokens
     */
    function withdraw(uint256 amount, address tokenAddr) external onlyOwner {
        require(
            LeafFT(tokenAddr).balanceOf(address(this)) - locked[tokenAddr] >=
                amount,
            "Vesting: Can't withdraw"
        );
        //Return the token to the sender
        LeafFT(tokenAddr).transfer(owner, amount);
    }
}