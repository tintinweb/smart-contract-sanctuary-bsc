...
contract WineryIDOLaunchpadNFT is
  Initializable,
  ...
  ReentrancyGuardUpgradeable
{
  ...
  uint256 public freezingTimeAtSecond = 3600 * 24 * 90; // 3 months
  ...
  function getLaunchesLength() public view returns (uint256) {
    ...
    returns (uint256)
  {
    ...
    launches[_launchIndex].soldCount += 1;

    require(_launch.priceInUSD > 0, "Wrong price given");

    IERC20(usdt).safeTransferFrom(msg.sender, treasuryAddress, _launch.priceInUSD);
  ...
  function sentNFT(
    ...
    bool freeze
  ) public onlyRole(ADMIN_ACTION_ROLE) nonReentrant whenNotPaused returns (uint256) {
    require(_launchIndex < launches.length, "Wrong launchpad number");
    //gas saving
    Launchpad memory _launch = launches[_launchIndex];
    require(checkLimits(_receiver, _launchIndex), "limit exceeding");
    boughtCount[_receiver][_launchIndex] += 1;
    launches[_launchIndex].sendCount += 1;
    ...
    emit Receive(nftId, _refCode, msg.sender, _launchIndex, _launch.robiBoost);

    return nftId;
  }

  function checkLimits(address user, uint256 launchIndex) internal view returns (bool) {
    Launchpad memory launch = launches[launchIndex];
    return
      boughtCount[user][launchIndex] < maxToUser &&
