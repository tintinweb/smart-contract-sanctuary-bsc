...
contract ParkSale is IParkSale{
  ...
  function getTokenValue(address tokenAddress_) public view returns(uint256){
  ...
  function purchaseToken(address tokenAddress_) external payable returns(bool){
        ...
        uint256 price=getTokenValue(tokenAddress_);
        ...
        emit PurchaseToken(transactionID,maturityDuration,msg.sender);
  ...
  function withdrawToken(address tokenAddress_,address walletAddress_,uint256 transactionID_) external returns(bool){
    ...
    emit WithdrawToken(transactionID_,block.timestamp,msg.sender);
  ...
  function withdrawUnsoldToken(address tokenAddress_,address walletAddress_,uint256 amount_) external returns(bool){
    ...
    emit WithdrawUnsoldToken(tokenAddress_,msg.sender,amount_);
    return true;
  }
  ...
  function withdrawBNB(address toWalletAddress_,uint256 amount_) external returns(bool){
    require(msg.sender==_owner," You do not have required authority");
        ...
        emit  WithdrawBNB(toWalletAddress_,amount_);
        return(true);
  }
  ...
  function setCurrentMaturityTimeStamp(uint256 newTimeStamp_) external  returns(bool){
     ...
     emit SetCurrentMaturityTimeStamp(_maturityDuration, newTimeStamp_);
  ...
  function setCurrentMaturityPercentage(uint256 newPercentage_) external  returns(bool){
     require(msg.sender==_owner," You do not have required authority");
     emit SetCurrentMaturityPercentage( _maturityPercentage, newPercentage_);
   ...
   function transferOwnerShip(address newOwner_) external  returns(bool){
      require(msg.sender==_owner," You do not have required authority");
      emit TransferOwnerShip(newOwner_,_owner); 
     _owner=newOwner_;
     return true;
   }
...
interface IParkSale {
  ...
  event SetCurrentMaturityTimeStamp(uint256 indexed oldMaturityTimeStamp_,uint256 indexed newMaturityTimeStamp_);

  event SetCurrentMaturityPercentage(uint256 indexed oldMaturityPercentage_,uint256 indexed newMaturityPercentage_);

  event TransferOwnerShip(address indexed newOwner_,address indexed oldOwner); 
