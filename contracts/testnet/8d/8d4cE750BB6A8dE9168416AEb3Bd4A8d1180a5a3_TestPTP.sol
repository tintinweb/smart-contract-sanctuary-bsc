...
contract TestPTP is ERC20 {
    ...
    function createStake(uint256 _stake,uint day,string memory stakeName) public {
        ...
        perDayShareStakes[currentDay].push(record);
    ...
    function setStakeTotalShare(uint256 day) public onlyOwner{
        require(day > 1,"Not set supply now");
        uint256 prevDay = day.sub(1);
        TotalStakedShare[] memory records = perDayShareStakes[prevDay];
        if(records.length > 0){
            for(uint i = 0; i<records.length; i++){
                if(records[i].endDay != day && records[i].isRemoved == false){ 
                    perDayShareStakes[day].push(records[i]);
                }
            }
        }
    }
