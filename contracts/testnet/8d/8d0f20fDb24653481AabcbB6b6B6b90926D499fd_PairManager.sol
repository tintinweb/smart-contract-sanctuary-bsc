...
contract PairManager is Ownable, IPoolFactory, IPoolManager {
  ...
  function registerAma(
    ...
    function updatePartnerInfo( string memory _name , string memory _username  ,uint256 _fees , string memory _configuration ,bool _status , address _updatedAddress) public onlyAmaAddress{
  ...
  function payAmaPartner(
     ...
     for (uint256 i = 0; i < _partnerAddress.length; i++) {
        if(_partnerAddress[i] != address(0)){
            if(amaPartnerAddress.contains(_partnerAddress[i]) == true){
