...
contract LemniscateContract {
    ...
    uint256 constant private DECIMALS = 1e8;
    ...
    function getSellPrice() public view returns (uint256) {
        if (_totalTokenSupply > 0) {
            return _totalTokenBalanceUsd.mul(DECIMALS).div(_totalTokenSupply);
        } else {
            return INITIAL_TOKEN_PRICE.mul(DECIMALS);
        }
    }

    function getBuyPrice() public view returns (uint256) {
        uint256 max = _totalTokenBalanceUsd.div(BALANCE_IN_SMART_CONTRACT_FOR_SAME_PRICE);
        uint256 buyPrice = INITIAL_TOKEN_PRICE.mul(DECIMALS);
        ...
        for (uint256 i = 0; i < max; i++) {
            if (buyPrice.subNoNegative(sellPrice).mul(100).div(buyPrice) < MAX_BUY_PRICE_SELL_PRICE_DIFFERENT_PERCENT) {
                uint256 newBuyPrice = buyPrice.div(DECIMALS).permill(1000 + BUY_PRICE_INCREASING);
                buyPrice = newBuyPrice.mul(DECIMALS);
            } else {
                buyPrice = sellPrice;
            }
        }
    ...
    function getTokensReceived(uint256 valueBuyUsd) public view returns (uint256) {
        return valueBuyUsd.mul(1e18).mul(DECIMALS).div(getBuyPrice());
    }

    function getUsdReceived(uint256 amountSellToken) public view returns (uint256) {
        return amountSellToken.mul(getSellPrice()).div(DECIMALS);
    }
