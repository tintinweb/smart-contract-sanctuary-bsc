...
contract NFTOperate is INFTOperate, Caller {
    ...
    function split(
        ...
        uint256 property_ = property.length / 8;
        bytes memory property__ = new bytes(8);
        for (uint256 i = 0; i < property_; i++) {
            ...
            property__[4] = property[k + 4];
            property__[5] = property[k + 5];
            property__[6] = property[k + 6];
            property__[7] = property[k + 7];
            child.mint(INFTBoxToken(address(0)), msg.sender, property__, key);
        }
        emit SplitCardEvent(token_, tokenId);
    }
    ...
    function composite(
        ...
        for (uint128 i = 0; i < tokenId.length; i++) {
            ...
            emit CompositeElementEvent(token_, tokenId_);
        ...
        parent.mint(INFTBoxToken(address(0)), msg.sender, property__, key);
    }
...
interface INFTOperate {
    // 分解NFT
    event SplitCardEvent(
        address indexed token, // 被分解NFT的合约地址
        uint256 indexed tokenId // 被分解NFT的TokenId
    );

    // 合成NFT
    event CompositeElementEvent(
        address indexed token, // 合成NFT的合约地址
        uint256 indexed tokenId // 合成NFT的TokenId
    );
