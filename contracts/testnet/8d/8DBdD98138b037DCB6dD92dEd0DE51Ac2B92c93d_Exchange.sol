...
contract Exchange is OrderState, TransferSafe {
        ...
        if (order.key.sellAsset.assetType == AssetType.BEP721 || order.key.buyAsset.assetType == AssetType.BEP721) {
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ...
                    require(order.key.sellAsset.token == address(_Store), "invalid sell asset");
                    BEP1155safeMintTransferFrom(
                        IStore(order.key.sellAsset.token),
                        order.key.owner,
                        buyer,
                        order.key.sellAsset.tokenId,
                        1,
                ...
                if (!isStore) {
                    ...
                    require(order.key.buyAsset.token == address(_Store), "invalid buy asset");
                    BEP1155safeMintTransferFrom(
                        IStore(order.key.buyAsset.token),
                        buyer,
                        order.key.owner,
                        order.key.buyAsset.tokenId,
                        1,
    ...
    function fiatMethod(uint256 _type,address _nftAddress , address payable _from ,address _to,address payable _royalty,uint256 royaltyFee ,uint256 _id,string memory uri ,uint _nftType,uint amount)external payable{
        ...
        if(_type == 1){
            require(_nftAddress == address(_Store),"Invalid Token Address");
             BEP1155safeMintwithuriTransferFrom(IStore(_Store),_from,_to,_id,1,uri);  
        }
        else{
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        ...
        _Store = IStore(newMintableStore);
