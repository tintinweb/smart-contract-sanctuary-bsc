...
contract Dummy is ERC20, Ownable {
 ...
 struct Plan {
	...
	struct WitthdrawHistory {
		...
		uint256 start;
		 uint8 typeWithdraw; //token =1, bnb =2
	}
...
struct morereferraleUsers {
        uint256 time;
        address useraddress;
    }
morereferraleUsers[] internal moreReferralUser;
address[] public smartUser;
address[] public goldUser;
address[] public diamondUser;
address[] public ddiamondUser;
  ...
  function getPlanInfo(uint8 plan) public view returns(uint256 perbnbRate,uint256 minBnb,uint256 maxBnb,uint256 planDays,uint8 planPecent,uint8 bonusPercent) {
    ...
    function getUserDividends(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			...
			uint256 percent=plans[user.deposits[i].plan].perDayPercent;
					uint256 share = user.deposits[i].profit.mul(percent).div(PERCENTS_DIVIDER);
    ...
    function claimToken(address userAdress,uint8 typeWithdraw) public{
        	User storage user = users[userAdress];

        uint256 claimTokencount=0;
        if(typeWithdraw==1){
claimTokencount=getUserDividends(userAdress);
        if(balanceOf(address(this))<claimTokencount){
      claimTokencount=balanceOf(address(this));
        }
        if(claimTokencount>0){
            _transfer(address(this),userAdress,claimTokencount);
        }
        user.checkpoint=block.timestamp;
        }else{
claimTokencount=user.bonus;
if(address(this).balance<claimTokencount){
      claimTokencount=address(this).balance;
        }
if(claimTokencount>0){
           payable(userAdress).transfer(claimTokencount);
        }
        }
        user.whistory.push(WitthdrawHistory(claimTokencount,block.timestamp, typeWithdraw));
    }
          	...
          	if(!settleStatus[timePointer]){
				...
				for(uint8 i= 0;i<8;i++){
					if(ranking[i]!=address(0)){
						...
						users[ranking[i]].bonus = users[ranking[i]].bonus.add(refBonus);
                        users[ranking[i]].bonusHistory.push(Bonus(block.timestamp,6,refBonus));
					}
				}
	...
	function getRewardUser() public view returns(address){
	     
		address tmp;
		for(uint8 i = 0;i<moreReferralUser.length;i++){
			if(moreReferralUser[i].time>lastBonusCheckpoint && address(tmp)==address(0)){
				tmp = moreReferralUser[i].useraddress;
			}
		}  
        if(tmp!=address(0)){
        	return tmp;
  ...
  function stakePlan(address referrer) public payable{
     ...
     require(isPlanavailabe==1,"No Plan found in your invested amount");
     uint256 token=0;
     uint256 price=plans[planUSed].ratePerBnb;
     token=price.mul(msg.value.div((10**9)));
     token=token.add(token.mul(plans[planUSed].bonusPercentage).div(100));
if(token>0){
...
amount = msg.value.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);
                  ...
                  amount = msg.value.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);  
					...
					users[upline].bonusHistory.push(Bonus(block.timestamp,5,amount));
					upline = users[upline].referrer;
        ...
        if(!userRefer.isSmartUser){
if(users[referrer].levelmember[0]>=10){
userRefer.isSmartUser=true;
smartUser.push(referrer);
}
        }
          if(!userRefer.isGoldUser){
          if(users[referrer].levelmember[1]>=10){
userRefer.isGoldUser=true;
goldUser.push(referrer);
}  
        }
          if(!userRefer.isDiamondUser){
            if(users[referrer].levelmember[2]>=10){
userRefer.isDiamondUser=true;
diamondUser.push(referrer);
}
        }
          if(!userRefer.isDDiamondUser){
            if(users[referrer].levelmember[3]>=10){
userRefer.isDDiamondUser=true;
ddiamondUser.push(referrer);
}
        }
       }else{
  ...
  function closingRoyalityincome(uint8 types,uint256 percent) public onlyOwner{
      address[] memory checkUSer;
if(types==1){
    checkUSer=smartUser;
}
if(types==2){
...
if(types==3){
    checkUSer=diamondUser;
}
if(types==4){
    checkUSer=ddiamondUser;
}

    uint256 balaceBnbDistribute=address(this).balance.div(smartUser.length).div(percent.div(100));
    for(uint256 i=0;i<checkUSer.length;i++){
User storage userRefer = users[checkUSer[i]];
userRefer.bonus=userRefer.bonus.add(balaceBnbDistribute);
userRefer.bonusHistory.push(Bonus(block.timestamp,types,balaceBnbDistribute));
}
  }
...
function getUserDownlineMembers(address userAddress) public view returns(uint256[12] memory levelmemories) {
    ...
    function getUserBonusHistory(address userAddress, uint256 index) public view returns(uint256 amount, uint256 start,uint256 types) {
	    User storage user = users[userAddress];
		amount = user.bonusHistory[index].amount;
		start=user.bonusHistory[index].timeD;
        types=user.bonusHistory[index].types;
	}
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 amount, uint256 token, uint256 start, uint256 finish) {
	    User storage user = users[userAddress];

		plan = user.deposits[index].plan;
		
		amount = user.deposits[index].amount;
		token = user.deposits[index].profit;
