 ...
 * -#%[email protected]#-  :@#@@+%[email protected]*@*:=%+..%%#=      *@  *@++##.    =%@%@%%#-  =#%[email protected]#-   :*+**+=: %%++%*
 *
 * @title: [Not an EIP] Max-721 UUPS v1 PBT
...
import "./Max721ImplementationUUPS.sol";
...
contract Max721UUPS is Initializable
                     , Max721ImplementationUUPS
                     , UUPSUpgradeable {
  ...
  function publicMint()
    ...
    emit Transfer(address(0), msg.sender, nftEngine.mintID());
    nftEngine.battersUp();
  }
...
abstract contract Max721ImplementationUUPS is Initializable
                                            , MaxErrors
                                            , MaxAccess
                                            , IERC721Enumerable
  ...
  modifier onlyPresale() {
    ...
    (uint256 start, uint256 end) = this.getTimes(epoch);
  ...
  modifier onlySale() {
    uint256 epoch = this.getEpoch();
    (uint256 start, ) = this.getTimes(epoch);
    if (block.timestamp < start) {
      revert TooSoonJunior({
        yourTime: block.timestamp
      , hitTime: start
  ...
  modifier allowedList() {
    if (!this.myListStatus(msg.sender)) {
      revert MaxSplaining({
        reason: "Max721:not allowed"
      });
    }
    _;
  }
  ...
  , bytes calldata _data
    ...
    returns(bytes4) {
    revert MaxSplaining({
      reason: "I don't want your stinkin tokens"
    });
  }
