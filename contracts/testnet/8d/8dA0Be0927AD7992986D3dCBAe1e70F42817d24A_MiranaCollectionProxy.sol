 ...
 *
 */
contract MiranaCollectionProxy is Ownable {
  address public beacon;

  event Mirana721ProxyCreated(address proxy);
  ...
  function createToken(
    ...
    address beaconProxy = deployProxy(_getData(_name, _symbol), salt);
    MiranaERC721 token = MiranaERC721(address(beaconProxy));
    token.transferOwnership(_msgSender());
    emit Mirana721ProxyCreated(beaconProxy);
  }
  ...
  function _getData(string memory _name, string memory _symbol)
    internal
    pure
    returns (bytes memory)
  {
    return
      abi.encodeWithSignature('__ERC721Mirana_init(string, string)', _name, _symbol);
  }
}
  ...
  OwnableUpgradeable
{
  ...
  function __ERC721Mirana_init(string memory _name, string memory _symbol)
    external
    initializer
  {
    __ERC721Mirana_init_unchained(_name, _symbol);
    emit MiranaERC721Created(_msgSender(), _name, _symbol);
  }

  function __ERC721Mirana_init_unchained(
    string memory _name,
    string memory _symbol
  ) internal {
    ...
    returns (string memory)
  {
    ...
    return
      bytes(baseURI).length > 0
        ? string(abi.encodePacked(baseURI, StringsUpgradeable.toString(tokenId), ".json"))
        : "";
  }
}
