...
contract PinkSaledeployer is ReentrancyGuard {
...
contract PreSaleBNBContributionTeam is ReentrancyGuard {
    ...
    modifier whitelist(address user){
        if(whitelistenabled){
            require(whitelisted[user] == true,"PinkSale: Not whitelisted");
        }
        _;
    }
    ...
    function initialize(
        ...
        pairLockDuration = pinkvalues[0];
        pairLocker = _pairLocker;
        antibotTokenminHold = pinkvalues[1];
        whitelistenabled = pinkenables[0];
        refundenabled = pinkenables[1];
        contributionenabled = pinkenables[2];
        teamenabled = pinkenables[3];
        initialPercentageContribution = pinkvalues[2];
        cyclePercentageContributation = pinkvalues[3];
        cyclePeriodContributation = pinkvalues[4];
        teamAmount = pinkvalues[5].mul(10**token.decimals());
        teamStartDuration = pinkvalues[6];
        initialPercentageTeam = pinkvalues[7];
        cyclePercentageTeam = pinkvalues[8];
        cyclePeriodTeam = pinkvalues[9];
    }
    ...
    function buyToken() public payable whitelist(msg.sender) allowed isHuman{
        ...
        require(amountRaised <= hardCap,"PinkSale: Hardcap reached");
        uint256 numberOfTokens = bnbToToken(msg.value);
        uint256 maxBuy = bnbToToken(maximumBuy);
        require(msg.value >= minimumBuy && msg.value <= maximumBuy,"PinkSale: Invalid Amount");
        require(numberOfTokens.add(tokenBalance[msg.sender]) <= maxBuy,"PinkSale: Amount exceeded");
        if(antibotenabled){
            require(antibotToken.balanceOf(msg.sender) >= antibotTokenminHold,"PinkSale: Not enough token holdings");
        }
    ...
    function internalclaim(address _user) internal {
        ...
        if(bnbBalance[_user] > 0 && amountRaised >= softCap && contributionenabled){
            require(contributionenabled,"PinkSale: Contribution not enabled");
            require(Contribution[_user].Amount>0,"PinkSale: Zero balance");
            require(Contribution[_user].Claimed < Contribution[_user].Amount,"PinkSale: Already claimed");
            require(contributionClaimable(_user) > 0,"PinkSale: No more claimable");
        ...
        if(bnbBalance[_user] > 0 && tokenBalance[_user] > 0 && amountRaised >= softCap && !contributionenabled){
            require(!contributionenabled,"PinkSale: Contribution enabled");
            require(tokenBalance[_user] > 0,"PinkSale: No more claimable");
    ...
    function withdrawAndInitializePool() public onlypreSaleOwner allowed isHuman{
        ...
        if(amountRaised >= softCap){
            ...
            uint256 refund = getContractTokenBalance().sub(soldTokens + teamAmount);
    ...
    function whitelistAddresses(address [] memory _addresses,bool _whitelist) external onlypreSaleOwner allowed{
        require(whitelistenabled,"PinkSale: Whitelist not enabled");
    ...
    function enableAntibot(IERC20 _token, uint256 minholdingamount) external onlypreSaleOwner allowed{
        require(!antibotenabled,"PinkSale: Antibot already enabled");
        antibotToken = _token;
    ...
    function enableWhitelist() external onlypreSaleOwner allowed{
        require(!whitelistenabled,"PinkSale: Whitelist already enabled");
        whitelistenabled = true;
        antibotenabled = false;
    }
    
    function enablePublic() external onlypreSaleOwner allowed{
        require(antibotenabled || whitelistenabled,"PinkSale: already Public");
        antibotenabled = false;
        whitelistenabled = false;
    }
