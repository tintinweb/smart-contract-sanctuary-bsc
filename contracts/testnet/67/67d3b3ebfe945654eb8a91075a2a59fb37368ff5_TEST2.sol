...
contract TEST2 is  Initializable ,ContextUpgradeable, BEP20Upgradeable, OwnableUpgradeable {
    using AddressUpgradeable for address;
    ...
    function swapAndLiquify() public onlyOwner lockTheSwap {
        ...
        uint256 otherHalf = ethBalance.sub(half);
         uint256 initialBalance = IBEP20Upgradeable(address(this)).balanceOf(msg.sender);

        swapBnbForTokens(half);

        uint256 newBalance = IBEP20Upgradeable(address(this)).balanceOf(msg.sender);
        uint256 swapeedToken = newBalance.sub(initialBalance);
    
          _approve(msg.sender, address(this), swapeedToken);
            IBEP20Upgradeable(address(this)).transferFrom(msg.sender,address(this),swapeedToken);
     
        addLiquidity(swapeedToken, otherHalf);

        emit SwapAndLiquify(half, swapeedToken, otherHalf);
