...
contract Revenue is Ownable {
    ...
    event RechargeToken(address tokenAddress,address from,uint256 amount);

    function rechargeToken(address tokenAddress,uint256 amount) public {
        require(amount>0,"rechagre amount must be  > 0");
        safeTransferFrom(tokenAddress,msg.sender,address(0),amount);
        emit RechargeToken(tokenAddress, msg.sender, amount);
    }

    function recharge() public payable {
        uint256 amount  = msg.value;
        require(amount>0,"rechagre amount must be  > 0");
        emit RechargeToken(address(0), msg.sender, msg.value);
    }

    function withdrawToken(address tokenAddress,address to,uint amount) public onlyController{
