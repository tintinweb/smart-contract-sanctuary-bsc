...
library PYESwapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'f575688887e732a19035570917cfb2edd672fd576eedd52205cc81b8b20b18c3' // init code hash
            ))));
    }
...
interface IPYESwapRouter is IPYESwapRouter01 {
    ...
    function _removeFee(bool removeFee) external returns (bool);
}
...
contract PYESwapRouter is IPYESwapRouter {
    ...
    function _removeFee(bool removeFee) external virtual override returns (bool){
        removeFee = feeRemoved;
        return feeRemoved;
    } 
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PYESwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PYESwapLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(PYESwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
