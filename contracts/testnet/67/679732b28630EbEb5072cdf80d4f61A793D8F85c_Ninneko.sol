...
contract Ninneko is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    event AuctionCreated(address indexed seller, uint256 indexed petId, uint256 startingPrice, uint256 endingPrice, uint256 duration);
    event AuctionBid(address indexed buyer, uint256 indexed petId, uint256 price);
    ...
    function _addAuction(
        ...
        emit AuctionCreated(_seller, _petId, _auction.startingPrice, uint256(_auction.endingPrice), uint256(_auction.duration));
    }
    ...
    function bid(uint256 _petId) external payable whenNotPaused nonReentrant notInBlackList(_petId) {
        require(this.getApproved(_petId) == address(this), "Seller did not give the allowance for us to auction this one.");
    ...
    function _bid(uint256 _petId, uint256 _bidAmount) private {
        ...
        require(bidExcessToBuyer);

        emit AuctionBid(msg.sender, _petId, price);
    }
