...
contract WoodCoin is Ownable, ERC20 {
    ...
    uint256 public maxSaleAmount = 250000 * (10**decimals());
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        ...
        uniswapV2Pair = IUniswapFactory(uniswapV2Router.factory())
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 swapTokensAtAmountSubLiq = swapTokensAtAmount.sub(halfTokenToLiquidity);
            ...
            if (marketingPart > 0) {
                ...
                marketingFeeTotal = marketingFeeTotal.sub(
                    swapTokensAtAmount.mul(tokenToMarketing).div(tokenToSwapPlusLiq)
                );
            }
            ...
            if (developmentPart > 0) {
                ...
                developmentFeeTotal = developmentFeeTotal.sub(
                    swapTokensAtAmount.mul(tokenToDevelopment).div(tokenToSwapPlusLiq)
                );
            }
            ...
            if (projectPart > 0) {
                ...
                projectFeeTotal = projectFeeTotal.sub(
                    swapTokensAtAmount.mul(tokenToproject).div(tokenToSwapPlusLiq)
                );
            }
            ...
            if (liquidityPart > 0) {
                ...
                liquidityFeeTotal = liquidityFeeTotal.sub(
                    swapTokensAtAmount.mul(tokenToLiquidity).div(tokenToSwapPlusLiq)
                );
            }
        ...
        if (takeFee) {
            (uint256 allfee, uint256 burnFeeTx) = collectFee(
                amount,
                automatedMarketMakerPairs[to],
                !automatedMarketMakerPairs[from] &&
                    !automatedMarketMakerPairs[to]
            ...
            if (allfee > 0) super._transfer(from, address(this), allfee);
            if (burnFeeTx > 0) _burn(from, burnFeeTx);

            amount = amount.sub(allfee).sub(burnFeeTx);
        }
    ...
    function collectFee(
        ...
        bool sell,
        bool p2p
    ) private returns (uint256 totalFee, uint256 burnFeeTx) {
        uint256 rewardFeeNew = amount
            .mul(
                p2p ? liquidityFee[2] : sell ? liquidityFee[1] : liquidityFee[0]
            )
            .div(10000);
        liquidityFeeTotal = liquidityFeeTotal.add(rewardFeeNew);
        ...
        totalFee = rewardFeeNew.add(projectFeeNew).add(marketingFeeNew).add(
            developmentFeeNew
        );
    }
