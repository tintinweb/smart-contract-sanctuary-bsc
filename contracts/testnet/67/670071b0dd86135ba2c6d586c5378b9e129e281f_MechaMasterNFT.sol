...
contract MechaMasterNFT is Initializable, ERC721, Member {
    ...
    mapping(uint8 => uint256) public ethnicityCount; // 種族NFT數量
    ...
    event Reveal(address indexed origin, uint256 tokenId, string ipfshash, uint256 power, uint256 ethnicity, uint256 level);
    ...
    function init() internal initializer {
        userMintStartMax = 1 days / 3; // 公開鑄造產量更新時間1天
        userMintCountMax = 150; // 公開鑄造釋放150個盲盒
    }

    function preOfficalStart(uint256 start) public {
    ...
    function _mintinternal(address origin, bool isOffcial, uint256 price, bool isAdvanced) internal {
        NFT_Id++;
        starAttributes[NFT_Id].origin = origin;
        starAttributes[NFT_Id].power = 400;
        starAttributes[NFT_Id].offical = isOffcial;
        starAttributes[NFT_Id].createTime = block.timestamp;
        starAttributes[NFT_Id].price = price;
        starAttributes[NFT_Id].isAdvanced = isAdvanced;
        _mint(origin, NFT_Id);
    }
    ...
    function preOfficalMint() public returns(uint256) {
        require(IPromote(manager.members("MCAStakePool")).isGamer(msg.sender), "need gamer");
        require(block.number >= preStart, "NOT start!");
        require(preCount < PRE_TOTAL,"Sale Over!");
        ...
        IERC20(pmca).transferFrom(user, manager.members("OfficalAddress"), 1);

        _mintinternal(user, true, needPay, true);
        emit PreMint(user, user, 400, NFT_Id);

        
        uint256 mca_price = getPrice();
        uint256 NFTprice = 500 * 1e18;
        uint256 NFTMcPrice = NFTprice.mul(1e18).div(mca_price);

        address upline = IPromote(manager.members("MCAStakePool")).getUser(msg.sender).f;
        if(upline != address(0) && upline != IPromote(manager.members("MCAStakePool")).genesis()) {
            bool uplineIsValid = IPromote(manager.members("MCAStakePool")).getUser(upline).isValid;
            
            // 鑄造時的 5% 給值推
            // 如果直推不是有效用戶，只Mint 2.5 %
            uint256 mintToUpline;
            if(uplineIsValid) {
                // 有效用戶
                mintToUpline = NFTMcPrice * 5 /100; // 5%
            }else{
                // 無效用戶
                mintToUpline = NFTMcPrice * 25 /1000; // 2.5%
            }
            mca.mint(upline, mintToUpline);
        }
    ...
    function officalMint() public returns(uint256){
        ...
        if (block.number <= (userMintStart + userMintStartMax)) {
            require(userMintCount < userMintCountMax, "mint has reached limit");
        }

        if (block.number > (userMintStart + userMintStartMax)) {
            userMintStart = block.number;
            uint256 amount = userMintCountMax - userMintCount;
            if(amount == 0){
                userMintCountMax = userMintCountMax * 110 / 100;
            }else{
                userMintCountMax = userMintCountMax * 90 / 100;
            }
            userMintCount = 0;
        }
        address user = msg.sender;
        ...
        _mintinternal(user, true, needPay, false);
        emit OfficalMint(user, user, 400, NFT_Id, needPay);
        return NFT_Id;
    }
    ...
    function _distribute(uint256 needpay, uint256 forOfficalMCA) internal {        
        ...
        IPromote(manager.members("MCAStakePool")).update(mcaStakeAmount);

        // 15% 进入 LP池
        uint256 lpStakeAmount = needpay.mul(15).div(100);
        IERC20(mca).transfer(address(manager.members("LPStakePool")), lpStakeAmount);
        ITokenStake(manager.members("LPStakePool")).update(lpStakeAmount);

        // 5%直接給推薦人
        uint256 upLineRewordAmount = needpay.mul(5).div(100);
        uint256 upLineBurn = 0;
        address upline = IPromote(manager.members("MCAStakePool")).getUser(msg.sender).f;
        if(upline != address(0) && upline != IPromote(manager.members("MCAStakePool")).genesis()) {
            bool uplineIsValid = IPromote(manager.members("MCAStakePool")).getUser(upline).isValid;
            
            // 鑄造時的 5% 給值推
            // 如果直推不是有效用戶，其中 2.5 % burn 掉

            uint256 payToUpline = upLineRewordAmount;
            if(!uplineIsValid) {
                // 無效用戶
                payToUpline = upLineRewordAmount / 2; // 2.5%
                upLineBurn = upLineRewordAmount - payToUpline; // 2.5%
            }
            IERC20(mca).transfer(upline, payToUpline);
        }

        // 40% or 42.5% 銷毀
        uint256 burnAmount = IERC20(mca).balanceOf(address(this));
        IERC20(mca).burn(burnAmount);
    }

    function withdrawFunds(IERC20 token,uint256 amount) public returns(bool){
        require(msg.sender == manager.members("owner"), "only owner");
        require(manager.members("funder") != address(0), "funder address need set");
    ...
    function reveal(uint256 tokenId) external {
        ...
        uint8 ethnicity = uint8(randomNum % 3 + 1);

        uint256 power;
        if(starAttributes[tokenId].isAdvanced) {
            // 1000-1200
            power = randomNum % 201 + 1000;
        }else{
            // 800-1200
            power = randomNum % 401 + 800;
        }
        ...
        emit Reveal(msg.sender, tokenId, _hash, power, ethnicity, 1);
    }

    function updateNFT(uint256 tokenId1, uint256 tokenId2) external {
        require(tokenId1 != tokenId2, "need two diff nft");
        require(ownerOf(tokenId1) == msg.sender && ownerOf(tokenId2) == msg.sender, "It's not your nft");
        require(starAttributes[tokenId1].ethnicity != starAttributes[tokenId2].ethnicity, "need different ethnicity nft");
        require(starAttributes[tokenId1].level > 0 && starAttributes[tokenId2].level > 0, "not ethnicity nft");
        require(starAttributes[tokenId1].level == starAttributes[tokenId2].level || 
            starAttributes[tokenId1].level + 1 == starAttributes[tokenId2].level || 
            starAttributes[tokenId1].level - 1 == starAttributes[tokenId2].level
        , "different level");
