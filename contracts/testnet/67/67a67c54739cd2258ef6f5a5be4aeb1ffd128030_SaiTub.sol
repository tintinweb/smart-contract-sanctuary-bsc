...
contract VoxFab {
    function newVox(uint256 rate) public returns (SaiVox vox) {
        vox = new SaiVox(rate);
...
contract DSGuard is DSAuth, DSAuthority, DSGuardEvents {
    bytes32 public constant ANY = bytes32(uint(-1));
...
contract SaiVox is DSThing {
    ...
    function inj(int128 x) internal pure returns (uint256) {
        return x >= 0 ? uint256(x) + RAY : rdiv(RAY, RAY + uint256(-x));
    }

    function prj(uint256 x) internal pure returns (int128) {
        return x >= RAY ? int128(x - RAY) : int128(RAY) - int128(rdiv(RAY, x));
    }
}
