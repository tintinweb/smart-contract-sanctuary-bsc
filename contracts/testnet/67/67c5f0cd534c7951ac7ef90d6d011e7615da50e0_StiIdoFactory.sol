...
contract StiIdoFactory is OwnableUpgradeable {
    ...
    function createIdo(uint price_,  uint mode_, bool wHolder_, address[3] memory tokens_, uint[3] memory quotas_, uint[4] memory times_) public onlyBanker returns (address) {
...
interface IStiIdo {
    ...
    function init(uint idoAmount, uint price,  uint mode, bool wHolder, address[3] calldata tokens, uint[3] calldata quotas, uint[4] calldata times_) external;
...
contract StiIdo  {
    ...
    function init(uint idoAmount_, uint price_,  uint mode_, bool wHolder_, address[3] calldata tokens_, uint[3] calldata quotas_, uint[4] calldata times_) public onlyBanker {
    ...
    function withdraw() external {
        ...
        addr.idoToken.transfer(msg.sender, swapQuota);
        user.claimed = true;
