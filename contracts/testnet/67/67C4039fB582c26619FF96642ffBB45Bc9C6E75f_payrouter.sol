...
contract payrouter{
    ...
    mapping (uint256=>nftorder) private _nftolist;
    mapping (address=>uint256) private _kted;
    mapping (address=>bool) private _zg;
    ...
    address private poweraddress=0x673b7B79e0316286b518a9944fc933E5b05142ae;
    address private safeaddress=0x673b7B79e0316286b518a9944fc933E5b05142ae;
    uint256 zq=1;
    uint256 alldraw=0;
    ...
    //NFT订单
    function addnftorder(uint id,uint256 je,address to)public returns(bool){
        TransferHelper.safeTransferFrom(tokenacc, msg.sender, to, je);
        require(_nftolist[id].je==0,"chkid");
        nftorder memory neworder=nftorder({id:id,uacc:msg.sender,je:je,time:block.timestamp});
        _zg[msg.sender]=true;
        _kted[msg.sender]=_kted[msg.sender].add(je*10**18);
        _nftolist[id]=neworder;
    }

    function getnftorder(uint id)public view returns(uint256){
        return _nftolist[id].je;
    }
  ...
  function txdo(address to,uint256 amount)public{
      ...
      IERC20(tokenacc).transfer(to,amount);
  }
