    ...
    PausableUpgradeable
{
    ...
    bytes32 private constant _RECRUIT_TYPEHASH =
        0x0b91ca91ec3cddc03c44311ded4f24a3143f0b5638ae2cad36113c2c5280de76;
    bytes32 public eStone;
    uint256 public constant HIGHEST_TOKEN_ID_IN_GENESIS = 24999;
    uint256 public recruitmentLimit;
    uint256 public recruitmentCountdown;
    uint256 public maximumNumberOfPermits;
    uint256 public ownerCutMegFeePerMillion;
    ...
    // Mapping from `tokenId`to allowed state to recruit
    mapping(uint256 => bool) private _tokenRecruiters;

    modifier recruitable(uint256 token0, uint256 token1) {
        require(
            _recruitedCounts[token0] < recruitmentLimit,
            "The token0 has reached the recruitment limit"
        );
        require(
            _recruitedCounts[token1] < recruitmentLimit,
            "The token1 has reached the recruitment limit"
        );
        require(
            block.timestamp >= _countdowns[token0],
            "Recruitment countdown of token0"
        );
        require(
            block.timestamp >= _countdowns[token1],
            "Recruitment countdown of token1"
        );
        _;
    }

    modifier notIdentical(uint256 token0, uint256 token1) {
        require(token0 != token1, "Identical token");
        _;
    }
    ...
    function updateValidator(address newValidator) external override onlyOwner {
        require(
            newValidator != address(0),
            "The new validator is the zero address"
        );

        validator = newValidator;
        emit ValidatorChanged(_msgSender(), newValidator);
    }
    ...
    function _getRecruitmentFees(uint256 _token0, uint256 _token1)
        internal
        view
        returns (
            uint256,
    ...
    {
        uint256 _index0 = _recruitedCounts[_token0];
        uint256 _index1 = _recruitedCounts[_token1];
        uint256 ebaFee =
            _recruitmentTokenFees[_index0][address(EBA)] +
                _recruitmentTokenFees[_index1][address(EBA)];
        uint256 megFee =
            _recruitmentTokenFees[_index0][address(MEG)] +
                _recruitmentTokenFees[_index1][address(MEG)];
        uint256 eStoneFee = _eStoneFees[_index0] + _eStoneFees[_index1];

        return (ebaFee, megFee, eStoneFee);
    }

    function getRecruitmentFees(uint256 token0, uint256 token1)
        external
        view
        override
        notIdentical(token0, token1)
        returns (
            uint256,
            uint256,
            uint256
        )
    {
        ...
        return _getRecruitmentFees(token0, token1);
    }
    ...
    function updateCurrencyManager(IElpisCurrencyManager newCurrencyManager)
        external
        onlyOwner
    {
        require(
    ...
    function setRecruitmentFees(
        ...
        uint256 length = recruitCounts.length;
        for (uint256 i = 0; i < length; ++i) {
            _setRecruitmentFee(
                recruitCounts[i],
                ebaFees[i],
    ...
    function setOwnerCutMegFeePerMillion(uint256 _ownerCutMegFeePerMillion)
        external
        override
        onlyOwner
    {
        require(
            _ownerCutMegFeePerMillion <= 200000,
            "The owner cut should be between 0 and 200000"
        );

        ownerCutMegFeePerMillion = _ownerCutMegFeePerMillion;
        emit OwnerCutMegFeePerMillionChanged(
            _msgSender(),
            _ownerCutMegFeePerMillion
        );
    }

    function isRecruitable(uint256 tokenId)
        external
        view
        override
        returns (bool)
    {
        require(EMH.ownerOf(tokenId) != address(0), "Token does not exist");

        return ((tokenId <= HIGHEST_TOKEN_ID_IN_GENESIS ||
            _tokenRecruiters[tokenId]) &&
            _recruitedCounts[tokenId] < recruitmentLimit &&
            block.timestamp >= _countdowns[tokenId]);
    }

    /**
     * @dev Returns whether `operator` is allowed to recruit `token0` and `token1`.
     *
     * Requirements:
     *
     * - `token0` must exist.
    ...
    function _isPermitHolderOrOwner(
        ...
        address owner0 = EMH.ownerOf(token0);
        address owner1 = EMH.ownerOf(token1);
        return ((owner0 == operator && owner1 == operator) ||
            (owner0 == owner1 &&
                _tokenPermits[owner0][operator].contains(token0) &&
                _tokenPermits[owner1][operator].contains(token1)));
    }

    function _recruit(uint256 token0, uint256 token1) internal {
        address owner = EMH.ownerOf(token0);
        uint256 megFeeShare;
        (uint256 _ebaFee, uint256 _megFee, uint256 _eStoneFee) =
            _getRecruitmentFees(token0, token1);

        if (_ebaFee > 0) {
            EBA.transferFrom(_msgSender(), treasury, _ebaFee);
        }
        if (_megFee > 0) {
            //if the cut is greater than zero, we will transfer that cut to the treasury
            if (ownerCutMegFeePerMillion > 0) {
                megFeeShare =
                    (_megFee * ownerCutMegFeePerMillion) /
                    1e6;
                _megFee -= megFeeShare;
                MEG.transferFrom(_msgSender(), treasury, megFeeShare);
            }
        ...
        uint256 countdown = block.timestamp + recruitmentCountdown;
        _countdowns[token0] = countdown;
        _countdowns[token1] = countdown;
        _recruitedCounts[token0] = _recruitedCounts[token0] + 1;
        _recruitedCounts[token1] = _recruitedCounts[token1] + 1;

        EMH.mint(owner);
        emit Recruited(owner, token0, token1);
    }
    ...
    function recruit(
        ...
        whenNotPaused
        notIdentical(token0, token1)
        recruitable(token0, token1)
    {
        //If token is not recruiter
        //We will verify the token is recruitable or not
        if (token0 > HIGHEST_TOKEN_ID_IN_GENESIS && !_tokenRecruiters[token0]) {
            require(_verify(token0, signature0), "Verify token0 failed");
            _tokenRecruiters[token0] = true;
        }
        if (token1 > HIGHEST_TOKEN_ID_IN_GENESIS && !_tokenRecruiters[token1]) {
            require(_verify(token1, signature1), "Verify token1 failed");
            _tokenRecruiters[token1] = true;
        }
        require(
            _isPermitHolderOrOwner(_msgSender(), token0, token1),
            "Recruit caller is not owner nor permit holder"
        );
        _recruit(token0, token1);
    }

    function _verify(uint256 tokenId, bytes calldata signature)
        internal
        view
        returns (bool)
    {
        bytes32 digest =
            ECDSA.toEthSignedMessageHash(
                keccak256(abi.encode(_RECRUIT_TYPEHASH, tokenId))
            );

        address recoveredAddress = ECDSA.recover(digest, signature);
        return recoveredAddress == validator;
    }
}
...
interface IElpisHeroRecruiter {
    /**
     * @dev Emitted when new hero is recruited from `token0` and `token1`
     */
    event Recruited(address indexed owner, uint256 token0, uint256 token1);
    ...
    /**
     * @dev Emitted when validator is changed.
     */
    event ValidatorChanged(address account, address newValidator);
    ...
    /**
     * @dev Emitted when `ownerCutMegFeePerMillion` is changed from `account`.
     */
    event OwnerCutMegFeePerMillionChanged(
        address account,
        uint256 ownerCutMegFeePerMillion
    );

    /**
     * @dev Returns the number recruited of `tokenId` token.
     */
    function recruitedCountOf(uint256 tokenId) external view returns (uint256);
     ...
     * Requirements:
     *
     * - `token0` token is recruitable.
     * - `token1` token is recruitable.
     */
    function getRecruitmentFees(uint256 token0, uint256 token1)
        external
        view
     ...
     * - the caller is the owner.
     *
     * Emits a {ValidatorChanged} event.
     */
     ...
     *
     * Emits a {RecruitmentFeesSet} event.
     */
    function setRecruitmentFees(
        uint256[] calldata recruitCounts,
        uint256[] calldata ebaFees,
    ...
    /**
     * @dev Sets value for `ownerCutMegFeePerMillion`.
     *
     * Requirements:
     *
     * - the caller is the owner.
     * - `ownerCutMegFeePerMillion` should be between 0 and 200000.
     *
     * Emits a {OwnerCutMegFeePerMillionChanged} event.
     */
    function setOwnerCutMegFeePerMillion(uint256 ownerCutMegFeePerMillion)
        external;

    /**
     * @dev Returns whether `tokenId` is recruitable.
     *
     * Requirements:
     *
     ...
     */
    function isRecruitable(uint256 tokenId) external view returns (bool);
     ...
     * Requirements:
     *
     * - `token0` token must exist and be owned by caller.
     * - `token1` token must exist and be owned by caller.
     *
