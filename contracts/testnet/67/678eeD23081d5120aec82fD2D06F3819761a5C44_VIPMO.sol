...
contract VIPMO is Ownable {
	...
	constructor(address _dev, address _mark, address _ba, address _brok) {		
	...
	function reinvest(address ref) public initializer {		
		...
		referrals_.claimedEggs = SafeMath.add(referrals_.claimedEggs,SafeMath.div(eggsUsed,16));
    ...
    function tradeinv(uint256 _amount) private view returns(uint256) {
		return SafeMath.div(SafeMath.mul(_amount,TradeBot),100);
	}
	...
	function sendFeesInv(uint _amountfee, uint _amountbroker) internal {
    ...
    function sendFeesWith(uint _amountfe) internal {
