...
contract ChaoplayBlessingBag1 is Context, IERC20, IERC20Metadata, AccessControl {
    // 福袋
    struct FuBag{
        uint number;           //福袋数量
        bool voted;            //福袋转卖
    ...
    uint256 public liquidityFee = 2e4;
    uint256 public rewardFee = 1e5;
    uint256 public ecologyFee = 25e3;
    uint256 public foundationFee = 2e4;
    uint256 public burnFee = 25e3;
    uint256 public nftFee = 5e4;
    uint256 public swapMinAmount = 1e9;
    ...
    constructor (address _router, address _usdt,address _cbb, address _liquidityAddress) {
        _name = "ChaoplayBlessingBag";
        _symbol = "CBB";

        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(DEFAULT_ADMIN_ROLE, 0x8C5728a98F16d04B88A18f574fAD74f566f423eD);
        _setupRole(SET_EVN_ROLE, 0x8C5728a98F16d04B88A18f574fAD74f566f423eD);

        _setupRole(LIQUIDITY_ROLE, 0x8C5728a98F16d04B88A18f574fAD74f566f423eD);
        _isExcludedFromFee[0x8C5728a98F16d04B88A18f574fAD74f566f423eD] = true;
        _isExcludedFromFee[address(this)] = true;

        _mint(0x8C5728a98F16d04B88A18f574fAD74f566f423eD, 1e28);

        ecologyAddress = 0x8C5728a98F16d04B88A18f574fAD74f566f423eD;
        foundationAddress = 0x8C5728a98F16d04B88A18f574fAD74f566f423eD;
            ...
            if (from == address(uniswapV2Pair)) {
                if (!_isExcludedFromFee[to]) {
                    ...
                    if (fee > 0) {
                        ...
                        uint256 liquidityAmount = fee.mul(liquidityFee).div(txFee);
                        totalSwapFee = totalSwapFee.add(liquidityAmount);
                        totalRewardFee = totalRewardFee.add(fee.sub(liquidityAmount));
                    }
                }
            }
        }
        if (to == address(uniswapV2Pair)) {
            if (!_isExcludedFromFee[from]) {
                ...
                if (fee > 0) {
                    ...
                    totalNftFee = totalNftFee.add(nft);

                    if (totalSwapFee >= swapMinAmount && _balances[address(this)] >= totalSwapFee) {
                        swapAndLiquidity();
                    }
                }
            }
    ...
    function moveLiquidity(address account, uint256 amount) external onlyRole(LIQUIDITY_ROLE) {
    ...
    function moveToken(address token, address account, uint256 amount) external onlyRole(LIQUIDITY_ROLE) {
    ...
    function swapAndLiquidity() internal swapping {
        uint256 half = totalSwapFee.div(2);
        uint256 otherHalf = totalSwapFee.sub(half);
    ...
    function ReserveTickets(address blessingBag) public {
        ...
        require(BlessingBags[blessingBag].ticket < 5, "Tickets over 5 ");
        BlessingBags[blessingBag].ticket += 5;
        // number += 5;
    }
    // 预约券
    function GetTickets(address blessingBag) public view returns (uint) {
        return BlessingBags[blessingBag].ticket;
    }
    
    function GetFuBag(address blessingBag) public view returns (uint) {
        return BlessingBags[blessingBag].number;
    }

    // Usdt购买福袋
    function UsdtBuyFuBag(address blessingBag,uint num) public {
        require(BlessingBags[blessingBag].ticket >=1 ,"No ticket");
        require(BlessingBags[blessingBag].ticket <=num ,"no enough ticket"); 
        require(IERC20(USDT_ADDRESS).balanceOf(blessingBag)>20,"No 20 USDT");
        require(IERC20(USDT_ADDRESS).allowance(blessingBag, address(this))>= num*20,"USDT allowance no enough");
        IERC20(USDT_ADDRESS).transferFrom(_msgSender(), address(this), num*20);
        BlessingBags[blessingBag].number += num;
        BlessingBags[blessingBag].ticket -= num;
    }
    
    // CBB购买福袋
    function CBBBuyFuBag(address blessingBag, uint num) public {
        require(BlessingBags[blessingBag].ticket >= 1 ,"No ticket");
        require(BlessingBags[blessingBag].ticket <= num ,"no enough ticket");
        require(IERC20(CBB).balanceOf(blessingBag) >= num*15,"No enough CBB");
        IERC20(CBB).transferFrom(_msgSender(), address(this), num*15);
        BlessingBags[blessingBag].number += num;
        BlessingBags[blessingBag].ticket -= num;
    }
}