...
contract FuckGirl is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Fuck Girls";
    string private _symbol = "FKG";
    uint8 private _decimals = 12;

    address payable public marketingWalletAddress = payable(0xAB3c523a498A8E6286b8F78ED2119C5C4d431B04); // Marketing Address
    address payable public BuyBackWalletAddress = payable(0xe2a675D12EC3f185D1c532ae61b68EDF5d8a06dF); // BuyBack Address
    address payable public teamWalletAddress = payable(0xc32463D195050E7a896E64FB0F5B3Df59Acf6694); // Team Address
    ...
    function setBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax, uint256 newBuyBackTax) external onlyOwner() {
        _buyLiquidityFee = newLiquidityTax;
        _buyMarketingFee = newMarketingTax;
        _buyTeamFee = newTeamTax;
        _buyBuyBackFee = newBuyBackTax;

        _totalTaxIfBuying = _buyLiquidityFee.add(_buyMarketingFee).add(_buyTeamFee).add(_buyBuyBackFee);
    }

    function setSellTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax, uint256 newBuyBackTax) external onlyOwner() {
        _sellLiquidityFee = newLiquidityTax;
        _sellMarketingFee = newMarketingTax;
        _sellTeamFee = newTeamTax;
        _sellBuyBackFee = newBuyBackTax;

        _totalTaxIfSelling = _sellLiquidityFee.add(_sellMarketingFee).add(_sellTeamFee).add(_sellBuyBackFee);
    }
    
    function setDistributionSettings(uint256 newLiquidityShare, uint256 newMarketingShare, uint256 newTeamShare, uint256 newBuyBackShare) external onlyOwner() {
        _liquidityShare = newLiquidityShare;
        _marketingShare = newMarketingShare;
        _teamShare = newTeamShare;
        _BuyBackShare = newBuyBackShare;

        _totalDistributionShares = _liquidityShare.add(_marketingShare).add(_teamShare).add(_BuyBackShare);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBTeam = amountReceived.sub(amountBNBLiquidity).sub(amountBNBBuyBack);
