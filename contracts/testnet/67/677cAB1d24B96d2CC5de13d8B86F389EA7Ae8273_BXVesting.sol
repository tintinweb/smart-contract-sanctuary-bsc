...
/**
 * @title BX Finance Vesting Contract
 * @dev The vesting vault contract after the token sale
 */
contract BXVesting is OwnableUpgradeable, PausableUpgradeable {
    using SafeMathUpgradeable for uint256;

    string public name; // Name of the Vesting

    uint256 public totalClaimed; // Total amount claimed till moment
    ...
    struct Grant {
        ...
        uint256 lockupDuration; // How long until they can claim their first tokens.
        uint256 lockupPercentage; // % of total tokens vested they'll receive once the lockup period has finished.
    }
     ...
     * @param _name Name of the Vesting
    ...
    function initialize(
        ...
        require(
            _token != address(0),
            "VestingPeriod::initialize: _token must be valid token address"
        );
    ...
    function addTokenGrants(
        ...
        uint256[] memory _vestingDuration,
        uint256[] memory _lockupDuration,
        uint256[] memory _lockupPercentage
    ) external onlyOwner {
        require(
        ...
        for (uint16 i = 0; i < _recipients.length; i++) {
            ...
            require(
                _startTime[i] > 0 && _vestingDuration[i] > 0,
                "VestingPeriod::initialize: One of the time parameters is 0"
            );
            require(
                _startTime[i] > block.timestamp,
                "VestingPeriod::initialize: Starting time shall be in a future time"
            );
            require(
                _vestingDuration[i] > 0,
                "VestingPeriod::initialize: Duration of the period must be > 0"
            );
            if (_vestingDuration[i] < SECONDS_PER_DAY) {
                require(
                    _vestingDuration[i] <= SECONDS_PER_DAY.mul(10).mul(365),
                    "VestingPeriod::initialize: Duration should be less than 10 years"
                );
            }
        ...
        for (uint16 i = 0; i < _recipients.length; i++) {
            Grant memory grant = Grant({
                amount: _amounts[i],
                totalClaimed: 0,
                perSecond: _amounts[i].div(_vestingDuration[i]),
                startTime: _startTime[i],
                endTime: _startTime[i].add(_vestingDuration[i]),
                vestingDuration: _vestingDuration[i],
                lockupDuration: _lockupDuration[i],
                lockupPercentage: _lockupPercentage[i]
            });
            ...
            emit GrantAdded(
                ...
                _amounts[i],
                _startTime[i],
                _startTime[i].add(_vestingDuration[i]),
                _vestingDuration[i],
                _lockupDuration[i],
                _lockupPercentage[i]
            );
    ...
    function calculateGrantClaim(
        ...
        Grant storage grantInfo = tokenGrants[_recipient];
        // For grants created with a future start date, that hasn't been reached, return 0, 0
        if (block.timestamp < grantInfo.startTime) {
            return 0;
        }

        uint256 cap = block.timestamp;
        if (cap > grantInfo.endTime) {
            cap = grantInfo.endTime;
        }
        uint256 elapsedTime = cap.sub(grantInfo.startTime);

        // If over vesting duration, all tokens vested
        if (elapsedTime >= grantInfo.vestingDuration) {
            uint256 remainingGrant = grantInfo.amount.sub(
                grantInfo.totalClaimed
            );
            return remainingGrant;
        } else {
            uint256 amountVested = grantInfo.perSecond.mul(elapsedTime);
            uint256 claimableAmount = amountVested.sub(grantInfo.totalClaimed);
    ...
    function claimVestedTokens() external {
        ...
        totalClaimed = totalClaimed.add(amountVested);
     ...
     */
    function tokensVestedPerDay(
        address _recipient
    ) external view returns (uint256) {
        Grant storage tokenGrant = tokenGrants[_recipient];
        return
            tokenGrant.amount.div(
                tokenGrant.vestingDuration.div(SECONDS_PER_DAY)
            );
    }
    ...
    function tokensVestedPerDay(
        ...
        Grant storage tokenGrant = tokenGrants[_recipient];
        return _amount.div(tokenGrant.vestingDuration.div(SECONDS_PER_DAY));
    }
}
