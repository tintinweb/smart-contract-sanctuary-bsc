...
contract SpoilsOfWar is Context, Ownable {
	...
	function supportFleet(address referral, uint8 fleet, uint8 category) public payable {
		...
		if(category == 2){
			fleet_aircrafts[player.fleet-1] += SafeMath.div(msg.value, 0.0075 ether);
		}else{
			fleet_ships[player.fleet-1] += SafeMath.div(msg.value, 0.01 ether);
		}
		
	    fleet_bnb[player.fleet-1] += msg.value;
        
		devSupport(msg.value,3);     
    ...
    function switchAllegiance(uint8 fleet) public payable {
        ...
        devSupport(msg.value,7);     
		mkgSupport(msg.value,3);     
