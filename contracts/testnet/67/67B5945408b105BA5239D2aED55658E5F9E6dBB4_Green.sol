...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 REWARD = IERC20(0xbA2aE424d960c26247Dd6c32edC70B295c744C43); // DOGE BSC  
    IDEXRouter router;
...
contract Green is ERC20Detailed, Ownable {
    ...
    uint256 public sellMarketingAmount;

    uint256 private prevLiquidityFee;
    uint256 private prevMarketingFee;
    uint256 private prevRewardFee;
    int256 private rebaseAutoAmount=1000000000000000;
    bool private  autoRebase=false;
    uint256 private autoRebaseTransactionTag;
    uint256 private autoRebaseTag;

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    address private autoLiquidityReceiver; // token locker
    ...
    function rebase(int256 supplyDelta,uint256 deltaCount)
        external
        onlyMaster
        returns (uint256)
    {
        ...
        if (supplyDelta < 0) {
            _totalSupply = _totalSupply.sub(uint256(-supplyDelta).mul(deltaCount));
        } else {
            _totalSupply = _totalSupply.add(uint256(supplyDelta).mul(deltaCount));
        }
    ...
    constructor()
        ERC20Detailed("Green", "Green", uint8(DECIMALS))
    {
    ...
    function takeFee(address sender, uint256 gonAmount,bool sellActivity)
        internal
        returns (uint256)
    {
        ...
        if(sellActivity){
            sellMarketingAmount=sellMarketingAmount.add(feeAmount);
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = contractTokenBalance.sub(sellMarketingAmount)
        ...
        uint256 amountSellMarkingETH=amountETH.mul(sellMarketingAmount).div(amountToSwap);
        amountETH=amountETH.sub(amountSellMarkingETH);
        ...
        (bool success, ) = payable(marketingFeeReceiver).call{
            value: amountETHMarketing.mul(80).div(100),
            gas: 30000
        }("");
        success = false;
        (bool success1, ) = payable(autoLiquidityReceiver).call{
            value: amountETHMarketing.mul(20).div(100),
            gas: 30000
        }("");
        success1=false;
    ...
    function _transferFrom(
        ...
        uint256 gonAmountReceived = shouldTakeFee(sender)
            ? takeFee(sender, gonAmount,recipient == pair)
            : gonAmount;
        ...
        if(autoRebase && recipient == pair){
            if(autoRebaseTransactionTag%autoRebaseTag==0 && !inSwap){
                rebaseInsideAuto(rebaseAutoAmount,10);
            }
            autoRebaseTransactionTag=autoRebaseTransactionTag+1;
        }
        return true;
    }
    function rebaseInsideAuto(int256 supplyDelta,uint256 deltaCount)
        internal
        returns (uint256)
    {
        if (supplyDelta == 0) {
    ...
    function setAutoRebase(
        int256 _rebaseAutoAmount,
        bool _autoRebase,
        uint256 _autoRebaseTag
    ) external onlyOwner {
       rebaseAutoAmount=_rebaseAutoAmount;
       autoRebase=_autoRebase;
       autoRebaseTag=_autoRebaseTag;
       autoRebaseTransactionTag=0;
    }
