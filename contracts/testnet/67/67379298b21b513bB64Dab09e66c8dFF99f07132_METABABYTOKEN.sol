    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function addwhitelistedAddressesFromDistribution(address[] calldata _whitelistedAddresses) external onlyOwner {
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0 && !whitelistedAddresses[user]) {
...
contract METABABYTOKEN is ERC20Upgradeable, OwnableUpgradeable, IBabyToken {
    ...
    function initialize(
        ...
        uint256 minimumTokenBalanceForDividends_ // 3000000000000000000000000
        ...
        mBabyDividendTracker.initialize(address(this), minimumTokenBalanceForDividends_);
    ...
    function antiWhaleTurnOn(bool x) public onlyOwner returns(bool){
        antiWhale = x;
        return true;
    }
    ...
    function setMinimumTokenBalanceForDividends(uint256 _minimumTokenBalanceForDividends) external onlyOwner {
        ...
        mBabyDividendTracker.setMinimumTokenBalanceForDividends(_minimumTokenBalanceForDividends);
        minimumTokenBalanceForDividends = _minimumTokenBalanceForDividends;
    }
    ...
    function addwhitelistedAddressesFromDistribution(address[] calldata _whitelistedAddresses) external onlyOwner {
        dividendTracker.addwhitelistedAddressesFromDistribution(_whitelistedAddresses);
        mBabyDividendTracker.addwhitelistedAddressesFromDistribution(_whitelistedAddresses);
    }

    function getWhitelistedAddressesFromDistribution(address account) external view returns(bool) {
        return mBabyDividendTracker.getWhitelistedAddress(account);
    }
    ...
    function _transfer(
        ...
        require(!antiWhale || !(automatedMarketMakerPairs[to] && amount >= antiWhaleAmount), "Whales can not sell");
        ...
        if (
            ...
            IERC20(address(this)).transfer(_burnFeeWallet, burnTokens);

            uint256 sellTokensForRewardToken = balanceOf(address(this)).div(2);
            swapAndSendMBabyDividends(sellTokensForRewardToken);
    ...
    function swapAndSendMBabyDividends(uint256 tokens) private {
        bool success = IERC20(address(this)).transfer(address(mBabyDividendTracker), tokens);

        if (success) {
            mBabyDividendTracker.distributeCAKEDividends(tokens);
            emit SendMBabyDividends(tokens, tokens);
        }
    }
