...
contract Timefour is Ownable {
    ...
    uint[5] public lockUps = [30 days,90 days,180 days, 360 days, 30 days];
    uint[2] public total; // 0- total deposited, 1- total withdrawn
    uint public reward = 51000000000000000000;
    ...
    function stake( uint amount, uint8 lockUp) external {        
        require((lockUp == 0) || (lockUp == 1) || (lockUp == 2) || (lockUp == 3), "lockup should be 0,1,2,3");
    ...
    function getReward( address account) public view returns (uint, uint) {
        ...
        uint totalMonth_ = (currentBlock - user_.lockTime) / lockUps[4];
