...
contract Charity is Ownable {
    ...
    function buyProduct(address _sponsorAddress, uint256 amount) external payable {
        ...
        uint256 corporateAmount = amountInEth - sponsorAmount - swapAmount;// - (uint256(2).mul(10 ** 18) / uint256(price).div(priceDivisor));
        ...
        uint256 _utilityDBMEwei = utilityDBME.mul(10 ** 18);
        uint256 utilityToBurn = _utilityDBMEwei.mul(burnPercentage).div(divisor);
        uint256 utilityToCharity = _utilityDBMEwei.mul(charityPercentage).div(divisor);
        uint256 utilityToUser = IERC20(DBME).balanceOf(address(this)) - utilityToBurn - utilityToCharity;
        //uint256 dbmeBalance = IERC20(DBME).balanceOf(address(this));
        //require(IERC20(DBME).balanceOf(address(this)) >= _utilityDBMEwei,"Insufficient DBME");
    ...
    function upgradeProduct(address _sponsorAddress, uint256 amount) external payable {
        ...
        uint256 corporateAmount = amountInEth.mul(100 - (sponsorPercentage + swapPercentage));
