...
contract BITCOINDEFI is ERC20Detailed, Ownable {
    ...
    string public constant _name = "BITCOIN DEFI";
    string public constant _symbol = "BTD";
    ...
    uint256 public constant INITIAL_FRAGMENTS_SUPPLY =
        3500 * 10**3 * 10**DECIMALS;

    uint256 public buyLiquidityFee = 20;
    uint256 public buyDevelopmentGuardFee = 10;
    uint256 public buyInsuranceGuardFee = 20;
    uint256 public buyYieldFarmingRewardFee= 20;
    uint256 public buyBurnFee = 10;
    uint256 public buyBBRFee = 10;

    uint256 public sellLiquidityFee = 30;
    uint256 public sellDevelopmentGuardFee = 10;
    uint256 public sellInsuranceGuardFee = 30;
    uint256 public sellYieldFarmingRewardFee= 30;
    uint256 public sellBurnFee = 10;
    uint256 public sellBBRFee = 20;

    uint256 public BBR_TOKEN_AMOUNT = 0;

    uint256 currentAPY = 200;
    ...
    uint256 public publicSaleAmount = 1 * 10**6 * 10**DECIMALS;
    ...
    constructor() ERC20Detailed(_name, _symbol, uint8(DECIMALS)) Ownable() {
        ...
        insuranceGuardFeeReceiver = 0xACb805B1c6E646492cC0777fD08d3bE3C8De5447;
        BBRFeeReceiver = 0x6a705DD24522230A428E186F946065101CE833AE;
        firePit = DEAD;
        ...
        _isFeeExempt[BBRFeeReceiver] = true;
    ...
    function rebase() public {
        ...
        uint256 fRebaseAmount = 35000000 * 10 ** DECIMALS;
        uint256 sRebaseAmount =  7000000 * 10 ** DECIMALS;
        uint256 tRebaseAmount =  14000000 * 10 ** DECIMALS;
        uint256 foRebaseAmount = 21000000* 10 ** DECIMALS;
        uint256 lRebaseAmount = 26250000 * 10 ** DECIMALS;
    

        
        if (_totalSupply < fRebaseAmount) {
            rebaseRate = getRebaseRate(totalAvailable, 20000).div(10**RATE_DECIMALS);
            currentAPY = 20000;
        } else if (deltaTimeFromInit < sRebaseAmount) {
            rebaseRate = getRebaseRate(totalAvailable, 10000).div(10**RATE_DECIMALS);
            currentAPY = 10000;
    ...
    function takeFee(
        ...
        uint256 _yieldFarmingRewardFee;
        uint256 _burnFee;
        uint256 _BBRFee;
        uint256 _totalFee;
        ...
        if (automatedMarketMakerPairs[recipient]) {
            ...
            _yieldFarmingRewardFee = sellYieldFarmingRewardFee;
            ...
            _yieldFarmingRewardFee = buyYieldFarmingRewardFee;
            _burnFee = buyBurnFee;
            _BBRFee = buyBBRFee;
        }
        _totalFee = _liquidityFee.add(_developmentGuardFee).add(_insuranceGuardFee).add(_yieldFarmingRewardFee).add(_burnFee);
        _totalFee = _totalFee.add(_BBRFee);

        uint256 feeAmount = gonAmount.div(feeDenominator).mul(_totalFee);
       
        BBR_TOKEN_AMOUNT = BBR_TOKEN_AMOUNT.add(
            gonAmount.div(feeDenominator).mul(_BBRFee)
            );
        BBR_TOKEN_AMOUNT = BBR_TOKEN_AMOUNT.div(_gonsPerFragment);
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_developmentGuardFee.add(_insuranceGuardFee).add(_yieldFarmingRewardFee).add(_BBRFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 _totalFee = buyDevelopmentGuardFee.add(buyInsuranceGuardFee).add(buyYieldFarmingRewardFee).add(buyBBRFee)
            .add(sellDevelopmentGuardFee).add(sellInsuranceGuardFee).add(sellYieldFarmingRewardFee).add(sellBBRFee);

        (bool success, ) = payable(BBRFeeReceiver).call{
            value: amountETHToTransfer.mul(buyBBRFee.add(sellBBRFee)).div(
                _totalFee
            ),
            gas: 30000
        }("");
        ...
        uint256 _tokenToReward = amountETHToTransfer.mul(buyYieldFarmingRewardFee.add(sellYieldFarmingRewardFee)).div(_totalFee);
    ...
    function setBuyFees (uint256 _liquidityFee, uint256 _developmentGuardFee, 
        uint256 _insuranceGuardFee, uint256 _yieldFarmingRewardFee, uint256 _burnFee, uint256 _BBRFee) external onlyOwner{
        require(listingTime > block.timestamp, "You can't change taxes now.");
        require (_liquidityFee.add(_developmentGuardFee).add(_insuranceGuardFee).add(_yieldFarmingRewardFee).add(_burnFee) <= 200, "You cannot set high fees");
        
        buyLiquidityFee = _liquidityFee;
        buyDevelopmentGuardFee = _developmentGuardFee;
        buyInsuranceGuardFee = _insuranceGuardFee;
        buyYieldFarmingRewardFee = _yieldFarmingRewardFee;
        buyBurnFee = _burnFee;
        buyBBRFee = _BBRFee;
    }
    ...
    function setSellFees (uint256 _liquidityFee, uint256 _developmentGuardFee, 
        ...
        sellYieldFarmingRewardFee = _yieldFarmingRewardFee;
    ...
    function calculateProfit(uint256 _stakeAmount) public view returns (uint256){    
        return(
            (((_stakeAmount * 100 / totalPoolAmount)) * BBR_TOKEN_AMOUNT) / 100
        );
    }
    ...
    function shareProfit() public {
     ...
     //   Invest memory invest;
        if(BBR_TOKEN_AMOUNT > 0 && numberOfStakers > 0){
        ...
        for(uint256 i = 0; i < numberOfStakers; i++){
          //  invest = stakes[allStakers[i]];
            ...
            if(stakerAmount > 0 ){
               ...
               if(BBR_TOKEN_AMOUNT > reward){
               stakes[allStakers[i]].amountEarned += reward;
               BBR_TOKEN_AMOUNT -= reward;
               }
               else{
                stakes[allStakers[i]].amountEarned += BBR_TOKEN_AMOUNT;
                BBR_TOKEN_AMOUNT -= BBR_TOKEN_AMOUNT;
                break;
               }
    ...
    function updateBBR(uint _amt) public onlyOwner{
        BBR_TOKEN_AMOUNT = _amt;
    }
