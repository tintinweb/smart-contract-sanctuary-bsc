...
/// @title RecurringPullPaymentWithFreeTrial
	...
	IVersionedContract
{
	...
	function createBillingModel(
	...
	) public override returns (uint256 billingModelID) {
		...
		_billingModels[newBillingModelID].trialPeriod = _trialPeriod;
		...
		returns (uint256 subscriptionID)
	{
		...
		bm.subscriptions[newSubscriptionID].nextPaymentTimestamp = block.timestamp + bm.trialPeriod;
	...
	/**
	 *@notice This method is used to cancel the subscription for given subscription id
	 *@param _subscriptionID indicates the subcription id
	 */
		...
		returns (uint256 subscriptionID)
	{
		...
		require(
			...
			'RecurringPullPaymentWithFreeTrial: INVALID_SUBSCRIBER'
		);
	...
	function editBillingModel(
		...
		if (_newPayee != address(0)) {
			require(
				...
				'RecurringPullPaymentWithFreeTrial: ALREADY_PAYEE'
			);
		...
		returns (BillingModelData memory bm)
	{
		...
		bm.trialPeriod = _billingModels[_billingModelID].trialPeriod;
	...
	/**
	 *@notice This method tells you whether user`s free trial is ended or not.
	 *@param _subscriptionID indicates the subscription ID.
