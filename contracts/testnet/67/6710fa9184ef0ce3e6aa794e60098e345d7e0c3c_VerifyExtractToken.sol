...
contract VerifyExtractToken is Comn{

    event VerifyCall(uint seed);
    ...
    // 提取验证
    function extractVerify(address sender,string memory amountToStr,string memory seed, bytes32 _r, bytes32 _s,uint8 _v) external isRuning isMember isBlack nonReentrant  returns(bytes memory signaturn){
        ...
        bytes32 _hashedMessage = keccak256(abi.encodePacked(senderStr,amountToStr,seed));
        address signer = verifyMessage(_hashedMessage,_v,_r,_s);
        if(signer != signerAddr){ _status = _NOT_ENTERED; revert("Verify : signer error"); }
        uint amountToWei = stringToUint(amountToStr);
        uint seedToUint = stringToUint(seed);
        extractToken(sender,amountToWei,signaturn);
        emit VerifyCall(seedToUint);
    }

    //提取代币
    function extractToken(address sender,uint amountToWei,bytes memory signaturn) private{
        ERC20(targetContract).transfer(sender, amountToWei);
