...
contract Amm is IAmm, OwnableUpgradeable, BlockContext {
    ...
    LiquidityChangedSnapshot[] internal liquidityChangedSnapshots;
    ...
    function shutdown() external override {
        require(
            ...
            "Amm: not owner nor globalShutdown"
        );
        implShutdown();
    }
    ...
    function setCounterParty(address _counterParty) external onlyOwner {
        require(_counterParty != address(0), "Amm: zero address");
    ...
    /**
     * @notice set fluctuation limit rate.
     * @dev only owner can call this function
    ...
    function setSpotPriceTwapInterval(uint256 _interval) external onlyOwner {
        require(_interval != 0, "Amm: can not set interval to 0");
    ...
    function setPriceFeed(IPriceFeed _priceFeed) external onlyOwner {
        require(address(_priceFeed) != address(0), "Amm: zero address");
        priceFeed = _priceFeed;
