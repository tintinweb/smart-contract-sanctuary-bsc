...
contract Crowdfunding is AccessControlEnumerable {
    ...
    function fund(uint256 _regionIndex, uint256 _amount) external {
        ...
        uint256 accountAmountLeft = getMaxAmountPerTime(p.targetAmount) - pf.fundAmount;
    ...
    function reserve(
        ...
        uint16 version = p.version;
        ProjectFund storage pf = projectFunds[msg.sender][_regionIndex][_round];
        if (pf.version == version && pf.reserveAmount > 0) revert ProjectAmountError();
        require(_amount <= getMaxAmountPerTime(p.targetAmount), "account max amount limited");
    ...
    function resetProjects(uint256 _regionIndex, uint256 _round) internal {
        ...
        for (uint256 i = _round + 1; i < _length; i++) {
            ...
            ++p.version;
            if (i == _round + 1) {
                p.targetAmount = (regions[_regionIndex].restartType == 1 && regions[_regionIndex].poolAmount > 2e18)
                    ? (regions[_regionIndex].poolAmount * 500) / 1000
                    : regions[_regionIndex].initAmount;
            } else {
                p.targetAmount = (projects[_regionIndex][i - 1].targetAmount * _multiplier) / 1000;
            }
            p.currentAmount = 0;
            p.reserveAmount = 0;
    ...
    //单次最大投资金额
    function getMaxAmountPerTime(uint256 _targetAmount) public view returns (uint256 maxAmountPerTime) {
        uint256 _max = (_targetAmount * rates[9]) / 1000;
