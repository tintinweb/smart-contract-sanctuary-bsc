...
// File: contracts/interfaces/game/IRange.sol
...
interface IRange {
     ...
     * @dev set the result score range of which match of which round trigger when num0-num1

     */

    event SetWhichGamesRange(uint256 indexed round, uint256 indexed which, uint256 indexed index, uint256 num0, uint256 num1);



    /**

     * @dev trigger when abnormal results

     */

    event EndError(bool indexed endError);



    /**

     * @dev start new round, new round includes few matches, index results (if force is true, means force to open next round, even former round did not end)
     ...
     * @dev set the range of score num0 ~ num1 of index of result of which match of which round (including num0, num1 not included. e.g. when num0=5, num1=8, so score 5,6,7 is included)

     */

    function setWhichGamesRange(uint256 round, uint256 which, uint256 index, uint256 num0, uint256 num1) external;



    /**

     * @dev Query the score range num0-num1 of which result of which match of which round 

     */

    function searchWhichGamesRange(uint256 round, uint256 which, uint256 index) external view returns (uint256 num0, uint256 num1);
...
contract Range is BaseFunction, IRange {
    ...
    struct RangeScoreInfo {

        mapping(uint256 => RangeScore) rangeScore; // score range

    }



    struct RangeScoreOnceRoundInfo {

        mapping(uint256 => RangeScoreInfo) rangeScoreMap;

    }



    struct RangeScoreRoundInfo {

        mapping(uint256 => RangeScoreOnceRoundInfo) roundRangeScoreMap;

    }



    RangeScoreRoundInfo internal _rangeScoreRoundInfo;



    uint256 constant NORMAL_RESULT_NUM = 2; // guessing the score range，normally only 2 set of range

    uint256 constant RESULT_LIST_LENGTH = 1; // only 1 score
    ...
    function startNewRound(uint256 few, bool force) public virtual override onlyOwner {
        ...
        if (force == false) {
            ...
            for (uint256 i = 0; i < state.length; ++i) {
                ...
                require(state[i] == GAME_STATE_END || state[i] == GAME_STATE_END_INVAL, "Range: startNewRound current round not end");
    ...
    function setWhichGamesRange(uint256 round, uint256 which, uint256 index, uint256 num0, uint256 num1) public virtual override onlyOwner {

        round = _setRound(round);



        require(round < _gameRoundInfo.roundNum, "Range: setWhichGamesConced Illegal round");

        require(which < _gameRoundInfo.roundGameMap[round].gameNum, "Range: setWhichGamesConced Illegal which");

        require(GAME_STATE_INIT == _gameRoundInfo.roundGameMap[round].gameMap[which].gameState, "Range: setWhichGamesConced gameState is not init");

        require(index < _gameRoundInfo.roundGameMap[round].gameMap[which].resultNum, "Range: setWhichGamesConced Illegal index");

        require(num0 < num1, "Range: setWhichGamesConced Illegal num0 and num1");



        _rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[index].start = num0;

        _rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[index].end = num1;



        emit SetWhichGamesRange(round, which, index, num0, num1);
    ...
    function searchWhichGamesRange(uint256 round, uint256 which, uint256 index) public view override returns (uint256 num0, uint256 num1) {
        ...
        return (_rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[index].start, _rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[index].end );
    ...
    function setWhichGamesResultNumber(uint256 round, uint256 which, uint256 number) public virtual override onlyOwner {
        ...
        require(round < _gameRoundInfo.roundNum, "Range: setWhichGamesResultNumber Illegal round");

        require(which < _gameRoundInfo.roundGameMap[round].gameNum, "Range: setWhichGamesResultNumber Illegal which");

        require(GAME_STATE_INIT == _gameRoundInfo.roundGameMap[round].gameMap[which].gameState, "Range: setWhichGamesResultNumber gameState is not init");

        require(number >= NORMAL_RESULT_NUM, "Range: setWhichGamesResultNumber number is not >= NORMAL_RESULT_NUM");
    ...
    function _startNewRound(uint256 round, uint256 few) internal virtual override onlyOwner returns (uint256 currentRound) {
        ...
        require(round == _gameRoundInfo.roundNum, "Range: _startNewRound Illegal round");
     ...
     * @dev Start the proprocessing function of betting

     */

    function _beforeStartBetWhichlGames(uint256 round, uint256 which) internal virtual override {

        super._beforeStartBetWhichlGames(round, which);



        for (uint256 i = 0; i < _gameRoundInfo.roundGameMap[round].gameMap[which].resultNum; ++i) {

            require(_rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[i].start < _rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[i].end, 

                "Range: _beforeStartBetWhichlGames rangeScore is error");
    ...
    function _beforeEndWhichlGames(uint256 round, uint256 which) internal virtual override {
        ...
        require(_gameRoundInfo.roundGameMap[round].gameMap[which].resultList.length == RESULT_LIST_LENGTH, "Range: _beforeEndWhichlGames resultList.length is error");



        bool endError = true;

        uint256 score = _gameRoundInfo.roundGameMap[round].gameMap[which].resultList[0];



        for (uint256 i = 0; i < _gameRoundInfo.roundGameMap[round].gameMap[which].resultNum; ++i) {

            if ((score >= _rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[i].start) && 

                (score < _rangeScoreRoundInfo.roundRangeScoreMap[round].rangeScoreMap[which].rangeScore[i].end)) {
        ...
        if (endError == true) {
            ...
            emit EndError(endError);

            invalWhichlGames(round, which);
