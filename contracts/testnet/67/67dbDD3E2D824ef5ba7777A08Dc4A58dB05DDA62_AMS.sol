...
contract AMS is ERC20, Ownable {
    ...
    function callCoverAmsUser() public {
        if (sbefReceiveAms[msg.sender] == false) { //用户的50%只能领取一次，拿去添加lp
            uint256 balance_ = ERC20(laoAMs).balanceOf(msg.sender);
            uint256 _num = balance_ / 2;
            ERC20(laoAMs).transferFrom(msg.sender, address(0), balance_);
    ...
    function getArrayInfo2(uint256 _num) public view returns (address[] memory, address[] memory, uint256[] memory){
        ...
        return (receiveAmsUser, recordAmsUser, _swapaftadd);
    }
