...
contract SgramLAAS is ERC721Metadata {
    ...
    modifier periodStarted() {
        require(block.timestamp >= startTime && startTime != 0, "SgramLAAS: Period not started");
        _;
    }
    ...
    constructor(
        string memory baseURI_,
        address admin_
    ) ERC721Metadata("Sgram LAA", "SgramLAAS", baseURI_) {
        _admin = admin_;
    }

    function setStartTime(uint256 _startTime) external onlyOwner {
        require(_startTime > 0, "SgramLAAS: invalid _startTime");
        require(_startTime > block.timestamp, "SgramLAAS: old start time");
    ...
    function presalePurchase(uint256 numberOfNfts) external payable onlyWhitelisted{
        ...
        require(pendingCount > 0, "SgramLAAS: All minted");
        require(numberOfNfts > 0, "SgramLAAS: numberOfNfts cannot be 0");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"SgramLAAS: sale already ended");
        require(MINT_PRICE * (numberOfNfts) == msg.value, "SgramLAAS: invalid ether value");
    ...
    function purchase(uint256 numberOfNfts) external payable periodStarted {
        require(pendingCount > 0, "SgramLAAS: All minted");
        require(numberOfNfts > 0, "SgramLAAS: numberOfNfts cannot be 0");
        require(numberOfNfts <= 20, "SgramLAAS: You may not buy more than 20 NFTs at once");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"SgramLAAS: sale already ended");
        if(tax_freeUsers[msg.sender] == 0) require(MINT_PRICE * (numberOfNfts) == msg.value, "SgramLAAS: invalid ether value");
        else if (tax_freeUsers[msg.sender] < numberOfNfts){
            require(MINT_PRICE * (numberOfNfts -  tax_freeUsers[msg.sender]) == msg.value, "SgramLAAS: invalid ether value");
    ...
    function claimReward(uint256 tokenId) public {
        require(
            ...
            "SgramLAAS: Only owner or approved can claim rewards"
        );
    ...
    function _randomMint(address _to) internal returns (uint256) {
        require(totalSupply() < MAX_NFT_SUPPLY, "SgramLAAS: max supply reached");
