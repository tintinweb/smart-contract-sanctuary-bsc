...
contract PoolsReferral is AccessControl {
    ...
    function verifyMessage(
        ...
        bytes32 hashedMessage = keccak256(
            abi.encode(_value, _sender, userNonce[_sender]++, _block)
        );
    ...
    function claimReward(
        ...
        require(block.number <= _block, "Signature is expired");
        require(
            verifyMessage(_amount, msg.sender, _block, _signature),
            "Invalid signature"
        );

        IERC20(supportedToken).transferFrom(bank, msg.sender, _amount);
