...
contract Cointainer {
    ...
    ERC20 USDT;
    // time
    uint256[4] LOCK_TIME = [ 1 days, 2 days, 3 days, 4 days];// [ 91 days, 182 days, 273 days, 365 days]; // TIME_LOCK;
    uint256 RETORNE = 30 minutes;//30 days;
    // Percentage
    ...
    function INVETS(uint256 amount, uint256 ref, uint8 TYPE) public NoContract{
        ...
        PROFIT_BALANCE[msg.sender].push(CALCULATE_UI(amount, GAIN_TIME[Quantity_Claims(TYPE)], 2));
    ...
    function PROFIT(uint256 index) public NoContract{
        ...
        if(MY_BATCH[msg.sender][index].TIME + LOCK_TIME[MY_BATCH[msg.sender][index].TYPE] > block.timestamp 
            ...
            BALANCE[msg.sender] += (free/Quantity_Claims(MY_BATCH[msg.sender][index].TYPE));
        ...
        }else if(MY_BATCH[msg.sender][index].TIME + LOCK_TIME[MY_BATCH[msg.sender][index].TYPE]<=block.timestamp
            && MY_BATCH[msg.sender][index].CLAIMS >= 0){
