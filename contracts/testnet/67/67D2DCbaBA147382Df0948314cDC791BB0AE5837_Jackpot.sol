...
interface I_HashGame {
    function resetUserConsecutiveBurstCount(address) external;
    function getUserConsecutiveBurstCount(address) external view returns(uint256); 
    function getUserMvccCurrentGameRound(address _userAddress) external view returns(uint256);
}
...
contract Jackpot is ReentrancyGuard {
    ...
    function purchaseJackpot(uint256 _gid, address _userAddress) external onlyOperator {
        ...
        require(!isValidJackpot(_userAddress), "Jackpot active, cannot purchase again");
        ...
        if(IHashGame.getUserConsecutiveBurstCount(_userAddress) > 0)
            IHashGame.resetUserConsecutiveBurstCount(_userAddress);
    ...
    function purchaseGrandJackpot() external {
        ...
        require(!isValidGrandJackpot(msg.sender), "Grand jackpot active, cannot purchase again");

        // Check user current Mvcc game round
        uint256 _currentGameRound = IHashGame.getUserMvccCurrentGameRound(msg.sender);
        require(_currentGameRound == 0, "Cannot purchase grand jackpot after new round started");

        // Check user consecutive burst count 
        uint256 _burstCount = IHashGame.getUserConsecutiveBurstCount(msg.sender);
        require(_burstCount == 2, "Consecutive burst count must be 2");
    ...
    function claimJackpot() external nonReentrant lockTheJackpot {
        ...
        // Reset user consecutive burst count
        IHashGame.resetUserConsecutiveBurstCount(msg.sender);
    ...
    function wonJackpot(address _userAddress) public view returns(bool) {
        bool _isValid = isValidJackpot(_userAddress);
        uint256 _burstCount = IHashGame.getUserConsecutiveBurstCount(_userAddress);

        if(_isValid && _burstCount == 2) {
            return true;
        } 
        return false;
    }
    ...
    function wonGrandJackpot(address _userAddress) public view returns(bool) {
        bool _isValid = isValidGrandJackpot(_userAddress);
        uint256 _burstCount = IHashGame.getUserConsecutiveBurstCount(_userAddress);

        if(_isValid && _burstCount == 3) {
            return true;
        } 
        return false;
    }
    ...
    function getPurchaseGrandJackpotFlag(address _userAddress) external view returns(bool) {
        uint256 _currentGameRound = IHashGame.getUserMvccCurrentGameRound(_userAddress);
        uint256 _burstCount = IHashGame.getUserConsecutiveBurstCount(_userAddress);

        if(isValidJackpot(_userAddress) && !isValidGrandJackpot(_userAddress) && _currentGameRound == 0 && _burstCount == 2)
            return true;
        else
            return false;
    } 
