...
contract CryptoCarouselBitSpin{
    ...
    function startGame(address _ref) payable public returns(historyItem[] memory){
        ...
        uint256 sum = 0;
        historyItem[] memory out = new historyItem[](countGame);
        for(uint256 i = 0; i < countGame; i++){
            (res, spin) = getWin();
            if(res > 0)
                sum = getMoney(res);
            history[countHistory] = historyItem(msg.sender, balance, sum, spin, res);
            out[i] = historyItem(msg.sender, balance, sum, spin, res);
            countHistory++;
        }
        return out;
    }
