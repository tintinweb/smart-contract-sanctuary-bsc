...
contract TSLVault is Initializable, OwnableUpgradeable, ReentrancyGuard {
    ...
    event Deposit(address indexed user, uint256 indexed amount, address token, address firstInviter);
        ...
        initializer
    {
        ...
        referralConfig.first = 30;
        referralConfig.second = 20;
        referralConfig.team = 5;
        referralConfig.threshold = 5000;
        referralConfig.num = 12;
    }
    ...
    function deposit(
        ...
        //burn token TODO
        // _swap(LevelBalance[level_].mul(10).div(100), tokenPath[token], address(0));
        ...
        referralReward[firstInviter] += depositAmount
            .mul(referralConfig.first)
            .div(1000);
        if (userInviters.length > 1) {
            address secondInviter = userInviters[1];
            referralReward[secondInviter] += depositAmount
                .mul(referralConfig.second)
                .div(1000);
        }
        if (userInviters.length > 2) {
            for (uint256 i = 2; i < userInviters.length; i++) {
                ...
                if (
                    ...
                    referralReward[userInviters[i]] += depositAmount
                        .mul(referralConfig.team)
                        .div(1000);
                }
            }
            //deal inviter info
            userInvitedValue[userInviters[0]] += depositAmount;
        }
    ...
    function getUserEarned(address user) public view returns (uint256) {
        UserDepositInfo[] memory userInfo = userDepositInfo[user];
        uint256 amount;
        for (uint256 i = 0; i < userInfo.length; i++) {
            if (userInfo[i].endTimestamp < block.timestamp)
                amount += userInfo[i]
                    .value
    ...
    function getUserDepositInfo(address user) public view returns(UserDepositInfo[] memory) {
