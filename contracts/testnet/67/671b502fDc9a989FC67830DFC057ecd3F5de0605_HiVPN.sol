...
contract HiVPN is IHiVPN, Payable, Tokenized {
  ...
  function pay(
    ...
    if (token == address(0)) {
      ...
      require(userTokenBalance(_msgSender(), token) >= amount, "BAL");
      require(userTokenAllowance(_msgSender(), token) >= amount, "APR");
    }

    uint256 value = toUSD(token, amount);

    require(isEnough(usd, value), "VAL");

    // Emit here to make it first event for easy reading
    ...
    if (token == address(0)) {
      ...
      IERC20(token).transferFrom(_msgSender(), admin, adminValue);
      IERC20(token).transferFrom(_msgSender(), owner, feeValue);
    }
  ...
  function isEnough(uint256 usd, uint value) public view override returns (bool) {
    uint min = usd.mul(100 - tolerance).div(100);
