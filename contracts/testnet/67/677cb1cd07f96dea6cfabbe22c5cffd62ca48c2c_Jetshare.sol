...
contract JetshareDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("Jetshare_Dividend_Tracker", "Jetshare_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Jetshare_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Jetshare_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Jetshare contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Jetshare_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "Jetshare_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Jetshare is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    JetshareDividendTracker public dividendTracker;
    ...
    uint256 private onSelltotalFees;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _burnFee) public onlyOwner {
        buyBNBRewardsFee = _bnbRewardFee;
        buyLiquidityFee = _liquidityFee;
        buyMarketingFee = _marketingFee;
        buyBurnFee = _burnFee;

        totalFees = buyBNBRewardsFee.add(buyLiquidityFee).add(buyMarketingFee).add(buyBurnFee); // total fee transfer and buy
    ...
    function setSellFee(uint256 _onSellbnbRewardFee, uint256 _onSellliuidityFee, uint256 _onSellMarketingFee, uint256 _onSellBurnFee) public onlyOwner {
        onSellBNBRewardsFee = _onSellbnbRewardFee;
        onSellliquidityFee = _onSellliuidityFee;
        onSellmarketingFee = _onSellMarketingFee;
        onSellburnFee = _onSellBurnFee;

        onSelltotalFees = onSellBNBRewardsFee.add(onSellliquidityFee).add(onSellmarketingFee).add(onSellburnFee);
        require(onSelltotalFees <= 200, "Must keep fees at 20% or less");
    }
    ...
    constructor() ERC20("Jetshare", "JETT$") {
        buyBNBRewardsFee = 10;
        buyLiquidityFee = 15;
        buyMarketingFee = 15;
        buyBurnFee = 10;

        onSellBNBRewardsFee = 20;
        onSellliquidityFee = 30;
        onSellmarketingFee = 30;
        onSellburnFee = 20;
    	...
    	dividendTracker = new JetshareDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "Jetshare: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "Jetshare: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "Jetshare: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }
     
    function setSWapToensAtAmount(uint256 newAmount) public onlyOwner {
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "Jetshare: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "Jetshare: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Jetshare: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function _transfer(
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(automatedMarketMakerPairs[from]){
                fees = amount.mul(totalFees.sub(buyBurnFee)).div(1000);
                burnShare = amount.mul(buyBurnFee).div(1000);
            }

            if(automatedMarketMakerPairs[to]) {
                fees = amount.mul(onSelltotalFees.sub(onSellburnFee)).div(1000);
                burnShare = amount.mul(onSellburnFee).div(1000);
            }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensToAddLiquidityWith = contractTokenBalance.div(totalFees.mul(2)).mul(buyLiquidityFee);
        ...
        uint256 bnbToAddLiquidityWith = deltaBalance.mul(buyLiquidityFee).div(totalFees.mul(2).sub(buyLiquidityFee));
        
        // add liquidity to uniswap
        addLiquidity(tokensToAddLiquidityWith, bnbToAddLiquidityWith);
        // worthy marketing fee
        uint256 marketingAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(buyLiquidityFee)).mul(buyMarketingFee);
