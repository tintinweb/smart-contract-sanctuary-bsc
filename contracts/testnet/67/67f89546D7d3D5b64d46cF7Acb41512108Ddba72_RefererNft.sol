...
contract RefererNft is ERC721, AccessControl,ReentrancyGuard,IEvents {
    ...
    constructor(IERC20 _token, address _admin) ERC721 ("referral_nft", "nft") {
 ...
 */
    function mint(address to, uint counter) public nonReentrant{
        counter = 1;
 ...
 */
    function Reward_Percentage( address user,  uint128[3] memory amount) public nonReentrant onlyRole(operatorRole) {
        require(user != address(0), "ReferralNFT: Null Address");
 ...
 */
    function rewardDistribution(address user, uint _reward) public nonReentrant onlyRole(operatorRole) returns(uint) {
 ...
 */
    ...
    function buy(address user, uint counter) public {
        ...
        token.transferFrom(user, admin.Treasury() , price); //price as variable
        mint(user, counter);
 ...
 */
    function reActivateNFT(address _address) public {
        ...
        token.transferFrom(msg.sender, admin.Treasury(), price);
        users[_address].lockPeriod = uint128 (block.timestamp + 1000 seconds);
        emit ReactivateNFT("ReferralNft", _address, users[_address].tokenId, block.number, block.timestamp);
    }
/**
