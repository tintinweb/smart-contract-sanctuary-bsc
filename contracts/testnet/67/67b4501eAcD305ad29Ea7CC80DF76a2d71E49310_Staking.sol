...
contract Staking {
    ...
    function sortAmount(uint256 _amount) internal pure returns (AmountPlane) {
        if (_amount < 100_000 * 1e18) {
            return AmountPlane.Small;
        } else if (_amount < 1_000_000 * 1e18) {
    ...
    function checkPlane(AmountPlane plane, uint256 day)
        internal
        pure
        returns (uint256 procents)
    {
        if (day == 90) {
            procents = checkSmallRate(plane);
        } else if (day == 180) {
            procents = checkMediumRate(plane);
        }
        procents = checkLargeRate(plane);
    }
