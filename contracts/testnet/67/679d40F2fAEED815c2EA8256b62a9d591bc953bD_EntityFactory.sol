...
contract EntityFactory is IEntityFactory, Ownable {
  ...
  address public immutable swapMile;

  address public immutable mileToken;

  address public immutable sMileToken;
    ...
    onlyOwner
  {
    ...
    emit UpdateUpgradeableBeacon(newBeacon, entityType);
  }
  ...
  function updateWhiteList(address newWhiteList) external onlyOwner {
    ...
    emit UpdateWhiteList(oldWhiteList, newWhiteList);
  }
  ...
  /// @param id Id of entity sended from BE;
  /// @return the address of the created proxy.
  function createCompanyEntity(uint256 id)
    external
    onlyUserEntity
    returns (address)
  {
  ...
  /// @param id Id of entity sended from BE;
  /// @return the address of the created proxy.
  function createProjectEntity(uint256 id)
    external
    onlyUserEntity
    returns (address)
  {
...
contract UserEntity is BaseEntity, IUserEntity {
  ...
  function createCompanyEntity(uint256 id)
    ...
    returns (address)
  {
    address newEntity = entityFactory.createCompanyEntity(id);
  ...
  function createProjectEntity(uint256 id)
    ...
    returns (address)
  {
    address newEntity = entityFactory.createProjectEntity(id);
    childeEntities[newEntity] = true;
  ...
  function stake(uint256 amount, bool fromSender)
    external
