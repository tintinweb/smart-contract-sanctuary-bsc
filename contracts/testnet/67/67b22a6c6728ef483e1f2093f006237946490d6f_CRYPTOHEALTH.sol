...
contract CRYPTOHEALTH is Context, IERC20, Ownable {
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        uint256 currentRate = _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        _rOwned[Wallet_Treasury] = _rOwned[Wallet_Treasury].add(rLiquidity);
        if (_isExcluded[Wallet_Treasury])
            _tOwned[Wallet_Treasury] = _tOwned[Wallet_Treasury].add(tLiquidity);
    }
