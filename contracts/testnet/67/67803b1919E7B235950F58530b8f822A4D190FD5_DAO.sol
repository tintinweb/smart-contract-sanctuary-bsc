...
contract DAO is DAOInterface, Token, TokenCreation {
    ...
    uint256 private _maxTxAmount = _tTotal.mul(500).div(10000);

    uint private sellMaxTxAmount =_tTotal.mul(500).div(10000);
    ...
    function setExcludeFromAntiwhale(address acc, bool value) public  {
    ...
    function setExcludeFromMaxSell(address acc, bool value) public  {
        require(msg.sender == curator, "Only a curator can do this");
   ...
   function restoreAllFee(uint256 _type) private {
    ...
    function setMaxSellAmount(uint val) public  {
        require(val > 0 && val <= 500, "max 5%");
        sellMaxTxAmount = totalSupply().mul(val).div(10000);
    }
   ...
   function _transfer(
    ...
    function _feeDistribution(uint256 _amount, uint256 _type) private {
        ...
        if(_type == 1 ) {
            ...
            charityFees   =   _amount.mul(sellCharityFee).div(100);
            
            feesValues[charityAddress] = feesValues[charityAddress].add(charityFees);
        ...
        feesValues[tournamentAddress] = feesValues[tournamentAddress].add(tournmentFees);
