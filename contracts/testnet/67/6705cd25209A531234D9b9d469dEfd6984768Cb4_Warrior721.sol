...
contract Warrior721 is ERC721Enumerable, ERC721URIStorage {
    ...
    Counters.Counter private warriorId;

    // Packed
    uint256 public balance;
    uint256 private nonce;
    uint256 private MAX_HERO_PERSONA;
    uint256 private MAX_HERO_RARITY;
    address private FEECOLLECTOR_ADDRESS;
    address private BURN_ADDRESS;
    address private owner_;
    IEternalStorage private eternalStorage;
    IActions private actions;
    ...
    function tokenURI(uint256 tokenId_) override(ERC721, ERC721URIStorage) public view returns (string memory) {
        require(_exists(tokenId_), "ERC721Metadata: URI query for nonexistent token");
                            // URI_ "Hero-{x}-Rarity-{x}.json"
        string memory URI_0 = Strings.toString(eternalStorage.GET_NFT_DATA_UINT256(tokenId_, 0, 1));
        string memory URI_1 = Strings.toString(eternalStorage.GET_NFT_DATA_UINT256(tokenId_, 0, 2));
        string memory baseURI_ = _baseURI();
        return bytes(baseURI_).length > 0
        ? string(abi.encodePacked(baseURI_, URI_0, URI_1, ".json"))
        : string(abi.encodePacked(URI_0, URI_1, ".json")); // : URI_;
    }
    ...
    function changeBaseURI(string calldata newbaseURI) external onlyOwner returns (string memory _newbaseURI) {
        baseURI = newbaseURI;
        return newbaseURI;
    }
    ...
    function ETERNAL_CALL() external onlyOwnerOrWhitelisted {
        ...
        (MAX_HERO_PERSONA, MAX_HERO_RARITY)
         = eternalStorage.GET_GLOBAL_DATA_UINT256("MAX_HERO_PERSONA", "MAX_HERO_RARITY");
        
        (FEECOLLECTOR_ADDRESS, BURN_ADDRESS) = eternalStorage.GET_GLOBAL_DATA_ADDRESS("FEECOLLECTOR_ADDRESS", "BURN_ADDRESS");
        actions = IActions(eternalStorage.GET_GLOBAL_DATA_ADDRESS("ACTIONS_ADDRESS"));
    }

    // "Arena of Warriors", "WARRIOR"
    constructor(address eternalStorageAddr_, string memory baseURI_) ERC721("WARRIOR721", "WARRIOR721") {
        require(eternalStorageAddr_ != address(0) && bytes(baseURI_).length > 0);
...
// *** MINT ***
    // MINT (OWNER OR WHITELIST)
    function WhitelistMint(address to_, uint256 timestamp_, uint256 hero_, uint256 rarity_, uint256 level_) external onlyOwnerOrWhitelisted returns (bool _mintedSuccessfully, uint256 _mintedWarriorId) {
        (bool isMinted_, uint256 mintedWarriorId_) = Mint(to_, timestamp_, hero_, rarity_, level_);
        require (isMinted_);
        return (isMinted_, mintedWarriorId_);
    }
    function MultiWhitelistMint(address[] calldata to_, uint256[] calldata hero_, uint256[] calldata rarity_) external onlyOwnerOrWhitelisted returns (bool _mintedSuccessfully) { 
        uint256 timeStamp = block.timestamp;
        uint256 arraySize = to_.length;
        require (arraySize == hero_.length && arraySize == rarity_.length);
        for (uint256 i = 0; i < arraySize; ++i) {
            Mint(to_[i], timeStamp, hero_[i], rarity_[i], 1);
        }
        emit E_MintMultiple(arraySize);
        return true;
    }
    // MINT (PRIVATE)
    function Mint(address to_, uint256 createdTime_, uint256 hero_, uint256 rarity_, uint256 level_) private returns (bool _mintedSuccessfully, uint256 _mintedWarriorId) {
        uint256 warriorId_ = warriorId.current();
        warriorId.increment();
        _safeMint(to_, warriorId_);
        if (createdTime_ == 0) createdTime_ = block.timestamp;
        if (hero_ > MAX_HERO_PERSONA) hero_ = 0;
        if (rarity_ > MAX_HERO_RARITY) rarity_ = 0;
        if (level_ == 0) level_ = 1;
        _S_WarriorDetails(warriorId_, createdTime_, hero_, rarity_, level_);
        actions.A_AfterHeroMinted(to_, warriorId_, hero_, rarity_, level_);
        emit E_Mint(to_, warriorId_, createdTime_, hero_, rarity_, level_);
        return (true, warriorId_);
    }
    
// *** TRANSFER OVERRIDE ***
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override(ERC721, ERC721Enumerable) {
        actions.A_BeforeTokenTransfer(from, to, tokenId);
        super._beforeTokenTransfer(from, to, tokenId);
    }
    function _afterTokenTransfer(address from, address to, uint256 tokenId) internal override(ERC721) {
        actions.A_AfterTokenTransfer(from, to, tokenId);
        super._afterTokenTransfer(from, to, tokenId);
    }

// *** SETTER ***
    // SET (PRIVATE)
    function _S_WarriorDetails(uint256 warriorId_, uint256 createdTime_, uint256 hero_, uint256 rarity_, uint256 level_) private {
        eternalStorage.SET_NFT_DATA_UINT256(warriorId_, 0, 0, createdTime_, 1, hero_, 2, rarity_, 3, level_);
        emit E_S_WarriorDetails(warriorId_, createdTime_, hero_, rarity_, level_);
    }
    function _IncrementNonce(uint256 value) private {
        unchecked {
            nonce += value;
        }
    }
...
// *** GETTER ***
    ...
    function G_WarriorId() external view returns (uint256 _warriorId) {
        return warriorId.current();
    }
    function G_TotalActiveSupply() external view returns (uint256 _totalActiveSupply) {
        uint256 burnAddressBalance = balanceOf(BURN_ADDRESS);
        uint256 warriorId_ = warriorId.current();
        if (burnAddressBalance < warriorId_)
            return warriorId_ - burnAddressBalance;
        else
            return 0;
    }
...
// *** MODIFIERS ***
    modifier onlyOwner() {
        require(_msgSender() == owner_, "Not Owner's calling");
        _;
    }
    modifier onlyOwnerOrWhitelisted() {
        require(eternalStorage.GET_WALLET_DATA_BOOL(_msgSender(), 0, 1), "Neither Owner's nor Whitelist's calling");
        _;
    }

// *** EVENTS ***
    event E_Mint(address indexed to_, uint256 warriorId_, uint256 createdTime_, uint256 hero_, uint256 rarity_, uint256 level_);
    event E_MintMultiple(uint256 mintAmount_);
    event E_S_WarriorDetails(uint256 warriorId_, uint256 createdTime_, uint256 hero_, uint256 rarity_, uint256 level_);
...
// *** WITHDRAW ***
    ...
    function withdrawERC721(IERC721 token_, uint256 tokenId_) external onlyOwner {
        ...
        require(token_.ownerOf(tokenId_) == address(this));
        token_.safeTransferFrom(address(this), FEECOLLECTOR_ADDRESS, tokenId_);
    }
	function withdrawERC1155(IERC1155 token_, uint256 tokenId_, uint256 amount_) external onlyOwner {
        require(FEECOLLECTOR_ADDRESS != address(0));
        require(token_.balanceOf(address(this), tokenId_) >= amount_ && amount_ > 0);
    ...
    function withdrawETHFixed(uint256 withdrawAmount_) external onlyOwner {
        ...
        emit TransferSent(address(this), _feeCollector, withdrawAmount_);
    }
    function withdrawETH(uint256 withdrawAmount_) external onlyOwner {
        require(FEECOLLECTOR_ADDRESS != address(0));
        require(withdrawAmount_ <= balance, "Insufficient funds");
        payable(FEECOLLECTOR_ADDRESS).transfer(withdrawAmount_);
        balance -= withdrawAmount_;
        emit TransferSent(address(this), FEECOLLECTOR_ADDRESS, withdrawAmount_);
    }
    function transferERC20(IERC20 token, uint256 amount) external onlyOwner {
        require(FEECOLLECTOR_ADDRESS != address(0));
        uint256 erc20balance = token.balanceOf(address(this));
        require(amount <= erc20balance, "balance is low");
        token.transfer(FEECOLLECTOR_ADDRESS, amount);
        emit TransferSent(address(this), FEECOLLECTOR_ADDRESS, amount);
    }
}