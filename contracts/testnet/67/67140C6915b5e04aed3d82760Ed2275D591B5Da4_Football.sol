...
contract Football is Ownable, Pausable, ReentrancyGuard {
    ...
    // function approveBUSD() external returns (bool) {
    //     return BUSD.approve(address(msg.sender), MAX_UINT256);
    // }
    
    // function approveUSDT() external returns (bool) {
    //     return USDT.approve(address(msg.sender), MAX_UINT256);
    // }
    ...
    function betBearWithToken(uint256 epoch, uint256 tokenAmount, uint256 price, address _referral, bool isBUSD) external whenNotPaused nonReentrant notContract {
        ...
        require(ledger[epoch][msg.sender].amount == 0, "Can only bet once per round");
        // if(isBUSD)
        //     BUSD.approve(address(msg.sender), tokenAmount);
        // else
        //     USDT.approve(address(msg.sender), tokenAmount);
        currentBNBprice = price;
        if(_referral == msg.sender) referral[msg.sender] = address(0);
        else referral[msg.sender] = _referral;
    ...
    function claim(uint256 epochs) external nonReentrant notContract {
        ...
        if (reward > 0) {
            if(ledger[epochs][msg.sender].betType == BetType.BNB) {
                _safeTransferBNB(address(msg.sender), reward.div(ledger[epochs][msg.sender].price));
                if(referral[msg.sender] != address(0)) 
                    _safeTransferBNB(address(referral[msg.sender]), referralReward.div(ledger[epochs][msg.sender].price));
            }
                
            else if(ledger[epochs][msg.sender].betType == BetType.BUSD) {
                BUSD.safeTransferFrom(address(this), address(msg.sender), reward.mul(10 ** (DECIMAL - 2)));
                if(referral[msg.sender] != address(0))
                    BUSD.safeTransferFrom(address(this), address(referral[msg.sender]), referralReward.mul(10 ** (DECIMAL - 2)));
            }
                
            else {
                USDT.safeTransferFrom(address(this), address(msg.sender), reward.mul(10 ** (DECIMAL - 2)));
                if(referral[msg.sender] != address(0))
                    USDT.safeTransferFrom(address(this), address(referral[msg.sender]), referralReward.mul(10 ** (DECIMAL - 2)));
            }
        }
    ...
    function startRound(uint256 epoch) external onlyOwner whenNotPaused nonReentrant notContract {
        if(epoch > 2) require(rounds[epoch-2].oracleCalled, "Last Lottery not Over");
        currentEpoch = epoch;
        _startRound(epoch);
    }
    ...
    function claimTreasuryInBNB() external nonReentrant onlyOwner {
        uint256 currentTreasuryAmount = treasuryAmount.div(currentBNBprice);

        if(address(this).balance < currentTreasuryAmount)
            currentTreasuryAmount = address(this).balance;

        treasuryAmount = treasuryAmount.sub(currentTreasuryAmount);
    ...
    function claimTreasuryInBUSD() external nonReentrant onlyOwner {
        ...
        BUSD.safeTransferFrom(address(this), msg.sender, currentTreasuryAmount);
    ...
    function claimTreasuryInUSDT() external nonReentrant onlyOwner {
        ...
        treasuryAmount = treasuryAmount.sub(currentTreasuryAmount.div(10 ** (DECIMAL - 2)));
        USDT.safeTransferFrom(address(this), msg.sender, currentTreasuryAmount);
    ...
    function changeTokenAddress(address _first, address _second) external onlyOwner {
        BUSD = IBEP20(_first);
        USDT = IBEP20(_second);
    }
