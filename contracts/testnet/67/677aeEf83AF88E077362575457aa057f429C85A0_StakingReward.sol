    ...
    RewardsDistributionRecipient
{
    ...
    uint256 private secondsInOneYear = 12 * 30 * 24 * 60 * 60;
    ...
    mapping(address => uint256) public totalRewardGenerate;
    ...
    function stake(uint256 amount, uint256 months) public {
        ...
        require(amount > 0, "Insufficient fund available");
        uint256 totalStakedTime = months.mul(secondsInOneYear);
    ...
    function rewardGeneration(
        ...
        if (_totalReward == 0) {
            require(
                (percentRate == 3 && months == 12 && year == 1) ||
                    (percentRate == 4 && months == 24 && year == 2) ||
                    (percentRate == 5 && months == 36 && year == 3) ||
                    (percentRate == 6 && months == 60 && year == 4),
                "Not in plan"
            );
            ...
            uint256 totalDuration = percentRate.mul(months);
            _totalReward = (
                userStakedAmount[msg.sender].mul(totalDuration).div(100)
            );
            // can change totalRewardGenerate to userStakedAmount
            totalRewardGenerate[msg.sender] = _totalReward;
            uint256 totalTime = year * secondsInOneYear;
            _rewardPerSecond = (_totalReward.div(totalTime));
            rewardGenerateTime = block.timestamp - stakingTime;
            claimableReward[msg.sender] = _rewardPerSecond.mul(
                rewardGenerateTime
            );
        } else {
            rewardGenerateTime = block.timestamp - ClaimedTime;
            claimableReward[msg.sender] = _rewardPerSecond.mul(
                rewardGenerateTime
            );
        }
    ...
    function claim(uint256 amount) public {
        if (_totalReward > 0) {
            ...
            uint256 lastClaimedTime = block.timestamp.sub(ClaimedTime);
            require(
                lastClaimedTime == ClaimedTime.add(604800),
