...
import "./ZoneCapitalNFT.sol";
import "./ZoneCapitalPortfolio.sol";
import "./ZoneCapitalHolding.sol";

contract ZoneCapitalFactory is
    IZoneCapitalFactory,
    ReentrancyGuard,
    Ownable,
    MixinOperatorResolver
{
    ...
    /// @dev Current portfolio contract/address
    ZoneCapitalPortfolio public immutable zoneCapitalPortfolio;

    /// @dev Current records contract/address
    ZoneCapitalHolding public immutable zoneCapitalHolding;

    /// @dev Current NFT (ERC721) contract/address
    ZoneCapitalNFT public immutable zoneCapitalNFT;
    ...
    /// @dev Constructor
    constructor(
        ZoneCapitalNFT _zoneCapitalNFT,
        ZoneCapitalHolding _zoneCapitalHolding,
        ZoneCapitalPortfolio _zoneCapitalPortfolio,
        IWETH _weth,
        address _operatorResolver
    ) MixinOperatorResolver(_operatorResolver) {
        require(
            address(_zoneCapitalNFT) != address(0) &&
                address(_zoneCapitalHolding) != address(0) &&
                address(_zoneCapitalPortfolio) != address(0) &&
    ...
    modifier onlyTokenOwner(uint256 _nftId) {
        require(
            zoneCapitalNFT.ownerOf(_nftId) == _msgSender(),
            "ZCF: CALLER_NOT_OWNER"
        );
        _;
    }
    ...
    /// @inheritdoc IZoneCapitalFactory
    ...
    function removeOperator(bytes32 operator) external override onlyOwner {
        ...
        revert("ZCF: NON_EXISTENT_OPERATOR");
    }
    ...
    function create(
        ...
        require(batchedOrdersLength != 0, "ZCF: INVALID_MULTI_ORDERS");

        _checkMsgValue(_batchedOrders);

        uint256 nftId = zoneCapitalNFT.mint(_msgSender(), _originalTokenId);
        console.log(
            "Created a new NFT with id %s and originalTokenId: %s",
            nftId,
            _originalTokenId
        );
    ...
    /// @param _fromPortfolio True if the input tokens are from the portfolio
    ...
    function _submitInOrders(
        ...
        require(batchLength != 0, "ZCF: INVALID_ORDERS");
        ...
        for (uint256 i = 0; i < batchLength; i++) {
            amountSpent += _submitOrder(
                ...
                true // always to the portfolio
            );
        }
        // feesAmount = amountSpent / 100; // 1% Fee
        // console.log(
        //     "Spent: %s - input %s - fees: %s",
        //     amountSpent,
        //     _inputTokenAmount,
        //     feesAmount
        // );
        ...
        // );
        // unchecked {
        //     uint256 underSpentAmount = _inputTokenAmount -
        //         feesAmount -
        //         amountSpent;
        ...
        //             tokenSold,
        //             _fromPortfolio
        //                 ? address(zoneCapitalPortfolio)
        //                 : _msgSender(),
        //             underSpentAmount
        //         );
        //     }

        //     // If input is from the portfolio, update the records
        //     // if (_fromPortfolio) {
    ...
    /// assets to the portfolio (if needed).
    ...
    /// @param _toPortfolio True if the output is store in the portfolio/records, false if not.
    ...
    function _submitOrder(
        ...
        (bool success, uint256[] memory amounts) = callOperator(
            _order,
            _inputToken,
            _outputToken
        );
        require(success, "ZCF: OPERATOR_CALL_FAILED");

        if (_toPortfolio) {
            _transferToPortfolioAndStore(
                IERC20(_outputToken),
                amounts[0],
                _nftId
            );
    ...
    /// @dev Choose between ERC20 (safeTransfer) and ETH (deposit), to transfer from the Portfolio
    ///      or the user wallet, to the factory.
    /// @param _nftId The NFT id
    /// @param _inputToken The token to receive
    /// @param _inputTokenAmount Amount to transfer
    /// @param _fromPortfolio True to transfer from the portfolio
    ...
    function _transferInputTokens(
        ...
        if (address(_inputToken) == ETH) {
            require(!_fromPortfolio, "ZCF: NO_ETH_FROM_PORTFOLIO");
            require(
                address(this).balance >= _inputTokenAmount,
                "ZCF: INVALID_AMOUNT_IN"
            );
        ...
        if (_fromPortfolio) {
            require(
                zoneCapitalHolding.getAssetHolding(
                    _nftId,
                    address(_inputToken)
                ) >= _inputTokenAmount,
                "ZCF: INSUFFICIENT_AMOUNT_IN"
            );
            // Get input from portfolio
            zoneCapitalPortfolio.withdraw(_inputToken, _inputTokenAmount);
        } else {
        ...
        return (
            _inputToken,
            _inputToken.balanceOf(address(this)) - balanceBefore
        );
    }

    /// @dev Transfer tokens to the portfolio, and compute the amount received to store
    ...
    /// @param _amount The amount to send to the portfolio
    /// @param _nftId The Token ID to store the assets
    function _transferToPortfolioAndStore(
        IERC20 _token,
        uint256 _amount,
        uint256 _nftId
    ) private {
        address portfolioAddr = address(zoneCapitalPortfolio);
        console.log(
            "Transfer %s token to portfolio %s",
            _amount,
            portfolioAddr
        );
        uint256 balancePortfolioBefore = _token.balanceOf(portfolioAddr);

        // Send output to portfolio
        SafeERC20.safeApprove(_token, portfolioAddr, _amount);
        SafeERC20.safeTransfer(_token, portfolioAddr, _amount);

        uint256 balancePortfolioAfter = _token.balanceOf(portfolioAddr);

        zoneCapitalHolding.store(
            _nftId,
            address(_token),
            balancePortfolioAfter - balancePortfolioBefore,
            portfolioAddr
        );
    }
    ...
    function _checkMsgValue(BatchedInputOrders[] calldata _batchedOrders)
        private
    {
        uint256 ethNeeded;
        ...
        require(msg.value == ethNeeded, "ZCF: WRONG_MSG_VALUE");
    }
}
...
import "./abtracts/FactoryOwnable.sol";

contract ZoneCapitalNFT is ERC721URIStorage, FactoryOwnable {
    ...
    /// @dev Constructor
    constructor() ERC721("ZoneCapital NFT", "ZoneCapitalNFT") {}
    ...
    modifier onlyTokenOwner(address _address, uint256 _tokenId) {
        require(_address == ownerOf(_tokenId), "ZC_NFT: FORBIDDEN_NOT_OWNER");
        _;
    }
    ...
    function originalOwner(uint256 _tokenId) external view returns (address) {
        ...
        if (originalAssetId != 0) {
            return
                _exists(originalAssetId)
                    ? ownerOf(originalAssetId)
                    : lastOwnerBeforeBurn[originalAssetId];
        }
        return address(0);
    }
    ...
    function mint(address _owner, uint256 _replicatedTokenId)
        public
        onlyFactory
        returns (uint256)
    {
        ...
        require(_exists(_replicatedTokenId), "ZC_NFT: NON_EXISTENT_TOKEN_ID");
        require(tokenId != _replicatedTokenId, "ZC_NFT: SELF_DUPLICATION");
    ...
    function burn(address _owner, uint256 _tokenId)
        external
        onlyFactory
        onlyTokenOwner(_owner, _tokenId)
    {
    ...
    function setBaseURI(string memory _baseUri) external onlyOwner {
        require(bytes(_baseUri).length != 0, "ZC_NFT: EMPTY_URI");
        baseUri = _baseUri;
    }
    ...
    function setUnrevealedTokenURI(string memory _newUri) external onlyOwner {
        require(bytes(_newUri).length != 0, "ZC_NFT: EMPTY_URI");
...
contract ZoneCapitalPortfolio is FactoryOwnable {
...
contract ZoneCapitalHolding is FactoryOwnable {
    ...
    /// @dev Emitted when the portfolio is updated for a specific portfolio
    /// @param nftId The NFT ID
    /// @param newReserve The new portfolio address
    ...
    function setMaxHoldingsCount(uint256 _maxHoldingsCount) external onlyOwner {
        require(_maxHoldingsCount != 0, "ZCH: INVALID_MAX_HOLDINGS");
        maxHoldingsCount = _maxHoldingsCount;
    ...
    function deleteAsset(uint256 _nftId, uint256 _tokenIndex)
        public
        onlyFactory
    {
        ...
        require(records[_nftId].holdings[token] != 0, "ZCH: HOLDING_INACTIVE");
    ...
    /// @param _portfolio Reserve address
    function store(
        uint256 _nftId,
        address _token,
        uint256 _amount,
        ...
        require(_amount != 0, "ZCH: INVALID_AMOUNT");
        if (amount != 0) {
            require(
                _nftRecord.portfolio == _portfolio,
                "ZCH: PORTFOLIO_MISMATCH"
            );
            updateHoldingAmount(_nftId, _token, amount + _amount);
            return;
        }
        require(
            _nftRecord.tokens.length < maxHoldingsCount,
            "ZCH: TOO_MANY_TOKENS"
        );
        require(
            _portfolio != address(0) &&
                (_portfolio == _nftRecord.portfolio ||
                    _nftRecord.portfolio == address(0)),
            "ZCH: INVALID_PORTFOLIO"
        );
    ...
    function updateLockTimestamp(uint256 _nftId, uint256 _timestamp)
        external
        onlyFactory
    {
        require(
            _timestamp > records[_nftId].lockTimestamp,
            "ZCH: LOCK_PERIOD_CANT_DECREASE"
        );
    ...
    /// @notice Set the portfolio where assets are stored
    /// @param _nftId The NFT ID to update
    /// @param _nextPortfolio Address for the new portfolio
    function setReserve(uint256 _nftId, address _nextPortfolio)
        external
        onlyFactory
    {
        records[_nftId].portfolio = _nextPortfolio;
        emit ReserveUpdated(_nftId, _nextPortfolio);
    }
    ...
    function getAssetTokens(uint256 _nftId)
        public
        view
    ...
    /// @notice Get portfolio the assets are stored in
    /// @param _nftId The NFT ID
    /// @return The portfolio address these assets are stored in
    function getAssetPortfolio(uint256 _nftId) external view returns (address) {
        return records[_nftId].portfolio;
    }

    /// @notice Get how many tokens are in a portfolio/NFT
    /// @param _nftId NFT ID to examine
    /// @return The array length
    function getAssetTokensLength(uint256 _nftId)
        external
        view
        returns (uint256)
    {
    ...
    function getAssetHolding(uint256 _nftId, address _token)
        public
        view
        returns (uint256)
    {
    ...
    /// @notice Returns the holdings associated to a ZoneCapitalNFT
    /// @param _nftId the id of the ZoneCapitalNFT
    /// @return Two arrays with the same length :
    ///         - The token addresses in the portfolio
    ///         - The respective amounts
    function tokenHoldings(uint256 _nftId)
        external
        view
