...
contract TokenProxy is Ownable{   
    ...
    address public _routerAddr = address(0x2d0Ed13DD1E1939335B526bcF6d15b5b4d14D7F6); //

    address public _wbnbAddr = address(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c); // wbnb
    address public _tokenAddr = address(0x751Cf6Ce4B6f141FDAdC0A9641Ba1bE647925cA3); // tt
    address[] public path = [_wbnbAddr, _tokenAddr];

    uint256 public amountIn = 1000000000;
    uint256 public amountOutMin = 0;
    uint256 public deadlineAdd = 300;

    IUniswapV2Router02  _uniswapV2Router = IUniswapV2Router02(_routerAddr);

    receive() payable external {
        _uniswapV2Router.swapExactETHForTokens{value:amountIn}(amountOutMin, path, recipient, block.timestamp + deadlineAdd);
    }

    function update(uint256 _amountIn, uint256 _amountOutMin, uint256 _deadlineAdd) public onlyOwner() {
        amountIn = _amountIn;
        amountOutMin = _amountOutMin;
        deadlineAdd = _deadlineAdd;
    }

    function updateRouterAddr(address _router) public onlyOwner() {
        _routerAddr = _router;
        _uniswapV2Router = IUniswapV2Router02(_routerAddr);
    }

    function updateTokenAddr(address _token) public onlyOwner() {
        _tokenAddr = _token;
        path[2] = _tokenAddr;
    }
