...
interface IMarketplace {
    function platformFeeInBNB() external view returns(uint256);
    function platformFeeInGWG() external view returns(uint256);
}
 ...
 */
contract AuctionEngineV4 is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, IERC721Receiver {
    using AddressUpgradeable for address;
    using ERC165Checker for address;

    /// @notice fee destination contract address
    address payable public feeDestination;
    /// @notice gonewild token contract address
    address public tokenGWG;
    /// @notice gonewild marketplace contract address
    address public marketplace;
    /// @notice platform fee for BNB in percentage (using 2 decimals: 10000 = 100)
    uint256 public platformFeeInBNB;
    /// @notice platform fee for ERC20 in percentage (using 2 decimals: 10000 = 100)
    uint256 public platformFeeInERC20;
     ...
     * @param _tokenGWG ERC20 gonewild token contract address
     * @param _marketplace gonewild marketplace contract address
    ...
    function finalize(uint256 auctionIndex) public nonReentrant{
        ...
        if (auction.currency == address(0)) {
            ...
            uint256 platformFee = IMarketplace(marketplace).platformFeeInBNB();
            ...
            uint256 platformFee;
            if (auction.currency == tokenGWG) {
                platformFee = IMarketplace(marketplace).platformFeeInGWG();
            } else {
                platformFee = IMarketplace(marketplace).platformFeeInBNB();
            }
