...
contract Null is Context, IERC20, Ownable
{
    ...
    string public name = "Null";
    string public symbol = "NULL";
    ...
    uint public swapThreshold = totalSupply / 1000000;
    ...
    constructor (address _treasury, address _marketing)
    {
    ...
    {
        require(value >= totalSupply / 100000, "Minimum tx must be greater than 0.1% of total supply!");
    ...
    {
        require(value >= totalSupply / 10000000, "Minimum wallet size must be greater than 1% of total supply!");
    ...
    {
        require(wallet != address(0), "Wallet must not be null address!");
    ...
    {
        require(wallets.length <= 200, "Maximum wallets at once is 200!");

        for (uint i = 0; i < wallets.length; i++)
            isBot[wallets[i]] = status;
    }

    function setBuyTaxes(uint treasury, uint marketing) external onlyOwner() 
    {
        require(treasury + marketing <= 16, "Buy tax cannot be more than 15%!");
    ...
    function setSellTaxes(uint treasury, uint marketing) external onlyOwner() 
    {
        require(treasury + marketing <= 25, "Sell tax cannot be more than 24%!");
    ...
    function EnableTrading(bool status) external onlyOwner {
        ...
        tradingEnabled = true;
    emit TradingEnabledUpdated(status);
    }
    ...
    {
        _approve(_msgSender(), spender, allowances[_msgSender()][spender].sub(subtractedValue, "Decreased allowance below zero!"));
        return true;
    }
    ...
    {
        require(recipient != address(0), "Cannot send to the 0 address!");
    ...
    function transferFrom(address sender, address recipient, uint amount) public override returns (bool)
    {
        ...
        _approve(sender, _msgSender(), allowances[sender][_msgSender()].sub(amount, "Transfer amount exceeds allowance!"));
        return true;
    }
    ...
    function _transfer(address sender, address recipient, uint amount) private returns (bool)
    {
        ...
        require(!isBot[recipient] && !isBot[sender], "No bots allowed!");
        ...
        else
        {
            ...
            if (!isWalletLimitExempt[recipient])
                require(balanceOf(recipient).add(finalAmount) <= maxWallet, "Transfer amount must not exceed max wallet conditions!");
    ...
    function swapAndLiquify(uint amount) private lockTheSwap 
    {
        ...
        uint totalFee = inFee.treasury + inFee.marketing + outFee.treasury + outFee.marketing;
    ...
    function takeFee(address sender, address recipient, uint amount) internal returns (uint256)
    {
        ...
        if (isMarketPair[sender])
            feeAmount = amount.mul(inFee.treasury + inFee.marketing).div(100);
        else if (isMarketPair[recipient])
            feeAmount = amount.mul(outFee.treasury + outFee.marketing).div(100);
    ...
    function clearStuckBNB(address recipient, uint amount) public onlyOwner
    {
        require(recipient != address(0), "Cannot send to the 0 address!");
    ...
    function clearBEP20Token(address tokenAddress, address recipient, uint amount) public onlyOwner
    {
        require(recipient != address(0), "Cannot send to the 0 address!");
