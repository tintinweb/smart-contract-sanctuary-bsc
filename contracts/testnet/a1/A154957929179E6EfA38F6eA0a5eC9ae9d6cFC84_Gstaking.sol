...
interface IGstaking {
    struct TermsProfit {
        ...
        uint256[] distribute; // distribute for agent ，0、1、2、3、4 ....11
    }
...
contract Gstaking is IGstaking, Ownable, NoteStake {
    ...
    function initialize(address _usdt, address _usdtAfaPair) external _onlyOwner {
    ...
    function setTermsProfit(
        ...
        if (index < termsPro.length) {
            ...
            term.distribute = _distribute;
        } else {
            termsPro.push(TermsProfit({vesting: _vesting, minAmount: _minAmount, amountInQuote: _amountInQuote, forfeit: _forfeit, earlyOut: _earlyOut, distribute: _distribute}));
        }
    }
        ...
        )
    {
        ...
        index_ = addNote(_user, uamount_, gamount_, uint48(expiry_), termId);
...
abstract contract NoteStake is INoteStake {
    ...
    function addNote(
        ...
        uint256 index = gAFA.index();
        notes[_user].push(Note({payout: payout, created: uint48(block.timestamp), matured: _expiry, redeemed: 0, amount: amount, initIndex: index, termId: _termId}));
    }
