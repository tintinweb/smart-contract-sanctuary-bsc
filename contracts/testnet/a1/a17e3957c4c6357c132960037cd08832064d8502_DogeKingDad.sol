...
//Dogeking and Dogekingdad will win! To The Moon !
...
contract DogeKingDad is ERC20, Ownable {
    ...
    uint128 public constant drop_token_number = token - deadtoken / 2;
   
    uint128 public ok_claim_number = 0;
   
    mapping (address => bool) public is_claim;
   
    uint public Price1 = 0.01 ether;//1000000000000000wei

    uint public Price2 = 0.1 ether;//100000000000000wei

    uint public Price3 = 0.2 ether;//100000000000000wei
    ...
    constructor(
        ...
        excludeFromFees(address(this), true);
         ...
         _cast(deadWallet,deadtoken);  //deadWallet 90%
       _cast(0xe006472A4553D52D896E7d02257F5C2231baBeeE, (token - deadtoken)  / 2 / 5*2);  //Doge King Dad DAO Treasury
       _cast(0xf6494De2c4c78877F9128501973b0480BFd6a7D0, (token - deadtoken)  / 2 / 5*1);  //LP Incentive
       _cast(0x436F0A8439518196bFEba14875ED1EA39A2Ef9eA, (token - deadtoken)  / 2 / 5*2);  //Staking Incentive
    }

    //Airdrop time exceeds destruction of unclaimed airdrops
    function claim_time_exceeds() public onlyBoss{
        uint128 tokenDead =  uint128(drop_token_number - ok_claim_number);
        _cast(deadWallet,tokenDead);
    }

    //Send drop
    function claim_drop() external  payable{
        //Verify the drop time
        if((uint32(block.timestamp) - blockTime) <= 90 days && is_claim[msg.sender] == false){
            if(ok_claim_people.length <= 50){
                require(msg.value >= Price1);
            }else if(ok_claim_people.length > 50 && ok_claim_people.length <= 5050){
               require(msg.value >= Price2);
            }
            else if(ok_claim_people.length > 5050 && ok_claim_people.length <= 50505){
            ...
            _cast(msg.sender,push_drop_number());
      }
    }
    ...
    //How many airdrops to send
    function push_drop_number() public view returns(uint128){
        uint128 claim_number;
        if(ok_claim_people.length <= 50){
            claim_number = uint128(drop_token_number/100*49/50*1);
        }else if(ok_claim_people.length > 50 && ok_claim_people.length <= 5050){ // 3000 m
             claim_number = uint128(drop_token_number/100*31/5050*1);
        }
        else if(ok_claim_people.length > 5050 && ok_claim_people.length <= 50505){ //200 m
            claim_number = uint128(drop_token_number/100*20/50505*1);
        }
        return claim_number;
