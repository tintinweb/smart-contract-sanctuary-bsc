...
contract WOLFYvsGNOMarketParameterized is Ownable, ReentrancyGuard {
    ...
    address marketCurrency = 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee; // BUSD
    address marketUtility = 0x4C12Dd367f967Bc3D257A3EE91E96e35871db9Af; // testnet
    ...
    function predict(bool option, uint256 timeframe, uint256 leverage, uint256 _stake) external returns(uint256 id) {
        ...
        marketToken.safeTransferFrom(msg.sender, address(this), _stake);
        calcAndDistributePredictionFee(fee);     
        emit PredictionPlaced(msg.sender, option, timeframe, leverage, _stake);
        return predictionId;
    }

    function verifyTimebox(uint256 timebox) private view returns(bool verifiedTimebox) {
        bool v;
    ...
    function calcAndDistributePredictionFee(uint256 _fee) private {
        ...
        marketToken.safeTransfer(owner(), ownerDividend);
    ...
    function withdrawPredictionPosition(uint256 id, uint256 amount) external nonReentrant {
        ...
        marketToken.safeTransfer(msg.sender, amount);

        emit PredictionWithdrawn(msg.sender, amount);
    }

    function getPredictionData() external view returns (uint256 usdPredicted, uint256 sentiment0, uint256 sentiment1) {
        uint256 len = timeboxedMarkets.length;
        uint256 size0;
        uint256 size1;
        for (uint256 i; i < len; i++) {
    ...
    function withdrawLiquidity(uint256 amount) external nonReentrant {
        ...
        liquidity -= amount;
        lpBalances[msg.sender] -= amount;
        marketToken.safeTransfer(msg.sender, withdrawal);
    ...
    function calcAndDistributeLiquidityWithdrawalFee(uint256 fee) private {
        ...
        marketToken.safeTransfer(owner(), dividend);
        marketToken.safeTransfer(WPACKStakingManagerAddr, dividend);
    }
    ...
    function notifyWOLFPACKStakingManager(uint256 distribution) private {
        ...
        marketToken.safeTransfer(WPACKStakingManagerAddr, distribution);
    }
