...
contract PistonTokenController is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    address public  uniswapV2Pair;
		...
		event TestEvent_I_WAS_HERE(string text);
		function initialize(address _Piston) public virtual initializer {
      
				PistonApp = IERC20Upgradeable(_Piston);
        ...
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(PistonApp), BUSD);
						//.getPair(address(PistonApp), BUSD);

        // set the rest of the contract variables
        uniswapV2Router = _uniswapV2Router;
				
				
				liquidityFee = 20; // 20% of all the fees collected 
emit TestEvent_I_WAS_HERE("init..");	

 _ecosystemWalletAddress = address(0x4ff8C8741256d729A8cC7F1Cc02330F8103CCD28);			
		...
		emit TestEvent_I_WAS_HERE("swapAndLiquify called..");	

				uint256 forLiquidity = PistonApp.balanceOf(address(this)).mul(liquidityFee).div(100);
				
				if ( forLiquidity >= 50 * (10**14)) {
				emit TestEvent_I_WAS_HERE("init..");	
        ...
        addLiquidity(otherHalf, newBalance);
				...
				// at present we are sending 80% of the total fees to _ecosystemWalletAddress , but can be split between: dev/ecosystem
				
				// swap the rest to send to  _ecosystemWalletAddress 
				swapTokensForBUSD(PistonApp.balanceOf(address(this)));
				
				//transfer BUSD to _ecosystemWalletAddress
        if(_ecosystemWalletAddress != address(0)){
     ...
     function swapTokensForBUSD(uint256 tokenAmount) private  {
        ...
        path[0] = address(PistonApp);
        path[1] = BUSD;
				
				PistonApp.approve(address(uniswapV2Router), tokenAmount);
    ...
    function setEcosystemWalletAddress(address _value) public {
        //@TODO!!!!! onlyOwner
        _ecosystemWalletAddress = _value;
    }
		
		function pistonBalance() public view  returns (uint256, uint256, uint256) {
				return (PistonApp.balanceOf(address(this)), IERC20Upgradeable(BUSD).balanceOf(address(uniswapV2Pair)), PistonApp.balanceOf(address(this)).mul(liquidityFee).div(100));
				
    }
 function getPairBalance() public view  returns (uint256, uint256) {
        return (PistonApp.balanceOf(address(uniswapV2Pair)), IERC20Upgradeable(BUSD).balanceOf(address(uniswapV2Pair)));
    }
		
		function getTokenAndControllerAddress() public view  returns (address, address) {
        return (address(PistonApp), address(this) );
    }
