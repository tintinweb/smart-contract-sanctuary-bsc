...
contract EnglishAuction is Configurable, IERC721Receiver {
    ...
    function initial(address _governor, address feeConfigContract, address botToken) public initializer {
        ...
        config[FeeConfigContract] = uint(uint160(feeConfigContract));
        config[BotToken] = uint(uint160(botToken)); // AUCTION
    }
    ...
    function bidderClaim(uint index) external
        ...
        isPoolClosed(index)
    {
        _bidderClaim(payable(msg.sender), index);
    }
    ...
    function getFeeConfigContract() public view returns (address) {
        return address(uint160(config[FeeConfigContract]));
    }
...
contract Configurable is Governable {
    ...
    function getConfig(bytes32 key, address addr) public view returns (uint) {
        uint i = uint(uint160(addr));
        return config[bytes32(uint(key) ^ i)];
    }
    ...
    function _setConfig(bytes32 key, address addr, uint value) internal {
        uint i = uint(uint160(addr));
        _setConfig(bytes32(uint(key) ^ i), value);
    }
