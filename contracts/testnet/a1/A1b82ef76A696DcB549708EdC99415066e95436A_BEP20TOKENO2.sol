    ...
    function getInterestAmount(uint stakeId,bool _type) public view returns (uint){
        ...
        if(_type==true){
            ...
            if(block.timestamp>=_info.stakeEndTime){
                //计算剩余
                return _info.stakeTotal*_info.rate.div(100)-_info.stakeInterestTotal;
            }
            return _info.stakeTotal*_info.rate.div(100)*(block.timestamp-_info.claimTime)/((_info.stakeEndTime-_info.claimTime));
        }else{
            if(block.timestamp>=_info.stakeEndTime){
                //计算剩余
                return _amountOut(_info.stakeTotal*_info.rate.div(100),_usdt,address(this))-_info.stakeInterestTotal;
            }
            return _amountOut(_info.stakeTotal*_info.rate.div(100),_usdt,address(this))*(block.timestamp-_info.claimTime)/((_info.stakeEndTime-_info.claimTime));
        }
    }
    //获取用户可提现的线性利息
