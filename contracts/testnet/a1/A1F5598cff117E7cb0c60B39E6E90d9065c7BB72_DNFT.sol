    ...
    ReentrancyGuardUpgradeable
{
    ...
    event TemporaryMerged(address _to, uint256[] _tokenIds, string indexed _sessionId);
    event TemporaryMergeCancelled(address _to, uint256[] _tokenIds, string indexed _sessionId);
    event TemporaryMergeExecuted(address _to, uint256[] _tokenIds, uint256 indexed _newTokenId);
    event PermanentMerged(address _to, uint256[] _tokenIds, uint256 indexed _newTokenId);
    event CrossSpeciesMerged(address _to, uint256[] _tokenIds, uint256 indexed _newTokenId);
    ...
    function initialize(
         ...
         */
        priceFeed = 0x0630521aC362bc7A19a4eE44b57cE72Ea34AD01c;
    ...
    function restrictedRescue() external whenNotPaused onlyOwner {
        if (currentSalePhase < 3 || block.timestamp < salePhaseStatistics[3].endTime) revert NOT_YET_RESCUE_TIME();
        ...
        for (uint256 i = 0; i < maxAvailableRescueNumber; i++) {
            uint256 newTokenId = safeMint(msg.sender);
            totalAdminRescued++;
            emit RescueWithKey(msg.sender, newTokenId);
        }
        ...
        nonReentrant
    {
        ...
        emit TemporaryMergeExecuted(msg.sender, _tokenIds, newTokenId);
    }
    ...
    function permanentMerge(
        ...
        emit PermanentMerged(msg.sender, _tokenIds, newTokenId);
    }
