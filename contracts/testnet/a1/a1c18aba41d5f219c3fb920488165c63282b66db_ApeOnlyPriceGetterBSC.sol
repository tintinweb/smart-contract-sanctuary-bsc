...
// File: polycrystal-on-chain-stats/contracts/interfaces/IPancakePair.sol
...
//    using AMMData for AmmData;
    ...
    address public constant FACTORY = 0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc; //ApeFactory

    bytes32 public constant INITCODEHASH = hex'ecba335299a6693cb2ebc4782e74669b84290b6378ea3a3873c7231a8d7d1074'; // for pairs created by ApeFactory
    ...
    address constant DAI = 0x8a9424745056Eb399FD19a0EC26A14316684e274;
    ...
    address constant BUSD_WBNB_PAIR = 0xe0e92035077c39594793e61802a350347c320cf2; // busd is token1
    ...
    function getBNBPrice() public view returns (uint) {
        ...
        (uint wbnbReserve1, uint busdReserve,) = IPancakePair(BUSD_WBNB_PAIR).getReserves();
    ...
    function getRawLPPrice(address lp, uint bnbPrice) internal view returns (uint) {
        ...
        try IPancakePair(lp).getReserves() returns (uint112 reserve0, uint112 reserve1, uint32) {
    ...
    function pegTokenPrice(address token, uint bnbPrice) private pure returns (uint) {
        ...
        if (token == BUSD) return PRECISION;
    ...
    function pairTokensAndValue(address token, address peg) private view returns (uint tokenNum, uint pegValue) {
        ...
        try IPancakePair(tokenPegPair).getReserves() returns (uint112 reserve0, uint112 reserve1, uint32) {
