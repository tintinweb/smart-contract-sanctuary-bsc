...
contract StakingRewards is  ReentrancyGuardUpgradeable,OwnableUpgradeable {
    ...
    IERC20Upgradeable public legacyToken;
    uint256 public periodFinish;
    uint256 public rewardRate;
    uint256 public rewardsDuration = 3600;
    uint256 public lastUpdateTime;
    uint256 public rewardPerTokenStored;
    uint256 public totalFees = 25;
    uint256 public holdingTime = 3600;
    uint256 public ratio = 50000*(1e9);
    uint256 public price = 0.25 ether;
    uint256 public discountedPrice = 0.15 ether;
    ...
    function initialize(address _stakingToken,address _legacyToken) public initializer{
         stakingToken = IERC20Upgradeable(_stakingToken);
         legacyToken = IERC20Upgradeable(_legacyToken);
         __Ownable_init();
         __ReentrancyGuard_init();

    }

    function updateTokenAddress(address _legacyToken) external onlyOwner{
        legacyToken = IERC20Upgradeable(_legacyToken);
    }

    function updateSTTokenAddress(address _stakingToken) external onlyOwner{
        stakingToken = IERC20Upgradeable(_stakingToken);
    }

    function updatePrices(uint256 _price,uint256 _discountedPrice) external onlyOwner{
        price = _price;
    ...
    function bulkUpdateWhiteList(address[] memory accounts, bool excluded) external onlyOwner{
    ...
    function getPrice(address account,uint256 amount) public view returns(uint256,uint256){
         uint256 legacy = amount.mul(100).div(ratio);
         uint256 newPrice = price;
         if(isWhiteListed[account] && legacyToken.balanceOf(address(this)) > 2900*(1e7))
         {
             newPrice = 0;
         }
         else if(isWhiteListed[account] && legacyToken.balanceOf(address(this)) > 2000*(1e7))
         {
             newPrice = discountedPrice;
         }

         uint256 _price = legacy.mul(newPrice).div(100);

         return (legacy,_price);
    }

    function stake(uint256 amount) external payable nonReentrant updateReward(msg.sender) {
        (uint256 legacy,uint256 _price) = getPrice(msg.sender,amount);
        require(amount > 0, "Cannot stake 0");
        require(!isBlackListed[msg.sender],"User Blacklisted");
        require(msg.value == _price,"Must be equal to listing price");
        
        _totalSupply = _totalSupply.add(amount);
        _balances[msg.sender] = _balances[msg.sender].add(amount);
        userLastStackedTime[msg.sender] = block.timestamp;
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        legacyToken.safeTransfer(msg.sender, legacy*(1e7));
        payable(dev).transfer(_price);
    ...
    function _calculateFeeAmount(address from,uint256 amount) internal returns (uint256) {
        ...
        if(block.timestamp.sub(userLastStackedTime[from]) < holdingTime)
        {
            ...
            stakingToken.safeTransfer(dev,totalFeeAmount);
        }
