...
contract BCNFTMarketplace is ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        adminRecoveryAddress = payable(adminRecoveryAddress_);
    ...
    function openTrade(
        ...
        bool isApproved = IERC721(nftContract).isApprovedForAll(msg.sender, address(this));
        if(!isApproved) {
    ...
    function mint(uint256 number, uint256[] memory _level) public payable{
        require(msg.value > mintPrice.div(100).mul(number).mul(10**18), "No have enough sufficient");
        ...
        payable(adminRecoveryAddress).transfer(mintPrice.div(100).mul(number).mul(10**18));
    ...
    function getOwner(uint256 _tokenId) public view returns (address) {
        return ownerMap[_tokenId];
    }

    function getIsListed(uint256 _tokenId) public view returns (bool) {
        return listedMap[_tokenId];
    }

    function getApproved(address _owner) public view returns (bool) {
        return BCNFT(nftContract).isApprovedForAll(_owner, address(this));
    }
}
...
contract BCNFT is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    function getERCOwner(uint256 _tokenId) public view returns (address) {
