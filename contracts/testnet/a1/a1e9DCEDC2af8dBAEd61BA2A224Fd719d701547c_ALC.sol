...
contract ALC is Ownable{
    ...
    address public constant uniRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; // testnet
    // address public constant uniRouter = 0x10ED43C718714eb63d5aA57B78B54704E256024E; // Live
    address public BUSD = address(0x8B7ba83BF0b026eAD4f2429273FF92C056227870); //testnet
    // address public BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56); //live
    address public SSID = address(0x34728d0121844A8Eb421614B498223F54e25d639); //testnet
    // address public SSID = address(0xe76525d35Ffb906243Ed6C1b291F1C1a17286748); //live
    ...
    function swapAndLiquifyOwner2(uint256 contractTokenBalance) public{
        uint256 half = contractTokenBalance / 2;
    ...
    function addLiquidity(uint256 token0Amt, uint256 token1Amt) public {
        ...
        if(token0Amt > 0 && token1Amt > 0) {
            // IERC20(SSID).approve(address(uniswapV2Router), 0);
            // IERC20(SSID).approve(address(uniswapV2Router), 115792089237316195423570985008687907853269984665640564039457584007913129639935);
