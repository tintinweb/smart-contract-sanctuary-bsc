...
contract NFT721 is Ownable, SignerRole, IERC721, ERC721Base {
    ...
    uint256 public feeBNB = 1e14;

    uint256 public feePreviewRate = 25000;//25000=2.5%
    uint256 public feePreviewRateDenominator = 1000000;

    address public KRC = 0x413C34BF2633edd6f1F1a3B88807Fb3b629C1E2E;
    mapping(address=>mapping(uint256=>bool)) public previewerOf;
    mapping(uint256=>uint256) public previewPriceOf;


    event UpdateFeeBNB(uint256 oldFeeBNB,uint256 newFeeBNB);
    event PreviewBuy(address buyerAddress,uint256 tokenId);
    event SetPreviewPrice(address owner,uint256 tokenId,uint256 oldPrice,uint256 newPrice);

    constructor (string memory name, string memory symbol, address signer, string memory contractURI, string memory tokenURIPrefix) 
    ...
    function updateFeeBNB(uint256 _feeBNB) public onlyOwner {
        require(feeBNB!=_feeBNB,"NFT:_feeBNB error");
        uint oldFeeBNB = feeBNB;
        feeBNB = _feeBNB;
        emit UpdateFeeBNB(oldFeeBNB,feeBNB);
    }
    ...
    function mint(uint256 tokenId, 
        ...
        uint256 previewPrice) public payable{
        require(msg.value==feeBNB,"NFT:fee error");
        ...
        safeTransferETH(feeTo,feeBNB);
        ownerSetPreviewPrice(tokenId,previewPrice);
    }

    function mintByOwner(uint256 tokenId,
        string memory tokenURI,
        uint256 previewPrice) public payable onlyOwner{
        require(msg.value==feeBNB,"NFT:fee error");
    ...
    function setBeforeContractList(address _address,bool flag) public onlyOwner {
        beforeContractList[_address] = flag;
    }

    function ownerSetPreviewPrice(uint256 tokenId,uint256 newPrice) public {
        address owner = ownerOf(tokenId);
        require(owner == msg.sender, "NFT: tokenId error");
        uint256 oldPrice = previewPriceOf[tokenId];
        previewPriceOf[tokenId] = newPrice;
        emit SetPreviewPrice(msg.sender,tokenId,oldPrice,newPrice);
    }


    function previewBuy(uint256 tokenId) external {
        if(previewPriceOf[tokenId]==0){
            revert("NFT:unnecessary");
        }
        require(previewerOf[msg.sender][tokenId]==false,"NFT:bought");
        //需要判断是在何时转入的交易合约
        address owner = ownerOf(tokenId);
        require(owner != address(0), "NFT: tokenId error");
        if(beforeContractList[owner]){
            owner = beforeOwnerOf(tokenId);
        }
       
        uint256 left = previewPriceOf[tokenId];

        if(feePreviewRate>0){
            uint256 fee = left * feePreviewRate / feePreviewRateDenominator;
            require(fee>0,"SafeMath: subtraction overflow");
            //协议费用
            TransferHelper.safeTransferFrom(KRC, msg.sender, feeTo, fee);
            left = left-fee;
        }
        TransferHelper.safeTransferFrom(KRC, msg.sender, owner, left);
        previewerOf[msg.sender][tokenId]=true;
        emit PreviewBuy(msg.sender,tokenId);
    }
...
abstract contract IERC721 is IERC165 {
     ...
     */
    function authorOf(uint256 tokenId) public view virtual returns (address author);
...
contract ERC721 is Context, ERC165, IERC721 {
    ...
    mapping (uint256 => address) private _beforeOwner;

    mapping (address => bool) public beforeContractList;
     ...
     *     bytes4(keccak256('getApproved(4)')) == 0x081812fc
    ...
    function beforeOwnerOf(uint256 tokenId) public view virtual returns (address) {
        address owner = _beforeOwner[tokenId];
        require(owner != address(0), "ERC721: owner query for nonexistent token");
        return owner;
    }

    function authorOf(uint256 tokenId) public view virtual override returns (address) {
        address author = _tokenAuthor[tokenId];
        require(author != address(0), "ERC721: author query for nonexistent token");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal virtual {
        ...
        if(beforeContractList[to]){
            _beforeOwner[tokenId] = _tokenOwner[tokenId];
        }
