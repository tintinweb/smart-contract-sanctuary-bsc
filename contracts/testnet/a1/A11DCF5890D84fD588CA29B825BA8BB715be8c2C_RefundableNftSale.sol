...
contract RefundableNftSale is Ownable, Pausable, IERC721Receiver {
    ...
    mapping(uint256 => bool) isTokenAvailable;
    ...
    mapping(uint256 => RefundableToken) refundableTokens;

    mapping(uint256 => bool) isTokenSold;
    uint256[] public soldTokens;

    mapping(uint256 => bool) isTokenRefunded;
    ...
    function buyToken() external whenNotPaused {
        require(availableTokens.length > 0, "Not tokens available");

        uint256 nextToken = availableTokens[availableTokens.length - 1];
    ...
    function refundToken(uint256 _tokenId) external whenNotPaused {
        ...
        require(isTokenSold[_tokenId], "Token not sold");
        require(!isTokenRefunded[_tokenId], "Token already refunded");

        nftToken.safeTransferFrom(msg.sender, address(this), _tokenId);
        paymentToken.safeTransferFrom(address(this), msg.sender, _token.price);
