...
contract INO is ERC1155ReceiverUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    address public mysteryBoxes;
    address public currency;
    uint256 public startSaleAt;
    uint public globalBuyLimit;
    ...
    event ChangeINO(address _mysteryBoxes);
    event ChangeCurrency(address _currency);
    event ChangeStartSaleAt(uint256 _startSaleAt);
    event ChangeGlobalBuyLimit(uint _globalBuyLimit);
    event CreateINO(
        uint256 _tokenId,
        uint256 _amount,
        uint256 _limit,
    ...
    function initialize(
        address _mysteryBoxes,
        address _currency
    ) initializer public {
        __Ownable_init();
        __ERC1155Receiver_init();
        setINO(_mysteryBoxes);
        setCurrency(_currency);
    }


    function setINO(address _mysteryBoxes) public onlyOwner {
        require(_mysteryBoxes != address(0), "Invalid MysteryBoxes address");
        mysteryBoxes = _mysteryBoxes;
        emit ChangeINO(_mysteryBoxes);
    }
    ...
    function setGlobalBuyLimit(uint _globalBuyLimit) public onlyOwner {
    ...
    function purchase(uint256 _tokenId, uint256 _amount)
    external
    whenNotPaused
    nonReentrant
    {
        ...
        require(
            walletTotalPurchasedBoxes + _amount <= globalBuyLimit,
            "Exceed global buy limit"
        );
        uint256 walletPurchasedBox = purchasedPerBox[msg.sender][_tokenId];
        require(
            walletPurchasedBox + _amount <= box.limit,
            "Exceed box buy limit"
        );
        totalPurchasedPerBox[_tokenId] += _amount;
        totalPurchasedBoxes[msg.sender] = walletTotalPurchasedBoxes + _amount;
        purchasedPerBox[msg.sender][_tokenId] = walletPurchasedBox + _amount;
        // transfer fund to owner wallet
        IERC20Upgradeable(currency).transferFrom(msg.sender, owner(), box.price * _amount);
        // transfer mysteryBoxes to buyer
        IERC1155Upgradeable(mysteryBoxes).safeTransferFrom(
            owner(),
            msg.sender,
            _tokenId,
            _amount,
        ...
        emit Purchase(msg.sender, _tokenId, _amount, block.timestamp);
    }
