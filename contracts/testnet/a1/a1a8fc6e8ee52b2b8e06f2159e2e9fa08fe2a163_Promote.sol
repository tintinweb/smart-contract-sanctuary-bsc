...
interface IIDO{
    function startTime() external returns (uint256 startTime);
    function endTime() external returns (uint256 endTime);
...
contract Promote is Initializable, Member {
    ...
    function pendingReward(uint256 nftId) external view returns(uint256) {
        ...
        return poolInfo[nftAttr.Level].accPerShare - nftRewardInfo[nftId].rewardDebt;
    }
    ...
    function nodeApply() external {
        require(block.timestamp >= IIDO(manager.members("ido")).startTime(), "ido not start");
        require(block.timestamp <= IIDO(manager.members("ido")).endTime(), "ido is end");
