...
contract BigexToken is ERC20, Ownable, BigexSupportToken {
	...
	{
		if (amount <= 0) {
			revert("ERC20: amount greater than zero");
		}
		if (blackListWallet[from]) {
			revert("ERC20: transfer from is blacklist");
		}
		if (blackListWallet[to]) {
			revert("ERC20: transfer to is blacklist");
		}
		...
		if (swapContract != address(0)) {
			if (!ILockToken(swapContract).canTransfer(from, amount)) {
				revert("ERC20: Some available balance has been locked and will be unlocked gradually.");
			}
		}
