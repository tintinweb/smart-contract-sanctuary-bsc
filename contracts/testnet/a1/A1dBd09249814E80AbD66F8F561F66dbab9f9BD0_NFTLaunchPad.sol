...
contract NFTCollection is ERC721A, ERC2981, Ownable, VerifyUser {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _phaseArgs.length; i++) {
            ...
            for (uint256 j = 0; j < _phaseArgs[i].currencies.length; j++) {
                ...
                if (_phaseArgs[i].isWhiteListed) {
                    phases[i].WhiteListStartTime = _phaseArgs[i]
                        .WhiteListEndTime;
                    phases[i].WhiteListEndTime = _phaseArgs[i].WhiteListEndTime;
                } else {
                    phases[i].WhiteListStartTime = 0;
                    phases[i].WhiteListEndTime = 0;
                }
    ...
    function mint(
        ...
        if (
            ...
            require(WhiteListed, "NFTCollection: only white list can mint");
        }
...
contract NFTLaunchPad is Initializable, RoleManager {
     ...
     */
    function updatePublicKey(address _newPlatformPublicKey) external isOperator {
    ...
    function createLaunchPad(
        ...
        for (uint256 i = 0; i < _phaseArgs.length; i++) {
            for (uint j = 0; j < _phaseArgs[i].currencies.length; j++) {
                require(getBrokerage(_phaseArgs[i].currencies[j]) != 0,"NFTLaunchpad: Currency not Supported");
            }
        }
        ...
        if (_enableWhiteList) {
          ...
          require(
             ...
             require(
                _phaseArgs[i].WhiteListEndTime > _phaseArgs[i].WhiteListStartTime,
