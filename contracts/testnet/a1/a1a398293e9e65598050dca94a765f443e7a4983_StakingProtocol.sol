    ...
    ClaimSender
{
    ...
    {
        __Ownable_init();
        __init_AllowedAssets(
            _assets,
            _assetRewardCoefficients
        );
...
////////////////////////////////////////// StakingStorage
    ...
    function deposit(
        ...
        uint256[] memory _tokensNftObligation
    )
        public
    {
        __deposit(
...
interface INFTO is IBalancesLikeERC1155, IERC721Upgradeable, IERC721MetadataUpgradeable {

   function hold(uint256 heroId) external payable;

   function unhold(uint256 heroId) external payable;
   
   function issoul(uint256 tokenId) external view returns(bool);

   function getPointAddress(uint256 tokenId) external view returns(address);
...
interface IBalancesLikeERC1155
{
    ...
    function get(uint256 tokenId, address asset_address) view external  returns(uint256 balance);


   function add(uint256 tokenId, address asset_address, uint256 value) external;

   function set(uint256 tokenId, address asset_address, uint256 value) external;

   function sub(uint256 tokenId, address asset_address, uint256 value) external;
...
////////////////////////////////////////// initialize
    ...
    function __init_StakingStorage(
        address _oracle,
        address _nftObligationManager
    ) internal {
        oracle = _oracle;
        nftObligationManager = _nftObligationManager;
    }

////////////////////////////////////////// fields definition

    address public oracle;
    address public nftObligationManager;
    ...
    // Account address => NFT Obligation tokenId[]
    mapping (address => EnumerableSetUpgradeable.UintSet) internal actualAccountNftObligations;
    // Account address => NFT Boost tokenId
    EnumerableMapUpgradeable.AddressToUintMap internal actualAccountNftBoost; // todo категорически не устраивает tokenId = 0 по умолчанию
...
////////////////////////////////////////// view methods
    ...
    function getAccountBalances(address _account)
        public
        view
        returns (address[] memory assets, uint256[] memory balances, uint256[] memory nftObligations)
    {
        (assets, balances) = actualAccountBalancesInERC20[_account].getKeyValueList();
        nftObligations = actualAccountNftObligations[_account].values();
    }
    ...
    function getNftObligationManager()
        internal
        view
        returns (INftoManager)
    {
        return INftoManager(nftObligationManager);
    }

    function getNftObligation()
        internal
        view
        returns (INFTO)
    {
        return INFTO(getNftObligationManager()._NFTO());
    }
        ...
        returns(uint256 _amountInBase)
    {
        ...
        for (uint256 i = 0; i < _accountPeriodInfo.assetsAmounts.length(); ++i) {
            ...
            _amountInBase += getOracle().consultByHistory(_asset, _accountPeriodInfo.time, block.timestamp)
                * _amount
                * getRewardCoefficient(_asset)
                / PERCENT
...
////////////////////////////////////////// deposit/withdraw
    ...
    function __UpdateAccountPeriodInfo(
        AccountPeriodInfo storage _accountPeriodInfo
    )
        private
        nonReentrant
    {
        uint256 _amountInBase = 0;
        if (_accountPeriodInfo.time == 0 || _accountPeriodInfo.isCalculated) {
            return;
        }
        for (uint256 i = 0; i < _accountPeriodInfo.assetsAmounts.length(); ++i) {
            (address _asset, uint256 _amount) = _accountPeriodInfo.assetsAmounts.at(i);
            _amountInBase += getOracle().consultByHistoryWithUpdate(_asset, _accountPeriodInfo.time, block.timestamp)
                * _amount
                * getRewardCoefficient(_asset)
                / PERCENT
                / 10 ** IERC20MetadataUpgradeable(_asset).decimals()
        ...
        _accountPeriodInfo.isCalculated = true;
        _accountPeriodInfo.amountInBase = _amountInBase;
    }
    ...
    function __deposit(
        ...
        for (uint256 i = 0; i < _tokensNftObligation.length; ++i) {
            actualAccountNftObligations[_account].add(_tokensNftObligation[i]);
            getNftObligation().safeTransferFrom(_account, address(this), _tokensNftObligation[i]);

            (address[] memory _assetsFromNFTO, uint256[] memory _amountsFromNFTO) = getNftObligationManager().currencyBalancesList(_tokensNftObligation[i]);

            for (uint256 j = 0; j < _assetsFromNFTO.length; ++j) {
                currentPeriod.assetsAmounts.add(_assetsFromNFTO[j], _amountsFromNFTO[j]);
            }
        }
    }
    ...
    function __withdraw(
        ...
        for (uint256 i = 0; i < _assets.length; ++i) {
            ...
            actualAccountBalancesInERC20[_account].sub(currentAsset, currentAmount);
        ...
        for (uint256 i = 0; i < _tokensNftObligation.length; ++i) {
            bool _tokenExist = actualAccountNftObligations[_account].remove(_tokensNftObligation[i]);
            require(_tokenExist, "StakingStorage::__withdraw - only NFTO token owner");

            getNftObligation().safeTransferFrom(address(this), _account, _tokensNftObligation[i]);

            (address[] memory _assetsFromNFTO, uint256[] memory _amountsFromNFTO) = getNftObligationManager().currencyBalancesList(_tokensNftObligation[i]);

            for (uint256 j = 0; j < _assetsFromNFTO.length; ++j) {
                currentPeriod.assetsAmounts.sub(_assetsFromNFTO[j], _amountsFromNFTO[j]);
            }
        }
    }
...
////////////////////////////////////////// methods for view
        ...
        returns (uint256 _rewardAllowedToClaim, uint256 _rewardInDelay)
    {
        ...
        (
            ...
            uint256 _cumulativeDelayBalance,
            uint256 _cumulativeDelayPercent,
        ...
        uint256 _medianOfPercent = _cumulativeFullPercent / _period;

        _rewardInDelay = CompoundPercent.compound(_medianOfBalance, baseRewardInPercent + _medianOfPercent, _period); // _compoundInterestOfFull
        ...
        _medianOfBalance -= _cumulativeDelayBalance / rewardDelayPeriod;
        _medianOfPercent -= _cumulativeDelayPercent / rewardDelayPeriod;

        _rewardAllowedToClaim = CompoundPercent.compound(_medianOfBalance, baseRewardInPercent + _medianOfPercent, _period); // _compoundInterestOfAvailable
        _rewardInDelay -= _rewardAllowedToClaim;
        return (_rewardAllowedToClaim, _rewardInDelay);
    }

    // |----_period[N]---delay---_period[N+1]---|
    //      delay < _period[N].time
    //          balanceDelay = (_period[N+1].time - _period[N].time) * _periodAmountInBase
    //          percentDelay = __calculateCumulativeTimePercent(_period[N].time, _period[N].time)
    //      else
    //          balanceDelay = (delay - _period[N+1].time) * _periodAmountInBase
    //          percentDelay = __calculateCumulativeTimePercent(_period[N].time, _period[N+1].time) - __calculateCumulativeTimePercent(delay, _period[N].time)
        ...
        )
    {
        ...
        _cumulativeDelayBalance = 0;
        _cumulativeDelayPercent = 0;
        _periodBegin = 0;
        uint256 _periodDelayEnd = block.timestamp - rewardDelayPeriod;

        uint256 _periodEnd = block.timestamp;
        uint256 _timedRewardPeriodEnd = block.timestamp;

        bool _calculationOfCumulativeDelayBalanceInProcess = true;
        bool _calculationOfCumulativeDelayPercentInProcess = true;

        bool _needBreak = false;
        for (uint256 i = accountLastPeriodNumber[_account]; i > 0;)
        {
            --i;
            AccountPeriodInfo storage _period = accountPeriodDataByPeriodNumber[_account][i];
            if (_period.time <= accountLastClaimTime[_account]) {
                _periodBegin = accountLastClaimTime[_account];
                _needBreak = true;
            } else {
                _periodBegin = _period.time;
            }

            uint256 _periodAmountInBase = __getAmountInBase(_period);
            _cumulativeFullBalance += (_periodEnd - _periodBegin) * _periodAmountInBase;

            if (_periodDelayEnd < _periodBegin) {
                _cumulativeDelayBalance += (_periodEnd - _periodBegin) * _periodAmountInBase;
            } else if (_calculationOfCumulativeDelayBalanceInProcess) {
                _cumulativeDelayBalance += (_periodDelayEnd - _periodBegin) * _periodAmountInBase;
                _calculationOfCumulativeDelayBalanceInProcess = false;
            }
            ...
            if (_period.isWithdraw || _needBreak) { // end of Basic time coefficients
                ...
                if (_periodDelayEnd < _periodBegin) {
                    _cumulativeDelayPercent += _calculatedCumulativeTimePercent;
                } else if (_calculationOfCumulativeDelayPercentInProcess) {
                    _cumulativeDelayPercent += _calculatedCumulativeTimePercent - __calculateCumulativeTimePercent(_periodDelayEnd, _periodBegin);
                    _calculationOfCumulativeDelayPercentInProcess = false;
                }
    ...
    function __calculateCumulativeTimePercent(uint256 _timedRewardPeriodBegin, uint256 _timedRewardPeriodEnd)
        ...
        returns (uint256)
    {
        uint256 _timedRewardPeriod = _timedRewardPeriodEnd - _timedRewardPeriodBegin;
        if (timedRewardPeriod <= _timedRewardPeriod) {
            ...
            return (timedRewardPeriod / 2 + (_timedRewardPeriod - timedRewardPeriod)) * timedRewardInPercent;
        }
...
////////////////////////////////////////// methods for claim
    ...
    {
        (
            uint256 _cumulativeAllowedBalance,
            uint256 _cumulativeAllowedPercent,
            uint256 _periodBegin
        ) = __calculateCumulativeValuesForClaim(_account);

        if (_cumulativeAllowedBalance == 0) {
            return 0;
        }

        uint256 _period = block.timestamp - _periodBegin;
        uint256 _medianOfBalance = _cumulativeAllowedBalance / _period;
        uint256 _medianOfPercent = _cumulativeAllowedPercent / _period;

        return CompoundPercent.compound(_medianOfBalance, baseRewardInPercent + _medianOfPercent, _period); // _compoundInterestOfFull
    }
    ...
    {
        _cumulativeAllowedBalance = 0;
        _cumulativeAllowedPercent = 0;
        _periodBegin = 0;
        uint256 _periodDelayEnd = block.timestamp - rewardDelayPeriod;
        uint256 _periodEnd = _periodDelayEnd;
        uint256 _timedRewardPeriodEnd = _periodDelayEnd;
        ...
        for (uint256 i = accountLastPeriodNumber[_account]; i > 0;)
        {
            ...
            if (_periodDelayEnd > _periodBegin) {
                uint256 _periodAmountInBase = __getAmountInBase(_period);
                _cumulativeAllowedBalance += (_periodEnd - _periodBegin) * _periodAmountInBase;

                if (_period.isWithdraw || _needBreak) {
                    _cumulativeAllowedPercent += __calculateCumulativeTimePercent(_periodBegin, _timedRewardPeriodEnd);
                    _timedRewardPeriodEnd = _periodBegin;
                }
            }
...
////////////////////////////////////////// fields definition
    ...
    address public rewardCurrency;

////////////////////////////////////////// methods view

    function getRewardCurrencyBalance()
        public
        view
        returns (uint256)
    {
        return IERC20MetadataUpgradeable(rewardCurrency).balanceOf(address(this));
    }
...
////////////////////////////////////////// methods for claim
        ...
        internal
    {
        ...
        require(_rewardAllowedToClaim <= getRewardCurrencyBalance(), "StakingProtocol::claim - not enough money to reward");

        accountLastClaimTime[_account] = block.timestamp - rewardDelayPeriod;
        IERC20MetadataUpgradeable(rewardCurrency).safeTransfer(_account, _rewardAllowedToClaim);
    }
}
...
abstract contract AssetsManager
{
...
////////////////////////////////////////// write methods
    ...
    {
        require(_rewardCoefficientInPercent > 0, '_rewardCoefficient should be more then 0');
        require(_rewardCoefficientInPercent <= MAX_ALLOWED_REWARD_COEFFICIENT_IN_PERCENT, '_rewardCoefficient is too big');
        assetWithRewardCoefficients.set(_asset, _rewardCoefficientInPercent);
    }
