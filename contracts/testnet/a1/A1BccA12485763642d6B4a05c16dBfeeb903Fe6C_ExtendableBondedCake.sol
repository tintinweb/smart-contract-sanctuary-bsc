...
contract ExtendableBondedCake is ExtendableBond {
    ...
    function secondsToPancakeLockExtend() public view returns (uint256 secondsToExtend) {
        ...
        if (
            ...
            cakeInfo.lockEndTime - cakeInfo.lockStartTime < cakeMaxLockDuration
        ) {
            if (cakeInfo.lockEndTime > block.timestamp) {
                uint256 totalLockDuration = checkPoints.maturity - cakeInfo.lockStartTime;
                return
                    MathUpgradeable.min(totalLockDuration, cakeMaxLockDuration) +
                    cakeInfo.lockStartTime -
