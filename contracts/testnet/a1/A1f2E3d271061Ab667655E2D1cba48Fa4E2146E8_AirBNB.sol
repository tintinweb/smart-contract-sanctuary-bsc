...
contract AirBNB is Context, Ownable {
    ...
    function calculateBalance(address user) private view returns (uint256) {
        require(claimDates[user] > 0, "GetBalance: Error claim date");
    ...
    function invest(address ref) public payable {
        ...
        require(amount > 0, "Invest: error invest amount");
        ...
        if (user == owner()) {
            balances[owner()] = SafeMath.add(getReferralCommission(amount), 1);
        } else {
    ...
    function createReferralIncome(address user, uint256 amount) private {
        ...
        depositReferralAmount = min(getReferralAmount(amount, firstLineReferralPercent), getReferralAmount(deposits[currentParent], firstLineReferralPercent));
        ...
        depositReferralAmount = min(getReferralAmount(amount, secondLineReferralPercent), getReferralAmount(deposits[currentParent], secondLineReferralPercent));
        leftReferral = SafeMath.add(leftReferral, depositReferralAmount);
        ...
        depositReferralAmount = min(getReferralAmount(amount, thirdLineReferralPercent), getReferralAmount(deposits[currentParent], thirdLineReferralPercent));
        leftReferral = SafeMath.add(leftReferral, depositReferralAmount);
        ...
        depositReferralAmount = min(getReferralAmount(amount, fourthLineReferralPercent), getReferralAmount(deposits[currentParent], fourthLineReferralPercent));
        leftReferral = SafeMath.add(leftReferral, depositReferralAmount);
        ...
        depositReferralAmount = min(getReferralAmount(amount, fifthLineReferralPercent), getReferralAmount(deposits[currentParent], fifthLineReferralPercent));
        leftReferral = SafeMath.add(leftReferral, depositReferralAmount);
    ...
    function refill() public {
        ...
        require(SafeMath.add(deposits[user], amountToRefill) <= MAXIMUM_DEPOSIT_AMOUNT, 'Invest: attempt to exceed the limit');

        claimDates[user] = now;
        balances[user] = 0;
        deposits[user] = SafeMath.add(deposits[user], SafeMath.sub(amountToRefill, referralCommission));
        createReferralIncome(user, referralCommission);
    }
    ...
    function withdrawDeposit() public {
        ...
        uint256 ceoCommission = getCeoCommission(userDeposit);

        // TODO add += for audit bug create
        balances[user] = momentumUserBalance;

        ceoAddress.transfer(min(ceoCommission, amountOnContract));
        msg.sender.transfer(min(SafeMath.sub(userDeposit, ceoCommission), amountOnContract));
    }
