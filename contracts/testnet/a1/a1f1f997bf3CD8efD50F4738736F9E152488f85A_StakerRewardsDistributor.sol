...
/**
 * @title StakerRewardsDistributor
 * @notice It handles the distribution of fees using
 * WETH along with the auto-compounding of SESA.
 */
contract StakerRewardsDistributor is ReentrancyGuard, AccessControl {
    ...
    /**
     * @notice Constructor
     * @param _sesameToken address of the token staked (SESA)
     ...
     * @param amount amount to deposit (in SESA)
     * @param claimRewardToken whether to claim reward tokens
     * @dev There is a limit of 1 SESA per deposit to prevent potential manipulation of current shares
     */
        ...
        nonReentrant
    {
        ...
        // Transfer SESA tokens to this address
        sesameToken.safeTransferFrom(msg.sender, address(this), amount);
        ...
        // Verify SESA token allowance and adjust if necessary
        _checkAndAdjustSESATokenAllowanceIfRequired(
            amount,
    ...
    /**
     * @notice Withdraw staked tokens (and collect reward tokens if requested)
     * @param amount amount of SESA to withdraw
     * @param claimRewardToken whether to claim reward tokens
     */
    function withdrawSESA(uint256 amount, bool claimRewardToken)
        external
        nonReentrant
    {
        require(amount > 0);
        uint256 shares = (amount * PRECISION_FACTOR) /
            calculateSharePriceInSESA();
        _withdraw(shares, claimRewardToken);
    }
    ...
    /**
     * @notice Calculate value of SESA for a user given a number of shares owned
     * @param user address of the user
     */
    function calculateSharesValueInSESA(address user)
        external
        view
        returns (uint256)
    {
    ...
    /**
     * @notice Calculate price of one share (in SESA token)
     * Share price is expressed times 1e18
     */
    function calculateSharePriceInSESA() public view returns (uint256) {
        (uint256 totalAmountStaked, ) = tokenDistributor.userInfo(
            address(this)
        );
     ...
     */
    function _checkAndAdjustSESATokenAllowanceIfRequired(
        uint256 _amount,
        address _to
    ) internal {
        if (sesameToken.allowance(address(this), _to) < _amount) {
            sesameToken.approve(_to, type(uint256).max);
        }
    }
    ...
    function _withdraw(uint256 shares, bool claimRewardToken) internal {
        ...
        // Retrieve total amount staked and calculated current amount (in SESA)
        (uint256 totalAmountStaked, ) = tokenDistributor.userInfo(
            address(this)
        );
        ...
        // Transfer SESA tokens to sender
        sesameToken.safeTransfer(msg.sender, currentAmount);
 ...
 * @notice It handles the distribution of SESA token.
 * It auto-adjusts block rewards over a set number of periods.
 */
contract TokenDistributor is ReentrancyGuard {
    using SafeERC20 for IERC20;
    using SafeERC20 for ISesame;

    struct StakingPeriod {
        uint256 rewardPerBlockForPlayer;
    ...
    ISesame public immutable sesameToken;

    // Contract which distributes token to players
    address public immutable playerRewardDistributor;

    // Contract which split token to team & treasury
    ...
    // Tokens distributed per block for player
    uint256 public rewardsPerBlockForPlayer;

    // Tokens distributed per block for other purposes (team + treasury)
    ...
    /**
     * @notice Constructor
     * @param _sesameToken SESA token address
    ...
    constructor(
        address _sesameToken,
        address _playerRewardDistributor,
        address _tokenSplitter,
        uint256 _startBlock,
        uint256[] memory _rewardsPerBlockForPlayer,
        ...
        uint256 nonCirculatingSupply = ISesame(_sesameToken).SUPPLY_CAP() -
            ISesame(_sesameToken).totalSupply();

        uint256 amountTokensToBeMinted;

        for (uint256 i = 0; i < _numberPeriods; i++) {
            amountTokensToBeMinted +=
                (_rewardsPerBlockForStaking[i] +
                    _rewardsPerBlockForOthers[i] +
                    _rewardsPerBlockForPlayer[i]) *
                _periodLengthesInBlocks[i];

            stakingPeriod[i] = StakingPeriod({
                rewardPerBlockForPlayer: _rewardsPerBlockForPlayer[i],
        ...
        // 2. Store values
        sesameToken = ISesame(_sesameToken);
        playerRewardDistributor = _playerRewardDistributor;
        tokenSplitter = _tokenSplitter;
        rewardsPerBlockForPlayer = _rewardsPerBlockForPlayer[0];
    ...
    function stakingAPR() public view returns (uint256) {
        return
            ((rewardPerBlockForStaking * 365 days * 1 ether) / 3) /
            totalAmountStaked;
    }
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        // Transfer SESA tokens to this contract
        sesameToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        // Transfer SESA tokens to the sender
        sesameToken.safeTransfer(msg.sender, amount);
    ...
    function withdrawAll() external nonReentrant {
        ...
        // Transfer SESA tokens to the sender
        sesameToken.safeTransfer(msg.sender, amountToTransfer);
    ...
    function _updatePool() internal {
        ...
        uint256 tokenRewardForPlayer = multiplier * rewardsPerBlockForPlayer;
        ...
        if (tokenRewardForStaking > 0) {
            ...
            bool mintStatus = sesameToken.mint(
                address(this),
            ...
            sesameToken.mint(playerRewardDistributor, tokenRewardForPlayer);
            sesameToken.mint(tokenSplitter, tokenRewardForOthers);
        }
    ...
    function _updateRewardsPerBlock(uint256 _newStartBlock) internal {
        ...
        // Update rewards per block
        rewardsPerBlockForPlayer = stakingPeriod[currentPhase]
            .rewardPerBlockForPlayer;
...
interface ISesame is IERC20 {
    ...
    function burned() external returns (uint256);
}