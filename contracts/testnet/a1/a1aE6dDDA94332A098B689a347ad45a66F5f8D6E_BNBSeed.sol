...
/**
official website: https://bnbseed.io
...
contract BNBSeed {
	...
	constructor(address payable wallet) public {
        ...
        plans.push(Plan(30, 25920));
		plans.push(Plan(30, 40320));
		plans.push(Plan(30, 54720));
        plans.push(Plan(10, 25920));
		plans.push(Plan(10, 40320));
		plans.push(Plan(10, 54720));
	}
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		if(user.yourboomplan <= plan) {
			user.yourboomplan = plan+1;
		}
		if(user.yourboomplan == 0){
			user.yourboompercent = 0;
		}else if(user.yourboomplan == 1){
			user.yourboompercent = 18;
		}else if(user.yourboomplan == 2){
			user.yourboompercent = 28;
		}else if(user.yourboomplan == 3){
			user.yourboompercent = 38;
		}
		...
		for (uint256 i = 0; i < users[msg.sender].deposits.length; i++) {
			if(user.deposits[i].plan < 3){
				if(user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(1 days)) > block.timestamp){
					amount1 = amount1.add(users[msg.sender].deposits[i].amount);
				}
			}
		}

		user.yourbonusminmoney = amount1*user.yourboompercent;
	...
	function bonus() public payable {
		...
		for (uint256 i = 0; i < users[msg.sender].deposits.length; i++) {
			if(user.deposits[i].plan < 3){
				if(user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(1 days)) > block.timestamp){
					amount = amount.add(users[msg.sender].deposits[i].amount);
				}
			}
		}
		uint8 plan = uint8(user.yourboomplan+2);
		require(plan > 2, "Invalid plan");
        require(plan < 6, "Invalid plan");
		
		if(plan==3){
			require(msg.value >= amount*18/1000);
		}else if(plan==4){
			require(msg.value >= amount*28/1000);
		}else if(plan==5){
			require(msg.value >= amount*38/1000);
		}
        ...
        uint index4 = uint(keccak256(abi.encodePacked(nonce, msg.sender, block.difficulty, index3)));
        uint index5 = uint(keccak256(abi.encodePacked(nonce, msg.sender, block.difficulty, index4))) % totalSize;
		if(index5 > 50){
			if(plan==3){
				user.yourbonuspercent = user.yourbonuspercent.add(18*30);
				user.yourbonus = 1;
			}else if(plan==4){
				user.yourbonuspercent = user.yourbonuspercent.add(28*30);
				user.yourbonus = 1;
			}else if(plan==5){
				user.yourbonuspercent = user.yourbonuspercent.add(38*30);
				user.yourbonus = 1;
			}
			totalrandom = totalrandom.add(block.timestamp);
			...
			if(plan==3){
				user.yourbonuspercent = user.yourbonuspercent.add(18*10);
				plan = 6;
				user.yourbonus = 0;
			}else if(plan==4){
				user.yourbonuspercent = user.yourbonuspercent.add(28*10);
				plan = 7;
				user.yourbonus = 0;
			}else if(plan==5){
				user.yourbonuspercent = user.yourbonuspercent.add(38*10);
				plan = 8;
				user.yourbonus = 0;
			}
			totalInvested = totalInvested.add(msg.value);
	...
	function getcurrentwithdrawntime(address userAddress) public view returns (uint256){
	    return users[userAddress].withdrawntimes;
	}	
