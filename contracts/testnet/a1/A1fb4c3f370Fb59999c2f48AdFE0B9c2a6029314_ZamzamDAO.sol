 ...
 */
contract ZamzamDAO is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
  struct Project {
    uint256 goalAmount;
    uint256 totalRaised;
    uint256 totalSpent;
  ...
  CountersUpgradeable.Counter private projectIds;

  mapping(uint256 => Project) private projects;

  address private _adminWallet;

  IERC20Upgradeable private zamzamToken;
   ...
   * Its new goal amount is `goalAmount`, new unpublished status is `unpublished`.
   */
  event ProjectUpdated(uint256 indexed projectId, uint256 goalAmount, bool unpublished);
   ...
   */
  event FundsReleased(uint256 indexed projectId, uint256 tokenAmount, address indexed receiver);
  ...
  /**
   * @dev Emitted when refundsAllowed field is updated on a project `projectId`.
   * `value` is the value of refundsAllowed for project `projectId`.
   */
  event RefundsAllowedUpdated(uint256 indexed projectId, bool value);

  /**
   * @dev Emitted when adminWallet is updated
   * `newAdminWallet` is the value of adminWallet
   */
  event AdminWalletChanged(address indexed newAdminWallet);
  ...
  modifier whenDonationsOpen(uint256 projectId) {
    ...
    require(projectId < projectIds.current(), "ZamzamDAO: Invalid projectId");
    require(!project.unpublished, "ZamzamDAO: Project unpublished");
    require(project.totalRaised < project.goalAmount, "ZamzamDAO: Goal amount reached");
    require(!project.refundsAllowed, "ZamzamDAO: Refunds allowed");
    _;
  }
  ...
  function initialize(
    address _zamzamToken,
    address adminWallet_,
    address zamzamAdmin,
    address fiatDonationController
  ) public initializer {
    _adminWallet = adminWallet_;
    zamzamToken = IERC20Upgradeable(_zamzamToken);
    _grantRole(DEFAULT_ADMIN_ROLE, zamzamAdmin);
    _grantRole(FIAT_DONATION_CONTROLLER_ROLE, fiatDonationController);
  }
  ...
  /**
   * @notice Updates the adminWallet
   * @dev Updates the _adminWallet field
   * @param newAdminWallet The address of the new _adminWallet
   * @return Returns the new _adminWallet
   */
  function changeAdminWallet(address newAdminWallet) external onlyRole(DEFAULT_ADMIN_ROLE) returns (address) {
    _adminWallet = newAdminWallet;
    emit AdminWalletChanged(_adminWallet);
   ...
   */
  function createProject(uint256 _goalAmount) external onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256) {
    require(_goalAmount > 0, "ZamzamDAO: Invalid amount V2 contract");
    ...
    emit ProjectCreated(newProjectId, _goalAmount);
   ...
   * @dev Update the goal amount of a Zamzam project, only a Zamzam admin can update a Zamzam project
   * @param _projectId The project ID of the project to be updated
   * @param _goalAmount The new goalAmount of the project
   * @return Returns the projectId of the created project
   */
  function updateProjectGoalAmount(uint256 _projectId, uint256 _goalAmount)
    external
    ...
    returns (uint256)
  {
    ...
    require(_goalAmount > 0, "ZamzamDAO: Invalid goal amount");

    _project.goalAmount = _goalAmount;

    emit ProjectUpdated(_projectId, _goalAmount, _project.unpublished);
   ...
   * @dev Update the goal amount of a Zamzam project, only a Zamzam admin can update a Zamzam project
   * @param _projectId The project ID of the project to be updated
   * @param _unpublished The new goalAmount of the project
   * @return Returns the projectId of the created project
   */
  ...
  /**
   * @dev Sets refundsAllowed field on a proejct `projectId`
   * @param projectId The ID of the project for whom refundsAllowed will be updated
   * @return Returns the new value of refundsAllowed for the project `projectId`
   * Only Zamzam admin can set this field
   */
  function updateRefundsAllowed(uint256 projectId, bool value) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
    Project storage project = projects[projectId];

    require(projectId < projectIds.current(), "ZamzamDAO: Invalid projectId");
    project.refundsAllowed = value;

    emit RefundsAllowedUpdated(projectId, value);
  ...
  function donate(uint256 projectId, uint256 tokenAmount)
    ...
    returns (bool)
  {
    return _donate(projectId, tokenAmount, msg.sender);
  }
  ...
  function donateFiat(
    ...
    uint256 tokenAmount,
    address donor
  ) external nonReentrant whenDonationsOpen(projectId) onlyRole(FIAT_DONATION_CONTROLLER_ROLE) returns (bool) {
    return _donate(projectId, tokenAmount, donor);
  }
   ...
   * The Zamzam admin then releases the tokens to the appropriate addresses. We cannot release funds if the project is unpublished
   ...
   */
  function releaseFunds(uint256 projectId, uint256 tokenAmount)
    external
    nonReentrant
    onlyRole(DEFAULT_ADMIN_ROLE)
    returns (bool)
  {
    ...
    require(!project.unpublished, "ZamzamDAO: Project unpublished");
    require(!project.refundsAllowed, "ZamzamDAO: Refunds are allowed");
    require((project.totalRaised - project.totalSpent) >= tokenAmount, "ZamzamDAO: Insufficient funds");

    SafeERC20Upgradeable.safeTransfer(zamzamToken, _adminWallet, tokenAmount);

    project.totalSpent += tokenAmount;

    emit FundsReleased(projectId, tokenAmount, _adminWallet);
    return true;
  }
  ...
  function refundDonor(uint256 projectId) external nonReentrant returns (bool) {
    ...
    SafeERC20Upgradeable.safeTransfer(zamzamToken, msg.sender, _refundableAmount);
  ...
  function _donate(
    ...
    SafeERC20Upgradeable.safeTransferFrom(zamzamToken, msg.sender, address(this), tokenAmount);
