...
abstract contract GRNDNftStaking is ERC1155Receiver, Ownable {
    ...
    function onERC1155Received(
        ...
        uint256 id,
        uint256, /*value*/
        bytes calldata cycleAmount /*data*/
    ) external virtual override returns (bytes4) {
        require(
            address(whitelistedNftContract) == _msgSender(),
            "NftStaking: contract not whitelisted"
        );
        uint amount = bytesToUint(cycleAmount);

        require(amount >= 2, "stake cycle must more than 2");
        _stakeNft(id, from, amount);
    ...
    function onERC1155BatchReceived(
        ...
        require(amount >= 2, "stake cycle must more than 2");
        _batchStakeNfts(ids, from, amount);
    ...
    function unstakeNft(uint256 tokenId) external virtual {
        ...
        if (enabled) {
            ...
            require(
                currentCycle - tokenInfo.depositCycle >= tokenInfo.stakeCycle,
            ...
            for (uint256 i = 0; i < currentStakingNFT.length; i++) {
                if (currentStakingNFT[i] == tokenId) {
                    currentStakingNFT[i] = currentStakingNFT[
                        currentStakingNFT.length - 1
                    ];
    ...
    function getCurrentStakingNFTLength() public view returns (uint256) {
