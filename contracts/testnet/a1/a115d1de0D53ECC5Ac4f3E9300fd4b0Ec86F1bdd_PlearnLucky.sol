...
contract PlearnLucky is IPlearnLucky, ReentrancyGuard, Ownable {
    ...
    IConfirmBracket public confirmBracket;
    ...
    event NewConfirmBracket(address confirmBracket);
    event TicketConfirm(
        address confirmer,
        uint256 luckyId,
        uint256 ticketId,
    ...
    constructor(
        ...
        confirmBracket = IConfirmBracket(_confirmBracketAddress);
    }
     ...
     */
    function closeLucky(uint256 _luckyId) external onlyOperatorOrOwner nonReentrant {
        Lucky storage luckyInfo = _lucks[_luckyId];
        ...
        luckyInfo.endTime = block.timestamp;
     ...
     * @param _luckyId: lucky id
     * @param _ticketNumber: ticket numbers to confirm
     * @param _bracket: bracket for the ticket ids
    ...
    function confirmTicket(
        ...
        require(_isConfirmable(_luckyId, _ticketNumber), 'Ticket cannot be confirmed yet');
        ...
        confirmBracket.confirmBracket(_luckyId, ticketId, _bracket, _isHaveFollower, _name);
     ...
     * @dev Callable by operator
     */
    function drawLuckyNumbersAndMakeTicketConfirmable(uint256 _luckyId) external onlyOperatorOrOwner nonReentrant {
        Lucky storage luckyInfo = _lucks[_luckyId];
        ...
        if (totalTicket > totalLuckyNumbers) {
            ...
            uint32[] memory luckyNumbers = new uint32[](totalTicket);
            for (uint32 i = 0; i < totalTicket; i++) {
    ...
    function enableConfirmationForReserve(uint256 _luckyId) external onlyOperatorOrOwner nonReentrant {
        require(_lucks[_luckyId].status == Status.Claimable, 'Lucky not claimable');
    ...
    {
        return (_lucks[_luckyId].countLuckyNumbersPerBracket, confirmBracket.getAvailablesBracket(_luckyId));
    }
        ...
        )
    {
        ...
        (bracket, isHaveFollower, name) = confirmBracket.getConfirmBracketInfo(ticketId);
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _size; i++) {
            ...
            (brackets[i], , ) = confirmBracket.getConfirmBracketInfo(ticketId);
        }
    ...
    function setConfirmBracket(address _confirmBracketAddress) external onlyOwner {
        require(_lucks[currentLuckyId].status != Status.Claimable, 'Lucky not in claimable');

        IConfirmBracket(_confirmBracketAddress);
        confirmBracket = IConfirmBracket(_confirmBracketAddress);

        emit NewConfirmBracket(_confirmBracketAddress);
    }
    ...
    function startLucky(
        ...
        confirmBracket.setAvailablesBracket(currentLuckyId, _countLuckyNumbersPerBracket);
    ...
    function updateLuckyStatus(uint256 _luckyId, Status _status) external onlyOperatorOrOwner nonReentrant {
    ...
    function updateBurnTransaction(uint256 _luckyId, string calldata _transactionHash)
        external
        onlyOperatorOrOwner
        nonReentrant
    {
    ...
    function _getTicketIdFromTicketNumber(uint256 _luckyId, uint32 _ticketNumber) internal view returns (uint256) {
        require(_lucks[_luckyId].status != Status.Pending, 'Invalid ticket number');
        require(_ticketNumber >= startNumber, 'Invalid ticket number');

        uint256 firstTicketIdInround = _lucks[_luckyId].firstTicketId;
        uint256 ticketId = ((_ticketNumber - startNumber) + firstTicketIdInround);

        require(
            ticketId >= _lucks[_luckyId].firstTicketId && ticketId <= _lucks[_luckyId].lastTicketId,
...
interface IConfirmBracket {
    ...
    function setAvailablesBracket(uint256 _luckyId, uint32[] memory _avilablesBracket) external;
}
