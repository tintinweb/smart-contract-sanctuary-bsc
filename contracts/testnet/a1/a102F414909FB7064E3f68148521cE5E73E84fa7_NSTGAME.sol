...
contract NSTGAME {
    ...
    function rewardAmount(address _account) public view returns (uint256) {
        ...
        if (block.timestamp - lastMiner[_account] >= epoch) {
            ...
            //算力*比例*天数
            reward = (power[_account] * scale) / 10000;
        }
    ...
    // 奖励
    function realRewardAmount(address _account) public view returns (uint256) {
        uint256 _days = rewardDays(_account);
        uint256 _reward;
        if (_days > 0) {
            _reward = rewardAmount(_account);
            if (_reward > 0) {
                uint256 maxReward = getMaxReward(_account);
                if (maxReward > 0) {
                    _reward = _reward * _days;
    ...
    function rewardDays(address account) public view returns (uint256) {
        uint256 _days = 0;
        if (block.timestamp - lastMiner[account] >= epoch) {
            _days = (block.timestamp - lastMiner[account]) / epoch;
            uint8 lv = getPowerLv(account);

            //v1及以上可以累计5天挖矿
            if (lv < 1) {
                _days = 1;
            } else if (lastMiner[account] == 0) {
                _days = 1; //第一次Mint只能1天
            } else if (_days > maxDays) {
                _days = maxDays; //单次最多领取天数
            }
        }
        return _days;
    }
    ...
    function doMint(address _account) private {
        ...
        uint256 reward = realRewardAmount(_account);
