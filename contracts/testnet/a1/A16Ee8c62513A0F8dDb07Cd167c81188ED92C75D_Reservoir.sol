...
contract Reservoir is Ownable {
    ...
    function updateAdminFee(uint8 newAdminFee) public onlyOwner {
        adminFee_ = newAdminFee;
    }
    ...
    function updateAdminWallet(address newAdmin) public onlyOwner {
    ...
    function calculateTaxedBnbToTokenLiquidity(uint256 _amount) public view returns (uint256) {
        if (_amount > 0){
            ...
            return amount.mul(SafeMath.sub(100,entryFee_ + adminFee_)).div(100);
        } else {
            return 0;
        }
    }
    ...
    function calculateTaxedLiquidityToBnb(uint256 _amount) public view returns (uint256){
        if (_amount > 0){
            _amount = _amount.mul(SafeMath.sub(100,entryFee_ + adminFee_)).div(100);
            (uint bnbAmount, uint tokenAmount) = swap.getLiquidityToReserveInputPrice(_amount);
            return bnbAmount.add(swap.getTokenToBnbInputPrice(tokenAmount));
        } else {
            return 0;
    ...
    function purchaseTokens(address _customerAddress, uint256 _incomingtokens) internal returns (uint256) {
        ...
        uint256 _adminShare = SafeMath.mul(_incomingtokens, adminFee_) / 100;             // 2% of drops
        uint256 totalFee = SafeMath.add(_undividedDividends, _adminShare);
        //TODO What is totalFee? LP?
        uint256 _amountOfTokens = SafeMath.sub(_incomingtokens, totalFee); // 88% of drops (100% - 12% above)
        ...
        //Stats
        stats[_customerAddress].taxes += totalFee;
