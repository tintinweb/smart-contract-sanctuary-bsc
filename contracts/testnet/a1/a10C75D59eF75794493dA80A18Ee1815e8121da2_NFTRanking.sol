...
contract NFTRanking is Ownable {
    ...
    function quickSort(UserInfo[] memory arr, uint left, uint right) private {
        ...
        UserInfo memory pivotUser = userInfo[pivot];
        while (i <= j) {
            UserInfo storage iUser = userInfo[i];
            UserInfo storage jUser = userInfo[j];

            while (iUser.power < pivotUser.power) i++;
            while (pivotUser.power < jUser.power) j--;
            if (i <= j) {
    ...
    function setRank() external {
        ...
        quickSort(userInfo, 0, userInfo.length);
    }
