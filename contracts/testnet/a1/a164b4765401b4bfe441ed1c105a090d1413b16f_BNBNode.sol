...
contract BNBNode {
    ...
    function compoundNodes() public {
        ...
        if (referrals[msg.sender].length > 0) {
            if (totalCompounds[msg.sender] >= max(uint256(15) / referrals[msg.sender].length, 1)) {
    ...
    function compoundDailyLimitTimeleft(address _address) public view returns(uint256) {
        if (totalCompounds[_address] < max(uint256(15) / (referrals[msg.sender].length == 0 ? 1 : referrals[msg.sender].length), 1) || (block.timestamp - lastBoughtOrCompounded[_address]) > 86400) {
            return 0;
        }
    ...
    function getTotalCompounds(address _address) public view returns(uint256) {
        return totalCompounds[_address];
    }
