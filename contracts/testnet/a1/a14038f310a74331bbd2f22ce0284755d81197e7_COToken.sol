...
contract COGraveyard is Context, Ownable {
...
contract COLiquidityDrive is Context, Ownable {
    using SafeMath for uint256;
    using Address for address;
...
contract COToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    string private _name = "CO-Test";
    string private _symbol = "$CO-Test";
    ...
    uint256 public _communityFee = 15;
    uint256 private _previousCommunityFee = _communityFee;

    address public walletPerformance = 0xE32e780C2b968Ba1Ee4f84bc9B9B1BA0baA16888;
    address public walletCharity = 0x367015167931bD8e3e28F2c3444DFDD1b801eE39;
    address public walletCommunity = 0x6d65cFe5bD36bF4146b3d149FE37463308c0BC76;

    COLiquidityDrive public liquidityDrive;
    uint256 public immutable _campaignPeriod = 5* 60;

    COGraveyard public graveyard;
    ...
    constructor () public {
        ...
        liquidityDrive = new COLiquidityDrive(25);

        //create the graveyard
        graveyard = new COGraveyard();
    ...
    function launch() public returns (bool) {
        ...
        uint256 _graveyardShare = _share.mul(48);
        uint256 _tokenLiquidity = _share.mul(25);
        uint256 _driveTokens = _share.mul(25);

        uint256 _performanceAmount = _share;
        uint256 _charityAmount = _share.div(2);
        uint256 _communityAmount = _share.div(2);


        //Transfers
        
        //dev/marketing
        _transfer(address(this), walletPerformance, _performanceAmount);
        _transfer(address(this), walletCharity, _charityAmount);
        _transfer(address(this), walletCommunity, _communityAmount);
        //graveyard
    ...
    function updatePerformanceWallet(address newAddress) public onlyOwner {
        walletPerformance = newAddress;
    }

    function updateCharityWallet(address newAddress) public onlyOwner {
        walletCharity = newAddress;
    }

    function updateCommunityWallet(address newAddress) public onlyOwner {
        walletCommunity = newAddress;
    }
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidityNCharityNCommunity) = _getValues(tAmount);
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
        _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);        
        _takeLiquidityNCharityNCommunity(tLiquidityNCharityNCommunity);
        _reflectFee(rFee, tFee);
        emit Transfer(sender, address(this), tLiquidityNCharityNCommunity);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tLiquidityNCharityNCommunity) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tLiquidityNCharityNCommunity, _getRate());
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tFee, tLiquidityNCharityNCommunity);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculateTaxFee(tAmount);
        uint256 tLiquidityNCharityNCommunity = calculateLiquidityNCharityNCharity(tAmount);
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidityNCharityNCommunity);
        return (tTransferAmount, tFee, tLiquidityNCharityNCommunity);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidityNCharityNCommunity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rLiquidityNCharityNCommunity = tLiquidityNCharityNCommunity.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidityNCharityNCommunity);
    ...
    function _takeLiquidityNCharityNCommunity(uint256 tLiquidityNCharityNCommunity) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidityNCharityNCommunity = tLiquidityNCharityNCommunity.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rLiquidityNCharityNCommunity);
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tLiquidityNCharityNCommunity);
    }
    ...
    function calculateLiquidityNCharityNCharity(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee.add(_charityFee).add(_communityFee)).div(
            10**3
        );
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 liquidityAmount = contractTokenBalance.mul(_liquidityFee).div(_liquidityFee.add(_charityFee).add(_communityFee));
        uint256 charityAmount = contractTokenBalance.mul(_charityFee).div(_liquidityFee.add(_charityFee).add(_communityFee));
        uint256 communityAmount = contractTokenBalance.sub(liquidityAmount).sub(charityAmount);

        uint256 half = liquidityAmount.div(2);
        uint256 otherHalf = liquidityAmount.sub(half);

        uint256 swapAmount = half.add(charityAmount).add(communityAmount);
        ...
        uint256 ethLiquidity = newBalance.mul(half).div(swapAmount);
        uint256 ethCharity = newBalance.mul(charityAmount).div(swapAmount);
        uint256 ethCommunity = newBalance.sub(ethLiquidity).sub(ethCharity);

        payable(walletCharity).transfer(ethCharity);
        payable(walletCommunity).transfer(ethCommunity);

        // add liquidity to uniswap
        addLiquidity(otherHalf, ethLiquidity);
        
        emit SwapAndLiquify(half, ethLiquidity, otherHalf);
    }
