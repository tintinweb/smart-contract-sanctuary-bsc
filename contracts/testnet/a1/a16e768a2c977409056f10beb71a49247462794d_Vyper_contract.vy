...
RATES: constant(uint256[N_COINS]) = [1000000000000000000, 1000000, 1000000000000000000, 1000000]
...
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256) -> uint256:
    ...
    # Mint pool tokens
    AnnexToken(_lp_token).mint(msg.sender, mint_amount)
...
def remove_liquidity(_amount: uint256, min_amounts: uint256[N_COINS]) -> uint256[N_COINS]:
    ...
    AnnexToken(_lp_token).burnFrom(msg.sender, _amount)  # dev: insufficient funds
...
def remove_liquidity_imbalance(amounts: uint256[N_COINS], max_burn_amount: uint256) -> uint256:
    ...
    AnnexToken(_lp_token).burnFrom(msg.sender, token_amount)  # dev: insufficient funds
    for i in range(N_COINS):
        if amounts[i] != 0:
            _response: Bytes[32] = raw_call(
                self.coins[i],
...
def remove_liquidity_one_coin(_token_amount: uint256, i: int128, _min_amount: uint256) -> uint256:
    ...
    AnnexToken(self.lp_token).burnFrom(msg.sender, _token_amount)  # dev: insufficient funds
