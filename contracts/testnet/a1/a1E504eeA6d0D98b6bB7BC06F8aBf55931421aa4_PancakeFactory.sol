...
abstract contract PancakeERC20 is IPancakePair {
    // using SafeMath for uint; REMOVED
    ...
    // bytes32 public constant PERMIT_TYPEHASH = 0x6e71edae12b1b97f4d1f60370fef10105fa2faae0126114a169c64845d6126c9; REMOVED
    bytes32 public constant PERMIT_TYPEHASH = keccak256("Permit(address owner,address spender,uint256 value,uint256 nonce,uint256 deadline)");

    mapping(address => uint) public nonces;

    // event Approval(address indexed owner, address indexed spender, uint value); REMOVED
    // event Transfer(address indexed from, address indexed to, uint value); REMOVED

    constructor() {
        uint256 chainId = block.chainid;
        // assembly ("memory-safe") { REMOVED
        //     chainId := chainid
        // }
    ...
    function transferFrom(address from, address to, uint value) external returns (bool) {
        require(allowance[from][msg.sender] >= value, 'Pancake: ALLOWANCE TOO LOW');
        // if (allowance[from][msg.sender] != uint256(-1)) { REMOVED
...
contract PancakePair is IPancakePair, PancakeERC20 {
    ...
    // event Mint(address indexed sender, uint amount0, uint amount1); REMOVED
    // event Burn(address indexed sender, uint amount0, uint amount1, address indexed to);
    // event Swap(
    //     address indexed sender,
    //     uint amount0In,
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        // require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'Pancake: OVERFLOW'); REMOVED
        // require(balance0 <= 2**112-1 && balance1 <= 2**112-1, 'Pancake: OVERFLOW'); Added but replaced with below
    ...
    function swap(uint amount0Out, uint amount1Out, uint amount0Fee, uint amount1Fee, address to, bytes calldata data) external lock {
        ...
        if(baseToken != address(0)) {
            require(amount0Fee > 0 || amount1Fee > 0, "Pancake: INSUFFICIENT_FEE");
        }
    ...
    function setBaseToken(address _baseToken) external {
        require(msg.sender == factory, "Pancake: NOT_FACTORY");
        require(_baseToken == token0 || _baseToken == token1, "Pancake: WRONG_ADDRESS");
    ...
    function updateTotalFee(uint _totalFee) external returns (bool) {
        ...
        require(feeTaker == msg.sender, "Pancake: NOT_ALLOWED");
...
contract PancakeFactory is IPancakeFactory {
    ...
    // event PairCreated(address indexed token0, address indexed token1, address pair, uint); REMOVED
    ...
    function createPair(address tokenA, address tokenB, bool supportsTokenFee) external returns (address pair) {
        ...
        allPairs.push(pair);
        if(supportsTokenFee) {
            IPancakePair(pair).setBaseToken(tokenB);
