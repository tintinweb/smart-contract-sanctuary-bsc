...
contract DinoV2 is Context, Auth, IERC20 {
    ...
    function _beforeTransferToken(address sender, address recipient, uint256 amount) internal view {
        if (isPair[sender]) {
            if (isLastTimeBuyEnable && !isExcludeFromTimeBuyLimit[recipient] && lastTimeBuy[recipient] > 0) {
                require(block.timestamp.sub(lastTimeBuy[recipient]) >= minimumTimeBuy, "DinoV2: Minimum Time Buy is Exceed");
            }
        } else {
            if (isLastTimeSellEnable) {
                require(block.timestamp.sub(lastTimeSell[sender]) >= minimumTimeSell, "DinoV2: Minimum Time Sell is Exceed");
            }
        }
    }
    ...
    function shouldSetAutoStaking(address sender) internal view returns (bool){
    ...
    function setTaxReceiver(address _marketingAddress, address _operationalAddress, address _treasuryAddress) external onlyOwner {
    ...
    function setPercentBuy(uint256 _percentStaking, uint256 _percentReferral, uint256 _percentOperational, uint256 _percentTreasury) external onlyOwner {
        require(_percentStaking + _percentReferral + _percentOperational + _percentTreasury <= 2500, "DinoV2: Maximum 25%");
    ...
    function setPercentSell(uint256 _percentStaking, uint256 _percentMarketing, uint256 _percentOperational, uint256 _percentTreasury) external onlyOwner {
        require(_percentStaking + _percentMarketing + _percentOperational + _percentTreasury <= 2500, "DinoV2: Maximum 25%");
    ...
    function _swapForWeth(uint256 amount) internal swapping {
        if (amount > 0) {
            ...
            if (isStakingEnable) {
                IDinostaking(routerStakingAddress).deposit{value : amountStaking}();
            } else {
                payable(operationalAddress).transfer(amountStaking);
            }
        }
    }

    function swapWethToOther(address _tokenAddress, uint256 amount, address to) internal swapping {
        IUniswapV2Router02 router = IUniswapV2Router02(routerAddress);
        address[] memory path = new address[](2);
        path[0] = wethAddress;
