...
contract OrbitPlay is Ownable, ReentrancyGuard {
    ...
    function diceRandom() private returns (uint) {       	        
        uint rnd = 0;
        randDiceNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randDiceNonce)))%256;        
        rnd = rnd / 43;
        return rnd + 1;
    }
    ...
    function coinFlipRandom() private returns (uint) {       	        
        uint rnd = 0;
        randCoinFlipNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randCoinFlipNonce)))%256;
        rnd = rnd / 128;
        return rnd + 1;
    }
    ...
    function spinRandom() private returns (uint) {       	        
        uint rnd = 0;
        randSpinNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randSpinNonce)))%256;
        rnd = rnd / 64;
        return rnd + 1;
    }
    ...
    function roshamboRandom() private returns (uint) {       	        
        uint rnd = 0;
        randRoshamboNonce ++;
        rnd = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, randRoshamboNonce)))%256;
        rnd = rnd / 86;
        return rnd + 1;
    }
