...
contract AkitaAirdrop is AccessControl, ReentrancyGuard {
    ...
    // admin function
    function setPrice(uint256 _price) public onlyRole(AIRDROP_ROLE) {
        price = _price;
    }

    // function addWhitelist(address _account) public onlyRole(AIRDROP_ROLE) {
    //     whitelist.add(_account);
    // }

    // function removeWhitelist(address _account) public onlyRole(AIRDROP_ROLE) {
    //     whitelist.remove(_account);
    // }

    function grantAirdropRole(address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    ) public onlyRole(AIRDROP_ROLE) returns (bool) {
    ...
    function bulkDropTokens(
        ...
        _checkRole(AIRDROP_ROLE, messageSigner);
        for (uint256 i = 0; i < _recipients.length; i++) {
            require(_recipients[i] != address(0));
            // require(whitelist.contains(_recipients[i]), "Not whitelist");
        }
