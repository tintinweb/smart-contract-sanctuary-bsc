...
interface IStakeNFT {
    function initialize(address factory_, address nft_address) external;

    function stake(uint256 tokenId_, uint256 maturity_duration) external;
...
import "./interfaces/IStakeNFTFactory.sol";
import "./libs/Adminable.sol";
import "./libs/DuetTransparentUpgradeableProxy.sol";

contract StakeNFTFactory is IStakeNFTFactory, Initializable, Adminable {
    mapping(address => address) public nftToStakeMapping;
    mapping(address => address) public stakeToNftMapping;
    ...
    event StakeNFTRepositoryCreated(address nftAddress, address stakeRpositoryAddress);
    ...
    function createStakeRepository(address nftAddress_) external onlyAdmin returns (address stakeNFTAddress) {
        address proxyAdmin = address(this);
        require(stakeImplementation != address(0), "StakeNFTFactory: Invalid StakeNFT implementation");
        require(
            nftToStakeMapping[nftAddress_] == address(0),
            "StakeNFTFactory:This  NFT Stake Repository has already been set"
        );
        ...
        stakeNFTAddress = address(proxy);

        IStakeNFT(stakeNFTAddress).initialize(address(this), nftAddress_);

        stakeRepositoryAddresses.push(stakeNFTAddress);
        nftToStakeMapping[nftAddress_] = stakeNFTAddress;
        stakeToNftMapping[stakeNFTAddress] = nftAddress_;

        emit StakeNFTRepositoryCreated(nftAddress_, stakeNFTAddress);
    }
}