...
contract LIFEGAMES is IERC20 {
    ...
    uint256 public swapAmount = 50000000000000000000;
    bool inSwap;
  ...
  function isExcludedFromFee(address account) public view returns(bool) {
    ...
    function _finalizeTransfer(address from, address to, uint256 amount, bool takeFee) internal returns (bool) {
        ...
        */
        ...
        _tokenTransfer(from, to, amount, takeFee && takeFeeEnabled);
        return true;
    }
    ...
    function contractSwap(uint256 numTokensToSwap) internal swapping {
        ...
        if (transferToPoolsOnSwaps) {
            ...
            if ( estimatedBusdForLiquidity > 0 && amountToLiquidity > 0 ) {
                IERC20(address(busdAddress)).transferFrom(address(this), busdReserveAddress, estimatedBusdForLiquidity);
            }

            if (estimatedBusdForBuybackAndBurn > 0 && amountToBuyBackAndBurn > 0) {
                IERC20(address(this)).transferFrom(address(this), buybackAndBurnAddress, estimatedBusdForBuybackAndBurn);
            }
        }
        ...
        if (autoAddLiquidity) {
            ...
            // amounts
            uint256 estimatedBusdForAutoLiquidity = getOutEstimatedTokensForTokens(busdAddress, address(this), 10000000000000000000);
            uint256 estimatedTokensForAutoLiquidity = getOutEstimatedTokensForTokens(address(this), busdAddress, estimatedBusdForLiquidity);
            ...
            if (estimatedTokensForAutoLiquidity > autoAddLiquidityThreshold) {
                ...
                IERC20(address(this)).transferFrom(buybackAndBurnAddress, address(this), estimatedTokensForAutoLiquidity);
