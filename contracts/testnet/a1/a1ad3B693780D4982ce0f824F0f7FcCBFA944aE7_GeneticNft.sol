  ...
  GeneticNftStorage
{
    ...
    initializer
  {
    ...
    data.setRole("managerRole", keccak256("MANAGER"));
    data.setRole("minterRole", keccak256("MINTER"));
  ...
  function setBaseURI(string memory baseURI_)
    external
    onlyRole(data.getRole("managerRole"))
  {
  ...
  function mint(address owner_)
    external
    override
    onlyRole(data.getRole("minterRole"))
    returns (uint256)
  {
    require(owner_ != address(this), "GeneticNFT_E001");

    tokenCount.increment();
    _safeMint(owner_, tokenCount.current());
...
interface IGeneticNft {
  ...
  function mint(address owner_) external returns (uint256);
