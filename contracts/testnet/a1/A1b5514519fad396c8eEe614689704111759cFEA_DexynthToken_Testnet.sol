...
// File: contracts\DexynthToken.sol

pragma solidity 0.6.6;

contract DexynthToken_Testnet is
ERC20Capped,
IChildToken,
AccessControlMixin,
...
ContextMixin
{
    ...
    // Control
    bool public callbackIsSet = false;
    bool public dBUSDVaultIsSet = false;
    ...
    constructor(
        ...
        address NFTRewards,
        address referrals
    ) public ERC20Capped(100*(10**6)*(10**18)) ERC20("Dexynth", "DEXY") {

        // Token init
        _setupContractId("ChildMintableERC20");
        _setupDecimals(18);
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _initializeEIP712("Dexynth");
    ...
    function setCallbackForTheFirstTime(address _callback) external only(DEFAULT_ADMIN_ROLE){
        if (!callbackIsSet){
            ...
            callbackIsSet = !callbackIsSet;
        }
    }

    // Used for the setup (by-passes the timelock the first time)
    function setdBUSDForTheFirstTime(address _dBUSD) external only(DEFAULT_ADMIN_ROLE){
        if (!dBUSDVaultIsSet){
            _setupRole(MINTER_ROLE, _dBUSD);
            _setupRole(BURNER_ROLE, _dBUSD);
            dBUSDVaultIsSet = !dBUSDVaultIsSet;
        }
    }
