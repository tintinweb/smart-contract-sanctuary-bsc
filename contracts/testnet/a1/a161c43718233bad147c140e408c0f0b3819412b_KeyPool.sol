...
contract KeyPool is Ownable {
    ...
    //购买并销毁
    function purchase(address _staker, uint256 _keyNum) external onlyOwner returns (bool) {
        uint256 bal = this.balanceOf(_staker);
        require(bal >= _keyNum,"K2028: not enough key");
        stake[ownerStake[_staker]].balance -= _keyNum;
    ...
    function getPairReserves() public view returns(uint112 _reserve0, uint112 _reserve1, uint32 _blockTimestampLast) {
        require(aPancakePair != address(0),"K2024: aPancakePair error");
        (_reserve0,_reserve1,_blockTimestampLast ) = PancakePair(aPancakePair).getReserves();
    }

    function showToken0() public view returns(address) {
        return PancakePair(aPancakePair).token0();
    }

    function showToken1() public view returns(address) {
        return PancakePair(aPancakePair).token1();
    }

    function getLPBusd(uint256 _LP) public view returns (uint256 _busd){
    ...
    //收益计算
    //质押收益计算方式：（两次质押操作时间间隔 / 1天） * （本次操作前的质押量 / 质押池总量） *  （上一次空投量）
    function calcIncome(uint256 _stakePrice, uint256 _delay) public view returns(uint256) {
    ...
    function addStake(uint256 _stakeNum) external lock returns(uint256) {
    ...
    function subStake(uint256 _stakeNum) external lock returns(uint256) {
        address _staker = _msgSender();
        ...
        //将在本池质押的LP退还给用户
        PancakePair(aPancakePair).transferFrom(address(this), _staker, _p);
        PancakePair(aPancakePair).transferFrom(address(this), mainPool, _f);
        settlement(_staker);
