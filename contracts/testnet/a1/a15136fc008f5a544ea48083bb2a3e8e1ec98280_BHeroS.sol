...
contract BHeroS is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function setMaxBurn(uint8 _maxBurn) external onlyRole(DESIGNER_ROLE) {
        maxBurn = _maxBurn;
    }
    ...
    function getPercentHeroS(uint256[] memory _heroTypes) public pure returns (uint256[] memory) {
        ...
        dropRate[0] = common * 20;
        dropRate[1] = rare * 20;
        dropRate[2] = superRare * 20;
        dropRate[3] = epic * 20;
        dropRate[4] = legend * 20;
        dropRate[5] = superLegen * 20;
  ...
  function processTokenRequests() external {
    ...
    for (uint256 i = requests.length; i > 0; --i) {
      ...
      for (uint256 j = 0; j < dropLength; ++j) {
          dropRateOption[j] = (request.details >> (45 + i * 5)) & 31;
      }
