...
interface vestingContractInterface {
    ...
    function getClaimableAmount(address _walletAddress,uint256 _vestingId) external returns(uint256);

    function userClaimData(address _walletAddress,uint256 _vestingId) external returns(bool ,address ,uint8 ,uint256 ,uint256 ,uint256 ,uint256 ,uint256 ,uint256 ,uint256 );

    function getVestingIds(address _walletAddress) external view returns(uint256[] memory);

    function timeUnit() external returns(uint256);

    function launchRound(uint8 _roundId, uint256 _vestingStartTime) external;

    function getIslaunched(uint8 _roundId) external returns(bool) ;
    ...
    function roundData(uint8 _roundId) external returns(bool,uint256,uint256,uint256,uint256,uint256,uint256);  
...
// this function has to be internal
    function getTicketRate(uint8 _roundId) public returns(uint256) {
        (,,uint256 tokenprice,,,,) = getRoundData(_roundId);
        return tokenprice;
    }

    function getTokensPerEth(uint256 amount,uint8 _roundId) public returns (uint256) {
    ...
    function getTimeUnit() public returns(uint _timeUnit){
    ...
    function getClaimAmount(address _walletAddress,uint256 _vestingId) public  returns(uint _claimAmount) {
        return vestingContractInterface(vestingAddress).getClaimableAmount(_walletAddress,_vestingId);
    }

    function getUserVestingData(address _address,uint256 _vestingId) public returns(
    ...
    function getTotalTokensForSale(uint8 _roundId) public returns(uint256 _totalTokensForSale ){
        (,_totalTokensForSale,,,,,) = getRoundData(_roundId);
    }

    function getRoundData(uint8 _roundId) public returns(bool,uint256,uint256,uint256,uint256,uint256,uint256) {
        return( vestingContractInterface(vestingAddress).roundData(_roundId)) ;
    }
