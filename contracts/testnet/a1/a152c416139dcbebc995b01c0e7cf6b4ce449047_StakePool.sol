...
contract StakePool is Ownable,ReentrancyGuard {
    ...
    function deposit(uint16 _lockDays, uint256 _amount) public { 
        ...
        require(aprCfg[_lockDays] != 0,"The lockDays is not legal.");
    ...
    function withdraw(uint256 _sid) public {
        ...
        uint256 myTotal = addressTotalAmount[msg.sender];  
        addressTotalAmount[msg.sender] = myTotal.sub(_amount);
    ...
    function stakeItem(address _user, uint256 _sid) external view returns (uint256 amount, uint256 unlockTime, uint128 lockDays, uint256 pendingReward){ 
        StakeInfo[] storage list = addressDepositList[_user];
        require(_sid<list.length,"_sid out of range.");
        StakeInfo storage _info = list[_sid]; 
        uint256 _time = now;
        uint _pendingReward = _calcReward(_info, _time);
        return (_info.amount,_info.unlockTime,_info.lockDays,_pendingReward);
    }
