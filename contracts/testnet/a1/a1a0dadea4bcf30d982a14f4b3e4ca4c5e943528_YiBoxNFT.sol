...
contract IERC721Metadata is IERC721 {
    ...
    function tokenBase(uint256 tokenId) external view returns (string memory _uri, uint16 _level, uint8 _quality, uint32 _hashrate, uint8 _status, uint256 _price, uint16 _type);
}
...
contract ERC721Metadata is Context, ERC165, ERC721, IERC721Metadata {
    ...
    function tokenBase(uint256 tokenId) external view returns (string memory _uri, uint16 _level, uint8 _quality,uint32 _hashrate, uint8 _status, uint256 _price, uint16 _type) {
        ...
        _price = _tokenBase.price;
        _type = _tokenBase.ttype;
    }

    function _setTokenBase(uint256 tokenId, string memory _uri, uint16 _level, uint8 _quality, uint32 _hashrate, uint8 _status,uint16 ttype) internal {
    ...
    function _burn(address owner, uint256 tokenId) internal {
        ...
        if (bytes(_tokenBases[tokenId].strURI).length != 0) {
...
contract YiBoxNFT is ERC721Full, Governance {
    ...
    constructor() public ERC721Full("YiBoxHero", "YBH") {
        _setBaseURI("http://47.99.149.223:8080/TgTel/id/?id=");
    }
    ...
    function openBox(uint256 tokenId, uint8 _quality, uint32 _hashrate, uint16 _type) external {
        ...
        _setTokenBase(tokenId, MathwalletUtil.uintToString(tokenId),1,_quality,_hashrate,4, _type);
