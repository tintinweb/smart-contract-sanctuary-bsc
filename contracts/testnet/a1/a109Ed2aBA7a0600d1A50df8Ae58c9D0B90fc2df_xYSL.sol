...
contract xYSL is ERC20, AccessControl, Pausable {
   ...
   /**
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        require(restrictTransfer[sender] != block.number,"xYSL: you can't interact in same block");
        address[] memory path = new address[](2);
                path[0] = address(this);
                path[1] = Admin.BUSD();
        if(isContract(sender) || isContract(recipient)){
            require(IWhitelist(Admin.whitelist()).getAddresses(sender) || IWhitelist(Admin.whitelist()).getAddresses(recipient),
        ...
        if(sender == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.BUSD()) || recipient == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.BUSD())){
            ...
            if(IWhitelist(Admin.whitelist()).getAddresses(user)){
                ...
                if(IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(balanceOf(address(this)),path)[1] >= xYSL_ThresholdLimit){
                    IERC20(address(this)).approve(Admin.ApeswapRouter(), taxAmount); //BSC Testnet pancake router address
                    uint convertedBUSD = IUniswapV2Router02(Admin.ApeswapRouter()).swapExactTokensForTokens( 
                        taxAmount,
                        0,
                        path,
                        address(this),
                    ...
                    IERC20(Admin.BUSD()).transfer(Admin.temporaryHolding(), (convertedBUSD * xYSL_Tax_Allocation[0])/xYSL_Tax);
                    IERC20(Admin.BUSD()).transfer(Admin.Treasury(), ((convertedBUSD * xYSL_Tax_Allocation[1])/xYSL_Tax));
                }
                amount -= taxAmount;
            ...
            if( timeInDays > 2 ){   
                ...
                console.log("exitRateAmount",exitRateAmount);
...
contract WhiteList is AccessControl, Initializable{
    ...
    Note only admin can call
     */
    function addWhiteList(address[] calldata _protocol) external onlyRole(ADMIN_ROLE){
    ...
    Note only admin can call
     */
    function revokeWhiteList(address[] calldata _protocol) external onlyRole(ADMIN_ROLE){
        require(hasRole(ADMIN_ROLE,msg.sender),"WhiteList: don't have access");
        for(uint i = 0; i < _protocol.length; i++){
            require(addresses[_protocol[i]],"WhitList: not whitelisted");
