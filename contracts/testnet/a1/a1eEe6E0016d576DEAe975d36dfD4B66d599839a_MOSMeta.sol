...
contract MOSMeta is Ownable{
    ...
    function updateWhitelist(address _account, bool _value) public onlyOwner returns(bool) 
    {
    ...
    function getHash(
        ...
        address to,     // Reward Address
        uint value,     //Send ETH value
        bytes memory data   // Signer Data
    ) public view returns(bytes32){
        return keccak256(abi.encodePacked(address(this), signer, to, value, data, nonce[signer]));
    }

    // original forward function copied from https://github.com/uport-project/uport-identity/blob/develop/contracts/Proxy.sol
    function forward(
        bytes memory sig,      //admin tx
        address signer,     // Signer address admin
        address to,   // Call to the game reward address
      ...
      emit Forwarded(sig, signer, to, value, data, _hash);
    }
    ...
    event Forwarded (bytes sig, address signer, address to, uint value, bytes data,bytes32 _hash);
