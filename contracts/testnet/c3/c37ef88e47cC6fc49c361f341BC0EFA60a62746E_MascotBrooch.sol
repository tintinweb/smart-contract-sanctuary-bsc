...
contract MascotBrooch is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public ALL_AMOUNT = 42;

    uint256 public PRICE = 0.0069 ether;

    uint256 public LIMIT = 20;

    uint256 public START_TIME = 1673317800;
    ...
    function freeInfo(address user) public view returns (uint256,uint256,uint256,uint256,uint256,uint256) {
        return  (ALL_AMOUNT,totalSupply(),PRICE,START_TIME,0,WALLET_CAP[user]);
    }
    ...
    function mint(uint256 quantity) public payable
    {
        ...
        require(msg.value >= quantity * PRICE,"Did not send enough eth.");
        require(_isActive, "must be active to mint tokens");
        require(WALLET_CAP[msg.sender] + quantity <= LIMIT, "Purchase would exceed max number of metacards per wallet."); 
