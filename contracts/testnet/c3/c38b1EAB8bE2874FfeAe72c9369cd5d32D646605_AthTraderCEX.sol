...
contract AthTraderCEX {
    ...
    function recordPayout(
        ...
        for (uint256 i = 0; i < addresses_.length; i++) {
            ...
            if (referrer != address(0)) {
                // Get first-level referrer fee
                uint8 _referrerFee = uint8(
                    calculateReferralCommission(referrer)
                );
                ...
                uint256 _firstReferrerPayout = _currAthenaPayout
                    .mul(_referrerFee)
                    .div(100);
                ...
                if (secondReferrer != address(0)) {
                    // Get second-level referrer fee
                    uint8 _secondReferrerFee = uint8(
                        calculateReferralCommission(secondReferrer)
                    );
                    ...
                    uint256 _secondReferrerPayout = _currAthenaPayout
                        .mul(_secondReferrerFee)
                        .div(100);
     ...
     */
    function calculateReferralCommission(address referrer_)
        public
        view
        returns (uint256)
    {
