...
contract EvolveFactory is Context, Ownable, EvolveAccessControll, IEvolveFactory {
    ...
    function addNewCompetion(uint256 _presetId, address[] calldata _teamA, address[] calldata _teamB , bool[] calldata _boolNeedReplace) external override ownerOrAdmin returns(uint competionId){
        ...
        (uint matchPrice,uint playerNumber,uint presetDate) = evolveStorage.getPreset(_presetId);

        require(_teamA.length == playerNumber, "team member need to be equal with preset team number!");
        address[] memory allPlayerAddressList = concatenateArrays(_teamA, _teamB);
        
        uint competionPriceByToken = calculateMatchPrice(matchPrice, tokenPriceByUSD, 18);
        evolveBulkSender.bulkReceive(competionPriceByToken, allPlayerAddressList, address(tokenAddress));
        competionId = evolveStorage.addNewCompetion(_presetId, _teamA, _teamB, tokenPriceByUSD);
    }
    ...
    function setCompetionWinner(uint256 _competionId, uint8 _winnerTeam) external override  ownerOrAdmin returns(bool){
       ...
       require(_winnerTeamAddress.length == playerCount || _winnerTeamAddress.length == playerCount.div(2));
...
interface IEvolveStorage{
    ...
    function addNewPreset(uint256 _matchPrice, uint256 _numberOfTeamMemebr ) external returns(uint);
    ...
    function addNewCompetion(uint256 _presetId, address[] calldata _teamA, address[] calldata _teamB, uint256 _priceRate) external returns(uint competionId);

    function getPreset(uint256 _presetId) external view returns(uint256,uint256,uint256);
...
interface IEvolveFactory{
    ...
    function addNewCompetion(uint256 _presetId, address[] calldata _teamA, address[] calldata _teamB , bool[] calldata _boolNeedReplace) external returns(uint competionId);
