...
contract TradeLens {
    function getLeverageOrders(address _orderbook, address _owner, bool _isMarket, uint256 _skip, uint256 _take)
        external
        view
        returns (LeverageOrder[] memory, uint256)
    {
        ...
        LeverageOrder[] memory pendingOrders = new LeverageOrder[](totalTaken);
        for (uint256 i = 0; i < totalTaken; i++) {
            ...
            LeverageOrder memory leverageOrder;
            leverageOrder.id = id;
    ...
    function getSwapOrders(address _orderbook, address _owner, uint256 _skip, uint256 _take)
        external
        view
        returns (IOrderManagerForTradeLens.SwapOrder[] memory, uint256)
    {
        IOrderManagerForTradeLens orderManager = IOrderManagerForTradeLens(_orderbook);
        ...
        IOrderManagerForTradeLens.SwapOrder[] memory pendingOrders = new IOrderManagerForTradeLens.SwapOrder[](totalTaken);
        for (uint256 i = 0; i < totalTaken; i++) {
            uint256 id = orderIds[i];
