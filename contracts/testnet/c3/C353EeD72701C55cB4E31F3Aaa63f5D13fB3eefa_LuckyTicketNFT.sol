    ...
    VRFConsumerBase
{
    ...
    event TicketNFTWin(
        address indexed owner,
        uint256 indexed ticketNFTIndex,
        uint256 ticketNFTId
    );
    event NewTicketBonus(uint256 indexed nftId);
    ...
    mapping(uint256 => address) ownerOfTicket;
    // player address => tokenId
    ...
    function setNewCrownBonus() public onlyValidTarget {
        ...
        uint256 _newTokenId = _CrownToken.tokenByIndex(
            _CrownToken.balanceOf(address(this)) - 1
        );
        emit NewTicketBonus(_newTokenId);
    ...
    function resetAllTicket() external onlyValidTarget {
        for (uint24 i = 0; i < ticketSoldPerDay; i++) {
            delete ownerOfTicket[i];
        }
        allTickets = new uint256[](0);
        ticketSoldPerDay = 0;
        setNewCrownBonus();
    }
    ...
    function buyTicket(uint256 _quantity) external nonReentrant {
        ...
        require(
            ownerTicketCount[msg.sender] + _quantity <= 250, // max allowance
        ...
        IERC20(_token).transferFrom(msg.sender, address(this), _totalFee);
        // create & store ticket
        for (uint256 i = 0; i < _quantity; i++) {
            allTickets.push(block.timestamp);
            // store address owner of ticket
            ownerOfTicket[ticketSoldPerDay] = msg.sender;
            ticketSoldPerDay++;
            // give player 1 scepter each ticket
    ...
    function randomWinner() external onlyValidTarget isPeriod {
        lastTimeCheck = block.timestamp;
        if (ticketSoldPerDay > 0) {
            uint256 _newIndexWinningTicket = _randomResult % ticketSoldPerDay;
            // give player an NFT Crown
            ownerWinningCrownNFT[ownerOfTicket[_newIndexWinningTicket]].push(
                todayTokenId
            );
            emit TicketNFTWin(
                ownerOfTicket[_newIndexWinningTicket],
                todayTokenId,
                _newIndexWinningTicket
            );
        } else {
    ...
    function claimNFT(uint8 _NFTType) external nonReentrant {
        //0: claim Crown NFT, 1: claim Scepter NFT
        if (_NFTType == 0) {
            require(
