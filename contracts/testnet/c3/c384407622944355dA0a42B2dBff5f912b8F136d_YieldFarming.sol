...
/**
 * @title Contract for staking LP tokens
 */
contract YieldFarming is Ownable {
	...
	// user's single stake for a pair
	struct Stake {
		address pairAddress;
		uint256 tokenAmount;
	...
	// address of the reward token which should be paid for staking LP tokens
	address public rewardTokenAddress;

	// all thresholds of a single pair
	mapping (address => RewardThreshold[]) public pairRewardThresholds;

	// all user stakes
	mapping (address => Stake[]) public userStakes;

	/**
	 * @notice Checks that user's stake at that index exists
	 * @param _stakeIndex stake index
	 */
	modifier stakeExists(uint256 _stakeIndex) {
		require(_stakeIndex < userStakes[msg.sender].length, 'STAKE_DOES_NOT_EXIST');
		_;
	}
	...
	/**
	 * @notice Returns reward amount of a single stake
	 * @param _stakeIndex stake index
	...
	function getRewardAmount(
		...
		public 
		view
		stakeExists(_stakeIndex) 
		returns (uint256) 
	{
		// get stake and reward thresholds
		Stake memory userStake = userStakes[msg.sender][_stakeIndex];
		RewardThreshold[] memory rewardThresholds = pairRewardThresholds[userStake.pairAddress];
		// get threshold index applied to the stake
		uint thresholdIndex = getRewardThresholdIndex(_stakeIndex);
		// calculate reward amount
		uint256 daysPassed = (block.timestamp - userStake.lastHarvestedAt) / 1 days;
		uint256 rewardAmount = userStake.tokenAmount / 1_000_000 * rewardThresholds[thresholdIndex].roiDaily * daysPassed;
		// return result
		return rewardAmount;
	}

	/**
	 * @notice Returns threshold index applied for a particular stake
	 * @param _stakeIndex stake index
		...
		returns (uint256) 
	{
		...
		Stake memory userStake = userStakes[msg.sender][_stakeIndex];
		RewardThreshold[] memory rewardThresholds = pairRewardThresholds[userStake.pairAddress];
		// get reward threshold applied to the stake
		uint256 totalLiquidity = ERC20(userStake.pairAddress).totalSupply();
	...
	{
		// get reward amount
		uint256 rewardAmount = getRewardAmount(_stakeIndex);
		// update last harvested at in stake
		userStakes[msg.sender][_stakeIndex].lastHarvestedAt = block.timestamp;
		// mint reward tokens
		EvxToken(rewardTokenAddress).distributeByYieldFarming(msg.sender, rewardAmount);
	}

	/**
	 * @notice Harvests all rewards
	 */
	function harvestAll() public {
		// for all user stakes make a harvest
	...
	/**
	 * @notice Stakes LP tokens to the contract
	...
	function stake(
		...
		// validation
		require(ERC20(_pairAddress).balanceOf(msg.sender) >= _tokenAmount, 'NOT_ENOUGH_LP_TOKENS');
		require(pairRewardThresholds[_pairAddress].length > 0, 'EMPTY_REWARD_THRESHOLDS');
		// save user's stake
		...
		ERC20(_pairAddress).transferFrom(msg.sender, address(this), _tokenAmount);
	}

	/**
	 * @notice Withdraws LP tokens from a single stake
	 * @param _stakeIndex stake index
	 */
	function withdraw(
		uint256 _stakeIndex
		...
		stakeExists(_stakeIndex)
	{
		...
		// validation
		require(!userStake.isWithdrawn, 'STAKE_ALREADY_WITHDRAWN');
		// mark stake as withdrawn
		userStake.isWithdrawn = true;
		// harvest stake
		harvest(_stakeIndex);
		// transfer LP tokens back to the user
		ERC20(userStake.pairAddress).transfer(msg.sender, userStake.tokenAmount);
	}
	...
	function withdrawAll() public {
		// for all user stakes make a withdraw
		for (uint256 i = 0; i < userStakes[msg.sender].length; i++) {
			withdraw(i);
		}
	}
	...
	/**
	 * @notice Creates reward thresholds for a pair. Thresholds should be sorted by token amount.
	 * @param _pairAddress pair address
	 * @param _rewardThresholds array of reward thresholds
	 */
	function createRewardThresholds(
		address _pairAddress,
		...
		delete pairRewardThresholds[_pairAddress];
		// save new reward thresholds
		for (uint256 i = 0; i < _rewardThresholds.length; i++) {
			pairRewardThresholds[_pairAddress].push(_rewardThresholds[i]);
		}
	}

	/**
	 * @notice Initializes a contract.
	 * We can not set variables in constructor because reward tokens depends on yield farming contract.
	 * So the flow is the following:
	 * 1. Deploy yield farming contract
	 * 2. Deploy reward token contract with yield farming address
	 * 3. Init yield farming contract with reward token address
	 * @param _rewardTokenAddress reward token address
	 */
	function init(address _rewardTokenAddress) public onlyOwner {
