...
contract OpenGateNFTMasterV2 is Ownable, IERC721Receiver {
    ...
    function createOrder(uint256 collectionId_, address payable owner_, uint256 times_) private {
        ...
        for (uint256 i = 0; i < times_; i++) {
            OrderInfo memory oi;
            oi.id = orderInfoMap[collectionId_].length;
    ...
    function claimUserNFT(uint256 collectionId_, address user, uint256 times_) public {
        require(owner() == address(user) || user == _msgSender(), 'forbidden!');
        ...
        for (uint256 i = 0; i < times_; i++) {
            claimNFT(collectionId_, _msgSender());
    ...
    function claimNFT(uint256 collectionId_, address user) private {
        ...
        claimRevenue(collectionId_, nftId);
    }

    // nft pending in collection
    function nftsPending(uint256 collectionId_) public returns (uint256[] memory){
        uint256[] memory nfts = nftsByCollectionId[collectionId_];
        uint256[] memory nftsPending;
        uint256 j = 0;
        for (uint256 i = 0; i < nfts.length; i++) {
            if (allNFTs[nfts[i]].claimAt == 0) {
                nftsPending[j] = nfts[i];
                j++;
            }
        }
        return nftsPending;
    ...
    function randomNft(uint256 collectionId_) public returns (NFT memory){
        uint256[] memory nftsPending_ = nftsPending(collectionId_);
        uint256 r = psuedoRandomness() % nftsPending_.length;
        return allNFTs[nftsPending_[r]];
    }


    // 获取收益（NFT原所有者（创作者））
    function claimRevenue(uint256 collectionId_, uint256 nftId) public {
        Collection storage collection = allCollections[collectionId_];
