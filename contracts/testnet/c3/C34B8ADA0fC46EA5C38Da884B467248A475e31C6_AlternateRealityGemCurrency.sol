...
contract AlternateRealityGemCurrency is ERC20, ERC20Burnable, ERC20Snapshot, Ownable, Pausable {
    ...
    function increaseCurrency(uint256 transactionId, uint256 amount, bytes memory signature) public {
        ...
        address addressCheck = recoverSigner(hash, signature);

        require(addressCheck == _gameAdmin, "must be game admin to increase currency");

        _signatureCounts_increase[_msgSender()]++;
        _mint(_msgSender(), amount);

        emit IncreaseCurrency(_msgSender(), transactionId);
    }

    function decreaseCurrency(uint256 transactionId, uint256 amount, bytes memory signature) public {
        bytes memory encoded = abi.encodePacked("\x19Ethereum Signed Message:\n117", _msgSender(), amount, _signatureCounts_decrease[_msgSender()], transactionId, TRANSACTION_TYPE_DECREASE);
        bytes32 hash = keccak256(encoded);
        address addressCheck = recoverSigner(hash, signature);

        require(addressCheck == _gameAdmin, "must be game admin to decrease currency");

        _signatureCounts_decrease[_msgSender()]++;
        _burn(_msgSender(), amount);
