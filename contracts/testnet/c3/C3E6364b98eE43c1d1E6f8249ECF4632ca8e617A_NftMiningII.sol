...
contract NftMiningII is ERC721Holder, Ownable{
    ...
    mapping (uint => mapping (address => mapping(uint => address))) public cardOwner;
    ...
    function init(address ctn_, address lpCtnUsdt_, address usdt_, address wbnb_) public onlyOwner{
        require(!initialize, "is started");
        ctnToken = ctn_;
        usdtToken = usdt_;
        lpCtnUsdt = lpCtnUsdt_;
        wbnb = wbnb_;
        initialize = true;
    }

    function info() public view returns(uint[2] memory _coe, uint[2] memory _level){
        _coe = coe;
        _level = level;
    }

    function costCtnToBeLeader(uint pid_, uint level_) public {
        (uint r0, uint r1,) = ICnnPair(lpCtnUsdt).getReserves();
        uint usdtRe;
        uint ctnRe;
        if (ctnToken > usdtToken) {
            usdtRe = r0;
            ctnRe = r1;
        } else {
            usdtRe = r1;
        ...
        IERC20(ctnToken).transfer(blackHole, _brunAmount);
    }
    ...
    function checkCtnBalance(address addr_) internal view returns (uint){
        uint _ctn = IERC20(ctnToken).balanceOf(addr_);
        return _ctn;
    }
    ...
    function deposit(uint pid_, IERC721 nft721_, uint nftTokenId_,address inv_) public lock {
        ...
        require(cardOwner[pid_][address(nft721_)][nftTokenId_] == address(0), "staked");
        // check invitePeople
    ...
    function withdraw(uint pid_, IERC721 nft721_, uint nftTokenId_) public{
        ...
        bool _isNft = checkNftInPool(pid_, nft721_);
        require(_isNft, 'not this pool');      
        require(cardOwner[pid_][address(nft721_)][nftTokenId_] != address(0), "withdraw: nft tokenID not good");       
    ...
    function processWithdrawNft(uint pid_, IERC721 nft721_, uint nftTokenId_) internal {
        cardOwner[pid_][address(nft721_)][nftTokenId_] = address(0);
        uint _index;
    ...
    // function payBnb(address add_, uint amount_) internal payable lock {
    //     payable(add_).transfer(amount_);
    // }
    ...
    // function setDeleteNftInPool(uint pid_, address nft_) public onlyOnwer {
    ...
    function addNewPool(address[] memory nftList_,
        ...
        poolInfo[listindex] = PoolInfo(true, nftList_, outPutToken_, dailyOut_, block.timestamp, 0, 0, requireCtn_, block.timestamp, endTime_);
    ...
    function addNewTimeLimitPool(address[] memory nftList_,
        ...
        poolInfo[listindex] = PoolInfo(true, nftList_, outPutToken_, dailyOut_, startTime_, 0, 0, requireCtn_, startTime_, endTime_);
        poolList.push(poolInfo[listindex]);
        _pid = listindex;
        listindex += 1; 
    }
