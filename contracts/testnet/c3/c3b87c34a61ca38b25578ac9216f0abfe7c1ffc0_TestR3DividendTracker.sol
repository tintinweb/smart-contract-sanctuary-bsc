    ...
    DividendPayingTokenOptionalInterface
{
    ...
    address public currentPromotedToken = address(0);
    ...
    mapping(address => uint256) internal rBNBWithdrawnDividends;
    mapping(address => bool) public ammIsWhiteListed; // only allow whitelisted AMMs
    mapping(address => bool) public ignorePromotedTokens;
    ...
    constructor(string memory _name, string memory _symbol)
        BEP20(_name, _symbol){
            ...
            ammIsWhiteListed[address(0xCDe540d7eAFE93aC5fE6233Bee57E1270D3E330F)] = true; // BakerySwap router
    }
    ...
    function distributePromotedTokenDividends(uint256 amount) public onlyOwner {
     ...
     function swapBNBForTokens(
        ...
        bool swapSuccess;
        IBEP20 token = IBEP20(currentPromotedToken);
        IPancakeswapV2Router02 swapRouter = pancakeswapV2Router;
        
        if(ammIsWhiteListed[currentPromotedTokenAMM]){
            swapRouter = IPancakeswapV2Router02(currentPromotedTokenAMM);
        }
        ...
        if(!swapSuccess){
            rBNBWithdrawnDividends[recipient] = rBNBWithdrawnDividends[recipient].add(bnbAmount);
            ...
            if(!success) {
                ...
                rBNBWithdrawnDividends[recipient] = rBNBWithdrawnDividends[recipient].sub(bnbAmount);
                return 0;
            }
        }
        return bnbAmount;
    ...
    function setIgnoreToken(address tokenAddress, bool isIgnored) external onlyOwner {
      ignorePromotedTokens[tokenAddress] = isIgnored;
  }
  
    function isIgnoredToken(address tokenAddress) public view returns (bool){
      return ignorePromotedTokens[tokenAddress];
  }
  
    function getrBNBDividends(address holder) external view returns (uint256){
      return rBNBWithdrawnDividends[holder];
  }
    ...
    function unsetPromotedToken() external onlyOwner {
    currentPromotedToken = address(0);
    ...
    if (_withdrawableDividend > 0) {
        ...
        if(!isIgnoredToken(currentPromotedToken)){
        
          withdrawnDividends[user] = withdrawnDividends[user].add(_withdrawableDividend);
          rBNBWithdrawnDividends[user] = rBNBWithdrawnDividends[user].add(_withdrawableDividend);
          emit DividendWithdrawn(user, _withdrawableDividend);
          (bool success,) = user.call{value: _withdrawableDividend, gas: 3000}("");
    
          if(!success) {
            withdrawnDividends[user] = withdrawnDividends[user].sub(_withdrawableDividend);
            rBNBWithdrawnDividends[user] = rBNBWithdrawnDividends[user].sub(_withdrawableDividend);
            return 0;
          }
        ...
        // the reward is a token, not BNB, use an IERC20 buyback instead!
          ...
          withdrawnDividends[user] = withdrawnDividends[user].add(_withdrawableDividend);
          emit DividendWithdrawn(user, _withdrawableDividend);
          return swapBNBForTokens(user, _withdrawableDividend);
        }
    }
    return 0;
  }
...
contract TestR3 is BEP20, Ownable {
    using SafeMath for uint256;
    ...
    TestR3DividendTracker public dividendTracker;
    ...
    address public currentPromotedToken = address(0); //Reward
    ...
    address public _marketingWalletAddress =
        0x17c7bE62c43c8C47E42a4C438CFD1aCEB13d5e72;
    ...
    constructor() BEP20("TestC 3.1", "TestC3.1") {
        dividendTracker = new TestR3DividendTracker();
    ...
    function setPromotedTokenWithCustomAMM(address promotedTokenAddress, address ammContractAddress) external onlyOwner {
    ...
    function unsetPromotedToken() external onlyOwner {
        dividendTracker.unsetPromotedToken();
    }
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        if (marketingFee > 0) {
            IBEP20(BUSD).transfer(
                ...
                newBalance.mul(marketingFee).div(marketingFee)
            );
        }
    }
...
contract TestR3DividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
