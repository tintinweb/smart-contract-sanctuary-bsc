...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    constructor(string memory _name, string memory _symbol, address _rewardToken, address _faucet) ERC20(_name, _symbol) { 
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            try faucet.airdrop(user, _withdrawableDividend, 0) {
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(uint256 minBalance, address _rewardToken, address _faucet) DividendPayingToken("Reward Tracker", "DividendTracker", _rewardToken, _faucet) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = minBalance * 10 ** 18;
        IERC20(_rewardToken).approve(_faucet, type(uint256).max);
    }
    ...
    function updateFaucet(address _newFaucet) external onlyOwner {
        faucet = IFaucet(_newFaucet);
        IERC20(rewardToken).approve(address(faucet), type(uint256).max);
    }
...
contract LuckyStake is ERC20, Ownable {
    uint256 public rewardPoolFeeOnBuy  = 1;
    uint256 public rewardPoolFeeOnSell = 1;

    uint256 public treasuryFeeOnBuy  = 2;
    uint256 public treasuryFeeOnSell = 2;

    uint256 public buybackBurnFeeOnBuy  = 4;
    uint256 public buybackBurnFeeOnSell = 4;

    uint256 public stakeRewardFeeOnBuy  = 5;
    uint256 public stakeRewardFeeOnSell = 5;

    uint256 public  wtowFee = 20;
    uint256 private accumulatedWtowFee;

    uint256 private _totalFeesOnBuy  = 12;
    uint256 private _totalFeesOnSell = 12;

    address public treasuryWallet   = 0x8a0a5CbDFf7Fb9aE5B6F7D3a792a9653014edb92;
    address public rewardPoolWallet = 0x8a0a5CbDFf7Fb9aE5B6F7D3a792a9653014edb92;

    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;

    IFaucet public faucet = IFaucet(0x6fc8f49e0aDEd906a6137A73E3ef18bf40262639);
    ...
    uint256 public bnbValueForBuyBurn = 0; // 0.3 BNB
    uint256 public accumulatedBuybackBNB;

    mapping (address => bool) private _isExcludedFromFees;
    mapping (address => bool) public automatedMarketMakerPairs;
    mapping (address => bool) public inFaucet;

    DividendTracker public dividendTracker;
    address public rewardToken = 0x8516Fc284AEEaa0374E66037BD2309349FF728eA;
    address public BUSD = 0x8516Fc284AEEaa0374E66037BD2309349FF728eA;
    ...
    event SendRewardPool(uint256 rewardPoolBUSD);
    event BuyBack(uint256 buybackAmount);
    event UpdateBuyFees(
        uint256 rewardPoolFeeOnBuy,
        uint256 treasuryFeeOnBuy,
        uint256 luckystakeBuybackBurnFeeOnBuy,
        uint256 stakeRewardFeeOnBuy
    );
    event UpdateSellFees(
        uint256 rewardPoolFeeOnSell,
        uint256 treasuryFeeOnSell,
        uint256 luckystakeBuybackBurnFeeOnSell,
        uint256 stakeRewardFeeOnSell
    );
    event UpdateWalletToWalletFee(uint256 walletToWalletFee);

    constructor() ERC20("LuckyStake", "$LUCKYSTAKE") 
    {
        _mint(owner(), 1_000_000 * (10 ** 18));
        swapTokensAtAmount = totalSupply() / 5000;

    	dividendTracker = new DividendTracker(1_000_000 / 50000, rewardToken, address(faucet));
    ...
    function updateBuyFees(
        uint256 _rewardPoolFeeOnBuy,
        uint256 _treasuryFeeOnBuy,
        uint256 _buybackBurnFeeOnBuy,
        uint256 _stakeRewardFeeOnBuy
    ) external onlyOwner {
        require(
            _rewardPoolFeeOnBuy +
                _treasuryFeeOnBuy +
                _buybackBurnFeeOnBuy +
                _stakeRewardFeeOnBuy <=
                13,
            "Fees must be less than 13%"
        );
        rewardPoolFeeOnBuy = _rewardPoolFeeOnBuy;
        treasuryFeeOnBuy = _treasuryFeeOnBuy;
        buybackBurnFeeOnBuy = _buybackBurnFeeOnBuy;
        stakeRewardFeeOnBuy = _stakeRewardFeeOnBuy;
        emit UpdateBuyFees(
            rewardPoolFeeOnBuy,
            treasuryFeeOnBuy,
            buybackBurnFeeOnBuy,
    ...
    function updateSellFees(
        uint256 _rewardPoolFeeOnSell,
        uint256 _treasuryFeeOnSell,
        uint256 _buybackBurnFeeOnSell,
        uint256 _stakeRewardFeeOnSell
    ) external onlyOwner {
        require(
            _rewardPoolFeeOnSell +
                _treasuryFeeOnSell +
                _buybackBurnFeeOnSell +
                _stakeRewardFeeOnSell <=
                13,
            "Fees must be less than 13%"
        );
        rewardPoolFeeOnSell = _rewardPoolFeeOnSell;
        treasuryFeeOnSell = _treasuryFeeOnSell;
        buybackBurnFeeOnSell = _buybackBurnFeeOnSell;
        stakeRewardFeeOnSell = _stakeRewardFeeOnSell;
        emit UpdateSellFees(
            rewardPoolFeeOnSell,
            treasuryFeeOnSell,
            buybackBurnFeeOnSell,
            stakeRewardFeeOnSell
        );
    }
    ...
    function updateWalletToWalletFee(uint256 _walletToWalletFee)
        ...
        onlyOwner
    {
        require(_walletToWalletFee <= 25, "Fee must be less than 25%");
        wtowFee = _walletToWalletFee;
    ...
    function changeRewardPoolWallet(address _rewardPoolWallet)
        external
        onlyOwner
    {
        require(
            _rewardPoolWallet != rewardPoolWallet,
            "Reward pool wallet is already that address"
        );
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            contractTokenBalance -= accumulatedWtowFee;
            
            uint256  treasuryTokens    = 
                contractTokenBalance * (treasuryFeeOnBuy + treasuryFeeOnSell) / (_totalFeesOnBuy + _totalFeesOnSell);
            
            uint256  rewardPoolTokens  = 
                contractTokenBalance * (rewardPoolFeeOnBuy + rewardPoolFeeOnSell) / (_totalFeesOnBuy + _totalFeesOnSell);
            
            uint256  buybackBurnTokens = 
                (contractTokenBalance * (buybackBurnFeeOnBuy + buybackBurnFeeOnSell) / (_totalFeesOnBuy + _totalFeesOnSell)) 
                    + accumulatedWtowFee;
            
            uint256  stakeRewardTokens = 
                contractTokenBalance * (stakeRewardFeeOnBuy + stakeRewardFeeOnSell) / (_totalFeesOnBuy + _totalFeesOnSell);

            contractTokenBalance += accumulatedWtowFee;

            if(treasuryTokens > 0) {
                uint256 treasuryBNB = newBalance * treasuryTokens / contractTokenBalance;
                payable(treasuryWallet).transfer(treasuryBNB);
            }

            if(rewardPoolTokens > 0) {
                uint256 rewardPoolBNB = newBalance * rewardPoolTokens / contractTokenBalance;
                swapAndSendBUSD(rewardPoolBNB);
            }

            if(stakeRewardTokens > 0) {
                uint256 stakeRewardBNB = newBalance * stakeRewardTokens / contractTokenBalance;
                swapAndSendDividends(stakeRewardBNB);
            }

            if(buybackBurnTokens > 0) {
                uint256 buybackBurnBNB = newBalance * buybackBurnTokens / contractTokenBalance;
                accumulatedBuybackBNB += buybackBurnBNB;
        ...
        if( inFaucet[from] ) {
            ...
            try faucet.team(from) {
                (, uplineFrom,,,,,,,,,,) = faucet.team(from);
            } catch{}

            if(uplineFrom != address(0)) {
                inFaucet[from] = true;
                try dividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
            }
        }
        ...
        if( inFaucet[to] ) {
            ...
            try faucet.team(to) {
                (, uplineTo,,,,,,,,,,) = faucet.team(to);
            } catch{}
            
            if(uplineTo != address(0)) {
                inFaucet[to] = true;
                try dividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}
            }
        }
    ...
    function setBNBValueForBuyBurn(uint value) public onlyOwner {
        require(value >= 1, "Minimum amount 0.1 BNB");
    ...
    function swapAndSendBUSD(uint256 amount) private{
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = BUSD;
    ...
    function updateFaucet(address newFaucet) external onlyOwner {
        dividendTracker.updateFaucet(newFaucet);
        faucet = IFaucet(newFaucet);
    }
