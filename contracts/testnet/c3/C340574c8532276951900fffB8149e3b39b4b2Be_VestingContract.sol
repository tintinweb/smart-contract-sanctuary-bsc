...
contract VestingContract is Ownable, ReentrancyGuard {
	...
	IERC20 public token;
	uint256 public constant END_TIME = 1661518800;		//	Aug 26th, 2022 01:00PM UTC

	mapping(address => Policy) private policies;
	mapping(address => uint256) public claimed;
	address[] private listOfBeneficiaries;
	uint256[] private schedules;
	bool public locked;
	...
	constructor(uint256[] memory _schedules) Ownable() {
		require(_schedules.length != 0, "Empty schedule");
    ...
    */
	...
	/**
	   	...
	   	@param _amounts				Vesting amounts per allocated schedules
    */
	function setVesting(
		...
		require(
			_amounts.length == schedules.length, "Invalid setting"
		);
    ...
    */
	...
	/**
       	@notice Query the vesting schedule
       	@dev Caller can be ANY
    */
	function getSchedules() external view returns (uint256[] memory _schedules) {
    ...
    */
	function claim() external nonReentrant {
		...
		require(
			policies[_beneficiary].amounts.length != 0, "Beneficiary not existed"
		);

		(uint256 _amount, uint256 _current) = _availableAmt(_beneficiary);
		require(_amount != 0, "Zero vesting amount. Please check your policy again");

		policies[_beneficiary].next = _current + 1;
		claimed[_beneficiary] += _amount;
	
		_releaseTokenTo(_beneficiary, block.timestamp, _amount);
	}

	function _availableAmt(address _beneficiary) private view returns (uint256 _amount, uint256 _current) {
		uint256 _now = block.timestamp;
		uint256[] memory _values =  policies[_beneficiary].amounts;
		uint256[] memory _schedules = schedules;
		uint256 _next = policies[_beneficiary].next;
		if (_next >= _schedules.length)
			return (0, _next);

		for(_next; _schedules[_next] <= _now; _next++) {
			_current = _next;
			_amount = _amount + _values[_next];
			if (_next + 1 == _schedules.length) break;
		}	
	}
    ...
    */
	function collect() external onlyOwner {
		uint256 _now = block.timestamp;
		require(
			_now > END_TIME + 7 days, "Should call 7 days after TGE"
		);
		uint256 _balance = token.balanceOf(address(this));
		require(_balance != 0, "Allocations completely vested");
		
		_releaseTokenTo(_msgSender(), _now, _balance);
	}
