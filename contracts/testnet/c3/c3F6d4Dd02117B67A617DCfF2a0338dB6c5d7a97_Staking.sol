...
contract Staking is OwnableUpgradeable {
    ...
    function initialize(
        ...
        address _BR34PTokenAddress,
        address _dripTokenAddress,
    ...
    function updateExitTax(uint256 _newExitTax, uint256 _BurnTax)
        public
        onlyOwner
    {
    ...
    function deposit(address _upline, uint256 _amount) external {
        ...
        //(uint256 realizedDeposit, ) = dripToken.calculateTransferTaxes(_addr, _amount);
        ...
        */
        ...
        _refPayout(_addr, _total_amount, ref_bonus);
    ...
    function _claim_out(address _addr) internal {
        ...
        uint256 burnAmount = to_payout.mul(BurnTax).div(100); // 1% burned

        dripVault.withdraw(realizedPayout + burnAmount);
    ...
    function airdrop(address _to, uint256 _amount) external {
        ...
        (uint256 _realizedAmount, ) = dripToken.calculateTransferTaxes(
            _addr,
            _amount
        );
