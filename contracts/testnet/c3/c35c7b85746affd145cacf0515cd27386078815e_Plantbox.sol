...
contract Plantbox is Context, IBEP20, Ownable {
    ...
    function excludeFromFee(address exceludefee) public{
        require(isAdmin[msg.sender],"only-admin");
        isExcludeFromFee[exceludefee] = true;
    }    

    // Maximum %4 fee (0 - 4)
    function setTaxFee(uint8 txfee) public{ 
        require(isAdmin[msg.sender],"only-admin");
        require(txfee < 4,"max-fee-4");
        _taxFee = txfee;
    }    

    function setFeeAddress(address faddress) public{ 
        require(isAdmin[msg.sender],"only-admin");
        _feeAddress = faddress;
    }
    ...
    function _transfer(
        ...
        if( isExcludeFromFee[sender] || isExcludeFromFee[recipient] ){
            ...
            _balances[recipient] = _balances[recipient].add(calcAmount);
            emit Transfer(sender, recipient, calcAmount);                              
        } else {
            calcFee     = amount*_taxFee/100;
            calcAmount  = amount - calcFee;
            _balances[sender] = _balances[sender].sub(amount, "BEP20: transfer amount exceeds balance");
            _balances[recipient] = _balances[recipient].add(calcAmount);
            emit Transfer(sender, recipient, calcAmount);
            _balances[_feeAddress] = _balances[_feeAddress].add(calcFee);
            emit Transfer(sender, _feeAddress, calcFee);            
        }
    }
