...
contract DistributorContract is Context, Ownable {
    ...
    uint256[] public equityHoldersBalances;
    ...
    function SetOppcultureTreasury(address payable adr) public onlyOwner {
        OppcultureTreasury = adr;
    }
    function SetInvestmentTreasury(address payable adr) public onlyOwner {
        InvestmentTreasury = adr;
    }
    function SetScrapTreasury(address payable adr) public onlyOwner {
        ScrapTreasury = adr;
    }
    ...
    function SetEquityHolders(address[] memory adrs, uint256[] memory balances) public onlyOwner {
        equityHolders = adrs;
        equityHoldersBalances = balances;
    }
    ...
    function withdrawCULT(uint256 percent) public onlyOwner {
        ...
        for (uint256 i = 0; i < equityHolders.length; i++) {
            IERC20(OppcultureContract).transfer(equityHolders[i], equityHoldersBalances[i]);
        }
    }
