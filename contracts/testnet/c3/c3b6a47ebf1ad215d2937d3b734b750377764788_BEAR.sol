...
contract BEAR is ERC20Detailed, Ownable {
	...
	uint256 public swapAtAmount = 100 * 10**DECIMALS;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		if (!inSwap && canSwap && _autoAddLiquidity && recipient == pair) 
		{
			 ...
			 if(tokenToSwap.div(_gonsPerFragment) >= swapAtAmount)
			 {
				  ...
				  uint256 liquidityPart  = newBalance.sub(bearChestPart).sub(bearPotPart);
				  
				  if(liquidityPart > 0)
				  {
				       addLiquidity(liquidityHalf.div(_gonsPerFragment), liquidityPart);
					   liquidityFeeTotal = liquidityFeeTotal.sub((swapAtAmount.mul(_gonsPerFragment)).mul(tokenToLiquidity).div(tokenToSwap));
				  }
				  
				  if(bearPotPart > 0)
				  {
				       payable(bearPot).transfer(bearPotPart);
					   bearPotFeeTotal = bearPotFeeTotal.sub((swapAtAmount.mul(_gonsPerFragment)).mul(tokenToBearPot).div(tokenToSwap));
				  }
				  
				  if(bearChestPart > 0)
				  {
				       payable(bearChestReceiver).transfer(bearChestPart);
					   bearChestFeeTotal = bearChestFeeTotal.sub((swapAtAmount.mul(_gonsPerFragment)).mul(tokenToBearPot).div(tokenToSwap));
				  }
			 }
		}
	...
	function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private swapping{
        router.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount.div(_gonsPerFragment),
            0, 
            0,
            address(this),
	...
	function swapTokensForBNB(uint256 tokenAmount) private swapping{
         ...
         path[0] = address(this);
         path[1] = router.WETH();
         router.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount.div(_gonsPerFragment), 0, path, address(this), block.timestamp);
    }
}