    ...
    ReentrancyGuard
{
    ...
    function removeCollateral(uint256[] calldata amountsRemoved)
        ...
        nonReentrant
    {
        uint256 totalAmountRemoved = positionBondFunction.verifyRemoveCollateral(
            amountsRemoved,
        ...
        require(
            LoanRatio.isNotReachMaxLoanRatio(
                UnderlyingAssetLending._underlyingAssetAmount() - totalAmountRemoved,
                (FaceAssetLending.getIssuePrice() * totalSupply()) / (10**18)
            ),
            "Not pass remove"
        );
        UnderlyingAssetLending._removeCollateral(amountsRemoved, totalAmountRemoved);
    ...
    function liquidate() external onlyPositionAdmin {
        if (isLiquidate()) {
            (bool success, ) = address(positionBondFunction).delegatecall(
                abi.encodePacked(
                    bytes4(keccak256("liquidated(uint256,uint256[])")),
    ...
    OwnableUpgradeable
{
    ...
    function verifyAddCollateral(
        ...
        if (underlyingAssetType == 0) {
            require(amountTransferAdded.length== 1, "amountAdded length must be 1");
    ...
    function aprroveBurnNFT() external {}
...
abstract contract LoanRatio {
    ...
    function isNotReachMaxLoanRatio(
        ...
        uint256 loanRatio = getLoanRatio(
            amountUnderlyingAsset,
            amountFaceAsset
        );
        return
