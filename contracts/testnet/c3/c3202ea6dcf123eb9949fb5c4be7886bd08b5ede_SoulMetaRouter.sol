...
contract SoulMetaRouter is DivestorUpgradeable, RandomGeneratorUpgradeable {
    ...
    function drawBox(address caller_, uint8 theme_, uint8 group_, uint amount_, address recommender_) public returns (bool){
        ...
        uint overAmount = _grant(player, theme_, group_, amount_, recommender_);

        require(overAmount == 0, "quantity");
        return true;
    }

    function _grant(address player, uint8 theme_, uint8 group_, uint amount_, address recommender_) private returns(uint){
        uint8 soulCount;
        uint poolId;
        uint[] memory ids;
        uint[] memory counts;
    ...
    function getBuildCount() private view returns (uint){
        uint openCount;
        uint time = block.timestamp/3600;
