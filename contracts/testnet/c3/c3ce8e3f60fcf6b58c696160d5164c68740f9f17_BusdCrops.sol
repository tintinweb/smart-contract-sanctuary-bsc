...
contract BusdCrops {
    ...
    function getUserInfo() public view returns(
        ...
        _usdtMiners = assets[msg.sender].usdtMiners;
        _usdtBuyTotalAmount = assets[msg.sender].usdtBuyTotalAmount;
        _usdtTotalWithdrawn = assets[msg.sender].usdtTotalWithdrawn;
        _stMiners = assets[msg.sender].stMiners;
        _stBuyTotalAmount = assets[msg.sender].stBuyTotalAmount;
        _stTotalWithdrawn = assets[msg.sender].stTotalWithdrawn;

        _beforeWithdrawnAmount = users[msg.sender].beforeWithdrawnAmount;
        _beforeStoneWithdrawnAmount = users[msg.sender].beforeStoneWithdrawnAmount;
        _referrer = users[msg.sender].referrer;
        _referralsCount = users[msg.sender].referralsCount;
        _referralEggRewards = users[msg.sender].referralEggRewards;
        _continuityDays = users[msg.sender].continuityDays;
    }
    ...
    function payFees(uint256 amount) internal returns(uint256) {
        //  uint256 tax = amount.mul(TAX).div(PERCENTS_DIVIDER);
        token_BUSD.transfer(mkt, amount);
        return amount;
    }
    //获取石头账号余额WA
