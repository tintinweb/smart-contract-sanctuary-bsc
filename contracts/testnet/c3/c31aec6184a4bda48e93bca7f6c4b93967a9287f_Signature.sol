...
contract Signature is EIP712, AccessControl, Pausable {
    ...
    function withdraw(address token,uint256 amount,bytes calldata signature) external whenNotPaused {
        require(verify(hash(_msgSender(), amount), signature), "Invalid signature");
         IERC20(token).safeTransfer(_msgSender(),amount);
    }

    function hash(address account, uint256 amount) private returns (bytes32){
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("Claim(address account,uint256 amount,uint256 nonce)"),
            account,
            amount,
            nonce[account]++
        )));
