...
contract YongQiang is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "YongQiang";
    string private _symbol = "YQT";
    uint8 private _decimals = 18;

    address public treasuryAddress;
    address payable public teamWalletAddress =payable(0xAdB9D902e66d8C40C177ba55378a6C6Fa9944209);
    ...
    uint256 public _treasuryShare = 1;
    ...
    event SetDistributionSettings(uint256 newLiquidityShare,uint256 newTreasuryShare,uint256 newTeamShare);

    event SetNumTokensBeforeSwap(uint256 newLimit);
    ...
    constructor(address _router) {
        ...
        isIncludedFromFee[address(uniswapPair)] = true;
        isMarketPair[address(uniswapPair)] = true;

        _totalDistributionShares = _liquidityShare.add(_treasuryShare).add(
            _teamShare
        );
    ...
    function setIsIncludedFromFee(address account, bool newValue)
        public
        onlyOwner
    {
        isIncludedFromFee[account] = newValue;
    }
    ...
    function setDistributionSettings(
        ...
        require(newLiquidityShare.add(newTreasuryShare).add(newTeamShare)>0, "totalDistributionShares must be greater than zero");
        ...
        emit SetDistributionSettings(newLiquidityShare,newTreasuryShare,newTeamShare);
    }

    function setNumTokensBeforeSwap(uint256 newLimit) external onlyOwner {
        minimumTokensBeforeSwap = newLimit;
        emit SetNumTokensBeforeSwap(newLimit);
    }
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            bool isIncluded = (isIncludedFromFee[sender] ||
                isIncludedFromFee[recipient]);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        uint256 amountBNBTreasury = amountReceived.sub(amountBNBLiquidity).sub(
            amountBNBTeam
        );

        if (amountBNBTreasury > 0){
            transferToAddressETH(payable(treasuryAddress), amountBNBTreasury);
            ITreasury(treasuryAddress).validatePayout();
        }
        if (amountBNBTeam > 0){
    ...
    function takeFee(address sender, uint256 amount)
        ...
        returns (uint256)
    {
        uint256 feeAmount = amount.mul(_totalDistributionShares).div(100);
