...
contract Mining is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function applyVoucher(address _user,uint256 _minerId, uint256 _vouchersAmount) 
        external 
        ...
        onlyAttachedMiner(_user,_minerId) 
    {
        ...
        miner.electricityLastDay = DateTime.addMonths(miner.electricityLastDay, _vouchersAmount);
    }
    ...
    {
        updatePool();
        MinerInfo memory miner = minersInfo[_user][_minerId];
    ...
    function claim(uint256 _minerId) external {
    ...
    function _deposit(address _user, uint256 _minerId, uint256 _hashrate) private {
        ...
        EnumerableSetUpgradeable.UintSet storage userIndexToMinerId = userToMiners[_user];
            ...
            miner.rewardDebt = 0;
            miner.electricityLastDay = block.timestamp;
    ...
    function _claimAll(address _user) internal {
        ...
        for (uint256 i = 0; i < userIndexToMinerId.length(); i++) {
            _claim(_user, userIndexToMinerId.at(i));
        }
     ...
     function getPendingRewardsFromAllMiners(address _user) external view returns (uint256)  {
        ...
        for (uint256 i = 0; i < userIndexToMinerId.length(); i++) {
            totalRewards += pendingReward(_user, userIndexToMinerId.at(i));
        }
   ...
   function _getPendingRewards(address _user,uint256 _minerId) private view returns (uint256) {
        ...
        if(block.timestamp > miner.electricityLastDay) {
            ...
            return (miner.hashrate * userAccRewardPerShare) / (1e18) - (miner.rewardDebt);
        }
    ...
    function getDateTimeConcat(uint256 _timestamp) internal view returns (uint256 date) {
