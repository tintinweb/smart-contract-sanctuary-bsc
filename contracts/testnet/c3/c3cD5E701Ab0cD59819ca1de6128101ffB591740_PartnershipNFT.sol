...
contract PartnershipNFT is ERC721Burnable, Pausable {
    ...
    function mint(uint _tier) whenNotPaused external returns(uint256 _tokenId) {
        ...
        // get tier of nft
        uint _newTier = _getTier(_tier);
        require(_newTier > 0, "PartnershipNFT: Invalid tier");

        //update state
        attributes[_tokenId].tier = _newTier;
        tierStatus[_newTier].currentMinted ++;
        tierIds[_newTier].add(_tokenId);
        ...
        //emit event
        emit PartnershipNFTCreated(msg.sender, _tokenId, _newTier);
        return _tokenId;
    }
    ...
    //================= internal ================== //
    function _getTier(uint _tier) internal returns (uint _newTier) {
        Status memory _status = tierStatus[_tier];
        if (_status.currentMinted < _status.supply) {
          return _tier;
        }
        return 0;
    }
