...
contract Crystals is ERC20Burnable, Operator {
    ...
    // Distribution for the DAO wallet
    uint256 public constant INITIAL_DAO_DISTRIBUTION = 5000 ether;
    ...
    uint256[] public taxTiersTwaps = [
        0, 1e17, 2e17, 3e17, 4e17, 5e17, 6e17, 7e17, 8e17, 9e17, 1e18
    ];
    ...
    function transferFrom(
        ...
        if (taxRate == 0 || excludedAddresses[_sender]) {
            _transfer(_sender, _recipient, _amount);
        } else if (pairs[_recipient]){
            _transferWithTax(_sender, _recipient, _amount);
        }
    ...
    function _transferWithTax(
        ...
        _transfer(_sender, taxCollectorAddress, taxAmount);

        // Transfer amount after tax to recipient
        _transfer(_sender, _recipient, amountAfterTax);
    ...
    function distributeReward(address _genesisPool) external onlyOperator {
        ...
        require(
            _genesisPool != address(0),
            "Genesis pool shouldn be non-zero address"
        );
