...
contract BuybackAndBurnCollector is Ownable, ReentrancyGuard
{
	...
	function buybackAndBurn(address _tokenSource, address _tokenIn, address _tokenOut, bool _directRoute, uint256 _minAmountOut) external nonReentrant returns (uint256 _amountOut)
	{
		require(whitelist[msg.sender], "access denied");

		ITokenSource(_tokenSource).collect();
		...
		IUniswapV2Router(router).swapExactTokensForTokens(_amountIn, _minAmountOut, _path, address(this), block.timestamp)[_path.length - 1];

		_amountOut = IERC20(_tokenOut).balanceOf(address(this));

		IERC20(_tokenOut).safeTransfer(FURNACE, _amountOut);
