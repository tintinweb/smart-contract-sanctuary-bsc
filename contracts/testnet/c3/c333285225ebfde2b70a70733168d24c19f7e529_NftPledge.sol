...
contract NftPledge is ERC1155Holder,Ownable {
    ...
    mapping(address => mapping(uint256 => uint256)) public userPledgeNftAmount;
    mapping(uint256 => uint256) public totalPledgeNftAmount;
    mapping(address => bytes32[]) public userOrderIds;
    mapping(address => uint256) public userOrderIdsAmount;
    ...
    constructor() ERC1155Holder() Ownable(){
        ...
        _setProjectMap(88802, 120, acc2 / 86400 , 300 * 10 ** 18, 10);
    ...
    function pledge(uint256 _nftId , uint256 _amount , uint256 _multiple)public{
        ...
        userOrderIdsAmount[msg.sender] = userOrderIdsAmount[msg.sender].add(1);
        userPledgeNftAmount[msg.sender][_nftId] =  userPledgeNftAmount[msg.sender][_nftId].add(_amount);
        totalPledgeNftAmount[_nftId] =  totalPledgeNftAmount[_nftId].add(_amount);
    ...
    function rePledge(bytes32 _assetId)  public{
        ...
        userOrderIdsAmount[msg.sender] = userOrderIdsAmount[msg.sender].sub(1);
        userPledgeNftAmount[msg.sender][nftId] =  userPledgeNftAmount[msg.sender][nftId].sub(userOrder[msg.sender][_assetId].amount);
        totalPledgeNftAmount[nftId] =  totalPledgeNftAmount[nftId].sub(userOrder[msg.sender][_assetId].amount);
    ...
    function removeByIndex(address _user, uint256 index) internal{       
        uint256 length = userOrderIdsAmount[_user];
        require(index < length, "ArrayForUint256: index out of bounds");
        delete userOrderIds[_user][index] ;
    ...
    function getMultiplier2(uint256 _startTime, uint256 _endTime, uint256 _multiple, uint256 amount, uint256 _accPerShare) public view returns(uint256){
        ...
        if(endGap == startGap){
        // same era
            startMod = _endTime.sub(_startTime);            
        }else{          
        // multiple eras,Calculate the start and end
            startMod = interval.sub( (_startTime.sub(genesis)).mod(interval));
            uint256 endMod = (_endTime.sub(genesis)).mod(interval);
            afterGit =  getPower(_accPerShare, endGap ) * endMod;
        }
        ...
        for(uint256 i = startGap.add(1); i < endGap; i++){
            betweenGit = betweenGit.add( getPower(_accPerShare, i ) * interval);
        }
    ...
    function calculateProductivity(uint256 _accPerShare, uint256 _nonce)public{
        ...
        if(length < _nonce){
            ...
            uint256 numerator = productivity[_accPerShare][i -1] * 90;
    ...
    function userOrderInfo(address _user,  uint256 page , uint256 pageSize)public view returns(UserOrders[] memory,uint256 total){		
        ...
        return (userOrders,userOrderIdsAmount[_user]);
    }
    
    function orderInfo(address _user, bytes32 _orderId)public view returns(UserOrders memory){		
        
     
        UserOrders memory UserOrder ;
        UserOrder.id = userOrder[_user][ _orderId ].id;           
        UserOrder.nftId = userOrder[_user][ _orderId ].nftId;
        UserOrder.amount = userOrder[_user][ _orderId ].amount;
        UserOrder.startTime = userOrder[_user][ _orderId ].startTime;
        UserOrder.maxTime = projectMap[userOrder[_user][ _orderId ].nftId ].maxTime;
        UserOrder.lastTime = userOrder[_user][ _orderId ].lastTime;
        UserOrder.ordertickets = projectMap[ UserOrder.nftId].tickets;
        UserOrder.orderMultiple =userOrder[_user][ _orderId ].multiple;
        UserOrder.fit = pendingFit(_user, _orderId);
        return UserOrder;
      
    }


    function nftTotal(address _user) public view returns(uint256[] memory,uint256[] memory){            
        uint256[] memory userPledgeNftAmounts = new uint256[](10);
        uint256[] memory totalPledgeNftAmounts = new uint256[](10);
        uint256 i = 0;
        for(uint256 nftId = 88801;  nftId <= 88810; nftId++ ){
            userPledgeNftAmounts[i] = userPledgeNftAmount[_user][nftId];
            totalPledgeNftAmounts[i] = totalPledgeNftAmount[nftId];
            i++;
        }
        return (userPledgeNftAmounts,totalPledgeNftAmounts);
    }
