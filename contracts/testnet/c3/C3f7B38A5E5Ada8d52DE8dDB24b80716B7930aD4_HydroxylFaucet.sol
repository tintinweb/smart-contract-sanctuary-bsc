...
contract HydroxylFaucet is Ownable {
    ...
    uint256 public everydayBindGift = 1501 ether ;
    uint64  public bindGiftTimeSpace = 1 days ;
    mapping(address => uint64) public bindGiftTime ;

    uint256 public minHolderBalance = 100 ether ;
    ...
    bool public isEveryCheck = false ;

    constructor(address mitAddr, address hydroxylAddr) {
    ...
    function setEveryCheck(bool everyCheck) external onlyOwner {
        isEveryCheck = everyCheck ;
    }

    function setMinHolder(uint256 minHolder) external onlyOwner {
        minHolderBalance = minHolder ;
    }
    ...
    function holderBalance(address player) private view returns(uint256) {
        ...
        if(current - preClaimTime >= holderGiftTimeSpace) {
            if(preClaimTime == 0) {
                // first claim
                uint256 balance = mitToken.balanceOf(player) ;
