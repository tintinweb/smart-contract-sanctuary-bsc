...
contract StakingLP is Ownable, ReentrancyGuard {
    ...
    event Claimed(address indexed staker, uint256 indexed id, uint256 interest, uint256 principal);
    ...
    /// @param _duration Claim lock duration
    function setClaimLockDuration(uint256 _duration) external onlyOwner {
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < stakingIds.length; i++) {
            currentTotalLPInterest = currentTotalLPInterest.add(getCurrentLPInterestById(stakingIds[i]));
        }
    ...
    /// @dev Get current total point of an address
    /// @param _address Address of the staker
    function getCurrentTotalPointOfAddress(address _address)
        ...
        returns (uint256)
    {
        uint256 currentTotalPoint = 0;
        uint256[] memory ids = addressToIds[_address];
        for (uint256 i = 0; i < ids.length; i++) {
            currentTotalPoint = currentTotalPoint.add(
                getCurrentPointById(ids[i])
            );
        }
    ...
    /// @param _id Id of the stake
    function requestClaim(uint256 _id) external nonReentrant onlyStakeholder(_id) {
        StakeDetail memory stakeDetail = idToStakeDetail[_id];
        require(
        ...
        idToClaimAbleAt[_id] = block.timestamp.add(claimLockDuration);
        emit ClaimRequested(msg.sender, _id);
    }
    ...
    function claim(uint256 _id) external nonReentrant onlyStakeholder(_id) {
        ...
        require(
            idToStakeStatus[_id] == StakeStatus.ClaimRequested &&
                block.timestamp >= idToClaimAbleAt[_id],
        ...
        pair.transfer(stakeDetail.owner, stakeDetail.amount);
        rewardToken.transfer(msg.sender, currentDNLInterest);
        emit Claimed(msg.sender, _id, stakeDetail.amount, currentDNLInterest);
    }
