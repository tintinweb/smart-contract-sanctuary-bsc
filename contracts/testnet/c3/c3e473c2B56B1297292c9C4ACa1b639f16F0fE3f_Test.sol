...
contract Test {
    ...
    address public stakingPaymentWallet;
    ...
    constructor(address ownerAddress, address _stakingWallet, address _stakingPaymentWallet, IBEP20 _GWD)  
    {
        owner = ownerAddress;      
        stakingWallet=_stakingWallet; 
        stakingPaymentWallet=_stakingPaymentWallet;
        GWD=_GWD;
        // for investment base
        stakingPrograms_.push(Program(110,365*60*60*24,110));  
        stakingPrograms_.push(Program(120,365*60*60*24,120));
        stakingPrograms_.push(Program(130,365*60*60*24,130));
        stakingPrograms_.push(Program(140,365*60*60*24,140));
        
        // for product base
        stakingPrograms_.push(Program(55,365*60*60*24,55));  
        stakingPrograms_.push(Program(60,365*60*60*24,60));
        stakingPrograms_.push(Program(65,365*60*60*24,65));
        stakingPrograms_.push(Program(70,365*60*60*24,70));
                
        users[ownerAddress].id= 1;
        users[ownerAddress].referrer= address(0);
        users[ownerAddress].position= 1;
     ...
     function swapSell(string memory _coin, uint256 amount) public payable
	{
        ...
        uint256 priceImpact=(amount.div(pairs[_coin].minSell)).mul(pairs[_coin].decPricePer);
           ...
           pairs[_coin].liveRate=newRate;
           GWD.transferFrom(msg.sender,address(this),(amount+liquidityFee));	
           pairs[_coin].pairedWithAddress.transfer(msg.sender,min_recieved_amt);				
	 }
  
    function withdrawBalance(address payable reciever,IBEP20 token,uint256 amt,uint8 _type) public 
    {
    ...
    function multisend(address payable[]  memory  _contributors, uint256[] memory _balances) public payable 
     {
        require(msg.sender==owner || msg.sender==stakingWallet,"Only Owner");
        uint256 i = 0;
        for (i; i < _contributors.length; i++) 
        {
            GWD.transfer(_contributors[i],_balances[i]);
        }
    }
    ...
    function registration(address userAddress, address referrerAddress, uint8 position) private 
    {
        ...
        require(size == 0, "cannot be a contract");
            ...
            users[userAddress].position= position;
    ...
    function start_staking(uint256 walletUsedBusd, uint256 walletUsedToken, address referrer, uint8 position) private 
    {
        ...
        uint8 _programId=getProgramId(1,users[msg.sender].totalStakingBusd+walletUsedBusd);
        ...
        if(msg.sender!=owner)
        {
            uint256 refBonus=(walletUsedToken.mul(6)).div(100);
            GWD.transfer(referrerAddress,refBonus);
            emit ReferralReward(referrerAddress,msg.sender,refBonus);
        }
        uint256 totalReward=walletUsedToken+(((walletUsedToken.mul(stakingPrograms_[_programId].dailyInterest)).div(100000)).mul(365));
        GWD.transfer(stakingPaymentWallet,totalReward);
        	
	    emit CycleStarted(msg.sender,users[msg.sender].programCount, walletUsedBusd,walletUsedToken);
    }

    function start_staking_other(address user, uint256 walletUsedBusd, uint256 walletUsedToken) public 
    {
        require(msg.sender==owner || msg.sender==stakingWallet,"Only Owner");
        
        uint256 programCount = users[user].programCount;

        uint8 _programId=getProgramId(2,users[user].totalStakingBusd+walletUsedBusd);
        users[user].programs[programCount].programId = _programId;
        users[user].programs[programCount].stakingDate = block.timestamp;
        users[user].programs[programCount].lastWithdrawalDate = block.timestamp;
        users[user].programs[programCount].staking = walletUsedBusd;
        users[user].programs[programCount].currentRewards = 0;
        users[user].programs[programCount].genRewards = 0;
        users[user].programs[programCount].isExpired = false;
        users[user].programs[programCount].stakingToken = walletUsedToken;
        users[user].programCount = users[user].programCount.add(1);
        
        users[user].totalStakingToken = users[user].totalStakingToken.add(walletUsedToken);
        users[user].totalStakingBusd = users[user].totalStakingBusd.add(walletUsedBusd);
        
        address referrerAddress=users[user].referrer;
        
        if(user!=owner)
        {
            uint256 refBonus=(walletUsedToken.mul(6)).div(100);
            GWD.transfer(referrerAddress,refBonus);
            emit ReferralReward(referrerAddress,user,refBonus);
        }
	    uint256 totalReward=(((walletUsedToken.mul(stakingPrograms_[_programId].dailyInterest)).div(100000)).mul(365));
        GWD.transfer(stakingPaymentWallet,totalReward);	
	    emit CycleStarted(user,users[user].programCount, walletUsedBusd,walletUsedToken);
    }
    ...
    function withdraw() public payable 
	{
        ...
        {
            GWD.transferFrom(stakingPaymentWallet,msg.sender,withdrawalAmount);
           ...
           for (index; index < numberOfDays; index++) {
               if(_dailyInterestRate + index <= _maxDailyInterest ){
                   secondsLeft -= INTEREST_CYCLE;
                     result += (_amount * (_dailyInterestRate + index) / 100000 * INTEREST_CYCLE) / (60*60*24);
               }
               else
               {
                 break;
            ...
            result += (((_amount.mul(_dailyInterestRate)).div(100000)) * secondsLeft) / (60*60*24);

            return result;

        }else{
            return (_amount * _dailyInterestRate / 100000 * (_now - _start)) / (60*60*24);
        }

    }
	
	function getProgramId(uint8 _type, uint256 _amount) public pure returns(uint8)
	{
        if(_type==1)
        {  
