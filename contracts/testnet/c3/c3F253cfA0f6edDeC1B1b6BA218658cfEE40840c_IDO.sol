...
//    mapping(address => IStake) public stakes; // idoToken => stake contract
    ...
    constructor(address _signer, address _stakeCT) {
        signer = _signer;
        stakeCT = _stakeCT;
    }
    function setMinMax(uint _pid, uint[] memory startTimes, uint[] memory endTimes, uint[] memory mins, uint[] memory maxs, uint cycleVestingDay) external onlyOwner {
        ...
        poolVesting[_pid].cycleVestingDay = cycleVestingDay;
    }
    ...
    function availableClaimFirstAmount(uint _pid, address _user) public view returns(uint available) {
        ...
        uint timespend = block.timestamp + cycleDays - p.endTime - pp.tgeTime;
        if(timespend >= pp.lockingTime) return isBuyer[_user][idoToken] * pp.tgePercent / 100;
        return 0;
    }
    function availableClaimAmount(uint _pid, address _user) public view returns(uint available) {
        ...
        uint timespend = block.timestamp + cycleDays - p.endTime - pp.lockingTime;
    ...
    function _buy(uint _pid, uint _amount) internal {
        ...
        address idoToken = address(_pool.idoToken);
        uint level = IStake(pp.stake).userLevel(_msgSender());
    ...
    function addPool(IERC20 _idoToken, IERC20 _idoToken2buy, uint _tokenBuy2IDOtoken,
        ...
        uint vestingPercent, uint _level, bool isWL) external onlyOperator{
//        _idoToken.transferFrom(_msgSender(), address(this), _totalAmount);
        poolInfo.push(PoolInfo(_idoToken, _idoToken2buy, _tokenBuy2IDOtoken, _totalAmount,
            _totalAmount, _startTime, _endTime, _level, 0, _msgSender(), isWL));
        poolVesting[poolInfo.length-1] = PoolVesting(tgePercent, tgeTime, vestingPercent, 30 days, lockingTime, stakeCT);
    }
