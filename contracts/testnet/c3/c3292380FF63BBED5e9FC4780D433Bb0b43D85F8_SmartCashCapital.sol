...
contract SmartCashCapital is Context, Ownable {
    ...
    uint256 constant maxRewards = 1500 ether;
    ...
    function poolLastDeposit(address userAddress, uint256 amount) private {
        ...
        lastDepositPoolBalance += amount.mul(10).div(percentDivider);
    ...
    function withdrawInitial(uint256 key) public isInitialized {
        ...
        uint256 dailyReturn;
        uint256 currDays = block.timestamp - user.depoList[key].depoTime;
        
        if (currDays <= PercsKey[10].daysInSeconds){
          	dailyReturn = user.depoList[key].amt.mul(PercsKey[10].divsPercentage).div(percentDivider);
            refundAmount = user.depoList[key].amt + (dailyReturn.mul(currDays).div(hardDays)) - (user.depoList[key].amt.mul(PercsKey[10].feePercentage).div(percentDivider));
        } else if (currDays > PercsKey[10].daysInSeconds && currDays <= PercsKey[20].daysInSeconds){
          	dailyReturn = user.depoList[key].amt.mul(PercsKey[20].divsPercentage).div(percentDivider);
            refundAmount = user.depoList[key].amt + (dailyReturn.mul(currDays).div(hardDays)) - (user.depoList[key].amt.mul(PercsKey[20].feePercentage).div(percentDivider));
        } else if (currDays > PercsKey[20].daysInSeconds && currDays <= PercsKey[30].daysInSeconds){
          	dailyReturn = user.depoList[key].amt.mul(PercsKey[30].divsPercentage).div(percentDivider);
            refundAmount = user.depoList[key].amt + (dailyReturn.mul(currDays).div(hardDays)) - (user.depoList[key].amt.mul(PercsKey[30].feePercentage).div(percentDivider));
        } else if (currDays > PercsKey[30].daysInSeconds && currDays <= PercsKey[40].daysInSeconds){
          	dailyReturn = user.depoList[key].amt.mul(PercsKey[40].divsPercentage).div(percentDivider);
            refundAmount = user.depoList[key].amt + (dailyReturn.mul(currDays).div(hardDays)) - (user.depoList[key].amt.mul(PercsKey[40].feePercentage).div(percentDivider));
        } else if (currDays > PercsKey[40].daysInSeconds && currDays <= PercsKey[50].daysInSeconds){
          	dailyReturn = user.depoList[key].amt.mul(PercsKey[50].divsPercentage).div(percentDivider);
            refundAmount = user.depoList[key].amt + (dailyReturn.mul(currDays).div(hardDays)) - (user.depoList[key].amt.mul(PercsKey[50].feePercentage).div(percentDivider));
        } else if (currDays > PercsKey[60].daysInSeconds){
          	dailyReturn = user.depoList[key].amt.mul(PercsKey[60].divsPercentage).div(percentDivider);
            refundAmount = user.depoList[key].amt + (dailyReturn.mul(currDays).div(hardDays)) - (user.depoList[key].amt.mul(PercsKey[60].feePercentage).div(percentDivider));
        } else {
    ...
    function getUserRefBonus(address userAddress) view external returns(uint256 bonus) {
		return users[userAddress].refBonus;
	}

    function getUserDeposits(address userAddress) view external returns(uint256 totalInvested) {
		return users[userAddress].totalInvested;
	}

    function getUserWithdrawn(address userAddress) view external returns(uint256 totalWithdrawn) {
		return users[userAddress].totalWithdrawn;
	}

    function getUserCompounded(address userAddress) view external returns(uint256 totalCompounded) {
		return users[userAddress].totalCompounded;
	}
