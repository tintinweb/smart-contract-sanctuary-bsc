...
contract DeerRun is IBEP20, Ownable {
    ...
    // Name and symbol
    string constant _name = "SUIIIIII";
    ...
    uint256 rewardFee;
    uint256 totalFee;
    uint256 amountXmasReward = 0;
    uint256 holdersCount =0;
    uint256 xmasDate = 1668387609;
    ...
    function _transferFrom(
        ...
        if(block.timestamp < xmasDate) {
            if(isSell) {
                if (balanceOf(sender) < 500 * (10**_decimals)) holdersCount--;
            } else {
                if(balanceOf(recipient)>500 * (10**_decimals)) holdersCount++;
            }
        }
        return true;
    }
    ...
    function swapBack() internal {
        ...
        uint256 amountBNBLiquidity = amountBNB
            ...
            .div(totalBNBFee)
            .div(2);
        uint256 rewardBNB = amountBNB.mul(rewardFee).div(totalBNBFee).div(2);

        uint256 amountBNBMarketing = amountBNB.mul(marketingFeeSell.add(marketingFeeBuy)).div(
            totalBNBFee
        );
        uint256 amountBNBDev = amountBNB.mul(devFeeSell.add(devFeeBuy)).div(totalBNBFee);

        amountXmasReward = amountXmasReward.add(rewardBNB);
    ...
    function getXmasRewardAmount() public view returns (uint256) {
        return amountXmasReward;
    }
    ...
    function recieveXmasReward() external {
        require(!hasAlreadyClaimed[msg.sender], "already claimed tokens");
        require(balanceOf(msg.sender) >= 500 * (10 ** _decimals), "you don't hold the min amount of Deer Run tokens");
        require (block.timestamp > xmasDate, "you cannot claim yet");
        
        payable(msg.sender).sendValue(amountXmasReward.div(holdersCount));
