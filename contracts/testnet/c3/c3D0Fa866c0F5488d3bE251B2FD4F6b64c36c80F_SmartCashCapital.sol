...
contract SmartCashCapital is Context, Ownable {
    ...
    uint256 public usersTotal;
    uint256 public compounds;
    uint256 public ovrTotalDeps;
    uint256 public ovrTotalComp;
    uint256 public ovrTotalWiths;
    ...
    mapping (address => mapping(uint256 => DepositList)) public DeposMap;
    ...
    function invest(address ref, uint256 amount) public isInitialized {
        require(ref != msg.sender, "User cannot self refer.");
        ...
        if (ref != 0x000000000000000000000000000000000000dEaD || ref != address(0)) user2.refBonus += refAmount;

        user.depoList.push(DepositList({
    ...
    function investRefBonus() public isInitialized { 
  	...
  	function reinvest() public isInitialized {
        ...
        user.depoList.push(DepositList({
              key: user.keyCounter,
              depoTime: block.timestamp,
              amt: getUserDividends(msg.sender),
              reffy: address(0), 
              initialWithdrawn: false
          }));
        
        ovrTotalComp += getUserDividends(msg.sender);
        user.lastWith = block.timestamp;  
        user.keyCounter++;
        compounds++;
    ...
    function userInfo() view external returns (DepositList [] memory depoList){
        return(users[msg.sender].depoList);
    }
    ...
    function getUserDepositInfo(address userAddress, uint256 index) view external returns(uint256 depositTime, uint256 amount, address ref, bool withdrawn) {
		depositTime = users[userAddress].depoList[index].depoTime;
		amount = users[userAddress].depoList[index].amt;
		ref = users[userAddress].depoList[index].reffy;
