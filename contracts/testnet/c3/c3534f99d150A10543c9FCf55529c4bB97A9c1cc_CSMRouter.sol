...
import ".././interfaces/ICSMRouter.sol";
import ".././interfaces/IPoolCrossChain.sol";
import "hardhat/console.sol";

contract CSMRouter is ICSMRouter, AccessControl, ReentrancyGuard {
    ...
    /// @notice approved assets by address and chain address=>chainId=>assetId
    mapping(address => mapping(uint16 => uint256)) private _approvedAssetIds;

    /// @notice approved assets by chain and asset id chainId=>assetId=>address
    ...
    /// @notice a mapping containing all pools indexed by their assets assetAddress=>poolAddress
    mapping(address => IPoolCrossChain) private _poolsPerAssets;

    ILayerZeroEndpoint public immutable layerZeroEndpoint;

    event MessageRouted(uint16 destinationChain, uint256 msgType, address indexed destinationAddress, uint64 nextNonce);
    ...
    function route(
        ...
        uint256 fee_,
        bytes calldata payload_
    ) external payable onlyRole(ROUTE_ROLE) nonReentrant {
        uint16 version = 1;
        uint256 gasForDestinationLzReceive = 350000; // TODO see this gas if it's enough
        bytes memory adapterParams = abi.encodePacked(version, gasForDestinationLzReceive);

        uint64 nextNonce = layerZeroEndpoint.getOutboundNonce(destinationChain_, address(this)) + 1;

        // send LayerZero message
        layerZeroEndpoint.send{ value: fee_ }( // {value: messageFee} will be paid out of this contract!
            destinationChain_, // destination chainId
            abi.encodePacked(_approvedRouters[destinationChain_]), // destination address of router contract on the other chain
        ...
        emit MessageRouted(destinationChain_, type_, destinationAddress_, nextNonce);
    }

    function lzReceive(
        uint16 _srcChainId,
        bytes memory _srcAddressBytes,
        uint64 _nonce,
        ...
        address _srcAddress;
        assembly {
            _srcAddress := mload(add(_srcAddress, 20))
        }
        require(_approvedAssetIds[address(_srcAddress)][_srcChainId] > 0, "SRC_CHAIN_NOT_APPROVED");
        (
            address sender,
            uint16 srcChainId,
            uint16 dstChainId,
            ...
            uint256 amount,
            uint256 haircut
        ) = abi.decode(_payload, (address, uint16, uint16, address, address, uint256, uint256));

        require(_srcChainId == srcChainId, "WRONG_CHAIN");
        require(_srcAddress == srcAsset, "WRONG_ASSET");
    ...
    function toggleAssetAndChain(
        ...
        if (add_) {
            ...
            require(_approvedAssetIds[assetAddress_][chainId_] != assetId_, "ASSET_EXISTS_1");
            require(_approvedAsset[chainId_][assetId_] != assetAddress_, "ASSET_EXISTS_2");
            require(_crossChainAsset[chainId_][assetId_].nativeChainAddress != assetAddress_, "ASSET_EXISTS_3");

            _approvedAssetIds[assetAddress_][chainId_] = assetId_;
            _approvedAsset[chainId_][assetId_] = assetAddress_;
            _crossChainAsset[chainId_][assetId_] = CrossChainAsset(0, 0, decimals_, uint64(assetId_), assetAddress_);
        } else {
...
interface ICSMRouter {
    struct CrossChainAsset {
        uint256 cash;
        uint256 liability;
        uint16 decimals;
        uint64 assetId;
        address nativeChainAddress;
    }
