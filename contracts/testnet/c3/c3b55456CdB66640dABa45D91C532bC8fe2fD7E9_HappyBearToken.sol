...
contract HappyBearToken is Context, Ownable {
    ...
    uint256 public _limitTxAmount = 500e9 * 1e9; // transaction limit: 500,000,000,000
    uint256 public numTokensSellToAddToLiquidity = 39e2 * 1e9; // swap token amount: 3900
    ...
    address public marketingAddress =
        0x8B2057A12503b7270A50955909445cE7287CBa1a;
    ...
    /* --------- Fees info --------- */
    Fees public buyFees =
        Fees({burn: 1, liquidity: 3, marketing: 1, buyback: 1, maxFee: 25});
    Fees public sellFees =
        Fees({burn: 2, liquidity: 4, marketing: 2, buyback: 2, maxFee: 25});
    Fees public transferFees =
        Fees({burn: 0, liquidity: 0, marketing: 0, buyback: 0, maxFee: 25});
    FeesStruct public _taxRates =
        FeesStruct({buyFee: 6, sellFee: 10, transferFee: 0});
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "LST::transferFrom: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "LST::decreaseAllowance: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function _burn(address account, uint256 amount) internal {
        require(
            account != address(0),
            "LST::_burn: burn from the zero address"
        );
        _balances[account] = _balances[account].sub(
            amount,
            "LST::_burn: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(
            owner != address(0),
            "LST::_approve approve from the zero address"
        );
        require(
            spender != address(0),
            "LST::_approve approve to the zero address"
        );
    ...
    function _burnFrom(address account, uint256 amount) internal {
        ...
        _approve(
            ...
            _allowances[account][_msgSender()].sub(
                amount,
                "LST::_burnFrom: burn amount exceeds allowance"
            )
        );
    }
    ...
    constructor() public {
        ...
        UniswapV2Pair = IUniswapV2Factory(_UniswapV2Router.factory())
            .createPair(address(this), _UniswapV2Router.WETH()); //MD vs USDT pair
    ...
    function setInitialAddresses(address _RouterAddress) external onlyOwner {
        ...
        UniswapV2Router = _UniswapV2Router;
        UniswapV2Pair = IUniswapV2Factory(_UniswapV2Router.factory())
            .createPair(address(this), _UniswapV2Router.WETH()); //LST vs USDT pair
    }

    function setFeeAddresses(address _marketingAddress, address _LPAddress)
        external
        onlyOwner
    {
    ...
    function setLimitTxAmount(uint256 limitTxAmount) external onlyOwner {
    ...
    function setBuyFee(
        ...
        require(
            buyFees.maxFee >= _LPFee + _marketingFee + _buybackFee + _burnFee,
            "LST::setBuyFee: Buy fee exceeds max buy fee"
        );
    ...
    function setSellFee(
        ...
        require(
            sellFees.maxFee >= _LPFee + _marketingFee + _buybackFee + _burnFee,
            "LST::setSellFee: Sell fee exceeds max transfer fee"
        );
    ...
    function setTransferFee(
        ...
        require(
            transferFees.maxFee >=
                _LPFee + _marketingFee + _buybackFee + _burnFee,
            "LST::setTransferFee: Transfer fee exceeds max transfer fee"
        );
    ...
    function _transfer(
        ...
        require(
            ...
            "LST::_transfer: Recipient is backlisted"
        );
        ...
        if (
            ...
            require(
                _limitTxAmount >= amount,
                "LST::_transfer: transfer amount exceeds max transfer amount"
            );
        ...
        if (
            ...
            _balances[marketingAddress] += amount.mul(checkFee.marketing).div(
                100
            );
            ...
            emit Transfer(
                sender,
                marketingAddress,
                amount.mul(checkFee.marketing).div(100)
            );
            emit Transfer(
                sender,
                LPAddress,
                amount.mul(checkFee.liquidity).div(100)
            );
            emit Transfer(
                sender,
                address(this),
                amount.mul(checkFee.buyback).div(100)
            );

            recieveAmount -= amount
                .mul(checkFee.marketing + checkFee.liquidity + checkFee.buyback)
                .div(100);
        }
        ...
        _moveDelegates(
            ...
            recieveAmount - amount.mul(checkFee.burn).div(100)
        );
        emit Transfer(
            sender,
            recipient,
        ...
        returns (uint256)
    {
        require(
            ...
            "LST::getPriorVotes: not yet determined"
        );
    ...
    function delMinter(address _delMinter) public onlyOwner returns (bool) {
        require(
            ...
            "LST::delMinter: _delMinter is the zero address"
        );
    ...
    function getMinter(uint256 _index) public view onlyOwner returns (address) {
        require(
            ...
            "LST::getMinter: index out of bounds"
        );
