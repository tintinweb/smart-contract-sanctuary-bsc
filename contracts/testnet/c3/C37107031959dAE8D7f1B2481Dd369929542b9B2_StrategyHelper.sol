...
contract StrategyHelper is LogicUpgradeable {
    ...
    address[] private pathToSwapBLIDToStableCoin;
    address[] private pathToSwapCAKEToStableCoin;
    address[] private pathToSwapBANANAToStableCoin;
    address[] private pathToSwapBSWToStableCoin;

    // Oracle of StableCoin for rewards, BLID
    address private oracleStableCoin4CAKE;
    address private oracleStableCoin4BANANA;
    address private oracleStableCoin4BSW;
    address private oracleStableCoin4BLID;
    ...
    address private lendBorrowFarmingPair;

    // BLID - USDT information
    address private blidSwapRouter;
        ...
        )
    {
        ...
        for (index = 0; index < xTokenList.length; ) {
            ...
            if (tokenInfo.totalSupply > 0 || tokenInfo.borrowAmount > 0) {
                // Calculation of sum
        ...
        )
    {
        ...
        PriceInfo[] memory priceUSDList = new PriceInfo[](xTokenList.length);
        ...
        (farmingPairStatistics, stakedMinusBorrow) = _getFarmingPairStatistics(
            logic,
            priceUSDList
        );
        ...
        // Staked minus borrowed amount
        if (strategyType == vStrategyType) {
            stakedMinusBorrow -= borrowAmountTotalUSD;
        }
    }
    ...
    function _getXTokenInfo(
        ...
        uint256 totalSupply = (xTokenBalance * exchangeRateMantissa) / 10**18;
        uint256 totalSupplyUSD = (totalSupply * priceUSD) / 10**18; // Supply Balance in USD

        // Get Lending Amount
        uint256 lendingAmount = IMultiLogicProxy(multiLogicProxy)
            .getTokenBalance(IXToken(xToken).underlying(), logic) -
            IERC20Upgradeable(IXToken(xToken).underlying()).balanceOf(logic);

        // Get collateralFactor from market to calculate borrowlimit
        uint256 collateralFactorMantissa;
        if (strategyType == vStrategyType) {
            (, collateralFactorMantissa, ) = IComptrollerVenus(comptroller)
                .markets(xToken);
        }
        ...
        tokenInfo = XTokenInfo(
            ...
            totalSupply,
            totalSupplyUSD,
            totalSupply - lendingAmount,
            borrowAmount,
            (borrowAmount * priceUSD) / 10**18,
            (totalSupplyUSD * collateralFactorMantissa) / 10**18,
            priceUSD
        );
    }
    ...
    function _getPriceUSDByCompound(
        ...
        address comptroller,
        uint8 strategyType
    ) private view returns (uint256 priceUSD) {
        if (strategyType == vStrategyType) {
    ...
    function _calcUSDAmountBySwap(
        ...
        AggregatorV3Interface oracle = AggregatorV3Interface(oracleStableCoin);
        amountUSD =
