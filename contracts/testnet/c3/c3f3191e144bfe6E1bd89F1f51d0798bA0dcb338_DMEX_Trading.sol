...
//--> 264 000
    ...
    function calculateLiquidationPrice(uint256[5] pos, uint256[4] values) public view returns (uint256)
    {
        ...
        if (fundingCost <= collateral)
        {
            movement = safeMul(safeSub(collateral, fundingCost), 1e8)/v.size;
        }
        else
        {
            movement = safeMul(safeSub(fundingCost, collateral), 1e8)/v.size;
        }
        ...
        if (pos[2] == 0)
        {
            ...
            {
                netLiqPrice = safeSub(v.price, min(v.price, movement));
            }
