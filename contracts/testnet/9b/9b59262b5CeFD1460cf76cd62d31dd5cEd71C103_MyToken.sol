    ...
    AccessControlUpgradeable
{
    ...
    function initialize() public initializer {
        ...
        targetSupply = 2**256 - 1;
        __ERC20Capped_init(targetSupply);
        ...
        onlyRole(OPERATOR_ROLE)
    {
        require(
            ...
            "MYTOKEN: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyRole(OPERATOR_ROLE)
    {
        require(
            ...
            "SetAccountCustomTax:: Invalid tax amount"
        );
        _hasCustomTax[account] = true;
        _customTaxRate[account] = taxRate;
    }

    function removeAccountCustomTax(address account)
        external
        onlyRole(OPERATOR_ROLE)
    {
    ...
    function setVaultAddress(address _newVaultAddress)
        public
        onlyRole(OPERATOR_ROLE)
    {
        ...
        returns (uint256 adjustedValue, uint256 taxAmount)
    {
        ...
        if (!_isExcludedFromFees[_from]) {
            uint8 taxPercent = totalFees; // set to default tax 10%
    ...
    function calculateTransactionTax(uint256 _value, uint8 _tax)
        ...
        returns (uint256 adjustedValue, uint256 taxAmount)
    {
        taxAmount = (_value * _tax) / 100;
        adjustedValue = (_value * (100 - _tax)) / 100;
        return (adjustedValue, taxAmount);
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            ...
            "TransferFrom:: Blacklisted address"
        );
        ...
        if (isExcluded) {
            ...
            super._beforeTokenTransfer(from, vaultAddress, taxAmount);
            emit TaxPayed(from, vaultAddress, taxAmount);
            super._beforeTokenTransfer(from, to, adjustedValue);
        }
    }
}
