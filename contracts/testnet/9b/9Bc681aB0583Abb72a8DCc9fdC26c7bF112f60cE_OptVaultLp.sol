...
contract OptVaultLp is AccessControl, Initializable {
     ...
     */
    function initialize(uint256 _id, 
    ...
    function deposit(address user, uint amountLp, bool isReceipt) external payable{
        ...
        IERC20(token).approve(address(LPSwap), (amountLp * 25)/100);
        uint busdBefore = IERC20(BUSD).balanceOf(address(this));
        ILpSwap(LPSwap).LPToToken(BUSD,token,(amountLp * 25)/100,address(this));
        ...
        IUSDY(USDy).mint(address(this), mintedUSDy);
        IERC20(USDy).approve(router, mintedUSDy);
        IERC20(BUSD).approve(router, (differenceAmount * 25/100));
        ...
        IERC20(token).approve(masterChef,(amountLp *75)/100);
        IPancakeMaster(masterChef).deposit(id, (amountLp *75)/100);
        if(IERC20(recieptToken).totalSupply() == 0){
            mintRecieptToken = (amountLp * 50)/exchangeRatio;
        }
        else{
            exchangeRatio = ((IPancakeMaster(masterChef).userInfo(id, address(this)).amount *  10000)/IERC20(recieptToken).totalSupply());   // for testing purpose multiple by 100
            mintRecieptToken = ((amountLp * 50))/exchangeRatio;
        }
        INTT(recieptToken).mint(address(this), mintRecieptToken);
        UserReciept[user] += mintRecieptToken;
        path[1] = address(bYSL);
        uint protocolPriceBYSL = IBYSL(bYSL).protocolPrice();
        byslPrice = protocolPriceBYSL * 100;
        uint mintedBYSL = (amountLp * 50)/ byslPrice;
        INTT(BShare).mint(user, mintedBYSL);
        IERC20(token).approve(masterChef,(amountLp *75)/100);
        IPancakeMaster(masterChef).deposit(id, (amountLp *75)/100);
        exchangeRatio = (IPancakeMaster(masterChef).userInfo(id, address(this)).amount* 100)/IERC20(recieptToken).totalSupply();
        rewardStateForDeposit(user,amountLp);
    }

    function rewardStateForDeposit(address user,uint amountLp) internal{
        if(stakedAmount[user] != 0){
            pendingReward[user] += (stakedAmount[user] * (S - share[user]))/10**18;
        }
        stakedAmount[user] += (amountLp *75)/100;
        totalStaked += (amountLp *75)/100;
        share[user] = S;
    }

    function rewardState(address user,uint _amount) public{
    ...
    function purchase(address user, uint amount, uint minAmount) external returns(uint){
        console.log("balance ff",IERC20(token).balanceOf(user));
        console.log("adress",address(this));
    ...
    function sell(address user, uint amount, uint minAmount) external returns(uint){
        INTT(recieptToken).burn(user, amount);
    ...
    function collectOptimizationTax() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        IERC20(BUSD).transfer(Treasury,(balanceUSD*_setOptimizationTaxFEE[2])/100);
        liqStatusValue = LiqStatus.SWAP_WANT_TO_BUSD;
    }
    ...
    function optimizationReward(address user, uint optMultiplier) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        IUSDY(USDy).mint(user, mintReward);
    }
...
interface ILpSwap{
    function initialize(address _owner, address _token, address _tokenA, address _tokenB, 
        address _WBNB, address _router,address _optFactory) external;
    function LPToToken(address token,address lp,  uint _amount,address sendTo) external;
    function LPToVault(address lp, address token, uint amount,bool isReceipt) external payable;
    function LPToLP(address lp1, address lp2, address sendTo, uint _amount) external payable;
