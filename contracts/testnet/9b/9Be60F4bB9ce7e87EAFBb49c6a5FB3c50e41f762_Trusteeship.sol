...
contract Trusteeship is Ownable, ReentrancyGuard {
    ...
    function getNftPropertyIdList(address _nftToken) public view returns (uint256[] memory){
        require(_nftToken != address(0), "NONEMPTY_NFT_ADDRESS");

        return (nftList[msg.sender][_nftToken].propertyIdList);
    }

    function getNftBalance(address _nftToken) public view returns (uint256, bool){
        require(_nftToken != address(0), "NONEMPTY_NFT_ADDRESS");
        return (nftList[msg.sender][_nftToken].propertyIdList.length, nftList[msg.sender][_nftToken].isTrust);
    }
