    ...
    BEP721BurnableUpgradeable
{
    ...
    function initialize() public initializer  {
        __Ownable_init();
        BEP721Upgradeable.__BEP721_init("OPENSALE", "OPNS");
        _tid = 1;
    ...
    function minting(string memory ipfsmetadata, uint256 royal, address from, address to) internal returns(uint256){
        _tid = _tid.add(1);
         uint256 id_ = _tid.add(block.timestamp);
         _creator[id_]= from;
         _safeMint(to, id_);
    ...
    function lazyMint(string memory ipfsmetadata,uint256 royal, address payable from,uint256 amount, string memory bidtoken, string memory status) public payable{
        uint256 id_ = 0;
        if(keccak256(abi.encodePacked((status))) == keccak256(abi.encodePacked(("Buy")))){
            id_ = minting(ipfsmetadata, royal, from, msg.sender);
            _lazySaleToken(from, id_, amount, bidtoken);
        }   
        else{
            id_ = minting(ipfsmetadata, royal, msg.sender, from);
            _acceptBId(bidtoken, from, amount, id_);
        }
         emit TokenId(msg.sender, id_);
    }
    function bidMint(string memory ipfsmetadata,uint256 royal, address from,uint256 amount, string memory bidtoken) public{
        uint256 id_ = minting(ipfsmetadata, royal, msg.sender, from);
        _acceptBId(bidtoken, from, amount, id_);
    }
