...
contract PandoPotV3 is Ownable, ReentrancyGuard, Pausable {
    ...
    enum PRIZE_TYPE {LEADERBOARD, MEGA, MINOR, TINY}
...
//    uint256 public currentMinorNumber2;
    ...
    function _updateRound(uint256 _id) internal {
        ...
        if (_roundInfo.status == 0) {
            ...
            (uint256[] memory _megas, uint256[] memory _minors, uint256[] memory _tinys) = getWinningNumbers(_id);
            emit RoundCompleted(_id, _expire, _megas, _minors, _tinys);
        }
    }
    ...
    // 0: leaderboard // 1 : mega // 2: minor // 3: tiny
    ...
    function _getTickets(uint _number) internal returns(uint, uint) {
        ...
        uint _tinyTicket = _number % typePrize[PRIZE_TYPE.TINY].digit;
        return (_minorTicket, _tinyTicket);
    }
    ...
    function _initPrize() internal {
        ...
        _addNewPrize(1e3, 4, 10, "Tiny", PRIZE_TYPE.TINY); // 3 digits
    }
    /*----------------------------EXTERNAL FUNCTIONS----------------------------*/
...
//        require(_type < 3, 'PandoPot: invalid type');
    ...
    // 2: expired
    // 3: claimed
    //_type: 0: distribute 1: mega 2: minor 3: tiny
    ...
    function getWinningNumbers(uint _roundId) public view returns (uint256[] memory, uint256[] memory, uint256[] memory) {
        return (roundInfo[_roundId].numbers[PRIZE_TYPE.MEGA].values(), roundInfo[_roundId].numbers[PRIZE_TYPE.MINOR].values(), roundInfo[_roundId].numbers[PRIZE_TYPE.TINY].values());
    }
    ...
    function getRoundInfo(uint _roundId) public view returns (uint256[] memory, uint256[] memory, uint256[] memory, uint256 finishedAt, uint256 status) {
        return (roundInfo[_roundId].numbers[PRIZE_TYPE.MEGA].values(), roundInfo[_roundId].numbers[PRIZE_TYPE.MINOR].values(), roundInfo[_roundId].numbers[PRIZE_TYPE.TINY].values(), roundInfo[_roundId].finishedAt, roundInfo[_roundId].status);
    }
    ...
    event RoundCompleted(uint256 roundId, uint256 expireTime, uint256[] mega, uint256[] minor, uint256[] tiny);
