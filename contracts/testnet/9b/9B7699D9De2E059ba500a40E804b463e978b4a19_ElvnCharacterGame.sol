...
contract ElvnCharacterGame is Ownable {
   ...
   struct StakingBoost{
    ...
    uint public expExponential;
    uint public equipCost;
    ...
    function setSecondaryContract(address _addressSecondary) public onlyOwner{
        secondaryContract = _addressSecondary;
    }

    function setTokenOwner(uint _tokenId, address _owner) public onlySetter{
    ...
    function equip(uint _tokenId, uint _part) public {
        ...
        require(INFTParts(secondaryContract).balanceOf(msg.sender,_part) > 0,"ELVN Character: You don't own this part");
        require(IERC20(gelvnAddress).balanceOf(msg.sender) >= equipCost, "ELVN Character: Not enough tokens");
        uint _class = INFTParts(secondaryContract).class(_part);
        if(_class == 1)
        {
        ...
        INFTParts(secondaryContract).burn(msg.sender,_part,1);
    ...
    function unequip(uint _tokenId, uint _part) internal {
        INFTParts(secondaryContract).mint(tokenOwner[_tokenId], _part, 1, "");
    }
    ...
    function createBaseCharacter(uint _start, uint _stop) public onlyOwner{
        ...
        {
            (uint[2] memory _baseParts, uint[2] memory _baseClass) = INFTParts(secondaryContract).getRandomBase();
            for(uint j = 0; j<= 1;j++)
            {
                if(_baseClass[j] == 1){
    ...
    function getEXPBatch(uint[] calldata _tokenId, uint[] calldata _expAmount) public onlySetter{
    ...
    function getEXP(uint _tokenId, uint _expAmount) public onlySetter{
        tokenExp[_tokenId] += _expAmount;
    }

    function useEXPBoost(uint _tokenId, uint _boostId) public {
        require(msg.sender == tokenOwner[_tokenId],"ELVN Character: You are not the Character Owner");
        require(INFTParts(secondaryContract).balanceOf(msg.sender,_boostId) > 0,"ELVN Character: You don't own this part");
        require(INFTParts(secondaryContract).class(_boostId) == 6,"ELVN Character: This is not a exp boost NFT");
        uint _expBoost = INFTParts(secondaryContract).expBoostPower(_boostId);
        getEXP(_tokenId,_expBoost);
        INFTParts(secondaryContract).burn(msg.sender,_boostId,1);
    }
    ...
    function activateBoostStaking(uint _tokenId, uint _boostId) public {
        require(msg.sender == tokenOwner[_tokenId],"ELVN Character: You are not the Character Owner");
        require(INFTParts(secondaryContract).balanceOf(msg.sender,_boostId) > 0,"ELVN Character: You don't own this part");
        require(INFTParts(secondaryContract).class(_boostId) == 7,"ELVN Character: This is not a staking boost NFT");
        uint _stakingBoost = INFTParts(secondaryContract).stakeBoostPower(_boostId);
        uint _stakingDuration = INFTParts(secondaryContract).stakeBoostDuration(_boostId);
    ...
    function sumStakingPower() public onlySetter{
    ...
    function totalStakingPower(uint _tokenId) public view returns (uint){
        ...
        uint _stakingPower = INFTParts(secondaryContract).totalStakingPower(_partsList);
        return _stakingPower;
    }

    function calculateStakingRewards() public onlySetter{
