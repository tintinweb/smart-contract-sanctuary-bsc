...
contract Test_Presale {
    ...
    uint public time;
    uint256 private numWalletsWhitelisted; 
	uint buyPrice;
    ...
    constructor(uint ADD_T, BEP20 ADD_T1, address ADD_T2) {
        ...
        minLimit = 50 * 10**18;
        maxLimit = 200 * 10**18;
        maxCollectionLimit = 750000 * 10**18;
        buyPrice = 500;
        buyPriceDecimal = 100;
    ...
    function deposit(uint amount) public {
        ...
        uint claimAmount1 = tokens * 75 / 1000;
        ...
        claim[sender].claimTimes.push(time + 40 minutes);
        claim[sender].claimTimes.push(time + 50 minutes);
        claim[sender].claimTimes.push(time + 60 minutes);
        claim[sender].claimTimes.push(time + 70 minutes);
        claim[sender].claimTimes.push(time + 80 minutes);
        claim[sender].claimTimes.push(time + 90 minutes);
        claim[sender].claimTimes.push(time + 100 minutes);
        claim[sender].claimTimes.push(time + 120 minutes);
        claim[sender].claimTimes.push(time + 130 minutes);
    ...
    function addToWhitelist(address account) private {
    if (!whitelist[account]) {
      whitelist[account] = true;
      numWalletsWhitelisted += 1;
    }
  }

  function addToWhitelist(address[] calldata accounts) public  {
    require(msg.sender == owner, "Only owner");
  ...
  function removeFromWhitelist(address account) public  {
    require(msg.sender == owner, "Only owner");
    require(whitelist[account], "Wallet not in whitelist");
