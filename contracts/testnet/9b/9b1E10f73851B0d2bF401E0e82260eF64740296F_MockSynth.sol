...
contract MockSynth is Owned, MixinResolver, Unimplemented, IMockSynth {
    ...
    {
        (_debt,) = _futuresMarketManager().totalDebt();
        return _debt;
    }
    ...
    function withdraw(address account, uint amount) external onlyAccountOrBridge(account)
    {
    ...
    modifier onlyAccountOrBridge(address account) {
        require(msg.sender == account || msg.sender == bridge, "Only Account Or Bridge Contract");
        _;
    }
}
