	...
	Pausable
{
	...
	// The member token
	IERC20 public immutable memberToken;

	uint256 public immutable multiplierBP;

	bool public unlock;

	bool public unlimitedDeposits;
		...
		isAllowedContract(msg.sender)
	{
		...
		require(
			unlimitedDeposits ||
				depositToken.balanceOf(address(this)).add(
					_depositTokenAmount
				) <=
				maxNakedDepositCap(),
			'Exceeds deposit cap'
	...
	/*
	 * @notice Updates unlimited deposits
	 * @param _unlimitedDeposits: New unlimitedDeposits
	 */
	function setUnlimitedDeposits(bool _unlimitedDeposits)
		external
		onlyRole(DEFAULT_ADMIN_ROLE)
	{
		unlimitedDeposits = _unlimitedDeposits;
	}
