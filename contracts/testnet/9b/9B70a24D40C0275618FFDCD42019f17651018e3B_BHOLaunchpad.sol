...
contract BHOLaunchpad is Ownable, Pausable, Whitelist, SharedConstants {
    ...
    constructor(
        ...
        // );
        require(
            _launchpadData.allocationAt > _launchpadData.registerAt,
            "Allocation round must be greater than Register round"
        );
        if (_launchpadData.fcfsAt != 0) {
            require(
                _launchpadData.fcfsAt > _launchpadData.allocationAt,
                "FCFS round must be greater than Allocation round"
            );
        }
        require(
    ...
    function register() external {
        ...
        require(
            block.timestamp <= launchpadData.registerAt,
            "Register round ended"
        );
    ...
    function buyTokenAllocation(uint256 _amount) external checkWhoCanBuy {
        ...
        require(
            block.timestamp <= launchpadData.allocationAt,
            "Allocation round ended"
        );
    ...
    function buyTokenFCFS(uint256 _amount) external checkWhoCanBuy {
        require(_amount > 0, "Amount must the greater than 0");
        require(launchpadData.fcfsAt != 0, "FCFS is not available");
        require(status != Statuses.CANCELLED, "Launchpad is cancelled");
        require(status != Statuses.FINZALIZE, "Launchpad is finalized");
        // check start, end time
        require(
        ...
        require(
            block.timestamp > launchpadData.allocationAt,
            "Allocation round do not end yet"
        );
        // check time FCFS
        require(block.timestamp <= launchpadData.fcfsAt, "FCFS round ended");
    ...
    function testStaking(address _addr)
        public
        view
        returns (
            string memory id,
