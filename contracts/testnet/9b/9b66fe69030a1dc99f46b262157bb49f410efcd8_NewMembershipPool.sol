...
contract NewMembershipPool is ReentrancyGuardUpgradeable, Authorizable {
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            ...
            members[account].userRewardPerPoolTokenPaid[_pool] = pools[_pool]
                .rewardPerPoolTokenStored;
            members[account].userRewardMedal[_pool] = earnedMedal(
                account,
                _pool
            );
            for (uint8 i = 1; i < POOLS_INDEX; i++) {
                members[account].userMedalRewardPoolTokenPaid[i] = pools[i]
