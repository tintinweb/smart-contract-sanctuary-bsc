    ...
    ReentrancyGuardUpgradeable
{
        ...
        returns (uint256 energy)
    {
        ...
        for (uint256 i = 0; i < _stakingContracts.length; i++) {
            _stakedElixir += IElixirDefi(_stakingContracts[i])
                .getUserBalance(_user);
        }
        uint256 _stakedLPs = IElixirDefi(_farmingContract).getUserBalance(_user);
...
import "@openzeppelin/contracts/access/Ownable.sol";
import "./PresaleMasterConfigOptions.sol";
