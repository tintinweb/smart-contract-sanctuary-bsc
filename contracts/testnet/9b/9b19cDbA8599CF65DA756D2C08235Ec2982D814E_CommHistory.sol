...
contract CommHistory is Ownable {
    ...
    function getRoot(address _user, uint _type) external view returns(address root) {
        address _refferBy;
        address from = _user;
        for(uint i = 0; i < 7; i++) {
            (, _refferBy,,,,) = refer.userInfos(from);
    ...
    function isSpecial2Tree(address _user) external view returns(bool isSpecial, uint _round, address root) {
        address _refferBy;
        address from = _user;
        for(uint i = 0; i < 7; i++) {
            (, _refferBy,,,,) = refer.userInfos(from);
