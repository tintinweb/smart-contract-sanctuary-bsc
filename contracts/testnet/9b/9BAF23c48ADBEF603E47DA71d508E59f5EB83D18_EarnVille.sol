...
contract EarnVille is ERC20Detailed, Ownable {
    ...
    function _transferFrom(
        ...
        if (sender == pair && !excludedAccount) {
            ...
            if (blkTime > tradeData[sender].lastBuyTime + TwentyFourhours) {
                tradeData[sender].lastBuyTime = blkTime;
                tradeData[sender].buyAmount = amount;
            } else if (
                (blkTime < tradeData[sender].lastBuyTime + TwentyFourhours) &&
                ((blkTime > tradeData[sender].lastBuyTime))
            ) {
                require(
                    tradeData[sender].buyAmount + amount <= onePercent,
                    "ERR: Can't buy more than 1% in One day"
                );
                tradeData[sender].buyAmount =
                    tradeData[sender].buyAmount +
                    amount;
            }
        }
