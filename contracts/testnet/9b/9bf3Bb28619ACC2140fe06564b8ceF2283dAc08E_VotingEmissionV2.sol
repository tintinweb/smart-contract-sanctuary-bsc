...
import "../interfaces/IVotingEmissionV2.sol";
import "../interfaces/IVotingEscrow.sol";
import "../library/CoreUtility.sol";
import "../library/SafeDecimalMath.sol";

contract VotingEmissionV2 is IVotingEmissionV2, Ownable, CoreUtility {
    ...
    constructor(address votingEscrow_, uint256 guardedLaunchStart_) public {
        votingEscrow = IVotingEscrow(votingEscrow_);
        _maxTime = IVotingEscrow(votingEscrow_).maxTime();
        guardedLaunchStart = guardedLaunchStart_;
        require(_endOfWeek(guardedLaunchStart_) == guardedLaunchStart_ + 1800, "must be end of a week");
    }

    function initialize(uint256 lastTimestamp_) external {
        require(lastTimestamp == 0, "Already initialized");
        require(
            _endOfWeek(lastTimestamp_) == lastTimestamp_ + 1800 &&
                lastTimestamp_ >= guardedLaunchStart - 1800
        );
        uint256 size = poolSize;
        uint256 weights0;
        uint256 weights1;
        for(uint256 i = 0; i < size; i++) {
            weights0 = weights0.add(weightsPerWeek[lastTimestamp_ + 1800][_pools[i]]);
    ...
    function replacePool(uint256 index, address newPool) external onlyOwner {
        require(index < poolSize, "oldPool has not existed");
        address oldPool = _pools[index];
        _pools[index] = newPool;
        emit PoolReplaced(oldPool, newPool);
    }
    ...
    function getWeights(uint256 timestamp) public view override returns (uint256[] memory weights, address[] memory pools){
        uint256 size = poolSize;
        ...
        returns (uint256)
    {
        ...
        uint256 weekTimestamp = _endOfWeek(timestamp).sub( 1800);
        uint256 lastTimestamp_ = lastTimestamp; // gas saver
        require(weekTimestamp <= lastTimestamp_ + 1800, "Previous week is empty");
        if (weekTimestamp <= lastTimestamp_) {
            return weightsPerWeek[weekTimestamp][pool];
        }
        lastTimestamp = lastTimestamp_ + 1800;
        return _updatePoolWeight(weekTimestamp, pool);
    }

    function _updatePoolWeight(uint256 weekTimestamp, address pool)
        private
        returns(uint256 weight)
    {
        (uint256[] memory poolWeights, address[] memory pools) =
            getWeights(weekTimestamp);
        uint256 totalWeight;
    ...
    function _updateWeight() private {
        uint256 weekTimestamp = _endOfWeek(block.timestamp).sub( 1800);
        uint256 lastTimestamp_ = lastTimestamp; // gas saver
        require(weekTimestamp <= lastTimestamp_ + 1800, "Previous week is empty");
        if (weekTimestamp > lastTimestamp_) {
            lastTimestamp = lastTimestamp_ + 1800;
    ...
    ManagedPausable
{
    ...
    constructor(address token_, uint256 maxTime_, string memory name_, string memory symbol_, uint256 maxTimeAllowed_) public {
        token = token_;
        maxTime = maxTime_;
    ...
    function totalSupply() external view override returns (uint256) {
        ...
        if (weekCursor < currentWeek) {
            ...
            for (; weekCursor < currentWeek; weekCursor += 1800) {
                ...
                newNextWeekSupply = newNextWeekSupply.sub(newTotalLocked.mul(1800) / maxTime);
            }
            newTotalLocked = newTotalLocked.sub(scheduledUnlock[weekCursor]);
    ...
    function createLock(uint256 amount, uint256 unlockTime) external nonReentrant whenNotPaused {
        _assertNotContract();
        require(
            unlockTime + 1800 == _endOfWeek(unlockTime),
    ...
    function _totalSupplyAtTimestamp(uint256 timestamp) private view returns (uint256) {
        uint256 weekCursor = _endOfWeek(timestamp);
        uint256 total = 0;
        for (; weekCursor <= timestamp + maxTime; weekCursor += 1800) {
    ...
    function _checkpoint(
        ...
        uint256 newNextWeekSupply = nextWeekSupply;
        if (weekCursor < currentWeek) {
            for (uint256 w = weekCursor + 1800; w <= currentWeek; w += 1800) {
    ...
    function calibrateSupply() external {
        uint256 nextWeek = checkpointWeek + 1800;
...
*/
    ...
    uint256 internal constant DURATION = 1800;
    uint256 internal constant START_TIME = 1648519200;
