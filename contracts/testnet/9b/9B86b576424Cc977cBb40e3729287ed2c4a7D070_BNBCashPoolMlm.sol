...
contract BNBCashPoolMlm is Ownable {
    ...
    constructor(IBNBCashPool bcp, IBEP20 busd, address signerAddress) {
        require(bcp != IBNBCashPool(address(0)), "constructor: bcp is zero");
        require(busd != IBEP20(address(0)), "constructor: bcp is zero");
        ...
        uniswapPair = IUniswapV2Pair(IUniswapV2Factory(router.factory()).getPair(router.WETH(), address(busd)));
    ...
    function updatePair(IUniswapV2Pair newUniswapPair) external onlyOwner {
    ...
    function register( bytes32 sponsorCode, bool payWithBNB) public payable {
    ...
    function _sponsorShip(address caller, bool payWithBNB, bytes32 sponsorCode) private {
        ...
        if (canSponsor) {
            ...
            if (payWithBNB) {
                ...
                BUSD.transferFrom(caller, address(this), entryFee);
                if (msg.value > 0) {
                    require(payable(caller).send(msg.value), "register: BNB transfer failed!");
                }
            }
