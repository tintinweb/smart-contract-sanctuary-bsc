...
contract Marketplace is Ownable, ReentrancyGuard {
    ...
    function cancelOffer(uint256 id, uint256 index) external payable {
    ...
    function acceptOffer(uint256 id, uint256 index) external {
        Offer storage currentOffer = offers[id];
        if (
            ...
            _cancelOrder(
                onOrders[currentOffer.collection][currentOffer.tokenId]
            );
            IERC721(currentOffer.collection).transferFrom(
                address(this),
                currentOffer.maker[index],
