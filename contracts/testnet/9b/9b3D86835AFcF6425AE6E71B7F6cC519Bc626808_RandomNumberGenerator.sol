    ...
    Ownable
{
     ...
     * @notice View random result
     */
    function viewRandomTicketId() external view override returns (uint32) {
        ...
        override
    {
        ...
        uint256 maxTicketNumber = ICryptonaireLottery(Lottery)
            .maxNumberTicketsPerLottery();
        uint256 currentLotteryId = ICryptonaireLottery(Lottery)
            .viewCurrentLotteryId();

        // Get lottery first ticket id
        uint256 firstLotteryTicketId = (
            ICryptonaireLottery(Lottery).viewLottery(currentLotteryId)
        ).firstTicketId;

        // Get winning ticket id from random number => Get winning number from ticket id
        uint32 winningTicketId = uint32(randomWords[0] % maxTicketNumber) +
            uint32(firstLotteryTicketId);
        uint32 winningTicketNumber = ICryptonaireLottery(Lottery)
            .viewTicketNumber(winningTicketId);

        // Update state variable
        randomResult = winningTicketNumber;
        randomTicketId = winningTicketId;
