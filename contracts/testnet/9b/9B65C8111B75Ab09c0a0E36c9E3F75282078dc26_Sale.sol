...
contract Sale is ERC721Holder, ERC1155Holder, Ownable, Initializable {
    ...
    function createRaiseSingle(uint256 _projectId, uint256[] memory _tokenIds, uint256[] memory _raisePrices) external idValid(_projectId) onlyManagerWithProjectId(_projectId) {
    ...
    function bidSingle(uint256 _saleId) external payable onlyWinner(_saleId) idValid(_saleId) {
        ...
        (uint256 percent, uint256 profitShare, uint256 remainingProfit) = _sharing(!_project.isCreatedByAdmin, _project.id);
    ...
    function _sharing(bool isAdmin, uint256 projectId) private returns(uint256, uint256, uint256) {
        uint256 profitShare;
        ...
        if (isAdmin) {
            ...
            profitShare = _getProfitShare(msg.value, percent);
            remainingProfit = msg.value.sub(profitShare);
        ...
        return (percent, profitShare, remainingProfit);
    }
