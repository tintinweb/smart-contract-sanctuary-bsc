...
contract TazBondingCalculator is IBondingCalculator {
    ...
    constructor( address _TAZ ) {
        require( _TAZ != address(0) );
        TAZ = _TAZ;
    }
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == TAZ ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( TAZ ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}