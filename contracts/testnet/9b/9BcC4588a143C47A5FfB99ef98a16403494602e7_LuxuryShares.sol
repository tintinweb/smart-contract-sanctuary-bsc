...
contract LuxuryShares is ERC20Capped, ERC20Mintable, ERC20Burnable {
    ...
    function getLimitInfo(uint64 i) public view returns(address, uint256, uint256, uint256, uint256, uint256, uint8){
        require(i < limitedCount, "Out of data");
        address account = limitedList[i];
        LimitInfo memory details = limitedInfos[account];
        uint256 from = getLastLimit(details.startTime);
        uint256 to = from + limitTime;
        uint256 balance = balanceOf(account);
        uint256 maxLimitAmount = details.maxSellAmount;
        uint256 validAmount = details.validAmount;
        if(from > details.lastUpdate) {
            maxLimitAmount = balance * sellLimit[account] / 100;
            validAmount = maxLimitAmount;
        }
        
        return (account, maxLimitAmount, validAmount, balance, from, to, sellLimit[account]);
    }
    ...
    function setPercentageLimit(address account, uint8 p) public onlyOwner {
        require(p < 100, "Invalid percent data");
        require(limitedGuy[account], "No limited account");
        sellLimit[account] = p;
    }
