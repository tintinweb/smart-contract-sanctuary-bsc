...
contract ShinobiVerse is Context, IERC20, Ownable {
    ...
    address payable public vaultRewardAddress = payable(0xAd9b97fa8f28daCa6731d116d6fD2C72A164Ae0b); 
    address payable public developmentAddress = payable(0xAd9b97fa8f28daCa6731d116d6fD2C72A164Ae0b); 

    address payable public usdcAddress = payable(0xc21223249CA28397B4B6541dfFaEcC539BfF0c59);
    ...
    // Protocol Fees
    uint256 public _bMaxTxAmount = 45000000000  * 10**9;
    uint256 public _sMaxTxAmount = 45000000000  * 10**9;
        ...
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(uniswapV2Pair) > 0) {
            if (to == uniswapV2Pair) {
                if (overMinimumTokenBalance) {
                    ...
                    uint256 usdcLiquidityToken = contractTokenBalance.mul(_sellMarketingFee+_sellDevelopmentFee).div(_sellLiquidityFee);
                    uint256 rewardToken = contractTokenBalance.mul(_sellRewardFee).div(_sellLiquidityFee);
                    uint256 liquidityToken = contractTokenBalance.sub(usdcLiquidityToken+rewardToken);

                    // Remove Hate Swap and Liquidity by breaking Token in proportion
                    addLiquidityToToken(liquidityToken);
                    
                    // Token Swapping in CRO for usdc
                    swapTokens(usdcLiquidityToken);    
                    // Fetch balance of the token swapped for CRO for usdc
                    uint256 bnbBalusdc = address(this).balance;
                    // Swap CRO for USDC and send to All Address
                    SwapAndLiquifyCROtoUsdc(bnbBalusdc);
    ...
    function swapTokensForCRO(uint256 tokenAmount) private {
        // Generate the uniswap pair path of token -> WCRO
    ...
    function swapCROForTokensToUsdc(uint256 amount) private lockTheSwap{
        // Generate the uniswap pair path of token -> WCRO
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(usdcAddress);
    ...
    function setBuyMaxTxAmount(uint256 bMaxTxAmount) external onlyOwner {
        ...
        require(_bMaxTxAmount>_tTotal.div(1000), "Too low limit");
    }

    function setSellMaxTxAmount(uint256 sMaxTxAmount) external onlyOwner {
        _sMaxTxAmount = sMaxTxAmount;
        require(_sMaxTxAmount>_tTotal.div(1000), "Too low limit");
    }
    ...
    {
        usdcAddress = payable(_newUsdc);
    }

    function setReawardAndMarketingFee(uint256 _sellRewardsPercent,uint256 _sellMarketingPercent, uint256 _sellDevelopmentPercent, uint256 _sellLiquidityPercent, uint256 _buyRewardsPercent,uint256 _buyMarketingPercent, uint256 _buyDevelopmentPercent, uint256 _buyLiquidityPercent ) external onlyOwner 
    {    
        ...
        _buyLiquidityFee = _buyLiquidityPercent+_buyRewardsPercent+_buyMarketingPercent+_buyDevelopmentPercent;
        _sellLiquidityFee = _sellLiquidityPercent+ _sellMarketingPercent+_sellRewardsPercent+_sellDevelopmentPercent;

        require(_buyLiquidityFee<16, "Too High Fee");
        require(_sellLiquidityFee<20, "Too High Fee");
    ...
    function changeRouterVersion(address _router) public onlyOwner returns(address _pair) {
     ...
     // To receive CRO from uniswapV2Router when swapping
    ...
    //Create additional liquidity using CRO tokens in contract
    function manualSwapAndLiquifyCROtoUsdc(uint256 bnbLiquifyAmount) external lockTheSwap onlyOwner {
        SwapAndLiquifyCROtoUsdc(bnbLiquifyAmount);
    }
    ...
    function SwapAndLiquifyCROtoUsdc(uint256 bnbLiquifyAmount) private lockTheSwap {

        // swap CRO for Tokens
        swapCROForTokensToUsdc(bnbLiquifyAmount); // <- this breaks the CRO -> HATE swap when swap+liquify is triggered

        // how much Tokens did we just swap into?
        uint256 newTokenBalance = IERC20(usdcAddress).balanceOf(address(this));

        uint256 marketingFunds = newTokenBalance.mul(_sellMarketingFee).div(_sellLiquidityFee);
        uint256 developmentFunds = newTokenBalance.sub(marketingFunds);

        require(IERC20(usdcAddress).transfer(marketingAddress,marketingFunds));
        require(IERC20(usdcAddress).transfer(developmentAddress,developmentFunds));
        
        emit SwapAndLiquifyCRO(bnbLiquifyAmount, newTokenBalance, bnbLiquifyAmount);
    }
    ...
    function addLiquidityToToken(uint256 tokenLiquifyAmount) private lockTheSwap{
        ...
        // this is so that we can capture exactly the amount of CRO that the
        // swap creates, and not make the liquidity event include any CRO that
        // has been manually sent to the contract
        uint256 initialBalance = address(this).balance;

        // swap tokens for CRO
        swapTokensForCRO(half); // <- this breaks the CRO -> HATE swap when swap+liquify is triggered

        // how much CRO did we just swap into?
