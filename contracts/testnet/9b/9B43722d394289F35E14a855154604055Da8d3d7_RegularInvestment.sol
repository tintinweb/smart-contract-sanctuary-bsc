...
import "./RegErrors.sol";

abstract contract RegDeclaration is RegErrors {
    ...
    // [7 days, 30 days, 180 days, 365 days, 1095 days, 1825 days]
    uint[] public regularRewardDurations;

        
    uint[] public regularRewardPercentages;
    ...
    uint256 public stableTotalRegularReward;
...
abstract contract RegEvents{

    event RegularInvestmentCreated(
        ...
        uint256 stablePrincipal,
        uint256 stableReward,
        uint256 stableTotalRegularReward,
        uint256 totalAccumulatedReward
    );
    event RegularInvestmentRecreated(
        bytes16 indexed investmentID,
        address indexed investor,
        uint256 investedAmount,
        uint256 indexed startDay,
    ...
    event RegularInvestmentRewardSet(
        uint256[] regularRewardPercentages,
        uint256[] regularRewardDurations
    );
...
import "./RegHelper.sol";
import "./RegEvents.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

abstract contract RegFetchers is
    RegHelper,
    ReentrancyGuard,
    RegEvents,
    Ownable
...
import "./RegDeclaration.sol";
import "./RegEvents.sol";

abstract contract RegHelper is RegDeclaration {
    ...
    function getInvestmentReward(
        ...
        for (uint256 i = 0; i < regularRewardDurations.length; i++) {
            uint256 currentDuration = regularRewardDurations[i];
            if (_investingDays == currentDuration) {
                reward = (regularRewardPercentages[i] * _investedAmount) / 100;
                break;
            } else {
                reward = 0;
            }
    ...
    {
        uint[] memory durations = regularRewardDurations;
...
import "./RegFetchers.sol";

contract RegularInvestment is RegFetchers {
     ...
     * @param _treasury to hold the unclaimed reward after 14 days and part of the Flexvis token if investor ends any investment prematurely
     ...
     */
    function createRegularInvestment(uint256 amount, uint256 duration)
        external
        nonReentrant
        returns (bytes16 investmentID)
    {
        ...
        if (
            ...
            // Create a Pending Regular Investment. Admin should be contacted before It can be approved.
            ...
            pendingInvestment.hasInsurance = false;
            ...
            uint256 stableReward = rewardInFlexvis == 0
                ? 0
                : flexvisToUSD(rewardInFlexvis);
            ...
            // Every investment has a unique ID
            investmentID = _generateInvestmentID(msg.sender);
            ...
            stableTotalRegularReward += stableReward;
            totalAccumulatedReward += rewardInFlexvis;

            emit RegularInvestmentCreated(
                investmentID,
                msg.sender,
                amount,
    ...
    /**
     * @notice If your investment has been approved by the admin, this method should be called to recreate the investment and to store it as a real regular investment.
     * @param investmentID ID for the investment to re-create
     * @param amount flexvis amount for re-creating the investment
     * @param duration The number of days the investment will be locked up. The longer the day, the higher the reward
     */
    function recreateRegularInvestment(
        bytes16 investmentID,
        uint256 amount,
        uint256 duration
        ...
        if (investment.hasInsurance == true) {
            revert InvestmentHasInsurance();
        }
        ...
        // Update the investment array
        updateInvestmentArray(investment);

        emit RegularInvestmentRecreated(
            investmentID,
            msg.sender,
    ...
    /**
     * @notice Only the admin can call this method. This disapproves a pending investment if it is not meeting up with the criterias of a real regular investment.
        ...
        onlyOwner
    {
        ...
        // Update the investor's investment array
        updateInvestmentArray(investment);
     ...
     */
    function endRegularInvestment(bytes16 investmentID)
        external
        nonReentrant
        returns (uint256 totalReturn)
    {
        ...
        uint256 principal = investment.investedAmount;
        uint256 reward = investment.reward;
        ...
        stableTotalRegularReward -= investment.stableReward;
    ...
    /**
     * @notice In an emergency situation, It might required to withdraw out the Flexvis token in the contract in order to safeguard the investment of the investors. Only the admin can invoke this method.
     */
    function emergencyWithdraw() external nonReentrant onlyOwner {
        ...
        for (uint256 i = 0; i < allInvestors.length; i++) {
            ...
            if (flexvis.balanceOf(address(this)) >= totalInvested) {
                ...
                totalRegularInvested -= totalInvested;
     ...
     */
    function setRegularInvestmentReward(
        uint256[] memory percentages,
        uint256[] memory durations
    ) external onlyOwner {
        if (percentages.length == 0 && percentages.length != durations.length) {
            revert InvalidRegularInvestmentReward();
        }

        isPaused = true;
        delete regularRewardPercentages;
        delete regularRewardDurations;
        for (uint256 i = 0; i < percentages.length; i++) {
            regularRewardPercentages.push(percentages[i]);
            regularRewardDurations.push(durations[i]);
        }
        isPaused = false;

        // Emit an event
        emit RegularInvestmentRewardSet(
            regularRewardPercentages,
            regularRewardDurations
        );
    }
