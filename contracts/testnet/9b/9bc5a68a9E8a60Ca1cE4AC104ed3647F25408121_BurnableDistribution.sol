...
contract BurnableDistribution is Ownable, Initializable {
    event initializedOn( 
        address account, 
        uint alloc, 
        uint time
    ...
    uint[2] public total; //0- total raised, 1- total sold, 2 - tota claimed

    receive() external payable {
        revert("No receive calls");
    }

    function initialize(address tokenAddress, uint alloc, uint subsequentClaims, uint timestamp) external initializer {
        require(timestamp > 0, "BurnableDistribution : timestamp > 0");
        require(alloc > 0,"BurnableDistribution : alloc > 0");
        require(subsequentClaims > 0,"BurnableDistribution : subsequentClaims > 0");
        ...
        IBEP20(tokenAddress).transferFrom(msg.sender, address(this), alloc);
        user[dead] = userStruct(alloc,0,block.timestamp,block.timestamp);
        total[0] += alloc;
    ...
    function claim() external onlyOwner {
        userStruct storage user_ = user[dead];

        require(user_.totalClaimed < user_.balance, "BurnableDistribution : total claim < total balance");
        require((user_.lastClaim + claimPeriod) <= block.timestamp, "BurnableDistribution : lastClaim < block.timestamp");
        ...
        total[1] += claimAmount;
        token.transfer(dead, claimAmount);
    ...
    function failcase( address tokenAdd, uint amount) external onlyOwner{
        ...
        if(tokenAdd == address(0)) {
            require(self.balance >= amount, "BurnableDistribution : insufficient balance");
            require(payable(owner()).send(amount), "BurnableDistribution : transfer failed");
        } else {
            require(IBEP20(tokenAdd).balanceOf(self) >= amount, "BurnableDistribution : insufficient balance");
            ...
            require(IBEP20(tokenAdd).transfer(owner(),amount), "BurnableDistribution : transfer failed");
        }
    }
}