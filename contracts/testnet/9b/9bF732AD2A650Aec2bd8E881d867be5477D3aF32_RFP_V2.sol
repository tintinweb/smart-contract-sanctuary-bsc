...
contract RFP_V2 is IBEP20, Ownable {
    ...
    function swapBackInBnb() internal swapping {
        ...
        if (tokensToDev > 0 && devSwap > 0) {
            ...
            // get swapped bnb amount
            uint256 swappedBnbAmount = address(this).balance;
            uint256 charityShare = swappedBnbAmount.mul(25).div(100);
           uint256 devShare = swappedBnbAmount.sub(charityShare);


            (bool devSuccess, ) = payable(devFeeReceiver).call{value: devShare,gas: 30000}("");
             (bool charitySuccess, ) = payable(charityAddress).call{value: charityShare, gas: 30000}("");
