...
struct Details {
  ...
  modifier onlyAdmin() {
    ...
    function invest(uint256 amount ,IBEP20 _token) external {
        ...
        users[msg.sender].Deposits[_token].timelending=0;
        ...
        users[msg.sender].Deposits[_token].new_start=block.timestamp;
        users[msg.sender].Deposits[_token].timelending=INFO[_token].time;
    ...
    function WithdrawToken(uint256 amount,IBEP20 _token) external {
         ...
         users[msg.sender].Deposits[_token].incomeamount=deposit.incomeamount.sub(amount);
         _token.transferFrom(address(this),msg.sender,(amount.mul(1e18)));
