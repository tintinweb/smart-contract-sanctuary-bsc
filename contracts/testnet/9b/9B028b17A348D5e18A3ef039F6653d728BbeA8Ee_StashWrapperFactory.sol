...
abstract contract WrapperFactory721 is WrapperFactoryShared {
  ...
  // TODO comment cleanup
  /**
   * @notice Wrap a specific NFT tokenId to enable rentals and grant approval for all on the wrapped contract for the
   ...
   * exists (unless the wrapped NFT implementation was since updated).
   */
  // TODO: for security this approval must be approved for all vs single token
...
abstract contract WrapperFactory1155 is WrapperFactoryShared {
  ...
  /**
   * @notice Checks the NFT type and deploys a new wrapped NFT contract if it does not already exist for the current
