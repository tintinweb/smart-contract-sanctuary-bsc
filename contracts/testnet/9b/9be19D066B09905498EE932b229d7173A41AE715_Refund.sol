...
contract Refund is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function isWhitelist(address _holder) public view returns (bool) {
        return contains(holders, _holder);
    }

    function getReceivedAmount(address _holder) public view returns (uint256) {
        return receivedAmount[_holder];
    }

    function getHolder(address _holderAddress) public view returns (uint256, uint256) {
    ...
    function refund(address _holderAddress) external nonReentrant onlyWhitelist {
        require(_holderAddress != address(0), "Refund: Holder address can't include address 0");
        Holder storage holderStruct = holderList[_holderAddress];

        require(IERC20Upgradeable(tokenAddr).balanceOf(_holderAddress) >= holderStruct.baseNVEAmount, "Refund: Your NVE balance is different with snapshot on 17/10/2022.");
        require(IERC20Upgradeable(tokenAddr).transferFrom(_msgSender(), address(this), holderStruct.baseNVEAmount), "Refund: Transfer NVE to contract failed.");
        ...
        require(IERC20Upgradeable(refundTokenAddr).transferFrom(address(this), _holderAddress, holderStruct.refundAmount), "Refund: Refund to holder failed.");
        receivedAmount[_holderAddress] = holderStruct.refundAmount;
    }
    ...
    function updateHolderList(address _holderAddress, uint256 _refundAmount, uint256 _baseNVEAmount, bool isRemove) public onlyOwner {
        ...
        if (contains(holders, _holderAddress)) {
