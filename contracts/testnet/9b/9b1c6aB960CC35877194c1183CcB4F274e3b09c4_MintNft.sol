...
contract Owner {
    ...
    mapping(uint8 =>mapping(uint8 => EachNFT)) public AllNfts;
    ...
    function addNft(
        ...
        tokenID++;
        AllNfts[_level][_nftPart] = EachNFT(_level,_nftPart,tokenID, amount, _metaData);
        segmentCount[_level]++;
        tokenMap[_level][_nftPart]=uint8(tokenID);
    }
    ...
    function changeSegmentPrice(bool _increase,uint8 _level , uint8 _percent) external  onlyOwner {
        uint8 count=segmentCount[_level];
        for (uint8 i = 1; i <= count; i++) {
            EachNFT storage currentItem = AllNfts[_level][i];
            
                uint256 finalAmount = AllNfts[_level][i].amount * _percent;
...
contract MintNft is ERC1155URIStorage, ReentrancyGuard {
    ...
    function mintNFT(
        ...
        uint8 level = _level;
        uint8 nftPart = _nftType;
        uint256 amount = _amount;
        uint8 howMany = _howMany;
        uint8 s_type = _type;
        address nftOwner = _nftOwner;
        string memory metaData = AllNfts[level][nftPart].metaData;
        uint256 finalAmount = AllNfts[level][nftPart].amount * howMany;
        ...
        if (s_type == 0) {
            ...
            uint256 bnbPrice = toBnbPrice(level, nftPart, howMany);
            require(msg.value >= bnbPrice, "this is not nft price");
            (bool sent, ) = ownerAddress.call{value: msg.value}("");
            require(sent, "Failed to send Ether");
            nftOwner = msg.sender;
        }
        uint8 ID = tokenMap[level][nftPart];
        _mint(nftOwner, ID, _howMany, "");
        TxID++;
    ...
    function toBnbPrice(
        ...
        uint256 nftPrice = AllNfts[_level][_nftPart].amount * 10**18;
    ...
    function updateAllTokenURI(
        uint8 _level,
        uint8 _nftPart,
        string memory _tokenURI
    ) external onlyOwner {
        uint8 ID = tokenMap[_level][_nftPart];
        _setURI(ID, _tokenURI);
        AllNfts[_level][_nftPart].metaData = _tokenURI;
        for (uint256 i = 1; i < TxID; i++) {
            if (transactions[i].level == _level) {
    ...
    function getNFT(uint8 _level, uint8 _nftPart)
        public
        view
        returns (EachNFT memory)
    {
        return AllNfts[_level][_nftPart];
    }
