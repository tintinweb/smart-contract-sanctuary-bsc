...
contract ArkarusTestFee7 is Context, IBEP20, Ownable {
  ...
  uint256 public maxAmountPerTime = 100000 * 10**18; // default 100k
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) override external returns (bool) {
    if(!(recipient == pancakeRouterAddress || recipient == creatorAddress)) {
      require(amount <= maxAmountPerTime, "Exceed limit per time");
    }

    if(_msgSender() == pancakeLPAddress) {
      if(buyFee > 0){
        _transfer(_msgSender(), feeReceiveAddress, amount * buyFee / 100);
      ...
      if(block.timestamp - _buyTime[_msgSender()] < 1 hours) {
        if(_msgSender() == depositWithdrawAddress || recipient == depositWithdrawAddress) {
          _transfer(_msgSender(), recipient, amount);
        }
        else {
          _transfer(_msgSender(), feeReceiveAddress, amount * _transferTax / 100);
  ...
  function transferFrom(address sender, address recipient, uint256 amount) override external returns (bool) {
    ...
    if(recipient == pancakeLPAddress && _fee > 0) {
      _transfer(sender, feeReceiveAddress, amount * _fee / 100);
  ...
  function setDepositWithdrawAddress(address _depositWithdrawAddress) external onlyOwner {
    depositWithdrawAddress = _depositWithdrawAddress;
  }

  function setFeeReceiveAddress(address _feeReceiveAddress) external onlyOwner {
