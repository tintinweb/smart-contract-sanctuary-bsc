...
contract SwellToken is Token {
    ...
    uint public MIN_TOKEN_STAKE_AMT = 5;
    uint public MIN_UNSTAKE_DAYS = 7;
    ...
    function unStakeToken() public {
        ...
        // need to check for MIN_UNSTAKE_DAY (7, etc.) has passed since last stake
        //require(now > MIN_UNSTAKE_DAYS > user.sT.lastStakeTime.add(TIME_TO_UNSTAKE).days, "Must wait until max unstake days")
        updateStakeToken_IP(msg.sender);
...
// SET Functions
    ...
    function SET_MIN_UNSTAKE_DAYS(uint256 value) external {
        require(msg.sender == ADMIN, "Admin use only");
        require(value >= MIN_UNSTAKE_DAYS);
        MIN_UNSTAKE_DAYS = value;
    }

    function SET_MIN_TOKEN_STAKE_AMOUNT(uint256 value) external {
        require(msg.sender == ADMIN, "Admin use only");
        require(value >=  MIN_TOKEN_STAKE_AMT);
        MIN_TOKEN_STAKE_AMT = value;
    }
