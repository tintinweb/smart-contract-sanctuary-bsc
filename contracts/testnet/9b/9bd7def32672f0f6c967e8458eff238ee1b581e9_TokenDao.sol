...
contract TokenDao is Context, IBEP20, Ownable, PancakeTool {
    ...
    constructor(
        ...
        // 各种费率
        _cPercent = 500;   // 转账买卖添加移除扣费5%
        rewardRate = 1000;   // 每次只分30%
        mustRemain = 10;    // 账户必须保留 0.1%
        managerFee = 2000;    // 基金费率 5%
        lotteryFee = 100;    // 抽奖费率
        ...
        tradeLockInterval = 10; // 卖出间隔 600 为600秒 或者10分钟
        ...
        lockRe.lockInterval = 7200;  // 交易对限制移除的时长 ， 从合约部署开始计算     
        lockRe.openInterval = 10;  // 可以移除的时长
    ...
    function _tokenReward(address _user) internal returns (bool) {
        ...
        uint256 reward = _balances[address(this)].mul(rewardRate).div(divBase);
        bool isLock = false;
        for (
            uint256 lockIndex = 0;
