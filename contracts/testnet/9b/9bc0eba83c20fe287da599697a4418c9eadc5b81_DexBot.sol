...
contract DexBot is Initializable {
    ...
    function stake(uint256 amount, uint256 _pid) public {
        ...
        if (block.timestamp > userstake.lastWithdrawRewards && userstake.amount > 0) {
    ...
    function withdrawStakeRewards(uint8 _pid, bool _unStake) public {
        ...
        uint256 perMonthsec = 86400 * pool.duration;
        if(!pool.isMine) {
            // require(block.timestamp > userstake.lastStaked + withdrawMinLimitStaking, "Still withdraw locked.");
            ...
            if(multiplier > 0) {
                userstake.stakeReward +=  (userstake.amount * multiplier * pool.rewardPercent) / (perMonthsec * 100000);
                userPending = ((userstake.stakeReward * referalEarnings[0]) /100);
                companyPending =(userstake.stakeReward * referalEarnings[10]) /100;
                distributeEarnings(parent.parent, 1, _pid);
                userstake.stakeReward = 0;
            }
            
        } else {
            // require(block.timestamp > userstake.lastStaked + withdrawMinLimit,"Still withdraw locked.");
            multiplier = getMultiplier(
                userstake.lastWithdrawRewards,
                block.timestamp
            );
            ...
            if(multiplier > 0) {
                userstake.mineReward += (userstake.amount * multiplier * pool.rewardPercent) / (perMonthsec * 100000);
                userPending = ((userstake.mineReward * referalEarnings[0]) / 100);
                companyPending =(userstake.mineReward * referalEarnings[10]) /100;
                distributeEarnings(parent.parent, 1, _pid);
    ...
    function getrewardAmount(uint8 _pid, address userAddr) public view returns (uint256) {
        StakePoolInfo storage pool = poolInfo[_pid];
        ...
        if(!pool.isMine) {
            ...
            if(multiplier > 0) {
                uint256 reward =  userstake.stakeReward + (userstake.amount * multiplier * pool.rewardPercent) / (perMonthsec * 100000);
            ...
            if(multiplier > 0) {
                uint256 reward =   userstake.mineReward + (userstake.amount * multiplier * pool.rewardPercent) / (perMonthsec * 100000);
                userPending = ((reward * referalEarnings[0]) / 100);
            }
        }
        return userPending;
    ...
    function withdrawReferalReward() public {
        UserInfo storage parent = userData[msg.sender];
        uint256 totalRefrerRewards = parent.totalRefrerRewards + parent.gainBusd;
