...
contract CEEKLandHash is ReentrancyGuard {
   ...
   function encode2(string memory ceekUserIdWithTs) public view returns(bytes32[] memory) {
       ...
       bytes memory ceekUserArr = bytes(ceekUserIdWithTs);
   ...
   function encode22(string memory ceekUserIdWithTs) public view returns(uint256,uint256) {
       ...
       uint256 resultIdxMax = bytes(ceekUserIdWithTs).length / 32;
       if (bytes(ceekUserIdWithTs).length % 32 != 0) {
           resultIdxMax++;
       }

        return (bytes(ceekUserIdWithTs).length,resultIdxMax);
