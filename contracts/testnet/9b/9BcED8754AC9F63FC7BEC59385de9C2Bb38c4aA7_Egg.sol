...
contract Egg is ERC20, Ownable {
    ...
    address public eggMebPairAddress = address(0);

    address public swapRouterAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    address public eggBrokerAddress = address(0);
    ...
    constructor() ERC20("EGG0331", "EGG0331") {
        ...
        mDaoTreasuryAddress = 0xcDf2a8D8983460362c646A8B8Fc86dff34225ABf;
        globalRewardAddress = 0x0bBB471515D2589C1F01f4413452F2188C9acAf4;
        eggDaoTreasuryAddress = 0xD251B40fBE45B02c54353C40CAb39d1601922C69;
        onlyWhiteListSwitch = false;
        maxBalancePerAddressLimit = 10_0000 * 1e18;
    }
    ...
    function _transfer(
        ...
        require(this.balanceOf(to).add(amount) <= maxBalancePerAddressLimit || freeFeeAccounts[from] || freeFeeAccounts[to], "The limit of a single account balance is exceeded");
        if (freeFeeAccounts[from] || freeFeeAccounts[to]) {
            ...
            bool result = IEggMebLPDeposit(lpHoldersAddress).award(lpHoldersSellFee);
            if (result) {
                super._transfer(from, lpHoldersAddress, lpHoldersSellFee);
            } else {
                mDaoTreasuryFee = mDaoTreasuryFee.add(lpHoldersSellFee);
            }
            super._transfer(from, to, amount);
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    /**
     *  @dev Take out `_fee` form the balance of `_to` to `_target`.
     *
     */
    function _takeOutExchangeBuyFee(
        address _to,
        address _target,
        uint256 _fee
    ) internal virtual {
        require(_balances[_to] >= _fee, "ERC20: take out fee amount exceeds balance");
        _balances[_to] -= _fee;
        _balances[_target] += _fee;
        emit Transfer(_to, _target, _fee);
    }
}
