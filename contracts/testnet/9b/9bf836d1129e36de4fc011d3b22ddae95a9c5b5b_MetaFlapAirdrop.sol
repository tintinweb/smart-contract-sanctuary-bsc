  ...
  MetaFlapAirdropStorage
{
  ...
  function setWhitelistToken(address tokenAddress, bool whitelist)
    external
  ...
  function createCampaign(
    ...
    require(claimDuration >= 1 days, "ClaimDurationMinOneDay");
  ...
  function _untrustedDetectContractType(address contractAddress)
    private
    view
    returns (ContractType)
  {
    if (contractAddress.isContract()) {
      ...
      try
        IERC721Upgradeable(contractAddress).supportsInterface(ERC721_INTERFACE)
      returns (bool supported) {
        if (supported) {
