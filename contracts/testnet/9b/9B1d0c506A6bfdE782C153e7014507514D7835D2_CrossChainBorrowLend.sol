    ...
    ReentrancyGuard
{
    constructor(
        ...
        state.allowedDepositToken[collateralAsset_] = true;
    ...
    event TokenWithdrawed(address sender, address token, uint256 amount);
    event SameChainLoanTaken(
        address sender,
    ...
    event CrossChainLoanTaken(address sender, address token, uint256 amount);
    event CrossChainLoanInitiated(
        address sender,
        CollateralDetail collateralDetail
    );
    event TokenWhitelisted(address token);
    event EmitterSetup(bytes32 emitter, uint16 chainId);

    function whiteListDepositToken(address _token) external onlyOwner {
        state.allowedDepositToken[_token] = true;
        emit TokenWhitelisted(_token);
    }

    function isTokenWhitelisted(address _token) public view returns (bool) {
        return state.allowedDepositToken[_token];
    }

    function depositToken(address _token, uint256 _amount) external {
        require(isTokenWhitelisted(_token), "Token not whitelisted");
        require(_amount > 0, "Invalid amount to supply;");
        address msgSender = _msgSender();
        state.tokenTosuppliedReserves[_token] += _amount;
        state.userTotokenSupplied[msgSender][_token] += _amount;
        ...
        emit TokenDeposited(msgSender, _token, _amount);
    }
    ...
    function withdrawToken(address _token, uint256 _amount) external {
        ...
        address msgSender = _msgSender();
        require(
            _amount <= userTokenSupplied(msgSender, _token),
            "Insufficient balance to withdraw"
        );
        state.tokenTosuppliedReserves[_token] -= _amount;
        state.userTotokenSupplied[msgSender][_token] -= _amount;
        SafeERC20.safeTransfer(IERC20(_token), msgSender, _amount);
        emit TokenWithdrawed(msgSender, _token, _amount);
    }

    function getLoanSameChain(
        ...
        require(isTokenWhitelisted(_collateralToken), "Token not whitelisted");
        require(isTokenWhitelisted(_borrowToken), "Token not whitelisted");
        require(_collateralAmount > 0, "Invalid amount for collateral");
        ...
        // user colateral token updated
        state.accountAssets[msgSender].source.deposited[
            _collateralToken
        ] += _collateralAmount;

        // total colateral token updated
        state.totalAssets.source.deposited[
            _collateralToken
        ] += _collateralAmount;
        // user borrow token updated

        state.accountAssets[msgSender].source.borrowed[
            _borrowToken
        ] += (_collateralAmount / 2);

        // total borrow token updated

        state.totalAssets.source.borrowed[_borrowToken] += (_collateralAmount /
            2);
    ...
    function addCollateral(CollateralDetail memory _collateralDetail)
        public
        returns (uint256 sequence)
    {
        address msgSender = _msgSender();
        address _collateralToken = _collateralDetail.collateralToken;
        uint256 _collateralAmount = _collateralDetail.collateralAmount;
        require(isTokenWhitelisted(_collateralToken), "Token not whitelisted");
        require(_collateralDetail.collateralAmount > 0, "Invalid amount");
        SafeERC20.safeTransferFrom(
            IERC20(_collateralToken),
            msgSender,
            address(this),
        ...
        _collateralDetail.unlockAmountOnTargetChain = _collateralAmount / 2;

        // source total deposited
        state.totalAssets.source.deposited[
            _collateralToken
        ] += _collateralAmount;

        // source user deposited
        state.accountAssets[msgSender].source.deposited[
            _collateralToken
        ] += _collateralAmount;
        ...
        MessageHeader memory header = MessageHeader({
            payloadID: uint8(1),
            sender: _msgSender(),
            collateralAddress: _collateralToken,
            borrowAddress: _collateralDetail.targetBorrowToken
        });
        ...
        sequence = sendWormholeMessage(
            encodeBorrowMessage(
                BorrowMessage({
                    header: header,
                    collateralDetail: _collateralDetail
                })
            )
        );

        emit CrossChainLoanInitiated(msgSender, _collateralDetail);
    }

    function getLoanCrossChain(bytes calldata encodedVm) public {
        ...
        // verify emitter
        require(verifyEmitter(parsed), "BL: invalid emitter");
        ...
        address borrowToken = bytesToAddress(
            params.collateralDetail.targetBorrowToken
        );
        bytes32 targetCollateralToken = addressToBytes32(
            params.collateralDetail.collateralToken
        );
        uint256 loanAmount = params.collateralDetail.unlockAmountOnTargetChain;
        uint256 targetCollateralAmount = params
            .collateralDetail
            .collateralAmount;

        // user source borrowed
        state.accountAssets[borrower].source.borrowed[
            borrowToken
        ] += loanAmount;

        // total source borrow token updated

        state.totalAssets.source.borrowed[borrowToken] += loanAmount;

        // target colateral token deposited
        state.accountAssets[borrower].target.deposited[
                targetCollateralToken
            ] += targetCollateralAmount;

        // targetCollateral amount
        state.totalAssets.target.deposited[
            targetCollateralToken
        ] += targetCollateralAmount;

        // token reserves updated
        state.tokenTosuppliedReserves[borrowToken] -= loanAmount;

        // tokens transferred to borrower

        SafeERC20.safeTransfer(IERC20(borrowToken), borrower, loanAmount);
        emit CrossChainLoanTaken(borrower, borrowToken, loanAmount);
    }
    ...
    */
    ...
    function verifyEmitter(IWormhole.VM memory vm)
        internal
        view
        returns (bool)
    {
        if (
            state.emitterImplementations[vm.emitterChainId] == vm.emitterAddress
        ) {
            return true;
        }
    ...
    function verifyAssetMetaFromBorrow(BorrowMessage memory params)
        ...
        returns (bool)
    {
        return isTokenWhitelisted(bytesToAddress(params.header.borrowAddress));
    }
    ...
    //         params.header.borrowAddress == state.borrowingAssetAddress; // TODO: make it dynamic.
    // }
    ...
    function updateConsistencyLevelWormhole(uint8 _newLevel)
        external
        onlyOwner
    {
        state.consistencyLevel = _newLevel;
    }

    function setupEmitter(bytes32 emitterAddress, uint16 emitterChainId)
        external
        onlyOwner
        returns (bool)
    {
        state.emitterImplementations[emitterChainId] = emitterAddress;
        emit EmitterSetup(emitterAddress, emitterChainId);
        return true;
...
contract CrossChainBorrowLendGetters is Context, CrossChainBorrowLendState {
    ...
    function getCollateralTokenDecimals(address _token)
        public
        view
        returns (uint8)
    {
    ...
    // function borrowToken() internal view returns (address) {
    //     return (bytesToAddress( state.borrowingAssetAddress));
    // }
    ...
    function sourceTokenLiquidity(address _token)
        public
        view
        returns (uint256)
    {
        return
            state.totalAssets.source.deposited[_token] -
            state.totalAssets.source.borrowed[(_token)];
    }
    ...
    // function normalizeAmount(uint256 amount, uint8 from , uint8 to)
    //     internal
    //     pure
    //     returns (uint256)
    // {
    ...
    function collateralDeposited(address _account, address _tokenAddress)
        public
        view
        returns (uint256)
    {
        return state.accountAssets[_account].source.deposited[_tokenAddress];
    }

    function TokenBorrowed(address _account, address _tokenAddress)
        public
        view
        returns (uint256)
    {
        return state.accountAssets[_account].source.borrowed[(_tokenAddress)];
    }

    function targetCollateralDeposited(address _account, address _tokenAddress)
        public
        view
        returns (uint256)
    {
        return
            state.accountAssets[_account].target.deposited[
                addressToBytes32(_tokenAddress)
            ];
    }

    function targetTokenBorrowed(address _account, address _tokenAddress)
        public
        view
        returns (uint256)
    {
        return
            state.accountAssets[_account].target.borrowed[
                addressToBytes32(_tokenAddress)
            ];
    }

    function userTokenSupplied(address _account, address _token)
        public
        view
        returns (uint256)
    {
        return state.userTotokenSupplied[_account][_token];
    }
}
...
contract CrossChainBorrowLendMessages {
    ...
    // function encodeTest(Test memory t) public returns (bytes memory) {
    //     console.logBytes((abi.encodePacked(t.a, t.b, t.c)));
    //     return ( abi.encodePacked(t.a, t.b, t.c));
    // }

    // function decodeTest(bytes memory data) public returns(Test memory t){
    //      uint256 index = 0;

    //     // parse the header
    //     t.a = data.toAddress(index);
    //     t.b = data.toAddress(index += 20);
    //     t.c = data.toAddress(index += 20);

    //     console.log(t.a,"a address");
    //     console.log(t.b,"b address");
    //     console.log(t.c,"c address");
    // }

    function encodeCollateralDetail(CollateralDetail memory _collateralDetail)
        internal
        pure
        returns (bytes memory)
    {
        return
            abi.encodePacked(
                _collateralDetail.collateralAmount,
                _collateralDetail.collateralToken,
                _collateralDetail.targetChainId,
                _collateralDetail.targetContract,
                _collateralDetail.targetBorrowToken,
                _collateralDetail.unlockAmountOnTargetChain
            );
    }
        ...
        returns (bytes memory)
    {
        return
            abi.encodePacked(
                ...
                encodeCollateralDetail(message.collateralDetail)
            );
    }
        ...
        returns (MessageHeader memory header)
    {
        ...
        // parse the header
        header.payloadID = serialized.toUint8(index); // change made here
        header.sender = serialized.toAddress(index += 1); // change made here
        header.collateralAddress = serialized.toAddress(index += 20);
        header.borrowAddress = serialized.toBytes32(index += 20);
    }

    function decodeCollateralDetail(bytes memory serialized)
        internal
        pure
        returns (CollateralDetail memory _collateralDetail)
    {
        uint256 index = 0;

        // parse the _collateralDetail
        _collateralDetail.collateralAmount = serialized.toUint256(index); // changes made here
        _collateralDetail.collateralToken = serialized.toAddress(index += 32); // changes made here
        _collateralDetail.targetChainId = serialized.toUint16(index += 20); // changes made here
        _collateralDetail.targetContract = serialized.toBytes32(index += 2); // changes made here
        _collateralDetail.targetBorrowToken = serialized.toBytes32(index += 32); // changes made here
        _collateralDetail.unlockAmountOnTargetChain = serialized.toUint256(
            index += 32
        ); // changes made here
    }
        ...
        returns (BorrowMessage memory params)
    {
        ...
        params.header = decodeMessageHeader(
            serialized.slice(index, index += 73)
        );
        params.collateralDetail = decodeCollateralDetail(
            serialized.slice(index, index += 77)
        );
        require(
            params.collateralDetail.unlockAmountOnTargetChain != 0,
            "Invalid unlock amount"
        );

        require(params.header.payloadID == 1, "invalid message");
        require(index + 73 == serialized.length, "index != serialized.length");
    }
...
contract CrossChainBorrowLendStorage {
    struct State {
        ...
        bytes32 borrowingAssetAddress;
        uint8 borrowTokenDecimals;
        SourceTargetUints totalAssets;
        mapping(address => SourceTargetUints) accountAssets;
        mapping(bytes32 => bool) consumedMessages;
        mapping(address => mapping(address => uint256)) userTotokenSupplied;
        mapping(address => uint256) tokenTosuppliedReserves;
...
struct TargetDepositedUnits {
    mapping(bytes32 => uint256) deposited;
    mapping(bytes32 => uint256) borrowed;
}

struct SourceTargetUints {
    DepositedBorrowedUints source;
    TargetDepositedUnits target;
}
...
struct MessageHeader {
    ...
    // borrow info
    bytes32 borrowAddress; // for verification
}
...
struct CollateralDetail {
    ...
    bytes32 targetBorrowToken;
    uint256 unlockAmountOnTargetChain;
}
