...
contract OmniDragons is ERC721, NonblockingReceiver, ILayerZeroUserApplicationConfig {
    ...
    uint256 public immutable maxMintPerTransaction = 2;
    ...
    function mint(uint256 numberMintOfOmniDragons) external payable{
        require(numberMintOfOmniDragons > 0 && numberMintOfOmniDragons <= maxMintPerTransaction, "Can only mint one or 5 at a time");
        require(nextTokenId + numberMintOfOmniDragons <= maxMint, "OMNI_DRAGONS_MAX_SUPPLY_FOR_CHAIN");
        require(numberMintOfOmniDragons <= maxPerTx && balanceOf(msg.sender) < maxPerTx, "OMNI_DRAGONS_MAX_2_OMNI_DRAGONS_PER_WALLET");
        unchecked {
            for(uint256 i = 0; i < numberMintOfOmniDragons; i++){
                _safeMint(msg.sender, nextTokenId++);
            }
        }
    }

    function reserveDragons() public onlyOwner {
        unchecked {
