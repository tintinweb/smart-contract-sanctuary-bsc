...
contract Mining is Ownable, ReentrancyGuard {
    ...
    event TeamEarnTake(uint256 turn, uint256 issue, address teamEarn, uint256 earnAmount, uint256 priceEte, uint256 totalUsdt); // team earn.
    ...
    function userDeposit(uint256 tokenAmount) public nonReentrant {
        ...
        if(_nowIssueMsg.userCount == 0 && _nowIssueMsg.gloryCardCount == 0 &&  _nowIssueMsg.starCardCount == 0) {
            ...
            if(_nowIssue > 1) {
                ...
                emit TeamEarnTake(turn, _lastIssue, teamEarn, _v0, _price, _IssueMsg[turn][_lastIssue].totalUsdt);
    ...
    function userTake(bytes32 order) public nonReentrant {
        ...
        uint256 _earnMy = _valueEte.mul(3).mul(_ratioMy).div(10000);               // my earin.
