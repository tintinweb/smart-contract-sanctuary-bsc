...
contract SafeswapRouterNew is ISafeswapRouter02 {
        ...
        returns (uint[] memory amounts)
    {
        ...
        if(nameToInfo[path[0]].exists == true && (nameToInfo[path[0]].actualSell != nameToInfo[path[0]].expectedSell)) {
            // add convertion equation, send difference tokens to address
            uint beforDeduction = amountIn;
            amountIn = nameToInfo[path[0]].expectedSell / nameToInfo[path[0]].actualSell;
            TransferHelper.safeTransferFrom(
            path[0], msg.sender, nameToInfo[path[0]].feesAddress, beforDeduction.sub(amountIn) 
            );
        }
