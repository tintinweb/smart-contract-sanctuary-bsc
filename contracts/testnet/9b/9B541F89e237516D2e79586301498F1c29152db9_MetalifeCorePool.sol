...
import "./IPool.sol";
import "./MetalifePoolFactory.sol";

/**
 * @title Metalife Pool Base
 *
 * @notice An abstract contract containing common logic for any pool,
 *      be it a flash pool (temporary pool like SNX) or a core pool (permanent pool like MLC/ETH or MLC pool)
 *
 * @dev Deployment and initialization.
 *      Any pool deployed must be bound to the deployed pool factory (MetalifePoolFactory)
 *      Additionally, 3 token instance addresses must be defined on deployment:
 *          - MLC token address
 *          - pool token address, it can be MLC token address, MLC/ETH pair address, and others
 *
 ...
 * @dev The weight is logically 10% for MLC pool and 90% for MLC/ETH pool.
 ...
 * @dev For MLC Pool we use 100 as weight and for MLC/ETH pool - 900.
  */
abstract contract MetalifePoolBase is IPool, MetalifeAware, ReentrancyGuard {
    ...
    /// @dev Link to the pool factory MetalifePoolFactory instance
    MetalifePoolFactory public immutable factory;

    /// @dev Link to the pool token instance, for example MLC or MLC/ETH pair
    address public immutable override poolToken;

    /// @dev Pool weight, 100 for MLC pool or 900 for MLC/ETH
    ...
    constructor(
        ...
        // verify the inputs are set
        require(address(_factory) != address(0), "MLC Pool fct address not set");
    ...
    function pendingYieldRewards(address _staker) external view override returns (uint256) {
        ...
        if (blockNumber() > lastYieldDistribution && usersLockingWeight != 0) {
            ...
            uint256 mlcRewards = (multiplier * weight * factory.mlcPerBlock()) / factory.totalWeight();

            // recalculated value for `yieldRewardsPerWeight`
            newYieldRewardsPerWeight = rewardToWeight(mlcRewards, usersLockingWeight) + yieldRewardsPerWeight;
        } else {
     ...
     *      updates factory state via `updateMLCPerBlock`
     */
    function _sync() internal virtual {
        // update MLC per block value in factory if required
        if (factory.shouldUpdateRatio()) {
            factory.updateMLCPerBlock();
        }
        ...
        uint256 mlcPerBlock = factory.mlcPerBlock();

        // calculate the reward
        uint256 mlcReward = (blocksPassed * mlcPerBlock * weight) / factory.totalWeight();

        // update rewards per weight and `lastYieldDistribution`
        yieldRewardsPerWeight += rewardToWeight(mlcReward, usersLockingWeight);
     ...
     *      MLC reward value, applying the 10^12 division on weight
     *
     * @param _weight stake weight
     * @param rewardPerWeight MLC reward per weight
    ...
    /**
     * @dev Converts reward MLC value to stake weight (not to be mixed with the pool weight),
     *      applying the 10^12 multiplication on the reward
     *      - OR -
     * @dev Converts reward MLC value to reward/weight if stake weight is supplied as second
