...
import "./interfaces/IDefiChainLottery.sol";

/** @title DefiChain Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract DefiChainLottery is ReentrancyGuard, IDefiChainLottery, Ownable {
    ...
    uint256 public maxPriceTicketInDfi = 50 ether;
    uint256 public minPriceTicketInDfi = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInDfi;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] dfiPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInDfi;
        uint32 finalNumber;
    }
     ...
     * @param _dfiTokenAddress: address of the DFI token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _dfiTokenAddress, address _randomGeneratorAddress) {
        ...
        nonReentrant
    {
        ...
        // Calculate number of DFI to this contract
        uint256 amountDFIToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInDfi,
            _ticketNumbers.length
        );

        // Transfer DFI tokens to this contract
        dfiToken.safeTransferFrom(address(msg.sender), address(this), amountDFIToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInDfi += amountDFIToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInDFIToTransfer
        uint256 rewardInDFIToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInDFIToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        dfiToken.safeTransfer(msg.sender, rewardInDFIToTransfer);

        emit TicketsClaim(msg.sender, rewardInDFIToTransfer, _lotteryId, _ticketIds.length);
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in DFI per group, and make lottery claimable
     * @param _lotteryId: lottery id
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInDfi) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in DFI for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].dfiPerBracket[j] =
                ...
                // A. No DFI to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].dfiPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInDfi - amountToShareToWinners);

        // Transfer DFI to treasury address
        dfiToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in DFI token
     * @dev Callable by owner or injector address
     */
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        require(_lotteries[_lotteryId].status == Status.Open, "Lottery not open");

        dfiToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInDfi += _amount;
     ...
     * @param _priceTicketInDfi: price of a ticket in DFI
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInDfi >= minPriceTicketInDfi) && (_priceTicketInDfi <= maxPriceTicketInDfi),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInDfi: _priceTicketInDfi,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            dfiPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInDfi: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(dfiToken), "Cannot be DFI token");
    ...
    /**
     * @notice Set DFI price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInDfi: minimum price of a ticket in DFI
     * @param _maxPriceTicketInDfi: maximum price of a ticket in DFI
     */
    function setMinAndMaxTicketPriceInDFI(uint256 _minPriceTicketInDfi, uint256 _maxPriceTicketInDfi)
        external
        onlyOwner
    {
        require(_minPriceTicketInDfi <= _maxPriceTicketInDfi, "minPrice must be < maxPrice");

        minPriceTicketInDfi = _minPriceTicketInDfi;
        maxPriceTicketInDfi = _maxPriceTicketInDfi;
    }
     ...
     * @param _priceTicket price of a ticket (in DFI)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].dfiPerBracket[_bracket];
        } else {
            return 0;
        }
    }
