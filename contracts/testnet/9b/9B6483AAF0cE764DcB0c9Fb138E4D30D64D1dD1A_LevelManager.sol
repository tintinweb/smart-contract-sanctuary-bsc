...
contract LevelManager is Adminable, ILevelManager, WithLevels, WithPools {
    ...
    function getUserUnlockTime(address account) public view returns (uint256) {
        if (!lockEnabled) {
            return 0;
        }
        
        return
        userState[account].getNextUnlockTime(
            pools,
            poolLockPeriod,
            poolEnabled
        );
...
library RegistrationLibrary {
    ...
    function getNextUnlockTime(
        ...
        mapping(address => uint256) storage lockPeriods,
        mapping(address => bool) storage poolsEnabled
    ) public view returns (uint256) {
        uint256 nextTime;
        for (uint256 i; i < pools.length; i++) {
            address addr = pools[i];
            if (self.locks[addr] == 0) {
                continue;
            }
            uint256 time = self.locks[addr] + lockPeriods[addr];
