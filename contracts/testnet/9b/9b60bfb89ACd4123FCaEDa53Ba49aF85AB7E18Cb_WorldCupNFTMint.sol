...
contract WorldCupNFTMint is Ownable {
	struct Meta {
		bool isOpen;
		uint totalMint;
		INFT nft;
	...
	function setOpen(bool open_) public onlyOwner {
		meta.isOpen = open_;
	}
	...
	function editWhiteList(
		...
		require(accounts_.length == cardIds_.length, "length not match");
		for (uint i; i < accounts_.length; i++) {
			userInfo[accounts_[i]].isW = isAdd_;
			userInfo[accounts_[i]].cardId = cardIds_[i];
		}
	}
	...
	function freeMint() public {
		...
		require(!uInfo.minted, "already mint");

		uInfo.minted = true;

		meta.nft.mint(msg.sender, uInfo.cardId);
	}
}
