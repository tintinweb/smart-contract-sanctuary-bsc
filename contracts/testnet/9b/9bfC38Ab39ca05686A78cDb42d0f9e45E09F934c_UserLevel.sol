...
contract UserLevel is Ownable, IUserLevel {
    ...
    function configBonus(address contractAddress, uint[] memory _bonus, uint[] memory _level) external onlyOwner override{
        ...
        for(uint i = 0; i < _level.length; i++) {
            ...
            if(_info.level.length > i){
                _info.level[i] = _level[i];
                _info.bonus[i] = _bonus[i];
            }else{
        ...
        console.log("Level: ",_info.level.length);
        console.log("Bonus: ",_info.bonus.length);
    ...
    function getListLevelBonus(address _contract) public onlyOwner view returns(uint[] memory level, uint[] memory bonus){
