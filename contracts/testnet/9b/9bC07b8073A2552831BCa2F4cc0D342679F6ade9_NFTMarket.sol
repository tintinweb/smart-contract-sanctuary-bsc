...
abstract contract NFTMarketReserveAuction is NFTMarketFees {
    ...
    mapping(uint => ReserveAuction) private auctionIdToAuction;

    uint private _minPercentIncrementInBasisPoints; // The minimum percent increment a bid must be from last bid
    uint private _defaultDuration; // If a duration is not supplied when creating an auction it will simply default to this duration
    uint private _minDuration; // Minimum duration an auction can be held
    uint private _extensionDuration; // The duration which each bid extends the auction
    uint private constant MAX_DURATION = 1000 days; // Max duration an auction can be held
    ...
    event ReserveAuctionDefaultDurationUpdated(uint reserveAuctionDuration);
    event ListingLimitUpdated(uint8 listingLimit);
    event MinDurationUpdated(uint minDuration);
        ...
        onlyInitializing
    {
        ...
        _defaultDuration = 24 hours;
        _minDuration = 12 hours;
        _extensionDuration = 30 minutes;
    }
    ...
    modifier onlyValidReservePrice(uint reservePrice) {
        require(
            reservePrice >= _minPrice,
            "Market: Reserve price must be greater than minimum price"
        );
        _;
    }
     ...
     */
    function getReserveAuction(uint auctionId)
        public
        view
     ...
     */
    function getReserveAuctionIdForNft(address nftContract, uint tokenId)
        public
        view
        returns (uint)
    {
     ...
     */
    function getSellerFor(address nftContract, uint tokenId)
        public
        view
        returns (address)
    {
    ...
    function getReserveAuctionConfig()
        ...
        returns (
            uint minPercentIncrementInBasisPoints,
            uint reserveAuctionDefaultDuration,
            uint8 listingLimit
        )
    {
        minPercentIncrementInBasisPoints = _minPercentIncrementInBasisPoints;
        reserveAuctionDefaultDuration = _defaultDuration;
     ...
     */
    function updateMinPercentIncrement(uint minPercentIncrementInBasisPoints)
        public
        onlyAdmin
        onlyValidMinPercentIncrement(minPercentIncrementInBasisPoints)
    {
     ...
     */
    function updateReserveAuctionDefaultDuration(
        uint reserveAuctionDefaultDuration
    ) public onlyAdmin {
        require(
            reserveAuctionDefaultDuration <= MAX_DURATION,
            "Market: Reserve auction duration must be <= 1000 days"
        );
        require(
            reserveAuctionDefaultDuration >= _extensionDuration,
            "Market: Reserve auction duration must be >= 30 minutes"
        );

        _defaultDuration = reserveAuctionDefaultDuration;

        emit ReserveAuctionDefaultDurationUpdated(
            reserveAuctionDefaultDuration
        );
    }
     ...
     */
    function updateMinDuration(uint minDuration) public onlyAdmin {
        _minDuration = minDuration;

        emit MinDurationUpdated(minDuration);
    }
    ...
    function _createReserveAuction(
        ...
        uint reservePrice,
        uint duration
    ) internal onlyValidReservePrice(reservePrice) {
        require(
            duration > _minDuration,
            "Market: Auction must be longer than min duration"
        );
        require(
     ...
     * @notice Creates an auctions for the given NFT contract and token IDs.
     * The NFTs are held in escrow until the auctions are finalized or canceled.
     * Uses the default duration as duration.
     */
     ...
     */
    function updateReserveAuction(uint auctionId, uint reservePrice)
        public
     ...
     */
    function _getMinBidAmount(uint currentBidAmount)
        private
        view
        returns (uint)
    {
...
abstract contract NFTMarketListing is NFTMarketFees {
    ...
    mapping(address => mapping(uint => uint))
        private nftContractToTokenIdToListingId;

    uint private _minListingTime; // The minimum time a listing can be up for
    ...
    event ListingCanceled(uint offerId);
    ...
    event MinListingTimeUpdated(uint minListingTime);
    ...
    function _createListing(
        ...
        require(
            endTime >= block.timestamp + _minListingTime,
            "Market: Too short listing time."
        );
        require(
            price >= _minPrice,
            "Market: Listing price must be greater than minimum price"
        );
     ...
     * @dev If an auction is canceled, it will not be returned. However the auction may be over and pending finalization.
     */
    function getListingIdForNft(address nftContract, uint tokenId)
        public
        view
        returns (uint)
    {
     ...
     * @dev Returns the seller that put a given NFT into escrow.
     */
    function getListingSellerFor(address nftContract, uint tokenId)
        public
        view
        returns (address)
    {
    ...
    function updateMinListingTime(uint minListingTime) external onlyAdmin {
        require(
            minListingTime >= 1 minutes,
            "Market: can not be less than 1 minute"
        );
        _minListingTime = minListingTime;
        emit MinListingTimeUpdated(minListingTime);
    }
...
abstract contract NFTMarketFees is Initializable, AccessControlUpgradeable {
    ...
    mapping(address => bool) private nftContractAllowed;
    uint private _secondaryMarketCreatorFeeInBasisPoints;
    ...
    function getIsAllowedContract(address nftContract)
        ...
        returns (bool)
    {
        return nftContractAllowed[nftContract];
    }
    ...
    function getSecondaryMarketCreatorFee()
        public
        view
        returns (uint secondaryMarketCreatorFeeInBasisPoints)
    {
...
abstract contract NFTMarketOffer is NFTMarketFees {
    ...
    uint private _minOfferTime; // The minimum time a offer can be active
    ...
    event MinOfferTimeUpdated(uint minOfferTime);
    ...
    function createOffer(
        ...
        uint price,
        uint endTime
    ) external onlyAllowedContract(nftContract) {
        ...
        require(
            price >= _minPrice,
            "Market: Offer price must be greater than minimum price"
        );
        require(
    ...
    function updateMinOfferTime(uint minOfferTime) external onlyAdmin {
        require(
            minOfferTime >= 1 minutes,
            "Market: can not be less than 1 minute"
        );
        _minOfferTime = minOfferTime;
        emit MinOfferTimeUpdated(minOfferTime);
    }
