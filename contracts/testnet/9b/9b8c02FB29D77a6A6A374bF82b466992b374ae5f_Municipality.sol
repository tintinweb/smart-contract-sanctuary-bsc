...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function grantParcels(uint8 _parcelsAmount, ParcelsMintSignature calldata _mintingSignature,
        uint256 _referrerId, address _user) external onlyOwner {
        require(_parcelsAmount <= 240, "Municipality: The amount of miners should be less than 240");
        _requireOnlyStandardParcels(_mintingSignature.parcels);
        require(ISignatureValidator(signatureValidatorAddress).verifySigner(_mintingSignature), "Municipality: Not authorized signer");
        require(_mintingSignature.parcels.length == _parcelsAmount, "Municipality: Invalid parcels amount");
        ...
        currentlySoldStandardParcelsCount += _parcelsAmount;
        emit BundlePurchased(_user, _parcelsAmount);
    }

    function grantMiners(uint8 _minersAmount, uint256 _referrerId, address _user) external onlyOwner returns(uint256, uint256) {
        require(_minersAmount <= 240, "Municipality: The amount of miners should be less than 240");
        INetGymStreet(netGymStreetAddress).addGymMlm(_user, _referrerId);
        (uint256 firstMinerId, uint256 count) = IMinerPublicBuildingInterface(minerPublicBuildingAddress).mintMinersBundle(_user, _minersAmount);
        emit BundlePurchased(_user, _minersAmount);
        return (firstMinerId, count);
    }
