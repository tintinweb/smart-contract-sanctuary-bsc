...
contract Store is Utils {
   ...
   //获取申请的贷款信息 
    function putCustomerOrder(string _mobiles) constant public returns (string,string,string,uint,uint,uint,uint,uint,uint) {
       bytes32  id = stringToBytes32(_mobiles);  
       return (getVal(Orders[id].mobile),getVal(Orders[id].username),getVal(Orders[id].reason),Orders[id].applyTime,Orders[id].amount,Orders[id].payAmount,Orders[id].commitTime,Orders[id].signTime,Orders[id].payment);
    }  
