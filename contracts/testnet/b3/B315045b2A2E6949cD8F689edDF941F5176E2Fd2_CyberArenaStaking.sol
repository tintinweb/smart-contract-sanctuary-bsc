...
contract CyberArenaStaking is ERC20, Ownable, ReentrancyGuard {
    ...
    */
    function stakedAmount(address _stakerAddress) external view returns (uint256) {
        uint256 totalStakedAmount = 0;
        for (uint256 index = 0; index < stakers[_stakerAddress].length; index++) {
            Stake storage stakeRef = stakers[_stakerAddress][index];
            if(!stakeRef.unstaked){
                totalStakedAmount += stakeRef.amount;
            }
        }
    ...
    function stake(uint128 _amount) external nonReentrant {
        ...
        // require(stakeRef.stakedTimestamp == 0, "CyberArenaStaking: already staked");
            ...
            emit Staked(msg.sender, stakeIndex, _amount, uint48(block.timestamp), penaltyDays, penaltyBP, uint128(totalSupply()), _amount, totalShares, true);
    }
    ...
    function unstake(uint256 _stakeIndex) external nonReentrant {
        ...
        emit Staked(msg.sender, _stakeIndex, stakeRef.amount, uint48(block.timestamp), stakeRef.penaltyDays, stakeRef.penaltyBP, uint128(totalSupply()), stakeRef.amount, totalShares, false);
