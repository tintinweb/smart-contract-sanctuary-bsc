...
contract LootProgress is Ownable{
    ...
    function getUserProgress(address _user) external view returns(uint256 lastActionTimestamp,uint256 dayOfWeek,uint256 weekNumber){
        return _getUserProgress(_user);
    }

    function _getUserProgress(address _user) internal view returns(uint256 lastActionTimestamp,uint256 dayOfWeek,uint256 weekNumber){
        Progress storage p = _progress[_user];
        uint256 _beginningDay = _getDayBegining();

        if(_beginningDay - p.lastActionTimestamp <= 1 days){
            return(p.lastActionTimestamp,p.dayOfWeek,p.weekNumber);
        }
        else if(_beginningDay - p.lastActionTimestamp <= 7 days ){
            return(p.lastActionTimestamp,0,p.weekNumber);
        }
        else if(
                _beginningDay - p.lastActionTimestamp > 7 days &&
                _beginningDay - p.lastActionTimestamp < 14 days
                ){
            return(p.lastActionTimestamp,0,p.weekNumber > 1 ? p.weekNumber - 1 : 1 );
        }
        else if(_beginningDay - p.lastActionTimestamp >= 14 days){
            return(p.lastActionTimestamp,0,0);
        }
