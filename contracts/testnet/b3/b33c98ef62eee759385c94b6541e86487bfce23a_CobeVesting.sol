    ...
    */
    ...
    function addInvestorDetails(Investor[] memory investorArray) public onlyOwner {
        for(uint16 i = 0; i < investorArray.length; i++) {
         if(isremoved){
                ...
                require(!isSameInvestor[investorArray[i].account][investorArray[i].saleType],"already Investor whitelisted");
                isSameInvestor[investorArray[i].account][investorArray[i].saleType]=true;
            }
            ...
            if(saleType == 1) {
                ...
                seedStartDate =investor.starttime;
                seedLockEndDate = seedStartDate +  2 minutes;
                seedVestingEndDate = seedLockEndDate + 300 minutes;
            }
            ...
            if(saleType == 2) {
                ...
                investor.initialAmount = (investor.totalBalance * 5)/(100);
                investor.tokensPerUnit = ((investor.totalBalance) - (investor.initialAmount))/(300);
                privateStartDate =investor.starttime;
            ...
            if(saleType == 3) {
                ...
                // publicStartDate =investor.starttime;
                // publicLockEndDate = publicStartDate + 2  minutes;
                // publicVestingEndDate = publicLockEndDate + 120 minutes; 
            }
    ...
    function withdrawTokens() public   nonReentrant  {
        ...
        activeLockDate=lockEnd[Investors[msg.sender].investorType];
           require(block.timestamp>activeLockDate,"wait until lockData complete");
            ...
            */
            ...
            require(!Investors[msg.sender].isInitialAmountClaimed, "Amount already withdrawn!");
             ...
             uint startDate=startDates[Investors[msg.sender].investorType];
             require(block.timestamp>startDate," wait for start date");
    ...
    // function setDates(uint256 StartDatePublic,bool _isStart) public onlyOwner{
    //     if(Investors[msg.sender].investorType == 1){
    //     seedStartDate =Investors[msg.sender].investorType;
    //     }else if(Investors[msg.sender].investorType == 2){
    //     privateStartDate = Investors[msg.sender].investorType;
    //     }
    //     else{
    //     publicStartDate = StartDatePublic;
    //     }
    //     isStart=_isStart;
    ...
    function getAvailableBalance(address _addr) public view returns(uint256, uint256, uint256){
        ...
        timeDifference = (block.timestamp)-(Investors[_addr].lastVestedTime);}
            ...
            uint balance = Investors[_addr].vestingBalance;
              ...
              if(block.timestamp <seedStartDate) {
                        return(0,0,0);}else{
            ...
            return (Investors[_addr].initialAmount,0,0);}
                   ...
                   if(block.timestamp <publicStartDate) {
                        return(0,0,0);}else{
                    Investors[_addr].initialAmount == 0 ;
                  return (Investors[_addr].initialAmount,0,0);}
            }
