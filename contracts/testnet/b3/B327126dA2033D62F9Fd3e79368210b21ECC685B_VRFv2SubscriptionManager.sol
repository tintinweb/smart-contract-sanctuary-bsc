...
contract VRFv2SubscriptionManager is VRFConsumerBaseV2 , IRandomFactory{
  VRFCoordinatorV2Interface COORDINATOR;
  LinkTokenInterface LINKTOKEN;

  address callerAddress = 0xB80008d5ac49DEc6a5eAEdBAb0e196CAFFa331cD;
  ...
  uint64 public  s_subscriptionId = 650;
  address s_owner;
  IBabuLottery babuLottery;
  uint256 latestLotteryId;
  bool isLotteryEnable = false;
  ...
  function fulfillRandomWords(
    ...
    if(isLotteryEnable){
      latestLotteryId = babuLottery.viewCurrentLotteryId();
    }
  }
  ...
  function setcaller(address _caller) external onlyOwner {
    ...
    babuLottery = IBabuLottery(callerAddress);
  }
  ...
  function getRandomNumber() external override returns (uint256) {
    require((msg.sender == s_owner || msg.sender == callerAddress), "Contract not allowed");
    // Will revert if subscription is not set and funded.
    s_requestId = COORDINATOR.requestRandomWords(
      keyHash,
      s_subscriptionId,
  ...
  // Assumes the subscription is funded sufficiently.
  function setLotteryEnable(bool isEnable) external onlyOwner {
    isLotteryEnable = isEnable;
  }
  ...
  function viewRandomResult() external view override returns (uint32) {
      uint32 finalnumber = uint32(1000000 + (s_randomWords[0] % 1000000));
      return finalnumber;
  }
  /**
    * @notice View Current LotteryId
    */
