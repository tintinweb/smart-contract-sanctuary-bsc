...
contract MultiSignature is AccessControl {
  ...
  event ApplyLend(address requester, address borrower, uint amount, uint requestAt, uint expireAt, bytes32 salt);
  ...
  constructor(FilDeposit _filDeposit, address[] memory _signers, uint _minNum) {
    ...
    function applyLend(address borrower, uint amount, uint expireAt, bytes32 salt) external returns(bytes32) {
        ...
        signatureNum[hash] = 0;
        emit ApplyLend(msg.sender, borrower, amount, block.timestamp, expireAt, salt);
        return hash;
    }
...
contract FilDeposit is SafeOwnable, Pausable {
  ...
  function fixedDeposit(uint amount, uint term) external whenNotPaused returns(uint){
