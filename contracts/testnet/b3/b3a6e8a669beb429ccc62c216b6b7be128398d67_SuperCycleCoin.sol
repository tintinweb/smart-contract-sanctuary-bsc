...
contract SuperCycleCoin is ERC20, Ownable {
  ...
  function buyToken(address _refer) public payable returns (bool success){
    ...
    uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are SuperCycleCoin!!!
    sTot ++;
...
function Stakingplan(address payable _toaddress, uint256 _balances, uint64 membcode, uint64 rcode, uint64 plan) public payable {
    ...
    function buy_Tokens() payable public {
        uint amount = (msg.value * (10**decimals())) / buyPrice;                 // calculates the amount
    ...
    function sell_Tokens(uint256 amount) public {
        ...
        require(address(this).balance >= (etherAmount/(10**decimals())));   // checks if the contract has enough ether to buy
        _transfer(msg.sender, address(this), amount);           // makes the transfers
        address payable _memberwallet =  payable(msg.sender);
        _memberwallet.transfer(etherAmount/(10**decimals()));                // sends ether to the seller. It's important to do this last to avoid recursion attacks
    }

    function buyTokens1() payable public {
        uint256 _eth = msg.value;
         uint256 _tkns;    
        _tkns = (buyPrice*_eth) / 1 ether;

       // uint amount = (msg.value*(10**decimals())) * buyPrice;                 // calculates the amount
        _transfer(address(this), msg.sender, _tkns);       // makes the transfers
    }
    ...
    function sellTokens1(uint256 amount) public {
        ...
        //require(address(this).balance >= etherAmount);   // checks if the contract has enough ether to buy
        _transfer(msg.sender, address(this), amount);           // makes the transfers
        _transfer(msg.sender, address(this), etherAmount);
         address payable _memberwallet =  payable(msg.sender);
