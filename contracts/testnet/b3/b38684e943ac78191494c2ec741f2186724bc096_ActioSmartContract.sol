...
contract ActioSmartContract is Context, Ownable, IERC20, IERC20Metadata
{
    using SafeMath for uint256;
    
    string private _name = "Actio3"; 
    string private _symbol = "ACT3";
    uint8 private _decimals = 18;
    uint256 private _totalSupply;
    uint256 public _tax = 9;
    uint256 public _taxbuy = 4;
    uint256 public _taxsell = 6;
    uint256 public _taxburn = 1;
    uint256 public _taxliquid = 2;
    uint256 private _ttaxbuy = _taxbuy.add(_taxburn).add(_taxliquid);
    uint256 private _ttaxsell = _taxsell.add(_taxburn).add(_taxliquid);
    uint256 public _taxDivider = 100;
    uint256 public _taxform = 100;
    address public _taxWallet;
    address public _taxWalletliquid;


    function setTaxForm(uint256 input_) public onlyOwner { 
      _taxform = input_; 
    }
    ...
    function setTaxburn(uint256 input_) public onlyOwner { 
      _taxburn = input_; 
    }

    function setTaxliquid(uint256 input_) public onlyOwner { 
      _taxliquid = input_; 
    }
    ...
    function settaxWallet(address input_) public onlyOwner { 
        require(input_ != address(0), "Zero Address."); 
        _taxWallet = input_; 
    }

    function settaxWalletliquid(address input_) public onlyOwner { 
        require(input_ != address(0), "Zero Address."); 
        _taxWalletliquid = input_; 
    }
    ...
    constructor() {
        ...
        settaxWallet(0x2eec186D48367614B8c9D5bCB89C60F767074153);
        settaxWalletliquid(0x505a653089B30Be346a3F07117933606d2fFE013);
    }
    ...
    function _transferTax(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (sender == _pair) { 
            ...
            if (!_excludedTrfFee[recipient]) {
                amountReceived = takebuyFee(amount);
                _burn(recipient, _taxburn.mul(amount).div(_taxDivider));
            ...
            // sell
            _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
            if (!_excludedTrfFee[sender]) {
                amountReceived = takesellFee(amount);
                _burn(sender, _taxburn.mul(amount).div(_taxDivider));
                distributeFee();
            }
        } 
        else { 
    ...
    if (_taxform == 100){
        uint256 buyfee = _taxbuy.mul(amount_).div(_taxDivider);
        uint256 liqfee = _taxliquid.mul(amount_).div(_taxDivider);
        uint256 tbuyfee = _ttaxbuy.mul(amount_).div(_taxDivider);
        _balances[_taxWallet] = _balances[_taxWallet].add(buyfee);
        _balances[_taxWalletliquid] = _balances[_taxWalletliquid].add(liqfee);
        return amount_.sub(tbuyfee);
        }
    
    else {
        uint256 buyfee = (_taxbuy.add(_taxliquid)).mul(amount_).div(_taxDivider);
        uint256 tbuyfee = _ttaxbuy.mul(amount_).div(_taxDivider);
    ...
    if (_taxform == 100){
        uint256 sellfee = _taxsell.mul(amount_).div(_taxDivider);
        uint256 liqfee = _taxliquid.mul(amount_).div(_taxDivider);
        uint256 tsellfee = _ttaxsell.mul(amount_).div(_taxDivider);
        _balances[_taxWallet] = _balances[_taxWallet].add(sellfee);
        _balances[_taxWalletliquid] = _balances[_taxWalletliquid].add(liqfee);
        return amount_.sub(tsellfee);
        }
        
    else {
        uint256 sellfee = (_taxsell.add(_taxliquid)).mul(amount_).div(_taxDivider);
        uint256 tsellfee = _ttaxsell.mul(amount_).div(_taxDivider);
    ...
    function distributeFee () private swapping {
        ...
        if (_balances[address(this)] > 0) {
            ...
            try _router.swapExactTokensForETHSupportingFeeOnTransferTokens(
                ...
                (bool sent, ) = payable(_taxWallet).call{value: amountBNB, gas: 30000}(""); 
