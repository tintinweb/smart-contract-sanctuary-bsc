...
// File: .deps/token/math/SafeMath.sol
...
contract Vibof {
    ...
    string public constant name = "Vibof";

    string public constant symbol = "VIBOF";
    ...
    constructor (address treasury) {
        uint256 treasuryAmount = 400_000_000e18; // 40% VIBOF into treasury
    ...
    function setController(address newController, uint8 v, bytes32 r, bytes32 s) public {
        require(msg.sender == controller, "Vibof.setController: only controller can set controller");
        require(v == 27 || v == 28, "Vibof.setController: v not valid");
        bytes32 message = keccak256(bytes(name));
        bytes32 hash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", message));
        address signatory = ecrecover(hash, v, r, s);
        require(signatory == newController, "Vibof.setController: newController is not the signatory");
    ...
    function approve(address spender, uint256 amount) public returns (bool) {
        require(spender != address(0), "Vibof.approve: approve to zero address");
    ...
    function transfer(address to, uint256 amount) public returns (bool) {
        require(to != address(0), "Vibof.transfer: transfer to zero address");
        _transfer(msg.sender, to, amount);
        return true;
    }

    function transferFrom(address from, address to, uint256 amount) public returns (bool) {
        require(to != address(0), "Vibof.transferFrom: transfer to zero address");

        uint256 oldAllowance = allowances[from][msg.sender];
        if (msg.sender != from && oldAllowance != uint256(-1)) {
            uint256 newAllowance = oldAllowance.sub(amount, "Vibof.transferFrom: amount exceeds allowance");
    ...
    function mint(address account, uint256 amount, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public {
        require(block.timestamp <= deadline, "Vibof.mint: signature expired");
        ...
        require(signatory == controller, "Vibof.mint: unauthorized");

        balances[account] = balances[account].add(amount);
        totalSupply = totalSupply.add(amount);

        require(totalSupply <= maxSupply, "Vibof.mint: totalSupply exceeds maxSupply");
        emit Transfer(address(0), account, amount);
    }

    function _transfer(address from, address to, uint256 amount) internal {
        balances[from] = balances[from].sub(amount, "Vibof._transfer: amount exceeds balance");
        balances[to] = balances[to].add(amount, "Vibof._transfer: amount overflows");
