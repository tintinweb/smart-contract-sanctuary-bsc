...
contract BranchV3 is ReentrancyGuard, AccessControlEnumerable {
    ...
    // 用户管理模块
    function active(uint32 _branchId, address parent) external {
        require(_branchId == branchId, "branch number error.");
    ...
    function invite(uint32 _branchId, address addr) external {
        require(_branchId == branchId, "branch number error.");
        require(!restartStatus, "system on restart status");
