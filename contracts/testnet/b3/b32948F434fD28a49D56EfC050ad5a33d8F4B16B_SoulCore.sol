...
contract SoulCore is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable{
    ...
    event CreateBlindBox(address indexed player, address indexed recommender, uint8 gender, uint8 soulCount, uint amount, uint tokenId);
    event OpenBlindBox(address indexed player, address indexed recommender, uint8 gender, uint8 soulCount, uint tokenId);
    ...
    function createToken(uint8 gender, uint amount, uint8 soulCount, address recommender) public returns(uint){
        ...
        coreInfo.SOUL1155.mint(address(this), tokenId, amount);
        ...
        if(soulCount > 0){
            ...
            //销毁soul
            coreInfo.SOULTOKEN.burnFrom(_msgSender(), uint(soulCount) * 1 ether);
        }
        ...
        emit CreateBlindBox(_msgSender(), recommender, gender, soulCount, amount, tokenId);
     ...
     **/
    function getFree(uint8 gender_, address recommender_, uint8 isJump_) private returns(uint){
        uint tokenId;
        uint length = publicPool[gender_].length;   //免费池的长度
        if(length == 0){
            if(isJump_ == 0){
                //跳转到收费池
        ...
        //池子数量维护
        countMaintain(false, gender_,  0, tokenIndex, length);           //tokenId数量维护

        emit OpenBlindBox(_msgSender(), recommender_, gender_, 0, tokenId);
        return tokenId;
    }
    ...
    //氪金池子的抽取
    function getCharge(uint8 gender_, address recommender_, uint8 isJump_) private returns(uint){
        uint tokenId;
        uint8 proIndex = getIndex();                //获取概率索引
        if(gender_ == 1){
            //女
            ...
            if(length > 0){
                ...
                countMaintain(true, gender_,  proIndex, tokenIndex, length);           //tokenId数量维护

                emit OpenBlindBox(_msgSender(), recommender_, gender_, proIndex, tokenId);
    ...
    //获取氪金池的key
    function getIndex() private returns(uint8){
        uint8 index = 1;
        //获取1-100的随机数
     ...
     *  length：数组长度
     **/
    function countMaintain(bool status, uint8 gender_, uint8 soulCount, uint tokenIndex_, uint length_) private{
        if(status){
            //氪金池
            if(gender_ == 1){
                //女
    ...
    function divide(address recommender_, address caster_, uint amount_) private{
        ...
        coreInfo.USDT.safeTransferFrom(_msgSender(), recommender_, (amount_ / 100) * 1 ether);
        if(caster_ == address(0)){
            //铸造本身没有铸造者分成，平台占两份
            coreInfo.USDT.safeTransferFrom(_msgSender(), address(this), (amount_ / 50) * 1 ether);
        }else{
            //给铸造者分成
            coreInfo.USDT.safeTransferFrom(_msgSender(), caster_, (amount_ / 100) * 1 ether);
            //平台分成
            coreInfo.USDT.safeTransferFrom(_msgSender(), address(this), (amount_ / 100) * 1 ether);
        }
        //合约奖池分成
        coreInfo.USDT.safeTransferFrom(_msgSender(), address(this), (amount_ / 100) * 1 ether);
    }
