...
*/
    ...
    function nextLevel() external {
        ...
        for (
            ...
            if (
                ...
                levlToIdToTable[adrToInfo[msg.sender].level][i].userCount != 2 // отправляем лидера учатником на след уровень, если стол не забит
            ) {
    ...
    function closeTable() internal {
        ...
        for (uint256 i = 1; i < 11; i++) {
            if (idWinnerToInfo[i].user == address(0x0)) {
                ...
                idWinnerToInfo[i].money =
                    (levlToPayment[adrToInfo[msg.sender].level] * 2 * 95) /
                    100;
                break;
            }
        ...
        emit tableClosed(
            ...
            (levlToPayment[adrToInfo[msg.sender].level] * 2 * 95) / 100,
            lost,
            adrToInfo[msg.sender].level,
            adrToInfo[msg.sender].tableId
        );

        if (adrToInfo[msg.sender].level == 2) {
            isParticipated[
                levlToIdToTable[adrToInfo[msg.sender].level][
                    adrToInfo[msg.sender].tableId
                ].leader
        ...
        )
    {
        ...
        if (block.timestamp > adrToInfo[user].timest + deadline) {
            lost = (levlToPayment[adrToInfo[user].level] * 2 * 95) / 100;
        }
        return (
