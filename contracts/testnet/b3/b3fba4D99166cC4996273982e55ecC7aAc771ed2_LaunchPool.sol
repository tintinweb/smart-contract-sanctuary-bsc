    ...
    StakingTierModels
{
    ...
    function emergencyWithdraw() external virtual override {
        require(_status != Statuses.FINALIZED, "pool is finalized");
    ...
    function claim(uint32 _index) external virtual override {
        require(_status == Statuses.FINALIZED, "pool is not finalized yet");
...
pragma solidity ^0.8.10;
import "./../../libs/SafeArrayAddress.sol";
...
abstract contract Whitelist {
    ...
    function _addWhitelist(address[] memory _addresses) internal virtual {
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            addresses.removeAddress(_addresses[i]);
            addresses.addAddress(_addresses[i]);
