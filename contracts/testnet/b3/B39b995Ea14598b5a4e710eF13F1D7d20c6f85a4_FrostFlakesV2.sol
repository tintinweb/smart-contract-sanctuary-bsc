...
contract FrostFlakesV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function reRequestMigration() public payable {
        require(migrationRequested[msg.sender] == true, "Initial migration request is required");
        require(canReRequestMigration(), "1 hour must pass between each re-request of migration");
        emit EmitMigrationRequested(msg.sender);
    }

    function canReRequestMigration() public view returns(bool) {
        if (lastMigrationRequest[msg.sender] == 0) {
            return true;
        }
        return block.timestamp > Math.add(lastMigrationRequest[msg.sender], 3600);
    }
