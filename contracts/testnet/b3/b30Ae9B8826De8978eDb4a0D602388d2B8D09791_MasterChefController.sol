...
interface IMasterchef {
    ...
    function updateEmissionRate(uint256 _KnightPerBlock) external;
}
...
contract MasterChefController is Ownable {
    ...
    function addPool(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate) external onlyOwner {
    ...
    function updateKnightPerBlock(uint256 _KnightPerBlock) external onlyOwner {
           require (_KnightPerBlock <= 10 ether, "updateemission: value must be less than 10");
           masterchef.updateEmissionRate(_KnightPerBlock);      
    } 
