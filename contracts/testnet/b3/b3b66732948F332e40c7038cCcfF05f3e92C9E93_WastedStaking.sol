...
contract WastedStaking is IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    function stake(uint256 poolId, uint256 amount) external nonReentrant {
        ...
        require(stakers[poolId][msg.sender].staked == 0, "unstake first");

        acceptedToken.safeTransferFrom(msg.sender, address(this), amount);

        stakers[poolId][msg.sender] = Staker(amount, claimTime);
    ...
    function _calculateTotalClaim(
        ...
        uint256 profit = apr.mul(amount).mul(lockTimes).div(100 * 365); // apr = ((Lãi/ (Tiền gốc * n)) x 365) x 100 => Lãi =(apr *  Tiền gốc * n)/ 100 * 365
