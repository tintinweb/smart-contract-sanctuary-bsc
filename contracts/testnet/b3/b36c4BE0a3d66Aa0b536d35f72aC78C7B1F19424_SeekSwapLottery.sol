...
// File: contracts/interfaces/ISeekSwapLottery.sol

pragma solidity ^0.8.4;

interface ISeekSwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in SEEK per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in SEEK token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInSEEK: price of a ticket in SEEK
...
// File: contracts/SeekSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title SeekSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract SeekSwapLottery is ReentrancyGuard, ISeekSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInSEEK = 50 ether;
    uint256 public minPriceTicketInSEEK = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInSEEK;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] SEEKPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInSEEK;
        uint32 finalNumber;
    }
     ...
     * @param _SEEKTokenAddress: address of the SEEK token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _SEEKTokenAddress, address _randomGeneratorAddress) {
        SEEKToken = IERC20(_SEEKTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of SEEK to this contract
        uint256 amountSEEKToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInSEEK,
            _ticketNumbers.length
        );

        // Transfer SEEK tokens to this contract
        SEEKToken.safeTransferFrom(address(msg.sender), address(this), amountSEEKToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInSEEK += amountSEEKToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInSEEKToTransfer
        uint256 rewardInSEEKToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInSEEKToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        SEEKToken.safeTransfer(msg.sender, rewardInSEEKToTransfer);

        emit TicketsClaim(msg.sender, rewardInSEEKToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInSEEK) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in SEEK for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].SEEKPerBracket[j] =
                ...
                // A. No SEEK to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].SEEKPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInSEEK - amountToShareToWinners);

        // Transfer SEEK to treasury address
        SEEKToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        SEEKToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInSEEK += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInSEEK >= minPriceTicketInSEEK) && (_priceTicketInSEEK <= maxPriceTicketInSEEK),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInSEEK: _priceTicketInSEEK,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            SEEKPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInSEEK: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(SEEKToken), "Cannot be SEEK token");
    ...
    /**
     * @notice Set SEEK price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInSEEK: minimum price of a ticket in SEEK
     * @param _maxPriceTicketInSEEK: maximum price of a ticket in SEEK
     */
    function setMinAndMaxTicketPriceInSEEK(uint256 _minPriceTicketInSEEK, uint256 _maxPriceTicketInSEEK)
        external
        onlyOwner
    {
        require(_minPriceTicketInSEEK <= _maxPriceTicketInSEEK, "minPrice must be < maxPrice");

        minPriceTicketInSEEK = _minPriceTicketInSEEK;
        maxPriceTicketInSEEK = _maxPriceTicketInSEEK;
    }
     ...
     * @param _priceTicket price of a ticket (in SEEK)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].SEEKPerBracket[_bracket];
        } else {
            return 0;
        }
    }
