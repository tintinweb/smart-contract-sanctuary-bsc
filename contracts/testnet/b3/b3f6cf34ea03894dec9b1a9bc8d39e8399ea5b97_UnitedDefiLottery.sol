 ...
 */
contract UnitedDefiLottery is ReentrancyGuard, IPancakeSwapLottery, Ownable {
    ...
    uint256 public commontrials;
    uint256 public raretrials;
    uint256 public epictrials;
    uint256 public legendarytrials;
    ...
    // Nested Mapping of Nftlevel 
    mapping(uint256 => mapping(address => uint256)) private commonnftlevel;
    mapping(uint256 => mapping(address => uint256)) private epicnftlevel;
    mapping(uint256 => mapping(address => uint256)) private rarenftlevel;
    mapping(uint256 => mapping(address => uint256)) private legendarynftlevel;
    mapping(address => bool) private commonaddusers;
    mapping(address => bool) private rareaddusers;
    mapping(address => bool) private epicaddusers;
    mapping(address => bool) private legendaryaddusers;
    ...
    /**
    * @notice Different buy function for nft holders
    **/
        function freeTickets(uint256 _lotteryId, uint32  _ticketNumbers, address nftholder)
        external
        /*override*/
        notContract
        nonReentrant
    {
        require(_ticketNumbers != 0, "No ticket specified");
        require(commonaddusers[nftholder] == true, "You didn't participate in nfts");
        commonnftlevel[_lotteryId][nftholder] = commontrials++;
        require(commontrials <= 1);
        //common Nft level
        require(rareaddusers[nftholder] == true, "You didn't participate in nfts");
        rarenftlevel[_lotteryId][nftholder] = raretrials++;
        require(raretrials <= 2);
        // Rare Nft level
        require(epicaddusers[nftholder] == true, "You didn't participate in nfts");
        epicnftlevel[_lotteryId][nftholder] = epictrials++;
        require(epictrials <= 3);
        //Epic Nft Level
        require(legendaryaddusers[nftholder] == true, "You didn't participate in nfts");
        legendarynftlevel[_lotteryId][nftholder] = legendarytrials++;
        require(legendarytrials <= 4);
        //Create custom requirement for ticket number length
        //require(_ticketNumbers.length <= maxNumberTicketsPerBuyOrClaim, "Too many tickets");
     ...
     * @notice Claim a set of winning tickets for a lottery
     **/
     function addnftusers(address commonuser, address epicuser, address rareuser, address legendaryuser) external onlyOwnerOrInjector{
         commonaddusers[commonuser] = true;
         rareaddusers[rareuser] = true;
         epicaddusers[epicuser] = true;
         legendaryaddusers[legendaryuser] = true;
     }

     function addmultiplecommon(address[] calldata users) external onlyOwner{
         for (uint256 i = 0; i < users.length; i++){
             address thisuser = users[i];
             commonaddusers[thisuser] = true;
         }
     }

    function addmultipleRare(address[] calldata users) external onlyOwner{
         for (uint256 i = 0; i < users.length; i++){
             address thisuser = users[i];
             rareaddusers[thisuser] = true;
         }
     }

    function addmultipleEpic(address[] calldata users) external onlyOwner{
         for (uint256 i = 0; i < users.length; i++){
             address thisuser = users[i];
             epicaddusers[thisuser] = true;
         }
     }

    function addmultipleLegendary(address[] calldata users) external onlyOwner{
         for (uint256 i = 0; i < users.length; i++){
             address thisuser = users[i];
             legendaryaddusers[thisuser] = true;
         }
     }

    function deletenftusers(address commonuser, address epicuser, address rareuser, address legendaryuser) external onlyOwnerOrInjector{
         commonaddusers[commonuser] = false;
         rareaddusers[rareuser] = false;
         epicaddusers[epicuser] = false;
         legendaryaddusers[legendaryuser] = false;
     }
