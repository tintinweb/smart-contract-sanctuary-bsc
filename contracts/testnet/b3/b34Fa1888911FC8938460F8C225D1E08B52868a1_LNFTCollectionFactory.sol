...
contract LNFTCollectionFactory is LNFTCollection, Market {
    ...
    function _tokenURI(address _collection, uint256 _tokenId)
        external
        view
        returns (string memory)
    {
        string memory tokenURIIPFS = LNFTCollection(_collection).getTokenURI(
            _tokenId
        );
        return tokenURIIPFS;
    }
    ...
    function _setTokenURI(
        ...
        LNFTCollection(_collection).setTokenURI(_tokenId, _ipfsLink);
    }

    function _burn(address _collection, uint256 _tokenId) external onlyOwner {
        LNFTCollection(_collection).burnToken(_tokenId);
    }

    function _ownerOf(address _collection, uint256 _tokenId) external
        view
        returns (address) {
        return LNFTCollection(_collection).ownerOf(_tokenId);
    }
}
...
import "../contracts/collection/Ownable.sol";
