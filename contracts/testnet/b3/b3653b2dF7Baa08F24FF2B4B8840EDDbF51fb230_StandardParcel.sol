...
contract StandardParcel is Parcel, IStandardParcel {
    ...
    function assignNFT(
        ...
        require(_exists(_parcelId), "StandardParcel: Parcel does not exist");
        if (!_isLocked[_parcelId]) {
            _changeTokenLocking(_parcelId, true);
        }
        if (_activatableNFTType == ActivatableNFTs.Aesthetic) {
            aestheticNFTIdToParcelId[_tokenId] = _parcelId;
            parcels[_parcelId].aestheticNFTTokenIds.push(_tokenId);
            totalAestheticNFTsAmount++;
        } else {
            uint8 slotsLimit = parcels[_parcelId].upgradeStatus
                ? standardParcelSlotsLimit
                : upgradedParcelSlotsLimit;
            require(
                parcels[_parcelId].minerNFTTokenIds.length <= slotsLimit,
                "StandardParcel: This parcell does not have any available slots"
            );
            totalMinerNFTsAmount++;
            minerIdToParcelId[_tokenId] = _parcelId;
            parcels[_parcelId].minerNFTTokenIds.push(_tokenId);
        }
        emit Assigned(_parcelId, _tokenId);
    }
    ...
    function getBrokerPBContractAddress() external view returns (address) {
        return brokerPBContractAddress;
    }
