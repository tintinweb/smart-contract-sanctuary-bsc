...
contract OmniCounter is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    uint public messageCounter; //Variable global del counter DAHHHHH! 
    // required: the LayerZero endpoint which is passed in the constructor
    ILayerZeroEndpoint public layerZeroEndpoint; //Crea una instancia de la interfaz ILayerZeroEndpoint
    mapping(uint16 => bytes) public trustedSourceLookup;

    constructor(address _endpoint) {
        layerZeroEndpoint = ILayerZeroEndpoint(_endpoint);
        /*Sirve para conectarse al endpoint (que es una direccion predeterminada para cada red) por la gente de L0
        */
    }
