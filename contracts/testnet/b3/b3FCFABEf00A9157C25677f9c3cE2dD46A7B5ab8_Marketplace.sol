...
contract Marketplace is Ownable{
    ...
    mapping (uint256 => Profit) private profitSet;
    ...
    function addProfit(uint256[] memory tokenID) external onlyOwner {
        ...
        for (uint256 i; i < tokenID.length; i ++) {
            ...
            if (!profitSet[tokenID[i]].status) {
                _profits[_profits.length] = tokenID[i];
                profitSet[tokenID[i]] = Profit(true, _profits.length);
            }
        }
        profits = _profits;
    }
