...
contract Exchange is Initializable, UUPSUpgradeable, EIP712Upgradeable, OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IERC20Upgradeable constant public WEKTA = IERC20Upgradeable(0x3c3024624699bDe873dD30618a2C3593fD7b91d2);
    ...
    bytes4 constant public ASSET_TYPE_ERC721A = bytes4(keccak256("ERC721A"));
    ...
    function createOrder(LibOrder.Order memory order, bytes calldata signature) external whenNotPaused nonReentrant {
        ...
        require(order.tokenType == ASSET_TYPE_ERC721 || order.tokenType == ASSET_TYPE_ERC721A || order.tokenType == ASSET_TYPE_ERC1155, "Exchange: Invalid token type");
    ...
    function completeFixedSale(LibOrder.Order memory order) external payable whenNotPaused nonReentrant {
        ...
        if(order.tokenType == ASSET_TYPE_ERC721A) IERC721AUpgradeable(order.tokenAddress).safeTransferFrom(order.seller, msg.sender, order.tokenId);
    ...
    function completeBidding(LibBidOrder.BidOrder memory bidOrder, bytes calldata signature) external whenNotPaused nonReentrant {
        ...
        // transfer token from seller to buyer
        {
            ...
            if(order.tokenType == ASSET_TYPE_ERC721A) IERC721AUpgradeable(bidOrderData.tokenAddress).safeTransferFrom(
                msg.sender, bidOrderData.buyer, bidOrderData.tokenId
            );
   ...
   /**
     * @dev update _ektaNftRegistry
     * @param _ektaNftRegistry.
     */
    function updateektaNftRegistry(address  _ektaNftRegistry)
        external
    {
