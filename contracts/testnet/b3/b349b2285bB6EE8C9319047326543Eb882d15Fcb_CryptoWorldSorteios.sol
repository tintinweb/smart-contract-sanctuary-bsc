...
contract CryptoWorldSorteios is Ownable, ReentrancyGuard{
    ...
    constructor(address _cryptoWorldContract) ReentrancyGuard()
    {
        ...
        console.log("_cryptoWorldContract: ", _cryptoWorldContract);
        entryValue = 0.001 ether;
        participantsLimit = 50;
    }

    function getAccountIn(address _address) external view returns(address accountAddress, uint256 totalReceivable, bool exist)
    {
        Account memory acc = accMapping[_address];
        accountAddress = acc.accountAddress;
        totalReceivable = acc.totalReceivable;
        exist = acc.exist;
    }

    function getContratoCrypto() external view returns(address _address)
    {
        _address = cryptoWorldContract;
        return _address;
    }
    ...
    function goodLuck() external payable
    {
        ...
        if(participantsLimit == players.length)
        {
            ...
            ICryptoWorldContract(cryptoWorldContract).donation{value: valueTransferContract}();
    ...
    function _getAccountCryptoWorld(address _address) public view returns(address referrerAccount)
    {
        uint256 entryDate;
        uint256 lastWithdrawal;
        uint256 depositedValue;
        (entryDate, lastWithdrawal, depositedValue, referrerAccount) = ICryptoWorldContract(cryptoWorldContract).getAccount(_address);
    }

    function getNetwork(address _address) external view returns(address[] memory account)
    {
        for (uint16 i = 0; i < 11; i++)
        {
            _address = _getAccountCryptoWorld(_address);

            if(!accMapping[_address].exist && _address != address(0))
            {
                account[i] = _address;
            }
        }
...
interface ICryptoWorldContract
{
    ...
    function getAccount(address _address) external view returns (uint256 entryDate, uint256 lastWithdrawal, uint256 depositedValue, address referrerAccount);
}
//carteira consulta: 0xaec420dD346040a14071C450A8B0a13623C5759e
//contrato: 0x4Dae761482Dc5Ff91D5e55Eda8ea3A8455A820C7
