...
contract KojiGamepass is ERC721URIStorage, ERC165, Ownable, ReentrancyGuard {
    ...
    function viewHashes(address _address, string memory _holder) external pure returns (bytes32, bytes32) {
        return(keccak256(bytes(Strings.toHexString(uint160(_address), 20))),keccak256(bytes(Strings.toHexString(uint160(bytesToUint(bytes(_holder))), 20))));
    }
    ...
    function viewAddressString(address _address) external pure returns (string memory, uint, bytes memory, bytes memory) {
        ...
        return(strung, i, bytes(strung), abi.encode(i));
    }
