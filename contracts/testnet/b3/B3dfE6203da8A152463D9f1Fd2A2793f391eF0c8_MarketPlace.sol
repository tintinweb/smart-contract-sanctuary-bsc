...
contract MarketPlace is Ownable {
    ...
    function sellNft(
        ...
        require(
            ...
            block.timestamp >= _offers[_nftOfferId[_nftAddress][_nftId]].offerEndAt,
    ...
    function bidForNft(
        ...
        require(IERC721(_nftAddress).ownerOf(_nftId) != msg.sender, "You can't bid on your own NFT");
        address payment = gameAddresses.getAddressOf(AddressesInit.Addresses.PAYMENTS);
        uint256 _balance = IPayments(payment).getMyReward(msg.sender) + IPayments(payment).getMyCentersRevenues(msg.sender);
    ...
    function acceptBid(uint256 _offerId) external {
        OfferDatas memory offer = _offers[_offerId];
        require(offer.nftOwner == msg.sender, "not your NFT");
        address payments = gameAddresses.getAddressOf(AddressesInit.Addresses.PAYMENTS);
        IPayments IPay = IPayments(payments);

        require(
            block.timestamp <= offer.offerEndAt,
            string(abi.encodePacked("Offers only avalaible", offerDuration, "days"))
        );
