...
contract PeopleOwnToken is Context, IBEP20, Ownable 
{
    using SafeMath for uint256;
    using Address for address;

    address payable public marketingWalletAddress = payable(0x7A26166bD2fa1B584fAb296C5fD1C0f63fF9Dc41); 
    address payable public charityWalletAddress = payable(0x703fAb3Ff649dDcA8D5a3420Df3702689409AC1c); 
    ...
    uint256 public _initialSupply = 120_000_000_000 * 10**18;
    ...
    string private _name = "PEOPLES OWN TOKEN";
    ...
    {
        uint256 _swapableFee = _liquidityFee.add(_marketingFee).add(_charityFee);
        ...
        uint256 bnbForMarketing = newBalance.mul(_marketingFee).div(_swapableFee);
    ...
    {
        _amount = _amount.mul(100).div(100-_burnFee);
        uint256 swapableFees = _liquidityFee+_charityFee+_marketingFee;
    ...
    function setTaxFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee, uint256 charityFee, uint256 burnFee) 
    external onlyOwner() 
    {
        _taxFee = taxFee;
        ...
        uint256 _totalFee = _taxFee.add(_liquidityFee).add(_marketingFee).add(_charityFee).add(_burnFee);
