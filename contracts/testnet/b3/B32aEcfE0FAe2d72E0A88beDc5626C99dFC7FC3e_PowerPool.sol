...
contract PowerPool is Ownable {
    ...
    function getPrice(IBEP20 token) public view returns(uint256) {
        if(priceSwitch) {
            return fixedPrice;
        }
        address[] memory path;
    ...
    function stake(uint256 _amount) public {
        ...
        uint256 power = _amount.mul(getPrice(stakeToken)).mul(3).div(stakeToken.decimals());
    ...
    function withdraw() public {
        ...
        uint256 rewardTokenPrice = getPrice(rewardToken);
        uint256 canExtractTokens = user.power.div(rewardTokenPrice).mul(rewardToken.decimals());
        uint256 amount=0;
        if(totalRewardToken>=canExtractTokens){
            ...
            uint256 extractPower = totalRewardToken.mul(rewardTokenPrice).div(rewardToken.decimals());
    ...
    function pendingReward(address _user) external view returns(uint256) {
        ...
        if(block.number>lastRewardBlock && totalPower != 0) {
            ...
            uint256 cafeReward = multiplier.mul(rewardPerBlock);
