 ...
 */
abstract contract ExceptionalList is AccessControlEnumerable {
    ...
    function addMembers(address[] calldata _accounts) external onlyRole(EL_MANAGER_ROLE) {
        ...
        uint256 i;
        for (i; i < l; i++) {
            require(!isMember(_accounts[i]), "Whitelist: Address is member already");
...
/**
 * @title GreedyArtToken - Deflation ERC20 token with burn and owner transaction fees.
 */
contract GreedyArtToken is ERC20, ExceptionalList, IMintableERC20 {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");

    uint256 public constant SYSTEM_DECIMAL = 10_000;
    uint256 public constant MAX_SUPPLY = 100_000_000e18; // 100M
    ...
    constructor(address _admin) ERC20("GREEDY", "GREEDY") {
    ...
    function _afterTokenTransfer(
        ...
        if (address(antibot) != address(0)) {
            if (!antibot.assureCanTransfer(from, to, amount)) {
                revert AntibotCallProtected();
            }
        }

        uint256 fees = _excludeFee(amount, burnFee);
        if (fees > 0) {
            _burn(to, fees);
        }

        fees = _excludeFee(amount, devFee);
        if (fees > 0 && devFeeTo != address(0)) {
            _transfer(to, devFeeTo, fees);
        }
    }

    function _excludeFee(uint256 _amount, uint256 _fee) internal pure returns (uint256 excluded) {
