...
contract DV2Token is ERC20, OperatorFunction, ERC20Burnable, ERC20Capped, ERC20Pausable {
    ...
    function updateMultiplier(address from) public returns (bool allow) {
      ...
      uint256 currentBasePrice = checkMultiplier().div(100).mul(basePrice);
      uint256 currentSellTargetPrice = checkMultiplier().div(100).mul(sellTargetPrice);
      if(isMultiEnabled){
        if(buyprice <= currentBasePrice && sellprice <= currentBasePrice){
					...
					editBasePrice(basePrice,checkMultiplier().div(100).mul(basePrice));
					editSellPrice(sellTargetPrice,checkMultiplier().div(100).mul(sellTargetPrice));
          toggleCanSell(false);
        }
    ...
    // Set Excluded From Fees users
    function readPower() public view returns(uint256 mul1,uint256 mul2,uint256 mul3,bool _status1,bool _status2,bool _status3) {
        uint256 buyprice = buyPriceNow();
        uint256 sellprice = sellPriceNow();
        uint256 currentBasePrice = checkMultiplier().div(100).mul(basePrice);
        uint256 currentSellTargetPrice = checkMultiplier().div(100).mul(sellTargetPrice);
