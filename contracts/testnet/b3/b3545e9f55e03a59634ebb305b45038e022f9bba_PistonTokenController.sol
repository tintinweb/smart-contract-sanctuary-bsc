...
contract PistonTokenController is Initializable, ERC20Upgradeable, OwnableUpgradeable {
	...
	function initialize(address _Piston) public virtual initializer {
		...
		//////@TODO!!!!!  remove later
		swapTokensAtAmount = 50 * (10**14);
		_ecosystemWalletAddress=address(0x4ff8C8741256d729A8cC7F1Cc02330F8103CCD28);
		_raceContractAddress=address(0x4ff8C8741256d729A8cC7F1Cc02330F8103CCD28);
	...
	function swapAndLiquify() external onlyOwner  {
		...
		if ( forLiquidity >= swapTokensAtAmount) {
			...
			// send 10% to burn, 40% to race contract
			pistonToken.transfer(deadWallet, forBurn);
			
			
			//remaining tokens to race contract - around 40%
			uint256 remainingTokens = pistonToken.balanceOf(address(this));
			pistonToken.transfer(_raceContractAddress, remainingTokens);
