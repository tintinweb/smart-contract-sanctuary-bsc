...
import "./interface/ICategory.sol";
...
contract Trade is AccessControl {
    ...
    struct PaymentFee {
        ...
        uint256 buyerFeePermileOffer;
        uint256 sellerFeePermileOffer;
    }
    ...
    event ServiceFee(uint256 buyerFee, uint256 sellerFee);
    event BuyAsset(
    ...
    event PaymentDetailsUpdated(
        ...
        string category,
        uint256 level,
        PaymentFee paymentDetails
    );

    event PaymentAddressStatusUpdated(address indexed paymentToken, bool status);
    ...
    mapping(address => mapping(string => mapping(uint256 => PaymentFee))) public paymentAssets;

    mapping(address => bool) internal paymentAddressStatus;

    mapping(string => bool) internal categoryStatus;

    mapping(string => uint256) private buyerFees;

    mapping(string => uint256) private sellerFees;
    ...
    constructor(
        ...
        address _weth,
        address[] memory paymentAssetAddress
    ) {
        transferProxy = _transferProxy;
        paymentAddressStatus[paymentAssetAddress[0]] = true;
        paymentAddressStatus[paymentAssetAddress[1]] = true;
        paymentAddressStatus[paymentAssetAddress[2]] = true;
        paymentAddressStatus[_weth] = true;
    ...
    function buyerServiceFee(string memory _category)
        external
        view
        virtual
        returns (uint256)
    {
        return buyerFees[_category];
    }
    ...
    function sellerServiceFee(string memory _category)
        external
        view
        virtual
        returns (uint256)
    {
        return sellerFees[_category];
    }
    ...
    function addPaymentDetails(
        address payementToken,
        string memory category,
        PaymentFee[] memory paymentDetails
    ) external onlyRole("ADMIN_ROLE") {
        require(payementToken != address(0), "Trade: address shouldn't be zero");
        require(paymentDetails.length == 4, "length Mismatch");
        categoryStatus[category] = true;
        for (uint8 i = 0; i < 4; i++) {
        paymentAssets[payementToken][category][i] = paymentDetails[i];
        if (i+1 != paymentDetails.length) {
            paymentDetails[i + 1].minimumAmount = paymentDetails[i].maximumAmount + 1;
        }
        emit PaymentDetailsUpdated(payementToken, category, i, paymentDetails[i]);
        }
    }

    function setServiceFee(string memory _category, uint256 buyerfee, uint256 sellerfee)
        external
        onlyRole("ADMIN_ROLE")
        returns (bool)
    {
        buyerFees[_category] = buyerfee;
        sellerFees[_category] = sellerfee;
        emit ServiceFee(buyerfee, sellerfee);
        return true;
    }

    function setPaymentTokenStatus(address paymentToken, bool status)
        external
        onlyRole("ADMIN_ROLE")
        returns (bool)
    {
        paymentAddressStatus[paymentToken] = true;
        emit PaymentAddressStatusUpdated(paymentToken, status);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        string memory category = ICategory(order.nftAddress).getCategory(
        order.tokenId
        );
        require(
        paymentAddressStatus[order.erc20Address] && categoryStatus[category],
        "Invalid Data"
        );
        Fee memory fee = getFees(order, category);
    ...
    function getFees(Order calldata order, string memory category) internal view returns (Fee memory) {
        address tokenCreator;
        uint256 platformFee;
        uint256 royaltyFee;
        uint256 assetFee;
        (uint256 _buyerFee, uint256 _sellerFee) = txFees(
        order.erc20Address,
        category,
        order.seller,
        order.buyer
        );
        uint256 price = (order.amount * 1000) / (1000 + _buyerFee);
        uint256 buyerFee = order.amount - price;
        uint256 sellerFee = (price * _sellerFee) / 1000;
    ...
    function tradeAsset(
        ...
        transferProxy.erc20safeTransferFrom(
            IERC20Metadata(order.erc20Address),
            buyer,
            owner,
            fee.platformFee
        );
    ...
    function txFees(
        ...
        address sender,
        address buyer
    ) internal view returns (uint256 buyerFee, uint256 sellerFee) {
        uint256 buyerBalance;
        uint256 sellerBalance;
        buyerBalance = IERC20Metadata(erc20Address).balanceOf(buyer);
        sellerBalance = IERC20Metadata(erc20Address).balanceOf(sender);
        for (uint8 i = 0; i < 4; i++) {
            if (
                buyerBalance >=
                paymentAssets[erc20Address][category][i].minimumAmount &&
                buyerBalance <= paymentAssets[erc20Address][category][i].maximumAmount
            ) {
                buyerFee = buyerFees[category] - (buyerFees[category] * paymentAssets[erc20Address][category][i].buyerFeePermileOffer) / 100;
            }
            if (
                sellerBalance >=
                paymentAssets[erc20Address][category][i].minimumAmount &&
                sellerBalance <= paymentAssets[erc20Address][category][i].maximumAmount
            ) {
                sellerFee = sellerFees[category] - (sellerFees[category] * paymentAssets[erc20Address][category][i].sellerFeePermileOffer) / 100;
            }
        }
