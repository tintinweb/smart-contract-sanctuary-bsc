...
contract deployer is OwnableUpgradeable {
    ...
    function createNewPool(
        ...
        if (!_isLpBurn) {
            require(
                ...
                "LP lock should be greater than 30 days"
            );
        }
    ...
    function whiteListRouters(address _routerAddress) external onlyOwner {
        require(!whitelistedRouters[_routerAddress], "Already Whitelisted");
    ...
    function whiteListTokens(address _tokenAddress) external onlyOwner {
        require(!whitelistedTokens[_tokenAddress], "Already Whitelisted");
...
contract Launchpad is Ownable, ReentrancyGuard {
    ...
    **/
    ...
    function participateToSale(uint256 _amount) public nonReentrant {
        ...
        require(poolStatus == 1, "Not currently live");
        if (isWhitelisted) {
            require(
        ...
        if (remainingBNB < minimumParticipate) {
            require(_amount <= remainingBNB, "Exceeding max amount");
        } else {
            require(
    ...
    function exitPresale() public nonReentrant {
        syncPool();
        require(poolStatus == 3, "Unsuccesful, did not cancel");
        require(
            shares[msg.sender].participateAmount > 0,
            "You have not contributed to presale"
        );
    ...
    function claimTokens() public nonReentrant {
        require(poolStatus == 4, "Presale has yet to be finalised");
        require(
            shares[msg.sender].participateAmount > 0,
            "You have not contributed to presale"
        );
        ...
        if (isVesting) {
            ...
            shares[msg.sender].nextClaimTime = finalizedAt.add(releaseCycle);
    ...
    function claimVesting() public nonReentrant {
        ...
        shares[msg.sender].nextClaimTime = releaseCycle.add(
    ...
    function cancelPool() public {
        require(
            ...
            "Owner may finalise the presale"
        );
        require(poolStatus != 4, "When presale is finalised you may not cancel it");
    ...
    function whiteListUsers(address[] memory _wallets) public {
        require(msg.sender == poolOwner, "Owner may add to whitelist");
    ...
    function distributeTokens(uint256 from, uint256 to) public {
        ...
        require(totalUsers <= 50, "Whitelist up-to 50 addresses at a time.");
