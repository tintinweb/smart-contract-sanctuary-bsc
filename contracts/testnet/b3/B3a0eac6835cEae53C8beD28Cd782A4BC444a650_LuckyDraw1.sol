...
contract LuckyDraw1 is Ownable {
    ...
    constructor(address _treasuryAddress, IBOX _box, address _MEXIAddress) {
        ...
        addPool(100, 50_000 ether, 100, commonHash);
        addPool(100, 100_000 ether, 300, epicHash);
        addPool(100, 250_000 ether, 500, legendHash);
    ...
    function claimWeeklyLuckyMembers(uint _week) external {
        require(isWeeklyLucky(_week, _msgSender()), 'LuckyDraw::claimWeeklyLuckyMembers:Not top');
        ...
        uint claimAmount = luckyMembers[_week].amount * percentToWeeklyLucky / percentDecimal;
        IERC20(MEXIAddress).transfer(_msgSender(), claimAmount);
    ...
    function isWeeklyLucky(uint _week, address _user) public view returns(bool result) {
        for(uint i = 0; i < luckyMembers[_week].list.length; i++) {
            if(luckyMembers[_week].list[i] == _user) {
                result = true;
                break;
            }
        }
    ...
    function _handleSpinFee(uint _pid, uint _week) internal {
        ...
        require(IERC20(MEXIAddress).transferFrom(_msgSender(), address(this), _p.price - toTreasury));
...
//        weeklyUsers[_week].weeklyUsersArr
        for(uint i = 0; i < 5; i++) {
