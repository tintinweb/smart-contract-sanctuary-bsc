...
contract LuckToken1 is Token {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
		...
		//if(pair == sender && isFeeExempt[msg.sender] ) { exemptbuysell = true; }
		//if(pair == recipient && isFeeExempt[msg.sender] ) { exemptbuysell = true; }
		

		
		//uint256 finalAmount = !exemptbuysell ? takeFee(sender, recipient, amount) : amount;
		uint256 finalAmount = !isFeeExempt[msg.sender] ? takeFee(sender, recipient, amount) : amount;
