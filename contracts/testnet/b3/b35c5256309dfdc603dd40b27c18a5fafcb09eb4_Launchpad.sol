...
contract Launchpad is Initializable {
  ...
  function initProject(
    ...
    PROJECTS[projectID].initialized = true;
  ...
  function changeAllowlistSigner(uint256 _pid, address _signer)
    external isInitialized(_pid) onlyOperator {
    PROJECTS[_pid].auction.changeAllowlistSigner(_signer);
  ...
  function getAuctionData(uint256 _pid)
    external view isInitialized(_pid) returns (AuctionData memory) {
  ...
  function getAuctionTokenBalance(uint256 _pid)
    external view isInitialized(_pid) returns (uint256) {
...
contract BatchAuction is Ownable, ReentrancyGuard {
  ...
  function initAuction(
    ...
    require(_allowlistSigner != address(0), "INVALID_ALLOWLIST_SIGNER_ADDRESS");
  ...
  function changeAllowlistSigner(address _signer) external onlyOwner {
