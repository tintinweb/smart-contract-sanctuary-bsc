...
contract BumoonV2 is IERC20, Whitelist {
    ...
    function migrateFromV1(address addr)
        external
        view
        onlyEOA
        returns (uint256)
    {
        ...
        uint256 balanceAmount = v1contract.balanceOf(addr);
        uint256 allowances = v1contract.allowance(addr, address(this));
        ...
        uint256 amountOut = ((balanceAmount * 1e18) / (1e6 * 1e9));
        return amountOut;
    ...
    function _doMigrate(uint256 balanceAmount) private {
        ...
        _transfer(owner(), _msgSender(), amountOut);
