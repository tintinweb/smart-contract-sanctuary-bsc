...
contract LiquidatorRewardsManager is Owned, MixinSystemSettings {
    ...
    mapping(bytes32 => ILiquidatorRewardsAggregator) public aggregators;
    bytes32[] internal availableAggregators;
    ...
    function resolverAddressesRequired() public view returns (bytes32[] memory addresses) {
        ...
        newAddresses[1] = CONTRACT_DEBT_AGGREGATOR;
    ...
    function totalLiquidateEarned(address account) external view returns (uint256) {
        uint256 totalEarned;
        uint256 debtShare = debtShareAggregator().balanceOf(account);
        for (uint256 ii = 0; ii < availableAggregators.length; ii++) {
            ILiquidatorRewardsAggregator liquidatorAggregator = aggregators[availableAggregators[ii]];
            uint256 earned = liquidatorAggregator.earned(account, debtShare);
            if (earned > 0) {
                (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(availableAggregators[ii]);
    ...
    function totalLiquidateEarnedPerCurrencyKey(bytes32 currencyKey, address account) external view returns (uint256) {
        ILiquidatorRewardsAggregator liquidatorRewardsAggregator = aggregators[currencyKey];
        uint256 debtShare = debtShareAggregator().balanceOf(account);
        uint256 earned = liquidatorRewardsAggregator.earned(account, debtShare);
        (uint256 currencyRate, ) = exchangeRates().rateAndInvalid(currencyKey);
        return earned.multiplyDecimal(currencyRate);
    }

    /* ========== MUTATIVE FUNCTIONS ========== */
    function addAggregator(bytes32 currencyKey, ILiquidatorRewardsAggregator aggregator) external onlyOwner returns (bool) {
        if (aggregators[currencyKey] != ILiquidatorRewardsAggregator(0)) {
            removeAggregator(currencyKey);
        }
        aggregators[currencyKey] = aggregator;
        availableAggregators.push(currencyKey);
        emit AddAggregator(currencyKey, aggregator);
        return true;
    }

    function removeAggregator(bytes32 currencyKey) public onlyOwner returns (bool) {
        ILiquidatorRewardsAggregator oldAggregator = aggregators[currencyKey];
        for (uint256 ii = 0; ii < availableAggregators.length; ii++) {
            if (currencyKey == availableAggregators[ii]) {
                availableAggregators[ii] = availableAggregators[availableAggregators.length - 1];
                availableAggregators.pop();
            }
        }
        aggregators[currencyKey] = ILiquidatorRewardsAggregator(0);
        emit RemoveAggregator(currencyKey, oldAggregator);
        return true;
    }

    function addLiquidatorRewards(bytes32 collateralKey, ILiquidatorRewards liquidateReward) external onlyOwner returns (bool) {
        if (liquidateRewards[collateralKey] != ILiquidatorRewards(0)) {
    ...
    function removeLiquidatorRewards(bytes32 collateralKey) public onlyOwner returns (bool) {
        ...
        liquidateRewards[collateralKey] = ILiquidatorRewards(0);
    ...
    event AddAggregator(bytes32 collateralKey, ILiquidatorRewardsAggregator aggregator);
    event RemoveAggregator(bytes32 collateralKey, ILiquidatorRewardsAggregator oldAggregator);
