...
contract PrivateV2UpgradeableV2 is Initializable, Context, AccessControlEnumerable {
    // whitelist ROLE
    ...
    function initialize(address bicAddr, address birAddr, address bgtAddr, address busdAddr, address whitelistAddr) public initializer {
    ...
    function expectResult(uint256 buyAmountBUSD, uint256 remainBIC, uint256 birUser, bool isBurnBIR, bool isCoreTeam) public view returns (uint256, uint256, uint256, uint256) {
        ...
        if (isBurnBIR) {
            uint256 minBIRByRemainBIC = remainBIC.mul(rateAirdropByBurnBIR.denominator).div(rateAirdropByBurnBIR.numerator.add(rateAirdropByBurnBIR.denominator));
            birBurn = min(min(birUser, bicByBUSD), minBIRByRemainBIC);
    ...
    function buy(uint256 buyAmountBUSD, uint256 burnAmountBIR, bool isBurnBIR) public {
        ...
        bool isPriorityRound = isPriority(msg.sender, isBurnBIR);
        if (isPriorityRound) {
        ...
        (uint256 bicByBUSD, uint256 birBurn, uint256 bicAirdropByBurnBIR, uint256 bicAirdropForCore) = expectResult(buyAmountBUSD, remainBIC, burnAmountBIR, isBurnBIR, isCoreTeam);
        if (isPriorityRound && !isCoreTeam) {
            require(
                birBurn >= 1000 * 1e18,
                "Amount BIR needs to burn in priority minimum is 1000 for community."
            );
        }
