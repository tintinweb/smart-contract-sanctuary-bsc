...
contract ProtocolStats is OwnableUpgradeSafe {
    ...
    function insuranceTotalDollarValue() public view returns (uint256) {
        ...
        return _priceChecker.getTokenPriceToUsd(v3s).mul(_v3sBalance).add(
            _priceChecker.getTokenPriceToUsd(vshare).mul(_vshareBalance)
        ).add(
            _priceChecker.getTokenPriceToUsd(wcro).mul(_wcroBalance)
        ).add(
            _priceChecker.getTokenPriceToUsd(vvs).mul(_vvsBalance)
        );
    }
    ...
    function tokenStats() external view returns (
        ...
        uint256 _vbondPremiumRate;
        {
            ...
            v3sSafePrice = insuranceTotalDollarValue().div(v3sCirculation);
        }
        vbondPrice = v3sPrice;
