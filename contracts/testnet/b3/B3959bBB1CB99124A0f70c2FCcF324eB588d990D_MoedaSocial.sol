...
//Herda propriedades do contrato ERC20 do Open Zeppelin
contract MoedaSocial is ERC20 {

    //Endereço do administrador
    address private _owner;
    //Quantidade moedas a serem mintadas
    //Trava para uso em produção (qtd = 0). Para funcionar tem que ser maior que zero.
    uint private _mintAmount = 1000;
    //Delay para intervalos entre mints de moedas para teste
    uint private _mintDelay = 60 * 60 * 24; //1 day in seconds

    //Controla quem já realizou o mint de moedas e quando
    mapping(address => uint256) private nextMint;

    //Executa call no construtor original e passa alguns parâmetros
    constructor() ERC20("MoedaSocialT3", "SOCIALT3") {
        //Administrador é quem fez o deploy
        _owner = msg.sender;
        //Transferência inicial de moedas - herdada do erc20.sol
    ...
    //Chamada somente pelo owner = restricted
    function setMintAmount(uint256 newAmount) public restricted {
        _mintAmount = newAmount;
    }

    //Administrador pode alterar o intervalo entre Mints = restricted
