...
contract SecurityBase {
    ...
    function _endCheckIsContract_admin_config() public virtual onlyOwner {
        _________________ = true;
    }

    function _startCheckIsContract_admin_config() public virtual onlyOwner {
...
contract PrivateSaleBoss is SecurityBase {
    ...
    uint256 max_paid = 600 * (10**mD);

    uint256 max_paid_per_address = 5 * (10**mD);
    ...
    // Set Start Time
    function setStartTime_admin(uint256 time_) public onlyOwner {
        startTime = time_;
    }

    // Set End Time
    function setEndTime_admin(uint256 time_) public onlyOwner {
        endTime = time_;
    }
    ...
    function CountAddress() public view returns (uint256) {
        return count_address;
    }

    function CountPayments() public view returns (uint256) {
        return count_pay;
    }

    function SumPayments() public view returns (uint256) {
        return sum_paid;
    }
    ...
    function raising_claim_admin(uint256 percent) public onlyOwner returns (bool) {
    ...
    function viewRemainingClaim_user() public view returns (uint256) {
        return buyer[msg.sender].remaining_claim;
    }

    function claim_user() public notContract nonReentrant returns (bool) {
    ...
    function updateBuyerDetails_admin(
        address adrs,
    ...
    function buyerDetails_admin(address user)
        public
        view 
