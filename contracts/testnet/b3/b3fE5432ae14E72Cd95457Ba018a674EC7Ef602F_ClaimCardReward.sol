...
contract ClaimCardReward is Ownable, Pausable {
    iCardNFT public CARD_ADDRESS;
    uint256 CARD_TYPE_ID;
    ...
    event Claimed(address receiver, uint256 cardTypeId);

    constructor (iCardNFT _cardAddress, uint256 _cardTypeId) {
        CARD_ADDRESS = _cardAddress;
        CARD_TYPE_ID = _cardTypeId;
    }

    function setCardContractAddress(iCardNFT _cardAddress) external onlyOwner {
        CARD_ADDRESS = _cardAddress;
    }

    function addWinners(address[] calldata _addresses) external onlyOwner {
    ...
    function claim() external whenNotPaused {
        ...
        CARD_ADDRESS.mint(_msgSender(), CARD_TYPE_ID);

        emit Claimed(_msgSender(), CARD_TYPE_ID);
    }
