...
contract Whitelist is Ownable {
    ...
    mapping(address => bool) public guaranteedWhitelist;
      mapping(address => bool) public fcfsWhitelist;
    address[] public guaranteedAddresses;

      address[] public fcfsAddresses;
  ...
  uint256 public phase = 0;
    ...
    modifier onlyWhitelisted() {
        if(hasWhitelisting){
            if (phase == 1) {
             require(isGuaranteedWhitelisted(msg.sender), "Not whitelisted in GA");
             } 
             else if (phase == 2) {
              require(isFcfsWhitelisted(msg.sender), "Not whitelisted in fcfs");
             }
    ...
    function addToGuaranteed(address[] memory _addresses) public onlyOwner {
        for (uint i = 0; i < _addresses.length; i++) {
            require(guaranteedWhitelist[_addresses[i]] != true);
            guaranteedWhitelist[_addresses[i]] = true;
            guaranteedAddresses.push(_addresses[i]);
        }
        emit AddedToWhitelist(_addresses);
    }

    function removeFromGuaranteed(address _address, uint256 _index) public onlyOwner {
        require(_address == guaranteedAddresses[_index]);
        guaranteedWhitelist[_address] = false;
        delete guaranteedAddresses[_index];
        emit RemovedFromWhitelist(_address);
    }

    function getAddressesForGuaranteed() public view returns(address[] memory) {
        return guaranteedAddresses;
    } 

    function isGuaranteedWhitelisted(address _address) public view returns(bool) {
        return guaranteedWhitelist[_address];
    }


      function addToFcfs(address[] memory _addresses) public onlyOwner {
        for (uint i = 0; i < _addresses.length; i++) {
            require(fcfsWhitelist[_addresses[i]] != true);
            fcfsWhitelist[_addresses[i]] = true;
            guaranteedAddresses.push(_addresses[i]);
        }
        emit AddedToWhitelist(_addresses);
    }

    function removeFromFcfs(address _address, uint256 _index) public onlyOwner {
        require(_address == guaranteedAddresses[_index]);
        fcfsWhitelist[_address] = false;
        delete guaranteedAddresses[_index];
        emit RemovedFromWhitelist(_address);
    }

    function getAddressesForFsfc() public view returns(address[] memory) {
        return guaranteedAddresses;
    } 

    function isFcfsWhitelisted(address _address) public view returns(bool) {
        return fcfsWhitelist[_address];
    }
}
 ...
 *Submitted for verification at Etherscan.io on 2021-02-15
*/
contract FixedSwapv3 is Pausable, Whitelist {
    using SafeMath for uint256;
    uint256 increment = 0;

    mapping(address => uint256) public gaRafflesTicketCount; 
    mapping(address => uint256) public fcfsRafflesTicketCount; 
    ...
    uint256 public phaseEndDate; /* Phase End Date */
 ...
 //   uint256 public secondUnlockDate;
    ...
    event ChangePhase(uint256 phase, uint256 timestamp);

    constructor(address _tokenAddress, address _currencyTokenAddress, uint256 _tradeValue, uint256 _tokensForSale, uint256 _startDate, 
        ...
        currencyToken = ERC20(_currencyTokenAddress);
    ...
    function setPhase(uint256 _phase, uint256 _phaseEndDate) external onlyOwner
    {
        if (_phase == 1) {
        ...
        emit ChangePhase(_phase, _phaseEndDate);
    }

    function getRafflesNormalAllocation(address walletAddress) public view returns(uint256) {
        uint256 ticketCount = fcfsRafflesTicketCount[walletAddress];
    ...
    function getRafflesGuaranteedAllocation(address walletAddress) public view returns(uint256) {
        uint256 ticketCount = gaRafflesTicketCount[walletAddress];
    ...
    function setGARafflesTicketCount(address[] memory wallets, uint256[] memory rafflesCount) external onlyOwner {
        require(wallets.length == rafflesCount.length, "Invalid length");

        for (uint256 i = 0; i < wallets.length; i++) {
            gaRafflesTicketCount[wallets[i]] += rafflesCount[i];
        }
    }

  function setFcfsRafflesTicketCount(address[] memory wallets, uint256[] memory rafflesCount) external onlyOwner {
        require(wallets.length == rafflesCount.length, "Invalid length");

        for (uint256 i = 0; i < wallets.length; i++) {
            fcfsRafflesTicketCount[wallets[i]] += rafflesCount[i];
        }
    }
  ...
  function setUnlockDates(uint _startDate, uint _endDate, uint _lastUnlockDate) public onlyOwner {
       // require(lastUnlockDate == 0, "already set");

        require(_startDate < _endDate && _endDate < _lastUnlockDate, "invalid input");
        lastUnlockDate = _lastUnlockDate;
     ...
     function getLocked(uint256 purchase_id) public view returns(uint) {
        if(block.timestamp > lastUnlockDate) {
            return 0;
        } else {
            uint256 diff = lastUnlockDate - now;
            uint256 totalDuration = lastUnlockDate - endDate;

            return  purchases[purchase_id].amount.mul(diff).div(totalDuration);
        }
    ...
    /* Action Functions */
    function swap(uint256 _amount, uint256 _currencyTokenAmount) external whenNotPaused isFunded isSaleOpen onlyWhitelisted {
        ...
        require(_currencyTokenAmount == cost(_amount), "User has to cover the cost of the swap in ETH, use the cost function to determine");
        ...
        currencyToken.transferFrom(msg.sender, address(this), _currencyTokenAmount);

        uint256 purchase_id = increment;
        increment = increment.add(1);

        /* Create new purchase */
        Purchase memory purchase = Purchase(_amount, _amount, msg.sender, _currencyTokenAmount, block.timestamp, isTokenSwapAtomic /* If Atomic Swap */, false);
    ...
    function redeemAllTokens() external isNotAtomicSwap isSaleFinalized whenNotPaused  {
        ...
        for (uint i=0;i<myPurchases2.length;i++) {
           // emit DebugEvent(myPurchases2[i]);
            ...
            uint256 purchase_id = myPurchases2[i];
    ...
    /**
     * Return latest information about the IDO
     */
    function getLatestInformation() public view returns (
        address,
        bool,
        uint256,
        uint256,
