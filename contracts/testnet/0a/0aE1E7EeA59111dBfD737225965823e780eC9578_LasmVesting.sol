...
contract LasmVesting is Ownable {
    ...
    function claimByScheduleName11(string memory _templateName) external {
        require(vestingToken != address(0), "Vesting: VESTINGTOKEN_NO__SET");
...
//        registerEvent(msg.sender, index, claimAmount);
    ...
    function claimByScheduleName44(string memory _templateName) external {
        require(vestingToken != address(0), "Vesting: VESTINGTOKEN_NO__SET");

        uint8 index = schedulesByName[_templateName];
        require(index >= 0 && index < schedules.length, "Vesting: NOT_SCHEDULE");

        aaa = beneficiary[_templateName];
        bbb = msg.sender;
    }

    function registerEvent(address _account, uint8 _scheduleIndex, uint256 _claimedTokens) public {
