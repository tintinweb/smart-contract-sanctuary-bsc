 ...
 * where they daily receive 3.5% from auto staking rewards daily compounded for 45 days until the max supply is reached.
 * In addition to 5% BUSD Rewards fixed for every buy and sell.
 *
 * Website : https://everhodl.com
...
contract EverHODL is ERC20Detailed, Ownable {
    ...
    uint256 private constant MAX_SUPPLY = 500 * 10**5 * 10**DECIMALS;

    uint256 public liquidityFee = 25;
    uint256 public treasuryFee = 35;
    uint256 public busdDividendFee = 50;
    uint256 public sellFee = 0;
    uint256 public totalFee = liquidityFee.add(treasuryFee).add(busdDividendFee);
    uint256 public feeDenominator = 1000;
    uint256 public _initRebaseStartTime;
    uint256 public _lastRebasedTime;
    uint256 public _lastAddLiquidityTime;
    uint256 public _swapEnabledTime;
    uint256 public _minSharesRequired = 100;
    ...
    address public eHodlDividendReceiver;
    ...
    event FeesChanged(uint256 liquidityFee, uint256 treasuryFee, uint256 busdDividendFee);
    event minShresRequiredChanged(uint256 minSharesRequired);
    event SwapEnabled(bool swapEnabled);
    event FeeReceiversChanged(address autoLiquidityReceiver, address treasuryReceiver);

    constructor() ERC20Detailed("Ever HODL", "eHODL", uint8(DECIMALS)) Ownable() {
        ...
        //contract addreses
        autoLiquidityReceiver = 0x1ED0d8bDe02089953C6C9FEEA600A0AAe9240137;
        ...
        eHodlDividendReceiver = address(distributor);
    ...
    function setFees(uint256 _liquidityFee, uint256 _treasuryFee, uint256 _busdDividendFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        treasuryFee = _treasuryFee;
    
        totalFee = liquidityFee.add(treasuryFee).add(busdDividendFee);
        require(totalFee <= 2100, "Must keep fees at 21% or less");

        emit FeesChanged(_liquidityFee, _treasuryFee, _busdDividendFee);
    }
    ...
    function setFeeReceivers(
        ...
        emit FeeReceiversChanged(_autoLiquidityReceiver, _treasuryReceiver);      
    }
