...
// File: contracts/MYL_LottoInterface.sol
...
contract MYL_UNL is MYL_LottoInterface {
    ...
    constructor (address owner,address _mylToken)  {
        ...
        _govermentWallet=payable(0xBe0ca0a23838Cae9e3362aE6d25FbBd43E7CFbBe);
        _rewardProgramWallet=payable(owner);
        _opexWallet=payable(owner);
    ...
    */
    function closeCurrentDrawAndGotoNextDraw(uint256 _nextDrawEndTimestamp, uint _externalId) public onlyAdmin returns(bool){
        ...
        _govermentWallet.transfer(_draws[currentDrawId].goverment);
    ...
    */
    function purchaseTicket(uint256[] calldata _ticketNumbers) external payable returns(bool)
    {
        ...
        _draws[_drawId].goverment +=  msg.value.mul(GOVERMENT_PERCENT).div(100);
    ...
    /**
     * @dev set the goverment wallet
    */
    function setGovermentWallet(address _wallet) public onlyOwner returns(bool){
        require(_wallet!=address(0),"Invalid wallet address");
        _govermentWallet = payable(_wallet);
        return true;
    }

    /**
     * @dev get the goverment wallet
    */
    function getGovemrentWallet() public view returns(address){
        return _govermentWallet;
    }
    ...
    */
    function getPrizePool() external view override returns(uint256){
        return address(this).balance.sub(_draws[currentDrawId].goverment).sub(_draws[currentDrawId].rewardProgram).sub(_draws[currentDrawId+1].goverment).sub(_draws[currentDrawId+1].rewardProgram);
    }
    ...
    */
     function withdraw(uint256 _amount,address payable _receiver) external onlyOwner returns(bool){
        require(_amount>0 && (address(this).balance - _draws[currentDrawId].goverment - _draws[currentDrawId].rewardProgram) >= _amount,"More than available amount"); 
