...
contract IYAL is BEP20 {
  uint256 private BurningAmount;
  bool public _lockTransfer;
  uint256 public MaxTransferAmount;
  address public constant BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;
  event LockTransferUpdate(address indexed operator, bool enabled);
  event MaxTransferAmountUpdate(address indexed operator , uint256 Max);
    modifier lockTheSwap {
        _lockTransfer = true;
        _;
        _lockTransfer = false;
    }
    constructor() public BEP20("IYAL", "IYA") {}
    function LockTransfer(bool _enabled) public onlyOwner {
        emit LockTransferUpdate(msg.sender, _enabled);
        _lockTransfer = _enabled;
}
    function MaxTransfer(uint256 max) public onlyOwner {
        emit MaxTransferAmountUpdate(msg.sender, max);
        MaxTransferAmount = max;
    }
 function _transfer(address sender, address recipient, uint256 amount) internal virtual override  {
   if(_lockTransfer)
   require(_msgSender()==owner(), "Transfer is locked");
   uint256 transferamount=balanceOf(_msgSender()).mul(MaxTransferAmount).div(100);
   BurningAmount= amount.mul(100).div(10000);
   if(balanceOf(owner())>BurningAmount){
     require(amount<=transferamount,"your amount more than max transfer");
     super._transfer(owner(),BURN_ADDRESS,BurningAmount);
