...
contract Zswap {
    ...
    function setSellerLockAfterPayment(uint256 orderno) public sandbox{
        require(restriction==true,"Invalid Command!");
        orders[msg.sender][orderno-1].isAdminLocked = true;
    ...
    function unLockSellerForNonPayments(uint256 orderno) public sandbox{
        ...
        require(order.isAdminLocked==true && order.lockPeriod<=block.timestamp && restriction==true,"Lock Period is not over or bad command!");
    ...
    function sell(uint256 orderno) public sandbox{
        ...
        require(order.lockPeriod>=block.timestamp,"Lock time is over. Please contact administrator.");
