...
contract OilIDO is Ownable, ReentrancyGuard {
    ...
    function getNodeNum(uint8 _type) public view returns (uint32) {
        require (_type == 2 || _type == 3, "Wrong type");
    ...
    function getSecondPushRewardHistory(address _owner, address directPushAddr) external view returns (Reward[] memory historys) {
        historys = secondPushRewardHistory[_owner][directPushAddr];
    }
}