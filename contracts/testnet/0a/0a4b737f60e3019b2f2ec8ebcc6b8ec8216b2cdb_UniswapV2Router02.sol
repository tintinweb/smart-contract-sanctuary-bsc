...
//                hex'e18a34eb0e04b04f7a0ac29a6e80748dca96319b42c54d679cb821dca90c6303' // init code hash
                hex'b1730b3fa01496c60cb7f5ed8d95314e81d1ce39dfd3df4948c0d36c9931a966' // init code hash
            ))));
    }
    ...
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint fee) internal pure returns (uint amountOut) {
        ...
        uint amountInWithFee = amountIn.mul(uint(100).sub(fee));
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut, uint fee) internal pure returns (uint amountIn) {
        ...
        uint denominator = reserveOut.sub(amountOut).mul(uint(100).sub(fee));
...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function addLiquidity(
        ...
        amountArr[0] = amountA.mul(98).div(100);
        amountArr[1] = amountB.mul(98).div(100);
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountArr[0]);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountArr[1]);
        address feeTo = IUniswapV2Factory(factory).feeTo();
        TransferHelper.safeTransferFrom(tokenA, msg.sender, feeTo, amountA.sub(amountArr[0]));
        TransferHelper.safeTransferFrom(tokenB, msg.sender, feeTo, amountB.sub(amountArr[1]));
