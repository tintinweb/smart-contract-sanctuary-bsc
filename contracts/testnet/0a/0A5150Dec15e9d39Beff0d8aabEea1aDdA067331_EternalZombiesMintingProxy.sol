...
interface IEternalZombies {
    ...
    function tokenURI(uint) external returns(string memory);
    function nftPrice() external returns(uint);
...
contract EternalZombiesMintingProxy is Ownable {
    ...
    function tokenURI(uint _tokenId) public returns(string memory) {
