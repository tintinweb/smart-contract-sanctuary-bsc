...
contract MyFundingNetwork is Ownable, ReentrancyGuard {
    ...
    uint256 public withdrawPeriod = 5 minutes;
    uint256 public apr = 1000; // 150 : 1.5 %. 10000 : 100 %
    ...
    struct DepositStruct {
        ...
        uint256 withdrawAt; // withdraw timestamp
    ...
    function withdrawRef() public nonReentrant {
        ...
        (bool success, ) = msg.sender.call{
            value: (players[msg.sender].match_bonus).div(reinvestRate)
        }("");
        require(success, "Failed to claim reward");



        players[msg.sender].reinvest_bonus += (players[msg.sender].match_bonus).div(reinvestRate);
     //   totalDeposited += (players[msg.sender].match_bonus).div(reinvestRate);
        players[msg.sender].total_invested += (players[msg.sender].match_bonus).div(reinvestRate);

        players[msg.sender].match_bonus = 0;
    }

    // withdraw capital by deposit id
    function withdrawCapital() public nonReentrant {
    //    require(
     //       depositState[id].investor == msg.sender,
     //       "only investor of this id can claim reward"
     //   );
    ...
    //function Total_match_bonus() public view returns (uint256) {
    //    return players[msg.sender].total_match_bonus;
    //}

    // function Match_bonus() public view returns (uint256) {
    //    return players[msg.sender].match_bonus;
    //}
