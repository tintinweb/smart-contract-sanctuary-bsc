...
contract AETHR_Marketplace is ReentrancyGuard {
    ...
    uint256 _maxFeeListingFuture = 1000;
    uint256 _feeListingFuture = 150;

    uint256 _maxFeeMarketFuture = 1000;
    uint256 _feeMarketFuture = 150;
    ...
    function createSale(
        ...
        AETHR_NFT(NFT_Factory).transferFrom(msg.sender, address(this), tokenId);
    ...
    /* Places an item for sale on the marketplace */
    function createSaleFuture(
        uint256 tokenId,
        uint256 priceItem
    ) public nonReentrant {
        require(priceItem > 0, "Price must be at least 0");

        require(priceItem > calculateFee(priceItem, _feeListingFuture), "Price must be equal to listing price");
        ...
        BUSD.transferFrom(
            msg.sender,
            address(this),
            calculateFee(priceItem, _feeListingFuture)
        );
        AETHR_NFT(NFT_Factory).transferFrom(msg.sender, address(this), tokenId);

        emit MarketItemFutureCreated(
            itemId,
            NFT_Factory,
            tokenId,
            msg.sender,
    ...
    function buyNFT(uint256 tokenId) public nonReentrant {
        ...
        require(
            BUSD.allowance(address(this), msg.sender) >= price,
            "eror allowance"
        );
        require(
        ...
        AETHR_NFT(NFT_Factory).transferFrom(address(this), msg.sender, tokenId);
    ...
    function setFeeListingFuture(uint256 fee) public onlyOwner(msg.sender) {
        require(fee <= _maxFeeListing, "Error input, fee < 500");
        _feeListingFuture = fee;
    }

    function setFeeMarketFuture(uint256 fee) public onlyOwner(msg.sender) {
        require(fee <= _maxFeeMarket, "Error input, fee < 500");
        _feeMarketFuture = fee;
    }
