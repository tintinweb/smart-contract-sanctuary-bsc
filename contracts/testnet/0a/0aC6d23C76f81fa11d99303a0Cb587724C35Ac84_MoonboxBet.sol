...
contract MoonboxBet is ReentrancyGuard, Ownable {
    ...
    function createBet(string memory betId, uint256 endTime)
        external
        nonReentrant
        onlyOwner
    {
    ...
    function placeBet(
        ...
        require(_bet.enable == true, "Not found");
        require(_bet.endTime > block.timestamp, "End time");
        require(_bet.status == 1, "Finished");
        if (_bet.players[msg.sender].amount > 0) {
