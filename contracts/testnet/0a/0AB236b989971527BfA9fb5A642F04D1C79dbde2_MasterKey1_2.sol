...
contract MasterKey1_2 is ERC721A, Ownable,Pausable,IMK {
   ...
   string private URL;
    ...
    function MINT_Price_MasterKey_1_2 (address adr, uint256 amount)  public  {
        address sender= msg.sender;
        uint256 Am = amount;
        dis.MITItrans(sender, amount, ID_gnft);
    ...
    function SkillUP_1_2 (uint256 ID)  public {
        address sender =msg.sender;
        uint256 Sid = Skill[ID];
        uint256 skE = skillUP_c.getSkillUP_EX_1(Sid);
        uint256 EXid = EX[ID];
        uint256 W = Wear[ID];
        ...
        EX[ID]=EXid - skE;
        Wear[ID]=W+skillUP_c.getSkillUP_Wear_1(Sid);
        Skill[ID]=Sid+1;
    }

    function WearUP_1_2 (uint256 ID, uint256 num_RepairKit) public {
         address sender =msg.sender;
        skillUP_c.GNFTtrans(num_RepairKit,sender);
        Wear[ID] = Wear[ID] + skillUP_c.GetWearPlus(num_RepairKit);
    }

    function OpeningMasterKey_1_2(uint256 ID, uint256 ChestLevel_) public {
    ...
    return string(abi.encodePacked(URL,_ID));
  }
