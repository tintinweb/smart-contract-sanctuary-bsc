...
abstract contract Ownable is Context {
    ...
    function transferAdminship(address newAdmin) public virtual onlyAdmin {
        require(newAdmin != address(0), "Ownable: new admin can not be the zero address");
        _setAdmin(newAdmin);
    }
...
contract FAIRDOGE is Context, IBEP20, Ownable {
    ...
    uint256 private _tTotal = 420 *10**12 * 10**_decimals; // TODO
    uint256 private _rTotal = (MAX - (MAX % _tTotal));

    uint256 public swapTokensAtAmount = 1e14 * 10**_decimals;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0xd932AAe70676C50E7013aE0Ac101BdCA238995D0;

    string private constant _name = "Fair Doge";
    string private constant _symbol = "FairDoge";

    struct Taxes {
        uint256 rfi;
        uint256 marketing;
    }
    // tax 5% reflection, 3% mkt
    Taxes public taxes = Taxes(5, 3);
    ...
    function bulkExcludeFee(address[] memory accounts, bool state) external onlyAdmin {
    ...
    function rescueBNB(uint256 weiAmount) external onlyAdmin {
        require(address(this).balance >= weiAmount, "insufficient BNB balance");
        payable(msg.sender).transfer(weiAmount);
    }

    //Use this in case BEP20 Tokens are sent to the contract by mistake
    function rescueAnyBEP20Tokens(address _tokenAddr,address _to, uint256 _amount) public onlyAdmin {
