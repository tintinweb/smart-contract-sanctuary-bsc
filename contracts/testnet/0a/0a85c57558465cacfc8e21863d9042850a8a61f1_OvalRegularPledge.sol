...
contract OvalRegularPledge is Initializable, Ownable, Pausable, ReentrancyGuard {
    ...
    // modifier nonContract(address sender) {
    //     require(!Address(sender).isContract, "Caller can't be contract");
    //     _;
    // }
    ...
    function pledge(uint256 amount, uint256 poolId) external whenNotPaused validPoolId(poolId) nonReentrant {
        ...
        require(amount > 10, "Pledge amount can't less than 10");
    ...
    */
    function claimPrinciple(uint256 pledgeId, uint256 poolId) external whenNotPaused validPoolId(poolId) nonReentrant {
        ...
        for (uint256 j = 0; j < principleMap[poolId][_msgSender()].length; j++) {
            require(principleMap[poolId][_msgSender()][j].deadline <= block.timestamp, "The pledge has not expired");
            if (principleMap[poolId][_msgSender()][j].id == pledgeId) {
                _index = j;
                isExits = true;
                break;
            }
    ...
    */
    function claimAllDuePrinciple(uint256 poolId) external whenNotPaused validPoolId(poolId) nonReentrant {
        ...
        // while(tmpIds.length > 0) {
        //     for(uint256 k = 0; k < principleMap[poolId][_msgSender()].length; k++) {
        //         if (tmpIds[tmpIds.length-1] == principleMap[poolId][_msgSender()][k].id) {
        //             deletePrinciple(k, poolId);
        //             tmpIds.pop();
        //             break;
        //         }
        //     }
        ...
        // for(uint256 k = 0; k < principleMap[poolId][_msgSender()].length; k++) {
        //     uint256 interestId = principleMap[poolId][_msgSender()][k].id;
        //     if (removedIdMap[interestId] == interestId) {
        //         deletePrinciple(k, poolId);
        //         k = k - 1;
        //     }        
        // }
    ...
    function claimAllDueInterest(uint256 poolId) external whenNotPaused validPoolId(poolId) nonReentrant {
        ...
        // while(tmpIds.length > 0) {
        //     for(uint256 k = 0; k < interestMap[poolId][_msgSender()].length; k++) {
        //         if (tmpIds[tmpIds.length-1] == interestMap[poolId][_msgSender()][k].id) {
        //             deleteInterest(k, poolId);
        //             tmpIds.pop();
        //             break;
        //         }
        //     }
        ...
        //     uint256 index = removedIdMap[tmpIds[i]] - i; 
        //     deleteInterest(index, poolId);
        // }
    ...
    function claimInterest(uint256 interestId, uint256 poolId) external whenNotPaused validPoolId(poolId) nonReentrant {
        ...
        bool isExits;
        for (uint256 j = 0; j < interestMap[poolId][_msgSender()].length; j++) {
            require(interestMap[poolId][_msgSender()][j].deadline <= block.timestamp, "The reward has not expired");
            if (interestMap[poolId][_msgSender()][j].id == interestId) {
                _index = j;
                isExits = true;
                break;
            }
