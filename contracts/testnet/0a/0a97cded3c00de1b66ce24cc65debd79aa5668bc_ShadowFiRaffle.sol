...
contract ShadowFiRaffle is Ownable, ReentrancyGuard {
    ...
    struct RaffleInfo {
        ...
        mapping (address => uint256) ticketCountByUser;
        uint256 deadAccrual;
    ...
    function deposit(address user, uint256 ticketAmount) external payable {
        ...
        for (uint256 ticketNo = lastTicketNo + 1; ticketNo <= lastTicketNo + (ticketCount * accrual1); ticketNo++) {
            ...
            raffleInfos[currentRaffleId].ticketsOwnedByUser[addr1][raffleInfos[currentRaffleId].ticketCountByUser[addr1]++] = raffleInfos[currentRaffleId].lastSoldTicketNo;
        ...
        for (uint256 ticketNo = lastTicketNo + (ticketCount * accrual1) + 1; ticketNo <= lastTicketNo + ticketCount * (accrual1 + accrual2); ticketNo++) {
            ...
            raffleInfos[currentRaffleId].lastSoldTicketNo = raffleInfos[currentRaffleId].lastSoldTicketNo + 1;
            raffleInfos[currentRaffleId].ticketsOwnedByUser[addr2][raffleInfos[currentRaffleId].ticketCountByUser[addr2]++] = raffleInfos[currentRaffleId].lastSoldTicketNo;
    ...
    function getTicketsOwnedByUser(address _user) public view returns (uint256[] memory) {
        ...
        uint256 ownedTicketCount;

        for (uint256 i = 1; i <= raffleInfos[currentRaffleId].lastSoldTicketNo; i++) {
            if (raffleInfos[currentRaffleId].ticketOwnerList[i] == _user) {
                result[ownedTicketCount] = i;
                ownedTicketCount ++;
            }
        }
    ...
    function getPoolInfo() public view returns (uint256 mainPoolBNB, uint256 mainPoolPercentage, uint256 mainCarryover, uint256 secondPoolBNB, uint256 secondPoolPercentage, uint256 secondCarryover, uint256 treasuryPercentage) {
        return (poolInfo.mainPool, poolInfo.mainPoolPercent, poolInfo.mainCarryPercent, poolInfo.secondPool, poolInfo.secondPoolPercent, poolInfo.secondCarryPercent, poolInfo.treasuryPercent);
    }
