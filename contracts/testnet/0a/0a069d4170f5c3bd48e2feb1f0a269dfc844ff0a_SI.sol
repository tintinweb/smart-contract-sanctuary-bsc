...
// The address of the player and => the user info
   ...
   function() public payable {}
    ...
    function allocatePrizes(uint _gameId, uint16 teamWinner) public onlyAgent {
        ...
        address[] memory winners = new address[](winners.length);
        address[] memory draw = new address[](draw.length);
        ...
        for(i = 0; i < count; i++){
            ...
            // token.transfer(winners[j],  (bets*(10000+(LoserBet*fee /WinnerBet)))/10000 );
             rewards = (bets*(10000-fee *(totalBets / totalBetsOne))) / 10000;
        ...
        for( i = 0; i < count; i++){
            ...
            rewards = (bets*(10000-fee *(totalBets / totalBetsTwo))) / 10000; 
            _pendingBalance[add] += rewards;
            }
        }
