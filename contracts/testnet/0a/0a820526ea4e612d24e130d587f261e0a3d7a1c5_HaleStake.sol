...
contract HaleStake is Owned {
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(hale).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        //makes sure amount is more than the registration fee and the minimum deposit
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough HALE to pay registration fee.");

        require(IERC20(hale).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(hale).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(hale).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient HALE balance in pool');
        ...
        IERC20(hale).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(hale).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(hale).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient HALE balance in pool');
        //transfers _amount to _address
        IERC20(hale).transfer(msg.sender, _amount);
        //emit event
