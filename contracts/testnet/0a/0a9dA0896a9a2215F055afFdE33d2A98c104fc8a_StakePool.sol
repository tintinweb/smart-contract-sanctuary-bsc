 ...
 */
    function sendReward( address to, uint256 amount) external onlyOwnerOrAdmin {
 ...
 */
    function removeAdmin(address formerAdmin) public onlyOwner {
        require(isAdmin[formerAdmin], "The address is not an admin");
        isAdmin[formerAdmin] = false;
    }

    modifier onlyOwnerOrAdmin() {
        require(msg.sender == owner() || isAdmin[msg.sender], "Sender must be owner or admin");
        _;
    }
...
contract StakePool is Ownable {
    ...
    */
    ...
    function deposit(uint256 amount) external {
        ...
        userInfo.rewardDebt = userInfo.amount * rewardPercentage / 1e12;
 ...
 * @dev Withdraws all tokens without caring about the reward.
    ...
    function getPendingReward(address user) public view returns (uint256) {
        ...
        if (lastProfitableBlock > lastRewardBlock && totalStakedTokens != 0) {
            ...
            uint256 userRewardPerBlock = userInfo.rewardDebt / lockingPeriodBlock;
            reward = userRewardPerBlock * elapsedBlocks;
    ...
    function _sendReward(address user) internal {
        ...
        if (pendingReward > 0) {
            UserInfo storage userInfo = usersInfo[user];
            //userInfo.rewardDebt = userInfo.amount * accRewardPerShare / 1e12;
            userInfo.rewardDebt = userInfo.rewardDebt - pendingReward;
