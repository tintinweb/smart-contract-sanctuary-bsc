...
contract LuckyMe is Ownable {
  ...
  ********************************************************/
  ...
  function currentGameDetail(
    uint8 _game
  ) public view returns (uint256 _GameId, currentGame memory _GameDetail) {
    _GameId = GameIds[_game].current();

    game storage _Game = Game[_game][_GameId];
    _GameDetail = currentGame(
      _Game.GameOver,
      _Game.Winners,
      _Game.AllNumbers,
  ...
  function userDetail(uint256 _id) public view returns (user memory _User) {
    _User = User[_id];
  }
}
