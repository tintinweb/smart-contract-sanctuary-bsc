...
contract WhaleFat is ERC20Detailed, Ownable {
    ...
    uint256 public WFTDividendFee = 70;
    uint256 public sellFee = 30;
    uint256 public blueHole = 20;

    uint256 public totalFee =
        liquidityFee.add(treasuryFee).add(WFTDividendFee).add(
            blueHole
        );
    ...
    address public WFTDividendReceiver;
    ...
    constructor() ERC20Detailed("WhaleFat", "WHALES", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x5aeFE0474FdDcb1476F45dE189F4Fe2eB7859400;
        treasuryReceiver = 0xb1cF0AA5839906435B48a0C55eBAcbFfC2d8476d;   
        ...
        WFTDividendReceiver = address(distributor);
    ...
    function takeFee(
        ...
        uint256 _WFTDividendFee = WFTDividendFee;
        ...
        if (recipient == pair) {
            ...
            _WFTDividendFee = WFTDividendFee.add(_sell);
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_treasuryFee.add(_WFTDividendFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        (bool success, ) = payable(treasuryReceiver).call{
            value: amountETHToTreasuryAndSIF.mul(treasuryFee).div(
                treasuryFee.add(WFTDividendFee)
            ),
            gas: 30000
        }("");
        if(success){}
        try distributor.deposit {
            value: amountETHToTreasuryAndSIF.mul(WFTDividendFee).div(treasuryFee.add(WFTDividendFee))
        } () {} catch {}        
    }
    ...
    function withdrawAllToTreasury() external swapping onlyOwner {
        ...
        uint256 amountToSwap = _gonBalances[address(this)].div(_gonsPerFragment);
        require( amountToSwap > 0,"There is no WHALES token deposited in token contract");
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = router.WETH();
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(
