...
import {Incentive} from "./contracts/Incentive_Final.sol";
...
contract IncentiveFactory {
  ...
  function createIncentive(address owner_, address _token, uint256[6] memory _seed_details, uint256[6] memory _presale_details, uint256[6] memory _community_details) public returns (address) {
...
contract Incentive is ReentrancyGuard {
    ...
    uint256 internal saleSetting = 0;
    ...
    function initIncentive(uint256[6] memory _seed_details,uint256[6] memory _presale_details,uint256[6] memory _community_details) internal onlyOwner {
    ...
    function setIncentiveDetails(uint256 _round, uint256[6] memory _round_details) internal onlyOwner {
        require(saleSetting == 0, "Already set");
    ...
    function setEntrantDetails(uint256 _round, address _entrant, uint256 _amount) public {
        ...
        if(entrantDetails.ballots > initBallots) {
            diffBallots = initBallots - entrantDetails.ballots;
    ...
    function getBallotList(uint256 _round) public view returns(uint256[] memory ballot, address[] memory entrant) {
