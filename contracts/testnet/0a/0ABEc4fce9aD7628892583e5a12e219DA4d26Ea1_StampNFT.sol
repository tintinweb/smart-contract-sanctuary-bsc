...
contract AgencyNFT is OwnPauseBase, ERC721URIStorageEnumerable {
  ...
  event EvtMintToken(address receiver, uint256 tokenId, string agencyId);
  ...
  function burnToken(uint256 tokenId_) external whenNotPaused {
  ...
  // "agencyId_" will be included in the event for metadata (name, description, image) build.
  // If it is empty string, the default agencyId will be used but with empty veriSign (i.e. unverified passport token)
  function mintToken(address receiver_, string memory agencyId_)
    external
    isAuthorized
    returns (uint256)
  {
    ...
    _agency.setTokenIdAgencyInfo(_tokenId, agencyId_);

    emit EvtMintToken(receiver_, _tokenId, _agency.safeGetAgencyId(agencyId_));
    return _tokenId;
  }
  ...
  function mintManyTokens(
    ...
    emit EvtMintManyTokens(receiver_, tokenIds, _agency.safeGetAgencyId(agencyId_));
    return tokenIds;
  }
    ...
    isAuthorized
  {
    ...
    emit EvtMassMint(receiverList_, tokenIds, _agency.safeGetAgencyId(agencyId_));
  }
