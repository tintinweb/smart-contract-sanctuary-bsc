...
interface TunnelContract {
    ...
    function triggerFunction(address token_addr, uint amount, uint userID, uint packageID, string calldata descID) external returns(bool outcome);
}
...
contract Ownable
{
    ...
    address public newOwner;        // The wallet new Owner (before confirmation)
...
contract Powermade is Ownable {
    ...
    uint8[] public commission_per_level;    // Commissions for each level. 7 elements, the first is for the sponsor, other 5 for the upline (down to up) and the last is the tunnel (can be 0)
    ...
    constructor(address _token_addr, address _projectWallet, address _headWallet, address _approveWallet) public {
        ...
        // index MaxDownLimit+1 : tunnel to external contract (if any)
        // The remaining percentage is the project Fee
        commission_per_level = [10, 10, 9, 8, 6, 7, 0];
        // --------
    ...
    function BuyPackage(uint sponsorID, uint16 packageID) public {
        ...
        buyPackageInt(msg.sender, sponsorID, packageID);
    }
    ...
    function BuyPackageExt(address userAddr, uint sponsorID, uint16 packageID) public {
        // Allow access only to enabled smart contracts
        require(isContract(msg.sender) && enabled_contracts[msg.sender], "Access denied");
        buyPackageInt(userAddr, sponsorID, packageID);
    }
    ...
    function buyPackageInt(address userAddr, uint sponsorID, uint16 packageID) internal {
        require(businessEnabled, "Business non enabled");
        require(!isContract(userAddr), "Smart Contracts not Allowed");
        require(userAddr != ownerWallet && userAddr != headWallet && userAddr != projectWallet && userAddr != headPlaceholder && userAddr != approveWallet, "This wallet cannot buy");
        ...
        // User is not banned
    	require(bannedLogic ? !userInfos[userAddr].banned : userInfos[userAddr].banned, "User is banned/disabled");
        ...
        for (uint8 i = 0; i < packagesIndex[packageID].prerequisites.length; i++) {
            ...
            require(userInfos[userAddr].packagesData[currentPrerequisite].bought_array.length > 0, "Prerequisites not fulfilled");
    ...
    function payCommissions(address payer, uint16 packageID) private {
        ...
        if (commission_per_level[maxDownLimit+1] > 0 && enabled_contracts[tunnelContract]) {
            tunnel_amount = amount*commission_per_level[maxDownLimit+1]/100;
            unpaid -= tunnel_amount;
            ////// SEND money out of the tunnel (with reentrant protection)
            require(!entered, "Reentrancy Error!");
            entered = true;
            ...
            require(success, "Error executing the trigger call");
            entered = false;
        }
        // Pay the unpaid to the projectWallet
        payUser20(projectWallet, unpaid);
        userInfos[projectWallet].totalEarned += unpaid;
        emit PaidUplineEv(userInfos[payer].id, packageID, amount-unpaid-tunnel_amount-remaining, remaining, unpaid+tunnel_amount);
    }
    ...
    function registerToEvent(uint16 packageID, address eventUID, bytes32 ticket_hashed) public {
        ...
        require(userInfos[msg.sender].packagesData[packageID].bought_array.length > 0, "Package never bought!");
    ...
    function checkEventRegistration(uint16 packageID, address eventUID, address ticket) public view returns (uint reg_timestamp, uint userID, uint PID_last, uint timestamp_last, uint duration_last, bool expired_last) {
        ...
        if (userID > 0) {
            ...
            if(bought_array_PIDs.length > 0) {
                ...
                ( , , timestamp_last, duration_last, expired_last) = getPurchaseInfo(PID_last);
            }
        }
    }
    ...
    function changeBusinessWallets(address newProjectWallet, address newTunnelContract, address newHeadWallet, address newApproveWallet) public onlyOwner {
        ...
        if (newTunnelContract != address(0)) {
            require(enabled_contracts[newTunnelContract], "newTunnelContract not enabled");
            tunnelContract = newTunnelContract;
            emit ChangedFeatureParameterEv("changeTunnelContract");
        }
    ...
    function retriveTokensContract(address token, uint amount, address destination) public onlyOwner {
    ...
    function enableDisableExtContract(address external_contract, bool status) public onlyOwner {
        require(external_contract != address(0) && isContract(external_contract), "Invalid contract address");
        enabled_contracts[external_contract] = status;
        emit ChangedFeatureParameterEv("enableDisableExtContract");
    }


    // Change the commission percentages end project percentage. Every percentage (except the tunnel_percentage) must be > 0 and the sum must be 100 otherwise the transaction is rejected. 
    // Owner must define the invited (directly invited) percentage, the 5 level (upline) percentages and the tunnel_percentage. The remaining percentage must be the project percentage, that is the percentage sent to the projectWallet.
    ...
    function changePercentages(uint8 invited_percentage, uint8[] memory commission_percentages, uint8 tunnel_percentage, uint8 project_percentage) public onlyOwner {
        require(invited_percentage > 0 && project_percentage > 0, "Check the values!");
        commission_per_level[0] = invited_percentage;
        uint16 sum_perc = invited_percentage + project_percentage+tunnel_percentage;
        for (uint8 i = 0; i < maxDownLimit; i++) {
        ...
        commission_per_level[maxDownLimit+1] = tunnel_percentage;
    ...
    function getUsersInfos(address tUserAddr, uint tUserID, uint16 packageID, uint invited_start_index, uint invited_stop_index) public view returns (uint[] memory globals, address[] memory addresses, uint[] memory IDs, uint[] memory counts, uint[] memory amounts, bool banned) {
        ...
        require(tUserID > 0 && tUserID <= lastIDCount, "Invalid userID");
        ...
        // Create arrays
        globals = new uint[](7+maxDownLimit);
        addresses = new address[](8+1+invited_stop_index-invited_start_index);
        IDs = new uint[](8+1+invited_stop_index-invited_start_index);
        counts = new uint[](4);
        amounts = new uint[](2);
        // Global information: N_users | N_purchases | N_added_packages | BusinessEnabled | Project % | Tunnel % | Sponsor % | Levels 1-5 %
        ...
        globals[5] = commission_per_level[maxDownLimit+1];
        globals[6] = commission_per_level[0];
        projectPercentage -= globals[6];
        for (uint8 i = 7; i < maxDownLimit + 7; i++) {
            globals[i] = commission_per_level[i-6];
