...
contract Block is ERC721, Ownable{
    ...
    function tokenidlist(address _useraddress)public view returns(uint256[] memory,ParselData[] memory){
        require(_useraddress != address(0x0),"not same list");
        uint256 _v = balanceOf(_useraddress);
        uint256[] memory idlist = new uint256[](balanceOf(_useraddress));
        ParselData[] memory data = new ParselData[](balanceOf(_useraddress));

        for(uint256 i=1;i<=tokenId.current();i++){
            if(ownerOf(i) == _useraddress){
                idlist[_v-1] = i;
                data[_v-1] = parseldata[i];
                _v -=1;
            }
        }
