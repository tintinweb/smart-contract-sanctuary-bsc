...
/**
 * @title StakingPoolForLP
 * @notice It is a staking pool for Pancakeswap V2 LP tokens (stake Pancakeswap V2 LP tokens -> get MST).
 */
contract StakingPoolForLP is Ownable, Pausable, ReentrancyGuard {
    ...
    // MST token (token distributed)
    IERC20 public immutable mstToken;

    // The staked token (i.e., Pancakeswap V2 MST/USDT LP token)
    ...
    // Tokens distributed per block (in mstToken)
     ...
     * @param _mstToken reward token address
     * @param _rewardPerBlock reward per block (in MST)
     ...
     * @param amount amount to withdraw (in mstToken)
