...
contract AtlasAsiaNFT is ERC721, Pausable, Ownable, ERC721Enumerable, ERC721Burnable, ReentrancyGuard{
    ...
    event PackageMinted(uint256 token_id, uint256 referral_token_id, uint256 package_number);
    ...
    constructor() ERC721("Atlas Asia NFT", "ATLASASIA") {
        operationsAddresses[msg.sender] = true;
        usdc = ERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
        treasury = 0x58d78AD2bedD423066e4b0F24a48b9950D8b1ff8;
        signer = 0xd22F849db05c8c60a284502D7415d5014C8BD1c2;
    }
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://api.atlas-asia.network/nft/";
    }
    ...
    function getAssetsOfAddress(address player)
        public
        view
    ...
    function mintPackage(address to, uint256 upline, uint256 package) external onlyOperators {
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(to, tokenId);
        uplines[tokenId] = upline;
        packages[tokenId] = package;
        emit PackageMinted(tokenId, upline, package);
    }

    function purchasePackage(uint256 upline, uint256 package, uint256 price, bytes memory _sig) external nonReentrant {
        bytes32 message = prefixed(
            keccak256(abi.encodePacked(msg.sender, upline, package, price))
        );
        
        require(recoverSigner(message, _sig) == signer, "Invalid signer");

        usdc.transferFrom(msg.sender,address(this),price);
        usdc.transfer(treasury, price);
