 ...
 //COLLECT TAX IN TOKENS WHEN SWAP LIQUIFY SET TO FALSE THIS IS FOR SLOW PERIODS AND COMMUNITY BURNS
  ...
  function restoreAllFee() private {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        uint256 burnAmt = (_burnFee).div(100);
        uint256 fundingAmt =(_fundingFee).div(100);
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient,(burnAmt).sub(fundingAmt));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient,(burnAmt).sub(fundingAmt));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (burnAmt).sub(fundingAmt));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient,(burnAmt).sub(fundingAmt));
        } else {
            _transferStandard(sender, recipient,(burnAmt).sub(fundingAmt));
        }
