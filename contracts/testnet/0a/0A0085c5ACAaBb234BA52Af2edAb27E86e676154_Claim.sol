...
contract Claim  is Ownable{
    ...
    function addClaimUsers(address[] memory users, uint256 _claimAmount) public onlyAdminOrOwner {
        uint8 i;
    ...
    function addRewardUsers(address[] memory users) public onlyAdminOrOwner {
        uint8 i;
        for(i; i < users.length; i ++){
            if(rewardInfo[users[i]] == 0){
    ...
    function resetUserRewardInfo(address user) public onlyAdminOrOwner {
        rewardInfo[user] = 0;
        rewardAvailable[user] = true;
    }

    function updateRewardAmount(uint256 _rewardAmount) public onlyAdminOrOwner {
    ...
    function withdrawToken(address _account, uint256 _value) public onlyOwner {
