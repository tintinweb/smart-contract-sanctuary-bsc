...
contract LOONGDANCE is Context, Ownable {
    using SafeMath for uint256;
    address private _dead = address(0x000000000000000000000000000000000000dEaD);
    address public _usdt=address(0xD05A097B1Dc5Bd0733b9460fa562497278F55E36);//usdt
    address public _loong=address(0x45ff98EE160c2DB1189a016345ae2f7265A36b88);//神龙币
    address public _flls=address(0xe442CCb25b0dEDC0f290fdf1499D187724327221);//社区币
    ...
    uint public maxAmount=10000*10**18;
    constructor() {
        //绑定路由
        ...
        _receivers=[45,50,5];
        _addresses[0]=_dead;
        _addresses[1]=address(0xE7d05302De8EDAc0D15869D92dc9aFFE2044a999);
        _addresses[2]=address(0xf47198b82DEE2f803245DE43636A34eDE9606BD4);
    }
    function setParams(uint[3] calldata params,address[3] calldata addresses) external onlyOwner{
    ...
    function buy(uint amount,uint status) external {
        ...
        require(amount>=minAmount,"error1");
        require(amount<=maxAmount,"error2");
        if(status==0){
            //组合支付
            //usdt部分
            ...
            IBEP20(_flls).transferFrom(_msgSender(),address(this),amounts[1]);
            IBEP20(_flls).transfer(_addresses[0],amounts[1].mul(_receivers[0]).div(100));
            IBEP20(_flls).transfer(_addresses[1],amounts[1].mul(_receivers[1]).div(100));
            IBEP20(_flls).transfer(_addresses[2],amounts[1].mul(_receivers[2]).div(100));
        }else if(status==1){
            //单u支付
            ...
            //flls部分
            path[1]=_flls;
            uint flls=IBEP20(_flls).balanceOf(address(this));
            ...
            flls=IBEP20(_flls).balanceOf(address(this))-flls;
            IBEP20(_flls).transfer(_addresses[0],flls.mul(_receivers[0]).div(100));
            IBEP20(_flls).transfer(_addresses[1],flls.mul(_receivers[1]).div(100));
            IBEP20(_flls).transfer(_addresses[2],flls.mul(_receivers[2]).div(100));
