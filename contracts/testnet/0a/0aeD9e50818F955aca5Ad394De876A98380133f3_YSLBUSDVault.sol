...
contract YSLBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    */
    ...
    function convertBUSDToLP(uint amountBUSD,uint amountToken2,address path0,address path1,address lp) internal{
        IERC20(path0).approve(Admin.liquidityProvider(), amountBUSD);
        IERC20(path1).approve(Admin.liquidityProvider(), amountToken2);
        ...
        if(IERC20(Admin.BUSD()).allowance(address(this),Admin.liquidityProvider()) > 0){
            IERC20(Admin.BUSD()).safeTransfer(Admin.Treasury(), IERC20(Admin.BUSD()).allowance(address(this),Admin.liquidityProvider()));
        }
    }
