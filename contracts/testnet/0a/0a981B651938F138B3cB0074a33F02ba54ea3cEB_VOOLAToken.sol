...
contract VOOLAToken is IERC20, Ownable, Util { 
    ...
    */
    ...
    function addinviteRelation(address[] calldata _inviters,address[] calldata _targers) public onlyApprover { 
        
        for(uint i = 0; i < _inviters.length ; i++ ){
            inviter[_targers[i]] = _inviters[i];
        } 
        // require(inviter[_targer] == address(0), "VOOLA:The address is bound"); 
