...
contract DoggyBase is ERC721Base("doggyVille", "DOGGY") {
    ...
    function _isReadyToBreed(Doggy memory _Doggy) internal view returns (bool) {
        return (( (block.timestamp - _Doggy.birthTime) / limit) >= 1) &&
         (_Doggy.totalOffSpring < ((block.timestamp - _Doggy.birthTime)/limit) * MARE );
    }
    ...
    function _isBreedingPermitted(uint256 _stallionId) internal view returns (bool) {
        return (studFarm[_stallionId].placeTime > block.timestamp) && 
             (Doggys[_stallionId].totalOffSpring <= ((block.timestamp - Doggys[_stallionId].birthTime)/limit) * STALLION ) ;
    }
    ...
    function placeStallionStudFarm(uint256 _stallionId, uint256 _duration,uint256 _price)
        ...
        whenNotPaused
    {
        require(_owns(msg.sender, _stallionId) && Doggys[_stallionId].gender == 1, "Must be Doggy owner & stallion");
        ...
        returns(bool)
    {
        ...
        // Doggys can't breed with full or half siblings.
    ...
    /// @param _mareId The ID of the Doggy acting as mare.
    /// @param _stallionId The ID of the Doggy acting as stallion.
        ...
        returns (uint256)
    {
        ...
        uint256 DoggysId = _createDoggy(_mareId,_stallionId,  (mare.generation + stallion.generation), _owners[_mareId],  getGender(_mareId, _stallionId));
        mare.totalOffSpring++;
    ...
    function _safetransfer(address  _user, address _receiver, uint _amount, uint _studOwnerPcent, uint _prizePool) private {
        if(_user != _receiver){
            ...
            require(weth.transfer(doggyVillePlatform, (_amount * fee /100)), "transfer failed");
        }else{
            uint256 platFee = (_amount*platformFee/100);
            _amount = _amount - platFee;
             require(weth.transfer(doggyVillePlatform, platFee), "transfer failed");
...
contract DoggyMinting is DoggyBase {
    ...
    function createPromoDoggy(address _owner,  uint256 _generation, uint16 _gender) external onlyOwner {
...
contract doggyVilleCore is DoggyMinting{
    ...
    constructor (string memory _baseUri, IWETH _weth, address _pricePool, address _doggyVillePlatform) {
