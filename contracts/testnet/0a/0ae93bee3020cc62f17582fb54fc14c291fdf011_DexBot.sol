...
contract DexBot is Initializable {
    ...
    address public  test7;
    address public  test8;
    ...
    function stake(uint256 amount, uint256 _pid) public {
        ...
        test1 = userstake.amount;
        test2 = userstake.lastStaked;
    ...
    function getrewardAmount(uint8 _pid) public view returns (uint256) {
        ...
        uint256 perMonthsec = 86400 * pool.duration;
        if(!pool.isMine) {
            uint256 Maxdiffrence =  userstake.lastStaked + (pool.duration * withdrawMinLimitStaking);
            if(Maxdiffrence <= block.timestamp) {
                multiplier = getMultiplier(
                    userstake.lastWithdrawRewards,
                    block.timestamp
