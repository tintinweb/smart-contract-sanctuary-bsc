...
contract Marketplace is Ownable, AccessControl {
    ...
    //[0x2170Ed0880ac9A755fd29B2688956BD959F933F8, 0xCC42724C6683B7E57334c4E856f4c9965ED682bD, 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d, 0x1AF3F329e8BE154074D8769D1FFa4eE058B1DBc3]
    ...
    function isAcceptedToken(address token) external view returns (bool) {
        ...
        // }
        // return false;
        return true; //Accepts any token on test network 
    }

    /// @notice Adds a new accepted token
    function addAcceptedToken(address newToken) external onlyOwner {
        _acceptedTokens.push(newToken);
    }

    /// @notice Removes an new accepted token
    function removeAcceptedToken(address tokenToRemove) external onlyOwner {
        uint256 length = _acceptedTokens.length;
        for(uint256 i = 0; i < length; i++) {
            if (tokenToRemove == _acceptedTokens[i]) {
                if(1 < _acceptedTokens.length && i < length-1) {
                    _acceptedTokens[i] = _acceptedTokens[length-1];
                }

                delete _acceptedTokens[length-1];
                _acceptedTokens.pop();
    ...
    function addWhitelisted(address user) external onlyRole(ADMIN_ROLE) {
