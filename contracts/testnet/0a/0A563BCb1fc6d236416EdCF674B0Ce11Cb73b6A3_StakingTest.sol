...
contract StakingTest is Ownable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        start = _start;
        end = _start + _duration * 1 minutes;
    }
    ...
    function unstake() external nonReentrant {
        require(block.timestamp <= end + 30 minutes, "Stake is lost");
    ...
    function claimRestTyz() external onlyOwner nonReentrant {
        require(
            block.timestamp > end + 30 minutes ||
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        if (
            block.timestamp >= pool.lastUpdateDayTimestamp + 1 minutes &&
    ...
    function updatePool() internal {
        ...
        uint256 reward = multiplier * rewPerDay;
        pool.lastUpdateDayTimestamp += 1 minutes * multiplier;
        if (totalStaked == 0) {
            if (pool.lastUpdateDayTimestamp < end) {
                rewPerDay +=
                    reward /
                    ((end - pool.lastUpdateDayTimestamp) / 1 minutes);
            }
            return;
        }
    ...
    function getMultiplier(uint256 from, uint256 to)
        ...
        returns (uint256)
    {
        return to <= end ? (to - from) / 1 minutes : (end - from) / 1 minutes;
    }
}
