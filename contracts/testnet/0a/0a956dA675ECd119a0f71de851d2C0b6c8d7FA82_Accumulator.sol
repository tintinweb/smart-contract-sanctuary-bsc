...
contract Accumulator is Initializable, OwnableUpgradeable, IERC20MetadataUpgradeable {
    ...
    modifier validAddr(address account) {
        require(account != address(0x0), "Zero address");
        _;
    }
    ...
    function _rebase() public {
        ...
        for(uint256 i = 0; i < pTimes; i ++) {
            _totalSupply += _totalSupply * rebaseStageRate[spId];
        }

        for(uint256 i = 0; i < cTimes; i ++) {
            _totalSupply += _totalSupply * rebaseStageRate[scId];
        }
