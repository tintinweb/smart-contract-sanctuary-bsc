    ...
    ERC721Burnable
{
    ...
    // isZone = zone + code [1,2,3,4,5] + index
    mapping(uint256 => bool) public isZone; // true for open
    uint256[5] public maxLandByCode = [52, 52, 52, 52, 32]; // maximum index for code
    ...
    function safeMint(
        ...
        //check setting
        require(isZone[zone], "Zone not open!!");
        require(code < maxLandByCode.length, "Code is wrong!!");
        require(
            index > 0 && index <= maxLandByCode[code - 1],
            "index is wrong!!"
        );
        ...
        _setTokenURI(
            tokenId,
            string(abi.encodePacked(baseURI, Strings.toString(_landId % 1000)))
        );
        ...
        emit eMintedLand(
            ...
            landId,
            tokenId,
            string(Strings.toString(_landId % 1000))
        );
