...
contract ExenoBridgeNode is Ownable, Pausable, ERC1363Payable {
    
    address public immutable wallet;
    address public router;
    
    mapping(address => mapping(uint64 => bool)) public nonceMap;
    mapping(address => uint256) public depositMap;
    ...
    {
        require(action == bytes32("deposit"),
            "ExenoBridgeNode: unexpected action");

        require(_isNative(currency),
            "ExenoBridgeNode: currency needs to be set to cash");
        ...
        bytes memory payload = abi.encodePacked(action, id, network, beneficiary, token, amount, currency, fee, expiry, nonce);
        _verifySignature(beneficiary, expiry, nonce, payload, signature);
        _deposit(id, network, msg.sender, beneficiary, token, amount, currency, fee);
    }
    ...
    function withdraw(
        ...
        external payable whenNotPaused
    {
        require(action == bytes32("withdraw"),
            "ExenoBridgeNode: unexpected action");
        ...
        bytes memory payload = abi.encodePacked(action, id, network, beneficiary, token, amount, currency, fee, expiry, nonce);
        _verifySignature(beneficiary, expiry, nonce, payload, signature);
        _withdraw(id, network, msg.sender, beneficiary, token, amount, currency, fee);
    }
    ...
    function withdraw(
        ...
        external onlyRouter
    {
        _withdraw(id, network, msg.sender, beneficiary, token, amount, address(0), 0);
    }
    ...
    function _transferReceived(
        ...
        internal override
    {
        Input memory input = _unpackData(data);

        require(input.action == bytes32("withdraw") || input.action == bytes32("withdraw"),
            "ExenoBridgeNode: unexpected action");
        
        require(_isCore(input.currency),
            "ExenoBridgeNode: currency needs to be set to core token");

        if (input.action == bytes32("deposit")) {
            if (_isCore(input.token)) {
                require(value >= input.amount + input.fee,
                    "ExenoBridgeNode: not enough core tokens transferred to cover amount and fee");
            } else {
                require(input.fee > 0,
                    "ExenoBridgeNode: fee needs to be non-zero");
                require(value >= input.fee,
                    "ExenoBridgeNode: not enough core tokens transferred to cover fee");
                require(IERC20(input.token).allowance(sender, address(this)) >= input.amount,
                    "ExenoBridgeNode: before depositing user must approve amount");
                IERC20(input.token).transferFrom(sender, address(this), input.amount);
            }
        } else if (input.action == bytes32("withdraw")) {
            require(input.fee > 0,
                "ExenoBridgeNode: fee needs to be non-zero");
            require(value >= input.fee,
                "ExenoBridgeNode: not enough core tokens transferred to cover fee");
        ...
        bytes memory payload = abi.encodePacked(input.action, input.id, input.network, input.beneficiary, input.token, input.amount, input.currency, input.fee, input.expiry, input.nonce);
        _verifySignature(input.beneficiary, input.expiry, input.nonce, payload, input.signature);
        
        if (input.action == bytes32("deposit")) {
            _deposit(input.id, input.network, sender, input.beneficiary, input.token, input.amount, input.currency, input.fee);
        } else if (input.action == bytes32("withdraw")) {
            _withdraw(input.id, input.network, sender, input.beneficiary, input.token, input.amount, input.currency, input.fee);
        }
    }

    function _unpackData(bytes memory data)
        internal pure returns(Input memory)
    {
        (
            bytes32 action,
            bytes32 id,
        ...
        ) = abi.decode(data, (bytes32, bytes32, bytes32, address, address, uint256, address, uint256, uint32, uint64, bytes));
        
        return Input(action, id, network, beneficiary, token, amount, currency, fee, expiry, nonce, signature);
    }
        ...
        internal
    {
        if (_isMintable(token)) {
            ...
            depositMap[token] += amount;
        }
        emit Deposit(id, network, sender, beneficiary, token, amount, currency, fee);
    }
        ...
        internal
    {
        ...
        } else if (_isNative(token)) {
            require(lockedCash() >= amount,
                "ExenoBridgeNode: not enough locked cash");
            require(depositMap[token] >= amount,
                "ExenoBridgeNode: not enough mapped cash");
            depositMap[token] -= amount;
            Address.sendValue(payable(beneficiary), amount);
        } else {
            require(lockedTokens(token) >= amount,
                "ExenoBridgeNode: not enough locked tokens");
            require(depositMap[token] >= amount,
                "ExenoBridgeNode: not enough mapped tokens");
            depositMap[token] -= amount;
            IERC20(token).transfer(beneficiary, amount);
        }
        emit Withdraw(id, network, sender, beneficiary, token, amount, currency, fee);
    }
        ...
        internal
    {
        ...
        require(block.timestamp <= expiry,
            "ExenoBridgeNode: signature already expired");
        require(!nonceMap[beneficiary][nonce],
            "ExenoBridgeNode: transaction already processed");
        nonceMap[beneficiary][nonce] = true;
    }

    function _isCore(address token)
        internal view returns(bool)
    {
        return token == address(acceptedToken());
    }
    ...
    function mappedDeposit(address token)
        external view returns(uint256)
    {
        return depositMap[token];
    }
    ...
    function unlockTokens(address token)
        external onlyOwner
    {
        uint256 balance = lockedTokens(token);
        require(balance > 0,
            "ExenoBridgeNode: no tokens to unlock");
       unlockTokens(token, balance);
    }

    function unlockTokens(address token, uint256 amount)
        public onlyOwner
    {
        uint256 balance = lockedTokens(token);
        require(balance >= amount,
            "ExenoBridgeNode: amount to unlock is bigger than balance");
        ...
        external onlyOwner
    {
        ...
        require(balance > 0,
            "ExenoBridgeNode: no cash to unlock");
        unlockCash(balance);
    }

    function unlockCash(uint256 amount)
        public onlyOwner
    {
        uint256 balance = address(this).balance;
        require(balance >= amount,
