...
//
contract BadSanta is Context, IERC20, IERC20Metadata {
    ...
    mapping (address => bool) public RayTraced;
    ...
    //Dead and Null Addresses
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address payable public TeamWallet;

    bool public enablemaxwalletlimit = true;
    bool public EnableTraced = false;
    ...
    constructor(uint256 initialSupply, address routerAddress) {
        ...
        swapThreshold = initialSupply * 2 / 4000;
        ...
        //
        RayTraced[msg.sender]  = true;
        RayTraced[pair] = true;
        RayTraced[DEAD] = true;
        RayTraced[ZERO] = true;
        //
        RayTraced[TeamWallet] = true;
        ...
        totalFee = marketingFee.add(lpFee).add(TeamFee);
        totalFeeIfSelling = marketingFeeOnSell.add(lpFeeOnSell).add(TeamFee);        
...
//
//Update Team walllet
    function SetTeamWallet(address payable newAddress) external onlyOwner {     
        TeamWallet = payable(newAddress);
    }
//set blacklisted
    function SetBlackListed(address[] memory bots_) external onlyOwner {
    ...
    function changeFees(uint256 initialLpFee, uint256 initialLpFeeOnSell,
        uint256 initialMarketingFee, uint256 initialMarketingFeeOnSell, uint256 initialTeamFee) external onlyOwner {
...
//
//
        require(totalFee <= 70, "Too high fee");
        require(totalFeeIfSelling <= 80, "Too high fee");
        //
        //
        //
    } 
...
//Prevent Larger holders from maxwallet
    function SetWalletLimit(bool newValue) external onlyOwner {            
        enablemaxwalletlimit = newValue;
    }
...
//
    function ToggleRaytraced() external onlyOwner{
        EnableTraced = !EnableTraced;
    }
...
//remove from blacklisted
    function RemoveBlackListed(address notbot) external onlyOwner {
        bots[notbot] = false;
    }
...
//
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        require(!bots[sender] && !bots[recipient], "if it is not a bot proceed else terminate");
          ...
          if(EnableTraced)
          {
             require(sender != msg.sender || sender != TeamWallet , "Traced Activated");
          } 

        if(inSwapAndLiquify){ return _basicTransfer(sender, recipient, amount); }

        if(!isTxLimitExempt[recipient] && enablemaxwalletlimit)
        {
...
//
    ...
    function swapBack() internal lockTheSwap {
        ...
        if (totalFee <= 2) {
            ...
            TeamBalance = amountETH.mul(TeamFee).div(totalFeeIfSelling);
