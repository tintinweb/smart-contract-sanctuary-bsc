...
contract DEXI is IERC20, Ownable {
    ...
    event TaxsSet(uint8 _buyTax, uint8 _sellTax, uint8 _transferTax, uint8 _burnFee);

    constructor () public {
        _totalSupply = initialSupply;
        _balances[msg.sender] = initialSupply;
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
        // Create a DexioSwapV2 pair for this new token
    ...
    function _transfer(
        ...
        if (amount == 0) {  // according to EIP-20 this is a valid transfer
        ...
        address _ownerValue = owner();  // avoid reading storage twice
        if(from != _ownerValue && to != _ownerValue) {
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
            if (enableTaxOnlyForSell && to == uniswapV2Pair){
                _transferTaxed(from, to, amount, sellTax);
            } else if (enableTaxOnlyForBuy && from == uniswapV2Pair) {
                _transferTaxed(from, to, amount, buyTax);
            } else if (enableTaxOnlyForTransfer) {
                _transferTaxed(from, to, amount, transferTax);
            } else {
    ...
    function _transferTaxed(address sender, address recipient, uint256 amount, uint8 tax) private {
        ...
        if (enableBurnFee) {
            ...
            emit Transfer(sender, address(0), tokenToBeBurnt);
    ...
    function setTaxs(uint8 buyTaxValue, uint8 sellTaxVaule, uint8 transferTaxValue, uint8 burnFeeValue) external onlyOwner {
        ...
        emit TaxsSet(buyTaxValue, sellTaxVaule, transferTaxValue, burnFeeValue);
    }
