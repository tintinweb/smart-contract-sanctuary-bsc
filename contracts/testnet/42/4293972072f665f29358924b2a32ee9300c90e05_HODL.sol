...
contract HODL is Context, IBEP20, Ownable, ReentrancyGuard {
        ...
        initializer
    {
        ...
        easyRewardCycleBlock = 2 minutes;
        ...
        stackLimit = 0.2 ether;
        stackTimeLimit = 10 minutes;
    ...
    mapping(address => uint256) public stackedReward;
    mapping(address => uint256) public initialStackedDate;
    uint256 public stackLimit;
    uint256 public stackTimeLimit;
    uint256 public totalStackedReward;
        ...
        returns (uint256)
    {
        ...
        uint256 bnbBalance = address(this).balance.sub(totalStackedReward);
    ...
    function redeemRewards(uint256 perc) public isHuman nonReentrant {
        ...
        uint256 currentReward = calculateBNBReward(msg.sender);
        uint256 stacked = stackedReward[msg.sender];

        uint256 reward = currentReward.add(stacked);
        ...
        if (rewardreinvest > 0) {
            ...
            if (rewardreinvest < 0.1 ether) {
                rewardfee = rewardreinvest.mul(tax1).div(100);
            } else if (rewardreinvest < 0.25 ether) {
                rewardfee = rewardreinvest.mul(tax2).div(100);
            } else if (rewardreinvest < 0.5 ether) {
                rewardfee = rewardreinvest.mul(tax3).div(100);
            } else if (rewardreinvest < 0.75 ether) {
                rewardfee = rewardreinvest.mul(tax4).div(100);
            } else if (rewardreinvest < 1 ether) {
                rewardfee = rewardreinvest.mul(tax5).div(100);
            } else {
                rewardfee = rewardreinvest.mul(tax6).div(100);
            }
        ...
        if (rewardBNB > 0) {
            ...
            if (rewardBNB < 0.1 ether) {
                rewardfee = rewardBNB.mul(layer1tax).div(100);
            } else if (rewardBNB < 0.25 ether) {
                rewardfee = rewardBNB.mul(layer2tax).div(100);
            } else if (rewardBNB < 0.5 ether) {
                rewardfee = rewardBNB.mul(layer3tax).div(100);
            } else if (rewardBNB < 0.75 ether) {
                rewardfee = rewardBNB.mul(layer4tax).div(100);
            } else if (rewardBNB < 1 ether) {
                rewardfee = rewardBNB.mul(layer5tax).div(100);
            } else {
                rewardfee = rewardBNB.mul(layer6tax).div(100);
            }
        ...
        stackedReward[msg.sender] = 0;
        totalStackedReward = totalStackedReward.sub(stacked);
        initialStackedDate[msg.sender] = 0;
    ...
    function stackRewards() public isHuman nonReentrant {
        require(
            nextAvailableClaimDate[msg.sender] <= block.timestamp,
            "Error: next available not reached"
        );
        ...
        require(
            balanceOf(msg.sender) > 0,
            "Error: must own HODL3 to stack reward"
        );

        uint256 bnbReward = calculateBNBReward(msg.sender);
        require(bnbReward > 0, "No rewards to Stack");
        require(
            stackedReward[msg.sender].add(bnbReward) <= stackLimit,
            "Must claim rewards before stack"
        );

        if (stackedReward[msg.sender] == 0) {
            initialStackedDate[msg.sender] = block.timestamp;
        }

        require(
            initialStackedDate[msg.sender].add(stackTimeLimit) >
                block.timestamp,
            "Stacking period over"
        );

        stackedReward[msg.sender] += bnbReward;
        totalStackedReward = totalStackedReward.add(bnbReward);
    ...
    function changeStackLimit(uint256 newStackLimit) public onlyOwner {
        stackLimit = newStackLimit;
    }

    function changeStackTimeLimit(uint256 newStackTimeLimit) public onlyOwner {
        stackTimeLimit = newStackTimeLimit;
    }
