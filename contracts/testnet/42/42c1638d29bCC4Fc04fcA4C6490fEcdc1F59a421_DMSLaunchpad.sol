...
contract DMSLaunchpad is ReentrancyGuard, Pausable, AccessControl {
    ...
    constructor(address _dms, address _locker) {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);

        DMSToken = IERC20(_dms);
    ...
    function setPackage(
        ...
        require(
            packages[_packageIndex].isExist == false,
            "Token: already exist"
        );
    ...
    function updatePackage(uint256 _packageIndex, uint256 _totalSupply)
        public
        whenNotPaused
    {
        packages[_packageIndex].totalSupply = _totalSupply;
    }

    function existPackage(uint256 _packageIndex, bool _exist)
        public
        whenNotPaused
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        packages[_packageIndex].isExist = _exist;
    }

    function activePackage(uint256 _packageIndex, bool _active)
        public
        whenNotPaused
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        packages[_packageIndex].isActive = _active;
    }
    ...
    function buy(
        ...
        require(
            getPackage(_packageIndex).isExist == true,
            "Package: does't exist"
        );
        require(
            getPackage(_packageIndex).isActive == true,
        ...
        updatePackage(_packageIndex, packages[_packageIndex].totalSupply - 1);
