...
contract Staking {
    ...
    modifier timePeriodIsSet() {
        require(timePeriodSet == true, "Please set the burn rate first, then try again.");
        _;
    }
    ...
    function setTimePeriod(uint256 _timePeriodInSeconds) public onlyOwner  {
    ...
    function stakeTokens(IERC20 token, uint256 amount) public timePeriodIsSet {
        ...
        require(amount <= token.balanceOf(msg.sender), "Not enough OSW tokens in your wallet, please try lesser amount");
        
        token.safeTransferFrom(msg.sender, address(this), amount);
        uint256 initialTimestamp = block.timestamp;
        balances[msg.sender] = balances[msg.sender].add(amount);
        timeLocks[msg.sender] = initialTimestamp.add(timePeriod);
    ...
    function unstakeTokens(IERC20 token, uint256 amount) public timePeriodIsSet {
        ...
        if (block.timestamp >= timeLocks[msg.sender]) {
