...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    function _transfer(
        ...
        if(_isExcluded[sender] || inSwapAndLiquify){
            removeAllFee();
        }else{
        ...
        /* Swap liquidity */
        if(balanceOf(address(this)) < tokenAmount)tokenAmount=balanceOf(address(this));
        ...
        if(!inTransfer){
            ...
            if (!_isExcluded[sender]) {
                _transferToIncludedToFee(sender, recipient, amount);
            } else {
    ...
    function _transferToIncludedToFee(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount) = _getValues(tAmount);
        (uint256 tTransferAmount,,,) = _getTValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
     ...
     */
	function updateFeeOrig(uint256 _txFee,uint256 _liquidityFee,uint256 _companyFee) public onlyOwner() {
		ORIG_TAX_FEE = _txFee;
