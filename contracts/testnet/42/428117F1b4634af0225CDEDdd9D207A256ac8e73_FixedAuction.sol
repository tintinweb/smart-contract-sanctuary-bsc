...
contract FixedAuction is Configurable, IERC721Receiver {
    ...
    function createErc721(
        ...
        uint amountTotal0 = 1;
        _create(name, token0, token1, tokenId, amountTotal0, amountTotal1, openAt, onlyBot, TypeErc721);
    }
    ...
    function createErc1155(
        ...
        _create(name, token0, token1, tokenId, amountTotal0, amountTotal1, openAt, onlyBot, TypeErc1155);
    }

    function _create(string memory name, address token0, address token1, uint tokenId, uint amountTotal0, uint amountTotal1, uint openAt, bool onlyBot, uint nftType) private {
    ...
    function swap(uint index, uint amount0) external payable isPoolExist(index) checkBotHolder(index) isPoolNotSwap(index) {
        ...
        (uint platformFee, uint royaltyFee, uint _actualAmount1) = IRoyaltyConfig(getFeeConfigContract()).getFeeAndRemaining(pools[index].token0, amount1);
        ...
        if (pool.token1 == address(0)) {
            ...
            IRoyaltyConfig(getFeeConfigContract()).chargeFeeETH{value: totalFee}(pools[index].token0, platformFee, royaltyFee);
        } else {
            ...
            IRoyaltyConfig(getFeeConfigContract()).chargeFeeToken(pools[index].token0, pools[index].token1, address(this), platformFee, royaltyFee);
        }
    ...
    modifier checkBotHolder(uint index) {
        if (onlyBotHolderP[index]) {
            require(getMinValueOfBotHolder() > 0 && IERC20(getBotToken()).balanceOf(msg.sender) >= getMinValueOfBotHolder(), 'BOT is not enough');
        }
        _;
    }
