...
interface ComptrollerLensInterface {
    ...
    function getAssetsIn(address) external view returns (EToken[] memory);
...
contract EvryLens {
    ...
    event ComptrollerChanged(address newComptroller);

    modifier onlyAdmin() {
        require(msg.sender == admin, "EVRYLENS: Unauthorized");
        _;
    }
    ...
    function changeComptroller(address newComptroller) public onlyAdmin {
        comptroller = newComptroller;
        emit ComptrollerChanged(newComptroller);
    }

    struct ETokenMetadata {
        address eToken;
    ...
    function eTokenMetadata(EToken eToken) public view returns (ETokenMetadata memory) {
        uint exchangeRateCurrent = eToken.exchangeRateStored();
        (bool isListed, uint collateralFactorMantissa) = ComptrollerLensInterface(comptroller).markets(address(eToken));
        address underlyingAssetAddress;
        uint underlyingDecimals;
        ...
        if (compareStrings(eToken.symbol(), "eBNB")) {
            ...
            EBep20 eBep20 = EBep20(address(eToken));
            underlyingAssetAddress = eBep20.underlying();
            underlyingDecimals = EIP20Interface(eBep20.underlying()).decimals();
        }

        return ETokenMetadata({
            eToken: address(eToken),
            exchangeRateCurrent: exchangeRateCurrent,
            supplyRatePerBlock: eToken.supplyRatePerBlock(),
            borrowRatePerBlock: eToken.borrowRatePerBlock(),
            reserveFactorMantissa: eToken.reserveFactorMantissa(),
            totalBorrows: eToken.totalBorrows(),
            totalReserves: eToken.totalReserves(),
            totalSupply: eToken.totalSupply(),
            totalCash: eToken.getCash(),
            isListed: isListed,
            collateralFactorMantissa: collateralFactorMantissa,
            underlyingAssetAddress: underlyingAssetAddress,
            eTokenDecimals: eToken.decimals(),
            underlyingDecimals: underlyingDecimals
        });
    }

    function eTokenMetadataAll(EToken[] calldata eTokens) external view returns (ETokenMetadata[] memory) {
        uint eTokenCount = eTokens.length;
        ETokenMetadata[] memory res = new ETokenMetadata[](eTokenCount);
        for (uint i = 0; i < eTokenCount; i++) {
            res[i] = eTokenMetadata(eTokens[i]);
        }
        return res;
    }

    struct ETokenBalances {
        address eToken;
        uint balanceOf;
    ...
    function eTokenBalances(EToken eToken, address payable account) public view returns (ETokenBalances memory) {
        uint balanceOf = eToken.balanceOf(account);
        uint borrowBalanceCurrent = eToken.borrowBalanceStored(account);
        uint balanceOfUnderlying;
        uint tokenBalance;
        uint tokenAllowance;

        {
            ...
            (err, eTokenBalance, borrowBalance, exchangeRateMantissa) = eToken.getAccountSnapshot(account);
            require(err == 0, "getAccountSnapshot Error");
            balanceOfUnderlying = (eTokenBalance * exchangeRateMantissa) / expScale;
        }
        ...
        if (compareStrings(eToken.symbol(), "eBNB")) {
            ...
            EBep20 eBep20 = EBep20(address(eToken));
            EIP20Interface underlying = EIP20Interface(eBep20.underlying());
            tokenBalance = underlying.balanceOf(account);
            tokenAllowance = underlying.allowance(account, address(eToken));
        }

        return ETokenBalances({
            eToken: address(eToken),
            balanceOf: balanceOf,
    ...
    function eTokenBalancesAll(EToken[] calldata eTokens, address payable account) external view returns (ETokenBalances[] memory) {
        uint eTokenCount = eTokens.length;
        ETokenBalances[] memory res = new ETokenBalances[](eTokenCount);
        for (uint i = 0; i < eTokenCount; i++) {
            res[i] = eTokenBalances(eTokens[i], account);
        }
        return res;
    }

    struct ETokenUnderlyingPrice {
        address eToken;
        uint underlyingPrice;
    }

    function eTokenUnderlyingPrice(EToken eToken) public view returns (ETokenUnderlyingPrice memory) {
        PriceOracle priceOracle = ComptrollerLensInterface(comptroller).oracle();

        return ETokenUnderlyingPrice({
            eToken: address(eToken),
            underlyingPrice: priceOracle.getUnderlyingPrice(eToken)
        });
    }

    function eTokenUnderlyingPriceAll(EToken[] calldata eTokens) external view returns (ETokenUnderlyingPrice[] memory) {
        uint eTokenCount = eTokens.length;
        ETokenUnderlyingPrice[] memory res = new ETokenUnderlyingPrice[](eTokenCount);
        for (uint i = 0; i < eTokenCount; i++) {
            res[i] = eTokenUnderlyingPrice(eTokens[i]);
        }
        return res;
    }
    ...
    function getAccountLimits(address account) public view returns (AccountLimits memory) {
        (uint errorCode, uint liquidity, uint shortfall) = ComptrollerLensInterface(comptroller).getAccountLiquidity(account);
        require(errorCode == 0, "account liquidity error");

        return AccountLimits({
            markets: ComptrollerLensInterface(comptroller).getAssetsIn(account),
            liquidity: liquidity,
            shortfall: shortfall
        });
    }

    function getAllMarketsEntered(address account) public view returns (EToken[] memory) {
        return ComptrollerLensInterface(comptroller).getAssetsIn(account);
    }

    function checkCollateralActive(address account, EToken eToken) external view returns (bool) {
        return ComptrollerLensInterface(comptroller).checkMembership(account, eToken);
    }
