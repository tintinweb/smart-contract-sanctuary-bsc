...
contract MetaPoint0517T is Ownable {
    ...
    uint8[] private allot = [30, 25, 5, 35, 5];

    uint32 public accountCnt;
    uint32 public pointCnt;
    ...
    constructor() {
        creator = msg.sender;
        first = 0xA83395CDd7125e25474845Dcbe7a526BCf0c0E17;
    ...
    function register(address head) public {
        ...
        accounts[msg.sender] = Account({
            ...
            wda: 0,
            grd: 0,
            rootLevel: accounts[head].level,
            rootSn: accounts[head].sn,
            level: accounts[head].level,
            sn: accounts[head].sn
        });
        accountCnt++;
    ...
    function up() public {
        ...
        uint256 gr = (expend * allot[1]) / 100;

        uint200 nextId = concat(msg.sender, accounts[msg.sender].point);
        uint32 level;
        uint32 sn;
        if (accounts[msg.sender].point == 0) {
            (level, sn) = take(head, nextId);
        ...
        creatorBalance += srd;
        donateBalance += (expend * allot[2]) / 100;
        creatorBalance += (expend * allot[3]) / 100;

        IERC20(lp).transfer(lpds, (expend * allot[4]) / 100);
    ...
    function withdraw(uint256 amount) public {
        uint256 globalReward = grping(msg.sender);
        uint256 srd = accounts[msg.sender].srd;
        accounts[msg.sender].wda += (globalReward + srd);
        ...
        IERC20(lp).transfer(wdr, withdrawFee);
    ...
    function take(address head, uint200 id) private returns (uint32 level, uint32 i) {
        level = accounts[head].level + 1;
        uint32 sn = accounts[head].sn;

        for (uint32 t = 1; true; t++) {
            for (i = sn * uint32(2**t); i < (sn + 1) * uint32(2**t); i++) {
                if (tree[level][i] == 0) {
                    tree[level][i] = id;
                    console.log("this time point up(%s,%s)=>%s", level, i, id);
                    emit Log(level, i, id);
                    return (level, i);
                }
            }
    ...
    mapping(address => mapping(uint32 => uint8)) public userDailyCnts;
    ...
    function verifysrd(address addr, uint32 level) public view returns (bool result) {
        uint32 vip = accounts[addr].vip;
        if (vip == 3) {
            result = (level < 30);
        } else if (vip == 2) {
            result = (level < 20);
    ...
    function concat(address addr, uint32 sn) private pure returns (uint200) {
