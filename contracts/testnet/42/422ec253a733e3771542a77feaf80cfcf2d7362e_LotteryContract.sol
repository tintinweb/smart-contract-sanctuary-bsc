...
contract LotteryContract is VRFConsumerBase, Ownable {
    ...
    mapping(uint256 => mapping(address => uint256)) public participatedUsers;
    ...
    function declareWinner(uint256 _lotteryId) public onlyOwner {
        Lottery storage lottery = lotteries[_lotteryId];
        // require(block.timestamp > lottery.endDate, "Lottery ended");
        ...
        override
    {
        ...
        for (uint256 k = 0; k < lottery.noOfWinner; k++) {
            uint256 winnerIndex = uint256(
                keccak256(abi.encode(randomness, k))
