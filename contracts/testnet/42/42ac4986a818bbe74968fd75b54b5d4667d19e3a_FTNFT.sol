...
import "./module/NFTExchange.sol";

contract FTNFT is ERC721URIStorage, ERC721Enumerable, ERC721Burnable, IFINFT, NFTExchange {
    ...
    function mint(address to, uint256 templateId) public override returns (uint256) {
        ...
        require(_templateIndex.contains(templateId), "FTNFT: Template id is not exist");
...
abstract contract NFTExchange is IFINFT {

    address public tokenAddress;

    function setTokenAddress(address token) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "FTNFT: Must have admin role to set token address");

        tokenAddress = token;
    }

    function _getToken() internal view returns (IERC20) {
        require(tokenAddress != address(0), "FTNFT: Token address not set");
        
        return IERC20(tokenAddress);
    }
    
    function exchangeByToken(uint256 templateId) public {
