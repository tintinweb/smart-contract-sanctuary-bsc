...
contract LuckyGame is Ownable, Pausable, ReentrancyGuard {
    ...
    error IncorrectAmount(uint256 input, uint256 expected);
        ...
        notContract
    {
        ...
        if (rawAmount != payAmount) {
            revert IncorrectAmount(payAmount, payAmount);
        }
    ...
    function claimERC20(
        ...
        bytes32 digest = keccak256(
            abi.encode(ClaimType.ERC20, _msgSender(), collectId, amount)
        );
        if (!SignatureChecker.isValidSignatureNow(adminSigner, digest, sig)) {
            revert InvalidSignature();
        }

        claimed[collectId] = true;
        currency.safeTransferFrom(address(this), _msgSender(), amount);
