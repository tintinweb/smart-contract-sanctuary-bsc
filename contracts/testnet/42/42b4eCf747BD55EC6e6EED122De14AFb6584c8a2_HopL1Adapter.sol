...
/**
 * @title The Hop L1 Adapter Contract
 * @author Plug Exchange
 * @notice Implemented hop bridge contract
 * @dev Plug users can deposit the bridge token into the hop exchange directly
 */
contract HopL1Adapter is IBridgeAdapter, AdapterBase {
  /**
   * @notice Initialization of hop adapter contract
   * @param _plugRouter The plug router contract address
   */
  constructor(address _plugRouter) AdapterBase(_plugRouter) {}
  ...
  function deposit(
    ...
    (
      ...
      address relayer,
      uint256 relayerFee
    ) = abi.decode(data, (address, uint256, uint256, uint256, address, uint256));
    require(_hopBridge != address(0), 'INVALID_HOP_BRIDGE');

    if (token != NATIVE_TOKEN_ADDRESS) {
      _approve(_hopBridge, token);
    }
    toChainId = chainId;

    _hopCall(_hopBridge, chainId, recipient, amount, amountOutMin, deadline, relayer, relayerFee);
  }

  /**
   * @notice Hop bridge deposit from L1 to L2 chain
   * @param _hopBridge The Hop bridge contract address
   * @param chainId The designation chain Id
   * @param recipient The account of recipient
   * @param amount The token amount to bridge
   * @param amountOutMin The minimum amount out
   * @param deadline The deadline to execute the transaction
   * @param relayer The relayer address
   * @param relayerFee The fee of relayer
   */
  function _hopCall(
    address _hopBridge,
    uint256 chainId,
    ...
    (bool success, bytes memory result) = _hopBridge.call{value: msg.value}(
