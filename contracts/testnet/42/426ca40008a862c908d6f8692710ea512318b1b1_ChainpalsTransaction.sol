...
contract ChainpalsTransaction is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        validateNonZeroAddress(_ChainpalsToken);
        validateNonZeroAddress(_coOwnerAddress);
        validateNonZeroAddress(_paidStakingAddress);
        validateNonZeroAddress(_escrowManagerAddress);
        validateNonZeroAddress(_escrowBonusAddress);
        validateNonZeroAddress(_adminAddress);
        validateNonZeroAddress(_referralBonusAddress);
        validateNonZeroAddress(_chainpalsPlatformAddress);
        validateNonZeroAddress(_feesHoldingWalletAddress);
     ...
     * - `_uid` unique transaction ID.
     */
    function resolveDispute(string memory _uid)
        external
        nonReentrant
        returns (bool)
    {
    ...
    function depositCHPTokens(uint256 _amount)
        ...
        returns (bool)
    {
        require(_amount != 0, "Wrong deposit amount");
        transferFromFunds(
            _amount,
            ChainpalsToken,
            address(msg.sender),
