...
contract Staking {
    ...
    function stake(
        ...
        emit Staked(newStakeId, msg.sender, _stakingType, _amount , _duration, _apr, _nftId);
    }
    ...
    function claim(uint256 _stakingId) external {
        ...
        if (staked.proposalPercentage != 0) {
            uint256 proposalToWDA = finalApr * staked.proposalPercentage / 100;
    ...
    function _deleteStakingPackage(
        ...
        emit Unstaked(stakingId, account, vault[account][stakingId].stakingType, amount);
