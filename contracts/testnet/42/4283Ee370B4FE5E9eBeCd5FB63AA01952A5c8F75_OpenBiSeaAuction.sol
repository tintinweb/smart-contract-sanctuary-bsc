...
contract OpenBiSeaAuction is Ownable,IERC721Receiver,IERC1155Receiver {
    ...
    function whitelistContractCreator(address contractNFT, uint256 fee) public {
        require(msg.sender == openBiSeaMainContract, "OpenBiSeaAuction: only main contract can send it");
        totalIncomeNFT = totalIncomeNFT.add(fee);
        contractsWhitelisted[contractsWhitelistedCount] = contractNFT;
        contractsWhitelistedCount++;
        emit ContractNFTWhitelisted(contractNFT);
    }

    function whitelistContractCreatorTokens(address contractNFT, uint256 fee) public {
        require(msg.sender == openBiSeaMainContract, "OpenBiSeaAuction: only main contract can send it");
        totalIncomeNFT = totalIncomeNFT.add(fee);
        contractsWhitelisted[contractsWhitelistedCount] = contractNFT;
        contractsWhitelistedCount++;
    ...
    function updateFirstDateAndValue(address buyer, address seller, uint256 value, address token) private {
        if (token == address (0x0) || IOpenBiSea(openBiSeaMainContract).getUsdContract() == token) { // only usd or main coin can add income value
            ...
            totalIncomeNFT = totalIncomeNFT.add(valueFinal);
            consumersRevenueAmount[buyer] = consumersRevenueAmount[buyer].add(value.div(2));
            consumersRevenueAmount[seller] = consumersRevenueAmount[seller].add(value.div(2));
            if (consumersDealFirstDate[buyer] == 0) {
                consumersDealFirstDate[buyer] = block.timestamp;
            }
            if (consumersDealFirstDate[seller] == 0) {
                consumersDealFirstDate[seller] = block.timestamp;
            }
        }
    }
    ...
    function bid(
        ...
        address sender,
        address token
    ) public returns (bool, uint256, address, address) {
        require(msg.sender == openBiSeaMainContract, "OpenBiSeaAuction: only main contract can send it");
        require(isContractNFTWhitelisted(contractNFT), "OpenBiSeaAuction: contract must be whitelisted");
        ...
        if (block.timestamp > auction.deadline) {
            emit AuctionNFTBid(contractNFT, tokenId, price, auction.deadline, isERC1155, sender, auction.seller, true, token);
            _bidWin(
                auction,
                isERC1155,
                contractNFT,
            ...
            return (true,0,auction.seller,auction.token);
        } else {
            auction.price = price;
            auction.latestBidder = sender;
            auction.latestBidTime = block.timestamp;
            emit AuctionNFTBid(contractNFT, tokenId, price,auction.deadline, isERC1155, sender,auction.seller, false, auction.token);
            if (auction.latestBidder != address(0)) {
                return (false,auction.price,auction.latestBidder,auction.token);
            }
        }
        return (false,0, address(0),auction.token);
    }
    ...
    function _cancelAuction(
        ...
        emit AuctionNFTCanceled(contractNFT, tokenId,auction.price,auction.deadline, isERC1155,auction.seller);
    ...
    function cancelAuction(
        ...
        require(isContractNFTWhitelisted(contractNFT), "OpenBiSeaAuction: contract must be whitelisted");
        require(indexOfNFTsAuctionList(contractNFT, tokenId) != type(uint256).max, "OpenBiSeaAuction: auction is not created");
        _cancelAuction( contractNFT, tokenId, sender, isERC1155, false );
    }
