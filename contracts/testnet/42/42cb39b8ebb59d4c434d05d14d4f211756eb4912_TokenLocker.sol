...
contract TokenLocker is Context {
    ...
    LockStruct[] public locker;

    /**
     * @dev Get Lock Info by ID.
     */
    function getLockInfo(uint256 lockID) public view returns (IERC20 token, address beneficiary, uint256 amount, uint256 releaseTimestamp) { 
        return (locker[lockID].token, locker[lockID].beneficiary, locker[lockID].amount, locker[lockID].releaseTimestamp);
    }
