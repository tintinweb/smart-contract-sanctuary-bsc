...
contract AutomationOrderIncrease is AutomationCompatible, Ownable {
    //---------- Libraries ----------//
    ...
    //---------- Variables ----------//
    address payable public keeperBalancer;

    //---------- Constructor ----------//
    constructor(address _automationRegistry, address _orderBook) public {
    ...
    {
        uint256 _book = totalOrders().div(100);
        uint256 remainder = _book.mul(100);
        _book = remainder < totalOrders() ? _book.add(1) : _book;
    ...
    function totalOrders() public view returns (uint256) {
        return orderBook.totalIncreaseOrders();
    }

    function orderAt(uint256 _index) public view returns (bytes32) {
        return orderBook.increaseOrderAt(_index);
    }
    ...
    function getBatch() public view returns (uint256 start, uint256 end) {
        if (totalOrders() != 0) {
            ...
            uint256 end_ = index.mul(100) > totalOrders()
                ? totalOrders()
                : index.mul(100);
            return (start_, end_);
        }
        ...
        returns (bool, bytes memory)
    {
        ...
        for (uint256 i = start; i < end; i++) {
            ...
            ) = orderBook.getIncreaseOrder(orderId);
            (, bool isPriceValid) = orderBook.validatePositionOrderPrice(
        ...
        onlyKeeper
    {
        ...
        require(indexToken != address(0), "Invalid order");
        require(isPriceValid, "Not executable");
        orderBook.executeIncreaseOrder(orderId, keeperBalancer);
    }
