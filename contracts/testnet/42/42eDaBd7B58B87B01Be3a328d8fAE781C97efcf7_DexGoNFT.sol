...
contract DexGoNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    function purchase(uint8 typeNft, address referrer) payable public {
        ...
        IHandshakeLevels(handshakeLevels).setHandshake(msg.sender, referrer);

        address[] memory friends;
        uint friendsCount;
        uint valueForOwner = msg.value;
        (friends, friendsCount) = IHandshakeLevels(handshakeLevels).getAllReferralsLine(msg.sender);
        for(uint8 x=0;x<friendsCount;x++) {
            if (friends[x] != address (0)) {
                ...
                valueForOwner.sub(valueForFriend);
            }
        }
    ...
    function rent(uint256 _tokenId) public payable {
        ...
        payable(ownerOf(_tokenId)).transfer(fixedAmountProject);
...
interface IHandshakeLevels {
    function getAllReferralsLine(address wallet) external view returns (address[] memory, uint);
    function getPercentPerLevelWei(uint8 position) external view returns (uint);
    function setHandshake(address wallet, address referrer) external view returns (uint,uint, bool, uint);
}
