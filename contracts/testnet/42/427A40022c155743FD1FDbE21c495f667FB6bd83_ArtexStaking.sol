...
contract ArtexStaking is Ownable, ReentrancyGuard {
    ...
    function getAllUserInfo(address _user) public view returns(uint256[] memory) {
        ...
        for(uint256 i = 0; i < stake.length; ++i){
            if(stake[i].amount>0){
                information[i*5+0]=stake[i].amount;
                information[i*5+1]=stake[i].depositTime;
                information[i*5+2]=pool[stake[i].poolId].interest;
                information[i*5+3]=pool[stake[i].poolId].lockPeriod;
                information[i*5+4]=i;
            }
        }
        return information;
