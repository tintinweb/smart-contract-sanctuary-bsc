...
contract HoneybeePledge is Initializable, OwnableUpgradeable, DivestorUpgradeable {
    ...
    function _processOut(uint stakeburn_, uint commBurn_, uint readReward_, uint reward_, address account_) public returns (bool) {
        if (readReward_ > reward_) {
            addrInfo.HBEE.safeTransfer(0x000000000000000000000000000000000000dEaD, readReward_ - reward_);
        }
    ...
    function _updateCommPool(address account_, uint poolId_, bool inPool_) private {
        ...
        if (commPool[poolId_].totalValue == 1) {
    ...
    function viewCommSlotReward(address account_, uint poolId_) public view returns (uint) {
        if (!userComm[account_][poolId_].exist) {
            return 0;
        }
    ...
    function _outStake(address account_, uint burnAmount_) private {
        if (burnAmount_ > 0) {
