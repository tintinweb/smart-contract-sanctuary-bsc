...
contract XoomMeters {
    ...
    address[] public mappedAddressesForAllowance; // To help contract see mapped allowed/denied in list (PUBLIC iterates over getMappedAddressesForAllowanceSize)

    // Feature => Function ID => Points
    mapping(uint => mapping(uint => uint256)) private tablePoints;

    // Mapped Features
    mapping(uint => uint) private featureIsMapped;
    uint[] private mappedFeatures;

    // Mapped Functions
    mapping(uint => uint) private functionIsMapped;
    uint[] private mappedFunctions;
    ...
    address[] public mappedTokensForRequirementBalance; // To help contract see mapped tokens (PUBLIC iterates over getMappedTokensForRequirementBalanceSize)
    ...
    {
        require(allowedList[msg.sender] == 1, 'NAL'); //Not allowed
        _;
    }
    ...
    {
        // Define feature as mapped
        if(featureIsMapped[feature] == 0)
        {
            featureIsMapped[feature] = 1;
            mappedFeatures.push(feature);
        }
        
        if(featureIsMapped[featureOfMeters] == 0)
        {
            featureIsMapped[featureOfMeters] = 1;
            mappedFeatures.push(featureOfMeters);
        }

        // Define function as mapped
        if(functionIsMapped[functionId] == 0)
        {
            functionIsMapped[functionId] = 1;
            mappedFunctions.push(functionId);
        }
        
        if(functionIsMapped[functionIdOfMeters] == 0)
        {
            functionIsMapped[functionIdOfMeters] = 1;
            mappedFunctions.push(functionIdOfMeters);
        }
    ...
    function getMappedTokensForRequirementBalanceSize() external view returns (uint)
    {
        return mappedTokensForRequirementBalance.length;
    }
    ...
    function validateRequirement(uint feature, uint functionId, address player) external view returns (uint)
    {
        ...
        for(uint ixFeatureOfMeters = 0; ixFeatureOfMeters < mappedFeatures.length; ixFeatureOfMeters++)
        {
            uint featureOfMeters = mappedFeatures[ixFeatureOfMeters];

            for(uint ixFunctionOfMeters = 0; ixFunctionOfMeters < mappedFunctions.length; ixFunctionOfMeters++)
            {
                uint functionOfMeters = mappedFunctions[ixFunctionOfMeters];
