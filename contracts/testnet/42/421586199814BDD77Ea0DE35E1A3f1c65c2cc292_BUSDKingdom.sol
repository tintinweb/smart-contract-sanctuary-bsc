...
contract BUSDKingdom is Ownable {
    ...
    uint256 public COMPOUND_STEP = 10 * 60; // 1 days;
	...
	uint256 public WITHDRAW_COOLDOWN = 70 * 60; //7 days;
    ...
    function ReinvestRewards() external {
        ...
        (uint256 userBalance, uint256 userRewards) = getUserRewardsState(msg.sender);
        // console.log("userBalance: ", userBalance);
        // console.log("userRewards: ", userRewards);

        user.userDeposit = userBalance + userRewards;
    ...
    function getUserRewardsState(address _account) public view returns (uint256 userBalance, uint256 userRewards) {
        User storage user = users[_account];

        uint256 period = min(WITHDRAW_COOLDOWN - 1, block.timestamp - user.lastHatch);
        ...
        if (times == 0) {
            userBalance = user.userDeposit;
            userRewards = 0;
        } else {
            userBalance = user.userDeposit.mul((1000 + rewardsRate) ** (times)).div(PERCENTS_DIVIDER ** (times));
            // userRewards = user.userDeposit.mul((1000 + rewardsRate) ** (times)).div(PERCENTS_DIVIDER ** (times)) - userBalance;
        }

        userRewards = userBalance.mul(rest).mul(rewardsRate).div(PERCENTS_DIVIDER).div(COMPOUND_STEP);
        // userRewards = userRewards + rest;
        
        // console.log("space -> userBalance: ", userBalance);
    ...
    function getUserInfo(address _adr) public view returns(uint256 _initialDeposit, uint256 _userDeposit, uint256 _lastHatch, 
    ...
    function getMyLands() public view returns(uint256){
        (uint256 userBalance, ) = getUserRewardsState(msg.sender);

        return userBalance.mul(landRate);
    }
    ...
    function LandsGift(address _account, uint256 _landsAmount) external {
        require(msg.sender == owner(), "Admin use only");
        users[_account].userDeposit = users[_account].userDeposit + _landsAmount.mul(1e18).div(landRate);
    }
