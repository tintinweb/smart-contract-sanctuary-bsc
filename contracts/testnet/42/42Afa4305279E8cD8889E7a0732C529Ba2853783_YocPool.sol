...
contract YocPool is Ownable, Pausable {
    ...
    function depositOperation(
        ...
        if (_amount > 0) {
            ...
            if (!isYocToken) {
                uint256 depositFeeAmount = (currentAmount * depositFee) / PERCENT_PRECISION;
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (block.timestamp < user.lastDepositedTime + withdrawFeePeriod && isYocToken) {
            uint256 feeRate = withdrawFee;
            uint256 currentWithdrawFee = (currentAmount * feeRate) / PERCENT_PRECISION;
    ...
    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingYOCRewards()) * (1e18)) / totalShares);
    }
