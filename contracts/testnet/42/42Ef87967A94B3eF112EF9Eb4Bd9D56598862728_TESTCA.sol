 ...
 *                
 *                  
 *      Fees on Buy: 6%   Sell:12% 
 *                          
 *      https://t.me/*
 *      
 *      Reward token: * (BEP-20 pegged)
 *      Reward address: *
 *      Token distribution:
 *       100% to LP, no dev wallets
*/         
...
contract TESTCA is ERC20, Ownable {
    ...
    constructor() ERC20("TESTCONTRACT", "TESTCA") {
    ...
    function setWhitelistStatus(bool newValue) external onlyOwner {
        require(whitelistStatus != newValue, "Value already set to that option");
    ...
    function setWhitelisted(address account, bool newValue) external onlyOwner {
        require(newValue != isWhitelisted[account], "Value already set to that option");
    ...
    function massSetWhitelisted(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isWhitelisted[accounts[i]], "Value already set to that option");
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _rewardsFee, uint256 _liquidityFee) external onlyOwner {
        ...
        require(totalSellFees <= 12, "Must keep fees at 12% or less");
    }
    ...
    function _transfer(
        ...
        if(!tradingActive){
            ...
            require(!whitelistStatus || (isWhitelisted[from] && isWhitelisted[to]), "Not Whitelisted");
        }
...
contract DividendTracker is DividendPayingToken {
    ...
    constructor() {
    	claimWait = 1200;
        minimumTokenBalanceForDividends = 100000000 * (10**18); //must hold 100M (0,1%) tokens
    }
