...
contract KeterNetwork is IBEP20, ReentrancyGuard, Context, Ownable {
	using Address for address;
    ...
    mapping(address => uint256) public _inv;

    mapping(address => uint256) public _sum;
    

    string private constant _name         = "Keter Network";
    string private constant _symbol       = "KTR";
	uint8 private constant _decimals      = 9;
	uint256 private constant _totalSupply = 150 * 10**6 * 10**_decimals;
	...
	function buyTokens(address _refer) external payable returns (bool success) {
		...
		if (_msgSender() != _refer && balanceOf(_refer) != 0 && _refer != address(0)) {
            ...
            _points[_msgSender()] += 3;
            _sum[_msgSender()] += msg.value;
    ...
    function claim() public {
        require(_points[msg.sender] >= 1, "Insufficient points");
        uint256 mult = _points[msg.sender] *20_000000000;
		_transfer(address(this), msg.sender, mult);
...
contract KeterAINFT is ERC721Enumerable, ERC721Holder, ReentrancyGuard, Ownable {
  using Strings for uint256;
  using SafeMath for uint256;
  using Address for address;
  ...
  bool public pausedAirdrop = false;
  uint8 private constant _decimals  = 9;
  ...
  KeterNetwork public myCounter;

  mapping(address => bool) public whitelisted;
  mapping(address => bool) public claimed;

 
  constructor(string memory _name, string memory _symbol, string memory _initBaseURI, string memory _initNotRevealedUri, address counterAddress) ERC721(_name, _symbol) {
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);
    
    myCounter = KeterNetwork(payable(address(counterAddress)));
 ...
 // public
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    require(!paused, "Pre-sale paused");
    require(_mintAmount > 0, "Min mint is 1");
    require(_mintAmount <= maxMintAmount, "Max mint is 100");
...
function airdrop() public {
    require(!pausedAirdrop, "Pre-sale paused");
    uint256 supply = totalSupply();
  
    uint256 first = myCounter._sum(msg.sender);
    uint256 sec = myCounter._inv(msg.sender);
  
   // require(sec >= 15 || sec >= 200000000000000000, "Not elegible to receive airdrop");

    
    uint256 base = 15;
	uint256 rest = sec / base;

	uint256 basetik = 200000000000000000;
	uint256 resttik = first / basetik;
		
	uint256 finalx = rest + resttik - _claimedCount[msg.sender];

    require(supply + finalx <= maxSupply, "Airdrop finished");
    require(finalx >= 1, "Not elegible to receive airdrop");
    for (uint256 i = 1; i <= finalx; i++) {
      _safeMint(msg.sender, supply + i);
      _claimedCount[msg.sender] += 1;
    }
  ...
  function setCostt(uint256 _newCostt) public onlyOwner {
    cost = _newCostt;
  }
...
function pauseairdrop(bool _state) public onlyOwner {
    pausedAirdrop = _state;
  }
 ...
 function whitelistUser(address _user) public onlyOwner {
  ...
  function clearBNB(uint256 bnbAmount) external onlyOwner {
