...
contract GEMUNILeasing is PermissionGroupUpgradeable, IGEMUNILeasing {
    ...
    function putOnLeasing(LeasingCreate[] memory inputs) public override {
        for (uint i = 0; i < inputs.length; i++) {
            ...
            } else if (nftToken == geniPass || gemuniItems[nftToken]){
                require(mortgageAmount == 0, "Lending: invalid mortgage amount");
            }
            address lender;
            if (!gameGuild[msg.sender]) {
                if (nftToken != geniPass && !gemuniItems[nftToken]) {
                    ...
                    require(lenderAddress == address(0) && guildAddress == address(0),"Leasing: invalid token");
                }
                ...
                if (leasing.lender == treasury) {
                    ...
                    require(block.timestamp <= leasingGuild.startLending + leasingGuild.duration * weekPerSecond, "Lending: over time");
                } else if (leasing.lender != address(0) && leasing.lender != treasury) {
                    revert("Leasing: NFT is leased by user");
                } else {
    ...
    function register(address nftToken, uint tokenId, uint duration, uint minDuration, uint maxDuration, uint pricePerWeek) internal returns (uint rentalAmount) {
        ...
        leasingGuild.minDuration = minDuration;
        leasingGuild.maxDuration = maxDuration;
        emit RegisterLeasing(nftToken, tokenId, duration, minDuration, maxDuration, pricePerWeek, geniRental);
    }
    ...
    function finishRenting(address nftToken, uint tokenId) external override {
        ...
        if (msg.sender == guild.gameGuild) {
            ...
            guildReturnToAdmin(nftToken, tokenId, guild.duration, leasing. mortgageAmount);
        //renter return pass to guild + admin/user
            ...
            //renter return pass to leaser(admin/user) 
                ...
                renterReturnToLeaser(nftToken, tokenId, leasing.mortgageAmount, leasing.lender, msg.sender);
            }
    ...
    function guildReturnToAdmin(address nftToken, uint tokenId, uint duration, uint mortgageAmount) internal {
        if (duration > 0) {
            if (nftToken != geniPass || !gemuniItems[nftToken]) {
    ...
    function renterReturnToGuild(address nftToken, uint tokenId, LeasingInfo storage leasing, LeasingGuildInfo storage guild ) internal {
        ...
        //this pass put on lending by guild
            ...
            renterReturnToLeaser(nftToken, tokenId, guild.mortgageAmount, guild.gameGuild, msg.sender);
        }
    }

    function renterReturnToLeaser(address nftToken, uint tokenId, uint mortgageAmount, address lender, address renter) internal {
        if (nftToken == geniPass) {
            IGENIPass(geniPass).unLockPass(tokenId);
            IGENIPass(geniPass).setActive(tokenId, false);
    ...
    function adminRetrieveNFT(address nftToken,uint tokenId, RenterInfo storage renter, LeasingInfo storage leasing, LeasingGuildInfo storage guild) internal {
        //admin retrieve pass from game guild (register/ rent+put)
    ...
    function userRetrieveNFT(address nftToken,uint tokenId, RenterInfo storage renter, LeasingInfo storage leasing) internal {
        //user retrieve pass from gameguild
        //user 
    }
