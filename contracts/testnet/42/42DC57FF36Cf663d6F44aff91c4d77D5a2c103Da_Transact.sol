...
contract Transact is ReentrancyGuard {
  ...
  // maps a user's address to his token balances
  ...
  function setConOwner(address newConOwner) internal {
    require(msg.sender == conOwner, 'This function is not public');

    conOwner = payable(newConOwner);
  }
  ...
  function depositTokens(uint256 amount, string memory symbol) payable external nonReentrant {
    // require(whitelistedTokens[symbol] != dead, 'we only accept busd');

    accountBalances[msg.sender][symbol] += amount;
    ERC20(whitelistedTokens[symbol]).transferFrom(msg.sender, address(this), amount);
  }

  function withdrawTokens(uint256 amount, string memory symbol) payable external nonReentrant {
  ...
  function withdrawToOwner(uint256 amount, string memory symbol) external payable nonReentrant {
    require(msg.sender == conOwner, 'This function is not public');
