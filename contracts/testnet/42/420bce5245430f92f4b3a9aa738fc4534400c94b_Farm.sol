...
// File: contracts/farmFarm.sol

pragma solidity 0.7.1;


// farmFarm is the master of BIG. He can make BIG and he is a fair cat.
//
// Note that it's ownable and the owner wields tremendous power. The owner will set
// the governance contract and will burn its keys once BIG is sufficiently
// distributed.
//
...
contract Farm is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BIGs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accBIGPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accBIGPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. BIGs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that BIGs distribution occurs.
        uint256 accBIGPerShare; // Accumulated BIGs per share, times 1e12. See below.
    }
    ...
    // Block number when bonus BIG period ends.
    uint256 public bonusEndBlock;
    // BIG tokens created per block.
    uint256 public BIGPerBlock;
    // Bonus muliplier for early BIG makers.
    ...
    // The block number when BIG mining starts.
    uint256 public startBlock;
    // The block number when BIG mining ends.
    ...
    constructor(
        farm _BIG,
        address _devaddr,
        uint256 _BIGPerBlock, // 100000000000000000000
    ...
    modifier onlyOwnerOrGovernance() {
        require(owner() == _msgSender() || governance == _msgSender(), "Caller is  the owner, neither governance");
        _;
    }
    ...
    // Update the given pool's BIG allocation point. Can only be called by the owner or governance contract.
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        //BIG minting ocurrs only until endBLock
        if(_to > endBlock){
            _to = endBlock;
        }
    ...
    // View function to see pending BIGs on frontend.
    ...
    function updatePool(uint256 _pid) public {
        ...
        //BIG.mint(devaddr, BIGReward.div(100));
        BIG.mint(address(this), BIGReward);
        pool.accBIGPerShare = pool.accBIGPerShare.add(BIGReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to farmFarm for BIG allocation.
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accBIGPerShare).div(1e12).sub(user.rewardDebt);
            safeBIGTransfer(msg.sender, pending);
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accBIGPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Withdraw LP tokens from farmFarm.
    ...
    function safeBIGTransfer(address _to, uint256 _amount) internal {
    ...
    function devFee() public {
        ...
        BIG.mint(devaddr, BIG.totalSupply().div(200));
