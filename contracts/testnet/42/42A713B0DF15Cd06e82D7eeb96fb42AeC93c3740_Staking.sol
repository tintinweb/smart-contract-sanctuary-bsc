...
contract Staking is Initializable, OwnableUpgradeable {
    ...
    function updateConfiguration(address _huh, address _sHuh, uint256 _minimumLimitToStake, address _exponentation) public onlyOwner {
        huh = ERC20Upgradeable(_huh);
        sHuh = ISToken(_sHuh);
        minimumLimitToStake=_minimumLimitToStake*10**huh.decimals();
    ...
    function stake(uint256 amount) public {
        uint256 originSTokenAmount = sHuh.balanceOf(msg.sender);
        ...
        require(_tBalanceAdded+originSTokenAmount>=minimumLimitToStake, "less than limit");
        sHuh.safeTransfer(msg.sender,_tBalanceAdded);
        totalStaked=totalStaked+_tBalanceAdded;
    ...
    function setStakingRewardPercent(
        ...
        if(stakingRewardBlocksSet.length>1){
    ...
    function distributeStakingReward() external {
        ...
        for (uint256 i = stakingRewardBlocksSet.length; i > 0; --i) {
            if (stakingRewardBlocksSet[i-1] < stakingRewardLastBlock) {
                if (i == stakingRewardBlocksSet.length) {
                    ...
                    blockCount =
                        stakingRewardBlocksSet[i]-stakingRewardLastBlock;
                }
                if (
            ...
            if (i == stakingRewardBlocksSet.length) {
                ...
                blockCount =
                    stakingRewardBlocksSet[i]-stakingRewardBlocksSet[i-1];
            }
        ...
        uint256 rewardAmount = totalStaked * times / 10000000000;
        stakingRewardLastBlock = block.number;
        stakingRewardAmountDistributed = stakingRewardAmountDistributed+rewardAmount;
        bool succeed=sHuh.distributeRewards(rewardAmount);
        if(succeed){
            totalStaked=totalStaked+rewardAmount;           
        }
