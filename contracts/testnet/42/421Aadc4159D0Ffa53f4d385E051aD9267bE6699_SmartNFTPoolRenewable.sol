...
contract SmartNFTPoolRenewable is IERC721Receiver ,Ownable, ReentrancyGuard {
    ...
    function userInfo() external view returns(uint256 amount, uint256 rewardDebt, uint256[] memory tokenIds) {
        amount = usersInfo[msg.sender].amount;
        rewardDebt = usersRewardDebt[msg.sender][currentRound];
        tokenIds = usersInfo[msg.sender].tokenIds;
    }
    ...
    */
    ...
    function startNewPoolOrRound( uint256 _startInDays, uint256 _poolLengthDays ) external onlySub {
        ...
        if(address(rewardToken) != roundInfo[currentRound].rwdToken) {
            currentRound++;
    ...
    function setupTokens(
        ...
        require(bonusEndBlock < block.number, "Pool has not ended");
    ...
    */
    ...
    function emergencyUnstakeALL() public {
        ...
        usersRewardDebt[msg.sender][currentRound] = 0;
    ...
    function prevPendingRewards(address _user) public view returns (address[] memory _rwdToken, uint256[] memory _amount, bool hasRewards) {
        ...
        for (uint i = 1; i > currentRound; i++) {
            ...
            _rwdToken[i-1] = round.rwdToken;
            _amount[i-1] = user.amount.mul(round.accTokenPerShare).div(PRECISION_FACTOR).sub(usersRewardDebt[msg.sender][i]);
            if (_amount[i-1] > 0) hasRewards = true;
        }
    }
    ...
    function claimPrevRewards() public {
        ...
        for (uint i = 1; i<currentRound; i++){
            ...
            if (pending > 0) {
                ...
                rwd.safeTransfer(address(msg.sender), pending);
