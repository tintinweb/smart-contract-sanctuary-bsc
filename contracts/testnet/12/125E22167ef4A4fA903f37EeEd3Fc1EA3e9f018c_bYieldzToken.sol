    ...
    */
    function withdraw(uint256 count) external {
        ...
        if (block.timestamp < data.lockTime) {
            uint256 _preWithdrawPercent = getPreWithdrawPercent(count);
    ...
    function claimable(uint256 _count) external view returns(bool) {
        StakeData storage data = userData[msg.sender][_count];
        bool result = block.timestamp - data.stakeTime > slowPeriod;
        return result;
    }

    function getPreWithdrawPercent(uint256 _count) public view returns(uint256) {
        StakeData storage data = userData[msg.sender][_count];
