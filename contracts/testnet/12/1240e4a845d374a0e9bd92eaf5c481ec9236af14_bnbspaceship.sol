...
contract bnbspaceship is Ownable {
    ...
    uint256 public MIN_INVEST_LIMIT = 1 * 1e15; /* 0.001 BNB  */
    ...
    function BuySeats(address ref) public payable{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function startSpaceship(address addr) public payable{
        if (!contractStarted) {
    		if (msg.sender == owner()) {
    ...
    /* How many Seats per day user will receive based on BNB deposit */
