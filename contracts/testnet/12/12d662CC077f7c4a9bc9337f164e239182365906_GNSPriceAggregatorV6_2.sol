...
contract GNSPriceAggregatorV6_2 is ChainlinkClient {
    ...
    mapping(uint => bool) public pairNoMedian;
    ...
    function removeNode(uint index) external onlyGov{
     ...
     // Manage pairNoMedian
     function setPairNoMedian(uint _pair, bool _state) public onlyGov {
        pairNoMedian[_pair] = _state;
     }
    ...
    function fulfill(
        ...
        // Use anchor
        if ((f.feed1 != 0x0000000000000000000000000000000000000001) && (f.feed1 != 0x0000000000000000000000000000000000000000)) {
        ...
        // Don´t use anchor
            ...
            // If price == feedPrice there is no deviation
            feedPrice = price;
        }
    ...
    function linkFee(uint pairIndex, uint leveragedPosDai) public view returns(uint){
        // Variable price in LINKs, it depends on $LINK price.

        // (, int linkPriceUsd, , , ) = linkPriceFeed.latestRoundData();

        // return pairsStorage.pairOracleFeeP(pairIndex)
        // * 100000000000000000 * 1e8 / uint(linkPriceUsd) / PRECISION / 100;
        
        // Fixed price in LINKs
        return pairsStorage.pairOracleFeeP(pairIndex);
    }
