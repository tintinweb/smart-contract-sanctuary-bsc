...
contract QuarkLock is Ownable {
    ...
    constructor(address owner, uint256 fee, address feeReceiver2) {
        transferOwnership(owner);
        _fee = fee;
        _feeReceiver2 = feeReceiver2;
    }
    ...
    function getFeeReceiver2() external view returns (address) {
        return _feeReceiver2;
    }

    function setFeeReceiver2(address feeReceiver2) external {
        require(msg.sender == _feeReceiver2, "Only fee receiver 2 can change fee receiver 2");
        _feeReceiver2 = feeReceiver2;
    }

    function feeTransfer() internal {
        require(msg.value >= _fee, "Not enough fee");
        if (_fee > 0) {
            uint256 fee2 = _fee / 5;
            payable(_feeReceiver).sendValue(_fee - fee2);
            payable(_feeReceiver2).sendValue(fee2);
