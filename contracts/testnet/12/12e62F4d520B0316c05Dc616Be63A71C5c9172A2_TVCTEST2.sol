...
contract TVCTEST2 is ERC20, Ownable {
     ...
     */
    constructor (address _treasuryAddress) ERC20( "TVCTEST2", "TVCTEST2") {
    ...
    function setMaxTxSize(uint256 _maxTx) external onlyOwner {
        require(_maxTx > 8, "Max transaction limit must be more than 0.008%");
        maxTx = _maxTx;
    }
    ...
    //Exemptions
    function exemptFromFee(address _address, bool _status) external onlyOwner{
        feeExemption[_address] = _status;
    }

    function exemptFromTxSize(address _address, bool _status) external onlyOwner{
        maxTxExemption[_address] = _status;
    }

    function exemptFromCooldown(address _address, bool _status) external onlyOwner{
        cooldownExemption[_address] = _status;
    }

    //Blacklist
    function setBlacklistAddress(address _address, bool _status) external onlyOwner{
    ...
    function _checkTransfer(address _from, address _to, uint256 _amount) internal returns (uint256) { 
        ...
        //Check Fee
        if(feeEnabled){               
            if(feeExemption[_from] || feeExemption[_to]){
                return _amount;  
            }else{
                uint256 feeAmount = _amount * fee / 10000;
                _amount -= feeAmount;
                _sendFeeToTreasury(_from, feeAmount);                
            }
        }    
    ...
    function _sendFeeToTreasury(address _from, uint256 _amount) internal returns (bool) {
        _transfer(_from, treasuryAddress, _amount);
        return true;
    }
