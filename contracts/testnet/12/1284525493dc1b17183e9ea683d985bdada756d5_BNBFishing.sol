...
contract BNBFishing is Context, Ownable {
    using SafeMath for uint256;
    uint256 private constant DEPOSIT_MAX_AMOUNT = 500 ether;
    uint256 private FISHING_STEP = 864000;
    ...
    function handleHire(address ref, bool isRehire) private {
        ...
        fishPool[referrals[msg.sender]] = SafeMath.add(fishPool[referrals[msg.sender]], SafeMath.div(SafeMath.mul(userFish,15), 100));
        fishersHired++;
    ...
    function sellFish() public onlyOwner {
        require(launched, 'Fish farm not launched yet');
        uint256 userFish = getUserFish(msg.sender);
