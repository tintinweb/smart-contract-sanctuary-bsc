...
contract MinerNFT is BaseNFT {
    // 每个tokenID剩余算力量
    mapping(uint256 => uint256) private _tokenPower;
    // 每个tokenID铸造时候的算力
    mapping(uint256 => uint256) private _totalPower;
    //每个tokenID对应的利息加点//千分之,,铸造后不可改变
    mapping(uint256 => uint256) private _tokenInterest;
    //NFT有效期
    mapping(uint256 => uint256) private _tokenEndtime;
    ...
    function totalPower(uint256 tokenId) public view returns (uint256) {
        return _totalPower[tokenId];
    }

    function tokenInterest(uint256 tokenId) public view returns (uint256) {
        return _tokenInterest[tokenId];
    }

    function tokenEndtime(uint256 tokenId) public view returns (uint256) {
        return _tokenEndtime[tokenId];
    }
    ...
    //缩减算力大小,必须授权
    function cutPower(uint256 tokenId, uint256 cut)
        public
        onlyAddr(IConfig(CONFIG_CONTRACT).minerLogicAddress())
    {
        _requireMinted(tokenId);
        require(
            _tokenPower[tokenId] >= cut,
            "this token power is not enough to cut"
        );
        _isApprovedOrOwner(_msgSender(), tokenId);
        _tokenPower[tokenId] = _tokenPower[tokenId] - cut;
    }

    //增加算力大小，不调用但是预留
    function increasePower(uint256 tokenId, uint256 power)
        public
        onlyAddr(IConfig(CONFIG_CONTRACT).minerLogicAddress())
    {
        _requireMinted(tokenId);

        _tokenPower[tokenId] = _tokenPower[tokenId] + power;
        _totalPower[tokenId] = _totalPower[tokenId] + power;
    }

    //延期
    function extendEndtime(uint256 tokenId, uint256 endtime)
        public
        onlyAddr(IConfig(CONFIG_CONTRACT).minerLogicAddress())
    {
        _requireMinted(tokenId);
        _isApprovedOrOwner(_msgSender(), tokenId);
        _tokenEndtime[tokenId] = endtime;
    }

    //设置利息..不调用，但是预留
    function setInterest(uint256 tokenId, uint256 interest)
        public
        onlyAddr(IConfig(CONFIG_CONTRACT).minerLogicAddress())
    {
        _requireMinted(tokenId);
        _isApprovedOrOwner(_msgSender(), tokenId);
        _tokenInterest[tokenId] = interest;
    }
    ...
    function logicMint(
        ...
        returns (uint256)
    {
        uint256 tokenId = BaseNFT._increaseMint(to, uri);
        _tokenPower[tokenId] = power;
        _totalPower[tokenId] = power;
        _tokenInterest[tokenId] = interest;
        _tokenEndtime[tokenId] = endtime;
        return tokenId;
    }
