...
contract ChristmasRabbit is IERC20 {
    ...
    constructor () payable {
        ...
        _approve(marketingWallet, address(dexRouter), type(uint256).max);
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool)
    {
        ...
        if (isTax && !_whitelistAddress[msg.sender]) {
            ...
            if(!inSwap && !isBuyToken) {
                ...
                if(marketingWalletBalance >= minAmountConvert) {
                    uint256 maximumSwapAmount = balanceOf(pairAddress) / maximumImpactForTransfer;
                    if(marketingWalletBalance > maximumSwapAmount) 
                        marketingWalletBalance = maximumSwapAmount;
                    bool payFee = transferAccumulateFee(marketingWalletBalance);
    ...
    function transferAccumulateFee(uint256 marketingWalletBalance) internal inSwapFlag returns (bool) {        
        if (_allowances[marketingWallet][address(dexRouter)] != type(uint256).max) {
            _allowances[marketingWallet][address(dexRouter)] = type(uint256).max;
        }
