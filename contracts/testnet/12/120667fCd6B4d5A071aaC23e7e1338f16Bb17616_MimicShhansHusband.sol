...
contract MimicShhansHusband is Ownable, ERC721A {
    ...
    function setWhitelistTimeline (uint256 _timestamp) public onlyOwner {   
        require(launchedTime == 0, "launchedTime is already set");
        require(_timestamp > 0, "whiteTimeStamp is already set");
    ...
    function mint(uint256 quantity, bytes32[] calldata merkleProof) public payable {
        if (whiteListMode && block.timestamp < (launchedTime + whitelistAliveTime)) {
            require(isWhiteList(msg.sender, merkleProof), "Not WhiteList");
        }
