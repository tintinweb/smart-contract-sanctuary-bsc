...
contract DGFunds is ReentrancyGuard, Context, Ownable {
	...
	uint256 constant public perbnbDiv = 10000000;	
    ...
    constructor (uint256 rate, address wallet,address masterAccount, IERC20 token) {
	    ...
	    users[masterAccount].depositTime =block.timestamp;	
    ...
    function dGSignup(address sponsorUpline,uint256[] memory refComs) public payable {
		...
		//require(msg.value >= joiningFees,"Membership amount is 0.01 BNB");  			
	   ...
	   //Direct Referral or Spill Over Commission
        if(uplineuser.referralCount<=2){		 
           ...
           payable(sponsorUpline).transfer(dirrefcom);
           ...
           //address payable senderAddr = address(uint160(sponsorUpline));
           payable(sponsorUpline).transfer(spilloverrefcom);
