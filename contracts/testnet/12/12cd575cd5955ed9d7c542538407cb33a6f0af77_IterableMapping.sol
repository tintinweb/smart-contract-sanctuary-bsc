...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "PTR: The router already has that address");
  ...
  constructor(string memory _name, string memory _symbol, uint8 _decimals) ERC20(_name, _symbol, _decimals) {
   // ammIsWhiteListed[address(0x10ED43C718714eb63d5aA57B78B54704E256024E)] = true; // PCS V2 router @note disabled
...
contract PTR is ERC20, Ownable {
    ...
    PTRDividendTracker public dividendTracker;
    ...
    uint256 public maxSellTransactionAmount = 100000 * (10**18); // @note Equals 1e+16 | 100,000,

    uint256 public swapTokensAtAmount = 1000 * (10**18); // @note Equals 1e+14 | 1000
    ...
    uint256 public operationsFee = 3; // This is a subset of the liquidity fee, not in addition to. operations fee + buyback fee cannot be higher than liquidity fee.
    ...
    uint256 public immutable tradingEnabledTimestamp = block.timestamp; // @note tradingIsEnabled
    ...
    constructor() ERC20("PoorToRich", "PTR", 18) {

        dividendTracker = new PTRDividendTracker();
        ...
        address _migrationSwapContract = 0xD152f549545093347A162Dce210e7293f1452150; //for mainnet
      //  address _migrationSwapContract = 0xF8eeB2989106bF1CE8fcb8341FD7aB79ee83A6e2; //for testent
        ...
        */
        _mint(owner(), 10000000 * (10**18)); // @note Equals 1e+18 | Set supply to 1,000,000,000 (without override)
    }
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
        require(isContract(ammAddress), "PTR: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PTR: The dividend tracker already has that address");

        PTRDividendTracker newDividendTracker = PTRDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PTR: The new dividend tracker must be owned by the PTR token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "PTR: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "PTR: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "PTR: The operations wallet is already this address");
    ...
    function updateMigrationSwapContract(address newMigrationSwapContract) external onlyOwner {
        require(newMigrationSwapContract != migrationSwapContract, "PTR: The migration swap contract is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "PTR: The buyback wallet is already this address");
    ...
    function updateFees(uint256 bnbRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "PTR: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PTR: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PTR: Cannot update gasForProcessing to same value");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "PTR: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "PTR: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isBlacklistedToken(rewardTokenAddress), "PTR: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "PTR: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(uniswapV2Router), "PTR: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "PTR: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isBlacklistedToken(rewardTokenAddress), "PTR: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "PTR: setRewardToken:: AMM is not whitelisted!");
    ...
    function buyBackTokensWithNoFees() external payable returns (bool) {
        ...
        require(userRawBNBDividends >= holderBNBUsedForBuyBacks[msg.sender].add(msg.value), "PTR: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PTR: Automated market maker pair is already set to that value");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if (liquidityFee > 0) {
            ...
            if (operationsFee > 0) {
                ...
                require(success, "PTR: SwapAndLiquify:: Unable to send BNB to Operations Wallet");
            }
            ...
            if (buyBackFee > 0) {
                ...
                require(success, "PTR: SwapAndLiquify:: Unable to send BNB to BuyBack Wallet");
            }
...
contract PTRDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PTR_Dividend_Tracker", "PTR_Dividend_Tracker", 18) {
        claimWait = 3600;

    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "PTR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "PTR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PTR contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PTR_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PTR_Dividend_Tracker: Cannot update claimWait to same value");
