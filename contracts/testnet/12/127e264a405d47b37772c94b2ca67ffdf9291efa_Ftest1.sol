  ...
  DividendPayingTokenOptionalInterface
{
  ...
  address public rewardToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; // mainnet BUSD
...
contract Ftest1DividendTracker is DividendPayingToken, Ownable {
  using SafeMath for uint256;
  using SafeMathInt for int256;
  ...
  constructor()
    DividendPayingToken("Ftest1_Dividend", "Ftest1_Dividend")
  {
    claimWait = 3600;
...
contract Ftest1 is ERC20, Ownable {
  using SafeMath for uint256;
  // Events Declarations
  ...
  Ftest1DividendTracker public dividendTracker;

  uint256 public immutable MIN_BUY_TRANSACTION_AMOUNT = 5000000 * (10**18); 
  uint256 public immutable MAX_BUY_TRANSACTION_AMOUNT = 100000000000 * (10**18); // 100% of supply
  uint256 public immutable MIN_SELL_TRANSACTION_AMOUNT = 10000000 * (10**18); // 1% of total supply
  ...
  uint256 public immutable MAX_SWAP_TOKENS_AT_AMOUNT = 5000000000 * (10**18); 

  uint256 public maxBuyTransactionAmount = 1000000000 * (10**18); // 1% of supply
  uint256 public maxSellTransactionAmount = 1000000000 * (10**18); // 1% of supply
  uint256 public swapTokensAtAmount = 20000000 * (10**18); // 0.02 % of supply
  uint256 public maxWallet = 2000000000 * (10**18); // % of supply

  uint256 public immutable MAX_MARKETING_FEE = 2;
  uint256 public immutable MAX_DEV_FEE = 2;
  uint256 public immutable MAX_BUYBACK_FEE = 2;
  uint256 public immutable MAX_REWARDS_FEE = 2;
  uint256 public immutable MAX_LIQUIDITY_FEE = 2;
  uint256 public immutable MAX_TOTAL_FEES = 10;
  uint256 public immutable MAX_SELL_FEE = 10;
  ...
  // Ftest1Guard is active on default
  // bool internal Ftest1GuardOffline = true;

  address payable public marketingWallet =
    payable(0x29d64BEeB2d0e78A4fE80638762F55e55F752D26);
  address payable public devWallet =
    payable(0x29e18e019bee6D2fd3BCebA36377f72932BFeb5b);
  address payable public buyBackWallet =
    payable(0x279417F19F075fF9bBeDa2e6872BB7939CbAE7C6);
  ...
  constructor() ERC20("Ftest1", "Ftest1") {
    dividendTracker = new Ftest1DividendTracker();
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(
      0xD99D1c33F9fC3444f8101754aBC46c52416550D1 // mainnet
    ...
    excludeFromMaxWallet(buyBackWallet, true);
    ...
    */
    _mint(owner(), 10000000000 * (10**18)); // 10,000,000,000
  }
  ...
  // function isFtest1GuardEnabled() public view returns (bool) {
  //   return Ftest1GuardOffline;
  // }
  ...
  // function shutdownFtest1Guard() external onlyOwner {
  //   require(!Ftest1GuardOffline, "Ftest1 guard is already offline");
  //   Ftest1GuardOffline = true;
  // }
  ...
  function updateDividendTracker(address newAddress) public onlyOwner {
    ...
    Ftest1DividendTracker newDividendTracker = Ftest1DividendTracker(
      payable(newAddress)
    );
  ...
  function launch() external onlyOwner {
    marketingFee = 4; // normalize the marketing fee
    maxBuyTransactionAmount = 1000000000 * (10**18); // 1% of supply
    maxSellTransactionAmount = 1000000000 * (10**18); // 1% of supply
    maxWallet = 4000000000 * (10**18); // 4% of supply
  }

  function noLimit() external onlyOwner {
    maxBuyTransactionAmount = 100000000000 * (10**18);
    maxSellTransactionAmount = 100000000000 * (10**18);
    maxWallet = 100000000000 * (10**18);
  }
  ...
  function _transfer(
    ...
    if (
      ...
      require(
        _isExcludedFromMaxWallet[to] || super.balanceOf(to) + amount <= maxWallet,
    ...
    //   require(
    //     Ftest1GuardOffline || isWhitelisted[to],
    //     "trading isnt enabled or account isnt whitelisted"
    //   );
    // } else if (to == uniswapV2Pair) {
    //   require(
    //     Ftest1GuardOffline || isWhitelisted[from],
    //     "trading isnt enabled or account isnt whitelisted"
    //   );
    // }
    bool isTxExempted = (isTxLimitExempt[from] || isTxLimitExempt[to]);
    ...
    if (canSwap && !swapping && automatedMarketMakerPairs[to]) {
      ...
      if(liquidityFee.add(marketingFee).add(devFee).add(buyBackFee) > 0){
  ...
  function DividendRewardAddress() external view returns (address) {
    return rewardToken;
  }

  function DividendDividendTrackerAddress() external view returns (address) {
    return address(dividendTracker);
  }

  function DividendRewardOnPool() external view returns (uint256) {
    return IERC20(rewardToken).balanceOf(address(dividendTracker));
  }

  function DividendTokenFees()
    external
    view
    returns (
      uint256,
  ...
  function DividendRewardDistributed() external view returns (uint256) {
    return dividendTracker.getTotalDividendsDistributed();
  }

  function DividendGetAccountDividendsInfo(address account)
    public
    view
    returns (
      address,
  ...
  function DividendGetAccountDividendsInfoAtIndex(uint256 index)
    public
    view
    returns (
      address,
  ...
  function DividendRewardPaid(address holder) external view returns (uint256) {
    (, , , , uint256 paidAmount, , , , ) = DividendGetAccountDividendsInfo(
      holder
    );
    return paidAmount;
  }

  function DividendRewardUnPaid(address holder)
    external
    view
    returns (uint256)
  {
    (, , , uint256 unpaidAmount, , , , , ) = DividendGetAccountDividendsInfo(
      holder
    );
    return unpaidAmount;
  }

  function DividendRewardClaim() external {
  ...
  function DividendNumberOfDividendTokenHolders()
    external
    view
    returns (uint256)
  {
  ...
  function swapAndSendDividends(uint256 tokens) private {
    ...
    uint256 initialDividends = IERC20(rewardToken).balanceOf(address(dividendTracker));
    swapTokensForRewards(tokens, address(dividendTracker));
    uint256 dividends = IERC20(rewardToken).balanceOf(address(dividendTracker)).sub(initialDividends);
    if (dividends > 0) {
