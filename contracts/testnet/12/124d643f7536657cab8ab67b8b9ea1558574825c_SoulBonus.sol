...
contract SoulBonus is DivestorUpgradeable {
    ...
    uint constant POSITIVE = 1; // 好评
    uint constant NEGATIVE = 2; // 差评

    uint constant SEARCH_BEGIN_DAY = 1651363200 / 1 days; // 北京时间2022-05-01 08:00:00

    // nft soul lp 权重
    uint[3] weightBonus;

    uint sameDayQuotient; // 当前天
    ...
    // 每日 -> 用户 -> 销毁soul、销毁NFT、质押代币
    mapping(uint => mapping(address => DayToken)) public dayToDayToken;
    ...
    function initialize(address usdt_, address soulToken_, address soul1155_) public initializer {
        __Divestor_init();
        addr = AddrCollection({
    ...
    function _getSameDay() private returns (uint) {
        uint tempQuotient = block.timestamp / 1 days;
        if (tempQuotient != sameDayQuotient) { // 不是当天
            sameDayQuotient = tempQuotient;
        }
        return sameDayQuotient;
    }

    // 投入soul
    function _inputSoul(uint soulCount_) private {
        uint sameDay = _getSameDay();
        DayToken storage dayToken = dayToDayToken[sameDay][_msgSender()];
        dayToken.soul += soulCount_;
        totalDayToken[sameDay].soul += soulCount_;
    }

    // 投入NFT(好评/差评)
    function _inputNft(uint8 type_, uint[] memory tokenIds) private {
    ...
    // 质押usdt
    function _inputToken(uint amount_) private {
        uint sameDay = _getSameDay();
        DayToken storage dayToken = dayToDayToken[sameDay][_msgSender()];
        dayToken.lp += amount_;
    ...
    // 用户查询分红
    function _searchUserBonus(uint fromDay_, uint toDay_, bool all) view private returns(uint) {
        uint total;
        for (uint i = fromDay_; i <= toDay_; i++) {
            ...
            DayToken storage dayToken = dayToDayToken[i][_msgSender()];
            if (!all && !dayToken.isGet) {
                continue;
            }
    ...
    // 设置isGet
    function _dealGetBonus(uint fromDay_, uint toDay_) private {
        for (uint i = fromDay_; i <= toDay_; i++) {
            uint bonus = bonusPool[i];
    ...
    // 代币进入记录
    function inputTokenRecord(uint add) external {
    ...
    function inputSoulApi(uint soulCount_) public {
        ...
        _inputSoul(soulCount_);
        addr.soulTokenAddr.burnFrom(_msgSender(), soulCount_ * 1 ether);
    }
    ...
    function inputTokenApi(uint amount_) public {
        ...
        _inputToken(amount_);
        addr.usdt.safeTransferFrom(_msgSender(), address(this), amount_ * 1 ether);
    }

    // 用户查询分红(前一天)
    function searchUserBonusApi() public returns(uint, uint) {
        // 1 领取+未领取 2 领取
        uint sameDay = _getSameDay() - 1;
        return (_searchUserBonus(SEARCH_BEGIN_DAY, sameDay, true), _searchUserBonus(SEARCH_BEGIN_DAY, sameDay, false));
    }

    // 用户领取分红 [fromDay_,toDay_]
    function getUserBonusApi() public {
        uint sameDay = _getSameDay() - 1;
        uint all = _searchUserBonus(SEARCH_BEGIN_DAY, sameDay, true);  // 领取+未领取
        uint get = _searchUserBonus(SEARCH_BEGIN_DAY, sameDay, false); // 领取

        require(all - get > 0, "not coin err");

        _dealGetBonus(0, sameDay);
        addr.usdt.safeTransfer(_msgSender(), all - get);
    }
}
