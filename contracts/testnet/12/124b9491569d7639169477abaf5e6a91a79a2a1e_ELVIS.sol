...
// Root file: contracts/baby/ELVIS.sol

pragma solidity >= 0.8.4;

contract ELVIS is ERC20, Ownable, WhiteList {
    ...
    IERC20 private constant USDT = IERC20(
    ...
    uint256 public _UsdtMaxReawrd = 200;
    ...
    event OwnerClear(uint256 i,address lpUser);
    ...
    ) payable ERC20("FireTestOne", "FireTestOne") {
        _startTime = startTime;
        rewardToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;   //usdt
    ...
    function setUsdtMaxReawrd(uint256 amount) external onlyOwner {
        ...
        onlyOwner
    {
        require(
            ...
            "ELVIS: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "ELVIS: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "ELVIS: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "ELVIS: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function _transfer(
        ...
        if (
            ...
            //50% marketingfee 
            // The returned token does not need to be converted to usdt
            /*   
            ...
            */
            //50% USDT rewardfee
            uint256 swapRewardTokens = contractTokenBalance.mul(rewardFee).div(totalFees);
        ...
        if (takeFee) {
            uint256 marketfees = amount.mul(marketingFee).div(100);  // 5%
            uint256 rewardfees = amount.mul(rewardFee).div(100);    // 5%
            /*****
            ...
            ******/
            ...
            super._transfer(from, address(this), rewardfees);
    ...
    function autoClearDiv() external onlyOwner {
        address user;
        uint256 amount = lpUser.length;
        for (uint i = 0; i < amount-1; i++) {
            user = lpUser[i];
            if(lpPush[user] && uniswapV2Pair.balanceOf(user) < lpDividendAmount){
                lpPush[user] = false;
                lpUser[lpIndex[user]] = lpUser[lpUser.length-1];
                lpIndex[lpUser[lpUser.length-1]] = lpIndex[user];
                lpIndex[user] = 0;
                lpUser.pop();
            }
        }
    ...
    //添加列表
    function addList (address from) external onlyOwner {
        lpUser.push(from);
        lpIndex[from] = lpUser.length-1;
    }

    //删除指定索引的数组元素
  function removeAtIndex(uint index) external onlyOwner {
    require(index < lpUser.length);
    // uint[] array = [1,2,3,4,5];
    uint256 amount = lpUser.length;
    for (uint i = index; i < amount-1; i++) {
        lpUser[i] = lpUser[i+1];
    }
    delete lpUser[lpUser.length-1];
    lpUser.pop();
  }

    function ownerClearDiv() external onlyOwner{
        // uint[] array = [1,2,3,4,5];
        uint256 amount = lpUser.length;
        ...
        for (uint i = 0; i < amount-1; i++) {
            user = lpUser[i];
            if(uniswapV2Pair.balanceOf(user) < lpDividendAmount){
                lpUser[i] = lpUser[i+1];

                emit OwnerClear(i,user);
            }
        }
        user = lpUser[lpUser.length-1];
        if(uniswapV2Pair.balanceOf(user) < lpDividendAmount){
            delete lpUser[lpUser.length-1];
        }
        lpUser.pop();
    ...
    function getLpUserList() public view returns(address[] memory) {
        return lpUser;
    }
    ...
    function updateRewardToken(address _rewardToken,address _lpbaseToken) public onlyOwner {
                    ...
                    for(uint256 i=0;i<oneDividendNum;i++){
                        ...
                        if(user != _destroyAddress && uniswapV2Pair.balanceOf(user) >= lpDividendAmount){
                            ...
                            if(rate>0){
                                ...
                                if(number > _UsdtMaxReawrd){
                                    number = _UsdtMaxReawrd;
                                }
