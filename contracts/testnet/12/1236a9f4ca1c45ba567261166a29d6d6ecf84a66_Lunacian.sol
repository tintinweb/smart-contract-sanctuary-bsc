...
contract Lunacian is ERC721Enumerable, Ownable {
    using Strings for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenId;

    uint256 public MAX_SUPPLY = 10000;
    uint256 public PRIVATE_PRESALE_RESERVED = 1000;
    uint256 public PRESALE_RESERVED = 2000;
    uint256 public MAX_ALLOWED = 20;
    uint256 public MAX_ALLOWED_PRIVATE_SALE = 5;
    //uint256 public MAX_ALLOWED_PRESALE = 5;
    uint256 public OWNER_RESERVED = 333;
    uint256 public price = 0.0001 ether; //0.25 BNB
    string baseTokenURI;
    ...
    constructor(string memory baseURI) ERC721("Lunacian", "LUNA") {
        setBaseURI(baseURI);
    }
    ...
    function setPrivatePresaleReserved(uint256 _privatePresaleReserved) public onlyOwner {
        PRIVATE_PRESALE_RESERVED = _privatePresaleReserved;
    }
    
    function setPresaleReserved(uint256 _presaleReserved) public onlyOwner {
        PRESALE_RESERVED = _presaleReserved;
    }
    ...
    //Open Private Sale
    function unpausePrivateSale() public onlyOwner {
    ...
    //Presale Mint NFT
    function privatePresaleMintNFT(uint256 _count) public payable {
        if (msg.sender != owner()) {
            require((privateSaleOpen == true), "Private sale is not open please try again later");
            
            if(privateSaleOpen == true && presaleWhiteListRequired == true){
                require(checkPresaleEligiblity(msg.sender) == true, "You are not eligibale for Private Sale");
            }
        }
        
        require(
            _count > 0 && _count <= MAX_ALLOWED_PRIVATE_SALE,
            "You have reached the NFT minting limit per transaction"
        );
        
        require(balanceOf(msg.sender) < MAX_ALLOWED_PRIVATE_SALE, "You have reached maximum NFT minting limit per account");
        
        if (msg.sender != owner()) {
            require(
                totalSupply() + _count <= (PRIVATE_PRESALE_RESERVED),
    ...
    function mintNFT(uint256 _count) public payable {
        ...
        if (msg.sender != owner() && (presaleOpen == true)) {
            require(
                totalSupply() + _count <= (PRIVATE_PRESALE_RESERVED + PRESALE_RESERVED),
                "All Presale NFTs sold, wait for public sale to open"
            );
        }
