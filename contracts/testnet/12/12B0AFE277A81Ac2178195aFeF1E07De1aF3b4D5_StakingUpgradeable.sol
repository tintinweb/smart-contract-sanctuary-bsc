    ...
    UUPSUpgradeable
{
    ...
    function _getStakingReward(
        ...
        uint256 baseReward = ((valueStaked * rewardRate) /
            _rewardRateDecimals) / duration;
    ...
    function claimStakingReward(uint256 _stakingID) external returns (bool) {
        ...
        _transferToken(_tokenContract, _msgSender, stakingReward);
    ...
    function _unStake(
        ...
        require(userStakingInfo.valueStaked > 0, "You have no staking yet.");
        uint256 valueStaked = userStakingInfo.valueStaked;
        require(valueStaked > 0, "You have no staking.");
        ...
        returns (address[] memory stakersList, uint256 stakersCount)
    {
        ...
        for (uint256 i; i < _stakers.length; i++) {
            if (_stakers[i] == address(0)) {
                continue;
            } else if (_isAddressInList(_stakersList, _stakers[i])) {
                continue;
            } else {
    ...
    function setIsStakingValueFixed(
        bool _trueOrFalse
    ) external onlyOwner returns (bool) {
    ...
    function _tokensToUSDT(uint256 _tokenValue) private view returns (uint256) {
        return
            (((_tokenValue / 10 ** IERC20_EXTENDED(_tokenContract).decimals()) *
                IPresale(_presaleContract).getPricePerUSD()) /
    ...
    function _isValueEqualToStakingFixed(
        uint256 _value,
        uint256 _duration
        ...
        for (uint16 i; i < stakingValuesFixedLength; i++) {
            if (
                _value ==
                stakingValuesFixed[i] *
                    10 ** IERC20_EXTENDED(_tokenContract).decimals() &&
                _duration == stakingDuration[i]
            ) {
                return true;
            }
        }
    ...
    function _isUserAlreadyUsedFixedStakingPackage(
        address _userAddress,
        uint256 _value
    ) private view returns (bool) {
        Account storage userAccount = user[_userAddress];
