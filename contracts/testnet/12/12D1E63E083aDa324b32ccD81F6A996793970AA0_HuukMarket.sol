    ...
    UUPSUpgradeable
{
    ...
    function __HuukMarket_init_unchained() internal initializer {
        ...
        returns (uint256 _orderId)
    {
        ...
        if (isERC721) {
            ...
            require(
                IERC721Upgradeable(_input._tokenAddress).ownerOf(
                    _input._tokenId
                ) == _input._owner,
                "token-balance"
            );
            IERC721Upgradeable(_input._tokenAddress).safeTransferFrom(
                _input._owner,
                address(this),
                _input._tokenId
            );
        ...
        if (
            isHuukNFTs[_input._tokenAddress] &&
            IHuukNFT(_input._tokenAddress).getCreator(_input._tokenId) ==
            _input._owner &&
            amountFirstSale[_input._tokenAddress][_input._tokenId] == 0 &&
            lastBuyPriceInUSDT[_input._owner][
                keccak256(
                    abi.encodePacked(_input._tokenAddress, _input._tokenId)
                )
            ] ==
            0
        ) {
            amountFirstSale[_input._tokenAddress][_input._tokenId] = balance;
        }
        ...
        bytes32 _id = keccak256(
            abi.encodePacked(_input._tokenAddress, totalOrders, _input._owner)
        );
    ...
    function _acceptBid(AcceptBidInput memory _input) internal returns (bool) {
        ...
        uint256 exactPaymentAmount = (orderAmount * (ZOOM_FEE + xUser)) /
            ZOOM_FEE;
        // 1.025
...
//                )
    ...
    function _match(
        ...
        if (buyerRef != address(0)) {
            uint256 amountToBuyerRef = (orderAmount *
                xUser *
                (ZOOM_FEE - yRefRate)) / (ZOOM_FEE**2);
            ...
            if (discountForBuyer > 0) {
                ...
                _paid(
                    _paymentToken,
                    _buyer,
                    (orderAmount * discountForBuyer) / ZOOM_FEE
                );
            }
        }
        if (isHuukNFTs[order.tokenAddress]) {
            ...
            if (
                ...
                if (sellerRef != address(0)) {
                    {
                        ...
                        if (
                            ...
                            {
                                uint256 a = amountFirstSale[order.tokenAddress][
                                    order.tokenId
                                ] *
                                    _price *
                                    xCreator;
                                amountToSeller =
                                    amountToSeller -
                                    (a / ZOOM_FEE);
                                a = (a * (ZOOM_FEE - yRefRate)) / (ZOOM_FEE**2);
                                amountToSellerRef = a;
                                a =
                                    _quantity -
                                    amountFirstSale[order.tokenAddress][
                                ...
                                amountToSeller =
                                    ...
                                    (a / ZOOM_FEE);
                                a = (a * (ZOOM_FEE - yRefRate)) / (ZOOM_FEE**2);
                                amountToSellerRef = amountToSellerRef + a;
                            }
                        }
                        _paid(_paymentToken, sellerRef, amountToSellerRef);
                    }
                } else {
                    if (
                        amountFirstSale[order.tokenAddress][order.tokenId] >=
                        ...
                        uint256 b = (amountFirstSale[order.tokenAddress][
                            order.tokenId
                        ] *
                            _price *
                            xCreator) / ZOOM_FEE;
                        amountToSeller = amountToSeller - b;
                        b =
                            _quantity -
                            amountFirstSale[order.tokenAddress][order.tokenId];
                        b = (b * _price * xUser) / ZOOM_FEE;
                        amountToSeller = amountToSeller - b;
                    }
                }
                ...
                amountToSeller =
                    amountToSeller -
                    ((orderAmount * xUser) / ZOOM_FEE);
                if (sellerRef != address(0)) {
                    _paid(
                        _paymentToken,
                        sellerRef,
                        (orderAmount * xUser * (ZOOM_FEE - yRefRate)) /
                            (ZOOM_FEE**2)
                    );
                }
                ...
                if (order.owner == creator) {
                    ...
                    uint256 amountToCreator = IProfitEstimator(profitSender)
                        .profitToCreator(
                            order.tokenAddress,
                            _paymentToken,
                            order.tokenId,
                    ...
                    _paid(
                        _paymentToken,
                        order.owner,
                        amountToSeller - amountToCreator
                    );
                }
            }
        } else {
