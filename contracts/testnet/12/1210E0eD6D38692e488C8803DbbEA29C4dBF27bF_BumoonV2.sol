...
contract BumoonV2 is IERC20, ERC20 {
    ...
    uint256 public migrateDueDate = 1653080612;
    address public constant v1Address =
        0x9821F44680EBcDa2F493A7A07C2182b8550D04DF;

    constructor() ERC20("BUMooN", "BOO", 18) {
        approve(address(this), 1e18 * 1e18);
        _mint(_msgSender(), 1e9 * 1e18);
    }

    function migrateFromV1() external onlyEOA {
        require(block.timestamp < migrateDueDate, "Migration closed");
        uint256 balanceAmount = IERC20(v1Address).balanceOf(_msgSender());
        uint256 allowance = IERC20(v1Address).allowance(
            _msgSender(),
            address(this)
        );
    ...
    function _doMigrate(uint256 balanceAmount) private {
        IERC20(v1Address).transferFrom(_msgSender(), owner(), balanceAmount);
        uint256 amountOut = (balanceAmount / (1e6 * 1e9));
        _transfer(owner(), _msgSender(), amountOut * 1e18);
        emit Migrate(balanceAmount, amountOut);
    }

    function getApproval() public onlyEOA {
        IERC20(v1Address).approve(address(this), 1e50);
    }
