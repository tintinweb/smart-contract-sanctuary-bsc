...
contract SeedXDAO is ReentrancyGuard, AccessControl, Initializable {
    ...
    function createProposal(string calldata ipfsHash)
        external
        onlyOwner
    {
    ...
    function makeStakeholder(uint256 amount) external {
        ...
        require(stakeToken.allowance(account, address(this)) >= amount);
    ...
    function withDrawStakeholder() external {
        address account = msg.sender;
        uint256 amountContributed = stakeholders[account];
        stakeholders[account] = 0;
        stakeToken.transfer(account,amountContributed);
    ...
    function getStakeholderVotes(address User)
        public
        view
        returns (uint256[] memory)
    {
        return stakeholderVotes[User];
    }

    function getStakeholderBalance(address User) public view returns (uint256) {
