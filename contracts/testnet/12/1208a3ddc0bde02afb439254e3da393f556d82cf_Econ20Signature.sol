...
contract Econ20Signature is EconBaseContract {
    ...
    mapping(bytes32 => bool) private _handled;

    event Deposited(address user, address token, uint256 amount);
    event Withdrawn(bytes32 messageHash, address user, address token, uint256 amount);
    event WithdrawFailed(bytes32 messageHash, address user, address token, uint256 amount);
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(address(this), _token, _to, _amount, _expiryTime));
    }
    ...
    function withdraw(
        ...
        bytes32 messageHash = getMessageHash(_token, _msgSender(), _amount, _expiryTime);

        require(!_handled[messageHash]);

        (address recovered, ) = messageHash.tryRecover(signature);

        require(recovered == signer, "Error: InvalidSignature");
        require(block.timestamp <= _expiryTime, "Error: ExpiriedTime");

        _handled[messageHash] = true;

        if (_token == address(0)) {
            (bool success, ) = payable(_msgSender()).call{ value: _amount }("");

            if (success) emit Withdrawn(messageHash, _msgSender(), _token, _amount);
            else emit WithdrawFailed(messageHash, _msgSender(), _token, _amount);
        } else {
            try IERC20(_token).transfer(_msgSender(), _amount) returns (bool) {
                emit Withdrawn(messageHash, _msgSender(), _token, _amount);
            } catch {
                emit WithdrawFailed(messageHash, _msgSender(), _token, _amount);
            }
        }
    }
}
