 ...
 */
contract FrstLottery is ReentrancyGuard, IFrstLottery, Ownable {
    ...
    uint256 public maxPriceTicketInFrst = 50 ether;
    uint256 public minPriceTicketInFrst = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInFrst;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] frstPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInFrst;
        uint32 finalNumber;
    }
     ...
     * @param _frstTokenAddress: address of the FRST token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _frstTokenAddress, address _randomGeneratorAddress) {
        frstToken = IERC20(_frstTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of FRST to this contract
        uint256 amountFrstToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInFrst,
            _ticketNumbers.length
        );

        // Transfer frst tokens to this contract
        frstToken.safeTransferFrom(address(msg.sender), address(this), amountFrstToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInFrst += amountFrstToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInFrstToTransfer
        uint256 rewardInFrstToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInFrstToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        frstToken.safeTransfer(msg.sender, rewardInFrstToTransfer);

        emit TicketsClaim(msg.sender, rewardInFrstToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInFrst) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in FRST for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].frstPerBracket[j] =
                ...
                // A. No FRST to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].frstPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInFrst - amountToShareToWinners);

        // Transfer FRST to treasury address
        frstToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        frstToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInFrst += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInFrst >= minPriceTicketInFrst) && (_priceTicketInFrst <= maxPriceTicketInFrst),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInFrst: _priceTicketInFrst,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            frstPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInFrst: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(frstToken), "Cannot be FRST token");
    ...
    /**
     * @notice Set FRST price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInFrst: minimum price of a ticket in FRST
     * @param _maxPriceTicketInFrst: maximum price of a ticket in FRST
     */
    function setMinAndMaxTicketPriceInFrst(uint256 _minPriceTicketInFrst, uint256 _maxPriceTicketInFrst)
        external
        onlyOwner
    {
        require(_minPriceTicketInFrst <= _maxPriceTicketInFrst, "minPrice must be < maxPrice");

        minPriceTicketInFrst = _minPriceTicketInFrst;
        maxPriceTicketInFrst = _maxPriceTicketInFrst;
    }
     ...
     * @param _priceTicket price of a ticket (in FRST)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].frstPerBracket[_bracket];
        } else {
            return 0;
        }
    }
