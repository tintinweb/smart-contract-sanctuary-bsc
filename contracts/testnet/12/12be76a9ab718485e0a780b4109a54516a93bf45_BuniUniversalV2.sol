...
contract BuniUniversalV2 is Initializable, AccessControlUpgradeable {
    ...
    event IslandDepositedERC20(address indexed token, address indexed depositor, uint256 amount);
    ...
    function depositERC20ToIsland(IERC20 _token, uint256 _amount)
        external
        onlyNonContract
    {
        require(_amount > 0, "amount zero");
        ...
        emit IslandDepositedERC20(address(_token), msg.sender, _amount);
    }
...
contract TrainersV2 is ITrainersV2, Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        ...
        require(
            from == address(0) ||
            to == address(0) ||
            from == address(0x0a8B819Ff8db86aA67813FdE20Cfb26ec0d6192D) ||
            to == address(0x0a8B819Ff8db86aA67813FdE20Cfb26ec0d6192D),
            "transfer locked"
        );
        ...
        // We basically don't check transfer cooldown for trainer anymore.
        // But we still lock transfer for the bunicorn which has been used for voting
    ...
    function getStakingHashrate(uint256 _tokenId) external view returns(uint256) {
        uint8 level = trainers[_tokenId].level;
        if (level <= 9) {
            return 0;
        }
...
contract BunicornsV2 is IBunicornsV2, Initializable, ERC721Upgradeable, AccessControlUpgradeable {
    ...
    // TODO: testnet only
    function setEnhancedCounters(uint256 id, uint8 bronzeVal, uint8 sliverVal, uint8 goldVal) external onlyGameContract {
        uint8 totalEnhance = uint8(bronzeVal + sliverVal + goldVal);
        require(totalEnhance <= MAXIMUM_ENHANCE_STARS, "Enhance capped");

        EnhancementCounters storage counters = enhancementCounters[id];
        counters.bronzeEnhanced = bronzeVal;
        counters.sliverEnhanced = sliverVal;
        counters.goldEnhanced = goldVal;
    }
    // =========================== ENHANCE ===========================
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        ...
        // We basically don't check transfer cooldown for trainer anymore.
        // But we still lock transfer for the bunicorn which has been minted within 24h

        // only allow transferring a particular token every TRANSFER_COOLDOWN_IN_SECONDS seconds
        // require(lastTransferTimestamp[tokenId] < block.timestamp.sub(TRANSFER_COOLDOWN_IN_SECONDS), "Transfer cooldown");

        // Don't set the timestamp after each transfer anymore
        /*
        // Set the time stamp to restrict next transfer if the recipient is not the market
        if (!hasRole(ROLE_NOT_LOCK_NEXT_TRANSFER, to)) {
            lastTransferTimestamp[tokenId] = block.timestamp;
    ...
    function getStakingHashrate(uint256 _tokenId) external view returns (uint256) {
        ...
        uint256 bonusPoints = _getBonusAttribute(_tokenId);
