...
contract MinerPublicBuilding is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    address private standardParcelNFTAddress;
    address private minerV1NFTAddress;
    address private miningAddress;
    address private municipalityAddress;
    ...
    function mintBundle(address user, Municipality.Parcel[] memory parcels) external onlyMunicipality {
        uint256 minerHashrate = IMinerNFT(minerV1NFTAddress).hashrate();
        for(uint256 i = 0; i < parcels.length; ++i) {
            Municipality.Parcel memory p = parcels[i];
            uint256 parcelId = IParcelInterface(standardParcelNFTAddress).mint(user, p.x, p.y, p.parcelLandType);
            IParcelInterface(standardParcelNFTAddress).updateTokenLocking(parcelId, true);
            for(uint256 j = 0; j < 4; ++j) {
                uint256 miner = IMinerNFT(minerV1NFTAddress).mint(user);
                IMinerNFT(minerV1NFTAddress).updateTokenLocking(miner, true);
                IMining(miningAddress).deposit(user, miner, minerHashrate);
                IMunicipality(municipalityAddress).attachMinerToParcel(user, miner, parcelId);
            }
        }
    }
}
...
interface IMunicipality {
    function attachMinerToParcel(address user, uint256 minerId, uint256 parcelId) external;
}
