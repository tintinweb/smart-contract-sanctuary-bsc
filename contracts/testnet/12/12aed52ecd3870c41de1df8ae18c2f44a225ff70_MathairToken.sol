...
// File: contracts/mathair token.sol
...
contract MathairToken{
    mapping(address => uint256) private _balances; 
    mapping(address => mapping(address => uint256)) private _allowances;
    uint private _totalSupply = 1000000000 * 10 ** 18; 
    string private _name = " Mathair Token";
    string private _symbol = "BRUS";
    uint8 private _decimals = 18;
    uint8 private _MAXTXFEE = 5; 
    uint8 private txFee = 3;
    ...
    event Donation(address indexed donator, uint amountBUSD);

    event DonationUsed(address indexed reciever, uint amountUsed);
    ...
    constructor(IERC20 _busd) {
        ...
        uint initialownerbalance = _totalSupply;
    ...
    function ChangeTxFees(uint8 newtxfee)public isOwner returns(bool) {
        txFee = newtxfee;
        return true;
    }
    ...
    */
    ...
    function contractBusdBalance() public view returns (uint) {
        return busd.balanceOf(address(this));
    }

    function seeDonators() public view returns (address[] memory) {
        return donators;
    }
    ...
    function donate(uint amountBUSD)external returns(bool success) {
        busd.transferFrom(msg.sender, address(this), amountBUSD);
        _mint(msg.sender, (amountBUSD * BRUSxBUSDdonated) );
        donators.push(msg.sender);
        emit Donation(msg.sender, amountBUSD);

        return true;
        
    }

    function useDonations(address _to, uint _amount)external isOwner {
        busd.transfer(_to, _amount);
        emit DonationUsed(_to, _amount);
    }

    function changeBRUSxBUSDdonated(uint newBRUSxBUSDdonated)external isOwner {
        BRUSxBUSDdonated = newBRUSxBUSDdonated;
    }

    function changebusdAddress (IERC20 newAddress) external isOwner {
        busd = newAddress;
    }
