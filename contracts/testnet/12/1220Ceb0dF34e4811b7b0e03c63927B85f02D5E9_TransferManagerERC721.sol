...
interface IOnusExchange {
    ...
    function matchAskWithTakerBid(
        OrderTypes.TakerOrder calldata takerBid,
        OrderTypes.MakerOrder calldata makerAsk,
        OrderTypes.AdminSignature calldata adminSignature
    ) external;
...
library OrderTypes {
    ...
    bytes32 internal constant MAKER_ADMIN_HASH =
        0x3df3ee7f6dcb19cbc58fc57c33cccfdeeb8f96335196ca4a69c9a1e5b9389990;
...
import {OnusExchange} from "../OnusMarketplace.sol";
import {MarnusNft} from "../token/MarnusNft.sol";
...
contract TransferManagerERC721 is ITransferManagerNFT {
    using OrderTypes for OrderTypes.MakerOrder;
    using OrderTypes for OrderTypes.TakerOrder;
    using OrderTypes for OrderTypes.AdminSignature;

    address public immutable ONUS_EXCHANGE;

    bytes32 public immutable DOMAIN_EXCHANGE;

    MarnusNft public marnusNft;

    mapping(uint256 => bool) public isUseNonce;

    event MappingOffchainOnchain(
        address user,
        uint256 uniqueIdOffchain,
        uint256 tokenId
    );
    ...
    /**
     * @notice Constructor
     * @param _onusExchange address of the onus exchange
     */
    constructor(address _onusExchange) {
        ONUS_EXCHANGE = _onusExchange;
        DOMAIN_EXCHANGE = OnusExchange(_onusExchange).DOMAIN_SEPARATOR();
    }

    function setMarnusFactoryNft(address _marnusNft) public {
        require(Ownable(ONUS_EXCHANGE).owner() == msg.sender);
        marnusNft = MarnusNft(_marnusNft);
    }
    ...
    function transferNonFungibleToken(
        ...
        require(msg.sender == ONUS_EXCHANGE, "Transfer: Only onus Exchange");
        // https://docs.openzeppelin.com/contracts/2.x/api/token/erc721#IERC721-safeTransferFrom

        if (adminSignature.offchainOwner != address(0)) {
            ...
            require(
                !isUseNonce[adminSignature.uniqueIdOffchain],
                "Nonce not right"
            );

            require(
                adminSignature.collection == collection,
                "Collection not right"
            );

            require(
                adminSignature.offchainOwner == from,
                "Collection not right"
            );

            isUseNonce[adminSignature.uniqueIdOffchain] = true;

            uint256 tokenIdJustMint = marnusNft.mint(to);

            emit MappingOffchainOnchain(
                from,
                adminSignature.uniqueIdOffchain,
                tokenIdJustMint
            );
