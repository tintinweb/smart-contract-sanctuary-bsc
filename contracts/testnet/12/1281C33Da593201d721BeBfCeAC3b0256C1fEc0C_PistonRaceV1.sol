...
contract PistonRaceV1 is OwnableUpgradeable {
    ...
    uint256 public max_payout_cap;           // 100k PISTON or 10% of supply
    ...
    function initialize() external initializer {
        ...
        //br34pToken = IToken(_BR34PTokenAddress); //@TODO
        pistonToken = IToken(address(0xccd40212f489CaB747b02a82b802F551b13f3e10));

        //Referral Balances
        //@TODO values from toni
    ...
    function updatePistonTokenPriceFeed(address _value) public onlyOwner {
        pistonTokenPriceFeed = ITokenPriceFeed(_value);
        STORE_BUSD_VALUE = true;
    }
    
    function updatePistonTokenPriceFeedEnabled(bool _value) public onlyOwner {
        STORE_BUSD_VALUE = _value;
    }
    ...
    function unstakeBoost(uint256 _amount) external {
        //@TODO: IMPLEMENTATION
    }
    ...
    function deposit(address _upline, uint256 _amount) external {
        ...
        (uint256 realizedDeposit, uint256 taxAmount) = pistonToken.calculateTransferTaxes(_addr, _amount);
        ...
        // here comes new cash
        // track it
        usersRealDeposits[_addr].deposits += _amount; // new cash here
        if(STORE_BUSD_VALUE){
    ...
    function _claim_out(address _addr) internal {
        ...
        uint256 vaultBalance = pistonToken.balanceOf(address(this));

        // @TODO:: dontknow if we need this. we will mint manually in small portions...?
        /*
    ...
    function eject() external {
        User memory user = users[msg.sender]; // user stats
        UserDepositReal memory userDeposits = usersRealDeposits[msg.sender]; // stats on real cash deposits (without compount and bonus)

        uint256 pistonTokenPrice = pistonTokenPriceFeed.getPrice(1);
    ...
    function teamAirdrop(uint256 teamId, uint256 _amount, bool excludeOwner) external {

        //@TODO implementation

    }

    //@dev Send specified PISTON amount supplying an upline referral
    
    function airdrop(address _to, uint256 _amount) external {

        address _addr = msg.sender;

        (uint256 _realizedAmount, uint256 taxAmount) = pistonToken.calculateTransferTaxes(_addr, _amount);
        ...
        require(
            ...
            "PISTON to contract transfer failed; check balance and allowance, airdrop"
        );
