...
contract NFTCHTS is Initializable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        __ERC721_init("NFT CHTS", "NFTCHTS");
    ...
    function setInitBoost(uint initialRobiBoost) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _initialRobiBoost = initialRobiBoost;
    }
    ...
    function increaseBoost(uint tokenId, uint rb) public onlyRole(RB_SETTER_ROLE) {
    ...
    function descreaseBoost(uint tokenId, uint rb) public onlyRole(RB_SETTER_ROLE) {
        require(_exists(tokenId), "Token does not exist");
        require(_tokens[tokenId].stakeFreeze == false, "Token is staked");
        Token storage token = _tokens[tokenId];
