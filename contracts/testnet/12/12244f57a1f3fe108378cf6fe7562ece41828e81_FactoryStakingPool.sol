...
contract StakePoolBNBForBNB is States, ReentrancyGuard, Ownable {
    ...
    function setTokens(uint256 _lockPeriod, uint256 date) public onlyOwner {
    ...
    function setData(uint256 _days) public onlyOwner {
        lockPeriod = _days;
    }
...
contract FactoryStakingPool is Ownable{
    ...
    function createBNBForBNB(address ownerAddress, uint256 _lockPeriod, uint256 unlockDate) public onlyOwner returns (address)
    {
        StakePoolBNBForBNB toDeploy = new StakePoolBNBForBNB();
        toDeploy.setTokens(_lockPeriod, unlockDate);
