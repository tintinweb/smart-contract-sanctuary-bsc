...
contract SelporteToken is ERC20, Ownable {
    ...
    event Transaction(uint256 _amount, uint256 _time, Status _type);
        ...
        returns (string memory)
    {
        ...
        return "minted successfull to succesfully";
    }
        ...
        returns (bool)
    {
        ...
        transactions[_newCount] = TransactionDetails(
            _amount,
            block.timestamp,
            Status.tokenTransfer
        );
        _counter.increment();
        emit Transaction(_amount, block.timestamp, Status.tokenTransfer);
        return true;
    }
    ...
    function burnToken(uint256 _amount) public onlyOwner {
        ...
        _counter.increment();
        emit Transaction(_amount, block.timestamp, Status.tokenBurn);
    }

    // Enlist All transaction made
    function allTransaction()
        public
        view
        returns (TransactionDetails[] memory)
    {
        uint256 totalTransactions = _counter.current();
        TransactionDetails[] memory _transactions = new TransactionDetails[](
            totalTransactions
        );
        for (uint256 i = 0; i < totalTransactions; i++) {
            TransactionDetails storage currentDetail = transactions[i];
            _transactions[i] = currentDetail;
        }
        return _transactions;
    }

    // returns burning count
    function burningCount() public view returns (uint256) {
