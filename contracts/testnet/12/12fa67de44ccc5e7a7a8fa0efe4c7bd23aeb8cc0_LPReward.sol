...
interface ISystemTokenRouter {
    function getAmountsOut(uint amountIn, address[] calldata path) external  view returns (uint[] memory amounts);
}

interface ISystemTokenFactory {
...
contract LPReward is Ownable {
    ...
    address public immutable SystemToken;
    address public swapRouter;
    ISystemTokenFactory public immutable swapFactory;
    ...
    event RecordRemoveLiquidityGivesystemToken(address indexed recipient, address indexed pair, uint systemtoken, uint amountA, uint amountB, uint liquidity);
    event ClaimLiquiditysystemToken(address indexed recipient, address indexed pair, uint systemtoken, uint amountA, uint amountB);
    event Rescue(address indexed to, uint amount);
    event RescueToken(address indexed token, address indexed to, uint amount); 

    constructor(address systemtoken, address factory) {
        require(systemtoken != address(0) && factory != address(0), "LPReward: Zero address(es)");
        swapFactory = ISystemTokenFactory(factory);
    ...
    function recordRemoveLiquidity(address recipient, address tokenA, address tokenB, uint amountA, uint amountB, uint liquidity) external lock onlyRouter { 
        ...
        uint amountsystemToken;
        if (tokenA != SystemToken && tokenB != SystemToken) {
            address tokenTosystemTokenPair = swapFactory.getPair(tokenA, SystemToken);
            if (tokenTosystemTokenPair != address(0)) {
                amountsystemToken = ISystemTokenRouter(swapRouter).getAmountsOut(amount0, getPathForToken(tokenA))[1];
            }

            tokenTosystemTokenPair = swapFactory.getPair(tokenB, SystemToken);
            if (tokenTosystemTokenPair != address(0)) {
                if (amountsystemToken != 0) {
                    amountsystemToken = amountsystemToken + ISystemTokenRouter(swapRouter).getAmountsOut(amount1, getPathForToken(tokenB))[1];
                } else  {
                    amountsystemToken = ISystemTokenRouter(swapRouter).getAmountsOut(amount1, getPathForToken(tokenB))[1] * 2;
                }
            } else {
                amountsystemToken = amountsystemToken * 2;
            }
        } else if (tokenA == SystemToken) { 
            amountsystemToken = amount0 * 2;
        } else {
            amountsystemToken = amount1 * 2;
        }
        
        if (amountsystemToken != 0 && amountsystemToken <= availableReward() && IBEP20(SystemToken).balanceOf(address(this)) >= amountsystemToken) {
            require(IBEP20(SystemToken).transfer(recipient, amountsystemToken), "LPReward: Erroe while transfering");
            lpRewardUsed = lpRewardUsed + amountsystemToken;
            emit RecordRemoveLiquidityGivesystemToken(recipient, pair, amountsystemToken, amountA, amountB, liquidity);            
        } else {
            uint amountS0;
            uint amountS1;
            {
    ...
    function claimBonus(address pair, address recipient) public lock {
        ...
        uint amountsystemToken = systemtokenAmountForPair(pair, amountA, amountB);
        require (amountsystemToken > 0, "LPReward: No SystemToken pairs to token A and token B");
        require (amountsystemToken <= availableReward(), "LPReward: Available reward for the period is used");
        
        require(IBEP20(SystemToken).transfer(recipient, amountsystemToken), "LPReward: Error while transfering");
        lpRewardUsed += amountsystemToken;
        emit ClaimLiquiditysystemToken(recipient, pair, amountsystemToken, amountA, amountB);            
    }

    function unclaimedAmountsystemToken(address recipient, address pair) external view returns (uint) {
        uint amountA;
        uint amountB;
        if (unclaimedAmounts[recipient][pair].length != 0) {
            amountA = unclaimedAmounts[recipient][pair][0];
        ...
        return systemtokenAmountForPair(pair, amountA, amountB);
    }
    ...
    function systemtokenAmountForPair(address pair, uint amountA, uint amountB) private view returns (uint amountsystemToken) {
        address tokenA = pairTokens[pair][0];
        address tokenB = pairTokens[pair][1];
        if (tokenA != SystemToken && tokenB != SystemToken) {
            address tokenTosystemTokenPair = swapFactory.getPair(tokenA, SystemToken);
            if (tokenTosystemTokenPair != address(0)) {
                amountsystemToken = ISystemTokenRouter(swapRouter).getAmountsOut(amountA, getPathForToken(tokenA))[1];
            }

            tokenTosystemTokenPair = swapFactory.getPair(tokenB, SystemToken);
            if (tokenTosystemTokenPair != address(0)) {
                if (amountsystemToken != 0) {
                    amountsystemToken = amountsystemToken + ISystemTokenRouter(swapRouter).getAmountsOut(amountB, getPathForToken(tokenB))[1];
                } else  {
                    amountsystemToken = ISystemTokenRouter(swapRouter).getAmountsOut(amountB, getPathForToken(tokenB))[1] * 2;
                }
            } else {
                amountsystemToken = amountsystemToken * 2;
            }
        } else if (tokenA == SystemToken) {
            amountsystemToken = amountA * 2;
        } else {
            amountsystemToken = amountB * 2;
        }
    }
