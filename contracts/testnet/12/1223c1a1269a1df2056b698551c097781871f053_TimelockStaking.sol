...
contract TimelockStaking is Ownable, Initializable, ReentrancyGuard {
    ...
    */
    ...
    function unstakableUsers (address _txUser,uint256 _txNo) public view returns (bool[] memory) {
        require( _txNo != 0,"transaction number cannot be 0");
        require( _txNo <= stakingTx[_txUser].txNo," transaction is not created ");
        bool[] memory _unstakableUsers;
        for(uint256 i=1; i<=_txNo; i++){
            if(!stakingTx[_txUser].stakingPerTx[_txNo].stakingOver){
