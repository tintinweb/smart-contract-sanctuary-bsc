...
struct Drop {
    ...
    address sender;
    uint256 expired;
    uint256 prize; // sender prize for drop
    uint256 amount; // total amount of distributed tokens
}

contract MerkleSender {
    ...
    function add(address token, bytes32 root, uint256 amount, uint256 expired) external payable {
    ...
    function send(bytes32 root, bytes32[] calldata proof, address[] calldata to, uint256[] calldata amounts, uint256 nonce, bytes32[] calldata clean) external {
        ...
        if (drop.amount == leaf_amount) {
            // drop complete
            prize = drop.prize; // send rest of prize to sender
            delete drops[root]; // remove complete drop
            _cleanup(root, clean);
        } else {
            // proportional prize:
            prize = drop.prize * leaf_amount / drop.amount;
            // decrement drop amount:
            drop.amount = drop.amount - leaf_amount;
            // update drop prize:
            drop.prize = drop.prize - prize;
        }
    ...
    function clame(bytes32 root) external {
        ...
        payable(drop.sender).transfer(drop.prize);
        delete drops[root];
    }
}