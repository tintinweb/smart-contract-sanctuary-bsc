...
contract Presale is Ownable {
    ...
    mapping(uint => uint256) public durationStart; //start claim
    ...
    function getVestingDurationStart(uint256 pid) public view returns (uint) {
        return durationStart[pid];
    } 
    function setVestingDurationStart(uint256 pid,uint256 _time) external onlyOwner {
        durationStart[pid] = _time;
    } 
    ...
    function buyToken(uint256 pid, address refaddr, uint ln) public payable {
        ...
        if (refaddr != address(0)) {
            ...
            _refLogs[refaddr].raised[beneficiary] = _refLogs[refaddr].raised[beneficiary].add(weiAmount); // refLogs.push(RefInfo(pid, msg.sender, weiAmount));
    ...
    function checkClaim(uint pid, address holder) public view returns(string memory mss, uint time, uint256 amount, uint256 reward){
        ...
        //duration: 
        if (durationStart[pid] != 0) {
            if(block.timestamp < durationStart[pid]) {
                return ("Claim on ", durationStart[pid], 0, 0);
            }

            uint256 _amount = _buyed[holder][pid].locked.sub(_buyed[holder][pid].claimed);
            ( ,uint256 _rewardd) = rewardAmount(pid, holder);

            if(block.timestamp >= durationStart[pid] + _vesting[pid][0].openingTime) {
                return ("Released", 0, _amount, _rewardd);
            }
            uint256 duration = block.timestamp - durationStart[pid];
            return ("Claim amount ", 0, _amount.mul(duration).div(_vesting[pid][0].openingTime), _rewardd);
        }
    ...
    function rewardAmount(uint pid, address holder) public view returns(uint, uint256){
        ...
        if(_buyed[holder][pid].lastClaim != 0){
            ...
            if (durationStart[pid] != 0) {
                timeElapsed = block.timestamp - durationStart[pid];
            } else {
