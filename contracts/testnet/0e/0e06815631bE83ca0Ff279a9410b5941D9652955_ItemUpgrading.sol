...
abstract contract BaseUpgrading is Auth, EIP712Upgradeable {
  ...
  function _getDexSeed() private view returns (uint) {
    ...
    return reserve00 + reserve01 + reserve02;
  }
  ...
  function _finishUpgrade(
    ...
    if (_success) {
      ...
      for(uint i = 0; i < _materials.length; i++) {
        ...
        require(_totalMaterialOwnPrice > 0, "Upgrading: total own price is invalid");
        itemNft.updateFailedUpgradingAmount(_materials[i], failedUpgradingAmount + _totalFee * suppliedItemOwnPrice / _totalMaterialOwnPrice);
      }
    }
  }
