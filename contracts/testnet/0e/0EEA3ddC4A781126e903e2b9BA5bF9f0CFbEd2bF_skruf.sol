...
Buy tax
Tax: 10% (slippage 11%)
4% Payouts in $BTCB, $ETH, $BUSD, $MATIC, $ADA, $BNB & $SKRUF, more added later
3% Auto buyback 
1% Gas Refunds 
1% Marketing in $Bnb
0.6% admin 
...
Payouts
→ all the sellers tax is given back in SKRUFple tokens → no $SKRUF price dump from selling the sell tax
Out of the seling tax:
50% holders in $BTCB, $ETH, $ADA, $MATIC, $BNB, $BUSD & $SKRUF
...
interface DAPP {
    ...
    function ChooseSKRUFplePayoutTokens(uint32 Token1,uint32 Slice1,uint32 Token2,uint32 Slice2,
...
// skruf Network contract

contract skruf is ERC20, ReentrancyGuard {
    string public constant symbol = "SKRUF"; //SKRUF
    string public constant name = "skruf"; //skruf Network
...
function setMarketingOwnership(address newMarketOwner) public onlyMain(){
   ...
   // address constant routerAddress=0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    constructor() {
        ...
        _enableTransfer = true; //defaSKRUF
...
//        __balanceOf[address(this)] = __totalSupply/2;
     ...
     //launch_time = block.timestamp;  //now as defaSKRUF - used for tax calculation
   AddTokentoPayoutList(_pancakeRouter.WETH(), "BNB"); //BNB added as defaSKRUF
   AddTokentoPayoutList(address(this), "SKRUF"); //this contract token
   
   
   //defaSKRUF testnet tokens for debugging
 ...
 //contract addresses are as defaSKRUF excluded from auto payouts of rewards and have
  ...
  function UpdateRegister(address recipient, bool ExcludedfromTax) private
   {
        ...
        _AutoPayoutEnabled[recipient] = true; // only human holders get payed auto rewards as defaSKRUF
         N_holders++;
         _excludedfromTax[recipient] = ExcludedfromTax;
         
         _SplitTokenRewards[recipient] = true;
         _RewardTokens[recipient].Slice1 = 75; //defaSKRUF 75% bnb
         _RewardTokens[recipient].Slice1 = 25; //25% SKRUF
...
function checkRewards(address addr) public view returns (uint256)
{
   return _earnings[addr];
        ...
        if(LotteryPot > 0)
        {
             if(random(100) < 5) //won - buyamount SKRUFplied 1/20 probability
