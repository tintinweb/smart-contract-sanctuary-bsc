...
contract DrivenLockup is Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        lockups.push(Lockup(0, 30, 0, 200, 951293759, 0, 0, 0, 15, false, false)); // 1% liquidity
        lockups.push(Lockup(1, 90, 0, 100, 1426940638, 0, 0, 0, 15, false, false)); // 1.5% liquidity
        lockups.push(Lockup(2, 180, 0, 0, 1902587518, 0, 0, 0, 10, true, true)); // 2% liquidity
        lockups.push(Lockup(2, 360, 0, 0, 1902587518, 0, 0, 0, 10, true, true)); // 2% liquidity
    ...
    function claimReward(uint8 _stakeType) external nonReentrant {
        ...
        for (uint256 j = 0; j < stakes.length; j++) {
            ...
            if (stake.stakeType != _stakeType) continue;
            if (stake.amount == 0) continue;
            if(!lockup.isHarvest && stake.end <= block.timestamp) continue;
    ...
    function compoundReward(uint8 _stakeType) external nonReentrant {
        ...
        Stake[] storage stakes = userStakes[msg.sender];
        Lockup storage lockup = lockups[_stakeType];
        require(lockup.isAutoCompound, "Compound Reward not allowed"); 
    ...
    function emergencyWithdraw(uint8 _stakeType) external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            uint256 fee = (amountToTransfer*lockup.emergencyWithdrawFee)/100;
