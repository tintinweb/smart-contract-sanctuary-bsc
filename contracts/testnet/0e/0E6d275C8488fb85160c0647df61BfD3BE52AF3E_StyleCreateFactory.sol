...
contract StyleCreateFactory {
     ...
     */
    function createStyle(address admin,address payable dexAddress,uint96 royaltyRate) external virtual returns (address) {
        ...
        styleItem.transferOwnership(admin);
        address result = address(styleItem);
        emit StyleCreated(admin, result);
        return result;
    }
}
...
import {DEXConfig, EIP712, Manager} from "./lib/DEXConfig.sol";
...
contract MgNftDex is DEXConfig, OrderHelper, ReentrancyGuard, OrderValidator, TransferHelper {
    ...
    constructor(address _manager, address feeRecipientAddr, string memory _name, string memory _version, uint256 _chainId) DEXConfig(
        _manager, feeRecipientAddr, _name, _version, _chainId)  {
    }
...
contract DEXConfig is EIP712, DEXEventsAndErrors, Manager {
    ...
    constructor(address _manager, address _feeRecipient, string memory _name, string memory _version, uint256 _chainId)
