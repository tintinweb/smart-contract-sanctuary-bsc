...
//    gameMiningPool = IFOTAToken(0x0A4E1BdFA75292A98C15870AeF24bd94BFFe0Bd4);
  ...
  function addPVEReward(address _user, uint[] memory _data) external onlyGameContract {
    ...
    // land lord
    uint halfLandlordLength = (_data.length - 7) / 2;
  ...
  function setContracts(address _heroNft, address _fotaToken, address _landLordManager, address _farmAddress, address _gameProxyContract) external onlyMainAdmin {
  ...
  function useTreasuryShareAmount(uint _amount) external onlyMainAdmin {
    ...
    gameMiningPool.releaseGameAllocation(treasuryAddress, _convertUsdToFota(_amount / 100));
  }
  ...
  function useFarmShareAmount(uint _amount) external onlyMainAdmin {
    ...
    uint distributedToFarm = _convertUsdToFota(_amount / 100);
  ...
  function summonPrestigeHero(address _user, uint _amount) external onlyContractAdmin {
    require(userRewards[_user] >= _amount, "RewardManager: insufficient balance");
