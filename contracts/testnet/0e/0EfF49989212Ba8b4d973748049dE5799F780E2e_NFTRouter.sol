...
interface IDataStorage {
    ...
    function getNewPowerLevel(uint256 _rand, uint256 _mainPower, uint256 _materialPower) external view returns (uint256 , uint256);
    function getSampleUpgradeBot() external view returns(uint256);
}
...
contract NFTRouter is Ownable {
    ...
    function _getUpgradeBotLv(uint256 _mainPower, uint256 _materialPower, uint256 _rand) internal view returns (uint256, uint256){
        ...
        //        }
        //        return (0, 0);
        uint256 _sample = dataStorage.getSampleUpgradeBot();
        uint256 _seed = uint256(keccak256(abi.encodePacked(_rand, blockhash(block.number - 1))));
        (uint256 _lv, uint256 _power) = dataStorage.getNewPowerLevel(_seed % _sample, _mainPower, _materialPower);
        return (_lv, _power);
    }
        ...
        if (_request.rType == RequestType.BUY) {
            ...
            if (_request.rType == RequestType.CREATE) {
                ...
                if (_request.rType == RequestType.UPGRADE) {
                    (uint256 _lv, uint256 _power) = _getUpgradeBotLv(_request.data[0], _request.data[1], _rand);
    ...
    function _processRequest(uint256 _id, uint256 _blockNum, bytes32 _blockHash, bytes memory _signature) internal {
        // latest
    ...
    function _createDroidBot(uint256 _pandoBoxId, uint256 _seed) internal isLock(address(pandoBox), _pandoBoxId) {
    ...
    isLock(address(droidBot), _droidBot1Id)
    {
        ...
        {
            uint256 _upgradeFee = upgradeBaseFee * (15 ** _info0.level) / (10 ** _info0.level);
        ...
        droidBot.burn(_id1);
        uint256[] memory _data = new uint[](5);
    ...
    function createPandoBoxBundle(uint256 _option, uint256 _quantity) external {
        ...
        if (pandoBoxCreated[_ndays] + _quantity <= pandoBoxPerDay) {
            // take fee
            if (_createPandoBoxFee > 0) {
                _createPandoBoxFee = _createPandoBoxFee * _quantity;
    ...
    function createDroidBotBundle(uint256[] memory _pandoBoxIds) external {
    ...
    function upgradeDroidBotBundle(uint256[] memory _droidBot0Ids, uint256[] memory _droidBot1Ids) external {
        ...
        for (uint i = 0; i < _length; i++) {
            _upgradeDroidBot(_droidBot0Ids[i], _droidBot1Ids[i], _seeds[i]);
        }
    }
    ...
    function processRequestBundle(uint256[] memory _ids, uint256[] memory _blockNums, bytes32[] memory _blockHashs, bytes[] memory _signatures) external {
        require(
            _ids.length == _blockNums.length && _ids.length == _blockHashs.length &&
            _ids.length == _signatures.length && _ids.length > 0,
            "NFTRouter: invalid length"
        );
        uint256 _length = _ids.length;
        for (uint256 i = 0; i < _length; i ++) {
            _processRequest(_ids[i], _blockNums[i], _blockHashs[i], _signatures[i]);
        }
    }
