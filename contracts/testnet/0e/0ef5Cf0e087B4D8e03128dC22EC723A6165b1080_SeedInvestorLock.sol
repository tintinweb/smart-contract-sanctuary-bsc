...
// 10% - For every 25%, 3 months are unlocked.
contract SeedInvestorLock is
    Initializable,
    OwnableUpgradeable,
    IERC20,
    PausableUpgradeable,
    IDeposit
{
    ...
    uint16 public constant phase = 4;
    ...
    function addSeedInvestor(address investor, uint256 amount)
        public
        whenNotPaused
        onlyOwner
    {
        if (block.timestamp > saleStartDate_) {
            revert LockPeriodOngoing();
        }

        require(totalSupply > amount, "Not enough money in pool");
        ...
        emit Transfer(address(this), investor, amount);
    }

    // For every 25%, 3 months are unlocked.
    ...
    function withdrawAllUnsold(uint256 amount) public onlyOwner {
        require(balanceOf[address(this)] >= amount, "Not enough money in pool");
