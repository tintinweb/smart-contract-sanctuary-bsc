    ...
    IERC20Upgradeable
{
    ...
    function initialize(
        ...
        pancakeV2Router = IPancakeRouter02(pancakeRouterAddress);
        ...
        _defaultFees = checkFees(
            FeeTier(4, 4, 4, feeReceiver)
        );
        
        excludeFromReward(feeReceiver);
    ...
    //maximum amount that user can transfer in 1 transaction
    ...
    {
        uint256 _fees = _tier.liquidityFee + _tier.taxFee + _tier.ecoSystemFee;
        require(_fees <= _maxFee, "Oi: Fees exceeded max limitation");
    ...
    function setLiquidityFeePercent( uint256 _liquidityFee)
        ...
        onlyOwner
    {
        checkFeesChanged(_defaultFees, _defaultFees.liquidityFee, _liquidityFee);
    ...
    function setTaxFeePercent(uint256 _taxFee)
        ...
        onlyOwner
    {
        checkFeesChanged(_defaultFees, _defaultFees.taxFee, _taxFee);
        _defaultFees.taxFee = _taxFee;
    }

    function setEcoSystemFeePercent(uint256 _ecoSystemFee)
        external
        onlyOwner
    {
        checkFeesChanged(_defaultFees, _defaultFees.ecoSystemFee, _ecoSystemFee);
        _defaultFees.ecoSystemFee = _ecoSystemFee;
    }

    function setEcoSystemFeeAddress(address _ecoSystem)
        external
        onlyOwner
    {
        require(_ecoSystem != address(0), "Oi: Address Zero is not allowed");
    ...
    ) internal 
    {
        ...
        //transfer amount, it will take tax, liquidity fee, owner fee
    ...
    function swapAndEvolve() public onlyOwner lockTheSwap {
        // split the contract balance into halves
            uint256 contractBnbBalance = IERC20Upgradeable(WBNB).balanceOf(address(this));
            ...
            uint256 initialBalance = IERC20Upgradeable(address(this)).balanceOf(msg.sender);
            // swap BNB for Tokens
            swapBnbForTokens(half);

            // how much BNB did we just swap into?
            uint256 newBalance = IERC20Upgradeable(address(this)).balanceOf(msg.sender);
            uint256 swapeedToken = newBalance - initialBalance;

            _approve(msg.sender, address(this), swapeedToken);
            IERC20Upgradeable(address(this)).transferFrom(msg.sender,address(this),swapeedToken);
    ...
    function swapBnbForTokens(uint256 bnbAmount) private {
        ...
        path[0] = WBNB;
        path[1] = address(this);
        _approve(owner(), address(pancakeV2Router), bnbAmount);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        ...
        _approve(address(this), address(pancakeV2Router), tokenAmount);
        _approve(address(this), address(pancakeV2Router), bnbAmount);
    ...
    function _takeFees(address sender, FeeValues memory values) private {
        ...
        _takeFee(sender, values.tOwner, _defaultFees.ecoSystem);
    }
        ...
        returns (FeeValues memory)
    {
        ...
        uint256 tTransferFee = tValues.tLiquidity + tValues.tOwner;
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tValues.tFee, tTransferFee, _getRate());
        return FeeValues(rAmount, rTransferAmount, rFee, tValues.tTransferAmount, tValues.tLiquidity, tValues.tFee, tValues.tOwner);
    }
        ...
        returns (tFeeValues memory)
    {
        tFeeValues memory tValues = tFeeValues(
            ...
            calculateFee(tAmount, _defaultFees.ecoSystemFee)
        );

        console.log("_defaultFees.ecoSystemFee: ", _defaultFees.ecoSystemFee);
        console.log("tValues.tOwner: ", tValues.tOwner);
        ...
        )
    {
        ...
        console.log("tTransferFee:", tTransferFee);
        uint256 rTransferFee = tTransferFee * currentRate;
        console.log("rTransferFee: ", rTransferFee);
    ...
    function withdrawBnb(uint256 _amount) public onlyOwner {
        IERC20Upgradeable(WBNB).transfer(msg.sender, _amount);
    }

    function setMaxFee(uint256 rate) external onlyOwner () {
        _maxFee = rate;
    } 
