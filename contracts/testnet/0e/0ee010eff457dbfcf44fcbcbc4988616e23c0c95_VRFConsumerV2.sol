...
contract VRFConsumerV2 is VRFConsumerBaseV2 {
    ...
    struct AirDropCampaign {
        ...
        address[] contestantsAddresses; // lock up on request
        address[] winners;
    ...
    function configureNewAirdrop(string memory nameOfContest, uint32 winnersCount, address[] memory contestantAddressArray, uint256 dateOfAnnouncement, string memory imageURL, uint256 prizeWorth, string[] memory tasks) external onlyOwner {
        address[] memory _winners;
        uint256 _randomSeed;
        AirDropCampaign memory campaign = AirDropCampaign(nameOfContest, winnersCount, contestantAddressArray, _winners, dateOfAnnouncement, false, imageURL, prizeWorth, _randomSeed, false,tasks);
        airdropCampaigns.push(campaign);
        requestCounter += 1;
    }

    // Stop the contest so that contestants can no longer be added to the 
...
function removeContestant(uint contestIndex, address contestant) external onlyOwner {
    ...
    function addContestant(uint contestIndex, address contestant_address) external onlyOwner {
        ...
        airdropCampaigns[contestIndex].contestantsAddresses.push(contestant_address);
    }
    ...
    function drawContest(uint contestIndex) external onlyOwner {
        ...
        uint256[] memory _tempWinners = _shuffle(airdropCampaigns[contestIndex].contestantsAddresses.length - 1, airdropCampaigns[contestIndex].randomSeed);
        for (uint i = 0; i < airdropCampaigns[contestIndex].numberOfWinners; i++) {
                airdropCampaigns[contestIndex].winners.push(airdropCampaigns[contestIndex].contestantsAddresses[_tempWinners[i]]);
            }  
    }
    ...
    function getWinners(uint contestIndex) external view returns(address[] memory) {
