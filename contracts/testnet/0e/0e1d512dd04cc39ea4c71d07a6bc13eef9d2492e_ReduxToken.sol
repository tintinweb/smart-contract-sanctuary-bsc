...
contract ReduxToken is ERC20Upgradeable {
    ...
    uint256 internal preSalePrice = 1e17; //  1 REDUX is equal to 0.001 BNB
    uint256 internal publicSalePrice = 2e17; //  1 REDUX is equal to 0.001 BNB
    address public makrtetingAddress = 0x7ACf46627094FA89339DB5b2EB862F0E8Ea4D9fc;

    
     function initialize() public initializer {
        __ERC20_init("Redux", "REDUX");
        owner = msg.sender;
    }
    modifier onlyOwner() {
    ...
    receive() external payable {}
     /**
     * @notice When Trade is enabled , this buy function will let you trade your REDUX to BNB
     */
     function buy(bytes32[] calldata proof) payable public {
        ...
        uint256 amountCalculation = (msg.value.div(preSalePrice)).mul(1e18);
        payable(makrtetingAddress).transfer(msg.value);
        _mint(msg.sender, amountCalculation);
    }

     function publicSale() payable public {
        require(msg.value > 0, "LOW BALANCE");
        uint256 amountCalculation = (msg.value.div(publicSalePrice)).mul(1e18);
        payable(makrtetingAddress).transfer(msg.value);
        _mint(msg.sender, amountCalculation);
    }


   function _leaf(string memory payload)
        internal
        pure
        returns (bytes32)
    {
