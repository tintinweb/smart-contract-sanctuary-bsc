...
abstract contract Crowdsale {
   ...
   */
  constructor(uint256 _rate, address _wallet, starCardToken _token) {
  ...
  /**
    ...
    */
  function _changeToken(starCardToken newToken) virtual internal {
    token = newToken;
  }
...
contract CrowdSale is Crowdsale, Pausable, FinalizableCrowdsale {
    ...
    function buyToken(address _beneficiary) external payable onlyWhileOpen whenNotPaused{
      ...
      require(msg.value <= 1500, "Maximum purchase Limit existed");
    ...
    function changeToken(starCardToken newToken) virtual external onlyOwner onlyWhileOpen whenNotPaused{
    ...
    function whitelisting(address whiteListingAddress) virtual external onlyOwner onlyWhileOpen whenNotPaused{
      require(whiteListingAddress != address(0), "whitelist Address: Address cant be zero address");
    }
 ...
 */
contract starCardToken is Context, IERC20, IERC20Metadata, Ownable {

    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    string  private constant _name = "STAR CARD TOKEN";
    string  private constant _symbol = "SCSG";
     ...
     */
    function burn (address account, uint256 token) external onlyOwner {
        _burn(account,token);
    }
