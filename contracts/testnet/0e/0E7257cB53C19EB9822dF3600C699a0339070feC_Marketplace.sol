...
contract Marketplace is Ownable {
    ...
    function addProfit(uint256[] calldata tokenIDs) external onlyOwner {
        ...
        for (uint256 i = 0; i < tokenIDs.length; i ++) {
            ...
            if (!profitSet[tokenIDs[i]].status) {
                profitSet[tokenIDs[i]] = Profit(true, profits.length);
                profits.push(tokenIDs[i]);
            }
        }
    }
    ...
    function removeProfit(uint256[] memory tokenID) external onlyOwner {
        ...
        require(tokenID.length <= profits.length, "exceed maximum");

        for (uint256 i; i < tokenID.length; i ++) {
            require(flexNFT.ownerOf(tokenID[i]) != address(0), "no nft created");
            removeProfitItem(tokenID[i]);
        }

    }

    function removeProfitItem(uint256 tokenID) private {
        if (profitSet[tokenID].status) {
            uint256 lastTokenIndex = profits.length - 1;
            uint256 tokenIndex = profitSet[tokenID].index;
            uint256 lastTokenId = profits[lastTokenIndex];
    
            profits[tokenIndex] = lastTokenId;
            profitSet[lastTokenId] = Profit(true, tokenIndex);
    
            delete profitSet[tokenID];
            profits.pop();
        }
    }
