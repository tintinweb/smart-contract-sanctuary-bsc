...
// Note that this pool has no minter key of igo (rewards).
// Instead, the governance will call igo distributeReward method and send reward to this pool at the beginning.
contract igoRewardPool is IRewardPool, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. igos to distribute per block.
        uint256 lastRewardTime; // Last time that igos distribution occurs.
        uint256 accigoPerShare; // Accumulated igos per share, times 1e18. See below.
    ...
    // The time when igo mining starts.
    uint256 public poolStartTime;

    // The time when igo mining ends.
    uint256 public poolEndTime;
    uint256 public lastTimeUpdateRewardRate;
    uint256 public accumulatedRewardPaid;

    uint256 public igoPerSecond = 0.00008127861 ether; 
    uint256 public runningTime = 7210 days;
    ...
    constructor(address _igo, uint256 _poolStartTime) public {
        if (_poolStartTime == 0 || _poolStartTime < block.timestamp) {
            _poolStartTime = block.timestamp;
        }
        if (_igo != address(0)) igo = IERC20(_igo);
        poolStartTime = _poolStartTime;
        poolEndTime = poolStartTime.add(runningTime);
        igoPerSecond = TOTAL_REWARDS.div(runningTime);
    ...
    modifier onlyOperator() {
        require(
            operator == msg.sender,
            "igoRewardPool: caller is not the operator"
        );
        _;
    }
    ...
    modifier onlyOperatorOrTreasury() {
        require(
            ...
            "igoRewardPool: caller is not the operator/treasury"
        );
        _;
    }
    ...
    // Update the given pool's igo allocation point. Can only be called by the owner.
        ...
        returns (uint256)
    {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime)
                return poolEndTime.sub(poolStartTime).mul(igoPerSecond);
            return poolEndTime.sub(_fromTime).mul(igoPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime)
                return _toTime.sub(poolStartTime).mul(igoPerSecond);
            return _toTime.sub(_fromTime).mul(igoPerSecond);
        }
    }

    // View function to see pending igos on frontend.
    function pendingReward(uint256 _pid, address _user)
        public
        view
        override
        returns (uint256)
    {
        ...
        uint256 accigoPerShare = pool.accigoPerShare;
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _igoReward = _generatedReward.mul(pool.allocPoint).div(
                totalAllocPoint_
            );
            accigoPerShare = accigoPerShare.add(
                _igoReward.mul(1e18).div(tokenSupply)
            );
        }
        return
            user.amount.mul(accigoPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            ...
            pool.accigoPerShare = pool.accigoPerShare.add(
                _igoReward.mul(1e18).div(tokenSupply)
            );
        }
        pool.lastRewardTime = block.timestamp;
        ...
        nonReentrant
    {
        ...
        if (user.amount > 0) {
            uint256 _pending = user
                .amount
                .mul(pool.accigoPerShare)
                .div(1e18)
                .sub(user.rewardDebt);
            if (_pending > 0) {
                _safeigoTransfer(msg.sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accigoPerShare).div(1e18);
    ...
    function _withdraw(
        ...
        uint256 _pending = user
            ...
            .sub(user.rewardDebt);
        if (_pending > 0) {
            _safeigoTransfer(_account, _pending);
    ...
    // Safe igo transfer function, just in case if rounding error causes pool to not have enough igos.
    function _safeigoTransfer(address _to, uint256 _amount) internal {
        uint256 _igoBal = igo.balanceOf(address(this));
        if (_igoBal > 0) {
            if (_amount > _igoBal) {
                igo.safeTransfer(_to, _igoBal);
            } else {
                igo.safeTransfer(_to, _amount);
            }
        }
    }
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 180 days) {
            // do not allow to drain core token (igo or lps) if less than 180 days after pool ends
            require(_token != igo, "igo");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
