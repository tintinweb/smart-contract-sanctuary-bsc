...
contract Token is ERC20, Ownable {
	...
	uint256 public swapTokensAtAmount = 100000 * 10 ** uint256(decimals());   

	
	uint256 public buyDividendFee = 3;      
	...
	constructor() ERC20("HydroDAO", "HDD") {
		...
		usdt = 0xb072DE429701d2257315f737DF68BeE7dbB5D5C5;   // bsctest
		...
		_mint(msg.sender, 531441000 * 10**4 * (10**decimals()));
  ...
  function mint(uint256 amount) public onlyOwner returns (bool) {
	...
	function distributeHDD(uint256 tokens) public {

		bool success = safeTransfer(address(this),address(dividendTracker),tokens);
		//(success,) = address(dividendTracker).call{value: forDividends}("");

		if(success) {
			dividendTracker.distributeDividends_HDD(tokens);
			emit SwapAndSendTo(tokens, "DIVIDENDS");
