...
contract  GGGTOKEN is IERC20, Ownable {
 ...
 constructor(
        string[] memory  stringP,//代币名称，简写
        uint256[] memory  uintP,//精度，数量（不用加精度个0）最大单笔买入交易量，最大持有量，杀多少秒机器人，裂变多少个地址， 买营销手续费，销毁手续费，回流手续费, 卖营销手续费，销毁手续费，回流手续费，最大单笔卖出交易量,买LP分红手续费，卖LP分红手续费
        address[] memory addrP,//所有权地址，营销钱包地址，路由合约地址，交易对地址，ido地址（传空地址）
        uint8[] memory boolP,//是否所有权才能添加流动性，转帐手续费设置（0无手续费，1与买一样，2与卖一样），ido推广手续费（0买卖都有，1与买一样，2与卖一样）
        uint256[] memory inviters//代数奖励
        ){ 
            ...
            _swapTokensAtAmount = _tTotal.mul(5).div(10**6);
   ...
   function getExcludedFromFee(address account) public view returns (bool) {
    ...
    function transferSwap(uint256 contractTokenBalance) private{
        ...
        if(_denominator>0){
            ...
            IERC20(_token).transfer(_marketAddr,  tokenBal.mul(_buyMarketingFee.add(_sellMarketingFee)).div(_denominator));
    ...
    function setInviterFee(uint8[] memory inviters )  external onlyOwner {
        _inviters = inviters;
        uint256 inviterFee;
        for(uint i ;i<_inviters.length;i++){
