...
// File: contracts/interfaces/game/IRatioScore.sol
...
interface IRatioScore {
    ...
    /**
     * @dev Set which match of which round, trigger when which team let the ball
     */
    event SetWhichGamesConced(uint256 indexed round, uint256 indexed which, uint256 indexed index, uint256 conced);
    ...
    /**
     * @dev Set which(index) team of which match of which round let the ball (index=0 concession=5 means team1 have 5 goals in the beginning)
     */
    function setWhichGamesConced(uint256 round, uint256 which, uint256 index, uint256 conced) external;
...
interface IBaseAdmini {
     ...
     */
    event SetWhichlGamesResult(uint256 indexed round, uint256 indexed which, uint256 indexed end);
    ...
    /**
     * @dev set the game result of which match in which round (round=2**256-1 represents the current round) (e.g. Team 0 score 5, Team 1 score 10, then call in twice this interface, first time end=5 second time end=10)
     */
    function setWhichlGamesResult(uint256 round, uint256 which, uint256 result) external;
...
abstract contract BaseFunction is IBaseUser, IBaseAdmini, Ownable {
     ...
     * @dev set the game result of which match in which round (round=2**256-1 represents the current round) (e.g. Team 0 score 5, Team 1 score 10, then call in twice this interface, first time end=5 second time end=10)
     */
    function setWhichlGamesResult(uint256 round, uint256 which, uint256 result) public virtual override onlyOwner {
        round = _setRound(round);
        ...
        _gameRoundInfo.roundGameMap[round].gameMap[which].resultList.push(result);
...
contract RatioScore is BaseFunction, IRatioScore {
    ...
    /**
     * @dev Set which(index) team of which match of which round let the ball (index=0 conced=5 means team1 will have 5 goals in the beginning)
     */
    function setWhichGamesConced(uint256 round, uint256 which, uint256 index, uint256 conced) public virtual override onlyOwner {
        round = _setRound(round);
        ...
        require(GAME_STATE_INIT == _gameRoundInfo.roundGameMap[round].gameMap[which].gameState, "RatioScore: setWhichGamesConced gameState is not init");
        require(index < (_gameRoundInfo.roundGameMap[round].gameMap[which].resultNum - 1), 
            "RatioScore: setWhichGamesConced Illegal index"); // minus one is because that the last result is to divde the money equally to users，so the teams number=result numbers -1

        _pointRoundInfo.roundPointMap[round].pointMap[which].point[index] = conced;

        emit SetWhichGamesConced(round, which, index, conced);
    }
