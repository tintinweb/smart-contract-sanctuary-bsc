...
contract Loan {
    ...
    function payBack(uint256 acceptId, uint256 _payBackAmount) public onlyBorrower {
        ...
        acceptLoanMap[msg.sender][acceptId].payedAmount += mPaybackAmount - feeAmount;
    ...
    function getOwedAmount(address _borrower) public view returns (uint256 result){
        ...
        while(index < acceptLoanMap[_borrower].length){
            result += (acceptLoanMap[_borrower][index].tokenAmount - acceptLoanMap[_borrower][index].payedAmount);
            index ++;
        }
        return result;
    }
