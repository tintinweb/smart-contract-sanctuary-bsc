...
import "./ActivityToken.sol";


contract ActivityMint is Pausable, AccessControl {

    //控制权限
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");

    //活动开始时间
    uint256 constant public START = 1652716800;
    //活动结束时间
    uint256 constant public END = 1653580800;
    //7天过期
    uint256 constant EXPIRED = 7 days;
    //权益1T
    uint256 constant POWER_GB = 1024;
    //领取次数
    uint256 constant public MAX = 10000;
    //中心化数据
    string constant MEAT_DATA = "http://123";

    //计数
    uint256 public count;
    //NFT
    ActivityToken public nft;
    ...
    function mint() external whenNotPaused {
        //活动时间开启领取
        require(block.timestamp > START && block.timestamp < END, "Error Time");
        require(count <= MAX, "already max");
        require(!records[msg.sender], "already minted");
        //领取
        uint256 endTime = block.timestamp + EXPIRED;
        records[msg.sender] = true;
        count++;
        uint256 tokenId = nft.safeMint(msg.sender, MEAT_DATA, POWER_GB, endTime);
        emit MintEvent(msg.sender, address(nft), tokenId, POWER_GB, MEAT_DATA, endTime, block.timestamp);
...
contract ActivityToken is Pausable, ERC721, ERC721URIStorage, ERC721Burnable, ERC721Enumerable, AccessControl {
    ...
    mapping(uint256 => BaseInfo) public details;
    ...
    external whenNotPaused onlyRole(PAUSER_ROLE) returns(bool) {
