...
import "./ArmyMintingStation.sol";
import "./DutyProfile.sol";

/** @title ArmySpecialV1.
 * @notice It is a contract for users to mint exclusive NFTs
 * based on a DUTY price and userId.
 */
contract ArmySpecialV1 is Ownable {
    using SafeBEP20 for IBEP20;
    using SafeMath for uint256;

    ArmyMintingStation public armyMintingStation;
    DutyProfile public dutyProfile;

    IBEP20 public dutyToken;

    uint256 public maxViewLength;
    uint256 public numberDifferentArmies;

    // Map if address for a armyId has already claimed a NFT
    mapping(address => mapping(uint8 => bool)) public hasClaimed;

    // Map if armyId to its characteristics
    mapping(uint8 => Armies) public armyCharacteristics;

    // Number of previous series (i.e. different visuals)
    uint8 private constant previousNumberArmyIds = 10;
    ...
    // Event to notify a new army is mintable
    event ArmyAdd(uint8 indexed armyId, uint256 thresholdUser, uint256 costDuty);

    // Event to notify one of the armies' requirements to mint differ
    event ArmyChange(uint8 indexed armyId, uint256 thresholdUser, uint256 costDuty, bool isActive);

    // Event to notify when NFT is successfully minted
    event ArmyMint(address indexed to, uint256 indexed tokenId, uint8 indexed armyId);

    constructor(
        ArmyMintingStation _armyMintingStation,
        IBEP20 _dutyToken,
    ...
    /**
     * @dev Mint NFTs from the ArmyMintingStation contract.
     * Users can claim once.
     */
    function mintNFT(uint8 _armyId) external {
        // Check that the _armyId is within boundary
        require(_armyId >= previousNumberArmyIds, "ERR_ID_LOW");
        require(armyCharacteristics[_armyId].isActive, "ERR_ID_INVALID");

        address senderAddress = _msgSender();

        // 1. Check _msgSender() has not claimed
        require(!hasClaimed[senderAddress][_armyId], "ERR_HAS_CLAIMED");

        uint256 userId;
        bool isUserActive;

        (userId, , , , , isUserActive) = dutyProfile.getUserProfile(senderAddress);

        require(userId < armyCharacteristics[_armyId].thresholdUser, "ERR_USER_NOT_ELIGIBLE");

        require(isUserActive, "ERR_USER_NOT_ACTIVE");

        // Check if there is any cost associated with getting the army
        if (armyCharacteristics[_armyId].dutyCost > 0) {
            dutyToken.safeTransferFrom(senderAddress, address(this), armyCharacteristics[_armyId].dutyCost);
        }

        // Update that _msgSender() has claimed
        hasClaimed[senderAddress][_armyId] = true;

        uint256 tokenId = armyMintingStation.mintCollectible(
            senderAddress,
            armyCharacteristics[_armyId].tokenURI,
            _armyId
        );

        emit ArmyMint(senderAddress, tokenId, _armyId);
    }
    ...
    function addArmy(
        ...
        require(!armyCharacteristics[_armyId].isCreated, "ERR_CREATED");
        require(_armyId >= previousNumberArmyIds, "ERR_ID_LOW_2");

        armyCharacteristics[_armyId] = Armies({
            tokenURI: _tokenURI,
        ...
        numberDifferentArmies = numberDifferentArmies.add(1);

        emit ArmyAdd(_armyId, _thresholdUser, _dutyCost);
    }

    /**
     * @dev It transfers the DUTY tokens back to the chef address.
    ...
    function updateArmy(
        ...
        require(armyCharacteristics[_armyId].isCreated, "ERR_NOT_CREATED");
        armyCharacteristics[_armyId].thresholdUser = _thresholdUser;
        armyCharacteristics[_armyId].dutyCost = _dutyCost;
        armyCharacteristics[_armyId].isActive = _isActive;

        emit ArmyChange(_armyId, _thresholdUser, _dutyCost, _isActive);
    }
    ...
    function canClaimSingle(address _userAddress, uint8 _armyId) external view returns (bool) {
        if (!dutyProfile.hasRegistered(_userAddress)) {
            return false;
        } else {
            uint256 userId;
            bool userStatus;

            (userId, , , , , userStatus) = dutyProfile.getUserProfile(_userAddress);

            if (!userStatus) {
                return false;
            } else {
                bool claimStatus = _canClaim(_userAddress, userId, _armyId);
                return claimStatus;
            }
        }
    }

    function canClaimMultiple(address _userAddress, uint8[] calldata _armyIds) external view returns (bool[] memory) {
        require(_armyIds.length <= maxViewLength, "ERR_LENGTH_VIEW");
        ...
        if (!dutyProfile.hasRegistered(_userAddress)) {
            ...
            if (!userStatus) {
                ...
                bool[] memory responses = new bool[](_armyIds.length);

                for (uint256 i = 0; i < _armyIds.length; i++) {
                    bool claimStatus = _canClaim(_userAddress, userId, _armyIds[i]);
    ...
    function _canClaim(
        ...
        uint256 armyThreshold = armyCharacteristics[_armyId].thresholdUser;
        bool armyActive = armyCharacteristics[_armyId].isActive;

        if (hasClaimed[_userAddress][_armyId]) {
            return false;
        } else if (!armyActive) {
            return false;
        } else if (userId >= armyThreshold) {
            return false;
        } else {
            return true;
        }
...
import "./DutyArmies.sol";

/** @title ArmyMintingStation.
 * @dev This contract allows different factories to mint
 * Duty Collectibles/Armies.
 */
contract ArmyMintingStation is AccessControl {
    DutyArmies public dutyArmies;
    ...
    constructor(DutyArmies _dutyArmies) public {
        dutyArmies = _dutyArmies;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    /**
     * @notice Mint NFTs from the DutyArmies contract.
     * Users can specify what armyId they want to mint. Users can claim once.
     * There is a limit on how many are distributed. It requires DUTY balance to be > 0.
     */
    function mintCollectible(
        ...
        uint256 tokenId = dutyArmies.mint(_tokenReceiver, _tokenURI, _armyId);
        return tokenId;
    }

    /**
     * @notice Set up names for armies.
     * @dev Only the main admins can set it.
     */
    function setArmyName(uint8 _armyId, string calldata _armyName) external onlyOwner {
        dutyArmies.setArmyName(_armyId, _armyName);
    }
    ...
    function changeOwnershipNFTContract(address _newOwner) external onlyOwner {
        dutyArmies.transferOwnership(_newOwner);
    }
}
...
/** @title DutyArmies.
 * @notice It is the contracts for DutySwap NFTs.
 */
contract DutyArmies is ERC721, Ownable {
    using Counters for Counters.Counter;

    // Map the number of tokens per armyId
    mapping(uint8 => uint256) public armyCount;

    // Map the number of tokens burnt per armyId
    mapping(uint8 => uint256) public armyBurnCount;

    // Used for generating the tokenId of new NFT minted
    Counters.Counter private _tokenIds;

    // Map the armyId for each tokenId
    mapping(uint256 => uint8) private armyIds;

    // Map the armyName for a tokenId
    mapping(uint8 => string) private armyNames;

    constructor(string memory _baseURI) public ERC721("Duty Armies", "DS") {
        _setBaseURI(_baseURI);
    }

    /**
     * @dev Get armyId for a specific tokenId.
     */
    function getArmyId(uint256 _tokenId) external view returns (uint8) {
        return armyIds[_tokenId];
    }

    /**
     * @dev Get the associated armyName for a specific armyId.
     */
    function getArmyName(uint8 _armyId) external view returns (string memory) {
        return armyNames[_armyId];
    }

    /**
     * @dev Get the associated armyName for a unique tokenId.
     */
    function getArmyNameOfTokenId(uint256 _tokenId) external view returns (string memory) {
        uint8 armyId = armyIds[_tokenId];
        return armyNames[armyId];
    }
    ...
    function mint(
        ...
        _tokenIds.increment();
        armyIds[newId] = _armyId;
        armyCount[_armyId] = armyCount[_armyId].add(1);
        _mint(_to, newId);
    ...
    /**
     * @dev Set a unique name for each armyId. It is supposed to be called once.
     */
    function setArmyName(uint8 _armyId, string calldata _name) external onlyOwner {
        armyNames[_armyId] = _name;
    }
    ...
    function burn(uint256 _tokenId) external onlyOwner {
        uint8 armyIdBurnt = armyIds[_tokenId];
        armyCount[armyIdBurnt] = armyCount[armyIdBurnt].sub(1);
        armyBurnCount[armyIdBurnt] = armyBurnCount[armyIdBurnt].add(1);
        _burn(_tokenId);
    }
}
