...
contract DSBStake is Ownable {
    ...
    function getStakedNFT(address addr) public view returns (uint256[] memory) {
        require(
            accountTracker[addr].stakeCount > 0,
            "address does not stake a single nft"
        );
        uint256[] memory output = new uint256[](
            accountTracker[addr].stakeCount
        );
        for (uint256 i = 1; i <= accountTracker[addr].stakeCount; i++) {
            StakeInfo storage sInfo = stakeTracker[
                accountTracker[addr].tokenId[i]
            ];
            if (sInfo.currentOwner == addr && sInfo.isStaked == true) {
                output[i] = accountTracker[addr].tokenId[i];
            }
        }
