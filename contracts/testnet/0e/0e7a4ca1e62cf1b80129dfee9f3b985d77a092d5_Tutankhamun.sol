...
/**
   ...
   2% fee auto moved to team wallet
   8% Marketing fee
...
contract Tutankhamun is Context, IERC20, Ownable {
    ...
    string private _name = "Tutankhamun";
    string private _symbol = "TTKH";
	...
	function _getteamWallet() public view virtual returns (address) {
        return _teamWallet;
	}
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketing, uint256 tteam) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, data.tFee, data.tLiquidity, data.tMarketing, data.tteam);
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, TData memory) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tLiquidity).sub(tMarketing).sub(tteam);
        return (tTransferAmount, TData(0, tFee, tLiquidity, tMarketing, tteam, 0));
    }
    ...
    function _getRValues( TData memory _data) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rLiquidity).sub(rMarketing).sub(rteam);
