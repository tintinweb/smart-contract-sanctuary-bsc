...
contract Test is Context, IERC20, Ownable {
    ...
    function _transferDeduction(address sender, address recipient, uint256 tAmount, uint256 _tfee, uint256 _actualAmt) private {
        ...
        _tOwned[recipient] = _tOwned[recipient].add(_actualAmt);
        _tOwned[adminAddress] = _tOwned[adminAddress].add(_tfee);
        emit Transfer(sender, recipient, tAmount);
    }
    ...
    //     public view returns(bool _buySellType, bool _deduction, uint256 newAmountIn, uint256 fee, uint256 _feededuct) {
    //     bool buySellType = false;
    //     if(sender == uniswapV2Pair) {
    //         buySellType = true;
    //     }
    ...
    //         _newAmountIn = amount.sub(_fee);
    //     }
    //     return(buySellType, deduction, _newAmountIn, _fee, buyTokenCommission);
    // }
