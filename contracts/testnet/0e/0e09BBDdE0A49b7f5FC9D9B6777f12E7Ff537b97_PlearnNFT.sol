    ...
    ReentrancyGuard
{
    ...
    function mintReserve(uint256 amount, address minter) external onlyOwner returns (uint256 id) {
        require(amount > 0, "Plearn NFT: Amount cant be zero");
        require(minter != address(0), "Plearn NFT: Minter cant be zero");
        require(minReservableTokenId + amount <= maxSupply, "Plearn NFT: Reserve range over max supply");
    ...
    function updateMintReserve(
        ...
        uint256 amount,
        address minter
    ) external onlyOwner whenMintReserveUpdatable(reservedRangeId) {
        require(amount > 0, "Plearn NFT: Amount cant be zero");
        require(minter != address(0), "Plearn NFT: Minter cant be zero");
        ReservedRange storage reserveRange = reservedRangeIds[reservedRangeId];
        require(reserveRange.minTokenId + amount <= maxSupply, "Plearn NFT: Reserve range over max supply");
    ...
    function removeMintReserve(uint256 reservedRangeId) external onlyOwner whenMintReserveUpdatable(reservedRangeId) {
        delete reservedRangeIds[reservedRangeId];
    }

    //Public functions
    function getMintReserve(uint256 reservedRangeId) external view returns (ReservedRange memory) {
        return reservedRangeIds[reservedRangeId];
    }

    function getTokens(uint256 cursor, uint256 size) external view returns (uint256[] memory, uint256) {
        uint256 length = size;
