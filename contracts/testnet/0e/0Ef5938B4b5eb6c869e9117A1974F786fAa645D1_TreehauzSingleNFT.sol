...
contract TreehauzSingleNFT is 
    ...
    */
    ...
    struct SingleItem {
        address assetContract; //token contract address
        uint256 tokenId; //contract address token ID
        address tokenMinter; //collab creator
        address tokenOwner; //current token owner
        string tokenIPFSURI; //IPFS URI
        uint256 totalRoyalty; //map address with royalty
        bool secondary; // indicator for secondary market
    }

    struct royaltyInfos {
        mapping (uint256 => address) royaltyReceiver;
        mapping (uint256 => uint256) royaltyPercentage;
        uint256 noOfRoyaltyReceiver;
    }
    ...
    /**
    * @dev Prefix base uri used in _baseURI().
    */
    ...
    */
    ...
    mapping(uint256 => mapping (address => Whitelist)) public _whitelisted;
    ...
    */
    ...
    event RoyaltyInfoUpdate(uint256 tokenId, address[] sender, uint256[] royaltyPercentage);
    ...
    // * @param _setTokenURI IPFS base URI.
    // */
    // function _setBaseURI(string memory _setTokenURI) external onlyOwner {
    //     require(bytes(_setTokenURI).length > 0, "Treehauz : No base token URI set.");
    //     _baseTokenURI = _setTokenURI;
    // }
    ...
    */
    function getIPFSCID(uint256 tokenId) internal view returns (string memory) {
        require(_exists(tokenId), "Treehauz : Query for nonexistent token");
        return _tokenItems[tokenId].tokenIPFSURI;
    }
    ...
    returns (string memory) {
        ...
        if (bytes(_tokenURI).length > 0) {
            console.log("bytesURI");
            return string(abi.encodePacked(base, _tokenURI));
        }
        console.log("not bytesURI");
    ...
    function getSecondaryInfo(uint256 _tokenId) 
    public view returns (bool) {
        return _tokenItems[_tokenId].secondary;
    }
    ...
    /// @param _tokenId - update token information after owner change on marketplace
    /// permission - caller must be marketplace
    function updateTokenInfo(uint256 _tokenId, address _owner) 
    external {
            require (_msgSender() == _marketplaceContract, "Treehauz: Caller not valid.");
            _tokenItems[_tokenId].tokenOwner = _owner;
            // after first change of owner will always be secondary
            _tokenItems[_tokenId].secondary = true; 
    }
    ...
    external view returns (
        ...
        royaltyAmount = (_salePrice / 100000) * _tokenItems[_tokenId].totalRoyalty; // set return for total royalty
    }
    ...
    external view returns (
        ...
        require(_exists(_tokenId), "Treehauz: nonexistent token");
        address[] memory __receiver = new address[](_tokenRoyalty[_tokenId].noOfRoyaltyReceiver);
        uint256[] memory __percentage = new uint256[](_tokenRoyalty[_tokenId].noOfRoyaltyReceiver);

        for (uint64 i=0; i < _tokenRoyalty[_tokenId].noOfRoyaltyReceiver; i++ ) {
            __receiver[i] = address(_tokenRoyalty[_tokenId].royaltyReceiver[i]);
            __percentage[i] = _tokenRoyalty[_tokenId].royaltyPercentage[i];
        }
        
        return (__receiver, 
                __percentage, 
                _tokenRoyalty[_tokenId].noOfRoyaltyReceiver);
    }
    ...
    */
    ...
    external onlyAdminOrOwner {
        ...
        require(_address[0] != address(0), "Treehauz: invalid primary royalty receiver");
        require(_royaltyPercentage[0] >= 0, "Treehauz: Royalty percentage less than 0 is not valid");
        _tokenItems[_tokenId].tokenMinter = _address[0];
        _tokenItems[_tokenId].totalRoyalty = 0;
        for (uint i=0; i < _address.length; i++) {
            if (_address[i] != address(0)) {
                _tokenRoyalty[_tokenId].royaltyReceiver[i] = _address[i];
                if (_royaltyPercentage[i] > 0) {
                    _tokenRoyalty[_tokenId].royaltyPercentage[i] = _royaltyPercentage[i];
                    _tokenItems[_tokenId].totalRoyalty += _royaltyPercentage[i];
                }
                else {
                    _tokenRoyalty[_tokenId].royaltyPercentage[i] = 0;
                }
            }
            else {
                _tokenRoyalty[_tokenId].royaltyReceiver[i] = address(0);
                _tokenRoyalty[_tokenId].royaltyPercentage[i] = 0;
            }
        }
        _tokenRoyalty[_tokenId].noOfRoyaltyReceiver = _address.length;

        emit RoyaltyInfoUpdate(_tokenId, _address, _royaltyPercentage);
    }

    /**
    * @dev Set royalty info during minting.
    * Permission: Internal caller.
    * @param _tokenId NFT asset.
    * @param _address Royalty receiver.
    * @param _royaltyPercentage Royalty percentage of each receiver.
    * @param _totalRoyalty Combined total royalty.
    */
    function setRoyaltyInfoMint(uint256 _tokenId, 
                            address[] memory _address, 
                            uint256[] memory _royaltyPercentage, 
                            uint256 _totalRoyalty) 
    ...
    /**
    * @dev Set royalty receiver(s).
    * Permission: External caller.
    * @param _tokenId NFT asset.
    * @param _address Royalty receiver(s) addresses.
    */
    function _setRoyaltyReceiver(uint256 _tokenId, address[] memory _address) 
    external {
        require(_exists(_tokenId), "Treehauz: nonexistent token");
        // require(_msgSender() == _tokenItems[_tokenId].tokenMinter || 
        require(_msgSender() == _marketplaceContract, "Treehauz: Caller is not token minter");
        require(_address[0] != address(0), "Treehauz : Cannot set royalty address 0x0");
        _tokenItems[_tokenId].tokenMinter = _address[0];
        for (uint128 i=0; i < _address.length; i++) {
            _tokenRoyalty[_tokenId].royaltyReceiver[i] = _address[i];
        }
        _tokenRoyalty[_tokenId].noOfRoyaltyReceiver = _address.length;
    }
    ...
    /**
    * @dev Set royalty percentage for all receiver.
    * Permission: External caller.
    * @param _tokenId NFT asset.
    * @param _royaltyPercentage Royalty percentage of each receiver.
    * @param _totalRoyalty Combined total royalty.
    */
    function _setRoyaltyPercentage(uint256 _tokenId, 
                                    uint256[] memory _royaltyPercentage, 
                                    uint256 _totalRoyalty) 
    external {
        require(_exists(_tokenId), "Treehauz: nonexistent token");
        ...
        require(_msgSender() == _marketplaceContract, "Treehauz: Caller is not token minter");
        require(_totalRoyalty < 90000, "Treehauz : Total royalty must less than 90%");
        _tokenItems[_tokenId].totalRoyalty = _totalRoyalty;
        for (uint i=0; i < _royaltyPercentage.length; i++) {
            if (_royaltyPercentage[i] > 0) {
                _tokenRoyalty[_tokenId].royaltyPercentage[i] = _royaltyPercentage[i];
            }
            else {
        ...
        _tokenRoyalty[_tokenId].noOfRoyaltyReceiver = _royaltyPercentage.length;
    }
    ...
    */
    function approveMarket(uint256 _tokenId, address _assetContract) external 
    returns (bool) {
        require(_exists(_tokenId), "Treehauz: nonexistent token");
        require(_msgSender() == IERC721Upgradeable(_assetContract).ownerOf(_tokenId) || 
        _msgSender() == _marketplaceContract, "Treehauz: caller not valid");
        setApprovalForAll(_marketplaceContract, true);
        approve(_marketplaceContract, _tokenId);

        emit SetApproveMarket(_msgSender(), _marketplaceContract, _tokenId);
        return true;
    }
    ...
    */
    function setMarketplaceContract(address _marketplaceAddress) external onlyOwner {
        _marketplaceContract = _marketplaceAddress;
    }
    ...
    */
    ...
    function singleMint(string memory _tokenURI, 
                                address[] memory _tokenMinter, 
                                uint256[] memory _royaltyPercentage,
                                uint256 _totalRoyalty) 
    external returns (uint) {
        console.log("test1");
        uint newItemId = _singleTokenIds.current();
        require(bytes(_tokenURIs[newItemId]).length == 0, "Treehauz : Cannot set URI twice");
        _tokenURIs[newItemId] = _tokenURI;
        require(_msgSender() != address(0), "Treehauz : Minter does not exists");
        require(_royaltyPercentage[0] >= 0, "Treehauz : Royalty percentage less than 0 is not valid");
        require(_totalRoyalty < 90000, "Treehauz : Total royalty must less than 90%");
        console.log("test2");
        SingleItem memory newSingleItem =  SingleItem ({
            assetContract : address(this), //token contract address
            tokenId : newItemId, //contract address token ID
            tokenMinter : _tokenMinter[0], //collab creator
            tokenOwner : _msgSender(), //current token owner
            tokenIPFSURI : _tokenURI, //IPFS content identifier
            totalRoyalty : _totalRoyalty, //map address with royalty
            secondary : false // set false for primary
        });
        _tokenItems[newItemId] = newSingleItem;

        if (_tokenMinter.length > 1) {
            bool _royalty = setRoyaltyInfoMint(newItemId, _tokenMinter, _royaltyPercentage, _totalRoyalty);
            require(_royalty, "Treehauz: Royalty info not valid");
        }
        else {
            _tokenRoyalty[newItemId].royaltyReceiver[0] = _tokenMinter[0];
            _tokenRoyalty[newItemId].royaltyPercentage[0] = _royaltyPercentage[0];
        }
        //count of total royalty receiver/percentage for a token    
        _tokenRoyalty[newItemId].noOfRoyaltyReceiver = _tokenMinter.length;                   

        _safeMint(_msgSender(), newItemId);
        // _setTokenURI(newItemId, tokenURI(newItemId));
        _setTokenURI(newItemId, _tokenURI);
        
        console.log("minted token ", newItemId);
        console.log("tokenURI(_singleTokenIds.current()) ",tokenURI(newItemId));

        _singleTokenIds.increment();

        emit SingleTokenMinted(_msgSender(), newItemId, newSingleItem);
    ...
    */
    ...
    internal virtual override (ERC721Upgradeable, ERC721URIStorageUpgradeable) {
        ...
        require(_msgSender() == _tokenItems[_tokenId].tokenOwner, "Treehauz : Must be token owner");
    ...
    
{
    ...
    */
    ...
    modifier onlyAccountOwner() {
        require(accountRoyalty[_msgSender()].accountId != 0, "Marketplace: caller is not royalty account owner.");
        _;
    }
    ...
    /// @dev tokenId => listing info related to offers on a direct listing.
    mapping(uint256 => Listing) private listingTokenId;
    ...
    */
    ...
    /// @dev Events
    event NewListing(address indexed assetContract, 
    ...
    returns(uint256 listingId) 
    {
        ...
        // Get values to populate Listing object.
        ...
        Listing memory newListing = Listing ({
            ...
            reservePricePerToken: _params.reservePricePerToken, //starting price
            buyoutPricePerToken: _params.buyoutPricePerToken, //auto buy price
            tokenType: tokenTypeOfListing,
            listingType: _params.listingType
        });

        listings[listingId] = newListing;
        listingTokenId[_params.tokenId] = newListing;

        bool success;
        if (tokenTypeOfListing == TokenType.ERC1155) {
            success = _treehauzNFT.approveMarket(_params.tokenId, _params.assetContract);
        } else if (tokenTypeOfListing == TokenType.ERC1155) {
            success = _treehauzGrpNFT.approveMarket(_params.tokenId, _params.assetContract);
        }
        ...
        if (newListing.listingType == ListingType.Auction) {
            ...
            transferListingTokens(_msgSender(), address(this), tokenAmountToList, newListing);
        }
    ...
    function updateListing (
        ...
        if (isAuction) {
            require(block.timestamp < targetListing.startTime, "Marketplace: cannot update auction already started.");
            require(_buyoutPricePerToken >= _reservePricePerToken, "Marketplace: reserve price exceeds buyout price.");
        }
    ...
    function purchase(
        ...
        uint256 _quantity = getSafeQuantity(targetListing.tokenType, _quantityToBuy);
        require(targetListing.quantity >= _quantity, "Marketplace: not valid quantity");
        // Check whether price more than 0.01 ether
        require(msg.value >= 10000000000000000, "Marketplace: invalid price");
        // Check whether the settled total price and currency to use are correct.
        require(
            _currency == targetListing.currency && _totalPrice == (targetListing.buyoutPricePerToken * _quantity),
    ...
    /// @dev Refund offer to offeror.
    function cancelOffer(uint256 _listingId)
        external payable
        nonReentrant
    {
        Listing memory targetListing = listings[_listingId];
        Offer memory targetOffer = offers[_listingId][_msgSender()];
        require(_msgSender() != address(0)
                || _msgSender() != IERC721Upgradeable(targetListing.assetContract).ownerOf(targetListing.tokenId)
                || _msgSender() != _treehauzGrpNFT.ownerOf(targetListing.tokenId), 
                "Marketplace: address not valid");
        // must be offerror
        require(_msgSender() == targetOffer.offeror, "Marketplace: address not valid");

        // update offer before fund transfer - avoid re-entrant
        targetOffer.offeror = address(0);
        offers[_listingId][_msgSender()] = targetOffer;

        // refund offeror
        address payable _offerror = payable(_msgSender());
        TreehauzEscrow _escrowContract = TreehauzEscrow(_escrow);
        _escrowContract.escrowTransfer(_offerror, targetOffer.pricePerToken * targetOffer.quantityWanted);

        emit OfferCancelled(
            targetOffer.listingId,
            _msgSender(),
            targetOffer.listingType,
            targetOffer.quantityWanted,
            targetOffer.pricePerToken * targetOffer.quantityWanted,
    ...
    function offer(
        ...
        require(!paused, "Marketplace: activity is paused");
        Listing memory targetListing = listings[_listingId];
        uint256 _quantity = getSafeQuantity(targetListing.tokenType, _quantityWanted);
        // Check whether listing quantity is enough
        require(targetListing.quantity >= _quantity, "Marketplace: not valid quantity");
        // Check whether caller is owner which cannot do own offering
        require(_msgSender() != address(0) || _msgSender() != targetListing.tokenOwner, "Marketplace: address not valid");
        require(
        ...
        if (targetListing.listingType == ListingType.Auction) {
            ...
            newOffer.quantityWanted = getSafeQuantity(targetListing.tokenType, _quantity);
    ...
    function closeAuction(uint256 _listingId, address _closeFor) external nonReentrant {
        Listing memory targetListing = listings[_listingId];
    ...
    function executeSale(
        ...
        validateDirectListingSale(_targetListing, _quantity, _amount);
        ...
        transferListingTokens(_targetListing.tokenOwner, _buyer, _quantity, _targetListing);
    ...
    function _closeAuctionForAuctionCreator(Listing memory _targetListing, Offer memory _winningBid) internal {
        ...
        uint256 tokenQuantity = _targetListing.quantity;
    ...
    function transferListingTokens(
        ...
        if (_listing.tokenType == TokenType.ERC1155) {
            ERC1155Upgradeable(_listing.assetContract).safeTransferFrom(_from, _to, _listing.tokenId, _quantity, "");
        } else if (_listing.tokenType == TokenType.ERC721) {
            ERC721Upgradeable(_listing.assetContract).safeTransferFrom(_from, _to, _listing.tokenId, "");
            if (_listing.assetContract == address(_treehauzNFT)) {
                _treehauzNFT.updateTokenInfo(_listing.tokenId, _to);
            }
        }
    }
    ...
    function transferTokensUtil(
        ...
        if (getTokenType(_assetContract) == TokenType.ERC1155) {
            ERC1155Upgradeable(_assetContract).safeTransferFrom(_from, _to, _tokenId, _quantity, "");
        } else if (getTokenType(_assetContract) == TokenType.ERC721) {
            ERC721Upgradeable(_assetContract).safeTransferFrom(_from, _to, _tokenId, "");
            if (_assetContract == address(_treehauzNFT)) {
                _treehauzNFT.updateTokenInfo(_tokenId, _to);
            }
        }

        emit UtilTokenTransferred(_from, _to, _assetContract, _tokenId, _quantity);
    }
    ...
    */
    function royaltyPayment(
        ...
        if (_listing.assetContract == address(_treehauzNFT)) {
            ...
            _secondary = _treehauzNFT.getSecondaryInfo(_listing.tokenId);
        // for ERC-1155
            ...
            require(receiver[0] != address(0) && totalPercentage >= 0, "Marketplace: failed to get royalty info");
            _secondary = _treehauzGrpNFT.getSecondaryInfo(_listing.tokenId);
        }
    ...
    */
    function setRoyaltyAccount(
        ...
        if (accountRoyalty[_receipient].owner == address(0)) {
            AccountInfo memory newAccount = AccountInfo ({
                                    accountId : _accountIds.current(),
                                    owner : _receipient,
                                    unclaimedRoyalty : _amount,
                                    claimedRoyalty : 0
                                    });
            // Register new account royalty
            accountRoyalty[_receipient] = newAccount;
            _accountIds.increment();
    ...
    function claimRoyalty() external nonReentrant onlyAccountOwner {
        ...
        uint256 _amount = accountRoyalty[_msgSender()].unclaimedRoyalty;
        accountRoyalty[_msgSender()].unclaimedRoyalty = 0;
        accountRoyalty[_msgSender()].claimedRoyalty += _amount;

        _escrowContract.escrowTransfer(payable(_msgSender()), _amount);
    }
    ...
    /// @dev Returns the total listing in marketplace.
    function getTotalListing() public view returns (uint256) {
        return totalListings;
    }

    /// @dev Returns the next listing Id to use.
    function getListingByTokenId(uint256 _tokenId) public view returns (Listing memory targetListing) {
        targetListing = listingTokenId[_tokenId];
    ...
    */
    ...
    /// @dev Restrict listing by LISTER_ROLE.
    // function setRestrictedListerRoleOnly(bool restricted) external onlyOwner {
  ...
  ReentrancyGuardUpgradeable
{
    ...
    */
    function escrowTransfer(address payable _to, uint256 _amount) external nonReentrant {
        require(address(this).balance > _amount, "Escrow: not enough contract fund");
        require(_marketplace == _msgSender(),
