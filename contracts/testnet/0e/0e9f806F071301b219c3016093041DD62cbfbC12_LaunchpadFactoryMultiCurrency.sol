...
import "./LaunchpadPresaleMultiCurrency.sol";

contract LaunchpadFactoryMultiCurrency is ReentrancyGuard, Ownable, Pausable {
    ...
    event CreateLaunch(
        ...
        LaunchpadPresaleMultiCurrency.Parameters par, 
        LaunchpadPresaleMultiCurrency.WhitelistParameters wpar, 
        LaunchpadPresaleMultiCurrency.VestingParameters vpar, 
        uint256 tokens, 
        uint256 raisedFee, 
    ...
    constructor(address _router, address _feeAddress, address _signer, uint256 _createFee) {
        ...
        tokenImplementation = address(new LaunchpadPresaleMultiCurrency());
    ...
    function requiredTokens(
        ...
        tokens = tokens + ((hardcap * liquidityPercent* listingRate) / (10000 * (10 ** raiseInDecimals)));
    }
    ...
    function createLaunch(
        address token, 
        address raiseIn, 
        LaunchpadPresaleMultiCurrency.Parameters memory par, 
        LaunchpadPresaleMultiCurrency.WhitelistParameters memory wpar, 
        LaunchpadPresaleMultiCurrency.VestingParameters memory vpar, 
        uint256 raisedFee, 
        uint256 influencerFee
    ) public payable nonReentrant whenNotPaused {
        require(msg.value >= createFee, "Fees are wrong");
        if(!par.isPrivateSale) {
            require(par.liquidityPercent >= liquidityPercentLimit, "Liquidity Percent Low");
        }
        
        address clone = Clones.clone(tokenImplementation);
        LaunchpadPresaleMultiCurrency(payable(clone)).initialize(
            owner(),
            signer,
            feeAddress,
            token,
        ...
        LaunchpadPresaleMultiCurrency(payable(clone)).transferOwnership(msg.sender);
        ...
        uint256 tokens = requiredTokens(raiseIn, par.presaleRate, par.hardcap, par.liquidityPercent, par.listingRate);
        IERC20(token).safeTransferFrom(msg.sender, clone, tokens);

        emit CreateLaunch(token, raiseIn, par, wpar, vpar, tokens, raisedFee, influencerFee);
    }
...
contract LaunchpadPresaleMultiCurrency is Initializable, ReentrancyGuard, Ownable, Pausable {
    using SafeERC20 for IERC20;
    ...
    function initialize(
        ...
        address _raiseIn,
        address _router, 
        Parameters memory _par, 
        WhitelistParameters memory _wpar, 
        VestingParameters memory _vpar, 
        uint256 _raisedFee, 
        uint256 _influencerFee, 
        uint256 _finalizeForOwner
    ) public initializer {
        require(_par.softcap >= (_par.hardcap / 2) && (_par.softcap <= _par.hardcap), "Invalid Softcap");
    ...
    function getEthSignedMessageHash() public view returns (bytes32) {
        ...
        return
            keccak256(
                abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(address(this), msg.sender, id)))
            );
    }
    ...
    function buy(uint256 amount) external payable nonReentrant whenNotPaused {
        ...
        if (wpar.whitelist == true && block.timestamp < (par.startTime + wpar.duration)) {
            ...
            require(wbought[msg.sender] + amount >= wpar.minBuy && wbought[msg.sender] + amount <= wpar.maxBuy, 
                "Whitelist: Can't Buy");
            ...
            require(bought[msg.sender] + amount >= par.minBuy && bought[msg.sender] + amount <= par.maxBuy, 
                "Can't Buy");
     ...
     * Returning remaining BNB/Tokens to owner
    ...
    function finalize() external {
        ...
        if (totalBought >= par.softcap) {
            ...
            // Adding Liquidity
            if (par.isPrivateSale == false) {
                amountToLiquidity = (remainingAmount * par.liquidityPercent) / 10000;
                tokensToLiquidity = (par.listingRate * amountToLiquidity) / (10 ** decimals);

                if (address(raiseIn) != address(0)) {
                    addLiquidity(tokensToLiquidity, amountToLiquidity);
                } else {
                    addLiquidityBNB(tokensToLiquidity, amountToLiquidity);
                }
            }
            
            // Paying Fees
            uint256 feesToPay = raisedFeeAmount + influencerFee;
            if(feesToPay > 0 && address(raiseIn) != address(0)) {
                raiseIn.safeTransfer(feeAddress, feesToPay);
            } else if (feesToPay > 0) {
            ...
            if(payToOwner > 0 && address(raiseIn) != address(0)) {
                raiseIn.safeTransfer(owner(), payToOwner);
            } else if (payToOwner > 0) {
    ...
    function withdrawRaiseToken(uint256 amount, address to) external {
        require(msg.sender == platformOwner, "Invalid Account");
        require(raiseIn.balanceOf(address(this)) >= amount, "insufficient token balance");
     ...
     */
    function addLiquidityBNB(uint256 tokensToLiquidity, uint256 amountToLiquidity) private {
    ...
    function addLiquidity(uint256 tokensToLiquidity, uint256 amountToLiquidity) private {
        token.safeApprove(address(router), tokensToLiquidity);
        raiseIn.safeApprove(address(router), amountToLiquidity);
