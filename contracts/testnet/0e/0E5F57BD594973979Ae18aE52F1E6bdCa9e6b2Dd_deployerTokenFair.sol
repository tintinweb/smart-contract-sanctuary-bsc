...
contract deployerTokenFair is ReentrancyGuard {
    ...
    function createTokenFairPreSaleTeam(
        ...
        IERC20(_primaryToken).transferFrom(msg.sender,admin,adminFee-insuranceFee);
        IERC20(_primaryToken).transferFrom(msg.sender,insuranceWallet,insuranceFee);
        
        require(address(_token) != address(0), 'Launchpad: ZERO_ADDRESS');
        require(isPreSaleExist[_token] == false, 'Launchpad: PRESALE_EXISTS'); // single check is sufficient
        ...
        uint256 tokenAmount = getTotalNumberOfTokensFair(values[4].mul(10 ** (IERC20(_token).decimals())), values[3]);
        if(team){
        tokenAmount = tokenAmount.add(values[6].mul(10 ** (IERC20(_token).decimals())));
        }
        IERC20(_token).transferFrom(msg.sender, preSaleContract,tokenAmount);
        
        getPreSale[_token] = preSaleContract;
        
        isPreSaleExist[_token] = true; // setting preSale for this token to aviod duplication
