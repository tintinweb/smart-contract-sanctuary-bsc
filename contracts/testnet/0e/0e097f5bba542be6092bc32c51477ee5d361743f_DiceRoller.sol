...
contract DiceRoller is VRFConsumerBaseV2, ReentrancyGuard {
  ...
  uint256 public minChainlinkBalance = 0.00001 ether;
  ...
  constructor() VRFConsumerBaseV2(vrfCoordinator) {
    ...
    //VRFv2SubscriptionManagerAddress = 0x1B926e9510daC8041A9868462094bc949BB0fd0A; //Mainnet
    VRFv2SubscriptionManagerAddress = 0xf5b556502E8cd726f16cd6E5AD68253aBC02EF8a; //Testnet

    minerAddress = 0xC9baa0Bcfa920dd8F6feBd9BFA2fda74B34e69fB; //Mining contract address
    devAddress = 0xC9baa0Bcfa920dd8F6feBd9BFA2fda74B34e69fB; //Developer wallet address
    //busdAddress = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; //BUSD Token contract address - Mainnet
    busdAddress = 0xf4675722dbC841EEF6D88C8890d9A50FcCB153b8; //BUSD Token contract address
  ...
  function getMaxBetAmount() public view returns(uint256){
    return (busdToken.balanceOf(address(this)) * maxBetPercentage)/10000;
  }
  ...
  function processDistribution(address _playerAddress, uint256 _amount, bool _isWon) internal {
    bool success;
    uint256 totalFeesAmount = _amount * getTotalFees()/10000;

    if(_isWon){
      playerTotalWinnings[_playerAddress] += _amount;
      totalWinnings += _amount;
      _amount = (_amount * 2) - totalFeesAmount;
      
      unclaimedWinning[_playerAddress] += _amount;
      
    }else{
      playerTotalLost[_playerAddress] += _amount;
    }

    uint256 minerFeeAmount = _amount * minerFees/10000;
    uint256 devFeeAmount = _amount * devFees/10000;
