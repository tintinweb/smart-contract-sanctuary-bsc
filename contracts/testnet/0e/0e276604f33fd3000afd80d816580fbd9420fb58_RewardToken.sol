...
contract RewardToken is INEW, Context, Ownable {
    using SafeMath for uint256;
    ...
    uint256 private _tTotal = 1 * 10**17; // 100,000,000 (one hundred million)
    ...
    constructor(address _router, address _rewardAdmin) {
        ...
        _defaultFees = Fees(
            ...
            400, // marketing
            200, // liquidity
            0xd52A67DCe5515C14CE2d1b16dFa75887D12523B9, // marketing 
            0x0E714CF5E1b5CE80F18Bc96b232E06916c3343E0, // development
            0xBc744B31Cf49d3E065bC7571bc7bfF40fd92dBBD // liquidity
        );

        // Set base token in the pair as WBNB, which acts as the tax token
        INEWSwapPair(newSwapPair).setBaseToken(WBNB);
        INEWSwapPair(newSwapPair).updateTotalFee(900); // sum of the defaultFees
    ...
    function _getValues(uint256 tAmount) private view returns (FeeValues memory) {
        ...
        values.transferAmount = tAmount.sub(values.reflection).sub(values.marketing).sub(values.liquidity);
        return values;
    }
