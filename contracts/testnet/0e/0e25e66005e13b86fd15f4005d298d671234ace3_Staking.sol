...
contract Staking is TokenStakeOwnable, ReentrancyGuard {
    ...
    function stake(
        ...
        IERC20(stakeTokenAddr).safeTransferFrom(
            _msgSender(),
            address(this),
            amount
        );
        _stake(_msgSender(), stakeTokenAddr, amount, option);
    }
        ...
        whenStakingNotPaused(stakeToken)
    {
        ...
        uint256 rewards = calculateTotalRewards(stakeToken, amount, option);
        ...
        require(
            rewards <= totalAllocatedRewards - totalReservedRewards,
            "Not enough rewards allocated for staking."
        );
    ...
    function allClaimableRewards(
        ...
        uint256 allRewards;
        for (uint16 i = 0; i < usersStake[account].length; ++i) {
           allRewards += claimableRewards(account, i);
        }
        return allRewards;
    }
    ...
    function claimableRewards(
        ...
        StakeInfo memory s = usersStake[account][stakeIndex];
        if (uint64(block.timestamp) > s.end) {
            return s.rewards - s.claimedTokens;
        } 
        else if (s.claimedTokens >= s.rewards) {
            return 0;
        } 
        else {
            return s.rewards * (uint64(block.timestamp) - s.lastClaimed) / (s.end - s.start);
        }
    }

    function claimAllRewards() external nonReentrant{
        require(usersStake[_msgSender()].length > 0, "No Staked Tokens.");
        for (uint16 i = 0; i < usersStake[_msgSender()].length; ++i) {
            if (claimableRewards(_msgSender(), i) > 0) {
                _claimRewards(_msgSender(), i);
            }
        }        
    }

    function claimRewards(uint16 stakeIndex) external nonReentrant{
        require(usersStake[_msgSender()].length > stakeIndex, "!index");
        require(
            claimableRewards(_msgSender(), stakeIndex) > 0,
            "No claimable rewards"
        );
        _claimRewards(_msgSender(), stakeIndex);
    }

    function _claimRewards(address account, uint16 stakeIndex) internal {
        StakeInfo memory s = usersStake[account][stakeIndex];
        uint256 claimableAmount = claimableRewards(account, stakeIndex);
        require(s.rewards > 0, "claim: !rewards");
        require(
            claimableAmount <= s.rewards - s.claimedTokens,
            "Invalid claimableRewards"
        );
        require(
            claimableAmount <= IERC20(stakeTokenAddr).balanceOf(address(this)) - totalStakedTokens,
            "No staking rewards to claim."
        );

        usersStake[account][stakeIndex].lastClaimed = uint64(block.timestamp);
        usersStake[account][stakeIndex].claimedTokens += claimableAmount;
        totalClaimedTokens += claimableAmount;

        IERC20(s.rewardToken).safeTransfer(
            account,
            claimableAmount
        );
    }

    function withdraw(uint16 stakeIndex)
        external
        nonReentrant
        whenStakingNotPaused(stakeTokenAddr)
    {
        require(usersStake[_msgSender()].length > stakeIndex, "!index");
        StakeInfo memory s = usersStake[_msgSender()][stakeIndex];
        
        // can only withdraw when stake period ends
        require(block.timestamp > s.end, "!end");

        _withdraw(_msgSender(), stakeIndex);
    }
    ...
    ) internal 
    {
        ...
        usersStake[account][stakeIndex] = usersStake[account][
            usersStake[account].length - 1
        ];

        usersStake[account].pop();
        _claimRewards(account, stakeIndex);


        totalStakedTokens -= s.amount;
        totalReservedRewards -= (s.rewards - s.claimedTokens);

        IERC20(stakeTokenAddr).safeTransfer(account, s.amount);
    ...
    function calculateTotalRewards(
        ...
        return amount * stakeOptions[stakeToken][optionIndex].apy / 10000;
    }
    ...
    function availableStakingRewards() public view returns (uint256) {
        return totalAllocatedRewards - totalReservedRewards;
    }
    ...
    /// @param apy - rewards apy in percents (100 = 1%)
    function addStakeOptions(
        ...
        require(token == stakeTokenAddr, "!Not DECODE");
        stakeOptions[token].push(
            StakeOption(period, apy, rewardToken)
        );
        emit OptionAdd(token, uint16(stakeOptions[token].length) - 1);
    }

    function updateStakeOptions(
        uint16 index,
        uint16 period,
        uint16 apy,
        address rewardToken
    ) external onlyOwner {
        stakeOptions[stakeTokenAddr][index] = StakeOption(period, apy, rewardToken);
    }
    ...
    {
        require(usersStake[account].length > stakeIndex, "!index");
        _withdraw(account, stakeIndex);
    }

    function setTotalAllocatedRewards(
        uint256 amount
    ) external onlyOwner gtZero(amount) {
    ...
    function rescueToken(
        ...
        if(token == stakeTokenAddr) {
            require(amount <= IERC20(stakeTokenAddr).balanceOf(address(this)) - totalStakedTokens, "Cannot transfer staked tokens.");
        }
