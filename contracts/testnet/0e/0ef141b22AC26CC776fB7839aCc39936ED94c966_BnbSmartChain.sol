...
contract BnbSmartChain is ContractData {
  ...
  // --------------------- PUBLIC METHODS ---------------------------
    ...
    function withdrawlAndUpgrade(uint _amount) external {
        address sender = msg.sender;
        (,uint freeToWithdrawl, uint freePassive,,) = availableForWithdrawal(sender);
        require((accountsInfo[sender].lastWithdraw + 10 minutes) < block.timestamp , "Time limit");
    ...
    function registerAccount(address ref) external payable {
        ...
        maxBalance += msg.value;
        emit updateNetwork(networkSize, maxBalance, address(this).balance);
        emit updateUser(ref, accountsInfo[ref].directCount, accountsInfo[ref].unlockedLevel);
        networkDeposits += 1;
        ...
        if (_type == 0 || _type == 2) {
            ...
            if (accountsInfo[sender].depositTotal >= minAmountToLvlUp && accountsInfo[sender].unlockedSponsor == false) {
                ...
                if (currentUnlockedLevel < _unilevelPercents.length) {
                    ...
                    emit updateUser(referral, accountsInfo[referral].directCount, accountsInfo[referral].unlockedLevel);
                }
