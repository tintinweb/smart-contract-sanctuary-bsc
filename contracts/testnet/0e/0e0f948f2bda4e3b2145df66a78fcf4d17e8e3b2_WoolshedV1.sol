...
contract WoolshedV1 is OwnableUpgradeable, ContractGuard {
    ...
    function _setUpline(address _addr, address _upline) internal {
        ...
        if (
            users[_addr].upline == address(0) &&                   // 1) Fresh meat only...
            _upline != _addr &&                                    // 2) You cannot be your own upline
            _addr != owner() &&                                    // 3) Dev wallet cannot set an upline
            (users[_upline].deposit_time > 0 || _upline == bertha) // 4) Upline must have a deposit, or be the bertha wallet
        ) {
