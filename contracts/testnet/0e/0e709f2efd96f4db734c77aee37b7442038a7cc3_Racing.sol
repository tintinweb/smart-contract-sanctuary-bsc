...
contract Racing {
    ...
    uint256 public horseVal = 0.05 ether;
    uint256 public speedUpval = 0.01 ether;
        ...
        payable
    {
        ...
        require(msg.value >= horseVal);
        bids[racIter][msg.sender] += horseVal;   

        userChoice[msg.sender][racIter] = horseNum;

        if(horseNum == 1)
           horseOne[racIter] += horseVal;    
        if(horseNum == 2)
           horseTwo[racIter] += horseVal;  
        if(horseNum == 3)
           horseThree[racIter] += horseVal;
        if(horseNum == 4)
            horseFour[racIter] += horseVal;        
    }
