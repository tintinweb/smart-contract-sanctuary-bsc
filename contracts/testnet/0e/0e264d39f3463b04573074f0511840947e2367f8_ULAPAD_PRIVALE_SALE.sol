...
contract ULAPAD_PRIVALE_SALE is Context, Ownable {
    ...
    uint256 private _totalSaleToken = 2000000 * 10**_tokenDecimals;

    // --- 1 BNB = 5000 ULAPAD
    uint256 private _tokenSaleRatePerEther = 5000;

    // --- 0.1 BNB
    uint256 public _minContributeAmount = (1 * 10**_tokenDecimals) / 10;

    // --- 0.5 BNB
    uint256 public _maxContributeAmount = (5 * 10**_tokenDecimals) / 10;
    ...
    function getTotalSaleToken() public view returns (uint256) {
        return _totalSaleToken;
    }
    ...
    {
        return
            _whitelistedAddresses[__address].add(
    ...
    function getTotalInvestedAddresses() external view returns (uint256) {
        return _investedAddressesLength;
    }

    function getInvestedAddressByIndex(uint256 __index)
        external
        view
        returns (address)
    {
        return _investedAddresses[__index];
    }
