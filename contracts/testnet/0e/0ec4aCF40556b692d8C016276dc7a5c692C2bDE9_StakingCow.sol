 ...
 */
contract StakingCow is Ownable {
  ...
  uint256 public dailyInterestRate = 110;
  ...
  modifier onlyStakeholder(uint256 _id) {
    require(_idToAddress[_id] == msg.sender, "StakingCow: Caller is not the stakeholder");
    _;
  }
  ...
  function setDailyInterestRate(uint256 _dailyInterestRate) external onlyOwner {
    require(!enabled, "StakingCow: Cannot set daily interest rate while enabled");
    require(_dailyInterestRate <= 10000, "StakingCow: Daily interest rate must be less than 100%");
    dailyInterestRate = _dailyInterestRate;
  }

  function stake(uint256 _amount) external {
    require(enabled, "StakingCow: Staking is disabled");
    require(_amount >= 1e18, "StakingCow: Amount must be >= 1 token");
