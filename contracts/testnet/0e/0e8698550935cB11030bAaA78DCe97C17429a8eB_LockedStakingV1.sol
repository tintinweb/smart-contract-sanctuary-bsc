...
contract LockedStakingV1 is IERC20Staking {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    uint256 public periodicTime = 365 days;
    uint256 planLimit = 3;
    uint256 public refRate = 2;
    ...
    constructor(address _stakingToken) IERC20Staking(_stakingToken) {
        ...
        plans[0].earlyPenalty = 70;

        plans[1].apr = 18;
        plans[1].stakeDuration = 60 days;
        plans[1].earlyPenalty = 70;

        plans[2].apr = 32;
        plans[2].stakeDuration = 180 days;
        plans[2].earlyPenalty = 70;      
    }

    function referralStake(uint256 _stakingId, uint256 _amount, address _referrer) public nonReentrant {   
        uint256 refReward = _amount.mul(refRate).div(100);
        if ((_referrer != msg.sender) && (_referrer != address(0))) {
            if(userRef[msg.sender].status == false){
                ...
                userRef[_referrer].myReferral = msg.sender;
                userRef[msg.sender].status = true;                              
            }                      
        }
        address lv0 = userRef[msg.sender].referredBy;
        address lv1 = userRef[lv0].referredBy;
        address lv2 = userRef[lv1].referredBy; 

        userRef[lv0].totalRef += 1;
        userRef[lv1].totalRef += 1;
        userRef[lv2].totalRef += 1;

        userRef[lv0].totalEarning = userRef[lv0].totalEarning.add(refReward);
        userRef[lv0].claimableEarning = userRef[lv0].claimableEarning.add(refReward);

        userRef[lv1].totalEarning = userRef[lv1].totalEarning.add(refReward);
        userRef[lv1].claimableEarning = userRef[lv1].claimableEarning.add(refReward);

        userRef[lv2].totalEarning = userRef[lv2].totalEarning.add(refReward);
        userRef[lv2].claimableEarning = userRef[lv2].claimableEarning.add(refReward);          
    ...
    function setReferralRewardRate(uint256 _percent) external onlyOwner {
        refRate = _percent;
    }
