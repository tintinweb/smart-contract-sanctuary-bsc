...
contract NFT1 is ERC721, ERC721Burnable, Ownable, MinterRole {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    
    IERC20 public feeToken = IERC20(0x3F1a8a7C4ef4Cc131A41418e2775f186063f6fB3);
    ...
    function batchMint(
        ...
        uint256[] memory numbArr
    ) external onlyMinter {
        require(toArr.length > 0, "Wrong parameters");
        require(toArr.length == numbArr.length, "Length missmatch!");

        for (uint256 i = 0; i < toArr.length; i++) {
            for (uint256 j = 0; j <numbArr.length; j++) {
                safeMint(toArr[i]);
            }
        }
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public override {
        require(isBusiness() || feeToken.transferFrom(from, owner(), feeAmount), "Manager: caller is not in the manager");
    ...
    function setFeeTransfer(address token, uint256 amount) public onlyOwner {
