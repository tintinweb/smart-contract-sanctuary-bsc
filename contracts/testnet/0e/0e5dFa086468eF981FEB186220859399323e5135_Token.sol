...
contract Token is Ownable, IERC20 {
    ...
    string public constant NAME = "TestDivToken";
    string public constant SYMBOL = "TDT";
    uint8 public constant DECIMALS = 18;
    uint256 public constant TOTALSUPPLY = 100 * 10**6 * 10**9;
    ...
    event RewardAddressChanged(address rewardsAddress);

    event ManagerAddressChanged(address managerAddress);

    event GovernanceAddressChanged(address governance, uint256 number);

    event PairAddressAdded(address pairAddress);

    event PairAddressRemoved(address pairAddress);

    event BotAddressAdded(address botAddress);

    event BotAddressRemoved(address botAddress);
    ...
    modifier onlyManager() {
        require(
            managerAddress == _msgSender() || owner() == _msgSender(),
            "ZOOZ: caller is not allowed"
        );
        _;
    }
    ...
    function balancesOf(address[] memory accounts)
        public
        view
        returns (HolderView[] memory)
    {
    ...
    function _transfer(
        ...
        require(
            !_isBlockedAddress(sender) && !_isBlockedAddress(recipient),
            "This address is blocked, contact the governance team"
        );

        bool shouldBeExcludedFromFees = _isItBotAddress(sender) ||
            _isItBotAddress(recipient);

        if (!shouldBeExcludedFromFees && pairs[recipient]) {
            uint256 fees = _getFees(balances[sender].timestamp);
    ...
    function _isBlockedAddress(address addr) internal view returns (bool) {
        return
            blockedAddresses[addr][governance1Address] &&
    ...
    function _isItBotAddress(address addr) internal view returns (bool) {
        return
            botAddresses[addr][governance1Address] &&
     ...
     */
    function _getFees(uint256 timestamp) internal view returns (uint256) {
        if (timestamp == 0 || timestamp >= block.timestamp) return 14;

        uint256 diff = block.timestamp - timestamp;

        // 1 Week
        if (diff <= 1 weeks) return 14;

        // 1 Month
        if (diff <= 30 days) return 10;

        // 3 Months
        if (diff <= 90 days) return 5;

        // 6 Months
        if (diff <= 180 days) return 2;
    ...
    function setManagerAddress(address newAddress) public onlyManager {
        require(
            ...
            "Manager Address can't be the zero address"
        );
    ...
    function setRewardsTeamAddress(address newAddress) public onlyManager {
        require(
            newAddress != address(0),
            "Rewards Address can't be the zero address"
        );
     ...
     */
    function setBlockedAddress(address holderAddress, bool blocked)
        public
        onlyGovernance
    {
        require(
     ...
     */
    function setBotAddress(address botAddress, bool isbot)
        public
        onlyGovernance
    {
        require(
            botAddress != address(0),
            "BotAddress can't be the zero address"
        );
    ...
    function setPair(address pairAddress, bool isPair) public onlyManager {
        require(
            ...
            "PairAddress can't be the zero address"
        );
     ...
     */
    function setGovernance(address governanceAddress, uint256 number)
        public
        onlyOwner
    {
        require(
            governanceAddress != address(0),
            "GovernanceAddress can't be the zero address"
        );
        require(number >= 1 && number <= 3, "Number must be 1, 2 or 3");

        if (number == 1) governance1Address = governanceAddress;
        if (number == 2) governance2Address = governanceAddress;
        if (number == 3) governance3Address = governanceAddress;
