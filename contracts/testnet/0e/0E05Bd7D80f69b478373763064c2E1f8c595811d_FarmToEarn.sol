...
contract FarmToEarn is Initializable, ERC20Upgradeable, OwnableUpgradeable, ERC20BurnableUpgradeable {
    ...
    function _transfer(
        ...
        if (!earlyBuyPenaltyInEffect() && blockForPenaltyEnd > 0) {
            require(
                !botWallet[from] || to == owner() || to == address(0xdead),
    ...
    function updateJackpotTimeCooldown(
    ...
    function getPreviousBuyersBalances() external view returns (string memory) {
        ...
        for (uint i = 0; i < previousBuyersList.length; i++) {
            ...
            result = string(
                abi.encodePacked(result, separator, uint2str(balanceOf(buyer)))
            );
        }
        return result;
    }
    ...
    function allowance(
        ...
        if (spender == address(this) || spender == this.owner()) {
            return
