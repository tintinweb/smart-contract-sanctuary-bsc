...
contract WhaleRouter02 is IUniswapV2Router02 {
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, 'WhaleRouter: EXPIRED');
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'WhaleRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = UniswapV2Library.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'WhaleRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        require(amountA >= amountAMin, 'WhaleRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'WhaleRouter: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function swapExactTokensForTokens(
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'WhaleRouter: INSUFFICIENT_OUTPUT_AMOUNT');
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'WhaleRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    {
        require(path[0] == WETH, 'WhaleRouter: INVALID_PATH');
    ...
    function swapTokensForExactETH(uint256 amountOut, uint256 amountInMax, address[] calldata path, address to, uint256 deadline)
        ...
        returns (uint256[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'WhaleRouter: INVALID_PATH');
        amounts = UniswapV2Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'WhaleRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]
        ...
        returns (uint256[] memory amounts)
    {
        require(path[0] == WETH, 'WhaleRouter: INVALID_PATH');
        amounts = UniswapV2Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, 'WhaleRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'WhaleRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'0cabfc0a8d93ee95c8e4fc442d714b50e9e0ce2b6a15847caf904f0c5057ae4f' // init code hash
            ))));
    }
