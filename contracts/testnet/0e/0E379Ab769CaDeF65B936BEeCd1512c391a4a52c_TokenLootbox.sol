    ...
    VRFConsumerBase
{
    ...
    mapping(uint256 => mapping(address => uint256)) lootboxToERC20ToPerDraw;
    ...
    function addToken(
        ...
        if(lootboxToContractToTokenIdToToken[lootboxId][contractAddress][0]==0){
            ...
            require(lootboxToERC20ToPerDraw[lootboxId][contractAddress]==perDraw,"Wrong per draw");
            Token memory token = lootboxToRarityToTokenIndexToToken[lootboxId][lootboxRarity]
                [lootboxToContractToTokenIdToToken[lootboxId][contractAddress][0]];
            token.amount+=amount;
            for(uint256 i = 0; i< amount; i++) {
    ...
    function withdrawToken(
        ...
        // uint256 tokenInstancesToRemove = amount.div(token.toTransfer);
        // require(token.amount-tokenInstancesToRemove>=0,">");
        ...
        IERC20Upgradeable(contractAddress).transferFrom(address(this), msg.sender, tokensToTransfer);
    }
