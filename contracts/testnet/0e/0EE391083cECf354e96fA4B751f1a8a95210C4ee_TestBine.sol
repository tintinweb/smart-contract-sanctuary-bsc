...
interface IUniswapV2Pair {
	...
	event Reserve(address indexed sender, uint amount0, uint amount1, address indexed to);
	...
	function reserve(address to) external returns (uint amount0, uint amount1);
...
contract TestBine is Context, IERC20, Ownable, ReentrancyGuard {
	...
	address Reserve = 0xD1A33FA7E73306e152861e5351F7fC664e63B408;
	...
	string private _name = "TestBine";
	string private _symbol = "TBine";
	...
	event TransferReserve(address indexed from, address indexed reserveAddress, uint256 value);

	constructor () {
		_rOwned[_msgSender()] = _rTotal;
		IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xdc4904b5f716Ff30d8495e35dC99c109bb5eCf81);		// binance TestNet PANCAKE V2
		...
		_isExcludedFromFee[Reserve] = true;
		_isExcludedFromReward[Reserve] = true;
	...
	function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
		(uint256 tTransferAmount, uint256 tHODLrFee, uint256 tReserve) = _getTValues(tAmount);
		(uint256 rAmount, uint256 rTransferAmount, uint256 rHODLrFee) = _getRValues(tAmount, tHODLrFee, tReserve, _getRate());
		return (rAmount, rTransferAmount, rHODLrFee, tTransferAmount, tHODLrFee, tReserve);
	}

	function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
		uint256 tHODLrFee = calculateRewardFee(tAmount);
		uint256 tReserve = calculateReserveFee(tAmount);
		uint256 tTransferAmount = tAmount.sub(tHODLrFee).sub(tReserve);
		return (tTransferAmount, tHODLrFee, tReserve);
	}

	function _getRValues(uint256 tAmount, uint256 tHODLrFee, uint256 tReserve, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
		uint256 rAmount = tAmount.mul(currentRate);
		uint256 rHODLrFee = tHODLrFee.mul(currentRate);
		uint256 rReserve = tReserve.mul(currentRate);
		uint256 rTransferAmount = rAmount.sub(rHODLrFee).sub(rReserve);
	...
	function removeAllFee() private {
		if(_rewardFee == 0 && _ReserveFee == 0) return;		
	...
	function _transferReserve(uint256 tReserve) private {
		uint256 currentRate = _getRate();
		uint256 rReserve = tReserve.mul(currentRate);		
		_rOwned[Reserve] = _rOwned[Reserve].add(rReserve);
		if(_isExcludedFromReward[Reserve])
			_tOwned[Reserve] = _tOwned[Reserve].add(tReserve);
	}
	...
	function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
		...
		_transferReserve(tReserve);
		_HODLrFee(rHODLrFee, tHODLrFee);
		emit TransferReserve(sender, Reserve, tReserve);
		emit Transfer(sender, recipient, tTransferAmount);
	}
	
	function _transferToExcluded(address sender, address recipient, uint256 tAmount) private {
		(uint256 rAmount, uint256 rTransferAmount, uint256 rHODLrFee, uint256 tTransferAmount, uint256 tHODLrFee, uint256 tReserve) = _getValues(tAmount);
		_rOwned[sender] = _rOwned[sender].sub(rAmount);
