...
library FilterLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint160(uint(keccak256(abi.encodePacked(
            ...
            hex"e5dab0b8d806cbcddaea3c71391dbc8cc35de6c8ea1eec3d1a8a85b6e7265b09"
        )))));
    }
...
contract FilterRouter is IFilterRouter {
    ...
    function removeLiquidity(address tokenA, address tokenB, uint liquidity, uint amountAMin, uint amountBMin, address to, uint deadline) 
        ...
        require(!filterManager.isLiquidityLocked(msg.sender, pair), "FilterRouter: LIQUIDITY_LOCKED");
    }
    ...
    function removeLiquidityETH(address token, uint liquidity, uint amountTokenMin, uint amountETHMin, address to, uint deadline) public virtual override ensure(deadline) returns (uint amountToken, uint amountETH) {
        ...
        require(!filterManager.isLiquidityLocked(msg.sender, IFilterFactory(filterManager.factoryAddress()).getPair(wethAddress, token)), "FilterRouter: LIQUIDITY_LOCKED");
    }
