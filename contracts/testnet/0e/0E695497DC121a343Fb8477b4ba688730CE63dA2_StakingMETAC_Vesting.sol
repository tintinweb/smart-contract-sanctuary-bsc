...
contract StakingMETAC_Vesting is Ownable {
    ...
    function addOldUsers(address[] memory accounts, uint256[] memory amounts, uint256[] memory lockedDaysList) public onlyOwner() {
        require(accounts.length == amounts.length, 'different length of list');
        require(accounts.length == lockedDaysList.length, 'different length of list');
    ...
    // _amount is the METAC_Vesting amount with decimals
    function addOldUser(address _account, uint256 _amount, uint256 _lockedDays) public onlyOwner() {
        UserInfo storage user = userInfo[0][_account];
        user.amount = user.amount.add(_amount);
        user.userRewardInfo.push(UserRewardInfo({
                amount: _amount,
