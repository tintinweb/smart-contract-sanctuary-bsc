...
contract ShillingOcean is Initializable, OwnableUpgradeable {
    ...
    address public fathomTokenAddress;
    ...
    event FathomTokenAddressUpdated(
        address oldAddress,
        address newAddress,
        uint256 time,
        address user
    ...
    function initialize(
        ...
        fathomTokenAddress = _fathomTokenAddress;
    }
    ...
    function setDistributorAddress(address _distributorAddress)
        ...
        onlyOwner
    {
        distributor = IDividendDistributor(_distributorAddress);
    }
     ...
     * @param _fathomTokenAddress: Fathom Address to be set in this method.
     */
    function setFathomTokenAddress(address _fathomTokenAddress)
        external
        onlyOwner
    {
        wave();
        emit FathomTokenAddressUpdated(
            fathomTokenAddress,
            _fathomTokenAddress,
            block.timestamp,
            msg.sender
    ...
    */
    function wave() public {
        uint256 principal = IERC20(fathomTokenAddress).balanceOf(address(this));

        if ((lastRewardTransactionTime + slotTime) < block.timestamp) {
            ...
            if (reward < IERC20(fathomTokenAddress).balanceOf(address(this))) {
                IERC20(fathomTokenAddress).transfer(
                    address(distributor),
                    reward
                );

                try distributor.deposit(reward) {} catch {}
                ...
                emit WaveFailed(
                    block.timestamp,
                    "Ocean: Insufficient Fathom Token in pool"
                );
            }
        }
    }
