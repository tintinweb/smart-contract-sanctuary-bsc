...
contract FarmBooster is Ownable {
    ...
    uint256 public cA;
    /// @notice cA precision
    uint256 public constant CA_PRECISION = 1e8;
    ...
    function _boostCalculate(
        ...
        uint256 aB = (((lp.balanceOf(MASTER_CHEF) * userLockedAmount * (lockEndTime - lockStartTime)) *
            BOOST_RATIO_PRECISION) / cB) / (totalLockedAmount * _duration);
        // should '*' BOOST_PRECISION
        return ((lpBalance < (dB + aB) ? lpBalance : (dB + aB)) / dB) * BOOST_PRECISION;
    }

    function getMulti(address _user, address _proxy,uint256 _pid) view public returns(uint256){
        return _boostCalculate(_user, _proxy, _pid, _avgLockedDuration());
    }
    ...
    function _avgLockedDuration() internal view returns (uint256) {
        ...
        uint256 flexibleCakeShares = ((totalStakedCakeAmount - totalLockedAmount) * 1e18) / pricePerFullShare;
        if (flexibleCakeShares == 0) return 0;

        uint256 fixedTermCakeOriginalShares = (totalLockedAmount * 1e18) / pricePerFullShare;
        if (fixedTermCakeOriginalShares == 0) return 0;

        uint256 boostRatio = ((ICakePool(CAKE_POOL).totalShares() - flexibleCakeShares) * BOOST_RATIO_PRECISION) /
            fixedTermCakeOriginalShares;
        // right?
        if (boostRatio <= BOOST_RATIO_PRECISION) return 0;
        return ((boostRatio - BOOST_RATIO_PRECISION) * 365) / 20;
    }
}
