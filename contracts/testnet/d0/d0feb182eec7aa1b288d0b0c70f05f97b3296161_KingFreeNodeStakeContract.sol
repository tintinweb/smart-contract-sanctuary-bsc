...
contract KingFreeNodeStakeContract is Ownable {
    ...
    function add(IERC20 _lpToken,uint256 _allocPoint) public onlyOwner nonDuplicated(_lpToken) {
    ...
    function setPoolAllocPoint(uint256 _pid,uint256 _allocPoint) public onlyOwner {
        poolInfos[_pid].allocPoint = _allocPoint;
    }

    function setPoolLPToken(uint256 _pid,IERC20 _newlpToken) public onlyOwner {
        poolInfos[_pid].lpToken = _newlpToken;
    }
    ...
    function pendingReward(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 blockInterval = getBlockInterval(pool.lastRewardBlock, block.number);
        ...
        return user.totalDepositAmount.mul(accTokenPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function getMyInviter(address _addr) public view returns(address) {
        return InviteContract.getMyInviter(_addr);
    }

    function getMyInviteeAddress(address _addr) public view returns(address[] memory) {
        return InviteContract.getMyInviteeAddress(_addr);
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        require(InviteContract.getMyInviter(msg.sender) != address(0),"deposite: The wallet is not yet bound");
    ...
    function claim(uint256 _pid) public {
        ...
        uint256 pending = user.totalDepositAmount.mul(pool.accTokenPerShare).div(1e18).sub(user.rewardDebt).add(myReferralReward(_pid,msg.sender));
        ...
        user.rewardDebt = user.totalDepositAmount.mul(pool.accTokenPerShare).div(1e18);
