...
import './interfaces/ITallyswapFactory.sol';
import './TallyswapPair.sol';
...
import "./interfaces/ITallyswapPair.sol";
import "./TallyswapERC20.sol";
import "./libraries/Math.sol";
import "./libraries/UQ112x112.sol";
import "./interfaces/IERC20.sol";
import "./interfaces/ITallyswapFactory.sol";
import "./interfaces/ITallyswapCallee.sol";
...
contract TallyswapPair is TallyswapERC20 {
    ...
    uint32 public marketingFee = 100; // uses 0.1%
        ...
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    if (swapFee > 0) {
                        require(marketingFee > 0, "Marketing Fee is zero");
                        require(operationsFee > 0, "Operating Fee is zero");
                        require(artisticFee > 0, "Artistic Fee is zero");
                        require(technicalFee > 0, "Technical Fee is zero");

                        uint256 marketingFeeAmount = uint256(swapFee).mul(marketingFee).div(400);
                        uint256 operationsFeeAmount = uint256(swapFee).mul(operationsFee).div(400);
                        uint256 artisticFeeAmount = uint256(swapFee).mul(artisticFee).div(400);
                        uint256 technicalFeeAmount = uint256(swapFee).mul(technicalFee).div(400);
...
import './interfaces/ITallyswapERC20.sol';
