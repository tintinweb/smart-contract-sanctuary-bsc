...
contract swagTestnet is ERC721Enumerable, Ownable {
	using Strings for uint256;
	...
	// max size will be 10
	function mintTo(address to_, uint256 amount_) external onlyLaunchpad {
        require(to_ != address(0), "can't mint to empty address");
        require(amount_ > 0, "size must greater than zero");
        require(LAUNCH_SUPPLY + amount_ <= LAUNCH_MAX_SUPPLY, "max supply reached");
    ...
    function ownerReserveMintTo(address to_,uint256 amount_) external onlyOwner {
         require(to_ != address(0), "can't mint to empty address");
        require(amount_ > 0, "size must greater than zero");
        require(LAUNCH_SUPPLY + amount_ <= LAUNCH_MAX_SUPPLY, "max supply reached");
