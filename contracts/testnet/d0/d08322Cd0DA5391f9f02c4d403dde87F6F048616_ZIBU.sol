...
interface LMSTracker {
    ...
    function removeFromLMS(address account) external;
    function process(uint256 gas) external;
}

contract ZIBU is ERC20, Ownable {
    ...
    uint256 public lmsShare       = 20;

    address public devWallet = 0xdE00000000000000000000000000000000000000;
    ...
    address public buybackToken = 0xFa60D973F7642B748046464e165A65B7323b0DEE;
    uint256 public bnbValueForBuyBurn; //! test
    uint256 public accumulatedBuybackBNB;

    mapping (address => bool) private _isExcludedFromFees;
    mapping (address => bool) private _isExcludedFromLMS;
    mapping (address => bool) public automatedMarketMakerPairs;

    address public BUSD = 0xAFE4e2756AabA3aE010DF8fee6Be638760d73Ba1;
    // address public lmsToken = 0xED36eC71739bc10dcD3E094bA0eE3e9f1253928b    0xC9ae4cECB4E0FEAC6b0Ba72f5AF8b362eCD39BAa

    DividendTracker public dividendTracker;
    uint256 public gasForProcessing = 300000;

    LMSTracker public lmsTracker;
    uint256 public gasForLMS = 300000; //! test
    ...
    event DepositLMS(uint256 amount);
    ...
    constructor() ERC20("ZIBU", "$ZIBU") 
    {
        _mint(owner(), 1_000_000 * (10 ** 18));
        swapTokensAtAmount = totalSupply() / 5000;

    	dividendTracker = new DividendTracker(1e6 / 50000, BUSD);
        ...
        _isExcludedFromLMS[owner()] = true;
        _isExcludedFromLMS[address(this)] = true;
        _isExcludedFromLMS[address(uniswapV2Router)] = true;
        _isExcludedFromLMS[address(_uniswapV2Pair)] = true;
        _isExcludedFromLMS[DEAD] = true;
    ...
    function updateFees(uint256 _buyFee, uint256 _sellFee) external onlyOwner {
        require(_buyFee + _sellFee <= 30, "Total buy fee and sell fee cannot be more than 30%");
        buyFee = _buyFee;
        sellFee = _sellFee;
        ...
        external onlyOwner 
    {
        ...
        emit FeeSharesUpdated(devShare, liquidityShare, rewardShare, buybackShare, lmsShare);
    }
    ...
    function changeDevWallet(address _devWallet) external onlyOwner {
        require(_devWallet != devWallet, "Developer wallet is already that address");
        require(!isContract(_devWallet), "Developer wallet cannot be a contract");
        require(_devWallet != address(0), "Developer wallet cannot be the zero address");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 bnbShare = devShare + rewardShare + buybackShare + lmsShare;
            ...
            if(bnbShare > 0) {
                ...
                if (lmsShare > 0) {
                    uint256 lmsBNB = newBalance * lmsShare / bnbShare;
                    swapAndSendLMS(lmsBNB);
                }
        ...
        if (!_isExcludedFromLMS[from]) {
            try lmsTracker.updateTracker(from, balanceOf(from), to == uniswapV2Pair) {} catch {}
        }
        if (!_isExcludedFromLMS[to]) {
            try lmsTracker.updateTracker(to, balanceOf(to), false) {} catch {}
        }

        if (!swapping) {
            uint256 gas = gasForLMS;

            try lmsTracker.process(gas) {} catch {}
        }
    }
    ...
    function swapAndSendLMS(uint256 amount) private{
        (bool success, ) = payable(address(lmsTracker)).call{value: amount}("");

        if (success) {
            lmsTracker.deposit(amount);
        }
    }
    ...
    function swapAndSendDev(uint256 amount) private{
        ...
        IERC20(BUSD).transfer(address(devWallet), _amount);
    ...
    //=======LMS Tracker=======//
    function isExcludedFromLMS(address account) public view returns(bool) {
        return _isExcludedFromLMS[account];
    }

    function excludeFromLMS(address account, bool value) public onlyOwner {
        _isExcludedFromLMS[account] = value;
        lmsTracker.removeFromLMS(account);
    }

    function setLmsTracker(address newLmsTracker) external onlyOwner {
        lmsTracker = LMSTracker(newLmsTracker);

        _isExcludedFromLMS[newLmsTracker] = true;
        _isExcludedFromFees[newLmsTracker] = true;
    }

    function updateGasForLMS(uint256 newGas) external onlyOwner {
        require(newGas != gasForLMS, "Cannot update gasForLMS to same value");
        gasForLMS = newGas;
    }
