...
contract StakingPlatform is Ownable {
    ...
    function updateReward() private{
        ...
        uint256 newReward = stakerStakedAmount.mul(block.timestamp.sub(_staker[msg.sender].lastUpdatedTime)).mul(_rewardRate).div(1 days).div(1e5);
    ...
    function rewardToHarvest(address _address) public view returns (uint256){
        ...
        uint256 newReward = stakerStakedAmount.mul(block.timestamp.sub(_staker[_address].lastUpdatedTime)).mul(_rewardRate).div(1 days).div(1e5);
    ...
    function withdraw(uint256 amount) external{
        ...
        amountTobeWithdrawn = amountTobeWithdrawn.sub(amountTobeWithdrawn.mul(fee).div(1e5));
    ...
    function harvest() public{
        ...
        uint256 rewardToClaim = curReward.sub(curReward.mul(_rewardFeeRate).div(1e5));
