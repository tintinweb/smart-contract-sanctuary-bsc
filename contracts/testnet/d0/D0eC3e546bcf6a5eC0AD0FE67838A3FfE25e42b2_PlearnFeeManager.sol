...
contract PlearnFeeManager is Ownable {
    ...
    function processAllFee(bool ignoreError) external onlyOwner {
        for (uint256 i = 0; i < getPairDestinationLength(); ++i) {
            ...
            IPlearnFeeHandler.SwapInfo[] memory swapList;
            ...
            if (i + 1 == getPairDestinationLength()) {
                plearnFeeHandler.processFee(liquidityList, swapList, ignoreError);
            }
        }

        for (uint256 i = 0; i < getPairDestinationLength(); ++i) {
            IPlearnFeeHandler.RemoveLiquidityInfo[] memory liquidityList;
