...
contract VideoGames is ERC20, ERC20Burnable, Ownable {
    ...
    function getHashOfCompetitor(int gameId, string memory name) private pure returns (bytes32) {
        return keccak256(abi.encodePacked(gameId, name));
    }
    
    function getHashOfBet(int gameId, address wallet) private pure returns (bytes32) {
