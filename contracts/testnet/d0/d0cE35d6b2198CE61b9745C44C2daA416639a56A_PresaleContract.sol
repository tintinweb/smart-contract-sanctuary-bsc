...
contract PresaleContract is Ownable{
    ...
    event purchasedbox(uint256 time, address indexed buyer,string typebox,uint256 noncebox);
    event ispaused(address indexed buyer,bool validator);


    function BuyBoxs(uint256 Box) public payable IsPaused{
        require(Box > 0 && Box <= 4, "selection error");
        require(NFTc.whiteList(address(this)) == true, "It's not authorized");
        if(Box == 1){buy_CommonBox();}
        else if(Box == 2){buy_RareBox();}
        else if(Box == 3){buy_EpicBox();}
        else{buy_LegendaryBox();}
    }

    function buy_CommonBox() internal returns (bool){
        ...
        emit purchasedbox(block.timestamp, _buyer, "CommonBox", Sold_CommonBox);

        Sold_CommonBox++;
        return true;
    }

    function buy_RareBox() internal returns (bool){
        address _buyer = _msgSender();
        uint256 Amount = msg.value;
        ...
        emit purchasedbox(block.timestamp, _buyer, "RareBox", Sold_RareBox);
        Sold_RareBox++;
        return true;
    }

    function buy_EpicBox() internal returns (bool){
        address _buyer = _msgSender();
        uint256 Amount = msg.value;
        ...
        emit purchasedbox(block.timestamp, _buyer, "EpicBox", Sold_EpicBox);
        Sold_EpicBox++;
        return true;
    }

    function buy_LegendaryBox() internal returns (bool){
        address _buyer = _msgSender();
        uint256 Amount = msg.value;
        ...
        emit purchasedbox(block.timestamp, _buyer, "LegendaryBox", Sold_LegendaryBox);
        Sold_LegendaryBox++;
        return true;
    }
    ...
    function setPaused() public onlyOwner{ 
        if(paused()){  _paused = false; }else{ _paused = true; }
        emit ispaused(msg.sender, _paused); 
    }
