...
// File @openzeppelin/contracts/utils/cryptography/MerkleProof.so[email protected]
...
abstract contract Presalable is ContextUpgradeable {
    ...
    modifier whenVcsaled() {
        require(vcsaled(), "Presalable: not vcsaled");
        _;
    }
    ...
    function _vcsale() internal virtual whenNotPresaled {
        _vcsaled = true;
        emit Vcsaled(_msgSender());
    }

    function _unvcsale() internal virtual whenPresaled {
        _vcsaled = false;
...
contract MetafameSaleMTv8_0715 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, Presalable{
    ...
    function setBUSEPerBox(uint256 saleType, uint256 newPrice) onlyOwner whenPaused public {
        if(saleType == 0){
