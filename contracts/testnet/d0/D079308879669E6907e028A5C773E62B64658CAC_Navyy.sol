...
contract Navyy is Initializable {
    ...
    // struct Fees {
    //     uint256 _capitalFee;
    //     uint256 _allowancefundFee;
    //     uint256 _bankFee;
    //     uint256 _marianatrenchFee;
    //     uint256 _antidumpFee;
    //     uint256 _jackpotFee;
    ...
    function initialize(address owner_,address _dexRouter,address _WBNB) public initializer {
        ...
        MAX = ~uint256(0);
        _INIT_TOTAL_SUPPLY = 100 * 10**3 * 10**_decimals; // 100,000 $NAVYY
        _MAX_TOTAL_SUPPLY = _INIT_TOTAL_SUPPLY * 10**4; // 1,000,000,000 $NAVYY (x10000)
        ...
        _allowancefund = address(0x69233b4e8D3C28Ba1f255BD5b03CE777F708d19d);
    ...
    function _swapBack(uint r1) internal returns (uint, uint) {
        ...
        try distributor.deposit{value: ethAmount.mul(busdrewardFee).div(totalFee)}() {} catch {} //busd reward
        SENDBNB(address(jackpot), ethAmount.mul(_jackpotFee).div(totalFee));//Jackpot      
    ...
    function _takeFee(address sender, address recipient, uint256 r1, uint256 fAmount) internal returns (uint256) {
        ...
        if (recipient == _uniswapV2Pair) { // sell, remove liq, etc
            ...
            totalFee = totalFee.add(antidumpFee).add(100);//add 100 for allowance fee and capital Fee and subtract 1% for bank fee
