...
contract CryptRise is ERC20Detailed, Ownable {
    ...
    uint256 public maxSellTransactionAmount = 5000000000 * 10**18;//0.1% of total supply
    uint256 public maxBuyTransactionAmount = 5000000000 * 10**18;//10% of total supply

    uint256 public rebaseFrequency = 300;
    uint256 public nextRebase = block.timestamp + 300; //31536000;
    ...
    uint256 public constant MIN_SELL_AMOUNT_RATE = 5000000000 * 10**18;
    uint256 public constant MIN_BUY_AMOUNT_RATE = 5000000000 * 10**18;
    ...
    address public stableCoin = 0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984; //0x462C98Cae5AffEED576c98A55dAA922604e2D875;//0xa3Fa99A148fA48D14Ed51d610c367C61876997F1;
    ...
    uint256 public totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeRFV).add(rewardFee);
    // uint256 public totalSellFee =
    // totalBuyFee.add(sellFeeTreasuryAdded).add(sellFeeRFVAdded).add(
    //     sellLaunchFeeAdded
    // );
    uint256 public totalSellFee =
    sellLiquidityFee.add(sellFeeTreasuryAdded).add(sellFeeRFVAdded).add(
        sellRewardFee
    );
    ...
    uint256 private gonSwapThreshold = (TOTAL_GONS * 10) / 5000;

    mapping(address => uint256) private _gonBalances;
    mapping(address => mapping(address => uint256)) private _allowedFragments;

    constructor() ERC20Detailed('CryptRise Finance', 'CRF', uint8(DECIMALS)) {
        router = IDEXRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); //0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    ...
    function setSaleFee() private {
        ...
        treasuryFee = sellFeeTreasuryAdded;
        buyFeeRFV = sellFeeRFVAdded;
        rewardFee = sellRewardFee;
        
        totalSellFee = liquidityFee + treasuryFee + burnFee + buyFeeRFV + rewardFee;
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToReward = contractTokenBalance
        .sub(amountToLiquify)
        ...
        if (amountToTreasury > 0) {
            _swapTokensForStableCoin(amountToTreasury, treasuryReceiver);
        }

         if (amountToReward > 0) {
            _swapTokensForMATIC(amountToReward, rewardReceiver);
        }
    ...
    function setFeeReceivers(
        ...
        require(_rewardReceiver != address(0), '_rewardReceiver not set');
    ...
    function setFees(
        ...
        uint256 maxTotalBuyFee = _liquidityFee.add(_treasuryFee).add(_rewardFee).add(
            _riskFreeValue
        );

        uint256 maxTotalSellFee = maxTotalBuyFee.add(_sellFeeTreasuryAdded).add(_sellRewardFee).add(
            _sellFeeRFVAdded
        );
        ...
        setSellFee(
            totalBuyFee.add(sellFeeTreasuryAdded).add(sellFeeRFVAdded).add(sellRewardFee).add(
