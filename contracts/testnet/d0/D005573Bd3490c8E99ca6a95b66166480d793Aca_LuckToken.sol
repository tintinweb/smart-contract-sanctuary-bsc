...
contract LuckToken is AllTheFees {
    ...
    address buybackwallet = 0xAc6bD92774d16462423e88318001903C79DfF4d7; //Wallet of burn and buyback fee
    ...
    constructor() Auth(msg.sender){
        ...
        exemptFromMaxWallet[owner] = true;

        exemptFromMaxTransaction[owner] = true;

        exemptFromFee[owner] = true;
        exemptFromFee[address(this)] = true;

        exemptFromDividends[owner] = true;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        else
        {
            ...
            bool overMinimumTokenBalance = contractTokenBalance >= _minimumTokensToSwap;

            if(overMinimumTokenBalance && !inSwapAndLiquify && !isMarketPair[sender] && swapAndLiquifyEnabled){swapAndLiquify(sender);}
    ...
    function swapAndLiquify(address sender) internal lockTheSwap{
        ...
        emit Transfer(sender, address(DEAD), swapTokens.burnAmount);
        ...
        swapBNB.marketingBNB = (swapTokens.startingBalance.mul(BuyFeeMarketing.add(SellFeeMarketing)).div(BuyFeeTotal.add(SellFeeTotal)).mul(swapBNB.newlyGainedBNB.div(swapTokens.tokensToSwapToEth)));
        swapBNB.bbBNB = (swapTokens.startingBalance.mul(BuyFeeBB.add(SellFeeBB)).div(BuyFeeTotal.add(SellFeeTotal)).mul(swapBNB.newlyGainedBNB.div(swapTokens.tokensToSwapToEth)));
        swapBNB.houseBNB = (swapTokens.startingBalance.mul(BuyFeeHouse.add(SellFeeHouse)).div(BuyFeeTotal.add(SellFeeTotal)).mul(swapBNB.newlyGainedBNB.div(swapTokens.tokensToSwapToEth)));

        swapBNB.luckyBNB = (swapTokens.startingBalance.mul(BuyFeeLuckyShares.add(SellFeeLuckyShares)).div(BuyFeeTotal.add(SellFeeTotal)).mul(swapBNB.newlyGainedBNB.div(swapTokens.tokensToSwapToEth)));
        swapBNB.winnerBNB = (swapTokens.startingBalance.mul(BuyFeeWinnersReward.add(SellFeeWinnersReward)).div(BuyFeeTotal.add(SellFeeTotal)).mul(swapBNB.newlyGainedBNB.div(swapTokens.tokensToSwapToEth)));
        swapBNB.jackpotBNB = (swapTokens.startingBalance.mul(BuyFeeJackpotReward.add(SellFeeJackpotReward)).div(BuyFeeTotal.add(SellFeeTotal)).mul(swapBNB.newlyGainedBNB.div(swapTokens.tokensToSwapToEth)));


        (bool tmpSuccess,) = payable(marketingwallet).call{value: swapBNB.marketingBNB, gas: 30000}("");
		(bool tmpSuccess1,) = payable(buybackwallet).call{value: swapBNB.bbBNB, gas: 30000}("");
		(bool tmpSuccess2,) = payable(housewallet).call{value: swapBNB.houseBNB, gas: 30000}("");
        ...
        //Swapping to BUSD
        swapEthForBUSD(swapBNB.newlyGainedBNB.sub(swapBNB.marketingBNB.add(swapBNB.bbBNB).add(swapBNB.houseBNB)));
    ...
    function swapTokensForEth(uint256 tokenAmount) public authorized {
    ...
    function swapEthForBUSD(uint256 EthAmount) public authorized {
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) public authorized {
