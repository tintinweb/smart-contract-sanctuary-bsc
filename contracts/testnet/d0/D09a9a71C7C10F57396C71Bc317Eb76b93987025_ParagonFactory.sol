...
contract ParagonFactory is Pausable, AccessControl {
    ...
    IParagon public paragon;
    IParaArt public art;
    IProxy public proxy;
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    uint256 public uncraftBurnPercent = 10;
    uint256 public minParagon = 200;
    uint256 public maxParagon = 2000;
    ...
    event UncraftParagon(
        uint256 tokenId,
        uint256[16] burnRunes,
        uint256[16] burnExternal
    );
    ...
    function editParagon(uint256 _min, uint256 _max)
        public
    ...
    function createParagon(
        ...
        require(
            totalRunes >= minParagon && totalRunes <= maxParagon,
            "Must be 200 - 2000 runes have power"
        );
        uint256 tokenId = paragon.safeMint(
            msg.sender,
            originalArt,
            rawHashed,
            runesList
    ...
    function uncraft(
        uint256 tokenId,
        uint256[16] memory burnRunes,
        uint256[16] memory burnExternal
    ) public virtual {
        (address owner, bytes32 originalArt, , bytes32 runesList, ) = paragon
            .paragonInfo(tokenId);
        art.copyrightRental(owner, originalArt);
        uint256[16] memory amount = bytes32ToAmount16(runesList);
        uint256 totalPower = 0;
        uint256 totalBurn = 0;
        for (uint256 i = 0; i < 16; i++) {
            if (amount[i] > 0) {
                totalPower = totalPower.add(amount[i].mul(runePower[i]));
                if (amount[i] > burnRunes[i]) {
                    IERC20(proxy.getRuneAddress(i)).transfer(
                        msg.sender,
                        amount[i].sub(burnRunes[i])
                    );
                }
            }
            require(amount[i] >= burnRunes[i], "Must have runes for burn");
            totalBurn = totalBurn.add(
                (burnRunes[i].add(burnExternal[i])).mul(runePower[i])
            );
            if (burnExternal[i] > 0) {
                require(
                    amount[i] == burnRunes[i],
                    "Burn before select extenal"
                );
                IERC20(proxy.getRuneAddress(i)).transferFrom(
                    msg.sender,
                    address(this),
        ...
        require(
            totalBurn >= totalPower.mul(uncraftBurnPercent).div(100),
            "Must be bigger than uncraft fee"
        );
        paragon.burn(tokenId);
        emit UncraftParagon(tokenId, burnRunes, burnExternal);
    }
