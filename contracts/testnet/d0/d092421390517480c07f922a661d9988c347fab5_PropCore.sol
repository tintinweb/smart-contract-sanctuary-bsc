...
contract PropCore is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable {
    ...
    function initialize(address usdt_, address soulToken_, address prop1155_, address soulController_, address soulPair_) public initializer {
        __Divestor_init();
    ...
    function setContracts(address soulToken_, address prop1155_, address soulController_, address soulPair_) public onlyOwner {
    ...
    function buyProp() public returns(bool){
        ...
        externalAddr.soulToken.transferFrom(_msgSender(), address(this), soulCount);

        //发放道具
        externalAddr.prop1155.safeTransferFrom(address(this), _msgSender(), propId, 1, '');
    ...
    function propBatchBox(uint8 theme_, uint8 group_, uint amount_, address recommender_) public returns(bool){
        require(recommender_ != address(0), "Uninvited");
        uint count = externalAddr.prop1155.balanceOf(_msgSender(), propId);
