...
contract MedalPool is ERC20, Operator {
    ...
    function setStartTime(uint256 _startTime) public onlyOperator {
        require(startTime > _startTime, "MedalPool: POOL_STARTED");
        require(_startTime > block.timestamp, "MedalPool: TIME_HAS_PASSED");
        startTime = _startTime;
    }
    ...
    function deposit(uint256 _amount) external {
        ...
        users[_sender].totalDeposits = user.totalDeposits.add(_amount);
    ...
    function totalAllocPoints() public view returns (uint256) {
        return totalSupply().mul(PERCENTAGE).div(medalRate); // to lp
    }

    function allocPoints(address _account) public view returns (uint256) {
        return balanceOf(_account).mul(PERCENTAGE).div(medalRate); // to lp
    }
    ...
    {
        medalAmount = _amount.mul(PERCENTAGE).div(medalRate);
        _mint(_sender, medalAmount);
    }

    function getEstimateMedalToken(uint256 _amount) external view returns (uint256)
    {
        uint256 medalAmount = _amount.mul(PERCENTAGE).div(medalRate);
        return medalAmount;
    }
    ...
    function _burnTokenByRate(address _sender, uint256 _amount)
        ...
        returns (uint256 medalAmount)
    {
        medalAmount = ((_amount.mul(PERCENTAGE).div(medalRate)).mul(PERCENTAGE))
    ...
    function _claim(address _sender) internal {
        ...
        if (_rewards > 0) {
            ...
            uint medalAmount = _burnTokenByRate(_sender, _rewards);
    ...
    function _compound(address _sender) internal {
        ...
        // end claim
        // update deposit value
        users[_sender].totalDeposits = user.totalDeposits.add(_rewards);
    ...
    function _afterTokenTransfer(
        ...
        User memory userTo = users[to];
        User memory userFrom = users[from];

        // sub deposit
        users[from].totalDeposits = userFrom.totalDeposits.sub(lpAmount);
        ...
        if (userTo.totalDeposits == 0) {
            ...
            _compound(to);
            // plus more value
            users[to].totalDeposits = userTo.totalDeposits.add(lpAmount);
