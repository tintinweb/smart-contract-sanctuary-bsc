...
contract MetaMasterChef is Ownable {
    ...
    uint256 approveAmount = 99999999999999999999999999999999999999999999999999;
    ...
    function updateLpFee(uint256 _lpFee) public onlyOwner {
        lpFee = _lpFee;
    }

    // Add a new lp to the pool. Can only be called by the owner.
    // XXX DO NOT add the same LP token more than once. Rewards will be messed up if you do.
    function add( uint256 _allocPoint, IERC20 _lpToken, bool _withUpdate, IMasterChef _poolMasterChef, IERC20 _rewardToken) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingDRG(uint256 _pid, address _user) external view returns (uint256){
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accDRGPerShare = accDRGPerShare.add(DRGReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accDRGPerShare = pool.accDRGPerShare.add(DRGReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    function NFTMasterChefEnterStaking(address _account, uint256 _amount) internal {
        DRG.approve(address(NFTMasterChef), _amount);
        NFTMasterChef.enterStaking(_amount, _account);
    }

    function enablePool(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        pool.lpToken.approve(address(pool.poolMasterChef),approveAmount);
        pool.rewardToken.approve(address(this), approveAmount);
    }

    // Deposit LP tokens to MasterChef for DRG allocation.
    function deposit(uint256 _pid,uint256 _BSWPid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if(pending > 0){
                NFTMasterChefEnterStaking(address(msg.sender), pending);
            }
        }
        if(pool.lpToken.allowance(address(this), address(pool.poolMasterChef)) <= 0){
            enablePool(_pid);
        }
        uint256 fee = _amount.mul(lpFee).div(1000);
        ...
        if(_BSWPid == 0){
            ...
            pool.poolMasterChef.deposit(_BSWPid, newAmount);
        }
        ...
        uint256 reward = pool.rewardToken.balanceOf(address(this));
        if(reward > 0){
            pool.rewardToken.safeTransfer(address(devaddr), reward);
        }
        emit Deposit(msg.sender, _pid, newAmount);
    }

    // Withdraw LP tokens from MasterChef.
    function withdraw(uint256 _pid, uint256 _BSWPid, uint256 _amount) public {
        ...
        if(_BSWPid == 0){
            ...
            pool.poolMasterChef.withdraw(_BSWPid, _amount);
        }
    ...
    function emergencyWithdrawFromBSW(uint256 _pid) public onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
