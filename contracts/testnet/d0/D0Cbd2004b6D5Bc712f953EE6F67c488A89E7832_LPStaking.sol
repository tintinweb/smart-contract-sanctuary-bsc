    ...
    PausableUpgradeable
{
    ...
    bool public lpAllowEmergencyWithdraw;
    ...
    mapping(uint256 => mapping(address => LPPendingEarlyWithdrawal))
        public lpPendingEarlyWithdrawals;
    ...
    struct LPPendingEarlyWithdrawal {
        uint256 reward;
        uint256 amount;
    ...
    event LPPendingWithdraw(uint256 poolId, address account, uint256 amount);
    event LPEmergencyWithdraw(uint256 poolId, address account, uint256 amount);
    ...
    event LPAddWhitelist(uint256 poolId, address account);
    event LPRemoveWhitelist(uint256 poolId, address account);

    event LPSetDelayDuration(uint256 poolId, uint256 duration);

    event LPSetEarlyWithdrawTier(
        uint256 poolId,
     ...
     */
    function lpSetAllowEmergencyWithdraw(bool _shouldAllow) external onlyOwner {
        lpAllowEmergencyWithdraw = _shouldAllow;
    }
    ...
    function lpAssignStakedData(
        ...
        LPPendingEarlyWithdrawal
            storage pendingWithdrawData = lpPendingEarlyWithdrawals[_poolId][
                _to
            ];
        require(
        ...
        // LPPendingEarlyWithdrawal
        lpPendingEarlyWithdrawals[_poolId][_to] = lpPendingEarlyWithdrawals[
            _poolId
        ][_from];
        delete lpPendingEarlyWithdrawals[_poolId][_from];
    ...
    function lpSetEarlyWithdrawTier(
        uint256 _poolId,
        ...
        emit LPSetEarlyWithdrawTier(_poolId, _percentageKey, _percentageValue);
    }

    function lpSetListEarlyWithdrawTier(
        uint256 _poolId,
        ...
        require(
            ...
            "LPStakingPool: length not match"
        );
    ...
    function lpAddWhitelist(uint256 _poolId, address _account)
        external
        onlyOwner
        lpValidatePoolById(_poolId)
    {
        ...
        emit LPAddWhitelist(_poolId, _account);
    }

    function lpRemoveWhitelist(uint256 _poolId, address _account)
        external
        onlyOwner
        lpValidatePoolById(_poolId)
    {
        ...
        emit LPRemoveWhitelist(_poolId, _account);
    }

    function lpAddListWhitelist(uint256 _poolId, address[] calldata _accounts)
        external
        onlyOwner
        lpValidatePoolById(_poolId)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            emit LPAddWhitelist(_poolId, _accounts[i]);
        }
    }
    ...
    //                             GETTER FUNCTION
    // ========================================================================
    function getLPPendingEarlyWithdrawals(uint256 _poolId, address _user)
        external
        view
        returns (LPPendingEarlyWithdrawal memory)
    {
        return lpPendingEarlyWithdrawals[_poolId][_user];
    }
     ...
     * @param _amount amount to withdraw
     */
    function lpWithdraw(uint256 _poolId, uint256 _amount)
        external
        nonReentrant
        whenNotPaused
        notBlacklisted
        lpValidatePoolById(_poolId)
    {
        ...
        stakingData.balance -= _amount;
        IERC20(pool.acceptedToken).safeTransfer(account, _amount);

        // claim early withdrawal pending rewards
        _lpClaimPendingWithdraw(_poolId);
        emit LPWithdraw(_poolId, account, _amount);
    }
    ...
    function lpEarlyWithdraw(uint256 _poolId, uint16 _percentage)
        external
        nonReentrant
        whenNotPaused
        notBlacklisted
        lpValidatePoolById(_poolId)
    {
        ...
        require(
            ...
            "LPStakingPool: invalid percentage"
        );
        require(
            poolInfo.isWhitelisted[account],
            "LPStakingPool: not whitelisted"
        );

        LPPendingEarlyWithdrawal storage pending = lpPendingEarlyWithdrawals[
            _poolId
        ][account];
        ...
        // emit event
        emit LPEarlyWithdraw(_poolId, account, amount, reward, penaltyFee);

        // remove user from whitelist
        delete poolInfo.isWhitelisted[account];

        IERC20(IERC20(pool.acceptedToken)).safeTransfer(treasury, penaltyFee);
    }
     ...
     * @param _poolId id of the pool
     */
    function lpEmergencyWithdraw(uint256 _poolId)
        external
        nonReentrant
        whenNotPaused
        notBlacklisted
    ...
    {
        require(
            lpAllowEmergencyWithdraw,
            "LPStakingPool: emergency withdrawal is not allowed yet"
        );

        address account = msg.sender;
        LPStakingData storage stakingData = lpStakingData[_poolId][account];

        require(stakingData.balance > 0, "LPStakingPool: nothing to withdraw");
        ...
        emit LPEmergencyWithdraw(_poolId, account, amount);
    }
    ...
    function _lpClaimPendingWithdraw(uint256 _poolId) private {
        require(
            lpRewardDistributor != address(0),
            "LPStakingPool: invalid reward distributor"
        );
        ...
        require(
            (pending.applicableAt <= block.timestamp) ||
                (block.timestamp >=
                    lpStakingData[_poolId][account].joinTime +
                        lpPoolInfo[_poolId].lockDuration),
            "LPStakingPool: not released yet"
        );
        delete lpPendingEarlyWithdrawals[_poolId][account];

        IERC20(lpPoolInfo[_poolId].acceptedToken).safeTransfer(account, amount);
        IERC20(lpPoolInfo[_poolId].rewardToken).safeTransferFrom(
            lpRewardDistributor,
            account,
            reward
        );
        emit LPClaimPendingWithdraw(_poolId, account, amount);
    }
}