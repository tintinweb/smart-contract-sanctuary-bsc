...
contract PandoPotV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    function updateLuckupNumberForQC(uint256 _roundId, uint256 _megaNumber, uint256 _minorNumber1, uint256 _minorNumber2) external onlyOwner{
        ...
        RoundInfo storage _roundInfo = roundInfo[_roundId];
            ...
            _roundInfo.minorNumber1 = currentMinorNumber1;
            _roundInfo.minorNumber2 = currentMinorNumber2;
            (uint256 _megaUSD, uint256 _megaPSR) = _calcMegaPrize(_roundId , currentMegaNumber, _roundInfo.finishedAt + prizeExpireTime);
            (uint256 _minorUSD1, uint256 _minorPSR1) = _calcMinorPrize(_roundId, currentMinorNumber1, _roundInfo.finishedAt + prizeExpireTime);
            (uint256 _minorUSD2, uint256 _minorPSR2) = _calcMinorPrize(_roundId, currentMinorNumber2,  _roundInfo.finishedAt + prizeExpireTime);
        emit RoundCompleted(_roundId, _roundInfo.finishedAt + prizeExpireTime, currentMegaNumber, currentMinorNumber1, currentMinorNumber2, _megaUSD, _megaPSR, _minorUSD1, _minorPSR1, _minorUSD2, _minorPSR2);
    }
    ...
    function _liquidate(uint256 _type, uint256 _roundId, address _owner) internal {
        ...
        if (_type == 0 || _type == 1) {
            ...
            if (_megaPrize.totalWinning > 0 && _megaPrize.totalWinning > _megaPrize.nClaimed) {              
                ...
                _megaPrize.nClaimed = _megaPrize.totalWinning;
            }
            if (_minorPrize.totalWinning > 0 && _minorPrize.totalWinning > _minorPrize.nClaimed) {               
                _totalUSD += _minorPrize.USD * (_minorPrize.totalWinning - _minorPrize.nClaimed) / _minorPrize.totalWinning;
                _totalPSR += _minorPrize.PSR * (_minorPrize.totalWinning - _minorPrize.nClaimed) / _minorPrize.totalWinning;
                _minorPrize.nClaimed = _minorPrize.totalWinning;
            }
        } else {
    ...
    function distribute(address[] memory _leaderboards, uint256[] memory ratios) external onlyWhitelist whenNotPaused {
        ...
        for (uint256 i = 0; i < _leaderboards.length; i++) {
            ...
            uint256 _USDAmount = USDForPreviousPot * ratios[i] / PRECISION;
