...
contract SMR is BEP20Detailed, Ownable, WhitelistedRole {
    ...
    address public busdToken = 0xCF1AeCc287027f797b99650B1E020fFa0fb0e248;//0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
    ...
    uint256 public buyFeemarketing = 2;
    uint256 public sellFeeDevelopmentAdded = 2;
    uint256 public sellFeemarketingAdded = 2;
    uint256 public totalBuyFee =DevelopmentFee.add(buyFeemarketing);
    uint256 public totalSellFee = totalBuyFee.add(sellFeeDevelopmentAdded).add(sellFeemarketingAdded);
    ...
    constructor() BEP20Detailed("Shiba Move Run", "SMR", uint8(DECIMALS)) {
...
riskFreeValueReceiver=msg.sender;
        ...
        _isFeeExempt[DevelopmentReceiver] = true;
        _isFeeExempt[riskFreeValueReceiver] = true;
        _isFeeExempt[address(this)] = true;
        _isFeeExempt[msg.sender] = true;

        IBEP20(busdToken).approve(address(router), uint256(-1));
        IBEP20(busdToken).approve(address(pairBusd), uint256(-1));
        IBEP20(busdToken).approve(address(this), uint256(-1));
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private {
        ...
        if(isLiquidityInBnb){
            ...
            uint256 initialBalance = IBEP20(busdToken).balanceOf(address(this));

            _swapTokensForBusd(half, address(this));

            uint256 newBalance = IBEP20(busdToken).balanceOf(address(this)).sub(initialBalance);
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToRFV = contractTokenBalance.mul(buyFeemarketing.mul(2).add(sellFeemarketingAdded)).div(realTotalFee);
        uint256 amountToDevelopment = contractTokenBalance.sub(amountToLiquify).sub(amountToRFV);
        ...
        if(amountToDevelopment > 0){
            _swapTokensForBNB(amountToDevelopment, DevelopmentReceiver);
        }

        emit SwapBack(contractTokenBalance, amountToLiquify, amountToRFV, amountToDevelopment);
    }
    ...
    function setFeeReceivers(address _liquidityReceiver, address _DevelopmentReceiver, address _riskFreeValueReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        DevelopmentReceiver = _DevelopmentReceiver;
        riskFreeValueReceiver = _riskFreeValueReceiver;
    }

    function setFees( uint256 _riskFreeValue, uint256 _DevelopmentFee, uint256 _sellFeeDevelopmentAdded, uint256 _sellFeemarketingAdded, uint256 _feeDenominator) external onlyOwner {
        require(
            
            _riskFreeValue <= MAX_FEE_RATE &&
            _DevelopmentFee <= MAX_FEE_RATE &&
            _sellFeeDevelopmentAdded <= MAX_FEE_RATE &&
            _sellFeemarketingAdded <= MAX_FEE_RATE,
            "wrong"
        );

       
        buyFeemarketing = _riskFreeValue;
        DevelopmentFee = _DevelopmentFee;
        sellFeeDevelopmentAdded = _sellFeeDevelopmentAdded;
        sellFeemarketingAdded = _sellFeemarketingAdded;
        totalBuyFee = DevelopmentFee.add(buyFeemarketing);
        totalSellFee = totalBuyFee.add(sellFeeDevelopmentAdded).add(sellFeemarketingAdded);
    ...
    function rescueToken(address tokenAddress, uint256 tokens) external onlyOwner returns (bool success){
        return BEP20Detailed(tokenAddress).transfer(msg.sender, tokens);
    }
    ...
    event SwapBack(uint256 contractTokenBalance,uint256 amountToLiquify,uint256 amountToRFV,uint256 amountToDevelopment);
