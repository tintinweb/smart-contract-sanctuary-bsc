...
contract DBSNFTCast  is Ownable{
    ...
    function IsCastClosed() public view returns (bool){
        return castOpen == true;
    }

    function GetCast() public  returns (uint256, uint256){
        uint256 oldTime = CastShip[msg.sender];
        require(block.timestamp >= oldTime.add(600), "DBSNFTTechnology: GetCast time error");
        // Transfer the token
    ...
    function GetEndCastTime() public view returns (uint256){
        return CastShip[msg.sender].add(600);
    }

    function SetCastPrice(uint256 _price) public onlyOwner{
        require(_price > 0);
        price = _price;
    }
    ...
    function modifyTech(address _tech) public onlyOwner(){
        require(_tech != address(0), "DBSNFTTechnology: modifyTech for the zero address");
        techAddress = DBSNFTTechnology(_tech);
    }
}
...
contract DBSNFTTechnology  is Ownable{
    ...
    Ship[] public copperShip;
    Ship[] public silverShip;
    Ship[] public goldShip;

    uint256[] public copperSkill;
    uint256[] public silverSkill;
    uint256[] public goldSkill;

    uint256 seed;
    mapping(uint256 => uint256) public NftToQuality;
    ...
    function RecruitDest(uint256[] memory _ids) public returns (uint256, uint256){
        ...
        uint256 maintableId = nftAddress.getPropertyID(_ids[0]);
        uint256 quality = NftToQuality[maintableId];
        ...
        if(quality == uint256(NFTQuality.NFTCopper)){
            nftAddress.burnBatch(msg.sender, _ids);
            uint256 tokenId = nftAddress.mint(msg.sender, copperShip[IDToIndex[maintableId]].skill);

            emit newRecruitDest(msg.sender, _ids, tokenId, copperShip[IDToIndex[maintableId]].skill);
            return (tokenId, copperShip[IDToIndex[maintableId]].skill); 
        }else if(quality == uint256(NFTQuality.NFTSilver)){
            nftAddress.burnBatch(msg.sender, _ids);
            uint256 tokenId = nftAddress.mint(msg.sender, silverShip[IDToIndex[maintableId]].skill);

            emit newRecruitDest(msg.sender, _ids, tokenId, silverShip[IDToIndex[maintableId]].skill);
            return (tokenId, silverShip[IDToIndex[maintableId]].skill); 
        }
        return (uint256(0),uint256(0));
    }
    ...
    function Decompose(uint256 _id) public  returns (uint256[] memory, uint256[] memory){
        ...
        if(quality == uint256(NFTQuality.NFTSilver)){
            ...
            uint256[] memory cids;
            uint256 count;
            (cids, count) = GetSameShipCamp(goldShip[IDToIndex[tableId]].camp, uint256(NFTQuality.NFTSilver));
