...
contract Galaxy is ERC20,Ownable,ReentrancyGuard {
    ...
    function initializeIndex(Component[] memory _components) public onlyOwner {
        for(uint i = 0; i < _components.length; i++) {
            // require(address(_components[i].token) != address(0), "Token address zero");
            // require(indexedTokens[address(_components[i].token)] == false, "Token added twice");
            indexedTokens[address(_components[i].token)] = true;
            tokens[address(_components[i].token)] = _components[i];

            components[i].token = _components[i].token;
            components[i].baseAmount = _components[i].baseAmount;
