...
interface IPresaleBigHead {
    function mint(address account_, uint256 amount_) external;
}

contract BigHeadPresale is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    struct UserInfo {
        uint256 amount; // Amount BUSD deposited by user
        uint256 debt; // total BHD claimed thus pBHD debt
        bool claimed; // True if a user has claimed BHD
    }

    address devAddress = 0xe0bd1D3cd2176d3740BA0577FFD366CC6dC4CB9c;
    address public homeAddress = 0x02bb1edB91F18e2aeD31D2D6b2949003c6B0BE64; 

    // Tokens to raise (BUSD) and for offer (pBHD) which can be swapped for (BHD)
    IERC20 public BUSD; // for user deposits
    IERC20 public pBHD;
    IERC20 public BHD;

    uint256 public price = 20 * 1e18; // 20 BUSD per pBHD

    uint256 public cap = 1000 * 1e18; // 1000 BUSD cap per user

    uint256 public totalRaisedBUSD; // total BUSD raised by sale

    uint256 public totalDebt; // total pBHD and thus BHD owed to users
    ...
    bool public claimPresale; // true when pBHD is claimable

    bool public contractPaused; // circuit breaker

    mapping(address => UserInfo) public userInfo;
    mapping(address => uint256) public bhdClaimable; // amount of BHD claimable by address
    ...
    constructor(
        ...
        require( _pBHD != address(0) );
        pBHD = IERC20(_pBHD);
        require( _BHD != address(0) );
        BHD = IERC20(_BHD);
    ...
    /**
     *  @notice set BHD Token
     *  @param _BHD: address of BHD
     */
    function setBHD(address _BHD) external onlyOwner {
        require( _BHD != address(0) );
        BHD = IERC20(_BHD);
    }

    /**
     *  @notice set pBHD Token
     *  @param _pBHD: address of pBHD
     */
    function setPBHD(address _pBHD) external onlyOwner {
        require( _pBHD != address(0) );
        pBHD = IERC20(_pBHD);
    }

    /**
     *  @notice set price for pBHD
     *  @param _price: price for pBHD
     */
    ...
    // @notice lets users claim BHD
    // @dev send sufficient BHD before calling
    function claimUnlock() external onlyOwner {
        require(ended, "Sale has not ended");
        require(!claimable, "Claim has already been unlocked");
        require(BHD.balanceOf(address(this)) >= totalDebt, 'not enough BHD in contract');
        claimable = true;
        emit ClaimUnlocked(block.number);
    }


    // @notice lets users claim pBHD
    ...
    function deposit(uint256 _amount) external checkIfPaused {
        ...
        uint256 payout = _amount.mul(1e18).div(price); // pBHD to mint for _amount

        totalDebt = totalDebt.add(payout);

        uint256 feeAmount = _amount.mul(10).div(100).mul(1e18);
        BUSD.safeTransferFrom( msg.sender, devAddress, feeAmount );
        BUSD.safeTransferFrom( msg.sender, homeAddress, _amount.sub(feeAmount) );

        IPresaleBigHead( address(pBHD) ).mint( msg.sender, payout );

        emit Deposit(msg.sender, _amount);
    }
    
    /**
     *  @notice it deposits pBHD to withdraw BHD from the sale
     *  @param _amount: amount of pBHD to deposit to sale (9 decimals)
     */
    function withdraw(uint256 _amount) external checkIfPaused {
        require(claimable, 'BHD is not yet claimable');
        ...
        pBHD.safeTransferFrom( msg.sender, address(this), _amount );

        BHD.safeTransfer( msg.sender, _amount );

        emit Mint(address(pBHD), msg.sender, _amount);
        emit Withdraw(address(BHD), msg.sender, _amount);
    }

    // @notice set the home address
    function setHomeAddress(address _homeAddress) external onlyOwner {
        require( _homeAddress != address(0) );
    ...
    // @notice it claims pBHD back from the sale
    function claimPresaleBHD() external checkIfPaused {
        require(claimPresale, 'pBHD is not yet claimable');
        ...
        pBHD.safeTransfer( msg.sender, payout );

        emit Withdraw(address(pBHD),msg.sender, payout);
    }
