...
contract Ra8Bits is Context, Ownable, ERC1155Holder {
    ...
    mapping(uint256 => TokenListForBreedingInfoList)
        public TokenListForBreedingInfo;
    ...
    function acceptSuperLike(
        ...
        require(
            ...
            "super like already accepted"
        );
        require(
            IERC20(ratBitTokens).transfer(
                _msgSender(),
                costForSuperLike.div(50)
            )
        );
        require(
            IERC20(ratBitTokens).transfer(
                tokenBurnAddress,
                costForSuperLike.div(25)
            )
        );
        require(
            IERC20(ratBitTokens).transfer(
    ...
    function rejectSuperLike(
        ...
        require(
            (IERC1155(toNftAddress).balanceOf(_msgSender(), toNftId) > 0),
            "You are not owner of NFT"
        );
        require(
            IERC20(ratBitTokens).transfer(formNftAddress, costForSuperLike)
        );
        isSuperliked[superLikeNFTId] = 1;
        emit SuperLikeResponse(
            fromNftId,
    ...
    function claimPendingTokensFromSuperLike(
        ...
        require(
            (IERC1155(toNftAddress).balanceOf(_msgSender(), fromNftId) > 0),
            "You are not owner of NFT"
        );
        require(
            IERC20(ratBitTokens).transfer(
                formNftAddress,
                isSuperliked[superLikeNFTId]
            )
        );
        isSuperliked[superLikeNFTId] = 1;
        ...
        returns (uint256 amount)
    {
        ...
        if (nftDetails[parentA].generation > nftDetails[parentB].generation) {
            ...
            nftDetails[parentA].generation < nftDetails[parentB].generation
        ) {
            genGap = nftDetails[parentB].generation.sub(
                nftDetails[parentA].generation
            );
        }
        if (genGap == 0) {
    ...
    function checkValidBid(uint256 _breedId) public view returns (bool) {
        if (
            breedInfoList[_breedId].ownerOfParentA == _msgSender() ||
            breedInfoList[_breedId].ownerOfParentB == _msgSender()
        ) return true;
        else return false;
    }
