...
contract EssExchangeERC20_V0{
    using SafeMath for uint256;

    address private _owner;
    uint public creationTime = block.timestamp;
    uint public ICO_RATE = 1000 * 10 ** 18; // tokens for 1 eth
    uint public rate = 1000 * 10 ** 18; // tokens for 1 eth
    IERC20 private token;

    event BoughtToken(address user, uint amount, uint costWei, uint balance);
    event SoldToken(address user, uint amount, uint costWei, uint balance);
    ...
    */
    function buyToken() payable public returns (bool success) {
        ...
        uint256 amountTobuy = payed * rate;
        uint256 dexBalance = token.balanceOf(address(this));
        require(amountTobuy > 0, "You need to send some ether");
        require(amountTobuy <= dexBalance, "Not enough tokens in the reserve");
        token.transfer(msg.sender, amountTobuy);
        emit BoughtToken(msg.sender, amountTobuy, payed, token.balanceOf(msg.sender));
        return true;
    }
    ...
    */
    function sellToken(uint amount) public returns (bool success) {
        ...
        require(_owner.balance >= costWei, "Owner has not enough balance");
        require(amount > 0, "You need to sell at least some tokens");
        uint256 allowance = token.allowance(msg.sender, address(this));
        require(allowance >= amount, "Check the token allowance");
        token.transferFrom(msg.sender, address(this), amount);
        payable(msg.sender).transfer(amount);
        emit SoldToken(msg.sender, amount, costWei, token.balanceOf(msg.sender));
        return true;
    }
