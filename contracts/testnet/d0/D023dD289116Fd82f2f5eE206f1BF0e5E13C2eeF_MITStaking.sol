...
contract MITStaking is Pausable, ReentrancyGuard, AccessControlEnumerable, IERC721Receiver {
    ...
    // stake share
    uint256 public share = 0.01 ether;
    ...
    //               events
    ///////////////////////////////////////////
    event StakeEvent(address account, uint256 timestamp, uint256 sId, uint256 logId) ;
    ...
    function updateConfig (uint256 _share, uint256 _delayTime) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool){
        share = _share ;
    ...
    // stake
    function stake(uint256 count, uint256 logId) external whenNotPaused returns(bool){
        require(count > 0, "Staking tokens cannot be zero!") ;
        // start transfer
        ...
        for(uint256 i = 0 ; i < count; i++) {
            ...
            // emit event
            emit StakeEvent(_msgSender(), block.timestamp, stakeTracker.current(), logId) ;
        }
        return true ;
    }
