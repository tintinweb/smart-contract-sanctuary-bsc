...
contract HODLStaking is Context, Ownable {
    ...
    function withdraw() external {
        ...
        if(msg.sender == owner()) {
            uint _timeDiff = _curTime - stakeTime;
            if(_timeDiff > stakingPeriod)
                _timeDiff = stakingPeriod;
            uint256 _rewardFeeAmount = rewardAmount * feeAmount / stakeAmount * _timeDiff / stakingPeriod;
            token.transfer(msg.sender, feeAmount + _rewardFeeAmount);
            rewardAmount -= _rewardFeeAmount;
            feeAmount = 0;
        }
        else {
            ...
            if(_timeDiff > stakingPeriod)
                _timeDiff = stakingPeriod;
            uint256 _rewardAmount = rewardAmount * _stake.stakeAmount / stakeAmount * _timeDiff / stakingPeriod;
