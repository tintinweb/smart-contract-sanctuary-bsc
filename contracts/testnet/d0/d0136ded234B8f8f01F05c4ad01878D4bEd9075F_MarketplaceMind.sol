    ...
    Withdraw
{
    ...
    function addListing(
        ...
        require(collection.active, "The collection is not active");
    ...
    function removeListing(uint256 collectionID, uint256 listing_id)
        public
        noReentrant
    {
        /// @dev get the collection address
        ...
        require(
            ...
            "Remove Listing: Must be owner"
        );

        require(
            listings[collection.sc_address][listing_id].is_active,
            "Remove Listing: Must be active"
        );
    ...
    function buy(uint256 collectionID, uint256 listing_id)
        public
        payable
        noReentrant
    {
        require(msg.value > 0, "Buy: Send  buy some tokens");
        ...
        require(
            listings[collection.sc_address][listing_id].price == msg.value,
            "Buy: Must pay the price"
        );
        ...
        require(
            ...
            "Buy: Failed to transfer token to fee contract Owner"
        );

        // @dev transfer the money to the seller
        (bool sent, bytes memory data) = address(
            listings[collection.sc_address][listing_id].owner
        ).call{value: msg.value}("");

        data;

        /// @dev if the contract is not sent, we send it
        require(sent, "Buy: Failed to send Ether");
    }

    /// @dev get the listing of the marketplace for contract
    function getActiveListings(uint256 collectionID, uint256 index)
        external
        view
        returns (uint256)
    {
    ...
    // @dev get the listing of the marketplace for contract
    function getListingsByOwnerCount(uint256 collectionID, address owner)
        external
        view
        returns (uint256)
    {
    ...
    /// @dev get count of listings of contract
    function getListingsCount(uint256 collectionID)
        external
        view
        returns (uint256)
    {
    ...
    // @dev get the listing of the marketplace for contract
    function getActiveListingsCount(uint256 collectionID)
        external
        view
        returns (uint256)
    {
...
contract Factory is Administered {
    ...
    function registerPair(
        address _sc_address, // address de la smart contract
        bool _active
    ) external onlyUser {
...
contract TransactionFee is Administered {
    // @dev fee per transaction for CRITTOWORLD
    uint256 public fee_fixed = 1000; // 10% (Basis Points);
    ...
    //  ________FEE______________
    // @dev fee calculation
    function calculateFee(uint256 amount) public view returns (uint256) {
        return (amount * fee_fixed) / 10000;
    }

    // @dev change transaction fee  (Basis Points)
    function changeTransactionFee(uint256 newValue) external onlyUser {
        fee_fixed = newValue;
    }

    // @dev Token purchase limit changes
    function setVaultAddress(address _address) external onlyUser {
