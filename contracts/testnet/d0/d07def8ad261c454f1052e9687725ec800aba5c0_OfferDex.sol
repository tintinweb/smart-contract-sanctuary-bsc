...
contract OfferDex is Ownable, DexStorage {
  ...
  function makeOfferPayable(uint256 tokenId, address collection, uint256 amount) isValid(collection) public payable {
   require(amount > 0, "129");
    ...
    ++_offers[tokenId][collection].count;
    offerId++;
    offer memory tempOffer =  offer(msg.sender, "", 2, amount, block.timestamp);
    BNB.deposit.value(msg.value)();
    _offers[tokenId][collection]._offer[offerId] = tempOffer;
    _offers[tokenId][collection].offerIds.push(offerId);
    emit MakeOffer(tokenId, collection, "", msg.sender, 2, amount, offerId);
  }
  ...
  function clearOffers(uint256 tokenId, address collection, uint256 index) private {
      ...
      for(uint256 i = 0; i < temp.count; i++){
        ...
        tempOfferId = temp.offerIds[i];
        if(tempOfferId != index){
              token = ALIA;
           if(temp._offer[tempOfferId].currencyType == 1){
               token = BUSD;
           }
         if(temp._offer[tempOfferId].currencyType == 2){
           chainTransfer(temp._offer[tempOfferId]._address, 1000, price);
         } else {
            token.transfer(temp._offer[tempOfferId]._address, price);
         }
        }
          delete _offers[tokenId][collection]._offer[tempOfferId];
      }
