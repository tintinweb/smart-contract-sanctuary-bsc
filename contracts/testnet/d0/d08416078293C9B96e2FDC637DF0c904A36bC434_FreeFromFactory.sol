...
contract FreeFromFactory is OwnableUpgradeable, Signature{
    ...
    function batchFactoryCreate(address[] memory _users, 
                          string[] memory _names, 
                          string[] memory _symbols, 
                          string memory _baseURI
                          ) public onlyOwner {
      require(_users.length == _names.length, "length not same");
      require(_users.length == _symbols.length, "length not same");
    ...
    function batchMintMessageByFactory(address[] memory _nfts, string[] memory _cids, bytes[] memory _signatures) public onlyOwner {
      require(_nfts.length == _cids.length, "length not same");
      require(_nfts.length == _signatures.length, "length not same");
