...
contract AurumArcade is Context, Ownable {
    ...
    * ---------------------------------------------- */
    ...
    GameEntity[] public arcadeGames;        // list of games    
    uint256 public totalArcadeGames = 0;    // total number of contests 
 
    // @dev Owner functions start -------------------------------------

    function addArcadeGame(string memory _gameName) external onlyOwner {        
    ...
    function getArcadeGamesInfo() external view returns (GameEntity[] memory){
        GameEntity[] memory structure = new GameEntity[](totalArcadeGames);
        for (uint i = 0; i < totalArcadeGames; i++) {
            GameEntity storage game = arcadeGames[i];
            structure[i] = game;
        }
        return structure;
    }
    ...
    * ---------------------------------------------- */
    ...
    function getAllActiveContestsInfo() external view returns(WeeklyContestEntity[] memory allContests) {         
        WeeklyContestEntity[] memory structure = new WeeklyContestEntity[](totalContestCreated);     
        uint activeCounter = 0;

        // get filter list of all active
        for (uint i = 0; i < totalContestCreated; i++) {
            if(contests[i].active){
                WeeklyContestEntity storage contest = contests[i];
                structure[activeCounter] = contest;
                activeCounter++;
            }
        }
        
        // create new active only array
        WeeklyContestEntity[] memory activeOnlyStructure = new WeeklyContestEntity[](activeCounter);
        for (uint i = 0; i < activeCounter; i++) { 
            activeOnlyStructure[i] =  structure[i]; 
        }
    ...
    * ---------------------------------------------- */
    ...
    function getAllPlayerConsumables(address _account) external view returns(OwnedConsumablesEntity[] memory) {                  
        ...
        OwnedConsumablesEntity[] memory structure = new OwnedConsumablesEntity[](consumablesOwned[_account].length);
        for (uint i = 0; i < consumablesOwned[_account].length; i++) {
            OwnedConsumablesEntity storage consumable = consumablesOwned[_account][i];
            structure[i] = consumable;
        }
        return structure;
    }

    function getAllActiveConsumables() external view returns(InGameConsumablesEntity[] memory allContests) {         
        InGameConsumablesEntity[] memory structure = new InGameConsumablesEntity[](totalConsumablesCreated);     
        uint activeCounter = 0;

        // get filter list of all active
        for (uint i = 0; i < totalConsumablesCreated; i++) {
            if(contests[i].active){
                InGameConsumablesEntity storage consumable = consumables[i];
                structure[activeCounter] = consumable;
                activeCounter++;
            }
        }
        
        // create new active only array
        InGameConsumablesEntity[] memory activeOnlyStructure = new InGameConsumablesEntity[](activeCounter);
        for (uint i = 0; i < activeCounter; i++) { 
            activeOnlyStructure[i] = structure[i]; 
        }
        return activeOnlyStructure;
    ...
    * ---------------------------------------------- */
    ...
    function getAllActiveChallenges() external view returns(InGameChallengesEntity[] memory allContests) {         
        InGameChallengesEntity[] memory structure = new InGameChallengesEntity[](totalChallengesCreated);     
        uint activeCounter = 0;

        // get filter list of all active
        for (uint i = 0; i < totalChallengesCreated; i++) {
            if(contests[i].active){
                InGameChallengesEntity storage challange = challenges[i];
                structure[activeCounter] = challange;
                activeCounter++;
            }
        }
        
        // create new active only array
        InGameChallengesEntity[] memory activeOnlyStructure = new InGameChallengesEntity[](activeCounter);
        for (uint i = 0; i < activeCounter; i++) { 
            activeOnlyStructure[i] =  structure[i]; 
        }
        return activeOnlyStructure;
            ...
            if(!challenges[i].claimed){
                if( challenges[i].challenger == account){            
                    ...
                    if((challenges[i].challengeScore > challenges[i].opponentScore) || (hoursDiff > challengeAllowedHours && !(challenges[i].challengeScore < challenges[i].opponentScore))){
                       ...
                       updatePlayerRanking(challenges[i].challenger, GameOutcome.WIN, rankingSystem.RankingIncrement);
                       updatePlayerRanking(challenges[i].opponent, GameOutcome.LOSE, -rankingSystem.RankingIncrement);
                    }
                } 
                    ...
                    if(challenges[i].opponentScore > challenges[i].challengeScore){
                        rewardsTotal += (challenges[i].challengeStake * 2);
                        challenges[i].claimed = true;
                       updatePlayerRanking(challenges[i].opponent, GameOutcome.WIN, rankingSystem.RankingIncrement);
                       updatePlayerRanking(challenges[i].challenger, GameOutcome.LOSE, -rankingSystem.RankingIncrement);
                    }
                }
            }
        }
