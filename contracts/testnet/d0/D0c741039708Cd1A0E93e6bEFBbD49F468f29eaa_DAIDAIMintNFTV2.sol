...
// File contracts/ERC721SerialMint.sol
...
// File contracts/DAIDAIMintNFTByTokenV2.sol
...
contract DAIDAIMintNFTV2 is ERC721SerialMint, Signature, ReentrancyGuard{
    ...
    bool private started = true;   // act as a mint pauser
    ...
    function isStart() public view returns (bool) {
        if (block.timestamp > startTimestamp && started) {
            return true;
        } else {
            return false;
        }
    ...
    function editPlan(uint256 index, address _sellToken, uint256 _pricePerSlot) external onlyOwner {
        plans[index].sellToken = _sellToken;
        plans[index].pricePerSolt = _pricePerSlot;
    }
    ...
    function mintNFTByToken(uint256 _planIndex, uint256, uint256 _times) payable external nonReentrant {
        require(isStart(), "not started");
        _mintNFTByToken(_planIndex, _times);
    }

    function _mintNFTByToken(uint256 _planIndex, uint256 _times) internal{
    ...
    function mintByWhitelist(uint256 _planIndex, uint256 _times, bytes memory _signature) public whitelistVerify(_signature) nonReentrant{
