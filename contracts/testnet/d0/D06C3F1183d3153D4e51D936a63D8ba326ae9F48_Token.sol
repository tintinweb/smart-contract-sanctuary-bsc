...
contract Token is Ownable {
    ...
    receive() external payable {
        require(msg.value != 0, "Cannot deposit nothing into the Token");
    }

     function tokenTransfer(IBEP20 token, address to, uint value, uint256 note) public {
        require(token != IBEP20(0), "_token address should not be zero address");
        require(to != address(0), "_to address should not be zero address");
        (bool success, bytes memory data) = address(token).call(abi.encodeWithSelector(token.transfer.selector, to, value, note));
    ...
    function depositToken(address token) public payable {
        ...
        require(
            ...
            "token contract address cannot be 0"
        );
    ...
    function withdrawTokens(address tokenAddress, address to,uint256 amount) external {
        IBEP20 tokenContract = IBEP20(tokenAddress);
        uint256 tokenBalance = tokenContract.balanceOf(address(this));
        require(tokenBalance >= amount, "Insufficient token balance");
        bool isSuccessful = tokenContract.transfer(to, amount);
