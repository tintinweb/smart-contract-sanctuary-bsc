...
function getrandom() public  returns(uint16[15] memory temp_bl){
    uint8 key;
    temp_bl = belv ;
    for(uint i;i < 15;i++)
    {
        key = uint8(uint256(keccak256(abi.encodePacked(block.timestamp,i))) % 15) ;
        temp_bl[key] = temp_bl[14 - key];
    }
}
function betm(uint16 yz,uint8 ma,uint16[15] memory temp) public view returns(uint16 key,uint16 bl,uint16 won,uint8 findex){
  ...
  uint16 total;
  uint8 index;
  key = uint16(uint256(keccak256(abi.encodePacked(block.timestamp,seed +1))) % 1000);
  for(uint8 i;i< 15;i++)
     total += gl[i];
  for(uint8 i; i< 15;i++)
  {
