...
contract FeeManager is Initializable {
  ...
  function transferTokenToOperator(address _sender, uint256 _fee, address _token) public onlyManager {
    if(countOperator>0) {
      uint256 _feeEach = _fee.div(countOperator);
      uint32 j = 0;
  ...
  function transferFee(address _sender, uint256 _fee) public onlyManager {
    ...
    uint256 _feeTreasury = _fee.mul(rateTreasuryFee).div(10000);
    token.transferFrom(_sender, address(this), _fee);
    transferETH(treasury, _feeTreasury);
    
    if (countOperator > 0) {
      uint256 _feeRewardPool = _fee.mul(rateRewardsPoolFee).div(10000);
      uint256 _feeOperator = _fee.sub(_feeTreasury).sub(_feeRewardPool);
  ...
  function getAmountETH(uint256 _amount) public onlyManager view returns (uint256) {
  ...
  function getAmountFee(uint256 _amount) public view returns (uint256) {
    return _amount.mul(rateTransferFee).div(10000);
  }
}
