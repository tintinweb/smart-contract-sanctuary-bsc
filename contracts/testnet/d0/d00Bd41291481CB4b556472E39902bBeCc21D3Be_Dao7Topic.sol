...
contract Dao7Topic is Ownable {
    ...
    function getDynamicOdds(address token,uint256 option) public view returns(uint256 odds,uint256 trueOptionAmount,uint256 sumAmount){

        sumAmount = option1Map[token];
        ...
        if(trueOptionAmount > 0){
            ...
            // odds = ( totalAmount ) * 1e8 / trueOptionAmount;
            uint256 bigCake =  10000 - tokenFee;
            uint256 otherOption =  sumAmount - trueOptionAmount;
            uint256 otherOptionWithOutFee = otherOption/10000*bigCake;
            uint256 allOption = trueOptionAmount + otherOptionWithOutFee;
            odds =  allOption * 1000000000 / trueOptionAmount;
        }
    ...
    function getWinAmount(address token,address userAddress) public view returns(uint256){
        ...
        (,bool tokenInserted,,,,,uint256 rebate1,uint256 rebate2,,,) = mainContract.getToken(token);
        if(!tokenInserted){
            return 0;
        }
        ...
        betAmount = betAmount -  betAmount / 10000 * (rebate1 + rebate2);
        return  betAmount * odds / 1e8;
    }
