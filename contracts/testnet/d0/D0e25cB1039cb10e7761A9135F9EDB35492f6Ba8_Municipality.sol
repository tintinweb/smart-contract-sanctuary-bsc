...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
    ...
    uint8 private constant BUNDLE_TYPE_1 = 10;
    uint8 private constant BUNDLE_TYPE_4 = 20;
    uint8 private constant BUNDLE_TYPE_16 = 30;
    uint8 private constant BUNDLE_TYPE_64 = 40;
    uint8 private constant BUNDLE_TYPE_128 = 50;
    uint8 private constant BUNDLE_TYPE_256 = 60;
    ...
    /// @notice Modifier for access by the whitelisted addresses only
    ...
    modifier notZeroAddress() {
        ...
        (
            ...
            "Municipality: Caller can not be address 0"
        );
        _;
    }
    ...
    modifier onlySaleActive() {
        ...
        (
            isSaleActive,
            "Municipality: Sale is deactivated now"
        );
        _;
    }
    ...
    function setParcelsSoldCountPricing(uint256[] calldata _standardParcelPrices,
        uint256[] calldata _businessParcelPrices) external onlyOwner notZeroAddress
    {
        require(_standardParcelPrices[0] == 0, "Municipality:: setParcelsSoldCountPricing: The given standard parcel array must start from 0");
        require(_businessParcelPrices[0] == 0, "Municipality:: setParcelsSoldCountPricing: The given business parcel array must start from 0");
        for (uint256 i; i < _standardParcelPrices.length; i += 2) {
        ...
        for (uint256 i; i < _businessParcelPrices.length; i += 2) {
    ...
    function setBundles(BundleInfo[6] memory _bundles) external onlyOwner notZeroAddress {
        bundles = _bundles;
    }

    /// @notice Set contract addresses for all NFTs we currently have
    function setNFTContractAddresses(address[3] memory _nftContractAddresses) external onlyOwner notZeroAddress {
        _requireNewAddressValid(_nftContractAddresses[0], standardParcelNFTAddress);
        _requireNewAddressValid(_nftContractAddresses[1], businessParcelNFTAddress);
        _requireNewAddressValid(_nftContractAddresses[2], minerV1NFTAddress);
    ...
    /// @notice Set addresses for all tokens we currently use
    function setTokensAddresses(address[4] memory _tokenAddresses) external onlyOwner notZeroAddress {
        _requireNewAddressValid(_tokenAddresses[0], busdAddress);
        _requireNewAddressValid(_tokenAddresses[1], wbnbAddress);
        _requireNewAddressValid(_tokenAddresses[2], vbtcAddress);
        _requireNewAddressValid(_tokenAddresses[3], gymNetworkAddress);

        busdAddress = _tokenAddresses[0];
        wbnbAddress = _tokenAddresses[1];
        vbtcAddress = _tokenAddresses[2];
        gymNetworkAddress = _tokenAddresses[3];
    }

    /// @notice Set addresses to all smart contract and wallet addresses we currently distribute the amounts to
    function setDistributionReceiverAddresses(address[8] memory _distributionReciverAddresses) external onlyOwner {
        _requireNewAddressValid(_distributionReciverAddresses[0], nftReflectionPoolAddress);
        _requireNewAddressValid(_distributionReciverAddresses[1], netGymStreetAddress);
        _requireNewAddressValid(_distributionReciverAddresses[2], ambassadorWalletAddress);
        _requireNewAddressValid(_distributionReciverAddresses[3], reserveWalletAddress);
        _requireNewAddressValid(_distributionReciverAddresses[4], companyWalletAddress);
        _requireNewAddressValid(_distributionReciverAddresses[5], gymTurnoverPoolAddress);
        _requireNewAddressValid(_distributionReciverAddresses[6], buyAndBurnAddress);
        _requireNewAddressValid(_distributionReciverAddresses[7], nftRankRewardsAddress);

        nftReflectionPoolAddress = _distributionReciverAddresses[0];
        netGymStreetAddress = _distributionReciverAddresses[1];
        ambassadorWalletAddress = _distributionReciverAddresses[2];
        reserveWalletAddress = _distributionReciverAddresses[3];
        companyWalletAddress = _distributionReciverAddresses[4];
        gymTurnoverPoolAddress =_distributionReciverAddresses[5];
        buyAndBurnAddress = _distributionReciverAddresses[6];
        nftRankRewardsAddress = _distributionReciverAddresses[7];
    }

    /// @notice Set the address to our back-end component
    function setBackendAddress(address _backendAddress) external onlyOwner notZeroAddress {
        _requireNewAddressValid(_backendAddress, backendAddress);
        backendAddress = _backendAddress;
    }

    /// @notice Set the address for the router
    function setRouterAddress(address _routerAddress) external onlyOwner notZeroAddress {
        _requireNewAddressValid(_routerAddress, routerAddress);
        routerAddress = _routerAddress;
    }

    /// @notice Set the address to the mining smart contract
    function setMiningAddress(address _miningAddress) external onlyOwner notZeroAddress {
        _requireNewAddressValid(_miningAddress, miningAddress);
        miningAddress = _miningAddress;
    }

    /// @notice Set the number of slots available for the miners for standard and upgraded parcels
    function setParcelsSlotsCount(uint8[2] memory _parcelsSlotsCount) external onlyOwner notZeroAddress {
        _requireNumberNotEqual(_parcelsSlotsCount[0], standardParcelSlotsCount);
        _requireNumberNotEqual(_parcelsSlotsCount[1], upgradedParcelSlotsCount);
    ...
    function setPurchasePrices(uint256[4] memory _purchasePrices) external onlyOwner notZeroAddress {
        _requireNumberNotEqual(_purchasePrices[0], upgradePrice);
        _requireNumberNotEqual(_purchasePrices[1], minerPrice);
        _requireNumberNotEqual(_purchasePrices[2], minerRepairPrice);
        _requireNumberNotEqual(_purchasePrices[3], electricityVoucherPrice);
    ...
    function setSaleActivation(bool _saleActivation) external onlyOwner notZeroAddress {
        require(
            _saleActivation != isSaleActive,
            "Municipality:: setSaleActivation: Sale activation is already set to this value"
        );
        isSaleActive = _saleActivation;
    }

    // @notice (Purchase) Generic minting functionality for parcels, regardless the currency
    function mintParcels(ParcelMintSignature calldata mintingSignature,
    ...
    { 
        ...
        uint256 purchasePrice = _getPriceForParcels(mintingSignature.parcels);
        _transferToContract(purchasePrice);
        _distributeAmounts(purchasePrice, OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, msg.sender);
        userToPurchasedAmountMapping[msg.sender] += purchasePrice;
        _mintParcels(mintingSignature.parcels);
    }

    // @notice (Purchase) Mint the given amount of miners
    function mintMiners(uint256 _count, uint256 _referrerId) external onlySaleActive notZeroAddress
    {
        INetGymStreet(netGymStreetAddress).addGymMlm(msg.sender, _referrerId);
        uint256 purchasePrice = _count * minerPrice;
        _transferToContract(purchasePrice);
        userToPurchasedAmountMapping[msg.sender] += purchasePrice;
        _distributeAmounts(purchasePrice, OPERATION_TYPE_MINT_MINERS, msg.sender);
        _mintMiners(_count);
    }

    function purchaseBundle(ParcelMintSignature calldata _mintingSignature, uint8 _bundleType,
        uint256 _referrerId) external onlySaleActive notZeroAddress
    {
        ...
        uint256 bundlePurchasePrice = _discountPrice(
            bundle.bundlePrice, 
            bundle.discountPct
        );
        ...
        uint256 minersPurchasePrice = _discountPrice(
            minersAmount * minerPrice,
            bundle.discountPct
        );
        uint256 parcelPurchasePrice = bundlePurchasePrice - minersPurchasePrice;
        _distributeAmounts(minersPurchasePrice, OPERATION_TYPE_MINT_MINERS, msg.sender);
        _distributeAmounts(parcelPurchasePrice, OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, msg.sender);
        _mintParcels(_mintingSignature.parcels);
        _mintMiners(minersAmount);
    }
    ...
    function purchaseVouchers(uint16 count) external onlySaleActive notZeroAddress {
        ...
        _distributeAmounts(_amount, OPERATION_TYPE_PURCHASE_ELECTRICITY, msg.sender);
    ...
    function repairAllMiners() external onlySaleActive notZeroAddress {
        uint256 _amount = IMining(miningAddress).getMinersCount(msg.sender) * minerRepairPrice;
        _transferToContract(_amount);
        _distributeAmounts(_amount, OPERATION_TYPE_PURCHASE_REPAIR, msg.sender);
    ...
    function applyVoucher(uint256 numRequestedVouchers, uint256 minerId) external notZeroAddress {
    ...
    function attachDetachMinersToParcel(uint256[] memory minersToAttach, uint256 parcelId) external notZeroAddress {
        require(IERC721Base(standardParcelNFTAddress).exists(parcelId), "Municipality::attachDetachMinersToParcel Parcel doesnt exist");
    ...
    function upgradeStandardParcel(uint256 _parcelId) external onlySaleActive
    {
        ...
        _distributeAmounts(upgradePrice, OPERATION_TYPE_MINT_OR_UPGRADE_PARCELS, msg.sender);
    ...
    function buyAndBurnGYMNET() external onlyWhitelisted {
        IERC20Upgradeable(busdAddress).safeTransfer(buyAndBurnAddress, buyAndBurnAmountGYM / 2);
        IBuyAndBurn(buyAndBurnAddress).buyAndBurnGymWithBUSD(
            buyAndBurnAmountGYM / 2,
            uint256(0),
            uint256(block.timestamp + 100)
        );

        uint256 BNBAmount = _convertBUSDtoBNB(buyAndBurnAmountGYM / 2);
        IERC20Upgradeable(wbnbAddress).safeTransfer(buyAndBurnAddress, BNBAmount);
        IBuyAndBurn(buyAndBurnAddress).buyAndBurnGymWithBNB(
            BNBAmount,
            uint256(0),
            uint256(block.timestamp + 100)
        );
        buyAndBurnAmountGYM = 0;
    }

    function linearBuyAndBurnVBTCAmt() external onlyWhitelisted {
        //TODO: expand later
    }

    function provideVBTCLiquidity() external onlyWhitelisted {
        _addLiquidity(busdAddress, vbtcLiquidityAmount, 0, block.timestamp + 100);
        vbtcLiquidityAmount = 0;
    }
    ...
    function getPriceForParcels(Parcel[] memory parcels) public view returns (uint256) {
        return _getPriceForParcels(parcels);
    }
    
    function getUserMiners() public view returns (AttachedMiner[] memory) {
        uint256[] memory userMiners = IERC721Base(minerV1NFTAddress).tokensOf(msg.sender);
        ...
        for(uint256 i = 0; i < userMiners.length; ++i) {
            ...
            AttachedMiner memory m = AttachedMiner(parcelId, minerId);
            result[i] = m;
        }
        return result;
    }
    ...
    function _getPriceForParcels(Parcel[] memory parcels) private view returns (uint256) {
        ...
        for (uint256 i = 0; i < parcels.length; ++i) {
            ...
            if (parcel.parcelType == PARCEL_TYPE_STANDARD) {
                ...
                revert("Municipality: Invalid Parcel type");
            }
        }
        return totalPrice;
    }
    ...
    function _requireNewAddressValid(address _newAddress, address _oldAddress) private pure {
        require(
            _oldAddress != _newAddress,
            "Municipality:: _requireNewAddressValid: New address matches old address"
        );
        require(
            _newAddress != address(0),
            "Municipality:: _requireNewAddressValid: New address can not be zero address"
        );
    }

    function _requireNumberNotEqual(uint256 _numberOne, uint256 numberTwo) private pure {
        require(
            _numberOne != numberTwo,
            "Municipality:: _requireNumberNotEqual: this numbers can not be equal"
        );
    }
    ...
    function _attachDetachMinersToParcel(uint256[] memory newMiners, uint256 parcelId) private {
        ...
        for (uint256 index; index < oldMiners.length; index++) {
            ...
            if (!_isMinerInList(tokenId, newMiners)) {
                ...
                IMining(miningAddress).withdraw(tokenId);
            }
        }
    ...
    function _distributeAmounts(uint256 _amount, uint8 operationType, address _user) private {
        uint256[10] memory distributionPercentages = operationTypeToPercentagesMapping[
        operationType
        ];
        _transferAmountTo((_amount * distributionPercentages[0]) / 100, companyWalletAddress);
    ...
    function _distributeNetGymStreet(uint256 _amount, uint256 _distributeAmount, address _user) private {
        if (_amount > 0) {
    ...
    function _distributeNftReflectionPool(uint256 _amount) private {
        if (_amount > 0) {
            _transferAmountTo(_amount, nftRankRewardsAddress);
            INftReflection(nftRankRewardsAddress).updatePool(_amount);
        }
    }
    ...
    function _addLiquidity(address _basePoolToken, uint256 _baseTokenAmount, uint256 _minAmountOut,
        uint256 _deadline) private returns (uint256 lpTokens)
    {
        ...
        uint256 baseTokenToLpAmount = IERC20Upgradeable(_basePoolToken).balanceOf(address(this));

        IERC20Upgradeable(_basePoolToken).approve(routerAddress, baseTokenToLpAmount / 2);
        IERC20Upgradeable(vbtcAddress).approve(routerAddress, vbtcToLpAmount);

        (, , lpTokens) = IPancakeRouter02(routerAddress).addLiquidity(
            _basePoolToken,
            address(vbtcAddress),
            baseTokenToLpAmount / 2,
            vbtcToLpAmount,
            _minAmountOut,
            0,
            address(this),
    ...
    function _convertBUSDtoBNB(uint256 _amount) private returns (uint256) {
        IERC20Upgradeable(busdAddress).approve(routerAddress, _amount);
        return _swapTokens(busdAddress, wbnbAddress, _amount, address(this), block.timestamp + 100);
    }

    function _swapTokens(address inputToken, address outputToken, uint256 inputAmount, address receiver,
    ...
    function _mintParcels(Parcel[] calldata parcels) private
    {
        for (uint256 i = 0; i < parcels.length; ++i) {
            Parcel memory parcel = parcels[i];
            require(
                parcel.parcelType == PARCEL_TYPE_STANDARD ||
                parcel.parcelType == PARCEL_TYPE_BUSINESS,
                "Municipality: Invalid parcel type"
            );
            require(
            ...
            require(
                !IParcelInterface(standardParcelNFTAddress).parcelExists(parcel.x, parcel.y, parcel.parcelType, parcel.parcelLandType),
                "Municipality: Parcel already exists as a standard parcel"
            );
            require(
                !IParcelInterface(businessParcelNFTAddress).parcelExists(parcel.x, parcel.y, parcel.parcelType, parcel.parcelLandType),
                "Municipality: Parcel already exists as a business parcel"
            );
            address addr;
            if(parcel.parcelType == PARCEL_TYPE_STANDARD) {
                currentlySoldStandardParcelsCount++;
                uint256 stParcelPrice = soldCountToStandardParcelPriceMapping[currentlySoldStandardParcelsCount];
                currentStandardParcelPrice = stParcelPrice != 0 ? stParcelPrice : currentStandardParcelPrice;
                addr = standardParcelNFTAddress;
            } else {
                currentlySoldBusinessParcelsCount++;
                uint256 bsParcelPrice = soldCountToBusinessParcelPriceMapping[currentlySoldBusinessParcelsCount];
                currentBusinessParcelPrice = bsParcelPrice != 0 ? bsParcelPrice : currentBusinessParcelPrice;
                addr = businessParcelNFTAddress;
            }
            IParcelInterface(addr).mint(msg.sender, parcel.x, parcel.y, parcel.parcelType, parcel.parcelLandType);
        }
    }

    function _mintMiners(uint256 _count) private {
        for (uint256 i = 0; i < _count; ++i) {
            IMinerNFT(minerV1NFTAddress).mint(msg.sender);
        }
    }

    function _requireMinersCountMatchingWithParcelSlots(uint256 _parcelId, uint256 _count) private view
    {
        bool isParcelUpgraded = IParcelInterface(standardParcelNFTAddress).isParcelUpgraded(_parcelId);
        require(isParcelUpgraded ? _count <= upgradedParcelSlotsCount : _count <= standardParcelSlotsCount,
            "Municipality:: _requireMinersCountMatchingWithParcelSlots: Miners count exceeds parcel's slot count"
        );
    }

    function _verifyMintParcelsSignature(ParcelMintSignature calldata mintParcelSignature) private view returns (address) {
        bytes32 _digest = _hash(mintParcelSignature);
        return ECDSAUpgradeable.recover(_digest, mintParcelSignature.signature);
    }

    function _hash(ParcelMintSignature calldata mintParcelSignature) private view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
...
interface IParcelInterface {
    function mint(address user, uint256 x, uint256 y, uint256 parcelType, uint256 landType) external returns (uint256);
    function parcelExists(uint256 x, uint256 y, uint256 parcelType, uint256 landType) external view returns(bool);
    function getParcelId(uint256 x, uint256 y, uint256 parcelType, uint256 landType) external pure returns (uint256);
...
interface IMining {
    ...
    function withdraw(uint256 _miner) external;
    ...
    function getMinerElectricityEndDate(uint256 _minerId) external view returns(uint256);
}
