...
contract JackpotToken is Context, IERC20, Ownable {
    ...
    uint256 presaleMaxAllowance = 3000 * 10 **_decimals;
    uint256 totalPresaleBuy;
    uint256 presaleMinBuy = 50 ether;
    /// 1 JCK token = 0.10$ for presale
    uint256 presaleTokenPriceStable = 0.1 ether;
    ...
    function presaleBuy(bytes32 merkleTreeRoot, uint256 amountToBuy) public whenPresaleOpen {
        require(amountToBuy >= presaleMinBuy && userAmountBuyPresale[_msgSender()] <= presaleMaxAllowance);
        /// TODO : require merkleTree
        userAmountBuyPresale[_msgSender()] += amountToBuy;
        uint256 amountInStable = amountToBuy.mul(presaleTokenPriceStable);
        IERC20(USDT).transferFrom(_msgSender(), _liquidityPresaleWallet, amountInStable);
        _mint(_msgSender(), amountToBuy);
        totalPresaleBuy += amountToBuy;
    }
