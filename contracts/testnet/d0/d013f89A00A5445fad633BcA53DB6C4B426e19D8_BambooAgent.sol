...
contract BambooAgent is Context, Ownable {
    ...
    // address public _bamboo=address(0x1473CA1ec28cfE2760686C27ABBC56Cfcb948198);
    // address public _setter=address(0x206265C5eFa5d4077533dAC7A4F40484d23f64fF);//test
    address public _usdt=address(0xD05A097B1Dc5Bd0733b9460fa562497278F55E36);
    address public _bamboo=address(0x63fb946d4A71d7aB93b4F4f174b4025d7f419062);
    ...
    function setContract(address bamboo,address setter) external onlyOwner{
        _bamboo=bamboo;
        _setter=setter;
    }
    //兑换
    function swap(uint inAmount, address[] calldata path, address to) external onlyBamboo{
        uint leftIn=IBEP20(path[0]).balanceOf(address(this));
        IBEP20(path[0]).transferFrom(_msgSender(),address(this),inAmount);
        uint plusIn=IBEP20(path[0]).balanceOf(address(this))-leftIn;
        IBEP20(path[0]).approve(address(uniswapV2Router),2**256-1);
        uint leftOut=IBEP20(path[1]).balanceOf(address(this));
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(plusIn,0,path,address(this),block.timestamp);
        uint plusOut=IBEP20(path[1]).balanceOf(address(this))-leftOut;
        if(plusOut>0){
            IBEP20(path[1]).transfer(to,plusOut);
        }
    }
    //设置余额
    ...
    function withdraw(uint _type,uint amount) external{
        ...
        require(amount>0,"err13");
        _sub(_type,_msgSender(),amount);
        address token=IBEP20(_bamboo)._token();
        uint outAmount=_amountOut(amount,_usdt,token);
        if(outAmount>0){
            IBEP20(token).transfer(_msgSender(),outAmount);
        }
    }
