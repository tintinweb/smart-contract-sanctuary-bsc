...
contract ESTracker is Initializable, OwnableUpgradeable {
    ...
    function initialize() initializer public{
        ...
        tokenContract = 0x9CdF641284ae6759d61ad1F0288CC7e10428993f;
    ...
    modifier onlyToken() {
        require(msg.sender == tokenContract, "ESTracker: only token");
        _;
    }

    modifier onlyOperator() {
        require(operatorWallets[msg.sender], "ESTracker: only operators");
        _;
    }
    ...
    function getUserHistory(address user) public view returns (UserHistory[] memory) {
        ...
        for (uint256 i = 1; i < weekIndex; i++) {
            ...
            history[i].tokens = weeklyTrackers[i].esToken.symbol();
            history[i].decimals = weeklyTrackers[i].esToken.decimals();
        }
        return (history);
    }
    ...
    function addOperator(address _operator) external onlyOwner {
        require(operatorWallets[_operator] != true, "ESTracker: User is already set to this status");
        require(_operator != address(0), "ESTracker: Zero address cannot be an operator");
    ...
    function vote(address token) external {
        require(
            ...
            "ESTracker: ES Token cannot be BNB"
        );
        require(weeklyTrackers[week].voted[msg.sender] == false, "ESTracker: already voted");
        require(isEligible(msg.sender), "ESTracker: Caller is not eligible for the week");

        if (!_proposedTokens.contains(token)) {
            require(allowNewTokens, "ESTracker: New tokens are not allowed");
    ...
    function _closeWeek() private {
        require(canClose(), "ESTracker: Too early to close the week");
        ...
        address esToken = weeklyTrackers[week].winningToken;
        if (esToken != address(0)){
            weeklyTrackers[week].esToken = IERC20Upgradeable(esToken);
        } else {
            weeklyTrackers[week].esToken = weeklyTrackers[week - 1].esToken;
        }

        if(weeklyTrackers[week].totalBNB > 0) {
            _swapBNBForESToken(weeklyTrackers[week].totalBNB);
        }
    ...
    function _swapBNBForESToken(uint256 amount) private {
        uint256 initialBalance = weeklyTrackers[week].esToken.balanceOf(address(this));

        address[] memory path = new address[](2);
        path[0] = router.WETH();
        path[1] = address(weeklyTrackers[week].esToken);
        ...
        uint256 newBalance = weeklyTrackers[week].esToken.balanceOf(address(this)) - initialBalance;
    ...
    function distributeESReward() external onlyOperator {
        _distributeESReward();
    }

    function _distributeESReward() private {
        require(weeklyTrackers[weekIndex].isClosed == true, "ESTracker: Week is not closed");
        ...
        if(weeklyTrackers[weekIndex].rewardPerHolder > 0) {
            for (uint256 i = 0; i < weeklyTrackers[weekIndex].eligibleUsers.length; i++) {
                ...
                weeklyTrackers[weekIndex].esToken.transfer(user, weeklyTrackers[weekIndex].rewardPerHolder);
