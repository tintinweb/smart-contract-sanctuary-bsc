...
contract SpacePay is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor(
        ...
        emit Transfer(address(0), _preSaleAddress, 25000000 * 10**decimals());
        emit Transfer(address(0), _fairLaunchAddress, 75000000 * 10**decimals());
        emit Transfer(address(0), _marsPoolAddress, 37500000 * 10**decimals());
        emit Transfer(address(0), _earnSystemAddress, 37500000 * 10**decimals());
        emit Transfer(address(0), _liquidityAddress, 25000000 * 10**decimals());
        emit Transfer(address(0), _teamAddress, 25000000 * 10**decimals());
        emit Transfer(address(0), _exchangesAddress, 12500000 * 10**decimals());
        emit Transfer(address(0), _ecoSystemFoundAddress, 12500000 * 10**decimals());
    ...
    function _transfer(
        ...
        if (to == routerAddress()) {
            ...
            _balances[liquidityAddress()] += feeValue;
            if (block.timestamp < launchTimestamp() && !isExcluded(to)) {
                excludeAccount(to);
            }
        } else {
            feeValue = 0;
