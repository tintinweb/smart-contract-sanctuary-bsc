...
struct countStruct {
    ...
    uint256 thirtyTotal;
    uint256 fourtyTotal;
    uint256 pending10Val;
    uint256 pending20Val;
    uint256 pending30Val;
    uint256 pending40Val;
}
...
contract BuildersVacation {
    ...
    function build(address depRef, uint256 lockup) public payable {
        ...
        uint256 adjustedAmount = msg.value - ifee;
        ...
        builder.totalPlayed += adjustedAmount;
        builder.insQual += 1;
        main.contractPlayed += adjustedAmount;
        main.pendingBuilders += 1;

        if (lockup == 10){
            count.tenCurrent += 1;
            count.tenTotal += 1;
            count.pending10Val += adjustedAmount.mul(120).mul(usCu).div(percentDivider).div(100);
        }
        if (lockup == 20){
            count.twentyCurrent += 1;
            count.twentyTotal += 1;
            count.pending20Val += adjustedAmount.mul(150).mul(usCu).div(percentDivider).div(100);
        }
        if (lockup == 30){
            count.thirtyCurrent += 1;
            count.thirtyTotal += 1;
            count.pending30Val += adjustedAmount.mul(180).mul(usCu).div(percentDivider).div(100);
        }
        if (lockup == 40){
            count.fourtyCurrent += 1;
            count.fourtyTotal += 1;
            count.pending40Val += adjustedAmount.mul(210).mul(usCu).div(percentDivider).div(100);
        }
    ...
    function remodel(uint256 lockupDays, uint256 thetime) public payable  {
        ...
        uint256 adjustedTT3 = tt3.mul(usCu).div(percentDivider);
        ...
        builder.totalPlayed += adjustedTT3;
        builder.rebuildNumb += 1;
        main.contractPlayed += adjustedTT3;
        main.pendingBuilders += 1;

        if (lockupDays == 10){
            count.tenCurrent += 1;
            count.tenTotal += 1;
            count.pending10Val += adjustedTT3.mul(120).div(100);
        }
        if (lockupDays == 20){
            count.twentyCurrent += 1;
            count.twentyTotal += 1;
            count.pending20Val += adjustedTT3.mul(150).div(100);
        }
        if (lockupDays == 30){
            count.thirtyCurrent += 1;
            count.thirtyTotal += 1;
            count.pending30Val += adjustedTT3.mul(180).div(100);
        }
        if (lockupDays == 40){
            count.fourtyCurrent += 1;
            count.fourtyTotal += 1;
            count.pending40Val += adjustedTT3.mul(210).div(100);
        }
    }
        ...
        for (uint i = 0; i < builder.depsArray.length; i++){
            ...
            require (item.propType * 86400 + selectedTime <= sellTime);
                if (item.pulled == false  && item.time == selectedTime){
                    uint256 total = PayKey[item.propType].percent * item.amount.div(100);
                    ...
                    if (property == 10){
                        count.tenCurrent -= 1;
                        count.pending10Val -= total;
                    }
                    if (property == 20){
                        count.twentyCurrent -= 1;
                        count.pending20Val -= total;
                    }
                    if (property == 30){
                        count.thirtyCurrent -= 1;
                        count.pending30Val -= total;
                    }
                    if (property == 40){
                        count.fourtyCurrent -= 1;
                        count.pending40Val -= total;
                    }
                }
            }
