...
contract Staking is Ownable, IERC721Receiver {
    ...
    uint256 private bronzeReward = 50 * 10 ** 18;
    uint256 private goldReward = 135 * 10 ** 18;
    ...
    function _harvestNormal(uint256 tokenId) internal {
        ...
        if (farmStartDate != 0 && farmStartDate <= block.timestamp) {
            ...
            if (nft.nftType == 0) {
                ...
                if (stakedSilver.length > 0 && userInfo[_msgSender()].stakedGold.length == 0 && pendingReward_ < 250 * 10 ** 18) {
    ...
    function _harvestUnstake(uint256 tokenId) internal {
        ...
        if (farmStartDate != 0 && farmStartDate <= block.timestamp) {
            ...
            if (nft.nftType == 0) {
                pendingReward_ = _pendingBronzeReward(tokenId);
                require(pendingReward_ >= 250 * 10 ** 18, '250 tokens were not farmed yet');
