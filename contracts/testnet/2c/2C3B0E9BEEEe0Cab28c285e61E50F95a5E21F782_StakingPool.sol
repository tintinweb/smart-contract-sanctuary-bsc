...
contract StakingPool{
    uint public totalLamaDeposited;
    ...
    struct user{
        uint lastLamaClaimTime;
        uint totalLamaStaked;
        uint lastCakeClaimTime; 
        uint totalCakeStaked; //same lama tokens but for those staking their cake
    ...
    modifier hasLamaStaked{
        require(stakingDetails[msg.sender].totalLamaStaked > 0,"You have not staked any tokens");
        _;
    }
    ...
    function depositLama(uint _amount) public{
        originalToken.transferFrom(msg.sender,address(this),_amount);
        stakingDetails[msg.sender].lastLamaClaimTime += block.timestamp;
        stakingDetails[msg.sender].totalLamaStaked += _amount;
        totalLamaDeposited += _amount;
    }
    ...
    function setLamaApy(uint _lamaApy) public onlyOwner{
        lamaApy = _lamaApy;
    }

    function calculateUserLamaEarnings() public view returns(uint){
        if(stakingDetails[msg.sender].totalLamaStaked > 0){
            uint timePassed = block.timestamp - stakingDetails[msg.sender].lastLamaClaimTime;
            uint currentPercentageReturns = (((timePassed * lamaApy) / dateRange) * stakingDetails[msg.sender].totalLamaStaked) / 100;
            return currentPercentageReturns;
        }
        return 0;
    }

    function calculateUserCakeEarnings() public view returns(uint){
        if(stakingDetails[msg.sender].totalCakeStaked > 0){
            uint timePassed = block.timestamp - stakingDetails[msg.sender].lastCakeClaimTime;
            uint currentPercentageReturns = (((timePassed * cakeApy) / dateRange) * stakingDetails[msg.sender].totalCakeStaked) / 100;
            return currentPercentageReturns;
        }
        return 0;
    }

    function compoundLamaStake() public hasLamaStaked {
        // get the current profits and add it to the stake
        uint currentPercentageReturns = calculateUserLamaEarnings();
        stakingDetails[msg.sender].totalLamaStaked += currentPercentageReturns;
        stakingDetails[msg.sender].lastLamaClaimTime += block.timestamp;
    }

    function compoundCakeStake() public hasCakeStaked {
        // get the current profits and add it to the stake
        uint currentPercentageReturns = calculateUserCakeEarnings();
        stakingDetails[msg.sender].totalCakeStaked += currentPercentageReturns;
        stakingDetails[msg.sender].lastCakeClaimTime += block.timestamp;
    }

    function withdrawLama(uint _amount) public hasLamaStaked {
        // withdrawing from staked amount and make the transfer
        require(_amount <= stakingDetails[msg.sender].totalLamaStaked, "Insufficient staking balance");
        stakingDetails[msg.sender].totalLamaStaked -= _amount;
        stakingDetails[msg.sender].lastLamaClaimTime += block.timestamp;
