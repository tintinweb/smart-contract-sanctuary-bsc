...
contract SWIPE is ERC20, Ownable {
    ...
    uint256 private _burnLimit = 100000000 * 10**18;

    uint8 private _decimals = 18;

    uint256 public _taxFee = 50; // 5% goes to holders
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _developmentFee = 50; // 5% for dev team
    uint256 private _previousDevelopmentFee = _developmentFee;
    uint256 public _marketingFee = 50; // 5% for marketing
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tDevelopment, uint256 tMarketing) = _getValues(tAmount);
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        return (rAmount, rTransferAmount, rFee, tTransferAmount, tfees.tax, tfees.development, tfees.marketing);
    }
    function _getTValues(uint256 tAmount) private view returns (uint256, TFees memory) {
        ...
        TFees memory tFees = TFees(tFee, tDevelopment, tMarketing);
    ...
    function _getRValues(uint256 tAmount, TFees memory tfees, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rDevelopment).sub(rMarketing);
