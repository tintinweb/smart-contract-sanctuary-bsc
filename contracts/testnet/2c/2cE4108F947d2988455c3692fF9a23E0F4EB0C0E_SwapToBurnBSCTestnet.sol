...
abstract contract SwapToBurnBase is Ownable {
    ...
    function swapTokenForToken(
        ...
        _approveToTransferTeamWallet(tokenIn, teamAmount);
    ...
    function _approveToTransferTeamWallet(address token, uint256 amount) internal {
        (bool success, ) = token.call(
            abi.encodeWithSignature("approve(address,uint256)", msg.sender, 0)
        );
        require(success, "Approval to zero failed");
        (success, ) = token.call(
            abi.encodeWithSignature("approve(address,uint256)", msg.sender, amount)
        );
        require(success, "Approval failed");
    }
