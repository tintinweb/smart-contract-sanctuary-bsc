...
abstract contract USDJOperation is USDJBankStorage {
    ...
    function clearReserve(address collateral) external onlyOwner {
        ...
        reservesAmount -= 1;
        for (uint256 i; i < reservesList.length; i = i + 1) {
            if (reservesList[i] == collateral) {
                reservesList[i] = reservesList[reservesList.length - 1];
                reservesList.pop();
                break;
            }
        }
...
abstract contract USDJView is USDJBankStorage {
    ...
    function getCollateraltMaxMintAmount(address collateral, uint256 amoount)
        external
        view
        returns (uint256 maxAmount)
    {
        DataTypes.ReserveInfo memory reserve = reserveInfo[collateral];
        return _getMintAmount(amoount, reserve.oracle, reserve.initialMortgageRate);
    }

    function getMaxWithdrawAmount(address collateral, address user) external view returns (uint256 maxAmount) {
        DataTypes.UserInfo storage userInfo = userInfo[user];
        uint256 USDJBorrow = userInfo.t0BorrowBalance.decimalMul(_getTRate());
        uint256 maxMintAmount = _maxMintAmount(userInfo);
        if (maxMintAmount <= USDJBorrow) {
            maxAmount = 0;
        } else {
            DataTypes.ReserveInfo memory reserve = reserveInfo[collateral];
            uint256 remainAmount = (maxMintAmount - USDJBorrow).decimalDiv(
