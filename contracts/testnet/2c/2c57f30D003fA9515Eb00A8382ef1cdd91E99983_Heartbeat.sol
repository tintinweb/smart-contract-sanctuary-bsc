...
import "./BaseBuildingBlockNonUpgradeable.sol";

contract Heartbeat is BaseBuildingBlockNonUpgradeable {

    event Heartbeat(uint256 newHeight);
    ...
    function setActionPool(address newActionPool) external onlyOwner {
        _actionPool = newActionPool;
    }

    function pulse(uint256 height) public onlySelf {
        relayerBlockHeight = height;

        emit Heartbeat(height);
    }
}
