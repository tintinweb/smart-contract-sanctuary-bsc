...
contract TradeExecutor is Ownable {
    ...
    function executeOrders(uint256[] calldata _orderIds, uint256[] calldata _swapOrderIds) external {
        for (uint256 i = 0; i < _orderIds.length;) {
            try orderManager.executeOrder(_orderIds[i], payable(address(this))) {} catch {}
            unchecked {
                ++i;
            }
        }
        for (uint256 i = 0; i < _swapOrderIds.length;) {
            try orderManager.executeSwapOrder(_orderIds[i], payable(address(this))) {} catch {}
            unchecked {
                ++i;
            }
        }
    ...
    function liquidate(address _account, address _indexToken, address _collateralToken, Side _side) external {
        pool.liquidatePosition(_account, _indexToken, _collateralToken, _side);
    }
    ...
    function withdraw(address _token, uint256 _amount, address _to) external onlyOwner {
        ...
        require(_amount <= IERC20(_token).balanceOf(address(this)), "Invalid amount");
