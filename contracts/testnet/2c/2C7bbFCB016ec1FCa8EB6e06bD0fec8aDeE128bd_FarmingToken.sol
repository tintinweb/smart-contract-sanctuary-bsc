...
contract FarmingToken is Ownable{
    ...
    uint256 private constant ACC_MAL_PRECISION = 1e12;
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accRewardPerShare = accRewardPerShare.add(rewardReward.mul(ACC_MAL_PRECISION) / lpSupply);
        }
        pending = int256(user.amount.mul(accRewardPerShare) / ACC_MAL_PRECISION).sub(user.rewardDebt).toUInt256();
    }
    ...
    function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                uint256 blocks = block.number.sub(pool.lastRewardBlock);
                uint256 rewardReward = blocks.mul(rewardPerBlock).mul(pool.allocPoint) / totalAllocPoint;
                pool.accRewardPerShare = pool.accRewardPerShare.add((rewardReward.mul(ACC_MAL_PRECISION) / lpSupply).to128());
            }
    ...
    function deposit(uint256 pid, uint256 amount, address to) public {
        ...
        user.rewardDebt = user.rewardDebt.add(int256(amount.mul(pool.accRewardPerShare) / ACC_MAL_PRECISION));
    ...
    function withdraw(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = user.rewardDebt.sub(int256(amount.mul(pool.accRewardPerShare) / ACC_MAL_PRECISION));
    ...
    function harvest(uint256 pid, address to) public {
        PoolInfo memory pool = updatePool(pid);
        UserInfo storage user = userInfo[pid][msg.sender];
        int256 accumulatedReward = int256(user.amount.mul(pool.accRewardPerShare) / ACC_MAL_PRECISION);
    ...
    function withdrawAndHarvest(uint256 pid, uint256 amount, address to) public {
        ...
        // Effects
        user.rewardDebt = accumulatedReward.sub(int256(amount.mul(pool.accRewardPerShare) / ACC_MAL_PRECISION));
        user.amount = user.amount.sub(amount);
