...
contract DividendDistributor is IDividendDistributor {
    ...
    // EARN
    IBEP20 RWRD = IBEP20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);    //RWRD address  BUSD TestNet
...
/* Token contract */
contract SUSREV is IBEP20, Auth {
    using SafeMath for uint256;

    // Addresses
    address RWRD = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;   //RWRD address
    address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;  //WBNB address
    address DEAD = 0x000000000000000000000000000000000000dEaD;  
    address ZERO = 0x0000000000000000000000000000000000000000;
    address VAULT = 0x499174CF158A0dc8B24B55Ca180C1ed58414a86d;   //VAULT address  VAULT
    ...
    // Name and symbol
    string constant _name = "SUSREV";
    string constant _symbol = "V_TOKEN";
    ...
    uint256 public totalFeeBuy = liquidityFeeBuy.add(reflectionFeeBuy).add(marketingFeeBuy).add(VAULTFeeBuy);
    ...
    uint256 public totalFeeSell = liquidityFeeSell.add(reflectionFeeSell).add(marketingFeeSell).add(VAULTFeeSell);
    // Sell fees   /* // 50% Sell tax on launch WILL BE ADJUSTED SOON AFTER // */
    ...
    constructor () Auth(msg.sender) {
        ...
        marketingFeeReceiver = 0x2DC79ae7b46e7B1d92724D499156e13c7b2Ef93D;  //XXXMARKETING ADDRESS
    ...
    function checkMaxWallet(address sender, address recipient, uint256 amount) internal view {
        if (!authorizations[sender] && recipient != owner && recipient != address(this)  && recipient != address(DEAD) && recipient != pair && recipient != marketingFeeReceiver && recipient != autoLiquidityReceiver && recipient != VAULT){
    ...
    function swapBack() internal swapping {
        ...
        (bool successVAULT, /* bytes memory data */) = payable(VAULT).call{value: amountBNBVAULT, gas: 30000}(""); 
    ...
    function triggerManualBuyback(uint256 amount) external authorized {
        ...
        uint256 amountToBuy = amountWithDecimals.div(10000);
    ...
    // Set our buy fees
    function setBuyFees(uint256 _liquidityFeeBuy, uint256 _reflectionFeeBuy, uint256 _marketingFeeBuy, uint256 _VAULTFeeBuy, uint256 _feeDenominator) external authorized {
        liquidityFeeBuy = _liquidityFeeBuy;
        reflectionFeeBuy = _reflectionFeeBuy;
        marketingFeeBuy = _marketingFeeBuy;
        VAULTFeeBuy = _VAULTFeeBuy;
        totalFeeBuy = _liquidityFeeBuy.add(_reflectionFeeBuy).add(_marketingFeeBuy).add(_VAULTFeeBuy);
        feeDenominator = _feeDenominator;
    }

    // Set our sell fees
    function setSellFees(uint256 _liquidityFeeSell, uint256 _reflectionFeeSell, uint256 _marketingFeeSell, uint256 _VAULTFeeSell, uint256 _feeDenominator) external authorized {
        liquidityFeeSell = _liquidityFeeSell;
        reflectionFeeSell = _reflectionFeeSell;
        marketingFeeSell = _marketingFeeSell;
        VAULTFeeSell = _VAULTFeeSell;
        totalFeeSell = _liquidityFeeSell.add(_reflectionFeeSell).add(_marketingFeeSell).add(_VAULTFeeSell);
        feeDenominator = _feeDenominator;
    }
