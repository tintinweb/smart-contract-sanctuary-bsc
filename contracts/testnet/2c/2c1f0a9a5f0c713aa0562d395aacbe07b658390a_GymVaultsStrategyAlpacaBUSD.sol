    ...
    PausableUpgradeable
{
    ...
    uint256 public accTotalClaimed;

    mapping (address => uint) userRewardDebt;

    uint256 public busdBalance;
    mapping (address => uint256) public userRewardAmount;
    address public alpacaRewardsPoolAddress;
    ...
    function setAlpacaRewardPoolAddress(address _alpacaRewardAddress) external onlyOwner {
        alpacaRewardsPoolAddress = _alpacaRewardAddress;
    }

    function setBusdBalance(uint256 _busdbalance) external onlyOwner {
        busdBalance = _busdbalance;
    }

    function setUserInfo(address _user, uint256 _shares, uint256 _rewardsClaimt, uint256 _rewardDebt) external onlyOwner {
        UserInfo storage user = userInfo[_user];
        user.shares = _shares;
        user.rewardsClaimt = _rewardsClaimt;
        user.rewardDebt = _rewardDebt;
    }

    function setPoolInfo(uint256 _lastChangeId, uint256 _accRewardPerShare) external onlyOwner {
    ...
    function setUserShares(address _user, uint256 _shares) external onlyOwner {
        UserInfo storage user = userInfo[_user];
        user.shares = _shares;
    }
    ...
    function _updatePool(uint256 _amount) private {
        uint256 lastChangeId = lastChangeId++;
    ...
    function setWantToEarnedPath(address _token0, address _token1) external onlyOwner {
        earnedToWantPath = [_token0, _token1];
    }

    function setRouterAddress(address _routerAddress) external onlyOwner {
        uniRouterAddress = _routerAddress;
    }
    ...
    function claimAlpacaRewards(address _user) external onlyGymVault {
        UserInfo storage user = userInfo[_user];
        if (busdBalance != 0) {
            _updatePool(busdBalance);
        }
        _claim(_user);
        user.rewardDebt = (user.shares * accRewardPerShare) / (1e18);
        busdBalance -= user.shares;
    }

    function setSharesTotal(uint256 _sharesTotal) external onlyOwner {
    ...
    function _swapAlpacaToWant() private {
        ...
        if (alpacaTokenAddress != wantAddress && earnedAmt > 0) {
            earnedAmt = earnedAmt - _distributeFee;
            IERC20Upgradeable(alpacaTokenAddress).safeIncreaseAllowance(uniRouterAddress, earnedAmt);
            busdBalance += IPancakeRouter02(uniRouterAddress).getAmountsOut(earnedAmt, earnedToWantPath)[1];
            // Swap half earned to token0
            IPancakeRouter02(uniRouterAddress)
