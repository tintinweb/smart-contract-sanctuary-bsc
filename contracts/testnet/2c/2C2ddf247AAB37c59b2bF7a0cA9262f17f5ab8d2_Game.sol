...
contract Game is Ownable, ReentrancyGuard {
    ...
    function reactivate(uint256 _sid) external nonReentrant {
        ...
        MBQ.safeTransferFrom(msg.sender, feeReceiver, _reactivateFee);

        _userPackageInfo.expiryTimestamp = block.timestamp + gracePeriod;

        emit Reactivate(msg.sender, _sid, activationFee);
    }
    ...
    event Reactivate(address userAddress, uint256 sid, uint256 activationFee);
