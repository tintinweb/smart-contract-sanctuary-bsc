...
abstract contract ReentrancyGuard {
 ...
 contract SafeVaultAndo is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        stakingToken = IBEP20(0x1Db2308Eda84929E0d4b18E90ebCa61C4790f69B);
        rewardToken = IBEP20(0x0820912778Cd194BEcEee28687FF52a44691211f);
    ...
    function startReward() external onlyOwner {
        //require(poolInfo[0].lastRewardTimestamp == 0, "Can only start rewards once");
    ...
    function deposit(uint256 _amount) public nonReentrant {
     require(totalStaked <= totalallowed, "Cannot withdraw other people's staked tokens.  Contact an admin."); 
    ...
    function withdraw() public nonReentrant {
        ...
        uint256 pending = user.amount.mul(pool.accTokensPerShare).div(1e12).sub(user.rewardDebt);
        if(pending > 0) {
            //require(pending <= _amount, "Cannot withdraw other people's staked tokens.  Contact an admin.");
            rewardToken.safeTransfer(address(msg.sender), pending);
        }
    ...
    function updatetokenamount(uint256 newtokenamount) external onlyOwner {
        require(newtokenamount <= 42000000000000000, "APY must be below 100%");
        updatePool(0);
       totalallowed = newtokenamount;
    }

     function updatelockduration(uint256 newlockDuration) external onlyOwner {
        require(newlockDuration <= 9676800, "Duration must be below 2 weeks");
