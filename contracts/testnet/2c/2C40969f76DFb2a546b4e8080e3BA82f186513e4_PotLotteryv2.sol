...
contract PotLotteryv2 is Initializable, ReentrancyGuardUpgradeable {
    ...
    modifier validBNBP() {
        require(BNBP_Address != address(0), 'BNBP Addr invalid');
        _;
    }

    //-----I added a new event
    event TransactionFail(address indexed userAddress, string tokenName);
     ...
     */
    function getTokenUsdValue(string memory _tokenName, uint256 _amount) public view returns (uint256 usdValue) {
        usdValue = ((tokenLatestPriceFeed[_tokenName] * _amount) / 10**tokenWhiteList[_tokenName].tokenDecimal);
    }

    /**   @dev returns the contract balance of a particular token
     * @param _tokenName the name of the token
     * @return balance contract balance of the token
     */
    function getContractBalance(string memory _tokenName) public view returns (uint256) {
        return IPRC20(tokenWhiteList[_tokenName].tokenAddress).balanceOf(address(this));
    }
    ...
    function attemptTransferFrom(
        ...
        try IPRC20(tokenWhiteList[_tokenName].tokenAddress).transferFrom(_userAddress, address(this), _amount) returns (bool) {
            return true;
        } catch (
    ...
    function addToken(
        ...
        tokenWhiteList[_tokenName] = Token(_tokenAddress, _swapToBNB, _tokenSymbol, _decimal);
    }
    ...
    modifier tokenInWhiteList(string memory _tokenName) {
        ...
        require(istokenWhiteListed, 'Not supported');
        _;
    }
    ...
    function EnterPot(
        ...
        for (uint256 index = 0; index < _tokenNames.length; index++) {
            if ((keccak256(bytes(_tokenNames[index])) == keccak256(bytes('BNB')))) {
                continue;
            }
            if ((getTokenUsdValue(_tokenNames[index], _amounts[index]) < minEntranceInUsd)) {
                continue;
            }
            if (
                getContractBalance(_tokenNames[index]) <=
    ...
    function enterPot(
        ...
        for (uint256 index = 0; index < _tokenNames.length; index++) {
            ...
            if (!attemptTransferFrom(_tokenNames[index], _participants[index], _amounts[index])) {
                continue;
            }
            _EnterPot(_tokenNames[index], _amounts[index], _participants[index]);
        }
    ...
    function getAmountToPayAsFees(address _address) internal view returns (uint256 amountToPay) {
        string memory tokenWithThehighestUsdValue = getPotTokenWithHighestValue();
        IPRC20 token = IPRC20(BNBP_Address);
        ...
        amountToPay = ((userBalance / BNBP_Standard) >= 1)
            ? baseFee / 2
            : (baseFee - ((userBalance / BNBP_Standard) * baseFee) / 2);
    }
    ...
    */
    function UpdatePrice() public nonReentrant {
        if (participantCount == 0) {
            for (uint256 index = 0; index < tokenWhiteListNames.length; index++) {
                (uint256 Res0, uint256 Res1) = _getTokenReserves(tokenWhiteList[tokenWhiteListNames[index]].tokenAddress, busdAddr);
                if (Res0 == 0 && Res1 == 0) {
                    ...
                    updateTokenUsdValue(tokenWhiteListNames[index], (price * 10**10) / 10**tokenWhiteList["BNB"].tokenDecimal * getBNBPrice());
                } else {
                    uint256 res1 = Res1 * (10**tokenWhiteList[tokenWhiteListNames[index]].tokenDecimal);
                    uint256 price = res1 / Res0;
                    updateTokenUsdValue(tokenWhiteListNames[index], (price * 10**10) / 10**tokenWhiteList["BUSD"].tokenDecimal);
                }
            }
        }
    }
    ...
    function _getTokenReserves(address token0, address token1) internal view returns (uint256, uint256) {
        IPancakeFactory factory = IPancakeFactory(pancakeswapV2FactoryAddr);
        IPancakePair pair = IPancakePair(factory.getPair(token0, token1));

        if (address(pair) == address(0)) {
            // IPancakePair token0TobnbPair = IPancakePair(factory.getPair(token0, router.WETH()));
            // IPancakePair bnbToToken1Pair = IPancakePair(factory.getPair(router.WETH(), token1));
            // if (address(token0TobnbPair) != address(0) && address(bnbToToken1Pair) != address(0)) {
            //     (uint256 res00, uint256 res01) = _getTokenReserves(token0, router.WETH());
            //     (uint256 res10, uint256 res11) = _getTokenReserves(router.WETH(), token1);

            //     return (res00, (res11 * res01) / res10);
            // } else {
            return (0, 0);
            // }
        }
    ...
    */
    function _payAccount(
        ...
        if (keccak256(bytes(_tokenName)) == keccak256(bytes('BNB'))) {
            sent = payable(_accountToPay).send(_tokenValue);
        } else {
            IPRC20 token = IPRC20(tokenWhiteList[_tokenName].tokenAddress);
            sent = token.transfer(_accountToPay, _tokenValue);
        }
        if (!sent) {
            emit TransactionFail(_accountToPay, _tokenName);
        }
    }
    ...
    function getBNBPrice() public view returns (uint256 price) {
        IPRC20 wethToken = IPRC20(wbnbAddr);
    ...
    function setHotWalletSettings(uint256 min, uint256 max) external onlyAdmin {
        require(min < max, 'Min not less than Max');
    ...
    function lotteryAccumulatedBNBP() external validBNBP returns (address) {
        ...
        require(BNBP_Balance > 0, 'No BNBP');
        require(lotteryPool > 0, 'No lottery amt');
        require(lotteryPool <= BNBP_Balance, 'Wrong BNBP Fee');
    ...
    function setTokenomicsPercentage(
        ...
        require(_airdrop + _lottery + _burn == 100, 'Should be 100 in total');
     ...
     */
    function setAdminFeeToken(string memory tokenName, uint256 value) external {
        uint256 i;
        for (i = 0; i < adminFeeToken.length; i++) {
            if (keccak256(bytes(adminFeeToken[i])) == keccak256(bytes(tokenName))) {
                break;
            }
        }
        if (i == adminFeeToken.length) {
            adminFeeToken.push(tokenName);
        }
        adminFeeTokenValues[tokenName] += value;
        if (keccak256(bytes(tokenName)) == keccak256(bytes('BNBP'))) {
    ...
    receive() external payable {
        ...
        require(
            (tokenLatestPriceFeed['BNB'] * msg.value) / 10**18 >= minEntranceInUsd,
            'less than min'
        );
