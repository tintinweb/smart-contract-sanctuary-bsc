...
contract Ticket is ITicket, ERC721, ERC721Enumerable, SafeTransfer, Ownable, Pausable {
	...
	event UpdateRound(uint256 roundCurrent);
	...
	function canClaimCard(address account) public view returns (bool claim) {
		if (whitelist[account].isMember) {
			claim = whitelist[account].claimed == false;
		} else {
			claim = true;
		}
	}
	...
	function setWhitelist(address[] memory _accounts, bool _isMember) public onlyOwner {
		require(_accounts.length > 0, "Ticket: invalid accounts");
		for (uint256 i = 0; i < _accounts.length; i++) {
			whitelist[_accounts[i]].isMember = _isMember;
		}
	}
	...
	function claimCard() public payable whenNotPaused {
		...
		if (whitelist[_msgSender()].isMember) {
			require(_tokenIdAirdrop <= SUPPLY_AIRDROP, "Ticket: airdrop ended");
			require(!whitelist[_msgSender()].claimed, "Ticket: claimed");
			require(msg.value >= cardPrice, "Ticket: price invalid");

			_safeMint(_msgSender(), _tokenIdAirdrop);
			...
			_safeMint(_msgSender(), _tokenIdSell);
			_tokenIdSell++;
		}
		emit ClaimCard(_msgSender(), whitelist[_msgSender()].isMember ? _tokenIdAirdrop-- : _tokenIdSell--);
	}
