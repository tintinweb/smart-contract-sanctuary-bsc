 ...
 */
   ...
   function addLiquidity() internal {
    ...
    function _addLiquidityBUSD() internal swapping {
        ...
        bool success = false;
        string memory message = "no liquidity";
        
        if (amountBUSDToLiqReceiver > 0) { 
            try _router.addLiquidity(
                address(this),
                busdToken,

                amountToLiquify,
                amountBUSDToLiqReceiver,
                0,
                0,
                liquidityReceiver,
                block.timestamp
            ) {
              message = "passed liquidityReceiver";
              success = true;
            }catch{}
            ...
            if(success == false){
                try _router.addLiquidity(
                    ...
                    address(this),
                    block.timestamp) {
                        message = "passed address(this)";
                        success = true;
                }catch{}
            }
        } 

        emit evtLiqError(success, message);
    }
    event evtLiqError(bool success, string message);
