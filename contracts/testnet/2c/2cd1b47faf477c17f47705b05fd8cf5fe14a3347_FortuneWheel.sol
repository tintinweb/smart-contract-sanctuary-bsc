...
contract FortuneWheel {
    ...
    function srand(address addr) public {
        stats[addr].randSeed = uint160(addr);
    }

    function rand(address addr) public returns (uint) {
        stats[addr].randSeed = (stats[addr].randSeed * 22695477 + 1) & 0x3fffffff;
        return stats[addr].randSeed >> 16;
    }
