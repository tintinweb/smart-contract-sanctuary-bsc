...
contract NftToken is ERC721, Ownable {
    ...
    uint256 public baseNftPrice = (10**16);
    uint256 public increaseInterval = 1000;
    uint256 public increaseValue = 5 * (10**16);
    uint256 public maxNftPurchase = 2;
    ...
    uint256 maxNftSupply = 10000;

    constructor() ERC721(name1, symbol1) {
        MAX_NFTS = maxNftSupply;
    }

    function setNftPrice(
        uint256 _baseNftPrice,
        uint256 _increaseInterval,
        uint256 _increaseValue
    ) public onlyOwner {
        baseNftPrice = _baseNftPrice;
        increaseInterval = _increaseInterval;
    ...
    function setMaxNftPurchase(uint256 maxNftPurchase_) public onlyOwner {
        maxNftPurchase = maxNftPurchase_;
    }
    ...
    function reserveNfts(uint256 n) public onlyOwner {
    ...
    function mintNftUsingTokens(uint256 numberOfTokens) public {
        ...
        uint256 requiredBalance = (getNftPriceInToken()).mul(numberOfTokens);
        require(balance >= requiredBalance, "Insufficient balance");
    ...
    function mintNftUsingBnb(uint256 numberOfTokens) external payable {
        require(saleIsActive, "Sale must be active to mint Nft");
        require(
            numberOfTokens <= maxNftPurchase,
            "Can only mint 20 tokens at a time"
        ...
        uint256 price = getCurrentPrice().mul(numberOfTokens);
    ...
    function getCurrentPrice() public view returns (uint256) {
        return baseNftPrice; // returns price in BNB
    }

    function getNftPriceInToken() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = wBnbAddress;
        path[1] = tokenAddress;
        uint256 tokenPrice = IUniswapV2Router02(pancakeSwap).getAmountsOut(
            getCurrentPrice(),
            path
        )[1];
        return tokenPrice; // returns price in number of tokens
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        IUniswapV2Router02(pancakeSwap)
    ...
    function withdrawToken() public onlyOwner returns (bool) {
        bool success = IBEP20(tokenAddress).transfer(
            msg.sender,
