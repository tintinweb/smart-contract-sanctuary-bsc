...
contract StableSwap is Ownable, ReentrancyGuard {
    ...
    // uint256[N_COINS] public PRECISION_MUL = [1, 1e10];
    // uint256[N_COINS] public RATES = [1e18, 1e28];
    uint256[N_COINS] public PRECISION_MUL;
    uint256[N_COINS] public RATES;

    uint256 public constant MAX_ADMIN_FEE = 1e9;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < N_COINS; i++) {
            ...
            require( coinDecimal <= 18 , "The maximum decimal cannot exceed 18");
            //set PRECISION_MUL and  RATES
            PRECISION_MUL[i] = 10**(18 - coinDecimal);
    ...
    function get_D_mem(uint256[N_COINS] memory _balances, uint256 amp) private view returns (uint256) {
...
interface IERC20m {
    ...
    function set_miner(address miner) external;
}
