...
abstract contract EnableTokensClaimable is Context {
   ...
   function tokensClaimableEnabled() public view virtual returns (bool) {
       return _enabled;
   }
 
   modifier whenTokensClaimableNotEnabled() {
       require(tokensClaimableEnabled(), "EnableTokensClaimable: enabled");
       _;
   }
 
   modifier whenTokensClaimableEnabled() {
       require(!tokensClaimableEnabled(), "EnableTokensClaimable: not enabled");
       _;
   }
...
contract ETMStaking is WhitelistedRole, Pausable, EnableTokensClaimable, LockStaking, ReentrancyGuard {
    ...
    event Deposit(address indexed from, address indexed to, uint256 indexed stakingSlot, uint256 amount);
    event Withdraw(address indexed from, address indexed withdrawer, uint256 fund, uint256 reward, uint8 indexed stakingSlot);
    ...
    uint256 public rewardPool; // giới hạn reward trong pool
    uint256 public poolLimit; // giới hạn pool
    uint256 public interestRewardPeriod; // số ngày trả lãi cố định
    uint256 public minInterestRewardPeriod; // số ngày trả lãi tối thiểu
    uint8 public fixedAPR; // lãi suất năm
    uint8 public unstakeFeeBeforeEnd; // phía unstake sớm
    uint8 public stakingThreshold; // số lần stake tối đa
    uint256 public maxQuantity; // lượng stake tối đa
    uint256 public minQuantity; // lượng stake tối thiểu
    ...
    mapping(address => uint256) private _numUserDeposits;
    ...
    constructor (
        ...
        uint256 rewardPool_,
        uint256 poolLimit_,
        uint256 interestRewardPeriod_,
        uint256 minInterestRewardPeriod_,
        uint8 fixedAPR_,
        uint8 unstakeFeeBeforeEnd_,
        ...
        interestRewardPeriod = interestRewardPeriod_;
        minInterestRewardPeriod = minInterestRewardPeriod_;
        fixedAPR = fixedAPR_;
        unstakeFeeBeforeEnd = unstakeFeeBeforeEnd_;
    ...
    function stake(uint256 depositAmount) public whenNotPaused whenNotStakingLocked {
        ...
        require(_userDeposits[_msgSender()].length < stakingThreshold, "ETMStaking: You can't stake more");
    ...
    function unStakes(uint8[] memory indexes) public whenNotPaused whenTokensClaimableEnabled nonReentrant {
        for (uint8 i = 0; i < indexes.length; i++) {
            require(_userDeposits[_msgSender()][indexes[i]].amount > 0, "ETMStaking: Nothing to withdraw at this index");
    ...
    function unStake(uint8 index) public whenNotPaused whenTokensClaimableEnabled nonReentrant {
        require(_userDeposits[_msgSender()][index].amount > 0, "ETMStaking: Nothing to withdraw at this index");
    ...
    function addReward() public whenNotPaused whenNotStakingLocked onlyWhitelistAdmin {
        require(IERC20(etmToken).balanceOf(address(this)) >= userFunds + stakingFunds, "ETMStaking: no rewards added");
        uint256 rewardAdded = IERC20(etmToken).balanceOf(address(this)) - (userFunds + stakingFunds);
        stakingFunds += IERC20(etmToken).balanceOf(address(this)) - (userFunds + stakingFunds);
        rewardPool += rewardAdded;
        emit RewardAdded(_msgSender(), rewardAdded);
    }

    function revokeReward(uint256 amount) public whenNotPaused whenNotStakingLocked onlyWhitelistAdmin {
    ...
    function getAllReward(address account, uint256 timestamp) public view returns(uint256) {
        uint256 totalReward;
        for (uint8 i = 0; i < uint8(_userDeposits[_msgSender()].length); i++) {
            totalReward += getReward(account, timestamp, i);
        }
        return totalReward;
    }

    function getReward(address account, uint256 timestamp, uint8 index) public view returns(uint256) {
        ...
        if (period >= interestRewardPeriod*DAY_IN_SECONDS) {
            return depositAmount*(fixedAPR/100*(interestRewardPeriod*DAY_IN_SECONDS/YEAR_IN_SECONDS));
        } else if (period >= minInterestRewardPeriod*DAY_IN_SECONDS && period < interestRewardPeriod*DAY_IN_SECONDS) {
            return depositAmount*(fixedAPR/100*(period*DAY_IN_SECONDS/YEAR_IN_SECONDS))*(1 - unstakeFeeBeforeEnd/100);
        } else return 0;
    }

    function updatePoolLimit(uint256 value) public whenNotPaused whenNotStakingLocked onlyWhitelistAdmin {
        poolLimit = value;
        emit PoolLimitChanged(_msgSender(), value);
    }

    function updateStakingThreshold(uint8 value) public whenNotPaused whenNotStakingLocked onlyWhitelistAdmin {
        stakingThreshold = value;
        emit StakingThresholdChanged(_msgSender(), value);
    }

    function updateMaxQuantity(uint256 value) public whenNotPaused whenNotStakingLocked onlyWhitelistAdmin {
        maxQuantity = value;
        emit MaxQuantityChanged(_msgSender(), value);
    }

    function updateMinQuantity(uint256 value) public whenNotPaused whenNotStakingLocked onlyWhitelistAdmin {
        minQuantity = value;
    ...
    function getUserDepositAtIndex(address account, uint8 index) public view returns(uint256,uint256) {
