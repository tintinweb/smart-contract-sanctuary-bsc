...
import "./commons/NFTBase.sol";

contract NFTBox is NFTBase {
    constructor() NFTBase("Kris Box", "KRISBOX") {}
}
...
contract NFTBase is ERC721, ERC721Enumerable, Pausable, ERC721Burnable, Ownable, INFT {
  using SafeMath for uint256;
  // only allow genesis minter to mint at most 10K NFT
  uint64 public MAX_GENESIS_SUPPLY = 10000;
  address public override genesisMinter;
  address public override genesisMarket;
  ...
  modifier onlyGenesisMinter() {
    require(genesisMinter == _msgSender() || genesisMarket == _msgSender(), "AmaERC721: caller is not genesis minter");
    _;
  }
  ...
  /**
   * @dev set genesis market to a new address.
   * Can only be called by the current owner.
   * @param newMarket the new genesis minter
   */
  function setGenesisMarket(address newMarket) external onlyOwner {
    genesisMarket = newMarket;
  }
  ...
  function mintGenesis(uint256 amount, address to, uint256 unitPrice) external override onlyGenesisMinter {
    _mintGenesis(amount, to, unitPrice);
  }

  function mintTo(address to, uint256 tokenId) external override onlyGenesisMinter {
    _safeMint(to, tokenId);
  }

  function _mintGenesis(uint256 amount, address to, uint256 unitPrice) internal {
    uint256 startId = currentId;
    require( 
      startId + amount <= MAX_GENESIS_SUPPLY,
      "AmaERC721: max genesis supply reached"
    );
   ...
   */
  function randomIndex(address sender) external override onlyGenesisMinter {
    randomizedStartIndex = randomizedStartIndex.add(uint256(
      keccak256(abi.encodePacked(blockhash(block.number), sender))
    ).mod(1 ether));
  }
  /**
  ...
  {
    require(tokenId <= currentId && tokenId > 0, "AmaERC721: invalid token id");
...
interface INFT is IERC721Enumerable {
   ...
   * @dev Call only by the Genesis Minter to roll the start index
   */
  function randomIndex(address sender) external;
   ...
   *  Can mint up to MAX_GENESIS_SUPPLY tokens
   * @param to recipient of genesis tokens
   * @param tokenId tokenId of genesis tokens to mint
   */
  ...
  /**
   * @dev Return the current genesis market address
   */
  function genesisMarket() external view returns (address);
  ...
  /**
   * @dev Return the base URI for tokens
   */
