...
contract MVP_DEELANCE {
    ...
    event Dispute_solved(uint256 ID, uint256 time, uint256 FLAG);
    ...
    function resolve_dispute (uint256 ID, uint256 FLAG) external {
    Transaction storage transaction = transactions[ID];
    ...
    transaction.dispute = 0;
    BUSD.safeTransfer((transaction.client), transaction.escrow);
    transaction.complete = 1;
    }
    
    emit Dispute_solved(ID, block.timestamp, FLAG);
    }
