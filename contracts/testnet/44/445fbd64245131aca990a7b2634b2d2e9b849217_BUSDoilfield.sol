...
interface IERC20 {
    ...
    event Transfer(address indexed from, address indexed to, uint256 recipient);
    event Approval(address indexed owner,address indexed spender,uint256 recipient);
}
...
contract BUSDoilfield {
	...
	uint256 constant public TIMER_FEE2 = 100;
	...
	address payable public timerWallet;
	...
	function invest(address referrer, uint8 plan, uint256 amounts) public  {
		...
		uint256 fee2 = amounts.mul(TIMER_FEE2).div(PERCENTS_DIVIDER);
        token.transfer(commissionWallet,fee);
        token.transfer(timerWallet,fee2);
