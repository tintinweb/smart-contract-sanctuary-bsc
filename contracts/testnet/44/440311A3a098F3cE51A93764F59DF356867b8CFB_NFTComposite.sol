...
import "./../utils/Contract_.sol";
import "./../config/IConfig.sol";
import "./INFT.sol";

// ERC721
// ERC721A
contract NFTComposite is ERC721, INFT, Caller, Fallback {
    ...
    mapping(uint256 => bytes) private property;
    ...
    constructor(
        ...
        _init(config_, child_, baseURI_, name__, symbol__);
    }
    ...
    function _init(
        ...
        child = child_;
        baseURI = string(
            abi.encodePacked(baseURI_, string(Contract_.getThisAddress()))
        );
        name_ = name__;
        symbol_ = symbol__;
    }
    ...
    function batchMint(
        ...
        require(maxId.add(1, "1168") == startTokenId, "1174");
        require(0 != property_.length, "1080");
        uint256 id = maxId;
        bytes memory property__;
        address to = config.getMintBoxNFT();
        for (uint256 i = 0; i < property_.length; i++) {
            property__ = property_[i];
            require(0 == property__.length % 4, "1085");
            id = id.add(1, "1091");
            property[id] = property__;
            _safeMint(to, id);
        }
        maxId = id;
        if (address(0) != address(token)) {
            token.mint(to, property_.length);
        }
    }
    ...
    function mint(
        ...
        require(0 == property_.length % 4, "1161");
        maxId = maxId.add(1, "1091");
        property[maxId] = property_;
        creator[maxId] = to;
        _safeMint(to, maxId);
        if (address(0) != address(token)) {
            token.mint(to, 1);
        }
        emit MintEvent(maxId, property_);
    }

    function burn(uint256 tokenId) external override {
        require(_isApprovedOrOwner(msg.sender, tokenId), "1082");
        delete property[tokenId];
        delete creator[tokenId];
        _burn(tokenId);
    }

    function isSplit() external pure override returns (bool) {
        return true;
    }
    ...
    function getCreator(uint256 tokenId)
        ...
        returns (address)
    {
        address creator_ = creator[tokenId];
        if (address(0) == creator_) {
            creator_ = config.getNFTCreator();
        }
        return creator_;
    }

    function getParent() external pure override returns (INFT) {
        return INFT(address(0));
    }

    function getChild() external view override returns (INFT) {
        return child;
    }

    function getMaxId() external view override returns (uint256) {
        return maxId;
    }
...
library Contract_ {
    function getThisAddress() internal view returns (bytes memory) {
        bytes memory alphabet = "0123456789abcdef";
        bytes memory data = abi.encodePacked(address(this));
        bytes memory contract_ = new bytes(3 + data.length * 2);
        contract_[0] = "0";
        contract_[1] = "x";
        contract_[contract_.length - 1] = "/";
        for (uint256 i = 0; i < data.length; i++) {
            contract_[2 + i * 2] = alphabet[uint256(uint8(data[i] >> 4))];
            contract_[3 + i * 2] = alphabet[uint256(uint8(data[i] & 0x0f))];
        }
        return contract_;
    }
}
...
interface INFT is IERC721 {
    event MintEvent(uint256 indexed orderId, bytes indexed property);
