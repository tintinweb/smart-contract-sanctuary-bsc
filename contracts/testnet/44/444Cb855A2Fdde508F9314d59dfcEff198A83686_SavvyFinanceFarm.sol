...
contract SavvyFinanceFarm is Ownable, AccessControl {
    ...
    function depositRewardToken(address _reward_token, uint256 _amount)
        public
        onlyRole(toRole(_reward_token))
    {
        require(tokenExists(_reward_token), "Token does not exist.");
        require(_amount > 0, "Amount must be greater than zero.");
        require(
            IERC20(_reward_token).balanceOf(_msgSender()) >= _amount,
            "Insufficient token balance."
        );
        IERC20(_reward_token).transferFrom(
            _msgSender(),
            address(this),
            _amount
        );
        tokensData[_reward_token].rewardBalance += _amount;
    }
