...
contract HVSPresale is Context, ReentrancyGuard, Ownable {
    ...
    uint256 public constant PRESALE_DURATION = 20 days;
    ...
    function getTokenPrice() public view returns (uint256) {
        if(block.timestamp < presale_info.presale_start)
            ...
            return 3;
        else if((block.timestamp >= (presale_info.presale_start + 1 days)) && (block.timestamp < (presale_info.presale_start + 3 days)))
            return 5;
        else if((block.timestamp >= (presale_info.presale_start + 3 days)) && (block.timestamp < (presale_info.presale_start + 7 days)))
            return 8;
        else if((block.timestamp >= (presale_info.presale_start + 7 days)) && (block.timestamp < (presale_info.presale_start + PRESALE_DURATION)))
            return 30;
        return 0;
    }
