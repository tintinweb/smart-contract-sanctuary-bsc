...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            uint256 feeAmount = 0;
            if (depositFee > 0) {
                feeAmount = _amount.mul(depositFee).div(100);
            }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (pending > 0) {
            uint256 feeAmount = 0;
            if (harvestFee > 0) {
                feeAmount = pending.mul(harvestFee).div(100);
                rewardToken.safeTransfer(address(opAccount), feeAmount);
            }
            rewardToken.safeTransfer(address(msg.sender), pending.sub(feeAmount));
        }
    ...
    function setDepositFee(uint256 _depositFee) external onlyOwner {
        require(_depositFee < 100, "_depositFee is too big");
        depositFee = _depositFee;
    }
    
    function setHarvestFee(uint256 _harvestFee) external onlyOwner {
        require(_harvestFee < 100, "_harvestFee is too big");
