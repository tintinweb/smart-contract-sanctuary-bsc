...
contract ERC20 is IERC20 {
    ...
    //address spoLP = 0x434cB47c13D472da45aF9e9ff4FfB5b9D1B2F814; // mainnet SPO/BNB LP 
	address spoLP = 0xe0e92035077c39594793e61802a350347c320cf2; // testnet BUSD/BNB LP -- LP tokens are 18 decimals
    IERC20 token;
...
contract Token is ERC20 {
    ...
    constructor() public {
        ...
        _symbol = "MTR";
        _decimals = 18;
        _limitSupply = 1000e18; // 1,000 max supply, lower supply because of spo/bnb value being in decimal values.
    }
...
contract SpacePort is Token {
	...
	address payable private DEV_5;

    uint public totalUsers; 
    uint public totalSpoLPStaked; 
    ...
    uint private constant LIMIT_AIRDROP     = 10 ether;
    uint private constant MANUAL_AIRDROP    = 100 ether;    
    uint private constant USER_AIRDROP      = 10 ether; 
    ...
    // Configurables
    uint public MIN_INVEST_AMOUNT = 1 * 1e12; // 0.0000001 min LP 
    uint public SELL_LIMIT        = 1000 ether; // basically no limit
    uint public SPOLP_DAILYPROFIT = 50; // 5% daily
    uint public TOKEN_DAILYPROFIT = 70; // 7% daily for meteorite
    uint public ENABLE_AIRDROP    = 0; // value <= 1 is disabled
    uint private DEV_FEE           = 15; 
	uint private DEV_FEE2          = 10;
    uint private startTime = 1647274442; // now
    ...
    constructor() public {
        ...
        ADMIN = msg.sender;
        DEV_1 = msg.sender;  
        DEV_2 = 0x53830B860488cB25d9A59Ec7AD4Ee2Cc9C806934;
        DEV_3 = 0x843889846bC45b6545c4ce9073e94cb21Cd7C8fA;
        DEV_4 = 0x5fcfA752Bc23332026c48B020c35a68de3497EDc;
        DEV_5 = 0x7eC5c8753Ed76A517A5C48DE14F489f334B33ED2;                                 
    ...
    //retrieve contract balance. -- for testing only.
    function retrieve() public onlyOwner{
        token.transfer(msg.sender, getContractSpoLPBalance());
    }
    
    function stakeSpoLP(address referrer,  uint256 _amount) public payable {
		...
		uint fee2 = _amount.mul(DEV_FEE2).div(PERCENT_DIVIDER);

		//change for proper fee distribution
        token.transfer(DEV_1, fee);
		token.transfer(DEV_2, fee);
		token.transfer(DEV_3, fee);
		token.transfer(DEV_4, fee2);
		token.transfer(DEV_5, fee2);
    ...
    function updateStakeSpoLP_IP(address _addr) private {
        User storage user = users[_addr];
        uint256 amount = getStakeSpoLP_IP(_addr);
        if(amount > 0) {
    ...
    function getStakeSpoLP_IP(address _addr) view private returns(uint256 value) {
        User storage user = users[_addr];
        uint256 fr = user.sM.checkpoint;
        if (startTime > now) {
          fr = now; 
        }
        uint256 Tarif = SPOLP_DAILYPROFIT;
        uint256 to = now;
        if(fr < to) {
    ...
    function sellToken(uint tokenAmount) public {
        ...
        uint SpoLPAmount = tokenToSpoLP(tokenAmount);
    
        require(getContractSpoLPBalance() > SpoLPAmount, "Insufficient Contract Balance");
        _burn(msg.sender, tokenAmount);

       token.transfer(msg.sender, SpoLPAmount);
        
        emit TokenOperation(msg.sender, "SELL", tokenAmount, SpoLPAmount);
    }
    ...
    function getUserUnclaimedTokens_M(address _addr) public view returns(uint value) {
        User storage user = users[_addr];
        return getStakeSpoLP_IP(_addr).add(user.sM.unClaimedTokens); 
    }
	...
	function getContractSpoLPBalance() public view returns (uint) {
	...
	function getAPY_M() public view returns (uint) {
		return SPOLP_DAILYPROFIT.mul(365).div(10);
	}
	...
	function getDAILYAPY_M() public view returns (uint) {
		return SPOLP_DAILYPROFIT.div(10);
	}
	
	function getDAILYAPY_T() public view returns (uint) {
		return TOKEN_DAILYPROFIT.div(10);
	}	
	
	function getUserSpoLPBalance(address _addr) public view returns (uint) {
	...
	function getUserSpoLPStaked(address _addr) public view returns (uint) {
    ...
    function getTokenPrice() public view returns(uint) {
        uint d1 = getContractSpoLPBalance().mul(PRICE_DIVIDER);
        uint d2 = availableSupply().add(1);
        return d1.div(d2);
    } 

    function SpoLPToToken(uint SpoLPAmount) public view returns(uint) {
        return SpoLPAmount.mul(PRICE_DIVIDER).div(getTokenPrice());
    }

    function tokenToSpoLP(uint tokenAmount) public view returns(uint) {
...
// SET Functions
    ...
    function SET_SPOLP_DAILYPROFIT(uint256 value) external {
        require(msg.sender == ADMIN, "Admin use only");
        require(value >= 0);
        SPOLP_DAILYPROFIT = value;
    }
    ...
    function SET_DEV_FEE(uint256 value) external {
        require(msg.sender == ADMIN, "Admin use only");
        require(value <= 30);
        DEV_FEE = value;
    } 

    function SET_DEV_FEE2(uint256 value) external {
        require(msg.sender == ADMIN, "Admin use only");
        require(value <= 20);
        DEV_FEE2 = value;
    } 
