...
contract CaVaMarcher is Ownable, ERC20 {
	using SafeMath for uint256;

	IUniswapV2Router02 public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name = "CaVaMarcher";
	string private constant _symbol = "CVM";
	uint8 private constant _decimals = 18;

	CaVaMarcherDividendTracker public dividendTracker;
	...
	uint256 public maxWalletAmount = 1000000 * (10**18); //1 000 000

	// max buy and sell tx is initialSupply
	uint256 public maxTxAmount = 500000 * (10**18); //500 000
	...
	address public dividendToken = 0xDAcbdeCc2992a63390d108e8507B98c7E2B5584a; //BNB
	...
	constructor() public ERC20(_name, _symbol) {
		dividendTracker = new CaVaMarcherDividendTracker();
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "CaVaMarcher: The dividend tracker already has that address");
		CaVaMarcherDividendTracker newDividendTracker = CaVaMarcherDividendTracker(payable(newAddress));
		require(newDividendTracker.owner() == address(this), "CaVaMarcher: The new dividend tracker must be owned by the CaVaMarcher token contract");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "CaVaMarcher: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "CaVaMarcher: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "CaVaMarcher: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "CaVaMarcher: Account is already the value of 'excluded'");
	...
	function setCropTax(uint256 newPenaltyFactor, uint256 newNumerator, uint256 newDenominator) public onlyOwner {
		require(newDenominator > 0, "CaVaMarcher: Denominator cannot be 0");
	...
	function setWallets(address newLiquidityWallet, address newMarketingWallet) public onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "CaVaMarcher: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "CaVaMarcher: The marketingWallet cannot be 0");
	...
	function setUniswapRouter(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "CaVaMarcher: The router already has that address");
	...
	function setGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue != gasForProcessing, "CaVaMarcher: Cannot update gasForProcessing to same value");
	...
	function setMaxTransactionAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxTxAmount, "CaVaMarcher: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxWalletAmount, "CaVaMarcher: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) public onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "CaVaMarcher: Cannot update minimumTokensBeforeSwap to same value");
	...
	function setDividendToken(address newDividendToken) external onlyOwner {
		require(newDividendToken != dividendToken, "CaVaMarcher: Cannot update dividend token to same value");
		require(newDividendToken != address(0), "CaVaMarcher: The dividend token cannot be 0");
		require(newDividendToken != address(this), "CaVaMarcher: The dividend token cannot be set to the current contract");
	...
	function claimBNBOverflow(uint256 amount) external onlyOwner {
		require(amount < address(this).balance, "CaVaMarcher: Cannot send more than contract balance");
	...
	function _transfer(
		...
		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "CaVaMarcher: Trading is currently disabled.");
			if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
				require(amount <= maxTxAmount, "CaVaMarcher: Buy amount exceeds the maxTxBuyAmount.");
			}
			if (!_isExcludedFromMaxWalletLimit[to]) {
				require(balanceOf(to).add(amount) <= maxWalletAmount, "CaVaMarcher: Expected wallet amount exceeds the maxWalletAmount.");
			}
		}
...
contract CaVaMarcherDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("CaVaMarcher_Dividend_Tracker", "CaVaMarcher_Dividend_Tracker") {
		claimWait = 3600;
	...
	function _transfer(address, address, uint256) internal override {
		require(false, "CaVaMarcher_Dividend_Tracker: No transfers allowed");
	}
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "CaVaMarcher_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
		minimumTokenBalanceForDividends = newValue;
	}
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "CaVaMarcher_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "CaVaMarcher_Dividend_Tracker: Cannot update claimWait to same value");
