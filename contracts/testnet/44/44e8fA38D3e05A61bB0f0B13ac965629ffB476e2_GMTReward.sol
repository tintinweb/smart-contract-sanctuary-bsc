...
contract GMTReward is Context, Ownable {
    ...
    address public gmtAddress = address(0x32FE68E9b7b521fa7F1E1C0ccC049034E26c0389);
    ...
    constructor() {
        _router = IPancakeRouter02(address(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3));
    ...
    function setGst (address _contract) external onlyOwner {
        gst = IERC20(_contract);
        gstAddress = _contract;
    }

    function takeReward () external {
        uint256 balance = gst.balanceOf(address(this));
    ...
    function syncMainnetPair () public {
        ...
        gmt.transfer(swapPair, gmt.balanceOf(address(this)));
    ...
    function _takeCommunityReward (
        ...
        for (uint256 i = 0; i < communityAddr.length; i++) {
            if (i == communityAddr.length.sub(1)) {
                ...
                gst.transfer(communityAddr[i], communityRewardAmount);
            } else {
                gst.transfer(communityAddr[i], communityRewardAmount);
            }
    ...
    function _takeSuperNodeReward (
        ...
        for (uint256 i = 0; i < superNodeAddr.length; i++) {
            if (i == superNodeAddr.length.sub(1)) {
                ...
                gst.transfer(superNodeAddr[i], superNodeFeeAmount);
            } else {
                gst.transfer(superNodeAddr[i], superNodeFeeAmount);
            }
    ...
    function _takeMarketingReward (
        ...
        for (uint256 i = 0; i < marketingAddr.length; i++) {
            if (i == marketingAddr.length.sub(1)) {
                ...
                gst.transfer(marketingAddr[i], marketingFeeAmount);
            } else {
                gst.transfer(marketingAddr[i], marketingFeeAmount);
            }
