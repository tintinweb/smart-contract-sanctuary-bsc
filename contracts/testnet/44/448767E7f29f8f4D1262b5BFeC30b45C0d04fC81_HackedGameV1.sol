...
contract HackedGameV1 is HackedGameBase, StakingTierProvider, VRFConsumerBaseV2, Recoverable, Pausable {
    ...
    function startGame(uint256 startAt) external onlyOwner isInitialized {
        (
    ...
    function drawRound(uint256 tokenId) external whenNotPaused {
        ...
        require(roundId == 0 || block.timestamp > roundStartedAt + roundDuration, "Round no finished");
    ...
    function completeGame() external whenNotPaused {
        ...
        emit GameComplete(gameId, tokenId, winner, prize);
    }
...
contract HackedGameBase is HackedGameState, RandomSequence {
    ...
    function _canReenter(uint256 tokenId, uint256 reentriesMax) internal view returns (bool, string memory) {
        (
            uint256 gameId,
            ,
            uint256 pattern,
        ...
        if (symbols == 0) return (false, "No symbols left");
        ...
        if(_matches(_symbols, patternMask, pattern)) return (false, "Token not hacked");
        if(!_matches(_symbols, patternMask ^ lastPatternMask, pattern)) return (false, "Too late");

        reentriesUsed = _gameId == gameId ? reentriesUsed+1 : 1;
        if(reentriesUsed > reentriesMax) return (false, "Max reentries reached");

        symbols = _nextSymbols(symbols, pattern, patternMask);
        if(!_matches(symbols, patternMask, pattern)) return (false, "No symbols left");
...
interface IStakingTierProvider {
    function getTier(address user) external;
}
