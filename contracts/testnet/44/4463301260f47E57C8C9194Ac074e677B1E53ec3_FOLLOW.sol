    ...
    function _transfer(
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && !swapping){
            if (from == uniswapV2Pair) {
                ...
                uint256 liquidityAmount = amount.mul(_buyLiquidityFee).div(1000).mul(currentRate);
            ...
            if (to == uniswapV2Pair) {
                ...
                uint256 liquidityAmount = amount.mul(_sellLiquidityFee).div(1000).mul(currentRate);
                _rOwned[address(this)] = _rOwned[address(this)] + liquidityAmount.mul(currentRate);
            }
        } 
