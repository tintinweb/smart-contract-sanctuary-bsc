...
contract DepositDistributor is ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    function deposit(
        uint256 amount
    ) external payable onlyAllowList nonReentrant {
    ...
    function distribute() private nonReentrant {
    ...
    AccessControl
{
    ...
    mapping(uint256 => bool) private requestIdFulfilled;
    uint public randomResult;

    address payable public depositDistributorAddress;
    ...
    // address private constant vrfCoordinatorV2Address = address(0x6A2AAd07396B36Fe02a22b33cf443582f682c82f);
    // VRFCoordinatorV2Interface public constant vrfCoordinatorV2 = VRFCoordinatorV2Interface(vrfCoordinatorV2Address);
    ...
    bool internal upkeepPerformed;
    bool internal upkeepRequired;
    ...
    constructor(
        ...
        upkeepRequired = false;
        interval = _updateInterval;
        lastTimeStamp = block.timestamp;
        priceFeed = AggregatorV3Interface(_priceFeed);
        entryFeeInUSD = 3.33 * 1e18;
        commissionPercentage = 10;
        vrfCoordinatorV2Address = address(_vrfCoordinatorV2Address);
        keyHash = _keyHash;
    ...
    function performUpkeep(
        ...
        require(
            ...
            "performUpkeep: performData must be non-empty"
        );
        upkeepPerformed = true;
        checkPlayersTrue();

        emit UpkeepPerformed(gameId);
    }

    function checkPlayersTrue() internal nonReentrant {
    ...
    function fulfillRandomWords(
        ...
        require(_requestId == lastRequestId, "Request already fulfilled");
        game_state = GAME_STATE.CLOSED;
        ...
        if (numRunnersUp > 0) {
            uint256[] memory runnerIndices = new uint256[](numRunnersUp);
            for (uint256 j = 0; j < numRunnersUp; j++) {
            ...
            declareRunnerUps(index, runnerIndices, runnersUpShare);
        }
    ...
    function declareWinner(uint256 index) private {
        require(
            game_state == GAME_STATE.CLOSED,
        ...
        emit WinnerDeclared(players[index], winnerShares);
    }

    function declareRunnerUps(
        uint256 index,
        uint256[] memory runnerIndices,
        uint256 runnersUpShare
    ) private {
        uint256 numRunnersUp = players.length - 1;
        if (numRunnersUp <= 100) {
            ...
            for (uint256 j = 0; j < numRunnersUp; j++) {
                if (j != index) {
                    ...
                    runner.sendValue(runnersUpShare / 100);
    ...
    function transferBalanceToDAO() private nonReentrant {
        ...
        // Call the deposit function on the provided contract
        depositDistributorInstance.deposit{value: depositAmount}(depositAmount);

        // Transfer the remaining balance to the DAO
        daoPayments.sendValue(remainingBalance);
