...
contract PresaleNFT is Ownable {
    ...
    // tokenId => limit per user
    mapping(uint256 => uint256) public UserLimitCardId;
    ...
    function registerForPreSale(address _erc20, LimitItem[] calldata _wanted) external {
        ...
        for (uint256 i = 0; i < _wanted.length; i ++){
            ...
            require(_wanted[i].limit + _getUserLimitForCard(msg.sender, _wanted[i].tokenId) <= UserLimitCardId[_wanted[i].tokenId],
                "Limit exceeded by tokenId");
    ...
    function setUserLimitCardId(uint _tokenId, uint _limit) external onlyOwner {
        UserLimitCardId[_tokenId] = _limit;
    }
    ///////////////////////////////////////////////////////////////////
