...
contract DreamHorseController is Ownable {
    ...
    struct User{
        ...
        uint256 forageNumber;//粮草数量
        
        uint256 horseshoeNumber;//马蹄铁数量
    }
    ...
    // HeroFightAddition英雄对战加成
    uint256 public foragePrice = 300;//粮草数量

    uint256 public horseshoePrice = 200;//粮草数量

    uint256 public forageOnceLimit = 3;//粮草数量

    uint256 public horseshoeOnceLimit = 4;//马蹄铁数量
    
    uint256 public forageAddition = 3;//粮草收益加成

    uint256 public horseshoeAddition = 1;//马蹄铁胜率加成
    ...
    constructor() {
        ...
        monsters[1] = Monster(1,87,1,15,6,1,1);
        monsters[2] = Monster(2,70,2,18,7,3,1);
        monsters[3] = Monster(3,56,1,23,8,5,2);
        monsters[4] = Monster(4,39,8,28,11,8,3);
        monsters[5] = Monster(5,25,16,35,13,12,5);
    ...
    function setForagePrice(uint256 price) public onlyOwner {
        foragePrice = price;
    }

    function setHorseshoePrice(uint256 price) public onlyOwner {
        horseshoePrice = price;
    }

    function setForageOnceLimit(uint256 limit) public onlyOwner {
        forageOnceLimit = limit;
    }

    function setHorseshoeOnceLimit(uint256 limit) public onlyOwner {
        horseshoeOnceLimit = limit;
    }

    function setForageAddition(uint256 addition) public onlyOwner {
        forageAddition = addition;
    }

    function setHorseshoeAddition(uint256 addition) public onlyOwner {
        horseshoeAddition = addition;
    }
    ...
    function buyForage(uint256 number) public {
        uint256 decimals = AMT.decimals();

        AMT.transferFrom(msg.sender, address(this), foragePrice * number * 10 ** decimals);

        users[msg.sender].forageNumber += number;
    }

    function buyHorseshoe(uint256 number) public {
        uint256 decimals = AMT.decimals();

        AMT.transferFrom(msg.sender, address(this), horseshoePrice * number * 10 ** decimals);

        users[msg.sender].horseshoeNumber += number;
    }
    ...
    function selectMonster(uint256 monsterID,uint256 tokenId,uint256 forageNumber, uint256 horseshoeNumber) public {
        ...
        require(forageNumber <= forageOnceLimit && horseshoeNumber <= horseshoeOnceLimit 
        && forageNumber <= users[msg.sender].forageNumber && horseshoeNumber <= users[msg.sender].horseshoeNumber,"This hero is unavailable");
        ...
        if(playResult){
            ...
            profit = userSelectMonster.reward * 10 ** 2;
    ...
    function userDrawMonsterProfit(uint256 tokenId) public {
        ...
        payable(msg.sender).transfer(profit * 10 ** 16);
    }

    function gertUserNotDrawMonsterProfit(uint256 tokenId) public view returns(uint256 profit) {
    ...
    function getUserBuyHeros(address user) public view returns(Hero[] memory userHeros) {
