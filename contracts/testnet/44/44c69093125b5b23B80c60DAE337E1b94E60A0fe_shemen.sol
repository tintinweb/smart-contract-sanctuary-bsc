...
contract shemen is Ownable{
    ...
    address public nftAddr = 0x1307E34e8A5c4e2D622FbF466f3259800D06C25E;
   ...
   // mapping(uint => Result) private result;
    ...
    event ResultDataEvent(address indexed sender,uint betNum,uint randomNum,uint amount,uint timestamp,uint income);
    constructor()  {}
    ...
    function betResult(bytes32 reqId, uint randomNum) internal{
        ...
        if (request[reqId].betNum >= randomNum){
            ...
            if (whiteAddrs[request[reqId].addr]) {
                ...
                (address inviter,) = IInvitation(invitationAddr).getInvitation(msg.sender);
        ...
        emit ResultDataEvent(request[reqId].addr,request[reqId].betNum,randomNum,request[reqId].amount,block.timestamp,income);
