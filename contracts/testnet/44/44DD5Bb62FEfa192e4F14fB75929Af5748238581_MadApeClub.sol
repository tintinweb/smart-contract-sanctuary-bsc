...
/**
 * @title MadApeClub contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract MadApeClub is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    */
    function mintApe(uint numberOfTokens) public payable {
        ...
        if (startingIndexBlock == 0 && (totalSupply() == maxApes || block.timestamp >= REVEAL_TIMESTAMP)) {
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint(blockhash(startingIndexBlock)) % maxApes;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint(blockhash(block.number - 1)) % maxApes;
        }
    ...
    */
    ...
    /*
    * Max supply can be changed by owner
    */
    function setMaxApes(uint maxApes_) public onlyOwner {
        maxApes = maxApes_;
    }

    /*
    * check pause status and call super transfer
    */
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        require(!isPaused);
