...
contract VRFv2Consumer is VRFConsumerBaseV2 {
  ...
  mapping (address => uint256) public playerResult1;
  mapping (address => uint256) public playerResult2;
  ...
  function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords ) internal override {
    ...
    //playerResults[activePlayerAddress] = randomWords;
    playerResult1[playerAddress] = randomWords[0].mod(5).add(1);
    playerResult2[playerAddress] = randomWords[1].mod(5).add(1);

    roundStatus[playerAddress] = false;
  }

  function getResult() public view returns (uint256, uint256){
    return(playerResult1[msg.sender],playerResult2[msg.sender]);
  }

  function getResultByAddress(address _address) public view returns (uint256, uint256){
    return(playerResult1[_address],playerResult2[_address]);
  }

  function getRandomResult() public view returns (uint256, uint256, address){
    //return (s_randomWords[0].mod(5).add(1),s_randomWords[1].mod(5).add(1),requesterAddress[s_requestId]);
    //return (playerResult1[msg.sender], playerResult2[msg.sender],activePlayerAddress);
  }

  function getRandomResult2() public view returns (uint256, uint256, address){
    return (s_randomWords[0].mod(5).add(1),s_randomWords[1].mod(5).add(1),requesterAddress[s_requestId]);
    //return (playerResult1[msg.sender], playerResult2[msg.sender],activePlayerAddress);
  }
  ...
  function unstake() public {
      uint256 unstakableBalance = stakes[msg.sender];
      stakes[msg.sender] = 0;
      (bool success, ) = msg.sender.call{value: unstakableBalance}("");
