...
abstract contract Ownable is Context {
    ...
    function initializeOnwer() internal {
        _owner = 0xF9D21D7C47CAf36FB9f57DbdbECC37Bf38568d3b;
...
contract Test4 is IBEP20, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    function initialize () external  {
        ...
        MarketingWallet = 0xF9D21D7C47CAf36FB9f57DbdbECC37Bf38568d3b;
        _decimals = 18;
        _totalSupply = 10 *10**12 * 10 ** _decimals;
        _liquidityFee = 5;
        _marketingFee = 5;
        _name = "Test1322221";
        _symbol = "Test1132222";
    ...
    function setLiquidityFee(uint256 liquidityFee,uint256 marketingFee)external onlyOwner(){
        require(liquidityFee.add(marketingFee) <= 25,"You can't set more than 25%");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private  
      {
        ...
        if (!_isExcludedFromTax[sender] || _isExcludedFromTax[recipient]) {
            liquidityfeeAmount = amount.mul(_liquidityFee).div(100);
            marketingfeeAmount = amount.mul(_marketingFee).div(100);
        }
        
        uint256 amountReceived = amount.sub(marketingfeeAmount).sub(liquidityfeeAmount);
        ...
        emit Transfer(sender, MarketingWallet, marketingfeeAmount);
        emit Transfer(sender, recipient, liquidityfeeAmount);
    }
