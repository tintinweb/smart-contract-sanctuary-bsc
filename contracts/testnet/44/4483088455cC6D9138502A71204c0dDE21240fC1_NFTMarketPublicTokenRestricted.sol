...
contract NFTMarketPublicTokenRestricted is ReentrancyGuard, AccessControl, ERC721Holder {
    ...
    error TokenContractIsNotAllowed();
    ...
    mapping(address => bool) private _tokenContractWhitelist;
    ...
    event TokenContractWhitelistChanged(address indexed tokenContract, bool allowed);
    ...
    constructor(uint96 defaultFee, bool defaultAuctionAllowed) {
        ...
        // for trading in native
        _tokenContractWhitelist[address(0)] = true;
    ...
    function setTokenContractAllowed(address tokenContract, bool allowed) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_tokenContractWhitelist[tokenContract] != allowed) {
            _tokenContractWhitelist[tokenContract] = allowed;

            emit TokenContractWhitelistChanged(tokenContract, allowed);
        }
    }
    ...
    function listTradeItem(
        ...
        if (!_tokenContractWhitelist[tokenContract]) revert TokenContractIsNotAllowed();
    ...
    function withdrawFeesNative() external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 amount = address(this).balance - getLockedFees(address(0));
    ...
    function withdrawFeesToken(address _tokenContract, uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        uint256 availableAmount = tokenContract.balanceOf(address(this)) - getLockedFees(_tokenContract);
        if (amount > availableAmount) revert IncorrectFunds({ sent: amount, required: availableAmount });
    ...
    function cancelTrade(uint256 tradeId) external nonReentrant {
        ...
        if (msg.sender != tradeItem.seller && !hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert CallerIsNotAllowed();
        if (tradeItem.state != TradeState.ON_SALE)
            revert WrongTradeState({ state: tradeItem.state, requiredState: TradeState.ON_SALE });
    ...
    function cancelAuction(uint256 auctionId) external nonReentrant {
        ...
        if (msg.sender != auctionItem.seller && !hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert CallerIsNotAllowed();
        if (auctionItem.state != AuctionState.ON_AUCTION)
            revert WrongAuctionState({ state: auctionItem.state, requiredState: AuctionState.ON_AUCTION });
    ...
    function fetchTokenContractAllowed(address tokenContract) external view returns (bool allowed) {
        allowed = _tokenContractWhitelist[tokenContract];
    }
    ...
    // returns the locked funds that the contract holds (which is: ongoing auction highest bids)
    function getLockedFees(address addressTokenContract) public view returns (uint256 lockedFees) {
        uint256 totalItemCount = _auctionId.current();

        for (uint256 i = 0; i < totalItemCount; i++) {
            AuctionItem memory currentItem = _auctionItems[i];
            if (currentItem.state == AuctionState.ON_AUCTION && currentItem.tokenContract == addressTokenContract) {
                lockedFees += currentItem.highestBid;
            }
        }
    }
