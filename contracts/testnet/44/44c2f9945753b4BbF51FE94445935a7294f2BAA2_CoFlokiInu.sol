...
contract CoFlokiInu is IBEP20, Auth {
    ...
    string constant _symbol             = "CoFlokiInu";
    uint8 constant _decimals            = 9;

    uint256 _totalSupply                = 1000000000 * 10**_decimals;

    uint256 public _maxTxLimit          = _totalSupply / 50;
    ...
    uint256 public totalSellFee         = sellMarketingFee + sellReflectionFee + sellLiquidityFee + sellTeamFee + sellDevFee;
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _marketingFee, uint256 _teamFee, uint256 _devFee, uint256 _stakingFee, uint256 _feeDenominator) external authorized {
        ...
        require(totalFee < feeDenominator/2, "Fees cannot be more than 100%");
    }

    function setSellFees(uint256 _sellLiquidityFee, uint256 _sellReflectionFee, uint256 _sellMarketingFee, uint256 _sellTeamFee, uint256 _sellDevFee, uint256 _feeDenominator) external authorized {
        sellLiquidityFee    = _sellLiquidityFee;
        sellReflectionFee   = _sellReflectionFee;
        sellMarketingFee    = _sellMarketingFee;
        sellTeamFee         = _sellTeamFee;
        sellDevFee          = _sellDevFee;
        totalSellFee        = _sellLiquidityFee + _sellReflectionFee + _sellMarketingFee + _sellTeamFee + _sellDevFee;
        feeDenominator      = _feeDenominator;
        require(totalSellFee < feeDenominator/2, "Fees cannot be more than 25%");
    }

    function setFeeReceivers(address _autoLiquidityReceiver, address _marketingFeeReceiver, address _teamFeeReceiver, address _stakingFeeReceiver) external authorized {
