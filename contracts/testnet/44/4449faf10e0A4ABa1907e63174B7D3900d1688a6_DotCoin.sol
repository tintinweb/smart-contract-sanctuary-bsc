...
contract DotCoin is Context, IERC20, Ownable{
    ...
    constructor (string memory name_,string memory symbol_,address[] memory wallets_,bool isMainNetwork_,uint256 maxTxAmount_,uint256 minTokensSellToAddToLiquidity_,address owner_,uint256 frequencyOfUnlock_) {
        ...
        //exclude from reflection address(0)
        excludeFromReflection(address(0));

        //exclude from reflection pancake 
        excludeFromReflection(address(_pancakeswapV2Router));
    ...
    function _initilizeTokenomics() private returns(bool){
        ...
        // Management Wallet – 35.8 %( 35, 90, 00, 000) – Single Wallet
        transfer(managementWallet, _tTotal.mul(358).div(1000));
    ...
    function _initilizeWallets(address[] memory wallets_) private returns(bool){
      ...
      excludeFromReflection(icoWallet);
   ...
   function _transferBothExcluded(address sender_, address recipient_, uint256 tAmount_) private {
    ...
    function addLiquidity(uint256 tokenAmount_) public payable returns(bool) {
    ...
    function includeInReward(address account_) external onlyOwner() returns(bool){
 ...
 //Locking
    ...
    uint256 private constant _PERCENTAGE_OF_UNLOCK = 1000; // 100%
        ...
        returns (uint256)
    {
        ...
        if (locked[recipient_].amount > 0) {
            ...
            unlockDate =
                ...
                (locked[recipient_].frequencyOfUnlock);
    ...
    function setSwapAndLiquifyEnabled(bool newStatus_) external returns(bool){
