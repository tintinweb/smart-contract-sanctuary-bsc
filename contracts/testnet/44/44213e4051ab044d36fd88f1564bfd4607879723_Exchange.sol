...
contract Exchange is Initializable, UUPSUpgradeable, EIP712Upgradeable, OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    IWEKTA public constant WEKTA = IWEKTA(0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd);
    ...
    function completeFixedSale(LibOrder.Order memory order) external payable whenNotPaused nonReentrant {
        ...
        {
            (bool success, ) = payable(getRoyaltyWallet(order)).call{value: royaltyAmount}("");
            require(success, "Exchange: Revenue transfer failed");
        }
    ...
    function completeBidding(LibBidOrder.BidOrder memory bidOrder, bytes calldata signature) external whenNotPaused nonReentrant {
        ...
        {
            (bool success) = IWEKTA(WEKTA).transferFrom(
                IEktaNftRegistry(ektaNftRegistry).ektaRevenueWallet(), 
                msg.sender, 
                revenueAmount
            );
        ...
        // transfer remaining WEKTA to seller
        {
            ...
            (bool success) = IWEKTA(WEKTA).transferFrom(bidOrderData.buyer, msg.sender, bidOrderData.bidAmount);
            require(success, "Exchange: WEKTA transfer failed");
        }
    ...
    function getRoyaltyWallet(LibOrder.Order memory order) view internal returns(address){
        return OwnableUpgradeable(order.tokenAddress).owner();
    }
