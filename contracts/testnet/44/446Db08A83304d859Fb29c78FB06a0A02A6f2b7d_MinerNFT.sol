...
contract MinerNFT is ERC721Base, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint8 private constant MAX_MINER_LEVEL = 10;

    uint256 lastMinerId;
    address public municipalityAddress;
    mapping(uint256 => uint256) minerIdToLevelMapping;
    ...
    function upgradeMinerLevels(uint256 _minerId, uint256 _levels) external OnlyMunicipality {
        require(_exists(_minerId),"MinerNFT: Miner does not exist");
        minerIdToLevelMapping[_minerId] += _levels;
    }

    function hashrate(uint256 _minerId) external view returns (uint256) {
        return (minerIdToLevelMapping[_minerId] + 1) * 1000;
    }
