...
contract WEMPToken is Context, IBEP20, IBEP20Metadata, Pausable, Ownable{
    ...
    string private constant _name = "WEMP Token";
    ...
    address public exchangeListWallet;
    address public womenWallet;
    ...
    uint256 public staffWagefee=1;
    uint256 public exchangefee=1;
    uint256 public womenfee=1;

   

    uint256 public totalSwapFee = marketingFee + charityFee + buyBackFee + womenfee + staffWagefee + exchangefee;
    uint256 private previousTotalSwapFee = totalSwapFee;

    IUniswapV2Router02 public immutable uniswapV2Router;
    address public uniswapV2Pair;

    address constant UNISWAPV2ROUTER = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    bool public exchangeswapEnabled=true;
    bool public womenswapEnabled=true;
    ...
    event SetStaffWageFeePercent(uint256 staffWageFeePercent);
    event SetExchangeFeePercent(uint256 exchangeFeePercent);
    event SetWomenFeePercent(uint256 womenFeePercent);
    ...
    event EnableFees(bool marketingfeeenabled,bool charityfeeenabled,bool buyBackfeeenabled,bool staffWagefeeenabled,bool exchangefeeenabled,bool womenfeeenabled);
    ...
    constructor (address _marketingWallet, address _charityWallet, address _buyBackWallet, address _staffWageWallet, address _exchangeListWallet, address _womenWallet ) {
        _rOwned[_msgSender()] = _rTotal;
        marketingWallet = _marketingWallet;
        charityWallet = _charityWallet;
        buyBackWallet = _buyBackWallet;
        staffWageWallet = _staffWageWallet;
        exchangeListWallet = _exchangeListWallet;
        womenWallet = _womenWallet;
    ...
    function setMarketingFeePercent(uint256 fee) external onlyOwner whenNotPaused {
        marketingFee = fee;
        totalSwapFee = marketingFee + charityFee + buyBackFee + womenfee + staffWagefee + exchangefee;
    ...
    /**
     * @dev Set staffwage fee percentage.
     * @param fee - staffwage fee percentage.
    */
    function setStaffWageFeePercent(uint256 fee) external onlyOwner whenNotPaused {
        staffWagefee = fee;
        totalSwapFee = marketingFee + charityFee + buyBackFee + womenfee + staffWagefee + exchangefee;
        emit SetStaffWageFeePercent(staffWagefee);
    }

    /**
     * @dev Set exchange fee percentage.
     * @param fee - exchange fee percentage.
    */
    function setExchangeFeePercent(uint256 fee) external onlyOwner whenNotPaused {
        exchangefee = fee;
        totalSwapFee = marketingFee + charityFee + buyBackFee + womenfee + staffWagefee + exchangefee;
        emit SetExchangeFeePercent(exchangefee);
    }

    /**
     * @dev Set women fee percentage.
     * @param fee - women fee percentage.
    */
    function setWomenFeePercent(uint256 fee) external onlyOwner whenNotPaused {
        womenfee = fee;
        totalSwapFee = marketingFee + charityFee + buyBackFee + womenfee + staffWagefee + exchangefee;
        emit SetWomenFeePercent(womenfee);
    }

  

    function enableswap(bool marketingfeeenabled,bool charityfeeenabled,bool buyBackfeeenabled,bool staffWagefeeenabled,bool exchangefeeenabled,bool womenfeeenabled) external onlyOwner whenNotPaused {
        marketingswapEnabled = marketingfeeenabled;
        charityswapEnabled = charityfeeenabled;
        buyBackswapEnabled = buyBackfeeenabled;
        staffWageswapEnabled = staffWagefeeenabled;
        exchangeswapEnabled = exchangefeeenabled;
        womenswapEnabled = womenfeeenabled;
        emit EnableFees(marketingswapEnabled,charityswapEnabled,buyBackswapEnabled,staffWageswapEnabled,exchangeswapEnabled,womenswapEnabled);
    }
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        uint256 rAmount = amount.mul(getRate());
    ...
    function getTValues(uint256 amount) internal view returns (uint256, uint256, uint256) {
        ...
        uint256 tTransferAmount = tAmount.sub(tFee).sub(tSwapFee);
    ...
    function getRValues(uint256 amount, uint256 tFee, uint256 tSwapFee) internal view returns (uint256, uint256, uint256, uint256) {
        ...
        uint256 rSwapFee = tSwapFee.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rSwapFee);
    ...
    function takeSwapFee(address sender, uint256 rSwapFee, uint256 tSwapFee) internal {
        _rOwned[address(this)] = _rOwned[address(this)].add(rSwapFee); 
        if(_isExcluded[address(this)]){
             _tOwned[address(this)] = _tOwned[address(this)].add(tSwapFee);
        }
    ...
    function distributeFee() internal {
        ...
        if(contractTokenBalance >= minimumTokensBeforeSwap) {
            ...
            bool initialSellLimitState = enableMaxSellLimit;
            // remove fee if initialFeeState was true
            if(initialFeeState) enableFee = false;
            // remove sell limit if initialSellLimitState was true
            if(initialSellLimitState) enableMaxSellLimit = false;
            ...
            if(marketingswapEnabled) transferBNBToAddress(payable(marketingWallet), transferredBalance.mul(marketingFee).div(totalSwapFee));

            // Send bnb to Charity address
            if(charityswapEnabled) transferBNBToAddress(payable(charityWallet), transferredBalance.mul(charityFee).div(totalSwapFee));

            //send bnb to staffwallet address
            if(staffWageswapEnabled) transferBNBToAddress(payable(staffWageWallet), transferredBalance.mul(staffWagefee).div(totalSwapFee));

            //send bnb to women address
            if(womenswapEnabled) transferBNBToAddress(payable(womenWallet), transferredBalance.mul(womenfee).div(totalSwapFee)); 

            //send bnb to exchangelist address
            if(exchangeswapEnabled) transferBNBToAddress(payable(exchangeListWallet), transferredBalance.mul(exchangefee).div(totalSwapFee)); 

            // Buy back tokens using eth
            if(buyBackswapEnabled) transferBNBToAddress(payable(buyBackWallet), transferredBalance.mul(buyBackFee).div(totalSwapFee));

            // enable fee if initialFeeState was true
            if(initialFeeState) enableFee = true;
            // enable sell limit if initialSellLimitState was true
            if(initialSellLimitState) enableMaxSellLimit = true;
