...
contract PotionNFT is ERC721Enumerable, Ownable {
    ...
    struct Potion {
        Powers powers;
        uint256 potionType; //  0: water ; 1: fire ; 2:metal ; 3:air ; 4:stone  ; 5:rest ; 6:xp ; 7:multiple ; 8:mana
        address seller;
    ...
    mapping(uint256 => Potion) private _potions;
    ...
    function offerPotion(
        ...
        _tokenIds.increment();
        uint256 _newPotionId = _tokenIds.current();
        _safeMint(_to, _newPotionId);

        Potion storage i = _potions[_newPotionId];
        i.potionType = _type;
        if(_type == 0){
    ...
    function mintPotionForSale(
        ...
        _tokenIds.increment();
        uint256 _newPotionId = _tokenIds.current();
        address _laboManagementAddress = gameAddresses.getLaboratoryAddress();
        _safeMint(_laboManagementAddress, _newPotionId);
        ...
        i.potionId = _newPotionId;
        address _laboNftAddress = gameAddresses.getLaboratoryNFTAddress();
        i.seller = IERC721(_laboNftAddress).ownerOf(_laboId);
        i.fromLab = _laboId;

        return (_newPotionId);
    }

    function changePotionPrice(uint256 _tokenId, uint256 _laboId, uint256 _price) external onlyAuth returns(bool){
        Potion storage i = _potions[_tokenId];
    ...
    function updatePotionSaleTimestamp(uint256 _tokenId) external onlyAuth returns(bool){
        _potions[_tokenId].saleTimestamp = block.timestamp;
        return true;
    }   

    function mintMultiplePotion(uint256[6] memory _powers, address _owner) external onlyAuth returns(uint256){
        _tokenIds.increment();
        uint256 _newPotionId = _tokenIds.current();
        _safeMint(_owner, _newPotionId);

        Potion storage i = _potions[_newPotionId];
        i.potionType = 7;
        i.powers.water = _powers[0]; 
        i.powers.fire = _powers[1]; 
        i.powers.metal = _powers[2]; 
        i.powers.air = _powers[3]; 
        i.powers.stone = _powers[4]; 
        i.powers.xp = _powers[5]; 
    ...
    // type : 0 Water , 1 Fire , 2 metal , 3 air , 4 stone , 5 rest , 6 xp
    function buyPotion(address _to, uint256 _type) external onlyAuth returns (uint256) {
        _tokenIds.increment();
        uint256 _newPotionId = _tokenIds.current();
        _safeMint(_to, _newPotionId);
        Potion storage i = _potions[_newPotionId];
    ...
    function getPotion(uint256 _tokenId) external view returns(address seller, uint256 price, uint256 fromLab){
        Potion memory i = _potions[_tokenId];
        return (i.seller,i.listingPrice,i.fromLab);
    }

    function getFullPotion(uint256 _tokenId) external view returns(Potion memory){
