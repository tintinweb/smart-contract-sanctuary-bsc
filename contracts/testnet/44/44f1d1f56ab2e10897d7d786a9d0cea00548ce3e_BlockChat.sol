...
contract Message is ERC20, ReentrancyGuard {
    ...
    mapping(address => address  ) public   Block_ID;
    mapping(address => string   ) public   Block_name;
    mapping(address => address  ) internal Block_owner;
    mapping(address => bytes32  ) internal Block_password;
    ...
    modifier require_Block_password(address BlockAddress, string memory password) {
        if (Block_password[BlockAddress] != keccak256(abi.encodePacked(""))) {
            require(keccak256(abi.encodePacked(password)) == Block_password[BlockAddress], "Password not match.");
        }
        _;
    }
    ...
    function pause_Block(address BlockAddress, string memory password) public nonReentrant() {
        require(keccak256(abi.encodePacked(password)) == Block_password[BlockAddress], "Password not match.");
    ...
    function create_Block(address BlockAddress, string memory set_name, string memory set_password) public nonReentrant() {
        require(Block_ID[BlockAddress] == address(0), "Block already taken.");
        Block_owner[BlockAddress]       = msg.sender;
        Block_name[BlockAddress]        = set_name;
        Block_password[BlockAddress]    = keccak256(abi.encodePacked(set_password));
        Block_subscribers[BlockAddress].push(msg.sender);
        Blocks_list.push(address(BlockAddress));
        Blocks_Owned[msg.sender].push(address(BlockAddress));
    }
...
contract BlockChat is Message {
    function check_address_by_Block_ID(address BlockAddress) public view returns(address Block_address) {
        return Block_ID[BlockAddress];
    }
    ...
    function Block_blockchat_pay(address receiver, uint amount, string memory password) public nonReentrant() {
        if (Block_password[receiver] != keccak256(abi.encodePacked(""))) {
            require(keccak256(abi.encodePacked(password)) == Block_password[receiver], "Password not match.");
        }
    ...
    function team_airdrop(address[] memory list, uint amount, string memory password) public nonReentrant() {
        if (Block_password[team] != keccak256(abi.encodePacked(""))) {
            require(keccak256(abi.encodePacked(password)) == Block_password[team], "Password not match.");
        }
...
// Powered by BlockChat Limited (2022-12-13)
// Alpha Version 1.1 on Binance Smart Chain Testnet