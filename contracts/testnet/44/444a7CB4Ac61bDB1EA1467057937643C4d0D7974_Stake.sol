...
contract Stake is Context, Ownable,ReentrancyGuard {
    ...
    function getStakeDataLP(address _user)public view returns(uint,uint,uint,uint,bool,uint) {
        return (poolBalance,lpRequire,allStakedAmountLP,mainPairC.balanceOf(_user),isLpStaker[_user],lpStakeAmount[_user]);
    }
    function getStakeDataFMT(address _user)public view returns(uint,uint,uint,bool,StakeData memory) {
        return (poolBalance,allStakedAmountFMT,FMT.balanceOf(_user),isStaker[_user],getStakeData[_user]);
    }




    function setData(address _fmtAddress,address _mainPair,uint _computeCycle)public onlyOwner{
