...
contract Arsames is Context, IERC20, Ownable {
    ...
    string private _name = "OLA";
    string private _symbol = "ola";
    ...
    address public constant CharityWallet = 0x2C355Eeb79621b9Da0ac2C8312f208aF8A476D0a;
    address public constant TeamWallet = 0xeB28A08124784F1175e04c9dE82485Fa17241557; 
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_isExcluded[account]) return balances_map[account];
    ...
    function _transfer(address sender,address recipient,uint256 amount) private {
        ...
        require(amount > 0, "Transfer amount must be greater than zero");
         if (recipient == dEaDWallet || recipient == CharityWallet || sender == CharityWallet || recipient == TeamWallet || sender == TeamWallet || recipient == owner() || sender == owner())
    ...
    function _tokenTransfer(address sender,address recipient,uint256 amount) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(
                sender,
                recipient,
                (amount.sub(burnAmt).sub(charityAmt).sub(teamAmt))
            );
    ...
    function _transferToExcluded(address sender,address recipient,uint256 tAmount) private {
        (uint256 rAmount,uint256 rTransferAmount,uint256 rFee,uint256 tTransferAmount,uint256 tFee) = _getValues(tAmount);
        addresses_map[sender] = addresses_map[sender].sub(rAmount);
        balances_map[recipient] = balances_map[recipient].add(tTransferAmount);
        addresses_map[recipient] = addresses_map[recipient].add(rTransferAmount);
        _reflectFee(rFee, tFee);
        emit Transfer(sender, recipient, tTransferAmount);
    }
    ...
    function _transferFromExcluded(address sender,address recipient,uint256 tAmount) private {
        (uint256 rAmount,uint256 rTransferAmount,uint256 rFee,uint256 tTransferAmount,uint256 tFee) = _getValues(tAmount);
        balances_map[sender] = balances_map[sender].sub(tAmount);
        addresses_map[sender] = addresses_map[sender].sub(rAmount);
        addresses_map[recipient] = addresses_map[recipient].add(rTransferAmount);
        _reflectFee(rFee, tFee);
        emit Transfer(sender, recipient, tTransferAmount);
