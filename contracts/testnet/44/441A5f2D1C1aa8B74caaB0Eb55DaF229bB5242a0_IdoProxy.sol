...
contract IdoProxy is Proxy, ERC1967Upgrade {

    constructor(address _logic) payable {
        _upgradeToAndCall(_logic, abi.encodeWithSignature("initialize()"),true);
    ...
    function setImplementation(address impl) external {
        require(msg.sender == _getAdmin(), "StakeProxy: only called by admin");
        ERC1967Upgrade._upgradeTo(impl);
    }

    function setAdmin(address admin) external {
        require(msg.sender == _getAdmin(), "StakeProxy: only called by admin");
        ERC1967Upgrade._changeAdmin(admin);
    }
}