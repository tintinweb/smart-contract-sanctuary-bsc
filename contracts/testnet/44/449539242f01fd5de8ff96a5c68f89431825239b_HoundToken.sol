...
contract HoundToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        swapTokensAtAmount = totalSupply.mul(3).div(10**4);
        ...
        dividendTracker = new TokenDividendTracker(_uniswapV2Pair);
        pair = _uniswapV2Pair;
        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair[_uniswapV2Pair] = true;

        marketWalletAddress = nftWalletAddr_;
        ...
        isDividendExempt[teamAddress] = true;
    ...
    function setMarketWalletAddress(address wallet) external onlyOwner{
    ...
    function takeAllFee(address from,address to,uint256 amount) private returns(uint256 amountAfter) {
        ...
        if(uniswapV2Pair[from] || uniswapV2Pair[to]){
            uint256 totalFee = lpShareFee.add(marketFee).add(liquidityFee);
            uint256 totalFeeAmount = amount.mul(totalFee).div(100);
            if(totalFeeAmount > 0) super._transfer(from, address(this), totalFeeAmount);
    ...
    function swapAndLiquify(uint256 tokens) private {
        uint256 totalFee = lpShareFee.add(marketFee).add(liquidityFee);
        uint256 amountToLiquify = tokens.mul(liquidityFee).div(totalFee).div(2);
        uint256 amountToSwap = tokens.sub(amountToLiquify);
        swapTokensForUSDT(amountToSwap, address(this));
        uint256 amountBNB = address(this).balance;

        uint256 marketBNB = amountBNB.mul(marketFee).div(totalFee);
        uint256 lpShareBNB = amountBNB.mul(lpShareFee).div(totalFee);
        uint256 liquifyBNB = amountBNB.sub(marketBNB).sub(lpShareBNB);
        payable(address(liquidityBackHelper)).transfer(liquifyBNB);
        payable(address(teamAddress)).transfer(marketBNB.div(2));
        payable(address(marketWalletAddress)).transfer(marketBNB.div(2));
        super._transfer(address(this), address(liquidityBackHelper), amountToLiquify);
        payable(address(dividendTracker)).transfer(lpShareBNB);
        // liquidityBackHelper.addLiquidity(amountToLiquify, liquifyBNB);
    }
