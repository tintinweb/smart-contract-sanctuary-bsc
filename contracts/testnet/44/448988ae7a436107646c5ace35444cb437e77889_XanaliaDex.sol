 ...
 */
contract XanaliaDex is IXanaliaDex, OwnableUpgradeable {
	uint256 public override collectionCount;
	address public override xanaliaUriAddress;
	address public override marketDexAddress;
	address public override auctionDexAddress;
	mapping(uint256 => address) public override collectionAddresses;
	...
	function initialize(
		address _xanaliaUriAddress,
		address _marketDexAddress,
		address _auctionDexAddress
	) public virtual initializer {
		OwnableUpgradeable.__Ownable_init();
		xanaliaUriAddress = _xanaliaUriAddress;
		marketDexAddress = _marketDexAddress;
		auctionDexAddress = _auctionDexAddress;
		processing = false;
	}
	...
	*/
	function createXanaliaCollection(
		...
		collectionAddresses[collectionCount_] = address(newCollection);
		isUserCollection[address(newCollection)] = _private;
		emit CollectionCreated(msg.sender, address(newCollection), name_, symbol_);
	...
	*/
	function mint(
		...
		uint256 _royaltyFee
	) external override onlyNotProcessing returns (uint256 newTokenId) {
		...
		newTokenId = xanaliaNFT.create(_tokenURI, _royaltyFee, msg.sender);
	...
	*/
	function mintAndPutOnSale(
		...
		uint256 _price
	) external override onlyNotProcessing returns (uint256 newTokenId, uint256 orderId) {
		IXanaliaNFT xanaliaNFT = IXanaliaNFT(_collectionAddress);
		IMarketDex marketDex = IMarketDex(marketDexAddress);
		if (isUserCollection[_collectionAddress]) {
			require(xanaliaNFT.getContractAuthor() == msg.sender, "Not-author-of-collection");
		}
		newTokenId = xanaliaNFT.create(_tokenURI, _royaltyFee, msg.sender);
		...
		if (!xanaliaNFT.isApprovedForAll(msg.sender, marketDexAddress)) {
			xanaliaNFT.setApprovalForAll(msg.sender, marketDexAddress, true);
		}

		orderId = marketDex.createOrder(_collectionAddress, _paymentToken, newTokenId, _price);

		return (newTokenId, orderId);
	}
	...
	*/
	function mintAndPutOnAuction(
		...
		uint256 _startTime,
		uint256 _endTime
	) external override onlyNotProcessing returns (uint256 newTokenId, uint256 auctionId) {
		IXanaliaNFT xanaliaNFT = IXanaliaNFT(_collectionAddress);
		IAuctionDex auctionDex = IAuctionDex(auctionDexAddress);
		if (isUserCollection[_collectionAddress]) {
			require(xanaliaNFT.getContractAuthor() == msg.sender, "Not-author-of-collection");
		}
		newTokenId = xanaliaNFT.create(_tokenURI, _royaltyFee, msg.sender);
		...
		if (!xanaliaNFT.isApprovedForAll(msg.sender, auctionDexAddress)) {
			xanaliaNFT.setApprovalForAll(msg.sender, auctionDexAddress, true);
		}
		auctionId = auctionDex.createAuction(_collectionAddress, _paymentToken, newTokenId, _startPrice, _startTime, _endTime);

		return (newTokenId, auctionId);
	}
	...
	*/
	...
	function setXanaliaUriAddress(address _collectionAddress, address _xanaliaUriAddress) external onlyOwner {
		IXanaliaNFT xanaliaNFT = IXanaliaNFT(_collectionAddress);
		xanaliaNFT.setXanaliaUriAddress(_xanaliaUriAddress);
	}
...
contract XNFT is ERC721, Ownable, ERC721URIStorage {
	...
	address public xanaliaUriAddress;
	...
	constructor(
		...
		address _author,
		address _xanaliaUriAddress,
		address _xanaliaDexAddress,
		address _auctionDexAddress
	) ERC721(_name, _symbol) {
		contractAuthorAddress = _author;
		xanaliaUriAddress = _xanaliaUriAddress;
		setApprovalForAll(_xanaliaDexAddress, true);
		setApprovalForAll(_auctionDexAddress, true);
	}
	...
	function _baseURI() internal view override returns (string memory) {
		return IXNftURI(xanaliaUriAddress).baseMetadataURI();
	}
	...
	function setXanaliaUriAddress(address _xanaliaUriAddress) external onlyOwner {
		xanaliaUriAddress = _xanaliaUriAddress;
	}
...
interface IXanaliaDex {
	...
	function xanaliaUriAddress() external view returns (address);

	function marketDexAddress() external view returns (address);

	function auctionDexAddress() external view returns (address);
	...
	function mintAndPutOnSale(
		...
		address _paymentToken,
		uint256 _price
	) external returns (uint256 newTokenId, uint256 orderId);
	...
	function mintAndPutOnAuction(
		...
		uint256 _startTime,
		uint256 _endTime
	) external returns (uint256 newTokenId, uint256 auctionId);
}
...
interface IMarketDex {
    ...
    function buy(uint256 _orderId, address _paymentToken) external payable returns (bool);
    ...
    function createBid(
		...
		uint256 _expireTime
	) external payable returns (uint256 _bidId);

    function acceptBid(uint256 _bidId, uint256 _orderId) external returns (bool isMatchedOrder);

    function cancelOrder(uint256 _orderId) external;

    function cancelBid(uint256 _bidId) external  returns (uint256);
...
interface IAuctionDex {
	...
	function bidAuction(
		...
		uint256 _auctionId,
		uint256 _price
	) external payable returns (uint256 _bidAuctionId);

	function editBidAuction(uint256 _bidAuctionId, uint256 _price) external payable returns (uint256);

	function cancelAuction(uint256 _auctionId) external returns (uint256);

	function cancelBidAuction(uint256 _bidAuctionId) external returns (uint256);

	function reclaimAuction(uint256 _auctionId) external;

	function claimWinnerAuction(uint256 _bidAuctionId) external;
}
