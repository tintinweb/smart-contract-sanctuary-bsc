    ...
    Ownable
{
    ...
    constructor() {
        ...
        if (block.chainid == 97) {
            tokenAddress = 0x057aF44e768a39B92030DEFAF8BbAF3B4EdcDF7e; //TESTNET FORK
...
contract BABYBILLMURRAY is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        address _teamWallet
    ) ERC20("Baby BillMurray", "$BBM (RCC)", 9) {
        ...
        maxSellTransactionAmount = (_totalSupply * 1) / 100; // 1% maxSellTransactionAmount
        swapTokensAtAmount = (_totalSupply * 1) / 1000; // 0.1% swap tokens amount
        maxWallet = (_totalSupply * 2) / 100; // 1% Max wallet
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (
                ...
                super._transfer(from, marketingWallet, sniperFees); //transfer 99% of fee for marketing
            }
            // on sell
                ...
                tokensForBurn += (fees * rccBurnSellFee) / totalSellFees;
            }
            // on buy
                ...
                tokensForBurn += (fees * rccBurnBuyFee) / totalBuyFees;
            }
    ...
    function swapEthForRcc(uint256 ethAmount) private {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(RealCoinCasino);
    ...
    function swapAndLiquify() private {
        ...
        uint256 ethForBurn = ethBalance -
            ethForLiquidity -
            ethForMarketing -
            ethForRewards -
            ethForTeam;

        uint256 ethForRcc = ethForLiquidity / 2;

        uint256 initialRCC = RealCoinCasino.balanceOf(address(this));

        swapEthForRcc(ethForBurn.add(ethForRcc));

        uint256 rccBalance = RealCoinCasino.balanceOf(address(this)).sub(
            initialRCC
        );

        uint256 totalFeeForRcc = rccBurnSellFee.add(liquiditySellFee / 2);
        uint256 rccToBurn = (rccBalance / rccBurnSellFee) / totalFeeForRcc;

        RealCoinCasino.transfer(DEAD_ADDRESS, rccToBurn);
        ...
        uint256 liquidityTokens = rccBalance.sub(rccToBurn);

        if (liquidityTokens > 0 && ethForLiquidity > 0) {
            addLiquidityRCC(liquidityTokens, ethForLiquidity);
            emit SwapAndLiquify(
                liquidityTokens,
                ethForLiquidity,
                tokensForLiquidity
