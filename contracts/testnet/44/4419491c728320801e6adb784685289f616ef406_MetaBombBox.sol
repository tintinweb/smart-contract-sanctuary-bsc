...
// File: contracts/MetaBombBoxFactory.sol
...
contract MetaBombBoxFactory is Ownable {
    ...
    function batchMint(address[] memory addrs, uint256[] memory types) external onlyOperator {
        for (uint256 i = 0; i < addrs.length; i++) {
            metaBombBox._mintBatchToAddress(addrs[i], 2, types);
        }
    }
    ...
    function setAddressTransferOrReceive(address _add, bool _isTransfer) external onlyOwner {
        metaBombBox.setAddressTransferOrReceive(_add, _isTransfer);
    }
}