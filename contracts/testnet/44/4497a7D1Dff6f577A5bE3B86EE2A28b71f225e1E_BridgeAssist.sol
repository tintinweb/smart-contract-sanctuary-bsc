...
// File: Copy_Nexus-Bridge-triple_flat.sol


// File: contracts/nexus/Copy_Nexus-Bridge-triple.sol
...
contract BridgeAssist {
    ...
    function collect(address _sender, uint256 _amount) public restricted returns (bool success) {
        require(tokenInterface.transferFrom(_sender, address(this), _amount), "transferFrom() failure. Make sure that your balance is not lower than the allowance you set");
        emit Collect(_sender, _amount);
        return true;
    }

    function dispense(address _sender, uint256 _amount) public restricted returns (bool success) {
        require(tokenInterface.transfer(_sender, _amount), "transfer() failure. Contact contract owner");
