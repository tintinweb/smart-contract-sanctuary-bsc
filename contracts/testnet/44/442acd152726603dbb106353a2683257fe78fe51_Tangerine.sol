...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.4.1/contracts/math/Math.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.4.1/contracts/math/SafeMath.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.4.1/contracts/GSN/Context.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v3.4.1/contracts/token/ERC20/ERC20Burnable.sol
...
contract Tangerine is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 48h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 17000 ether;
    // Initial distribution for the day 2-5 TANGERINE-BNB LP -> TANGERINE pool
    uint256 public constant INITIAL_TANGERINE_POOL_DISTRIBUTION = 0 ether;
    ...
    address public tangerineOracle;
    ...
    /**
     * @notice Constructs the TANGERINE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("TANGERINE", "Tangerine") {
        // Mints 1 TANGERINE to contract creator for initial pool setup
    ...
    function _getTangerinePrice() internal view returns (uint256 _tangerinePrice) {
        try IOracle(tangerineOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Tangerine: failed to fetch TANGERINE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _tangerinePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_tangerinePrice >= taxTiersTwaps[tierId]) {
    ...
    function setTangerineOracle(address _tangerineOracle) public onlyOperatorOrTaxOffice {
        require(_tangerineOracle != address(0), "oracle address cannot be 0 address");
        tangerineOracle = _tangerineOracle;
    }
    ...
    /**
     * @notice Operator mints TANGERINE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of TANGERINE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentTangerinePrice = _getTangerinePrice();
            currentTaxRate = _updateTaxRate(currentTangerinePrice);
            if (currentTangerinePrice < burnThreshold) {
                burnTax = true;
            }
        }
