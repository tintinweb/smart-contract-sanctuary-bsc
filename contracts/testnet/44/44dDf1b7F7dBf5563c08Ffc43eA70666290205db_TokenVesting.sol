...
contract TokenVesting is Ownable {
  ...
  uint256 public monthPeriod; //2592000

  event UserStatus(address user, bool status);
  event UpdateTgeConfig(uint256 tgeTime, uint256 tgePercent);
  event UpdateMonthConfig(uint256 monthTime);
  ...
  constructor(
    ...
    require(
      _completeVestingPeriod >=
        _monthPeriod + _cliffTime + _secondDistributionTime,
   ...
   * update month config
   */
  function updateMonthConfig(uint256 _monthTime)
    external
    onlyOwner
    returns (bool)
  {
    require(_monthTime > tgeTime, "Month period is too low");
    require(
      monthPeriod.add(vestingStartTime) >= block.timestamp,
      "Month already done"
    );
    if (_monthTime > 0) {
      monthPeriod = _monthTime;
    }

    emit UpdateMonthConfig(_monthTime);
    return true;
  }
    ...
    returns (bool)
  {
    ...
    emit EmergencyWithdrawToken(_to, _amount);
    return true;
  }
  ...
  function unlock(address _of) external returns (uint256 unlockableTokens) {
    ...
    require(!blackListUser[_of], "Can not claim tokens");
