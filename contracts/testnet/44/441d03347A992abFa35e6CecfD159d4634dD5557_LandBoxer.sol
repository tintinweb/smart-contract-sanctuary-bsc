...
contract LandBoxer is Context, Ownable {
    ...
    modifier onlyCapacityEnoughGlobalBatch(uint256[] memory _scales, uint256[] memory _amounts) {
        ...
        for(uint256 i=0; i<_scales.length; i++) {
            ...
            require(globalPrices[_scales[i]].price > 0, "no p info");
        }
        _;
    }
    ...
    modifier onlyCapacityEnoughBatch(string[] memory _zoneIds, uint256[] memory _scales, uint256[] memory _amounts) {
        ...
        for(uint256 i=0; i<_zoneIds.length; i++) {
            require(_isValidScale(_scales[i]));
            require(globalScaleCapacity[_scales[i]] >= _amounts[i], "no enough capacity");
            require(_getPriceByZoneIdAndScale(_getZoneId(_zoneIds[i]), _scales[i]).price > 0, "no z info");
    ...
    function capacityRemaining(string memory _zoneId) public view returns (uint256[] memory, uint256[] memory) {
        bytes32 zoneId = _getZoneId(_zoneId);
        return (landCapacities[zoneId].scales, landCapacities[zoneId].capacities);
    }

    function capacityRemaining(uint256 _scale) public view onlyValidScale(_scale) returns (uint256 amount) {
        return globalScaleCapacity[_scale];
    }
    ...
    function addOrUpdateZone(ZoneInfo memory _zoneInfo) public onlyOwner {
    ...
    function _metaTrade(bytes32 _zoneId, uint256 _scale, PriceInfo memory _priceInfo, uint256 _amount) private {
        ...
        for (uint256 i=0; i<_amount; i++) {
            tokenIds[i] = IERC721WithAutoId(landAddress).currentId();
            IERC721WithAutoId(landAddress).mint(_msgSender());
            seeds[i] = _generateSeed(i);
        }
        require(IERC721WithAutoId(landAddress).currentId() == tokenId.add(_amount), "something wrong");
        landCapacities[_zoneId].capacities[_getIndexByZoneIdAndScale(_zoneId, _scale)] = landCapacities[_zoneId].capacities[_getIndexByZoneIdAndScale(_zoneId, _scale)].sub(_amount);
    ...
    function _generateSeed(uint256 nonce) private view returns(uint256 seed) {
        seed = LibSeedGenerator.generateRandomSeed(nonce);
    }
    ...
    function _getFirstRandomZoneIdWithEnoughCapacity(uint256 nonce, uint256 _scale, uint256 _amount) private view returns (bytes32 zoneId) {
        ...
        while (capacityRemaining(_getZoneIdentifierByZoneId(zoneId), _scale) < _amount) {
    ...
    function _refundRemainEth(uint256 refundAmount) private {
        if (refundAmount > 0) {
            ...
            require(sent, "refund user Ethers failure");
        }
    }
}
...
library LibSeedGenerator {
    ...
    function offsetBlockNumber(uint256 limit) public view returns (uint256 blockNum) {
        ...
        return block.number.sub(Math.min(block.number, randomOffset));
    }
