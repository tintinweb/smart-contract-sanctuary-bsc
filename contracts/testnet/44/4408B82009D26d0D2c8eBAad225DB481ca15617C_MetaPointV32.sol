...
contract MetaPointV32 is Ownable {
    ...
    event LogUp(address indexed addr, address indexed nextAddr, uint256 indexed nextSn, bool isLeft);
    ...
    function take(address parent, address addr) private returns (uint32) {
        ...
        while (!queues[parent].empty()) {
            ...
            if (nodes[current].lid == 0) {
                ...
                emit LogUp(parent, addr, next, true);
                return next;
            } else {
            ...
            if (nodes[current].rid == 0) {
                ...
                emit LogUp(parent, addr, next, false);
                return next;
            } else {
