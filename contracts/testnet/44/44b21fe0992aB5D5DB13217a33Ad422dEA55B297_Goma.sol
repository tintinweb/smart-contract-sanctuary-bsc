...
contract Goma is Context, IBEP20, Ownable {
    ...
    string private _symbol = "GOMAV2T9";
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tReward) private view returns (uint256, uint256, uint256, uint256) {
        uint256 currentRate = _getRate();
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rReward = tReward.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee);
        return (rAmount, rTransferAmount, rFee, rReward);
    }
    ...
    function _transferFromExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 rReward) = _getRValues(tAmount, tFee, tReward);
