...
contract Mining {
    ...
    /** ============== TEST ONLY ================ */
    function removeDuration(uint256 miningId) public {
        vault[msg.sender][miningId].timestamp = 0;
        vault[msg.sender][miningId].duration = 0;
    }
    ...
    function initialize() external {
        ...
        roleToPackage[PACKAGE1].amount = 330000 * 10**18;
        roleToPackage[PACKAGE1].duration = 360;
        roleToPackage[PACKAGE2].amount = 800000 * 10**18;
        roleToPackage[PACKAGE2].duration = 180;
        roleToPackage[PACKAGE3].amount = 2000000 * 10**18;
        roleToPackage[PACKAGE3].duration = 90;
    }
    ...
    function setProposal(uint256 percentChange, uint8 action) external {
        ...
        uint256 amountChangePackage1 = (roleToPackage[PACKAGE1].amount *
            percentChange) / 100;
        uint256 amountChangePackage2 = (roleToPackage[PACKAGE2].amount *
            percentChange) / 100;
        uint256 amountChangePackage3 = (roleToPackage[PACKAGE3].amount *
            percentChange) / 100;
        if (action == 0) {
            roleToPackage[PACKAGE1].amount -= amountChangePackage1;
            roleToPackage[PACKAGE2].amount -= amountChangePackage2;
            roleToPackage[PACKAGE3].amount = amountChangePackage3;
        } else {
            roleToPackage[PACKAGE1].amount += amountChangePackage1;
            roleToPackage[PACKAGE2].amount += amountChangePackage2;
            roleToPackage[PACKAGE3].amount += amountChangePackage3;
        ...
        returns (Package[3] memory)
    {
        ...
        if (nftId != 0) {
            ...
            finalPackage[0].amount -= ((finalPackage[0].amount *
                nftDetail.reduce) / 100);
            finalPackage[1].amount -= ((finalPackage[1].amount *
                nftDetail.reduce) / 100);
            finalPackage[2].amount -= ((finalPackage[2].amount *
                nftDetail.reduce) / 100);
        }
        return finalPackage;
    }
    ...
    function mint(uint8 _miningType, uint256 _nftId) external {
        ...
        Package memory finalPackage = getListPackage(_nftId)[_miningType];
