...
abstract contract SSV2sBase is Context, IERC20Metadata, Ownable, ReentrancyGuard {
	...
	uint8 private _additionalBuyFee; //Additional % fee to apply on buy transactions.
	...
	uint8 private _salaryFee; // % of each transaction that will be used for salary tax
	...
	// TRANSACTION LIMIT
	uint256 private _transactionLimit = _totalTokens; //1 trillion The amount of tokens that can be sold at once
	...
	constructor (address routerAddress, address pinkAntiBot_) {
		...
		// Initialize PancakeSwap V2 router and SSV2s <-> BNB pair.  Router address will be: 0x10ed43c718714eb63d5aa57b78b54704e256024e or for testnet: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
		setPancakeSwapRouter(routerAddress);

		// 1% liquidity fee, 7% reward fee, 1% buyback fee, 3% marketing fee, 2% salary fee, 2% additional sell fee, 0% additional buy fee
		setFees(1, 7, 1, 3, 2, 2, 0);
	...
	function calculateFeeRate(address sender, address recipient) private view returns(uint256) {
		...
		if (applyFees) {
			...
			if (isPancakeSwapPair(sender)) {
				return _poolFee + _additionalBuyFee;
			}
			return _poolFee;
		}
	...
	// This function swaps a {tokenAmount} of SSV2s tokens for BNB and returns the total amount of BNB received
	function swapTokensForBNB(uint256 tokenAmount) internal returns(uint256) {
		uint256 initialBalance = address(this).balance;
		
		// Generate pair for SSV2s -> WBNB
	...
	// Returns how many more $SSV2s tokens are needed in the contract before triggering a swap
	...
	function setFees(uint8 liquidityFee, uint8 rewardFee, uint8 buybackFee, uint8 marketingFee, uint8 salaryFee, uint8 additionalSellFee, uint8 additionalBuyFee) public onlyOwner {
		require(liquidityFee >= 0 && liquidityFee <= 8, "Liquidity fee must be between 0% and 8%");
		require(rewardFee >= 0 && rewardFee <= 15, "Reward fee must be between 0% and 15%");
    	require(buybackFee >= 0 && buybackFee <= 5, "Buyback fee must be between 0% and 5%");
    	require(marketingFee >= 0 && marketingFee <= 5, "Marketing fee must be between 0% and 5%");
		require(salaryFee >= 0 && salaryFee <= 5, "Salary fee must be between 0% and 5%");
		require(additionalSellFee <= 16, "Additional sell fee cannot exceed 16%");
		require(additionalBuyFee <= 16, "Additional sell fee cannot exceed 16%");
		require(liquidityFee + rewardFee + buybackFee + marketingFee + salaryFee + additionalSellFee + additionalBuyFee <= 30, "Total fees cannot exceed 30%");
		
		_liquidityFee = liquidityFee;
		_rewardFee = rewardFee;
    	_buybackFee = buybackFee;
    	_marketingFee = marketingFee;
		_salaryFee = salaryFee;
		_additionalSellFee = additionalSellFee;
		_additionalBuyFee = additionalBuyFee;
		
		// Enforce invariant
		_poolFee = _rewardFee + _liquidityFee + _buybackFee + _marketingFee + _salaryFee;
	}
...
contract SSV2s is SSV2sBase {
	...
	uint256 private _totalBNBAsSSV2sClaimed; // The total number of BNB that was converted to SSV2s and claimed by all addresses
	mapping(address => uint256) private _bnbRewardClaimed; // The amount of BNB claimed by each address
	mapping(address => uint256) private _bnbAsSSV2sClaimed; // The amount of BNB converted to SSV2s and claimed by each address
	...
	mapping(address => uint256) private _claimRewardAsTokensPercentage; //Allows users to optionally use a % of the reward pool to buy SSV2s automatically
	...
	bool private _reimburseAfterSSV2sClaimFailure; // If true, and SSV2s reward claim portion fails, the portion will be given as BNB instead
	...
	CustomTaxPeriod private _SSV21 = CustomTaxPeriod('SSV21', 3600,7,3,2,1,1,16,0);
	CustomTaxPeriod private _SSV22 = CustomTaxPeriod('SSV22', 3600,7,3,2,1,1,8,0);
	...
	constructor (address routerAddress, address pinkAntiBot_) SSV2sBase(routerAddress, pinkAntiBot_) {
	...
	function onActivated() internal override {
		...
		setReimburseAfterSSV2sClaimFailure(true);
		setMinRewardBalance(5000000000 * 10**decimals());  //At least 5 billion tokens are required to be eligible for rewards
	...
	function claimReward(address user) public {
		...
		require(isIncludedInRewards(user), "Address is excluded from rewards, make sure there is enough SSV2s balance");
	...
	function doClaimReward(address user) private returns (bool) {
		...
		if (!claimRewardToken(user, claimBnbAsTokens)) {
			...
			if (_reimburseAfterSSV2sClaimFailure) {
	...
	function claimRewardToken(address user, uint256 bnbAmount) private returns (bool) {
		...
		_bnbAsSSV2sClaimed[user] += bnbAmount;
		_totalBNBAsSSV2sClaimed += bnbAmount;
		return true;
	}
	...
	function calculateRewardCycleExtension(uint256 balance, uint256 amount) public view returns (uint256) {
		...
		if (balance == 0) {
			// Receiving $SSV2s on a zero balance address:
		...
		// Depending on the % of $SSV2s tokens transferred, relative to the balance, we might need to extend the period
    ...
    function bnbRewardClaimedAsSSV2s(address byAddress) public view returns (uint256) {
		return _bnbAsSSV2sClaimed[byAddress];
	}
    ...
    function totalBNBClaimedAsSSV2s() public view returns (uint256) {
		return _totalBNBAsSSV2sClaimed;
	}
	...
	function reimburseAfterSSV2sClaimFailure() public view returns(bool) {
		return _reimburseAfterSSV2sClaimFailure;
	}


	function setReimburseAfterSSV2sClaimFailure(bool value) public onlyOwner {
		_reimburseAfterSSV2sClaimFailure = value;
	}
	...
	function setHappyHour1Fees(uint256 liquidityFee,uint256 rewardFee,uint256 buybackFee,uint256 marketingFee,uint256 salaryFee,uint256 additionalSellFee,uint256 additionalBuyFee) public onlyOwner {
		_setCustomSellTaxPeriod(_SSV21,liquidityFee, rewardFee,buybackFee,marketingFee,salaryFee,additionalSellFee,additionalBuyFee);
	}

	function setHappyHour2Fees(uint256 liquidityFee,uint256 rewardFee,uint256 buybackFee,uint256 marketingFee,uint256 salaryFee,uint256 additionalSellFee,uint256 additionalBuyFee) public onlyOwner {
		_setCustomSellTaxPeriod(_SSV22,liquidityFee, rewardFee,buybackFee,marketingFee,salaryFee,additionalSellFee,additionalBuyFee);
	}
	...
	function _setCustomSellTaxPeriod(CustomTaxPeriod storage map,
		...
		if (map.liquidityFee != liquidityFee) {
				emit CustomTaxPeriodChange(liquidityFee, map.liquidityFee, 'liquidityFee', map.periodName);
				map.liquidityFee = liquidityFee;
		}
		if (map.rewardFee != rewardFee) {
				emit CustomTaxPeriodChange(rewardFee, map.rewardFee, 'marketingFee', map.periodName);
				map.rewardFee = rewardFee;
		}
		if (map.buybackFee != buybackFee) {
				emit CustomTaxPeriodChange(buybackFee, map.buybackFee, 'buyBackFee', map.periodName);
				map.buybackFee = buybackFee;
		}
		if (map.marketingFee != marketingFee) {
				emit CustomTaxPeriodChange(marketingFee, map.marketingFee, 'marketingFee', map.periodName);
				map.marketingFee = marketingFee;
		}
		if (map.salaryFee != salaryFee) {
				emit CustomTaxPeriodChange(salaryFee, map.salaryFee, 'salaryFee', map.periodName);
				map.salaryFee = salaryFee;
		}
		if (map.additionalSellFee != additionalSellFee) {
				emit CustomTaxPeriodChange(additionalSellFee, map.additionalSellFee, 'additionalSellFee', map.periodName);
				map.additionalSellFee = additionalSellFee;
		}
		if (map.additionalBuyFee != additionalBuyFee) {
				emit CustomTaxPeriodChange(additionalBuyFee, map.additionalBuyFee, 'additionalBuyFee', map.periodName);
				map.additionalBuyFee = additionalBuyFee;
		}
	}

	function getHappyHour1Fees() external view returns (uint256, uint256, uint256,uint256,uint256,uint256){
        return (_SSV21.liquidityFee,_SSV21.rewardFee, _SSV21.buybackFee, _SSV21.marketingFee, _SSV22.salaryFee, _SSV21.additionalSellFee);
	}

	function getHappyHour2Fees() external view returns (uint256, uint256, uint256,uint256,uint256,uint256){
        return (_SSV22.liquidityFee,_SSV22.rewardFee, _SSV22.buybackFee, _SSV22.marketingFee, _SSV22.salaryFee, _SSV22.additionalSellFee);
	}
	...
	function onBeforeCalculateFeeRate() internal override view returns (bool SSV2Flag, uint256 SSV2BuyFee, uint256 SSV2SellFee) {
		if(this.isInHappyHour()) {
			SSV2Flag = true;
			if(_isInHappyHour1()) {
				SSV2BuyFee = _SSV21.liquidityFee + _SSV21.rewardFee + _SSV21.buybackFee + _SSV21.marketingFee + _SSV21.salaryFee + _SSV21.additionalBuyFee;
				SSV2SellFee = _SSV21.liquidityFee + _SSV21.rewardFee + _SSV21.buybackFee + _SSV21.marketingFee + _SSV21.salaryFee + _SSV21.additionalSellFee;
			}
			else if (_isInHappyHour2()) {
 				SSV2SellFee = _SSV22.liquidityFee + _SSV22.rewardFee + _SSV22.buybackFee + _SSV22.marketingFee + _SSV22.salaryFee + _SSV22.additionalSellFee;
				SSV2BuyFee = _SSV22.liquidityFee + _SSV22.rewardFee + _SSV22.buybackFee + _SSV22.marketingFee + _SSV22.salaryFee + _SSV22.additionalBuyFee;
			}
