...
contract ProjectIDO is Ownable {
	...
	function getUsdtAmount(uint256 vnsAmount) internal view returns (uint256 usdtAmount) {
		if (config.vnsPrice == 0) {
			...
			address USDT = config.tokenAddress[uint256(TokenType.USDT_TOKEN)];
	...
	function ido(TokenType tokentype, uint256 amount) external {
		...
		require(IERC20(tAddress).balanceOf(msg.sender) >= amount, "invalid balance");
	...
	function startClaim(uint256 time) external onlyOwner {
		require(time > config.endTime, "invalid time0");
		startClaimTime = time;
	}
	...
	function claim(TokenType tokentype, uint256 amount) external {
		...
		UserInfo memory userInfo = IVNSMemberShip(config.memberShip).getUserInfo(msg.sender);
		uint256 pending = uamount / config.price;
		uint256 pAmount = (pending * config.parentRatio) / 100;
		
		IERC20(config.projectToken).transfer(userInfo.parent, pAmount);
		IERC20(config.projectToken).transfer(msg.sender, pending - pAmount);
		...
		if (tokentype == TokenType.VNS_TOKEN) {
			uint256 rewardAmount = (amount * (block.timestamp - user.firstIdoTime)) / (1000 * 1 days);
			require(IERC20(tAddress).balanceOf(address(this)) >= rewardAmount, "vns not enough");
