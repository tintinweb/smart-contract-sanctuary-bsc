...
abstract contract Pausable is Context {
     ...
     */
    function _setPaused(bool _isPaused) internal virtual {
        _paused = _isPaused;
...
contract ERC1155 is Ownable, Pausable , ERC165, IERC1155 {
     ...
     */
    function setApprovalForAll(address operator, bool approved) public virtual override defense {
    ...
    function safeTransferFrom(
        ...
        uint256 amount,
        bytes memory data
    ) public virtual override defense{
        require(
...
contract VBNFT is ERC1155{
    ...
    constructor() ERC1155("VBNFT","VBNFT") Ownable(){
    ...
    function composite(uint256 _nftId)public defense{
        ...
        if( upgradeMap[nextLevel].HydroDAOAmount > 0){
            ...
            IERC20(HydroDAO).burn(upgradeMap[nextLevel].HydroDAOAmount.div(2));
    ...
    function airdrop(uint256 _nftId, address[] memory _addr, uint256[] memory _nftAmount) external onlyOwner{
        ...
        for(uint256 i = 0; i < length ; i++){
            require(_nftAmount[i] <= 200,"Unsafe quantity");
