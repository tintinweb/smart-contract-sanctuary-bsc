...
contract Staking is IStaking, Ownable, ERC721Holder {
    ...
    function stake(
        ...
        uint256 _refCode,
        bytes memory _data
    ) public validPeriod(_stakingPeriod) {
        ...
        uint8 baseApy = nftTierApys[AnotherNFT(nft).getNftTier(_nftIds[0])];
        bool isValidNftArray = true;
        for (uint index = 1; index < _nftIds.length; index++) {
            uint8 currentApy = nftTierApys[AnotherNFT(nft).getNftTier(_nftIds[0])];
    ...
    function stakeExecute(
        ...
        for (uint index = 0; index < _nftIds.length; index++) {
            require(
                AnotherNFT(nft).ownerOf(_nftIds[index]) == msg.sender,
                "MARKETPLACE: NOT OWNER THIS NFT ID"
            );
            try AnotherNFT(nft).safeTransferFrom(msg.sender, address(this), _nftIds[index], _data) {
                // Emit event
    ...
    function unstake(uint256 _stakeId, bytes memory _data) public {
        ...
        for (uint index = 0; index < nftIds.length; index++) {
            try AnotherNFT(nft).safeTransferFrom(address(this), staker, nftIds[index], _data) {
                // Emit event
    ...
    function estimateValueUsdForListNft(uint256[] memory _nftIds) public view returns (uint256) {
        ...
        for (uint index = 0; index < _nftIds.length; index++) {
            uint256 priceNftUsd = AnotherNFT(nft).getNftPriceUsd(_nftIds[index]);
