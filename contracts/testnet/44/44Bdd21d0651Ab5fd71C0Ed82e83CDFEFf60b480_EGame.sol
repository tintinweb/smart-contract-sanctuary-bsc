...
contract EGame is ERC20 {
    ...
    event PlayerScore(address indexed playerAddress, uint256 playerScore);

    modifier onlyNaturalNumber(uint256 token_amount) {
        require(
            token_amount > 0,
            "Input number can only be natural number starting from 1, 2, 3, ... N"
        );
        _;
    }
    ...
    function claim(uint256 token_amount)
        external
        onlyNaturalNumber(token_amount)
        returns (bool)
    {
        ...
        if (!claimed[msg.sender]) {
            ...
            emit PlayerScore(msg.sender, token_amount);
        }
