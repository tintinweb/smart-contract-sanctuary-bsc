// contracts/XWorldDCEquipUpgrader.sol
...
contract XWorldDCEquipUpgrader_V3 is
    Context, 
    Pausable, 
    AccessControl,
    XWorldDCEquipStruct
{
    ...
    event XWorldDCEquipUpgrade(address indexed userAddr, uint256 indexed equipId, uint256[] fuelIds);
    event XWorldDCEquipUpgradeEx(address indexed userAddr, uint256 indexed equipId, uint256[] fuelIds, uint256 equipPiecesValue);

    XWorldDCEquip public _equipNFT;
    XWorldDCEquip1155 public _equip1155;

    mapping(uint8=>uint32) public _equipFuelExp; // indexed by equip grade, value is fuel equip experience
    mapping(uint8=>uint32) public _equipUpgradeExp; // indexed by level, value indicate how many exp is needed for upgrade to next level

    mapping(uint8=>uint32) public _equipLevelExps; // indexed by level, value indicate how many exp is needed for upgrade to this level from level 1
    ...
    function pause() public virtual {
        require(
            ...
            "XWorldDCEquipUpgrade: must have pauser role to pause"
        );
        _pause();
    }
    ...
    function unpause() public virtual {
        require(
            hasRole(PAUSER_ROLE, _msgSender()),
            "XWorldDCEquipUpgrade: must have pauser role to unpause"
        );
        _unpause();
    }

    function setEquipFuelExp(uint8 grade, uint32 exp) external {
        require(hasRole(MANAGER_ROLE, _msgSender()), "XWorldDCEquipUpgrade: must have manager role to manage");

        _equipFuelExp[grade] = exp;
    }

    function setEquipUpgradeExp(uint8 level, uint32 exp) external {
        require(hasRole(MANAGER_ROLE, _msgSender()), "XWorldDCEquipUpgrade: must have manager role to manage");

        _equipUpgradeExp[level] = exp;
    }

    function setEquipNFTAddress(address equipAddr) external {
        require(hasRole(MANAGER_ROLE, _msgSender()), "XWorldDCEquipUpgrade: must have manager role to manage");

        _equipNFT = XWorldDCEquip(equipAddr);
    }

    function setEquip1155Address(address equip1155Addr) external {
        require(hasRole(MANAGER_ROLE, _msgSender()), "XWorldDCEquipUpgrade: must have manager role to manage");

        _equip1155 = XWorldDCEquip1155(equip1155Addr);
    }

    function calcEquipLevelExps(uint8 maxLevel) external {
        require(hasRole(MANAGER_ROLE, _msgSender()), "XWorldDCEquipUpgrade: must have manager role to manage");

        for(uint8 lv = 1; lv <= maxLevel; ++lv) {
            _equipLevelExps[lv] = _equipUpgradeExp[lv-1] + _equipLevelExps[lv-1];
        }
    }

    function upgradeEquip(uint256 equipId, uint256[] calldata fuelIds) external {
        require(_equipNFT.ownerOf(equipId) == _msgSender(), "XWorldDCEquipUpgrade: equip ownership error");

        uint32 totalExp = 0;
        for(uint i = 0; i< fuelIds.length; ++i){
            require(_equipNFT.ownerOf(fuelIds[i]) == _msgSender(), "XWorldDCEquipUpgrade: equip ownership error");

            EquipData memory fuelEquipData = _equipNFT.getNftData(fuelIds[i]);
            totalExp += _equipFuelExp[fuelEquipData.grade]; // add exp
            
            // add exp on equip
            totalExp += fuelEquipData.exp;
            totalExp += _equipLevelExps[fuelEquipData.level];
            _equipNFT.burn(fuelIds[i]); // burn equip
            require(!_equipNFT.exists(fuelIds[i]), "XWorldDCEquipUpgrade: burn fuel failed");
        }

        require(totalExp > 0, "XWorldDCEquipUpgrade: no exp added");

        EquipData memory eqpData = _equipNFT.getNftData(equipId);
        eqpData.exp += totalExp;

        uint32 nextLevelExp = _equipUpgradeExp[eqpData.level];
        for(;nextLevelExp > 0 && eqpData.exp >= nextLevelExp;) {

            eqpData.level++; // level up
            eqpData.exp -= nextLevelExp; // sub exp

            // check next level
            nextLevelExp = _equipUpgradeExp[eqpData.level];
        }

        _equipNFT.modifyNftData(equipId, eqpData);

        emit XWorldDCEquipUpgrade(_msgSender(), equipId, fuelIds);
    }


    function upgradeEquipEx(uint256 equipId, uint256[] calldata fuelIds, uint256 fuel1155Value) external {
        require(_equipNFT.ownerOf(equipId) == _msgSender(), "XWorldDCEquipUpgrade: equip ownership error");
        require(_equip1155.balanceOf(_msgSender(), 1) >= fuel1155Value, "XWorldDCEquipUpgrade: fuel pieces not enough");

        uint32 totalExp = 0;
        
        // add equip pieces exp
        totalExp += uint32(fuel1155Value * _equipFuelExp[1]); // equip pieces exp = grade 1 equip exp
        _equip1155.burn(_msgSender(), 1, fuel1155Value);
        ...
        emit XWorldDCEquipUpgradeEx(_msgSender(), equipId, fuelIds, fuel1155Value);
    }
}
