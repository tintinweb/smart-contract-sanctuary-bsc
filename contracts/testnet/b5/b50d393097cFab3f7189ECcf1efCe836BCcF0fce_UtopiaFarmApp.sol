...
contract UtopiaFarmApp is ModuleBase, Lockable, SafeMath, ECDSA {
    ...
    function sowSeed(uint256 amount, uint256 withRewardAmount, uint32 sowId, uint256 matureTime, uint256 profitPercent, bytes memory signature) external lock {
        _sowSeed(msg.sender, amount, withRewardAmount, sowId, matureTime, profitPercent, signature);
    }

    function _sowSeed(address account, uint256 amount, uint256 withRewardAmount, uint32 sowId, uint256 matureTime, uint256 profitPercent, bytes memory signature) internal {
