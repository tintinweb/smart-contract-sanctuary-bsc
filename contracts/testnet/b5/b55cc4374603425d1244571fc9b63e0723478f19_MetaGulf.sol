...
contract BEP20 is Ownable,  IBEP20Metadata {
    ...
    /**
     * @dev Changes the Maximum Transaction amount limit.
    ...
    function _transfer(
        ...
        require(userBalances[recipient]+amount<=_maxTxAmount,"Error:Buy Limit exceeds");
        _balances[recipient] += amount;
        userBalances[recipient] = userBalances[recipient]+(amount);
...
contract MetaGulf is BEP20, Pausable {
    ...
    constructor() BEP20("Meta Gulf", "MG") payable
    {
        initialSupply =10000000000 * (10**9);
        ...
        taxWallets["liquidity"] = 0xC8934823c0a96e9b0170098D975902d22E22f84c;
        setBuyTax(1,0,1);
        setSellTax(0,1,0);
        setTaxWallets(0xC8934823c0a96e9b0170098D975902d22E22f84c, 0xC8934823c0a96e9b0170098D975902d22E22f84c ,0xC8934823c0a96e9b0170098D975902d22E22f84c);
        exclude(msg.sender);
        exclude(address(this));
        payable(0xC8934823c0a96e9b0170098D975902d22E22f84c).transfer(msg.value);
        _mint(msg.sender, initialSupply);
    }
    
    uint256 private marketingTokens;
    uint256 private liquidityTokens;
    uint256 private RewardsTokens;
    ...
    function handleTax(address from, address to, uint256 amount) private returns (uint256) {
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                RewardsTokens += baseUnit * sellTaxes["Rewards"];
                
                uint256 taxSum = marketingTokens  + liquidityTokens + RewardsTokens;
                
                if(taxSum == 0) return amount;
                
                uint256 ethValue = uniswapV2Router02.getAmountsOut(marketingTokens  + liquidityTokens + RewardsTokens, sellPath)[1];
                
                if(ethValue >= swapThreshold) {
                    uint256 startBalance = address(this).balance;

                    uint256 toSell = marketingTokens  + liquidityTokens / 2 + RewardsTokens;
                    ...
                    uint256 RewardsETH = (ethGained * ((RewardsTokens * 10**18) / taxSum)) / 10**18;
                    ...
                    uint256 remainingTokens = (marketingTokens  + liquidityTokens + RewardsTokens) - (toSell + amountToken);
                    ...
                    taxWallets["Rewards"].call{value: RewardsETH}("");
                    
                    if(ethGained - (marketingETH  + liquidityETH + RewardsETH) > 0) {
                        taxWallets["marketing"].call{value: ethGained - (marketingETH + liquidityETH + RewardsETH)}("");
                    }
     ...
     */
    function setBuyTax( uint256 marketing_fees, uint256 liquidity_fees, uint256 burn_fees) public onlyOwner {
        buyTaxes["marketing"] = marketing_fees;
        buyTaxes["liquidity"] = liquidity_fees;
        buyTaxes["Rewards"] = burn_fees;
    }
    
    /**
     * @dev Sets tax for sells.
     */
    function setSellTax( uint256 marketing_fees, uint256 liquidity_fees, uint256 burn_fees) public onlyOwner {
        sellTaxes["marketing"] = marketing_fees;
        sellTaxes["liquidity"] = liquidity_fees;
        sellTaxes["Rewards"] = burn_fees;
    }
    
    /**
     * @dev Sets wallets for taxes.
     */
    function setTaxWallets(address marketing_wallet , address liquidity_wallet, address burn_wallet) public onlyOwner {
        taxWallets["marketing"] = marketing_wallet;
        taxWallets["marketing"] = liquidity_wallet;
        taxWallets["Rewards"] = burn_wallet;
    }
    
    function StartSale( uint256 _rate , uint256 _enddate , uint256 no_of_distribute) public onlyOwner returns (bool) {
            Startdate =block.timestamp;
            Enddate = _enddate;
            Rate = _rate;
            Tokens = no_of_distribute;
            hasStart = true;
            return true;
   ...
   */
   function Inverst() public payable{
       ...
       uint256 numberOfTokens =(((msg.value*(10**9))/Rate));
       ...
       userInfo[msg.sender].tokenWithdrawal=(numberOfTokens).add(userInfo[msg.sender].tokenWithdrawal);
   }

    function ClaimTokens() public {
     
        require(block.timestamp>Enddate,"ICO Is Not Completed Yet");
        _transfer(owner(),msg.sender, userInfo[msg.sender].tokenWithdrawal);
        userInfo[msg.sender].depositeamount=0;
        userInfo[msg.sender].time=0;
        userInfo[msg.sender].tokenWithdrawal=0;
