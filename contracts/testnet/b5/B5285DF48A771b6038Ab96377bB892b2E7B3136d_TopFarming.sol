...
contract TopFarming is Pausable, Ownable, ReentrancyGuard {
    ...
    function _harvest(uint256 _pid) internal {
        ...
        pool.rewardToken.safeTransfer(msg.sender, netAmount);

        if (_fee > 0) {
            pool.rewardToken.safeTransfer(feeRecipient, _fee);
        }
    ...
    function withdraw(address _rewardAddress, uint256 _amount) external onlyOwner {
