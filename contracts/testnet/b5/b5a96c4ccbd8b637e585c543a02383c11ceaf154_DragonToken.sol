...
contract DragonToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public teamAddress = payable(0x2F011f865356566568581FD5e7e16765e6e810DD); // Team Address
    ...
    function _transfer(
        ...
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            ...
            // Buy
            if(from == uniswapV2Pair && _buyTaxFee > 0){
                uint256 teamBuyFee = amount.mul(_buyTaxFee).div(100);
                _transfer(from, teamAddress, teamBuyFee);
                _liquidityFee = _buyLiquidityFee;
            }
            // Sell
            if(to == uniswapV2Pair && _sellTaxFee > 0){
                uint256 teamSellFee = amount.mul(_sellTaxFee).div(100);
                _transfer(from, teamAddress, teamSellFee);
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(teamAddress, transferredBalance.mul(teamDivisor).div(100));
