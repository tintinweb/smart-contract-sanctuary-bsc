...
contract test {
    ...
    address devWallet = 0x08163F8C90143341624Cc056986E60468C64ecC9;
...
/// Funkcije ki spreminjajo zgornje vrenosti
    ...
    function deliverRewards() public {
        

        if (block.timestamp >= IDToUser[rewardID].time) {
             totalID--;           
             uint256 x = 0;
    ...
    receive() payable external {
        ...
        rewardID = totalID;
        IDToUser[rewardID].time = block.timestamp + (timePassed * 60) ;
        totalID++;
        ...
        uint256 availableBalance = address(this).balance - reward;
