...
contract StorageOperator is KeeperCompatibleInterface, Ownable {
    ...
    constructor(address _fundAddress, uint256 _storageCostRate, uint256 _transactionFee ) {
    ...
    function _isValidMonth(uint256 month)internal view returns(bool){
        return (timeDuration == 3 && (month == 1 ||  month == 3 || month == 6 || month == 9 || month == 12 ))
               || (timeDuration == 6 && (month == 1 || month == 6 || month == 12 ));
    }
    ...
    function _isUpkeepNeeded() internal view returns (bool) {
        ...
        return ( day == performDate && monthDifference >= 1 && (timeDuration == 1 || _isValidMonth(month)));
    }
    ...
    function performUpkeep(
        ...
        if (_isUpkeepNeeded()) {    
            ...
            address[] memory _defaultOperators = token.defaultOperators();
            
            ITokenTransferOperator tokenTransferOperator = ITokenTransferOperator(_defaultOperators[0]); 
    ...
    function _deductStorageCost(IERC777 token, IBWGToken iBWGToken, ITokenTransferOperator tokenTransferOperator) internal {   
        ...
        uint256 _transactionFee = transactionFee;
        uint256 _totalCost;
        uint256 _storageCostRate = storageCostRate;      
       
        uint256 batchLastIndex = _batchLastIndex(costableWallets.length);
       
        if(lastIndex == 0 ){
            iBWGToken.pause();
        }  

        for (uint256 index = lastIndex; index < batchLastIndex; index = increament(index)) {
            address _balanceWallet = costableWallets[index];
            uint256 _balance = token.balanceOf(_balanceWallet);
          
            // calculating cost to deduct
            ...
            uint256 _cost = ((_balance * _storageCostRate)/10000) + _transactionFee;
                
            // deduct all token balance if balance is less than cost or double cost
            if (_balance <  (_cost * 2)){
                _cost = _balance;
            }
        ...
        if( batchLastIndex == costableWallets.length){
            lastPerformTimestamp = block.timestamp;
            lastIndex = 0;           
            iBWGToken.unpause();
        }else{
    ...
    /**
     * @notice The external method used to get getNextStorageCostRunDetails     
     */
    function getNextStorageCostRunDetails() external view returns ( uint256 totalStorageCost, uint256 totalWallet, uint256 totalWalletUnderThreshold, uint256 totalCostUnderThreshold ){
        IERC777 token = IERC777(baseContract);
        IBWGToken iBWGToken = IBWGToken(baseContract);             
        address[] memory costableWallets = iBWGToken.costableWallets();                 
    ...
    /**
     * @notice The external method used to reset lastIndex      
     */
    function resetLastIndex() external onlyOwner {
        lastIndex = 0;       
    }

    function getAvailableWalletsNextRun() external view returns (StorageCost[] memory storageCost, uint256 nextIndex){

        IERC777 token = IERC777(baseContract);
        IBWGToken iBWGToken = IBWGToken(baseContract);             
        address[] memory costableWallets = iBWGToken.costableWallets();         
        storageCost = new StorageCost[](batchSize);
     
        uint256 index =0;
        nextIndex = 0;
        while (costableWallets.length > nextIndex  && index < batchSize ) {            
             address balanceWallet = costableWallets[nextIndex];
             uint256 balance = token.balanceOf(balanceWallet);
             if (balance > 0 && !iBWGToken.isCostFreeWallet(balanceWallet)){                         
                 ...
                 storageCost[index] = StorageCost(balanceWallet, cost); 
                 index++;
             }  
            nextIndex++;      
        }    
    ...
    function offchainCostRun(StorageCost[] calldata storageCost, uint256 nextIndex, uint256 totalCost) external onlyOwner {
        
        if (lastIndex < nextIndex && _isUpkeepNeeded()){
            IERC777 token = IERC777(baseContract);       
            address[] memory _defaultOperators = token.defaultOperators();            
            ITokenTransferOperator tokenTransferOperator = ITokenTransferOperator(_defaultOperators[0]); 
            address _fundAddress = fundAddress;

            for (uint256 index = 0; index < nextIndex; index = increament(index)) {
                if (storageCost[index].wallet != address(0)){
                    tokenTransferOperator.operatorSend(
                        token,
                        storageCost[index].wallet,
                        _fundAddress,
