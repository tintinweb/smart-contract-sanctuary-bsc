...
contract BNBBeatsV2 is Ownable {
    ...
    function mapUser(address adr) public {
        UserOrigin storage user = usersOrigin[adr];
        (   uint256 hatcheryMiners_,
            uint256 claimedBeats_,
            uint256 lastHatch_,,,
            address referrals_,
            uint256 checkpoint
        ) = contractOrigin.userData(adr);
        user.referrals = referrals_;
        user.hatcheryMiners = hatcheryMiners_;
        user.claimedBeats = claimedBeats_;
        user.lastHatch = lastHatch_;
