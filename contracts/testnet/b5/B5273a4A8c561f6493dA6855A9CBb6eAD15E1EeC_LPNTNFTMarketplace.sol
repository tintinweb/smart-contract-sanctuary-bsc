/// @title LPNT NFT Marketplace V2
/// @notice contracts/LPNTNFTMarketplaceV2.sol
...
contract LPNTNFTMarketplace is
    ...
    /**
     * @notice Initialize function to initilize the default values.
     * @dev Initializing the ERC721 contract with URI Storage.
     * Also initilizing the Pausable nad Ownable Contracts.
     */
    function initialize(address _priceFeedAddress) public initializer {
        __ERC721_init("LPNT NFT Marketplace", "LPNTM");
     ...
     * @param priceInDollars: The token price in dollars.
    ...
    function mintNFTToAddressWithFixedPrice (
        ...
        require(priceInDollars > 0, "Dollars should not be zero.");
        ...
        uint256 tokenToReceive = getTokenAmount(priceInDollars, tokenPrice);

        require(
            IERC20Upgradeable(tokenAddress).transferFrom(msg.sender, address(this), tokenToReceive),
        ...
        marketItemMetadata.price = priceInDollars;
        ...
        marketItemMetadata.unlockAt = block.timestamp + (noOfMonths * 10); // 180 sconds
        marketItemMetadata.paymentAddress = tokenAddress;

        /// @notice Emiting the event.
        emit TokenMintedToAddress(tokenId, to, priceInDollars);
    }
    ...
    function refundNFT(uint256 tokenId) external nonReentrant whenNotPaused {
        ...
        /// @notice Requirements.
        require(_exists(tokenId), "TokenId doesnot exists.");
        ...
        require((marketItemMetadata.listedAt + (refundDays * 20)) >= block.timestamp, "You cannot refund anymore."); // 140 seconds
        ...
        uint256 priceInDollars = marketItemMetadata.price;

        uint256 tokenPrice = IPriceFeed(priceFeedAddress).getPrice(tokenAddress);
        uint256 tokens = getTokenAmount(priceInDollars, tokenPrice);
        ...
        /// @notice Emiting the event.
        emit TokenRefunded(tokenId, buyer, priceInDollars);
    }

    /**
     * @notice List NFT after holding period.
     * @param tokenId: The token Id which buyer wants to list in market.
     * @param priceInDollars: The dollar price of the token.
     */
    function listNFT(uint256 tokenId, uint priceInDollars) external whenNotPaused nonReentrant {
        MarketItemMetadata storage marketItemMetadata = marketItemByTokenId[tokenId];
        ...
        /// @notice Emiting the event.
        emit TokenListed(tokenId, msg.sender, priceInDollars);
    }
    ...
    function buyNFT(uint256 tokenId, uint paymentId) external whenNotPaused nonReentrant {
        ...
        uint256 tokenPrice = IPriceFeed(priceFeedAddress).getPrice(tokenAddress);
        uint256 tokenToSent = getTokenAmount(priceInDollars, tokenPrice);
     ...
     */
    function getTokenAmount(uint256 priceInDollars, uint256 tokenPriceInDollars) public pure returns(uint256) {
        uint256 usd = priceInDollars * (10 ** 18);
        uint256 token = tokenPriceInDollars * (10 ** 18);
        uint256 result = (usd * (10 ** 18))/ token;
        return result;
    }
...
/// @title Interface Price Feed
/// @notice contracts/IPriceFeed.sol
// SPDX-License-Identifier: ISC
pragma solidity ^0.8.10;
