...
library PresaleHelper {
    ...
    function calculateAmountRequired (uint256 _amount, uint256 _tokenPrice, uint256 _listingRate, uint256 _liquidityPercent, uint256 _tokenFee) public pure returns (uint256) {
        ...
        uint256 digichainTokenFee = _amount.mul(_tokenFee).div(1000);
        uint256 amountMinusFee = _amount.sub(digichainTokenFee);
        uint256 liquidityRequired = amountMinusFee.mul(_liquidityPercent).mul(listingRatePercent).div(1000000);
        uint256 tokensRequiredForPresale = _amount.add(liquidityRequired).add(digichainTokenFee);
...
contract Presale01 is ReentrancyGuard {
  ...
  struct PresaleInfo {
    ...
    uint256 START_BLOCK;
    uint256 END_BLOCK;
    uint256 LOCK_PERIOD; // timestamp -> e.g. 2 weeks
  ...
  struct PresaleFeeInfo {
    uint256 DIGICHAIN_BASE_FEE; // divided by 1000
    uint256 DIGICHAIN_TOKEN_FEE; // divided by 1000
  ...
  constructor(address _presaleGenerator) public {
    ...
    PRESALE_SETTINGS = IPresaleSettings(0x7D937a6daC946964174c6F13f032A9B66Ec03872);
    PRESALE_LOCK_FORWARDER = IPresaleLockForwarder(0x342bc8263f9AEE45571b05f6E26d9143FE0bCeb5);
    DIGICHAIN_DEV_ADDRESS = 0x248D51a074e7062ca465802A782FABdbf3EdBa68;
  }
  ...
  function init2 (
    IERC20 _baseToken,
    IERC20 _presaleToken,
    uint256 _digichainBaseFee,
    uint256 _digichainTokenFee,
    ...
    address payable _referralAddress
      ...
      PRESALE_FEE_INFO.DIGICHAIN_BASE_FEE = _digichainBaseFee;
      PRESALE_FEE_INFO.DIGICHAIN_TOKEN_FEE = _digichainTokenFee;
  ...
  function forceFailByDigichain () external {
      require(msg.sender == DIGICHAIN_DEV_ADDRESS);
      STATUS.FORCE_FAILED = true;
  }
  ...
  function addLiquidity() external nonReentrant {
    ...
    uint256 digichainBaseFee = STATUS.TOTAL_BASE_COLLECTED.mul(PRESALE_FEE_INFO.DIGICHAIN_BASE_FEE).div(1000);
    
    // base token liquidity
    uint256 baseLiquidity = STATUS.TOTAL_BASE_COLLECTED.sub(digichainBaseFee).mul(PRESALE_INFO.LIQUIDITY_PERCENT).div(1000);
    if (PRESALE_INFO.PRESALE_IN_ETH) {
    ...
    // transfer fees
    uint256 digichainTokenFee = STATUS.TOTAL_TOKENS_SOLD.mul(PRESALE_FEE_INFO.DIGICHAIN_TOKEN_FEE).div(1000);
    // referrals are checked for validity in the presale generator
    if (PRESALE_FEE_INFO.REFERRAL_FEE_ADDRESS != address(0)) {
        // Base token fee
        uint256 referralBaseFee = digichainBaseFee.mul(PRESALE_FEE_INFO.REFERRAL_FEE).div(1000);
        ...
        digichainBaseFee = digichainBaseFee.sub(referralBaseFee);
        // Token fee
        uint256 referralTokenFee = digichainTokenFee.mul(PRESALE_FEE_INFO.REFERRAL_FEE).div(1000);
        TransferHelper.safeTransfer(address(PRESALE_INFO.S_TOKEN), PRESALE_FEE_INFO.REFERRAL_FEE_ADDRESS, referralTokenFee);
        digichainTokenFee = digichainTokenFee.sub(referralTokenFee);
    }
    TransferHelper.safeTransferBaseToken(address(PRESALE_INFO.B_TOKEN), PRESALE_FEE_INFO.BASE_FEE_ADDRESS, digichainBaseFee, !PRESALE_INFO.PRESALE_IN_ETH);
    TransferHelper.safeTransfer(address(PRESALE_INFO.S_TOKEN), PRESALE_FEE_INFO.TOKEN_FEE_ADDRESS, digichainTokenFee);
