...
contract GoldenNugget is Context, Ownable {
    ...
    event PaidBonus(address indexed receiver, uint256 indexed amount);
    ...
    function BuryGold(address ref) public {
        ...
        recAdd.transfer(devFee(GoldUsed));
        internalPool = SafeMath.add(internalPool, bonusFee(GoldUsed));
        GoldUsed = SafeMath.sub(SafeMath.sub(GoldUsed, devFee(GoldUsed)), bonusFee(GoldUsed));
        ...
        if(setRndNum) {
            ...
            if(randomNumber == reinvestmentCount) {
                lastWinner.addr = msg.sender;
                lastWinner.bonusAmount = internalPool;
                payable(msg.sender).transfer(internalPool);
                emit PaidBonus(msg.sender, internalPool);
                totalBonus = SafeMath.add(totalBonus, internalPool);
                internalPool = 0;
                reinvestmentCount = 0;
                setRndNum = false;
                randomNumber = 0;
    ...
    function seedMarket() public payable onlyOwner {
        ...
        initialized = true;
        marketGold = 1080000000000;
        lastWinner.addr = address(0);
        lastWinner.bonusAmount = 0;
    }
    ...
    function requestRandomNumber(uint256 _startNumber, uint256 _endNumber) private view returns (uint256) {
        return SafeMath.add(SafeMath.mod(block.timestamp, SafeMath.sub(_endNumber, _startNumber)), _startNumber);
    }

    function getLastWinner() public view returns (address, uint256) {
        require(initialized);
        return (lastWinner.addr, lastWinner.bonusAmount);
    }
    function transferOwnership(address newOwner) public override onlyOwner {
        _transferOwnership(newOwner);
        recAdd = payable(newOwner);
    }
}