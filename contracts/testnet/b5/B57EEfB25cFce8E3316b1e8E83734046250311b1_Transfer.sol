...
contract Transfer is ReEntrancyGuard, Ownable {
     ...
     */
    function adminWithdraw(uint256 _amount, string memory _tokenTypeSymbol)
        public
        onlyAdmin
        pausable
        noReentrant
    {
        if (keccak256(bytes(_tokenTypeSymbol)) == keccak256("USDT")) {
            ...
            require(
                IERC20(busdAddress).transfer(msg.sender, _amount),
                "TransferTokensFor: Failed to transfer tokens from the vendor to the admin"
            );
            ...
            require(
                payable(address(msg.sender)).send(_amount),
        ...
        emit WithdrawAdminEvent(msg.sender, _amount, _tokenTypeSymbol);
    }
    ...
    function withdraw(
        ...
        emit WithdrawEvent(_wallets, _amounts, _tokenTypeSymbol);
    }
