...
contract plan is owned {
    ...
    uint40 public token_per_bnb = 1;
    uint256 public min_tokens = 1;
    uint256 private distribute_persent = 20;
    uint256 private reward_persent = 6;
    ...
    bool private isEventTokenDistrubute;
    uint256 private noOfTokenEventDistribute;
    ...
    mapping(address => eventIncome) private eventIncomes;
    ...
    function buy(address sponsor) public payable returns(bool) {
        //msg.value = amnt;
        require(msg.value>0,"Amount Should be greater then 0");        
        stake(sponsor, (msg.value/(10**14)   * token_per_bnb));  
        return true;
    }

    function stake(address _upline, uint256 _tokens) public returns (bool){      
    ...
    function _stake(address _upline, uint256 _tokens) internal {        
        ...
        users[msg.sender].reward_persent = reward_persent;
            ...
            if(upline != address(0)){
                if(users[upline].directs>i){
                    if(users[upline].deposit_time>0){
                       uint256 tcom = users[msg.sender].deposit_amount * (level_commision[i]/plan_diveder)/100;
                        ...
                        if(isEventOn == true && (event_start_time <= block.timestamp && event_end_time >= block.timestamp)){
                            if(eventIncomes[upline].added_time <= event_start_time){
                                ...
                                eventIncomes[upline].income = eventIncomes[upline].income + tcom;
                            }
    ...
    function unstake() public returns(bool){
        ...
        if(users[msg.sender].upto <= block.timestamp){
           addition = (users[msg.sender].deposit_amount * users[msg.sender].reward_persent / 100) ;
        }

        stakingToken.transfer(msg.sender, (users[msg.sender].deposit_amount+addition));
    ...
    function setlevel_distribute_persent(uint256 _percentage) public onlyOwner returns (bool){
        reward_persent = _percentage;
        return true;
    }
    function setmin_tokens(uint256 _min_tokens) public onlyOwner returns (bool){
        min_tokens = _min_tokens;
        return true;
    }
    
    function setdistribute_persent(uint256 _percentage) public onlyOwner returns (bool){
        distribute_persent = _percentage;
        return true;
    }
