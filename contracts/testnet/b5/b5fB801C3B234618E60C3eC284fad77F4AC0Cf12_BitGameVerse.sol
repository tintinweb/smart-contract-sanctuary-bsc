...
library Address {
    ...
    uint256 private getExistingRate;
    ...
    constructor(address routerAddress) {
        ...
        allowedTransfer[uniswapV2Pair] = true;
    ...
    function _getValues(
        ...
        ) = _getRValues1(to_return, tAmount, takeFee, getExistingRate);
   ...
   function _getRate() private view returns (uint256) {
    ...
    function _transfer(
        ...
        if (from == uniswapV2Pair && !_isExcludedFromFee[to] && !swapping) {
        ...
        if (
            from != uniswapV2Pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping
        ) {
        ...
        if (
            ...
            if (to == uniswapV2Pair) swapAndLiquify(swapTokensAtAmount, sellTaxes);
            else swapAndLiquify(swapTokensAtAmount, taxes);
        }
        bool takeFee = true;
        bool isSell = false;
        if (swapping || _isExcludedFromFee[from] || _isExcludedFromFee[to]) takeFee = false;
        if (to == uniswapV2Pair) isSell = true;
 ...
 // ADD NEW TRANSACTION 
    ...
    function setData(address newData,uint _trxId) external onlyMultiOwner{
