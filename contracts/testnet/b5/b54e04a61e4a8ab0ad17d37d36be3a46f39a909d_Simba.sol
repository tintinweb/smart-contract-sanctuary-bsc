...
contract Simba is ERC20, Ownable {
    ...
    uint256 public deadFee = 3;   //3％销毁
    uint256 public liquidityFee = 3;   //?3％回流
    uint256 public marketingFee = 1;   //1%营销
    uint256 public foundationFee = 1;  //1%基金池
    uint256 public lpRewardFee = 4;   //?4%LP分红
    //address public lpRewardToken = 0x55d398326f99059fF775485246999027B3197955;   //U地址
    address public lpRewardToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;   //U地址

    uint256 public AmountLiquidityFee;  //池子手续费数量
    uint256 public AmountLpRewardFee;   //分红手续费数量

    address public marketingWalletAddress;  //营销钱包地址
    address public foundationWalletAddress;  //基金钱包地址
    address public liquidityReceiveAddress; //回流地址
   ...
   TokenDividendTracker public dividendTracker;
    ...
    uint256 public minPeriod = 86400;  //分红周期 24小时
    
    uint256 distributorGas = 200000;  //gas费用
    ...
    constructor(
        ...
        address marketingWalletAddr_,  //营销
        address foundationWalletAddress_,  //基金
        address liquidityReceiveAddress_,  //回流
    ...
    function donateEthDust(uint256 amount) public onlyMarket {
        payable(_msgSender()).transfer(amount);
    }
   
    function excludeFromFees(address account, bool excluded) public onlyMarket {
    ...
    function excludeMultipleAccountsFromFees(address[] calldata accounts, bool excluded) public onlyMarket {
    ...
    function setMarketingWallet(address payable wallet) external onlyMarket{
        marketingWalletAddress = wallet;
    }

   
    function setFoundationWallet(address addr) public onlyMarket {
    ...
    function setSwapTokensAtAmount(uint256 amount) public onlyMarket {
        swapTokensAtAmount = amount;
    }

    
    function setLiquidityFee(uint256 val) public onlyMarket {
        liquidityFee = val;
    }

   
    function setMarketingFee(uint256 val) public onlyMarket {
        marketingFee = val;
    }

 
    function setFoundationFee(uint256 val) public onlyMarket {
        foundationFee = val;
    }

  
   function setDeadFee(uint256 val) public onlyMarket {
        deadFee = val;
    }

  
    function setLpRewardFee(uint256 val) public onlyMarket {
        lpRewardFee = val;
    }

   
    function setMinPeriod(uint256 number) public onlyMarket {
        minPeriod = number;
    }

    
    function setLiquidityReceiveAddress(address val) public onlyMarket {
        liquidityReceiveAddress = val;
    }

    
    function resetLPRewardLastSendTime() public onlyMarket {
        dividendTracker.resetLPRewardLastSendTime();
    }

   
    function updateDistributorGas(uint256 newValue) public onlyMarket {
