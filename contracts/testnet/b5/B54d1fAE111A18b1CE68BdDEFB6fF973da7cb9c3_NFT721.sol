...
contract NFT721 is Ownable, SignerRole, IERC721, ERC721Base, ExchangeDomain {
    ...
    function mint(uint256 tokenId, uint8 v, bytes32 r, bytes32 s, Fee[] memory _fees, string memory tokenURI, uint price, AssetType assetType, address addressToken)  payable public  {
        require(_startTime <= block.timestamp || (_startTime >= block.timestamp && (_whitelists[msg.sender] == true ||  _airdrops[msg.sender] > 0) ), 'Can not purchase this time');
        ...
        if (_airdrops[msg.sender] == 0) {
            transfer(owner(), assetType,price, addressToken);    
        } else {
           _airdrops[msg.sender]++;
        }
    ...
    function setWhitelist(address _addr) public onlyOwner {
        _whitelists[_addr] = true;
    }

    function setAirdrop(address _addr) public onlyOwner {
        if (_airdrops[_addr] == 0) {
            _airdrops[_addr] = 2;
        }
    }

    function getWhiteList(address _addr) public view returns  (bool status) {
        return _whitelists[_addr];
    }

    function setTimeInfo(uint256 startTime) public onlyOwner {
