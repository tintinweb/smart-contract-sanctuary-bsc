...
interface IWhaleAntiBot {
    ...
    function checkHasPayFee() external view returns (bool);

    function settingPayFee() external view returns (bool);
}

contract TokenAntiBot is IERC20, Ownable {
    ...
    constructor(
        ...
        // Initiate whaleAntiBot instance from its address
        whaleAntiBot = IWhaleAntiBot(whaleAntiBot_);
        // Register deployer as the owner of this token with whaleAntiBot contract
        whaleAntiBot.setTokenOwner(msg.sender);
        whaleAntiBot.setTokenAntiBotAddress(address(this));
    ...
    function setUsingAntiBot(bool enabled_) external payable onlyOwner {
        bool hasPayFee = whaleAntiBot.checkHasPayFee();
        if (hasPayFee == false) {
            require(msg.value == 10000000000000000, "Fee Service incorrect");
            whaleAntiBot.settingPayFee();
        }
    ...
    function _transfer(
        ...
        if (antiBotEnabled) {
            ...
            whaleAntiBot.onPreTransferCheck(sender, recipient, amount);
        }
