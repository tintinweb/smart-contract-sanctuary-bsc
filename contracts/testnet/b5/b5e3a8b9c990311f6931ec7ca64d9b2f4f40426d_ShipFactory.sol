...
contract ShipFactory {
  event DeployedNewShip(address newShipAddress);
  function deployShip() public {
    bytes memory bytecode = type (Ship).creationCode;
    bytes32 salt = keccak256(abi.encodePacked());

    address newShipAddress;

    assembly {
      newShipAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
    }
    emit DeployedNewShip(newShipAddress);
  }
}