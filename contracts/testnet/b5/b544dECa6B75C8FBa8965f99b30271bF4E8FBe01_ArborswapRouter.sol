...
import './interfaces/IArborswapRouterPair.sol';
import './interfaces/IArborswapRouterPairs.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract ArborswapRouter is IArborswapRouter {
    ...
    modifier onlyAggregator() {
        require(feeAggregator == msg.sender, "ArborswapRouter: ONLY_FEE_AGGREGATOR");
        _;
    }
    ...
    function _addLiquidity(
        ...
        if (IArborswapFactory(baseFactory).getPair(tokenA, tokenB) == address(0)) {
            IArborswapFactory(baseFactory).createPair(tokenA, tokenB);
        }
        (uint256 reserveA, uint256 reserveB) = IArborswapRouterPairs(routerPairs).getReserves(baseFactory, tokenA, tokenB);
    ...
    returns (uint256 amountA, uint256 amountB, uint256 liquidity) {
        ...
        address pair = IArborswapRouterPairs(routerPairs).pairFor(baseFactory, tokenA, tokenB);
        TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
        TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
        liquidity = IArborswapRouterPair(pair).mint(to);
    }
    ...
    returns (uint256 amountToken, uint256 amountETH, uint256 liquidity) {
        ...
        address pair = IArborswapRouterPairs(routerPairs).pairFor(baseFactory, token, WETH);
    ...
    function removeLiquidity(
        ...
        address pair = IArborswapRouterPairs(routerPairs).pairFor(baseFactory, tokenA, tokenB);
        IArborswapRouterPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint256 amount0, uint256 amount1) = IArborswapRouterPair(pair).burn(to);
    ...
    function permit(
        ...
        IArborswapRouterPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    }
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 
                ? IArborswapRouterPairs(routerPairs).pairFor(factories[i + 1], output, path[i + 2])
                : _to;
            if (output == path[path.length - 1] && output == feeToken)
                amountOut += feeAmount;

            _trySwap(
                IArborswapRouterPair(IArborswapRouterPairs(routerPairs).pairFor(factories[i], input, output)),
    ...
    function swapExactTokensForTokens(
        ...
        uint256 feeAmount;
        address feeToken;
        (amounts, feeAmount, feeToken) = IArborswapRouterPairs(routerPairs).getAmountsOut(factories, amountIn, path);
    ...
    function swapTokensForExactTokens(
        ...
        uint256 feeAmount;
        address feeToken;
        (amounts, feeAmount, feeToken) = IArborswapRouterPairs(routerPairs).getAmountsIn(factories, amountOut, path);

        uint256 totalAmount0 = amounts[0];
        if (path[0] == feeToken) totalAmount0 += feeAmount;
        require(totalAmount0 <= amountInMax, 'ArborswapRouter: EXCESSIVE_INPUT_AMOUNT');
    ...
    function swapExactETHForTokens(
        ...
        uint256 feeAmount;
        address feeToken;
        (amounts, feeAmount, feeToken) = IArborswapRouterPairs(routerPairs).getAmountsOut(factories, msg.value, path);
        ...
        assert(IWETH(WETH).transfer(IArborswapRouterPairs(routerPairs).pairFor(factories[0], path[0], path[1]), amounts[0]));
    ...
    function swapETHForExactTokens(
        ...
        uint256 totalAmount0 = amounts[0];
        if (path[0] == feeToken) totalAmount0 += feeAmount;
        require(totalAmount0 <= msg.value, 'ArborswapRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: totalAmount0}();
        assert(IWETH(WETH).transfer(IArborswapRouterPairs(routerPairs).pairFor(factories[0], path[0], path[1]), amounts[0]));
        ...
        internal virtual
    {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = ArborswapLibrary.sortTokens(input, output);
            IArborswapRouterPair pair = IArborswapRouterPair(IArborswapRouterPairs(routerPairs).pairFor(factories[i], input, output));
            ...
            (amountOutput,) = IArborswapRouterPairs(routerPairs).getAmountOut(
                factories[i],
                input,
                true,
                amountInput,
            ...
            address to = i < path.length - 2 ? IArborswapRouterPairs(routerPairs).pairFor(factories[i + 1], output, path[i + 2]) : _to;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, IArborswapRouterPairs(routerPairs).pairFor(factories[0], path[0], path[1]), amountIn
        );
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(IArborswapRouterPairs(routerPairs).pairFor(factories[0], path[0], path[1]), amountIn));
    ...
    function transferAndSwap(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            IArborswapRouterPairs(routerPairs).pairFor(factories[0], path[0], path[1]),
            amounts[0]
        );

        _swap(factories, amounts, path, to, feeAmount, feeToken);
    }
    function _trySwap(IArborswapRouterPair pair, uint256 amount0Out, uint256 amount1Out, address to) internal {
...
library ArborswapLibrary {
    ...
    function getReserves(address factory, bytes32 initPairHash, address tokenA, address tokenB) internal view returns (uint256 reserveA, uint256 reserveB) {
        ...
        (uint256 reserve0, uint256 reserve1,) = IArborswapPair(pairFor(factory, initPairHash, tokenA, tokenB)).getReserves();
...
import "./IArborswapPair.sol";

interface IArborswapRouterPair is IArborswapPair {
...
interface IArborswapRouterPairs {
    function feeAggregator() external returns (address);
