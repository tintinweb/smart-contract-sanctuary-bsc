 ...
 *
 */
contract MiranaCollection is Ownable {
  ...
  function createToken(
    ...
    address beaconProxy = deployProxy(getData(_name, _symbol, baseURI), salt);
...
contract ERC721RaribleMinimal is ERC721BaseMinimal {
    ...
    function __ERC721Rarible_init(string memory _name, string memory _symbol) external initializer {
        __ERC721Rarible_init_unchained(_name, _symbol);

        isPrivate = false;
        emit CreateERC721Rarible(_msgSender(), _name, _symbol);
    }

    function __ERC721Rarible_init_unchained(string memory _name, string memory _symbol) internal {
    ...
    function safeMint(address to, string memory uri) public override virtual {
        if (isPrivate){
            require(owner() == _msgSender(), "minter is not the owner");
        }
...
contract ERC721BaseMinimal is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, ERC721BurnableUpgradeable, OwnableUpgradeable {
