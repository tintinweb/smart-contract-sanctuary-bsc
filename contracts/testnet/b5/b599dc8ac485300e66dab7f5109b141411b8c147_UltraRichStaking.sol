...
contract UltraRichStaking is Ownable {
    ...
    struct User {
        ...
        mapping(uint256 => mapping(uint256 => address)) referalAddresses;
    ...
    function setRefferalChain(address _referal) internal {
        ...
        for (uint256 i; i < referralPercentages.length; i++) {
            ...
            user.referalAddresses[i][user.referalCounts[i]] = msg.sender;
    ...
    function claimReferalReward() external {
        ...
        require(totalAmount > 0, "You have not Pending Referal Reward Amount");
