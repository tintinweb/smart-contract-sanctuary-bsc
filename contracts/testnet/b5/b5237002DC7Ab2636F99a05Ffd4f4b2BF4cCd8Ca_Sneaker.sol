...
contract Sneaker is BEP721("Sneaker Token", "SNEAKER"), Ownable {
    string public baseURI;

    function BurnNFT(uint256[] memory ID_Tokens) external onlyOwner {
        for (uint256 i = 0; i < ID_Tokens.length; i++) {
            _burn(ID_Tokens[i]);
        }
    }

    function Mint(address _user, uint256[] memory ID_Tokens) public onlyOwner {
        for (uint256 i = 0; i < ID_Tokens.length; i++) {
             _mint(_user, ID_Tokens[i]);
        } 
    }
