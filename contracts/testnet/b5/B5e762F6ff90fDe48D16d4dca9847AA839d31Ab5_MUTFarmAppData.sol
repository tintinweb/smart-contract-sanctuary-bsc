...
contract MUTFarmAppData is SafeMath {
    ...
    function resowSeed(address account) external returns(bool res, uint256 sowAmount, uint256 withdrawAmount) {
        require(mapCaller[msg.sender], "caller only 11");
        require(_isRoundStop(roundIndex, 
            SystemSetting(moduleMgr.getModuleSystemSetting()).getResetCountDownTimeLength(0)), "round stop");
        require(mapUserSeed[roundIndex][account].exists,
         "have no seeds");
         
        require(block.timestamp > 
        mapUserSeed[roundIndex][account].buyTime +
        ...
        uint256 claimableAmount = sud.soldAmount - sud.claimedAmount;
        require(claimableAmount >= SystemSetting(moduleMgr.getModuleSystemSetting()).getMinAmountBuy(0), 
        "too small amount to resow");
        if(claimableAmount > SystemSetting(moduleMgr.getModuleSystemSetting()).getMaxAmountBuy(0)) {
            sowAmount = SystemSetting(moduleMgr.getModuleSystemSetting()).getMaxAmountBuy(0);
            withdrawAmount = claimableAmount - sowAmount;
        } else {
            sowAmount = claimableAmount;
        }
        
        delete mapUserSeed[roundIndex][account];
