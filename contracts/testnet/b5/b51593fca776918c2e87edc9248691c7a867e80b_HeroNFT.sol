...
//      }
  ...
  function experienceUp(uint[] memory _tokenIds, uint32[] memory _experiences) external onlyGameContract {
    for (uint i = 0; i < _tokenIds.length; i++) {
      if (heroes[_tokenIds[i]].experience + _experiences[i] >= experienceCheckpoint[heroes[_tokenIds[i]].level + 1]) {
        _levelUp(_tokenIds[i]);
      }
  ...
  function updateLockedFromMKPStatus(uint[] calldata _tokenIds, bool[] calldata _statuses) external onlyContractAdmin {
    for(uint i = 0; i < _tokenIds.length; i++) {
      lockedFromMKP[_tokenIds[i]] = _statuses[i];
      emit LockedFromMKPStatusUpdated(_tokenIds[i], _statuses[i]);
    }
  }
