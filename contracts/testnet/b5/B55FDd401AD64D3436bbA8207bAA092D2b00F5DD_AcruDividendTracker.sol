...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  function getGroupIndex(uint _balance) public view  returns (uint8 index) {
    uint currentPrice = _balance.mul(price).div(10 ** 18);
  ...
  /// @notice Get the reflection rate for the group.
  ...
  function getTotalRatesOfEmptyGroups() public view returns (uint Rate) {
    ...
    for (uint8 i=0; i<length; i++) {
        if (mapMemberCountInGroup[i] == 0) {
            Rate += groupInfo[i].reflectionRate;
        }
    }
  }
 ...
 function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
  ...
  function accumulativeDividendOf(address _owner) public view override returns(uint256) {
    ...
    for (uint8 i=0; i<length; i++) {
        if (i == currentGroupIndex)
            totalAmount += magnifiedDividendPerShare[i]
                            .sub(magnifiedDividendCorrectionsFrom[_owner][i]) / magnitude;
        else 
            totalAmount += magnifiedDividendCorrectionsTo[_owner][i]
                            .sub(magnifiedDividendCorrectionsFrom[_owner][i]) / magnitude;
    }
  ...
  function _setBalance(address account, uint256 newBalance) public {
    uint8 prevGroupIndex = mapGroupIndexofUser[account];
    uint8 newGroupIndex = getGroupIndex(newBalance);
    ...
    mapGroupIndexofUser[account] = newGroupIndex;

    // Store the last dividend share in previous group
    magnifiedDividendCorrectionsTo[account][prevGroupIndex] = magnifiedDividendPerShare[prevGroupIndex];
    ...
    if (magnifiedDividendCorrectionsFrom[account][newGroupIndex] > 0 && 
        magnifiedDividendCorrectionsTo[account][newGroupIndex] >= magnifiedDividendCorrectionsFrom[account][newGroupIndex]) {
        uint remainReward = magnifiedDividendCorrectionsTo[account][newGroupIndex]
                        .sub(magnifiedDividendCorrectionsFrom[account][newGroupIndex]);

        magnifiedDividendCorrectionsFrom[account][newGroupIndex] = magnifiedDividendPerShare[newGroupIndex]
                                                                .sub(remainReward);
    }
  }
}
...
//  address public constant BUSD = address(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56);  // BSC mainnet
    ...
    uint256 public swapTokensAtAmount = 1000; //200_000 * (10 ** _decimals);
    ...
    // Rate of native token for ETH
    uint256 public rate2Eth;
    ...
    address public marketWalletAddress = address(0x17A8cb0CE0aadC2DCa353a34ae17405A8EE1f9f2);
...
//      IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        ...
        */
        ...
        _mint(address(this), 170_000_000 * (10 ** _decimals));      // Locked supply
        _mint(marketWalletAddress , 500_000 * (10 ** _decimals));   // marketwallet supply
        _mint(owner(), 75_000_000 * (10 ** _decimals));             // Liquidity and unlocked supply
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "ACRU: The dividend tracker already has that address");

        AcruDividendTracker newDividendTracker = AcruDividendTracker(payable(newAddress));
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ACRU: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ACRU: Account is already the value of 'excluded'");
     ...
     */ 
    function setMarketWallet(address payable wallet) external onlyOwner{
    ...
    /**
     * @notice Set the transaction fee for market wallet
     */ 
     ...
     */ 
    function setGiveAwayFee(uint256 value) external onlyOwner{
        giveAwayFee = value;
        totalFees = rewardsFee.add(liquidityFee).add(marketingFee).add(giveAwayFee).add(nftFee);
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ACRU: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ACRU: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 100000 && newValue <= 500000, "ACRU: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "ACRU: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 marketinfTokens = contractTokenBalance.mul(marketingFee).div(totalFees);
            swapAndSendToMarketingFee(marketinfTokens, marketWalletAddress);

            uint256 swapTokens = contractTokenBalance.mul(liquidityFee).div(totalFees);
            swapAndLiquify(swapTokens);

            uint256 giveawayTokens = contractTokenBalance.mul(giveAwayFee).div(totalFees);
     //       swapAndSendToGiveAwayFee(giveawayTokens);

            uint256 nftTokens = contractTokenBalance.mul(nftFee).div(totalFees);
    //        swapAndSendToNFTFee(nftTokens);
    ...
    /**
     * @notice Swap and send marketing fee
     */
    function swapAndSendToMarketingFee(uint256 tokens, address receiver) public {
        ...
        bool success = IERC20(BUSD).transfer(receiver, newBalance);

        if (success) {
          emit SendMarketingFee(tokens, newBalance);
        }
    }
    ...
    /**
     * @notice Swap and send transaction fee to dividend trancer.
     */
    function swapAndSendDividends(uint256 tokens) public {
        uint256 initialBUSDBalance = IERC20(BUSD).balanceOf(address(this));

        swapTokensForCake(tokens);
        uint256 dividends = IERC20(BUSD).balanceOf(address(this)).sub(initialBUSDBalance);
...
contract AcruDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(address _cake) DividendPayingToken("ACRU_Dividend_Tracker", "ACRU_Dividend_Tracker", _cake) {
        claimWait = 10 minutes; //1 hours;
...
//      priceFeedOfEth = AggregatorV3Interface(0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE); // BNB/USD in BSC mainnet
    ...
    function initGroup() internal {
        addGroup(0, 100, 0);
        addGroup(100, 500, 29);
        addGroup(500, 2000, 23);
        addGroup(2000, 4000, 9);
        addGroup(4000, 8000, 8);
        addGroup(8000, 16000, 7);
        addGroup(16000, 32000, 6);
        addGroup(32000, 65000, 5);
        addGroup(65000, 125000, 4);
        addGroup(125000, 250000, 3);
        addGroup(250000, 500000, 2);
        addGroup(500000, 1000000, 2);
        addGroup(1000000, MAX, 2);
    }

    function setRate2Eth(uint256 _rate) external  {
        rate2Eth = _rate;
        (, int _price, , , ) = priceFeedOfEth.latestRoundData();

        price = _rate * _price.toUint256Safe() / (10 ** 8);
    }
