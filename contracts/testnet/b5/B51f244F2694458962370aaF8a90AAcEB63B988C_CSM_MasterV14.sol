...
contract CSM_MasterV14{
    ...
    function initialize(address payable _ownerWallet) public {
        ...
        Group_Rewards_Program = [10,50,100,200,500,1000];
    ...
    function invest(address _referrer) public payable {
       ...
       require((users[_referrer].deposits.length > 0 && _referrer != msg.sender) || ownerWallet == msg.sender,  "No upline found");
        ...
        uint percent = (msg.value).mul(5).div(100);
        uint totalDeposit = (msg.value).add(percent);
        user.totalDepositInUSD += uint256(TotalusdPrice(int(totalDeposit)));
    ...
    function payAndGetGroupReward() public payable returns(uint256){
        ...
        uint256 reInvestmentAmount = getReInvestmentAmount(msg.sender);
    ...
    function getDepositInfo(uint _index ,address _useraddress) public view returns(uint _amount  , uint _start, uint _lastdate){
      ...
      function updateInitialize() public {
        require(ownerWallet == msg.sender);
        ref_bonuses = [0,5];
        plan = [1,5,10,50];
        plan_ref_bonuses = [10,15,20,25];
        ...
        profit_sharing_bonus = [100,120,200,350,610,1140,2280,6840,19000,85600,371800,612000];
        flushCycleDays = [180 minutes, 120 minutes, 60 minutes];
        priceFeed = AggregatorV3Interface(0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526);
	}
    ...
    function updateFlushEndDateByuser(address _useraddress) public {
         
            flushBusiness[_useraddress].endDate = block.timestamp.add(60 minutes);
    }
