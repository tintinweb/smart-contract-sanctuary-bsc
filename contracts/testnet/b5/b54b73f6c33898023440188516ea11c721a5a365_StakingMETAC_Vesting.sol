...
contract StakingMETAC_Vesting is Ownable {
    ...
    function addOldUsers(address[] memory _walletAddresses, uint256[] memory amounts, uint256[] memory lockedDaysList, uint256[] memory stakedDaysList) public onlyOwner() {
        ...
        for (uint256 i = 0; i < _walletAddresses.length; i ++) {
            ...
            user.userRewardInfo.push(UserRewardInfo({
                ...
                //bonusRate: 0,
                bonusRate: bonusRateForTime[_lockedDays],
                isPaid: false
                }));    
        }
    }
    ...
    function addOldUser(address _walletAddress, uint256 _amount, uint256 _lockedDays,uint256 _stakedDays) public onlyOwner() {
       ...
       // PoolInfo storage pool = poolInfo[_pid];
        ...
        // user.lockedDays= user.lockedDays.add(_lockedDays);
        // user.stakedDays= user.stakedDays.add(_stakedDays);
       //  pool.lpToken.safeTransferFrom(address(_walletAddress), address(this), _amount);
         uint256 harvestTime= _lockedDays.sub(_stakedDays);
        user.userRewardInfo.push(UserRewardInfo({
                amount: _amount,
                depositAt: block.timestamp.sub(_lockedDays * 1 days),
    ...
    function lockedTokenForDays(uint256 _pid, address _user) public view returns (uint256 lockedDays){
        ...
        //lockedDays= user.userRewardInfo[_pid].canHarvestTimestamp.sub(block.timestamp);
        lockedDays= lockedDays.div(1 days);
        return lockedDays;
    }
