...
contract Utopia is  Ownable {
    ...
    function setRecommender(address recommender) public {
        ...
        if(nowTime() > lastTime && lastTime != 0 && autoStatisticsReward) {
            statisticsReward();
            lastTime = nowTime();
        }
    }
    ...
    //结算奖励
    function statisticsReward() private {
        require(autoStatisticsReward || msg.sender == owner() || msg.sender == admin, "Operation without permission!");
        require(totalBonus > 0, "not enough totalBonus!");
        uint256 totalNftCount = utopiaNFT.totalSupply();
        require(totalNftCount > 0, "not enough user!");
        uint256 bonus = 0;
