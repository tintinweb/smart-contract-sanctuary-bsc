...
contract DreamHorseController is Ownable {
    ...
    event SelectMonster(uint256 monsterID,uint256 userHeroId,uint256 forageNumber,uint256 horseshoeNumber,bool playResult,uint256 profit,uint256 thisPoint);

    event DrawHeroLastMonsterProfit(address indexed user, uint256 tokenId, uint256 monsterProfit);
    ...
    uint256 public foragePrice = 300;//粮草价格

    uint256 public horseshoePrice = 200;//马蹄铁价格

    uint256 public forageOnceLimit = 3;//粮草购买数量限制

    uint256 public horseshoeOnceLimit = 4;//马蹄铁购买数量限制
    ...
    function buyHeroCard(uint256 tokenId) public {
        ...
        BUSD.transferFrom(msg.sender, address(this), fee);//10%手续费
    ...
    function selectMonster(uint256 monsterID, uint256 tokenId, uint256 forageNumber, uint256 horseshoeNumber) public {
        ...
        require(forageNumber <= forageOnceLimit && horseshoeNumber <= horseshoeOnceLimit,"Quantity exceeds limit");

        uint256 decimals = AMT.decimals();

        if(forageNumber > 0){
            AMT.transferFrom(msg.sender, address(this), foragePrice * forageNumber * 10 ** decimals);
        }

        if(horseshoeNumber > 0){
            AMT.transferFrom(msg.sender, address(this), horseshoePrice * horseshoeNumber * 10 ** decimals);
        }
    ...
    function drawHeroLastMonsterProfit(uint256 tokenId) public {
        ...
        uint256 intervalDays = (drawTime - profit.fightTime).div(24 hours);

        uint256 drawRatio = 50 + 5 * intervalDays;
        ...
        AMT.transferFrom(address(this), msg.sender, monsterProfit * 10 ** 14);
    ...
    function getSellsOrSolds(uint256 types, uint256 level) public view returns(Sell[] memory sells) {
        ...
        if(types == 1 && totalSell > 0){
            sells = new Sell[](uint256(totalSell));
        }

        if(types == 2 && totalSold > 0){
            sells = new Sell[](uint256(totalSold));
        }
        ...
        for(uint256 i = 0; i < DHN.counter(); i++){
            ...
            if(types == 1 && level == 0 && tokenIdToHeros[tokenId].isOnsell){
                sells[counter] = tokenIdToSells[tokenId];
                counter++;
            }

            if(types == 1 && level != 0 && tokenIdToHeros[tokenId].isOnsell && DHN.NFTIDToTypes(tokenId) == level){
                sells[counter] = tokenIdToSells[tokenId];
                counter++;
            }

            if(types == 2 && level != 0 && tokenIdToSells[tokenId].sold && DHN.NFTIDToTypes(tokenId) == level){
                sells[counter] = tokenIdToSells[tokenId];
                counter++;
            }
        }  
