...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///     the CAKE rewards to all the PlexSwap products.
   ...
   struct PoolInfo {
    ...
    /// @notice The only address can withdraw all the CAKE Reserves.
    ...
    /// @notice total cake rate = toReserve + toRegular + toSpecial
    uint256 public constant CAKE_RATE_TOTAL_PRECISION = 1e12;

    /// @notice The last block number of CAKE reserve action being executed.

    /// @notice CAKE distribute % for reserve
    uint256 public cakeRateToReserve = 25750000000;

    /// @notice CAKE distribute % for regular farm pool
    uint256 public cakeRateToRegularFarm = 175365000000;

    /// @notice CAKE distribute % for special pools
    uint256 public cakeRateToSpecialFarm = 798885000000;

    //-------------------------------- The Beginning of Heaven  ---------------------
    // The CAKE TOKEN!
    CakeToken public immutable CAKE;
   ...
   //------------------------------------------------------------------------------------------
    ...
    event UpdatePoolReward(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accCakePerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event UpdateCakeRate(uint256 reserveRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    function linkedPoolInfo(uint256 _pid) external view returns (IERC20 _lpTokenAddress, PoolInfo memory _poolInfo) {
     ...
     /// @notice Updates CAKE emission.
    function updateEmissionPerBlock(uint256 _newEmissionPerBlock) public onlyOwner {
        uint256 _oldCakePerBlock = emissionPerBlock;
        emissionPerBlock = _newEmissionPerBlock * (1e18);
        emit EmissionPerBlockUpdated(_oldCakePerBlock, _newEmissionPerBlock);
    }
    ...
    /// only for CAKE distributions within PlexSwap products.
    ...
    /// @notice Calculates and returns the `amount` of CAKE per block, depending on type of Farm.
    /// @param _isRegular If the pool belongs to regular or special.
    function cakePerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = (emissionPerBlock * cakeRateToRegularFarm) / CAKE_RATE_TOTAL_PRECISION;
        } else {
            amount = (emissionPerBlock * cakeRateToSpecialFarm) / CAKE_RATE_TOTAL_PRECISION;
        }
    }

    /// @notice Calculates and returns the `amount` of CAKE per block to reserve.
    function cakePerBlockToReserve() public view returns (uint256 amount) {
        amount = (emissionPerBlock * cakeRateToReserve) / CAKE_RATE_TOTAL_PRECISION;
    }
    ...
    function updatePoolReward(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                uint256 multiplier = block.number - pool.lastRewardBlock;
                uint256 cakeReward = (multiplier * cakePerBlock(pool.isRegular) * pool.allocPoint) /  totalAllocPoint;
                CAKE.mint(address(this), cakeReward);
				pool.accCakePerShare = pool.accCakePerShare + (((cakeReward * ACC_CAKE_PRECISION) / lpSupply));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePoolReward(_pid, pool.lastRewardBlock, lpSupply, pool.accCakePerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = (((user.amount * multiplier) / BOOST_PRECISION) * pool.accCakePerShare) / 
            ACC_CAKE_PRECISION;
    ...
    /// @notice Send CAKE pending for reserve to `financialController`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function reserveCake(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
        uint256 multiplier = block.number - lastAccruedBlock;
        uint256 pendingCakeToReserve = multiplier * cakePerBlockToReserve();

        // SafeTransfer CAKE
        _safeCakeTransfer(financialController, pendingCakeToReserve);
        lastAccruedBlock = block.number;
    }

    /// @notice Update the % of CAKE distributions for reserve, regular pools and special pools.
    /// @param _reserveRate The % of CAKE to reserve each block.
    ...
    function updateCakeRate(
        ...
        require(
            _reserveRate + _regularFarmRate + _specialFarmRate == CAKE_RATE_TOTAL_PRECISION,
        ...
        emit UpdateCakeRate(_reserveRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        user.rewardDebt = (((user.amount * _newMultiplier) / BOOST_PRECISION) * pool.accCakePerShare) / 
            ACC_CAKE_PRECISION;
