...
contract Registration is BasicMetaTransaction, Initializable, CustomOwnable {
    ...
    mapping(address => bool) public poap;
    ...
    function getPOAP(POAP_SBT.sbt memory _sbt) external {
        ...
        require(!poap[user], "User has POAP already minted");
        poap[user] = true;
        ...
        external
    {
        ...
        require(
            ISBT721(baptContract).balanceOf(user) > 0,
            "User does not have BAPTokens"
        );

        registered[user] = true;

        if (POAP_SBT(poap_sbt).balanceOf(user) != 0) {
            // (poap_sbt).burn(1);
        ...
        _sbteq.MembershipID = ISBT721(baptContract).tokenIdOf(user);
    ...
    function setBaptContract(address _bapt) external onlyOwner {
        baptContract = _bapt;
    }
