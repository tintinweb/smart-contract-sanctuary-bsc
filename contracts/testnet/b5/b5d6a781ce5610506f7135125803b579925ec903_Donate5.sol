...
abstract contract Donate3 {
    function myCode(address account) public virtual view returns (uint256);
}
...
contract Donate5 is Initializable, OwnableUpgradeable {
    ...
    address public _service;
    address public _donate3; 

    IBEP20 public usdt;
    IBEP20 public token;
    ISC public sc;
    Donate3 public donate3;
    ...
    event WithdrawAllToken(address indexed user, uint256 amount);
    ...
    function __version() public pure returns(uint256) {
        return 1600;
    }
    ...
    function initialize() public initializer{
        ...
        _TOKEN = 0x492e1abAE7F716197dAe20842533d56A1f119693; 
        _service = 0xF8b3092D136Bf2d296de7038EeBB79589A03273e;
        _donate3 = 0x35BC679046046D00cC6936225ECEe9D582184e13; 

        usdt = IBEP20(_USDT); // USDT bsc
        token = IBEP20(_TOKEN); // token bsc
        sc = ISC(_service);
        donate3 = Donate3(_donate3);
        ...
        purchase.push(500 * 10**18); // USDT
	}

    function setPurchase(uint256[] memory ps) public onlyOwner {
        delete purchase;
        for (uint256 i = 0; i < ps.length; i++) {
            purchase.push(ps[i] * 10 ** 18);
        }
    }
    ...
    function setDonate3(address _addr) public onlyOwner {
        _donate3 = _addr;
        donate3 = Donate3(_addr);
    }
    ...
    function buy(uint256 purchaseIndex, address referer) public {
        ...
        require(donate3.myCode(msg.sender) == 0, "Cannot buy!");
        ...
        if (referer != address(0)) {
            if(_addr2Code[referer] != 0 || donate3.myCode(referer) != 0){
                // add bonus    
                ...
                revert("A valid address is required");
            }
        }else{
    ...
    function withdrawAllToken(address account) public onlyOwner {
        uint256 total = token.balanceOf(address(this));
        token.transfer(account, total);
        emit WithdrawAllToken(account, total);
    }
