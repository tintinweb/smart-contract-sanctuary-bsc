...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(
            _owners[0] == _msgSender() || _owners[1] == _msgSender(),
...
contract ERC20 is Context, IERC20, Ownable {
    ...
    struct FeeStruct {
        uint256 level;
        uint256 levelInviteFee;
        uint256 levelFundFee;
    }
    mapping(uint256 => FeeStruct) feeLevel;
    address internal USDT_ADDR;
    address internal tokenUsdtPair;
    mapping(address => uint256) USDTPairNum;
    ...
    function setUSDTADDR(address _usdt) public onlyOwner returns (bool) {
        USDT_ADDR = _usdt;
        return true;
    }

    function setTokenUsdtPair(address _pair) public onlyOwner returns (bool) {
        tokenUsdtPair = _pair;
        return true;
    }
    ...
    function _transfer(
        ...
        } else if (takeFee && tradetype == TradeType.Sell && isSellFee) {
            uint256 bAmount = totalBurn < MAX_BURN
                ? amount.mul(bBurnFee).div(totalRate)
                : 0;
            uint256 rLp = amount.mul(sLpFee).div(totalRate);
            bAmount = bAmount <= MAX_BURN.sub(totalBurn)
                ? bAmount
                : MAX_BURN.sub(totalBurn);
            require(
    ...
    function _checkFeeLevel() internal {
        uint256 usdtResvrve = tokenUsdtPair == address(0) ||
            USDT_ADDR == address(0)
            ? 0
            : IERC20(USDT_ADDR).balanceOf(tokenUsdtPair);

        if (usdtResvrve <= feeLevel[0].level) {
            sFundFee = feeLevel[0].levelFundFee;
            inviteFee = feeLevel[0].levelInviteFee;
        }

        if (
            usdtResvrve > feeLevel[0].level && usdtResvrve <= feeLevel[1].level
        ) {
            sFundFee = feeLevel[1].levelFundFee;
            inviteFee = feeLevel[1].levelInviteFee;
        }

        if (
            usdtResvrve > feeLevel[1].level && usdtResvrve <= feeLevel[2].level
        ) {
            sFundFee = feeLevel[2].levelFundFee;
            inviteFee = feeLevel[2].levelInviteFee;
        }

        if (
            usdtResvrve > feeLevel[2].level && usdtResvrve <= feeLevel[3].level
        ) {
            sFundFee = feeLevel[3].levelFundFee;
            inviteFee = feeLevel[3].levelInviteFee;
        }

        if (
            usdtResvrve > feeLevel[3].level && usdtResvrve <= feeLevel[4].level
        ) {
            sFundFee = feeLevel[4].levelFundFee;
            inviteFee = feeLevel[4].levelInviteFee;
        }

        if (usdtResvrve > feeLevel[4].level) {
            sFundFee = feeLevel[5].levelFundFee;
            inviteFee = feeLevel[5].levelInviteFee;
        }
    }
...
contract Token is ERC20, ERC20Detailed {
    ...
    constructor(
        ...
        address _usdt // 主网 USDT： 0x55d398326f99059fF775485246999027B3197955
        ...
        uint256 baseFeeLevel = uint256(1 * 10**3 * 10**18); 

        feeLevel[0].levelInviteFee = inviteFee;
        feeLevel[0].levelFundFee = sFundFee;
        feeLevel[0].level = baseFeeLevel; 

        feeLevel[1].levelInviteFee = 80;
        feeLevel[1].levelFundFee = 40;
        feeLevel[1].level = feeLevel[0].level * 5; 

        feeLevel[2].levelInviteFee = 40;
        feeLevel[2].levelFundFee = 40;
        feeLevel[2].level = feeLevel[1].level * 10;

        feeLevel[3].levelInviteFee = 30;
        feeLevel[3].levelFundFee = 30;
        feeLevel[3].level = feeLevel[2].level * 2; 

        feeLevel[4].levelInviteFee = 0;
        feeLevel[4].levelFundFee = 30;
        feeLevel[4].level = feeLevel[3].level * 5; 

        feeLevel[5].levelInviteFee = 0;
        feeLevel[5].levelFundFee = 0;
        feeLevel[5].level = feeLevel[3].level * 10; 

        MAX_BURN = totalSupply().mul(_MAX_BURN).div(totalRate);
        ...
        tokenUsdtPair = IUniswapV2Factory(defaultRouter.factory()).createPair(
            address(this),
            USDT_ADDR
        );
        USDTPairNum[USDT_ADDR] = IUniswapV2Pair(tokenUsdtPair).token0() ==
            address(this)
            ? 1
            : 0;
        tokenAmountsAutoSellToAddLpPerTime =
        ...
        setPairStatus(tokenUsdtPair, true);
