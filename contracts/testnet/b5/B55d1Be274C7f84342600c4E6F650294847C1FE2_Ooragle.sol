...
contract Ooragle is Ownable {
    ...
    function addUserLink(string memory link) external {
        if (bytes(realLink[link]).length == 0) unhandledLinks.push(link);
    ...
    function getUnhandledLinks(address user) public view returns (string[] memory) {
