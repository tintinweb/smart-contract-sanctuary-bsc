...
contract GYM is ERC721Enumerable, Ownable, Pausable, ReentrancyGuard {
    ...
    function mint(address to, uint256 mintAmount) private whenNotPaused {
        require(_sale.remainAmounts>=mintAmount);
        require(mintAmount > 0);
        require(mintAmount <= MAX_MINT_BATCH);
        require(totalSupply() + mintAmount <= MAX_SUPPLY);
    ...
    function buyForUSD(address to,uint256 mintAmount,address ref ) external whenNotPaused nonReentrant {
        uint256 priceBUSD = getBUSDPerNFT(mintAmount);
        mint(to, mintAmount);
        addRef(to, ref);
        rewardingRef(to, priceBUSD);
    ...
    function buyForBNB( address to, uint256 mintAmount, address ref ) external payable whenNotPaused nonReentrant {
        require(mintAmount>0);
    ...
    function setSale( uint64 start, uint64 end, uint256 priceUsd, uint256 amount, bool paused ) external onlyOwner {
        require(start>=block.timestamp-300, "start expired");
        require(end>=start+60, "bad interval");
        require(priceUsd>=1e15,"price < 0.001");
        require(amount>0,"amount 0");
        _sale = Sale( start, end, priceUsd, amount, amount, paused);
    }
