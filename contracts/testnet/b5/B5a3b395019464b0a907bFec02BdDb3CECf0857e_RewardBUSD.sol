...
contract BUSDDividendTracker is DividendPayingToken {
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Test_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Test contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Test_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "Test_Busd_Dividend_Tracker: Cannot update claimWait to same value"
        );
...
contract RewardBUSD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    event UpdatebusdDividendTracker(
    ...
    constructor() ERC20("RewardBUSD Token", "RBUSD") {
        // place your Marketing Wallet address
        marketingWallet = 0x936BAce081ff8DfB420C98294074fC9490e23065;
        busdDividendToken = 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee;  //testnet
    ...
    function updateMarketingWallet(address _newWallet) external onlyOwner {
        require(
            ...
            "Test: The marketing wallet is already this address"
        );
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(
            newAddress != address(busdDividendTracker),
            "Test: The dividend tracker already has that address"
        );

        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(
            payable(newAddress)
        );

        require(
            newbusdDividendTracker.owner() == address(this),
            "Test: The new dividend tracker must be owned by the Test token contract"
        );

        newbusdDividendTracker.excludeFromDividends(
            address(newbusdDividendTracker)
        );
        ...
        emit UpdatebusdDividendTracker(
            newAddress,
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "Test: Account is already exluded from fees"
        );
        isExcludedFromFees[account] = excluded;
    ...
    function updateMinimumBalanceForDividends(uint256 newMinimumBalance)
        ...
        onlyOwner
    {
        busdDividendTracker.updateMinimumTokenBalanceForDividends(
            newMinimumBalance
        );
    }
    ...
    function getTotalBusdDividendsDistributed()
        external
        view
        returns (uint256)
    {
    ...
    function withdrawableBusdDividendOf(address account)
        external
        view
        returns (uint256)
    {
    ...
    function busdDividendTokenBalanceOf(address account)
        external
        view
        returns (uint256)
    {
    ...
    function processDividendTracker(uint256 gas) external onlyOwner {
        (
            uint256 busdIterations,
            uint256 busdClaims,
            uint256 busdLastProcessedIndex
        ) = busdDividendTracker.process(gas);
        emit ProcessedbusdDividendTracker(
            busdIterations,
            busdClaims,
            busdLastProcessedIndex,
            false,
            gas,
            tx.origin
        );
    ...
    function getLastBusdDividendProcessedIndex()
        external
        view
        returns (uint256)
    {
    ...
    function getNumberOfBusdDividendTokenHolders()
        external
        view
        returns (uint256)
    {
    ...
    function _transfer(
        ...
        require(
            ...
            "Test: Trading has not started yet"
        );
        ...
        if (takeFee) {
            ...
            tmpBusdDividedRewardsInContract = amount
                .mul(busdDividendRewardsFee)
                .div(100);
            tmpLpRewardInContract = amount.mul(liquidityFee).div(100);

            fees = tmpMarketingRewardPercent
                .add(tmpBusdDividedRewardsInContract)
                .add(tmpLpRewardInContract);

            busdDividedRewardsInContract = busdDividedRewardsInContract.add(
                tmpBusdDividedRewardsInContract
            );
        ...
        try
            busdDividendTracker.setBalance(payable(from), balanceOf(from))
        {} catch {}

        try
            busdDividendTracker.setBalance(payable(to), balanceOf(to))
        {} catch {}

        if (!swapping) {
            uint256 gas = gasForProcessing;

            try busdDividendTracker.process(gas) returns (
                uint256 iterations,
                uint256 claims,
                uint256 lastProcessedIndex
            ) {
    ...
    function swapAndSendBusdDividends(uint256 tokens) private {
        ...
        uint256 busdDividends = IERC20(busdDividendToken).balanceOf(
            address(this)
        );
        transferDividends(
            busdDividendToken,
