...
contract LabradoOperator is AccessControlEnumerable, ReentrancyGuard {
    ...
    event DepositToken(address token, address indexed sender, uint256 amount);
    event DepositNFT(address nft, address indexed sender, uint256 tokenId);
    ...
    /**
     * @dev set ERC20 token support status for specified token
    ...
    /**
     * @dev set NFT token support status for specified NFT
     * - Can only be called by user has role admin
    ...
    /**
     * @dev withdraw tokens stuck in contract when user wrong transfer
     * - Can only be called by user has role admin
    ...
    /**
     * @dev withdraw all tokens stuck in contract when user wrong transfer
     * - Can only be called by user has role admin
     * @param _token Address token will withdraw
     **/
    ...
    function depositNFT(address _nft, uint256 _tokenId) public {
        ...
        emit DepositNFT(_msgSender(), _nft, _tokenId);
    }
