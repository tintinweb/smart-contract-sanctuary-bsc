...
contract HeadTail {
	...
	constructor(address payable ceoAddr,address payable walletadmin, uint256 start)  {
...
struct betdata{
	...
	//	admin_amount = 0.002 ether;
	//	winner_amount = 0.003 ether;

		//adminWallet.transfer(admin_amount);
		//Winner.transfer(winner_amount);

		this.sendMoney(Winner, winner_amount);
	...
	/*event transferSend(address _from, address _destAddress, uint256 amount);
	function transferToken(address token, address transferTo, uint256 amount) public {
		uint256 addressBlance =  token.balanceOf(address(this));
		require(addressBlance <= amount, "Balance is low");
		token.transfer(transferTo, amount);
		emit transferSend(msg.sender, transferTo, amount);
	}*/

	//function availableBalance(address balAdrs){	}
