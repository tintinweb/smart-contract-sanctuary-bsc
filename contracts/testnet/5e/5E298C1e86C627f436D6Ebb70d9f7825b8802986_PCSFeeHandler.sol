...
contract PCSFeeHandler is OwnableUpgradeable {
    ...
    function withdraw(address tokenAddr,  address payable _to) external onlyOwner {
        if (tokenAddr == address(0)) {
            ...
            if (balance > 0) {
                IERC20Upgradeable(tokenAddr).safeTransfer(_to, balance);
            }
        }
    }
