...
contract Pool is Initializable, PoolStorage, OwnableUpgradeable, ReentrancyGuardUpgradeable, IPool {
    ...
    function increasePosition(
        ...
        uint256 collateralPrice = _getCollateralPrice(_collateralToken, false);
    ...
    function _calcDecreasePayout(
        ...
        vars.collateralPrice = _getCollateralPrice(_collateralToken, true);
    ...
    function _getCollateralPrice(address _token, bool _max) internal view returns (uint256) {
        if (isStableCoin[_token]) {
            // force collateral price = 1 incase of using stablecoin as collateral
            return 10 ** (USD_VALUE_DECIMAL - IDecimalsErc20(_token).decimals());
        } else {
            return _getPrice(_token, _max);
        }
    }
