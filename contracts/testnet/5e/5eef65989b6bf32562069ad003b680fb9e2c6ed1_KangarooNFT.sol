...
contract KangarooNFT is ERC721URIStorage, Ownable {
    ...
    // mapping(address => uint256) public openBoxForKG;
    ...
    function bindRecommender(address recommender) public {
        ...
        // require(checkRecommender(msg.sender, recommender), "Users cannot bind to each other");
    ...
    // function checkRecommender(address user, address recommender) public view returns (bool) {

    //     require(recommender != address(0) && recommender != user, "Please enter the correct address");

    //     for(uint256 i = 1; i <= recommendUserNumber[recommender]; i ++){
    //         if(recommendUsers[recommender][i] == user) return false;
    //     }
