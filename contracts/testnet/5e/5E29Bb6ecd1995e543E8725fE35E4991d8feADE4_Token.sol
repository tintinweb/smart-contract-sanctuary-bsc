...
struct TokenInfo {
	...
	bytes32 unikey; // unique key on chain, but uniform across networks for each token
}
...
abstract contract TokenData {
	...
	function _isToken(address _token) internal view returns (bool) {
		...
		return ((tokeninfo[_token].token == _token) && (tokeninfo[_token].chainid == block.chainid));
	}

	function _getStatusToken(address _token) internal view returns (uint8 _status) {
		return tokeninfo[_token].status; // Get the first digit of the status variable
	}
	...
	function _getToken() internal view returns (TokenInfo[] memory _tokeninfo) {
		_tokeninfo = new TokenInfo[](tokens.length);
		for (uint256 i = 0; i < tokens.length; i++) _tokeninfo[i] = tokeninfo[tokens[i]];
	}
	...
	function _tokenDeposit(address _token, uint256 _amount) internal {
		uint256 _oldvolume = tokeninfo[_token].volumeOfToken;
		tokeninfo[_token].volumeOfToken += _amount;
		if (tokeninfo[_token].volumeOfToken != _oldvolume + _amount) revert("Token: deposit failed");
	...
	function _tokenWithdraw(address _token, uint256 _amount) internal {
		uint256 _oldvolume = tokeninfo[_token].volumeOfToken;
		if (_oldvolume >= _amount) tokeninfo[_token].volumeOfToken -= _amount;
		else revert("Token: insufficient balance");
		if (tokeninfo[_token].volumeOfToken != _oldvolume - _amount) revert("Token: withdraw failed");
	...
	function _addUpdateToken(
		...
		if (_isToken(_token)) {
			// Update exits token
			if (tokeninfo[_token].status != _status) tokeninfo[_token].status = _status;
			if (tokeninfo[_token].unikey != _unikey) tokeninfo[_token].unikey = _unikey;
		} else {
			// Add new token
			if (tokens.add(_token)) {
				tokeninfo[_token] = TokenInfo(_token, block.chainid, _status, 0, _unikey);
...
contract Token is TokenData, IToken {
	...
	function getAvailableToken() external view onlyAccepted returns (TokenInfo[] memory tokeninfo) {
		...
		uint256 k = 0;
		for (uint256 i = 0; i < _token.length; i++) if (_isAvailableToken(_token[i].token)) k++;
		tokeninfo = new TokenInfo[](k);
		k = 0;
	...
	function addUpdateToken(
		ERC20 token,
		uint8 status,
		bytes memory unikey
	) external virtual onlyAccepted {
		...
		bytes32 _unikey = (unikey.length == 32 && bytes32(unikey) != bytes32(0))
			? bytes32(unikey)
			: keccak256(abi.encodePacked(token, status, block.difficulty, block.chainid, block.timestamp));
		_addUpdateToken(address(token), status, _unikey);
	}
