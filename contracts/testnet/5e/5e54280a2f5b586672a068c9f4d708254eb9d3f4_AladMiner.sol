...
contract AladMiner is Ownable {
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 r = block.timestamp.sub(acc.jClaimAt) / 86400;

        uint256 cWithdraw = acc.selfExpectedRewards.mul(r).mul(getp1()).div(
            1e18
        );
        ...
        return
            accRes >= cWithdraw
                ? (cWithdraw, cWithdraw.mul(getp1()).div(1e18))
                : (accRes, accRes.mul(getp1()).div(1e18));
    }
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 r = block.timestamp.sub(acc.dClaimAt) / 86400;
        ...
        return
            acc.getDynamicRewards <= accRes
                ? (
                    ...
                    acc.getDynamicRewards.mul(getp1()).div(1e18)
                )
                : (accRes, accRes.mul(getp1()).div(1e18));
    }

    function withdrawPrivateRewards() external {
        require(
            block.timestamp.sub(accountInfo[msg.sender].jClaimAt) >= 86400,
        ...
        (uint256 rew, uint256 trew) = earnedPrivateRewards(msg.sender);
        ...
        if (trew > 0) {
            ...
            if (r > 0) token.safeTransfer(msg.sender, trew.mul(r).div(100));
        }
    ...
    function withdrawDynamicRewards() external {
        ...
        (uint256 rew, uint256 trew) = earnedDynamicRewards(msg.sender);
