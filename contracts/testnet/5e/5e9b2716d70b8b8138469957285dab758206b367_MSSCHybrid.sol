...
contract MSSCHybrid is MSSCBase, MembraneAuth, HTLC {
   ...
   * @param  cycleId Cycle's bytes32 obfuscatedId to execute.
   */
  function executeInstructions(bytes32 cycleId, bytes calldata secret)
    external
...
abstract contract HTLC {
  ...
  modifier unlock(bytes32 lockId_, bytes calldata secret) {
  ...
  function _hash(bytes calldata preimage) private pure returns (bytes32) {
