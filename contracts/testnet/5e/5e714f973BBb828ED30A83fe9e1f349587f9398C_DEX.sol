...
contract DEX is ReentrancyGuardUpgradeable  {
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeMath for uint256;
    address private constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    IMasterChef public  StakingContract =IMasterChef(0xa5f8C5Dbd5F286960b9d90548680aE5ebFf07652);
IWBNB WBNBCONTRACT = IWBNB(WBNB);
    ...
    function _zapIn(
        address _tokenToZap,
        uint256 _pid
    ) external payable nonReentrant returns (uint256 lpTokenReceived) {
        
        
        address _lpToken = StakingContract.lpToken(_pid);
        IPancakeV2Pair pairLP = IPancakeV2Pair(_lpToken);
        address token0 = pairLP.token0();
        address token1 = pairLP.token1();
        // console.log(_lpToken,token1,token0,"jplp");
 ...
 uint256 tokenBValue;
  ...
  tokenAValue = IPancakeV2Router(ROUTER).swapExactETHForTokens{
            value: msg.value / 2
        }(1, path0, address(this), block.timestamp + 1)[
            path0.length - 1
        ];
        }else{
                   tokenAValue =WBNBCONTRACT.balanceOf(address(this));
        WBNBCONTRACT.deposit{value:msg.value/2}();
        tokenAValue  = WBNBCONTRACT.balanceOf(address(this))- tokenAValue;
        }
if(token1 != WBNB){

        uint256 tokenBValue = IPancakeV2Router(ROUTER).swapExactETHForTokens{
            value: msg.value / 2
        }(1, path1, address(this), block.timestamp + 1)[
            path1.length - 1
        ];
}else{
        tokenBValue =WBNBCONTRACT.balanceOf(address(this));
        WBNBCONTRACT.deposit{value:msg.value/2}();
        tokenBValue  = WBNBCONTRACT.balanceOf(address(this))- tokenBValue;

        }
// console.log(tokenAValue, tokenBValue,"Yeeeeeeeeeeet");
{
IERC20Upgradeable tokenA = IERC20Upgradeable(token0);
IERC20Upgradeable tokenB = IERC20Upgradeable(token1);
    ...
    uint256 allowanceA = tokenA.allowance(
        ...
        if (allowanceA < tokenAValue) {
            require(tokenA.approve(address(ROUTER), 1157920892373161954235709850086879078532699846656405640394575840079131296399), "FS");
             allowanceA = tokenA.allowance(
            address(this),
            address(ROUTER)
        );
        }
        ...
        uint256 allowanceB = tokenB.allowance(
            ...
            require(tokenB.approve(address(ROUTER), 1157920892373161954235709850086879078532699846656405640394575840079131296399), "FS");
            allowanceB = tokenB.allowance(
            address(this),
            address(ROUTER)
        );
        // console.log(allowanceA,WBNBCONTRACT.balanceOf(address(this)),allowanceB);
        }
...
//         // // Add liquidity and retrieve the amount of LP received by the sender
        (  usedTokenA,  usedTokenB,   lpTokenReceived) = IPancakeV2Router(ROUTER).addLiquidity(
...
// console.log(usedTokenA,usedTokenB);
        // IERC20Upgradeable(_lpToken).safeTransfer(msg.sender,lpTokenReceived);
        return lpTokenReceived;
    }
function getSwapIn( address _tokenToZap,
        uint256 _tokenAmountIn,
        address _lpToken,
        uint256 _tokenAmountOutMin,
        uint256 _pid) internal returns(uint256 swapAmountIn) {
{
        ...
        returns (uint256)
    {
        ...
        (uint256 amount0, uint256 amount1) = IPancakeV2Pair(_lpToken).burn(
            address(this)
        );
        ...
        uint256[] memory tokenAmountOutMin = IPancakeV2Router(ROUTER)
            .getAmountsOut(swapAmountIn, path);
        // Swap tokens
        uint256 balBef = IERC20Upgradeable(_tokenToReceive).balanceOf(
            address(this)
        );
        ...
        uint256 balAft = IERC20Upgradeable(_tokenToReceive).balanceOf(
            address(this)
        );

        // Return full balance for the token to receive by the sender
        // require(_totalTokenAmountOutMin < IERC20Upgradeable(_tokenToReceive).balanceOf(address(this)), "amount is not enough");
        uint256 amountReceived = balAft - balBef;

        IERC20Upgradeable(_tokenToReceive).safeTransfer(
            msg.sender,
            amountReceived
        );
  ...
  function convo(uint256 amount, address token)
        external
 ...
 path[1] =WBNB;
        ...
        // if (allowance < amount) {
            ...
            require(tokenInstance.approve(address(ROUTER), 1157920892373161954235709850086879078532699846656405640394575840079131296399), "FS");
            allowance = tokenInstance.allowance(address(this),address(ROUTER));
        // console.log(allowance);
        // }
           
     ethAmount=    IPancakeV2Router(ROUTER).swapExactTokensForETH(
            amount,
            1,
            path,
            address(this),
        ...
        private
    {
        if (
            ...
            // Re-approve
            IERC20Upgradeable(_token).safeApprove(ROUTER, MAX_INT);
        }
    }
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            ...
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) /
                IPancakeV2Pair(_lpToken).totalSupply();

            swapAmountIn = tokenAmountIn;
            swapAmountOut = IPancakeV2Router(ROUTER).getAmountOut(
                swapAmountIn,
                reserveA,
                reserveB
            );
