...
contract MedabotsFactory is Ownable, Pausable {
  ...
  - MINT
  - ownerMintLotes
  -> Las partes minteadas tienen que tener todas las funciones que vimos en medabots.sol.
  */


  mapping(string => bool) public usedHash;//
 //TODO: implement base uri sistem.
  ...
  function mint(    
    ...
    uint medamonAllowance = medamon.allowance(msg.sender,address(this));
    uint medacoinAllowance = medacoin.allowance(msg.sender,address(this));
    require( medacoinAllowance >= priceMedacoin, "need aprove more tokens");
    require( medamonAllowance >= priceMedamon, "need aprove more tokens");

    //Sistem to no repit a same hash in url.
    require(usedHash[_metadataURI] == false, "This data was already created ");
    usedHash[_metadataURI] = true;

    
    if (priceMedacoin > 0){
      medamon.transferFrom(msg.sender, owner(), priceMedamon);
    }
    if(priceMedamon > 0){
      medacoin.transferFrom(msg.sender, owner(), priceMedacoin);
    }
...
//---------OnlyOwner functions-----------
//TODO: implement a function whit for.
  function ownerMint(string calldata _metadataURI, MedabotsMetadata.Metadata calldata _metadata) public onlyOwner {
    require(usedHash[_metadataURI] == false, "This data was already created ");
    medabots.mint(msg.sender, _metadataURI, _metadata);
  }
  function ownerMintLotes(string[] calldata _metadataURI, MedabotsMetadata.Metadata[] calldata _metadata) public onlyOwner {
    require(_metadataURI.length == _metadata.length, "Los vectores tiene que concidir in size" );
    for(uint i = 0; i<_metadataURI.length ;i++){
      require(usedHash[_metadataURI[i]] == false, "This data was already created ");
      medabots.mint(msg.sender, _metadataURI[i], _metadata[i]);
    }
  }
