...
contract Colfi{
  ...
  event Airdropped(address indexed _userAddress, uint256 _amount);
    ...
    constructor(address ownerAddress,IBEP20 _COLFI) public
    {
        owner = ownerAddress;  
        COLFI = _COLFI;
    }
	...
	function StakeColfi(string memory investorId,uint investment) public payable
	{
	  require(COLFI.balanceOf(msg.sender)>=investment*1e18);
   	//COLFI.transferFrom(msg.sender ,owner,investment*1e18);
		emit Staking( investorId,investment*1e18,msg.sender);
	}
