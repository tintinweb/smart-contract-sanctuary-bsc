...
contract ETH_Vault {
    ...
    function getReward() external payable {
        ...
        require(block.timestamp - lastGetRewardTime > 1 days, "You can get reward only 1 time per day");
        for (uint256 i = 0; i < investHolders.length; i++)
        {
            ...
            if(player.invested > 0)
            {
                ...
                {
                    reward = investHolders[i].investAmount * (((investHolders[i].investAmount - minStakingAmount) * 264 / 10 ** 17) + 19300)/ (365 * 10 ** 5);
                }else{
                    reward = investHolders[i].investAmount * (((investHolders[i].investAmount - maxStakingAmount) * 215 / 10 ** 18) + 2770000)/ (365 * 10 ** 7);
                }
