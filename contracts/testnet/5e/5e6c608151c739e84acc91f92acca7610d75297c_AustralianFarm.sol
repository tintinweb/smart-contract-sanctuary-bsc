...
contract AustralianFarm is Ownable{
    ...
    enum CropType{Cattle,Bull,Cow,Chick,Egg,Hen,Corn,Breeding}
    ...
    // 是否同步
    mapping(uint256 => bool) public isSynchronization;
    // NFT转背包类型
    mapping(uint256 => BackpackType) nftTypeToBackpackType;
    ...
    // 喂养条件
    FeedingTimesRule[8] public feedingTimesRules;
    // 盲盒概率
    ...
    constructor(){
        ...
        bulidingRules[uint256(BulidingType.FarmLand)] = BulidingRule(BulidingType.FarmLand,200,200,2);
        bulidingRules[uint256(BulidingType.Cowshed)] = BulidingRule(BulidingType.Cowshed,300,300,2);
        bulidingRules[uint256(BulidingType.Henhouse)] = BulidingRule(BulidingType.Henhouse,100,100,2);
        ...
        // feedingTimesRules[uint256(CropType.Egg)] = FeedingTimesRule(CropType.Egg,1,0);
        // feedingTimesRules[uint256(CropType.Breeding)] = FeedingTimesRule(CropType.Breeding,40,100);
        feedingTimesRules[uint256(CropType.Cattle)] = FeedingTimesRule(CropType.Cattle,2,30);
        feedingTimesRules[uint256(CropType.Bull)] = FeedingTimesRule(CropType.Bull,0,50);
        feedingTimesRules[uint256(CropType.Cow)] = FeedingTimesRule(CropType.Cow,0,50);
        feedingTimesRules[uint256(CropType.Chick)] = FeedingTimesRule(CropType.Chick,2,10);
        feedingTimesRules[uint256(CropType.Hen)] = FeedingTimesRule(CropType.Hen,2,20);
        feedingTimesRules[uint256(CropType.Corn)] = FeedingTimesRule(CropType.Corn,2,0);
        feedingTimesRules[uint256(CropType.Egg)] = FeedingTimesRule(CropType.Egg,1,0);
        feedingTimesRules[uint256(CropType.Breeding)] = FeedingTimesRule(CropType.Breeding,2,100);
        ...
        backpackTypeToMainType[3] = MainType(BackpackType.FarmLand,1,0);
        ...
        // 官网NFT对应的背包中的类型
        nftTypeToBackpackType[1] = BackpackType.Land;
        nftTypeToBackpackType[2] = BackpackType.Cowshed;
        nftTypeToBackpackType[3] = BackpackType.Henhouse;
        nftTypeToBackpackType[4] = BackpackType.FarmLand;
    }
    ...
    function putLand() public{        
        ...
        uint256 lastLandNO = userLandTokenIds[msg.sender].length;
    ...
    function putBuliding(BackpackType backpackType)public{
        ...
        // 类型校验
        require(backpackType == BackpackType.FarmLand || backpackType == BackpackType.Cowshed || backpackType == BackpackType.Henhouse,"Type error");
        ...
        if(backpackType == BackpackType.FarmLand){
            ...
            userBulidings[tokenId].crops.push(totalCropNO);

            userCrops[totalCropNO] = Crop(1,CropType.Corn,false,0,0,0,false);
        }
    }
    ...
    function putBulidingInBackpack(uint256 landTokenId,uint256 bulidingTokenId) public {
        ...
        _putInTheBackpack(msg.sender,backpackType,true,bulidingTokenId);
    }

    // 作物放入背包,建筑tokenId,作物编号
    function putCropInBackpack(uint256 bulidingTokenId,uint256 cropNO)public{
        ...
        for(uint256 i = 0 ; i < cropNos.length; i++){
            ...
            if(cropNos[i] == cropNO && cropNO != 0){

                delete userBulidings[bulidingTokenId].crops[i];

                valid = true;
            }
        }

        require(valid,"The crop not found");
        ...
        require(crop.lastTime + 60 < block.timestamp,"This crop can't be put into the backpack yet");

        BackpackType backpackType = _mainType2BackpackType(2,uint256(crop.cropType));
        
        delete userCrops[cropNO];
        
        _putInTheBackpack(msg.sender,backpackType,true,0);
    }
    ...
    function firstBulid(uint256 landTokenId,BulidingType bulidingType) public{
        ...
        uint256 bulidingTokenId = farmNFT.createNFT(msg.sender,_getNFTType(backpackType));

        isSynchronization[bulidingTokenId] = true;
    ...
    function bulid(uint256 bulidingTokenId) public{
        ...
        // 已经建造完成
        require(!buliding.isBulided,"It has been built");

        // 间隔4小时
        // require(buliding.lastBulidingTime + 4 * 3600 < block.timestamp,"It can't be built yet");

        require(buliding.lastBulidingTime + 60 < block.timestamp,"It cant be built yet");
        ...
        if(bulidingRules[uint256(buliding.bulidingType)].clicks == userBulidings[bulidingTokenId].clicks){
            ...
            if(buliding.bulidingType == BulidingType.FarmLand){
                ...
                userBulidings[bulidingTokenId].crops.push(totalCropNO);

                userCrops[totalCropNO] = Crop(totalCropNO,CropType.Corn,false,0,0,0,false);
            }
        }
    }


    // 放入农作物,建筑tokenId，背包中物品类型
    function putCrops(uint256 bulidingTokenId,BackpackType backpackType) public{
        ...
        Crop memory crop = Crop(0,CropType(_backpackType2MainType(backpackType)),false,0,0,tokenId,false);        
        ...
        if(buliding.bulidingType == BulidingType.Cowshed && (crop.cropType == CropType.Cattle || crop.cropType == CropType.Bull || crop.cropType == CropType.Cow)){
            ...
            if(num < cowshedsOnTheLandNum){
                ...
                if(num >= 1){        
                    ...
                    for(uint256 i=0; i<crops.length; i++){
                        ...
                        // 公牛母牛合养
                        if(userCrops[crops[i]].cropType != CropType.Cattle && crop.cropType != CropType.Cattle && userCrops[crops[i]].cropType != crop.cropType && userCrops[crops[i]].NO != 0 && !userCrops[crops[i]].isBreeding){
            ...
            validNum = num < henHouseOnTheLandNum;
        }else{
        ...
        if(cropIndex == 999){
            ...
            userBulidings[bulidingTokenId].crops.push(crop.NO);
        }else{

            userBulidings[bulidingTokenId].crops[cropIndex] = crop.NO;
        }

        // cropIndex == 999 ? userBulidings[bulidingTokenId].crops.push(crop.NO) : userBulidings[bulidingTokenId].crops[cropIndex] = crop.NO;

        userCrops[crop.NO] = crop;
    }


    // 喂养/浇水/孵化,建筑tokenId，农作物编号
    function feedOrWatering(uint256 bulidingTokenId,uint256 cropNO)public{
        ...
        // 浇水/喂养间隔4小时
        // require(crop.lastTime + 4 * 3600 < block.timestamp,"You can't feed or watering it yet");

        require(crop.lastTime + 60 < block.timestamp,"You cant feed or watering it yet");

        // 未收获不能浇水/喂养/孵化
        require(!crop.isHarvest,"You must feed or watering  after harvest");
        ...
        if(crop.isBreeding){
            ...
            for(uint256 i=0; i<crops.length; i++){
                ...
                if(userCrops[crops[i]].isBreeding && userCrops[crops[i]].NO != cropNO){
                    ...
                    require(userCrops[crops[i]].lastTime + 60 < block.timestamp,"You cant feed or watering it yet");

                    userCrops[crops[i]].lastTime = block.timestamp; 

                    isGet = crop.clicks == feedingTimesRules[uint256(CropType.Breeding)].clicks;

                    isGet ? userCrops[crops[i]].clicks = 0 : userCrops[crops[i]].clicks ++;
                }
            }
        }
        
        // 是否可以收获
        crop.isHarvest = isGet && (crop.cropType == CropType.Hen || crop.cropType == CropType.Corn || crop.isBreeding);
    ...
    function harvest(uint256 cropNO)public{
        ...
        // 还未成熟
        require(crop.isHarvest,"You cant harvest yet");
        ...
        if((crop.cropType == CropType.Bull || crop.cropType == CropType.Cow) && crop.isBreeding){
    ...
    function buyingGoodsInShop(GoodsType goodsType,uint256 number) public{
        ...
        _putInTheBackpack(msg.sender,BackpackType.Bull,true,0);

        _putInTheBackpack(msg.sender,BackpackType.Cow,true,0);

        _putInTheBackpack(msg.sender,BackpackType.FarmLand,true,0);
        _putInTheBackpack(msg.sender,BackpackType.Henhouse,true,0);
        _putInTheBackpack(msg.sender,BackpackType.Hen,true,0);
        ...
        for(uint256 i = 0; i < userBackpacks[msg.sender][backpackType].count; i ++ ){
            ...
            if(count < amount){
                ...
                if(set){
                    ...
                    NFTGoods[totalSales].tokenIds[count] = tokenId;
                }
            }else{
                break;
            }        
    ...
    function cancleNFTSale(uint256 saleIndex) public {
        ...
        for(uint256 i = 0; i < NFTGoods[saleIndex].surplusAmount; i ++ ){
            ...
            _putInTheBackpack(msg.sender,BackpackType(NFTGoods[saleIndex].nftType),true,tokenId);
        }
    ...
    function getMyAssets(address owner) public view returns(uint256[10] memory){
        ...
        for(uint256 i = 0 ; i < 10; i++){
            ...
            if(i != uint256(BackpackType.Egg) && i != uint256(BackpackType.Chick) && i != uint256(BackpackType.Cattle)){
        ...
        for(uint256 i = 1 ; i <= totalSales ; i++){
            ...
            if(NFTGoods[i].surplusAmount != 0){

                myAssets[uint256(NFTGoods[i].nftType)]++;
            }
        }
    ...
    function getLength(uint256 tokenId) public view returns(uint256){
        return userBulidings[tokenId].crops.length;
    }

    // 同步官网NFT
    function synchronization() public {

        for(uint256 i = 1; i <= farmNFT.counter(); i++){

            uint256 tokenId = farmNFT.NFTs(i);

            if(!isSynchronization[tokenId] && farmNFT.ownerOf(tokenId) == msg.sender){
               
                BackpackType backpackType = nftTypeToBackpackType[farmNFT.NFTTypes(tokenId)];
                
                _putInTheBackpack(msg.sender,backpackType,true,tokenId); 

                isSynchronization[tokenId] = true;
            }
        }
    }


    // 获取土地tokenid,owner return:landTokenIds
    function getMyLandTokenIds(address owner)public view returns(uint256[] memory){
        return userLandTokenIds[owner];
    }

    // 获取建筑tokenId,土地tokenId,return: bulidingTokenIds
    ...
    function _putInTheBackpack(address owner,BackpackType backpackType,bool isPut,uint256 tokenId) private returns(uint256){
        ...
        uint256 count = tokenId = userBackpacks[owner][backpackType].count;

       if(isPut){

           if(tokenId == 0 && backpackType != BackpackType.Egg && backpackType != BackpackType.Chick && backpackType != BackpackType.Cattle){

               tokenId = farmNFT.createNFT(msg.sender,_getNFTType(backpackType));
           ...
           userBackpacks[owner][backpackType].count = count;

           userBackpacks[owner][backpackType].tokenIds[count] = tokenId;

       }else{

           // 数量不足    
           require(count > 0,"Insufficient items");

           tokenId = userBackpacks[owner][backpackType].tokenIds[count];

           delete userBackpacks[owner][backpackType].tokenIds[count];
    ...
    // 获取NFT类型
    function _getNFTType(BackpackType backpackType) private pure returns(uint256){
        return backpackType == BackpackType.Land || backpackType == BackpackType.Cowshed || backpackType == BackpackType.Henhouse || backpackType == BackpackType.FarmLand ? uint256(backpackType) + 1 : uint256(backpackType);
    }
