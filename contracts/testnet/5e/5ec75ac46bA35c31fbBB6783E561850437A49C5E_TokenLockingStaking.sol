...
contract TokenLockingStaking is TokenWrapper, OwnableUpgradeable {
    ...
    mapping(LockType => uint256) public lockTypeStakingAmount; // 每一种锁仓类型对应的数量
    bool public canLockStack; // 是否可以锁仓质押
    ...
    event LockStaked(address indexed user, uint256 orderId, LockType lockType, uint256 amount);
    event LockWithdrawn(address indexed user, uint256 orderId, LockType lockType, uint256 amount);
    ...
    function _initLocks() internal {
        locks[LockType.DAYS15] = LockData(15 days, 15);
        locks[LockType.DAYS30] = LockData(30 days, 20);
        locks[LockType.DAYS60] = LockData(60 days, 40);
    }
    ...
    function setCanLockStack(bool _enable) external onlyOwner {
        canLockStack = _enable;
    }
    ...
    function lockStake(uint256 amount, LockType _lockType) public updateReward(msg.sender) {
        require(amount > 0, "TokenLockingStaking: Cannot stake 0");
        require(canLockStack, "TokenLockingStaking: Cannot lock stack");
        ...
        lockTypeStakingAmount[_lockType] += amount;
        addLockStackList(amount, _lockType);
        stakeInToken.safeTransferFrom(msg.sender, address(this), amount);
        emit LockStaked(msg.sender, personalOrderId[_msgSender()], _lockType, amount);
    }
    ...
    // 计算质押总量, 和日产量
    function inputParams() public view returns (uint256, uint256) {
        uint256 ratio = fffExchange.fff2ffftRatio();
        return (totalBalance(), dayOut * ratio / 1e18);
    }

    // 计算剩余可产出的数量, 和已产出
    function outputParams() public view returns (uint256, uint256) {
        // uint256 ratio = fffExchange.fff2ffftRatio();
        return ((totalOutput - rewardsGloble), rewardsGloble);
    }

    // 计算每个类型的APY
    function getAPY() public view returns (uint256, uint256[3] memory) {
        uint256[3] memory weigth;
        uint256[3] memory apys;
        (,weigth ) = getWeight();
        uint256 _totalSupply = totalSupply();
        
        uint256 oneWeigthApy = _totalSupply == 0 ? 0: dayOut * 365 * 1e4 / _totalSupply;
        for(uint i=0; i< weigth.length; i++){
            if (_totalSupply == 0) {
                apys[i] = 0;
            } else {
                apys[i] = weigth[i] * oneWeigthApy / 10;
            } 
        }
        return (oneWeigthApy, apys);
    }

    // 获取每个锁仓类型的权重
    function getWeight() public view returns (uint256, uint256[3] memory) {
        uint256[3] memory weigth;
        weigth[0] = locks[LockType.DAYS15].multiplicator;
        weigth[1] = locks[LockType.DAYS30].multiplicator;
        weigth[2] = locks[LockType.DAYS60].multiplicator;
        return (10, weigth);
    }

    // 获取总质押数量
    function totalAmount(address account) public view returns (uint256) {
        uint256 demandBalances = demandBalancesOf(account);
        uint256 amount15 = lockStakingAmount[account][LockType.DAYS15];
        uint256 amount30 = lockStakingAmount[account][LockType.DAYS30];
        uint256 amount60 = lockStakingAmount[account][LockType.DAYS60];
        return demandBalances + amount15 + amount30 + amount60;
    }
    ...
    function lockWithdraw(uint256 _orderId) public updateReward(msg.sender) {
        require(canWithdraw, "TokenLockingStaking: inactive");
        ...
        require(lockOrder.orderId == lockOrderInList.orderId, "TokenLockingStaking: Order Id Error");
        require(lockOrder.status != RedeemStatus.REDEEMED, "TokenLockingStaking: REDEEMED" );
        require(canRedeem(lockOrder.lockTime, lockOrder.lockType), "TokenLockingStaking: NON_REDEEM");
        ...
        _totalBalance = _totalBalance - lockOrder.amount;
        ...
        lockTypeStakingAmount[lockOrder.lockType] -= lockOrder.amount;                 // 记录总的每种锁仓类型的数量
        stakeInToken.safeTransfer(msg.sender, lockOrder.amount);
        emit LockWithdrawn(msg.sender, lockOrder.orderId, lockOrder.lockType, lockOrder.amount);
    }
    ...
    function extractTokens(address outAddress, uint amountToWei) public onlyOwner {
        stakeInToken.safeTransfer(outAddress, amountToWei);
    }
}