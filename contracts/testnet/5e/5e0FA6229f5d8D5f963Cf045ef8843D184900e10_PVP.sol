...
contract PVP is VRFConsumerBaseV2, Ownable {
  ...
  event Played(uint indexed raceId, address indexed winnerAddress, uint winnerRiderId, uint reward, uint jackPot);
  ...
  function createRace(bytes12 _raceType, uint16 _buyin, uint32 _startingShot) external onlyOwner{
    raceCounter++;
  ...
  function register(uint32 _riderId, uint32 _raceId) external payable{
    require(msg.value == radikalStoreInstance.getPvpChainLinkFee(), "PVP: insufficient funds");
    // Pay fee to Radikals team for using Chainlink VRF
    chainLinkRDKFeeAddress.transfer(msg.value);
    Race storage race = races[_raceId];
    require(ridersInstance.isOwner(_riderId, msg.sender) == true, "PVP: rider is not yours");
    require(ridersInstance.getMotorbike(_riderId) == race.raceType, "PVP: motorbike is not compatible");
    require(!ridersInstance.getIsFusioned(_riderId), "PVP: rider blocked after fusion");
    require(block.timestamp < race.startingShot, "PVP: register period is over");
    // Checks if Rider is already registered to this race
    _AlreadyRegistered(_raceId, _riderId);
    require(race.riderId.length < 10, "PVP: players limit exceeded");
    uint rewardAmount = race.buyinToken * 80 / 100;
    uint jackpotAmount = race.buyinToken * 13 / 100;
    rdkToken.transferFrom(msg.sender, address(this), rewardAmount + jackpotAmount);
    // Send 7% of buyin to rewardPool to help having positive token balance of PVE
		rdkToken.transferFrom(msg.sender, rewardPoolAddress, race.buyinToken - rewardAmount - jackpotAmount);
    buyinToJackpot[race.buyinUsdt] += jackpotAmount;
    race.rewardAmount += rewardAmount;
    // Update list with rider Ids that are registered to this race
    race.riderId.push(_riderId);
    race.userList.push(msg.sender);
    emit Registered(msg.sender, _raceId, race.startingShot, race.buyinToken, _riderId, race.riderId, race.rewardAmount, buyinToJackpot[race.buyinUsdt]);
  }
  ...
  function play(uint32 _raceId) external onlyOwner {
    Race memory race = races[_raceId];
    uint8 riderQuantity = uint8(race.riderId.length);
    require(block.timestamp > race.startingShot, "PVP: too soon to play");
    require(riderQuantity > 2, "PVP: registered players are not enough");
    require(race.winnerRiderId == 0, "PVP: This race is already finished");
    // Obtain the total rider points for each skill. Also brings individual rider points for each skill in attributesArray.
    ...
    */
    ...
    races[_raceId].ridersPoints = ridersPoints;
  ...
  function claim(uint32 _raceId) payable external {
    Race memory race = races[_raceId];
    require(race.claimed == false, "PVP: already claimed");
    require(ridersInstance.isOwner(race.winnerRiderId, msg.sender) == true, "PVP: winner rider is not yours");
    rdkToken.transfer(msg.sender, race.rewardAmount + race.jackpot);
  ...
  function _AlreadyRegistered(uint32 _raceId, uint _riderId) internal view {
    ...
    for(uint i = 0; i < race.riderId.length; i++ ){
      ...
      require(repetitionCounter == 0, "PVP: rider already registerd to this race");
    }
  }
  ...
  function _requestRandomWords(uint32 _numWords, uint32 _raceId, uint32[] memory _ridersPoints) internal {
  ...
  function fulfillRandomWords(
    ...
    internal override
  {
    uint32 _raceId = requestToRaceId[requestId];
    ...
    races[_raceId].winnerRiderId = races[_raceId].riderId[winner];
    // Decide if jackpot is released or not. There is a 2% of chances to have a jackpot in every race
    if(randomWords[1] % 100 < 2){
      races[_raceId].jackpot = buyinToJackpot[races[_raceId].buyinUsdt];
      buyinToJackpot[races[_raceId].buyinUsdt] = 0;
    }
    emit Played(_raceId, ridersInstance.getOwnerOf(races[_raceId].winnerRiderId), races[_raceId].winnerRiderId, races[_raceId].rewardAmount, races[_raceId].jackpot);
  }
  ...
  /// @param _raceId Id of race to be checked
  function getRace(uint32 _raceId) external view returns(Race memory) {
  ...
  /// @param _buyin amount of dollars with 2 digits. E.g: 5$ -> 500
  function getJackpot(uint _buyin) external view returns(uint) {
    return buyinToJackpot[_buyin];
  }
  ...
  function setRaceStartingShot(uint32 _raceId, uint32 _newStartingShot) external onlyOwner {
