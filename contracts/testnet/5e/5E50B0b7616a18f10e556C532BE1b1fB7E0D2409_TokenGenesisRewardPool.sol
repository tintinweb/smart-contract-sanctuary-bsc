...
// Note that this pool has no minter key of Bird (rewards).
// Instead, the governance will call Bird distributeReward method and send reward to this pool at the beginning.
contract TokenGenesisRewardPool {
    ...
    uint256 public tokenPerSecond = 0.11574074 ether; // 10000 Tokens / (24h * 60min * 60s)
    ...
    constructor(
        ...
        if (_token != address(0)) tomb = IERC20(_token);
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            ...
            pool.accTokenPerShare = pool.accTokenPerShare.add(_tombReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
