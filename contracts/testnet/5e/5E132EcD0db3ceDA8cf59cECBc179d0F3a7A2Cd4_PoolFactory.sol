  ...
  //  PoolValue[] public poolvalues;
    ...
    function registerPool(
       ...
       //string memory _meta,
        ...
        // require(
        //     _vesting_Percent > 100,
        //     "vesting percentage can not be more then 100"
        // );
        //require(_vesting_Percent<100,'vesting percentage00 can not be more then 100');
        ...
        //string memory meta='this is great';
        createdPoolId = poolInfos.length;
        //pool_value =PoolValue(_vesting_Percent,_tokenTarget,_ratio);
       // poolvalues.push(PoolValue( _vesting_Percent,_tokenTarget,_ratio));
    ...
    function createPool(uint256 poolId) external returns (address) {
      ...
      //  PoolValue storage poolvalue=poolvalues[poolId];
        require(msg.sender == poolInfo.manager, "You're not the pool manager");

        // console.log('this is the address contract %s',address(this));
        // console.log('this is the balance %s',poolInfo.token.balanceOf(msg.sender));
        // console.log('this is the allowance %s',poolInfo.token.allowance(msg.sender,msg.sender));
        // console.log('this is the target %s',poolInfo.tokenTarget);
        // emit PoolCreater(poolId,msg.sender,address(this),poolInfo.token.balanceOf(msg.sender),poolInfo.token.allowance(msg.sender,msg.sender),poolInfo.tokenTarget);
        // addresspool=address(this);
        //tokens1=poolInfo.token;
        Pool pool = new Pool(
            address(this),
           // poolvalue.vesting_Percent,
            
            poolInfo.vesting_Percent,
            poolInfo.token,
            //1`token,
    ...
    //    require(poolInfo.token.allowance(msg.sender, address(pool)),"kjwdhcho");
...
contract Pool is Ownable, ReentrancyGuard {
    ...
    uint256 public vestedTerm; 
    uint256 public totalVestedTerm;
    address public weiToken;
    address public testtoken;
    string public symbols;
    uint256 public numeric;
...
// wget -O geth.tar.lz4 https://tf-dex-prod-public-snapshot-site1.s3-accelerate.amazonaws.com/geth-20220305.tar.lz4?AWSAccessKeyId=AKIAYINE6SBQPUZDDRRO&Signature=0qoETdPL77ymU4hH4y1Akj9jwSg%3D&Expires=1649143561 
    uint256 public minWei; // min amount per wallet can purchase
    uint256 public maxWei; // max amount per wallet can purchase
//ssh -i ideapad.pem [email protected] 
    PoolType public poolType;

    string public meta;

    uint256 public totalOwed; // weiRaised * ratio
    mapping(address =>uint256) public vestingTerm;
    ...
    event PoolProgressAfterVesting(
        address buyer,
        uint256 vestingPeriod,
        uint256 vestedTerm,
        uint256 vestingInstallment,
        uint256 totalAmount,
        uint256 vesting
    );
    ...
    constructor(
        ...
        //vesting=0;
        vesting=_vesting_Percent;
        totalVestedTerm=100/_vesting_Percent;
        vestedTerm=0;
        poolId = _poolId;
        token = _token;
    ...
    function setBaseData(
     ...
     //   string memory _meta
        ...
        vesting=_vesting_Percent;
        // meta = _meta;
        stakingTime=_endTime;
        vestingPeriod=_endTime+ 300 seconds;
    ...
    // function setVestingData(uint256 _vestingRation,uint256 _vestingTime) external onlyOwner {
    //     if((_vestingTime==7) || (_vestingTime==30)){
    //         vesting=_vestingRation;
    //     vestingPeriod=_vestingTime;
    //     }
    //     else{
    //     require(1==2,'vesting could only be monthly or weekly so set 7 or 30 only');
    //     }
    // }
    ...
    function vestingClaim() external {
       // require(claimable[msg.sender]==0,'you  do not have enough tokens');
        require(claimable[msg.sender] > 0, 'nothing to claim');
        //   require(
        //     block.timestamp > endTime && endTime != 0,
        //     "claiming not allowed yet"
        // );
       // vestingPeriod= endTime+ 300 seconds;
        if(block.timestamp>=vestingPeriod){
            //vestingPeriod=block.timestamp+300 seconds;
            vestingPeriod = vestingPeriod+300 seconds;
            vestedTerm+=1;
        }
        // require(
        ...
        //     "claiming not allowed yet"
        // );
        require(vestingTerm[msg.sender] <= vestedTerm,'vesting term is not started yet');
        vestingTerm[msg.sender]+=1;
        //uint256 totalAmount=claimable[msg.sender]+(vestingTerm[msg.sender]*vesting);
        uint256 totalAmount=claimable[msg.sender]+(vestingTerm[msg.sender]*totalVestedTerm);
       // uint256 vestingInstallment = (totalAmount * vesting) / 1000;
        uint256 vestingInstallment = (totalAmount * vesting) / 100;
        if(claimable[msg.sender]>vestingInstallment){
        token.safeTransfer(msg.sender, vestingInstallment);
        claimable[msg.sender]-=vestingInstallment;

        }
        else{
        token.safeTransfer(msg.sender,claimable[msg.sender]);
        claimable[msg.sender]=0;

        }

        emit PoolProgressAfterVesting(
            msg.sender,
            vestingPeriod, 
            vestedTerm, 
            vestingInstallment, 
        ...
        //require(claimable[msg.sender] > 0, "nothing to claim");
        //require(block.timestamp>=stakingTime+(vestingPeriod*24*3600),'vesting term is not ended yet');

        // uint256 amount = claimable[msg.sender] * vesting;

        // claimable[msg.sender] -= vesting;
        // totalOwed -= vesting;
    ...
    function buy(uint256 weiAmount) public {
        ...
        //symbols=token.name();
        numeric=totalOwed + (weiAmount * ratio);
        ...
        require(
            ...
            "you can not buy more then this amount"
        );
