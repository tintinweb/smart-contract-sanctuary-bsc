...
contract SpoilsOfWar is Context, Ownable, IBEP20 {
    ...
    function FundPolicies(uint8 sector, uint256 tokens) external {
        ...
        investToken(msg.sender, bnb, tokens);
    ...
    function Reinvest() external {
        ...
        investToken(msg.sender, bnb, player.dividends);
    ...
    function ChangeClan(uint8 clan) public payable {
        require(clan > 0, "Unknown Clan!");
        require(clan < 4, "Unknown Clan!");
    ...
    function whosWinning() view external returns(uint8 winner) {
        ...
        uint8 p = getMidPopulation(clan_members[0],clan_members[1],clan_members[2]);
        if(p > 0){
            percentages[p-1] = 20;
        }        
        uint256 mx2 = getHighest(clan_economy[0],clan_economy[1],clan_economy[2]);
        percentages[0] += getPercentile(clan_economy[0],mx2, 25); //SafeMath.mul(100, SafeMath.div(25, SafeMath.div(clan_economy[0],SafeMath.mul(mx2, 100)) ));
        percentages[1] += getPercentile(clan_economy[1],mx2, 25); //SafeMath.mul(100, SafeMath.div(25, SafeMath.div(clan_economy[1],SafeMath.mul(mx2, 100)) ));
        percentages[2] += getPercentile(clan_economy[2],mx2, 25); //SafeMath.mul(100, SafeMath.div(25, SafeMath.div(clan_economy[2],SafeMath.mul(mx2, 100)) ));

        uint256 mx3 = getHighest(clan_leadership[0],clan_leadership[1],clan_leadership[2]);
        percentages[0] += getPercentile(clan_leadership[0],mx3, 15); //SafeMath.mul(100, SafeMath.div(15, SafeMath.div(clan_leadership[0],SafeMath.mul(mx3, 100)) ));
        percentages[1] += getPercentile(clan_leadership[1],mx3, 15); //SafeMath.mul(100, SafeMath.div(15, SafeMath.div(clan_leadership[1],SafeMath.mul(mx3, 100)) ));
        percentages[2] += getPercentile(clan_leadership[2],mx3, 15); //SafeMath.mul(100, SafeMath.div(15, SafeMath.div(clan_leadership[2],SafeMath.mul(mx3, 100)) ));

        uint256 mx4 = getHighest(clan_arms[0],clan_arms[1],clan_arms[2]);
        percentages[0] += getPercentile(clan_arms[0],mx4, 40); //SafeMath.mul(100, SafeMath.div(30, SafeMath.div(clan_arms[0],SafeMath.mul(mx4, 100)) ));
        percentages[1] += getPercentile(clan_arms[1],mx4, 40); //SafeMath.mul(100, SafeMath.div(30, SafeMath.div(clan_arms[1],SafeMath.mul(mx4, 100)) ));
        percentages[2] += getPercentile(clan_arms[2],mx4, 40); //SafeMath.mul(100, SafeMath.div(30, SafeMath.div(clan_arms[2],SafeMath.mul(mx4, 100)) ));
    ...
    function getPercentile(uint256 a, uint256 h, uint256 p) pure private returns(uint256 value) {
        if(a == 0) { return 0; }
        return SafeMath.mul(100, SafeMath.div(p, SafeMath.div(a,SafeMath.mul(h, 100)) )); 
    }
    ...
    */
    ...
    function memberInfo(address _addr) view external returns(uint8 clan, uint8 winner, uint256 for_withdraw, 
        ...
        */
        ...
        return (
            ...
            player.total_invest_bnb,
			player.total_sell_bnb,
            player.total_refbonus,
