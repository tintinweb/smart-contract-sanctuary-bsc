...
contract CowMedal is Authorizable, ERC1155 {
    ...
    modifier validTokenId(uint256 _id){
        require(_id > 0 && _id <= totalMedals,"CowMedal: not a valid medal id ");
        _;
    }
    ...
    function changeMedalTypes(uint _totalMedalType) external onlyOwner(){
        require(_totalMedalType > totalMedals,"CowMedal: _totalMedalType must be different");
    ...
    function uri(uint256 _tokenId) public view override returns (string memory) {
        require(_exists(_tokenId), "CowMedal: URI query for nonexistent token");
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _tokenId.toString(),".json")) : "";
    }

    function dropMedal(address[] memory _addresses,uint256[] memory  _quantities,uint256 _id) external {
        require(_addresses.length == _quantities.length,"CowMedal: both array are not equal");
    ...
    function mint(address _to,uint256 _id,uint256 _quantity) public onlyAuthorized() validTokenId(_id) {
        require(_to != address(0),"CowMedal: enter valid address");
        require(_quantity >0,"CowMedal: quantity must be greater than zero");
        if(_id == 1 || _id == 2){
            require(_tokenSupply[_id].add(_quantity) <= maxSupply,"CowMedal: exceed from maxLegendaryMedalSupply");
        }
    ...
    function batchMint(address _to,uint256[] memory _ids,uint256[] memory _quantities) external onlyAuthorized() {
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 _id = _ids[i];
            require(_id <= totalMedals && _id > 0,"CowMedal: not a valid medal Id");
            uint256 quantity = _quantities[i];
            if(_id == 1 || _id == 2){
                require(_tokenSupply[_id].add(quantity) <= maxSupply,"CowMedal: exceed from maxLegendaryMedalSupply");
            }
            require(quantity >0,"CowMedal: quantity must be greater than zero");
    ...
    function burn(address _account,uint256 _id,uint256 _amount) external validTokenId(_id) {
        require(msg.sender == _account || isApprovedForAll(_account,msg.sender),"CowMedal: caller is not owner nor approved");
        require(_amount >0,"CowMedal: _amount must be greater than zero");
    ...
    function batchBurn(address _account,uint256[] memory _ids,uint256[] memory _quantities) external {
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            uint256 _id = _ids[i];
            require(_id <= totalMedals && _id > 0,"CowMedal: not a valid Id");
            uint256 quantity = _quantities[i];
            require(quantity >0,"CowMedal: quantity must be greater than zero");
            require(_tokenSupply[_id] >= quantity,"CowMedal: not a valid amount");
