...
contract MotoDEX is Ownable,IERC721Receiver {
    ...
    */
    ...
    function getGameSessions() public view returns (uint256 [] memory, uint256) {
        ...
        uint256 [] memory list = new uint256[](balanceOf * 5);
        uint256 listCount = 0;
        for (uint trackIdIndex; trackIdIndex < trackIdsCount; trackIdIndex++) {
            ...
            for (uint motoIdIndex = 0; motoIdIndex < motoIdsCount; motoIdIndex++) {
                ...
                if (gameSessions[trackTokenId][motoTokenId].latestUpdateTime > 0) {
                    ...
                    list[listCount] = trackTokenId;
                    listCount++;
                    list[listCount] = motoTokenId;
                    listCount++;
                    list[listCount] = latestUpdateTime;
                    listCount++;
                    list[listCount] = latestTrackTimeResult;
                    listCount++;
                    list[listCount] = attempts;
                    listCount++;
                }
            }
        }
