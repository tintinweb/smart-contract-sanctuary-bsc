...
contract Pool is OwnableUpgradeSafe, ReentrancyGuard, IPool {
    ...
    function mint(
        ...
        if (_required_shareAmount > _shareAmount) {
            ...
            require(_mainCollateralAmount <= _collateralAmounts[0].add(_collateralAmounts[1]).add(_collateralAmounts[2]), "invalid input quantities");
        }
        ...
        require(_mainCollateralAmount <= _totalMainCollateralAmount.add(10), "Not enough _collateralAmount"); // plus some dust for overflow
        ...
        if (_required_collateralAmounts[0] >= _slippageAmount) {
            _required_collateralAmounts[0] = _required_collateralAmounts[0].sub(_slippageAmount);
        } else {
            if (_collateralAmounts[0] >= _slippageAmount) {
                _required_collateralAmounts[0] = _collateralAmounts[0].sub(_slippageAmount);
            } else {
                _required_collateralAmounts[0] = 0;
                _slippageAmount = _slippageAmount.sub(_collateralAmounts[0]);
                if (_required_collateralAmounts[1] > _slippageAmount) {
                    _required_collateralAmounts[1] = _required_collateralAmounts[1].sub(_slippageAmount);
                } else {
                    _required_collateralAmounts[1] = 0;
                }
            }
        }
    ...
    function trimExtraToTreasury() public returns (uint256 _collateralAmount, uint256 _darkAmount, uint256 _shareAmount) {
        uint256 _collateral_price = getCollateralPrice(0);
        uint256 _total_dollar_supply = IERC20(dollar).totalSupply();
        uint256 _total_dollar_FullValue = _total_dollar_supply.mul(_collateral_price).div(PRICE_PRECISION);
        uint256 _totalCollateralValue = ITreasury(treasury).globalCollateralTotalValue();
        uint256 _dark_bal = ITreasury(treasury).globalDarkBalance();
        uint256 _share_bal = ITreasury(treasury).globalShareBalance();
        address _profitSharingFund = ITreasury(treasury).profitSharingFund();
    ...
    function _transferToReserve(address _token, address _sender, uint256 _amount, uint256 _fee) internal {
        if (_amount > 0) {
            ...
            if (_token == share) {
                ITreasury(treasury).requestBurnShare(_fee);
                ITreasury(treasury).reserveReceiveShare(_amount.sub(_fee));
            } else if (_token == dark) {
                ITreasury(treasury).requestTransferDarkFee(_fee);
                ITreasury(treasury).reserveReceiveDark(_amount.sub(_fee));
            }
        }
    }
    ...
    function updateTargetCollateralRatio() external override onlyStrategist {
        if (lastUpdatedTargetCR.add(updateCoolingTimeTargetCR) <= block.timestamp) { // to avoid update too frequent
            ...
            if (_dollarPrice > PRICE_PRECISION) {
                ...
                // the system should be in de-collateralize mode by decreasing the collateral ratio, minimum to 5%
                targetCollateralRatio = Math.min(9500, targetCollateralRatio.add(updateStepTargetCR));
            } else {
                // When the price of DUSD is below $1, the function increases the collateral ratio, maximum to 30%
