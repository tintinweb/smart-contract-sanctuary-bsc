...
contract CerosYieldConverterStrategy is BaseStrategy {
    ...
    function _deposit(uint256 amount) internal returns (uint256 value) {
        ...
        // (, bool enoughLiquidity) = IBinancePool(_binancePool).getAmountOut(true, amount, false); // (amount * ratio) / 1e18
    ...
    function _withdraw(uint256 amount) internal returns (uint256 value) {
        ...
        if (canWithdraw(amount - wethBalance)) {
            value = _ceRouter.withdrawWithSlippage(address(this), amount - wethBalance,  amount - wethBalance);
            require(value >= amount - wethBalance, "invalid out amount");
    ...
    function canWithdraw(uint256 amount) public view returns(bool) {
        uint256 amountOut = _ceRouter.getAmountOut(address(_certToken), address(underlying), (amount * _certToken.ratio()) / 1e18);
    ...
    function harvestAndSwap() external onlyStrategist {
        ...
        // (uint256 amountOut, bool enoughLiquidity) = _ceRouter.getAmountOut(address(_certToken), address(underlying), yield);
    
        // if (enoughLiquidity && amountOut > 0) {
        //     amountOut = IBinancePool(_binancePool).swap(false, yield, address(this));
