...
contract PandoraZap is ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    address public WBNB = 0xD4Bb551E18CB643d1086deDF4bCada3A6a4AB38f;
    ...
    uint public constant SWAP_FEE_PERCENT = 30;

    mapping(address => mapping(address => address[])) public swapPairs; // [input -> output] => pair
    ...
    event swapPairsChanged(address indexed tokenIn, address indexed tokenOut, address[] oldPairs, address[] newPairs);
    event swapRouterChanged(address indexed oldRouter, address indexed newRouter);
    event VSwapRouterChanged(address indexed oldRouter, address indexed newRouter);
    event VSwapFactoryChanged(address indexed oldFactory, address indexed newFactory);
    event VSwapFormulaChanged(address indexed oldFormula, address indexed newFormula);
    ...
    function zapInToken(
        ...
        if (_from == IUniswapV2Pair(_to).token0() || _from == IUniswapV2Pair(_to).token1()) {
            ...
            uint256 sellAmount = amounts[0] / 2;
            uint256 otherAmount = _swap(_from, sellAmount, other, address(this), _to);
            require(otherAmount >= amounts[1], "High slippage");

            lpAmt = _pairDeposit(_to, _from, other, amounts[0] - sellAmount, otherAmount, msg.sender, transferResidual);
        } else {
    ...
    function zapIn(
        ...
        uint256 lpAmt = _swapBNBToLp(IUniswapV2Pair(_to), msg.value, msg.sender, _minTokenB, transferResidual);
    ...
    function zapOutToPair(address _from, uint256 amount) public nonReentrant returns (uint256 amountA, uint256 amountB) {
        ...
        if (token0 == WBNB || token1 == WBNB) {
            (amountA, amountB) = swapRouter.removeLiquidityETH(token0 != WBNB ? token0 : token1, amount, 1, 1, msg.sender, block.timestamp);
        } else {
            (amountA, amountB) = swapRouter.removeLiquidity(token0, token1, amount, 1, 1, msg.sender, block.timestamp);
        }
    }
    ...
    function zapOut(
        ...
        uint256 amountB;
        {
            ...
            token0 = pair.token0();
            token1 = pair.token1();
            (amountA, amountB) = swapRouter.removeLiquidity(token0, token1, amount, 1, 1, address(this), block.timestamp);
        }
        ...
        if (_toToken == BNB_ADDRESS) {
            ...
            tokenBought += _swapTokenForBNB(token1, amountB, address(this), _lpOfFromAndTo);
        } else {
    ...
    function zapOutToPairWithPermit(
        ...
        // permit
        IUniswapV2Pair(_from).permit(msg.sender, address(this), _approvalAmount, _deadline, v, r, s);
    ...
    function getAmountBToZapIn(
        ...
        uint256 _fromAmount,
        address lp
    ) external view returns (uint256 amountBConverted, uint256 amountASell, uint256 amountLP) {
        address other;
        address token0 = IUniswapV2Pair(lp).token0();
        address token1 = IUniswapV2Pair(lp).token1();
        other = _from == token0 ? token1 : token0;

        uint256 sellAmount = _fromAmount / 2;
        address[] memory path = swapPairs[_from][lp];
        uint256[] memory amounts = swapRouter.getAmountsOut(sellAmount, path);

        uint256 _totalSupply = IERC20(lp).totalSupply();
        uint256 amountLP0 = amounts[0] * _totalSupply / IERC20(token0).balanceOf(lp);
        uint256 amountLP1 = amounts[amounts.length - 1] * _totalSupply / IERC20(token1).balanceOf(lp);
        return (amounts[amounts.length - 1], sellAmount, amountLP0 < amountLP1 ? amountLP0 : amountLP1);
    }
    ...
    )
    {
        ...
        uint256 amount1;
        {
            ...
            amount0 = amount * IERC20(token0).balanceOf(_from) / _totalSupply;
            amount1 = amount * IERC20(token1).balanceOf(_from) / _totalSupply;
        }

        address other = _toToken == token0 ? token1 : token0;
        uint256 sellAmount = _toToken == token0 ? amount1 : amount0;
        address[] memory path =  swapPairs[_from][_toToken];
        uint256[] memory amounts = swapRouter.getAmountsOut(sellAmount, path);
        return (sellAmount, amounts[amounts.length - 1], _toToken == token0 ? amount0 : amount1);
    }

    // toToken : lp
    function getPriceImpactWhenZapIn(address fromToken, uint amountIn, address toToken) external view returns (uint priceImpact) {
       return calculatePriceImpact(fromToken, amountIn, toToken);
    }
    ...
    function _pairDeposit(
        ...
        uint256 amountA;
        uint256 amountB;
        (amountA, amountB, lpAmt) = swapRouter.addLiquidity(_poolToken0, _poolToken1, token0Bought, token1Bought, 1, 1, receiver, block.timestamp);

        if (transferResidual) {
            //Returning Residue in token0, if any.
            if (token0Bought - amountA > 0) {
                _transferToken(_poolToken0, msg.sender, token0Bought - amountA);
            }

            //Returning Residue in token1, if any
            if (token1Bought - amountB > 0) {
                _transferToken(_poolToken1, msg.sender, token1Bought - amountB);
            }
        }
    ...
    function _swapBNBToLp(
        ...
        if (pair.token0() == WBNB || pair.token1() == WBNB) {
            ...
            uint256 wbnbAmount = amount - swapValue;
            IWETH(WBNB).deposit{value: wbnbAmount}();
            lpAmt = _pairDeposit(lp, WBNB, token, wbnbAmount, tokenAmount, receiver, transferResidual);
        } else {
            address token0 = pair.token0();
            address token1 = pair.token1();
            ...
            {
                uint256 swap0Value = amount / 2;
                token0Amount = _swapBNBForToken(token0, swap0Value, address(this), address(0));
                token1Amount = _swapBNBForToken(token1, amount - swap0Value, address(this), address(0));
            }

            lpAmt = _pairDeposit(lp, token0, token1, token0Amount, token1Amount, receiver, transferResidual);
        }
    }
    ...
    function _swapBNBForToken(
        ...
        address[] memory path = swapPairs[WBNB][token];
        uint256[] memory amounts;
        if (path.length > 0) {
            amounts = swapRouter.swapExactETHForTokens{value: value}(1, path, _receiver, block.timestamp);
        } else if (lpBNBToken != address(0)) {
            path = new address[](1);
            path[0] = lpBNBToken;
            amounts = swapRouter.swapExactETHForTokens{value: value}(1, path, _receiver, block.timestamp);
        } else {
            revert("PandoraZap: !path BNBToken");
        }
    ...
    function _swapTokenForBNB(
        ...
        address[] memory path = swapPairs[token][WBNB];
        uint256[] memory amounts;
        if (path.length > 0) {
            amounts = swapRouter.swapExactTokensForETH(amount, 1, path, _receiver, block.timestamp);
        } else if (lpTokenBNB != address(0)) {
            path = new address[](1);
            path[0] = lpTokenBNB;
            amounts = swapRouter.swapExactTokensForETH(amount, 1, path, _receiver, block.timestamp);
        } else {
            revert("PandoraZap: !path TokenBNB");
        }
    ...
    function _swap(
        ...
        address[] memory path = swapPairs[_from][_to];
        uint256[] memory amounts;
        if (path.length > 0) {
            // use vSwap
            amounts = swapRouter.swapExactTokensForTokens(_amount, 1, path, _receiver, block.timestamp);
        } else if (_lpOfFromTo != address(0)) {
            path = new address[](1);
            path[0] = _lpOfFromTo;
            amounts = swapRouter.swapExactTokensForTokens( _amount, 1, path, _receiver, block.timestamp);
        } else {
            revert("PandoraZap: !path swap");
        }
    ...
    function getReserves(address lp, address fromToken) internal view returns(uint256 reserveA, uint256 reserveB){
        (reserveA, reserveB, ) = IUniswapV2Pair(lp).getReserves();
        if (fromToken != IUniswapV2Pair(lp).token0()) {
            return (reserveB, reserveA);
        }
    }

    //toToken : lp
    function calculatePriceImpact(address fromToken, uint amountIn, address toToken) internal view returns (uint priceImpact) {
        address[] memory path = swapPairs[fromToken][toToken];

        uint midPrice = MULTIPLIER;

        for (uint i = 0; i < path.length - 1; i++) {
            (uint reserveA, uint reserveB) = getReserves(swapRouter.pairFor(path[i], path[i + 1]), path[i]);
            midPrice = midPrice * reserveB / reserveA;
        }

        uint exactQuote = midPrice * amountIn / MULTIPLIER;

        uint[] memory amountOuts = swapRouter.getAmountsOut(amountIn, path);

        uint impact = (exactQuote - amountOuts[amountOuts.length - 1]) * ONE_HUNDRED_PERCENT / exactQuote;
    ...
    function setSwapPairs(
        ...
        address[] memory oldpair = swapPairs[_input][_output];
        swapPairs[_input][_output] = _pair;
        emit swapPairsChanged(_input, _output, oldpair, _pair);
    }
    ...
    function setWBNB(address _WBNB) external onlyOwner {
        ...
        WBNB = _WBNB;
        emit WBNBChanged(oldWBNB, _WBNB);
    }
}