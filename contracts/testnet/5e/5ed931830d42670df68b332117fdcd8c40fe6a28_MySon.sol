...
contract MySon is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MySonDividendTracker public dividendTracker;
    ...
    constructor() ERC20("My Son", "SON") {
        ...
        address router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; //Mainnet 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506 - testnet bsc 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        ...
        dividendTracker = new MySonDividendTracker(
            payable(this),
            router,
            0x64544969ed7EBf5f083679233325356EbE738930, // USDC Arbitrum 0xFF970A61A04b1cA14834A43f5dE4533eBDDB5CC8 - testnet bsc 0x64544969ed7EBf5f083679233325356EbE738930
            "MySonTRACKER",
            "MySonTRACKER"
        );
        ...
        uint256 totalTokenSupply = (1_000_000) * (10**18); // 1 Million
...
contract MySonDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        MySonContract = MySon(mainContract);
    ...
    function _transfer(
        ...
        require(false, "MySon_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "MySon_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(MySonContract);

            uint256 prevBalance = MySonContract.balanceOf(address(this));
            ...
            {
                uint256 received = MySonContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    MySonContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
