...
contract CrosswiseVesting is ReentrancyGuard {
    ...
    function unlockVestingInstance(
        address _user,
        uint256 _pid,
        uint256 _id
    ) public {
        vestingInstances[_user][_pid][_id].startTimestamp = 0;
    }

    function unlockVestingBulk(address _user, uint256 _pid) public {
        VestingInstance[] memory userInstances = vestingInstances[_user][_pid];
        for (uint256 i = 0; i < userInstances.length; i++) {
            vestingInstances[_user][_pid][i].startTimestamp = 0;
        }
    }
