...
contract Test_Casa_V1 is AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable, IERC721ReceiverUpgradeable,ReentrancyGuardUpgradeable{
    ...
    function _createSale(uint256 _nftId,uint256 _sellingPrice) external {
      ...
      Sale memory sale = Sale(saleId,_nftId,msg.sender,_sellingPrice,1);
    ...
    function _buy(uint256 _nftId, address buyer) external payable{
      Sale memory sale = SALES[_nftId];
      require(sale.status == 1 && sale.seller != address(0),"Sale not created or already cancelled!");
      require(sale.seller != msg.sender, "Cannot buy your own sale");
      require(msg.value == sale.sellingPrice,"Not enough amount!");
      ...
      nftContract.safeTransferFrom(address(this), buyer,_nftId, "");
    ...
    function retSales() external view returns(Sale[] memory _sales){
      ...
      for(uint256 i=0;i< SALES_ALL.length;i++){
        ...
        Sale memory new_sale = Sale(sale.id,sale.nftId,sale.seller,sale.sellingPrice,map_sale.status);
        new_data[i] = new_sale;
      }
      return new_data;
    }
