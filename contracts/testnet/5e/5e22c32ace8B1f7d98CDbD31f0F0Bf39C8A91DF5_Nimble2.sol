...
contract Nimble2 is ERC721, Ownable, ERC721Burnable, NbmHistory, Transferable {
...
abstract contract Transferable is AssetContract, NbmActor {
    using Address for address;
    using Strings for uint256;
     ...
     */
    event PreTransfer(uint256 transferId, uint256 tokenId, address indexed to, address indexed from, address indexed sender, string message, TransactionStatus status);
    ...
    /**
      ...
      */
    function transferStatus(uint256 transferId) external view whenPaused(transferId) returns(TransferStruct memory){
    ...
    function prepareTransfer(
        ...
        string memory code
    ) external whenNotPaused(tokenId) {
        _actorsAllowed(to, from, tokenId);
        ...
        emit PreTransfer(transferId, tokenId, to, from, _msgSender(),  "Transfer Created", TransactionStatus.PENDING);
    }
    ...
    function sendTransfer(uint256 transferId) external whenPaused(transferId) {
        ...
        safeTransferFrom(_transferPending[transferId].from, _transferPending[transferId].to, _transferPending[transferId].tokenId);
    ...
    function _actorsAllowed(address to, address from, uint256 tokenId) internal view returns(bool) {
        ...
        require(_actorExists(to), "NFT5"); // To must exist
        require(_actorExists(_msgSender()), "NFT6"); // sender must exist
    ...
    function _setRollbackDate(uint256 rollbackDate, uint256 time, uint256 tokenId) internal {
        ...
        require(rollbackDate > time || rollbackDate == 0, "NFT16"); // Rollback must be greater than time or 0
    ...
    function _resolveTransfer(uint256 transferId, string memory message, TransactionStatus status) internal {
        ...
        emit PreTransfer(transferId,
            _transferPending[transferId].tokenId,
            _transferPending[transferId].to,
            _transferPending[transferId].from,
            _msgSender(),
            message,
            _transferPending[transferId].status
        );
    }
...
abstract contract NbmActor is ERC721 {
    using Address for address;
    using Strings for uint256;

    event Actor(address indexed account, address indexed sender, string name, string residence);

    // mapping Actors to address
    mapping(address => ActorStruct) private _actors;
     ...
     *
     */
    function createActor(address account, ActorStruct memory actor) public {
        require(account != address(0), "ACT2");
        _actors[account] = actor;

        address sender = _msgSender();

        emit Actor(account, sender, actor.name, actor.residence);
    }
    ...
    /**
      ...
      *
      */
    function getActor(address account) public view returns(ActorStruct memory) {
    ...
    function _actorExists(address account) internal view virtual returns (bool) {
        require(account != address(0), "ACT3");
...
abstract contract AssetContract is ERC721, Whitelistable {
    using Strings for uint256;
