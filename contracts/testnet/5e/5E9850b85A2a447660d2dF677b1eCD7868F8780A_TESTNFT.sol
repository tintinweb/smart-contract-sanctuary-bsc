...
contract TESTNFT is ERC721AA, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721AA("TESTTOKEN", "TTT") {
    setUriPrefix("ipfs://QmP6NYVazcMkcZXHQggewaPfmNDax6Q2FoWo7hgGezVhJv/");
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    for(uint256 i = 0; i < _mintAmount; i++) {
        ...
        nfts[_currentIndex].blindBoxType = _generateRandomNum(_currentIndex);
        emit Mint(msg.sender, _currentIndex); 
    }
  }
  ...
  function walletOfOwner(address  _owner) public view returns(uint16[] memory) {
      uint16[] memory ownedNFT = new uint16[](balanceOf(_owner));
      uint16 counter = 0;
