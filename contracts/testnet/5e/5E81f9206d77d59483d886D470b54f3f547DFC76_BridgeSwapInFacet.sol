...
import "../interfaces/IBridgeSwapInFacet.sol";
import "../interfaces/IAccessFacet.sol";
import "../interfaces/IBridgeManagerFacet.sol";
import "../interfaces/IERC20.sol";

contract BridgeSwapInFacet is IBridgeSwapInFacet {
    ...
    event BalanceInStatementEvent(
        address _tokenAddr,
        uint256 _balance,
        uint256 _blocktime
    );
    // swap
    // from other chain this chain 
    function bridgeIn(
        address _tokenAddr,
        address _toAddr,
        ...
        IBridgeManagerFacet(address(this)).checkPermisionBridgeControlByToken(msg.sender,_tokenAddr);
        ...
        require(
            ...
            "Ticket ID is require"
        );

        LibDiamond.SwapInTicketStruct memory swapInTicket = LibDiamond.getSwapInTicket(_ticket);
        require(swapInTicket.isExistTicket == false,"BridgeSwapOutFacet: Ticket is ready exist");
        ...
        require(
            ...
            IBridgeManagerFacet(address(this)).getAllowSourceChain(_tokenAddr,_sourceChain),
        ...
        require(IBridgeManagerFacet(address(this)).erc20ContractBalance(_tokenAddr) >= _amount, "Balance is not enough");

          // Store transaction data into mapping with key sourceTx(Bridge Ticket ID)
        LibDiamond.SwapInTicketStruct memory swapInTicketNew = LibDiamond.SwapInTicketStruct({
            destAddr: _toAddr,
        ...
        LibDiamond.setSwapInTicket(_ticket,swapInTicketNew);

          // Transfer erc20 Token from contract to user's destination address
    ...
    function getBridgeInTicket(string memory _ticketId) public view override returns (LibDiamond.SwapInTicketStruct memory){
        return LibDiamond.getSwapInTicket(_ticketId);
    }
...
interface IBridgeSwapInFacet {
    ...
    function getBridgeInTicket(string memory _ticketId) external view returns (LibDiamond.SwapInTicketStruct memory);
}
