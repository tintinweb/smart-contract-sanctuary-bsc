...
contract Vesting is IVesting, Ownable {
  ...
  function createVesting(
    ...
    _createVesting(_beneficiary, _amount, _vestingSchedule);
  }
  ...
  function createVestingsBatch(
    ...
    for (uint256 i = 0; i < _beneficiaries.length; i++) {
      _createVesting(_beneficiaries[i], _amounts[i], _vestingSchedules[i]);
      if (gasleft() < 40000) {
        break;
      }
    }
  ...
  function _calculateAvailableAmount(Vesting memory _vesting) internal view returns (uint256) {
    ...
    uint256 vestingStartDate = _vesting.vestingSchedule.startDate;
    // solhint-disable-next-line not-rely-on-time
    uint256 currentTime = block.timestamp;

    if (currentTime < vestingStartDate) {
      return 0;
    } else if (currentTime >= vestingStartDate + _vesting.vestingSchedule.vestingDuration) {
      return totalVestingAmount;
    } else {
      return
        (totalVestingAmount * (currentTime - vestingStartDate)) /
        _vesting.vestingSchedule.vestingDuration;
    }
  }
}
