...
contract StrategyHelper is LogicUpgradeable {
    ...
    function _test() external view returns (address[] memory tokenList) {
        tokenList = IStorage(_storage).getUsedTokens();
    }

    function _getStorage() external view returns (address) {
        return _storage;
    }
