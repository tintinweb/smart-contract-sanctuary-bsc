...
contract Chance is ERC721URIStorage, ERC721Enumerable, Ownable, Pausable, ReentrancyGuard{
    ...
    function batchMint(address _to, uint256 _count) public whenNotPaused returns(uint256[] memory) {
        ...
        // require(_tokenIds.current() + _count - lastEnd <= 100, "roundJoinMax"); 
        ...
        for (uint i = 0; i < _count; i++) {
            ...
            result[i] = tokenId;
            // emit mintLog(_to, tokenId, round, lastEnd);
        }
        return result;
    }
    ...
    //     require(block.timestamp - winnerMap[_r]._timestamp <= winnerTTL, "WithdrawTimeExpired" );
    //     _;
    // }
    ...
    //     require(ownerOf(winnerMap[_r]._winnerID) == msg.sender, "NotWin");
...
contract ChanceDrop is Ownable {
    ...
    function drop_winner_token_id(uint256 round, uint256 end, uint256 lastEnd) external view returns (uint256){
        ...
        // uint256 _seed = IVRFv2Consumer(VRF_ADDR).getSeed(round%24);
