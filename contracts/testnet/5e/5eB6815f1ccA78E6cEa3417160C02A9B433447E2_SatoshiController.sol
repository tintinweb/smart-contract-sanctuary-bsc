    ...
    PausableUpgradeable
{
    ...
    /**
     * @dev returns the latest price of a particular coin using its address
     * note This value returned by this function has been scaled by a constant value,
     * if you are just calling to get the value of your token then make sure you descale it
     */
    function getLatestPrice(address _tokenAddress) public view isAddress(_tokenAddress) returns (uint) {
        require(StableCoin != address(0), "INVALID_STABLE_COIN_ADDRESS");
        require(Factory != address(0), "INVALID_FACTORY_ADDRESS");
        uint price = _getPrice(_tokenAddress);
        return price;
    }
        ...
        returns (uint)
    {
        ...
        {  //preventing stack too deep
            if (_tokenAddress == StableCoin) {
                ...
                tokenUsdPrice = (token0 == _tokenAddress)
    ...
    /**
     * @dev returns the products detail using its ID
     */
    ...
    function makePayment(address _paymentToken, uint256 _amountInUSD)
        ...
        whenNotPaused
    {
        //check if user is paying with a native or normal 
        if (msg.value > 0) {
        ...
        internal
    {
        ...
        uint256 tokenPriceInUsd = uint256(getLatestPrice(paymentTokenInfo.tokenAddress));
    ...
    function _chargeNative(uint256 _amountInUSD) internal { 
        require(nativeToken.tokenAddress != address(0), "INVALID_STABLE_COIN_ADDRESS");
        uint256 tokenPriceInUsd = uint256(getLatestPrice(nativeToken.tokenAddress));
        require(tokenPriceInUsd > 0 , "SATOSHICONTROLLER : Native Token price is zero");
        uint256 tokenAmountToPay = (((_amountInUSD * (10**nativeToken.decimal)) / (tokenPriceInUsd/scale))); //descaling
        {
    ...
    function addToken(
        ...
        address tokenAddress,
        uint256 discountPercent
    ) external isAddress(tokenAddress) onlyOwner {
    ...
    function setNativeTokenData(
        ...
        address tokenAddress,
        uint256 decimal
    ) external isAddress(tokenAddress) isZero(decimal) onlyOwner {
        nativeToken = NativeTokenDetail(status, isDiscountActive, discountPercent, tokenAddress, decimal);
    }
