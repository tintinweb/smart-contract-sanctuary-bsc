...
contract PancakeRouter is IPancakeRouter02 {
    ...
    modifier onlyFeeSetter() {
        require(msg.sender == feeSetter, "PancakeRouter: FORBIDDEN");
        _;
    }
    ...
    function setFees(uint16 _fees) external onlyFeeSetter {
        require(fees <= feeDenominator, "PancakeRouter: INVALID_FEES");
        fees = _fees;
    }
    ...
    function swapExactTokensForTokens(
        ...
        amounts = PancakeLibrary.getAmountsOut(factory, amountIn, path, fees);
    ...
    function swapTokensForExactTokens(
        ...
        amounts = PancakeLibrary.getAmountsIn(factory, amountOut, path, fees);
    ...
    function swapExactETHForTokens(
        ...
        amounts = PancakeLibrary.getAmountsOut(factory, amountInWithFee, path, 10000);
        require(amounts[amounts.length - 1] >= amountOutMin, "PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function swapTokensForExactETH(
        ...
        amounts = PancakeLibrary.getAmountsIn(factory, amountOut, path, 10000);
        require(amounts[0] <= amountInMax, "PancakeRouter: EXCESSIVE_INPUT_AMOUNT");
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
    ...
    function swapExactTokensForETH(
        ...
        require(path[path.length - 1] == WETH, "PancakeRouter: INVALID_PATH");
        amounts = PancakeLibrary.getAmountsOut(factory, amountIn, path, 10000);
        require(amounts[amounts.length - 1] >= amountOutMin, "PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT");
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
    ...
    function swapETHForExactTokens(
        ...
        require(amountInWithFee <= msg.value, "PancakeRouter: EXCESSIVE_INPUT_AMOUNT");
        IWETH(WETH).deposit{value: amountInWithFee}();

        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, path[0], path[1]), amountInWithFee));
        _swap(amounts, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to, uint16 fees_) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 amountOutput;
            {
                ...
                amountOutput = PancakeLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, fees_);
            }
            (uint256 amount0Out, uint256 amount1Out) =
    ...
    function getAmountOut(
        ...
        return PancakeLibrary.getAmountOut(amountIn, reserveIn, reserveOut, fees_);
    }
    ...
    function getAmountIn(
        ...
        return PancakeLibrary.getAmountIn(amountOut, reserveIn, reserveOut, fees_);
    }
    ...
    {
        return PancakeLibrary.getAmountsOut(factory, amountIn, path, fees_);
    }
    ...
    function getAmountsIn(uint256 amountOut, address[] memory path, uint16 fees_)
        ...
        returns (uint256[] memory amounts)
    {
        return PancakeLibrary.getAmountsIn(factory, amountOut, path, fees_);
    }
