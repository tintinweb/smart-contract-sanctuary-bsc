...
pragma solidity 0.5.16;
  ...
  contract MyCoin is Context, IBEP20, Ownable {
    ...
    uint256 private globalLoss;
    uint256 private globalGain;
    uint256 private decreaseRate = 20000;
    uint256 private w = 14000;
    uint256 private v = 7;
    uint256 private l = 200000;
    uint256 private N;
    ...
    function getGlobalLoss(uint256 R) public view returns (uint256) {
      uint256 c = getCurrentPrice();
      uint256 _globalLoss = globalLoss + nextDecrease(R);
      uint256 _globalGain = globalGain + nextIncrease(R);
      return c * _globalLoss / (_globalLoss + _globalGain);
    }

    function getGlobalGain(uint256 R) public view returns (uint256) {
      uint256 c = getCurrentPrice();
      uint256 _globalLoss = globalLoss + nextDecrease(R);
      uint256 _globalGain = globalGain + nextIncrease(R);
      return c * _globalGain / (_globalLoss + _globalGain);
    }
    ...
    function getGain() internal view returns (uint256) {
      ...
      uint256 _globalLoss = globalLoss + nextDecrease(R);
      uint256 _globalGain = globalGain + nextIncrease(R);
      uint256 diffA = (_globalGain > _globalLoss) ? _globalGain - _globalLoss : 1;
      uint256 diffB = (_globalLoss > _globalGain) ? _globalLoss - _globalGain : 1;
    ...
    function nextDecrease(uint256 R) internal view returns (uint256) {
      ...
      uint256 diffA = (globalGain > globalLoss) ? globalGain - globalLoss : 1;
      uint256 diffB = (globalLoss > globalGain) ? globalLoss - globalGain : 1;
      uint256 temp = (c - R); 
      if(temp > c) temp = c;
      uint256 d = temp * decreaseRate * (getNow() - lastUpdate) * l / 100000000000000000;
      return d * (c + diffA) ** 2 / (c + diffB) ** 2; 
    }
    ...
    function nextIncrease(uint256 R) internal view returns (uint256) {
      ...
      uint256 temp = (R - c);
      if(temp > c) temp = c;      
      return temp * (getNow() - lastUpdate) * l / 10000000000000;
    }
    ...
    function getGlobals() external view returns (uint256[13] memory) {
      (uint256 R, uint256 c, uint256 r) = getVars();
      return [c, R, r, getGain(), prevPrice, globalLoss, globalGain, v, w, l, nextDecrease(R), nextIncrease(R), getNow()];
    }
