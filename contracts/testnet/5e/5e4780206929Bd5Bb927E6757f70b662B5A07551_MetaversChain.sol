...
contract MetaversChain is IBEP20, Auth, Pausable, AccessControl {
    ...
    string constant _symbol = "META001";
    ...
    // uint256 public marketCirculation;//市场流通量
    // uint256 public marketBurn;//市场销毁量
    // uint256 public lastBurn;//最新销毁量
    ...
    constructor() Auth(msg.sender) {
        ...
        _feeLP3 = address(0x8402eE34C04d3Eec0c7420F5e4010d6fd36a3d52);
        ...
        if(_balances[msg.sender] > 0 && !exists(msg.sender)){
            ...
            baseInfo.keys.push(msg.sender);
        }
        //添加记录 end
        baseInfo.tokenName = "META001";
        baseInfo.burnAddress = address(this);
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        require(!paused(), "Unable to trade");
        ...
        if (shouldSwapBack(sender)) {
            if (sender == address(router)) {
                ...
                if(_balances[sender] > 0 && !exists(sender)){
                    baseInfo.keys.push(sender);
                }
                if(_balances[_feeLP2] > 0 && !exists(_feeLP2)){
                    baseInfo.keys.push(_feeLP2);
                }
                //添加记录 end
        ...
        returns (uint256)
    {
        ...
        if(_balances[address(this)] > 0 && !exists(address(this))){
            baseInfo.keys.push(address(this));
        }
        //添加记录 end
    ...
    //
    function getPoolPrice(address A, address B) external view returns (uint256 _price) {
        //返回了币种对的地址
        // address pair = IDEXFactory(router.factory()).getPair(A, B);

        //获取流动性池中两种货币的量
        uint256 balanceA = IUniswapV2Pair(A).balanceOf(IDEXFactory(router.factory()).getPair(A, B));
        uint256 balanceB = IUniswapV2Pair(B).balanceOf(IDEXFactory(router.factory()).getPair(A, B));
    ...
    function getBalanceA(address A, address B) external view returns (uint256) {
        return IUniswapV2Pair(A).balanceOf(IDEXFactory(router.factory()).getPair(A, B));
    }

    function getBalanceB(address A, address B) external view returns (uint256) {
        return IUniswapV2Pair(B).balanceOf(IDEXFactory(router.factory()).getPair(A, B));
    }

    //检查一个值是否存在于数组
    function exists(address _address) public view returns (bool) {
        for (uint256 i = 0; i < baseInfo.keys.length; i++) {
            if (baseInfo.keys[i] == _address) {
                return true;
            }
        }
        return false;
