...
contract Token is ReentrancyGuard, IToken{
    ...
    function purchaseTokensWithOtherToken(uint256 amount, address tokenToTrade, uint256 minAssetValue) external nonReentrant returns(bool) {
        ...
        require(tradeToken.transferFrom(msg.sender, address(this), amount), 'Failure transferring tokens');

        // Set the amount to the number of tokens we actually received from the transfer.
        amount = IERC20(tokenToTrade).balanceOf(address(this)) - amountOfTokenToTradeBefore;
    ...
    function mint(uint256 amount, address receiver) private returns(bool) {
        ...
        // If the recipient does not already have a principal balance
        // then they won't be in the holder array yet. Add them to the array.
        if(balances[receiver].principal == 0){
            addHolder(receiver);
        }
    ...
    function swapForAsset(uint256 amount, address tokenToTrade, uint256 minAssetValue) private returns(uint256){
        ...
        uint256 amount_out_a;
        uint256 amount_out_b;

        // Make sure the token is paired with the asset token on the exchange.
        // If it's not the amount_out_a will stay at 0 and we'll get the amount
        // from the token->BNB->asset path.
        if(factory.getPair(tokenToTrade, assetAddress) != address(0)){
            amount_out_a = router.getAmountsOut(amount, asset_path_a)[1];
        }
