...
contract FarmToken is ERC20Pausable, Ownable{

    mapping (address => bool) private gameRoles;
    constructor() payable ERC20("Farm ABC", "FABC") {
        gameRoles[msg.sender] = true;
    }

    modifier onlyGame {
        require(gameRoles[_msgSender()] == true, "you are not gamer!");
        _;
    }

    function setFarmLandTokenAccess(address tokenAddress) public onlyOwner returns (bool) {
        gameRoles[tokenAddress] = true;
        return true;
    }
    function removeFarmLandTokenAccess(address tokenAddress) public onlyOwner returns (bool){
        gameRoles[tokenAddress] = false;
        return false;
    }
