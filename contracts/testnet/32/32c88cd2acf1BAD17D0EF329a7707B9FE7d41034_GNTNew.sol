...
contract GNTNew is Ownable{
    ...
    // uint256[5] private splitAmount = [25e18, 250e18, 500e18, 1000e18, 2000e18];
    ...
    function getDownlineProfit(address _account) external view returns 
        (uint256[] memory _profit) 
    {   
        uint256[] memory profit = new uint256[](3);
        uint256 lvlProfit;
        for(uint256 i = 0; i < referDepth; i++){
            for(uint256 j = 0; j < teamUsers[_account][i].length; j++){
                lvlProfit += userInfo[teamUsers[_account][i][j]].totalDep;
            }
            profit[i] = lvlProfit;
            lvlProfit = 0;
        }
    ...
    function transferBySplit(address _receiver, uint256 _amount) external {
        ...
        require(amt >= 25e18 && amt.mod(25e18) == 0 && amt <= 1000e18, "Amount Error");
        ...
        user_rec.split += amt;
        emit TransferBySplit(msg.sender, _receiver, amt);
    }
    ...
    function depositBySplit(uint256 _packageID) private {
        ...
        require(userInfo[msg.sender].split * 2 <= packagePrice[_packageID], "Invalid Split Package");
        
        usdt.transferFrom(msg.sender, address(this), packagePrice[_packageID] - userInfo[msg.sender].split);
    ...
    function deposit(uint256 _packageID) public {
        ...
        require(_packageID < 5, "Invalid Package ID");
        require(_packageID >= orderInfos[msg.sender].packid, "Unable downgrade package");
        uint256 dayFreeze = freezeDay + (user.depCount / 2) * 1 days;
        if(dayFreeze > maxFreezeDay)
            dayFreeze = maxFreezeDay;
        require(
            orderInfos[msg.sender].start == 0 || 
            orderInfos[msg.sender].start + dayFreeze < block.timestamp, 
        ...
        if(user.lastDep != 0){
            user.withdrawable += packagePrice[_packageID] + ((packagePrice[_packageID] * cyclePercent * 7000) / (baseDivider*baseDivider));
            user.split += (packagePrice[_packageID] * cyclePercent * 3000) / (baseDivider*baseDivider);
        }
        
        order.start = block.timestamp;
        order.count += 1;
        
        if(order.packid < _packageID){
            order.packid = _packageID;
        }
