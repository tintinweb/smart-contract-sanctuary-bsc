 ...
 */
contract TokenModel is ERC20, Ownable {
    ...
    /** Antibots & Antiwhale **/            
    uint256 public maxHold = 1000;  // 1% of total supply
    uint256 public maxBuy = 500; // 0.5% of total supply
    uint256 public maxSell = 500; // 0.5% of total supply
    uint256 public maxTx = 500; // 0.5% of total supply

    /** Cooldown */
    bool public _cooldown = true;
    uint256 public _cooldownTime = 60 seconds;
    mapping (address => uint256) private _lastTx;

    /** Wallets **/
    address public marketingWallet = 0x66A09c6a65F016BbD78602C35340f5468fE0D873;    
    address public stakingWallet = 0x8Fa406F4202Cc8c7611861Ad609F530d3E1EdF36; 
    address public teamWallet = 0x2aE138571be873F77e6b5e1cFf4BF28E1a058732;
    address private devWallet = 0x3Db3A2F1AC72d1cd90a715a5c3abC65eCb01D23A;    
    ...
    Taxes public taxes = Taxes(40, 10, 20, 30, 20, 10);
    Taxes public sellTaxes = Taxes(40, 10, 20, 30, 20, 10);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (!exemptFee[sender] && !exemptFee[recipient]) {
            ...
            // Cooldown 
            if (_cooldown && !exemptFee[tx.origin]) {
            require(_lastTx[tx.origin] < block.timestamp, "Please wait for cooldown to finish.");
            _lastTx[tx.origin] = block.timestamp + _cooldownTime;
            }
        ...
        //Sell                   
            ...
            feeswap = sellTaxes.marketing + sellTaxes.team + sellTaxes.staking + sellTaxes.liquidity + sellTaxes.dev;
            feesum = feeswap;
            currentTaxes = sellTaxes;
        //Buy
        } else if (!useLaunchFee) {               
            feeswap = taxes.marketing + taxes.team + taxes.staking + taxes.liquidity + taxes.dev;
            feesum = feeswap;
            currentTaxes = taxes;
    ...
    function handle_fees(uint256 feeswap, Taxes memory swapTaxes) private mutexLock {
        ...
        if (contractBalance >= tokenLiquidityThreshold) {
            ...
            uint256 teamAmount = (unitBalance * 2 * swapTaxes.team) / swapTaxes.denominator;
            if (teamAmount > 0) {
                payable(teamWallet).sendValue(teamAmount);
            }

            uint256 devAmount = (unitBalance * 2 * swapTaxes.dev) / swapTaxes.denominator;
            if (devAmount > 0) {
                payable(devWallet).sendValue(devAmount);
            }

            uint256 stakingAmount = (unitBalance * 2 * swapTaxes.staking) / swapTaxes.denominator;
            if (stakingAmount > 0) {
                payable(stakingWallet).sendValue(stakingAmount);
            }
    ...
    function setBuyTaxes(
        ...
        require((_marketing + _team + _staking + _liquidity + _dev) <= 25 * taxes.denominator, "Must keep fees at 25% or less");
    ...
    function setSellTaxes(
        ...
        require((_marketing + _team + _staking + _liquidity + _dev) <= 25 * sellTaxes.denominator, "Must keep fees at 25% or less");
    ...
    function updateStakingWallet(address _wallet) external onlyOwner {
        stakingWallet = _wallet;
    }
