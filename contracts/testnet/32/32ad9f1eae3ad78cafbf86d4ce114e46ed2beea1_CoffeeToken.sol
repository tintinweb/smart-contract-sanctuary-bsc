   ...
   function zeroWalletBalance() public view returns (uint256) {
    ...
    function burn(uint256 tAmount) public returns (bool) {
        ...
        if (_isExcluded[_msgSender()]) {
            accountBalance = _tOwned[_msgSender()];
        }
        else{
            accountBalance = tokenFromReflection(_rOwned[_msgSender()]);
        }
        require(accountBalance >= tAmount, "BEP20: burn amount exceeds balance");


        uint256 currentRate =  _getRate();
        uint256 rAmount = tAmount.div(currentRate);
    ...
    function deliver(uint256 tAmount) public {
        require(_msgSender() != address(0), "BEP20: deliver from the zero address");
    ...
    function _reflectBurnFee(uint256 tBurnFee) private {
        uint256 currentRate =  _getRate();
