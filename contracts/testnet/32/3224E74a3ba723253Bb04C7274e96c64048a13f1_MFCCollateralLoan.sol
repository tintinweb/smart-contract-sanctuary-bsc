...
contract MFCCollateralLoan is BackendAgent, ExchangeCheck {
  ...
  uint256 public constant MIN_OFFER_PRICE_LTV_RATIO = 1020000000000000000; // 1.02;
  ...
  struct LoanTerm {
    ...
    uint256 minimumExtensionRepayment;
    DataTypes minimumExtensionRepaymentType;
    uint256 remainingPrincipalExtensionLimit;
    DataTypes remainingPrincipalExtensionLimitType;
  }
  ...
  event CreateLoan(
    ...
    uint256 startsAt,
    uint256 endsAt);
  event PayLoan(address borrower, uint256 loanId, uint256 busdAmount, uint256 remainingPrincipalBUSD, uint256 collateralReturned, uint256 paidAt);
  ...
  function setupInitialLoanTerm(
    ...
    _createNewLoanTerm(
      ...
      minimumExtensionRepayment,
      minimumExtensionRepaymentType,
      remainingPrincipalExtensionLimit,
      remainingPrincipalExtensionLimitType
    );
  }
  ...
  function setMaxOpenLoans(uint256 maxOpenLoans_) external onlyBackendAgents {
  ...
  function payLoan(uint256 loanId, uint256 busdAmount) external onlyActiveLoan(_msgSender(), loanId) {
    ...
    _payLoan(_msgSender(), loanId, busdAmount);
  }
  ...
  function closeLoan(address borrower, uint256 loanId) external onlyBackendAgents {
    ...
    _mfc.transfer(_mfcExchangeCapAddress, collateralMFC);
  ...
  function createOffer(uint256 loanId, uint256 quantity, uint256 price) external onlyValidMember(_msgSender()) onlyActiveLoan(_msgSender(), loanId) {
    _createOffer(loanId, quantity, price);
  }

  // This is for other members to trade on the offer the borrower created
  function tradeOffer(address borrower, uint256 loanId, uint256 amountBUSD) external onlyValidMember(_msgSender()) onlyActiveOffer(borrower, loanId) {
    ...
    uint256 mfcInterests = _accruedInterestMFC(borrower, loanId);
    offer.unfilledQuantity -= mfcInterests;
    uint256 maxInput = offer.unfilledQuantity * offer.price / MULTIPLIER;
    require(amountBUSD <= maxInput, "Not enough to sell");

    uint256 busdFee = amountBUSD * BUSD_FEE / MULTIPLIER;
    uint256 mfcFee = amountBUSD * MFC_FEE / offer.price;
    uint256 effectiveBusdValue = amountBUSD - busdFee;

    _mfc.transfer(_mfcExchangeCapAddress, mfcFee);

    _payLoan(borrower, loanId, effectiveBusdValue);
  }

  // This is for the borrower to sell their collateral to other users
  function tradeCollateral(uint256 loanId, uint256 offerId, address seller, uint256 amountMFC) external onlyValidMember(_msgSender()) onlyActiveLoan(_msgSender(), loanId) {
    ...
    uint256 currentBusdBalance = _busd.balanceOf(address(this));
    _mfcExchange.tradeOffer(offerId, seller, amountMFC);
    uint256 busdReceived = _busd.balanceOf(address(this)) - currentBusdBalance; // not sure if this will work since everything is on the same transaction/block

    _payLoan(_msgSender(), loanId, busdReceived);
  }

  // This is for the borrower to sell their collateral to the floor
  function tradeCollateral(uint256 loanId, uint256 amountMFC, uint256 minimumOut) external onlyValidMember(_msgSender()) onlyActiveLoan(_msgSender(), loanId) {
    ...
    uint256 currentBusdBalance = _busd.balanceOf(address(this));
    _mfcExchangeFloor.tradeOffer(amountMFC, minimumOut);
    uint256 busdReceived = _busd.balanceOf(address(this)) - currentBusdBalance; // not sure if this will work since everything is on the same transaction/block
  ...
  function getOffer(address borrower, uint256 loanId) external view returns (Offer memory) {
    Offer memory offer = _offers[borrower][loanId];
    uint256 mfcInterests = _accruedInterestMFC(borrower, loanId);
    offer.unfilledQuantity -= mfcInterests; // this offer is in memory so it does not alter the data in the actual offer
    return offer;
  }
  ...
  function _createNewLoanTerm(
    ...
    require(minimumExtensionRepaymentType == DataTypes.PERCENTAGE || minimumExtensionRepaymentType == DataTypes.BUSD, "Invalid type");
    require(remainingPrincipalExtensionLimitType == DataTypes.PERCENTAGE || remainingPrincipalExtensionLimitType == DataTypes.BUSD, "Invalid type");
  ...
  function _getMinimumExtensionRepayment(uint256 loanTermId, uint256 originalPrincipalBUSD) private view returns (uint256) {
    ...
    if (loanTerm.minimumExtensionRepaymentType == DataTypes.BUSD) {
      return loanTerm.minimumExtensionRepayment;
    } else if (loanTerm.minimumExtensionRepaymentType == DataTypes.PERCENTAGE) {
      return originalPrincipalBUSD * loanTerm.minimumExtensionRepayment / MULTIPLIER;
    } else {
      return 0;
    }
  }
  ...
  function _getRemainingPrincipalExtensionLimit(uint256 loanTermId, uint256 originalPrincipalBUSD) private view returns (uint256) {
    LoanTerm memory loanTerm = _loanTerms[loanTermId];
    if (loanTerm.remainingPrincipalExtensionLimitType == DataTypes.BUSD) {
      return loanTerm.remainingPrincipalExtensionLimit;
    } else if (loanTerm.remainingPrincipalExtensionLimitType == DataTypes.PERCENTAGE) {
      return originalPrincipalBUSD * loanTerm.remainingPrincipalExtensionLimit / MULTIPLIER;
    } else {
      return 0;
    }
  }
  ...
  function _payLoan(address borrower, uint256 loanId, uint256 busdAmount) private {
    Loan storage loan = _loans[borrower][loanId];
    uint256 mfcInterests = _accruedInterestMFC(borrower, loanId);
    loan.collateralMFC -= mfcInterests;
    ...
    _mfc.transfer(_mfcExchangeCapAddress, mfcInterests);
    ...
    emit PayLoan(borrower, loanId, busdAmount, loan.remainingPrincipalBUSD, collateralReturned, loan.lastPaidAt);
  }

  function _createOffer(uint256 loanId, uint256 quantity, uint256 price) private {
    require(!_offers[_msgSender()][loanId].isOpen, "Limit one offer per loan");

    // MIN_OFFER_PRICE_LTV_RATIO
    uint256 mfcPrice = _mfcExchangeFloor.getPrice();
    Loan memory loan = _loans[_msgSender()][loanId];
    uint256 minimumPrice = mfcPrice * MIN_OFFER_PRICE_LTV_RATIO / MULTIPLIER;
    if (loan.ltv > mfcPrice) {
      minimumPrice = loan.ltv * MIN_OFFER_PRICE_LTV_RATIO / MULTIPLIER;
    }
    require(price >= minimumPrice, "Minimum price not met");

    // OFFER_NET_COLLATERAL_RATIO
    uint256 mfcInterests = _accruedInterestMFC(_msgSender(), loanId);
    uint256 maximumQuantity = (loan.collateralMFC - mfcInterests) * OFFER_NET_COLLATERAL_RATIO / MULTIPLIER;
    require(quantity <= maximumQuantity, "Quantity exceeds limit");

    // Cannot open an offer if loan is to expire within 30 days
    require(loan.endsAt > block.timestamp + DAY_IN_SECONDS * 30, "Loan is about to expire");

    // Create offer
    uint256 expiresAt = loan.endsAt;
    _offers[_msgSender()][loanId] = Offer(quantity, price, expiresAt, true);

    emit CreateOffer(_msgSender(), loanId, quantity, price, expiresAt, block.timestamp);
  }

  function _tradeCollateralPrerequisite(uint256 loanId, uint256 amountMFC) private {
    require(amountMFC > 0, "Invalid quantity");
    Offer memory offer = _offers[_msgSender()][loanId];
    Loan storage loan = _loans[_msgSender()][loanId];
    uint256 mfcInterests = _accruedInterestMFC(_msgSender(), loanId);
    loan.collateralMFC -= mfcInterests;
    uint256 availableMFC = loan.collateralMFC - offer.unfilledQuantity;
    require(amountMFC <= availableMFC, "Not enough to sell");
  }

  function _closeOffer(uint256 loanId) internal {
    delete _offers[_msgSender()][loanId];
    emit CloseOffer(loanId, block.timestamp);
  }

  function test_createNewLoanTerm(
    uint256 dailyInterestRate,
    uint256 loanDurationInDays,
    uint256 minimumLoanBUSD,
    uint256 originationFeePercentage,
  ...
  function test_payLoan(
    ...
    emit PayLoan(
      ...
      loanId,
      busdAmount,
      remainingPrincipalBUSD,
      collateralReturned,
      paidAt
    );
  }

  function test_ExtendLoan(address borrower, uint256 loanId, uint256 endsAt) external {
    emit ExtendLoan(borrower,loanId, endsAt);
  }

  function test_closeLoan(address borrower, uint256 loanId, uint256 collateralMFCTransferred) external {
    emit CloseLoan(borrower, loanId, collateralMFCTransferred);
  }
  ...
  function test_closeOffer(uint256 loanId, uint256 timestamp) external {
    emit CloseOffer(loanId, timestamp);
  }
}
