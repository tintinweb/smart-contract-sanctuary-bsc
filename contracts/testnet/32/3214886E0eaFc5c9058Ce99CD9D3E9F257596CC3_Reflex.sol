...
contract Reflex is IERC20, Pausable, Ownable {
    ...
    uint256 public autoBuybackThreshold = 1 * 10**_decimals;
    ...
    function setStakeAddress(address _staking) external onlyOwner whenNotPaused {
      require(_staking != address(0), "ReflexV2: staking address cant be zero address");
      stake = IReflexStaking(_staking);
    }

    function setRewardToken(address _reward) external onlyOwner whenNotPaused {
      require(_reward != address(0), "ReflexV2: reward address cant be zero address");
      rewardToken = _reward;
    }
    ...
    function blacklistAddress(address account) external onlyOwner whenNotPaused {
      require(account != address(0), "ReflexV2: address cant be zero address");
      require(isBlacklisted[account] != true, "ReflexV2: Account is already blacklisted");
      isBlacklisted[account] = true;
    }

    function removeFromBlacklist(address account) external onlyOwner whenNotPaused {
      require(account != address(0), "ReflexV2: address cant be zero address");
