...
// File: contracts/IFarmBooster.sol


interface IFarmBooster {
    function onEvocPoolUpdate(
        address _user,
...
// File: contracts/FarmBoosterProxy.sol
...
contract FarmBoosterProxy is ReentrancyGuard {
     ...
     * @param _evocToken: the address of the evoc token
     */
    function initialize(
        address _admin,
    ...
    function harvestEvoc() internal {
        uint256 evocBalance = evocToken.balanceOf(address(this));
        if (evocBalance > 0) {
            evocToken.safeTransfer(msg.sender, evocBalance);
        }
    }
}
...
contract FarmBoosterProxyFactory {
    ...
    address public immutable evocToken;
    ...
    function createFarmBoosterProxy() external {
        ...
        FarmBoosterProxy(farmBoosterProxyAddress).initialize(msg.sender, Farm_Booster, masterchefV2, evocToken);
