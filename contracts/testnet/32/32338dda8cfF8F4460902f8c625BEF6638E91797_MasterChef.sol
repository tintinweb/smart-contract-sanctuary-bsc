...
contract MasterChef is Ownable {
    ...
    function pendingMILK(
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accMILKPerShare = accMILKPerShare.add(
                MILKReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        if (_amount > 0) {
            ...
            if (pool.withdrawFeeBP > 0) {
                uint256 withdrawFee = pending.mul(pool.withdrawFeeBP).div(10000);
                safeMILKTransfer(feeAddress, withdrawFee);
                uint256 rewithdrawal = pending.sub(withdrawFee);
                safeMILKTransfer(msg.sender, rewithdrawal);
            }
    ...
    function changeMilkShakeSwapAddress(
        address payable _uniswaprouter1
    ) external onlyOwner {
        uniswaprouter1 = IUniswapV2Router01(_uniswaprouter1);
    }
    ...
    function distributefunction(
        ...
        if (
            _affAddr1 != address(0) &&
            (userInfo[farmPid][_affAddr1].amount > 0 ||
                userInfo[poolPid][_affAddr1].amount > _min)
        ) {
            _affRewards = (_tokenamount.mul(40)).div(100);

            affinfo[_affAddr1].affRewards = _affRewards.add(
                affinfo[_affAddr1].affRewards
            );
        ...
        if (
            _affAddr2 != address(0) &&
            (userInfo[farmPid][_affAddr2].amount > 0 ||
                userInfo[poolPid][_affAddr2].amount > _min)
        ) {
            _affRewards = (_tokenamount.mul(20)).div(100);

            affinfo[_affAddr2].affRewards = _affRewards.add(
                affinfo[_affAddr2].affRewards
            );
        ...
        if (
            _affAddr3 != address(0) &&
            (userInfo[farmPid][_affAddr3].amount > 0 ||
                userInfo[poolPid][_affAddr3].amount > _min)
        ) {
            _affRewards = (_tokenamount.mul(15)).div(100);

            affinfo[_affAddr3].affRewards = _affRewards.add(
                affinfo[_affAddr3].affRewards
            );
        ...
        if (
            _affAddr4 != address(0) &&
            (userInfo[farmPid][_affAddr4].amount > 0 ||
                userInfo[poolPid][_affAddr4].amount > _min)
        ) {
            _affRewards = (_tokenamount.mul(10)).div(100);

            affinfo[_affAddr4].affRewards = _affRewards.add(
                affinfo[_affAddr4].affRewards
            );
        ...
        if (
            _affAddr5 != address(0) &&
            (userInfo[farmPid][_affAddr5].amount > 0 ||
                userInfo[poolPid][_affAddr5].amount > _min)
        ) {
            _affRewards = (_tokenamount.mul(5)).div(100);

            affinfo[_affAddr5].affRewards = _affRewards.add(
                affinfo[_affAddr5].affRewards
            );
        ...
        if (
            _affAddr6 != address(0) &&
            (userInfo[farmPid][_affAddr6].amount > 0 ||
                userInfo[poolPid][_affAddr6].amount > _min)
        ) {
            _affRewards = (_tokenamount.mul(4)).div(100);

            affinfo[_affAddr6].affRewards = _affRewards.add(
                affinfo[_affAddr6].affRewards
            );
        ...
        if (
            _affAddr7 != address(0) &&
            (userInfo[farmPid][_affAddr7].amount > 0 ||
                userInfo[poolPid][_affAddr7].amount > _min)
        ) {
            _affRewards = (_tokenamount.mul(3)).div(100);

            affinfo[_affAddr7].affRewards = _affRewards.add(
                affinfo[_affAddr7].affRewards
            );
        ...
        if (
            _affAddr8 != address(0) &&
            (userInfo[farmPid][_affAddr8].amount > 0 ||
                userInfo[poolPid][_affAddr8].amount > _min)
        ) {
            _affRewards = (_tokenamount.mul(3)).div(100);

            affinfo[_affAddr8].affRewards = _affRewards.add(
                affinfo[_affAddr8].affRewards
            );
