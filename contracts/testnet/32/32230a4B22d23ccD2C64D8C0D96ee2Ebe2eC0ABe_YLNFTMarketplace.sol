...
contract YLNFTMarketplace is ERC1155Holder, ReentrancyGuard, Ownable {
    ...
    function minterListedNFT(
        ...
        if (_itemId == 0) {
            ...
            if (_isERC721) idToMarketItem[_itemId].amount = _amount;
            else idToMarketItem[_itemId].amount += _amount;
        }
    ...
    function directTransferToBuyer(
        ...
        if (_isERC721) {
            require(
                ylnft721.ownerOf(_tokenId) == _from,
            ...
            require(
                ylnft1155.balanceOf(_from, _tokenId) >= _amount,
                "You haven't this NFT."
            );
            bool isTransferred = ylt20.transfer(
                address(this),
                _price * _amount + marketfee
            );
            require(isTransferred, "Insufficient Fund.");
            require(
                ylnft1155.isApprovedForAll(_from, address(this)) == true,
                "NFT must be approved to market"
            );
    ...
    function adminPauseToggle(
        ...
        require(
            ylnft721.ownerOf(_tokenId) == address(this) ||
                ylnft1155.balanceOf(
                    address(this),
    ...
    function depositApproval(
        ...
        require(
            ylnft721.ownerOf(_tokenId) == _user ||
                ylnft1155.balanceOf(_user, _tokenId) >= _amount,
    ...
    function withdrawApproval(
        ...
        require(
            ylnft721.ownerOf(idToMarketItem[_itemId].tokenId) ==
                address(this) ||
                ylnft1155.balanceOf(
                    address(this),
                    idToMarketItem[_itemId].tokenId
    ...
    function transferToVault(
        ...
        require(
            ylnft721.ownerOf(_tokenId) == address(this) ||
                ylnft1155.balanceOf(address(this), _tokenId) >= _amount,
        ...
        emit TransferedNFTfromMarkettoVault(
            _tokenId,
            _vault,
            _amount,
            block.timestamp
    ...
    function transferFromVaultToMarketplace(
        ...
        require(
            ylnft721.ownerOf(_tokenId) == _vault ||
                ylnft1155.balanceOf(_vault, _tokenId) >= _amount,
            "The team haven't this token."
        );
        IVault vault = IVault(_vault);
        vault.transferToMarketplace(
            address(this),
            msg.sender,
            _tokenId,
            _amount
        ); // Implement this function in the Vault Contract.
        ...
        emit TransferedNFTfromVaulttoMarket(
            _tokenId,
            _vault,
            _amount,
            block.timestamp
    ...
    function adminTransfer(
        ...
        require(isTransferred, "Insufficient Fund.");
        uint256 _tokenId = idToMarketItem[_itemId].tokenId;
        if (_isERC721) ylnft721.transferFrom(address(this), _to, _tokenId);
        else
            ylnft1155.safeTransferFrom(
                address(this),
                _to,
    ...
    function MarketItemSale(
        ...
        bool isTransferred = ylt20.transfer(
            address(this),
            idToMarketItem[itemId].price + marketfee
        );
        require(isTransferred, "Insufficient Fund.");
        require(
            idToMarketItem[itemId].seller != msg.sender,
        ...
        if (_isERC721) {
            ...
            if (idToMarketItem[itemId].amount == _amount) {
                idToMarketItem[itemId].state = State.Release;
                idToMarketItem[itemId].owner = msg.sender;
            } else idToMarketItem[itemId].amount -= _amount;
        }
    ...
    function editMarketItem(uint256 _itemId, uint256 _price) public {
        require(
            idToMarketItem[_itemId].state == State.Active,
            "This item is not active on marketplace"
        );
        require(
            idToMarketItem[_itemId].owner == msg.sender,
            "You can't edit this market item"
        );
        idToMarketItem[_itemId].price = _price;
        emit MarketItemEditted(
            msg.sender,
            idToMarketItem[_itemId].tokenId,
            _price,
            block.timestamp
