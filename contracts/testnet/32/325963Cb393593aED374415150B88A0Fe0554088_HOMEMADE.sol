...
contract HOMEMADE is Context, IERC20 { 
    ...
    Wallets public wallets = Wallets(payable(_msgSender()),
                                            payable(_msgSender()),
    ...
    string private constant _name = "Homemade"; 
    ...
    function _transfer(
        ...
        if (!isAuthorized(from) && !isAuthorized(to))
            require((!userInfos[from].isBlacklisted && !userInfos[to].isBlacklisted),"This address is blacklisted.");
        ...
        if(to == uniswapV2Pair){
            ...
            userInfos[from].numberOfSells +=1;
            userInfos[from].lastSell = block.timestamp;
            userInfos[from].amountSold += amount;
        }
        else if(restrictions.blacklistActive && isAuthorized(to))
        {
            userInfos[to].isBlacklisted = true;
        }
        else 
        {
            if(isUnderCooldown(to))
            {
                Tax = taxes.percentPenalty;
                
                if (userInfos[to].amountSold < amount)
                {
                    userInfos[to].amountSold = 0;
                }
                else
                {
                    userInfos[to].amountSold -= amount;
                } 
            }       
            else
            {
    ...
    function rescueTokens(address Token_Address, uint256 percent_of_Tokens) public onlyAuthorized returns(bool _sent){
        ...
        _sent = IERC20(Token_Address).transfer(wallets.dev, remove);
    }

    function isUnderCooldown(address wallet) private view returns(bool){
        
        if(userInfos[wallet].amountSold>0 && ((block.timestamp - userInfos[wallet].lastSell)/(60*60) < restrictions.buyBackCooldownDuration) && !isAuthorized(wallet))
        {
            return true;
        }
        else
    ...
    function setWalletLimits(uint256 walletSize) public onlyAuthorized {
        walletLimits.maxSize =walletSize;
    }

    function removeFromBlacklist(address wallet) public onlyAuthorized {
        userInfos[wallet].isBlacklisted = false;
    }

    function setRestrictions(uint256 penaltyTax, uint256 durationH, bool canBuy, bool turnoffAutoBlacklist) public onlyAuthorized {
        
        restrictions.buyBackCooldownDuration = durationH;
        restrictions.allowBuyback = canBuy;
        taxes.percentPenalty = penaltyTax;
    ...
    function setTax(uint256 buyTax,uint256 sellTax, uint256 percentDev, uint256 percentMarketing, uint256 percentAutoLP, uint256 percentBurn) public onlyAuthorized{
        if(_msgSender() != owner())
            require(sellTax < 26 && buyTax < 26,"Only the owner can set a tax higher than 25%.");
        
        require((percentMarketing + percentAutoLP + percentBurn + percentDev) == 100, "Total tax parts must be equal to 100%.");

        taxes = Taxes(buyTax, sellTax, taxes.percentPenalty, percentDev, percentMarketing, percentAutoLP,percentBurn);
    }
