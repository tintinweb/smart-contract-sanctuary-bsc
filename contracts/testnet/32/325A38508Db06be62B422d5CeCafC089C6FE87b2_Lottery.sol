...
contract Lottery is ReentrancyGuard {
    ...
    function getWinningsForAddress(address addr) public view returns (uint256) {
       ...
       function setToken(address _tokenAddress, uint256 _maxTickets, uint256 _tokenDecimals) public isOperator  {
       tokenAddress = _tokenAddress;
       maxTickets = _maxTickets;
       tokenDecimals = _tokenDecimals;

}
       function setTicketPrice(uint256 _ticketPrice, uint256 _ticketCommission) public isOperator  {
       ticketPrice = _ticketPrice * (10** tokenDecimals);
       ticketCommission = _ticketCommission * (10** tokenDecimals);
...
function BuyTickets(uint256 amount) public payable {
        require(
            amount * (10** tokenDecimals) % ticketPrice == 0,
            string.concat(
        ...
        // uint256 amount = amount * (10** tokenDecimals);
        uint256 numOfTicketsToBuy = amount * (10** tokenDecimals) / ticketPrice;
    ...
    function WithdrawWinnings() public isWinner nonReentrant {
        ...
        tokenContract.transfer( winner, contractBalance - operatorTotalCommission );
    ...
    function RefundAll() public isOperator {
        ...
        for (uint256 i = 0; i < tickets.length; i++) {
            ...
            tickets[i] = address(0);
             IERC20 tokenContract = IERC20(tokenAddress);
             tokenContract.transfer(to, ticketPrice);
    ...
    function WithdrawCommission() public isOperator nonReentrant {
        ...
        IERC20 tokenContract = IERC20(tokenAddress);
        tokenContract.transfer( operator, operatorTotalCommission );
