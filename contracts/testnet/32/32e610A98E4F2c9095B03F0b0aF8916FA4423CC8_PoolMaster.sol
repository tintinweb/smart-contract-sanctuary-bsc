...
contract PoolIDO1 is Ownable, ReentrancyGuard {
    ...
    uint256 public requireStakingToken; // Require staking token
    uint256 public feeAmount;

    // Token
    ERC20 public stakeToken; // BAMI
...
contract PoolMaster {
    ...
    event SlaveCreated(address stakeToken, address claimToken, address paymentToken, address pool);

    function createPoolIDO1(
        address _devWallet,
        address _idoOwner,
        address _stakeToken,
        address _claimToken,
        ...
        bytes memory bytecode = type(PoolIDO1).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(block.timestamp));
        assembly {
            pool := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        PoolIDO1(pool).initialize(_devWallet, _idoOwner, _stakeToken, _claimToken, _paymentToken);
        PoolIDO1(pool).setup(_payAmountPerSlot, _tokenPerSlot, _totalOfSlot, _requireStakingToken, _feeAmount);
        PoolIDO1(pool).transferOwnership(_owner);
        pools.push(Pool(1, pool));

        emit SlaveCreated(_stakeToken, _claimToken, _paymentToken, pool);
    }
}