    ...
    ReentrancyGuard
{
    ...
    /// @notice mapping of token address to media eye chainlink price feed
    mapping(address => address) internal priceFeeds;
    /// @notice mapping of listingId to price feed
    mapping(uint256 => address) internal salePriceFeed;
    ...
    /**
     * @param _listingConstructor.owner address of the owner
     * @param _listingConstructor.admins addresses of initial admins
     * @param _listingConstructor.treasuryWallet address of treasury wallet
     * @param _listingConstructor.basisPointFee initial basis point fee
     * @param _listingConstructor.feeContract contract of MediaEyeFee
     * @param _listingConstructor.mediaEyeMarketplaceInfo address of info
     * @param _listingConstructor.priceFeeds address list of price feeds
     */
    constructor( MediaEyeOrders.ListingConstructor memory _listingConstructor ) {
        require(_listingConstructor.treasuryWallet != address(0));
        require(_listingConstructor.basisPointFee <= 500, "Max fee");

        _setupRole(DEFAULT_ADMIN_ROLE, _listingConstructor.owner);
        for (uint256 i = 0; i < _listingConstructor.admins.length; i++) {
            _setupRole(ROLE_ADMIN, _listingConstructor.admins[i]);
        }

        treasuryWallet = _listingConstructor.treasuryWallet;
        feeContract = _listingConstructor.feeContract;

        basisPointFee = _listingConstructor.basisPointFee;
        mediaEyeMarketplaceInfo = IMarketplaceInfo(_listingConstructor.mediaEyeMarketplaceInfo);

        for (uint256 i = 0; i < _listingConstructor.priceFeeds.length; i++) {
            require(_listingConstructor.priceFeeds[i] != address(0));
            priceFeeds[
                IChainlinkPriceFeeds(
                    _listingConstructor.priceFeeds[i]
                ).getTokenAddress()
            ] = _listingConstructor.priceFeeds[i];
        }
    ...
    /**
     * @notice Update constants/contracts. enter 0 address if you dont want to change a param
     * @param _newTreasuryWallet: new treasury wallet
     * @param _newFeeContract: new subscription fee contract
     * @param _basisPointFee: new basis point fee for treasury
     * @param _subscriptionCheck: enable/disable subscription check
     * @param _newInfoContract: new Marketplace Info contract
     */
     ...
     *
     */
    function addMediaEyePriceFeed(address _priceFeed) external {
        require(
            hasRole(ROLE_ADMIN, msg.sender) ||
                hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "admin"
        ...
        priceFeeds[
            IChainlinkPriceFeeds(_priceFeed).getTokenAddress()
        ] = _priceFeed;
    }
        ...
        returns (uint256)
    {
        ...
        if (_listingInput.chainlinkPayment.isValid) {
            ...
            if (
                ...
                require(
                    salePaymentAmounts[listingId][_listingInput.chainlinkPayment.baseAddress] > 0,
                    "chainlink payment"
                );
                require(
                    priceFeeds[_listingInput.chainlinkPayment.baseAddress] != address(0),
                    "price feed not supported"
                );
                salePriceFeed[listingId] = priceFeeds[_listingInput.chainlinkPayment.baseAddress];
            } else {
                require(
                    _listingInput.chainlinkPayment.baseAddress ==
                        address(0),
                    "impossible chainlink payment"
                );
                require(
                    salePaymentAmounts[listingId][address(0)] > 0,
                    "chainlink payment"
                );
                require(
                    priceFeeds[_listingInput.chainlinkPayment.quoteAddress] != address(0),
                    "price feed not supported"
                );
                salePriceFeed[listingId] = priceFeeds[_listingInput.chainlinkPayment.quoteAddress];
            }
    ...
    function updateListing(
        ...
        if (_chainlinkPayment.isValid) {
            ...
            // check if the opposite address is a payment method
            if (
                _chainlinkPayment.quoteAddress ==
                address(0)
            ) {
                require(
                    salePaymentAmounts[_listingId][_chainlinkPayment.baseAddress] > 0,
                    "chainlink payment"
                );
                require(
                    priceFeeds[_chainlinkPayment.baseAddress] != address(0),
                    "price feed not supported"
                );
                salePriceFeed[_listingId] = priceFeeds[_chainlinkPayment.baseAddress];
            } else {
                require(
                    _chainlinkPayment.baseAddress ==
                        address(0),
                    "impossible chainlink payment"
                );
                require(
                    salePaymentAmounts[_listingId][address(0)] > 0,
                    "chainlink payment"
                );
                require(
                    priceFeeds[_chainlinkPayment.quoteAddress] != address(0),
                    "price feed not supported"
                );
                salePriceFeed[_listingId] = priceFeeds[_chainlinkPayment.quoteAddress];
            }
    ...
    function buyTokens(
        ...
        MediaEyeOrders.PaymentChainlink memory chainlink = saleChainlinkAddresses[_listingId];
        ...
        if (
            ...
            // calculate price
            if (_paymentMethod.paymentMethod == address(0)) {
                price = salePaymentAmounts[_listingId][chainlink.baseAddress];
                require(price > 0, "chainlink payment");
                convertedPrice = IChainlinkPriceFeeds(
                    salePriceFeed[_listingId]
                ).convertPrice(
                    price * _amount,
                    true
                );
                ...
                price = salePaymentAmounts[_listingId][address(0)];
                require(price > 0, "chainlink payment");
                convertedPrice = IChainlinkPriceFeeds(
                    salePriceFeed[_listingId]
                ).convertPrice(
