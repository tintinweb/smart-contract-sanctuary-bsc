...
contract Whitelist is IWhitelist {
  ...
  mapping(address => address) private _ref; // ref addr of user
  mapping(address => uint256) private _ga; // game amount of user
  ...
  function whitelist(address ref) external override returns (bool) {
    ...
    // event
    emit NewWhiteListed(msg.sender, ref);
    return true;
  }

  // NOTE: add RBCA
  function remove(address account) external override returns (bool) {
    _is[account] = false;
    _count--;
  ...
  function addGameAmount(address account, uint256 amount) external override returns (bool) {
    _ga[account] += amount;
  ...
  // NOTE: add RBCA
  function subGameAmount(address account, uint256 amount) external override returns (bool) {
    _ga[account] -= amount;
  ...
  function setUsdValue(uint256 amount) public returns (bool) {
    usdValue = amount;
    return true;
  }
