...
interface ILaunchpad  {
    function deployContract(address newOwner, address tokenContract, uint256[30] memory numberArray) external returns (address newContract);
}
...
contract LaunchpadFactory is WalletDetector, Ownable {
    ...
    function getBDeployed(address _ta, uint256 index) public view returns (address tc){
        return deployedAddresses[_ta].bDeployed[index];
    }
    ...
    function storeNumberArray(address tokenContract, uint256[30] memory numberArray) external
    {
    ...
    function deployContract (address tokenContract, uint256[30] memory numberArray) public payable onlyWallet {
        if( block.chainid == 97) {
...
interface ITrustLaunchLaunchpad  {
    ...
    function finished() external view returns (bool active);

    function getTotalRequiredTokens() external view returns (uint256 requiredTokens);
    function storeNumberArray(uint256[30] memory numberArray) external;
