...
contract Fusion is OwnableUpgradeable, ERC721Holder {
    ...
    event SeedmonsFusing(uint256 tokenId, address indexed owner);
    ...
    function multiply(uint256 num1) external pure returns (uint256 result) {
        result = num1.mul(40).div(100);
    }

    function testMintSeedmon(uint256 mainTokenId, uint256 materialTokenId, address to) external {  
        (,, SeedmonAttribute.Stat memory mainStat) = seedmon.tokenInfo(mainTokenId);
        (,, SeedmonAttribute.Stat memory materialStat) = seedmon.tokenInfo(materialTokenId);
            
            SeedmonAttribute.Stat memory pendingStat;
            // pendingStat.vitality = 
            

            // seedmon.mint(to, name, uniqueProfile, stat);
            // emit SeedmonsFusing(mainTokenId, to);
    }
