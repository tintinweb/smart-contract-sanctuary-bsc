...
contract OvalRegularPledge is Initializable, Ownable, Pausable, ReentrancyGuard {
    ...
    function pushPool(bool isEnable, uint256 startTime, uint256 duration) external onlyOwner {
        require(duration > 0, "duration can't be zero");
        pools.push (
            PledgePool(autoIncrementId(), isEnable, startTime, duration)
        );
    }
    ...
    function changeDuration(uint256 poolIndex, uint256 _duration) external onlyOwner {
        ...
        require(!pools[poolIndex].isEnable, "Activity has started");
        pools[poolIndex].duration = _duration;
    }

    function enablePool(uint256 poolIndex) external onlyOwner {
        require(poolIndex < pools.length, "Index out of boundry");
        pools[poolIndex].isEnable = true;
    }
    ...
    function pledge(uint256 amount, uint256 poolIndex) external whenNotPaused nonReentrant {
        require(poolIndex < pools.length, "Index out of boundry");
        require(pools[poolIndex].isEnable && pools[poolIndex].startTime <= block.timestamp, "Activity has not started");
