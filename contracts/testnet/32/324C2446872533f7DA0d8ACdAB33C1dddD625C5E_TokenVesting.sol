...
import './interfaces/ITokenBlacklist.sol';
...
contract TokenVesting is Ownable, ReentrancyGuard {
    ...
    constructor (IAdmin _gramAdmins) {
        GRAM_ADMINS = _gramAdmins;
        FEES.tokenFee = 35;
   ...
   */
    ...
    // Callable by GRAM_ADMINS or whitelisted contracts (such as presale contracts)
    function editZeroFeeWhitelist (address _token, bool _add) external {
        require(GRAM_ADMINS.userIsAdmin(msg.sender) || TOKEN_WHITELISTERS.contains(msg.sender), 'ADMIN');
        if (_add) {
            ZERO_FEE_WHITELIST.add(_token);
        } else {
