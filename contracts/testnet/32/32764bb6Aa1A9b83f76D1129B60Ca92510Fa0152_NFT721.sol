...
contract NFT721 is Ownable, SignerRole, IERC721, ERC721Base, ExchangeDomain {
    ...
    function mint(uint256 tokenId, uint8 v, bytes32 r, bytes32 s, Fee[] memory _fees, string memory tokenURI, uint256 price, AssetType assetType, address addressToken) public {
        require(isSigner(ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(this, tokenId, price, assetType, addressToken)))), v, r, s)), "owner should sign tokenId");
        if(assetType == AssetType.ETH &&  addressToken == address(0x0)) {
            address payable toPayable = payable(owner());
