...
contract FuckiesTrade is Pausable, Ownable {
    ...
    uint constant TRADE_AMOUNT = 50000000000000000000;
    uint constant PAYOUT_AMOUNT = 20000000000000000000;
    ...
    function startFreeTrade(uint _pairIndex, uint _marketPrice, bool _long) public isNotPaused {
        ...
        tradeConditions.positionSizeDai = TRADE_AMOUNT;
    ...
    function closeFreeTradeCallback(uint _toPay, uint _pairIndex, uint _tradeIndex) external onlyCallbacks {
        ...
        bool win = (_toPay > TRADE_AMOUNT) ? true : false;
        if (win) {
            BUSDToClaim[trader] += PAYOUT_AMOUNT;
            winningTrades += 1;
            winningAmount += _toPay - TRADE_AMOUNT;
        }
        else {
            losingTrades += 1;
            losingAmount += TRADE_AMOUNT - _toPay;
        }
