...
interface IERC20 {//繼承token 的 balanceOf & transfer的功能
    ...
    function transfer(address _to, uint256 _value,string calldata note) external returns (bool success);

}

contract contest{//目前設定公司抽成100代幣
    address public creater;//比賽創辦者
    ...
    string private registerData;
    //uint256 private status=0;// 1:創建完成 2:繳費完成
    IERC20 token = IERC20(address(0xc758CC4A842aA7D8652e9E255693DA4DA7Ec22F5));//代幣地址
    ...
    function endContest(address[] memory ranking)public returns(bool success)
    {
        ...
        token.transfer(companyAddress,100,"Auto Message");
        
        uint256 totalPrize=token.balanceOf(address(this));
        for(uint i=0 ;i<proportion.length;i++)
        {
            token.transfer(ranking[i],totalPrize/100*proportion[i],"Auto Message");
        }
        //status=4;
    ...
    function setregisterData(string memory _registerData)public returns (bool success)
    {
        require(parentContract==msg.sender,"only owner can execute this function");
    ...
    function getRegisterData()public view returns(string memory _registerData)
    {
        return registerData;
    }
}
...
contract mainContest{ 
    ...
    */
    ...
    function setregisterData(uint256 ID,string memory _registerData)public returns (bool success)
    {
        require(owner==msg.sender,"only owner can execute this function");
        contestById[ID].setregisterData(_registerData);
        return true;
    }

    function getRegisterData(uint256 ID)public view returns(string memory registerData)
    {
        return contestById[ID].getRegisterData();
    }
