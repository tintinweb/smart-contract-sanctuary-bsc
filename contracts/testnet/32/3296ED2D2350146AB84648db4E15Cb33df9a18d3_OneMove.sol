...
contract OneMove is Ownable, ERC1155Holder, ERC721Holder, ReentrancyGuard {
    ...
    function depositRT1(uint256 uid, uint256 amount) external {
        ...
        emit DepositToken(uid, msg.sender, amount, "RT1");
    }

    function withdrawRT1(address account, uint256 amount) external onlyOwner {
        tokenRT1.transfer(account, amount);
        emit WithdrawToken(account, amount, "RT1");
    }
    ...
    function depositArtifact(uint256 uid, uint256 id, uint256 amount) external {
        require(nftArtifact.balanceOf(msg.sender, id) >= amount);
        nftArtifact.safeTransferFrom(msg.sender, address(this), id, amount, "");

        emit DepositArtifact(uid, msg.sender, id, amount);
    }

    function withdrawArtifact(uint256 id, uint256 amount, address account) external onlyOwner{
    ...
    function mintArtifact(uint256 id, uint256 amount) external onlyOwner{
        nftArtifact.mint(id, amount - nftArtifact.balanceOf(address(this), id));
    }

    function burnArtifact(uint256 id, uint256 amount) external onlyOwner {
    ...
    function depositShoebox(uint256 uid, uint256 id, uint256 amount) external {
        ...
        emit DepositShoebox(uid, msg.sender, amount, id);
    }

    // function depositShoeboxBatch(uint256[] memory ids, uint256[] memory amounts) external{
    //     nftShoebox.safeBatchTransferFrom(msg.sender, address(this), ids, amounts, "");
    // }

    function withdrawShoebox(uint256 id, uint256 amount, address account) external onlyOwner {
        ...
        emit WithdrawShoebox(account, amount, id);
    }

    // function withdrawShoeboxBatch(uint256[] memory ids, uint256[] memory amounts, address account) external onlyOwner {
    //     nftShoebox.safeBatchTransferFrom(address(this), account, ids, amounts, "");
    // }

    function mintShoebox(uint256 id, uint256 amount) external onlyOwner {
        nftShoebox.mint(id, amount - nftShoebox.balanceOf(address(this), id));
    }

    function burnShoebox(uint256 id, uint256 amount) external onlyOwner {
