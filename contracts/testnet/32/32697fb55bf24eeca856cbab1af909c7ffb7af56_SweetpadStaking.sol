 ...
 */
contract SweetpadStaking is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 private constant DAYS_IN_YEAR = 100;
    ...
    uint256 public totalFrozenSWT;
    uint256 public totalFrozenLP;

    /// @dev The data for each account
    mapping(address => FreezeInfo) public freezeInfo;

    /// @dev The data for each account, returns power
    mapping(address => uint256) public totalPower;

    struct FreezeInfo {
        uint256 frozenUntil; // delete from here
        uint256 period; // delete from here
    ...
    address public sweetTokenAddress;
    ...
    function initialize(
        address _sweetTokenAddress
    ) external initializer {
        sweetTokenAddress = _sweetTokenAddress;
    ...
    event Freeze(address indexed account, uint256 amount, uint256 power, uint8 asset);
    /// @notice Emitted when tokens are unFrozen
    event UnFreeze(address indexed account, uint256 power, uint8 asset);
    ...
    function setSweetTokenAddress(address _sweetTokenAddress) external onlyOwner {
        sweetTokenAddress = _sweetTokenAddress;
    }
     ...
     * @param depositId_ Id of which deposit we will unstake
     */
    function unfreezeSWT(uint256 amount_, uint256 depositId_) external {
        UserDeposits memory _userDeposits = userDeposits[msg.sender][depositId_];
        require(_userDeposits.asset == 0, "SweetpadFreezing: Wrong ID");
        require(_userDeposits.frozenAmount != 0, "SweetpadFreezing: Frozen amount is Zero");
        require(_userDeposits.frozenAmount >= amount_, "SweetpadFreezing: Insufficient frozen amount");
        require(block.number >= _userDeposits.frozenUntil, "SweetpadFreezing: Locked period dosn`t pass");
        uint256 expectedPower = getPower(_userDeposits.frozenAmount - amount_, _userDeposits.period);
        require(
            expectedPower >= 10000 ether || expectedPower == 0,
            "SweetpadFreezing: At least 10.000 xSWT is required"
        );
        uint256 powerDelta = getPower(amount_, _userDeposits.period);
        _unfreezeSWT(msg.sender, amount_, powerDelta, depositId_);
    }

    /**
     * @notice Unfreeze LP tokens
     * @param depositId_ Id of freezing
     */
    function unfreezeLP(uint256 depositId_) external {
        UserDeposits memory _userDeposits = userDeposits[msg.sender][depositId_];
        require(_userDeposits.asset == 1, "SweetpadFreezing: Wrong ID");
        require(block.number >= _userDeposits.frozenUntil, "SweetpadFreezing: Locked period dosn`t pass");
        _unfreezeLP(msg.sender, depositId_);
    }
    ...
    function getAllStakersInfo() external view returns(address[] memory, FreezeInfo[] memory) {
        FreezeInfo[] memory stakersInfo = new FreezeInfo[](stakers.length());
        address[] memory users = new address[](stakers.length());
        address _user;
        for (uint256 i; i < stakersInfo.length; i++) {
            _user = stakers.at(i);
            stakersInfo[i] = freezeInfo[_user];
            users[i] = _user;
        }
        return (users, stakersInfo);
    }
    ...
    function seedUser(
        ...
        FreezeInfo storage account = freezeInfo[_user];
        ...
        stakers.add(_user);
        account.frozenAmount += _frozenAmount;
        account.power += _power;
        account.depositId = userDeposits[_user].length;
    }
    ...
    function _freeze(
        ...
        FreezeInfo storage freeze = freezeInfo[account_];
        userDeposits[account_].push(
            UserDeposits({
        ...
        stakers.add(account_);
        totalPower[account_] += power_;
        freeze.frozenAmount += amount_;
        freeze.power += power_;
        freeze.depositId = userDeposits[account_].length;
        ...
        emit Freeze(account_, amount_, power_, asset_);
    }
    ...
    function _transferAssetsToContract(
        ...
        address asset = sweetTokenAddress;
        if (asset_ == 1) {
    ...
    function _unfreezeSWT(
        ...
        FreezeInfo storage account = freezeInfo[account_];
        UserDeposits storage depositDetails = userDeposits[account_][depositId_];
        if (amount_ == depositDetails.frozenAmount) {
            totalPower[account_] -= depositDetails.power;
            delete userDeposits[account_][depositId_];
        } else {
            totalPower[account_] -= power_;
            depositDetails.frozenAmount -= amount_;
            depositDetails.power -= power_;
        }
        account.frozenAmount -= amount_;
        account.power -= power_;
        account.depositId = userDeposits[account_].length;
        totalFrozenSWT -= amount_;

        emit UnFreeze(account_, amount_, 0);
        stakers.remove(account_);

        IERC20Upgradeable(sweetTokenAddress).safeTransfer(account_, amount_);
    }

    function _unfreezeLP(address account_, uint256 depositId_) private {
        FreezeInfo storage account = freezeInfo[account_];
        UserDeposits storage depositDetails = userDeposits[account_][depositId_];
        totalPower[account_] -= depositDetails.power;
        delete userDeposits[account_][depositId_];
        totalFrozenLP -= depositDetails.frozenAmount;
        account.frozenAmount -= depositDetails.frozenAmount;
        account.power -= depositDetails.power;
        account.depositId = userDeposits[account_].length;
        stakers.remove(account_);
        emit UnFreeze(account_, depositDetails.frozenAmount, 1);

        IERC20Upgradeable(lpTokenAddress).safeTransfer(account_, depositDetails.frozenAmount);
    }
    ...
    function _transferBackUnusedAssets(
        ...
        if (tokenToTransfer > 0) {
            IERC20Upgradeable(sweetTokenAddress).safeTransfer(to, tokenToTransfer);
        }
    }
    ...
    function _addLiquidityETH(
        ...
        // slither-disable-next-line reentrancy-events
        IERC20Upgradeable(sweetTokenAddress).safeApprove(routerAddress, tokenAmount);

        (uint256 amountTokenAdded, uint256 amountETHAdded, uint256 liquidity) = IPancakeRouter02(routerAddress).addLiquidityETH{
            value: ethAmount
        ...
        IERC20Upgradeable(sweetTokenAddress).safeApprove(routerAddress, 0);
    ...
    function _swapExactETHForSwtTokens(
        ...
        path[0] = IPancakeRouter02(routerAddress).WETH();
        path[1] = sweetTokenAddress;

        amounts = IPancakeRouter02(routerAddress).swapExactETHForTokens{value: amount}(amountOutMin, path, address(this), deadline_);
        return amounts;
    }
}