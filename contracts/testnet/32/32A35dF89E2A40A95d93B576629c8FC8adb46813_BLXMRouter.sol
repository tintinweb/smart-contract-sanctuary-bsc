...
contract BLXMRewardProvider is ReentrancyGuardUpgradeable, BLXMMultiOwnable, BLXMTreasuryManager, IBLXMRewardProvider {
    ...
    function _addRewards(address token, uint totalAmount) internal nonReentrant onlyOwner returns (uint amountPerDays) {
        ...
        for (uint i = 0; i <= right; i++) {
            dailyStatistics[token][today.add(i)].rewards = amountPerDays;
        }
        for (uint i = 1; i < left; i++) {
            dailyStatistics[token][today.sub(i)].rewards = amountPerDays;
            // set aggregate rewards for elapsed in the current period
            Statistics storage statistics = dailyStatistics[token][i];
    ...
    function _arrangeFailedRewards(address token, uint rewardAmount) internal {
        ...
        if (rewards != 0) {
            ...
            for (uint i = day; i < target; i++) {
                dailyStatistics[token][i].rewards = dailyStatistics[token][i].rewards.add(rewards);
            }
        }
    }
...
interface IBLXMRouter {
    ...
    function addRewards(address token, uint amountBlxm) external returns (uint amountPerDays);
