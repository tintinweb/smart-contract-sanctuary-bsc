...
contract MpoToken is ERC20Upgradeable, OwnableUpgradeable {
    ...
    function init() external initializer {
        __ERC20_init_unchained("MPO Token", "t-MPO-2");
        ...
        buringSwitch = true;
        // bsctest
        AUTOPOOL = 0x13eadBB4Ee1234455917fFB8A32fB10c562d54Ad;
    ...
    // 3.0
    struct NewBuyTBonusInfo {
        bool status;
        uint[] percentage;
        uint startTime;
    }
    mapping(uint => NewBuyTBonusInfo) public newBuyTBonusInfo;
    ...
    function setNewBuyTokensBonusPhase(
        ...
        buyTBonusInfo[phases].status = false;
        uint newPhase = phases + 1;
        newBuyTBonusInfo[newPhase] = NewBuyTBonusInfo({
            status: true,
    ...
    function setBuyTokensBonusPhaseStatus(bool b_) external onlyDepolyer {
        newBuyTBonusInfo[phases].status = b_;
    }

    function bouns(address user_, uint amount_) internal returns (bool) {
        if (
            newBuyTBonusInfo[phases].status &&
            block.timestamp > newBuyTBonusInfo[phases].startTime
        ) {
            address _use;
            address _inv = IMPOInvite(INVITE).checkInviter(user_);
            for (uint i; i < newBuyTBonusInfo[phases].percentage.length; i++) {
                if (_inv == address(0)) {
                    return true;
                }

                newBuyTBonus[phases][_inv] +=
                    (newBuyTBonusInfo[phases].percentage[i] * amount_) /
                    100;

                _use = _inv;
                _inv = IMPOInvite(INVITE).checkInviter(_use);
            }
            return true;
        }
    }
