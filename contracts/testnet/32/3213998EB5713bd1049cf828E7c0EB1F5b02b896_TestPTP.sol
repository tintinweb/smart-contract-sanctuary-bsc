...
contract TestPTP is ERC20 {
    ...
    function setDailyShare (uint256 day) public onlyOwner{
        ...
        dailyReward = dailyReward.add(totalPenalties);
        dailyRewardRecords[day] = dailyReward * 10 ** 8; 
        dailyShare[day] = true;
        if(day >= 1 ){
        uint256 _prevDay = day.sub(1);
        bool _prevDayCheck = dailyShare[_prevDay];
        if(!_prevDayCheck){
          dailyRewardRecords[_prevDay] = dailyReward * 10 ** 8; 
          dailyShare[_prevDay] = true;
        }}
        totalPenalties = 0;
    }
    ...
    function getAllDayReward(uint256 beginDay,uint256 endDay,uint256 stakeShare) public view returns (uint256 ){
        ...
        for (uint256 day = beginDay; day < endDay; day++) {
            uint256 dayTotalShares = getStakeTotalOfDay(day);
            uint256 currDayAmount = dailyRewardRecords[day].mul(stakeShare);
    ...
    function latePenalties (stakeRecord memory stakeData,uint256 totalAmountReturned) public  returns(uint256){
            ...
            if(totalPenalty < totalAmountReturned){
             ...
             totalPenalties = totalPenalties.add(halfAmount);
    ...
    function removeStakeRecord(uint256 stakeShare) internal returns (uint256 share) {
        uint256 currentDay = findDay();
        TotalStakedShare[] memory records = perDayShareStakes[currentDay];
        if(records.length > 0){
    ...
    function claimStakeReward (uint id) public  {
        ...
        uint256 totalIntrestAmount = getAllDayReward(stakeData.startDay,currDay,stakeData.stakeShare);
        if(block.timestamp < stakeData.endDayTimeStamp){
           require(stakeData.isFreeStake != true,"Free Stake can't be claim early");
            ...
            removeStakeRecord(stakeData.stakeShare); 
        }
    ...
    function freeStaking(uint256 stakeAmount,address userAddress) internal onlyOwner {
        ...
        perDayShareStakes[startDay].push(record);
        }
    ...
    function setAvailabePTP (uint256 day) public onlyOwner {
        ...
        if(day >= 1 ){
        uint256 _prevDay = day.sub(1);
        bool _prevDayCheck = checkDay[_prevDay];
        uint256 prevDaySupply = hexAvailabel;
        if(!_prevDayCheck){
            if(_prevDay == 0){
            prevDaySupply = firstDayAvailabilty;
        }
            totalHexAvailable[_prevDay] = totalHexAvailable[_prevDay].add(prevDaySupply);
            checkDay[_prevDay] = true;
        }}
