...
abstract contract BEP20GlobalData is BEP20{
	string internal constant _name = "P10";
	string internal constant _symbol = "P10";
...
abstract contract DEX is Ownable, BEP20Modifier{
	...
	function getFee(address from, address to, uint256 value) internal returns(uint256){
		...
		//1 _balance[adr_team] = _balance[adr_team].add(amount_taken);
		//1 emit Transfer(from, adr_team, amount_taken);
		return amount_taken;
	}
