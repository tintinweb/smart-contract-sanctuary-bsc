...
contract WarcraftUniverseController is Ownable {
    ...
    uint256[3] public boxPrices = [3500,10000,15700];

    // uint256 public recoveryTime = 6 hours;

    uint256 public recoveryTime = 5 minutes;
    ...
    constructor() {
        ...
        SHE = ERC20(0x1c2187e42fd11Fc15FCef659946697598ddCbA2e);
    ...
    function setPoolCircleTime(uint256 poolId, uint256 circleTime) public onlyOwner {
        pools[poolId].circleTime = circleTime;
    }
    ...
    function inPool(uint256 poolType, uint256 heroId) public {
        ...
        userPools[msg.sender][users[msg.sender].poolIds] = Pool(
            ...
            amount.mul(poolRate[poolType - 1][userHero.id - 1]).div(100), 
    ...
    function outPool(uint256 poolId) public {
        ...
        if(userPoolRewardTime[msg.sender][myPool.id] < time){
            ...
            uint256 reward = myPool.amount.mul(rewardTime * 100 / 5 minutes).mul(myPool.rate).div(myPool.circleTime.div(5 minutes) * 10000);
        ...
        for(uint256 i = 1; i <= users[user].poolIds; i ++ ){
            ...
            if(myPool.id == poolType && myPool.amount > 0){
                ...
                if(time > startTime){
                    ...
                    // uint256 _reward = myPool.amount.mul(time.sub(startTime) * 100 / 5 minutes).mul(myPool.rate).mul().div(10000000);//10000000
                    uint256 _reward = myPool.amount.mul(time.sub(startTime) * 100 / 5 minutes).mul(myPool.rate).div(myPool.circleTime.div(5 minutes) * 10000);
    ...
    function selectMonster(uint256 monsterID,uint256 heroId) public {
        ...
        if(userHero.usedTimes > 0 && userHero.usedTimes == userHero.totalTimes && userHero.lastUsedTime + 5 minutes < block.timestamp){
