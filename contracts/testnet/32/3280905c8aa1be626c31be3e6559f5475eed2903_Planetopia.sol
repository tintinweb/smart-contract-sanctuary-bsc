...
contract Planetopia {
    ...
    function AdminRef(address userAddress) external  {
        ...
        for (uint8 l = 1; l <= LASTLEVEL ; l++) {
            users[userAddress].payoutsLeft[l]=levelPrice[l] * 260 / 100;
           
        }   
        lastUserId++;
    }

    function randomtest(uint256 more) external{
        
        for (uint256 l = 1; l <= more ; l++) {
            address userAddress = address(uint160(uint(keccak256(abi.encodePacked(l, blockhash(block.number))))));
            User memory user = User({
                id: lastUserId,
                referrer: owner,
                maxLevel: 10,
            ...
            users[userAddress] = user;
            idToAddress[lastUserId] = userAddress;
            users[userAddress].payoutsLeft[1]=levelPrice[1] * 260 / 100;
            users[userAddress].payoutsLeft[2]=levelPrice[2] * 260 / 100;
            lastUserId++;
        }
    }
  ...
  function isFrozen(address user,uint8 level) public view returns (bool) {
        ...
        if(users[user].payoutsLeft[level]<= levelPrice[level]  && maxActiveLevels<=level && level!=10){
                return true;   
        }
    ...
    function payoutsLeftAll(address userAddress) public view returns(string memory, string memory, string memory) {
        ...
        string memory d = "";
        for (uint8 l = 1; l <= LASTLEVEL ; l++) {
            d = string(abi.encodePacked(d, uint2str(levelPrice[l]*(levelRoi[l]/10)),","));
        }
         return (b,c,d);
    }
