...
contract Farm is Ownable, IGEMUNIFarming {
    ...
    function setIsEarlyClaimAllowed(uint pid, bool _isEarlyClaimAllowed) external onlyOwner {
    ...
    function addPoolToken(
        ...
        require(stakedToken != address(0), "Token is address(0)");
        require(rewardToken != address(0), "Token is address(0)");
        require(startTime < endTime, "Require startTime < endTime");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function increaseReward(uint256 _pid, uint256 _amount) external override notEnded(_pid) {
        updatePool(_pid);
        PoolInfoToken storage pool = poolInfoTokens[_pid];
    ...
    function depositToken(uint256 _pid, uint256 _amount) external override notEnded(_pid) {
        PoolInfoToken storage pool = poolInfoTokens[_pid];
    ...
    function withdrawToken(uint256 _pid, uint256 _amount) external override {
        PoolInfoToken storage pool = poolInfoTokens[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "withdraw: not good");
        uint256 startTime = pool.timePoolInfo.startTime;
        uint256 stakeTime = user.stakeTime < startTime ? startTime : user.stakeTime;
        require(stakeTime + pool.timePoolInfo.stakingPeriod < block.timestamp, "withdraw: token is locked");
        updatePool(_pid);
    ...
    function claimReward(uint256 _pid) external override {
        PoolInfoToken storage pool = poolInfoTokens[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        if (!pool.isEarlyClaimAllowed) {
            require(user.stakeTime + pool.timePoolInfo.stakingPeriod <= block.timestamp, "Reward is locked");
        }
        updatePool(_pid);
        ...
        pool.rewardToken.safeTransfer(msg.sender, currentReward);
    ...
    function emergencyRewardWithdraw(uint256 _pid, uint256 _amount, address _admin) external override onlyOwner {
        PoolInfoToken storage pool = poolInfoTokens[_pid];
