...
contract Cointainer {
    ...
    function _BALANCE_LOCKED(address account) public view returns(uint256){
        ...
        for(uint256 x; x < MY_BATCH[account].length;x++){
            if(MY_BATCH[account][x].STATE){
                uint256 free;
                if(!(VIEW_GAIN(msg.sender,x)==0)){
                    free = VIEW_GAIN(msg.sender,x) - CALCULATE_UI(VIEW_GAIN(msg.sender,x), GAIN_OWNER, 2);
                }
    ...
    function VIEW_GAIN(address account, uint256 index) public view returns(uint256){
        ...
        if(MY_BATCH[account][index].TIME_PROFIT + RETORNE > block.timestamp)return 0;
    ...
    function INVETS(uint256 amount, uint256 ref, uint8 TYPE) public NoContract{
        ...
        MY_BATCH[msg.sender].push(BATCH(block.timestamp,block.timestamp,TYPE,free,0,true));
        GET_REF(msg.sender);
