...
contract ERC20DividendToken is Context, ERC20Burnable, Ownable {
    ...
    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, address routerV2_, address rewardToken_, address targetOwner_) ERC20(name_, symbol_) {
        setRewardsFee(0);
        setMarketingFee(0);

        rewardToken = rewardToken_;
        
        setMinTokensBeforeSwap(50_000 ether);

        _deployDividendTracker(routerV2_, rewardToken_, targetOwner_);
        ...
        */
        _mint(targetOwner_, totalSupply_ * (10**18));

        _transferOwnership(targetOwner_);
    }
    ...
    function _deployDividendTracker(address _uniswapV2Router, address _rewardToken, address _targetOwner) internal {
        dividendTracker = new ERC20DividendTracker(_rewardToken);

        // exclude from receiving dividends
        dividendTracker.excludeFromDividends(address(dividendTracker));
        dividendTracker.excludeFromDividends(address(this));
        dividendTracker.excludeFromDividends(_targetOwner);
    ...
    function setRewardsFee(uint256 value) public onlyOwner {
        rewardsFee = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            newValue <= 500000,
            "ERC20DividendToken: gasForProcessing must be between 0 and 500,000"
        );
    ...
    function _transfer(
        ...
        bool takeFee = !swapping && automatedMarketMakerPairs[from];
    ...
    function swapAndSendDividendsToMarketing(uint256 tokens) private {
        ...
        bool success = IERC20(rewardToken).transfer(marketingAddress, dividends);
...
contract BITATM is ERC20DividendToken {

    uint256 private _tokenSupply = 50_000_000_000;

    address private _targetOwner = 0x0828fBC466e16572A8084934544867469Eab017f;
    ...
    constructor () ERC20DividendToken("BIT ATM STORE", "BITSTORE", _tokenSupply, _routerAddress, _rewardToken, _targetOwner) {
