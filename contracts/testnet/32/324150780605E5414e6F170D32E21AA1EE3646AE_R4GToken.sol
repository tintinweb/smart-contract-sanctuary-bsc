...
contract R4GToken is Context, IERC20, IERC20Metadata, PriceConsumerV3 {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _totalSupply = 100 * 10**9 * 10**18;
    uint8 private _decimals = 18;
    string private _symbol = "R4G";
    string private _name = "Run4Green";
    ...
    function transferAddLock(address addrTo, uint256 amountLock, uint256 timeLock1, uint256 timeLockEnd, uint8  step) public onlyOwner returns (bool success){
        _addUserLock(addrTo, amountLock, timeLock1, timeLockEnd, step);
        return true;
    }
