...
contract BeastNFT is Context, IBeastNFT, Ownable {
    ...
    function getAllBeasts(address _address) public view returns(uint256[] memory, string[] memory, uint256[] memory) {
        uint length = addressTokenIds[_address].length;
        uint256[] memory rIds = new uint256[](length);
        string[] memory rNames = new string[](length);
        uint256[] memory rCaps = new uint256[](length);
        for (uint i = 0; i < length; i++) {
            rIds[i] = addressTokenIds[_address][i];
            rNames[i] = names[tokenData[addressTokenIds[_address][i]]-1];
            rCaps[i] = capacities[tokenData[addressTokenIds[_address][i]]-1];
        }
        return (rIds, rNames, rCaps);
    }
...
contract Marketplace {
    ...
    constructor (address _rewardpool, address _feehandler, address _beast, address _warrior, address _legion) {
        ...
        rewardpool = _rewardpool;
        feehandler = IFeeHandler(_feehandler);
        bloodstone = IERC20(0x340516B933597F131E827aBdf0E3f700E24e84Ff);
