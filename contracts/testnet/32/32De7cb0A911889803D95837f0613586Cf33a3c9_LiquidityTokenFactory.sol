...
contract LiquidityTokenFactory {
    ...
    IFactoryManager public tokenFactoryManager;
    ...
    constructor(address _owner, address _tokenFactoryManager, address _router_address) {
        
        owner = _owner;

        tokenFactoryManager = IFactoryManager(_tokenFactoryManager);
    ...
    function createLiuidity(address creator_, address reciever, string memory name_, string memory symbol_, uint8 decimal_, uint256 supply, uint256 settingflag, uint256[4] memory fees, uint256 _canmint, uint256 _canburn, uint256 _canpause, uint256 _canBlacklist) public payable{
        ...
        require(msg.value >= tokenFactoryManager.calcFee(_canmint, _canburn, _canpause, _canBlacklist), "Balance is insufficent");
        ...
        tokenFactoryManager.assignTokensToOwner(creator_,address(token),1);
...
interface IFactoryManager {
  ...
  function calcFee(uint256 _canmint, uint256 _canburn,
  uint256 _canpause, uint256 _canBlacklist) external  returns(uint256);
}
