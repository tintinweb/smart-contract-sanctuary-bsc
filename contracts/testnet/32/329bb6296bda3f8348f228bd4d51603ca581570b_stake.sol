...
contract stake is Context{
    ...
    function _normalStake(uint256 _stakingamount, address _reffAdress) private returns(bool){
        ...
        users[_msgSender()].staker = _msgSender();
            ...
            if(_reffAdress != address(0x0)){
                ...
                users[_reffAdress].profit = users[_reffAdress].profit + (_stakingamount * 500).div(10000);
    ...
    function dailyStakingProfit() public returns(uint256){        
        ...
        if(users[_msgSender()].starttime <= directs[_msgSender()].joinTime[0] && directs[_msgSender()].joinTime[1] <= users[_msgSender()].starttime + 2 hours){
            ...
            users[_msgSender()].profit = users[_msgSender()].profit + _profit;
        ...
        for(uint256 j = 0; j<_users.length; j++){
            if(levelOpenofUser[_users[j]] >= j + 1){
                ...
                if(interestType[_msgSender()] == 1){
                    users[_users[j]].profit =  _profit + (maxStakeProfit.mul(levelreff[j])).div(100000000);
                }else{
                    require(interestType[_msgSender()] == 0,"user's daily profit is 0.66%");
                    users[_users[j]].profit =  _profit + (minStakeProfit.mul(levelreff[j])).div(100000000);
                }     
            }
        }
    ...
    function claim(uint256 _amount) public returns(bool){
        ...
        require(_amount <= users[_msgSender()].profit,"not enough profit");       
        require(_amount >= 10 * 10**18 && _amount <= (users[_msgSender()].stakeamount * 3), "minimum 10 & maximum 3X of stake amount can be withdrawn");
        require(block.timestamp <= users[_msgSender()].starttime + 30 hours, "get reward till 300 days");

        users[_msgSender()].profit = users[_msgSender()].profit - _amount;
