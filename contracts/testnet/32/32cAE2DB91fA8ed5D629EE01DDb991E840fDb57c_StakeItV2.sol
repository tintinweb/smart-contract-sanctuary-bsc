...
contract StakeItV2 is Ownable {
    ...
    event ClaimedCapital(address indexed user, Pack indexed pack, uint256 amount);
    ...
    function buyPackage(Pack pack) public {
        ...
        require(
            !userdetails.packages[pack].isActive || userdetails.packages[pack].stakedAmount == 0,
    ...
    function buyLevel(
        ...
        if (reffererAddress != _Owner) {
            ...
            userReffererDetails
                .levelRefferredUsers[userRefferralDetails.level]
                .push(msg.sender);
    ...
    function stake(
        ...
        userdetails.packages[pack].unClaimedAmt += pendingReward(pack, msg.sender);
    ...
    function claimRewardAmount(
        ...
        setHashCompleted(
            prepareClaimHash(pack, msg.sender, expiry),
            true
        );
    ...
    function pendingReward(Pack pack, address userAddress)
        ...
        returns (uint256)
    {
        return (_pendingReward(pack,userAddress) + UserDetails[userAddress].packages[pack].unClaimedAmt);
    }
    ...
    function addAffiliateRewardAmountToTheReffs(
        ...
        for (uint8 i = 1; i <= userRefferralDetails.uplineLength; i++) {
            ...
            if (startingLevel == 1) {
                ...
                if (checkRefBoughtAnyPack(reffererAddresses[i]))
    ...
    function claimCapital(Pack pack) public {
        ...
        require(pendingReward(pack, msg.sender) == 0, "StakeIt: Claim the pending rewards before capital!");
    ...
    function updateRewardPerShare() public onlyOperator {
        require(lastRewardPerShareUpdate + updateRewardPershareLimit < block.timestamp, "StakeIt: Only once a day");
    ...
    function viewUserAffiliateRewardForLevels(address userAddress) public view returns(uint[12] memory level) {
        for (uint i = 1; i <= 12; i++) {
    ...
    function isPackNotExpired(address userAddress)
        public
        view
        returns (bool)
    {
        for (uint256 i = 0; i <= 2; i++) {
            if (UserDetails[userAddress].packages[Pack(i)].packageBoughtTime + packages[Pack(i)].lifeSpan > block.timestamp) {
                    return true;
            }
        }
        return false;
