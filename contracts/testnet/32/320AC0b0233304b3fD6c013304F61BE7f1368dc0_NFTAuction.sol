...
contract NFTAuction is ProofsVerifier{
    ...
    function startAuction(int x, int y, bytes32[] calldata proof) public {
        ...
        require(highestPrice[getHash(x, y, 0)] == 0,"already-sold");
    ...
    function bidPlot(int x, int y, uint256 amount) public{
        require(highestPrice[getHash(x, y, currentRound)]>0,"not-started");
        require(highestPrice[getHash(x, y, currentRound)]*11/10<=amount,"bid-at-least-10-percent");
        require(block.timestamp - lastBidTime[getHash(x, y, currentRound)] > BID_TIMEOUT, "bid-too-late");
        paymentToken.transferFrom(msg.sender, address(this), amount);
        paymentToken.transfer(
            tokenOwner[getHash(x, y, currentRound)], 
            highestPrice[getHash(x, y, currentRound)]);
        lastBidTime[getHash(x, y, currentRound)] = block.timestamp;
        tokenOwner[getHash(x, y, currentRound)] = msg.sender;
        highestPrice[getHash(x, y, currentRound)] = amount;
    ...
    function timeLeft(int x, int y) public view returns(int256){
        if(lastBidTime[getHash(x, y, 0)]!=currentRound){
            return 0;
        }
        return int256(BID_TIMEOUT - (block.timestamp - lastBidTime[getHash(x, y, currentRound)]));
    }

    function getPlotDetails(int x, int y) public view  returns(uint256 price, address owner, bool isFinal){
        uint256 round = highestPrice[getHash(x, y, 0)];
        return (highestPrice[getHash(x,y,round)], tokenOwner[getHash(x,y,round)], round<currentRound || (block.timestamp - lastBidTime[getHash(x, y, currentRound)] > BID_TIMEOUT));
    }
