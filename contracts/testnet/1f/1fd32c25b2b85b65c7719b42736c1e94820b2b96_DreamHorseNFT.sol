...
contract DreamHorseNFT is ERC721URIStorage, Ownable {
    ...
    function getUserIamgeIds(address user, uint256 pageNumber) public returns(uint256[10] memory pageIamgeIds) {
        ...
        for(uint256 j = start - count; j >= end - count; j++){
            if(NFTIDToIamgeId[userNFTIDs[user][j]] != 0){
                pageIamgeIds[j] = iamgeIds[j];
            }
        }
    ...
    function getUserIamgeIds2(address user) public view returns(uint256[] memory iamgeIds) {
