...
contract V3Launch is ReentrancyGuard, OwnableUpgradeable {
    ...
    function depositNFT(uint256 _tokenId) external nonReentrant {
        ...
        require(userStakedNft[msg.sender] == 0, "Already Deposited");
        UserInfo storage user = userInfo[msg.sender];

        IERC721(nft).transferFrom(msg.sender, address(this), _tokenId);

        userStakedNft[msg.sender] = _tokenId;
    }

    // Withdrawing of NFTs
    function withdrawNFT() external nonReentrant onlyFinished notContract {
        uint256 _tokenId = userStakedNft[msg.sender];
        require(_tokenId != 0, "invalid tokenId");
        userStakedNft[msg.sender] = 0;
    ...
    function lock(uint256 _amount) public onlyActive nonReentrant notContract {
        ...
        require(userStakedNft[msg.sender] != 0, "NFT required");
