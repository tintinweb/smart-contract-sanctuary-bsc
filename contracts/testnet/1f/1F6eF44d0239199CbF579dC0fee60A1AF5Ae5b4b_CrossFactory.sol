...
contract CrossPair is ICrossPair, CrossERC20 {
    ...
    uint256 private currentSessionRatio;
    uint256 private currentSessionLPValue;

    address public constant controlCenter =
        0xd23c96c45D1A4B1a0df0134E842B4eAF0B9c2367;
    ...
    function swap(
        ...
        //**************************************** */
        //this will update once per session length in a random swap that occurs in any of the pairs within the protocol(once every minute, hour or day for eg.)
        ...
        //creates another scope to avoid stack too deep errors
        {
            ...
            //with a large amount of existing state variables in the original uniswap pair contract this was based on,
            //the variables needed to be stored and updated in a separate contract
            (
            ...
            if (crossPriceCheckActive == true) {
                uint256 currentTokenRatio = (balance0 * 10**18) / balance1;
                ...
                CrosswisePriceCheck(currentTokenRatio, maxSessionPriceChange);
            }
    ...
    function CLPriceCheck(
        uint256 _poolValue,
        uint256 _chainlinkPriceDeviation,
        uint256 _maxSessionLPValueChange
    ) private view {
        uint256 maxPoolValueRange = (_poolValue * _chainlinkPriceDeviation) /
            10000;

        uint256 maxPoolValueChange = maxPoolValueRange +
            (currentSessionLPValue * _maxSessionLPValueChange) /
            10000;
        require(
            currentSessionLPValue - maxPoolValueChange <= _poolValue &&
                _poolValue <= currentSessionLPValue + maxPoolValueChange,
            "Cross: Token ratio out of range"
        );
    }

    function CrosswisePriceCheck(
        uint256 _currentTokenRatio,
        uint256 _maxSessionPriceChange
    ) private view {
        uint256 maxSessionChange = (currentSessionRatio *
            _maxSessionPriceChange) / 10000;
        //compare current pool token ratio to saved ratio of current session
        require(
            currentSessionRatio - maxSessionChange <= _currentTokenRatio &&
                _currentTokenRatio <= currentSessionRatio + maxSessionChange
        );
    }
