  ...
  BancorFormula
{
  ...
  mapping(address => uint256) public lifetimeMinted; // stores how much each player has minted (GWT)
  mapping(address => uint256) public lifetimeBurned; // stores how much each player has burned (GWT)
  mapping(address => uint256) public lifetimeDeposited; // stores how much each player has deposited (BUSD)
  mapping(address => uint256) public lifetimeWithdrawn; // stores how much each player has withdrawn (BUSD)

  mapping(address => uint256) public lastTransactionBlock; // flashbot protection
  ...
  event LiquidityProvided(address by, uint256 tokenA, uint256 tokenB, uint256 gwt);
  ...
  function buy(uint256 tokenAAmount) public onlyNotBlocklocked {
    ...
    // calculate amount of GWT to mint and update pool balances
    uint256 gwtAmount = tokenAToGWT(tokenAAmount);
    lifetimeDeposited[msg.sender] += tokenAAmount;
    lifetimeMinted[msg.sender] += gwtAmount;
  ...
  function sell(uint256 gwtAmount) public onlyNotBlocklocked {
    ...
    treasuryA += feeA; // TODO: tem um bug em algum lugar, esse valor tá ficando mais alto que o saldo de BUSD do contrato no final da venda
    treasuryB += feeB;
    poolBalanceA -= tokenA;
    poolBalanceB -= tokenB;
    lifetimeBurned[msg.sender] += gwtAmount;
    lifetimeWithdrawn[msg.sender] += tokenA;
  ...
  /**
   * @notice Lets the operator set the blocklist address
   * @param _blocklist The address of the blocklist contract
   */
  function setBlocklist(address _blocklist) public onlyRole(OPERATOR_ROLE) {
    _setBlocklist(_blocklist);
  }
  ...
  function provideLiquidity(uint256 tokenAAmount, uint256 tokenBAmount)
    public
    onlyRole(OPERATOR_ROLE)
  {
    ...
    // mint GWT to the contract itself
    _mint(address(this), gwtAmount);

    emit LiquidityProvided(msg.sender, tokenAAmount, tokenBAmount, gwtAmount);
  }

  function burnGwt(uint256 gwtAmount) public onlyRole(OPERATOR_ROLE) {
    // burn GWT from contract
    _burn(address(this), gwtAmount);
  }
}