...
contract AiMine is Ownable {
    ...
    mapping (uint256 => mapping (address => bool)) private buyType;
    mapping (uint256 => uint256) private mineNum;


    event BuyMine(address indexed _user,address indexed _currency,uint256 mine_id,uint256 num ,uint256 _value);
 
    function buyMine(uint256 id,uint256 num)  external {
        require(id>0&&num>0,"Mine: id num error !");
        require(!buyType[id][msg.sender],"Mine: Insufficient times !");
        MineConfig storage detail =MineDetail[id];
        require(detail.status==0&&detail.id==id&&detail.amount>0,"Mine: Id not open !");
         require(mineNum[id]+num<=detail.num,"Mine: num error !");
        uint256 fee=detail.amount.mul(num).mul(20).div(100);
        IERC20(detail.currency).safeTransferFrom(msg.sender,acceptAddress,detail.amount.sub(fee));
        if(fee>0){
            IERC20(detail.currency).safeTransferFrom(msg.sender,address(this),fee);
        }
        mineNum[id]=mineNum[id]+num;
        emit BuyMine(msg.sender,detail.currency,id,num,detail.amount);
    }
    function setMine(uint256 id,uint256 value,address currency, uint256 num,uint256 status) external onlyOwner {
        MineDetail[id]=MineConfig(id,value,currency,num,status);
    }
