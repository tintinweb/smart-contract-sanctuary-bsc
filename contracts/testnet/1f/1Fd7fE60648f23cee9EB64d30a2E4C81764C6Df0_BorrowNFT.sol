...
contract BorrowNFT is Ownable, ERC721Holder {
    ...
    uint256 public feePcent = 35; // 3.5 up to 2 decimal
    // Pause contract
    bool public pause;
    ...
    uint256 public minETH = 0.000001e18;
    ...
    function getUserInfo(address _user) external view returns(Borrower memory userInfo){
        userInfo = borrowerInfo[_user];
    ...
    function liquidate(address _borrowerAddr, address payable _receiver)
        external
        onlyOwner
        ifOnlyBorrowed(_borrowerAddr)
    {
        require(
            (block.timestamp > borrowerInfo[_borrowerAddr].timestamp + endTime) ,
            "Too earley!.."
        );
