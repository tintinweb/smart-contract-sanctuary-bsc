...
contract HEXNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function acceptOfferBid(acceptOfferBidData memory _transferData) external payable {
        ...
        if(_transferData.payThrough==1) {
            transferMyntToOwner(_transferData.newOwner, tokenInfoById.currentOwner, _transferData.ownerShare);
        }
        else {
            transferwbnbToOwner(_transferData.newOwner, tokenInfoById.currentOwner, _transferData.ownerShare);
        }
    ...
    function buyNFT(buyNFTData memory _buyData) external payable {
        ...
        if(_buyData.currency==1) {
            ...
            transferMyntToOwner(msg.sender, _buyData.owner, _buyData.ownerShare);
        }
    ...
    function transferRoyaltiesToken(Royalty[] memory nftRoyalty, uint256 paymentType, address buyer, uint256 platformShareAmount) internal {
        for(uint x = 0; x < nftRoyalty.length; x++) {
            Royalty memory royalty = nftRoyalty[x];
            if(paymentType==1) {
                transferMyntToOwner(buyer, royalty.wallet, royalty.amount);
            }
            else {
                transferwbnbToOwner(buyer, royalty.wallet, royalty.amount);
            }
        }
        if(paymentType==1) {
            transferMyntToOwner(buyer, address(this), platformShareAmount);
        }
        else {
            transferwbnbToOwner(buyer, address(this), platformShareAmount);
        }
    }
    ...
    function withdrawMYNT() public onlyOwner {
        MYNT myntistToken = MYNT(myntTokenAddress);
        uint256 balance = myntistToken.balanceOf(address(this));
        require(balance >= 0, "insufficient balance" );
        myntistToken.transfer(address(this), balance);
    }
    function withdrawWBNB() public onlyOwner {
        WBNB wbnb = WBNB(wbnbAddress);
        uint256 balance = wbnb.balanceOf(address(this));
        require(balance >= 0, "insufficient balance" );
        wbnb.transfer(address(this), balance);
    }
}