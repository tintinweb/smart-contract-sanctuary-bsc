...
// Owned By https://www.champions.io/
...
contract CA is Context, IERC20, Ownable, Pausable {
  ...
  string private _name = "Champions Ascension";



    /*
	function getCAWhitelistRound() public view returns (uint256, uint256, uint256, uint256, bool, uint256) {
        
        if(_CATimestamp > 0) {
            
            uint256 wlCloseTimestampLast = _CATimestamp;
        
            for (uint256 i = 0; i < _CAWhitelistRounds.length; i++) {
                
                WhitelistRound storage wlRound = _CAWhitelistRounds[i];
     ...
     */
     ...
     /*
     function modifyCAWhitelist(uint256 index, uint256 duration, uint256 amountMax, address[] calldata addresses, bool enabled) external onlyWhitelister() {
        require(index < _CAWhitelistRounds.length, "Invalid index");
        require(amountMax > 0, "Invalid amountMax");

        if(duration != _CAWhitelistRounds[index].duration)
            _CAWhitelistRounds[index].duration = duration;
        
        if(amountMax != _CAWhitelistRounds[index].amountMax)  
            _CAWhitelistRounds[index].amountMax = amountMax;
        
        for (uint256 i = 0; i < addresses.length; i++) {
            _CAWhitelistRounds[index].addresses[addresses[i]] = enabled;
        }
    }
...
/*
  ...
  function _burnFrom(address account, uint256 amount) internal {
   ...
   //this method is responsible for taking all fee, if takeFee is true
    ...
    function _applyCAWhitelist(address sender, address recipient, uint256 amount) internal {
        
        if(_CAPairAddress == address(0) || _CAWhitelistRounds.length == 0)
            return;
        
        if(_CATimestamp == 0 && sender != _CAPairAddress && recipient == _CAPairAddress && amount > 0)
            _CATimestamp = now;
        
        if(sender == _CAPairAddress && recipient != _CAPairAddress) {
            //buying
            
            (uint256 wlRoundNumber,,,,,) = getCAWhitelistRound();
        
            if(wlRoundNumber > 0) {
                
                WhitelistRound storage wlRound = _CAWhitelistRounds[wlRoundNumber.sub(1)];
                
                require(wlRound.addresses[recipient], "CA - Buyer is not whitelisted");
                ...
                require(amount <= amountRemaining, "CA - Amount exceeds whitelist maximum");
