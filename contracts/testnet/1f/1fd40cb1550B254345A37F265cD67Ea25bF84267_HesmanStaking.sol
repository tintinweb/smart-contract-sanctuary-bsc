...
contract HesmanStaking is Ownable, ReentrancyGuard, IERC721Receiver {
  ...
  function stakeNft(uint256[] calldata _idNftList) public nonReentrant isRun {
    ...
    nftDetail.nftNumber = _idNftList.length;
    for(uint index = 0; index < _idNftList.length; index++){
        nftDetail.nftId[index] = _idNftList[index];
  ...
  function unstakeToken() public nonReentrant {
    ...
    for (uint256 index = 0; index < userNftMap[msg.sender].length; index++) {
      ...
      for(uint256 jndex = 0; jndex < nftDetail.nftId.length; jndex++){
        // Give back NFT
        IERC721(hesicNftAddress).transferFrom(address(this), msg.sender, nftDetail.nftId[jndex]);

        emit EventUserClaimNFT(msg.sender, hesicNftAddress, nftDetail.nftId[jndex], block.timestamp);
      }
    }
  ...
  function claimToken() public nonReentrant isRun {
    ...
    for (uint256 index = userNftMap[msg.sender].length-1; index >=0 ; index--) {
      userNftDetail storage nftDetail = userNftMap[msg.sender][index];
      nftTotalPrice += (unstakeBlock[msg.sender] - nftDetail.stakeBlock).mul(nftPrice).mul(nftDetail.nftNumber);
      userNftMap[msg.sender].pop();
    }
    
    uint256 amountClaim = nftTotalPrice.mul(benefitPercentPerYear).div(100).div(10368000);
