...
// MAIN CONTRACT & CURRENT COMMIT - 09/26/22 dvncan
// Update to withdraw entire balance - 09/28/22 dvncan
// Revision @ambuj-k
...
interface IStakingModule{
    ...
    function rotateReward() external;
...
contract StakingModule is IStakingModule, Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    uint256 public lockup_period; //Optional lockup period
    bool public timestampSet; // change to set when staked
    uint256 public interestPercent; // new
    bool public interestSet; // new create getter
    bool public approved; // is the contract approved to spend the token?
    ...
    mapping(address => uint256) private timePeriod; // amount of time + lockup 
   ...
   function initialize(address _erc20_contract_address) external initializer {
        // Timestamp values not set yet
        timestampSet = false;
        // Set the erc20 contract address which this timelock is deliberately paired to
        if (_erc20_contract_address == address(0)){
            revert Error_Address_Zero();
        }
        erc20Contract = IERC20Upgradeable(_erc20_contract_address);
   ...
   function _authorizeUpgrade(address) internal override onlyOwner {}
    ...
    modifier timestampNotSet() {
        if (timestampSet) {
            revert Error_Timestamp_Already_Set();
        }
        _;
    }
    ...
    */
    modifier timestampIsSet() {
        if (!timestampSet){
            revert Error_Timestamp_Not_Set();
        }
        _;
    }
    ...
    */
    modifier interestNotSet() {
        if (interestSet) {
            revert Error_Interest_Already_Set();
        }
        _;
    }
    ...
    // Modifier - new
    /**
    * @dev Throws if stake is still locked
    */
    modifier stakeTimeElapsed(){
        if (timePeriod[msg.sender] >= block.timestamp) {
            revert Error_Stake_Still_Locked();
        }
        _;
    }
    ...
    // ************
    // Need to modify so that the timestamp is set on staked sum [done]
    function setTimestamp(uint256 _lockup_period) public onlyOwner {
        timestampSet = true;
        initialTimestamp = block.timestamp;
        lockup_period = _lockup_period;
    }
    ...
    // Reward Calculation Logic Virtual Override!
    function _wps() internal  returns (bool) {
        ...
        rewardPerSecond[msg.sender] = weiPerSecond;
        return true;
    }

    // Deposit Logic Virtual Override option
    function _rotateReward() internal returns (bool) {
        if(balances[msg.sender] == 0){
            return true; //done
        }else{
            // i want to add any yield earned to balance
            // now
            uint256 unStake = block.timestamp;

            // calculate staking time
            uint256 timeStaked = unStake-(stakeTime[msg.sender]);

            // calculte reward
            uint256 rewards = timeStaked*(rewardPerSecond[msg.sender]);
    ...
    function _timeLogic() internal  {
        stakeTime[msg.sender] = block.timestamp;
    }

    //doesnt make sense
    function _lockupLogic() internal  {
        //lock up period
        // is the stake locked?
        if(lockup_period!=0){
            timePeriod[msg.sender] = stakeTime[msg.sender]+(lockup_period);
        }else{
            timePeriod[msg.sender] = stakeTime[msg.sender]+(2); // min 2 second lockup
        }
    }

    function _depositLogic(uint256 amt) internal {
        // Transfer tokens to smart contract 
        //erc20Contract.allowance(msg.sender, address(this));
        // erc20Contract.increaseAllowance(address(this) , amt.add(1));
        erc20Contract.safeTransferFrom(msg.sender, address(this), amt);
        balances[msg.sender] = balances[msg.sender]+(amt);
    }

    function _withdrawLogic(uint256 amount) internal {
    ...
    function stakeTokens(uint256 amount) public noReentrant {
        if(amount > erc20Contract.balanceOf(msg.sender)){ 
            revert Error_stakeToken_Not_Enough_Stake_Tokens();
        }
        _rotateReward();
    ...
    function unstakeTokens(uint256 amount) public stakeTimeElapsed timestampIsSet noReentrant {
        if(balances[msg.sender] < amount) {
            revert Error_unstakeToken_Insufficient_Token();
        }
        
        _rotateReward(); // changes on staked amount
    ...
    function rotateReward() public noReentrant {
        _rotateReward();
        _wps();
        _timeLogic();
    }
    ...
    function transferAccidentallyLockedTokens(IERC20Upgradeable token, uint256 amount) public onlyOwner noReentrant {
        if(address(token) == address(0)) {
            revert Error_transferAccidentallyLockedTokens_Token_address_Zero();
        }
        ...
        if(token != erc20Contract){
            revert Error_transferAccidentallyLockedTokens_Token_address_ERC20();
        }
