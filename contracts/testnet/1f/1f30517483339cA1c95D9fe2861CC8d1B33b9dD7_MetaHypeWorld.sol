...
contract MetaHypeWorld is IERC20 {
    ...
    string constant private _name = "Souls of Meta";
    string constant private _symbol = "SOM";
    ...
    TaxWallets public _taxWallets = TaxWallets({
        staking: payable(0x65ECE70129D94337122675B8cB7E11Fd8287940F)
        });
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint16 reflect, uint16 liquidity, uint16 staking) external onlyOwner {
        uint16 check = reflect + liquidity + staking;
        require(check <= maxTransferTaxes);
        _buyTaxes.reflect = reflect;
        _buyTaxes.liquidity = liquidity;
        _buyTaxes.staking = staking;
        _buyTaxes.totalSwap = check - reflect;
    }

    function setTaxesSell(uint16 reflect, uint16 liquidity, uint16 staking) external onlyOwner {
        uint16 check = reflect + liquidity + staking;
        require(check <= maxTransferTaxes);
        _sellTaxes.reflect = reflect;
        _sellTaxes.liquidity = liquidity;
        _sellTaxes.staking = staking;
        _sellTaxes.totalSwap = check - reflect;
    }

    function setTaxesTransfer(uint16 reflect, uint16 liquidity, uint16 staking) external onlyOwner {
        uint16 check = reflect + liquidity + staking;
        require(check <= maxTransferTaxes);
        _transferTaxes.reflect = reflect;
        _transferTaxes.liquidity = liquidity;
        _transferTaxes.staking = staking;
        _transferTaxes.totalSwap = check - reflect;
    }

    function setRatios(uint16 liquidity, uint16 staking, uint16 burn) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.staking = staking;
        _ratios.burn = burn;
        _ratios.total = liquidity + staking + burn;
    }
    ...
    function setWallets(address payable staking) external onlyOwner {
    ...
    function contractSwap(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 burn = (amtBalance * ratios.burn) / ratios.total;
        uint256 marketingBalance = amtBalance - ( burn);
        
        if (ratios.staking > 0) {
            _taxWallets.staking.transfer(marketingBalance);
        }
    }
