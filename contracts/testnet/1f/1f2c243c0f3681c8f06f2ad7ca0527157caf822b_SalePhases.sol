...
contract SalePhases is Context, Ownable, Initializable {
    ...
    mapping (address => uint256) private _partialWrittenuint;
    mapping (address => uint256) private _partialWrittenuint2;
    mapping (address => uint256) private _purchasedAmountInStable;

    // map address to bool
    mapping (address => bool) private _isWhitelisted;
    mapping (address => bool) private _didClaim;
    mapping (address => bool) private _PartialWritten;
    mapping (address => bool) private _PartialWritten2;
    ...
    function checkVesting() internal {
        ...
        if(!notClaimed() && block.timestamp >= TGE) {
            _PartialWritten[msg.sender] = true;
            ...
            _partialWrittenuint[msg.sender] = _pendingpurchasedPreSaleTokens[msg.sender] + _pendingpurchasedPrivateSale2Tokens[msg.sender] + _pendingpurchasedPrivateSaleTokens[msg.sender] + _pendingpurchasedSaleTokens[msg.sender];
    ...
    function claim() external {
        ...
        _partialWrittenuint2[msg.sender] = tokens;
    ...
    function showPurchasedAmount(address holder) external view returns (uint256) {
        return _purchasedAmountInStable[holder];
    }
