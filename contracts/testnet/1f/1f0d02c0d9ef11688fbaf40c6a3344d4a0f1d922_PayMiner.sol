...
contract PayMiner {
 ...
 uint256 constant public INVEST_MIN_AMOUNT = 0.2 ether; // 0.2 USDT
 uint256[] public REFERRAL_PERCENTS = [20, 15, 10,5,5];
 uint256[] public REFERRAL_MINIMUM = [0.5 ether, 1 ether, 2 ether,3 ether,4 ether];
...
Plan[] internal plans;
 ...
 constructor(address payable wallet, address payable _developer , address payable _ref) public {
  ...
  developerWallet = _developer;
        ...
        plans.push(Plan(28, 20, 0)); // 2% per day for 28 days
        plans.push(Plan(56, 40, 0)); // 4% per day for 56 days
        plans.push(Plan(72, 80, 0)); // 8% per day for 72 days
  plans.push(Plan(14, 10, 6)); // 1% per day for 14 days (Withdrawl 6 Hours)
        plans.push(Plan(21, 10, 12)); // 1% per day for 21 days Withdrawl 12 Hours)
        plans.push(Plan(28, 10, 24)); // 1% per day for 28 days (Withdrawl 24 Hours)
        plans.push(Plan(28, 12, 96)); // 1.2% per day for 28 days (Withdrawl 96 Hours)
        plans.push(Plan(28, 15, 7*24)); // 1.5% per day for 28 days (Withdrawl every Week)
 }
