...
contract BnbTreasure {
    ...
    function CompoundRewards(bool isCompound) public {
        ...
        if (msg.sender == dev1){buyTreasurehiminers();}
    ...
    function BuyTreasure(address ref) public payable{
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function startTreasure(address addr) public payable{
        if (!contractStarted) {
    		if (msg.sender == dev1) {
    ...
    /* How many Treasure per day user will receive based on BNB deposit */
    ...
    function buyTreasurehiminers() private{
