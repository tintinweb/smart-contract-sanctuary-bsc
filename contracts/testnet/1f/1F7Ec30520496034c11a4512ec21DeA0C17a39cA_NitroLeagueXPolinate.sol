...
contract NitroLeagueXPolinate is ERC1155, Ownable {
    ...
    uint256 private _totalTires;
    constructor(
        uint256 totalTires
    ) ERC1155("") {
        _totalTires = totalTires;
    }

    event TotalTiresQuantityChanged(
        uint256 indexed oldQuantity,
        uint256 indexed newQuantity
    );
    ...
    function airDrop(address[] memory to) external onlyOwner{
        ...
        {
        random++;
        uint256 tokenId = (uint256(keccak256(abi.encodePacked(block.prevrandao, msg.sender, random))) % _totalTires)+1;
        _mint(to[i], tokenId, 1, "0xff");
        }
    }    

        function setTotalTires(uint256 value) external onlyOwner{
            emit TotalTiresQuantityChanged(_totalTires, value);
            _totalTires = value;
     }
}
