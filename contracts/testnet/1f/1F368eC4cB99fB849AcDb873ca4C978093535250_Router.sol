...
contract Router is ERC1155Holder{
    ...
    function getNftInPair(address[] memory path, uint[] memory id) public view returns (bool[] memory nft) {
        require(path.length >= 2, 'PancakeLibrary: INVALID_PATH');
        nft = new bool[](path.length);
        nft[0] = false;
        for(uint i; i < path.length - 1; i++){
            nft[i+1] = IPancakePair(pairAddress(path[i+1], path[i], id[i])).getNft(path[i], id[i]);
        }
    }
}
