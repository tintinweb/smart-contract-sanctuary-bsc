...
contract OmniverseNFTMarketPlace is Initializable, IERC721ReceiverUpgradeable, PausableUpgradeable, OwnableUpgradeable,   ReentrancyGuardUpgradeable {
    ...
    function createItemForSale(
        ...
        _createItemForSale( _msgSender(), nftContract, tokenId, paymentToken, price, duration, extra_, saleType_, newOwner, _actualFee, _actualFeeDenominator);
    }
    function createItemForSaleFromUser(
        ...
        require(_feeRate > 0 && _feeDenominator>0,"Fees not set");
        validateItemForSale(nftContract, paymentToken, price, duration, extra_, saleType_, newOwner);

        /* transfer ownership of the nft to the contract itself */
        IERC721Upgradeable(nftContract).safeTransferFrom(contractAddress, address(this), tokenId);

        _createItemForSale(user, nftContract, tokenId, paymentToken, price, duration, extra_, saleType_, newOwner, _feeRate, _feeDenominator);
    }
    ...
    function buyItemForUser(address user, uint256 itemId) public payable nonReentrant whenNotPaused  {
        address contractAddress = _msgSender();
        address nftContract = idMarketItem[itemId].nftContract;
        // only the whitelisted contract will able to buy this item;
        require(_whitelisted[nftContract] == contractAddress, "Not allowed to buy");

        _buyItem(itemId, user);
    }
    ...
    function buyItem(
        ...
        address nftContract = idMarketItem[itemId].nftContract;
        require(excludeForSell[nftContract]==false, "Not allowed to buy this item");

        _buyItem(itemId, _msgSender());
    }
    ...
    function _buyItem(
        ...
        handlePayment(itemId, false, buyer);
    ...
    * isClaim - flag if payment amount from contract for the case of the auction
     */
    function handlePayment(uint256 itemId, bool isClaim, address buyer) private {
        // address buyer = _msgSender();
    ...
    function claimAuctionItem(uint256 itemId) public payable nonReentrant whenNotPaused {
        ...
        handlePayment(itemId, true, claimer);
