...
abstract contract GEMUNIStaking is PermissionGroupUpgradeable, ReentrancyGuardUpgradeable, IGEMUNIStaking{
    ...
    modifier onlyTokenOwner(uint pid, uint[] memory tokenIds) {
        ...
        for (uint i; i < tokenIds.length; i ++) {
            require(IERC721(pool.tokenPoolInfo.stakedNFT).ownerOf(tokenIds[i]) == msg.sender, "GS: not pass owner");
        }
        _;
    }
    ...
    modifier onlyTokenOwnerStaking(uint pid, uint tokenId) {
        PoolInfoNft storage pool = poolInfoNfts[pid];
        require(NFTStaked[pid][tokenId].user == msg.sender, "GS: not pass staked owner");
        _;
    }

    modifier notEnded(uint256 _pid) {
        PoolInfoNft storage pool = poolInfoNfts[_pid];
        uint256 endTime = pool.timePoolInfo.startTime + pool.rewardBalance / pool.rewardPerSecond;
    ...
    function emergencyRewardWithdraw(uint pid, uint256 _amount, address _admin) external override validPool(pid) activePool(pid) onlyOperator {
        ...
        uint256 endTime = pool.timePoolInfo.startTime.add(pool.rewardBalance.div(pool.rewardPerSecond));
        
        pool.rewardBalance = pool.rewardBalance.sub(_amount);
        pool.rewardPerSecond = pool.rewardBalance.div(endTime.sub(pool.timePoolInfo.startTime));

        pool.tokenPoolInfo.rewardToken.safeTransfer(_admin, _amount);
    ...
    function increaseReward(uint _pid, uint256 _amount) external override notEnded(_pid) activePool(_pid) validPool(_pid) onlyOperator {
        ...
        uint256 endTime = startTime.add(pool.rewardBalance.div(pool.rewardPerSecond));
    ...
    function updateStartAndEndTimes(uint pid, uint256 _startTime, uint256 _bonusEndTime) external onlyOwner {
        ...
        emit NewStartAndEndBlocks(pid, _startTime, _bonusEndTime);
    }
    ...
    function addPoolNft(
        ...
        //     if (pool.tokenPoolInfo.stakedNFT == tokenNFT && address(pool.tokenPoolInfo.stakedToken) == stakedToken && address(pool.tokenPoolInfo.rewardToken) == rewardToken && pool.timePoolInfo.stakingPeriod == stakingPeriod && block.timestamp < endTime && pool.isActive) {
        //         foundToken = true;
        //         break;
        //     }
        //     foundToken = false;
        ...
        require(address(tokenPoolInfo.stakedToken) != address(0), "Farm: Token is address(0)");
        require(address(tokenPoolInfo.rewardToken) != address(0), "Farm: Token is address(0)");
    ...
    function lockPoolNft(uint pid) external onlyOwner {
        require(poolInfoNfts.length > 0, "GS: List of token is empty");
        uint maxLengthPool = poolInfoNfts.length - 1;
        require(pid <= maxLengthPool, "GS: invalid pid");
        PoolInfoNft storage pool = poolInfoNfts[pid];
    ...
    function depositNFT(Param memory param) external override onlyTokenOwner(param._pid, param.tokenIds) notEnded(param._pid) validPool(param._pid) nonReentrant {
        ...
        if (address(pool.tokenPoolInfo.stakedToken )== address(0)){
            require(param._amountToken == 0, "GS: invalid amount");
        } else {
            require(IERC20(pool.tokenPoolInfo.stakedToken).balanceOf(msg.sender) >= param._amountToken, "GS: Don't have enough geni");
    ...
    function claimRewardNft(uint _pid) external override validPool(_pid) {
        ...
        if (!pool.isEarlyClaimAllowed) {
            require(user.stakeTime + pool.timePoolInfo.stakingPeriod <= block.timestamp, "Farm: Reward is locked");
        }
        updatePool(_pid);
    ...
    function emergencyWithdrawNFT(uint pid) external override validPool(pid) {
        PoolInfoNft storage pool = poolInfoNfts[pid];
        UserInfoNft storage user = userInfoNft[pid][msg.sender];
        require(user.amount > 0, "GS: not found info");
        uint _totalAmountNFT;
        for (uint256 i = 0; i < user.stakedNFTs.length; i++) {
            uint priceNFT = getPriceNFT(pool.tokenPoolInfo.stakedNFT, user.stakedNFTs[i]);
            _totalAmountNFT = _totalAmountNFT.add(priceNFT);
            if (IERC721(pool.tokenPoolInfo.stakedNFT).ownerOf(user.stakedNFTs[i]) == address(this)) {
                IERC721(pool.tokenPoolInfo.stakedNFT).transferFrom(address(this), msg.sender, user.stakedNFTs[i]);
        ...
        returns (uint256)
    {
        ...
        uint256 startTime = pool.timePoolInfo.startTime;
        uint256 endTime = startTime.add(
            pool.rewardBalance.div(pool.rewardPerSecond)
        );
