...
contract PancakeV3LmPool is IPancakeV3LmPool {
  ...
  modifier onlyMasterChef() {
    require(msg.sender == address(masterChef), "Not Master Chef");
    _;
  }

  modifier onlyPoolAndMasterChef() {
    require(msg.sender == address(pool) || msg.sender == address(masterChef), "Not Pool or Master Chef");
    _;
  }
  ...
  function accumulateReward(uint32 currTimestamp) external override onlyPoolAndMasterChef {
...
interface IPancakeV3FactoryOwner {
...
contract PancakeV3LmPoolDeployer {
    ...
    IPancakeV3FactoryOwner public immutable factoryOwner;

    modifier onlyMasterChef() {
        require(msg.sender == masterChef, "Not Master Chef");
        _;
    }

    constructor(address _masterChef, IPancakeV3FactoryOwner _factoryOwner) {
    ...
    function deploy(IPancakeV3Pool pool) external onlyMasterChef returns (IPancakeV3LmPool lmPool) {
        ...
        factoryOwner.setLmPool(address(pool), address(lmPool));
    }
}