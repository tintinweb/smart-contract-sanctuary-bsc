...
contract Ballot {
    ...
    bool public isPollOpened = false;
    ...
    /// @notice Store all Projects submitted per index, the index can be found using the stakes mapping
    ProjectSummary[] private projectSummary;

    mapping(uint256 => mapping(uint256 => Project)) private _specificProject; // map projectId Struct per projectId per pollId
    mapping(uint256 => Project[]) private _projectsPerPoll; // map projectIds per pollId
    ...
    function submitProject(bytes32 _title, uint256 _amount) external {
        ...
        ProjectSummary memory summary = projectSummary[pollId];
        uint256 _projectId = summary.projects.length;
        ...
        summary.pollId = pollId;
        summary.projects[_projectId] = submission;
    ...
    {
        Project[] memory projects = projectSummary[_pollId].projects;
        return projects;
    }
    ...
    function getWinner(uint256 _pollId) external view returns (Project memory) {
        ...
        if (projects.length > 2) {
            ...
            for (uint256 i = 2; i <= projects.length; i++) {
                if (this.getVoteCount(projects[i].id) > prevVotes) {
                    winner = projects[i];
                }
            }
        }
        ...
        onlyOwner
    {
        ...
        projectSummary[pollId].projects.push(); //Push once to avoid <projectId == 0>
    }
    ...
    function vote(
        ...
        ProjectSummary memory summary = projectSummary[pollId];
        require(
            projectId <= summary.projects.length && projectId != 0,
            "Invalid project id"
        );
