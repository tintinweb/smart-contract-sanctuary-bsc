...
contract RichMonkey is Context, Ownable{
    ...
    event ReceiveProfitNFT(address _addr,uint256 _amount,uint256 _time_interval,uint256 timestamp,address _nft_contract,uint256 _nft_id,uint256 reward_amount,uint256 usdt_amount,uint256 rmt_amount);
    ...
    modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, 'RichMonkeyRouter: EXPIRED');
        _;
    }

    modifier workstatus() {
        require(work == true, 'RichMonkey: PAUSE');
        _;
    }
    ...
    constructor() public {
        origin = msg.sender;
        work = true;
        dy_addr = 0x3B1654c2E7e0f4F0B513Eda0902F33FeB27Aa30A;
        usdt_receive = 0xCDfB33C87e68297D3EfD9Bb3aA36DeE918125F7A;
        _factory = 0x7Eb1e9ebEeF59EBDb71b0e23c212c997f8B9De1f;
        _router = 0x4DD635F49B77Aaa3dE80aa6E86486d38a07ED4c0;
        ...
        Vault_addr = 0x8cd4f73A1dC0C3F5EE1771e6421c781404954B0D;
        ...
        dy_min_lp = 100*usdtToken_decimals;
        usdt_price = 5*usdtToken_decimals;
        token_price = 10*rmtToken_decimals;
        initMinerRatio();
        initNftInfo();
        initNftIndex();
    ...
    function initNftIndex() private{
        nft_index[0x591e9C64eb382ED4a05F1d59FEf3FDdfa6aDAcdc] = 1;
        nft_index[0x5A90F4efd019d6F102850123e1E5e567D5dD11f9] = 2;
        nft_index[0x2deaa31C4A43a30F01574939a59550Ed0A25Ed8b] = 3;
        nft_index[0xb52912d93a587344851D958a13F6F1681fc8516B] = 4;
        nft_index[0xf113598Ea8DFad566f368A54Ec8e034Ad2cd752f] = 5;
        nft_index[0x76074523AFdA125576ec8975f2cf60c6c803a75c] = 6;
        nft_index[0xE068bC9fEC864AA3A2a0b43204605921C04Fe8ef] = 7;
    }
    ...
    function initNftInfo() private{
        ...
        nft[7].nft_num = 200000;
        nft[7].pay_amount = 15 * usdtToken_decimals;
        nft[7].pay_type = 1;
    ...
    function bindNFT(uint256 usdt_amount,uint256 rmt_amount,uint256 minusdt_amount,uint256 minrmt_amount,uint256 _nftId,address _nft_contract) public workstatus{
        ...
        uint256 _fee =  usdt_amount*2/100;
        IERC20(usdtToken).transferFrom(msg.sender, Vault_addr, _fee); 
        lp_total = lp_total + _fee;
        usdt_amount = usdt_amount - _fee;
        rmt_amount = rmt_amount*98/100;
        minusdt_amount = minusdt_amount*98/100;
        minrmt_amount = minrmt_amount*98/100;
        ...
        (uint256 amountA, uint256 amountB, uint256 liquidity) = _AddLPLiquidity(usdtToken,rmtToken,usdt_amount,rmt_amount,minusdt_amount,minrmt_amount,msg.sender,block.timestamp);
        ...
        if(lp_pool[msg.sender].liquidity==0) lp_num++;
        ...
        (uint256 reserve0,uint256 reserve1) = PancakeLibrary.getReserves(_factory,usdtToken,rmtToken);
    ...
    function addLPLiquidity(uint256 usdt_amount,uint256 rmt_amount,uint256 minusdt_amount,uint256 minrmt_amount) public workstatus{
    ...
    function removeLPLiquidity(uint256 usdt_amount,uint256 rmt_amount,uint256 liquidity) public workstatus{
    ...
    function swapAmount(uint amountIn,uint amountOutMin,address[] memory path,address to) private returns (uint256){
        ...
        require(_pair.isSwap(address(this)), 'PancakeRouter: SWAP PAUSE');
    ...
    function swap(uint256 _amount,uint256 min_amount,bool _Is) public workstatus{
        ...
        if(_Is){
            ...
            uint256 _valut = 0;
            if(usdt>0&&rmt>0){
                if(_amount>rmt) usdt_fee = usdt;
                else usdt_fee = usdt * _amount / rmt;
            }
            ...
            IERC20(usdtToken).transferFrom(msg.sender, Vault_addr, _fee); 
            //swap
            uint256 _rmt = swapAmount(trans_amount,min_amount,path,msg.sender);
            //swap end
            _isc_b = true;
            _isu_b = true;
            _rmt_w = _rmt;
    ...
    function receiveProfitNFT(uint256 _nftId,address _nft_contract) public workstatus{
        ...
        if(_amount>0){
            (uint256 _amountA,uint256 _amountB) = PancakeLibrary.getReserves(_factory,usdtToken,rmtToken);
            ...
            IERC20(rmtToken).transfer(msg.sender, receive_amount);
            nft_pledge_info[msg.sender].nft_miner_rmt = nft_pledge_info[msg.sender].nft_miner_rmt + receive_amount;
    ...
    function receiveProfit() public workstatus{
        ...
        uint256 _amount = 0;
        if(lp_num>=1000) _amount = (lp_release /86400) * (lp_pool[msg.sender].liquidity * _time_interval) /lp_weight_total;
        else _amount = (lp_release /2 /86400) * (lp_pool[msg.sender].liquidity * _time_interval) /lp_weight_total;
        if(_amount>0){
            // (uint256 _amountA,uint256 _amountB) = PancakeLibrary.getReserves(_factory,usdtToken,rmtToken);
            // _amount = _amount *  (usdt_price * _amountB) /(_amountA *  token_price) ;
            ...
            // re_amount = re_amount *  (usdt_price * _amountB) /(_amountA *  token_price) ;
    ...
    function team_rewards(uint256 _amount) private{
        ...
        if(_amount>0){
            ...
            for (uint i = 1; i <= 15; i++) {
                ...
                lp_pool[pre].rmt_amount += reward;
                nft_pledge_info[pre].recommend_rmt = nft_pledge_info[pre].recommend_rmt + reward;
                pre = relation[pre];
            }
        }
        if(total>0){
            lp_pool[dy_addr].rmt_amount += total;
        }
    }
