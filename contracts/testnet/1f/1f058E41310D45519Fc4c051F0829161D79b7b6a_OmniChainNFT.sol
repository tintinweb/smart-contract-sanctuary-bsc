...
contract OmniChainNFT is ERC721, Ownable, ILayerZeroReceiver, NonblockingReceiver {
    uint256 public mintPrice;
    uint256 public nextTokenId;
    uint256 public maxMint;
    uint256 public maxPerWallet;
    string internal baseTokenUri;
    uint256 public publicMintTimeStamp;
    ...
    constructor(
        ...
        uint256 startToken_,
        uint256 maxMint_,
        uint256 publicMintTimeStamp_
    ) payable ERC721('StickySticks','STIK') {
        ...
        maxPerWallet = 5;
        withdrawWallet = payable(owner());//set withdraw wallet address
    }
    ...
    function mintAllowList(uint256 quantity_) public payable {
        require(quantity_ <= allowList[msg.sender]);
        require(msg.value == quantity_ * mintPrice, 'wrong mint price');
        require(nextTokenId + quantity_ <= maxMint, 'sold out');
        require(walletMints[msg.sender] + quantity_ <= maxPerWallet, 'exceeded max per wallet');
    ...
    function transferToChain(
        ...
        (uint quotedLayerZeroFee, ) = endpoint.estimateFees(dstChainId_, address(this), payload, false, adapterParams);
