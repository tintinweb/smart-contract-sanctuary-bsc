...
contract ERC721NFT is Ownable, SignerRole, IERC721, ERC721Burnable, ERC721Base {
    ...
    uint256 public maxBatchCount=20;
    ...
    function setMaxBatchCount(uint256 _count) public onlyOwner {
        maxBatchCount = _count;
    }
    ...
    function batchMint(uint256 tokenId, uint256 nftCount, uint8 v, bytes32 r, bytes32 s, Fee[] memory _fees, string[] memory _tokenURIs) public {
        require(nftCount<=maxBatchCount, "max limit reached");
        require(_tokenURIs.length == nftCount, "invalid tokenURI length");
        require(isSigner(ecrecover(toEthSignedMessageHash(keccak256(abi.encodePacked(this, tokenId, nftCount))), v, r, s)) , "owner should sign tokenId");
        
        for(uint i=0; i<nftCount; i++) {
            _mint(msg.sender, tokenId + i, _fees);
            _setTokenURI(tokenId + i, _tokenURIs[i]);
        }
    }
