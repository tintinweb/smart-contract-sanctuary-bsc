    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => ManagerData) public positionManager;
    ...
    function openMarketOrderByQuote(
        address _positionManager,
        Side _side,
        uint256 _quantity,
        uint16 _leverage
...
//        validateOrderQuantity(_positionManager, _quantity);
//        uint256 _depositQuantity = _quantity / _leverage;
//        uint256 _fee = 0;
//        uint256 tollRatio = uint256(
//            positionManager[_positionManager].tollRatio
//        );
//        if (tollRatio != 0) {
//            _fee = _depositQuantity / tollRatio;
//        }
//        deposit(_positionManager, msg.sender, _depositQuantity, _fee);
//
//        futuresAdapter.crossBlockchainCall(
//            posiChainId,
//            posiChainCrosschainGatewayContract,
//            uint8(Method.OPEN_MARKET_BY_QUOTE),
//            abi.encode(
//                _positionManager,
//                _side,
//                _quantity,
...
//        );
        ...
        nonReentrant
    {
        futuresAdapter.crossBlockchainCall(
            posiChainId,
            posiChainCrosschainGatewayContract,
            uint8(Method.INSTANTLY_CLOSE_POSITION),
            abi.encode(_positionManager, _quantity, msg.sender)
        );
    }
    ...
    function calcFeeBasedOnDepositAmount(
        ...
        uint256 tollRatio = uint256(positionManager[_manager].tollRatio);
        if (tollRatio != 0) {
        ...
        returns (uint256)
    {
        ...
        return
            (_margin * 100) / (100 - positionManager[_manager].assetRfiPercent);
    }
    ...
    function calcQuantity(address _manager, uint256 _quantity)
        ...
        returns (uint256)
    {
        uint256 contractPrice = positionManager[_manager].contractPrice;
        if (contractPrice > 0) {
    ...
    function calcNotional(
        ...
        uint256 baseBasicPoint = positionManager[_manager].baseBasicPoint;
        // coin-m
        _quantity = calcQuantity(_manager, _quantity);
        if (positionManager[_manager].contractPrice > 0) {
        ...
        returns (uint256)
    {
        return
            (uint256(_pip) *
                uint256(positionManager[_manager].baseBasicPoint)) /
            uint256(positionManager[_manager].basicPoint);
    }
        ...
        internal
    {
        uint256 minimumQuantity = uint256(
            positionManager[_manager].minimumOrderQuantity
        );
    ...
    function setPositionManagerMapping(
        address _positionManager,
        uint64 _tollRatio,
        uint32 _basicPoint,
        ...
        positionManager[_positionManager].tollRatio = _tollRatio;
        positionManager[_positionManager].basicPoint = _basicPoint;
        positionManager[_positionManager].baseBasicPoint = _baseBasicPoint;
        positionManager[_positionManager].contractPrice = _contractPrice;
        positionManager[_positionManager].assetRfiPercent = _assetRfiPercent;
        positionManager[_positionManager]
            .minimumOrderQuantity = _minimumOrderQuantity;
    }

    function setManagerTollRatio(address _positionManager, uint64 _tollRatio)
        public
        onlyOwner
    {
        require(_positionManager != address(0), Errors.VL_EMPTY_ADDRESS);
