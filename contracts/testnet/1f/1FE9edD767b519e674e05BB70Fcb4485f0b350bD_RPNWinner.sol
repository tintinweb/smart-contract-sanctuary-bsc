...
contract RPNWinner {
    ...
    uint256[3] private level3Percents = [100, 200, 300];    
    ...
    uint256[20] private level6Percents = [3]; 
    ...
    uint256 private constant roylPoolPercents = 30;
    ...
    function register(address _referral) external {
            if(userInfo[_referral].totalDeposit > 0) {
                ...
                user.start = block.timestamp;
                _updateTeamNum(msg.sender);
                unpaidUser = unpaidUser.add(1);
                emit Register(msg.sender, _referral);
            }
    }

    function deposit(uint256 _amount) external { //possible reentrancy vulnerability
        usdt.transferFrom(msg.sender, address(this), _amount);
        _deposit(msg.sender, _amount);
        emit Deposit(msg.sender, _amount);
        
/*        UserInfo storage user = userInfo[msg.sender];
        require(userInfo[user] == unpaidUser);
        unpaidUser = unpaidUser.sub(1);
        totalUser = totalUser.add(1);
*/            
        }
    ...
    function depositByDvlptFund(uint256 _amount) external {
        require(_amount >= 5 && _amount.mod(minDeposit) == 0, "amount err");
...
//        function withdraw() external nonReentrant {
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 1000e6){
            ...
            if(total >= 2000e6 && user.teamNum >= 300 && maxTeam >= 75000e6 && otherTeam >= 75000e6){
                levelNow = 6;
                    }else if(user.teamNum >= 200 && maxTeam >= 50000e6 && otherTeam >= 50000e6){
                    levelNow = 5;            
                    }else if(user.teamNum >= 100 && maxTeam >= 20000e6 && otherTeam >= 20000e6){
                    levelNow = 4;
    ...
    function _deposit(address _user, uint256 _amount) private { //possible reentrancy vulnerability
        UserInfo storage user = userInfo[_user];
    ...
    function _distributeDeposit(uint256 _amount) private { //possible reentrancy vulnerability
        ...
        luckPool = luckPool.add(luck); //possible reentrancy vulnerability
