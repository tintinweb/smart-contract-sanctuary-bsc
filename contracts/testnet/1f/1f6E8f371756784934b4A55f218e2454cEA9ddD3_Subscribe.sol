...
contract Subscribe is Base, ISubscribers {
    ...
    function write_Off(address _addr) public onlyForUser(_addr) {
        if (users[_addr].untilTime <= block.timestamp) {
            ...
            users[_addr].untilTime = block.timestamp + subscriptionDuration;
    ...
    function set_trial(uint256 _trialDuration) public onlyOwner {
        trialDuration = _trialDuration;
    }
    ...
    function set_paymentDelta(uint256 _paymentDelta) public onlyOwner {
        paymentDelta = _paymentDelta;
    }
    ...
    function set_subscription_options(
        ...
        set_subscriptionPrice(_subscriptionPrice);
        set_subscriptionDuration(_subscriptionDuration);
        set_paymentDelta(_paymentDelta);
        set_trial(_trialDuration);
    }
