...
contract Primaax is ERC20, Ownable {
    ...
    uint256 public constant TIME_COUNT = 1 minutes;
    uint8[7] public bonusPercentage = [10, 5, 3, 2, 2, 3, 5];
    ...
    constructor (address _usdtAddress) ERC20 ("Primaax", "PMX") {
    ...
    function stake(address _referrer, uint256 _amount, uint8 _planId) external {
        ...
        (uint8 ftd, uint8 ttd, uint8 otd, uint8 fh, uint8 oh, uint8 f) = _calCurrentLevel(msg.sender);
        ...
        if(user.packId != 7) {
            if (user.totalStake >= 50000e18 && ftd >= 7) {
                ...
                user.packId = 5;
            } else if (user.totalStake >= 5000e18 && fh >= 4) {
                user.packId = 4;
            } else if (user.totalStake >= 1000e18 && oh >= 3) {
                user.packId = 3;
            } else if (user.totalStake >= 500e18 && f >= 2){
                user.packId = 2;
            } else {
                user.packId = 1;
            }
        ...
        user.deposits.push(Deposit(stakeAmt, block.timestamp + TIME_COUNT, block.timestamp + 366 minutes, _planId));
    ...
    function _checkRef(address _user, address _referrer) internal returns (address) {
        ...
        if (user.referrer == address(0) || userInfo[_referrer].totalStake < 250 * 10 ** 18) {
            
            user.referrer = _referrer;

            downline[user.referrer].push(msg.sender);
    ...
    function _calRefBonus(address _upline, uint256 _stakeAmt) internal {
        ...
        for(uint256 i = 0; i < 7; i++) {
            ...
            upline.totalAllBonus += bonusPercentage[i] * stakeAmt / 100;
            upline.totalRefBonus += bonusPercentage[i] * stakeAmt / 100;
    ...
    function _updateGroupAmt(address _upline, uint256 _stakeAmt) internal {
        ...
        for(uint256 i = 0; i < 15; i++) {
            ...
            if(upline.flushDeathline < block.timestamp) {
                upline.flushAmt = 0;
                upline.flushDeathline = block.timestamp + 90 * TIME_COUNT;
            }
    ...
    function _calCurrentLevel(address _user) internal view returns (uint8, uint8, uint8, uint8, uint8, uint8) {
        address user = _user;
        uint8 ftd;
        uint8 ttd;
        uint8 otd;
        ...
        for(uint256 i = 0; i < downline[user].length; i++) {
            ...
            if (direct_down.totalStake >= 50000e18)
                ftd++;
            ...
            if (direct_down.totalStake >= 5000e18)
                fh++;
            if (direct_down.totalStake >= 1000e18)
                oh++;
            if (direct_down.totalStake >= 500e18)
                f++;

        }
        return (ftd, ttd, otd, fh, oh, f);
    }
    function userReward(address _user) external view 
    returns(uint groupTotalBonus, uint stakeTotalBonus, uint256 totalBonus, uint rewardClaim, uint256 claimableUsd) {
        address puser = _user;
        ...
        for(uint256 i = 0; i < downline[puser].length; i++) {
            ...
            if(direct_down.flushAmt > gi.highest) {
                theRest += gi.second;
                gi.second = gi.highest;
                gi.highest = direct_down.flushAmt;
            }
            else if(direct_down.flushAmt + 1 > gi.second) {
                theRest += gi.second;
                gi.second = direct_down.flushAmt;
            }
            else {
        ...
        if(lowest > 10000e18 && gi.claimed[0] < 1) {
            groupBonus += 10000e18;
        }

        if(lowest > 100000e18 && gi.claimed[1] < 3) {
            groupBonus += 100000e1;
        }

        if(lowest > 500000e18 && gi.claimed[2] < 2) {
            groupBonus += 500000e18;
        }

        if(lowest > 1000000e18 && gi.claimed[3] < 1) {
            groupBonus += 1000000e18;
        }

        if(lowest > 5000000e18 && gi.claimed[4] < 1) {
            groupBonus += 5000000e18;
        }

        if(lowest > 10000000e18 && gi.claimed[5] < 1) {
            groupBonus += 10000000e18;
        }

        user.totalGroupBonus += groupBonus;
        user.totalStakeBonus = bonus;
        user.totalAllBonus = user.totalStakeBonus + user.totalGroupBonus;
        
        uint256 claimable = user.totalAllBonus - user.totalClaimed;
        uint256 in_usd = claimable / 10;
        return (user.totalGroupBonus, user.totalStakeBonus, user.totalAllBonus, user.totalClaimed, in_usd );
    }

    function userGroupInfo(address _user) external view returns(
        address legOne, address legTwo, 
        uint256 highest, uint256 second, uint256 rest, 
        uint8 oneThousand, uint8 tenThousand, uint8 fiftyThousand, uint8 hundredThousand, uint8 fiveHundredThousand, uint8 oneMillion) {
        address puser = _user;
        Group memory gi = groupInfo[puser];
        // delete gi.legThree;
        for(uint256 i = 0; i < downline[puser].length; i++) {
            ...
            if(direct_down.flushAmt > gi.highest) {
                ...
                gi.rest += direct_down.flushAmt;
            }
        }
        return (gi.legOne, gi.legTwo, gi.highest, gi.second, gi.rest, gi.claimed[0], gi.claimed[1], gi.claimed[2], gi.claimed[3], gi.claimed[4], gi.claimed[5]);
    }
    ...
    function withdraw() external {
        ...
        for(uint256 i = 0; i < user.deposits.length; i++) {
            // console.log("Index: ", i);
            // console.log("Principal: ", user.deposits[i].amount);
            // console.log("ROR: ", plan[user.deposits[i].planId].ror);
            // console.log("Rate: ", RATE);
            // console.log("Payback in year: ", user.deposits[i].amount * plan[user.deposits[i].planId].ror / RATE);
            // console.log("Start: ", user.deposits[i].start);
            // console.log("Now: ", block.timestamp);
            // console.log("Day: ", (block.timestamp - user.deposits[i].start) / TIME_COUNT);
            bonus += user.deposits[i].amount * plan[user.deposits[i].planId].ror * ((block.timestamp - user.deposits[i].start) / TIME_COUNT  > 365 ? 365 : (block.timestamp - user.deposits[i].start) / TIME_COUNT ) / RATE / 365;
        }
        ...
        for(uint256 i = 0; i < downline[msg.sender].length; i++) {
            User memory direct_down = userInfo[downline[msg.sender][i]];
            if(direct_down.flushAmt > gi.highest) {
                theRest += gi.second;
                gi.second = gi.highest;
                gi.highest = direct_down.flushAmt;
                gi.legOne = downline[msg.sender][i];
            }
            else if(direct_down.flushAmt + 1 > gi.second) {
                theRest += gi.second;
                gi.second = direct_down.flushAmt;
        ...
        if(lowest > 10000e18 && gi.claimed[0] < 1) {
            user.flushDeathline = block.timestamp + 60 * TIME_COUNT;
            groupBonus += 10000e18;
            gi.claimed[0] += 1;
        }

        if(lowest > 100000e18 && gi.claimed[1] < 3) {
            user.flushDeathline = block.timestamp + 30 * TIME_COUNT;
            groupBonus += 100000e1;
            gi.claimed[1] += 1;
        }
        ...
        if(user.flushDeathline < block.timestamp) {
            user.flushAmt = 0;
            user.flushDeathline = block.timestamp + 90 * TIME_COUNT;
        }
