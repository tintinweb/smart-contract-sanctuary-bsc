...
contract Staking { 
    ...
    function stakeVRMC(uint numDays) public payable{
    ...
    function modifyLockUpPeriods(uint numDays, uint changedPercentage   ) public{
        require(owner == msg.sender , "only VRMetaCenter can change staking Period");
    ...
    function changeLockUpDate(uint positionId,uint newLockUpDate) external returns(uint){
        require(owner == msg.sender , "only VRMetaCenter can change lockUpPeriod");
