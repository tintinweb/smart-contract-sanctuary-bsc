...
// File: contracts/charity-maker.sol


pragma solidity ^0.8.0;


contract CharityGenrator {
     uint256 charityCreationFee = 100 gwei;

    event CharityCreated(address charityAddress);
    ...
    function CreateCharity(
        ...
        require(msg.value >= charityCreationFee, "Amount must be greater ");
          IERC20 instance = IERC20(_rToken);

        require(instance.allowance(msg.sender, address(this)) > _rewardAmount,"Charity: Insufficent allowance");
        require(
            address(_rToken) != address(0),
            "Charity: Reward must not be eqauls to Zero"
        );

       require(instance.balanceOf(msg.sender) > _rewardAmount,"insufficet amt");
        ...
        Charity charity = new Charity(
         ...
         _rToken,
         _rewardAmount,
         _startTime, // the timestamp to start the charity
         _endTime, // the timestamp to end the charity
         _adminFee,
         _minimumDonationAchive,
         _minimumDonate,
         _howManyTimeDistribution
        );
        instance.transferFrom(msg.sender , address(charity), _rewardAmount);  
        emit CharityCreated(address(charity));
    }
}