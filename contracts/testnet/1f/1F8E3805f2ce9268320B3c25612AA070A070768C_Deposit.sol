...
contract Deposit is Ownable {
    ...
    mapping(address => uint256) public addressToNumberOfDeposits;
    mapping(address => Deposit[]) public addressToDeposits;

    event DepositCreated(address _payer, string _id, uint256 _amount);
    ...
    function createDeposit(string memory _id) public payable {
        //Check if value is greater than 0
        require(msg.value > 0, "Amount cannot be equal to 0");
        //Fetch mapppings
        // bytes32[] storage hashes = addressToHashes[msg.sender];
        Deposit[] storage deposits = addressToDeposits[msg.sender];
        uint256 numberOfDeposits = addressToNumberOfDeposits[msg.sender];
        //Create Hash from id
        bytes32 hash = keccak256(abi.encodePacked(_id));
        //Update Mappings
        hashToAmount[hash] = msg.value;
        // hashes.push(hash);
        // addressToHashes[msg.sender] = hashes;
        numberOfDeposits += 1;
        addressToNumberOfDeposits[msg.sender] = numberOfDeposits;
        deposits.push(Deposit(block.timestamp, _id, msg.value));
        addressToDeposits[msg.sender] = deposits;
        console.log("Amount", msg.value);
        emit DepositCreated(msg.sender, _id, msg.value);
    }

    function checkDepositExist(string memory _id) public view returns (bool) {
        bytes32 hash = keccak256(abi.encodePacked(_id));
    ...
    function viewDeposit(string memory _id) public view returns (uint256) {
        bytes32 hash = keccak256(abi.encodePacked(_id));
        uint256 amount = hashToAmount[hash];
        return amount;
    }

    function withdrawDeposit(string memory _id, address _to) public {
        ...
        emit DepositWithdrawn(msg.sender, _id, amount, _to);
    }
    ...
    function sendFees() internal returns (bool){
        (bool sent, bytes memory data) = payable(feeWallet).call{value: feeAmount}("");
        return sent;
    }
}