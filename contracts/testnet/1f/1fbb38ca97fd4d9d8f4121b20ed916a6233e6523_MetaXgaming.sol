...
contract MetaXgaming is Context, IERC20 {
    ...
    string private constant TOKEN_NAME = "MetaXgaming";
    string private constant TOKEN_SYMBOL = "MXG";
    ...
    uint256 private buyMaxTxPercent = 0;
    ...
    uint256 public sellMaxTxPercent = 3;
...
//===============================================================================================================
    ...
    function setTaxesBuy(uint16 liquidityFee, uint16 reflectionFee, uint16 marketingFee) external onlyOwner {
        uint16 totalTax = liquidityFee + reflectionFee + marketingFee ;
        require(totalTax <= MAX_TAX);
        _buyFees = Fees(reflectionFee, liquidityFee, marketingFee, totalTax);
    }

    function setTaxesSell(uint16 liquidityFee, uint16 reflectionFee, uint16 marketingFee) external onlyOwner {
        uint16 totalTax = liquidityFee + reflectionFee + marketingFee ;
        require(totalTax <= MAX_TAX);
        _sellFees = Fees(reflectionFee, liquidityFee, marketingFee, totalTax);
    }

    function setTaxesTransfer(uint16 liquidityFee, uint16 reflectionFee, uint16 marketingFee) external onlyOwner {
        uint16 totalTax = liquidityFee + reflectionFee + marketingFee ;
        require(totalTax <= MAX_TAX);
        _transferFees = Fees(reflectionFee, liquidityFee, marketingFee, totalTax);
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee) private view returns (ExtraValues memory) {
        ...
        values.rTransferAmount = values.rAmount - (values.rFee + (values.tLiquidity * currentRate) + (currentRate));
        return values;
    }
