...
contract SoulmetaFingerPlay is DivestorUpgradeable {
    ...
    function setMeta(address serverAddr_, address soulToken_, address fishPondBonus_, address timeBank_, address aSm3d_, bool isOpen_, address kakaToken_, address iziToken_, address usdtToken_, address stiToken_, uint gasFress_) public onlyOwner {
        serverAddr = serverAddr_;
        gasFees = gasFress_;
        soulTokenAddr = ISoulToken(soulToken_);
        kakaTokenAddr = ISoulToken(kakaToken_);
        iziTokenAddr = ISoulToken(iziToken_);
        usdtTokenAddr = ISoulToken(usdtToken_);
        stiTokenAddr = ISoulToken(stiToken_);
        aFishPondBonus = fishPondBonus_;
        aTimeBank = timeBank_;
        aSm3d = aSm3d_;
        isOpen = isOpen_;
    ...
    // position_ : 1左 2右
    // pay_ : 1 - soul | 2 - kaka | 3 - IZI | 4 - BNB | 5 - usdt | 6 - STI
    function enterRoom(uint roomId_, uint amount_, uint position_, uint pay_) public payable {
        ...
        require(pay_ >= 1 && pay_ <= 6, "pay err");

        RoomInfo storage info = roomIdToUserInfo[roomId_];
        if (info.amount <= 0 && pay_ == 1) {
            require(amount_ >= 2 ether, "amount error");
            info.amount = amount_;
        } else if (info.amount <= 0 && pay_ == 2) {
            require(amount_ >= 100 ether, "amount error");
            info.amount = amount_;
        } else if (info.amount <= 0 && pay_ == 3) {
            require(amount_ >= 1 ether, "amount error");
            info.amount = amount_;
        } else if (info.amount <= 0 && pay_ == 4) {
            require(amount_ >= 0.02 ether, "amount error");
            info.amount = amount_;
        } else if (info.amount <= 0 && pay_ == 5) {
            require(amount_ >= 4 ether, "amount error");
            info.amount = amount_;
        } else if (info.amount <= 0 && pay_ == 6) {
            require(amount_ >= 10 ether, "amount error");
            info.amount = amount_;
        }
        ...
        } else if (roomPay[roomId_] == 2) {
            kakaTokenAddr.transferFrom(_msgSender(), address(this), info.amount);
        } else if (roomPay[roomId_] == 3) {
            iziTokenAddr.transferFrom(_msgSender(), address(this), info.amount);
        } else if (roomPay[roomId_] == 4) {
            payable(address(this)).transfer(info.amount);
        } else if (roomPay[roomId_] == 5) {
            usdtTokenAddr.transferFrom(_msgSender(), address(this), info.amount);
        } else if (roomPay[roomId_] == 6) {
            stiTokenAddr.transferFrom(_msgSender(), address(this), info.amount);
        }
    ...
    function settleGame(uint roomId_, address winner_) public {
        ...
        if (roomPay[roomId_] == 2) {
            extractKaka += extractCount;
            // KAKA
            kakaTokenAddr.transfer(winner_, winCount);
        } else if (roomPay[roomId_] == 3) {
            extractIzi += extractCount;
            iziTokenAddr.transfer(winner_, winCount);
        } else if (roomPay[roomId_] == 4) {
            extractBnb += extractCount;
            payable(winner_).transfer(winCount);
        } else if (roomPay[roomId_] == 5) {
            extractUsdt += extractCount;
            usdtTokenAddr.transfer(winner_, winCount);
        } else if (roomPay[roomId_] == 6) {
            extractSti += extractCount;
            stiTokenAddr.transfer(winner_, winCount);
        }
    ...
    function extractionKaka(address wallet_, uint pay_) public onlyOwner {
        if (pay_ == 2) {
            require(extractKaka != 0, "Insufficient balance");
            kakaTokenAddr.transfer(wallet_, extractKaka);
            extractKaka = 0;
        } else if(pay_ == 3) {
            require(extractIzi != 0, "Insufficient balance");
            iziTokenAddr.transfer(wallet_, extractIzi);
            extractIzi = 0;
        } else if(pay_ == 4) {
            require(extractBnb != 0, "Insufficient balance");
            payable(wallet_).transfer(extractBnb);
            extractBnb = 0;
        } else if(pay_ == 5) {
            require(extractUsdt != 0, "Insufficient balance");
            usdtTokenAddr.transfer(wallet_, extractUsdt);
            extractUsdt = 0;
        } else if(pay_ == 6) {
            require(extractSti != 0, "Insufficient balance");
            stiTokenAddr.transfer(wallet_, extractSti);
            extractSti = 0;
        }
    }
}
