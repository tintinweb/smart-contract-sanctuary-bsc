...
contract GweiDaoToken is ERC20, ERC20Permit, ERC20Votes, Ownable {
    ...
    uint256 private transferTaxRate = 100;    // 1%
    uint256 private burnRate = 0;
    uint256 public _marketingFee = 25; // 1* 25 / 100 = 0.25%
    uint256 public _teamFee = 25; // 1* 25 / 100 = 0.25%
    uint256 public _liquidityFee = 50; // 1* 50 / 100 = 0.5%
    bool public swapAndLiquifyEnabled = true;

    address public _marketingWallet = 0xa33dF9272C7EdCdce4bf920f601FE5A00C38F75c;
    address public _teamWallet = 0xa33dF9272C7EdCdce4bf920f601FE5A00C38F75c;

    uint256 private _swapTokensAtAmount = 500_000e18;
    ...
    function updateTransferTaxRate(uint256 _transferTaxRate) public onlyOwner {
        require(_transferTaxRate <= 1000, "GweiDao: Transfer tax rate must not exceed the maximum rate.");
        transferTaxRate = _transferTaxRate;
    }
    
    function getTransferTaxRate() external view onlyOwner returns (uint256) {
    ...
    function getBurnRate() external view onlyOwner returns (uint256) {
        return burnRate;
    }
    ...
    function updateTeamFee(uint8 newFee) external onlyOwner {
        require ( _marketingFee + _liquidityFee + newFee == 100, "GweiDao: total tax must be 100%");
        _teamFee = newFee;
    }

    function updateMarketingFee(uint8 newFee) external onlyOwner {
        require ( _teamFee + _liquidityFee + newFee == 100, "GweiDao: total tax must be 100%");
        _marketingFee = newFee;
    }
    
    function updateLiquidityFee(uint8 newFee) external onlyOwner {
        require ( _teamFee + _marketingFee + newFee == 100, "GweiDao: total tax must be 100%");
        _liquidityFee = newFee;
    }

    function updateMarketingWallet(address _newWallet) external onlyOwner {
        require(_newWallet != _marketingWallet, "GweiDao: The marketing wallet is already this address");
        excludeFromFee(_newWallet);
        _marketingWallet = _newWallet;
    }
    
    function updateTeamWallet(address _newWallet) external onlyOwner {
        require(_newWallet != _teamWallet, "GweiDao: The marketing wallet is already this address");
        excludeFromFee(_newWallet);
    ...
    function _transfer(
        ...
        if (transferTaxRate == 0 || _isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            bool canSwap = taxBalance >= _swapTokensAtAmount;
            if (
                canSwap == true
                ...
                uint256 swapForLiq = taxBalance.mul(_liquidityFee).div(100);
                swapAndLiquify(swapForLiq);

                uint256 swapForEth = taxBalance.sub(swapForLiq);
                swapTokensForEth(swapForEth);
    ...
    function sendETHToFee(uint256 amount) private {
        payable(_marketingWallet).transfer(amount.mul(_marketingFee).div(_marketingFee.add(_teamFee)));
        payable(_teamWallet).transfer(address(this).balance);
    }

    function swapAndLiquify(uint256 swapForLiq) private lockTheSwap {
        // split the contract balance into halves
        uint256 half = swapForLiq.div(2);
        uint256 otherHalf = swapForLiq.sub(half);
