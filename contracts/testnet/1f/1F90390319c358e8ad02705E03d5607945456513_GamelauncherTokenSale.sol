...
contract GamelauncherTokenSale  {
    ...
    struct UserPhaseVestingStatus {
        string orderId;
        bool[20] vestingStatus;
	}

    mapping (string => UserPurchased) public getUserPurchaseDetails;
    ...
    mapping (string => UserPhaseVestingStatus) public getUserPhaseVestingStatus;
    ...
    function addProjectPhase(string memory _phaseId,string memory _projectId,uint _startUTCDateTime,uint _endUTCDateTime,uint256 _tokenPrice,uint256 _tokenSaleCap) public onlyOwner() {
        ...
        projectphasedetail.phaseStatus=true;
    ...
    function updateProjectPhaseVesting(string memory _phaseId,uint _vestingIndex,uint _releaseUTCDateTime,uint _releasePer) public onlyOwner(){
        ...
        if(getProjectPhaseVestingDetails[_phaseId].releasePer[_vestingIndex]==0)
        {
         ProjectPhaseVestingDetails storage projectphasevestingdetail = getProjectPhaseVestingDetails[_phaseId];
    ...
    function getPhaseVestingDetails(string memory _phaseId,uint _vestingIndex)public view returns(uint _releaseUTCDateTime, uint _releasePer,uint _lastUpdatedUTCDateTime){
       return (getProjectPhaseVestingDetails[_phaseId].releaseUTCDateTime[_vestingIndex], getProjectPhaseVestingDetails[_phaseId].releasePer[_vestingIndex],getProjectPhaseVestingDetails[_phaseId].lastUpdatedUTCDateTime);
    }
    ...
    function Participate(string memory _phaseId,string memory _projectId,string memory _orderId,uint256 _numberOfTokens) public payable returns (bool) {
       ...
       require(getProjectPhaseDetails[_phaseId].phaseStatus == true ,'Phase Is Not Active !');
       require(getProjectDetails[_projectId].projectStatus == true ,'Project Is Not Active !');
       require(getUserPurchaseDetails[_orderId].totalPurchased == 0 ,'Order Id Already Exists !');
       uint256 scaledNumberOfTokens = safeMultiply(_numberOfTokens,uint256(10) ** getProjectDetails[_projectId].decimals);
       ...
       UserPurchased storage userpurchased = getUserPurchaseDetails[_orderId];
       Projects storage project = getProjectDetails[_projectId];
       ProjectPhaseDetails storage projectphasedetail = getProjectPhaseDetails[_phaseId]; 
       ...
       userpurchased.totalPurchased+=scaledNumberOfTokens;
    ...
    function _setupTokenClaim(string memory _projectId,uint _amount) public onlyOwner() {
        require(primaryAdmin==msg.sender, 'Admin what?');
        getProjectDetails[_projectId].projectContract.transferFrom(msg.sender, address(this), _amount);
    }
    ...
    function _reverseSetupTokenClaim(string memory _projectId,uint _amount) public onlyOwner() {
        require(primaryAdmin==msg.sender, 'Admin what?');
        getProjectDetails[_projectId].projectContract.transfer(primaryAdmin, _amount);
    }

    function Claim(string memory _phaseId,string memory _projectId,uint _vestingIndex,string memory _orderId) public returns (bool) {
       ...
       require(getProjectPhaseDetails[_phaseId].phaseStatus == true ,'Phase Is Not Active !');
       require(getProjectDetails[_projectId].projectStatus == true ,'Project Is Not Active !');
       require(getUserPhaseVestingStatus[_orderId].vestingStatus[_vestingIndex]==false ,'Already Claimed !');
       require(getUserPurchaseDetails[_orderId].walletAddress == msg.sender, 'Ha Ha Ha.. System Is Secure !');
       Projects storage project = getProjectDetails[_projectId];
       ProjectPhaseDetails storage projectphasedetail = getProjectPhaseDetails[_phaseId];
       UserPurchased storage userpurchased = getUserPurchaseDetails[_orderId];
       UserPhaseVestingStatus storage userphasevestingstatus = getUserPhaseVestingStatus[_orderId];
       ProjectPhaseVestingDetails storage projectphasevestingdetail = getProjectPhaseVestingDetails[_phaseId];      
       if(userpurchased.totalAvailable>0){
         ...
         {
            userphasevestingstatus.orderId=_orderId;
            userphasevestingstatus.vestingStatus[_vestingIndex]=true;
            getProjectDetails[_projectId].projectContract.transfer(msg.sender, _totalPayableAmount);
