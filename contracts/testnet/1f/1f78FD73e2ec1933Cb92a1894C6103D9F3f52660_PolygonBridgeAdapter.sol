...
/**
 * @title The Polygon Bridge Adapter Contract
 * @author Plug Exchange
 * @notice Implemented polygon bridge contract
 * @dev Plug users can deposit the bridge token into the polygon bridge directly
 */
contract PolygonBridgeAdapter is IBridgeAdapter, AdapterBase {
  /// @dev The Root Chain Manager Address
  address private _rootChainManager;

  /// @dev The ERC20 Predicate for Token Approvals
  address private _erc20Predicate;

  /**
   * @notice Initialization of polygon bridge adapter contract
   * @param _plugRouter The plug router contract address
   * @param rootChainManager_ The Root Chain Manager Address
   * @param erc20Predicate_ The ERC20 predicate for token approvals
   */
  constructor(
    address _plugRouter,
    address rootChainManager_,
    address erc20Predicate_
  ) AdapterBase(_plugRouter) {
    _rootChainManager = rootChainManager_;
    _erc20Predicate = erc20Predicate_;
  }

  /**
   * @notice Sets root chain manager contract address
   * @dev Called by only owner
   * @param rootChainManager_ The root chain manager contract address
   */
  function setRootChainManager(address rootChainManager_) external onlyOwner {
    _rootChainManager = rootChainManager_;
  }

  /**
   * @notice Sets ERC20 predicate contract address for token approvals
   * @dev Called by only owner
   * @param erc20Predicate_ The ERC20 predicate contract address for token approvals
   */
  function setERC20Predicate(address erc20Predicate_) external onlyOwner {
    _erc20Predicate = erc20Predicate_;
  }
  ...
  function deposit(
    ...
    (address _rootToken, uint256 chainId) = abi.decode(data, (address, uint256));

    bool success;
    bytes memory result;
    toChainId = chainId;
    if (token == NATIVE_TOKEN_ADDRESS) {
      (success, result) = _rootChainManager.call{value: msg.value}(
        abi.encodeWithSignature('depositEtherFor(address)', recipient)
      );
    } else {
      _approve(_erc20Predicate, token);
      (success, result) = _rootChainManager.call(
        abi.encodeWithSignature('depositFor(address,address,bytes)', recipient, _rootToken, abi.encodePacked(amount))
      );
    }
