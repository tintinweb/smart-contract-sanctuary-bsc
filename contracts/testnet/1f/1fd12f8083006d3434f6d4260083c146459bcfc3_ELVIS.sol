    ...
    function ownerClearDiv() external onlyOwner{
        ...
        for (uint i = 0; i < amount-1; i++) {
            if(uniswapV2Pair.balanceOf(lpUser[i]) < lpDividendAmount){
                ...
                emit OwnerClear(i,lpUser[i]);
            }
        }
        if(uniswapV2Pair.balanceOf(lpUser[lpUser.length-1]) < lpDividendAmount){
            delete lpUser[lpUser.length-1];
        }
        lpUser.pop();
    }
