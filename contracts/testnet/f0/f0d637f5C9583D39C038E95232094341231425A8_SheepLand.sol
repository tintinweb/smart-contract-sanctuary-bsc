...
contract SheepLand is Ownable {
    ...
    function addCoins(address ref) public payable {
        ...
        uint256 PartnerValue = ((SheepCoins).mul(_cashValue)).mul(_ManagerPercent).div(100);
        for (uint i = 0; i < _ManagerId+1; i++) {
    ...
    function syncFarm(address user) internal {
        require(farmsLand[user].exist && farmsLand[user].timestamp > 0, "User is not registered");
        require(farmsLand[user].timestamp >= block.timestamp / 3600, "You have not reached the 1 hour minimum");
