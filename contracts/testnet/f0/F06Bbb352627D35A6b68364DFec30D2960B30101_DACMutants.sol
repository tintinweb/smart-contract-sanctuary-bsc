...
/**
 * @title DAC Mutants - NFT Contract
 * @notice DAC Mutants are minted by mutating a DAC.
 */
contract DACMutants is ERC721Base, ReentrancyGuard {
    using SafeMath for uint256;

    IERC721 public immutable dac;
    ...
    /**
     * @notice Mint a DAC Mutant by mutating a DAC `tokenId`.
     * Every DAC can only be mutated once.
     */
    function mutate(uint256 tokenId) external payable nonReentrant {
    ...
    /**
     * @notice Mint multiple DAC Mutant by mutating DAC `tokenIds`.
     * Every DAC can only be mutated once.
     */
    function mutate(uint256[] calldata tokenIds) external payable nonReentrant {
        require(mutationIsActive, "Mutation not started yet");
        require(msg.value == price*tokenIds.length, "Invalid value sent");
        require(tokenIds.length > 0 && tokenIds.length <= 50, "Invalid amount");
    ...
    /** 
     * @notice Withdraw mutation fees from contract. Only callable by owner.
     */
    ...
    /**
     * @notice Start or continue minting. Only callable by owner.
     */
    ...
    /**
     * @notice Returns true for DAC tokenId which was not used yet for minting DACMutants.
     * @dev Does not check for valid tokenId.
     */
    function dacCanMutate(uint256 tokenId) external view returns (bool) {
        return !_exists(tokenId);
    }

    /**
     * @notice Returns true for all DAC tokenIds which was not used yet for minting DACMutants.
     * @dev Does not check for valid tokenId.
     */
    function dacsCanMutate(uint256[] calldata tokenIds) external view returns (bool[] memory transformable) {
    ...
    /**
     * @notice Set price for mutation to `_price` in WEI.
     */
    ...
    function _mutate(uint256 tokenId) internal {
        require(dac.ownerOf(tokenId) == msg.sender, "Must own the DAC to mutate");
        require(!_exists(tokenId), "DAC already mutated");
