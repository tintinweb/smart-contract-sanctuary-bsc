...
contract ImplementationV1 is Implementation{
    ...
    * iaoId
     ...
     * ** Params **
     * data (0) tokenAmount, (1) revenue
     * receiver receiver
     * ids (0) signature, (1) iaoId, (2) internalTxId
     */
...
contract Implementation is Upgradeable{
    ...
    function getFNFTOwned(
        ...
        for(uint256 i = 0; i < leng; i ++){
            if(!isDeactivated[fnftLists[0].fnftList[i]] && IERC20(fnftLists[0].fnftList[i]).balanceOf(addr) > 0){
...
contract Upgradeable is Ownable, ReentrancyGuard, IERC721Receiver, ERC1155Receiver{
    ...
    mapping(bytes => bool) fractorClaimed;
    ...
    event CloseIAOEvent(bytes iaoId);
    event FundReturnedEvent(bytes iaoId, address receiver, uint256 amount);
