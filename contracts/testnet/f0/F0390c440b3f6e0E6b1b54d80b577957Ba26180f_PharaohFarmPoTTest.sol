...
contract PharaohFarmPoTTest is Ownable {
    ...
    function changeSetDays(uint256 newSetDays) public onlyOwner {
        setDays += ( newSetDays * 1 days) ;
    }

    function changeStartTime(uint256 startTimeNew) public onlyOwner {
        startTime = startTime - startTimeNew * 1 days;
    }
    ...
    function availableRewards(address userAddress) public view returns (uint256) {
        ...
        uint256 time = (block.timestamp + setDays - deposit_time) / 1 days;
    ...
    function claimRewards() public {
        uint256 myRew = availableRewards(msg.sender);
        ...
        uint256 today = dailyPayoutRecords[day].amount;

        require(myRew >= 0, "Can't claim if nothing is available");
        require(
            block.timestamp + setDays - lastClaim / 1 days >= 1 days,
        ...
        if (myRew > myPayout) {
            uint256 toClaim = myPayout -
                ((myPayout * (withdrawTax + 10)) / 100) -
                claimed;
            require(
                calculatePayout() > today + toClaim,
                " Daily Payout Supply Ended"
            );
        ...
        } else if (myRew + claimed > myPayout) {
            uint256 toClaim = (myPayout - claimed) -
                (((myPayout - claimed) * (withdrawTax + 10)) / 100);
            require(
                calculatePayout() > today + toClaim,
                " Daily Payout Supply Ended"
            );
            ...
            uint256 toClaim =( myRew - claimed) - (myRew * (withdrawTax + 10) / 100);
            require(
                calculatePayout() > today + toClaim,
                " Daily Payout Supply Ended"
            );
    ...
    function doubleUp() public {
        ...
        uint256 totalStakingDays = block.timestamp +
            setDays -
            startDate /
            1 days;
        require(
    ...
    function _reinvestRefer(address userAddress, uint256 amount) internal {
