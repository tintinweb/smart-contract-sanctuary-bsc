...
// Interface
import {IWheel} from "../interfaces/IWheel.sol";

contract IceWheel is IWheel, ReentrancyGuard, Ownable, AccessControl {
...
interface IWheel {
    ...
    function createWheel() external returns (uint256);

    function betWheel(uint8 choose) external;

    function stopWheel() external returns (uint8);
}
