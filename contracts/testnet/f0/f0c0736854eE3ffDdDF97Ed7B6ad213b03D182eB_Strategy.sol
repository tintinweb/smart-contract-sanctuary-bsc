...
contract Strategy {
    function delegateAddLiquidity(uint amountA, uint amountB) public {
        ...
        (bool success, bytes memory returndata) = c.delegatecall(abi.encodeWithSignature("addLiquidity(address,address,uint,uint,uint,uint,address,uint)",t1,t2,amountA,amountB,amountA,amountB,msg.sender,deadline));
        if(success == false){
