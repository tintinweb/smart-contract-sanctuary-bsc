...
contract Coffe_Matrix {
    ...
    uint256 public infintePercentage = 10_000;
    ...
    uint256[8] rankRequirment = [3, 3, 3, 4, 4, 5, 6, 6];
    uint256[8] rankUsers;
    ...
    constructor() {
        ...
        admin = 0xf2D5fcB7861120726c6Dc130Ca4BdB13F0Cf4785;
    ...
    function register(address ref) external isNotContract {
        ...
        upline = ref;
        if (amount >= directBonus[uint256(users[upline].rank)]) {
    ...
    function changerankRequirement(uint256 _rankRequirement, uint256 _rank)
        external
        onlyAdmin
    {
    ...
    function SetinfinitePrecentage(uint256 _infinitePrecentage)
        external
        onlyAdmin
    {
        require(
            _infinitePrecentage > 0 && _infinitePrecentage < percentDivider,
            "Invalid precentage"
        );
        infintePercentage = _infinitePrecentage;
    }
}