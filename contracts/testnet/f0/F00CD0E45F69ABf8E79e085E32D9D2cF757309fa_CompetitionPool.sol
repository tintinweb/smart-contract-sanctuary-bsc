    ...
    ICompetitionPool
{
    ...
    mapping(bytes32 => bool) public hashedMessageToBool;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _paymentTokenAddresses.length; i++) {
            ...
            paymentToken[_paymentTokenAddress] = PaymentToken(
    ...
    function setAirdropToken(address _airdropToken) external onlyOwner {
        require(_airdropToken != address(0), "CP03");
        airdropToken = _airdropToken;
        emit UpdateAirdropToken(_airdropToken);
    }
    ...
    function updatePaymentToken(
        ...
        PaymentToken storage _paymentToken = paymentToken[_tokenAddress];
        require(_paymentToken.tokenAddress != address(0), "CP04");
        _paymentToken.tokenAddress = _tokenAddress;
    ...
    function acceptP2P(address _p2pCompetition)
        ...
        nonReentrant
    {
        address _paymentTokenAddress = ICompetitionContract(_p2pCompetition)
            .tokenAddress();
    ...
    function createNewRegularCompetitionMulti(
        IRegularCompetitionContractMultiMatches.CompetitionMulti
            memory _competitionInfo,
        ICompetitionFactory.Time memory _time,
        uint256 _entryFee,
        uint256 _minEntrant,
        IRegularCompetitionContractMultiMatches.BetOption[][]
            memory _betOptions,
        address _paymentTokenAddress
    ) external nonReentrant {
        PaymentToken memory _paymentToken = paymentToken[_paymentTokenAddress];
        require(_paymentToken.isActive, "CP04");
        address competitionContract = competitionFactory
            .createRegularCompetitionContractMulti(
                msg.sender,
                _competitionInfo,
                _time,
                _entryFee,
        ...
        pools.push(competitionContract);
        existed[competitionContract] = Pool(Type.RegularMulti, true);
        creator[competitionContract] = msg.sender;
        uint256 fee = getFee(_paymentTokenAddress);
        emit CreatedNewRegularCompetition(msg.sender, competitionContract, fee);
        IERC20(_paymentTokenAddress).safeTransferFrom(
    ...
    function createNewGuaranteedCompetitionMulti(
        IRegularCompetitionContractMultiMatches.CompetitionMulti
            memory _competitionInfo,
        ICompetitionFactory.Time memory _time,
        uint256 _entryFee,
        ...
        require(_paymentToken.isActive, "CP04");
        address competitionContract = competitionFactory
            .createNewGuaranteedCompetitionContractMulti(
                msg.sender,
                _competitionInfo,
                _time,
                _entryFee,
        ...
        pools.push(competitionContract);
        existed[competitionContract] = Pool(Type.GuaranteeMulti, true);
        creator[competitionContract] = msg.sender;
        uint256 fee = getFee(_paymentTokenAddress);
        IERC20(_paymentTokenAddress).safeTransferFrom(
            msg.sender,
        ...
        returns (bool)
    {
        ...
        if (
            ...
            !IChainLinkOracleSportDataMultiMatches(betting.oracle())
                .checkFulfill(_resultId)
        ) return true; //refund
    ...
    function getFee(address _tokenAddress)
        public
        view
        override
        returns (uint256)
    ...
    function getPaymentTokenAddresses()
        ...
        view
        override
        returns (PaymentToken[] memory _paymentTokenList, uint256 _size)
    {
        ...
        for (uint256 i = 0; i < paymentTokenAddresses.length; i++) {
            ...
            PaymentToken memory _paymentToken = paymentToken[
                paymentTokenAddress
            ];
    ...
    function _placeBet(
        ...
        address _paymentTokenAddress = ICompetitionContract(
            _competitionContract
        ).tokenAddress();
        IERC20(_paymentTokenAddress).safeTransferFrom(
            _user,
    ...
    function setAdminAddress(address _adminAddress) external onlyOwner {
        require(_adminAddress != address(0), "CP03");
        emit UpdateAdmin(adminAddress, _adminAddress);
        adminAddress = _adminAddress;
    }

    // $"{userAddress}{campaignId}{chainNetworkId}{airdropValue}"
    function validateSignature(
        MessageSignature calldata _adminSignature,
        string memory _campaignId,
        uint256 _chainNetworkId,
        ...
        require(signer == owner() || signer == adminAddress, "CP05");
        return hashedMessage;
    }

    function placeBetWithAirdropReward(
        address _competitionContract,
        uint256[] memory betIndexs,
        MessageSignature calldata _adminSignature,
        string memory _campaignId,
        uint256 _chainNetworkId,
        uint256 _airdropValue
    ) external onlyExistedPool(_competitionContract) nonReentrant {
        bytes32 hashedMessage = validateSignature(
            _adminSignature,
            _campaignId,
            _chainNetworkId,
            _airdropValue
        );
        require(!hashedMessageToBool[hashedMessage], "CP06");
        ...
        IERC20(airdropToken).safeTransfer(
            _competitionContract,
            competitionContract.getEntryFee()
        );
        competitionContract.placeBet(_user, betIndexs);

        hashedMessageToBool[hashedMessage] = true;
    }
