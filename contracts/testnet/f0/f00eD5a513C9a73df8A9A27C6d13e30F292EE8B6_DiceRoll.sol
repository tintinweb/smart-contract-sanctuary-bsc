...
{
    address constant priceFeed = 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526;
    ...
    function getUserBets(address user, uint256 start, uint256 end) public view returns (Bet[] memory) {
    ...
    function getLastUserBets(address user, uint256 quantity) public view returns (Bet[] memory) {
      uint256[] memory userBets_ = userBets[user];
      uint256 start = quantity > userBets_.length ? 0 : userBets_.length - quantity;
      Bet[] memory bets_ = new Bet[](start + quantity);
      for (uint256 i = start; i < start + quantity; i++) {
        bets_[i] = (bets[userBets_[i]]);
      }
      return bets_;
    }
