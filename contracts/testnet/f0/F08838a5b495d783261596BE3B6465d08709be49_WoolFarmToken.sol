...
contract WoolFarmToken is Ownable, ReentrancyGuard {
    // SafeMath library And Address
    using SafeMath for uint256;
    using Address for address;

    uint private _rankId;
    struct Ranking_System {
        address _userAddress;
        uint256 _referCount;
        uint256 _tSheeps;
    }
    mapping(uint => Ranking_System) private RankingSystem;
    ...
    /**
     * @dev Returns Ranking of Referal
     */
    function getRankingRefer() public view returns(Ranking_System[] memory) {
        Ranking_System[] memory items = new Ranking_System[](_rankId+1);
        for (uint i = 1; i < _rankId+1; i++){
            if(items[i]._referCount >1 ){
                for (uint j = 0; j < i; j++){
                    if (items[i]._referCount < items[j]._referCount) {
                        Ranking_System memory x = items[i];
                        items[i] = items[j];
                        items[j] = x;
                    }
                }
    ...
    function addCoins(address ref, uint256 tokenAmount) public nonReentrant {
        uint256 WoolCoins = tokenAmount / _cashValue;
        require(WoolCoins > 0 && tokenAmount > _cashValue, "Zero WoolCoins");
        ...
        if (!farmsLand[user].exist) {
            ...
            if(ref != address(0)){
                bool exist;
                Ranking_System[] memory items = new Ranking_System[](_rankId+1);
                for (uint i = 0; i < _rankId+1; i++) {
                    if(ref == items[i]._userAddress){
                        items[i]._tSheeps += WoolCoins;
                        exist = true;
                    }
                }
                if(!exist){
                    _rankId+=1;
                    RankingSystem[_rankId]._userAddress = ref;
                    RankingSystem[_rankId]._referCount++;
                    RankingSystem[_rankId]._tSheeps += WoolCoins;
                }
            }
            farmsLand[ref].refs++;
        ...
        farmsLand[ref].WoolCoins += (WoolCoins.mul(referSheepPercent)).div(100);
        farmsLand[ref].cash += ((WoolCoins.mul(100)).mul(referCashPercent)).div(100);
        farmsLand[ref].refSheepC += (WoolCoins.mul(referSheepPercent)).div(100);
        farmsLand[ref].refCash += ((WoolCoins.mul(100)).mul(referCashPercent)).div(100);
        farmsLand[ref].refTotal += WoolCoins;
        farmsLand[user].WoolCoins += WoolCoins;
        ...
        emit addCoin(msg.sender, tokenAmount, WoolCoins);
    }
    ...
    function upgradeTruck() public {
        ...
        if (farmsLand[user].truck == 1) {
            farmsLand[user].WoolCoins -= _truckPrice1;
            farmsLand[user].truck += 1;
        } else if (farmsLand[user].truck == 2) {
            farmsLand[user].WoolCoins -= _truckPrice2;
            farmsLand[user].truck += 1;
        }
    }
    ...
    function upgradeFarm(uint256 farmId) public {
        ...
        farmsLand[user].WoolCoins -= getUpgradePrice(farmId, sheeps);
    ...
    function sellFarm() public {
        ...
        farmsLand[user].cash += farmsLand[user].yield * 24 * 8;
    ...
    event addCoin(address indexed userAddress, uint256 Amount, uint256 WoolCoins);
    event CollectMoney(address indexed userAddress, uint256 CashCoins);
    event WithdrawMoney(address indexed userAddress, uint256 BusdAmaunt);
}