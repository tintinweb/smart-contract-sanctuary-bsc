...
contract LgoDutchAuctionLens {
    function getAuctions(ILgoDutchAuctionFactory _auctionFactory)
        public
        view
    ...
    function getAuctionInfo(ILgoDutchAuction _dutchAuction)
        public
        view
        returns (LgoAuctionInfo memory)
    {
        LgoAuctionInfo memory info;
    ...
    function getAuctionInfos(
        ILgoDutchAuctionFactory _auctionFactory,
        uint256 _skip,
        uint256 _take
    )
        external
        view
        returns (LgoAuctionInfo[] memory auctionInfos)
    {
        ...
        auctionInfos = new LgoAuctionInfo[](_take);
        for (uint256 i = 0; i < _take; i++) {
            LgoAuctionInfo memory dutchAuction =
                getAuctionInfo(ILgoDutchAuction(auctions[_skip + i]));
            auctionInfos[i] = dutchAuction;
        }
    }

    function getOnGoingAuctionInfos(ILgoDutchAuctionFactory _auctionFactory)
        external
        view
        returns (LgoAuctionInfo[] memory auctionInfos)
    {
        (address[] memory auctions, uint256 totalAuctions) =
            getAuctions(_auctionFactory);
        auctionInfos = new LgoAuctionInfo[](totalAuctions);
        uint256 onGoingIndex = 0;
        for (uint256 i = 0; i < totalAuctions; i++) {
            LgoAuctionInfo memory dutchAuction =
                getAuctionInfo(ILgoDutchAuction(auctions[i]));
            if (
                block.timestamp
    ...
    function getUpCommingAuctionInfos(ILgoDutchAuctionFactory _auctionFactory)
        external
        view
        returns (LgoAuctionInfo[] memory auctionInfos)
    {
