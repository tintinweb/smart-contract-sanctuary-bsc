...
contract Lepricon_Staking is DSMath, Ownable, ReentrancyGuard, Pausable {
     ...
     */
    uint256 public APR_NOLOCK = wdiv(1370, 10000000); // = 5% APR == 0.05/year == 0.0001370/day
    uint256 private constant TIMELOCK_3MONTHS = 91 days;
    uint256 public APR_3MONTHS = wdiv(2129, 10000000); // = 7.77% APR == 0.0777/year == 0.0002129/day
    uint256 private constant TIMELOCK_6MONTHS = 182 days;
    uint256 public APR_6MONTHS = wdiv(3288, 10000000); // = 12% APR == 0.12/year == 0.0003288/day
    uint256 private constant TIMELOCK_12MONTHS = 364 days;
    uint256 public APR_12MONTHS = wdiv(5480, 10000000); // = 20% APR == 0.2/year == 0.0005480/day
    ...
    function _stake(
        ...
        if (_isNFT && boost[msg.sender].isBoost == false) {
        ...
        if (_current_stake.timeLock == 0) {
            if (_NftBoost != 0) {
                if (boost[_current_stake.user].since > _current_stake.since) {
                    uint256 extraBoost = wdiv(_convertBoostPercent(_NftBoost), 10000000);
    ...
    function _setNftStatus(
        ...
        else if (boost[_account].isBoost == true) {
