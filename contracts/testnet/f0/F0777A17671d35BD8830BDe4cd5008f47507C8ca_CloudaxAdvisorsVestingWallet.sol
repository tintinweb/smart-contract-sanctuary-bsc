...
contract CloudaxAdvisorsVestingWallet is
    Ownable,
    Initializable,
    ReentrancyGuard,
    Pausable
{
     ...
     * @param beneficiary_ address of the beneficiary.
     */
    function initialize(address beneficiary_) external initializer onlyOwner {
        _startTime = block.timestamp.add(_CLIFF_PEROID);
        uint256 userAllocation = _token.totalSupply().mul(4).div(100);
        uint256 RELEASE_AMOUNT_UNIT = userAllocation.div(100);
        _setBeneficiaryAddress(beneficiary_);
