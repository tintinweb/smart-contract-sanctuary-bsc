...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    function transferFrom(
        ...
        require(currentAllowance >= amount, "OxT: transfer amount exceeds allowance");
        unchecked {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "OxT: decreased allowance below zero");
        unchecked {
    ...
    function _transfer(
        ...
        require(sender != address(0), "OxT: transfer from the zero address");
        require(recipient != address(0), "OxT: transfer to the zero address");
        _beforeTokenTransfer(sender, recipient, amount);
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "OxT: transfer amount exceeds balance");
        unchecked {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "OxT: mint to the zero address");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "OxT: burn from the zero address");
        _beforeTokenTransfer(account, address(0), amount);
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "OxT: burn amount exceeds balance");
        unchecked {
    ...
    function _approve(
        ...
        require(owner != address(0), "OxT: approve from the zero address");
        require(spender != address(0), "OxT: approve to the zero address");
...
abstract contract ERC20BurnableUpgradeable is Initializable, ContextUpgradeable, ERC20Upgradeable {
    ...
    function burnFrom(address account, uint256 amount) public virtual {
        ...
        require(currentAllowance >= amount, "OxT: burn amount exceeds allowance");
        unchecked {
...
contract OxT is Initializable, ERC20Upgradeable,ERC20BurnableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    using SafeMathUpgradeable for uint256;
    using AddressUpgradeable for address;
    struct Stake {
        uint256 id;
    ...
    function initialize() initializer public {
        __ERC20_init("OxT", "OxT");
        __ERC20Burnable_init();
        __Ownable_init();
        __UUPSUpgradeable_init();
        _ownerAddress = msg.sender;
        _setLiquidityAddress(owner());
        _setTaxRewardAddress(0x8833F047149e9173ce477Fee93813F9b94c1253C);
        _setBurnFee(10);
        _setLiqFee(10);
        _setTaxFee(10);
        _basicRate = 11;
    ...
    function setMinimumAmountForStake(uint256 minimumAmount) public virtual onlyOwner returns(bool){
        require(minimumAmount > 0 && minimumAmount < 100 , "OxT : Minimum should be between 1 to 99");
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        require(balanceOf(_msgSender()).sub(stakeOf(_msgSender())) >= amount,"OxT : cant transfer more than Stake");
        return super.transfer(recipient,amount);
    }
    function stake(uint256 amount) public virtual returns(uint256) {
        address sender = msg.sender;
        require(amount>=(minimumAmountForStake().mul(10**decimals())),"OxT : minimum amount for staking isn't Meet");
        require(balanceOf(sender)!=0,"OxT : stake address balance not enough for staking");
        require(balanceOf(sender).sub(stakeOf(sender)) >= amount,"OxT : cant transfer more than Stake");
    ...
    function InterestPay(address stakeAddress) public virtual onlyOwner returns (bool){
        require(balanceOf(stakeAddress)!=0,"OxT: address balance not enough for InterestPayment");
        require(stakeOf(stakeAddress)!=0,"OxT: address stake balance not enough for InterestPayment");
