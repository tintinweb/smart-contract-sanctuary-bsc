...
contract StakingPool is IStakingPool, Ownable {
	...
	constructor(address _rewardVNS, address _rewardCPD, address _stakedToken, StakeType _stakeType) {
		VNS = _rewardVNS;
		CPD = _rewardCPD;
	...
	function sweepToken(IERC20 token) external onlyOwner {
		...
		if (address(token) == STAKED_TOKEN) sweepAmount -= totalStaked;
	...
	function stakeTo(uint256 poolId, uint256 amount, address to) public override needPoolOpen {
	...
	function pending(
		uint256 poolId,
		address _user
	) public view returns (uint256 pendingVNS, uint256 pendingCPD, bool includDividendVns) {
