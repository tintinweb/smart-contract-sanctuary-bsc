...
abstract contract ReentrancyGuard {
    ...
    // ocean's contents, replace the bits taken up by the boolean, and then write
...
contract Treasury is ContractGuard {
    ...
    address[] public excludedFromTotalSupply = [
        address(0x2D4836c8a2bdC005EC203892470c31195b8d1de2), // WhaleGenesisPool
        address(0xB338996F02532c7ac596a9D031a88b4Ed7FD508f) // new WhaleRewardPool
    ];
    ...
    function getBurnableWhaleLeft() public view returns (uint256 _burnableWhaleLeft) {
        ...
        if (_whalePrice <= whalePriceOne) {
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnableWhale = _maxMintableBond.mul(_whalePrice).div(1e17);
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_whalePrice > whalePriceCeiling) {
            ...
            if (_rate > 0) {
                _redeemableBonds = _totalWhale.mul(1e17).div(_rate);
            }
        }
    }
    ...
    function initialize(
        ...
        whalePriceOne = 10**17; // This to allow a PEG of 10 WHALE per AVAX
        ...
        maxSupplyContractionPercent = 300; // Upto 3.0% supply for contraction (to burn WHALE and mint tBOND)
    ...
    function buyBonds(uint256 _whaleAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _bondAmount = _whaleAmount.mul(_rate).div(1e17);
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        uint256 _whaleAmount = _bondAmount.mul(_rate).div(1e17);
    ...
    function _sendToMasonry(uint256 _amount) internal {
        ...
        IERC20(whale).safeApprove(masonry, 0);
        IERC20(whale).safeApprove(masonry, _amount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            _sendToMasonry(whaleSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else {
            if (previousEpochWhalePrice > whalePriceCeiling) {
                // Expansion ($WHALE Price > 1 $FTM): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(anchor).totalSupply();
                uint256 _percentage = previousEpochWhalePrice.sub(whalePriceOne);
                uint256 _savedForBond;
                uint256 _savedForMasonry;
                uint256 _mse = _calculateMaxSupplyExpansionPercent(whaleSupply).mul(1e14);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
                if (seigniorageSaved >= bondSupply.mul(bondDepletionFloorPercent).div(10000)) {
                    // saved enough to pay debt, mint as usual rate
                    _savedForMasonry = whaleSupply.mul(_percentage).div(1e17);
                } else {
                    // have not saved enough to pay debt, mint more
                    uint256 _seigniorage = whaleSupply.mul(_percentage).div(1e17);
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(anchor), "anchor");
