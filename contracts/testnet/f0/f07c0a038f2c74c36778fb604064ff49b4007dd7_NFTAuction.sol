...
contract NFTAuction {
    ...
    modifier onlyApplicableBuyerBatch(
        address _nftContractAddress,
    ...
    function makeBatchBid(
        ...
        external
        payable
        batchAuctionOngoing(_nftContractAddress, _batchTokenIds)
        onlyApplicableBuyerBatch(_nftContractAddress, _batchTokenIds)
    {
        
        uint256 total_pay = 0;
         
        for (uint256 i = 0; i < _batchTokenIds.length; i++) {
            uint256 buyNowPrice = nftContractAuctions[_nftContractAddress][_batchTokenIds[i]]
            .buyNowPrice;
                total_pay += buyNowPrice;
        }
        require(
                msg.value > 0 &&
                (msg.value >= total_pay ) , 'Not enough funds to bid on NFT'
            );
        
        for (uint256 i = 0; i < _batchTokenIds.length; i++) {
            uint256 buyNowPrice = nftContractAuctions[_nftContractAddress][_batchTokenIds[i]]
            .buyNowPrice;
            _makeBatchBid(_nftContractAddress, _batchTokenIds[i], _erc20Token, buyNowPrice);
        }
    }
    ...
    function _payFeesAndSeller(
        ...
        for (
            ...
            uint256 removesellerper = _highestBid -  (_highestBid * (nftContractAuctions[_nftContractAddress][_tokenId]
                    .feePercentages[i])) / 10000 ;
            uint256 fee = _getPortionOfBid(
                removesellerper,
                nftContractAuctions[_nftContractAddress][_tokenId]
                    .feePercentages[i]
            );
            uint256 total_fees = fee + removesellerper;
            feesPaid = feesPaid + fee + removesellerper;
