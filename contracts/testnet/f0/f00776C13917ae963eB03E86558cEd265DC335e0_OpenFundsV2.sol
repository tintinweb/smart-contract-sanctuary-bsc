...
contract OpenFundsV2 is Context, Ownable , ReentrancyGuard {
    ...
    function deposit(uint256 plan , uint256 _amount) public noReentrant  {
        ...
        if (plan == 1) {
            ...
            investPlan1[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 2 minutes,30);
        } else if (plan == 2) {
            ...
            uint256 userCurrentInvestment = _amount;
            uint256 totalInvestment = SafeMath.add(userLastInvestment,userCurrentInvestment);
            investPlan2[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 2 minutes,50);
        } else if (plan == 3) {
            ...
            uint256 userCurrentInvestment = _amount;
            uint256 totalInvestment = SafeMath.add(userLastInvestment,userCurrentInvestment);
            investPlan3[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 2 minutes,80);
        } else {
    ...
    function userReward(address _userAddress, uint256 plan) public view returns(uint256) {
        ...
        uint256 userWeellyReturn = SafeMath.mul(DailyRoi(userInvestment,roi),7);

        // invested time
        uint256 totalDeadline = deadline - lastWithdraw;
        uint256 stakeTime = nowTime - lastWithdraw;
        if(deadline>= nowTime) {
            uint256 totalEarned = SafeMath.div(SafeMath.mul(stakeTime, userWeellyReturn),totalDeadline);
            return totalEarned;
        }
        else {
          
            return userWeellyReturn;
        }
    }
    ...
    function withdrawal(uint256 plan) public noReentrant {
        ...
        uint256 userWeellyReturn = SafeMath.mul(DailyRoi(totalInvestment,roi),7);
        uint256 wFee = withdrawFee(userWeellyReturn);

        BusdInterface.transfer(msg.sender,1 ether);
        BusdInterface.transfer(dev,wFee);
        

        if(plan == 1) {
            investPlan1[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 2 minutes,30);
            totalWithdrawPlan1[msg.sender] = userTotalWithdraw(msg.sender,lastTotalWithdraw + 1 ether);
        } else if (plan == 2) {
            totalWithdrawPlan2[msg.sender] = userTotalWithdraw(msg.sender,lastTotalWithdraw + 1 ether);
            investPlan2[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 2 minutes,50);
        } else if (plan == 3) {
            totalWithdrawPlan3[msg.sender] = userTotalWithdraw(msg.sender,lastTotalWithdraw + 1 ether);  
            investPlan3[msg.sender] = user_investment_details(msg.sender,totalInvestment,block.timestamp,block.timestamp + 2 minutes,80);
        } else {
            require(false,'Wrong plan!');
        }
    ...
    function DailyRoi(uint256 _amount, uint256 _roi) public view returns(uint256) {
        return SafeMath.div(SafeMath.mul(_amount,_roi),100);
    }
