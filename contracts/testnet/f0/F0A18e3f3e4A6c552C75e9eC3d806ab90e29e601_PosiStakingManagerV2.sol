...
abstract contract VestingScheduleBase is ReentrancyGuard {
    ...
    function getVestingSchedules(address user, VestingFrequencyHelper.Frequency freq) public view returns (VestingData[] memory) {
        return vestingSchedule[user][freq];
    }

    function claimVesting(VestingFrequencyHelper.Frequency freq, uint256 index) public nonReentrant {
        bool success = _claimVesting(msg.sender, freq, index);
        require(success, "claimVesting: failed");
    }

    function claimVestingBatch(VestingFrequencyHelper.Frequency[] memory freqs, uint256[] memory index) public nonReentrant {
