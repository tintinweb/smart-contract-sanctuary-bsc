    ...
    IExenoFinanceNode
{
        ...
        internal
    {
        ...
        if (params.length > 0) {
            ...
            if (action == ACTION_DEBIT_REWARD) {
                ...
                ExenoFinance.validateAndEmitReward(d, !isDebitId(id), id, token, amount);
                debitIds[id] = true;
                ...
                ExenoFinance.validateAndEmitOption(d, token, amount);
            } else {
        ...
        internal
    {
        if (params.length > 0) {
            ...
            if (action == ACTION_CREDIT_REWARD) {
                (address token, uint256 amount) = abi.decode(args, (address, uint256));
                ExenoFinance.validateAndEmitReward(d, token, amount);
                _registerUndeposits(address(this), token, amount);
    ...
    function _processOption(
        ...
        internal
    {   
        ExenoFinance.validateAndEmitOption(d, !isDebitId(d.id));
        debitIds[d.id] = true;
    }
        ...
        internal
    {
        ...
        if (action == ACTION_VOUCHER_CEDE) {
            ...
            ExenoFinance.validateAndEmitVoucherCede(d, !isCreditId(d.id), NETWORK, sender, beneficiary, proof);
        } else if (action == ACTION_VOUCHER_CLAIM) {
            (address beneficiary, bytes memory proof) = abi.decode(args, (address, bytes));
            ExenoFinance.validateAndEmitVoucherClaim(d, !isCreditId(d.id), NETWORK, sender, beneficiary, proof);
        } else {
            revert("EFN: unknown act");
        }
    }
        ...
        internal
    {
        ...
        if (action == ACTION_ESCROW_CEDE) {
            ...
            ExenoFinance.validateAndEmitEscrowCede(d, !isCreditId(d.id), NETWORK, sender, beneficiary);
        } else if (action == ACTION_ESCROW_REVOKE) {
            ExenoFinance.validateAndEmitEscrowRevoke(d, !isCreditId(d.id), NETWORK, sender);
        } else {
            revert("EFN: unknown act");
        }
    }
...
library ExenoFinance {
     ...
     */
    function validateAndEmitOption(
        Data calldata d,
        bool unprocessed
    )
        external
        ...
        external
    {
        ...
        require(d.method == METHOD_COLLATERAL,
            "EFN: method mismatch");
        emitOption(d, token, amount);
    }

    /**
     * Validate pre-conditions for a debit reward
     */
    function validateAndEmitReward(
        Data calldata d,
        bool unprocessed,
        bytes32 id,
        address token,
        ...
        external
    {
        require(amount > 0,
            "EFN: undefined a/t");
        emitCredit(d, token, amount);
    }

    /**
     * Validate pre-conditions for ceding a voucher
     */
    function validateAndEmitVoucherCede(
        Data calldata d,
        bool unprocessed,
        bytes32 network,
        address sender,
        ...
        external
    {
        ...
        require(d.method == METHOD_VOUCHER,
            "EFN: method mismatch");
        emitUpdatePayer(d, beneficiary);
    }

    /**
     * Validate pre-conditions for claiming a voucher
     */
    function validateAndEmitVoucherClaim(
        Data calldata d,
        bool unprocessed,
        bytes32 network,
        address sender,
        ...
        external
    {
        ...
        require(d.method == METHOD_VOUCHER,
            "EFN: method mismatch");
        emitUpdatePayee(d, beneficiary);
    }

    /**
     * Validate pre-conditions for ceding an escrow
     */
    function validateAndEmitEscrowCede(
        Data calldata d,
        bool unprocessed,
        bytes32 network,
        address sender,
     ...
     */
    function validateAndEmitEscrowRevoke(
        Data calldata d,
        bool unprocessed,
        bytes32 network,
        address sender
        ...
        internal
    {
        publish(Event.DEBIT, Data(
            ...
            d.timeout,
            d.mode,
            amount > 0 ? METHOD_REWARD : METHOD_ORACLE,
            d.id
        ));
    }
