...
contract FurCollSurplusPool is BaseContract, IFurCollSurplusPool {
    ...
    address public furAddress;

    IERC20Upgradeable furToken;
    // Collateral surplus claimable by vault owners
    mapping (address => uint) internal balances;

    // --- Events ---
    event CollBalanceUpdated(address indexed _account, uint _newBalance);
    event CollSurplusPoolFURBalanceUpdated(uint _FUR);
    ...
    function claimColl(address _account) external override whenNotPaused {
        ...
        furToken.safeTransfer(_account, claimableColl);

        uint256 FUR = getFUR();
        emit CollSurplusPoolFURBalanceUpdated(FUR);
        emit FURSent(_account, claimableColl);
