    ...
    SignerRole
{
    ...
    mapping(bytes32 => bool) private _mintedHashs;
    ...
    function mint(
        ...
        require(mintCount > 0 && mintCount <= maxMintCount && tokenIds.length == mintCount, "amount is invalid");
        bytes32 mintHash = toEthSignedMessageHash(keccak256(abi.encodePacked(this, tokenIds[0], mintCount, timestamp)));
        require(isSigner(ecrecover(mintHash, v, r, s)), "owner should sign tokenId");
        require(!_mintedHashs[mintHash], "signed hash can be used one time only");
        require(timestamp > block.timestamp.sub(_expiredTime), "token id expired");
        _mintedHashs[mintHash] = true;
        for (uint256 x = 0; x < mintCount; x++) {
            uint256 curTokenId = tokenIds[x];
            _safeMint(owner(), curTokenId);
            _transfer(owner(), msg.sender, curTokenId);
        }
    }

    function withdraw() external onlyOwner {
        require(address(this).balance > 0, "No balance");
        _royaltiesPaymentsAddress.transfer(address(this).balance);
    }
    ...
    function setRoyalitiesInfo(address newRoyaltiesPaymentAddress, uint256 royailiesPercent) external onlyOwner {
