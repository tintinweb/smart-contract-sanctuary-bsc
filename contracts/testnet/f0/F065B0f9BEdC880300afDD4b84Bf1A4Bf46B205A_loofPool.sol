...
contract loofPool is Ownable {
    ...
    address public relationshipAddress;
    ...
    address public defaultReferrerAddress=0x3289cd34715d13c573556128fB89b70D20DA01BD;
    ...
    function newMining(
        ...
        tokenPerBlock = remainingAmount.mul(12).div(1000).div(28800);
        startBlock = _startBlock;
        endBlock = startBlock.add(remainingAmount.div(tokenPerBlock));
        updatePoolLastRewardBlock(_startBlock);
        emit NewMining(msg.sender, _tokenAmount, _startBlock, tokenPerBlock);
    }
    ...
    function deposit(address _lpToken, uint256 _amount,address _referrer) public {
        uint256 pid = getPid(_lpToken);
        PoolInfo storage pool = poolInfos[pid];
        ...
        if (user.amount > 0) {
            ...
            if (pendingAmount > 0) {
                ...
                if(referrer!=address(0)){
                    UserInfo storage referrerUser=userInfo[pid][referrer];
                    uint256 referrerFee;
                    if (referrerUser.amount>=user.amount){
                        referrerFee = pendingAmount.mul(20).div(100);
                    }else{
                        uint256 referrerPendingAmount =
                            referrerUser.amount.mul(pool.accTOKENPerShare).div(1e12).sub(
                                referrerUser.rewardDebt
                            );
                        referrerFee = referrerPendingAmount.mul(20).div(100);
                    }
        ...
        Relationship(relationshipAddress).updateReferralRelationship(msg.sender, _referrer); 
