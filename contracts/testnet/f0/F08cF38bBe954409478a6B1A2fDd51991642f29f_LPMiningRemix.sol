...
contract LPMiningRemix is Ownable {
    ...
    function getLpTokenLen(uint pid_) public view returns(uint) {
        return poolInfo[pid_].lpToken.length;
    }
    //-------------------------------------------------  mining  ------------------------------------------------
    ...
    function deposit(uint256 pid_, address lp_, uint256 amountIn_) public {
        ...
        UserInviInfo storage user_inv = userInviInfo[pid_][msg.sender];
        require(lpInPool[pid_][lp_] && lpPower[pid_][lp_] != 0, "lp not in pool");
    ...
    function addPool(bool lockToTime_, uint lockTime_,
        ...
        for(uint i=0; i<lpToken_.length; i++) {
            lpInPool[_pid][lpToken_[i]] = true;
            lpPower[_pid][lpToken_[i]] = power_[i];
        }
    ...
    function setLpInPool (uint pid_, address[] memory addrList_, uint[] memory power_) public onlyAdmin {
        for (uint i=0; i<addrList_.length; i++) {
            lpInPool[pid_][addrList_[i]] = true;
            lpPower[pid_][addrList_[i]] = power_[i];
            poolInfo[pid_].lpToken.push(addrList_[i]);
        }
    }
