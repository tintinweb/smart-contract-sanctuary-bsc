...
contract BranchV4Random is AccessControlEnumerable {
    // 管理员配置
    ...
    constructor() {
        ...
        _setupRole(BRANCH_ROLE, _msgSender());
    }

    // 获取开始缴纳尾款的天数
    function randomDaysStartFinalPay(uint256 k, bool first) external view onlyRole(BRANCH_ROLE) returns (uint256 days_) {
        if (first) {
            k = 400;
        }
        if (k <= 400) {
    ...
    // 获取可以取款的的天数
    function randomDaysStartWithdraw(uint256 k, uint8 level) external view onlyRole(BRANCH_ROLE) returns (uint256 days_) {
        if (k < 150) {
