    ...
    UUPSUpgradeable
{
    ...
    function createVesting(
        ...
        require(msg.value >= createFee, "Provide fee");
        bytes memory data = abi.encodeWithSelector(
            BXVesting(address(0)).initialize.selector,
            _name,
            _token
        );
        BeaconProxy beconProxy = new BeaconProxy(address(beacon), data);

        BXVesting vesting = BXVesting(address(beconProxy));
        vesting.transferOwnership(msg.sender);
        emit CreateVesting(_token, msg.sender, address(vesting));
    }
