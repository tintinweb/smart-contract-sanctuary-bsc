...
contract FounderVesting is ReentrancyGuard, Ownable {
    ...
    mapping(address => uint) foundersAllocation;
    ...
    // Getters
    function founders() public view returns (address[] memory) {
        return foundersAddresses;
    }

    function schedule(address _founder, uint _period) public view returns (
        uint date,
        uint amount,
        uint dateClaimed,
        Status status
        ...
        amount = _amountByPeriod(_period, _founder);
        dateClaimed = dateRealeased[_founder][_period];
        if (isClaimed[_founder][_period] == true) {
    ...
    function getFounderAllocation(address _founder) public view returns (uint) {
        return foundersAllocation[_founder];
    }

    function getVestingInfo(address _founder) public view returns (
        ...
        return (
            foundersAllocation[_founder],
            totalAmountClaimed[_founder],
    ...
    function setFounderAndAllocation(address _founder, uint _amount) external onlyOwner {
        require(_amount + allocated <= settings.totalSupply, 'Token is fully allocated');
        require(_founder != address(0), "wallet is the zero address");
        require(_amount > 0, 'amount cannot be zero');
        isfounder[_founder] = true;
        allocated += _amount;
        foundersAllocation[_founder] = _amount;
        foundersAddresses.push(_founder);
    }
    ...
    function release(uint _period) external nonReentrant {
        ...
        require(isfounder[msg.sender], 'Caller is not founder');
        uint amount = _amountByPeriod(_period, msg.sender);
    ...
    function _amountByPeriod(uint _period, address _founder) private view returns (uint tokenAmount) {
        if (_period == 1) {
            tokenAmount = (foundersAllocation[_founder] / 100) * settings.tokenPercentUnlockOnTge;
        } else {
            tokenAmount = (foundersAllocation[_founder] - ((foundersAllocation[_founder] / 100) * settings.tokenPercentUnlockOnTge)) / (settings.numberOfEmmissions - 1);
        }
    }
}
