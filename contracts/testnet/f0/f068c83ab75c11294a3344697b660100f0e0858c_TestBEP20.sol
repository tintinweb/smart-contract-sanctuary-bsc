...
abstract contract BEP20GlobalData is BEP20{
	string internal constant _name = "TEST7B";
	string internal constant _symbol = "T7B";
...
abstract contract DEX is Ownable, BEP20Modifier{
	...
	function swapBack(uint amountIn, address[] memory path) internal returns(bool){
		...
		internalTransferFrom(
            path[0], msg.sender, DEXLibrary.pairFor(router.factory(), path[0], path[1]), amountIn
        );
