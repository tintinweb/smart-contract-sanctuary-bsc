...
contract BlockChat is ERC20, ReentrancyGuard {
...
// (Block address) => check the total likes of Chats in block
    ...
    function check_Block_likes(bytes32 Block_ID) 
...
// (Block address) => check the total dislikes of Chats in block
    ...
    function check_Block_dislikes(bytes32 Block_ID) 
...
// () => change "nounce" point to $MSG ERC20 token
    ...
    constructor() ERC20 ("Message (tBSC)", "tMSG") 
...
// * user address will show up as "address(0)" and timestamp of Chat will show "0"
    ...
    {
        require(balanceOf(_msgSender()) >= 99 * 10 ** decimals(), "Need to deposit 99 $MSG.");
        _burn(_msgSender(), 99 * 10 ** decimals());
        MASKED[_msgSender()] += 99;
    }
...
// (Chat ID) => dislike-to-earn
    ...
    function dislike_Chat(bytes32 id) 
    public nonReentrant() 
...
// Contract autoswap BNB for $MSG to msg.sender
    ...
    {
        _mint(_msgSender(), msg.value * 99 * total_MSGs_burnt / total_MSGs_minted);
        total_MSGs_minted += msg.value * 99 * total_MSGs_burnt / total_MSGs_minted;
...
// Powered by BlockChat Limited (2023-01-08)
// Alpha Dev version 1.5 on Binance Smart Chain Testnet