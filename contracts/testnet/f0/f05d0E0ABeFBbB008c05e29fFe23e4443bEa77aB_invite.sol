  ...
  /*重启数据*/
   ...
   /*初始化*/
    ...
    function UpdateEarning(address _address, uint8 _weight) private {
        ...
        (uint256 totalU, uint256 totalE ) = _countAddV(_address, balanceU, balanceE);
    ...
    function Earning() payable public {
        ...
        uint256 balanceU = _getBalance(usdtAddress);
        uint256 balanceE = _getBalance(xtokenAddress);
        (uint256 totalU, uint256 earnU, uint256 totalE, uint256 earnE) = countEarningV(msg.sender, balanceU, balanceE);
...
//?
    ...
    function EarningV(address _address) public view returns (uint256, uint256, uint256, uint256){

        uint256 balanceU = _getBalance(usdtAddress);
        uint256 balanceE = _getBalance(xtokenAddress);
        return countEarningV(_address, balanceU,balanceE);

    }
    function countEarningV (address _address, uint256 _balanceU, uint256 _balanceE) private view returns (uint256, uint256, uint256, uint256){
        uint256 totalU;
        uint256 totalE;
        uint256 earnU;
        uint256 earnE;
        uint256 balanceU = _balanceU;
        uint256 balanceE = _balanceE;
        address countaddress = _address;
        AccountData storage accdata = userReward[countaddress];

        //计算收益
        (uint256 countU, uint256 countE) = _countAddV(countaddress, balanceU, balanceE);
    ...
    function _countAddV (address _address, uint256 _balanceU, uint256 _balanceE) private view returns (uint256, uint256){
