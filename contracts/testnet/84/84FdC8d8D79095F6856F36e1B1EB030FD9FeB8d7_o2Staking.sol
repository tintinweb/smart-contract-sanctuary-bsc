...
interface IStaking{
    ...
    function _stakeTimes() external returns(uint);
    function _claimTimes() external returns(uint);
    function _userNum() external returns(uint);
    function _stakingTotal() external returns(uint);
    function _interestTotal() external returns(uint);
    function _breachRatio() external returns(uint);
    function _stakeCharge() external returns(uint);
    function _maxBuyAmountEveryDay() external returns(uint);
    function _breachOpen() external returns(bool);
    function _o2() external returns(address);
    function _rbl() external returns(address);
    function _recipientRblAddress() external returns(address);
    function getStakes(address account,uint256 index,uint256 offset) external returns(stakeInfo[] memory);
    ...
    function _stakeInfos(uint stakeId) external returns(stakeInfo memory);
    function _userClaimInfos(address account) external returns(claimInfo[] memory);
    function _users(uint userId) external returns(address);
    function _userStakeList(address account) external returns(uint[] memory);
}
...
contract o2Staking is Context, Ownable {
    ...
    function syncData(address token) external onlyOwner{
        ...
        for(uint i=1;i<=_userNum;i++){
            ...
            uint[] memory temp=IStaking(token)._userStakeList(_users[i]);
            for(uint j;j<temp.length;j++){
                _userStakeList[_users[i]].push(temp[j]);
            }
        }
    }
