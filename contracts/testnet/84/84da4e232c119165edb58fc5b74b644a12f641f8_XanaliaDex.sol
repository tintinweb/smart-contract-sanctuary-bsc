 ...
 */
contract XanaliaDex is IXanaliaDex, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	// address public override launchpadAdminAddress;
	...
	function initialize(
		...
		// launchpadAdminAddress = _launchpadAdminAddress;
		processing = false;
	}
	...
	*/
	function mintAndPutOnSale(
		...
		orderId = marketDex.createOrder(_collectionAddress, _paymentToken, msg.sender, newTokenId, _price);
	...
	*/
	function mintAndPutOnAuction(
		...
		auctionId = auctionDex.createAuction(
			_collectionAddress,
			_paymentToken,
			msg.sender,
			newTokenId,
	...
	@param _collectionAddress address of collection that store NFT
	@param _paymentToken token address that is used for transaction
	@param _price price to buy an NFT in launchpad
	@param _tokenURI URI of NFT
	@param _royaltyFee royalty fee to pay to author when sell NFT
	*/
	// function buyLaunchpadNFT(
	// 	address _collectionAddress,
	// 	address _paymentToken,
	// 	uint256 _price,
	// 	string calldata _tokenURI,
	...
	// ) external payable override onlyNotProcessing nonReentrant returns (uint256 newTokenId) {
	// 	IMarketDex marketDex = IMarketDex(marketDexAddress);
	// 	IXanaliaNFT xanaliaNFT = IXanaliaNFT(_collectionAddress);
	// 	require(isUserWhitelisted[msg.sender] == true, "Invalid-user-address");
	// 	require(marketDex.paymentMethod(_paymentToken), "Payment-method-does-not-support");

	// 	address treasuryAddress = marketDex.treasuryAddress();
	// 	uint256 platformFee = marketDex.platformFee();

	// 	if (_paymentToken == address(0)) {
	// 		require(msg.value >= _price, "Payment-value-invalid");
	// 	} else {
	...
	// 	if (platformFee > 0) {
	// 		_totalEarnings = (_price * (BASE_DENOMINATOR - platformFee)) / BASE_DENOMINATOR;
	// 		if (_paymentToken == address(0)) {
	// 			payable(treasuryAddress).sendValue((_price * platformFee) / BASE_DENOMINATOR);
	// 		} else {
	// 			IERC20Upgradeable(_paymentToken).safeTransfer(treasuryAddress, (_price * platformFee) / BASE_DENOMINATOR);
	// 		}
	// 	}

	// 	if (_paymentToken == address(0)) {
	// 		payable(treasuryAddress).sendValue(_totalEarnings);
	// 	} else {
	// 		IERC20Upgradeable(_paymentToken).safeTransfer(launchpadAdminAddress, _totalEarnings);
	// 	}

	// 	newTokenId = xanaliaNFT.create(_tokenURI, _royaltyFee, msg.sender);

	// 	emit LaunchpadCollectibleBought(msg.sender, _paymentToken, _price, newTokenId);
	...
	*/
	...
	function setPlatformFee(uint256 _platformFee) external override onlyOwner {
		IMarketDex(marketDexAddress).setPlatformFee(_platformFee);
		IAuctionDex(auctionDexAddress).setPlatformFee(_platformFee);
	}

	function setPaymentToken(address _token, bool _status) external override onlyOwner {
		IMarketDex(marketDexAddress).setPaymentMethod(_token, _status);
		IAuctionDex(auctionDexAddress).setPaymentMethod(_token, _status);
	}

	function setWhitelistAddress(address[] calldata _user, bool[] calldata _status) external override onlyOwner {
	...
	event LaunchpadCollectibleBought(
...
interface IXanaliaDex {
	...
	// function launchpadAdminAddress() external view returns (address);
