...
contract FarmBooster is Ownable {
    ...
    function removeBoostMultiplier(uint256 _tokenId) external onlyMasterChefV3 {
        (,address user,, ) = getUserPositionInfo(_tokenId);
        ItMap storage itmap = userInfo[user];
        itmap.remove(_tokenId);
    }
    ...
    function getUserPositionInfo(uint256 _tokenId) view internal returns(uint128 liquidity, address user, uint256 pid, uint256 boostMultiplier) {
