...
contract InvestmentsPresale {
    ...
    constructor(address _FactoryAddress, address _DevAddress){
    ...
    function setAddressInfo(address _presaleCreator,address _tokenAddress,address _unsoldTokensDumpAddress) external onlyFactory {
    ...
    function setGeneralInfo(uint256 _totalTokens,uint256 _tokenPriceInWei,uint256 _hardCapInWei,uint256 _softCapInWei,uint256 _maxInvestInWei,
        uint256 _minInvestInWei,uint256 _openTime,uint256 _closeTime) external onlyFactory {
    ...
    function setUniswapInfo(uint256 _uniListingPriceInWei,uint256 _uniLiquidityAddingTime,uint256 _uniLPTokensLockDurationInDays,
        uint256 _uniLiquidityPercentageAllocation) external onlyFactory {
    ...
    function setStringInfo(bytes32 _saleTitle,bytes32 _linkTelegram,bytes32 _linkDiscord,bytes32 _linkTwitter,bytes32 _linkWebsite) external onlyPresaleCreatorOrFactory {
    ...
    function setInfo(address _LiqLockAddress,uint256 _DevFeePercentage,uint256 _MinDevFeeInWei,uint256 _Id) external onlyDev {
        LiqLockAddress = _LiqLockAddress;
        DevFeePercentage = _DevFeePercentage;
        MinDevFeeInWei = _MinDevFeeInWei;
        Id = _Id;
    }
    function setDevFeesExempted(bool _DevFeesExempted)external onlyDev{
        DevFeesExempted = _DevFeesExempted;
    }
    function setOnlyWhitelistedAddressesAllowed(bool _onlyWhitelistedAddressesAllowed)external onlyPresaleCreatorOrFactory{
        onlyWhitelistedAddressesAllowed = _onlyWhitelistedAddressesAllowed;
    }
    function addwhitelistedAddresses(address[] calldata _whitelistedAddresses)external onlyPresaleCreatorOrFactory{
