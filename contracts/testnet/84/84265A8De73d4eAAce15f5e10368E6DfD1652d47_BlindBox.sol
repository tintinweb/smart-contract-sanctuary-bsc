...
contract  BlindBox is  Ownable, ReentrancyGuard  {
    ...
    event randAmount(address indexed buyer,  uint256 indexed amount);
    ...
    constructor() public {
        ...
        _materialIdToPrice[1] = 1 * 1e10;
        _materialIdToPrice[2] = 1 * 1e10;
        _materialIdToPrice[3] = 1 * 1e10;
        _materialIdToPrice[4] = 1 * 1e10;
        _materialIdToPrice[5] = 1 * 1e10;
    }
    ...
    function randNumber(uint256 materialId) public returns (uint256){
        nonce ++;
        uint result = _materialIdBaseAmount[materialId].add(uint(keccak256(abi.encodePacked(now, block.difficulty, msg.sender, nonce)))  %  _materialIdRandomRange[materialId]);
        emit randAmount(msg.sender, result);
        return result;
    }
