...
contract escrow is Backend, OwnableUpgradeable {
    ...
    function adminRelease(uint[] memory _mileStones,address user,BackendSigner memory whitelist) external{
        require (getSigner(whitelist) == signer, '!Signer');
        require (msg.sender == owner(),'!Not A Owner');
        uint amount;
        for(uint i=0;i<_mileStones.length; i++){
        amount = mileStonePaymentAmount[whitelist.senderAddress][whitelist.slotId]
        ...
        CBS.transfer(user, amount * 1 ether);
