...
contract VotingEscrow is IVotingEscrow, ApolloxERC721EnumerableUpgradeable, UUPSUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    uint internal constant WEEK = 1 hours;
    uint public constant MAX_TIME = 1 * 1 * 86400;
    int128 internal constant iMAX_TIME = 1 * 1 * 86400;
    ...
    bool public _canForcedWithdraw;
    uint256 public _minRatio;
    uint256 public _maxRatio;
    uint256 public _penaltyK;
    address public _penaltyAddress;
    ...
    function forceWithdraw(uint tokenId) external whenNotPaused nonReentrant oncePerBlock(msg.sender){
        forceWithdraw(tokenId, 0);
    }
    // Withdraw before expiration, with penalty
    function forceWithdraw(uint tokenId, uint256 unlockAmount) private{
        require(_canForcedWithdraw, "Not supported");
        ...
        require(lockedAmount >= unlockAmount, "Invalid unlock amount");
        if(unlockAmount == 0){
            unlockAmount = lockedAmount;
        }
        uint256 amountLeft = lockedAmount - unlockAmount;
        uint256 timeLeft = locked.end - block.timestamp;
        //calculate penalty = unlockAmount* max(minRatio%,min(maxRatio%,k*lock_duration_left / 4 years * 100%))
        uint256 penalty = unlockAmount * Math.max(_minRatio, Math.min(_maxRatio, _penaltyK * timeLeft / MAX_TIME)) / MULTIPLIER;
        ...
        //transfer
        _lockToken.safeTransfer(msg.sender, unlockAmount - penalty);
        if(_penaltyAddress != address(0) && penalty > 0){
            _lockToken.safeTransfer(_penaltyAddress, penalty); // to penalty account
        }
        uint supplyBefore = _totalLocked;
        _totalLocked = supplyBefore - unlockAmount;
        _sumLockedTime =  _sumLockedTime - timeLeft;
        //emit Event
    ...
    function configForcedWithdraw(bool canForcedWithdraw) external onlyRole(DEFAULT_ADMIN_ROLE){
        bool old = _canForcedWithdraw;
        require(old != canForcedWithdraw, "Same state");
        _canForcedWithdraw = canForcedWithdraw;
        emit CanForceWithdraw(old, _canForcedWithdraw);
    }

    function configMinRatio(uint256 minRatio) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(minRatio <= _maxRatio, "E1");
        _minRatio = minRatio;
    }

    function configMaxRatio(uint256 maxRatio) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(maxRatio < MULTIPLIER, "E1");
        require(maxRatio >= _minRatio, "E2");
        _maxRatio = maxRatio;
    }

    function configPenaltyK(uint256 penaltyK) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(penaltyK <= 10*MULTIPLIER, "E1");
        _penaltyK = penaltyK;
    }

    function configPenaltyAddress(address penaltyAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(penaltyAddress != address(0), "E1");
