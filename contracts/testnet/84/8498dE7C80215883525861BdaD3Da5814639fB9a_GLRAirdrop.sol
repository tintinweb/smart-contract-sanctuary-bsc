...
contract GLRAirdrop is Pausable, ReentrancyGuard, Ownable {
    ...
    function claim(uint256 amount, bytes32[] calldata merkleProof) external whenNotPaused nonReentrant {
        require(isMerkleRootSet, "Claim: Merkle root not set");
        require(amount <= MAXIMUM_AMOUNT_TO_CLAIM, "Claim: Amount too high");
        require(block.timestamp <= endTimestamp, "Claim: Too late to claim");
