...
contract GetdoneFreelanceContract is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    modifier allowEnd(uint256 contractId, address customer) {
        ...
        if (length > 0) {
            for (uint256 i = 0; i < length; i++) {
                ...
                if (milestone.status == MILESTONE_STATUS_NEW) {
                    allow = false;
                    break;
                }
            }
        ...
        allowCreate(contractId, _msgSender())
    {
        ...
        milestones[customer][contractId][milestoneId] = Milestone(contractId, milestoneId, orderId, talent, customer, price, 0, 0, MILESTONE_STATUS_NEW);
