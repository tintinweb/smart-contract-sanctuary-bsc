...
// File: stakin.sol
    ...
    contract StakingRewards is Ownable {
      ...
      constructor(address _stakingToken, address _rewardsToken,uint _rewards,string memory _name) {
        ...
        function withdrawRewards (uint poolId) internal {
        ...
        function getReward(address _user, uint _poolId) public view returns (uint) {
            ...
            amount += depositedAmount * 1 * (aprRate); //rate = 30
            amount = amount / 100;
            amount = amount / 365;
    ...
    StakingRewards[] public stakeContract;

    function create_project (
        address _stakingToken,
        address _rewardsToken,
        uint _rewards,string memory  pairName) external onlyOwner {

    StakingRewards projects = new StakingRewards(
         _stakingToken,_rewardsToken,_rewards, pairName
    );
        stakeContract.push(projects);
    }
}