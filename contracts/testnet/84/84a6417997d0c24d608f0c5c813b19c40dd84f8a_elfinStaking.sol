...
contract elfinStaking is Ownable {
    ...
    event ElfinStaking(address user, uint poolIndex, uint256 amount, uint256 lockDay);
    ...
    function staking(uint poolIndex, uint256 amount, uint lockDay) external payable {
        PoolInfo storage pool = poolInfo[poolIndex];
        require(now >= pool.startTimestamp && now <= pool.endTimestamp,"Not in staking duration");
        require(amount >= pool.minStakingAmount, "Amount is too small");
        require(pool.lockDays[lockDay] == 1  ,"Lock day not allowed");
        ...
        if(pool.stakingToken != IERC20(address(0))) {
            poolInfo[poolIndex].stakingToken.safeTransferFrom(msg.sender,address(this),amount);
        } else {
            require(amount == msg.value, "Amount must equal call value");
        }
        emit ElfinStaking(_msgSender(), poolIndex, amount, lockDay);
    }
    ...
    function withdraw(uint userStakingIndex) external {
        ...
        PoolInfo storage pool = poolInfo[info.poolIndex];
    ...
    function updatePoolLockDays(uint index, uint[] memory lockDayKey, uint[] memory lockDayValue) external onlyCmo {
        PoolInfo storage pool = poolInfo[index];
        require(pool.totalLimit != 0,"Can't set not exist token pool");
        require(lockDayKey.length == lockDayValue.length,"Length not equal");
        for (uint i = 0; i < lockDayKey.length; i++) {
            pool.lockDays[lockDayKey[i]] = lockDayValue[i];
        }
    }


    function setStakingPool(uint index, address _newToken, 
        uint256 _newTotalLimit, uint256 _minStakingAmount,
        uint256 _startTimestamp, uint256 _endTimestamp) external onlyCmo {
        PoolInfo storage pool = poolInfo[index];
        ...
        pool.startTimestamp = _startTimestamp;
        pool.endTimestamp = _endTimestamp;
    }


    function updateStakingPool(uint index, 
        uint256 _newTotalLimit, uint256 _minStakingAmount,
        uint256 _startTimestamp, uint256 _endTimestamp) external onlyCmo {
        PoolInfo storage pool = poolInfo[index];
        require(pool.totalLimit != 0,"Can't update not exist token pool");
        pool.totalLimit = _newTotalLimit;
        pool.minStakingAmount = _minStakingAmount;
        pool.startTimestamp = _startTimestamp;
        pool.endTimestamp = _endTimestamp;
    ...
    function updateDuration(uint _duration) external onlyCmo {
        duration = _duration;
    }
    ...
    function getPoolLockDay(uint poolIndex, uint lockDay) public view returns (uint) {
        PoolInfo storage pool = poolInfo[poolIndex];
        return pool.lockDays[lockDay];
    }
