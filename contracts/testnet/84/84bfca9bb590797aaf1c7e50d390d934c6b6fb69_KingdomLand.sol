...
// File: contracts/nft/KingdomLand.sol
...
/**
 * @title
 * Kingdom Land NFT ERC721
 */
contract KingdomLand is ERC721Tradable {
    ...
    // Stock Setting
    uint256[] private mintCatIds;
    mapping(uint256 => uint256) private mintCatStocks;
    ...
    function mintTo(address _to, uint256 _size) external onlyLaunchpad returns(uint256[] memory _tokenIds){
        require(_to != address(0), "can't mint to empty address");
        require(_size > 0, "size must greater than zero");
        require(LAUNCH_SUPPLY + _size <= LAUNCH_MAX_SUPPLY, "max supply reached");

        _tokenIds = new uint256[](_size);
        for (uint256 i=1; i <= _size; i++) {
            uint256 _catId = _randomNFTCatId(_to, i);
            if(_catId == 0){
               revert("mintTo NFT catId error.");
            }
            uint256 _tokenId = mint(_to, "");
            catIds[_tokenId] = _catId;
      ...
      require(catIds[tokenId] == 0, "nft: used tokenId");
    ...
    function setLauchpad(address _launchpad, uint256 _launchMaxSupply) external onlyOwner {
        LAUNCHPAD = _launchpad;
        LAUNCH_MAX_SUPPLY = _launchMaxSupply;
    }

    function setMintCatStocks(uint256[] memory _catIds,
                        uint256[] memory _stocks) external onlyOwner{
          require(_catIds.length == _stocks.length, "stocks length not match");
          mintCatIds = _catIds;

          for(uint256 i = 0; i < mintCatIds.length; i ++){
              mintCatStocks[mintCatIds[i]] = _stocks[i];
          }
    }
    ...
    function getStock() public view returns(uint256){
        uint256 _totalCap = 0;
        for(uint256 i = 0; i < mintCatIds.length; i++){
          _totalCap = _totalCap.add(mintCatStocks[mintCatIds[i]]);
        }

        return _totalCap;
    }

    function getMintStocks() public view returns(uint256[] memory, uint256[] memory){
        uint256[] memory _stocks = new uint256[](mintCatIds.length);
        for(uint256 i = 0; i < mintCatIds.length; i ++){
            _stocks[i] = mintCatStocks[mintCatIds[i]];
        }

        return (mintCatIds, _stocks);
    }

    function _randomNFTCatId(address _to, uint256 _seedNum) private notContract returns(uint256 catId) {
          uint256 _remaingNum = getStock();
          require(_remaingNum > 0, "mint Stock empty");

          uint256 _random = _seed(_to, _seedNum, _remaingNum);
          uint256 _weight = 0;
          for(uint256 i = 0; i < mintCatIds.length; i++){
            _weight = _weight.add(mintCatStocks[mintCatIds[i]]);
            if(_weight > _random){
                catId = mintCatIds[i];
                mintCatStocks[catId] = mintCatStocks[catId].sub(1);
                break;
            }
          }
    ...
    function _seed(address _user, uint256 _seedNum, uint256 _supply) internal view returns (uint256)
    {
      return uint256( uint256( keccak256(
              abi.encodePacked(_user, block.number, gasleft(), _supply, _seedNum, block.timestamp, block.difficulty, block.coinbase)
            ) ) % _supply );
    }
}