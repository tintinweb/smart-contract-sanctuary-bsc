...
Buy tax
Tax: 10% (slippage 11%)
4% Payouts in $BTCB, $ETH, $BUSD, $MATIC, $ADA, $BNB & $LEG, more added later
3% Auto buyback 
1% Gas Refunds 
1% Marketing in $Bnb
0.6% admin 
...
Payouts
→ all the sellers tax is given back in multiple tokens → no $LEG price dump from selling the sell tax
Out of the seling tax:
50% holders in $BTCB, $ETH, $ADA, $MATIC, $BNB, $BUSD & $LEG
...
// Legenden Network contract

contract Legenden is ERC20, ReentrancyGuard {
    string public constant symbol = "LEG"; //LEG
    string public constant name = "Legenden"; //Legenden Network
...
//        __balanceOf[address(this)] = __totalSupply/2;
   ...
   AddTokentoPayoutList(address(this), "LEG"); //this contract token
  ...
  function UpdateRegister(address recipient, bool ExcludedfromTax) private
   {
        ...
        _AutoPayoutEnabled[recipient] = true; // only human holders get payed auto rewards as default
         ...
         _RewardTokens[recipient].Slice1 = 25; //25% LEG
