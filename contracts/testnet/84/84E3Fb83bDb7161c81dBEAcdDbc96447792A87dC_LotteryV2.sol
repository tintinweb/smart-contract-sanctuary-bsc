...
contract LotteryV2 is Initializable {
    ...
    mapping(uint256 => uint256) public _lotteriesJackPot;
    mapping(uint256 => uint256) public _numOfWinnerJackPot;
    ...
    function setPotSize(uint256 _amtJackPot) external onlyOperator {
        require(_amtJackPot > 0);
        _lotteriesJackPot[currentLotteryId] = _amtJackPot;
    }
    ...
    function setNumberOfWinners(uint256 _lotteryId, uint256 _numberOfWinner) external onlyOperator {
        require(_numberOfWinner > 0, "Number Of JackPot cannot be 0");
        _numOfWinnerJackPot[_lotteryId] = _numberOfWinner;
    }
    ...
    function batchBuyLottoTicket(
        ...
        _lotteriesJackPot[_lotteryId] += totalCost / 2;
    ...
    returns (uint256 lotteryId)
    {
        ...
        if (lotteryId > 1) {
            _lotteriesJackPot[lotteryId] = _lotteriesJackPot[lotteryId - 1];
        }
        _numOfWinnerJackPot[lotteryId] = 0;
    ...
    function _prizeForMatching(uint8 _noOfMatching, uint256 _lotteryId) public view returns (uint256) {
        ...
        if (_noOfMatching < 6) {
            ...
            // jackpot
            require (_numOfWinnerJackPot[_lotteryId] > 0, "Need to update number of JackPot");
            return (_lotteriesJackPot[_lotteryId] * (PRECISION)) / _numOfWinnerJackPot[_lotteryId] / (PRECISION ** 2);
        }
    }
    ...
    function getPotSize() external view returns (uint256) {
        return _lotteriesJackPot[currentLotteryId];
    }
