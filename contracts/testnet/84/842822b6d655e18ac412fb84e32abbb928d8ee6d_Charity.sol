...
contract Charity is Ownable {
    ...
    uint public sponsorPercentage = 7000;
    uint public swapPercentage = 2000;
    //uint public corporatePercent = 300;
    uint public burnPercentage = 9000;
    uint public charityPercentage = 1000;
    ...
    function setSponsorPercentage(uint256 _sponsorPercentage) external onlyOwner{
    ...
    function buyProduct(address _sponsorAddress, uint256 amount) external payable {
        ...
        uint256 sponsorAmount = amountInEth.mul(sponsorPercentage).div(divisor);
        uint256 swapAmount = amountInEth.mul(swapPercentage).div(divisor);
        uint256 corporateAmount = msg.value - sponsorAmount - swapAmount;
        ...
        uint256 utilityTokens = IERC20(DBME).balanceOf(address(this)).mul(70).div(100);
        uint256 tokensToBurn = utilityTokens.mul(burnPercentage).div(divisor);
        uint256 tokensToCharity = utilityTokens.mul(charityPercentage).div(divisor);
        uint256 tokensToClient = IERC20(DBME).balanceOf(address(this)) - tokensToBurn - tokensToCharity;
    ...
    function upgradeProduct(address _sponsorAddress, uint256 amount) external payable {
        require(_sponsorAddress != address(0), "Invalid address");
        int256 price = getLatestPrice();
        uint256 balanceInUSD = (msg.value * uint256(price)).div(priceDivisor);
        require(balanceInUSD >= amount.mul(10 ** 18), "Insufficient balance");
        ...
        uint256 utilityDBME = amount.mul(10 ** 18) / utilityPrice;
        require(IERC20(DBME).balanceOf(msg.sender) >= utilityDBME, "Insufficient DBME");
        uint256 tokensToBurn = utilityDBME.mul(10 ** 18).mul(burnPercentage).div(divisor);
        uint256 tokensToCharity = utilityDBME.mul(10 ** 18).mul(charityPercentage).div(divisor);

        uint256 sponsorAmount = amountInEth.mul(sponsorPercentage + swapPercentage).div(divisor);
        uint256 corporateAmount = msg.value - sponsorAmount;
        payable(_sponsorAddress).transfer(sponsorAmount);
        payable(corporateAddress).transfer(corporateAmount);

        IERC20(DBME).burnFrom(
            msg.sender,
            tokensToBurn
        );
    ...
    function marketPurchase(address _receiver, uint256 amount) external payable{
        require(_receiver != address(0), "Invalid address");
        int256 price = getLatestPrice();
        uint256 balanceInUSD = (msg.value * uint256(price)).div(priceDivisor);

        require(balanceInUSD >= amount.mul(3 *(10 ** 18)).div(100), "Insufficient balance");
        uint256 utilityDBME = amount.mul(10 ** 18) / utilityPrice;
        payable(corporateAddress).transfer(msg.value);

        IERC20(DBME).transferFrom(
            msg.sender,
            _receiver,
            utilityDBME.mul(10 ** 18)
        );
    }
