...
contract ShareWrapper {
    ...
    function withdraw(uint256 amount) public virtual {
        ...
        for(uint256 i; i < amount; i++) {
            nfts.transferFrom(address(this), address(msg.sender), user.tokensId[user.tokensId.length - 1]);
            user.tokensId.pop();
        }
...
contract Boardroom is ShareWrapper, ContractGuard {
    ...
    function initialize(
        ...
        withdrawLockupEpochs = 6; // Lock for 60 epochs (360h) before release withdraw
        rewardLockupEpochs = 3; // Lock for 30 epochs (180h) before release claimReward
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
           // require(boards[msg.sender].epochTimerStart.add(rewardLockupEpochs) <= treasury.epoch(), "Boardroom: still in reward lockup");
            ...
            loon.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        loon.safeTransferFrom(msg.sender, address(this), amount);
