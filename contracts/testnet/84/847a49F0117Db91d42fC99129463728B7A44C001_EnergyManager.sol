...
//    marketPlace = IMarketPlace(0x57573e2239C0A9E67E74D8AC8EA590E8FD99fA63);
  ...
  function updateEnergy(address _user, uint _consumeAmount) external onlyGameContract returns (bool) {
    ...
    if (firstConsumeInDay) {
      uint todayEnergy = _calculateUserTodayEnergy(_user);
      require(todayEnergy >= _consumeAmount, "EnergyManager: data invalid");
      energy.amount = todayEnergy - _consumeAmount;
      energy.from = _getStartOfDay();
      energy.to = energy.from + secondInADay;
      energy.tempPoint = 0;
      energy.lastUpdateTempPoint = block.timestamp;
    } else {
      require(energy.amount >= _consumeAmount, "EnergyManager: consume amount invalid");
      energy.amount -= _consumeAmount;
    }
    emit EnergyUpdated(_user, energy.amount, firstConsumeInDay, _user);
    return firstConsumeInDay;
  }

  function updatePoint(address _user, int _point) external onlyPointContract {
    Energy storage energy = energies[_user];
    bool noEndgameCalled = (energy.to == 0) || (energy.to != 0 && block.timestamp > energy.to);
    if (noEndgameCalled) {
      uint startOfDay = _getStartOfDay();
      bool needClearYesterdayPoint = energy.to <= startOfDay - secondInADay && energy.lastUpdateTempPoint <= startOfDay;
      if (needClearYesterdayPoint) {
        energy.tempPoint = 0;
      }
      energy.tempPoint += _point;
      energy.lastUpdateTempPoint = block.timestamp;
    }
  }
  ...
  function getUserCurrentEnergy(address _user) external view returns (uint) {
    Energy storage energy = energies[_user];
    bool firstConsumeInDay = energy.to != 0 && block.timestamp > energy.to;
    if (firstConsumeInDay) {
      return _calculateUserTodayEnergy(_user);
    }
...
//  function _getStartOfDay() private view returns (uint) { TODO
  function _getStartOfDay() public view returns (uint) {
    return block.timestamp - block.timestamp % secondInADay;
  }

  function _calculateUserTodayEnergy(address _user) private view returns (uint) {
    uint owned = heroNft.balanceOf(_user);
    uint rented = marketPlace.currentRentedHeroCounter(_user);
    Energy storage userEnergy =  energies[_user];
    int tempPoint = int(owned) + int(rented) - userEnergy.tempPoint;
    require(tempPoint >= 0, "EnergyManager: user point data invalid");
    uint energy = uint(tempPoint) * energyMultiplication;
    if (energy > maxEnergy) {
      energy = maxEnergy;
    }
    return energy;
