...
contract TreasuryV6 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    event OwnerCashReservesChanged(address indexed caller, uint cash, uint buffer, uint reserve0, uint reserve1);
    event OwnerPoolStateChanged(address indexed caller, address poolAddress, uint lpAmount);
    event OwnerConfigurationChanged(address indexed caller, uint maxBuffer, uint minCash, uint threshold);
    event OwnerTokenReceiverAdded(address indexed caller, address[] tokenReceivers);
    event OwnerTokenReceiverRemoved(address indexed caller, address tokenReceiverRemoved, address[] tokenReceivers);
    event OwnerInvestmentProductAdded(address indexed caller, address pool);
    event OwnerInvestmentProductRemoved(address indexed caller, address pool);
    event OwnerInvestmentProductsChanged(address indexed caller);
    address public lsc;
    ...
    function send_tokens_investment(uint amount0, uint amount1, uint poolIndex) public onlyOwner {
        ...
        emit OwnerCashReservesChanged(msg.sender, cash, buffer, reserve0, reserve1);
        emit OwnerPoolStateChanged(msg.sender, pools[poolIndex].router, pools[poolIndex].amountOfLPTokens);
    }
    ...
    function retrieve_tokens(uint amountLpTokens, uint poolIndex) public onlyOwner {
        ...
        emit OwnerCashReservesChanged(msg.sender, cash, buffer, reserve0, reserve1);
        emit OwnerPoolStateChanged(msg.sender, pool.router, pool.amountOfLPTokens);
    }
    ...
    function set_maximum_buffer(uint _buffer) public onlyOwner {
        ...
        emit OwnerConfigurationChanged(msg.sender, maximumBuffer, minimumCash, threshold);
    }
    ...
    function add_new_investment_product(string memory name, address router, address pair, uint[] memory newPercentages) public onlyOwner {
        ...
        numberOfPools += 1;
        emit OwnerInvestmentProductAdded(msg.sender, router);
    }
    ...
    function remove_investment_product(uint index, uint[] memory newPercentages) public onlyOwner {
        ...
        uint totalPercent;
        address poolAddressRemoved = pools[index].router;
        ...
        require(totalPercent == 100, "Total percent of all products must be 100");
        emit OwnerInvestmentProductRemoved(msg.sender, poolAddressRemoved);
    }
    ...
    function change_pools_percentages(uint[] memory newPercentages) public onlyOwner {
        ...
        require(totalPercent == 100, "Total percent of all products must be 100");
        emit OwnerInvestmentProductsChanged(msg.sender);
    }
    ...
    function add_token_receiver(address receiver) public onlyOwner {
        ...
        emit OwnerTokenReceiverAdded(msg.sender, tokenReceivers);
    }
    ...
    function remove_token_receiver(uint index) public onlyOwner {
        ...
        address tmp = tokenReceivers[index];
        tokenReceivers[index] = tokenReceivers[tokenReceivers.length - 1];
        tokenReceivers[tokenReceivers.length - 1] = tmp;
        tokenReceivers.pop();
        emit OwnerTokenReceiverRemoved(msg.sender, tmp, tokenReceivers);
    }
    ...
    function set_lp_amount(uint index, uint amountOfLP) public onlyOwner {
        ...
        require(IERC20(pools[index].pair).allowance(msg.sender, address(this)) >= amountOfLP, "Allowance");
        IERC20(pools[index].pair).transferFrom(msg.sender, address(this), amountOfLP);
        sentLPTokens[index] -= Math.min(amountOfLP, sentLPTokens[index]);
        pools[index].amountOfLPTokens += amountOfLP;
        emit OwnerPoolStateChanged(msg.sender, pools[index].router, pools[index].amountOfLPTokens);
    }

    function set_reserves_amount(uint amount0, uint amount1) public onlyOwner {
        require(IERC20(token0Address).allowance(msg.sender, address(this)) >= amount0 && IERC20(token1Address).allowance(msg.sender, address(this)) >= amount1, "Allowance");
        if(amount0 > 0) {
            IERC20(token0Address).transferFrom(msg.sender, address(this), amount0);
        }
        if(amount1 > 0) {
            IERC20(token1Address).transferFrom(msg.sender, address(this), amount1);
        }
        sentReserve0 -= Math.min(amount0, sentReserve0);
        sentReserve1 -= Math.min(amount1, sentReserve1);
        reserve0 += amount0;
        reserve1 += amount1;
        uint liquidity = calculate_liquidity_amount(reserve0, reserve1);
        cash = Math.min(liquidity, minimumCash);
    ...
    function find_pool_to_add(uint amountLiquidity, uint amountAll0, uint amountAll1) private view returns (uint, uint){
        uint liquidityInPools = calculate_liquidity_amount(amountAll0 - reserve0, amountAll1 - reserve1);
        uint poolToAddPercentage = pools[0].percentage;
        uint poolIndex;
        uint poolMismatch;
        uint poolPercentage;
        ...
        if (numberOfPools > 1) {
            for (uint i = 0; i < numberOfPools; i++) {
                poolPercentage = pools[i].percentage * PERCENT_PRECISION;
                uint currentPoolPercentage = pools[i].amountLiquidity * PERCENT_PRECISION * 100 / Math.max(liquidityInPools, 1);
                if (poolPercentage > currentPoolPercentage &&
                    (poolPercentage - currentPoolPercentage > poolMismatch ||
                    (poolPercentage - currentPoolPercentage == poolMismatch && pools[i].percentage > poolToAddPercentage))
                ) {
                    poolToAddPercentage = pools[i].percentage;
                    poolMismatch = poolPercentage - currentPoolPercentage;
                    poolIndex = i;
                }
            }
        }
        return (poolIndex, amountLiquidity);
    }
    ...
    function send_tokens_investment_buffer() private {
        ...
        (uint poolIndex, uint amountsToAdd) = find_pool_to_add(buffer, amount0, amount1);

        uint amountToken = reserve1 * amountsToAdd / (buffer + cash);
        uint amountBlxm = amountToken * totalAmounts0[poolIndex] / totalAmounts1[poolIndex];
        if (amountBlxm > reserve0) {
            amountBlxm = reserve0 * amountsToAdd / (buffer + cash);
            amountToken = amountBlxm * totalAmounts1[poolIndex] / totalAmounts0[poolIndex];
        }
    ...
    function find_pool_to_fill_reserves(FindPoolArgs memory args) private view returns (uint[] memory, uint[] memory) {
        ...
        if (args.liquidityInPools - args.amountLiquidity == 0) {
            ...
            uint liq = Math.max(
                args.amountLiquidity * Math.sqrt(args.currentAmounts1[poolIndex] * args.expectedBlxm / (args.currentAmounts0[poolIndex] * args.expectedToken)),
                args.amountLiquidity * Math.sqrt(args.currentAmounts0[poolIndex] * args.expectedToken / (args.currentAmounts1[poolIndex] * args.expectedBlxm))
            );
            ...
            if (poolToRemove.amountLiquidity < liq ||
                ...
                for (uint i = 0; i < numberOfPools; i++) {
                    ...
                    if (isPossibleToBalance) {
                        ...
                        amountsToRemove[i] = Math.max(
                            amountsToRemove[i] * Math.sqrt(args.currentAmounts1[i] * args.expectedBlxm / (args.currentAmounts0[i] * args.expectedToken)),
                            amountsToRemove[i] * Math.sqrt(args.currentAmounts0[i] * args.expectedToken / (args.currentAmounts1[i] * args.expectedBlxm))
                        );
