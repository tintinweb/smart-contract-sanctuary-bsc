...
import "./erc721/rewards/ERC721RewardSplitter.sol";

import "@openzeppelin/contracts/utils/Counters.sol";
import "@openzeppelin/contracts/utils/math/SafeMath.sol";

/**
 * @title K(AI) NFT Contract
 */
contract KAI is ERC721Base, ERC721RewardSplitter {
    ...
    uint256 public immutable maxMintPerTx = 60;
    uint256 public immutable maxSupply;
    uint256 public immutable mintReflectionPercentage = 5;
    address public communityWallet;
    uint256 public communityWithdrawable;
    ...
    function mint(uint256 _amount) external nonReentrant payable {
        ...
        _distributeToHolders(msg.value * mintReflectionPercentage / 100);
    }
    ...
    /**
     * @notice Mint `amount` free nfts to `_to`. Only callable by owner.
     */
    ...
    function withdraw() external override onlyOwner nonReentrant {
        uint256 pending = pendingTotalTokens();
        uint256 available = address(this).balance;

        require(available > pending, "Nothing to withdraw");
        
        uint256 withdrawable = available - pending - communityWithdrawable;
        uint256 community = communityWithdrawable;
        communityWithdrawable = 0;

        payable(communityWallet).transfer(community);
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(ERC721Enumerable, ERC721RewardSplitter) {
        ERC721RewardSplitter._beforeTokenTransfer(from, to, tokenId);
    }


    /** 
     * @notice Split marketplace royalties between community, holders and owner
     */
    receive() external payable {
        communityWithdrawable += msg.value / 3;
        _distributeToHolders(msg.value / 3);
        // 1/3 withdrawable by owner
    }
}
...
/**
 * @title Contract to manage distribution of rewards to holders, usage through inheritance.
 *
 * @dev Holders of this token are eligable to claim rewards send to this contract via {distributeToHolders} or booked internally via {_distributeToHolders}.
 * - holders receive an equal split relative to current {totalSupply}
 * - rewards are bound to the tokenId. 
 * - unclaimed rewards can be claimed by a new owner after transfer.
 * 
 * Developed by Fab
 */
abstract contract ERC721RewardSplitter is ERC721Enumerable, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    event RewardsReceived(uint256 amount, uint256 totalSupply);
    ...
    function _distributeToHolders(uint256 amount) internal nonReentrant {
        ...
        emit RewardsReceived(amount, totalSupply());
    }
        ...
        nonReentrant
    {
        ...
        emit RewardsClaimed(msg.sender, _tokenId, pending);
    ...
    /**
     * @notice Total pending for withdrawal by owner.
     */
    function pendingWithdrawal() external view returns (uint256) {
        return address(this).balance - pendingTotalTokens();
    }
    ...
    function pendingTotalTokens() public view returns (uint256 pending) {
        pending = (totalSupply() * rewardPerToken) - totalTokenDept;
    }
