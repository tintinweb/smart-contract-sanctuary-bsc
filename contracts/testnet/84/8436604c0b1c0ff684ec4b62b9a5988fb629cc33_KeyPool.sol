...
contract KeyPool is Ownable {
        ...
        if (ownerStake[_staker] == 0) {
            if (stake.length == 0) {
                ...
                if (_staker == stake[0].staker) {
                    updateStaker(true, 0, _stakeNum, now);
                } else {
                    stake.push(Stake(_staker, _stakeNum, now, now, 0, 0));
                    ownerStake[_staker] = stake.length - 1;
                }
            ...
            if (_staker == stake[_idx].staker) {
                updateStaker(true, _idx, _stakeNum, now);
            } else {
    ...
    function queryIncome(address _staker) external isOpen view returns(uint256,uint256,uint256,uint256){
        ...
        (,uint256 _busd) = this.getLPOnStake();
        uint256 _incomePer = calcIncome(stake[_idx].Amount, 1, _busd, lastAirDrop); //每秒收益
        uint256 _incomeNow = getIncome(_idx, now); //calcIncome(stake[_idx].Amount, now - stake[_idx].lastTime);
