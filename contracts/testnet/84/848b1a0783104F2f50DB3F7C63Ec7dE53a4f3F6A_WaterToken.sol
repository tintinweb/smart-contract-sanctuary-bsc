...
contract WaterToken is ERC20, Ownable {
    ...
    constructor(address _router) ERC20("Mfer Token", "MFER") {
        _mint(msg.sender, 2000000 * 10 ** 18);

        if (_router != address(0)) {
            router = _router;
            address pair = IPancakeFactory(IPancakeRouter02(router).factory()).createPair(IPancakeRouter02(router).WETH(), address(this));
    ...
    function _transfer(
        ...
        if (from != owner() && automatedMarketMakerPairs[to] != true && to != address(this) && from != address(this)) {
