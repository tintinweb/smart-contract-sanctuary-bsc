...
contract NFTAA {
    ...
    constructor() {
        nftContract = IMYNTIST_NFT(0xca7f22c28cfD9e9B34992A21613B81C74bd027A2);
        tokenContarct = IMYNTIST(0x82063aa685abeE1B697fDc3Ae4F25ED34bd3b4a2);
    }
    ...
    function enterLobby(address user) external {
        ...
        require(qRef.nftBalance == 0 && nftBalance > 0,"Already Added for current day");
        qRef.nftBalance = nftBalance;
        totalBalancePerDay[enterDay] += nftBalance;
    }

    function claimTokens(uint256 day) external {
        uint256 currentDay = _currentDay();
        require(day == currentDay - 1 && day != 0,"Not claim Day");
        recordsStruct storage qRef = userRecord[day][msg.sender];
        require(qRef.nftBalance == 0,"Record Not Found");
        uint256 userShare = (perDaySupply * qRef.nftBalance) / totalBalancePerDay[day];
        tokenContarct.mintAmount(msg.sender, userShare);
        delete userRecord[day][msg.sender];
