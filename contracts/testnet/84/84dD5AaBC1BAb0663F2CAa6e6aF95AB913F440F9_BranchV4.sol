...
interface IBranchV4Random {
    function randomDaysStartFinalPay(uint256 k, bool first) external view returns (uint256 days_);

    function randomDaysStartWithdraw(uint256 k, uint8 level) external view returns (uint256 days_);

    function createId(address addr) external view returns (uint256);
}

contract BranchV4 is ReentrancyGuard, AccessControlEnumerable {
    ...
    address branchRandom = 0xf0Eff379DF2189Cc1B8ec92777BA9B2A318E54df;
    // 管理员配置
    ...
    uint256 public fuelBurnedTotal; // 燃料消耗统计
    ...
    function userPreDeposit(uint256 amount, bool useCommentReward) external nonReentrant {
        ...
        uint256 days_ = IBranchV4Random(branchRandom).randomDaysStartFinalPay(depositWithdrawRatePercent(), brokers[msg.sender].depositTotal == 0);
        // deposit.startFinalPayTimestamp = uint32(block.timestamp + days_ * dayInterval);
        deposit.startFinalPayTimestamp = uint32(block.timestamp);
    ...
    function userFinalDeposit(uint256 _index) external nonReentrant {
        ...
        uint256 days_ = IBranchV4Random(branchRandom).randomDaysStartWithdraw(depositWithdrawRatePercent(), accounts[msg.sender].level);
    ...
    function brokerBonusWithdraw() external nonReentrant {
        ...
        uint256 amount = brokerBounsAvailable(msg.sender);
        if (amount > 0) {
        ...
        if (accounts[msg.sender].brokerBounsAvailable > 0) {
            amount += accounts[msg.sender].brokerBounsAvailable;
            accounts[msg.sender].brokerBounsAvailable = 0;
        }
        if (amount > 0) {
            IFuel(fuel).burnFrom(msg.sender, amount / 100);
            fuelBurnedTotal += amount / 100;
            IERC20(usdt).safeTransfer(msg.sender, amount);
        }
    }

    function brokerBounsAvailable(address addr) public view returns (uint256 amount) {
        if (brokers[addr].brokerBonusMax > 0 && brokers[addr].brokerBonusCalc > brokers[addr].brokerBonusWithdrawed) {
            amount = brokers[addr].brokerBonusCalc - brokers[addr].brokerBonusWithdrawed;
            if (amount > brokers[addr].brokerBonusMax) {
                amount = brokers[addr].brokerBonusMax;
            }
        }
    }
    ...
    function userDealBlacklist(uint256 index) external nonReentrant {
        ...
        fuelBurnedTotal += 100e18 * brokers[msg.sender].punishTimes;
    }
    ...
    function fomoRewardWithdraw() external nonReentrant {
        ...
        fuelBurnedTotal += fomoRewardDrawables[msg.sender] / 100;
    ...
    function sprintRewardWithdraw() external nonReentrant {
        ...
        fuelBurnedTotal += sprintRewardDrawables[msg.sender] / 100;
        IERC20(usdt).safeTransfer(msg.sender, sprintRewardDrawables[msg.sender]);
        // brokers[msg.sender].profitAmount += int256(sprintRewardDrawables[msg.sender]); // 因为数据将会被清零,所以此句无意义
    ...
    function restart(uint256 depositIndex) external {
        ...
        require(block.timestamp > deposits[msg.sender][depositIndex].createTimestamp + 2 * dayInterval, "You deposit not satisfy to restart");
    ...
    function _lossCompensationClaim(address addr) private {
        if (versionTimestamps[versionTimes] != brokers[addr].currentVersionTimestamp) {
            ...
            uint256 amount = brokerBounsAvailable(addr);
            if (amount > 0) {
                accounts[addr].brokerBounsAvailable += amount;
            }
            delete brokers[addr];
            delete deposits[addr];
            delete commentRewardSubmitTimes[addr];
            delete commentRewardTimes[addr];
    ...
    function lossCompensationWithdraw(uint32 _versionTimestamp) external nonReentrant {
        ...
        IERC20(fuel).safeTransfer(msg.sender, drawable);
    ...
    function depositRemainToday() public view returns (uint256) {
        uint32 days_ = ((uint32(block.timestamp) - versionTimestamps[versionTimes])) / dayInterval;
        if (days_ > 20) {
            days_ = 20;
        }
