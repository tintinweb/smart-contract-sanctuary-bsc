...
contract BananaTaskForceApeBlindBox {
    ...
    constructor() {
        ...
        // Nft Contract
        nftContractAddress = 0x58992005849482AA9886A88A175DBBD129f35CE5;
   ...
   function mint(address who) private {
        ...
        for (uint256 i = 1; i <= total; i++) {
            ...
            if (roll <= current) {
                uri = string(abi.encodePacked("https://bafybeibj6fe77rzv5afyl7d6qndbb62luoqsqqr6zeyeizp5mdlqt223xe.ipfs.nftstorage.link/", uint2str(i), ".json"));
                issued[i] = true;
                tokenId = i;
                break;
            }
    ...
    function reserveNfts(address who) external onlyOwner {
        require(reserved == false, "NFTS have already been reserved");

        for (uint256 i = 0; i < reserveLimit; i++) {
            mint(who);
        }
