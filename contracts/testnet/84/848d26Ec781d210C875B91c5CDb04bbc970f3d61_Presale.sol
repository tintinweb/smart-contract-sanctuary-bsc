...
contract Presale is ERC20Upgradeable, OwnableUpgradeable {
    uint256 public zeroTokenBalance;
    address public StableCoin;
    address public SubZero;
    address public myaddress = address(this);

    event Sold(uint256 amount);
    constructor(address token1, address _subZero) public {
        StableCoin = token1;
        SubZero = _subZero;
    ...
    function getBalance(address token, address addr) public view returns (uint256) {
        return IERC20(token).balanceOf(addr);
    }

uint256 public userBalance;
    function sell(uint256 _stalbelCoinAmount, uint256 _zeroCoinAmount, address token) external {
        setStableCoin(token);
        require(_stalbelCoinAmount > 0, "Invalid amount");
        userBalance = IERC20(StableCoin).balanceOf(msg.sender);
        require(userBalance >= _stalbelCoinAmount, "Not enough funds in your wallet");

        bool res = IERC20(SubZero).transfer(msg.sender, _zeroCoinAmount);
        // IERC20(StableCoin).transferFrom(msg.sender, address(this), _stalbelCoinAmount);
        require(res, "fail to transfer");
        // emit Sold(_zeroCoinAmount);
    }
