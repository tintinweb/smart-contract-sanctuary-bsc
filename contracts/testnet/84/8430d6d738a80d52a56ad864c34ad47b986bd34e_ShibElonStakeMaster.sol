...
contract ShibElonStakeMaster is Ownable{
    ShibElonStake StakeContract;
   
    address[] StakeShibElonEarnShibElonContract;
    address[] StakeShibElonEarnAnyContracts;
    address[] StakeAnyEarnAnyContracts;
    //address public owner;
  
    //mapping (uint256 => anyTokenStake) public anyTokenStakeDetailsById;
    //mapping (address => bool) public isAuthorized;
    uint256 stakeIndex;


    function createStake(uint256 _type, address _stakeToken, address _rewardToken) public onlyOwner {
       
        StakeContract = new ShibElonStake(_stakeToken,_rewardToken,msg.sender);
        
        if(_type==1){
            StakeShibElonEarnShibElonContract.push(address(StakeContract));
        }else if(_type==2){
            StakeShibElonEarnAnyContracts.push(address(StakeContract));
        }else if(_type==3){
            StakeAnyEarnAnyContracts.push(address(StakeContract));
        }
    }
    
    function getAllStakes() public view returns(address[] memory,address[] memory,address[] memory){
        return (StakeShibElonEarnShibElonContract,StakeShibElonEarnAnyContracts,StakeAnyEarnAnyContracts);
    }
}
...
contract ShibElonStake is IERC721Receiver, Ownable {
    ...
    address nftTokenAddress = 0x57121F009e7ceE900631d17a2CF6EB63040a3aF8;
    ...
    function updateNFTRarity(uint256[] calldata _tokenIds, uint256[] calldata _rarities) external onlyOwner {
    ...
    function getRarityToApply() public view returns(uint256){
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if(rarity < nftRarities[tokenIds[i]]){
               rarity = nftRarities[tokenIds[i]];
            }
        }
        return rarity;
    }
    ...
    */
    ...
    function getUnclaimedRewards(uint256 _stakeId) public view returns (uint256){
        ...
        stakePool memory stakePoolDetails = stakePoolsById[stakePoolId];
        uint256 stakeApr = (getAPY(getRarityToApply(), stakePoolDetails.duration) * magnitude) / 365;
    ...
    */
    ...
    //Testing function
    function setLockTime(uint256 _stakeId, uint256 _days) unpaused external returns (bool){
        userStake memory userStakeDetails = userStakesById[_stakeId];
