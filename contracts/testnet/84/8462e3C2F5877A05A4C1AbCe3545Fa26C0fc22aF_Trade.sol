...
contract Trade is WhitelistChecker{
    ...
    function transferAsset(whitelisted memory order, Fee memory fee) internal virtual {
        if (order.inEth){
            if (fee.platformFee >0) {
                payable(owner).transfer((fee.platformFee * 1 ether) / 1000);
            }
            if (fee.royaltyFee > 0) {
                payable(fee.tokenCreator).transfer((fee.royaltyFee * 1 ether) / 1000);
            }
            payable(order.seller).transfer((fee.assetFee * 1 ether) / 1000);
    ...
    */
    function excuteOrder(whitelisted memory order) external payable returns(bool) {
        ...
        if (order.inEth) {
            uint price =  (order.amount * 1 ether) / 1000;
            require (msg.value >= price,'Insufficient Amount');
        }
        usedNonce[msg.sender][order.timestamp] = true;
        Fee memory fee = calculateFees((order.amount * 1 ether)/1000, order.nftAddress, order.tokenId);
