...
contract GolarToken {
    ...
    string public constant symbol = "GolaR"; // solium-disable-line uppercase
    ...
    */
    ...
    function _betaDelegatedTransfer(
        ...
        bytes32 hash = keccak256(abi.encodePacked(EIP191_HEADER, EIP712_DOMAIN_HASH, keccak256(abi.encodePacked(// solium-disable-line
                EIP712_DELEGATED_TRANSFER_SCHEMA_HASH, bytes32(keccak256(abi.encodePacked(to))), value, serviceFee, seq, deadline
        ))));
    ...
    function betaDelegatedTransferBatch(
        bytes32[] memory r, bytes32[] memory s, uint8[] memory v, address[] memory to, uint256[] memory value, uint256[] memory serviceFee, uint256[] memory seq, uint256[] memory deadline
    ) public returns (bool) {
