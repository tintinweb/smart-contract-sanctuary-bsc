...
contract DiscoBurnNFT is ERC1155, ERC1155Supply, Ownable, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    uint public idCounter;
    string public baseURI;
    DiscoBurnNFTAirdrop public airdropper;
    ...
    constructor() ERC1155("https://storageapi2.fleek.co/82fe97e9-75f8-4098-8249-59b097b6bfe7-bucket/{id}") {
        baseURI = "https://storageapi2.fleek.co/82fe97e9-75f8-4098-8249-59b097b6bfe7-bucket/";
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        grantRole(MINTER_ROLE, msg.sender);

        airdropper = new DiscoBurnNFTAirdrop(address(this));

        _mint(address(airdropper), 1, 160, ""); airdropper.supply(1, 160);
        _mint(address(airdropper), 2, 160, ""); airdropper.supply(2, 160);
        _mint(address(airdropper), 3, 160, ""); airdropper.supply(3, 160);
        _mint(address(airdropper), 4, 20, ""); airdropper.supply(4, 20);

        idCounter = 4;

        airdropper.transferOwnership(msg.sender);
    }
    ...
    function setAirdropper(address _airdropper) external onlyOwner {
        airdropper = DiscoBurnNFTAirdrop(payable(_airdropper));
    }
    ...
    function mint(uint _amount) public onlyMinter {
        ...
        idCounter++;
        _mint(address(airdropper), idCounter, _amount, "");
        airdropper.supply(idCounter, _amount);
    }

    function extraMint(uint _id, uint _amount) public onlyMinter {
        require (_amount > 0, "!amount");

        _mint(address(airdropper), _id, _amount, "");
        airdropper.supply(_id, _amount);
    }
...
contract DiscoBurnNFTAirdrop is Ownable, Pausable, ERC1155Holder, AccessControl, ReentrancyGuard {
    ...
    IERC1155 public dbtNFT;

    uint public price;
    uint public devPercent = 1000;

    address public ownerWallet = 0x25b27209D991135Ac58742596E60cF1a21Eb9008;
    address public devWallet = 0x89352214a56bA80547A2842bbE21AEdD315722Ca;
    ...
    constructor(address _dbtNFT) {
        dbtNFT = IERC1155(_dbtNFT);
    ...
    function supply(uint _id, uint _amount) external onlySupplier {
        ...
        // dbtNFT.safeTransferFrom(msg.sender, address(this), _id, _amount, "");
    ...
    function airdrop(uint _amount) internal nonReentrant whenNotPaused returns (uint[] memory) {
        ...
        if (price > 0) {
            payable(devWallet).call{
                value: msg.value.mul(devPercent).div(MAX_FEE),
                gas: 30000
            }("");

            payable(ownerWallet).call{
                value: msg.value.mul(MAX_FEE.sub(devPercent)).div(MAX_FEE),
                gas: 30000
            }("");
        }

        if (dbtNFT.isApprovedForAll(address(this), msg.sender) == false) {
            dbtNFT.setApprovalForAll(msg.sender, true);
        }
    ...
    function _airdrop(address _to) internal returns (uint) {
        ...
        dbtNFT.safeTransferFrom(address(this), _to, id, 1, "");
    ...
    function withdraw(uint _id, uint _amount) external onlySupplier {
        ...
        dbtNFT.safeTransferFrom(address(this), msg.sender, _id, _amount, "");
    ...
    function setWallets(address _owner, address _dev) external onlyOwner {
        ownerWallet = _owner;
        devWallet = _dev;
    }
    ...
    receive() external payable {
        require (msg.value >= price, "!budget");
        airdrop(msg.value.div(price));
        if (msg.value.mod(price) > 0) {
            payable(msg.sender).transfer(msg.value.mod(price));
        }
    }
}
