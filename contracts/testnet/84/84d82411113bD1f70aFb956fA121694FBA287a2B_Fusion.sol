...
contract Fusion is OwnableUpgradeable, ERC721Holder {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using AddressUpgradeable for address;

    uint256 constant BONUS_STAT = 40;
    uint256 constant TOTAL_ALLOCATION = 100;
    uint256 constant GROWTH_CAP = 30000;

    Seedmon public seedmon;

    event BaseStat(uint256 vitality, uint256 strength, uint256 smart, uint256 speed, uint256 dexterity, uint256 luck);
    event FinalStat(uint256 vitality, uint256 strength, uint256 smart, uint256 speed, uint256 dexterity, uint256 luck);
    function initialize(Seedmon _seedmon) external initializer {
        seedmon = _seedmon;
        __Ownable_init();
    }
    ...
    function multiply(uint256 num1) external pure returns (uint256 result) {
        result = num1.mul(BONUS_STAT).div(TOTAL_ALLOCATION);
    }

    function testMintSeedmon(uint256 mainTokenId, uint256 materialTokenId, address to) external {  
        
        (bytes32 name,SeedmonAttribute.UniqueProfile memory mainUniqueProfile, SeedmonAttribute.Stat memory mainStat) = seedmon.tokenInfo(mainTokenId);
        (,SeedmonAttribute.UniqueProfile memory materialUniqueProfile, SeedmonAttribute.Stat memory materialStat) = seedmon.tokenInfo(materialTokenId);
            
        require(mainUniqueProfile.numberOfFused == materialUniqueProfile.numberOfFused, "Fuse: Invalid star of Material Mon");
        require(mainUniqueProfile.numberOfFused < 5, "Fuse: maximum limit exceeded");

        SeedmonAttribute.Stat memory pendingStat;
        pendingStat.vitality = materialStat.vitality.mul(BONUS_STAT).div(TOTAL_ALLOCATION);
        pendingStat.strength = materialStat.strength.mul(BONUS_STAT).div(TOTAL_ALLOCATION);
        pendingStat.smart = materialStat.smart.mul(BONUS_STAT).div(TOTAL_ALLOCATION);
        pendingStat.speed = materialStat.speed.mul(BONUS_STAT).div(TOTAL_ALLOCATION);
        pendingStat.dexterity = materialStat.dexterity.mul(BONUS_STAT).div(TOTAL_ALLOCATION);
        pendingStat.luck = materialStat.luck.mul(BONUS_STAT).div(TOTAL_ALLOCATION);
        
        uint256 star = mainUniqueProfile.numberOfFused;
        if (star == 0) {
            star = 1;
        }
        uint256 sumOfPendingBonusStat = pendingStat.vitality.add(pendingStat.strength).add(pendingStat.smart).add(pendingStat.speed).add(pendingStat.dexterity).add(pendingStat.luck);
        
        uint256 factor = GROWTH_CAP.mul(star).add(1).div(sumOfPendingBonusStat);

        SeedmonAttribute.Stat memory finalStat;
        finalStat.vitality = mainStat.vitality.add(pendingStat.vitality.mul(factor));
        finalStat.strength = mainStat.strength.add(pendingStat.strength.mul(factor));
        finalStat.smart = mainStat.smart.add(pendingStat.smart.mul(factor));
        finalStat.speed = mainStat.speed.add(pendingStat.speed.mul(factor));
        finalStat.dexterity = mainStat.dexterity.add(pendingStat.dexterity.mul(factor));
        finalStat.luck = mainStat.luck.add(pendingStat.luck.mul(factor));
        emit BaseStat(mainStat.vitality, mainStat.strength, mainStat.smart, mainStat.speed, mainStat.dexterity, mainStat.luck);
        emit FinalStat(finalStat.vitality, finalStat.strength, finalStat.smart, finalStat.speed, finalStat.dexterity, finalStat.luck);
        // seedmon.mint(to, name, mainUniqueProfile, mainStat);
