...
contract MvcpHashGame is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    IERC20Upgradeable public MVCC;
    IERC20Upgradeable public MVCP;
    I_MVCP public IMVCP;
    ...
    function initialize(address _mvcc, address _mvcp, address _deadAddr,
    ...
    function registerHashGame(uint256 _gid, bool _premium) external nonReentrant {
        ...
        // Reset and update user Mvcp game info
    ...
    function joinGame() external nonReentrant {
        ...
        require(MVCP.balanceOf(msg.sender) >= _amount, "Insufficient MVCP");

        // Check allowance
        uint256 _allowance = MVCP.allowance(msg.sender, address(this));
        require(_allowance >= _amount, "Insufficient allowance");
        
        // Transfer MVCP
        MVCP.safeTransferFrom(msg.sender, DEAD_ADDR, _amount);
    ...
    function processGame(address _userAddress, bool _result) external nonReentrant onlyOperator {
        ...
        if(_result) {    
            ...
            // User game burst
            // Mint MVCP to user if with valid premium
    ...
    function distributePremiumRewards(uint256 _gameMode, address _userAddress) internal {
        ...
        // Mint MVCP rewards to user, accoring to the premium plan
        IMVCP.mint(_userAddress, _amount);
    ...
    function distributeGameRewards(uint256 _gameMode, address _userAddress, uint256 _amount) internal {
        ...
        uint256 _mvcpRewards = (_amount * 2) - _busdRewards;

        ITreasury.withdraw(_userAddress, _busdRewards);

        // Mint MVCP rewards 
        if(_mvcpRewards > 0)
            IMVCP.mint(_userAddress, _mvcpRewards);
        
        emit DistributeGameRewards(_gameMode, _userAddress, _busdRewards, _mvcpRewards);
    }
    ...
    function setMvcpAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "Address zero");
        MVCP = IERC20Upgradeable(_newAddress);
        IMVCP = I_MVCP(_newAddress);

        emit SetMvcpAddress(_newAddress);
    }
    ...
    event DistributeGameRewards(uint256 gid, address userAddress, uint256 busdReward, uint256 mvcpReward);
    ...
    event SetPremiumFee(uint256[] mvcpPremiumFeeList);
