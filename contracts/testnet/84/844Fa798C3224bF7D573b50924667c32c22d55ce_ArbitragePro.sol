...
contract ArbitragePro is FlashLoanReceiverBase, IPancakeCallee {
    ...
    function multiSingleSwapFLParams(uint256 _amountToFirstMarket, bytes memory _params, uint256 totalLoanDebt) internal {
        ( , uint256 _ethAmountToCoinbase, address[] memory _targets, bytes[] memory _payloads, , address payable _dexRouterContractAddress, address payable _baseToken) = abi.decode(_params, (uint256, uint256, address[], bytes[], address[], address, address));
        require(_targets.length == _payloads.length);
        ITOKEN baseToken = ITOKEN(_baseToken);
        ...
        require(_token0BalanceAfter > totalLoanDebt + _ethAmountToCoinbase);

        uint256 _bnbBalance = address(this).balance;
        if (_baseToken == address(WBNB)) {
            if (_bnbBalance < _ethAmountToCoinbase) {
                WBNB.withdraw(_ethAmountToCoinbase - _bnbBalance);
            }
        } else { 
            IERC20(_baseToken).approve(_dexRouterContractAddress, _ethAmountToCoinbase);
            address[] memory path = new address[](2);
            path[0] = address(_baseToken);
            path[1] = address(WBNB);
            // check next line for correct implementation 
            (uint256[] memory _amountOutMin) = IUniswapV2Router(_dexRouterContractAddress).getAmountsOut(_ethAmountToCoinbase, path);
            // doublecheck the _amountOutMin[0] to see which is the correct output.
            IUniswapV2Router(_dexRouterContractAddress).swapExactTokensForTokens(_ethAmountToCoinbase, (_amountOutMin[0] * 95) / 100  , path, payable(address(this)), block.timestamp);
            WBNB.withdraw(_amountOutMin[0]);
        }

        if (_ethAmountToCoinbase > 0) {
            (bool _success, ) = payable(block.coinbase).call{value: _ethAmountToCoinbase}(new bytes(0));
            require(_success);
        }
        
    }

    function multiCyclicFLParams(uint256 _amountToFirstExchange, bytes memory _params, uint256 totalLoanDebt) internal {
        ( , uint256 _ethAmountToCoinbase, address[] memory _targets, bytes[] memory _payloads, , , address[] memory _dexRouterContractAddress, address[][] memory _routerTokens) = abi.decode(_params, (uint256, uint256, address[], bytes[], address[], address[], address[], address[][]));
        require(_targets.length == _payloads.length);
        // RESTUDY THIS SECTION OF THE CODE
        ...
        for (uint256 j = 0; j < _targets.length; j++) {
            if ( j == 0 && address(WBNB) == _routerTokens[0][0]) {
                IERC20(_routerTokens[0][0]).approve(_dexRouterContractAddress[0], _amountToFirstExchange);
            } else if (j == 0 && address(WBNB) != _routerTokens[0][0]) {
                IERC20(_routerTokens[0][0]).approve(_dexRouterContractAddress[0], _amountToFirstExchange + _ethAmountToCoinbase); 
            } else if (j > 0) {
                // Check parameter Types specially amountOutMin
                (uint256[] memory _amountOutMin) = IUniswapV2Router(_dexRouterContractAddress[j-1]).getAmountsOut(_amountToFirstExchange, _routerTokens[j-1]);
                IERC20(_routerTokens[j][0]).approve(_dexRouterContractAddress[j], _amountOutMin[0] );
            }
        ...
        uint256 _tokenBalanceAfter = IERC20(_routerTokens[0][0]).balanceOf(address(this));
        require(_tokenBalanceAfter > totalLoanDebt + _ethAmountToCoinbase);

        
        if (_routerTokens[0][0] == address(WBNB)) {
            uint256 _ethBalance = address(this).balance;
            if (_ethBalance < _ethAmountToCoinbase) {
                WBNB.withdraw(_ethAmountToCoinbase - _ethBalance);
            }
        } else {
            // check if this is actually a working solution.
            address[] memory path = new address[](2);
            path[0] = address(_routerTokens[0][0]);
            path[1] = address(WBNB);
            
            // check next line for correct implementation 
            uint256[] memory _amountOutMin = IUniswapV2Router(_dexRouterContractAddress[0]).getAmountsOut(_ethAmountToCoinbase, path);
            IUniswapV2Router(_dexRouterContractAddress[0]).swapExactTokensForTokens(_ethAmountToCoinbase, _amountOutMin[0], path, payable(address(this)), block.timestamp);
            WBNB.withdraw(_amountOutMin[0]);
        }
    ...
    function getAmountOutMin(address _tokenIn, address _tokenOut, uint256 _amountIn, address[] memory _tokens, address _dexRouterContractAddress) external view returns (uint256) {
        ...
        returns (bool)
    {
        ...
        if (cycleType == 0) {
            ...
            ITOKEN(_tokens[0]).approve(address(LENDING_POOL), amountOwing);
        } else if (cycleType == 1) {
            multiCyclicFLParams(amounts[0], params, amountOwing);
            ITOKEN(_routerTokens[0][0]).approve(address(LENDING_POOL), amountOwing);
        }
        
        return true;
    }

    function flashloanAave(address borrowedTokenAddress, uint256 amountToBorrow, bytes memory _params) external permitRole(USER) {
    ...
    // optimise the params for further configuration.
    function flashloanUniswap(address borrowedTokenAddress, uint256 amountToBorrow, bytes memory _params) external permitRole(USER) {
        address pair = IUniswapV2Factory(PANCAKE_V2_Factory).getPair(borrowedTokenAddress, address(WBNB));
