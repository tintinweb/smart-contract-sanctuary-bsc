...
contract PreSalePool is Initializable, Whitelist, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function verifyWhitelist(
        ...
        return (verifySignature(_salePhase, signer, _candidate, _BUSDAmount, _signature));
    }
    ...
    function buyTokenWithExactlyBUSD(
        ...
        require(verifyWhitelist(_salePhase, _candidate, _BUSDAmount, _signature), "POOL: NOT IN WHITELIST");
    ...
    function buyTokenWithExactlyBNB(
        ...
        require(verifyWhitelist(_salePhase, _candidate, msg.value, _signature), "POOL: NOT IN WHITELIST");
        ...
        BUSDToken.transferFrom(msg.sender, salePhaseStatistics[currentSalePhase].treasuryAddress, taxAmount);
        ...
        payable(salePhaseStatistics[currentSalePhase].treasuryAddress).transfer(msg.value);
    ...
    function claimPurchasedToken(uint8 _salePhase) external whenNotPaused nonReentrant {
        ...
        salePhaseStatistics[currentSalePhase].userClaimedAmount[msg.sender] += claimableAmount;
        salePhaseStatistics[currentSalePhase].totalClaimedAmount += claimableAmount;
    ...
    function getTotalSoldAmount(uint8 _salePhase) external view returns (uint256) {
        return salePhaseStatistics[_salePhase].totalSoldAmount;
    }

    function getTotalClaimedAmount(uint8 _salePhase) external view returns (uint256) {
        return salePhaseStatistics[_salePhase].totalClaimedAmount;
    }
...
contract Whitelist {
    function verifySignature(
        ...
        bytes32 messageHash = getMessageHash(_salePhase, _signer, _candidate, _BUSDAmount);
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_salePhase, _signer, _candidate, _BUSDAmount));
    }
