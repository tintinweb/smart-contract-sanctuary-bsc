...
contract DWGameFarmer is DWSubGame {
    ...
    function resolveMint(uint256 _id, address _target) external {
        for (uint256 i = 0; i < mintRequests[_target][_id]; i++) {
            ...
            for (uint256 j = 0; j < wps.length; j++) {
                if (wps[j] / 50 > rarity) rarity = wps[j];
            }
            if (rarity == 5) rarity = 4;
            uint256 durability = generator.generateDurability(seed, rarity, 0);
            farmer.mint(_target, dna, durability, wps);
        }
        // Already minted
    ...
    function addLicense(uint256 _farmerId, uint8 dayIndex) external payable {
        require(
        ...
        game.transferFrom(
            msg.sender,
            game.refillPool(),
            game.licensePrice(dayIndex).mul(
                game.decimalMultiplier().div(peach.getCurrentPrice())
            )
        );
        farmer.setLicensed(_farmerId, game.licenseDays(dayIndex));
    }

    function addLicenses(uint256[] memory _farmerIds, uint8 dayIndex)
        external
        payable
    {
        require(
        ...
        game.transferFrom(
            msg.sender,
            game.refillPool(),
            game.licensePrice(dayIndex).mul(_farmerIds.length).mul(
                game.decimalMultiplier().div(peach.getCurrentPrice())
            )
        );
        for (uint256 i = 0; i < _farmerIds.length; i++) {
            farmer.setLicensed(_farmerIds[i], game.licenseDays(dayIndex));
        }
    }
