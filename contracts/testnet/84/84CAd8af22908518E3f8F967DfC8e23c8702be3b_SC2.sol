...
contract SC2 is Ownable, OverflowToken {
    ...
    uint256 public reward_eABCD_amount = 10**18;
    ...
    function getTotalStakedTHALES() public view returns(uint256){
        uint256 result=0;
        for(uint i=0; i<stake_data.length; i++){
    ...
    function stakeEABCD(uint256 amount, uint256 _type) public {
        ...
        stakeState memory init;
        init.stakeType = _type;
        init.tokenType = 2;
        init.amount = amount;
        init.stakedDate = block.timestamp;
        init.owner = msg.sender;
        stake_data.push(init);
        stakeNum ++;
    ...
    function getRewardInTHALESForOwner( uint256 idx) internal view returns(uint256){
