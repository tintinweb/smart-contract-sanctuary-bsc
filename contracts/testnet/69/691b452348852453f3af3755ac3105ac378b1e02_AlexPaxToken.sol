...
contract AlexPaxToken is ERC20, Ownable {
    ...
    constructor () ERC20("AlexPaxToken", "APT", 18, 2100000 * 10 ** 18){
        address msgSender = _msgSender();
    ...
    function _transfer(address sender, address recipient, uint amount) internal override {
        ...
        bool isAWhiteList = _whiteList[sender] || _whiteList[recipient];
        if (!isAWhiteList) {
            ...
            uint256 fees = amount.mul(jackpotFee.add(addLPFee).add(communityBonusFee).add(lpBonusFee)).div(100);
