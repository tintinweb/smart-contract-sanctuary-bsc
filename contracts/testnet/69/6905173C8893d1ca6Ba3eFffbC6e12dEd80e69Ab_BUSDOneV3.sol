...
contract BUSDOneV3 is Initializable {
    ...
    function claimOldInvestment() external {
        (uint256 claiming, , ) = getClaimableOldInvestment();
    ...
    {
        (, , , , , , , , , , , , staked) = busdone.users(msg.sender);
        (, , totalClaimed, , ) = getStakingInfo(msg.sender);
        if (totalStaked > totalClaimed) {
            uint256 remainingClaimable = (staked - totalClaimed) -
                projectUsers[msg.sender].claimed;
            uint256 servable = ((projectReserves -
                projectUsers[msg.sender].lastReserve) * projectClaimPercent) /
