...
contract DaoSpace is ERC20, Pausable, Ownable, ERC20Permit {
    ...
    // Minimum amount to swap and add liquidity
    uint256 public minAmountToLiquify = 5_000 * 10 ** decimals();
    // Router Address
    ...
    constructor(address _router, address _liquidityWallet, address _marketingWallet) ERC20("DaoSpace", "DAOP") ERC20Permit("DaoSpace") {
        ...
        isExcludedFromMaxTx[_liquidityWallet] = true;
        isExcludedFromMaxTx[_marketingWallet] = true;

        _approve(address(this), address(router), type(uint256).max);
    }

    // Receive BNB from router when swapping
        ...
        override
    {
        ...
        if (from == address(0) || to == address(0)) {
            return; // Minting or burning tokens
        }
    ...
    // override _transfer function to meet tokenomics of DAO SPACE
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        if ((!isDEX[sender] && !isDEX[recipient])) {
            super._transfer(sender, recipient, amount);
        } else if (isExcludedFromFee[sender] || isExcludedFromFee[recipient] || inSwapAndLiquify) {
            super._transfer(sender, recipient, amount);
        } else {
            // Collect fee
            uint256 sendAmount = collectFee(sender, recipient, amount);
    ...
    function collectFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        // Check if is sell
        bool _isSell = isDEX[recipient];
            
        uint256 fee = _isSell ? sellFee : buyFee;
        ...
        uint256 taxAmount = (amount * fee) / FEE_DENOMINATOR;
        ...
        if (_isSell) {
            if (sellFeeCollected >= minAmountToLiquify) {
                uint256 swapAmount = sellFeeCollected;
                sellFeeCollected = 0;
                swapTokensForEth(swapAmount, liquidityWallet);
                // IFeeReceiver(feeReceiver).receiveFee(swapAmount, _isSell);
            }
            sellFeeCollected += taxAmount;
        } else {
            if (buyFeeCollected >= minAmountToLiquify) {
                uint256 swapAmount = buyFeeCollected;
                buyFeeCollected = 0;
                swapTokensForEth(swapAmount, marketingWallet);
                // IFeeReceiver(feeReceiver).receiveFee(swapAmount, _isSell);
            }
            buyFeeCollected += taxAmount;
        }
    ...
    function swapTokensForEth(uint256 _amount, address _feeReceiver) internal lockTheSwap {
    ...
    function excludeFromMaxTx(address _address, bool _isFree) external onlyOwner {
        isExcludedFromMaxTx[_address] = _isFree;
    }

    function excludeFromFee(address _address, bool _isExcluded) external onlyOwner {
        isExcludedFromFee[_address] = _isExcluded;
    }

    function setDEX(address _address, bool _isDEX) external onlyOwner {
    ...
    function withdrawERC20(address _recipient, address _ERC20address, uint256 _amount) external onlyOwner returns (bool) {
        ...
        bool isSent = ERC20(_ERC20address).transfer(_recipient, _amount);
