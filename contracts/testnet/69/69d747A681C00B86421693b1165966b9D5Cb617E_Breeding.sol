...
contract Breeding is ReentrancyGuard, Ownable {
    ...
    function getBreedingItems(address userAddress) public view returns(BreedingVault[] memory) {
        uint256 _blength = breedingVaults.length;
        BreedingVault[] memory _returnValues = new BreedingVault[](_blength);
        uint k = 0;
        for(uint i = 0 ; i < _blength; i++) {
            if(breedingVaults[i].owner == userAddress) {
                _returnValues[k] = breedingVaults[i];
                k += 1;
            }
        }
