...
contract MasterVI is Ownable {
    ...
    function checkPoolId(address lpAddr_) public view returns(uint _pid){
        uint _length = poolLength();
        for (uint i=0; i<=_length; i++) {
            if (poolInfo[i].lpToken == IPair(lpAddr_)){
                _pid = i;
                return _pid;
            }
        }
    ...
    function getPool(uint pid_) public view returns(string memory t0, 
                                                        string memory t1, 
    ...
    function withdraw(uint256 pid_, uint256 amountOut_) public {
        require (pid_ != 0, 'withdraw by unstaking');
    ...
    function closeOrOpenPool(address lpAddr_, bool b) public onlyOwner {
        uint pid = checkPoolId(lpAddr_);
        poolInfo[pid].status = b;
    }
