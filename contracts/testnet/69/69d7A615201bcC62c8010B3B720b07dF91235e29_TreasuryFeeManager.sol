...
contract LpManager {
     ...
     * @return amountWbnbReceived Amount of WBNB received after unwrapping all lp tokens and paying for gas
    ...
    function _swapToWbnb(
        ...
        uint256 amountWbnbReceived,
        uint256 gasCost
    ) {
        uint256 startGas = gasleft();
        gasUsedPerLp = new uint256[](lpData.length);
        amountWbnbReceived = 0;

        // remove liquidity
        for (uint i = 0; i < lpData.length; i++) {
            (address tokenA, address tokenB, uint256 gas) = _removeLiquidity(lpData[i].lpTokenAddress);
            gasUsedPerLp[i] = gas;
            if (lpData[i].unwrapPath0.length == 0 && lpData[i].unwrapPath1.length == 0) {
                revert InvalidPath(i);
            }

            address token0 = lpData[i].unwrapPath0.length == 0 ? WETH : lpData[i].unwrapPath0[0];
            address token1 = lpData[i].unwrapPath1.length == 0 ? WETH : lpData[i].unwrapPath1[0];
            ...
            if (
                (lpData[i].unwrapPath0.length > 0
                && lpData[i].unwrapPath0[lpData[i].unwrapPath0.length - 1] != WETH)
                ||
                (lpData[i].unwrapPath1.length > 0
                && lpData[i].unwrapPath1[lpData[i].unwrapPath1.length - 1] != WETH)
            ) {
                revert InvalidPath(i);
            }
        }
        ...
        uint256 gasAmount = startGas - gasleft() - lpData.length * 10_000 + gasRefund;
        gasCost = gasAmount * tx.gasprice;
        uint256 wbnbBalance = IERC20(WETH).balanceOf(address(this));
        require(
            wbnbBalance > gasCost
            && wbnbBalance - gasCost > minWbnbPerLp * lpData.length,
        ...
        return (gasUsedPerLp, wbnbBalance - gasCost, gasCost);
    }
