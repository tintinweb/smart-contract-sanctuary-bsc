...
contract Token is ERC20, PauseOwners {
	...
	uint256 private minTokenBalanceToLiquify = 10;
	...
	function _transfer(
		...
		bool isInnerTransfer = sender == gameAddress ||
	...
	function takeFees(
		...
		if (
			...
			sender != pair &&
			!inSwapAndLiquify &&
			tokenBalance >= minTokenBalanceToLiquify * (10**decimals())
		) {
	...
	function swapAndTransferWalletFees(
		...
		if (balance > 0) {
			uint256 proportion = (totalFee * 100) /
				(developmentFee / marketingFee);
			uint256 devCut = (balance *
				developmentFee *
				totalFee *
				proportion) / 10000;
			uint256 marCut = (balance * marketingFee * totalFee * proportion) /
				10000;
