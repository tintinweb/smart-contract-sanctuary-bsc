...
contract USBT is Ownable{
    ...
    mapping(uint256 => uint256) public dailyDiamond;
    mapping(uint256 => uint256) public dailyDoubleDiamond;
    ...
    function distributePoolRewards() private {
        ...
        uint256 curDay = getCurDay();   // 4 < 5 => true
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 1000e18){
            ...
            if(total >= 2000e18 && maxTeam >= 25000e18 && otherTeam >= 25000e18){   // max/other 25000
                levelNow = 5;
            }else if(maxTeam >= 10000e18 && otherTeam >= 10000e18){   // other/max 10000
                levelNow = 4;
            }else{
                levelNow = 3;
            }
    ...
    function claimROIReward() public{
        ...
        for(uint256 i = 0; i < orderInfos[msg.sender].length; i++){
            ...
            if(block.timestamp > order.unfreeze && order.isUnfreezed == false)
            {
                ...
                // rewardInfo[msg.sender].statics = (rewardInfo[msg.sender].statics).add(staticReward1[msg.sender][i]);
                break;
            }
        }
    }
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private{
        ...
        for(uint256 i = 0; i < orderInfos[_user].length; i++){
            ...
            if(block.timestamp > order.unfreeze && order.isUnfreezed == false && _amount >= order.amount)
            {
                ...
                // rewardInfo[_user].statics = rewardInfo[_user].statics.add(staticReward1[msg.sender][i]);
    ...
    function _distributediamond() private {
        ...
        if(level4Count > 0){
            uint256 reward = dailyDiamond[curTime-1].div(level4Count);
            uint256 totalReward;
            for(uint256 i = 0; i < level4Users.length; i++){
                if(userInfo[level4Users[i]].level == 4){
            ...
            // if(dailyDiamond[curTime-1] > totalReward){
            //     dailyDiamond[curTime-1] = dailyDiamond[curTime-1].sub(totalReward);
            // }else{
                dailyDiamond[curTime-1] = 0;
            // }
        }
        else{
            dailyDiamond[curTime-1] = 0;
    ...
    function _distributeDirectPool() private {
        ...
        if(directCount > 0){
            ...
            // if(dailyDeposit[curTime-1] > totalReward){
            //     dailyDeposit[curTime-1] = dailyDeposit[curTime-1].sub(totalReward);
            // }
            dailyDeposit[curTime-1] = 0;
    ...
    function _distributedoubleDiamond() private{
        ...
        uint256 curTime = getCurDay();
        if(level5Count > 0){
            uint256 reward = dailyDoubleDiamond[curTime - 1].div(level5Count);
            uint256 totalReward;
            for(uint256 i = 0; i < level5Users.length; i++){
                if(userInfo[level5Users[i]].level == 5){
            ...
            // if(dailyDoubleDiamond[curTime - 1] > totalReward){
            //     dailyDoubleDiamond[curTime - 1] = dailyDoubleDiamond[curTime - 1].sub(totalReward);
            // }else{
            dailyDoubleDiamond[curTime - 1] = 0;
            // }
        }
        else {
            dailyDoubleDiamond[curTime - 1] = 0;
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        BUSD.transfer(founderFee, fee);
        uint256 diamond_ = _amount.mul(diamondPoolPercents).div(baseDivider);
        dailyDiamond[curTime] = dailyDiamond[curTime].add(diamond_);
        // diamond = diamond.add(diamond_);
        uint256 doubleDiamond_ = _amount.mul(doubleDiamondPoolPercents).div(baseDivider);
        dailyDoubleDiamond[curTime] = dailyDoubleDiamond[curTime].add(doubleDiamond_);
        // doubleDiamond = doubleDiamond.add(doubleDiamond_);
        uint256 upline_ = _amount.mul(directsPercentage).div(baseDivider);
        dailyDeposit[curTime] += upline_;
    }
