...
contract ERC20 is Context,IERC20,Ownable{
    ...
    address[] shareholdersOfLP;
    mapping (address => uint256) shareholderIndexesOfLP;

    uint256 currentIndexOfLP;  

    mapping(address => bool) public _updatedOfLP;


    address[] shareholdersOfBenefitNft;
    mapping (address => uint256) shareholderIndexesOfBenefitNft;

    uint256 currentIndexOfBenefitNft;  

    mapping(address => bool) public _updatedOfBenefitNft;

    address[] shareholdersOfShareholderNft;
    mapping (address => uint256) shareholderIndexesOfShareholderNft;

    uint256 currentIndexOfShareholderNft;  

    mapping(address => bool) public _updatedOfShareholderNft;
    ...
    uint256 public numTokensToSell=100*10**18;
    ...
    function _transfer(address from,address to, uint256 amount) private {
        ...
        if (
           ...
           swapTokensForExactTokens(balance);
        }
        bool takeFee = true;
        ...
        if (takeFee){
            ...
            if (from==uniswapV2Pair){
                ...
                if(priceNow<previousPrice){
                    uint spread=previousPrice.sub(priceNow);
                    if(spread.mul(10**18).div(previousPrice)>=10*10**17){
        ...
        if(!address(from).isContract() && from != address(0) ) setShareOfBenefitNft(from);
        if(!address(from).isContract() && from != address(0) ) setShareOfShareholderNft(from);
        if(!address(to).isContract() && to != address(0) ) setShareOfBenefitNft(to);
        if(!address(to).isContract() && to != address(0) ) setShareOfShareholderNft(to);

        if(_balances[address(0x8888888888888888888888888888888888888888)] >= numTokensSellToDividendsFee) {
             processDividendOfBenefitNft(200000);
        }
        if(_balances[address(0x1111111111111111111111111111111111111111)] >= numTokensSellToDividendsFee) {
             processDividendOfShareholderNft(200000);
        }


        if(block.timestamp>=nextUpdateTime&&isCreatePair){
            nextUpdateTime=nextUpdateTime+24 hours;
            previousPrice=priceNow;
...
function processDividendOfShareholderNft(uint256 gas) private {
        uint256 shareholderCount = shareholdersOfShareholderNft.length;

        if(shareholderCount == 0)return;
        uint256 nowbanance = _balances[address(0x1111111111111111111111111111111111111111)];
        uint256 gasUsed = 0;
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            if(currentIndexOfShareholderNft >= shareholderCount){
                currentIndexOfShareholderNft = 0;
            }
            uint256 amount = nowbanance.mul(IERC721(ShareholderNftAddress).balanceOf(shareholdersOfShareholderNft[currentIndexOfShareholderNft])).div(IERC721(ShareholderNftAddress).totalSupply());

            if(_balances[address(0x1111111111111111111111111111111111111111)] < amount)return;
                distributeDividendOfShareholderNft(shareholdersOfShareholderNft[currentIndexOfShareholderNft],amount);
                gasUsed = gasUsed.add(gasLeft.sub(gasleft()));
                gasLeft = gasleft();
                currentIndexOfShareholderNft++;
                iterations++;
            }
    }

    function distributeDividendOfShareholderNft(address shareholder ,uint256 amount) internal {
            _balances[address(0x1111111111111111111111111111111111111111)] = _balances[address(0x1111111111111111111111111111111111111111)].sub(amount);
            _balances[shareholder] = _balances[shareholder].add(amount);
             emit Transfer(address(0x1111111111111111111111111111111111111111), shareholder, amount);
    }

    function setShareOfShareholderNft(address shareholder) private {
           if(_updatedOfShareholderNft[shareholder] ){      
                if(IERC721(ShareholderNftAddress).balanceOf(shareholder) == 0) quitShareOfShareholderNft(shareholder);              
                return;  
           }
           if(IERC721(ShareholderNftAddress).balanceOf(shareholder) == 0) return;  
            addShareholderOfShareholderNft(shareholder);
            _updatedOfShareholderNft[shareholder] = true;
      }
    function addShareholderOfShareholderNft(address shareholder) internal {
        shareholderIndexesOfShareholderNft[shareholder] = shareholdersOfShareholderNft.length;
        shareholdersOfShareholderNft.push(shareholder);
    }
    function quitShareOfShareholderNft(address shareholder) private {
           removeShareholderOfShareholderNft(shareholder);   
           _updatedOfShareholderNft[shareholder] = false; 
      }
    function removeShareholderOfShareholderNft(address shareholder) internal {
        shareholdersOfShareholderNft[shareholderIndexesOfShareholderNft[shareholder]] = shareholdersOfShareholderNft[shareholdersOfShareholderNft.length-1];
        shareholderIndexesOfShareholderNft[shareholdersOfShareholderNft[shareholdersOfShareholderNft.length-1]] = shareholderIndexesOfShareholderNft[shareholder];
        shareholdersOfShareholderNft.pop();
    }


    function processDividendOfBenefitNft(uint256 gas) private {
        
        uint256 shareholderCount = shareholdersOfBenefitNft.length;
        ...
        while(gasUsed < gas && iterations < shareholderCount) {
            if(currentIndexOfBenefitNft >= shareholderCount){
                currentIndexOfBenefitNft = 0;
                
            }
            uint256 amount = nowbanance.mul(IERC721(BenefitNftAddress).balanceOf(shareholdersOfBenefitNft[currentIndexOfBenefitNft])).div(IERC721(BenefitNftAddress).totalSupply());

            if(_balances[address(0x8888888888888888888888888888888888888888)] < amount)return;
                distributeDividendOfBenefitNft(shareholdersOfBenefitNft[currentIndexOfBenefitNft],amount);
                gasUsed = gasUsed.add(gasLeft.sub(gasleft()));
                gasLeft = gasleft();
                currentIndexOfBenefitNft++;
                iterations++;
            }
    }

    function distributeDividendOfBenefitNft(address shareholder ,uint256 amount) internal {
    ...
    function setShareOfBenefitNft(address shareholder) private {
           if(_updatedOfBenefitNft[shareholder] ){      
                if(IERC721(BenefitNftAddress).balanceOf(shareholder) == 0) quitShareOfBenefitNft(shareholder);              
                return;  
           }
           if(IERC721(BenefitNftAddress).balanceOf(shareholder) == 0) return;  
            addShareholderOfBenefitNft(shareholder);
            _updatedOfBenefitNft[shareholder] = true;
      }
    function addShareholderOfBenefitNft(address shareholder) internal {
        shareholderIndexesOfBenefitNft[shareholder] = shareholdersOfBenefitNft.length;
        shareholdersOfBenefitNft.push(shareholder);
    }
    function quitShareOfBenefitNft(address shareholder) private {
           removeShareholderOfBenefitNft(shareholder);   
           _updatedOfBenefitNft[shareholder] = false; 
      }
    function removeShareholderOfBenefitNft(address shareholder) internal {
        shareholdersOfBenefitNft[shareholderIndexesOfBenefitNft[shareholder]] = shareholdersOfBenefitNft[shareholdersOfBenefitNft.length-1];
        shareholderIndexesOfBenefitNft[shareholdersOfBenefitNft[shareholdersOfBenefitNft.length-1]] = shareholderIndexesOfBenefitNft[shareholder];
        shareholdersOfBenefitNft.pop();
    }
    ...
    function _takeShareholderNftDividendsFee(address from,uint256 ShareholderNftDividendsFee) private {
        ...
        _balances[address(0x1111111111111111111111111111111111111111)] = _balances[address(0x1111111111111111111111111111111111111111)].add(ShareholderNftDividendsFee);

        emit Transfer(from, address(0x1111111111111111111111111111111111111111),ShareholderNftDividendsFee);
    }
    ...
    function _takeBenefitNftDividendsFee(address from,uint256 BenefitNftDividendsFee) private {
        ...
        _balances[address(0x8888888888888888888888888888888888888888)] = _balances[address(0x8888888888888888888888888888888888888888)].add(BenefitNftDividendsFee);

        emit Transfer(from, address(0x8888888888888888888888888888888888888888),BenefitNftDividendsFee);
    }
