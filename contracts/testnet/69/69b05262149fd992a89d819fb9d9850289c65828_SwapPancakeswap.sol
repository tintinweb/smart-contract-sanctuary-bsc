...
contract SwapPancakeswap is Ownable{
    ...
    //        swapExactTokensForTokens(...) cannot be called but the BNB(ETH) equivalent instead: swapExactETHForTokens(...)
    //      - 
    ...
    function tradeFromBNBToToken(uint256 amountOutMin, address[] calldata path, uint256 deadline) external payable onlyOwner{       
        _tradeFromBNBToToken(amountOutMin, path, deadline);
    }

    function _tradeFromTokenToToken(uint256 amountIn, uint256 amountOutMin, address[] calldata path, uint256 deadline) private{
    ...
    function previewSwap(address receiverAddress, uint256 amountIn, address[] calldata path) external view returns(uint256) {
        require(path.length >= 2, "Length of path needs to be 2 at least");
        ...
        if (path[0]==path[path.length-1]) {
            return outputTokenBalance - inputTokenBalance;
        } else {
            return 0;
        }
    }
