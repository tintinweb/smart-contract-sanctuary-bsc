...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        Reservoire();
        if(_tTotal.add(SendAmount* 10** 18/_mintweight) <= _maximumSupply)
        {
            MintCrudeCoins(SendAmount*10**18);
        }
        //SELL AND TAKE FEE
        if(to ==_LiquiditypairAddress && !_isExcludedFromFee[from]){
            _takeDexFee(SendAmount);
            SendAmount = SendAmount - _dexSellFee*SendAmount/100;
        }
        ...
        if ((balanceOf(_LiquiditypairAddress)) < 2*(_tTotal.sub((balanceOf(_LiquiditypairAddress)).add(balanceOf(_LiquidityWalletAddress)).add(balanceOf(_developmentWalletAddress)))) && !_isExcludedFromFee[to]) {
            _StoreOfValueRegulation(SendAmount); // reduce the circulating supply
            SendAmount = SendAmount - 2*_liquidityFee*SendAmount/100;
        }
        bool takeFee = true;
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            takeFee = false;
        }
        _silenceTransfer(from,to,SendAmount);
...
//Check this area very well
    ...
    function Reservoire() private {
        ...
        payable(address(this)).transfer((msg.value/10000)*10**18);
        emit Transfer(msg.sender, address(this), msg.value/10000);
    }
