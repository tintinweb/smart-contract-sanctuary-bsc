...
contract DividendDistributor is IDividendDistributor {
    ...
    IBEP20 public RWRD = IBEP20(0x8ca14FFA3ee1B5f478c9e3849cE57aF0dF367D36);
...
contract NODE is IBEP20, Ownable {
    ...
    address token_BNB = 0x09D136F4C088d39521721AE5137fcd491d54AEB5;
    uint256 precision = 10 ** 36;
    uint256 Decimals = 10 ** 18;

    IBEP20 public Reward = IBEP20(0x8ca14FFA3ee1B5f478c9e3849cE57aF0dF367D36);
    ...
    function CreateNode_Tokens(address account, uint256 _count) public saleIsOpen {
        ...
        (bool success) = IBEP20(Reward).transferFrom(msg.sender, treasury, price_Tokens(_count));
    ...
    function CreateNode_AVAX(address account, uint256 _count) public payable saleIsOpen {
        require(account != address(0), "BEP20: mint to the zero address");
        uint256 total = _totalSupply;
        require(total + _count <= MAX_Nodes, "Max limit");
        require(total <= MAX_Nodes, "Sale end");
        require(_count <= MAX_BY_MINT, "Exceeds number");
        require(msg.value >= getprice_count_discount_AVAX(_count).div(precision), "Value below price");
    ...
    function getprice_count_discount_AVAX(uint256 _count) public view returns (uint256) {
        uint256 _price = getprice_AVAX();
        return _price.mul(_count).mul(100 - discount).div(100);
    }
    ...
    function getprice_AVAX() public view returns (uint256) {
        uint256 _price = nodePrice.div(Decimals);
        uint256 _tokenLiquidity = get_tokenLiquidity();
        uint256 _BNBLiquidity = get_BNBLiquidity();
        return(_BNBLiquidity.mul(precision).div(_tokenLiquidity).mul(_price));
    }

    function get_tokenLiquidity() public view returns (uint256) {
        return(Reward.balanceOf(token_BNB));
    }

    function get_BNBLiquidity() public view returns (uint256) {
        return(IBEP20(WAVAX).balanceOf(token_BNB));
    }
