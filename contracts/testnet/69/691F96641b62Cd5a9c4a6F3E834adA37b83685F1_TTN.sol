...
library Address{
 ...
 contract TTN is Context, IERC20, Ownable { //97 : 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 || 0x10ED43C718714eb63d5aA57B78B54704E256024E
    ...
    address public marketingWallet = 0x14EDa3687ef2aed810e4976a9724a6bb8239BA12;
    address public devWallet = 0x324D9eA3F7403C2e07ee833080834E3Bfc8958c2;
    ...
    uint256 public maxBuyLimit = 1_000_000_000 * 10**9;
    uint256 public maxSellLimit = 80_000_000 * 10**9;
    uint256 public maxWalletLimit = 1_000_000_000 * 10**9;

    uint256 private genesis_block;
    
    string private constant _name = "ETH V2";
    ...
    Taxes public buytaxes = Taxes(2, 3, 3, 3);
    Taxes public sellTaxes = Taxes(2, 3, 3, 3);
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _project) public onlyOwner {
       buytaxes = Taxes(_rfi,_marketing,_liquidity,_project);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _project) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_project);
        emit FeesChanged();
    }

    function _reflectRfi(uint256 rRfi, uint256 tRfi) private {
        _rTotal -=rRfi;
        totFeesPaid.reward +=tRfi;
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rProject) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tRfi = tAmount*temp.reward/100;
        s.tMarketing = tAmount*temp.marketing/100;
        s.tLiquidity = tAmount*temp.liquidity/100;
        s.tProject = tAmount*temp.project/100;
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tProject;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        uint256 rProject = s.tProject*currentRate;
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rProject;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rProject) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tProject);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rProject > 0 || s.tProject > 0) _takeProject(s.rProject, s.tProject);
        emit Transfer(sender, recipient, s.tTransferAmount);
        
    }

    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.project) * 2;
        ...
        uint256 projectAmt = unitBalance * 2 * temp.project;
        if(projectAmt > 0){
            payable(devWallet).sendValue(projectAmt);
        }
    }
    ...
    //Use this in case BUSD are sent to the contract by mistake
    function rescueBUSD(uint256 weiAmount) external onlyOwner{
