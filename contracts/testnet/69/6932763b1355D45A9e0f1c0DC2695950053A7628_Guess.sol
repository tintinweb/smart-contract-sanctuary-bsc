...
contract Guess {
    ...
    //     uint amount;
    // }
    mapping(address => uint) public mappingAmountGreaterThan;
    mapping(address => uint) public mappingAmountLessThan;
    address[] listPlayerGreaterThan;
    address[] listPlayerLessThan;
    uint[] listGreaterThanAmount;
    uint[] listLessThanAmount;
    ...
    function getGreaterThanTeam() public returns (address[] memory, uint[] memory) {
        for (uint i = 0 ; i < listPlayerGreaterThan.length; i++) {
            uint betAmount = mappingAmountGreaterThan[listPlayerGreaterThan[i]];
            listGreaterThanAmount.push(betAmount);
        }
        return (listPlayerGreaterThan, listGreaterThanAmount);
    }

    function getLessThanTeam() public returns (address[] memory, uint[] memory) {
        for (uint i = 0 ; i < listPlayerLessThan.length; i++) {
            uint betAmount = mappingAmountLessThan[listPlayerLessThan[i]];
            listLessThanAmount.push(betAmount);
        }
        return (listPlayerLessThan, listLessThanAmount);
    }
}