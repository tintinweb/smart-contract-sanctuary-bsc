...
contract MilkFarmMinerV21 is Context, Ownable {
    ...
    */
    ...
    function setAlgorithmLimits(uint256 _minDaysSell, uint256 _maxDaysSell) public onlyOwner {
    ...
    //CUSTOM (ROI events)//
    //One time event
    uint256 private roiEventBoostPercentage = 0;
    uint256 private roiEventDuration = 1 days;
    uint256 private timestampRoiEventBegins = 0; //when roi event begins, 0 means disabled
    ...
    function getOneTimeEventBoostPercentage(uint256 _currentEventBoostPercentage) public view returns (uint256) {
        ...
        //One time event
        if(timestampRoiEventBegins != 0 && block.timestamp > timestampRoiEventBegins){
            if(block.timestamp < timestampRoiEventBegins.add(roiEventDuration)){
    ...
    function setOneTimeEventBoost(uint256 _roiEventBoostPercentage, uint256 _roiEventDuration, uint256 _timestampRoiEventBegins) public onlyOwner {
    ...
    //Periodic event
    uint256 private roiPeriodicEventBoostPercentage = 0;
    uint256 private roiPeriodicEventDuration = 1 days;
    uint256 private timestampRoiEventPeriodicityBegins = 0; //when periodic events begins, 0 means disabled
    uint256 private roiEventPeriodicity = 7 days;

    function getPeriodicEventBoostPercentage(uint256 _currentEventBoostPercentage) public view returns (uint256) {
        uint256 eventBoostPercentage = _currentEventBoostPercentage;

        //Periodic events
        if(timestampRoiEventPeriodicityBegins != 0 && block.timestamp > timestampRoiEventPeriodicityBegins){
            //Formula to check if we are on event period
            //(currentTimestamp - timestampInit) % (duration + restPeriod) < duration
            if(block.timestamp.sub(timestampRoiEventPeriodicityBegins).mod(roiEventPeriodicity.add(roiPeriodicEventDuration)) < roiPeriodicEventDuration){
    ...
    function setPeriodicEventBoost(uint256 _roiPeriodicEventBoostPercentage, uint256 _roiPeriodicEventDuration, uint256 _timestampRoiEventPeriodicityBegins, uint256 _roiEventPeriodicity) public onlyOwner {
    ...
    mapping(uint256 => uint256) private tvlMilestoneSteps;
    mapping(uint256 => uint256) private tvlMilestoneBoostPercentages;
    uint256 private tvlMilestonesEventDuration = 1 days;
    mapping(uint256 => uint256) private tvlMilestoneStepsTimestampBegin; //when step begins, 0 means still not started
    ...
    function getMilestoneEventBoostPercentage(uint256 _currentEventBoostPercentage) public view returns (uint256) {
        ...
        if(ntvlMilestoneSteps > 0){
            ...
            for(uint256 i = 0; i < ntvlMilestoneSteps; i++){
                if(address(this).balance > tvlMilestoneSteps[i].mul(10 ** 18)){
                    _milestoneBoostPercentage = tvlMilestoneBoostPercentages[i];
                    _stepTimestampBegin = tvlMilestoneStepsTimestampBegin[i];
                }
            }

            if(block.timestamp > _stepTimestampBegin && block.timestamp < _stepTimestampBegin.add(tvlMilestonesEventDuration)){
    ...
    function setMilestoneEventBoost(uint256 [] memory _tvlMilestoneSteps, uint256 [] memory _tvlMilestoneBoostPercentages, uint256 _tvlMilestonesEventDuration, uint256 [] memory _tvlMilestoneStepsTimestampBegin) public onlyOwner {
    ...
    function updateMilestoneEventBoostTimestamp() private {
        for(uint256 i = 0; i < ntvlMilestoneSteps; i++){
            if(address(this).balance > tvlMilestoneSteps[i].mul(10**18)){
    ...
    function updateEventsBoostTimestamps() private {
        updateMilestoneEventBoostTimestamp();
    }

    function applyROIEventBoost(uint256 amount) public view returns (uint256) {
    ...
    function calculateHireMilkers(uint256 eth,uint256 contractBalance) public view returns(uint256) {
        return applyROIEventBoost(calculateHireMilkersNoEvent(eth,contractBalance));
    }
