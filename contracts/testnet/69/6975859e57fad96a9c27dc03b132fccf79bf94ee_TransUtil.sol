...
contract TransUtil is ReentrancyGuard {
  ...
  function setOKTAddress(address _OKTAddress) external onlyGovernance {
    require(_OKTAddress != address(0), "TransUtils: OKT address cannot be 0x0");
    governance = _OKTAddress;

    emit OKTAddressUpdate(_OKTAddress);
  }

  function safeTransfrom(IERC20 _token,address payable[] memory _receiveAddressLists,uint256 _sendAmount) external payable onlyGovernance nonReentrant noContractAllowed {
    require(_receiveAddressLists.length > 0,"TransUtil: empty receive address");
    require(_sendAmount > 0,"TransUtil: zero transfer amount");
    if(address(IERC20(_token)) != OKT) {
        require(IERC20(_token).balanceOf(msg.sender) >= _sendAmount.mul(_receiveAddressLists.length),"TransUtil: not enough balance");
    }else {
        require(address(this).balance >= _sendAmount.mul(_receiveAddressLists.length),"TransUtil: not enough balance");
    }
    for(uint i = 0;i < _receiveAddressLists.length;i++) {
      if (address(IERC20(_token)) != OKT) {
        IERC20(_token).safeTransferFrom(msg.sender, _receiveAddressLists[i], _sendAmount);
      } else {
        _receiveAddressLists[i].transfer(_sendAmount);
      }
    }
  }
}