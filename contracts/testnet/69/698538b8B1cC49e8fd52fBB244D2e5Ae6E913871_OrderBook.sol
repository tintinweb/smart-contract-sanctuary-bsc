...
contract OrderBook is Ownable {
  ...
  function limitSell(uint asset, uint value) public payable {
    ...
    // price = BASE/QUOTE = asset/value

    /* minSellPrice    asset   _quoteDivisor      maxSellPrice
      ------------- <= ----- * -------------- <= -------------
      _priceDivisor    value    _baseDivisor     _priceDivisor */

    require((minSellPrice)*(value * _baseDivisor) <= (asset * _quoteDivisor)*(_priceDivisor), "Price too low");
    require((maxSellPrice)*(value * _baseDivisor) >= (asset * _quoteDivisor)*(_priceDivisor), "Price too high");
  ...
  function limitBuy(uint asset, uint value) public payable {
    ...
    // price = BASE/QUOTE = value/asset

    /* minBuyPrice     value   _quoteDivisor     maxBuyPrice
       ------------ <= ----- * -------------- <= ------------
      _priceDivisor    asset    _baseDivisor    _priceDivisor */

    require((minBuyPrice)*(asset * _baseDivisor) <= (value * _quoteDivisor)*(_priceDivisor), "Price too low");
    require((maxBuyPrice)*(asset * _baseDivisor) >= (value * _quoteDivisor)*(_priceDivisor), "Price too high");    
