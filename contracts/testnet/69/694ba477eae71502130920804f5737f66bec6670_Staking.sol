...
contract ShashiToken is BEP20 {
    ...
    string public constant name = "Shashi Token";
    string public constant symbol = "Shashi";
...
contract Staking is Context, Ownable, ShashiToken {
    using SafeMath for uint256;

    address[] internal stakeholders;

    ShashiToken public Shashi;
    ...
    constructor(ShashiToken _address) public payable {
        Shashi = _address;
    }
    ...
    function createStake(uint256 _numberOfTokens) public payable returns (bool)
    {
        ...
        require(
            Shashi.totalSupply() >=(_numberOfTokens.mul(Shashi.decimalFactor().add(totalTokenStaked))),
            "addition error"
        );
        require(
            _mint(_msgSender(), _numberOfTokens.mul(Shashi.decimalFactor())),
            "mint error"
        );
        stakeholders.push(_msgSender());
        totalTokenStaked = totalTokenStaked.add(
            _numberOfTokens.mul(Shashi.decimalFactor())
        );
        ...
        returns (bool)
    {
        ...
        _burn(_msgSender(), _numberOfTokens.mul(Shashi.decimalFactor()));
        return true;
    }
