...
contract staking is Ownable {
    ...
    function invest(uint256 investment,address reffer) public payable {
        ...
        if(reffer==address(0) || reffer==msg.sender){
            users.refferAddress=msg.sender;   
        }else{
    ...
    function withdrawRewardBUSD()public {
        ...
        totalDevRewardsBUSD+=tax;
        if(investor[msg.sender].refferAddress!=address(this)) investor[investor[msg.sender].refferAddress].refferalRewardsBUSD+=taxR;
    ...
    function withdrawRewardBNB()public {
        ...
        uint256 taxR=totalRewards.mul(withdrawRTax).div(divider);
        totalDevRewardsBNB+=tax;
        if(investor[msg.sender].refferAddress!=address(this)) investor[investor[msg.sender].refferAddress].refferalRewardsBNB+=taxR;
