...
interface IMatchOrdersFeature {
    event Fill(bytes32 indexed orderHash, uint256 amountFinal, uint256 feePaid);
    ...
    function batchMatchOrders(LibNativeOrder.BatchMatches calldata batch)
        external;
...
library LibExchangeRichErrors {
    ...
    function SignatureError(SignatureErrorCodes errorCode, bytes32 hash)
        internal
        pure
        returns (bytes memory)
    {
        return
            abi.encodeWithSelector(SIGNATURE_ERROR_SELECTOR, errorCode, hash);
    }
...
library LibNativeOrder {
    ...
    uint256 private constant BOOL_MASK = (1 << 8) - 1;
    ...
    uint256 private constant _LIMIT_ORDER_TYPEHASH =
        0x282b55c3bcea48035a16f8b775ad1e37579297dcc4eabaadd4be2c1dda518ce7;
        ...
        returns (bytes32 structHash)
    {
        assembly {
            ...
            // order.isBuy;
            mstore(add(mem, 0x40), and(BOOL_MASK, mload(add(order, 0x20))))
            // order.price;
            mstore(add(mem, 0x60), and(UINT_256_MASK, mload(add(order, 0x40))))
            // order.amount;
            mstore(add(mem, 0x80), and(UINT_256_MASK, mload(add(order, 0x60))))
            // order.takerTokenFeeAmount;
            mstore(add(mem, 0xA0), and(UINT_128_MASK, mload(add(order, 0x80))))
            // order.maker;
            mstore(add(mem, 0xC0), and(ADDRESS_MASK, mload(add(order, 0xA0))))
            // order.expiry;
            mstore(add(mem, 0xE0), and(UINT_64_MASK, mload(add(order, 0xC0))))
            // order.salt;
            mstore(add(mem, 0x100), and(UINT_256_MASK, mload(add(order, 0xE0))))
    ...
    NativeOrdersInfo
{
        ...
        returns (LibNativeOrder.MatchedFillResults memory matchedFillResults)
    {
        ...
        if (sellOrder.price > buyOrder.price) {
            _maxPrice = sellOrder.price;
            _minPrice = buyOrder.price;
        } else {
            _minPrice = sellOrder.price;
            _maxPrice = buyOrder.price;
        }
        require(price == _minPrice || price == _maxPrice, "Price incorrect");

        // Get sell & buy order info
        uint256 amountSell = sellOrder.amount;
        uint256 amountBuy = buyOrder.amount;
        LibNativeOrder.OrderInfo memory sellOrderInfo = getOrderInfo(
            sellOrder,
            amountSell
        );
        LibNativeOrder.OrderInfo memory buyOrderInfo = getOrderInfo(
            buyOrder,
            amountBuy
        );

        // Either our context is valid or we revert
        _assertFillableOrder(sellOrder, sellOrderInfo, sellSignature);
        _assertFillableOrder(buyOrder, buyOrderInfo, buySignature);

        LibNativeOrder.MatchOrderInfoPlus memory matchInfo;

        matchInfo.sellOrderFilledAmount = sellOrderInfo.filledAmount;
        matchInfo.buyOrderFilledAmount = buyOrderInfo.filledAmount;
...

        {
            ...
            _updateFilledState(
                ...
                matchedFillResults.amountFinal,
            ...
            _updateFilledState(
                ...
                matchedFillResults.amountFinal.safeMul(price),
    ...
    {
        filled = LibNativeOrdersStorage.getStorage().orderHashToFilledAmount[
            orderHash
        ];
    ...
    function getOrderInfo(
        ...
        orderInfo = getLimitOrderInfoV2(order, amount);
    ...
    function _assertFillableOrder(
        ...
        uint256 hasLocked = LibNativeOrdersStorage.getStorage().orderLocked[
            orderInfo.orderHash
        ];
        if (hasLocked == 0) {
            LibRichErrors.rrevert(
                LibExchangeRichErrors.OrderStatusError(
                    orderInfo.orderHash,
                    LibNativeOrder.OrderStatus(
                        LibNativeOrder.OrderStatus.INVALID
                    )
                )
            );
        }
        ...
        returns (LibNativeOrder.MatchedFillResults memory matchedFillResults)
    {
        ...
        uint256 sellAmountRemaining = sellOrder.amount.safeSub(
            matchInfo.sellOrderFilledAmount
        );
        uint256 buyAmountRemaining = buyOrder.amount.safeSub(
            matchInfo.buyOrderFilledAmount
        );
        // uint256 buyAmountRemaining = matchInfo.buyType ==
        //     LibNativeOrder.MATCH_AMOUNT
        //     ? buyRemaining
        //     : buyRemaining.safeMul(decimal).safeDiv(matchInfo.price);

        if (sellAmountRemaining == buyAmountRemaining) {
            matchedFillResults.amountFinal = sellAmountRemaining;
            // matchedFillResults.takerAmountFinal = sellAmountRemaining
            //     .safeMul(matchInfo.price)
            //     .safeDiv(decimal);
            //fee
            // matchedFillResults.sellFeePaid = LibNativeOrdersStorage
            //     .getStorage()
            //     .orderHashToFeeAmountRemaining[matchInfo.sellOrderHash];
            // matchedFillResults.buyFeePaid = LibNativeOrdersStorage
            //     .getStorage()
            //     .orderHashToFeeAmountRemaining[matchInfo.buyOrderHash];

            // matchedFillResults.realSellFee = sellAmountRemaining
            ...
            //     .safeMul(matchInfo.price)
            //     .safeDiv(buyOrder.amount)
            //     .safeDiv(decimal);
        } else if (sellAmountRemaining > buyAmountRemaining) {
            matchedFillResults.amountFinal = buyAmountRemaining;
            // matchedFillResults.takerAmountFinal = buyAmountRemaining
            //     .safeMul(matchInfo.price)
            //     .safeDiv(decimal);
            // fee
            // matchedFillResults.sellFeePaid = LibNativeOrdersStorage
            ...
            // matchedFillResults.realSellFee = buyAmountRemaining
            //     .safeMul(sellOrder.takerTokenFeeAmount)
            //     .safeDiv(sellOrder.amount);
        ...
        matchedFillResults.returnBuyAmount = matchedFillResults
            .amountFinal
            .safeMul(buyOrder.price.safeSub(matchInfo.price))
            .safeDiv(decimal);
    ...
    /// @param price Second matched order.
    ...
    function _settleMatchedOrders(
        ...
        {
            sendBalanceTo(
                IERC20TokenV06(LibNativeOrdersStorage.getStorage().stableToken),
                sellOrder.maker,
                matchedFillResults.amountFinal.safeMul(price)
            );
        }
        ...
        {
            sendBalanceTo(
                IERC20TokenV06(sellOrder.token),
                buyOrder.maker,
                matchedFillResults.amountFinal
            );
        }
    ...
    function _updateFilledState(
        bytes32 orderHash,
        uint256 amountInTokenFinal,
        uint256 feePaid,
        uint256 realFee,
        uint256 returnAmount,
        uint256 recentAmount
    ) private {
        LibNativeOrdersStorage.getStorage().orderHashToFilledAmount[
                orderHash
        ...
        // LibNativeOrdersStorage.getStorage().orderHashToFeeAmountRemaining[
        //         orderHash
        //     ] = LibNativeOrdersStorage
        //     .getStorage()
        //     .orderHashToFeeAmountRemaining[orderHash]
        ...
        LibNativeOrdersStorage.getStorage().orderLocked[
                orderHash
            ] = LibNativeOrdersStorage
            .getStorage()
            .orderLocked[orderHash]
            .safeSub(amountInTokenFinal)
            .safeSub(realFee)
            .safeSub(returnAmount);

        // Update state

        emit Fill(orderHash, amountInTokenFinal, realFee);
    }
...
abstract contract NativeOrdersInfo is FixinEIP712, FixinTokenSpender {
    ...
    constructor(address stockAddress) FixinEIP712(stockAddress) {
        // solhint-disable no-empty-blocks
    }
        ...
        returns (LibNativeOrder.OrderInfo memory orderInfo)
    {
        ...
        uint256 minValidSalt = LibNativeOrdersStorage
            .getStorage()
            .limitOrdersMakerToMakerTokenToMinValidOrderSalt[order.maker][
                address(order.token)
            ];
        orderInfo.price = order.price;
        orderInfo.isBuyer = order.isBuyer;
        orderInfo.token = order.token;
    ...
    function getLimitOrderRelevantState(
        ...
        returns (
            LibNativeOrder.OrderInfo memory orderInfo,
            uint256 actualFillableTakerTokenAmount,
    ...
    function batchGetLimitOrderRelevantStates(
        ...
        returns (
            ...
            uint256[] memory actualFillableTakerTokenAmounts,
        ...
        )
    {
        ...
        actualFillableTakerTokenAmounts = new uint256[](orders.length);
    ...
    function _populateCommonOrderInfoFields(
        ...
        // Get the filled and direct cancel state.
        {
            ...
            uint256 rawMakerTokenFilledAmount = stor.orderHashToFilledAmount[
                orderInfo.orderHash
            ];
            orderInfo.filledAmount = rawMakerTokenFilledAmount;

            if (orderInfo.filledAmount >= amount) {
    ...
    function _getActualFillableTakerTokenAmount(
    ...
    ) private view returns (uint256 actualFillableTakerTokenAmount) {
        ...
        uint256 fillableMakerTokenAmount = params.orderAmount.safeSub(
            params.orderInfo.filledAmount
        );
        // Convert to taker token amount.
        return fillableMakerTokenAmount.safeMul(params.orderInfo.price);
    }
...
abstract contract FixinEIP712 {
    ...
    constructor(address stockExchange) {
...
import "../interfaces/IEtherTokenV06.sol";
import "../libs/LibSafeMathV06.sol";
...
library LibNativeOrdersStorage {
    ...
    struct Storage {
        ...
        // for limit orders.
        mapping(address => mapping(address => uint256)) limitOrdersMakerToMakerTokenToMinValidOrderSalt;
