...
contract AutlarToken is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    
    string private constant _name = "AutlarToken";
    string private constant _symbol = "AUT";
    ...
    uint256 private _thresholdBalance = 1000000000000000000;
    address private _metatankRewardContract;
    ...
    address payable private _marketingAddress = payable(0xCa6faA4b5e9D5910cec5A42dAC6d6d05CBF10fA1);

    address payable private _liqudityAddress = payable(0xB22466D2f8ceC7EACbB2CF5cc895d3D37415b2b0);
    ...
    constructor() {
       ...
       (uint256 liquidity,uint256 marketing)=getTransferDistribution(_tTotal);
   ...
   function getTransferDistribution(uint _amount)public view returns(uint256 calculateLiquidityFee,uint256 calculateMarketingFee){
        ...
        if (from != owner() && to != owner() && !presale && from != address(this) && to != address(this) && from != bridge && to != bridge) {
            ...
            if (!inSwap && from != uniswapV2Pair) { //sells, transfers
                ...
                if (contractTokenBalance > _thresholdBalance) {
                     if(from != _metatankRewardContract && to != _metatankRewardContract){
