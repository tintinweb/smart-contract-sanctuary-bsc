...
contract LaunchPool {
    ...
    address wBNB = 0xF5755c04e6a31132Fc9c611e0c67877fca2C344a;
    ...
    function _finalize() internal {
        ...
        uint allowanceB = IERC20(tokenB).allowance(msg.sender, address(this));
        ...
        lock(msg.sender, pair, liquidity, lockPeriod);
    }
    ...
    function addLiquidity(
        ...
        require (deadline > block.timestamp, "deadline must greater than block timestamp");
        address factory = PancakeRouter.factory();
    ...
    function getReserveAB() public view returns (uint, uint) {
        (uint reserveA, uint reserveB) = PancakeLibrary.getReserves(0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc, activeCurrency, address(tokenSell));
        return (reserveA, reserveB);
    }

    function getPancakePair() public view returns (address) {
        address pair = IPancakeFactory(0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc).getPair(activeCurrency, address(tokenSell));
        return pair;
    }

    function getAmountOptimal() public view returns (uint, uint) {
        (uint reserveA, uint reserveB) = PancakeLibrary.getReserves(0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc, activeCurrency, address(tokenSell));
        uint amountBOptimal = PancakeLibrary.quote(address(this).balance, reserveA, reserveB);
        uint amountAOptimal = PancakeLibrary.quote(minBuy, reserveB, reserveA);
