...
contract TokenVestingCloneFactoryPaid is Ownable {
    ...
    event CloneCreated(address indexed clone, address indexed creator, address indexed contractOwner);
    ...
    function withdrawFeesNative(uint256 amount) external onlyOwner {
    ...
    function createClone(
        ...
        TokenVestingCloneable(clone).init(_token, _claimTime, _claimPercentage, contractOwner);

        _clones.add(clone);

        // Free for contract owner and other addresses should sent exact fee
        if (msg.sender != owner() && msg.value != fee) revert IncorrectFunds({ sent: msg.value, required: fee });

        emit CloneCreated(clone, msg.sender, contractOwner);
    }
