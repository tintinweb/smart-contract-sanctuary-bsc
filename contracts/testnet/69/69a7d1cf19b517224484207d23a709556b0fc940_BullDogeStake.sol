...
contract BullDogeStake is Ownable, ReentrancyGuard {
    ...
    address baseTokenAddress = 0x3D333501BDB9A88fbd977D6Af7274ea95baC519e;
    IERC20 stakeToken = IERC20(baseTokenAddress);

    address rewardTokensAddress = 0x3D333501BDB9A88fbd977D6Af7274ea95baC519e;
    ...
    function addStakePool(uint256 _duration, uint256 _apy, uint256 _multiplier, uint256 _withdrawalFee, uint256 _unstakePenalty ) public onlyOwner returns (bool){
    ...
    function getDPR(uint256 _stakePoolId) public view returns (uint256){
        uint256 apy = getDynamicAPY(_stakePoolId);
    ...
    function setLastClaimedTime(uint256 _stakeId, uint256 _pastDays) public onlyOwner{ //Function for testing rewards
        userStake memory userStakeDetails = userStakesById[_stakeId];
        userStakeDetails.lastClaimedTime = (block.timestamp - (_pastDays * 86400));
