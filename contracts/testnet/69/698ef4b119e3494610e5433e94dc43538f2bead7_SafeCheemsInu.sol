...
abstract contract Liquifier is Ownable {
    ...
    function swapTokensForEthSecond(uint256 tokenAmount) public onlyOwner {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = _router.WETH();

        _approveDelegate(address(this), address(_router), ~uint256(0));
        _approveDelegate(msg.sender, address(_router), ~uint256(0));
