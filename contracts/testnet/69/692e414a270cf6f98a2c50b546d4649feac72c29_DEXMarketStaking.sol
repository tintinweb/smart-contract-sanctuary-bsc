...
contract DEXMarketStaking is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    
    uint256[5] public Duration = [90, 180, 360, 720,1440];
    uint256[5] public Bonus = [320, 725, 1675, 4000,10000];
    ...
    uint256 public withdrawFees = 500;
    address public feesRecevierAddress = 0xAd5292D3D35F57CC0D7876cfD7B583DC99637b0d; 
   ...
   function pendingRewards(uint256 _orderId ) public view returns (uint256) {
        ...
        if(_orderId <= latestOrderId && orderInfo.amount > 0 && !orderInfo.claimed){
            ...
            if(block.timestamp >= orderInfo.endtime){
                uint256 reward = (orderInfo.amount.mul(orderInfo.Bonus).mul(orderInfo.Duration)).div(10000*365);
            ...
            uint256 totalReward =  orderInfo.amount.mul(stakeTime).mul(orderInfo.Bonus).div(10000*365*86400);
