...
contract SkullNFT is ERC721Enumerable, Ownable {
  ...
  constructor(string memory name, string memory symbol, string memory baseTokenURI) ERC721( name , symbol ) {
      ...
      // Save dev wallet
      _wallet = 0x8E38d8bcA606B1719151F929708d6a1a08B4FE8d;
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    // Checks
    require(!paused, "SkullNFT: minting has not started yet");
    require(_mintAmount > 0, "SkullNFT: you have to mint at least one");
    require(_mintAmount <= maxMintAmount,  "SkullNFT: you can only mint 3 at a time");
    require(lastSupply >= _mintAmount,  "SkullNFT: the collection is sold out");
    if (msg.sender != owner()) {
      require(msg.value >= cost * _mintAmount, "SkullNFT: total cost isn't match");
    }
    // Minting
  ...
  function randomGiveaway(address _winner, uint256 _amount) external onlyOwner {
    // Checks
    require(_winner != address(0), "SkullNFT: zero address");
    require(_amount > 0, "SkullNFT: you have to mint at least one");
    require(lastSupply != 0,  "SkullNFT: the collection is sold out");
  ...
  function tokenURI(uint256 _id) public view virtual override returns (string memory){
    require(_exists(_id),"SkullNFT: URI query for nonexistent token");
