...
contract Game is Ownable {
  ...
  function depositToken(address token, uint256 amount) public{   
    ...
    function withdrawToken(
        ...
        require(
            _isValidAccessMessage(message, signature),
            "Game: Signature incorrect"
        );

        uint256 gameContractBalance = ZODIBO(token).balanceOf(address(this));
        require(gameContractBalance >= amount, "Game: Contract balance is not enough");
        ZODIBO(token).transfer(_msgSender(), amount);
    ...
    function _isValidAccessMessage(bytes32 message, bytes memory signature) internal view returns (bool){
