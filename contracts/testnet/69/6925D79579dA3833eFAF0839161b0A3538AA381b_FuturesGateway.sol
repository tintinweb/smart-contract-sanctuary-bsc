    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        require(_posiChainCrosschainGatewayContract != address(0), Errors.VL_EMPTY_ADDRESS);
...
//        validateOrderQuantity(_positionManager,_quantity);
        ...
        //TODO update calcMarginAndFee function
        uint256 _fee = 0;
        uint256 _margin = 0;
        deposit(_positionManager,msg.sender,_margin,_fee);
    }
...
//        validateOrderQuantity(_positionManager,_quantity);
        ...
        uint256 _fee = 0;
        uint256 tollRatio = uint256(positionManager[_positionManager].tollRatio);
        if (tollRatio != 0) {
...
//        validateOrderQuantity(_positionManager,_uQuantity);
        (uint256 _margin, uint256 _fee) = calcMarginAndFee(_positionManager,_uQuantity,_pip,_leverage);
        deposit(_positionManager,msg.sender,_margin,_fee);
        futuresAdapter.crossBlockchainCall(
            posiChainId,
            posiChainCrosschainGatewayContract,
    ...
    function claimFund(address _pmAddress)
    external
    nonReentrant
    {
        futuresAdapter.crossBlockchainCall(
    ...
    {
        uint256 notional = calcNotional(_manager, pipToPrice(_manager,_pip), _pQuantity);
    ...
    function calcDepositMargin(
        ...
        return _margin * 100 / (100 - positionManager[_manager].assetRfiPercent);
    }
    ...
    function pipToPrice(address _manager ,uint128 _pip) internal view returns (uint256) {
        return (uint256(_pip) * uint256(positionManager[_manager].baseBasicPoint)) / uint256(positionManager[_manager].basicPoint);
    }
...
//        uint256 _quantity
//    ) internal {
//        uint256 minimumQuantity = uint256(positionManager[_manager].minimumOrderQuantity);
...
//    }
    ...
    function setPositionManagerMapping(
        ...
        positionManager[_positionManager].minimumOrderQuantity = _minimumOrderQuantity;
    }

    function setManagerTollRatio(
        address _positionManager,
        uint64 _tollRatio
    ) public onlyOwner{
        require(_positionManager != address(0), Errors.VL_EMPTY_ADDRESS);
    ...
    function updateFuturesAdapterContract(
        ...
        futuresAdapter = CrosschainFunctionCallInterface(_futuresAdapterContract);
    }
