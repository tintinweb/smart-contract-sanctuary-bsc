...
contract Motherfucker is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (
            ...
            uint256 liquidityTokens = contractTokenBalance.mul(_liquidityFee).div(totalTaxForSwap);
            uint256 liqHalf = liquidityTokens.div(2);
            uint256 liqOtherHalf = liquidityTokens.sub(liqHalf);

            uint256 tokensToSwap = liqHalf.add(devTokens).add(marketingTokens);

            uint256 initialBalance = address(this).balance;
            swapTokensForEth(tokensToSwap);
            uint256 newBalance = address(this).balance.sub(initialBalance);
            
            uint256 markEth = newBalance.mul(marketingTokens).div(tokensToSwap);
            uint256 devEth = newBalance.mul(devTokens).div(tokensToSwap);
            uint256 liqEth = newBalance.mul(liqHalf).div(tokensToSwap);
            ...
            addLiquidity(liqOtherHalf, liqEth);

            emit SwapAndLiquify(liqHalf, liqEth, liqOtherHalf);
