...
contract MainstreetNFT is ERC721Enumerable, Ownable, ReentrancyGuard, IERC721Receiver {

    uint public TOKEN_ID = 2000; // starts from the total supply of the previous contract

    address public PREVIOUS_CONTRACT;
    ...
    constructor(uint256 maxNftSupply, uint publicSalePrice, address previousContract, address staker) ERC721("MAINSTREET", "MAINST") {
    ...
    function setStartingTokenIndex(uint index) public onlyOwner {
        TOKEN_ID = index;
    }

    function setPreviousContractAddress(address previousContract) public onlyOwner {
        PREVIOUS_CONTRACT = previousContract;
    }
    ...
    */
    ...
    function claimNFT(uint[] memory tokenIds) public nonReentrant() {
        require(IMinter(PREVIOUS_CONTRACT).balanceOf(msg.sender) > 0, "Balance nill");
        for(uint index = 0; index < tokenIds.length; index++) {
            require(IMinter(PREVIOUS_CONTRACT).ownerOf(tokenIds[index]) == msg.sender, "Not your token");
            IMinter(PREVIOUS_CONTRACT).transferFrom(msg.sender, address(this), tokenIds[index]);
            _safeMint(msg.sender, tokenIds[index]);
        }
    }

    function mintNFT(uint amount) public payable nonReentrant() {
        require(nftPrice != 0, "Minting price not set yet");
        require(bytes(baseTokenURI).length > 0, 'Base URI is not set.');
        ...
        IMainstStaker(STAKER).deposit{value: msg.value}();
    ...
    // mint a specific token function to mint an specific nft for a given address, can be called only by owner
