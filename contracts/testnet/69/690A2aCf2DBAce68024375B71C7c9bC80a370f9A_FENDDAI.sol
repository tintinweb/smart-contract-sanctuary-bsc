...
contract FENDDAI is Ownable, ERC721Burnable, ERC721EnumerableEx {
    ...
    string public baseUrl = "http://sandbox-nft/api/fenddai/"; 
    ...
    // tokenId => details
    mapping(uint256 => Stake) internal deposits;

    event Deposited(uint256 tokenId, uint256 amount, uint256 timestamp, uint8 period, uint16 rate);
    ...
    constructor() ERC721("First ever NFT deposit - DAI", "FENDDAI") {
        numTokens = 0; 
        mintEnabled = true; 
    ...
    function mint(address _to, uint256 _amount, uint256 _timestamp, uint8 _period, uint16 _rate) public onlyAdmin returns (uint256) {
        ...
        deposits[numTokens] = staked; 

        emit Deposited(numTokens, _amount, _timestamp, _period, _rate);

        return numTokens;  
    }

    function getDepositInfo(uint256 tokenId) public view returns(Stake memory) {
        return deposits[tokenId];
    }

    // Enable or disable minting status
