...
contract DexGoNFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    constructor(uint256 networkId, address _gameServer) ERC721("dexGoNFT", "DEXGO") {
        ...
        players = [SHOES0,SHOES2,SHOES2,SHOES3,SHOES4,SHOES5, SHOES6, SHOES7, SHOES8, SHOES9];
        setupType(SHOES0, 0.01 ether, 0, "https://openbisea.mypinata.cloud/ipfs/QmPWngDTtimoYUXkvf2UyaDFGqAh3XseSzJ9CCLbFen4zB/shoes1.json");
        setupType(SHOES1, 0.011 ether, 0, "https://openbisea.mypinata.cloud/ipfs/QmPWngDTtimoYUXkvf2UyaDFGqAh3XseSzJ9CCLbFen4zB/shoes2.json");
        setupType(SHOES2, 0.012 ether, 0, "https://openbisea.mypinata.cloud/ipfs/QmPWngDTtimoYUXkvf2UyaDFGqAh3XseSzJ9CCLbFen4zB/shoes3.json");
        setupType(SHOES3, 0.013 ether, 0, "https://openbisea.mypinata.cloud/ipfs/QmPWngDTtimoYUXkvf2UyaDFGqAh3XseSzJ9CCLbFen4zB/shoes4.json");
        setupType(SHOES4, 0.014 ether, 0, "https://openbisea.mypinata.cloud/ipfs/QmPWngDTtimoYUXkvf2UyaDFGqAh3XseSzJ9CCLbFen4zB/shoes5.json");
        setupType(SHOES5, 0.015 ether, 0, "https://openbisea.mypinata.cloud/ipfs/QmPWngDTtimoYUXkvf2UyaDFGqAh3XseSzJ9CCLbFen4zB/shoes6.json");
        setupType(SHOES6, 0.016 ether, 0, "https://openbisea.mypinata.cloud/ipfs/QmPWngDTtimoYUXkvf2UyaDFGqAh3XseSzJ9CCLbFen4zB/shoes7.json");
        setupType(SHOES7, 0.017 ether, 0, "https://openbisea.mypinata.cloud/ipfs/QmPWngDTtimoYUXkvf2UyaDFGqAh3XseSzJ9CCLbFen4zB/shoes8.json");
        setupType(SHOES8, 0.018 ether, 0, "https://openbisea.mypinata.cloud/ipfs/QmPWngDTtimoYUXkvf2UyaDFGqAh3XseSzJ9CCLbFen4zB/shoes9.json");
        setupType(SHOES9, 0.019 ether, 0, "https://openbisea.mypinata.cloud/ipfs/QmPWngDTtimoYUXkvf2UyaDFGqAh3XseSzJ9CCLbFen4zB/shoes10.json");
    ...
    function setKmForId(uint256 tokenId, uint256 km) public {
        require(msg.sender == gameServer,'DexGoNFT: only server account can change km');
    ...
    function safeMint(address to, string memory uri) public onlyOwner {
        ...
        rentables[tokenId] = RentableItem({
            ...
            percentToShareWei: 0,
            borrower:address (0),
            borrowerChangedLatestTimestamp:0,
            revenue:0
        });
    }
    ...
    function _safeMintType(address to, string memory uri, uint8 typeNft, bool needURI) private {
        ...
        kmLeavesForId[tokenId] = priceForType[typeNft];
        rentables[tokenId] = RentableItem({
            rentable: false,
            percentToShareWei: 0,
            borrower:address (0),
    ...
    function purchase(uint8 typeNft, address referrer) payable public {
        ...
        if (IHandshakeLevels(handshakeLevels).getFullList(msg.sender) == 0) IHandshakeLevels(handshakeLevels).setHandshake(msg.sender, referrer);

        address[] memory friends;
        uint friendsCount;
        uint valueForOwner = msg.value;
        (friends, friendsCount) = IHandshakeLevels(handshakeLevels).getHandshakes(msg.sender);
        for(uint8 x=0;x<friendsCount;x++) {
            if (friends[x] != address (0) && block.timestamp - latestPurchaseTime[friends[x]] < 60 * 60 * 24 * 30) { // must make at least one purchase per months to receive reward
                uint valueForFriend = msg.value * IHandshakeLevels(handshakeLevels).getPercentPerLevelWei(x) / 1 ether;
    ...
    function purchaseBatch(uint8[] memory typesNft, address referrer) payable public {

        uint256 totalValueToPay;
        for(uint256 x=0;x<typesNft.length;x++) {
            totalValueToPay = totalValueToPay + valueInMainCoin(typesNft[x]);
        }
        require(msg.value >= totalValueToPay.sub(100000), "DexGoNFT: wrong value to send");
    ...
    uint256 public minRentalTime = 0.001 ether;
    ...
    function setRentPercentToShare(uint256 _tokenId, uint256 _percentToShareWei) public {
    ...
    function rent(uint256 _tokenId) public payable {
        ...
        payable(owner()).transfer(fixedAmountProject);
