    ...
    function _baseURI() internal view virtual override returns (string memory) {
        ...
        require(supply + _mintAmount <= maxSupply);
            ...
            for (uint256 i = 1; i <= _mintAmount; i++) {
                paytoken.transferFrom(msg.sender, 0xeC1FC34da08C99D7ABfab2c1EcB5318dC9AdeF6C, cost);
                _safeMint(_to, supply + i);
            }
        }

    function mint2(address _to, uint256 _mintAmount, uint256 _pid) public payable {
        TokenInfo storage tokens = AllowedCrypto[_pid];
        IERC20 paytoken;
        paytoken = tokens.paytoken;
        uint256 cost;
        ...
        function mint3(address _to, uint256 _mintAmount, uint256 _pid) public payable {
            TokenInfo storage tokens = AllowedCrypto[_pid];
            IERC20 paytoken;
            paytoken = tokens.paytoken;
            uint256 cost;
