...
contract IrenaUsers is Owned,VerifySignature {
    ...
    function emergencyClaim(uint256 _userCoveredMeters, uint256 _shoePid) internal returns(TransactionReceipt[] memory){
      ...
      require(exactAmount < IBEP20(usdt).balanceOf(address(this)),"Insufficient liquidity to claim reward!");
   ...
   function withdrawIrena() public onlyOwner (){
     address owner = getOwner();
     require(irena.transfer(owner,IBEP20(irena).balanceOf(address(this))),"insufficient irena!");
   }

   function withdrawUsdt() public onlyOwner (){
     address owner = getOwner();
     require(usdt.transfer(owner,IBEP20(usdt).balanceOf(address(this))),"insufficient usdt!");
   }
