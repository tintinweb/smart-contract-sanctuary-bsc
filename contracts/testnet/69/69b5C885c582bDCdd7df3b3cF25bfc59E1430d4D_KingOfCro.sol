...
contract KingOfCro is Auth, IBEP20 {
	...
	function claimPot() external {
		...
		require(block.timestamp - lastCrownChange >= minutesToClaim * 60, "You cannot claim the pot yet.");
		require(balanceOf(msg.sender) >= minForCrown, "You need to own at least the minimum to claim.");
		givePot(msg.sender);
	}

	event PotForTheKing(address indexed king, uint256 amount);
	event RandomWinner(address indexed winner, uint256 amount);
	...
	function givePot(address winner) internal {
		...
		uint256 randomWin = pot / 4;
		if (randomOne != address(0)) {
			payable(randomOne).call{value: randomWin, gas: 34000}("");
			emit RandomWinner(randomOne, randomWin);
		}
		if (randomTwo != address(0)) {
			payable(randomTwo).call{value: pot / 4, gas: 34000}("");
			emit RandomWinner(randomTwo, randomWin);
		}
	}
	...
	function mark(address add, bool set) external authorized {
		safu.mark(add, set);
	}
