...
contract GREENTRANSMIT is Context, Ownable {
    ...
    mapping(address => userBankInfo) public _userBankInfos;//用户银行信息
    ...
    */
    ...
    function getAllUserBankInfo() public view returns (uint inAmount, uint reward){
        for (uint i; i < _maxUserId; i++) {
            inAmount += _userBankInfos[_userAddress[i]].inAmount;
            reward += (_userBankInfos[_userAddress[i]].reward + getUserBankReward(_userAddress[i]));
        }
    }

    //获取用户银行信息
    function getUserBankInfo(address account) public view returns (userBankInfo memory){
        return _userBankInfos[account];
    }
    //获取银行总待领取收益
    function getUserBankReward(address account) public view returns (uint reward){
        if (_userBankInfos[account].inAmount > 0) {
            //获取实际收益
            reward = getDay(block.timestamp).sub(getDay(_userBankInfos[account].claimTime)).mul(_params.userBankReward).div(100).mul(_userBankInfos[account].inAmount);
        }
    }
    //领取本金
    function claimUserBankInAmount(uint amount) public returns (bool){
        ...
        require(_userBankInfos[account].inAmount >= amount && amount > 0);
        _userBankInfos[account].inAmount=_userBankInfos[account].inAmount-amount;
    ...
    function claimUserBankReward() public returns (uint reward){
        ...
        if (_userBankInfos[account].inAmount > 0) {
            //获取实际收益
            reward = getDay(block.timestamp).sub(getDay(_userBankInfos[account].claimTime)).mul(_params.userBankReward).div(100).mul(_userBankInfos[account].inAmount);
            _userBankInfos[account].reward += reward;
            _userBankInfos[account].claimTime = nowTime;
        }
        require(reward > 0);
        //提取利息
        IBEP20(_usdt).transfer(account, reward);
