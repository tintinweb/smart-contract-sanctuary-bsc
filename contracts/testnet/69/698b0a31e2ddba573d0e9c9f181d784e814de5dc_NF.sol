...
contract NF is Context,IERC20Metadata,Ownable {
   ...
   function setLPContractAddress(address _address) onlyOwner public returns (bool success) {
    ...
    if(rate>=10&&rate<30){ //当日币价下跌10%-30% 买手续费 2%，卖手续费4%
       if(msg.sender==lpContractAddress){//接收者为路由地址 buy操作
         require(msg.sender != address(0),"NF: transfer from the zero address ");
         require(amount>0,"NF: amount must be >0");
         ...
         emit Transfer(sender, address(this), amount.mul(4).div(100));     
         }
                
    }else if(rate>=30&&rate<50){//当日币价下跌30%-50% 买手续费 1%，卖手续费5% 
           if(msg.sender==lpContractAddress){//接收者为路由地址 buy操作
         require(msg.sender != address(0),"NF: transfer from the zero address ");
         require(amount>0,"NF: amount must be >0");
         amount = amount.mul(99).div(100);
         emit Transfer(sender, address(this), amount.mul(1).div(100));   
       }   
       else if(recipient==lpContractAddress){
         require(msg.sender != address(0),"NF: transfer from the zero address ");
         require(amount>0,"NF: amount must be >0");
         amount = amount.mul(95).div(100);
         emit Transfer(sender, address(this), amount.mul(5).div(100));   
         }
    }else if(rate>=50){//当日币价下跌50% 买手续费 0%，卖手续费6%
      if(recipient==lpContractAddress){
         require(msg.sender != address(0),"NF: transfer from the zero address ");
         require(amount>0,"NF: amount must be >0");
         amount = amount.mul(94).div(100);
         emit Transfer(sender, address(this), amount.mul(6).div(100)); 
         }
    }
