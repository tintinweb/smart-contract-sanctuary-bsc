...
contract ShareTokenRewardPoolV2 is Operator {
    ...
    function getDaoReward(uint256 _fromTime, uint256 _toTime) internal view returns (uint256) {
        ...
        for (uint256 i = fromMonth; i <= toMonth; ++i) {
            ...
            uint256 timeTo = rewardInfos[i + 1].startTime > _toTime ? _toTime : rewardInfos[i + 1].startTime;
    ...
    function getUserReward(uint256 _pid, address _user, uint256 _fromTime, uint256 _toTime) internal view returns (uint256) {
        ...
        for (uint256 i = fromMonth; i <= toMonth; ++i) {
            ...
            uint256 userUser = user.amount.mul(accRewardTokenPerShare).div(1e18).sub(user.rewardDebt[i]);
            reward = reward + userUser;
            _fromTime = timeTo;
        } 
    ...
    function updatePool(uint256 _pid) internal {
        ...
        if (totalAllocPoint > 0) {
            uint256 _fromTime = pool.lastRewardTime;
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = getUserRewardToClaim(_pid, _sender, lastRewardTime, block.timestamp);
            if (_pending > 0) {
    ...
    function getMonthFrom(uint256 _time) public view returns (uint256) {
        ...
        return (_time - poolStartTime) / MONTH;
    }
}