...
contract AXPE is ERC20 {
    mapping(address => address) public Recommend; //推荐关系 一个地址的上级关系
    mapping(address => address[]) public lowerlevel; //显示一个用户所有的下级
    bool public lock;
    address public OWNER;
    uint public AMOUNT;
    uint public VIPAMOUNT; //参与人数
    ...
    function _transfer(
        ...
        if (Recommend[to] == address(0)) {
            //问题，需要直接转还是需要转一个特定的数字，如果是一个特定的数字的话，
            //就会存在转两次现象
            //无上级并转帐金额为指定数字，则进行绑定上下级关系
            Recommend[to] = from; //绑定上级关系
            VIPAMOUNT++; //人数
            lowerlevel[from].push(to); //统计所有的下级
        }
