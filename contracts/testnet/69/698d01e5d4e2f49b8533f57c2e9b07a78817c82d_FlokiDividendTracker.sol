...
contract ERC20 is Context, IERC20 {
     ...
     * FLOKI and Wei. This is the value {ERC20} uses, unless {_setupDecimals} is
     * called.
     *
...
contract DividendPayingToken is ERC20, Authorizable, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    function distributeDividends(uint256 amount) public onlyAuthorized {
        require(totalSupply() > 0);
...
contract AtomicFloki is ERC20, Ownable, Authorizable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Atomic Floki", "ATOMIC") {
    	flokiDividendTracker = new FlokiDividendTracker();

    	marketingWallet = 0xC571115Ea8616E88334f2E8Cf59a1B6E274543c8;
    	teamWallet = 0x47DcF5A606377F906b26501A4B8cbC1336757f2F;
    ...
    receive() external payable {
  	...
  	function setMaxBuyTransaction(uint256 _maxTxn) external onlyAuthorized {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "AtomicFloki: The marketing wallet is already this address");
        excludeFromFees(_newWallet, true);
        emit MarketingWalletUpdated(marketingWallet, _newWallet);
  	    marketingWallet = _newWallet;
  	}
  	function updateTeamWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != teamWallet, "AtomicFloki: The team wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function setLiquifyEnabledEnabled(bool _enabled) external onlyOwner {
        require(LiquifyEnabled != _enabled, "Can't set flag to same status");
        if (_enabled == false) {
            ...
            totalFees = LiquidityFee.add(marketingFee).add(flokiDividendRewardsFee);
            LiquifyEnabled = _enabled;
        }
    ...
    function setFlokiDividendEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = flokiDividendRewardsFee.add(marketingFee).add(LiquidityFee);
            flokiDividendEnabled = _enabled;
        }
    ...
    function setMarketingEnabled(bool _enabled) external onlyOwner {
        ...
        if (_enabled == false) {
            ...
            totalFees = marketingFee.add(flokiDividendRewardsFee).add(LiquidityFee);
            marketingEnabled = _enabled;
        }
    ...
    function updateFlokiDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(flokiDividendTracker), "AtomicFloki: The dividend tracker already has that address");

        FlokiDividendTracker newFlokiDividendTracker = FlokiDividendTracker(payable(newAddress));

        require(newFlokiDividendTracker.owner() == address(this), "AtomicFloki: The new dividend tracker must be owned by the AtomicFloki token contract");
    ...
    function updateSwapTokensAtAmount(uint newAmount) external onlyOwner {
    ...
    function updateLiquidityFee(uint8 newFee) external onlyOwner {
        LiquidityFee = newFee;
        totalFees = LiquidityFee.add(flokiDividendRewardsFee).add(marketingFee);
    }

    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "AtomicFloki: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "AtomicFloki: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "AtomicFloki: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "AtomicFloki: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "AtomicFloki: Cannot update gasForProcessing to same value");
    ...
    function getAccountFlokiDividendsInfo(address account)
 	...
 	function getAccountFlokiDividendsInfoAtIndex(uint256 index)
  	...
  	function processDividendTracker(uint256 gas) external onlyOwner {
		(uint256 ethIterations, uint256 ethClaims, uint256 ethLastProcessedIndex) = flokiDividendTracker.process(gas);
		emit ProcessedFlokiDividendTracker(ethIterations, ethClaims, ethLastProcessedIndex, false, gas, tx.origin);
        ...
        if (
            ...
            if (!swapping && canSwap) {
                ...
                if (marketingEnabled) {
                    ...
                    uint256 teamPortion = address(this).balance.div(10**2).mul(37);
                ...
                if (LiquifyEnabled) {
                    ...
                    if (buyBackOrLiquidity <= 50) {
                        ...
                        if (buyBackBalance > uint256(1 * 10**18)) {
                            ...
                            uint256 swapTokens = contractTokenBalance.div(totalFees).mul(LiquidityFee);
                            swapTokensForBNB(swapTokens);
                        }
                ...
                if (flokiDividendEnabled) {
                    uint256 sellTokens = swapTokensAtAmount.div(flokiDividendRewardsFee).mul(flokiDividendRewardsFee);
                    swapAndSendFlokiDividends(sellTokens.div(10**2).mul(rand()));
                }
...
contract FlokiDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("AtomicFloki_Flokiereum_Dividend_Tracker", "AtomicFloki_Flokiereum_Dividend_Tracker", 0x2B3F34e9D4b127797CE6244Ea341a83733ddd6E4) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "AtomicFloki_Flokiereum_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "AtomicFloki_Flokiereum_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main AtomicFloki contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait != claimWait, "AtomicFloki_Flokiereum_Dividend_Tracker: Cannot update claimWait to same value");
