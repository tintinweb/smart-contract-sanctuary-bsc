...
contract NFTHeroShopSell is NFTHeroShopBase {
    function sellHero(uint256[] memory id721, uint256[] memory id115, uint256[] memory num1155, uint256 _price, address _coinType) external haveMainPool haveHeroAddress returns(uint256 _groupID) {
        require(_coinType != address(0) && (_coinType == payToken || _coinType == payNFTToken), "cointype error");
        require(id115.length == num1155.length,"1155 len error");
        ...
        require(_num721 + _num1155 <= 100,"sell err");
    ...
    function unSell(uint64 _GroupID) external haveMainPool haveHeroAddress {
