...
contract Game6 is Ownable  {
    ...
    function getTurnOfToken(uint256 _tokenId) external view returns(uint256) {
        if(turnOfToken[_tokenId] == 0 && (timeOfToken[_tokenId] == 0 || block.timestamp - timeOfToken[_tokenId] >= 5 minutes)){
            return 6;
        }else if(turnOfToken[_tokenId] != 0 && block.timestamp - timeOfToken[_tokenId] >= 5 minutes){
            return 6;
        }else{
