...
contract BitDefiInvestments {
            ...
            if(players[up].b5_level[poolNo][0]>2){
                if(players[up].b5Entry[poolNo]>0){
                   players[up].poolIncome[poolNo][0]+=_amount.mul(50).div(100);
                    ...
                    players[feereceiver].poolIncome[0][1]+=_amount.mul(50).div(100);
                }
        ...
        for(uint8 i = 0; i < level_bonuses.length; i++) {
            if(poolNo>=1){
                ...
                if(players[_referral].b5Entry[poolNo]>0){
                    ...
                    players[feereceiver].poolIncome[0][1]+=_refAmount.mul(50).div(100);
                }
            }
            if(isNew==0){
    ...
    function sDetails(address _addr) view external returns(uint256[5] memory stakelevel,uint256 stakeinc,uint256 stakeTt,uint256 sw) {
        ...
        if(userDepts[_addr].length>0){
            ...
            if(totalDays>pl.lastwDays){
                ...
                if(roiDay>0 && pl.stakeTotal>0){
                    myRoi+=pl.stakeTotal.mul(roiDay).div(100);
                }
            }
        }
    ...
    function swithdraw(uint256 _amount) public{
        ...
        require(_amount<=bonus && player.sWithdraw <= player.myLastDeposit.mul(200).div(baseDivider),"Amount exceeds withdrawable");
        
        bdf.transfer(msg.sender,_amount);
        if(player.sWithdraw >= player.myLastDeposit.mul(200).div(baseDivider)){
            player.stakeRoi = 0;
            player.stakeTotal = 0;
            for(uint8 i = 0; i < 5; i++) {
