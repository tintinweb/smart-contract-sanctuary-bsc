...
contract UUXGAME is AccessControlEnumerableUpgradeable,IUUXGAME{
    ...
    //升级事件
    event Uplevel(address indexed to, uint256 indexed level);
    ...
    function _profit(address owner) internal returns (bool) {
        ...
        //     userLevel = _levelStruct[j].level;
        //     if(_levelStruct[j].poolPrice<=0){
        //         continue;
        //     }
        //     levelPrice = _levelStruct[j].poolPrice / count(userAddressLevel[userLevel]);
        //     for(uint256 index=0;index<userAddressLevel[userLevel].length;index++){
        //         //等级加权分红
        //         _addAsset(userAddressLevel[userLevel][index], GOLD_NAME, levelPrice,owner,3,0);
        //     }
        //     countProfit += _levelStruct[j].poolPrice;
        // }
        ...
        while(true){
            ...
            for(uint256 j = 0;j < _levelStruct.length; j++){
                if(userLevel[parentAddress] < _levelStruct[j].level && 
                    ...
                    //     delete userAddressLevel[userLevel[parentAddress]][parentAddress];
                    // }
                    userLevel[parentAddress] = _levelStruct[j].level;
                    //userAddressLevel[_levelStruct[j].level][parentAddress] = 1;
                    emit Uplevel(parentAddress, _levelStruct[j].level);
                }
            }
        }
        return true;
    ...
    * 设置参数
    ...
    function setLevelAddress(address owner, uint256 level) public{
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "error");
        // if(userLevel[owner]>0){
        //     delete userAddressLevel[userLevel[owner]][owner];
        // }
        userLevel[owner] = level;
        //userAddressLevel[level][owner] = 1;
        emit Uplevel(owner, level);
    }
      /**
     * @dev Get 直推列表
     */
