...
contract PlearnLucky is IPlearnLucky, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => mapping(uint256 => uint32)) _confirmedTicketIdsPerRound;

    // Keep track of user ticket ids for a given round Id
    mapping(address => mapping(uint256 => uint256[])) private _userTicketIdsPerRound;
    ...
    event TicketsConfirm(address indexed confirmer, uint256 indexed luckyId, uint256[] ticketIds, uint32[] barckets);
    event TicketsPurchase(address indexed buyer, uint256 indexed luckyId, uint256 numberTickets);
    event TicketsTransfer(address indexed from, address indexed to, uint256[] ticketIds);
    ...
    function buyTickets(uint256 _luckyId, uint256 _numberTickets) external override notContract nonReentrant {
        ...
        require(
            (_userTicketIdsPerRound[msg.sender][_luckyId].length + _numberTickets) <=
        ...
        for (uint256 i = 0; i < _numberTickets; i++) {
            _userTicketIdsPerRound[msg.sender][_luckyId].push(thisTicketId);
            _tickets[thisTicketId] = Ticket({number: thisLuckyNumber, owner: msg.sender});
    ...
    function closeLucky(uint256 _luckyId) external override onlyOperator nonReentrant {
        ...
        _lucks[_luckyId].endTime = block.timestamp;
    ...
    function checkLuckyForTicketId(uint256 _luckyId, uint256 _ticketId) public view returns (bool) {
        // Check lucky is in claimable status
        if (_lucks[_luckyId].status != Status.Claimable) {
            return false;
        }

        // Check ticketId is within range
        if ((_lucks[_luckyId].firstTicketIdNextLottery < _ticketId) && (_lucks[_luckyId].firstTicketId >= _ticketId)) {
            return false;
        }
        ...
        for (uint32 i = 0; i < _lucks[_luckyId].luckyNumbers.length; i++) {
            if (ticketNumber == _lucks[_luckyId].luckyNumbers[i]) {
                isLucky = true;
                break;
            }
        }
    ...
    function confirmTickets(
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            require((bracket - 1) < _lucks[_luckyId].countLuckyNumbersPerBracket.length, 'Bracket too high');
            require(msg.sender == _tickets[ticketId].owner, 'Not the owner');
            require(_confirmedTicketIdsPerRound[_luckyId][ticketId] == 0, 'Ticket is confirmed');

            (, uint32[] memory countAvailablesPerBracket) = getBracketStatus(_luckyId);
            uint32 bracketIndex = bracket - 1;

            require(countAvailablesPerBracket[bracketIndex] != 0, 'Bracket unavailable');
            _confirmedTicketIdsPerRound[_luckyId][ticketId] = bracket;
        }
        ...
        nonReentrant
    {
        ...
        if (_autoAllocateBracket) {
            ...
            for (uint256 i = 0; i < luckyNumbers.length; i++) {
                ...
                _confirmedTicketIdsPerRound[_luckyId][ticketId] = (bracketIndex + 1);
                count++;
    ...
    {
        uint256[] memory luckyTicketIds = _getLuckyTicketIds(_luckyId);
        countLuckyNumbersPerBracket = _lucks[_luckyId].countLuckyNumbersPerBracket;
        countAvailablesPerBracket = countLuckyNumbersPerBracket;

        for (uint256 i = 0; i < luckyTicketIds.length; i++) {
            uint256 ticketId = luckyTicketIds[i];
            if (_confirmedTicketIdsPerRound[_luckyId][ticketId] != 0) {
                uint32 bracketIndex = _confirmedTicketIdsPerRound[_luckyId][ticketId] - 1;
    ...
    function getLuckyNumbers(uint256 _luckyId) external view returns (uint32[] memory) {
        return _lucks[_luckyId].luckyNumbers;
    }

    function getLuckyTickets(uint256 _luckyId) external view returns (Ticket[] memory) {
        Ticket[] memory luckyTickets = new Ticket[](_lucks[_luckyId].luckyNumbers.length);

        for (uint32 i = 0; i < luckyTickets.length; i++) {
            uint256 ticketId = _getTickettIdFromTicketNumber(_luckyId, _lucks[_luckyId].luckyNumbers[i]);
    ...
    function getTicketInfo(uint256 _luckyId, uint256 _ticketNumber)
        external
        view
        returns (
            uint256 id,
    ...
    {
        uint256 ticketId = _getTickettIdFromTicketNumber(_luckyId, _ticketNumber);
        Ticket memory ticket = _tickets[ticketId];
        bracket = _confirmedTicketIdsPerRound[_luckyId][ticketId];
        return (ticketId, ticket.owner, checkLuckyForTicketId(_luckyId, ticketId), bracket, (bracket != 0));
    }
    ...
    function getUserInfoForLuckyId(
        ...
        returns (
            ...
            bool[] memory ticketConfirmatons,
            uint256
        )
    {
        if (_size > (_userTicketIdsPerRound[_user][_luckyId].length - _cursor)) {
            _size = _userTicketIdsPerRound[_user][_luckyId].length - _cursor;
        }

        ticketIds = new uint256[](_size);
        ticketNumbers = new uint32[](_size);
        ticketStatuses = new bool[](_size);
        ticketConfirmatons = new bool[](_size);

        for (uint256 i = 0; i < _size; i++) {
            ticketIds[i] = _userTicketIdsPerRound[_user][_luckyId][i + _cursor];
            ticketNumbers[i] = _tickets[ticketIds[i]].number;
            ticketConfirmatons[i] = _confirmedTicketIdsPerRound[_luckyId][ticketIds[i]] != 0;
            ticketStatuses[i] = false;
            if (_lucks[_luckyId].status != Status.Open) {
                for (uint32 j = 0; j < _lucks[_luckyId].luckyNumbers.length; j++) {
                    if (ticketNumbers[i] == _lucks[_luckyId].luckyNumbers[j]) {
        ...
        return (ticketIds, ticketNumbers, ticketStatuses, ticketConfirmatons, _cursor + _size);
    }
    ...
    function tranferTickets(address _to, uint256[] calldata _ticketIds) external override {
        ...
        emit TicketsTransfer(msg.sender, _to, _ticketIds);
    }
    ...
    function _getLuckyTicketIds(uint256 _luckyId) internal view returns (uint256[] memory ticketIds) {
        ticketIds = new uint256[](_lucks[_luckyId].luckyNumbers.length);
        for (uint256 i = 0; i < _lucks[_luckyId].luckyNumbers.length; i++) {
            ticketIds[i] = _getTickettIdFromTicketNumber(_luckyId, _lucks[_luckyId].luckyNumbers[i]);
        }
    }

    function _getTickettIdFromTicketNumber(uint256 _luckyId, uint256 _ticketNumber) internal view returns (uint256) {
        uint256 firstTicketId = _lucks[_luckyId].firstTicketId;
        uint256 ticketId = firstTicketId + (_ticketNumber % 100000);
        return ticketId;
    }
