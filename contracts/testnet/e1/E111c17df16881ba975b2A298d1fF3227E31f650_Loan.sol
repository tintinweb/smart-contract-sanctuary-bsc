...
contract Loan is LoanAdmin {
    ...
    function setDepositedAddress(address _address) external onlyOwner {
        depositedAddress = _address;
    }
    ...
    function beginLoan(
        ...
        require(ITreasury(depositedAddress).depositedBalanceOf(tx.origin) >= _price, "Please deposit more");
        ...
        ITreasury(depositedAddress).transferFrom(loan.borrower ,_lender, _price);
    ...
    function liquidateOverdueLoan(uint256 _loanId) external nonReentrant {
        ...
        uint256 feeAmount = _computeAdminFee(loanIdToLoan[_loanId].price, loanAdminFeeInBasisPoints);

        ITreasury(depositedAddress).transferFrom(loanIdToLoan[_loanId].lender, feeAddress, feeAmount);
        ITreasury(depositedAddress).transferFrom(loanIdToLoan[_loanId].lender, loanIdToLoan[_loanId].borrower, loanIdToLoan[_loanId].price.sub(feeAmount));
