...
contract DynamicPair is DynamicVoting {
    ...
    function _updateBaseLine(uint32[8] memory _vars, uint112 _reserve0, uint112 _reserve1) internal {

        if (block.timestamp/_vars[uint(Vars.timeFrame)] != lastPeriodCounter) {   //new time frame 

            baseLinePrice0 = uint(UQ112x112.encode(_reserve1).uqdiv(_reserve0)); // current price

            baseLineLP = totalSupply;

            lastPeriodCounter = block.timestamp/_vars[uint(Vars.timeFrame)];
    ...
    function _getFeeAndDumpProtection(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private returns(uint fee0, uint fee1){
        ...
        _updateBaseLine(_vars, _reserve0, _reserve1);  // update base line price and LP
...
*/
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        _updateBaseLine(vars, _reserve0, _reserve1);
    ...
    function burn(address to) public lock returns (uint amount0, uint amount1) {
        ...
        _updateBaseLine(_vars, _reserve0, _reserve1);
