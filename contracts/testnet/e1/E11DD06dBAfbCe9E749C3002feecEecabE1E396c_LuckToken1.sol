...
contract LuckToken1 is Token {
    ...
    function transfer(address _to, uint256 _value) public override returns (bool success) {
        ...
        _balances[msg.sender] = senderBalance.sub(_value);
        _balances[_to] = receiverBalance.add(_value);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        uint256 senderBalance = _balances[msg.sender];
        uint256 fromAllowance = _allowances[sender][msg.sender];
        uint256 receiverBalance = _balances[recipient];
        ...
        require(fromAllowance >= amount, "Not enough allowance");

        _balances[sender] = senderBalance.sub(amount);

        //Ha a vásárló nincs kivételezve adózás alól, akkor levonja az adót.
        //uint256 finalAmount = !isFeeExempt[sender] ? amount.mul(totalBuyFee).div(100) : amount;
        
        _balances[recipient] = receiverBalance.add(amount);
        //TotalFeePayed += finalAmount;


        _allowances[sender][msg.sender] = fromAllowance.sub(amount);
