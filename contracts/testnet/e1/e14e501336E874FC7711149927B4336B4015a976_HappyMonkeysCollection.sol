...
import "./Evolvable.sol";

contract HappyMonkeysCollection is Evolvable, ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, VRFConsumerBaseV2{
    ...
    mapping(uint256 => EvolutionLevel) public availableLevels;
    ...
    function contractURI() public view returns (string memory) {
        EvolutionLevel memory level = availableLevels[0];
        require( level.enabled, "ERROR: No evolution level available" );
        return bytes(level.baseURI).length > 0 ? string(abi.encodePacked(level.baseURI, "metadata.json")) : "";
    }


    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require( _exists(tokenId), "ERROR: URI query for nonexistent token" );
        uint256 currentNftLevel = nftLevel[tokenId];
        EvolutionLevel memory level = availableLevels[currentNftLevel];
        return bytes(level.baseURI).length > 0 ? string(abi.encodePacked(level.baseURI, tokenId.toString(), ".json")) : "";
    }
    ...
    function mintNFT(uint256 numberOfNfts) external payable nonReentrant{
        ...
        splitBalance(numberOfNfts, price);
    ...
    function splitBalance(uint256 numOfNft, uint256 amount) internal {
        uint256 reflectionShare = (amount * reflectionTax) / 100;
        uint256 devShare = (amount * devTax) / 100;
        _reflectDividend(numOfNft, reflectionShare);
        (bool success, ) = admin.call{value: devShare}("");
    ...
    // Reflection
    function _reflectDividend(uint256 numOfNft, uint256 amount) internal {
        reflectionBalance = reflectionBalance + amount;
        totalDividend = totalDividend + (amount / ((totalSupply() + numOfNft)));
    }
    ...
    function getReflectionBalances(address addressToCheck) public view returns (uint256) {
        uint count = balanceOf(addressToCheck);
        uint256 balance = 0;
        for (uint i = 0; i < count; i++) {
            uint tokenId = tokenOfOwnerByIndex(addressToCheck, i);
    ...
    function addEvolvableAddress( address newAddress) external onlyOwner{
        require(evolvableAddress[newAddress] == false, "Evolvable: address already set");
        evolvableAddress[newAddress] = true;
        emit NewEvolvableAddress(newAddress);
    }

    function removeEvolvableAddress( address newAddress) external onlyOwner{
        require(evolvableAddress[newAddress] == true, "Evolvable: address not found");
        evolvableAddress[newAddress] = false;
        emit RemoveEvolvableAddress(newAddress);
    }

    function setNftLevel(uint256 tokenId, uint256 level) external onlyEvolvableAddress{
        require( _exists(tokenId), "ERROR: Set nft level for nonexistent token" );
        require( availableLevels[level].enabled, "ERROR: Trying to set a not available level" );
        nftLevel[tokenId] = level;
        emit NftEvolved(tokenId, level);
    }
    ...
    function setNewEvolutionLevel(uint256 levelId, bool enable, string memory baseUri, string memory name) external onlyOwner {
        availableLevels[levelId] = EvolutionLevel(levelId, enable, baseUri, name);
    }
...
abstract contract Evolvable{
    mapping(address => bool) public evolvableAddress;

    event NewEvolvableAddress(address newAddress);
    event RemoveEvolvableAddress(address newAddress);
    event NftEvolved(uint256 nftId, uint256 newLevel);

    function isEvolvableAddress(address addressToCheck) external view returns(bool) {
        return evolvableAddress[addressToCheck];
    }

    /**
     * @dev Throws if called by any account other than the owner.
     */
    modifier onlyEvolvableAddress() {
        require(evolvableAddress[msg.sender], "Evolvable: caller is not an evolvable address");
        _;
    }
}
