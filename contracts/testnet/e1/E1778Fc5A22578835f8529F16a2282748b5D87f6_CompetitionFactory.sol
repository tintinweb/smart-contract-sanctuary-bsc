...
import "../interface/IGuaranteedCompetitionContractMultiMatches.sol";
...
// CF02: Exceed limit
// CF03: Address 0x00
// CF04: Bet options length invalid
...
contract CompetitionFactory is Ownable, ICompetitionFactory {
    ...
    constructor(
        ...
        address _regular,
        address _guarantee,
        address _regularMulti,
        address _guaranteeMulti,
        ...
        require(
            _p2p != address(0) &&
                ...
                _regularMulti != address(0) &&
                _guaranteeMulti != address(0) &&
        ...
        typeToAddress[CompetitionType.RegularCompetitionMulti] = _regularMulti;
        typeToAddress[CompetitionType.GuaranteedCompetitionMulti] = _guaranteeMulti;
    ...
    function createRegularCompetitionContractMulti(
        address _creator,
        IRegularCompetitionContractMultiMatches.CompetitionMulti memory _competitionInfo,
        Time memory _time,
        uint256 _entryFee,
        uint256 _minEntrant,
        IRegularCompetitionContractMultiMatches.BetOption[][] memory _betOptions,
        address _paymentTokenAddress
    ) external override returns (address) {
        require(_creator != address(0), "CF03");
        require(sportManager.checkSupportedGame(_competitionInfo.sportTypeAlias), "CF01");
        require(_betOptions.length <= limitOption, "CF02"); //need confirm limit by match
        require(_competitionInfo.competitionId.length == _betOptions.length, "CF04");

        address proxyAddress = _createCompetitionProxy(
            typeToAddress[CompetitionType.RegularCompetitionMulti], 
            _creator,
            _paymentTokenAddress
        );
        ...
        IRegularCompetitionContractMultiMatches(proxyAddress).setBasic(
            _time.startBetTime,
            _time.endBetTime,
            _entryFee,
            _minEntrant,
        ...
        IRegularCompetitionContractMultiMatches(proxyAddress).setCompetition(
            _competitionInfo,
            address(sportManager)
        );
        IRegularCompetitionContractMultiMatches(proxyAddress).setOracle(oracle);
        IRegularCompetitionContractMultiMatches(proxyAddress).setBetOptions(_betOptions);
        IRegularCompetitionContractMultiMatches(proxyAddress).setGapvalidationTime(regularGapValidatitionTime);
        return proxyAddress;
    }
    ...
    function createNewGuaranteedCompetitionContractMulti(
        address _creator,
        IRegularCompetitionContractMultiMatches.CompetitionMulti memory _competitionInfo,
        Time memory _time,
        uint256 _entryFee,
        ...
        address proxyAddress = _createCompetitionProxy(
            typeToAddress[CompetitionType.GuaranteedCompetitionMulti], 
            _creator,
            _paymentTokenAddress
        );
        ...
        IGuaranteedCompetitionContractMultiMatches(proxyAddress).setMaxEntrantAndGuaranteedFee(_guaranteedFee, _entrant.maxEntrant);
        IGuaranteedCompetitionContractMultiMatches(proxyAddress).setCompetition(
            _competitionInfo,
            address(sportManager)
        );
        IGuaranteedCompetitionContractMultiMatches(proxyAddress).setOracle(oracle);
        IGuaranteedCompetitionContractMultiMatches(proxyAddress).setBetOptions(_betOptions);
        IGuaranteedCompetitionContractMultiMatches(proxyAddress).setGapvalidationTime(regularGapValidatitionTime);
        IGuaranteedCompetitionContractMultiMatches(proxyAddress).setBasic(
            _time.startBetTime,
            _time.endBetTime,
            _entryFee,
            _entrant.minEntrant,
...
import "../metadata/Metadata.sol";
import "./IRegularCompetitionContractMultiMatches.sol";

interface IGuaranteedCompetitionContractMultiMatches is
    IRegularCompetitionContractMultiMatches
{
...
interface ICompetitionFactory {
    enum CompetitionType {
        ...
        RegularCompetitionMulti,
        GuaranteedCompetitionMulti
    }
