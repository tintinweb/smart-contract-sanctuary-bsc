...
contract ERC20 is Context, IERC20 {
    ...
    /** @dev Creates `amount` tokens and assigns them to `account`, to create starting supply
     * 
    ...
    IDividendPayingTokenOptional
{
    ...
    function setDividendTokenAddress(address newToken) private {
...
contract KingChusky is ERC20, Ownable {
    ...
    uint256 public maxBuyTranscationAmount = 3400000 * (10**9); //0.4%   
    uint256 public maxSellTransactionAmount = 1000000000000 * (10**9);
    uint256 public maxWalletBalance = 6800000 * (10**9); //0.8%   

    KingChuskyDividendTracker public dividendTracker;

    
    address public marketingWallet;
    address public PurchaseBackWallet;
    
    uint256 public swapTokensAtAmount = 850000 * (10**9); //0.1%   

    // buy fees
    uint256 public buyDividendRewardsFee = 4;
    uint256 public buyLiquidityFee = 2;
    uint256 public buyPurchaseBackWalletFee = 1;
    ...
    uint256 public sellPurchaseBackWalletFee = 4;
    ...
    constructor() ERC20("KingChusky", "KCHU") {
        ...
        excludeOrIncludeFromMaxWallet(marketingWallet, true);
        excludeOrIncludeFromMaxWallet(deadAddress, true);
    ...
    {
        require(_isExcludedFrommaxWalletBalance[account] != excluded, "KCHU: Account is already the value of 'excluded'");
    ...
    function isExcludedFrommaxWalletBalance(address account) public view returns(bool) {
        return _isExcludedFrommaxWalletBalance[account];
    }   
    ...
    function updateBuyFees(
        uint256 reward,
        uint256 PurchaseBack,
        uint256 liquidity,
        uint256 burn,
        uint256 marketing
    ) public onlyOwner {
        buyDividendRewardsFee = reward;
        buyPurchaseBackWalletFee = PurchaseBack;
    ...
    function updatePurchaseBackWalletAddress(address newAddress) public onlyOwner {
        excludeFromFees(PurchaseBackWallet, false);
        PurchaseBackWallet = newAddress;
        excludeFromFees(newAddress, true);
    }
      ...
      pinkAntiBot.onPreTransferCheck(from, to, amount);
        ...
        if (tradingIsEnabled && automatedMarketMakerPairs[to]) {
            ...
            if (!swapping && canSwap) {
                ...
                uint256 bnbSwap = buyPurchaseBackWalletFee.add(
                    buyMarketingFee
                );
    ...
    function swapAndSendBnb(uint256 tokenAmount, uint256 totalFee) private {
        ...
        transferToWallet(
            payable(PurchaseBackWallet),
            address(this).balance.div(totalFee).mul(buyPurchaseBackWalletFee)
        );
