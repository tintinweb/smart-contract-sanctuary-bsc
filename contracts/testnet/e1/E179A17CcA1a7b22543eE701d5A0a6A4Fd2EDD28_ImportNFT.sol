    ...
    ERC721Holder
{
    ...
    function _checkOwnerAndApproved(address tokenContract, uint tokenId) private view{
        require(
            IERC721(tokenContract).ownerOf(tokenId) == msgSender(),
            "Trade: The NFT have been transfer away"
        );
        require(
            IERC721(tokenContract).getApproved(
    ...
    function _withdrawNFT(IERC721 tokenContract, uint tokenId) private{
        tokenContract.transferFrom(address(this), _msgSender(), tokenId);
    }

    function Import(address tokenContract, uint tokenId) external checkInTime(){
        require(isInWhiteList(tokenContract), "The tokenContract is not whiteList");
        _checkOwnerAndApproved(tokenContract, tokenId);
        _transferNFT(tokenContract, tokenId);
        emit import_nft(msgSender(), tokenContract, tokenId, block.timestamp);

    }
    
    function ImportBatch(address[] memory tokenContract, uint[] memory tokenId) external checkInTime(){
        require(tokenContract.length == tokenId.length, "tokenContract and tokenId length is different");
        for(uint i = 0 ; i < tokenContract.length ; i++){
            require(isInWhiteList(tokenContract[i]), "The tokenContract is not whiteList");
            _checkOwnerAndApproved(tokenContract[i], tokenId[i]);
            _transferNFT(tokenContract[i], tokenId[i]);
            emit import_nft(msgSender(), tokenContract[i], tokenId[i], block.timestamp);
        }
        
        
        // _checkOwnerAndApproved(tokenId);
        // _createOrder(msgSender(), tokenContract, tokenId, block.timestamp);
        // uint tokenId = _mintToken(_msgSender(), "clubMMember.json");
        // emit Buy(_msgSender(), tokenId, block.timestamp);
    }

    function _transferNFT(address tokenContract, uint tokenId) private{
        IERC721(tokenContract).transferFrom(msgSender(), address(this), tokenId);
    }
    ...
    function _createOrder(
        ...
        orders[orderId].newOrder(
            sender, tokenContract, tokenId, createTime
        );
...
//onlyOwner
    ...
    function addWhiteListContract(address newWhiteList) external onlyRole(MANAGER_ROLE){
    ...
    ERC721URIStorage
{
    ...
    constructor(
        ...
        startApproveList = true;
    }

    function setApproveWhiteList(
        address contractAddress, bool isWhiteList
    ) external onlyRole(MANAGER_ROLE){
        approveWhiteList[contractAddress] = isWhiteList;
    }

    function setStartApproveList(bool _startApproveList) external onlyRole(MANAGER_ROLE){
        startApproveList = _startApproveList;
    }
