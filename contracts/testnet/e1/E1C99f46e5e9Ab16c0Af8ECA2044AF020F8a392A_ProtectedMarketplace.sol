...
contract ProtectedMarketplace {
    ...
    createOrderValidator(_nftInfo.tokenAddress, _nftInfo.nftTokenId, _tokenPrice, _acceptOffers, _offerClosingTime)
    {
        require(_protectionArgs.protectionRate <= 10000 , "ProtectedMarketplace::createOrder: Protection rate above 100 percent");
        IERC721(_nftInfo.tokenAddress).safeTransferFrom(msg.sender, address(this), _nftInfo.nftTokenId);
        ...
        order.tokenAddress = _nftInfo.tokenAddress;
        order.nftTokenId = _nftInfo.nftTokenId;
        order.isDerivativeNft = derivativeFactory.isDerivative(_nftInfo.tokenAddress);
        order.sellerAddress = payable(msg.sender);
        // order.buyerAddress = payable(address(0));
        // order.tokenPrice = _tokenPrice;
        // order.useDepository = _protectionArgs.useDepository;
        // order.protectionRate = _protectionArgs.protectionRate;
        // order.isFixedProtection = _protectionArgs.isFixedProtection;
        // order.protectionTime = _protectionArgs.protectionTime;
    ...
    function createOffer(
        ...
        offer.tokenAddress = _nftInfo.tokenAddress;
        offer.nftTokenId = _nftInfo.nftTokenId;
        offer.isDerivativeNft = derivativeFactory.isDerivative(_nftInfo.tokenAddress);
