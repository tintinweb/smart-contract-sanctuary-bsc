...
contract BrewlabsTreasury is Ownable {
    ...
    address[] public bnbToBrewsPath;
    uint256 public slippageFactor = 830; // 17%
    uint256 public constant slippageFactorUL = 995;


    event BuyBack(uint256 eth);
     ...
     * @param _token: BREWLABS address
     * @param _dividendToken: reflection token address
     * @param _uniRouter: uniswap router address for swap tokens
     * @param _bnbToBrewsPath: swap path to buy BREWLABS
     */
    function initialize(
        IERC20 _token,
        address _dividendToken,
        address _uniRouter,
        address[] memory _bnbToBrewsPath
    ) external onlyOwner {
        ...
        bnbToBrewsPath = _bnbToBrewsPath;
    }

    /**
     * @notice Buy brewlabs token from BNB
     */
    function buyBack() external onlyOwner {
        uint256 ethAmt = address(this).balance;
        ethAmt = ethAmt.mul(buybackRate).div(100);

        if(ethAmt > 0) {
            _safeSwapEth(ethAmt, bnbToBrewsPath, address(this));
    ...
    /**
     * @notice Harvest reflection for BREWLABS
     */
     ...
     */
    function setBuybackRateage(uint256 _percent) external onlyOwner {
        require(_percent < 10000, "Invalid percentage");
    ...
    function recoverWrongTokens(address _token) external onlyOwner {
        require(_token != address(token) && _token != address(dividendToken), "Cannot be token & dividend token");
