    ...
    if (offersSigned[nonce_][left] == offersSigned[nonce_][right] == true) {
      ...
      for (uint256 i = 0; i < tokensLeft.length; i++) {
        if (!_isValidERC721(tokensLeft[i].getAddress())) {
          IERCSimplified(tokensLeft[i].getAddress()).approve(
            address(this),
            tokensLeft[i].getProp()
          );
        }
      ...
      for (uint256 i = 0; i < tokensRight.length; i++) {
        if (!_isValidERC721(tokensLeft[i].getAddress())) {
          IERCSimplified(tokensRight[i].getAddress()).approve(
            address(this),
            tokensRight[i].getProp()
          );
        }
  ...
  function cancelOffer(bytes32 nonce_) external returns (bytes32) {
    ...
    if (offersSigned[nonce_][offer.left]) {
      for (uint256 i = 0; i < offer.tokensLeft.length; i++) {
        ...
        IERCSimplified token = IERCSimplified(offer.tokensLeft[i].getAddress());
        if (!_isValidERC721(address(token))) {
          token.approve(address(this), offer.tokensLeft[i].getProp());
        }
        token.transferFrom(
          address(this),
          offer.left,
    ...
    if (offersSigned[nonce_][offer.right]) {
      for (uint256 i = 0; i < offer.tokensRight.length; i++) {
        ...
        IERCSimplified token = IERCSimplified(
          offer.tokensRight[i].getAddress()
        );
        if (!_isValidERC721(address(token))) {
          token.approve(address(this), offer.tokensRight[i].getProp());
        }
        token.transferFrom(
          address(this),
          offer.right,
...
interface IERCSimplified {
  ...
  function approve(address to, uint256 prop) external;
}
