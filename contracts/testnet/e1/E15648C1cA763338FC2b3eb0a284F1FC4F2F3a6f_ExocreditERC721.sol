    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    mapping(uint256 => SharedStructs.ExoInfo) public exoInfoMap;
    mapping(uint256 => uint256) public exoValuesMap;
    mapping(uint256 => SharedStructs.RequestData) public randomRequestMap;
    mapping(uint8 => SharedStructs.RangesByTier) public rangesMap;
    ...
    function initialize(
        ...
        __ERC721_init("ExocreditERC721", "ExoNFT");
        ...
        rangesMap[1] = SharedStructs.RangesByTier({minRange: 0, maxRange: 4});
        rangesMap[2] = SharedStructs.RangesByTier({minRange: 5, maxRange: 9});
        rangesMap[3] = SharedStructs.RangesByTier({minRange: 10, maxRange: 14});
        rangesMap[4] = SharedStructs.RangesByTier({minRange: 15, maxRange: 19});
        rangesMap[5] = SharedStructs.RangesByTier({minRange: 20, maxRange: 24});
        rangesMap[6] = SharedStructs.RangesByTier({minRange: 25, maxRange: 30});
    }
    ...
    function setExoValuesMap(uint256[] memory _index, uint256[] memory _value)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _index.length; i++) {
            exoValuesMap[i] = _value[i];
        }
    }

    function lootBoxMint(address _to, uint8 _tier)
        public
        onlyRole(MINTER_ROLE)
    {
        ...
        SharedStructs.ExoInfo storage exoInfo = exoInfoMap[_tokenId];
        exoInfo.firstOwner = _to;
    ...
    function fulfillRandomWords(
        ...
        uint16 module = (range.maxRange - range.minRange) + 1;
        string memory _tokenURI = string(
            abi.encodePacked(
                baseURI,
                "_",
                StringsUpgradeable.toString(
                    (randomWords[0] % module) + range.minRange
                ),
                ".json"
            )
        );
        SharedStructs.ExoInfo storage exoInfo = exoInfoMap[requestData.tokenId];
        exoInfo.value = exoValuesMap[
            (randomWords[0] % module) + range.minRange
        ];
