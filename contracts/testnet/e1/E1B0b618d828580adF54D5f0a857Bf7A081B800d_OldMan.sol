...
contract OldMan {
    ...
    mapping(address => BetInfo[]) public playerBetDetail;
    ...
    function getPlayerBetDetail(address _player)
        external
        view
        returns (BetInfo[] memory)
    {
        return playerBetDetail[_player];
    }
        ...
        notContract
    {
        ...
        if (betted[msg.sender] == true) {
            playerBetDetail[msg.sender][_teamId].totalBetAmount += betAmount;
        } else {
            for (uint256 i = 0; i <= 8; i++) {
                playerBetDetail[msg.sender].push(
                    BetInfo(msg.sender, i, 0, block.timestamp, feeRatio)
                );
            }
        ...
        emit NewBet(msg.sender, _amount, _teamId, feeRatio);
    }
    ...
    function claim() external notContract {
        ...
        // Check if win
        require(
            playerBetDetail[msg.sender][answer].totalBetAmount > 0,
            "No Rewards need to claime"
        );

        uint256 reward;
        reward =
            (playerBetDetail[msg.sender][answer].totalBetAmount *
                totalBetMoney) /
