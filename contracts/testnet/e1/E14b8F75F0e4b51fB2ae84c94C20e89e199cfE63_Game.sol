...
contract Game is Ownable,Pausable,ReentrancyGuard{
    ...
    uint256[QUOTA-1] public rewardRates = [833,833,833,833,833,833,833,1600,2500];//奖励率
    ...
    constructor(IERC20 ft_,IERC20 ald_,IFTFarm ftFarm_,IERC20 usdt_,IUniswapV2Router02 uniswapV2Router_,address teamRewardWallet_){
        ald = ald_;
        usdt = usdt_;
        ft = ft_;
        ftFarm = ftFarm_;
        uniswapV2Router = uniswapV2Router_;
        teamRewardWallet = teamRewardWallet_;
    }
    ...
    function _lottery(uint256 _phase,uint256 _amount) private {
        ...
        for(uint256 i=0;i<QUOTA;i++){
            ...
            if(account != winAddress){
                uint256 reward = _amount.mul(randomRewardRates[loseIndex]).div(1000);
    ...
    function _getRandomRewardRates()private view returns(uint256[QUOTA-1] memory result){
        result = _swapOrder(result,QUOTA-2);
        result = _swapOrder(rewardRates,QUOTA-3);
    }
    ...
    function _random(uint256 _number)  public view returns(uint) {
