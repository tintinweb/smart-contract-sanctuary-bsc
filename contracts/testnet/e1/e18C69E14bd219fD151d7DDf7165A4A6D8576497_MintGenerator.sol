...
contract MintGenerator is Ownable {
    ...
    function createToken (
        ...
        IERC20 newToken = new NonMintableToken(name, symbol, decimals, payable(msg.sender), totalSupply);
...
contract MintFactory is Ownable {
    ...
    /**
     * @notice returns true if the token is a valid NonMintableToken token generated by the AvaStation
     */
    function isNonMintableToken(address _tokenAddress) external view returns (bool) {
        return tokens.contains(_tokenAddress);
    }
    
    /**
     * @notice The length of all valid NonMintableToken tokens on the platform
     */
    ...
    /**
     * @notice gets an NonMintableToken token at a specific index. Although using Enumerable Set, since tokens are only added and not removed, indexes will never change
     * @return the address of the NonMintableToken token at index
     */
...
contract TokenFees is Ownable{
    ...
    constructor(address _tokenFee) {
        SETTINGS.FLAT_FEE = 5 * 1e17;
        SETTINGS.TS_FEE = 1;
        SETTINGS.TOKEN_FEE_ADDRESS = payable(_tokenFee);
    }
