...
contract AnnexNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function changeSellingStatus(uint256 _tokenId, uint status, uint256 _newPrice, address nft, uint256 nftId, address creator, bytes32 hash, bytes memory signature) public {
        address signer = hash.recover(signature);
        // require(signer == msg.sender && _nftData.creator == msg.sender, "Invalid creator signature");
        tokenInfo memory tokenInfoById = allTokensInfo[nftId];
        if(nft == address(this)) {
            ...
            // address nftOwner = checkNFTOwner(nft, _tokenId);
            // require(nftOwner != msg.sender, "Not the owner 2");
    ...
    function transferByAcceptOffer(uint256 tokenId, uint256 nftId, address newOwner, address nft, bytes32 hash, bytes memory signature, uint payThrough, uint256 amount) external payable onlyNftOwner(nftId) {
        // address signer = hash.recover(signature);
        // require(signer == msg.sender, "Invalid User signature");
        require(bidsOffers[nftId][newOwner] > 0, "Offer does not exist");
        require(amount == bidsOffers[nftId][newOwner], "Invalid amount");
        tokenInfo memory tokenInfoById = allTokensInfo[nftId];
        ...
        if(PLATFORM_SHARE_PERCENT > 0) {
            ...
            if(payThrough==1) {
                transferAnnToOwner(newOwner, address(this), platformSharePercent);
            }
            else {
                transferwbnbToOwner(newOwner, address(this), platformSharePercent);
            }
        }
        if(ROYALTY_PERCENT > 0) {
            uint256 royaltyPercent = calculatePercentValue(amountToTransfer, PLATFORM_SHARE_PERCENT);
            amountToTransfer = amountToTransfer-royaltyPercent;
            if(payThrough==1) {
                transferAnnToOwner(newOwner, tokenInfoById.creator, royaltyPercent);
            }
            else {
                transferwbnbToOwner(newOwner, tokenInfoById.creator, royaltyPercent);
            }
        }
        if(payThrough==1) {
            transferAnnToOwner(newOwner, msg.sender, amountToTransfer);
        }
        else {
            transferwbnbToOwner(newOwner, msg.sender, amountToTransfer);
        }
        if(nft == address(this)) {
            _transfer(msg.sender, newOwner, tokenId);
        }
        else {
            transferNFT(nft, msg.sender, newOwner, tokenId);
        }
        emit OfferAccepted(tokenId,bidsOffers[nftId][newOwner],msg.sender,newOwner);
    }
    function transferByBid(uint256 tokenId, uint256 nftId, address newOwner, address nft, bytes32 hash, bytes memory signature, uint payThrough, uint256 amount) external payable onlyNftOwner(nftId) {
        // address signer = hash.recover(signature);
        // require(signer == msg.sender, "Invalid User signature");
        require(bidsOffers[nftId][newOwner] > 0, "User did not palce bid");
        require(amount == bidsOffers[nftId][newOwner], "Invalid amount");
        tokenInfo memory tokenInfoById = allTokensInfo[nftId];
        require(tokenInfoById.selling > 0, "NFT not available for sale");
        tokenInfoById.currentOwner = payable(newOwner);
        ...
        emit BidAccepted(tokenId,bidsOffers[nftId][newOwner],msg.sender,newOwner);
    }
