...
contract PoolMaster3 {
    ...
    event SlaveCreated(address claimToken, address stakeToken, address paymentToken, address pool);

    function createPoolIDO3(
      address _devWallet,
			address _idoOwner,
			address _claimToken,
			address _stakeToken,
      ...
      address _owner
        ...
        bytes memory bytecode = type(PoolIDO3).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(block.timestamp));
        assembly {
            pool := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        PoolIDO3(pool).initialize(_devWallet, _idoOwner, _claimToken, _stakeToken, _paymentToken);
        PoolIDO3(pool).setup(_payAmountPerSlot, _tokenPerSlot, _totalOfSlot);
        PoolIDO3(pool).transferOwnership(_owner);
        pools.push(Pool(3, pool));

        emit SlaveCreated(_claimToken, _stakeToken, _paymentToken, pool);
    }
}