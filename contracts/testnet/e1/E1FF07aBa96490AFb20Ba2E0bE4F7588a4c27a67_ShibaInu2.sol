...
contract ShibaInu2 is ERC20, Ownable {
    ...
    uint256 public  buybackburnFeeOnBuy   = 1;
    uint256 public  buybackburnFeeOnSell  = 1;
    ...
    address public buybackWallet = 0x1A3B047d1215557588253b58dC27b20e9106DbeF;
    ...
    address public rewardToken = 0x802050B036Fa8384521a40EB36E5ACfCE269D992;
    uint256 public gasForProcessing = 300000;

    
    event ExcludeFromFees(address indexed account, bool isExcluded);        
    event UpdateBuyFees(uint256 buybackburnFeeOnBuy, uint256 marketingFeeOnBuy, uint256 rewardFeeOnBuy);
    event UpdateSellFees(uint256 buybackburnFeeOnSell, uint256 marketingFeeOnSell, uint256 rewardFeeOnSell);
    event MarketingWalletChanged(address marketingWallet);
    event buybackWalletChanged(address buybackWallet);        
    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 bnbReceived, uint256 tokensIntoLiqudity);
    event Sendbuybackburn(uint256 tokensSwapped);
    event SendMarketing(uint256 tokensSwapped);
    //event SendMarketing(uint256 currencySend);
    ...
    {
        address newOwner = 0x25dB81039B53EdDDCF99705Fb093693938180Ad0;
        transferOwnership(newOwner);

        _mint(owner(), 200_000_000_000 * (10 ** 18));
        swapTokensAtAmount = totalSupply() / 50000;

    	dividendTracker = new DividendTracker(200_000_000_000 / 50000, rewardToken);
    ...
    function updateBuyFees(uint256 _buybackburnFeeOnBuy, uint256 _marketingFeeOnBuy, uint256 _rewardFeeOnBuy) external onlyOwner {
        require(
    ...
    function _transfer(
        ...
        require(tradingEnabled,"Not launched yet");
        ...
        if( canSwap &&
            ...
            uint256 buybackburnShare = buybackburnFeeOnBuy + buybackburnFeeOnSell;
            uint256 marketingShare = marketingFeeOnBuy + marketingFeeOnSell;
            uint256 rewardShare    = rewardFeeOnBuy    + rewardFeeOnSell;

            

            uint256 bnbShare = marketingShare + rewardShare + buybackburnShare;
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(buybackburnShare > 0) {
                    uint256 buybackburnBNB = newBalance * buybackburnShare / bnbShare;
                    swapAndSendbuybackburn(buybackburnBNB);
                }
            }
...
//=======Swap=======//
    ...
    function swapAndSendbuybackburn(uint256 amount) private{
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = Currency;
        ...
        uint256 buybackburnTokens = IERC20(Currency).balanceOf(address(this));
        bool success = IERC20(Currency).transfer(address(buybackWallet), buybackburnTokens);

        if (success) {
            emit Sendbuybackburn(buybackburnTokens);
        }
    }
    ...
    function swapAndSendMarketing(uint256 amount) private{
        ...
        uint256 marketingTokens = IERC20(Currency).balanceOf(address(this));
        bool success = IERC20(Currency).transfer(address(marketingWallet), marketingTokens);
