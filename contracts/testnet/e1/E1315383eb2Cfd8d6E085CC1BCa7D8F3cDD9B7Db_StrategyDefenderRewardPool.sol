...
abstract contract FeeManager is StratManager {
    ...
    uint constant public MAX_WITHDRAWAL_FEE = 50;   // 0.50%

    uint constant public MAX_FEE = 1000;            // 10.00%
    uint constant public MAX_CALL_FEE = 111;        // 11.10% of MAX_FEE
    uint constant public STRATEGIST_FEE = 112;      // 11.20% of MAX_FEE

    uint public totalPerformanceFee = 450; // 4.50%
    uint public withdrawalFee = 10;
    uint public callFee = 111;
    uint public defFee = MAX_FEE - STRATEGIST_FEE - callFee;

    function setCallFee(uint _fee) public onlyManager {
        require(_fee <= MAX_CALL_FEE, "FeeManager: MAX_CALL_FEE");
        callFee = _fee;
        defFee = MAX_FEE - STRATEGIST_FEE - callFee;
    }

    function setWithdrawalFee(uint _fee) public onlyManager {
        require(_fee <= MAX_WITHDRAWAL_FEE, "FeeManager: MAX_WITHDRAWAL_FEE");
        withdrawalFee = _fee;
    }

    function setTotalPerformanceFee(uint _fee) public onlyManager {
        require(_fee <= MAX_FEE, "FeeManager: MAX_FEE");
...
contract StrategyDefenderRewardPool is FeeManager {
    ...
    function setHarvestOnDeposit(bool _harvestOnDeposit) external onlyManager {
        ...
        setWithdrawalFee(harvestOnDeposit ? 0 : 10);
    }
...
contract StratManager is Operator, Pausable {
    ...
    modifier onlyEOA() {
        require(msg.sender == tx.origin, "StratManager: IS_NOT_EOA");
        _;
    }
    ...
    function setStrategist(address _strategist) external {
        require(msg.sender == strategist, "StratManager: IS_NOT_STRATEGIST");
