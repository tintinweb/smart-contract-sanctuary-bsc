...
contract SUB {
    uint public CarouselIndex;
    Carousel[10] public CarouselList;
    mapping(address => uint) public JoinLogIndex;
    mapping(address => JoinLog[]) public JoinLogList;
    mapping(uint => uint) public WinLogIndex;
    mapping(uint => WinLog[10]) public WinLogList;
    struct Carousel {
        address user;
        uint amount;
        uint gameType;
