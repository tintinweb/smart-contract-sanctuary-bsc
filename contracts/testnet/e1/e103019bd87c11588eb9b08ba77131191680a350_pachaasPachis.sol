...
contract pachaasPachis is owned {
    ...
    function buyLevel(uint _referrerID, uint _level) public payable returns(bool)
    {
        require(userInfos[msg.sender][_level-1].joined, "buy previous level");
        require(isContract(msg.sender) ==0, "Contract can't call");
        address referredBy = userAddressByID[_referrerID][_level];
        require(userInfos[referredBy][_level].joined, "referrer does not in this level");
        ...
        emit registerEv(msg.sender,_level,temp.id, referredBy, now);
    ...
    function payForLevel(uint _parentID, uint _level, uint _amount) internal returns(bool)
    {
        ...
        while(paidSection < 4)
        {
            ...
            (_parent, tf, h) = findEligibleToPayNext(_parent,_level, h);
        ...
        if(userInfos[_parent][_level].paymentCount == 12 ) 
        {
    ...
    function findEligibleToPayNext(address _user,uint _level, bool fourth) public view returns(address, uint, bool) // id, payCount, payHold
    {
        uint _pid = userInfos[_user][_level].parentID;
        if(!fourth)
        {
