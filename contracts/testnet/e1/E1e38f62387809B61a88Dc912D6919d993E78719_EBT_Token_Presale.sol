...
contract EBT_Token_Presale is Ownable {
    ...
    uint256 PhaseOne = 85000000000000; //(100M to 75M) 1 EBT = 0.000085 BNB
    uint256 PhaseTwo = 102000000000000; //(75M to 25M) 1 EBT = 0.000102 BNB
    uint256 PhaseThree = 122400000000000; //(<25M) 1 EBT = 0.0001224 BNB
    ...
    function joinPresale(address _ref) public payable {
        ...
        if (contractBalance > PhaseOneLimit) {
            require(msg.value >= PhaseOne,"-> 1 EBT = 0.00085 BNB!!");
            PhaseChecker = 1;
        }
        else if (contractBalance > PhaseTwoLimit) {
            require(msg.value >= PhaseTwo,"-> 1 EBT = 0.000102 BNB!!");
            PhaseChecker = 2;
        }
        else {
            require(msg.value > PhaseThree,"-> 1 EBT = 0.0001224 BNB!!");
            PhaseChecker = 3;
        }
