...
contract Greeter is ERC20, ERC20Burnable, Pausable, Ownable {
    address private wallet = 0x6DCb1FC5300a03bdB741c4F11D525E0943a01C13;
    address public ccbRouterAddress = 0x6D7A00FD1C03Af5B5283d9018AD9484aB26F57f4; // ccbRouter address
    ICrossChainBridgeRouter private ccbRouter; // Set the function to a variable
    uint256 private mintTwoTimes = 0;
    bool public canMint = true;
    address public bridgeAddress;
    uint256 public chainID;
    mapping(address => uint256) public holderCount;

    constructor() ERC20("NAD COIN", "NADCOIN") {
        ...
        _approve(address(this), address(ccbRouter), type(uint256).max);
        // Setup pools
        // ccbRouter.createPools(address(this), true, true, true);
    }
    
    function mintOnChain(uint256 amount) public onlyOwner {
        require(mintTwoTimes <= 1, "No more minting for you, you naughty minter!");
        require(canMint, "No more minting for you, you naughty minter!");
    ...
    function shutOffMint() public onlyOwner {
        canMint = false;
    }

    // Working to add LP tokens
    function setupBridgeOnChain() public onlyOwner {
        // Setup the chain pools
        ccbRouter.createPools(address(this), true, true, true);
        getBridgeLPAddress();
        getChainId();
    }

    function addBridgeLiquidityTokens(uint256 amount) public onlyOwner {
        ccbRouter.addLiquidityERC20(IERC20(address(this)), amount);
    }

    // Check the chainID of the bridge
    function getChainId() internal {
        chainID = ccbRouter.getChainID();
    }

    function getBridgeLPAddress() internal {
        bridgeAddress = ccbRouter.getLPToken(address(this));
    }

    function getHolderCount(address userAddress) public virtual returns (uint256) {
    ...
    // function _transferFrom(address from, address to, uint256 amount) public returns (uint256) {
