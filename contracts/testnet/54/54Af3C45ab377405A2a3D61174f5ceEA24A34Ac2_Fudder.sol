...
contract Fudder is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FTRividendTracker public dividendTracker;
    ...
    constructor() ERC20("Fudder", "FUD") {
        marketingWallet = payable(0x00778F65C4C32E150b1731F0c7e60C088bE564E4);
        ...
        dividendTracker = new FTRividendTracker(
            payable(this),
            router,
...
contract FTRividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(
        ...
        defaultToken = token;
        FudderContract = Fudder(mainContract);
    ...
    function _transfer(
        ...
        require(false, "BR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(FudderContract);

            uint256 prevBalance = FudderContract.balanceOf(address(this));
            ...
            {
                uint256 received = FudderContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    FudderContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
