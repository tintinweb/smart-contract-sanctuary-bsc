        ...
        uint(
          keccak256(
            abi.encodePacked(
              ...
              hex'67ab296c6e0e76d231d9f401a4813411c00b67f16d37049b8d898b907bd4cf3d'
            )
          )
        )
      )
...
contract C3SwapRouter1 is I3SwapRouter1 {
    ...
    )
  {
    ...
    if (reserveA == 0 && reserveB == 0 && reserveC == 0)
      ...
      if (amountCOptimal <= amountsDesired.amountC) {
        require(amountCOptimal >= amountsMin.amountC, 'insufficient C amount');
        ...
        require(amountBOptimal >= amountsMin.amountB, 'insufficient B amount');
        ...
        require(amountAOptimal >= amountsMin.amountA, 'insufficient A amount');
    ...
    )
  {
    ...
    assert(IWETH(WETH).transferETH(triad, amountETH));
    liquidity = I3SwapTriad(triad).mint(to);
    ...
    )
  {
    ...
    require(amountA >= amountsMin.amountA, 'insufficient A amount');
    require(amountB >= amountsMin.amountB, 'insufficient B amount');
    require(amountC >= amountsMin.amountC, 'insufficient C amount');
  }
    ...
    )
  {
    Tokens memory _tokensLiquidity = tokensLiquidity;
    _tokensLiquidity.tokenC = WETH;
    (amountToken1, amountToken2, amountETH) = removeLiquidity(_tokensLiquidity, liquidity, amountsMin, to, deadline);
  ...
  function _swap(
    ...
    for (uint i; i < path.length - 2; i++) {
      ...
      uint amount2Out = amounts[i + 2];
      (uint amount0Out, uint amount1Out) = (uint(0), uint(0));
  ...
  function swapTokensForExactTokens(
    ...
    amounts = L3SwapLibrary.getAmountsIn(factory, amount2Out, path);
    require(amounts[0] <= amount0InMax && amounts[1] <= amount1InMax, 'excessive input amounts');
    TransferHelper._safeTransferFrom(
      path[0],
      msg.sender,
      L3SwapLibrary.triadFor(factory, path[0], path[1], path[2]),
  ...
  function swapExactETHForTokens(
    ...
    require(path[0] == WETH, 'invalid path at 0');
    amounts = L3SwapLibrary.getAmountsOut(factory, msg.value, amount1In, path);
    require(amounts[amounts.length - 1] >= amount2OutMin, 'insufficient output amount');
    IWETH(WETH).deposit{value: amounts[0]}();
    assert(IWETH(WETH).transferETH(L3SwapLibrary.triadFor(factory, path[0], path[1], path[2]), amounts[0]));
    TransferHelper._safeTransferFrom(
      path[1],
      msg.sender,
      L3SwapLibrary.triadFor(factory, path[0], path[1], path[2]),
