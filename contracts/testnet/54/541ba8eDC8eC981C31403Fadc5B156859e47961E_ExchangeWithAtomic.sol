...
contract ExchangeWithOrionPool is Exchange, IPoolSwapCallback {
    ...
    /**
     * @notice Eth can only be used for asset A - not for asset B
     * @dev adds Liquidity to the Orion Pool from User's deposits
     * @param assetAddressA address of token A or Eth
     * @param assetAddressBNoEth address of token B. Cannot be Eth
     ...
     * @param useUserBalance bool, which is true if user allows using his/her own wallet balance if balance on exchange is not enough.
     */
    function withdrawToPool(
        address assetAddressA,
        address assetAddressBNoEth,
        ...
        if(useUserBalance == true){
            uint256 deltaA;
            uint256 deltaB;

            (deltaA, deltaB) = LibPool._getWalletFunds(assetAddressA, amountADesired, assetAddressBNoEth, amountBDesired, assetBalances);
            
            if(deltaA > 0){
                generalDeposit(assetAddressA, uint112(deltaA));
            }
            if(deltaB > 0){
                generalDeposit(assetAddressBNoEth, uint112(deltaB));
            }

        }

        (amountA, amountB) = LibPool._withdrawToPool(assetAddressA, assetAddressBNoEth, amountADesired, amountBDesired, amountAMin, amountBMin, deadline, assetBalances, _orionpoolRouter);

        require(checkPosition(msg.sender), "E1w2"); //TODO
...
library LibPool {
    ...
    function _getWalletFunds(
        address assetAddressA,
        uint256 amountADesired,
        address assetAddressBNoEth,
        uint256 amountBDesired,
        mapping(address => mapping(address => int192)) storage assetBalances
    ) public returns(uint256 deltaA, uint256 deltaB){
        
        require(assetAddressBNoEth != address(0), "E1W3");

        deltaA = amountADesired - uint(LibUnitConverter.decimalToBaseUnit(assetAddressA, uint(assetBalances[msg.sender][assetAddressA])));        
        deltaB = amountBDesired - uint(LibUnitConverter.decimalToBaseUnit(assetAddressBNoEth, uint(assetBalances[msg.sender][assetAddressBNoEth])));        
        
        if (assetAddressA != address(0)) {
            if(deltaA > 0){
                IERC20(assetAddressA).safeTransferFrom(msg.sender, address(this), deltaA);
            } else {
                deltaA = 0;
            }
        } else {
            require(int(LibUnitConverter.baseUnitToDecimal(assetAddressA,amountADesired)) <= int(assetBalances[msg.sender][assetAddressA]));
            deltaA = 0;
        }

        if(deltaB > 0){
            IERC20(assetAddressBNoEth).safeTransferFrom(msg.sender, address(this), deltaB);
        }
