...
contract BakedPizza{
    ...
    mapping (address => uint256) public claimedEggs;    // монеты контракта которые есть у людей 
    mapping (address => uint256) public lastHatch;      // время последнего вывода в BNB 
    ...
    function rebakePizza(address ref) public{
        ...
        // если у человека нет рефералов, то записывается адрес разработчика
