...
contract KnightBattle is KnightFamily {
    ...
    constructor(IKnightNFT _knightNFT , IKnightItems _contractItem, IKnightToken _knighToken, IMinter _minter, IMAL _mal) {
        Item = _contractItem; 
    ...
    function attack(uint _attackknightID, uint _defenseKnightID) external {
        ...
        require(_isReady(infoKnightAtt)); //"Knight can't fight yet"
        require(msg.sender != infoKnightDef.owner); // "You can't attack your knight"
        ...
        external 
    {
        ...
        require(_deposit == _mission * 300 * 10 ** 18, "Invalid deposit");
        require(_mission <= 5 && _mission > 0, "Invalid mission");
    ...
    // function getHistoryBattle(uint _knightId) 
    //     external 
    //     returns(HistoryBattle.Info[] memory ) 
    // {
    //     require(IKnightNFT(knightNFT).checkOwner(_knightId, msg.sender), "Invalid owner");
...
contract KnightFamily is KnightUpgrade {
    ...
    function requestMarry(uint32 _IDknightRequest, uint32 _IDknightResponse, uint _diposit) 
        external 
    {
        require(IKnightNFT(knightNFT).checkOwner(_IDknightRequest, msg.sender), "Invalid owner");
        require(_diposit >= minAmountgift); //  "Gift <= 600 STICoin"
...
contract KnightUpgrade is AccessControl {
    ...
    string baseURI          = "ipfs://QmeAPecjTgMb5YBofM3HjT1gSCz7WvtgZRERxUFLJiE4zh/";
    ...
    {
        Knight.Info memory myKnight = IKnightNFT(knightNFT).getInfoKnight(_knightID);
        require(_deposit == payTolevelUp * (myKnight.rank + 1), "insufficient supply of token");
        _levelUp(_knightID);
    ...
    function _levelUp(uint _knightID) internal  {
        ...
        if(myKnight.level == 30) {
            revert("Please upgrade knight to 2 stars ");
        } else if(myKnight.level == 70 ){
            revert("Please upgrade knight to 3 stars ");
        }
        ...
        if(  myKnight.level < 30) {
            ...
            powerOfKnight.damage          += 5  * (myKnight.rank + 1);
            powerOfKnight.defense         += 2  * (myKnight.rank + 1);
        } else if( myKnight.level < 70) {
            myKnight.level             += 1;
            myKnight.defaultAttack     += 540;
            myKnight.defaultSex        += 540;
            powerOfKnight.healthPoint     += 30 * (myKnight.rank + 1);
            ...
            powerOfKnight.damage          += 25 * (myKnight.rank + 1);
            powerOfKnight.defense         += 15 * (myKnight.rank + 1);
        }
    ...
    function _findMaximum(uint[] memory _ids, address _owner) 
        public returns(uint32, uint32[] memory)
    {
        ...
        returns(bool ,  Knight.Info memory)
    {
        ...
        require(checkOwner,"Invalid owner of the Knight");
        require(checkCharm,"Invalid owner of charm!");
        Knight.Info memory  myKnight      = IKnightNFT(knightNFT).getInfoKnight(_knightID);
        uint8 lenght = uint8(_charmIDs.length);
        require(myKnight.star < 3, " knight is max star");
        if(myKnight.level < 30) {
            revert("upgrade knight to level 30");
        } else if(myKnight.level < 70 && myKnight.level > 30) {
            revert("upgrade knight to level 70");
        }
    ...
    function useElixir(uint _knightID, uint _elixirID) 
        external 
    {
        require(IKnightNFT(knightNFT).checkOwner(_knightID, msg.sender),"Invalid owner of the Knight");
        require(IKnightNFT(knightNFT).ownerOfElixir(_elixirID) == msg.sender, "invalid owner of Elixir");
        Knight.Power memory powerOfKnight = IKnightNFT(knightNFT).getPowerKnight(_knightID);
        Knight.Info memory  myKnight      = IKnightNFT(knightNFT).getInfoKnight(_knightID);
