...
contract CIRAStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor() {
        token = IBEP20(0xF0d872671284E7ffeA3cb7aF1BE42Dc05e57f47b);      //CryptoIRA Contract Address
        lpToken = IBEP20(0x1DD673400757751D11602EE7799E6Ba8e4806759);    //CryptoIRA Liquidity Pool
    ...
    function add(uint256 _apy, IBEP20 _lpToken, bool _withUpdate,
    ...
    function set(uint256 _pid, uint256 _apy, bool _withUpdate,
        uint256 _withdrawLockPeriod ) external onlyOwner {
        require(_withdrawLockPeriod <= 90 days, "withdraw lock must be less than 90 days");
        if (_withUpdate) {
            massUpdatePools();
    ...
    function pendingToken(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (myBlock > pool.lastRewardBlock && lpSupply != 0 ) {
            ...
            uint256 tokenPerBlock = lpSupply.mul(tokenAmountPerLP()).mul(pool.apy).div(100).div(10512000);
    ...
    function withdraw(uint256 _pid) external nonReentrant {
        ...
        if(!canHarvest(_pid, msg.sender)){
            ...
            if(burnAmount > 0)
                lpToken.safeTransfer(DEAD, burnAmount);
        }
        
        withdrawAmount = user.amount.sub(burnAmount);
        lpToken.safeTransfer(address(msg.sender), withdrawAmount);
    ...
    function addRewardTreasure(uint256 _amount) external {
    ...
    function recoverTreasure( IBEP20 recoverToken, uint256 amount) external onlyOwner{
        ...
        for (uint256 pid = 1; pid < length; ++pid) {
            require(recoverToken != lpToken,"can't transfer lp");
        }
    ...
    function tokenAmountPerLP() internal view returns(uint256) {
        uint256 amountTokenInLP = token.balanceOf(address(lpToken));
        uint256 totalLP = lpToken.totalSupply();
        uint256 tokenPerLp = amountTokenInLP.mul(2).div(totalLP);
        return tokenPerLp;
    }
}