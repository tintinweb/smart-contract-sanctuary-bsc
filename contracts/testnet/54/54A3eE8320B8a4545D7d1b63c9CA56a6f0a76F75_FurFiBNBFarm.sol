    ...
    PausableUpgradeable
{
    ...
    uint256 public lastRoundMaskUpdateBlock;
    uint256 public blockRewardPhase1End;
    uint256 public blockRewardPhase2Start;
    uint256 public blockRewardPhase1Amount;
    uint256 public blockRewardPhase2Amount;
    ...
    /// @notice Stakes the desired amount of FurFi-BNB-LP tokens into the pool
    /// @dev Executes claimRewards before the staking to get a clean state for the roundMask and the rewards
    /// @param amount The desired staking amount for an investor in FurFi-BNB-LP tokens
    ...
    /// @notice Unstakes the desired amount of FurFi-BNB-LP tokens from the pool
    /// @dev Executes claimRewards before the unstaking to get a clean state for the roundMask and the rewards
    /// @param amount The desired amount to unstake for an investor in FurFi-BNB-LP tokens
    ...
    /// @notice Converts the supplied amount of ETH into FurFi-BNB-LP tokens, then stakes it into the pool
    /// @dev Executes claimRewards before the staking to get a clean state for the roundMask and the rewards
    function stakeFromEth(
        ...
        (uint256 amount, uint256 unusedEth, uint256 unusedTokens) = DEX
            .convertEthToTokenLP{value: msg.value}(address(FurFiToken));
        ...
        IERC20Upgradeable(address(FurFiToken)).transfer(
            msg.sender,
            unusedTokens
        );
    ...
    /// @notice Unstakes the desired amount of FurFi-BNB-LP tokens from the pool, then converts it into ETH and sends it back to the caller
    /// @dev Executes claimRewards before the unstaking to get a clean state for the roundMask and the rewards
    /// @param amount The desired amount to unstake for an investor in FurFi-BNB-LP tokens
    ...
    /// @notice Converts the specified amount of the specified token into FurFi-BNB-LP tokens, then stakes it into the pool
    /// @dev Executes claimRewards before the staking to get a clean state for the roundMask and the rewards
    /// @param token the address of the token to be converted into FurFi-BNB-LP tokens
    /// @param tokenAmount the amount of tokens to be converted into FurFi-BNB-LP tokens
    function stakeFromToken(
        ...
        (uint256 amount, uint256 unusedEth, uint256 unusedTokens) = DEX
            .convertEthToTokenLP{value: ethAmount}(address(FurFiToken));
    ...
    /// @notice Unstakes the desired amount of FurFi-BNB-LP tokens from the pool, then converts it into the specified token
    /// @dev Executes claimRewards before the unstaking to get a clean state for the roundMask and the rewards
    /// @param token The address of the token into which the FurFi-BNB-LP tokens will be converted
    /// @param amount The desired amount to unstake for an investor in FurFi-BNB-LP tokens
    ...
    function pendingRewards(address participant) external view returns (uint256) {
        ...
        return
            ((currentRoundMask - participantData[participant].rewardMask) *
                participantData[participant].stakedAmount) / DECIMAL_OFFSET;
    }

    /// @notice Claims the current rewards for an investor
    /// @dev The round mask is updated in the first place to update the current rewards for the investor. The rewards in furFi tokens are then minted and transferred to the investor
    function claimRewards() public whenNotPaused {
        ...
        if (rewardsToTransfer > 0) {
            FurFiToken.claimTokens(rewardsToTransfer);
