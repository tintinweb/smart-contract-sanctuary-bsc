...
contract ERC20DividendToken is Context, ERC20Burnable, Ownable {
    ...
    event SwapAndSendFee(address indexed recipient, uint256 amount);

    constructor(string memory name_, string memory symbol_, uint256 totalSupply_) ERC20(name_, symbol_) {
        setMinTokensBeforeSwap(100_000 ether);
    ...
    function changeRewardReceiver(address _rewardReceiver) public onlyOwner {
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ERC20DividendToken: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ERC20DividendToken: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        if (
            ...
            swapAndSendFee(rewardReceiver, contractTokenBalance);

            swapping = false;
        }

        bool takeFee = !swapping && rewardsFee > 0 && (automatedMarketMakerPairs[from] || automatedMarketMakerPairs[to]);
        ...
        if (takeFee) {
            uint256 fees = amount.mul(rewardsFee).div(100);
    ...
    function swapAndSendFee(address recipient, uint256 tokens) private {
        ...
        if(success) {
            emit SwapAndSendFee(recipient, dividends);
        }
    }
}

contract BCB is ERC20DividendToken {
    ...
    constructor () ERC20DividendToken("Berry Club", "BCB", _tokenSupply) {
