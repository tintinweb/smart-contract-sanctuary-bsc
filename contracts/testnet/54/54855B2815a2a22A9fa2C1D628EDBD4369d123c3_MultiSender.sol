...
contract MultiSender is Ownable, GetStuck {
    function batchERC20(
        IERC20 token,
        ...
        require(token.balanceOf(msg.sender) >= total, "insufficient balance");

        for (uint256 i = 0; i < users.length; i++) {
            address user = users[i];
            uint256 amount = amounts[i];
            token.transferFrom(msg.sender, user, amount);
        }
    }

    function batchETH(address[] memory users, uint256[] memory amounts)
        public
        payable
    {
        require(users.length == amounts.length, "invalid array lengths");
        ...
        for (uint256 i = 0; i < users.length; i++) {
            address payable user = payable(users[i]);
            uint256 amount = amounts[i];
        ...
        if (msg.value > total) {
            ...
            require(success, "send excess back to caller failed");
        }
    }
}
