...
contract Lottery is Ownable, Initializable {
  ...
  using SafeERC20 for IERC20Metadata; // Safe ERC20
  using Address for address; // Address functionality
  ...
  struct PoolInfo {
    uint8 ticketTypeId;
    address lpToken; // liquidity pool
    uint256 minStake; // in wei
  }
  ...
  mapping(uint8 => TicketTypeInfo) public ticketTypeInfo_;
  uint8 public constant TICKET_TYPE_LENGTH = 5;

  mapping(uint256 => PoolInfo) internal pools_;
  Counters.Counter private poolCounter_;

  mapping(uint256 => RoundInfo) internal rounds_;
  Counters.Counter private roundCounter_;

  ITicketNFT internal nft_; // Storing of the NFT

  //-------------------------------------------------------------------------
  // CONSTRUCTOR
  //-------------------------------------------------------------------------

  function initialize(address _ticketNFT) external initializer onlyOwner {
    nft_ = ITicketNFT(_ticketNFT);

    // id => { size, odds, [start, end]  }
    ticketTypeInfo_[SPECIAL] = TicketTypeInfo(1, 80, [0, 1]);
    ticketTypeInfo_[MULTI_1] = TicketTypeInfo(1, 80, [0, 27]);
    ticketTypeInfo_[MULTI_2] = TicketTypeInfo(2, 10, [0, 27]);
    ticketTypeInfo_[MULTI_3] = TicketTypeInfo(3, 40, [0, 27]);
    ticketTypeInfo_[MULTI_4] = TicketTypeInfo(4, 100, [0, 27]);
  }
  ...
  event RoundClose(uint256 roundId, uint256 playerCount);

  event RoundComplete(uint256 roundId, uint256 totalTickets);

  event RoundOpen(uint256 roundId);

  event NewBatchMint(address indexed minter, uint256[] ticketIds, uint8[] numbers, uint256 stake);
  ...
  modifier notContract() {
    ...
    require(!sender.isContract(), "Contract not allowed");
    require(sender == tx.origin, "Proxy contract not allowed");
    _;
  }
  ...
  function setTicketTypeInfo(uint8 _id, TicketTypeInfo memory _info)
    external
    onlyOwner
    returns (bool)
  {
    ticketTypeInfo_[_id] = _info;
    return true;
  }

  function getPools(uint256 _roundId) external view returns (PoolInfo[] memory) {
    uint256[] memory poolIds = rounds_[_roundId].poolIds;

    PoolInfo[] memory items = new PoolInfo[](poolIds.length);
  ...
  function getAllPools() external view returns (PoolInfo[] memory) {
    PoolInfo[] memory items = new PoolInfo[](poolCounter_.current());

    for (uint256 i = 0; i < poolCounter_.current(); i++) {
      items[i] = pools_[i];
    }
  ...
  function getAllTicketTypeInfo() external view returns (TicketTypeInfo[] memory) {
    TicketTypeInfo[] memory items = new TicketTypeInfo[](TICKET_TYPE_LENGTH);
    for (uint8 i = 0; i < TICKET_TYPE_LENGTH - 1; i++) {
      items[i] = ticketTypeInfo_[i];
    }

    return items;
  }

  function getTicketTypeInfo(uint8 _ticketId) external view returns (TicketTypeInfo memory) {
    return ticketTypeInfo_[_ticketId];
  }
  ...
  /* Add a new pool. only be called by the owner
   * @note the owner must approve the added tokens for this contract
   */
  function batchAddPools(PoolInfo[] calldata _info)
    external
    onlyOwner
    returns (uint256[] memory newPoolIds)
  {
    uint256[] memory _newPoolIds = new uint256[](_info.length);
    PoolInfo[] memory _pools = _info;

    for (uint256 i = 0; i < _pools.length; i++) {
      uint256 newTokenId = poolCounter_.current();

      pools_[newTokenId] = PoolInfo({
        ticketTypeId: _pools[i].ticketTypeId,
        lpToken: _pools[i].lpToken,
        minStake: _pools[i].minStake
      });
      _newPoolIds[i] = newTokenId;

      poolCounter_.increment();
    }

    return _newPoolIds;
  }

  /* Add a new round to the pool. only be called by the owner.
   * @note DO NOT add the same ticket type (0~4) and ticket pool more than once
   * Rewards will be messed up if you do.
   */
  function createNewRound(uint256[] memory _poolIds) external onlyOwner returns (uint256 roundId) {
    uint256 prevRoundId = roundCounter_.current();
    if (prevRoundId > 0) {
      RoundInfo memory prevRound = rounds_[prevRoundId];
      require(prevRound.status == Status.Closed, "Previous round not finished yet");
    }

    require(_poolIds.length > 0, "Require pools");

    uint256 newRoundId = roundCounter_.current();

    rounds_[newRoundId] = RoundInfo({
      status: Status.Open,
      totalPlayers: 0,
    ...
    roundCounter_.increment();

    emit RoundOpen(newRoundId);

    return newRoundId;
  }

  function endsRound(uint256 _roundId, uint8[] calldata _winningNumbers) external onlyOwner {
    RoundInfo storage _round = rounds_[_roundId];

    require(_round.status == Status.Open, "Round not completed yet");

    _round.status = Status.Closed;
    _round.winningNumbers = _winningNumbers;

    emit RoundComplete(_roundId, nft_.getRoundTotalSupply(_roundId));
  }
  ...
  function batchBuyTicket(
    ...
    uint256 _stake
  ) external notContract returns (uint256[] memory _ticketIds) {
    uint8 size = ticketTypeInfo_[_typeId].size;
    uint256 numberOfTickets = _numbers.length.div(size);

    RoundInfo memory round = rounds_[_roundId];
    PoolInfo memory pool = pools_[_poolId];

    require(
      _typeId == pool.ticketTypeId && size != 0 && _numbers.length.mod(size) == 0,
      "Invalid ticket type"
    );
    require(_stake >= pool.minStake, "Not enough staking");
    require(round.status == Status.Open, "Round not open yet");
    require(_containsPool(round, _poolId), "Ticket's pool not for this round");
    require(numberOfTickets <= MAX_TICKETS, "Batch mint too large");

    address owner = _msgSender();
    uint256 roundId = _roundId;
    uint256 poolId = _poolId;
    uint8[] calldata numbers = _numbers;
    uint256 stake = _stake;

    require(
      IERC20(pool.lpToken).transferFrom(owner, address(this), stake.mul(numberOfTickets)) == true,
      "Could not transfer stake"
    );

    uint256[] memory ticketIds = nft_.batchMint(
      size,
      numberOfTickets,
      ITicketNFT.TicketInfo({
        owner: owner,
        roundId: roundId,
        poolId: poolId,
        numbers: numbers,
    ...
    emit NewBatchMint(owner, ticketIds, numbers, stake);
    return ticketIds;
  }

  function batchClaimTicket(uint256[] calldata _ticketIds, uint256 _roundId)
    external
    notContract
    returns (bool)
  {
    RoundInfo memory _roundInfo = rounds_[_roundId];

    require(_ticketIds.length <= MAX_TICKETS, "Batch claim too large");
    require(_roundInfo.status == Status.Closed, "Round not closed yet");

    for (uint256 i = 0; i < _ticketIds.length; i++) {
      uint256 _ticketId = _ticketIds[i];
      ITicketNFT.TicketInfo memory ticketInfo = nft_.getTicketInfo(_ticketId);

      require(ticketInfo.owner == _msgSender(), "Only owner can claim");
      require(
        nft_.claimTicket(_ticketId, ticketInfo.poolId, _roundId),
        "Numbers for ticket invalid"
      );

      _rewardPrizeForMatching(_ticketId, _roundId);
    }

    return true;
  }

  function _rewardPrizeForMatching(uint256 _ticketId, uint256 _roundId) internal {
    ITicketNFT.TicketInfo memory _ticketInfo = nft_.getTicketInfo(_ticketId);
    PoolInfo memory _poolInfo = pools_[_ticketInfo.poolId];
    TicketTypeInfo memory _ticketType = ticketTypeInfo_[_poolInfo.ticketTypeId];

    uint8[] memory _winningNumbers = rounds_[_roundId].winningNumbers;
    uint8[] memory _userNumbers = _ticketInfo.numbers;

    uint8 start = _ticketType.range[0];
    uint8 end = _ticketType.range[1];

    uint8 _matchedCounter = 0;

    for (uint256 i = start; i < end; i++) {
      uint8 a = _winningNumbers[i];

      for (uint256 j = 0; j < _userNumbers.length; j++) {
        uint8 b = _userNumbers[j];
    ...
    uint8 hitCount = _matchedCounter.div(_ticketType.size);
    if (hitCount > 0) {
      uint256 stake = _ticketInfo.stake;
      uint256 odds = _ticketType.odds;
      uint256 minStake = _poolInfo.minStake;

      uint256 prize = stake.mul(odds);

      bool isMultiple = _ticketType.size == 1;
      ...
      if (minStake > 0) {
        uint8 decimals = IERC20Metadata(_poolInfo.lpToken).decimals();
      ...
      IERC20Metadata(_poolInfo.lpToken).transfer(_ticketInfo.owner, prize);
    }
  }

  function _containsPool(RoundInfo memory _round, uint256 _poolId) private pure returns (bool) {
    for (uint256 i = 0; i < _round.poolIds.length; i++) {
      if (_round.poolIds[i] == _poolId) return true;
    }
...
interface ITicketNFT {
  ...
  function getRoundTotalSupply(uint256 roundId) external view returns (uint256);

  function getTicketInfo(uint256 _ticketId) external view returns (TicketInfo memory _info);

  function getTicketInPool(uint256 _roundId, uint256 _poolId)
    external
    view
