...
interface IBaseDoNFT is IERC721Receiver {
    ...
    event DurationUpdate(uint256 tokenId,uint64 start,uint64 end);

    event DurationBurn(uint256[] durationIdList);

    event CheckIn(address opreator,address to,uint256 tokenId);
    ...
    function mint(uint256 tokenId,uint64 end,address to) external returns(uint256 tid);

    function setMaxDuration(uint64 v) external;

    function getDoNftInfo(uint256 tokenId) external view returns(DoNftInfo memory info);

    function getNonce(uint256 tokenId) external view returns(uint128);
...
abstract contract BaseDoNFT is OwnableContract,ReentrancyGuard,ERC721Enumerable,IBaseDoNFT {
    ...
    function isValidNow(uint256 tokenId) public view returns(bool isValid){
        ...
        isValid = info.start <= block.timestamp && block.timestamp <= info.end;
    }

    function getDoNftInfo(uint256 tokenId) public view returns(DoNftInfo memory info){
        info = doNftMapping[tokenId];
    }
    ...
    function mint(
        ...
        DoNftInfo storage info = doNftMapping[tokenId];
        require(start >= info.start && end <= info.end, "invalid info");
    ...
    function concat(uint256 tokenId,uint256 targetTokenId) public {
        ...
        DoNftInfo storage info = doNftMapping[tokenId];
        DoNftInfo storage targetInfo = doNftMapping[targetTokenId];
        if(info.end < targetInfo.start){
            require(info.end+1 == targetInfo.start);
            targetInfo.start = info.start;
            _burn(tokenId);
        }
        else if(targetInfo.end < info.start){
            require(targetInfo.end+1 == info.start);
    ...
    function checkIn(address to,uint256 tokenId) public virtual{
        require(_isApprovedOrOwner(_msgSender(), tokenId) || _isApprovedOrOwner(tx.origin, tokenId), "not owner nor approved");
        DoNftInfo storage info = doNftMapping[tokenId];
        require(info.end >= block.timestamp,"invalid end");
        require(info.start <= block.timestamp,"invalid start");
        checkInUserMapping[info.oid] = to;
        emit CheckIn(tx.origin,to,tokenId);
    }
    ...
    function getFingerprint(uint256 tokenId) public view returns(bytes32 print){
        DoNftInfo storage info = doNftMapping[tokenId];
        print = keccak256(abi.encodePacked(info.oid,info.nonce,info.start,info.end));
    }
...
contract WrapDoNFT is BaseDoNFT,IWrapDoNFT{
    ...
    function couldRedeem(uint256 tokenId) public view returns(bool) {
        require(isXNft(tokenId) , "not xNFT") ;
        DoNftInfo storage info = doNftMapping[tokenId];
        return info.start <= block.timestamp && info.end == type(uint64).max;
    }
...
contract DclDoNFT is WrapDoNFT{
    ...
    function checkIn(address to,uint256 tokenId) public override virtual{
        BaseDoNFT.checkIn(to,tokenId);
        DoNftInfo storage info = doNftMapping[tokenId];
