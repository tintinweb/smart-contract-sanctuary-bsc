...
contract UberTaxiController is Ownable {
    ...
    constructor() {
        ...
        TAXI20 = ERC20(0x5710EE664a8Ca6c9e8C056cDa6b8d6A60bbfD4E4);
    ...
    function queryUserRentVehicleAmount(address addr) public view returns(uint rentAmount){
        ...
        for(uint256 i = 0; i < myVehiclesIds[addr].length; i++){
            ...
            if(index != 0 && allVehicles[index].vehicleState == 1){
                rentAmount++;
            }
        }
        return rentAmount;
    ...
    function queryUserRentLimit(address addr) public view returns(bool exceed){

        uint256 rentAmount = queryUserRentVehicleAmount(addr);

        if(rentAmount < rentAmountLimit) exceed = true;

        if(rentAmount >= rentAmountLimit) exceed = false;
    }
