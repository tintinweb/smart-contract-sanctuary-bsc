...
contract StakePoolTokenForToken is States, ReentrancyGuard, Ownable {
    ...
    function setTokens(uint256 _apy, uint256 _earlyUstakeFee, IBEP20 _stakingToken, IBEP20 _rewardingToken, address _rewardingTokenWallet, uint256 _lockPeriod, uint256 _maxReward, uint256 _earlyRewardUstakeFee, bool _autoCompund, bool _earlyUstake) public onlyOwner {
        apyTier = _apy;
    ...
    function _claim(address _address) internal nonReentrant{
        User storage info = deposit[_address];
         uint256 claimcurrentReward = checkReward(_address);
  
        if(claimcurrentReward <= pendingRewards() ){
            // rewardingToken.transferFrom(rewardingTokenWallet , _address , info.reward);
    ...
    function _earlyClaim(address _address) internal nonReentrant{
        ...
        if(currentReward > 0 && currentReward <= pendingRewards()){
            // rewardingToken.transferFrom(rewardingTokenWallet , _msgSender(), currentReward.sub(_rewardFee));
            TransferHelper.safeTransferFrom(address(rewardingToken),rewardingTokenWallet, _msgSender(), currentReward.sub(_rewardFee));
        } else{
    ...
    function compoundInterest(address _address) public {
        uint256 ratio = apyTier.mul(10 ** 18).div(365).div(100);
        ...
        uint256 stakedtime;
         ...
         if(block.timestamp > info.lockPeriod){
          ...
          stakedtime = (info.lockPeriod) - (info.lastClaimTime);
          stakedtime = stakedtime / 1 days;

          uint256 _reward = _compound(info.totalAmount, ratio, stakedtime);
          info.totalAmount += _reward;

         }else{

            stakedtime = (block.timestamp) - (info.lastClaimTime);
            stakedtime = stakedtime / 1 days;
            uint256 _reward = _compound(info.totalAmount, ratio, stakedtime);
            info.totalAmount += _reward;
    ...
    function _compound(uint256 _principal, uint256 _ratio, uint256 _exponent) public pure returns (uint256) {
        if (_exponent == 0) {
            return 0;
        }
    ...
    function checkReward(address _address) public view returns (uint256){
      ...
      if(apyEnabled){
          ...
          uint256 reward = _compound(cUser.totalAmount, ratio, stakedtime);
          ...
          uint256 reward = apyTier * stakedtime;
          cUser.totalAmount += reward;
          cUser.reward += reward;
          return reward;
...
contract FactoryStakingPool is Ownable{
    ...
    function createTokenForToken(address ownerAddress, uint256 _apy, uint256 _earlyUstakeFee, IBEP20 _stakingToken, IBEP20 _rewardingToken, address _rewardingTokenWallet, uint256 _lockPeriod, uint256 _maxReward, uint256 _earlyRewardUstakeFee, bool _autoCompund, bool _earlyUstake) public onlyOwner returns (address)
    {
        StakePoolTokenForToken toDeploy = new StakePoolTokenForToken();
        toDeploy.setTokens(_apy, _earlyUstakeFee, _stakingToken, _rewardingToken, _rewardingTokenWallet, _lockPeriod, _maxReward, _earlyRewardUstakeFee, _autoCompund, _earlyUstake);
