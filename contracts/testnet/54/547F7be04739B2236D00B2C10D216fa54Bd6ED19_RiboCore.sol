...
import "./interface/IRiboStrategy.sol";

contract RiboCore is Ownable, ReentrancyGuard {
    ...
    function _claim(address _userAddr, uint256 _pid) internal validatePid(_pid) {
        ...
        if(released > 0){
            _safeRiboTransfer(_userAddr, released);
        }
        address strategyAddr = tokenStrategy[_pid];
        if(strategyAddr != address(0)){
            IRiboStrategy(strategyAddr).claim(pool.depositToken);
        }
    ...
    function _withdraw(address _userAddr, uint256 _pid, uint256 _amount) internal validatePid(_pid) {
        ...
        if(_amount > 0){
            ...
            if(bal < _amount){
                address strategyAddr = tokenStrategy[_pid];
                if (strategyAddr != address(0)) {
                    IRiboStrategy(strategyAddr).withdraw(pool.depositToken, _amount.sub(bal));
                }
            }
            // transfer
            if (pool.depositToken != ETH_ADDR) {
        ...
        if (strategyAddr != address(0)) {
            ...
            if (pool.depositToken != ETH_ADDR) {
                ...
                if(bal >= pool.earnThreshold){
                    ...
                    IRiboStrategy(strategyAddr).deposit(pool.depositToken);
                }
            } else {
                bal = address(this).balance;
                if(bal >= pool.earnThreshold){
    ...
    function setStrategy(uint256 _pid, address _tokenAddr, address _strategyAddr) external onlyOwner {
        ...
        if (_oldStrategy != address(0)) {
            IRiboStrategy(_oldStrategy).withdrawAll(_tokenAddr);
        }
    ...
    function emergencyWithdrawStrategy(uint256 _pid) external onlyOwner {
        ...
        IRiboStrategy(strategyAddr).emergencyWithdraw(poolInfo[_pid].depositToken);
    ...
    function emergencyWithdraw(address _token, uint256 _amount) external onlyAuth {
        if(_token == ETH_ADDR){
    ...
    function _safeRiboTransfer(address _to, uint256 _amount) internal {
        uint256 riboBal = ribo.balanceOf(address(this));
        if (_amount > riboBal) {
            ribo.transfer(_to, riboBal);
        } else {
            ribo.transfer(_to, _amount);
        }
    }
}
