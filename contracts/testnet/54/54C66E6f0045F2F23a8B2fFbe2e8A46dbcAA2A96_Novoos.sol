...
contract Novoos is ERC20Detailed, Ownable {
    ...
    uint256 public extraRewardFee = 1;
    uint256 public extraallowanceFee = 1;
    ...
    constructor() ERC20Detailed("Novoos", "NOVO", uint8(DECIMALS)) Ownable() {
        ...
        autoLiquidityReceiver = 0x1bF99f349eFdEa693e622792A3D70833979E2854;
        AllowanceReceiver = msg.sender; 
        BankReceiver = 0x15fa92d177B72DE827273Bf7736dbC854BeC2Bd3;
    ...
    function rebase() internal {
        ...
        rebaseRate = 4029143074; //apy 423543.25%
        for (uint256 i = 0; i < times; i++) {
            _totalSupply = _totalSupply.add(rebaseRate*10**DECIMALS);
        }
    ...
    function takeFee(
        ...
        uint256 _rewardFee = rewardFee;

        if (recipient == pair) {
            _totalFee = totalFee.add(extraallowanceFee).add(extraRewardFee);
            _AllowanceFee = AllowanceFee.add(extraallowanceFee);
            _rewardFee = rewardFee.add(extraRewardFee);
        }
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)].add(
            gonAmount.div(feeDenominator).mul(_AllowanceFee.add(BankFee).add(_rewardFee))
        );
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHReward = amountETH.mul(rewardFee).div(rewardFee.add(BankFee).add(AllowanceFee));
        try distributor.deposit{value: amountETHReward}() {} catch {}
        (bool success, ) = payable(AllowanceReceiver).call{
            value: amountETH.mul(AllowanceFee).div(rewardFee.add(BankFee).add(AllowanceFee)),
            gas: 30000
        }("");
        (success, ) = payable(BankReceiver).call{
            value: amountETH.mul(BankFee).div(rewardFee.add(BankFee).add(AllowanceFee)),
            gas: 30000
        }("");
    }
