...
interface IDMSLaunchpad {
    function getPackageIdbyOwner(address _owner) external returns (uint256);
    ...
    ReentrancyGuardUpgradeable
{
    ...
    event claimed(address owner, uint256 packageId, uint256 tokenId);
    event bought(address owner, uint256 rankId, uint256 tokenId);

    IChainSeed chainSeed;
    IDMSLaunchpad launchPad;
    ...
    mapping(uint256 => uint256) public packageId;

    constructor(address _chainSeed, address _launchPad) {
        chainSeed = IChainSeed(_chainSeed);
        launchPad = IDMSLaunchpad(_launchPad);
    ...
    function setRank(
        ...
        require(bytes(name).length != 0, "Rank: name is empty");
        require(ranks[_rankId].isExist == false, "Rank: already exist");
        ranks[_rankId] = Rank(name, _price, false, _tokenId, true);
        ranksId.push(_rankId);
    }

    function existRank(uint256 _rankId, bool exist)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ranks[_rankId].isExist = exist;
    }
    ...
    function addTokenIdInRank(uint256 _rankId, uint256[] memory tokenId)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(ranks[_rankId].isExist == true, "Clan: don't already exist");
        require(tokenId.length > 0, "Token: is empty");

        for (uint256 i = 0; i < tokenId.length; i++) {
            ranks[_rankId].tokenId.push(tokenId[i]);
        }
    }
    ...
    function setPackageIdforLaunchpad(uint256 _rankId, uint256 _packageId)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        packageId[_rankId] = _packageId;
    }
    ...
    function getTokenId(uint256 _rankId, uint256 _random)
        private
        whenNotPaused
        returns (uint256)
    {
        ...
        if (ranks[_rankId].tokenId.length > 1) {
            _index = _random % (ranks[_rankId].tokenId.length);
            _tokenId = ranks[_rankId].tokenId[_index];
            ranks[_rankId].tokenId[_index] = ranks[_rankId].tokenId[
                ranks[_rankId].tokenId.length - 1
            ];
            ranks[_rankId].tokenId.pop();
        } else if (
            ranks[_rankId].tokenId.length <= 1 &&
            ranks[_rankId].tokenId[0] != 1000
        ) {
            _index = 0;
            _tokenId = ranks[_rankId].tokenId[_index];
            ranks[_rankId].tokenId[0] = 1000;
    ...
    function safeMint(address _owner, uint256 _tokenId) private whenNotPaused {
        uint256 _rankId = _tokenId.div(10000);
    ...
    {
        require(
            launchPad.getPackageIdbyOwner(_owner) == _rankId,
            "User owns the asset"
        );
        require(
            launchPad.getClaimStatus(_owner, "monster") == false,
            "User hasn't cliamed the asset"
        );
        require(ranks[_rankId].isExist == true, "Rank: don't already exist");
        require(ranks[_rankId].isOpen == true, "Rank: isn't open");
        //claim
        uint256 _packageId = packageId[_rankId];
        launchPad.claimAsset(_owner, _packageId, "monster");
        // mint
        uint256 _random = getRandomNumber();
        uint256 _tokenId = getTokenId(_rankId, _random);
        safeMint(_owner, _tokenId);
        // record
        allMonsters[_tokenId] = Monster(_owner, _tokenId, _rankId, false, true);
        ownedMonsters[_owner].push(_tokenId);

        emit claimed(_owner, _packageId, _tokenId);
    }
        ...
        nonReentrant
    {
        ...
        require(
            launchPad.getPackageIdbyOwner(msg.sender) == _rankId,
            "User owns the asset"
        );
        require(
            launchPad.getClaimStatus(msg.sender, "monster") == false,
            "User hasn't cliamed the asset"
        );
        require(ranks[_rankId].isExist == true, "Rank: don't already exist");
        require(ranks[_rankId].isOpen == true, "Rank: isn't open");
        ...
        ownedMonsters[msg.sender].push(_tokenId);

        emit bought(msg.sender, _rankId, _tokenId);
    }
