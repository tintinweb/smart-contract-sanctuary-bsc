...
/**
 * @author Publius
 * @title Oracle tracks the TWAP price of the BUSD/BNB and POPCORN/BNB Pancake pairs.
 **/
contract OracleFacet {
    ...
    /// @notice Function that calls capture for tracking the Delta B on both the Pancake and Curve LP pools
    ...
    /// @notice Internal function for initializing the Curve pool oracle by calculating the cumulative balances within the pool.
    function initializeOracle() internal {
        ...
        if (reserve0 != 0 && reserve1 != 0 && blockTimestampLast != 0) {
            ...
            (uint256 peg_priceCumulative, , uint32 peg_blockTimestamp) = PancakeOracleLibrary.currentCumulativePrices(s.c.pegPair);
    ...
    function getTWAPPrices() public view returns (uint256, uint256) {
        ...
        if (pegTimeElapsed > 0) {
            ...
            // We assume that the index of BUSD is 0 in this instance.
