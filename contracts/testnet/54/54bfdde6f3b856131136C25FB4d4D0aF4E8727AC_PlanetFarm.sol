...
contract PlanetFarm is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) external nonReentrant {
        ...
        if(_pid != iGammaPid){
            usersClaimableGamma[_msgSender()] = usersClaimableGamma[_msgSender()] + (userShares*pool.accGAMMAPerShare + userFactor * pool.accGAMMAPerFactorPerShare)/1e12 - user.rewardDebt;
        }
        ...
        if (_wantAmt != 0) {
            ...
            if(pool.isInfinity) {
                ...
                if(_pid == iGammaPid) {
                    updateFactorInternal(_msgSender(), iTokens);
                    GammaTrollerAddress.updateFactor(_msgSender(), iTokens);
                }
                else{
                    userShares = iTokens - user.iTokenToBeUnstaked;
                }
    ...
    function depositAuthorized(address _user,uint256 _wantAmt, bool takeWantFromMsgsender) internal  {
        ...
        if (_wantAmt != 0) {
            if(takeWantFromMsgsender){
                ...
                want.safeTransferFrom(address(_msgSender()), address(this),_wantAmt);
                _wantAmt = want.balanceOf(address(this)) - balanceBefore;
                delete balanceBefore;
            }

            want.safeIncreaseAllowance(strat, _wantAmt);
            ...
            uint256 mintAmount = IStrategy(strat).depositAuthorized(_wantAmt);
    ...
    function withdraw(uint256 _pid, uint256 _wantAmt) external nonReentrant {    
        ...
        usersClaimableGamma[_msgSender()] = usersClaimableGamma[_msgSender()] + (userShares*accGAMMAPerShare + userFactor * accGAMMAPerFactorPerShare)/1e12 - user.rewardDebt;
        ...
        delete userShares;
        delete userFactor;
        delete accGAMMAPerShare;
        delete accGAMMAPerFactorPerShare;
        delete sharesRemoved;
    }
    ...
    function claimAllPoolsPendingGamma() external nonReentrant {
        ...
        for(uint256 i = 0; i < length; ++i) {
            ...
            pendingRewards = pendingRewards + (userShares * accGAMMAPerShare + userFactor * accGAMMAPerFactorPerShare)/1e12 - user.rewardDebt;
            user.rewardDebt = (userShares * accGAMMAPerShare + userFactor * accGAMMAPerFactorPerShare)/1e12;
        }
    ...
    function startUnstakeProcess(uint256 _pid, uint256 unstakeAmount) external nonReentrant{
        ...
        if(_pid != iGammaPid){
        usersClaimableGamma[_msgSender()] = usersClaimableGamma[_msgSender()] + ((useriTokenBalance - iTokenAlreadyGivenForUnstake) * pool.accGAMMAPerShare)/1e12 - user.rewardDebt;
        }
    ...
    function unstakeInstantly(uint256 _pid, uint256 unstakeAmount ) external nonReentrant{
        ...
        if(_pid != iGammaPid){
            usersClaimableGamma[_msgSender()] = usersClaimableGamma[_msgSender()] + ((useriTokenBalance - useriTokenToBeUnstaked)*pool.accGAMMAPerShare)/1e12 - user.rewardDebt;
        }
        ...
        require(itokens <= useriTokenBalance - useriTokenToBeUnstaked,
            "!enough itokens");
    ...
    function unstakeAfterMinWithdrawTime(uint256 _pid) external nonReentrant{        
        ...
        require(user.unstakeStartTime + user.minTimeToWithdraw < block.timestamp, "early");
