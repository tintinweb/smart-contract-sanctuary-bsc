...
contract DXB is IERC20, Context, Ownable, Initializable {
    ...
    function initialize (address _cycleBurn, address _teamVault, address _IAirDrop, address _IRewardPool, address[3] memory _feeWallet) external initializer {
        ...
        _taxRestoreFee = _taxFee = 5;
        _addLPRestoreFee = _addLPFee = 2;
        _liquidityRestoreFee = _liquidityFee = 50;
        _burnRestoreFee = _burnFee = 50;
        ...
        excludeFromDeduction[msg.sender] = true;
    ...
    modifier notBlackList(address from, address to) {
        require((!blackList[msg.sender]) && (!blackList[from]) && !blackList[to], "blacklisted");
        _;
    }
    ...
    function updateTaxFee( uint _tax) public onlyOwner {
        _taxRestoreFee = _taxFee = _tax; 
    }
    ...
    function updateBurnAndLiquidityFee( uint _liqFee, uint _bFee) external onlyOwner {
        ...
        _liquidityRestoreFee = _liquidityFee = _liqFee;
        _burnRestoreFee = _burnFee = _bFee; 
    }
    
    function updateAddLiquidityFee( uint _lpFee) external onlyOwner {
        _addLPRestoreFee = _addLPFee = _lpFee; 
    }
     ...
     * @param value value to send.
     */
    function _transfer(address from, address to, uint256 value) private notBlackList( from, to) whenUnpaused {
        ...
        if((!isSwapEnable) || ((excludeFromDeduction[from]) || (excludeFromDeduction[to])))
            removeAllFees(); // remove all fees.
        ...
        if(isSwapEnable){_takeLiquidity( _faddlp.add(_fburn)); if(_fburn > 0) _burn( address(this), _fburn);}
    ...
    function removeAllFees() private {
        _restoreTaxFee = _taxRestoreFee;
        _restoreLiquidityFee = _liquidityRestoreFee;
        _restoreBurnFee = _burnRestoreFee;
        _restoreAddLPFee = _addLPRestoreFee;
    ...
    function resotreAllFees() private {
        _taxFee = _taxRestoreFee;
        _liquidityFee = _liquidityRestoreFee;
        _burnFee = _burnRestoreFee;
        _addLPFee = _addLPRestoreFee;
    ...
    function monitorReductionInFee() private {
        if((totalSupply() <= _rInSupply) && (reductionInTax)){
            _taxRestoreFee = _taxFee = 2;
            _liquidityRestoreFee = _liquidityFee = 100;
            _burnRestoreFee = _burnFee = 0;
    ...
    function __transfer(address from, address to, uint256 value) private notBlackList( from, to) whenUnpaused {
        require(to != address(0), "DXB :: _transfer : invaild to address");
        uint fee = value.mul(_taxFee).div(100);
