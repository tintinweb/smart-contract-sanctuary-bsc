...
// File: contracts/seanStaking.sol
...
contract BadBullsNFTStaking is IERC721Receiver, Ownable {
    ...
    uint256 public rewardsLeftToGo;
    ...
    function initializeOrAddMore(uint256 amount) public onlyOwner {
        RTOKEN.approve(address(this), amount); // amount of tokens you want to send to contracts for rewarad in wei
        rewardsLeftToGo += amount;
    }

    function updatePool(uint256[] memory ids, bool isDeposit) internal {

        if(block.number > lastRewardBlock) {
            rewardsLeftToGo -= currentRewardRate * (block.number - lastRewardBlock);
            totalPending += currentRewardRate * (block.number - lastRewardBlock);
        ...
        if(isDeposit) currentRewardRate += rateChange;
        if(!isDeposit) currentRewardRate -= rateChange;

        EXPIRATION = block.number + ((RTOKEN.balanceOf(address(this)) - totalPending) / currentRewardRate);
    }
    ...
    function toggleStart(bool _state) public onlyOwner() {
        ...
        updatePool(blank,false);
        if (!_state) EXPIRATION = block.number;
    }
