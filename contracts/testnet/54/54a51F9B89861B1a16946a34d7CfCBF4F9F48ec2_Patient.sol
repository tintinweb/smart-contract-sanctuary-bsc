...
contract clinic {
    ...
    constructor(address _patient, string memory _emr) public {
        patient = _patient;
        records[0] = _emr;
    }

    // The patient can add a new record to their EMR.
    function addRecord(string memory _record) public {
        // Only the patient is allowed to add records to their EMR.
        require(
            msg.sender == patient,
            "Only the patient can add records to the EMR."
        );

        // Find the next available record ID by incrementing the ID of the last record.
        // uint id = records.length++;

        // Add the new record to the EMR.
        // records[id] = _record;
    }

    function grantAccess(address _authorized) public {
        // Only the patient is allowed to grant access to their EMR.
        require(
            msg.sender == patient,
            "Only the patient can grant access to the EMR."
        );

        // Set the authorized party's access to true.
        authorized[_authorized] = true;
    }

    function revokeAccess(address _authorized) public {
        // Only the patient is allowed to revoke access to their EMR.
        require(
            msg.sender == patient,
            "Only the patient can revoke access to the EMR."
        );

        // Set the authorized party's access to false.
        authorized[_authorized] = false;
    }

    // Authorized parties can view the patient's EMR.
    function viewEMR() public view {
        // Check if the caller is an authorized party.
        require(
            authorized[msg.sender],
            "The caller is not an authorized party."
        );

        // Return the patient's EMR.
        // return records;
    }
}
...
contract Patient {
    ...
    struct Document {
      ...
      function personalInformation11(uint256 _passportnumber, string memory _prefix, string memory _gender, uint256 _DOF,
      string memory _nationality, string memory _Occupation, string memory _BloodGroup, string memory _firstName, string memory _Mothername,
      string memory _NamewithMiddlename, string memory _Month, string memory _Countryofbirth, string memory _Religion, string memory _socialsecurityNumber,
      uint256 _year, string memory _provinceOfBirth) public {
    ...
    function dressInformation(string memory _AddressThailand, string memory _Moo, string memory _Soi, string memory _Street,
    string memory _Subdistrict, string memory _District, string memory _Province, uint256 _Zipcode, uint256 _Mobiletelephone,
    uint256 Hometelephone, string memory _email ) public {
