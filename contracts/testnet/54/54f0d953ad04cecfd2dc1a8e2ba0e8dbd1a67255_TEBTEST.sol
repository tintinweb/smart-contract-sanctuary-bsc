...
contract TEBTEST is Context, IERC20, Ownable {
    ...
    uint256 public _employmentHold = 7.5 * 10**6 * 10**9; //Maximum hold limit of 7.5% of total supply for team expansion
    ...
    function _transfer(
        ...
        if (_isEmploymentWallet[to])
            ...
            require((heldTokens + amount) <= _employmentHold,
                "Employment wallet can only hold at most 7.5% of the supply"
             );
            
        }

        // Employment Wallet transfer limits
        if (_isEmploymentWallet[from] && !_isExcluded[to]) 
            {
            require (amount >= 0, "Employment wallet cannot transfer to others."
            );
        }
        ...
        // Donation Wallet transfer limits
        if (_isDonationRecipient[from] && !_isExcluded[to]) 
            {
            require (amount >= 0, "Donation wallet cannot transfer to others."
            );
        }
        ...
        if (
            ...
            uint256 forDevelopment = contractTokenBalance
                .mul(buyFee.developmentFee + sellFee.developmentFee)
                .div(
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] 
            || _isDonationRecipient[from] || _isDonationRecipient[to] || _isEmploymentWallet[to]) {
            takeFee = false;
        }
