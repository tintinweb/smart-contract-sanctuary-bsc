...
contract BrkLayer {
    ...
    event Claim(bytes hashCode, address indexed user, uint claimAmount);
    event RegisterNftHolder(address indexed user, uint tokenId, bool isNewRegister);
    ...
    function purchaseLand(uint8 indexOfLand) external {
        ...
        require(indexOfLand <= _players[user].landLevel + 1, "You have to buy the land gradually");
        uint needTokenAmount = landPrice[indexOfLand] / tokenPrice;
        ...
        player.landLevel = (uint8)(_max((uint8)(player.landLevel), (uint8)(indexOfLand)));
    ...
    function purchaseBricks(uint8 indexOfBrick, uint brickAmount) external {
        ...
        uint needTokenAmount = brickPrice[indexOfBrick] * brickAmount / tokenPrice;
    ...
    function claim(bytes memory hashCode, address user, uint claimAmount) external onlyOwner {
        UserInfo storage player = _players[user];
        uint _lastRewardTime = player.lastRewardTime;
        uint _claimDays = player.isNFTHolder ? claimFastDays : claimDays;
        require(block.timestamp - _lastRewardTime >= _claimDays*3600*24, "You wait the time of claim");
        require(!usedHash[hashCode], "Confirmed transaction.");
        
        usedHash[hashCode] = true;
        ...
        emit Claim(hashCode, user, claimAmount);
    }
    ...
    function registerNftHolder() external {
        ...
        address _holder = _regNft[tokenId];
        bool isNewRegister = true;
        if (_holder != address(0)) {
            // if an user already exist
            _players[_holder].isNFTHolder = false;
            isNewRegister = false;
        }
        _regNft[tokenId] = user;
        _players[user].isNFTHolder = true;

        emit RegisterNftHolder(user, tokenId, isNewRegister);
    }

    function getPlayerInfo(address userAddr) external view returns (uint8 landLevel, uint lastRewardTime, bool isNFTHolder) {
        UserInfo memory player = _players[userAddr];
        landLevel = player.landLevel;
        lastRewardTime = player.lastRewardTime;
        isNFTHolder = player.isNFTHolder;
    }
