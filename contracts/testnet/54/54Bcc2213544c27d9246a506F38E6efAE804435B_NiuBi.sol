...
contract NiuBi is ERC20, Ownable{
    ...
    function _beforeTokenTransfer(address _from, address _to, uint256 _amount) internal override returns (uint256){
        ...
        if (isPair[_from]){
            ...
            if (fiveWriteList[_from] == false) {
                require(ordWriteList[_from] ? block.timestamp >= startTradeTime + _FIVE_MIN : block.timestamp >= startTradeTime + _FIVE_OTH, "not start exchange 2");
            }
            ...
            //超级白名单前3分钟，普通白名单3分钟后，其他7分钟后
            if (fiveWriteList[_from] == false) {
                require(ordWriteList[_from] ? block.timestamp >= startTradeTime + _FIVE_MIN : block.timestamp >= startTradeTime + _FIVE_OTH, "not start exchange 4");
            }
