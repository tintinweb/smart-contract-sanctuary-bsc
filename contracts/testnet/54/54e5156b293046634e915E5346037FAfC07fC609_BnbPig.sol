...
contract BnbPig is Ownable, MinerBasic, AutoEXE, Algorithm, Airdrop, InvestorsManager, EmergencyWithdrawal, Testable {
    ...
    function renounceUnstuck() public onlyOwner { renounce_unstuck = true; }
    ...
    function unstuck_bnb(uint256 _amount) public onlyOwner { 
    ...
    function capToMaxSell(uint256 piggiesValue, uint256 piggies) public view returns(uint256, uint256){
        ...
        if(maxSell >= piggiesValue){
            ...
            uint256 nPiggiesHire = calculateHirePigsSimpleNoEvent(piggiesValue.sub(maxSell));
            if(nPiggiesHire <= piggies){
                return (maxSell, piggies.sub(nPiggiesHire));
            }
            else{
                return (maxSell, 0);
            }
