...
contract MountSapphire is Pausable, Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    uint256 public stakingFee = 0 ether;
    ...
    constructor(
        ...
        require(_rewardTokenAddress != address(0), "S_MTS_0103");
    ...
    function _updateRewardPerDay() internal {
        for (uint256 index = 0; index < powerList.length; index++) {
            if (totalPower <= powerList[index]) {
                currentRewardPerDay = tokenPerDayList[index];
                return;
            }
        }

        currentRewardPerDay = tokenPerDayList[tokenPerDayList.length - 1];
    }
    ...
    function getRewardToken() external view returns (uint256) {
        return currentRewardPerDay;
    }

    function getTotalPowerInPool() external view returns (uint256) {
        return totalPower;
    }
    ...
    function getPendingReward(address _user) external view returns (uint256) {
        ...
        uint256 userTimestamp = _userInfo.timestamp;
        uint256 power = _userInfo.finalPower;
        uint256 duration = _getDuration(userTimestamp);
        uint256 weight = _getUserWeight(power, totalPower);
        uint256 rewardPerSecond = currentRewardPerDay / SECONDS_PER_DAY;
        return rewardPerSecond.mul(weight).div(EXA_CONSTANT).mul(duration);
    }
    ...
    function _getUserWeight(uint256 _userPower, uint256 _totalPower)
        internal
        pure
        returns (uint256)
    {
    ...
    function _withdrawReward() internal {
        ...
        uint256 weight = _getUserWeight(power, totalPower);
        uint256 rewardPerSecond = currentRewardPerDay / SECONDS_PER_DAY;
        uint256 reward = rewardPerSecond.mul(weight).div(EXA_CONSTANT).mul(
            duration
        );
        if (reward > 0) {
            uint256 balance = _getCurrentBalance();
            if (reward > balance) {
                reward = balance;
            }
            IERC20(rewardTokenAddress).transfer(address(msg.sender), reward);
        }
    }
    ...
    function _getDuration(uint256 _timestamp) private view returns (uint256) {
    ...
    function _validateStaking(address _sender, uint256[] memory blucamonIds)
        ...
        view
    {
        require(isBlucamonPowerExist(blucamonIds), "S_MTS_0500");
        uint256 blucamonCount = blucamonIds.length;
        require(blucamonCount > 0, "S_MTS_0600");
        require(
    ...
    function _validateUnstaking(address _sender, uint256[] memory blucamonIds)
        internal
        view
    {
        require(blucamonIds.length > 0, "S_MTS_0800");
        require(
    ...
    function _checkIsRewardEmpty() internal view {
        require(_getCurrentBalance() > 0, "S_MTS_1000");
    }
        ...
        whenNotPaused
    {
        ...
        if (userInfo[msg.sender].finalPower > 0) {
            _withdrawReward();
        }
        totalPower = totalPower.sub(userInfo[msg.sender].finalPower);
        ...
        require(msg.value == totalFee, "S_MTS_1100");
        if (totalFee > 0) {
        ...
        emit StakeTokens(
            msg.sender,
            userInfo[msg.sender].finalPower,
            _blucamonTokenIds
        );
    }
    ...
    function unstake(uint256[] memory _blucamonTokenIds)
        ...
        whenNotPaused
    {
        _validateUnstaking(msg.sender, _blucamonTokenIds);
        bool findToken;
        if (_getCurrentBalance() > 0) {
            _withdrawReward();
        }
        totalPower = totalPower.sub(userInfo[msg.sender].finalPower);
        for (uint256 i = 0; i < _blucamonTokenIds.length; i++) {
    ...
    function emergencyUnstake() external whenNotPaused {
        ...
        totalPower = totalPower.sub(userPower);
        _updateRewardPerDay();
        emit EmergencyWithdraw(msg.sender, blucamonTokenIds.length);
    }

    function emergencyRewardTokenWithdraw(uint256 _amount) external onlyOwner {
        require(_getCurrentBalance() >= _amount, "S_MTS_1300");
    ...
    function claim() external {
        require(isWhitelist(), "S_MTS_1400");
        IERC20(rewardTokenAddress).transfer(
            msg.sender,
            whitelistReward[msg.sender]
        );
