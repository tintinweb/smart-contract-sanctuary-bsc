...
contract PowermadeAffiliation is Ownable {
    ...
    constructor(address _token_addr, address _projectWallet, address _headWallet) public {
        ...
        _addEditPackage(5, true, -1, 100, false, false, 0, false, false, empty_array, false);
        _addEditPackage(6, true, -1, 100, false, false, 600, true, false, empty_array, false);
    }
    ...
    function isSubscriptionExpired(address userAddr, uint16 packageID) internal view returns (bool is_expired) {
        ...
        if (packagesIndex[packageID].duration > 0) {
            ...
            duration = duration > 0 ? duration : packagesIndex[packageID].duration;
        } else {
    ...
    function _isEligibleReceiveCommissions(address userAddr, uint16 packageID, address payer, uint level, uint8 sourceID) internal returns (bool is_eligible) {
        ...
        if (isBanned(userAddr) || isSubscriptionExpired(userAddr, packageID)) { return false; } 
        is_eligible = true;
...
// MAKE spotFinder private IN MAINNET
...
// DEBUG - TO REMOVE IN MAINNET

    // Function used to read the Round Robin memory for a userID. The elemID is the index of the round-robin memory (starting from 1), with the history of all the previous targeted sub-structures
    function getRoundRobinInfo(uint userID, uint elemID) internal view returns (uint invitedCount, uint32 round_robin_next_index, uint round_robin_val) {
