 ...
 */
contract Bridge is Pausable, ContextUpgradeable {
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public usedNonces;
     ...
     */
    function initialize(uint256 domainID, address accessControl) public initializer {
        _domainID = domainID;
     ...
     */
    function adminSetDepositNonce(uint256 domainID, uint64 nonce) external onlyAllowed {
     ...
     */
    function deposit(uint256 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) external payable whenNotPaused {
     ...
     */
    function executeProposal(uint256 originDomainID, uint64 depositNonce, bytes calldata data, bytes32 resourceID, bytes calldata signature) public whenNotPaused {
     ...
     */
    function isProposalExecuted(uint256 domainID, uint256 depositNonce) public view returns (bool) {
...
interface IFeeHandler {
    ...
    /**
        ...
        @param feeData Additional data to be passed to the fee handler.
     */
    function collectFee(address sender, uint256 fromDomainID, uint256 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) payable external;
     ...
     */
    function calculateFee(address sender, uint256 fromDomainID, uint256 destinationDomainID, bytes32 resourceID, bytes calldata depositData, bytes calldata feeData) external view returns(uint256, address);
}
