...
interface IUNQ is IERC721AQueryable{
...
interface ISHOEBOX is IERC721AQueryable{
...
contract OneMove is Ownable, ERC1155Holder, ERC721Holder, ReentrancyGuard {
    ...
    event DepositShoebox(uint256 uid, address account, uint256 tokenId);
    event WithdrawShoebox(address account, uint256 tokenId);
    event BurnShoebox(uint256 tokenId);
    event BurnShoeboxBatch(uint256[] tokenIds);
    ...
    //Shoebox-------------------------
    function depositShoebox(uint256 uid, uint256 _tokenId) external {
        nftShoebox.safeTransferFrom(msg.sender, address(this), _tokenId);
        emit DepositShoebox(uid, msg.sender, _tokenId);
    }

    function withdrawShoebox(uint256 _tokenId, address account) external onlyAdminWhitelist {
        nftShoebox.safeTransferFrom(address(this), account, _tokenId);
        emit WithdrawShoebox(account, _tokenId);
    }

    function mintShoebox(address account, uint256 amount) external onlyAdminWhitelist {
        nftShoebox.mint(account, amount);
    }

    function balanceShoebox() external view returns(uint256) {
        return nftShoebox.balanceOf(address(this));
    }

    function burnShoebox(uint256 id) external onlyAdminWhitelist {
        nftShoebox.burn(id);
        // nftShoebox.safeTransferFrom(address(this), address(0x000000000000000000000000000000000000dEaD), id);
        emit BurnShoebox(id);
    }

    function burnShoeboxBatch(uint256[] memory ids) external onlyAdminWhitelist {
        nftShoebox.burnBatch(ids);
        // nftShoebox.safeTransferFrom(address(this), address(0x000000000000000000000000000000000000dEaD), id);
        emit BurnShoeboxBatch(ids);
    }
