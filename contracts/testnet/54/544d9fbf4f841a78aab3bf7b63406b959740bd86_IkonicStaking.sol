...
contract IkonicStaking is Ownable,ReentrancyGuard {
    ...
    function getReward(address account,uint id) public view returns (uint256) {
        ...
        if(Stakers[account][id].poolType ==poolTypes[Stakers[account][id].poolType]){
           ...
           if(!isGetReward[account][id][Stakers[account][id].poolType]){
             totalDays = (block.timestamp - Stakers[account][id].stakeTime)/ 60 ;
           }else{
             totalDays=(block.timestamp - lastUpdatedTime[account][id][Stakers[account][id].poolType])/ 60;
           }
  ...
  // User Staking function
    function stake(uint256 poolType) public nonReentrant{
