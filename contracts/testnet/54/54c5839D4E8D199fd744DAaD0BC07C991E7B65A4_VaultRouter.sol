...
contract VaultRouter is Ownable, ReentrancyGuard
{
    ...
    function Deposit(uint256 amount, uint256 poolId) public onlyRegisteredUsers nonReentrant
    {
        ...
        if(Fairlaunch != address(0)){
            IFairLaunch(Fairlaunch).WorkerDeposit(shares, poolId, _msgSender());
        }
        ...
        if(Fairlaunch != address(0))
        {
            IFairLaunch(Fairlaunch).UpdateWorkerInfo(amount, poolId, true);
        }
    ...
    function WithDraw(uint256 share, uint256 poolId) public onlyRegisteredUsers nonReentrant
    {
        ...
        if(Fairlaunch != address(0))
        {
            IFairLaunch(Fairlaunch).WorkerWithdraw(share, poolId, _msgSender());
        }
        ...
        if(Fairlaunch != address(0))
        {
            IFairLaunch(Fairlaunch).UpdateWorkerInfo(amount, poolId, false);
        }
    }
    ...
    function EmergencyWithdrawToken(address token, address to, uint256 amount, uint256 poolId) public onlyOwner
    {
     ...
     function SetasideBaseToken(uint256 amount, uint256 poolId) public onlyFairlaunchOrOwner
    {
        IAlpacaWorker worker = IAlpacaWorker(Workers[poolId]);
        address token = worker.BaseToken();
        worker.ForcedClose(amount.mul(CashSafeThreshold).div(1000));
        Operatable(Workers[poolId]).emergencyWithdraw(token, Fairlaunch,  amount);
        UpdateShareVal(poolId);
    }
