...
contract Blockstaking is Context, Auth, IERC20 {
    using SafeMath for uint256;

    //ERC20
    uint8 private _decimals = 18;
    uint256 private _totalSupply;
    string private _name = "Blockstaking";
    string private _symbol = "STK-BSAFU";
    ...
    address[] autoStakingAddress;
    mapping (address => uint256) shareholderIndexes;
    mapping (address => uint256) shareholderClaims;
    mapping (address => uint256) autostakingIndexes;
    mapping (address => bool) public isCanSetShares;
    mapping (address => Share) public shares;
    mapping (address => bool) public isAutoStakingAddress;

    uint256 public indexCurrentAutoStaking = 0;

    bool public isFeeForBountyEnable = true;
    uint public loopForBounty = 5;
    uint256 public percentFeeForBounty = 100;
    uint256 public percentTaxDenominator = 10000;
    uint256 public totalFeeBounty = 0;
    uint256 public totalAutoStaking = 0;
    ...
    function stake(address account,uint256 amount,bool isAutoStake) external {
      ...
      registeringAddressForAutoStaking(account, isAutoStake);
    ...
    function unstake(address account,uint256 amount) external {
      ...
      _setShare(account, _balances[account]);
      registeringAddressForAutoStaking(account,false);
    ...
    function setFeeBounty(bool state, uint256 _percentFeeForBounty, uint _loopForBounty) external onlyOwner {
      isFeeForBountyEnable = state;
      percentFeeForBounty = _percentFeeForBounty;
      loopForBounty = _loopForBounty;
    }

    function getFee(uint256 amountReward) internal returns(uint256){
        return amountReward;
//        if(!isFeeForBountyEnable) return amountReward;
//        uint256 amountFee = amountReward.mul(percentFeeForBounty.mul(percentTaxDenominator).div(percentTaxDenominator)).div(percentTaxDenominator);
//        totalFeeBounty = totalFeeBounty.add(amountFee);
//        return amountReward.sub(amountFee);
    }
    ...
    function registeringAddressForAutoStaking(address account,bool state) public {
      if(state && !isAutoStakingAddress[account]) {
        autostakingIndexes[account] = autoStakingAddress.length;
        autoStakingAddress.push(account);
        totalAutoStaking = totalAutoStaking.add(1);
      } 
      if(!state && isAutoStakingAddress[account]){
        autoStakingAddress[autostakingIndexes[account]] = autoStakingAddress[autoStakingAddress.length-1];
        autostakingIndexes[autoStakingAddress[autoStakingAddress.length-1]] = autostakingIndexes[account];
        autoStakingAddress.pop();
        totalAutoStaking = totalAutoStaking.sub(1);
      }
      isAutoStakingAddress[account] = state;
    }

    function setAutoStaking() external {
      if(autoStakingAddress.length > 0){
        uint256 maxLoop= (loopForBounty > autoStakingAddress.length) ? autoStakingAddress.length : loopForBounty;
        uint256 bountyLoop = 0;
        for(uint256 j;j<maxLoop;j++){
          if(indexCurrentAutoStaking >= autoStakingAddress.length) {
            indexCurrentAutoStaking = 0;
          }
          // console.log("> index current auto staking",indexCurrentAutoStaking);
          uint256 dividend = dividendOf(autoStakingAddress[indexCurrentAutoStaking]);
          if(dividend > 0) {
            claimDividendAndAutoStaking(autoStakingAddress[indexCurrentAutoStaking]);
            bountyLoop = bountyLoop.add(1);
          }
          if(indexCurrentAutoStaking.add(1) < autoStakingAddress.length){
            indexCurrentAutoStaking = indexCurrentAutoStaking.add(1);
          } else {
            indexCurrentAutoStaking = 0;
          }
        }
        if(bountyLoop > 0) sendRewardToBounty(bountyLoop);
      }
    }

    function claimDividendAndAutoStaking(address account) internal {
        IUniswapV2Router02 router = IUniswapV2Router02(routerDexAddress);
        uint256 amount = dividendOf(account);
        uint256 amountFee = amount.mul(percentFeeForBounty.div(percentTaxDenominator));
        totalFeeBounty = totalFeeBounty.add(amountFee);
        uint256 amountAfterFee = amount.sub(amountFee);
        if(amountAfterFee > 0){
            IWETH(wbnbAddress).withdraw(amountAfterFee);
            IWETH(router.WETH()).deposit{value:amountAfterFee}();
            address[] memory path = new address[](2);
            ...
            IWETH(router.WETH()).approve(routerDexAddress,amountAfterFee);
            uint256[] memory estimate = router.getAmountsOut(amountAfterFee,path);
            uint256 balanceBeforeSwap = IERC20(tokenAddress).balanceOf(address(this));
            router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
                amountAfterFee,
                estimate[1],
                path,
            ...
            uint256 balanceAfterSwap = IERC20(tokenAddress).balanceOf(address(this));
            uint256 balanceTransfer = balanceAfterSwap.sub(balanceBeforeSwap);
            
            // restake
            _mint(account,balanceTransfer);
    ...
    function sendRewardToBounty(uint256 bountyLoop) internal {
      uint256 amountReward = bountyLoop.mul(getRewardPrice());
        if(amountReward > 0){
          IUniswapV2Router02 router = IUniswapV2Router02(routerDexAddress);
          IWETH(wbnbAddress).withdraw(amountReward);
          IWETH(router.WETH()).deposit{value:amountReward}();
          address[] memory path = new address[](2);
          path[0] = router.WETH();
          path[1] = tokenAddress;
          IWETH(router.WETH()).approve(routerDexAddress,amountReward);
          router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
              amountReward,
              0,
              path,
            ...
            totalFeeBounty = totalFeeBounty.sub(amountReward);
        }
    }

    function getRewardPrice() public view returns(uint256){
        return totalFeeBounty.mul(loopForBounty.mul(percentTaxDenominator.div(autoStakingAddress.length)).div(percentTaxDenominator));
    }
