...
contract StakingAllocationProvider is BasicAllocationProvider {
    ...
    function takeSnapshot() public onlyOwner {
        (address[] memory accounts, uint256[] memory balances) = staking
            .currentlyStaked();
        require(accounts.length != 0, "StakingAllocationProvider: no stakers");
        for (uint256 i = 0; i < accounts.length; i++) {
            grantAllocation(accounts[i], balances[i]);
        }
    }
}
...
contract BasicAllocationProvider is IAllocationProvider, Ownable {
    ...
    function _grantAllocation(address _account, uint256 _amount) internal {
        ...
        _allocations[_account] = current + _amount;
