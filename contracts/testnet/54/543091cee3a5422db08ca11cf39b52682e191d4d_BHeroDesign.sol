...
contract BHeroDesign is AccessControlUpgradeable, UUPSUpgradeable, IBHeroDesign {
  ...
  function setSkinCount(uint256 value) external onlyRole(DESIGNER_ROLE) {
    skinCount = value;
  }
  ...
  function createRandomToken(
    ...
    if (rarity == BHeroDetails.ALL_RARITY) {
      // Random rarity.
      if (dropRateOption.length > 0){
        // HeroS: 1
        // Default other Hero: 0
        details.heroS = 1;
        (seed, details.rarity) = Utils.weightedRandom(seed, dropRateOption);
      }else{
        (seed, details.rarity) = Utils.weightedRandom(seed, isSuperBox ? superBoxDropRate : dropRate);
      }
    ...
    (seed, details.skin) = Utils.randomRangeInclusive(seed, 1, skinCount);
...
library BHeroDetails {
  ...
  /**
  * @notice detail info Hero
  * @param heroS - 0: is default normal hero, 1: is heroS
   */
  struct Details {
    uint256 id;
    uint256 index;
