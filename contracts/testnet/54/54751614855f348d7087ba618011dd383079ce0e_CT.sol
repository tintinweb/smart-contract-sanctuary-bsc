...
contract CT is Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC20SnapshotUpgradeable {

    uint256 private denominator;
    uint256 private swapThreshold;

    uint256 private taxTokens;
    ...
    function initialize() initializer public {
        __ERC20_init("CTest", "ctst");
        __ERC20Snapshot_init();
        __Pausable_init();
        __Ownable_init();

        denominator = 100;
        swapThreshold = 0.0000005 ether;
    ...
    function handleTax(address from, address to, uint256 amount) private returns (uint256) {
        ...
        if(from == address(uniswapV2Pair)) {
            ...
            taxTokens += baseUnit * buyTaxes["tax"];
            liquidityTokens += baseUnit * buyTaxes["liquidity"];
        } else if(to == address(uniswapV2Pair)) {
            tax += baseUnit * sellTaxes["tax"];
            ...
            taxTokens += baseUnit * sellTaxes["tax"];
            liquidityTokens += baseUnit * sellTaxes["liquidity"];
            
            uint256 taxSum = taxTokens + liquidityTokens;
            
            if(taxSum == 0) return amount;
            
            uint256 ethValue = uniswapV2Router02.getAmountsOut(taxTokens + liquidityTokens, sellPath)[1];
            
            if(ethValue >= swapThreshold) {
                uint256 startBalance = address(this).balance;

                uint256 toSell = taxTokens + liquidityTokens / 2;
                ...
                uint256 taxETH = (ethGained * ((taxTokens * 10**18) / taxSum)) / 10**18;
                ...
                uint256 remainingTokens = (taxTokens + liquidityTokens) - (toSell + amountToken);
                
                if(remainingTokens > 0) {
                    _transfer(address(this), taxWallet, remainingTokens);
                }
                
                taxWallet.call{value: taxETH}("");
                
                if(ethGained - (taxETH + liquidityETH) > 0) {
                    taxWallet.call{value: ethGained - (taxETH + liquidityETH)}("");
                }
    ...
    function _transfer(
        ...
        require(!isBlacklisted(msg.sender), "CT: sender blacklisted");
        require(!isBlacklisted(recipient), "CT: recipient blacklisted");
        require(!isBlacklisted(tx.origin), "CT: sender blacklisted");
    ...
    function pause() public onlyOwner {
        require(!paused(), "CT: Contract is already paused");
        _pause();
    }
    ...
    function unpause() public onlyOwner {
        require(paused(), "CT: Contract is not paused");
        _unpause();
    }
    ...
    function enableBlacklist(address account) public onlyOwner {
        require(!blacklist[account], "CT: Account is already blacklisted");
    ...
    function disableBlacklist(address account) public onlyOwner {
        require(blacklist[account], "CT: Account is not blacklisted");
    ...
    function exclude(address account) public onlyOwner {
        require(!isExcluded(account), "CT: Account is already excluded");
    ...
    function removeExclude(address account) public onlyOwner {
        require(isExcluded(account), "CT: Account is not excluded");
     ...
     */
    function setBuyTax(uint256 tax, uint256 liquidity) public onlyOwner {
        require(tax <= 10, "CT: Buy tax may not be set higher then 10%");
        require(liquidity <= 3, "CT: Buy liquidity may not be set higher then 3%");
        buyTaxes["tax"] = tax;
     ...
     */
    function setSellTax(uint256 tax, uint256 liquidity) public onlyOwner {
        require(tax <= 14, "CT: Sell tax may not be set higher then 14%");
        require(liquidity <= 3, "CT: Sell liquidity may not be set higher then 3%");
        sellTaxes["tax"] = tax;
        sellTaxes["liquidity"] = liquidity;
    }

    /**
     * @dev Sets tax wallet for taxes.
     */
    function setTaxWallet(address account) public onlyOwner {
       require(account != address(0), "CT: Tax Wallet cannot be zero address");
        taxWallet = account;
        exclude(taxWallet);
    }

    /**
     * @dev Sets liquidity wallet for taxes.
     */
    function setLiquidityWallet(address account) public onlyOwner payable{
        require(account != address(0), "CT: Liquidity Wallet cannot be zero address");
        liquidityWallet = account;
        exclude(liquidityWallet);
    }

    /**
     * @dev Sets router address and creates pair if needed.
     */
    function setRouterAddress(address account) public onlyOwner{
        require(account != address(0), "CT: Router Address cannot be zero address");
