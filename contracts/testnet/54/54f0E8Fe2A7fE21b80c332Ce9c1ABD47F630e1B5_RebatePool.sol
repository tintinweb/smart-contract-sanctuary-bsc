...
contract RebatePool is CoreUtility, Ownable {
    ...
    function claimRewardsAndUnwrap() external returns (uint256 rewards) {
        rewards = _claimRewards(msg.sender);
    ...
    function checkpoint() public {
        ...
        if (weekCursor < currentWeek) {
            ...
            for (uint256 w = weekCursor + System.SYSTEM_INTERVAL; w <= currentWeek; w += System.SYSTEM_INTERVAL) {
                ...
                newSupply = newSupply.sub(newLocked.mul(System.SYSTEM_INTERVAL) / _maxTime);
    ...
    function _rewardCheckpoint(address account) private returns (uint256) {
        ...
        while (weekCursor < currentWeek) {
            ...
            rewards = rewards.add(
                veGoenBalance.mul(rewardsPerWeek[weekCursor]) / veSupplyPerWeek[weekCursor]
            );
            weekCursor += System.SYSTEM_INTERVAL;
        }
...
library System {
    ...
    uint256 constant public SYSTEM_INTERVAL = 1800;
}
