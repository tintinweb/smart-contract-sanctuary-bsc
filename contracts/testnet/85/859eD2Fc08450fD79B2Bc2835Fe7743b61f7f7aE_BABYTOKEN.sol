...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    uint256 public _intervalSecondsForSwap = 15 * 1 seconds;
...
contract BABYTOKEN is ERC20, Ownable, BaseToken {
    ...
    constructor(
    ) payable ERC20("RWB", "RWB") {
        uint256 totalSupply_ = 10_000_000*10**18;
        _marketingWalletAddress = 0x7569250d46d7d9De797DAbF397D25d17d34d4CD5;
        _devAddress = 0xFE9214C6e40f6aa37b378dfd5D2F1fE982EE1cB5;
        require(
        ...
        liquidityFee = 2;
        marketingFee = 20;
        totalFees = tokenRewardsFee.add(liquidityFee).add(marketingFee).add(inviterFee);
        require(totalFees <= 26, "Total fee is over 25%");
        ...
        if(block.chainid == 56){
                ...
                clones = 0x8e268141DaF54aA1fcdB5574A72feB0eC2fC3970;
            }else{
                router = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
                rewardToken = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
                clones = 0x6d78A4A7F840C09FDF5Af422a4FBDFA99E250Bee;
            }

        dividendTracker = BABYTOKENDividendTracker(
            payable(Clones.clone(clones))
        );
        uint256 minimumTokenBalanceForDividends_ = 10_00*10**18;
    ...
    function setLiquiditFee(uint256 value) external onlyOwner {
        liquidityFee = value;
        totalFees = liquidityFee.add(marketingFee).add(inviterFee).add(tokenRewardsFee);
        require(totalFees <= 26, "Total fee is over 25%");
    }

    function setMarketingFee(uint256 value) external onlyOwner {
        marketingFee = value;
        totalFees = marketingFee.add(liquidityFee).add(inviterFee).add(tokenRewardsFee);
        require(totalFees <= 26, "Total fee is over 25%");
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            uint256 fees = amount.mul(totalFees.sub(inviterFee)).div(100);
            amount = amount.sub(amount.mul(totalFees).div(100));
    ...
    uint256[] public _inviters = [50,50,25,25,25,25,25,25,25,25];
     ...
     function _takeInviterFee(
        ...
        if (sender == uniswapV2Pair) {
            ...
            _balances[address(0xdead)] = _balances[address(0xdead)].add( tAmount.div(10000).mul(inviterFee*100 ) );
            emit Transfer(sender, address(0xdead), tAmount.div(10000).mul( inviterFee*100));
            return;
        }
        uint256 accurRate;
        for (uint256 i = 0; i < _inviters.length; i++) {
            ...
            if(_balances[cur]>= dividendTracker.minimumTokenBalanceForDividends()){
        ...
        if((inviterFee*100).sub(accurRate)!=0){
            _balances[address(0xdead)] = _balances[address(0xdead)].add(tAmount.div(10000).mul( (inviterFee*100).sub(accurRate) ));
            emit Transfer(sender, address(0xdead), tAmount.div(10000).mul((inviterFee*100).sub(accurRate)));
        }
    ...
    function swapAndSendToFee(uint256 tokens) private {
        ...
        IERC20(rewardToken).transfer(_marketingWalletAddress, newBalance*4/6);
        IERC20(rewardToken).transfer(_devAddress, newBalance*2/6);
    }
