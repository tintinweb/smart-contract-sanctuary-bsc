...
contract DripMaxi is ERC20, Ownable {
    ...
    uint256 private _totalFeesOnBuy  = 8;
    uint256 private _totalFeesOnSell = 8;

    address public treasuryWallet = 0x29224fC97BcB2Df0ceCB56BC1E7b579708fFa3ea;
    address public charityWallet  = 0x29224fC97BcB2Df0ceCB56BC1E7b579708fFa3ea;
    ...
    constructor() ERC20("Drip Maxi", "$DRIPMAXI") 
    {
        _mint(owner(), 1_000_000 * (10 ** 18));
        swapTokensAtAmount = totalSupply() / 5000;

    	dividendTracker = new DividendTracker(1_000_000 / 50_000, rewardToken);
    ...
    function updateBuyFees(uint256 _rewardFeeOnBuy, uint256 _treasuryFeeOnBuy, uint256 _buybackFeeOnBuy) external onlyOwner {
        require(
            (launchTime > 0 && launchTime + 14 minutes < block.timestamp) ||
            _rewardFeeOnBuy + _treasuryFeeOnBuy + _buybackFeeOnBuy <= _totalFeesOnBuy,
    ...
    function updateSellFees(uint256 _rewardFeeOnSell, uint256 _treasuryFeeOnSell, uint256 _buybackFeeOnSell) external onlyOwner {
        require(
            (launchTime > 0 && launchTime + 14 minutes < block.timestamp) ||
            _rewardFeeOnSell + _treasuryFeeOnSell + _buybackFeeOnSell <= _totalFeesOnSell,
            "Owner can not increase taxes in the first 14 days from listing"
        );
        require(
            _rewardFeeOnSell + _treasuryFeeOnSell + _buybackFeeOnSell <= 15,
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 buybackBNB;
            if (buybackShare > 0) {
                buybackBNB = (newBalance * buybackShare) / totalShares;
