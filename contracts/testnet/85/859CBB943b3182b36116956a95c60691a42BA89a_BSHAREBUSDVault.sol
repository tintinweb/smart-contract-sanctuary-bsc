...
contract BSHAREBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    */
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused {
        ...
        uint reserveBSHARE = IERC20(Admin.YSL()).balanceOf(address(BSHAREBUSD));
        ...
        uint USDyAmount;
        if(IERC20(Admin.YSL()).balanceOf(address(this)) >= BSHARERequire){
            percentYSL = 30;
            USDyAmount = ((2 * amount * (10 ** 18))/USDyPrice);
            IReceipt(Admin.USDy()).mint(address(this), USDyLiquidity + USDyAmount);
            convertBUSDToLP(amount * percentYSL/100, (reserveBSHARE * (amount * percentYSL/100)) / reservesBUSD, Admin.BUSD(), Admin.YSL(), address(BSHAREBUSD));
        }
        else{
            percentYSL = 15;
            ...
            convertBUSDToLP(reservesBUSD * IERC20(Admin.YSL()).balanceOf(address(this))/ reserveBSHARE, 
                IERC20(Admin.YSL()).balanceOf(address(this)), Admin.BUSD(), Admin.YSL(), address(BSHAREBUSD));
