...
contract NEWSwapERC20 is INEWSwapERC20 {
    using SafeMath for uint;

    string public constant name = 'NEWSwap-LP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'NEWSwap: TIMESTAMP_DEADLINE_EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'NEWSwap: SIGNATURE_IS_INVVALID');
        _approve(owner, spender, value);
    }
}

interface INEWSwapRouter {
...
contract NEWSwapPair is INEWSwapPair, NEWSwapERC20 {
    using SafeMath  for uint;
    ...
    INEWSwapRouter public routerAddress;
    ...
    modifier lock() { // IMPORTANT TO PREVENT RE-ENTRANCY
        require(msg.sender == address(routerAddress), "NEWSwap: ONLY_ROUTER_CAN_ACCESS");
        require(unlocked == 1, "NEWSwap: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value, bool isSwapping) private {
        ...
        if (routerAddress.pairFeeAddress(address(this)) == token && isSwapping){
            ...
            if(adminFee != 0){
                ...
                require(success && (data.length == 0 || abi.decode(data, (bool))), "NEWSwap: TRANSFER_FAILED");
                value = value.sub(getOutFee);
            }
            (bool success1, bytes memory data1) = token.call(abi.encodeWithSelector(SELECTOR, to, value));
            require(success1 && (data1.length == 0 || abi.decode(data1, (bool))), "NEWSwap: TRANSFER_FAILED");
        }else {
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, "NEWSwap: ONLY_FACTORY_CAN_CALL"); // sufficient check
        token0 = _token0;
        token1 = _token1;
        routerAddress = INEWSwapRouter(INEWSwapFactory(factory).routerAddress());
    }

    // update reserves and, on the first call per block, price accumulators
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), "NEWSwap: OVERFLOW_ERROR");
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = INEWSwapFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, "NEWSwap: INSUFFICIENT_LIQUIDITY_MINTED");
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, "NEWSwap: INSUFFICIENT_LIQUIDITY_BURNED");
    ...
    function swap(uint amount0Out, uint amount1Out, uint amount0Fee, uint amount1Fee, address to, bytes calldata data) external lock {
        ...
        (uint112 _reserve0, uint112 _reserve1,,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, "NEWSwap: INSUFFICIENT_LIQUIDITY");

        if(baseToken != address(0)) {
            require(amount0Fee > 0 || amount1Fee > 0, "NEWSwap: INSUFFICIENT_FEE_AMOUNT");
        }
        ...
        { // scope for _token{0,1}, avoids stack too deep errors
            ...
            require(to != _token0 && to != _token1, "NEWSwap: INVALID_RECEIVER");
            if (amount0Out > 0) {
            ...
            if (data.length > 0) INEWSwapCallee(to).newSwapCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
    ...
    // Token contract needs to implement a function to set the base token in its New-LP pair
    function setBaseToken(address _baseToken) external {
        require(msg.sender == token0 || msg.sender == token1, "NEWSwap: NOT_ALLOWED");
        require(_baseToken == token0 || _baseToken == token1, "NEWSwap: WRONG_ADDRESS");
    ...
    // This function can ONLY be called by the token contract
    // Token contract needs to implement a function to update fees in its New-LP pair
    function updateTotalFee(uint _totalFee) external returns (bool) {
        ...
        require(feeTaker == msg.sender, "NEWSwap: NOT_ALLOWED");
...
contract NEWSwapFactory is INEWSwapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(NEWSwapPair).creationCode));
    ...
    function routerInitialize(address _router) external {
        require(!routerInit, "NEWSwap: ROUTER_INITIALIZED_ALREADY");
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, "NEWSwap: CANNOT_BE_IDENTICAL_ADDRESSES");
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), "NEWSwap: CANNOT_BE_THE_ZERO_ADDRESS");
        require(getPair[token0][token1] == address(0), "NEWSwap: PAIR_ALREADY_EXISTS"); // single check is sufficient
        bytes memory bytecode = type(NEWSwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        INEWSwapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, "NEWSwap: ACCESS_DENIED");
        feeTo = _feeTo;
    }
