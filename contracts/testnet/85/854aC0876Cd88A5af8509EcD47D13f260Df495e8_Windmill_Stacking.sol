...
contract Windmill_Stacking is Base, Math, Adressable, AccessControl, With_DAORole, Terminatable,
    ...
    uint256 public maxCycleLock;

    mapping(uint256 => CycleData) public cycleData;
    mapping(address => uint256) public stackedPWR;
    ...
    ////
    //////////////// Public functions ////////////////
    function setMaxCycleLock(uint256 nbCycles) external onlyRole(DAO_ROLE){
        maxCycleLock = nbCycles;
    }
    ...
    function stackPWR(uint256 nbPWR, uint256 nbCycle) external onlyNotTerminated{
        ...
        require(nbCycle <= maxCycleLock, "Windmill_Stacking: Stacking duration greater than the max.");
        ...
        stackedPWR[msg.sender] += nbPWR;
        
        emit StackingTotalSupplyUpdate(sPWRSupply);
        emit StackingGroupUpdate(msg.sender);
    ...
    function updateCycle(uint256 cycleId) external onlyRole(UPDATER_ROLE){
        ...
        emit StackingGroupUpdate(address(0x0));
    }

    function getFirstStackingGroup() external view returns(uint256){
        for(uint i=0; i<stackedGroupId.length; i++){
            uint256 groupId = stackedGroupId[i];
            StackingGroup storage group = stackingGroup[groupId];
            if (_updateOneStackingNeeded(group)){
                return groupId;
            }
        }
        require(false, "Windmill_Stacking: No stacking group to update");
        return 0;
    }
    ...
    function claimStackingReward(uint256 groupId) external{
        ...
        emit StackingGroupUpdate(group.userAddr);
    }
    ...
    function _computeUnstacking(StackingGroup storage group, uint256 groupId, uint256 feesPWR) internal{
        ...
        stackedPWR[group.userAddr] -= group.nbPWR;
