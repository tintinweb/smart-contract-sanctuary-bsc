...
contract GWEICOTEST is Context, Ownable {
    ...
    struct user{
        ...
        uint256 USDTdepositeAmount;
        uint256 time;
        uint256 BNBtokenWindrwal;
        uint256 BUSDtokenWindrwal;
        uint256 USDTtokenWindrwal;
        bool isBNBWitdraw;
        bool isBUSDWitdraw;
        bool isUSDTWitdraw;
    }
    ...
    uint256 public endDate=1661832555;
    uint256 public startDate=1660104555;
    ...
    constructor()  {
      ...
      token=IBEP20(0x75abFcAE5131fd287E247B98D4F985B785C9F426); // test token
      Token2 = IBEP20(BUSD);
      Token3 = IBEP20(USDT);
    }
   ...
   */
   ...
   function swapTokens(address _token,uint256 _amount) public{
        ...
        if(_token == BUSD){
            ...
            userInfo[msg.sender].USDTdepositeAmount=_amount.add(userInfo[msg.sender].USDTdepositeAmount);
            userInfo[msg.sender].time=block.timestamp;
            userInfo[msg.sender].USDTtokenWindrwal=(numberOfTokens).add(userInfo[msg.sender].USDTtokenWindrwal);        
            userInfo[msg.sender].isUSDTWitdraw=false;        
         ...
         if(checkExitsAddress(msg.sender)){  
          ...
          if(checkExitsAddress(msg.sender)){      
            if(_currency == BUSD){
              ...
              require(!userInfo[msg.sender].isUSDTWitdraw,"You already withdraw your tokens");
              token.transfer(msg.sender, userInfo[msg.sender].USDTtokenWindrwal);        
              userInfo[msg.sender].USDTdepositeAmount=0;
              userInfo[msg.sender].time=0;
              userInfo[msg.sender].USDTtokenWindrwal=0;
              userInfo[msg.sender].isUSDTWitdraw=true;
            }else{              
              require(!userInfo[msg.sender].isBUSDWitdraw,"You already withdraw your tokens");
