...
contract APHCore is CoreBaseFunc, CoreSetting, APHCoreProxy {
    function initialize() external initializer {
        ...
        emit SetAdvancedInterestDuration(msg.sender, 0, advancedInterestDuration);
    }
     ...
     */
    function getLoanExt(uint256 nftId, uint256 loanId) external view returns (LoanExt memory) {
     ...
     */
    function getLoanConfig(address borrowTokenAddress, address collateralTokenAddress)
        external
        view
    ...
    function getActiveLoans(uint256 nftId) external view returns (Loan[] memory) {
        uint256 loanIndex = currentLoanIndex[nftId];
        Loan[] memory activeLoans = new Loan[](loanIndex);

        uint256 count = 0;
        for (uint256 i = 1; i <= loanIndex; i++) {
            if (loanExts[nftId][i].active) {
                activeLoans[count] = loans[nftId][i];
                count++;
            }
        }
        return activeLoans;
     ...
     */
    function getLoanCurrentLTV(uint256 loanId, uint256 nftId) external view returns (uint256 ltv) {
        Loan memory loan = loans[nftId][loanId];
        (uint256 rate, uint256 precision) = IPriceFeed(priceFeedAddress).queryRate(
        ...
        ltv = (loan.borrowAmount + Math.max(loan.minInterest, loan.interestOwed));
        ltv =
            ltv +
            ((loan.owedPerDay * (block.timestamp - uint256(loan.lastSettleTimestamp))) / 1 days);
        ltv = (ltv * WEI_PERCENT_UNIT * precision) / (loan.collateralAmount * rate);
    ...
    /**
      ...
      NOTE: This can be checked by comparing current LTV with liquidation LTV in loanConfig
     */
...
contract CoreBaseFunc is CoreBase {
        ...
        if (lastSettleForw[msg.sender] != 0) {
            ...
            if (targetBlock != 0) {
                if (targetBlock >= block.number) {
                    ...
                    forwAmount =
                        ((targetBlock - lastSettleForw[msg.sender]) * forwDisPerBlock[msg.sender]) +
    ...
    /**
      @dev Returns usable nftId of caller (tx.origin)
     */
    function _getUsableToken(uint256 nftId) internal view returns (uint256) {
        return IMembership(membershipAddress).usableTokenId(nftId);
    }
     ...
     */
    function _queryRate(address tokenA, address tokenB)
        internal
        view
     ...
     */
    function _queryRateUSD(address token) internal view returns (uint256 price) {
        return IPriceFeed(priceFeedAddress).queryRateUSD(token);
    }
}
...
contract APHCoreProxy is CoreBaseFunc {
    ...
    function borrow(
        ...
        (bool success, bytes memory data) = coreBorrowingAddress.delegatecall(
    ...
    function rollover(uint256 loanId, uint256 nftId)
        external
        returns (uint256 delayInterest, uint256 bountyReward)
    {
        (bool success, bytes memory data) = coreBorrowingAddress.delegatecall(
            abi.encodeWithSignature("rollover(uint256,uint256)", loanId, nftId)
        );
        if (!success) {
            if (data.length == 0) revert();
            assembly {
    ...
    {
        (bool success, bytes memory data) = coreBorrowingAddress.delegatecall(
            abi.encodeWithSignature("liquidate(uint256,uint256)", loanId, nftId)
        );
        if (!success) {
            if (data.length == 0) revert();
            assembly {
...
contract CoreSetting is CoreBaseFunc, CoreSettingEvent {
    ...
    function setPriceFeedAddress(address _address) external onlyManager {
    ...
    function setForwDistributorAddress(address _address) external onlyManager {
    ...
    function setRouterAddress(address _address) external onlyManager {
    ...
    function setCoreBorrowingAddress(address _address) external onlyManager {
    ...
    function setFeeController(address _address) external onlyManager {
    ...
    // Set value
    function setLoanDuration(uint256 _value) external onlyManager {
    ...
    function setAdvancedInterestDuration(uint256 _value) external onlyManager {
    ...
    function setFeeSpread(uint256 _value) external onlyManager {
        require(_value <= WEI_PERCENT_UNIT, "CoreSetting/value-exceed-100-percent");
    ...
    function registerNewPool(
        ...
        require(poolToAsset[_poolAddress] == address(0), "CoreSetting/pool-is-already-exist");

        address assetAddress = IAPHPool(_poolAddress).tokenAddress();
        IERC20(forwAddress).approve(forwDistributorAddress, type(uint256).max);
        IERC20(assetAddress).approve(routerAddress, type(uint256).max);
    ...
    function _setForwDisPerBlock(
        ...
        require(poolToAsset[_poolAddress] != address(0), "CoreSetting/pool-is-not-exist");
        ...
        emit SetFowPerBlock(msg.sender, _amount, _targetBlock);
    }
    ...
    function setupLoanConfig(
        ...
        LoanConfig memory configOld = loanConfigs[_borrowTokenAddress][_collateralTokenAddress];
        LoanConfig storage config = loanConfigs[_borrowTokenAddress][_collateralTokenAddress];
        ...
        config.liquidationLTV = _liquidationLTV;
