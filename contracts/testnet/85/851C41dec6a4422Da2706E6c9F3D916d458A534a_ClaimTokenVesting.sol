...
contract ClaimTokenVesting is Ownable, Pausable, ReentrancyGuard {
    ...
    function claim(uint256 _amount) external nonReentrant whenNotPaused {
        ...
        if (amountToTransfer > 0) {
            ...
            userInfo[_msgSender()].rewardDebt = amountToTransfer - _amount;
            user.totalReceived = user.totalReceived + amountToTransfer;
            emit ClaimedToken(_msgSender(), amountToTransfer);
        }
    }
