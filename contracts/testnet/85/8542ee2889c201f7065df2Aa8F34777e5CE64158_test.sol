...
import {SD59x18, mul, sub, div, wrap, exp} from "@prb/math/src/SD59x18.sol";
contract test {
    /**
     * f(x) = 1 - (1/2 * e^(-log2(2/Pi) * x - C * x^2))
     *  c = C = 0.3452
     *  p1 = -log2(2/Pi) = -0.7978
     *  p2 = c * x^2
     *  p3 = p1 * x
    ...
    SD59x18 public one;
    SD59x18 public two;
    SD59x18 public one_two;
    function f(SD59x18 x) public returns(SD59x18) {
        a = 1;
        SD59x18 p2 = x.mul(x).mul(c);
        SD59x18 p3 = p1.mul(x);
        SD59x18 p4 = exp(p3.sub(p2));
        SD59x18 p5 = one_two.mul(p4);
        return one.sub(p5);
    }
}