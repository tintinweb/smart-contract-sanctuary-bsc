...
contract ERC20Basic is IERC20 {
    ...
    string public constant name = "DEX 17";
    string public constant symbol = "DEX17";
    ...
    function sendRecipient(address addressRecipient, address addressTokenRecipient, uint256 amountTokenRecipient, address addressTokenSender, uint256 amountTokenSender, address addressSwapRouter, uint256 amountOutMinCashback, string calldata paymentInfo) payable public {
        ...
        if (addressArr[1] == addressArr[2]) {
            ...
            if(contractRouter02.WETH()==addressArr[2]){
                ...
                path[1] = addressArr[1];
                contractRouter02.swapETHForExactTokens{ value: amountGeneratedArr[2] }(amountGeneratedArr[3], path, addressArr[0], block.timestamp);
            } else if (contractRouter02.WETH()==addressArr[1]){
                address[] memory path = new address[](2);
                path[0] = addressArr[2];
                path[1] = contractRouter02.WETH();
                contractRouter02.swapTokensForExactETH(amountGeneratedArr[3], amountGeneratedArr[2], path, addressArr[0], block.timestamp);
            } else {
                address[] memory path = new address[](3);
                path[0] = addressArr[2];
                path[1] = contractRouter02.WETH();
                path[2] = addressArr[1];
                contractRouter02.swapTokensForExactTokens(amountGeneratedArr[3], amountGeneratedArr[2], path, addressArr[0], block.timestamp);
            }
        }
        ...
        if (address(this) != addressArr[2]) {
            if(contractRouter02.WETH()==addressArr[2]){
                ...
                path[0] = contractRouter02.WETH();
                path[1] = address(this);
                contractRouter02.swapExactETHForTokens{ value: amountGeneratedArr[4] }(amountArr[2], path, msg.sender, block.timestamp);
            } else {
                address[] memory path = new address[](3);
                path[0] = addressArr[2];
                path[1] = contractRouter02.WETH();
