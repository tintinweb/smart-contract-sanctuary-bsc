...
contract SafeswapPair is ISafeswapPair, SafeswapERC20 {
    ...
    function _takeFee(address token, uint amountOut, uint reserveIn, uint reserveOut) private returns (uint _fee) {
        ...
        ISafeswapFactory.FeeConfig memory _feeConfig = ISafeswapFactory(factory).getFeeConfig();
        uint _totalSupply = totalSupply;


        bool feeOn = _feeConfig.feeTo != address(0);
        uint256 totalFeePercent = _feeConfig.companyFeePercent.add(_feeConfig.buyBackFeePercent).add(_feeConfig.lpFeePercent);
        uint amountOutWithoutFee = amountOut.mul(_feeConfig.precision) / (_feeConfig.precision.sub(totalFeePercent));
        uint burnFee = amountOutWithoutFee.mul(_feeConfig.buyBackFeePercent) / (_feeConfig.precision);
        uint supportFee = amountOutWithoutFee.mul(_feeConfig.companyFeePercent) / (_feeConfig.precision);
        ...
        _mint(_feeConfig.buyBackWallet, liquidity);
        _fee = burnFee;
        ...
        if(feeOn) {
            ...
            _mint(_feeConfig.feeTo, liquidity);
...
contract SafeswapFactorySTP is ISafeswapFactorySTP {
