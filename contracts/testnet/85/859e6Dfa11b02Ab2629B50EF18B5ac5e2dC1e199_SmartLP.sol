...
contract SmartLP is SmartLPStorage, IBEP721, IBEP721Metadata {
    ...
    function buySmartLPforToken(uint256 amount) external {
      ...
      uint256 toLend = amount / 3;
      // we re-use LendedBNBAmount as LendedBUSDAmount
      tikSupplies[tokenCount].LendedBNBAmount = toLend;
      tikSupplies[tokenCount].LendedITokenAmount = lendingContract.mint(address(this), toLend);

      address[] memory path = new address[](2);
      path[0] = address(purchaseToken);
      path[1] = pancakeRouter.WETH();
      (uint[] memory amountsTokenBnb) = pancakeRouter.swapExactTokensForETH(amount - toLend, 0, path, address(this), block.timestamp + 1200);
    ...
    function updateLendingContract(address _newLendingContract) external onlyOwner {
        ...
        IERC20Upgradeable(paymentToken).approve(_newLendingContract, type(uint256).max);
