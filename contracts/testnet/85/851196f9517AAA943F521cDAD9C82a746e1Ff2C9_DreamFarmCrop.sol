...
abstract contract INft {
    ...
    function getSoldTimes(uint256 _tokenId)
        public
        view
        virtual
        returns (uint256);
    ...
    Ownable
{
    ...
    function setSupplies(uint256 _current_supply, uint256 _max_supply)
        public
        onlyOwner
    {
    ...
    function setNames(string memory name_, string memory symbol_)
        public
        onlyOwner
    {
        _name = name_;
    ...
    function _transfer(
        ...
        require(
            ownerOf(tokenId) == from || ownerOf(tokenId) == address(this),
    ...
    function setCurrentSupplyAndPrice(uint256 _num, uint256 _price)
        public
        onlyOwner
    {
        uint256 supply = SafeMath.add(current_supply, _num);
    ...
    function setSale(uint256 _tokenId, uint256 setType) public {
        address sender = _msgSender();
        require(ownerOf(_tokenId) == sender, "Caller not token owner");
    ...
    function offload(uint256 _tokenId, address receiver) public onlyOwner {
        _tokenFreeze[_tokenId] = 0;
        transferFrom(address(this), receiver, _tokenId);
    }

    function tokenState(uint256 tokenId) public view returns (uint256) {
        return _tokenFreeze[tokenId];
    }

    function upgrade(uint256[15] calldata _tokenIdList, address receiver)
        public
    {
    ...
    function buy(uint256 amount, uint256 adv_time) public payable {
        uint256 requiredValue = SafeMath.mul(amount, price);
        require(msg.value >= requiredValue, "Not_Enough_Payment");
        require(
            current_supply >= SafeMath.add(current_sold, amount),
            "Not_Enough_Stock"
        );
