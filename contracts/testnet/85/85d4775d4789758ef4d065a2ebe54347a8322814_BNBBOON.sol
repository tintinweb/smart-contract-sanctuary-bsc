...
contract BNBBOON is Ownable, ReentrancyGuard {
    ...
    uint256 public WALLET_DEPOSIT_LIMIT = 200 * 1e18; /* 200 BNB  */
    ...
    function fireBaboons() public nonReentrant {
        require(contractStarted, "Contract not yet Started.");
    ...
    function hireBaboons(address ref) public payable nonReentrant {
        require(contractStarted, "Contract not yet Started.");
        User storage user = users[msg.sender];
    ...
    function startWorld(address addr) public payable{
        if (!contractStarted) {
    		if (msg.sender == owner()) {
