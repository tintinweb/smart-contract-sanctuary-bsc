...
contract InvestorsDAO {
    ...
    function makeDaoProfit() public {
        ...
        for (uint256 i = 0; i < treasuries.length; i++) {
            ...
            uint256 balance = mf.balanceOf(treasury);
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < investors[_investor].vestings.length; i++) {
            availableVestingSum +=
                investors[_investor].vestings[i].amount -
                investors[_investor].vestings[i].amountWithdrawn;
        }
        return availableVestingSum + investors[_investor].fixedAmount;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        if (investors[_investor].fixedAmount != 0) {
            sendProfit(_investor);
        }

        investors[_investor].startLockIndex = index;
        investors[_investor].fixedAmount += _amount;
        totalLock += _amount;
    ...
    function vestingDeposit(
        ...
        mf.transferFrom(msg.sender, address(this), amount);

        if (investors[investor].fixedAmount != 0) {
            sendProfit(investor);
        }

        totalLock += amount;
        investors[investor].startLockIndex = index;
        investors[investor].vestings.push(
            Vesting(amount, vestingStartDate, vestingDuration, 0)
        );

        emit Deposit(investor, getTotalInvestmentAmount(msg.sender), index);
    }
    ...
    function sendProfit(address _investor) public {
        ...
        for (uint256 i = investors[_investor].startLockIndex; i < index; i++) {
            totalRevenue +=
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < investors[_investor].vestings.length; i++) {
            uint256 progress = (investors[_investor].vestings[i].amount *
                (block.timestamp -
                    investors[_investor].vestings[i].startDate)) /
                investors[_investor].vestings[i].duration;
            vestingAmount +=
                progress -
                investors[_investor].vestings[i].amountWithdrawn;
        }

        return investors[_investor].fixedAmount + vestingAmount;
    }
    ...
    function withdraw() public {
        ...
        sendProfit(_investor);
        uint256 fixedAmount = investors[_investor].fixedAmount;
        ...
        investors[_investor].fixedAmount = 0;
