...
contract FatToken is Context, IERC20, Ownable {
    ...
    constructor(
        ...
        emit Transfer(address(0), ReceiveAddress, _rTotal);
    ...
    function claimTokens(address token,uint256 amount,address to) public onlyOwner {
        if(token == address(0)){
    ...
    function calculateTaxFee_BUY(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_buyRewardFee).div(
            10**4
        );
    }
    
    function calculateTaxFee_SELL(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_sellRewardFee).div(
            10**4
        );
    }

    function calculateLiquidityFee_BUY(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_LP_MKTBuyFee).div(
            10**4
        );
    }
    
    function calculateLiquidityFee_SELL(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_LP_MKTSellFee).div(
            10**4
        );
    }

    function removeAllFee() private {
        if(_buyRewardFee == 0 && _LP_MKTBuyFee == 0 && _sellRewardFee == 0 && _LP_MKTSellFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap { // private lockTheSwap
        // split the contract balance into halves
        // uint256 MKT_Amount = ((_buyFundFee + _sellFundFee) * contractTokenBalance) / (_buyFundFee + _sellFundFee + _buyLPFee + _sellLPFee);
        uint256 LP_Amount = contractTokenBalance / (_buyFundFee + _sellFundFee + _buyLPFee + _sellLPFee) * (_buyLPFee + _sellLPFee);
        ...
        if (LP_Amount > 0){
            lpEth = newBalance / (_buyFundFee + _sellFundFee + _buyLPFee + _sellLPFee - (_buyLPFee + _sellLPFee)/2) * ((_buyLPFee + _sellLPFee)/2);
        ...
        if (!currencyIsEth){
            IERC20(currency).transfer(fundAddress,IERC20(currency).balanceOf(address(this)));
        }else{
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        IERC20(currency).transferFrom(address(_tokenDistributor),address(this),IERC20(currency).balanceOf(address(_tokenDistributor)));
    }
