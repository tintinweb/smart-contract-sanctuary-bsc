...
contract Raffle is Ownable, ReentrancyGuard {
    ...
    event EndRaffle(address indexed winner);
    ...
    function buyTicket(uint256 amount) external {
        ...
        totalTicket += amount;
        if (!exists[msg.sender]) users.push(msg.sender);
    ...
    function setEndTime(uint256 _time) external onlyOwner {
        endTime = _time;
        uint256 random = rand() + 1;
        uint256 t = 0;
        for (uint256 i = 0; i < users.length; i++) {
            if (random >= t && random <= t + tickets[users[i]]) {
                winner = users[i];
                break;
            }
            t += tickets[users[i]];
        }
        emit EndRaffle(winner);
    }
    ...
    function rand() public view returns (uint256) {
        ...
        return (seed - ((seed / totalTicket) * totalTicket));
    }
