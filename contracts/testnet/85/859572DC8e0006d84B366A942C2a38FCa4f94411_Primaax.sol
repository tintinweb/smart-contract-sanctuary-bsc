...
contract Primaax is ERC20, Ownable {
    ...
    function userPastStakes(address _userAddr, uint256 _index) external view returns (uint256 amt, uint256 start, uint256 end, uint8 plan_id) {
        User memory u = userInfo[_userAddr];
        DepositHistory memory dep = u.pastDepos[_index];
        return (dep.amount, dep.start, dep.end, dep.planId);
    }
    ...
    function stake(address _referrer, uint256 _amount, uint8 _planId) external {
        ...
        if(user.packId != 7) {
            if (user.totalStake >= 50000e18 && ftd >= 7) {
                ...
                user.packId = 4;
            } else if (user.totalStake >= 1000e18) {
                user.packId = 3;
            } else if (user.totalStake >= 500e18){
                user.packId = 2;
            } else {
                user.packId = 1;
            }
        ...
        user.pastDepos.push(DepositHistory(stakeAmt, block.timestamp + TIME_COUNT, block.timestamp + 366 minutes, _planId));
    ...
    function withdraw() external {
        ...
        user.pastWiths.push(WithdrawHistory(claimable / 10, block.timestamp));
    ...
    function getStakeHistoryLength(address _userAddr) external view returns(uint){
        User memory u = userInfo[_userAddr];
        return u.pastDepos.length;
    }
