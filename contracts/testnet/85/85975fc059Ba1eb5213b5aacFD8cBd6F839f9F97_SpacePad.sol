...
contract SpacePad is Pausable, Ownable, ReentrancyGuard {
    ...
    address immutable BUSD_ADDRESS;
    ...
    function deposit(uint256 _amount, address _referrer) external whenNotPaused nonReentrant {
        ...
        if(_referrer == address(0)) {
            IERC20(BUSD_ADDRESS).approve(msg.sender, _amount);
            IERC20(BUSD_ADDRESS).allowance(msg.sender, address(this));
            IERC20(BUSD_ADDRESS).safeTransferFrom(msg.sender, address(this), _amount);
        }
        else {
            uint256 fee = _amount * referrerFee / 10000;
            uint256 depositAmount = _amount - fee;
            IERC20(BUSD_ADDRESS).approve(msg.sender, fee);
            IERC20(BUSD_ADDRESS).allowance(msg.sender, _referrer);
            IERC20(BUSD_ADDRESS).safeTransferFrom(msg.sender, _referrer, fee);

            IERC20(BUSD_ADDRESS).approve(msg.sender, depositAmount);
            IERC20(BUSD_ADDRESS).allowance(msg.sender, address(this));
            IERC20(BUSD_ADDRESS).safeTransferFrom(msg.sender, address(this), depositAmount);
        }
