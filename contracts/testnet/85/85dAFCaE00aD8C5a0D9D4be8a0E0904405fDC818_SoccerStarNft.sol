...
PausableUpgradeable {
    ...
    mapping(address=>FreeMint[]) public userFreeMintMap;
   ...
   function updateProperty(uint[] memory tokenIds, SoccerStar[] memory _soccerStars)
    ...
    function publicSellMint(
    uint round, 
    BlindBoxesType boxType) public whenNotPaused  {
        require(isPublicRound(round), "NOT_PUBLIC_ROUND_NUM");
        require(isRoundOpen(round), "ROUND_NOT_OPEN");
        require(boxType != BlindBoxesType.presale, "PRESALE_BOX_NOT_ALLOWED");

        uint256 quantity = 0;uint j=0;
        FreeMint[] storage fmList = userFreeMintMap[msg.sender];
        for(;j<fmList.length;j++) {
            if(fmList[j].round == round && fmList[j].boxType == boxType) {
                quantity = fmList[j].count;
                break;
            }
        }
        require(quantity > 0, "NO_FREE_MINT_QUOTA");
        fmList[j].count = 0;
    ...
    function setFreeMint(
        ...
        BlindBoxesType[] calldata types, 
        uint[] calldata counts) external onlyAllowToCall {

        require(users.length == types.length && types.length == counts.length, "Invalid args");
        for(uint i=0; i<users.length; i++) {
            FreeMint[] storage fmList = userFreeMintMap[users[i]];
            bool isExist = false;
            for(uint j=0;j<fmList.length;j++) {
                if(fmList[j].round == round && fmList[j].boxType == types[i]) {
                    fmList[j].count += counts[i];
                    isExist = true;
                }
            }
            if(isExist) continue;
            fmList.push(FreeMint({
                round: round,
                boxType: types[i],
                count: counts[i]
            }));
    ...
    function getFreeMint(uint round, BlindBoxesType boxType, address user) view public returns(uint) {
        FreeMint[] storage fmList = userFreeMintMap[user];
        for(uint j=0;j<fmList.length;j++) {
            if(fmList[j].round == round && fmList[j].boxType == boxType) {
                return fmList[j].count;
            }
        }
        return 0;
    }
...
interface ISoccerStarNft {
    ...
    function updateProperty(uint[] memory tokenIds, SoccerStar[] memory _soccerStars) external;
