...
contract HesmanIDO is Ownable, ReentrancyGuard {
  ...
  event EventUserUnstake(address user, uint256 nftId, uint256 time);

  constructor(
    address _idoOwner,
    address _hesmanToken,
    address _paymentToken,
    address _nftStakeAddress
  ) {
    idoOwner = _idoOwner;
  ...
  function setTokenIdo(address _hesmanToken, address _paymentToken, address _nftStakeAddress) public onlyOwner {
    hesmanToken = ERC20(_hesmanToken);
    nftStakeAddress = _nftStakeAddress;
    paymentToken = ERC20(_paymentToken);
  }
  ...
  function setClaimTime(uint256[] calldata _startTimeList, uint256[] calldata _percentList) public onlyOwner {
    ...
    for (uint256 index = 0; index < _startTimeList.length; index++) {
      ...
      stageObject.amountClaimPerStage = _percentList[index].mul(totalNftStaked[msg.sender]).mul(tokenPerSlot).div(100);
  ...
  function stakeNftGetSlot(uint256[] calldata _idNftList) public validStakeTime nonReentrant isRun {
    ...
    require(_idNftList.length <= 5, "Max NFT is 5");
    ...
    for(uint index = 0; index < _idNftList.length; index++){
        ...
        IERC721(nftStakeAddress).transferFrom(msg.sender, address(this), _idNftList[index]);

    }

    totalNftStaked[msg.sender] = _idNftList.length;
  ...
  function buyIdo() public validBuyTime nonReentrant isRun {
    ...
    paymentToken.safeTransferFrom(msg.sender, idoOwner, payAmountPerSlot*totalNftStaked[msg.sender]);
  ...
  function claimIdo() public nonReentrant isRun {
    ...
    uint256 amountClaim = currentPercentVesting.mul(tokenPerSlot*totalNftStaked[msg.sender]).div(100);
    hesmanToken.safeTransfer(msg.sender, amountClaim);

    emit EventUserClaim(msg.sender, amountClaim, block.timestamp);
  }

  function unstakeNft() public nonReentrant validUnstakeTime {
    require(blacklistMap[msg.sender] == false, "You are in Blacklist");
    ...
    for (uint256 index = 0; index < totalNftStaked[msg.sender]; index++) {
      ...
      // Send NFT
      IERC721(nftStakeAddress).transferFrom(address(this), msg.sender, nftDetail.nftId);

      emit EventUserUnstake(msg.sender, nftDetail.nftId , block.timestamp);
    }
  }
  ...
  function urgentWithdrawAllToken() public onlyOwner {
    ...
    uint256 nftInContract = IERC721(nftStakeAddress).balanceOf(address(this));
    if (nftInContract > 0) {
      IERC721(nftStakeAddress).transferFrom(address(this), idoOwner, nftInContract);
    }
  }
