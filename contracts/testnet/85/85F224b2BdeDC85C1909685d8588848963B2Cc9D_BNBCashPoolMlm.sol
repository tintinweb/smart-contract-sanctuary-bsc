...
contract BNBCashPoolMlm is Ownable {
    ...
    uint constant ELIGIBLE_TO_STAKE = 4;
    ...
    function stake(uint level, bool allowSponsor, bytes memory signature, uint deadLine) public payable {
        address caller = _msgSender();
        User storage userStorage = user[caller];
        ...
        require(userStorage.currentLvl == level, "stake: user haven't reached level 1");
        bytes32 msgHash = keccak256(abi.encodePacked(caller,address(this), level, allowSponsor, msg.value, deadLine));
    ...
    function cashOut(CashOut[] memory params) public {
        address caller = _msgSender();
        User storage userStorage = user[caller];        
        ...
        for(uint index = 0; index < params.length; index++) {
            ...
            bytes32 msgHash = keccak256(abi.encodePacked(caller,address(this), param.level, param.isUpgradeExceed, param.deadLine));
            ...
            if (poolReserveId > 0) {
                if (!isUserExistInPool(caller, findPool(poolReserveId))) {
                    continue;
                }
            }
        ...
        _swapToBCP(caller, owner(), totalCashOut);
        _swapToBCP(owner(), address(0), exceedDiv);
    }

    function claimDividendPool(uint poolReserveId) public {
    ...
    function claimReservePool() public {
        address caller = msg.sender;
        User storage userStorage = user[caller];
        ...
        _swapToBCP(caller, owner(), dividend);
        _swapToBCP(owner(), address(0), fee);
    ...
    function _swapToBCP(address receiver, address referrer, uint amount) private {
        if(amount == 0) {
            return;
        }
        require(address(this).balance >= amount,"swapToBCP: not enough balance");
        (bool txStatus, ) = address(BCP).call{value:amount}(abi.encodeWithSelector(IBNBCashPool.purchase.selector, receiver,referrer));
