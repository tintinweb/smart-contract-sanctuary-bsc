...
contract NSIMidWallet is AccessControl {
    ...
    function disable(address tokenContractAddress) public onlyOwner{
        IERC20 token = IERC20(tokenContractAddress);
        require(token.balanceOf(address(this)) <=0 , "You have these tokens in Mid Wallet. Withdraw them before disabling this token for trading.");
        address spender = address(this);
        token.decreaseAllowance(spender,type(uint256).max);
