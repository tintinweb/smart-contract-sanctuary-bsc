...
contract PancakeDuels is ERC721, Ownable, ReentrancyGuard {
    ...
    event DuelFinished(uint duelId, uint boosterId, uint nftId, bool isBunnies, uint timestamp, address sender, uint rnd, bool takerWins, uint mintCount); 
    ...
    constructor(address _pb, address _ps, address _st, address _vrf, bool _isTestnet) 
    ...
    function mint(uint qty) external payable nonReentrant {
        ...
        vrfm.setMint(reqId, id, qty, isDev);
        if (!isDev) {
    ...
    function fulfillMint(uint id, uint randomValue, uint qty, bool isDev) external {
        ...
        for (uint i = id; i < end; i++) {
            ...
            if (!isDev && currentUserSupply > 0 && currentUserSupply % 190 == 0) {
                ...
                round2Fund[round] += 190 * (mintPrice - mintPrice / 20);
            }
            ...
            emit Minted(i, msg.sender, randomNumber);
        }
    }
    ...
    function openDuel(uint boosterId, uint nftId, bool isBunnies) external payable checkDuelBasics(boosterId) nonReentrant {
        ...
        duels.push(Duel({ finished: false, maker: msg.sender, makerBoosterId: boosterId }));
        duelCount += 1;
    ...
    function joinDuel(uint boosterId, uint nftId, bool isBunnies, uint duelId) 
        ...
        if (isTestnet) {
            ...
            vrfm.requestRandomWords();
            rndNumber = vrfm.getRandomNumber();
        }
        (bool takerIsWinner, uint mintCount) = pickSkillsAndDetermineWinner(boosterId, duel.makerBoosterId, rndNumber);

        emit DuelFinished(duelId, boosterId, nftId, isBunnies, block.timestamp, msg.sender, rndNumber, takerIsWinner, mintCount);
        duel.finished = true;
        boosterId2ActiveDuel[duel.makerBoosterId] = false;
        duelTimestamps[boosterId] = block.timestamp;
        duelTimestamps[duel.makerBoosterId] = block.timestamp;

        checkRoundTimeAndSet();
        distributeValue();
        if (mintCount > 0) {
            boosters[takerIsWinner ? boosterId : duel.makerBoosterId].multiplier += mintCount;
            round2MultiplierCount[currentRound] += mintCount;
            round2ID2Multiplier[currentRound][takerIsWinner ? boosterId : duel.makerBoosterId] += mintCount;
        }
    }
    ...
    function topUpSubscriptionMinting(uint256 amount) external onlyOwner {
        vrfm.topUpSubscription(amount);
    }

    function withdrawMintLink(uint amt, address to) external onlyOwner {
        vrfm.withdraw(amt, to);
    }
    ...
    function distributeValue() private {
        uint devFee = msg.value / 20;
        uint treasuryFee = msg.value - devFee; 
...
interface IVRFv2Minting {
    ...
    function setMint(uint rid, uint id, uint qty, bool isDev) external;
