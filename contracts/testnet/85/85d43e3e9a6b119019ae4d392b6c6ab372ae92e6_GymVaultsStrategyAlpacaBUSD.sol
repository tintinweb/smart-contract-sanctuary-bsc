    ...
    PausableUpgradeable
{
     ...
     */
    function deposit(address _user, uint256 _wantAmt)
        public
        onlyGymVault
        whenNotPaused
        strategyRunning
     ...
     */
    function withdraw(address _user, uint256 _wantAmt)
        public
        onlyGymVault
        nonReentrant
        returns (uint256)
    {
        ...
        uint256 userReward = userRewardAmount[_user];
        ...
        _safeRewardTransfer(_user, userReward);
        busdBalance -= userReward;
        userRewardAmount[_user] = 0;
    ...
    function claimAlpacaRewards(address _user) external onlyGymVault {
        _safeRewardTransfer(_user, userRewardAmount[_user]);
    }
    ...
    function transferDustBusd() external onlyOwner {
        uint256 busdBal = IERC20Upgradeable(wantAddress).balanceOf(address(this));
        IERC20Upgradeable(wantAddress).safeTransfer(msg.sender, busdBal);
    }

    function transferDustAlpaca() external onlyOwner {
        uint256 alpacaBal = IERC20Upgradeable(alpacaTokenAddress).balanceOf(address(this));
        IERC20Upgradeable(alpacaTokenAddress).safeTransfer(msg.sender, alpacaBal);
    }
    ...
    function _swapAlpacaToWant(address _user) private {
        uint256 earnedAmt = IERC20Upgradeable(alpacaTokenAddress).balanceOf(address(this));
        uint256 _distributeFee = distributeFees(earnedAmt);
        ...
        if (alpacaTokenAddress != wantAddress && earnedAmt > 0) {
            earnedAmt = earnedAmt - _distributeFee;
            IERC20Upgradeable(alpacaTokenAddress).safeIncreaseAllowance(uniRouterAddress, earnedAmt);
            uint256 busdAmount = IPancakeRouter02(uniRouterAddress).getAmountsOut(earnedAmt, earnedToWantPath)[1];
            ...
            userRewardAmount[_user] += busdAmount;
        }
    }
