...
contract NDBcoin is IBEP20, Auth ,INDBcoin{
    using SafeMath for uint256;
    ...
    function swapBack() internal swapping {
        ...
        if(amountToLiquify > 0){
            ...
            amountTokenMin = amountTokenMin.mul(80).div(10000); //slippage 0.8%
    ...
    function buyTokens(uint256 amount, address to) internal swapping {
        ...
        amountOutMin=amountOutMin.mul(80).div(10000);//slippage 0.8%
        ...
        onlyOwner 
    {
        ...
        require(tokenAmount >0 , 'balance is not enough');
        ...
        )
    {
        ...
        if (buyTax.percentOfSellPrice > 0) {
            ...
            if (currentReserves.token == 0 || currentReserves.bnb == 0) {
                ...
                // calculating bnb amount to get optimal token amounts before calling swap.
                uint256[] memory amounts = router.getAmountsIn(syncAmounts.token, path);
                syncAmounts.bnb = amounts[0];
                
                if ((address(this).balance) > syncAmounts.bnb && syncAmounts.bnb > 0) {
                    // all ok
                    
                    eventState = NeedToEmitEvent.None;
                    success = true;
                } else {
                    eventState = NeedToEmitEvent.NotEnoughBNBToBuyTokens;
                }
        ...
        )
    {
        ...
        if (success == true) {
            ...
            } else if (eventState == NeedToEmitEvent.NoAvailableReserveBNB) {
                emit NoAvailableReserveBNB();
            } else if (eventState == NeedToEmitEvent.NotEnoughTokenToSell) {
                emit NotEnoughTokenToSell(syncAmounts.token);
            } else if (eventState == NeedToEmitEvent.NotEnoughBNBToBuyTokens) {
                emit NotEnoughBNBToBuyTokens(syncAmounts.bnb);
            }
        }
    }
        ...
        )
    {
        ...
        if (currentReserves.token == 0 || currentReserves.bnb == 0) {
            ...
            if (sellTokenAmount > 0) {
                ...
                uint256 _amountBnb = router.getAmountOut(sellTokenAmount, currentReserves.token, currentReserves.bnb);
            
                _amountBnb = _amountBnb.sub(_amountBnb.mul(sellTax.slippage).div(100));
                
                if (balanceOf(address(this)) >= sellTokenAmount) {
                    
                    if (_amountBnb < currentReserves.bnb && _amountBnb > 0) {
                        syncAmounts.token = sellTokenAmount;
                        syncAmounts.bnb = _amountBnb;                  
                        
                        eventState = NeedToEmitEvent.None;
                        success = true;
                        
                    } else {
                        eventState = NeedToEmitEvent.NoAvailableReserveBNB;
                    }
                } else {
...
interface INDBcoin {
    ...
    event PresaleBuy(address sender, uint bnb, uint256 token);
    
    event NotEnoughTokenToSell(uint256 amount);
    event NotEnoughBNBToBuyTokens(uint256 amount);
    event NoAvailableReserves();
    event NoAvailableReserveToken();
    event NoAvailableReserveBNB();
    
    event ContractSellTokens(uint256 amount, uint256 bnb);
    event ContractBuyBackTokens(uint256 amount, uint256 bnb);
    ...
    enum NeedToEmitEvent { 
        ...
        ShouldBuy,
        NoAvailableReserves,
        NoAvailableReserveBNB,
        NotEnoughTokenToSell,
        NotEnoughBNBToBuyTokens
    }
}