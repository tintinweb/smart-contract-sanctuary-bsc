...
contract ValkyrieDoge is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 86 * 10**8 * 10**9;
    ...
    // Reflection
    uint256 public _liquidityFee = 4; //reflection distribution to holders
    ...
    constructor(address networkV2Router02, address payable marketingAddress) {
    ...
    function _transferBothExcluded(
        ...
        _takeMarketing(sender, _marketingAddress, tMarketing);
        ...
        )
    {
        ...
        uint256 tTransferAmount = tAmount.sub(tBurn).sub(tLiquidity).sub(
            tMarketing
        );
    ...
    function _getRValues(
        ...
        uint256 rTransferAmount = rAmount.sub(rBurn).sub(rLiquidity).sub(
            rMarketing
        );
    ...
    function _transfer(
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && from != networkV2Pair) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) public lockTheSwap {
        uint256 tokenAmountToSell = contractTokenBalance.div(_marketingFee);
        ...
        _impactTranfers(payable(address(this)));
    ...
    function _takeMarketing(
        ...
        if (tMarketing > 0) {
            uint256 currentRate = _getRate();
            uint256 rMarketing = tMarketing.mul(currentRate);
            _rOwned[recipient] = _rOwned[recipient].add(rMarketing);

            if (_isExcluded[recipient])
                _tOwned[recipient] = _tOwned[recipient].add(tMarketing);
            ...
            emit Transfer(sender, recipient, tMarketing);
        }
    }

    function _impactTranfers(address payable sender) private {
        _tMarketingTotal = _tMarketingTotal.add(
            _transferETH(sender, _marketingFee, _marketingAddress)
        );
    }
    ...
    function _transferETH(
        ...
        uint256 amount = sender.balance.div(_fee).mul(_fee);
        _address.transfer(amount);
        emit Transfer(sender, _address, amount);
        return amount;
    }

    //to recieve Main Currency from networkV2Router when swaping
