...
contract Crowdfunding is AccessControlEnumerable {
    ...
    uint256 public constant MAX_FUND_AMOUNT = 2**256 - 1; // 500e18; //单用户最大投资金额
    ...
    function setEquitys(
        uint256 _index,
        uint16 _equity /*onlyRole(OPERATER_ROLE)*/
    ) external {
        equitys[_index] = _equity;
    }

    function upgrade(
        address _addr /*onlyRole(OPERATER_ROLE)*/
    ) external {
    ...
    function setAddresses(
        address _fee,
        address _donate,
        address _stake /*onlyRole(OPERATER_ROLE)*/
    ) external {
        fee = _fee;
        donate = _donate;
        stake = _stake;
    ...
    function setRate(
        uint16 index,
        uint16 rate /*onlyRole(OPERATER_ROLE)*/
    ) external {
        rates[index] = rate;
    }

    function setMaxReserveRound(
        uint16 _maxRound /*onlyRole(OPERATER_ROLE)*/
    ) external {
    ...
    function setRegionParams(
        ...
        uint256 _period,
        uint16 _multiplier,
        uint16 _restartType /*onlyRole(OPERATER_ROLE)*/
    ) external {
    ...
    function addRegion(
        ...
        uint16 multiplier,
        uint256 startTime,
        uint16 restartType /*onlyRole(OPERATER_ROLE)*/
    ) external {
    ...
    function addProject(
        uint256 regionIndex /*onlyRole(OPERATER_ROLE)*/
    ) external {
    ...
    function fund(uint256 _regionIndex, uint256 _amount) external {
        ...
        uint256 _maxAmount = getMaxAmountPerTime(p.targetAmount, regions[_regionIndex].poolType);
    ...
    function pending(
        ...
        returns (
            ...
            uint256 freeWithdrawAmount, //免费提现额度
            uint256 priorityFundAmount, //优先额度
            uint256 brokerReward //分给相应经纪人奖励
        )
    {
        Project storage p = projects[_regionIndex][_round];
        ProjectFund storage pf = projectFunds[_account][_regionIndex][_round];
    ...
    function _harvest(
        ...
        if (_brokerReward > 0) {
            //以为_brokerReward 已经乘了经纪人奖比例rates[1] 这里须除以rates[1]再按极差算
            processBrokerReward(msg.sender, (_brokerReward * 1000) / rates[1]);
        }
    ...
    function processBrokerReward(address account, uint256 amount) internal {
        ...
        for (uint256 i = 0; i < 4; i++) {
            if (_equities[i] > 0) {
                uint256 _amount = (amount * _equities[i]) / 1000;
        ...
        if (amount > used) {
            IERC20(lp).transfer(fee, amount - used);
        }
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < pids.length; i++) {
            (, uint256 _refundAmount, uint256 _rewardAmount, uint256 _freeWithdrawAmount, uint256 _priorityFundAmount, uint256 _brokerReward) = pending(
                _account,
                _regionIndex,
                pids[i]
            );
    ...
    function harvestAll(uint256 _regionIndex) public {
        ...
        (uint256 _refundAmount, uint256 _rewardAmount, uint256 _freeWithdrawAmount, uint256 _priorityFundAmount, uint256 _brokerReward) = pendingAll(
            msg.sender,
            _regionIndex
        );
        ...
        _harvest(_refundAmount, _rewardAmount, _freeWithdrawAmount, _priorityFundAmount, _brokerReward);
    }
    ...
    function pendingForUnlock(address _account) public view returns (uint256 lockedAmount, uint256 unlockAmount) {
        ...
        if (accounts[msg.sender].pointFlag) {
            uint40 point = IMetaPoint(metaPoint).accounts(msg.sender).point;
            uint40 _point = accounts[msg.sender].point;
            if (point > _point) {
    ...
    function setAddresses1(
        address _lp,
        address _metaPoint /*onlyRole(OPERATER_ROLE)*/
    ) external {
        lp = _lp;
