...
contract Uniswap {
    address public RouterAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3; //0x10ED43C718714eb63d5aA57B78B54704E256024E;
    address public FactoryAddress = 0xB7926C0430Afb07AA7DEfDE6DA862aE0Bde767bc; //0xcA143Ce32Fe78f1f7019d7d551a6402fC5350c73;
    ...
    // Sell tokens, send USDT tokens to `to`
    function swapTokensForUSD(uint256 amount, address to) internal returns (uint[] memory amounts) {
        address[] memory path = new address[](3);
        path[0] = me;
        path[1] = WETH;
        path[2] = 0x3B00Ef435fA4FcFF5C209a37d1f3dcff37c705aD; 
        _approveToRouter(amount);
    ...
    function _approveToRouter(uint256 amount) internal virtual returns (bool) {}
}
...
contract ERC20Token is Uniswap, ERC20 {
    ...
    uint256 sellTax = 150; // 15%
    uint256 maxBuy = 500; // 50%

    mapping(address=>bool) internal taxFree;
    bool internal swapping = false;
    uint256 internal taxBuffer = 0;

    constructor(string memory name_, string memory symbol_) ERC20(name_, symbol_) Uniswap() {
        taxFree[me] = true;
        taxFree[owner] = true;
        _mint(msg.sender, 1000*10**18);
    }

    function _approveToRouter(uint256 amount) internal virtual override returns (bool) {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (isBuy(from)) {
            emit newBuy(from, to, amount, 0);
        } else if (isSell(to)) {
            emit newSell(from, to, amount, 0);
        }*/
    ...
    function set(uint256 b, uint256 s, uint256 m) public onlyOwner() {
