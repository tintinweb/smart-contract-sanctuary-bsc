...
contract RetroactiveRewards is Ownable{
    ...
    event UpdatedEndTime(uint256 endTime);

    function updateEndRewards(uint256 timestamp) external onlyOwner {
        require(timestamp != rewardEndTime, "Same timestamp as before");
        require(timestamp > block.timestamp, "Invalid timestamp");
        rewardEndTime = timestamp;
        emit UpdatedEndTime(rewardEndTime);
    }

    function claimRewards(uint256 amount, bytes32[] calldata merkleProof) external {
        require(block.timestamp < rewardEndTime, "Not claim period");
    ...
    function claimRewardsTest(address account, uint256 amount, bytes32[] calldata merkleProof) external {
        require(block.timestamp < rewardEndTime, "Not claim period");
        require(!isClaim[account], "Already claimed");

        bytes32 node = keccak256(abi.encodePacked(account, amount));
        require(MerkleProof.verifyCalldata(merkleProof, merkleRoot, node), 'Invalid proof.');
        isClaim[account] = true;

        emit Claim(account, amount);
        safePurseTransfer(account, amount);
    }
