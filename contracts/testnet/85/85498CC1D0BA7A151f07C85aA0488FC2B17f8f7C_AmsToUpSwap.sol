...
contract AmsToUpSwap is Ownable {
    ...
    function init(address _usdtJia, address _UP, address _amsswap,
        ...
        IERC20(_usdtJia).approve(address(_amsswap), uint256(- 1));
    }
    ...
    function upPutUsdRsUp(uint256 _num, uint256 _type, address _usr) external returns (uint256){
        ...
        IERC20(usdtJia).transferFrom(amsswap, UP, _num);//up业务完成，抽取假u
    ...
    function sellUp(uint256 _amount) external {
        ...
        IERC20(UP).transferFrom(msg.sender, address(0), _amount);
        IERC20(usdtJia).transferFrom(UP, address(this), usdtnum);
        ...
        //余下假u兑换真u给用户
        INewAms(amsswap).upSellUsdtAndAms(IERC20(usdtJia).balanceOf(address(this)), 1, msg.sender);
    }
