...
contract Numr is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint public priceBidFourNumer = 300000000000000000;
    uint public priceBidThreeNumer = 3000000000000000000;
    ...
    function mintReserve(uint[] memory _tokenId) internal {
    ...
    function bid(uint tokenId, uint _bid, address ref) external nonReentrant payable {
        ...
        require(msg.value >= _bid && _bid >= priceBid && _bid >= minBid[tokenId] && _bid > maxBid[tokenId]);
        require(isTimeBid);
        require(!_exists(tokenId));
        require(tokenId < thresholdPaidMint && tokenId > 0);
        addSponsor(msg.sender, ref);
    ...
    function cancelBid(uint tokenId) external nonReentrant{
        require(userBid[tokenId] == msg.sender);
        require(payable(msg.sender).send(maxBid[tokenId]));
        maxBid[tokenId] = 0;
        userBid[tokenId] = address(0);
        bidUser[msg.sender][tokenId] = 0;
    }
    ...
    function combine(uint[] memory tokens) external payable {
    ...
    function getTier(uint tokenId) internal pure returns (uint value) {
        if (tokenId < 10) {
            value = 1;
        } else if (tokenId < 100) {
            value = 2;
    ...
    function getMetadata(uint tokenId) internal view returns (string memory) {
        ...
        if (!_exists(tokenId)) {
            ...
            ', "Tier": ', utils.uint2str(getTier(tokenId)),'}], "image": "https://imgtest.numr.io/',
            utils.uint2str(tokenId),
            '"}'
        ));
        }
