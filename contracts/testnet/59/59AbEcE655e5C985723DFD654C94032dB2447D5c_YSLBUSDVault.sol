...
contract YSLBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    uint256 startTime;// startTime as uint
    uint256 public No_of_Epochs;//Number of Epochs as uint
    uint256 public no_of_days;// Number of days as uint
    ...
    uint256 public USDyOfDollarValue;
    ...
    */
    ...
    function initialize(
        ...
        USDyBUSD = Admin.USDyBUSD();
        yslBUSD_A = Clones.clone(Admin.masterNTT());
        IReceipt(yslBUSD_A).initialize(_admin,address(this), "YSL-BUSD-A", "YSL-BUSD-A");
        depositTax = 10;
        withdrawTax = 0;
        exchangeRatio = 10 ** 18;
    }
    ...
    modifier _lastExchangeRatio(){
        ...
        if(lastExchangeRatio > exchangeRatio){
            require(lastExchangeRatio > exchangeRatio, "YSLBUSDVault: exchangeRatio decreased.");        
            _pause();
        }
    }
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused {
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "YSLBUSDVault: address is Blacklisted");
        require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender) || user == msg.sender);
        require(amount > 0,"YSLBUSDVault: LPamount can't be 0");
        require(isBUSD,"YSLBUSDVault: Only Deposit BUSD");
        ...
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(restrictTransfer[user] != block.number, "YSLBUSDVault: you can't interact in same block");
            require(!isContract(msg.sender),"YSLBUSDVault: No external contract interaction");
        }
        IERC20(Admin.BUSD()).safeTransferFrom(user,address(this),amount);
        IERC20(Admin.BUSD()).safeTransfer(Admin.Treasury(), amount / 10);
        uint reserveYSL = IERC20(Admin.YSL()).balanceOf(address(yslBUSD));
        uint reservesBUSD = IERC20(Admin.BUSD()).balanceOf(address(yslBUSD));
        uint YSLRequire = (reserveYSL * amount * 30)/(reservesBUSD * 100);
        uint USDyLiquidity = (IERC20(Admin.USDy()).balanceOf(address(USDyBUSD)) * (amount * 1/2)/ IERC20(Admin.BUSD()).balanceOf(address(USDyBUSD)));
        uint USDyPrice  = IReceipt(Admin.USDy()).mintByPrice();
        uint USDyAmount;
        if(IERC20(Admin.YSL()).balanceOf(address(this)) >= YSLRequire){
            percentYSL = 30;
            USDyAmount = ((2 * amount * (10 ** 18))/USDyPrice);
            IReceipt(Admin.USDy()).mint(address(this), USDyLiquidity + USDyAmount);
            convertBUSDToLP(amount * percentYSL/100, (reserveYSL * (amount * percentYSL/100)) / reservesBUSD, Admin.BUSD(), Admin.YSL(), address(yslBUSD));
        }
        else{
            percentYSL = 15;
            ...
            path[0] = Admin.BUSD();
            path[1] = Admin.YSL();
            IERC20(Admin.BUSD()).safeApprove(router, USDyAmount);
            swap(
                (amount * percentYSL/100),
                path,
                address(this)
            );
            reserveYSL = IERC20(Admin.YSL()).balanceOf(address(yslBUSD));
            reservesBUSD = IERC20(Admin.BUSD()).balanceOf(address(yslBUSD));
            IReceipt(Admin.USDy()).mint(address(this), USDyLiquidity + (amount* (10 ** 18)/USDyPrice));
            convertBUSDToLP(reservesBUSD * IERC20(Admin.YSL()).balanceOf(address(this))/ reserveYSL, 
                IERC20(Admin.YSL()).balanceOf(address(this)), Admin.BUSD(), Admin.YSL(), address(yslBUSD));

        }
        convertBUSDToLP((amount * 1/2), USDyLiquidity,Admin.BUSD(), Admin.USDy(), address(USDyBUSD));

        (uint256 BUSDAmount, uint256 leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution
                    (msg.sender,(amount * 9) / 100,amount);
        if (BUSDAmount != 0) {
            IERC20(Admin.BUSD()).safeTransfer(msg.sender, (amount * 1) / 100);
            IERC20(Admin.BUSD()).safeTransfer(Admin.Refferal(), BUSDAmount);
        }else{
            IERC20(Admin.BUSD()).safeTransfer(Admin.TeamAddress(), (amount * 1) / 100);
        }
        if (leftAmount != 0) {
            IERC20(Admin.BUSD()).safeTransfer(Admin.TeamAddress(), leftAmount);
        }
        ...
        USDyOfDollarValue += USDyAmount;
        exchangeRatio = USDyOfDollarValue * (10 ** 18)/(IERC20(yslBUSD_A).totalSupply());
        emit Deposit("YSLBUSDVault",address(this),user,amount,block.number,block.timestamp);
    }
    ...
    */
    ...
    function withdraw(address user, bool isReciept,uint _amount,address sendTo) nonReentrant external _lastExchangeRatio whenNotPaused{
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "YSLBUSDVault: address is Blacklisted");
        require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender) || (sendTo == msg.sender && user == msg.sender),"YSLBUSDVault: No external contract interaction");
        require(_amount <= UserDeposit[user],"YSLBUSDVault: Invalid Amount");
        if(UserDeposit[user] > 0){
            _claimRebaseReward(user);
        }
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
    ...
    */
    ...
    function claimRebaseReward(address user) public nonReentrant whenNotPaused{
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "YSLBUSDVault: address is Blacklisted");
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(restrictTransfer[user] != block.number, "YSLBUSDVault: you can't interact in same block");
            require(isContract(msg.sender),"YSLBUSDVault: No external contract interaction");
    ...
    function convertBUSDToLP(uint amountBUSD,uint amountToken2,address path0,address path1,address lp) internal{
        IERC20(path0).safeApprove(Admin.liquidityProvider(), amountBUSD);
        IERC20(path1).safeApprove(Admin.liquidityProvider(), amountToken2);
        ILiquidityProvider(Admin.liquidityProvider()).addLiquidity(
            path0,
            path1,
            amountBUSD,
            amountToken2,
        ...
        uint lpPrice = lpDollarValue(lp, IUniswapV2Pair(lp).token0(), IUniswapV2Pair(lp).token1());
    }
    ...
    */
    ...
    function rewards(address user) public view returns(uint){
        return(((_userAmount(user) * (currentReward - ss[msg.sender])) / 10 ** 18));
    }
    
    function lpDollarValue(address lp,address tokenA, address tokenB) public view returns(uint256){
        address[] memory path = new address[](2);
        path[0] = tokenA;
        path[1] = tokenB;
        (uint amountTokenA,uint amountTokenB,) = IUniswapV2Pair(lp).getReserves();
        uint dollarval=IUniswapV2Router01(router).getAmountsOut(amountTokenA,path)[1];
        uint Lpprice = (((amountTokenA * dollarval) + (amountTokenB))/(IUniswapV2Pair(lp).totalSupply()));
        return Lpprice;
    }

    function _claimRebaseReward(address user) internal {
        userAmount = ((UserDeposit[user] * exchangeRatio)/ IERC20(address(yslBUSD)).totalSupply());
        reward = ((userAmount * (currentReward - ss[user])) / 10 ** 18);
        if(Admin.PhoenixNFT() == address(0)){
            IERC20(Admin.USDy()).safeTransfer(user, reward);
        }
        else{
            IERC20(Admin.USDy()).safeTransfer(user, reward + (reward/ 4));
    ...
    /**
    @dev Function will provide user amount through exchange ratio
    @param user , address of user
    */

    function _userAmount(address user) internal view returns(uint){
        return((UserDeposit[user] * exchangeRatio)/ IERC20(address(yslBUSD)).totalSupply());
    }
