...
contract GrabSv3 is Ownable{
    ...
    *
    ...
    function getLastStakeDate (uint _poolID ) public view returns (uint){
        uint stakeTimes = pool[_poolID].lastTimeUserStaked[msg.sender];
        uint PoolStakeDuration = (stakeTimes + (pool[_poolID].stakingDuration));
        return PoolStakeDuration;
    }
