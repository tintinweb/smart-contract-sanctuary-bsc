...
contract bonusC {
	function nftbonustime(uint256) public returns (uint256) {}
}
...
contract UCenterContract{
	...
	address public addFoundation = 0x813D35c87f931DC18a3F6C6be405485AD34149f4; 
	bonusC public _bonusC;  
	...
	uint256 public _yNFTnum = 1;
	uint256 public _gNFTnum = 1;
	...
	function BonusUp(uint256 _times) public {
		require(bonustime<_times, "Err: error bonus time");
		uint256 _value = _bonusC.nftbonustime(_times);
		require(_value>0, "Err: error bonus time");
		...
		if (_yNFTnum>0){
			...
			_LP.transfer(addFoundation, _value);
		}	
