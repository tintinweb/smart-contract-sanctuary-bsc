...
abstract contract Referral is Initializable, OwnableUpgradeable {
    ...
    function payReferral(uint256 value, address _referee) internal {
...
contract PresaleUpgradable is Referral, UUPSUpgradeable, PausableUpgradeable {
    ...
    function _getTokensValueUSD(
        ...
        tokenValue = (_USDValue * _price) / 10 ** _getUSDContractDecimals();
    }

    function _getTokensValueBUSD(
        uint256 _USDValue,
        uint256 _price
    ) private view returns (uint256 tokenValue) {
        tokenValue = (_USDValue * _price) / 10 ** _getBUSDContractDecimals();
    }

    function _payReferralInBNB(uint256 value, address _referee) private {
        Account memory userAccount = accounts[_referee];
        uint totalReferal;
    ...
    function BuyWithUSD(
        ...
        uint256 tokenValue = _getTokensValueUSD(_msgValue, PricePerUSDT);
    ...
    function BuyWithBUSD(
        ...
        uint256 tokenValue = _getTokensValueBUSD(_msgValue, PricePerUSDT);
    ...
    function _getUSDContractDecimals() private view returns (uint256) {
        return IERC20_EXTENDED(USDContract).decimals();
    }
    ...
    function _getBUSDContractDecimals() private view returns (uint256) {
        return IERC20_EXTENDED(BUSDContract).decimals();
    }
