...
contract BCNFTStaking is Context, Ownable {
    ...
    function stake() external {
        ...
        stakers[msg.sender].stakeLevel = getStakeLevel(msg.sender);
        stakers[msg.sender].lastStartTime = _getNow();
        stakers[msg.sender].readyTime = _getNow() + SECONDS_PER_DAY;
        stakers[msg.sender].isStaking = true;
    }

    function isStakable (address owner) internal returns(bool) {
        bool isStaked = false;
    ...
    function getStakeLevel(address _owner) internal view returns (uint256) {
        uint256 _stakeReward = 0;

        if(levelOfOwner[_owner][1] && levelOfOwner[_owner][2]) {
            _stakeReward = 5;
        }
    ...
    function claimReward() external returns(bool) {
        ...
        bool result = iBCToken.transfer(rewardTokenAddress, msg.sender, totalToken.mul(stakers[msg.sender].stakeLevel.add(100)).div(1000));
