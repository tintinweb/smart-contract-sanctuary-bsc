...
library SafeswapLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'60f510fa97fbadb0ccee73e60db92b5fd7eacc7499e8ff2db819ee1764ede42e' // init code hash
            ))));
    }
...
contract SafeswapRouter is ISafeswapRouter02 {
    ...
    function getTokenDeduction(address token, uint amount) external view returns(uint, address) {
        ...
        uint subt = nameToInfo[token].expectedBuy.sub(nameToInfo[token].actualBuy);
        uint deduction = amount.mul(subt).div(100-subt);
