...
contract MetaproRoyalty is Ownable, ReentrancyGuard {
    ...
    // @dev: dictionary with RoyaltyTeamMember[] - token_id -> RoyaltyTeamMember[]
    mapping(uint256 => RoyaltyTeamMember[]) private royaltyTeamMembers;

    // @dev: dictionary with RoyaltyTeamConfiguration - token_id -> RoyaltyTeamConfiguration
    mapping(uint256 => RoyaltyTeamConfiguration) public royaltyTeam;
    ...
    {
        require(
            token.creators(_tokenId) == msg.sender,
            "Account needs to be creator of the token"
        );

        require(_members.length > 0, "Members should not be empty");

        RoyaltyTeamConfiguration storage currentTeam = royaltyTeam[_tokenId];
        ...
        royaltyTeam[_tokenId] = teamConfiguration;
        currentTeamId += 1;
        ...
        royaltySumShouldBeCorrect(_royaltyMemberFees)
    {
        ...
        for (uint256 i = 0; i < _members.length; i++) {
            ...
            if (_members.length >= currentTeamMembers.length) {
    ...
    {
        RoyaltyTeamMember[] storage allMembers = royaltyTeamMembers[_tokenId];
    ...
    {
        RoyaltyTeamConfiguration storage team = royaltyTeam[_tokenId];
        return team;
    }
