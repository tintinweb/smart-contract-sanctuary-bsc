...
contract GymVaultsBank is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        console.log("dasdsa",block.number , _startBlock);
        ...
        alpacaToWBNB = [alpaca, WBNBAddress];
    ...
    function claimUserBnbReward(uint256 _pid, address _user) public {
        ...
        if (user.shares > 0) {
            // _claim(_pid, _user); //新需求要求只领羊驼金融收益
    ...
    function deposit(
        ...
        if (address(pool.want) == WBNBAddress && _wantAmt == 0) {
            // If `want` is WBNB
            IWETHGym(WBNBAddress).deposit{value: msg.value}();
            _wantAmt = msg.value;
        }
    ...
    function _claim(uint256 _pid, address _user) internal {
        ...
        if (pending > 0) {
            ...
            console.log("_claim pending distributeRewardTokenAmt",pending, distributeRewardTokenAmt);
            ...
            console.log("_claim pending toUser",(pending-distributeRewardTokenAmt));
    ...
    function _deposit(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        console.log("deposit user.shares",user.shares);
        if (user.shares > 0) {
            _claim(_pid, msg.sender);
        }
        console.log("deposit _wantAmt",_wantAmt);
        if (_wantAmt > 0) {
            ...
            console.log("deposit wantBal",wantBal);
            uint256 investment = IGymMLM(relationship).investment(msg.sender);
            console.log("deposit investment",investment);
            uint256 accumulated = (wantBal - investment);
            console.log("deposit accumulated",accumulated);

            if (user.shares > 0 && accumulated > 100) {
                uint256 sharesRemoved = IStrategy(poolInfo[_pid].strategy).withdraw(msg.sender, accumulated);
                console.log("deposit strategy.sharesRemoved",sharesRemoved);
                user.shares -= sharesRemoved; 
                
                uint256 amountToDistribute = (accumulated * RELATIONSHIP_REWARD) / 100;
                uint256 amountToTeamManagement = (accumulated * MANAGEMENT_REWARD) / 100;
                console.log("deposit amountToDistribute amountToTeamManagement",amountToDistribute,amountToTeamManagement);
                pool.want.safeTransfer(relationship, amountToDistribute);
                pool.want.safeTransfer(treasuryAddress, amountToTeamManagement);
                console.log("deposit _distributeRewards ",accumulated);
            ...
            console.log("deposit strategy.deposit ",_wantAmt,sharesAdded);
            user.shares += sharesAdded; 
            console.log("deposit sharesAdded user.shares ", sharesAdded, user.shares);
    ...
    function _withdraw(uint256 _pid, uint256 _wantAmt) private {
        ...
        console.log("_withdraw strategy.wantLockedTotal ", wantLockedTotal);
        console.log("_withdraw strategy.sharesTotal ", sharesTotal);
        ...
        console.log("_withdraw amount user.shares ", amount, user.shares);
        uint256 investment = IGymMLM(relationship).investment(msg.sender);
        console.log("_withdraw relation.investment ", investment);
        uint256 accumulated = amount - investment > 100 ? amount - investment : 0;

        console.log("_withdraw accumulated", accumulated);
        _wantAmt += accumulated > 100 ? accumulated * (RELATIONSHIP_REWARD + MANAGEMENT_REWARD) / 100 : 0;

        console.log("_withdraw _wantAmt", _wantAmt, RELATIONSHIP_REWARD,MANAGEMENT_REWARD);
        if (_wantAmt > amount) {
            _wantAmt = amount;
        }
        if (_wantAmt > 0) {
            ...
            user.shares -= sharesRemoved;
            console.log("_withdraw sharesRemoved user.shares", sharesRemoved, user.shares);
        
            if (accumulated > 0) {
                uint256 amountToDistribute = (accumulated * RELATIONSHIP_REWARD) / 100;
                uint256 amountToTeamManagement = (accumulated * MANAGEMENT_REWARD) / 100;
                console.log("_withdraw RELATIONSHIP_REWARD MANAGEMENT_REWARD", RELATIONSHIP_REWARD, MANAGEMENT_REWARD);
            ...
            console.log("_withdraw _wantAmt", _wantAmt);
        ...
        user.rewardDebt = (user.shares * (pool.accRewardPerShare)) / (1e18);
        console.log("_withdraw rewardDebt", user.rewardDebt);

        wantLockedTotal = IStrategy(poolInfo[_pid].strategy).wantLockedTotal();
        sharesTotal = IStrategy(poolInfo[_pid].strategy).sharesTotal();
        console.log("_withdraw wantLockedTotal, sharesTotal", wantLockedTotal, sharesTotal);
    ...
    function _transfer(
        ...
        //     If _token is WBNB
        //     IWETHGym(_token).withdraw(_amount);
