...
import "./interfaces/IArborswapRouter.sol";
...
contract FeeAggregator is IFeeAggregator, Ownable {
    ...
    event LogSetArborswapFee(uint256 fee);
    ...
    function calculateFee(uint256 amount) public override view returns (uint256 fee, uint256 amountLeft) {
        amountLeft = amount * (FEE_DENOMINATOR - arborswapFee) / FEE_DENOMINATOR;
     ...
     */
    function setArborswapFee(uint256 fee) external override onlyMultiSig {
        require(fee <= FEE_DENOMINATOR * 30 / 100, "FeeAggregator: FEE_MIN_0_MAX_30");
        arborswapFee = fee;

        emit LogSetArborswapFee(fee);
    }
...
interface IFeeAggregator {
    ...
    function setArborswapFee(uint256 fee) external;
}
