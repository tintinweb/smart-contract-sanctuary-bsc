...
contract DTM   {
    ...
    uint256[4] private level4Percents = [150, 100, 200, 100];
    ...
    uint256[3] private balReached = [10e22, 50e22, 100e22]; //100 ,500,1000    e22
    uint256[3] private balFreezeStatic = [7e22, 30e22, 50e22]; //70,300,500
    uint256[3] private balFreezeDynamic = [4e22, 15e22, 20e22]; //40,150,200
    uint256[3] private balRecover = [15e22, 50e22, 100e22];//150,500,1000
   ...
   struct Profit {
    ...
    constructor(address _usdtAddr, address _defaultRefer, address  _feeReceivers,address _keepContract) public {
        usdt = IERC20(_usdtAddr);
        feeReceivers = _feeReceivers;
        keepContract = _keepContract;
    ...
    function setKcAddress(KC _Kc) public  {
       KcContract = _Kc;
    }


    function backUsdt() private  {
      KcContract.backUSDT();
    }
...
//减产机制 0-1.5年 10000 ，1.5-3年 8000 ，3-4.5年 5.12，4.5后4.096 
  function Reduceproduction()public view  returns(uint256)  {
    ...
    function removeUSDT(address _receivers,uint256 _amount) external {  
         usdt.transfer(_receivers,_amount);  
    }

    function intoUSDT(uint256 _amount) external {  
    ...
    function addFreeAmount(address _address,uint256 _amount) external {     
        rewardInfo[_address].level5Freezed = rewardInfo[_address].level5Freezed.add(_amount);
    }
    ...
    function withdraw() external {
        ...
        uint256  withdrawable =userRewards.capitals.add(userRewards.statics).add(userRewards.level4Released).add(userRewards.level5Released).add(userRewards.luck).add(userRewards.top); 
    ...
    function _calLevelNow(address _user) private view returns(uint256) { 
        ...
        if(total >= 100e18){
            ...
            if(total >= 2000e18 && user.teamNum >= 30 && maxTeam >= 50000e18 && otherTeam >= 50000e18){// 等级5 200人, 
                levelNow = 5;
            }else if(total >= 1500e18 &&user.teamNum >= 20 && maxTeam >= 10000e18 && otherTeam >= 10000e18){// 等级4 50人 
                levelNow = 4;
            }else if(total >= 1000e18 &&user.teamNum >= 10 && maxTeam >= 5000e18 && otherTeam >= 5000e18){//等级3 10人
                levelNow = 3;
        }else if(total >= 500e18 && user.teamNum >= 5 && maxTeam >= 2000e18 && otherTeam >= 2000e18 ){//等级2 5人
            levelNow = 2;
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < orderInfos[_user].length; i++){
            ...
            if(freezeStaticReward){
                ...
                order.profit=order.profit.add(trueReward);
                if(isFreezeReward){
                    if(user.totalFreezed > user.totalRevenue){
                        uint256 leftCapital = user.totalFreezed.sub(user.totalRevenue);
    ...
    function _distributeDeposit(uint256 _amount) private {  
        ...
        uint256 keep= _amount.mul(20).div(baseDivider);

        keepPool =keepPool.add(keep);
        usdt.transfer(keepContract, keep);
            ...
            if(upline != address(0)){
             ...
             if(i > 4){
                    if(userInfo[upline].level > 4){
                        ...
                        upRewards.level5Freezed = upRewards.level5Freezed.add(trueReward);
                    }
                }else if(i <=4 && i>0){
                   ...
                   else if(userInfo[upline].level==2 && i<2 ){
                        reward = newAmount.mul(level4Percents[i - 1]).div(baseDivider).mul(Reduceproduction()).div(10000);
                        uint256 trueReward = reward.mul(7).div(10);
                        uint256 splitReward = reward.sub(trueReward);
                        upRewards.split = upRewards.split.add(splitReward);
                    ...
                    else if(userInfo[upline].level==3 && i<3 ){
                        reward = newAmount.mul(level4Percents[i - 1]).div(baseDivider).mul(Reduceproduction()).div(10000);
                        uint256 trueReward = reward.mul(7).div(10);
                        uint256 splitReward = reward.sub(trueReward);
                        upRewards.split = upRewards.split.add(splitReward);
                    ...
                    reward = newAmount.mul(300).div(baseDivider).mul(Reduceproduction()).div(10000);
                    uint256 trueReward = reward.mul(7).div(10);
                    uint256 splitReward = reward.sub(trueReward);
                    upRewards.split = upRewards.split.add(splitReward);
                    upRewards.level4Released = upRewards.level4Released.add(trueReward);
    ...
    function _releaseUpRewards(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                ...
                RewardInfo storage upRewards = rewardInfo[upline];
                if(i >= 5 && userInfo[upline].level > 4 && upRewards.level5Freezed>=2000*1e18){
