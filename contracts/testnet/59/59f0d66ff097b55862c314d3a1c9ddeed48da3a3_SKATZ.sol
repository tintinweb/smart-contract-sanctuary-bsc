...
contract SKATZDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("SKATZ_Dividend_Tracker", "SKATZ_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "SKATZ_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SKATZ_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main SKATZ contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "SKATZ_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "SKATZ_Dividend_Tracker: Cannot update claimWait to same value");
...
contract SKATZ is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    SKATZDividendTracker public dividendTracker;
   ...
   function setMarketingWallet(address payable _newMarketingWallet) public onlyOwner {
    ...
    constructor() ERC20("STRAYKATZ", "SKATZ") {
        BNBRewardsFee = 3;
        liquidityFee = 1;
        marketingFee = 3;
        charityFee = 3;
        ...
        charityWallet = payable(0x1C1bBafD23140b4B32b593a7439f63bAfC1124e8);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(charityFee); // total fee transfer and buy

     
    	dividendTracker = new SKATZDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "SKATZ: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "SKATZ: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "SKATZ: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "SKATZ: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "SKATZ: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "SKATZ: Cannot update gasForProcessing to same value");
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 charityAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee)).mul(charityFee);
