...
contract Dao7Topic is Ownable {
    ...
    function getDynamicOdds(address token,uint256 option) public view returns(uint256){
        ...
        (,,,,,uint256 tokenFee,uint256 rebate1,uint256 rebate2,,,) = mainContract.getToken(token);
        ...
        if(trueOptionAmount > 0){
            uint256 totalAmount = sumAmount - ((sumAmount - trueOptionAmount) / 10000 * tokenFee) - rebateRecord[token];
            trueOptionAmount = trueOptionAmount -  trueOptionAmount / 10000 * (rebate1 + rebate2);
    ...
    function openBet(uint256 _trueOption) public{
        ...
        for(uint256 i = 0; i < mainContract.tokensLength(); i++){
            (address token,,,,,uint256 tokenFee,uint256 rebate1,uint256 rebate2,,,) = mainContract.getTokenOfIndex(i);
            _openBetSet(token, _trueOption, tokenFee,rebate1,rebate2);
        }
    }
    ...
    function _openBetSet(address token,uint256 trueOption,uint256 tokenFee,uint256 rebate1,uint256 rebate2) private{
        ...
        if(trueOptionAmount > 0){
            ...
            uint256 totalAmount = sumAmount - feeRecord[token] - rebateRecord[token];
            trueOptionAmount = trueOptionAmount -  trueOptionAmount / 10000 * (rebate1 + rebate2);
            odds = totalAmount * 1e18 / trueOptionAmount;
        }
        if(sumAmount > 0 && trueOptionAmount == 0){
