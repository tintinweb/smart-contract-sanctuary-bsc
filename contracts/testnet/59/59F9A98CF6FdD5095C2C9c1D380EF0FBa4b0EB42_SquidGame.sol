...
contract SquidGameBasic {
    ...
    struct User {
        ...
        uint256 refID;
        mapping(uint8 => Checkment) CheckPayment;
...
contract SquidGame is SquidGameBasic {
    ...
    constructor(address _ownerAddress, address _secWallet, address _regWallet, IERC20 _depositTokenAddress) {
        ...
        levelPrice[9] = 80e15;
        levelPrice[10] = 110e15;
        levelPrice[11] = 160e15;
        levelPrice[12] = 220e15;
        levelPrice[13] = 320e15;
        levelPrice[14] = 440e15;
        levelPrice[15] = 650e15;
        levelPrice[16] = 800e15;

        id1 = _ownerAddress;
        LevelPayments[0].timeToOpen = block.timestamp;
        LevelPayments[1].timeToOpen = block.timestamp + 1*3 minutes;
        LevelPayments[2].timeToOpen = block.timestamp + 2*3 minutes;
        LevelPayments[3].timeToOpen = block.timestamp + 3*3 minutes;
        LevelPayments[4].timeToOpen = block.timestamp + 4*3 minutes;
        LevelPayments[5].timeToOpen = block.timestamp + 5*3 minutes;
        LevelPayments[6].timeToOpen = block.timestamp + 6*3 minutes;
        LevelPayments[7].timeToOpen = block.timestamp + 7*3 minutes;
        LevelPayments[8].timeToOpen = block.timestamp+ 8*3 minutes;
        LevelPayments[9].timeToOpen = block.timestamp + 9*3 minutes;
        LevelPayments[10].timeToOpen = block.timestamp + 10*3 minutes;
        LevelPayments[11].timeToOpen = block.timestamp + 11*3 minutes;
        LevelPayments[12].timeToOpen = block.timestamp + 12*3 minutes;
        LevelPayments[13].timeToOpen = block.timestamp + 13*3 minutes;
        LevelPayments[14].timeToOpen = block.timestamp + 14*3 minutes;
        LevelPayments[16].timeToOpen = block.timestamp;
    ...
    modifier levelIsAvailable(uint8 level) {
        require(block.timestamp >= LevelPayments[level-1].timeToOpen,"Level is temporarily unavailable");
        _;
    }
    ...
    function sendETHDividends(uint8 level) private returns (bool success) {
        ...
        if(users[prev_user].CheckPayment[level].secPayment) {

            if(((users[prev_user].activeMainLevels[level+1]) && (level != 16)) || (level==16)) {
            ...
            if(!users[prev_user].CheckPayment[level].firstPayment) {

                users[prev_user].CheckPayment[level].firstPayment = true;

            } else {

                users[prev_user].CheckPayment[level].secPayment = true;
            }
    ...
    // function payObgonCheck(address prev_user, uint8 level) private returns(bool success){
    //     uint256 mainPercent =levelPrice[level]*74/100;

    //     if((!users[prev_user].CheckPayment[level].secPayment) && ((users[prev_user].activeMainLevels[level+1])))
    //     {
    //         (success, ) = (prev_user).call{value:mainPercent}('');
    //     }else
    //     {
    ...
    function timeToOpenLevel(uint8 level) public view returns (uint256 time){
        return LevelPayments[level-1].timeToOpen;
    }
    ...
    function timeLeftToOpenLevel(uint8 level) public view returns (uint256 time){
        if(level==1 || level==16){
            ...
            if(LevelPayments[level-1].timeToOpen < block.timestamp){
                time = 0;
            }
            else{
                time = LevelPayments[level-1].timeToOpen - block.timestamp;
            }
        }
        return time;
    }
    ...
    function listOfLevelOpenTime() public view returns (uint256[] memory){
        ...
        levelTime[0]=levelTime[15]=0;

        for (uint8 i=1; i < LAST_LEVEL-1; i++)
        {
            if(LevelPayments[i].timeToOpen < block.timestamp){
                levelTime[i] = 0;
            }else{
                levelTime[i] = LevelPayments[i].timeToOpen - block.timestamp;
            }
        }
        return levelTime;
    }
