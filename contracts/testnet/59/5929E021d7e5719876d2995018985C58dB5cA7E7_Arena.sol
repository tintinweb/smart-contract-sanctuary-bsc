...
contract Arena is Ownable{
    ...
    uint256 public arenaNumb = 0; // 竞技的期数
    mapping(uint256=>uint256) _tokenIdType; // tokenId对应的属性
    mapping(uint256=>arenaInfo) _arenaInfo; // 每期竞技场设置参数
    mapping(uint256=>tokenArena[]) _tokenSortArenas;  // 每期前15名
    arenaSet _arenaSet = arenaSet(7,1000*10**18,15); // 竞技场设置
    ...
    mapping(uint256=>RinKInfo) public rinkInfo;
    ...
    function athletics(uint256 tokenId)  public isUser(tokenId) returns(uint256){
        // cards = _game.getTokenDetails(tokenId);
        // _badhero.getNftKind(_game.getTokenDetails(tokenId).nftKindId);
        // //   nftKind memory _nftKind =  _nftKinds[_tokenIdType[tokenId]];
        // nftKind memory _nftKind =  _nftKinds[_tokenIdType[tokenId]];
        // 来3次随机数来求值
        ...
        emit JoinArena(arenaNumb, wins, msg.sender);
        return wins;
    }

    //  排名统计
    function sort(uint256 tokenId,uint256 wins,address sender)  internal returns(bool){
        uint256 lenth = _tokenSortArenas[arenaNumb].length;
        tokenArena[] memory arenas ;
        if (wins<=_tokenSortArenas[arenaNumb][lenth-1].wins){
            return false;
        }

        if (wins>_tokenSortArenas[arenaNumb][0].wins){
            arenas[0]= tokenArena(tokenId,wins,sender);
            for (uint256 i=1;i<lenth;i++){
                // _tokenSortArenas[arenaNumb][i] = _tokenSortArenas[arenaNumb][i-1];
                arenas[i] = _tokenSortArenas[arenaNumb][i-1];
            }
            // delete _tokenSortArenas[arenaNumb];
            // _tokenSortArenas[arenaNumb]= arenas;
            delete _tokenSortArenas[arenaNumb];
            for(uint256 i=0;i<arenas.length;i++){
                _tokenSortArenas[arenaNumb].push(arenas[i]);
            }
            return true;
        }
    ...
    function doWeeklyTasks() public{
        require(isJoinWkTask(weekRound,msg.sender) == true,"Have attended");
    ...
    function getSortArenas() view public returns(tokenArena[] memory){
        return _tokenSortArenas[arenaNumb];
    }
    //  发放竞技奖励
    function disReward()  public onlyOwner{
       tokenArena[] memory arena =  _tokenSortArenas[arenaNumb];
       uint256 maxNum = 15;
       if (arena.length>=maxNum){
           maxNum = arena.length;
       }
       for (uint256 i = 0; i < maxNum; i++) {
           for (uint256 j = 0; j < _arRewardSet.length; j++) {
               if(i>=_arRewardSet[j].start && i<_arRewardSet[j].end){
                uint256 reward = arenaPool * _arRewardSet[j].rewards ;
                _game.DisReward(arena[i].sender,arena[i].tokenId,reward);
               }
           }
       }
    }
    ...
    function disCompetitiveReward(uint256 _arenaNumb,uint256 ranking,address sender,uint256 tokenId,uint256 reward)public onlyOwner{
        require(rinkInfo[_arenaNumb].rink !=ranking,"The rewards have already been handed out");
        _game.DisReward(sender,tokenId,reward);
        rinkInfo[_arenaNumb].rink = ranking;
        rinkInfo[_arenaNumb].win = sender;
        rinkInfo[_arenaNumb].reward = reward;
        rinkInfo[_arenaNumb].tokenId = tokenId;
...
contract Game is AccessControl,Ownable {
    ...
    constructor(address _priceAddress) {
        getFeeRate = GetFee(_priceAddress);
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        pushTask();
        }
    ...
    function createCard(uint256 tokenId,uint256 ce,uint256 armor,uint256 luk,uint256 unLockTime,uint256 nftKindId,string memory name,uint256 maxNum,address _userAddress) public onlyRole(MINTER_ROLE) returns(uint256){
        ...
        if(_userTem[_userAddress].length<maxNum){
            _userTem[_userAddress].push(_carDetails);
        }else{
            _userBackpack[_userAddress].push(_carDetails);
        }
    ...
    function fighting(uint256 tokenId,uint256 enemyId) public isUnlock(tokenId) isteam(tokenId,msg.sender) returns(FightingEndInfo memory fig){
        ...
        if(fig.suc == true){
            ...
            // uint256 level = _tokenDetail[tokenId].level;
            ...
            // crdDt.hp = fig.hp;
            // crdDt.xp = fig.xp;
            // crdDt.rgTime = fig.unLkTime;
        }else{
            // crdDt.hp = 0;
            // crdDt.rgTime = block.timestamp + 86400;
            _tokenDetail[tokenId].hp = 0;
            _tokenDetail[tokenId].rgTime = block.timestamp + 86400;
        }
        fig.fgType = 1;
        emit Fighting(fig.suc,1,fig.hp,fig.xp,fig.reward);
    ...
    function DoTask(uint256 tokenId,uint256 enemyId) public isUnlock(tokenId) isteam(tokenId,msg.sender) returns(FightingEndInfo memory fig) {
        ...
        if(fig.suc == true){
            ...
            _userRewardPools[msg.sender].push(_rewardPool);
            _tokenDetail[tokenId].hp = fig.hp;
            _tokenDetail[tokenId].unLockTime = fig.unLkTime;
        }else{
            _tokenDetail[tokenId].hp = 0;
            _tokenDetail[tokenId].unLockTime = block.timestamp + 86400;
        }
        fig.fgType = 2;
        emit Fighting(fig.suc,2,fig.hp,0,fig.reward);
        return fig;
    }
...
modifier isteam(uint256 tokenId,address sender){
       CardDetails[] memory cards = _userTem[sender];
        ...
        require(_isTeam,"nft is no team");
        _;
    }

    function isBack(uint256 tokenId,address sender) public view returns(bool) {
       CardDetails[] memory cards = _userBackpack[sender];
        bool _isBack = false;
        for(uint32 i=0;i<cards.length;i++){
            if(cards[i].tokenId == tokenId){
                _isBack =true;
    ...
    function getUserTesmCards(address sender) view public returns(CardDetails[] memory){
        return _userTem[sender];
    }

    function getUserBkCards(address sender) view public returns(CardDetails[] memory){
...
contract BADHero is Ownable{
    ...
    uint256 _upEqCost = 2000*10**18;      // 升级装备费用
 ...
 // 设置怪物NFT属性
    function _setNftKind() internal  {
        _nftKinds[0] = nftKind(55,100,70,"N","Normal","");
        _nftKinds[1] = nftKind(23,55,75,"R","Rare","");
        _nftKinds[2] = nftKind(8,23,80,"SR","Super Rare","");
        _nftKinds[3] = nftKind(2,8,85,"SSR","Super Super Rare","");
        _nftKinds[4] = nftKind(0,2,90,"UR","Ultra Rare","");
...
contract GetFee is Ownable {
    ...
    address public wbnbAddress = 0x7ef95a0FEE0Dd31b22626fA2e10Ee6A223F8a684;
    IPancakeSwapRouter public router;
    address[] _path = [usdtAddress,tokenAddress];
    address[] _wpath = [wbnbAddress,tokenAddress];
    ...
    function getBNBPrice1(uint amountIn) public view  returns(uint){
        uint[] memory amounts = getRateByAddress(amountIn,_wpath);
        return amounts[1];
    }
...
contract Monster is Ownable  {
    ...
    function fighting(uint256 tkId,uint256 enemyId,address addr) public  view isFullHp(tkId) returns(bool,uint256,uint256,uint256,uint256){
        ...
        (_carDetail.level,_carDetail.ce,_carDetail.xp,_carDetail.armor,_carDetail.luk,) = _game.getTokenDetail(tkId);
        
        // 判断是否胜利
        if (rand(100)>=_figInfo.totalSuc){
            // 修改血量 basicHp
            _carDetail.hp = 0;
            _carDetail.unLockTime = block.timestamp + 86400;
            _figInfo.sHp= basicHp;
        }else{
            _figInfo.sHp = basicHp - _combatOdds.addHp;
            ...
            _addXpInfo.totalXp = _carDetail.xp + _enemy.basicXp + _combatOdds.addXp;
        ...
        return (_figInfo.isSuccess,_figInfo.reward,_combatOdds.addHp,_carDetail.xp,_carDetail.unLockTime);
    ...
    function DoTask(uint256 tokenId,uint256 odds,uint256 basicReward,address addr ) public view isFullHp(tokenId)  returns(bool,uint256,uint256,uint256){
        ...
        return (_figInfo.isSuccess,_figInfo.reward,_carDetail.hp,_carDetail.unLockTime);
    }
    //获取怪兽信息
    ...
    function initEney() internal{
        if(enemys.length ==0 ){
            addEnemy(70,2*10*16,20,200*10*8,"Gabriel","");
            addEnemy(65,2*10*16,20,200*10*8,"Horace","");
            addEnemy(60,2*10*16,20,200*10*8,"Rufio","");
            addEnemy(55,2*10*16,20,200*10*8,"Hadrea","");
            addEnemy(50,2*10*16,20,200*10*8,"Sirius","");
        }
    }
    ...
    function rgHp(uint256 useTime) view public returns(uint256 hp){
        uint256 rate = basicHp/86400;
        return basicHp - rate*useTime;
    }
    function setGame(address _token) public onlyOwner{
        _game = Game(_token);
    }
