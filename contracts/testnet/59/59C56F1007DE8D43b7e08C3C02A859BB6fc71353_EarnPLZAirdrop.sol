...
contract EarnPLZAirdrop is Ownable,Pausable {
    ...
    constructor(address _signerAddress,address _token, uint256 _startTime,uint256 _endTime,uint256 _airDropQuantity){
    ...
    function claim(string memory hash,uint256 deadline,bytes memory signature) public whenNotPaused {
        ...
        require(!userInfo.contains(user), "user already claimed");
        ...
        token.safeTransfer(user,airDropQuantity);
        userInfo.insert(user);
        
        emit claimEvent(user,airDropQuantity,block.timestamp);
    }
