...
contract Protector is Ownable {
    ...
    bool public sellingBlocked = true;

    uint256 public howLongCantSendToNextWallet = 5*86400;
    uint256 public howLongCantSendToNextWalletAfterReceived = 5*86400;
    uint256 public howLongCantSellAfterReceived = 5*86400;
    uint256 public howLongCantSellAfterSendToWallet = 5*86400;
    ...
    struct user {
        ...
        uint256 lastWalletSendToTime;
        uint256 lastWalletReceivedTime;
    }
    ...
    uint256 public pancakeswapFee = 250;
    ...
    function processSell(address from, address to, uint256 amount) external onlyMetagold returns (bool allowed) {
        ...
        if( to == metaGold.tokenPair()) {  //selling tokens through PancakeSWAP
            ...
            if( block.timestamp < tradeData[from].lastWalletSendToTime + howLongCantSellAfterSendToWallet) {
                return false;
            }

            if( block.timestamp < tradeData[from].lastWalletReceivedTime + howLongCantSellAfterReceived) {
                return false;
            }
           ...
           (, uint256 a2) = getADMLimits(from, limit1, limit2);
            ...
            if( block.timestamp < tradeData[from].lastWalletSendToTime + howLongCantSendToNextWallet) {
                return false;
            }

            if( block.timestamp < tradeData[to].lastWalletReceivedTime + howLongCantSendToNextWalletAfterReceived) {
                return false;
            }

            tradeData[from].lastWalletSendToTime = block.timestamp;
            tradeData[to].lastWalletReceivedTime = block.timestamp;
    ...
    function getTradeData(address whoWantsToSell) external onlyMetagold view returns(uint256 startBalance,
        ...
        ,tradeData[whoWantsToSell].lastWalletSendToTime,tradeData[whoWantsToSell].lastWalletReceivedTime);
    }

    //onlyMetagold
    function getTradeData2() external view returns(uint256 _howLongCantSendToNextWallet,
        ...
        return (howLongCantSendToNextWallet,howLongCantSendToNextWalletAfterReceived,howLongCantSellAfterReceived,howLongCantSellAfterSendToWallet);
    }
    function setTradeData2(uint256 _howLongCantSendToNextWallet,
        uint256 _howLongCantSendToNextWalletAfterReceived,
        uint256 _howLongCantSellAfterReceived,
        uint256 _howLongCantSellAfterSendToWallet
    ) external onlyOwner {
        howLongCantSendToNextWallet = _howLongCantSendToNextWallet;
        howLongCantSendToNextWalletAfterReceived = _howLongCantSendToNextWalletAfterReceived;
        howLongCantSellAfterReceived = _howLongCantSellAfterReceived;
        howLongCantSellAfterSendToWallet = _howLongCantSellAfterSendToWallet;
    }
    ...
    function getTokensAllowedToSellUSDLimit(bool mcLimit, address whoWantsToSell) public view returns(uint256 tokensAllowedToSellUSDLimit) {
        ...
        return remainingUSD.mul(1*10**18).div(currentPrice);
    }
    ...
    function setSellingBlocked(bool value) external onlyOwner {
        sellingBlocked = value;
    }
    ...
    function setFixForPriceUSD(uint256 value) external onlyOwner { 
        FixForPriceUSD = value;
    }
    function setPromileDailyLimitADM(uint256 value) external onlyOwner {
        PromileDailyLimitADM = value;
    }
    function setUSDdailyLimitNOADM(uint256 value) external onlyOwner {
        USDdailyLimitNOADM = value;
    }
    function setUSDdailyLimitADM(uint256 value) external onlyOwner {
        USDdailyLimitADM = value;
    }
    function setPancakeswapFee(uint256 value) external onlyOwner {
        pancakeswapFee = value;
    }
