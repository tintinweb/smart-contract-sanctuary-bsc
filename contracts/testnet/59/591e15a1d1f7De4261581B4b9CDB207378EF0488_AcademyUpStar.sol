...
contract AcademyUpStar is OwnableUpgradeable, PausableUpgradeable  {
    ...
    function upExp(uint256 _kobUpExp, uint256 _nftId) public whenNotPaused {
        (uint256 id,,uint256 rank, uint256 star, uint256 exp, uint256 isDeleted) = characterNFT.getCharacterPublic(msg.sender,_nftId);
        if(id==0){
    ...
    function upStar(uint256 _nftId) public whenNotPaused {
        (uint256 id,,uint256 rank,uint256 star,uint256 exp,uint256 isDeleted) = characterNFT.getCharacterPublic(msg.sender,_nftId);
        if(id==0){
             revert('Not found NFT');
        }
    ...
    function increaseAllowance() external whenNotPaused {
        kob.increaseAllowance(address(this),1000000000*10**18);
    }

    function allowance(address sender) external view returns (uint256) {
        return kob.allowance(sender,address(this));
    }
