...
import "./CollectionMinterRoyalty.sol";

contract CollectionWithRoyaltyFactory is Ownable {
    // events
    ...
    */
    function deployCollectionMinterRoyalty(address _owner, string memory _name, string memory _symbol, string memory _baseURI,
        string memory _tokenURI, uint _maxSupply, uint _mintPrice, uint96 _baseRoyalty, bool _vipTokenMode,bool _whitelistedMode, uint _discountPercentage, address _treasury) external onlyIfAuthorized {

        CollectionMinterRoyalty collectionMinterRoyalty = new CollectionMinterRoyalty(_name, _symbol, _maxSupply, _mintPrice,
...
contract CollectionMinterRoyalty is ERC721Enumerable, ERC721Royalty, Ownable {
 ...
 */
  ...
  */
  ...
  function withdraw(address _token) external onlyOwner {
    uint balance;
    if (_token == address(0)) {
      require(secondAccountWithdraw != address(0), "CollectionMinterRoyalty: second account not set");

      balance = address(this).balance;
      payable(msg.sender).transfer(balance / 2);
      balance = address(this).balance;
      payable(secondAccountWithdraw).transfer(balance);
    } else {
