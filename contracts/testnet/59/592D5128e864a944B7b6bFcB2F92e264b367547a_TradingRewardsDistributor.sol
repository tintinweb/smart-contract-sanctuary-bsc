...
contract TradingRewardsDistributor is IShares, ITradingRewardsDistributor, Pausable, ReentrancyGuard {
  using SafeERC20 for IMetaSpacecyToken;
  ...
  modifier onlyAdmin() {
    require(metaspacecyAccessControls.hasAdminRole(_msgSender()), "TRD: only admin");
    _;
  }

  constructor(address _metaspacecyToken, address _metaspacecyAccessControls) {
  ...
  function maxBalanceMST() public view override returns (uint256) {
    return (metaspacecyToken.maxTotalSupply() * TRADING_REWARD_SHARES) / TOTAL_SHARES;
  }
  ...
  function claim(uint256 amount, bytes32[] memory merkleProof) external override whenNotPaused nonReentrant {
    ...
    metaspacecyToken.safeTransfer(_msgSender(), adjustedAmount);
  ...
  function withdrawTokenRewards(uint256 amount) external onlyAdmin whenPaused {
    ...
    metaspacecyToken.safeTransfer(_msgSender(), amount);
