...
contract TownToken is BEP20Burnable, Ownable{
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(!inTransfer){
            uint256 currentRate =  _getRate();
            uint256 tFee = amount.mul(_TAX_FEE).div(100);
            uint256 tLiquid = amount.mul(_LIQUIDITY_FEE).div(100);
            uint256 tBurn = amount.mul(_BURN_FEE).div(100);
            ...
            if (_isExcluded[sender] || _isExcluded[recipient]) {
               ...
               super._transfer(sender, recipient, amount);
                ...
                if(tBurn > 0){
                    super._transfer(sender, burnAddress, tBurn);
                }
