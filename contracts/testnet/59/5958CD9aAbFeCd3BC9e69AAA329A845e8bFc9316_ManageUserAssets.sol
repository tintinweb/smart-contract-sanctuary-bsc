...
interface IManageUserAssets {
    ...
    function withdrawACD(address to, uint256 amountWithdraw) external;

    function withdrawSND(address to, uint256 amountWithdraw) external;

    function mintACD(address to, uint256 amount) external;
    function burnACD(address owner, uint256 amount) external;

    
    function rewardSND(address to, uint256 amount) external;
    function burnSND(address owner, uint256 amount) external;

    function burnRobot(address owner, uint256 _tokenId) external;
    function burnLand(address owner, uint256 _tokenId) external;
}
...
contract ManageUserAssets is AccessControl, IManageUserAssets, ReentrancyGuard {
    ...
    event WithdrawACD(address indexed owner, uint256 withdrawAmount);
    event WithdrawSND(address indexed owner, uint256 withdrawAmount);
    // variables
    ISNLand landNFT;
    IRobot robotNFT;
    IACDToken ACDToken;
    ...
    function withdrawLand(address ownerLand, uint256 _tokenId) external override onlyOperator {
        ...
        Assets storage pool = pools[ownerLand];
    ...
    function withdrawRobot(address ownerRobot, uint256 _tokenId) external override onlyOperator {
    ...
    function withdrawACD(address to, uint256 amountWithdraw) external override onlyOperator {
        require(to != address(0), "ManageUserAssets: owner is zero");
        Assets storage pool = pools[to];
        require(pool.state == 1, "ManageUserAssets: pool is not exist");
        require(pool.ACD >= amountWithdraw, "ManageUserAssets: ACD not enough");

        pool.ACD = pool.ACD - amountWithdraw;
        ACDToken.transfer(to, amountWithdraw);

        emit WithdrawACD(to, amountWithdraw);
    }

    function withdrawSND(address to, uint256 amountWithdraw) external override onlyOperator {
        require(to != address(0), "ManageUserAssets: owner is zero");
        Assets storage pool = pools[to];
        require(pool.state == 1, "ManageUserAssets: pool is not exist");
        require(pool.SND >= amountWithdraw, "ManageUserAssets: ACD not enough");

        pool.SND = pool.SND - amountWithdraw;
        ACDToken.transfer(to, amountWithdraw);

        emit WithdrawSND(to, amountWithdraw);
    }

    function mintACD(address to, uint256 amount) external override onlyOperator {
        require(to != address(0), "ManageUserAssets: owner is zero");
        require(pools[to].state == 1, "ManageUserAssets: pool is not exist");

        Assets storage pool = pools[to];
        pool.ACD = pool.ACD + amount;
        ACDToken.mintToken(address(this), amount);
    }

    function burnACD(address owner, uint256 amount) external override onlyOperator {
        require(owner != address(0), "ManageUserAssets: owner is zero");
        require(pools[owner].state == 1, "ManageUserAssets: pool is not exist");
        require(pools[owner].ACD >= amount, "ManageUserAssets: pool is not exist");

        Assets storage pool = pools[owner];
        pool.ACD = pool.ACD - amount;

        ACDToken.burn(amount);
    }

    function rewardSND(address to, uint256 amount) external override onlyOperator {
        require(to != address(0), "ManageUserAssets: owner is zero");
        require(pools[to].state == 1, "ManageUserAssets: pool is not exist");

        Assets storage pool = pools[to];
        pool.SND = pool.SND + amount;

        SNDToken.transferFrom(gameReward, address(this), amount);
    }

    function burnSND(address owner, uint256 amount) external override onlyOperator {
        require(owner != address(0), "ManageUserAssets: owner is zero");
        require(pools[owner].state == 1, "ManageUserAssets: pool is not exist");
        require(pools[owner].SND >= amount, "ManageUserAssets: pool is not exist");

        Assets storage pool = pools[owner];
        pool.SND = pool.SND - amount;

        SNDToken.transfer(gameReward, amount);
    }

    function burnRobot(address owner, uint256 _tokenId) external override onlyOperator {
        require(owner != address(0), "ManageUserAssets: owner is zero");
        Assets storage pool = pools[owner];
        require(pool.state == 1, "ManageUserAssets: pool is not exist");
        ...
        require(robotInPool == true, "ManageUserAssets: robot is not exist");
        robotNFT.burn(_tokenId);
        delete pool.robots[uint256(indexRobotInPool)];

    }

    function burnLand(address owner, uint256 _tokenId) external override onlyOperator {
        require(owner != address(0), "ManageUserAssets: owner is zero");
        Assets storage pool = pools[owner];
        require(pool.state == 1, "ManageUserAssets: pool is not exist");
