...
contract SgBridge is Initializable, UUPSUpgradeable, OwnableUpgradeable, IStargateReceiver, WardedLiving {
    ...
    function setSwapper(address swapperLib_, address dex_) external auth {
        swapper = Swapper(swapperLib_);
        dex = dex_;
    }
    ...
    function isTokenSupported(address token) public view returns (bool) {
        return swapper.isTokenSupported(factory, defaultBridgeToken, token);
    }

    function isTokensSupported(address[] calldata tokens) public view returns (bool[] memory) {
        return swapper.isTokensSupported(factory, defaultBridgeToken, tokens);
    }

    function isPairsSupported(address[][] calldata tokens) public view returns (bool[] memory) {
        return swapper.isPairsSupported(factory, tokens);
    }
    ...
    function bridge(address token,
        ...
        if (currentChainId == destChainId) {
            swap(token, destinationToken, amount, msg.sender);
            return;
        }
        ...
        if (srcPoolId == 0) {//There are no stargate pool for this token => swap on DEX
            ...
            (bool swapSuccess, uint256 quoteAmount) = swap(token, defaultBridgeToken, amount, address(this));
            require(swapSuccess, "SgBridge/swap-failed");
    ...
    function sgReceive(
        ...
        if (_tokenOut != _token) {
            (bool swapSuccess, uint256 amount) = swap(_token, _tokenOut, amountLD, _toAddr);
            if (swapSuccess) {
    ...
    function swap(
        ...
        (bool success, bytes memory data) = address(swapper).delegatecall(
            abi.encodeWithSelector(swapper.swap.selector,
            dex, tokenA, tokenB, amountA, recipient)
        );
        if (success) {
    ...
    function quote(address tokenA, address tokenB, uint256 amountA) external returns (uint256) {
        revert("SgBridge/Unsupported-chain");
    }
}
...
interface Swapper {
    function swap(address amm, address tokenA, address tokenB, uint256 amount, address recipient) external payable returns (uint256);
    function quote(address amm, address tokenA, address tokenB, uint256 amount) external returns (uint256);
    function isTokenSupported(address factory, address bridgeToken, address token) external view returns(bool);
    function isTokensSupported(address factory, address bridgeToken, address[] memory tokens) external view returns(bool[] memory);
    function isPairsSupported(address factory, address[][] calldata tokens) external view returns(bool[] memory);
}
