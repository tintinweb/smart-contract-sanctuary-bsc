    ...
    ERC721
{
    ...
    {
        require(value <= 30 days, "The lockup period for tranches is too long");
        ...
        returns (uint256 amount)
    {
        ...
        require(
            block.timestamp > t.creationTimestamp + lockupPeriodForTranches,
    ...
    function sellOption(
        ...
        require(
            strike % strikePricePrecision == 0,
            "OptionPool: strike price precision not correct"
        );
        ...
        require(period <= 30 days, "Pool Error: The period is too long");
        require(
            _checkUTCSpecificTime(expiry),
            "Pool Error: Only option that expires at 8am UTC is allowed"
        );
        require(
            (lockedAmount + amountToBeLocked) * 100 <=
                totalBalance * maxUtilizationRate,
            "Pool Error: The amount is too large, pool doesn't have enough liquidity"
        );
    ...
    {
        require(
            address(optionErc20s[expiry][strikePrice]) == address(0),
            "Option exist for this expiry and strikeprice"
        );
        ...
        nonReentrant
    {
        ...
        require(detail.expiry != 0, "Pool Error: Invalid OptionErc20");
        uint256 balanceOf = optionErc20.balanceOf(holder);
        require(balanceOf > 0, "Pool Error: User does not have balance");
