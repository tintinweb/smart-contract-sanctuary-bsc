...
contract JuneLogic is OwnableUpgradeable, ERC20Upgradeable, Manager {
    ...
    //marketing wallet
    address private MarketingWalletAddr;
    uint256 private WithdrawBurnFee;//%
    uint256 private DayMintAmount;//days mint
    uint256 private DayBurnAmount;//days burn
    address private WithdrawalAddr;
    address private MintAddr;
    bool private PairPaused = false;//is pair paused
    uint256 private sellMarketingRates;//sell:amount + (sellMarketingRates / 100 + amount)
    uint256 private buyMarketingRates;//buy:amount - (buyMarketingRates / 100 + amount)
    address private LiquidityAddr;//Liquidity wallet address
    uint256 private DividendMin;
    EnumerableMapUpgradeable.AddressToUintMap private DividendAdds;
    //    address []  private DividendAdds;
    //    mapping(address => uint256) DividendAddsIndex;
    address  private SlippageAddr;
    ...
    function Breed(address token, address _to, uint256 amount) public onlyDraw {
    ...
    function setDividendAdds(address value, bool _is) public onlyManager {
        if (_is) {
            require(!EnumerableMapUpgradeable.contains(DividendAdds, value), 'existed');
            EnumerableMapUpgradeable.set(DividendAdds, value, 0);
        } else {
            require(EnumerableMapUpgradeable.contains(DividendAdds, value), 'existed');
            EnumerableMapUpgradeable.remove(DividendAdds, value);
        }
    }

    function getDividendAdds(address value) public view virtual returns (bool) {
        return EnumerableMapUpgradeable.contains(DividendAdds, value);
    }
    ...
    function Dividend() public onlyManager {
        ...
        uint256 DividendNum = balanceOf(SlippageAddr).mul(uint256(10) ** 18).div(LpTotal);
        for (uint i = 0; i < EnumerableMapUpgradeable.length(DividendAdds); i++) {
            (address _addr,uint256 _addrAm) = EnumerableMapUpgradeable.at(DividendAdds, i);
            uint256 userBalance = LiquidityWallet.balanceOf(_addr);
            uint256 userDividend = DividendNum.mul(userBalance);
            if (userDividend > 0 && userBalance >= DividendMin && _addr != address(0)) {
                super._transfer(SlippageAddr, _addr, userDividend.div(uint256(10) ** 18));
            }
        }
    }
