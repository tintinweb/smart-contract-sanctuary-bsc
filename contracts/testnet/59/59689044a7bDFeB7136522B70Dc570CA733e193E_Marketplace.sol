...
contract Marketplace is ReentrancyGuardUpgradeable {
    ...
    event ItemSold(address indexed buyer, address indexed nftAddress, uint256 indexed tokenId, uint256 pricew);

    modifier onlyItemOwner(address nftAddress, uint256 tokenId) {
        IERC721MetadataUpgradeable tokenContract = IERC721MetadataUpgradeable(nftAddress);
     ...
     * @param tokenId ERC721 token id for NFT
     * @param nftAddress ERC721 contract address for NFT
     * @param price required price for the item
     */
        ...
        notListed(nftAddress, tokenId, msg.sender)
    {
        ...
        itemsForSale[nftAddress][tokenId] = SaleItem(payable(msg.sender), price);
        emit ItemAdded(msg.sender, nftAddress, tokenId, price);
    }
     ...
     */
    function removeItemFromMarket(address nftAddress, uint256 tokenId)
        external
        nonReentrant
        onlyItemOwner(nftAddress, tokenId)
    ...
    function updatePrice(
        ...
        uint256 tokenId,
        uint256 newPrice
    ) external nonReentrant onlyItemOwner(nftAddress, tokenId) isListed(nftAddress, tokenId) {
        require(newPrice > 0, "Price must be at least 1 wei");

        SaleItem storage item = itemsForSale[nftAddress][tokenId];

        require(newPrice != item.price, "Price must be different from current price");
        ...
        hasTransferApproval(nftAddress, tokenId)
    {
        ...
        IERC721MetadataUpgradeable(nftAddress).safeTransferFrom(item.seller, msg.sender, tokenId);
