...
abstract contract VRFConsumerBase is VRFRequestIDBase {
    ...
    function setNewLink(address newLink) internal {
        LINK = LinkTokenInterface(newLink);
    }

    function _setVrfCoordinator(address newVrfCoordinator) internal {
...
contract RandomNumberGenerator is VRFConsumerBase, IRandomNumberGenerator, Ownable {
    ...
    function setLinkToken(address newLink) external onlyOwner {
        setNewLink(newLink);
    }

    function setVrfCoordinator(address newVrfCoordinator) external onlyOwner {
        _setVrfCoordinator(newVrfCoordinator);
    }

    /**
     * @notice Request randomness from a user-provided seed
     * @param _seed: seed provided by the PBEXLottery lottery
     */
    function getRandomNumber(uint256 _seed) external override {
        require(msg.sender == PBEXLottery, "Only PBEXLottery");
    ...
    /**
     * @notice Set the address for the PBEXLottery
     * @param _PBEXLottery: address of the PBEXLottery lottery
     */
    function setLotteryAddress(address _PBEXLottery) external onlyOwner {
        PBEXLottery = _PBEXLottery;
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        latestLotteryId = IPBEXLottery(PBEXLottery).viewCurrentLotteryId();
    }
}