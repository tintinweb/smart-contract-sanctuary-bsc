...
// File contracts/game/sigGuardian/SigValidator.sol
...
contract SigValidator {
  ...
  function _validateSignatureTokenData(
    bytes32 hashDigest,
    ...
    uint256 claimID = _getTokenClaimID(
  ...
  function _validateSignatureGenericData(
    bytes32 hashDigest,
    ClaimGenericData calldata claimData,
    SignatureData calldata signatureData
  ) internal view {
    require(_verifyNetworkDescriptor(claimData.networkDescriptor), "INV_NETWORK");
    require(playerNonces[claimData.playerAddress] == claimData.playerNonce - 1, "INV_NONCE");
    ...
    uint256 claimID = _getGenericClaimID(
      claimData.guardianAddress,
      claimData.playerAddress,
      claimData.index,
      claimData.value,
  ...
  function _getGenericClaimID(
    ...
    return
      uint256(
        keccak256(
          abi.encode(guardianAddress, playerAddress, index, value, networkDescriptor, _playerNonce)
        )
      );
  }
...
// File contracts/game/sigGuardian/SigGuardian.sol
...
contract SigGuardian is SigValidator {
   ...
   */
  function validateTokenReq(
    bytes32 hashDigest,
    ClaimTokenData calldata claimData,
    SignatureData calldata signatureData
  ) public view {
    _validateSignatureTokenData(hashDigest, claimData, signatureData);
  }

  function validateGenericReq(
    bytes32 hashDigest,
    ClaimGenericData calldata claimData,
    SignatureData calldata signatureData
  ) public view {
    _validateSignatureGenericData(hashDigest, claimData, signatureData);
  }
}

// File contracts/game/playerStash/interfaces/IERCDec.sol
...
// File contracts/game/playerStash/PlayerStash.sol
...
contract PlayerStash is Initializable, PausableUpgradeable, AccessControlUpgradeable, SigGuardian {
  ...
  function depositToken(
    ...
    validateTokenReq(hashDigest, claimData, signatureData);

    _depositToken(claimData.tokenAddress, claimData.value, claimData.playerAddress);
  }
  ...
  function _depositToken(
    ...
    require(tokenType != TokenType.NIL, "INV_TOKEN");
    ...
    if (tokenType == TokenType.ERC20) {
      ...
      require(
        trustedToken.allowance(playerAddress, address(this)) >= value,
        "NOT_ENOUGH_ALLOWANCE"
      );

      uint256 playerBalanceBefore = playerToTokenToBalance[playerAddress][tokenAddress];
      playerToTokenToBalance[playerAddress][tokenAddress] = playerBalanceBefore + value;
      tokenToBalance[tokenAddress] = tokenToBalance[tokenAddress] + value;
    } else {
      address tokenOwner = trustedToken.ownerOf(value);

      require(playerAddress == tokenOwner, "INV_OWNER");
      require(
        trustedToken.getApproved(value) == address(this) ||
          trustedToken.isApprovedForAll(tokenOwner, address(this)),
        "NOT_ENOUGH_ALLOWANCE"
      );

      tokenToIdToOwner[tokenAddress][value] = playerAddress;
    ...
    emit DepositToken(playerAddress, tokenAddress, value, playerNonces[playerAddress]);

    trustedToken.transferFrom(playerAddress, address(this), value);
  }
  ...
  function withdrawToken(
    ...
    _withdrawToken(claimData.tokenAddress, claimData.value, claimData.playerAddress);
  }
  ...
  function _withdrawToken(
    ...
    if (tokenType == TokenType.ERC20) {
      ...
      playerToTokenToBalance[playerAddress][tokenAddress] = balance - value;
      tokenToBalance[tokenAddress] = tokenToBalance[tokenAddress] - value;
    } else {
      address tokenOwner = tokenToIdToOwner[tokenAddress][value];
      require(tokenOwner == playerAddress, "INV_OWNER");

      tokenToIdToOwner[tokenAddress][value] = address(0);
      tokenToBalance[tokenAddress] = tokenToBalance[tokenAddress] - 1;
    }

    _incrementPlayerNonce(playerAddress);

    emit WithdrawToken(playerAddress, tokenAddress, value, playerNonces[playerAddress]);

    if (tokenType == TokenType.ERC721) {
      trustedToken.transferFrom(address(this), playerAddress, value);
    } else {
      trustedToken.transfer(playerAddress, value);
    }
  }
  ...
  function depositGeneric(
    ...
    validateGenericReq(hashDigest, claimData, signatureData);

    _depositGeneric(claimData.index, claimData.value, claimData.playerAddress);
  }
  ...
  function withdrawGeneric(
    ...
    validateGenericReq(hashDigest, claimData, signatureData);
    _withdrawGeneric(claimData.index, claimData.value, claimData.playerAddress);
  }
