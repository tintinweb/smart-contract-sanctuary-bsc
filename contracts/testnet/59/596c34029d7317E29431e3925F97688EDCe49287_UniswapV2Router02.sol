...
interface IUniswapV2Router01 {
    ...
    function getAmountOut(uint256 amountIn, uint256 reserveIn, uint256 reserveOut, uint256 aReserveIn, uint256 aReserveOut) external pure returns (uint256 amountOut);
    function getAmountIn(uint256 amountOut, uint256 reserveIn, uint256 reserveOut, uint256 aReserveIn, uint256 aReserveOut) external pure returns (uint256 amountIn);
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'7e0874fa318eaf1fd6d9bdcf81cbea0ab5298196de8d27fcf83ee15c98e91e77' // init code hash
            ))));
    }
        ...
        )
    {
        ...
        uint256 aReserve0;
        uint256 aReserve1;
        (reserve0, reserve1, aReserve0, aReserve1) = IUniswapV2Pair(pairFor(factory, tokenA, tokenB)).getPairInfo();
    ...
    function getAmountOut(uint256 amountIn, uint256 reserveIn, uint256 reserveOut,
                            uint256 aReserveIn, uint256 aReserveOut) internal pure returns (uint256 amountOut) {
    ...
    function getAmountIn(uint256 amountOut, uint256 reserveIn, uint256 reserveOut,
                        uint256 aReserveIn, uint256 aReserveOut) internal pure returns (uint256 amountIn) {
        require(amountOut > 0, 'UniswapV2Library: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'UniswapV2Library: INSUFFICIENT_LIQUIDITY');
        uint256 numerator = aReserveIn.mul(amountOut).mul(1000);
        uint256 denominator = aReserveOut.sub(amountOut).mul(997);
    ...
    function getAmountsOut(address factory, uint256 amountIn, address[] memory path) internal view returns (uint256[] memory amounts) {
        ...
        for (uint256 i; i < path.length - 1; i++) {
            (uint256 reserveIn, uint256 reserveOut, uint256 aReserveIn, uint256 aReserveOut) = getPairInfo(factory, path[i], path[i + 1]);
            amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut, aReserveIn, aReserveOut);
        }
    }
    ...
    function getAmountsIn(address factory, uint256 amountOut, address[] memory path) internal view returns (uint256[] memory amounts) {
        ...
        for (uint256 i = path.length - 1; i > 0; i--) {
            (uint256 reserveIn, uint256 reserveOut, uint256 aReserveIn, uint256 aReserveOut) = getPairInfo(factory, path[i - 1], path[i]);
            amounts[i - 1] = getAmountIn(amounts[i], reserveIn, reserveOut, aReserveIn, aReserveOut);
        }
    }
}
...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function _addLiquidity(
        ...
        if (IUniswapV2Factory(factory).getPair(tokenA, tokenB) == address(0)) {
            IUniswapV2Factory(factory).createPair(tokenA, tokenB, aFactor);
        }
        (uint256 reserveA, uint256 reserveB, ,) = UniswapV2Library.getPairInfo(factory, tokenA, tokenB);
    ...
    function addLiquidity(
        ...
        (amountA, amountB) = _addLiquidity(tokenA, tokenB, 0, amountADesired, amountBDesired, amountAMin, amountBMin);
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            amountOutput = UniswapV2Library.getAmountOut(amountInput, reserveInput, reserveOutput, aReserveInput, aReserveOutput);
            }
    ...
    {
        return UniswapV2Library.getAmountOut(amountIn, reserveIn, reserveOut, aReserveIn, aReserveOut);
    }
    ...
    {
        return UniswapV2Library.getAmountIn(amountOut, reserveIn, reserveOut, aReserveIn, aReserveOut);
    }
