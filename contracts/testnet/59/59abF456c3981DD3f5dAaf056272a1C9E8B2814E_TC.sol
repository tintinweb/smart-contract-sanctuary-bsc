...
contract TC is IERC20, Ownable {
    ...
    address constant WMATIC = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    address constant DEAD   = 0x000000000000000000000000000000000000dEaD;
    address constant ZERO   = 0x0000000000000000000000000000000000000000;

    string _name = "Test Coin";
    string _symbol = "TC";
    ...
    uint256 public _maxTradeLimit = (_totalSupply * 10) / 1000; // 1%
    ...
    bool public cooldownIsEnabled = false;
    uint256 smallSwapThreshold = _totalSupply.mul(10).div(1000); // 1%
    uint256 largeSwapThreshold = _totalSupply.mul(40).div(1000); // 4%
    ...
    */
    ...
    function viewFees() external view returns (uint256, uint256, uint256, uint256, uint256, uint256) { 
        return (liquidityFee, marketingFee, reflectionFee, burnFee, totalFee, feeDenominator);
    }
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        require(!_isBot[sender],"You are a bot");
        require(!_isBot[recipient],"You are a bot");
        require(
            tradingIsEnabled ||
        ...
        if(sender == pair && !isTxLimitExempt[recipient]){
            if (cooldownIsEnabled) {
                require(_lastBuy[recipient] + antiBotBuyCoolDown < block.timestamp, "You can not buy tokens right now");
                _lastBuy[recipient] = block.timestamp;
            }
            if(!isTxLimitExempt[recipient]){
                require(amount <= _maxTradeLimit,"You can not buy more than 0.5% of total supply");
            }
        }

        if(recipient == pair && !isTxLimitExempt[sender]){
            if (cooldownIsEnabled) {
                require(_lastSell[sender] + antiBotSellCoolDown < block.timestamp, "You can not sell tokens right now");
                _lastSell[sender] = block.timestamp;
            }
            if(!isTxLimitExempt[sender]){
                require(amount <= _maxTradeLimit,"You can not sell more than 0.5% of total supply");
            }
        }
    ...
    function takeFeeInProportions(address sender, address receiver, uint256 proportionAmount) internal returns (uint256) {
        ...
        // burn
        uint256 proportionBurned = proportionFeeAmount.mul(burnFee).div(totalFee);
        _totalBurn = _totalBurn.sub(proportionBurned);
        emit Transfer(sender, DEAD, _totalBurn);

        // take fees
        uint256 _proportionToContract = proportionFeeAmount.sub(proportionReflected).sub(proportionBurned);
    ...
    function swapBack() internal swapping {
        ...
        uint256 _totalFee = totalFee.sub(reflectionFee).sub(burnFee);
    ...
    function changeFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _burnFee, uint256 _marketingFee) external onlyOwner {
        liquidityFee = _liquidityFee;
        reflectionFee = _reflectionFee;
        burnFee = _burnFee;
        marketingFee = _marketingFee;
        totalFee = liquidityFee.add(reflectionFee).add(burnFee).add(marketingFee);
    ...
    function changeMaxTrade(uint256 percent, uint256 denominator) external onlyOwner {
        require(isTxLimitExempt[msg.sender] && percent >= 1, "Max trade must be greater than 1%");
        _maxTradeLimit = _totalSupply.mul(percent).div(denominator);
    }
