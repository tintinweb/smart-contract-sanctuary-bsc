...
contract GBPToken is ERC20Detailed, Ownable {
    ...
    uint256 public swapInterval = 6 minutes;
    uint256 private _lastSwapTime;
    uint256 private _lastRebasedTime;
    uint256 public _rebaseStime;
    uint256 public rebaseInterval = 3 minutes;
    ...
    constructor(
        ...
        MAX_SUPPLY = _totalSupply * 100000000;
        TOTAL_GONS = MAX_UINT256 - (MAX_UINT256 % _totalSupply);
        _gonsBalances[msg.sender] = TOTAL_GONS;
        _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
        _isFeeExempt[msg.sender] = true;
        _isFeeExempt[address(this)] = true;
        startTradingTime = startTradingTime_;
        _lastRebasedTime = _rebaseStime = startRebaseTime_;
        emit Transfer(address(0x0), msg.sender, _totalSupply);
        _allowedFragments[address(this)][address(pancakeSwapRouter)] = MAX_UINT256;
    }
    ...
    function _transferFrom(
        ...
        if (swaping) {
            return _basicTransfer(sender, recipient, amount.mul(_gonsPerFragment));
        }
    ...
    function _rebase() private {
        ...
        for (uint256 i = 0; i < times; i++) {
            _totalSupply = _totalSupply
                .mul((10**decimals()).add(_getRebaseRate(rebaseCount)))
                .div(10**decimals());
    ...
    function _getRebaseCount() private view returns (uint256) {
        return block.timestamp.sub(_rebaseStime).div(rebaseInterval);
    }
        ...
        returns (uint256)
    {
        if (_rebaseCount < 96 * 7) {
            return 7246412223703898;
        }
        if (_rebaseCount < 96 * 14) {
            return 4232526823514061;
        }
        if (_rebaseCount < 96 * 21) {
            return 2327115532720302;
        }
        if (_rebaseCount < 96 * 28) {
            return 1227659579254334;
        }
        if (_rebaseCount < 96 * 35) {
            return 656202698113905;
        }
        return 320589488941726;
    }
    ...
    function _swap() private {
        ...
        uint256 swapTokenAmount = amountBuyBackFee.add(amountEcologyFee).add(amountMarketingFee);
        if (swapTokenAmount > 0){
            ...
            uint256 marketingAmount = receiveUsdtAmount.sub(ecologyAmount).sub(buyBackAmount);
            if(buyBackAmount>0){
    ...
    //token换usdt
    function _swapTokenForUsdt(uint256 tokenAmount) private returns(uint256 swapUsdtAmount) {
