...
contract Pool is ERC20 {
    ...
    function exit(uint256 lp) public {
        ...
        uint256 preDepositReceived;
        if (depositTokenBalance > 0) {
            uint256 ratio = lp * depositTokenBalance;
            if (ratio >= totalSupply()) {
                preDepositReceived = ratio / totalSupply();
            }
        }
        _swapToDeposit(lp);
        uint256 postDepositReceived = _getAssetBalance(depositToken);
        uint256 totalReceived = postDepositReceived - preDepositReceived;
