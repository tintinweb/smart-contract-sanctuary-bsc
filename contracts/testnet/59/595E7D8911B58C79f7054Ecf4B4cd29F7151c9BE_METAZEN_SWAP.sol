...
contract METAZEN_SWAP{
    ...
    uint public token_price = 157000000000000000000;    //0.5 Paisa
    ...
    function BuyToken(uint256 tokenQty) public payable
	{
        ...
        require(MTZ.balanceOf(address(this))>=tokenQty*1e18,"Low Token Balance In Contract");
        uint256 BUSD_amt=tokenQty*token_price;   
        require(BUSD_amt>0,"Invalid buy amount");
        require(BUSD.balanceOf(msg.sender)>=BUSD_amt);
		require(msg.value>=BUSD_amt,"Invalid buy amount");
		BUSD.transferFrom(msg.sender,owner , BUSD_amt);
		MTZ.transfer(msg.sender , (tokenQty*1e18));
	...
	function sellToken(uint256 tokenQty) public payable 
	{
			...
			uint BUSD_amt=tokenQty/token_price;
