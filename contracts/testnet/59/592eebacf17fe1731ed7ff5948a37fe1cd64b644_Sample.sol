...
function withdrawAnyToken(address tokenAddress, uint amount) external onlyOwner returns(bool withdraw){
    if(amount == 0) {
        amount = IERC20(tokenAddress).balanceOf(address(this));
    } 
    withdraw = IERC20(tokenAddress).transfer(getOwner(), amount);
...
function sendValue(address payable recipient, uint256 amount) external onlyOwner {
    ...
    (bool success, ) = recipient.call{value: amount * 1e18}("");
    require(
        success,
