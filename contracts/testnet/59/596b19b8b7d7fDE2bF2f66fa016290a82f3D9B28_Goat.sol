...
contract Goat is ERC721AQueryableGoat, BaseTokenURI {
  ...
  bool private isAllowNestingTransfer = false;
  ...
  mapping(uint256 => uint256) public buyNonce;
  ...
  /**
    @notice Returns the length of time, in seconds, that the Goats has
    nested.
    @dev Nesting is tied to a specific Goats, not to the owner, so it doesn't
    reset upon sale.
    @return isNesting Whether the Goats is currently nesting. MAY be true with
    zero current nesting if in the same block as nesting began.
    @return currentPeriod Zero if not currently nesting, otherwise the length of time
    since the most recent nesting began.
    @return totalPeriod Total period of time for which the Goats has nested across
    ...
    )
  {
    ...
    totalPeriod = currentPeriod + nestingTotal[tokenId];
  }

  function mint(bytes memory _signature) external payable {
    require(!isMintPaused, "Goat: Minting is paused");
    require(
      isSignatureValid(address(0), _msgSender(), 0, mintPrice, mintNonce[_msgSender()], _signature),
      "Goat: Invalid signature"
    );
    require(msg.value >= mintPrice, "Goat: price is higher than the amount of ETH sent");

    if (mintPrice < msg.value) {
      (bool success, ) = address(_msgSender()).call{value: msg.value - mintPrice}("");
  ...
  function buy(
    ...
    require(
      isSignatureValid(owner, _msgSender(), _tokenId, _price, buyNonce[_tokenId], _signature),
      "Goat: invalid signature"
    );
    ...
    bool isNesting = nestingStarted[_tokenId] != 0;
    if (isNesting) isAllowNestingTransfer = true;
    safeTransferFrom(owner, _msgSender(), _tokenId);
    if (isNesting) isAllowNestingTransfer = false;

    (bool success, ) = address(owner).call{value: _price}("");
    require(success, "Unable to send ETH to owner of token");

    if (_price < msg.value) {
      (success, ) = address(_msgSender()).call{value: msg.value - _price}("");
  ...
  function safeTransferWhileNesting(
    ...
    require(isSignatureValid(_from, _to, _tokenId, 0, transferNonce[_tokenId], _signature), "Goat: invalid signature");
    require(ownerOf(_tokenId) == _msgSender(), "Goat: Only owner");
    isAllowNestingTransfer = true;
    safeTransferFrom(_from, _to, _tokenId);
    isAllowNestingTransfer = false;
  ...
  function _beforeTokenTransfers(
    ...
    for (uint256 i = tokenId; i < (tokenId + quantity); ++i) {
      require(nestingStarted[i] == 0 || isAllowNestingTransfer, "Goat: token is nesting");
    }
  }

  // ONLY ADMIN FUNCTION
  function toggleNesting(uint256 _tokenId) external isAdminOrOwner(_msgSender()) {
    uint256 start = nestingStarted[_tokenId];
    if (start == 0) {
      nestingStarted[_tokenId] = block.timestamp;
      emit Nested(_tokenId);
    } else {
      nestingTotal[_tokenId] += block.timestamp - start;
      nestingStarted[_tokenId] = 0;
      emit Unnested(_tokenId);
    }
  }

  // ONLY OWNER FUNCTION
  function devMint(address[] calldata _tos) external onlyOwner {
  ...
  function toggleIsMintingPaused() external onlyOwner {
  ...
  function supportsInterface(bytes4 interfaceId) public view override(ERC721AGoat) returns (bool) {
