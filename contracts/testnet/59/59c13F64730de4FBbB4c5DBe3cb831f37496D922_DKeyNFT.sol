...
library Whitelist {
    ...
    function verifySignatureWhenMergeDNFT(
        ...
        bytes32 messageHash = getMessageHashWhenMergeDNFT(_signer, _to, _timestamp, _sessionId);
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
        address signerAddress = getSignerAddress(ethSignedMessageHash, _signature);
        return signerAddress == _signer;
    }
    ...
    function verifySignatureWhenBuyKey(
        address _signer,
        uint256 _balanceOfBuyer,
        address _to,
        bytes memory _signature
    ) internal pure returns (bool) {
        bytes32 messageHash = getMessageHashWhenBuyKey(_signer, _balanceOfBuyer, _to);
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
        address signerAddress = getSignerAddress(ethSignedMessageHash, _signature);
        return signerAddress == _signer;
    }
    ...
    function getMessageHashWhenMergeDNFT(
        ...
        return keccak256(abi.encodePacked(_signer, _to, _timestamp, _sessionId));
    }
    ...
    function getMessageHashWhenBuyKey(
        ...
        return keccak256(abi.encodePacked(_signer, _balanceOfBuyer, _to));
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function buyUsingBUSD(address _buyer, bytes memory _signature)
        ...
        nonReentrant
    {
        require(keyPrice > 0, "DNFT: KEY PRICE NOT SET");
        require(verifySignature(_buyer, _signature), "DNFT: INVALID SIGNATURE");
        require(canBuy(msg.sender), "DKEYNFT: CANNOT BUY");
        ...
        nonReentrant
    {
        require(keyPrice > 0, "DNFT: KEY PRICE NOT SET");
        require(verifySignature(_buyer, _signature), "DNFT: INVALID SIGNATURE");
        require(canBuy(msg.sender), "DKEYNFT: CAN NOT BUY");
    ...
    function verifySignature(address _buyer, bytes memory _signature) private view returns (bool) {
        require(msg.sender == _buyer, "DNFT: WRONG CANDIDATE");
        return (Whitelist.verifySignatureWhenBuyKey(signer, balanceOf(_buyer), _buyer, _signature));
    }
