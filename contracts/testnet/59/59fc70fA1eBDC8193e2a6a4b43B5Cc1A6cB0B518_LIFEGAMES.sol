...
contract LIFEGAMES is IERC20 {
    ...
    // reflection
    uint256 private _tTotal = 1_000_000 * 1e6 * 1e18;
    ...
    constructor () {
        ...
        // totalSupply to owner when deploy
    ...
    function setNewRouter(address newRouter) external onlyOwner() {
        ...
        // todo si el router cambia no tendríamos que cambiar el busd? entiendo que si, que el busd no cambia, ¿podría hacerlo?
        // tengo esta duda porque enteindo que el ETH siempre es el mismo dentro de una red, ¿porqué habría que volver a cargarlo?
        address get_pair = IFactoryV2(_newRouter.factory()).getPair(busdAddress, address(this));
        if (get_pair == address(0)) {
            lpPair = IFactoryV2(_newRouter.factory()).createPair(busdAddress, address(this));
        }
        else {
            lpPair = get_pair;
        }
