...
contract AICrewNFT is ERC721, Ownable, Pausable, ERC721Burnable {
    ...
    function setTokenURI(uint256 _nftId, string memory _tokenURI) public onlyOwner {
        require(ownerOf(_nftId) != address(0), "NFT ID NOT EXIST");
        require(bytes(_tokenURI).length > 0, "TOKEN URI MUST NOT EMPTY");
    ...
    function batchMint(uint256 _amount, string memory _tokenURI, uint8 _tier) public onlyOwner {
        require(bytes(_tokenURI).length > 0, "TOKEN URI MUST NOT EMPTY");
        require(_amount <= 100, "THE AMOUNT IS TOO LARGE TO BE MINTED");
        uint256 index;
        for (index = 0; index < _amount; ++index) {
            mint(_tokenURI, _tier);
        }
    }
...
interface IStaking {
    ...
    function convertUsdOnlyToTokenWithDecimal(uint256 _amountUsd) external view returns (uint256);
...
contract Staking is IStaking, Ownable, ERC721Holder {
    ...
    mapping(uint32 => uint8) public nftTierApys;
    ...
    function initCommissionLevel() public onlyOwner {
        ...
        comissionLevels[3] = 5;
        comissionLevels[4] = 5;
        comissionLevels[5] = 10;
        comissionLevels[6] = 10;
        comissionLevels[7] = 10;
        comissionLevels[8] = 20;
        comissionLevels[9] = 30;
        comissionLevels[10] = 40;
    }
    ...
    function stake(
        ...
        uint256 _refCode,
        bytes memory _data
    ) public validPeriod(_stakingPeriod) validRefCode(_refCode) {
        ...
        uint8 baseApy = nftTierApys[AICrewNFT(nft).getNftTier(_nftIds[0])];
        bool isValidNftArray = true;
        for (uint index = 1; index < _nftIds.length; index++) {
            uint8 currentApy = nftTierApys[AICrewNFT(nft).getNftTier(_nftIds[0])];
            if (currentApy != baseApy) isValidNftArray = false;
        }
    ...
    function stakeExecute(
        ...
        stakedNFTs[msg.sender][nextCounter].unlockTime =
            block.timestamp +
            _stakingPeriod *
            30 *
            24 *
            3600;
        stakedNFTs[msg.sender][nextCounter].totalValueStake = totalAmountStakeUsd;
        ...
        require(
            AICrewNFT(nft).isApprovedForAll(msg.sender, address(this)),
            "STAKING: MUST APPROVE FIRST"
        );
        for (uint index = 0; index < _nftIds.length; index++) {
            require(
                AICrewNFT(nft).ownerOf(_nftIds[index]) == msg.sender,
                "MARKETPLACE: NOT OWNER THIS NFT ID"
            );
            try AICrewNFT(nft).safeTransferFrom(msg.sender, address(this), _nftIds[index], _data) {
                // Emit event
                emit Staked(nextCounter, msg.sender, _nftIds[index], _stakingPeriod, _apy);
        ...
        uint256 effectNumberDecimal = getEffectDecimalForCurrency();
        uint256 totalAmountStakeUsdWithDecimal = totalAmountStakeUsd * effectNumberDecimal;
    ...
    function unstake(uint256 _stakeId, bytes memory _data) public {
        ...
        uint256 amountStakedUser = stakedNFTs[msg.sender][_stakeId].totalValueStake;
        ...
        for (uint index = 0; index < nftIds.length; index++) {
            try AICrewNFT(nft).safeTransferFrom(address(this), staker, nftIds[index], _data) {
                // Emit event
    ...
    function claim(uint256 _stakeId) public {
        ...
        require(stakeInfo.unlockTime > 0, "STAKING: INVALID CLAIM REQUESTING");
        require(block.timestamp > stakeInfo.startTime, "STAKING: WRONG TIME TO CLAIM");
        uint256 timeEndStake = stakeInfo.unlockTime;
        require(block.timestamp <= timeEndStake, "STAKING: MUST CALL UNSTAKE INSTEAD OF CLAIM");
        ...
        uint256 claimableRewardInTokenWithDecimal = convertUsdOnlyToTokenWithDecimal(
            claimableRewardInUsd
        );
        require(
     ...
     * @param _totalValueStake total value of stake (USD)
    ...
    function calculateRewardInUsd(
        ...
        uint256 rewardInUsd = (_totalValueStake * _apy * yearAmount) / 100;
        return rewardInUsd;
    }
    ...
    function claimableForStakeInUsd(
        ...
        uint256 accumulateRewardInUsd = (stakeInfo.totalRewardAmount * accumulateTimeStaked) /
            totalDurationStaked;
    ...
    function tranferNftEmergency(address _receiver, uint256 _nftId) public override onlyOwner {
        require(
            AICrewNFT(nft).ownerOf(_nftId) == address(this),
            "MARKETPLACE: NOT OWNER OF THIS NFT"
        );
        try AICrewNFT(nft).safeTransferFrom(address(this), _receiver, _nftId, "") {} catch (
            bytes memory _error
        ) {
            emit ErrorLog(_error);
