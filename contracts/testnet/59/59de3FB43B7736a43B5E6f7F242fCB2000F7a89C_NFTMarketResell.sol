...
contract NFTMarketResell is IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    function addToken(IERC20 _paytoken //uint256 _costvalue
    ) public onlyOwner {
        AllowedCrypto.push(
            TokenInfo({
                paytoken: _paytoken
    ...
    function listSale(uint256 tokenId, uint256 price, uint256 _pid) public payable nonReentrant {
    ...
    function buyNft(uint256 tokenId, uint256 _pid) public payable nonReentrant {
        TokenInfo storage tokens = AllowedCrypto[_pid];
        IERC20 paytoken;
        paytoken = tokens.paytoken;
        //price = vaultItems[tokenId].price;
