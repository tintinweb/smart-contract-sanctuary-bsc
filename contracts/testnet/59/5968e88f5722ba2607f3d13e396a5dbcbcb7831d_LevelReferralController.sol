...
contract LevelReferralController is Initializable, OwnableUpgradeable {
    ...
    uint256 public constant MAX_EPOCH_VESTING_DURATION = 7 days;
    mapping(uint256 => uint256) public epochVestingDurations;
    uint256 public epochVestingDuration;
    bool disbaleNextEpoch;
    ...
    function claimable(uint256 _epoch, address _user) public view returns (uint256) {
        ...
        uint256 vestingDuration = epochVestingDurations[_epoch];
        if (vestingDuration != 0) {
            uint256 duration = block.timestamp >= (epoch.allocationTime + vestingDuration)
                ? vestingDuration
    ...
    function nextEpoch() external {
        require(!disbaleNextEpoch, "LevelReferralController::nextEpoch: disbaleNextEpoch");
        ...
        epochVestingDurations[currentEpoch] = _vestingDuration;
    ...
    function setDisbaleNextEpoch(bool _disbaleNextEpoch) external {
        require(msg.sender == distributor, "LevelReferralController::setDisbaleNextEpoch: !distributor");
        if (disbaleNextEpoch != _disbaleNextEpoch) {
            disbaleNextEpoch = _disbaleNextEpoch;
        }
        emit DisbaleNextEpochSet(_disbaleNextEpoch);
    }
    ...
    event DisbaleNextEpochSet(bool disable);
