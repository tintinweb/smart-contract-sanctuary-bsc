...
contract LuckySpin is Ownable, Initializable, Pausable, ERC1155Holder {
    ...
    struct RoundInfo {
        ...
        address winner;
        bool claimed;
        mapping(address => uint256) userIndex; // check user is in users random round
        address[] users; //users joins
    ...
    event StartSpinRound(address indexed creator, uint256 indexed roundId, uint256 totalSlots, uint256 ticketPrice);
    ...
    function join(uint256 _amount) public whenNotPaused {
        ...
        require(info.totalSlots > info.totalJoins, 'Full Slots');
        require(info.status == 1, 'Round Is Not Available');
        require(info.userIndex[_msgSender()] == 0, 'Already Join!');
        ...
        require(info.userStakedTokens[_msgSender()].add(_amount) <= info.ticketPrice, 'Over Token Per Ticket');
        ...
        if (info.userStakedTokens[_msgSender()] == 0) {
            ...
            info.users.push(_msgSender());
        }

        info.userStakedTokens[_msgSender()] = info.userStakedTokens[_msgSender()].add(_amount);
        if (info.userStakedTokens[_msgSender()] == info.ticketPrice) {
            info.rewardUsers.push(_msgSender());
            info.userIndex[_msgSender()] = info.rewardUsers.length;
            info.totalJoins = info.totalJoins.add(1);
        }
    ...
    function spinReward(
        uint256 _totalSlots,
        uint256 _ticketPrice) public onlyWhiteListUser {
        require(_totalSlots > 0, 'Invalid  Total Slots');
        require(_ticketPrice > 0, 'Invalid  Total Slots');
        ...
        if (currentRoundId == 0) {
            ...
            emit StartSpinRound(_msgSender(), nextRoundId, _totalSlots, _ticketPrice);
            return;
        }
...
//            nextRoundInfo.totalJoins = info.totalJoins;
//
//            if (info.rewardUsers.length > 0) {
//                for (uint256 i = 0; i < info.rewardUsers.length; i++) {
//                    nextRoundInfo.rewardUsers.push(info.rewardUsers[i]);
//                    nextRoundInfo.userIndex[info.rewardUsers[i]] = info.userIndex[info.rewardUsers[i]];
//                }
//            }
//
//            if (info.users.length > 0) {
//                for (uint256 i = 0; i < info.users.length; i++) {
//                    nextRoundInfo.users.push(info.users[i]);
//                    nextRoundInfo.userStakedTokens[info.users[i]] = info.userStakedTokens[info.users[i]];
//                    nextRoundInfo.userStakedTimes[info.users[i]] = info.userStakedTimes[info.users[i]];
//                }
//            }
        }
    ...
    function getTotalStakedUser(uint256 _roundId, address _user) public view returns (uint256, uint256, uint256) {
        RoundInfo  storage round = roundInfos[_roundId];
        return (round.userStakedTokens[_user], round.ticketPrice, round.userIndex[_user]);
    }
    ...
    function getRoundInfoUser(uint256 _roundId, address _user) public view returns (RoundReturnInfo memory) {
        ...
        result.winner = round.winner;
        result.status = round.status;
        result.index = round.userIndex[_user];
