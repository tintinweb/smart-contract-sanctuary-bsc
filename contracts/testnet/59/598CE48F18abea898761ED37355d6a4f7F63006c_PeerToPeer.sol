...
contract PeerToPeer is Ownable,Pausable,SendAsset{
    ...
    function addToken (address _feePayer,address _token,string memory _name,string memory _symbol,uint256 _listingPrice) external payable {
        ...
        feeTransfer(_feePayer,_msgSender(),owner(),importTokenFee);
    ...
    function assetTopup (postOrderDetails calldata post,address _source,bytes memory _signature,uint256 _expiry,bool _feeOn) external whenNotPaused payable {
        ...
        if (post.tradeType == 1) {
          if (_source == address(0) && _feeOn) 
          require(post.amount + adFee == msg.value,'Incorrect type1 amount and fee amount');
          else require(msg.value > 0 && post.amount == msg.value,'Incorrect type1 amount');
        }
        else {
        require(approvalToken[post.given],'NOT_AUTHORIZED_TOKEN');
        require(post.amount > 0,'Incorrect type2 amount');
        tokenSafeTransferFrom(IBEP20(post.given),_msgSender(),address(this),post.amount);
        }

        if (_feeOn) 
        feeTransfer(_source, _msgSender(),owner(), adFee);
    ...
    function feeTransfer(address source,address from,address to,uint256 amount) private {
        if (source == address(0))
        sendEth(to,amount);
        else tokenSafeTransferFrom(IBEP20(source),from,to,amount);
        emit FundTransfer(address(this),source,to,amount);
    }
