...
contract PoolIDO2 is Ownable, ReentrancyGuard {
  ...
  uint256 public requireStakingToken = 1000; // 1,000 BAMI
  uint256 public feeAmount = 200 * generalDec; // 200 BAMI
  ...
  function setup(
        ...
        uint256 _feeAmount) public onlyOwner {
  ...
  function stakeBamiGetSlot() public validStakeTime nonReentrant isRun {
    ...
    claimToken.safeTransferFrom(msg.sender, address(this), requireStakingToken);
    claimToken.safeTransferFrom(msg.sender, devWallet, feeAmount);
  ...
  function unstakeToken() public nonReentrant validUnstakeTime {
    ...
    claimToken.safeTransfer(msg.sender, requireStakingToken);
...
contract PoolMaster2 {
    ...
    function createPoolIDO2(
       ...
       address _owner
        ...
        bytes memory bytecode = type(PoolIDO2).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(block.timestamp));
        assembly {
            pool := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        PoolIDO2(pool).initialize(_devWallet, _idoOwner, _stakeToken, _claimToken, _paymentToken);
        PoolIDO2(pool).setup(_payAmountPerSlot, _tokenPerSlot, _totalOfSlot, _feeAmount);
        PoolIDO2(pool).transferOwnership(_owner);
        pools.push(Pool(2, pool));
