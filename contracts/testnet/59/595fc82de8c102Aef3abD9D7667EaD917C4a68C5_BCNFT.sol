...
contract BCNFT is ERC721, Ownable {
    ...
    function getOwnIdInff(address _owner) external view returns(uint256) {
        return _ownIdsExist[_owner].index;
    }

    function _updateOwnIds(address _account, uint256 _tokenId, uint256 _flag) public {
        if (_flag == 1) {
    ...
    function updateHolders(address _account) public {
    ...
    function balanceOfOwner(address account) public view returns(uint256) {
        return _tokenBalance[account];
    }

    function tokensOfOwner(address _owner) public view returns (uint256[] memory) {
        return _ownIds[_owner];
    }
