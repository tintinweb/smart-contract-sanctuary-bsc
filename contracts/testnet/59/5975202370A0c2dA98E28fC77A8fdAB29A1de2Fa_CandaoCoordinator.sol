...
contract CandaoCoordinator is Ownable, Pausable {
    ...
    // @dev: EVENTS
    event PriorityPassBought(address indexed buyer, string indexed reservationToken, uint256 packageIndex, uint256 packageValueIndex, uint256 price, uint256 domainLength);
    ...
    function buy(uint256 packagePrice, string memory domain, string memory reservationToken) external whenNotPaused {
        ...
        token.transfer(wallet, domainConfiguration.totalPrice);
    ...
    function buyAdditionalDomain(string memory domain, string memory reservationToken) external {
        ...
        token.transfer(wallet, domainPrice);
