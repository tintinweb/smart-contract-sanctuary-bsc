...
contract User is Ownable {
    ...
    mapping(address => uint8) public levelInfo;
    INode public node;
    ...
    function updateCreator(address _account, address _inviter)
        public
        onlyOwner
    {
    ...
    function updateNode(address _node) public onlyOwner {
        node = INode(_node);
    }
    ...
    function setInviter(address inviter) public {
        ...
        uint256 length = users[inviter].length;
        if (length <= 7) {
            levelInfo[inviter] = uint8(length);
        } else if (node.isNode(inviter)) {
            levelInfo[inviter] = 8;
        }
    }
}