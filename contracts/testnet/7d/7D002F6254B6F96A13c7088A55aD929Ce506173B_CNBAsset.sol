...
contract CNBAsset is ERC1155, AccessControl, Nonce, ReentrancyGuard {
    bytes32 public constant URI_SETTER_ROLE = keccak256("URI_SETTER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant CLAIM_ASSET = keccak256(
        "claimAsset(uint256 amount, uint256 tokenId, uint256 nonce, bytes memory signature)"
    );
    string public baseURI;
    address public vaultWallet; // collect asset that already used
    address public USDC;
    constructor(address usdc) ERC1155("CNB Asset") {
    ...
    function setVaultWallet(address wallet) public onlyRole(DEFAULT_ADMIN_ROLE){
    ...
    function tokenURI(string memory id) public view returns(string memory) {
    ...
    function claimAsset(
        uint256 amount,
        uint256 tokenId,
        uint256 transportationPrice,
        uint256 nonce,
        bytes memory signature
    )
        external
        nonReentrant
        onlySignerWithUint256ThreeValue(
            CLAIM_ASSET,
            amount,
            tokenId,
            transportationPrice,
        ...
        IERC20(USDC).transferFrom(msg.sender, vaultWallet, transportationPrice);
        safeTransferFrom(msg.sender, vaultWallet, tokenId, amount, "Used asset");
    }
...
contract Nonce {
    ...
    modifier onlySignerWithUint256ThreeValue(
        bytes32 _identity,
        uint256 _value1,
        uint256 _value2,
        uint256 _value3,
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(this, _identity, msg.sender, _value1, _value2, _value3, _nonce)
        );
