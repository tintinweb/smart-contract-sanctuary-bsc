...
contract test3 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor() {
        ...
        marketing = payable(0x51B3743dA3C1597fc293bA8101cC6054468CC4d1);
        rewardsVault = payable(0xAeE535bEeE7FECd53912FF67e792e94c4b8eF9b5);
    ...
    function setTaxAddresses(address[3] memory _taxAddress) public onlyOwner {
        devs = payable(_taxAddress[0]);
        marketing = payable(_taxAddress[1]);
        rewardsVault = payable(_taxAddress[2]);
    }
    ...
    function _transfer(
        ...
        if (sender == pairAddress && takeFee || recipient == pairAddress && takeFee) {
            uint256 taxFee = (amount * dexTaxFee) / 10000;
    ...
    function transferFee() internal virtual {
        bool success;

        uint256 contractBalance = balanceOf(address(this));
        _swapTokensForEth(contractBalance);
        uint256 ethAmount = address(this).balance;
        uint256 amountPerWallet = ethAmount/3;

        (success, ) = address(devs).call{
                value: amountPerWallet
            }("");

        (success, ) = address(marketing).call{
                value: amountPerWallet
            }("");

        (success, ) = address(rewardsVault).call{
                value: amountPerWallet-1
            }("");
    }

    function _swapTokensForEth(uint256 amountIn) public {
