...
contract BSHAREBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    */
    ...
    function initialize(
        ...
        IReceipt(BSHAREBUSD_A).initialize(_admin,address(this), "YSL-BUSD-A", "YSL-BUSD-A");
        tradeTax = 5;
        depositTax = 10;
        withdrawTax = 0;
        exchangeRatio = 10 ** 18;
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio() whenNotPaused() {
        require(amount > 0,"BSHAREBUSD: LPamount can't be 0");
        require(isBUSD,"BSHAREBUSD: Only Deposit BUSD");
        uint Price;
        ...
        if(IERC20(path[1]).balanceOf(address(this)) >= reserveBSHARE){
            percentBSHARE = 30;
            mintUSDy = 2;
            reserveBSHARE = IERC20(Admin.YSL()).balanceOf(address(BSHAREBUSD));
            ...
            console.log("reserveBSHARE",reserveBSHARE);
            Price = convertBUSDToLP((amount * percentBSHARE/100), (reserveBSHARE * (amount * percentBSHARE/100)) / reservesBUSD, Admin.BUSD(), Admin.YSL(), address(BSHAREBUSD));
        ...
        IReceipt(BSHAREBUSD_A).mint(address(this),(((_amount) / 10 ** 18) * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100));
    ...
    */
    ...
    function withdraw(address user, bool isReciept,uint _amount,address sendTo) nonReentrant external _lastExchangeRatio() whenNotPaused(){
        ...
        restrictTransfer[msg.sender]= block.number; 
        emit Withdraw("BSHAREBUSD Vault",address(this),user, _amount,block.number,block.timestamp);     
