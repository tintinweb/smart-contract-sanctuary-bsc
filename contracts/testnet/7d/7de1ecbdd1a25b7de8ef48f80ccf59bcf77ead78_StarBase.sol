...
contract StarBase is ERC20, Ownable {
    ...
    uint256 public _tokenLiquidityThreshold = 200e5 * 10e18;
    bool public ProvidingLiquidity = false;

    address public marketingAddress;

    uint16 private feeliq;
    uint16 private feemarketing;
    uint16 public feeliq_buy = 3;
    uint16 public feemarketing_buy = 6;
    uint16 public feeliq_sell = 6;
    uint16 public feemarketing_sell = 9;
    ...
    constructor() ERC20("StarBase", "STB") {
        _mint(msg.sender, 225e12 * 10 ** decimals());
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(recipient == pair){
            feeliq = feeliq_sell;
            feemarketing = feemarketing_sell;
        }
        else{
            feeliq = feeliq_buy;
            feemarketing = feemarketing_buy;
        }

        uint256 feesum = feeliq + feemarketing;
    ...
    function handle_fees() private mutexLock {
        ...
        if (contractBalance >= _tokenLiquidityThreshold) {
            ...
            uint256 totalFee = feeliq + feemarketing;
            if(feemarketing > 0){
                uint256 toSwap = contractBalance / totalFee * feemarketing;
                swapTokensForBNB(toSwap, marketingAddress);
            }

            if(feeliq > 0){
                //calculate how many tokens we need to exchange
                uint256 exchange = (contractBalance / totalFee * feeliq) / 2;
                uint256 exchangeOtherHalf = exchange;

                uint256 initBalance = address(this).balance;
                swapTokensForBNB(exchange, address(this));
    ...
    function swapTokensForBNB(uint256 tokenAmount, address recipient) private {
        ...
        // make the swap
        router.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, 0, path, payable(recipient), block.timestamp);
    ...
    function changeBuyFees(uint16 _feeliq_buy, uint16 _feemarketing_buy) external onlyOwner{
        feeliq_buy = _feeliq_buy;
        feemarketing_buy = _feemarketing_buy;
    }

    function changeMarketingAddress(address newWallet) external onlyOwner{
        marketingAddress = newWallet;
    }

    function changeSellFees(uint16 _feeliq_sell, uint16 _feemarketing_sell) external onlyOwner{
        feeliq_sell = _feeliq_sell;
        feemarketing_sell = _feemarketing_sell;
    }
