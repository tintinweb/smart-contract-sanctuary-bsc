    ...
    IChainLinkOracleSportData
{
    ...
    function getResultByAttribute(
        address _regular,
        Metadata.Attribute _attribute,
        string memory _teamId
    ) external view returns (string memory) {
        bytes32 requestId = IRegularCompetitionContract(_regular)
            .getRequestId();
        uint256 index = _getOracleResultIndex(_regular, _attribute, _teamId);
        return results[requestId][index].toString();
    }

    function _getOracleResultIndex(
        address regular,
        ...
        (
            string memory team1,
            string memory team2
        ) = IRegularCompetitionContract(regular).getTeam();
        if (Metadata.Attribute.WinTeam == attribute) return 0;
        if (playerId.compare(team1)) {
            if (Metadata.Attribute.Kill == attribute) return 1;
            if (Metadata.Attribute.Death == attribute) return 2;
            if (Metadata.Attribute.Assist == attribute) return 3;
            if (Metadata.Attribute.TowersDestroyed == attribute) return 4;
        }
        if (playerId.compare(team2)) {
            if (Metadata.Attribute.Kill == attribute) return 5;
            if (Metadata.Attribute.Death == attribute) return 6;
            if (Metadata.Attribute.Assist == attribute) return 7;
            if (Metadata.Attribute.TowersDestroyed == attribute) return 8;
        }
    }
