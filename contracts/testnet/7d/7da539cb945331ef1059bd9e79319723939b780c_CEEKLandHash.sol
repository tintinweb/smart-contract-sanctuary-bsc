...
contract CEEKLandHash is ReentrancyGuard {
   ...
   function convertBytesToBytes32(bytes memory bytesArr, uint256 startIdx) public pure returns(bytes32) {
    bytes memory result = new bytes(32);
    for(uint i = startIdx; i < startIdx+32; i++) {
        result[i-startIdx] = bytesArr[i];
    }
   ...
   function encode2(string memory ceekUserIdWithTs) public view returns(bytes32[] memory) {
       require((msg.sender==_SCOwner || checkEncodeAddress()), "Only contract owner or allowed addresses can do this");

       bytes memory ceekUserArr = bytes(abi.encode(ceekUserIdWithTs));
       uint resultIdxMax = ceekUserArr.length / 32;
       if (ceekUserArr.length % 32 != 0) {
           resultIdxMax++;
       }

       bytes32[] memory result = new bytes32[](resultIdxMax);

       for (uint i=0; i<resultIdxMax; i++) {
            result[i] = convertBytesToBytes32(abi.encode(ceekUserIdWithTs),i*32) ^ _pK;
       }

       return result;
   }

   function encode22(string memory ceekUserIdWithTs) public view returns(uint256,uint256) {
       require((msg.sender==_SCOwner || checkEncodeAddress()), "Only contract owner or allowed addresses can do this");

       uint256 resultIdxMax = bytes(abi.encode(ceekUserIdWithTs)).length / 32;
       if (bytes(abi.encode(ceekUserIdWithTs)).length % 32 != 0) {
           resultIdxMax++;
       }

        return (bytes(abi.encode(ceekUserIdWithTs)).length,resultIdxMax);

   }

   function decode2(bytes32[] memory ceekUserHash) public view returns(string memory) {
       require((msg.sender==_SCOwner || checkDecodeAddress()), "Only contract owner or allowed addresses can do this");
       uint resultIdxMax = ceekUserHash.length * 32;

       bytes memory b = new bytes(resultIdxMax);

       bytes32 result;
       for (uint i=0; i<ceekUserHash.length; i++) {
           result = ceekUserHash[i] ^ _pK;
           uint idx = 32*(i+1);
            assembly {
                mstore(add(b, idx), result)
            }
       }
