...
interface OpenerMintInterface is IERC721 {
    ...
    function setRegisteredID(address _address, uint256 _id) external;
    function pushRegisteredIDsArray(address _address, uint256 _id) external;
...
interface NFTidInterface is IERC721 {
...
contract CollectionId is Ownable, ERC721URIStorage , OpenerMintInterface {
    ...
    constructor(address _owner, string memory name, string memory symbol) ERC721(name, symbol) {
        _opener = msg.sender;
        lastTokenId = 1;
        _admin = _owner; 
    }
    ...
    function checkRegisteredID(address _address, uint256 _tokenIdToMint) external view returns(bool) {
        return registeredIDs[_address][_tokenIdToMint];
    }

    function setRegisteredID(address _address, uint256 _id) external override onlyOpener() {
        registeredIDs[_address][_id] = true;
    }

    function pushRegisteredIDsArray(address _address, uint256 _id) external override onlyOpener() {
    ...
    function mint(uint256 tokenIdToMint) external override onlyOpener() {
        require(registeredIDs[tx.origin][tokenIdToMint], "Token was not registered or not the rightful owner");
        require(!alreadyMinted[tokenIdToMint], "Already minted");

        alreadyMinted[tokenIdToMint] = true;
        _safeMint(tx.origin, tokenIdToMint);
        emit NftMinted(tokenIdToMint);
    }

    // only set by opener when pack is created
    function setMarketplaceDistribution(uint256[] memory _amounts, address[] memory _addresses, uint256 _id) external override onlyOpener() {
    ...
    // only set by admin
    function setMarketplaceDistForNFT(uint256 _tokenId, uint256[] memory _marketplaceDistributionAmounts, 
    address[] memory _marketplaceDistributionAddresses) external {
    ...
    function getMarketplaceDistributionForERC721(uint256 _tokenId) external override view returns(uint256[] memory, address[] memory) {
