...
contract PeerToPeer is Ownable,Pausable,SendAsset{
    ...
    function addToken (address _token,string memory _name,string memory _symbol,uint256 _listingPrice) external payable {
        ...
        fundTransfer(address(0),owner(),msg.value);
    ...
    function assetTopup (postOrderDetails calldata post) external whenNotPaused payable {
        postId++;
        postOrderDetails storage data = postOrder[postId];
        
        if (post.tradeType == 1) {
        require(msg.value > 0 && post.amount + post.amount*adFee/100e18 == msg.value,'Incorrect type1 amount');
        fundTransfer(address(0),owner(),post.amount*adFee/100e18);
        }
        else {
        require(approvalToken[post.given],'NOT_AUTHORIZED_TOKEN');
        require(msg.value == 0 && post.amount > 0,'Incorrect type2 amount');
        tokenSafeTransferFrom(IBEP20(post.given),_msgSender(),address(this),post.amount*adFee/100e18);
        fundTransfer(post.given,owner(),post.amount*adFee/100e18);
        }
