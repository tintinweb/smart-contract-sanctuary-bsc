...
/********************************************************************************************
     _______  _______  ______    ___   __   __             _______  _______  ______   
    |       ||       ||    _ |  |   | |  |_|  |           |       ||       ||    _ |  
    |    ___||_     _||   | ||  |   | |       |   ____    |    ___||_     _||   | ||  
    |   |___   |   |  |   |_||_ |   | |       |  |____|   |   |___   |   |  |   |_||_ 
    |    ___|  |   |  |    __  ||   |  |     |            |    ___|  |   |  |    __  |
    |   |___   |   |  |   |  | ||   | |   _   |           |   |___   |   |  |   |  | |
    |_______|  |___|  |___|  |_||___| |__| |__|           |_______|  |___|  |___|  |_|
...
contract ETRToken is ERC20Detailed {
    ...
    uint256 public rewardYield = 6740440;
    ...
    address public xETRReceiver = DEAD;
    ...
    /*
        ...
        2: Whale Sell Fee 
    */
    uint256[3] public totalFee          = [1300, 2000, 7000];
    uint256[3] public liquidityFee      = [390, 500, 1750];
    uint256[3] public treasuryFee       = [390, 400, 1400];
    uint256[3] public riskFeeValueFee   = [195, 300, 1050];
    uint256[3] public ecosystemFee      = [130, 200, 700];
    uint256[3] public operationsFee     = [65, 200, 700];
    uint256[3] public xETRFee           = [130, 400, 1400];
    uint256[3] public burnFee           = [0, 200, 700];
    ...
    modifier onlyOwner {
        require(msg.sender == owner, "ETRToken: Caller is not owner of the contract.");
        _;
    }
    ...
    constructor(address _router) ERC20Detailed("Etrix Finance", "ETR", uint8(DECIMALS)) {
        owner = msg.sender;
        ...
        _isFeeExempt[xETRReceiver] = true;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToOperation = contractTokenBalance.mul(operationsFee[0] + operationsFee[1] + operationsFee[2]).div(realTotalFee);
        uint256 amountToxETR = contractTokenBalance.mul(xETRFee[0] + xETRFee[1] + xETRFee[2]).div(realTotalFee);
        uint256 amountToEcosystem = contractTokenBalance.mul(ecosystemFee[0] + ecosystemFee[1] + ecosystemFee[2]).div(realTotalFee);
        uint256 amountToBurn = contractTokenBalance - amountToLiquify - amountToRFV - amountToTreasury - amountToOperation - amountToxETR - amountToEcosystem;
        ...
        if (amountToxETR > 0) {
            _swapTokensForBNB(amountToxETR, xETRReceiver);
        }

        if (amountToOperation > 0) {
            _swapTokensForBNB(amountToOperation, operationsReceiver);
        }
    ...
    function setFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _riskFreeValueReceiver, address _futureEcosystemReceiver, address _operationReceiver, address _xETRReceiver, address _burnReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        riskFreeValueReceiver = _riskFreeValueReceiver;
        operationsReceiver = _operationReceiver;
        xETRReceiver = _xETRReceiver;
        futureEcosystemReceiver = _futureEcosystemReceiver;
        burnReceiver = _burnReceiver;
    }

    function setFees(uint8 _feeKind, uint256 _total, uint256 _liquidityFee, uint256 _riskFreeValue, uint256 _treasuryFee, uint256 _feeFee, uint256 _operationFee, uint256 _xETRFee, uint256 _burnFee) external onlyOwner {
        require (_liquidityFee + _riskFreeValue + _treasuryFee + _feeFee + _operationFee + _xETRFee + _burnFee == 100, "subFee is not allowed");
        ...
        xETRFee[_feeKind] = _total * _xETRFee;
