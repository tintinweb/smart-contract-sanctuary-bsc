...
contract DAOLY is Context, IERC20, IERC20Metadata, Ownable {
  ...
  address public lockAddress = 0xdf4eC9B1F244b91e349E5F39285731F9C9A060C2;
  // address public reciver = 0x3243211d531165C3D62b3A380Dc39616F1A06fd2;
  address public reciver = 0x6007680ee14d4dFB22872Da9F508e8e9152b4478;
  ...
  address public usdtReceiver = projectAddress;
  ...
  function setProjectAddress(address _projectAddress)
    public
    onlyOwner
    returns (bool)
  {
  ...
  function setUsdtReceiver(address _receiver) public onlyOwner returns (bool) {
    usdtReceiver = _receiver;
    return true;
  }

  function setnumTokensSellToAddToLiquidity(uint256 num)
    public
    onlyOwner
    returns (bool)
  {
  ...
  function setPreAdd(address _pre, address upper)
    public
    onlyOwner
    returns (bool)
  {
  ...
  function Intergenerational_rewards(address sender, uint256 amount) private {
    ...
    if (pre != address(0)) {
      // 十代奖励
      //   _balances[pre] += total;
      IERC20(rewardToken).transferFrom(usdtReceiver, pre, total);
  ...
  function _transfer(
    ...
    if (sender != pair_usdt && sender != pair_wbnb) {
    ...
    if (
      ...
      sender != reciver &&
      sender != address(this) &&
      IERC20(rewardToken).balanceOf(usdtReceiver) >= 15e18
    ) {
      IERC20(rewardToken).transferFrom(
        usdtReceiver,
        market,
    ...
    if (
      ...
      _balances[sender] = _balances[sender].sub(
        amount,
        "ERC20: transfer amount exceeds balance1552542"
      );
      _balances[recipient] = _balances[recipient].add(amount);
      ...
      if (
        ...
        swapForRewardToken(numTokensSellToAddToLiquidity); //这里是可以走的
      }
    } else {
