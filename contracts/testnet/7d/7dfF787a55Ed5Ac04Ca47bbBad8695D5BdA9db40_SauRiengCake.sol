...
contract SauRiengCake {
    ...
    function buyXuWithXeng() public payable {
        uint minBNB = minXu * 10**18 / bnbPerXuRatio;
        require(msg.value >= minBNB, "Minimum purchase of 1000 XU required!");
        ...
        require(tokenXeng.allowance(msg.sender, address(this)) >= xengAmount, "Please approve to transfer XENG first!");

        tokenXeng.transferFrom(msg.sender, address(this), xengAmount); // Get XENG
        tokenXu.transferFrom(address(this), msg.sender, xuAmount); // Send XU
    }

    function buyXengWithXu() public payable {
        uint minBNB = minXeng * 10**18 / bnbPerXengRatio;
        require(msg.value >= minBNB, "Minimum purchase of 2000 XENG required!");

        uint xuAmount = msg.value * bnbPerXuRatio;
        ...
        require(tokenXu.allowance(msg.sender, address(this)) >= xuAmount, "Please approve to transfer XU first!");

        tokenXu.transferFrom(msg.sender, address(this), xuAmount); // Get XU
