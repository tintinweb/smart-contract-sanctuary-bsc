...
contract CEEKLandGate is ERC165, ReentrancyGuard, Context, IERC721TokenReceiver {
    ...
    // Mapping from msg.sender from token ID to sell price in buy payment token EIP20 token
    mapping(address => mapping(uint256 => uint256)) internal preBuyBidPriceInCurrency;
    mapping(address => mapping(uint256 => address)) internal preBuyBidPriceCurrency;
    ...
    event PreBuyPrice(uint256 indexed tokenId, uint256 indexed price, address indexed paymentToken);
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet, bytes32 ceekUserIdParam) public {	
    ...
    function beforeBuyCheck(uint256 tokenId, uint256 tokenPrice, bytes32 ceekUserIdParam, address paymentTokenAddress) private returns(address, string memory, Currency memory) {
        // is on sale
        bool onSale = false;
    ...
    function preBuyPriceAccept(uint256 tokenId, EIP20 payment_token) public nonReentrant {
        uint256 token_price;

        if (payment_token == contract_payment_token) {
            preBuyBidPriceInCurrency[msg.sender][tokenId] = sellBidPrice[tokenId];
            preBuyBidPriceCurrency[msg.sender][tokenId] = address(payment_token);
        } else {
			...
			token_price = sellBidPrice[tokenId].mul(17848114913568367989).div(1000000000000000000);
            preBuyBidPriceInCurrency[msg.sender][tokenId] = token_price;
            preBuyBidPriceCurrency[msg.sender][tokenId] = address(payment_token);
        }

        emit PreBuyPrice(tokenId, preBuyBidPriceInCurrency[msg.sender][tokenId], preBuyBidPriceCurrency[msg.sender][tokenId]);
    }
    ...
    // Buy option
    function buy(uint256 tokenId, bytes32 ceekUserIdParam, EIP20 payment_token) public payable nonReentrant {
        uint256 token_price;
        ...
        if (preBuyBidPriceInCurrency[msg.sender][tokenId] > 0 && preBuyBidPriceCurrency[msg.sender][tokenId] == address(payment_token)) {
            token_price = preBuyBidPriceInCurrency[msg.sender][tokenId];
        }
	...
	function markAsUsedCeekUserId(bytes32 ceekUserIdParam) internal {
		...
		require(successMark, "CEEKLANDGATE_029");
	}
