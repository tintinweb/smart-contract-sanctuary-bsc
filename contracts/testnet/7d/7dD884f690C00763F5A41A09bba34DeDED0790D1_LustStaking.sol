...
contract LustStaking is AccessControl {
    ...
    constructor(){
        ...
        lustToken = IERC20(0x62E85fd5e980FB168cb5C1E3DaACd0698EB973C1);
        lwapToken = IERC20(0x02D65F62BEA0B5a053Ef7D2eFdD6a15B8513ed7E);
    ...
    function unStake(uint256 stakeTime) public {
        ...
        // uint256 reward = getReward(msg.sender, stakeTime);
        ...
        if (pastTime < defaultPacages[packageIndex]) {
            // unstake with penalty
            uint256 penaltyAmount = users[msg.sender][packageIndex].amount * penaltyPercent / 100;
            // lustToken.transfer(msg.sender, users[msg.sender][packageIndex].amount - penaltyAmount);
            ...
            lustToken.transfer(msg.sender, lustReward + users[msg.sender][packageIndex].amount);
    ...
    function checkout(address user, uint256 stakeTime) public onlyAdmin {
        ...
        // uint256 reward = getReward(user, stakeTime);
        ...
        if (users[user][packageIndex].lwapToPay > 0) {
            ...
            lwapToken.transfer(msg.sender, users[user][packageIndex].lwapToPay);
        }
        lustToken.transfer(msg.sender, users[user][packageIndex].lustToPay);
    ...
    function earned(address user, uint256 stakeTime) public view returns(uint256 reward) {
        reward = 0;
        uint256 packageIndex;
        for (uint i = 0; i < users[user].length; i++) {
            if (users[user][i].stakeTime == stakeTime) {
                packageIndex = users[user][i].packageIndex;
            }

            // if (users[user][i].isActive) {
                // uint256 periodByDay = (block.timestamp - users[user][i].claimTime) / timePeriod;
                // reward += periodByDay * users[user][i].amount * defaultAPY[users[user][i].packageIndex] / 365 / 100;
            // }
        }
        // if (packageIndex > 0) {
        //     uint256 totalLustReward = 
        //     users[user][packageIndex];
        // }
    }
    ...
    function getReward(address user, uint256 stakeTime) public view returns(uint256 lustReward, uint256 lwapReward) {
        ...
        if (packageIndex > 0) {
            ...
            uint256 totalLustReward = users[user][packageIndex].amount * packagePercent[packageIndex] / 12 / 100;
