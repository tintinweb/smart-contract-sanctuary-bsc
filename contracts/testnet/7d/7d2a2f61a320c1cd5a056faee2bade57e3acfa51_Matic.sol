...
contract Matic {
    ...
    function withdrawAmount(uint256 _amount) public {
     ...
     function ClaimReward() public  returns(uint256) {
                ...
                for(uint8 i=0 ;i<= UserAllDetailByAddress[msg.sender].userAmount.length; i++){

                if( UserAllDetailByAddress[msg.sender].TimeInvested[i] + 3456000 >= block.timestamp ){
                if(UserAllDetailByAddress[msg.sender].IsWithdarawl[i] == true){
                ...
                else if( UserAllDetailByAddress[msg.sender].TimeInvested[i] + 6912000 >= block.timestamp ){
                if(UserAllDetailByAddress[msg.sender].IsWithdarawl[i] == true){
                uint256 ContractBalance = address(this).balance;  
                ...
                else if( UserAllDetailByAddress[msg.sender].TimeInvested[i] + 10368000 >= block.timestamp ){
                if(UserAllDetailByAddress[msg.sender].IsWithdarawl[i] == true){
                uint256 ContractBalance = address(this).balance;  
