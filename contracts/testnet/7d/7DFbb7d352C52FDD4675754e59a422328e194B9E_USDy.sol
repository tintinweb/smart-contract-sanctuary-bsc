...
contract USDy is ERC20, AccessControl, Pausable, IEvents, ReentrancyGuard {
    ...
    uint public USDy_Tax = 3000; //Tax always multiply by 100
    uint [] public USDy_Tax_Allocation = [1000, 1000, 1000]; //index-1 => , index-2 =>  All index value multiply by 100;
    uint [] public USDy_BuyBack = [3334,3333,3333]; //[buyBack and burn, team address]
    ...
    **/
    function mint(address account, uint256 amount) external nonReentrant onlyRole(OPERATOR_ROLE) whenNotPaused{
     ...
     */
    function burn(address account, uint256 amount) external nonReentrant onlyRole(OPERATOR_ROLE) whenNotPaused{
    ...
    function tax( uint256 taxAmount, address sender, uint amount) internal {
        ...
        if(Admin.buyBackActivation()){
            if(Admin.buyBackActivationEpoch() + (3 * Admin.epochDuration()) < block.timestamp) {
                (10**18 + 10**16) <= IUniswapV2Router02(router).getAmountsOut(10**18,path)[1] ? 
                    Admin.setBuyBackActivation(false) : Admin.setBuyBackActivationEpoch();
            }
            _burn(address(this), taxAmount * USDy_BuyBack[0]/10000);
            swap(taxAmount * USDy_BuyBack[1]/10000, path, Admin.TeamAddress());
            swap(taxAmount * USDy_BuyBack[2]/10000, path, Admin.Treasury());
            ...
            (uint amount, uint leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution(sender, (IERC20(Admin.BUSD()).balanceOf((address(this))) * USDy_Tax_Allocation[0])/ USDy_Tax, amount);
            if(amount != 0){
            ...
            IERC20(Admin.BUSD()).transfer(Admin.TeamAddress(), (IERC20(Admin.BUSD()).balanceOf((address(this))) * USDy_Tax_Allocation[2]) / (USDy_Tax));
        }        
    }
