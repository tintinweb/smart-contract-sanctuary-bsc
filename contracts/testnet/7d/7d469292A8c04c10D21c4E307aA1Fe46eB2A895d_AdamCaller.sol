...
contract AdamCaller is NonblockingLzApp {
    uint totalDaos = 0;

    constructor() NonblockingLzApp(0x6Fcb97553D41516Cb228ac03FdC8B9a0a9df04A1) {}

    function setTotalDaos(uint amount) internal {
        totalDaos = amount;
    }

    function getTotalDaos() public payable {
        ...
        _lzSend(10001, abi.encodePacked("totalDaos"), payable(msg.sender), address(0x0), bytes(""));
    }

    function _nonblockingLzReceive(uint16 _srcChainId, bytes calldata _srcAddress, uint64 _nonce, bytes calldata _payload) override internal {
        require(_srcChainId == 10001, "Not desired chain");

        string memory target;
        (target) = abi.decode(_payload, (string));

        if (keccak256(abi.encodePacked(("setTotalDaos"))) == keccak256(abi.encodePacked((target)))) {
            uint amount;
            (, amount) = abi.decode(_payload, (string, uint));
            setTotalDaos(amount);
        }
    }
