...
contract KangarooDistribution is Ownable {
    ...
    IBEP20 public immutable usdtToken; //0x55d398326f99059fF775485246999027B3197955  usdt
    IBEP20 public immutable kangarooToken;
    KangarooStake public immutable stakePool;
    address public immutable philanthropyAddress;
    uint256 public immutable minDistrBalance;
    ...
    constructor(KangarooStake _stakePool, address _philanthropyAddress) {
    ...
    function distribute() external {
        ...
        if (kangarooBalance > minDistrBalance) {
            if (chargingPool) {
                ...
                if (toBurnAmount > 0) {
                    (bool success,) = address(kangarooToken).call(
                        abi.encodeWithSignature("burn(uint256)", toBurnAmount)
                    );
                ...
                if (kangarooBalance > 0) {
                    (bool success,) = address(kangarooToken).call(
                        abi.encodeWithSignature("burn(uint256)", kangarooBalance)
                    );
                    require(success, "burn FAIL");
                }
            }
