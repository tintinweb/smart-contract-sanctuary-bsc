...
contract FuseG is Ownable, Blacklistable, Pausable, ERC20, AccessControl {
    ...
    event transferTest( address indexed sender , 
         address indexed sender2, address indexed sender3,address to , uint256);
    event senderAddress(address sender);
    event toAddress(address toAddress);
    ...
    function transfer(address to, uint256 amount) public override whenNotPaused returns (bool) {
        ...
        emit transferTest(sender, sender2, sender3, to, amount);
        uint256 reward = 0;
        if(isDex(sender)) {
