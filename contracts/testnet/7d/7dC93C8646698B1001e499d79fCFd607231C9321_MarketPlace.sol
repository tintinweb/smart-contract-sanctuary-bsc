...
import "./eth/SendEther.sol";
import "./eth/ReceiveEther.sol";

contract MarketPlace is ReentrancyGuard, ERC721Holder, ReceiveEther {
    ...
    modifier isMartketOwner() {
        require(msg.sender == ownerMartket, "You are not owner!");
        _;
    }
    ...
    function setOffers(uint _ver, address _ct, uint256 _tokenId, address _unit, uint _offersPrice, uint256 _startTime, uint256 _duration) public payable isSale(_ver, _ct, _tokenId) {
    ...
    function userRefundOffer(address _ct, uint256 _tokenId) public {
        Offers memory currentOffer = offers[msg.sender][_ct][_tokenId];
        require(currentOffer.offersPrice > 0, "Offer is not valid!");
        SendEther ethSender = SendEther(address(this));
        ethSender.sendViaTransfer{value: currentOffer.offersPrice}(payable(address(msg.sender)));
    }

    function withdralToOwner() public isMartketOwner {
        SendEther ethSender = SendEther(address(this));
        uint amount = getBalance() / 2;
        ethSender.sendViaTransfer{value: amount}(payable(address(msg.sender)));
    }
}
