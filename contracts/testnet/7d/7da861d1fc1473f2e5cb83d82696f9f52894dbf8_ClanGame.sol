...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("Boda_Busd_Dividend_Tracker", "Boda_Busd_Dividend_Tracker",_dividentToken) {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 300000 * (10**18); //must hold 3,000,000+ tokens
    }
...
contract ClanGame is ERC20, Ownable {
    ...
    function swapAndSendToFee(uint256 tokens) private  {
        ...
        uint256 initialUSDTBalance = IERC20(busdDividendToken).balanceOf(address(this));
        swapTokensForBUSD(tokens);
        uint256 newBalance = (IERC20(busdDividendToken).balanceOf(address(this))).sub(initialUSDTBalance);
