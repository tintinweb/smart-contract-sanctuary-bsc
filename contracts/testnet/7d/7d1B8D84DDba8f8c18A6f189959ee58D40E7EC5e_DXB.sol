...
contract DXB is IERC20, Context, Ownable, Initializable {
     ...
     * @param value value to send.
     */
    function _transfer(address from, address to, uint256 value) private notBlackList(from, to) {
    ...
    function removeAllFees() private {
        _restoreTaxFee = (_taxFee == 0) ? _restoreTaxFee : _taxFee;
        _restoreLiquidityFee = (_liquidityFee == 0) ? _restoreLiquidityFee : _liquidityFee;
        _restoreBurnFee = (_burnFee == 0) ? _restoreBurnFee : _burnFee;
        _restoreAddLPFee = ( _addLPFee == 0) ? _restoreAddLPFee :  _addLPFee;
