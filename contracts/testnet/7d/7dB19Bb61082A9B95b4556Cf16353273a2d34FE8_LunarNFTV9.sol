...
*/
contract LunarNFT is Initializable, ERC721Upgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
  /*
  Includes
  */
  ...
  */
  ...
  /**
  * @notice Triggered when the minting process is finished.
  ...
  * @param amount           - Amount being refunded to the wallet address.
  */
  event RefundCompleted(address indexed toAddress, uint256 updatedTimestamp, uint256 amount);
  ...
  */
  ...
  /**
  * @notice Triggered when the burn process is finished.
  ...
  */
  ...
  /**
  * @notice Withdraws all the BNB in contract to a given address.
  * @dev Withdraws all user deposited funds in the contract.
  * @param account - Account to withdraw the funds to.
  */
  function withdrawFunds(address payable account) external whenNotPaused onlyRole(LIQUIDITY_ROLE) nonReentrant
  {
    (bool sent,) = account.call{ value: address(this).balance }("");
    require(sent, "We were unable to complete a withdrawal at this time.");
  }
  ...
  * @dev Mints one token at a time. Includes refund path.
  ...
  */
  function plexusMint(address to, uint256[] memory tokenIds) external whenNotPaused onlyPlexusRoles
  {
    ...
    bool preCheckResult = false;    
    string memory preCheckMessage;
  
    // SHEPARD: Complete a pre-check for minting for non-admin users.
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender))
    {
      (preCheckResult, preCheckMessage) = _canUserMintToken(to);
      require(preCheckResult, preCheckMessage);
    }
    ...
    for (uint256 i = 0; i < tokenIds.length; i++)
    {
      ...
      // SHEPARD: Use a conditional (ternary) operator to determine the validation path being taken
      //          If an admin is minting, only check supply availability, otherwise check if the user can mint.  
      (canUserResult, canUserMessage) = (hasRole(DEFAULT_ADMIN_ROLE, msg.sender) ? _isSupplyAvailable() : _canUserMintToken(to));
      ...
      if (!canUserResult)
      {
        ...
        mintResult = false;
        //exit loop if _canUserMintToken result has failed
        break;
      }
    ...
    // MASTERJEDI: Refunds - If you're not an admin, check to see if we should issue a refund.
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender))
    {
      // MASTERJEDI: If the user received less tokens than they asked for, if they a balance greater than 
      //             the price of one token, and if they have either maxed out or the supply did, issue a refund.    
      if (tokensMinted < tokenIds.length &&
          userBalances[to] >= mintPrice &&
          (userAlreadyMinted[to] == 10 || tokenSupply.current() == maxTokenMintable)) {
          
          //Refund all available userBalance[to].
          refund(to);
      }
    }

    // SHEPARD: If we couldn’t mint any NFTs, revert and tell them why.
  ...
  */
  ...
  /**
  * @notice Refunds remaining userBalance to the `toAddress` address and then nullifies the
  *         remaining userBalance for that same address.
  * @dev Function purposely made private in order to prevent any unauthorized refunds.
  * @param toAddress - Account where the refund will be issued to.
  */
  function refund(address toAddress) private nonReentrant
  {
    uint256 toBalance = userBalances[toAddress];
    if(toBalance > 0)
    {
      payable(toAddress).transfer(toBalance);
      emit RefundCompleted(toAddress, block.timestamp, toBalance);
      userBalances[toAddress] = 0;
    }
  }

  /**
  * @notice Checks if supply exists for tokens to be minted.
  * @dev Ensures token minting do not surpass the maxTokenMintable. Prevents reverts from occurring but allows messaging to go through.
  * @return success - Boolean flag to check if a mint can occur.
  * @return message - Empty if successful, otherwise contains a detailed response describing the failure.
  */
  function _isSupplyAvailable() internal view returns(bool success, string memory message)
  {
     // SHEPARD: Check if the token supply still exists
    if (tokenSupply.current() >= maxTokenMintable)
    {
      return (false, "We're sorry, but there are no more NFTs available to be minted.");
    }
    //Role can mint the token.
    return (true, "");
  }
}
