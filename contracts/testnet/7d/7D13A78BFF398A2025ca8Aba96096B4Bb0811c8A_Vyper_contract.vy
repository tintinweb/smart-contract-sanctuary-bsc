...
def _add_claim(_to: address, _holo_amount: uint256, _gilt: Gilt):
	...
	self.GiltInfoMap[_to][_id] = Claim({_id: _id, deposit: _holo_amount, gons: self.holoyield.gonsForBalance(_amount_holoyield_mint), maturity_time: _maturity_time, redeemed: False})
...
def claim(_to: address, _id: uint256):
	...
	if (not claim.redeemed) and (claim.maturity_time < block.timestamp) and (claim.deposit > 0):
		...
		self.GiltInfoMap[_to][_id] = Claim({_id: _id, deposit: 0, gons: 0, maturity_time: 0, redeemed: True})
