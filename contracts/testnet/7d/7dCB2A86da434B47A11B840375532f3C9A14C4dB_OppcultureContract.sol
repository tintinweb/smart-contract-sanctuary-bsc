...
abstract contract ERC20Coinbase is Context, ERC20 {
    uint256 public EmitAmount = 1 * 10**uint256(18);
    uint256 public StartTimeStamp = 0;
    uint256 private oneDay = 60;
    ...
    function Emit() public {
        uint256 tokenCoinbase = ((block.timestamp.sub(StartTimeStamp).div(oneDay)) * 10**uint256(18)).sub(totalSupply().add(TotalBurned));
...
contract OppcultureContract is ERC20, ERC20Burnable, ERC20Coinbase {
    ...
    mapping (address => uint256) private volumeTraded;
    uint256[] public daily24hrVolumes = new uint256[](24);
    uint256 private lastTimestamp;

    address public OppcultureEquityContract = 0x05b17aeC63805A8ab5739dE25FceE158EBa7a2C5;
    ...
    uint256[] public BuyFeesVolumeSchedule = [1000,10000,20000,30000,40000,50000,100000,500000,1000000,10000000,0];
    uint256[] public BuyFeesPercentSchedule = [200,100,90,80,70,60,50,40,30,20,10];
    uint256[] public SellFeesVolumeSchedule = [1000,100000,500000,1000000,10000000,0];
    uint256[] public SellFeesPercentSchedule = [100,5,4,3,2,1];
    ...
    constructor() public ERC20(NAME, SYMBOL) { // Mainnet: 0x10ED43C718714eb63d5aA57B78B54704E256024E Testnet: 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
        ...
        bnbPriceFeed = AggregatorV3Interface(0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526); // Mainnet: 0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE Testnet: 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526
    ...
    function getLatestBNBPrice() public view returns (uint) { 
        (, int price ,,,) = bnbPriceFeed.latestRoundData(); 
        return uint(price).div(10**uint256(6));
    }

    function VolumeTraded(address account) public view returns(uint256) {
        return volumeTraded[account].div(10**uint256(16));
    }
    function DailyVolume() public view returns(uint256) {
        ...
        return volume.div(10**uint256(16));
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(!excludedAccount && (automatedMarketMakerPairs[sender] || automatedMarketMakerPairs[recipient])) {
            ...
            if (automatedMarketMakerPairs[sender]) {
                ...
                updateTradeVolumes(amount, recipient);
            } else if (automatedMarketMakerPairs[recipient]) {
                fees = amount.div(1000).mul(SellFee);
                updateTradeVolumes(amount, sender);
            }
    ...
    function updateTradeVolumes(uint256 amount, address adr) private {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = uniV2Router.WETH();
        amount = uniV2Router.getAmountsOut(amount, path)[1].mul(getLatestBNBPrice());

        volumeTraded[adr] = volumeTraded[adr].add(amount);
