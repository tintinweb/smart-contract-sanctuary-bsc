 ...
 * @author NitroLeague.
 */
contract NitroLeagueXPolinate is ERC1155, Ownable {
    uint256 private random = block.timestamp/block.prevrandao;
    constructor() ERC1155("") {
    }
    ...
    function airDrop(address[] memory to, uint256 totalTokens) external onlyOwner{
        require(to.length==totalTokens,"Length Mismatch");
        ...
        random++;
        _tokenIds[0]=(uint256(keccak256(abi.encodePacked(block.prevrandao, msg.sender, random))) % 5)+1;
        _mintBatch(to[i], _tokenIds, _amounts, "0xff");
        }
    }    
}
