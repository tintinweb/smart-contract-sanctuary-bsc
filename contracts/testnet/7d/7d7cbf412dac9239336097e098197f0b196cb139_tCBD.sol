...
contract tCBD is Context,IERC20Metadata,Ownable {
    ...
    uint256 private bankerFeeRate = 250; 
   ...
   event userFeeEV(address indexed topc, address from, address to, uint256 feeAmount); 
   ...
   event payUsdtEV(address indexed topc, address from, address to, uint256 payCBD); 
   event payFeeEV(address indexed topc, address from, address to, uint256 feeCBD);


       function setBankerFeeRate(uint256 bankFee_) onlyOwner public returns(bool){
     bankerFeeRate = bankFee_;
     return true;
    }
    ...
    function _transfer(
        ...
        //todo take cbd
        uint256  payCBD = amount.div(10**18);  
        uint256  realCBD = payCBD.sub(amount.div(totalFeeRate));  
        uint256  burnCBD = burnFeeRate.div(totalFeeRate).mul(payCBD); 
        uint256  feeCBD = bankerFeeRate.div(totalFeeRate).mul(payCBD); 
        totalFee += feeCBD;
        totalBurn += burnCBD;
        _feeAccount[msg.sender]=_feeAccount[msg.sender].mul(feeCBD);
        _burnAccount[msg.sender]=_burnAccount[msg.sender].mul(realCBD);
        emit burnFeeEV(lpContractAddress,address(this),msg.sender,burnCBD);  
        emit Transfer(address(this), msg.sender,burnCBD);
        emit intoAccountEV(lpContractAddress,address(this),msg.sender,realCBD); 
        emit Transfer(address(this), msg.sender,realCBD);
        }
        else  if(recipient==lpContractAddress){
         require(msg.sender != address(0),"CarbonDao: transfer from the zero address ");
         require(amount>0,"CarbonDAO: amount must be >0");
         
         uint256 totalCBD =  amount.div(10**18);
         uint256 burnCBD = saleBurnRate.div(totalFeeRate).mul(totalCBD);
         uint256 feeCBD = saleFeeRate.div(totalFeeRate).mul(totalCBD);
        totalFee += feeCBD;
        totalBurn += burnCBD;
        _feeAccount[msg.sender]=_feeAccount[msg.sender].add(feeCBD);
        _burnAccount[msg.sender]=_burnAccount[msg.sender].add(burnCBD);
         emit intoContractEV(lpContractAddress,msg.sender,address(this),feeCBD);
         emit Transfer( msg.sender,address(this),feeCBD);
