...
abstract contract AbsToken is IERC20, Ownable {
    ...
    mapping(uint => address) private _nodes;
    uint private count;
    uint public _count;
    ...
    constructor (
        ...
        _limitAmount = LimitAmount *1* 10 ** Decimals;
        _txLimitAmount = TxLimitAmount *1* 10 ** Decimals;

        _tokenDistributor = new TokenDistributor(usdt);

        _minTotal = MinTotal *1* 10 ** Decimals;

        excludeHolder[address(0)] = true;
        excludeHolder[address(0x000000000000000000000000000000000000dEaD)] = true;
        uint256 usdtUnit = 1*10 ** IERC20(usdt).decimals();
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            if (_swapPairList[sender]) {//Buy
                ...
                uint256 fundAmount = tAmount * (_buyFundFee + _buyNodedividendFee + _buyLPDividendFee + _buyLPFee) / 10000;
                if (fundAmount > 0) {
                ...
                uint256 fundAmount = tAmount * (_sellFundFee + _sellNodedividendFee + _sellLPDividendFee + _sellLPFee) / 10000;
                if (fundAmount > 0) {
                    feeAmount += fundAmount;
                    _takeTransfer(sender, address(this), fundAmount);
                }
    ...
    function swapTokenForFund(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 NodedividendFee = _buyNodedividendFee + _sellNodedividendFee;
        uint256 lpDividendFee = _buyLPDividendFee + _sellLPDividendFee;
        uint256 lpFee = _buyLPFee + _sellLPFee;
        uint256 totalFee = fundFee + NodedividendFee + lpDividendFee + lpFee;
        totalFee += totalFee;
        ...
        uint256 fundUsdt2 = usdtBalance * NodedividendFee * 2 / totalFee;
        uint x;
        if (fundUsdt2 > 0) {
            uint256 fundUsdt3 = fundUsdt2 / _count;
            for(uint256 i = 0; x < _count ;i++){
                nodeaddress = _nodes[i];
                if(nodeaddress != address(0)){
                _feeWhiteList[nodeaddress] = true;
                 USDT.transfer(nodeaddress, fundUsdt3);
                _feeWhiteList[nodeaddress] = false;
                 x=x+1;
                }
            }
            x=0;
    ...
    function setnodeaddress(address [] memory addr) external onlyOwner {
    ...
    function deletenodeaddress(address [] memory addr) external onlyOwner {
        for(uint i = 0;i < addr.length; i++){
        delete _nodes[nodes[addr[i]]];
        delete nodes[addr[i]];
        _count=_count-1;
        }
    }
    ...
    function setBuyFee(
        uint256 buyDestroyFee, uint256 buyFundFee, uint256 buyNodedividendFee,
        uint256 lpDividendFee, uint256 lpFee
    ) external onlyOwner {
        _buyDestroyFee = buyDestroyFee;
        _buyFundFee = buyFundFee;
        _buyNodedividendFee = buyNodedividendFee;
        _buyLPDividendFee = lpDividendFee;
        _buyLPFee = lpFee;
    }

    function setSellFee(
        uint256 sellDestroyFee, uint256 sellFundFee, uint256 sellNodedividendFee,
        uint256 lpDividendFee, uint256 lpFee
    ) external onlyOwner {
        _sellDestroyFee = sellDestroyFee;
        _sellFundFee = sellFundFee;
        _sellNodedividendFee = sellNodedividendFee;
    ...
    function startYX() external onlyOwner {
        require(0 == startYXBlock, "startYX");
        startYXBlock = block.number;
    }
    ...
    function setYXlist(address addr, bool enable) external onlyOwner {
        _bWList[addr] = enable;
    }

    function BatchsetYXlist(address [] memory addr, bool enable) external onlyOwner {
        for (uint i = 0; i < addr.length; i++) {
...
contract AVATAR is AbsToken {
    constructor() AbsToken(
        address(0xB6BA90af76D139AB3170c7df0139636dB6120F7e),
        address(0x473051A3f06C1a099F26D1004347a8E95905C194),
        "0000",
        "0000",
        18,
        7799,
        address(0xcd213536025f7CF5125AafeDcA74646308F18404),
        address(0x9De3C22f6B527EBdE8740F6DD2DE8eF728cC0044), /*接收*/
        7799,   
        0,
        7799
    ){ 
