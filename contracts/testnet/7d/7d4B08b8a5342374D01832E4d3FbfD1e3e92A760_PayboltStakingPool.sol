...
contract PayboltStakingPool is OwnableUpgradeable, ReentrancyGuard {
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if(remain == 0) {// unstake all 
                user.amount = remain;
                pool.totalSupply = pool.totalSupply.sub(_amount);
                emit Withdraw(address(msg.sender), _pid, _amount);
            } 
            else {
                uint256 newPid = _getTierPid(remain);
                require(newPid != poolInfo.length, "withdraw: need more amount than min staked amount");
    ...
    // claim reward tokens
    function claimPendingReward(uint256 _pid, address _address) public {
    ...
    function _getTierPid(uint256 _amount) internal view returns (uint256) {
        uint256 pid = poolInfo.length; // over pid value
    ...
    function _upgradePool(
        ...
        if (user.amount > 0) {
            claimPendingReward(_pid, _address);
        } else {
        ...
        if (_amount > 0) {
            ...
            uint256 newPid = _getTierPid(remain);
            require(newPid != poolInfo.length, "update pool: need more amount than min staked amount");
