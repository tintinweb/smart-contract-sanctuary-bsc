...
////////////////////////////////////////// methods for manage stop periods
    ...
    function removeSuspensionPeriod(uint256 index)
        public
    {
        require(suspensionPeriods.length > index, 'incorrect index');

        if (suspensionPeriods.length - 1 == index) {
            suspensionPeriods.pop();
            return;
        }

        for (uint256 i = index; i < suspensionPeriods.length - 1; ++i) {
            suspensionPeriods[i].suspensionPeriodBegin = suspensionPeriods[i + 1].suspensionPeriodBegin;
            suspensionPeriods[i].suspensionPeriodEnd = suspensionPeriods[i + 1].suspensionPeriodEnd;
        }
        suspensionPeriods.pop();
    }
...
////////////////////////////////////////// view methods
    ...
    {
        uint256 count = actualAccountBalancesInERC20[_account].length();
        assets = new address[](count);
        balances = new uint256[](count);

        for (uint256 i = 0; i < count; ++i) {
            (assets[i], balances[i]) = actualAccountBalancesInERC20[_account].at(i);
            if (assets[i] == stablecoin) {
                balances[i] /= IStablecoin(stablecoin).divisor();
            }
        }
...
////////////////////////////////////////// methods for calculate individual pending
        ...
        returns (uint256 _rewardAllowedToClaim, uint256 _rewardInDelay)
    {
        ...
        {
            uint256 _medianOfBalanceFull = _cumulativeFullBalance / _nonSuspensionPeriodFull;
...

        {
            ...
            uint256 _medianOfBalanceAllowedToClaim = _cumulativeAvailableBalance / _nonSuspensionPeriodAllowedToClaim;
        ...
        )
    {
        ...
        for (uint256 i = accountLastPeriodNumber[_account]; i > 0; --i)
        {
...

            {
                ...
                uint256 _timedRewardPeriod = _calculateNonSuspensionPeriod(_periodBegin, _periodEnd);
                _cumulativeFullBalance += _timedRewardPeriod * _periodAmountInBase;

                if (_periodAvailableEnd >= _periodBegin) {
                    if (_calculationOfCumulativeAvailableBalanceFirstTouch) {
                        _cumulativeAvailableBalance = _calculateNonSuspensionPeriod(_periodBegin, _periodAvailableEnd) * _periodAmountInBase;
                        _calculationOfCumulativeAvailableBalanceFirstTouch = false;
                    } else {
                        _cumulativeAvailableBalance += _timedRewardPeriod * _periodAmountInBase;
                    }
                }
            }
...
////////////////////////////////////////// methods for calculate individual claim amount
        ...
        returns (uint256)
    {
        ...
        uint256 _medianOfBalance = _cumulativeAvailableBalance / _nonSuspensionPeriod;
        DoubleMath.Double memory _medianOfPercent = DoubleMath.fraction(_cumulativeAvailablePercent, _nonSuspensionPeriod).add_(baseRewardInPercent);

        return CompoundPercent.compound(_medianOfBalance, _medianOfPercent, _nonSuspensionPeriod);
    }
        ...
        for (uint256 i = accountLastPeriodNumber[_account]; i > 0; --i)
        {
            ...
            if (_periodAvailableEnd >= _periodBegin) {
                ...
                if (_calculationOfCumulativeAvailableBalanceFirstTouch) {
                    ...
                    _cumulativeAvailableBalance += _calculateNonSuspensionPeriod(_periodBegin, _periodEnd) * _periodAmountInBase;
                }
