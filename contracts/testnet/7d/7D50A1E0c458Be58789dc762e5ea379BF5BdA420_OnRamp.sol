...
contract OnRamp is Ownable {
  ...
  ) external onlyOwner isValidAddress(operatorAddress) {
  ...
  function setfundCollectorAddress(
    address _fundCollectorAddress
  ) external onlyOwner isValidAddress(_fundCollectorAddress) {
  ...
  function setMaxCurrencyAmountPerSend(
    address tokenAddress,
    uint256 maxCurrencyAmountPerSend
  ) external onlyOwner isValidAddress(tokenAddress) {
  ...
  function sendCurrency(
    address receiver
  ) external payable onlyOperater isValidAddress(receiver) {
    require(maxCurrencyAmountToSend > 0, "DD_Currency is not sendable");

    require(msg.value > 0, "DD_Send amount invalid");

    require(
      msg.value <= maxCurrencyAmountToSend,
      "DD_Send currency amount exceeds limit"
    );

    require(msg.value <= address(this).balance, "DD_Not enough amount to send");

    payable(receiver).transfer(msg.value);

    emit SendCurrency(receiver, msg.value);
  }
