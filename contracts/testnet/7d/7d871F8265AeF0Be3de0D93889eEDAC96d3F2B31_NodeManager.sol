...
contract NodeManager is Initializable {
  ...
  mapping(address => bool) public airdropSupplied;
  address public airdropToken;
  string public airdropTierName;
  uint32 public airdropAmount;
  ...
  function initialize(address _feeManager, address _nftAddress) public initializer {
    ...
    addTier('basic', 10 ether, 0.13 ether, 1 days, 0.001 ether, 100);
    addTier('light', 50 ether, 0.80 ether, 1 days, 0.0005 ether, 10);
    addTier('pro', 100 ether, 2 ether, 1 days, 0.0001 ether, 5);
  ...
  function configAirdrop(address token, string memory tierName, uint32 amount) public onlyOwner {
    airdropToken = token;
    airdropTierName = tierName;
    airdropAmount = amount;
  }

  function decode(bytes memory data) private pure returns (bool b){
    assembly {
      let len := mload(data)
  ...
  function claimAirdrop() public {
    require(airdropToken!=address(0), "Airdrop does not configured.");
    require(airdropAmount!=0, "Airdrop does not configured.");
    bytes4 allowance = bytes4(keccak256(bytes('allowance(address)')));
    (bool success, bytes memory data) = airdropToken.call(abi.encodeWithSelector(allowance, msg.sender));
    require(success, "Presale error.");
    bool isAllowed = decode(data);
    require(isAllowed, "Invalid airdop wallet.");
    // require(!airdropSupplied[msg.sender], "Already claimed.");
    // _create(msg.sender, airdropTierName, '', airdropAmount);
    airdropSupplied[msg.sender] = true;
  }
  ...
  function _claim(uint256 exceptAmount) private {
    ...
    feeManager.transfer(address(msg.sender), claimableAmount.sub(exceptAmount));
  }
  ...
  function upgrade(
    ...
    if (claimableAmount > 0) {
      rewardsOfUser[msg.sender] = rewardsOfUser[msg.sender].add(claimableAmount);
      rewardsTotal = rewardsTotal.add(claimableAmount);
      feeManager.transfer(address(msg.sender), claimableAmount);
    }
    ...
    if(msg.value < feeETH) {
      uint256 fee = feeToken.sub(feeETH.sub(msg.value).mul(feeToken).div(feeETH));
  ...
  function getUpgradeFee(string memory tierNameFrom, string memory tierNameTo, uint32 count) public view returns (uint256, uint256) {
    ...
    uint256 amountToken = tierTo.price.mul(count).mul(rateFee).div(10000);
  ...
  function transfer(
    ...
    uint256 fee = feeManager.getAmountFee(tier.price.mul(count));
    ...
    if (fee > claimableAmount)
      ...
      feeManager.transfer(address(msg.sender), claimableAmount.sub(fee));
...
interface IFeeManager {
  ...
  function getAmountFee(uint256 _amount) external view returns (uint256);
