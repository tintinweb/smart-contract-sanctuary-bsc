...
contract WorldCup is Initializable, OwnableUpgradeable {
    ...
    function betting(uint amount_, uint team_) public {
        ...
        if (competitionType == 128 || competitionType == 64) {
            ...
            mapInstant[matchId][player] = (Instant({
                ...
                team: team_,
                team1Total: mapInstantTotal[matchId].team1Amount,
                team2Total: mapInstantTotal[matchId].team2Amount,
                matchId: matchId
            }));
            // 总量
    ...
    // 结算收益
    function setProceeds(address player_) public {
        address player = _msgSender();
        require(player == player_, "setProceeds: Not_the_player_himself");
        ...
        if (winner == mapPlayerRecord[player][len].team) {
            // 赢
            mapPlayerInfo[player].leverage += 1;
            // (投注数量 / 赢者池子) * 败者池子数量
            if (winner == 1) {
                totalBonus += mapPlayerRecord[player][len - 1].team1Total * mapPlayerRecord[player][len - 1].team2Total / mapPlayerRecord[player][len - 1].leverageAmount;
            } else {
                totalBonus += mapPlayerRecord[player][len - 1].team2Total * mapPlayerRecord[player][len - 1].team1Total / mapPlayerRecord[player][len - 1].leverageAmount;
            }
        } else {
        ...
        if (matchId % 127 == 0) {
            ...
            mapPlayerInfo[player].championEarnings = mapChampionTotal[championIndex].soulAmount * mapChampionTotal[championIndex].soulAmount / mapPlayerInfo[player].ssoulAmount;
        ...
        // 记录可领收益 与 本金
        mapPlayerInfo[player_].principal += mapPlayerRecord[player][len - 1].amount;
        mapPlayerInfo[player_].instantEarnings += totalBonus;
        // 删除已结算记录
    ...
    function getInstantEarnings() public {
        ...
        mapPlayerInfo[player].principaled += earnings;
