...
contract MiningPool is Ownable,IMiningPool {
    IContracts_TITIMITI Contr;
    IERC20 MITI;
    IMining M;

     uint256 MiningPoolNum;

     //MiningPoolNum=>blockTimestamps
     mapping (uint256 => uint) public blockTimestamps;

    //MiningPoolNum=>StructMP
     mapping (uint256 => StructMP) public Mining_pool_number;
     
    struct StructMP{
       uint256 Summ_MiningPool; 
       uint256 Sale;
       uint256 Miners;
       uint256 ZoomLoupe;
       uint256 Stakers;
    ...
    function Mining_UP () public onlyOwner  {
        uint256 Balance = MITI.balanceOf(address(this));
        uint256 One=Balance/100;

        uint256 Sale_=Balance/2;
        uint256 Miners_=One*25;
        uint256 ZoomLoupe_=One*10;
        uint256 Stakers_=One*15;

        StructMP memory newStructMP;
        newStructMP.Summ_MiningPool = Balance;
        newStructMP.Sale=Sale_;
        newStructMP.Miners=Miners_;
        newStructMP.ZoomLoupe=ZoomLoupe_;
        newStructMP.Stakers=Stakers_;
        
        Mining_pool_number[MiningPoolNum]=newStructMP;
        M.Set_TrueMiningPoolNum();
        MITI.transfer(Contr.getMining(), Miners_);
        MITI.transfer(Contr.getZoomLoupe(), ZoomLoupe_);
        MITI.transfer(Contr.getSaleMITI(), Sale_);
        MITI.transfer(Contr.getStaking(), Stakers_);
        uint256 MPN = MiningPoolNum +1;    
        
        blockTimestamps[MPN]= block.timestamp;
        MiningPoolNum=MPN;
    }    

    function getSumm_MiningPool(uint256 _MiningPoolNum) public virtual override view returns (uint256) {
        return Mining_pool_number[_MiningPoolNum].Summ_MiningPool;
    } 
    function getSale(uint256 _MiningPoolNum) public virtual override view returns (uint256) {
        return Mining_pool_number[_MiningPoolNum].Sale;
    } 
    function getMiners(uint256 _MiningPoolNum) public virtual override view returns (uint256) {
        return Mining_pool_number[_MiningPoolNum].Miners;
    } 
    function getZoomLoupe(uint256 _MiningPoolNum) public virtual override view returns (uint256) {
        return Mining_pool_number[_MiningPoolNum].ZoomLoupe;
    } 
    function getStakers(uint256 _MiningPoolNum) public virtual override view returns (uint256) {
        return Mining_pool_number[_MiningPoolNum].Stakers;
    } 
    function getMiningPoolNum() public virtual override view returns (uint256) {
        return MiningPoolNum;
    }
    function getblockTimestamps(uint256 _MPN)public virtual override view returns (uint) {
        return blockTimestamps[_MPN];
    }
