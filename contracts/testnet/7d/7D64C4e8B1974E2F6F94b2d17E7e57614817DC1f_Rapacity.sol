...
contract Rapacity is ERC20, Ownable {
    ...
    address nodeca = address(0);


    /* Address of flush accs */ //TODO
    address internal constant rapacityTeam_addr_1 = 0x261745d06B800E737d711308aec770e1216025cD; // 50%
    address internal constant rapacityTeam_addr_2 = 0x7C55Aa30a938Da7081D653917D7846856fAA1a92; // 50%
    //address internal marketing_addr = 0xb7a0D2Ba085F8CBB4d93cd9717C93E397b6cAB7c;
    //address internal buyBack_addr = 0x1706765C9751368682d30B8b7039E0d5E4E29F13;
    ...
    uint256 internal constant DM_rapacityTeam_percentage = 4;
    ...
    uint256 public topDeposit = 0;
    address public topDepositAddr = address(0);
    uint256 public rewardPot = 0;
    mapping(uint256 => bool) public lotteryPotSent;
    ...
    mapping(uint256 => mapping(uint256 => address)) public lobbyMembers;
    ...
    function flushFirstDayLobbyEntry() external onlyOwner() nonReentrant {
        ...
        //payable(avariceTeam_addr_1).transfer((lobbyEntry[1] * 94) /100);
        IERC20(busd).transfer(rapacityTeam_addr_1, (lobbyEntry[1] * 94) /100);
    }
    ...
    function flushdevShareOfStakeSells() external onlyOwner() nonReentrant {
        ...
        IERC20(busd).transfer(rapacityTeam_addr_1, devShareOfStakeSellsAndLoanFee);
        devShareOfStakeSellsAndLoanFee = 0;
    }


    /* Time of contract launch */
    uint256 LAUNCH_TIME = 1655659500; //TODO
    uint256 currentDay;

    function setLaunch(uint256 time) external onlyOwner() {
        require(LAUNCH_TIME - block.timestamp > 0);

        LAUNCH_TIME = time;
    }

    function _clcDay() public view returns (uint256) {
        return (block.timestamp - LAUNCH_TIME) / 1 hours;
    }
        ...
        if (currentDay != _clcDay()) {
            ...
            if (currentDay < dividendsPoolCapDays) {
                for(uint256 _day = currentDay + 1 ; _day <= currentDay * 2 ; _day++){
                    dayBNBPool[_day] += (lobbyEntry[currentDay] * 94 ) / (currentDay * 100);
                }

            } else {
                for(uint256 _day = currentDay + 1 ; _day <= currentDay + dividendsPoolCapDays ; _day++){
                    dayBNBPool[_day] += (lobbyEntry[currentDay] * 94 ) / (dividendsPoolCapDays * 100);
                }
            }
            ...
            // total of 6% from every day's lobby entry goes to:
            _sendDevShare();
            // 1% marketing share
            //_sendMarketingShare();
            // 1% buy back to current lobby day
            //_buyLobbyBuybackShare();
            // top deposit gets reward
            _sendPotRewards();
            // lottery winner gets rewards
            //_sendLotteryRewards();
            // nodes contract gets share
            _sendNodeShare();
    ...
    function _sendPotRewards() internal nonReentrant {
        require(currentDay > 0);
        require(topDepositAddr != address(0));
        require(rewardPot > 0);

        //payable(topDepositAddr).transfer(rewardPot);
        IERC20(busd).transfer(topDepositAddr, rewardPot);

        rewardPot = 0;
        topDeposit = 0;
        topDepositAddr = address(0);
    }
    ...
    /*uint256 blockHashToBeUsed;

    function _sendLotteryRewards() internal nonReentrant {
        require(currentDay > 0);
        require(!lotteryPotSent[currentDay-1]);

        if(!blockHashToBeUsed) {
            blockHashToBeUsed = block.number + 2;
            return;
        }

        uint256 randomNumber = uint256(blockhash(blockHashToBeUsed));
        uint winner = 0;
        if(randomNumber != 0)
        {
            winner = randomNumber % usersCountDaily[currentDay-1];
            address payable lotteryWinner = lobbyMembers[currentDay-1][winner];
            //payable(lotteryWinner).transfer(rewardPot);
            IERC20(busd).transfer(lotteryWinner, rewardPot);
            lotteryPotSent[currentDay-1] = true;
        }
    }*/

    function _sendNodeShare() internal nonReentrant {
        require(currentDay > 0);
        require(nodeca != address(0));
        require(isContract(nodeca));
        require(rewardPot > 0);

        //payable(nodeca).transfer(rewardPot);
        IERC20(busd).transfer(nodeca, rewardPot);
    }

    function _setNodeCA(address ca) external onlyOwner() {
        nodeca = ca;
    }

    /* Gets called once a day and withdraws avarice team's share for the previous day of lobby */
    function _sendDevShare() internal nonReentrant {
        require(currentDay > 0);

        // avariceTeamPercentage = 4% of every day's lobby entry
        uint256 avariceTeamPercentage = (lobbyEntry[currentDay - 1] * DM_rapacityTeam_percentage) /100;

        // [50%, 50%]
        IERC20(busd).transfer(rapacityTeam_addr_1, (avariceTeamPercentage * 50) /100);
        IERC20(busd).transfer(rapacityTeam_addr_2, (avariceTeamPercentage * 50) /100);
        //payable(rapacityTeam_addr_1).transfer((avariceTeamPercentage * 50) /100);
        //payable(rapacityTeam_addr_2).transfer((avariceTeamPercentage * 50) /100);
    }

    /* Gets called once a day and withdraws marketing's share for the previous day of lobby */
    /*function _sendMarketingShare() internal nonReentrant {
        ...
        IERC20(busd).transfer(marketing_addr, (lobbyEntry[currentDay - 1] * DM_marketing_percentage) /100);
    }*/

    /* Gets called once a day and withdraws buy back share for the previous day of lobby */
    /*function _buyLobbyBuybackShare() internal nonReentrant {
        ...
        IERC20(busd).transfer(buyBack_addr, (lobbyEntry[currentDay - 1] * DM_buyBack_percentage) /100);
    }*/
     ...
     */
    function EnterLobby(address referrerAddr, uint256 amount) external payable {
        ...
        if (mapMemberLobby[msg.sender][currentDay].memberLobbyValue == 0) {
            lobbyMembers[currentDay][usersCountDaily[currentDay]] = msg.sender;
            usersCount++;
        ...
        rewardPot += SafeMath.div(rawAmount, 100);
        ...
        if(rawAmount > topDeposit) {
            topDeposit = rawAmount;
            topDepositAddr = msg.sender;
        }
    ...
    function EnterStake(uint256 amount, uint256 stakingDays) external {
        require(block.timestamp > LAUNCH_TIME);
    ...
    // calculating step (stake days) for bonus ex. day 0-10 => step 1, day 40-50 = > step 5
    // function _clcStep(uint256 _stakeDays) public view returns (uint256) {
    //     require(_stakeDays < 180);

    //     uint256 _step;

    //     for  (uint256 i = 0 ; i < 19 ; i++) {
    //         if (_stakeDays >= i * 10 && _stakeDays < (i + 1) * 10) {
    //             _step = i + 1;
    //         }
    //     }
    //     return _step;
    ...
    function buyStakeRequest(address sellerAddress , uint256 stakeId) external payable {
        ...
        uint256 priceP = mapMemberStake[sellerAddress][stakeId].price;
        ...
        IERC20(busd).transferFrom(address(msg.sender), address(this), priceP);
    ...
    function withdrawSoldStakeFunds() external nonReentrant {
        ...
        IERC20(busd).transfer(msg.sender, toBeSend);
    }
    ...
    mapping(address => uint256) public totalStakesLended;
    ...
    function lendOnStake(address loanerAddress , uint256 stakeId) external payable nonReentrant {
        ...
        uint256 LenderStakeId = totalStakesLended[msg.sender];
    ...
    */
    ...
    uint256 public PSSold = 0;
    bool PSClaimed = false;

    mapping(address => PSMember) public PSMembers;

    function cWhitelist(address addr, bool add) external onlyOwner() {
        require(block.timestamp < SALE_TIME, "ERR: Private Sale already started");
        PSMembers[addr].whitelisted = add;
        PSMembers[addr].amtContributed = 0;
        PSMembers[addr].tokensReceived = false;
    }

    function updateSaleTime(uint256 time) external onlyOwner() {
        SALE_TIME = time;
    }
    ...
    function updateSaleData(uint256 max, uint256 tokenRate) external onlyOwner() {
        require(block.timestamp < SALE_TIME, "ERR: Private Sale already started");

        PSData.maxSale = max;
        PSData.rate = tokenRate;
    }

    function commitPSale(uint256 amount) public payable {
        require(PSMembers[msg.sender].whitelisted, "ERR: Not whitelisted");
        require(block.timestamp > SALE_TIME && block.timestamp < SALE_END_TIME, "ERR: Private Sale not started or over");
        require(amount <= PSData.maxSale, "ERR: Exceeding limit");

        IERC20(busd).transferFrom(address(msg.sender), address(this), amount);
        PSMembers[msg.sender].amtContributed = amount;
        PSSold += amount;
    }

    function claimPSale() public {
        require(PSMembers[msg.sender].whitelisted && PSMembers[msg.sender].amtContributed > 0, "ERR: Did not contribute");
        require(!PSMembers[msg.sender].tokensReceived, "ERR: Tokens already received");

        uint256 tokensToReceive = SafeMath.mul(PSMembers[msg.sender].amtContributed, PSData.rate);
        //PSMembers[msg.sender].amtContributed * PSData.rate;

        _mint(payable(msg.sender), tokensToReceive);
        PSMembers[msg.sender].tokensReceived = true;
    }

    function withdrawPSale() public onlyOwner() {
        require(block.timestamp > SALE_END_TIME, "ERR: Private Sale not over");
        require(!PSClaimed, "ERR: Private Sale proceeds already claimed!");

        IERC20(busd).transfer(address(msg.sender), PSSold);
        PSClaimed = true;
    }
}