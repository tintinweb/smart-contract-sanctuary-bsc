...
contract Launchpad is Controller, ReentrancyGuard {
     ...
     */
    function register(string memory launchpad_) external onlyRunningLaunchpad(launchpad_) nonReentrant{
        bool registerable;
        uint256 level;
    ...
    /**
     * @dev User can redeem staking token in refund period
     * @notice User has to register in order to refund 
     * @param launchpad_ string id of the launchpad
     */
    function refund(string memory launchpad_) external onlyRunningLaunchpad(launchpad_) nonReentrant{
        require(stakingToken != address(0), "Invalid staking token");
        require(isUserRegistered(launchpad_, msg.sender), "User has not registered yet");
        ...
        // Return the ticket
        projectToLaunchpads[launchpad_].ticketSolds[registerUser.enterLevel] -= 1;

        // Allow user to register again
        registerUser.enterTime = 0;  //note: view isUserRegistered function
    }
        ...
        returns (uint256, uint256)
    {
        ...
        if (!isUserRegistered(launchpad_, user_)){
            return (0, 0);
        }
    ...
    function getClaimableAmount(string memory launchpad_, address user_) external view returns(uint256){
        return projectToRegisterUser[launchpad_][user_].claimableAmount;    
    }
