...
contract FathomToken is IERC20, Ownable {
    ...
    address public shillingAddress = 0xd7F79B7DE736eC5f8E026D23D2C46953847E1418;
    ...
    function takeFee(
        ...
        if (shillingFee > 0) {
            _balances[address(shillingAddress)] = _balances[
                address(shillingAddress)
            ].add(shillingFee);
            emit Transfer(sender, address(shillingAddress), shillingFee);
        }
        if (feeAmount > 0) {
    ...
    function swapBackInBnb() internal swapping {
        ...
        if (bnbToJetStream > 0) {
            (bool bnbToJetStreamBnbSuccess, ) = payable(jetStreamWallet).call{
                value: bnbToJetStream,
                gas: 30000
            }("");
            bnbToJetStreamBnbSuccess = false;
        }
        if (bnbToDevops > 0) {
            (bool bnbToDevopsSuccess, ) = payable(devopsWallet).call{
                value: bnbToDevops,
                gas: 30000
            }("");
            bnbToDevopsSuccess = false;
        }
    }
    ...
    function changeShillingAddress(address _shillingAddress)
        external
        onlyOwner
    {
        shillingAddress = _shillingAddress;
    }
