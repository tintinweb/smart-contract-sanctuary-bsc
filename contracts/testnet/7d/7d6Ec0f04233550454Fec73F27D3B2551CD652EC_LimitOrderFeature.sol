...
contract MatchOrdersFeature is IFeature, IMatchOrdersFeature, FixinCommon, FixinTokenSpender, NativeOrdersInfo {
    ...
    function matchOrders(
        ...
        return
            _matchOrders(sellOrder, buyOrder, sellSignature, buySignature, price, sellType, buyType, sellTier, buyTier);
    }
    ...
    function _matchOrders(
        ...
        matchedFillResults = calculateMatchedFillResultsWithFeeForTaker(matchedFillResults, sellTier, buyTier);
    ...
    function calculateMatchedFillResultsWithFeeForTaker(
        ...
        // Recalculate fee
        // matchedFillResults.takerAmountFinal =
        uint256 tempRealBuyFee = matchedFillResults.realBuyFee.safeMul(sellTier).safeDiv(buyTier);
        uint256 tempRealSellFee = matchedFillResults.realSellFee.safeMul(buyTier).safeDiv(sellTier);

        matchedFillResults.takerAmountFinal = matchedFillResults.takerAmountFinal - tempRealBuyFee;
        matchedFillResults.makerAmountFinal = matchedFillResults.makerAmountFinal - tempRealSellFee;

        matchedFillResults.realBuyFee -= tempRealBuyFee;
        matchedFillResults.realSellFee -= tempRealSellFee;
...
contract ManageTokenFeature is IFeature, IManageTokenFeature, FixinCommon, FixinTokenSpender {
    ...
    function withdrawFeeSystem(
        ...
        emit SentUserFeeSystem(_to, address(_token), _amount - _gasFee);
...
interface IManageTokenFeature {
    ...
    event SentUserFeeSystem(address _user, address _token, uint256 _amount);
