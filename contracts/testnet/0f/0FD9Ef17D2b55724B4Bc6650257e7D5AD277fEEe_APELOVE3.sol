...
contract APELOVE3 {
    ...
    string public name     = "APE Capital DAO III";
    string public symbol   = "ApeLOV3";
    ...
    function addLPs(address[] calldata addresses, uint256[] calldata limit) _ownerOnly public {
        for(uint i =0;i<addresses.length;i++) {
            LPs[addresses[i]] = limit[i] * 10 ** 18;
        }
    }
    ...
    function getAmountLeft(address _address) external view returns (uint256) {
        return LPs[_address] - claimed[_address];
    }

    function deposit(uint256 _value) public {
        require(IERC20(LOVContract).balanceOf(msg.sender) >= _value);
        require(claimed[msg.sender] + _value <= LPs[msg.sender]);
