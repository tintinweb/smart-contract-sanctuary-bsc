...
contract UniverzToken is IERC20Metadata, Ownable {
    ...
    constructor() {
        ...
        salePhasePrices = [50000, 1000000, 1500000, 2500000];
    }
    ...
    function buyToken(uint256 _payAmount) public {
        ...
        if(saleCounter>=(totalSupply()*salePhasesPercentages[currentSalePhase])/percentDivider && currentSalePhase<4){
                currentSalePhase++;
                saleCounter=0;
        }
    }
    ...
    function setSalePhasesPercentages(uint256[4] memory _percentages, uint _percentDivider) external onlyOwner {
        salePhasesPercentages=_percentages ;
        percentDivider = _percentDivider;
    }
    function setMinAmountofPurchase(uint256[4] memory _minPurchaseAmount) external onlyOwner {
        minPurchaseAmount = _minPurchaseAmount;
    }
    function withdrawStuckToken(IERC20Metadata _token , uint256 _amount ) external onlyOwner {
    ...
    function getTokenAmount(uint256 _amount) public view returns(uint256){
         return (_amount/salePhasePrices[currentSalePhase])*1e12;
    }
}