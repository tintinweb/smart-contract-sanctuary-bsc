...
contract ELVIS is ERC20, Ownable, WhiteList {
    ...
    function autoClearDiv() private {
        ...
        for (uint i = 0; i < amount-1; i++) {
            //uniswapV2Pair.balanceOf(user) >= lpDividendAmount
            if(lpPush[lpUser[i]] && uniswapV2Pair.balanceOf(lpUser[i]) < lpDividendAmount){
                lpPush[lpUser[i]] = false;
                lpUser[lpIndex[lpUser[i]]] = lpUser[lpUser.length-1];
                lpIndex[lpUser[lpUser.length-1]] = lpIndex[lpUser[i]];
                lpIndex[lpUser[i]] = 0;
                lpUser.pop();
            }
        }
