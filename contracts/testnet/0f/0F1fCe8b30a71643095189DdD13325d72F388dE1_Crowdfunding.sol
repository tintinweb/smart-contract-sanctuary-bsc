...
contract Crowdfunding is AccessControlEnumerable {
    ...
    function finishProject(uint256 _regionIndex, uint256 _round) public {
        ...
        // regions[_regionIndex].poolAmount += p.reserveFundAmount;
        if (p.currentAmount == p.targetAmount) {
            ...
            if (_round >= 3 && projects[_regionIndex][_round - 3].state == 3) {
                ...
                p3.refundAmount = (p3.targetAmount * (1000 + staticRewardRate + brokerRewardRate)) / 1000;
                _poolAmount -= p3.refundAmount;
                emit PoolAmountChange(_regionIndex, _round - 3, regions[_regionIndex].poolAmount);
            }
        } else {
            //当期失败
            p.state = 4;
            ...
            //当期奖励底池50%
            _poolAmount -= p.currentAmount;
            uint256 _bonusAmount = (_poolAmount * bonusRate) / 1000;
            p.refundAmount = _bonusAmount + p.currentAmount;
