...
// File: contracts/interfaces/IFantasticStaking.sol
...
// File: contracts/farms/FantasticChef.sol
...
contract FantasticChef is Ownable {
    ...
    function airdrop(uint256 allnum , uint256 addressnum , address[] memory _add) public {
        require(allnum <= (recommend.getsumget(msg.sender) - recommend.getsendairdrop(msg.sender)), "allnum > reward");
        uint256 sendval = allnum / addressnum;
        for(uint i = 0;i < addressnum; i++){
            if(sendval != 0){
                address d1;
                address d2;
                (d1,d2) = recommend.getdata(_add[i]);
                if(msg.sender == d1){
                    UserInfo memory pool0 = userInfo[0][_add[i]];
                    UserInfo memory pool1 = userInfo[1][_add[i]];
                    uint256 pool0num = pool0.amount;
                    uint256 pool1num = pool1.amount;
                    if((pool0num > 0) ||(pool1num > 0)){
                        rewardMinter.mint(_add[i], sendval);
                        recommend.setsendairdrop(msg.sender,sendval);
                    }
                }
            }
        }
    ...
    function harvest(uint256 pid, address to) public {
        ...
        if (_pendingReward != 0) {
            ...
            uint256 aReward;
            uint256 bReward;
            (a,b) = recommend.getdata(to);
            if(a != address(0)){
                aReward = _pendingReward / 10;
                _pendingReward = _pendingReward - aReward;
                }
                if(b!=address(0)){
                    bReward = aReward / 10;
                    aReward =  aReward - bReward;
                    recommend.setsumget(b , bReward);
                    rewardMinter.mint(b , bReward);
                }
                recommend.setsumget(a , aReward);
                rewardMinter.mint(a, aReward);
    ...
    function setrecommend(Irecommend _recommend) external onlyOwner{
        require(address(_recommend) != address(0), "Pool::setrecommend: invalid address");
