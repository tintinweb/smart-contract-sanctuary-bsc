...
contract TokenDao is Context, IBEP20, Ownable, PancakeTool {
    ...
    constructor(
        ...
        manager = _manager;
        uRewardsInterval = 1800; // 每次领取UsdtLP 奖励时间间隔10分钟， 一小时为3600秒。
    ...
    function _transfer(
        ...
        if(Address.isContract(msg.sender)){
            ...
            (user, amount) = !sender.isContract() && recipient.isContract() ? (sender, amount) : sender.isContract() && !recipient.isContract() ? (recipient, amount.sub(inviteFee, "BEP20: transfer amount exceeds balance")) :( address(0), amount);
        }

        if(user == sender && inviteFee >0){
            _balances[sender] = _balances[sender].sub(
                amount.add(inviteFee),
        ...
        if(inviteFee >0) {
            _distributeInviteRewards(user, inviteFee);
        }
