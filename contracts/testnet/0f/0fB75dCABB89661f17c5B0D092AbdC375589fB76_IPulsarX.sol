...
import "./interfaces/IIPulsarXDex.sol";

import "./ReentrancyGuard.sol";
import "./IPulsarXDex.sol";

contract IPulsarX is IBEP20, ReentrancyGuard {
        ...
        returns (bool)
    {
        require(
            ...
            "BEP20: transfer from zero address not allowed"
        );
    ...
    function transferFrom(
        ...
        require(
            sender != address(0),
            "BEP20: transferFrom from zero address not allowed"
        );
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "BEP20: pre-sale trading happens only when ICO begins"
        );

        // after ICO, use pancakeswap to buy
        require(
            icoEndsAt > block.timestamp,
            "BEP20: trader is allowed to BUY through pancakeswap after ICO"
        );

        uint256 amountToBuy = msg.value;

        // negative or zero not allowed
        require(amountToBuy > 0, "BNB: negative or zero not allowed");
    ...
    function sell(uint256 amount) public override nonReentrant returns (bool) {
        ...
        require(
            icoEndsAt > block.timestamp,
            "BEP20: trader is allowed to SELL through pancakeswap after ICO"
        );

        uint256 amountToSell = amount;
        address recipient = msg.sender;

        require(amountToSell > 0, "BEP20: negative or zero not allowed");
        require(
            balanceOf(recipient) >= amountToSell,
            "BEP20: not enough token"
        );
        require(
            allowance(recipient, address(dex)) >= amountToSell,
            "BEP20: not enough token approval"
        );
    ...
    {
        // not enough token
        require(balanceOf(address(dex)) >= amount, "BEP20: not enough token");
...
contract IPulsarXDex is IIPulsarXDex {
    /// @dev 0.001 bnb
    ...
    /// @dev
    uint256 public BUY_DEVELOPER_AND_MARKETING_FEE = 3;
    uint256 public SELL_DEVELOPER_AND_MARKETING_FEE = 5;

    /// @dev developer and marketing team
    ...
    /// @dev to keep tracks of the eligible token to sell - 20%
    mapping(address => uint256) public eligibles;
    ...
    constructor(
        ...
        token = IPulsarX(payable(_tokenSmartContract));
        ...
        returns (bool)
    {
        ...
        require(isTradingOn == true, "BEP20: trading is not allowed");
        ...
        require(amountToSend > 0, "BEP20: negative or zero not allowed");

        // send token
        if (token.transfer(recipient, amountToSend)) {
            // calculate the permitted 20% to sell during and adds
            // to the previous eligibles
            eligibles[recipient] += (20 * amountToSend) / 100;
        ...
        returns (bool)
    {
        ...
        // BEP20: 20% of your tokens is allowed for SALE during ICO or you're currently not eligible to sell your tokens.
        // Your tokens could have be gotten through airdrop or transfer from one wallet adddress to another.
        // ONLY THIS HAPPEN during ICO. Pancakeswap will be use for trading after ICO.
        require(
            amountToSell <= eligibles[recipient],
            "BEP20: 20% of your tokens is allowed for SALE during ICO or you're currently not eligible to sell your tokens"
        );
        ...
        // deduct the eligibles
        eligibles[recipient] -= amountToSell;
