...
contract VestingToken is Ownable, Pausable {
    ...
    uint256 private _duration; // in days
    ...
    constructor(
        uint256 releaseTGE,
        uint256 cliffDuration, // duration from release TGE to first cliff
        uint256 cliffDuration2, // duration from first cliff to second cliff
        uint256 duration, // duration from unlock the first time
        uint256 firstReleasePercent
    ) {
        require(cliffDuration2 <= duration, "DFHVesting: cliff is longer than duration");
        require(duration > 0, "DFHVesting: duration is 0");
        require(releaseTGE.add(cliffDuration).add(duration) > block.timestamp, "DFHVesting: ended before current time");

        _releaseTGE = releaseTGE;
        _duration = duration;
        _start = _releaseTGE.add(cliffDuration);
        _startSecond = _start.add(cliffDuration2);
    ...
    function addVestingBatch(
        ...
        for (uint256 i=0; i < beneficiary.length; i++) {
            ...
            listVesting[beneficiary[i]] = info;
        }
    }
        ...
        whenNotPaused
    {
        ...
        if (info.isFirstTime) {
            uint256 claimableAmount = _vestableAmountFirstTime(beneficiary);
            require(claimableAmount > 0, 'DFHVesting Insufficient balance');
            listVesting[beneficiary].isFirstTime = false;
            ...
            uint256 claimableAmount = _releasableAmount(beneficiary);
            require(claimableAmount > 0, 'DFHVesting Insufficient balance');
            listVesting[beneficiary].releasedAmount = listVesting[beneficiary].releasedAmount.add(claimableAmount);
            _token.transfer(beneficiary, claimableAmount);
        }
    ...
    function _releasableAmount (address beneficiary) private view returns (uint256) {
        InfoVesting memory info = listVesting[beneficiary];
        return _vestableAmount(beneficiary).sub(info.releasedAmount);
    }

    function _vestableAmountFirstTime(address beneficiary)
        internal
        view
        returns (uint256 claimableAmount)
    {
        ...
        claimableAmount = info.amount.mul(_firstReleasePercent)/100;
        return claimableAmount;
    }

    function _vestableAmount(address beneficiary)
        internal
        view
        returns (uint256 claimableAmount)
    {
        InfoVesting memory info = listVesting[beneficiary];
        uint256 totalBalance = info.amount;

        if (block.timestamp < _startSecond) {
            return 0;
        } else if (block.timestamp >= (_start.add(_duration))) {
            return totalBalance;
        } else {
            return totalBalance.mul(block.timestamp - _start).div(_duration);
        }
    }
    ...
    function getFirstReleasePercent() public view onlyOwner returns( uint256 ) {
        return _firstReleasePercent.mod(100);
    }
    ...
    function withdrawToken(IERC20 token, uint256 _amount) public onlyOwner {
        ...
        require(token.balanceOf(address(this)) >= _amount , "balanceOfToken:  is not enough");
        token.transfer(msg.sender, _amount);
    }
    
    function withdrawTokenAll(IERC20 token) public onlyOwner {
        require(token.balanceOf(address(this)) > 0 , "balanceOfToken:  is equal 0");
