...
interface ExchequerLike {
    function supPool(address) external returns (uint);
    function getsupPool() external view returns (uint);
}

contract supPoolFarm {
    ...
    function updateReward() internal {
        ...
        uint256 yield = exchequer.supPool(address(this));
    ...
    function beharvest(address usr) public view returns (uint256) {
        uint lpSupply = lptoken.balanceOf(address(this));
        uint256 yield = exchequer.getsupPool();
        uint256 lpReward = div(mul(yield,uint(1e18)),lpSupply);
