...
contract WeeklyLottery is Initializable, IWeeklyLottery, OwnableUpgradeable {
    ...
    mapping(uint256 => uint256) public nextLotteryTimestamps;
    ...
    event UserWon(address user, uint256 bet, uint256 reward);
    ...
    function initialize(address _randomizer) public initializer {
        __Ownable_init();
        randomizer = IRandomizer(_randomizer);
        prizes = [20, 15, 12, 10, 8, 7, 7, 7, 7, 7];
        lotteryPeriod = 60480; // 1 week
    }
    ...
    function changeStorage(address _storage) external onlyOwner {
        require(
            _storage != address(0),
        ...
        override
    {
        ...
        slotsToUserTikets[_count][_bet][_participant]++;
        slotsToUserStats[_bet][_participant].participateAmount++;
        emit UserEnter(_participant, _bet);
    }

    function recordTransfer(uint256 _bet, uint256 _amount) external override {
        require(msg.sender == slots, "Lottery:Not slots");
        slotsToFunds[_bet] += _amount;
    }

    function startLottery(uint256 _bet) external onlyOwner {
        require(
            slotsToParticipants[_bet].length >= 10,
        ...
        address[10] memory winners = _getWinners(_bet);
        _payProcess(_bet, winners);
        slotsToLotteryCount[_bet]++;
        nextLotteryTimestamps[_bet] = block.timestamp + lotteryPeriod;
        ...
        returns (address[10] memory winners)
    {
        ...
        for (uint256 i = 1; i < 10; i++) {
            cyclesCounter++;
            uint256 randomIndex = randomizer.addressRandom(
                lastWinner,
                cyclesCounter,
                slotsToParticipants[_bet].length
            );
    ...
    function _payProcess(uint256 _bet, address[10] memory winners) internal {
        ...
        if (monthLottery != address(0)) {
            ...
            IWeeklyLottery(monthLottery).recordTransfer(_bet, monthShare);
            balance -= monthShare;
        }
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            emit UserWon(winners[i], _bet, prize);
        }
        delete slotsToParticipants[_bet];
        slotsToFunds[_bet] = 0;
        // IStorage(storageAddress).recordWeeklyLotteryPayment(
        //     _bet,
        //     winnersAddresses,
        //     prizesAmounts
        // );
...
interface IRandomizer {
    function random(uint256 game_id, uint256 range)
        external
        view
        returns (uint256);
    function addressRandom (address _addr, uint256 id, uint256 range) external view returns (uint256);
}
