...
contract GymVaultsBank is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    uint256 public lastRewardBlockBeforeV2;
    /* ========== EVENTS ========== */
    ...
    function setLastRewardBlockBeforeV2(uint256 _lastRewardBlockBeforeV2)
        external
        onlyOwner
    {
        lastRewardBlockBeforeV2 = _lastRewardBlockBeforeV2;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accRewardPerShare =
            pool.accRewardPerShare +
            ((_reward * 1e18) / sharesTotal);
        if (lastRewardBlockBeforeV2 > 0) {
            pool.accRewardPerShare = lastRewardBlockBeforeV2;
        } else {
            pool.accRewardPerShare =
                pool.accRewardPerShare +
                ((_reward * 1e18) / sharesTotal);
    ...
    function _updateLevelPoolQualification(address wallet) internal {
        ...
        IGymLevelPool(levelPoolAddress).updateUserQualification(
            wallet,
            userLevel
        );
    }
