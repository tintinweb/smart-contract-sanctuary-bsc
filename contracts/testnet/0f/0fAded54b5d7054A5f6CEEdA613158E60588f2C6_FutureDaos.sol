...
contract FutureDaos is BEP20 {
    ...
    // Min amount to liquify. (default 500 FDAOs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to FDAO's router when our own AMM release
    IUniswapV2Router02 public FDAORouter;
    // The trading pair
    address public FDAOPair;
    // In swap and liquify
        ...
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(
                    ...
                    "FDAO::antiWhale: Transfer amount exceeds the maxTransferAmount"
                );
            }
        }
        _;
    ...
    /**
     * @notice Constructs the Future Daos contract.
     */
    constructor() public BEP20("Future Daos", "FDAO") {
        ...
        uint256 TOTAL_SUPPLY = 200 * 10**9 * 10**18; // 200,000,000,000 FDAO
        _mint(address(this), TOTAL_SUPPLY);
        _moveDelegates(address(0), _delegates[address(this)], TOTAL_SUPPLY);
    }

    /// @dev overrides transfer function to meet tokenomics of FDAO
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(
                ...
                "FDAO::transfer: Burn value invalid"
            );
            ...
            require(
                ...
                "FDAO::transfer: Tax value invalid"
            );
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the FDAO pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = FDAORouter.WETH();

        _approve(address(this), address(FDAORouter), tokenAmount);

        // make the swap
        FDAORouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        FDAORouter.addLiquidityETH{value: ethAmount}(
            address(this),
            tokenAmount,
    ...
    // To receive BNB from FDAORouter when swapping
        ...
        onlyOperator
    {
        require(
            ...
            "FDAO::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate."
        );
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(
            _burnRate <= 100,
            "FDAO::updateBurnRate: Burn rate must not exceed the maximum rate."
        );
        ...
        onlyOperator
    {
        require(
            ...
            "FDAO::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate."
        );
     ...
     * Can only be called by the current operator.
     */
    function updateFDAORouter(address _router) public onlyOperator {
        FDAORouter = IUniswapV2Router02(_router);
        FDAOPair = IUniswapV2Factory(FDAORouter.factory()).getPair(
            address(this),
            FDAORouter.WETH()
        );
        require(
            FDAOPair != address(0),
            "FDAO::updateFDAORouter: Invalid pair address."
        );
        emit FDAORouterUpdated(msg.sender, address(FDAORouter), FDAOPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(
            ...
            "FDAO::transferOperator: new operator is the zero address"
        );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "FDAO::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "FDAO::delegateBySig: invalid nonce"
        );
        ...
        returns (uint256)
    {
        require(
            ...
            "FDAO::getPriorVotes: not yet determined"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "FDAO::_writeCheckpoint: block number exceeds 32 bits"
        );
    ...
    uint256 private salePrice = 50000000; // 1BNB = 50,000,000 FDAO, 0.01BNB = 500,000 FDAO
    ...
    function transferFutureDaos(address _recipient, uint256 _amount)
        external
        onlyOwner
    {
