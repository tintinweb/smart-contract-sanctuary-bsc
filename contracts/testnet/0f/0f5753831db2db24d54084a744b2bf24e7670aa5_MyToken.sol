...
interface IAccountManagement {
    ...
    function removeTokenomics() external;
...
//    }
    ...
    function setAccountManagement(address newAddress) public onlyOwner {
        ...
        IAccountManagement(newAddress).updateOwner(owner());
        if (_tokenomics != address(0))
            IAccountManagement(newAddress).setTokenomics(_tokenomics);
    }
    ...
    function setTokenomics(address newAddress) public onlyOwner {
        require(newAddress != address(0), "Invalid tokenomics");
        require(ITokenomics(newAddress).token() == address(this), "ERC20: not linked");
        _tokenomics = newAddress;
        ITokenomics(newAddress).updateOwner(owner());
        if (_accountManagement != address(0))
            IAccountManagement(_accountManagement).setTokenomics(newAddress);
    }

    function removeTokenomics() public onlyOwner {
        _tokenomics = address(0);
        if (_accountManagement != address(0))
            IAccountManagement(_accountManagement).removeTokenomics();
    }
