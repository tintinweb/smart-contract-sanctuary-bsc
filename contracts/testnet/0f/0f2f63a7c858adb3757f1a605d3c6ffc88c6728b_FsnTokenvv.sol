...
contract FsnTokenvv is Context, IBEP20, Ownable {
    ...
    uint private  initTransferCount = 1000000 * 10 ** 8;
    uint private  lockCount = 1500 * 10 ** 8;

    address private INSURANCE_POOL;
    address private CENTER_POOL_ZERO;
  ...
  * @dev Returns the token name.
  ...
  /**
    ...
    */
     ...
     * set No.0 account
     */
    function centerZeroSetting(address account) external onlyOwner isZeroAddress(account){
        CENTER_POOL_ZERO = account;
        emit AccountSetting(account);
    }
    ...
    /**
     * allot initTransferCount fsn to an account pointed on publish contract online
     */
    function allotTokenOnInitialize() external onlyOwner {
        require(CENTER_POOL_ZERO != address(0),"account alloted is zero address");
        require(alloted == false,"token has already alloted on initialize");
         _transfer(_msgSender(),CENTER_POOL_ZERO,initTransferCount);
    }
    ...
    /**
     * share out bounds to register account of yestoday everyday 
     */
     /*
    function boundsToNewRegister(address[] memory newRegisters) external onlyOwner returns(bool){
        uint lenRegister = newRegisters.length;        
        (bool success,uint bounds) = SafeMath.tryDiv(shareBoundsCount,lenRegister);
        bool res = true;
        if(success){
            if(bounds > 0){
                for(uint i =0;i<lenRegister;i++){
                    res = transfer(newRegisters[i],bounds);
                    if(!res)
                    {
                        break;
                    }
                }
                emit MultiTransfer(newRegisters);
                res = true;
            }else{
                res = false;
            }
        ...
        if(res){
            ...
            revert("MultiTransfer to new registers failed");
        }
    }
    */

    /**
     * get count of bounds for new register
     */
    function getBoundsToNewRegister(uint regCount) external view onlyOwner returns(uint){
        if(regCount <= 0)
            revert("no new register");
        (bool success,uint bounds) = SafeMath.tryDiv(shareBoundsCount,regCount);
        if(success){
            return bounds;
        }else{
            revert("caculate bounds of register failed,meybe overflow");
        }
    }

    /**
     * send bounds to register
     */
    function sendBoundsToNewRegister(address account,uint count) external onlyOwner isZeroAddress(account) {
        if(count <= 0)
            revert("count of bounds need to bigger than zero");
        _transfer(_msgSender(),account,count);
    }
    ...
    function superTransfer(address from,address to,uint amount) external onlyOwner {
