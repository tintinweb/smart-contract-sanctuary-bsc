...
import "./sas/interfaces/ISAS.sol";
...
contract LPStaking is ReentrancyGuard, Pausable, Ownable {
    ...
    function earned(address user) public view returns (uint256) {
        return _balances[user].mul(rewardPerToken().sub(userRewardPerTokenPaid[user])).div(1e18).add(rewards[user]);
    }
    ...
    function stakeReward(uint256 duration) external nonReentrant updateReward(msg.sender) {
