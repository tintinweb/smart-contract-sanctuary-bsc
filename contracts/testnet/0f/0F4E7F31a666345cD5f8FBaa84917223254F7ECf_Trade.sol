...
contract Trade {
    ...
    event BuyAsset(address indexed assetOwner , uint256 indexed tokenId, address indexed buyer);
    event ExecuteBid(address indexed assetOwner , uint256 indexed tokenId, address indexed buyer);
    ...
    function verifyBuyerSign(address buyer, uint256 tokenId, uint amount, address paymentAssetAddress, address assetAddress, Sign memory sign) internal pure {
        bytes32 hash = keccak256(abi.encodePacked(assetAddress, tokenId, paymentAssetAddress, amount, sign.nonce));
        require(buyer == getSigner(hash, sign), "buyer sign verification failed");
    }

    function getFees(uint paymentAmt, address buyingAssetAddress) internal view returns(Fee memory){
        address tokenCreator;
        uint platformFee;
        uint royaltyFee;
        uint assetFee;
        ...
        royaltyPermille = IERC721(buyingAssetAddress).getRoyaltyFee();
        tokenCreator = IERC721(buyingAssetAddress).getCreator();
    ...
    function buyAsset(Order memory order, Sign memory sign) public returns(bool) {
        ...
        Fee memory fee = getFees(order.amount, order.nftAddress);
        require((fee.price >= order.unitPrice), "Paid invalid amount");
    ...
    function executeBid(Order memory order, Sign memory sign) public returns(bool) {
        ...
        Fee memory fee = getFees(order.amount, order.nftAddress);
        verifyBuyerSign(order.buyer, order.tokenId, order.amount, order.erc20Address, order.nftAddress, sign);
