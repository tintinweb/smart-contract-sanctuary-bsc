...
// File contracts/BSCToETH.sol


pragma solidity ^0.8.9;
contract BSCToETH is Context, Initializable {
    ...
    function createSwapPair(
        ...
        address bep20AddrBSC,
        string calldata nameETH,                
        string calldata symbolETH,
        uint8 decimalsETH
    ) external onlyOwner returns (address) {
        require(swapMappingETH2BSC[erc20AddrETH] == address(0x0), "Duplicated swap pair for ETH");
        require(swapMappingBSC2ETH[bep20AddrBSC] == address(0x0), "Duplicated swap pair for BSC");
        ...
        swapMappingETH2BSC[erc20AddrETH] = bep20AddrBSC;
        swapMappingBSC2ETH[bep20AddrBSC] = erc20AddrETH;

        emit SwapPairCreated(ethTxHash, erc20AddrETH, bep20AddrBSC, nameETH, symbolETH, decimalsETH, bsc.name, bsc.symbol, bsc.decimals);

        return bep20AddrBSC;
    }

    function swapBSC2ETH(address bep20AddrBSC, uint256 amount)
        external
        payable
        notContract
        returns (bool)
    {
        address erc20AddrETH = swapMappingBSC2ETH[bep20AddrBSC];

        require(erc20AddrETH != address(0x0), "No swap pair for this token");
        require(msg.value == swapFee, "Swap fee not equal");
        
        IERC20(bep20AddrBSC).safeTransferFrom(msg.sender, address(this), amount);

        if (msg.value != 0) {
            owner.transfer(msg.value);
        }

        emit SwapStartedBEP(bep20AddrBSC, erc20AddrETH, msg.sender, amount, msg.value);
    ...
    function fillETH2BSCSwap(
        ...
        address bscTokenAddr = swapMappingETH2BSC[erc20AddrETH];
        require(bscTokenAddr != address(0x0), "No swap pair for this token");

        filledETHTx[ethTxHash] = true;

        emit SwapFilledBEP(ethTxHash, bscTokenAddr, toAddress, amount);
