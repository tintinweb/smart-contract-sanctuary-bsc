...
contract BUSDStake {
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        uint256 refTotal = totalREF(_addr);
        for(uint8 i = 0; i < ref_bonuses.length; i++) {
            if(up == address(0)) break;
            uint256 bonus = 0;
            if ((i == 0) && (refTotal > 2)) {
            uint256 bonus = _amount * 60 / PERCENT_DIVIDER;        
            } else {
    ...
    function deposit(uint8 _tarif, address _upline, uint256 amount) external {
        ...
        require(amount >= 100 ether, "Minimum deposit amount is 100 BUSD");
    ...
    function payoutOf(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            if(from < to) {
                value += dep.amount * (to - from) * (tarif.percent)/ 100 / tarif.life_days / 8640000;
            }
        }
    ...
    function userInfo(address _addr) view external returns(uint256 for_withdraw, uint256 total_invested, uint256 total_withdrawn, uint256 total_match_bonus, uint256[BONUS_LINES_COUNT] memory structure) {
        ...
        function totalREF(address _addr) view internal returns(uint256 refTotal) {
        ...
        refTotal = structure[0] +  structure[1] +  structure[2] +  structure[3] +  structure[4];
