...
contract MetaStrikeEarnedBox is ERC1155, Pausable, AccessControl, ERC1155Burnable {
    ...
    string public constant name = "Metastrike EarnedBox";
    string public constant ticker = "MTEB";
    ...
    event EarnedBoxMinted(address player,uint8 boxId, uint256 amount);

    constructor(address _metastrikeCore) ERC1155("https://resource.metastrike.io/earnedbox/{id}.json"){
    ...
    function claimEarnedBox(uint8 boxId, uint256 _amount, bytes memory data,bytes calldata signature) public {
        _mint(msg.sender, boxId, _amount, data); 
        emit EarnedBoxMinted(msg.sender, boxId,_amount);
    }
    ...
    function openEarnedBox(uint256 _id) whenOpenBoxOperating external returns (uint256) {
        ...
        uint256 randomNumber = VerichainsNetRegistry(randomRegistry).randomService(MetaStrikeEarnedBox.randomKey).random();
