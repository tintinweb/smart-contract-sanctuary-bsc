...
contract Vote is Auth {
    ...
    function getAllMembers() public view returns (Member[] memory, bool[] memory) {
        Member[] memory _members = new Member[](totalMembers);
        bool[] memory activities = new bool[](totalMembers);

        for (uint i = 0; i < totalMembers; i++) {
            _members[i] = members[i];
            activities[i] = activations[members[i].addr];
        }

        return (_members, activities);
    }
    ...
    function getMemberByAddress(address addr) public view returns (Member memory, bool, bool) {
        ...
        return (_member, joins[addr], activations[addr]);
    }
