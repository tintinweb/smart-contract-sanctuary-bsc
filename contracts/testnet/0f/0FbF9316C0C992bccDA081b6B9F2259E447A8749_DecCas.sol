...
// https://testnet.bscscan.com/address/0x7DDc121968B2893A3f2E8CAA55AE5a37085e3B5B#code
...
contract DecCas is Ownable {
    ...
    mapping (address => uint[]) public top_winners;
    mapping (address => uint []) public wallet_plays;


    function get_total_wallet_plays(address wallet) public view returns(uint){
        return wallet_plays[wallet].length;
    }
    ...
    function get_total_winners_plays(address token) public view returns(uint){
        return top_winners[token].length;
    }
    ...
    function result(uint block_number, uint numbers) public view returns (uint) {
        require(block_number + 3 <= block.number, "Wait 3 block confirms!");
        require(block_number >= block.number - 256, "Expired block, 256 blocks pass!");

        return uint(blockhash(block_number)) % numbers;
    }

    function play_bet(address token, uint[] memory numbers, uint[] memory amounts) public returns (uint) {
        ...
        require(house_balances[token].div(10) >= bet_total.mul(36), "10% house balance lower than x36 bet amount. Please decrease bet or donate to house");
        ...
        plays.push(PlayInfo({block_number: block.number, play_numbers: numbers, play_amounts: amounts, result: 0, earnings: 0, 
            player: msg.sender, claimed: false, token: token}));
    ...
    function claim_win(uint play_id) public returns (uint [] memory) {
    ...
    function _add_winner(uint play_id) private{

        PlayInfo memory play = plays[play_id];
        address token = play.token;
        if(play.earnings > 0){
            if(top_winners[token].length < 100){
                top_winners[token].push(play_id);
            }else{

                for (uint i=0; i< top_winners[token].length; i++){

                    if(plays[top_winners[token][i]].earnings < play.earnings){

                        if(top_winners[token].length > 100){
                            top_winners[token].pop();
                        }
                        top_winners[token].push(play_id);
