...
error VelasTicketSystem__NeedToBeMoreThanZero();
...
contract VelasTicketSystem is Ownable, ReentrancyGuard {
    ...
    function purchaseTicket(uint256 numberOfTickets)
        external
        payable
        nonReentrant
    {
        if (numberOfTickets < 0) {
            revert VelasTicketSystem__NeedToBeMoreThanZero();
        }

        uint256 totalTicketAmountBUSD = numberOfTickets * ticketPriceInBUSD;

        (bool success, ) = payable(address(this)).call{
            value: totalTicketAmountBUSD
        }("");
