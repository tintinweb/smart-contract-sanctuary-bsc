...
contract Cryptopump is ContextUpgradeable, OwnableUpgradeable {
    ...
    event getMoneyForLevelEvent(address indexed UserAddress, uint256 UserId, address indexed ReferrerAddress, uint256 ReferrerId, uint256 Levelno, uint256 LevelPrice, uint256 Time);
    event lostMoneyForLevelEvent(address indexed UserAddress, uint256 UserId, address indexed ReferrerAddress, uint256 ReferrerId, uint256 Levelno, uint256 LevelPrice, uint256 Time);
    ...
    function initialize(address _ownerAddress) public initializer {
        ...
        LEVEL_PRICE[1] = 0.000001 ether;
        LEVEL_PRICE[2] = 0.000002 ether;
        LEVEL_PRICE[3] = 0.000003 ether;
        LEVEL_PRICE[4] = 0.000004 ether;
        LEVEL_PRICE[5] = 0.000005 ether;
        ...
        for (uint256 i = 1; i <= 5; i++) {
            users[_ownerAddress].levelExpired[i] = 77777777777;
        }
    ...
    function buyLevel(uint256 _level) external payable {
        ...
        if (_level == 1) {
            ...
            require(users[msg.sender].levelExpired[i] >= block.timestamp, "Buy the previous level");

            if (users[msg.sender].levelExpired[_level] == 0)
                users[msg.sender].levelExpired[_level] = block.timestamp + PERIOD_LENGTH;
