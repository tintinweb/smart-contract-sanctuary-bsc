...
contract conPhase is Context, IERC20, Ownable {
    ...
    string private constant _name = "Kabutar";
  ...
  function _burnTokenFromBuybackAddress(uint256 amount) external onlyOwner {
    ...
    // uint256 amount, 
    //address deadAddress, uint256 towneddeadwallet);

    function _burnToken (uint256 amount) external onlyOwner
    {
        uint256 amnt=amount.mul(1000);
        uint256 ownerAmnt=_tOwned[_msgSender()].div(1000);
        require(amnt > 0, "Balance should not Zero");
        require(_msgSender()!=address(0),"No Blank address"); 
        
        emit seeburnData(_msgSender() , _tOwned[_msgSender()] , amnt) ;
        emit seeburnData(_msgSender() , ownerAmnt , amnt) ;

          _tOwned[_msgSender()] -= amnt;
