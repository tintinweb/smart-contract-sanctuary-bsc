...
contract AnyaInu is IERC20, Ownable
{
    //@TresFlames
    //mapping
    ...
    //strings
    string private constant _name = 'ANYA Inu';
    string private constant _symbol = '$ANYA';
    //uints
    ...
    constructor () {
        ...
        projectWallet = 0xE46F6F8E3769A92EF361833271c1124C13EA873D;
    ...
    function _taxedTransfer(address sender, address recipient, uint amount) private{
        ...
        uint tax;
        if(isSell){  
            uint SellTaxDuration=15 seconds;          
            if(block.timestamp<LaunchTimestamp+SellTaxDuration){
                tax=750;
                }else tax=sellTax;
            }
        else if(isBuy){
            uint BuyTaxDuration=15 seconds;
