...
/**
@dev This contract is for BShareBUSD vault 
 */
contract BShareBUSDVault is AccessControl, Initializable {
    using SafeERC20 for IERC20;

    uint256 startTime;
    uint256 public No_of_Epochs;
    uint256 public no_of_days;
    uint256 public rewardsPerEpoch;
    IAdmin public  Admin; 
    uint256 public tradeTax = 5;
    uint256 public depositTax = 10;
    uint256 public exchangeRatio = 10 ** 18;
    address public router;  //router address
    address public BShareBUSD_A;
    IUniswapV2Pair BShareBUSD;
    ...
    @param _BShareBUSD BShareBUSD address
    ...
    function initialize(
        ...
        Admin= IAdmin(_Admin);
        BShareBUSD = IUniswapV2Pair(_BShareBUSD);
        startTime = block.timestamp;
        router = _router;
        BShareBUSD_A = Clones.clone(Admin.masterNTT());
        INTT(BShareBUSD_A).initialize(address(this), "Admin.BShare()-Admin.BUSD()-A", "Admin.BShare()-Admin.BUSD()-A");
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
    }
/**
    @dev deposit BShareBUSD LPtokens
    @param user address of user
    @param amountLp amount to be deposited
 */
    function deposit(address user,uint amountLp) external {
        require(amountLp > 0,"LPamount can't be 0");
            IERC20(address(BShareBUSD)).transferFrom(user, address(this), amountLp);//transferring LPtokens at this contract address

        INTT(BShareBUSD_A).mint(address(this),(amountLp * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100));
        exchangeRatio = (IERC20(address(BShareBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(BShareBUSD_A).totalSupply());
        UserDeposit[user] += (amountLp * (100 - depositTax) * (10 ** 18))/ (exchangeRatio * 100);
    }
/**
    @dev withdraw BShareBUSD LPtokens
    @param user address of user
    @param tokenA tokenA
    @param tokenB tokenB
    @param isReciept LP is reciept or not
    @param _amount amount to be deposited
 */ 
    function withdraw(address user,bool tokenA, bool tokenB,bool isReciept, uint _amount) external{
        require(_amount <= UserDeposit[user],"BShareBUSDVault: Invalid Amount");
        if(isReciept){
            IERC20(BShareBUSD_A).safeTransfer(user,_amount);//transferring receipt tokens to user
        }
        else{
            uint balance = (_amount * exchangeRatio)/10 ** 18;
                IERC20(address(BShareBUSD)).safeTransfer(user,balance);//transferring LPtokens to user  
            INTT(BShareBUSD_A).burn(address(this),balance); //minitng receipt LPtokens at this contract address
        }
        UserDeposit[user] -= _amount;
        exchangeRatio = (IERC20(address(BShareBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(BShareBUSD_A).totalSupply());
    }
// function for Claiming reward after depositing BShareBUSD LPtokens
    function claimReward() external{
        require(UserDeposit[msg.sender] > 0, "BShareBUSDVault: Deposit first");
        uint balance = (UserDeposit[msg.sender] * exchangeRatio)/(IERC20(address(BShareBUSD)).balanceOf(address(this)) * 10 ** 18);
        IERC20(Admin.YSL()).safeTransfer(msg.sender, balance);
    }
/**
    @dev purchase BShareBUSD receipt tokens
    @param user address of user
    @param amount purchase amount
    @param minAmount minAmount
 */
    function purchase(address user, uint amount, uint minAmount) external returns(uint){
        IERC20(address(BShareBUSD)).safeTransferFrom(user, address(this),amount);
        uint balance = purchaseOf(amount);
        require(balance > minAmount, "BShareBUSD: Amount is less than the minAmount");
        INTT(BShareBUSD_A).mint(user, balance); //minting receipt tokens in users account
        exchangeRatio = (IERC20(address(BShareBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(BShareBUSD_A).totalSupply()); //calculating new exchange ratio
        return(balance);
    }
/**
    @dev sell BShareBUSD receipt tokens and acquire BShareBUSD LPtokens
    @param user address of user
    @param amount sell amount
    @param minAmount minAmount
 */
    function sell(address user, uint amount, uint minAmount) external returns(uint){
        INTT(BShareBUSD_A).burn(user,amount);
        uint balance = sellOf(amount);
        require(balance > minAmount, "BShareBUSD: Amount is less than the minAmount");
        IERC20(address(BShareBUSD)).safeTransfer(user,balance);
        exchangeRatio = (IERC20(address(BShareBUSD)).balanceOf(address(this)) * (10 ** 18))/(IERC20(BShareBUSD_A).totalSupply());
        return(balance);
    }
 /**
...
/**
    ...
    */
    function setTradeTax(uint _tradeTax) external onlyRole(DEFAULT_ADMIN_ROLE){
        tradeTax = _tradeTax;
    }
/**
    ...
    */
    function setDepositTax(uint _depositTax) external onlyRole(DEFAULT_ADMIN_ROLE){
