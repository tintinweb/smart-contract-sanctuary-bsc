    ...
    Ownable
{
    ...
    mapping(address => EnumerableSet.Bytes32Set) private openOrders; // Mapping of User's Open trade
    mapping(address => EnumerableSet.Bytes32Set) private closeOrders; // mapping of User's closed trade
    mapping(bytes32 => string) private orderStatus; // order status
    ...
    function _deleteOrder(bytes32 orderId) private {
        ...
        openOrders[orderBook[orderId].user].remove(orderId);
        closeOrders[orderBook[orderId].user].add(orderId);
    ...
    function _performSwap(
        ...
        if (tokenIn == WETH) {
            ...
            returns (uint256[] memory amounts) {
                orderStatus[orderId] = string(abi.encodePacked("Order Filled at ",amounts[path.length - 1]));
                orderBook[orderId].success = true;
                emit OrderFilled(orderId, amounts[path.length - 1]);
            } catch {
                _forceFail(amountIn, tokenIn, user);
                 orderStatus[orderId] = "Order Force Failed ";
