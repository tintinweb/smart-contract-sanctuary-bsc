...
contract QuadrixToken is BEP20('QuadrixToken', 'quadrix') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (QuadrixSwapRouter).
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), 'quadrix::delegateBySig: invalid signature');
        require(nonce == nonces[signatory]++, 'quadrix::delegateBySig: invalid nonce');
        require(now <= expiry, 'quadrix::delegateBySig: signature expired');
    ...
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, 'quadrix::getPriorVotes: not yet determined');
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying quadrixs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, 'quadrix::_writeCheckpoint: block number exceeds 32 bits');
...
contract QuadrixMaster is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of quadrixs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (userInfo.amount * pool.accquadrixPerShare) - userInfo.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accquadrixPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint; // How many allocation points assigned to this pool. quadrixs to distribute per block.
        uint256 lastRewardBlock; // Last block number that quadrixs distribution occurs.
        uint256 accquadrixPerShare; // Accumulated quadrixs per share, times 1e12. See below.
        bool exists; //
    }
    // quadrix tokens created first block.
    uint256 public quadrixStartBlock;
    // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public totalAllocPoint = 0;
    // The block number when quadrix mining starts.
    uint256 public startBlock;
    // Block number when bonus quadrix period ends.
    ...
    // quadrix tokens created at bonus end block.
    uint256 public quadrixBonusEndBlock;
    // max reward block
    ...
    // Accumulated quadrixs per share, times 1e12.
    uint256 public accquadrixPerShareMultiple = 1E12;
    // The quadrix TOKEN!
    QuadrixToken public quadrix;
    // Dev address.
    ...
    constructor(
        QuadrixToken _quadrix,
        address _devAddr,
        uint256 _quadrixStartBlock,
        uint256 _startBlock,
        ...
        quadrix = _quadrix;
        devAddr = _devAddr;
        quadrixStartBlock = _quadrixStartBlock;
        ...
        // quadrix created when bonus end first block.
        // (quadrixStartBlock - bonusBeforeCommonDifference * ((bonusEndBlock-startBlock)/bonusBeforeBulkBlockSize - 1)) * bonusBeforeBulkBlockSize*(bonusEndBulkBlockSize/bonusBeforeBulkBlockSize) * bonusEndBulkBlockSize
        quadrixBonusEndBlock = quadrixStartBlock
        ...
        // max mint block number, _quadrixInitBlock - (MAX-1)*_commonDifference = 0
        // MAX = startBlock + bonusEndBulkBlockSize * (_quadrixInitBlock/_commonDifference + 1)
        maxRewardBlockNumber = startBlock.add(
            bonusEndBulkBlockSize.mul(quadrixBonusEndBlock.div(bonusEndCommonDifference).add(1))
        );
    }
    ...
    function add(
        ...
        pool.accquadrixPerShare = 0;
        pool.exists = true;
        poolAddresses.push(_pair);
    }

    // Update the given pool's quadrix allocation point. Can only be called by the owner.
    function set(
        address _pair,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    function getTotalRewardInfoInSameCommonDifference(
        ...
        if (fromBulkNumber == currentBulkNumber) {
            return _to.sub(_from).mul(_quadrixInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
        }
        ...
        uint256 currentPreviousBulkLastBlock = startBlock.add(_bulkBlockSize.mul(currentBulkNumber.sub(1)));
        {
            ...
            .mul(_quadrixInitBlock.sub(currentBulkNumber.sub(1).mul(_commonDifference)));
            if (lastRewardBulkLastBlock > tempFrom && lastRewardBulkLastBlock <= tempTo) {
                totalReward = totalReward.add(
                    lastRewardBulkLastBlock.sub(tempFrom).mul(
                        _quadrixInitBlock.sub(fromBulkNumber > 0 ? fromBulkNumber.sub(1).mul(_commonDifference) : 0)
                    )
                );
            }
        }
        {
            ...
            uint256 tempquadrixInitBlock = _quadrixInitBlock;
            ...
            if (currentPreviousBulkLastBlock > lastRewardBulkLastBlock) {
                ...
                if (N > 1) {
                    ...
                    totalReward = totalReward.add(
                        tempBulkBlockSize.mul(tempquadrixInitBlock.sub(currentBulkNumber.sub(2).mul(tempCommonDifference)))
                    );
                }
            }
        }
    ...
    // View function to see pending quadrixs on frontend.
    function pendingquadrix(address _pair, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfoMap[_pair];
        if (!pool.exists) {
            return 0;
        }
        UserInfo storage userInfo = poolUserInfoMap[_pair][_user];
        uint256 accquadrixPerShare = pool.accquadrixPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && pool.lastRewardBlock < maxRewardBlockNumber) {
            ...
            uint256 quadrixReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
            accquadrixPerShare = accquadrixPerShare.add(quadrixReward.mul(accquadrixPerShareMultiple).div(lpSupply));
        }
        return userInfo.amount.mul(accquadrixPerShare).div(accquadrixPerShareMultiple).sub(userInfo.rewardDebt);
    }
    ...
    function updatePool(address _pair) public {
        ...
        uint256 totalReward = getTotalRewardInfo(pool.lastRewardBlock, block.number);
        uint256 quadrixReward = totalReward.mul(pool.allocPoint).div(totalAllocPoint);
        quadrix.mintTo(devAddr, quadrixReward.div(100));
        quadrix.mintTo(address(this), quadrixReward);
        pool.accquadrixPerShare = pool.accquadrixPerShare.add(quadrixReward.mul(accquadrixPerShareMultiple).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to QuadrixMaster for quadrix allocation.
    function deposit(address _pair, uint256 _amount) public {
        PoolInfo storage pool = poolInfoMap[_pair];
        UserInfo storage userInfo = poolUserInfoMap[_pair][msg.sender];
        updatePool(_pair);
        if (userInfo.amount > 0) {
            uint256 pending = userInfo.amount.mul(pool.accquadrixPerShare).div(accquadrixPerShareMultiple).sub(
                userInfo.rewardDebt
            );
            if (pending > 0) {
                safequadrixTransfer(msg.sender, pending);
            }
        }
        IBEP20(_pair).safeTransferFrom(address(msg.sender), address(this), _amount);
        userInfo.amount = userInfo.amount.add(_amount);
        userInfo.rewardDebt = userInfo.amount.mul(pool.accquadrixPerShare).div(accquadrixPerShareMultiple);
        emit Deposit(msg.sender, _pair, _amount);
    }

    // Withdraw LP tokens from QuadrixMaster.
    ...
    // Safe quadrix transfer function, just in case if rounding error causes pool to not have enough quadrixs.
    function safequadrixTransfer(address _to, uint256 _amount) internal {
        uint256 quadrixBal = quadrix.balanceOf(address(this));
        if (_amount > quadrixBal) {
            quadrix.transfer(_to, quadrixBal);
        } else {
            quadrix.transfer(_to, _amount);
        }
    }
