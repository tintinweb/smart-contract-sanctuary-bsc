...
contract EAGLENFT is ERC721 {
    ...
    constructor(address _MarketingWallet, address _usdt)
        ...
        returns (string memory)
    {
        ...
        string memory creationTime = string(
            abi.encodePacked(NFTConfig[tokenId].creationTime)
        );
        bool sale = NFTConfig[tokenId].onSale;
    ...
    function sellNFT(uint256 _tokenId, uint256 _price) external {
        ...
        require(
            ...
            "This NFT cannot be sold. Please get the token first"
        );
        require(
            block.timestamp - NFTConfig[_tokenId].creationTime > 30 minutes,
            "Your NFT casting has not lasted more than a month!!!"
        );
        uint256 receiveNum = (block.timestamp -
            NFTConfig[_tokenId].creationTime) / 30 minutes;
        require(
        ...
        returns (string memory)
    {
        ...
        string memory description = string(
            abi.encodePacked(
                "This NFT is ",
                cardName,
                ", Buy NFT ,You can receive EAGLE token rewards every month!!!",
                describe
            )
        );
        string memory image = string(abi.encodePacked(imageURL[serial]));
        string memory price = string(
            abi.encodePacked(orderNFTLists[orderID].price)
        );
        string memory NFTListingTime = string(
            abi.encodePacked(orderNFTLists[orderID].NFTListingTime)
        );
        string memory lowerShelf = string(
            abi.encodePacked(
                "Is this picture for sale",
                orderNFTLists[orderID].lowerShelf
            )
        );
        string memory json = Base64.encode(
            bytes(
    ...
    // @getOrderIds query orderId nftid order is not lowerShelf
    function getOrderIds()
        ...
        returns (
            ...
            bool[] memory orderLowerShelfs
        )
    {
        orderNum = orderNFTLists.length;
        uint256 Id = 0;

        for (uint i = 0; i < orderNum; i++) {
            NFTIds[Id] = orderNFTLists[i].tokenId;
            serials[Id] = getNFTEAGLESerial(NFTIds[Id]);
            orderLowerShelfs[Id] = orderNFTLists[i].lowerShelf;
            Id++;
        }
        return (orderNum, NFTIds, serials, orderLowerShelfs);
    }
    ...
    // @getUserOwnerNFTIds Query your owner NFT IDs
    function getUserOwnerNFTIds()
        public
        view
        returns (uint256[] memory NFTIds, bool[] memory sale)
    {
    ...
    {
        uint256 nowTotal = _currentSupply;
        address[] memory tigerEaglecardHolders;
        address[] memory PhoenixEagleCardHolders;
        uint256 numA = 0;
        uint256 numB = 0;
        for (uint256 i = 0; i < nowTotal; i++) {
            if (NFTConfig[i].EAGLESerial == 0) {
                tigerEaglecardHolders[numA] = ownerOf(i);
                numA++;
            } else {
                PhoenixEagleCardHolders[numB] = ownerOf(i);
                numB++;
            }
        }
        return (tigerEaglecardHolders, PhoenixEagleCardHolders);
    }
}
