...
contract Cryptosyno is Context, IBEP20, Ownable, ReentrancyGuard {
    ...
    function setmaxTransferingAmount(uint256 _percentage) public onlyOwner {
      require(_percentage > 0 && _percentage <= 100, "BEP20: percentage must be less than 100");
    ...
    function _takeAllFee(uint256 tAmount, uint256 currentRate) internal {
        ...
        if(tFeeslot > 0){
            _accumulatedslotMachine = _accumulatedslotMachine.add(tAmount.mul(_currentslotMachineFee).div(percentDivider));
        ...
        if(tFeeburn > 0){
            _accumulatedburn = _accumulatedburn.add(tAmount.mul(_currentburnFee).div(percentDivider));
    ...
    function _reflectFee(uint256 tAmount) private {
        uint256 tFee = tAmount.mul(_currentRedistributionFee).div(percentDivider);
