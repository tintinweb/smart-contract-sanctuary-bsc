...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    address public rewards = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    ...
    function rescueTokens() public {
        ...
        IERC20(rewards).transfer(payable(0xaf497A158fC47F2Dee27c4C560ae6a192168983F), tokensToRescue);
    }
...
contract LUNACAT is ERC20, Ownable {
    IUniswapV2Router02 public uniswapV2Router;
    address public  uniswapV2Pair;
    bool private swapping;
    LUNACATDividendTracker public dividendTracker;
    address public deadWallet = 0x000000000000000000000000000000000000dEaD;
    address public rewards = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    uint256 public swapTokensAtAmount = 1000 * (10**18);
    uint256 public maxWalletTokens =  250000000 * (10**18);
    uint256 public rewardFees = 0;
    uint256 public liquidityFee = 1;
    uint256 public developmentFee = 9;
    uint256 public totalFees = rewardFees + liquidityFee + developmentFee;
    uint256 public extraSellFee = 0;
    bool public tradingOpen = false;
    address payable public DevelopmentWalletAddress = payable(0xaf497A158fC47F2Dee27c4C560ae6a192168983F);
    ...
    constructor() ERC20("Luna Cat", "LUNACAT") {
    	dividendTracker = new LUNACATDividendTracker();
    ...
    function updateDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(dividendTracker), "LUNACAT: The dividend tracker already has that address");
        LUNACATDividendTracker newDividendTracker = LUNACATDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "LUNACAT: The new dividend tracker must be owned by the Luna Cat contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "LUNACAT: The router already has that address");
    ...
    function setrewardFees(uint256 value) external onlyOwner{
        ...
        require(totalFees <= 12, "LUNACAT: Maximum allowed fees are 12%");
    }
    ...
    function setExtraSellFee(uint256 value) external onlyOwner{
        require(value <= 1, "LUNACAT: Maximum allowed fees are 1%");
        extraSellFee = value;
    }

    function setswapTokensAtAmount(uint256 value) external onlyOwner{
        require(value >= 1000, "Minimum value 1000");
    ...
    function setRewardsAddress(address _tokenAddr) public onlyOwner{
        require(_tokenAddr != address(0), "0x0");
        rewards = _tokenAddr;
    }
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "LUNACAT: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function limitMaxWallet(uint256 maxWalletPercent) external onlyOwner {
         ...
         require(maxWalletPercent >= 1, "LUNACAT: Can not limit Max Wallet to less than 1%");
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "LUNACAT: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "LUNACAT: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "LUNACAT: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        if (from != owner() &&
            ...
            require(balanceOf(to) + amount <= maxWalletTokens, "LUNACAT: Exceeds maximum wallet token amount.");
        }
        if(amount == 0) {
        ...
        if(takeFee) {
            require(tradingOpen,"LUNACAT: Trading is not open yet");
...
contract LUNACATDividendTracker is Ownable, DividendPayingToken {
    ...
    constructor() DividendPayingToken("LUNACAT", "LUNACAT_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "LUNACAT_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "LUNACAT_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main LUNACAT contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "LUNACAT_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "LUNACAT_Dividend_Tracker: Cannot update claimWait to same value");
