...
contract TestabdexMasterChef is Ownable {
    ...
    function addPool(
        ...
        if (_isPair) {
            ...
            uint256 points = (IBEP20(_token0) == weightsToken) ? (uint256(_reserveA) / _totalSupply) : (uint256(_reserveB) / _totalSupply);
            _info.pointsPerShare = points;
        } else {
            require(IBEP20(_lpToken) == weightsToken, 'CANOT_ADD_INTO_POOL');
            _info.pointsPerShare = 1;
        }
    ...
    function massUpdatePools() public {
        ...
        for (uint256 pid = 0; pid < poolLength; pid++) { 
            ...
            uint256 _rewardPerBlock = rewardPerBlock * pool.pointsPerShare * pool.lpSupply / totalPoints;  
    ...
    function updatePoints(uint256 pid, uint256 amountIn, uint256 amountOut) internal {
        PoolInfo memory pool = poolInfo[pid];
        pool.lpSupply =  pool.lpSupply + amountIn - amountOut;
        uint256 pointsBefore = totalPoints;
        totalPoints = totalPoints + amountIn * pool.pointsPerShare - amountOut * pool.pointsPerShare;
        poolInfo[pid] = pool;
        if (totalPoints != pointsBefore) { massUpdatePools(); }
    }
    ...
    function updateLPWeights(address lp, address tokenA, address tokenB, uint256 priceA, uint256 priceB) external lock {
        require(msg.sender == CHEF_FACTORY, "Not factory");
        if (tokenA != address(weightsToken) && tokenB != address(weightsToken)) { return; }
        uint256 pid = lpPoolID[lp];
        PoolInfo memory pool = poolInfo[pid];
        pool.pointsPerShare = (tokenA == address(weightsToken)) ? priceA : priceB;
        poolInfo[pid] = pool;
        updatePoints(pid, 0, 0);
    } 
}
