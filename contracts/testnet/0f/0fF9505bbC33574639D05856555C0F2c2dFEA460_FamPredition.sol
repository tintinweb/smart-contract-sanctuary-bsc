...
contract FamPredition is Ownable {
    ...
    mapping(address => address) public club;
    ...
    function betBatch(
        ...
        uint256[2][] calldata infos // uint256[] memory betNumbers_, // uint256[] memory betAmounts_
    ) public {
        require(meta.isOpen, "not open");
        require(statue == Statue.OPEN, "bet is not open");
        ...
        // BetStatus memory uBetStatus = userBetStatus[epoch_][msg.sender];
    ...
    function bet(
        ...
        if (position_ == BetType.FIVE) {
            require(betAmount_ >= 111 && betAmount_ <= 144, "mode error");
        }

        // BetInfo memory uBetInfo = userBets[epoch_][msg.sende][index_];
        userBets[epoch_][msg.sender].push(BetInfo({ position: position_, betAmount: betAmount_, betNumber: betNumber_ }));
    }
    ...
    function _takeInviterReward(uint256 amount_, uint256 totalRefReward_) internal {
        ...
        for (uint8 i; i < 5; i++) {
            ...
            if (cludAddr == address(0) && club[inviter] != address(0)) {
                cludAddr = club[inviter];
            }
    ...
    function claimBetReward(uint256 epoch_) public {
    ...
    // 获取n轮次的幸运数字
    function getRoundResult(uint256[] calldata epochs_) external view returns (uint256[] memory numbers) {
    ...
    function coutingUserBetReward(
        ...
        if (isWin) {
            reward = uBetInfo.betAmount * winRate[uBetInfo.position];
        }
    }
    ...
    function getUserRounds(
        ...
        for (uint256 i = 0; i < length; i++) {
            values[i] = userRounds[account_][cursor_ + i];
        }

        return (values, cursor_ + length);
    }
    ...
    function requestRandomWords() internal {
        ...
        rounds[meta.lastRoundId].requestId = requestId;
        reqIdToRoundId[requestId] = meta.lastRoundId;
        // requestIds[meta.lastRoundId] = requestId;
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) public {
        ...
        Round storage round = rounds[meta.lastRoundId];
        uint256 luckNumber = randomWords[0] % 10000;
        round.luckNumber = luckNumber;

        emit ReturnedRandomness(requestId, meta.lastRoundId, luckNumber, randomWords[0]);

        // 开启新一轮
        meta.lastRoundId++;
        rounds[meta.lastRoundId].startTime = block.timestamp;
    }
    ...
    {
        Round memory round = rounds[meta.lastRoundId];
        bool isOpen = (meta.isOpen && Statue.OPEN == statue);
        bool timePassed = (round.startTime > 0 && (block.timestamp - round.startTime) > meta.interval);

        upkeepNeeded = isOpen && round.start && timePassed;
    }
    ...
    function isMatchModeTwo(uint256 userNumber_, uint256 luckNumber_) public pure returns (bool) {
        ...
        uint256 userAdd;
        for (uint256 i; i < 3; i++) {
            userAdd += userNumber_ % 10;
            roundAdd += luckNumber_ % 10;

            userNumber_ /= 10;
            luckNumber_ /= 10;
        }
        return roundAdd == userAdd;
    }
