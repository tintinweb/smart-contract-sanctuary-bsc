...
contract TicketSystemCD is Ownable, ReentrancyGuard, Pausable {
    ...
    //============== MAPPINGS ==============
    mapping(address => uint256) public ticketBought;
        ...
        nonReentrant
    {
        ...
        ticketBought[_msgSender()] += numOfTicket;
        ...
        nonReentrant
    {
        ...
        if(lastWithdrawalTime[recipient] != 0){
        require(
            lastWithdrawalTime[recipient] + 24 hours <= block.timestamp,
            "Withdrawal is only allowed once every 24 hours"
        );
        }
        uint256 latestPrice = 1* 10**18;
        require(latestPrice > 0, "Price feed error");
        uint256 GQValue = amount * latestPrice; // calculate ERC20 value of the ticketAmount
        require(GQValue <= 500 * 10**18, "Withdrawal amount exceeds $500 equivalent in ERC20 tokens");
