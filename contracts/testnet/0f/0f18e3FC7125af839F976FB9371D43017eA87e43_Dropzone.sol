...
contract Dropzone is Initializable,Ownable {
    ...
    function applyforAirdrop(address _token,uint _amount,uint _from ,uint _to ,string[] calldata _socialMedia ,uint[] calldata amount) external  {
    ...
    function getClaimableAmount(bytes32 _hash) internal view returns(uint claimAmt){
        for(uint8 i=0;i<airdrops[_hash]._socialMediaAmount.length;i++) {
            claimAmt+=airdrops[_hash]._socialMediaAmount[i];
        }
    }
