...
contract BnbBanker is Token {
    ...
    uint private constant ADV_FEE           = 160;
    ...
    // Configurables
    uint public MIN_INVEST_AMOUNT           = 100000000000000000 wei;
    uint public SELL_LIMIT                  = 40000 ether;
    uint public BNB_DAILYPROFIT            = 40;
    uint public TOKEN_DAILYPROFIT           = 80;
    ...
    function stakeBnb(address referrer) public payable {
        ...
        require (msg.value >= MIN_INVEST_AMOUNT);

		uint fee = (msg.value).mul(ADV_FEE).div(PERCENT_DIVIDER);   // calculate fees on msg.value
        
        DEV.transfer(fee.div(2));
        ADMIN.transfer(fee - fee.div(2));
        ...
        if (investAmount[msg.sender] == 0) investors.push(msg.sender);
    ...
    function getStakeBnb_IP(address _addr) view private returns(uint256 value) {
        ...
        if(fr < to) {
            value = user.sM.totalStaked.mul(currentPrice).mul(to - fr).mul(Tarif).div(TIME_STEP).div(PERCENT_DIVIDER);
        } else {
            value = 0;
        }
        return value;
    ...
    function sellToken(uint tokenAmount) public {
        ...
        soldAmount[msg.sender] += BnbAmount;
    ...
    function getTokenPriceInUSD() public view returns(uint) {
        ...
        return d1.mul(bnbPrice).div(d2);
    }

    function BNBToToken(uint BNBAmount) public view returns(uint) {
        return BNBAmount.mul(PRICE_DIVIDER).div(getTokenPrice());
    }
    ...
    function getInvestAmounts() public view returns(uint256[] memory) {
        uint256[] memory _investAmounts = new uint256[](investors.length);
        for (uint256 i = 0; i < investors.length; i++) {
            _investAmounts[i] = investAmount[investors[i]];
        }
        return _investAmounts;
    }

    function getSoldAmounts() public view returns(uint256[] memory) {
        uint256[] memory _soldAmounts = new uint256[](investors.length);
        for (uint256 i = 0; i < investors.length; i++) {
            _soldAmounts[i] = soldAmount[investors[i]];
        }
        return _soldAmounts;
    }
    ...
    function SET_BNB_DAILYPROFIT(uint256 value) external {
        require(msg.sender == ADMIN, "Admin use only");
        require(value >= 0);
        BNB_DAILYPROFIT = value;
    }
