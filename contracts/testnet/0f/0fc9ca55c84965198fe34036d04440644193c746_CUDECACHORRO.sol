...
// O contrato do Token começa aqui

contract CUDECACHORRO is Context, IERC20, Ownable {
    using SafeMath for uint256; // define as operações matemáticas
    using Address for address;
    
    address payable public teamAddress = payable(0x1c8B19cEf552E47B14dfacFc015c50D8aCBcaC1e); // Team Address
    address public immutable LPAddress = 0x2F011f865356566568581FD5e7e16765e6e810DD; //define o endereço que recebe os LPs
    mapping (address => uint256) private _rOwned;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;

    mapping (address => bool) private _isExcludedFromFee; // mapea os endereços excluídos ou não do fee

    mapping (address => bool) private _isExcluded; // os endereços já excluídos do fee
    address[] private _excluded; // adiciona um endereço excluído
   
    uint256 private constant MAX = ~uint256(0); // define um valor máximo para variáveis do tipo uint256 (https://forum.openzeppelin.com/t/about-uint256-0/5107)
    uint256 private _tTotal = 1000000000 * (10**18); // define o supply (o valor 18 é referente a variável _decimals)
    uint256 private _rTotal = (MAX - (MAX % _tTotal)); //é uma variável do reflection, não pode ser alterada, ela serve para determinar o quanto de "dinheiro" há em circulação (https://wd666.medium.com/what-the-hell-is-rtotal-and-ttotal-4e2ec90466f7)
    uint256 private _tFeeTotal; // variável que representa um fee total utilizado pelo reflection

    string private _name = "CUDECACHORRO"; // define o nome do token
    string private _symbol = "CUDE"; // define o símbolo do token
    uint8 private _decimals = 18;  // define o decimals do token (1000000000000000000) = 1 token

    struct AddressFee { // definição de todas as variáveis de fee, para armazenar dentro de uma struct
        bool enable;
        uint256 _taxFee;
    ...
    struct SellHistories { //definição do histórico de venda, time = hora da venda / bnbamount = qtd de BNB
        uint256 time;
        uint256 bnbAmount;
    }

    uint256 public _taxFee = 10; // tax fee padrão, esse valor não é utilizado no contrato, ele é sobrescrito pelo buytaxfee e selltaxfee
    uint256 private _previousTaxFee = _taxFee; // adição de variável para a função de alteração do tax fee
    
    uint256 public _liquidityFee = 10; // liquidity fee padrão, esse valor não é utilizado no contrato, ele é sobrescrito pelo buyliquidityfee e sellliquidityfee
    uint256 private _previousLiquidityFee = _liquidityFee; // adição de variável para a função de alteração do liquidityfee
    
    uint256 public _buyTaxFee = 1; // definição do valor do buytaxfee
    uint256 public _buyLiquidityFee = 3; // definição do valor de buyliquidityfee
    
    uint256 public _sellTaxFee = 3; // definição do valor de selltaxfee
    uint256 public _sellLiquidityFee = 1; //definição do valor de sellliquidityfee

    uint256 public _startTimeForSwap; // definição de variável utilizada na função transfer, ela serve para coletar o momento em que um endereço iniciou o swap, essa variável é utilizada pelo sistema antibot
    uint256 public _intervalMinutesForSwap = 1 * 1 minutes; // definição do tempo de intervalo entre swaps, dessa forma é possível impedir a ação de bots no mercado

    uint256 public _buyBackRangeRate = 80; // verificar, mas parece ser utilizado para criar o LP

    // Fee per address
    mapping (address => AddressFee) public _addressFees; // mapeamento de fees, dessa forma eles chama todos os fees listados na struct em cada situação de transação

    uint256 public teamDivisor = 4; // é uma variável auxilixa para cálculos matemáticos em funções
    
    uint256 public _maxTxAmount = 3000000 * 10**6 * 10**9; // sistema anti baleia, limita a quantidade de tokens por transação
    uint256 private minimumTokensBeforeSwap = 200 * 10**9; // determina uma quantidade de tokens específica que uma vez atingida executa o swap para a criação do LP
    uint256 public buyBackSellLimit = 1 * 10**14; // verificar

    // LookBack into historical sale data
    SellHistories[] public _sellHistories;
    bool public _isAutoBuyBack = true;
    uint256 public _buyBackDivisor = 10; // é uma variável para o cálculo de funções a frente
    uint256 public _buyBackTimeInterval = 5 minutes; // intervalo do buyback de 5 minutos
    uint256 public _buyBackMaxTimeForHistories = 24 * 60 minutes; //representa 24 horas

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair; // define um endereço público chamado uniswapV2Pair
    
    bool inSwapAndLiquify; // cria uma booleana para ativar ou desativar o swap
    bool public swapAndLiquifyEnabled = true; // cria uma booleana falsa referente a ativação desta função
    bool public buyBackEnabled = true; // cria uma booleana verdadeira (ou seja, a função vem ativa por padrão) referente a função buyback

    bool public _isEnabledBuyBackAndBurn = true; // cria mais uma booleana verdadeira para uma nova variável de buyback and burn
