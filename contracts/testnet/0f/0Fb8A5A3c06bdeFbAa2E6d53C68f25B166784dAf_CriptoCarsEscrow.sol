...
contract CriptoCarsEscrow is Initializable, ReentrancyGuardUpgradeable  {
   ...
   function getState(uint _orderId) public view returns (EscrowStatus){
    ...
    function createEscrow(uint _orderId, address payable _buyer, address payable _seller, uint _value, IERC20 _currency) external nonReentrant {
        ...
        escrows[_orderId] = Escrow(_buyer, _seller, _value, sellerfee, buyerfee,  _currency, EscrowStatus.Funded);
    ...
    function releaseEscrow(uint _orderId) external onlyBuyer(_orderId) nonReentrant{            
        ...
        uint256 _AmountFeeSeller =  escrows[_orderId].value * (sellerfee/100);
