...
contract TSwap is Ownable {
    ...
    IPancakeswapRouter public pancakeSwap = IPancakeswapRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    ...
    function addLiquidity(
        ...
        IBEP20(tokenA).approve(address(pancakeSwap), amountADesired);
        IBEP20(tokenB).approve(address(pancakeSwap), amountBDesired);
        ...
        (amountA, amountB, liquidity) = pancakeSwap.addLiquidity(tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin, to, block.timestamp + 5 minutes);
    }
    ...
    function removeLiquidity(
        ...
        uint amountBMin,
        address to
    ) public virtual onlyOwner returns (uint amountA, uint amountB) {
        (amountA, amountB) = pancakeSwap.removeLiquidity(tokenA, tokenB, liquidity, amountAMin, amountBMin, to, block.timestamp + 5 minutes);
    }
    ...
    function swapExactTokensForTokens(
        ...
        if (isTeamToken(path[0])){
            ...
            ( amounts ) = pancakeSwap.swapExactTokensForTokens(amountIn.mul(100 - totalFee).div(100), amountOutMin, path, to, block.timestamp + 5 minutes);
            ...
            (amountToken) = pancakeSwap.swapExactTokensForTokens(variables.liquidityAmount.div(2), amountOutMin, path, to, block.timestamp + 5 minutes);
