...
contract BoldCryptoLock is IBoldCryptoLock {
    ...
    //percentages are entered, stored, and calculated as multiples of 1000
    ...
    function _getBatchReleaseAmount(
        ...
        uint256 numOfCycles = (A_HUNDRED_PCT - userLock.tgeBps) /
            userLock.cycleBps;
        uint256 cycleDuration = userLock.vestingDuration / numOfCycles;
        if (cycleDuration == 0) {
            //To mean cycleDuration is smaller than 1 second, fallback to _getLinearReleaseAmount
            return
                _getLinearReleaseAmount(
                    userLock,
                    tgeReleaseAmount,
