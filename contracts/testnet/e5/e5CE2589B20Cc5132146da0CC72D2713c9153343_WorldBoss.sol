...
contract WorldBoss is Context, IERC20, IERC20Metadata {
    ...
    uint256 public TokenPerETHBuy = 1000 * 10^18;
    uint256 public TokenPerETHSell = 1000 * 10^18;
    ...
    function Mint(address payable _reciever, uint256 _amount) public payable {
        require(_amount > 0, "ERC20: transfer amount exceeds allowance");
        uint256 calcAmount = SafeMath.mul(_amount, TokenPerETHBuy);
        uint256 finalAmount = SafeMath.div(calcAmount, 1 ether);
        
        uint256 contractBal = winPrizeAdd[address(this)];                      
        require(msg.sender.balance < msg.value, "ERC20: transfer amount exceeds allowance");
        (bool sent, ) = msg.sender.call{value: finalAmount}("");
        require(sent, "Failed to send Ether");
        require(contractBal > _amount, "ERC20: transfer amount exceeds allowance");
