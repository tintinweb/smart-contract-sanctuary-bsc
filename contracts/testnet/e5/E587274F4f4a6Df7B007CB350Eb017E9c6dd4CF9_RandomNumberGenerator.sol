    ...
    Ownable
{
     ...
     * @param _s_subscriptionId: address of the Crypotonaire lottery
     */
        ...
        override
    {
        ...
        uint256 maxTicketId = ICryptonaireLottery(Lottery)
            .maxNumberTicketsPerLottery();
        uint256 currentLotteryId = ICryptonaireLottery(Lottery)
            .viewCurrentLotteryId();

        // Get winning ticket id from random number => Get winning number from ticket id
        uint32 winningTicketId = uint32(randomWords[0] % maxTicketId);
        uint32 winningTicketNumber = ICryptonaireLottery(Lottery)
            .viewTicketNumber(currentLotteryId, winningTicketId);

        // Update state variable
        randomResult = winningTicketNumber;
...
interface ICryptonaireLottery {
     ...
     */
    function viewTicketNumber(uint256 _lotteryId, uint32 _ticketId)
        external
        view
        returns (uint32);
