...
contract staking is Ownable {
    ...
    function withdrawRewardBUSD()public {
        ...
        if(investor[msg.sender].refferAddress!=address(0)) token.transfer(investor[msg.sender].refferAddress, taxR);
        else token.transfer(address(this), taxR);
        for(uint256 i=0;i<investor[msg.sender].deposites.length;i++){
    ...
    function withdrawRewardBNB()public {
        ...
        if(investor[msg.sender].refferAddress!=address(0)) payable(investor[msg.sender].refferAddress).transfer(taxR);
        else payable(address(this)).transfer(taxR);
