...
contract HYPresale is Ownable {
    ...
    function buy(uint256 amount) external {
        ...
        if(0 == userInfo.claimTime){
            userInfo.claimTime = block.timestamp+claimPerid;
        }
    }
    ...
    function claimToken() external {
        ...
        address account = msg.sender;
        UserInfo storage userInfo = _userInfo[account];
        require(block.timestamp> userInfo.claimTime, "error time");
    ...
    function getPendingToken(address account) public view returns (uint256){
        UserInfo storage userInfo = _userInfo[account];
        uint256 leftclaimNum = 6 - userInfo.claimTime;
