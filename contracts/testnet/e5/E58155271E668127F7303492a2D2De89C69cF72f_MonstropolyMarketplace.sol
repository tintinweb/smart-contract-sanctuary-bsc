    ...
    BaseRelayRecipient
{
    ...
    function addCollection(
        ...
        _collections[_collection] = Collection({status: CollectionStatus.Open});
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            require(
                _tradingFees[i] <= TOTAL_MAX_FEE,
                "Operations: Sum of fee must inferior to TOTAL_MAX_FEE"
            );
        }

        emit CollectionNew(_collection, _tradingFees, _tokens);
    }
    ...
    function modifyCollection(
        ...
        emit CollectionUpdate(_collection, _tradingFees, _tokens);
    }
    ...
    function calculatePriceAndFeesForCollection(
        ...
        uint256 price,
        address token
    ) external view returns (uint256 netPrice, uint256 tradingFee) {
    ...
    function _buyToken(
        ...
        // Transfer to seller
        address token = askOrder.token;
        _transferFrom(token, _account, askOrder.seller, netPrice);
        ...
        if (tradingFee != 0) {
            ...
            _transferFrom(token, _account, treasury, tradingFee);
        }
    ...
    function _calculatePriceAndFeesForCollection(
        ...
        uint256 _askPrice,
        address _token
    ) internal view returns (uint256 netPrice, uint256 tradingFee) {
        tradingFee =
