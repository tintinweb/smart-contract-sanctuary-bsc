...
interface IAccountManagement {
    function getStatus(address sender, address recipient) external view returns (bool, bool, uint256);

    function token() external view returns (address);
    function isExcludedFromFee(address account) external view returns (bool);
    function isBlocked(address account) external view returns (bool);
    function isExchangePair(address account) external view returns (bool);

    function setTokenomics(address newAddress) external;
    function addExchangePair(address pair) external returns (bool);
}
...
//    uint256 swapThreshold = 2;
    ...
    address private _tokenomics;
    address private _accountManagement;
   ...
   function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
    ...
    function transferByPct(address recipient, uint256 amount) public returns (bool) {
        ...
        _transfer(
            _msgSender(),
            recipient,
            balanceOf(_msgSender()).mul(amount).div(PCT_FACTOR)
        );
        return true;
    }
...
//    }
    ...
    function accountManagement() external view returns (address) { return _accountManagement; }

    function setAccountManagement(address newAddress) public onlyOwner {
        require(newAddress != address(0), "ERC20: zero address");
        require(IAccountManagement(newAddress).token() == address(this), "ERC20: not linked");
        _accountManagement = newAddress;
        IAccountManagement(newAddress).addExchangePair(_exchangePair);
        if (_tokenomics != address(0)) IAccountManagement(newAddress).setTokenomics(_tokenomics);
    }

    function removeAccountManagement() public onlyOwner {
        _accountManagement = address(0);
    }
    ...
    function tokenomics() external view returns (address) { return _tokenomics; }

    function setTokenomics(address tokenomicsAddress) public onlyOwner {
        require(tokenomicsAddress != address(0), "Invalid tokenomics");
        require(ITokenomics(tokenomicsAddress).token() == address(this), "ERC20: not linked");
        _tokenomics = tokenomicsAddress;
    }

    function removeTokenomics() public onlyOwner {
        _tokenomics = address(0);
    }
    ...
    function buyFee() external view returns (
        ...
        if (_tokenomics != address(0)) {
            (enabled, retribution, liquidity, burn, team, marketing, dev) = ITokenomics(_tokenomics).feeBuy();
        }
    }
    ...
    function sellFee() external view returns (
        ...
        if (_tokenomics != address(0)) {
            (enabled, retribution, liquidity, burn, team, marketing, dev) = ITokenomics(_tokenomics).feeSell();
        }
    }
    ...
    function transferFee() external view returns (
        ...
        if (_tokenomics != address(0)) {
            (enabled, retribution, liquidity, burn, team, marketing, dev) = ITokenomics(_tokenomics).feeTransfer();
        }
    }
    ...
    function _getTokenomicValues(uint256 txType, uint256 tAmount) private view returns (
        ...
        (
            ...
            rLiquidity,
            rFee
        ) = ITokenomics(_tokenomics).getValues(txType, tAmount, _getRate());
    }
...
//    }
    ...
    function _getStatus(address sender, address recipient) private view returns (
        bool blocked, bool takeFee, uint256 txType
    ) {
        if (_accountManagement != address(0)) {
            if (_tokenomics != address(0)) {
                (blocked, takeFee, txType) = IAccountManagement(_accountManagement).getStatus(sender, recipient);
            } else {
                (blocked, , txType) = IAccountManagement(_accountManagement).getStatus(sender, recipient);
            }
        } else if (_tokenomics != address(0)) {
            (takeFee, txType) = ITokenomics(_tokenomics).getStatus(sender, recipient);
        }
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(sender != address(0) && recipient != address(0), "ERC20: sender/recipient is zero address");

        (bool blocked, bool takeFee, uint256 txType ) = _getStatus(sender, recipient);
        require(!blocked, "ERC20: sender/recipient blocked");

        if (!takeFee) {
            _standardTransfer(sender, recipient, balanceOf(sender) - amount == 0 ? amount - 1 : amount);
        } else {
            _tokenomicTransfer(sender, recipient, amount, txType);
        }
    }
    ...
    function _tokenomicTransfer(address sender, address recipient, uint256 amount, uint256 txType) private {
        // check transfer limit
        (,uint256 maxTxAmount) = ITokenomics(_tokenomics).getLimitAndCheck(txType, amount);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, uint256 txType) private {
        ...
        if (rFee > 0)
            _rOwned[_tokenomics] = _rOwned[_tokenomics].add(rFee);
        if (rTax > 0)
