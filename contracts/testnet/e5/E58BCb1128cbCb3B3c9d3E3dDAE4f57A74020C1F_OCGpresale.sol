...
contract OCGpresale is Context, IERC20, Ownable {
    ...
    string private _name = "Ocean Guardian Token";
    ...
    address  public marketingAddress = 0x0d7eF14563D44687355a293cacce92aED4e9d439;
    address  public partnerAddress = 0x0d7eF14563D44687355a293cacce92aED4e9d439;
    ...
    function _takefunding(uint256 tfunding) private {
        uint256 currentRate =  _getRate();
        uint256 rfunding = tfunding.mul(currentRate);
        _rOwned[marketingAddress] = _rOwned[marketingAddress].add(rfunding.div(100).mul(5));
        _rOwned[partnerAddress] = _rOwned[partnerAddress].add(rfunding.div(100).mul(3));

        if(_isExcludedFromReward[marketingAddress])
            _tOwned[marketingAddress] = _tOwned[marketingAddress].add(tfunding.div(100).mul(5));

        if(_isExcludedFromReward[partnerAddress])
            _tOwned[partnerAddress] = _tOwned[partnerAddress].add(tfunding.div(100).mul(3));
    }
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        ...
        if (tfunding > 0) emit Transfer(sender, marketingAddress, (tfunding.div(100).mul(5)));
        if (tfunding > 0) emit Transfer(sender, partnerAddress, (tfunding.div(100).mul(3)));
    }
