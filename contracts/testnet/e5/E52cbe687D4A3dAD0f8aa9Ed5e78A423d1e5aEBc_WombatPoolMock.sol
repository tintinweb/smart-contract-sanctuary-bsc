...
contract WombatPoolMock {
    ...
    function withdraw(
        ...
        IWombatAssetMock lpAsset = IWombatAssetMock(depositTokenToLp[token]);
    ...
    function quotePotentialSwap(
        ...
        uint256 out = (toBalance * uint256(fromAmount) )/ fromBalance;
        if(out > toBalance) {
            potentialOutcome = toBalance;
        } else {
            potentialOutcome = out;
        }
        haircut = 0;
    }
    ...
    function swap(
        ...
        (uint256 potentialOutcome, uint256 haircut) = quotePotentialSwap(fromToken,toToken, int256(fromAmount));

        uint256 toBalance = IERC20(toToken).balanceOf(address(this));
        require(potentialOutcome <= toBalance, "Not enough token!");
        actualToAmount = potentialOutcome;
        require(actualToAmount >= minimumToAmount, "Minimum!");
        haircut = 0;
        IERC20(fromToken).safeTransferFrom(address(msg.sender), address(this), fromAmount);
        IERC20(toToken).safeTransfer(to, actualToAmount);
    }
