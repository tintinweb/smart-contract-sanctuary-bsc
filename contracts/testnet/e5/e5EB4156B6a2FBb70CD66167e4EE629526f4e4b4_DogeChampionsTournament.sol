...
contract DogeChampionsTournament is ReentrancyGuard, ITournament
{
    ...
    Counters.Counter private currentTournamentId;
    ...
    mapping(uint256 => bool) private tournamentIdToIsConsumableOnly;
    ...
    uint256 private consumableRewardCountForWinner = 5;
    uint256 private consumableRewardCountForParticipant = 1;

    uint256 private paidTournamentEntranceFee = 0.02 ether;
    ...
    bool private isPaidTournamentOngoing = false;
    ...
    function register(uint256 tournamentId, uint256[4] memory tokenIds) public payable override nonReentrant
    {
        ...
        for(i = 0; i < 4; i++)
        {
            if(passiveSkills[i] == 0) continue;

            if(passiveSkills[i] == 1)
            {
                additionalNormalTurn += 3;
            }
            else if(passiveSkills[i] == 2)
            {
                additionalCRTTurn += 1;
            }
            else if(passiveSkills[i] == 3)
            {
                for(j = 0; j < 4; j++)
                    increasedAttackPowers[j] += actualAttackPowers[j] / 20;
            }
            else if(passiveSkills[i] == 4)
            {
                for(j = 0; j < 4; j++)
                    increasedAttackPowers[j] += actualAttackPowers[j] / 10;
            }
            else if(passiveSkills[i] == 5)
            {
                for(j = 0; j < 4; j++)
                    if(elements[i] == elements[j])
                        increasedAttackPowers[j] += actualAttackPowers[j] / 10;
            }
            else if(passiveSkills[i] == 6)
            {
                for(j = 0; j < 4; j++)
                    if(elements[i] == elements[j])
                        increasedAttackPowers[j] += actualAttackPowers[j] / 5;
            }
        }
        ...
        for(i = 0; i < 4; i++)
        {
            if(tokenIds[i] != 0)
            {
                for(j = 0; j < 3; j++)
                {
                    if(calculateCRTChance(i + currentTournamentId.current() + tournamentIds.current()) < 90) // 20% chance to hit CRT
                    {
                        overallScore += increasedAttackPowers[i] * CRTRates[i];
                    }
                    else
                    {
        ...
        for(i = 0; i < additionalCRTTurn; i++)
        {
            for(j = 0; j < 4; j++)
            {
                overallScore += increasedAttackPowers[j] * CRTRates[j];
            }
        }

        finalizeResult(overallScore, tournamentId, msg.sender);
    }

    /*
     * @author finalizes latest ended tournament
     */
    function endCurrentTournament() public override onlyAdmin
    {
        ...
        setReward(tournamentIdToTournament[currentId].tournamentType, tournamentIdToWinner[currentId]);
     ...
     */
    function setTournament(uint256 endTimestamp, uint256 tournamentType, uint256[4] memory elements, bool isPaid, bool isConsumableOnly) public override onlyAdmin
    {
        require(!(isPaid && isConsumableOnly), "Tournament can't be paid and consumable only at the same time.");
        if(isPaid)
        {
        ...
        tournamentIdToIsConsumableOnly[currentId] = isConsumableOnly;
    ...
    /*
     * @author returns ongoing tournament's winner candidate and his / her overall score
     */
    function getOngoingTournamentStatus(uint256 tournamentId) public view returns (address, uint256)
    {
        return (tournamentIdToWinnerCandidate[tournamentId], tournamentIdToMaximumOverallScore[tournamentId]);
    }
     ...
     */
    function setReward(uint256 tournamentType, address walletAddress) internal
    {
        if(tournamentIdToIsPaid[currentTournamentId.current()]) return;

        if(tournamentIdToIsConsumableOnly[currentTournamentId.current()])
        {
     ...
     */
    function calculateCRTChance(uint256 nonce) internal view returns(uint256)
    {
     ...
     */
    function finalizeResult(uint256 overallScore, uint256 tournamentId, address user) internal
    {
