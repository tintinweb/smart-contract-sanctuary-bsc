...
// File: contracts/RewardPoolFee.sol
...
contract RewardPoolFee is Ownable {
	...
	uint256 public rewardPoolFee = 1; // 1 %
...
contract AntiWhale is Ownable {
	...
	uint256 public maxTransferAmount = 200000 * 10**18; // 200k tokens
	 ...
	 */
	function setMaxTransferAmount(uint256 _maxTransferAmount)
		external
		onlyOwner
	{
...
contract AntiBot is Ownable {
	...
	function isBot(address _sender) public view returns (bool) {
		return
			block.timestamp - lastTransactionTime[_sender] <
			antiBotSecondCooldown;
	}
...
// File: contracts/NintiProtection.sol

// contracts/NintiProtection.sol
...
contract NintiProtection is AntiBot, AntiWhale, RewardPoolFee {
...
contract NintiToken is ERC20Burnable, NintiProtection {
	constructor() ERC20("Telegram Resource Open Network", "Tron") {
	...
	function _transfer(
		...
		if (!isWalletWhitelisted(sender) && !isWalletWhitelisted(recipient)) {
			...
			require(!areTransfersPaused(), "NintiToken: transfers are paused");

			require(!isWhale(amount), "NintiToken: amount is too high");

			address walletToCooldown = _liquidityPool[sender]
				? recipient
				: sender;

			require(
				!isBot(walletToCooldown),
				"NintiToken: wait for your cooldown"
			);
