...
contract NFTFractional is Upgradable {
     ...
     * @param _value: 0: controller, 1: admin, 2: user
    */
    ...
    */
    ...
    function setNFTToken(string memory _name, string memory _symbol) external onlyController notEmpty(_name) notEmpty(_symbol) {
        nftToken = address(new NFTToken(_name, _symbol));
    }

    function setBaseURI(string memory _baseURI) external onlyController notEmpty(_baseURI) {
        require(nftToken != address(0), "NFTFractional: Address is not the zero address");
        NFTToken(nftToken).setBaseURI(_baseURI);
    }

    function mintNFT(
        address _receiver,
        uint256 _tokenId) external onlyAdmins {
        require(bytes(NFTToken(nftToken).baseURI()).length > 0, "NFTFractional: BaseURI not empty");
        NFTToken(nftToken).mintNFT(_receiver, _tokenId);
        emit MintNFT(nftToken, _receiver, _tokenId);
    }
    ...
    /**
    * @dev create functional F-NFT Pool
    ...
    */
    function createFNFTPool(address[] memory _addrs, uint256[] memory _datas, uint256[] memory _configs, uint256[] memory _tiers) external onlyAdmins {
        ...
        FNFTInfo memory fnft = fnftInfos[_datas[0]];
    ...
    function createTieringPool() external onlyAdmins {

    }

    function nameNFT() external view returns (string memory) {
        return NFTToken(nftToken).name();
    }

    function symbolNFT() external view returns (string memory) {
        return NFTToken(nftToken).symbol();
    }

    function baseURI() external view returns (string memory) {
        return NFTToken(nftToken).baseURI();
    }
...
contract Upgradable {
    ...
    mapping(address => uint256) public adminList; // 0: controller, 1: admin, 2: user
    ...
    // struct TierInfo {
    //     uint256 tierId;
    //     uint256 allocation; // allocation token
    // }
...
contract NFTToken is Ownable, ERC721URIStorage {
    ...
    */
    ...
    function burn (uint256 _tokenId) external onlyControllers virtual {
    ...
    function setBaseURI(string memory _uri) external onlyControllers {
        baseURI = _uri;
    }
}
