...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    constructor(string memory _name, string memory _symbol, address _routerAddress, address _rewardTokenAddress)  ERC20(_name, _symbol) {
    ...
    function distributeDividends() public override payable {
        ...
        if (msg.value > 0) {
            uint256 beforeBalance = IERC20(rewardTokenAddress).balanceOf(address(this));
            swapETHForTokens(msg.value);
            uint256 swapBalance = IERC20(rewardTokenAddress).balanceOf(address(this)).sub(beforeBalance);
    ...
    // generate the uniswap pair path of token -> weth
      ...
      path[1] = address(rewardTokenAddress);
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            IERC20(rewardTokenAddress).transfer(user, _withdrawableDividend);
...
contract DividendERC20KillBlock is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor(string memory _name, string memory _symbol, uint256 _supply, uint256[3] memory _buyFees, uint256[3] memory _sellFees, address[4] memory _addresses, uint256 _minimumTokenBalanceForDividends, uint256 _coolBlock, address serviceFeeReceiver_, uint256 serviceFee_) payable ERC20(_name, _symbol) {
        buyback = _buyFees[0];
...
contract CoinDividendTracker is DividendPayingToken, Ownable {
    ...
    constructor(uint256 _minimumTokenBalanceForDividends, address _routerAddress, address _rewardTokenAddress) DividendPayingToken("CoinDividendTracker", "Coin_DividendTracker", _routerAddress, _rewardTokenAddress) {
        claimWait = 3600;
