...
contract ExtendableBondedCakeReader is ExtendableBondReader, Initializable {
    ...
    function initialize(
      ...
      IPancakePair pairTokenAddress__DUET_CAKE_   // optional, if so, the previous should be required
        ...
        require(address(registry_) != address(0), "Cant set Registry to zero address");
        registry = registry_;
        require(address(pancakePool_) != address(0), "Cant set PancakePool to zero address");
        pancakePool = pancakePool_;
        require(address(pancakeMasterChef_) != address(0), "Cant set PancakeMasterChef to zero address");
        pancakeMasterChef = pancakeMasterChef_;
        require(address(pairTokenAddress__CAKE_BUSD_) != address(0), "Cant set PairTokenAddress__CAKE_BUSD to zero address");
        pairTokenAddress__CAKE_BUSD = pairTokenAddress__CAKE_BUSD_;

        require(
          address(pairTokenAddress__DUET_BUSD_) != address(0) || address(pairTokenAddress__DUET_CAKE_) != address(0),
...
contract ExtendableBondRegistry is Initializable, Adminable {
    ...
    event GroupCreated(string indexed groupName);
    event GroupDestroyed(string indexed groupName);
    event GroupItemAppended(string indexed groupName, address item);
    event GroupItemRemoved(string indexed groupName, address item);
