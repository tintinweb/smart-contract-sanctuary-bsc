...
contract OPVAuction is ReentrancyGuard {
    ...
    function Bid(uint _idOnMarket, uint256 _price) public nonReentrant {
        tokenId = idToMarketItem[_idOnMarket].tokenId;
        ...
        factory = idToMarketItem[_idOnMarket].nftContract;
        ...
        (uint256[] memory saveNumber, address[] memory saveAddr) = getBuyData(
            distance,
            factory,
            seller,
            tokenId
        ...
        for (uint256 i = 0; i < saveAddr.length; i++) {
            MainToken.transferFrom(msg.sender, saveAddr[i], saveNumber[i]);
        }
        ...
        emit NewBid(_idOnMarket, tokenId, factory, seller, msg.sender, _price);
    }

    function claimNft(uint _idOnMarket) public nonReentrant {
        tokenId = idToMarketItem[_idOnMarket].tokenId;
        endTime = idToMarketItem[_idOnMarket].endTime;
        latestBid = idToMarketItem[_idOnMarket].latestBid;
        latestUserBid = idToMarketItem[_idOnMarket].latestUserBid;
        ...
        if (latestUserBid == address(0)) {
            OPVFactory(factory).transferFrom(address(this), seller, tokenId);
            emit ClaimNft(
                _idOnMarket,
                tokenId,
                factory,
            ...
            OPVFactory(factory).transferFrom(
                address(this),
                latestUserBid,
                tokenId
            );
    ...
    function cancelSell(uint256 _idOnMarket) public nonReentrant {
        ...
        emit CancelSell(_idOnMarket, tokenId, factory, seller, owner);
    ...
    function setMainTokenAddress(address _MainToken)
        public
    ...
    function setBlackListFee(address[] memory user)
        public
        onlyOwner(msg.sender)
    {
    ...
    Ownable
{
    ...
    function mint(address to) public virtual {
        ...
        if (hasRole(MINTER_ROLE, _msgSender())) {
            ...
            require(OPV.balanceOf(_msgSender()) >= _feeMint, "Mint NFT : Unavailable by balanceOf");
            require(!_exists(tokenId), "Must have unique tokenId");
