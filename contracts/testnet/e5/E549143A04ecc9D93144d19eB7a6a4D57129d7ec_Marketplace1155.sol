...
contract Marketplace1155 is ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    event ListingBought(uint256 indexed id, address indexed buyer);
    ...
    event CollectibleClaimed(uint256 indexed listingId, address indexed target);
    ...
    constructor(address bump_, uint256 gracePeriod_) Ownable() {
    ...
    function createListing(
        ...
        IERC1155(collection).safeTransferFrom(msg.sender, address(this), tokenId, 1, new bytes(0));
    ...
    function cancel(uint256 listingId) external {
        ...
        IERC1155(listings[listingId].collection).safeTransferFrom(address(this), msg.sender, listings[listingId].tokenId, 1, new bytes(0));
    ...
    function claimCollectible(uint256 listingId) external nonReentrant {
        ...
        IERC1155(listings[listingId].collection).safeTransferFrom(
            address(this),
            target,
        ...
        emit CollectibleClaimed(listingId, target);
    }
    ...
    function bump(uint256 listingId_, uint256 amount_) external {
        ...
        listings[listingId_].bumpAmount += amount_;
        listings[listingId_].currency.safeTransferFrom(msg.sender, bumpAccount, amount_);

        emit Bumped(listingId_, listings[listingId_].bumpAmount);
    }
    ...
    function _distributeValue(uint256 listingId) private {
        ...
        if(listings[listingId].collection.supportsInterface(type(IERC2981).interfaceId)) {
            (royaltyReceiver, royaltyAmount) = IERC2981(listings[listingId].collection).royaltyInfo(listings[listingId].tokenId, listings[listingId].lastBid);
        }
