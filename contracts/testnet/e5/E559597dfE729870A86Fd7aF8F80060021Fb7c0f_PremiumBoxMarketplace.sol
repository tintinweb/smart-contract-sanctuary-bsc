...
contract PremiumBoxMarketplace is IERC721Receiver,Ownable {
    ...
    function setNft(IERC721Enumerable _box) public onlyOwner {
     ...
     function getListedBox() view public returns (ListDetail [] memory)  {
        
        uint balance = box.balanceOf(address(this));
    ...
    function listBox(uint256 _tokenId, uint256 _price) public {
        require(box.ownerOf(_tokenId) == msg.sender, "You are not the owner of this NFT");
        require(box.getApproved(_tokenId) == address(this), "Marketplace is not approved to transfer this NFT");
    ...
    function updateListingBoxPrice(uint256 _tokenId, uint256 _price) public {
        require(box.ownerOf(_tokenId) == address(this), "This NFT doesn't exist on marketplace");
