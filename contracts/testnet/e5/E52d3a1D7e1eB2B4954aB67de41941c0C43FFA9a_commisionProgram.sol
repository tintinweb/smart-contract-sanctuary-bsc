...
contract commisionProgram is Ownable, ReentrancyGuard {
     ...
     */
    function addToken(IERC20 _token_address) external onlyAdmin{
        require(_token_address != token_info[token_count].token_address, "Token already added!");

        AddToken storage tokenAdd = token_info[token_count];

        tokenAdd.token_id = token_count;
        tokenAdd.token_address = _token_address;

        emit AddNewToken(token_count, _token_address);
    ...
    function getProductByAdmin ()
        ...
        {
            uint256 length = product_counter.length;

            AddProduct[] memory prod_info = new AddProduct[](length);

            for (uint256 i = 0; i < length; i++) {
                prod_info[i] = product_info[i];
            }
