...
contract FilterERC20LoadParameters {
    address public routerAddr;
    bool routerAddrSet = false;

    function setRouter(address _routerAddr) public {
        require(!routerAddrSet); //only once
        routerAddr = _routerAddr;
        routerAddrSet = true;
    }
}

contract FilterERC20 is IFilterERC20, FilterERC20LoadParameters {
...
contract FilterFactory is IFilterFactory, FilterERC20LoadParameters {
    ...
    function setAdmin(address _admin) external {
        require(msg.sender == admin, "FilterFactory: FORBIDDEN");
        admin = _admin;
    }

    
    function setRouterAddress(address _routerAddr) public {
        setRouter(_routerAddr);
    }
}