...
contract SoulCore is DivestorUpgradeable, RandomGeneratorUpgradeable, ERC1155HolderUpgradeable{
    ...
    mapping(uint => nftStruct[]) public publicPool;              //免费池子：
    mapping(uint => mapping(uint => nftStruct[])) public  soulPool;
    ...
    function createToken(uint gender, uint amount, uint soulCount, address recommender) public returns(bool){
        ...
        if(soulCount > 0){
            ...
            //销毁soul
            coreInfo.SOULTOKEN.burnFrom(_msgSender(), soulCount * 1 ether);
        }

        //插入的数据
        nftStruct memory insertData = nftStruct(tokenId, amount);
        if(soulCount > 0){
            //氪金池子
            soulPool[gender][probability].push(insertData);
        }else{
            //免费池子
            publicPool[gender].push(insertData);
        }
    ...
    function getFree(uint gender_, address recommender_, uint isJump_) private returns(uint){
        ...
        if(length == 0){
            if(isJump_ != 1){
                //当前免费池子无数据且不是氪金池跳转过来的，需要跳转到氪金池去抽取
        ...
        //维护池子数量
        countMaintain(true, gender_, 0, tokenIndex, length);
    ...
    function getCharge(uint gender_, address recommender_, uint isJump_) private returns(uint){
        ...
        uint length = soulPool[gender_][proIndex].length;
        if(length == 0){
            if(isJump_ != 1){
                //不是氪金池跳转过来的
        ...
        //抽中有数据
        uint tokenIndex = randomCeil(length) - 1;
        tokenId = soulPool[gender_][proIndex][tokenIndex].tokenId;

        address caster = coreInfo.SOUL721.ownerOf(tokenId);
        //需要通过tokenId获取铸造者，用于分成。
        divide(recommender_, caster, 100);
        //tokenId数量维护
        countMaintain(false, gender_, proIndex, tokenIndex, length);
    ...
    //tokenId数量维护
    function countMaintain(bool status_, uint gender_,  uint proIndex_, uint tokenIndex_, uint length_) private{
        if(status_){
            //免费池
            ...
            soulPool[gender_][proIndex_][tokenIndex_].count -= 1;
            if(soulPool[gender_][proIndex_][tokenIndex_].count == 0){
                if(tokenIndex_ != (length_ - 1) && length_ > 1){
                    soulPool[gender_][proIndex_][tokenIndex_].tokenId = soulPool[gender_][proIndex_][length_ - 1].tokenId;
                    soulPool[gender_][proIndex_][tokenIndex_].count = soulPool[gender_][proIndex_][length_ - 1].count;
                }
                soulPool[gender_][proIndex_].pop();
            }
        }
    }
