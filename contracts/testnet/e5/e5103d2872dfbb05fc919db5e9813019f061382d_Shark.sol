...
contract Shark is ERC20, Ownable, Pausable {
    // CONFIG START

    uint256 denominator = 100;
    
    // TOKEN
    string tokenName = "Shark Token";
    string tokenSymbol = "Shark";
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        if(isExcluded[msg.sender] || isExcluded[tx.origin] || inTax) {
            ...
            if(!isExcluded[sender] && !isExcluded[recipient]) {
                require(!paused(), "Shark: Transfers paused");
                require(!blacklistStatus || (!isBlacklisted[sender] && !isBlacklisted[recipient]), "Shark: Blacklisted");
                require(!whitelistStatus || (isWhitelisted[sender] && isWhitelisted[recipient]), "Shark: Not Whitelisted");

                if(sender == address(_UniswapV2Pair) || recipient == address(_UniswapV2Pair)) {
                    if(sender == address(_UniswapV2Pair)) {
                        require(amount <= maxTxAmount, "Shark: Max tx amount");
                        require(sender != address(_UniswapV2Pair) || (sender == address(_UniswapV2Pair) && block.number >= nextBuyBlock[recipient]), "Shark: Cooldown");
                    ...
                    require(isExcludedFromBotProtection[recipient] || balanceOf(recipient) + amount <= maxWalletAmount, "Shark: Max wallet amount");                
                }
            }

            if(sender == address(_UniswapV2Pair) && !isExcludedFromBotProtection[recipient]) {
                require(!recipient.isContract(), "Shark: Bot Protection");
            } else if(recipient == address(_UniswapV2Pair) && !isExcludedFromBotProtection[sender]) {
                require(!sender.isContract(), "Shark: Bot Protection");
            }
    ...
    function setToken(address newValue) external onlyOwner {
        require(newValue != token, "Shark: Value already set to that option");

        token = newValue;
    }

    function setDenominator(uint256 newValue) external onlyOwner {
        require(newValue != denominator, "Shark: Value already set to that option");

        denominator = newValue;
    }

    function setMaxTxAmount(uint256 newValue) external onlyOwner {
        require(newValue != maxTxAmount, "Shark: Value already set to that option");

        maxTxAmount = newValue;
    }

    function setMaxWalletAmount(uint256 newValue) external onlyOwner {
        require(newValue != maxWalletAmount, "Shark: Value already set to that option");
    ...
    function setExcluded(address account, bool newValue) external onlyOwner {
        require(newValue != isExcluded[account], "Shark: Value already set to that option");

        isExcluded[account] = newValue;
    }

    function setExcludedFromBotProtection(address account, bool newValue) external onlyOwner {
        require(newValue != isExcludedFromBotProtection[account], "Shark: Value already set to that option");
    ...
    function massSetExcluded(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isExcluded[accounts[i]], "Shark: Value already set to that option");
    ...
    function massSetExcludedFromBotProtection(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isExcludedFromBotProtection[accounts[i]], "Shark: Value already set to that option");
    ...
    function setBlacklistStatus(bool newValue) external onlyOwner {
        require(blacklistStatus != newValue, "Shark: Value already set to that option");

        blacklistStatus = newValue;
    }

    function setWhitelistStatus(bool newValue) external onlyOwner {
        require(whitelistStatus != newValue, "Shark: Value already set to that option");

        whitelistStatus = newValue;
    }

    function setBlacklisted(address account, bool newValue) external onlyOwner {
        require(newValue != isBlacklisted[account], "Shark: Value already set to that option");

        isBlacklisted[account] = newValue;
    }

    function setWhitelisted(address account, bool newValue) external onlyOwner {
        require(newValue != isWhitelisted[account], "Shark: Value already set to that option");
    ...
    function massSetBlacklisted(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isBlacklisted[accounts[i]], "Shark: Value already set to that option");
    ...
    function massSetWhitelisted(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isWhitelisted[accounts[i]], "Shark: Value already set to that option");
    ...
    function setTaxStatus(bool newValue) external onlyOwner {
        require(taxStatus != newValue, "Shark: Value already set to that option");

        taxStatus = newValue;
    }

    function withdrawETH(address to, uint256 value) external onlyOwner {
        require(address(this).balance >= value, "Shark: Insufficient ETH balance");

        (bool success,) = to.call{value: value}("");
        require(success, "Shark: Transfer failed");
    }

    function withdrawTokens(address tokenAddress, address to, uint256 value) external onlyOwner {
        require(IERC20(tokenAddress).balanceOf(address(this)) >= value, "Shark: Insufficient token balance");

        try IERC20(token).transfer(to, value) {} catch {
            revert("Shark: Transfer failed");
        }
    }
