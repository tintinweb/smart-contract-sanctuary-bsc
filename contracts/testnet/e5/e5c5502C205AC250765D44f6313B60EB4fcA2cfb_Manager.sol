...
contract Manager is IManager2, AccessControl {
    ...
    {
        uint256 limit = Math.min(offset + size, projects.length);
        address[] memory res = new address[](limit - offset);

        for (uint256 i = offset; i < limit; ) {
            res[i - offset] = projects[i];
            unchecked {
                ++i;
            }
        }
    ...
    constructor(
        ...
        airdropLogic = logicContracts.airdrop;
    ...
    function invest(
        ...
        address lpad = project[token];
        require(lpad != address(0), 'project not found');
        ...
        ILaunchpad(lpad).invest(msg.sender, usd, round, data, signature);
    }
    ...
    function paybackProject(address token) external {
        address lpad = project[token];
        require(lpad != address(0), 'project not found');
        ILaunchpad(lpad).payBackProject();
    }
    ...
    function airdropOvercap(address token, uint256 size)
        external
    ...
    function changeVestingSettings(
        address token,
        uint8 round,
        IVesting.VestingSchedule calldata settings
    ) external onlyRole(LP_ADMIN_ROLE) {
        address lpad = project[token];
        require(lpad != address(0), 'project not found');

        address vesting = ILaunchpad(lpad).vestingContracts(round);
        IVesting(vesting).changeSettings(settings);
    }
        ...
        onlyRole(LP_ADMIN_ROLE)
    {
        ...
        ILaunchpad(lpad).setVestingContracts(vestingContracts_);
    }
    ...
    function create(
        ...
        IVesting.VestingSchedule[ROUNDS] calldata sched,
        ...
        require(
            project[launchData.projectToken] == address(0),
            'cant list the same token twice'
        );

        // deploy the launchpad contract
        address lpad = address(new IQProxy(launchpadLogic, ''));
        project[launchData.projectToken] = lpad;
        projects.push(launchData.projectToken);

        // get all the project tokens
        IERC20Metadata(launchData.projectToken).safeTransferFrom(
            msg.sender,
            address(this),
            launchData.projectTokensAmount
        );

        require(
            IERC20Metadata(launchData.projectToken).balanceOf(address(this)) >=
                launchData.projectTokensAmount,
            'tokens with fees are not supported'
        );

        // allow the launchpad contract to take them from us
        IERC20Metadata(launchData.projectToken).safeIncreaseAllowance(
            lpad,
            launchData.projectTokensAmount
        );
        ...
        // deploy the vesting contracts for each round
        address[] memory vesting = new address[](ROUNDS);
        for (uint8 i = 0; i < ROUNDS; ) {
            vesting[i] = address(new IQProxy(vestingLogic, ''));
            IVesting(vesting[i]).initialize(i, lpad, sched[i], address(this));
            unchecked {
                ++i;
            }
        }

        // deploy the airdrop contract and initialize it
        address airdropContract = address(new IQProxy(airdropLogic, ''));
        ...
        IAirdrop(airdropContract).initialize(launchData.projectToken, airdropAmount);

        // let the launchpad know about them
        ILaunchpad(lpad).setVestingContracts(vesting);
        ILaunchpad(lpad).setAirdropContract(airdropContract);

        emit CreateLaunch(launchData.projectToken, lpad);
    }
    ...
    {
        address lpad = project[token];
        require(msg.sender == ILaunchpad(lpad).projectWallet(), 'not project wallet');

        address airdropContract = ILaunchpad(lpad).airdropContract();
        IAirdrop(airdropContract).airdrop(receivers);
    }
