...
contract sTrapeza_V2 is IsFIDL, ERC20Permit {
  ...
  event LogContractsUpdated(address stakingContract, address oldsFIDL);
  ...
  // do this last
  function initialize(address stakingContract_, address oldsFIDL_) external onlyInitializer {
    ...
    emit LogContractsUpdated(stakingContract, oldsFIDL_);

    initializer = address(0);
    emit OwnershipPushed(initializer, address(0));
  }

  function setTreasury(address _treasury) external onlyTreasuryManager {
  ...
  function renounceTreasuryManager() external onlyTreasuryManager {
    treasuryManager = address(0);
    
    emit OwnershipPushed(treasuryManager, address(0));
  }
  ...
  function circulatingSupply() public view override returns (uint256) {
    return
      _totalSupply
        ...
        .add(balanceForGons(oldsFIDL.gonsForBalance(oldsFIDL.circulatingSupply())))
