...
contract BOREDCATS is Context, IERC20, Ownable {
    ...
    string private constant _name = unicode"BORED CATS";
    ...
    function setlpWallet(address payable _address) external onlyOwner {
        _lpWallet = _address;
    }
    ...
    function setteamWallet(address payable _address) external onlyOwner {
    ...
    function swapTokensForEth(uint256 tokenAmount) private lockTheSwap {
        ...
        uint256 lp = amount.mul(1000).div(4000);             
        uint256 marketing = amount.mul(1000).div(2000);          
        uint256 team = amount.mul(1000).div(9600);     
        uint256 buyback = amount.sub(lp).sub(team).sub(marketing);
        _lpWallet.transfer(lp);
