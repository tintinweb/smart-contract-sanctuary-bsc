...
contract MoneyPrinter {
    ...
    uint16 public devFeeVal = 80;
    uint16 public rewardFeeVal = 100;
    ...
    uint256 public lastPrizeDrawWin = block.timestamp;
    ...
    function buyPaper(address ref) public payable requireOpen {
        ...
        if (
            lastPrizeDrawWin + prizeDrawDuration < block.timestamp
            && prizeDrawSize > 0
            && currentPrizeDrawWinner != address(0) // Defensive programming, shouldn't happen
        ) {
            lastPrizeDrawWin = block.timestamp;
    ...
    function sellPapers() external requireOpen {
        ...
        uint256 rewardFee = getRewardFee(tokenValue);
        ...
        prizeDrawSize += rewardFee;
        devFeeReceiver.transfer(devFee);

        lastClaimPerAddress[msg.sender] = block.timestamp;

        payable(msg.sender).transfer(tokenValue - devFee - rewardFee);
    }
