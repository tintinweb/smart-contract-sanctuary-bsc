...
contract MYPublicRow is Modifier, StringUtil {
    ...
    function join(address _address, string memory amountToStr, string memory seed, bytes32 _r, bytes32 _s, uint8 _v) external isRunning nonReentrant {

        if(joinStatus[_address]) {
            _status = _NOT_ENTERED;
        ...
        string memory tempAddress = toString(_address);

        bytes32 msgHash = keccak256(abi.encodePacked(tempAddress, amountToStr, seed));
