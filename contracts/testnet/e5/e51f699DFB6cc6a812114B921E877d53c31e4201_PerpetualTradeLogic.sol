...
contract PerpetualUpdateFunctions is PerpetualRebalanceFunctions {
    ...
    function _accumulateFunding(PerpetualData storage _perpetual, int128 _fTimeElapsed) internal {
        ...
        int128 fInterestPaymentLong = _fTimeElapsed.mul(_perpetual.fCurrentFundingRate).div(FUNDING_INTERVAL_SEC);
    ...
    function _getMarkPremiumRateEMA(PerpetualData storage _perpetual) internal view returns (int128) {
        return _perpetual.state != PerpetualState.NORMAL ? _perpetual.settlementMarkPremiumRate.fPrice : _perpetual.currentMarkPremiumRate.fPrice;
    }
     ...
     * @param   _iCurrentTime The current timestamp, uint256
     */
    function _updateFundingRatesInPool(uint256 _iPoolIndex, uint256 _iCurrentTime) internal {
        ...
        //update iLastFundingTime (we need it in _accumulateFundingInPool and _updateFundingRatesInPool)
     ...
     */
    function _updateDefaultFundTargetSize(uint256 _iPoolIndex, uint256 _iCurrentTime) internal {
        require(_iPoolIndex <= iPoolCount, "pool index out of range");
        LiquidityPoolData storage liquidityPool = liquidityPools[_iPoolIndex];
        ...
        if (_iCurrentTime - liquidityPool.iLastTargetPoolSizeTime > liquidityPool.iTargetPoolSizeUpdateTime) {
            ...
            emit UpdateDefaultFundTargetSize(liquidityPool.id, liquidityPool.fDefaultFundCashCC, fTargetDFSize);
        }
    }
    ...
    function _updateOraclePricesForPool(uint256 _iPoolIndex, uint256 _iCurrentTimeSec) internal {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (ISpotOracle(perpetual.oracleS2Addr).isTerminated()) {
                ...
                _setEmergencyState(perpetual);
            } else if (perpetual.oracleS3Addr != address(0) && ISpotOracle(perpetual.oracleS3Addr).isTerminated()) {
                _rebalance(perpetual);
                _setEmergencyState(perpetual);
            } else if (!oraclePriceData[perpetual.oracleS2Addr].isInSignificant || !oraclePriceData[perpetual.oracleS3Addr].isInSignificant) {
...
contract PerpetualTradeLogic is PerpetualUpdateFunctions, IFunctionList, IPerpetualTradeLogic {
    ...
    function _preTrade(
        ...
        bool closePos = !_hasTheSameSign(fTraderPos, _fAmount) && fTraderPos.add(_fAmount).abs() <= _perpetual.fLotSizeBC;
        if (closePos) {
