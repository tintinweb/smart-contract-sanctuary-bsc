	...
	function bonus() public payable {
		...
		for (uint256 i = 0; i < users[msg.sender].deposits.length; i++) {
			if(user.deposits[i].plan < 3){
				...
				if(user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(1 days)) > block.timestamp){
					if(user.deposits[i].plan == 3){
						user.yourboompercent = user.yourboompercent.add(18);
					}else if(user.deposits[i].plan == 4){
						user.yourboompercent = user.yourboompercent.add(28);
					}else if(user.deposits[i].plan == 5){
						user.yourboompercent = user.yourboompercent.add(38);
		...
		percent = percent.div(p);
		user.yourbonuspercent = user.yourbonuspercent.add(percent);
