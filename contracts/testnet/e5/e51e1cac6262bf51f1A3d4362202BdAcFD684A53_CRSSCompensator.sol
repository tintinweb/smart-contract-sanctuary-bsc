...
contract CRSSCompensator is Ownable, ReentrancyGuard {
    ...
    function claimCompensation(uint256 _amount, bool _vesting)
        public
        nonReentrant
    {
        UserCompensation storage user = userCompensation[msg.sender];
        uint256 percentageClaimable = ((block.number - compensationStartBlock) *
            1000000) / compensationLengthInBlocks;
        uint256 crssClaimable = (uint256(user.totalCrssOwed) *
            percentageClaimable) /
            1000000 -
            uint256(user.crssClaimed);
        ...
        if (_amount <= crssClaimable) {
            ...
            if (_vesting) {
                ICRSS(crssToken).controlledMint(_amount);
                userVestingInstances[msg.sender].push(
                    VestingObject({
                        crssAmount: uint128(_amount),
                        startTimestamp: uint128(block.timestamp)
                    })
                );
                totalVesting += _amount;
            } else {
                uint256 adjustedAmount = (_amount * 3) / 4;
                ICRSS(crssToken).controlledMintTo(msg.sender, adjustedAmount);
                ICRSS(crssToken).controlledMintTo(
                    accountant,
            ...
            user.crssClaimed += uint128(_amount);
        }
    }

    function withdrawVested(uint256 _id) public nonReentrant {
        VestingObject memory userVest = userVestingInstances[msg.sender][_id];
        require(
            userVest.startTimestamp + vestDuration <= block.timestamp,
            "cCRSS:Not unlocked yet"
        );

        userVestingInstances[msg.sender][_id] = userVestingInstances[
            msg.sender
        ][userVestingInstances[msg.sender].length - 1];
        userVestingInstances[msg.sender].pop();
        uint256 adjustedVestAmount = uint256(userVest.crssAmount);
    ...
    function getUnmintedCrssRemaining()
        public
        view
