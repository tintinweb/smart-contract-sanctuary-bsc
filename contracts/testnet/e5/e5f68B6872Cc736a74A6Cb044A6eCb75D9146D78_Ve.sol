...
contract Ve is IERC721, IERC721Metadata, IVe, Reentrancy {
  ...
  function _tokenURI(uint _tokenId, uint _balanceOf, uint untilEnd, uint _value) public pure returns (string memory output) {
    ...
    string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "veTRIT #', _toString(_tokenId), '", "description": "Locked TRIT tokens", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
