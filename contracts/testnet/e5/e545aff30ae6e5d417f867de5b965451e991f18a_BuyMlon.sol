...
contract BuyMlon is ReentrancyGuard, Ownable{
   using SafeMath for uint256; 
    ...
    constructor( address _treasury, address _mlon, address _usdt) public{
        treasury = _treasury;
        mlon = IBEP20(_mlon);
        usdt = IBEP20(_usdt);
    }

    function buyMLon(uint256 _amount, uint256 slippage) public nonReentrant{ 
        require(usdt.balanceOf(msg.sender) >= _amount, "BUYMLON: insufficient usdt");
        _amount.mul(priceOfMlon);
        usdt.transferFrom(msg.sender, treasury, priceOfMlon);
        
    }

    function addLiquiduity(uint256 _usdtAmount, uint256 _mlonAmount) public onlyOwner{
        require(usdt.balanceOf(msg.sender) >= _usdtAmount, "BUYMLON: insufficient usdt");
        require(mlon.balanceOf(msg.sender) >= _mlonAmount, "BUYMLON: insufficient mlon");

        mlonSupply = mlonSupply.add(_mlonAmount);
        usdtSupply = usdtSupply.add(_usdtAmount);
        usdt.transferFrom(msg.sender, address(this), _usdtAmount);
        mlon.transferFrom(msg.sender, address(this), _mlonAmount);

        k = mlonSupply.mul(usdtSupply);
    }
