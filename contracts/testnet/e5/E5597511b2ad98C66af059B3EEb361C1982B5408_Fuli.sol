...
contract Fuli is ERC20Detailed, Ownable {
    ...
    function manualRebase2() external {
        ...
        uint256 deltaTime = block.timestamp - _lastRebasedTime;
        uint256 times = deltaTime.div(rebaseInterval);
        uint256 epoch = times.mul(rebaseInterval.div(60));
    ...
    function manualRebase6() external {
        require(shouldRebase(), "rebase not required");
        // uint256 rebaseRate = 1700000; //1.7% per times
        uint256 deltaTime = block.timestamp - _lastRebasedTime;
        uint256 times = deltaTime.div(rebaseInterval);
        ...
        emit LogRebase(deltaTime, times);
    }

    function manualRebase7() external {
        require(shouldRebase(), "rebase not required");
        // uint256 rebaseRate = 1700000; //1.7% per times
        uint256 deltaTime = block.timestamp - _lastRebasedTime;
        // uint256 times = deltaTime.div(rebaseInterval);
        ...
        emit LogRebase(deltaTime, deltaTime);
    }


    function manualRebase8() external {
        require(shouldRebase(), "rebase not required");
        _lastRebasedTime +=1;
    }


    function manualRebase9() external {
        _lastRebasedTime +=2;
    }
