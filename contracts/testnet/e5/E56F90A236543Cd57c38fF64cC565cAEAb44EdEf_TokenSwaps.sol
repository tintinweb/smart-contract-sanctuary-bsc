...
contract TokenSwaps {
    ...
    function swapCitrus(address walletAddress, uint256 amount) public returns (uint256) {
        uint256 amouts = amount * 10 **18; 
        require(block.timestamp < times, "time over");
        require(amouts >= minAmount, "must be greater than OR equal minimum amount");
        require(amouts > 0, "amount must be greater then zero");
        require(
            token1.balanceOf(walletAddress) >= amouts,
            "sender doesn't have enough Tokens"
        );

        uint256 exchange = (amouts * ratio) / 10**18;
        ...
        token1.transferFrom(walletAddress, address(this), amouts);
