...
// File: ../leveraged/contracts/interfaces/IMasterChef.sol
...
/**
* @dev Interface for a MasterChef contract
 **/

interface IMasterChef {
    function duringITokenTransfer(address _asset, address _user, uint256 _amount) external;
}
...
// File: ../leveraged/contracts/IToken.sol
...
/**
 * @title BEP-20 interest bearing token
 * @dev Implementation of the interest bearing token for the lending platform.
 */
contract IToken is BEP20 {
    /**
    ...
    * @param _user the address of the user from which the iTokens were burned
    ...
    **/
    ...
    IBEP20Metadata public asset;

    IMasterChef public masterChef;
    ILeveragedVault public vault;

    modifier onlyMasterChefContract {
        require(msg.sender == address(masterChef), "The caller of this function must be a MasterChef contract");
        _;
    }
    ...
    /**
    * @notice BEP-20 implementation internal function backing transfer() and transferFrom()
    **/
    function _transfer(address _from, address _to, uint256 _amount) internal override {
        require(_from != _to, "Sending tokens to yourself");
        require(_amount <= vault.getUserAssetBalance(address(asset), _from), "Amount more than the user deposit of asset");

        masterChef.duringITokenTransfer(address(asset), _from, _amount);

        super._transfer(_from, _to, vault.storedAmount(address(asset), _amount));
    }

    /**
    * @dev mints token in the event of users depositing the asset into the MasterChef contract
    * @param _user the address of the user receiving the minted tokens
    * @param _amount the amount of tokens to mint
    **/
    function mint(address _user, uint256 _amount) external onlyMasterChefContract {
        _mint(_user, vault.storedAmount(address(asset), _amount));
    ...
    /**
    * @dev burns iTokens.
    * @param _user the address from which burn the iTokens
    * @param _amount the amount to burn
    **/
    function burn(address _user, uint256 _amount) external onlyMasterChefContract {
        _burn(_user, vault.storedAmount(address(asset), _amount));
    ...
    **/
    function balanceOf(address _user) override public view returns (uint256) {
        return vault.cumulatedAmount(address(asset), super.balanceOf(_user));
    }

    /**
    * @dev calculates the total supply
    * @return the current calculated total supply
    **/
    function totalSupply() override public view returns (uint256) {
        return vault.cumulatedAmount(address(asset), super.totalSupply());
    }
