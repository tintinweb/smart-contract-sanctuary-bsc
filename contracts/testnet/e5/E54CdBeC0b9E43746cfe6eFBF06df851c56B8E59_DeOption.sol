...
contract Buyer is IBuyer, BaseStorage {
    ...
    function _buyOption(bytes32 productId, uint256 optionCost, uint256 feedPrice, uint80 positionOpenedRound) internal {
        ...
        if (timeUnits != 0) {
            _products[productId].lastHReducedTimeStamp += uint64(timeUnits) * product.reduceTimeUnit;
        ...
        int optionPrice = Calculator.calculateOptionPrice(
            feedPrice,
        ...
        uint128 optionAmount = Calculator.calculateOptionAmount(optionCost, optionPrice);
    ...
    function _addProduct(
        ...
        _products[productId] = Product(
            ...
            valitility, c, normalDistributionValue, 
            lambda, reducedRate, 
            uint64(block.timestamp), reduceTimeUnit,
            0
        );
...
contract DeOption is IDeOption, Liquidator, AccessControl { 
    ...
    bytes32 public constant LIAUIDATE_PROVIDER_ROLE = keccak256("LIAUIDATER_PROVIDER_ROLE");
    ...
    constructor(
        ...
        _setupRole(LIAUIDATE_PROVIDER_ROLE, liquidateProvider);
    ...
    function addFunds(uint256 amount) external onlyRole(LIAUIDATE_PROVIDER_ROLE) {
        baseCurrency().safeTransferFrom(msg.sender, address(this), amount);
        _updateLPFunds(int256(amount));
    }

    function removeFunds(uint256 amount) external onlyRole(LIAUIDATE_PROVIDER_ROLE) {
...
contract Liquidator is Buyer, ILiquidator {
    ...
    function selfUnclaim(uint128 positionId) external {
        ...
        (uint256 unclaimTokenAmount, uint80 targetRoundId, uint256 targetPrice) =
            _getUserUnclaimTokenAmount(userPosition);
        // Calculate user profit fee amount
        ...
        _setNextSearchRound(userPosition.target, targetRoundId);
