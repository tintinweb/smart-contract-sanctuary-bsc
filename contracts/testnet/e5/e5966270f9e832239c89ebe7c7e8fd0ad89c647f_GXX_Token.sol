...
contract GXX_Token is ERC20("Dynasty of Legends", "DOL", 100*10**9*10**18, 10*10**9*10**18) {
...
contract Token_PreSale is Ownable {
    ...
    GXX_Token public _tokenPresale;
    ...
    constructor(address _tokenSale) public {
		_tokenPresale = GXX_Token(_tokenSale) ;
		_maxTokenSale = 30*10**9*10**18;
		_saledToken = 0;
		_saleStatus = true;
		_tokenPrice = 4 * 10** 10;
        // _meltStatus = false;
		_endSaleBlock = block.timestamp.add(5*86400);
		_openBlockArr[0] = _endSaleBlock.add(30 *86400);
		_openBlockArr[1] = _endSaleBlock.add(60*86400);
	...
	function checkTimeUnlockPercent (address user, uint256 checkNumber) public view returns (uint256){
		// if(block.timestamp < _openBlockArr[0]){
		// 	return 0;
		// }else if (_openBlockArr[0] <= block.timestamp && block.timestamp < _openBlockArr[1]){
		// 	return 1;
		// }else if (_openBlockArr[1] <= block.timestamp && block.timestamp < _openBlockArr[2]){
		// 	return 2;
		// }else if (_openBlockArr[2] <= block.timestamp && block.timestamp < _openBlockArr[3]){
		// 	return 3;
		// }else if(_openBlockArr[3] <= block.timestamp){
		// 	return 4;
		// }
		...
		if(checkNumber < _openBlockArr[0]){
			...
			return 80;
		}else if(_openBlockArr[3] <= checkNumber){
			return 100;
		}
		
	}

	function unlockToken(uint256 checkNumber) public returns (bool) {
		uint256 checkPercent = checkTimeUnlockPercent(address(msg.sender), checkNumber) ;
