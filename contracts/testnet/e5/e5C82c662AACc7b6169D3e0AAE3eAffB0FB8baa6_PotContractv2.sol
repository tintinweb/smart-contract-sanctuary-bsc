...
contract PotContractv2 is Initializable, ReentrancyGuardUpgradeable {
    /*
    ...
    function initialize(address _owner) public initializer {
        ...
        usdtAddr = 0x6DE71f139556bf8F5cBe39232053EAfE9fCBcF50;
    ...
    function attemptTransferFrom(string memory _tokenName, address _userAddress, uint256 _amount) public returns (bool success){
        IPRC20 token = IPRC20(tokenWhiteList[_tokenName].tokenAddress);
        try token.transferFrom(_userAddress, address(this), _amount) returns(bool){
            success = true;
        } catch (bytes memory /*lowLevelData*/) {
            success = false;  // Will not get here
        }
    }
    ...
    */
    ...
    function getBNBPrice() public view returns (uint256 price) {
        IPancakeFactory factory = IPancakeFactory(pancakeswapV2FactoryAddr);
        IPancakePair pair = IPancakePair(factory.getPair(wbnbAddr, usdtAddr));
        IPRC20 wethToken = IPRC20(pair.token0());
    ...
    /**
     * @dev Swaps accumulated fees into BNB first, and then to BNBP
     */
    function swapAccumulatedFees() external validBNBP nonReentrant {
        ...
        uint256 amountOutMinForBNBP = router.getAmountsOut(BNBFee, path)[1];

        uint256[] memory amounts = router.swapExactETHForTokens{ value: BNBFee }(
            amountOutMinForBNBP,
            path,
            address(this),
            block.timestamp
        );
        adminFeeTokenValues['BNB'] -= amounts[0];
        adminFeeTokenValues['BNBP'] += amounts[1];
