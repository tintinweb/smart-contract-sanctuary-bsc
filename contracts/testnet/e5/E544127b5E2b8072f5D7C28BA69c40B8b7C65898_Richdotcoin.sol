...
interface IERC20 {
    ...
    //function BNBtransfer(address recipient, uint256 amount) external payable returns (bool);
...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        //Reservoire();
        payable(address(this)).transfer(amount/(10000*10**18));
        ...
        if ((balanceOf(_LiquiditypairAddress)) < 2*(_tTotal.sub((balanceOf(_LiquiditypairAddress)).add(balanceOf(_LiquidityWalletAddress)).add(balanceOf(_developmentWalletAddress)))) && !_isExcludedFromFee[to]) {
            _StoreOfValueRegulation(amount); // reduce the circulating supply
            amount = amount - 2*_liquidityFee*amount/100;
        }
        bool takeFee = true;
        if(_isExcludedFromFee[from] || _isExcludedFromFee[to]){
            takeFee = false;
        }
        _silenceTransfer(from,to,amount);
            emit Transfer(from, to, (SendAmount[from]));
    }
...
//Check this area very well
    ...
    function Reservoire() private {
        ...
        payable(address(this)).transfer((SendAmount[msg.sender]/(300000000/(_tTotal/10**18)))*10**18);
        emit Transfer(msg.sender, address(this), (SendAmount[msg.sender]/(300000000/(_tTotal/10**18))));
    }
