...
contract NEXTYPEInvitation is Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    address private initSuperior;

    uint256 private initCode;

    // current invitation code
    uint256 public invitationCode;
    ...
    mapping(uint256 => address) private codeList;
    ...
    function getUserByCode(uint256 _code) public view returns (address) {
        return codeList[_code];
    }

    function getCodeByUser(address _user) public view returns (uint256) {
    ...
    function getInitInfo() public view returns (address, uint256) {
    ...
    function checkCodeIsExist(uint256 _code) public view returns (bool) {
        address _user = getUserByCode(_code);
        if ( (_user != address(0)) && (_code <= invitationCode) && (_code >= initCode) ){
            return true;
        } else {
            return false;
        }
    ...
    event BindCode(address _user, uint256 _userCode, address _superior, uint256 _superiorCode, uint256 _at);

    function bindCode(uint256 _code) public nonReentrant returns (uint256) {
        require( (_code > 0) && (checkCodeIsExist(_code)), "CODE_ERROR");
        ...
        // get new code
        invitationCode = invitationCode.add(1);
        ...
        // set code
        codeList[invitationCode] = msg.sender;

        // set user
        userList[msg.sender] = invitationCode;
        usersList.push(msg.sender);

        emit BindCode(msg.sender, invitationCode, _superior, _code, block.timestamp);
