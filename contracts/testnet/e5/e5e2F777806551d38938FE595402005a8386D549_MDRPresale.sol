...
contract MDRPresale {
    ...
    uint256 public startDate = 1669341600;                  // 2022/11/25 02:00:00 UTC
    uint256 public endDate = 1670551200;                    // 2022/12/9 02:00:00 UTC
    uint256 public lockDate = 1672452000;                    // 2022/12/31 02:00:00 UTC
    
    uint256 public totalTokensToSell = 50000000 * 10**18;          // 40000000 MDR tokens for sell
    uint256 public mdrPerBnb = 900 * 10**18;             // 1 BNB = 900 MDR
    uint256 public minPerTransaction = 90 * 10**18;         // min amount per transaction (0.1BNB)
    uint256 public maxPerUser = 9000 * 10**18;                // max amount per user (10BNB)
    uint256 public totalSold;

    address payable private teamAddr = 0xFa3d3799a51e131d4DD5BF34d6505A9C0171eead;
    ...
    function withdrawCollectedTokens() public checkWithdrawLPRequirements(){
        require(msg.sender == owner);
        require(address(this).balance > 0, "Insufficient balance");
        uint256 teamAmount = address(this).balance.div(10);
        uint256 treasuryAmount = address(this).balance - teamAmount;
        teamAddr.transfer(teamAmount);
        owner.transfer(treasuryAmount);
    }

    function claimTokens(uint256 amount) public checkWithdrawLPRequirements(){
        require(msg.sender == owner);
        require(amount > 0, "Invalid amount");
        require(address(this).balance > amount, "Insufficient balance");
        uint256 teamAmount = amount.div(10);
        uint256 treasuryAmount = amount - teamAmount;
        teamAddr.transfer(teamAmount);
        owner.transfer(treasuryAmount);
    }
    ...
    //only owner can call this function
    function withdrawUnsoldTokens() public checkWithdrawLPRequirements(){
        require(msg.sender == owner);
