...
import "./NFTAStorage.sol";
import "../access/Roles.sol";
import "./INFT.sol";

error URISetofNonexistentToken();
error BatchSizeTooLarge(uint256 maximum, uint256 actual);
error MaximumTotalSupplyReached(uint256 maxTokenSupply);
error BurningIsNotEnabled();
error NotImplemented();

contract NFTA is INFT, ERC721AUpgradeable, NFTAStorage {
    ...
    modifier onlyOperator() {
        _aclContract.checkRole(Roles.OPERATOR, msg.sender);
        _;
    }

    modifier onlyMinter() {
        _aclContract.checkRole(Roles.MINTER, msg.sender);
        _;
    }
    ...
    function setTokenURI(uint256 tokenId, string memory _tokenURI) external onlyOperator {
        if (!_exists(tokenId)) revert URISetofNonexistentToken();
    ...
    function _beforeTokenTransfers(
        ...
        if (from == address(0) && totalSupply() + quantity > _maxTokenSupply)
            revert MaximumTotalSupplyReached(_maxTokenSupply);
    }
...
import "../access/IACL.sol";

abstract contract NFTAStorage {
