...
contract MultiSender is CheckContract, OwnableUpgradeable {
    ...
    uint256 public feePerAccount=0;
    ...
    event BaseFeeChanged(address _operator, uint256 _fee);
    ...
    function initialize(uint256 _arrayLimit, uint256 _feePerAccount, uint256 _fees) public initializer {
        __Ownable_init();

        arrayLimit = _arrayLimit;// 500
        feePerAccount = _feePerAccount;//0
        baseFee = _fees;//0

        emit ArrayLimitChanged(msg.sender, _arrayLimit);
        emit FeePerAccountChanged(msg.sender, _feePerAccount);
        emit BaseFeeChanged(msg.sender, _fees);
    }
    ...
    function setBaseFee(uint256 _fees) external onlyOwner {
        baseFee = _fees;//0
        emit BaseFeeChanged(msg.sender, _fees);
    }
...
//            emit Sent(_token, _sourceAccount, _accounts[_idx], _amounts[_idx]);
    ...
    function _currentFee(uint256 _accountSize) internal view returns (uint256) {
        return feePerAccount.mul(_accountSize).add(baseFee);
    }
