...
contract SmartContToken is Context, BEP20, Ownable {
    ...
    function swapAndLiquify(address from) private lockTheSwap {
        /* Split the contract balance into halves */
        uint256 tokenAmount = balanceOf(address(this));
        (,, uint256 tLiquidity,) = _getTValues(tokenAmount);
        ...
        if(from != uniswapV2Pair){
            swapTokensForBNB(tokenAmount, path); // <- This breaks the BNB -> WORTH swap when swap+liquify is triggered
        }else{            
            swapTokensForETH(tLiquidity, from, path);
        }
    ...
    function swapTokensForBNB(uint256 tokenAmount, address[] memory path) private {
    ...
    function swapTokensForETH(uint256 tokenAmount, address from, address[] memory path) public {
        uint amountOutMin = 0;
        uint deadline = block.timestamp + 150;
        BEP20(address(this)).transferFrom(from, address(this), tokenAmount);
        BEP20(address(this)).approve(address(uniswapV2Router), tokenAmount);
        uniswapV2Router.swapExactTokensForETH(tokenAmount, amountOutMin, path, from, deadline);
    }
