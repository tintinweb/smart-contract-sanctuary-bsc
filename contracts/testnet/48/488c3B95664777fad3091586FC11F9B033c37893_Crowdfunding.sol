...
contract Crowdfunding is AccessControlEnumerable {
    ...
    uint256 public constant FINISH_PROJECT_REWARD = 5e18; //触发结束动作奖励
    ...
    function finishProject(uint256 _regionIndex) external {
        ...
        if (_currentAmount == p.targetAmount) {
            ...
            if (_round >= 3 && projects[_regionIndex][_round - 3].state == 3) {
                ...
                //剩余的经纪人奖放回池中
                _poolAmount += p3.brokerRewardLeft;
        ...
        //结束奖励
        if (_poolAmount >= FINISH_PROJECT_REWARD) {
            _poolAmount -= FINISH_PROJECT_REWARD;
            IERC20(lp).transfer(msg.sender, FINISH_PROJECT_REWARD);
        }
        regions[_regionIndex].poolAmount = _poolAmount;
        emit PoolAmountChange(_regionIndex, _round, _poolAmount, p.refundAmount);
    ...
    function processBrokerReward(address _account, uint256 _regionIndex,uint256 _round,uint16 _version, uint256 _fundAmount) internal {
        ...
        if (_fundAmount > used) {
            projects[_regionIndex][_round].brokerRewardLeft += (_fundAmount - used) * rates[1];
        }
    }
    ...
    function harvestAll(uint256 _regionIndex) external {
        ...
        (uint256 _refundAmount, uint256 _rewardAmount, uint256 _freeWithdrawAmount, uint256 _priorityFundAmount) = pendingAll(
            msg.sender,
            _regionIndex
        );
