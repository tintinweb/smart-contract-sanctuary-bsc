...
contract interfaceServer  is Ownable {
    ...
    address public businessAddr = 0x01a4E2C1B99eE34ECF190AE10cBC415963773223;
    ...
    event battleEvent(uint indexed isWin);
    ...
    function pvpbattle(uint from ,uint to) external returns(uint){
        uint isWin = IBusinessBattle(businessBattleAddr).pvpbattle(from,to);
        emit battleEvent(isWin);
        return isWin;
        
    }
    function pvebattle(uint self,uint stage)external returns(uint){
        uint isWin = IBusinessBattle(businessBattleAddr).pvebattle(self,stage);
        emit battleEvent(isWin);
        return isWin;
    }
