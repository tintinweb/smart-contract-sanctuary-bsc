...
contract Staking is Ownable, ReentrancyGuard {
    ...
    function initialize(
        ...
        lockups.push(Lockup(0, 0, 0, 200, 1500, 0, 0, 0, 0, 0));
        lockups.push(Lockup(1, 30 * 24 * 60 * 60, 0, 0, 2500, 0, 0, 0, 0, 0));
        lockups.push(Lockup(2, 90 * 24 * 60 * 60, 0, 0, 4000, 0, 0, 0, 0, 0));
        lockups.push(Lockup(3, 180 * 24 * 60 * 60, 0, 0, 5000, 0, 0, 0, 0, 0));
        lockups.push(Lockup(4, 360 * 24 * 60 * 60, 0, 0, 6000, 0, 0, 0, 0, 0));
    ...
    function withdraw(uint256 _amount, uint8 _stakeType) external nonReentrant {
        ...
        for (uint256 j = 0; j < stakes.length; j++) {
            ...
            if (stake.end <= _getSlot() && stake.end != 0) continue;
            if (remained == 0) break;
        ...
        if (pending > 0) {
            ...
            if (lockup.withdrawFee > 0) {
                uint256 fee = pending.mul(lockup.withdrawFee).div(10000);
                IERC20(stakingToken).safeTransfer(walletA, fee);
                pending = pending.sub(fee);
            }

            IERC20(stakingToken).safeTransfer(address(msg.sender), pending);
            emit Withdraw(msg.sender, _stakeType, pending);
    ...
    function compoundReward(uint8 _stakeType) external payable nonReentrant {
        ...
        if (pending > 0) {
            ...
            emit Deposit(msg.sender, _stakeType, pending);
        }
    }

    /* +++ */
    function _calcUserLockupReward(uint8 _stakeType) internal nonReentrant {
        require(isInitialized, "Not initialized");
        require(startBlock > 0, "Pool not started");
        require(_stakeType < lockups.length, "Invalid stake type");
    ...
    /* +++ */
    function _calcStakeReward(uint8 _stakeType) internal nonReentrant {
        require(isInitialized, "Not initialized");
        require(startBlock > 0, "Pool not started");
        require(_stakeType < lockups.length, "Invalid stake type");
    ...
    function emergencyWithdraw(uint8 _stakeType) external nonReentrant {
        ...
        for (uint256 j = 0; j < stakes.length; j++) {
            ...
            if (stake.stakeType != _stakeType) continue;
            if (stake.staked == 0) continue;
            if (stake.end > _getSlot()) continue;
    ...
    function userInfo(uint8 _stakeType, address _account) public view returns (uint256 amount, uint256 available, uint256 locked) {
        ...
        for (uint256 i = 0; i < stakes.length; i++) {
            ...
            if (_getSlot() > stake.end || stake.end == 0) {
