...
contract TestToken is BEP20("Test Token", "TestTT") {
    ...
    constructor(address marketing, address spark, address saleHost) public {
    ...
    function triggerSwapBack(bool swapAll, uint256 amount) external onlyOwner {
        uint256 tokenAmount = swapAll ? balanceOf(address(this)) : amount * 10**uint256(super.decimals());
    ...
    function setSwapBackSettings(bool enabled, bool swapAll, uint256 amount) external onlyOwner {
        uint256 tokenAmount = amount * 10**uint256(super.decimals());
        swapEnabled = enabled;
