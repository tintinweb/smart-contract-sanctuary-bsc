...
contract StandardToken is ERC20, Ownable {
    ...
    constructor(
        ...
        uint256[2] memory _amountLimits
        ...
        require(_fees[0]+(_fees[3])<=300);
        require(_fees[1]+(_fees[4])<=300);
        require(_fees[2]+(_fees[5])<=300);
        ...
        maxTransactionAmount = _amountLimits[0];
        maxWalletAmount = _amountLimits[1];
    ...
    function updateLiquidityFee(
        ...
        require(
            _transferLiquidityFee+(transferMarketingFee) <= 300,
            "transfer fee <= 30%"
        );
    ...
    function updateMarketingFee(
        ...
        require(
            _transferMarketingFee+(transferLiquidityFee) <= 300,
            "transfer fee <= 30%"
        );
        ...
        if (!inSwapAndLiquify) {
            if (
                ...
                require(
                    ...
                    "transfer over max amount"
                );
                require(
                    amount+(balanceOf(to)) <= maxWalletAmount,
                    "Cannot exceed max wallet"
                );
            } else if (!isExcludedMaxTransactionAmount[from]) {
                require(
                    amount <= maxTransactionAmount,
                    "transfer over max amount"
                );
