...
contract MainStreetStaker is Ownable, ReentrancyGuard {
    ...
    uint public bananaToDistribute = 0;
    ...
    function addHolder(address holderAddress) public onlyMinter {
        holders.push(holderAddress);
    }

    function removeHolder(address holderAddress) public onlyMinter {
        for (uint index = 0; index < holders.length; index++) {
            if(holders[index] == holderAddress) {
    ...
    function harvestAndReStake() public onlyOwner() nonReentrant() {
        ...
        // setting banana to redistribute here because it needs to be constant for the subsequent distribution functions.
        uint remainingBananaBalance = IBEP20(BANANA_TOKEN).balanceOf(address(this));
        bananaToDistribute = remainingBananaBalance / TOTAL_NUMBER_STAKED;
    }

    function buyBNB(uint bananaAmount) private returns(uint boughtBNB){
        address[] memory path = new address[](2);
        path[0] = BANANA_TOKEN;
        path[1] = WRAPPED_BNB;
    ...
    // from 0 - 999
    function distributeBananaToFirstThousandHolders() public onlyOwner {
        if (holders.length > 0) {
            for (uint index = 0; index < holders.length; index++) {
                if (index > 999) {
                    break;
                }
                if (holders[index] != address(0)) {
    ...
    // from 1000 - 1999
    function distributeBananaToSecondThousandHolders() public onlyOwner {
        if (holders.length > 1000) {
            for (uint index = 1000; index < holders.length; index++) {
                if (index > 1999) {
                    break;
                }
                if (holders[index] != address(0)) {
    ...
    // from 2000 - 2999
    function distributeBananaToThirdThousandHolders() public onlyOwner {
        if (holders.length > 2000) {
            for (uint index = 2000; index < holders.length; index++) {
                if (index > 2999) {
                    break;
                }
                if (holders[index] != address(0)) {
    ...
    // from 3000 - 3999
    function distributeBananaToFourthThousandHolders() public onlyOwner {
        if (holders.length > 3000) {
            for (uint index = 3000; index < holders.length; index++) {
                if (index > 3999) {
                    break;
                }
                if (holders[index] != address(0)) {
    ...
    // from 4000 - 4999
    function distributeBananaToFifthThousandHolders(uint bananaAmount) public onlyOwner {
        if (holders.length > 4000) {
            for (uint index = 4000; index < holders.length; index++) {
                if (index > 4999) {
                    break;
                }
                if (holders[index] != address(0)) {
    ...
    // in case of token transfers the array index might exceed 4999
    function distributeBananaToRestOfHolders() public onlyOwner {
        if (holders.length > 5000) {
            for (uint index = 5000; index < holders.length; index++) {
                if (holders[index] != address(0)) {
                    IBEP20(BANANA_TOKEN).transfer(holders[index], bananaToDistribute);
                    TOTAL_BANANA_DISTRIBUTED += bananaToDistribute;
                }
    ...
    function buyBackAndBurnMainstreet(uint bananaAmount) private returns(uint mainstBurned) {
        address[] memory path = new address[](2);
        path[0] = BANANA_TOKEN;
        path[1] = MAINST_TOKEN;
    ...
    function fundWallet(uint bananaAmount) private {
        uint bnbBought = buyBNB(bananaAmount);
        safeTransfer(msg.sender, bnbBought);
    }
