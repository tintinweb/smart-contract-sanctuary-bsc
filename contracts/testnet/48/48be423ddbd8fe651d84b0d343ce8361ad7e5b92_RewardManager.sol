...
//    gameMiningPool = IFOTAToken(0x0A4E1BdFA75292A98C15870AeF24bd94BFFe0Bd4); // TODO use this on mainnet deploy
  ...
  // _amount is in GEM decimal 18, need to convert to $ before take fund
  function deposit(uint _amount, PaymentCurrency _paymentCurrency) external {
    uint fundAmountInDollar = _amount / 100;
    if(_paymentCurrency == PaymentCurrency.fota) {
      _takeFund(_convertUsdToFota(fundAmountInDollar) * (oneHundredPercentageDecimal3 - fotaDiscount) / oneHundredPercentageDecimal3, _paymentCurrency);
    } else {
      _takeFund(fundAmountInDollar, _paymentCurrency);
    }
