...
contract Marketplace1155 is Ownable {
    ...
    function buyToken(uint256 tokenId) external payable {
        ...
        if (assetContract == address(0x0)) {
            if (whitelist[idToAdr[tokenId]] > 0) {
                require(
                ...
                (bool success, ) = payable(idToAdr[tokenId]).call{
                    value: ((msg.value * (100 - whitelist[idToAdr[tokenId]])) /
                        100)
                }("");
                require(success);
            } else {
            ...
            if (whitelist[idToAdr[tokenId]] > 0) {
                token20.transferFrom(
                    msg.sender,
                    address(this),
                    (tokenIdToCost[tokenId] * whitelist[idToAdr[tokenId]]) / 100
                );
                ...
                token20.transferFrom(
                    ...
                    (tokenIdToCost[tokenId] *
                        (100 - whitelist[idToAdr[tokenId]])) / 100
                );
            } else {
                token20.transferFrom(
                    msg.sender,
