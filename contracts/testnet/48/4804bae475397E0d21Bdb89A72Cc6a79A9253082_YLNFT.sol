...
contract YLNFT is ERC721URIStorage, EIP712, Ownable, AccessControl, ReentrancyGuard {
    ...
    event PauseContract(address admin, address minted721contract, uint256 timestamp);
    event UnpauseContract(address admin, address minted721contract, uint256 timestamp);
    event Transfer721to(address admin, address recipient, uint256 tokenId);

    constructor() ERC721("YourLifeGame NFT", "YLNFT") EIP712("LazyNFT-Voucher", "1") {
        _ylnft721Owner = payable(msg.sender);
        marketAddress = 0xDAF325F0988c1d061a7a2542daDF232Dac476B1d;
        proxy = IProxy(0xDAF325F0988c1d061a7a2542daDF232Dac476B1d);
        _setupRole(MINTER_ROLE, _ylnft721Owner);
    }

    function setProxyAddress(address _proxyAddress) public onlyOwner returns(bool) {
        proxy = IProxy(_proxyAddress);
        return true;
    }

    function setMarketAddress(address _marketAddress) public onlyOwner returns(bool) {
        marketAddress = _marketAddress;
        return true;
    }

    function setCategoryAmount(string memory _sport, string memory _cnft, uint256 _amount) public onlyOwner returns (bool) {
        categoryAmount[_sport][_cnft] = _amount;
        return true;
    }

    function deleteCategory(string memory _sport, string memory _cnft) public onlyOwner returns (bool) {
        delete categoryAmount[_sport][_cnft];
        return true;
    }

    function getCategoryCount(string memory _sport, string memory _cnft) public view returns (uint) {
        return categoryCount[_sport][_cnft];
    }

    function setPauseContract(bool _yltpause) public returns (bool) {
        require(proxy.isPauseAccount(msg.sender), "you can't pause this contract, please contact the Admin");
        yltpause = _yltpause;
        if(yltpause == true) {
    ...
    //transfer
    function ylnft721Transfer(address _to, uint256 _tokenId) public nonReentrant returns(bool) {
        require(proxy.isTransferAccount(msg.sender), "you can't transfer YL NFT, please contact the Admin");
        require(getPauseContract() == true, "NFT721 Contract was paused!");
        require(_to != address(0), "Can't transfer NFT721 to address(0)");

        address _tokenOwner = ownerOf(_tokenId);
        // if(isApprovedForAll(_tokenOwner, msg.sender) == false) {
        //     setApprovalForAll(msg.sender, true);
        // }

        transferFrom(_tokenOwner, _to, _tokenId);
        emit Transfer721to(_tokenOwner, _to, _tokenId);
        return true;
    }

    //mint
    function createToken(string memory tokenURI, string memory _sport, string memory _cnft) public returns (uint) {
        uint256 startGas = gasleft();

        require(proxy.isMintableAccount(msg.sender), "you can't mint YLT NFT, please contact the Admin");
        require(categoryAmount[_sport][_cnft] > categoryCount[_sport][_cnft], " Overflow! Amount of NFT category");
    ...
    function withdraw(address payable _to, uint256 _value) public nonReentrant onlyOwner {
    ...
    function burnNFT721(uint256 _tokenId) public returns (bool) {
        uint256 startGas = gasleft();
        require(proxy.isBurnAccount(msg.sender), "you can't transfer YL NFT, please contact the Admin");
        require(getPauseContract() == true, "NFT721 Contract was paused!");
    ...
    //lazy minting part
    function lazyCreateToken(/*NFTVoucher calldata voucher*/string memory _tokenURI, /*bytes memory signature,*/ string memory _sport, string memory _cnft) public payable returns (uint) {
        require(categoryAmount[_sport][_cnft] < categoryCount[_sport][_cnft], " Overflow! Amount of NFT category");
        require(getPauseContract() == true, "NFT721 Contract was paused!");
    ...
    function burnNFT721Signature(uint256 _tokenId) public returns(uint256){
        ...
        require(burnAddress[_tokenId][msg.sender] == false, "Already you signed");
