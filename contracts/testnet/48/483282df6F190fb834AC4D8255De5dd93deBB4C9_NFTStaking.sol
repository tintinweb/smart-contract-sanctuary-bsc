...
contract NFTStaking is Ownable {
    ...
    bool public isStakingStart = false;
    
    function startStaking() public onlyOwner{
        require(isStakingStart==false, "Staking Already Started");
        isStakingStart = true;
    }
    function changeToken(address _stakedToken,address _rewardToken) public  onlyOwner{
        stakedToken = IERC721(_stakedToken);
        rewardToken = IBEP20(_rewardToken);
    }
    function pauseStaking() public onlyOwner{
        require(isStakingStart==true, "Staking Already Paused");
    ...
    function userInfo(uint256 tokenId) internal{
        uint256 _currentId = currentIdOfUser[msg.sender];
        stakesOfUser[msg.sender][_currentId].tokenId=tokenId;
        stakesOfUser[msg.sender][_currentId].amount=uint8(1);
        stakesOfUser[msg.sender][_currentId].userAddress=msg.sender;
        stakesOfUser[msg.sender][_currentId].time=block.timestamp;
    ...
    function deposite(uint256 tokenId) public  {
        require(isStakingStart == true, "Staking is not Start yet");
    ...
    function calclulateReward(address account, uint256 id) public view returns(uint256){
        ...
        uint256 depositeTime = stakesOfUser[account][id].time;
    ...
    function claim(uint256 id) public{
        ...
        uint256 totalAmount= stakesOfUser[msg.sender][id].amount + reward;

        require(rewardToken.balanceOf(address(this))>=totalAmount,"Insufficent Balance");
        
        stakesOfUser[msg.sender][id].isWithdrawn = true;
        rewardToken.safeTransfer(msg.sender,totalAmount);
        stakedToken.transferFrom(address(this),msg.sender,stakesOfUser[msg.sender][id].tokenId);
    }
    
    constructor() {
        tokenPerSecond = 1000000000000;  //0.033
        stakedToken = IERC721(0x8F83BfA2B234e72515876569344de4FC813E8335);
        rewardToken = IBEP20(0x514B904A2CFCc342c6aea38aca38b8a4B6E893cd); 
    }
}