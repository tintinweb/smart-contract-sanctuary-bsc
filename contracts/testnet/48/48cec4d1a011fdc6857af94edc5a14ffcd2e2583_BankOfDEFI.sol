...
contract BankOfDEFI is Ownable, SafeMath {
    ...
    function stakePeriod() public view returns (uint) {
        /* Get the balance of the sender of this transaction */
        return fixedtime[msg.sender];
    }
    ...
    function userRefRewards() public view returns (uint) {
        /* Get the referal rewards of the sender of this transaction */
    ...
    function deposit(uint amtofTokens) external checkRegistered returns (uint) {
        // require(enrolled[msg.sender] == true, "Please enroll first");
    ...
    function withdraw(uint withdrawAmount) external checkRegistered returns (uint accountBalance) {
        //    require(enrolled[msg.sender] == true, "Please enroll first");
    ...
    function sendFunds(uint256 _amt, address _receiver) external checkRegistered returns (uint accountBalance) {
        // require(enrolled[msg.sender] == true, "Please enroll first");
    ...
    function stake(uint256 _tokens_amount) private {
        ...
        require(_tokens_amount <= balances[msg.sender], "Insufficient Funds");
        balances[msg.sender] -= _tokens_amount;
    ...
    function initStake(uint256 _token, uint256 _time) external checkIfStakingStopped checkRegistered {
        stake(_token);
    ...
    function claimReward() external checkRegistered {
        // require(enrolled[msg.sender] == true, "Please enroll first");
        ...
        if (pendingReward > 0) {
            ...
            balances[msg.sender] += pendingReward; //update user balance

            emit claimedReward(msg.sender, pendingReward);
        }
    }

    function unstake(uint256 _tokens_amount) external checkRegistered {
        // require(enrolled[msg.sender] == true, "Please enroll first");
        require(_tokens_amount > 0 && stakers[msg.sender].stakedTokens >= _tokens_amount, "Invalid token amount to unstake.");
        require(fixedtime[msg.sender] < block.timestamp, "Wait Till Stake Time");
        balances[msg.sender] += _tokens_amount;
