...
contract TokenSwapper is ISwapper {
    using BoringMath for uint256;
    ...
    uint256 constant deadline = 2 ** 256 - 1;
    ...
    function swap(
        ...
        // withdraw collateral swap to dollar
        (uint256 amountFrom,) = bentoBox.withdraw(fromToken, address(this), address(this), 0, shareFrom);
        IERC20(fromToken).approve(address(router), amountFrom);
        uint256 blBefore = toToken.balanceOf(address(bentoBox));
        ...
        uint256 blAfter = toToken.balanceOf(address(bentoBox));
        uint256 amountTo = blAfter.sub(blBefore);
