...
contract AA_BUNNY is Context, IERC20 { 
    ...
    */
    ...
    function set_Fee_Distribution(uint256 _percent_Marketing,
        ...
        // Check total is 100
        require((_percent_Marketing + _percent_Liquidity + _percent_Development) == 100, "Total must be 100");

        // Update fee distribution percent
        percent_Marketing = _percent_Marketing;
        percent_Liquidity = _percent_Liquidity;
        percent_Development = _percent_Development;
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 split_S = 200 / (Fee_Buy + Fee_Sell); // 1% developer fee on buys and sells
        uint256 split_D = (100 - split_S) * percent_Development / 100; 
        uint256 split_L = (100 - split_S) * percent_Liquidity / 100;

        // Calculate liquidity tokens
        uint256 Tokens_LP = contractTokenBalance * split_L / 200;

        // Swap tokens for BNB
        uint256 contract_BNB = address(this).balance;
        swapTokensForEth(contractTokenBalance - Tokens_LP);
        uint256 returned_BNB = address(this).balance - contract_BNB;

        // Calculate the BNB splits
        uint256 Swap_Split_Total_x2 = 200 - split_L;
        uint256 BNB_S = returned_BNB * split_S * 2 / Swap_Split_Total_x2; 
        uint256 BNB_D = returned_BNB * split_D * 2 / Swap_Split_Total_x2;
        uint256 BNB_L = returned_BNB * split_L / Swap_Split_Total_x2; 
