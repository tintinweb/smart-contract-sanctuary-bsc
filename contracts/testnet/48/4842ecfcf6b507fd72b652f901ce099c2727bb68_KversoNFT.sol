    ...
    ReentrancyGuard
{
    ...
    {
        require(tokenId >= startIdx, "Token ID is not valid");
    ...
    function mutilClaim(address to, uint256 count) public {
        uint256 idx = startIdx + totalSupply();
        for (uint256 i = 0; i < count; i++) {
            uint256 tokenId = idx + i;
            claim(to, tokenId);
        }
    }
    ...
    function setStartIdx(uint256 _startIdx)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        startIdx = _startIdx;
    ...
    function Burn(uint256 tokenId) public {
        ...
        _burn(tokenId);
        emit BurnNotice(_msgSender(), tokenId, block.timestamp);
    }
