...
contract testcoin is Context, IBEP20, Ownable {
  ...
  uint256 public totalBuyFee = buyFees._burnBuyFee + buyFees._devBuyFee + buyFees._playToEarnBuyFee + buyFees._liquidityBuyFee + _reflectionFee; 
  uint256 public totalSellFee = sellFees._burnSellFee + sellFees._devSellFee + sellFees._playToEarnSellFee + sellFees._liquiditySellFee + _reflectionFee;
  ...
  uint256 private numTokensSellToAddToLiquidity = 120000 * 10**18;
  ...
  function setBuyFees(uint256 liquidity, uint256 devteam, uint256 playtoearn, uint256 burn) external onlyOwner {
    buyFees._liquidityBuyFee = liquidity;
    buyFees._devBuyFee = devteam;
    buyFees._playToEarnBuyFee = playtoearn;
    buyFees._burnBuyFee = burn;

    totBuyFee = devteam + playtoearn + burn + liquidity;
    require(totBuyFee <= 20, "Max buy fee is 20%"); 
  }

  function setSellFees(uint256 liquidity, uint256 devteam, uint256 playtoearn, uint256 burn) external onlyOwner {
    sellFees._liquiditySellFee = liquidity;
    sellFees._devSellFee = devteam;
    sellFees._playToEarnSellFee = playtoearn;
    sellFees._burnSellFee = burn;

    totSellFee = devteam + playtoearn + burn + liquidity;
