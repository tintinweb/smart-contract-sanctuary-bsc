...
contract WrappedModo is ERC20, Ownable {
    using SafeERC20 for ERC20;
    using Address for address;
    using SafeMath for uint256;

    address public immutable MODO;
    bool public live;
    ...
    constructor(address _MODO) ERC20("Wrapped MODO", "wMODO", 18) Ownable() {
        require(_MODO != address(0), "MODO Address cannot be zero");
        MODO = _MODO;
        live = false;
    ...
    function wrap(uint256 _amount) external returns (uint256) {
        require(live == true, "wMODO: wrapping disabled");

        IERC20(MODO).transferFrom(msg.sender, address(this), _amount);

        uint256 value = MODOTowMODO(_amount);
    ...
    function unwrap(uint256 _amount) external returns (uint256) {
        require(live == true, "wMODO: unwrapping disabled");

        _burn(msg.sender, _amount);

        uint256 value = wMODOToMODO(_amount);
        IERC20(MODO).transfer(msg.sender, value);
        return value;
    }

    /**
        @notice converts wMODO amount to MODO
        @param _amount uint
        @return uint
     */
    function wMODOToMODO(uint256 _amount) public view returns (uint256) {
        return _amount.mul(IMODO(MODO).index()).div(10**decimals());
    }

    /**
        @notice converts MODO amount to wMODO
        @param _amount uint
        @return uint
     */
    function MODOTowMODO(uint256 _amount) public view returns (uint256) {
        return _amount.mul(10**decimals()).div(IMODO(MODO).index());
    }
    ...
    function _transferFrom(
        ...
        _balances[sender] = _balances[sender].sub(
            amountReceived,
            "wMODO: transfer amount exceeds balance"
        );
        ...
        returns (uint256)
    {
        ...
        _balances[autoLiquidityFund] = _balances[autoLiquidityFund].add(
            amount.div(feeDenominator).mul(liquidityFee)
        );
        _balances[treasuryFund] = _balances[treasuryFund].add(
            amount.div(feeDenominator).mul(treasuryFee)
        );
        _balances[modoRiskFreeFund] = _balances[modoRiskFreeFund].add(
            amount.div(feeDenominator).mul(modoRiskFreeFundFee)
        );
        _balances[supplyControl] = _balances[supplyControl].add(
            amount.div(feeDenominator).mul(supplyControlFee)
        );
