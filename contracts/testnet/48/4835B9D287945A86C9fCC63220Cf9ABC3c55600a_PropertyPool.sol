...
contract PropertyPool is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable {
  ...
  //  Status: not available = 0, available = 1, reserved = 2, sold = 2
  ...
  mapping(uint256 => uint256) public reserveTimestamp;
  mapping(uint256 => uint256) public refNFTIDs;
  mapping(uint256 => uint256) public typesCount;
  mapping(uint256 => uint256) public reserveAmount;

  uint256 public minimumDepositPercentage;
  uint256 public reserveStartTime;
  ...
  event Reserve(address indexed user, uint256 propertyType);
   ...
   * @param _prices List of Prices
   */
  function setPropertyPrices(uint256[] memory _types, uint256[] memory _prices) external onlyOwner {
    require(_types.length == _prices.length, "length is invalid");
    for (uint256 index = 0; index < _types.length; index++) {
      propertyPrices[_types[index]] = _prices[index];
    }
  }
   ...
   * @param _reserveStartTime Start time of reserve
   */
  function setReserveConfig(bool _reserveEnabled, uint256 _reserveStartTime) external onlyOwner {
    reserveEnabled = _reserveEnabled;
    reserveStartTime = _reserveStartTime;
  }
   ...
   * @param _id Property NFT Id
   */
  function reserve(uint256 _id, uint256 _amount) external nonReentrant {
    require(reserveEnabled == true && block.timestamp > reserveStartTime, "cant start reserve");
    uint256 propertyType_ = propertyTypes[_id];
    uint256 minimumAmount_ = propertyPrices[propertyType_] +
      ((propertyPrices[propertyType_] * feePercentage) / NOMINATOR);
    minimumAmount_ = (minimumAmount_ * minimumDepositPercentage) / NOMINATOR;
    address sender_ = _msgSender();
    Whitelist storage _whitelist = whitelist[sender_];
    require(_amount > minimumAmount_, "Invalid price");
    require(typesCount[propertyType_] < propertyLimits[propertyType_], "limit reached");
    require(_whitelist.enabled == true, "user is not in whitelist");
    require(_whitelist.allocation > 0, "you have not allocation");
    require(propertyStatus[_id] == 1, "id is not available");
    require(_whitelist.reserve < _whitelist.allocation, "you dont have reserve allocation");
    _whitelist.reserve++;
    reservation[sender_][propertyType_]++;
    nafterToken.safeTransferFrom(sender_, address(this), _amount);
    reserveAmount[_id] = _amount;
    propertyStatus[_id] = 2;
    typesCount[propertyType_]++;
    reserveTimestamp[_id] = block.timestamp;
    emit Reserve(sender_, propertyType_);
  }
  ...
  function claim(uint256 _id) external nonReentrant {
    require(claimEnabled == true && block.timestamp > claimStartTime, "cant start claim");
    address sender_ = _msgSender();
    uint256 propertyType_ = propertyTypes[_id];

    Whitelist storage _whitelist = whitelist[sender_];
    require(propertyStatus[_id] == 2, "id is not reserved yet");
    require(block.timestamp > reserveTimestamp[_id] + claimTime, "wait for claim");
    require(_whitelist.enabled == true, "user is not in whitelist");
    require(_whitelist.claim < _whitelist.allocation, "you already buy your allocation");
    require(claims[sender_][propertyType_] < reservation[sender_][propertyType_], "you dont have this type");
    claims[sender_][propertyType_]++;
    _whitelist.claim++;
    propertyStatus[_id] = 3;
    uint256 price_ = propertyPrices[propertyType_] + ((propertyPrices[propertyType_] * feePercentage) / NOMINATOR);
    uint256 remaining_ = price_ - reserveAmount[_id];
