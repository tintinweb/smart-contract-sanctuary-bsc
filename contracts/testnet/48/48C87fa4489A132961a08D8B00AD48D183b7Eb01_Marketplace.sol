...
contract Marketplace is Ownable, ReentrancyGuard {
    ...
    function getArtlux() public view returns(address){
        return ArtLuxNFTAddr;
    }
    ...
    function burnToken(uint256 tokenId) public onlyOwner returns(bool){
        if(ArtLuxNFT(ArtLuxNFTAddr).burnToken(tokenId)){
            delete(idToMarketItem[tokenId]);
            removeItemsbyHolder(msg.sender,tokenId);
            return true;
        }
        return false;
    } 
    ...
    function updatePriceById(uint256 _id, uint256 _price,bool listed) public onlyHolder(_id) {
        ...
        if(!idToMarketItem[_id].listed&&listed){
            Count_listed.increment();
        }else if(idToMarketItem[_id].listed&&!listed){
            Count_listed.decrement();
        }
    ...
    // function lazyPurchaseItem (uint256 _id,vouch) public payable {
        
    // }
    function purchaselazyMintItem(address redeemer,ArtLuxNFT.NFTVoucher calldata voucher) public payable{
        // require(redeemer != ArtLuxNFT(ArtLuxNFTAddr).owner, " You are owner of ArtluxNFT! ");
        uint256 _tokenId=ArtLuxNFT(ArtLuxNFTAddr).redeem{value:msg.value}(redeemer,voucher);
        uint256 id = Count_Minted.current();
        idToMarketItem[id] =  MarketItem(
            ArtLuxNFTAddr,
            _tokenId,
            payable(msg.sender),
            voucher.minPrice*1 ether,
            false
        );
    ...
    function purchaseItem(uint256 _id) public payable {
        ...
        require(idToMarketItem[_id].listed == true,"no listed");
        ...
        uint256 totalprice = idToMarketItem[_id].price * (100-royalties) / 100;
        ...
        idToMarketItem[_id].listed=false;
        Count_listed.decrement();
        // address marketplace = ArtLuxNFT(ArtLuxNFTAddr).getArtluxMarketplaceAddress();
        // ArtLuxNFT(ArtLuxNFTAddr).setApprovalForAll(marketplace, true);
    }
...
contract ArtLuxNFT is ERC721URIStorage,EIP712, AccessControl,Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;
    Counters.Counter private _tokenIds;
    string public _artluxURI;   //base uri
    address private _artluxMarket;

    //----------------- lazy minting
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    string private constant SIGNING_DOMAIN = "ArtLuxNFT-Vouter";
    string private constant SIGNATURE_VERSION = "1";
    //------------
    constructor(address payable minter) 
        ERC721("ArtLuxNFT", "ALN") 
    ...
    function setArtluxMarketplaceAddress(address addr) public onlyOwner{
        _artluxMarket = addr;
    }
    function getArtluxMarketplaceAddress () public view returns(address){
        return _artluxMarket;
    }
    ...
    function mintToken(string memory token_uri) public onlyOwner returns(uint256) {        
        ...
        _tokenIds.increment();
        //  setApprovalForAll(_artluxMarket, true);
        // setApprovalForAll(_artluxMarket, true);
        return newItemId;
    }

    function burnToken ( uint256 token_id) public onlyOwner returns(bool) {
        if(!_exists(token_id))
        return false;
        _burn(token_id);
        return true;
