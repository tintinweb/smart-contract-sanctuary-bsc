...
contract Test {
    ...
    constructor(IUniswapV2Router02[] memory routers_) {
        routers = routers_;
    }
    ...
    function quote(
        ...
        address tokenIn,
        address tokenOut
    ) external view returns (uint amountOut, IUniswapV2Router02 router, address[] memory path) {
        path[0] = address(tokenIn);
        path[1] = address(tokenOut);
        uint _amountOut = 0;
        for (uint i = 0; i < routers.length; i++) {
            _amountOut = routers[i].getAmountsOut(amountIn, path)[1];
    ...
    function swap(
        ...
        amountOut = router.swapExactTokensForTokens(amountIn, amountOutMin, path, msg.sender, block.timestamp + 100)[path.length - 1];
    }
}
