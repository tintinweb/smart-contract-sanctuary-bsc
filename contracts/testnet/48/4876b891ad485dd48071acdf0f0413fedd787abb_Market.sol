...
contract Market is Ownable {
    ...
    mapping(uint256 => TokenSellingInfo) public tokens;

    struct TokenSellingInfo {
        address seller;
        uint256 price;
        uint32 expireTime;
    }
    ...
    function sellToken(uint256 tokenId, uint256 price, uint32 expireTime) public {
        require(address(NFTAddress) != address(0), "Address NFT must set");
        require(isApproved(tokenId), "Token has not been approved");
        totalTokens += 1;
        tokens[tokenId] = TokenSellingInfo(msg.sender, price, expireTime);
        NFTAddress.safeTransferFrom(msg.sender, owner(), tokenId);
    }

    function approveSellToken(uint256 tokenId) public {
        NFTAddress.approve(address(this), tokenId);
    }

    function isApproved(uint256 tokenId) view public returns(bool) {
