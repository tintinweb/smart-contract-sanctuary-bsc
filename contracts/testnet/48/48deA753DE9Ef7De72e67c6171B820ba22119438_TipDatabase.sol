...
contract TipDatabase is NBLGovernance {
    ...
    function amountNobilityReceived(address user) external view returns (uint256) {
        return userInfo[user].amountNBLReceived;
    }

    function amountNobilitySent(address user) external view returns (uint256) {
        return userInfo[user].amountNBLSent;
    }
