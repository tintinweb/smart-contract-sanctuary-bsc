...
contract PrivateSale is OwnableUpgradeable, IERC20 {
    IPrivateSale public token;
    ERC20 public tokenERC20;
    ERC20 public zeroAddress;
    uint256 public exchange;
    uint256 public depositDeadline;
    uint256 public lockDuration;

    mapping(address => ERC20) internal quotes;
    ...
    function initialize(
        ...
        token = IPrivateSale(_baseToken);
        tokenERC20 = ERC20(_baseToken);
        zeroAddress = ERC20(address(0));

        quotes[_tokenBUSD] = ERC20(_tokenBUSD);
        quotes[_tokenUSDC] = ERC20(_tokenUSDC);
        quotes[_tokenUSDT] = ERC20(_tokenUSDT);
    ...
    function updateQuote(address _quote, address _newQuote) public onlyOwner {
        ...
        _removeQuote(_quote);
        quotes[_newQuote] = ERC20(_newQuote);
    }

    function _removeQuote(address _quote) internal {
        quotes[_quote] = zeroAddress;
    }
    ...
    function deposit(address asset, uint256 amount) public {
        ...
        require(IERC20(asset) == quotes[asset], "You don't have enough money.");

        uint256 quote = amount * exchange;

        if (!quotes[asset].transferFrom(msg.sender, owner(), quote)) {
        ...
        if (!token.privateSaleTransferFrom(msg.sender, address(this), amount)) {
            revert TransferFailed();
        }
    ...
    function withdraw(uint256 amount) public {
        ...
        if (!token.privateSaleTransfer(msg.sender, amount)) {
            revert TransferFailed();
        }
