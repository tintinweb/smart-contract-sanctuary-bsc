...
contract FurAveragePriceOracle is
    BaseContract,
    IFurAveragePriceOracle
{
    ...
    uint224 private furUSDCPriceAverage;
    uint256 private furUSDCCumulativeLast;
    bool private furIsToken0;

    IERC20Upgradeable private FurToken;
    IUniswapV2Pair private FurUsdcLpToken;
    ...
    function setAddresses(
        address _furTokenAddress,
        address _furUsdcLpToken
    ) public onlyOwner {
        require(
            IUniswapV2Pair(_furUsdcLpToken).token0() == _furTokenAddress ||
            IUniswapV2Pair(_furUsdcLpToken).token1() == _furTokenAddress,
            "LA"
        );
        FurToken = IERC20Upgradeable(_furTokenAddress);
        FurUsdcLpToken = IUniswapV2Pair(_furUsdcLpToken);
        furIsToken0 = FurUsdcLpToken.token0() == _furTokenAddress;
    }
    ...
    {
        return (furUSDCPriceAverage * 1e18) / Q112;
    }


    function updateFurUSDCPrice() external override whenNotPaused {
        (
            uint256 _price0Cumulative,
            uint256 _price1Cumulative,
            uint32 _blockTimestamp
        ) = currentCumulativePrices(FurUsdcLpToken);
        ...
        if (blockTimestampLast == 0) {
            (
                uint112 _reserve0,
                uint112 _reserve1,
            ) = FurUsdcLpToken.getReserves();

            if (furIsToken0) {
                furUSDCPriceAverage = (Q112 * _reserve0) / _reserve1;
                furUSDCCumulativeLast = _price1Cumulative;
            } else {
                furUSDCPriceAverage = (Q112 * _reserve1) / _reserve0;
                furUSDCCumulativeLast = _price0Cumulative;
            }
        ...
        if (_timeElapsed >= TIME_PERIOD) {
            if (furIsToken0) {
                furUSDCPriceAverage = uint224(
                    (_price1Cumulative - furUSDCCumulativeLast) / _timeElapsed
                );
                furUSDCCumulativeLast = _price1Cumulative;
            } else {
                furUSDCPriceAverage = uint224(
                    (_price0Cumulative - furUSDCCumulativeLast) / _timeElapsed
                );
                furUSDCCumulativeLast = _price0Cumulative;
            }
            blockTimestampLast = _blockTimestamp;
