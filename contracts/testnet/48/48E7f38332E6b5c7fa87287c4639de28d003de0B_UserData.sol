...
contract UserData is IUserData, Secure {
  ...
  constructor() {
    ...
    users[_msgSender()].latestWithdraw = block.timestamp.toUint64();
  ...
  function registerAndInvest(
    ...
    require(register(user, ref), "USER::RFA");
  ...
  // Modifier functions ----------------------------------------------------------
  function subTotalAmount(address user, uint256 value)
    public
    override
    onlyContract
  {
    require(exist(user), "USER::USN");
    changeTotalAmount(user, users[user].totalAmount.sub(value));
  }
  ...
  function deleteUserInvestIndex(address user, uint256 index)
    ...
    onlyContract
  {
    (uint256 amount, , , , ) = depositDetail(user, index);
    subTotalAmount(user, amount);
    ...
    returns (uint256 rewards)
  {
    ...
    for (uint8 i = 0; i < userIvestLength; i++) {
      (, , uint256 reward, uint256 startTime, ) = depositDetail(user, i);
  ...
  function refAmount(address user) public view override returns (uint256) {
    return users[user].refAmount;
  }

  function giftAmount(address user) public view override returns (uint256) {
    return users[user].giftAmount;
  }

  function totalAmount(address user) public view override returns (uint256) {
