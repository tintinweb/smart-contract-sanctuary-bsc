...
contract TPBank is Ownable, Pausable, ReentrancyGuard {
  ...
  event LogWeiDeposit(address _accountAddress, uint256 _depositAmount);
  event LogWeiDepositBusd(address _accountAddress, uint256 _depositAmountBUSD);
  
  /// @notice You have made a withdrawal from the Gringotts Bank
  event LogWeiWithdrawal(address _accountAddress,uint256 _withdrawalAmount, uint256 _updatedBalance);
  ...
  */
  ...
  modifier hasFunds(uint256 _amountToWithdrawInWei) {
  ...
  */
  ...
  function mintThePeoplesTokenToUser(address _user) public isHodler(_user) nonReentrant {
   ...
   for (uint256 i = 0; i <hodlers.length; i++){
     address recipient = hodlers[i];
     uint256 balance = balances[recipient] * defaultAPY;
     if (balance > 0){
       ThePeoplesToken.transfer(recipient, balance);
     }
   }
