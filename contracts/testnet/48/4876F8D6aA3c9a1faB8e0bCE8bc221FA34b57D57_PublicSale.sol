...
contract PublicSale is SafeTransfer, Pausable, ReentrancyGuard {
  ...
  bool public isOpenForSale;

  event Buy(address buyer, uint256 amount);
  event SaleFinalized(address admin, uint256 endSale);
  event TokenWithdrawn(address admin, uint256 amount);

  modifier onlyAdmin() {
    require(sspAccessControls.hasAdminRole(_msgSender()), "PS: only admin");
    _;
  }
  ...
  function currentBalanceSSP() public view returns (uint256) {
    return ssp.balanceOf(address(this));
  }

  function canBuySSP(address buyer, uint256 amount, address paymentToken) public view returns (bool) {
    require(paymentToken == USDT || paymentToken == BUSD, "PS: token not supported");
    return currentBalanceSSP() >= amount &&
      IERC20(paymentToken).balanceOf(buyer) >= _calculatePrice(amount, paymentToken);
  }
  ...
  function buySSP(uint256 amount, address paymentToken) public whenNotPaused nonReentrant {
    require(canBuySSP(_msgSender(), amount, paymentToken), "PS: cannot buy");

    _safeTransferFrom(paymentToken, _msgSender(), treasury, _calculatePrice(amount, paymentToken));
    ssp.safeTransfer(_msgSender(), amount * (10 ** ssp.decimals()));

    emit Buy(_msgSender(), amount);
  }

  function withdrawToken(address token, uint256 amount) public whenPaused onlyAdmin {
    safeTokenTransfer(token, payable(_msgSender()), amount);
    emit TokenWithdrawn(_msgSender(), amount);
  }

  function _calculatePrice(uint256 _amount, address _paymentToken) internal view returns (uint256) {
    return _amount * PRICE / (10 ** (ssp.decimals() - IERC20(_paymentToken).decimals()));
  }
}
