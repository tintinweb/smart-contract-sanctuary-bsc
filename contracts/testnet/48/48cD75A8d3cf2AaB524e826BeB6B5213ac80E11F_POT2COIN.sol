...
contract POT2COIN is Context, IERC20, Ownable {
    ...
    string private constant _name = "POT2COIN";
    string private constant _symbol = "POT2";
    ...
    address payable private _treasuryWallet = payable(0x7f30174D3Ca49052eE11dAbFE709E7E87E9F0227);
    address payable private _opsWallet = payable(0x2c3035BD0FAAAAC815Da67cf334f621967A1D580);
    ...
    function sendETHToFee(uint256 amount) private {
        uint256 totalfee = buyTreasuryFee.add(sellTreasuryFee).add(buyOpsFee).add(sellOpsFee);
        uint256 totaltreasuryfee = buyTreasuryFee.add(sellTreasuryFee);
        
        uint256 treasuryAmt = amount.mul(totaltreasuryfee).div(totalfee);
        _treasuryWallet.transfer(treasuryAmt);
        _opsWallet.transfer(amount.sub(treasuryAmt));
    }

    

    function openTrading(bool enabletrade) public onlyOwner() {
       tradingOpen = enabletrade;
        swapEnabled = enabletrade;
         require(tradingOpen, "Trading is not open yet wait for some time.");
    ...
    function setBuyFee(uint256 newBuyTreasuryFee, uint256 newBuyOpsFee) public onlyOwner {
        require(newBuyTreasuryFee.add(newBuyOpsFee) <= 30, "Must keep buy taxes below 30%");
    ...
    function setSellFee(uint256 newSellTreasuryFee, uint256 newSellOpsFee) public onlyOwner {
        require(newSellTreasuryFee.add(newSellOpsFee) <= 30, "Must keep sell taxes below 30%");
