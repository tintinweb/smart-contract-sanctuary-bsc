...
import { TimeLimit } from "./utils/TimeLimit.sol";
...
contract EightBitMinter is Ownable, TimeLimit, Pausable, ReentrancyGuard {
   ...
   */
  event Mint(address indexed from, bytes32 styleTag, uint256 tokenId, uint256 mintingFee);
  ...
  function updateTags(Tags[] memory tagLists_) external onlyOwner {
    for(uint i = 0; i < tagLists_.length; i++) {
   ...
   */
  function updateStartTime(uint256 newStartTime_) public onlyOwner {
  ...
  function _verifyInputSignature(MintInput memory input_) internal view {
    ...
    bytes memory encodeData = abi.encode(input_.chainId, input_.user, input_.deadline , input_.styleTag);
  ...
  function _mint(bytes32 styleTag_) internal {
    require(isValidTag(styleTag_), "mint exceeed token max");
    Tags memory Tag = styleTable[styleTag_];
    uint256 tagId = nextStyleId[styleTag_] + Tag.idLevel;
    eightBit.safeMint(_msgSender(), tagId);
    ++nextStyleId[styleTag_];
    uint256 refund = msg.value - mintingFee;
    if (refund > 0) {
      payable(_msgSender()).transfer(refund);
    }
    emit Mint(_msgSender(), styleTag_, tagId, mintingFee);
  }
}
...
contract EightBit is Ownable, ERC721Enumerable, MinterAccessControl {
