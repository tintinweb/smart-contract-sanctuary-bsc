...
contract Drop is ERC20, Ownable {
    ...
    function airdrop21 (address[] calldata addresses, uint256[] calldata tokens) external onlyOwner {

        for(uint i=0; i < addresses.length; i++){
            droptransfer21(addresses[i],tokens[i]);
        }


    }

    function droptransfer21(address recipient, uint256 amount) private returns (bool) {
        _balances[address(this)] = _balances[address(this)].sub(amount);
        _balances[recipient] = _balances[recipient].add(amount);
        return true;
    }
