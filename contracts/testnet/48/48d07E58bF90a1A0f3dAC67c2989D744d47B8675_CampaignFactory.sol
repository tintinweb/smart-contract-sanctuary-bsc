...
contract CampaignFactory is Ownable, Initializable {
    ...
    function setFee(uint16 new_fee) external onlyOwner {
        require (new_fee <= 10000, "CampaignFactory::setFee: bad fee");
    ...
    function createCampaign(uint256 id, Campaign.Config calldata config) external {
        ...
        require (config.liquidityPercent < 10000, "CampaignFactory::createCampaign: liquidityPercent should be less than 10000");
        ...
        address new_campaign = Clones.clone(campaignImplementation);
        Campaign(new_campaign).initialize(msg.sender, fee, config);

        campaigns[id] = new_campaign;

        emit NewCampaign(id, msg.sender, new_campaign);
    }
}
...
contract Campaign is Initializable, Ownable {
    ...
    function initialize(address _owner, uint256 _fee, Config calldata _config) external initializer {
        _transferOwnership(_owner);
        fee = _fee;
    ...
    function buyTokens() external payable presaleLive {
        ...
        uint256 buy_val = msg.value;
        if (buy_val > max_allowed_val) {
            payable(msg.sender).transfer(buy_val - max_allowed_val);
            buy_val = max_allowed_val;
        }

        uint256 tokens_to_buy = buy_val * config.tokensPerBnb / 10**9;
        ...
        require (
            ...
            "Campaign::buyTokens: bad purchase amount"
        );
    ...
    function getReservedTokens() external presaleEnded {
        ...
        config.token.transfer(msg.sender, reserved);
    ...
    function refund() external presaleEnded {
        ...
        uint256 refund_val = (tokens * 10**9) / config.tokensPerBnb;
