...
contract Game is AccessControl,Ownable {
    ...
    function fighting(uint256 tokenId,uint256 enemyId) public isteam(tokenId,msg.sender) {
        ...
        uint256 xp;
        uint256 unLkTime;
        (suc,reward,hp,xp,unLkTime) = _monster.fighting(tokenId,enemyId,msg.sender);
        if(suc == true){
            ...
            _tokenDetail[tokenId].hp = hp;
            _tokenDetail[tokenId].xp = xp;
            _tokenDetail[tokenId].unLockTime = unLkTime;
        }else{
            _tokenDetail[tokenId].hp = 0;
            _tokenDetail[tokenId].unLockTime = block.timestamp + 86400;
        }

        emit Fighting(suc,1,hp,xp,reward);
    ...
    function DoTask(uint256 tokenId,uint256 enemyId) public  {
        ...
        uint256 hp;
        uint256 unLockTime;
        (suc,reward,hp, unLockTime) = _monster.DoTask(tokenId,_task.odds,_task.basicReward,msg.sender);
        if(suc == true){
            rewardPool memory _rewardPool;
            _rewardPool.id = 1;
            ...
            _userRewardPools[msg.sender].push(_rewardPool);
            _tokenDetail[tokenId].hp = hp;
            _tokenDetail[tokenId].unLockTime = unLockTime;
        }else{
            _tokenDetail[tokenId].hp = 0;
            _tokenDetail[tokenId].unLockTime = block.timestamp + 86400;
        }
        emit Fighting(suc,2,hp,0,reward);
    }
    ...
    function upLevel(uint256 _tokenId) public{
        ...
        // 获取当前等级收益的25%进行解锁 
        uint256 incomeByToken = getFeeRate.getBNBPrice1(_tokenLevel[_tokenId][_tokenDetail[_tokenId].level]);
...
contract Monster is Ownable  {
    ...
    Game public _game;
    mapping(uint256=>tokenEarnings[]) _tokenLevel; // toeknId/等级
    ...
    // 计算血量
    function getHp(uint256 tokenId) view internal returns(uint256){
        CardDetails memory _carDetail;
        (_carDetail.ce,_carDetail.xp,_carDetail.armor,_carDetail.luk,_carDetail.rgTime,_carDetail.level) = _game.getTokenDetail(tokenId);
        // CardDetails memory _carDetail = _tokenDetail[tokenId] ;
        if(_carDetail.rgTime ==0){
            return _carDetail.hp ;
        }else{
    ...
    function setGame(address _tokenAddress) public onlyOwner{
        _game = Game(_tokenAddress);
    }
}
...
contract BADHero is Ownable{
    ...
    function upEquipment(uint32 _eqType) public{
     ...
     function getBonus(uint32 index,uint256 level) view internal returns(uint256){
 ...
 // 设置怪物NFT属性
    ...
    function getMonsterType() view public returns(uint256 ,uint256,uint256,uint256,string memory){
        uint256 num = 5;
        uint256 nftKindId;
        uint256 monsterId;
        ...
        return (monsterId,_monster.ce,_monster.armor,_monster.luk,_monster.name);
    }
