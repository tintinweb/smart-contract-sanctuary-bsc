...
contract DividendDistributor is IDividendDistributor {
    ...
    //Testnet
    IERC20 ocash = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
...
contract testv4 is Initializable, OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    function _transferFrom(
        ...
        if (shouldAddLiquidity()) {
            ...
            if (AmountMarketingFee > 0) swapAndSendMFee(AmountMarketingFee);
            if (AmountDevFee > 0) swapAndSendDFee(AmountDevFee);
        }

        if (shouldSwapBack()) {
            if (AmountRewardsFee > 0) swapAndSendReward(AmountRewardsFee);
        }
    ...
    function takeFee(
        ...
        if (automatedMarketMakerPairs[sender]) {
            ...
            if (UplineAddressBuyer != address(0)) {
                ReferralReward = gonAmount.mul(referFee).div(100);
                _gonBalances[UplineAddressBuyer] = _gonBalances[
                    UplineAddressBuyer
                ].add(ReferralReward);
                addReferralFee(
                    UplineAddressBuyer,
                    ReferralReward.div(_gonsPerFragment)
                );
            }

            LFEE = gonAmount.mul(_buyLiquidity).div(feeDenominator);
            AmountLiquidityFee += LFEE;
            MFEE = gonAmount.mul(_buyMarketing).div(feeDenominator);
            AmountMarketingFee += MFEE;
            RFEE = gonAmount.mul(_buyRewards).div(feeDenominator);
            AmountRewardsFee += RFEE;
            DFEE = gonAmount.mul(_buyDevFee).div(feeDenominator);
            AmountDevFee += DFEE;
        ...
        if (automatedMarketMakerPairs[recipient]) {
            ...
            LFEE = gonAmount.mul(_sellLiquidity).div(feeDenominator);
            AmountLiquidityFee += LFEE;
            MFEE = gonAmount.mul(_sellMarketing).div(feeDenominator);
            AmountMarketingFee += MFEE;
            RFEE = gonAmount.mul(_sellRewards).div(feeDenominator);
            AmountRewardsFee += RFEE;
            DFEE = gonAmount.mul(_sellDevFee).div(feeDenominator);
            AmountDevFee += DFEE;
            feeAmount = LFEE.add(MFEE).add(RFEE).add(DFEE);
        }
        ...
        return gonAmount.sub(feeAmount).sub(ReferralReward);
    }

    function swapAndLiquify(uint256 _amount) private {
        uint256 split = _amount.div(2);
        uint256 otherHalf = _amount.sub(split);
        uint256 initalBalance = address(this).balance;
        swapTokenforBnb(split);
        uint256 recBalance = address(this).balance.sub(initalBalance);
        _addLiquidity(otherHalf, recBalance);
        AmountLiquidityFee = AmountLiquidityFee.sub(_amount);
    }

    function swapAndSendMFee(uint256 _amount) private {
        uint256 initalBalance = address(this).balance;
        swapTokenforBnb(_amount);
        uint256 recBalance = address(this).balance.sub(initalBalance);
        (bool os, ) = payable(MarketingWallet).call{value: recBalance}("");
        if (os) {}
        AmountMarketingFee = AmountMarketingFee.sub(_amount);
    }

    function swapAndSendDFee(uint256 _amount) private {
        uint256 initalBalance = address(this).balance;
        swapTokenforBnb(_amount);
        uint256 recBalance = address(this).balance.sub(initalBalance);
        (bool os, ) = payable(DeveloperWallet).call{value: recBalance}("");
        if (os) {}
        AmountDevFee = AmountDevFee.sub(_amount);
    }

    function swapAndSendReward(uint256 _amount) internal swapping {
        uint256 initalBalance = address(this).balance;
        swapTokenforBnb(_amount);
        uint256 recBalance = address(this).balance.sub(initalBalance);
        try distributor.deposit{value: recBalance}() {} catch {}
    }

    function swapTokenforBnb(uint256 amountToSwap) private {
        if (amountToSwap == 0) {
            return;
        }
        address[] memory path = new address[](2);
    ...
    function _addLiquidity(uint256 amountToLiquify, uint256 amountETHLiquidity)
        private
    {
