...
contract TokenLockStakeable is Ownable {
    ...
    // reward has been claimed by users
     ...
     * @param _unlockTimestamps Array of timestamps when tokens will unlock
    ...
    constructor(
        ...
        uint256 _unlockTimestampsLength = _unlockTimestamps.length;
        uint256 _unlockPercentsLength = _unlockPercents.length;

        require(_token != address(0), "TOKEN_ADDRESS_ZERO");

        require(_unlockPercentsLength > 0, "EMPTY_ARRAY");

        require(_unlockTimestampsLength == _unlockPercentsLength, "INVALID_SCHEDULE");
        ...
        for (uint256 i; i < _unlockTimestampsLength; i++) {
            if (i > 0) {
        ...
        uint256 arraySum = 0;
        unchecked {
            for (uint256 i = _unlockPercentsLength - 1; i + 1 != 0; i--) {
    ...
    function lock(
        ...
        require(arrLength == amounts.length && arrLength == rewards.length, "INVALID_ARRAY_LENGTHS");
        ...
        try IProtocolToken(token).transferFrom(msg.sender, address(this), amount) {
    ...
    function claim() external pausable {
        ...
        if (claimable > 0) {
            ...
            require(IProtocolToken(token).transfer(msg.sender, claimable), "TOKEN_TRANSFER_FAILED");
    ...
    function claimReward() external pausable {
        ...
        require(IProtocolToken(token).mintTo(msg.sender, _reward), "TOKEN_MINT_FAILED");
