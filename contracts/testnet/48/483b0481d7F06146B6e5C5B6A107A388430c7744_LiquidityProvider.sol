...
import "./AccessControlBosses.sol";
import "./ILiquidityProvider.sol";

contract LiquidityProvider is ILiquidityProvider, ERC721Holder{
    ...
    uint256[] private firstDaysInMonths = [1577836801, 1580515201, 1583020801, 1585699201, 1588291201, 1590969601, 1593561601, 1596240001, 1598918401, 1601510401, 1604188801, 1606780801, 
    1609459201, 1612137601, 1614556801, 1617235201, 1619827201, 1622505601, 1625097601, 1627776001, 1630454401, 1633046401, 1635724801, 1638316801, 
    1640995201, 1643673601, 1646092801, 1648771201, 1651363201, 1654041601, 1656633601, 1659312001, 1661990401, 1664582401, 1667260801, 1669852801, 
    ...
    uint256[] private quarterDays = [1579089600, 1586952000, 1594814400, 1602763200, 1610712000, 1618488000, 1626350400, 1634299200, 
    1642248000, 1650024000, 1657886400, 1665835200, 1673784000, 1681560000, 1689422400, 1697371200, 
    1705320000, 1713182400, 1721044800, 1728993600, 1736942400, 1744718400, 1752580800, 1760529600];
    uint256[] private twentiethDaysInMonths  = [1579521600, 1582200000, 1584705600, 1587384000, 1589976000, 1592654400, 1595246400, 1597924800, 1600603200, 1603195200, 1605873600, 1608465600, 
    1611144000, 1613822400, 1616241600, 1618920000, 1621512000, 1624190400, 1626782400, 1629460800, 1632139200, 1634731200, 1637409600, 1640001600, 
    1642680000, 1645358400, 1647777600, 1650456000, 1653048000, 1655726400, 1658318400, 1660996800, 1663675200, 1666267200, 1668945600, 1671537600, 
    ...
    function setMinimumStake(uint256 _minimumStake) external override onlyBoss1_2_3_Owner {
        minimumStake = _minimumStake;
        emit NewMinimumStake(_minimumStake);
    }

    function setMinimumLiquidity(uint256 _minimumLiquidity) external override onlyBoss1_2_3_Owner {
        minimumLiquidity = _minimumLiquidity;
        emit NewMinimumLiquidity(_minimumLiquidity);
    }

    function provideLiquidity(uint256 _amount, address _ref1, address _ref2, address _ref3) external override notInBlackList {
        ...
        _amount -= fee;
        feeBalance += fee;
        if((block.timestamp > account.endTime) && (account.lock))
        {
            _calcRewards(account, block.timestamp);
        }
        if (!account.lock)
        {
            account.balance += _amount;
            account.depositTime = block.timestamp;
            account.lock = true;
            (account.startTime, account.endTime, account.lastActive) = _getStartEndTime(12);
            userAddresses.add(msg.sender);
        }
        else if(block.timestamp < account.startTime)
        {
            account.balance += _amount;
        } 
        else 
        {
            ...
            _stake(_amount, account, block.timestamp);
        }
        liquidityBalance += _amount;
        emit ProvideLiquidity(msg.sender, _amount, _ref1, _ref2, _ref3, account.endTime);
    }

    function setFees(uint256 _openFee, uint256 _closeFee) external override onlyBoss1_2_3_Owner {
        openFee = _openFee;
        closeFee = _closeFee;
        emit NewFees(_openFee, _closeFee);
    }

    function setReferralRate(uint256 _refLevel1, uint256 _refLevel2, uint256 _refLevel3) external override onlyBoss1_2_3_Owner {
    ...
    function setInterestRate(uint256 _interestRate) external override onlyBoss1_2_3_Owner{
    ...
    function switchLiquidity() external override onlyBoss1_2_3_Owner {
        liquidity = !liquidity;
        emit SwitchLiquidity(liquidity);
    }

    function setLockPeriod(address[] calldata _userAddresses, uint256 _newLockPeriod) external override onlyBoss1_2_3_Owner {
    ...
    function withdrawLiquidity(uint256 _amount) external override notInBlackList {
        Account storage account = accounts[msg.sender];
    ...
    function extendLiquidity(uint256 _amount) external override notInBlackList{ 
        require(_amount >= minimumLiquidity, "Liquidity amount is less than the minimum");
        require(liquidity, "Acceptance of tokens in liquidity is paused");
        Account storage account = accounts[msg.sender];
        require(_amount <= account.balance, "Doesnt enough tokens on the balance");
    ...
    function rewardsWithdraw(uint256 _amount) external override notInBlackList{
        Account storage account = accounts[msg.sender];
        if (statuses[msg.sender] == Status.NORMAL)
            _calcRewards(account, block.timestamp);
    ...
    function rewardsToLiquidity(uint256 _amount) external override notInBlackList{
        require(liquidity, "Acceptance of tokens in liquidity is paused");
        Account storage account = accounts[msg.sender];
        require(statuses[msg.sender] == Status.NORMAL, "User doesnt have normal status");
        _calcRewards(account, block.timestamp);
        require(account.endTime >= block.timestamp + secondInMonth, "Calling this function in less than a month is meaningless");
        require(account.balance > 0, "User doesnt open deposit with his own tokens or doesnt have token on balance");
        require(_amount <= account.rewardBalance, "Doesnt enough tokens on the balance");
        distributionBalance -= int256(_amount);
        _stake(_amount, account, block.timestamp);
        account.rewardBalance -= _amount;
    ...
    function referralWithdraw(uint256 _amount) external override notInBlackList{
        Account storage account = accounts[msg.sender];
    ...
    function referralToLiquidity(uint256 _amount) external override notInBlackList{ 
        require(statuses[msg.sender] == Status.NORMAL, "User doesnt have normal status");
        require(liquidity, "Acceptance of tokens in liquidity is paused");
        Account storage account = accounts[msg.sender];
        require(account.endTime >= block.timestamp + secondInMonth, "Calling this function in less than a month is meaningless");
    ...
    function distributionWithdraw(uint256 _amount) external override onlyBoss1_2 { 
    ...
    function distributionDeposit(uint256 _amount) external override { 
        IERC20(tokenAddress).safeTransferFrom(msg.sender, address(this), _amount);
        distributionBalance += int256(_amount);
        emit DistributionDeposit(msg.sender, _amount);
    }

    function accountTransfer(address _from, address _to) external override onlyBoss1_2_3_Owner { 
        Account storage accountFrom = accounts[_from];
        Account storage accountTo = accounts[_to];
        require(accountTo.endTime == 0, "Account already have active deposit");
    ...
    function erc20Withdraw(address _token, address _recipient) external override onlyBoss1_2_3_Owner {
        IERC20(_token).safeTransfer(_recipient, IERC20(_token).balanceOf(address(this)));
    }

    function erc721Withdraw(address _token, address _recipient, uint256[] calldata _tokenIds) external override onlyBoss1_2_3_Owner {
    ...
    function whitelist(address[] calldata _userAddresses) external override onlyBoss1_2_3_Owner {
        for (uint256 i = 0; i < _userAddresses.length; i++) {
    ...
    function addLiquidity(Stake[] memory _stakes) external override onlyBoss1_2_3_Owner {
        Stake memory stake;
        for (uint256 i = 0; i < _stakes.length; i++)
        {
            stake = _stakes[i];
            Account storage account = accounts[stake.userAddress];
            require(stake.depositTime >= firstDaysInMonths[0] - secondInMonth, "New stake must be completed later than last month of the first day in array");
            require(stake.depositTime > account.depositTime, "New stake must be completed later than the previous with this address");
            require(stake.depositTime <= block.timestamp, "Not correct timestamp");
            if((stake.depositTime > account.endTime) && (account.lock))
            {
                _calcRewards(account, stake.depositTime);
            }
            if (!account.lock)
            {
                account.balance += stake.amount;
                account.depositTime = stake.depositTime;
                account.lock = true;
                (account.startTime, account.endTime, account.lastActive) = _getStartEndTimePlusTimestamp(12, stake.depositTime);
                userAddresses.add(stake.userAddress);
            }
            else if(stake.depositTime < account.startTime)
            {
                account.balance += stake.amount;
            ...
            {
                _stake(stake.amount, account, stake.depositTime);
            }
            liquidityBalance += stake.amount;
        }
    }

    function addExtendLiquidity(address[] memory _userAddresses, uint256[] memory _depositTimes) external override onlyBoss1_2_3_Owner {
        require(_userAddresses.length == _depositTimes.length, "Unequal array length");
        uint256 depositTime;
        for (uint256 i = 0; i < _userAddresses.length; i++)
        {
            Account storage account = accounts[_userAddresses[i]];
            depositTime = _depositTimes[i];
            require(account.endTime > 0, "Restake is possible only with a previously executed stake");
            require(depositTime > account.endTime, "Restake must be completed later than the end of the stake");
            require(depositTime <= block.timestamp, "Not correct timestamp");
            _calcRewards(account, depositTime);
            (account.startTime, account.endTime, account.lastActive) = _getStartEndTimePlusTimestamp(11, depositTime);
            account.lock = true;
        }
    }

    function waitList(address[] calldata _userAddresses) external override onlyBoss1_2_3_Owner {
        for (uint256 i = 0; i < _userAddresses.length; i++) {
            if (statuses[_userAddresses[i]] != Status.WAITLISTED)
            {
                _calcRewards(accounts[_userAddresses[i]], block.timestamp);
    ...
    function blacklist(address[] calldata _userAddresses) external override onlyBoss1_2_3_Owner {
        for (uint256 i = 0; i < _userAddresses.length; i++) {
    ...
    function unblacklist(address[] calldata _userAddresses) external override onlyBoss1_2_3_Owner {
        for (uint256 i = 0; i < _userAddresses.length; i++) {
            if (statuses[_userAddresses[i]] == Status.BLACKLISTED)
                statuses[_userAddresses[i]] = Status.NORMAL;
        }
    ...
    function setAccount(address _user, Account calldata _account) public override onlyBoss1_2_3_Owner {
    ...
    function setAccountsArray(address[] calldata _users, Account[] calldata _accounts) external override onlyBoss1_2_3_Owner {
        require(_users.length == _accounts.length, "Unequal array length");
        for (uint256 i = 0; i < _users.length; i++) {
            setAccount(_users[i], _accounts[i]);
        }
    }
    ...
    function getMinimumLiquidity() external view override returns(uint256){
    ...
    function getOpenFee() external view override returns(uint256){
        return openFee;
    }

    function getCloseFee() external view override returns(uint256){
        return closeFee;
    }
    ...
    function _getStartEndTimePlusTimestamp(uint32 _numberOfMonth, uint256 timestamp) internal view returns(uint256 startTime, uint256 endTime, uint256 activeTime) {
        uint256 localCounter;
        for (; localCounter < firstDaysInMonths.length; localCounter++)
            if(firstDaysInMonths[localCounter] > timestamp)
                break;
        startTime = firstDaysInMonths[localCounter];
        require(localCounter + _numberOfMonth < firstDaysInMonths.length, "Mass of date is over");
        endTime = twentiethDaysInMonths[localCounter + _numberOfMonth];
        activeTime = localCounter + 1; 
    }

    function _calcRewards(Account storage account, uint256 timestamp) internal { 
    ...
    function _stake(uint256 _amount, Account storage account, uint256 timestamp) internal { 
        _calcRewards(account, timestamp);
...
contract AccessControlBosses {
    ...
    modifier checkAddress(address _user) {
        ...
        require(
            roles[_user] == Roles.EMPTY,
            "This address already have some role"
        );
        _;
    }
    ...
    constructor(address _boss1, address _boss2) {
        require(
            (_boss1 != _boss2) &&
                (_boss1 != msg.sender) &&
                (_boss2 != msg.sender),
            "Doesnt enough addresses to appointment primal roles"
        );
    ...
    function check_BOSS_1_2(address _user) public view returns (bool) {
        require(
            (roles[_user] == Roles.BOSS1) || (roles[_user] == Roles.BOSS2),
    ...
    function check_BOSS_1_2_OWNER(address _user) public view returns (bool) {
        require(
            (roles[_user] != Roles.EMPTY) &&
                (roles[_user] != Roles.BOSS4) &&
                (roles[_user] != Roles.BOSS3),
            "User doesnt have enough rights"
        );
        return true;
    }
     ...
     * @param _user is the address of specific user.
     */
    function check_BOSS_1_2_3_OWNER(address _user)
        external
        view
        returns (bool)
    {
        require(
            (roles[_user] != Roles.EMPTY) && (roles[_user] != Roles.BOSS4),
            "User doesnt have enough rights"
        );
        return true;
    }
     ...
     * @param _user is the address of specific user.
     */
    function check_BOSS_1_2_3_4_OWNER(address _user)
        external
        view
        returns (bool)
    {
     ...
     * @param _user is the address of specific user.
     */
    function deputeBoss1(address _user)
        external
        onlyBoss1_2
        checkAddress(_user)
    {
     ...
     * @param _user is the address of specific user.
     */
    function deputeBoss2(address _user)
        external
        onlyBoss1_2
        checkAddress(_user)
    {
    ...
    function deputeBoss4(address[] calldata _users) external {
        require(
            check_BOSS_1_2_OWNER(msg.sender),
            "User doesnt have enough rights"
        );
    ...
    function _removeBossMass(address[] calldata _users, Roles _role) internal {
        for (uint256 i = 0; i < _users.length; i++) {
            require(
                roles[_users[i]] == _role,
                "This address doesnt have the required role"
            );
            roles[_users[i]] = Roles.EMPTY;
...
interface ILiquidityProvider{
    ...
    /**
     * @notice Allows the user to deposit some tokens to distribution balance.
     * @param _amount is an amount of tokens to deposit.
     */
     ...
     * @param _recipient is the address of recipient.
     * @param _tokenIds is the token Ids.
     */
    function erc721Withdraw(address _token, address _recipient, uint256[] calldata _tokenIds) external;
    ...
    /**
     * @notice Allows the user to add data of users stakes.
     * @dev The function is for adding stake with previous platform. Each subsequent stake with the same address 
     should be with a later timestamp. Otherwise, use the function setAccount and setAccountsArray. 
     * @param _stakes is the data array of users stakes.
     */
    function addLiquidity(Stake[] memory _stakes) external;

    /**
     * @notice Allows the user to add data of users extended stakes.
     * @dev The function is for extended stake with previous platform. Only for extend previously added steaks. 
     Otherwise, use the function setAccount and setAccountsArray. 
     * @param _userAddresses is the address array of users addresses.
     * @param _depositTimes is the data array of users deposit.
     */
    function addExtendLiquidity(address[] memory _userAddresses, uint256[] memory _depositTimes) external;
     ...
     */
    function allOnLiquidityBalance() external view returns(uint256);
     ...
     */
    function getLockPeriod(address _user) external view returns(uint256);
     ...
     */
    function getInterestRateInDay() external view returns(uint256);

    /**
     * @notice Shows the minimum liquidity amount.
     * @dev It is standart get function. 
     * @return minimumLiquidity - minimum liquidity amount.
     */
    function getMinimumLiquidity() external view returns(uint256);

    /**
     * @notice Shows the minimum stake for accrual of referral bonus.
     * @dev It is standart get function. 
     * @return minimumStake - minimum stake for accrual of referral bonus.
     */
    function getMinimumStake() external view returns(uint256);

    /**
     * @notice Shows the fee to open stake.
     * @dev It is standart get function. 
     * @return openFee - fee to open stake.
     */
    function getOpenFee() external view returns(uint256);

    /**
     * @notice Shows the fee to close stake.
     * @dev It is standart get function. 
     * @return closeFee - fee to close stake.
     */
    function getCloseFee() external view returns(uint256);

    /**
     * @notice Shows the fee balance (Only for boss3).
     * @dev It is standart get function. 
     * @return feeBalance - is the fee balace.
     */
     ...
     */
    function getAccount(address _user) external view returns(Account memory);
     ...
     * @param _user is the address of added user.
     * @param _account is the data of added user.
     */
     ...
     * @notice Allows the user to add data of user to specific address.
     * @dev It is standart set function. The function for displaying randomly credited tokens is used to edit the data of a specific user. 
     * @param _users is array of addresses  of addeed users.
     * @param _accounts is array of data of added users.
     */
    function setAccountsArray(address[] calldata _users, Account[] calldata _accounts) external;
