...
//
//
// 💻 web:      https://OLIVEAI.net
// 📞 telegram: https://t.me/OLIVEAI
// 📢 twitter:  https://twitter.com/OLIVEAI
//
...
contract OLIVEAI is ERC20, Ownable {
    ...
    uint256 public tokensForMission;
    ...
    event checkFees(bool fees);
    event checkIfBuy(bool check);
    event checkAmount(uint256 amount);
    event totalBuy(uint256 total);
    ...
    constructor() ERC20("OLIVEAI", "OLAI") {
        uint256 totalSupply = 1_000_000_000 * (10**18);

        marketingSellFee = 2;
        liquiditySellFee = 2;
        developmentSellFee = 1;
        missionSellFee = 2;
        totalSellFees =
            marketingSellFee +
            liquiditySellFee +
        ...
        // MAINNET
        marketingWallet = address(0xDd95ba6f1Fe4088A7da79A0cBD0AdE36a4cA724e); // set as marketing wallet
        missionWallet = address(0xD5C72C404C37b8FBc096D23D88e3176720f620ca); // set as mission wallet
        developmentWallet = address(0xaDa53619D14aA4e92Ac4C76581549f8f24C2c68B); // set as development wallet
    ...
    function updateBuyFees(
        ...
        burnBuyFee = _burnFee;
        totalBuyFees = marketingBuyFee + liquidityBuyFee + burnBuyFee;
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "OLAI: The router already has that address"
        );
    ...
    function updateSellFees(
        ...
        totalSellFees = marketingSellFee + liquiditySellFee + burnSellFee;
    ...
    function updateMissionWallet(address newMissionWallet) external onlyOwner {
        excludeFromFees(newMissionWallet, true);
        emit marketingWalletUpdated(newMissionWallet, missionWallet);
        missionWallet = newMissionWallet;
    }
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to] && totalSellFees > 0) {
                ...
                tokensForDevelopment +=
                    (fees * developmentSellFee) /
                    totalSellFees;
                tokensForMission += (fees * missionSellFee) / totalSellFees;
            }
            // on buy
            else if (automatedMarketMakerPairs[from] && totalBuyFees > 0) {
                emit checkIfBuy(true);
                ...
                tokensForDevelopment +=
                    (fees * developmentBuyFee) /
                    totalBuyFees;
                tokensForMission += (fees * missionBuyFee) / totalBuyFees;
            }
        ...
        // FEES == 0 for some reason...
        emit checkAmount(fees);
        emit totalBuy(totalBuyFees);
    ...
    function swapBack() private {
        ...
        uint256 ethForMission = ethBalance.mul(tokensForMission).div(
            totalTokensToSwap
        );
        ...
        (success, ) = address(missionWallet).call{value: ethForMission}("");
