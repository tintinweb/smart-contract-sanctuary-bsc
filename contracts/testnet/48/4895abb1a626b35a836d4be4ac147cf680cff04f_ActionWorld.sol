...
contract ActionWorld {
    ...
    function withdrawal() external {
        ...
        if(_user.amountpurchase >=  defaultPackages[1] && _user.amountpurchase < defaultPackages[2]) {
            require(actualAmountToSend.mul(withdrwal).div(100)+_user.totalWithdrawn <= _user.amountpurchase.mul(200).div(100) ,"Invest for withdraw");
        } else if(_user.amountpurchase >=  defaultPackages[2] && _user.amountpurchase <=  defaultPackages[3]) {
            require( actualAmountToSend.mul(withdrwal).div(100)+_user.totalWithdrawn <= _user.amountpurchase.mul(250).div(100),"Invest for withdraw");
        } else if(_user.amountpurchase >=  defaultPackages[3]) {
            require(actualAmountToSend.mul(withdrwal).div(100)+_user.totalWithdrawn <=  _user.amountpurchase.mul(300).div(100),"Invest for withdraw");
        }
