...
contract MinerTycoon2 is Ownable {
    ...
    constructor () {
        ...
        MT0 = MinerTycoon(payable(0x1E01D5fb57aFB592BBF8C88Ef0EE62aA099c7E2b));
    ...
    function setMT0(address MT0Addr) public onlyOwner {
        MT0 = MinerTycoon(payable(MT0Addr));
    }
    ...
    function synchDataOfMyMines(uint start, uint end) public onlyOwner {
        ...
        for(uint i = start; i < end; i++){
            ...
            if(myMines[i].buyDays > 0){
                (uint types,uint256 buyDays,uint256 checkTime,uint256 release,uint8 color,uint256 total) = MT0.myMines(i);
    ...
    function synchDataOfRedirectReward(uint start, uint end) public onlyOwner {
        ...
        for(uint i = start; i < end; i++){
            ...
            if(myMines[i].buyDays > 0){
                (address addr,uint256 mineType,uint256 reward) = MT0.redirectReward(i);
    ...
    function synchDataOfIndirectReward(uint start, uint end) public onlyOwner {
        ...
        for(uint i = start; i < end; i++){
            ...
            if(myMines[i].buyDays > 0){
                (address addr,uint256 mineType,uint256 reward) = MT0.indirectReward(i);
    ...
    function synchDataOfThreeReward(uint start, uint end) public onlyOwner {
        ...
        for(uint i = start; i < end; i++){
            ...
            if(myMines[i].buyDays > 0){
                (address addr,uint256 mineType,uint256 reward) = MT0.threeReward(i);
    ...
    function synchDataOfUsersMineIds(address[] memory useraddrs) public onlyOwner {
        ...
        for(uint i = 0; i < useraddrs.length; i++){
            ...
            // uint[6] memory mineIds = MT0.getMinesIndex(useraddrs[i]);

            users[useraddrs[i]].mineIds = MT0.getMinesIndex(useraddrs[i]);
    ...
    function synchDataOfUserReward(address[] memory useraddrs) public onlyOwner {
        ...
        for(uint i = 0; i < useraddrs.length; i++){
            ...
            users[useraddr].redirectIds = MT0.getRewardIndex(1, useraddr);

            users[useraddr].indirectIds = MT0.getRewardIndex(2, useraddr);

            users[useraddr].threeIds = MT0.getRewardIndex(3, useraddr);
    ...
    uint256  indirectNum, uint256  threeNum, string memory nickName) public  {
    ...
    uint256[] memory indirectNum, uint256[] memory threeNum, string[] memory nickName) public {
    ...
    // function synchDataOfUsers1(address[] memory useraddrs, uint[] memory mineIds) public onlyOwner {

    //     require(useraddrs.length == mineIds.length,"wrong data");

    //     uint count;

    //     for(uint i = 0; i < mineIds.length; i++){

    //         uint index = _findIndex(useraddrs[i], 0);

    //         if(index <= 6){
    //             users[useraddrs[i]].mineIds[index] = mineIds[i];

    //             if(!contractUsers[useraddrs[i]]) {
    //                 contractUsers[useraddrs[i]] = true;
    //                 userAddrs.push(useraddrs[i]);
    //             }
    ...
    // function synchDataOfUsers2(address[] memory useraddrs, uint8[] memory mineNum, address[] memory welMembers, 
    // uint256[] memory myGold, uint256[] memory receiveEth, uint256[] memory receiveGold) public onlyOwner {

    //     uint256 num = useraddrs.length;

    //     require(num == mineNum.length && num == welMembers.length && num == myGold.length && num == receiveEth.length && num == receiveGold.length,"wrong data");

    //     uint count;

    //     for(uint i = 0; i < useraddrs.length; i++){

    //         users[useraddrs[i]].mineNum = mineNum[i];

    //         users[useraddrs[i]].welMember = welMembers[i];

    //         users[useraddrs[i]].myGold = myGold[i];

    //         users[useraddrs[i]].receiveEth = receiveEth[i];

    //         users[useraddrs[i]].receiveGold = receiveGold[i];
    ...
    // function synchDataOfUsers3(address[] memory useraddrs, uint8[] memory redirectNum, uint256[] memory indirectNum, 
    // uint256[] memory threeNum, string[] memory nickName) public onlyOwner {

    //     uint256 num = useraddrs.length;

    //     require(num == redirectNum.length && num == indirectNum.length && num == threeNum.length && num == nickName.length,"wrong data");

    //     uint count;

    //     for(uint i = 0; i < useraddrs.length; i++){

    //         users[useraddrs[i]].redirectNum = redirectNum[i];

    //         users[useraddrs[i]].indirectNum = indirectNum[i];

    //         users[useraddrs[i]].threeNum = threeNum[i];

    //         users[useraddrs[i]].nickName = nickName[i];
    ...
    // function synchDataOfUsers4(address[] memory useraddrs, uint8[] memory mineNum) public onlyOwner {

    //     require(useraddrs.length == mineNum.length,"wrong data");
    ...
    // function synchDataOfUsers5(address[] memory useraddrs, address[] memory welMembers) public onlyOwner {

    //     require(useraddrs.length == welMembers.length,"wrong data");
    ...
    // function synchDataOfUsers6(address[] memory useraddrs, uint256[] memory myGold) public onlyOwner {

    //     require(useraddrs.length == myGold.length,"wrong data");
    ...
    // function synchDataOfUsers7(address[] memory useraddrs, uint256[] memory receiveEth) public onlyOwner {

    //     require(useraddrs.length == receiveEth.length,"wrong data");
    ...
    // function synchDataOfUsers8(address[] memory useraddrs, uint256[] memory receiveGold) public onlyOwner {

    //     require(useraddrs.length == receiveGold.length,"wrong data");
    ...
    // function synchDataOfUsers9(address[] memory useraddrs, uint8[] memory redirectNum) public onlyOwner {

    //     require(useraddrs.length == redirectNum.length,"wrong data");
    ...
    // function synchDataOfUsers10(address[] memory useraddrs, uint256[] memory indirectNum) public onlyOwner {

    //     require(useraddrs.length == indirectNum.length,"wrong data");
    ...
    // function synchDataOfUsers11(address[] memory useraddrs, uint256[] memory threeNum) public onlyOwner {

    //     require(useraddrs.length == threeNum.length,"wrong data");
    ...
    // function synchDataOfUsers12(address[] memory useraddrs, string[] memory nickName) public onlyOwner {

    //     require(useraddrs.length == nickName.length,"wrong data");
