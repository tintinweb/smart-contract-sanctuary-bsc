...
contract YSL is ERC20, AccessControl, Pausable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if(isContract(sender) || isContract(recipient)){
            ...
            if(!(IWhitelist(Admin.whitelist()).getUserAddresses(sender)) && !(IWhitelist(Admin.whitelist()).getUserAddresses(recipient))){
                require(restrictTransfer[sender] != block.number,"bYSL: you can't interact in same block");
                ...
                if(IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(balanceOf(address(this)),path)[1] >= YSL_ThresholdLimit){
                    ...
                    // IERC20(BUSD).transfer(Treasury, (convertedBUSD * YSL_Tax_Allocation[0])/YSL_Tax);
                    // IERC20(BUSD).transfer(TemporaryHoldingContract, ((convertedBUSD * YSL_Tax_Allocation[1])/YSL_Tax));
                }
                amount -= taxAmount;
            }
