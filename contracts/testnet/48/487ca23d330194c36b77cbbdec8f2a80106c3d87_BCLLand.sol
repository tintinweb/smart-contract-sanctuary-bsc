...
contract BCLLand is ERC721, Ownable{
    ...
    function changeSupportAddress(address saddress,bool _LockCreateCountry) public onlyOwner returns(bool){
        support = saddress;
        LockCreateCountry = _LockCreateCountry;
        return true;
    }
    ...
    function CreateCountry(uint256 _tokenID,string memory _name,uint256 totalblock,string memory _plan_cid,bool iscountrystatus,address _countryowner) public returns(bool){
        require(LockCreateCountry,"NCC");
        require(bytes(_name).length > 0,"SNB");
        ...
        if(msg.sender != owner()){
            ...
            iscountryIsFree[totalcountry] = true;
        }
        
        
        noregister[totalcountry] = true;
        ...
        emit countryContrctMint(msg.sender,_countryAddress, newname,totalcountry,0);
    ...
    mapping(uint256 => bool) public noregister;
    mapping(uint256 => bool) public iscountryIsFree;
    function changefree(uint256 _cname,bool status,bool _st) public returns(bool){
        require(owner() == msg.sender || support == msg.sender,"00");
        noregister[_cname] = status;
        iscountryIsFree[totalcountry] = _st;
        return true;
    }
    function chnagecountryOwner(uint256 _countryID,address _newowner,uint256 _mintedblock,bool _mintend,bool _true,bool _isactivate) public onlyOwner returns(bool){
        require(iscountryIsFree[_countryID] ,"country is not Free");
        countryInfo[_countryID].owner = _newowner;
        countryInfo[_countryID].mintedblock = _mintedblock;
        countryInfo[_countryID].mintend = _mintend; // true
        iscountry[_countryID] = _true;  // true
        countryInfo[_countryID].isactivate = _isactivate; // true
        return true;
    }
    ...
    function CountryMintBlock(uint256 _countryID,int[] memory x,int[] memory y,bool[] memory _isstatus,address[] memory _address)public returns(bool){
        require(iscountry[_countryID],"Country not minted ");
        ...
        require(countryInfo[_countryID].owner == msg.sender || owner() == msg.sender || support == msg.sender,"is not country owner");
    ...
    event changemintfee(uint256 OldCamount,uint256 newCamount,uint256 OldVamount,uint256 newVamount);
    function changemint_fee(uint256 _Camount,uint256 _Vamount) public onlyOwner returns(bool){
        uint256 OldCamount = country_mint_fee;
        uint256 OldVamount = valleymint;
        country_mint_fee = _Camount;
        valleymint = _Vamount;
        emit changemintfee( OldCamount,country_mint_fee,OldVamount,valleymint);
        return true;
    }
    ...
    function changeBlockInfo(uint256 _tokenId, string memory _s3img,string memory _jsoncid,string memory _Type) public returns(bool){
        ...
        require(ownerOf(_tokenId) == _msgSender(),"Not TokenId Owner");
    ...
    function transferFrom(address from,address to,uint256 _tokenid) public override {
        ...
        require(!countryInfo[country].mintend,"All Block Not Minted");
        ...
        }else if(countryInfo[country].countryAddress != from && countryInfo[country].countryAddress != to){
            if(noregister[country]){
                onlyvalidUser(from);
                onlyvalidUser(to);
            }
    ...
    function countryCitizenShip(string memory _countryname,address _user)public returns(bool){
        ...
        if(noregister[_countryId]){
            onlyvalidUser(_user);
        }
        require(countryInfo[_countryId].countryAddress == msg.sender || support == msg.sender,"is not call countryContract");
    ...
    function blockCountryCitizenship(string memory _countryname,address _user,bool status)public returns(bool){
        onlyvalidUser(_user);
        uint256 _countryId = countrynameofstring[_countryname];
        require(countryInfo[_countryId].countryAddress == msg.sender || owner() == msg.sender || support == msg.sender,"is not call countryContract");
        require(iscountry[_countryId],"Country not mint at time");
    ...
    function Revokeblock(uint256[] memory _tokenId) public returns(bool){
        require(owner() == msg.sender || support == msg.sender,"is not call owner");
