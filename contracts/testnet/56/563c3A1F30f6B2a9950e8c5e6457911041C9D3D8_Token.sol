...
/**
  ...
  _____       _                     _______    _              
 |  __ \     (_)                   |__   __|  | |             
 | |__) |__ _ _ ___________ _ __      | | ___ | | _____ _ __  
 |  _  // _` | |_  /_  / _ \ '_ \     | |/ _ \| |/ / _ \ '_ \ 
 | | \ \ (_| | |/ / / /  __/ | | |    | | (_) |   <  __/ | | |
 |_|  \_\__,_|_/___/___\___|_| |_|    |_|\___/|_|\_\___|_| |_|
                                                                                                                
 
*/
contract Token is Ownable, Stakeable {
    /**
     * @notice crucial variables that are needed
     */
    ...
    function _mint(address account, uint256 amount) internal {
        require(
            account != address(0),
            "Raizzen Token: cannot mint to zero address"
        );
    ...
    function _burn(address account, uint256 amount) internal {
        require(
            account != address(0),
            "Raizzen Token: cannot burn from zero address"
        );
        require(
            _balances[account] >= amount,
            "Raizzen Token: Cannot burn more than the account owns"
        );
    ...
    function _transfer(
        ...
        require(
            sender != address(0),
            "Raizzen Token: transfer from zero address"
        );
        require(
            recipient != address(0),
            "Raizzen Token: transfer to zero address"
        );
        require(
            _balances[sender] >= amount,
            "Raizzen Token: cant transfer more than your account holds"
        );
    ...
    function _approve(
        ...
        require(
            owner != address(0),
            "Raizzen Token: approve cannot be done from zero address"
        );
        require(
            spender != address(0),
            "Raizzen Token: approve cannot be to zero address"
        );
    ...
    function transferFrom(
        ...
        require(
            ...
            "Raizzen Token: You cannot spend that much on this account"
        );
        // Transfer first
    ...
    /**
     * @notice setrewardPerHour
     * Set reward per hour for a stake
     */
    function setrewardPerHour(uint256 _amount) public onlyOwner {
        _setrewardPerHour(_amount);
    }

    /**
     * @notice getrewardPerHour
     * Get reward per hour for a stake
     */
    function getrewardPerHour() public view onlyOwner returns (uint256) {
        return _getrewardPerHour();
    }
    ...
    /**
     * @notice redeem
     * Add functionality to burn token to the executioner msg.sender
     */
    function redeem(uint256 _amount) public {
        // Make sure redeemer actually is good for it
        require(
            _amount <= _balances[msg.sender],
