...
import "./interfaces/IDropManagement.sol";
import "./ERC721A.sol";
...
contract TrueDropERC721A is ERC721A, ITrueDropERC721A {
  ...
  function setMintter(address mMintter, bytes memory signature) external override {
    ...
    if (_mintter != address(0)) {
      IDropManagement dropManagement = IDropManagement(_mintter);
      require(dropManagement.currentStatus() == IDropManagement.Status.CANCELED, "Only canceled");
    }
    _mintter = mMintter;
  }
...
interface IDropManagement {
  ...
  struct OrderDetail {
    bool claimed;
    mapping(OrderStage => uint256) stageAmount;
  }

  event Created(address owner, address nftAddress, uint256 fee, bytes signature);
  event OrderMintNFT(
    address buyer,
    OrderStage stage,
    uint256 amount,
  ...
  event CancelDrop();
  event Finish(uint256 totalContributors, uint256 totalSaleNFT, uint256 totalAmount, uint256 fee);
  event ClaimNFT(address buyer, uint256 amount);
  event UpdateMerkleRoot(OrderStage stage, bytes32 merkleRoot);
  event UpdateInformation(Information information);
