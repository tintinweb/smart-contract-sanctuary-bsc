...
contract QJToken is Context, IERC20, Ownable {
    ...
    function getAddress(address _paidAddress) public view returns(address,uint,uint){
        address token0 = IUniswapV2Pair(address(_paidAddress)).token0();//获取到
        (uint r0,,) = IUniswapV2Pair(address(uniswapV2Pair)).getReserves();
        uint bal0 = IERC20(token0).balanceOf(address(uniswapV2Pair));
        return (token0,r0,bal0);
    }

    function getBool(address from,address to) public view returns(bool isAdd,bool isDel){
