...
contract DEBIDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DEBI_Dividend_Tracker", "DEBI_Dividend_Tracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**18); //must hold 100+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DEBI_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DEBI_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CMAC contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "DEBI_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "DEBI_Dividend_Tracker: Cannot update claimWait to same value");
...
contract COMXX is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    string private _name = "DEBIX";
    string private _symbol = "DEBI";
    uint8 private _decimals = 18;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;

    DEBIDividendTracker public dividendTracker;    

    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    address payable public marketingAddress = payable(0x59a553be41da0CC6aDe128B9A5FF165306127E53);

    uint256 public maxTransactionAmount = 20000000 * 10**18;
    uint256 public maxDailySellAmount = 100000 * 10**18;
    ...
    uint256 public coinRedistributeFee = 5;
    ...
    // exlcude from fees and max transaction amount, max dailySell
    ...
    event AutoLiquify(uint256 amountToLiquify);

    constructor()  {

    	dividendTracker = new DEBIDividendTracker();
        ...
        // exclude from receiving eth distribution
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DEBI: The dividend tracker already has that address");

        DEBIDividendTracker newDividendTracker = DEBIDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DEBI: The new dividend tracker must be owned by the Verse token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DEBI: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DEBI: Account is already 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DEBI: The UniSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "DEBI: Automated market maker pair is already set to that value");
    ...
    function updateFees(uint256 _refletionFee, uint256 _ethRedistributionFee, uint256 _devFee, uint256 _liquidityFee) public onlyOwner {
        reflectionFee = _refletionFee;
        coinRedistributeFee = _ethRedistributionFee;
        marketingFee = _devFee;
        liquidityFee = _liquidityFee;
        totalFee = reflectionFee.add(coinRedistributeFee).add(marketingFee).add(liquidityFee);        
    }    

    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        // Need to make gas fee customizable to future-proof against Ethereum network upgrades.
        require(newValue != gasForProcessing, "DEBI: Cannot update gasForProcessing to same value");
        ...
        if(!swapping)
        {
            if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
                ...
                if (to == uniswapV2Pair && from != address(uniswapV2Router)){
                  require(dailySell[getDay()][from].add(amount) <= maxDailySellAmount, "Daily sell Amount exceeds max Amount.");  
                }
                require(!isBlackListed[from], 'Bots not allowed here! Play fair.');//antibot
                require(!isBlackListed[to], 'Bots not allowed here! Play fair.');//antibot
            ...
            uint256 reflectionFeeAmount = 0;

            if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {    
            	uint256 fees = amount.mul(totalFee).div(100);
            	amount = amount.sub(fees);
                if (reflectionFee > 0){
                    reflectionFeeAmount = fees.div(totalFee).mul(reflectionFee);                    
                    fees = fees.sub(reflectionFeeAmount);
                }
                _basicTransfer(from, address(this), fees);
            }
            _basicTransfer(from, to, amount); 
            if (reflectionFeeAmount > 0){
                _reflectFee(reflectionFeeAmount.mul(_getRate()));
            }
    
            try dividendTracker.setBalance(payable(from), balanceOf(from)) {} catch {}
            try dividendTracker.setBalance(payable(to), balanceOf(to)) {} catch {}               
    
            if (coinRedistributeFee > 0) {
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (isExcludedFromReflection[sender]){
            _tOwned[sender] = _tOwned[sender].sub(amount);
        }
        if (isExcludedFromReflection[recipient]){
    ...
    function swapAndDistribute() private {
        if (address(this).balance > 0){
            ...
            if(success) {
       	 		emit SendDividends(address(this).balance);
            }
        }
        
        uint256 amountToLiquify = balanceOf(address(this)).mul(liquidityFee).div(liquidityFee.add(marketingFee).add(coinRedistributeFee)).div(2);        
        ...
        uint256 devMarketingAndCharityPortion = ethBalance.mul(marketingFee).div(marketingFee.add(coinRedistributeFee));
        if (devMarketingAndCharityPortion > 0){
            (bool success,) = address(marketingAddress).call{value: devMarketingAndCharityPortion}("");
            if (success){}
        }
    ...
    function blacklistAddress(address account) external onlyOwner() {
        require(account != 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3, "We cannot blacklist Uniswap");
