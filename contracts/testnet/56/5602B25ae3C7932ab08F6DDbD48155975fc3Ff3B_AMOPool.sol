...
interface IBinRouter {
    ...
    function swapAddBinLiquidity(uint256 returnAmount, address sender)
        external
        returns (uint256);
...
contract AMOPool is Ownable {
        ...
        checkStart
    {
        ...
        binRouter.swapAddBinLiquidity(usdtNum, address(0));
        //10%
        usdt.safeTransferFrom(
            msg.sender,
            fundAddr,
    ...
    function giveInvReward(address account, uint256 amount) internal {
        ...
        referrer = user.referrers(referrer);
        if (referrer != address(0)) {
            giveInvStock(referrer, amount.mul(2), 2);
        } else {
            referrer = fundAddr;
        }
        usdt.safeTransferFrom(
        ...
        referrer = user.referrers(referrer);
        if (referrer != address(0)) {
            giveInvStock(referrer, amount.mul(2), 3);
        } else {
            referrer = fundAddr;
        }
        usdt.safeTransferFrom(
    ...
    function upgradeNode(address account, uint256 _teamStock) internal {
        ...
        if (count >= 3 && _teamStock >= 30000 * 10**18) {
            userNode[account] = 3;
            if (poolContract[3] == address(0)) {
                poolContract[3] = createDispatchHandle();
            }
            newLevel = 3;
        } else if (count >= 2 && _teamStock >= 20000 * 10**18) {
            userNode[account] = 2;
            if (poolContract[2] == address(0)) {
                poolContract[2] = createDispatchHandle();
            }
            newLevel = 2;
        } else if (count >= 1 && _teamStock >= 10000 * 10**18) {
            userNode[account] = 1;
            if (poolContract[1] == address(0)) {
        ...
        if (poolContract[4] == address(0)) {
            poolContract[4] = createDispatchHandle();
        }
