...
import "../dex/libraries/Babylonian.sol";
import "../dex/interfaces/ITangleseaPair.sol";
import "../dex/interfaces/ITangleseaRouter02.sol";
import "../dex/interfaces/IWETH.sol";
import "../interfaces/IMasterChef.sol";
import "hardhat/console.sol";

contract FarmUniV2Zap is Ownable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    ITangleseaRouter02 public immutable router;
    ...
    constructor(
        ...
        router = ITangleseaRouter02(_router);
    ...
    function zapInETH(uint256 pid, uint256 tokenAmountOutMin) external payable {
        require(msg.value >= minimumAmount, "Beefy: Insignificant input amount");

        IWETH(WETH).deposit{value: msg.value}();

        _swapAndStake(pid, tokenAmountOutMin, WETH);
    }
    ...
    /**
     * Zap out from LP Token (if its from a farm you have to do withdrawAndHarvest beforehand)
     */
    function zapOut(address lpToken, uint256 withdrawAmount) external {
        ITangleseaPair pair = _getPair(lpToken);
    ...
    function _getMasterChefPair(uint256 pid) private view returns (IMasterChef.PoolInfo memory pool, ITangleseaPair pair) {
        IMasterChef.PoolInfo memory poolInfo = masterChef.poolInfo(pid);
        pool = poolInfo;
        pair = ITangleseaPair(address(poolInfo.lpToken));
        require(pair.factory() == router.factory(), "Beefy: Incompatible liquidity pair factory");
    }

    function _getPair(address lpToken) private view returns (ITangleseaPair pair) {
        pair = ITangleseaPair(lpToken);
        require(pair.factory() == router.factory(), "Beefy: Incompatible liquidity pair factory");
    }
    ...
    function _swapAndStake(
        ...
        (IMasterChef.PoolInfo memory pool, ITangleseaPair pair) = _getMasterChefPair(pid);
        ...
        _approveTokenIfNeeded(address(pair), address(masterChef));
        masterChef.deposit(pid, amountLiquidity, msg.sender);
    ...
    function _getSwapAmount(
        ...
        uint256 nominator = router.getAmountOut(halfInvestment, reserveA, reserveB, feeAmount);
    ...
    {
        checkWETH();
        (, ITangleseaPair pair) = _getMasterChefPair(pid);
        ...
        swapAmountOut = router.getAmountOut(swapAmountIn, reserveA, reserveB, pair.feeAmount());
...
interface IMasterChef {
    ...
    struct PoolInfo {
        ...
        uint256 accPearlPerShare; // Accumulated SUSHI per share, times 1e12. See below.
    }
...
import "./ITangleseaRouter01.sol";

interface ITangleseaRouter02 is ITangleseaRouter01 {
