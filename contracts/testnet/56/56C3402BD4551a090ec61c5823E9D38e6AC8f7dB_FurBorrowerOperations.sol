...
contract FurBorrowerOperations is BaseContract, FurLiquityBase, IFurBorrowerOperations {
    ...
    function openVault(uint _maxFeePercentage, uint _FurAmount, uint _FURUSDAmount, address _upperHint, address _lowerHint) external override whenNotPaused {
        ...
        vars.ICR = LiquityMath._computeCR(_FurAmount, vars.netDebt, vars.price);
        vars.NICR = LiquityMath._computeNominalCR(_FurAmount, vars.netDebt);

        if (isRecoveryMode) {
            _requireICRisAboveCCR(vars.ICR);
        } else {
            _requireICRisAboveMCR(vars.ICR);
            uint newTCR = _getNewTCRFromVaultChange(_FurAmount, true, vars.netDebt, true, vars.price);  // bools: coll increase, debt increase
            _requireNewTCRisAboveCCR(newTCR); 
        }

        // Set the vault struct's properties
        contractsCache.vaultManager.setVaultStatus(msg.sender, 1);
        contractsCache.vaultManager.increaseVaultColl(msg.sender, _FurAmount);
        ...
        // Move the FUR to the Active Pool, and mint the FURUSDAmount to the borrower
        _activePoolAddColl(contractsCache.activePool, _FurAmount);
        _withdrawFURUSD(contractsCache.activePool, contractsCache.furUSDToken, msg.sender, _FURUSDAmount, vars.netDebt);

        emit VaultUpdated(msg.sender, vars.netDebt, _FurAmount, vars.stake, BorrowerOperation.openVault);
        emit FURUSDBorrowingFeePaid(msg.sender, vars.FURUSDFee);
    }

    // Send FUR as collateral to a vault
    ...
    // Send FUR as collateral to a vault. Called by only the Stability Pool.
    function moveFURGainToVault(address _borrower, uint _collDeposital, address _upperHint, address _lowerHint) external override whenNotPaused {
        _requireCallerIsStabilityPool();
        _adjustVault(_borrower, _collDeposital, 0, 0, false, _upperHint, _lowerHint, 0);
    }

    // Withdraw FUR collateral from a vault
    ...
    */
    function _adjustVault(address _borrower, uint _collDeposital, uint _collWithdrawal, uint _FURUSDChange, bool _isDebtIncrease, address _upperHint, address _lowerHint, uint _maxFeePercentage) internal {
        ...
        // Confirm the operation is either a borrower adjusting their own vault, or a pure FUR transfer from the Stability Pool to a vault
        ...
        // Get the collChange based on FUR or not FUR was sent in the transaction
        ...
        _moveTokensAndFURfromAdjustment(
    ...
    function closeVault() external override whenNotPaused {
        ...
        activePoolCached.sendFUR(msg.sender, coll);
    }
    ...
    function claimCollateral() external override whenNotPaused {
        // send FUR from CollSurplus Pool to owner
        collSurplusPool.claimColl(msg.sender);
    }

    // --- Helper functions ---

    function _triggerBorrowingFee(IFurVaultManager _vaultManager, IFURUSDToken _furUSDToken, uint _FURUSDAmount, uint _maxFeePercentage) internal returns (uint) {
    ...
    // Update vault's coll and debt based on whFUR they increase or decrease
    function _updateVaultFromAdjustment(
        IFurVaultManager _vaultManager,
        address _borrower,
        uint _collChange,
    ...
    function _moveTokensAndFURfromAdjustment(
        ...
        internal
    {
        ...
        if (_isCollIncrease) {
            ...
            _activePool.sendFUR(_borrower, _collChange);
        }
    }

    // Send FUR to Active Pool
    function _activePoolAddColl(IFurActivePool _activePool, uint _amount) internal {
        furToken.transferFrom(msg.sender, address(_activePool), _amount);
    }
    ...
    function _withdrawFURUSD(IFurActivePool _activePool, IFURUSDToken _furUSDToken, address _account, uint _FURUSDAmount, uint _netDebtIncrease) internal {
    ...
    function _repayFURUSD(IFurActivePool _activePool, IFURUSDToken _furUSDToken, address _account, uint _FURUSD) internal {
    ...
    function _requireVaultisActive(IFurVaultManager _vaultManager, address _borrower) internal view {
        uint status = _vaultManager.getVaultStatus(_borrower);
        require(status == 1, "BorrowerOps: Vault does not exist or is closed");
    }

    function _requireVaultisNotActive(IFurVaultManager _vaultManager, address _borrower) internal view {
        uint status = _vaultManager.getVaultStatus(_borrower);
