...
// This contract locks digichain v2 liquidity tokens. Used to give investors peace of mind a token team has locked liquidity
// and that the digiv2 tokens cannot be removed from digi until the specified unlock date has been reached.
...
contract DigiV2Locker is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using EnumerableSet for EnumerableSet.AddressSet;

  IDigiFactory public digiFactory;
  ...
  struct TokenLock {
    ...
    uint256 lockID; // lockID nonce per digi pair
    address owner;
  }
  ...
  mapping(address => TokenLock[]) public tokenLocks; //map digiv2 pair to all its locks
  ...
  struct FeeStruct {
    ...
    uint256 liquidityFee; // fee on digiv2 liquidity tokens
  ...
  constructor(IDigiFactory _digiFactory) public {
    devaddr = msg.sender;
  ...
  /**
   * @notice set the migrator contract which allows locked lp tokens to be migrated to digi v3
   */
   ...
   * @param _lpToken the digiv2 token address
   * @param _amount amount of LP tokens to lock
   * @param _unlock_date the unix timestamp (in seconds) until unlock
   * @param _referral the referrer address if any or address(0) for none
   * @param _fee_in_eth fees can be paid in eth or in a secondary token such as UNCX with a discount on digiv2 tokens
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_eth, address payable _withdrawer) external payable nonReentrant {
    ...
    // ensure this pair is a digiv2 pair by querying the factory
    IDigiV2Pair lpair = IDigiV2Pair(address(_lpToken));
    address factoryPairAddress = digiFactory.getPair(lpair.token0(), lpair.token1());
    require(factoryPairAddress == address(_lpToken), 'NOT DIGIV2');
    ...
    // record the lock for the digiv2pair
  ...
  function relock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _unlock_date) external nonReentrant {
    ...
    // send digiv2 fee to dev address
    TransferHelper.safeTransfer(_lpToken, devaddr, liquidityFee);
  }
  ...
  /**
   * @notice migrates liquidity to digi v3
   */
