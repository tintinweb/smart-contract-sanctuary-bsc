...
contract a is ERC20Detailed, Auth {
    ...
    function swapBack() internal swapping {
        ...
        if (swappingOnlyFromContract) {
            if (amountToLiquify > 0) {
                ...
                liquidityManager.swapTokenForBusdToWallet(address(this), liquidityReceiver, half, 100);
                _transferFrom(address(this), liquidityReceiver, half);
            }

            if (amountToRFV > 0) {
                liquidityManager.swapTokenForBusdToWallet(address(this), riskFreeValueReceiver, amountToRFV, 100);
            }

            if (amountToTreasury > 0) {
                liquidityManager.swapTokenForBusdToWallet(address(this), treasuryReceiver, amountToTreasury, 100);
            }
        }else{
            if (amountToLiquify > 0) {
    ...
    function swapBusdForToken(uint256 amountIn) external {
       ...
       IERC20(busdToken).transferFrom(msg.sender, address(this), amountIn);
       address[] memory path = new address[](2);
        path[0] = busdToken;
        path[1] = address(this);
    ...
    function swapTokenForBusd(uint256 amountIn) external {
        //_setSwapAllowed(msg.sender, true);
