...
contract FananeesNFT is Ownable, AccessControl, ERC721Royalty {
    // NFT supply handling
    ...
    LifecycleStage internal stage = LifecycleStage.PreLaunch;

    uint256 public immutable STARTING_DATE;
    uint256 public constant WHITELIST_LENGTH = 1 days;


    // Payment details
    uint256 public constant PUBLIC_PRICE = 15 * 1e16 wei; // 0.15 ETH <=> 0.15 * 1e18 wei <=> 15 * 1e16 wei
    uint256 public constant RESERVE_PERCENT = 1;

    uint256 public totalETHCollected = 0;
    uint256 public proceedsWithdrawn = 0;
    uint256 public reserveWithdrawn = 0;


    // Whitelist handling
    bytes32 public whitelistRoot;
    mapping(address => mapping (string => uint256)) public whitelistClaimsMade;
    uint256 public whitelistClaimsLimit = 5;
    ...
    event ReserveTransferred(address indexed recipient, uint256 amount);


    // MODIFIERS

    modifier stageReached(LifecycleStage _stage) {
        require( getStage() >= _stage, "The required stage hasn't been reached yet" );
        _;
    }
    ...
    constructor(address _teamAddress, uint256 _whitelistStart) ERC721("Fananees", "FNN") {
    ...
    function mintMultiple(uint256 amount)
        public
        payable
        stageReached(LifecycleStage.Public)
        returns(
            uint256 firstID,
            uint256 lastID
        ) {
        ...
        totalETHCollected += PUBLIC_PRICE * amount;
    ...
    function mint() public payable stageReached(LifecycleStage.Public) returns (uint256) {
    ...
    function mintWhitelistMultiple(
        ...
        public
        payable
        stageReached( LifecycleStage.Whitelist )
        returns(
            uint256 firstID,
            uint256 lastID
    ) {
    ...
    function setWLRoot(bytes32 merkleroot) public onlyOwner {
        require(whitelistRoot.length == 0);
    ...
    function pushTeamAndVCUnlocks(uint256 newTimestamp) public onlyOwner {
        require(newTimestamp >= teamAndVCLockupTimestamp);

        teamAndVCLockupTimestamp = newTimestamp;
        emit TeamAndVCUnlockDateUpdated(newTimestamp);
    }

    function reduceTeamAndVCLimit(uint256 _teamAndVCLimit) public onlyOwner {
        require(_teamAndVCLimit < teamAndVCLimit);
        require(_teamAndVCLimit >= mintedForTeamAndVC);

        teamAndVCLimit = _teamAndVCLimit;
        emit TeamAndVCLimitReduced(_teamAndVCLimit);
    }

    function publishNFTs(string calldata newURI) public onlyOwner stageReached(LifecycleStage.Public) {
    ...
    function withdrawProceeds(address beneficiary, uint256 amount) public onlyRole(MINTER_ROLE) {
        require(amount != 0);
        require(beneficiary != address(0));

        uint256 reserve = totalETHCollected * RESERVE_PERCENT / 100;
        uint256 withdrawLimit = totalETHCollected - proceedsWithdrawn - (reserve - reserveWithdrawn);
    ...
    function withdrawProceedsToSelf(uint256 amount) public onlyRole(MINTER_ROLE) {
        withdrawProceeds(msg.sender, amount);
    }

    function transferReserve(address destination, uint256 amount) public onlyOwner {
        require(destination != address(0));
        require(amount != 0);

        uint256 withdrawLimit = totalETHCollected * RESERVE_PERCENT / 100;
        require(reserveWithdrawn + amount <= withdrawLimit);

        reserveWithdrawn += amount;
        payable(destination).transfer(amount);

        emit ReserveTransferred(destination, amount);
    }
    ...
    function getStage() internal view returns (LifecycleStage) {
        if (block.timestamp <= STARTING_DATE) {
