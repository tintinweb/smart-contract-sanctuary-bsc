...
contract RAMEN is ERC20, Ownable {
    ...
    uint256 public marketingShare = 185;
    uint256 public teamShare = 55;
    uint256 public teamShare2 = 55;
    uint256 public teamShare3 = 55;
    uint256 public rewardShare = 450;

    address public marketingWallet = 0x09E626Eb98bCF39d17163C01B8F3c46135218491;
    address public teamWallet = 0xF963f145cd74C4c63EbD6AB4867aA846D99e8f66;
    address public teamWallet2 = 0x37F07949d9094C31696484497158aD63C1414ec6;
    address public teamWallet3 = 0xcd5E1393F324b7c1e79C4Dc681f553b90375105b;
    ...
    event FeeSharesUpdated(uint256 liquidityShare, uint256 marketingShare, uint256 rewardShare, uint256 teamShare, uint256 teamShare2, uint256 teamShare3);
    event MarketingWalletChanged(address marketingWallet);
    event TeamWalletsChanged(address teamWallet, address teamWallet2, address teamWallet3);
    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 bnbReceived, uint256 tokensIntoLiqudity);
    event SendMarketing(uint256 bnbSend);
    event SendTeam(uint256 bnbSend);
    event SendTeam2(uint256 bnbSend);
    event SendTeam3(uint256 bnbSend);
    ...
    constructor(address newOwner) ERC20("Ramen", "RAMEN") 
    {
        _mint(newOwner, ((1 * 1e15) * (10 ** 18)));
    ...
    function updateFeeShares(uint256 _marketingFeeShare, uint256 _liquidityFeeShare, uint256 _rewardShare, uint256 _teamShare, uint256 _teamShare2, uint256 _teamShare3) external onlyOwner {
        require(_marketingFeeShare + _liquidityFeeShare + _rewardShare + _teamShare + _teamShare2 + _teamShare3 == 1000, "Fee shares must add up to 1000");
        marketingShare = _marketingFeeShare;
        liquidityShare = _liquidityFeeShare;
        rewardShare    = _rewardShare;
        teamShare      = _teamShare;
        teamShare2     = _teamShare2;
        teamShare3     = _teamShare3;
        emit FeeSharesUpdated(marketingShare, liquidityShare, rewardShare, teamShare, teamShare2, teamShare3);
    }
    ...
    function changeTeamWallets(address _teamWallet, address _teamWallet2, address _teamWallet3) external onlyOwner {
        require(_teamWallet != teamWallet, "Team wallet is already that address");
        require(_teamWallet2 != teamWallet2, "Team wallet 2 is already that address");
        require(_teamWallet3 != teamWallet3, "Team wallet 3 is already that address");
        require(!isContract(_teamWallet), "Team wallets cannot be a contract");
        require(!isContract(_teamWallet2), "Team wallets cannot be a contract");
        require(!isContract(_teamWallet3), "Team wallets cannot be a contract");
        teamWallet = _teamWallet;
        teamWallet2 = _teamWallet2;
        teamWallet3 = _teamWallet3;
        emit TeamWalletsChanged(teamWallet, teamWallet2, teamWallet3);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            if(liquidityShare > 0) {
                liquidityTokens = contractTokenBalance * liquidityShare / 1000;
                swapAndLiquify(liquidityTokens);
            }

            contractTokenBalance -= liquidityTokens;
            uint256 bnbShare = marketingShare + rewardShare + teamShare + teamShare2 + teamShare3;
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(teamShare2 > 0) {
                    uint256 team2BNB = newBalance * teamShare2 / bnbShare;
                    sendBNB(payable(teamWallet2), team2BNB);
                    emit SendTeam2(team2BNB);
                }

                if(teamShare3 > 0) {
                    uint256 team3BNB = newBalance * teamShare3 / bnbShare;
                    sendBNB(payable(teamWallet3), team3BNB);
                    emit SendTeam3(team3BNB);
                }
