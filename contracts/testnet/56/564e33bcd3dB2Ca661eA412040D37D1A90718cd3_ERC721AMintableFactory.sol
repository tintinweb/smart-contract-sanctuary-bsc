...
import "./MogulERC721AMintable.sol";
import "./FactoryInterfaces.sol";

// ERC721A Mintable Factory
contract ERC721AMintableFactory is AccessControl, IERC721AMintableFactory {
  address immutable tokenImplementation;
  bytes32 public constant CREATOR_ROLE = keccak256("CREATOR_ROLE");

  event ERC721AMintableCreated(address contractAddress, address owner);

  constructor() {
    _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    _setupRole(CREATOR_ROLE, msg.sender);

    tokenImplementation = address(new MogulERC721AMintable());
  }
  ...
  function createERC721AMintable(
    ...
    IInitializableERC721AMintable(clone).init(
      _stars,
      _owner,
      _ethAllowed,
      _starsAllowed,
    ...
    emit ERC721AMintableCreated(clone, _owner);
  }
}
...
contract MogulERC721AMintable is ERC721A {
  address owner;
  modifier onlyOwner() {
  ...
  uint256 public NFT_1_PRICE;
  uint256 public NFT_3_PRICE;
  uint256 public NFT_5_PRICE;

  uint256 public NFT_1_PRICE_STARS;
  uint256 public NFT_3_PRICE_STARS;
  uint256 public NFT_5_PRICE_STARS;
  ...
  modifier mintCompliance(uint256 _numberOfTokens, bool isStarsPurchase) {
    if (isStarsPurchase) {
      ...
      if (_numberOfTokens == 1) {
        stars.safeTransferFrom(msg.sender, address(this), NFT_1_PRICE_STARS);
      } else if (_numberOfTokens == 3) {
        stars.safeTransferFrom(msg.sender, address(this), NFT_3_PRICE_STARS);
      } else if (_numberOfTokens == 5) {
        stars.safeTransferFrom(msg.sender, address(this), NFT_5_PRICE_STARS);
      }
    } else {
  ...
  constructor() ERC721A("MogulERC721A", "mNFT") {}
  ...
  function init(
    ...
    NFT_1_PRICE = ethPrices[0];
    NFT_3_PRICE = ethPrices[1];
    NFT_5_PRICE = ethPrices[2];

    NFT_1_PRICE_STARS = starsPrices[0];
    NFT_3_PRICE_STARS = starsPrices[1];
    NFT_5_PRICE_STARS = starsPrices[2];
