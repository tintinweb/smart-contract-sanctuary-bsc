...
contract MarsHouse is ERC721("Mars House", "Mars"), IERC721Receiver, ERC721Enumerable, Ownable {
    ...
    mapping(address => unstakeTokenInfo) private unstakeToken;
    ...
    function checkUnstake(uint256 tokenId) external returns (uint256[] memory) {
        uint256[] memory result = new uint256[](2);
        result = _check(tokenId, block.timestamp, msg.sender);
        
        unstakeToken[msg.sender] = unstakeTokenInfo(tokenId, result[0], result[1]);
    ...
    function unstake(uint256 _tokenId, uint256 _fee, uint256 _rewards) external {

        require(msg.sender == ownerOf(_tokenId), "Token owner does not match.");
        require(unstakeToken[msg.sender].rewards == _rewards, "The amount of rewards does not match.");
        require(unstakeToken[msg.sender].fee == _fee, "The amount of fee does not match.");
        ...
        delete unstakeToken[msg.sender];
