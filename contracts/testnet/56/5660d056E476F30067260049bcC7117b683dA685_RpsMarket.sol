...
/**
 * @title Bingo NFT Launchpad
 * @notice Pre-market sell Bingo NFT tokens.
 */
contract RpsMarket is
    Initializable,
    ...
    ReentrancyGuardUpgradeable
{
    ...
    event Buy(uint256 nftId, address indexed user, uint256 launchIndex);
    ...
    function setWhitelistDealToken(address _dealToken)
        public
    ...
    function delWhitelistDealToken(address _dealToken)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_dealToken != address(0), "Must be non zero");
    ...
    function addNewLaunch(Launchpad[] calldata _launches)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_launches.length > 0, "Must be non zero");
        for (uint256 i = 0; i < _launches.length; i++) {
            require(_launches[i].price > 0, "Must be non zero");
            require(_launches[i].totalCount > 0, "Must be non zero");
            require(_launches[i].rank > 0, "Must be non zero");
            require(_launches[i].dealToken != address(0), "Must be non zero");

            launches.push(
                Launchpad({
                    totalCount: _launches[i].totalCount,
                    soldCount: 0,
                    price: _launches[i].price,
                    rank: _launches[i].rank,
                    enable: true,
    ...
    function leftToSell(uint256 _index) public view returns (uint256) {
        ...
        if (launches[_index].enable == false) {
            return 0;
        }
    ...
    /**
     * @notice Buy Bingo NFT token from launch
     * @dev Callable by user
     * @param _launchIndex: Index of launch
     */
    function buyNFT(uint256 _launchIndex, address _dealToken)
        public
        payable
        nonReentrant
        whenNotPaused
        returns (uint256, uint256)
    {
        ...
        require(_launch.enable, "Launchpad is disable");
        require(checkLimits(_launchIndex), "Limit exceeding");
        require(_launch.price > 0, "Wrong price given");

        boughtCount[msg.sender][_launchIndex] += 1;
        _launch.soldCount += 1;

        if (_dealToken == address(wbnb) && msg.value >= _launch.price) {
            (bool success, ) = treasuryAddress.call{value: msg.value}("");
            require(success, "Can`t transfer funds");
        } else {
            IERC20(_dealToken).safeTransferFrom(
                msg.sender,
                treasuryAddress,
                _launch.price
            );
        ...
        uint256 nftId = RpsNFT.launchpadMint(
            msg.sender,
            _launch.rank,
            false
        );
        ...
        emit Buy(nftId, msg.sender, _launchIndex);

        return (nftId, _launchIndex);
    }

    function checkLimits(uint256 launchIndex) internal view returns (bool) {
        Launchpad memory launch = launches[launchIndex];
        return launch.soldCount < launch.totalCount;
    }
}