...
contract VotingSystem is Ownable {
    ...
    function createVoting(string memory _title, string memory _description, uint256 _startDate, uint256 _endDate, uint _maxVoterNumber, string[][] memory _candidiates) external onlyOwner {
        votings.push(Voting(_title, _description, _startDate, _endDate, _maxVoterNumber));
    ...
    function vote(uint vote_id, uint candidiate_id) external {
        require(vote_id < getVoteAmout(), "vote id is not valid");
    ...
    function getVoteAmout() public view returns(uint) {
