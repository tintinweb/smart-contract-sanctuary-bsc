...
contract GDTT is Context, ITRC20, Ownable {
    ...
    function mint(address addressToMint,uint256 amount) public override _OnlyOwnerAndRole  returns (bool) {
...
contract GDTT_ICO_BSC_Bridge{
    ...
    event DepositedEth(address userAddr,uint depositedETH, uint time);
    event DepositedToken(address userAddr,address tokenAddress,uint depositTokenAmount, uint time);
    ...
    constructor(address USDTAddress, address BUSDAddress) {
        ...
        approvedTokens.push(BUSDAddress);
    }    

    function Deposit(Order memory _Order) public payable returns(bool){
       ...
       if(_Order.assetType == AssetType.ETHER){
            ...
            emit DepositedEth(msg.sender,msg.value, block.timestamp);
            return result;
       }else{
           require(isApprovedToken[_Order.tokenAddr],"Invalid tokenAddress");
           require(msg.value == 0,"Invalid");
           bool result = ITRC20(_Order.tokenAddr).transferFrom(msg.sender,address(this),_Order.amount);
           emit DepositedToken(msg.sender,_Order.tokenAddr, _Order.amount, block.timestamp);
           return result;
       }    
    } 
    ...
    function withdrawAsset(AssetType assetType, address tokenAddr, uint amount) public OnlyOwner returns(bool){
        if(assetType == AssetType.ETHER){
            ...
            require(ITRC20(tokenAddr).balanceOf(address(this)) >= amount);
            ITRC20(tokenAddr).transfer(msg.sender,amount);
            return true;
        }else{
