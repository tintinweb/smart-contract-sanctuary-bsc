...
contract CNTPresale is ReentrancyGuard, Ownable {
    ...
    constructor (address _cnt, address _busd, address _wallet) {
        ...
        roundInfos[1].cntPrice = 35;
        roundInfos[2].cntPrice = 38;
        roundInfos[3].cntPrice = 42;

        roundInfos[1].hardCap = 500_000 * 10**18;
        roundInfos[2].hardCap = 750_000 * 10*18;
        roundInfos[3].hardCap = 1_000_000 * 10**18;
    }
     ...
     */
    function startICO(uint8 _roundLV, uint256 _endTime, uint256 _hardCap) external onlyOwner {
        require(_roundLV > 0, "presale-err: roundLV can't be zero");
        require(_roundLV <= maxRoundLV, "presale-err: roundLV exceed maxRoundLV");
        require(_roundLV > currentRound, "presale-err: can't start ICO again");
        require(_endTime > block.timestamp, "presale-err: duration should be > 0");
        require(roundInfos[currentRound].endTime < block.timestamp, "presale-err: previous round is running");

        currentRound = _roundLV;

        roundInfos[_roundLV].startTime = block.timestamp;
        roundInfos[_roundLV].endTime = _endTime;
        if (_hardCap > 0) {
    ...
    function buyTokens(uint256 _amount, address _referrer) external nonReentrant {
        ...
        roundInfos[currentRound].busdAmount += _amount - referralAmount;
        roundInfos[currentRound].investors += 1;

        uint256 purchaseAmount = _getTokenAmount(currentRound, _amount - referralAmount);
        cnt.safeTransfer(msg.sender, purchaseAmount);
    ...
    function _preValidatePurchase(address _beneficiary, uint256 _amount) internal view {
        ...
        require(roundInfos[currentRound].startTime < block.timestamp, "presale-err: no active round");
        require(roundInfos[currentRound].endTime > block.timestamp, "presale-err: no active round");
        this; 
    }
