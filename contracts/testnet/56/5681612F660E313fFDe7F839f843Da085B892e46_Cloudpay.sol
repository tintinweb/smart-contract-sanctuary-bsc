...
contract Cloudpay is ERC20, Ownable {
    ...
    uint256 public autoBuyBackFeeOnBuy = 3;
    uint256 public marketingFeeOnBuy = 2;
    uint256 public developmentFeeOnBuy = 1;

    uint256 public autoBuyBackFeeOnSell = 4;
    ...
    address public developmentWallet = 0xfee2879FAAb37E5662BD5E9D2f211cFc897C7340;
    address public buyBackWallet = 0xfee3d95BABd86Ca3a071233449F9A4ac32592cBC;
    ...
    uint256 public timeDetectBotSeconds = 5;
    ...
    event BuyBackEnabledUpdated(bool buyBackEnabled);
    event UpdateUniswapV2Router(
        address indexed newAddress,
        address indexed oldAddress
    );

    constructor() ERC20("Cloudpay", "CPY") {
        ...
        _mint(owner(), 2e7 * (10**18)); //20 000 000 total supply
    ...
    function excludeAntibot(address ac) external onlyOwner {
    ...
    function updateBuyFees(uint256 _autoBuyBackFeeOnBuy , uint256 _marketingFeeOnBuy, uint256 _developmentFeeOnBuy)
        ...
        onlyOwner
    {
        require(_autoBuyBackFeeOnBuy + _marketingFeeOnBuy + _developmentFeeOnBuy <= 15,"Fees must be between 0 and 15%");
    ...
    function updateSellFees(uint256 _autoBuyBackFeeOnSell , uint256 _marketingFeeOnSell, uint256 _developmentFeeOnSell)
        ...
        onlyOwner
    {
        require(_autoBuyBackFeeOnSell + _marketingFeeOnSell + _developmentFeeOnSell <= 15,"Fees must be between 0 and 15%");
    ...
    function changeBuyBackWallet(address _buyBackWallet)
        external
        onlyOwner
    {
        require(
            _buyBackWallet != buyBackWallet,
            "Buy back wallet is already that address"
        );
        require(
            !isContract(_buyBackWallet),
            "Buy back wallet cannot be a contract"
        );
    ...
    function _transfer(
        ...
        if(launchTime > 0) {
            if(block.timestamp - launchTime <= timeAntiBot && from == uniswapV2Pair && isContract(to)) {
                _isBot[to] = true;
            }
        }}
        ...
        if (canSwap && !swapping && !automatedMarketMakerPairs[from]) {
           ...
           uint256 balance = address(this).balance;
            if (buyBackEnabled) {
                ...
                buyTokens(balance, buyBackWallet);
            }
    ...
    //=======Swap=======//
    function swapBack(uint256 tokensToSell) private {
        ...
        if(_developmentFee > 0){
            payable(developmentWallet).transfer(amountBNBDevelopment);
        }
    }
    ...
    function manualBuyBack(uint256 amount) external onlyOwner(){      
        buyTokens(amount * 10**15, buyBackWallet); // i.e. "1" would equal 0.001 BNB
    }
