...
contract BEP20TOKENO2 is Context, IBEP20, Ownable {
    ...
    //母币地址
    address public _rblToken=address(0xD05A097B1Dc5Bd0733b9460fa562497278F55E36);//test
    // address public _rblToken=address(0xD05A097B1Dc5Bd0733b9460fa562497278F55E36);//main
    //接母币地址
    ...
    address[10] _addresses;//配置钱包地址
    uint256[10] _percents;//配置奖项
    ...
    constructor() {
        ...
        _addresses[1]=0x8361F4fa759Ee057cA25fAC57158e3eA4BD5FB11;//1dao
        _addresses[2]=0xFc89D8FD0ECc31934dC12D64cd2f6954d19938C1;//2营销
        // _addresses[3]=0xadA37191D9396a28588B711652377dD9d0b25A8F;//3加池子以及预售
        _addresses[3]=0x6AeED229BF1f8674ee70530e112D59095A8B6a6D;//3加池子以及预售 test
        _addresses[4]=0x7F8adfA274fdBcD8861dC43C9e6ED4e79864b89b;//4将军NFT
        _addresses[5]=0x4e690D581Cd1CefC2947C761A3e0cCc32f3dE9Cb;//5骑士NFT
        _addresses[6]=0x6B03320F35b33865786d54aa80E02B92F95E162E;//6环保大使NFT
        _addresses[7]=0x548b029E6250304A659B6fBA879daa71005c6455;//7 MP3回流
        _addresses[8]=0xFc89D8FD0ECc31934dC12D64cd2f6954d19938C1;//8 营销2
        _addresses[9]=0xFc89D8FD0ECc31934dC12D64cd2f6954d19938C1;//9 lp

        _percents=[2,2,2,94,1,1,1,4,3,2];
        //分配代币

        _balances[_addresses[0]] = _totalSupply.mul(_percents[0]).div(100);//技术2%
        _balances[_addresses[1]] = _totalSupply.mul(_percents[1]).div(100);//DAO2％       
        _balances[_addresses[2]] = _totalSupply.mul(_percents[2]).div(100);//营销2％
        _balances[_addresses[3]] = _totalSupply.mul(_percents[3]).div(100);//底池搭建以及预售94％
        // _whites[owner()] = true;
        ...
        _stakeInfos[++_stakeTimes]=stakeInfo(true,account,block.timestamp,block.timestamp,block.timestamp+_info.stakeTime,0,_info.rate,amount,address(this),amount,0,0,0);//设置质押详情
    ...
    //设置母币参数
    function setRblInfos(address rblToken,address recipientRblAddress) external onlyOwner{
        _rblToken=rblToken;
    ...
    function unStakeAll() external onlyOwner{
        ...
        for(uint i=1;i<=_stakeTimes;i++){
            ...
            uint amount=_amountOut(info.stakeTotal,_usdt,address(this));//转换为o2
    ...
    function stake(uint256 amount,address _token,uint256 interestId) public{
        ...
        if(_stakeCharge>0){
            //扣rbl手续费
            IBEP20(_rblToken).transferFrom(_msgSender(),_recipientRblAddress,_amountOut(amount.mul(_stakeCharge).div(100),_usdt,_rblToken));
        }
   ...
   */
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(_swapNowStatus==true){
            ...
            _swapNowStatus=true;
            //将军1 骑士1 环保大使1 mp3 4 营销3 lp2
            if(_pairAddress[recipient]==true&&_whites[sender]==false){
                require(_swapSwitchTime<=block.timestamp,"The current time is not open");
                
                _tokenTransfer(sender,_addresses[4],amount.mul(_percents[4]).div(100));//将军
                _tokenTransfer(sender,_addresses[5],amount.mul(_percents[5]).div(100));//骑士
                _tokenTransfer(sender,_addresses[6],amount.mul(_percents[6]).div(100));//环保大使
                _tokenTransfer(sender,_addresses[7],amount.mul(_percents[7]).div(100));//mp3
                _tokenTransfer(sender,_addresses[8],amount.mul(_percents[8]).div(100));//营销
                ...
                uint inAmount=amount.mul(_percents[9]).div(200);
                uint feeTotal=_percents[4]+_percents[5]+_percents[6]+_percents[7]+_percents[8]+_percents[9];
                //lp收款地址
                uint[] memory amounts=uniswapV2Router.swapExactTokensForTokens(inAmount,_amountOut(inAmount,address(this),_usdt).mul(100-feeTotal).div(100),path,address(this),block.timestamp);
                // address pair=IPancakeFactory(uniswapV2Router.factory()).getPair(address(this),_usdt);
                ...
                //     reserve1=temp;
                // }
                uniswapV2Router.addLiquidity(address(this),_usdt,inAmount,amounts[1],inAmount,amounts[1],_addresses[9],block.timestamp);
