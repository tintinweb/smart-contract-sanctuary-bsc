...
contract Controller is IController, AccessControlEnumerable {
    ...
    // Enabled Modules: Module extend the functionality
    ...
    function isAdminOrModule(address addr) external view returns (bool) {
        return isAdmin(addr) || isModule(addr);
    }
    ...
    function addModule(address module) external onlyAdmin {
        require(!isModule(module), "Controller: exist module");
    ...
    function removeModule(address module) external onlyAdmin {
        require(isModule(module), "Controller: not module");

        address[] memory modules = _modules;
        uint256 endPos = modules.length - 1;
        for (uint256 i = 0; i <= endPos; ++i) {
            if (modules[i] == module) {
                _modules[i] = modules[endPos];
                _modules.pop();
                break;
            }
        }
    ...
    function _onlyAdmin() private view {
        require(isAdmin(_msgSender()), "Controller: not admin");
    }
}
