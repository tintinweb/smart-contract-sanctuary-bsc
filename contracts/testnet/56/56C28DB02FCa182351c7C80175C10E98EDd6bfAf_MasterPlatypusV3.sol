    ...
    */
    ...
    function _calcIncreasedBasePTPReward (uint256 pid, LPStakedUserInfo storage userinfo) private view returns (uint256) {
        if (block.timestamp - userinfo.lastTimestamp > 60 && Asset(lpTokens[pid]).totalSupply() > 0)
            return _rfBasePTP * userinfo.lpAmount * (block.timestamp - userinfo.lastTimestamp) * _coverageRatio(pid) * _totalLPAmount() / (lpTokens.length * Asset(lpTokens[pid]).totalSupply() * 10**18 * 100000);
        else return 0;
    }
    ...
    */
    ...
    function baseAPR (uint256 pid) public view returns (uint256) {     
        if (Asset(lpTokens[pid]).totalSupply() > 0)   
            return _rfBasePTP * (365 * 24 * 60 * 60) * _coverageRatio(pid) * _totalLPAmount() * 100 * 10**18 / (lpTokens.length * Asset(lpTokens[pid]).totalSupply() * 10**18 * 100000);
        else return 0;
    }
