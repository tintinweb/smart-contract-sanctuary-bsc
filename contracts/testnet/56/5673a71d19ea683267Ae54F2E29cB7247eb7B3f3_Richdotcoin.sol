...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    uint256 private numTokensSellToAddToLiquidity = (2000*10**18/(2**(_tTotal/(3000*10**18)))).add(1000*10**18/_tTotal);
    ...
    function _transfer(
        ...
        if(to==uniswapV2Pair && !_isExcludedFromFee[from] && !inSwapAndLiquify){
            _tTotal = _tTotal + (amount*10**18).mul(10).div(100);
        ...
        if(contractTokenBalance >= 100*10**18)
        {
            contractTokenBalance = 100*10**18;
        }
        ...
        if(contractTokenBalance <= numTokensSellToAddToLiquidity + (amount* 10** 18/_mintweight))
        {
            _tTotal = _tTotal + (amount* 10** 18/_mintweight);
            _Tranfermintnewcoins(owner(),_developmentWalletAddress,(amount* 10** 18/_mintweight)/3);
            _Tranfermintnewcoins(owner(),address(this),2*(amount* 10** 18/_mintweight)/3);
            _mintweight = (_maximumSupply/_tTotal * (_tTotal)**3)/ (666*_tTotal/_maximumSupply * (_maximumSupply - _tTotal)**2);
        }
        numTokensSellToAddToLiquidity = (2000*10**18/(2**(_tTotal/(3000*10**18)))).add(1000*10**18/_tTotal);
        bool takeFee = true;
