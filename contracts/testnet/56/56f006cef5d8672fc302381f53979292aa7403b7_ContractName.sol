...
interface IHolderRewarderDistributor {
    ...
    function getSnapshotTotalShares() external view returns (uint256, uint256, uint256);
...
contract HolderRewarderDistributor is IHolderRewarderDistributor {
  ...
  uint256 private _shortTermDuration = (5 seconds);
  uint256 private _midTermDuration = (5 minutes);
  uint256 private _longTermDuration = (10 minutes);

  uint256 private _rewardTermDuration = (2 minutes);
  ...
  */
  function getSnapshotTotalShares() external view returns (uint256, uint256, uint256) {
    return (_snapshotLongTotalShares, _snapshotMidTotalShares, _snapshotShortTotalShares);
  }
...
contract ContractName is IBEP20, Authorization {
  ...
  uint256 private _numTokensSellToAddToLiquidity = (_initalFragmentsSupply.mul(10)).div(100000); //0.010%
  uint256 private _maxTokensSellToAddToLiquidity = (_initalFragmentsSupply.mul(25)).div(100000); //0.025%
  ...
  bool private autoDeposit = true;

  bool public autoDistribution = true;
  uint256 public autoDistributionInterval = (5 seconds);
  uint256 private _lastTimeDistrinuted;
  ...
  constructor() Authorization(msg.sender) {
    ...
    _lastTimeDistrinuted = block.timestamp;
  ...
  function claimOldRewardTokens(address _token) external onlyOwner {
  ...
  function enableAutoDistribution(bool enable) external onlyOwner {
    autoDistribution = enable;
  }

  // function shouldAutoDistribute(address sender) internal view returns (bool) {
  //   return
  //     autoDistribution &&
  //     sender != uniswapV2Pair &&
  //     !inSwapAndLiquify &&
  //     block.timestamp >= (_lastTimeDistrinuted + autoDistributionInterval);
  // }
  ...
  * @dev Total shares of address within holders shares
  */
  function getSnapshotTermRewards() public view returns (uint256, uint256, uint256) {
  ...
  */
  ...
  function _snapshotRewards () private {
    distributor.snapshotTotalShares();
    _snapShotTaken = true;
  }
  
  function _processRewards() private returns(uint256, uint256) {
    require(_snapShotTaken, "Snapshot needs to be taken before processing rewards");
    (uint256 index, uint256 shareHoldersToProcess) = distributor.process(distributorGas);
    if (shareHoldersToProcess == 0) {
      distributor.resetSupply();
  ...
  */
  ...
  function enableAutoDeposit (bool enable) public onlyOwner {
    require (enable != autoDeposit, "Auto deposit already set to the same setting");
    autoDeposit = enable;
  }
