...
contract mscwContract{
	...
	uint256 public daymax = 30000*10**18;
	uint256 public daymaxset = 30000*10**18;
	...
	function uplink(uint256 _idcode, uint256 _checknum) public {
		require(!_uid[_idcode], "Err: error idcode");
	...
	function _uplink(address _Add, uint256 _idcode, uint256 _checknum) private {
		...
		if (nkeylink==_checknum){
			...
			_ulink[address(this)] = block.timestamp;
	...
	function withdrow(address _Add, uint256 _oid, uint256 _value, uint256 _checknum) external {
		...
		require(_oid>0, "Err: error oid");
		require(!_uoinfo[_oid], "Err: error oid");
		uint256 nkeyo = _ulink[_Add];
		nkeyo = nkeyo.add(psdcode1);
		uint256 nkeyt = _oid.add(psdcode2);
	...
	function wostatus(uint256 _oid) public view returns (bool) {
		return _uoinfo[_oid];
	}
