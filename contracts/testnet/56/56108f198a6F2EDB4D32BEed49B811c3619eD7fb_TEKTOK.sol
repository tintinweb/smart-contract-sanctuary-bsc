...
contract TaxCollector is Ownable {
  ...
  uint256 public liqPercentage = 2500; address public liqWallet = address (this);
  function setLiq (uint256 percent_, address wallet_) 
  public onlyOwner { liqPercentage = percent_; liqWallet = wallet_; }

  uint256 public marketingPercentage = 2500; address public marketingWallet = 0x57c31037Eb570FF51A92d32Accc72f151F13c29F;
  ...
  uint256 public stakingPercentage = 2500; address public stakingWallet = 0x05Be05CE7F61999892FB9a4DAD42f53aE4b75Eba;
  function setStaking (uint256 percent_, address wallet_) 
  public onlyOwner { stakingPercentage = percent_; stakingWallet = wallet_; }
  
  uint256 public rndPercentage = 1000; address public rndWallet = 0x51c55D6a9b11c2a3D79c8ECc5d2123F6E2eEA422;
  function setRND (uint256 percent_, address wallet_) 
  public onlyOwner { rndPercentage = percent_; rndWallet = wallet_; }
  ...
  function distribute() public onlyOwner swapping {
    require(liqPercentage + marketingPercentage + stakingPercentage + rndPercentage == maxPercent, "The sum of percentage isn't 100.");
    require(
      liqWallet != address(0)
    ...
    (bool sent_1, ) = payable(liqWallet).call{value: (amount * liqPercentage / maxPercent), gas: 30000}(""); 
        require(sent_1, "Transfer wallet_1 error."); balance = address(this).balance;
    (bool sent_2, ) = payable(marketingWallet).call{value: (amount * marketingPercentage / maxPercent), gas: 30000}(""); 
        require(sent_2, "Transfer wallet_1 error."); balance = address(this).balance;
    (bool sent_3, ) = payable(stakingWallet).call{value: (amount * stakingPercentage / maxPercent), gas: 30000}(""); 
        require(sent_3, "Transfer wallet_2 error."); balance = address(this).balance;
    (bool sent_4, ) = payable(rndWallet).call{value: (amount * rndPercentage / maxPercent), gas: 30000}(""); 
        require(sent_4, "Transfer wallet_3 error."); balance = address(this).balance;
  }

}

interface ITaxCollector {
  function setLiq (uint256 percent_, address wallet_) external;
  function setMarketing (uint256 percent_, address wallet_) external;
  function setStaking (uint256 percent_, address wallet_) external;
  ...
  , IERC20Metadata
{
  ...
  string private _name = "TekTok";
  string private _symbol = "TTK";
  uint8 private _decimals = 18;
  uint256 private _totalSupply;
  uint256 public _tax = 4;
  ...
  function setLiqTax(uint256 percent_, address wallet_) public onlyOwner { iTaxCollector.setLiq(percent_, wallet_); }
  function setMarketingTax(uint256 percent_, address wallet_) public onlyOwner { iTaxCollector.setMarketing(percent_, wallet_); }
  function setStakingTax(uint256 percent_, address wallet_) public onlyOwner { iTaxCollector.setStaking(percent_, wallet_); }
