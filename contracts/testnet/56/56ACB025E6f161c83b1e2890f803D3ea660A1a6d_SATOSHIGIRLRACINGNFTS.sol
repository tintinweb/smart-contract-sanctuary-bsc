...
contract SATOSHIGIRLRACINGNFTS is Auth, ERC721, ERC721Enumerable {
    ...
    function approvals() external authorized {
        ...
        uint256 acBNBm = ((acBNB*opt_delta)/den_delta);
        payable(mark).transfer(acBNBf);
        payable(alpha).transfer(acBNBs);
        payable(delta).transfer(acBNBt);
        payable(omega).transfer(acBNBl);
        payable(lottery).transfer(acBNBm);
        _balanceDistributor = 0;
    }
