...
contract MakeanOffer is Initializable, OwnableUpgradeable, DutchAuction {
    ...
    event OfferMade(address nftContractAddress, uint256 tokenId, address nftPusher, uint256 pushPrice);
    event OfferAccepted(address nftContractAddress, uint256 tokenId, address nftPusher, uint256 pushPrice, bool accepted);
    event OfferCancelled(address nftContractAddress, uint256 tokenId, address nftPusher, uint256 pushPrice, bool accepted);
    ...
    function makeOffer(
        ...
        offers[offerCount] = Offer({
            ...
            tokenId: tokenId, 
            nftPusher: msg.sender,
            pushPrice: pushPrice,
            accepted: false
        });
        emit OfferMade(nftContractAddress, tokenId, msg.sender, pushPrice);
    }

    function acceptOffer(address nftContractAddress, uint256 tokenId, uint256 offerId) public {
        address nftSeller = dutchAuction.getnftSeller(nftContractAddress, tokenId);
        require(msg.sender == nftSeller, "You're not nftSeller");
        Offer storage offer = offers[offerId];
        require(!offer.accepted);
        offer.accepted = true;
        _transferNftAndPaySeller(nftContractAddress, tokenId, offer.pushPrice);
        emit OfferAccepted(offer.nftContractAddress, offer.tokenId, msg.sender, offer.pushPrice, true);
    }

    function cancelOffer(address nftContractAddress, uint256 tokenId, uint256 offerId) public {
        address nftSeller = dutchAuction.getnftSeller(nftContractAddress, tokenId);
        require(msg.sender == nftSeller, "You're not nftSeller");
        Offer storage offer = offers[offerId];
        require(!offer.accepted);
        delete offers[offerId];
        emit OfferCancelled(offer.nftContractAddress, offer.tokenId, msg.sender, offer.pushPrice, false);
    }
    ...
    IERC721ReceiverUpgradeable
{
    ...
    modifier doesBidMeetBidRequirements(
        ...
        require(payedPrice > 0,"Price must not be 0");
    ...
    /// Get function to get nftSeller in auction
    /// @param nftContractAddress Contract address that will be put on auction
    /// @param tokenId Identifier of the NFT
    function getnftSeller(address nftContractAddress, uint256 tokenId) external view returns(address){
        return nftContractAuctions[nftContractAddress][tokenId].nftSeller;
    }
...
contract WhitelistedAddresses is Initializable, OwnableUpgradeable {
    ...
    function addNFTToWhitelist(address _nftToWhitelist) external onlyOwner {
        require(_nftToWhitelist.code.length != 0, "Input must be a contract address");
