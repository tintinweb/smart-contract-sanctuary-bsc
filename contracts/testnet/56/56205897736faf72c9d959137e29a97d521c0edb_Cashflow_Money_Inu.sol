...
contract Cashflow_Money_Inu is IBEP20, Ownable {
    ...
    function getsellfee() internal view returns (uint256) {
    ...
    function setBuyFeeCount(uint256 _amount) internal {
     ...
     function setSellFeeCount(uint256 _amount) internal {
        _liquedityFeeCounter = (_amount * (_liqueditySellFee+(getsellfee()/totalSellFee)*_liqueditySellFee)) / (feeDivider);
        _reflectionFeeCounter = (_amount * (_reflectionSellFee+(getsellfee()/totalSellFee)*_reflectionSellFee)) / (feeDivider);
        _marketFeeCounter = (_amount * (_marketSellFee+(getsellfee()/totalSellFee)*_marketSellFee)) / (feeDivider);
