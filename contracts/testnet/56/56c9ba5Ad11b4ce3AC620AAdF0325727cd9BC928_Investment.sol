...
contract Investment is Context, IERC20 { 
    ...
    */
    ...
    address payable public Wallet_Investment = payable(0x0Aa57f85bfe0579c29758C123451eF8A523EB0d9); 
    address payable public Wallet_LP = payable(0x7beCAFc64B35077841CD07C352e4e528007795d2);
    address payable public Wallet_Development = payable(0x2251b92edB4579526D9700b452E0855dE1AAfB1E); 
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 


    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 18;
    uint256 private _tTotal = 5000000000000 * 10**_decimals;
    uint256 private OriginalSupply = _tTotal;
    string private constant _name = "We Finance"; 
    ...
    uint256 public Percent_Investment = 50;
    uint256 public Percent_Burn = 0;
    uint256 public Percent_Development_Wallet = 20;
    ...
    constructor () {
        ...
        _owner = 0xd826A4F62011B4245D38a99478e5022f814a56bc;
        ...
        */
        ...
        _isLimitExempt[Wallet_Investment] = true; 
        ...
        _isExcludedFromFee[Wallet_Investment] = true; 
    ...
    */
    ...
    function _set_Fee_Distribution_Percent__Total_100(uint256 Investment_Wallet, uint256 Auto_Liquidity, uint256 Development_Wallet, uint256 Auto_Burn) external onlyOwner() {

        require((Investment_Wallet + Auto_Liquidity + Development_Wallet + Auto_Burn) == 100, "Must add up to 100!");
   
        Percent_Investment = Investment_Wallet;
        Percent_Burn = Auto_Burn;
        Percent_Development_Wallet = Development_Wallet;
    ...
    */
    ...
    event updatedInvestmentWallet(address indexed oldWallet, address indexed newWallet);
    function Wallet_Update_Investment(address payable wallet) external onlyOwner() {
        // Can't be zero address
        require(wallet != address(0), "new wallet is the zero address");
        emit updatedInvestmentWallet(Wallet_Investment,wallet);

        // Update mapping on old wallet
        _isExcludedFromFee[Wallet_Investment] = false; 

        Wallet_Investment = wallet;
        // Update mapping on new wallet
        _isExcludedFromFee[Wallet_Investment] = true;
    }

    //Update the Token wallet 
    event updatedDevelopmentWallet(address indexed oldWallet, address indexed newWallet);
    function Wallet_Update_Tokens(address payable wallet) external onlyOwner() {
        // Can't be zero address
        require(wallet != address(0), "new wallet is the zero address");      
        emit updatedDevelopmentWallet(Wallet_Development,wallet);
    ...
    */
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        if (Percent_Development_Wallet != 0){

            // Send Tokens to the token wallet - used for team tokens and giveaway promotions
            uint256 tokens_to_Wallet = contractTokenBalance * Percent_Development_Wallet / 100;
            _tOwned[Wallet_Development] = _tOwned[Wallet_Development] + tokens_to_Wallet; 
        ...
        if (Percent_AutoLP != 0 && Percent_Investment != 0){

            // Calculate how many tokens need to be swapped for BNB (Investment BNB and 1/2 Auto Liquidity)

            uint256 tokens_to_M = contractTokenBalance * Percent_Investment / 100;
            ...
            uint256 split_M = Percent_Investment * 100 / (Percent_AutoLP + Percent_Investment);
            ...
            // Send BNB to Investment wallet - Check again incase some is added during calculations
            BNB_Total = address(this).balance;
            sendToWallet(Wallet_Investment, BNB_Total);

          

        } else if (Percent_AutoLP == 0 && Percent_Investment != 0){

            // Swap tokens for BNB and send to Investment Wallet if Auto LP is 0

            uint256 tokens_to_M = contractTokenBalance * Percent_Investment / 100;
            swapTokensForBNB(tokens_to_M);
            uint256 BNB_M = address(this).balance;
            sendToWallet(Wallet_Investment, BNB_M);
            ...
            // Create the Auto LP if Investment is 0
    ...
    */
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!takeFee){
            ...
            // Transaction is Buy p
            uint256 buyFEE = (tAmount.mul(_Tax_On_Buy)).div(100);
            uint256 tTransferAmount = tAmount.sub(buyFEE);
            uint256 Wallet_Investmentfee= (buyFEE.mul(50)).div(100);
            uint256 autoliquidityfee=(buyFEE.mul(23)).div(100);
            uint256 developmentfee=(buyFEE.mul(27)).div(100);

            _tOwned[sender] = _tOwned[sender].sub(tAmount);
            _tOwned[recipient] = _tOwned[recipient].add(tTransferAmount);
            _tOwned[Wallet_Investment] = _tOwned[Wallet_Investment].add(Wallet_Investmentfee); 
            _tOwned[Wallet_LP] = _tOwned[Wallet_LP].add(autoliquidityfee);
            _tOwned[Wallet_Development] = _tOwned[Wallet_Development].add(developmentfee);
            ...
            ///////////////////////////////////
             uint256 Wallet_Investmentfee= (sellFEE.mul(46)).div(100);
            uint256 autoliquidityfee=(sellFEE.mul(39)).div(100);
            uint256 developmentfee=(sellFEE.mul(15)).div(100);
