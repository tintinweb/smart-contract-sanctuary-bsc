    ...
    TokensRecoverable
{
    ...
    //mapping(uint32 => uint256) public stakedAtValue; // nft id => stake value from BLL
    ...
    function stake(
        ...
        uint totValue;
        for (uint i; i < tokenIds.length; i = i) {
    ...
    function syncPointsForTokenIDs(
        ...
        for (uint i; i < nftIDs.length; i = i) {
            uint32 tokenId = nftIDs[i];
            /*             if (BLLPoints[tokenId] == 0)
 ...
 */
            ...
            uint256 value = BLLContract.getPointsForTokenID(tokenId) *
                1e18;
    ...
    function _updatePointsForID(uint32 tokenId, uint256 prevValue, uint256 newValue) private onlyOwner {
    ...
    function syncPointsForTokenIDsRange(
        ...
        for (uint32 i = startNFTID; i <= endNFTID; i = i) {
            /*             if (BLLPoints[i] == 0)
                // will not delete this nft id from array if withdrawn NFT as it is gas costly, keeping isStaked[tokenId] to track status & may call call refresh_AllNFTids()
                allNFTIds.push(i); */
            uint256 prevValue = tokenPointsFromBll[i];
            uint256 value = BLLContract.getPointsForTokenID(i) *
                1e18;
    ...
    function tokenIdsStaked(
        ...
        for (uint32 x; x < arrStaked.length; ++x) {
            //StakedNFT memory stakedNFT = tokenIDsToStakedNFT[arrStaked[x]];
        ...
        for (uint32 x; x < arrStaked.length; ++x) {
            //StakedNFT memory stakedNFT = tokenIDsToStakedNFT[arrStaked[x]];
            //if (stakedNFT.user != account || !stakedNFT.isStaked) continue;
            if (ownerOfNFT[arrStaked[x]] != account) continue;
...
/*     function updatePoints(uint32[] memory nftIDs) public onlyOwner {
        for (uint32 i; i < nftIDs.length; i = unchecked_inc_32(i))
            updatePointsForID(nftIDs[i]);
        emit UpdatedPoints(nftIDs);
    }

    function updatePointsForID(uint32 tokenId) public onlyOwner {
        //if (!tokenIDsToStakedNFT[tokenId].isStaked) return;
        if (ownerOfNFT[tokenId] == address(0)) return;
        ...
        uint value = tokenPointsFromBll[tokenId];
        uint prevValue = stakedAtValue[tokenId];
        //address account = ownerOfNFT[tokenId];
        //address account = tokenIDsToStakedNFT[tokenId].user;
        address account = ownerOfNFT[tokenId];
        _refreshReward(account);
        _unstakeValue(
            account,
            prevValue > value ? prevValue - value : value - prevValue
        );
        stakedAtValue[tokenId] = value;
    } */
...
/*     function updatePointsRange(
    ...
    function _withdraw(
        ...
        for (uint32 i = 0; i < tokenIds.length; i) {
            ...
            //require(stakedNFT.user == _msgSender(), "NOT_NFT_OWNER");
            //require(stakedNFT.isStaked, "NOT_STAKED");
            //value += stakedAtValue[tokenIds[i]];
