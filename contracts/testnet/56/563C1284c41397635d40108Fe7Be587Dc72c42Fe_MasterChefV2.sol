...
/// @notice The (older) MasterChef contract gives out a constant number of DUTY tokens per block.
/// It is the only address with minting rights for DUTY.
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    ///   pending reward = (user share * pool.accDutyPerShare) - user.rewardDebt
    ///
    ...
    ///     DUTY rewards each pool gets.
    /// `accDutyPerShare` Accumulated DUTYs per share, times 1e12.
    ...
    ///     the DUTY rewards to all the DutySwap products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accDutyPerShare;
    ...
    /// @notice Address of DUTY contract.
    IBEP20 public immutable DUTY;

    /// @notice The only address can withdraw all the burn DUTY.
    ...
    ///  @notice 40 dutys per block in MCV1
    uint256 public constant MASTERCHEF_DUTY_PER_BLOCK = 20 * 1e18;
    uint256 public constant ACC_DUTY_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total duty rate = toBurn + toRegular + toSpecial
    uint256 public constant DUTY_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of DUTY burn action being executed.
    /// @notice DUTY distribute % for burn
    uint256 public dutyRateToBurn = 175000000; // 0.0175%
    /// @notice DUTY distribute % for regular farm pool
    uint256 public dutyRateToRegularFarm = 703847222222; // 70.384%
    /// @notice DUTY distribute % for special pools
    uint256 public dutyRateToSpecialFarm = 295902777778; // 29.590%
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accDutyPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateDutyRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// @param _MASTER_CHEF The DutySwap MCV1 contract address.
    /// @param _DUTY The DUTY token contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of DUTY.
    ...
    function init(IBEP20 dummyToken) external onlyOwner {
        ...
        // MCV2 start to earn DUTY reward from current block in MCV1 pool
    ...
    /// only for DUTY distributions within DutySwap products.
    function add(
        uint256 _allocPoint,
        IBEP20 _lpToken,
        bool _isRegular,
        ...
        // stake DUTY token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != DUTY, "DUTY token can't be added to farm pools");
    ...
    /// @notice Update the given pool's DUTY allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending DUTY rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    ...
    function pendingDuty(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 dutyReward = multiplier.mul(dutyPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accDutyPerShare = accDutyPerShare.add(dutyReward.mul(ACC_DUTY_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accDutyPerShare).div(ACC_DUTY_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update duty reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of DUTY per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function dutyPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_DUTY_PER_BLOCK.mul(dutyRateToRegularFarm).div(DUTY_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_DUTY_PER_BLOCK.mul(dutyRateToSpecialFarm).div(DUTY_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of DUTY per block to burn.
    function dutyPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_DUTY_PER_BLOCK.mul(dutyRateToBurn).div(DUTY_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accDutyPerShare = pool.accDutyPerShare.add((dutyReward.mul(ACC_DUTY_PRECISION).div(lpSupply)));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accDutyPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingDuty(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accDutyPerShare).div(
            ACC_DUTY_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @notice Harvests DUTY from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    /// @notice Send DUTY pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnDuty(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingDutyToBurn = multiplier.mul(dutyPerBlockToBurn());

        // SafeTransfer DUTY
        _safeTransfer(burnAdmin, pendingDutyToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of DUTY distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of DUTY to burn each block.
    /// @param _regularFarmRate The % of DUTY to regular pools each block.
    /// @param _specialFarmRate The % of DUTY to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateDutyRate(
        ...
        require(
            ...
            "MasterChefV2: Duty rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == DUTY_RATE_TOTAL_PRECISION,
            "MasterChefV2: Total rate must be 1e12"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
        // burn duty base on old burn duty rate
        burnDuty(false);

        dutyRateToBurn = _burnRate;
        dutyRateToRegularFarm = _regularFarmRate;
        dutyRateToSpecialFarm = _specialFarmRate;

        emit UpdateDutyRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingDuty(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accDutyPerShare).div(
            ACC_DUTY_PRECISION
        );
    ...
    /// @notice Settles, distribute the pending DUTY rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingDuty(
        address _user,
        uint256 _pid,
        ...
        uint256 accDuty = boostedAmount.mul(poolInfo[_pid].accDutyPerShare).div(ACC_DUTY_PRECISION);
        uint256 pending = accDuty.sub(user.rewardDebt);
        // SafeTransfer DUTY
        _safeTransfer(_user, pending);
    }

    /// @notice Safe Transfer DUTY.
    /// @param _to The DUTY receiver address.
    /// @param _amount transfer DUTY amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            // Check whether MCV2 has enough DUTY. If not, harvest from MCV1.
            if (DUTY.balanceOf(address(this)) < _amount) {
                harvestFromMasterChef();
            }
            uint256 balance = DUTY.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            DUTY.safeTransfer(_to, _amount);
        }
    }
}
