 ...
 * @author Leo
 */
contract SamuraiLegendsNFTs is ERC721Enumerable, AccessControl {
    ...
    constructor(string memory name_, string memory symbol_, string memory tokenURI_, uint maxSupply_) ERC721(name_, symbol_) {
         /**
    ...
    /**
     * @notice Updates max supply.
     * @param maxSupply_ New max supply.
     */
    function updateMaxSupply(uint maxSupply_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(maxSupply_ > this.totalSupply(), "invalid maxSupply value");
     ...
     */
    function mint(address user) public onlyRole(MINTER_ROLE) returns (uint) {
        uint256 id = _tokenIds.current();

        require(id < maxSupply, "mint limit reached");
