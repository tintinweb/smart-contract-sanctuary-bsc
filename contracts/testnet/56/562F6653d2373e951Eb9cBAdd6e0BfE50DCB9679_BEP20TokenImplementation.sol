...
contract BEP20TokenImplementation is
  // Initializable,
  OwnableUpgradeable,
  ...
  ReentrancyGuardUpgradeable
{
  ...
  function setMintable(bool _setMint) external onlyOwner{
    _mintable = _setMint;
  }
  ...
  function setMarketingFeePercent(uint256 marketingFeeBps) external onlyOwner {
    require(marketingFeeBps >= 0 && marketingFeeBps <= 10**4, "Invalid bps");
  ...
  function _mint(address account, uint256 amount) internal{
        require(account != ZERO, "BEP20: mint to the zero address");

        _beforeTokenTransfer(ZERO, owner(), amount);
        _tTotal = _tTotal.add(amount);
        _rOwned[owner()] = _rOwned[owner()].add(amount);
        emit Transfer(ZERO, owner(), amount);
        _afterTokenTransfer(ZERO, owner(), amount);
  }
  ...
  function _burn(address account, uint256 amount) internal {
    ...
    //====================
        ...
        _beforeTokenTransfer(account, ZERO, amount);
        ...
        _afterTokenTransfer(account, ZERO, amount);
  }
