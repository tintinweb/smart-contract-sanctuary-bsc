...
contract Eternal is Context, Ownable, Token {
    ...
    constructor() {
        ...
        _properties.sellCooldown            = 1 days;
    ...
    function deposit(address referrer, uint256 amount) public payable {
        ...
        _compoundBUSD(msg.sender); //compound accumulated divs to ETRNL Pool before accepting new BUSD deposits.
    ...
    function _refPayout(address addr, uint256 amount, uint256 refBonus) internal {
        ...
        for (uint256 i = 0; i < _properties.refDepth; i++) {
            ...
            if (user.referralRoundRobinPosition == i) {
                ...
                if (
                    _users[upline].stakeBUSD.totalStaked >= _properties.minInvestAmount &&
                    _users[upline].stakeEternal.totalStaked <= maxStakeFor(upline)
                ) {
                    // pay in minting token deposits
    ...
    function _processFee(uint256 amount) internal returns (uint256) {
        ...
        BUSDToken.transfer(owner(), devTax);
        return devTax;
    }
    ...
    function maxPayoutFor(address addr) public view returns(uint256) {
        ...
        return amount.mul(maxPayoutBUSD).div(percentDivider); 
    }
