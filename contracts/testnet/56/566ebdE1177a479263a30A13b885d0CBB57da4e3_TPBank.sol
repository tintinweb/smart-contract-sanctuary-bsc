...
contract TPBank is Ownable, Pausable, ReentrancyGuard {
  /// @dev refer to https://docs.openzeppelin.com/contracts/2.x/api/token/erc20#IERC20
  IERC20 public ThePeoplesToken;
  IERC20 public busd;

  mapping (address => uint) private balances;
  mapping (address => uint) private balancesBusd;
  ...
  event LogWeiDepositBusd(address _accountAddress, uint _depositAmount);
  ...
  event LogWeiWithdrawalBusd(address _accountAddress,uint _withdrawalAmountBusd, uint _updatedBalanceBusd);

  /**
    @param _Token the ERC20 token this contract will use
  ...
  */
  constructor (IERC20 _Token, IERC20 _Busd) {
    ThePeoplesToken = _Token; // TPT TestNet = 0xB6651E4E9D3280E7FB3Ee59139953a18d15715A9
    busd = _Busd;  // for MainNet BUSD = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56  // for TestNet BUSD = 0xeD24FC36d5Ee211Ea25A80239Fb8C4Cfd80f12Ee
  }
  ...
  modifier hasFundsBusd(uint _amountToWithdrawInWeiBusd) {
    require(balances[msg.sender] >= _amountToWithdrawInWeiBusd, "Insufficient funds");
    _; 
  }
  ...
  /** @notice In production the requirement would be for a user to hold a deposit in the bank for >= 1 year:
      For the purpose of testing this project, the user will receive ThePeoplesToken for making an initial deposit 
  ...
  */
  ...
  function depositBusd(uint amount) external payable nonReentrant {

    IERC20(busd).transferFrom(msg.sender, address(this), amount);
    require (amount > 0, "Must deposit at least 1 wei");
    if(timestamp[msg.sender] == 0 && balances[msg.sender] == 0) {
      timestamp[msg.sender] = block.timestamp;
    }

    balancesBusd[msg.sender] += amount;

    emit LogWeiDepositBusd(msg.sender, amount);


  }

  function withdraw(uint _amountToWithdrawInWei) external payable hasFunds (_amountToWithdrawInWei) nonReentrant {
  ...
  function withdrawBusd(uint _amountToWithdrawInWeiBusd) external payable hasFunds (_amountToWithdrawInWeiBusd) nonReentrant {
    
    address payable _user = payable(msg.sender);
    
   
    balancesBusd[msg.sender] -= _amountToWithdrawInWeiBusd;
    
    
    timestamp[msg.sender] = 0;

    
    (bool success, ) = _user.call{value:_amountToWithdrawInWeiBusd}("");
    require(success, "Transfer failed.");
   
    emit LogWeiWithdrawalBusd(msg.sender, _amountToWithdrawInWeiBusd, balancesBusd[msg.sender]);
  }

  
  function transferThePeoplesTokenToContract(uint _amountToContractInThePeoplesToken) external onlyOwner {
    address from = msg.sender;

   ThePeoplesToken.transferFrom(from, address(this), _amountToContractInThePeoplesToken);
  }

 
  function mintThePeoplesTokenToUser(address _user) public isHodler(_user) nonReentrant {
   
    require(ThePeoplesToken.balanceOf(_user) == 0, "User has already claimed rewards");
    
    /// @notice Bank transfer 1000 ThePeoplesToken (ERC20 token) to an account holder
    ThePeoplesToken.transfer(_user, 1000 * 10**18);
  ...
  function getWeiBalanceBusd() external view returns (uint) {
    return balancesBusd[msg.sender];
  }
