...
import "./PositionBondFunction.sol";
import "./PositionBondLending.sol";
import "../test/TestPositionBondLending.sol";

contract PositionBondFactory is BondFee, ReentrancyGuard {
    mapping(address => address[]) public positionBonds;
    PositionBondFunction public positionBondFunction;
    ...
    constructor(PositionBondFunction positionBondFunction_, address chainLinkPriceFeed_ ) {
    ...
    function setBondFee(uint256 bondFee_) public override(BondFee) onlyAdmin {
    ...
    function issueBond(
        ...
        TestPositionBondLending positionBond = new TestPositionBondLending(
            bondInformation,
            assetInformation,
            issuer
        );
        positionBond.initAdmin(positionAdmin);
        positionBond.initChainLinkPriceFeed(chainLinkPriceFeed);

        require(positionBond.isNotReachMaxLoanRatio(), "!Loan Ratio");

        positionBonds[issuer].push(address(positionBond));
    ...
    function updatePositionBondFunction(address positionBondFunction_) public onlyAdmin{
        positionBondFunction = PositionBondFunction(positionBondFunction_);
    }

    function updateChainLinkPriceFeed(address chainLinkPriceFeed_) public onlyAdmin {
        chainLinkPriceFeed = chainLinkPriceFeed_;
    }

    function updatePositionBondAdmin(address positionAdmin_) public onlyAdmin {
        positionAdmin = positionAdmin_;
    }

    //    function issueBondNFT(
    //        string memory bondName,
    //        string memory bondSymbol,
    //        string memory bondDescription,
    ...
    //            PositionBondLending.BondInformation(
    //                bondName,
    //                bondSymbol,
    //                bondDescription,
    //                totalSupply,
    ...
    //                issuePrice
    //            ),
    //            PositionBondLending.AssetInformation(
    //                underlyingAsset,
    //                collateralAmount,
    //                faceAsset,
    //                faceValue,
    ...
    //        );
    //
    //        positionBonds[issuer].push(
    //            address(positionBond)
    //        );
    ////        positionBond.active(block.timestamp, active, active + duration * 1 days);
    //
    //
    //        // TODO transfer NFT
    //    }
    ...
    function _takeFee(
        ...
        uint256 fee = _calculateFeeIssue(collateralAmount);
        if (fee > 0 ) {
            IERC20(underlyingAsset).transferFrom(
                issuer,
                address(this),
                _calculateFeeIssue(collateralAmount)
            );
        }
    }
}
...
abstract contract BondFee {
    uint256 private _bondFee;

    function _calculateFeeIssue(uint256 collateralAmount)
        internal
        virtual
        returns (uint256)
    {
        if (_bondFee != 0 ) {
            return (collateralAmount * 10000) / _bondFee;
        }
        return 0;
    }
    ...
    function setBondFee(uint256 bondFee) public virtual {
        _bondFee = bondFee;
    }
}
...
contract PositionBondLending is
    ...
    ReentrancyGuard
{
    uint64 private duration; // in days
    PositionBondFunction private positionBondFunction;
        ...
        )
    {
        ...
        BondStatus.active(
            ...
            bondInformation.active + bondInformation.duration * 1 days
        );
    ...
    function removeCollateral(uint256[] calldata amountRemoved)
        public
        onlyIssuer
        nonReentrant
    {
    ...
    function addCollateral(
        ...
        uint256 amountAdded
    ) public payable override(UnderlyingAssetLending) onlyIssuer nonReentrant {
        require(
        ...
        super.addCollateral(amountTransferAdded, amountAdded);
    ...
    function claimRemainderUnderlyingAsset() external onlyActive onlyIssuer {
        UnderlyingAssetLending._transferRemainderUnderlyingAsset(
            bondSupply,
            totalSupply()
        );
    }
    ...
    function liquidate() external onlyPositionAdmin {
        if (isLiquidate()) {
            positionBondFunction.liquidated(
    ...
    {
        //        UnderlyingAssetLending._transferUnderlyingAsset();
    }
        ...
        virtual
    {
        if (totalSupply() == bondSupply) {
            _updateTimePhase(
                ...
                uint64(block.timestamp + duration * 1 days)
            );
        }
    }
...
contract PositionBondFunction is
    ...
    OwnableUpgradeable
{
    IPosiNFTFactory _posiNFTFactory =
    ...
    function verifyRequire(
        ...
        if (_getLengthString(bondName) > 30) return false;
        if (_getLengthString(description) > 300) return false;

        if (underlyingAssetType == 0) {
            return
                verifyRequireToken(collateralAmount, collateralAmountTransfer);
        }
        if (underlyingAssetType == 1) {
            return verifyRequireNFT(nftIds, collateralAmount);
        }
        if (underlyingAssetType == 2) {
    ...
    function _coverRFI(
        ...
        return (collateralAmountTransfer * 9_900) / 10_000 >= collateralAmount;
    }
}
...
contract TestPositionBondLending is PositionBondLending {
...
abstract contract BondStatus {
    ...
    function _updateTimePhase(uint64 _active, uint64 _maturity)
        internal
        virtual
    {
...
abstract contract UnderlyingAssetLending {
    ...
    IERC20 private _underlyingAssetToken;
    ...
    function addCollateral(
        ...
        if (underlyingAssetType == 0) {
            ...
            BondTransfer._transferInPosiNFT(
                _underlyingAsset,
                amountAdded,
                msg.sender
            );
    ...
    function onERC721Received(
        ...
        //        require(msg.sender == address(_underlyingAssetPosiNFT), "Only PosiNFT");
        return
            bytes4(
...
abstract contract PositionAdmin {
    ...
    function initAdmin(address newAdmin) external virtual {
        require(_positionAdmin == address(0), "admin already set");
        _positionAdmin = newAdmin;
    }
}
...
abstract contract LoanRatio {
    ...
    function initChainLinkPriceFeed(address chainLinkPriceFeed_) external {
        require(
        ...
        returns (bool)
    {
        ...
        return
            getLoanRatio(amountUnderlyingAsset, amountFaceAsset) >=
            liquidateRate;
    }
    ...
    function isNotReachMaxLoanRatio(
        ...
        uint256 loanRatio = getLoanRatio(amountUnderlyingAsset, amountFaceAsset);
