...
contract Bridge is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        fee = _fee;
        BridgeVault _vault = new BridgeVault(_governance, address(this));
        vault = _vault;
        master = _master;
        emit VaultCreated(address(_vault), block.timestamp);
    }

    event VaultCreated(address _bridge, uint256 _when);
    ...
    function updateBridgeFee(uint256 _fee) external {
        require(
            IMasterBridge(master).getTreasury() == msg.sender,
        ...
        nonReentrant
    {
        ...
        vault.bridgeWithdrawal(token, msg.sender, _data[0]);
        emit Withdrawal(
            msg.sender,
            _data[0],
            _data[2],
    ...
    function transferFee(uint256 _fee) internal {
        TransferHelper.safeTransferFrom(
            token,
            msg.sender,
            IMasterBridge(master).getTreasury(),
            _fee
        );
    }
    ...
    function getTestTokens() external {
        ITBridge(token).getTestTokens(msg.sender);
    }
}
...
interface ITBridge {
    function getTestTokens(address to) external;
}

// SPDX-License-Identifier: BSD-3-Clause
pragma solidity 0.8.12;

interface IMasterBridge {
...
contract BridgeVault is Context, AccessControl {
