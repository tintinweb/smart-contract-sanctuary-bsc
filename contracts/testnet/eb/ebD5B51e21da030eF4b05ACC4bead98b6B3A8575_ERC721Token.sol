...
contract ERC721Token is ERC721URIStorage, Ownable {
    ...
    mapping(address => uint256[]) private addedFeeDetails;
    ...
    function addImageGenFee() public payable {
        require(msg.value >= imageGenerationFee, "Sending low fee amount");
        payable(owner()).transfer(msg.value);
        imageGenFeeDetails[feeId] = feeDetails ({
            amount : msg.value,
            time : block.timestamp
        });
        addedFeeDetails[msg.sender].push(feeId++);
        emit ImageGenerationFee(msg.sender, msg.value, block.timestamp);
    }

    function getUserDetails(address _usrAddress) public view returns(uint256[] memory){
        return addedFeeDetails[_usrAddress];
    }
}