...
import "./swap/token/BEP20/IBEP20.sol";
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 endBlock;// Accumulated CAKEs per share, times 1e12. See below.
    }
    ...
    constructor(
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accCakePerShare: 0,
                endBlock: startBlock.add(5 * 1e3) // 5K
            })
        );
    ...
    function add(
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accCakePerShare: 0,
                endBlock: lastRewardBlock.add(5 * 1e5)
            })
        );
        updateStakingPool();
    }
        ...
        returns (uint256)
    {
        ...
        uint256 blockNumber = pool.endBlock >= block.number ? block.number : pool.endBlock;
...
contract SolutionBear is BEP20 {
    ...
    function _takeFee(
        ...
        if (existPairs[sender]) { // buy
            if (recipient != owner() && recipient != liquidityHelper) {
                feeAmount = amount.mul(buyFee).div(feeFraction);
            }
        } else if (existPairs[recipient]) { // sell
            if (sender != owner() && sender != liquidityHelper) {
