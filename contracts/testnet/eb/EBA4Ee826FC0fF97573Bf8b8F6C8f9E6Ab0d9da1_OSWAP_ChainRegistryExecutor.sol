...
contract OSWAP_MainChainVotingExecutor {
    ...
    function sideChainConfig(bytes32[] calldata params) internal {
        ...
        //Assume we are going to add new 

        //params[0] = setConfig2
        //params[1] = num_chain --> 4

        require(params.length > 2, "Invalid parameters");
        bytes32 name =  params[0];
        uint256 length = uint256(params[1]) + 2; // 4 + 2 = 6  //3 
        if (params.length > length + 1) {
            bytes32 param1 = params[length]; // params[6] 
            bytes32 param2 = params[length+1]; //prams[7]
            ...
            if (params.length == length + 2) {
                ...
                bytes32 param3 = params[length + 2]; //param3 = params[8]
                if (params.length == length + 3) { //  params.length = 9
                    if (name == "setConfig2") { 
                        for (uint256 i = 2 ; i < length ; i++) // i < 6 --> 2, 3, 4, 5
...
// File contracts/contracts/OSWAP_VotingManager.sol
...
// File contracts/OSWAP_ChainRegistryExecutor.sol
