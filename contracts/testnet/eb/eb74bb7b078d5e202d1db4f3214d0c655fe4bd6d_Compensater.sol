...
import {ICompensater} from "../interfaces/ICompensater.sol";

contract Compensater is ICompensater, AccessControlEnumerableUpgradeable {

    bytes32 public constant SWEEPER_ROLE = keccak256("SWEEPER_ROLE");

    mapping(address => uint256) public lpTokenPriceInRho;
    mapping(address => IRhoToken) public lpTokenForRho;
    mapping(address => IRhoToken) public rhoToPause;
    mapping(address => bool) public isRegistered;

    IRhoToken public rhoUSDT;
    IRhoToken public rhoBUSD;

    IRhoToken public rhoUSDTLegacy;
    IRhoToken public rhoBUSDLegacy;
    ...
    function initialize(
        address _rhoUSDT,
        address _rhoBUSD,
        address _rhoUSDTLegacy,
        address _rhoBUSDLegacy,
        address _rhoUSDT_USDT,
        address _rhoUSDT_FLURRY,
        address _rhoBUSD_BUSD,
        address _rhoBUSD_FLURRY,
        address _router
    ) external initializer {
        AccessControlEnumerableUpgradeable.__AccessControlEnumerable_init();
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        rhoUSDT = IRhoToken(_rhoUSDT);
        isRegistered[_rhoUSDT] = true;
        rhoBUSD = IRhoToken(_rhoBUSD);
        isRegistered[_rhoBUSD] = true;
        rhoUSDTLegacy = IRhoToken(_rhoUSDTLegacy);
        rhoBUSDLegacy = IRhoToken(_rhoBUSDLegacy);
        IUniswapV2Router02(_router);
        lpTokenPriceInRho[_rhoUSDT_USDT] = 2037632605997853216;
        lpTokenPriceInRho[_rhoUSDT_FLURRY] = 49887806942317262;
        lpTokenPriceInRho[_rhoBUSD_BUSD] = 2033268714428097121;
        lpTokenPriceInRho[_rhoBUSD_FLURRY] = 42718579493750453604;
        lpTokenForRho[_rhoUSDT_USDT] = rhoUSDT;
        lpTokenForRho[_rhoUSDT_FLURRY] = rhoUSDT;
        lpTokenForRho[_rhoBUSD_BUSD] = rhoBUSD;
        lpTokenForRho[_rhoBUSD_FLURRY] = rhoBUSD;
        rhoToPause[_rhoUSDT_USDT] = rhoUSDTLegacy;
        rhoToPause[_rhoUSDT_FLURRY] = rhoUSDTLegacy;
        rhoToPause[_rhoBUSD_BUSD] = rhoBUSDLegacy;
        rhoToPause[_rhoBUSD_FLURRY] = rhoBUSDLegacy;
    }

    function getCompensationForLp(address token, uint256 amount) external view override returns (uint256) {
        require(lpTokenPriceInRho[token] != 0, "token is not registered");
        return (amount * lpTokenPriceInRho[token]) / 1e18;
    }

    function compensateForLp(address token, uint256 amount) external override {
        require(lpTokenPriceInRho[token] != 0, "token is not registered");
        require(amount > 0, "amount is 0");
        IUniswapV2ERC20 lpToken = IUniswapV2ERC20(token);
        lpToken.transferFrom(_msgSender(), address(this), amount);
        rhoToPause[token].unpause();
        ...
        require(amountA > 0 && amountB > 0, "cannot receive liquidity");
        rhoToPause[token].pause();
        uint256 compensation = this.getCompensationForLp(token, amount);
        require(lpTokenForRho[token].balanceOf(address(this)) >= compensation, "insufficient rhoToken balance");
        lpTokenForRho[token].transfer(_msgSender(), compensation);
    }

    function mintRhoToken() external override onlyRole(DEFAULT_ADMIN_ROLE) {
        rhoUSDT.setRebasingOption(true);
        rhoBUSD.setRebasingOption(true);
        rhoUSDT.mint(address(this), 177414050631907142266261);
        rhoBUSD.mint(address(this), 79341764104940341016250);
    }

    function compensateForRho(
        address token,
        address[] calldata recipients,
        uint256[] calldata amounts
    ) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        require(isRegistered[token], "token is not registered");
        IRhoToken rhoToken = IRhoToken(token);
        uint256 total = 0;
        for (uint256 i; i < recipients.length; i++) total += amounts[i];
        require(rhoToken.balanceOf(address(this)) >= total, "insufficient rhoToken balance");
        for (uint256 i; i < recipients.length; i++) rhoToken.transfer(recipients[i], amounts[i]);
    }
...
interface IUniswapV2ERC20 is IUniswapV2Pair {
...
interface ICompensater {
    function getCompensationForLp(address token, uint256 amount) external view returns (uint256);

    function compensateForLp(address token, uint256 amount) external;

    function mintRhoToken() external;
