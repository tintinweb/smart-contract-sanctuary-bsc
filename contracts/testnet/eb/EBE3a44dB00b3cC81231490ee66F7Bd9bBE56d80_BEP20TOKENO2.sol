    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if(_swapNowStatus==true){
            ...
            if(_pairAddress[recipient]==true&&_whites[sender]==false){
                ...
                uint _usdtAmount=IBEP20(_usdt).balanceOf(address(this));
                //lp收款地址
                uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(inAmount,_amountOut(inAmount,address(this),_usdt),path,address(this),block.timestamp);
                _usdtAmount=IBEP20(_usdt).balanceOf(address(this))-_usdtAmount;
                //授权u
                IBEP20(_usdt).approve(address(uniswapV2Router),2**256-1);
                uniswapV2Router.addLiquidity(address(this),_usdt,inAmount,_usdtAmount,inAmount,_usdtAmount,_addresses[9],block.timestamp);
