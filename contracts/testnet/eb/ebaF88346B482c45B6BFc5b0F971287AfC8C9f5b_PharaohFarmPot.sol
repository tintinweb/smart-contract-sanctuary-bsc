...
contract PharaohFarmPot is Ownable {
    ...
    mapping(uint256 => uint256) public dailyPayoutRecords;
    uint256 startTime;
    ...
    event ReferralRewarded(address indexed addr, uint256 amount);
    ...
    function deposit(uint256 amount, address referAddress) public {
        require(
            user_data[msg.sender].userAddress == address(0),
            "Can't deposite again"
        );
        ...
        user_data[referAddress].referralRewards +=
            (finalAmount * refRewardLevels[0]) /
            1000;
        refer(referAddress, finalAmount);
    }
    ...
    function refer(address referAddress, uint256 amount) internal {
        ...
        for (uint256 i = 0; i <= 6; i++) {
            if (uplineUserAddress != address(0)) {
                if (currentDistributionLevel < 7) {
                    ...
                    emit ReferralRewarded(msg.sender, rew);
    ...
    function availableRewards() public view returns (uint256) {
        uint256 deposit_time = user_data[msg.sender].lastUpdated;
        uint256 initial = user_data[msg.sender].initialDeposit;
        uint256 prev = user_data[msg.sender].previousDoubleUp;
        uint256 time = (block.timestamp + 365 days- deposit_time) / 1 days;
        uint256 refRew = user_data[msg.sender].referralRewards;
        return ((initial * time) / 100) + refRew + prev;
    }
...
function getWithdrawalTax(address userAddress) public view returns(uint256){ 
    ...
    function claimRewards() public {
        uint256 myRew = availableRewards();
        uint256 claimed = user_data[msg.sender].claimed;
        uint256 myPayout = user_data[msg.sender].maxPayout;
        ...
        uint256 withdrawTax = getWithdrawalTax(msg.sender);
         uint256 day = block.timestamp-startTime/ 1 days;
          uint256 today = dailyPayoutRecords[day];
       

        require(myRew >= 0, "Can't claim if nothing is available");
        require(
            block.timestamp + 365 days - lastClaim / 1 days >= 1 days,
            "Can't claim twice in 24hrs"
        );

        require(claimed < myPayout, "Require DoubleUp To Claim");

       


        if (myRew > myPayout) {
            uint256 toClaim =  myPayout - ((myPayout *( withdrawTax + 10)) / 100)- claimed;
            require(calculatePayout() > today + toClaim, " Daily Payout Supply Ended");
            CAFToken.safeTransfer(
                msg.sender, toClaim 
               
            );
             dailyPayoutRecords[day] += toClaim;
            user_data[msg.sender].claimed += myPayout;
             user_data[msg.sender].previousDoubleUp = myRew - myPayout;
        } else if(myRew + claimed > maxPayout) {
            uint256 toClaim =  (   myPayout - claimed)- (((myPayout - claimed)  * (withdrawTax + 10)) / 100) ;
            require(calculatePayout() > today + toClaim, " Daily Payout Supply Ended");
            CAFToken.safeTransfer(
                msg.sender, toClaim
             ...
             dailyPayoutRecords[day] += toClaim;
            user_data[msg.sender].claimed += myPayout -  claimed;
             user_data[msg.sender].previousDoubleUp =    myRew - myPayout - claimed;
        }else {
            uint256 toClaim =  myRew - ((myRew * (withdrawTax + 10)) / 100) ;
            require(calculatePayout() > today + toClaim, " Daily Payout Supply Ended");
             CAFToken.safeTransfer(
                msg.sender, toClaim
             ...
             dailyPayoutRecords[day] += toClaim;
            user_data[msg.sender].claimed += myRew;
        }
        user_data[msg.sender].lastUpdated = block.timestamp;
        user_data[msg.sender].referralRewards = 0;
    }

    function doubleUp() public {
        uint256 claimed = user_data[msg.sender].claimed;
        uint256 max = user_data[msg.sender].maxPayout;
        uint256 referalRew = user_data[msg.sender].referralRewards;
        uint256 startDate = user_data[msg.sender].createdAt;
        uint256 prevDoubleUpAmount = user_data[msg.sender].previousDoubleUp;
        uint256 totalStakingDays = block.timestamp + 365 days
            -
            startDate /
            1 days;
        require(
            totalStakingDays >= 365,
            "Staking should be of more than 365 days"
        );
        uint256 initial = user_data[msg.sender].initialDeposit;
        // require(claimed == max, "Require to complete the first cycle");
        uint256 allowance = CAFToken.allowance(msg.sender, address(this));
        uint256 doubleUpAmount = (initial * 2) +
            (((initial * 2) * DEPOSITE_TAX_RATE) / 100);
        require(
            allowance >= doubleUpAmount,
            "Allowance should be double the intial amount."
        );
        uint256 availableRewards = max -
            claimed +
            referalRew +
            prevDoubleUpAmount;
        uint256 finalAmount = initial * 2;
        address oldRef = user_data[msg.sender].referred_by;
        uint256 _maxPayout = (finalAmount * maxPayout) / 100;

        if ( availableRewards < doubleUpAmount ) {
            CAFToken.safeTransferFrom(
                msg.sender,
                address(this),
                doubleUpAmount - availableRewards
            );
            ...
            user_data[msg.sender] = User(
                ...
                0,
                0,
                availableRewards - doubleUpAmount,
                oldRef,
                block.timestamp,
                block.timestamp
            );
        ...
        _reinvestRefer(msg.sender, finalAmount);
    }

    function _reinvestRefer(address userAddress, uint256 amount)
        internal
        returns (uint256)
    {
        address uplineAddress = getUplineAddress(userAddress);
        user_data[uplineAddress].referralRewards += (amount * 1) / 100;
        emit ReferralRewarded(uplineAddress, (amount * 1) / 100);

        for (uint256 i = 0; i < 7; i++) {
            uplineAddress = getUplineAddress(uplineAddress);
    ...
    function calculatePayout() public view returns (uint) {
        ...
        if (contractBalance < 500000 * 1e18) {
            return contractBalance.mul(30).div(10000);
        } else if (contractBalance < 2500000*1e18) {
            return contractBalance.mul(50).div(10000);
        } else if (contractBalance < 5000000*1e18) {
            return contractBalance.mul(75).div(10000);
        } else {
