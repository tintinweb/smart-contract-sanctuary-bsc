    ...
    ReentrancyGuard
{
    ...
    uint256 public burnSwapClaimBalance = 0;

    uint256 incineratorBalance = 0;
    uint256 incineratorEthBalance = 0;
    ...
    modifier isNotZeroAddress(address sender, address recipient) {
        require(
            sender != address(0) && sender != burnAddress,
            "TOKEN20: transfer from the zero address"
        );
        require(
            recipient != address(0) && recipient != burnAddress,
    ...
    constructor(
        ...
        _projectBlacklist[burnAddress] = false;

        balances[address(0)] = 0;
        balances[burnAddress] = 0;
    ...
    function pendingBurn() external view returns (uint256) {
        return pendingTokensToBurnFromLP + queuedDailyBurn;
    }
        ...
        returns (bool)
    {
        ...
        balances[burnAddress] = balances[burnAddress] + amountToBurn;
    ...
    function _controlDailyBurn() private {
        ...
        if (dailyBurn[_dayCounter.current()].startTimestamp != _timestamp) {
            ...
            dailyBurn[_dayCounter.current()] = DAILY_BURN({
                ...
                tokenStartDay: _TotalSupply - balances[burnAddress],
    ...
    function dailyBurns() public view returns (uint256 _tokensToBurn) {
        if (dailyBurn[_dayCounter.current()].startTimestamp > 0) {
            ...
            _tokensToBurn =
                ...
                (_toBeBurnedPerTick * tickSinceStart) -
                dailyBurn[_dayCounter.current()].amountBurned : 0;
    ...
    function _getLPandUnpair(uint256 getTokensToRemove, uint256 _dailyBurnCount)
        internal
        returns (
            bool success,
        ...
        )
    {
        ...
        ) = calculateLPtoUnpair(getTokensToRemove + _dailyBurnCount);
        ...
        if (tokensToExtract > 0 && lpToUnpair >= (defaultMultiplier)) {
            ...
            (success, ___amountToken, ___txETHAmount) = removeLiquidity(
                lpToUnpair,
                amountControlled
            );
            ...
            return (success, ___amountToken, ___txETHAmount);
        }
        ...
        )
    {
        ...
        approveContract(primePair, address(dexRouter), type(uint256).max);
    ...
    function sendFees() private onlyOwner {
        ...
        if (
            ...
            balances[address(this)] >= incineratorBalance &&
            ...
            uint256 priorIncineratorEthBalance = address(this).balance;
            try
            ...
            {
                incineratorBalance = 0;
                incineratorEthBalance = address(this).balance - priorIncineratorEthBalance;
            } catch {
                _actualTransfer(
                    address(this),
                    contractAddresses.marketingWallet,
                    incineratorBalance / 2
                );
                _actualTransfer(
                    address(this),
    ...
    function extraTransferActions(
        ...
        if (_tokensToBurn > 0) {
            _burnToken(_sender, _tokensToBurn);
        }

        if (_secondTokenBurnTally > 0) {
            pendingTokensToBurnFromLP += _secondTokenBurnTally;
        }
        ...
        if (_takeFee && balances[primePair] > 0) {
            if (
                ...
                if (pendingTokensToBurnFromLP > 0 || queuedDailyBurn > 0) {
                    _unpairBakeAndBurn(
    ...
    function _unpairBakeAndBurn(
        ...
        if (tokensBeingTransacted > 0 || _dailyBurnPortion > 0) {
            (
                bool goBurn,
                uint256 _tokenAmount_,
                uint256 nativeETHReceived
            ) = _getLPandUnpair(tokensBeingTransacted, _dailyBurnPortion);
            ...
            if (goBurn && _tokenAmount_ > 0 && nativeETHReceived > 0) {
                ...
                if (success) {
                    dailyBurn[_dayCounter.current()].amountBurned += _tokenAmount_;
                    queuedDailyBurn = 0;
                    pendingTokensToBurnFromLP = 0;
                    // dailyBurn[_dayCounter.current()].amountBurned = _dailyBurnPortion;
    ...
    function unpairBakeAndBurn(
        ...
        if (tokensBeingTransacted > 0) {
            _unpairBakeAndBurn(tokensBeingTransacted, dailyBurnPortion);
        }
    }
    ...
    function withDrawFees() public onlyOwner nonReentrant {
        require(incineratorEthBalance > 0, "Nothing to Withdraw");
        uint256 etherToTransfer = incineratorEthBalance;        
