...
contract CampaignFactory is Ownable{
    ...
    mapping(uint256 => bool) public isExisting;
    ...
    function createCampaign(
        ...
        // [3] = hardCap, [4] = campaignId, [5] = priceDiscount, [6] = priceDiscountEndDate
        uint256[7] memory _settings,
        string memory _mintType,        
        address _mintToken,
        address _implementation,
        bool enableCampaignWhitelist,
        JumpStartLib.MintNft[] memory _nftTiers
    ) external returns (address campaign) {
        if (isExisting[_settings[3]] == false) {
            (uint256 actualCreationFee, uint256 actualCampaignFee) = _manager
                .calculateFees(_settings[0], _settings[1], _mintToken);

            require(
                _manager._pay(msg.sender, actualCreationFee),
                "Must pay fee"
            );

            address campaignClone = Clones.clone(_implementation);
            _manager.pushCampaign(campaignClone);

            Campaign(campaignClone).initialize(
                [msg.sender, _mintToken],
                _settings,
                enableCampaignWhitelist,
                _mintType,
                _nftTiers,
                actualCampaignFee
            );

            isExisting[_settings[3]] = true;
...
contract Campaign is Initializable,OwnableUpgradeable, Whitelist {    
    ...
    mapping(address => uint256) public campaignMapping;
    ...
    function initialize(
        ...
        campaignOwner = _addresses[0];
        campaignId = _settings[4];
        whitelistEnabled = enableCampaignWhitelist;
        ...
        for (uint256 i = 0; i < _nftTiers.length; i++) {
            ERC721JumpStart nft = new ERC721JumpStart(
            ...
            campaignNfts.push(address(nft));
            campaignMapping[address(nft)] = campaignId;
            ...
            if (_nftTiers[i].redeemable) {
                ...
                // ticket.transferOwnership(address(campaignOwner));
                nft.setTicketContract(address(ticket));
            }
            // nft.transferOwnership(address(campaignOwner));
        }        
    }

    function completeCampaign() public onlyOwner {
        require(totalRaised >= softCap, "Campaign not reached soft cap");
        require(endDate >= block.timestamp, "Campaign not ended");

        campaignCompleted = true;
        uint256 balance = IERC20(mintToken).balanceOf(address(this));
        IERC20(mintToken).transfer(campaignOwner, balance);
    }

    function checkWhitelist() public view {
    ...
    function updateUserContributed(address _user, uint256 _amount) public {
        require(campaignMapping[msg.sender] > 0, "Not campaign nft");
        userContributed[_user] += _amount;
    }

    function claimRefund() public {
        require(refundActive, "Refund not active");
        require(block.timestamp >= endDate, "Refund not available");
        require(userContributed[msg.sender] > 0, "No contribution refund");
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize() external virtual initializer {
        ...
        fees[3] = 5;
        createFee = 3 * 10**17;
        feeCollector = payable(0xbC31e237494844Ce74Be95eD1768B4eE7107AF52);
        sidekickToken = address(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7); //testnet busd
    }
        ...
        returns (uint256 actualCreationFee, uint256 actualCampaignFee)
    {
        if (softCap == hardCap) {
            ...
            actualCampaignFee = campaignFee
                .mul(SafeMathUpgradeable.sub(100, fees[1]))
                .div(100);
        } else if (softCap.div(hardCap).mul(100) > 75) {
            // 75% hc discount
            actualCreationFee = createFee
            ...
            actualCampaignFee = campaignFee
                .mul(SafeMathUpgradeable.sub(100, fees[2]))
                .div(100);
        } else if (softCap.div(hardCap).mul(100) > 50) {
            actualCreationFee = createFee
        ...
        if (mintToken == sidekickToken) {
            actualCreationFee = actualCreationFee
                .mul(SafeMathUpgradeable.sub(100, fees[0]))
                .div(100);
            actualCampaignFee = actualCampaignFee
                .mul(SafeMathUpgradeable.sub(100, fees[0]))
                .div(100);
        }
    ...
    function _pay(address payee, uint256 fee) public virtual returns (bool) {
        IERC20Upgradeable token = IERC20Upgradeable(sidekickToken);
        token.transferFrom(payee, feeCollector, fee);
        return true;
    }
...
contract ERC721JumpStart is ERC721A, OwnableUpgradeable, Whitelist {
    ...
    constructor(
        // [0] = tokenName, [1] = tokenSymbol, [2] = tokenUri [3] = mintType
        string[4] memory _stringSettings,
        // [0] = quantity, [1] = mintPrice, [2] = priceDiscount, 
        //[3] = priceDiscountEndDate, [4] = mintPerAddress, 
        //[5] = mintPerTransaction, [6] = ticketsPerMint, [7] = campaignFee, [8] = nftUID
        uint256[9] memory _intSettings,                        
        address _mintToken,        
        bool _enableWhitelist,
        bool _redeemable,        
        address _campaignAddress
    ) ERC721A (_stringSettings[0], _stringSettings[1]) {
        uri = _stringSettings[2];
        ...
        if (
            ...
            paymentToken = _mintToken;
        }
        campaignContract = Campaign(address(_campaignAddress));
    }
    ...
    function mintChainPay(address _minter, uint256 _numberOfNfts) private {
        require(
            IERC20(paymentToken).balanceOf(_minter) >=
                mintPrice * _numberOfNfts,
            "Insufficient balance"
        );
        require(_pay(_minter, mintPrice * _numberOfNfts), "Payment failed");
    ...
    //mint any number of nfts, up to the maxMintPerTransaction
    function mint(address _minter, uint256 _numberOfNfts) public {
        require(
            campaignContract.startDate() >= block.timestamp,
            "Campaign has not started"
        );
        require(
            campaignContract.endDate() <= block.timestamp,
            "Campaign has ended"
        );
        ...
        require(_totalMinted() + _numberOfNfts <= mintLimit, "Exceeds mintLimit");
        require(
            mintedPerWallet[_minter] + _numberOfNfts < maxMintPerWallet,
            "Mint goes over limit"
        );

        if (whitelistEnabled) {
            require(
                whitelist[_minter] == true,
                "Not whitelisted"
            );
        }
        ...
        if (
            ...
            mintPayable(_minter, _numberOfNfts);
        } else {
            // use mint function that takes chain tokens
            mintChainPay(_minter, _numberOfNfts);
        }

        // mint tickets
        if (redeemable) {
            mintTickets(_minter, _numberOfNfts * ticketsPerMint);
        }
    }
    ...
    function _pay(address payee, uint256 fee) internal virtual returns (bool) {
        ...
        if (campaignContract.endDate() <= block.timestamp) {
            // transfer to campaign
            token.transferFrom(payee, campaign, fee);
        } else {
            // transfer to admin
            token.transferFrom(payee, admin, fee);
        }
        return true;
    }
}
...
contract JumpStartTicket is ERC721A, OwnableUpgradeable {
    address public admin;
    uint256 public mintLimit;
