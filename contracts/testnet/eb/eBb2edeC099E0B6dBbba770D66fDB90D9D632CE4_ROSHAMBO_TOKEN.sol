...
contract ROSHAMBO_TOKEN is Context, IBEP20, Ownable {
    ...
    uint256 public maxBuyAmount = 1e25; // 0.1% of total supply
    ...
    function addPair(address pairAddress) public onlyOwner {
        require(!isPair[pairAddress], "ROS: Account is already added.");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        require(!isBlacklisted[sender] && !isBlacklisted[recipient], "ROS: You are blacklisted..");
                
        uint256 transferAmount = amount;
        uint256 rate = _getReflectionRate();

        if (!isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && isPair[sender]) {
            require(amount <= maxBuyAmount, "ROS: Buy amount exceeds then maximum buy amount");
        ...
        if (!isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && isPair[recipient]) {
            require(amount <= maxSellAmount, "ROS: Sell amount exceeds then maximum sell amount");
            transferAmount = collectFeeOnSell(sender,amount,rate);
        }
        
        if (!isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && !isPair[sender] && !isPair[recipient]) {
        ...
        if (!isPair[sender] && !isPair[recipient] && swapAndLiquifyEnabled && lastSwapTime >= swapInterval) {
    ...
    function excludedFromFee(address account) public onlyOwner {
        require(!isExcludedFromFee[account], "ROS: Account is already excluded");
        isExcludedFromFee[account] = true;
    }
    
    function includedForFee(address account) public onlyOwner {
        require(isExcludedFromFee[account], "ROS: Account is already included");
    ...
    function updateFees(uint256 tax, uint256 liquidity, uint256 marketing) public onlyOwner {
        require(tax <= 10 && liquidity <= 10 && marketing <= 10, "ROS: Enter a valid number..");
        _taxFee = tax;
        _liquidityFee = liquidity;
        _marketingFee = marketing;
    }
    
    function updateBuyFees(uint256 tax, uint256 liquidity, uint256 marketing) public onlyOwner {
        require(tax <= 10 && liquidity <= 10 && marketing <= 10, "ROS: Enter a valid number..");
        _buyTaxFee = tax;
    ...
    function updateSellFees(uint256 tax, uint256 liquidity, uint256 marketing) public onlyOwner {
        require(tax <= 10 && liquidity <= 10 && marketing <= 10, "ROS: Enter a valid number..");
        _sellTaxFee = tax;
