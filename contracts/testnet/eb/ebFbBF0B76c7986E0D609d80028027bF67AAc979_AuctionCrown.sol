...
contract AuctionCrown is Ownable {
     ...
     */
    constructor(address token_, address daoTreasuryAddress_, address winDaoAddress_) {
        wdaToken = token_;
        daoTreasuryAddress = daoTreasuryAddress_;
        winDaoAddress = winDaoAddress_;
    ...
    function openAuction() external onlyAdmin{
        ...
        auctions[_nftId].highestBidder = address(0);
    ...
    function closeAuction() external onlyAdmin {
        ...
        if (auctions[lastNftId].highestBid == 0) {
            ...
            (bool sent, ) = payable(winDaoAddress).call{
                    value: auctions[lastNftId].withBnb
                }("");
            require(sent, "Transfer BNB failed");
            // Transfer WDA to the DAO Treasury
            bool success = IERC20(wdaToken).transfer(daoTreasuryAddress, auctions[lastNftId].highestBid);
            require(success,"Transfer WDA failed");
    ...
    function augmentBnb() internal view returns (uint256 _bnb) {
        ...
        return ((_time * 2) * 10**18 / 10) / 10000 ; //Test only /100
    }
