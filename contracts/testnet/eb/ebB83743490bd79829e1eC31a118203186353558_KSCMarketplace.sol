    ...
    PermissionExtension
{
    ...
    event ListNFT(address indexed from, uint256 tokenId, uint256 price, uint256 expireTime, bool isBUSDPrice);
    ...
    uint256 private _busdTax; // percentage

    mapping(uint256 => ListDetail) _listDetail;
    ...
    modifier isValidToBuy(uint256 tokenId, uint256 price) {
        require(
            (_listDetail[tokenId].isActive && !_listDetail[tokenId].isSold),
            "Nft is not active or sold"
        );
        require(
            (_listDetail[tokenId].expireTime != 0 &&
                _listDetail[tokenId].expireTime <= block.timestamp),
            "Expired time"
        );
        require(
        ...
        if (!_listDetail[tokenId].isBUSDPrice)
            require(
            ...
            require(
                _busdToken.balanceOf(msg.sender) >= price,
                "Insufficient account balance"
            );
    ...
    modifier isValidChange(uint256 itemId) {
        require(
            _nft.ownerOf(itemId) == address(this),
            "This NFT doesn't exist on marketplace"
        );
        require(
            _listDetail[itemId].author == msg.sender,
            "Only owner can unlist this NFT"
        );

        require(
            (_listDetail[itemId].isActive && !_listDetail[itemId].isSold),
            "Nft is not active or sold"
        );
        _;
    }

    function setTax(uint256 tax, uint256 busdTax) public onlyOperator {
        _tax = tax;
        _busdTax = busdTax;
    }
    ...
    function listNft(
        ...
        uint256 expireTimeStamp = expireTime == 0 ? 0 : block.timestamp + expireTime * 1 days;
        _listDetail[tokenId] = ListDetail(
            payable(msg.sender),
            price,
            tokenId,
            expireTimeStamp,
        ...
        emit ListNFT(msg.sender, tokenId, price, expireTimeStamp, isBUSDPrice);
    }

    function updateListingNftPrice(uint256 tokenId, uint256 price)
        public
        isValidChange(tokenId)
    {
        _listDetail[tokenId].price = price;
        ...
        nonReentrant
    {
        ...
        _listDetail[tokenId].isActive = false;
        emit UnlistNFT(msg.sender, tokenId);
    }

    function buyNft(uint256 tokenId, uint256 price)
        public
        isValidToBuy(tokenId, price)
        nonReentrant
    {
        ...
        if (!_listDetail[tokenId].isBUSDPrice)
            _token.transfer(
                _listDetail[tokenId].author,
                (price * (100 - _tax)) / 100
            );
        else
            _busdToken.transfer(
                _listDetail[tokenId].author,
                (price * (100 - _busdTax)) / 100
            );

        _nft.safeTransferFrom(address(this), msg.sender, tokenId);

        _listDetail[tokenId].isSold = true;
        _listDetail[tokenId].isActive = false;

        emit BuyNFT(msg.sender, _listDetail[tokenId].author, tokenId, price);
    }
