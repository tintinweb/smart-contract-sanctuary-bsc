...
contract CryptoBlades is Initializable, AccessControlUpgradeable {
    ...
    int128 public constant PAYMENT_USING_STAKED_DKC_COST_AFTER_DISCOUNT =
    ...
    mapping(address => uint256) tokenRewards; // user adress : dkc wei
    ...
    struct MintPaymentDkcDeposited {
        uint256 dkcDepositedFromWallet;
        uint256 dkcDepositedFromRewards;
        uint256 dkcDepositedFromIgo;

        uint256 dkcRefundableFromWallet;
        uint256 dkcRefundableFromRewards;
        uint256 dkcRefundableFromIgo;

        uint256 refundClaimableTimestamp;
    }

    uint256 public totalMintPaymentDkcRefundable;
    mapping(address => MintPaymentDkcDeposited) mintPaymentDkcDepositeds;
    ...
    function getDkcToSubtractSingle(uint256 _needed, uint256 _available)
        public
        pure
    ...
    function getDkcToSubtract(uint256 _inGameOnlyFunds, uint256 _tokenRewards, uint256 _dkcNeeded)
        public
        pure
        returns (uint256 fromInGameOnlyFunds, uint256 fromTokenRewards, uint256 fromUserWallet) {

        if(_dkcNeeded <= _inGameOnlyFunds) {
            return (_dkcNeeded, 0, 0);
        }

        _dkcNeeded -= _inGameOnlyFunds;

        if(_dkcNeeded <= _tokenRewards) {
            return (_inGameOnlyFunds, _dkcNeeded, 0);
        }

        _dkcNeeded -= _tokenRewards;

        return (_inGameOnlyFunds, _tokenRewards, _dkcNeeded);
    }

    function getDkcNeededFromUserWallet(address playerAddress, uint256 dkcNeeded)
        public
        view
        returns (uint256 dkcNeededFromUserWallet) {

        (,, dkcNeededFromUserWallet) = getDkcToSubtract(
    ...
    /*function mintPaymentDkcRefundable(address _minter) external view
        returns (uint256 _refundInGameOnlyFunds, uint256 _refundTokenRewards, uint256 _refundUserWallet) {

        return (
            mintPaymentDkcDepositeds[_minter].dkcRefundableFromIgo,
            mintPaymentDkcDepositeds[_minter].dkcRefundableFromRewards,
            mintPaymentDkcDepositeds[_minter].dkcRefundableFromWallet
        );
    }

    function mintPaymentSecondsUntilDkcRefundClaimable(address _minter) external view
        returns (uint256) {

        (bool success, uint256 result) =
            mintPaymentDkcDepositeds[_minter].refundClaimableTimestamp.trySub(block.timestamp);
    ...
    function mintPaymentClaimRefund() external {
        ...
        require(mintPaymentDkcDepositeds[msg.sender].refundClaimableTimestamp <= block.timestamp);

        uint256 dkcRefundableFromIgo = mintPaymentDkcDepositeds[msg.sender].dkcRefundableFromIgo;
        uint256 dkcRefundableFromRewards = mintPaymentDkcDepositeds[msg.sender].dkcRefundableFromRewards;
        uint256 dkcRefundableFromWallet = mintPaymentDkcDepositeds[msg.sender].dkcRefundableFromWallet;

        require(dkcRefundableFromWallet > 0 || dkcRefundableFromRewards > 0 || dkcRefundableFromIgo > 0);

        mintPaymentDkcDepositeds[msg.sender].dkcRefundableFromIgo = 0;
        mintPaymentDkcDepositeds[msg.sender].dkcRefundableFromRewards = 0;
        mintPaymentDkcDepositeds[msg.sender].dkcRefundableFromWallet = 0;

        totalMintPaymentDkcRefundable = totalMintPaymentDkcRefundable
                .sub(dkcRefundableFromWallet)
                .sub(dkcRefundableFromRewards)
                .sub(dkcRefundableFromIgo);

        totalInGameOnlyFunds = totalInGameOnlyFunds.add(dkcRefundableFromIgo);
        inGameOnlyFunds[msg.sender] = inGameOnlyFunds[msg.sender].add(dkcRefundableFromIgo);

        tokenRewards[msg.sender] = tokenRewards[msg.sender].add(dkcRefundableFromRewards);
        dkcToken.transfer(msg.sender, dkcRefundableFromWallet);
    }
    ...
    function _discardPaymentIfExpired(address _minter) internal {
        ...
        if ((mintPayments[_minter].count > 0) &&
            ...
            uint256 depositedDkcFromWallet = mintPaymentDkcDepositeds[_minter].dkcDepositedFromWallet;
            uint256 depositedDkcFromRewards = mintPaymentDkcDepositeds[_minter].dkcDepositedFromRewards;
            uint256 depositedDkcFromIgo = mintPaymentDkcDepositeds[_minter].dkcDepositedFromIgo;
            mintPaymentDkcDepositeds[_minter].dkcDepositedFromWallet = 0;
            mintPaymentDkcDepositeds[_minter].dkcDepositedFromRewards = 0;
            mintPaymentDkcDepositeds[_minter].dkcDepositedFromIgo = 0;

            mintPaymentDkcDepositeds[_minter].dkcRefundableFromWallet =
                mintPaymentDkcDepositeds[_minter].dkcRefundableFromWallet.add(depositedDkcFromWallet);
            mintPaymentDkcDepositeds[_minter].dkcRefundableFromRewards =
                mintPaymentDkcDepositeds[_minter].dkcRefundableFromRewards.add(depositedDkcFromRewards);
            mintPaymentDkcDepositeds[_minter].dkcRefundableFromIgo =
                mintPaymentDkcDepositeds[_minter].dkcRefundableFromIgo.add(depositedDkcFromIgo);

            totalMintPaymentDkcRefundable = totalMintPaymentDkcRefundable
                .add(depositedDkcFromWallet)
                .add(depositedDkcFromRewards)
                .add(depositedDkcFromIgo);
            mintPaymentDkcDepositeds[_minter].refundClaimableTimestamp = block.timestamp + 3 hours;
    ...
    function payForMint(address nftAddress, uint count) public {
        ...
        mintPayments[msg.sender].blockHash = 0;
        mintPaymentDkcDepositeds[msg.sender].dkcDepositedFromWallet =
            mintPaymentDkcDepositeds[msg.sender].dkcDepositedFromWallet.add(_paidFeeFromUserWallet);
        mintPaymentDkcDepositeds[msg.sender].dkcDepositedFromRewards =
            mintPaymentDkcDepositeds[msg.sender].dkcDepositedFromRewards.add(_paidFeeFromTokenRewards);
        mintPaymentDkcDepositeds[msg.sender].dkcDepositedFromIgo =
            mintPaymentDkcDepositeds[msg.sender].dkcDepositedFromIgo.add(_paidFeeFromInGameOnlyFunds);
    }
    ...
    function mintCharacter() public onlyNonContract oncePerBlock(msg.sender) {
        ...
        require(dkcToken.balanceOf(msg.sender) >= fromUserWallet && promos.getBit(msg.sender, 4) == false);

        uint256 convertedAmount = usdToDkc(mintCharacterFee);
    ...
    function mintWeapon() public onlyNonContract oncePerBlock(msg.sender)  {
        ...
        try weapons.mint(msg.sender, randoms.getRandomSeedUsingHash(msg.sender, hash)) {
            ...
            mintPaymentDkcDepositeds[msg.sender].dkcDepositedFromWallet = 0;
            mintPaymentDkcDepositeds[msg.sender].dkcDepositedFromRewards = 0;
            mintPaymentDkcDepositeds[msg.sender].dkcDepositedFromIgo = 0;
    ...
    function mintWeaponN(uint32 num)
        ...
        oncePerBlock(msg.sender)
    {
        _payContractConvertedSupportingStaked(msg.sender, usdToDkc(mintWeaponFee * num));
        _mintWeaponNLogic(num);
    }

    function mintWeapon() external onlyNonContract oncePerBlock(msg.sender) {
        _payContractConvertedSupportingStaked(msg.sender, usdToDkc(mintWeaponFee));
        _mintWeaponLogic();
    }

    function mintWeaponNUsingStakedDkc(uint32 num)
        external
        onlyNonContract
        oncePerBlock(msg.sender)
    {
        int128 discountedMintWeaponFee =
            mintWeaponFee
                .mul(PAYMENT_USING_STAKED_DKC_COST_AFTER_DISCOUNT)
                .mul(ABDKMath64x64.fromUInt(num));
        _payContractStakedOnly(msg.sender, usdToDkc(discountedMintWeaponFee));

        _mintWeaponNLogic(num);
    }

    function mintWeaponUsingStakedDkc() external onlyNonContract oncePerBlock(msg.sender) {
        int128 discountedMintWeaponFee =
            mintWeaponFee
                .mul(PAYMENT_USING_STAKED_DKC_COST_AFTER_DISCOUNT);
        _payContractStakedOnly(msg.sender, usdToDkc(discountedMintWeaponFee));
    ...
    function burnWeapon(uint256 burnID) external isWeaponOwner(burnID) {
        _payContractConvertedSupportingStaked(msg.sender, usdToDkc(burnWeaponFee));

        _burnWeaponLogic(burnID);
    }

    function burnWeapons(uint256[] calldata burnIDs) external isWeaponsOwner(burnIDs) {
        _payContractConvertedSupportingStaked(msg.sender, usdToDkc(burnWeaponFee.mul(ABDKMath64x64.fromUInt(burnIDs.length))));
    ...
    function reforgeWeapon(uint256 reforgeID, uint256 burnID) external isWeaponOwner(reforgeID) isWeaponOwner(burnID) {
        _payContractConvertedSupportingStaked(msg.sender, usdToDkc(reforgeWeaponFee));
    ...
    function reforgeWeaponWithDust(uint256 reforgeID, uint8 amountLB, uint8 amount4B, uint8 amount5B) external isWeaponOwner(reforgeID) {
        _payContractConvertedSupportingStaked(msg.sender, usdToDkc(reforgeWeaponWithDustFee));

        _reforgeWeaponWithDustLogic(reforgeID, amountLB, amount4B, amount5B);
    }

    function burnWeaponUsingStakedDkc(uint256 burnID) external isWeaponOwner(burnID) {
        int128 discountedBurnWeaponFee =
            burnWeaponFee.mul(PAYMENT_USING_STAKED_DKC_COST_AFTER_DISCOUNT);
        _payContractStakedOnly(msg.sender, usdToDkc(discountedBurnWeaponFee));

        _burnWeaponLogic(burnID);
    }

    function burnWeaponsUsingStakedDkc(uint256[] calldata burnIDs) external isWeaponsOwner(burnIDs) {
        int128 discountedBurnWeaponFee =
            burnWeaponFee
    ...
    function reforgeWeaponUsingStakedDkc(uint256 reforgeID, uint256 burnID) external isWeaponOwner(reforgeID) isWeaponOwner(burnID) {
        int128 discountedReforgeWeaponFee =
            reforgeWeaponFee
                .mul(PAYMENT_USING_STAKED_DKC_COST_AFTER_DISCOUNT);
        _payContractStakedOnly(msg.sender, usdToDkc(discountedReforgeWeaponFee));

        _reforgeWeaponLogic(reforgeID, burnID);
    }

    function reforgeWeaponWithDustUsingStakedDkc(uint256 reforgeID, uint8 amountLB, uint8 amount4B, uint8 amount5B) external isWeaponOwner(reforgeID) {
        int128 discountedReforgeWeaponWithDustFee =
            reforgeWeaponWithDustFee
                .mul(PAYMENT_USING_STAKED_DKC_COST_AFTER_DISCOUNT);
        _payContractStakedOnly(msg.sender, usdToDkc(discountedReforgeWeaponWithDustFee));
    ...
    function _payContractTokenOnly(address playerAddress, uint256 convertedAmount) internal {
        ...
        dkcToken.transferFrom(playerAddress, address(this), fromUserWallet);
    }
    ...
    function _payContract(address playerAddress, int128 usdAmount) internal
        ...
        return _payContractConverted(playerAddress, usdToDkc(usdAmount));
    }
    ...
    function _payContractConverted(address playerAddress, uint256 convertedAmount) internal
        ...
        _deductPlayerDkcStandard(playerAddress, fromInGameOnlyFunds, fromTokenRewards, fromUserWallet);
    ...
    function _payContractConvertedSupportingStaked(address playerAddress, uint256 convertedAmount) internal
        ...
        (uint256 fromUserWallet, uint256 fromStaked) =
            getDkcToSubtractSingle(
                _remainder,
                dkcToken.balanceOf(playerAddress)
            );
    ...
    function _deductPlayerDkcStandard(
    ...
    function _payPlayerConverted(address playerAddress, uint256 convertedAmount) internal {
        dkcToken.safeTransfer(playerAddress, convertedAmount);
    }
    ...
    function giveInGameOnlyFunds(address to, uint256 dkcAmount) external restricted {
        totalInGameOnlyFunds = totalInGameOnlyFunds.add(dkcAmount);
        inGameOnlyFunds[to] = inGameOnlyFunds[to].add(dkcAmount);

        dkcToken.safeTransferFrom(msg.sender, address(this), dkcAmount);

        emit InGameOnlyFundsGiven(to, dkcAmount);
    }

    function _giveInGameOnlyFundsFromContractBalance(address to, uint256 dkcAmount) internal {
        totalInGameOnlyFunds = totalInGameOnlyFunds.add(dkcAmount);
        inGameOnlyFunds[to] = inGameOnlyFunds[to].add(dkcAmount);
    ...
    function giveInGameOnlyFundsFromContractBalance(address to, uint256 dkcAmount) external restricted {
        _giveInGameOnlyFundsFromContractBalance(to, dkcAmount);
    }
    ...
    function stakeUnclaimedRewards() external {
        ...
        if(promos.getBit(msg.sender, 4) == false) {
            dkcToken.approve(address(stakeFromGameImpl), _tokenRewards);
    ...
    function getTotalDkcOwnedBy(address wallet) public view returns (uint256) {
        return inGameOnlyFunds[wallet] + getTokenRewardsFor(wallet) + dkcToken.balanceOf(wallet);
    }
...
contract Blacksmith is Initializable, AccessControlUpgradeable {
    ...
    uint256 public constant SHIELD_DKC_FEE = 3 ether;
    ...
    function purchaseShield() public {
        ...
        game.payContractTokenOnly(msg.sender, SHIELD_DKC_FEE);
