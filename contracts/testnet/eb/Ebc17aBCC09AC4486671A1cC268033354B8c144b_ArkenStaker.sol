...
// File contracts/token/ArkenToken.sol
...
// File contracts/token/ArkenSmithy.sol
...
// File contracts/token/ArkenSmithyPool.sol
...
// File contracts/token/ArkenStaker.sol
...
contract ArkenStaker is ArkenSmithyPool {
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo memory pool = updatePool(_pid);
        UserInfo memory user = userInfos[_pid][msg.sender];
        if (user.amount > 0) {
            _settlePendingArken(msg.sender, _pid);
        }
        if (_amount > 0) {
        ...
        user.rewardDebt =
            (user.amount * pool.accPerShare) /
            ACC_ARKEN_PRECISION;

        userInfos[_pid][msg.sender] = user;
        poolInfos[_pid] = pool;
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        PoolInfo memory pool = updatePool(_pid);
        UserInfo memory user = userInfos[_pid][msg.sender];
        require(_amount <= user.amount, 'withdraw: Insufficient');
        _settlePendingArken(msg.sender, _pid);
        if (_amount > 0) {
            user.amount = user.amount - _amount;
            lpTokens[_pid].safeTransfer(msg.sender, _amount);
        }
        user.rewardDebt =
            (user.amount * pool.accPerShare) /
            ACC_ARKEN_PRECISION;
    ...
    /// @param _pid The id of the pool.
    function _settlePendingArken(address _user, uint256 _pid) internal {
        UserInfo memory user = userInfos[_pid][_user];
        uint256 accArken = (user.amount * poolInfos[_pid].accPerShare) /
            ACC_ARKEN_PRECISION;
        uint256 pending = accArken - user.rewardDebt;
        _withdrawFromSmithy(_user, pending);
    }

    function pendingArken(address _user, uint256 _pid)
        public
        view
        returns (uint256 pending)
    {
        ...
        UserInfo memory user = userInfos[_pid][_user];
        uint256 accArken = (user.amount * pool.accPerShare) /
            ACC_ARKEN_PRECISION;
        return accArken - user.rewardDebt;
    }

    // emergencyWithdraw by User
}