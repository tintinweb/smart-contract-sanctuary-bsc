...
contract Market is Ownable, ReentrancyGuard {
    ...
    uint256 private _interval = 3600;
    ...
    mapping(address => uint256) private _userRequest;
    mapping(address => bool) private _winnerReward;
    ...
    modifier advanceTraderRewards(address account) {
        ...
        if (_currentEpochTime > _traders[account].currentPeriod) {
            ...
            _purchases[account] += _traders[account].volume;
    ...
    function getCurrentEpochVolume(address account) external view returns (uint256) {
        return 
            block.timestamp < getCurrentEpochTime() && 
            getCurrentEpochTime() <= _traders[account].currentPeriod ?
            _traders[account].volume : 0;
    }

    function getVolumeOfUser(address account) external view returns (uint256) {
        return _purchases[account] + _traders[account].volume;
    }
    ...
    function hasRequest(address account) public view returns (bool) {
        return
            _userRequest[account] < block.number &&
            _userRequest[account] > (block.number - 256);
    }
    ...
    function purchase(address account, address token_, uint256 amount, uint256 source)
        ...
        if (account != address(0)) {
            ...
            _executeTransfer(account, reward);
            
            _traders[account].volume += totalPrice;
    ...
    function open() external nonReentrant {
        ...
        uint256 tokenId = IOracle(oracle).random(_userRequest[_msgSender()]);
        if (winner == 0) {
            _cardSales[tokenId] += 1;
        }
        
        _userRequest[_msgSender()] = 0;
    ...
    function claim() external nonReentrant advanceTraderRewards(_msgSender()){
        ...
        _userRewards[_msgSender()] += reward;
        fund -= reward;
    ...
    function winnerClaim() external nonReentrant {
        ...
        require(!_winnerReward[_msgSender()],"Already received");
        _winnerReward[_msgSender()] = true;
    ...
    function _executeTransferFrom(address token_, address from, uint256 amount) private {
        ...
        uint256 payeeAmount = amount.mul(40).div(100);
        jackpot += amount - fundAmount.mul(2) - payeeAmount;
        fund += fundAmount;
