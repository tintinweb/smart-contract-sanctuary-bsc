...
contract Crowdfunding is AccessControlEnumerable {
    ...
    //公共参数 0提现手续费比例1% 本金提现也收手续费,1:奖励锁定需点灯解锁比例20%,2:普通区单笔最大投资比例1%,3: 快速区单笔最大投资比例5%
    uint16[] public commonRates = [10, 200, 10, 50];
    //慢速区/快速区级差
    uint16[][] public bonusRates = [[0, 0, 100, 200, 250], [0, 0, 50, 100, 125]];
    /*
    0:静态奖比例,1:经纪人奖比例,2:失败前三期返还本金比例,3:捐赠比例,4:质押池比例,5:失败当期彩蛋池奖励比例
    6:收益免费提现比例
    */
    uint16[][] public rates = [[100, 0, 800, 10, 50, 600, 200], [50, 0, 900, 5, 25, 600, 200]];
    uint16 public maxReserveRound = 5; //最多预约后续5期
    uint16 public fundInterval = 6; //可变
    ...
    function setBonusRate(
        ...
        bonusRates[poolType][index] = bonus;
    }
    ...
    //参与众筹
    function fund(uint16 _regionIndex, uint256 _amount) external {
        ...
        uint32 _round = regions[_regionIndex].currentRound;
        ...
        processBrokerReward(msg.sender, _regionIndex, _round, fundAmount);
    ...
    function reserve(
        ...
        uint32 currentRound = regions[_regionIndex].currentRound;
    ...
    //完成预约
    function finishReserve(uint256 _regionIndex, uint32 _round) external {
        Project storage p = projects[_regionIndex][_round];
        ...
        accounts[msg.sender].lastFundTime = block.timestamp;
        processBrokerReward(msg.sender, _regionIndex, _round, _amount);
    ...
    function finishProject(uint256 _regionIndex) public {
        ...
        uint16 poolType = regions[_regionIndex].poolType;
        if (_currentAmount == p.targetAmount) {
            ...
            if (_round >= 3 && projects[_regionIndex][_round - 3].state == 3) {
                ...
                uint256 _stakeAmount = (_targetAmount * rates[poolType][4]) / 1000;
                uint256 _donateAmount = (_targetAmount * rates[poolType][3]) / 1000;
                //剩余的经纪人奖放回池中
                uint256 _refundAmount = (_targetAmount * (10000 + 10 * rates[poolType][0] + bonusRates[poolType][4])) / 10000 - p3.brokerRewardLeft;
            ...
            if (_round > 0) {
                ...
                for (uint256 i = start; i < _round; i++) {
                    ...
                    if (pi.state == 3) {
                        pi.state = 6;
                        pi.refundAmount = (pi.targetAmount * (rates[poolType][2])) / 1000;
            ...
            if (_currentAmount > 0) {
                ...
                uint256 _bonusAmount = (_poolAmount * rates[poolType][5]) / 1000;
    ...
    )
    {
        ...
        if (pf.version == projects[_regionIndex][_round].version) {
            ...
            if (state == 4) {
                ...
                amounts[1] = _fundAmount;
                amounts[2] = (_fundAmount * rates[poolType][0]) / 1000;
            } else if (state == 6) {
                amounts[1] = (_fundAmount * rates[poolType][2]) / 1000;
                amounts[3] = (_fundAmount * rates[poolType][6]) / rates[poolType][7];
    ...
    function _harvest(
        ...
        uint256 lockedAmount = (_rewardAmount * commonRates[1]) / 1000;
        if (lockedAmount > 0) {
        ...
        if (_amount > 0) {
            ...
            if (freeWithdrawAmount >= _amount) {
                ...
                uint256 _feeAmount = ((_amount - freeWithdrawAmount) * commonRates[0]) / 1000;
    ...
    function getBonus(uint256 _regionIndex) internal view returns (uint16[] memory) {
        return bonusRates[regions[_regionIndex].poolType];
    }
    ...
    function processBrokerReward(
        ...
        uint16 used = 0;
        uint16 level = 0;
        uint16[] memory bonus = getBonus(_regionIndex);
        for (uint16 i = 0; i < 45; i++) {
            ...
            if (bonus[parentLevel] > bonus[level]) {
                ...
                uint256 _amount = (_fundAmount * diff) / 10000;
                used += diff;
        ...
        if (bonus[4] > used) {
            projects[_regionIndex][_round].brokerRewardLeft += (_fundAmount * (bonus[4] - used)) / 10000;
        }
    }
    ...
    function getMaxAmountPerTime(uint256 _targetAmount, uint16 _poolType) public view returns (uint256) {
        uint256 _max = (_targetAmount * (_poolType == 0 ? commonRates[2] : commonRates[3])) / 1000;
