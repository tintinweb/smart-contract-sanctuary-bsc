...
contract SelfServiceEditionCurationV4 is Whitelist, Pausable {
  ...
  uint256 public boxPrice = 0.02 ether;
  ...
  /**
   * @dev Called by administrator, Create a blind box
   */
  ...
  /**
   * @dev Called by users, Open the blind box
   */
  function openBlindBox(uint256 times) public payable {
    ...
    for(uint256 i = 0; i < times; i++){
      ...
      (uint256 _tokenId, string memory _uri) = kodaV2.mintTo(msg.sender, _editionNumber);

      blindBoxNFTRarityNumber[typeId][rarity] --;

      emit OpenBlindBox(msg.sender, _editionNumber, _tokenId, typeId, rarity, block.number, _uri);
  ...
  /**
   * @dev Called by users, Get NFT rarity
   */
  function getNFTType(uint256 typeId) internal view returns(uint256 rarity) {
...
interface IKODAV2SelfServiceEditionCuration {
  ...
  function mintTo(address _to, uint256 _editionNumber) external returns (uint256, string);
}
