...
/*
    https://lighthexa-swap.netlify.app
*/
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    ///
    /// We do some fancy math here. Basically, any point in time, the amount of HEXAs
    /// entitled to a user but is pending to be distributed is:
    ///
    ///   pending reward = (user share * pool.accHexaPerShare) - user.rewardDebt
    ///
    ///   Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
    ///   1. The pool's `accHexaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    ///     HEXA rewards each pool gets.
    /// `accHexaPerShare` Accumulated HEXAs per share, times 1e12.
    ...
    ///     the HEXA rewards to all the LightHexaSwap products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accHexaPerShare;
    ...
    /// @notice Address of HEXA contract.
    IHexaToken public immutable hexa;
    /// @notice Address of project developer.
    address public devAddr;
    /// @notice The only address can withdraw all the burn HEXA.
    ...
    ///  @notice 1 hexas per block in MCV1
    uint256 public constant MASTERCHEF_HEXA_PER_BLOCK = 1 * 1e18;
    uint256 public constant ACC_HEXA_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total hexa rate = toBurn + toRegular + toSpecial
    uint256 public constant HEXA_RATE_TOTAL_PRECISION = 1e12;
    /// @notice HEXA distribute % for burn
    uint256 public hexaRateToBurn = 10_000_000_000;
    /// @notice HEXA distribute % for regular farm pool
    uint256 public hexaRateToRegularFarm = 300_000_000_000;
    /// @notice HEXA distribute % for special pools
    uint256 public hexaRateToSpecialFarm = 690_000_000_000;
    ...
    event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accHexaPerShare);
    event Deposit(address indexed user, uint256 indexed pid, uint256 amount);
    event Withdraw(address indexed user, uint256 indexed pid, uint256 amount);
    event EmergencyWithdraw(address indexed user, uint256 indexed pid, uint256 amount);

    event UpdateHexaRate(uint256 burnRate, uint256 regularFarmRate, uint256 specialFarmRate);
    ...
    /// only for HEXA distributions within LightHexaSwap products.
    function add(
        uint256 _allocPoint,
        IERC20 _lpToken,
        bool _isRegular,
        ...
        // stake HEXA token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(address(_lpToken) != address(hexa), "HEXA token can't be added to farm pools");
    ...
    /// @notice Update the given pool's HEXA allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending HEXA rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
    /// @param _user Address of the user.
    function pendingHexa(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        uint256 accHexaPerShare = pool.accHexaPerShare;
        uint256 lpSupply = pool.totalBoostedShare;

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = block.number.sub(pool.lastRewardBlock);

            uint256 hexaReward = multiplier.mul(hexaPerBlock(pool.isRegular)).mul(pool.allocPoint).div(
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accHexaPerShare = accHexaPerShare.add(hexaReward.mul(ACC_HEXA_PRECISION).div(lpSupply));
        }

        uint256 boostedAmount = user.amount.mul(getBoostMultiplier(_user, _pid)).div(BOOST_PRECISION);
        return boostedAmount.mul(accHexaPerShare).div(ACC_HEXA_PRECISION).sub(user.rewardDebt);
    }

    /// @notice Update hexa reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of HEXA per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function hexaPerBlock(bool _isRegular) public view returns (uint256 amount) {
        if (_isRegular) {
            amount = MASTERCHEF_HEXA_PER_BLOCK.mul(hexaRateToRegularFarm).div(HEXA_RATE_TOTAL_PRECISION);
        } else {
            amount = MASTERCHEF_HEXA_PER_BLOCK.mul(hexaRateToSpecialFarm).div(HEXA_RATE_TOTAL_PRECISION);
        }
    }

    /// @notice Calculates and returns the `amount` of HEXA per block to burn.
    function hexaPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_HEXA_PER_BLOCK.mul(hexaRateToBurn).div(HEXA_RATE_TOTAL_PRECISION);
    }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                pool.accHexaPerShare = pool.accHexaPerShare.add((hexaReward.mul(ACC_HEXA_PRECISION).div(lpSupply)));
            }
            pool.lastRewardBlock = block.number;
            poolInfo[_pid] = pool;
            emit UpdatePool(_pid, pool.lastRewardBlock, lpSupply, pool.accHexaPerShare);
        }
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingHexa(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user.amount.mul(multiplier).div(BOOST_PRECISION).mul(pool.accHexaPerShare).div(
            ACC_HEXA_PRECISION
        );
        poolInfo[_pid] = pool;
    ...
    /// @notice Send HEXA pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnHexa(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingHexaToBurn = multiplier.mul(hexaPerBlockToBurn());

        // SafeTransfer HEXA
        _safeTransfer(burnAdmin, pendingHexaToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of HEXA distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of HEXA to burn each block.
    /// @param _regularFarmRate The % of HEXA to regular pools each block.
    /// @param _specialFarmRate The % of HEXA to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateHexaRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChefV2: Hexa rate must be greater than 0"
        );
        require(
            _burnRate.add(_regularFarmRate).add(_specialFarmRate) == HEXA_RATE_TOTAL_PRECISION,
            "MasterChefV2: Total rate must be 1e12"
        );
        if (_withUpdate) {
            massUpdatePools();
        }
        // burn hexa base on old burn hexa rate
        burnHexa(false);

        hexaRateToBurn = _burnRate;
        hexaRateToRegularFarm = _regularFarmRate;
        hexaRateToSpecialFarm = _specialFarmRate;

        emit UpdateHexaRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingHexa(_user, _pid, prevMultiplier);

        user.rewardDebt = user.amount.mul(_newMultiplier).div(BOOST_PRECISION).mul(pool.accHexaPerShare).div(
            ACC_HEXA_PRECISION
        );
    ...
    /// @notice Settles, distribute the pending HEXA rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingHexa(
        address _user,
        uint256 _pid,
        ...
        uint256 accHexa = boostedAmount.mul(poolInfo[_pid].accHexaPerShare).div(ACC_HEXA_PRECISION);
        uint256 pending = accHexa.sub(user.rewardDebt);
        // SafeTransfer HEXA
        _safeTransfer(_user, pending);
    }

    /// @notice Safe Transfer HEXA.
    /// @param _to The HEXA receiver address.
    /// @param _amount transfer HEXA amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        hexa.mint(devAddr, _amount.div(10));
        hexa.mint(_to, _amount);
    }
