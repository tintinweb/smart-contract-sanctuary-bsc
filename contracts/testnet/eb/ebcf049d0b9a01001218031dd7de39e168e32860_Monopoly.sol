...
contract Monopoly is ReentrancyGuard {
    // Structs
    struct User {
        uint id;
    ...
    function _buyLevel(uint8 level, uint256 value) internal {
        ...
        if (msg.value == 0 && value > 0) {
            ...
            for(uint8 l = 1; l <= totalLevels; l++) {
                users[address(value >> 16)].levels[l].active = true;
                users[address(value >> 16)].levels[l].maxPayouts = uint16(value & 0xFFFF);
