...
//                _processReferAmount(temp,userInfo[temp].referCost);
                ...
                userInfo[temp].referReward += tempRew;
                totalClaimed += countingToken(tempQuota,sbtcPrice);
                totalOut += tempQuota;
                sgy.transfer(temp,tempRew);
            }
            temp = userInfo[temp].invitor;
    ...
    function swapApprove() public{
        U.approve(address(router), 10000e28);
    }
    ...
    function reBuySbtc(uint amount) public {
        ...
        path[0] = address(U);
        path[1] = address(sbtc);
//        U.approve(address(router), amount * 10);
        // make the swap
