...
contract Exchange is IExchange, Owned {
  ...
  function setCustodian(address payable newCustodian) external onlyAdmin {
    require(_custodian == address(0x0), 'Custodian set once');
  ...
  function deposit(
    ...
    require(quantityInPips > 0, 'Quantity is low');
    ...
    _balancesInPips[wallet][assetAddress] = _balancesInPips[wallet][assetAddress].add(
      quantityInPips
    );
    _depositIndex++;
...
library AssetRegistry {
  ...
  function registerToken(
    ...
    require(
      tokenAddress != IERC20(address(0)) && Address.isContract(address(tokenAddress)),
