...
contract BLXNBusdRocket is Context, Ownable, ReentrancyGuard {
    ...
    function MakeDeposit(uint256 amtx, address reffy) public noReentrant {
        ...
        require (secondlaunchTime < nowTime || Whitelist[msg.sender] == 1, "General launch or VIP required.");
        // First, we check to see if general launch has started OR if you're on the VIP whitelist.

        require (firstlaunchTime < nowTime, "VIP required.");
    ...
    function MakeClaim() public noReentrant{
        ...
        //Your initial deposit divided by 10 is your ROI amount.

        require (user.maxPayout - user.totalWithdrawn > 0, "You cannot claim anymore if your max payout has been reached.");
        // In order to claim, you must not have reached your max payout.

        require (rn - user.lastClaim >= 0, "You can only claim once per day."); //change to 86400 LM
    ...
    function MakeWithdraw() public noReentrant{
        ...
        require(rn - user.lastWithdraw > 0, "You can only withdraw once per week."); //change to 604800 LM
        // We make sure at least 1 week has passed before you can withdraw.
    ...
    function MakeRefWithdraw() public noReentrant{
        ...
        require (user.totalRefClaimable > 0, "You don't have any referrals to claim!");
    ...
    function whiteListSignup() public payable noReentrant{
        ...
        require (nowTime < 0, "VIP reservation period has ended."); //change time to whitelist launch time LM
        // We check to make sure the VIP reservation period has not closed yet.

        require (msg.value == 0.05 ether, "VIP reservation price is set at 0.05 BNB.");
