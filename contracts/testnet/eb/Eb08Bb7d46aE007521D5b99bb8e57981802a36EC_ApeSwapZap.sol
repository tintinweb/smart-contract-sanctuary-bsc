...
contract ApeSwapZap {
    ...
    /// @param _slippage slippage allowed for swap. 1 = 0.01% slippage
    function getMinAmounts(
        uint256 _inputAmount,
        address[] calldata _path0,
        address[] calldata _path1,
        uint256 _slippage //1 = 0.01%
        ...
        _minAmountSwap0 = (_minAmountSwap0 * (10000 - _slippage)) / 10000;
        _minAmountSwap1 = (_minAmountSwap1 * (10000 - _slippage)) / 10000;

        address token0 = _path0.length == 0
            ? _path1[0]
            : _path0[_path0.length - 1];
        address token1 = _path0.length == 0
            ? _path0[0]
        ...
        uint256[] memory _minAmountsSwap = new uint256[](2);
        _minAmountsSwap[0] = _minAmountSwap0;
        _minAmountsSwap[1] = _minAmountSwap1;

        uint256[] memory _minAmountsLP = new uint256[](2);
        _minAmountsLP[0] = _minAmountSwap0;
        _minAmountsLP[1] = amountB;

        return (_minAmountsSwap, _minAmountsLP);
    }
