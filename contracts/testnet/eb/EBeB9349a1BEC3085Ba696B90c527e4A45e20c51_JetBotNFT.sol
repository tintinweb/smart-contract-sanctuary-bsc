...
contract JetBotNFT is ERC721, RoleControl, SignatureControl {
    ...
    mapping(address => uint256) userToCurrentlyNestedToken;

    uint256[] public planPrices = [50 ether, 100 ether, 200 ether, 1500 ether];
    string[] planMetadata = [
        "ipfs://QmY1",
        "ipfs://QmY2",
        "ipfs://QmY3",
    ...
    event NestingToggled(address user, uint256 tokenId, bool state);
    ...
    function mint(uint256 plan, address referrer) public {
        ...
        emit PlanBought(msg.sender, supply, plan, userToReferrers[msg.sender]);
        if (userToCurrentlyNestedToken[msg.sender] == 0) {
            _toggleNesting(supply);
        }
    }

    function payReferrers(address[] memory referrers, uint256[] memory amounts)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < referrers.length; i++) {
            paymentToken.transfer(referrers[i], amounts[i]);
        }
    }
    ...
    function _toggleNesting(uint256 tokenId) internal {
        ...
        emit NestingToggled(msg.sender, tokenId, nestingStatus[tokenId]);
    }
    ...
    function disableNesting(
        ...
        tokenIdToMetadata[supply] = metadata;
        _toggleNesting(tokenId);
    }
