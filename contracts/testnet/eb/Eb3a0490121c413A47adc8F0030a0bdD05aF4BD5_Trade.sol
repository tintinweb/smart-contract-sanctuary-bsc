...
//        console.log("--- Fee ---");
//        console.log("paymentAmt\t %i", paymentAmt);
//        console.log("price\t\t %i", price);
//        console.log("sellerFee\t %i", sellerFee);
//        console.log("platformFee\t %i", platformFee);
//        console.log("royaltyFee\t %i", royaltyFee);
//        console.log("assetFee\t %i", assetFee);
        return Fee(platformFee, assetFee, royaltyFee, price, tokenCreator);
    }

    function _tradeBid(Order memory order, Fee memory fee) internal virtual {
    ...
    function _tradeAsset(Order memory order, Fee memory fee) internal virtual {
        if (order.nftType == BuyingAssetType.ERC721) {
            transferProxy.erc721safeTransferFrom(
                IERC721(order.nftAddress),
                order.seller,
        ...
        if (fee.platformFee > 0) {
            require(payable(owner).send(fee.platformFee));
        }
        if (fee.royaltyFee > 0) {
            require(payable(fee.tokenCreator).send(fee.royaltyFee));
        }
        console.log("sum\t\t %i", fee.assetFee + fee.royaltyFee + fee.platformFee);
        require(payable(order.seller).send(fee.assetFee));
    }

    // ToDO разделитть метот на работу с 721 и 1155
    // ToDO выкинуть из Order лишене
