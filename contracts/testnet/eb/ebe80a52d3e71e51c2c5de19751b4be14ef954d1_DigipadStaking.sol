...
contract DigipadStaking is Ownable {
    using SafeMath for uint256;
   ...
   function pendingRewards(uint256 _orderId ) external view returns (uint256) {
        ...
        if(_orderId <= latestOrderId && orderInfo.amount > 0 && !orderInfo.claimed){
            ...
            if(block.timestamp >= orderInfo.endtime){
                uint256 reward = ((orderInfo.amount.mul(orderInfo.returnPer).div(10000)).div(365)).mul(orderInfo.lockupDuration);
    ...
    function deposit(uint256 _amount , uint256 _lockupDuration ) external {
        ...
        uint256 userReward = ((_amount.mul(pool.returnPer).div(10000)).div(365)).mul(_lockupDuration);
        uint256 requiredToken = totalStake.add(totalRewardPending).add(userReward).sub(totalWithdrawal);
        require(token.balanceOf(address(this)) > requiredToken , "Staking reward not avalible right now");
        ...
        orders[++latestOrderId] = OrderInfo(
            ...
            pool.returnPer,
            block.timestamp,
            block.timestamp.add(_lockupDuration.mul(1)),
            0,
            false
        );

        totalStake =totalStake.add(_amount);
        totalRewardPending = totalRewardPending.add(((_amount.mul(pool.returnPer).div(10000)).div(365)).mul(_lockupDuration));
    ...
    //     require(block.timestamp >= orderInfo.endtime , "You can claim after lock period over !!");
    ...
    //     totalRewardPending -= claimAvlible;
    // }
    ...
    function withdraw(uint256 orderId) external {
        ...
        uint256 reward = (amount.mul(orderInfo.returnPer).div(10000).div(365)).mul(orderInfo.lockupDuration);
        uint256 claimAvlible =  reward.sub(orderInfo.claimedReward);
        uint256 total = amount.add(claimAvlible);
        ...
        totalRewardPending = totalRewardPending.sub(claimAvlible);
    ...
    function emergencyWithdraw(uint256 orderId) external {
        ...
        token.transfer(address(msg.sender) , total);
        totalWithdrawal = totalWithdrawal.add(total);

        orderInfo.claimed = true;
        balanceOf[msg.sender] = balanceOf[msg.sender].sub(orderInfo.amount);
       
        totalRewardPending = totalRewardPending.sub(((orderInfo.amount.mul(orderInfo.returnPer).div(10000)).div(365)).mul(orderInfo.lockupDuration)).add(fees);
        emit WithdrawAll(msg.sender , total);
        emit test(totalRewardPending,totalRewardPending,fees);
    } 
    ...
    function withdrawToken() public onlyOwner 
    {
       ...
       uint256 total = contract_balance.sub(totalStake.sub(totalWithdrawal)).sub(totalRewardPending);
