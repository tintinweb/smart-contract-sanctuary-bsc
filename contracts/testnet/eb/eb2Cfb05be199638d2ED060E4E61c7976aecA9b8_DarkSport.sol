...
contract DarkSport is OwnableUpgradeSafe {
    ...
    mapping(uint256 => address) public tokenBets;
    ...
    mapping(uint256 => HouseInfo) public houseInfo;

    mapping(address => bool) public admin;

    mapping(uint256 => mapping(address => bool)) private _status;
    uint256[100000][3][3][5] public _betMatchDoor;
    mapping(uint256 => mapping(uint256 => uint256)) public totalMatchesBet;

    /* =================== Added variables (need to keep orders forproxy to work) =================== */
    // ...
    ...
    event EditStandardPrice(uint256 index, uint256 standardPrice);
    ...
    //event DrawAllTicket(address player, uint256 matchId, uint256 numPendingTickets, uint256[] memory totalPayout, uint256 claimedTime);
    ...
    function ediTokenBet(uint256 _index, address _tokenBet) external onlyOwner {
        tokenBets[_index] = _tokenBet;
    }

    function addTokenBet(address _tokenBet) external onlyOwner {
        tokenBets[numToken] = _tokenBet;
        numToken = numToken.add(1);
    }
    ...
    function setStandardPrice(uint256 _index, uint256 _standardPrice) external onlyOwner {
        require(_standardPrice >= 0.01 ether, "too low");
        houseInfo[_index].standardPrice = _standardPrice;
        emit EditStandardPrice(_index, _standardPrice);
    }

    function setMaxBetInMatch(uint256 _index, uint256 _maxPayoutInDoor) external onlyOwner {
        require(_maxPayoutInDoor >= 0.01 ether, "too low");
        houseInfo[_index].maxPayoutInDoors = _maxPayoutInDoor;
    }
    ...
    function getPlayerStats(address _player) public view returns (uint256[] memory, uint256[] memory) {
        return (playerStats[_player].totalBet, playerStats[_player].totalPayout);
    }
    ...
    returns (uint8 _index, string memory _description, uint8 _numDoors, uint32[] memory _odds, uint8[] memory _doorResults,
        uint256 _numTickets, uint256[] memory _totalBetAmount, uint256[] memory _totalPayoutAmount, uint256[][] memory _doorBetAmount, uint256 _maxBudget) {
    ...
    function getMaxBetAmount(uint256 _index, uint256 _matchId, uint8 _betTypeId, uint8 _door) public view returns (uint256 _amount) {
        BetType memory betType = matchBetTypes[_matchId][_betTypeId];
        uint256 _odd = betType.odds[_door];
        if (_betMatchDoor[_matchId][_betTypeId][_door][_index] >= houseInfo[_index].maxPayoutInDoors) return 0; else
        return houseInfo[_index].maxPayoutInDoors.sub(_betMatchDoor[_matchId][_betTypeId][_door][_index].div(_odd).mul(10000));
    }

    function addMatchInfoThreeStandardBetTypes(string memory _matchName, uint256 _startBettingTime, uint256 _endBettingTime,
    ...
    function addMatchInfo(string memory _matchName, uint256 _startBettingTime, uint256 _endBettingTime,
        string memory _betDescription, uint8 _numDoors, uint32[] memory _odds, uint256 _maxBudget) public onlyAdmin returns (uint256 _matchId) {
    ...
    function addMatchBetType(uint256 _matchId, string memory _betDescription, uint8 _numDoors, uint32[] memory _odds, uint256 _maxBudget) public onlyAdmin returns (uint8 _betTypeId) {
        MatchInfo storage matchInfo = matchInfos[_matchId];
        ...
        matchBetTypes[_matchId].push(
            BetType({
                ...
                numTickets: 0,
                totalBetAmount: new uint256[](numToken),
                totalPayoutAmount: new uint256[](numToken),
    ...
    function settleMatchResult(uint256 _matchId, uint8 _betTypeId, uint8[] memory _doorResults) public onlyAdmin {
        MatchInfo storage matchInfo = matchInfos[_matchId];
    ...
    function settleMatchResultThreeStandardBetTypes(uint256 _matchId, uint8[] memory _doorResults1x2, uint8[] memory _doorResultsHandicap, uint8[] memory _doorResultsOverUnder) external onlyAdmin {
    ...
    function buyTicket(uint256 _indexToken, uint256 _matchId, uint8 _betTypeId, uint8 _betDoor, uint32 _betOdd, uint256 _betAmount) public lock returns (uint256 _ticketIndex) {
        require(_betAmount >= houseInfo[_indexToken].standardPrice, "_betAmount less than standard price");
        uint256 _maxBetAmount = getMaxBetAmount(_indexToken, _matchId, _betTypeId, _betDoor);
        require(_betAmount <= _maxBetAmount, "_betAmount exceeds _maxBetAmount");
        address _tokenAddress = address(tokenBets[_indexToken]);
        MatchInfo storage matchInfo = matchInfos[_matchId];
        ...
        IERC20(_tokenAddress).safeTransferFrom(_player, address(fund), _betAmount);
        totalMatchesBet[_matchId][_indexToken] = totalMatchesBet[_matchId][_indexToken].add(_betAmount);
        ...
        betType.totalBetAmount[_indexToken] = betType.totalBetAmount[_indexToken].add(_betAmount);
        betType.doorBetAmount[_betDoor][_indexToken] = betType.doorBetAmount[_betDoor][_indexToken].add(_betAmount);
        houseInfo[_indexToken].totalBetAmount = houseInfo[_indexToken].totalBetAmount.add(_betAmount);
        //totalBetAmount[_indexToken] = totalBetAmount[_indexToken].add(_betAmount);
        matchesOf[_player][_matchId].push(_ticketIndex);
        ticketsOf[_player].push(_ticketIndex);
        playerStats[_player].totalBet[_indexToken] = playerStats[_player].totalBet[_indexToken].add(_betAmount);
        _betMatchDoor[_matchId][_betTypeId][_betDoor][_indexToken] = _betMatchDoor[_matchId][_betTypeId][_betDoor][_indexToken].add(_betAmount.mul(uint256(_betOdd)).div(10000));
    ...
    function settleBet(uint256 _ticketIndex) public lock returns (address _player, uint256 _payout) {
        ...
        betType.totalPayoutAmount[ticket.indexToken] = betType.totalPayoutAmount[ticket.indexToken].add(_payout);
        houseInfo[ticket.indexToken].totalPayoutAmount = houseInfo[ticket.indexToken].totalPayoutAmount.add(_payout);
        playerStats[_player].totalPayout[ticket.indexToken] = playerStats[_player].totalPayout[ticket.indexToken].add(_payout);
        ticket.claimedTime = now;

        if (_payout > 0) {
            IERC20(tokenBets[ticket.indexToken]).safeTransferFrom(address(fund), _player, _payout);
        }

        emit DrawTicket(_player, _ticketIndex, _matchId, _betTypeId, _payout, now);
    }

    function settleAll(uint256 _matchId) external onlyOneBlock returns (uint256[] memory _totalPayouts) {
        _totalPayouts = new uint256[](numToken);
        address _player = msg.sender;
        ...
        for (uint256 _ticketIndex = 0; _ticketIndex < _length; _ticketIndex++) {
            ...
            if (ticket.matchId == _matchId && ticket.status == 0) {
                ...
                _totalPayouts[ticket.indexToken] = _totalPayouts[ticket.indexToken].add(_payout);
    ...
    function getTotalPendingRewardOfMatch(address _player, uint256 _matchId) public view returns (uint256[] memory _totalPayouts) {
        _totalPayouts = new uint256[](numToken);
        uint256 _payout = 0;
        uint256 _length = ticketsOf[_player].length;
        for (uint256 _ticketIndex = 0; _ticketIndex < _length; _ticketIndex++) {
