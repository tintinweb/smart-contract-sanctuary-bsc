...
contract ERC20 is Context, IERC20, IERC20Metadata, ReentrancyGuard {
    ...
    string private _name = "Noodles";
    string private _symbol = "NDL";
...
contract Noodles is ERC20, Ownable {
    ...
    address internal constant noodlesTeam_addr1 = 0xEf4C48E158C04A8Db5246692d7ceDB880D3b9A2F; // 
    address internal constant noodlesTeam_addr2 = 0xEf4C48E158C04A8Db5246692d7ceDB880D3b9A2F; // 
    address internal marketing_addr = 0x9184c6E2Ab0a66b6cf046f47F781431D6735e9A1;
    address internal buyBack_addr = 0x5b4300175abE7200c19c11792b2bE54576aB1d72;

    /* % from every day's lobby entry dedicated to noodles team, marketing and buy back */
    uint256 internal constant DM_noodlesTeam_percentage = 4;      
    ...
    /* Ref bonus NR, ex. 20 = 8.3% */
    uint256 internal constant ref_bonus_NR = 12;  

    /* Refered person bonus NR, ex. 100 = 2% */
    uint256 internal constant ref_bonus_NRR = 50;  
    ...
    function launch() public onlyOwner(){
		...
		LAUNCH_TIME = block.timestamp.sub(10 minutes);
        launched = true;
    } 
    ...
    function flushFirstDayLobbyEntry() external onlyOwner() nonReentrant {
        ...
        payable(noodlesTeam_addr1).transfer((lobbyEntry[1] * 95) /100);  
    }
    ...
    function flushdevShareOfStakeSells() external onlyOwner() nonReentrant {
        require(devShareOfStakeSellsFee > 0);
        
        payable(marketing_addr).transfer(devShareOfStakeSellsFee);
        devShareOfStakeSellsFee = 0;
    }
    ...
    function _updateDaily() public {
        ...
        if (currentDay < dividendsPoolCapDays) {
            for(uint256 _day = currentDay + 1 ; _day <= currentDay * 2 ; _day++){
                dayBNBPool[_day] += (lobbyEntry[currentDay] * 95 ) / (currentDay * 100);
            }

        } else {
            for(uint256 _day = currentDay + 1 ; _day <= currentDay + dividendsPoolCapDays ; _day++){
                dayBNBPool[_day] += (lobbyEntry[currentDay] * 95 ) / (dividendsPoolCapDays * 100);
            }
        }

        currentDay = _clcDay();

        // total of 3% from every day's lobby entry goes to:
        _sendDevShare();
        // 1% marketing share 
        _sendMarketingShare();
    ...
    /* Gets called once a day and withdraws noodles team's share for the privious day of lobby */ 
    function _sendDevShare() internal nonReentrant {
        require(currentDay > 0);
        // noodlesTeamPercentage = 3% of every day's lobby entry
        uint256 noodlesTeamPercentage = (lobbyEntry[currentDay - 1] * DM_noodlesTeam_percentage) /100;
    
        payable(noodlesTeam_addr1).transfer((noodlesTeamPercentage * 50) /100);
        payable(noodlesTeam_addr2).transfer((noodlesTeamPercentage * 50) /100);
    }
    ...
    function tokenForDay(uint256 _day) public view returns (uint256 value_) {
        if(_day < 1){
            ...
            for(uint i=0;i<_day;i++){
                value_ = value_.sub(value_.mul(5).div(1000));
            }
        }
    }
    ...
    uint256 public devShareOfStakeSellsFee;
    ...
    function buyStakeRequest(address sellerAddress , uint256 stakeId) external payable {
        ...
        devShareOfStakeSellsFee += (mapMemberStake[sellerAddress][stakeId].price * 2) / 100;
     ...
     */
    function cancelSellStakeRequest(uint256 _stakeId) external {
