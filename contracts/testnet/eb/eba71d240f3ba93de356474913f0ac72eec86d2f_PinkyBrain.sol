...
contract PinkyBrain is Context, IERC20, Ownable {
    ...
    uint256 private totalFeesBuyandSell = _taxFee.add(_liquidityFee).add(_poolRewardsFee).add(_marketingFee);

    mapping(address => bool) _isBot;

    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    bool private enableAntiwhale = true;
    uint256 public _poolRewardsFee = 2;
    address public devWallet = 0xF963f145cd74C4c63EbD6AB4867aA846D99e8f66;
    address public marketingWallet = 0x09E626Eb98bCF39d17163C01B8F3c46135218491;
    address payable public liquidityWallet = payable(0xb31d2B1Cb48569a549CF65200292EFA8499C95ba);
    ...
    uint256 private _previousDevFee = _poolRewardsFee;
    uint256 public totalFeesTax = _taxFee.add(_liquidityFee).add(_poolRewardsFee).add(_marketingFee);
    ...
    uint256 public _poolRewardsFeeBuy = 2;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _poolRewardsFee ==0 && _marketingFee == 0) return;
        
        _previousTaxFee = _taxFee;
        _previousLiquidityFee = _liquidityFee;
        _previousDevFee = _poolRewardsFee;
    ...
    function restoreAllFee() private {
       ...
       _poolRewardsFee = _previousDevFee;
    ...
    function _transfer(
        ...
        if (
            ...
            if(_poolRewardsFee > 0) {
                uint256 tokensForPoolRewards = contractTokenBalance.mul(_poolRewardsFee).div(totalTaxForSwap);
                transfer(poolRewardsWallet, tokensForPoolRewards);
            }
    ...
    function setBotFees() private {
        ...
        _poolRewardsFee = _previousDevFee * _botIncreaseFee;
    ...
    function setBuyFees() private {
        ...
        _liquidityFee = 2;
        _poolRewardsFee = 2;
        totalFeesBuyandSell = _taxFee.add(_marketingFee).add(_liquidityFee).add(_poolRewardsFee);
    }
    ...
    returns (uint256, uint256)
    {
        ...
        uint256 tDev = tAmount.div(100).mul(_poolRewardsFee);
    ...
    function setDevFee(uint256 devFee) external onlyOwner() {
        _poolRewardsFee = devFee;
        totalFeesTax = _taxFee.add(_poolRewardsFee).add(_marketingFee).add(_liquidityFee);
