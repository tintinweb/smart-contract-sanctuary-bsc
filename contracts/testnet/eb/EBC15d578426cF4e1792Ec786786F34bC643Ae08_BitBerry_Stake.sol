...
contract BitBerry_Stake is Ownable {
    ...
    uint256 public minBBStakePrice = 500000000000000000000;
    uint256 public minLPStakePrice = 50000000000000000000;

    uint256 private BBLockTime = 3;   // 7 days=  10080
    uint256 private LPLockTime_ = 5;
    ...
    constructor()
    {
        Token = IERC20(0x1F2B9a0813B334b2dfba9f742EBb90bD2fDB2a74);
        LpToken = IERC20(0xea43eFa4Cd998370aC9f82DdF2fE3f8133985B5C);   
    }
    ///////////////////////////////////////////////////////////
    /////////////////    STRUCTURES     //////////////////////
    ...
    */
    ...
    require(_amount >= minBBStakePrice, "less amount");
    ...
    function BBPcalculator(address addr) public view returns(uint256){

    uint256 remainingenergy;
    // uint256 _timeSlot = ((block.timestamp).sub(User[addr].Deposit_time)).div(time);
    uint256 _timeSlot = BBRTimeCheck(addr);
    if(User[addr].Deposit_time!=0) {
        if(_timeSlot >= BBLockTime){
            _timeSlot = BBLockTime;
        }
       uint256 reward = (_timeSlot).mul(User[addr].Total_Amount.mul(BBR_Perecent));
    ...
    uint256 point=BBPcalculator(msg.sender);
    currentRP+=point;
    ...
    */
    ...
    bool isTrue = isBBRTimeCompleted(msg.sender);
    require(isTrue, "Time not completed");
    redeem();
    ...
    */
    ...
    function checkLPTime(address _user) internal view returns(uint256){
        uint256 time_ = (block.timestamp.sub(UserLP[_user].Deposit_time)).div(time);
        return time_;
    }

    function isLPTimeCompleted(address _user) public view returns(bool){
        uint256 timecheck = checkLPTime(_user);
        if(timecheck > LPLockTime_){     return true;    }
        else {  return false;    }
    }
    
    function BBPcalculatorforLP(address user) public view returns(uint256) {
    uint256 remainingenergy;
    uint256 time_ = checkLPTime(user);

    if(UserLP[user].Deposit_time!=0) {
        if(time_ >= LPLockTime_){
            time_ = LPLockTime_;
        }
        uint256 reward = (time_).mul((UserLP[user].Total_Amount.mul(Lppercent)));
        remainingenergy= (reward.div(1E18)).sub(UserLP[user].redeemedRP);    
    }
    return remainingenergy;
    }

    // The user will get his ROAD points after calling this function.
   
    function redeemforLp() public {

    uint256 point=BBPcalculatorforLP(msg.sender);
    currentRP+=point;
    ...
    */
    ...
    require(isLPTimeCompleted(msg.sender), "Time not completed!!!");
    redeemforLp();
    ...
    LPLockTime_ = _LPlocktime;
    }


    function setBBLockTime(uint256 _bbLockTime) public onlyOwner{
        BBLockTime = _bbLockTime;
    }
    ...
    function setLPstakePrice(uint256 _amount) public onlyOwner{
        minLPStakePrice = _amount;
    }
    function setBBRstakePrice(uint256 _amount) public onlyOwner{
        minBBStakePrice = _amount;
    }
