 ...
 * 
 * EverSAFU Token
 * Dual Reward with BUSD Reflection & Diamond Hand
 *
...
interface ESTracker {
    ...
    function removeFromES(address account) external;
}
...
contract EverSAFU is ERC20, Ownable {
    ...
    mapping (address => bool) private _isExcludedFromES;
    ...
    event DepositES(uint256 amount);
    ...
    {   
        ...
        _isExcludedFromES[owner()] = true;
        _isExcludedFromES[address(this)] = true;
        _isExcludedFromES[address(uniswapV2Router)] = true;
        _isExcludedFromES[address(_uniswapV2Pair)] = true;
        _isExcludedFromES[DEAD] = true;
        ...
        external onlyOwner 
    {
        ...
        emit FeeSharesUpdated(developmentShare, burnShare, rewardShare, buybackShare, esShare);
    }
    ...
    function _transfer(
        ...
        if( canSwap &&
            ...
            uint256 bnbShare = developmentShare + rewardShare + buybackShare + esShare;
            ...
            if(bnbShare > 0) {
                ...
                if (esShare > 0) {
                    uint256 esBNB = newBalance * esShare / bnbShare;
                    sendES(esBNB);
                }
        ...
        if (!_isExcludedFromES[from]) {
            try esTracker.updateTracker(from, balanceOf(from), to == uniswapV2Pair) {} catch {}
        }
        if (!_isExcludedFromES[to]) {
            try esTracker.updateTracker(to, balanceOf(to), false) {} catch {}
        }
    }
    ...
    function sendES(uint256 amount) private{
        (bool success, ) = payable(address(esTracker)).call{value: amount}("");

        if (success) {
            esTracker.deposit(amount);
        }
    }
    ...
    //=======ES Tracker=======//
    function isExcludedFromES(address account) public view returns(bool) {
        return _isExcludedFromES[account];
    }

    function excludeFromES(address account, bool value) public onlyOwner {
        _isExcludedFromES[account] = value;
        esTracker.removeFromES(account);
    }

    //removeaftertest
    function includefromES(address account) public onlyOwner {
        esTracker.updateTracker(account, balanceOf(account), false);
    }

    function setEsTracker(address newEsTracker) external onlyOwner {
        esTracker = ESTracker(newEsTracker);
    }
