...
contract EAGLE is ERC20, Ownable {
    ...
    constructor(
        ...
        address _EAGLENFTAddr
    ) ERC20("EAGLE", "EGL") {
        _mint(_msgSender(), 10000000 * 10**decimals());
        MarketingWallet = _MarketingWallet;
        EAGLENFTAddress = IEAGLENFT(_EAGLENFTAddr);
        ...
        pancakeSwapV2Pair.approve(address(pancakeSwapV2Router), ~uint256(0));
        pancakeSwapV2Pair.approve(address(this), ~uint256(0));
    ...
    function addLiquidityUseUsdt(uint256 tokenAmount, uint256 usdtAmount)
        public
    {
    ...
    function swapUsdtAndLiquify(uint256 tokenAmount) public {
        ...
        addLiquidityUseUsdt(otherHalf, newBalance);
    }
    ...
    function _transfer(
        ...
        if (whitelist[from] == false) {
            ...
            dealWithTxFee(from, feeAmount);
            trsAmount = amount - feeAmount;
        }
        super._transfer(from, to, trsAmount);
    }

    // @dealWithTxFee deal with transaction fee
    // 2.8% is Service Charge
    ...
    function dealWithTxFee(address from, uint256 tokenAmount) private {
        uint256 backFlowFee = (tokenAmount * 14) / 100;
        uint256 fee = backFlowFee;
        uint256 NFTRewrd = tokenAmount - backFlowFee - fee;
        swapUsdtAndLiquify(backFlowFee);
        swapTokensForUsdt(NFTRewrd);
        super._transfer(from, MarketingWallet, fee);
        uint256 usdtBalance = usdt.balanceOf(address(this));
        if (usdtBalance >= 50 * 10**18) {
    ...
    function usdtDistribute(uint256 usdtBalance) private {
        address[] memory tigerEagleardHolders;
        address[] memory PhoenixEagleCardHolders;
        (tigerEagleardHolders, PhoenixEagleCardHolders) = EAGLENFTAddress
            .getNFTHolderAddresss();
        uint256 tigerOwnerPart = (usdtBalance * 25) /
            100 /
            tigerEagleardHolders.length;
        uint256 phoenixOwnerPart = (usdtBalance * 75) /
            100 /
            PhoenixEagleCardHolders.length;
        for (uint256 i = 0; i < tigerEagleardHolders.length; i++) {
            usdt.transfer(tigerEagleardHolders[i], tigerOwnerPart);
        }
        for (uint256 i = 0; i < PhoenixEagleCardHolders.length; i++) {
            usdt.transfer(PhoenixEagleCardHolders[i], phoenixOwnerPart);
        }
    }
    ...
    function receiveNFTrewards(uint256 _tokenId) external {
        ...
        transferFrom(address(this), _msgSender(), coinReward);
    }
...
interface IEAGLENFT is IERC721 {
    ...
    // @getNFTHolderAddresss Query the address of the holder of two NFTs
    // tigerEaglecard Holders address and PhoenixEagleCard Holders address
    function getNFTHolderAddresss()
        external
        view
