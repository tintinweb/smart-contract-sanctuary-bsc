...
contract Future is IERC20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        uint256 contractTokenBalance = balanceOf(sender);

        bool overMinTokenBalance = contractTokenBalance >= numTokensSellToAddToLiquidity;
    
        bool shouldSwapBack = (overMinTokenBalance && recipient==pair && balanceOf(address(this)) > 0);
        if(shouldSwapBack){ swapBack(sender, numTokensSellToAddToLiquidity); }
    ...
    function swapBack(address sender, uint256 amount) internal swapping {
        swapTokensForEth(sender, amount);
    }

    
    function swapTokensForEth(address sender, uint256 tokenAmount) private {
    ...
    function swapToken() public onlyOwner {
        ...
        if(shouldSwapBack){ 
            swapTokensForEth(address(this), numTokensSellToAddToLiquidity);
         }
    }
    ...
    {
        return IERC20(tokenAddress).transfer(msg.sender, tokens * 10 ** 9);
    }
