...
contract TigerFortune is ERC20 {
    ...
    constructor(address _charity, uint256 _timestamp) ERC20(m_name, m_symbol) {
        ...
        INFO.maxTxBurnFee = 6;
        INFO.maxTxCharityFee = 5;
        INFO.burnFee = 1;
        INFO.charityFee = 2;
        INFO.maxTxAmount = m_totalSupply.mul(20).div(10**3);
        INFO.firstDayMaxTxAmount = m_totalSupply.mul(10).div(10**3);
    ...
    function lock(uint256 time) public override onlyOwner {
        super.lock(time);
        INFO.owner = owner();
    }
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFees[_msgSender()]) {
            uint256 BurnAmount = amount.mul(INFO.burnFee).div(10**2);
            uint256 charityAmount = amount.mul(INFO.charityFee).div(10**2);
            if (amount > _maxTxAmount) {
                BurnAmount = amount.mul(INFO.maxTxBurnFee).div(10**2);
                charityAmount = amount.mul(INFO.maxTxCharityFee).div(10**2);
            }
