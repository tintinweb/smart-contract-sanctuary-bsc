...
contract bBGenesis is ReentrancyGuard {
    ...
    uint256 private constant RewardAmount = 33e18;
    uint256 private constant RewardAmountx10 = 330e18;
    ...
    mapping(address => bool) private _userChoise;
    ...
    event AddToGenesis(address wallet, uint256 deposit);
    ...
    function setFreeze(bool _isFreezed) external onlyAdmin afterThis {
        if (!_isFreezed && claimIsFreezed) {
            FreezedSwapTime = block.timestamp + 1 days;
    ...
    function addToGenesis(address wallet, bool userChoise) external nonReentrant {
        ...
        // choise false = 10 BUSD
        // choise true = 100 BUSD
        uint256 deposit = !userChoise ? 10e18 : 100e18;
        ...
        unchecked {
            if (!isOnGenesis[wallet]) {
                ...
                if (userChoise) {
                    if (counting + 1 <= 100) {
                        _userChoise[wallet] = true;
                        GenesisTreeNFT.mintTree(wallet, 1);
                        finalbBRewards = RewardAmountx10;
                        counting += 1;
                    }
                    else {
                        finalbBRewards = RewardAmountx10;
    ...
    function claim(bytes32[] memory _proof) external nonReentrant {
        ...
        if (verifyProof(_root, _proof, keccak256(abi.encodePacked(sender)))) {
            ...
            if (_userChoise[sender])
                bBMirror.safeTransfer(sender, RewardAmountx10);
            else
                bBMirror.safeTransfer(sender, RewardAmount);
