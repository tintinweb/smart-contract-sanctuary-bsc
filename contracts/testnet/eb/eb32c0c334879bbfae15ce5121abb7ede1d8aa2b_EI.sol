...
contract EI is Context, IERC20, Ownable {
    ...
    function withdrawInvitation(address spender, uint256 amount) public returns (bool) {
        require(_allowances[invitationWallet][spender] >= amount, "ERC20: withdraw amount exceeds allowance");

        _allowances[invitationWallet][spender] -= amount;
        _balances[invitationWallet] -= amount;
        _balances[address(this)] += amount;
        uint256 bnbBefore = address(this).balance;
        swapTokensForEth(amount);
        uint256 bnbDiff = address(this).balance.sub(bnbBefore);
        payable(spender).transfer(bnbDiff);
