...
contract ERC721Mochi is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable, ERC721Burnable {
	...
	function autoUngroupTokenURI(string memory _baseURI, string[] memory _tokenURIs, string[] memory _tokenCoords, uint256 tokenId) public onlyMinter {
		address owner = ERC721.ownerOf(tokenId);
		
		grantRole(UNGROUP_ROLE, owner);
		_tokenBaseURI = _baseURI;		
		_tokenURIsToUngroup[tokenId] = _tokenURIs;
		_tokenCoordsToUngroup[tokenId] = _tokenCoords;
	}
	
	function autoUngroupTokenURIGroup(string memory _baseURI, string[] memory _tokenURIs, string[] memory _tokenCoords, string memory groupId) public onlyMinter {
		autoUngroupTokenURI(_baseURI, _tokenURIs, _tokenCoords, getTokenIdByCoordsOrGroup(groupId,0,0));
	}
	...
	function autoUngroupGroup(string memory groupId) public onlyUngroup {		
        autoUngroup(getTokenIdByCoordsOrGroup(groupId,0,0));
	}
    ...
    function autoMintCoords(string memory _tokenURI, address to, uint256 coord_x, uint256 coord_y) public onlyMinter {
        uint id;
    ...
    function autoMintGroup(string memory _tokenURI, address to, string memory groupId) public onlyMinter {
        uint id;
	...
	function burnCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public {
		uint256 tokenId = getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y); 
		
		burn(tokenId);
		
		delete _tokenGroup[groupId];
		delete _tokenCoords[string(abi.encodePacked(coord_x,'_',coord_y))];
	}
...
contract ERC721Suika is ERC721Mochi, ReentrancyGuard {
	...
	function canSellCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public view returns (bool) {	
        return canSell(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
    ...
    // Sell option for a fixed price
    function sellCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y, uint256 price, address payable wallet) public {
		sell(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y), price, wallet);
    }
	...
	// simple function to return the price of a tokenId
    // returns: sell price, bid price, sold price, only one can be non zero
    function getPriceCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public view returns (uint256, uint256, uint256) {
		return getPrice(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
	...
	function canBuyCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public view returns (uint256) {
		return canBuy(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
	...
	// Buy option
    function buyCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public nonReentrant {		
		buy(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
	...
	function canAuctionCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public view returns (bool) {
		return canAuction(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
	...
	// Instantiate an auction contract for a tokenId
    function createAuctionCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y, uint _closingTime, address payable _beneficiary, uint256 _reservePrice) public {		
		createAuction(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y), _closingTime, _beneficiary, _reservePrice);
    }
	
	// cancelation the auction for a tokenId
	function cancelAuction(uint256 tokenId) public {
	
	    require(ownerOf(tokenId)==msg.sender, "ERC721Suika: Only owner can cancel auction this item");
		require(auctions[tokenId].open, "ERC721Suika: The selected NFT has not opened auction");
	...
	// cancelation the auction for a tokenId
	function cancelAuctionCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public {		
	    cancelAuction(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
	}
	...
	function canBidCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public view returns (bool) {
        return canBid(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
    ...
    /// The value will only be refunded if the
    /// auction is not won.
    function bidCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y, uint256 bid_value) public nonReentrant {
		bid(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y), bid_value);
    }
	...
	/// Withdraw a bid when the auction is not finalized
    function withdrawCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public nonReentrant {		
        withdraw(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
	...
	// implement the auctionFinalize including the NFT transfer logic
    function auctionFinalizeCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public nonReentrant {		
		auctionFinalize(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
	...
	// Bid query functions
    function highestBidderCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public view returns (address payable) {		
        return highestBidder(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
    ...
    function highestBidCoordsOrGroup(string memory groupId, uint256 coord_x, uint256 coord_y) public view returns (uint256) {		
		return highestBid(getTokenIdByCoordsOrGroup(groupId, coord_x, coord_y));
    }
