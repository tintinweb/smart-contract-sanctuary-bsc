...
// File: contracts/AnotherDaoToken.sol

pragma solidity ^0.6.0;


contract AnotherDaoToken is BEP20('AnotherDao Token', 'ADO') {
...
// File: contracts/AnotherDaoSplitBar.sol
...
contract AnotherDaoSplitBar is BEP20('AnotherDaoSplitBar Token', 'WNTSPLIT') {
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterApe).
    function mint(address _to, uint256 _amount) public onlyOwner {
        _mint(_to, _amount);
        _moveDelegates(address(0), _delegates[_to], _amount);
    ...
    // The BANANA TOKEN!
    AnotherDaoToken public cake;


    constructor(
        AnotherDaoToken _cake
    ) public {
        cake = _cake;
    }
...
contract MasterAnotherDao is Ownable {
    using SafeMath for uint256;
    ...
    AnotherDaoSplitBar public syrup;
    // Dev address.
