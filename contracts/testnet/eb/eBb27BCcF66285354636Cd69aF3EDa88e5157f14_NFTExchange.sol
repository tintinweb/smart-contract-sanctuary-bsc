...
contract NFTExchange is Member{
    ...
    event ChangePrice(address indexed maker, uint256 indexed tokenid, uint256 newPrice);
    ...
    //创建订单
    function createOrder(uint256 tokenid, uint256 tradePrice, uint8 payment, bool canExchange) external {
        ...
        if(canExchange == false) {
            require(tradePrice == 2**256 - 1, "canExchange is false, but tradePrice too low");
        }
        ...
        makerOrders[tokenid].bidAmount = uint256(0);
    ...
    function changePrice(uint256 tokenid, uint256 newPrice) public returns(bool){
        ...
        require(makerOrders[tokenid].canExchange == true, "canExchange is false");
        makerOrders[tokenid].payAmount = newPrice;
        makerOrders[tokenid].createTime = block.timestamp;
        emit ChangePrice(msg.sender, tokenid, newPrice);
        return true;
    }
    ...
    function takeOrder(uint256 tokenid) external payable {
        ...
        //收取手续费
        uint256 tradeFee = payAmount.mul(funderFee).div(1000); // 項目方2% 
        uint256 stampFee = payAmount.mul(nftOwnerFee).div(1000); // 鑄造者2%
        uint256 nftStakeFee = payAmount.mul(nftStakePoolFee).div(1000); // 回流NFT質押池4.5%
        uint256 mcaStakeFee = payAmount.mul(mcaStakePoolFee).div(1000); // 回流MCA質押池1.5%
        uint256 sendAmount = payAmount.sub(tradeFee).sub(stampFee).sub(nftStakeFee).sub(mcaStakeFee);
        ...
        IERC20(token).safeTransferFrom(msg.sender, manager.members("NFTStakePool"), nftStakeFee);
        IERC20(token).safeTransferFrom(msg.sender, manager.members("MCAStakePool"), mcaStakeFee);
        ...
        removeOrder(order.maker, tokenid);
    ...
    function bid(uint256 tokenid, uint256 acutionAmount) public payable {
        ...
        //Rebid
        if (!(takerOrders[tokenid][user].taker == address(0))) {    
        ...
        if (acutionAmount > makerOrders[tokenid].bidAmount) {
            makerOrders[tokenid].highestBidder = msg.sender;
            makerOrders[tokenid].bidAmount = acutionAmount;
        }
   
        //Only first bid
        if(oldAuctionAmount == 0){
    ...
    function cancelBid(uint256 tokenid) public payable {
        ...
        uint256 backAmount = takerOrders[tokenid][user].auctionAmount;
        
        IERC20 token;
        token = mca;
        IERC20(token).transfer(user, backAmount);
        ...
        emit CancelBid(tokenid, backAmount, user);
    }
    ...
    function auctionEnd(uint256 tokenid, address taker) public payable{
        ...
        if (auctionOrdersArray[tokenid].length > 0) {
            TakerOrder memory takerOrder = takerOrders[tokenid][taker];

            emit AuctionEnded(taker, takerOrder.auctionAmount, tokenid, payment);
            IERC721(manager.members("nft")).transferFrom(address(this),taker,tokenid);

            INFT.starAttributesStruct memory nftAttr = INFT(manager.members("nft")).getStarAttributes(tokenid);

            //收取手续费
            uint256 tradeFee = takerOrder.auctionAmount.mul(funderFee).div(1000); // 項目方2% 
            uint256 stampFee = takerOrder.auctionAmount.mul(nftOwnerFee).div(1000); // 鑄造者2%
            uint256 nftStakeFee = takerOrder.auctionAmount.mul(nftStakePoolFee).div(1000); // 回流NFT質押池4.5%
            uint256 mcaStakeFee = takerOrder.auctionAmount.mul(mcaStakePoolFee).div(1000); // 回流MCA質押池1.5%
            uint256 sendAmount = takerOrder.auctionAmount.sub(tradeFee).sub(stampFee).sub(nftStakeFee).sub(mcaStakeFee);
            ...
            taker = makerOrders[tokenid].maker; // 回傳給用戶
        }
