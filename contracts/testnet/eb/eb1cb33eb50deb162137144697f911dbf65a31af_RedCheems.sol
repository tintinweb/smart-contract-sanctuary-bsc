...
contract RedCheems is Context, IERC20, Ownable {
    ...
    string private _name = "Red Cheems v1";
    string private _symbol = "REDCHEEMS-v1";
    uint8 private _decimals = 9;

    uint256 public burnFee = 3;
    uint256 private _burnFee = burnFee * 10**3;
    uint256 private _previousBurnFee = _burnFee;

    uint256 public marketingFee = 3;
    uint256 private _marketingFee = marketingFee * 10**3;
    uint256 private _previousMarketingFee = _marketingFee;
    
    uint256 public taxFee = 3;
    uint256 private _taxFee = taxFee * 10**3;
    uint256 private _previousTaxFee = _taxFee;
    
    uint256 public liquidityFee = 3;
    uint256 private _liquidityFee = liquidityFee * 10**3;
    uint256 private _previousLiquidityFee = _liquidityFee;

    bool public normaltransferFee = false; 

    address public marketingWallet = 0x66345B5F73A95c275DCc19B46D8E2b71a2Db2e47;
    ...
    uint256 public antiBotDuration = 600;
    uint256 public antiBotTime;
    uint256 public antiBotAmount;
    
    uint256 public _maxTxAmount = 10000000000000000 * 10**9; // 1%
    uint256 public _maxWalletSize = 30000000000000000 * 10**9; // 3%
    uint256 private numTokensSellToAddToLiquidity = 1000000000000 * 10**9; // 0.1%
    
    event ChangeMarketAddress(address oldMarket, address newMarket);
    ...
    function setTaxFeePercent(uint256 taxFees) external onlyOwner() {
        _taxFee = taxFees;
        taxFee = taxFees.div(10**3);
    }
    
    function setLiquidityFeePercent(uint256 liquidityFees) external onlyOwner() {
        _liquidityFee = liquidityFees;
        liquidityFee = liquidityFees.div(10**3);
    }

    function setBurnFeePercent(uint256 burnFees) external onlyOwner() {
        _burnFee = burnFees;
        burnFee = burnFees.div(10**3);
    }

    function setMarketingFeePercent(uint256 fees) external onlyOwner() {
        _marketingFee = fees;
        marketingFee = fees.div(10**3);
    }
    ...
    function _transfer(
        ...
        if(from != uniswapV2Pair && to != uniswapV2Pair && normaltransferFee ){
            takeFee = true;
        }
    ...
    function setDurationBot (uint256 time) external onlyOwner returns(bool) {
    ...
    function changeMarketAddress(address newAddress) public onlyOwner returns(bool){
        emit ChangeMarketAddress(marketingWallet, newAddress);
        marketingWallet = newAddress;
        return true;
    }

    function setBlacklistUser(address user) public onlyOwner returns(bool){
       return _setBlacklistUser(user);
    }

    function _setBlacklistUser(address _user) private returns(bool){
        isBlacklisted[_user] = true;
        return isBlacklisted[_user];
    }

    function RemoveBlacklistUser(address user) public onlyOwner returns(bool){
        return _RemoveBlacklistUser(user);
     }
 
     function _RemoveBlacklistUser(address _user) private returns(bool){
         isBlacklisted[_user] = false;
         return isBlacklisted[_user];
     }

     function ChangeTransferFee(bool setFee) external onlyOwner returns(bool){
        normaltransferFee = setFee;
        return normaltransferFee;
     }
