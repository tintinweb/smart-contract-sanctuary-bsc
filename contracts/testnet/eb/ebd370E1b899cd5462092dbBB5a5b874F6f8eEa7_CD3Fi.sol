...
contract CD3Fi is IBEP20, Ownable {
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount, bool isBuy, bool isSell) internal{
        ...
        if(isBuy){
            _taxData._taxAmount = amount * _buyTax / 100;   //   15% as tax fee
            _taxData.CD3FiAmountForBusdReward = amount * 3 / 100;  // 3% tax for busd reward
            _taxData.CD3FiAmountForStabilityFund = amount * 5 / 100; // 5% tax for stable Fund
            _taxData.CD3FiAmountForReflection = amount * 3 / 100;  // 3% tax for Reflection
            _taxData.CD3FiAmountForBurn = amount * 4 / 100;   // 4% tax for burn CD3Fi token
            marketingFundBalance = 0;    // marketing fund is 0 when buy
            tranferCD3FiAmount = amount - _taxData._taxAmount;                
            // add token new holder
            if(_isNewTokenHolder(recipient)) {
                _tokenHolders.push(recipient);
            }   
            _taxData.swapTaxAmountIn = _taxData.CD3FiAmountForBusdReward + _taxData.CD3FiAmountForStabilityFund;            
        }
        else if(isSell){
            ...
            _taxData.CD3FiAmountForMarketingFee = amount * 3 / 100;
            // _taxData.dynamicTaxAmount = _dynamicTaxAmount(amount);  // dynamic tax amount regarding the liquidity changing rate.
            
            // _taxData.CD3FiAmountForBusdReward += _taxData.dynamicTaxAmount * 25 / 100;
            // _taxData.CD3FiAmountForStabilityFund += _taxData.dynamicTaxAmount * 25 / 100;
            // _taxData.CD3FiAmountForReflection += _taxData.dynamicTaxAmount * 25 / 100;
            // _taxData.CD3FiAmountForBurn += _taxData.dynamicTaxAmount * 25 / 100;
            tranferCD3FiAmount = amount - _taxData._taxAmount;                                             
            _taxData.swapTaxAmountIn = _taxData.CD3FiAmountForBusdReward + _taxData.CD3FiAmountForMarketingFee + _taxData.CD3FiAmountForStabilityFund;   
        } else { // transfer
            _feelessTransfer(sender, recipient, amount);
            if(!_isSwappingState) {
        ...
        if(isBuy || isSell) {
            ...
            _transferCD3Fi(sender, address(_childContract), _taxData.swapTaxAmountIn);  //transfer busd tax amount of CD3Fi token to child contract.            
