...
abstract contract Referral is Initializable, OwnableUpgradeable {
    ...
    function LevelDecimals() external view returns (uint256) {
        return decimals;
    }
...
contract PresaleUpgradable is Referral, UUPSUpgradeable, PausableUpgradeable {
    ...
    {
        uint256 bnbPrice = GetBNB_USDPrice();
        uint256 BNBValue = (_bnbValue * bnbPrice) / (10**DECIMALS);
        tokenValue = BNBValue * PricePerUSDT;
        tokenValue = tokenValue / (10**DECIMALS);
    }
    ...
    function ChangeTokenContractAddressAdmin(address _address)
        external
        onlyOwner
    {
    ...
    function ChangeBNBPriceContractAddressAdmin(address _address)
        external
        onlyOwner
    {
    ...
    function ChangeTokenOwnerAdmin(address _address) external onlyOwner {
