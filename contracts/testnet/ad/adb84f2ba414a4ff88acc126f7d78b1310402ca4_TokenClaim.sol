...
contract TokenClaim is Ownable {
    ...
    function pauseVesting(uint256 phaseNo, bool value)
        internal
        checkPhaseNo(phaseNo)
    {
        paused[phaseNo] = value;
    }

    function pauseMultipleVesting(uint256[] memory phaseNo, bool[] memory value)
        external
        onlyOwner
    {
        require(phaseNo.length == value.length, "Length mismatch");
        for (uint256 i; i < phaseNo.length; i++) {
            pauseVesting(phaseNo[i], value[i]);
        }
    }
    ...
    function withdrawTokens(uint256 phaseNo) internal checkPhaseNo(phaseNo) {
        require(
            vestingDetail[phaseNo].phaseRewardBalance > 0,
            "Not enough tokens"
        );
        ERC20Interface.safeTransfer(
            msg.sender,
            vestingDetail[phaseNo].phaseRewardBalance
        );
        vestingDetail[phaseNo].phaseRewardBalance = 0;
    }

    function withdrawMultiplePhaseTokens(uint256[] memory phaseNo)
        external
        onlyOwner
    {
        for (uint256 i; i < phaseNo.length; i++) {
            withdrawTokens(phaseNo[i]);
        }
    }
