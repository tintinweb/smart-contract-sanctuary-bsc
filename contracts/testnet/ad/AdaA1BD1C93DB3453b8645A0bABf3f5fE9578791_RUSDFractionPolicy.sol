...
contract RUSDFractionPolicy is Ownable {
    ...
    function getpricedata() public view returns (uint256) {
        ...
        uint256 reserve0Decimal = numDigits(reserve0);
        uint256 reserve1Decimal = numDigits(reserve1);
        uint256 decimalDiff;
        ...
        exchangeRate = (uint256((10**DECIMALS * reserve0) / reserve1));
    ...
    function rebase() external onlyHandler {
        ...
        if (
            supplyDelta > 0 &&
            rusdFracs.totalSupply().add(uint256(supplyDelta)) > MAX_SUPPLY
        ) {
            supplyDelta = (MAX_SUPPLY.sub(rusdFracs.totalSupply()))
                .toInt256Safe();
        }
     ...
     */
    function globalReflectoEpochAndRUSDSupply()
        external
        view
     ...
     */
    function initialize(address owner_, IRUSDFraction rusdFracs_)
        public
        initializer
    {
