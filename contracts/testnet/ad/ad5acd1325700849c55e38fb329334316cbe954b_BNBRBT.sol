...
contract BNBRBT is IERC20, Ownable {
	...
	string private constant _name =  "BNB RABBIT BSC";
	string private constant _symbol = "BNBRBT";
	...
	uint256 private constant _tTotal = 10_000_000_000_000_000_000_000_000 * _decimals;
	...
	uint256 public minimumTokensBeforeSwap = 25000000 * _decimals;
	...
	// Base taxes
	CustomTaxPeriod private _base = CustomTaxPeriod('base',0,0,1,1,3,3,5,5);
	...
	event FeeChange(string indexed identifier, uint8 liquidityFee, uint8 marketingFee, uint8 holdersFee);
	...
	constructor() {
        ...
        marketingWallet = 0xA68e5f5d9079A1f5c1606d148cBd71638479e387;
    ...
    function setFeeOnSelectedWalletTransfers(address account, bool value) external onlyOwner {
		require(_feeOnSelectedWalletTransfers[account] != value, "BNBRBT: The selected wallet is already set to the value ");
		_feeOnSelectedWalletTransfers[account] = value;
		emit FeeOnSelectedWalletTransfersChange(account, value);
	}
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "BNBRBT: Automated market maker pair is already set to that value");
		automatedMarketMakerPairs[pair] = value;
		emit AutomatedMarketMakerPairChange(pair, value);
	}
    function blockAccount(address account) external onlyOwner {
		require(!_isBlocked[account], "BNBRBT: Account is already blocked");
		if (_launchStartTimestamp > 0) {
			require((block.timestamp - _launchStartTimestamp) < _blockedTimeLimit, "BNBRBT: Time to block accounts has expired");
		}
		_isBlocked[account] = true;
		emit BlockedAccountChange(account, true);
	}
	function unblockAccount(address account) external onlyOwner {
		require(_isBlocked[account], "BNBRBT: Account is not blcoked");
		_isBlocked[account] = false;
		emit BlockedAccountChange(account, false);
	}
	function excludeFromFees(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "BNBRBT: Account is already the value of 'excluded'");
		_isExcludedFromFee[account] = excluded;
		emit ExcludeFromFeesChange(account, excluded);
	}
	function excludeFromMaxTransactionLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "BNBRBT: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) external onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "BNBRBT: Account is already the value of 'excluded'");
		_isExcludedFromMaxWalletLimit[account] = excluded;
		emit ExcludeFromMaxWalletChange(account, excluded);
	}
	function excludeFromDividends(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromDividends[account] != excluded, "BNBRBT: Account is already the value of 'excluded'");
		if(excluded) {
	...
	function setWallets(address newLiquidityWallet, address newMarketingWallet) external onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "BNBRBT: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "BNBRBT: The marketingWallet cannot be 0");
			emit WalletChange('marketingWallet', newMarketingWallet, marketingWallet);
			marketingWallet = newMarketingWallet;
		}
	}
	function setBaseFeesOnBuy(uint8 _liquidityFeeOnBuy, uint8 _marketingFeeOnBuy, uint8 _holdersFeeOnBuy) external onlyOwner {
	...
	function setBaseFeesOnSell(uint8 _liquidityFeeOnSell,uint8 _marketingFeeOnSell, uint8 _holdersFeeOnSell) external onlyOwner {
	...
	function setUniswapRouter(address newAddress) external onlyOwner {
		require(newAddress != address(uniswapV2Router), "BNBRBT: The router already has that address");
		emit UniswapV2RouterChange(newAddress, address(uniswapV2Router));
		uniswapV2Router = IRouter(newAddress);
	}
	function setMaxTransactionAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxTxAmount, "BNBRBT: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) external onlyOwner {
		require(newValue != maxWalletAmount, "BNBRBT: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) external onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "BNBRBT: Cannot update minimumTokensBeforeSwap to same value");
		emit MinTokenAmountBeforeSwapChange(newValue, minimumTokensBeforeSwap);
		minimumTokensBeforeSwap = newValue;
	}
	function claimEthOverflow(uint256 amount) external onlyOwner {
		require(amount < address(this).balance, "BNBRBT: Cannot send more than contract balance");
	...
	function tokenFromReflection(uint256 rAmount) public view returns(uint256) {
		require(rAmount <= _rTotal, "BNBRBT: Amount must be less than total reflections");
		uint256 currentRate =  _getRate();
		return rAmount / currentRate;
	}
	function reflectionFromToken(uint256 tAmount, bool deductTransferFee) external view returns (uint256) {
		require(tAmount <= _tTotal, "BNBRBT: Amount must be less than supply");
		uint256 currentRate = _getRate();
	...
	uint256 amount
		...
		require(amount > 0, "BNBRBT: Transfer amount must be greater than zero");
		require(amount <= balanceOf(from), "BNBRBT: Cannot transfer more than balance");

		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "BNBRBT: Trading is currently disabled.");
            require(!_isBlocked[to], "BNBRBT: Account is blocked");
			require(!_isBlocked[from], "BNBRBT: Account is blocked");
			if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
				require(amount <= maxTxAmount, "BNBRBT: Buy amount exceeds the maxTxBuyAmount.");
			}
			if (!_isExcludedFromMaxWalletLimit[to]) {
				require((balanceOf(to) + amount) <= maxWalletAmount, "BNBRBT: Expected wallet amount exceeds the maxWalletAmount.");
			}
		}
	...
	function _getTValues(uint256 tAmount, bool takeFee) private view returns (uint256,uint256,uint256){
		if (!takeFee) {
			...
			uint256 tOther = tAmount * (_liquidityFee + _marketingFee) / 100;
	...
	function _swapAndLiquify() private {
        ...
        uint256 amountEthHolders = ethBalanceAfterSwap * holdersFeePrior / totalEthFee;
        ...
        if (amountEthHolders > 0) {
        _distributeBNBRewards(amountEthHolders);
        }
    ...
    function _distributeBNBRewards(uint256 ethAmount) private {
    uint256 totalRewards = ethAmount;
    uint256 remainingRewards = totalRewards;

    for (uint256 i = 0; i < _holders.length; i++) {
        address holder = _holders[i];
        uint256 holderReward = totalRewards * balanceOf(holder) / _tTotal;

        if (holderReward > 0 && remainingRewards >= holderReward) {
            Address.sendValue(payable(holder), holderReward);
            remainingRewards -= holderReward;
        } else {
            break;
        }
    }
