...
contract MGStake is Ownable{
    ...
    function deposit(address referrer,uint256 minerType) external returns (bool) {
        ...
        require(nextBuy[startBlock][_sender] == 0,"Invalid Block Num");
        nextBuy[startBlock][_sender]=nextBuy[startBlock][_sender].add(1);

        if(addressToId[_sender]>0){
            if(invites[_sender]!=referrer){
                revert("Invalid referrer");
            }
        }else{
            addressMaxId=addressMaxId.add(1);
            addressToId[_sender]=addressMaxId;
            idToAddress[addressMaxId]=_sender;
            invites[_sender]=referrer;
        }
        ...
        _dividend(_sender,referrer,payUsdt,payMc);
        _grant(_sender);
        powers[_sender]=powers[_sender].add(hashRate);
    ...
    //分成
    function _dividend(address _sender,address referrer,uint256 payUsdt,uint256 payMc) internal returns (bool){
        //10%MC销毁
        IERC20(MCToken).transferFrom(_sender,deadAddress,payMc);
        ...
        if(referrer!=address(0)){
            ...
            IERC20(USDT).transferFrom(_sender,referrer,referrerAmount);

            //间推
            if(_mg[0]!=address(0)){
                uint256 uParent2=payUsdt.mul(7).div(usdtRatio);
                payAmount=payAmount.sub(uParent2);
                IERC20(USDT).transferFrom(_sender,_mg[0],uParent2);
            }
            ...
            if(_mg[2]!=address(0)){
                ...
                IERC20(USDT).transferFrom(_sender,_mg[2],uSuper2);
            }else{
            ...
            if(_mg[1]!=address(0)){
                ...
                IERC20(USDT).transferFrom(_sender,_mg[1],uSuper1);
            }
        ...
        IERC20(USDT).transferFrom(_sender,bufferAddress,u2);

        //基金
        IERC20(USDT).transferFrom(_sender,fundAddress,payAmount);
