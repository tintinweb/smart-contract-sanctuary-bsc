...
contract MinerPublicBuilding is Distributor, Ownable, IMinerPublicBuilding {
    ...
    function mintMiner() external payable returns(uint tokenId){
        ...
        minerNFTContract.mintNFTTo(msg.sender);
    }
    ...
    function assignMiner(
        ...
        require(
            msg.sender == minerNFTContract.ownerOf(_minerTokenId),
            "BrokerPublicBuilding: You are not the owner of this NFT"
        );
        require(
            !minerNFTContract.isLocked(_minerTokenId),
            "BrokerPublicBuilding: NFT is already assigned"
        );

        _addNFTStaking(msg.sender);
        minerNFTContract.changeTokenLocking(_minerTokenId, true);
        standardParcelContract.assignNFT(
            IStandardParcel.ActivatableNFTs.Miner, 
            _x, _y, 
            _minerTokenId
        );
        miningContract.deposit(0, address(this), _minerTokenId, 1000, msg.sender);
    }
    ...
    function unassignNFT(
        ...
        minerNFTContract.changeTokenLocking(_minerTokenId, false);
        standardParcelContract.unassignNFT(
            IStandardParcel.ActivatableNFTs.Aesthetic, 
            _x, _y, 
            _minerTokenId
        );
        miningContract.withdraw(0, address(this), _minerTokenId, msg.sender);
    }
    ...
    function setStandardParcelContract(IStandardParcel _standardParcelContract)
        external
        onlyOwner
    {
        standardParcelContract = _standardParcelContract;
    }
}
