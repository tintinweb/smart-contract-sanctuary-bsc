...
interface IERC20 {
    ...
    function treasuryRewards(address _account) external view returns (uint256);
...
contract Richdotcoin is Context, IERC20, Ownable {
    ...
    modifier updateReward(address _account) {
        ...
        remainingtreasuryAmount[msg.sender] = _treasuryAmount[msg.sender] - _treasuryAmount[msg.sender]*(block.timestamp/finishAt[msg.sender]);

        _;
    }
    function stakeTreasury(uint256 _amount) external updateReward(msg.sender) {
        require(_amount > 0, "amount = 0");
        uint256 remainingtreasury = remainingtreasuryAmount[msg.sender];
        uint256 _treasuryClaim = treasuryClaim[msg.sender];
        _transfer(msg.sender, address(this), _amount*10**18);
        _treasuryAmount[msg.sender] = _amount + remainingtreasury;
        _treasuryWithdrawn[msg.sender] = 0;
        treasuryClaim[msg.sender] = _treasuryClaim + _treasuryAmount[msg.sender]*treasuryRATE/100;
        finishAt[msg.sender] = block.timestamp + duration;
        startAt[msg.sender] = block.timestamp;
    }

    function treasuryRewards(address _account) public view override returns (uint256) {
        uint256 rewardearned = _treasuryAmount[_account]*(10**18)*treasuryRATE/(100*duration) * (block.timestamp - startAt[_account]) - _treasuryWithdrawn[_account]*10**18;
        if (block.timestamp <= finishAt[_account]) return rewardearned;
        return treasuryClaim[_account]*10**18;
    }
