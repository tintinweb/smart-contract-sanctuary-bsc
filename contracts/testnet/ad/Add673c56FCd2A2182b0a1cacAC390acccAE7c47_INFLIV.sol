...
contract INFLIV is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, UUPSUpgradeable {
	...
	function _updatePools() public {
	  if(block.timestamp >= (teamPool.lastClaimTime + teamPool.claimInterval) && teamAddress != address(0)) 
	  {
	  ...
	  if(block.timestamp >= (communityPool.lastClaimTime + communityPool.claimInterval) && stakingAddress != address(0) && farmAddress != address(0)) 
	  {
	   ...
	   if(availableInInsurance > 0 && insuranceAddress != address(0))
	   {
	   ...
	   if(availableInBounty > 0 && bountyAddress != address(0))
	   { 
