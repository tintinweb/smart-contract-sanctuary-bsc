...
contract Tits is Context, IERC20, Ownable {
    ...
    uint256 private startingSupply = 2;
    ...
    address private _routerAddress = 0xdc4904b5f716Ff30d8495e35dC99c109bb5eCf81;
    address public burnAddress = 0x000000000000000000000000000000000000dEaD;
    address payable private _marketingWallet =
        payable(0x9C787a7aE9BAC83b6Ada133Dc934EeB4B8B282fd);
    ...
    uint256 public maxWalletAmountUI =
    ...
    constructor() {
        ...
        lpPair = IUniswapV2Factory(_dexRouter.factory()).createPair(
            address(this),
            _dexRouter.WETH()
        );
    ...
    function setTaxes(
        ...
        require(
            reflectFee + liquidityFee + marketingFee + buyBackFee + devFee <=
                maximumTaxesPercent
        );
    ...
    function setBuyBackAmount(uint256 amount, uint256 multiplier)
        external
        onlyOwner
    {
    ...
    function setBoostedTaxesEnabled(bool enabled) external onlyOwner {
        if (boostedTaxes != enabled) boostedTaxes = enabled;
    }

    function setBlacklistAddress(address account, bool enabled)
        external
        onlyOwner
    {
    ...
    function _transfer(
        ...
        if (_hasLimits(from, to) && to != _routerAddress && to != lpPair) {
        ...
        if (!inSwapAndLiquify && to == lpPair && swapAndLiquifyEnabled) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 totalBNBFees = _marketingFee
            ...
            .add(_buyBackFee)
            .add(_devFee);
        if (totalBNBFees == 0) return;
        uint256 toMarketing = contractTokenBalance.mul(_marketingFee).div(
            totalBNBFees
        );
        uint256 toBuyBack = contractTokenBalance.mul(_buyBackFee).div(
            totalBNBFees
        );
        uint256 toDev = contractTokenBalance.mul(_devFee).div(totalBNBFees);
        uint256 toLiquify = contractTokenBalance
            .sub(toMarketing)
            .sub(toBuyBack)
            .sub(toDev);
        ...
        uint256 marketingBalance = fromSwap.sub(
            liquidityBalance.add(devBalance).add(buyBackBalance)
        );
    ...
    function transferEthOut(uint256 devBalance, uint256 marketingBalance)
        internal
    {
    ...
    function swapETHForTokens(uint256 amount) private {
        ...
        dexRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{
            value: amount
        }(0, path, burnAddress, block.timestamp.add(300));
    ...
    function _tokenTransfer(
        ...
        if (!takeFee || boosted) restoreAllFee();
    }
    ...
    function _finalizeTransfer(
        ...
        if (rFee > 0 || tFee > 0) _takeReflect(rFee, tFee);
        ...
        returns (uint256)
    {
        return
            _amount
                .mul(
                    _liquidityFee.add(_buyBackFee).add(_marketingFee).add(
                        _devFee
                    )
                )
                .div(masterTaxDivisor);
    }
