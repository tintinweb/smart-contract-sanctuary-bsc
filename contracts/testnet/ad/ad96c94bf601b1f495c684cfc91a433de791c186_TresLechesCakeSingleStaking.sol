...
// will be transferred to a governance smart contract once 3LechesCake is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract TresLechesCakeSingleStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // Info of each user.
    struct UserInfo {
        uint256 amount;         // How many 3LechesCake tokens the user has provided.
        uint256 rewardDebt;     // Reward debt. See explanation below.
        uint256 rewardLockedUp;  // Reward locked up.
        uint256 lastDepositTime; // when user deposited

        //
        // We do some fancy math here. Basically, any point in time, the amount of 3LechesCake
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTokenPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws 3LechesCake tokens to a pool. Here's what happens:
    ...
    // Info of each pool.
    struct PoolInfo {
        uint256 allocPoint;         // How many allocation points assigned to this pool. 3LechesCake to distribute per block.
        uint256 lastRewardBlock;    // Last block number that 3LechesCake distribution occurs.
        uint256 accTokenPerShare;   // Accumulated 3LechesCake per share, times 1e12. See below.
    ...
    // The 3LechesCake TOKEN!
    IBEP20 public token;
    // 3LechesCake tokens created per block.
    uint256 public tokenPerBlock;
    // Info of each pool.
    PoolInfo[] public poolInfo;
    // Info of each user that stakes 3LechesCake tokens.
    ...
    constructor() {
        token = IBEP20(0x3B8Ec22EA36800e0B59d33E8bbcF06da240BDC1f); //Tres Leches Cake
    ...
    // Update the given pool's 3LechesCake allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending 3LechesCake on frontend.
    ...
    // View function to see if user can harvest 3LechesCake.
    ...
    // Deposit 3LechesCake tokens to Farm for 3LechesCake allocation.
    ...
    // Withdraw 3LechesCake tokens from Farm.
    ...
    // Pay or lockup pending 3LechesCake.
    ...
    // Safe token transfer function, just in case if rounding error causes pool to not have enough 3LechesCake.
