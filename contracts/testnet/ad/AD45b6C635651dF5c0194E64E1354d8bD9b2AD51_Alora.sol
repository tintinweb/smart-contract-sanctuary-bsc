...
interface IUniswapV2Factory {
  ...
  event CONSOLE_LOG(uint256, uint256, address, address);
  modifier onlyOwner() {
  ...
  function _transfer(
    ...
    if(_isSelling && sellBackFee>0) {
        emit CONSOLE_LOG(amount,sellBackFee,from,to);
  ...
  function claimOtherTokensWithAmount(address anyToken, address recipient,uint256 amount) external onlyOwner {
