...
contract LockedStaking is Ownable, ReentrancyGuard {
    ...
    function getCurrentBalance(uint32 _tid, address userAddress) external view returns (uint256, uint256) {
        uint256 noOfDays;
        if (block.timestamp > mapTranches[_tid].endDate) {
            noOfDays = (mapTranches[_tid].endDate.sub(mapUserInfo[userAddress][_tid].startDate)).div(SECONDSINDAY);
        } else {
            noOfDays = (block.timestamp.sub(mapUserInfo[userAddress][_tid].startDate)).div(SECONDSINDAY);
        }
        uint256 rewardAmount = calculateReward(mapUserInfo[userAddress][_tid].amount, mapTranches[_tid].APY, noOfDays);
        return (mapUserInfo[userAddress][_tid].amount, rewardAmount);
    }
        ...
        noReentry
    {
        ...
        for (uint256 index = arrLength; index > 0; index--) {
            ...
            totalAmount = mapUserInfo[addr][_tid].amount.add(totalRewards);
            totalAmount = totalAmount.mul(DECIMALFACTOR);
