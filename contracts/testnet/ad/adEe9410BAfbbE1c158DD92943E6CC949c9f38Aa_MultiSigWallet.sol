...
contract MultiSigWallet is Initializable, UUPSUpgradeable {
    ...
    function updateOwnerDetails(address[] memory _owners) public onlyOwner {
        require(_owners.length > 0, "owners required");
        for (uint i = 0; i < _owners.length; i++) {
            address owner = _owners[i];
            require(owner != address(0), "invalid owner");
    ...
    function updateConfirmationsRequired(uint _numConfirmationsRequired) public onlyOwner {
