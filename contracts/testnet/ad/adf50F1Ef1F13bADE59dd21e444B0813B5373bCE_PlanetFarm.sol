...
contract PlanetFarm is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function updatePool(uint256 _pid) public{
        ...
        if(pool.totalFactor != 0){
            pool.accGAMMAPerFactorPerShare += ((GAMMAReward*1e12*pool.gammaRewardBoostPercentage)/(pool.totalFactor*10_000));
        }
        //Common gamma to distributed to everyone else
        pool.accGAMMAPerShare += ((GAMMAReward*1e12*(10_000 - pool.gammaRewardBoostPercentage))/(sharesTotal*10_000));
    }
    ...
    function deposit(uint256 _pid, uint256 _wantAmt) external nonReentrant {
        ...
        uint256 iTokens;
        if(pool.isInfinity){
            iTokens = IERC20(pool.iToken).balanceOf(_msgSender());
            userShares = iTokens - user.iTokenToBeUnstaked;
        }  

        usersClaimableGamma[_msgSender()] += ((userShares*pool.accGAMMAPerShare + userFactor * pool.accGAMMAPerFactorPerShare)/1e12 - user.rewardDebt);
        ...
        if (_wantAmt != 0) {
            ...
            pool.want.safeTransferFrom(address(_msgSender()), pool.strat,_wantAmt);
        ...
        user.rewardDebt = (userShares * pool.accGAMMAPerShare + userFactor * pool.accGAMMAPerFactorPerShare)/1e12;
    ...
    function startUnstakeProcess(uint256 _pid, uint256 unstakeAmount) external nonReentrant{
        ...
        if(_pid != iGammaPid){
        usersClaimableGamma[_msgSender()] += (((useriTokenBalance - iTokenAlreadyGivenForUnstake) * pool.accGAMMAPerShare)/1e12 - user.rewardDebt);
        }
        ...
        if(_pid != iGammaPid) {
            user.rewardDebt = (((useriTokenBalance - iTokenAlreadyGivenForUnstake) * pool.accGAMMAPerShare) / (1e12));
    ...
    function unstakeInstantly(uint256 _pid, uint256 unstakeAmount ) external nonReentrant{
        ...
        uint256 useriTokenCanBeInstantlyUnstaked = useriTokenBalance - useriTokenToBeUnstaked;

        if(_pid != iGammaPid){
            usersClaimableGamma[_msgSender()] += ((useriTokenCanBeInstantlyUnstaked*pool.accGAMMAPerShare)/1e12 - user.rewardDebt);
        }
        ...
        uint256 underlyingEquivalentOfUsersBal = (useriTokenCanBeInstantlyUnstaked*exchange_rate_before_withdraw*gToken_exchange_rate)/1e36;
    ...
    function unstakeAfterMinWithdrawTime(uint256 _pid) external nonReentrant{        
        ...
        if(_pid != iGammaPid){
            usersClaimableGamma[_msgSender()] = usersClaimableGamma[_msgSender()] + ((useriTokenBalance - totalItokensUnstaking) * pool.accGAMMAPerShare)/1e12 - user.rewardDebt;
        }
        ...
        uint256 lostRewards = ((itokens * pool.accGAMMAPerShare) / (1e12)) - ((unstakingRewardDebt* itokens)/totalItokensUnstaking);
        if(_pid == iGammaPid) {
            IERC20(GAMMA).safeTransfer(strat, lostRewards);
            IStrategy(strat).earnGammaProfits();
            ...
            pool.accGAMMAPerShare += lostRewards;
            user.rewardDebt = ((useriTokenBalance - (totalItokensUnstaking - itokens)) * pool.accGAMMAPerShare)/1e12;
