...
contract Ra8Bits is Context, Ownable, ERC1155Holder {
    ...
    constructor(address _transferAddress) {
        ...
        discountTierList[0].breedingTime = 10 minutes;
    ...
    function rejectSuperLike(
        ...
        require(IERC20(ratBitTokens).transfer(fromOwnerAddress,  isSuperliked[superLikeNFTId]));
        isSuperliked[superLikeNFTId] = 0;
        emit SuperLikeResponse(
            fromNftId,
            fromOwnerAddress,
    ...
    function claimPendingTokensFromSuperLike(
        ...
        require(IERC20(ratBitTokens).transfer(_msgSender(),isSuperliked[superLikeNFTId]));
        isSuperliked[superLikeNFTId] = 0;
    ...
    function sendToBreedingRoomFirst(
        ...
        require((IERC1155(NFTAddress).balanceOf(_msgSender(), parentA) > 0),"caller is not owner of NFT");
        if(finalCost>0)
        {
        ...
        emit BreedParentA(parentA, parentB, breedId, _msgSender(),NFTAddress,block.timestamp);
    ...
    function isBreedingStart(uint256 _breedId) public view returns (bool) {
        if (breedInfoList[_breedId].ownerOfParentB == address(0) ) 
        return false;
        else
         return true;
    }
    ...
    function withdrawalNft(uint256 _breedId) public {
        ...
        {
            require(IERC20(ratBitTokens).transferFrom(address(this),_msgSender(),details.costParentA));
        }
    ...
    function claimNft(uint256 _breedId) public {
        ...
        if (details.ownerOfParentA == _msgSender()) {
            ...
            if (details.costParentA == 0) {
                ...
                {
                  require(IERC20(ratBitTokens).transferFrom(address(this),_msgSender(),details.costParentB.div(50)));
                }
            }
            else{
            ...
            if (details.costParentB == 0) {
                require(IERC20(ratBitTokens).transferFrom(address(this),_msgSender(),details.costParentA.div(50)));
            }
            else
            {
                IERC1155(Gen1NFTAddress).safeTransferFrom(address(this),_msgSender(),Gen1NFTId,1,"0x");
            ...
            emit ClaimNft(_breedId,details.parentANFTAddress,_msgSender(), details.parentB,block.timestamp);
        }
    }
