...
contract PandoPotV3 is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public priceMegaTicket = 10000;//USD 10000 = 1USD = ONE_HUNDRED_PERCENT
    uint256 public priceMinorTicket = 5000;//USD 10000 = 1USD = ONE_HUNDRED_PERCENT
    ...
    //Only for Test
    //user => roundID
    mapping (address => mapping (uint256 => uint[])) public userMegaTickets;
    mapping (address => mapping (uint256 => uint[])) public userMinorTickets;
    ...
    function _calcPrizeOfRoundCompleted(uint256 _id) internal returns(uint256 _expire, uint256 _megaUSD, uint256 _megaPSR, uint256 _megaPAN, uint256 _minorUSD1, uint256 _minorPSR1, uint256 _minorPAN1, uint256 _minorUSD2, uint256 _minorPSR2, uint256 _minorPAN2) {
        RoundInfo storage _roundInfo = roundInfo[_id];
        _expire = _roundInfo.finishedAt + prizeExpireTime;
        (_megaUSD, _megaPSR, _megaPAN) = _calcMegaPrize(_id, currentMegaNumber, _expire);
        (_minorUSD1, _minorPSR1, _minorPAN1) = _calcMinorPrize(_id, currentMinorNumber1, _expire);
        (_minorUSD2, _minorPSR2, _minorPAN2) = _calcMinorPrize(_id, currentMinorNumber2, _expire);

        uint256 _totalUSD = _megaUSD + (_minorUSD1 > 0 ? _minorUSD1 : _minorUSD2);
        uint256 _totalPAN = _megaPAN + (_minorPAN1 > 0 ? _minorPAN1 : _minorPAN2);
    ...
    function _updateRound(uint256 _id) internal{
        ...
        if (_roundInfo.status == 0) {
            ...
            //Fix: CompilerError: Stack too deep, try removing local variables.
            (uint256 _expire, uint256 _megaUSD, uint256 _megaPSR,uint256 _megaPAN, uint256 _minorUSD1, uint256 _minorPSR1, uint256 _minorPAN1, uint256 _minorUSD2, uint256 _minorPSR2, uint256 _minorPAN2) = _calcPrizeOfRoundCompleted(_id);
            emit RoundCompleted(_id, _expire, currentMegaNumber, currentMinorNumber1, currentMinorNumber2, _megaUSD, _megaPSR, _megaPAN, _minorUSD1, _minorPSR1, _minorPAN1, _minorUSD2, _minorPSR2, _minorPAN2);
        }
    }
    ...
    function _calcMegaPrize(uint256 _roundId, uint256 _megaNumber, uint256 _expire) internal returns(uint256, uint256, uint256) {
        ...
        //Rooney: Save price jackpot if winner
//        if (_prize.totalWinner > 0) {}
        ...
        _prize.PAN = PANForCurrentPot * megaPrizePercentage / ONE_HUNDRED_PERCENT;
        lastDistribute = _expire - prizeExpireTime;
        megaPrize[_roundId] = _prize;
        return (_prize.USD, _prize.PSR, _prize.PAN);
    }

    function _calcMinorPrize(uint256 _roundId, uint256 _minorNumber, uint256 _expire) internal returns(uint256, uint256, uint256) {
        PrizeInfo storage _prize = minorPrize[_roundId];
        uint256 _totalWinner = nMinorTickets[_roundId][_minorNumber];
        //Rooney: Save price jackpot if winner
//        if ( _totalWinner > 0) {}

        if (_prize.USD == 0 || _prize.PAN == 0 || _prize.PSR == 0) {
            ...
            _prize.PAN = PANForCurrentPot * minorPrizePercentage / ONE_HUNDRED_PERCENT;
    ...
    function _liquidate(uint256 _type, uint256 _roundId, address _owner) internal {
        ...
        if (_type == 0 || _type == 1) {//Mega or Minor
            ...
            if (_megaPrize.totalWinner > _megaPrize.nClaimed) {
                ...
                _totalPAN = _megaPrize.PAN * (_megaPrize.totalWinner - _megaPrize.nClaimed) / _megaPrize.totalWinner;
            ...
            if (_minorPrize.totalWinner > _minorPrize.nClaimed) {
                ...
                _totalPAN += _minorPrize.PAN * (_minorPrize.totalWinner - _minorPrize.nClaimed) / _minorPrize.totalWinner;
    ...
    function buyTicket(address _receiver, uint256 _quantityMega, uint256 _quantityMinor) external onlyEOA whenNotPaused nonReentrant{
        require(_receiver != address(0),"PandoPot: Receiver must different address zero");
        require(_quantityMega > 0 || _quantityMinor > 0 ,"PandoPot: Quantity ticket buy must more than zero");

        uint256 _priceMega = priceMegaTicket * _quantityMega / ONE_HUNDRED_PERCENT;
        uint256 _priceMinor = priceMinorTicket * _quantityMinor / ONE_HUNDRED_PERCENT;
        if(_priceMega + _priceMinor > 0){
            ...
            uint256 _amount_PAN = (_priceMega + _priceMinor) * _price_USD / _price_PAN;

            _transferToken(PAN,address(this), _amount_PAN);
            ERC20Burnable(address(PAN)).burnFrom(address(this), _amount_PAN/2);
        }

        uint256 _currentRoundId = currentRoundId;
        uint256 _nSeeds = seeds.length;
        uint256 _salt = block.timestamp;
        uint256 _rand = _computerSeed();

        if(_quantityMega > 0){
            uint256[] memory _megaTickets = new uint[](_quantityMega);
            uint256 _megaSpace = megaSampleSpace;
            for (uint256 i = 0; i < _quantityMega; i++) {
                uint256 _megaTicket = _generateTicket(_rand, _megaSpace, _nSeeds);

                userMegaTickets[_receiver][_currentRoundId].push(_megaTicket);
            ...
            emit NewBuyMegaTicket(_currentRoundId, _receiver, _megaTickets);
        }

        if (_quantityMinor > 0){
            uint256[] memory _minorTickets = new uint[](_quantityMinor);
            uint256 _minorSpace = minorSampleSpace;
            for (uint256 i = 0; i < _quantityMinor; i++) {
                uint256 _minorTicket = _generateTicket(_rand, _minorSpace, _nSeeds);

                userMinorTickets[_receiver][_currentRoundId].push(_minorTicket);
            ...
            emit NewBuyMinorTicket(_currentRoundId, _receiver, _minorTickets);
        }
    }

    function enter(address _receiver, uint256 _rand, uint256 _quantity) public whenNotPaused nonReentrant onlyWhitelist() {
        uint256 _megaTicket;
    ...
    function claim(uint256 _type, uint256 _roundId, uint256 _ticketNumber, address _receiver) external whenNotPaused nonReentrant {
        ...
        if (_type != 2) {//Not Leaderboard
            ...
            } else if (_type == 1) {//For Minor JackPot
        ...
        if (currentRoundId > 1) {
            ...
            if (roundInfo[_preRound].status == 0) {
                if(nMegaTickets[_preRound][currentMegaNumber] > 0){
                    ...
                    _PAN -= PANForCurrentPot * megaPrizePercentage / ONE_HUNDRED_PERCENT;
                ...
                if (nMinorTickets[_preRound][currentMinorNumber1] > 0 || nMinorTickets[_preRound][currentMinorNumber2] > 0) {
                    ...
                    _PAN -= PANForCurrentPot * minorPrizePercentage / ONE_HUNDRED_PERCENT;
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "Jackpot: must use EOA");
        _;
    }
    ...
    function changePriceTicket(uint256 _newPriceMega, uint256 _newPriceMinor) external onlyOwner {
        uint256 _oldPriceMega = priceMegaTicket;
        uint256 _oldPriceMinor = priceMinorTicket;
        priceMegaTicket = _newPriceMega;
        priceMinorTicket = _newPriceMinor;
        emit PriceTicketChanged(_oldPriceMega, _oldPriceMinor, priceMegaTicket, priceMinorTicket);
    }
    ...
    event NewBuyMegaTicket(uint256 roundId, address user, uint256[] numbers);
    event NewBuyMinorTicket(uint256 roundId, address user, uint256[] numbers);

    event Claimed(uint256 _type, uint256 roundId, uint256 ticketNumber, uint256 USD, uint256 PSR, uint256 PAN, address receiver);
    event Liquidated(uint256 _type, uint256 id, address owner, uint256 USD, uint256 PSR, uint256 PAN);
    ...
    event RoundCompleted(uint256 roundId, uint256 expireTime, uint256 megaNumber, uint256 minorNumber1, uint256 minorNumber2, uint256 megaUSD, uint256 megaPSR,uint256 megaPAN, uint256 minorUSD1, uint256 minorPSR1, uint256 minorPAN1, uint256 minorUSD2, uint256 minorPSR2, uint256 minorPAN2);
    ...
    event PriceTicketChanged(uint256 oldPriceMega, uint256 oldPriceMinor, uint256 newPriceMega, uint256 newPriceMinor);
}