...
contract KnightQuery is KnightReward {
    ...
    constructor(IKnightItems _contractItem, IKnightToken _knighToken, IMinter _minter, IMAL _mal) {
        Item = _contractItem; 
    ...
    {
        Knight.Info[]  memory listKnight = knightOfOwner[_owner];
        for(uint i = 0; i < listKnight.length; i++){
            if(infoOfKnight[listKnight[i].id].owner == _owner) {
                knights[i] = infoOfKnight[listKnight[i].id];
            }
        }
        return knights;
    }
    ...
    function getPowerKnight(uint _knightId) external view onlyRole(OPERATOR_ROLE) returns(Knight.Power memory) {
        return powerOfKnight[_knightId];
    }

    function getEquipment(uint _idEquipment) external view onlyRole(OPERATOR_ROLE) returns(Knight.Equipment memory) {
...
contract KnightReward is KnightItems {
    ...
    {
            uint equipId = IKnightToken(KnightToken).getCurrentId();
            ...
            IKnightToken(KnightToken).safeMintEquipment(_owner, rarity);
            myEquipment.image =  IKnightToken(KnightToken).getImgToken(equipId);
...
contract KnightItems is KnightMission {
    ...
    uint priceLv1 = 500 * 10**18;
    uint priceLv2 = 1000 * 10**18;
    uint priceLv3 = 1500 * 10**18;
        ...
        external  
    {
        ...
        require(_deposit / _amount == priceLv1 || _deposit / _amount == priceLv2 || _deposit / _amount == priceLv3, "insufficient supply of token");
        ...
        internal returns (uint[]  memory ) 
    {
        ...
        for (uint8 i = 0; i < _amount; i++) {
            ...
            IKnightItems(Item).mintByERC721(_owner, ELIXIR, 1, "");
        ...
        external  
    {
        ...
        require(_deposit / _amount == priceLv1 || 
                _deposit / _amount == priceLv2 || 
                _deposit / _amount == priceLv3 ||
                _deposit / _amount == priceLv1 * 4 ||
                _deposit / _amount == priceLv1 * 5 ||
                _deposit / _amount == priceLv1 * 6 ||
                _deposit / _amount == priceLv1 * 7, "insufficient supply of STI Coin");
        IMAL(MAL).burnByOperator(_msgSender(), _deposit);
...
contract KnightMission is KnightFactory , ERC721Holder{
    ...
    function initMission() public onlyRole(OPERATOR_ROLE) {
        ...
        equipmentList[IKnightToken(KnightToken).getCurrentId()] = equipmentO;
        limitKnight[address(this)] = 10000;
        IKnightToken(KnightToken).safeMintEquipment(address(this), 0);
        for (uint32 i = 1; i <= 5 ; i++) {
            ...
            IKnightToken(KnightToken).safeMintEquipment(address(this), i - 1);
            equipment.image = IKnightToken(KnightToken).getImgToken(EquipId);
...
contract KnightFactory is AccessControl {
    ...
    {
        require(countKnight[_owner] < limitKnight[_owner], "You've run out of space for knights");
        ...
        _info.attackTime = uint32(block.timestamp + coolDownTime);
        _info.sexTime = uint32(block.timestamp + coolDownSex);
        _info.id = _knightId;
