...
contract Token is ERC20, PauseOwners {
	...
	function addLiquidity(uint256 amountToken) external payable onlyOwners {
		...
		transferFrom(msg.sender, address(this), amountTokenDecimals);
	...
	function swapAndTransferFees(
		...
		if (balance > 0) {
			uint256 devCut = (balance * developmentFee * totalFee) / 100;
			uint256 marCut = (balance * marketingFee * totalFee) / 100;
	...
	function setRouter(address routerAddress) external onlyOwners {
		...
		isExcludedFromTax[routerAddress];
	}
}
