...
contract Launchpad {
  ...
  function cancelAuctionBeforeStart(uint256 _pid) external onlyOperator {
    require(address(PROJECTS[_pid].batchAuction) != address(0), 'project_should_be_initialized');
    PROJECTS[_pid].batchAuction.cancelAuctionBeforeStart();
...
//contract BatchAuction {
  ...
  address public PANCAKE_LP_ADDRESS;
  ...
  function finalizeAuction() public onlyOwner {
    ...
    if (isAuctionSuccessful()) {
      ...
      // 80% of funds & 20% of AUCTION_TOKEN instantly goes to the POOL
      uint256 token1Amount = auctionData.totalOfferingTokens.mul(4).div(5); // 50%
      uint256 token2Amount = auctionData.totalCommitments.mul(2).div(5); // 50%

      (PANCAKE_LP_ADDRESS, auctionData.totalLPTokenAmount) =
      ...
      require(PANCAKE_LP_ADDRESS != address(0), "INVALID_PANCAKE_LP_ADDRESS");
  ...
  function setupPancakeSwapPool(
    ...
    lpAddress = IPancakeFactory(pancakeFactory).getPair(_token1, _token2);
  ...
  function getLPTokenBalance() public view returns (uint256) {
    require(address(PANCAKE_LP_ADDRESS) != address(0), "BALANCER_POOL_NOT_INITIALIZED");
