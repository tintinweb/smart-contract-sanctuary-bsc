...
contract STRFTokenLocker is ReentrancyGuard, AccessControl {
    ...
    IMintableToken private _STRFToken;
    ...
    function setSTRFToken(IMintableToken STRFToken) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(address(_STRFToken) == address(0), "STRF Token is already set");
        _STRFToken = STRFToken;
    }
    ...
    function getTotalBalance(address user) view external returns (uint256) {
    ...
    function getWithdrawableBalance(
        ...
        if (_balances[user] > 0) {
            ...
            for (uint i = 0; i < length; i++) {
                if (_userLocks[user][i].amount == 0 || _userLocks[user][i].unlockTime > block.timestamp) continue;
                unlockedAmount = unlockedAmount + _userLocks[user][i].amount;
            }
            penaltyAmount = (_balances[user] - unlockedAmount) / 2;
        }
        return (unlockedAmount, penaltyAmount);
    }

    function getSTRFToken() view external returns (IMintableToken) {
        return _STRFToken;
    }
    ...
    /* ========== MUTATIVE LOCKER_ROLE FUNCTIONS ========== */

    // Mint and lock new tokens
    // Minted and locked tokens incur a 50% penalty when
    // withdrawn before lockDuration has passed.
    function lock(address user, uint256 amount) external onlyRole(LOCKER_ROLE) {
    ...
    function exit() external {
        (uint256 amount, uint256 penaltyAmount) = getWithdrawableBalance(msg.sender);
        require(amount + penaltyAmount > 0, "No balance");
        delete _userLocks[msg.sender];
        _balances[msg.sender] = 0;

        _STRFToken.safeTransfer(msg.sender, amount + penaltyAmount);
        _STRFToken.safeTransfer(_treasury, penaltyAmount);
