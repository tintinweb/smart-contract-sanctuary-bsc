...
contract SMToken is ERC20, Ownable {
    ...
    ) payable ERC20("SincereMoon", "SM")  {
        rewardToken = addr_[0];
        marketingWalletAddress = addr_[1];
        address router = addr_[2];
    ...
    function setBuyTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(marketingFee).add(deadFee) <= 250, "Total buy fee is over 25%");
    ...
    function setSelTaxes(uint256 liquidity, uint256 rewardsFee, uint256 marketingFee, uint256 deadFee) external onlyOwner {
        require(rewardsFee.add(liquidity).add(marketingFee).add(deadFee) <= 250, "Total sel fee is over 25%");
    ...
    function _transfer(
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[from]){
                LFee = amount.mul(buyLiquidityFee).div(denominator);
                AmountLiquidityFee += LFee;
                RFee = amount.mul(buyTokenRewardsFee).div(denominator);
                AmountTokenRewardsFee += RFee;
                MFee = amount.mul(buyMarketingFee).div(denominator);
                AmountMarketingFee += MFee;
                DFee = amount.mul(buyDeadFee).div(denominator);
                fees = LFee.add(RFee).add(MFee).add(DFee);
            }
            if(automatedMarketMakerPairs[to]){
                LFee = amount.mul(sellLiquidityFee).div(denominator);
                AmountLiquidityFee += LFee;
                RFee = amount.mul(sellTokenRewardsFee).div(denominator);
                AmountTokenRewardsFee += RFee;
                MFee = amount.mul(sellMarketingFee).div(denominator);
                AmountMarketingFee += MFee;
                DFee = amount.mul(sellDeadFee).div(denominator);
                fees = LFee.add(RFee).add(MFee).add(DFee);
            }
