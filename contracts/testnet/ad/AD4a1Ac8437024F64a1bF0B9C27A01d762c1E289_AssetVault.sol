...
contract AssetVault {
    ...
    function vaultAsset(uint256 _amount) external onlyController {
        IERC20(assetAddress).transferFrom(address(msg.sender), address(this), _amount); 
    }

    // allow the depositor to withdraw an amount of the asset
    function approveDevaulter(address _recipient, uint256 _amount) external onlyController {
        IERC20(assetAddress).approve(_recipient, _amount);
    }
    ...
    function vaultCounterasset(address _counterassetAddress, uint256 _amount) external onlyController {
        IERC20(_counterassetAddress).transferFrom(address(msg.sender), address(this), _amount);
    }

    // allow the depositor to withdraw an amount of a counterasset
    function approveCounterassetDevaulter(address _recipient, address _counterassetAddress, uint256 _amount) external onlyController {
        IERC20(_counterassetAddress).approve(_recipient, _amount);
    }
