...
contract VARPredictionMarket is Ownable, Pausable, ReentrancyGuard {
    ...
    function executeRound(
        ...
        _safeLockRound(currentEpoch, currentPrice, currentTimestamp);
        _safeEndRound(currentEpoch - 1, currentPrice, currentTimestamp);
        _calculateRewards(currentEpoch - 1);

        // Increment currentEpoch to current round (n)
        currentEpoch = currentEpoch + 1;
        _safeStartRound(currentEpoch, currentTimestamp);
    }
    ...
    function genesisLockRound(
        ...
        currentEpoch = currentEpoch + 1;
        _startRound(currentEpoch, currentTimestamp);
     ...
     * @dev Callable by admin
     */
    function setVerifier(address _verifierAddress) external onlyAdmin {
        require(_verifierAddress != address(0), "Cannot be zero address");
        verifier = _verifierAddress;

        emit NewVerifierAddress(_verifierAddress);
    }
    ...
    function _safeEndRound(
        ...
        round.status = Status.Ended;
        round.closeTimestamp = currentTimestamp;
    ...
    function _safeLockRound(
        ...
        round.lockTimestamp = currentTimestamp;
     ...
     * @param epoch: epoch
     */
    function _safeStartRound(uint256 epoch, uint256 currentTimestamp) internal {
        require(
            genesisStartOnce,
            "Can only run after genesisStartRound is triggered"
        );
        ...
        _startRound(epoch, currentTimestamp);
    }
     ...
     * @param epoch: epoch
     */
    function _startRound(uint256 epoch, uint256 currentTimestamp) internal {
        Round storage round = rounds[epoch];
        round.status = Status.Started;
        round.epoch = epoch;
        round.totalAmount = 0;
        round.startTimestamp = currentTimestamp;
