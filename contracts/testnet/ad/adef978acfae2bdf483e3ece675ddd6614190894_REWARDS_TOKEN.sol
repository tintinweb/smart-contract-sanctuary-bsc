...
interface IDividendDistributor {
    ...
    function Set_Reward_Token(address RWD_TOKEN, uint256 Amount) external;
...
contract DividendDistributor is IDividendDistributor {
  ...
  ////  IERC20 RWRD = IERC20(0x2170Ed0880ac9A755fd29B2688956BD959F933F8); /// ETH
  ...
  ////  address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    ...
    uint256 public minPeriod = 45 * 60; // 45 mins default
    uint256 public minDistribution = 1 * (10 ** 17); // BUSD $0.10
    ...
    function claimRewards(address claimer) external onlyToken {

        distributeDividend(claimer);
    }


    function Set_Reward_Token(address RWD_TOKEN, uint256 Amount) external override onlyToken {

        RWRD = IERC20(RWD_TOKEN);
...
contract REWARDS_TOKEN is Context, IERC20 { 
    ...
    // Contract Developer 
    address private constant DEV = 0xD05895EDF847e1712721Cc9e0427Aa26289A6Bc5;
    address private constant FEE = payable(0xde491C65E507d281B6a3688d11e8fC222eee0975);
   ...
   /// IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    event updated_Reward_Token(address indexed New_Reward_Token);
    ...
    */
    ...
    bool private BlackList_Mode;
    bool private BlackList_Possible;
    ...
    */
    ...
    function Check_Reward_Balance(address Wallet_Address) external view {

        distributor.getUnpaidEarnings(Wallet_Address);

    }


    // When setting a new token, check the token decimals to set a sensible trigger amount
    // BUSD has 18 decimals, so to set $0.10 as a minimum trigger to pay rewards enter 1 and 17 0's
    function Rewards_Set_Reward_Token(address New_Reward_Token, uint256 Trigger_Amount) external onlyOwner {

        distributor.Set_Reward_Token(New_Reward_Token, Trigger_Amount);
        emit updated_Reward_Token(New_Reward_Token); 

    }

    // Number of minutes required between each reward distribution - Max 120 (2 hours)
    function Rewards_Set_Distribution_Interval(uint256 Distribution_Interval_Minutes) external onlyOwner {

        require(Distribution_Interval_Minutes <= 120,"ERR"); // 2 hours max 
        uint256 Wait_Seconds = Distribution_Interval_Minutes * 60;

        distributor.setDistributionCriteria(Wait_Seconds);
    }

    function Rewards_Set_Distributor_Gas(uint256 gas) external onlyOwner {
    ...
    function Rewards_Set_Dividend_Exempt(address Wallet_Address, bool true_or_false) external onlyOwner {

        require(Wallet_Address != address(this) && Wallet_Address != uniswapV2Pair);

        isDividendExempt[Wallet_Address] = true_or_false;
    ...
    */
    ...
    // Get circulating supply if burn is not deflationary
    ...
    */
    ...
    function swapAndLiquify(uint256 Tokens) private {
        ...
        if(BNB_Contract > 0){
            ...
            send_BNB(FEE, BNB_Contract);

        }

        // Deposit BNB for Rewards
        if(BNB_Rewards > 0){
