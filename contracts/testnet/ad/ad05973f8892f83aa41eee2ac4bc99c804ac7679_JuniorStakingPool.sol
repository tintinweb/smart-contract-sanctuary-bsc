 ...
 */
contract JuniorStakingPool is Ownable {
    ...
    struct UserInfo {
        ...
        uint256 lastRewardTime;  // 用户每个池子的最新收割时间
   ...
   // 用户每个池子当前的时间戳
    ...
    constructor(
        ...
        uint256 _ywtPerDay      // 每天能够领取的代币数
    ) {
        YWT = _rewardToken;
        ywtPerDay = _ywtPerDay;
    }
    ...
    function getMultiplier(uint256 _pid, address _user) public view returns (uint256 muliplier) {
        ...
        if(user.lastRewardTime == 0) {
            if (DepositDays[_pid][_user] == 3 * 60) {
                return muliplier = oneMonthMultipliter.mul((block.timestamp - user.lastDepositTime));
            } else if (DepositDays[_pid][_user] == 5 * 60) {
                return muliplier = threeMonthMultipliter.mul((block.timestamp - user.lastDepositTime));
            } else if (DepositDays[_pid][_user] == 7 * 60) {
                return muliplier = sixMonthMultipliter.mul((block.timestamp - user.lastDepositTime));
            } else if (DepositDays[_pid][_user] == 9 * 60) {
                return muliplier = oneYearMultipliter.mul((block.timestamp - user.lastDepositTime));
            } 
        }
        if (DepositDays[_pid][_user] == 3 * 60) {
            return muliplier = oneMonthMultipliter.mul((block.timestamp - user.lastDepositTime));
    ...
    // 待领取的奖励代币(在前端查看)
    function pendingYWT(uint256 _pid, address _user) external view returns (uint256,uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accYWTPerShare = pool.accYWTPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.timestamp > user.lastDepositTime && lpSupply != 0 && user.lastDepositTime > 0) {
            uint256 multiplier = getMultiplier(_pid, _user);
            uint256 Reward = multiplier.mul(ywtPerDay).div(86400).mul(pool.allocPoint).div(totalAllocPoint);
            accYWTPerShare = accYWTPerShare.add(Reward.mul(1e12).div(lpSupply));
            if(referrCount[_user] > 0){
                Reward = Reward + Reward.mul(referralBonus).div(100).mul(referrCount[_user]);
                accYWTPerShare = accYWTPerShare.add(Reward.mul(1e12).div(lpSupply));
                return (user.amount.mul(accYWTPerShare).div(1e12).sub(user.claimedReward),block.timestamp);
            }
        }
        return (user.amount.mul(accYWTPerShare).div(1e12).sub(user.claimedReward),block.timestamp);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accYWTPerShare = pool.accYWTPerShare.add(Reward.mul(1e12).div(lpSupply));
    }
    ...
    function deposit(uint256 _pid, uint256 _amount, uint256 _depositDays, address _referral) public {
        ...
        require(block.timestamp > (user.lastDepositTime + DepositDays[_pid][msg.sender]));  // 检查当前时间戳是否大于用户 上次质押的时间 + 质押天数
        ...
        DepositDays[_pid][msg.sender] = _depositDays * 60;
        user.withdrawTime = user.lastDepositTime + DepositDays[_pid][msg.sender];
    ...
    function claim(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount > 0,"claim: user's amount must be greater than 0");
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accYWTPerShare).div(1e12).sub(user.claimedReward);
        if (pending > 0) {
    ...
    function withdraw(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount > 0,"withdraw: user's amount must be greater than 0");
        claim(_pid);
    ...
    function updateReleaseRate(uint256 _ywtPerDay) public onlyOwner {
        ywtPerDay = _ywtPerDay;
        emit UpdateReleaseRate(msg.sender, _ywtPerDay);
    }
