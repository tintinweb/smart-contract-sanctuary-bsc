...
//绿色传递
contract GREENTRANSMITIDO is Context, Ownable {
    using SafeMath for uint256;

    uint public _activeAmount = 5*10**18;
    uint public _activeTime = 10*24*3600;
    uint public _activeMaxTime = 19*24*3600;
    mapping (uint => uint) public _userRecommends;//获取推荐人
    mapping (address => uint) public _userIds;
    ...
    mapping (uint => uint) public _userActive;//激活到期时间
    mapping (uint => bool) public _actived;//激活状态
    ...
    address public _first = address(0x6AeED229BF1f8674ee70530e112D59095A8B6a6D);//test
    address public _receiver = address(0x6AeED229BF1f8674ee70530e112D59095A8B6a6D);//test
    //
    constructor() {
        //543juy
        //‘’【poiuy9ugfdkkkkkkoo8ii\][poiuytrewq
        _userIds[_first]=++_maxUserId;
    ...
    function bind(address account) public {
        require(_userIds[account]>0,"error0");
        require(_userIds[_msgSender()]==0,"error1");
        _userIds[_msgSender()]=++_maxUserId;
        _userAddress[_maxUserId]=_msgSender();
        _userRecommends[_userIds[_msgSender()]]=_userIds[account];
        _recommends[_userIds[account]].push(_userIds[_msgSender()]);
        _userRenum[_userIds[account]]=_userRenum[_userIds[account]]+1;
    }
    //激活
    function active() public {
        require(_userIds[_msgSender()]>0,"error");
        uint userId=_userIds[_msgSender()];
    ...
    //获取网体
    function getRecommends(address account) external view returns(address[] memory addresses,uint[] memory renums){
        uint userId=_userIds[account];
        uint len=_userRenum[userId];
        addresses=new address[](len);
        renums=new uint[](len);
        for(uint i;i<len;i++){
            addresses[i]=_userAddress[_recommends[userId][len-i-1]];
            renums[i]=_userRenum[_recommends[userId][len-i-1]];
        }
    }
    //获取团队人数
