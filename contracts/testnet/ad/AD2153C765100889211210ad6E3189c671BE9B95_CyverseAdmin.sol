...
contract CyverseAdmin {
    ...
    constructor() {
        adminList.push(0x0000000000000000000000000000000000000001); //Keeps index from hitting 0
        _owner = msg.sender;
        admins[msg.sender] = 1;
...
contract CyverseFactory  {
    ...
    function setAdminContract(address adminAddress_) external adminOnly {
        cyverseAdmin = CyverseAdmin(adminAddress_);
    }

    function setSecondsInDay(uint secondsInDay_) external adminOnly {
        secondsInDay = secondsInDay_;
    }

    function setMinHP(uint8 minHP_) external adminOnly {
        minHP = minHP_;
    }
    ...
    function playerPayout(uint amount_) external reentranceGuard {
        ...
        uint ndays = (block.timestamp - player.cashOutTimer()) / secondsInDay; //how many days have passed
