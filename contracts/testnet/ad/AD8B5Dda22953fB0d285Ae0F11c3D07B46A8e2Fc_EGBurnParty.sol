...
contract EGBurnParty is Ownable, KeeperCompatibleInterface {
    ...
    event SetBurnAddress(address indexed tokenAddress, address indexed burnAddress);
    event SetMinStakeAmount(address indexed tokenAddress, uint256 minStakeAmount);
    event AddBurnToken(address indexed tokenAddress, address indexed burnAddress, uint256 minStakeAmount);
    ...
    event CreateBurnParty(
        ...
        uint256 stakeAmount,
        uint256 minSatkeAmount,
        uint256 gasFeeAmount
    );
    event EndBurnParty(uint256 partyId, address indexed burnToken, uint256 indexed amount, address indexed burnAddress);
    event CancelBurnParty(uint256 partyId, address indexed burnToken, uint256 indexed amount);
    ...
    */
    function addBurnToken(address tokenAddress, address burnAddress, uint256 minStakeAmount) external onlyOwner {
        require(tokenAddress != address(0), "EGBurnParty: The zero address should not be added as a burn token");
        ...
        emit AddBurnToken(tokenAddress, burnAddress, minStakeAmount);
    }

    /**
    * @param tokenAddress burn token address
    * @dev  remove burn token
    *       fire `RemoveBurnToken` event
    */
    // function removeBurnToken(address tokenAddress) external onlyOwner {
    ...
    *       fire `SetBurnAddress` event
    */
    function setBurnAddress(address tokenAddress, address burnAddress) external onlyOwner {
        require(tokenAddress != address(0), "EGBurnParty: The zero address should not be added as a burn token");
        require(burnAddress != address(0), "EGBurnParty: The zero address should not be added as a burn address");
        require(existBurnToken(tokenAddress) == true, "EGBurnParty: The token is not added as a burn token.");

        burnTokens[tokenAddress].burnAddress = burnAddress;

        emit SetBurnAddress(tokenAddress, burnAddress);
    }
    ...
    */
        ...
        external payable
    {
        ...
        stakesList[msg.sender][_partyId] = stakeAmount;
    ...
    */
    function endBurnParty(uint256 partyId) public {
        ...
        emit EndBurnParty(partyId, _party.token, _party.currentQuantity, burnTokens[_party.token].burnAddress);
    }
    ...
    */
    function cancelBurnParty(uint256 partyId) public {
        ...
        emit CancelBurnParty(partyId, _party.token, _party.currentQuantity);
    }
    ...
    */
    function unstakeFromBurnParty(uint256 partyId) external {
        ...
        require(IERC20(_party.token).balanceOf(address(this)) >= stakesList[msg.sender][partyId], 
                    "EGBurnParty: Out of balance.");
        
        uint256 unstakeAmount = stakesList[msg.sender][partyId];
        _party.currentQuantity -= unstakeAmount;
        _party.stakeCounter--;
        
        stakesList[msg.sender][partyId] = 0;
        
        IERC20(_party.token).transfer(msg.sender, unstakeAmount);

        emit UnstakeFromBurnParty(partyId, msg.sender, unstakeAmount);
    }
}