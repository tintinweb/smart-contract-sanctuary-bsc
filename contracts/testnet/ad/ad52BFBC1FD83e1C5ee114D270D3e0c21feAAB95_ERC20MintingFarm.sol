...
contract ERC20MintingFarm is Ownable, ReentrancyGuard, IERC20MintingFarm{
    ...
    IFarmDeployer public immutable farmDeployer;

    bool public keepReflectionOnDeposit;
    uint32 public lastRewardBlock;
    uint32 public minimumLockTime;
    uint16 public earlyWithdrawalFee;
    uint56 public defaultStakePPS;
    uint96 public PRECISION_FACTOR;
    ...
    struct UserInfo {
        ...
        uint224 rewardDebt;  // Reward debt
        uint32 depositBlock; // DepositBlock
    }


    /*
     * @dev Initialize implementation with immutable Farm Deployer address (for gas savings)
     * @param _farmDeployer Farm deployer address
     */
    constructor(IFarmDeployer _farmDeployer) {
    ...
    function initialize(
        ...
        PRECISION_FACTOR = SafeCast.toUint96(10**(30 - decimalsRewardToken));
        require(decimalsRewardToken >= 5 && decimalsStakeToken >= 5, "Invalid decimals");

        defaultStakePPS = SafeCast.toUint56(10 ** (decimalsStakeToken - decimalsStakeToken / 2));
    }
    ...
    function _deposit(uint256 amount, address account) internal nonReentrant {
        ...
        uint256 _currentShares = user.shares;
        if (userStakeLimit > 0) {
            require(
                amount + _currentShares * stakePPS() <= userStakeLimit,
        ...
        uint256 pending = 0;
        if (_currentShares > 0) {
            pending = _currentShares * accTokenPerShare / PRECISION_FACTOR - user.rewardDebt;
        }

        uint256 depositedAmount = 0;
        {
            uint256 amountToDeposit = amount - (amount % PPS);
            require(amountToDeposit > 0, "Below minimum amount");
        ...
        user.shares = _currentShares + newShares;
        stakeTotalShares += newShares;

        user.rewardDebt = SafeCast.toUint224((_currentShares + newShares) * accTokenPerShare / PRECISION_FACTOR);
        if (_currentShares == 0) {
            user.depositBlock = uint32(block.number);
        } else {
            // weighted average
            uint256 _depositBlock = user.depositBlock;
            uint256 _minimumLockTime = minimumLockTime;
            uint256 _startBlock = (_depositBlock + _minimumLockTime) < block.number
                ? block.number - _minimumLockTime
                : _depositBlock;
            uint256 subtrahend = _currentShares * (block.number - _startBlock)
             / (_currentShares + newShares);
            user.depositBlock = uint32(block.number - subtrahend);
        }

        if (pending > 0) {
            IRewardMinter(address(rewardToken)).mint(account, pending);
        }
    ...
    function withdraw(uint256 _shares) external nonReentrant {
        ...
        uint256 transferAmount = _shares * stakePPS();
        ...
        user.rewardDebt = SafeCast.toUint224(user.shares * accTokenPerShare / PRECISION_FACTOR);

        if (pending > 0) {
            IRewardMinter(address(rewardToken)).mint(address(msg.sender), pending);
        }
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        uint256 amountToTransfer = user.shares * stakePPS();
    ...
    /*
     * @notice Calculates the last block number according to reward per block
     * @return End block
     */
     ...
     */
    function setEarlyWithdrawalFee(uint16 _earlyWithdrawalFee) public onlyOwner {
     ...
     */
    function setMinimumLockTime(uint32 _minimumLockTime) public onlyOwner {
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (rewardPerBlock > 0 && stakeTotalShares != 0) {
    ...
    function _updatePool() private {
        ...
        if (stakeTotalShares == 0 || rewardPerBlock == 0) {
            lastRewardBlock = uint32(block.number);
            return;
        }

        uint256 cakeReward = rewardPerBlock == 0
            ? 0
            : _getMultiplier(lastRewardBlock, block.number) * rewardPerBlock;
