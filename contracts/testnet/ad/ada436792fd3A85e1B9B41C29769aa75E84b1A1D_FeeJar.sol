...
contract FeeJar is AccessControl {
    ...
    {
        if (networkFee > 0) {
            networkFeeAmount = (totalFee * networkFee) / 100;
        }
        if (lpFee > 0) {
            lpFeeAmount = (totalFee * lpFee) / 100;
        }
        if (supportFee > 0) {
            supportFeeAmount = (totalFee * supportFee) / 100;
        }
    }
    ...
    function fee() public payable {
        ...
        if (supportFee > 0) {
            address supportFeeCollector = ISafeswapFactory(factory).feeTo();
