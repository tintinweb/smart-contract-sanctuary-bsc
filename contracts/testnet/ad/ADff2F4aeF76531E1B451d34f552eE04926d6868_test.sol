...
contract test is IERC20, Context, Ownable, ReentrancyGuard {
    ...
    mapping (uint256 => uint256) public buyDValue;
    mapping (uint256 => uint256) public gotTokens;
    ...
    function _buy(uint256 minTokenOut, uint256 deadline) public nonReentrant ensure(deadline) payable returns (bool) {
        ...
        //update buys
        totalBuys +=1;
        buyValue[totalBuys]= bnbAmount;
        buyDValue[totalBuys]= bnbAmount.mul(getBNBPrice());
        gotTokens[totalBuys]= tokensToSend;
    ...
    function getTokenAmountOut(uint256 amountBNBIn) external view returns (uint256) {
        uint256 amountAfter = liqConst.div(liquidity.sub(amountBNBIn));
        uint256 amountBefore = liqConst.div(liquidity);
        return amountAfter.sub(amountBefore);
    }

    function getBNBAmountOut(uint256 amountIn) public view returns (uint256) {
        uint256 beansBefore = liqConst.div(_balances[address(this)]);
        uint256 beansAfter = liqConst.div(_balances[address(this)].add(amountIn));
        return beansBefore-beansAfter;
    }

    function addLiquidity() external onlyOwner payable {
        uint256 tokensToAdd= _balances[address(this)].mul(msg.value).div(liquidity);
