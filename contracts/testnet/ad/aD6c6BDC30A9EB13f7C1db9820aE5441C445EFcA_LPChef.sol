...
contract LPChef is Ownable {
    ...
    constructor() {
        _creator = msg.sender;
        awardPerSecond = 1 * 1e18;

        rewardWithdrawSwitch = true;
        OSATokenAddress = 0x0DA7B66EFc97A25C66cC0f2189AA2F0Ff4Df6202;
        lpPairAddress = 0x5f9C8287a59519d9D2d14807c4c96a72833E0019;
        startTimestamp = block.timestamp;
        endTimestamp = block.timestamp + 30 * 24 * 3600;
    }

    address _creator;
    bool public rewardWithdrawSwitch;
    address public OSATokenAddress;
    address public lpPairAddress;
    uint256 public startTimestamp;
    uint256 public endTimestamp;

    function setRewardWithdrawSwitch(bool _bool) public onlyOwner {
        require(rewardWithdrawSwitch != _bool, "RewardWithdrawSwitch is already this value");
        rewardWithdrawSwitch = _bool;
    }

    function setOSATokenAddress(address _addr) public onlyOwner {
        require(OSATokenAddress != _addr, "OSATokenAddress is already this value");
        OSATokenAddress = _addr;
    }

    function setLpPairAddress(address _addr) public onlyOwner {
        require(lpPairAddress != _addr, "LpPairAddress is already this value");
        lpPairAddress = _addr;
    }

    function setStartTimestamp(uint256 _startTimestamp) public onlyOwner {
        require(startTimestamp != _startTimestamp, "StartTimestamp is already this value");
        startTimestamp = _startTimestamp;
    }

    function setEndTimestamp(uint256 _endTimestamp) public onlyOwner {
        require(endTimestamp != _endTimestamp, "EndTimestamp is already this value");
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (pool.lockSecond == 0) {
            userCurrentInfos[msg.sender].amount = userCurrentInfos[msg.sender].amount.add(_amount);
            userCurrentInfos[msg.sender].rewardDebt = userCurrentInfos[msg.sender].rewardDebt.add(_amount.mul(poolInfos[_pid].accAwardPerShare).div(1e18));
        } else {
            userDepositInfos[msg.sender][_pid].push(UserDepositInfo({amount: _amount, rewardDebt: _amount.mul(pool.accAwardPerShare).div(1e18), totalAward: 0, lockTimestampUtil: block.timestamp.add(pool.lockSecond), status: true}));
        }
        IERC20(lpPairAddress).safeTransferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw(
        ...
        if (pool.lockSecond == 0) {
            ...
            userCurrentInfos[msg.sender].rewardDebt = accumulatedAward.sub(_amount.mul(pool.accAwardPerShare).div(1e18));
            userCurrentInfos[msg.sender].amount = userCurrentInfos[msg.sender].amount.sub(_amount);
            userCurrentInfos[msg.sender].totalAward = userCurrentInfos[msg.sender].totalAward.add(_pending);

            // IOSAToken(OSATokenAddress).mintByChef(msg.sender, _pending);
            availableWithdrawBalance[msg.sender] = availableWithdrawBalance[msg.sender].add(_pending);
            IERC20(lpPairAddress).safeTransfer(msg.sender, _amount);
        } else {
            ...
            // IOSAToken(OSATokenAddress).mintByChef(msg.sender, depositInfo.totalAward);
    ...
    function harvest(uint256 _pid) public {
        ...
        if (pool.lockSecond == 0) {
            ...
            if (totalPedding != 0) {
                IOSAToken(OSATokenAddress).mintByChef(msg.sender, totalPedding);
            }
        }
    }
