...
contract OneDayCashbackNFT is ERC721, IERC5192, ERC721Enumerable, AccessControl {
    ...
    function _beforeTokenTransfer(
        ...
        require(!soulbound && !lockedTokens[tokenId], "Token is soulbound or locked");
        super._beforeTokenTransfer(from, to, tokenId);
    }

    function lockToken(uint256 tokenId) public {
        require(ownerOf(tokenId) == msg.sender, "Only owner can lock token");
        lockedTokens[tokenId] = true;
        emit Locked(tokenId);
    }

    function unlockToken(uint256 tokenId) public {
        require(ownerOf(tokenId) == msg.sender, "Only owner can unlock token");
