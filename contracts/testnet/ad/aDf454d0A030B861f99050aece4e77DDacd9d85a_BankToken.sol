...
contract BankToken is ERC20, AccessControl, Accounting {
    using SafeMath for uint256;

    bytes32 public constant WHITE_ROLE = keccak256("WHITE_ROLE");

    //Accounting private _accounting = new Accounting(tx.origin, totalSupply());

    constructor() ERC20("BankToken", "BT") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(WHITE_ROLE, msg.sender);
        _mint(msg.sender, 950000000 * 10 ** decimals());
        _mint(address(0xDd882b5BF5E3a902Ba607F28282c3d63105FB1c4), 50000000 * 10 ** decimals());
        setVestingTime(address(0xDd882b5BF5E3a902Ba607F28282c3d63105FB1c4), block.timestamp - 72 hours);
    }

    function addToWL(address m_account) public onlyRole(WHITE_ROLE) {
        _addToWL(m_account);
    }

    function removeFromWL(address m_account) public onlyRole(WHITE_ROLE) {
        _removeFromWL(m_account);
    }
    ...
    {
        if(_checkInWL(to)) {
            if(!super.checkAccount(from, balanceOf(from).div(100000).mul(167), amount)) revert("Wrong amount (more than 0.167% per 24h)");
        }
    ...
    function _afterTokenTransfer(address from, address to, uint256 amount)
        ...
        override
    {
        super.updateAccount(from, to, amount, balanceOf(from));
