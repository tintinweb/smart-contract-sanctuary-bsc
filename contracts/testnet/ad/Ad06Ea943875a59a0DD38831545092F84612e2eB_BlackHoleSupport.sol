...
contract BlackHoleSupport is Ownable, AccessControl {
    struct TeamLeader {
        uint referredNumber;
        string name;
    ...
    uint public totalTeamLeaderDonateNumber;
    ...
    uint public totalPersonalDonateNumber;
    ...
    uint public totalInstituteDonateNumber;
    ...
    event PersonalDonate(address indexed sender, uint amount);
    event InstituteDonate(address indexed sender, uint amount);
    event TeamDonate(address indexed sender, uint amount);

    // create recorder role for record the detail of team
    bytes32 public constant RECORDER_ROLE = keccak256("RECORDER_ROLE");

    constructor(address _usdtAddress, address _recorder) {
        usdtAddress = _usdtAddress;
        _setupRole(RECORDER_ROLE, _recorder);
    }
    ...
    function increaseTeamBuyBond(address _user, address _bondAddress, uint _amount) public {
        require(hasRole(RECORDER_ROLE, msg.sender), "Caller is not a recorder");
        if (userInfo[_user].referred) {
            bondOfTeam[userInfo[_user].teamLeaderAddress][_bondAddress] += _amount;
        }
    }

    function decreaseTeamBuyBond(address _user, address _bondAddress, uint _amount) public {
        require(hasRole(RECORDER_ROLE, msg.sender), "Caller is not a recorder");
        if (userInfo[_user].referred) {
            bondOfTeam[userInfo[_user].teamLeaderAddress][_bondAddress] -= _amount;
        }
    ...
    function donatePersonal(uint _amount) public {
        ...
        if (personalDonateNumber[msg.sender] == 0) {
            // record persons address
            persons.push(msg.sender);
        }
        personalDonateNumber[msg.sender] += _amount;
        totalPersonalDonateNumber += _amount;
        emit PersonalDonate(msg.sender, _amount);
    }
    ...
    function donateInstitute(uint _amount) public {
        ...
        totalInstituteDonateNumber += _amount;
        // this address to institutes

        emit InstituteDonate(msg.sender, _amount);
    }
    ...
    function donateTeamLeader(uint _amount) public {
        ...
        if (teamLeaderDonateNumber[msg.sender] == 0) {
            // this address to teamLeaders
            teamLeaders.push(msg.sender);
        }

        teamLeaderDonateNumber[msg.sender] += _amount;
        totalTeamLeaderDonateNumber += _amount;
        // init team leader info
        teamLeaderInfo[msg.sender].referredNumber += 1;

        // set this user belong to this team
        userInfo[msg.sender].teamLeaderAddress = msg.sender;
        userInfo[msg.sender].referred = true;


        emit TeamDonate(msg.sender, _amount);
    }


    function getPersonalDonateNumber(address _user) public view returns (uint){
        return personalDonateNumber[_user];
    }

    function getInstituteDonateNumber(address _user) public view returns (uint){
        return instituteDonateNumber[_user];
    }

    function getTeamLeaderDonateNumber(address _user) public view returns (uint){
        return teamLeaderDonateNumber[_user];
    }

    function getTotalPersonalDonateNumber() public view returns (uint){
        return totalPersonalDonateNumber;
    }

    function getTotalInstituteDonateNumber() public view returns (uint){
        return totalInstituteDonateNumber;
    }

    function getTotalLeaderDonateNumber() public view returns (uint){
        return totalTeamLeaderDonateNumber;
    }
