...
contract WoolRewardsV2 is ERC721Holder, Pausable, Ownable {
    ...
    mapping (uint256 => mapping(address => bool)) public _whitelist;
    // projectId => time
    mapping (uint256 => uint256) public _whitelistActivatedUntil;

    // events
    event RewardClaimed (uint256 indexed rewardId, uint256[] tokenIds);
    event NewReward (uint256 rewardId, uint256 projectId, uint256 totalAmount, string description);
    ...
    function setWhitelistByProject(uint256 projectId, address[] calldata accounts, uint256 endDate) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            _whitelist[projectId][accounts[i]] = true;
        }
        _whitelistActivatedUntil[projectId] = endDate;
    }
    ...
    modifier checkWhiteList(uint256 projectId) {
        if (_whitelistActivatedUntil[projectId] > block.timestamp) {
            require(_whitelist[projectId][msg.sender] == true, "You aren't in the whitelist!");
        }
        _;
    }

    function buyTokens(uint256 projectId, uint256 amount) external checkWhiteList(projectId) {
        Project memory project = IWoolStateNFT(_nftRegistry).getProject(projectId);
    ...
    function addReward(uint256 projectId, uint256 totalRewards, string memory description) external onlyOwner {

        Project memory project = IWoolStateNFT(_nftRegistry).getProject(projectId);
        require (project.status == State.Approved, "Can't add reward if project is not approved yet");

        rewardCounter++;
        Reward memory _reward = Reward(rewardCounter, projectId, description, totalRewards, totalRewards);
        ...
        emit NewReward (rewardCounter, projectId, totalRewards, description);
    }
    ...
    function claimReward(uint256 rewardId, uint256[] calldata tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require (_rewardsClaimed[_rewardsAvailable[rewardId].projectId][rewardId][tokenIds[i]] == false, "Reward already claimed using that NFT");
            _rewardsClaimed[_rewardsAvailable[rewardId].projectId][rewardId][tokenIds[i]] = true;

            address account = IERC721(_nftRegistry).ownerOf(tokenIds[i]);
            require (account == msg.sender, "You can't to claim rewards of another accounts");

            amountToReceive += calculateCost(
        ...
        if (amountToReceive > 0) {
            require(IERC20(_aceptedToken).transfer(msg.sender, amountToReceive), "ERC20: Transfer has failed");
        }

        emit RewardClaimed (rewardId, tokenIds);
    }
