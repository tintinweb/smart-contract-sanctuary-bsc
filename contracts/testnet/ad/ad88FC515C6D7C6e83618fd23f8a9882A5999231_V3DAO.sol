...
contract V3DAO is OwnableUpgradeSafe, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    mapping(address => EnumerableSet.UintSet) private userStakedNfts;
    uint256[5] public limitedAmount;
    ...
    event RewardTaxed(address indexed user, uint256 earned);
    ...
    function initialize(
        ...
        uint256 _startTime,
        uint256[5] memory _limitedAmount
        ...
        lastEpochTime = _startTime.sub(7 hours);
    ...
    function setV3spa(address _v3spa) external onlyOwner {
        require(_v3spa != address(0), "zero");
        v3spa = _v3spa;
    }

    function updateLimitedAmount(
        uint256[5] memory _limitedAmount
    ) external onlyOwner {
        limitedAmount = _limitedAmount;
    }
    ...
    function addMoreEpochRewardManualValue(uint256 _epochRewardManualValue) external onlyAuthorizer {
        epochRewardManualValue = epochRewardManualValue.add(_epochRewardManualValue);
    }
    ...
    function setEpochRewardLimit(uint256 _epochRewardLimit) external onlyOwner {
        require(_epochRewardLimit <= 300000 ether, "Limit is too high");
    ...
    // Depositing of NFTs
    function depositNFT(uint256[] memory _tokenIds) external nonReentrant {
        for (uint256 i = 0 ; i < _tokenIds.length; i++){
            uint256 _tokenId = _tokenIds[i];
            require(IERC721(v3spa).ownerOf(_tokenId) == msg.sender, "user does not have specified NFT");
            IERC721(v3spa).transferFrom(msg.sender, address(this), _tokenId);
            userStakedNfts[msg.sender].add(_tokenId);
        }
    }

    // Withdrawing of NFTs
    function withdrawNFT(uint256[] memory _tokenIds) external nonReentrant {
        for (uint256 i = 0 ; i < _tokenIds.length; i++){
            uint256 _tokenId = _tokenIds[i];
            require(userStakedNfts[msg.sender].contains(_tokenId), 'Only owner can withdraw this NFT');
            IERC721(v3spa).transferFrom(address(this), msg.sender, _tokenId);
            userStakedNfts[msg.sender].remove(_tokenId);
        }
    }

    function getListNftByUser(address _user) public view returns (uint256 _len, uint256[] memory _tokenIds){
        _len =  userStakedNfts[_user].length();
        _tokenIds = new uint256[](_len);
        for (uint256 i = 0; i < _len; ++i) {
            _tokenIds[i] = userStakedNfts[_user].at(i);
        }
    }

    function getLimitedAmount(address _user) public view returns (uint256 _amountLimited, uint256 _amountAvailable){
        uint256 _len =  userStakedNfts[_user].length();
        _amountLimited = 0;
        for (uint256 i = 0; i < _len; ++i) {
            uint256 _tokenId = userStakedNfts[_user].at(i);
            uint256 _level = IV3SPA(v3spa).getTokenLevel(_tokenId);
            _amountLimited  = _amountLimited.add(limitedAmount[_level]);
        }
        _amountAvailable =  _amountLimited.sub(_balances[_user]);
    }
    ...
    function stake(uint256 _amount) external nonReentrant updateReward(msg.sender) {
        ...
        uint256 _totalBal = _amount.add(_balances[msg.sender]);
        (uint256 _limitBal,) = this.getLimitedAmount(msg.sender);
        require(_totalBal <=  _limitBal, "Need deposit more NFT");
    ...
    function _taxReward() internal updateReward(msg.sender) {
        ...
        if (_earned > 0) {
            ...
            uint256 _half_earned = _earned.div(2);
            uint256 _tax = _earned.sub(_half_earned);
            IERC20(reward).safeTransfer(msg.sender, _half_earned);
            IERC20(reward).safeTransfer(reserveFund, _tax);
            emit RewardPaid(msg.sender, _half_earned);
            emit RewardTaxed(msg.sender, _tax);
        }
    }
