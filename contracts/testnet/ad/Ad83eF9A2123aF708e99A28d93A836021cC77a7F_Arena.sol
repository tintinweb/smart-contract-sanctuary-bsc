...
contract Arena is Ownable {
    ...
    // returns all Matador ids in this contract
    function viewArena() public view returns (uint256[] memory) {
    ...
    function selectRandomMatadorOwnerToReceiveStolenBull(uint16 _stolenBullID) external onlyBullRunGame() returns (address) {
        uint256[] memory matadorsInArena = viewArena();
        uint256 length = matadorsInArena.length;

        uint256 randomIndex = _random(_stolenBullID) % length;
        uint256 selectedMatadorID = matadorsInArena[randomIndex];
        address thief = OriginalOwner[uint16(selectedMatadorID)];
        emit BullThiefSelected(thief);
        return thief;
    }

    // pRNG generator
    function _random(uint16 _stolenBullID) internal view returns (uint256) {
        uint256 seed = uint256(keccak256(abi.encodePacked(
            block.timestamp + block.difficulty +
            ((uint256(keccak256(abi.encodePacked(block.coinbase)))) / (block.timestamp)) +
            block.gaslimit + 
            ((uint256(keccak256(abi.encodePacked(_stolenBullID)))) / (block.timestamp)) +
            block.number
        )));
