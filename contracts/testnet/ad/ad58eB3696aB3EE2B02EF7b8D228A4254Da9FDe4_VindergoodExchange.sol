...
contract ExchangeMain is ReentrancyGuarded, Ownable {
        ...
        returns (bool, uint256)
    {
        ...
        assembly {
            ...
            let tokenId := mload(add(sellCallData, mul(0x20, 4)))
            buyAmount := mload(add(buyCallData, mul(0x20, 5)))
            sellAmount := mload(add(sellCallData, mul(0x20, 5)))
        }
    ...
    function calculateFinalPrice(
        ...
        if (saleKind == SaleKind.FixedPrice) {
            ...
            if (side == Side.Sell) {
                ...
                return SafeMath.mul(SafeMath.sub(basePrice, diff), amount);
            } else {
                /* Buy-side - start price: basePrice. End price: basePrice + extra. */
                return SafeMath.mul(SafeMath.add(basePrice, diff), amount);
            }
        }
    }
}
