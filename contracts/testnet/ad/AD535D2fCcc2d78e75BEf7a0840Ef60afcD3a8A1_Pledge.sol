...
contract Pledge is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount, uint256 _grade) external nonReentrant {
        ...
        uint256 _lp = _lpAmount * (10 ** IERC20Metadata(USDTToken).decimals()) * (10 ** IERC20Metadata(USDTToken).decimals()) / _balance / _totalSupply;
    ...
    function withdraw() external nonReentrant {
        ...
        require(grade * 7 * 86400 + user.depositTime <= block.timestamp, "The pledge time has not expired");
        ...
        if (pending > 0) {
            ...
            if (superior != address(0)) {
                ...
                if (twoSuperior != address(0) && userInfo[twoSuperior].amount > 0) {
                    fee2 = pending * 5 / 100;
...
contract GSS is ERC20Basics {
    ...
    constructor(address _USDTToken, address _PopeFeeDividendToken, address _IPOToken, address _GSSHoldToken, address _PledgeToken, address _ConstellationToken, address _PopeToken, address _MarketingToken, address _uniswapV2RouterAddress)
    ERC20Basics("Gold Saints", "GSS", 18)
    {
        ...
        uint256 _decimals = uint256(decimals());
         ...
         _mint(_IPOToken, 4000000 * (10 ** _decimals));
    ...
    function balanceOf(address account) public view override returns (uint256) {
        return super.balanceOf(account) + IPO(IPOToken).balanceOf(account);
    }
    ...
    function invite(address to) public returns (bool) {
        address from = _msgSender();
    ...
    function _invite(address from, address to) private {

        require(inviter[to] == address(0), "The current user has a superior");
    ...
    function _tokenTransfer(address from, address to, uint256 amount) private {
        ...
        if (msg.sender != IPOToken) {
            IPO(IPOToken).release(msg.sender);
        }

        _transfer(from, to, amount);

        GSSHold(GSSHoldToken).deposit(msg.sender, amount);
...
contract IPO is Ownable {
    ...
    function buy(uint256 _amount) external {
        ...
        require(block.timestamp < endTime, "Private placement has ended");
        require(GSS(GSSToken).balanceOf(address(this)) >= _amount, "Insufficient balance");

        uint256 money = _amount * price;
        ...
        address superior = GSS(GSSToken).inviter(msg.sender);
        if (superior != address(0)) {
            IERC20Metadata(USDTToken).transfer(superior, money * 10 / 100);
            address twoSuperior = GSS(GSSToken).inviter(superior);
            if (twoSuperior != address(0)) {
                IERC20Metadata(USDTToken).transfer(twoSuperior, money * 5 / 100);
            }
        }

        emit Buy(msg.sender, _amount, money);
    }
    ...
    function destroy() external {
        ...
        uint256 _amount = IERC20Metadata(GSSToken).balanceOf(address(this)) - _totalSupply;
