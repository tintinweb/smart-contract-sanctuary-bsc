...
contract BlucamonNFT is Ownable {
    ...
    IBlucamonOwnership BLUCAMON_CONTRACT; // blucamon NFT contract
    ...
    constructor(
        ...
        BLUCAMON_CONTRACT = IBlucamonOwnership(_blucamonContract);
        rarity = _rarity;
        name = _name;
    }
    ...
    function tokenURI(uint256 _id) external view returns (string memory) {
        return BLUCAMON_CONTRACT.tokenURI(_id);
    }

    function ownerOf(uint256 _id) external view returns (address) {
        return BLUCAMON_CONTRACT.ownerOf(_id);
    }

    function _mint(address _to) internal {
        uint256 newTokenId = _getBlucamonId().add(1);
        string memory tokenUri = _getTokenUri(newTokenId);
        BLUCAMON_CONTRACT.mintBlucamon(_to, tokenUri, false, rarity, 0, 0);
    }
