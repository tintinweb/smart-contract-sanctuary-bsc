...
contract DecentMinter is ERC721, ERC721URIStorage, Ownable {
    ...
    uint256 public sold;
    uint256 public gasFee = 0.0001 ether;
    ...
    function mintNFT(
        string memory suit,
        string memory trouser,
        string memory shoe,
        uint256 _quantitiy
    ) public payable {
        require(
            msg.value >= gasFee.mul(_quantitiy),
            "Should have gas fee in BNB"
        );
        require(sold <= totalSupply, "All packages Sold");
        uint256 newTokenId;
        for (uint256 i = 0; i < _quantitiy; i++) {
            _tokenIdCounter.increment();
            newTokenId = _tokenIdCounter.current();
            _safeMint(msg.sender, newTokenId);
            _setTokenURI(newTokenId, suit);
            ownedTokens[msg.sender].push(newTokenId);

            _tokenIdCounter.increment();
            newTokenId = _tokenIdCounter.current();
            _safeMint(msg.sender, newTokenId);
            _setTokenURI(newTokenId, trouser);
            ownedTokens[msg.sender].push(newTokenId);

            _tokenIdCounter.increment();
            newTokenId = _tokenIdCounter.current();
            _safeMint(msg.sender, newTokenId);
            _setTokenURI(newTokenId, shoe);
            ownedTokens[msg.sender].push(newTokenId);
        }
        sold += _quantitiy;
        mintCount[msg.sender] += _quantitiy;
