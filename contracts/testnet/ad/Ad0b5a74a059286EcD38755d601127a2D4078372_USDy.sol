    ...
    **/
    ...
    function tax( uint256 taxAmount, address sender, uint amount) internal {
        ...
        if(Admin.buyBackActivation()){
            ...
            (uint amount, uint leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution(sender, (IERC20(BUSD).balanceOf((address(this))) * USDy_Tax_Allocation[0])/ USDy_Tax, amount);
            if(amount != 0){
            ...
            IERC20(Admin.BUSD()).transfer(Admin.Treasury(), (IERC20(BUSD).balanceOf((address(this))) * USDy_Tax_Allocation[1]) / (USDy_Tax));
            IERC20(Admin.BUSD()).transfer(Admin.TeamAddress(), (IERC20(BUSD).balanceOf((address(this))) * USDy_Tax_Allocation[2]) / (USDy_Tax));
        }        
    }
