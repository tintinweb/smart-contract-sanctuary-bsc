...
contract RDNDistributor is AccessControlEnumerable {
    // todo: Treasury fee
    ...
    event Distributed(uint indexed userId, address indexed userAddress, uint initUserId, address initAddress, uint amount);
        ...
        while (user.parentId > 0 && count < 12) {
            ...
            if (factor > maxFactor) {
                ...
                if (user.activeUntill > block.timestamp) {
                    ...
                    emit Distributed(userId, user.userAddress, initUserId, _initAddress, bonus);
                }
                // emit DebugLog(user.tariff, factor, bonus);
            }
        }
