...
contract DinoReflection is Context, Auth, IEarn {
    ...
    constructor(address _tokenAddress) Auth(msg.sender) {
        ...
        wbnbAddress = IUniswapV2Router02(routerAddress).WETH();
    ...
    function deposit() public payable override{
        ...
        if(totalShares > 0 && amount > 0) dividendsPerShare = dividendsPerShare.add(dividendsPerShareAccuracyFactor.mul(amount).div(totalShares));
    ...
    function distributeDividend() external override {
        ...
        while(gasUsed < minimumGasDistribution && iterations < shareholderCount) {
            ...
            if(shouldDistribute(shareholders[currentIndex])){
                // If holder doesn t have preference token to share, then send in Dino
    ...
    */
    ...
    function _claimToToken(address account, address targetToken) internal {
        ...
        if(amountAfterFee > 0){
            if(wbnbAddress != wethAddress){
              IWETH(wbnbAddress).withdraw(amountAfterFee);
              IWETH(wethAddress).deposit{value:amountAfterFee}();
            }            
    ...
    function _claimToDino(address account) internal {
        IUniswapV2Router02 router = IUniswapV2Router02(routerAddress);
        uint256 amount = dividendOf(account);
        if(amount > 0 && shouldClaim(account)){
            if(wbnbAddress != wethAddress){
