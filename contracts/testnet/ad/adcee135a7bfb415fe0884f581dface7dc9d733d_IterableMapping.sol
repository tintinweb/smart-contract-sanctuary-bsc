...
contract BUSDDividendTracker is DividendPayingToken  {
    ...
    constructor(address _dividentToken) DividendPayingToken("EDWIIN_Busd_Dividend_Tracker", "EDWIIN_Busd_Dividend_Tracker",_dividentToken) {
        // Change to 3600
    	claimWait = 300;
    ...
    function _transfer(address, address, uint256) pure internal override {
        require(false, "EDWIIN_Busd_Dividend_Tracker: No transfers allowed");
    }
 
    function withdrawDividend() pure public override {
        require(false, "EDWIIN_Busd_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main EDWIIN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        // 3600 to 84600
        require(newClaimWait >= 300 && newClaimWait <= 420, "EDWIIN_Busd_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "EDWIIN_Busd_Dividend_Tracker: Cannot update claimWait to same value");
...
contract Edwiin is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    receive() external payable {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != marketingWallet, "EDWIIN: The marketing wallet is already this address");
  	...
  	function updateDontationWallet(address _newWallet) external onlyOwner {
  	    require(_newWallet != donationWallet, "EDWIIN: The donation wallet is already this address");
        excludeFromFees(_newWallet, true);
    ...
    function updatebusdDividendTracker(address newAddress) external onlyOwner {
        require(newAddress != address(busdDividendTracker), "EDWIIN: The dividend tracker already has that address");
 
        BUSDDividendTracker newbusdDividendTracker = BUSDDividendTracker(payable(newAddress));
 
        require(newbusdDividendTracker.owner() == address(this), "EDWIIN: The new dividend tracker must be owned by the EDWIIN token contract");
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "EDWIIN: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }
 
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "EDWIIN: Account is already exluded from fees");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "EDWIIN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
 
        _setAutomatedMarketMakerPair(pair, value);
    }
 
    function _setAutomatedMarketMakerPair(address pair, bool value) private onlyOwner {
        require(automatedMarketMakerPairs[pair] != value, "EDWIIN: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue != gasForProcessing, "EDWIIN: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(tradingIsEnabled || (isExcludedFromFees[from] || isExcludedFromFees[to]), "EDWIIN: Trading has not started yet");
