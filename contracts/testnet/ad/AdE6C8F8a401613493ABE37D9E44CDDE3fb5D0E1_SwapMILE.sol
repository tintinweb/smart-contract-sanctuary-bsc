  ...
  ReentrancyGuardUpgradeable
{
    ...
    returns (uint256[] memory, uint256)
  {
    ...
    return (array, withdrawalRequestIdsByEntity[entity].length);
  }
  ...
  function getRequestsByEntity(
    ...
    if (ascOrder) {
      ...
      for (uint256 i = offset; i > arrayLimit; i--) {
        uint256 id = withdrawalRequestIdsByEntity[entity][i - 1];
        array[j] = withdrawalRequests[entity][id];
        j++;
      }
    }
  ...
  function getAvailableSMILEToWithdraw(address entity)
    ...
    returns (uint256)
  {
    uint256 balance = sMILEToken.balanceOf(entity);
    uint256 sum = _sumRequestedSMILE(entity);
    if (balance <= sum) {
      return 0;
    }
    return balance - sum;
  ...
  function getRequestedSMILE(address entity) external view returns (uint256) {
    return _sumRequestedSMILE(entity);
  }
    ...
    returns (uint256)
  {
    ...
    require(
      sMILEToken.balanceOf(sender) - _sumRequestedSMILE(sender) >= amount,
    ...
    withdrawalRequestIdsByEntity[sender].push(id);
    withdrawalRequests[sender][id] = WithdrawRequest({
      id: id,
      amountOfsMILE: amount,
      creationTimestamp: currentTime,
      coolDownEnd: currentTime + cooldownPeriod,
      withdrawEnd: currentTime + withdrawPeriod,
      status: RequestWithdrawStatus.Active,
      statusUpdateTimestamp: currentTime
    });
  ...
  function withdraw(uint256 withdrawalId) external override returns (bool) {
    ...
    require(
      request.status == RequestWithdrawStatus.Active,
      "Passed withdraw request isn't active"
    );
    ...
    if (request.withdrawEnd <= block.timestamp) {
      withdrawalRequests[msg.sender][withdrawalId]
        .status = RequestWithdrawStatus.Expired;
      // solhint-disable-next-line not-rely-on-time
      withdrawalRequests[msg.sender][withdrawalId].statusUpdateTimestamp = block
        .timestamp;
      emit Withdrawn(
        withdrawalId,
        msg.sender,
    ...
    withdrawalRequests[msg.sender][withdrawalId].status = RequestWithdrawStatus
      .Done;
    // solhint-disable-next-line not-rely-on-time
    withdrawalRequests[msg.sender][withdrawalId].statusUpdateTimestamp = block
      .timestamp;
  ...
  function cancelWithdraw(uint256 withdrawalId) external override {
    require(
      withdrawalRequests[msg.sender][withdrawalId].status ==
        RequestWithdrawStatus.Active,
      "Passed withdraw request isn't active"
    );
    require(
    ...
    emit WithdrawCanceled(
      withdrawalId,
      withdrawalRequests[msg.sender][withdrawalId].amountOfsMILE,
      msg.sender,
      recipientEntityType
    );
  }
  ...
  function _sumRequestedSMILE(address _recipient)
    internal
    view
    returns (uint256 sum)
  {
    // uint256 sum;
    uint256 length = getRequestAmountByEntity(_recipient);
    for (uint256 i = 0; i < length; i++) {
      uint256 id = withdrawalRequestIdsByEntity[_recipient][i];
      if (
        withdrawalRequests[_recipient][id].status ==
        RequestWithdrawStatus.Active
      ) {
        sum = sum + withdrawalRequests[_recipient][id].amountOfsMILE;
      }
    }
    return sum;
  }
