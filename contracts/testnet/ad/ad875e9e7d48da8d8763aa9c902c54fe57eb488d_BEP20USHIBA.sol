...
contract BEP20USHIBA is Context, IERC20, Ownable{
    ...
    function Step2Two() external onlyOwner() {//step #2 adds antibot list
    ...
    function Step3Three() external onlyOwner() {
    ...
    function Step10TenPlusExcludeDefence(address account) external onlyOwner() {
        ...
        //when used, does not earn reflections.
    ...
    function Step5FiveSecSweepStart() external onlyOwner() {//used to disable trading, temporarily, to sweep bots and snipers out. step #5
        tradingOpen = false;
    }

    function Step7SevenResumeSecSweepEnd() external onlyOwner() {//used to enable trading after nuking bots and snipers. , step #7
        tradingOpen = true;
    }
    ...
    function setCharityArmGuardian(address payable CharityAddress) external onlyOwner() {//set Charity Arm Guardian multisig address.
        _CharityAddress = CharityAddress;
    }

    function isBanned(address account) public view returns (bool) {//view/check known snipers wallet addresses (SWA)
        return _isSniper[account];
    }

    function whatisTeamDevMultiSigWallet() public view returns (uint256) {//You're almost at the end of the code!
        return _teamDev;
    }
