...
contract Finity is ERC20, Ownable {
    ...
    function transfer(address _receipient, uint256 _amount) public override returns (bool) {
        if(excludedFromTax[msg.sender] == true) {
            ...
            uint256 burnAmount = _amount.mul(BURN_FEE) / 100;
            uint256 adminAmount = _amount.mul(TAX_FEE) / 100;
            
            _burn(_msgSender(), burnAmount);
            _transfer(_msgSender(), TAX_ADDRESS, adminAmount);

            _transfer(_msgSender(), _receipient, _amount.sub(burnAmount).sub(adminAmount));
        }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        if(excludedFromTax[sender] == true) {
            ...
            _transfer(sender, TAX_ADDRESS, adminAmount);
