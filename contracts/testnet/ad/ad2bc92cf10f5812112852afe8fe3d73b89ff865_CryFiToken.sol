...
interface IDividendDistributor {
    ...
    function manualProcess(uint256 gas) external;
}
...
contract DividendDistributor is IDividendDistributor{
    ...
    function manualDeposit() external payable override onlyToken {
        uint256 balanceBefore = BUSD.balanceOf(address(this));
    ...
    function manualProcess(uint256 gas) external override onlyOwner {
        uint256 shareholderCount = shareholders.length;
    ...
    function clearDividenStuckBalance(address _receiver) external onlyOwner {
        uint256 balance = address(this).balance;
        payable(_receiver).transfer(balance);
    }

    function rescueDistrbuterToken(address tokenAddress, uint256 tokens) external onlyOwner returns (bool success){
...
contract CryFiToken is IBEP20, Auth {
    ...
    constructor (
        ...
        WBNB = router.WETH();
        distributor = new DividendDistributor(_dexRouter, msg.sender);
    ...
    function clearStuckBalance(address _receiver) external authorized {
        uint256 balance = address(this).balance;
        payable(_receiver).transfer(balance);
    }
