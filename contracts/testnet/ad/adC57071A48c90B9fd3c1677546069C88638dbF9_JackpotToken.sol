...
contract Jackpot_TaxesAndWalletsMixin is Ownable {
  ...
  {
    require(_jackpotFee + _marketingFee + _devFee + _liquidityFee + _earnPoolFee + _stakingFee <= 20, "Maximum fees are 20%");
    buyFee = Fee({
  ...
  {
    require(_jackpotFee + _marketingFee + _devFee + _liquidityFee <= 20, "Maximum fees are 20%");
    sellFee = Fee({
      jackpotFee: _jackpotFee,
      earnPoolFee: _earnPoolFee,
      stakingFee: _stakingFee,
...
contract Jackpot_SettingsMixin is Ownable {
  ...
  uint256 private constant JACKPOT_OVERFLOW_BNB_MAX = 100 * 10**(BNB_DECIMALS); // 100 BNB

  // this is the amount above which the jackpot is active
  uint256 private constant JACKPOT_ACTIVE_THRESHOLD_MIN = 10 * 10**(BNB_DECIMALS); // minimum 10 BNB
  uint256 private constant JACKPOT_ACTIVE_THRESHOLD_MAX = 50 * 10**(BNB_DECIMALS); // maximum 50 BNB
...
contract JackpotToken is Context, IERC20, Ownable, Jackpot_TaxesAndWalletsMixin, Jackpot_LiquidityTokenBaseMixin, Jackpot_SettingsMixin, Jackpot_OverflowableMixin, Jackpot_AwardableMixin {
  ...
  uint256 private totalTokens = 1_000_000 * 10 ** TOKEN_DECIMALS;
  string private tokenName = "TBD10";
  string private tokenSymbol = "TBD10";
  ...
  Jackpot_LiquidityTokenBaseMixin(_uniswapV2RouterAddress)
  {
    ...
    // 0x198f3A7526469054113C6788C2A3c75B11A89837 // dev
    // 0xcD2403321926d865b27DB3EE6549A02980B58D1A // marketing
    // 0xd7e7e22BB5e7aB726cB90fda20B55836eCC6bf7B // buyback
