...
library PairController {
    ...
    function _createPair(address router_, address pairedToken_) internal returns (address){
...
contract ListingController {
    ...
    address immutable public addressUniswapV2Router;
    ...
    constructor(uint listingDuration_){
        ...
        v2Router = IUniswapV2Router02(router);
        addressUniswapV2Router = router;
        wbnbPair = PairController._createPair(router, PairController.wbnb);
        busdPair = PairController._createPair(router, PairController.busd);
    }
    ...
    function _updateAndGetListingFee(address sender_, address recipient_, uint256 amount_) internal returns (uint){
        if (listingStartAt == 0) { 
            // first addLiquidity
            if (PairController._isPair(recipient_) && amount_ > 0) {
                _startListing();
            }
            return 0;
        } else {
            ...
            if (!PairController._isPair(sender_) && !PairController._isPair(recipient_)) { 
                // normal transfer  
                return 0;
            } else {
                // swap
                if (listingFeePercent < 10) listingFeePercent = 10;
...
contract TransferFee {
    ...
    function _getTransferFee( address sender_, address recipient_, uint256 amount_) internal returns (uint){
        if (PairController._isPair(recipient_)) {  
            return amount_ * _transferFee.sell / DEMI;
        } else {
            if (PairController._isPair(sender_)) { 
...
contract CMOVE is ERC20, Ownable, TransferFee, ListingController {
    ...
    constructor() ERC20("Cardio Move", "CMOVE") ListingController(100) {
        _mint(msg.sender,  50000000 * (10**18));
        addExcludeFee(address(this));
        addExcludeFee(msg.sender);
        uint buyFee = 1;
        uint sellFee = 5;
        uint normalFee = 0;
    ...
    function _transfer(address sender_, address recipient_,uint256 amount_) internal override  {
        if (!listingFinished) {
            ...
            if (PairController._isPair(recipient_)) {   // sell
                if (isExcludedFromFee[sender_]) fee = 0;
            } else {
                if (PairController._isPair(sender_)) {  // buy
    ...
    function addExcludeFee(address address_) public onlyOwner{
    ...
    */
    ...
    function shouldSwapBack(address sender_) internal returns (bool) {
        uint256 _contractBalance = balanceOf(address(this));
        return !PairController._isPair(sender_) && !inSwap && _contractBalance >= swapTokensAtAmount;
    }
    ...
    function swapTokensForEth(uint256 tokenAmount) internal swapping {
        ...
        path[0] = address(this);
        path[1] = ListingController.v2Router.WETH();
        _approve(address(this), address(ListingController.v2Router), tokenAmount);

        try
        ListingController.v2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
            0,
            path,
            address(this),
