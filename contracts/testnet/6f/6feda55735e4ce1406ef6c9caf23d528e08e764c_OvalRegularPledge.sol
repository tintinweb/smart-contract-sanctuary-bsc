...
contract OvalRegularPledge is Initializable, Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => uint256[])) private principleIndexMap;
    mapping(uint256 => mapping(address => uint256[])) private interestIndexMap;
    mapping(uint256 => mapping(address => BalanceInfo)) private balanceMap;
    mapping(uint256 => address[]) private walletsMap;
    ...
    function initialize() external initializer {
        ...
        poolMap[firstPoolId] = PledgePool(firstPoolId, 1672502400, 5184000);
        uint256 secondPoolId = autoIncrementId();
        poolMap[secondPoolId] = PledgePool(secondPoolId, 1672502400, 10368000);
        __Ownable_init();
        __Pausable_init();
    ...
    function pledge(uint256 amount, uint256 poolId) external whenNotPaused validPoolId(poolId) nonReentrant {
        ...
        require(amount >= (10 * (10 ** IBEP20(token).decimals())), "Pledge amount can't less than 10 tokens");
        
        balanceMap[poolId][_msgSender()].pledgeAmount = balanceMap[poolId][_msgSender()].pledgeAmount.add(amount);
        if(principleMap[poolId][_msgSender()].length <= 0) {
            walletsMap[poolId].push(_msgSender());
        }
        uint256 indexLength = principleIndexMap[poolId][_msgSender()].length;
        if (indexLength > 0) {
            uint256 index = principleIndexMap[poolId][_msgSender()][indexLength - 1];
            principleMap[poolId][_msgSender()][index] = Collect(
                autoIncrementId(),
                amount,
                getDeadline(poolMap[poolId].duration),
                poolId
            );
            principleIndexMap[poolId][_msgSender()].pop();
        }else {
        ...
        emit Pledge(_msgSender(), amount, primaryId, poolId, block.timestamp, getDeadline(poolMap[poolId].duration));
    }

    /**
     * group airdrop: the interests of Pledge
     */
    function groupAirdrop(AirdropItem[] calldata items) external whenNotPaused onlyOwner nonReentrant {
        for (uint256 i = 0; i < items.length; i++) {
            require(items[i].wallet != address(0), "Invalid token address");
            require(items[i].amount > 0 && items[i].pledgeIds.length > 0, "Invalid reward or Ids");
            airdrop(items[i]);
        }
    }
     ...
     */
    function airdrop(AirdropItem calldata item) private validPoolId(item.poolId) {
        require(principleMap[item.poolId][item.wallet].length > 0, "No pledge record found");
        balanceMap[item.poolId][item.wallet].interestAmount = balanceMap[item.poolId][item.wallet].interestAmount.add(item.amount);
        uint256 indexLength = interestIndexMap[item.poolId][item.wallet].length;
        if (indexLength > 0) {
            uint256 index = interestIndexMap[item.poolId][item.wallet][indexLength - 1];
            interestMap[item.poolId][item.wallet][index] = Collect(autoIncrementId(), item.amount, getDeadline(lockDuration), item.poolId);
            interestIndexMap[item.poolId][item.wallet].pop();
        }else {
    ...
    function claimPrinciple(uint256 pledgeId, uint256 poolId) external whenNotPaused validPoolId(poolId) nonReentrant {
        ...
        for (uint256 j = 0; j < principleMap[poolId][_msgSender()].length; j++) {
            if (principleMap[poolId][_msgSender()][j].id == pledgeId && principleMap[poolId][_msgSender()][j].deadline <= block.timestamp) {
                dueAmount = principleMap[poolId][_msgSender()][j].amount;
                savePrincipleIndex(poolId, j);
                isExits = true;
                break;
            }
        }
        require(isExits, "The 'id' has not reached the end time yet or not found");
        didClaimPrinciple(poolId, dueAmount);
    }
    ...
    */
    function claimAllDuePrinciple(uint256 poolId) external whenNotPaused validPoolId(poolId) nonReentrant {
        ...
        uint256 count = 0;
        for (uint256 j = 0; j < principleMap[poolId][_msgSender()].length; j++) {
            uint256 deadline = principleMap[poolId][_msgSender()][j].deadline;
    ...
    function didClaimPrinciple(uint256 poolId, uint256 dueAmount) private {
        require(token != address(0), "Invalid token address");
        balanceMap[poolId][_msgSender()].pledgeAmount = balanceMap[poolId][_msgSender()].pledgeAmount.sub(dueAmount, "Transfer amount exceeds pledge balance");
        IBEP20(token).safeTransfer(_msgSender(), dueAmount);
        emit ClaimPrinciple(token, address(this), _msgSender(), dueAmount);
    }

    function savePrincipleIndex(uint256 poolId, uint256 index) private {
        principleIndexMap[poolId][_msgSender()].push(index);
        delete principleMap[poolId][_msgSender()][index];
    }
    ...
    function claimAllDueInterest(uint256 poolId) external whenNotPaused validPoolId(poolId) nonReentrant {
        ...
        for (uint256 j = 0; j < interestMap[poolId][_msgSender()].length; j++) {
            uint256 deadline = interestMap[poolId][_msgSender()][j].deadline;
            if (deadline > 0 && deadline <= block.timestamp) {
                if (count > 240) {
                    break;
                }
                ...
                operationInterestIndex(poolId, j);
            }
        }
        require(dueAmount > 0, "No expired reward found");
        didClaimInterest(poolId, dueAmount);
    }
    ...
    function claimInterest(uint256 interestId, uint256 poolId) external whenNotPaused validPoolId(poolId) nonReentrant {
        ...
        bool isExits;
        for (uint256 j = 0; j < interestMap[poolId][_msgSender()].length; j++) {
            if (interestMap[poolId][_msgSender()][j].id == interestId && interestMap[poolId][_msgSender()][j].deadline <= block.timestamp && interestId > 0) {
                dueAmount = interestMap[poolId][_msgSender()][j].amount;
                operationInterestIndex(poolId, j);
                isExits = true;
                break;
            }
    ...
    function didClaimInterest(uint256 poolId, uint256 dueAmount) private {
        require(token != address(0), "Invalid token address");
        balanceMap[poolId][_msgSender()].interestAmount = balanceMap[poolId][_msgSender()].interestAmount.sub(dueAmount, "Transfer amount exceeds reward balance");
        IBEP20(token).safeTransfer(_msgSender(), dueAmount);
        emit ClaimInterest(token, address(this), _msgSender(), dueAmount);
    }
    
    function operationInterestIndex(uint256 poolId, uint256 index) private {
        interestIndexMap[poolId][_msgSender()].push(index);
        delete interestMap[poolId][_msgSender()][index];
    }
    ...
    function withdraw(address _token, uint256 amount) external onlyOwner whenNotPaused {
        require(_token != address(0) && amount > 0, "Invalid token address or Withdraw amount can't be zero");
    ...
    // get the balance of 'address', (address, pledgeAmount, interestAmount)
    function getBalance(uint256 poolId, uint256 index) external onlyOwner validPoolId(poolId) view returns (address, uint256, uint256) {
        require(index < walletsMap[poolId].length, "Index out of bounds");
        return (walletsMap[poolId][index], balanceMap[poolId][walletsMap[poolId][index]].pledgeAmount, balanceMap[poolId][walletsMap[poolId][index]].interestAmount);
    }

    // get walletsMap the number of wallets
    function getWalletsLength(uint256 poolId) external onlyOwner validPoolId(poolId) view returns (uint256) {
        return walletsMap[poolId].length;
    }
