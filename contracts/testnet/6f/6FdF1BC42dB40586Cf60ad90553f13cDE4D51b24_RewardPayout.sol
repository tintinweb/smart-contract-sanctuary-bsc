...
contract RewardPayout is Context, Ownable {
    ...
    function earned(address account) public view returns (uint256) {
        ...
        if (_lastTimeRewardApplicable <= 0) {
            return 0;
        }
        return rewards[account] * (_lastTimeRewardApplicable - periodStart) / (_term);
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    function setPeriodStart(uint256 periodStart_) external onlyOwner {
        periodStart = periodStart_;
        _term = periodFinish - periodStart;
    ...
    function emergencyAssetWithdrawal(address asset) external onlyOwner {
        ...
        SafeERC20.safeTransfer(token,Ownable.owner(), token.balanceOf(address(this)));
    }
    ...
    function getReward() public {
        require(block.timestamp > periodStart, "RewardPayout: Vesting has not started yet!");

        uint256 _earned = earned(_msgSender());
        require(_earned <= rewards[_msgSender()], "RewardPayout: earned is more than reward!");
        require(_earned > payouts[_msgSender()], "RewardPayout: earned is less or equal to already paid!");

        uint256 reward = _earned - (payouts[_msgSender()]);

         emit RewardPaid(_msgSender(), reward);

        if (reward > 0) {
            payouts[_msgSender()] = _earned;
            //wgToken.safeTransfer(msg.sender, reward);
            address payable beneficiaryPayable = payable(_msgSender());
            SafeERC20.safeTransferFrom(wgToken,Ownable.owner(), beneficiaryPayable, reward);
            emit RewardPaid(beneficiaryPayable, reward);
        }
    }
}
