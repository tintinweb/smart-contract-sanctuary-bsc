...
@@@@@@@@@@@@@@@@@@@@@@@@@@@@@&#B####&@@@#! .~!!^~~^Y#BBB[email protected]@@[email protected]@@&BGBB&@@@@@@@@@@@@@@@@@@@@@@@@@@
...
contract NanoMinions {
    ...
    function participateNanottery(uint _ticketCount) external onlyNanoMinionsPlayer {
        uint nowInviteRebatePrice = calculateRobotCleam(cleamedRobot[msg.sender]);
        ...
        uint robot = calculateRobotClone(buyPrice, BUSD.balanceOf(address(this)));
        marketRobot += robot;
        ...
        if(cleamedRobot[msg.sender] >= robot) {
            ...
            cleamedRobot[msg.sender] = 0;
            buyPrice = calculateRobotCleam(robot);

            uint nowWorksTime = min(ROBOT_TO_WORKING_WORKER, block.timestamp - lastWorking[msg.sender]);
            uint useTime = nowWorksTime * buyPrice / nowRobotCleamPrice;
