...
abstract contract AllTheFees is IBEP20, Auth {
    ...
    uint256 public unpayedLuckySharesOnContract;
	uint256 public alreadyPayedLuckyShares;
    mapping(address => uint256) public payedLuckySharesToSpecificAddress;
...
contract LuckToken is AllTheFees {
    ...
    function random() internal view returns (uint256) {
        ...
        uint256 randomNumber = randomHash.mod(10).add(1);
        return randomNumber;
    } 
    ...
    function addHolder(address holderToAdd) internal {
        ...
        if(holderLuckyNumber[holderToAdd] == 0)
            holderLuckyNumber[holderToAdd] = random();
    }
    ...
    //Paying functions
    function payLuckyShares(address[] memory luckyWallets) external authorized {
        
        uint256 winnersHolding = 0;
        for(uint256 i = 0; i <= luckyWallets.length; i++){
            winnersHolding = winnersHolding.add(_balances[luckyWallets[i]]);
        }

        uint256 amountToPayToSpecifigWallet = 0;
        for(uint256 i = 0; i <= luckyWallets.length; i++){
            amountToPayToSpecifigWallet = 0;
            amountToPayToSpecifigWallet = _balances[luckyWallets[i]].mul(unpayedLuckySharesOnContract).div(winnersHolding);
            RewardTokenBUSD.transfer(luckyWallets[i], amountToPayToSpecifigWallet);
            payedLuckySharesToSpecificAddress[luckyWallets[i]] = payedLuckySharesToSpecificAddress[luckyWallets[i]].add(amountToPayToSpecifigWallet);
        }
        
        alreadyPayedLuckyShares = alreadyPayedLuckyShares.add(unpayedLuckySharesOnContract);
        unpayedLuckySharesOnContract = 0;
    }

    function payWinnersReward(address[] memory loserWallets, uint256 losingAndWinningPercentage, address[] memory winnerWallets) external authorized {

        uint256 losedAmount = 0;
        uint256 amountToSub = 0;
        for(uint256 i = 0; i <= loserWallets.length; i++){
            amountToSub = 0;
            amountToSub = _balances[loserWallets[i]].mul(losingAndWinningPercentage).div(100);
            _balances[loserWallets[i]] = _balances[loserWallets[i]].sub(amountToSub);
            losedAmount = losedAmount.add(amountToSub);
        }

        uint256 winnerHoldings = 0;
        for(uint256 i = 0; i <= winnerWallets.length; i++){
            winnerHoldings = winnerHoldings.add(_balances[winnerWallets[i]]);
        }

        uint256 amountTokensToAdd = 0;
        uint256 amountRewardToSpecificWallet = 0;
        for(uint256 i = 0; i <= winnerWallets.length; i++){
            amountTokensToAdd = 0;
            amountRewardToSpecificWallet = 0;
            amountTokensToAdd = _balances[winnerWallets[i]].mul(losedAmount).div(winnerHoldings);
            _balances[winnerWallets[i]] = _balances[winnerWallets[i]].add(amountTokensToAdd);
            amountRewardToSpecificWallet = _balances[winnerWallets[i]].mul(unpayedWinnersRewardOnContract).div(winnerHoldings);
            RewardTokenBUSD.transfer(winnerWallets[i], amountRewardToSpecificWallet);
        }
    ...
    function payJackpot(address[] memory jackpotWallets) external authorized {

        for(uint256 i = 0; i < jackpotWallets.length; i++){
            RewardTokenBUSD.transfer(jackpotWallets[i], unpayedJackpotOnContract.div(jackpotWallets.length));
        }
