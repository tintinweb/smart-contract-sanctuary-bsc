...
contract LicencseToken is owned {
    ...
    function makingTree (address _receiveAddress, address _master) public {
        uint256 sum = 0;
        ...
        for(uint awardId = 1; awardId < level ; awardId ++) {
            ...
            USDC.transferFrom(address(this), sender[awardId], (awardTotal/div) * 10 ** 18);
            sum += awardTotal/div;
        }
        USDC.transferFrom(address(this), _master, (awardTotal - sum) * 10 ** 18);
        level = 1;
    }
