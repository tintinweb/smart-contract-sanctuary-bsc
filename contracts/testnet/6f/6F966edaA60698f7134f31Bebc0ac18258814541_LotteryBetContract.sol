...
contract LotteryBetContract is Ownable{
    ...
    //竞猜平台竞猜NFT
    address public _LotteryNFTTokenContract= 0x00E85E4e35b9a3806e05313F3cB62cc2F5380ef3;
    ...
    function sendBetPool()private view returns(uint256){
        uint256 prizePoolValue=100;
        if(_IA[_msgSender()]==poolAddress){
            return prizePoolValue.sub(buyBackFee).sub(poolFee).sub(platformFee);
        }else{
            return prizePoolValue.sub(inviteAwardFee).sub(platformFee);
        }
    }
    ...
    //key: tokenId  key2:ways  value:price
    mapping(uint256 =>mapping(uint8=>uint256)) public _TWP;
    ...
    function betOrder(uint256[] memory _tokenId,uint8[] memory ways,string[] memory wayValue,uint256[] memory betMoney,address inviteAddress)external returns(bool){
        ...
        for(uint index=0;index<_tokenId.length;index++){
            ...
            if(_BWTUM[_tokenId[index]][ways[index]]<=0){
            ...
            _BP[_tokenId[index]][ways[index]]+=betMoney[index].mul(nftKolFee).div(100);
            _BWTUM[_tokenId[index]][ways[index]]+=betMoney[index].mul(prizePool()).div(100);
            _TWP[_tokenId[index]][ways[index]]+=betMoney[index];
    ...
    function queryLotteryWayBet(uint256 tokenId,uint8[] memory ways) external view returns(uint256[] memory wayBonus,uint256[] memory wayMoney,uint256[] memory receiveBonus){
        ...
        for(uint index=0;index<ways.length;index++){
            ...
            wayMoney[index]= _TWP[tokenId][ways[index]];
