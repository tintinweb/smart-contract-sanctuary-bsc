...
contract Moc is ERC20Detailed, Ownable {
    ...
    address public inviteRewardReciver;
    ...
    function takeFee(
        ...
        if (from == mocPair && to != mocPair) {
            ...
            if (IERC20(usdtAddress).balanceOf(mocPair) >= reserve0 + amountA) {
                _totalFee = buyInviteFee.add(buyBackExiFee).add(buyLiqMocFee);
                uint256 tempFee = 0;
                
                tempFee = gonAmount.mul(buyInviteFee).div(feeDenominator).div(
                    _gonsPerFragment
                );
                _balances[inviteRewardReciver] += tempFee;
                unTotalSupply += tempFee;
                emit Transfer(from, inviteRewardReciver, tempFee);

                totalInviteAmount = totalInviteAmount.add(
                    tempFee.div(_gonsPerFragment)
                );
    ...
    function setReceiver(
        ...
        inviteRewardReciver = _inviteRewardReciver;
    }
}