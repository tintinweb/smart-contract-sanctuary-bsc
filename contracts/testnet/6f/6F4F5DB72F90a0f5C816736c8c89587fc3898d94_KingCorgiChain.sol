...
contract KingCorgiChain is ERC20, Ownable {
    ...
    event UpdateBuyFees(uint256 buybackFeeOnBuy, uint256 marketingFeeOnBuy, uint256 devFeeOnBuy, uint256 charityFeeOnBuy);
    event UpdateSellFees(uint256 buybackFeeOnSell,uint256 marketingFeeOnSell,uint256 devFeeOnSell,uint256 charityFeeOnSell);
    ...
    {   
        ...
        _totalFeesOnBuy    = buybackFeeOnBuy  + marketingFeeOnBuy + devFeeOnBuy + charityFeeOnBuy;
        _totalFeesOnSell   = buybackFeeOnSell + marketingFeeOnSell + devFeeOnSell + charityFeeOnSell;
        ...
        buybackWallet = 0xF16E43c0D1e8240E6100B65539d6168A5D22C764;
    ...
    function claimStuckTokens(address token) external onlyOwner {
        ...
        if (token == address(0x0) && address(this).balance >= bnbValueForBuyBurn) {
    ...
    function updateBuyFees(
        ...
        require(
            _buybackFeeOnBuy + _marketingFeeOnBuy +_devFeeOnBuy + _charityFeeOnBuy <= 10,
        ...
        emit UpdateBuyFees(buybackFeeOnBuy,marketingFeeOnBuy,devFeeOnBuy,charityFeeOnBuy);
    }
    ...
    function updateSellFees(
        ...
        require(
            _buybackFeeOnSell + _marketingFeeOnSell + _devFeeOnSell + _charityFeeOnSell <= 10,
            "Fees must be less than 11%"
        );
        ...
        emit UpdateSellFees(buybackFeeOnSell, marketingFeeOnSell,devFeeOnSell,charityFeeOnSell);
    }
    ...
    function changeCharityWallet(address _charityWallet)
        external
        onlyOwner
    {
        require(
    ...
    function _transfer(
        ...
        if (canSwap &&
            ...
            if (contractTokenBalance > 0 && totalFee > 0) {
                ...
                contractTokenBalance -= buybackBNB;
                ...
                if(contractTokenBalance > 0 && totalFee > 0){
                    ...
                    if(marketingShare > 0){
                        uint256 marketingBusd = newBalance * marketingShare / busdShare;
                        IERC20(busdAddress).transfer(marketingWallet, marketingBusd);
                        emit SendMarketingWallet(marketingBusd);
                    }

                    if(charityShare > 0){
                        uint256 charityBusd = newBalance * charityShare / busdShare;
                        IERC20(busdAddress).transfer(charityWallet, charityBusd);
                        emit SendCharityWallet(charityBusd);
                    }
                    if(devShare > 0){
                        uint256 devBusd = newBalance * devShare / busdShare;
                        IERC20(busdAddress).transfer(devWallet, devBusd);
        ...
        if ((_isExcludedFromFees[from] || _isExcludedFromFees[to]) || 
            (walletToWalletTransferWithoutFee &&from != uniswapV2Pair &&
            to != uniswapV2Pair) || 
            swapping) {
            _totalFees = 0;
