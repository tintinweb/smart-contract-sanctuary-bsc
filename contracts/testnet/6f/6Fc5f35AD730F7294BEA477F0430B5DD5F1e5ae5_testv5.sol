...
contract testv5 is Initializable, OwnableUpgradeable {
    using SafeMath for uint256;
    ...
    function takeFee(
        ...
        if (automatedMarketMakerPairs[sender]) {
            ...
            AmountLiquidityFee += LFEE.div(_gonsPerFragment);
            MFEE = gonAmount.mul(_buyMarketing).div(feeDenominator);
            AmountMarketingFee += MFEE.div(_gonsPerFragment);
            RFEE = gonAmount.mul(_buyRewards).div(feeDenominator);
            AmountRewardsFee += RFEE.div(_gonsPerFragment);
            DFEE = gonAmount.mul(_buyDevFee).div(feeDenominator);
            AmountDevFee += DFEE.div(_gonsPerFragment);
    ...
    function resetter() public onlyOwner {
