...
library rigelMapped {
    struct libStorage {
        ...
        mapping(address => uint256)  MerchantFee;
    ...
    function _isOnDispute(
        ...
        DisputeRaised memory rDispute = DisputeRaised(
            who, against, token, amount, fee , block.timestamp, block.timestamp + _join, 0, false
        );
...
abstract contract Rigelp2pPriceSet is Ownable, events {    
    ...
    function setAssetPrice(address _asset, uint256[] memory _tadeAmount, bytes[] memory _ranks, uint256[] memory sellerFee, uint256[] memory buyerFee, uint256 merchant) external onlyOwner {
        ...
        ds.MerchantFee[_asset] = merchant;
    }

    function resetAssetPrice(uint256 assetID, uint256 seller, uint256 buyer) external onlyOwner {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
    ...
    function getBatchTokenFeeForMerchant(address[] memory _asset) external view returns(uint256[] memory _fee) {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
        _fee = new uint256[](_asset.length);
        for ( uint256 i; i < _asset.length; i++) {
            _fee[i] = ds.MerchantFee[_asset[i]];
        }
        return _fee;
    }

    function getAssetPrice(uint256[] memory assetID) external view returns(uint256[] memory seller, uint256[] memory buyer, bool[] memory) {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
        uint256 _assetLength = assetID.length;
        seller = new uint256[](_assetLength);
        buyer = new uint256[](_assetLength);
        bool[] memory _added = new bool[](_assetLength);
        for (uint256 i; i < _assetLength; i++) {
            TokenDetails memory tD = ds.tokenDetails[assetID[i]];
            (seller[i], buyer[i]) = _getAssetPrice(assetID[i]);
            _added[i] = _isAssestAdded(tD.token);
        }
        return (seller, buyer, _added);
    }

    function getLengthOfTokens() public view returns(uint256) {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
        return ds.tokenDetails.length;
    }

    function getTokenFeeFormerchant(address _asset) internal view returns(uint256) {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
        return ds.MerchantFee[_asset];
    }
    ...
    function _getAssetPrice(uint256 assetID) internal view returns(uint256 seller, uint256 buyer) {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
        TokenDetails memory tD = ds.tokenDetails[assetID];
        seller = tD.sellerFee;
        buyer = tD.buyerFee;
    }
...
contract RigelDecentralizedP2PSystem is Rigelp2pPriceSet {
     ...
     * @param _sellerFee this fee should not be included with decimals, as the price of token that user trade price and decimals are unknown;
    ...
    */
    ...
    function makeSellOrder(IERC20 purchaseToken, uint256 amount, address from, address to, address referral) external onlyAdmin returns(uint256){   
        ...
        // bool haveIReferred = ds.hasReferral[_msgSender()];
        address referred = ds.whoIreferred[to];

        settleReferralRewards(_s_seller, to, referral);

        if (referred != address(0)) {            
            uint256 refShare = (_s_seller * ds.refWithoutWhitelistPercent) / 100E18;
            contractTransfer(address(ds.RGP), referred, refShare);  
        }
        ds.allTrade[address(purchaseToken)] += tDecimals;
    ...
    function settleReferralRewards(uint256 s_seller, address to,  address referral) internal {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();   
        if (referral != address(0) && !(ds.hasBeenReferred[referral])) {
            ds.hasBeenReferred[referral] = true;
            ds.whoIreferred[referral] = to;
            if(!ds.refWhitelist[referral]) {
                uint256 refShare = (s_seller * ds.refWithoutWhitelistPercent) / 100E18;
                contractTransfer(address(ds.RGP), referral, refShare);  
            } else {
                uint256 specialReferral = ds.refSpecialWhitelist[referral];
                if(specialReferral != 0) {
                    uint256 refWShare = (s_seller * specialReferral) / 100E18;
                    contractTransfer(address(ds.RGP), referral, refWShare); 
                } else {
                    uint256 refWShare = (s_seller * ds.refWithWhitelistPercent) / 100E18;
                    contractTransfer(address(ds.RGP), referral, refWShare);  
                }
            }
        }
    ...
    function makeBuyPurchase(IERC20 purchaseToken, address from, uint256 productID, uint256 fee) external onlyAdmin {
        ...
        if (fee < getTokenFeeFormerchant(address(purchaseToken)))  revert Low_Gas(); 
        all.isCompleted = true;
        all.endSales = block.timestamp;
        address s_token = all.token;
        address s_buyer = all.buyer;
        uint256 subGas = all.amount - fee;
        contractTransfer(s_token, s_buyer, subGas);  
        emit Buy(s_buyer, from, s_token, subGas, productID, block.timestamp);
    }

    function raiseDispute(uint256 productID, address who) external onlyAdmin{
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();  
        AllProducts storage all = ds.allProducts[productID];
        ...
        if (_who) {
            ...
            if (len > 0) {
                for (uint256 i; i < len; i++){
                    ...
                    _update(voters);
                    contractTransfer(s_token, voters, forEach);
                }
            }   
            contractTransfer(s_token, who, all.amount);  
    ...
    function joinDispute(uint256 productID) external {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage(); 
        DisputeRaised storage disputeRaise = ds.raisedDispute[productID][0];
        ...
        if (tipMe.joinedCouncilMembers.length == ds.maxNumbersofCouncils) {
            uint256 commence = disputeRaise.votesCommence;
            if (block.timestamp < commence)  revert CompletedMember();
            if (block.timestamp > commence)  revert VoteCommence();
        }
        ...
        if (tipMe.joinedCouncilMembers.length == ds.maxNumbersofCouncils) { 
            disputeRaise.votesEnded = block.timestamp + ds.votingEllapseTime;
        }
    ...
    function _tip(uint256 productID, address who) internal {
        ...
        if(s_qualified != address(0)) {        
            ...
            contractTransfer(s_token, s_qualified, all.amount);  
            if(s_qualified == all.buyer) { 
    ...
    function _l(uint256 productID, uint256 amountShared, uint256 fee,address token, address who) internal {
        ...
        for (uint256 j = 0; j < meme; j++) {
            ...
            contractTransfer(token, consM, forEach);   
    ...
    function getDisputeFees(uint256 _amountBeenTraded, address token) public view returns(
        ...
        uint256 lent = getLengthOfTokens();
        for (uint256 i; i < lent; i++) {
     ...
     */
    function getFees() external view returns (uint256 _sellerFee) {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
    ...
    */
    function resetFee(uint256 _sellerFee) external onlyOwner {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
        ds.sellerFee = _sellerFee;
    }
     ...
     */
    function setWhiteList(address[] memory accounts, uint256[] memory percent, bool status) external onlyOwner {
        rigelMapped.libStorage storage ds = rigelMapped.diamondStorage();
