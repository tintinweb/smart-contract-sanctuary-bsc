...
contract Oceanft is ERC721Enumerable, TrustCaller {
    ...
    string[] public collections; //TODO will try to remove it
    mapping(string => string[]) public collectionRanks;
    mapping(string => bool) public collectionMap;
    ...
    function allRanksInCollection(string memory name_) external view returns (string[] memory){
        return collectionRanks[name_];
    }

    function tokenInfo(uint256 tokenId) external view returns (string memory collection, string memory rank){  
        return (tokenIdToCollection[tokenId], tokenIdToRank[tokenId]);
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory uri) {
        require(_exists(tokenId), "token does not exist");
        uri = collectionRankURIMap[tokenIdToCollection[tokenId]][tokenIdToRank[tokenId]];
    }
    ...
    function createCollectionRankWithURI(string memory name_, string memory rank_, string memory uri_) external onlyTrustCaller {
        require(!collectionMap[name_], "collection does not exist");
        require(bytes(collectionRankURIMap[name_][rank_]).length != 0, "rank is duplicated");
        
        collectionRanks[name_].push(rank_);
        collectionRankURIMap[name_][rank_] = uri_;
        emit CollectionRankURICreated(name_, rank_, uri_);
    }
    function mint(string memory name_, string memory rank_) external onlyTrustCaller {
        require(bytes(collectionRankURIMap[name_][rank_]).length == 0, "collection and rank is not found");
