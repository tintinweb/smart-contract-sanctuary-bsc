...
contract AV2 is Context, IERC20, Ownable {
    
    string constant private _name = "AV2";
    string constant private _symbol = "AV2";
    ...
    function updateLaunchRestrictions() private {
        if((block.number - launchBlock) > 4 minutes && (block.number - launchBlock) < 6 minutes) {
            maxBuy = maxBuyStage2;
