...
contract TheSoilverseMarketV6 is
    ...
    /**
     * @notice Event to keep track when the NFT issued(without token) from owner account to another account.
     * @param tokenId: The new tokenId.
     * @param to: The address to whom token minted.
     * @param price: The price in dollars.
     */
    event TokenIssuedToAddressWithoutERC20(
        uint256 tokenId,
        address to,
        uint256 price
    );
    ...
    /**
     * @notice Issuing NFT to a buyer without ERC20 tokens.
     * @param to: The next owner address.
     * @param uri: The token URL.
     * @param priceInWei: The token price in wei.
     */
    function issueNFTToAddressWithoutERC20 (
        address to,
        string calldata uri,
        uint256 priceInWei,
        uint256 noOfMonths
        ...
        /// @notice Emiting the event.
        emit TokenIssuedToAddressWithoutERC20(tokenId, to, priceInWei);
    }
     ...
     */
    function updateHoldingPeriod(uint256 tokenId, uint256 noOfMonths) external nonReentrant whenNotPaused {
       /// @notice Requirements.
        _requireManager(msg.sender);
     ...
     */
    function transferERC20Token(address tokenAddress, address to, uint256 amount) external whenNotPaused nonReentrant {
        /// @notice Requirements.
        _requireOwner(msg.sender);
        require(
