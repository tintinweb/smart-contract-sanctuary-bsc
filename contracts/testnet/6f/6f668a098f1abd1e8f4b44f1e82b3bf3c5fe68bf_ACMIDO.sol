...
contract ACMIDO is Ownable, ReentrancyGuard {
    ...
    function addPosition(uint startAmount) external onlyOwner {
        ...
        // 初始化第一期
        epochs[pl].push(Epoch(startTime, startTime + 5 * 60 * 60, startAmount * 10**18, 0, 0, 0, pl, 0));
        // 初始化第二期
        epochs[pl].push(Epoch(startTime + 6 * 60 * 60, startTime + 9 * 60 * 60, startAmount * 10**18, 0, 0, 0, pl, 1));
    }
