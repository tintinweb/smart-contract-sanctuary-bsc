...
contract EPTX is IERC20 {
    ...
    event SetLeaveEarlyFee(uint256 newFee);
    event SetLeaveEarlyFeeTimer(uint256 newTimer);
    ...
    function setLeaveEarlyFee(uint256 newLeaveEarlyFee) external onlyOwner {
        require(newLeaveEarlyFee <= 800, "Early Fee Too High");
    ...
    function setLeaveEarlyFeeTimer(uint256 newLeaveEarlyFeeTimer)
        ...
        onlyOwner
    {
        require(newLeaveEarlyFeeTimer <= 10**8, "Fee Timer Too High");
     ...
     */
    function withdraw(uint256 amount, bool forETH)
        public
        nonReentrant
        returns (uint256)
    {
     ...
     */
    function _registerFirstPurchase(address user, uint256 received) internal {
    ...
    function _mintTo(
        ...
        uint256 nToMint = (_totalSupply.mul(received).div(previousBalance));
