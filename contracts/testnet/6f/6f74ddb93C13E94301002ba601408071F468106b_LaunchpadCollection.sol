...
contract LaunchpadCollection is ERC721, Ownable, ERC721Royalty{
    ...
    uint256 private _numTokensReserved;
    ...
    mapping(address => uint256) public TokenToMintPriceInWei;

    event LogSetMaxMintAmount(uint256 num);
    event LogSetMintPriceInWei(uint256 mintPriceInWeiBNB);
    ...
    event LogSetMintPriceForToken(address _token, uint256 price);
    event LogSetValidERC20(address _token, bool _isValid);
    event LogSetMaxTotalSupply(uint256 _maxTotalSupply);
    event LogSetServiceFee(uint256 fee);
    ...
    constructor(
        ...
        address _empire, 
        address _feeAddr) 
        ERC721(strings[1], strings[2]) {
        
        require(_empire != address(0), "_empire: cannot be address zero");
        ...
        // Set variables
        maxMintAmount = uints[0];
        mintPriceInWeiBNB = uints[1];
        maxTotalSupply = uints[2];
        mintStartTime = uints[3];

        // Set valid erc20
        validERC20[_empire] = true;
    
        // Set fee addr
        feeAddr = _feeAddr;

        // Transfer ownership
        _transferOwnership(addys[2]);
    ...
    function setMaxMintAmount(uint256 _maxMintAmount) external onlyOwner{
        require(maxMintAmount != _maxMintAmount,"Already set to this Value");
        require(block.timestamp >= mintStartTime, "Mint already started");
        maxMintAmount = _maxMintAmount;
        emit LogSetMaxMintAmount(_maxMintAmount);
    }

    function setMaxTotalSupply(uint256 _maxTotalSupply) external onlyOwner{
        require(maxTotalSupply != _maxTotalSupply, "Already set to this Value");
        require(block.timestamp >= mintStartTime, "Mint already started");
    ...
    function setMintPriceForToken(address _token, uint256 price) external onlyOwner{
        require(_token != address(0),"cannot be address zero");
        require(TokenToMintPriceInWei[_token] != price,"Already set to this price");
        require(block.timestamp >= mintStartTime, "Mint already started");
        TokenToMintPriceInWei[_token] = price;
        emit LogSetMintPriceForToken(_token, price);
    }

    function setValidErc20(address _token, bool _isValid) external onlyOwner{
        require(_token != address(0), "Cannot be zero address");
        require(validERC20[_token] != _isValid, "Already set");
        validERC20[_token] = _isValid;
        emit LogSetValidERC20(_token, _isValid);
    }

    function reserveTokens(uint256 num) external onlyOwner{
        require(num != 0,"reserveTokens:cannot be zero");
    ...
    function unReserveTokens(uint256 num) external onlyOwner{
        require(num != 0,"unreserveTokens:cannot be zero");
        require(_numTokensReserved >= num, "Insufficient amount of tokens");
        _numTokensReserved -= num;
    ...
    function enableMinting() external onlyOwner{
        require(!mintingEnabled, "Minting already enabled");
        mintingEnabled = true;
        emit LogEnableMinting();
    }

    function disableMinting() external onlyOwner{
        require(mintingEnabled, "Minting already disabled");
    ...
    function changeBaseURI(string memory baseURI_) public onlyAdminOrOwner {
        baseURI = baseURI_;
        emit LogChangeBaseURI(baseURI_);
    }

    function changeRevealed(bool _revealed) public onlyAdminOrOwner {
        revealed = _revealed;
    }
    
    function mint(uint256 num, address _token) external payable {
        uint256 supply = totalSupply();
        require(num != 0,"num cannot be zero");
        require(num <= maxMintAmount, "Exceeds max token mint for user");
        require(mintingEnabled, "Minting disabled");
        require(supply + num <= maxTotalSupply, "Exceeds max supply");
        require(block.timestamp >= mintStartTime, "Mint haven't started yet");
        if(_tokensReserved = true){
            require(supply + num <= maxTotalSupply - _numTokensReserved, "Exceeds total supply");
        }
        if(_token != address(0)){
            uint256 mintPriceInWei = TokenToMintPriceInWei[_token];
            uint256 amount = num * mintPriceInWei;
            uint256 fee = _calculateServiceFee(amount);
            require(validERC20[_token], "Invalid ERC20");
            require(IERC20(_token).balanceOf(msg.sender) >= amount + fee, "Insufficient balance.");
            require(IERC20(_token).allowance(msg.sender, address(this)) >= amount + fee, "Insufficient allowance.");
            ...
            IERC20(_token).transferFrom(msg.sender, feeAddr, fee);
        } else {
            uint256 amount = num * mintPriceInWeiBNB;
            uint256 fee = _calculateServiceFee(amount);
            require(msg.value >= amount + fee, "Not enough bnb sent");
            for(uint256 i; i < num; i++){
                _tokenIds.increment();
                uint256 newItemId = _tokenIds.current();
                _totalSupply += 1;
        ...
        returns (uint256)
    {
        ...
        require(serviceFee > 0,"ServiceFee not set");
...
contract LaunchpadCollectionMinter is Ownable{

    using Counters for Counters.Counter;
    Counters.Counter private _collectionIds;

    address public empire; 
    address public feeeAddr;

    mapping(address => mapping(uint256 => address)) userCollection;
    mapping(uint256 => address) ownerOf;
    mapping(address => bool) isWL;

    event NewCollection(address collection, address owner, uint256 id);
    event LogAddToWL(address acc);
    
    
    function createNewCollection(string[] memory strings, address[] memory addys, uint256[] memory uints) external{
        require(isWL[msg.sender], "Only for whitelisted");
        _collectionIds.increment();
        uint256 newCollectionId = _collectionIds.current();
        LaunchpadCollection collection = new LaunchpadCollection(strings, addys, uints, empire, feeeAddr);
        address collectionAddress = address(collection);
        userCollection[msg.sender][newCollectionId] = collectionAddress;
        ownerOf[newCollectionId] = msg.sender;
        emit NewCollection(collectionAddress, msg.sender, newCollectionId);
    }
    ...
    function addToWL(address acc) external onlyOwner{
        require(isWL[acc] != true, "Already whitelisted");
        isWL[acc] = true;
        emit LogAddToWL(acc);
    }
}
