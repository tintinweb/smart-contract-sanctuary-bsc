...
contract StakingPool is InjectorContextHolder, IStakingPool {
    ...
    function stake(address validator, uint256 amount) external advanceStakingRewards(validator) override {
        ValidatorPool memory validatorPool = _getValidatorPool(validator);
        ...
        _STAKING_CONTRACT.delegate(validator, amount);
        // emit event
    ...
    function claim(address validator) external advanceStakingRewards(validator) override {
        ...
        require(IERC20(_CHAIN_CONFIG_CONTRACT.getRewardToken()).balanceOf(address(this)) >= amount, "not enough balance");
        IERC20(_CHAIN_CONFIG_CONTRACT.getRewardToken()).safeTransfer(msg.sender, amount);
