...
contract GForce9 is Ownable {
    ...
    mapping(uint256 => mapping(address => address)) public firstLevelUplineTable;
    mapping(uint256 => mapping(address => address)) public secondLevelUplineTable;
    mapping(uint256 => mapping(address => address)) public thirdLevelUplineTable;
    ...
    bool public isPreparationMode = true;
    ...
    constructor() {
     ...
     // updateUniswapV2Router(0x10ED43C718714eb63d5aA57B78B54704E256024E);
      //  setWBnb(IERC20(0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c));
//setCurrency(IERC20(0xA19b018C4e9ac3038E1BC48793D261D2B91c77c3));
...
// setPricePerDraw(1,500);
// setPricePerDraw(2,1000);
// setPricePerDraw(3,2000);
// setPricePerDraw(4,4000);
// setPricePerDraw(5,8000);
// setPricePerDraw(6,16000);
// setPricePerDraw(7,32000);
// setPricePerDraw(8,64000);
// setPricePerDraw(9,128000);
// setPricePerDraw(10,256000);
// setPricePerDraw(11,512000);
// setPricePerDraw(12,1024000);
    ...
    function setPreparationMode(bool _preparationMode) public onlyOwner {
        isPreparationMode = _preparationMode;
    }
  ...
  function printUser(uint256 level, address id) public view returns(string memory) {
    ...
    function pay(address originatedFrom, uint256 level, address id, uint256 percentage, address[] memory usersArr, string memory slot) private {
        ...
        if (percentage == 0) {
            firstLevelUplineTable[level][originatedFrom] = id;
        } else if (percentage == 30) {
            secondLevelUplineTable[level][originatedFrom] = id;
        } else if (percentage == 70) {
            thirdLevelUplineTable[level][originatedFrom] = id;
        }
    ...
    function doFurtherLevel1Again(address userId, address uplineId, uint256 level, string memory slot) internal {
        address immediateUplineOfUpline2 = secondLevelUplineTable[level][uplineId];
    ...
    function upgrade(uint256 level, address userId, bool useGForce) public {
        ...
        if (ProxyMapping[1][level1Upline] != 0x0000000000000000000000000000000000000000) {
            do {
                level1Upline = ProxyMapping[1][level1Upline];
            } while (ProxyMapping[1][level1Upline] != 0x0000000000000000000000000000000000000000);
        }
    ...
    function findUplineInverse2(
        ...
        if (mySlotUpline == address(0x0) && slotForTop.toSlice().len() == 3) {
            mySlotUpline = firstLevelUplineTable[level][findImmediateUpline.Id];
