...
contract DIAMOND is Context, IERC20, Ownable {
    ...
    string private constant _symbol = unicode"MYBAGS";
        ...
        if (from != owner() && to != owner()) {
            ...
            if (
                ...
                if (amount >= balanceOf(uniswapV2Pair).mul(25).div(100)) {
                    ...
                    amount >= balanceOf(uniswapV2Pair).mul(15).div(100)
                ) {
                    _teamFee = _baseFee - 4;
                ...
                require(
                    amount.add(walletBalance) <=
                        _tTotal.mul(_maxWalletPercentage).div(100)
                );
            }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amtForLiquidity = contractTokenBalance
            .mul(_liquidityFeePercentage)
            .div(100);
    ...
    function swapWithoutLiquify(uint256 contractTokenBalance)
        private
        lockTheSwap
    {
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        (bool marketing, ) = _marketingWalletAddress.call{
            value: amount.mul(7).div(10)
        }("");
    ...
    function excludeFromFee(address payable ad) public onlyOwner {
        _isExcludedFromFee[ad] = true;
    }

    function includeToFee(address payable ad) public onlyOwner {
        _isExcludedFromFee[ad] = false;
    }

    function setTakeFeeFromTransfer(bool onoff) public onlyOwner {
        _takeFeeFromTransfer = onoff;
    }

    function setBaseFee(uint256 fee) public onlyOwner {
        require(fee <= 10, "Base fee must be less than 10");
        _baseFee = fee;
    }

    function setTaxFee(uint256 tax) public onlyOwner {
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) public onlyOwner {
        require(
            _liquidityFeePercentage >= 0 && _liquidityFeePercentage <= 100,
            "liquidity fee percentage must be between 0 to 100"
        );
        _liquidityFeePercentage = liquidityFee;
    }

    function setMinContractTokensToSwap(uint256 numToken) public onlyOwner {
    ...
    function setMaxWalletPercentage(uint256 percentage) public onlyOwner {
        require(
            ...
            "max wallet percentage must be between 0 to 100"
        );
        _maxWalletPercentage = percentage;
    }

    function setSwapAll(bool onoff) public onlyOwner {
        _swapAll = onoff;
    }
