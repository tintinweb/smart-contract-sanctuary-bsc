...
contract OrbMarket is ERC1155Holder, Ownable, ReentrancyGuard {
    /*
     struct
    */
    struct SellOrder {
    ...
    */
    ...
    function addWhiteListToken(address addresToken)external onlyOwner {
        whiteListTokens.add(addresToken);
    }
    function removeWhiteListToken(address addresToken)external onlyOwner {
        whiteListTokens.remove(addresToken);
    }
    function getListTokenWhitelist() external view returns (address[] memory) {
    ...
    function listForSale(
        ...
        require(whiteListTokens.contains(tokenAddress),"ORB: token is not whitelist");
    ...
    function updatePriceListOrder(uint256 orderId, uint256 newPrice)
        external
        nonReentrant
    {
        ...
        require(sellOrder.price != newPrice, "ORB: price invalid");
        uint256 oldPrice = sellOrder.price;
        sellOrder.price = newPrice;
        emit OrderUpdate(
            orderId,
            0,
            sellOrder.amount,
            UpdateOrderType.UPDATE_PRICE_LIST_ORDER,
            oldPrice,
            newPrice,
            address(0),
            sellOrder.seller
    ...
    function buy(uint256 orderId, uint256 _amount, uint256 price) external nonReentrant {
        require(_amount > 0, "Orb: invalid amount");
        SellOrder storage sellOrder = eggForSale[orderId];
        require(msg.sender != sellOrder.seller, "Orb: Cannot buy your order");
        require(_amount <= sellOrder.amount, "ORB: amount not enough");
        require(price == sellOrder.price,"ORB: price invalid");
    ...
    function takeOffer(uint256 orderId, uint256 _amount) external nonReentrant {
        ...
        require(_amount > 0, "Orb: invalid amount");
        BuyOrder storage buyOrder = eggForBuy[orderId];
        require(msg.sender != buyOrder.buyer, "Orb: Cannot sell your order");
        ...
        uint256 marketFee = (buyOrder.price * _amount * marketFeeInBps) / BPS;
