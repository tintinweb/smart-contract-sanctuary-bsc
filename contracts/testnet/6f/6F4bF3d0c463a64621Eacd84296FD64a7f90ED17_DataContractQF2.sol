...
contract DataContractQF2 is AccessControl {
    mapping(uint256 => uint256) public withdrawalTime;
    mapping(uint256 => uint256) public minimumDepositSize;
    mapping(uint256 => bool) public isPoolPrivate;
    mapping(uint256 => mapping(address => bool)) public isAuthorized;
    bool intialized;
    address public dataContract;

    function intializeDataContract2(address _dataContractQF, address _owner)
        public
    {
        if (!intialized) {
            dataContract = _dataContractQF;
            _setupRole(DEFAULT_ADMIN_ROLE, _owner);
            intialized = true;
        }
    }

    function setDataContractAddress(address _dataContract) public {
        IDataContractQF(dataContract).checkRole(msg.sender, keccak256("ADMIN_ROLE"));
    ...
    function giveWhiteListRole(address _address, uint256 _poolId) public {
        IDataContractQF(dataContract).checkRole(msg.sender, keccak256("ADMIN_ROLE"));
        grantRole(keccak256(abi.encodePacked("WHITELIST_ROLE", Strings.toString(_poolId))), _address);
    }

    function revokeWhiteListRole(address _address, uint256 _poolId) public {
        IDataContractQF(dataContract).checkRole(msg.sender, keccak256("ADMIN_ROLE"));
        revokeRole(keccak256(abi.encodePacked("WHITELIST_ROLE", Strings.toString(_poolId))), _address);
    }

    function getWithdrawalTime(uint256 _poolId) public view returns (uint256) {
        return withdrawalTime[_poolId];
    }

    function setWithdrawalTime(uint256 _poolId, uint256 _withdrawalTime)
        public
    {
        IDataContractQF(dataContract).checkEitherACCESSorADMIN(msg.sender);
        withdrawalTime[_poolId] = _withdrawalTime;
    }

    function getIsPoolPrivate(uint256 _poolId) public view returns (bool) {
        return isPoolPrivate[_poolId];
    }

    function setIsPoolPrivate(uint256 _poolId, bool _value) public {
        IDataContractQF(dataContract).checkEitherACCESSorADMIN(msg.sender);
        isPoolPrivate[_poolId] = _value;
    }

    function getIsAuthorized(uint256 _pid, address _user)
        public
        view
        returns (bool)
    {
        return isAuthorized[_pid][_user];
    }
    ...
    function setIsAuthorized(
        ...
        IDataContractQF(dataContract).checkEitherACCESSorADMIN(msg.sender);
        isAuthorized[_pid][_user] = _value;
    }

    function getPoolIsPrivateForUser(uint256 _pid, address _user)
        public
        view
        returns (bool, bool)
    {
        return (isPoolPrivate[_pid], isAuthorized[_pid][_user]);
    }

    function getMinimumDepositSize(uint256 _pid) public view returns (uint256) {
        return minimumDepositSize[_pid];
    }

    function setMinimumDepositSize(uint256 _pid, uint256 _depositSize)
        external
        returns (uint256)
    {
        IDataContractQF(dataContract).checkEitherACCESSorADMIN(msg.sender);
        return minimumDepositSize[_pid] = _depositSize;
    }

    function addPreApprovedUser(uint256 _pid, address[] memory _users)
        external
    {
        IDataContractQF(dataContract).checkEitherACCESSorADMIN(msg.sender);
        for (uint256 i = 0; i < _users.length; i++) {
            isAuthorized[_pid][_users[i]] = true;
        }
    }

    function removePreApprovedUsers(uint256 _pid, address[] memory _users)
        external
    {
        IDataContractQF(dataContract).checkEitherACCESSorADMIN(msg.sender);
        for (uint256 i = 0; i < _users.length; i++) {
            isAuthorized[_pid][_users[i]] = false;
        }
    }
}
