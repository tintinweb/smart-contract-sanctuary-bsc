...
contract FilterManager {
    ...
    */
    ...
    function createPresaleWithTemplate(
            uint _tokenType, 
        ...
        require(presaleBuyersShare >= _ownerShare, "FilterManager: BUYERS_SHARE_TOO_LOW");

        uint totalTokensRequired = (_presaleArgs[0] * _presaleArgs[3]) + ((_presaleArgs[1] * _presaleArgs[3] * _presaleArgs[6]) / 100) + ((_ownerShare * _tokenArgs[0] * 1e36) / 100);
        require((_tokenArgs[0] * 1e36) >= totalTokensRequired, "FilterManager: TOKEN_SUPPLY_TOO_LOW");
    ...
    function createPresaleFromToken() public {
    ...
    function investInPresale(uint _presaleID, uint _buyAmount) public payable {
        ...
        if (isBaseETH) {
            ...
            uint userTokenAmount = (userContributedAmount * presaleInfo[_presaleID].presaleArgs[0]) / 1e18; // assuming 18 decimals!!!
