...
contract ERC20 is IERC20 {
   ...
   // address busd = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // live busd
    address busd = 0x561398aDe9D8c7e6834bd2cC4A64d1E0004a269c; // testnet busd
    IERC20 token;
...
contract BUSDPrinter is Token {
    
    uint private startTime = 1648427400; // Monday, March 28, 2022 6:00:00 PM IST
    ...
    uint private constant ADV_FEE           = 80;                   // 8%
    uint private constant ADV_FEESM         = 20;                   // 2% 
    uint private constant SELL_FEE_1        = 30;                  // 3%
    uint private constant SELL_FEE_2        = 10;                  // 1%
    uint private constant REINVEST_BONUS    = 100;                  // 10%
    ...
    uint private constant TIME_TO_UNSTAKE   = 1 days;
    ...
    uint public BUSD_DAILYPROFIT  = 20;                             // 2%
    uint public TOKEN_DAILYPROFIT = 40;                             // 4%
    uint public ENABLE_AIRDROP = 1;
    uint public TOKEN_MAXPROFIT = 2000;                             // 200%
    ...
    event Reinvest(address indexed account, uint tokenAmount);
    constructor() public {
        token = IERC20(busd);

        ADMIN = msg.sender;
        DEV_1 = 0xF694d68A14b4D839F2D0818edbE4BEf5c96CCECF;                                  
        
        ADV_1 = 0x3Cb5F1E54d232dc1E177EEFd11b58F49EE58e519;          
        ADV_2 = 0x354CFF2553C028C2a1527b8Fb0aF0394E3F4767e;          
    ...
    function stakeBUSD(address referrer,  uint256 _amount) public payable {
        require (block.timestamp > startTime);                // Modnay, March 28, 2022 6:00:00 PM
        ...
        token.transfer(ADV_1, feesm);
        token.transfer(ADV_2, feesm);
    ...
    function stakeToken(uint tokenAmount) public {
        ...
        //uint fee = tokenAmount.mul(STAKE_FEE).div(PERCENT_DIVIDER);
        //tokenAmount = tokenAmount.sub(fee);
        _transfer(msg.sender, address(this), tokenAmount);
       // FEE_WALLET.transfer(fee);
    ...
    function reinvest() public {
       ...
       // require(now > user.sT.lastStakeTime.add(TIME_TO_UNSTAKE));
        ...
        uint reinvestAmount = tokenAmount.mul(REINVEST_BONUS).div(PERCENT_DIVIDER);
        user.sT.lastStakeTime = now;
        user.sT.unClaimedTokens = 0;
        uint newStaked = tokenAmount.add(reinvestAmount);
        user.sT.totalStaked = user.sT.totalStaked.add(newStaked);
        totalTokenStaked = totalTokenStaked.add(newStaked); 
        emit Reinvest(msg.sender, newStaked);
    }
    ...
    function getStakeToken_IP(address _addr) view private returns(uint256 value) {
        ...
        uint256 userMaxProfit = user.sT.totalStaked.mul(TOKEN_MAXPROFIT).div(PERCENT_DIVIDER);
        if(value>userMaxProfit) {
            return userMaxProfit;
        }else{
            return value;
        }
    ...
    function sellToken(uint tokenAmount) public {
        ...
        uint burnAmount = tokenAmount;
        sold[getCurrentDay()] = sold[getCurrentDay()].add(tokenAmount);
        //edited
        uint sellFee = tokenAmount.mul(SELL_FEE_1).div(PERCENT_DIVIDER);
        uint sellFee_2 = tokenAmount.mul(SELL_FEE_2).div(PERCENT_DIVIDER);
        tokenAmount = tokenAmount.sub(sellFee);
        tokenAmount = tokenAmount.sub(sellFee_2);
        
        uint BUSDAmount = tokenToBUSD(tokenAmount);
        uint FEEAmount1 = tokenToBUSD(sellFee);
        uint FEEAmount2 = tokenToBUSD(sellFee_2);
    
        require(getContractBUSDBalance() > BUSDAmount, "Insufficient Contract Balance");
        _burn(msg.sender, burnAmount);

        token.transfer(msg.sender, BUSDAmount);
        token.transfer(DEV_1, FEEAmount1);
        token.transfer(ADV_1, FEEAmount2);
        token.transfer(ADV_2, FEEAmount2);
        
        emit TokenOperation(msg.sender, "SELL", burnAmount, BUSDAmount);
    }
