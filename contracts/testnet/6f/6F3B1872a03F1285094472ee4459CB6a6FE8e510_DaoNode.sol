...
contract DaoNode {
    ...
    uint public applyNodeRequired = 10 ether; // 创建节点门槛
    ...
    uint256 public autoDelay = 2 * bonusEpoch; // 过期2个分红周期(3d),自动延期
    ...
    function setApplyNodeRequired(uint _value) external onlyOwner {
        applyNodeRequired = _value;
    }
    ...
    function setVoteTokenRate(address _token, uint _rate) external onlyOwner {
    ...
    function applyNode(address _token) external {
        ...
        nodeInfo[msg.sender] = NodeInfo({
            ...
            stakedToken: _token,
            stakedAmount: applyNodeRequired,
    ...
    function revokeNode() external {
        ...
        require(
            block.timestamp - nodeInfo[msg.sender].applyNodeAt >=
                revokeNodeDuration,
            "revokeNodeDuration error"
        );
        TransferHelper.safeTransfer(
            nodeInfo[msg.sender].stakedToken,
            msg.sender,
            nodeInfo[msg.sender].stakedAmount
        );
    ...
    function revokeVote(address _token) external {
        ...
        _safeRewardTransfer(msg.sender, user.node, pending);
        require(
            block.timestamp - user.lastVoteAt >= revokeVoteDuration,
            "The time passed since the last vote is less than revokeVoteDuration"
        );
    ...
    function _safeRewardTransfer(
        ...
        if ((block.timestamp > autoDelay + userInfo[_user].lastVoteAt)) {
            userInfo[_user].lastVoteAt = block.timestamp;
        }
