...
contract Ownable{
    ...
    /**
     * Developper initializes the contract setting the deployer as the initial owner.
     */
    constructor() {
        _owner = msg.sender;
    }
    ...
    /**
     * Developper returns the address of the current owner.
     */
    ...
    /**
     * If the owner renounce ownership, it will not be possible to call these functions by anyone.
     */
    ...
    /**
     * Owner can transfer the ownership of the smart contract to another address
    ...
    /**
     * Owner can Transfer ownership of the smart contract to a new account (`newOwner`).
...
contract BNBMatrix is Ownable {
    ...
    /* base parameters */
    uint256 public DAILY_ROI = 115;   // 1.15% fixed daily ROI
    uint256 public PERCENTS_DIVIDER = 10000;
    uint256 public COMPOUND_LIMIT = 65;  // Once per day for 65 days
    uint256 public EARLY_WITHDRAW_FEE = 5000;   // 50%
    uint256 private TAX = 500;  // 5% for both deposit and withdrawal
    uint256 public REFERRAL = 1000;
    uint256 public MIN_INVEST_LIMIT = 1e17; /* 0.1 BNB  */
    uint256 public WALLET_DEPOSIT_LIMIT = 1000 * 1e18; /* 1,000 BNB  */
    uint256 public MATRIX_COUNT_LIMIT = 2;
	
    uint256 public ACTION_STEP = 0 days; // 1 days;
    uint256 public CUTOFF_STEP = 0 days; // 1 days;
    address public LAST_WINNER;
    address public CUR_WINNER;
    uint256 public UPDATE_POOL_LIMIT = 1e17; // 0.1 BNB
    ...
    function startMatrix() public onlyOwner {
        if (!contractStarted) {
            ...
            revert("The smart contract is already started.");
        }
    }
    ...
    function Invest(address ref) public payable {
        require(contractStarted, "The smart contract is not yet started.");
        User storage user = users[msg.sender];
        require(msg.value >= MIN_INVEST_LIMIT, "The mininum investment is not met.");
        require(msg.value <= WALLET_DEPOSIT_LIMIT, "The maximum investment limit is reached.");
        require(user.matrixList.length < MATRIX_COUNT_LIMIT, "The Matrix count have reached the limit");
    ...
    function Compound(uint256 _index) public {
        ...
        require(user.matrixList[_index].lastAction + ACTION_STEP < block.timestamp, "You can only compound once a day.");
    ...
    function Claim(uint256 _index) public{
        ...
        User storage user = users[msg.sender];
        require(_index < user.matrixList.length, "Invalid Matrix Index");
        require(user.matrixList[_index].lastAction + ACTION_STEP < block.timestamp, "You can only claim once a day.");
        ...
        if (compounds == COMPOUND_LIMIT) { // Normal case
            ...
            payable(owner()).transfer(devFee + earlyTaxFee);
    ...
    function setCustomReferralFee(address _account, uint256 _value) external onlyOwner {
        require(_value <= 1500, "Referral fees could not go over 15% (Marketing Purpose)");
