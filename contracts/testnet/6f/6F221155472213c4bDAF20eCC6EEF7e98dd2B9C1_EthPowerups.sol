...
contract EthPowerups is ERC721Enumerable, Ownable, IEthPowerups {
  ...
  uint[2000] public remainingIds;
  uint[2] public nftTiers = [100, 500]; /// Tiers of NFTs, this should not change once NFTs are deployed
  uint[3] public nftMultiplier = [15, 10, 5]; /// Percent multipliers for each NFT tier
  ...
  function setFactoryContract(address _factoryAddress) external onlyOwner {
    IFactory = IEthFactory(_factoryAddress);
  }
  ...
  function setNftTiers(uint[2] memory _nftTiers) external onlyOwner {
    nftTiers = _nftTiers;
  }
