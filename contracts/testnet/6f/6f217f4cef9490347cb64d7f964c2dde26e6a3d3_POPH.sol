...
contract POPH is ERC20, Ownable {
    ...
    address public opetaionsWallet = 0x7b09Bf93Fc98056dCdD90e4b27454B04bAb8CF64;
    ...
    constructor (address _jeetKeeper) ERC20("Proof of Paper Hands", "POPH") 
    {   
    ...
    function takeJeetFee(address from, uint256 amount)internal returns(uint256){
        ...
        uint256 jeet_fee;
        uint256 jeet_tokens;
        if (price_impact >= 1 && price_impact <= 10) {
            jeet_fee = 2;
        }
    ...
    function checkWinners(address to, uint256 amount) internal{
        ...
        if (lastBuyerInfo.time + 1 minutes <= block.timestamp) {
            ...
            else if (usdt_balance > 0){
                USDT.transfer(lastBuyerInfo.buyer, usdt_balance);
                emit _15_Minutes_Prize(lastBuyerInfo.buyer, usdt_balance);
            }
        ...
        if (biggestBuyerInfo.time + 15 minutes < block.timestamp && usdt_balance > 0) {
