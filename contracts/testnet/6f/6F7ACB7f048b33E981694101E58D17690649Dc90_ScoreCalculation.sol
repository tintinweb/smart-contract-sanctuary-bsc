...
contract ScoreCalculation is Ownable {
    ...
    mapping(bytes32 => mapping(uint256 => Survey[])) private visibilitySurveys;
    mapping(bytes32 => mapping(uint256 => Survey[])) private commitmentSurveys;

    mapping(bytes32 => uint256[]) private contractIds;
    ...
    function getCommitmentScore (
        ...
        for (uint256 index = 0; index < surveys.length; index++) {
            if((surveys[index].timestamp/3600)*3600 == timestamp) {
                count++;
