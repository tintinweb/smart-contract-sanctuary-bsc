...
contract o2Staking is Context, Ownable {
    ...
    function unStakeAll() external onlyOwner{
        ...
        for(uint i=1;i<=_stakeTimes;i++){
            ...
            if(interestAmount>0){
                ...
                _userClaimInfos[_msgSender()].push(claimInfo(interestAmount,block.timestamp,info.stakeEndTime.sub(info.claimTime),2,(info.stakeEndTime-info.stakeTime)/(24*3600),info.rate,info.stakeTotal));
            }
            info.status=false;
    ...
    function unStake(uint256 stakeId) external{
        ...
        if(info.status==false&&info.waitReceiveAmount>0){
            ...
            if(interestAmount>0){
                ...
                info.stakeInterestTokenTotal+=interestAmount;//添加到当前累计收益
                _userClaimInfos[_msgSender()].push(claimInfo(interestAmount,block.timestamp,block.timestamp>info.stakeEndTime?info.stakeEndTime.sub(info.claimTime):block.timestamp.sub(info.claimTime),1,(info.stakeEndTime-info.stakeTime)/(24*3600),info.rate,info.stakeTotal));
    ...
    function claim(uint256 stakeId) external{
        ...
        _userClaimInfos[_msgSender()].push(claimInfo(interestAmount,block.timestamp,block.timestamp>info.stakeEndTime?info.stakeEndTime.sub(info.claimTime):block.timestamp.sub(info.claimTime),0,(info.stakeEndTime-info.stakeTime)/(24*3600),info.rate,info.stakeTotal));
        info.claimTime=block.timestamp;
        info.stakeInterestTotal+=getInterestAmount(stakeId,true);
        info.stakeInterestTokenTotal+=interestAmount;//添加到当前累计收益
