...
contract NFTCollection is ERC721A, ERC2981, Ownable {
    ...
    function mint(uint256 quantity,uint256 nonce, bytes calldata _signature) external payable {
        require(
            quantity <= maxSupply,
        ...
        require(!proceedNonce[nonce],"NFTCollection: Nonce already proceed !");
           ...
           if(platformPubKey == verifyWhiteListUser(msg.sender,nonce,_signature)){
        require(
        ...
        if (platformPubKey == verifyUser(msg.sender,nonce ,_signature)) {
        require(
