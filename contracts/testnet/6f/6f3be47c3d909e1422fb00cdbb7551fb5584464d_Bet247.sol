...
contract Bet247 is Pausable {
    ...
    function withdrawMatch(string calldata key) external returns (uint256) {
        ...
        matches[key].remainAmount = matches[key].amount - payAmount;
        ...
        requireEditor
    {
        ...
        require(matches[key].startTime + 30 * 24 * 3600 < block.timestamp, "_");
        if (matches[key].amount > 0) {
            payable(devFeeAddress).transfer(matches[key].amount);
            matches[key].amount = 0;
        }
        emit EndMatch(key);
    }
