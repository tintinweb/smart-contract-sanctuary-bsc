...
contract Pool is Ownable {
    ...
    address public mTokenAddress = 0x682855D0217942500c2EE8bf6e2739769076e624;
    ...
    function addPool(
        IERC20 _lpToken,
        uint256 _lockBlockCount,
        uint256 _depositLimit,
        uint256 _firstDepositLimit
    ) public onlyOwner {
        poolInfoList.push(
            PoolInfo({
                lpToken: _lpToken,
    ...
    function deposit(uint256 _poolId, uint256 _depositLpAmount) public {
        ...
        uint256 userDepositPrice = (_depositLpAmount * lpPrice) / 1e18;
        ...
        userDepositRecordList[msg.sender][_poolId].push(
            DepositRecord({
                ...
                endBlockNumber: lastBlock + pool.lockBlockCount,
        ...
        user.balance += userDepositPrice;
        pool.totalBalance += _depositLpAmount;
    ...
    function withdraw(uint256 _poolId, uint256 _depositRecordId) public {
        ...
        PoolInfo storage pool = poolInfoList[_poolId];
        UserInfo storage user = userInfoList[msg.sender];
        uint256 userDepositAmount = userDepositRecord.userDepositAmount;
        uint256 userDepositPrice = userDepositRecord.userDepositPrice;
        pool.lpToken.safeTransfer(msg.sender, userDepositAmount);
        userDepositRecord.operateType = 2;

        user.balance -= userDepositPrice;
        pool.totalBalance -= userDepositAmount;

        emit LogWithdrawLp(_poolId, msg.sender, block.number, userDepositPrice, userDepositAmount);
    }
    ...
    function getLpPrice(uint256 _poolId) public view returns (uint256) {
        PoolInfo memory pool = poolInfoList[_poolId];
        (uint112 reserve0, uint112 reserve1, ) = IPancakePair(address(pool.lpToken)).getReserves();
        uint112 reserveMToken = reserve0;
        address token0 = IPancakePair(address(pool.lpToken)).token0();
        if (mTokenAddress == token0) {
            reserveMToken = reserve1;
        }
        uint256 totalSupply = getLpTotalSupply(_poolId);
        uint256 result = (2 * uint256(reserveMToken) * 1e18) / totalSupply;
        return result;
    }
    ...
    function getDepositRecordItemExternal(
