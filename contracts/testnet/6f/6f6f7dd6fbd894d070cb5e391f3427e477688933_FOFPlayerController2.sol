...
contract FOFPlayerController is Ownable {
...
contract FOFPlayerController2 is Ownable {
    ...
    // event MergeAdvancedVehicle(address from, uint256 burnId1, uint256 burnId2, Vehicle vehicle);
    ...
    mapping(uint256 => bool) public advancedVehicleMerge;
    ...
    FOFPlayerController FPC;

    constructor() {
        FPN = FOFPlayerNFT(0x750de8b9Ba3998982421DbBc83A2450c78815f6a);
        FPC = FOFPlayerController(payable(0x76ae521ca435A1D407F7469cC162dDFD8e6E4058));

        advancedVehicleMerge[101] = true;
        advancedVehicleMerge[102] = true;
        advancedVehicleMerge[103] = true;
    }
    ...
    function setFOFPlayerController(address _FOFPlayerController) public onlyController {
        FPC = FOFPlayerController(payable(_FOFPlayerController));
    }

    function openAdvancedVehicleMerge(uint256 VehicleType, bool open) public onlyController {
        advancedVehicleMerge[VehicleType] = open;
    }
    ...
    function mergeVehicle(uint256[] memory cIds) public returns (uint256) {
        ...
        require(advancedVehicleMerge[101], "Vehicle synthesis of this type is not available");
        ...
        // vehicles[tokenId] = Vehicle(tokenId, msg.sender, 101, speed, stability, controllability, explosive, attribute);
    ...
    function mergeAdvancedVehicle(uint256[] memory tokenIds) public returns (uint256) {
        ...
        require(advancedVehicleMerge[types], "Vehicle synthesis of this type is not available");
    ...
    // function mergeAdvancedVehicle(uint256 tokenId1, uint256 tokenId2) public returns (uint256) {

    //     require(tokenId1 != tokenId2, "The vehicle types don't match");

    //     require(FPN.ownerOf(tokenId1) == msg.sender && FPN.ownerOf(tokenId2) == msg.sender , "You are not the owner of the vehicle");

    //     require(FPN.NFTTypes(tokenId1) == FPN.NFTTypes(tokenId2) && FPN.NFTTypes(tokenId1) > 100, "The vehicle types don't match");

    //     uint256 types = FPN.NFTTypes(tokenId1) + 1;

    //     require(advancedVehicleMerge[types], "Vehicle synthesis of this type is not available");

    //     uint256 tokenId = FPN.createNFT(msg.sender, types);

    //     (uint256 speed,uint256 stability, uint256 controllability,uint256 explosive) = countVehicleAttribute2(tokenId1,tokenId2);

    //     uint256 attribute = speed.add(stability).add(controllability).add(explosive);

    //     vehicles[tokenId] = Vehicle(tokenId, types, speed, stability, controllability, explosive, attribute);

    //     delete vehicles[tokenId1];

    //     delete vehicles[tokenId2];

    //     FPN.approveToController(msg.sender, tokenId1);

    //     FPN.approveToController(msg.sender, tokenId2);

    //     // FPN.burn(tokenId1);

    //     // FPN.burn(tokenId2);

    //     FPN.transferFrom(msg.sender, deadWallet, tokenId1);

    //     FPN.transferFrom(msg.sender, deadWallet, tokenId2);

    //     emit MergeAdvancedVehicle(msg.sender, tokenId1, tokenId2, vehicles[tokenId]);
    ...
    // function checkUserComponent(address user) public view returns(bool){

    //     uint256 i;

    //     for(i = 1; i < 6; i++){
    //         uint256 length = FPC.queryMyComponentByType(user, i).length;
    //         if(length == 0){
    //             return false;
    //         }
    //     }
    ...
    function getUserNFTIDs(address user, uint256 NFTType) public view returns(uint256[] memory NFTIDs, uint256[] memory types) {
