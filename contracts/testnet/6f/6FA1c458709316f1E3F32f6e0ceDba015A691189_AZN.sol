...
contract AZN is Ownable, ERC721Enumerable, EIP712, ILaunchpadNFT {
    ...
    uint256 public constant TOTAL_MINT_MAX_QTY = 9700;
    uint256 public constant GIFT_MAX_QTY = 300;
    ...
    uint256 public maxLaunchpadSupply = 9700;
    ...
    modifier onlyLaunchpadOrOwner() {
        require(launchpadAddress != address(0), "XADD");
        require(
            msg.sender == launchpadAddress || msg.sender == owner(),
            "XALL"
        );
        _;
    }
    ...
    constructor() ERC721("AZN", "AZN") EIP712("AZN", "1") {}
    ...
    function preSalesMint(
        ...
        uint256 _nonce,
        bytes memory _signature
    ) external payable isSenderWhitelisted(_signedQty, _nonce, _signature) {
        require(isPreSalesActivated, "XOPN");
        require(mintedQty + _mintQty <= TOTAL_MINT_MAX_QTY, "TLMT");
        require(
            preSalesMinterToTokenQty[msg.sender] + _mintQty <= _signedQty,
            "SQTY"
        );
        require(msg.value >= _mintQty * price, "SETH");
    ...
    function publicSalesMint(uint256 _mintQty) external payable {
        require(isPublicSalesActivated, "XOPN");
        require(mintedQty + _mintQty <= TOTAL_MINT_MAX_QTY, "TLMT");
        require(
    ...
    function mintTo(address to, uint256 _mintQty)
        external
        onlyLaunchpadOrOwner
    {
        require(mintedQty + _mintQty <= TOTAL_MINT_MAX_QTY, "TLMT");
        mintedQty += _mintQty;
    ...
    function toggleIsPresalesActivated() external onlyOwner {
        isPreSalesActivated = !isPreSalesActivated;
    }

    function toggleIsPublicSalesActivated() external onlyOwner {
    ...
    // BSC: 0xa88f57dAF834572f994eEa20c3Ba6924ad3bF00f
    // BSC Tesnet: 0xe53d5bF04f7fE2e3C255274fcE6594D64b6EfBaD
    // Reference: https://galler.readme.io/docs/launchpad-specification#launchpad-deployed-address
    function setLaunchpadAddress(address _address) external onlyOwner {
        launchpadAddress = _address;
    }

    function setMaxLaunchpadSupply(uint256 qty) external onlyOwner {
        maxLaunchpadSupply = qty;
    }
    ...
    function getLaunchpadSupply() external view returns (uint256) {
        uint256 mintedBeforeLaunchpad = TOTAL_MINT_MAX_QTY - maxLaunchpadSupply;
        return mintedQty - mintedBeforeLaunchpad;
    }
}
