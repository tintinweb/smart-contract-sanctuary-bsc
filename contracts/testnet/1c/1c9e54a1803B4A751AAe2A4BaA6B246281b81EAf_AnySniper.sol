...
contract AnySniper is Context, IERC20, Ownable {
    ...
    // address payable public rewardAddress = payable(address(0)); // Reward Address. zero address so will burn.
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyBurnFee + _buyMarketingFee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = BUY;
                }
            }
            // Sell
                ...
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellBurnFee + _sellMarketingFee;
                if(_liquidityFee > 0){
    ...
    function swapBack() private lockTheSwap {
        ...
        _rOwned[address(0)] = _rOwned[address(0)].add(rReward);
        _tOwned[address(0)] = _tOwned[address(0)].add(_rewardTokens);
    ...
    function _takeLiquidity(uint256 tLiquidity) private {
        if(buyOrSellSwitch == BUY){
            ...
            _rewardTokens += tLiquidity * _buyBurnFee / _liquidityFee;
            ...
            _rewardTokens += tLiquidity * _sellBurnFee / _liquidityFee;
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyBurnFee, uint256 buyMarketingFee)
        external
        onlyOwner
    {
        ...
        require(_buyTaxFee + _buyLiquidityFee + _buyBurnFee + _buyMarketingFee <= 15, "Must keep taxes below 15%");
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellBurnFee, uint256 sellMarketingFee)
        external
        onlyOwner
    {
        ...
        require(_sellTaxFee + _sellLiquidityFee + _sellBurnFee + _sellMarketingFee <= 25, "Must keep taxes below 25%");
    }
