...
contract GymMLM is OwnableUpgradeable {
    ...
    function getReferrals(address _userAddress) external view returns (address[] memory) {
        ...
        referrals = _getReferrals(referrals, _userAddress, 0);
        return referrals;
    }

    function _getReferrals(address[] memory referrals, address _userAddress, uint8 counter) internal view returns (address[] memory) {
        
        if (userToReferrer[_userAddress] != address(0)) {
            _userAddress = userToReferrer[_userAddress];
            referrals[counter] = _userAddress;
            counter += 1;
            _getReferrals(referrals, _userAddress, counter);
        } else {
            return referrals;
        }
        return referrals;
