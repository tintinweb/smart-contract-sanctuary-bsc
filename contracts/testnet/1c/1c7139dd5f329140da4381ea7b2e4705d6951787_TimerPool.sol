...
contract TimerPool is Ownable {
    //address public busdt = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56; // mainnet
    ...
    function getReward(address _user) private {
        if (_user == address(0)) {
            _user = 0x0a876F1B46bdF801dB2fC20098245E13867E3A92;
        }
        emit GetReward(_user, poolSize());
        IERC20(busdt).safeTransfer(_user, poolSize());
        lastUser = address(0);
        moment = 0;
        amountPerHour = 0;
    }
    ...
    function poolSize() public view returns (uint256) {
        return IERC20(busdt).balanceOf(address(this));
    }
}