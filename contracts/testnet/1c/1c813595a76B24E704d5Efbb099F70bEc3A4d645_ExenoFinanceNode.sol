    ...
    IExenoFinanceNode
{
        ...
        internal
    {
        ...
        if (d.method != ExenoFinance.METHOD_SWAP) {
        ...
        if (params.length > 0) {
            ...
            if (action == ACTION_DEBIT_REWARD) {
                ...
                ExenoFinance.validateReward(!isDebitId(id), amount);
                _executeReward(d, id, token, amount);
            } else if (action == ACTION_DEBIT_OPTION) {
                (bytes32[2] memory id, address[2] memory token, uint256[2] memory amount, uint256 timeout) = abi.decode(args, (bytes32[2], address[2], uint256[2], uint256));
                ExenoFinance.validateOption(d, !isDebitId(id[0]), token[0], amount[0], timeout);
                ExenoFinance.emitDebitOption(d, id[0], token[0], amount[0], timeout);
                ExenoFinance.validateReward(!isDebitId(id[1]), amount[1]);
                _executeReward(d, id[1], token[1], amount[1]);
            } else {
        ...
        internal
    {
        ...
        ExenoFinance.emitReward(d, id, token, amount);
    }
        ...
        internal pure returns(ExenoFinance.Data memory)
    {
        (
            ...
            bytes32 method,
            bytes memory memo
        ) = abi.decode(payload, (bytes32, address, address, bytes32[2], address[2], address[2],  uint256[2], int256[2], bool[2], bytes32, bytes));
        return(ExenoFinance.Data(id, platform, affiliate, network, beneficiary, token, amount, timeout, explicit, method, memo));
    }
        ...
        external
    {
        ...
        emit ReleaseFees(actor, [payoutCash, payoutCore]);
        if (payoutCash > 0) {
            Address.sendValue(payable(actor), payoutCash);
            platformFees[actor][1][0] += payout[0][0];
            affiliateFees[actor][1][0] += payout[1][0];
        }
        if (payoutCore > 0) {
            ExenoFinance.transferTo(actor, coreToken(), payoutCore, availableTokens(coreToken()));
...
library ExenoFinance {
    ...
    // Indicates that a reward transaction has been triggered
    event Reward(
        bytes32 indexed id,
        address platform,
        address affiliate,
        ...
        external pure
    {
        ...
        require(amount > 0,
            "EFN: amount mismatch");
        require(timeout > 0 && d.timeout[0] > 0 && d.timeout[1] == 0,
            "EFN: timeout mismatch");
        require(unprocessed,
            "EFN: debit a/y processed");
        require(d.method == METHOD_SWAP,
    ...
    /**
     * Publish events resulting from an option
     */
    function emitDebitOption(
        Data calldata d,
        bytes32 id,
        address token,
        uint256 amount,
        ...
        external
    {
        publish(Event.OPTION, Data(
            ...
            [d.token[0], token],
            [d.amount[0], amount],
            [int256(timeout), int256(0)],
            d.explicit,
            d.method,
            d.memo
        ));
        publish(Event.DEBIT, Data(
            id,
            d.platform,
            d.affiliate,
            d.network,
    ...
    /**
     * Publish event while reducing its storage footprint to mimimum
     */
    function publish(
        Event e,
        Data memory d
        ...
        internal
    {
        ...
        if (e == Event.DEBIT) {
            emit Debit(d.id, d.platform, d.affiliate, network, d.beneficiary, d.token, d.amount, timeout, d.explicit, method, d.memo);
        } else if (e == Event.CREDIT) {
            emit Credit(d.id, d.platform, d.affiliate, network, d.beneficiary, d.token, d.amount, timeout, d.explicit, method, d.memo);
        } else if (e == Event.SWAP) {
            emit Swap(d.id, d.platform, d.affiliate, network, d.beneficiary, d.token, d.amount, timeout, d.explicit, method, d.memo);
        } else if (e == Event.OPTION) {
            emit Option(d.id, d.platform, d.affiliate, network, d.beneficiary, d.token, d.amount, timeout, d.explicit, method, d.memo);
        } else if (e == Event.UPDATE) {
            emit Update(d.id, d.platform, d.affiliate, network, d.beneficiary, d.token, d.amount, timeout, d.explicit, method, d.memo);
        } else if (e == Event.REVOKE) {
            emit Revoke(d.id, d.platform, d.affiliate, network, d.beneficiary, d.token, d.amount, timeout, d.explicit, method, d.memo);
        } else if (e == Event.REWARD) {
            emit Reward(d.id, d.platform, d.affiliate, network, d.beneficiary, d.token, d.amount, timeout, d.explicit, method, d.memo);
        }
    }
