...
contract MerkleValidator {
    ...
    function batchMatchERC721UsingCriteria2(Item[] memory orderItems, bytes32 root) external returns (bool) {
        ...
        for(uint i=0;i<orderItems.length;i++){
            ...
            if (root != bytes32(0)) {
                ...
                bytes32  leaf = keccak256(abi.encodePacked(item.tokenAddress, item.tokenId,item.price));
    ...
    function batchMatchERC721UsingCriteria(
        Item[] memory orderItems, bytes32 root,bytes32[] calldata proof
