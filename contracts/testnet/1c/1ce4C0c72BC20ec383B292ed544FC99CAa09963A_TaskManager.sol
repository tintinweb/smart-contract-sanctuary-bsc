...
contract TaskManager is Validatable, ITaskManager, ReentrancyGuardUpgradeable {
    ...
    /**
     *  @notice address verify message of function completeTask
     */
    ...
    event SetTaskDone(uint256 indexed taskId);
     ...
     * @param taskId Id of task
     *
     * emit {SetTaskDone} events
     */
    function setTaskDone(uint256 taskId) external nonReentrant validTaskId(taskId) {
        TaskInfo storage taskInfo = tasks[taskId];
        require(taskInfo.status == StatusTask.ACTIVE, "Task was done or cancel");
