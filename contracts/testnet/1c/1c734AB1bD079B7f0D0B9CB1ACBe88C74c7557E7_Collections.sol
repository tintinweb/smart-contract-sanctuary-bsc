...
contract Collections is Ownable,Pausable,ICollections {
    ...
    uint256 BypassMax=11;
    bool Bypassbool;


    //NFTWA
    uint256 Max_Col_NFTWA =27;
    uint256 public Total_ColNFTWA;
    //NFTW
    uint256 public Total_ColNFTW;
    ...
    constructor () {
        ...
        MITI=IMiticoin(Contr.getMiticoin());
        NFTA = Contr.getNFTA(); 
    }
         function BypassMax_NFTA () public onlyOwner {
            require(Bypassbool==false);
            MITI=IMiticoin(Contr.getMiticoin());
            NFTA = Contr.getNFTA(); 
            require(Primary==false);
            uint256 amount=BypassMax;
            require(Total_ColNFTA+amount<=Max_Col_NFTA,"MAX NFTA");
            uint256 X;
            while(X<amount){
            uint256 ID_=CNF.Get_NFTA_num(Total_ColNFTA); 
    ...
    function ADD_Collection_NFTA (uint256 amount) public onlyOwner {
        MITI=IMiticoin(Contr.getMiticoin());
            ...
            What_Collection[Total_ColNFTA]=1;
            Total_ColNFTA++;
            MITI.PrimaryMINT(One_Col_NFTA);
            X++;
            }
    }
 
    function ADD_Collection_NFTWA(address Collection_address,uint256 Max_world) public onlyOwner {
        MITI=IMiticoin(Contr.getMiticoin());
            require(Primary==false);
            require(Collection_ID[Collection_address]==0);
            require(Total_ColNFTWA+1<=Max_Col_NFTWA,"MAX NFTWA");
            uint256 ID = CNF.Get_NFTWA_num(Total_ColNFTWA);
