...
import "./interfaces/ISpyPool.sol";
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // The mining pool
    ISpyPool public miningPool;

    // The mining pool
    ISpyPool public marketingPool;
    ...
    uint256 public harvestInterval = 72 hours;
    ...
    event RewardHarvested(address indexed user, uint256 indexed pid, uint256 amountHarvested);
    ...
    constructor(
        ...
        miningPool = ISpyPool(_miningPool);
        marketingPool = ISpyPool(_marketingPool);
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint256 _lpLockUntil, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Update the give pool's SPY allocation point with harvest interval
    function updateLpLockUntil(uint256 _pid, uint256 _lpLockUntil) public onlyOwner {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require(vaults[msg.sender] || block.timestamp > pool.lpLockUntil, "LP locked");
    ...
    function payOrLockupPendingSpy(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                emit RewardHarvested(msg.sender, _pid, totalRewardsToHarvest);
            }
        } else if (pending > 0) {
