...
contract WoolshedV1 is OwnableUpgradeable, ContractGuard {
    ...
    function deposit(address _upline, uint256 _amount) external {
        ...
        uint256 airdropsPending = airdrops[_addr].pending;

        if (airdropsPending > 0) {
            realizedDeposit += airdropsPending;
            airdrop(_addr, airdropsPending);
        }
        ...
        require(_amount >= minimumAmount, "MIN_DEPOSIT");
        ...
        require(woolToken.transferFrom(_addr, address(taxVault), _amount), "BAD_XFER");
    ...
    function _setUpline(address _addr, address _upline) internal {
        ...
        */
        if (
            ...
            _upline != _addr &&
            _addr != owner() &&
            (users[_upline].deposit_time > 0 || _upline == bertha)
        ) {
    ...
    function _deposit(address _addr, uint256 _amount) internal {
        ...
        require(users[_addr].upline != address(0) || _addr == owner(), "NO_BUDDY");
            ...
            if(users[_addr].ref_claim_pos == i) {
                if (isBalanceCovered(_up, i + 1) && isNetPositive(_up)){
                    ...
                    if(users[_up].referrals >= 5 && !_team_found) {
                        ...
                        emit NewAirdrop(_addr, _share, block.timestamp);
    ...
    function airdrop(address _to, uint256 _amount) internal {
        ...
        //Let em know!
        emit NewAirdrop(_to, _amount, block.timestamp);
    }

    function teamAirdrop(address[] memory _recipients, uint256[] memory _amount) external {
        require(_recipients.length == _amount.length, "BAD_LENGTH");

        address _addr = msg.sender;
        uint256 _requiredAmt = 0;

        for (uint i = 0; i < _recipients.length; i++) {
            airdrops[_recipients[i]].pending += _amount[i];
            _requiredAmt += _amount[i];
        }

        airdrops[_addr].airdrops += _requiredAmt;
        airdrops[_addr].last_airdrop = block.timestamp;

        require(woolToken.transferFrom(_addr, address(taxVault), _requiredAmt), "BAD_XFER");

        emit NewTeamAirdrop(_addr, _requiredAmt, block.timestamp);
    }
}