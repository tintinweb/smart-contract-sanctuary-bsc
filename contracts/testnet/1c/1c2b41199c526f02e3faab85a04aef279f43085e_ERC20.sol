...
contract DeflatToken is ERC20 {
    ...
    constructor() ERC20("DFT", "DFT") {
        _mint(msg.sender, 100000 * 10 ** 9);
        owner = msg.sender;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool){
        if (excludedFromTax[msg.sender] == true){
            ...
            uint burnAmount = amount.mul(BURNFEE) / 100;
            uint ownerAmount = amount.mul(TAXFEE) / 100;
            _burn(_msgSender(), burnAmount);
            _transfer(_msgSender(), owner, ownerAmount);
            _transfer(_msgSender(), recipient, amount.sub(burnAmount).sub(ownerAmount));
        }
        return true;
    }
