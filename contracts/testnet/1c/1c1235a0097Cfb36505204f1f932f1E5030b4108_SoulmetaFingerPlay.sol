...
contract SoulmetaFingerPlay is DivestorUpgradeable {
    ...
    // =============================弃用============================
    uint public extractKaka; // 抽取入平台的 KAKA 代币
    ISoulToken public kakaTokenAddr;
    // 房间号 => 支付方式 （1 - soul | 2 - kaka）
    mapping(uint => uint) public roomPay;
    ISoulToken public iziTokenAddr;
    ISoulToken public usdtTokenAddr;
    ISoulToken public stiTokenAddr;
    uint public extractIzi; // 抽取入平台的 IZI 代币
    uint public extractBnb; // 抽取入平台的 IZI 代币
    uint public extractUsdt; // 抽取入平台的 USDT 代币
    uint public extractSti; // 抽取入平台的 STI 代币

    ISoulToken public fistTokenAddr;
    uint public extractFist; // 抽取入平台的 FIST 代币
    ISoulToken public gmtTokenAddr;
    uint public extractGmt; // 抽取入平台的 GMT 代币
    // =============================弃用============================
    ...
    // 设置支持代币
    function setCurrency(uint payId_, address token_, uint min_) public onlyOwner {
        mapCurrency[payId_] = Currency({
            token: token_,
            min: min_,
            extract: 0
        });
    ...
    function enterRoom(uint roomId_, uint amount_, uint position_, uint pay_) public payable {
        ...
        require(roomId_ >= 1 && roomId_ <= MAX_ROOM_COUNT, "room id error");
        require(inRoomId[_msgSender()] == 0, "in room");
        require(position_ == 1 || position_ == 2, "position err");
        require(pay_ >= 1 && pay_ <= 8, "pay err");
        require(msg.value >= gasFees, "gas bnb err");

        RoomInfo storage info = roomIdToUserInfo[roomId_];
        if (info.amount <= 0) {
            require(amount_ >= mapCurrency[pay_].min, "amount error");
            info.amount = amount_;
        } 

        if (roomPay[roomId_] == 0) roomPay[roomId_] = pay_;
        ...
        if (info.amount != 0 && pay_ == 1) {
            require(msg.value == info.amount + gasFees, "bnb err");
        }
        // gas
        payable(serverAddr).transfer(gasFees);
        if (roomPay[roomId_] != 1) {
            ISoulToken(mapCurrency[roomPay[roomId_]].token).transferFrom(_msgSender(), address(this), info.amount);
        } 
        
        emit EnterRoom(roomId_, _msgSender(), position_, info.amount, pay_);
    }
    ...
    function settleGame(uint roomId_, address winner_) public {
        ...
        mapCurrency[roomPay[roomId_]].extract += extractCount;
        if (roomPay[roomId_] != 1) {
            // 自定义代币
            ISoulToken(mapCurrency[roomPay[roomId_]].token).transfer(winner_, winCount);
        } else {
            // bnb
    ...
    function extractionKaka(address wallet_, uint pay_) public onlyOwner {
        ...
        if (pay_ > 2) {
            ...
            payable(wallet_).transfer(extractBnb);
        }
...
interface ISoulmetaFingerPlayBonus{
