...
event NewpkGame(string name1,string name2,uint256 jiegui,uint256 peilv1,uint256 peilv2,uint256 peilv3,string gametype);
    ...
    struct Game {
        ...
        uint256 chengbai;//成功失败:0未开始1赚钱2赔钱
    }
...
//会员添加比赛订单
    function jionGames(uint256 pkgameid,uint256 mai_jieguo,uint256 amount) public  {
        ...
        require(usdt.balanceOf(msg.sender)>=amount,"USDT balance too low");
        usdt.transferFrom(msg.sender,address(this), amount*10**decimals);
        uint id = games.push(Game(pkgameid, amount*10**decimals,uint32(block.timestamp),1,mai_jieguo,0))-1;
    ...
    //填充结果
    function endgame(uint256 _pkgameId,uint256 _mai_jieguo) public onlyOwner {
      require(pkgames[_pkgameId].status==1,"已经开赛过了!");
...
//获取某会员购买的。未开赛的比赛
  function getGameByOwner(address  _owner) external view returns(uint[] memory) {
    uint[] memory result = new uint[](getGameByOwnergeshu(_owner));
    uint counter = 0;
...
//获取某会员以开赛的的的比赛
  function getGameByOwneryikai(address  _owner) external view returns(uint[] memory) {
    uint[] memory result = new uint[](getGameByOwnergeshuyikai(_owner));
    uint counter = 0;
    for (uint i = 0; i < games.length; i++) {
      if (gameToOwner[i] == _owner&&games[i].status==2) {
        result[counter] = i;
        counter++;
      }
    }
...
//获取某个会员的购买数    Game games game  
  function getGameByOwnergeshuyikai(address  _owner) public view returns(uint counter) {
...
//创建比赛列表
    function newpkGames(string memory _team1,string memory _team2,uint256 peilv1,uint256 peilv2,uint256 peilv3,uint256 time,string memory gametype) public onlyOwner  {
        uint id = pkgames.push(Pkgame(_team1,_team2,0,peilv1,peilv2,peilv3,1,gametype,time))-1;
        pkgameCount = pkgameCount.add(1);
        pkgamesToOwner[id] = msg.sender;
        ownerPkgameCount[msg.sender] = ownerPkgameCount[msg.sender].add(1);
        emit NewpkGame(_team1, _team2, 0, peilv1,peilv2,peilv3,gametype);
    }
