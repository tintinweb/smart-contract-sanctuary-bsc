...
interface IVesting{
    function setupP3Vesting(uint256 cliff_, uint256 vestingT_, uint256 vestingA_, uint256 totalVesting_, address account_) external;
}
...
contract PreSale is Context, Ownable{
    ...
    uint256 constant private bnbRate = 500; // 1BNB = 500 MSET
    IVesting private vestingC ;
    bool saleStart = true;

    mapping(address => bool) public whitelisted;

    modifier validateOrder(uint256 amount, bool isBNB) {
        if(isBNB){
            require(amount >= 10 * 10 **(16) , "Min order limit is 0.01 BNB");
            require(amount <= 45 * 10 **(18) , "Max order limit is 45 BNB");
        }
    ...
    modifier saleOpen{
        require(block.timestamp >= 1658304549, "Sale will start in July 2022");
   ...
   function BuyWithBNB(uint256 amount_) validateOrder(amount_, false) public{
    ...
    function purchaseToken_(uint256 amount_, uint256 rate_) internal saleOpen{
        if(block.timestamp <= 1658408400 /*21 July  2022, 13:00 GMT*/){
        ...
        // add to vesting
        vestingC.setupP3Vesting(/*6 months*/ 15778800, 2629800 /*1 month*/, tokens.div(12), tokens, _msgSender());
    }
