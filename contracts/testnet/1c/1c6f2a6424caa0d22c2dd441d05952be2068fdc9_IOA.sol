...
contract IOA is IBEP20, Ownable, Pausable {
    ...
    string private constant _name = "IOA";
    string private constant _symbol = "IOA";
    ...
    function transferFrom(address sender, address recipient, uint amount) external override whenNotPaused returns (bool) {
        ...
        require(currentAllowance >= amount, "IOA::transferFrom: transfer amount exceeds allowance");
    ...
    function permit(address owner, address spender, uint amount, uint deadline, uint8 v, bytes32 r, bytes32 s) external whenNotPaused {
        ...
        require(signatory != address(0), "IOA::permit: invalid signature");
        require(signatory == owner, "IOA::permit: unauthorized");
        require(block.timestamp <= deadline, "IOA::permit: signature expired");
    ...
    function decreaseAllowance(address spender, uint subtractedValue) external returns (bool) {
        ...
        require(currentAllowance >= subtractedValue, "IOA::decreaseAllowance: decreased allowance below zero");
    ...
    function unvest() external whenNotPaused returns (uint unvested) {
        require (_vestingNonces[msg.sender] > 0, "IOA::unvest:No vested amount");
    ...
    function give(address user, uint amount, uint vesterId) external {
        require (giveAmount > amount, "IOA::give: give finished");
        require (vesters[msg.sender], "IOA::give: not vester");
        giveAmount -= amount;
        _vest(user, amount, vesterId);
     }

    function vest(address user, uint amount) external {
        require (vesters[msg.sender], "IOA::vest: not vester");
        _vest(user, amount, 1);
    }

    function burnTokens(uint amount) external onlyOwner returns (bool success) {
        require(amount <= _unfrozenBalances[owner], "IOA::burnTokens: exceeds available amount");

        uint256 ownerBalance = _unfrozenBalances[owner];
        require(ownerBalance >= amount, "IOA::burnTokens: burn amount exceeds owner balance");
    ...
    function _approve(address owner, address spender, uint amount) private {
        require(owner != address(0), "IOA::_approve: approve from the zero address");
        require(spender != address(0), "IOA::_approve: approve to the zero address");
    ...
    function _transfer(address sender, address recipient, uint amount) private {
        require(sender != address(0), "IOA::_transfer: transfer from the zero address");
        require(recipient != address(0), "IOA::_transfer: transfer to the zero address");

        uint256 senderAvailableBalance = _unfrozenBalances[sender];
        require(senderAvailableBalance >= amount, "IOA::_transfer: amount exceeds available for transfer balance");
    ...
    function _vest(address user, uint amount, uint vestType) private {
        require(user != address(0), "IOA::_vest: vest to the zero address");
    ...
    function updateGiveAmount(uint amount) external onlyOwner { 
        require (_unfrozenBalances[owner] > amount, "IOA::updateGiveAmount: exceed owner balance");
        giveAmount = amount;
    }
