...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
	...
	function _baseURI() internal view virtual returns (string memory) {
		return "https://ipfs.io/ipfs/Qmc7UKutp2yB9xhrNbWKagLxFzneFg594nWn3MgiEVMUDH/";
	}
...
contract DDBFENFT is ERC721Enumerable {
	...
	constructor() ERC721("DDB Founder Edition NFT", "DDBFENFT") {
		owner = msg.sender;
		for (uint i0 = 0; i0 < uint256(2); i0++){
 ...
 * checks that ((totalSupply) + (_amt)) is less than or equals to 20
 * checks that ((balanceOf with variable owner as (the address that called this function)) + (_amt)) is less than or equals to 7
 * checks that (amount of native currency sent to contract) is equals to ((_amt) * (50000000000000000))
 * checks that ((mintClaims with element the address that called this function) + (1)) is less than or equals to 10
...
*/
	function mint(uint256 _amt) public payable {
		...
		require(((totalSupply() + _amt) <= uint256(20)), "Exceeded Total Supply");
		require(((balanceOf(msg.sender) + _amt) <= uint256(20)), "Exceeded Individual Supply Constraints");
		require((msg.value == (_amt * uint256(10000000000000000))), "Wrong Fee");
		require(((mintClaims[msg.sender] + uint256(1)) <= uint256(10)), "Exceed number of claims for this phase");
 ...
 * if reveal then (returns super.tokenURI with variable _tokenId as tokenId as output) otherwise (returns "https://ipfs.io/ipfs/Qmc7UKutp2yB9xhrNbWKagLxFzneFg594nWn3MgiEVMUDH" as output)
