...
contract Contract is ERC20Pausable, AccessControl {
    ...
    function renounceOwnership() external onlyRole(DEFAULT_ADMIN_ROLE){
        require(msg.sender == _owner, "Error: Only owner can renounce ownership!");
        _owner = address(0);
    }
    ...
    function _setFee(uint256 inputTotalFee) internal {
        require(inputTotalFee <= 25, "Error: fees need to be smaller than 25!");
        _totalFee = inputTotalFee;
    }
    function setFee(uint256 newTotalFee) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _setFee(newTotalFee);
    }
    function setRewardWallet(address newRewardWallet) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _rewardWallet = newRewardWallet;
    }
    function totalFee() view public returns (uint256) {
        return (_totalFee);
    }
    ...
    function setRouterAndPair() internal onlyRole(DEFAULT_ADMIN_ROLE) {
