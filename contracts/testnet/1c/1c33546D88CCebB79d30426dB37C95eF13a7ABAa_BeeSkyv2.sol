...
contract BeeSkyv2 is Ownable {
  ...
  mapping(bytes32 => address) public tbytesaddress;
  ...
  function tcodeaddress(string memory _tcode) public view returns (address) {
    return tbytesaddress[bytes32(abi.encode(_tcode))];
  }
  ...
  function contribution(uint256 _id,address _refer,address _teamaddress) external noReentrant() returns (bool) {
    if(!users[msg.sender].registered){
        ...
        tbytesaddress[bytes32(abi.encode(code))] = msg.sender;
    }else{
