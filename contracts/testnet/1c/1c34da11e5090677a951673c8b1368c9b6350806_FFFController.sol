...
contract FFFController is Ownable {
    ...
    // mapping(address => uint256) public userNodeTokenRelease;

    // mapping(address => uint256) public userDrawNodeTokenRelease;

    // mapping(address => uint256) public userNodeTokenDividend;
    ...
    // mapping(uint256 => address) public teamLeader;
    ...
    // mapping(address => uint256) public nodeRecommendReward;
    ...
    uint256 public nodeRecommendRewardRate = 30;
    uint256 public nodeRecommendRewardOfUsdt = 5 * 10 ** 17;
    // uint256 public nodeMarketingFees;
    // uint256 public nodeActivityFees;
    uint256 public buyBackFees;

    // uint256 public dayMining = 83 * 10 ** 18;
    ...
    constructor() {
        ...
        srd = ERC20(0xC443Cb40FE20d8BA24D9d1592b304Ea3905d081D);

        force = ERC20(0x22Fa8506fe5E6859d3896Da570fE33FC89ab458a);
        ...
        srdPair = address(0xD2d47dA33F3A083edeb638A6464AD6471B2F9728);
        forcePair = address(0x3140d909c47c4A16671810ec61Da3f3Ac822baEd);

        // _swapRouter = ISwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        _swapRouter = ISwapRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);

        //teamLeader
        nodeIds[address(0xf8ED7d92d6209ec070223DCaD57F98E6C9feE7a5)] = 1;
        nodeIds[address(0xC5a89985D57FdddEFCE3E9f26faB99EbF471516A)] = 2;
        nodeIds[address(0xf34a7238cF9423A5FC6852c8882d774BF08446DE)] = 3;

        nodeIdToAddress[1] = address(0xf8ED7d92d6209ec070223DCaD57F98E6C9feE7a5);
        nodeIdToAddress[2] = address(0xC5a89985D57FdddEFCE3E9f26faB99EbF471516A);
        nodeIdToAddress[3] = address(0xf34a7238cF9423A5FC6852c8882d774BF08446DE);

        userTeamId[address(0xf8ED7d92d6209ec070223DCaD57F98E6C9feE7a5)] = 1;
        userTeamId[address(0xC5a89985D57FdddEFCE3E9f26faB99EbF471516A)] = 2;
        userTeamId[address(0xf34a7238cF9423A5FC6852c8882d774BF08446DE)] = 3;
    ...
    function activateNode(address recommender, uint256 tokenId) public {
        ...
        _bindRecommender(msg.sender, recommender);
    ...
    function buyNode(address recommender, uint256 nodeType) public {
        ...
        require(checkCanBuyNode(recommender, nodeType) == 0, "You can not open the node");
        ...
        userNodeRecommendeProfits[recommender] += recommendFees;
        addNodeProfit(nodeType, recommendFees);

        divideMarketingFees(marketingFees);
    ...
    function addNodeProfit(uint256 nodeType, uint256 fees) public {
    ...
    function divideMarketingFees(uint256 fees) public {
    ...
    function divideRecommendFees(address user, uint256 fees) public returns(uint256 divideFees) {

        uint256 i;
        uint256 count;
        address user0 = myRecommender[user];

        userNodeProfits[user0] += fees.mul(nodeRecommendRewardRate).div(100);

        for(i = 1; i < 21; i++){
            if(myRecommender[user0] != address(0)){
                // usdt.transfer(myRecommender[user0], nodeRecommendRewardOfUsdt);
                userNodeProfits[myRecommender[user0]] += nodeRecommendRewardOfUsdt;
                user0 = myRecommender[user0];
                count++;
            }else{
                break;
        ...
        //     usdt.transfer(myRecommender[user], fees - nodeRecommendRewardOfUsdt.mul(count));
        // }
        divideFees = fees.mul(nodeRecommendRewardRate).div(100) + nodeRecommendRewardOfUsdt.mul(count);
    }
    ...
    function buyPower(uint256 types) public {
        ...
        if(srd.balanceOf(msg.sender) >= cost.mul(buyPowerSrdDiscount).div(100)){
            tokenType = 2;
            ...
            srd.transferFrom(msg.sender, deadWallet, cost);
        }else{
            ...
            divideMarketingFees(marketingFees);
            uint256 divideFees = divideRecommendFees(msg.sender, recommendFees);

            addNodeProfit(_queryNodeType(nodeIds[msg.sender]), divideFees);
    ...
    function _bindRecommender(address user, address recommender) public {
        ...
        require(checkCanBindRecommender(user, recommender) == 0, "Recommender binding failed");

        myRecommender[user] = recommender;

        invitees[recommender].push(user);

        bindTime[user] = block.timestamp;

        userTeamId[user] = userTeamId[recommender];

        updateUnderPower(user);

        setRegisterTime(user);

        emit BindRecommender(user, recommender);
    }

    function drawNodeRecommendeProfit() public {

        require(queryUserProfitProgress(msg.sender) < 300, "The profit has reached the upper limit, please reinvest");
    ...
    function drawNodeTokenDividend() public {
        ...
        uint256 totalDividend = _queryMyNodeLevelDividend(msg.sender);

        require(userDrawNodeTokenDividend[msg.sender] < totalDividend, "No profit can be withdrawn");

        uint256 dividend = totalDividend - userDrawNodeTokenDividend[msg.sender];

        srd.transfer(msg.sender, dividend);
    ...
    function _queryTokenMining(address user, uint256 power) public view returns(uint256 release) {
        ...
        if(block.timestamp - registerTime[user] < 1 days){
            ...
            if(userPower[user] > 0 && newPower > 0){
                release = perBlockMining.mul(intervalBlock * _newUserFee * userPower[user]).div(100 * newPower);
            }
        }else{
            if(userPower[user] > 0 && totalPower > newPower){
                release = perBlockMining.mul(intervalBlock * (100 - _newUserFee) * power).div(100 * (totalPower - newPower));
            }
        }
    }
    ...
    function _queryAvailableUnderPower(address user) public view returns(uint256 availableUnderPower) {
        ...
        if(level > 0){
            availableUnderPower = _queryEffectiveUnderPower(msg.sender);
    ...
    function queryNodeTokenReward(address user) public view returns(uint256 total, uint256 draw, uint256 notDraw, uint256 canDraw) {
        ...
        notDraw = total - draw;
        canDraw = _queryNodeTokenTotalRelease(msg.sender).div(10 ** 14);
    }
    ...
    uint256 totalMining, uint256 blockMining, uint256 dayMining, uint256 perPowerMining){
        unMining = srd.balanceOf(address(this)).div(10 ** 14);
    ...
    function queryMyMining(address user) public view returns(uint256 total, uint256 draw, uint256 canDraw){
        draw = userMiningProfits[user];
        canDraw = _queryTokenMining(user, _queryAvailableUnderPower(msg.sender));
        total = draw + canDraw;
    }
    ...
    function queryUserProfitProgress(address user) public view returns(uint256 progress) {
        uint256 srdProfit = userMiningProfits[user] + tokenForTimeRelease[nodeIds[user]] + tokenForPriceRelease[nodeIds[user]];
        uint256 usddtProfit = userNodeRecommendeProfits[user] + userNodeProfits[user];
        uint256 total = srdEqualsToUsdt(srdProfit) + usddtProfit;
    ...
    function checkCanBindRecommender(address user, address recommender) public view returns (uint256) {
        if(nodeIds[recommender] == 0) return 1;//"The recommender have not opened the node"
