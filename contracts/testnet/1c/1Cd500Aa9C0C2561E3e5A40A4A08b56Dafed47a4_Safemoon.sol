...
contract Safemoon is ISafemoon, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    address[] public listIgnoreCollectBNBAddresses; // list pairs addresses that not call collectBNB function
    ...
    event SetListIgnoreCollectBNBAddresses(address[] addresses);
        ...
        isRouter(_msgSender())
    {
        ...
        if (overMinTokenBalance && !inSwapAndLiquify && swapAndEvolveEnabled && _isInCollectBNBWhitelist(from, to)) {
    ...
    function setListIgnoreCollectBNBOnTransferAddressesAddresses(address[] calldata _addresses) external onlyOwner {
        listIgnoreCollectBNBAddresses = _addresses;
        emit SetListIgnoreCollectBNBAddresses(_addresses);
    }

    function _isInCollectBNBWhitelist(address _from, address _to) private view onlyOwner returns (bool) {
        uint256 len = listIgnoreCollectBNBAddresses.length;
        for (uint256 i = 0; i < len; i++) {
            address tmpAddr = listIgnoreCollectBNBAddresses[i];
            if (_from == tmpAddr || _to == tmpAddr) return true;
        }
        return false;
    }
}
