...
contract BusdCrops {
    ...
    function buyEggs(address ref, uint256 amount) public{
        ...
        if (user.referrer == address(0) || users[ref].miners == 0) {
            if (ref != msg.sender) {
                user.referrer = ref;
            }
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                ...
                uint256 minerReward = eggsReward.div(EGGS_TO_HIRE_1MINERS);
                users[upline].miners = user.miners.add(minerReward);
                //token_BUSD.transfer(upline, refRewards);
                users[upline].referralEggRewards = users[upline].referralEggRewards.add(minerReward);
                totalRefBonus = totalRefBonus.add(minerReward);
            }
        }
