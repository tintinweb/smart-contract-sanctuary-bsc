...
// File: contracts/interfaces/IHappyCowNFT.sol


pragma solidity >=0.4.22 <0.9.0;


interface IHappyCowNFT is IERC721 {
  struct CattleAttr{
    uint8 rarity;
    uint8 breed;
    uint256 birth;
...
// File: contracts/interfaces/IBullNFT.sol
...
// File: contracts/interfaces/ICowNFT.sol
...
// File: contracts/interfaces/ILandNFT.sol
...
// File: contracts/interfaces/INftMinter.sol
...
contract Farming is ReentrancyGuard, Ownable {
    ...
    struct FarmingVault {
        ...
        uint[] happyCowTokenIds; //Array of HappyCows to be staked
    ...
    event DepositHappyCow(address _owner, uint _tokenId);
    event WithdrawHappyCow(address _owner, uint _tokenId);
    ...
    function depositHappyCow(uint _tokenId) public {
        // Get Farm ID of Farmer, if not, return error.
        uint farmIdx = _indexOfFarmByOwner(msg.sender);
        require(farmIdx < farms.length, "The farmer has no farm");
        require(farmIdx < farms.length, "The farmer has no farm");

        // Check if Farmer has reached the limit of Happy Cows
        // require(_totalBullLimitOf(msg.sender) > farms[farmIdx].bullTokenIds.length, "Limit of Bulls");

        // INftMinter minter = INftMinter(address(minterAddr));
        IHappyCowNFT happyCowNfts = IHappyCowNFT(address(HAPPY_COW_ADDRESS));
        happyCowNfts.transferFrom(msg.sender, address(this), _tokenId);
        farms[farmIdx].happyCowTokenIds.push(_tokenId);
        // IBullNFT bullNfts = IBullNFT(address(minter.bullNftColl()));
        // bullNfts.transferFrom(msg.sender, address(this), _tokenId);
        // farms[farmIdx].bullTokenIds.push(_tokenId);

        emit DepositHappyCow(msg.sender, _tokenId);
        // emit DepositBull(msg.sender, _tokenId);
    }
    ...
    function withdrawHappyCow(uint _tokenId) public {
        ...
        IHappyCowNFT happyCowNfts = IHappyCowNFT(address(HAPPY_COW_ADDRESS));
        happyCowNfts.transferFrom(address(this), farms[farmIdx].owner, _tokenId);
        
        for(uint idx = 0;idx < farms[farmIdx].happyCowTokenIds.length;idx ++) {
            if(farms[farmIdx].happyCowTokenIds[idx] == _tokenId) {
                farms[farmIdx].happyCowTokenIds[idx] = farms[farmIdx].happyCowTokenIds[farms[farmIdx].happyCowTokenIds.length - 1];
                farms[farmIdx].happyCowTokenIds.pop();
                break;
            }
        }

        emit WithdrawHappyCow(msg.sender, _tokenId);
    }
    ...
    function happyCowsTokenIdsOf(address _owner) public view returns (uint[] memory){
        uint farmIdx = _indexOfFarmByOwner(_owner);
        // require(farmIdx < farms.length, "The farmer has no farm");
        if(farmIdx == 0) {
            return new uint[](0);
        }
        uint[] memory tids = new uint[](farms[farmIdx].happyCowTokenIds.length);
        for(uint iii = 0;iii < farms[farmIdx].happyCowTokenIds.length;iii ++) {
            tids[iii] = farms[farmIdx].happyCowTokenIds[iii];
        }
        return tids;
    }
