...
contract JuneLogic is OwnableUpgradeable, ERC20Upgradeable, Manager, Console {
    ...
    function setDividendAdds(address value, bool _is) public onlyManager {
        if (_is) {
            require(DividendAddsIndex[value] <= 0, 'existed');
            DividendAdds.push(value);
            DividendAddsIndex[value] = DividendAdds.length;
        } else {
            require(DividendAddsIndex[value] > 0, 'existed');
            delete DividendAdds[DividendAddsIndex[value] - 1];
        }
    }
    ...
    function getDividendAddsAll() public view returns (address [] memory) {
        return DividendAdds;
    }
    ...
    function Dividend() public onlyManager {
        ...
        uint256 quantity = balanceOf(SlippageAddr);
        uint256 DividendNum = quantity.mul(uint256(10) ** 2).div(LpTotal);
        log("Dividend:", DividendNum);
        for (uint i = 0; i < DividendAdds.length; i++) {
            uint256 userBalance = LiquidityWallet.balanceOf(DividendAdds[i]);
            log("userAddress:", DividendAdds[i]);
            uint256 userDividend = DividendNum.mul(userBalance).div(uint256(10) ** 2);
            log("userDividend", userDividend.div(uint256(10) ** 2));
            if (userDividend > 0 && userBalance >= DividendMin && DividendAdds[i] != address(0)) {
                //                super._transfer(SlippageAddr, DividendAdds[i], userDividend);
            }
        }
    }
