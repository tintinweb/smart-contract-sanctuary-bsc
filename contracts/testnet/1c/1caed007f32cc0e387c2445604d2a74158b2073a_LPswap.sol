...
contract LPswap {
    ...
    uint256 public base_bby;
    uint256 public max_private_bby = 250000000000000;
    uint256 public max_pledge_bby = 250000000000000;
    uint256 public month3_rate = 20;
    uint256 public month6_rate = 30;
    uint256 public month12_rate = 50;
    bool    public private_switch = true;
    bool    public pledge_switch = true;
    mapping(address => bool) public node_index;
    mapping(address => bool) public private_index;
    ...
    constructor() public {
        ...
        base_bby = 100 * 10 ** bbyToken_decimals;
    }

    function setSwitch(bool Is) public {
        require(msg.sender==owner,' only owner');
        private_switch = Is;
    }

    function setPledgeSwitch(bool Is) public {
        require(msg.sender==owner,' only owner');
        pledge_switch = Is;
    }

    function setPledgeRate(uint256 amount1,uint256 amount2,uint256 amount3) public {
        require(msg.sender==owner,' only owner');
        month3_rate = amount1;
        month6_rate = amount2;
        month12_rate = amount3;
    }
    ...
    function monthRate() public view returns (uint256 amount1,uint256 amount2,uint256 amount3){
        return (month3_rate,month6_rate,month12_rate);
    }
    ...
    function privatePlacement(uint256 amount) public{
        require(amount>=1, "LP: amount must be a multiple of 1");
        require(amount%1==0, "LP: amount must be a multiple of 1");
        require(amount<10, "LP: amount max 10");
        require(private_switch == true, "LP: private close");
        require(private_index[msg.sender] == false, "LP: already private placement");
        uint _quote = amount * base_bby / 2; 
        private_index[msg.sender] = true;
    ...
    function pledgeAmount(uint256 amount,uint256 month) public {
        ...
        require(pledge_switch == true, "LP: pledge close");
        uint256 rate = 0;
        if(month==3){
            rate = month3_rate;
        }else if(month==6){
