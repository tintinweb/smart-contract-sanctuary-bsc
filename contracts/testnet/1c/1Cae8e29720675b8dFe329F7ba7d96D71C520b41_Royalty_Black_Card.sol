...
contract Royalty_Black_Card  is ERC721Enumerable, Ownable
{
    ...
    uint public mint_price= 0.22 ether;
    uint public wl_mint_price =0.17 ether;
    ...
    function reservenfts(uint chosenAmount) public onlyOwner{
            require(totalSupply()+chosenAmount<=_TOTALSUPPLY-reserve,"Quantity must be lesser then MaxSupply");
            require(chosenAmount > 0, "Number of tokens can not be less than or equal to 0");
    ...
    function allpayout(uint256 Newam) public onlyOwner{
        // require(Newam >= 0.000001 ether);
        Newam= Newam.mul(1000000000000000000);
