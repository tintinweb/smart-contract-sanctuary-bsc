...
// File: contracts/farms/DTG/common/IERC20Metadata.sol
...
// File: contracts/farms/DTG/DTGPool.sol
...
contract DTGPool is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
...
//        uint256 _poolLimitPerUser,
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        IBEP20 EGGC = IBEP20(payTokenDTG);
         require(EGGC.transferFrom(msg.sender, vault, amountDTG));
    ...
    /* DTG Admin functions to setup pool */

    function DTGStartIn(uint256 _blocks) public onlyOwner {
        DTGSetStart(block.number.add(_blocks));
    }

    function DTGSetStart(uint256 _startBlock) public onlyOwner {
    ...
    function DTGSetDuration(uint256 _durationBlocks) public onlyOwner {
        bonusEndBlock = startBlock.add(_durationBlocks);
        DTGCalcRewardPerBlock();
    }

    function DTGCalcRewardPerBlock() public onlyOwner {
        uint256 rewardBal = rewardToken.balanceOf(address(this));
        rewardPerBlock = rewardBal.div(rewardDuration());
    }

    function DTGSetRewardMaxTxAmount(uint256 _maxTxAmount) public onlyOwner {
    ...
    function rewardPerBlockUI() public view returns (uint256) {
      ...
      function setPayDTG(address _pay) public onlyOwner {
   payTokenDTG = _pay; 
         
     }
     
     function setPriceDTG(uint _price) public onlyOwner {
    amountDTG = _price;
    }
