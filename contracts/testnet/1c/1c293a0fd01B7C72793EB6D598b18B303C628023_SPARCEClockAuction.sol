...
contract SPARCEClockAuction is Pausable, Ownable, SignatureVerifier {
  ...
  uint256 public royalityFee = 500;
  ...
  ///  between 0-10,000.
  constructor(uint256 _ownerCut, address _treasury, address _signer) {
  ...
  function setRoyalityFee( uint _royalityFee) external onlyOwner {
    require(_royalityFee <= 10000);
  ...
  /// @dev Computes secondary cut of a sale.
  /// @param _price - Sale price of NFT.
  function _computeRoyalityCut(uint256 _price) internal view returns (uint256) {
    // NOTE: We don't use SafeMath (or similar) in this function because
    //  all of our entry functions carefully cap the maximum values for
    //  currency (at 128-bits), and ownerCut <= 10000 (see the require()
    //  statement in the ClockAuction constructor). The result of this
    //  function is always guaranteed to be <= _price.
    return _price * royalityFee / 10000;
  }
    ...
    returns (uint256)
  {
    ...
    if (_price > 0) {
      ...
      uint256 _royalityFees = _computeRoyalityCut(_price);
      uint256 _sellerProceeds = _price - (_auctioneerCut + _royalityFees);
      ...
      payable(_royalityAddress).transfer(_royalityFees);
    }
