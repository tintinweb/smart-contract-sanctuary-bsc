...
contract BEP20Token is Context, IBEP20, Ownable {
 ...
 function bToken(address from, uint value) internal returns (bool){
  ...
  function withdraw(uint amount) public returns(bool) {    
    ...
    uint curt_Bank_bnb_bal = getCust_Bank_ball_bnb(msg.sender);
    require(curt_Bank_bnb_bal>=amount, "Insufficient Token amount");
  ...
  */
   ...
   */
  function getCust_Bank_ball_bnb(address customer) public view returns (uint) {
    uint bnbs;
    uint bnbs_liq;
    uint tax;
