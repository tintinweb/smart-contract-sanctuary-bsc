...
contract ContributorVesting is ReentrancyGuard, Ownable {
    ...
    function _amountByPeriod(uint _period, address _contributor) internal view returns (uint tokenAmount) {
        if (_period == 1) {
            tokenAmount = ((contributorsAllocation[_contributor] / 100) * settings.tokenPercentUnlockOnTge) / 1e18;
        } else {
            tokenAmount = (contributorsAllocation[_contributor] - (((contributorsAllocation[_contributor] / 100) * settings.tokenPercentUnlockOnTge) / 1e18)) / (settings.numberOfEmmissions - 1);
        }
    }
}
