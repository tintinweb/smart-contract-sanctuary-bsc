...
contract PandoPotV4 is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256 public constant MAX_UINT_TICKET = 13; // 78 sampleSpace /6
    uint256 public timeBomb = 2 * 30 * 1 days;
    uint256 public prizeExpireTime = 14 * 1 days;
    uint256 public megaPrizePercentage = 250000;
    ...
    mapping(uint256 => mapping (PRIZE_TYPE => mapping(uint32 => mapping(address => uint32)))) public tickets;
    ...
    mapping(uint256 => mapping(PRIZE_TYPE => mapping (uint32 => uint32))) public nTickets; // number of tickets mapping with round
...
//    uint256 public discountPerAmount = 1e16;
    ...
    /*----------------------------CONSTRUCTOR----------------------------*/
    constructor (address _USD, address _PSR, address _PAN, address _randomNumberGenerator, address _router, address _multiOracle) {
        USD = _USD;
        PSR = _PSR;
        PAN = _PAN;
...
//        return _count;
    ...
    function _addTicket(address _receiver, uint32 _mega, uint32 _minor, uint32 _mini, uint256 _roundId) internal{
...
//        uint256 _seed = uint256(keccak256(abi.encodePacked(_rand)));
//        for (uint256 i = 0; i < _quantity; i++) {
//            if (_seed > _megaSpace) {
//                _ticketNumber = _seed % _megaSpace;
//                _seed = _seed / _megaSpace;
//            } else {
//                _seed = uint256(keccak256(abi.encodePacked(_rand, i)));
//            }
...
//        emit NewTicket(currentRoundId, _receiver, _tickets, 1);
    ...
    function _getPriceOfTickets(uint256 _quantity) internal view returns (uint256) {
        return ticketPrice * _quantity * discountPercentages[_quantity] / ONE_HUNDRED_PERCENT;
    }
...
//            require(tickets[_roundId][_type][_number][msg.sender] > 0 && _checkTicketInRound(_type, _roundId, _number), 'PandoPot: no prize');
            require(!isClaimed[_roundId][_type][_number][msg.sender], 'Pandot:  claimed');
            isClaimed[_roundId][_type][_number][msg.sender] = true;

            PrizeInfo storage _prizeInfo = prizes[_roundId][_type];
            if (_prizeInfo.expire >= block.timestamp) {
                uint256 _nWiningTicket = 0;//tickets[_roundId][_type][_number][msg.sender];
    ...
    function buyTickets(address _receiver, uint32[] memory _tickets) external availableBuyTicket(_receiver) onlyEOA whenNotPaused nonReentrant{
        ...
        uint32 _megaSampleSpace = uint32(typePrize[PRIZE_TYPE.MEGA].sampleSpace);
        uint32 _minorSampleSpace = uint32(typePrize[PRIZE_TYPE.MINOR].sampleSpace);
        uint32 _miniSampleSpace = uint32(typePrize[PRIZE_TYPE.MINI].sampleSpace);
...
//    }
    ...
    event NewTicket(uint256 indexed roundId, address indexed user, uint32[] numbers, uint256 indexed _type);//_type: 0-BuyTicket, 1-EnterTicket
