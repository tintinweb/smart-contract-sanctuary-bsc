...
contract StackProNFT is ERC721Enumerable{
    ...
    function createMulti(uint256 collectionid,address to,string memory _tokenURI,string memory _mintname,string memory _nftowner,uint256 _copies,string memory description) external 
    {
        require(!stopduplicate[_tokenURI],"value not allowed");

        for(uint256 i=1; i<=_copies; i++){ 
            tokenidmint+=1;
            uint256 timeperiod = block.timestamp;
            collectionstored[collectionid].push(tokenidmint);
            totalnft[collectionid]+=1;
