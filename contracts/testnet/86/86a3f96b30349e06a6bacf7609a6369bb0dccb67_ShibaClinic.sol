...
contract ShibaClinic is Context, IBEP20, Ownable {
   ...
   function SetWalletLimit(address wallet, uint256 amount,uint day) onlyOwner() public{
      ...
      uint256 endDate = date +  day * 1 days;
    ...
    function ReportWalletLimit() public view returns(LimitedAddress[] memory){

        LimitedAddress[] memory mylist = new LimitedAddress[](_addressLimited[msg.sender].length);

        for  (uint i=0 ; i < _addressLimited[msg.sender].length;i++)
        {
            uint32 limitedId = _addressLimited[msg.sender][i];
    ...
    function ReportWalletLimit(address wallet) public onlyOwner view returns(LimitedAddress[] memory){
