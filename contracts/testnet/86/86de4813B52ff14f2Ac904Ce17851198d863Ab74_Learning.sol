...
contract Learning is Ownable, IERC721Receiver
{
    ...
    function UpgrateLevelRobot(uint256 tokenId) public IsEnableSystem
    {
        ...
        require(Robot.ownerOf(tokenId) == user || RobotJoin[user] == tokenId, "Error UpgrateLevelRobot: Invalid tokenId");
    ...
    )
    {
        ...
        priceUpgrateLevel = new uint256[](maxLevel);
        for(uint level = 1; level <= maxLevel; level++)
        {
            priceUpgrateLevel[level] = PriceUpgrate[level]; 
        }

        delayBlockLearn = DelayBlockLearn;
        totalBlockLearn = TotalBlockLearn;

        rewardPerBlockOfLevel = new uint256[](maxLevel);
        ...
        // if(startBlockLearn != 0)
        // {
        //     if(blockNumber >= startBlockLearn) {

        //         pendingReward = (blockNumber.sub(startBlockLearn) >= pendingBlockLearn) ? 
        //                             blockNumber.sub(startBlockLearn).mul(rewardPerBlockOfLevel[levelRobotJoin]):
