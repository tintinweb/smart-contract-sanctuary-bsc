...
contract Tournament is AccessControl {
    ...
    function initialize(
        ...
        require(msg.sender == factory, "TOURNAMENT::UNAUTHORIZED");
        require(
            _registrationStartTime > block.timestamp,
            "TOURNAMENT:: REGISTRATION START TIME MUST BE GREATER THAN NOW"
        );
        require(
            _registrationStartTime < _registrationEndTime,
            "TOURNAMENT:: REGISTRATION END TIME MUST BE GREATER THAN START TIME"
        );
    ...
    function setTokens(address[] calldata _tokens) external {
        require(
            ...
            "TOURNAMENT::NO_PERMISSION"
        );
        require(
            _tokens.length > 0,
            "TOURNAMENT::ATLEAST ONE TOKEN MUST BE ADDED"
        );
        require(
            block.timestamp < registrationStartTime ||
                registrationStartTime == 0,
            "TOURNAMENT::UNABLE TO SET TOKENS THIS TIME"
        );
    ...
    function setRegistrationFee(
        ...
        require(
            ...
            "TOURNAMENT::REGISTRATION START TIME MUST BE SET"
        );
        require(
            _registrationStartTime < _registrationEndTime,
            "TOURNAMENT::REGISTRATION START CANNOT BE GREATER THAN THE END TIME"
        );
        if (_registrationToken == address(0) && _registrationFeeAmount == 0) {
            require(
                _validToken(_registrationToken),
                "TOURNAMENT::TOKEN NOT ALLOWED"
            );
        }
        require(
            block.timestamp < registrationStartTime ||
                registrationStartTime == 0,
            "TOURNAMENT::CANNOT SET REGISTRATION FEE THIS TIME"
        );
        registrationToken = _registrationToken;
        registrationFeeAmount = _registrationFeeAmount;
        registrationStartTime = _registrationStartTime;
    ...
    function register() external payable {
        ...
        //require(block.timestamp > registrationStartTime && block.timestamp < registrationEndTime, "TOURNAMENT::UNABLE TO REGISTER THIS TIME");

        registeredAddresses[msg.sender] = true;

        if (registrationFeeAmount == 0) {
            IEventEmitter(eventEmitter).emitRegsiterEvent(
                address(this),
                msg.sender,
        ...
        if (registrationToken == address(0)) {
            require(
                ...
                "TOURNAMENT::NOT ENOUGH REGISTRATION FEE AMOUNT"
            );
        } else {
    ...
    function refundMultiple(address[] calldata _userAddresses) external {
        ...
        require(!isRefunded, "TOURNAMENT::REFUNDED");
        for (uint256 i = 0; i < _userAddresses.length; i++) {
            require(
                registeredAddresses[_userAddresses[i]],
                "TOURNAMENT:: USER NOT REGISTERED"
            );
    ...
    function withDrawTournament() external {
        ...
        uint256 calculatedPlatformFee = (registrationFeeAmount *
            (pfeeRate / 100));

        uint256 calculatedTournamentFee = (registrationFeeAmount *
            (tfeeRate / 100));
    ...
    function claimRefund() external {
        require(
            ...
            "TOURNAMENT:: CANNOT CALL FROM CONTRACT"
        );
        require(
            !blackListAddresses[msg.sender],
            "TOURNAMENT:: YOU ARE IN BLACK LIST"
        );
        require(
            teamTokenRefund[msg.sender] > 0,
            "TOURNAMENT:: NO TOKEN TO CLAIM"
        );
    ...
    function distributeTokensReward(
        ...
        require(
            ...
            "TOURNAMENT::MUST REFUND BEFORE DISTRIBUTE"
        );
        require(
            _userAddresses.length == _tokenRewards.length &&
                _userAddresses.length == _percentages.length,
            "TOURNAMENT:: ADDRESSES,TOKENS AND AMOUNTs MUST BE SAME LENGTH"
        );
        require(
    ...
    function claimTokensReward() external {
        ...
        require(
            ...
            "TOURNAMENT:: NOT ALLOWED TO CLAIM THIS TIME"
        );
        require(
            ITournamentFactory(factory).getpWallet() != address(0),
            "TOURNAMENT::PLATFORM WALLET HAS NOT BEEN SETUP"
        );
