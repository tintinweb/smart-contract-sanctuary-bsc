...
contract COWSGAME1155 is ReentrancyGuard {
    ...
    event NFTWithdraw(address nft, address withdrawer, bytes tokenIds, uint256 amount, string message);
    event NFTClaim(address nft, address withdrawer, bytes tokenIds, uint256 amount, string message);
    ...
    constructor() public 
    {
        ...
        POOL_GAME = 0x5b30eee7F0d04e044b11C8E7Aa32eF9B4792c847;
    ...
    */
    ...
    function withdrawNFTs(
        ...
        userInfo[msg.sender].nftDeposit[_nft] -= _amount;
        userInfo[msg.sender].lastUpdatedAt = block.timestamp;
        emit NFTWithdraw(_nft, msg.sender, abi.encodePacked(_tokenId), _amount, _message);
    ...
    ) external onlySupportedNFT(_nft) ifNotPaused returns (uint256)
    {
        ...
        emit NFTClaim(_nft, msg.sender, abi.encodePacked(_tokenId), _amount, _message);
        userSigned[msg.sender][signature] = true;
        return _tokenId;
    }
