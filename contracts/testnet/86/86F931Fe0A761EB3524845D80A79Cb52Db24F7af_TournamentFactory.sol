...
contract TournamentFactory is Pausable, Initializable, AccessControl
{
    ...
    function initialize(address _signer, address _pWallet, address _destAddress) external initializer{
        //Setup Role
    ...
    function setClaimTimeDuration(uint256 _claimTimeDuration) public onlyRole(DEFAULT_ADMIN_ROLE){
    ...
    function setPlatformFeeRate(uint _pFeeRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        pFeeRate = _pFeeRate;
    }

     // set tournament fee rate
    function setTournamentFeeRate(uint _tFeeRate) external onlyRole(DEFAULT_ADMIN_ROLE) {
        tFeeRate = _tFeeRate;
    }
    ...
    function createTournament(
        ...
        require(_verifyCreateTournament(_tournamentId, _tokens, _registrationToken, _registrationStartTime, _registrationEndTime, _registrationFeeAmount, _signature), "TOURNAMENTFACTORY:INVALID_SIGNATURE");

        bytes memory bytecode = type(Tournament).creationCode;

        bytes32 salt =
            keccak256(abi.encodePacked(msg.sender, pFeeRate, tFeeRate, _tournamentId, _tokens, _registrationToken, _registrationStartTime, _registrationEndTime, _registrationFeeAmount, block.timestamp, allTournaments.length));
        assembly {
            tournament := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        ITournament(tournament).initialize(_tournamentId, _tokens, _registrationToken, _registrationStartTime, _registrationEndTime, _registrationFeeAmount, pFeeRate, tFeeRate);
        
        if(!isReady){
            IDESPortsToken(destAddress).applyWhiteList(tournament);
        }
        
        getTournaments[msg.sender].push(tournament);
        allTournaments.push(tournament);
        emit CreateTournamentEvent(msg.sender, _tokens, tournament, _registrationToken, _registrationStartTime, _registrationEndTime, _registrationFeeAmount, pFeeRate, tFeeRate);
    }
    ...
    function setOrganizer(address organizerAddress, address tournamentAddress) external onlyRole(DEFAULT_ADMIN_ROLE){
        ...
        emit SetOrganizerEvent(msg.sender, organizerAddress, tournamentAddress);
    }
    ...
    function _verifyCreateTournament(
        ...
        return verify(signer, _tournamentId, _tokens, _registrationToken, _registrationStartTime, _registrationEndTime, _registrationAmount, _signature);
    }
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_tournamentId, _tokens, _registrationToken, _registrationStartTime, _registrationEndTime, _registrationFeeAmount));
    }
    ...
    function verify(
        ...
        bytes32 messageHash = getMessageHash(_tournamentId, _tokens, _registrationToken, _registrationStartTime, _registrationEndTime, _registrationFeeAmount);
...
contract Tournament is AccessControl {
    ...
    event SetClaimTimeEvent(address adminAddress, uint256 claimTime);
    ...
    */
    function initialize(
        ...
        require(_registrationStartTime > block.timestamp,"TOURNAMENT:: REGISTRATION START TIME MUST BE GREATER THAN NOW");
        require(_registrationStartTime < _registrationEndTime,"TOURNAMENT:: REGISTRATION END TIME MUST BE GREATER THAN START TIME");
        
        //Setup Role
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(DEFAULT_ADMIN_ROLE, ITournamentFactory(factory).getpWallet());
    ...
    function setTokens(address[] calldata _tokens) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(ORGANIZER_ROLE, msg.sender), "TOURNAMENT::NO_PERMISSION");
    ...
    function register() external payable {
        ...
        require(!registeredAddresses[msg.sender], "TOURNAMENT::ALREADY REGISTERED");
        
        registeredAddresses[msg.sender] = true;

        if(registrationFeeAmount == 0)
        {
            emit RegsiterEvent(msg.sender, registrationFeeAmount);
            return;
        } 
        ...
        if(registrationToken == address(0)) {
            ...
            IERC20(registrationToken).safeTransferFrom(msg.sender, address(this), registrationFeeAmount);
        }
    ...
    function refundMultiple(address[] calldata _userAddresses) external {
        ...
        emit RefundMultipleEvent(msg.sender, _userAddresses, registrationFeeAmount);
    }
    ...
    function withDrawTournament() external {
        require(registeredAddresses[msg.sender], "TOURNAMENT:: USER NOT REGISTERED");
        ...
        uint calculatedPlatformFee = (registrationFeeAmount * (pfeeRate/100));

        uint calculatedTournamentFee = (registrationFeeAmount * (tfeeRate/100));

        uint claimAmount = registrationFeeAmount - calculatedPlatformFee - calculatedTournamentFee;
        ...
        else
        {
            ...
            IERC20(registrationToken).safeTransfer(ITournamentFactory(factory).getpWallet(), calculatedTournamentFee);

            // Transfer fee to Organizer
            IERC20(registrationToken).safeTransfer(organizer, calculatedTournamentFee);

            //Transfer reward to team captain
            IERC20(registrationToken).safeTransfer(msg.sender, registrationFeeAmount);
        }

        emit WithDrawEvent(msg.sender, registrationFeeAmount);
    }
    ...
    function claimRefund() external {
        require(msg.sender == tx.origin, "TOURNAMENT:: CANNOT CALL FROM CONTRACT");
        require(!blackListAddress[msg.sender], "TOURNAMENT:: YOU ARE IN BLACK LIST");
        require(teamTokenRefund[msg.sender] > 0, "TOURNAMENT:: NO TOKEN TO CLAIM");

        uint claimAmount = teamTokenRefund[msg.sender];
        ...
        emit ClaimRefundEvent(msg.sender, registrationToken, claimAmount);
    }
    ...
    function addSponsorToPrizePool(address _token, uint256 _amount) external payable {
        ...
        emit AddPrizePoolEvent(msg.sender,_token, _amount);
    }
    ...
    function removeSponsorFromPrizePool(address _token) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || msg.sender == ITournamentFactory(factory).getpWallet(), "TOURNAMENT:: NO_PERMISSION");
        require(msg.sender == tx.origin, "TOURNAMENT:: CANNOT CALL FROM CONTRACT");
        ...
        emit RemovedSponsoredEvent(msg.sender, _token, removedAmount);
    }
    ...
    function donate(address _token, uint256 _amount) external payable {
        ...
        userDonated[msg.sender][_token] = userDonated[msg.sender][_token] + (_amount);
        ...
        emit DonatedEvent(msg.sender, _token, _amount);
    }
    ...
    function withDrawEmergency() external payable{
        require(msg.sender == ITournamentFactory(factory).getpWallet(), "TOURNAMENT:: NO_PERMISSION");
        ...
        for(uint i = 0; i < tokens.length; i++){
            ...
            {
                IERC20(tokens[i]).safeTransfer(msg.sender, currPrizePool[tokens[i]]);
            }
        }
    }
    ...
    function distributeTokensReward(
        ...
        //set Claim Time
        claimTime = block.timestamp + ITournamentFactory(factory).getClaimTimeDuration();
        ...
        for(uint i = 0; i < _userAddresses.length; i++){
            ...
            {
                uint256 rewardAmount = (totalPrizePool[_tokenRewards[i]] * _percentages[i]) / (1e18);
                teamTokenRewards[_userAddresses[i]][_tokenRewards[i]] = teamTokenRewards[_userAddresses[i]][_tokenRewards[i]] + (rewardAmount);
            }
        }
        
        emit DistributeRewardEvent(msg.sender, _userAddresses, _tokenRewards, _percentages);
    }

    function setClaimTime(uint256 _claimTime) external onlyRole(DEFAULT_ADMIN_ROLE){
        claimTime = _claimTime;
        emit SetClaimTimeEvent(msg.sender, _claimTime);
    }
    ...
    function claimTokensReward() external {
        ...
        for(uint i=0; i < tokens.length; i++){
                uint totalAmount = teamTokenRewards[msg.sender][tokens[i]];
                if (totalAmount == 0){
                    continue;
                }
                ...
                uint calculatedPlatformFee = (totalAmount * (pfeeRate/100));

                uint calculatedTournamentFee = (totalAmount * (tfeeRate/100));

                uint claimAmount = totalAmount - calculatedPlatformFee - calculatedTournamentFee;
                ...
                if (tokens[i]== address(0)){
                    ...
                    // Transfer fee to Fee Wallet
                    IERC20(tokens[i]).safeTransfer(ITournamentFactory(factory).getpWallet(), calculatedTournamentFee);

                    // Transfer fee to Organizer
                    IERC20(tokens[i]).safeTransfer(organizer, calculatedTournamentFee);
        ...
        emit ClaimedEvent(msg.sender, tokens, rewardsAmount);
    }
