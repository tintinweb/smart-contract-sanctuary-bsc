...
contract NextLevel is LevelProcessing {
    ...
    function joinRefZeroTable(uint tableId) external {
        ...
        clientRef.tablesRefByLevel[nextLevel] = tableId;
        clientRef.actualLevel = nextLevel;

        emit FirstJoin(client, tableId);
    }
    ...
    function toNextLevel() external {
        ...
        if (nextLevel == 0) {
            ...
            emit FirstJoin(client, levels[nextLevel].tablesSize - 1);
        } else {
            ...
            emit JoinNextLevel(client, nextLevel - 1, tableId);
        }
