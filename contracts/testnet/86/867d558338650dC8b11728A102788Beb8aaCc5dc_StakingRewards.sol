...
contract StakingRewards is  ReentrancyGuardUpgradeable,OwnableUpgradeable {
    ...
    mapping(address => uint256) private _balances;
     ...
     struct Stake{
      ...
      Stake[] public stakess;
   // mapping(address => Stake[]) internal stakesholders;
   ...
   mapping(address => uint256) public stakes;
    ...
    function stake(uint256 amount) external  nonReentrant updateReward(msg.sender) {
        ...
        require(!isBlackListed[msg.sender],"User Blacklisted");
         stakess.push(Stake(msg.sender, amount, block.timestamp));
    ...
    function withdraw(uint256 amount, uint256 index) public  nonReentrant updateReward(msg.sender) {
        require(amount > 0, "Cannot withdraw 0");
        Stake storage _stake = stakess[index];
        // require(block.timestamp.sub(userLastStackedTime[msg.sender]) > holdingTime);
          require(block.timestamp.sub(_stake.since) > holdingTime);
          amount = _stake.amount;
        _totalSupply = _totalSupply.sub(amount);
        _balances[msg.sender] = _balances[msg.sender].sub(amount);
        stakingToken.safeTransfer(msg.sender, _partialFee(msg.sender,amount));
         if(_stake.amount == 0){
             delete stakess[index];
         }else{
             _stake.amount = _balances[msg.sender];
