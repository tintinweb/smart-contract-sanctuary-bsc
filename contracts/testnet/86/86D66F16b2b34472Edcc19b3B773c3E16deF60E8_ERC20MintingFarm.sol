...
contract ERC20MintingFarm is Ownable, ReentrancyGuard, IERC20MintingFarm{
    ...
    uint256 public excessiveReward = 0;
    ...
    function recoverERC20(
        ...
        require(_tokenAddress != address(rewardToken), "Can't recover reward token");
    ...
    function _updatePool() private {
        ...
        uint256 cakeReward = _mintRewards();
    ...
    /*
     * @notice Mints rewards
     */
    function _mintRewards() private returns(uint256 newRewards) {
        if (rewardPerBlock == 0) return 0;
        uint256 rewardsToMint = _getMultiplier(lastRewardBlock, block.number) * rewardPerBlock;
        newRewards = rewardsToMint;

        uint256 _excessiveReward = excessiveReward;
        if (rewardsToMint <= _excessiveReward) {
            excessiveReward -= rewardsToMint;
        } else {
            if (_excessiveReward > 0) {
                rewardsToMint -= _excessiveReward;
                excessiveReward = 0;
            }
            IRewardMinter(address(rewardToken)).mint(address(this), rewardsToMint);
        }
    }
