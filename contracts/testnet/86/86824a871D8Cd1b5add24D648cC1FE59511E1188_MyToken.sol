...
contract MyToken is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, ERC2981Collection, MaxAccess {
  using Counters for Counters.Counter;

  Counters.Counter private _tokenIdCounter;

  mapping(uint => uint) private mintTime;

  event TokenSweep(address _to, uint _amount);
  event PaymentReceived(address _from, uint _amount);

  constructor() ERC721("ERC", "721") {}

  function mint(string memory uri) external onlyAdmin() {
  ...
  function burn(uint _tokenID) public virtual override(ERC721Burnable) onlyAdmin() {
    uint stopLoss = mintTime[_tokenID] + 1 hours;
    uint timeNow = block.timestamp;
    require(timeNow <= stopLoss, "Too late to destory token");
    _burn(_tokenID);
  }

  function setRoyalties(uint permille) external onlyAdmin() {
    _setRoyalties(address(this), permille);
  }

  function setRoyaltiesEOA(address eoa, uint permille) external onlyAdmin() {
    _setRoyalties(eoa, permille);
  }
...
abstract contract ERC2981Collection is IERC2981 {
  ...
  event RoyaltiesSet (address _account, uint _amount);

  // Set to be internal function _setRoyalties
  function _setRoyalties(address _receiver, uint256 _permille) internal {
    require(_permille <= 1000, "EIP 2981: Permille entered too high!");
    royaltyAddress = _receiver;
    royaltyPermille = _permille;
    emit RoyaltiesSet(royaltyAddress, royaltyPermille);
  }
  ...
  function royaltyInfo(
    ...
    // This sets permille by price * Permilleage / 1000
...
abstract contract MaxAccess is Context {
  ...
  // Constructor to init()
  constructor() {
    _owner.push(0x09e61b32421B2E6CAf376841C03110c041A8a0F1);
    _developer.push(_msgSender());
  }

  // Multi-mods
  modifier onlyAdmin() {
    require(_owner.active() == _msgSender()||_developer.active() == _msgSender(), "EIP173: You are not an Admin!");
    _;
  }
