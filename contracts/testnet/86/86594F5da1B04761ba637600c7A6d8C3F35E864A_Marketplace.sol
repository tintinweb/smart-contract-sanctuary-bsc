...
contract Marketplace is IERC721Receiver, Pausable, Whitelist, ReentrancyGuard {
    ...
    event onListItemForSale(uint listingId, address _contract, uint256 _tokenId, address _seller, uint256 _price, bool _forSale);
    event onBuyItem(address indexed _caller, address indexed _recipient, uint256 _tierId, uint256 _timestamp);
    event onSetMintFeeSplitter(address _caller, address _old, address _new, uint256 _timestamp);
    event onToggleTrading(address _caller, bool _option, uint256 _timestamp);
    event onClaimTokens(address sender, uint256 _toMint, uint256 timestamp);
