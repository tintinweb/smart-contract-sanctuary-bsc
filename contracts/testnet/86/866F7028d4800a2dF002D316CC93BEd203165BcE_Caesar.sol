...
contract Caesar is ERC20Detailed, Ownable {
    ...
    event set_Bond(address addr,bool indexed value);
    ...
    uint256 private constant MAX_UINT256 = type(uint256).max / 100; // TIMELOCK: modified for large value movements
    ...
    uint256 private constant MaxFirePitfee = 5;
    // Buy Fees
    ...
    uint256 private constant MaxliquidityFeeSell = 10; 
    uint256 public treasuryFeeSell = 10;
    uint256 private constant MaxtreasuryFeeSell = 10;
    uint256 public firePitFeeSell = 5;
    ...
    address public wrapped; // TIMELOCK
    bool public swapEnabled = true;
    uint256 public INDEX; // TIMELOCK
    bool public wrapped_set = false; // TIMELOCK
    
    address public bondContract;
    bool public bond_set;
    ...
    constructor(
        ...
        setIndex(uint256(1).mul(10**DECIMALS)); // TIMELOCK defines initial index number
    }
    
    modifier onlyWrapped() { // TIMELOCK
        require(msg.sender == wrapped, "Sender must be the wrapped contract.");
        _;  
    }

    modifier onlyBond() {
        require(msg.sender == bondContract, "Sender must be the Bond contract.");
        _;  
    }

    function setBond(address addr) external onlyOwner {
        require(
            bond_set == false,
            "Bond contract address can only be defined once."
        );

        require(isContract(addr),"Address must be of Contract!!");

        bondContract = addr;
        bond_set = true; // prevents modifying the wrapped address after it was modified once
        
        emit set_Bond(addr,true);
    }

    function setWrapped(address addr) external onlyOwner { // TIMELOCK
        require(
            wrapped_set == false,
            "Wrapped contract address can only be defined once."
        );
    ...
    function mintforBond(address recipient, uint256 amount) external onlyBond { 
        // Only Bond can mint tokens.
        ...
        emit Transfer(address(0x0),recipient,gonsAmount);
    }

    function mintforWrapper(address recipient, uint256 amount) external onlyWrapped { // TIMELOCK
        // Only WCAESAR can mint tokens to provide additional rewards.
        _totalSupply = _totalSupply.add(uint256(amount));
    ...
    function gonsForBalance(uint256 amount) public view returns (uint256) { // TIMELOCK
        return amount.mul(_gonsPerFragment);
    }

    function balanceForGons(uint256 gons) public view returns (uint256) { // TIMELOCK
        return gons.div(_gonsPerFragment);
    }

    function index() external view returns (uint256) { // TIMELOCK
        return balanceForGons(INDEX);
    }
    function _index() internal view returns (uint256) { // TIMELOCK
        return balanceForGons(INDEX);
    }

    function getDecimals() external pure returns (uint256) { // TIMELOCK
        return 10**DECIMALS;
    }

    function setIndex(uint256 _INDEX) internal { // TIMELOCK
