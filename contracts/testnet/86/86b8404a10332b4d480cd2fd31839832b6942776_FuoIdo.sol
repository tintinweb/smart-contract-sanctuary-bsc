...
contract FuoIdo is Ownable {
    ...
    function pledgeJoin(uint256 usdtAmount) public {
        ...
        futPancakePair.sync();
        updatePledge(msg.sender,usdtAmount);
    }

    function pledgeActive(uint256 futAmount) public {
        require(msg.sender != masterAddr,"cannot be masterAddr");
        require(futAmount >0 && pledgeMap[msg.sender].lockAmount >= futAmount,"param error");
        uint256 futPrice = calcFutPrice();
        uint256 usdtAmount =  futAmount * futPrice * 10**18;
        usdtToken.transferFrom(address(msg.sender),masterAddr,usdtAmount * 6/10);
        usdtToken.transferFrom(address(msg.sender),futPair,usdtAmount * 4/10);
        futPancakePair.sync();
    ...
    function updatePledge(address user,uint256 usdtAmount) internal {
        pledgeMap[user].takeTime = block.timestamp;
        pledgeMap[user].myPower = pledgeMap[user].myPower+usdtAmount*2;
        if(idoMap[user].parent == address(0)){
            idoMap[user].parent = masterAddr;
        }
        address level1 = idoMap[user].parent;
        if(level1 != address(0)){
