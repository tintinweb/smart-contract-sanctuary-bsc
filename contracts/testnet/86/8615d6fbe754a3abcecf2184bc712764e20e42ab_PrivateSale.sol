...
contract PrivateSale is Initializable,PausableUpgradeable, OwnableUpgradeable  {
    ...
    function getBuyerWithdrawal(address account  ) public view returns (uint256[]memory) {
        // return buyerWithdrawal[account][period];
        uint256[] memory buyerWithdrawals;
        for(uint256 i =0;i<vesting.length;i++){
            buyerWithdrawals[i] = (buyerWithdrawal[account][i]);
        }
        return buyerWithdrawals;
    }
}