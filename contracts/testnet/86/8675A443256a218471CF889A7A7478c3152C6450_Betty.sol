...
// File: contracts/extension/erc20.sol
...
// File: contracts/extension/vrf.sol

pragma solidity ^0.8.0;


abstract contract vrfContract is VRFConsumerBaseV2(0x6A2AAd07396B36Fe02a22b33cf443582f682c82f){
    // Chainklink VRF V2
    VRFCoordinatorV2Interface immutable COORDINATOR=VRFCoordinatorV2Interface(0x6A2AAd07396B36Fe02a22b33cf443582f682c82f);
...
// File: contracts/mock/betty1155.sol
...
contract Betty is Initializable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155BurnableUpgradeable,ERC1155SupplyUpgradeable, UUPSUpgradeable,vrfContract {
    ...
    function _keyHash() internal override returns (bytes32){
        return bytes32(0xd4bb89654db74673a187bd804519e65e3f71a52bc55f11da7601a13dcf505314);
    }
    ...
    function _processRandomnessFulfillment(uint256 requestId, address requestAddress, uint256 randomness) internal override{
        uint256 RandomNum=seed(requestAddress);
        uint256 count=tokensCountOf(saleWallet);
        uint256 index=RandomNum%count;
    ...
    function buyNFTsByBUSD(uint amount) public{
        ...
        feeTokenBUSD.transferFrom(_msgSender(), owner(), moneyAmount);
    ...
    function buyNFTsByGBY(uint amount) public{
        require(amount>0, "Amount must be more than zero. ");
        require(getLeftSalesAmount()>=amount, "NFTs are insufficient in quantity. ");
        feeTokenGBY.transferFrom(_msgSender(), owner(), buyFeeGBY*amount);
        for(uint i; i < amount; i++){
            _buyNFT();
        }
    }
