...
contract Box is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(address => uint256) public buyAtblockNumber;
    ...
    event BuyAt(address indexed user, uint256 blockNumber);
    ...
    function buy() external {
        require(buyAtblockNumber[msg.sender]==0, "claim first");
        require(IERC20(usdtAddress).transferFrom(msg.sender, address(this), boxPrice), "transfer usdt fail");

        buyAtblockNumber[msg.sender] = block.number;
        emit BuyAt(msg.sender, block.number);
    }

    function get() external {
        uint256 number = buyAtblockNumber[msg.sender];
        require(number > 0 && block.number > number + 1, "invalid operation");
        //transfer VNS
        ...
        //nft
        bytes32 hash = blockhash(number + 1);
        uint256 random = uint256(keccak256(abi.encodePacked(hash, msg.sender))) % 100;

        if (random <= nftOdds && hash != 0) {
            //mint nft
            gotNft = true;
        ...
        delete buyAtblockNumber[msg.sender];
