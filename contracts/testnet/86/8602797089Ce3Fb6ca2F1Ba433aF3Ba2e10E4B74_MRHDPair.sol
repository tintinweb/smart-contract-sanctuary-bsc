...
contract MRHDPair is MRHDPairSetUp{
    ...
    function addLiquidity(address _token, uint256 amount) public {
        ...
        uint256 usdtamount;
        uint256[17] memory n;
        (usdtamount,n) = algorithmBuyNumber(tokenconfiguration[_token],amount,0);
        usdtamount = usdtamount * (tokenconfiguration[_token][5] / 10 ** 18);
    ...
    function extractLiquidity(address _token, uint256 amount) public {
        ...
        uint256 usdtamount;
        uint256[17] memory n;
        (usdtamount,n) = algorithmSellNumber(tokenconfiguration[_token],amount,0);
        usdtamount = usdtamount * (tokenconfiguration[_token][5] / 10 ** 18);
