...
contract DigireAdmin is OwnableUpgradeable{
    ...
    event Auto_Settlement_Transfer(address to, uint amount);
    ...
    function add_trader_request(bytes32 email_hash, bytes32 mobile_hash, uint trader_id, uint trader_debt_to_agent, uint trader_credit_rating, uint account_type) external { 
        ...
        require(trader.agent_address == msg.sender, "Not agent's trader");
    ...
    function loan_to_agent(address broker_address, address agent_address, uint debt) external {
        require(msg.sender == address(DIGIRE), "Only DIGIRE contract");
        Agent storage agent = agents[agent_address];
    ...
    function auto_settlement_transfer(address to, uint amount) external onlyGatekeeper {
        ...
        emit Auto_Settlement_Transfer(to, amount);
    }
    ...
    function repay_broker_debt(address agent_address, address broker_address, uint amount) external {
        require(msg.sender == address(DIGIRE), "Only DIGIRE contract");
        Agent storage agent = agents[agent_address];
        require(agent.status == AgentStatus.Active, "Only agent");
        require(agent.broker_address == broker_address, "Invalid broker address");
