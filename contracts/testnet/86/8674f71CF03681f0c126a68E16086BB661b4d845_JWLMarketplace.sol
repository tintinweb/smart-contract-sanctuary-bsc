...
contract JWLMarketplace is Ownable, ReentrancyGuard {
    ...
    function _updateHighestBid(
        ...
        if (_isERC20Auction(auctionERC20Token)) {
            ...
            if(auctionERC20Token == address(0)){
                    (bool success,) = payable(address(this)).call{value: _tokenAmount}("");
                    if(!success){
                        require(success , "address(this) fee not transfer");
                    }
            }else{
    ...
    function _payout(
        ...
        if(auctionERC20Token == address(0)){
            ...
            if(!success){
                require(success , "seller fee not transfer");
            }
        }else{
    ...
    function _purchaseAndTransfer(
        ...
        if (royaltyStorage.isActivated(_nftContractAddress, _tokenId)) {
            ...
            if(erc20Token == address(0)){
                (bool success,) = payable(royaltyOwner).call{value: royaltyAmount}("");
                if(!success){
                    require(success , "seller fee not transfer");
                }
            }else{
        ...
        if (adminFees > 0) {
            if(erc20Token == address(0)){
                (bool success,) = payable(owner).call{value: adminFees}("");
                if(!success){
                    require(success , "owner fee not transfer");
                }
            }else{
        ...
        if(erc20Token == address(0)){
            ...
            if(!success){
                require(success , "_nftSeller fee not transfer");
            }
        }else{
    ...
    function _buyNFT(address _nftContractAddress, uint256 _tokenId) internal {
        ...
        if (royaltyStorage.isActivated(_nftContractAddress, _tokenId)) {
            ...
            if(erc20Token == address(0)){
                (bool success,) = payable(royaltyOwner).call{value: royaltyAmount}("");
                if(!success){
                    require(success , "royaltyOwner fee not transfer");
                }
            }else{
                IERC20(erc20Token).transferFrom(
                msg.sender,
        ...
        internal
    {
        ...
        if(erc20Token == address(0)){
            ...
            (bool success1,) = payable(seller).call{value: totalAmount}("");
            if(!success1){
                require(success1 , "seller fee not transfer");
            }
        }else{
            IERC20(erc20Token).transferFrom(msg.sender, owner, adminFees);
            IERC20(erc20Token).transferFrom(msg.sender, seller, totalAmount);
