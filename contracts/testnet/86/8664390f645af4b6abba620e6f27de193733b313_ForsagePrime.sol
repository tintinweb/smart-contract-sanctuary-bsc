...
contract ForsagePrime is Ownable {
  ...
  function joinProgram(uint depAmount, uint _referrerID) public {
        require(!isContract(msg.sender) && msg.sender == tx.origin,"1");
        uint _userId = autoPoolId[msg.sender];
        require(users[msg.sender].isExist == false && autoPoolUniqueUsers[_userId].isExist ==  false,"2"); //User Already Exist
        usdt.transferFrom(msg.sender, address(this), depAmount);
        require(_referrerID > 0 && _referrerID <= userCurrentId,"3"); // invalid REF ID
        ...
        require(size == 0,"4"); //Invalid Contract
