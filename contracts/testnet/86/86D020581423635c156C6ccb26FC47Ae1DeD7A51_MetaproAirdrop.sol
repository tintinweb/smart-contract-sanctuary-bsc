...
contract MetaproAirdrop is Ownable, ERC1155Holder, ReentrancyGuard  {
    ...
    function createAirdrop(uint256 _tokenId, uint256 startBlock, uint256 endBlock, uint256 amount, bytes memory _data) external onlyOwner nonReentrant {
        ...
        address creator = token.creators(_tokenId);

        require(creator == msg.sender, "MetaproAirdrop: You must be creator of the token");
