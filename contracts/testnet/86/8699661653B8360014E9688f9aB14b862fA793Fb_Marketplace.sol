...
contract Marketplace is Ownable, ReentrancyGuard, ERC721Holder, ERC1155Holder {
    ...
    constructor(address anitoAddress, address stonesAddress, address _laroToken, address _busdToken) {
    ...
    function setAllowAsset(address _assetAddress, uint256 _assetType, bool allow)
        public
        onlyOwner
    {
    ...
    function createMarketItem(
        ...
        require(paymentMethod == LARO_TOKEN || paymentMethod == BUSD_TOKEN, "invalid payment method");
        ...
        if(assetType == ASSET_ERC1155){
            IERC1155(assetContract).safeTransferFrom(msg.sender, address(this), tokenId, quantity, "");
        }
    ...
    function delistMarketItem(address assetContract, uint256 itemId)
        public
        nonReentrant
    {
        require(!maintenanceMode, "Maintenance mode");
        ...
        if(assetType == ASSET_ERC1155){
            IERC1155(assetContract).safeTransferFrom(address(this), msg.sender, idToMarketItem[itemId].tokenId, idToMarketItem[itemId].quantity, "");
        }
    ...
    function createMarketSale(address assetContract, uint256 itemId)
        public
        nonReentrant
    {
        require(!maintenanceMode, "Maintenance mode");
        ...
        if(paymentMethod == LARO_TOKEN){
            laroToken.safeTransferFrom(msg.sender, idToMarketItem[itemId].seller, price);
        }

        if(paymentMethod == BUSD_TOKEN){
            busdToken.safeTransferFrom(msg.sender, idToMarketItem[itemId].seller, price);
        }

        if(assetType == ASSET_ERC721){
            IERC721(assetContract).safeTransferFrom(address(this), msg.sender, tokenId);
        }

        if(assetType == ASSET_ERC1155){
            IERC1155(assetContract).safeTransferFrom(address(this), msg.sender, tokenId, idToMarketItem[itemId].quantity, "");
        }
    ...
    function fetchMarketItem(uint256 _itemId)
        public
        view
    ...
    function fetchMarketItems(uint256[] memory itemIds)
        public
        view
        returns (MarketItem[] memory)
    {
    ...
    function fetchMyPurchasedItems(address account, uint256 cursor, uint256 size)
        public
        view
        returns (uint256[] memory, MarketItem[] memory, uint256)
    {
        uint256 length = size;
    ...
    function fetchMyItemsListed(address account, uint256 cursor, uint256 size)
        public
        view
        returns (uint256[] memory, MarketItem[] memory, uint256)
    {
    ...
    function fetchMyPurchasedItemIds(address account)
        public
        view
    ...
    function fetchMyListedItemIds(address account)
        public
        view
        returns (uint256[] memory)
    {
