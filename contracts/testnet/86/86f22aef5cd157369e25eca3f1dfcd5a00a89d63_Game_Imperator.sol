...
contract Game_Imperator is BEP20Token {
    ...
    event MyLog(address emperor, uint256 sum, uint256 number_imperia, uint256 data);
    event EmperorLog(address my_adress, uint256 sum, uint256 number_imperia, uint256 data);
    event Referalsum(address ref_adr, uint256 sum, uint256 number_imperia, uint256 data);
    ...
    ///плата рефералу///   
         if (my_inviter[msg.sender] != address(0)){
           ...
           emit Referalsum(my_inviter[msg.sender],all_game[a].referal_gift,a+1,block.timestamp);
       }
    ///////////////////  
        ...
        if (all_game[a].adr.length > 0)
        {
            ...
            emit EmperorLog(all_game[a].adr[all_game[a].count], all_game[a].price - all_game[a].referal_gift,a+1,block.timestamp);
        ...
        all_game[a].number[msg.sender] = all_game[a].adr.length - all_game[a].count; 
    }
    ...
    function Second_Game() public{
       ...
       require(all_game[a].in_game[msg.sender] == false, "You are in the game");
       require(second_game[msg.sender] >= 2 , "Big circle not active");

    ///плата за вход///   
       require(tokenUSDT.allowance(msg.sender,address(this)) >= all_game[a].price * 10 ** 18 , "Payment not confirmed");
       require(tokenUSDT.balanceOf(msg.sender)  >= all_game[a].price * 10 ** 18, "Insufficient funds to pay" );

       if (second_game[msg.sender] == 2){
           first_game[msg.sender] = true;
       }
        ...
        if (all_game[a].adr.length > 0)
        {
            ...
            if ( all_game[a].progress[all_game[a].adr[all_game[a].count]] == 9){
             ...
             all_game[a].progress[all_game[a].adr[all_game[a].count]] = 0;
              ...
              if (number_table[all_game[a].adr[all_game[a].count]] < 4)
              number_table[all_game[a].adr[all_game[a].count]]++;
              else{
                   number_table[all_game[a].adr[all_game[a].count]] = 2;
                    first_game[msg.sender] = false;
                    second_game[msg.sender]++;
              }
    ...
    function Check_Number(address adr,uint256 number) public In_game(adr,number-1) view returns(uint256) {
     ...
     function Check_status(address adr,uint256 number) In_game(adr,number-1) public view returns(string memory) {
         if (all_game[number-1].number[adr] - all_game[number-1].count <=  8 * all_game[number-1].count && all_game[number-1].count > 0 )
        return "Emperor";
        else
        return "Legion";
    }

    function Check_Table(address adr,uint256 number) In_game(adr,number) public view returns(uint256){
    ...
    function TEST(uint256 number) public view returns(uint256){
        return  all_game[number].adr.length;
    }
   
   function TEST2(uint256 number) public view returns(address){
        return  all_game[number].adr[all_game[number].count];
    }
}