...
contract LockoffToken is ERC20Detailed, Ownable, ReentrancyGuard {
    ...
    modifier validLocker(address _sender, uint256 _amount) {
        require(_sender != address(0x0), "Recipient zero address");
        uint256 presaleLocked = ILockOff(presale).totalLocked(_sender);
        uint256 airdropLocked = ILockOff(airdrop).totalLocked(_sender);
        uint256 avaiable = balanceOf(_sender).sub(presaleLocked).sub(airdropLocked);
        require(avaiable >= _amount || _isFeeExempt[_sender], "Insufficient balance");
        _;
    }
    ...
    constructor() ERC20Detailed("Token Test", "TEST", uint8(DECIMALS))
    {
    ...
    function _basicTransfer(
        ...
        address to,
        uint256 amount
    ) internal validLocker(from, amount) returns (bool) {
    ...
    function _transferFrom(
        ...
        address recipient,
        uint256 amount
    ) internal validLocker(sender, amount) returns (bool) {
