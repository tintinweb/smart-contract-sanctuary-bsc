...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => address[]) private buyersWaitingToTokens; //TODO
    ...
    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, uint256 _amount, uint256 _price) private {
        ...
        if (_project.minSales == 0 && _project.isInstantPayment) {
            ...
            project.setTotalBuyersWaitingToToken(_project.id, project.getTotalBuyersWaitingToToken(_project.id) + 1);//TODO
            buyersWaitingToTokens[_sale.id].push(_msgSender()); //TODO
    ...
    //TODO
    function close(uint256 _closeLimit, ProjectInfo memory _project, SaleInfo memory _sale, uint256 _closeCount, bool _isGive) external onlyProject nonReentrant returns (uint256) {
        ...
        for (uint256 i; i < buyersWaiting.length; i++) {
            ...
            if (getBuyersWaitingToTokens(_sale.id).length == 0) sales[_sale.id].isClose = true;
            if (_closeCount == _closeLimit) break;
        }
...
interface IProject {
    ...
    function setTotalBuyersWaitingToToken(uint256 _projectId, uint256 _total) external; //TODO
    function getTotalBuyersWaitingToToken(uint256 _projectId) external view returns (uint256); //TODO
...
interface ISale {
    ...
    function close(uint256 closeLimit, ProjectInfo memory _project, SaleInfo memory _sale, uint256 closeCount, bool _isGive) external returns (uint256);
