...
contract MetaSpacecyTicketCard is IMetaSpacecyTicketCard, ERC1155, ContextMixin, NativeMetaTransaction, Ownable {
  ...
  modifier creatorOnly(uint256 _id) {
    require(creator[_id] == _msgSender(), "MTC: only creator");
    _;
  }

  modifier ownersOnly(uint256 _id) {
    require(balanceOf(_msgSender(), _id) > 0, "MTC: only owners");
    _;
  }

  modifier onlyAdminOrTokenMinter() {
    require(metaspacecyAccessControls.hasAdminRole(_msgSender()) ||
      metaspacecyAccessControls.hasTokenMinterRole(_msgSender()),
      "MTC: only admin or token minter role"
    );
    _;
  }
  ...
  constructor (
    ...
    address metaspacecyAccessControls_
  ) ERC1155("") {
    _name = name_;
    _symbol = symbol_;
    baseTokenURI = baseTokenURI_;
    metaspacecyAccessControls = MetaSpacecyAccessControls(metaspacecyAccessControls_);
  ...
  function create(
    ...
    require(expiryId[_currentId] < block.timestamp, "MTC: current ID still valid");
    require(expiry > block.timestamp, "MTC: expiry invalid");
  ...
  function mint(
    ...
    require(!_isIdExpired(_currentId), "MTC: id expired");
  ...
  function burn(address account, uint256 id, uint256 amount) public override {
    require(_isIdExpired(id), "MTC: id not expired");
    require(
      account == _msgSender() || isApprovedForAll(account, _msgSender()),
      "ERC1155: transfer caller is not owner nor approved"
    );
    require(balanceOf(account, id) >= amount, "MTC: insufficient");
  ...
  function safeTransferFrom(
    ...
    require(!_isIdExpired(id), "MTC: id expired");
    require(
  ...
  function safeBatchTransferFrom(
    ...
    for (uint256 i = 0; i < ids.length; ++i) {
      require(!_isIdExpired(ids[i]), "MTC: id expired");
      require(
        from == _msgSender() || isApprovedForAll(from, _msgSender()),
        "ERC1155: transfer caller is not owner nor approved"
      );
  ...
  function setCreator(address _to, uint256[] memory _ids) public {
    require(_to != address(0), "MTC: zero address");
