...
contract USDReward {
    ...
    address public depositAddress; // USD抵押地址
    uint256 public rewardRate1 = 100; // 奖励比例1 100/1000
    uint256 public rewardRate2 = 50; // 奖励比例2 50/1000

    constructor(address _usdAddress, address _depositAddress) {
    ...
    function inviteReward(address account, uint256 amount) public onlyUseFor {
        address _invite = IUSD(usdAddress).inviter(account);
        if (_invite != address(0) && rewardRate1 > 0) {
            uint256 _stakedOf = IDepositUSD(depositAddress).stakedOf(account);
            if (_stakedOf > 0) {
                _inviteReward(_invite, _stakedOf, amount, rewardRate1);

                address _invite2 = IUSD(usdAddress).inviter(_invite);
                if (_invite2 != address(0) && rewardRate2 > 0) {
                    _inviteReward(_invite2, _stakedOf, amount, rewardRate2);
                }
            }
        }
    }
    ...
    function _inviteReward(
        ...
        if (_allReward > 0) {
            uint256 _inviteStakedOf = IDepositUSD(depositAddress).stakedOf(
                _invite
            );
