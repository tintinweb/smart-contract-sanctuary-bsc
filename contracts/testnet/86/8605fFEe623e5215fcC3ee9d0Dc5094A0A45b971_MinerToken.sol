...
contract MinerToken is Ownable {
    ...
    mapping (uint256 => Item) private _products;
    mapping (address => uint256) private _beforeBalance; // 之前未领取的
    ...
    constructor(address _dct) {
        _owner = msg.sender;
        dct = _dct;
        totalMinerAmount = 30000000e18;
        minRewardPower = 1;

        _products[1] = Item(300e18, 1);
        _products[2] = Item(3000e18, 12);
    }

    // 获取总算力和总质押USDT、总销毁DCT
    function getTotalPowerAndUSDT() external view returns(uint256, uint256){
        return (totalComputPower, totalBurnDCTAmount);
    }
    ...
    // 质押
    function deposit(uint256 option, uint256 amount) external {
        address sender = msg.sender;
        require(_products[option].dctAmount > 0 && amount >= 1, "Product serial number does not exist");
        uint256 approveAmountDCT = IERC20(dct).allowance(sender, address(this));

        uint256 dctAmount = _products[option].dctAmount.mul(amount);

        require(approveAmountDCT >= dctAmount, "Insufficient authorized amount");
        IERC20(dct).transferFrom(sender, address(this), dctAmount);

        uint256 beforeAmount = getRewardAmount(sender);
        if (beforeAmount > 0){
            _beforeBalance[sender] = beforeAmount;
        }

        totalBurnDCTAmount += dctAmount;
        totalComputPower += _products[option].power;
        _user[sender] = block.timestamp;
        _balance[sender] = _balance[sender].add(_products[option].power);
        emit Deposit(sender, _products[option].power);
    }
    ...
    // 新增产品
    function setAddProduct(uint256 option, uint256 dctAmount, uint256 power) external onlyOwner {
        require(_products[option].dctAmount == 0, "Product serial number does exist");
        _products[option] = Item(dctAmount*10**18, power);
    }
    ...
    function getRewardAmount(address account) public view  returns(uint256){
        ...
        rewardAmount = rewardAmount + _beforeBalance[account];
        if (dctBalance < rewardAmount) {
            rewardAmount = dctBalance;
        }
        return rewardAmount;
