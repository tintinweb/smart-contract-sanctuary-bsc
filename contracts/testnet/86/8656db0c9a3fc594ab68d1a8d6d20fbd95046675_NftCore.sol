...
contract NftFactory is Ownable {
  ...
  function createUrlNftAdmin(string memory _name,string memory url,uint32 _level) public {
    require(msg.sender == admin,"you are not admin");
    _createNft(_name, url, 0, 1, _level);
  }

  function createDnaNftAdmin(string memory _name,uint32 _level) public{
    require(msg.sender == admin,"you are not admin");
...
contract NftMarket is NftOwnership {
    ...
    function buyShopNft(uint _nftId)public {
        ...
        require(usdt.balanceOf(msg.sender)>=nftShop[_nftId].price,"Token balance too low");
        usdt.transferFrom(msg.sender,nftShop[_nftId].seller,nftShop[_nftId].price);
