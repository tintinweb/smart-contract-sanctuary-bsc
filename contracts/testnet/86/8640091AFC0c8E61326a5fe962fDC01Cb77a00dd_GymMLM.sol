...
contract GymMLM is OwnableUpgradeable {
    ...
    EnumerableSetUpgradeable.AddressSet private referrals;
    ...
    function agreeTermsOfConditions(address[] calldata _directPartners) external {
        ...
        // remove in future
        // userMLMDepth[msg.sender] = _userDepth(msg.sender);
    }
    ...
    function distributeRewards(
        ...
        while (index < directReferralBonuses.length && addressToId[userToReferrer[_user]] != 1) {
            ...
            if (index <= _level && hasInvestment[referrer]) {
     ...
     */
    function getPendingRewards(address _userAddress, uint32 _type) external view returns (uint256) {
        require(
            accountantAddress != address(0),
     ...
     */
    function getReferrals(address _userAddress) external view returns (address[] memory) {
        // uint256 _depth = userMLMDepth[_userAddress];
        address[] memory referrals = new address[](25);
    ...
    function _addUser(address _user, address _referrer) private {
        ...
        userMLMDepth[_user] = userMLMDepth[_referrer] + 1;
        ...
        returns (uint256)
    {
        ...
        if (_type == 1) {
            ...
            uint256 _poolLength = IGymVault(bankAddress).getPoolLength() - 1;
            for (uint32 i = 0; i < _poolLength; i++) {
                convertBalance += IGymVault(bankAddress).pendingReward(i, _userAddress);
            }
        } else if (_type == 2) {
            require(
            ...
            require(
                singlePoolAddress != address(0),
                "GymMLM:: Single Pool contract address is zero address"
            );
            uint256 _depositId = IGymSinglePool(singlePoolAddress)
                .getUserInfo(_userAddress)
                .depositId;
            for (uint32 i = 0; i < _depositId; i++) {
                convertBalance += IGymSinglePool(singlePoolAddress).pendingReward(i, _userAddress);
            }
        }
    ...
    function _userDepth(address _user) private view returns (uint256) {
        uint256 depth = 1;
        while (addressToId[userToReferrer[_user]] != 1) {
            _user = userToReferrer[_user];
            depth++;
        }
        return depth;
