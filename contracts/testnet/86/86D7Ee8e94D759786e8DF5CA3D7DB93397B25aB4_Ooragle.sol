...
contract Ooragle is Ownable {
    ...
    function getAllLinksInfo() public view returns (Link[] memory) {
        Link[] memory links = new Link[](allLinks.length);
        for (uint256 i = 0; i < allLinks.length - 1; i++) {
            links[i] = linkInfo[allLinks[i]];
        }
