...
function findTotalRewardFor(address _user, uint _totalFees) external view returns(uint256 _referralFeeAmount){
    ...
    address parentAddress = parents[user];
    if(parents[user] == address(0)){
        totalReward = 0;
    }
    // calculate total reward from totalFees based on tierToPercentage
    uint256 totalReward = (totalFees * tierToPercentage[referrerTier[parentAddress]]) / SCALE;
