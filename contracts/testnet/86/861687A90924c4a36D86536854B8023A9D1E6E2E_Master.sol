...
contract Master is Ownable, ReentrancyGuard {
    ...
    uint256 public depositRate = 1;

    constructor(
        IERC20 _werx,
        address _masterWallet
    ) public {
        require(address(_werx) != address(0), "WERX address should not be zero address");
    ...
    function Receive(uint256 _amount) public nonReentrant {
        require(_amount > 0, "WERX Amount is less than zero");
        uint256 receiveWERXAmount = _amount.mul(depositRate);
        WERX.transferFrom(masterWallet, msg.sender, receiveWERXAmount);
    }
