...
contract SafeswapFactory is ISafeswapFactory, Initializable {
    ...
    address public feeTo; // company wallet
...
contract SafeswapPair is ISafeswapPair, SafeswapERC20 {
    ...
    function _takeFee(
        ...
        uint256 amountOutWithoutFee = amountOut.mul(10000) / 9975;
        uint256 burnFee = amountOutWithoutFee.mul(3) / 10000;
        uint256 supportFee = amountOutWithoutFee.mul(5) / 10000;

        uint256 numerator = reserveIn.mul(burnFee);
        uint256 denominator = reserveOut.sub(burnFee);
        uint256 amountIn = numerator / denominator;

        uint256 liquidity = Math.min(amountIn.mul(_totalSupply) / reserveIn, burnFee.mul(_totalSupply) / reserveOut);
