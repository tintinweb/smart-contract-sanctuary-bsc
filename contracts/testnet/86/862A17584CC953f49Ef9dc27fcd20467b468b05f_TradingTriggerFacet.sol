...
contract TradingTriggerFacet is ITradingTrigger {

    using SafeERC20 for IERC20;

    function limitOrderDeal(LimitOrder memory order) external override {
    ...
    function marketTradeCallback(bytes32 tradeHash, uint upperPrice, uint lowerPrice) external override {
        LibAccessControlEnumerable.checkRole(Constants.SELF_ROLE);
        LibTrading.TradingStorage storage ts = LibTrading.tradingStorage();
        ITradingReader.PendingTrade memory pt = ts.pendingTrades[tradeHash];
    ...
    function _marketTradeDeal(
        LibTrading.TradingStorage storage ts, ITradingReader.PendingTrade memory pt,
    ...
    function closeTradeCallback(bytes32 tradeHash, uint upperPrice, uint lowerPrice) external override {
        LibAccessControlEnumerable.checkRole(Constants.SELF_ROLE);
        LibTrading.TradingStorage storage ts = LibTrading.tradingStorage();
        ...
        LibTrading.settleForCloseTrade(ts, ot, tradeHash, marketPrice, longAccFundingFeePerShare);
    ...
    function executeTpSlOrLiq(TpSlOrLiq[] memory arr) external override {
        ...
        require(arr.length > 0, "TradingFacet: Parameters are empty");
        for (UC i = ZERO; i < uc(arr.length); i = i + ONE) {
            TpSlOrLiq memory t = arr[i.into()];
            // todo: 更新  LibPriceFacade 里的价格
            require(t.price > 0, "TradingFacet: price must be greater than 0");
    ...
    function _executeTpOrSl(bytes32 tradeHash, uint64 price) private {
        LibTrading.TradingStorage storage ts = LibTrading.tradingStorage();
        ITrading.OpenTrade storage trade = ts.openTrades[tradeHash];
        require(trade.margin > 0, "LibTrading: Trade information does not exist");
        // todo: 相关逻辑处理
        trade.entryPrice = price;

        // todo: 发相关事件
        _removeOpenTrade(ts, trade, tradeHash);
    }


    function _executeLiquidate(bytes32 tradeHash, uint64 price) private {
        LibTrading.TradingStorage storage ts = LibTrading.tradingStorage();
        ITrading.OpenTrade storage trade = ts.openTrades[tradeHash];
        require(trade.margin > 0, "LibTrading: Trade information does not exist");
        // todo: 相关逻辑处理
