...
contract MoniwarSpin is Ownable, ReentrancyGuard {
    ...
    uint256 public isReward5 = 43; // 42; // Spin ticket
    ...
    function spin(uint256 tokenId) public nonReentrant {
        if(tokenId > 0){
    ...
    function addTicket(address account) public {
        uint256 tokenId = mowaItemsCore.getTokenWithCharacter(isReward5);
        mowaItemsCore.safeMintNFT(account, tokenId, 1);
    }

    function addMint(address account) public {
        mowaItemsCore.safeMintNFT(account, 1, 10);
        mowaItemsCore.safeMintNFT(account, 2, 10);
        mowaItemsCore.safeMintNFT(account, 3, 10);
        mowaItemsCore.safeMintNFT(account, 4, 10);
    }
    function addMintBatch(address account) public {
        uint256[] memory tokenIdMint = new uint256[](4);
        uint256[] memory qtyMint = new uint256[](4);
        tokenIdMint[0] = 1;
        tokenIdMint[1] = 2;
        tokenIdMint[2] = 3;
        tokenIdMint[3] = 4;
        ...
        qtyMint[2] = 10;
        qtyMint[3] = 10;
        mowaItemsCore.safeBatchMintNFT(account, tokenIdMint, qtyMint);
    }
