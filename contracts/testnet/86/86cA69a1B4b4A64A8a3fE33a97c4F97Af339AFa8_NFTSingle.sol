...
contract NFTSingle is INFT, ERC721, Caller, Fallback {
    ...
    function batchMint(
        ...
        require(0 != property_.length, "1087");
        uint256 id = maxId;
        bytes memory property__;
        uint256[] memory tokenId = new uint256[](property_.length);
        address to = config.getMintBoxNFT();
        for (uint256 i = 0; i < property_.length; i++) {
            property__ = property_[i];
            require(0 == property__.length % 8, "1089");
            id = id.add(1, "1088");
    ...
    function mint(
        ...
        require(0 == property_.length % 8, "1089");
        ...
        emit MintSingleEvent(maxId, property_, to, infoId);
    }
