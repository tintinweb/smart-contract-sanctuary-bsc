...
contract V3Club is OwnableUpgradeSafe, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    function setv3NFT(address _v3sNFT) external onlyOwner {
        require(_v3sNFT != address(0), "zero");
        v3sNFT = _v3sNFT;
    }
    ...
    function depositNFT(uint256[] memory _tokenIds) external nonReentrant {
        for (uint256 i = 0 ; i < _tokenIds.length; i++){
            ...
            require(IERC721(v3sNFT).ownerOf(_tokenId) == msg.sender, "user does not have specified NFT");
            IERC721(v3sNFT).transferFrom(msg.sender, address(this), _tokenId);
    ...
    function withdrawNFT(uint256[] memory _tokenIds) external nonReentrant {
        for (uint256 i = 0 ; i < _tokenIds.length; i++){
            ...
            IERC721(v3sNFT).transferFrom(address(this), msg.sender, _tokenId);
    ...
    function getLimitedAmount(address _user) public view returns (uint256 _amountLimited, uint256 _amountAvailable){
        ...
        _amountLimited = _len.mul(limitedAmount);
