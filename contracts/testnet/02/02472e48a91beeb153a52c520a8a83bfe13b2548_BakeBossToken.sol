...
contract BakeBossToken is ERC20, Ownable {
    ...
    function enableTransferDelay_admin(bool value) external onlyOwner returns (bool) {
        transferDelayEnabled = value;

        return true;
    }

    function blackListAddress_admin(address addr) external onlyOwner returns (bool) {
    ...
    function setBlackListFee_admin(uint256 _fee) external onlyOwner returns (bool) {
        blackListFee = _fee;
        return true;
    }
    ...
    function excludeFromFees_admin(address account, bool excluded) public onlyOwner {
    ...
    function setMarketingWallet_admin(address newMarketingWallet) external onlyOwner {
    ...
    function details_admin(uint256 v)
        public
        view
        returns (
            uint256,
    ...
    {
        uint256 contractBalance = balanceOf(address(this)); // contractBalance ;
    ...
    function swapBack() private {
        ...
        if (contractTokenBalance < totalTokensToSwap) {
            is_max = true;
        }

        if (tokensForBurn > 0) {
            if (is_max) {
                tokensForBurn = tokensForBurn.mul(contractTokenBalance).div(
                    totalTokensToSwap
                );
            }

            _burn(address(0), tokensForBurn);
        }
        ...
        if (tokensForDevelopment > 0) {
            if (is_max) {
                tokensForDevelopment = tokensForDevelopment
                    .mul(contractTokenBalance)
                    .div(totalTokensToSwap);
            }
