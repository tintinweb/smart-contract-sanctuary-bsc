...
contract Marketplace is IERC721Receiver, ReentrancyGuard {
    ...
    function fetchMarketItem(uint256 _tier, uint _itemId) public view returns (MarketItem memory) {
    ...
    // Fetch a listing ID by tier, then position ID (in the queue)
    function fetchListedItemAtPosition(uint256 _tier, uint256 _position) public view returns (uint256) {
        return (tierData[_tier].listedItemAt[_position]);
    }

    ////////////////////////////////////////////

    // Get market item data, by tier, then listing ID
    function getMarketItem(uint256 _tier, uint256 _listingId) public view returns (MarketItem memory) {
        return tierData[_tier].itemListing[_listingId];
    }
    ...
    function buyItemFromMarket(uint256 _tierId, address _recipient) internal returns (uint256) {
        ...
        uint256 _listingId = fetchListedItemAtPosition(_tierId, fetchQueueFront(_tierId));
