...
contract NFTRefelection is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    /**
     * @notice Info of Pool
     * @param lastChangeId: Last block number that reward distribution occurs
     * @param accUTacoPerShare: Accumulated rewardPool per share, times 1e18
     */
    struct PoolInfo {
        uint256 lastChangeId;
    ...
    modifier onlyWhitelistedContract() {
        require(
            ...
            "GymNFTReflection: not whitelisted or owner"
        );
        _;
    }
    ...
    function initialize(address _gym, address _singlePoolAddress,address _busdAddress) external initializer {
        tokenAddress = _gym; // address of GYMNET Contract
        singlePoolAddress = _singlePoolAddress; // address of GYMNET Contract
        isPoolActive = true;
        busdAddress = _busdAddress;
        poolInfo = PoolInfo({lastChangeId: 0, accRewardPerShare: 0});
        __Ownable_init();
    ...
    */
    function updateUser(uint256 _ggymnetAmt, address from) external onlyWhitelistedContract{
        _claim(from);
        UserInfo storage user = userInfo[from];
        PoolInfo memory pool = poolInfo;
        user.totalGgymnetAmt = _ggymnetAmt;
        user.rewardDebt = (user.totalGgymnetAmt * pool.accRewardPerShare) / (1e18);
    }

    function updatePool(uint256 receivedAmount) external  onlyWhitelistedContract{
        _updatePool(receivedAmount);
    }
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        return (user.totalGgymnetAmt * _accRewardPerShare) / (1e18) - (user.rewardDebt);
    }

    function _updatePool(uint256 _amount) private {
        PoolInfo storage pool = poolInfo;
        uint256 lastChangeId = pool.lastChangeId++;

        uint256 sharesTotal = IGymSinglePool(singlePoolAddress).totalGGymnetInPoolLocked();
        if (sharesTotal == 0) {
            pool.lastChangeId = lastChangeId;
            return;
        }
    ...
    function _claim(address _user) private {
        ...
        require(busdAddress != address(0),"BUSD Address is not set");
        PoolInfo memory pool = poolInfo;
        UserInfo storage user = userInfo[_user];
        IERC20Upgradeable rewardToken = IERC20Upgradeable(busdAddress);

        uint256 pending = (user.totalGgymnetAmt * pool.accRewardPerShare) /
            (1e18) -
            (user.rewardDebt);

        if (pending > 0) {
        require(rewardToken.balanceOf(address(this)) > pending,"Not Enought balance on Contract");
