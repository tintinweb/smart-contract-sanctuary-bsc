...
contract Vault is Ownable {
    ...
    function _ownerApprove(address _pool) internal {
        IERC20(tokenAddress).safeApprove(_pool, type(uint256).max);
    }

    function approveInterestVault(address _core) external onlyOwner {
        IERC20(tokenAddress).safeApprove(_core, type(uint256).max);
    }
}
