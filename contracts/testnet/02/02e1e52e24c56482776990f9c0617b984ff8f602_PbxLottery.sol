...
contract PbxLottery is ReentrancyGuard, IPbxLottery, Ownable {
    ...
    uint256 public maxPriceTicketInPbx = 50 ether;
    uint256 public minPriceTicketInPbx = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInPbx;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] pbxPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInPbx;
        uint32 finalNumber;
    }
     ...
     * @param _pbxTokenAddress: address of the PBX token
        ...
        nonReentrant
    {
        ...
        // Calculate number of PBX to this contract
        uint256 amountPbxToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInPbx,
            _ticketNumbers.length
        );

        // Transfer pbx tokens to this contract
        pbxToken.safeTransferFrom(address(msg.sender), address(this), amountPbxToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInPbx += amountPbxToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInPbxToTransfer
        uint256 rewardInPbxToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInPbxToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        pbxToken.safeTransfer(msg.sender, rewardInPbxToTransfer);

        emit TicketsClaim(msg.sender, rewardInPbxToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInPbx) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in PBX for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].pbxPerBracket[j] =
                ...
                // A. No PBX to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].pbxPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInPbx - amountToShareToWinners);

        // Transfer PBX to treasury address
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        _lotteries[_lotteryId].amountCollectedInPbx += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInPbx >= minPriceTicketInPbx) && (_priceTicketInPbx <= maxPriceTicketInPbx),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInPbx: _priceTicketInPbx,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            pbxPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInPbx: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(pbxToken), "Cannot be PBX token");
    ...
    /**
     * @notice Set PBX price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInPbx: minimum price of a ticket in PBX
     * @param _maxPriceTicketInPbx: maximum price of a ticket in PBX
     */
    function setMinAndMaxTicketPriceInPbx(uint256 _minPriceTicketInPbx, uint256 _maxPriceTicketInPbx)
        external
        onlyOwner
    {
        require(_minPriceTicketInPbx <= _maxPriceTicketInPbx, "minPrice must be < maxPrice");

        minPriceTicketInPbx = _minPriceTicketInPbx;
        maxPriceTicketInPbx = _maxPriceTicketInPbx;
    }
     ...
     * @param _priceTicket price of a ticket (in PBX)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].pbxPerBracket[_bracket];
        } else {
            return 0;
        }
    }
