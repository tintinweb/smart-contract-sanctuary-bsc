...
contract NormalTicket is ERC721, INormalTicket, Caller, Fallback {
    ...
    event MintInfoEvent(
        ...
        uint256 managerFee,
        uint256 superPrizeFee,
        uint256 prizePoolFee
    ); //0x4fc36969dc8a4dead30d53a2b8c503ee199a1ffb5c172600bc092037ede25e49

    IConfig public config; //配置合约
    string public uri;
    mapping(uint256 => MintInfo) public mintInfos;
    mapping(uint256 => bool) private use; //是否使用
    uint256 public totalManageFee; //总平台管理费
    uint256 public totalSuperPrizeFee; //总普通场注入超级大奖池费用
    uint256 public totalPrizePoolFee; //总奖池费用
    ...
    function mint(address token) external payable {
        uint256 fee = config.getNormalTicketFee(token);
        if (address(0) == token) {
        ...
        _mintFee(token, msg.sender, fee);
    }

    function mintBatch(address token) external payable {
        uint256 fee = config.getNormalTicketFee(token);
        if (address(0) == token) {
            require(msg.value >= fee, "insufficient balance");
        } else {
        ...
        IList list = config.getList();
        require(list.getStateV1(msg.sender), "permission denied");
        _mintFee(token, msg.sender, fee);
    }

    function mintFree(address to, uint8 count) external onlyCaller {
        for (uint8 i = 0; i < count; i++) {
            _mintFee(address(0), to, 0);
        }
    }
    ...
    function _mintFee(
        ...
        id = id.add(1, "");
        uint256 managerFee = fee
            .mul(config.getNormalManagerFeeRate(), "")
            .div(100, "");
        uint256 superPrizeFee = fee
            .mul(config.getNormalSuperPrizeFeeRate(), "")
            .div(100, "");
        uint256 prizePoolFee = fee.sub(
            superPrizeFee.add(managerFee, ""),
            ""
        );
        if (0 < managerFee) {
            //管理费
            TransferV1.safeTransferFrom(
                token,
                msg.sender,
                config.getNormalManagerFeeAddress(),
                managerFee
            );
            totalManageFee = totalManageFee.add(managerFee, "");
        }
        if (0 < superPrizeFee) {
            //超级奖池费
            TransferV1.safeTransferFrom(
                token,
                msg.sender,
                config.getNormalSuperPrizeFeeAddress(),
                superPrizeFee
            );
            totalSuperPrizeFee = totalSuperPrizeFee.add(
                superPrizeFee,
                ""
            );
        }
        if (0 < prizePoolFee) {
            ...
            TransferV1.safeTransferFrom(
                token,
                msg.sender,
                config.getNormalPrizePoolAddress(),
                prizePoolFee
            );
            totalPrizePoolFee = totalPrizePoolFee.add(prizePoolFee, "");
        }
        mintInfos[id] = MintInfo(
            id,
            token,
            managerFee,
            superPrizeFee,
    ...
    // pass
    function setUse(uint256 tokenId) external override {
        require(ownerOf(tokenId) == msg.sender, "permission denied");
        use[tokenId] = true;
    }

    // pass
    function getUse(uint256 tokenId) external view override returns (bool) {
        ownerOf(tokenId);
        return use[tokenId];
    }

    // pass
    function tryGetNFTInfo(uint256 tokenId)
        external
        view
        override
        returns (bool, MintInfo memory)
    {
        MintInfo memory mintInfo = mintInfos[tokenId];
        return (0 != mintInfo.id, mintInfo);
    }
}
...
library TransferV1 {
    ...
    function safeTransferFrom(
        ...
        if (address(0) == token) {
            (bool success, ) = to.call{value: value}(new bytes(0));
            require(success, "TransferHelper::safeTransferFrom failed");
        } else {
...
contract Caller is Ownable {
    ...
    modifier onlyCaller() {
        require(caller[msg.sender], "permission denied");
        _;
    }
