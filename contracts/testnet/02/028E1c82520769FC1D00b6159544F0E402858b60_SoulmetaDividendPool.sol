...
contract SoulmetaDividendPool is ERC721Holder, DivestorUpgradeable {
    ...
    function searchUserBonus(address player_) view public returns(uint) {
        ...
        for (uint i = _getDiffMonth(lastMonth, lastSettleMonth); i < count - 1; i++) {
            tempPledgeNftCount[i+1] = tempPledgeNftCount[i];
            tempMoneyRate[i] = tempPledgeNftCount[i] > 0 ? bonusPool[_getAddMonthByMonth(lastSettleMonth, i)] / tempPledgeNftCount[i] : 0;
        }
    ...
    function setUserData(address player, uint accumulate, uint cumulative, uint[] calldata nftIdList, uint lastSettleMonth) public onlyOwner {
        UserInfo storage userInfo = userInfos[player];
        userInfo.accumulate = accumulate;
        userInfo.cumulative = cumulative;
        userInfo.nftIdList = nftIdList;
        userInfo.lastSettleMonth = lastSettleMonth;
    }

    function setUserDayData(uint month, address player, uint nft) public onlyOwner {
        userPledgeNftCount[month][player] = nft;
    }

    function setTotalDayData(uint month, uint nft) public onlyOwner {
        pledgeNftCount[month] = nft;
    }

    function setBonusPool(uint month, uint count) public {
        bonusPool[month] = count;
    }

    function setMoneyRate(uint month, uint count) public onlyOwner {
        moneyRate[month] = count;
    }

    function setLastSettleDay(uint month) public onlyOwner {
        lastMonth = month;
    }

    function pledgeNftTest(uint[] calldata tokenIds_, uint month_) public {
        require(isOpen, "not open");
        ...
        pledgeNftCount[month_] += tokenIds_.length;
        userPledgeNftCount[month_][_msgSender()] += tokenIds_.length;
        ...
        // userInfo.cumulative = cumulative;
        // userInfo.nftIdList = nftIdList;
        userInfo.lastSettleMonth = month_;
    }
}
