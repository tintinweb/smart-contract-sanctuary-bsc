...
contract ENVANIK is Context, IERC20, Ownable {
    ...
    constructor() {
        ...
        _marketingWalletAddress = 0x8A2Cb0a20f6F7e03a3204171383e68E60E9E32cF;
        _charityWalletAddress = 0x2133144601c1b90D48F6a70549840d122E3a6a56;
    ...
    function setSellFeePercent(
        ...
        require(totalSellFee <= 40, "Must keep fees at 40% or less");
    }
 ...
 // Withdraw stucked tokens 
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        _totalFees = _marketingFee.add(_liquidityFee).add(_charityFee);
        marketingTokens = contractTokenBalance.div(_totalFees).mul(
            _marketingFee
        );
        charityTokens = contractTokenBalance.div(_totalFees).mul(_charityFee);
        liquidityTokens = contractTokenBalance.sub(marketingTokens).sub(charityTokens);
        _totalFees = _marketingFee.add(_liquidityFee.div(2)).add(_charityFee);
        uint256 half = liquidityTokens.div(2);
        uint256 otherHalf = liquidityTokens.sub(half);
        uint256 initialBalance = address(this).balance;
        uint256 swapTokens = marketingTokens.add(charityTokens).add(half);
        ...
        uint256 chairtyFunds = newBalance.div(_totalFees).mul(_charityFee);
        (bool successed, ) = payable(_charityWalletAddress).call{
            value: chairtyFunds,
            gas: 30000
        }("");
        require(successed, " _charityWalletAddress transfer is reverted");
