...
interface VerifyContract {
    function verifyBuyBox(bytes memory data)
        external
        returns (
            uint256,
            string memory,
...
contract BoxNFT is ERC721URIStorage, Member {
    ...
    event BuyBox(address, address, uint256, uint256);
    event ReceivedEther(address indexed sender, uint256 amount);


    constructor(address _verify) ERC721("BoxNFT", "BoxNFT") {
        getTokenAddr = msg.sender;
        verify = VerifyContract(_verify);
        fee = 350;
    }

    function buyBox(bytes memory data) public payable {
        (
            uint256 amount,
            string memory tokenURI,
            address payToken,
            address user
        ) = verify.verifyBuyBox(data);
        uint fees = amount.mul(fee).div(1000);
        if (payToken != address(0)) {
            IERC20(payToken).transfer(mainAddress, fees);
            IERC20(payToken).transfer(getTokenAddr, amount.sub(fees));
        } else {
            payable(mainAddress).transfer(fees);
            payable(getTokenAddr).transfer(amount.sub(fees));
        }

        uint256 tokenId = awardItem(user, tokenURI);
        emit BuyBox(user, payToken, amount, tokenId);
    }
