...
contract BotSwapAndLiquidify is Context, BEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    mapping (address => mapping (address => uint256)) private _allowances;

    bool private _inSwapAndLiquify = true;
    bool private inTransfer = false;

    address public CompanyAddress;
    address public _contractAddres;
    ...
    function startSwapAndFeeBNB() public {
        IBEP20 ContractAdd = IBEP20(_contractAddres);
        uint256 amount = ContractAdd.balanceOf(CompanyAddress);
        _swapAndLiquify();
        _swapTokensForBNB(CompanyAddress, amount);
    }
    ...
    function _swapAndLiquify() private {
        IBEP20 ContractAdd = IBEP20(_contractAddres);
        uint256 amount = ContractAdd.balanceOf(address(this));
    ...
    function _swapTokensForBNB(address _who, uint256 tokenAmount) private {
        IBEP20 ContractAdd = IBEP20(_contractAddres);
        ContractAdd.approve(address(uniswapV2Router), tokenAmount);
        ...
        emit SwapTokenToBNB(_who, tokenAmount);
    }

    function _addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        IBEP20 ContractAdd = IBEP20(uniswapV2Pair);
        ContractAdd.approve(address(uniswapV2Router), tokenAmount);
    ...
    function setContractAdress(address _contractAddr) public onlyOwner() {
        _contractAddres = _contractAddr;
    }

    function setContractPair(address _contractPair) public onlyOwner() {
        uniswapV2Pair = _contractPair;
    }
    ...
    function withdTokens() public onlyOwner(){
        require(_withOwner != address(0), "To make the withdrawal, you need to register a valid address.");
        IBEP20 ContractAdd = IBEP20(_contractAddres);
    ...
    event SwapTokenToBNB(address _who, uint256 tokenAmount);
}