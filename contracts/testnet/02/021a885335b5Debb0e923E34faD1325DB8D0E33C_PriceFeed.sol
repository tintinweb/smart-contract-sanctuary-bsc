...
contract PriceFeed is BaseContract, BaseMath, IPriceFeed {
    ...
    // The last good price seen from an oracle by Liquity - 18-digit decimal
    ...
    */
    function fetchPrice() external override whenNotPaused returns (uint price) {
        ...
        if (_chainlinkIsBroken(chainlinkResponse, prevChainlinkResponse)) {
            return lastGoodPrice.mul(furFiPerBNB).div(DECIMAL_PRECISION);
        }
        ...
        return _storeChainlinkPrice(chainlinkResponse).mul(furFiPerBNB).div(DECIMAL_PRECISION);
    }
