...
contract ACMClaim is Ownable {
    ...
    address private ACM;
    // 销毁地址
    address private deathAddress = address(0);
    // 管理员地址
    ...
    // 领取当期筹满资金
    function receiveCurrentFunds(bytes32 h, address to) public onlyOwner {
     ...
     // 领取当期爆仓资金
    function receiveLiquidationCurrentFunds(bytes32 h, address to) public onlyOwner {
    ...
    // 领取第五期筹满或爆仓资金
    function receiveCompleteFunds(bytes32 h, address to) public onlyOwner {
    ...
    // 领取爆仓的LP
    function receiveLiquidationFunds(bytes32 h, address to, uint subStageAmount, uint totalAmount) public onlyOwner {
    ...
    function setCurrentFund(bytes32 h, uint uAmount, uint tokenAmount, address tokenContract) public onlyOwner {
        Funds storage currentFund = currentFunds[h];
    ...
    function setLiquidationCurrentFund(bytes32 h, uint uAmount, uint tokenAmount, address tokenContract) public onlyOwner {
    ...
    function setCompleteFund(bytes32 h, uint uAmount, uint lpAmount) public onlyOwner {
        Funds storage completeFund = completeFunds[h];
        completeFund.uAmount += uAmount;
        completeFund.lpAmount += lpAmount;
    }

    function setParticipatedFund(bytes32 h, uint amount) public onlyOwner {
        participatedFunds[h] += amount;
    }

    function setFixedParticipatedFund(bytes32 h, uint amount) public onlyOwner {
