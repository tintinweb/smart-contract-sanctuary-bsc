...
contract BasicToken is ERC20, ERC20Burnable, Pausable, Ownable, ReentrancyGuard {
    ...
    whenNotPaused
    {
        ...
        if (
            ...
            // Note - swap and addLiquidity are done for any transfer except buy (to avoid uniswap reentrancy guard and failure)
            if (liquidityFee > 0) {
    ...
    function payFeesInBnb() external onlyOwner doNotApplyFees nonReentrant {
        for (uint8 i = 0; i < feeRecipientAddresses.length; i++) {
            address feeRecipientAddress = feeRecipientAddresses[i];
            FeeRecipient memory feeRecipient = feeRecipients[feeRecipientAddress];
            uint256 amount = feeRecipient.amountToSwapDeposit;
            if (amount > 0) {
                uint256 bnbAmount = swapToBNB(amount);
                if (bnbAmount > 0) {
    ...
    function addFeeRecipient(address recipient, uint16 buyFee, uint16 sellFee, bool swap) external onlyOwner {
        ...
        uint256 totalFees = buyFee + burnFee + liquidityFee;
        for (uint8 i = 0; i < feeRecipientAddresses.length; i++) {
            address feeRecipientAddress = feeRecipientAddresses[i];
            FeeRecipient memory feeRecipient = feeRecipients[feeRecipientAddress];
        ...
        totalFees = sellFee + burnFee + liquidityFee;
        for (uint8 i = 0; i < feeRecipientAddresses.length; i++) {
            address feeRecipientAddress = feeRecipientAddresses[i];
            FeeRecipient memory feeRecipient = feeRecipients[feeRecipientAddress];
        ...
        feeRecipients[recipient] = FeeRecipient(buyFee, sellFee, true, swap, 0);   // Note that 'exists' must always be true
