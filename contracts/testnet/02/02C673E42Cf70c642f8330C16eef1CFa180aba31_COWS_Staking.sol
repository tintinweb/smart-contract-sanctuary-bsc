...
contract COWS_Staking is ReentrancyGuard {
  ...
  event Staking(address indexed user, uint256 indexed amount, uint256 indexed timeRelease);
  event ClaimAt(address indexed user, uint256 indexed claimAmount);
  event ConfigPack(uint256 indexed daysRelease, uint256 indexed amountPack);
   ...
   */
  function configPack(uint256 _daysRelease ,uint256 _amountPack) public onlyAdmin {
      amountPack = _amountPack * DECIMAL_18;
      daysRelease = _daysRelease;
      emit ConfigPack(daysRelease, amountPack);
  }
  ...
  function getStakingInfo(address account) external view returns (uint256 total, uint256 claim , uint256 time )
  {
      return (buyerAmount[account],claimAmount[account],timeRelease[account]);
  }
   ...
   function releaseCOWS() public ifNotPaused returns (uint256) {
       ...
       require(IERC20(STAKING_TOKEN).balanceOf(address(this)) >= claim_amount, "Sorry: not enough tokens to release");
       IERC20(STAKING_TOKEN).transfer(recipient, claim_amount);   
  ...
  function StakingCOWS() public ifNotPaused returns (address account, uint256 total, uint256 time) {
      ...
      uint256 allowance = IERC20(STAKING_TOKEN).allowance(msg.sender, address(this));
      require(allowance >= amount, "Check the token allowance.");
      IERC20(STAKING_TOKEN).transferFrom(recipient, address(this), amount);   
      if(buyerAmount[recipient]==0)
      {
          totalUser += 1;
      } 
      buyerAmount[recipient] += amount;
      timeRelease[recipient] = block.timestamp + daysRelease * TIME_1DAY;
      totalStaking += amount;
      emit Staking(recipient, amount, timeRelease[recipient]);
      return (recipient, buyerAmount[recipient], timeRelease[recipient]);
  }
}