...
contract PancakeFlashSwap {
    ...
    function setTokens(address _tokenA, address _tokenB, address _tokenC) public {
        TOKENA = _tokenA;
        TOKENB = _tokenB;
        TOKENC = _tokenC;
    ...
    function getDecimals(address _address) public view returns (uint8) {
        uint8 decimals = IERC20(_address).decimals();
        return decimals;
    }
    ...
    function getLoanAmount(uint _amount) public pure returns(uint) {
        return _amount; 
    }
    ...
    function startArbitrage(address _tokenBorrow, uint256 _amount) external {
        IERC20(TOKENA).safeApprove(address(PANCAKE_ROUTER), MAX_INT);
        IERC20(TOKENB).safeApprove(address(PANCAKE_ROUTER), MAX_INT);
        IERC20(TOKENC).safeApprove(address(PANCAKE_ROUTER), MAX_INT);
    ...
    function pancakeCall(
        ...
        // Place Trades
        uint256 trade1AcquiredCoin = placeTrade(TOKENA, TOKENB, loanAmount);
        uint256 trade2AcquiredCoin = placeTrade(TOKENB, TOKENC, trade1AcquiredCoin);
        uint256 trade3AcquiredCoin = placeTrade(TOKENC, TOKENA, trade2AcquiredCoin);
        ...
        // Pay Myself
        IERC20 otherToken = IERC20(TOKENA);
