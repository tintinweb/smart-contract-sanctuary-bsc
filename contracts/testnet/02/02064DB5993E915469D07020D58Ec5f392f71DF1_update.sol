...
contract update is Iupdate{
    ...
    function UP_memory() internal {
        Coll=ICollections(address(Contr.getCollections()));
        Cash=ICashback(address(Contr.getCashback()));
        FNFTA=IFundNFTA(address(Contr.getFundNFTA()));
        dis=Idis(address(Contr.getdis()));
        Det=IDetails(address(Contr.getDetails()));
        MINT=IMINT(address(Contr.getMINT_GNFT()));
        Skill=ISkill_c(address(Contr.getSkillUP()));
        Zoom=IZoomLoupe(address(Contr.getZoomLoupe()));
        Lord=ILandLord(address(Contr.getLandLord()));
        TF=ITITIFUND(address(Contr.getTitifund()));
    }
...
import "./IContracts_TITIMITI.sol";
import "./ICollections.sol";
import "./ILandLord.sol";

contract LandLord is Ownable, Pausable,ILandLord{
    IContracts_TITIMITI Contr;
    IERC20 MITI;
    IERC20 USDT;
    ICollections Coll;
    ...
    function TakeLandLord (uint256 id_Collection, uint256 id_NFT) public {
        ...
        IERC721 NFTW;
        NFTW =IERC721(Coll.GetID_Collection(id_Collection));
        require(NFTW.ownerOf(id_NFT)==msg.sender);
        uint256  One_MITI;
        uint256  One_USDT;

        uint256 Min_Bank = MITI.balanceOf(address (this));
    ...
    function Get_balance (uint256 ID_NFTW,uint256 id_Collection) public virtual override view returns(uint256 _USDT,uint256 _MITI ){
        uint256 ONE_BANK_USDT_= (USDT.balanceOf(address (this))+BANK_USDT_deduction)/Total_NFTW;
        uint256 ONE_BANK_MITI_= (MITI.balanceOf(address(this))+BANK_MITI_deduction)/Total_NFTW;
        uint256 SummaUSDT= ONE_BANK_USDT_-CheckLandLord_USDT[id_Collection][ID_NFTW];
        uint256 SummaMITI= ONE_BANK_MITI_-CheckLandLord_MITI[id_Collection][ID_NFTW];
...
require(hs);
    ...
    function set_Total_NFTW(uint256 new_Total_NFTW) public onlyOwner {
        Total_NFTW=new_Total_NFTW;
    }
...
interface ILandLord { 
    function  update() external;
    function Get_balance (uint256 ID_NFTW,uint256 id_Collection) external view returns(uint256 _USDT,uint256 _MITI );
}
