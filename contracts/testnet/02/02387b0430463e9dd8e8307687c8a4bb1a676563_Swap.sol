...
contract Swap is Pausable, AccessControl {
  ...
  mapping(uint256 => bool) public tradeToExecuted;
  ...
  function getOfferStatus(uint256 _offerId) external view returns (bool) {
    return tradeToExecuted[_offerId];
  }
  ...
  function createTradeOffer(
    ...
    require(_friend != msg.sender, "CANNOT_TRADE_WITH_SELF");
    require(msg.sender != address(0), "INV_ADDRESS");
    require(_friend != address(0), "INV_ADDRESS");

    require(trustedToken.ownerOf(_traderTokenId) == msg.sender, "NOT_OWNER");
    require(trustedToken.ownerOf(_friendTokenId) == _friend, "FRIEND_NOT_OWNER");
