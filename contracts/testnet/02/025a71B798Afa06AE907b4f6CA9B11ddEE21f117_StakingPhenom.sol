...
contract StakingPhenom{
    ...
    event AddedToStaking(address indexed user, uint256 amount, StakingType token, uint256 period);
    event WithdrawalPercents(address indexed user, uint256 amount, StakingType token);
    event WithdrawalBodyDeposit(address indexed user, uint256 amount, StakingType token);
    event ReinvestedPercents(address indexed user, uint256 amount, StakingType token);
    event AddedToTokenStaking(address indexed user, uint256 amount, StakingType token);
    ...
    function transferTokenToStaking(uint256 amount, uint256 period) public {
        ...
        holders[msg.sender].push(Staking(amount, StakingType.TOKEN, period, StakingPeriods[period], block.timestamp, periodDays, false));
        emit AddedToStaking(msg.sender, amount, StakingType.TOKEN, period);
    }
    ...
    function transferFrozenToStaking(uint256 period) public {
        ...
        for(uint i = 0; i < holders[msg.sender].length; i++){
            if(holders[msg.sender][i].frozen){
                ...
                emit AddedToStaking(msg.sender, holders[msg.sender][i].amount, StakingType.USDT, period);
            }
        }
    ...
    function delivery(
        ...
        if(getSales() == 1){
            ...
            holders[user].push(Staking(amount, StakingType.USDT, 0, 0, 0, 0, true));
    ...
    function withdrawPercentsForPeriod(address user, StakingType token, uint256 period) public{
        for(uint i = 0; i < holders[user].length; i++){
    ...
    function withdrawOneStakingPercents(address user, uint256 stakingNum) public{
        ...
        if(amount > 0){
            ...
            emit WithdrawalPercents(user, amount, StakingType.TOKEN);
        }
        if(bAmount > 0){
            ...
            holders[user].pop();
            emit WithdrawalBodyDeposit(user, bAmount, StakingType.TOKEN);
        }
        if(amountUSDT > 0){
            ...
            emit WithdrawalPercents(user, amountUSDT, StakingType.USDT);
        }
        if(bAmountUSDT > 0){
            ...
            holders[user].pop();
            emit WithdrawalBodyDeposit(user, bAmountUSDT, StakingType.USDT);
        }
    ...
    function addToStaking(uint256 amount, uint256 stakingNum) public{
        ...
        require(holders[msg.sender][stakingNum].staking == StakingType.TOKEN, "Staking is not in the project tokens!");
        require(amount > 0, "The amount cannot be zero");
        _depositToken.transferFrom(msg.sender, address(this), amount);
        ...
        emit AddedToTokenStaking(msg.sender, amount, StakingType.TOKEN);
    }
    ...
    function reinvestStakingPercentsForPeriod(address user, StakingType token, uint256 period) public{
        for(uint i = 0; i < holders[user].length; i++){
            if(holders[user][i].staking == token && holders[user][i].StakingMonths == period){
                reinvestOneStakingPercents(user, i);
            }
    ...
    function reinvestOneStakingPercents(address user, uint256 stakingNum) public{
        ...
        if(amount > 0){
            ...
            user,nAmount,allStakingAmount));
            require(success,"depositPools call FAIL");
            emit ReinvestedPercents(user, amount, StakingType.TOKEN);
        }
        if(amountUSDT > 0){
            holders[user][stakingNum].amount += amountUSDT;
            emit ReinvestedPercents(user, amountUSDT, StakingType.USDT);
        }
    }
