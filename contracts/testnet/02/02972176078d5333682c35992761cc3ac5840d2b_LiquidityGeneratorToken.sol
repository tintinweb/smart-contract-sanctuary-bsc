...
contract LiquidityGeneratorToken is IERC20, Ownable, BaseToken {
	...
	mapping(address => bool) private _isExcludedFromFeesell;
	mapping(address => bool) private _isExcludedFromFeebuy;
	...
	address public _charityAddresstwo;

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled;
	bool public tradingEnabled = false;
	uint256 public _maxTxAmount = 5000000 * 10**6 * 10**9;
    uint256 public _walletMax =  5000000 * 10**6 * 10**9;
    ...
    constructor(
		...
		require(extraSellFeeBps_ >= 0, "Invalid extra charity fee");
		...
		_extraSellFee = extraSellFeeBps_;
    ...
    function excludeFromFeeSell(address account) public onlyOwner {
        _isExcludedFromFeesell[account] = true;
    }
    ...
    function includeInFeeSell(address account) public onlyOwner {
        _isExcludedFromFeesell[account] = false;
    }
	
	function excludeFromFeeBuy(address account) public onlyOwner {
        _isExcludedFromFeebuy[account] = true;
    }

    function includeInFeeBuy(address account) public onlyOwner {
        _isExcludedFromFeebuy[account] = false;
    }
	...
	function setextraSellFee(uint256 value) external onlyOwner {
            _extraSellFee = value;
        require(
            _taxFee + _liquidityFee + _charityFee + _extraSellFee <= 10**4,
            "Total fee is over 100%"
        );
    }
	...
	function setMarketingWallettwo(address payable wallet) external onlyOwner {
        _charityAddresstwo = wallet;
    }
        ...
        )
    {
        ...
        uint256 tCharityFee = calculateCharityFee(tAmount, issell);
    ...
    function calculateCharityFee(uint256 _amount,bool _issell)
        private
        view
        returns (uint256)
    {
        if (_charityAddress == address(0)) return 0;
		uint256 _totalfee = _charityFee;
		if(_issell == true)
		{
			_totalfee = _charityFee.add(_extraSellFee);
		}
        return _amount.mul(_totalfee).div(10**4);
    }
	
	function calculateextraCharityFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        if (_charityAddress == address(0)) return 0;
        return _amount.mul(_extraSellFee).div(10**4);
    }
    ...
    function _transfer(
        ...
        {
            //require(_balances[to].add(amount) <= _walletMax);
        ...
        {
			tissell = true;
            if (_isExcludedFromFeesell[from] ) {
            takeFee = false;
            }
        }
        ...
        _tokenTransfer(from, to, amount, takeFee, tissell);
    }
    ...
    function _tokenTransfer(
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount, _issell);
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount,_issell);
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount, _issell);
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount, _issell);
        } else {
            _transferStandard(sender, recipient, amount, _issell);
        }
