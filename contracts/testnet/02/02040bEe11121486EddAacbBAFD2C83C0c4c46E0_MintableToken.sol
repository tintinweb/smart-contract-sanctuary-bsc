...
contract MintableToken is ERC20, Whitelist {
    ...
    constructor(string memory _name, string memory _symbol, address _operator, address _feeRecipient) ERC20(_name, _symbol) Ownable() {

        feeRecipient = _feeRecipient;

        addAddressToWhitelist(_operator);

        _mint(_operator, (1000000 * (10**18)));
    ...
    function topUpRewards(address _user, uint256 _amount) external onlyWhitelisted returns (bool) {
    ...
    function setFeeRecipientAddress(address _recipient) public onlyOwner() {
