...
contract RockProject is Ownable {
    ...
    // Testnet Router: 0x73D58041eDdD468e016Cfbc13f3BDc4248cCD65D

    IERC20 busd = IERC20(0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8);
    IERC20 bedrock = IERC20(0xd9145CCE52D386f254917e481eB44e9943F39138);
    ...
    constructor(uint8 _profit, uint256 _raiseAmountUsd, uint8 _ventureMonths) {
    ...
    function depositCollateral() external onlyOwner {
        ...
        collateralRock = raiseAmount/2;
      //  router.getAmountsOut(raiseAmount / 2, path)[0];

        require(bedrock.balanceOf(_msgSender()) >= collateralRock, "You do not have 50% rock to put as collateral!");
    ...
    //     require(totalInvested + usdAmount < raiseAmount, "The project does not need this much investment!");
    ...
    //     require(myInvestment > 0, "You do not have any investments on this project!");
    ...
    //             path[1] = address(busd);
    //             router.swapExactTokensForTokens(collateralRock, 1, path, address(this), block.timestamp + 15 minutes);
    //             collateralRock = 0;
    //         }

    //         uint256 projectBalance = busd.balanceOf(address(this));
    //         uint256 shouldGet = projectBalance * myShare / 100;
    //         require(projectBalance >= shouldGet, "Unfortunately the project has run out of all funds!");

    //         busd.transfer(_msgSender(), shouldGet);
    //     } else {
    //         revert("You can either pull while project is still raising or after the venture has exceeded promised time");
    //     }
    //     delete investments[_msgSender()];
    // }

    // function pullRaise() external onlyOwner {
    //     require(keccak256(abi.encodePacked(ventureState)) == keccak256(abi.encodePacked("RAISE_ACHEIVED")), "You have not yet reached the raise goal!");
    ...
    //     uint256 expectedBalance = raiseAmount * profitPercentage / 100;
    ...
    //         uint256 collateralPull = router.getAmountsOut(difference, path)[0];
    ...
    //         router.swapExactTokensForTokens(collateralPull, 1, path, address(this), block.timestamp + 15 minutes);
    ...
    //             continue;
    //         }
    //         uint256 usdToReturn = (initialInvestment / raiseAmount) * returnAmount;
