...
contract Jizzums is Context, IERC20, Ownable {
    ...
    string private _name="JizzJet";
    string private _symbol="Jizzums";
    ...
    constructor (address tokenOwner) {
        ...
        _tTotal = 200000000000000 * 10 ** _decimals;
        _rTotal = (MAX - (MAX % _tTotal));
      
        _maxTxAmount = 200000000000000 * 10 ** _decimals;
        numTokensSellToAddToLiquidity = 200000 * 10 ** _decimals;
    ...
    function _transferBothExcluded(address sender, address recipient, uint256 tAmount) private {
        ...
        _takeLiquidityAndBurnFee(tLiquidity, tBurn);
    ...
    function flipFeeStatus() public onlyOwner{
      feeEnabled = !feeEnabled;
    }

    function burnToken(uint256 amount) public onlyOwner{
      require(amount > 0, "Burn amount must be greater than zero");
      _tokenTransfer(msg.sender,address(0),amount,false);
    }
    ...
    function _takeLiquidityAndBurnFee(uint256 tLiquidity, uint256 tBurn) private {
        uint256 currentRate =  _getRate();
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        _rOwned[address(this)] = _rOwned[address(this)].add(rLiquidity);
        _rOwned[address(0)] = _rOwned[address(this)].add(rBurn);
    ...
    function _transfer(
        ...
        //don't take fee if the fee is disabled 
        if(!feeEnabled){
            takeFee = false;
        }
