...
contract Creator is Ownable {
        ...
        payable
    {
        ...
        payable(feeWallet).transfer(7 * 10**16);

        address _feeDistributor = feeDistributorFactory.createFeeDistributor(
            _creationParameters.token,
            _creationParameters.dualToken,
            _creationParameters.dualTokenVault,
            router,
        ...
        address _vault = stakingFactory.createStaking(
            ...
            _creationParameters.tokenDecimals,
            _creationParameters.dualTokenDecimals,
            _creationParameters.partnerAdmin,
        ...
        stakings[_creationParameters.token] = [
            _vault,
            _feeDistributor,
            _rewardDistributor
        ];
        ...
        StakingVaultV6_0_0(payable(_vault)).setFeeDistributor(payable(_feeDistributor));
        StakingVaultV6_0_0(payable(_vault)).setFeeTracker(feeTracker);
        StakingVaultV6_0_0(payable(_vault)).setRewardDistributor(
...
contract VaultInfoBundler is Ownable {
    ...
    struct TokensInfo {
        uint256 totalSupply;
        string tokenSymbol;
        string secondTokenSymbol;
    }

    struct VaultPageInfo {
        StakingVaultV6_0_0.Controls controls;
        StakingVaultV6_0_0.UserInfo1 userInfo1;
        StakingVaultV6_0_0.UserInfo2 userInfo2;
        FeeDistributorV6.EntryExitFees entryExitFees;
        FeeDistributorV6.APYData apyData;
        address adminPayOutWallet;
    ...
    struct VaultListInfo {
        address[4] addresses;
        StakingVaultV6_0_0.VaultVariables vaultVariables;
        address token;
        address secondToken;
        StakingVaultV6_0_0.Durations durations;
    ...
    function bundleVaultListPageInfo(address staking)
        public
        view
        returns (VaultListInfo memory)
    {
        VaultListInfo memory _vaultListInfo;
        (
            address addr1,
            address addr2,
            IFeeDistributorV6 addr3,
            IRewardDistributorV6 addr4
        ) = StakingVaultV6_0_0(payable(staking)).getAddresses();
        StakingVaultV6_0_0.VaultVariables memory _vaultVariables = StakingVaultV6_0_0(
            payable(staking)
        ).getVaultVariables();
        (address _token, address _secondToken) = StakingVaultV6_0_0(payable(staking))
            .getTokens();
        StakingVaultV6_0_0.Durations memory _durations = StakingVaultV6_0_0(
            payable(staking)
        ).getDurations();
        uint256 _pricePerFullShare = StakingVaultV6_0_0(payable(staking))
            .getPricePerFullShare();

        _vaultListInfo = VaultListInfo({
            addresses: [addr1, addr2, address(addr3), address(addr4)],
            vaultVariables: _vaultVariables,
            token: _token,
            secondToken: _secondToken,
            durations: _durations,
            pricePerFullShare: _pricePerFullShare,
            price: getPrice(_token),
            level: creator.getVaultLevel(staking)
        });
        return _vaultListInfo;
    }

    function bundleVaultPageInfo(address staking, address user)
        public
        view
        returns (VaultPageInfo memory)
    {
        VaultPageInfo memory _vaultPageInfo;
        StakingVaultV6_0_0.Controls memory _controls = StakingVaultV6_0_0(
            payable(staking)
        ).getAllControls();
        StakingVaultV6_0_0.UserInfo1 memory _userInfo1 = StakingVaultV6_0_0(
            payable(staking)
        ).getUserInfo1(user, 0);
        StakingVaultV6_0_0.UserInfo2 memory _userInfo2 = StakingVaultV6_0_0(
            payable(staking)
        ).getUserInfo2(user, 0);
        (, , IFeeDistributorV6 feeDistributorAddress, ) = StakingVaultV6_0_0(
            payable(staking)
        ).getAddresses();
        FeeDistributorV6.EntryExitFees memory _entryExitFees = FeeDistributorV6(
            payable(feeDistributorAddress)
        ).getEntryExitFees(0);
        FeeDistributorV6.APYData memory _apyData = FeeDistributorV6(
            payable(feeDistributorAddress)
        ).getAPYData();
        StakingVaultV6_0_0.VaultVariables memory _vaultVariables = StakingVaultV6_0_0(
            payable(staking)
        ).getVaultVariables();
        (address token, address secondToken) = StakingVaultV6_0_0(payable(staking))
            .getTokens();
        TokensInfo memory _tokensInfo = TokensInfo({
            totalSupply: ERC20(token).totalSupply(),
            tokenSymbol: ERC20(token).symbol(),
            secondTokenSymbol: ''
        });
        if (secondToken != address(0)) {
            _tokensInfo.secondTokenSymbol = ERC20(secondToken).symbol();
        }
        (uint256 _userBalance, ) = StakingVaultV6_0_0(payable(staking))
            .calculateUserBalances(user, 0);

        _vaultPageInfo = VaultPageInfo({
            controls: _controls,
            userInfo1: _userInfo1,
            userInfo2: _userInfo2,
            adminPayOutWallet: FeeDistributorV6(payable(feeDistributorAddress))
                .adminPayoutWallet(),
            entryExitFees: _entryExitFees,
            apyData: _apyData,
            tokensInfo: _tokensInfo,
            userBalance: _userBalance,
            dualTokenVault: FeeDistributorV6(payable(feeDistributorAddress))
                .dualTokenVault()
        });
        return _vaultPageInfo;
    }
    ...
    function getPrice(address token) public view returns (uint256) {
        ...
        if (
            factory.getPair(path[0], path[1]) == address(0) ||
            factory.getPair(path[1], path[2]) == address(0)
        ) return 0;
...
/*
    implementation steps: todo
        features dual token (don't forget BNB dual reward dist), can choose distribution in withdraw function
        variables
        constructor
        deposit (addRewards) func
        withdraw func (will be called with the # of tokens needed, no need to do the calcs here)
        set functions
        e withdraw func
        runway left function? (just to put less code in other contract)
*/
