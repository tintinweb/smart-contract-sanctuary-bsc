...
contract SmartCleanEnergy is Context, IERC20, Values {
    ...
    string private _name = "Smart Clean Energy";
    ...
    function checkAndSwapCollectedTokens(address sender) public {
        if(sender != address(pair) && !feeSwappingInProgress){
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if((sender == pairAddress || recipient == pairAddress) && !excludedFromTaxes[sender]){
            checkAndSwapCollectedTokens(sender);
            takeFee = true;
        }
