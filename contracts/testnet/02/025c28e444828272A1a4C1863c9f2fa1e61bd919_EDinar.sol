...
contract EDinar is ContractData {
  ...
  function registerAccount(address ref) external payable  {
    ...
    require(maxAllowedDeposit >= msg.value, "Deposit to high");
    accounts[sender].up = ref;
    accounts[sender].registered = true;
    accountsRefs[ref].push(sender);
    emit ReferralRegistration(sender, ref);
  ...
  function _withdraw(uint amount) private {
    ...
    _payCumulativeNetworkFeeForWithdraw(totalToWithdraw.mul(5).div(100));
...
token.transferFrom(msg.sender,address(this),amount);
  ...
  function _payCumulativeNetworkFee() private {
    ...
    if (networkFee <= 0) return;
      payable(networkReceiverA).transfer((networkFee * 250) / 1000 );
      payable(networkReceiverB).transfer((networkFee * 250) / 1000 );
      payable(networkReceiverC).transfer((networkFee * 250) / 1000 );
      payable(networkReceiverD).transfer((networkFee * 250) / 1000 );
  ...
  function _payCumulativeNetworkFeeForWithdraw(uint256 amount) private {
    ...
    if (amount <= 0) return;
      payable(networkReceiverA).transfer((amount * 25) / 1000 );
      payable(networkReceiverB).transfer((amount * 25) / 1000 );
      payable(networkReceiverC).transfer((amount * 25) / 1000 );
      payable(networkReceiverD).transfer((amount * 25) / 1000 );
