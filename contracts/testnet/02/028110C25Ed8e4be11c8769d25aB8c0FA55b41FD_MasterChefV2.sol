...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    /// `accCakePerShare` Accumulated WAYAs per share, times 1e12.
   ...
   struct PoolInfo {
    ...
    // The WAYA TOKEN!
    CakeToken private immutable cake;
   ...
   //------------------------------------------------------------------------------------------
    ...
    event CakePerBlockUpdated (uint256 oldCakePerBlock, uint256 newCakePerBlock);


    constructor(
        CakeToken _cake,
        uint256 _cakeEmissionPerBlock,
        address _financialController
    ) {
        cake = _cake;
        cakePerBlock = _cakeEmissionPerBlock * (1e18);
    ...
    function CakeAddress() external view returns (CakeToken) {
        return cake;
    }
    ...
    function updateCakePerBlock(uint256 _newCakePerBlock) public onlyOwner {
        uint256 _oldCakePerBlock = cakePerBlock;
        cakePerBlock = _newCakePerBlock * (1e18);
        emit CakePerBlockUpdated(_oldCakePerBlock, _newCakePerBlock);
    }
    ...
    function addPool(
        ...
        require(_lpToken != cake, "WAYA token can't be added to farm pools");
    ...
    function pendingCake(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cakeReward = (multiplier * (_cakePerBlockFarm(pool.isRegular)) * (pool.allocPoint)) / (
                (pool.isRegular ? totalRegularAllocPoint : totalSpecialAllocPoint)
            );
            accCakePerShare = accCakePerShare + ((cakeReward *(ACC_WAYA_PRECISION)) / lpSupply);
        }

        uint256 boostedAmount = (user.amount * (getBoostMultiplier(_user, _pid))) / BOOST_PRECISION;
        return ((boostedAmount * (accCakePerShare)) / ACC_WAYA_PRECISION)- user.rewardDebt;
    }
    ...
    function _cakePerBlockFarm(bool _isRegular) internal view returns (uint256 amount) {
        if (_isRegular) {
            amount = (cakePerBlock * cakeRateToRegularFarm) / WAYA_RATE_TOTAL_PRECISION;
        } else {
            amount = (cakePerBlock * cakeRateToSpecialFarm) / WAYA_RATE_TOTAL_PRECISION;
        }
    }
    ...
    function updatePoolReward(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                uint256 multiplier = block.number - pool.lastRewardBlock;
                uint256 cakeReward = (multiplier * _cakePerBlockFarm(pool.isRegular) * pool.allocPoint) /  totalAllocPoint;
                cake.mint(address(this), cakeReward);
	pool.accCakePerShare = pool.accCakePerShare + (((cakeReward * ACC_WAYA_PRECISION) / lpSupply));
            }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = (((user.amount * multiplier) / BOOST_PRECISION) *pool.accCakePerShare) / ACC_WAYA_PRECISION;
    ...
    function reserveCake(bool _withUpdate) public onlyOwner {
        ...
        uint256 pendingCakeToReserve = multiplier * ((cakePerBlock * cakeRateToReserve) / WAYA_RATE_TOTAL_PRECISION);
    ...
    function settlePendingCake(
        ...
        uint256 accCake = (boostedAmount * poolInfo[_pid].accCakePerShare) / ACC_WAYA_PRECISION;
