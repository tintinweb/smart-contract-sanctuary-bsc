...
///
/// @custom:security-contact [email protected]
contract Mithrilverse_StaticStat is IERC1155,
                                    ERC165,
                                    Mithrilverse_Permissioned
{
    ...
    /// @dev Awarded but unclaimed tokens.
    /// @dev account => ID => amount
    mapping(address => mapping(uint256 => uint256)) internal _awarded;
    ...
    function initialMint() private {
        // Create initial generic stat.
        uint256 id = _counter.current();
        stats[id] = Stat(id, "GenericStat", 0, "");
        _counter.increment();

        // Transfer stat to owner.
        _balances[_msgSender()][id] = 1;
        stats[id].supply = 1;

        // Emit events.
        emit StatMinted(_msgSender(), stats[id].name, 1);
        emit TransferSingle(_msgSender(), address(0), _msgSender(), 0, 1);
    }
    ...
    /// Get the amount of token `id` awarded to but unclaimed by `account`.
    ///
    /// @param account of user.
    /// @param id of token.
    ///
    /// @return uint256 as `account`'s balance.
    function unclaimedBalance(address account, uint256 id) public view returns (uint256) {
        require(account != address(0), "Zero address");
        return _awarded[account][id];
    }

    /// Award tokens to `receivers` to be minted later.
    ///
    /// @param receivers addresses of account to award tokens to.
    /// @param ids of tokens.
    /// @param amounts of tokens to award.
    function awardBatch(
        ...
        onlyPermissioned
    {
        require(receivers.length == ids.length, "Unequal param lengths");
        require(ids.length == amounts.length, "Unequal param lengths");

        for (uint256 i = 0; i < ids.length; i++) {
            _awarded[receivers[i]][ids[i]] += amounts[i];
            emit StatAwarded(receivers[i], stats[ids[i]].name, amounts[i]);
        }
    }

    /// User claims awarded tokens.
    ///
    /// @param id of token.
        ...
        external
    {
        ...
        _awarded[account][id] -= amount;
        _balances[account][id] -= amount;

        emit StatMinted(account, stats[id].name, amount);
        emit TransferSingle(account, address(0), account, id, amount);
    }
        ...
        onlyPermissioned
    {
        ...
        require(ids.length == amounts.length, "Unequal ids and amounts length");

        address operator = _msgSender();

        for (uint256 i = 0; i < ids.length; i++) {
            _balances[to][ids[i]] += amounts[i];
            stats[ids[i]].supply += amounts[i];
...
///
/// @custom:security-contact [email protected]
contract Mithrilverse_Accolades is Mithrilverse_StaticStat
{
    /// @param _baseURI as shared URI prefix.
