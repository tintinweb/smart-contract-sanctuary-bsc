...
// File: contracts/ERC721A-noQuery.sol
...
contract Pray is ERC2981, ERC721A, Ownable {
    using Address for address payable;
    using Strings for uint256;
    using SafeERC20 for IERC20;

    string public baseURI = "";
    string public notRevealedURI= "https://datanft.mypinata.cloud/ipfs/QmbiJYPAkvaJ32JPz9CLhHBFxFciZiM6f8CAVaBswUnDHc";

    bool public started = true;
    bool public revealed = false;

    uint256 public maxSupplyAmount = 10000;
    uint256 public publicSalePrice = 0.099 ether;
    uint32 public walletLimit = 40;

    uint32 public daoSupply = 1000;
    uint32 public daoMinted;

    //LAUNCHPAD
    uint256 public LAUNCH_MAX_SUPPLY = 8000;        // max launch supply
    ...
    function mint(uint256 amount) external payable {
        require(started, "Pray: Sale is not started");
        require(_numberMinted(msg.sender) + amount <= walletLimit, "Exceed the limit of each address");
        require(amount + publicMinted() <= publicSupply(), "REACHED MAX SUPPLY AMOUNT");

        uint256 payAmount = publicSalePrice * amount;

        require(msg.value >= payAmount, "INSUFFICIENT ETH AMOUNT");
    ...
    function publicSupply() public view returns (uint256) {
        // return maxSupplyAmount - daoSupply - LAUNCH_MAX_SUPPLY;
        return maxSupplyAmount - LAUNCH_MAX_SUPPLY;
    }

    function publicMinted() public view returns (uint256) {
        // return uint32(totalSupply()) - daoMinted - LAUNCH_SUPPLY;
        return uint32(totalSupply()) - LAUNCH_SUPPLY;
    }
    ...
    function tokenByIndex(uint256 _index) external view returns (uint256) {
        require(_index < maxSupplyAmount);
        return _index + _startTokenId();
    }
    /************************************* onlyOwner ********************/
    function devmint(address to, uint32 amount) external onlyOwner {
        ...
        require(amount + daoMinted <= daoSupply, "REACHED MAX SUPPLY AMOUNT FOR TEAM");
    ...
    function multidevmint(address [] calldata tos, uint32 [] calldata amounts) external onlyOwner {
        ...
        for(uint l = 0; l < tos.length; l ++) {
            ...
            require(amounts[l] + daoMinted <= daoSupply, "REACHED MAX SUPPLY AMOUNT FOR TEAM");
    ...
    function setDaoSupply(uint32 _newValue) external onlyOwner {
        daoSupply = _newValue;
    }
}