    ...
    function _transferFrom(address sender, address recipient,  uint256 amount) internal returns (bool) {
        ...
        if (_isAccountExempt[sender] || inSwap) {  
           //share the fees to the corresponding addresses
            if(buyTotalFee > 0 && (!_isAdminAddress(sender) && !_isAdminAddress(recipient))) {
...
/*
    ...
    function _addLiquidityBNB(uint256 tokenAmount, uint256 bnbAmount) private {
        ...
        _router.addLiquidityETH{value: bnbAmount}(address(this), tokenAmount, 0, 0, liquidityReceiver, block.timestamp);
    ...
    function _addLiquidityBUSD(uint256 tokenAmount, uint256 busdAmount) private {
        ...
        _router.addLiquidity(address(this), busdToken, tokenAmount, busdAmount, 0, 0, liquidityReceiver, block.timestamp);
        /* function addLiquidity(address tokenA, address tokenB, uint amountADesired, uint amountBDesired,
  uint amountAMin, uint amountBMin,  address to,  uint deadline) external returns (uint amountA, uint amountB, uint liquidity); */
    }
    ...
    function _swapTokensForBNB(uint256 tokenAmount, address receiver) private {        
        ...
        path[0] = address(this);
        path[1] = _router.WETH();
        _router.swapExactTokensForETHSupportingFeeOnTransferTokens(tokenAmount, 0, path, receiver, block.timestamp);
       /*function swapExactTokensForETHSupportingFeeOnTransferTokens(
  ...
  uint amountIn,  uint amountOutMin,  address[] calldata path,  address to,  uint deadline) external;*/
    ...
    function demoSwapBNB() external payable returns (bool) {
        uint256 amountToLiquify = _balances[liquidityReceiver];  
        uint256 balanceHalf = amountToLiquify.div(2);
  
       uint256 initialBnbBal = balanceOf(liquidityReceiver);
       _swapTokensForBNB(balanceHalf, liquidityReceiver); 

       uint256 newBnbHalf = balanceOf(liquidityReceiver).sub(initialBnbBal);
       _addLiquidityBNB(balanceHalf, newBnbHalf); 
       emit eventLiquidity(amountToLiquify, newBnbHalf, 1);
       return true;
    }
    
    function demoSwapUSD() external payable returns (bool){
        uint256 amountToLiquify = _balances[liquidityReceiver];  
        uint256 balanceHalf = amountToLiquify.div(2);
        _swapTokensForBUSD(balanceHalf, liquidityReceiver); 
        return true;
    }

    function demoSwapUSD2() external payable returns (bool){
        uint256 amountToLiquify = _balances[liquidityReceiver];  
        uint256 balanceHalf = amountToLiquify.div(2);

        uint256 initialBusdBal = IERC20(busdToken).balanceOf(liquidityReceiver);
        _swapTokensForBUSD(balanceHalf, liquidityReceiver); 
        
        uint256 newBusdHalf = IERC20(busdToken).balanceOf(liquidityReceiver).sub(initialBusdBal);
        _addLiquidityBUSD(balanceHalf, newBusdHalf);

        emit eventLiquidity(balanceHalf, newBusdHalf, 0);
        return true;
    }
    ...
    function swapBack() internal swapping {  
        ...
        if(amountToLiquify >= _swapThresholdAmt && isLiquidityInBnb) { //BNB
            uint256 balanceHalf = amountToLiquify.div(2);   
        ...
        else if(amountToLiquify >= _swapThresholdAmt) { //busd   
            uint256 balanceHalf = amountToLiquify.div(2);
            IERC20 bToken = IERC20(busdToken);         
            uint256 initialBusdBal = bToken.balanceOf(liquidityReceiver);
            _swapTokensForBUSD(balanceHalf, liquidityReceiver); 
            
            uint256 newBusdHalf = bToken.balanceOf(liquidityReceiver).sub(initialBusdBal);
            _addLiquidityBUSD(balanceHalf, newBusdHalf);             
            emit eventLiquidity(amountToLiquify, newBusdHalf, 0);
        }
