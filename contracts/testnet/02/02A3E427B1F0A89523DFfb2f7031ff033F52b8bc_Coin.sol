...
contract Coin is SecureLaunch, Auth {
    ...
    constructor(
        ...
        setUpMarks();
        router = IDEXRouter(router_); // mmf router
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
...
contract DividendDistributor is IDividendDistributor {
    ...
    IDEXRouter router;
    IBEP20 token = IBEP20(0xc21223249CA28397B4B6541dfFaEcC539BfF0c59); // Token to be distributed
    ...
    constructor(address _router) {
        router = _router != address(0)
            ? IDEXRouter(_router) // Default is MMF router if token constructor sets it no need too update.
