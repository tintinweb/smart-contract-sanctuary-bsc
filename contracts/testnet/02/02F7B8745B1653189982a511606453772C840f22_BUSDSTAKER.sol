...
contract BUSDSTAKER {
    ...
    struct User {
        ...
        address referrer;
        uint256 bonus;
        uint256 [3] levelamount;
        mapping(uint256 => uint256) levelusers;
        uint256 downlinelength;
    ...
    function invest(address referrer, uint256 _amount) public {
        ...
        if (msg.sender == owner) {
            ...
            while(upline != address(0)){
                if(index < 3){
                    uint256 amount = _amount.mul(REFERRAL_PERCENTS[index]).div(
                        PERCENTS_DIVIDER);
                users[upline].bonus = users[upline].bonus.add(amount);
                users[upline].levelamount[index] += amount;
                }
                
                users[upline].levelusers[index] += 1;
                users[upline].downlinetotaldeposits =
                users[upline].downlinetotaldeposits.add(_amount);
                if(index > users[upline].downlinelength){
                    users[upline].downlinelength = index;
                }
    ...
    function getUserDownlineCount(address userAddress,uint256 index)
        public
        view
        returns (
            uint256
    ...
    {
        return (
            users[userAddress].levelusers[index]
        );
    }
    function getUserTreeUsersCount(address user) external view returns (uint256 members) {
        for(uint256 index;index <users[user].downlinelength + 1;index++){
            members+=users[user].levelusers[index];
        }
    }

    function getUserLevelAmount(address user,uint256 index)external view returns(uint256 amount){
        return users[user].levelamount[index];
    } 
