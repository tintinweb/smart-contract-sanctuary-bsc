...
contract Marketplace is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, ISimpleMarketplaceNativeERC721, IInitContract {
    ...
    CountersUpgradeable.Counter private _currentListingCount;    
    address public nft;
    ...
    function isItemListing(uint256 tokenId) public view returns(bool) {
        return tokensListing[tokenId] > 0;
    }

    function fetchListInfo(uint256 tokenId) public view returns(Listing memory) {
        return listings[tokensListing[tokenId]];
    }

    function fetchAllListingItemTokenID() public view returns (uint256[] memory){
        uint256 length = _currentListingCount.current();
        uint256 listingsIndex = lastListingId.current();
        uint256[] memory listingItems = new uint256[](length);
        for (uint256 i = 0; i < listingsIndex; i++) {
            Listing memory listing = listings[i+1];
            if( listing.exist == true ){
                listingItems[i] = listing.tokenId;
            }
        }
        return listingItems;
    }

    function fetchAllListingItemInfo() public view returns (Listing[] memory){
        uint256 length = _currentListingCount.current();
        uint256 listingsIndex = lastListingId.current();
        Listing[] memory listingItems = new Listing[](length);
        for (uint256 i = 0; i < listingsIndex; i++) {
            Listing memory listing = listings[i+1];
            if( listing.exist == true && listing.isSold == false ){
                listingItems[i] = listing;
            }
        }
        return listingItems;
    }
    ...
    function list(uint256 tokenId, uint256 price) external override onlyItemOwner(tokenId) onlyTransferApproval(tokenId) {
        ...
        _currentListingCount.increment();
    ...
    function buy(uint256 tokenId) external payable override{
        ...
        _currentListingCount.decrement();      
        token.transferFrom(_list.seller, msg.sender, tokenId);

        payable(_list.seller).transfer(msg.value*95/100);     //hold 5% 
