...
contract OKGBredCocoon is ERC721Enumerable {
	IOKGManagement public gov;
	IRestriction public restriction;
	...
	constructor(IOKGManagement _gov, IRestriction _restriction)
		ERC721("OKG Bred Cocoon", "OKGBC")
	{
		gov = _gov;
	...
	/**
		...
		@param	_gov				Address of OKGManagement contract
	*/
	...
	/**
		...
		@dev  Caller must have MANAGER_ROLE
		@param _restriction        Address of Restriction contract
		Note: To disable, MANAGER_ROLE sets address(0)
	*/
	function setRestriction(address _restriction) external onlyManager {
		restriction = IRestriction(_restriction);
	}
	...
	*/
	...
	/**
		@notice breed new cocoon
		@dev caller must have MINTER_ROLE;
		@param _owner address of the owner of new cocoon
		@param _tokenId token ID of new cocoon
	*/
	function breedCocoon(
		address _owner,
		uint256 _tokenId,
		uint256 _lockTill
	) external onlyMinter {
		lock[_tokenId] = _lockTill;
	...
	function _beforeTokenTransfer(
		...
		// cannot transfer before unlock
		require(
			_from == address(0) ||
				lock[_tokenId] == 0 ||
				lock[_tokenId] < block.timestamp,
			"Locked"
		);

		if (address(restriction) != address(0)) {
			if (!restriction.whitelisted(msg.sender)) {
				uint256 _restricted = restriction.allowances(address(this), _tokenId);
				if (
					_restricted == type(uint64).max &&
					(_from != address(0) && _to != BLACK_HOLE)
				) revert("Not for Trade");
				else if (
					_restricted == type(uint32).max &&
					(gov.hasRole(MARKETPLACE, msg.sender) || restriction.stores(_to))
				) revert("Restricted listing");

				// Item can be `Restricted Listing` and `On Leasing` concurrently
				require(
					restriction.onLeasing(address(this), _tokenId) < block.timestamp,
					"On leasing"
				);
			}
		}
