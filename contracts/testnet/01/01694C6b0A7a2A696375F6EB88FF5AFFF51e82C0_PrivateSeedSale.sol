...
contract PrivateSeedSale is Ownable {
    ...
    function setQuillSupply(uint _quillTotalSupply, uint _quillTotalPurchased, uint _quillMinimumPurchase) public onlyOwner {
        ...
        onlyValid(signature)
    {
        ...
        uint scaled = usdValue * sale.rate;
        uint token = scaled / 1e18;
    ...
    { 
        ...
        uint256 scaled = _busdAmount * sale.rate;
        uint token = scaled / 1e18;
        require(token >= sale.minimumPurchase, "Must be grater than minimum purchase");
        require(token <= sale.totalSupply - sale.totalPurchasedPerSale, "Token exceeds total supply" );
        require(tokenPurchased[msg.sender].add(token) <= purchaseLimit, "Individual cap exceeded");
        ...
        external
    {
        ...
        require(_quillAmount >= quillTotalPurchased, "Must be grater than minimum purchase");
