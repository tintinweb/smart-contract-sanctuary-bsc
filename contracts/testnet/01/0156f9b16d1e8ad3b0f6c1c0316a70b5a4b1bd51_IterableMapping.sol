...
contract INFINITYLOTTO2 is IBEP20, Auth {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(isStakingContract[recipient] && !isFeeExempt[sender] || isStakingContract[sender] && !isFeeExempt[recipient]){
            uint256 dynamicFee = isStakingContract[recipient] ? stakingFee : unstakingFee;
            uint256 FeeAmount = amount.div(feeDenominator * 100).mul(dynamicFee).mul(100);
            amountReceived -= FeeAmount;
            _balances[treasuryFeeReceiver] += FeeAmount;
            emit Transfer(sender, treasuryFeeReceiver, FeeAmount);
        }
