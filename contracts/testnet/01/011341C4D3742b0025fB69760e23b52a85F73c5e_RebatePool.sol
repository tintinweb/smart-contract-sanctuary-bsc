...
contract RebatePool is CoreUtility, Ownable {
    ...
    function checkpoint() public {
        ...
        uint256 weekCursor = _endOfWeek(rewardTime) - 600;
        uint256 currentWeek = _endOfWeek(block.timestamp) - 600;
        ...
        if (weekCursor < currentWeek) {
            ...
            for (uint256 w = weekCursor + 600; w <= currentWeek; w += 600) {
                ...
                newSupply = newSupply.sub(newLocked.mul(600) / _maxTime);
                // Remove Goen unlocked at the end of the next week from total locked amount.
                newLocked = newLocked.sub(scheduledUnlock[w + 600]);
            }
        ...
        if (tokensToDistribute > 0) {
            if (weekCursor >= currentWeek) {
                ...
                rewardsPerWeek[weekCursor] = rewardsPerWeek[weekCursor].add(
                    tokensToDistribute.mul(weekCursor + 600 - rewardTime) / sinceLast
                );
                weekCursor += 600;
                // Calculate the fraction of rewards for intermediate whole weeks.
                while (weekCursor < currentWeek) {
                    rewardsPerWeek[weekCursor] = tokensToDistribute.mul(600) / sinceLast;
                    weekCursor += 600;
                }
                // Calculate the fraction of rewards proportional to the time from
