...
contract deployerFair is ReentrancyGuard {
    ...
    function createFairPreSaleTeam(
        address[2] calldata add,
        bool team,
        uint256 [11] memory values
    ) external payable isHuman returns(address preSaleContract) {
        require(msg.value == adminFee,"Launchpad: Admin fee not paid");
        admin.transfer(adminFee-insuranceFee);
        insuranceWallet.transfer(insuranceFee);
        token = IERC20(add[0]);
...
contract PreSaleBnbFairTeam is ReentrancyGuard {
    ...
    function initialize(
        ...
        teamenabled = _team;
        routerAddress = IPancakeRouter02(add[1]);
    }
    ...
    function emergencywithdraw() external{
        ...
        require(!presaleCancelled,"LAUNCHPAD: preSale cancelled");
        require(bnbBalance[msg.sender] > 0,"LAUNCHPAD: No BNB balance to withdraw");
        payable(msg.sender).transfer(bnbBalance[msg.sender]);
        amountRaised = amountRaised.sub(bnbBalance[msg.sender]);
        bnbBalance[msg.sender] = 0;

        emit emergencyWithdraw(msg.sender,bnbBalance[msg.sender]);
    ...
    function internalclaim(address _user) internal {
        ...
        else if(bnbBalance[_user] > 0 && amountRaised >= softCap && !presaleCancelled) {
            require(canClaim == true,"LAUNCHPAD: pool not initialized yet");
            require(bnbBalance[_user] > 0,"LAUNCHPAD: Zero balance");
            uint256 numberOfTokens = getrate(bnbBalance[_user]);
    ...
    function withdrawScamFundsBNB(uint256 _amountBNB) external onlyadminIDO {
        require(!allow,"LAUNCHPAD: Stop preSale First");
        adminIDO.transfer(_amountBNB);
    }
    ...
    function getrate(uint256 ethamount) public view returns (uint256){
        uint256 numberOfTokens = preSaleAmount.mul(ethamount).div(amountRaised);
        return numberOfTokens;
    }
}
