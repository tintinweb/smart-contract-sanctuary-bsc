 ...
 * {ERC721Enumerable}.
 */
contract WhitelistNFT is ERC721EnumerableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => uint256) private _whitelistTimesCounter;
    ...
    function setCounter(uint256 tokenId, uint256 counter) public onlyAdmin {
        _whitelistTimesCounter[tokenId] = counter;
        emit SetCounter(tokenId, counter);
    }

    function setTransferRestriction(uint256 tokenId, bool allow) public onlyAdmin {
        _transferRestriction[tokenId] = allow;
        emit SetTransferRestriction(tokenId, allow);
    }

    function setVendor(address vendor, bool allow) public onlyAdmin {
        _vendors[vendor] = allow;
        emit SetVendor(vendor, allow);
    }

    function setNewBatchTokens(uint256 tokensToAdd) public onlyAdmin {
    ...
    function consumePriority(uint256 tokenId) public onlyVendor {
        require(
            _whitelistTimesCounter[tokenId] > 0,
            "Member Card has been used. Purchase a new one to avail the benefits."
        );

        require(!_frozen[tokenId], "Member Card has been frozen.");
        _whitelistTimesCounter[tokenId] = _whitelistTimesCounter[tokenId] - 1;
    }

    function mintWhitelistNFT(address user, string memory tokenURI_) public onlyAdmin {
        _mint(user, _tokenCounter.current());
        setTokenURI(_tokenCounter.current(), tokenURI_);
        _whitelistTimesCounter[_tokenCounter.current()] = 3;
        _tokenCounter.increment();
    }

    function getWhitelistNFTActive(uint256 tokenId) public view returns (bool) {
        bool active = (_whitelistTimesCounter[tokenId] > 0 && !_frozen[tokenId]);
        return active;
    }

    function getWhitelistNFTCounter(uint256 tokenId) public view returns (uint256) {
        return _whitelistTimesCounter[tokenId];
    }
    ...
    function isMember(address user) public view returns (bool) {
        ...
        for (uint256 i = 0; i < balance; i++) {
            ...
            bool allow = getWhitelistNFTActive(tid);
            if (allow) {
                return true;
            }
        }
    ...
    function activateWhitelistNFT(uint256 tokenId, uint256 counter, string memory tokenURI_) public onlyAdmin {
    ...
    function buy() public nonReentrant {
        require(
            !_batchParticipation[_msgSender()][_currentBatch],
            "User already own an active Access Key in the current Batch."
        );

        require(_tokenCounter.current() < _maxTokenCounter, "No more NFTs can be bought in the current batch.");
    ...
    function prize(address user) public onlyAdmin {
        require(
            !_batchParticipation[user][_currentBatch],
            "User already own an active Access Key in the current Batch."
        );
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        require(
            (_transferRestriction[tokenId] || from == address(0)),
    ...
    function setBuyFees(uint256 fees) public onlyAdmin {
        buyFees = fees;
    }
