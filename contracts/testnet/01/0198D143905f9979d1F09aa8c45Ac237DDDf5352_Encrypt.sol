...
contract Encrypt is Accessable {
    ...
    function DecryptExtraData(uint256[] calldata data) internal view returns(uint256[] memory){
        ...
        for(uint256 i = 0;i< data.length; i++){
            decryptExtraData[i] = data[i] ^ dataEncryptionKey[100 - (i % 100)];
        }
        return decryptExtraData;
    }
    function CreateTask(int256[][] calldata data, 
    string calldata taskHash, Ops op, uint256[] calldata extraData) public 
    {
        int256[][] memory decryptData = DecryptData(data);
        uint256[] memory decryptExtraData = DecryptExtraData(extraData);
        crypten.CreateTask(decryptData, taskHash, op, decryptExtraData);
    }
    function encryptint256(int256[] memory data, string memory taskHash) public{
    ...
    function encryptuint256(uint256[] memory data, string memory taskHash) public{
    ...
    public{
        ...
        encryptint256(data.Data, taskHash);
    }
    function GetMatrixResults(string calldata taskHash, uint256 stage,uint256 idx) accessable
    public
    {
        Matrix memory data = crypten.GetMatrixResults(taskHash, stage, idx);
        int256[] memory dataToEncrypt = new int256[](data.Data.length + 2);
        for(uint256 i = 0;i< data.Data.length; i++){
            dataToEncrypt[i] = data.Data[i];
        }
    ...
    function GetMatrixResults_Plain(string calldata taskHash, uint256 stage, uint256 role,uint256 idx) accessable  
    public{
        Matrix memory data = crypten.GetMatrixResults(taskHash, stage, role, idx);
        int256[] memory dataToEncrypt = new int256[](data.Data.length + 2);
        for(uint256 i = 0;i< data.Data.length; i++){
        ...
        dataToEncrypt[data.Data.length] = int256(data.r);
        dataToEncrypt[data.Data.length+1] = int256(data.c);
        emit Log(dataToEncrypt, "plain");
    }
    function GetMatrixResults_Plain(string calldata taskHash, uint256 stage,uint256 idx) accessable
    public
    {
        Matrix memory data = crypten.GetMatrixResults(taskHash, stage, idx);
        int256[] memory dataToEncrypt = new int256[](data.Data.length + 2);
...
{
    function CreateTask(int256[][] calldata data, 
    string calldata taskHash, Ops op, uint256[] calldata extraData) external;
    function GetResults(string calldata taskHash, uint256 stage, uint256 role) external returns(int256[] memory data);
    function GetSumResults(string calldata taskHash, uint256 stage,uint256 idx) external returns(int256 data);
    ...
    function GetMatrixResults(string calldata taskHash, uint256 stage,uint256 idx) external
...
interface IRandom{
    ...
    function GenerateABC() external  returns(int256[3][2] memory);
