...
contract MasterPlatypusV2 {    
    ...
    */
    ...
    function stakingPTP (uint256 amount) public { 
        ...
        IERC20(PTP).transferFrom(msg.sender, address(this), amount);
    ...
    function unStakingPTP(uint256 amount) public {
        ...
        IERC20(PTP).transfer(msg.sender, amount);
        uint256 rewardAmount = user.rewardAmount + calcIncreasedVePTPReward(user.ptpAmount, user.lastTimestamp);        
        VePTP.transferAsReward(msg.sender, rewardAmount);
        _updatePTPStakedUserInfoForUnStaking(msg.sender, amount);
        emit PTPUnStaked(msg.sender, address(PTP), amount);
    }

    function multiClaimPTP(uint256[] memory pids) external {
        _multiClaimPTP(pids, msg.sender);
    }

    function _multiClaimPTP(uint256[] memory pids, address u) private {
        uint256 rewardAmount;
