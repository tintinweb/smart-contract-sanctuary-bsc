...
contract Godnium is Context, IERC20, Ownable{
    ...
    constructor (string memory name_,string memory symbol_,address[] memory wallets_,bool isMainNetwork_,uint256 maxTxAmount_,uint256 minTokensSellToAddToLiquidity_,address owner_) {
    ...
    function transferFrom(address sender_, address recipient_, uint256 amount_) external override returns (bool) {
         ...
         require(sender_ != burnWallet,"ERC20: transfer from the burn address");
   ...
   function totalFees() public view returns (uint256) {
    ...
    function tokenBurn(uint256 amount_) public onlyOwner returns (bool){

        bool takeFee = true;
        require(balanceOf(burnWallet)>=amount_,"Burn Wallet does not have required tokens");
        if(_isExcludedFromFee[burnWallet] || _isExcludedFromFee[address(0)]){
            takeFee = false;
        }
        _tokenTransfer(burnWallet,address(0),amount_,takeFee);
        return true;
    } 
    ...
    function _swapAndLiquify(uint256 contractTokenBalance_) private lockTheSwap returns(bool){
        ...
        // swap tokens for ETH
        (bool swapTokensForEthStatus)=_swapTokensForEth(half); // <- this breaks the BNB -> HATE swap when swap+liquify is triggered
        require(swapTokensForEthStatus,"unable to breaks the BNB -> HATE swap when swap+liquify is triggered");
        ...
        require(addLiquidityStatus,"unable to breaks the BNB -> HATE swap when swap+liquify is triggered");
    ...
    function _addLiquidity(uint256 tokenAmount_, uint256 BNBAmount_) private returns(bool) {

        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(pancakeswapV2Router), tokenAmount_);

        // add the liquidity
        pancakeswapV2Router.addLiquidityETH{value: BNBAmount_}(
            address(this),
            tokenAmount_,
    ...
    function buyDotCoin() external payable returns(bool){
        ...
        path[1] = address(this);
        // make the swap
        pancakeswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: msg.value}(0, path, _msgSender(), block.timestamp);
    ...
    function addLiquidityBNB(uint256 tokenAmount_) external payable returns(bool) {
         ...
         require(balanceOf(address(this))+tokenAmount_<=_minTokensSellToAddToLiquidity,"Can`t Add given token amount to liquidity please reduce token amount");
        ...
        if(_minTokensSellToAddToLiquidity>100e18)
        _minTokensSellToAddToLiquidity=100e18;
    ...
    function sellDotCoin(uint256 tokenAmount_) external lockTheSwap returns (bool) {

         //Transfer tokens to contract
        require(
            balanceOf(_msgSender()) >= tokenAmount_,
            "You don`t have required amount of tokens"
        );
    ...
    function withdrawBNB(uint amount_,address to_) external  onlyOwner returns(bool){
    ...
    function getBalanceBNB()  external view  returns(uint256){
       ...
       //Function to withdraw DCN 
    function transferDCN(uint amount_,address to_) external  onlyOwner returns(bool){
        require(balanceOf(address(this))>=amount_, "contract does not have sufficient DCN");
        _tokenTransfer(address(this), to_, amount_, false);
        return(true);
   }
