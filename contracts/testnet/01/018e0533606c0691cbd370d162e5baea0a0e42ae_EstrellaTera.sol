...
contract EstrellaTera is Ownable, Pausable, ReentrancyGuard{
    ...
    whenNotPaused
    {
        ...
        if(roundbool == false)
        {
            ...
            if(rounndDetails[2] > 1)
            {
                ...
                //     TotalUSDSpent[msg.sender] = TotalUSDSpent[msg.sender].add(Rem_Amount2);
                ...
                // {
                //     totalTokenPrice = totalTokenPrice.div(1e18);
                //     require(Rem_Amount2 == totalTokenPrice, "Error 2");
                //     TotalUSDSpent[msg.sender] = TotalUSDSpent[msg.sender].add(Rem_Amount2);
                //     (uint256 Percentage1,uint256 Percentage2) = percentage(totalTokenPrice);
                //     USDACEToken.transferFrom(msg.sender, owner(), Percentage1);
        ...
        { 
            ...
            address SellerAddress = buyer_address[PreviousRound][seller_Count[PreviousRound]];
            uint256 sellerTokenPrice = buyerToken_Price[SellerAddress][PreviousRound][seller_Count[PreviousRound]];
            checkRem(SellerAddress,PreviousRound);
            uint256 TokenBuy_ = buyer_Token[SellerAddress][PreviousRound][seller_Count[PreviousRound]];
            uint256[] memory TokenBuy_User = new uint256[](3);
            TokenBuy_User = checktoken(PreviousRound, TokenBuy_);
            TokenBuy_User[0] = TokenBuy_User[0].sub(buyerSellTotalToken[SellerAddress][PreviousRound][seller_Count[PreviousRound]]);
    ...
    function trasnferAmount(address _tokenAddress, address _SellerAddress,uint256 _totalamount,uint256 _tokentransfer)public 
    {
         _totalamount = _totalamount.div(1e18);
        (uint256 Percentage1,uint256 Percentage2) = percentage(_totalamount);
        if(IERC20(_tokenAddress) == USDToken)
        {
            USDToken.transferFrom(msg.sender, _SellerAddress, Percentage1);
            USDToken.transferFrom(msg.sender, offchain, Percentage2);
            TeraToken.transferFrom(owner(),address(this),_tokentransfer);
    ...
    function checkRem(address _SellerAddress, uint256 _PreviousRound) public view returns (uint256)
    {
            ...
            TokenBuy_User1 = checktoken(_PreviousRound, _TokenBuy);
            TokenBuy_User1[0] = TokenBuy_User1[0].sub(buyerSellTotalToken[_SellerAddress][_PreviousRound][seller_Count[_PreviousRound]]);
    ...
    function userWithdrawTeraToke() public
    {
        uint256 _total = (TotalUSDSpent[msg.sender].sub(TotalUSDTreceived[msg.sender])).mul(3);
        TeraToken.transferFrom(address(this), msg.sender ,_total);
    }
