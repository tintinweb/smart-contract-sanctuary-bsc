...
contract AFIFarmB is Ownable, ReentrancyGuard {
    ...
    constructor() public {
        AFI = 0xEfE1398573dB948F63E22FD7F6E31fA8e5fd480c;
    ...
    function updatePool(uint256 _pid) public {
        ...
        AFIToken(AFIB).mint(address(this), AFIReward);
    ...
    function safeAFITransfer(address _to, uint256 _AFIAmt) internal {
        uint256 AFIBal = IERC20(AFIB).balanceOf(address(this));
        if (_AFIAmt > AFIBal) {
            IERC20(AFIB).transfer(_to, AFIBal);
        } else {
            IERC20(AFIB).transfer(_to, _AFIAmt);
        }
    }

    function inCaseTokensGetStuck(address _token, uint256 _amount) public onlyOwner
    {
        require(_token != AFIB, "!safe");
    ...
    function migrateToAFIB(uint256 _inputAmt) public {
        ...
        AFIToken(AFIB).mint(msg.sender, _inputAmt);
    }
