...
contract GenesisFarmBNB is Ownable {
	...
	function getLeaderboard() external view returns (address[10] memory addresses, uint256[10] memory values){
		for(uint8 i = 0; i < 10; i++){
			addresses[i] = topReferrers[getCurrentWeek()][i].addr;
			values[i] = topReferrers[getCurrentWeek()][i].amount;
		}
