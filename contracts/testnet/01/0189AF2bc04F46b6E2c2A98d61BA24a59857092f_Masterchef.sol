...
// File: contracts\Venera.sol

pragma solidity 0.6.12;


contract Venera is BEP20('Vsw Token', 'ISW') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "VswSwap::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "VswSwap::delegateBySig: invalid nonce");
        require(now <= expiry, "VswSwap::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "VswSwap::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "VswSwap::_writeCheckpoint: block number exceeds 32 bits");
...
contract Masterchef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accVswPerShare;   // Accumulated ORANGEs per share, times 1e12. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        bool emergencyWithdrawnable; // enable by the owner
        IBEP20 rewardToken;   //rewardToken
    }
    ...
    uint256 public VswPerBlock;
    // Bonus muliplier for early Vsw makers.
    ...
    mapping(address => uint256) public totalReferralCommissions; // referrer address => total referral commissions
     ...
     uint256 public stakeAmountLv1 = 100*10**18;    // Minimum stake VSW token condition level1 for referral program.
    uint256 public stakeAmountLv2 = 1000*10**18;    // Minimum stake VSW token condition level2 for referral program.
    ...
    bool public canDistributeReward = true;
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _VswPerBlock,
        uint256 _startBlock
    ) public {
        vsw = _vsw;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        VswPerBlock = _VswPerBlock;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate,IBEP20 _rewardToken) public onlyOwner {
    ...
    function toggleDistribute(bool _value) public onlyOwner{
        canDistributeReward = _value;
    }

    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate, IBEP20 _rewardToken) public onlyOwner {
    ...
    function pendingVsw(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accVswPerShare = pool.accVswPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 vswReward = multiplier.mul(VswPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accVswPerShare = accVswPerShare.add(vswReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accVswPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function setLevelPercent(uint256 _level1, uint256 _level2) external onlyOwner {
        percentForReferLv1 = _level1;
        percentForReferLv2 = _level2;
    }

    function setstakeAmount(uint256 _stakeAmountLv1, uint256 _stakeAmountLv2) external onlyOwner{
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 vswReward = multiplier.mul(VswPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        if(isMinted){
        vsw.mint(devaddr,vswReward.div(10));
        vsw.mint(address(this),vswReward);
        }
       if(vswReward > 0)
        safeVswTransfer(devaddr,vswReward.div(10),_pid);
        pool.accVswPerShare = pool.accVswPerShare.add(vswReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function setMinted(bool _value)external onlyOwner{
        isMinted = _value;
    }
    ...
    function harvest(uint256 _pid) internal {
          PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
          if (canDistributeReward && user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accVswPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
                ...
                safeVswTransfer(msg.sender, amount,_pid);
            }
            user.rewardDebt = user.amount.mul(pool.accVswPerShare).div(1e12);
        }
     ...
     function _transferReferral(uint256 _pid, uint256 _referAmountLv1, uint256 _referAmountLv2) internal {
        ...
        if (referrerLv1 != address(0)) {
            uint256 lpStaked = vsw.balanceOf(referrerLv1);
            if (lpStaked >= stakeAmountLv1) {
                safeVswTransfer(referrerLv1,  _referAmountLv1,_pid);
            ...
            uint256 lpStaked2 = vsw.balanceOf(referrerLv2);
            if (referrerLv2 != address(0) && lpStaked2 >= stakeAmountLv2) {
                safeVswTransfer(referrerLv2, _referAmountLv2,_pid);
        ...
        if (referAmountForDev > 0) {
           safeVswTransfer(devaddr, referAmountForDev,_pid);
        }
    }
    ...
    // Safe isal transfer function, just in case if rounding error causes pool to not have enough ISWs.
    function safeVswTransfer(address _to, uint256 _amount,uint256 _pid) internal {
        PoolInfo memory pool = poolInfo[_pid];
        uint256 isalBal = pool.rewardToken.balanceOf(address(this));
         if(pool.rewardToken.decimals() != pool.lpToken.decimals()){
         _amount = getEqualientToken(pool.lpToken.decimals(),pool.rewardToken.decimals(),_amount);
         }
        if (_amount > isalBal) {
            pool.rewardToken.transfer(_to, isalBal);
        } else {
    ...
    function updateEmissionRate(uint256 _VswPerBlock) public onlyOwner {
        massUpdatePools();
        VswPerBlock = _VswPerBlock;
    }
}