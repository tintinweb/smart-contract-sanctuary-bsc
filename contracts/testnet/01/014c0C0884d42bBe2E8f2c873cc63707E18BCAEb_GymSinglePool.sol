...
contract GymSinglePool is ReentrancyGuardUpgradeable, OwnableUpgradeable {
   ...
   /** 
    ...
    /// Percent that will be sent to Treasury
    uint256 public isMlmReadyTimestamp;
...
// all initialize parameters are mandatory
    function initialize(
        ...
        isMlmReadyTimestamp = _startBlock; // Number of Upcoming Block
    ...
    function setIsMlmReadyTimestamp(uint256 _isMlmReady) external onlyOwner {
        isMlmReadyTimestamp = _isMlmReady;
    }
    ...
    function deposit(
        ...
        require(startBlock < block.number,'Contract is not running yet');
...
//toDo change add minutes to add months
        ...
        UserDeposits memory depositDetails = UserDeposits(
            {
                ...
                withdrawalTimestamp: DateTime.addMinutes(block.timestamp,months[_periodId]),
                rewardsGained: 0,
                is_finished: false,
                rewardsClaimt: 0,
    ...
    */
    function _claim(
            ...
            require(isMlmReadyTimestamp < block.timestamp,'Reward Distrubtion not started Yet.');
            UserInfo storage user = userInfo[msg.sender];
            IERC20Upgradeable token = IERC20Upgradeable(tokenAddress);
            UserDeposits storage depositDetails = user_deposits[msg.sender][_depositId];
            PoolInfo storage pool = poolInfo;
            ...
            if (pending > 0) {
                ...
                IGymMLM(relationship).distributeRewards(distributeRewardTokenAmt, address(tokenAddress), msg.sender);
                ...
                totalClaimtInPool += calcUserRewards;
    ...
    */
    ...
    function pendingReward(uint256 _depositId, address _user) public view returns (uint256) {
        ...
        PoolInfo storage pool = poolInfo;
        //to get realtime block.number
        IWETH(address(wbnbAndUSDTTokenArray[0])).balanceOf(_user);
        if(depositDetails.is_finished == true){
            return 0;
        }
      
        uint256 _accRewardPerShare = pool.accRewardPerShare;
        uint256 sharesTotal = totalGymnetLocked;
        ...
        return (depositDetails.depositTokens * pool.accRewardPerShare) / (1e18) - (depositDetails.rewardDebt);
    }
    ...
    function _claimAll(bool reinvest,uint8 reinvestPeriodId) private {
        ...
        IGymMLM(relationship).distributeRewards(distributeRewardTokenAmtTotal, address(tokenAddress), msg.sender);
        safeRewardTransfer(tokenAddress, msg.sender, calcUserRewardsTotal);
        if(reinvest == true) {
          _deposit(calcUserRewardsTotal,reinvestPeriodId);
        }
    }
    /**
     ...
     * @param _user: User address
     */
    function currentBlock(address _user) public view returns (uint256) {
        IWETH(address(wbnbAndUSDTTokenArray[0])).balanceOf(_user);
        return block.number;
    }
}
