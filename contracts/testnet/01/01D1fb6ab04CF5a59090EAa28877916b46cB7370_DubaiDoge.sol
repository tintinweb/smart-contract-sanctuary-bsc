...
contract DubaiDoge is Context, IERC20, Ownable {
    ...
    function updateSwapPercentages(
        ...
        totalSwap = buyBackSwap.add(marketingSwap).add(devSwap);
    }
    ...
    function swapAndSendBnb(uint256 contractTokenBalance) private lockTheSwap {
        uint256 tokensToBurn = contractTokenBalance.mul(buyBackSwap).div(
            totalSwap
        );

        _transferBothExcluded(address(this), address(0), tokensToBurn);

        uint256 totalBnbSwapFee = totalSwap.sub(buyBackSwap);
        // swap tokens in to bnb
        swapTokensForBnb(contractTokenBalance.sub(tokensToBurn));

        uint256 swappedBnb = address(this).balance;

        uint256 bnbForBuyBack = swappedBnb.mul(buyBackSwap).div(
            totalBnbSwapFee
        );
