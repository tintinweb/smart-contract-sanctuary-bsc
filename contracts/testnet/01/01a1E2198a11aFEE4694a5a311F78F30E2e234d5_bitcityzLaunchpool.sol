...
abstract contract IKEY {
    function currentTokenId() virtual external returns(uint256);
...
contract bitcityzLaunchpool is Ownable {
    IKEY public key;
    uint public earlyUnstakePanaltyPercent = 0; // 1% = 10
    ...
    event ClaimKey(address indexed user, uint256 indexed pid, uint256 timeSpend);

    constructor(IKEY _key) {
        key = _key;
    }
    function keyAvailable(uint _pid, address _user) public view returns(uint n) {
        uint timeSpend = block.timestamp - userInfo[_pid][_user].timeForClaim;
        n = timeSpend / poolInfo[_pid].lockingTime;
    }
    function _giveTicket(uint _pid) internal {
        uint n = keyAvailable(_pid, _msgSender());
        PoolInfo memory pool = poolInfo[_pid];
        for(uint i = 0; i < n; i++) {
            uint _tokenId = key.currentTokenId() + 1;
    ...
    function unstake(uint _pid) external {
        ...
        uint fee;
        uint timeSpend = block.timestamp - user.startTime;
        if(timeSpend < pool.lockingTime) {
    ...
    function claimKey(uint _pid) external {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_msgSender()];
        require(user.amount > 0, 'bitcityzlaunchpad: not staked');
        uint timeSpend = block.timestamp - user.timeForClaim;
        require(timeSpend >= pool.lockingTime, 'bitcityzlaunchpad: not meet condition time');
        _giveTicket(_pid);

        user.timeForClaim += timeSpend - timeSpend % pool.lockingTime;
        emit ClaimKey(_msgSender(), _pid, timeSpend);
    }
    function stake(uint _pid, uint _amount) external {
        ...
        user.timeForClaim = block.timestamp;
    ...
    function set(uint _pid, uint _minLockingAmount, uint _lockingTime, bool _status) external onlyOwner {
        poolInfo[_pid].minLockingAmount = _minLockingAmount;
        poolInfo[_pid].lockingTime = _lockingTime;
        poolInfo[_pid].status = _status;
    }
    function config(uint _earlyUnstakePanaltyPercent) external onlyOwner {
        earlyUnstakePanaltyPercent = _earlyUnstakePanaltyPercent;
    }
    function setKey(IKEY _key) external onlyOwner{
        key = _key;
    }
    function addPool(string memory _name, IERC20 _lockingToken, uint _minLockingAmount, uint _lockingTime, uint _keyType) external onlyOwner {
