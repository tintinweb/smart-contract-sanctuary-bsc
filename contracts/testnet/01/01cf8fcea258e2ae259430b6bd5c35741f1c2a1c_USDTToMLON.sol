...
contract USDTToMLON is ReentrancyGuard, Ownable{
   using SafeMath for uint256; 
    ...
    uint256 public mlonSupply;

    uint256 public  usdtSupply;

    constructor(address _mlon, address _usdt) public{
        mlon = IBEP20(_mlon);
        usdt = IBEP20(_usdt);
    }
    ...
    function currentPrice() public view returns(uint256){
        return mlonSupply.div(usdtSupply);
    }

    function swapUSDToMLON(uint256 _amountIn, uint256 _amountOutMin) public nonReentrant{ 
        require(usdt.balanceOf(msg.sender) >= _amountIn, "USDTToMLON: insufficient usdt");
        require(mlon.balanceOf(address(this)) >= _amountOutMin, "USDTToMLON: insufficient mlon");
        uint256 amountOut = _getAmountOut(_amountIn);
        require(amountOut >= _amountOutMin, "USDTToMLON: slippage too low");
        mlon.transfer(msg.sender, amountOut);
    ...
    function getAmountOut(uint256 _amountIn) public view returns(uint256 amountOut){
        uint256 uSupply = usdtSupply;
        uSupply = uSupply.add(_amountIn);
        amountOut = mlonSupply.sub((k.div(uSupply)));
    }

    function _getAmountOut(uint256 _amountIn) internal returns(uint256 amountOut){

        usdtSupply = usdtSupply.add(_amountIn);
        amountOut = mlonSupply.sub((k.div(usdtSupply)));
        mlonSupply = mlonSupply.sub((k.div(usdtSupply)));
        return amountOut;
    }

    function addLiquiduity(uint256 _usdtAmount, uint256 _mlonAmount) public onlyOwner{
        require(usdt.balanceOf(msg.sender) >= _usdtAmount, "USDTToMLON: insufficient usdt");
        require(mlon.balanceOf(msg.sender) >= _mlonAmount, "USDTToMLON: insufficient mlon");
    ...
    function removeLiquiduity(uint256 _usdtAmount, uint256 _mlonAmount) public onlyOwner{
        require(usdt.balanceOf(address(this)) >= _usdtAmount, "USDTToMLON: insufficient usdt");
        require(mlon.balanceOf(address(this)) >= _mlonAmount, "USDTToMLON: insufficient mlon");

        mlonSupply = mlonSupply.sub(_mlonAmount);
        usdtSupply = usdtSupply.sub(_usdtAmount);
