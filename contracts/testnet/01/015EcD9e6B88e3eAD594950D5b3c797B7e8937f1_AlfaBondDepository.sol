...
contract AlfaBondDepository is IBondDepository, NoteKeeper {
        ...
        )
    {
        ...
        treasury.manageReserves(address(market.quoteToken), _amount);
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
    ...
    function updateTreasury() external {
        require(
            msg.sender == authority.governor() || msg.sender == authority.guardian() || msg.sender == authority.policy(),
            'Only authorized'
        );
    ...
    function addNote(
        ...
        notes[_user].push(
            Note({payout: gAFA.balanceTo(_payout), created: uint48(block.timestamp), matured: _expiry, redeemed: 0, marketID: _marketID})
        );
