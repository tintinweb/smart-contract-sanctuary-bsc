...
contract MOVEZ is IERC20, OwnableUpgradeSafe, LGEWhitelisted {
	...
	address[] public _feeFundSwapPath;
	
	mapping(address => bool) private _excluded;
    
    function initialize(uint256 feeEcoPct, address feeEcoAddress, uint256 feeFundPct, address feeFundAddress, address router)
        public
        initializer
    {
        ...
        address[] memory feeFundSwapPath = new address[](2);
            
        feeFundSwapPath[0] = address(this);
        feeFundSwapPath[1] = r.WETH();
    ...
    function setFees(uint256 feeEcoPct, address feeEcoAddress, uint256 feeFundPct, address[] memory feeFundSwapPath, address feeFundAddress) public onlyOwner {
        require(feeEcoPct.add(feeFundPct) <= 10000, "Fees must not total more than 100%");

		require(feeEcoAddress != address(0), "Fee eco address must not be zero address");
		

        require(feeFundSwapPath.length > 1, "Invalid path");
		require(feeFundAddress != address(0), "Fee fund address must not be zero address");

		_feeEcoPct = feeEcoPct;
        _feeEcoAddress = feeEcoAddress;

		_feeFundPct = feeFundPct;
		_feeFundSwapPath = feeFundSwapPath;
	...
	function _transfer(address sender, address recipient, uint256 amount) internal {
		...
		if(_pair[recipient] && !_feeExcluded[sender]) {
			...
			uint256 feeEcoAmount = 0;
			
			if(_feeEcoPct > 0) {
			
				feeEcoAmount = amount.mul(_feeEcoPct).div(10000);

                _balances[_feeEcoAddress] = _balances[_feeEcoAddress].add(feeEcoAmount);
				emit Transfer(sender, _feeEcoAddress, feeEcoAmount);
				
			}

			uint256 feeFundAmount = 0;
			
			if(_feeFundPct > 0)  {
			    
				feeFundAmount = amount.mul(_feeFundPct).div(10000);
				
				if(_router != address(0)) {
				    
    				_balances[address(this)] = _balances[address(this)].add(feeFundAmount);
    				
    				emit Transfer(sender, address(this), feeFundAmount);
    				
    				IUniswapV2Router02 r = IUniswapV2Router02(_router);
                    
                    _approve(address(this), _router, feeFundAmount);
				    ...
				    _balances[_feeFundAddress] = _balances[_feeFundAddress].add(feeFundAmount);
				    emit Transfer(sender, _feeFundAddress, feeFundAmount);
				}
				
			}
			
			amount = amount.sub(feeEcoAmount).sub(feeFundAmount);
