    ...
    function getSlots(address userAddr) public view verified returns(uint[] memory slots, uint[] memory partnerBonuses, uint[] memory poolProfits, uint[] memory missedProfits, uint contractState) {
        ...
        for (uint level = 0; level < LEVELS_COUNT; level++) {
            if (block.timestamp < initialDate + levelIntervals[level]) {
                slotList[level] = 10010; // Not availabled yet (user need to wait some time once it bacome available)
                continue;
            }
            ...
            if (level > users[userAddr].maxLevel) {
                slotList[level] = 10020; // Not allowed yet (previous level is not opened)
                continue;
            }
            ...
            if (users[userAddr].levels[level].openState != 2) {
                if (users[userAddr].levels[level].openState == 0)
                    slotList[level] = 10030; // Available for opening
                else
                    slotList[level] = 10040; // Available for reopening
            ...
            for (uint i = headIndex[level]; i < levelQueue[level].length; i++) {
                if (levelQueue[level][i] == userAddr) {
                    uint n = levelQueue[level].length - headIndex[level] - 1;
                    slotList[level] = (n - place) * 1000 / n; // Slot is opened
                    break;
                }
                place++;
            }
