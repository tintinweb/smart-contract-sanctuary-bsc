...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
    ...
    address public RWRD = address(0x757dBF1c18aE85b193F9cBb8E6875cf41E9F14C5);
...
contract BERMUDA is IERC20Metadata, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    string constant private _name = "BERMUDA";
    string constant private _symbol = "BMD";
    ...
    mapping (address => bool) public isBermudaTriangle;
    ...
    BermudaDividendTracker public dividendTracker;
    uint256 public gasForProcessing = 300000;
    address public RWRD = address(0x757dBF1c18aE85b193F9cBb8E6875cf41E9F14C5); // Reward Token
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY = 100_000 * (10**_decimals);
    uint256 private swapThreshold = rSupply / 5000;
    uint256 private maxTokensToSell = rSupply / 1000;
    ...
    constructor () {
        ...
        dividendTracker = new BermudaDividendTracker();
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Dividend tracker
        try dividendTracker.setBalance(payable(sender), setBalanceOf(sender)) {} catch {}
        try dividendTracker.setBalance(payable(recipient), setBalanceOf(recipient)) {} catch {}

        if(!inSwap && rewardsEnabled && !isBermudaTriangle[sender] && !isBermudaTriangle[recipient]) {
    ...
    function setBalanceOf(address _of) internal view returns (uint256) {
        return _rBalance[_of] / (10**48);
    }
    ...
    function shouldSwapBack(address sender, address recipient) internal view returns (bool) {
        return sender != pair
        && !isBermudaTriangle[sender]
        && !isBermudaTriangle[recipient]
        && !inSwap
        && swapEnabled
    ...
    function manualSwapAndDistribute() public onlyOwner {
        swapBack();
        if(!inSwap && rewardsEnabled) {
	    	uint256 gas = gasForProcessing;
    ...
    function swapBack() internal swapping {
        ...
        if (tokensToSell > maxTokensToSell) {
            tokensToSell = maxTokensToSell;
        }
        ...
        if(amountBNBReflection > 0 && rewardsEnabled) {
    ...
    function excludeTriangles (address[] calldata accounts, bool isTriangle) external onlyOwner {
        for(uint256 i = 0; i < accounts.length; i++) {
            isFeeExempt[accounts[i]] = isTriangle;
            isMaxWalletExempt[accounts[i]] = isTriangle;
            isBermudaTriangle[accounts[i]] = isTriangle;
            dividendTracker.excludeFromDividends(accounts[i]);
        }
    }

    function setIsTriangle (address account, bool isTriangle) external onlyOwner {
        isBermudaTriangle[account] = isTriangle;
    }
    ...
    function setFees(uint256 _liquidityFee, uint256 _reflectionFee, uint256 _treasuryFee, uint256 _insuranceFee, uint256 _burnFee, uint256 _additionalFeeOnSell) external onlyOwner {
        ...
        require((totalFee.add(_additionalFeeOnSell)) <= 15, "Fees cannot be more than 15%");
    }
    ...
    function setSwapBackSettings(bool _enabled, uint256 _swapThresholdPerc, uint256 _maxTokensToSellPerc) external onlyOwner {
        swapEnabled = _enabled;
        swapThreshold = rSupply.div(100000).mul(_swapThresholdPerc);
        maxTokensToSell = rSupply.div(100000).mul(_maxTokensToSellPerc);
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        ...
        BermudaDividendTracker newDividendTracker = BermudaDividendTracker(payable(newAddress));
...
contract BermudaDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("Bermuda Reward Tracker", "BermudaDividendTracker") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 20_000_000 * (10**18);
    }
