...
contract LuckyDraw is VRFConsumerBaseV2, ConfirmedOwner, ReentrancyGuard {
    ...
    uint life = 30 * 24 * 3600;

    function setLife(uint _life) external onlyOwner{
        life = (_life);
    }
    ...
    function createLottery(
        ...
        require(winnerRatios.length==winners && winners > 0, "invalid params");
        IERC20 token = IERC20(tokenAddr);
        require(token.totalSupply() > 0, "invalid token");
        uint ratioSum;
        for(uint i;i<winners;i++){
    ...
    function prize(uint256 lotId)
        public
        view
        returns (uint256)
    {
        Lottery storage lot = lotteries[lotId];
        if(lot.indexedTickets[msg.sender].length==0)
            return 0;
    ...
    function withdraw(uint lotId) public onlyOwner nonReentrant{
        Lottery storage lot = lotteries[lotId];
