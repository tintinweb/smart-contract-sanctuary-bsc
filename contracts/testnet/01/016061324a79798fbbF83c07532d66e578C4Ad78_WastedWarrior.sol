    ...
    AcceptedToken
{
    ...
    modifier onlyWarriorOwner(uint256 warriorId) {
        require(ownerOf(warriorId) == msg.sender, "invalid");
        _;
    }

    modifier onlyNotBlacklist(uint256 warriorId) {
        require(blacklist[warriorId] == false, "blacklisted");
        _;
    }
    ...
    function addWarriorToBlacklist(uint256 warriorId) external onlyOwner {
        require(warriorId < _warriors.length, "WAW: invalid");
    ...
    function removeWarriorFromBlacklist(uint256 warriorId) external onlyOwner {
        require(warriorId < _warriors.length);
    ...
    function getWarriorListing(uint256 warriorId)
        ...
        returns (uint256)
    {
        return warriorsOnSale[warriorId];
    }
    ...
    function _removeWastedEquipment(uint256 warriorId, uint256[] memory itemIds)
        private
    {
        require(warriorsOnSale[warriorId] == 0, "WAW: on sale");
    ...
    function _setWastedEquipment(uint256 warriorId, uint256[] memory itemIds)
        private
    {
        require(warriorsOnSale[warriorId] == 0);
        ...
        for (uint256 i = 0; i < itemIds.length; i++) {
            ...
            require(wastedEquipment.ownerOf(itemId) == msg.sender);

            if (itemType == IWastedEquipment.ItemType.WEAPON) {
                require(warrior.weapon == 0);
                warrior.weapon = itemId;
                itemSet[uint256(IWastedEquipment.ItemType.WEAPON)] = true;
            } else if (itemType == IWastedEquipment.ItemType.ARMOR) {
                require(warrior.armor == 0);
                warrior.armor = itemId;
                itemSet[uint256(IWastedEquipment.ItemType.ARMOR)] = true;
            } else if (itemType == IWastedEquipment.ItemType.ACCESSORY) {
                require(warrior.accessory == 0);
    ...
    function buy(
        ...
        address buyer
    ) external payable override onlyNotBlacklist(warriorId) nonReentrant {
        // require(msg.sender == 0xEd4697102a48e1e4644F899b0c2449B698333Ee7); // operator to buy
        uint256 price = warriorsOnSale[warriorId];
        address seller = ownerOf(warriorId);

        require(buyer != seller);
        require(price == expectedPrice);
        require(price > 0, "WAW: not sale");
    ...
    function adoptPet(uint256 warriorId, uint256 petId)
        ...
        onlyWarriorOwner(warriorId)
    {
        require(wastedPet.ownerOf(petId) == msg.sender);
        require(_warriorsWithPet[warriorId] == 0);
    ...
    {
        require(_validateStr(replaceName));
        require(usedNames[replaceName] == false);
    ...
    function mintFor(
        ...
        require(rarityPackage >= 1 && rarityPackage <= 4);
    ...
    function breedingWarrior(
        ...
        external
        override
        onlyNotBlacklist(fatherId)
        onlyNotBlacklist(motherId)
        onlyOperator
    {
    ...
    function fusionWarrior(
        ...
        address owner,
        uint256[] memory lockParts
    )
        external
        override
        onlyNotBlacklist(firstWarriorId)
        onlyNotBlacklist(secondWarriorId)
        onlyOperator
    {
        ...
        Warrior memory firstWarrior = _warriors[firstWarriorId];
        Warrior memory secondWarrior = _warriors[secondWarriorId];
        require(
            firstWarrior.weapon == 0 &&
                firstWarrior.armor == 0 &&
                firstWarrior.accessory == 0 &&
                secondWarrior.weapon == 0 &&
                secondWarrior.armor == 0 &&
                secondWarrior.accessory == 0
        );
        require(
            _warriorsWithPet[firstWarriorId] == 0 &&
                _warriorsWithPet[secondWarriorId] == 0
        );

        _burn(secondWarriorId);

        emit FusionWarrior(firstWarriorId, secondWarriorId, owner, lockParts);
    }
...
contract AcceptedToken is Initializable, PermissionGroup {
    ...
    modifier collectTokenAsFee(uint256 amount, address destAddress) {
        require(acceptedToken.balanceOf(msg.sender) >= amount, "insufficient");
        _;
