...
constructor(
        ...
        uint256 _numberOfRandomADDRToPick) 
    {
        ...
        tokenData(bidToken,_NFTContractAddress, URIRequired);
    }
    ...
    function getUri(uint256 pid, uint256 tokenOwned) public view returns(bool WIHave, string memory tok, string memory tokS) {
        ...
        if (keccak256(abi.encodePacked(tokenSet)) == keccak256(abi.encodePacked(token))) {
            return (true, token, tokenSet);
        }
        return (false, token, tokenSet);
    }
    ...
    function fundTopBidders(uint256 pid) internal {
        ...
        tokenInfo memory info = request_token_info[pid]; 
        (address FirstTBidder, address secondTBidder, address thirdTBidder) = Top3Bidders(pid);
        (uint256 positionOne, uint256 positionTwo, uint256 positionThree, , uint256 devShare,  ) = position(pid);        
        // bid.totalBidding = bid.totalBidding.sub((positionOne.add(positionTwo.add(positionThree).add(devShare)));
        bid.totalBidding = bid.totalBidding - (positionOne + positionTwo + positionThree +  devShare);
