...
// ArtemisX
// The next safe mission to the moon
// Any bots or other suspicious activity on launch will be blacklisted
...
contract ArtemisX is ERC20Detailed, Ownable {
    ...
    uint256 public autofirePitFee = 15;
    ...
    constructor(
        ...
        address _busd
    ) ERC20Detailed("ArtemisX", "ArtemisX", uint8(DECIMALS)) Ownable() {
        router = IPancakeSwapRouter(_router); 
    ...
    function _transferFrom(
        ...
        if(_maxBalance != 0 && !isMaxExempt[recipient]){
            require(balanceOf(recipient) <= _maxBalance, "Max Balance exceeded");
        }
    ...
    function setMaxBalance(uint256 newMaxBalance_) external onlyOwner {
        _maxBalance = newMaxBalance_;
    }
    ...
    function setMaxExempt(address _addr, bool state) external onlyOwner {
        isMaxExempt[_addr] = state;
    }

    function setContractBotBlacklist(address _botAddress, bool _flag) external onlyOwner {
        require(isContract(_botAddress), "Only contract addresses allowed");
        blacklist[_botAddress] = _flag;    
    }

    function setLaunchBotterBlacklist(address _botAddress, bool _flag) external onlyOwner {
        require(_totalSupply < 32700000000, "Can only be used for a few hours after launch");
        blacklist[_botAddress] = _flag;    
    }

    function setSellFee(uint256 _newFee) external onlyOwner {
        require(_newFee < 90, "Additional sell tax cannot be set higher than 9%");
        sellFee = _newFee;    
    }
