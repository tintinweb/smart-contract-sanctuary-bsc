...
contract KhephrenNFTStaking is IKhephrenNFTStaking {
    ...
    */
    ...
    function setPlatformPrice(uint256 _price) external onlyOwner(msg.sender) {
        price = _price;
    }
...
contract Project721 is IProject721 {
    ...
    */
    ...
    function createPool721(uint256[] memory _dates, uint16[] memory _data)
        external
        virtual
        override
        returns (address)
    {
        ...
        Pool721(_clone).setPoolDetails(_dates, _data, nftAddress, msg.sender);
...
interface IProject721{
    function setProjectDetails(address _nftAddress, bytes[14] memory _projectdetails, address _project721Owner) external;
...
contract Pool721 is IERC721Receiver, IPool721, ReentrancyGuard{
    ...
    uint8 private tokenRewardType = 0;
    ...
    */
    ...
    function addTokenRewards(
        ...
        uint8 _tokenRewardType
    ) external onlyPoolOwner(msg.sender) {
        require(
            isToken == 0 || isToken == 1,
            "POOL721: NFT reward is already active"
        );
        if (isToken == 1) {
            require(
                rewardAddress == _rewardAddress,
                "POOL721: Reward address is not match with current reward Address"
            );
        }
        require(
    ...
    function addNftRewards(uint256 _tokenId)
        external
        onlyPoolOwner(msg.sender)
    {
        require(
            isToken == 0 || isToken == 2,
            "POOL721: Token reward is already active"
        );
    ...
    function getPoolDetails() external view returns (poolDetails721 memory, address, uint8, uint256, uint8, uint256, address) {
        return (mapPoolDetails721[address(this)], rewardAddress, 
        isfinalize, rewardAmount, tokenRewardType, taxAmount, address(this));
    }
