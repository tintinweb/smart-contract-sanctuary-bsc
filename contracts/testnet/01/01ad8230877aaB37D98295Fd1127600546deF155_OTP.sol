...
contract OTP is Context, IBEP20, Ownable {
    ...
    string private _name = "Octopus controller";
    string private _symbol = "OTP";
    uint8 private _decimals = 18;
    

    uint256 public _MarketingFee = 6;
    address public MarketingWallet = 0xfE5D92dd5a63Bf238Ce8e4396FfAf648de69F19b;
    uint256 private _previousMarketingFee = _MarketingFee;

    uint256 public _charityFee = 4;
    address public charityWallet = 0x7e0022Eac1E259FeF4E0E00ccBF0D0FcB644e5A1;
    ...
    function removeAllFee() private {
        if(_charityFee==0 && _MarketingFee==0 && _devFee==0) return;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //Calculate Marketing amount , dev amount and charity amount
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(MarketingAmt).sub(DevAmt).sub(charityAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(MarketingAmt).sub(DevAmt).sub(charityAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(MarketingAmt).sub(DevAmt).sub(charityAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(MarketingAmt).sub(DevAmt).sub(charityAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(MarketingAmt).sub(DevAmt).sub(charityAmt)));
        }
    ...
    function setMarketingWallet(address newMWallet) external onlyOwner() {
