...
// File: contracts/Utopia/UtopiaToken.sol
...
contract UtopiaToken is Ownable, IERC20 {
    ...
    address manager;
    bool public clearUser;//开关清理分红用户
    uint256 public clearUserCount;//清理用户计数
    uint256 public inspectUserNum = 5;//清理检查用户个数
    ...
    function init() private onlyOwner {
        ...
        _token_trusteeship = address(0x85C4001179A2B614978e9949894817441A26c31a);
        _dao_reward= address(0x47A29dFAC86A03f25bbC7B5A8D402D002C091392);
    ...
    function clearBuyUser() public {
        if(clearUser){
            ...
            uint256 userLen = clearUserCount+inspectUserNum < buyUserLen ? clearUserCount+inspectUserNum : buyUserLen;

            for(uint256 i = clearUserCount; i < userLen; i++){
                if(i > buyUser.length){
                    break;
                }
    ...
    function totalSupply() public view returns (uint256) {
        ...
        if (address(utopiaNFT) != address(0)) {
            totalRewardFromNft = utopiaNFT.totalRewards();
        }
    ...
    function balanceOf(address account) public view returns (uint256) {
        ...
        if (address(utopiaNFT) != address(0))
            balance += utopiaNFT.currentReward(account);
        return balance;
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (address(utopiaNFT) == address(0))
            return;
        utopiaNFT.claimRewards(from);
    }

    function refreshTotalSupply() public returns (uint256) {
        if (address(utopiaNFT) == address(0))
            return 0;
        require(msg.sender == address(utopiaNFT), "[METoken] This function can be called only from reward NFT!");
        _totalSupply += utopiaNFT.totalRewards();
        return _totalSupply;
    }
    ...
    function setUtopiaNFT(address utopiaNFTAddr) public onlyOwner {
        utopiaNFT = UtopiaNFT(utopiaNFTAddr);
    }
    ...
    function refreshBalance(address account) public onlyNFT {
        uint256 amount = utopiaNFT.rewardBalance(account);
