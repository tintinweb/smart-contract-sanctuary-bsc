...
contract Saviour is ERC721A, Ownable {
    ...
    using   Strings for uint256;
    string  private unRevealedURL="ipfs://bafybeiaagn4byhow7alhrsmo4evkics5gj3j2mfu7pdynieqevf3lxjekq";
    ...
    uint    public publicSaleCost = 2 ether;
    uint    public preSaleCost = 1 ether;
    uint    public MintedFreeNFTs;
    uint    public perWalletAllowed;
    uint    public maxMintAllowed;

    mapping(address => uint) public totalHold;

    constructor(string memory _name, string memory _symbol, uint _perWalletAllowed, uint _maxMintAllowed) ERC721A(_name, _symbol ) {
        perWalletAllowed = _perWalletAllowed;
   ...
   function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721A) returns (bool) {
    ...
    function mint(uint _howMuch) public payable callerIsUser checkWalletLimit(_howMuch) checkMintingLimit(_howMuch) checkNFTsAvailable(_howMuch) checkSaleStart{  
        ...
        if(totalHold[_msgSender()] == 0 && MintedFreeNFTs < 2 && totalSupply() < 6){
        ...
        else if((totalHold[_msgSender()] > 0 && totalSupply() < 6) || (MintedFreeNFTs >= 2 && totalSupply() < 6)){
            require(msg.value >= (_howMuch * preSaleCost), "You have insufficient balance.");
            _safeMint(_msgSender(), _howMuch);
            totalHold[_msgSender()] += _howMuch;
        }
        else if(MintedFreeNFTs >= 2 && totalSupply() >= 6){
    ...
    function setPerWalletLimit(uint256 _Quantity) public onlyOwner{
        perWalletAllowed = _Quantity;
    }

    function setMintingLimit(uint256 _Quantity) public onlyOwner{
        maxMintAllowed = _Quantity;
    }
    ...
    modifier checkMintingLimit(uint256 _Quantity) {
        require(_Quantity >= 1 && _Quantity <= maxMintAllowed, "Try to mint less NFTs.");
        _;
    }

    modifier checkWalletLimit(uint _Quantity) {
        require(totalHold[_msgSender()] + _Quantity <= perWalletAllowed, "You already reached the maximum limit or may mint less.");
        _;
    }
    ...
    modifier checkSaleStart() {
        require(paused == false, "Minting paused by Owner.");
        _;
    }
