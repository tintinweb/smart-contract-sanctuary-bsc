...
// File: presale.sol
    ...
    contract Presale is Ownable {
        ...
        uint256 secInMonth = 2629740;
        uint256 public vestingUnitSecs = secInMonth;
        ...
        function setTGEPercentage(uint256 _TGEPercentage) external onlyOwner {
            ...
            require(_TGEPercentage <= 100 && _TGEPercentage > 0, "Value should be 100 or less");
            TGEPercentage = _TGEPercentage;
        }
        function setVestingSchedule(uint256 _vestingMonths) external onlyOwner {
            require(
                startTime > block.timestamp || startTime == 0,
                "Presale already started!"
            );
            require(_vestingMonths >= 1, "Value should be 100 or less");
            vestingMonths = _vestingMonths;
        }
        function addWhitelistedAddress(address _address, uint256 _weiLimit)
            ...
            onlyOwner
        {
            require(_weiLimit > minWei, "WeiLimit should be greater than MinWei");
        ...
        function claimableAmount(address user) external view returns (uint256) {
            uint256 TGEAmount = ((owned[user].mul(TGEPercentage)).div(100));
            if(TGEPercentage == 100) {
                return claimable[user].mul(multiplier);
            }
            if(block.timestamp < claimTime + secInMonth && TGEPercentage < 100) {
                return (TGEAmount.sub(claimed[user])).mul(multiplier);
            }
            if(block.timestamp > (claimTime + (secInMonth.mul(vestingMonths)))  && TGEPercentage < 100) {
                return claimable[user].mul(multiplier);
            }
            uint month = 1;
            uint256 _installmentAmount = (owned[user].sub(TGEAmount)).div(vestingMonths);
            for(month = 1; month <= vestingMonths; month++) {
                if(block.timestamp < claimTime + (secInMonth * (month + 1))) {
                    return ((TGEAmount.add(month * _installmentAmount)).sub(claimed[user])).mul(multiplier);
                }
            }
            return claimable[user].mul(multiplier);
            //return owned[user].mul(multiplier);
        }
        ...
        function claim() external {
            ...
            uint256 TGEAmount = ((owned[msg.sender].mul(TGEPercentage)).div(100));
            if(TGEPercentage == 100) {
                _claimableAmount = claimable[msg.sender];
            }
            else if(block.timestamp < claimTime + secInMonth && TGEPercentage < 100) {
                _claimableAmount = TGEAmount.sub(claimed[msg.sender]);
            }
            else if(block.timestamp > (claimTime + (secInMonth.mul(vestingMonths))) && TGEPercentage < 100) {
                _claimableAmount = claimable[msg.sender];
            }
            else {
                uint month = 1;
                uint256 _installmentAmount = (owned[msg.sender].sub(TGEAmount)).div(vestingMonths);
                for(month = 1; month <= vestingMonths; month++) {
                    if(block.timestamp < claimTime + (secInMonth * (month + 1))) {
                        _claimableAmount = TGEAmount.add(month * _installmentAmount) - claimed[msg.sender];
                        break;
                    }
                }
            }
            ...
            claimed[msg.sender] = claimed[msg.sender].add(_claimableAmount);
            claimable[msg.sender] = claimable[msg.sender].sub(_claimableAmount);
            uint256 amount = _claimableAmount.mul(multiplier);
            totalOwed = totalOwed.sub(amount);
            require(token.transfer(msg.sender, amount), "failed to claim");

            // require(owned[msg.sender] > 0, "nothing to claim");
            // uint256 amount = owned[msg.sender].mul(multiplier);
            // owned[msg.sender] = 0;
            // totalOwed = totalOwed.sub(amount);
            // require(token.transfer(msg.sender, amount), "failed to claim");
        }
        ...
        function buy(uint256 value) external {
            ...
            if (isPublic) {
                require(
                    owned[msg.sender].add(value) <= maxWei,
                    "maximum purchase cap hit"
                );
            } else {
                require(
                    owned[msg.sender].add(value) <= weiLimits[msg.sender],
                    "maximum purchase cap hit"
                );
            }
            owned[msg.sender] = owned[msg.sender].add(value);
            claimable[msg.sender] = owned[msg.sender];
        ...
        function buyWithEth() public payable {
            ...
            if (isPublic) {
                require(
                    owned[msg.sender].add(msg.value) <= maxWei,
                    "maximum purchase cap hit"
                );
            } else {
                require(
                    owned[msg.sender].add(msg.value) <= weiLimits[msg.sender],
                    "maximum purchase cap hit"
                );
            }
            owned[msg.sender] = owned[msg.sender].add(msg.value);
            claimable[msg.sender] = owned[msg.sender];
            claimed[msg.sender] = 0;
            totalOwed = totalOwed.add(amount);
