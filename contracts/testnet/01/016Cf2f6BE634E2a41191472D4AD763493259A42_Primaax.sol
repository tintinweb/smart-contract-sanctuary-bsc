...
contract Primaax is ERC20, Ownable {
    ...
    returns(uint gTotalBonus, uint sTotalBonus, uint totalBonus, uint rClaimed, uint gSales, uint gEnd) {
        ...
        for(uint256 i = 0; i < downline[puser].length; i++) {
            ...
            if(direct_down.flushAmt > gi.highest) {
                theRest += theSecond;
                theSecond = gi.highest;
                gi.highest = direct_down.flushAmt;
            }
            else if(direct_down.flushAmt + 1 > theSecond) {
                if(gi.legOne != downline[puser][i]) {
                    theRest += theSecond;
        ...
        if(user.flushDeathline < block.timestamp) {
            ...
            if(lowest > 10000e18 && gi.claimed[0] < 1 && gi.level < 1) {
                groupBonus += 10000e18;
            }

            if(lowest > 100000e18 && gi.claimed[1] < 3 && gi.level < 2) {
                groupBonus += 100000e18;
            }

            if(lowest > 500000e18 && gi.claimed[2] < 2 && gi.level < 3) {
                groupBonus += 500000e18;
            }

            if(lowest > 1000000e18 && gi.claimed[3] < 1 && gi.level < 4) {
                groupBonus += 1000000e18;
            }

            if(lowest > 5000000e18 && gi.claimed[4] < 1 && gi.level < 5) {
                groupBonus += 5000000e18;
            }

            if(lowest > 10000000e18 && gi.claimed[5] < 1 && gi.level < 5) {
                groupBonus += 10000000e18;
            }
        }
  
        console.log("Group:", user.totalGroupBonus);
        user.totalGroupBonus += groupBonus;
        user.totalStakeBonus = bonus;
        user.totalAllBonus = user.totalStakeBonus + user.totalGroupBonus;
        return (user.totalGroupBonus, user.totalStakeBonus, user.totalAllBonus, user.totalClaimed, user.flushAmt, user.flushDeathline );
    }

    function userGroupInfo(address _user) external view returns(
        address a, address b, 
        uint256 gOneAmt, uint256 gTwoAmt, uint256 gRestAmt, 
        uint8 oneThousand, uint8 tenThousand, uint8 fiftyThousand, uint8 hundredThousand, uint8 fiveHundredThousand, uint8 oneMillion) {
        address puser = _user;
        uint256 real_rest = 0;
        uint256 real_second = 0;
        Group memory gi = groupInfo[puser];
        // delete gi.legThree;
        for(uint256 i = 0; i < downline[puser].length; i++) {
            User memory direct_down = userInfo[downline[puser][i]];
            if(direct_down.flushAmt > gi.highest) {
                real_rest += real_second;
                real_second = gi.highest;
                gi.highest = direct_down.flushAmt;
                gi.legOne = downline[puser][i];
            }
            else if(direct_down.flushAmt + 1 > real_second) {
                if(gi.legOne != downline[puser][i]) {
                    real_rest += real_second;
                    real_second = direct_down.flushAmt;
                    gi.legTwo = downline[puser][i];
                }
            }
            else {
                real_rest += direct_down.flushAmt;
            }
        }
        return (gi.legOne, gi.legTwo, gi.highest, real_second, real_rest, gi.claimed[0], gi.claimed[1], gi.claimed[2], gi.claimed[3], gi.claimed[4], gi.claimed[5]);
    }
    ...
    function withdraw() external {
        ...
        uint256 bonus = 0;
        for(uint256 i = 0; i < user.deposits.length; i++) {
            require(block.timestamp > user.deposits[i].start && block.timestamp - user.deposits[i].start > TIME_COUNT, "At least 1 day");
            bonus += user.deposits[i].amount * plan[user.deposits[i].planId].ror * ((block.timestamp - user.deposits[i].start) / TIME_COUNT  > 365 ? 365 : (block.timestamp - user.deposits[i].start) / TIME_COUNT ) / RATE / 365;
        }
        ...
        for(uint256 i = 0; i < downline[msg.sender].length; i++) {
            ...
            if(direct_down.flushAmt > gi.highest) {
                ...
                if(gi.legOne != downline[msg.sender][i]) {
                    theRest += theSecond;
                    theSecond = direct_down.flushAmt;
