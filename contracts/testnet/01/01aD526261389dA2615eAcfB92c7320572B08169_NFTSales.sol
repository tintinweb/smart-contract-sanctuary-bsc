...
contract NFTSales is Ownable, NFTType, ReentrancyGuard, Proxyable {
    ...
    address public uncratePaymentToken; // PIP

    bool public lootcrateSaleActive;
    bool public uncrateActive;

    uint8 public nftsPerLootCrate = 3;
    uint256 public lootcratePrice = 0.1 ether; // ETH
    uint256 public uncratePrice = 25 ether; // PIP
    uint256 public stakingPercent = 2500; // 2500 = 25%
    ...
    constructor(
        ...
        address _treasury,
        address _uncratePaymentToken,
        uint256 _lootcratePrice,
        uint8 _nftsPerLootcrate,
        uint256 _uncratePrice,
        address _stakingContract
    ) NFTType(name, symbol) notZeroAddress(_treasury) {
        treasury = _treasury;
        uncratePaymentToken = _uncratePaymentToken;
        lootcratePrice = _lootcratePrice;
        nftsPerLootCrate = _nftsPerLootcrate;
        uncratePrice = _uncratePrice;
    ...
    modifier saleIsActive() {
        if (!lootcrateSaleActive) revert SaleIsClosed();
        _;
    }
    ...
    function buyLootCrate() external payable saleIsActive nonReentrant {
        if (msg.value != lootcratePrice) revert InsufficientETH();
        (bool success, ) = stakingContract.call{value: msg.value * stakingPercent / 1e4}("");
    ...
    function batchBuyLootCrate(uint8 amount)
        ...
        nonReentrant
    {
        uint256 payment = lootcratePrice * amount;
        if (msg.value != payment) revert InsufficientETH();
        (bool success, ) = stakingContract.call{value: msg.value * stakingPercent / 1e4}("");
        require(success, "Transfer failed");        
    ...
    function setLootCratePriceInNFTs(uint256 value) external onlyOwner {
        lootcratePrice = value;
    }
    ...
    function uncrate(uint32 id) external payable senderOwnsToken(id) {
        _isLootCrate(id);
        IERC20(uncratePaymentToken).safeTransferFrom(
            _msgSender(),
            address(this),
            uncratePrice
        );
    ...
    function updateLootCrateConfig(
        ...
        nftsPerLootCrate = _nftsPerLootcrate;
        uncratePrice = _uncratePrice;
        lootcrateSaleActive = _lootcrateSaleActive;
        uncrateActive = _uncrateActive;
    }

    function setLootCrateSaleActive(bool value) external onlyOwner {
        lootcrateSaleActive = value;
    }
...
abstract contract NFTType is Ownable, ERC721Base {
    using Strings for uint256;

    uint256 public maxNFTsToParse = 500;
...
abstract contract ERC721Base is Ownable, ERC721Min {
    ...
    // mint NFTs to a list of receivers, assigning an NFT type to each minted NFT
    function _airdrop(address[] calldata receivers)
        internal
        virtual
        onlyOwner
    {
