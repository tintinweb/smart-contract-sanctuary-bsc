...
contract FiveElements is Permission {
    ...
    function pushOrderList(bytes32 newHash, uint winBouns) private {
        pool = pool - winBouns;
    ...
    function endGame() public {
        ...
        isEnd = 2;
        uint allTicket = getAllComps(0).length;
        onlyBouns = pool * 500 / 1000;
        pool = pool - onlyBouns;
        dividendBouns[getAllComps(0)[allTicket - 1]] = dividendBouns[getAllComps(0)[allTicket - 1]] + onlyBouns;
        if(allTicket >= 20){
            seatBouns = pool * 5 / 1000;
            pool = pool - (pool * 100 / 1000);
            for(uint i = 2; i < 21; i++){
                dividendBouns[getAllComps(0)[allTicket - i]] = dividendBouns[getAllComps(0)[allTicket - i]] + seatBouns;
            }
        }else{
            seatBouns = pool * 100 / 1000 / getAllComps(0).length;
            pool = pool - (pool * 100 / 1000);
            for(uint i = 2; i < 21; i++){
                dividendBouns[getAllComps(0)[allTicket - i]] = dividendBouns[getAllComps(0)[allTicket - i]] + seatBouns;
            }
