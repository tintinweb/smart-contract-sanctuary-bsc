...
interface IMasterChef {
    ...
    function pendingRosh(uint256 _pid, address _user)
        external
        view
        returns (uint256);
...
/// @notice The (older) MasterChef contract gives out a constant number of ROSH tokens per block.
/// It is the only address with minting rights for ROSH.
...
contract MasterChefV2 is Ownable, ReentrancyGuard {
    ...
    ///
    /// We do some fancy math here. Basically, any point in time, the amount of ROSHs
    /// entitled to a user but is pending to be distributed is:
    ///
    ///   pending reward = (user share * pool.accRoshPerShare) - user.rewardDebt
    ///
    ///   Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
    ///   1. The pool's `accRoshPerShare` (and `lastRewardBlock`) gets updated.
    ...
    ///     ROSH rewards each pool gets.
    /// `accRoshPerShare` Accumulated ROSHs per share, times 1e12.
    ...
    ///     the ROSH rewards to all the RoshiniSwap products.
    /// `totalBoostedShare` The total amount of user shares in each pool. After considering the share boosts.
    struct PoolInfo {
        uint256 accRoshPerShare;
    ...
    /// @notice Address of ROSH contract.
    IBEP20 public immutable ROSH;

    /// @notice The only address can withdraw all the burn ROSH.
    ...
    ///  @notice 40 roshs per block in MCV1
    uint256 public constant MASTERCHEF_ROSH_PER_BLOCK = 40 * 1e18;
    uint256 public constant ACC_ROSH_PRECISION = 1e18;

    /// @notice Basic boost factor, none boosted user's boost factor
    uint256 public constant BOOST_PRECISION = 100 * 1e10;
    /// @notice Hard limit for maxmium boost factor, it must greater than BOOST_PRECISION
    uint256 public constant MAX_BOOST_PRECISION = 200 * 1e10;
    /// @notice total rosh rate = toBurn + toRegular + toSpecial
    uint256 public constant ROSH_RATE_TOTAL_PRECISION = 1e12;
    /// @notice The last block number of ROSH burn action being executed.
    /// @notice ROSH distribute % for burn
    uint256 public roshRateToBurn = 75 * 1e10;
    /// @notice ROSH distribute % for regular farm pool
    uint256 public roshRateToRegularFarm = 10 * 1e10;
    /// @notice ROSH distribute % for special pools
    uint256 public roshRateToSpecialFarm = 15 * 1e10;
    ...
    /// @param _MASTER_CHEF The RoshiniSwap MCV1 contract address.
    /// @param _ROSH The ROSHINI token contract address.
    ...
    /// @notice Deposits a dummy token to `MASTER_CHEF` MCV1. This is required because MCV1 holds the minting permission of ROSH.
    ...
    function init(IBEP20 dummyToken) external onlyOwner {
        ...
        // MCV2 start to earn ROSH reward from current block in MCV1 pool
    ...
    /// only for ROSH distributions within RoshiniSwap products.
    ...
    function add(
        ...
        // stake ROSH token will cause staked token and reward token mixed up,
        // may cause staked tokens withdraw as reward token,never do it.
        require(_lpToken != ROSH, "ROSH token can't be added to farm pools");
    ...
    /// @notice Update the given pool's ROSH allocation point. Can only be called by the owner.
    ...
    /// @notice View function for checking pending ROSH rewards.
    /// @param _pid The id of the pool. See `poolInfo`.
        ...
        returns (uint256)
    {
        ...
        uint256 accRoshPerShare = pool.accRoshPerShare;
        uint256 lpSupply = pool.totalBoostedShare;

        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier = block.number.sub(pool.lastRewardBlock);

            uint256 roshReward = multiplier
                .mul(roshPerBlock(pool.isRegular))
                .mul(pool.allocPoint)
                .div(
                    (
                        pool.isRegular
            ...
            accRoshPerShare = accRoshPerShare.add(
                roshReward.mul(ACC_ROSH_PRECISION).div(lpSupply)
            );
        }
        ...
        uint256 boostedAmount = user
            ...
            .div(BOOST_PRECISION);
        return
            boostedAmount.mul(accRoshPerShare).div(ACC_ROSH_PRECISION).sub(
                user.rewardDebt
            );
    }

    /// @notice Update rosh reward for all the active pools. Be careful of gas spending!
    ...
    /// @notice Calculates and returns the `amount` of ROSH per block.
    /// @param _isRegular If the pool belongs to regular or special.
    function roshPerBlock(bool _isRegular)
        public
        view
        returns (uint256 amount)
    {
        if (_isRegular) {
            amount = MASTERCHEF_ROSH_PER_BLOCK.mul(roshRateToRegularFarm).div(
                ROSH_RATE_TOTAL_PRECISION
            );
        } else {
            amount = MASTERCHEF_ROSH_PER_BLOCK.mul(roshRateToSpecialFarm).div(
                ROSH_RATE_TOTAL_PRECISION
            );
        }
    }

    /// @notice Calculates and returns the `amount` of ROSH per block to burn.
    function roshPerBlockToBurn() public view returns (uint256 amount) {
        amount = MASTERCHEF_ROSH_PER_BLOCK.mul(roshRateToBurn).div(
            ROSH_RATE_TOTAL_PRECISION
        );
    }
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0 && totalAllocPoint > 0) {
                ...
                uint256 roshReward = multiplier
                    ...
                    .mul(pool.allocPoint)
                    .div(totalAllocPoint);
                pool.accRoshPerShare = pool.accRoshPerShare.add(
                    (roshReward.mul(ACC_ROSH_PRECISION).div(lpSupply))
                );
            }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            settlePendingRosh(msg.sender, _pid, multiplier);
        }
        ...
        user.rewardDebt = user
            ...
            .mul(multiplier)
            .div(BOOST_PRECISION)
            .mul(pool.accRoshPerShare)
            .div(ACC_ROSH_PRECISION);
        poolInfo[_pid] = pool;
    ...
    /// @notice Harvests ROSH from `MASTER_CHEF` MCV1 and pool `MASTER_PID` to MCV2.
    ...
    /// @notice Send ROSH pending for burn to `burnAdmin`.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function burnRosh(bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }

        uint256 multiplier = block.number.sub(lastBurnedBlock);
        uint256 pendingRoshToBurn = multiplier.mul(roshPerBlockToBurn());

        // SafeTransfer ROSH
        _safeTransfer(burnAdmin, pendingRoshToBurn);
        lastBurnedBlock = block.number;
    }

    /// @notice Update the % of ROSH distributions for burn, regular pools and special pools.
    /// @param _burnRate The % of ROSH to burn each block.
    /// @param _regularFarmRate The % of ROSH to regular pools each block.
    /// @param _specialFarmRate The % of ROSH to special pools each block.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function updateRoshRate(
        uint256 _burnRate,
        ...
        require(
            ...
            "MasterChefV2: Rosh rate must be greater than 0"
        );
        require(
        ...
        // burn rosh base on old burn rosh rate
        burnRosh(false);

        roshRateToBurn = _burnRate;
        roshRateToRegularFarm = _regularFarmRate;
        roshRateToSpecialFarm = _specialFarmRate;

        emit UpdateRoshRate(_burnRate, _regularFarmRate, _specialFarmRate);
    }
    ...
    function updateBoostMultiplier(
        ...
        settlePendingRosh(_user, _pid, prevMultiplier);
    ...
    /// @notice Settles, distribute the pending ROSH rewards for given user.
    /// @param _user The user address for settling rewards.
    /// @param _pid The pool id.
    /// @param _boostMultiplier The user boost multiplier in specific pool id.
    function settlePendingRosh(
        address _user,
        uint256 _pid,
        ...
        uint256 accRosh = boostedAmount.mul(poolInfo[_pid].accRoshPerShare).div(
            ACC_ROSH_PRECISION
        );
        uint256 pending = accRosh.sub(user.rewardDebt);
        // SafeTransfer ROSH
        _safeTransfer(_user, pending);
    }

    /// @notice Safe Transfer ROSH.
    /// @param _to The ROSH receiver address.
    /// @param _amount transfer ROSH amounts.
    function _safeTransfer(address _to, uint256 _amount) internal {
        if (_amount > 0) {
            // Check whether MCV2 has enough ROSH. If not, harvest from MCV1.
            if (ROSH.balanceOf(address(this)) < _amount) {
                harvestFromMasterChef();
            }
            uint256 balance = ROSH.balanceOf(address(this));
            if (balance < _amount) {
                _amount = balance;
            }
            ROSH.safeTransfer(_to, _amount);
        }
    }
}