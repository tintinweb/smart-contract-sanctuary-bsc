...
contract LandSale is AccessProtected {
    ...
    function purchaseProperty(uint256 nftId) external onlyDuringSale {
        if (earlySale) {
            ...
            require(
                purchaseEarlySale[msg.sender] + 1 <= allotment,
                "You cannot buy greater than allotment"
            );
        }
        ...
        onlyDuringSale
    {
        if (earlySale) {
            ...
            uint256 allotment = whitelist.checkAllotment(msg.sender);
            require(
                purchaseEarlySale[msg.sender] + nftIds.length <= allotment,
                "You cannot buy greater than allotment"
            );
        }
