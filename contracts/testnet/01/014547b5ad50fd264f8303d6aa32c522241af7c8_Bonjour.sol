...
contract Bonjour is Ownable,ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public maxInvestment     = 100000 * 1e18;
    uint256 public referralPercent   = 7000; // 1 means 1000; So 7%
    uint256 public roiPercent        = 150;  // 1 means 1000; So 1.5% 
    uint256 public globalPoolPercent = 1000; // 1 means 1000; So 1%
    uint256 public tradeAmount       = 29500; // 29.5
    uint256 public earnLimit         = 200;   
    uint256 public globalVolume      = 600 * 1e18;
    mapping(uint256 => uint256) public LEVEL_INCOME;
    address public ownerAddress;
    uint256 public currUserID;
    uint256 public globalPoolShare;
    uint256 public minWithdrawAmount = 10 * 1e18;
     constructor(
        address _ownerAddress
    ) public {
        BusdAddress = IBEP20(0x8CE7720fD183AeC96b676FD8250724b05b0d7a6F);
    ...
    function regUser(address _referrer,uint256 _amount) public {
        ...
        }else if(users[_referrer].referralCount == 16 && _referrer!= ownerAddress){
        ...
        BusdAddress.safeTransfer(ownerAddress,externalAmt);
        //direct Referral
    ...
    function reInvest(uint256 _amount) public {
         ...
         require(_amount >= minInvestment,"Investment is low");
         uint startDate = users[msg.sender].investmentDate;
         uint endDate = block.timestamp;
         uint diff = (endDate - startDate) / 60 / 60 / 24; // 40 days 
         uint256 uptoroi = diff.mul(users[msg.sender].investmentAmount.mul(roiPercent).div(100000));
         uint256 totalearned = uptoroi.add(users[msg.sender].earnedAmount).add(users[msg.sender].referupto);
         
         users[msg.sender].earnedAmount += uptoroi;
    ...
    function claimOrRetopop(bool isClaim) public {
         ...
         uint256 uptoroi = diff.mul(users[msg.sender].investmentAmount.mul(roiPercent).div(100000));
         uint256 totalearned = uptoroi.add(users[msg.sender].earnedAmount);
         users[msg.sender].earnedAmount = totalearned;
         uint256 maxlimit = users[msg.sender].investmentAmount.mul(earnLimit).div(100);
         if(totalearned.add(users[msg.sender].referupto) >= maxlimit){
             users[msg.sender].earnedAmount =  maxlimit - users[msg.sender].referupto;
                if(isClaim){
                  ...
                  users[msg.sender].referupto = 0;
                   ...
                   require(users[msg.sender].earnedAmount >= minInvestment,"Investment is low");
        ...
        if (upliner != 0x0000000000000000000000000000000000000000) {
            ...
            if(_cnt < 8){
                ...
                if(users[upliner].levelIncomeEligible >= _cnt && checkIsEarn(upliner)){
                    ...
                    uint256 sendAmount = _amount.mul(LEVEL_INCOME[_cnt]).div(100000);
                    users[upliner].levelIncome += sendAmount;
                    users[upliner].earnedAmount += sendAmount;
                }
            }    
    ...
    function checkIsEarn(address _user) public view returns(bool) {
        ...
        if(totalearned >= users[_user].investmentAmount.mul(earnLimit).div(100) && _user!= ownerAddress){
           return false; 
        }else{
            return true;
        }
    ...
    function globalPoolDistribution() public onlyOwner {
      if(globalPoolAddress.length > 0 && globalPoolShare > 0){
          for(uint i=0; i < globalPoolAddress.length; i++){
            uint256 perUserAmount =  globalPoolShare/globalPoolAddress.length;
            users[globalPoolAddress[i]].globalPoolIncome += perUserAmount;
            users[globalPoolAddress[i]].earnedAmount += perUserAmount;
          }
          globalPoolShare = 0;
      }
    }
