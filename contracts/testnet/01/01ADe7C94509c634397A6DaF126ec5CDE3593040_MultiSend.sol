...
/// @title MultiSend
///
/// @notice Send ERC20 token and Eth to multiple addresses
contract MultiSend is Pausable, Ownable, ReentrancyGuard
{
  ...
  /// @notice Send Eth to multiple addresses using two arrays which
  ...
  {
    require(_addresses.length == _amounts.length, "invalid parm");
    require(_addresses.length <= 255, "exceed max addresses allowed");

    uint256 amountSum = msg.value;

    for (uint8 i; i < _addresses.length; i++) {
      amountSum = amountSum.sub(_amounts[i]);
      _addresses[i].transfer(_amounts[i]);
    }

    // make sure contract is not overfunded
    require(amountSum == 0, "overfunded");
  }
  ...
  ) external whenNotPaused nonReentrant
  {
    ...
    // make sure contract is not overfunded
    require(_amountSum == 0, "overfunded");
  }
  ...
  fallback() external payable { require(msg.data.length == 0, "Invalid function"); }
  ...
  function escapeHatch(address _token, address payable _escapeHatchDestination) external onlyOwner nonReentrant {
    require(_escapeHatchDestination != address(0x0), "Cannot escape to zero address");
