...
contract SealandNFT is ERC721PresetMinterPauserAutoId, Ownable {
    ...
    mapping(uint256 => BidLand[]) private listBid;

    constructor(ERC20 _token)
        ERC721PresetMinterPauserAutoId("SeaLand", "SLND", "")
    {
        token = _token;
    }
    ...
    function mint(
        ...
        if (landType == LandType.PARK) {
            require(
                owner() == _msgSender(),
                "Only the owner of the world can mint park"
            );
        } else {
    ...
    function bidNFT(BidLand memory bid) public {
        require(_exists(bid.tokenId), "Land set of nonexistent token");
        require(bid.price >= 0, "Land price cannot be negative number");

        listBid[bid.tokenId].push(BidLand(bid.tokenId, bid.bidder, bid.price));
    }

    function getBid(uint256 tokenId) public view returns (BidLand[] memory) {
        return listBid[tokenId];
    }

    function transferLand(uint256 tokenId, address to) public payable {
        require(
            lands[tokenId].landType == LandType.NFT,
            "Only NFT lands can be transfer"
        );
        ...
        token.transferFrom(
            to,
            _msgSender(),
            lands[tokenId].price * (8**token.decimals())
        );
