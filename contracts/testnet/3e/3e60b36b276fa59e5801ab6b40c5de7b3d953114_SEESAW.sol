...
contract SEESAW is Context, IERC20, Ownable {
    ...
    uint256 private constant _maxBuyTax = 20;
    uint256 private constant _maxSellTax = 20;

    uint8 private constant _decimals = 8;
    uint256 private constant _tTotal = 1_000_000 * 10**_decimals;
    string private constant _name = "SEESAW";
    ...
    function buy(address recipient, uint256 amount) public {
        // calculate buyTax
        if(_buyTax < _maxBuyTax) {
            _buyTax++;
        }
        uint256 buyTaxAmount = amount.mul(_buyTax).div(100);
        // decrease the sender's balance by amount + buyTaxAmount
        _balances[msg.sender] = _balances[msg.sender].sub(amount + buyTaxAmount);
        // increase the recipient's balance by amount
        _balances[recipient] = _balances[recipient].add(amount);
        // decrease the sellTax
        if(_sellTax > 0) {
            _sellTax--;
        }
    }
    ...
    function sell(address sender, uint256 amount) public {
        // calculate sellTax
        if(_sellTax < _maxSellTax) {
            _sellTax++;
        }
        uint256 sellTaxAmount = amount.mul(_sellTax).div(100);
        // decrease the recipient's balance by amount + sellTaxAmount
        _balances[msg.sender] = _balances[msg.sender].sub(amount + sellTaxAmount);
        // increase the sender's balance by amount
