...
contract DyBNBVenus is Initializable, OwnableUpgradeable, DyETH {
    ...
    function testMint() public payable onlyOwner {
        tokenDelegator.mint{value: msg.value}();
    }

    function testBurn(uint256 _amount) public payable onlyOwner {
    ...
    function withdrawDepositTokens(uint256 amountUnderlying_) public onlyOwner {
        _withdrawDepositTokens(amountUnderlying_);
    }
    ...
    function _unrollDebt(uint256 amountToBeFreed_) public {
        (uint256 balance, uint256 borrowed) = _getAccountData();
...
abstract contract DyETH is DyToken, ReentrancyGuardUpgradeable {
    ...
    function _doTransferOut(address payable to_, uint256 amount_)
        ...
        override
    {
        uint256 roundDownAmount = 500;
        to_.transfer(amount_ - roundDownAmount);
    }
}
