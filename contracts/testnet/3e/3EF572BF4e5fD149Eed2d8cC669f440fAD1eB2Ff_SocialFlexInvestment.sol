...
CREATE INSURED INVESTMENT
CREATE INVESTMENT
REWARD SYSTEM FOR THE TWO 
...
contract SocialFlexInvestment is Fetchers, Ownable {
    ...
    function createInvestment(
        ...
        if (!(flexvis.transferFrom(msg.sender, address(this), amount))) {
            revert TransferFromFailed();
        }

        Investment memory newInvestment;
        newInvestment.investedAmount = amount;
        newInvestment.startDay = startDay;
  ...
  ******************************************************************************************************** */
    ...
    function withdrawFlexvis() external onlyOwner {
...
contract Fetchers is Tradeable {
    ...
    {
        Investment memory userInvestment = investments[_user][_investmentID];
        investedAmount = userInvestment.investedAmount;
        reward = userInvestment.reward;
        startDay = userInvestment.startDay;
        investingDays = userInvestment.investingDays;
        count = investmentCount[_user];
    }
...
abstract contract Tradeable is Conversion, ReentrancyGuard, Events {
    function buyFlexvisWithBUSD(
        ...
        require(
            busd.balanceOf(msg.sender) >= busdAmount,
            "Insufficient BUSD amount"
        );

        // Swapping Flexvis to BNB;
    ...
    function sellFlexvisForToken(
        ...
        require(
            flexvis.balanceOf(msg.sender) >= flexvisAmount,
            "Insufficient FLEXVIS"
        );

        // I have to check the path here............................................

        // _sellFlexvis(msg.sender, flexvisAmount, path, deadline, minPercentage);
        IUniswapV2Router02 router = IUniswapV2Router02(ROUTER);

        uint256[] memory amountsOut = router.getAmountsOut(flexvisAmount, path);

        uint256 amountOutMin = (amountsOut[amountsOut.length - 1] * minPercentage) / 10000;
...
abstract contract Helper is Declaration {
    ...
    function getInvestmentReward(
        ...
        if (_investingDays == 7 days) {
            ...
            // REWARD IS ACTUALLY 0. FOR TESTING: I WILL PUT 100
            reward = 100;
        }
    }
...
abstract contract Conversion is Helper {
    ...
    //     (uint112 wbnbReserve, uint112 busdReserve) = getReserves(WBNB, BUSD);
    //     uint256 wbnbEquivalent = (wbnbReserve * busdAmount) /
    //         (busdReserve + busdAmount);
    //     return wbnbEquivalent;
    // }

    // function bnbToFlexvis(uint256 wbnbAmount) public view returns (uint256) {
    //     (uint112 flexvisReserve, uint112 wbnbReserve) = getReserves(
    //         FLEXVIS,
    //         WBNB
    //     );
    //     uint256 flexvisEquivalent = (flexvisReserve * wbnbAmount) /
    //         (wbnbReserve + wbnbAmount);
    //     return flexvisEquivalent;
    // }

    // function busdToFlexvis(uint256 busdAmount) public view returns (uint256) {
    //     uint256 wbnbAmount = busdToBnb(busdAmount);
    //     uint256 flexvisEquivalent = bnbToFlexvis(wbnbAmount);
