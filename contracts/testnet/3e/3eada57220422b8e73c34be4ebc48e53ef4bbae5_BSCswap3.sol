...
contract BSCswap3 {
    ...
    function leaderPool() view external returns(uint256 mp,uint256 mpTeam,uint256 lp,uint256 lpTeam) {
        uint256 totalDays=getCurDay();
        return (manager_reward[totalDays],manager_array.length,leader_reward[totalDays],leader_array.length);
    }
