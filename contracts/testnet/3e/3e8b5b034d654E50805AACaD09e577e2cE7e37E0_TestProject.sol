...
contract TestProject is Context, IERC20, Ownable {
    ...
    uint256 private _previousDevFee = _liqFee;
    ...
    function _takeDevFee(uint256 tDev) private {
        uint256 currentRate =  _getRate();
        uint256 rDev = tDev.mul(currentRate);
        _rOwned[_liqAddress] = _rOwned[_liqAddress].add(rDev);
        if(_isExcluded[_liqAddress])
            _tOwned[_liqAddress] = _tOwned[_liqAddress].add(tDev);
    }
