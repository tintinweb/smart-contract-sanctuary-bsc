...
contract PokerNut is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getAttributes(uint256 tokenId) private view returns (string memory) {
        return string(
            abi.encodePacked(
                '"attributes": [',
                '{trait_type: "card_1", "value":"', metaInfo[tokenId].card1,'"}',
                '{trait_type: "card_2", "value":"', metaInfo[tokenId].card2,'"}',
                '{trait_type: "card_3", "value":"', metaInfo[tokenId].card3,'"}',
                '{trait_type: "card_4", "value":"', metaInfo[tokenId].card4,'"}',
                '{trait_type: "card_5", "value":"', metaInfo[tokenId].card5,'"}',
                '{trait_type: "signed_by", "value":"', Helper.addressToString(metaInfo[tokenId].signedBy),'"}',
                ']'
            )
        );
    }
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Nut #',
                        Helper.toString(tokenId),
                        '", "description": "PokerNut (POKER) are randomized, generated, and stored on chain. Images and other functionality are intentionally omitted for others to interpret. Feel free to use POKER in any way you want. Inspired and compatible with Poker Loot. This NFT was signed by ',
    ...
    function _mintPokerNut(uint256 tokenId) private {
    ...
    function claimForFree() public nonReentrant {
        ...
        require(
            tokenId <= 9999,
            "Error: supply for free mint is limited at 10,000 POKER"
        );

        _mintPokerNut(tokenId);
    }

    function buy() public payable nonReentrant {
        require(msg.value == buyFee, "Error: buy fee must be 0.01 BNB");
        ...
        require(
            ...
            "Error: supply for buy is limited at 90,000 POKER"
        );
...
/// @author Khang Tran @ [email protected]
library Helper {
