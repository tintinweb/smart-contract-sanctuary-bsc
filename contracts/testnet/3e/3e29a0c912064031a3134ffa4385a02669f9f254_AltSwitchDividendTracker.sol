...
contract AltSwitch is ERC20, Ownable {
    ...
    constructor() ERC20("AltSwitch", "ALTX", 9) {
        ...
        address _migrationSwapContract = 0xa8D787106eC0b48c820372C70A591a6f4e09Fc2F;  // @note Testnet Migration Contract
        ...
        updateDefaultRebate(0); // @note No rebate on launch
     ...
     */
    function updateDefaultRebate(uint16 percentRebate) public {
     ...
     */
    function setRebate(address addr, uint16 percentRebate) external {
    ...
    function isRebated(address addr) public view virtual returns (bool) {
        uint16 rebate = getPercentRebate(addr);
        return rebate == 0 ? false : true;
    }
    ...
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require (airdropWallets.length == amount.length, "ALTSWITCH: airdropToWallets:: Arrays must be the same length");
    ...
    function updateMigrationSwapContract(address newMigrationSwapContract) external onlyOwner {
        require(newMigrationSwapContract != migrationSwapContract, "ALTSWITCH: The migration swap contract is already this address");
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        ...
        require(!isBlacklistedToken(rewardTokenAddress), "ALTSWITCH: setRewardToken:: Reward Token is blacklisted from being used as rewards.");
    ...
    function _transfer(
        ...
        // Wallet-to-wallet transfer actions
        if (!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to]) {
            uint256 fees = amount.mul(totalFees).div(100);

            // @note substract tax on wallet-to-wallet transfer and add holders rebates

            // amount = amount.sub(tax).add(rebates);

            super._transfer(from, operationsWallet, fees);
        }
