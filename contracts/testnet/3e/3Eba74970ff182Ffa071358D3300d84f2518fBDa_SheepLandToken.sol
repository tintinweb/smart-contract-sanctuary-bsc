...
contract SheepLandToken is Ownable, ReentrancyGuard {
    ...
    {
        return (farmsLand[account].SheepCoins, farmsLand[account].cash, (farmsLand[account].cash.div(100)).mul(_cashValue));
    }
    ...
    function addCoins(address ref, uint256 tokenAmount) public nonReentrant(){
        ...
        for (uint i = 0; i < _MangerId; i++) {
            if(ManagersTokenomics[i]._PartnerAddress != address(0) || ManagersTokenomics[i]._PartnerPercent > 0 ){
                uint256 ManageAmount = ((PartnerValue).mul(ManagersTokenomics[i]._PartnerPercent)).div(100);
                ContractAdd.transferFrom(user, ManagersTokenomics[i]._PartnerAddress, ManageAmount);
            }
        }

        ContractAdd.transferFrom(user, address(this), SheepCoins.sub(PartnerValue));
    ...
    function sheepRewards(uint256 amount, address userAddress) public onlyOwner {
        require(
            farmsLand[userAddress].exist,
            "A User does not exist, check the contract or create it first"
        );

        uint256 SheepCoins = amount;
        farmsLand[userAddress].SheepCoins += SheepCoins;
    }
