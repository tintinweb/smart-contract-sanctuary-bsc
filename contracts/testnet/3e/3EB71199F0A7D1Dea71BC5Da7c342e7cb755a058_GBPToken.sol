...
contract GBPToken is ERC20Detailed, Ownable {
    ...
    function getCirculatingSupply() public view returns (uint256) {
        return
            (TOTAL_GONS.sub(_gonsBalances[DEAD]).sub(_gonsBalances[ZERO])).div(
                _gonsPerFragment
            );
    }
    ...
    function setStartTradingTime(uint256 _startTime)external onlyOwner{
        _setStartTradingTime(_startTime);
    }

    function _setStartTradingTime(uint256 _startTime) private {
        startTradingTime = _startTime;
        if(block.timestamp < rebaseStime || rebaseStime == 0){
            _lastRebasedTime = rebaseStime = startTradingTime.add(rebaseInterval);
        }
    }
    ...
    function _rebase() private {
        ...
        uint256 rebaseCount = _getRebaseCount() - times;
        for (uint256 i = 0; i < times; i++) {
            uint256 rebaseRate = _getRebaseRate(rebaseCount);
            ...
            _gonsPerFragment = TOTAL_GONS.div(_totalSupply);
            emit LogRebase(rebaseCount, rebaseRate, _totalSupply);
            rebaseCount++;
        }
    }

    function _getRebaseCount() private view returns (uint256) {
        return block.timestamp.sub(rebaseStime).div(rebaseInterval);
    }

    function _getRebaseRate(uint256 _rebaseCount)
        private
        pure
        returns (uint256)
    {
        if (_rebaseCount < 96 * 7) {
            return 7246412223703898;//100%
        }
        if (_rebaseCount < 96 * 14) {
            return 4232526823514061;//50%
        }
        if (_rebaseCount < 96 * 21) {
            return 2327115532720302;//25%
        }
        if (_rebaseCount < 96 * 28) {
            return 1227659579254334;//12.5%
        }
        if (_rebaseCount < 96 * 35) {
