...
contract SmartWallet is ReentrancyGuard {
    ...
    function deploy(
        ...
        require(_timeout > block.number, "Transaction timed out");
        bytes32 hash = keccak256(abi.encode("deploy", address(this), nonce, _value, _bytecode, _salt, _timeout));
        require(verifySig(owner, hash, _v, _r, _s), "Incorrect sig");
        ...
        address addr;
        assembly {
            addr := create2(_value, add(_bytecode, 0x20), mload(_bytecode), _salt)
        }

        emit DeployEvent(nonce - 1, addr);
    }
