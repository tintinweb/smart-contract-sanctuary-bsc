...
contract GREENTRANSMIT is Context, Ownable {
    ...
    //参数
    struct params{
        uint[3] inAmount;//组合u金额 分别1-3层的支付金额
        uint tokenAmount;//组合价值u金额的代币
        uint gtOdds;//爆仓奖励投资额gt倍数
        uint[3] staticAmount;//静态收益 分别1-3层的奖励
        uint[2] dynamicAmount;//动态收益 代数，u
    ...
    constructor() {
        ...
        _params.inAmount=[100*10**18,130*10**18,160*10**18];
        _params.tokenAmount=10*10**18;
        _params.gtOdds=2;
        _params.staticAmount=[3*10**18,4*10**18,5*10**18];
        _params.dynamicAmount=[7,1*10**18];
    ...
    function transmit() external {
        ...
        IBEP20(_usdt).transferFrom(_msgSender(),address(this),_params.inAmount[buyLevel]);

        IBEP20(_depm).transferFrom(_msgSender(),_vault,_amountOut(_params.tokenAmount,_usdt,_depm));//depm放资金池
        ...
        _transmits[floor].push(transmitInfo(true,buyLevel,_msgSender(),block.timestamp));
        _transmitCount++;
        ...
        for(uint i=maxFloor;i>=floor;i--){
            ...
            for(uint ii=len-1;ii>0;ii--){
                if(_transmits[i][ii].status==true){
                    bangAmountTotal+=_params.inAmount[_transmits[i][ii].addType];
                    _bangs[bangNum]=bang(true,_transmits[i][ii].account,_params.inAmount[_transmits[i][ii].addType],0);
    ...
    function _outTransmit(transmitInfo storage info) internal{
        ...
        info.status=false;
        uint inAmount=_params.inAmount[info.addType];
        uint staticAmount=_params.staticAmount[info.addType];
