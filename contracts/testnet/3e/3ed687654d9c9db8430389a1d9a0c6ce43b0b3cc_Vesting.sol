...
contract Vesting {
    ...
    constructor(address _tokenAddress, uint _setStartDate ) {
        ...
        LaunchStartDate=_setStartDate;
         ...
         publicNextPay = publicStartDate + 30 days;
         launchNextPay = LaunchStartDate + 30 days;

        seedLockEndDate = seedStartDate +  30 minutes;
        privateLockEndDate = privateStartDate + 30 minutes;
        StratrgiclockEndDate = StrategicStartDate + 30 minutes;

        publicvestDate = publicStartDate + 30 minutes;
        launchvestDate = LaunchStartDate + 30 minutes;



        seedVestingEndDate = seedLockEndDate +  365 minutes;
        privateVestingEndDate = privateLockEndDate + 300 minutes;
        StrategicVestingEndDate = StratrgiclockEndDate + 240 minutes;
    }
            ...
            if(saleType == 3) {//strategic
              ...
              if(saleType == 4){// public
                ...
                if (block.timestamp >= publicNextPay){
                    investor.initialAmount = investor.totalBalance.mul(20).div(100); 
                }else if( block.timestamp >= publicNextPay + 30 days){
                     investor.initialAmount = investor.totalBalance.mul(20).div(100);
                } else if( block.timestamp >= publicNextPay + 60 days){
                     investor.initialAmount = investor.totalBalance.mul(20).div(100);
                }else if(block.timestamp >= publicNextPay + 90 days){
                      investor.initialAmount = investor.totalBalance.mul(20).div(100);
                }
             ...
             if(saleType == 5){//launch
                ...
                if(block.timestamp >= launchNextPay){
                  ...
                  else if(block.timestamp>= launchNextPay +  60 ){
                    investor.initialAmount = investor.totalBalance.mul(25).div(100);

                }
                else if(block.timestamp >= launchNextPay + 120 ){
                    investor.initialAmount = investor.totalBalance.mul(25).div(100);
                }
                else if( block.timestamp >= launchNextPay + 180 ){
                  investor.initialAmount = investor.totalBalance.mul(25).div(100);   
                }
                investor.tokensPerUnit = investor.totalBalance.sub(investor.initialAmount).div(160);  
        }
