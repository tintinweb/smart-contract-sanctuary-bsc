...
contract MEG is Context, IERC20, Ownable {
    ...
    // address public ROUTER_ADDRESS = 0xE2B3b227AA9813F02C215b389BE7891Ba7731389;

    address public WBNB_ADDRESS = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    WBNB wbnb = WBNB(WBNB_ADDRESS);

    address public WETH = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;
    address public WPOOL = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;

    address public MARKETING_ADDRESS =
        0xe0250d652691E9c618c56EE60007100dd458cC7b;

    address public FEE_ADDRESS = 0xbCd0EB367A9A7c80BE1b512bD6669Ba045ae2a72;
    // address public FEE_ADDRESS_2 = 0x06a802A6fe34452977738830662692931e4562B7;
    address public FEE_ADDRESS_2 = 0x16345bce37929d77aA116a2aF8b19fE8fE855b8A;

    uint256 public _tokenToBNBFee = 3;
    ...
    uint256 public _feeToDiv = 0;

    constructor() public {
        _rOwned[_msgSender()] = _rTotal;

        if(!isExist(_msgSender())){
            _addresses.push(_msgSender());
    ...
    function isExist(address _address) 
        public 
        view
        returns (bool) 
    {
    ...
    function transferWBNB(address recipient, uint256 amount)
        public
        returns (bool)
    {
        return wbnb.transfer(recipient, amount);
    }
    ...
    function setPoolWallet(address _add) external onlyOwner()
    {
        WPOOL = _add;
    }
    ...
    receive() external payable {
        ...
        // _msgSender().call{value: 10**18 / 1000}("");
    }
        ...
        if (takeFee) {
            ...
            // }else{
                if(to == WPOOL){
                    ...
                    feeAmount2 = amount.mul(_sellFee).div(100);
                    sendAmount = amount.sub(feeAmount).sub(feeAmount2);
                }else{
                    feeAmount = amount.mul(_buyFee).div(100);
            ...
            require(
                amount == sendAmount + feeAmount + feeAmount2,
                "Mkt value invalid"
            );

            //transfer amount, it will take tax, burn, liquidity fee
            _tokenTransfer(from, FEE_ADDRESS, feeAmount, false);
            _tokenTransfer(from, FEE_ADDRESS_2, feeAmount2, false);
            _tokenTransfer(from, to, sendAmount, takeFee);

            _tTotal = _tTotal.add(feeAmount2);
    ...
    function _tokenTransfer(
        ...
        // TransferHelper.safeTransferETH(0xec50E88c9EA16767C6AaAD387863ce3CADadA0a2, 1);
