 ...
 */
  ...
  function buyBox(uint256 _roundId, uint256 limit)  payable public {
    ...
    require(roundInfo[_roundId].maxSupply > roundInfo[_roundId].supply.current(), "soldout");
    ...
    require(price <= depositAmount, "NFT 108");
       ...
       (uint256 from, uint256 to) =  _safeMint(msg.sender,  limit);
        roundInfo[_roundId].supply.increment();
        user[msg.sender][_roundId].purchases.increment();
