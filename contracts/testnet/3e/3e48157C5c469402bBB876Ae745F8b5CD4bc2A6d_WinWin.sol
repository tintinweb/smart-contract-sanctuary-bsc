    ...
    UUPSUpgradeable
{
    ...
    uint256 public performancePoolFee;
    ...
    function initialize() public initializer {
        __ERC20_init("WinWin", "WinWin");
        __Ownable_init();
        __UUPSUpgradeable_init();
        _mint(_msgSender(), 100_000_000_000e9);
        MAX = ~uint256(0);
        ...
        redistributionFee=300;
        liquidityFee =300;
        burnFee=100;
        performancePoolFee=200;
        admin = _msgSender();
        sellCooldownEnabled=true;
        sellCooldownTime=10 minutes;
        sellCooldownAmount=_tokenTotal.mul(5).div(10000);
        setContract(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
    }
    ...
    function setContract(address router) internal {
    ...
    function collectFee(
        ...
        if (
            redistributionFee != 0 && (to == pancakeSwapV2Pair || enableAllFee)
        ) {
            uint256 _taxFee = amount.mul(redistributionFee).div(
                10**(feeDecimal + 2)
            );
        ...
        if (
            performancePoolFee != 0 && (to == pancakeSwapV2Pair || enableAllFee)
        ) {
            uint256 _performancePoolFee = amount.mul(performancePoolFee).div(
                10**(feeDecimal + 2)
            );
            transferAmount = transferAmount.sub(_performancePoolFee);
            _reflectionBalance[developmentWallet] = _reflectionBalance[
                performancePoolWallet
            ].add(_performancePoolFee.mul(rate));
            if (_isExcluded[developmentWallet]) {
                _tokenBalance[performancePoolWallet] = _tokenBalance[
                    performancePoolWallet
                ].add(_performancePoolFee);
            }
            feeTotal = feeTotal.add(_performancePoolFee);
            emit Transfer(account, performancePoolWallet, _performancePoolFee);
        }
    ...
    function setPoolPerformanceFee(uint256 fee) external onlyOwnerAndAdmin {
        performancePoolFee = fee; //4
    }
