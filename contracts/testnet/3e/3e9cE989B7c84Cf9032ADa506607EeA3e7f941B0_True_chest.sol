...
mapping (address => mapping (uint256 => mapping (uint256 => mapping (uint256=>uint256)))) public LavelPlayer;

//address ZoomLoupe = dis.getZoomLoupe();
address ZoomLoupe;
...
public virtual override view  returns (uint256) {
    return  LavelPlayer[adr][num_track][num_chest][Num];
}

function Get_LevelPlayer_num (address adr,uint256 num_track, uint256 num_chest) 
public virtual override view  returns (uint256) {
     return  LavelPlayer[adr][num_track][num_chest][Get_chest(adr, num_track, num_chest)];
}
...
function Take_chest (uint256 num_track, uint256 num_chest,address adr) public virtual override {
    require(msg.sender == ZoomLoupe);
    LavelPlayer[adr][num_track][num_chest][Get_chest(adr, num_track, num_chest)]=0;
    if (num_chest == 1) {
...
function T_chest (address adr, uint256 num_track, uint256 num_chest, uint256 LavelPlayer_ ) external onlyOwner{
    if (num_chest == 1) {
        chest1[adr][num_track]++;
    }
    if (num_chest == 2) {
    ...
    LavelPlayer[adr][num_track][num_chest][Get_chest(adr, num_track, num_chest)]=LavelPlayer_;
}

function set_ZoomLoupe (address adr) public onlyOwner {
    ZoomLoupe=adr;
}
}
