...
contract SafeSwapTradeRouter is Initializable {
    ...
    function fetchSwapFees(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            if (_feekind == FeeKind.TOKEN_FEE) {
                _fees =
                    _fees +
                    getTokenSwapFee(amounts[i], TransactionType.SELL, path[i]) +
                    getTokenSwapFee(amounts[i + 1], TransactionType.BUY, path[i + 1]);

                // if (tokensFeeList[path[i]][TransactionType.SELL].tokenInfo.isEnabled) {
                //     _fees = _fees + getTokenSwapFee(amounts[i], TransactionType.SELL, path[i]);
                // }

                // if (tokensFeeList[path[i]][TransactionType.BUY].tokenInfo.isEnabled) {
                //     _fees = _fees + getTokenSwapFee(amounts[i + 1], TransactionType.BUY, path[i + 1]);
                // }
            } else {
    ...
    function getTokenSwapFee(
        ...
        if (
            ...
            try ISafeSwapRouter(swapRouter).getAmountsOut(_amount, tokenSwapPath) returns (uint256[] memory amounts) {
                _fee =
                    (amounts[amounts.length - 1] * tokensFeeList[_token][_transactionType].tokenInfo.feePercentage) /
                    percent;
            } catch {
                _fee = 0;
            }
        ...
        if (tokensFeeList[_assetIn][_transactionType].tokenInfo.isEnabled) {
            for (uint256 i; i < tokensFeeList[_assetIn][_transactionType].singleSwapFees.length; i++) {
                if (
                    ...
                    uint256 swapKindPercentage = (tokensFeeList[_assetIn][_transactionType]
                        .singleSwapFees[i]
                        .percentage * percent) / tokensFeeList[_assetIn][_transactionType].tokenInfo.feePercentage;
    ...
    function _claimTokenFees(address[] memory _path, uint256 _amountIn) private {
        ...
        for (uint256 i; i < _path.length - 1; i++) {
            // if (tokensFeeList[_path[i]][TransactionType.SELL].tokenInfo.isEnabled) {
            uint256 tokenAfeeAmount = getTokenSwapFee(amounts[i], TransactionType.SELL, _path[i]);
            _claimTokenFee(_path[i], TransactionType.SELL, tokenAfeeAmount);
            // }

            // if (tokensFeeList[_path[i]][TransactionType.BUY].tokenInfo.isEnabled) {
            uint256 tokenBfeeAmount = getTokenSwapFee(amounts[i + 1], TransactionType.BUY, _path[i + 1]);
