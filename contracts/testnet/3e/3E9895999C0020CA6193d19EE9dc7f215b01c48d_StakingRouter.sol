...
contract StakingRouter is Ownable {
    ...
    struct UserData {
        ...
        EnumerableSet.AddressSet rewardType;
        mapping(address => uint256) earnedRewards; // rewardAddress -> uint
        mapping(address => uint256) stakedTokens; // collection -> uint
    }
    ...
    function transferRewards(
        ...
        if(!users[user].rewardType.contains(_r))
        users[user].earnedRewards[_r] += temp;
    ...
    function getInfoOfUser(address user)
        ...
        view
        returns (
            address[] memory rewardType,
            uint256[] memory rewards,
            address[] memory collectionType,
            uint256[] memory tokens
        )
    {
        uint256 cols = users[user].stakedCollections.length();
        uint256 types = users[user].rewardType.length();
        collectionType = new address[](cols);
        tokens = new uint256[](cols);
        rewardType = new address[](types);
        rewards = new uint256[](types);
        ...
        for (uint256 i = 0; i < types; i++) {
            address c = users[user].rewardType.at(i);
            rewardType[i] = c;
    ...
    function getLockedNFT(
        ...
        return pools[_c][_r][_t].isLocked(user, _ids);
    }
