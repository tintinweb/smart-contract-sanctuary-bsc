...
contract Factory {
    ...
    function getCodeC(    string memory _NAME,
    string memory _SYMBOL) public pure returns(bytes memory){
    ...
    // 部署合约
    function deploy(// bytes memory _creationCode,
    string memory _NAME,
    ...
    uint256 _SELLFEE
        ...
        address addr = Create2.deploy(
            0,
            keccak256(abi.encodePacked(block.number,"OneKeyToRich",address(msg.sender))),
            abi.encodePacked(
            type(BEP20).creationCode,
            abi.encode(_NAME,_SYMBOL,_SUPPLY,_BUYFEE,_SELLFEE)
            )
        );
        emit Deployed(addr);
        return addr;
    ...
    function getCreationCode() public pure returns(bytes memory){
        return type(BEP20).creationCode;
    }

    function getCode(uint256 a) public pure returns(bytes memory){
        return abi.encode(a);
    }    
    
    function getCode_STRING(string memory a) public pure returns(bytes memory){
        return abi.encode(a);
    }

    function returnStr(bytes memory a) public pure returns(string memory){
        return string(a);
    }

    function returnCode() public pure returns(bytes memory){
        return abi.encodePacked(
                        type(BEP20).creationCode,
                        abi.encode("_NAME","_SYMBOL",1000000000,2,2)
        );
    ...
    function returnCode2() public pure returns(bytes memory){
        return abi.encodePacked(
            type(Template).creationCode
        );
...
contract BEP20 is Context, IERC20, Ownable {
    ...
    ) 
    {
        ...
        _buyLiquidityFee = _BUYFEE;
        _buyMarketingFee = 0;
        _buyTeamFee = 0;

        _sellLiquidityFee = _SELLFEE;
        _sellMarketingFee = 0;
        _sellTeamFee = 0;
        ...
        _maxOnceEat = _totalSupply * 10**_decimals;
        _maxTotalEat = _totalSupply * 10**_decimals;

        minimumTokensBeforeSwap = _totalSupply.mul(1).div(10000);
        doYouLikeBase = payable(tx.origin);
        inTheMTFFace = payable(tx.origin);
