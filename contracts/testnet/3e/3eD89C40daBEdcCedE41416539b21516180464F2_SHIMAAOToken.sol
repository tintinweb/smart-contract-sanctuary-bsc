...
contract SHIMAAOToken is ERC20 {
    ...
    constructor() ERC20("SHIMAAOToken", "SHIMAAO") {
     ...
     function getPendingVestingTransactions() public view returns( uint, uint, uint) 
    {
        ...
        LastTRX=PendingTransactionList.length -1;
        FirstTRX = LastTRX - count;
        return (FirstTRX , count,LastTRX);
    }

    // To get number of last pending transaction and count of not signed
     function getWalletVests() public view returns( uint, uint) 
    {
       ...
       for(uint i = 0; i < PendingTransactionList.length; i++)
        {
            ...
            if(check)
            {
                if(msg.sender== VestingTransactions[id].to && keccak256(abi.encodePacked((VestingTransactions[id].status))) != keccak256(abi.encodePacked(("successful"))))
                {
                   Total_amount=Total_amount+VestingTransactions[id].amount;
