...
contract NftMarket is Ownable, ReentrancyGuard {
    ...
    modifier validListing(uint _listing) {
        ...
        require(listings[_listing].saleEnd == 0 || block.timestamp <= listings[_listing].saleEnd, 'Listing: sale ended');
        _;
    }
    ...
    */
    ...
    function setBep20(address _addr, uint _marketTax, bool _enabled) public {
    ...
    */
    ...
    function directBuyBnb(uint _listing, address[] memory _discountAddresses) public payable validListing(_listing) nonReentrant() {
        ...
        _payBnb(listings[_listing].targetPrice, msg.sender, _listing, calculateDiscount(msg.sender, _discountAddresses));
    ...
    function directBuyBep20(uint _listing, address[] memory _discountAddresses) public validListing(_listing) nonReentrant() {
        ...
        _payBep20(listings[_listing].targetPrice, msg.sender, _listing, calculateDiscount(msg.sender, _discountAddresses));
        _sendNft(_listing, msg.sender);
    ...
    function createOffer(uint _listing, uint _amount, address _paymentToken, address[] memory _discountAddresses) public payable validListing(_listing) {
        ...
        require(listing.saleType == SaleType.OFFER || listing.saleType == SaleType.BOTH, 'Listing: must be of type Offer or Both');
    ...
    */
    ...
    function listNft(address _nft, uint _tokenId, PaymentMethod _paymentMethod, address _paymentToken, SaleType _saleType, uint _targetPrice, uint _saleEnd) public {
        ...
        if(_paymentMethod == PaymentMethod.BNB) {
            require(_paymentToken == address(0), 'Payment Token: must be zero address for BNB listings');
        } else {
            require(bep20Info[_paymentToken].enabled, 'Payment Token: must be an enabled BEP20');
        }
    ...
    */
    ...
    function calculateDiscount(address user, address[] memory discountAddresses) public view returns (uint) {
        ...
        for (uint x = 0; x < discountAddresses.length; x++) {
            if (IDiscount(discountAddresses[x]).isApplicable(user)) {
    ...
    */
    ...
    function _payBep20(uint _amount, address _buyer, uint _listing, uint _discount) private {
        ...
        if(_discount > paymentInfo.marketTax) {
            _discount = paymentInfo.marketTax;
        }
    ...
    function _payBnb(uint _amount, address _buyer, uint _listing, uint _discount) private {
        ...
        if(taxRebateBP > marketTaxBnb) {
            taxRebateBP = marketTaxBnb;
        }

        if (marketTaxBnb - taxRebateBP < minTax) {
            taxRebateBP -= minTax;
        }
        require(taxRebateBP >= minTax, 'Discount error: taxRebate < minTax');
        remainingTaxBP -= taxRebateBP;
        require(remainingTaxBP >= minTax, 'Discount error: remainingTxax < minTax');
        ...
        require(IBEP20(zombie).balanceOf(address(this)) == initialZombieBalance + boughtZmbe, 'Buyback error occurred');
        IBEP20(zombie).approve(address(rebatesManager), boughtZmbe);
        rebatesManager.addUserRebate(_buyer, block.timestamp, boughtZmbe);
        require(IBEP20(zombie).balanceOf(address(this)) == initialZombieBalance, 'Rebates error occurred');
