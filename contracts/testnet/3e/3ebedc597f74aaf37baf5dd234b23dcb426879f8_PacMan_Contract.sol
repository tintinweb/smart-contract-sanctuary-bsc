...
contract PacMan_Contract is Context, Ownable {
    ...
    uint256 private dailyReturn = 80;
    ...
    function buyGhosts(address referrer) public payable {
        ...
        address _upline3 =  rewards[_upline2].upline2; 
        address _upline4 =  rewards[_upline3].upline3;
    ...
    function sellGhosts() public {
        require(initialized);
        require(block.timestamp > timelock[msg.sender].deadline, "You are not allowed to withdraw");
        uint256 timer = block.timestamp + 1 days;
        timelock[msg.sender] = Timelock(msg.sender, timer);
        ...
        uint256 fee = projectFee(soldGhost);
        claimedGhosts[msg.sender] = soldGhost;
        lastHatch[msg.sender] = block.timestamp;
        marketGhosts = SafeMath.add(marketGhosts, hasGhosts);
        projectAddress.transfer(fee);
        payable (msg.sender).transfer(SafeMath.sub(soldGhost, fee));
    }
