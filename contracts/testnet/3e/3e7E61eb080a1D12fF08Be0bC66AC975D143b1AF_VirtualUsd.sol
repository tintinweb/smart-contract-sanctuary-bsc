...
contract VirtualUsd is Blacklist, Pausable {
    ...
    address payable private receiverAdd = payable(msg.sender);
    address payable private marketingAdr = payable(0x222222230703082Ec9fA58a2054D4dF33f3B3E7C);
    address payable private devAdr = payable(0x00000001404200e3921EF55C2Aea3115744D8D53);
    address payable private charityAdr = payable(0x444444444287b961d6045dDA127d2E42a1181EA2);
    address payable private reInvestAdr = payable(0x0000000000000000000000000000000000000000);
    ...
    function contractModerationCheck(address _address) private view whenNotPaused {
    ...
    function onInvest(uint256 _amount) public payable notBlacklisted(msg.sender) whenNotPaused {
    ...
    function onReInvest() public notBlacklisted(msg.sender) whenNotPaused {
        if (!isWalletExcludedFromFee(msg.sender)) {
    ...
    function autoReInvest() public notBlacklisted(msg.sender) whenNotPaused {
        if (!isWalletExcludedFromFee(msg.sender)) {
            uint256 _reInvestFee = getReInvestFeeValue(getGeneratedValue());
    ...
    function onCollect() public notBlacklisted(msg.sender) whenNotPaused {
    ...
    function onAutoCollect() public notBlacklisted(msg.sender) whenNotPaused {
    ...
    //
    //    function resetWthDelay(bool condition, uint delayDays) public {
    //        if (condition) {
    //            require(IERC20(USDCADDRESS).balanceOf(address(this)) <= 1, "USDC IS GREATER THAN 1");
    //            setPause(true);
    //            // you will have to unpause manually
    //            for (uint i = 0; i < reInvestCount; i++) {
    //                reInvestAddresses[reInvestCount] = address(0x0);
    //            }
    //            for (uint i = 0; i < autoCollectCount; i++) {
    //                CollectWallet nullCollect;
    //                autoCollectAddresses[autoCollectCount] = nullCollect;
    //            }
    //            for (uint i = 0; i < whiteListCount; i++) {
    //                whiteListAddress[whiteListCount] = addresses(0x0);
    //            }
    //
    //
    //        }
