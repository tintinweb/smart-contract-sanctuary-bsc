...
contract SpynGovernor is Governor, GovernorCompatibilityBravo , GovernorTimelockControl {
    ...
    constructor(
        ...
        address _spyn, 
        address _masterchef,
        uint256[] memory _spynFarmIds, 
        address[] memory _compoundPools)
        Governor("SPYNCompanyReserveGovernor")
    ...
    function setSpynFarmIds(uint256[] memory _spynFarmIds) public onlyDev {
        spynFarmIds = _spynFarmIds;
    }
    ...
    function voteWeightForProposal(uint256 proposalId, address account) public view returns (uint256) {
        uint256 snapshot = proposalSnapshot(proposalId);
        return spyn.balanceOf(account) + extraWeights[account][snapshot];
    }
    ...
    function calculateWeight(uint256 proposalId, address account) public {
        ...
        for (uint256 i = 0; i < spynFarmIds.length; i ++) {
            (address lpToken,,,,,,,) = masterchef.poolInfo(spynFarmIds[i]);
            (uint256 amount,,,) = masterchef.userInfo(spynFarmIds[i], account);
            uint256 totalSupply = IUniswapV2Pair(lpToken).totalSupply();
            uint256 balance0 = spyn.balanceOf(lpToken);
    ...
    function getVotes(address account, uint256 blockNumber)
        ...
        returns (uint256)
    {
        return spyn.balanceOf(account) + extraWeights[account][blockNumber];
    }
    ...
    function cancel(uint256 proposalId) public virtual override (GovernorCompatibilityBravo) {
        ...
        if(rewardLeft > 0){
            if(spyn.balanceOf(address(this)) < rewardLeft){
                rewardLeft = spyn.balanceOf(address(this));
            }
            spyn.transfer(_msgSender(), rewardLeft);
        }
    }
        ...
        returns (uint256)
    {
        ...
        spyn.transferFrom(_msgSender(), address(this), totalReward);
    ...
    ) internal virtual override(Governor) returns (uint256)
    {
        ...
        if(proposalReward[proposalId].totalDistributed < proposalReward[proposalId].totalCountedReward){
            ...
            if(spyn.balanceOf(address(this)) > myReward){
                spyn.transfer(_msgSender(), myReward);
