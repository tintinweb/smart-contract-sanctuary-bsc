...
contract SmartLP is SmartLPStorage, IBEP721, IBEP721Metadata {
    ...
    function getTokenRewardsAmounts(uint tokenId) public view returns (uint lpBnbNimbUserRewards, uint lpBnbGnbuUserRewards) {
        UserSupply memory userSupply = tikSupplies[tokenId];
        require(userSupply.IsActive, "SmartLP: Not active");
    ...
    function getTotalAmountsOfRewards(uint tokenId) public view returns (uint nbuReward) {
        (uint lpBnbNimbUserRewards, uint lpBnbGnbuUserRewards) = getTokenRewardsAmounts(tokenId);
    ...
    function getNftLpInfo(uint256 tokenId) public view returns (uint lpNimbBnb, uint lpNimbToken, uint lpGnimbBnb, uint lpGnimbToken) {
        ...
        uint256 gnimbPairPerc = tikSupplies[tokenId].GnbuBnbLpAmount / curGnimbPair.totalSupply();
        uint256 nimbPairPerc = tikSupplies[tokenId].NbuBnbLpAmount / curNimbPair.totalSupply();

        bool gb = curGnimbPair.token0() == swapRouter.NBU_WBNB();
        lpGnimbBnb = gnimbPairPerc * (gb ? gnimbReserve0 : gnimbReserve1);
        lpGnimbToken = gnimbPairPerc * (gb ? gnimbReserve1 : gnimbReserve0);

        bool nb = curNimbPair.token0() == swapRouter.NBU_WBNB();
        lpNimbBnb = nimbPairPerc * (nb ? nimbReserve0 : nimbReserve1);
        lpNimbToken = nimbPairPerc * (nb ? nimbReserve1 : nimbReserve0);
    }
