...
contract LayerZeroBridge is ILayerZeroReceiver, IBridge {
    ...
    function _deployNonNativeWrapper(DeployExternalParams memory deployData, bytes32 collectionId) internal {
        ...
        factory.createContractInstanceByName(
            ERC721_NON_NATIVE_WRAPPER_BLUEPRINT_NAME, abi.encode(systemContext, deployData.originalCollection), collectionId, deployData.collectionName, deployData.owner
        );
    ...
    function deployContractEncode(string memory blueprintName_, bytes32 collectionId_, bytes calldata ctorParams_, string calldata collectionName_, address owner_) public view returns (bytes memory) {
        return abi.encode(
            Data({
    ...
    function deployExternalCollection(
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            ...
            _sendMessageWithValue(
                ...
                deployContractEncode(ERC721_BLUEPRINT_NAME, collectionId_, deploymentParams_[i].params, params.collectionName, params.owner),
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            ...
            if(deploymentParams_[i].chainId == systemContext.chainId()) {
                systemContext.getContractFactory().createContractInstanceByName(
                    ERC721_WITH_SLOTS_BLUEPRINT_NAME, deploymentParams_[i].params, collectionId_, params.collectionName, params.owner);
            } else {
                _sendMessageWithValue(
                    deploymentParams_[i].chainId,
                    deploymentParams_[i].bridgeAddress,
                    deployContractEncode(ERC721_WITH_SLOTS_BLUEPRINT_NAME, collectionId_, deploymentParams_[i].params, params.collectionName, params.owner),
                    params.refundAddress,
                    deploymentParams_[i].value,
                    params.gasAmount
                );
 ...
 */
contract ERC721Mintable is ERC721EnumerableUpgradeable, ITokenBridgeable, OwnableUpgradeable {
    ...
    function initialize(string memory name_, string memory symbol_, address owner_, SystemContext system_) initializer public {
        _initializeUnchainedInternal(name_, symbol_, owner_, system_);
    }

    function _initializeUnchainedInternal(string memory name_, string memory symbol_, address owner_, SystemContext system_) onlyInitializing internal {
        system = system_;
