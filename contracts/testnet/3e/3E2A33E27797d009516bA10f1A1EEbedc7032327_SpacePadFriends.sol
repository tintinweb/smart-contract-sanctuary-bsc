...
contract SpacePadFriends is Pausable, Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(uint256 => uint256)) private walletRoundDeposits;

    mapping(address => ReferrallDeposits[]) private referralDeposits;

    mapping(address => ReferralEaarnings) public referralEarnings;

    uint256 public currentRoundIndex = 1;
    ...
    function createRound(
        ...
        uint256 _singleWalletDepositsLimit,
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < currentRoundIndex; i++) {
            if (spacePadRound[i].valid) {
                currentRound = spacePadRound[i];
            }
        }

        uint256 roundWalletDepositsAmount = walletRoundDeposits[msg.sender][
            currentRound.roundIndex
        ];

        require(
            roundWalletDepositsAmount < currentRound.singleWalletDepositsLimit,
            "The deposits amount for the round has been reached"
        );

        require(currentRound.valid, "No active round at the moment");
        ...
        uint256 unitsAmountSold = getSoldUnitsValue(currentRound);
        currentRound.roundUnitsLeft -= unitsAmountSold;

        spacePadRound[currentRound.roundIndex] = currentRound;
        roundWalletDepositsAmount += 1;
        Referral(spacePadReferralAddress).setReferral(msg.sender, _referrer);
        emit Deposited(_referrer, _amount);
    }

    function withdraw(address _recipient, uint256 _roundIndex)
        external
        whenNotPaused
        onlyOwner
    {
    ...
    function depositOnReferrer(
        ...
        ReferralEaarnings storage earnings = referralEarnings[_referrer];
        ...
        uint8 level = 1;
        if (_referralFee == level1ReferrerFee) {
            earnings.level1 += referralFeeAmount;
        }
        if (_referralFee == level2ReferrerFee) {
            earnings.level2 += referralFeeAmount;
            level = 2;
        }
        if (_referralFee == level3ReferrerFee) {
            earnings.level3 += referralFeeAmount;
            level = 3;
        }

        earnings.all += referralFeeAmount;
    ...
    function walletReferrals(address referralAddress)
        public
        view
        returns (ReferrallDeposits[] memory)
    {
        return referralDeposits[referralAddress];
    }
    ...
    function setFee(
        ...
        require(
            _level1Fee < 10000 || _level2Fee < 10000 || _level3Fee < 10000,
            "One of fees is too high, maximum single value is 10000"
        );
    ...
    function getSoldUnitsValue(
        SpacePad.SpacePadRoundConfiguration memory currentRound
    ) private pure returns (uint256) {
        uint256 currentCapFromUnitsSold = 0;
        uint256 unitPrice = currentRound.initialUnitPrice;
        uint256 unitsSold = 0;
        do {
            if (
                currentRound.currentCap - currentCapFromUnitsSold <=
                unitPrice * currentRound.nextStepDepositsAmountIncrease
            ) {
                uint256 capLeft = currentRound.currentCap -
                    currentCapFromUnitsSold;
                currentCapFromUnitsSold += capLeft;
                unitsSold += capLeft / unitPrice;
            } else {
                currentCapFromUnitsSold +=
                    unitPrice *
                    currentRound.nextStepDepositsAmountIncrease;
                unitsSold += currentRound.nextStepDepositsAmountIncrease;
            }

            unitPrice += currentRound.nextStepUnitPriceIncrease;
        } while (currentCapFromUnitsSold == currentRound.currentCap);
    ...
    function setSpacePadReferralAddress(address _spacePadReferral)
        external
        onlyOwner
    {
