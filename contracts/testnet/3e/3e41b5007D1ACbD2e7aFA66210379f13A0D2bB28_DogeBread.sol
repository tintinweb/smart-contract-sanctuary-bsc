...
// File: DogeBread/treasury/ITreasuryHandler.sol
...
// File: DogeBread/governance/IGovernanceToken.sol
...
/**
 * @title DogeBread token contract
 */
contract DogeBread is IERC20, IGovernanceToken, Ownable {
    ...
    function transferFrom(
        ...
        require(
            ...
            "DogeBread:transferFrom:ALLOWANCE_EXCEEDED: Transfer amount exceeds allowance."
        );
        unchecked {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) external returns (bool) {
        ...
        require(
            ...
            "DogeBread:decreaseAllowance:ALLOWANCE_UNDERFLOW: Subtraction results in sub-zero allowance."
        );
        unchecked {
    ...
    function delegateBySig(
        ...
        require(signatory != address(0), "DogeBread:delegateBySig:INVALID_SIGNATURE: Received signature was invalid.");
        require(block.timestamp <= expiry, "DogeBread:delegateBySig:EXPIRED_SIGNATURE: Received signature has expired.");
        require(nonce == nonces[signatory]++, "DogeBread:delegateBySig:INVALID_NONCE: Received nonce was invalid.");
    ...
    function getVotesAtBlock(address account, uint32 blockNumber) public view returns (uint224) {
        require(
            ...
            "DogeBread:getVotesAtBlock:FUTURE_BLOCK: Cannot get votes at a block in the future."
        );
    ...
    function _approve(
        ...
        require(owner != address(0), "DogeBread:_approve:OWNER_ZERO: Cannot approve for the zero address.");
        require(spender != address(0), "DogeBread:_approve:SPENDER_ZERO: Cannot approve to the zero address.");
    ...
    function _transfer(
        ...
        require(from != address(0), "DogeBread:_transfer:FROM_ZERO: Cannot transfer from the zero address.");
        require(to != address(0), "DogeBread:_transfer:TO_ZERO: Cannot transfer to the zero address.");
        require(amount > 0, "DogeBread:_transfer:ZERO_AMOUNT: Transfer amount must be greater than zero.");
        require(amount <= _balances[from], "DogeBread:_transfer:INSUFFICIENT_BALANCE: Transfer amount exceeds balance.");
