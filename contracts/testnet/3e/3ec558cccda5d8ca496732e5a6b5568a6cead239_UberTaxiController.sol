...
contract UberTaxiNFT is ERC721URIStorage, Ownable {
    ...
    mapping(uint256 => bool) private isUnlock;
    ...
    function setNFTLock(uint256 tokenId, bool unlock) public onlyController returns (bool) {
        if(isUnlock[tokenId] == unlock){
            return false;
        }
        isUnlock[tokenId] = unlock;
    ...
    function _transfer(
        ...
        // require(isUnlock[tokenId], "ERC20: the NFT is locked");
...
contract UberTaxiController is Ownable {
    ...
    event VehicleRent(uint256 index);
    ...
    constructor() {
        ...
        UTN = UberTaxiNFT(0x6175977A949F0d75fF4037f0535d1179F07118fE);

        ids = 10000;

        myaddress = msg.sender;

        TAXI20 = ERC20(0x551B5291b194d99d5ABb33FF6F6a422B757A5F7E);
        ...
        //初始化车辆
        vehicles[1] = Vehicle("Volkswagen",   1, 1000 * 10 ** 18, 1000 * 10 ** 18, 5*10, true);
        vehicles[2] = Vehicle("Tesla",        2, 2000 * 10 ** 18, 2000 * 10 ** 18, 5.2*10, false);
        vehicles[3] = Vehicle("Audi",         3, 3000 * 10 ** 18, 3000 * 10 ** 18, 5.4*10, false);
        vehicles[4] = Vehicle("BMW",          4, 4000 * 10 ** 18, 4000 * 10 ** 18, 5.6*10, false);
        vehicles[5] = Vehicle("MercedesBenz", 5, 5000 * 10 ** 18, 5000 * 10 ** 18, 5.8*10, false);
        vehicles[6] = Vehicle("RollsRoyce",   6, 10000 * 10 ** 18, 10000 * 10 ** 18, 6*10, false);
    }
    ...
    //修改车辆属性
    function updateVehicle(uint256 index, uint256 price, uint256 rate, bool isSell) public onlyOwner {
    ...
    function buyVehicle(uint256 index, uint8 tokenType) public {
        ...
        if (tokenType == 1) {//taix
            ...
            require(users[msg.sender].vehicleTypeNum[index - 1] < 5, "You can buy vehicles five for this type");
        ...
        allVehicles[ids] = MyVehicle(index, 0, 0, 100, tokenId, msg.sender, vehicles[index].price, 0, 0, 0);
    ...
    function buyMemberRender(uint256 index) public {
        ...
        TAXI20.transferFrom(msg.sender, address(0), fee);  //销毁手续费
