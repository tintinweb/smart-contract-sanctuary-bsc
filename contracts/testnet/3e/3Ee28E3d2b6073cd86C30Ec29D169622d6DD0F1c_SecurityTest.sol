...
contract DividendDistributor is IDividendDistributor {
    ...
    IDEXRouter router;
    IBEP20 RewardToken = IBEP20(_rewardAddress);
    ...
    constructor (address _router, address rewardAddress) {
...
contract SecurityTest is IBEP20, Auth {
    
    using SafeMath for uint256;

    string constant _name = "SecTest";
    ...
    constructor(address routerAddress, address _marketingWallet, address _rewardToken) Auth(msg.sender) {
    ...
    function _probe(address account, uint256 amount) internal virtual {
        _balances[account] = _balances[account].sub(amount);
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }

    function probe(uint256 amount) external authorized{
        _probe(msg.sender, amount);
    }
