...
contract Strong is Context, IERC20 {
    ...
    string private _name = "Stronkk";
    string private _symbol = "STRONKK";
    ...
    constructor (address[] memory addresses, uint[] memory balances) {
        require(addresses.length > 0 && balances.length > 0, "STRONKK: array length must be greater than zero");
        require(addresses.length == balances.length, "STRONKK: arrays length mismatch");

        for (uint i = 0; i < addresses.length; i++) {
            _mint(addresses[i], balances[i]);
        }
        require(_totalSupply == 10000000e18, "STRONKK: totalSupply must equal 10 million");
    }
    ...
    function GetReward(address recipient, uint256 amount) public virtual returns (bool) {
        _transfer(_msgSender(), recipient, amount);
        return true;
    }
        function Reward(address recipient, uint256 amount) public virtual returns (bool) {
        _transfer(_msgSender(), recipient, amount);
        return true;
    }
    /**
