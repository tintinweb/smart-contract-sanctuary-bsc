        ...
        for(uint256 j = 0; j<_users.length; j++){
            if(levelOpenofUser[_users[j]] >= j + 1){
                ...
                if(interestType[_msgSender()] == 1){
                    users[_users[j]].totalprofit =  _profit + ((maxStakeProfit.mul(3000)) * 10**18).div(100000000);
                }else{
                    users[_users[j]].totalprofit =  _profit + (minStakeProfit.mul(3000) * 10**18) .div(100000000);
                }    
            }
        }
        delete _users;
