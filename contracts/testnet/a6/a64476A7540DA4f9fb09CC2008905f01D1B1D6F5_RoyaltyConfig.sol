...
contract RoyaltyConfig is Configurable {
    ...
    event CollectionConfigSet(address indexed sender, address indexed collection, address indexed royaltyReceiver, uint royaltyRatio);
    event ChargeFeeETH(address indexed platformReceiver, address indexed royaltyReceiver, address indexed collection, uint platformFee, uint royaltyFee);
    event ChargeFeeToken(address indexed platformReceiver, address indexed royaltyReceiver, address indexed collection, address token, uint platformFee, uint royaltyFee);

    function initialize(address signer_) public override initializer {
        super.initialize(msg.sender);
        signer = signer_;

        config[PlatformFeeReceiver] = uint(uint160(address(this)));
    ...
    function setPlatFormatReceiver(address feeReceiver) external governance {
        config[PlatformFeeReceiver] = uint(uint160(feeReceiver));
    }
    ...
    function getPlatformFeeReceiver() public view returns (address) {
        return address(uint160(config[PlatformFeeReceiver]));
    }
