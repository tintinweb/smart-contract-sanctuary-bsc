    ...
    ReentrancyGuard
{
    ...
    function mint(uint256 count) external nonReentrant {
        ...
        // Transfer coin token.
        coinToken.transferFrom(to, vault, design.getMintCost() * count);
    ...
    function processTokenRequests() external nonReentrant {
        address to = msg.sender;
        uint256 size = tokenIds[to].length;
        uint256 limit = design.getTokenLimit();
        require(size < limit, "User limit reached");
        ...
        for (uint256 i = requests.length; i > 0; --i) {
            ...
            // console.log("\x1b[36m%s\x1b[0m", "seed", seed);
            // console.logBytes32(blockhash(targetBlock));
    ...
    function upgradePowerDigger(uint256 combineDiggerId, uint256 diggerId)
        external
        nonReentrant
        returns (uint256 power)
    {
        // Transfer coin token.
        coinToken.transferFrom(
            msg.sender,
            vault,
            design.getUpgradePowerDiggerCost()
        );
    ...
    function rent(uint256 diggerId) external {
        require(tokenDetails[diggerId] > 0, "Digger not found");

        // transfer MINERA token to vault
        coinToken.transferFrom(msg.sender, vault, design.getMintCost() / 2);
    ...
    function upgrade(uint256 baseId, uint256 materialId) external nonReentrant {
        ...
        coinToken.transferFrom(to, vault, cost);
    ...
    // function diggerStats(uint256 id)
    //     external
    //     view
    //     returns (DiggerDetails.Details memory)
    // {
    //     return DiggerDetails.decode(tokenDetails[id]);
    // }
...
/**
 * @dev Interface of the ERC20 standard. Does not include
 ...
 */
contract ERC20 is BEPContext, IERC20, BEPPausable {
