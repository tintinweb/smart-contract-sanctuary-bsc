...
contract MbtcSales is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(IERC20Upgradeable _sellToken, uint256 _startTime, uint256 _endTime, IERC20Upgradeable[] calldata _buyWiths, uint256[] calldata decimals, address _beneficiary) public initializer {
        ...
        endTime = _endTime;
        sellPrice = 0.008 ether; //this actually means 0.008 usd
	...
	function buy(IERC20Upgradeable buyWith, uint256 amount) public nonReentrant {
		...
		// this is abit complicated, let me explain a little bit
		// ((amount * sellPrice) / MAX_DECIMALS) returns 0.008 as human readable price
		// * 10 ** selectedDecimals converts to correct integer format of selectedToken
		// since solidity doesn't support decimal point (0.0008), to avoid this, we use formula below instead
		
		uint256 reqTokenBal = ((amount * sellPrice) * (10 ** selectedDecimals)) / (10 ** MAX_DECIMALS);
		
		//and since compilerV8 protects us from integer overflow/underflow, so do not worry much about that!
		
		selectedToken.safeTransferFrom(msg.sender, beneficiary, reqTokenBal);
