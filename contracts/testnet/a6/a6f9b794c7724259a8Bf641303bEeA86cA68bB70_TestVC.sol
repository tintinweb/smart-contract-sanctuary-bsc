...
function setWhitelistforProject(uint64 ProjectID, bool isRequired) public onlyMain()
{
  WhitelistRequired[ProjectID] = isRequired;
}

function getProjectInvestorList(uint64 ProjectID) public view returns (address[] memory Investorlist)
{
   for(uint64 i = 0; i < N_Investors[ProjectID]; i++)
   {
      Investorlist[i] = _investorID[ProjectID][i];
   }

   return Investorlist;
}

function getProjectInvestmentsList(uint64 ProjectID) public view returns (uint256[] memory Investmentlist)
{
   for(uint64 i = 0; i < N_Investors[ProjectID]; i++)
   {
      Investmentlist[i] = _totalUSDshare[_investorID[ProjectID][i]][ProjectID];
   }

   return Investmentlist;
}

function getProjectRemainingUSDList(uint64 ProjectID) public view returns (uint256[] memory RemainingUSDlist)
{
   for(uint64 i = 0; i < N_Investors[ProjectID]; i++)
   {
      RemainingUSDlist[i] = _balanceUSD[_investorID[ProjectID][i]][ProjectID];
   }

   return RemainingUSDlist;
}

function getProjectTokensClaimedList(uint64 ProjectID) public view returns (uint256[] memory TokensClaimedlist)
{
   for(uint64 i = 0; i < N_Investors[ProjectID]; i++)
   {
      TokensClaimedlist[i] = _tokensReceived[_investorID[ProjectID][i]][ProjectID];
   }

   return TokensClaimedlist;
}
