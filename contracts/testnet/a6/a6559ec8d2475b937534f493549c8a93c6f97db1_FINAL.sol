...
contract FINAL is Context, IERC20, Ownable {
    ...
    string private _name = "FINAL3";
    string private _symbol = "FCHT3";
    ...
    address public Wallet_Prize = 0x74762D067EAe44a58a43FFA4A5C9527E19a5957f;
    ...
    uint256 public _minHoldToClaim = 1 * 10**5 * 10**9; //10,000 units to be eligible

    uint256 private _teamSellLimit = 5 * 10**5 * 10**9; //Team Wallet sell limits after unlock date
    uint256 private _teamSellStart = 	1652446212; //Team can sell units starting June 10, 2025 12:00:00 GMT
    uint256 private _presaleEnd = 	1652446212; //Remove from fee open until July 10, 2022 12:00:00 GMT
    ...
    function checkLastDonated (address account) public view returns (
        uint256 lastDonate,
        uint256 donate500,
        uint256 donate2500,
        uint256 donate10K,
        ...
        lastDonate = transferTimeFrom[account];
        ...
        return (lastDonate, donate500, donate2500, donate10K, donate100K);
    }
    ...
    function checkClaimEligibility(address account) public view returns (
        ...
        if (balanceOf(account) <= _minHoldToClaim){
            return (balanceOf(account), 0, 0, 0, 0, "Hold at least 10,000 to claim.");
        }                      
        else if (trans >= 157784630){ //5 years hold
            return (balanceOf(account), lastTransfer, trans, 60, 8, "TBA");
        }
        else if (trans >= 110449241){ //42 months hold
            return (balanceOf(account), lastTransfer, trans, 42, 7, "TBA");
        }
        else if (trans >= 63113852){ //24 months hold
            return (balanceOf(account), lastTransfer, trans, 24, 6, "8X multiplier");
        }
        else if (trans >= 47335389){ //18 months hold
            return (balanceOf(account), lastTransfer, trans, 18, 5, "5X multiplier");
        }
        else if (trans >= 31556926){ //12 months hold
            return (balanceOf(account), lastTransfer, trans, 12, 4, "3X multiplier");
        }
        else if (trans >= 15778458){ //6 months hold
            return (balanceOf(account), lastTransfer, trans, 6, 3, "1X multiplier");
        }
        else if (trans >= 7889229){ //3 months hold
            return (balanceOf(account), lastTransfer, trans, 3, 2, "0.5X multiplier");
        }
        else if (trans >= 2629743){ //1 month hold
            return (balanceOf(account), lastTransfer, trans, 1, 1, "0.25X multiplier");
        }
        else if (trans >= 300){ //5 minute test
            return (balanceOf(account), lastTransfer, trans, 50, 50, "5 minute test");
        }
        else if (trans > 120){ //2 minute test
            return (balanceOf(account), lastTransfer, trans, 20, 20, "2 minute test");
        }
        else if (trans == block.timestamp){
            return (balanceOf(account), transferTimeTo[account], 0, 0, 0, "Diamond Hands!");
            }
        else {
            return (balanceOf(account), 0, 0, 0, 0, "N/A");
        }
    ...
    function _transfer(
        ...
        if (_isSniper[from])
            {
            if (block.timestamp <= 1652532612) { // Will be allowed transfer until June 10, 2028 12:00:00 GMT
            require ((to == uniswapV2Pair
                    ...
                    && (block.timestamp >= transferTimeFrom[from] + 5 minutes), // Can only transfer every 5 minutes
        ...
        if (_isTeamWallet[from])
            ...
            require (_isExcluded[to] // Can only sell or burn, cannot transfer to others
                    ...
                    && block.timestamp >= transferTimeFrom[from] + 5 minutes, // Can only sell once every 5 minutes
        ...
        if (_isEmploymentWallet[from]) 
            ...
            require (to == Wallet_Treasury || to == uniswapV2Pair // Can only sell or transfer to Treasury
                    ...
                    && block.timestamp > transferTimeFrom[from] + 10 minutes, // Can only sell once every 10 mins
        ...
        if (_isDonationRecipient[to])
            ...
            require (from != owner() && ((heldTokens + amount) <= _maxDonationRecipientHold) 
                    ...
                    && block.timestamp > transferTimeFrom[from] + 5 minutes) // 5 mins donation cooldown
                    ||
                    (amount == _donationLimit2 // Can only donate 2500 tokens at a time
                    && block.timestamp > transferTimeFrom[from] + 2 minutes) // 2 mins donation cooldown
                    ||
                    (amount == _donationLimit4 // Can only donate 500 tokens at a time
                    && block.timestamp > transferTimeFrom[from] + 10 minutes) // 10 mins donation cooldown
                    ||
                    ...
                    "Donation cooldown times as follows: Donate 100K (1h), 10K (10m), 2500 (5m), 500 (2m)."
            );
            transferTimeFrom[from] = block.timestamp;
        ...
        if (_isDonationRecipient[from]) 
            ...
            require ((amount == donationTransferMax && _isEmploymentWallet[to]) // Send 250,000 tokens to Employment Wallet
                    || ((amount == donationTransferMin && to == uniswapV2Pair)
                       && (block.timestamp > transferTimeFrom[from] + 5 minutes)) // Sell 99,900 tokens every 5 minutes
        ...
        if (_isExcludedFromFee[from] || _isExcludedFromFee[to] 
            ...
            || _isTeamWallet[from] || _isTeamWallet[to] || from == Wallet_Treasury) {
            takeFee = false;
        }
