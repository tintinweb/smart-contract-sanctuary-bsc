...
contract FManCard is ERC1155, Ownable {
    ...
    struct Season {
        ...
        uint256[] cardIds;
        // mapping(uint256 => bool) mintableLevels;
        // uint256[] levels;
    }
    ...
    function updateSeasonCards(
        ...
        for (uint256 i = 0; i < _cardIds.length; i++) {
            ...
            require(cardSupply[_cardId] < fetchedCard.totalSupply, "Card supply exhausted, update card's totalSupply");
    ...
    function isMintDroppable(uint256 _cardId) public view returns (bool droppable) {
        require(cardExists(_cardId), "Card does not exist");

        return mintDroppableMap[_cardId];
    }

    function setMintDroppable(uint256 _cardId, bool droppable) public onlyOwner {
        require(cardExists(_cardId), "Card does not exist");
        ...
        returns (uint256[] memory ids)
    {
        ...
        for (uint256 i = 0; i < seasonCardIds.length; i++) {
            ...
            uint256 supply = fetchedCard.totalSupply - cardSupply[fetchedCard.id];
