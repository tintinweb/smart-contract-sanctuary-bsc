...
contract IpfsIdStorage is Ownable {
    ...
    event GodMinted(uint256 nftId, uint256 ipfsId, uint256 date);
    event ZaiMinted(string indexed zaiType, uint256 nftId, uint256 date);
    ...
    // We store ids of each state, used for marketplace and list each Zais (espacially rarest)
    mapping(uint256 => EnumerableSet.UintSet) private _bronzeIds;
    mapping(uint256 => EnumerableSet.UintSet) private _silverIds;
    mapping(uint256 => EnumerableSet.UintSet) private _goldIds;
    mapping(uint256 => EnumerableSet.UintSet) private _platinumIds;

    IAddresses public gameAddresses;

    modifier onlyAuth() {
        require(msg.sender == gameAddresses.getZaiMetaAddress(), "Not Authorized to managed IPFS storage");
        _;
    }
    ...
    function getStatesLengths(uint256 _season)
        external
        view
        returns (
            uint256 bronzeLength,
            uint256 silverLength,
            uint256 goldLength,
            uint256 platinumLength
        )
    {
        return (
            EnumerableSet.length(_bronzeIds[_season]),
            EnumerableSet.length(_silverIds[_season]),
            EnumerableSet.length(_goldIds[_season]),
            EnumerableSet.length(_platinumIds[_season])
        );
    }

    // return NFT ids Minted
    function getNftId(
        uint256 _season,
        uint256 _state,
        uint256 _index
        ...
        if (_state == 0) {
            return EnumerableSet.at(_bronzeIds[_season], _index);
        } else if (_state == 1) {
            return EnumerableSet.at(_silverIds[_season], _index);
        } else if (_state == 2) {
            return EnumerableSet.at(_goldIds[_season], _index);
        } else if (_state == 3) {
            return EnumerableSet.at(_platinumIds[_season], _index);
        } else {
            return 0;
        }
    }
    ...
    function _getGodId() internal view returns (uint256) {
        uint256 _godId;
        for (uint256 i = 7; i > 0; ) {
            if (EnumerableSet.at(_freeIds[_currentSeason][3], i) <= 7) {
                _godId = EnumerableSet.at(_freeIds[_currentSeason][3], i);
                break;
            }
            unchecked {
                --i;
    ...
    function _getIdsLength(uint256 _season, uint256 _state)
        internal
        view
        returns (uint256)
    {
    ...
    {
        if (_state == 0) {
            EnumerableSet.add(_bronzeIds[_currentSeason], _nftId);
        }
        if (_state == 1) {
            EnumerableSet.add(_silverIds[_currentSeason], _nftId);
        }
        if (_state == 2) {
            EnumerableSet.add(_goldIds[_currentSeason], _nftId);
        }
        if (_state == 3) {
            EnumerableSet.add(_platinumIds[_currentSeason], _nftId);
        }
        ...
        if (
            ...
            _nonce = IOracle(gameAddresses.getOracleAddress()).getRandom(
                keccak256(
                    abi.encodePacked(_currentSeason, block.timestamp, _nonce)
                )
            );
            if (
                block.timestamp >=
        ...
        if (_state == 3 && newIpfsId <= 7) {
            ...
            emit GodMinted(_nftId, newIpfsId, block.timestamp);
        }
        emit ZaiMinted(
    ...
    function _getRandomId(uint256 _state) internal returns (uint256) {
        _nonce = IOracle(gameAddresses.getOracleAddress()).getRandom(
            keccak256(abi.encodePacked(_currentSeason, block.timestamp, _nonce))
        );

        uint256 index = _nonce %
            (EnumerableSet.length(_freeIds[_currentSeason][_state]));
