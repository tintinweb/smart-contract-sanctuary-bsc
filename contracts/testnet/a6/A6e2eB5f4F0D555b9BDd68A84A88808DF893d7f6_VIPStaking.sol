...
contract VIPStaking is Pausable, ReentrancyGuard, AccessControlEnumerable {
    ...
    function getStakeInfo(address account) external view returns(StakeInfo memory, uint256){
        if(accountStakeInfo[account].amount > 0) {
            return (accountStakeInfo[account], minStakePriod - (block.timestamp - accountStakeInfo[account].stakeTime)) ;
        } else {
            return (accountStakeInfo[account], 0) ;
        }
    }
