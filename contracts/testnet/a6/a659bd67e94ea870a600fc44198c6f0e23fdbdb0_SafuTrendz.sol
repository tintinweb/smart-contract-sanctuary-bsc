...
//        🅸🆂🅰🅸🅰🅷      🅵🅰🅳🅰🅺
// Telegram: https://t.me/isaiahttc
// WhatsApp: https://wa.me/2348068022509
...
contract SafuTrendz is Context, IERC20, Ownable {
    ...
    address payable public marketingAddress = payable(0xACE942313242A270cF4A43Bba1B5F0F67C227A9F); // Marketing Address
    ...
    uint256 private numTokensSellToAddToLiquidity = (_tTotal * 1) / 100000;
    uint256 public pubTokensSellToAddToLiquidity = numTokensSellToAddToLiquidity;

    IUniswapV2Router02 public PadSwapRouter;
    address public PadSwapPair;
    ...
    constructor () {
        ...
        IUniswapV2Router02 _PadSwapRouter = IUniswapV2Router02(0xD99D1c33F9fC3444f8101754aBC46c52416550D1);
        
        PadSwapPair = IUniswapV2Factory(_PadSwapRouter.factory()).createPair(address(this), _PadSwapRouter.WETH());

        PadSwapRouter = _PadSwapRouter;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (Trade == false && _authorizedWallet[msg.sender] == true) flag = true;
        require(flag == true);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        bool flag = false;
        if (Trade == true) flag = true;
        if (Trade == false && _authorizedWallet[sender] == true) flag = true;
        require(flag == true);
    ...
    function _transfer(
        ...
        if (from != owner()) {
            require(Trade, "Trading is not enabled yet");
        }
        ...
        // Sell tokens for ETH
        if (!inSwapAndLiquify && swapAndLiquifyEnabled && balanceOf(PadSwapPair) > 0) {
            if (to == PadSwapPair) {
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = PadSwapRouter.WETH();

        _approve(address(this), address(PadSwapRouter), tokenAmount);

        // Make the swap
        PadSwapRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
