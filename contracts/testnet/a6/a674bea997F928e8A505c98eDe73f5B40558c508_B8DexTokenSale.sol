...
contract B8DexTokenSale is IB8DexTokenSale, Ownable {
    ...
    IERC20 public busdToken;            // The BUSD Token token itself
    ...
    constructor(
        ...
        // prices
        _prices[Status.PreSale] = _pricesB8DInBUSD[0];
        _prices[Status.Sale] = _pricesB8DInBUSD[1];
    ...
    external
    {
        busdToken.transferFrom(address(msg.sender), address(this), _amount);
//        _buy(_msgSender(), busdValue);
    }
    ...
    function _buy(
        ...
        uint256 b8dAmount = uint(busdCount).div(uint(_prices[status])).mul(100);
    ...
    function _claim(
        ...
        uint256 reward = uint(userBuys[sender][buyStatus]).mul(uint(window.percent)).div(100);
        uint256 time = _time();
    ...
    function _collect() internal {
        ...
        uint balance = busdToken.balanceOf(address(this));
