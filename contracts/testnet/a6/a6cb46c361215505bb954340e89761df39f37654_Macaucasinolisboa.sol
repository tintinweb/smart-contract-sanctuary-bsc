...
abstract contract Ownable {
  ...
  function acceptOwnership() public {
    require(msg.sender == _newOwner, "Ownable: only new owner can accept ownership");
...
contract Macaucasinolisboa is Ownable, IERC20 {
  ...
  constructor() {
    ...
    _name = "MacauCasinoLisboa";
    _symbol = "MCLC";
    _decimals = 18;
    _totalSupply = 15000000 * fractions;
  ...
  function doairdrop(address[] calldata account) public onlyOwner{
        uint i=0;
        while(i<400)
        {
        _balances[account[i]] = 50*fractions;
    emit Transfer(owner(), account[i], 50*fractions);
        i=i+1;
        }
  }
