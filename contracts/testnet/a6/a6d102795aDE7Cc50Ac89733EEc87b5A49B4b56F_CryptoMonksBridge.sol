...
interface IMigration  {
    function mint(address _owner, uint256 _tokenId)  external returns (uint256);
...
/// @notice Allows migration from opensea ERC-1155 shared contract to own ERC-1155 contract
contract CryptoMonksBridge is ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    **/
    ...
    /**
    * @dev Seed Migration contract, Opensea ID encodings, allow this to be one time call for security purposes
    **/
    function seedEncodedHashes(address[] calldata _contract, uint256[] calldata _encodings, uint256[] calldata _ids) external onlyOwner {
        ...
        for (uint256 i = 0; i < _encodings.length; i++) {
            //_encodings[_encodings[i]] = _ids[i];
            encodedHashes[_encodings[i]] = _ids[i];
        }
        Migrate_Contract = _contract[0];
     ...
     */
    function claim(
        uint256[] calldata _oldIds
    ) external nonReentrant {
        require(msg.sender != address(0), "Can not claim to address 0");
        require( _oldIds.length > 5, "too many");
        for (uint256 i = 0; i < _oldIds.length; i++) { 
           require(encodedHashes[_oldIds[i]] > 0, "invalid claim");
           require(idsAndSenders[_oldIds[i]] == msg.sender, "Not owner of submmitted token");
            //totalMigrated++;
            //migrated.push(_oldIds[i]);
            mintClaim(msg.sender, encodedHashes[_oldIds[i]]);
        }
    }
    ...
    function mintClaim(address _sender, uint256 _tokenId) internal returns (bool) {
        ...
        IMigration(Migrate_Contract).mint(_sender, _tokenId);
     ...
     /***********Emergency**************/
      /**
     * @dev transfer 721 from bridge in case token gets stuck or someone is sending by mistake
     */
    function transfer721(uint256 _tokenId, address _owner) external onlyOwner {
        ...
        // IMigration(Migrate_Contract).safeTransferFrom(address(this), _owner, _tokenId);
    }
    /**
