...
contract OGEscrow is Ownable, Pausable, ReentrancyGuard {
    ...
    event SendOGAmounts(address indexed caller,address indexed Receiver,uint256 projectID,uint256 flag,uint256 TokenAmount);
    ...
    event UpdateOGtoken(address indexed caller,address indexed newOGtoken);
    ...
    constructor(address _OGToken,address _router, address[3] memory _feeWallets, address _BUSD) {
        require(_OGToken != address(0x0) && _router != address(0x0) && _BUSD != address(0x0),"Zero address appears" );
        ...
        tierInfo[0] = TierLevel({level:1, USDAmount:0, expirySeconds: 0, feePercent: 50, projectAllocation:1 }); //50 means 5%
        tierInfo[1] = TierLevel({level:2, USDAmount:50, expirySeconds: 30 days, feePercent: 30, projectAllocation:2  }); //30 means 3%
        tierInfo[2] = TierLevel({level:3, USDAmount:250, expirySeconds: 30 days, feePercent: 10, projectAllocation:5  }); //10 means 1%
    }
    ...
    function buyTier(uint256 _tierLevel,uint256 _tokenAmount ) external whenNotPaused nonReentrant {
        ...
        if(_tokenAmount > 0){ swapBUSDToOG(_tokenAmount); }
    ...
    function setTier(uint256 _tierLevel,address _toAccount) internal {
        TierLevel storage tier = tierInfo[_tierLevel];
        require(developerInfo[_toAccount].expiryTime <= block.timestamp, "existing level not expiry");

        uint256 expiry = block.timestamp + tier.expirySeconds;

        if(tier.expirySeconds == 0 ) { expiry = type(uint).max; }
    ...
    function updateTier(uint256 _tierLevel, uint256 _USDamount, uint256 _expiryDays, uint256 _feePercentage, uint256 _monthlyProject) external onlyOwner{
        require( _tierLevel < 3,"Invalid tier level");
        TierLevel storage tier = tierInfo[_tierLevel];
        tier.USDAmount = _USDamount;
        tier.expirySeconds = _expiryDays * 86400;
    ...
    function updateOGtoken(address _OGToken) external onlyOwner {
        require(address(_OGToken) != address(0x0),"Invalid address" );
        OGToken = IERC20(_OGToken);

        emit UpdateOGtoken(_msgSender(), _OGToken);
    }
    ...
    function swapBUSDToOG(uint256 _tokenAmount) internal {
        address[] memory path = new address[](2);
        path[0] = BUSD;
        path[1] = address(OGToken);
    ...
    function createProject(address _tokenAddress,uint256 _tokenAmount, address _devAddress, uint256[3] memory _mileStonePercentage) external payable tokenApproved(_tokenAddress) whenNotPaused nonReentrant {
        ...
        if(address(OGToken) == _tokenAddress){
            require(OGToken.transferFrom(_msgSender(),address(this),_tokenAmount),"deposit failed");
            require(OGToken.transfer(feeWallets[2], amount),"fee amount failed");
        } else {
    ...
    function sendMileStoneAmounts(uint256 _projectID, uint256 _flag, bool success) external whenNotPaused nonReentrant {
        Project storage project = projects[_projectID];
        require(project.tokenAddress == address(OGToken),"Invalid Token");
        ...
        if(success){ 
            require(OGToken.transfer(project.developer, tokenAmount),"sending failed"); 
            emit SendOGAmounts(_msgSender(), project.developer, _projectID, _flag, tokenAmount);
        }else { 
            require(_msgSender() == owner(),"caller must be owner");
            require(OGToken.transfer(project.provider, tokenAmount),"sending failed"); 
            emit SendOGAmounts(_msgSender(), project.provider, _projectID, _flag, tokenAmount);    
        } 
    }

    function updatePacakeRouter(address _router) external onlyOwner {
