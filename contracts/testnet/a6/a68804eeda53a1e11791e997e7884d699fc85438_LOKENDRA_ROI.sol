...
// File contracts/utils/ownerShip.sol
...
// File contracts/LOKENDRA_ROI.sol
...
contract LOKENDRA_ROI {
    ...
    constructor(address _token, address stableCoin_) public {
        ...
        // Set the start time of the contract to the current block timestamp
...
function topUp(uint256 _amount) public{
    ...
    function payReferral(uint _level, address _user, uint _value) internal {
     ...
     function widraw() public {
       ...
       uint256 reward = totalFreeze[msg.sender] + ((totalFreeze[msg.sender] * numDays * 1 ) / 10000 );
    ...
    // Increase the price of the item by .25% for every day that has passed
       uint256 percn = (Usdtprice / 100) *  4 ;
       return  Usdtprice = percn * numDays;
