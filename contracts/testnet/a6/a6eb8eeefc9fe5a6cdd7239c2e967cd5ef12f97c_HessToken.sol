...
contract HessToken is ERC20Detailed, Ownable {
    ...
    uint256 public rewardYield = 9000000;
    ...
    uint256 public constant MAX_FEE_RATE = 15;
    ...
    address public liquidityReceiver = 0xdCc22cD54AdE7325bfc69c7b12A451C8E8B3931c;
    address public treasuryReceiver = 0xdA798858e59E96CB1ce4538a249b0fff347f07d8;
    address public riskFreeValueReceiver = 0xbd77055714cbB472710fd259e61CDf0832e5d285;
    address public rewardReceiver = 0x47b24A50B9A2C4Ef4CbD1fe047a7F396b4e27932;
    ...
    uint256 public totalBuyFee = liquidityFee.add(treasuryFee).add(buyFeeRFV).add(burnFee).add(rewardFee);
    uint256 public totalSellFee = totalBuyFee;
    ...
    constructor() ERC20Detailed("HESS DAO", "HESS", uint8(DECIMALS)) {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToRFV = contractTokenBalance.mul(buyFeeRFV.mul(2)).div(realTotalFee);
        uint256 amountToTreasury = contractTokenBalance.sub(amountToLiquify).sub(amountToRFV);
        uint256 amountToReward = contractTokenBalance.sub(amountToTreasury).sub(amountToLiquify).sub(amountToRFV);
        ...
        if(amountToReward > 0){
            _swapTokensForBNB(amountToReward, rewardReceiver);
        }

        emit SwapBack(contractTokenBalance, amountToLiquify, amountToRFV, amountToTreasury, amountToReward);
    }
    ...
    function setFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _riskFreeValueReceiver, address _rewardReceiver) external onlyOwner {
    ...
    event SwapBack(uint256 contractTokenBalance,uint256 amountToLiquify,uint256 amountToRFV,uint256 amountToTreasury,uint256 amountToReward);
