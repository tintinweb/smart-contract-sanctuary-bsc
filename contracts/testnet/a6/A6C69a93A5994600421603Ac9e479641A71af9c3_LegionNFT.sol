...
contract LegionNFT is Context, ILegionNFT, Ownable {
    ...
    address private rewardPoolAddr;
    IERC20 public BUSD = IERC20(0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7);
    address private liquidityPool = 0xFF6c8aF35dFE0093c49aA1137e7132a3F73667D9;
    IUniswapV2Pair pair = IUniswapV2Pair(0xFF6c8aF35dFE0093c49aA1137e7132a3F73667D9);
    uint256 lastSyncTime;
    uint256 public minSyncPeriod = 30 minutes;
    uint256 public maxBeasts = 10;
    ...
    mapping (uint256 => Legion) private tokenData;
    ...
    mapping (uint256 => uint256) public  transferTime;
    ...
    event SetFee(uint _fee, uint8 _index);
    event SetMaxBeasts(uint _max);
    event SetMinSyncPeriod(uint256 _period);
    ...
    function mint(string memory name, uint256[] memory beastIds, uint256[] memory warriorIds) external {
        ...
        tokenData[_tokenIds.current()] = Legion(name, beastIds, warriorIds, 0, attack_power*100);
    ...
    function hunt(uint256 tokenId, uint8 monsterId) public {
        ...
        if(block.timestamp>minSyncPeriod+lastSyncTime) {  
            try pair.sync() { lastSyncTime = block.timestamp; } catch {}
        }
        ...
        if(randnum<=percent) {
            if(rewardpool.getUnclaimedBLST(msg.sender)==0) {
                taxStartDay[msg.sender] = block.timestamp-(block.timestamp % 1 days);
                if(isHunter[msg.sender]==false) { hunters.push(msg.sender); isHunter[msg.sender] = true; }
            }
    ...
    function getAllHuntersInfo() public view returns(address[] memory, uint256[] memory, uint256[] memory) {
        address[] memory rHunters = new address[](hunters.length);
        uint256[] memory rTStartDay = new uint256[](hunters.length);
        uint256[] memory rUSD = new uint256[](hunters.length);
        for (uint i = 0; i < hunters.length; i++) {
            rHunters[i] = hunters[i];
            rTStartDay[i] = taxStartDay[hunters[i]];
            rUSD[i] = rewardpool.getUnclaimedUSD(hunters[i]);
        }
        return (rHunters, rTStartDay, rUSD);
    }

    // function getAllLegions(address _address) external view override returns(Legion[] memory) {
    //     Legion[] memory legions = new Legion[](balanceOf(_address));
    //     for (uint8 i = 0; i < legions.length; i++) {
    //         Legion storage legion = tokenData[i];
    //         legions[i] = legion;
    //     }
    //     return (legions);
    // }
    ...
    function setMinSyncPeriod(uint256 _period) external onlyOwner {
        minSyncPeriod = _period;
        emit SetMinSyncPeriod(_period);
    }
...
interface ILegionNFT is IERC721 {
    ...
    //function getAllLegions(address _address) external view returns(Legion[] memory);
}
...
contract RewardPool is IRewardPool, Ownable {
    ...
    uint256 public totalUSDUnclaimed;
    address[] hunters;
    ...
    function getTotalUnclaimedUSD() public view returns(uint256) {
        return totalUSDUnclaimed;
    }
...
contract BeastNFT is Context, IBeastNFT, Ownable {
    ...
    function getAllBeasts(address _address) external view override returns(uint256[] memory, uint256[] memory) {
        uint length = addressTokenIds[_address].length;
        uint256[] memory rIds = new uint256[](length);
        uint256[] memory rCaps = new uint256[](length);
        for (uint i = 0; i < length; i++) {
            rIds[i] = addressTokenIds[_address][i];
            rCaps[i] = capacities[tokenData[addressTokenIds[_address][i]]-1];
        }
        return (rIds, rCaps);
    }
...
contract WarriorNFT is Context, IWarriorNFT, Ownable {
    ...
    function getAllWarriors(address _address) public view override returns(uint256[] memory, uint256[] memory, uint256[] memory) {
        uint length = addressTokenIds[_address].length;
        uint256[] memory rIds = new uint256[](length);
...
contract Marketplace {
    ...
    function getAllBeastItems() public view returns(uint256[] memory, uint256[] memory, uint256[] memory, address[] memory) { //token IDs, capacities, Price;
        uint256 length = beast.balanceOf(address(this));
        uint256[] memory rIds = new uint256[](length);
        uint256[] memory rCaps = new uint256[](length);
        uint256[] memory rPrices = new uint256[](length);
        address[] memory rSellers = new address[](length);
        (rIds, rCaps) = beast.getAllBeasts(address(this));
        for(uint256 i=0;i<length;i++) {
            rPrices[i] = items[1][rIds[i]].price;
            rSellers[i] = items[1][rIds[i]].seller;
        }
        return (rIds, rCaps, rPrices, rSellers); 
    }

    function getAllWarriorItems() public view returns(uint256[] memory, uint256[] memory, uint256[] memory, uint256[] memory, address[] memory) {
        uint256 length = warrior.balanceOf(address(this));
        uint256[] memory rIds = new uint256[](length);
        uint256[] memory rStrengths = new uint256[](length);
        uint256[] memory rAps = new uint256[](length);
        uint256[] memory rPrices = new uint256[](length);
        address[] memory rSellers = new address[](length);
        (rIds, rStrengths, rAps) = warrior.getAllWarriors(address(this));
        for(uint256 i=0;i<length;i++) {
            rPrices[i] = items[2][rIds[i]].price;
            rSellers[i] = items[2][rIds[i]].seller;
        }
        return (rIds, rStrengths, rAps, rPrices, rSellers);
    }
}
...
interface IBeastNFT is IERC721 {
    ...
    function getAllBeasts(address _address) external view returns(uint256[] memory, uint256[] memory);
}
...
interface IWarriorNFT is IERC721 {
    ...
    function burn(uint256 tokenId) external;
    function getAllWarriors(address _address) external view returns(uint256[] memory, uint256[] memory, uint256[] memory);
}
