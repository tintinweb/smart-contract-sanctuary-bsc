...
contract BusdPurge {
    ...
    uint256 public EGGS_TO_HIRE_1MINERS = 1440000; // 6%
    ...
    constructor(address _dev1, address _lottery, address _charity, address _mkt) {
		require(!isContract(_dev1) && !isContract(_lottery) && !isContract(_charity) && !isContract(_mkt));
        owner = msg.sender;
        dev1 = _dev1;
        lottery = _lottery;
        charity = _charity;
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        token_BUSD.transfer(lottery, tax);
        token_BUSD.transfer(charity, tax);
    ...
    function CHANGE_lottery(address value) external {
        require(msg.sender == lottery, "Admin use only.");
        lottery = value;
    }

    function CHANGE_charity(address value) external {
        require(msg.sender == charity, "Admin use only.");
        charity = value;
    }
...
/**
BUSD Purge Miner
 Hire Farmers, Gather Purgers, Harvest Purge and Sell for BUSD. 
  6% daily daily Rate
  8% Referral Bonus, will go directly to referrer wallet.
  2% stacking compound bonus every 12 hrs, max of 6 days. 
  48 hours cut off time.
  10 BUSD minimum investment.
  10,000 BUSD max deposits per wallet.
  80% feedback for withdrawals that will be done not after 6 consecutive compounds.(12%)
