...
contract ShadowFiDonationPortal is Ownable, ReentrancyGuard {
    ...
    function donateBUSD(uint256 _amountToken) payable nonReentrant public {
        ...
        path[0] = pancakeRouter.WETH();
        path[1] = BUSD;
        uint256[] memory amountsBNB = pancakeRouter.swapExactTokensForETH(_amountToken, 0, path, receiver, block.timestamp + 120);

        uint256 bnbAmountForBuy = amountsBNB[amountsBNB.length - 1];
        uint256 bnbAmount = bnbAmountForBuy / 2;

        address[] memory path2 = new address[](2);
        path2[0] = pancakeRouter.WETH();
        path2[1] = address(shadowFiToken);
        uint256[] memory amountsBUSD = pancakeRouter.swapExactETHForTokens{value: bnbAmount}(0, path2, receiver, block.timestamp + 120);

        uint256 bnbLoadAmountBUSD = amountsBUSD[amountsBUSD.length - 1];

        uint256 sumAmount = amountsBUSD[amountsBUSD.length - 1];
        shadowFiToken.approve(address(pancakeRouter), sumAmount);
        (uint256 amountToken, uint256 amountBNB, uint256 liquidity) = pancakeRouter.addLiquidityETH{value: bnbLoadAmountBUSD}(address(shadowFiToken), sumAmount, 0, 0, address(vaultContract), block.timestamp + 120);

        if (sumAmount > amountToken) {
            shadowFiToken.transfer(msg.sender, sumAmount - amountToken);
        }

        if (bnbLoadAmountBUSD > amountBNB) {
            payable(msg.sender).transfer(bnbLoadAmountBUSD - amountBNB);
        }
