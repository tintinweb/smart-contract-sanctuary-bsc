...
abstract contract Pausable {
    ...
    event UnMintpaused(address account);
...
_Mintpaused = false;
    ...
    /**
     * @dev Initializes the contract in unMintpaused state.
     */

    /**
     * @dev Modifier to make a function callable only when the contract is not Mintpaused.
     *
     * Requirements:
     *
     * - The contract must not be Mintpaused.
     */
    ...
    /**
     * @dev Modifier to make a function callable only when the contract is Mintpaused.
     *
     * Requirements:
     *
     * - The contract must be Mintpaused.
     */
    ...
    /**
     * @dev Returns true if the contract is Mintpaused, and false otherwise.
     */
    ...
    /**
     * @dev Throws if the contract is Mintpaused.
     */
    function _requireNotMintPaused() internal view virtual {
        require(!Mintpaused(), "Pausable: Mintpaused");
    }

    /**
     * @dev Throws if the contract is not Mintpaused.
     */
    function _requireMintPaused() internal view virtual {
        require(Mintpaused(), "Pausable: not Mintpaused");
    }
     ...
     * - The contract must be Mintpaused.
     */
    function _unMintpause() internal virtual whenMintPaused {
        _Mintpaused = false;
        emit UnMintpaused(msg.sender);
    }
}
...
contract CryptoPeso is IERC20, Ownable, Pausable,ReentrancyGuard {
    ...
    */
    ...
    function _mintWithNative(address recipient, uint256 minOut) internal whenNotMintPaused nonReentrant returns (uint256) {        
    ...
    function _mintWithBacking(uint256 numBUSD, address recipient) internal whenNotMintPaused returns (uint256) {
        require(
            tokenActivated || canTransactPreLaunch[msg.sender],
            'Token Not Activated'
        );
    ...
    function _mintTo(address recipient, uint256 received, uint256 totalBacking, uint256 oldPrice) private whenNotMintPaused returns(uint256) {
    ...
    */
    function setFees(uint256 _mintFee, uint256 _transferFee, uint256 _sellFee, uint256 _stableSwapFee) external onlyOwner {
              require(_mintFee >= 97000 &&_mintFee <= 3000);       // capped at 3% fee
        require(_transferFee >= 97000 && _transferFee <= 3000);   // capped at 3% fee
        require(_sellFee >= 97000 && _sellFee <= 3000);       // capped at 3% fee
        require(_stableSwapFee >= 97000 && _stableSwapFee <= 3000); // capped at 3% fee         
        mintFee = _mintFee;
