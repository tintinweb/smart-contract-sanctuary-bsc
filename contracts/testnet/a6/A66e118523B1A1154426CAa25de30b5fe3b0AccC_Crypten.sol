...
contract Crypten is Accessable
{
    ...
    mapping(string => Ops) public TaskType;
    mapping(string => MatrixTask) public MatrixTasks;
    ...
    string calldata taskHash, Ops op, uint256[] calldata extraData) public accessable
    {
        ...
        if(op == Ops.Mul){
            ...
            for(uint i = 0;i< 2; i++){
                for(uint j = 0;j<2;j++){
                    Matrix memory test = Matrix(data[i*2 + j], extraData[j*2], extraData[j*2 + 1]);
                    task.Results[0][i].push(test);
                }
    ...
    function ExecuteTask(uint256 role, string calldata taskHash) public
    {
        ...
        if(op == Ops.Mul){
            ...
            ExecuteMatrixTask(role, taskHash);(role, taskHash);
        }
    }
    ...
    function GetSumResults(string calldata taskHash, uint256 stage,uint256 idx) accessable 
    public returns(int256 data){
        emit Read(msg.sender);
        return Tasks[taskHash].Results[stage][0][idx] + Tasks[taskHash].Results[stage][1][idx];
    }
    ...
    public returns(Matrix memory data){
        emit Read(msg.sender);
        return MatrixTasks[taskHash].Results[stage][role][idx];
    }
    function GetMatrixResults(string calldata taskHash, uint256 stage,uint256 idx) accessable
    public returns(Matrix memory data)
    {
        emit Read(msg.sender);
        return Arithmatic.MatrixAdd(MatrixTasks[taskHash].Results[stage][0][idx],MatrixTasks[taskHash].Results[stage][1][idx]);
    }
    ...
    function ExecuteMatrixTask(uint256 role, string calldata taskHash) public
    {
        require(IsAdded[taskHash], "No such task");
        MatrixTask storage task = MatrixTasks[taskHash];
        require(task.CurStage > 0, "Not initialze correctly");
