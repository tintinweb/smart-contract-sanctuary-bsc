...
//****************************************************************************//
 ...
 contract xMan is owned {
    ...
    uint public lockTime = 3600 seconds;
    ...
    {  
        ...
        for (uint i=0 ; i < 15; i++)
        {
            ...
            if(userInfos[ref].id > 1)
            {
                ...
                amount = getLess(amount, stakeInfos[ref][stakeInfos[ref].length - 1].stakedAmount);
            }
        }
    ...
    function emergencyRescueToken(address _token, uint amount) public onlyOwner returns(bool)
    {
        tokenInterface(_token).transfer(owner,amount);
        return true;
    }
