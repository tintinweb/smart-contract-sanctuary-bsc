...
contract CRICKICO {
    ...
    function purchase(address payable referrer, uint256 token_quantity) public payable beforeStarted() {
        ...
        uint256 crickCoinQuantity = USDT.mul(current_phase_price).div(1000);
        ...
        if (user.referrer != address(0)) {
            ...
            for (uint256 i = 0; i < DIRECT_REFERRAL.length; i++) {
                if (upline != address(0)) {
                       ...
                       Cricktoken.transferFrom(address(this),upline, amount);
        ...
        user.deposits.push(Deposit(crickCoinQuantity, block.timestamp,0,0,0,0,0));
             ...
             uint256 time_elapsed = block.timestamp.sub(user.deposits[i].start); // time elapsed in seconds
                ...
                if (time_elapsed > PHASE_TIME_PERIOD[0] &&  user.deposits[i].is_released_for_phase1==0) {
                    ...
                    uint256 dividends = (
                        ...
                        Cricktoken.transferFrom(address(this),msg.sender, dividends);
    ...
    function withdrawUSDT(uint256 amt) public{
        ...
        USDTtoken.transferFrom(address(this), msg.sender, USDT_to_withdraw);
    }


    function withdrawCrick(uint256 amt) public{

        require(msg.sender ==owner,"permisson denied");

        uint256 crick_to_withdraw = amt * 10 ** 18;

        Cricktoken.transferFrom(address(this), msg.sender, crick_to_withdraw);
    ...
    function investmentsInfo(address _addr) view external returns(uint256[] memory _amounts, uint256[] memory _start, uint8[] memory p1, uint8[] memory p2, uint8[] memory p3, uint8[] memory p4, uint8[] memory p5) {
        ...
        for(uint256 i = 0; i < user.deposits.length; i++) {
         ...
         _start[i] =  dep.start;
         p1[i] = dep.is_released_for_phase1;
         p2[i] = dep.is_released_for_phase2;
         p3[i] = dep.is_released_for_phase3;
         p4[i] = dep.is_released_for_phase4;
         p5[i] = dep.is_released_for_phase5;
        }
