...
contract X_X {
    ...
    uint256 public BASIC_DURATION = 30 days; //basic duration
    uint256 public BASIC_INTEREST = 200_000; //basic interest
    uint256 public ADVANCED_DURATION = 180 days; //advanced duration
    uint256 public ADVANCED_INTEREST = 250_000; //advanced interest
    uint256 public PREMIUM_DURATION = 360 days; //premium duration
    ...
    function buy_level(
        ...
        require(
            _plan <= accounts[msg.sender].current_level + 1,
            "Invalid Plan"
        );
        if (
        ...
        require(_plan >= 1 && _plan <= 12, "Invalid Plan");
    ...
    function withdraw(uint256 _plan) public {
        ...
        for (uint256 i; i < user.ROI[_plan].length; i++) {
            if (user.ROI[_plan][i].FirstWithdrawTime == user.ROI[_plan][i].matrixTime) {
                if (block.timestamp > user.ROI[_plan][i].matrixTime + 90 days) {
    ...
    {
        if (durationpassed > 0 && durationpassed <= BASIC_DURATION) {
            ...
            durationpassed > ADVANCED_DURATION &&
            durationpassed <= PREMIUM_DURATION
        ) {
