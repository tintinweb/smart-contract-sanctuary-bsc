...
contract DiplomasNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function checkEligibility() public returns (uint256) {
        uint256 total = 0;
        for (uint256 index = 0; index < _totalSupply; index++) {
            Student storage student = _students[index];
            student._eligible = _signatures.isCompletelySigned(
                student._address
            );
            total += 1;
        }
