...
event check(uint256 data,uint256 profit);
...
// event check( uint256 );
            ...
            function IsRewardClaimPending(address useradrs ) public view returns(bool){
               ...
               if(data > 0 &&  UserDetail[msg.sender].totalIncentiveEarned > 0 && UserDetail[msg.sender].amountEarnedByRef > 0){
            ...
            // function checkForReInvesting(address useraddress) public returns(bool) {

            //         uint256 data =  calculateWithdrawlAmount(msg.sender);
        
            //         uint256 profitDecider = calculateProfit(msg.sender);

            //     uint256 Profit = UserDetail[useraddress].totalIncentiveEarned + UserDetail[useraddress].amountEarnedByRef + data + UserStakingDetail[useraddress].userLastTimeAmountTotalRewardClaimed ;

            //      if( Profit >= (UserDetail[useraddress].userLastAmountInvested*300)/100  ){
