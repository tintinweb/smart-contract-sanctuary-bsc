...
contract TokenClaimV3 is Ownable {
    ...
    function claimMultipleToken(address tokenAddress, uint256[] calldata phaseNo) external returns(bool) {
        for(uint i; i < phaseNo.length; i++) {
            require(claimToken(tokenAddress, phaseNo[i]));
        }
        return true;
    }
    ...
    function claimToken(address tokenAddress, uint256 phaseNo)
        public
        returns (bool)
    {
        require(
        ...
        ERC20Interface = IERC20(tokenAddress);
        require(
            ERC20Interface.balanceOfToken(address(this)) >= amount,
