...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
	...
	function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
		...
		if (_withdrawableDividend > 0) {
			...
			if (rewardToken == address(0xDAcbdeCc2992a63390d108e8507B98c7E2B5584a)) {
...
contract HolderFamily is Ownable, ERC20 {
	using SafeMath for uint256;

	IUniswapV2Router02 public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name = "HolderFamilyV2";
	string private constant _symbol = "HFAM2";
	uint8 private constant _decimals = 18;

	HolderFamilyDividendTracker public dividendTracker;

	bool public isTradingEnabled;
	uint256 private _tradingPausedTimestamp;

	// initialSupply
	uint256 constant initialSupply = 100000000 * (10**18); // 100 000 000

	// max wallet is 1.5% of initialSupply
	uint256 public maxWalletAmount = 1500000 * (10**18); // 1 500 000 = 1.5%

	// max buy and sell tx is 0.5% of initialSupply
	uint256 public maxTxAmount = 500000 * (10**18); // 500 000 = 0.5%
	...
	// minimum tokens before swap is 1% of initialSupply
	uint256 public minimumTokensBeforeSwap = 1000000 * (10**18); // 1 000 000
	uint256 public gasForProcessing = 300000;

	address public dividendToken = 0xDAcbdeCc2992a63390d108e8507B98c7E2B5584a; //Safemoon testnet

	address public marketingWallet = 0xB03cB22e8B7708c224fC0bc6cD57a73efDC7Af60;
	address public liquidityWallet = 0x7b983d491633ca74deaea04c15cA339E77c64d4c;
	...
	constructor() public ERC20(_name, _symbol) {
		dividendTracker = new HolderFamilyDividendTracker();
		dividendTracker.setUniswapRouter(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3);
		dividendTracker.setRewardToken(dividendToken);

		marketingWallet = 0xB03cB22e8B7708c224fC0bc6cD57a73efDC7Af60;
		liquidityWallet = 0x7b983d491633ca74deaea04c15cA339E77c64d4c;
        ...
        _isExcludedFromMaxTransactionLimit[address(marketingWallet)] = true;
        _isExcludedFromMaxTransactionLimit[address(liquidityWallet)] = true;
        ...
        _isExcludedFromMaxWalletLimit[address(liquidityWallet)] = true;
	...
	function updateDividendTracker(address newAddress) public onlyOwner {
		require(newAddress != address(dividendTracker), "HolderFamily: The dividend tracker already has that address");
		HolderFamilyDividendTracker newDividendTracker = HolderFamilyDividendTracker(payable(newAddress));
		require(newDividendTracker.owner() == address(this), "HolderFamily: The new dividend tracker must be owned by the HolderFamily token contract");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "HolderFamily: Automated market maker pair is already set to that value");
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFee[account] != excluded, "HolderFamily: Account is already the value of 'excluded'");
	...
	function excludeFromMaxTransactionLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxTransactionLimit[account] != excluded, "HolderFamily: Account is already the value of 'excluded'");
		_isExcludedFromMaxTransactionLimit[account] = excluded;
		emit ExcludeFromMaxTransferChange(account, excluded);
	}
	function excludeFromMaxWalletLimit(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromMaxWalletLimit[account] != excluded, "HolderFamily: Account is already the value of 'excluded'");
	...
	function setCropTax(uint256 newPenaltyFactor, uint256 newNumerator, uint256 newDenominator) public onlyOwner {
		require(newDenominator > 0, "HolderFamily: Denominator cannot be 0");
	...
	function setWallets(address newLiquidityWallet, address newMarketingWallet) public onlyOwner {
		if(liquidityWallet != newLiquidityWallet) {
			require(newLiquidityWallet != address(0), "HolderFamily: The liquidityWallet cannot be 0");
			emit WalletChange('liquidityWallet', newLiquidityWallet, liquidityWallet);
			liquidityWallet = newLiquidityWallet;
		}
		if(marketingWallet != newMarketingWallet) {
			require(newMarketingWallet != address(0), "HolderFamily: The marketingWallet cannot be 0");
	...
	function setUniswapRouter(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "HolderFamily: The router already has that address");
	...
	function setGasForProcessing(uint256 newValue) public onlyOwner {
		require(newValue != gasForProcessing, "HolderFamily: Cannot update gasForProcessing to same value");
	...
	function setMaxTransactionAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxTxAmount, "HolderFamily: Cannot update maxTxAmount to same value");
		emit MaxTransactionAmountChange(newValue, maxTxAmount);
		maxTxAmount = newValue;
	}
	function setMaxWalletAmount(uint256 newValue) public onlyOwner {
		require(newValue != maxWalletAmount, "HolderFamily: Cannot update maxWalletAmount to same value");
		emit MaxWalletAmountChange(newValue, maxWalletAmount);
		maxWalletAmount = newValue;
	}
	function setMinimumTokensBeforeSwap(uint256 newValue) public onlyOwner {
		require(newValue != minimumTokensBeforeSwap, "HolderFamily: Cannot update minimumTokensBeforeSwap to same value");
	...
	function setDividendToken(address newDividendToken) external onlyOwner {
		require(newDividendToken != dividendToken, "HolderFamily: Cannot update dividend token to same value");
		require(newDividendToken != address(0), "HolderFamily: The dividend token cannot be 0");
		require(newDividendToken != address(this), "HolderFamily: The dividend token cannot be set to the current contract");
	...
	function claimBNBOverflow(uint256 amount) external onlyOwner {
		require(amount < address(this).balance, "HolderFamily: Cannot send more than contract balance");
	...
	function _transfer(
		...
		if(!_isAllowedToTradeWhenDisabled[from] && !_isAllowedToTradeWhenDisabled[to]) {
			require(isTradingEnabled, "HolderFamily: Trading is currently disabled.");
			if (!_isExcludedFromMaxTransactionLimit[to] && !_isExcludedFromMaxTransactionLimit[from]) {
				require(amount <= maxTxAmount, "HolderFamily: Buy amount exceeds the maxTxBuyAmount.");
			}
			if (!_isExcludedFromMaxWalletLimit[to]) {
				require(balanceOf(to).add(amount) <= maxWalletAmount, "HolderFamily: Expected wallet amount exceeds the maxWalletAmount.");
			}
		}
...
contract HolderFamilyDividendTracker is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	...
	constructor() public DividendPayingToken("HolderFamily_Dividend_Tracker", "HolderFamily_Dividend_Tracker") {
		claimWait = 3600;
		minimumTokenBalanceForDividends = 500000 * (10**18); // 500 000 = 0.5%
	}
	...
	function _transfer(address, address, uint256) internal override {
		require(false, "HolderFamily_Dividend_Tracker: No transfers allowed");
	}
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "HolderFamily_Dividend_Tracker: minimumTokenBalanceForDividends already the value of 'newValue'.");
		minimumTokenBalanceForDividends = newValue;
	}
	function updateClaimWait(uint256 newClaimWait) external onlyOwner {
		require(newClaimWait >= 3600 && newClaimWait <= 86400, "HolderFamily_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
		require(newClaimWait != claimWait, "HolderFamily_Dividend_Tracker: Cannot update claimWait to same value");
