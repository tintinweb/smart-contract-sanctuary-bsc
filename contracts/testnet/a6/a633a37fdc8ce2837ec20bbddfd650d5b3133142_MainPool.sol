...
contract MainPool is Ownable, ERC1155Holder {
    ...
    function allocationKey(address _pair) public {
        require(keyToken != address(0), "M2003: keyToken is invali");
        IKeyToken(keyToken).mintForEvent(getKeyPool[mainToken][_pair]);
    }

    function swapBox(address _pair, uint256 _num) public lock {
    ...
    function allocKeyAirDrop(address _pair, uint256 _stake) external view returns (uint256 _need) {
        address _KeyPool = getKeyPool[mainToken][_pair];
