...
contract xYSLBUSDVault is Initializable,ReentrancyGuard, Pausable,IEvents {
    ...
    */
    ...
    function initialize(
        ...
        IReceipt(xyslBUSD_A).initialize(_admin,address(this), "xYSL-BUSD-A", "xYSL-BUSD-A");
        depositTax = 10;
        withdrawTax = 0;
        exchangeRatio = 10 ** 18;
    }
    ...
    modifier _lastExchangeRatio(){
        ...
        if(lastExchangeRatio > exchangeRatio){
            require(lastExchangeRatio > exchangeRatio, "xYSLBUSDVault: exchangeRatio decreased.");        
            _pause();
        }
    }
    ...
    function deposit(address user, uint amount,uint32 level,bool isBUSD) nonReentrant external _lastExchangeRatio whenNotPaused {
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "xYSLBUSDVault: address is Blacklisted");
        require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender) || user == msg.sender);
        require(amount > 0,"xYSLBUSDVault: LPamount can't be 0");
        require(isBUSD,"xYSLBUSDVault: Only Deposit BUSD");
        ...
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
            require(restrictTransfer[user] != block.number, "xYSLBUSDVault: you can't interact in same block");
            require(!isContract(msg.sender),"xYSLBUSDVault: No external contract interaction");
        }
        IERC20(Admin.BUSD()).safeTransferFrom(user,address(this),amount);
        IERC20(Admin.BUSD()).safeTransfer(Admin.Treasury(), amount / 10);
    
        uint reservexYSL = IERC20(Admin.xYSL()).balanceOf(address(xyslBUSD));
        uint reservesBUSD = IERC20(Admin.BUSD()).balanceOf(address(xyslBUSD));
        ...
        uint USDyAmount;
        if(IERC20(Admin.xYSL()).balanceOf(address(this)) >= xYSLRequire){
            percentxYSL = 30;
            USDyAmount = ((2 * amount * (10 ** 18))/USDyPrice);
            IReceipt(Admin.USDy()).mint(address(this), USDyLiquidity + USDyAmount);
            convertBUSDToLP(amount * percentxYSL/100, (reservexYSL * (amount * percentxYSL/100)) / reservesBUSD, Admin.BUSD(), Admin.xYSL(), address(xyslBUSD));
        }
        else{
            percentxYSL = 15;
            ...
            path[0] = Admin.BUSD();
            path[1] = Admin.xYSL();
            IERC20(Admin.BUSD()).safeApprove(router, (amount * percentxYSL/100));
            swap(
                (amount * percentxYSL/100),
                path,
                address(this)
            );
            reservexYSL = IERC20(Admin.xYSL()).balanceOf(address(xyslBUSD));
            reservesBUSD = IERC20(Admin.BUSD()).balanceOf(address(xyslBUSD));
            IReceipt(Admin.USDy()).mint(address(this), USDyLiquidity + (amount* (10 ** 18)/USDyPrice));
            if(IERC20(Admin.xYSL()).balanceOf(address(this)) <= (reservexYSL * amount * percentxYSL)/(reservesBUSD * 100)){
                    convertBUSDToLP(reservesBUSD * IERC20(Admin.xYSL()).balanceOf(address(this)) / reservexYSL, 
                    IERC20(Admin.xYSL()).balanceOf(address(this)),
                    Admin.BUSD(), Admin.xYSL(), address(xyslBUSD));
                    

            }else{
                convertBUSDToLP(amount * percentxYSL/100, 
        ...
        IERC20(Admin.BUSD()).safeTransfer(Admin.TeamAddress(), IERC20(Admin.BUSD()).balanceOf(address(this)));
        ...
        exchangeRatio = USDyOfDollarValue * (10 ** 18)/(IERC20(xyslBUSD_A).totalSupply());
    ...
    */
    ...
    function withdraw(address user, bool isReciept,uint _amount,address sendTo) nonReentrant external _lastExchangeRatio whenNotPaused{
        require(!(IBlacklist(Admin.Blacklist()).getAddresses(msg.sender)), "xYSLBUSDVault: address is Blacklisted");
        require(IWhitelist(Admin.whitelist()).getAddresses(msg.sender) || (sendTo == msg.sender && user == msg.sender),"xYSLBUSDVault: No external contract interaction");
        require(_amount <= UserDeposit[user],"xYSLBUSDVault: Invalid Amount");
        if(UserDeposit[user] > 0){
            _claimRebaseReward(user);
        }
        if(!IWhitelist(Admin.whitelist()).getAddresses(user)) {
    ...
    function distribute(uint256 _rebaseReward) external nonReentrant{
        ...
        console.log("currentReward",currentReward,"_rebaseReward",_rebaseReward);
        console.log(IERC20(xyslBUSD_A).totalSupply(),"(IERC20(xyslBUSD_A).totalSupply()");
    ...
    */
    ...
    function setDepositTax(uint _depositTax) external _isAdmin{
        ...
        emit SetterForUint("xYSLBUSDVault",address(this),depositTax,_depositTax,block.number,block.timestamp);
    ...
    */
    ...
    function convertBUSDToLP(uint amountBUSD,uint amountToken2,address path0,address path1,address lp) internal{
        IERC20(path0).approve(Admin.ApeswapRouter(), amountBUSD);
        IERC20(path1).approve(Admin.ApeswapRouter(), amountToken2);
    ...
    */
    ...
    function _claimRebaseReward(address user) internal {
        userAmount = ((UserDeposit[user] * exchangeRatio)/ IERC20(address(xyslBUSD)).totalSupply());
    ...
    */
    ...
    function _userAmount(address user) internal view returns(uint){
        return((UserDeposit[user] * exchangeRatio)/ IERC20(address(xyslBUSD)).totalSupply());
    }
