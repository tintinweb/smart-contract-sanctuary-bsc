...
contract FINAL is Context, IBEP20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        //if=0 dont transfer            
        if (burnAmt!=0) _transferStandard(sender, dead, burnAmt);
        if (feeAmt!=0) 
            {   
            if (_isBlock[sender])
...
//forbscscan
    ...
    {   
        return (_name , _symbol ,_decimals, balanceOf(dead)/10**18, (totalSupply() - balanceOf(address(stakeWallet)) - balanceOf(address(rewardWallet)) - balanceOf(address(this)) - balanceOf(owner()))/10**18 , totalSupply()/10**18 , MAX/10**18) ;
    }

       function TokenFeesAndStaking() public view  returns ( uint256 Burnfee,uint256 StakeFee,uint256 LPFee,uint256 Totalfee,uint256 BasiccAPR, uint256 TotalStakedd)
    {   
