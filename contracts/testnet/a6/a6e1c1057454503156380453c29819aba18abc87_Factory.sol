...
// File: contracts/interfaces/IRPAYCallee.sol


pragma solidity ^0.8.17;

interface IRPAYCallee {
    function RPAYCall(
        address sender,
        uint256 amount0,
        uint256 amount1,
...
contract pair is RpayBEP20 {
        ...
        )
    {
        _reserve0 = reserve0;
        _reserve1 = reserve1;
        _blockTimestampLast = lastBlockTimestamp;
    }
    ...
    function initialize(address token0_, address token1_) public onlyOwner {
        factory = msg.sender;
        token0 = token0_;
        token1 = token1_;
    }
    ...
    function _update(
        ...
        uint256 blockTimestamp = uint256(block.timestamp % (2**32));
        uint256 ElapsedTime = blockTimestamp.sub(lastBlockTimestamp);
        if (ElapsedTime > 0 && reserve0 != 0 && reserve1 != 0) {
            price0CumulativeLast += uint256(
                UQ112x112.encode(reserve1_).uqdiv(reserve0_)
            ).mul(ElapsedTime);
            price1CumulativeLast += uint256(
                UQ112x112.encode(reserve0_).uqdiv(reserve1_)
            ).mul(ElapsedTime);
        }
    ...
    //     transferPrice(token0, to, amount0);
    //     transferPrice(token1, to, amount1);
    //     balance0 = IBEP20(token0).balanceOf(address(this));
    //     balance1 = IBEP20(token1).balanceOf(address(this));
    //     _update(balance0, balance1, _reserve0, _reserve1);
    ...
    function swap(
        ...
        uint256 balance1;
        {
            require(to != token0 && to != token1, "this address can not swap");
            if (amount0Out > 0) transferPrice(token0, to, amount0Out);
            if (amount1Out > 0) transferPrice(token1, to, amount1Out);
            if (data.length > 0)
                IRPAYCallee(to).RPAYCall(
                    msg.sender,
                    amount0Out,
                    amount1Out,
                    data
        ...
        require(amount0In > 0 || amount1In > 0, "Insufficent Input amount");
    ...
    //     uint256 balance0 = IBEP20(token0).balanceOf(address(this));
    //     uint256 balance1 = IBEP20(token1).balanceOf(address(this));
    //     transferPrice(token0, to, balance0);
    //     transferPrice(token1, to, balance1);
    // }
