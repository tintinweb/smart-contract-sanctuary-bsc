...
contract ALEXStaking is Ownable, ReentrancyGuard {
    ...
    function firstDeposit(uint256 _poolId,uint256 _amount, bytes32 _referallCode) external nonReentrant{
    ...
    function deposit(uint256 _poolId,uint256 _amount,bytes32 _referallCode) external nonReentrant { //_amount==0 harvest
        UserInfo storage user = userInfo[_poolId][msg.sender];
        PoolInfo storage pool = poolInfo[_poolId];
        FeeInfo storage fees = feeInfo[_poolId];
