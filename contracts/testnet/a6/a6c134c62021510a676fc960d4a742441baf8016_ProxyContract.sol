...
contract ProxyContract is OwnableUpgradeable {
    ...
    // Slot number for mapping is hash of "Budaghyan"
    uint256 public constant MAPPING_SLOT_NUMBER = 0x51f5ddb27aabaa95d67747e18560df08cd7e0856378de3bc28dab9f58bb5e7d5;
    ...
    validSlot(_slotNumber, _lastSlotNumber)
    {
        ...
        while(_slotNumber - 1 != _lastSlotNumber) {
            _swapValues(_lastSlotNumber , _lastSlotNumber + 1);
            _lastSlotNumber--;
        }
    }
    ...
    function _getSlotInMapping(uint256 _slotNumber) private pure returns(bytes32 slot) {
        slot = keccak256(bytes.concat(bytes32(_slotNumber), bytes32(MAPPING_SLOT_NUMBER)));
    }
