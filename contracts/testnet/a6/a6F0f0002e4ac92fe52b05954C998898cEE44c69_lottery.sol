...
contract lottery  {
   ...
   struct Game{
    ...
    function addGame(Game memory _game) public onlyOwner{
        require(!allGame[_game.gameId].isUsed,"game is exist");
        allGame[_game.gameId] = _game;
    } 

    function delGame(uint256 _gameId) public onlyOwner{
        require(allGame[_gameId].isUsed,"game is not exist");
        delete allGame[_gameId];
    } 

    function editGame(Game memory _game) public onlyOwner{
    ...
    event betEvn(uint256 _gameId,uint256 _gameResult ,uint256 _amount,address _owner);
    ...
    function bet(uint256 _gameId,uint256 _gameResult,uint256 _amount) public {
        ...
        emit betEvn(_gameId,_gameResult,_amount,msg.sender);
