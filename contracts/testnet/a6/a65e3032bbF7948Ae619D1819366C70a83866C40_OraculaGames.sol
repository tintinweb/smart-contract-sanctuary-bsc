...
contract OraculaGames is Context, Ownable {
    ...
    struct EventInfo {
        ...
        uint256[] betsInOutcome;
        uint256[] playersInOutcome;
    }
    ...
    mapping(address => AllPlayer) public allPlayer;
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public reverseOutcomes;
    ...
    modifier onlyBetMarket() {
        require(_msgSender() == owner() || _msgSender() == betMarket, "Contract: Authorisation failed");
        _;
    }
    ...
    function getEventOutcomeIds(uint256 _eventId) external view returns (uint256[] memory) {
    ...
    function getEventTotals(uint256 _eventId) public view returns (uint256 players, uint256 bets) {
        return (eventInfo[_eventId].totalNumOfPlayers, eventInfo[_eventId].totalNumOfBets);
    }

    function getOutcomeTotals(uint256 _eventId, uint256 _outcomeId) public view returns (uint256 numOfBets, uint256 numOfPlayers) {
        uint256 realOutcome = eventOutcomeIds[_eventId][_outcomeId];
        return (eventInfo[_eventId].betsInOutcome[realOutcome], eventInfo[_eventId].playersInOutcome[realOutcome]);
    }

    function getCoefficients(uint256 _eventId) public view returns (uint256[] memory) {
        if (eventInfo[_eventId].outcomeSet) {
            return eventInfo[_eventId].legacyCoefficients;
        } else {
    ...
    function placeBet(uint256 _eventId, uint256 _betAmount, uint256 _outcomeID) external {
        ...
        if (!_player.inGame) {
            ...
            _event.playersInOutcome[contractId] += 1;
        }
        _event.totalNumOfBets ++;
        _event.betsInOutcome[contractId] += 1;
        ...
        allPlayer[_msgSender()].eventIds.push(_eventId);
        allPlayer[_msgSender()].outcomeIds.push(_outcomeID);
        allPlayer[_msgSender()].timestamp.push(block.timestamp);
        allPlayer[_msgSender()].value.push(_betAmount);
    ...
    function claimReward(uint256 _eventId) external {
        ...
        uint256 claimAmount;
        uint256 tempoDenom = _event.bets[_event.realOutcome] - _player.betAmount;
        if (tempoDenom > 0) {
            claimAmount = (_event.totalBets - _event.bets[_event.realOutcome]) * 10**18 / (_event.bets[_event.realOutcome] - _player.betAmount) + _player.betAmount;
        } else {
            claimAmount = _event.totalBets - _event.bets[_event.realOutcome] + _player.betAmount;
        }
    ...
    function claimRefund(uint256 _eventId) external {
        ...
        require(_player.betAmount > 0);

        uint256 refundAmount = _player.betAmount;
    ...
    function initializeGame(uint256[] memory _outcomeIds, uint256 _startTime, uint256 _endTime, address _creator, uint256 _oraculaFee, uint256 _creatorFee) external onlyOracula returns (uint256) {
        ...
        eventInfo.push(EventInfo({
            ...
            totalNumOfBets: 0,
            totalNumOfPlayers: 0,
            betsInOutcome: new uint256[](_numberOfOutcomes),
            playersInOutcome: new uint256[](_numberOfOutcomes)
        }));
    ...
    function transferBet(uint256 _eventId, address from, address to) external onlyBetMarket {
        EventInfo storage _event = eventInfo[_eventId];
        require(block.timestamp >= _event.startTime, "Event has not started");
        require(block.timestamp <= _event.endTime, "Event has ended");
        require(!_event.refundAllowed, "Refunds issued for this event");
        require(playerInfo[_eventId][from].inGame, "Player has not participated");
        require(playerInfo[_eventId][from].betAmount > 0);

        PlayerInfo storage _from = playerInfo[_eventId][from];

        uint256 betAmount = _from.betAmount;
        uint256 outcome = _from.outcome;

        _from.betAmount = 0;
        _from.inGame = false;
        _from.outcome = 0;

        PlayerInfo storage _to = playerInfo[_eventId][to];

        _to.inGame = true;
        _to.betAmount = betAmount;
        _to.outcome = outcome;

        emit BetTransferres(from, to);
    }
    ...
    function emergencyStop() external onlyOwner {
        oraculaToken.safeTransfer(owner(), oraculaToken.balanceOf(address(this)));
