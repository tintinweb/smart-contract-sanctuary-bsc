...
contract RewardManager is RewardAuth {
  ...
  event Claimed(address indexed _user, uint _fotaAmount, uint _usdAmount, uint _fotaPrice);
  event ReferralShare(address indexed _inviterOrTreasury, uint _fotaAmount, uint _usdAmount, uint _fotaPrice);
...
//    secondInADay = 86400; // 24 * 60 * 60
  ...
  function claim() public {
    ...
    emit ReferralShare(inviterOrTreasury, _convertUsdToFota(distributedToReferralOrTreasury), distributedToReferralOrTreasury, fotaPricer.fotaPrice());
  }
...
//    require(rewards[_user][dayToClaim].length > 0, "RewardManager: you have no reward to claim today");
    if (rewards[_user][dayToClaim].length <= 0) {
      return 0;
    }
...
interface IGameNFT is IERC721Upgradeable {
  ...
  function LAUNCH_SUPPLY() external view returns (uint);
  function LAUNCH_MAX_SUPPLY() external view returns (uint);
  function mintHeroLaunchPad(address _owner, uint16 _classId, uint _price, uint _quantity) external;
  function mintItemLaunchPad(address _owner, uint8 _gene, uint16 _classId, uint _price, uint _quantity) external;
}
