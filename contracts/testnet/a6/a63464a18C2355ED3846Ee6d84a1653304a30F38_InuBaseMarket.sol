...
contract InuBaseMarket is Initializable, AccessControlUpgradeable {
    ...
    IERC20 public inubToken;
    IInubNFT public inubNFT;

    bytes32 public constant MARKET_ADMIN = keccak256("MARKET_ADMIN");

    bool public paused;
    mapping(uint256 => uint256) internal timeAction;
    mapping(uint256 => uint256) internal updateBlocks;
    ...
    function initialize(address _router) public initializer {
        ...
        router = RouterInterface(_router);

        inubToken = IERC20(router.inubToken());
        inubNFT = IInubNFT(router.inubNFT());
    }
    ...
    modifier preventUpdateInBlock(uint256 _tokenId) {
        require(updateBlocks[_tokenId] != block.number, "access denied: block");
        updateBlocks[_tokenId] = block.number;
        _;
    }
    ...
    modifier delayAction(uint256 _tokenId) {
        require(
            timeAction[_tokenId].add(5 minutes) < block.timestamp,
            "Cancel Order And Replace After 5 minutes Listing"
        );
        timeAction[_tokenId] = block.timestamp;
        _;
    }

    function pause(bool _paused) public restricted {
        paused = _paused;
    }
    ...
    function placeOrder(uint256 _tokenId, uint256 _price)
        public
        notPaused
        delayAction(_tokenId)
        preventUpdateInBlock(_tokenId)
    {
        require(
            inubNFT.ownerOf(_tokenId) == _msgSender(),
            "Only owner can sell"
        );
        require(_price > 0, "Cannot sell without fee");
        inubNFT.transferFrom(_msgSender(), address(this), _tokenId);
        ...
        preventUpdateInBlock(_tokenId)
    {
        ...
        inubNFT.transferFrom(address(this), _msgSender(), _tokenId);
    ...
    function fillOrder(uint256 _tokenId) public preventUpdateInBlock(_tokenId) {
        require(tokenSales.contains(_tokenId), "not sale");
        ItemSale storage itemSale = markets[_tokenId];
        uint256 feeMarket = itemSale.price.mul(router.feeMarketRate()).div(
            router.divPercent()
        );

        inubToken.transferFrom(_msgSender(), router.feeAddress(), feeMarket);
        inubToken.transferFrom(
            _msgSender(),
            itemSale.owner,
...
interface RouterInterface {
    ...
    function inubToken() external view returns (address);

    function inubNFT() external view returns (address);
}
