...
contract XoomMeters {
    ...
    uint public maxFeatureToCheckRequirements = 100;
    uint public maxFunctionToCheckRequirements = 100;
    ...
    function setMaxRequirementVerification(uint maxFeatures, uint maxFunctions) external onlyOwner noReentrancyForOwner validWallet
    {
        maxFeatureToCheckRequirements = maxFeatures;
        maxFunctionToCheckRequirements = maxFunctions;
    }
    ...
    function validateRequirement(uint feature, uint functionId, address player) external view returns (uint)
    {
        ...
        for(uint featureOfMeters = 0; featureOfMeters <= maxFeatureToCheckRequirements; featureOfMeters++)
        {
            for(uint functionOfMeters = 0; functionOfMeters <= maxFunctionToCheckRequirements; functionOfMeters++)
            {
