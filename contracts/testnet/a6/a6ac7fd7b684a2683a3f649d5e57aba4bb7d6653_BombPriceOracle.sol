...
// File contracts/BombPriceOracle.sol

pragma solidity =0.5.16;

contract BombPriceOracle is IBombPriceOracle {
    ...
    function toUint224(uint256 input) internal pure returns (uint224) {
        require(input <= uint224(-1), "BombPriceOracle: UINT224_OVERFLOW");
    ...
    function initialize(address uniswapV2Pair) external {
        ...
        require(
            ...
            "BombPriceOracle: ALREADY_INITIALIZED"
        );
        ...
        returns (uint224 price, uint32 T)
    {
        ...
        require(pair.initialized, "BombPriceOracle: NOT_INITIALIZED");
        Pair storage pairStorage = getPair[uniswapV2Pair];
        ...
        require(T >= MIN_T, "BombPriceOracle: NOT_READY"); //reverts only if the pair has just been initialized
