...
contract PurchaseNft is Ownable {
    ...
    enum Quality {Normal, Good, Rare, Epic, Legendary}
    ...
    uint256 public _purchasedBoxs;

    mapping(address => Group) public _whitelists;

    mapping(address => uint256) public _purchasedCounts;

    mapping(Quality => SaleSetting) public _salesSettings;

    mapping(Group => WhitelistSetting) public _whitelistSettings;
    ...
    constructor (NFT721 _nft,address beneficiary, SaleSetting memory _settingNormal, SaleSetting memory _settingGood,
    SaleSetting memory _settingRare, SaleSetting memory _settingEpic, SaleSetting memory _settingLegendary
    ) {
        nft = _nft;
        _startTime = 1637676000;
        _endTime= 1675545768;
        _beneficiary = beneficiary;
        _isRunning = true;
        _salesSettings[Quality.Normal] = _settingNormal;
        _salesSettings[Quality.Good] = _settingGood;
        _salesSettings[Quality.Rare] = _settingRare;
        _salesSettings[Quality.Epic] = _settingEpic;
        _salesSettings[Quality.Legendary] = _settingLegendary;
    }
    

    function purchase(Quality _quality)  payable public whenRunning {        
        uint price = _salesSettings[_quality].price;
        Group _group = _whitelists[msg.sender];
        uint tokenId = _salesSettings[_quality].totalSold + _salesSettings[_quality].startTokenId;
        bool isDiscount = false;
        require(_salesSettings[_quality].totalBox > _salesSettings[_quality].totalSold, 'Quantity is 0');
        if ((_group == Group.A || _group == Group.B) && _whitelistSettings[_group].endTime >= block.timestamp && _whitelistSettings[_group].startTime < block.timestamp && _purchasedCounts[msg.sender] == 0 ) {
            isDiscount = _whitelistSettings[_group].discounted;
            price = (isDiscount) ? _salesSettings[_quality].discountPrice : _salesSettings[_quality].price; 
        } else {
            require(_startTime < block.timestamp, 'Can not purchase this time');
        }
        if (price > 0) {
            transfer(_beneficiary, price);
        }        
        nft.purchaseMint(tokenId, fees, tokenId.toString(), msg.sender);
        emit Buy(msg.sender, tokenId, price, isDiscount, block.timestamp);
        _salesSettings[_quality].totalSold++;
        _purchasedCounts[msg.sender]++;
    }

    function transfer( address to, uint price) internal { 
        require(msg.value >= price, "Price False");        
        address payable toPayable = payable(to);
        toPayable.transfer(price);      
    }

    function setWhiteListUser(address[] memory accounts, Group group) public onlyOwner {
        require(accounts.length > 0, "Invalid input");
        for(uint256 index = 0; index < accounts.length; index++){
            _whitelists[accounts[index]] = group;
        }
    }

    function setWhiteListSetting(Group group, WhitelistSetting memory setting ) public onlyOwner {
        _whitelistSettings[group] = setting;
    }

    function setSaleSetting(Quality quality, SaleSetting  memory setting) public onlyOwner {
        _salesSettings[quality] = setting;
    }

    function setPurchaseTime(uint256 time, uint256 endTime) public onlyOwner {
        _startTime = time;
        _endTime = endTime;
    }
    ...
    function purchaseInfo() public view returns (
        uint256 startTime, uint256 endTime, WhitelistSetting memory whitelistA, WhitelistSetting memory whiteListB, 
        SaleSetting memory normal, SaleSetting memory good, SaleSetting memory rare , SaleSetting memory epic, SaleSetting memory legendary
    ) {
        startTime = _startTime;
        endTime = _endTime;
        whitelistA = _whitelistSettings[Group.A];
        whiteListB = _whitelistSettings[Group.B];
        normal = _salesSettings[Quality.Normal];
        good = _salesSettings[Quality.Good];
        rare =  _salesSettings[Quality.Rare];
        epic = _salesSettings[Quality.Epic];
        legendary = _salesSettings[Quality.Legendary];
    }
...
contract NFT721 is Ownable, SignerRole, IERC721, ERC721Base {
    ...
    function purchaseMint(uint256 tokenId, Fee[] memory _fees, string memory tokenURI, address _buyAddress ) public onlySigner {
         _mint(_buyAddress, tokenId, _fees);
        _setTokenURI(tokenId, tokenURI);
    }
