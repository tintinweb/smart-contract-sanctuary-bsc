...
//    uint256 public priceTicket = 0.3*10e18;
    uint256 public panBurnPercent = 5000;
    uint256 public priceMegaTicket = 0.3 * 1e18;
    ...
    mapping(uint256 => mapping (PRIZE_TYPE => mapping(uint256 => mapping(address => uint)))) public nClaimed;

    // round => type => number => quantity
    mapping(uint => mapping(PRIZE_TYPE => mapping (uint => uint))) public nTickets; // tickets mapping with round

    // round => type => prize
    mapping(uint => mapping(PRIZE_TYPE => PrizeInfo)) public roundPrize;
...
//    uint256 public minorSampleSpace = 1e4;
    ...
    uint256 public discountPercentage = 1e2;
...
//    uint256 public currentMinorNumber2;
    ...
    /*----------------------------CONSTRUCTOR----------------------------*/
    constructor (address _USD, address _PSR, address _PAN, address _randomNumberGenerator, address _multiOracle) {
        USD = _USD;
        PSR = _PSR;
        PAN = _PAN;
    ...
    function _updateRound(uint256 _id) internal {
        ...
        if (_roundInfo.status == 0) {
            ...
            (uint _usd, uint _psr, uint _pan) = _updatePrize(_id, _roundInfo, _expire);
            pendingUSD += _usd;
            pendingPAN += _pan;
            PSRForCurrentPot -= _psr;
            (uint256[] memory _megas, uint256[] memory _minors, uint256[] memory _minis) = getWinningNumbers(_id);
            emit RoundCompleted(_id, _expire, _megas, _minors, _minis);
        }
    }
    ...
    function _updateLuckyNumber(uint256 _id, RoundInfo storage _roundInfo) internal  {
        ...
        for(uint i = 1; i < maxTypes; i++) {
            {
                uint _winners = typePrize[PRIZE_TYPE(i)].winners;
                _calculateNumbers(_megaNumber, _minorNumber1, _minorNumber2, PRIZE_TYPE(i), _winners, _roundInfo.numbers[PRIZE_TYPE(i)]);
            }
        }
        seeds.push(_megaNumber);
        seeds.push(_minorNumber1);
        seeds.push(_minorNumber2);
    }

    function _updatePrize(uint256 _id, RoundInfo storage _roundInfo, uint _expire) internal returns(uint, uint, uint) {
        uint _totalUSD = 0;
        uint _totalPSR = 0;
        uint _totalPAN = 0;
        for(uint i = 1; i < maxTypes; i++) {
            {
                (uint256 _usd, uint256 _psr, uint256 _pan) = _calcPrize(_id, _roundInfo.numbers[PRIZE_TYPE(i)].values(), _expire, PRIZE_TYPE(i));
                _totalUSD += _usd;
                _totalPSR += _psr;
                _totalPAN += _pan;
            }
    ...
    //Cheat for QC
    function _calculateNumbers(uint _number1, uint _number2, uint _number3, PRIZE_TYPE _type, uint _quantity, EnumerableSet.UintSet storage _set) internal{
        uint _seed = ( block.number + uint(_type) + _quantity ) % 256;
        uint _rand = uint256(keccak256(abi.encodePacked(_number1 >> _seed, _number2, _number3 >> (256 - _seed ), _type, _quantity)));

        uint _value = (12345+uint(_type)) % typePrize[_type].digit;
        _set.add(_value);
        uint256 digit = typePrize[_type].digit;
        for(uint i = 1; i < _quantity; i++) {
            _value = uint256(keccak256(abi.encodePacked(_rand, i))) % digit;
...
//            uint _value = uint256(keccak256(abi.encodePacked(_rand, i))) % digit;
//            while(!_set.add(_value)){
//                _value += 1;
//            }
//        }
//    }
    ...
    function _calcPrize(uint256 _roundId, uint256[] memory _numbers, uint256 _expire, PRIZE_TYPE _type) internal returns(uint256, uint256, uint256) {
        ...
        uint256 USDForWinner = USDForCurrentPot * typePrize[_type].percentage / ONE_HUNDRED_PERCENT;
        uint256 PSRForWinner = PSRForCurrentPot * typePrize[_type].percentage / ONE_HUNDRED_PERCENT;
        uint256 PANForWinner = PANForCurrentPot * typePrize[_type].percentage / ONE_HUNDRED_PERCENT;

        _prize.totalWinner = _countWinners(_roundId, _numbers, _type);
        if (_prize.totalWinner > 0) {
            _prize.USD = USDForWinner;
            _prize.PSR = PSRForWinner;
            _prize.PAN = PANForWinner;
            if(_type == PRIZE_TYPE.MEGA) {
                lastDistribute = _expire - prizeExpireTime;
            }
        }
        roundPrize[_roundId][_type] = _prize;

        emit PriceForWinner(_roundId, _type, USDForWinner, PSRForWinner, PANForWinner);
    ...
    function _liquidate(PRIZE_TYPE _type, uint256 _roundId, address _owner) internal {
        ...
        if (_type != PRIZE_TYPE.LEADERBOARD) {
            PrizeInfo storage _prize = roundPrize[_roundId][_type];
    ...
    function _getTickets(uint _number) internal returns(uint, uint) {
        uint _minorTicket = _number % typePrize[PRIZE_TYPE.MINOR].digit;
        uint _miniTicket = _number % typePrize[PRIZE_TYPE.MINI].digit;
        return (_minorTicket, _miniTicket);
    }
    ...
    function _randomTicket(address _receiver, uint256 _rand, uint256 _quantity) internal {
        ...
        uint256 _sampleSpace = typePrize[PRIZE_TYPE.MEGA].digit; // mega digit
        for (uint256 i = 0; i < _quantity; i++) {
            _ticketNumber = _generateTicket(_rand, _sampleSpace, _nSeeds);
            (uint _minor, uint _mini) = _getTickets(_ticketNumber);

            tickets[_currentRoundId][PRIZE_TYPE.MEGA][_ticketNumber][_receiver]++;
            tickets[_currentRoundId][PRIZE_TYPE.MINOR][_minor][_receiver]++;
            tickets[_currentRoundId][PRIZE_TYPE.MINI][_mini][_receiver]++;

            nTickets[_currentRoundId][PRIZE_TYPE.MEGA][_ticketNumber]++;
            nTickets[_currentRoundId][PRIZE_TYPE.MINOR][_minor]++;
            nTickets[_currentRoundId][PRIZE_TYPE.MINI][_mini]++;

            _tickets[i] = _ticketNumber;
            _rand = uint256(keccak256(abi.encodePacked(_salt, _rand, i)));
        }
        emit NewTicket(currentRoundId, _receiver, _tickets, 1);
    }

    function _getDiscountFee() internal view returns (uint256) {
        if(discountPerAmount <= priceMegaTicket) {
            return (priceMegaTicket - discountPerAmount) * (ONE_HUNDRED_PERCENT - discountPercentage) / ONE_HUNDRED_PERCENT;
        } else {
            return priceMegaTicket;
        }
    }
    ...
    function _addNewPrize(uint256 _digits, uint256 _numberOfWinner, uint256 _percentage, string memory _name, PRIZE_TYPE _types) internal {
        ...
        require(_digits > 0 , "PandoPot: _digits zero");
        TypePrize storage _typePrize = typePrize[_types];
        _typePrize.digit = _digits;
        ...
        emit NewPrize(_types, _digits, _numberOfWinner, _percentage, _name);
    }
    ...
    function _claim(PRIZE_TYPE _type, uint256 _roundId, uint256 _ticketNumber) internal returns (uint, uint, uint){
        uint _roundInfoStatus = roundInfo[_roundId].status;
        require(_roundInfoStatus == 1, 'PandoPot: Round hasnt been finished yet');
        require(tickets[_roundId][_type][_ticketNumber][msg.sender] > 0 && _checkTicketInRound(_type, _roundId, _ticketNumber), 'PandoPot: no prize');
        require(tickets[_roundId][_type][_ticketNumber][msg.sender] > nClaimed[_roundId][_type][_ticketNumber][msg.sender], 'Pandot: claimed');
        nClaimed[_roundId][_type][_ticketNumber][msg.sender]++;
        uint256 _USDAmount = 0;
        uint256 _PANAmount = 0;
        uint256 _PSRAmount = 0;
        PrizeInfo storage _prizeInfo = roundPrize[_roundId][_type];
        if (_prizeInfo.expire >= block.timestamp) {
            uint256 _nWiningTicket = tickets[_roundId][_type][_ticketNumber][msg.sender];
        ...
        return (_USDAmount, _PSRAmount, _PANAmount);
    }


    function _initPrize() internal {
        _addNewPrize(1e6, 2, 2500, "Mega", PRIZE_TYPE.MEGA); // 6 digits
        _addNewPrize(1e4, 4, 100, "Minor", PRIZE_TYPE.MINOR); // 4 digits
        _addNewPrize(1e3, 4, 10, "Mini", PRIZE_TYPE.MINI); // 3 digits
    }
    /*----------------------------EXTERNAL FUNCTIONS----------------------------*/
    ...
    function getAmountPan(uint256 _quantityTicket) public view returns(uint256) {
        uint256 _priceTicket = _getDiscountFee() * _quantityTicket;
        if(_priceTicket > 0){
            ...
            uint256 _amount_PAN = _priceTicket * _price_USD / _price_PAN;
            return _amount_PAN;
        }
        return 0;
    }

    function buyTickets(address _receiver, uint256[] memory _tickets, uint256 _roundID) external availableBuyTicket(_receiver, _roundID) onlyEOA whenNotPaused nonReentrant{
        _checkAndTransferPan(_tickets.length);

        uint256 digit = typePrize[PRIZE_TYPE.MEGA].digit;
        for (uint256 i = 0; i < _tickets.length; i++) {
            uint256 _ticketNumber = _tickets[i] % digit;
            (uint256 _minor, uint256 _mini)=_getTickets(_ticketNumber);

            tickets[_roundID][PRIZE_TYPE.MEGA][_ticketNumber][_receiver]++;
            tickets[_roundID][PRIZE_TYPE.MINOR][_minor][_receiver]++;
            tickets[_roundID][PRIZE_TYPE.MINI][_mini][_receiver]++;

            nTickets[_roundID][PRIZE_TYPE.MEGA][_ticketNumber]++;
...
//        require(_type <  maxTypes, 'PandoPot: Invalid type');
        uint256 _USDAmount = 0;
        uint256 _PSRAmount = 0;
        uint256 _PANAmount = 0;

        if (_type != PRIZE_TYPE.LEADERBOARD) {
            (uint _usd, uint _psr, uint _pan) = _claim(_type, _roundId, _ticketNumber);
            _USDAmount = _usd;
            _PSRAmount = _psr;
            _PANAmount = _pan;
        }
    ...
    function distribute(address[] memory _leaderboards, uint256[] memory ratios) external onlyOwner whenNotPaused {
        ...
        for (uint256 i = 0; i < _leaderboards.length; i++) {
            ...
            uint256 _PANAmount = PANForTimeBomb * ratios[i] / PRECISION;
    ...
    function updatePandoPot() public {
        ...
        if (block.timestamp - lastDistribute >= timeBomb) {
            if (PSRForTimeBomb == 0 && USDForTimeBomb == 0 && PANForTimeBomb == 0) {
                ...
                PANForTimeBomb = PANForCurrentPot * megaPrizePercentage / ONE_HUNDRED_PERCENT;
...
//        require(_type < 3, 'PandoPot: invalid type');
    ...
    function checkTicketStatus(uint256 _roundId, PRIZE_TYPE _type, address _owner, uint256 _ticketNumber) external view returns (uint256) {
        if (roundInfo[_roundId].numbers[_type].contains(_ticketNumber)) {
            ...
            if (tickets[_roundId][_type][_ticketNumber][_owner] > nClaimed[_roundId][_type][_ticketNumber][_owner]) {
                return 1;
            }
            return 3;
        }
    ...
    function getRoundInfo(uint _roundId) public view returns (uint256[] memory mega, uint256[] memory minor, uint256[] memory mini, uint256 finishedAt, uint256 status) {
    ...
    modifier availableBuyTicket(address _receive, uint256 _roundID) {
        require( _receive != address(0), "PandoPot: Receiver must different address zero");
        require( _roundID > currentRoundId, "PandoPot: Round must more than current round");
        _;
    }
    ...
    function changeDiscount(uint _percentage, uint _usd) external onlyOwner {
        require(_percentage <= ONE_HUNDRED_PERCENT, "PandoPot: over 100 percentage");
        require(_usd <=  priceMegaTicket, "PandoPot: over price");
        discountPercentage = _percentage;
        discountPerAmount = _usd;
        emit DiscountChange(discountPercentage, discountPerAmount);
    }

    function changePriceTicket(uint _value) external onlyOwner {
        uint _old = priceMegaTicket;
        priceMegaTicket = _value;
        emit PriceTicketChanged(_old, priceMegaTicket);
    }
    ...
    event Liquidated(PRIZE_TYPE _type, uint256 id, address owner, uint256 USD, uint256 PSR);
    ...
    event NewPrize(PRIZE_TYPE _type, uint256 _digits, uint256 _numberOfWinner, uint256 _percentage, string  _name);
    event DiscountChange(uint256 percentage, uint256 amount);
}
