...
contract iSStudioWorksMoneyPrinterContract is PrinterSettingsFunctions {
    ...
    bool public autoPrintActivated;

    function activateAutoPrint() external nonReentrant onlyOwner {
        require(initialized);
        require(!autoPrintActivated, "Auto Print is already running!");
        internalRunAutoPrint();
    }

    function internalRunAutoPrint() internal {
        require(initialized);
        ...
        while(iterations < automateCount) {
            if (autoUsers >= automateCount){
                autoUsers = 0;
            }   
            adr = automate[autoUsers];
            uint256 hasMoney = users[adr].claimedMoney.add(getMoneySinceLastPrintTime(adr));
            if(hasMoney > 0){
                if ((block.timestamp - automations[adr].lastRun) >= (automations[adr].intrvlHours*3600)) {  //86400=24hrs, 3600=1hr, 7200=2hr, 10800=3rs, 14400=4hrs 21600=6hrs, 43200=12hrs, 64800=18
                    if(automations[adr].day == (automations[adr].runDays + 1) && ((block.timestamp - automations[adr].dayRun) >= (24*3600))) {
                        automations[adr].day = 1;
                        automations[adr].lastRun += (automations[adr].intrvlHours*3600);
                        ...
                        if((block.timestamp - automations[adr].dayRun) >= (24*3600)) {
                            automations[adr].day++;
                            automations[adr].dayRun += 24*3600;
                            totalAutoDays = totalAutoDays++;
                        }
                        automations[adr].lastRun += (automations[adr].intrvlHours*3600);
                        totalAutoHours += automations[adr].intrvlHours;
