...
contract Engine is IERC20, Auth {
    ...
    // Fees
    Fees public fee = Fees(500,0,500,200,0,0);
    ...
    //Engine Token Info
    string  constant _name          = "Engine";
    string  constant _symbol        = "ENG";
    ...
    mapping(address => RocketContractExclusions)    public rocketExclusions; //Manages exclusions with external Engine contracts
    ...
    mapping (address => bool)                       public isRocketContract; //Manages each Engine contract that is authorized to interact with the burn pools function
    ...
    //Other 
    uint256 public  tokensBurned; //Counter for tokens burned within the Engine EcoSystem
    bool    private allowContracts; //Used to prevent other contracts from interacting with our holder accessible functions that are not Engine contracts. Used to prevent spamming.
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (address(poolDistributionGateway) != address(0)) { 
            ...
            try poolDistributionGateway.onTransfer(transf) {} //store transfer data externally to be used across Engine EcoSystem
            catch Error (string memory reason) {
    ...
    //threshold of Engine to collect before burning from supply
    function setBurnThreshold(uint256 _burnThreshold) external authorized {
        burnThreshold = _burnThreshold;
    }
    //threshold to determine how much Engine needs to be in the contract to liquidate for rewards
    ...
    //Allows authorized Engine eco system contracts access to the _burnTokens function 
