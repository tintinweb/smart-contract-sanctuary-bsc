...
contract BananaAllocator is SweeperUpgradeable {
    ...
    function _moveBanana(uint256 pid, bool revert_) private {
        ...
        require(bananaRoute.toAddress != address(0), "BananaAllocator: No configured route found");
    ...
    /// @notice remove a banana route
    /// @param index index of banana route to remove
    function removeBananaRoute(uint256 index) external onlyOwner {
        require(index < bananaRoutesLength, "BananaAllocator: Index out of bounds");

        uint256 pid = bananaRoutePids[index];
