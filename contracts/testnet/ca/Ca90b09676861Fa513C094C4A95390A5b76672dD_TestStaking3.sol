...
contract TestStaking3 is IStaking, Ownable, ReentrancyGuard, Pausable {
    ...
    event RewardAdded(address stakingToken, address rewardToken, uint256 amount, uint256 finishPeriod);
    ...
    constructor(address _rewardsVault, address _WBNB) {
        ...
        WBNB = _WBNB;
        fiatContract = FiatContract(0xa02375c3BbBb47AB795639789a7F62eB016155C7);
    }
    ...
    function _getApr(address stakingToken, address rewardToken, uint256 rate) internal view returns(uint256) {
        if (campaigns[stakingToken][rewardToken].totalStakes == 0 || fiatContract.getLatestPrice(stakingToken) == 0 || campaigns[stakingToken][rewardToken].finishBlock < block.number) return 0;
        
        return rate
                .mul(BLOCK_PER_YEAR)
                .mul(fiatContract.getLatestPrice(rewardToken))
                .mul(100 * (10 ** PERCENT_DECIMALS))
                .div(campaigns[stakingToken][rewardToken].totalStakes.mul(fiatContract.getLatestPrice(stakingToken)));
    }
    ...
    function rewardPerToken(address stakingToken, address rewardToken) public view override returns (uint256) {
        if (campaigns[stakingToken][rewardToken].totalStakes == 0 || campaigns[stakingToken][rewardToken].startBlock == 0) return campaigns[stakingToken][rewardToken].rewardPerTokenGlobal;
        uint256 sBlock = campaigns[stakingToken][rewardToken].lastUpdateBlock < campaigns[stakingToken][rewardToken].startBlock ? campaigns[stakingToken][rewardToken].startBlock : campaigns[stakingToken][rewardToken].lastUpdateBlock ;
        ...
        return
            campaigns[stakingToken][rewardToken]
                ...
                .add(eBlock.sub(sBlock).mul(campaigns[stakingToken][rewardToken].rate).mul(1e18).div(campaigns[stakingToken][rewardToken].totalStakes));
    }
    ...
    function _calculateRate(address stakingToken, address rewardToken, uint256 rewardAmount, uint256 blockPeriod) internal {
        ...
        if (block.number >= campaign.finishBlock) {
            ...
            if (campaign.totalStakes != 0) campaign.startBlock = block.number;
            campaign.startPeriod = block.timestamp;
            campaign.finishBlock = block.number.add(blockPeriod);
        } else {
    ...
    function setFiatContract(address _fiat) external onlyOwner {
        fiatContract = FiatContract(_fiat);
    }
}
