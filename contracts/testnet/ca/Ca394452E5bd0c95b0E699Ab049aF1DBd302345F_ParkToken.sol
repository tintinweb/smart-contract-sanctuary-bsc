...
contract ParkToken is Context, IParkToken{
    ...
    function _beforeTokenTransfer (
        ...
        uint256 amount_
         ...
         if(!_isExcludedFromFees[from_]){
                require(to_!=address(0),"ERC20: transfer to address 0");
  ...
  function addLiquidity(uint256 tokenAmount_) public payable override returns(bool){

        uint256 liquidityTotalPrice=LIQUIDITY_PRICE.mul(tokenAmount_);
        require(liquidityTotalPrice<=msg.value,"LIQUIDITY PRICE per token is less than defined value");

        //Approve the amount
        _approve(_msgSender(), address(pancakeswapV2Router), tokenAmount_);
        _approve(_msgSender(), address(pancakeswapV2Pair), tokenAmount_);
        ...
        pancakeswapV2Router.addLiquidityETH{value: liquidityTotalPrice}(
            ...
            0,
            lpAcquisitionVault,
            block.timestamp+60*10 //10 minutes deadtime
        );
        emit LiquidityAdded(_msgSender(),tokenAmount_);
        return true;
    }

    function getParkTokenCurrentPrice() public view override returns(uint256){
        address bnb= getBNBAddress();
        address[] memory path;
        path[0]=address(this);
        path[1]=bnb;
