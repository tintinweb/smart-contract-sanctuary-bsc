...
// OpenLuck
import "./interfaces/IExecuteLucks.sol";
import "./interfaces/IBridgeLucks.sol";
...
contract BridgeLucks is Ownable, IBridgeLucks, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    IExecuteLucks public immutable executor;
    ...
    modifier onlyExecutor() {
        require(msg.sender == address(executor), "Lucks: caller must be ExecuteLucks.");
        _;
    }

    modifier onlyBridge() {
        require(msg.sender == address(this), "Lucks: caller must be bridge.");
        _;
    }
    ...
    constructor(address _layerZeroEndpoint, address _executor) {
        ...
        executor = IExecuteLucks(_executor);
    }
...
// OpenLuck
import {TaskItem,TaskExt} from "./IExecuteLucks.sol";
...
import { lzTxObj } from "./IBridgeLucks.sol";
...
struct TaskItem {
    ...
    // uint256 exclusiveAmount; // exclusive token holding amount required
...
struct Ticket {
    // the ticket's id, equal to the end number (last ticket id)
    uint32 number;
    // how many QTY the ticket joins, (number-count+1) equal to the start number of this ticket.
    uint32 count;
    // ticket owner
    address owner;
}

interface IExecuteLucks {
    // ============= events ====================
    ...
    function createTask(TaskItem memory item, TaskExt memory ext, lzTxObj memory _lzTxParams) external payable;
    function joinTask(uint256 taskId, uint32 num, string memory note) external payable;
    function cancelTask(uint256 taskId, lzTxObj memory _lzTxParams) external payable;
    function closeTask(uint256 taskId, lzTxObj memory _lzTxParams) external payable;
    function pickWinner(uint256 taskId, lzTxObj memory _lzTxParams) external payable;
