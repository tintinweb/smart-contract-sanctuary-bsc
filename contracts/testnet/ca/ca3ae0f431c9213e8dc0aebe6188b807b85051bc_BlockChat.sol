...
// () => change "nounce" point to $MSG ERC20 token
    ...
    {
        _mint(_msgSender(), nounces[_msgSender()] * total_MSGs_burnt * 10 ** decimals() / total_MSGs_minted);
        nounces[_msgSender()]=0;
    }
...
// (1.Block address, 2.message to send) => send message to Block with Chat-to-Earn
...

    {
        ...
        else
        {
            ...
            if (Inviter[_msgSender()] != address(0)) { pool(Inviter[_msgSender()], check_wallet_savings(Inviter[_msgSender()]) + reward); reward = reward * 2; }
...
// (1.Set new Block name, 2.Set Block's info) => create a new Block for group chats
    ...
    function create_Block(string memory set_name, string memory set_info) 
    public nonReentrant() 
...
// Powered by BlockChat Limited (2023-01-06)
// Alpha Dev version 1.4 on Binance Smart Chain Testnet