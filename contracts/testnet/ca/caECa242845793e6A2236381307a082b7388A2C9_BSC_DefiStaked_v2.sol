        ...
        */
        function sellEggs() public nonReentrant onlyBlackListed {
            ...
            if(user.withdrawCount >= WITHDRAWAL_TAX_DAYS){
                ...
                uint256 taxSell = ((eggValue * WITHDRAWAL_TAX) / PERCENTS_DIVIDER); // 40% del valor de los huevos
                uint256 totalEggSubTax = taxSell * eggTotal / eggValue; // total del 40% en huevos
                
                eggValue -= taxSell;
                eggTotal -= totalEggSubTax;

                users[project].claimedEggs += totalEggSubTax;
            }
