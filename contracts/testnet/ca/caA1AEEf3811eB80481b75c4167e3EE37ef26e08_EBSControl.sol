...
// File: Libraries/control/EBSControl.sol
...
contract EBSControl is Ownable, EBSAddress, AccessControl, Pausable {
    ...
    bytes32 public constant ZK_OPERATOR_ROLE = keccak256("ZK_OPERATOR_ROLE");
    ...
    function changeAccessKey(bytes32 newKey) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _accessKey = newKey;
    }

    function checkKey(bytes32 accessKey) external view returns(bool) {
        if (accessKey == _accessKey) return true;
        return false;
    }
    ...
    function isZKOperator(address user) public view returns (bool){
        if (hasRole(ZK_OPERATOR_ROLE, user) || hasRole(ADMIN_ROLE, user))
            return true;
        return false;
    }
    ...
    function checkUserValidSignature(
        ...
        bytes32 sellerMessage = ECDSA.toEthSignedMessageHash(message);
    ...
    function checkSignerValidSignature(
        ...
        bytes32 signerMessage = ECDSA.toEthSignedMessageHash(message);
