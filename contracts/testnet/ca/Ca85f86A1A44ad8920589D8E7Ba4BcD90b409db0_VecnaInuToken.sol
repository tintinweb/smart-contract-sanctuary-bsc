...
contract VecnaInuToken is IBEP20, Auth {
    ...
    constructor (
        ...
        address _exMarketingFreeReceiver
    ) Auth(msg.sender) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isMaxWalletExempt[recipient] && recipient != pair) {
            require(balanceOf(recipient).add(amount) <= _maxWallet,"Recipient Wallet exceeds the max wallet amount");
        }
    ...
    require(_balances[msg.sender] >= VTOKENS, "Not enough tokens in wallet");
...
//        emit Transfer(sender, recipient, amount);
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee,uint256 _workerRewardFee, uint256 _marketingFee, uint256 _teamFee ,uint256 _feeDenominator) external authorized {
        ...
        require(totalFee < 2500, "Fees cannot exceed 25%");
    }
