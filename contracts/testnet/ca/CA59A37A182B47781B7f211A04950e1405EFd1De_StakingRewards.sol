...
contract StakingRewards is  ReentrancyGuardUpgradeable,OwnableUpgradeable,AccessControlUpgradeable {
    ...
    function updatePeriodFinish(uint timestamp) external onlyAdmins updateReward(address(0)) {
        periodFinish = timestamp;
    }

    function _transferFeesWallets(address newOwnerCom) public onlyAdmins {
        community = newOwnerCom;
    }

    function updateTotalFees(uint256 _totalFees) external onlyAdmins{
        totalFees = _totalFees;
    }

    function updateCHoldingTime(uint256 _holdingTime) external onlyAdmins{
