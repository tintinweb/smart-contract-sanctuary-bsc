...
contract BoxBase is OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    function setOpenCoin(address _openCoin) public onlyOwner {
        openCoin = IERC20(_openCoin);
    }
    function setColdWallet(address account) public onlyOwner {
        coldWallet = account;
    }
...
contract DotaBox is BoxBase {
    ...
    function openBox(uint count) public contractIsReady {
        openCoin.transferFrom(_msgSender(), coldWallet, count * price);
        emit OpenSuccess(_msgSender(), count);
    }


    //withdraw
    function getHash(address account, uint amount) public view returns (bytes32) {
        return keccak256(abi.encodePacked(account, amount, nonces[account]));
    }

    event WithdrawSuccess(address account, uint amount);

    function withdraw(uint amount, bytes memory signature) public contractIsReady {

        //check signature
        require(ECDSA.recover(getHash(_msgSender(), amount), signature) == signer, "Invalid signature");

        //rewards
        nonces[_msgSender()] += 1;
        openCoin.transfer(_msgSender(), amount);

        emit WithdrawSuccess(_msgSender(), amount);
    }
}
