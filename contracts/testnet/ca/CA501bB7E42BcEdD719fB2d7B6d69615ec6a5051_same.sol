...
// 0x23E87E1793B17725a0c241b659A3b5fb89aE4914 remember to remove liq 0xD99D1c33F9fC3444f8101754aBC46c52416550D1
    ...
    constructor() ERC20("same", "SMT") {
        ...
        // 3% on SELL
        // 0% on Transfer
        _feesRates = Fees({buyFee: 1200, sellFee: 1200, transferFee: 1200});
    ...
    function _transfer(
        ...
        if (inSwap) {
            ...
            emit Log("super._transfer in swap", amount);
            return;
        }
        ...
        if (contractMustSwap(from, to)) {
            ...
            emit Log("contractMustSwap in swap", amount);

            uint256 numTokensToSwap = balanceOf(address(this));

            emit Log("numTokensToSwap in swap", numTokensToSwap);
    ...
    function swapTokensForUSD(uint256 tokenAmount) internal swapping  {
        address[] memory path = new address[](2);
        path[0] = address(this);
        // path[1] = dexRouter.WETH();
        path[1] = address(BUSD);
        emit Log("swapTokensForUSD",tokenAmount);
        ...
        emit Log("swapExactTokensForTokensSupportingFeeOnTransferTokens",tokenAmount);
    }

    function swapTokensForUSD1(uint256 tokenAmount) public swapping  {
        address[] memory path = new address[](2);
        path[0] = address(this);
        // path[1] = dexRouter.WETH();
        path[1] = address(BUSD);
    ...
    function swapTokensForUSD2(uint256 tokenAmount) public swapping  {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = dexRouter.WETH();
        path[2] = address(BUSD);
     ...
     function swapTokensForUSD3(uint256 tokenAmount) public   {
        address[] memory path = new address[](2);
        path[0] = address(this);
        // path[1] = dexRouter.WETH();
        path[1] = address(BUSD);
     ...
     function swapTokensForUSD4(uint256 tokenAmount) public   {
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = dexRouter.WETH();
        path[2] = address(BUSD);
