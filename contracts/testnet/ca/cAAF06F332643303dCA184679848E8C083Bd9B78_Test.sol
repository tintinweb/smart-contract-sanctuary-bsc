...
contract Test {
    address busdt = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7;
    ...
    uint256[] public REFERRAL_PERCENTS_BUY = [6, 5, 4, 3, 2, 3, 4]; 
    uint256[] public REFERRAL_PERCENTS_SELL = [0, 0, 0, 0, 1, 1, 1]; 
    ...
    function sellTokens(uint256 amountSell) public {
                ...
                if (upline != address(0)) {
                    if (users[upline].invest >= REFERRAL_MINIMUM[i] || go == true) {
                        ...
                        if (amount4 > 0) {
                            ...
                            users[upline].referalsIncome[i] += amount4;
                        }
                    }

                    // Add referral
                    addReferral(msg.sender, upline, i);
        ...
        usersMiner[msg.sender] = SafeMath.mul(SafeMath.div(usersMiner[msg.sender],100),96);
    }
    ...
    function checkAmbassadorLvl(address userAddr) public view returns (uint256) {
        uint256 invested = users[userAddr].invest;

        if (invested > REFERRAL_MINIMUM[6]) {
            return 7;
        }

        if (invested > REFERRAL_MINIMUM[5]) {
            return 6;
        }

        if (invested > REFERRAL_MINIMUM[4]) {
            return 4;
        }

        if (invested > REFERRAL_MINIMUM[3]) {
            return 3;
        }

        if (invested > REFERRAL_MINIMUM[2]) {
            return 2;
        }

        if (invested > REFERRAL_MINIMUM[1]) {
            return 1;
        }
    ...
    function checkRefIncome(address userAddr) public view returns (uint256[7]) {
        return users[userAddr].referalsIncome;
    }

    function checkRefCount(address userAddr) public view returns (uint256[7]) {
        User storage user = users[userAddr];

        return [
            user.referrals[0].length,
            user.referrals[1].length,
            user.referrals[2].length,
            user.referrals[3].length,
            user.referrals[4].length,
            user.referrals[5].length,
            user.referrals[6].length
        ];
    }

    // Add referrals
    function addReferral(address userAddr, address ref, uint256 lvl) internal {
        User storage user = users[ref];
        bool exists = false;

        for (uint256 i = 0; i < user.referrals[lvl].length; i++) {
            if (user.referrals[lvl][i] == userAddr) {
                exists = true;
            }
        }

        if (!exists && userAddr != ref) {
            user.referrals[lvl].push(userAddr);
        }
    }
