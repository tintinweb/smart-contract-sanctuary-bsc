    ...
    ReentrancyGuard
{
    ...
    function startAirdrop(
        ...
        if (tokenAmount.chainlinkFeed && _paymentMethod == address(0)) {
            price = convertPrice(
                (baseUSDTokenAmounts.airdropPricePerDay * _numDays / 1e18),
        ...
        if (airdrop.promoCode) {
            ...
            if (
                !MerkleProofLib.verify(
                    _proof,
                    airdrops[_id].merkleRoot,
                    keccak256(abi.encodePacked(_promoCode, _amount))
                )
            ) {
            ...
            if (
                !MerkleProofLib.verify(
                    _proof,
                    airdrops[_id].merkleRoot,
                    keccak256(abi.encodePacked(_who, _amount))
                )
            ) {
                require(false, "Invalid proof");
            }
    ...
    function claimERC721Tokens(
        ...
        if (airdrop.promoCode) {
            require(
                ...
                "promo code already collected from this airdrop"
            );
            if (
                !MerkleProofLib.verify(
                    _proof,
                    airdrops[_id].merkleRoot,
                    keccak256(abi.encodePacked(_promoCode, _tokenId))
                )
            ) {
                require(false, "Invalid proof");
            }
            ...
            require(
                ...
                "User already collected from this airdrop"
            );
            require(
                msg.sender == _who,
                "Only the recipient can receive for themselves"
            ...
            if (
                !MerkleProofLib.verify(
                    _proof,
                    airdrops[_id].merkleRoot,
                    keccak256(abi.encodePacked(_who, _tokenId))
                )
            ) {
                require(false, "Invalid proof");
            }
    ...
    function claimERC1155Tokens( ClaimInfo calldata _claim )
        external
        returns (bool success)
    {
        if (_claim.root.isValid) {
            require(
                !airdrops[_claim.id].rooted,
                "MediaEyeAirdrops: Airdrop already rooted."
            );
            _checkRooted(_claim.root);
        }
        Airdrop storage airdrop = airdrops[_claim.id];
        ...
        require(
            _claim.tokenIds.length == _claim.amounts.length,
        ...
        if (airdrop.promoCode) {
            ...
            for (uint256 i = 0; i < _claim.tokenIds.length; i++) {
                require(
                    airdrop.promoCollectedNfts[_claim.promoCode][_claim.tokenIds[i]] !=
                        true,
                    "promo code has already collected from this airdrop"
                );
                airdrop.promoCollectedNfts[_claim.promoCode][_claim.tokenIds[i]] = true;
            }
        } else {
            require(
                msg.sender == _claim.who,
                "Only the recipient can receive for themselves"
            );
            if (
                !MerkleProofLib.verify(
            ...
            for (uint256 i = 0; i < _claim.tokenIds.length; i++) {
                require(
                    airdrop.collectedNfts[_claim.who][_claim.tokenIds[i]] != true,
                    "User has already collected from this airdrop"
                );
                airdrop.collectedNfts[_claim.who][_claim.tokenIds[i]] = true;
            }
        ...
        emit Airdrop1155Transfer(_claim.id, msg.sender, _claim.tokenIds, _claim.amounts);
        return true;
    }
