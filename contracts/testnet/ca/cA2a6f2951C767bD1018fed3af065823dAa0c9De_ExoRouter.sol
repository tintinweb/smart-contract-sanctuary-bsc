...
// File: contracts\interfaces\IExoRouter01.sol
...
// File: contracts\interfaces\IExoRouter02.sol
...
// File: contracts\interfaces\IExoFactory.sol
...
// File: contracts\interfaces\IExoPair.sol
...
// File: contracts\libraries\ExoLibrary.sol
...
library ExoLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'699ebb9cead244f3911b42802709323d5e8cf33f743616590818ddb716be1182' // init code hash
            ))));
    }
...
contract ExoRouter is IExoRouter02 {
    ...
    function addLiquidityETH(
        ...
        address pair = ExoLibrary.pairFor(factory, token, WETH);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? ExoLibrary.pairFor(factory, output, path[i + 2]) : _to;
            IExoPair(ExoLibrary.pairFor(factory, input, output)).swap(
    ...
    function swapExactTokensForTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ExoLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        require(path[0] == WETH, 'ExoRouter: INVALID_PATH');
        amounts = ExoLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'ExoRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(ExoLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        ...
        returns (uint[] memory amounts)
    {
        require(path[path.length - 1] == WETH, 'ExoRouter: INVALID_PATH');
        amounts = ExoLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'ExoRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ExoLibrary.pairFor(factory, path[0], path[1]), amounts[0]
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = ExoLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, ExoLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(ExoLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    {
        return ExoLibrary.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    {
        return ExoLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
