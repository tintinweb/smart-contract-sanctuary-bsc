...
contract Telecrypto {
    ...
    event Raffle(uint[] luckynumbers);
    ...
    function shuffle() public view returns (uint256[] memory) {
        require(msg.sender == Admin, "Only admin can shuffle!");
        ...
        uint256 counter = 0;
        uint256 j = 0;
        bytes32 b32 = keccak256(abi.encodePacked(block.timestamp + block.number + BNBPrice + counter));
        uint256 length = uintsCopy.length;

        for (uint256 i = 0; i < uintsCopy.length; i++) {
            if (j > 31) {
                b32 = keccak256(abi.encodePacked(block.timestamp + block.number + BNBPrice + ++counter));
                j = 0;
            }
    ...
    function raffle() public {
        ...
        uint256[] memory RaffleLuckyNums = this.shuffle();

        emit Raffle(RaffleLuckyNums);
    }
}