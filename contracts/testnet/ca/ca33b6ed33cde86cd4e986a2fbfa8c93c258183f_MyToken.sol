    ...
    ERC20Upgradeable
{
     ...
     * - `_feeCollectorAddress` fee address .
     * - `tokenName` token name .
     * - `tokenSymbol` token symbol .
     * - `initialSupply` initial supply in whole number eg for 100 tokens enter 100 excluding decimals.
     * - `_sellTax` sell tax in format (wholeNumber,firstNumberAfterDecimal,secondNumberAfterDecimal) eg for 3% - 300.
     * - `_buyTax` sell tax in format (wholeNumber,firstNumberAfterDecimal,secondNumberAfterDecimal) eg for 3.59% - 359 .
     */
    function initialize(
        address _router,
    ...
    function _transfer(
        ...
        if (tradeTaxEnabled && (from == uniswapV2Pair || to == uniswapV2Pair)) {
            totalDeductions = _deductTax(amount, to);
        }
    ...
    function _deductTax(
        ...
        _balances[feeCollectorAddress] += fees;
        emit TradeTax(feeCollectorAddress, fees);
        /* went to designated tofeeCollectorAddress */
