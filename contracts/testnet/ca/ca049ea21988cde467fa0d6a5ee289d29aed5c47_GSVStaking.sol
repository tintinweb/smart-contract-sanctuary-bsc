...
contract GSVStaking {
    ...
    function _GetStakingRewardDetails(address _user,uint _tierYear) view public returns(uint256[10] memory _totalReward,uint256[10] memory _rewards,uint256[10] memory _totalRewardWithdrawal,uint256[10] memory _penaltyCollected){
        if(_tierYear==0){
            UserStakingDetails1Year storage user1year = userstakingdetails1year[_user];
