...
contract MyCoin is Context, IMyCoin, Ownable {
  ...
  uint256 public gain = 100000000;
  uint256 public prevGain = gain;
  uint256 public tokenPrice = 423934765597942400000;
  ...
  function getDecreaseVariables() public view returns (uint256, uint256) {
    uint256 _gain = getGain();
    uint256 R = 10000 - (5 * (mintedProfits * 10 / burnedTokens ) ** 3) / 1000;
    uint256 I = 200 - _gain * 200 / prevGain;
    return (R, I);
  }

  function getIncreaseVariables() public view returns (uint256, uint256) {
    uint256 _gain = getGain();
    uint256 R = 10000 + (4 * (mintedProfits * 10 / burnedTokens ) ** 3) / 1000;
    uint256 I = 200 * _gain / prevGain - 200; 
    return (R, I);
  } 

  // gain = gain * R^I 
  // R = amount to increase/decrease 
  // I = every 0.5% price change
