...
contract EMET_LpMining is ERC721Holder, Ownable, RandomGenerator{
    event AddPool(string indexed _type, address indexed LP, address indexed outputToken);
    ...
    function addPoolShare(address LP_, address outputToken_, uint share_, uint startTime_, uint endTime_,uint lp_LockTime, uint updataTime_) public onlyAdmin returns(uint pid){
        ...
        lenShare += 1;
        string memory type_ = "share";
        emit AddPool(type_, address(LP_), address(outputToken_));
    }

    function addPoolBox(address LP_, uint share_, uint startTime_, uint endTime_, uint reward_ReleaseTime, uint updataTime_) public onlyAdmin returns(uint pid){
        pid = lenBox;
        require(block.timestamp < startTime_ && startTime_ < endTime_, "out of time");
        poolInfo_Box[pid] = PoolInfo_Box(true, true, 0, share_, 0, startTime_,startTime_,endTime_, reward_ReleaseTime, updataTime_, LP_);
        lenBox +=1;
        string memory type_ = "Box";
    ...
    function updataUserReward_Pos(uint pid_, address user_) view public returns (uint) {
    ...
    function updatePool_Pos(uint pid_) internal {
        PoolInfo_Pos storage pool = poolInfo_Pos[pid_];
    ...
    function deposit_Pos(uint256 pid_, uint256 amountIn_) public {
        ...
        user.userDebt = updataUserReward_Pos(pid_, msg.sender);
        updatePool_Pos(pid_);
        // user
    ...
    function claim_Pos(uint256 pid_) public {
        ...
        if (user.lockReward != 0) {
            uint _temp1 = user.lockReward * (block.timestamp - user.lastClaimtTime) / (user.rewardLockTime - user.lastClaimtTime);
            _reward += _temp1;
            reward = _reward;
        }
        // team
    ...
    // calculate user reward
    function updataUserReward_Share(uint pid_, address user_) view public returns (uint) {
        UserInfo_Share storage user = userInfo_Share[pid_][user_];
        PoolInfo_Share storage pool = poolInfo_Share[pid_];
        uint _reward = ((block.timestamp - user.lastClaimtTime) / pool.updataTime) * (user.amount / pool.share);
        return _reward;
    }

    // Update reward variables of the given pool to be up-to-date.
    function updatePool_Share(uint pid_) internal {
        PoolInfo_Share storage pool = poolInfo_Share[pid_];   
        if (block.timestamp <= pool.lastRewardTime || block.timestamp <= pool.startTime) {
            return;
        }
    ...
    function updataUserReward_Box(uint pid_, address user_)public view returns(uint) {
        UserInfo_Share storage user = userInfo_Share[pid_][user_];
        PoolInfo_Share storage pool = poolInfo_Share[pid_];
        uint _reward = ((block.timestamp - user.lastClaimtTime) / pool.updataTime) * (user.amount / pool.share);
    ...
    // Update reward variables of the given pool to be up-to-date.
    function updatePool_Box(uint pid_) internal {
    ...
    function claim_Box(uint256 pid_)public returns(bool){
        UserInfo_Box storage user = userInfo_Box[pid_][msg.sender];
        PoolInfo_Box storage pool = poolInfo_Box[pid_];
        require (user.amount > 0, 'no amount');
        bool _claim;
        if (block.timestamp >= (user.depositTime + pool.reward_ReleaseTime)){
            _claim = true;
        }
        
        // require(_claim, "claim: not yet time");
        if (_claim){
            uint _reward = updataUserReward_Box(pid_, msg.sender);
            uint reward = _reward;
