...
contract Escrow is Ownable,Initializable {
    ...
    ///@param communityAddress community address
    ///@param allocated community allocation amount in wei
    ///@param allocated community start time
    function initialize(Dividend memory dividentParams, address communityAddress, uint allocated, uint64 startTime) external onlyOwner initializer {
        require(!isZero(uint160(dividentParams.token)), "the dividend token must not be zero");
        require(!isZero(uint160(communityAddress)), "the community address must not be zero");
        ...
        require(!isZero(allocated), "the community allocation must not be zero");
        
        dividend = dividentParams;
        community.account = communityAddress;
        community.allocated = allocated;
        community.lastClaim = community.startTime = startTime;
    }
    ...
    ///@dev update the community address
    ///@param communityAddress community address
    function updateCommunity(address communityAddress) external onlyOwner {
        require(!isZero(uint160(communityAddress)), "updateCommunity: the community address must not be zero");
        community.account = communityAddress;
    }
    ...
    ///@dev community address claim its dividends from here
    function claim() external {
        require(community.account == _msgSender(), "claim: the community wallet is not a caller");
        require(!isZero(community.startTime), "claim: start time must not be zero");
        require(!isZero(community.allocated), "claim: allocation must not be zero");
        require(community.released <= community.allocated, "claim: the community wallet has claimed all dividends");

        if(!dividend.isInitialReleased) {
            _claimInitial();
        } else {
            require(
                community.lastClaim + dividend.period <= blockTimestamp(),
    ...
    function _claimInitial() private {
        ...
        community.released += (community.allocated * dividend.initialRelease) / DIVISOR;
        
        if(community.lastClaim + dividend.period <= blockTimestamp()) {
            _claim();
        }
    }
    ...
    function _claim() private {
        ...
        if(!isZero(dividends)) {
            community.released += dividends;
            community.lastClaim += (dividend.period * claims).toUint64();
    ...
    function viewDividend() public view returns(uint dividends, uint claims) {        
        if(!isValidClaim(community)) {   
            return (0,0);
        }

        uint months = (blockTimestamp() - community.lastClaim) / dividend.period;

        if(!isZero(months)) {
            uint divAmount = (community.allocated * dividend.percent) / DIVISOR;
            dividends = divAmount * months;
            claims = months;

            if((community.released + dividends) > community.allocated) {
                dividends = community.allocated - community.released;
            }
        }
    }

    ///@dev returns true if community is allowed to claim
    ///@param wallet community info
    ///@return bool is it a valid claim, returns in boolean
    function isValidClaim(Community memory wallet) private view returns(bool) {
        return !(
