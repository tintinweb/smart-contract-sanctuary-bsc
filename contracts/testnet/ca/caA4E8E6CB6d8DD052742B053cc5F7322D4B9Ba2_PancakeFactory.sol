...
contract PancakePair is IPancakePair {
    ...
    function swap(address tokenA, address tokenB, uint256 coinNum, address to) external lock {
        ...
        if (tokenA == usdtAddress) {
            ...
            uint256 getCoinNum = (floatPrice - 2 * nowPrice + Math.sqrt((2*nowPrice-floatPrice)*(2*nowPrice-floatPrice) + 8 * floatPrice * realNum)) / (2 * floatPrice);
            nowPrice = nowPrice + getCoinNum * floatPrice;
            ...
            if(realCoinNum < max_vrw_num * (10 ** decimal)) {
                max_usdt = nowPrice + nowPrice * realCoinNum - (1+realCoinNum)*realCoinNum*floatPrice/2;
                nowPrice = nowPrice - realCoinNum * floatPrice;
            } else {
                max_usdt = nowPrice + nowPrice * max_vrw_num - (1+max_vrw_num)*max_vrw_num*floatPrice/2;
                else_usdt = (realCoinNum - max_vrw_num) * lowPrice;
                nowPrice = lowPrice;
            }
    ...
    function getSwapOut(address tokenA, address tokenB, uint256 coinNum) public view returns (uint256, uint256) {
        ...
        if (tokenA == usdtAddress) {
            ...
            if(realCoinNum < max_vrw_num * (10 ** decimal)) {
                max_usdt = nowPrice + nowPrice * realCoinNum - (1+realCoinNum)*realCoinNum*floatPrice/2;
                _nowPrice = nowPrice - realCoinNum * floatPrice;
            } else {
                max_usdt = nowPrice + nowPrice * max_vrw_num - (1+max_vrw_num)*max_vrw_num*floatPrice/2;
                else_usdt = (realCoinNum - max_vrw_num) * lowPrice;
                _nowPrice = lowPrice;
