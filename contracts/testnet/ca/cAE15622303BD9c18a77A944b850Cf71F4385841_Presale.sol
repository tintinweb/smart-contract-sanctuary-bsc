...
contract Presale is Initializable, OwnableUpgradeable {
        ...
        view
    {
        ...
        require(raisedBnb.add(_weiAmount) <= Cap);
         uint256 maxAllocation = isPublicSale
            ? preSaleInfo.maxAllocation
            : bscDaoUserAllocation[_msgSender()];
        require(
            _weiAmount >= preSaleInfo.minAllocation &&
                _weiAmount <= maxAllocation &&
                userPurchases[_msgSender()] + _weiAmount <= maxAllocation,
            "Buy Failed , u excided the max allocation"
        );
    }
    ...
    function _getTokenAmount(uint256 _weiAmount)
        ...
        returns (uint256)
    {
        return _weiAmount.div(bscDaoPrice);
    }
