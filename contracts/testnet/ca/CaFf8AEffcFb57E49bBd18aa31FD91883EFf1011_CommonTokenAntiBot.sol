...
contract CommonTokenAntiBot is IERC20, Ownable {
    ...
    uint256 public _cappedTotalSupply;
    ...
    constructor(
        ...
        uint8 decimals_,
        uint256[] memory initSet, // initialSupply_, totalSupply_
        address[2] memory initAddr, // owner_, whaleAntiBotAddress
        uint256 feeEnableAntiBot_
    ) {
        transferOwnership(initAddr[0]);
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
        _mint(initAddr[0], initSet[0]);
        _initialSupply = initSet[0];
        _address = address(this);
        _cappedTotalSupply = initSet[1];

        whaleAntiBot = IWhaleAntiBot(initAddr[1]);
        // Enable using whaleAntiBot in this contract
        antiBotEnabled = false;
        _whaleAntiBot = initAddr[1];
    ...
    {
        require(
            _totalSupply + amount <= _cappedTotalSupply,
            "cannot mint more than capped total supply"
        );
    ...
    function changeCappedSupply(uint256 amount)
        public
        onlyOwner
        returns (bool)
    {
        require(
            _cappedTotalSupply + amount >= 0,
            "capped total cannot be zero"
        );
        _cappedTotalSupply = _cappedTotalSupply + amount;
        return true;
    }
}
