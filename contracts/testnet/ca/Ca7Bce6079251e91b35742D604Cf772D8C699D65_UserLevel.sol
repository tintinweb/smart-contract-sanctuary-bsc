...
interface INftRouter {
    function createAvatar(uint256 _level, address _user) external;
}
...
contract UserLevel is Ownable, IUserLevel {
    ...
    event UpdateUserExp(address _user, uint _old, uint _new, address[] _validators);
    ...
    event NftRouterAddressChanged(address _nftRouter);
    ...
    function getBonus(address _user, address _contract) external view override returns(uint256, uint256) {
        if(msg.sender == address(0) || boostLevel[_contract].level.length == 0){
    ...
    function updateUserExp(uint _exp, uint _expiredTime, address[] memory _lStaking, uint[] memory _pIds, bytes[] memory _signature) external override{
        ...
        emit UpdateUserExp(msg.sender, _old, userInfo.exp, _validatorSignature);
    }
    ...
    function setNftRouterAddress(address _nftRouter) external override onlyOwner{
        nftRouter = INftRouter(_nftRouter);
        emit NftRouterAddressChanged(_nftRouter);
    }

    function setFarmingAddress(address _farming) external override onlyOwner{
