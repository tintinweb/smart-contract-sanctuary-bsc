...
abstract contract Crowdsale is Initializable {
  ...
  // The token being sold
  IERC20Upgradeable public _BUSDToken;
   ...
   * @param _BUSDTokenAmount BUSD Token Amount
   */
  function buyTokens(address _beneficiary, uint256 _BUSDTokenAmount) internal {
      _preValidatePurchase(_beneficiary, _BUSDTokenAmount);
      // update state
      BUSDRaised = BUSDRaised + _BUSDTokenAmount;
      _forwardFunds(_beneficiary, _BUSDTokenAmount);
  }
   ...
   * @param _beneficiary Address performing the token purchase
   * @param _BUSDTokenAmount Value in wei involved in the purchase
   */
  function _preValidatePurchase(
    address _beneficiary,
    uint256 _BUSDTokenAmount
  )
    internal
    virtual
  {
    require(_beneficiary != address(0), "Address cant be zero address");
    require(_BUSDTokenAmount != 0, "Amount cant be 0");
  }
   ...
   */
  function _forwardFunds(address _beneficiary, uint256 _BUSDTokenAmount) internal {
    _BUSDToken.transferFrom(_beneficiary, wallet, _BUSDTokenAmount);
  }
...
contract PlanetBullsCrowdSale is Crowdsale, FinalizableCrowdsale, UUPSUpgradeable{
    ...
    function initialize(
      ...
      public initializer
    {      
        _BUSDToken = IERC20Upgradeable(0x184836E7fe78F98A14f1fB5F5E0AdE4BEB33edA8); // BUSD Tokenz
    ...
    */
    ...
    /**
    * @dev Setting token busd address  
    * @param busdToken token BUSD will be updated
    */
    function setBothTokens(address busdToken) external onlyOwner {
     ...
     * @param _beneficiary Address performing the token purchase
     */
    function buyToken(address _beneficiary, uint256 _BUSDTokenAmount) external onlyWhileOpen whenNotPaused{
      ...
      _BUSDTokenAmount  = _BUSDTokenAmount * 10 ** 18;

      require(_BUSDToken.balanceOf(_beneficiary) >= _BUSDTokenAmount, "BUSD Token balance limit exceeded");

      require((_purchasedAmount[_beneficiary][round] + _BUSDTokenAmount) <= purchaseLimitInWei, "Maximum purchase Limit exceeded for this round");

      buyTokens(_beneficiary, _BUSDTokenAmount);

      // calculate token amount to be created
      uint256 token_amount = _getTokenAmount(_BUSDTokenAmount);

      // uint256 tokens = token_amount / 10;
      // uint256 balanceAmount = token_amount - tokens;
      // Dev comments - For development _vestingDurationInMonths is set to "vestingMonths" months and _lockDurationInMonths to 1 month
      // vestingToken.addTokenGrant(_beneficiary, balanceAmount, vestingMonths, 1);
      token.transfer(_beneficiary, token_amount);
      // token.transfer(vestingAddress, balanceAmount);
      _purchasedAmount[_beneficiary][round] += _BUSDTokenAmount;
    }
    ...
    //  */
    // function setVestingAddress(address _vestingAddress) external onlyOwner whenNotPaused {
