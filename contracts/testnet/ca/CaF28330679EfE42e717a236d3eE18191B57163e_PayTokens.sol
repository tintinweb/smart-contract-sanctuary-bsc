...
contract PayTokens is Ownable{


    event PayToken(address indexed token, address indexed sender, uint indexed amount, int _type);

    event WithDrawalToken(address indexed token, address indexed sender, uint indexed amount);


    function payToken(address token, uint amount, int _type) external returns(bool){
        ...
        emit PayToken(token, sender, amount, _type);
