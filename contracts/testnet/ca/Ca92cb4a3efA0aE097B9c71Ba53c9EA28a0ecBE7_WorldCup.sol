...
contract WorldCup is OwnableUpgradeable {
    ...
    function openCompetition(uint team1_, uint team2_) public onlyOwner {
        ...
        } else if (mapInstantTotal[matchId].team2Amount == 0) {
    ...
    function _setProceeds(address player) private {
        ...
        uint nowTimestamp = block.timestamp % 1 days;
        // 计算收益 - 冠军赛
        if (nowTimestamp > (endTime % 1 days)) {
            ...
            for (uint i = 0; i < len; i++) {
                ...
                // 当天收益结算
                mapPlayerInfo[player].ssoulAmount -= mapPlayerInfo[player].destroySsoul;
            }
            // 删除已结算参与的冠军池记录
            for (uint i = 0; i < len; i++) {
    ...
    function getEarningsData(address player) public view returns(PlayerEarningInfo memory){
        ...
        // earningInfo.instantEarningsed = mapPlayerInfo[player].instantEarningsed;
        earningInfo.championEarnings = mapPlayerInfo[player].championEarnings;
        // earningInfo.championEarningsed = mapPlayerInfo[player].championEarningsed;
        if (mapPlayerRecord[player].length != 0 && mapPlayerRecord[player][0].matchId != matchId) {
            ...
            // 计算收益 - 冠军赛
            uint len = mapPlayerChampion[player].length;
            if (nowTimestamp > (endTime % 1 days) && len > 0) {
                for (uint i = 0; i < len; i++) {
                    // 冠军收益 = 当天 ssoul 积分 / 当天冠军池 ssoul 总量 * 当天冠军池 soul 总量
                    earningInfo.championEarnings += mapPlayerChampion[player][i].ssoulAmount * 1 gwei / mapChampionTotal[mapPlayerChampion[player][i].championIndex].ssoulAmount * mapChampionTotal[mapPlayerChampion[player][i].championIndex].soulAmount / 1 gwei;
                }
            }
        } 

        earningInfo.instantEarnings = earningInfo.instantEarnings + mapPlayerInfo[player].instantEarnings;
        earningInfo.instantEarningsed = earningInfo.instantEarningsed + mapPlayerInfo[player].instantEarningsed;
