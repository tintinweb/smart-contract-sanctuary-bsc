...
contract DecCas  {
    ...
    function play_bet(uint[] memory numbers, uint amount) public returns (uint) {

        plays.push(PlayInfo({block_number: block.number, play_numbers: numbers, play_amount: amount}));
    ...
    function check_play(uint play_id) public view returns (uint [] memory) {
        ...
        uint[] memory winners = new uint[](play.play_numbers.length + 1);
        winners[0] = res;
        uint win_count = 1;

        for (uint i; i< play.play_numbers.length;i++){
          if (play.play_numbers[i] == res){
              winners[win_count] = play.play_numbers[i];
              win_count = win_count + 1;
          }
        }

        uint[] memory win_res = new uint[](win_count);
