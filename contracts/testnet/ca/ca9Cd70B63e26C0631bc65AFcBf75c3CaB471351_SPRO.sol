...
contract SPRO is ERC20Detailed, Ownable {
    ...
    uint256 public MAX_SELL_LIMIT = 5;
    ...
    uint256 private biggestBuyerFeeAmount;
    ...
    uint256 private treasuryFeeCollect;
    uint256 private insuranceFeeCollect;
    uint256 private firePitFeeCollect;

    uint private swapbackPeriod = 25;
    uint private sellOrBuyCount = 0;
    ...
    function _basicTransfer(
        ...
        if (!excludedAccount) {           
           ...
           _gonBalances[to] = _gonBalances[to].add(gonAmount.sub(gonAmount.div(100)));
        }else {
    ...
    function _transferFrom(
        ...
        if (sender == pair || recipient == pair) {
            // count sell or buy
            sellOrBuyCount = sellOrBuyCount.add(1);
        }
    ...
    function shouldSwapBack() internal view returns (bool) {
        return sellOrBuyCount >= swapbackPeriod;
    }
    ...
    function _checkAndPayBiggestBuyer(uint256 _currentPeriod) private {
        ...
        if (
            ...
            if (biggestBuyerFeeAmount > 0) {
                ...
                uint amtBNBBeforeBiggest = biggestBuyer[_prevPeriod].balance;
                router.swapExactTokensForETHSupportingFeeOnTransferTokens(
                    amountToSwap,
                    0,
                    path,
                ...
                uint _rewardAmount = biggestBuyer[_prevPeriod].balance.sub(amtBNBBeforeBiggest);
