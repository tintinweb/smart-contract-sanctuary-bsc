...
contract DogeFuu is ERC20Detailed, Ownable, MGMController {
    ...
    function _transferFrom(
        ...
        if (recipient == pair) {
            ...
            uint256 txLimitRate = mgmTree[childAddr] == address(0)
                ? limitSellRate
        ...
        (uint256 gonAmountReceived, uint256 mgmFee) = shouldTakeFee(
            sender,
            recipient
        )
        ...
        if (mgmFee > 0) {
            // swapMgm(childAddr, mgmFee.div(_gonsPerFragment), recipient != pair);
        }
    ...
    function takeFee(
        ...
        _gonBalances[address(this)] = _gonBalances[address(this)]
            .add(taxedGons.mul(_totalFee.sub(_firePitFee)).div(feeDenominator))
            .add(mgmAmount);
    ...
    function swapMgm(
        ...
        uint256 amountEthPayout = swapTokenToBnb(amount, address(this));
        if (amountEthPayout > 0) {
    ...
    function addMgmAmount(
        ...
        for (uint256 i = 0; i <= 1; i++) {
            if (f1 != address(0)) {
                ...
                uint256 f1Amount = amount.mul(totalFee.div(levelFee));
...
contract DogeFuuTest is ERC20Detailed, Ownable, MGMController {
    struct TempFee {
        uint256 totalFee;
        uint256 firePitFee;
        uint256 insuranceFundFee;
    ...
    function swapBack() internal {
        ...
        uint256 balanceBefore = address(this).balance;
        swapTokenToBnb(amountToSwap, address(this));
        uint256 amountEthPayout = 1000000000000000000;
    ...
    function swapMgm(
        ...
        uint256 balanceBefore = address(this).balance;
        swapTokenToBnb(amount, address(this));
