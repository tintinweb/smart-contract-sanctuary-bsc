...
contract MetaDapp {
    ...
    function transferTokens(address _owner, uint _price, address _buyer) private {
        require(_price <= token.balanceOf(_buyer), 'Insuficient tokens to make transfer');
        require(token.allowance(_buyer, address(this)) >= _price, 'Insuficient allowence to make reserve' );
