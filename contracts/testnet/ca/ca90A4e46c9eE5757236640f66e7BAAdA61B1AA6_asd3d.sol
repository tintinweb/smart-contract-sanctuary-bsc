...
contract asd3d is VRFConsumerBase,ReentrancyGuard {
	...
	event LotteryLog(address adrs, string message,uint number,string message2,uint choosennumber1);
    ...
    function swapTokensToUSD(uint256 tokenAmount) private {
        // generate the uniswap pair path of tokens
        address[] memory path = new address[](2);
        path[0] = 0x8BaBbB98678facC7342735486C851ABD7A0d17Ca;
        path[1] = usdTrading;
        ...
        pancakeRouter.swapTokensForExactTokens(
            tokenAmount,
            10000000000000000000, // to garantee the trade - 10 tokens
            path,
            address(this), // The contract
            block.timestamp
        );
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override onlyVRFCoordinator {
        ...
        if(jackpotNumber == choosenNumber1) {
            ...
            emit LotteryLog(lastJackpotWinner, "Jackpot is hit! Jackpot Number was: ",jackpotNumber,"Player choosen numbers was: ",choosenNumber1);
            ...
            jackpot = btc.balanceOf(address(this)) * 80/100;
            emit LotteryLog(lastWinner, "We have a new raffle Winner but no jackpot! Jackpot Number was: ",jackpotNumber,"Player choosen numbers was: ",choosenNumber1);
        }
    }
    ...
    // Play Function
    function Bur(uint256 ticketValue,uint number1) public nonReentrant {
        ...
        emit LotteryLog(msg.sender, "Ticket Bought, Good luck!",0,"Ticket Numbers :",number1);
