    ...
    ERC2981
{
    ...
    event Blocklisted(address indexed account, bool isBlocklisted);
    ...
    modifier allowTransfer(address sender, address recipient) {
        require(
            !paused() || allowlisted[sender] || allowlisted[recipient],
    ...
    function mintMultiple(
        ...
        require(to.length == ids.length, "Invalid ids length");
        require(to.length == amounts.length, "Invalid amounts length");

        for (uint256 i = 0; i < to.length; ) {
            require(!blocklisted[to[i]], "Blocklisted  recipient");
    ...
    function mintMultipleWithData(
        ...
        require(to.length == ids.length, "Invalid ids length");
        require(to.length == amounts.length, "Invalid amounts length");
        require(to.length == data.length, "Invalid data length");
    ...
    function _safeTransferFrom(
        ...
        uint256 amount,
        bytes memory data
    ) internal virtual override allowTransfer(sender, recipient) {
        super._safeTransferFrom(sender, recipient, id, amount, data);
    }
    ...
    function _safeBatchTransferFrom(
        ...
        super._safeBatchTransferFrom(sender, recipient, ids, amounts, data);
    }
}