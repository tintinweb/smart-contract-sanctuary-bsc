...
contract PinkyBrain is Context, IERC20, Ownable {
    ...
    address public constant BUSD =
        address(0x8301F2213c0eeD49a7E28Ae4c3e91722919B8B47); //BUSD
    ...
    address public liqWallet = 0xF963f145cd74C4c63EbD6AB4867aA846D99e8f66;
    ...
    uint256 public totalFeesTax = _taxFee.add(_reflectionFee).add(_liquidityFee).add(_marketingFee);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculatereflectionFee(tAmount);
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _reflectionFee == 0 && _liquidityFee ==0 && _marketingFee == 0) return;
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && !_isExcludedFromFee[to])
        {
            ...
            uint256 totalTaxForSwap = _reflectionFee.add(_marketingFee).add(_liquidityFee);
            ...
            if(_liquidityFee > 0) {
                uint256 tokensForLiq = contractTokenBalance.mul(_liquidityFee).div(totalTaxForSwap);
                swapAndSendLiq(tokensForLiq);
            }
            swapAndLiquify(contractTokenBalance.mul(_reflectionFee).div(totalTaxForSwap));
      ...
      function swapAndSendLiq(uint256 tokens) internal {
        uint256 initialBalance = address(this).balance;
        // swap tokens for ETH
        swapTokensForEth(tokens); 
        uint256 newBalance = address(this).balance.sub(initialBalance);
        (bool success,) =  address(liqWallet).call{value: newBalance}("");
        if (success) {
    ...
    function setBotFees() private {
        ...
        _reflectionFee = _previousreflectionFee * _botIncreaseFee;
    ...
    {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
        (tTransferAmount, rTransferAmount) = takeLiq(sender, tTransferAmount, rTransferAmount, tAmount);
     ...
     function takeLiq(address sender, uint256 tTransferAmount, uint256 rTransferAmount, uint256 tAmount) private
    returns (uint256, uint256)
    {
        if(_liquidityFee==0) {  return(tTransferAmount, rTransferAmount); }
        uint256 tLiq = tAmount.div(100).mul(_liquidityFee);
        uint256 rLiq = tLiq.mul(_getRate());
        rTransferAmount = rTransferAmount.sub(rLiq);
        tTransferAmount = tTransferAmount.sub(tLiq);
        _rOwned[address(this)] = _rOwned[address(this)].add(rLiq);
        emit Transfer(sender, address(this), tLiq);
    ...
    function setliquidityFee(uint256 liquidityFee) external onlyOwner() {
        ...
        totalFeesTax = _taxFee.add(_liquidityFee).add(_marketingFee).add(_reflectionFee);
