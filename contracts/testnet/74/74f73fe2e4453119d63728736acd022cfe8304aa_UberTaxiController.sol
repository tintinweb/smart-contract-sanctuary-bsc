...
contract UberTaxiController is Ownable {
    ...
    //修改兑换手续费
    function setServiceCharge(uint256 serviceChargeNum) public view returns(uint256 serviceCharge){

        return serviceCharge = serviceChargeNum;

    }

    //修改转账手续费
    function setTransferCharge(uint256 transferChargeNum) public view returns(uint256 transferCharge){

        return transferCharge = transferChargeNum;

    }

    //商城购买手续费
    function setSellFee(uint256 sellFeeNum) public view returns(uint256 sellFee){

        return sellFee = sellFeeNum;

    }

    //返佣比例
    function setCommissionFee(uint256 commissionFeeNum) public view returns(uint256 commissionFee){

        return commissionFee = commissionFeeNum;
