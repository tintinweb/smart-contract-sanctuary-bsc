...
contract SantaClubDAO is Ownable {
    ...
    uint256 public votesNeeded = (SANTA_TOKEN_TOTAL_SUPPLY * 1) / 20; // 5%
    ...
    constructor() {
        tokenCustody = ITokenCustody(
            0x2D91A3d97098A6019a623a0f7325c450a8bCa924
        );
        staking = ISantaClubStaking(0xbe32fF0C5c4dbC880869C55e5Dba94AFD728b678);
    ...
    function isEligibleVoter(address voter) public view returns (bool) {
        return tokenCustody.balanceOf(voter) > 0;
    }

    function isEligibleProposer(address proposer) public view returns (bool) {
    ...
    function getProposalIDCount() external view returns (uint256) {
        return proposalIDCount;
    }
    ...
    function submitProposal(
        ...
        require(
            isEligibleProposer(msg.sender),
            "Submit Proposal: You have no proposal rights."
        );

        require(
            endTime - block.timestamp < 8 days,
            "Submit Proposal: You cannot set a voting period of more than 8 days."
        );
    ...
    function submitVote(uint256 proposalID, Vote vote) external returns (bool) {
        require(
            isEligibleVoter(msg.sender),
            "Submit Vote: You have no voting rights."
        );
        ...
        require(
            tokensUnlockedAt >= proposals[proposalID].endTime,
            "Submit Vote: You need to lock tokens for more than a proposal end time."
        );
...
contract SantaClubStaking is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    uint256 private constant DEFAULT_DAILY_REWARD = 5;
 ...
 * @dev Store Santa tokens to cast votes on DAO
 * @author Kris Kringle
 */
...
contract TokenCustody is ReentrancyGuard, Ownable {
    ...
    event LockTokens(address owner, uint256 amount, uint256 unlockedAt);
    ...
    function lockTokens(uint256 endTime) external nonReentrant {
        require(
        ...
        require(
            endTime - block.timestamp < 8 days,
            "Lock Tokens For DAO votes: You cannot lock more than 8 days."
        );
        require(
            custody[msg.sender].owner == msg.sender,
            "Lock Tokens For DAO votes: You are not an owner."
        );
        custody[msg.sender].unlockedAt = endTime;

        emit LockTokens(msg.sender, custody[msg.sender].amount, endTime);
    }
