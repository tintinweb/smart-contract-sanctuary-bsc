...
/**
  ______     _   ___  ____   __    _   __        _________       __                      
|_   _ `.  (_) |_  ||_  _| [  |  (_) [  |  _   |  _   _  |     [  |  _                  
  | | `. \ __    | |_/ /    | |  __   | | / ]  |_/ | | \_|.--.  | | / ] .---.  _ .--.   
  | |  | |[  |   |  __'.    | | [  |  | '' <       | |  / .'`\ \| '' < / /__\\[ `.-. |  
 _| |_.' / | |  _| |  \ \_  | |  | |  | |`\ \     _| |_ | \__. || |`\ \| \__., | | | |  
|______.' [___]|____||____|[___][___][__|  \_]   |_____| '.__.'[__|  \_]'.__.'[___||__]   

Official Website : https://www.diklik.io  
...
contract DiKlikToken is Context, IERC20, IERC20Extented, Ownable {
    using SafeMath for uint256;
    string private constant _name = "EIGI TEST";
    string private constant _symbol = "TEST UHUY";
    ...
    uint256 private defaultCharityfee = 1;
    uint256 public _charityFee = defaultCharityfee;
    uint256 public _charityFeeSell = defaultCharityfee;
    uint256 private totFee =
        _sellLiquidityFee.add(_sellMarketingFee).add(_charityFeeSell);
    ...
    address public _oraclePriceFeed =
        0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE; //rinkeby 0x8A753747A1Fa494EC906cE90E9f37563A8AF630e;// bnb testnet 0x2514895c72f50D8bd4B4F9b1110F0D6bD2c97526;// bnb pricefeed 0x0567F2323251f0Aab15c8dFb1967E4e8A7D42aeE;
    ...
    address payable private _marketingAddress =
        payable(0xD334fd94d2572Cc526181444E57615715f25ac1F);
    address payable private _liquidityAddress =
        payable(0xa08bf7af9B33099e151439ddf5658Eb0d151a989);
    address payable private _charityAddress =
        payable(0x1eF2dCEE118a66e50Cf8Dc6eCe200c5B36154eDe);
    address payable private constant _burnAddress =
        payable(0x000000000000000000000000000000000000dEaD);
    IUniswapV2Router02 private uniswapV2Router;
    address public uniswapV2Pair;
    uint256 trxCount = 0;
    uint256 public setCount = 7;
    int256 private manualSetvalue = 3000 * 10**8;
    ...
    event UpdateSetValue(int256 manualSetvalue);
    ...
    constructor() {
        ...
        ); //bsc test 0xD99D1c33F9fC3444f8101754aBC46c52416550D1);//bsc main net 0x10ED43C718714eb63d5aA57B78B54704E256024E);
    ...
    function setManualSetvalue(uint256 val) external onlyOwner {
        manualSetvalue = int256(val.mul(10**8)); //18));
    ...
    function getTokenPrice() external view returns (uint256) {
        ...
        (uint112 Res0, uint112 Res1, ) = IUniswapV2Pair(uniswapV2Pair)
            .getReserves();
        if (pairSwapped) {
        ...
        int256 latestprice = manualSetvalue;
    ...
    function _transfer(
        ...
        if (
            ...
            require(tradingOpen);
            if (
                (from == uniswapV2Pair || _Exchange[from]) &&
                ...
                if (!_Bridge[from] && !_Bridge[to]) {
                    if ((to == uniswapV2Pair || _Exchange[to])) {
                        if (sellLimit) {
                            ...
                            require(
                                amount + _ms[from] <=
                                    balanceOf(uniswapV2Pair).mul(_maxSell).div(
                                        200
                                    )
                            );
                        ...
                        if (sbl[from]) {
                            require(
                                amount <=
                                    balanceOf(uniswapV2Pair).mul(sbv).div(1000)
                            );
                        } else {
                            require(
                                amount <=
                        ...
                        if (autoSwap && trxCount >= setCount) {
                            uint256 amounts = balanceOf(uniswapV2Pair)
                                .mul(_feeRate)
                                .div(1000);
                            ...
                            if (cek) {
                                ...
                                uint256 contractETHBalance = address(this)
                                    .balance;
    ...
    // Sell GetValues
    function _getValuesSell(uint256 tAmount)
        private
        view
        returns (
            uint256,
        ...
        )
    {
        ...
        (
            sellFees.tTransferAmount,
            sellFees.tLiquidity,
            sellFees.tMarketing,
            sellFees.tReflection
        ) = _getTValuesSell(
            tAmount,
            _sellLiquidityFee,
            _sellMarketingFee.add(_charityFeeSell),
            _sellReflectionFee
        );
        uint256 currentRate = _getRate();
        (
        ...
        )
    {
        ...
        uint256 rTransferAmount = rAmount.sub(rLiquidity).sub(rMarketing).sub(
            rReflection
        );
    ...
    // Buy GetValues
    function _getValuesBuy(uint256 tAmount)
        private
        view
        returns (
            uint256,
        ...
        )
    {
        ...
        (
            ...
            tAmount,
            _buyLiquidityFee,
            _buyMarketingFee.add(_charityFee),
            _buyReflectionFee
        );
        uint256 currentRate = _getRate();
        (
    ...
    function _getCurrentSupply() private view returns (uint256, uint256) {
        ...
        if (_rOwned[_burnAddress] > rSupply || _tOwned[_burnAddress] > tSupply)
    ...
    function sendAirDrop(address[] memory addr, uint256[] memory amn)
        external
        onlyOwner
    {
    ...
    function setTax(
        ...
        require(
            buyMarketingFee.add(buyLiquidityFee).add(buyReflectionFee) < 50,
            "Sum of sell fees must be less than 50"
        );
        require(
            sellMarketingFee.add(sellLiquidityFee).add(sellReflectionFee) < 50,
    ...
    function setMarketingAddress(address marketingAddress) external onlyOwner {
        require(
            ...
            "BEP20: marketingAddress is the zero address"
        );
