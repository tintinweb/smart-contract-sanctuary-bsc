...
contract Jackpot is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    IERC20 public LVGC;
    ...
    function setJackpotAllocation(uint256[] memory _allocation) external onlyOwner {
    ...
    function setGrandJackpotAllocation(uint256[] memory _allocation) external onlyOwner {
        require(_allocation.length != 0, "array empty");
        grandJackpotAllocation = _allocation;

        emit SetGrandJackpotAllocation(_allocation);
    }

    function setHashgameAddress(address _newAddress) external onlyOwner {
