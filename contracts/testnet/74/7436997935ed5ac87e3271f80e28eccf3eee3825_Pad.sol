...
contract Pad is ERC20, Ownable {
    ...
    constructor() ERC20("ABC", "ABC") {
        ...
        uint256 totalSupply = 1 * 1e9 * 1e5;

        maxTransactionAmount = 1 * 1e6 * 1e5;
        maxWallet = 10 * 1e6 * 1e5;

        buyTotalFees = 10;
        sellTotalFees = 10;
        
        devWallet = 0x4D6fa494444A69ac8b2CEF6f521A14045c3e3fd7;
    ...
    function updateMaxTransactionAmount(uint256 _amount) external onlyOwner {
        require(_amount >= 10000, "Must keep fees at 10000 token or more");
        maxTransactionAmount = _amount * 1e5;
    }

    function updateMaxWallet(uint256 _amount) external onlyOwner {
        require(_amount >= 100000, "Must keep fees at 100000 token or more");
        maxWallet = _amount * 1e5;
    }
    ...
    function updateDevWallet(address _devWallet) external onlyOwner {
        require(deadAddress != _devWallet, "DevWallet can not be dead address");
