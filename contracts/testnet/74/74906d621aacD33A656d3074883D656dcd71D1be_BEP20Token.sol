...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  enum StakeType { ACCUMULATING, DISTRIBUTING }
  ...
  constructor() public {
    ...
    _stakes.push(Stake(0, StakeType.DISTRIBUTING, address(0x0)));
  ...
  */
  ...
  function stake(uint256 amount) external returns (bool) {
    _stake(_msgSender(), amount, StakeType.DISTRIBUTING);
    return true;
  }
  ...
  function payStakes(uint256 percents) external onlyOwner returns (bool) {
    _payStakes(percents, owner());
    return true;
  }
  ...
  function _createTempWallet(address creator, address tempWalletAddress, uint256 amount) internal {
    ...
    _stake(tempWalletAddress, amount, StakeType.DISTRIBUTING);
  }
  ...
  function _getAvailableBalance(address spender) internal view returns (uint256) {
    if (!_tempWallets[spender].isValid) {
      ...
      if (lockedAmount < stakedBalanceOf(spender))
        return balanceOf(spender);
      
      return totalBalanceOf(spender).sub(lockedAmount);
    }
  }

  function _stake(address stakeholder, uint256 amount, StakeType stakeType) internal {
    ...
    if (stakeIndex > 0 && _stakes[stakeIndex].amount > 0)
      require(_stakes[stakeIndex].stakeType == stakeType, "WBM: can't add to stake of different type");
    else if (stakeIndex == 0) {
    ...
    _stakes[stakeIndex].stakeType = stakeType;
    _stakes[stakeIndex].stakeholder = stakeholder;
  ...
  function _payStakes(uint256 percents, address payer) internal {
    require(_balances[payer] >= _totalStakedAmount.mul(percents).div(100), "WBM: account not able to pay for stake dividends");
    ...
    for(uint256 i = 0; i < _stakes.length; i++) {
      ...
      uint256 payout = stakeAmount.mul(percents).div(100);
      uint256 newAmount = stakeAmount.add(payout);
      payedAmount = payedAmount.add(payout);

      if (_stakes[i].stakeType == StakeType.ACCUMULATING) {
        _totalStakedAmount = _totalStakedAmount.add(payout);
        _stakes[i].amount = newAmount;
      }
      else {
