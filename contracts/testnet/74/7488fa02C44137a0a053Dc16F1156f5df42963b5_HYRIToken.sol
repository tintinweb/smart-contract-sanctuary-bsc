...
contract HYRIToken is ERC20Detailed {
    ...
    address public operationsReceiver = 0x7f101E79146c80ee21b035BE8c0F96dC0977F8A9;
    address public xHYRIReceiver = 0x5c88770e7F09BdEE255373F02aFEE7CdB766108F;
    ...
    */
    ...
    uint256[3] public xHYRIFee           = [130, 400, 1400];
    ...
    constructor(address _router) ERC20Detailed("Test Hyperion", "TESTHYRI", uint8(DECIMALS)) {
        owner = msg.sender;
        ...
        _isFeeExempt[xHYRIReceiver] = true;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToxHYRI = contractTokenBalance.mul(xHYRIFee[0] + xHYRIFee[1] + xHYRIFee[2]).div(realTotalFee);
        uint256 amountToEcosystem = contractTokenBalance.mul(ecosystemFee[0] + ecosystemFee[1] + ecosystemFee[2]).div(realTotalFee);
        uint256 amountToBurn = contractTokenBalance - amountToLiquify - amountToRFV - amountToTreasury - amountToOperation - amountToxHYRI - amountToEcosystem;
        ...
        if (amountToxHYRI > 0) {
            _swapTokensForBNB(amountToxHYRI, xHYRIReceiver);
        }
    ...
    function setFeeReceivers(address _liquidityReceiver, address _treasuryReceiver, address _riskFreeValueReceiver, address _futureEcosystemReceiver, address _operationReceiver, address _xHYRIReceiver, address _burnReceiver) external onlyOwner {
        liquidityReceiver = _liquidityReceiver;
        treasuryReceiver = _treasuryReceiver;
        riskFreeValueReceiver = _riskFreeValueReceiver;
        operationsReceiver = _operationReceiver;
        xHYRIReceiver = _xHYRIReceiver;
        futureEcosystemReceiver = _futureEcosystemReceiver;
        burnReceiver = _burnReceiver;
    }

    function setFees(uint8 _feeKind, uint256 _total, uint256 _liquidityFee, uint256 _riskFreeValue, uint256 _treasuryFee, uint256 _feeFee, uint256 _operationFee, uint256 _xHYRIFee, uint256 _burnFee) external onlyOwner {
        require (_liquidityFee + _riskFreeValue + _treasuryFee + _feeFee + _operationFee + _xHYRIFee + _burnFee == 100, "subFee is not allowed");
        ...
        xHYRIFee[_feeKind] = _total * _xHYRIFee;
