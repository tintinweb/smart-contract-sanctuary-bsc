...
contract BCNFTStaking is Context, Ownable {
    ...
    function stake(uint256 _period) external {
        ...
        for(uint256 i = 0; i < _tokens.length; i++) {
            iBCNFT.transferFrom(msg.sender, address(this), _tokens[i]);
        }
    ...
    function isStakable (address owner) public returns(bool) {
        ...
        uint256[] memory levels = iBCNFT.checkAccountLevels(owner);
    ...
    function getAccountLevelsInfo(address _owner) public view returns(uint256[] memory) {
        return iBCNFT.checkAccountLevels(_owner);
    }
    ...
    function getTokensOfOwner(address _owner) public view returns(uint256[] memory) {
        return iBCNFT.getTokensOfOwner(_owner);
    }
    ...
    function claimReward() external returns(bool) {
        ...
        bool result = iBCToken.transfer(address(this), msg.sender, totalToken.mul(stakers[msg.sender].rewardPercentage.add(stakers[msg.sender].bonusPercentage)).div(100));

        uint256[] memory _tokens = stakers[msg.sender].tokenIds;
        for(uint256 i = 0; i < _tokens.length; i++) {
            iBCNFT.transferFrom(address(this), msg.sender, _tokens[i]);
        }
