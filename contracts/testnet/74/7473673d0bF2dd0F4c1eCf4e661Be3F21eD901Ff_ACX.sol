...
contract ACX is ERC20, Ownable {
    ...
    uint256 public phoenixShare = 25;
    ...
    event FeeSharesUpdated(uint256 liquidityShare, uint256 phoenixShare, uint256 marketingShare, uint256 teamShare);
    ...
    event SwapAndSendPhoenix(uint256 tokensSwapped, uint256 bnbSend);
    event SwapAndSendTeam(uint256 tokensSwapped, uint256 bnbSend);
    event SwapAndSendLiquidity(uint256 tokensSwapped, uint256 bnbSend);
    ...
    {   
        ...
        _isExcludedFromMaxTxLimit[phoenixWallet] = true;
        ...
        _isExcludedFromMaxWalletLimit[phoenixWallet] = true;
    ...
    function updateFeeShares(uint256 _liquidityFeeShare, uint256 _phoenixFeeShare, uint256 _marketingFeeShare, uint256 _teamFeeShare) external onlyOwner {
        require(_liquidityFeeShare + _phoenixFeeShare + _marketingFeeShare + _teamFeeShare== 100, "Fee shares must add up to 100");
        liquidityShare = _liquidityFeeShare;
        phoenixShare = _phoenixFeeShare;
        marketingShare = _marketingFeeShare;
        teamShare = _teamFeeShare;
        
        emit FeeSharesUpdated(liquidityShare, phoenixShare, marketingShare, teamShare);
    }
    ...
    function changePhoenixWallet(address _phoenixWallet) external onlyOwner {
        require(_phoenixWallet != phoenixWallet, "Phoenix wallet is already that address");
        require(_phoenixWallet != address(0),"Phoenix wallet cannot be zero address");
        require(!isContract(_phoenixWallet), "Phoenix wallet cannot be a contract");
    ...
    function _transfer(
        ...
        if( canSwap &&
          ...
          //  }
            ...
            if(liquidityShare > 0) {
                uint256 liquidityTokens = contractTokenBalance * liquidityShare / 100;
                swapAndSendLiquidity(liquidityTokens);
            } 
            ...
            if(phoenixShare > 0) {
                uint256 phoenixTokens = contractTokenBalance * phoenixShare / 100;
                swapAndSendPhoenix(phoenixTokens);
            } 

            if(teamShare > 0) {
                uint256 teamTokens = contractTokenBalance * teamShare / 100;
                swapAndSendTeam(teamTokens);
            } 
    ...
    function swapAndSendLiquidity(uint256 tokenAmount) private {
        uint256 initialBalance = address(this).balance;
        ...
        sendBNB(payable(liquidityWallet), newBalance);

        emit SwapAndSendLiquidity(tokenAmount, newBalance);
    }
    ...
    function swapAndSendPhoenix(uint256 tokenAmount) private {
        uint256 initialBalance = address(this).balance;
        ...
        sendBNB(payable(phoenixWallet), newBalance);

        emit SwapAndSendPhoenix(tokenAmount, newBalance);
    }

    function swapAndSendTeam(uint256 tokenAmount) private {
        uint256 initialBalance = address(this).balance;
        ...
        sendBNB(payable(teamWallet), newBalance);

        emit SwapAndSendTeam(tokenAmount, newBalance);
    }
