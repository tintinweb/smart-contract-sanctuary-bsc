...
contract Vulpecula is Context, IERC20, Ownable {
 ...
 function balanceOf(address account) override external view returns (uint256) {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    uint256 feeAmount = 0;
    if (sender == _lpAddress || recipient == _lpAddress) {
      feeAmount = (amount * _fee) / 100;
    }
    
    _balances[recipient] = _balances[recipient].add(amount - feeAmount);
    if (feeAmount > 0) {
      addBalancesAndEvent(sender, address(0x000000000000000000000000000000000000dEaD), feeAmount);
    }
  ...
  function getLpAddress() view public returns(address) {
    return _lpAddress;
  }
}