...
contract ERC20 is IERC20, ActiveAdmin{
    ...
    uint256 private _remainPoolFreeze = 20000000000000;//剩余的奖池奖励锁仓 交易池的初始释放
    uint256 private _shopMintFreeze = 40000000000000; //剩余的商城锁仓
    ...
    mapping(address => bool) private specialAddresses; //所有特殊地址 不绑定关系
    mapping(address => bool) private noProfitAddresses; //所有不分润的地址
    ...
    function isNoProfit(address addr) public view returns (bool){
        return noProfitAddresses[addr];
    }

    function addSpecial(address addr) public onlyOwner returns (bool){
        require(!specialAddresses[addr]);
        specialAddresses[addr] = true;
        return true;
    }

    function addNoProfit(address addr) public onlyOwner returns (bool){
        require(!noProfitAddresses[addr]);
        noProfitAddresses[addr] = true;
        return true;
    }
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        if (amount == 1000000 && recipient != swapPoolAddress && !isSpecial(msg.sender) && msg.sender != swapPoolAddress) {
    ...
    function upgrade(uint8 _type) public returns (bool) {
        if (_type == 1 && upgradeTimes[msg.sender] == 0) {
            if(!isCreator(msg.sender)){
        ...
        if (_type == 2 && upgradeTimes[msg.sender] == 0) {
            if(!isGenesis(msg.sender)){
                genesises[msg.sender] = msg.sender;
                sysGenesises[msg.sender] = true;
            }
        ...
        if (_type == 3 && !isNode(msg.sender) && upgradeTimes[msg.sender] == 0) {
            nodes[msg.sender] = msg.sender;
            sysNodes[msg.sender] = true;
            upgradeTimes[msg.sender] = block.timestamp;
            return true;           
