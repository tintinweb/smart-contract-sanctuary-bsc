...
contract Presale is ReentrancyGuard, Context, Ownable {
    ...
    IERC20  public _token;                   //继承标准类
    uint256 private _tokenDecimals;          //令牌小数位
    address payable public _wallet;          //项目方钱包
    uint256 public _rate;                    //定价
    uint256 public _weiRaised;               //ICO统计
    uint256 public endICO;                   //结束ICO
    uint    public minPurchase;              //最小
    uint    public maxPurchase;              //最大
    uint    public hardCap;                  //硬顶 
    uint    public softCap;                  //软顶
    uint    public availableTokensICO;       //预售数量
    bool    public startRefund = false;      //开关
    ...
    function buyTokens(address beneficiary) public nonReentrant icoActive payable {
        uint256 weiAmount  = msg.value;                       //获取BNB数量
        _preValidatePurchase(beneficiary, weiAmount);         //验证预售条件
        uint256 tokens     = _getTokenAmount(weiAmount);      //计算令牌数量
        _weiRaised = _weiRaised.add(weiAmount);               //累计ICO金额
        availableTokensICO = availableTokensICO - tokens;     //减少总IOC令牌数量
        _processPurchase(beneficiary, tokens);                //发送代币
        _contributions[beneficiary] = _contributions[beneficiary].add(weiAmount);   //购买日志
        emit TokensPurchased(_msgSender(), beneficiary, weiAmount, tokens);   //执行事件
    }
