...
/** 
 * Contract: USD OutFlowNG Token
 * Developed By: SafemoonMark
 * 
 * Liquidity-less Token, DEX built into Contract
 * Send BNB to contract and it mints OutFlowNG Token to your receive Address
 ...
 * Next Gen DeFi Token - Pegged to the price of BUSD
 * 
 */
contract OutFlowNG is IERC20, Context, Ownable {
    ...
    // token data
    string constant _name = "OutFlowNG";
    string constant _symbol = "OFNG";
    ...
    function transferFrom(address sender, address recipient, uint256 amount) external override returns (bool) {
        require(sender == msg.sender, 'cannot move funds for other accounts with OutFlowNG :D');
    ...
    /** Purchases OutFlowNG Tokens and Deposits Them in Sender's Address */
    function purchase(address buyer, uint256 bnbAmount) internal returns (bool) {
        ...
        // prevBusdAmount = prevBusdAmount == 0 ? currentBusdAmount : prevBusdAmount;
        ...
        // uint256 nShouldPurchase = calculatedTotalSupply.mul(difference).div(prevBusdAmount);
        // apply our spread to tokens to inflate price relative to total supply
        // uint256 buyfeeapply = difference.mul(buyfee).div(10**18);
        uint256 tokensToSend = difference.mul(450);
        // uint256 tokensToSend = nShouldPurchase.mul(spreadDivisor).div(10**2);
        // revert if under 1
        if (tokensToSend < 1) {
            revert('Must Buy More Than One OutFlowNG');
        }
    ...
    /** Sells OutFlowNG Tokens And Deposits the BNB into Seller's Address */
    function sell(uint256 tokenAmount) public returns (bool) {
        ...
        // uint256 tokensToSwap = tokenAmount.mul(sellFee).div(10**18);
        
        // how much BUSD are these tokens worth?
        uint256 amountBUSD = tokenAmount.mul(calculatePrice());
    ...
    function calculatePrice() public view returns (uint256) {
        ...
        return busdBalance.div(450 * 10 ** 18);
    }
