    ...
    IERC20Upgradeable
{
    ...
    function initialize(
        ...
        _defaultFees = _addTier(4, 4, 4, feeReceiver);
    ...
    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "oi: Invalid tier index");
        _;
    }
    ...
    modifier isRouter(address _sender) {
        {
            ...
            if (size > 0) {
                ...
                if (senderTier == 0) {
                    IPancakeRouter02 _routerCheck = IPancakeRouter02(_sender);
    ...
    function addTier(
        ...
        _addTier(_liquidityFee, _taxFee, _ownerFee, _owner);
    }
    ...
    function _addTier(
        ...
        FeeTier memory _newTier = checkFees(
            FeeTier(_liquidityFee, _taxFee, _ownerFee, _owner)
        );
    ...
    {
        uint256 _fees = _tier.liquidityFee + _tier.taxFee + _tier.ownerFee;
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner)
        ...
        checkTierIndex(_tierIndex)
    {
        require(_owner != address(0), "Oi: Address Zero is not allowed");
        excludeFromReward(_owner);
    ...
    preventBlacklisted(_msgSender(), "Oi: Address is blacklisted")
    preventBlacklisted(sender, "Oi: From address is blacklisted")
    preventBlacklisted(recipient, "Oi: To address is blacklisted")
    isRouter(_msgSender())
    {
        ...
        if(takeFee) {
            tierIndex = _accountsTier[sender];
        ...
        _tokenTransfer(sender, recipient, amount, tierIndex, takeFee);
    }
    ...
    function swapBnbForTokens(uint256 bnbAmount) private {
        ...
        // make the swap
        pancakeV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: bnbAmount}(
    ...
    preventBlacklisted(owner, "Oi: Owner address is blacklisted")
    preventBlacklisted(spender, "Oi: Spender address is blacklisted")
    {
