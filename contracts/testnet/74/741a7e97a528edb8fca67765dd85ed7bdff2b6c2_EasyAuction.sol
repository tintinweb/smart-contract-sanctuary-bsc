...
// File contracts/EasyAuction.sol
...
contract EasyAuction is Ownable {
    ...
    modifier atStageOrderPlacement(uint256 auctionId) {
        require(
            block.timestamp < auctionData[auctionId].aucData.auctionEndDate,
    ...
    modifier atStageOrderPlacementAndCancelation(uint256 auctionId) {
        require(
            block.timestamp < auctionData[auctionId].aucData.orderCancellationEndDate,
    ...
    modifier atStageSolutionSubmission(uint256 auctionId) {
        {
            uint256 auctionEndDate = auctionData[auctionId].aucData.auctionEndDate;
            require(
                auctionEndDate != 0 &&
    ...
    function initiateAuction(
        ...
        require(
            _data.minimumBiddingAmountPerOrder > 0,
            "minimumBiddingAmountPerOrder is not allowed to be zero"
        );
        require(
            _data.orderCancellationEndDate <= _data.auctionEndDate,
            "time periods are not configured correctly"
        );
        require(
            _data.auctionEndDate > block.timestamp,
    ...
    function _placeSellOrders(
        ...
        uint256 minimumBiddingAmountPerOrder =
            auctionData[auctionId].aucData.minimumBiddingAmountPerOrder;
        for (uint256 i = 0; i < _minBuyAmounts.length; i++) {
            require(
                _minBuyAmounts[i] > 0,
        ...
        returns (bytes32 clearingOrder)
    {
        ...
        auctionData[auctionId].aucData.minimumBiddingAmountPerOrder = uint256(0);
    }
    ...
    function getSecondsRemainingInBatch(uint256 auctionId)
        ...
        returns (uint256)
    {
        if (auctionData[auctionId].aucData.auctionEndDate < block.timestamp) {
            return 0;
        }
        return auctionData[auctionId].aucData.auctionEndDate.sub(block.timestamp);
    }
}