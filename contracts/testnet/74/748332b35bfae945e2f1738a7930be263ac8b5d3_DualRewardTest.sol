...
import "./Ownable.sol";
import "./TOKEN1DividendTracker.sol";
import "./TOKEN2DividendTracker.sol";
...
contract DualRewardTest is ERC20, Ownable, SafeToken {
    ...
    uint256 private maxSellTransactionAmount = 1000000 * (10**_decimals); // max sell 1% of supply
    uint256 private maxWalletAmount = 6000000 * (10**_decimals); // max wallet amount 6%
    uint256 private swapTokensAtAmount = 100000 * 10**_decimals; // swap at 100K tokens min
    uint256 public MaxSell = maxSellTransactionAmount / (10**_decimals); // max sell 1% of supply
    uint256 public MaxWallet = maxWalletAmount / (10**_decimals); // max wallet amount 6%
    uint256 public SwapTokens = swapTokensAtAmount / 10**_decimals; // swap at 100K tokens min
 ...
 //   event ExcludeMultipleAccountsFromFees(address[] accounts, bool isExcluded);
    ...
    constructor() ERC20("Dual Rewards Test", "DRT") {
     ...
     //   excludeFromDividend(DeadWallet);
        ...
        excludeFromFeeandDividend(marketingWallet);
        excludeFromFeeandDividend(address(this));
        excludeFromFeeandDividend(DeadWallet);
        excludeFromFeeandDividend(owner());
        excludeFromFeeandDividend(address(token1DividendTracker));
        excludeFromFeeandDividend(address(token2DividendTracker));
        excludeFromFeeandDividend(address(_uniswapV2Router));
     ...
     //   isMarketPair[address(_uniswapV2Pair)] = true;
        ...
        */
        _mint(owner(), 100000000 * (10**_decimals)); // 100 millions tokens
    }
   ...
   function setContractLimits(uint256 _maxWalletAmount, uint256 _maxSellTransactionAmount) public onlyOwner {
        ...
        maxSellTransactionAmount = _maxSellTransactionAmount* (10**_decimals);
    }

    function setSwapTokensAtAmount(uint256 _swapAmount) external onlyOwner {
  	    swapTokensAtAmount = _swapAmount* (10**_decimals);
  	}
...
//  	}
    ...
    function SetLiquidityandBurn(uint256 newLiquidityTax, uint256 newBurnTax) external onlyOwner() {
    ...
    function setSellTaxes(uint256 newMarketingTax, uint256 newReward1Tax, uint256 newReward2Tax) external onlyOwner() {
  	...
  	function updateMarketingWallet(address _newWallet) external onlyOwner {
        ...
        excludeFromFeeandDividend(_newWallet);
...
//    }
    ...
    function excludeFromFeeandDividend(address _address) public onlyOwner {
    ...
    function setExcludeFromMaxTx(address _address) external onlyOwner { 
...
//    }
 ...
 //   }
    ...
    function isExcludedFromFeesandDividend(address account) public view returns(bool) {
 ...
 //   function getIsExcludedFromFeesandDividends(address account) public view returns(bool) {
 //       return _isExcludedFromFees[account];
 //   }
 
//    function TOKEN1DividendStatusOf(address account) external view returns(uint256, uint256) {
//    	return token1DividendTracker.withdrawableDividendOf(account).balanceOf(account);
//  	}
 
//  	function withdrawableTOKEN2DividendOf(address account) external view returns(uint256, uint256) {
