...
contract BlindBox is Ownable {
    ...
    // Constants
    uint256 public constant MAX_SUPPLY_BLINDBOX = 10;
    uint256 public remaining_blindBox = 10;
    uint256 public blindBoxPrice = 0.02 ether;
    uint256[] public selledTokenId;

    // Random index assignment
    uint256 internal nonce = 0;
    uint256[MAX_SUPPLY_BLINDBOX] internal indices;
    ...
    function initStakingPool(address _nft) public onlyOwner {
        NftToken = NFTInteface(_nft);
    }

    function buyBlindBox() public payable returns (uint256) {

        require(_isSaleActive, "Sale must be active to sale blindBox");

        require(
            1 * blindBoxPrice <= msg.value, 
            "Not enough ether sent"
        );
        uint256 tokenId = randomIndex();
        
        _transferNFT(msg.sender, tokenId);
        selledTokenId.push(tokenId);
        return tokenId;
    }

    function _transferNFT( address _to, uint256 tokenId) internal {

        NftToken.transferFrom(address(this), _to, tokenId);
        remaining_blindBox -= 1;
        numTokens += 1;

    }

    function randomIndex() internal returns (uint256) {
        uint totalSize = MAX_SUPPLY_BLINDBOX - numTokens;
    ...
    function withdrawNft(address to, uint256 tokenId) public onlyOwner {
        ...
        remaining_blindBox = MAX_SUPPLY_BLINDBOX - 1;
    }
