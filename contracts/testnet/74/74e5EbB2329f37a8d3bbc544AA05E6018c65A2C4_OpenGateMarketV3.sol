...
contract RoyaltiesLib is Ownable{

    // 创作者和版税相关属性
    // 版税比例 uint256 / royaltiesBase
    uint256 public royaltiesBase = 10000;
    // 版税最高 10%
    uint256 public royaltiesTop = 1000;
    // 是否由所有者自由上架
    bool public openForOwner = false;
    // NFT创作者映射：nftAddress => (tokenId => creatorAddress)
    mapping(address => mapping(uint256 => address payable)) public _tokenCreators;
    // NFT版税映射：nftAddress => (tokenId => royalties)
    mapping(address => mapping(uint256 => uint256)) public _tokenRoyalties;

    // 创作者及版税相关
    // 创作者设置事件
    event SetCreator(address indexed nftAddress, uint256 indexed tokenId, address indexed creator, address owner);
    // 版税设置事件
    event SetRoyalties(address indexed nftAddress, uint256 indexed tokenId, uint256 indexed royalties, address owner);

    function creator(address _nftAddr, uint256 _tokenId) external view returns (address payable){
        return _tokenCreators[_nftAddr][_tokenId];
    }

    function royalties(address _nftAddr, uint256 _tokenId) external view returns (uint256){
        return _tokenRoyalties[_nftAddr][_tokenId];
    }

    // 设置创作者
    function _setCreator(address _nftAddr, uint256 _tokenId, address payable _creator) internal {
        _tokenCreators[_nftAddr][_tokenId] = _creator;
        emit SetCreator(_nftAddr, _tokenId, address(_creator), address(_msgSender()));
    }
    // 设置版税
    function _setRoyalties(address _nftAddr, uint256 _tokenId, uint256 _royalties) internal {
        _tokenRoyalties[_nftAddr][_tokenId] = _royalties;
        emit SetRoyalties(_nftAddr, _tokenId, _royalties, address(_msgSender()));
    }

    function setRoyaltiesTop(uint256 _royalties) public onlyOwner {
        royaltiesTop = _royalties;
    }

    function setOpenForOwner(bool _open) public onlyOwner {
        openForOwner = _open;
    }
...
interface IERC721Helper is IERC165 {
...
* 主合约
*/
contract OpenGateMarketV3 is ERC721Holder, Ownable, Pausable, RoyaltiesLib {
    using SafeMath for uint256;
    ...
    IERC721Helper public nftH;
    address payable public feeAddr;
    uint256 public feePercent;

    event Trade(address nftAddress, address indexed seller, address indexed buyer, uint256 indexed tokenId, uint256 price, uint256 fee, address coin);
    ...
    modifier checkOpen(address _nftAddress) {
        if (!openNFT) {
    ...
    function getBids(address _nftAddress, uint256 _tokenId) public view returns (BidEntry[] memory) {
        ...
        // 正序排序
        if (be.length > 0) {
            for (uint256 j = 0; j < be.length - 1; j++) {
                for (uint256 k = j + 1; k < be.length; k++) {
                    if (be[j].price > be[k].price) {
                        BidEntry memory temp = be[j];
                        be[j] = be[k];
                        be[k] = temp;
                    }
                }
    ...
    // 获取当前用户已上架报价NFT信息（TODO：报错了：Error: Returned error: invalid opcode: ode 0xfe not defined）
    ...
    function readyToSellToken(address _nftAddress, address _coinAddress, uint256 _tokenId, uint256 _price, uint256 _start, uint256 _end) public whenNotPaused {
        _readyToSellTokenTo(_nftAddress, _coinAddress, _tokenId, _price, _msgSender(), _start, _end, address(0), 0);
    }


    function readyToSellTokenV2(address _nftAddress, address _coinAddress, uint256 _tokenId, uint256 _price, uint256 _start, uint256 _end) public whenNotPaused {
        nftH = IERC721Helper(_nftAddress);
        address payable _creator = nftH.tokenCreator(_tokenId);
        uint256 _royalties = nftH.tokenRoyaltiesById(_tokenId);
        _readyToSellTokenTo(_nftAddress, _coinAddress, _tokenId, _price, _msgSender(), _start, _end, _creator, _royalties);
    }

    function readyToSellTokenByOwner(address _nftAddress, address _coinAddress, uint256 _tokenId, uint256 _price, uint256 _start, uint256 _end, address payable _creator, uint256 _royalties) public whenNotPaused {
        require(openForOwner, 'forbidden!');
        _readyToSellTokenTo(_nftAddress, _coinAddress, _tokenId, _price, _msgSender(), _start, _end, _creator, _royalties);
    }
    ...
    function _readyToSellTokenTo(
        ...
        uint256 _royalties
    ) private whenNotPaused checkOpen(_nftAddress) {
        ...
        _setCreator(_nftAddress, _tokenId, _creator);
        _setRoyalties(_nftAddress, _tokenId, _royalties);
    ...
    function _checkIsHigher(address _nftAddress, uint256 _tokenId, uint256 _price) private returns (bool){
        BidEntry[] memory bids = getBids(_nftAddress, _tokenId);
        if (bids.length == 0) {
            return true;
        }
        if (bids[bids.length - 1].price < _price) {
            return true;
        }
        return false;
    }
    ...
    function bidToken(address _nftAddress, uint256 _tokenId, uint256 _price) public payable whenNotPaused {
        ...
        require(_price != 0, 'Price must be granter than zero');
        require(_checkIsHigher(_nftAddress, _tokenId, _price), 'Price must be granter');
    ...
    function sellTokenTo(address _nftAddress, uint256 _tokenId, address _to) payable public whenNotPaused {
        ...
        uint256 feeAmount = price.mul(feePercent).div(royaltiesBase);
        // 版税
        uint256 _royaltiesAmount = 0;
        address payable _creator = _tokenCreators[_nftAddress][_tokenId];
        uint256 _royalties = _tokenRoyalties[_nftAddress][_tokenId];
        if (_royalties > 0) {
            _royaltiesAmount = price.mul(_royalties).div(royaltiesBase);
        }
        ...
        if (be.coin == address(0)) {
            ...
            if (_royaltiesAmount != 0) {
                _creator.transfer(_royaltiesAmount);
            }
            ae.owner.transfer(price.sub(feeAmount).sub(_royaltiesAmount));
        } else {
            ...
            if (_royaltiesAmount != 0) {
                quoteErc20.transfer(_creator, _royaltiesAmount);
            }
            quoteErc20.transfer(_seller, price.sub(feeAmount).sub(_royaltiesAmount));
        }
        ...
        emit Trade(_nftAddress, _seller, _to, _tokenId, price, feeAmount, be.coin);
    }
    ...
    // 转让收益账户 （仅当前收益账户可操作）
    function transferFeeAddress(address payable _feeAddr) public onlyOwner {
