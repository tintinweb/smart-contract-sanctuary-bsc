...
import "./interfaces/IGretAuthority.sol";
import "hardhat/console.sol";

/// @title Gret Bond Depository 
/// @author Zeus, Indigo
/// Review by: JeffX

contract GretBondDepository is IBondDepository, NoteKeeper {
/* ======== DEPENDENCIES ======== */
...
/* ======== CONSTRUCTOR ======== */
  ...
  constructor(
    IGretAuthority _authority,
    IERC20 _grt,
    IgGRT _ggrt,
    IStaking _staking,
    ITreasury _treasury
  ) NoteKeeper(_authority, _grt, _ggrt, _staking, _treasury) {
    // save gas for users by bulk approving stake() transactions
    _grt.approve(address(_staking), 1e45);
  }
...
/* ======== DEPOSIT ======== */
   ...
   * @return payout_     the amount of gGRT due
  ...
  function deposit(
     ...
     * price = quote tokens : grt (i.e. 42069 DAI : GRT)
     *
     * 1e18 = GRT decimals (9) + price decimals (9)
     */
    console.log("payout_");
    payout_ = (_amount * 1e18 / price) / (10 ** metadata[_id].quoteDecimals);
    console.log(payout_);
    console.log("market.maxPayout");
    console.log(market.maxPayout);
     ...
     *
     * this is either the number of GRT that the market can sell
     * (if capacity in quote is false), 
     *
     * or the number of quote tokens that the market can buy
     * (if capacity in quote is true)
     */
    console.log("market.capacity");
    ...
    // purchased, and how much GRT has been sold
    console.log("working");
  ...
  function _tune(uint256 _id, uint48 _time) internal {
    ...
    if (_time >= meta.lastTune + meta.tuneInterval) {
      ...
      // grt decimals (9) + price decimals (9)
       ...
       * is 10,000 GRT. max payout would be 1,000 GRT (10,000 * 1 / 10).
       */  
...
/* ======== CREATE ======== */
   ...
   * @param _market      [capacity (in GRT or quote), initial price / GRT (9 decimals), debt buffer (3 decimals)]
  ...
  function create(
     ...
     *
     * 1e18 = grt decimals (9) + initial price decimals (9)
     */
     ...
     * interval. for example, if capacity is 1,000 GRT, there are 10 days to conclusion, 
     * and the preferred deposit interval is 1 day, max payout would be 100 GRT.
     */
    ...
    emit CreateMarket(id_, address(grt), address(_quoteToken), _market[1]);
  }
...
/* ======== EXTERNAL VIEW ======== */
   ...
   * @param _id          ID of market
   * @return             price for market in GRT decimals
   *
  ...
  function marketPrice(uint256 _id) public view override returns (uint256) {
    console.log("*********************");
    console.log(currentControlVariable(_id));
    console.log(debtRatio(_id));
    console.log(10 ** metadata[_id].quoteDecimals);
    (uint64 decay,,) = _controlDecay(_id);
    console.log(decay);
    console.log(terms[_id].controlVariable);
    console.log(terms[_id].controlVariable - decay);
    console.log(10 ** metadata[_id].quoteDecimals);
   ...
   * @param _id          ID of market
   * @return             amount of GRT to be paid in GRT decimals
   *
   * @dev 1e18 = grt decimals (9) + market price decimals (9)
   */
   ...
   * @param _id          ID of market
   * @return             current debt for market in GRT decimals
   */
   ...
   * @param _id          ID of market
   * @return             control variable for market in GRT decimals
   */
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
  ...
  constructor (
    ...
    IStaking _staking,
    ITreasury _treasury
  ) FrontEndRewarder(_authority, _grt) {
    gGRT = _ggrt;
    staking = _staking;
    treasury = _treasury;
  }
...
/* ========== ADD ========== */
   ...
   * @param _payout      the amount of GRT due to the user
  ...
  function addNote(
    ...
    console.log(_payout + rewards);
    treasury.mint(address(this), _payout + rewards);

    console.log("mint");
    // note that only the payout gets staked (front end rewards are in GRT)
...
/* ========== REDEEM ========== */
   ...
   * @param _sendgGRT    send payout as gGRT or sGRT
   * @return payout_     sum of payout sent, in gGRT
   */
  function redeem(address _user, uint256[] memory _indexes, bool _sendgGRT) public override returns (uint256 payout_) {
    ...
    if (_sendgGRT) {
      gGRT.transfer(_user, payout_); // send payout as gGRT
    } else {
      staking.unwrap(_user, payout_); // unwrap and send payout as sGRT
    }
  }
   ...
   * @param _sendgGRT    send payout as gGRT or sGRT
   * @return             sum of payout sent, in gGRT
   */ 
  function redeemAll(address _user, bool _sendgGRT) external override returns (uint256) {
    return redeem(_user, indexesFor(_user), _sendgGRT);
  }
...
/* ========== VIEW ========== */
   ...
   * @return payout_     the payout due, in gGRT
  ...
  function pendingFor(address _user, uint256 _index) public view override returns (uint256 payout_, bool matured_) {
    ...
    console.log("note.payout");
    payout_ = note.payout;
    console.log(payout_);

    console.log("note.redeemed");
    console.log(note.redeemed == 0);
     
    console.log("note.redeemed");
    console.log(note.redeemed == 0);

    console.log("note.matured");
    console.log(note.matured);
...
interface IBondDepository {
  ...
  struct Market {
    ...
    bool capacityInQuote; // capacity limit is in payment token (true) or in GRT (false, default)
  ...
  struct Terms {
    ...
    uint64 maxDebt; // 9 decimal debt maximum in GRT
  }
...
import "../types/GretAccessControlled.sol";
import "../interfaces/IERC20.sol";

abstract contract FrontEndRewarder is GretAccessControlled {
  ...
  IERC20 internal immutable grt; // reward token

  constructor(
    IGretAuthority _authority, 
    IERC20 _grt
  ) GretAccessControlled(_authority) {
    grt = _grt;
  }
  ...
  function getReward() external {
    ...
    grt.transfer(msg.sender, reward);
  }
...
interface INoteKeeper {
  ...
  struct Note {
    uint256 payout; // gGRT remaining to be paid
  ...
  function redeem(address _user, uint256[] memory _indexes, bool _sendgGRT) external returns (uint256);
  function redeemAll(address _user, bool _sendgGRT) external returns (uint256);
