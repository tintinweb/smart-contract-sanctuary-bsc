...
interface IbankContribution{
  function depositBank(address payable referrer,uint256 _amount)external payable;
  function migrateBonus(address payable referrer) external payable;
...
interface IBankMigration {
     ...
     * @dev Record referral.
     */
    function userInfo(uint number, address user) external view returns(uint256,uint256,uint256,uint256);
    function pendingReward(address user,uint number)  external view returns (uint256);
...
abstract contract DCVCToken1{
 ...
 contract metabank is ReentrancyGuard,IbankContribution,Pausable{
  ...
  function settotalPoint(uint _amount) public onlyOperator{
    totalPoint=_amount;
  }

  function setpayout(uint _amount) public onlyOperator{
    busdtotalpayout=_amount;
  }
  ...
  function readBonus(address _user) public view returns(bool){
    if(userInfo[2][_user].bankmigration==1){
      return false;
    }
    (uint256 amount,uint point,uint rewardDebt,uint lastwithdraw)=bankMigration.userInfo(2,_user);
    if(amount>0){
      return true;
    }else{
      return false;
   ...
   */
  function migrateBonus(address payable referrer)public override payable {
    require(msg.value>=fees,'Fees is zero!');
    require(userInfo[2][msg.sender].bankmigration==0,'already migrate');
    bool status=addReferrer(referrer);
    require(status==true,'invalid referer');
    payable(feeAddr).transfer(msg.value);
    userInfo[2][msg.sender].bankmigration=1;
    (uint256 amount,uint point,uint rewardDebt,uint lastwithdraw)=bankMigration.userInfo(2,msg.sender);
    (uint256 reward)=bankMigration.pendingReward(msg.sender,2).div(1e18);
    updatePoolBUSD();
    if(reward>0){
    ...
    function depositBank(address payable referrer,uint256 _amount) public override payable {
    require(msg.value>=fees,'Fees is zero!');
    require(_amount>=minstake);
    payable(feeAddr).transfer(msg.value);
    bool status=addReferrer(referrer);
    require(status==true,'invalid referer');
    UserInfo storage user = userInfo[2][msg.sender];
     updatePoolBUSD();
    if (user.point > 0) {
                  // uint256 pending = user.point.mul(accBnbPerShare).div(1e12).sub(user.rewardDebt);
      uint256 totalpoint=user.point.add(user.referralpoint);
      uint256 pending = totalpoint.mul(accBnbPerShare).div(1e12).sub(user.rewardDebt);
      if(pending > 0) {
    ...
    if(depositFeeBP > 0){
        ...
        DCVCToken.safeTransfer(management , depositFee);
    ...
    if(user.referralpoint>0){

    user.rewardDebt = user.point.add(user.referralpoint).mul(accBnbPerShare).div(1e12);
    }else{
          user.rewardDebt = user.point.mul(accBnbPerShare).div(1e12);
 ...
 updatePoolBUSD();
  ...
  function setAccbnbpershare(uint _amount,uint _bnbtime) public onlyOperator{
    accBnbPerShare=_amount;
       bnbPerTime=_bnbtime;
  }

   function setendtimelastreward(uint _endtime,uint _lastreward) public onlyOperator{
    endtime=_endtime;
    lastRewardTime=_lastreward;
  }
  ...
  function setminstake(uint _decimals) public onlyOperator
  {
    require(_decimals>0);
    minstake=_decimals;
  }
  ...
  function settechnical(address payable _technical) public onlyOperator{
  ...
  function changeuserInfo(uint poolid,address payable user, uint256 _point,uint256 _rewardDebt, uint256 _lastwithdraw, uint256 _lastDeposit,uint256 _bankmigration) public onlyOperator{
    UserInfo storage newuser= userInfo[poolid][user];
    newuser.point=_point;
    newuser.rewardDebt=_rewardDebt;
    newuser.lastwithdraw=_lastwithdraw;
    newuser.lastDeposit=_lastDeposit;
    newuser.bankmigration=_bankmigration;
  }
