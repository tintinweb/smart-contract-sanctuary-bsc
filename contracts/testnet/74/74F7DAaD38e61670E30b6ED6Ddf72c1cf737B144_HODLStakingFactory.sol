...
contract HODLStaking is Context, Ownable {
    ...
    function APYCalculator() public view returns(uint256){
        uint256 _apyValue = rewardAmount * 100 / (stakeAmount + feeAmount);
        return _apyValue;
    }
    ...
    function withdraw() external {
        ...
        if(msg.sender == owner()) {
            ...
            uint256 _rewardFeeAmount = rewardAmount * feeAmount / (stakeAmount + feeAmount) * _timeDiff / stakingPeriod;
            ...
            if(_timeDiff > stakingPeriod)
                _timeDiff = stakingPeriod;
            uint256 _rewardAmount = rewardAmount * _stake.stakeAmount / (stakeAmount + feeAmount) * _timeDiff / stakingPeriod;
