...
//    address public busdToken = 0xe9e7cea3dedca5984780bafc599bd69add087d56; //mainnet
    address public busdToken = 0x78867BbEeF44f2326bF8DDd1941a4439382EF2A7; //testnet

    address public metaGoldAddress; //testnet
    ...
    function processSell(address from, address to, uint256 amount) external onlyMetagold returns (bool allowed) {
        if( to == metaGold.tokenPair()) {
            //selling tokens to PancakeSWAP
            if( from == metaGoldAddress){
        ...
        (uint256 myLimit,) = _getMaxTokenAllowedToSell(from);
//        uint256 myLimit = 0;
        emit AllowCheck(from,to,amount, myLimit);
        return true;
    }
    ...
    function getMaxTokenAllowedToSell(address whoWantsToSell) external view returns(uint256 maxTokens, uint256 maxTokensFilter)  {
        return _getMaxTokenAllowedToSell(whoWantsToSell);
    }
