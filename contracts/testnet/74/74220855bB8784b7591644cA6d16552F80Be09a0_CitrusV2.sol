...
contract CitrusV2 is Initializable, Proposal {
    ...
    function init(address[] memory _owners, uint[] memory _sharePercentage) external initializer {
        symbol = "CTS 2.0";
        name = "Citrus V2";
        decimals = 18;
        ...
        for(uint i; i < _owners.length; i++){
            ...
            require(owner != address(0), "PROMPT 2009: Invalid address. Please enter a valid address!");
            require(!isOwner[owner], "PROMPT 2010: Owner is not unique. Please enter a unique owner!");
            isOwner[owner] = true;
            balances[owner] = (totalSupply * _sharePercentage[i])/100 ;
            lockTime[owner] = block.timestamp + 31536000; // 1 year
            noOfOwners++;
        }
    }
...
contract Proposal is Owned, BEP20 {
    ...
    uint public numProposals;  // till now number of Proposal is made
    mapping(uint => Proposals) public proposals; // for checking Poposal to all Users 
    mapping(uint => mapping(address => bool)) public proposalVotes; // for Checking Who is voted Agree or not Agree
    bool public isSuccess; 
    mapping (uint => mapping(address => bool)) history;
        ...
        onlyOwners 
    {
        ...
        newProposal.lockTime = LOCK_TIME;
        ...
        onlyOwners
    {
        ...
        require(thisProposal.voters[msg.sender] == false, "PROMPT 2011: Already voted for this request!");
    ...
    {
        Proposals storage thisProposal = proposals[_index];
        require(thisProposal.isCompleted == false, "PROMPT 2012: Request already completed!");
...
contract Owned {
    ...
    uint public noOfOwners; // It'll return No of Owners
    ...
    modifier onlyOwners() {
        require(isOwner[msg.sender] == true, "PROMPT 2015: Access denied! Only owners can perform this activity!");
        _;
    }
    ...
    modifier onlyUsers(){
        require(isOwner[msg.sender] == false, "PROMPT 2016: Access denied! Only users can perform this activity!");
        _;
    }
    ...
    function addOwner(address _addNewOwner) internal {
        require(isOwner[_addNewOwner] == false, "PROMPT 2017: Owner already added!");
    ...
    function removeOwner(address _removeOwner) internal {
        require(isOwner[_removeOwner] == true, "PROMPT 2018: Owner already removed!");
...
contract BEP20{
    ...
    modifier onlyAfterTimeLimit(address _owner) {
        require(
            ...
            Iswap(0x929336a17aF293b16d025170e310d7C408C5447e).lockTime(msg.sender) < block.timestamp), 
    ...
    function transferFrom(
        ...
        returns (bool success)
    {
        require(balances[_from] >= _amount, "PROMPT 2003: Insufficient balance! Please add balance to your wallet and try again!");
        require(allowed[_from][msg.sender] >= _amount, "PROMPT 2004: Please enter lesser or same amount than the allowed value!"); 
        require(_amount > 0,"PROMPT 2005: Please enter value greater than 0!");
        require(balances[_to] + _amount > balances[_to], "PROMPT 2006: Insufficient balance! Please add balance to your wallet and try again!");
    ...
    function _burn(address account, uint256 amount) 
        ...
        virtual 
    {
        require(account != address(0), "PROMPT 2007: It should not be the zero address. Please enter valid burn address!");
        uint256 accountBalance = balances[account];
        require(accountBalance >= amount, "PROMPT 2008: Number of tokens to burn should be lesser or same as available balance. Please check the value entered!");
