...
contract GovSaleRemote is
    ...
    function getAdapterParams() internal view returns(bytes memory adapterParams){
        uint16 version = 1;
        adapterParams = abi.encodePacked(version, uint256(gasForDestinationLzReceive));
    }
    ...
    function receiveCall(bytes calldata _payload) external {
        ...
        remoteRaisedC = remoteRaised;
        remoteRevenueC = remoteRevenue;
        remoteSoldC = remoteSold;
        ...
        for(uint16 i = 0; i < remoteUsers.length; ++i){
            ...
            if(summary.bought > remoteUsersBought[i]){
                ...
                emit ReceiveCall(
                    ...
                    remoteRevenue,
                    remoteSold,
                    summaries[buyer].received,
                    summaries[buyer].bought,
                    summaries[buyer].feeGiven
                );
            }
        }
    }
    ...
    event ReceiveCall(
        ...
        uint128 remoteRevenue,
        uint128 remoteSold,
        uint256 remoteUsersReceived,
        uint128 remoteUsersBought,
        uint128 remoteUsersFee
    );
