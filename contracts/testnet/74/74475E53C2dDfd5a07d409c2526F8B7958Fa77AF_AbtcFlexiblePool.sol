...
interface IAbtcPool {
    struct UserInfo {
        ...
        uint256 abtcAtLastUserAction; // keep track of abtc deposited at the last user action.
...
interface IReferral {
    function notifyReward(address _user, uint256 _amount) external;
    function setReferrer(address _investor, address _referrer) external;
    function getReferrer(address _investor) external view returns (address);
    function rFee() external view returns (uint256);
}

interface IABTCSafe {
    function safeABTCTransfer(address _to, uint256 _amount) external;
}

contract AbtcFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 abtcAtLastUserAction; // keeps track of abtc deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    address public referral;
    /// @notice Address of ABTCSafe contract.
    IABTCSafe public immutable abtcSafe;
    IERC20 public immutable token; // Abtc token
    IAbtcPool public immutable abtcPool; //Abtc pool
    ...
    //When call abtcpool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositAbtc(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
    ...
    constructor(
        ...
        referral = _referral;
        token = _token;
        abtcSafe = IABTCSafe(_abtcSafe);
        abtcPool = _abtcPool;

        // Infinite approve
        IERC20(_token).safeApprove(address(_abtcPool), type(uint256).max);
    ...
    /**
     * @notice Deposits funds into the Abtc Flexible Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in ABTC)
     */
    function deposit(uint256 _amount, address _referrer) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            uint256 earnAmount = totalAmount - user.abtcAtLastUserAction;
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    abtcPool.withdrawByAmount(withdrawAmount);
        ...
        user.abtcAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        user.lastUserActionTime = block.timestamp;

        address referrer = IReferral(referral).getReferrer(msg.sender);
        if (referrer == address(0)) {
            referrer = _referrer;
            IReferral(referral).setReferrer(msg.sender, _referrer);
        }

        emit DepositAbtc(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the Abtc Flexible Pool
    ...
    function withdraw(uint256 _shares) public {
        ...
        address referrer = IReferral(referral).getReferrer(msg.sender);
        if (referrer != address(0)) {
            uint256 rFee = IReferral(referral).rFee();
            uint256 refAmount = currentAmount * rFee / 100;
            abtcSafe.safeABTCTransfer(referral, refAmount);
            IReferral(referral).notifyReward(referrer, refAmount);
        }

        if (user.shares > 0) {
            user.abtcAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.abtcAtLastUserAction = 0;
        }
    ...
    /**
     * @notice Withdraws from Abtc Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking abtc");
        staking = false;
        abtcPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Abtc Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in AbtcPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = abtcPool.userInfo(address(this));
        uint256 pricePerFullShare = abtcPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into AbtcPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            abtcPool.deposit(bal, 0, address(0));
        }
    }
}