...
contract ExtendableBondedCake is ExtendableBond {
    ...
    function secondsToPancakeLockExtend() public view returns (uint256 secondsToExtend) {
        ...
        if (
            ...
            cakeInfo.lockEndTime - cakeInfo.lockStartTime < cakePool.MAX_LOCK_DURATION()
        ) {
            secondsToExtend = MathUpgradeable.min(
                checkPoints.maturity -
                    (cakeInfo.lockEndTime > block.timestamp ? cakeInfo.lockEndTime : block.timestamp),
                cakePool.MAX_LOCK_DURATION()
            );
        }
    }
