...
contract Token_Pintar is Context, IBEP20, Ownable {
    ...
    string public constant name     = "Token Pintar";
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        ...
        uint256 tFee       = tAmount.mul(_taxFeeSell).div(0);
        uint256 tLiquidity = tAmount.mul(_liquidityFeeSell).div(10000);
        uint256 tMarketing = tAmount.mul(_marketingFeeSell).div(10000);
        uint256 tStaking    = tAmount.mul(_stakingFeeSell).div(20000);
