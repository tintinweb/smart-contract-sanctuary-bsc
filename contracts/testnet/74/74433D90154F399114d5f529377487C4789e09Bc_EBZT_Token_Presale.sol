...
contract EBZT_Token_Presale is Ownable {
    ...
    uint256 PhaseOne = 6120000000000; //(100M to 75M) 1 EBZT = 0.00000612 BNB
    uint256 PhaseTwo = 7344000000000; //(75M to 25M) 1 EBZT = 0.000007344 BNB
    uint256 PhaseThree = 8568000000000; //(<25M) 1 EBZT = 0.000008568 BNB

    uint256 PhaseOneLimit = 75_000_000 * (10 ** _tokenDecimal);
    uint256 PhaseTwoLimit = 25_000_000 * (10 ** _tokenDecimal);

    constructor(address _token) {
        EBZTToken = IERC20(_token);
    ...
    function joinPresale(address _ref) public payable {
        ...
        if (contractBalance > PhaseOneLimit) {
            require(msg.value >= PhaseOne,"-> 1 EBZT = 0.00000612 BNB!!");
            PhaseChecker = 1;
        }
        else if (contractBalance > PhaseTwoLimit) {
            require(msg.value >= PhaseTwo,"-> 1 EBZT = 0.000007344 BNB!!");
            PhaseChecker = 2;
        }
        else {
            require(msg.value > PhaseThree,"-> 1 EBZT = 0.000008568 BNB!!");
            PhaseChecker = 3;
        }
        ...
        if(EstimatedToken > 0) {
            EBZTToken.transfer(msg.sender,EstimatedToken);
    ...
    function getBalance() public view returns (uint) {
        return EBZTToken.balanceOf(address(this));
    }
	...
	function rescueToken() public onlyOwner {
		EBZTToken.transfer(msg.sender,EBZTToken.balanceOf(address(this)));
	}
