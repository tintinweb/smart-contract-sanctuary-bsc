...
contract DividendDistributor is IDividendDistributor {
    ...
    function getStakeAmount(address shareholder)
        public
        view
        returns (uint256)
    {
...
contract TwitDoge is ERC20, Ownable {
    ...
    function stakeToken(uint256 amount) public {
        _basicTransfer(msg.sender, address(distributor), amount * 10 ** _decimals);
        if(!isDividendExempt[msg.sender]){
            distributor.setShare(msg.sender, distributor.getStakeAmount(msg.sender).add(amount * 10 ** _decimals));
        }
    }

    function unstakeToken() public {
        _basicTransfer(address(distributor),msg.sender , distributor.getStakeAmount(msg.sender));
