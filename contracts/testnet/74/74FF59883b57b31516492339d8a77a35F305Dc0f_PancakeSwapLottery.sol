...
contract PancakeSwapLottery is Initializable, ReentrancyGuardUpgradeable, IPancakeSwapLottery, OwnableUpgradeable {
    ...
    uint256 public pendingUsdtInjectionNextLottery; // in usdt
    ...
    event LotteryOpen(
        ...
        uint256 injectedUsdtAmount // in usdt
    );
    ...
    function claimTickets(
        ...
        require(
            maxNumberTicketsPerBuyOrClaim != 0 && _ticketIds.length <= maxNumberTicketsPerBuyOrClaim,
            "Too many tickets"
        );
        require(
        ...
        // Initializes the rewardInCakeToTransfer, rewardInUsdtToTransfer
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            uint256 usdtRewardForTicketId = _calculateUsdtRewardsForTicketId(
                _lotteryId,
                thisTicketId,
                _brackets[i]
            );
            ...
            require(rewardForTicketId != 0, "No prize in cake for this bracket");
            ...
            if (_brackets[i] != 4) {
                require(
                    ...
                    "Bracket in cake must be higher"
                );
                if(usdtRewardForTicketId != 0) {
                    require(
                        _calculateUsdtRewardsForTicketId(
                            _lotteryId,
                            thisTicketId,
                            _brackets[i] + 1
                        ) == 0,
                        "Bracket in usdt must be higher"
                    );
                }
            ...
            rewardInUsdtToTransfer += usdtRewardForTicketId;
        }

        // Transfer money to msg.sender
        cakeToken.safeTransfer(msg.sender, rewardInCakeToTransfer);
        if(rewardInUsdtToTransfer > 0){
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
            ...
            if (_lotteries[_lotteryId].countWinnersPerBracket[j] != 0) {
                ...
                amountToWithdrawToTreasury +=
                    (_lotteries[_lotteryId].rewardsBreakdown[j] *
                        cakeAmountToShareToWinners) /
                    10000;
                pendingUsdtInjectionNextLottery += (_lotteries[_lotteryId].usdtRewardsBreakdown[j] *
                        usdtAmountToShareToWinners) /
                    3000;
            }
        }
        ...
        emit LotteryNumberDrawn(
            currentLotteryId,
            finalNumber,
            _winnersPerLotteryId[_lotteryId][0].length
        );
    }
    ...
    function startLottery(
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            amountCollectedInUsdt: pendingUsdtInjectionNextLottery,
            finalNumber: 0
        });
        ...
        emit LotteryOpen(
            ...
            pendingUsdtInjectionNextLottery
        );

        pendingInjectionNextLottery = 0;
        pendingUsdtInjectionNextLottery = 0;
    }
    ...
    // /**
    //  * @notice View winner per bracket for a lotery id
    //  * @param _lotteryId: lottery id
    //  */
    // function viewWinnersForLotteryId(
    //     uint256 _lotteryId
    // ) external view returns (Ticket[][5] memory) {
    //     Ticket[][5] memory result;
    //     for (uint32 i = 0; i < 5; i++) {
    //         uint256 countWinners = _winnersPerLotteryId[_lotteryId][i].length;
    //         uint256[] memory winnersInThisBracket = new uint256[](countWinners);
    //         winnersInThisBracket = _winnersPerLotteryId[_lotteryId][i];

    //         Ticket[] memory winnerTicketsInThisBracket = new Ticket[](countWinners);
    //         for (uint256 j = 0; j < countWinners; j++) {
    //             uint256 ticketId = winnersInThisBracket[j];
    //             winnerTicketsInThisBracket[j] = _tickets[ticketId];
    //         }
            
    //         result[i] = winnerTicketsInThisBracket;
    //     }
