...
contract LaborToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000000000 * 10**18; //1Billion totaltoken supply
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "LaborToken";
    string private _symbol = "LABTKN";
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, (amount.sub(devAmt).sub(charityAmt)));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, (amount.sub(devAmt).sub(charityAmt)));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, (amount.sub(devAmt).sub(charityAmt)));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, (amount.sub(devAmt).sub(charityAmt)));
        } else {
            _transferStandard(sender, recipient, (amount.sub(devAmt).sub(charityAmt)));
        }
