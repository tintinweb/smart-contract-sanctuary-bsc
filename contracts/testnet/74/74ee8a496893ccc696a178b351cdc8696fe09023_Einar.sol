...
error MaxBalanceExceeded(uint256 _maxWalletLimit);
error MaxTxLimitExceeded(uint256 _maxTxLimit);
...
contract Einar is ERC20, Ownable {
    ...
    uint256 public maxTxAmountLimit;
    bool public maxTxAmountLimitEnabled = true;
    ...
    constructor() ERC20("Einar", "Einar", 18) {
        
        // default launch buy taxes
        operationsFeeBuy = 1;
        liquidityFeeBuy = 2;
        totalFeesBuy = operationsFeeBuy + liquidityFeeBuy;

        // default launch sell taxes
        operationsFeeSell = 1;
        liquidityFeeSell = 2;
        totalFeesSell = operationsFeeSell + liquidityFeeSell;

    	liquidityWallet = 0xB8EF3054f4639f766458f8bEbb5ee9736e1D69A0;
    	operationsWallet = 0x5d410E4857e827c2bB8E6Fa59a7257a6905EeD67;
        ...
        maxWalletLimit = (totalSupply * 3) / 100; //max wallet holding of 3% 
        maxTxAmountLimit = (totalSupply * 3) / 100; //max tx limit of 3% 
        ...
        setLimitExemptionStatus(owner(), true);
        setLimitExemptionStatus(address(this), true);
        setLimitExemptionStatus(address(0xdead), true);
    ...
    // change the max bag limit percentage
    function updateMaxWalletPercentage(uint maxPercentage) external onlyOwner {
  	    require(maxPercentage > 1, "MaxPercentage cannot be lower than 1");
        maxWalletLimit = (totalSupply() * maxPercentage) / 100; //max wallet holding of 3% 
  	}

    // toggle max bag limit status
    function setMaxWalletEnabled(bool status) external onlyOwner {
        maxWalletLimitEnabled = status; 
  	}
      
    // change the max tx limit
    function updateMaxTxAmountPercentage(uint maxPercentage) external onlyOwner {
  	    require(maxPercentage > 1, "MaxPercentage cannot be lower than 1");
        maxTxAmountLimit = (totalSupply() * maxPercentage) / 100; //max tx amount of 3% 
  	}

    // toggle max tx limit
    function setMaxTxAmountEnabled(bool status) external onlyOwner {
        maxTxAmountLimitEnabled = status; 
  	}

    // set whether wallet is exempt from limits
    function setLimitExemptionStatus(address account, bool exempt) public onlyOwner
    {
    ...
    function updateUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "CONTRACTNAME: The router already has that address");
        require(isContract(newAddress), "CONTRACTNAME: updateUniswapV2Router:: Provided address is a wallet, not a contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "CONTRACTNAME: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    
    function airdropToWallets(address[] memory airdropWallets, uint256[] memory amount) external onlyOwner() {
        require(airdropWallets.length == amount.length, "CONTRACTNAME: airdropToWallets:: Arrays must be the same length");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != address(0), "CONTRACTNAME: Cannot set to address 0");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != address(0), "CONTRACTNAME: Cannot set to address 0");
    ...
    function updateFees(uint256 _operationsFee, uint256 _liquidityFee) external onlyOwner {
        ...
        totalFeesBuy = operationsFeeBuy + liquidityFeeBuy;
        require(totalFeesBuy <= 10, "Must keep fees at 10% or less");
    }
    ...
    function updateFeesSell(uint256 _operationsFee, uint256 _liquidityFee) external onlyOwner {
        ...
        totalFeesSell = operationsFeeSell + liquidityFeeSell;
        require(totalFeesSell <= 10, "Must keep fees at 10% or less");
    }
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "CONTRACTNAME: Automated market maker pair is already set to that value");
        ...
        if(totalFeesBuy > 0){
            ...
            if(takeFee) {
                ...
                if(automatedMarketMakerPairs[from] && totalFeesBuy > 0){
                    ...
                    uint256 newAmount = ( balanceOf(address(from)) + amount.sub(fees));

                    // check if wallet exceeds max limit
                    if(maxWalletLimitEnabled){
                        if (newAmount > maxWalletLimit && !limitExempt[from]) 
                        {
                            revert MaxBalanceExceeded(maxWalletLimit);
                        }
                    }
                    
                    // check if tx exceeds max limit
                    if(maxTxAmountLimitEnabled){
                        if (newAmount > maxTxAmountLimit && !limitExempt[from]) 
                        {
                            revert MaxTxLimitExceeded(maxTxAmountLimit);
                        }
                    }
                }
                    ...
                    // check if wallet getting tranferred amount exceeds max limit
                    if(maxWalletLimitEnabled){
                        if (( balanceOf(address(to)) + amount.sub(fees)) > maxWalletLimit && !limitExempt[to]) 
                        {
                            revert MaxBalanceExceeded(maxWalletLimit);
                        }
                    }
    ...
    function swapBack() private {
        ...
        address(operationsWallet).call{value: ethForOperations};
    }
