...
contract Nanottery {
    ...
    address public nanoMinionsContract;
    address public owner;
    uint[] numberResults;
    address[] playerList;
    address[] winnerPlayerList;
    mapping(uint => bool) checkRepeatNumber;
    mapping(address => uint) public buyTicketCount;
    ...
    modifier onlyNanoMinions {
        require(msg.sender == nanoMinionsContract, "not nanoBot");
        _;
    }
    ...
    // Participate
    function participate(address _player, uint _ticketCount) public onlyNanoMinions {
        require(playerList.length + _ticketCount <= participateCountLimit, "over participate limit");
        require(block.timestamp <= participateEndTime, "not start lottery time");
        for(uint i = 0; i < _ticketCount; i++) {
            playerList.push(_player);
            buyTicketCount[_player]++;
        }
    }
    ...
    function setNanoMinionsContract(address _contractAddress) external onlyOwner {
        nanoMinionsContract = _contractAddress;
    }
    ...
    function removeLastList() private {
        ...
        for (uint i = 0; i < playerList.length; i++) {
            delete buyTicketCount[playerList[i]];
        }
