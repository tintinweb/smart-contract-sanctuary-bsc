...
contract Marketplace is Initializable, OwnableUpgradeable, IERC721ReceiverUpgradeable {
  ...
  function setFirstPairAddress(address _address) external onlyOwner {
    firstPairAddress = _address;
  }

  function getFirstPairAddress() external view returns(address) {
    return firstPairAddress;
  }

  function setSecondPairAddress(address _address) external onlyOwner {
    secondPairAddress = _address;
  }

  function getSecondPairAddress() external view returns(address) {
    return secondPairAddress;
  }
  ...
  function setUniswapV2Factory(address _address) external onlyOwner {
    uniswapV2Factory = IUniswapV2Factory(_address);
  }
  ...
  function getPairAddress() external view returns(address) {
    return uniswapV2Factory.getPair(this.getFirstPairAddress(), this.getSecondPairAddress());
  }

  function setUniswapV2Pair() external onlyOwner {
    address pairAddress = this.getPairAddress();

    if (pairAddress == address(0)) {
      pairAddress = uniswapV2Factory.createPair(this.getFirstPairAddress(), this.getSecondPairAddress());
    }
  ...
  */
  ...
  function ownerOf(uint _nftId) internal returns(address) {
  ...
  function purchaseWithBNB(uint _nftId, string memory _callbackId) external payable purchaseVerifications(_nftId, msg.sender, _callbackId) {
    ...
    path[1] = address(token);
    uniswapV2Router.swapExactETHForTokens(sellPrice, path, this.getTeamWallet(), block.timestamp);
  ...
  function buy(uint _nftId, string memory _callbackId) external purchaseVerifications(_nftId, msg.sender, _callbackId) {
    address buyer = msg.sender;
    address payable seller = payable(address(this));
    uint sellPrice = _nftDetails[_nftId].price;
  ...
  function afterNFTTransfer(uint _nftId, string memory _callbackId, bool _isSuccess) internal {
    if (_isSuccess) {
  ...
  modifier purchaseVerifications(uint _nftId, address buyer, string memory _callbackId) {
    address payable seller = payable(address(this));
    uint sellPrice = _nftDetails[_nftId].price;
