...
contract RUNSWAP is BEP20 {
    // Transfer tax rate in basis points. (default 5%)
    uint16 public transferTaxRate = 0; // 500
    ...
    // Min amount to liquify. (default 500 RUNSWAPs)
    uint256 public minAmountToLiquify = 500 ether;
    // The swap router, modifiable. Will be changed to router when our own AMM release
    IUniswapV2Router02 public RUNSWAPRouter;
    // The trading pair
    address public RUNSWAPPair;
    // In swap and liquify
    ...
    event RUNSWAPRouterUpdated(address indexed operator, address indexed router, address indexed pair);
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "RUNSWAP::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    constructor() public BEP20("RUNSWAP", "RUNSWAP") {
        ...
        mint(address(this), 100000000000000000000000000000);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) {
        ...
        if (
            ...
            && _inSwapAndLiquify == false
            && address(RUNSWAPRouter) != address(0)
            && RUNSWAPPair != address(0)
            && sender != RUNSWAPPair
            && sender != owner()
        ) {
            swapAndLiquify();
        ...
        if (recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            require(taxAmount == burnAmount + liquidityAmount, "RUNSWAP::transfer: Burn value invalid");

            // default 95% of transfer sent to recipient
            uint256 sendAmount = amount.sub(taxAmount);
            require(amount == sendAmount + taxAmount, "RUNSWAP::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        path[0] = address(this);
        path[1] = RUNSWAPRouter.WETH();

        _approve(address(this), address(RUNSWAPRouter), tokenAmount);

        // make the swap
        RUNSWAPRouter.swapExactTokensForETHSupportingFeeOnTransferTokens(
            tokenAmount,
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        // add the liquidity
        RUNSWAPRouter.addLiquidityETH{value : ethAmount}(
            address(this),
            tokenAmount,
    ...
    function updateBurnRate(uint16 _burnRate) public onlyOperator {
        require(_burnRate <= 100, "RUNSWAP::updateBurnRate: Burn rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "RUNSWAP::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
     ...
     * Can only be called by the current operator.
     */
    function updateRUNSWAPRouter(address _router) public onlyOperator {
        RUNSWAPRouter = IUniswapV2Router02(_router);
        RUNSWAPPair = IUniswapV2Factory(RUNSWAPRouter.factory()).getPair(address(this), RUNSWAPRouter.WETH());
        require(RUNSWAPPair != address(0), "RUNSWAP::updateRUNSWAPRouter: Invalid pair address.");
        emit RUNSWAPRouterUpdated(msg.sender, address(RUNSWAPRouter), RUNSWAPPair);
    }
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "RUNSWAP::transferOperator: new operator is the zero address");
    ...
    */
    ...
    internal
    {
        ...
        // balance of underlying  (not scaled);
