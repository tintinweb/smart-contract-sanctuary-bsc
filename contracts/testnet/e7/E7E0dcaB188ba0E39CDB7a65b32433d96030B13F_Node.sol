...
contract Node is Comn{
    ...
    function endBefore(uint nodeId,uint timeDistance) private {
        ...
        testEndBefore(nodeId,lastBlockOutPut,totalEffective,isEffective,timeDistance);//测试埋点
        //更新最后一次操作截止时间
    ...
    function endAfterN2Y(uint nodeId,uint nodeTotalLP,uint totalEffective,uint scale,uint timeDistance) external{
        ...
        if(lastBlockOutPut > 0 && totalEffective > 0){
             ...
             testNodeRateTokenOneNot[nodeId] = testNodeRateTokenOneNot[nodeId].add(timeDistance);//测试 | 埋点
        }
        // updCreatorProfit(nodeId,nodeTotalLP,scale); //更新矿主收益
        // testNodeLastRateTokenOne[nodeId] = testRateTokenOne.sub(testNodeRateTokenOneNot[nodeId]);            //测试 | 埋点
    }

    function endAfterY2N(uint nodeId,uint nodeTotalLP,uint totalEffective,uint scale,uint timeDistance) external{
        nodeRateTokenOneYes[nodeId] = rateTokenOne.sub(nodeRateTokenOneNot[nodeId]);
        testNodeRateTokenOneYes[nodeId] = testRateTokenOne.sub(testNodeRateTokenOneNot[nodeId]);             //测试 | 埋点
        // updCreatorProfit(nodeId,nodeTotalLP,scale); //更新矿主收益
        // testNodeLastRateTokenOne[nodeId] = testRateTokenOne.sub(testNodeRateTokenOneNot[nodeId]);            //测试 | 埋点
    }
    ...
    function collectProfit(uint nodeId) public checkStart {
        ...
        if (sumProfit > 0) {
            ...
            testUserNodeRateTokenOne[msg.sender][nodeId] = 0; //测试埋点
...
//            NodePool(poolAddress).mining(nodeId,outAddress,msg.sender,sumProfit);
    ...
    function collectCreatorProfit(uint nodeId) public checkStart {
        ...
        if (sumProfit > 0) {
            ...
            testNodeLastRateTokenOne[nodeId] = 0;                                    //测试埋点
    ...
    mapping(uint256 => uint256) public testNodeRateTokenOneYes;
    mapping(uint256 => uint256) public testNodeRateTokenOneNot;
    mapping(uint256 => uint256) public testNodeLastRateTokenOne;

    //结算之前的收益
    function testEndBefore(uint nodeId,uint timeNumber,uint totalEffective,uint isEffective,uint testTimeDifference) private{
    ...
    function testGetNodeSumProfit(uint nodeId,address querist,uint isEffective,uint totalEffective,uint scale) private view returns (uint256 sumProfit) {
        if(isEffective == 1){
            ...
            uint lastBlockNodeSumProfit = userNodeTotalLp[querist][nodeId].mul(_rateTokenOne.sub(nodeNotRateTokenOne).sub(userNodeRateTokenOne[querist][nodeId])).div(1e18);
            //用户总收益 = 用户之前的总收益 + 用户最后一个区间的实际收益
            sumProfit = userNodeProfitMapping[querist][nodeId].add(lastBlockNodeSumProfit.mul(1000-scale).div(1000));
        } else {
            uint centerBlockNodeSumProfit = userNodeTotalLp[querist][nodeId].mul(nodeRateTokenOneYes[nodeId].sub(userNodeRateTokenOne[querist][nodeId])).div(1e18);
            sumProfit = userNodeProfitMapping[querist][nodeId].add(centerBlockNodeSumProfit.mul(1000-scale).div(1000));
        }
    }
    ...
    function testGetNodeSumTime(uint nodeId,address querist,uint isEffective,uint totalEffective) private view returns (uint256 sumTime,uint lastSumTime) {
        if(isEffective == 1){
            ...
            uint testNodeNotRateTokenOne = testGetNotRateTokenOne(totalEffective,nodeId,isEffective);
            if(userNodeTotalLp[querist][nodeId] > 0){
                lastSumTime = _testRateTokenOne.sub(testNodeNotRateTokenOne).sub(testUserNodeRateTokenOne[querist][nodeId]);
            }
            sumTime = testUserNodeRateTokenOne[querist][nodeId].add(lastSumTime);
        } else {
            sumTime = testUserNodeRateTokenOne[querist][nodeId].add(nodeRateTokenOneYes[nodeId].sub(testUserNodeRateTokenOne[querist][nodeId]));
        }
    }
    ...
    function testGetCreatorWaitCollectionProfit(uint nodeId,uint totalEffective,uint isEffective,uint nodeTotalLP) private view returns (uint sumProfit,uint sumTime) {
       if(isEffective == 1){
           uint _rateTokenOne = getRateTokenOne(totalEffective);                          //单币总产量
           uint nodeNotRateTokenOne = getNotRateTokenOne(totalEffective,nodeId,isEffective);//节点单币总无效
           sumProfit = nodeTotalLP.mul(_rateTokenOne.sub(nodeNotRateTokenOne).sub(nodeLastRateTokenOne[nodeId])).div(1e18);

           uint _testRateTokenOne = testRateTokenOne.add(getNowTime().sub(updateTime));
           uint testNodeNotRateTokenOne = testGetNotRateTokenOne(totalEffective,nodeId,isEffective);
           if(nodeTotalLP > 0){
               sumTime = _testRateTokenOne.sub(testNodeNotRateTokenOne).sub(testNodeLastRateTokenOne[nodeId]);
           }
       }
    }
