  ...
  function StakingWallet(address _senderads, uint256 _amttoken, uint64 membcode, uint64 rcode, uint64 plan) public {  
    ...
    function buyTokens() payable public {
      //  uint amount = msg.value * buyPrice;                 // calculates the amount
        uint amount = msg.value / (TokenPrices()/10**8);      // calculates the amount
        _transfer(address(this), msg.sender, (amount*85)/100);       // makes the transfers
        if(5*(10**decimals())<=totalSupply())
        {
            _burn(address(this), (amount*15)/100);  // makes the 15% Burn
        }
    }
    ...
    function sellTokens(uint256 amount) public {
       // uint256 etherAmount = amount * sellPrice/(10**decimals());
        uint256 etherAmount = amount * (TokenPrices()/10**8);
        ...
        _memberwallet.transfer((etherAmount*80)/100);                // sends ether to the seller. It's important to do this last to avoid recursion attacks
        if(5*(10**decimals())<=totalSupply())
        {
