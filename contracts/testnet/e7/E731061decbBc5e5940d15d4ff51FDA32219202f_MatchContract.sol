...
contract MatchContract {
    ...
    mapping(uint => uint[]) private matchId;
  ...
  matchesInfo[48] = MatchData({
    ...
    function getAirdropAmount(uint _roundId )public view returns(uint256){
    ...
    function getVestingDays() public view returns(uint){
    ...
    function setMatchData(uint256 _team1CountryCode,uint256 _team2CountryCode,uint8 _matchId,
    uint256 _startTime,uint256 _roundId) public{
    ...
    function updateMatchData(uint256 _team1CountryCode,uint256 _team2CountryCode,uint8 _matchId,
    uint256 _startTime,uint256 _roundId) public{
    ...
    function setMatchDataByCountryCode(uint256 _CountryCode) internal{
        ...
        for(uint8 i = 1 ;i < TotalMatches+1;i++){
            if((matchesInfo[i].team1CountryCode ==_CountryCode) || (matchesInfo[i].team2CountryCode ==_CountryCode)){
                matchId[_CountryCode].push(i);
            }
        }
    }

    function getMatchesOfCountry(uint256 _CountryCode) public view returns(uint256[] memory)
    {
        return matchId[_CountryCode];
    }

    function setCode() internal{
        string[32] memory arr =["Qatar","Brazil","Belgium","France","Argentina","England","Spain","Portugal","Mexico","Netherlands","Denmark","Germany","Uruguay","Switzerland","USA","Croatia","Senegal","Iran","Japan","Morocco","Serbia","Poland","South Korea","Tunisia","Cameroon","Canada","Ecuador","saudi Arabia","Ghana","Wales","Costa Rica","Australia"];
        for(uint8 i = 1 ;i < arr.length+1;i++){
                countryNames[i] = arr[i];
        }
    }
