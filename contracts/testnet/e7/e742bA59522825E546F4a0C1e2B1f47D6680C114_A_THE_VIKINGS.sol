...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // locking time
    uint256 public jarlsLockTime = 2 minutes;
    uint256 public karlsLockTime = 3 minutes;

    // jarls and karls minting time storing in mappings
    ...
    function setJarlsLockingTime(uint256 _time) public {
        jarlsLockTime = _time;
    }

    function setKarlsLockingTime(uint256 _time) public {
        karlsLockTime = _time;
    }
    ...
    function _transfer(
        ...
        require(block.timestamp>jarlsMintingTime[tokenId]+jarlsLockTime,"jarls Time not reached");

        require(block.timestamp>karlsMintingTime[tokenId]+karlsLockTime,"karls Time not reached");
...
contract A_THE_VIKINGS is Ownable, Pausable, ERC721Enumerable{

    using Strings for uint256;
    using SafeMath for uint256;

    uint256 public constant MAX_SUPPLY = 5555;
    uint256 constant public init_supply = 100;
    uint256 public Owner_WALLET_Minted;
    ...
    function InitMinting() internal  {
        ...
        Owner_WALLET_Minted +=  init_supply;
    ...
    function JarlsVikingsSale(uint256 _count) public
    {
        ...
        for (uint256 i = 1; i <= _count; i++) {
            ...
            jarlsMintingTime[tokenId]= block.timestamp;
        }
    ...
    function KarlsVikings_Sale(uint256 _count) public
    {
        ...
        for (uint256 i = 1; i <= _count; i++) {
            uint256 tokenId=totalSupply() + 1;
            _safeMint(msg.sender,tokenId);
            karlsMintingTime[tokenId]= block.timestamp;
        }
