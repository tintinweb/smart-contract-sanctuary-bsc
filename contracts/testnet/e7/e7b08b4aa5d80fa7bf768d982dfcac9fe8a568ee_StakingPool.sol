...
contract Upgradable {
    ...
    struct PoolInfo {
        ...
        uint256[] configs; // startDate(0), endDate(1), duration(2), endStakeDate(3), stakingLimit(4), stopPool(5)
...
contract StakingPool is Upgradable {
   ...
   */
    ...
    */
        ...
        poolExist(strs[0])
    {
        ...
        require(msg.sender == _to,"msg.sender not equal _to");
        ...
        require(verify(_signer, _to, _tokenAddress, _tokenId, _amount, strs[0], signature), "Dont have NFT reward");
        ...
        emit ClaimRewardNFT(_to, address(this), _signer, _tokenId, _amount, strs[0], strs[1]);
    }
    ...
    */
        ...
        poolExist(strs[0])
    {
        ...
        require(msg.sender == _to,"msg.sender not equal _to");
        require(canGetReward(strs[0]),"Cant get Reward");
        require(rewardNFT1155pPerPool[strs[0]][_tokenAddress][_tokenId] >= _amount, "Pool not has enough balance of this tokenId");
        require(!invalidSignature[signature], "This signature has been used");
        require(verify(_signer, _to, _tokenAddress, _tokenId, _amount, strs[0], signature), "Dont have NFT reward");
        invalidSignature[signature] = true;
        rewardNFT1155pPerPool[strs[0]][_tokenAddress][_tokenId] -= _amount;
        ERC1155(_tokenAddress).safeTransferFrom(address(this),_to,  _tokenId, _amount, "");
        emit ClaimRewardNFT(_to, address(this), _signer, _tokenId, _amount, strs[0], strs[1]);
    }
   ...
   */
    function createPool(string[] memory strs, address[] memory addr, uint256[] memory data, uint256[] memory configs) external onlyAdmins {
        ...
        poolInfo[strs[0]].configs.push(0);
        totalPoolCreated += 1;
   ...
   */
    ...
    */
    function setStopPool(string memory poolId) external onlyAdmins poolExist(poolId) {
        PoolInfo storage pool = poolInfo[poolId];
        pool.rewardPerTokenStored = rewardPerToken(poolId);
        require(block.timestamp < pool.configs[1], "this pool has been ended");
        if (pool.configs.length == 6) {
    ...
    */
    ...
    {
        string memory poolId = strs[0];
        require(rewardNFT721pPerPool[poolId][_tokenAddress][tokenId] != 0,"Pool not has this token");
        // require(ERC721(_tokenAddress).ownerOf(tokenId) == address(this),);
    ...
    */
    ...
    {
        string memory poolId = strs[0];
        require(rewardNFT1155pPerPool[poolId][_tokenAddress][tokenId] >= amount, "Pool not has enough balance of this tokenId");
