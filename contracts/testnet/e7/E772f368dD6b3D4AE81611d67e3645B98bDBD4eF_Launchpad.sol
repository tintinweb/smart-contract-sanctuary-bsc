...
contract Launchpad is Ownable {
    ...
    struct Pool {
        ...
        uint[9] targetAmountEveryTier; // IGO
        uint[9] investedAmountEveryTier; // IGO
    }
    
    uint public fee = 0; // 1:1000000
    uint public constant stakingPoolCount = 5;
    address[stakingPoolCount] stakingPools;
    uint[9] public tierPercentage;
    ...
    function setTierPercentage(uint[9] memory _tierPercentage) public onlyOwner {
        for (uint i = 0; i < 9; i++) {
            tierPercentage[i] = _tierPercentage[i];
        }
    }
    ...
    function setTokenTotalSupply(uint256 poolId, uint256 tokenTotalSupply) private onlyPoolOwner(poolId) validPoolId(poolId){
        ...
        for (uint i = 0; i < 9; i++) {
            _poolList[poolId].targetAmountEveryTier[i] = tokenTotalSupply / _poolList[poolId].tokenPrice * tierPercentage[i] / 100;
        }  
    }
    ...
    function bid(uint poolId, uint amount) external validPoolId(poolId) {
        ...
        require(_poolList[poolId].targetAmountEveryTier[tierLevel] >= (_poolList[poolId].investedAmountEveryTier[tierLevel] + amount), "Invest amounts are full in this tier system");
        ...
        _poolList[poolId].investedAmountEveryTier[tierLevel] += amount;

        if (_poolList[poolId].targetAmountEveryTier[tierLevel] == _poolList[poolId].investedAmountEveryTier[tierLevel]) {
            _poolList[poolId].completed = true;
        }
    ...
    function claim(uint poolId) public validPoolId(poolId) onlyPoolOwner(poolId) {
        ...
        if (fee > 0) GovernanceToken.transfer(owner(), remain * fee / 10**6);
