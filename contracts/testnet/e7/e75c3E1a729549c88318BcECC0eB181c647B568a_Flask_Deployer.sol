...
interface I_Flask_Deployer is I_With_PancakeRouter, I_With_BUSDToken, I_With_WBNBToken{
    struct Flask{
        I_Flask_DAO DAO;
        I_Flask_Fund fund;
        I_Flask_Power PWR;
        I_Flask_Royalty royalty;
        I_Flask_Stacking stacking;
        I_Flask_Trade_Manager tradeManager;
        I_Flask_Updater updater;
        I_Flask_Withdrawer withdrawer;
        I_Flask_Competition competition;
        I_Flask_Trade_Deployer_Abstract[] tradeDeployers;
        bool initialized;
    }
}// Version: 1.0.0
...
interface I_Flask_Trade_Deployer_Swap is I_With_PancakeRouter, I_Flask_Trade_Deployer_Abstract{
	struct Token{
		IERC20 token;
		uint256 routeId;
		bool disabled;
...
interface I_Flask_Trade_Deployer_Long is I_Flask_Trade_Deployer_Swap{
}
// Version: 1.0.0
...
abstract contract With_PancakeRouter is Base, AccessControl, With_DAORole, I_With_PancakeRouter{
    PancakeRouter internal pancakeRouter;
...
contract Flask_Deployer is Base, Adressable, Ownable, With_PancakeRouter, With_BUSDToken, With_WBNBToken, I_Flask_Deployer{
    ////
    ////
    ////
    //////////////// Public variables ////////////////
    ...
    function deployUpdater(bytes memory bytecode) external onlyOwner returns (address payable){
        ...
        flask.updater = I_Flask_Updater(addr);
    ...
    function deployRoyalty(bytes memory bytecode) external onlyOwner returns (address payable){
        ...
        flask.royalty = I_Flask_Royalty(addr);
    ...
    function deployStacking(bytes memory bytecode) external onlyOwner returns (address payable){
        ...
        flask.stacking = I_Flask_Stacking(addr);
    ...
    function deployTradeManager(bytes memory bytecode) external onlyOwner returns (address payable){
        ...
        flask.tradeManager = I_Flask_Trade_Manager(addr);
    ...
    function deployWithdrawer(bytes memory bytecode) external onlyOwner returns (address payable){
        ...
        flask.withdrawer = I_Flask_Withdrawer(addr);
    ...
    function deployCompetition(bytes memory bytecode) external onlyOwner returns (address payable){
        ...
        flask.competition = I_Flask_Competition(addr);
        
        return addr;
    }
    
    function getContracts() external view returns (Flask memory){
        return flask;
    }
    
    function initFlask(uint256[16] calldata data,
                          uint256[] calldata proposalLevel,
                          uint256[32] calldata data2) external onlyOwner{
        require(!flask.initialized, "Flask_Deployer: Already initialized");
        
        address payable DAOAddress = payable(address(flask.DAO));
        address fundAddress = address(flask.fund);
        address royaltyAddress = address(flask.royalty);
        address stackingAddress = address(flask.stacking);
        address tradeManagerAddress = address(flask.tradeManager);
        address updaterAddress = address(flask.updater);
        
        bytes32 DAORole = flask.PWR.DAO_ROLE();
        bytes32 updaterRole = flask.DAO.UPDATER_ROLE();
        bytes32 tradeManagerRole =  flask.fund.TRADE_MANAGER_ROLE();

        
        
        //Setup PWR
        flask.PWR.setFund(fundAddress);
        
        bytes32 role = flask.PWR.MINTER_ROLE();
        flask.PWR.grantRole(role, fundAddress);
        flask.PWR.grantRole(role, royaltyAddress);
        flask.PWR.grantRole(role, stackingAddress);
        flask.PWR.grantRole(role, address(flask.competition));
        
        role = flask.PWR.BURNER_ROLE();
        flask.PWR.grantRole(role, fundAddress);
        flask.PWR.grantRole(role, stackingAddress);
        flask.PWR.grantRole(role, address(flask.withdrawer));
        
        role = flask.PWR.MOVER_ROLE();
        flask.PWR.grantRole(role, stackingAddress);
        flask.PWR.grantRole(role, address(flask.withdrawer));
        flask.PWR.grantRole(role, address(flask.competition));
        
        flask.PWR.grantRole(DAORole, DAOAddress);
        flask.PWR.revokeRole(DAORole, thisAddr);
        
        //Setup Fund
        flask.fund.setBaseWithdrawFees(data2[0], data2[1]);
        flask.fund.setDynamicFeesDurationNbBlocks(data2[16]);
        flask.fund.setPWRToken(flask.PWR);
        flask.fund.setBUSDToken(BUSDToken);
        flask.fund.setWBNBToken(WBNBToken);
        flask.fund.setUpdaterAddress(updaterAddress);
        flask.fund.setPancakeRouter(pancakeRouter);
        flask.fund.setTradeManager(tradeManagerAddress);
        flask.fund.setDAO(DAOAddress);
        
        flask.fund.grantRole(tradeManagerRole, tradeManagerAddress);
        
        flask.fund.grantRole(updaterRole, updaterAddress);
        
        role = flask.fund.PWR_TOKEN_ROLE();
        flask.fund.grantRole(role, address(flask.PWR));
        
        role = flask.fund.WITHDRAWER_ROLE();
        flask.fund.grantRole(role, address(flask.withdrawer));
        
        flask.fund.grantRole(DAORole, DAOAddress);
        flask.fund.revokeRole(DAORole, thisAddr);
        
        //Setup Withdrawer
        flask.withdrawer.setPWRToken(flask.PWR);
        flask.withdrawer.setBUSDToken(BUSDToken);
        flask.withdrawer.setFund(fundAddress);
        
        role = flask.withdrawer.TRADE_MANAGER_ROLE();
        flask.withdrawer.grantRole(role, tradeManagerAddress);
        
        flask.withdrawer.grantRole(DAORole, DAOAddress);
        flask.withdrawer.revokeRole(DAORole, thisAddr);
        
        //Setup Competition
        flask.competition.setPWRToken(flask.PWR);
        flask.competition.setFund(fundAddress);
        flask.competition.setTradeManager(tradeManagerAddress);
        flask.competition.setPercentPWRRewardMint(data2[30], data2[31]);
        
        flask.competition.grantRole(tradeManagerRole, tradeManagerAddress);
        
        role = flask.competition.UPDATER_ROLE();
        flask.competition.grantRole(role, updaterAddress);
        
        flask.competition.grantRole(DAORole, DAOAddress);
        flask.competition.revokeRole(DAORole, thisAddr);
        
        //Setup Royalty
        flask.royalty.setRoyaltyRatio(data2[2], data2[3]);
        flask.royalty.mintTo(msg.sender, 100);
        flask.royalty.setPWRToken(flask.PWR);
        flask.royalty.setFund(fundAddress);
        
        flask.royalty.grantRole(updaterRole, updaterAddress);
        
        flask.royalty.grantRole(DAORole, DAOAddress);
        flask.royalty.revokeRole(DAORole, thisAddr);
        
        //Setup Stacking
        flask.stacking.setPWRToken(flask.PWR);
        flask.stacking.setStackingRewardRatio(data2[4], data2[5]);
        flask.stacking.setStackingBonusRatio(data2[6], data2[7]);
        flask.stacking.setEarlyUnstackingFeesPercent(data2[8], data2[9]);
        flask.stacking.setMaxCycleLock(data2[17]);
        flask.stacking.setFund(fundAddress);
        
        flask.stacking.grantRole(updaterRole, updaterAddress);
        
        flask.stacking.grantRole(DAORole, DAOAddress);
        flask.stacking.revokeRole(DAORole, thisAddr);
        
        //Setup Trades
        flask.tradeManager.setFund(fundAddress);
        flask.tradeManager.setDAOAddress(DAOAddress);
        flask.tradeManager.setUpdater(updaterAddress);
        flask.tradeManager.setWithdrawer(address(flask.withdrawer));
        flask.tradeManager.setCompetition(address(flask.competition));
        flask.tradeManager.setBUSDToken(BUSDToken);
        flask.tradeManager.setMaxLeverage(data2[10], data2[11]);
        flask.tradeManager.setMaxTradeStopLoss(data2[12], data2[13]);
        flask.tradeManager.setMaxTradeDurationNbBlock(data2[14]);
        flask.tradeManager.setMinimumBUSDToTrade(data2[15]);
        flask.tradeManager.setPWRToken(flask.PWR);
        flask.tradeManager.setStacking(flask.stacking);
        
        flask.tradeManager.setNbBlockPerPointCycle(data2[18]);
        flask.tradeManager.setMinPWRToStartPaperTrade(data2[19]);
        flask.tradeManager.setNbPointsToPromoteJuniorTrader(data2[20]);
        flask.tradeManager.setNbPointsToPromoteSeniorTrader(data2[21]);
        flask.tradeManager.setNbNegativePointsToDemoteJuniorTrader(data2[22]);
        flask.tradeManager.setNbNegativePointsToDemoteSeniorTrader(data2[23]);
        flask.tradeManager.setBaseEnergyBonusRatio(data2[28], data2[29]);
        
        flask.tradeManager.grantRole(DAORole, DAOAddress);
        flask.tradeManager.revokeRole(DAORole, thisAddr);
        
        
        //Setup updater
        flask.updater.setStacking(flask.stacking);
        flask.updater.setRoyalty(flask.royalty);
        flask.updater.setTradeManager(tradeManagerAddress);
        flask.updater.setFund(fundAddress);
        flask.updater.setDAO(DAOAddress);
        flask.updater.setCompetition(address(flask.competition));
        flask.updater.setRefundGasDefaultPrice(data2[24]);
        flask.updater.setRefundNbBNBMin(data2[25]);
        flask.updater.setRefundBNBBonusRatio(data2[26]);
        flask.updater.setRefundNbBlockDelay(data2[27]);

        
        flask.updater.grantRole(DAORole, DAOAddress);
        flask.updater.revokeRole(DAORole, thisAddr);
        
        //Setup DAO
        flask.DAO.setFund(fundAddress);
        flask.DAO.setPWRToken(flask.PWR);
        flask.DAO.setTradeManager(tradeManagerAddress);
        flask.DAO.setStacking(flask.stacking);
        flask.DAO.setRoyalty(flask.royalty);
        flask.DAO.setUpdater(updaterAddress);
        flask.DAO.setCompetition(address(flask.competition));
        flask.DAO.init(data, proposalLevel);
        
        flask.DAO.addUser(msg.sender, true, false, true);
        
        flask.DAO.grantRole(updaterRole, updaterAddress);
        
        flask.DAO.revokeRole(DAORole, thisAddr);
