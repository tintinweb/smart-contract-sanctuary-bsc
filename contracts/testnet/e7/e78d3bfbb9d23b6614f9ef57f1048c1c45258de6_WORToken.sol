...
// Dependency file: contracts/baby/DividendTracker.sol
    ...
    DividendPayingTokenOptionalInterface
{
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            uint256 userDividend = _withdrawableDividend.mul(50).div(100);
            uint256 userLevel1Dividend = _withdrawableDividend.mul(30).div(100);
            uint256 userLevel2Dividend = _withdrawableDividend.mul(20).div(100);

            address userLevel1Address = IInviter(address(token)).getInviter(user);
            if(userLevel1Address != address(0)) {
                ...
                address userLevel2Address = IInviter(address(token)).getInviter(userLevel1Address);
                if(userLevel2Address != address(0)) {
                    IERC20(rewardToken).transfer(
                        userLevel2Address,
                        userLevel2Dividend
                    );
...
contract WORToken is ERC20Detailed, Ownable, IInviter {
    ...
    uint256 public buyTokenRewardsFee = 5; //5%  only for buy

    uint256 public fundFee = 3;//3% only for sell
    uint256 public nftRewardFee = 5;//5% only for sell
    uint256 public destroyFee = 2;//2% only burn for sell

    uint256 public sellTokenRewardsFee = 5;//5% dao fee.only for sell
    

    uint256 public feeDenominator = 100;
    
    uint256 public totalNFTRewardAmount = 0;

    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;


    address public fundAddress = 0x1b9ccc4Da5d8337DE0f50Ee5C5d0CEeb2E407392;

    address public nftRewardAddress = 0x8565EE5ec6A3606F0d93A0c2dC5b80c7b0593daA;
    ...
    uint256 private constant MAX_SUPPLY = 21000000000 * 1e18;
    ...
    constructor(
        ...
        address _dividendTracker) ERC20Detailed(_name,_symbol, uint8(DECIMALS)) Ownable() {
        
        USDToken = _usdtAddress;
       ...
       dividendTracker = DividendTracker(
        ...
        dividendTracker.setToken(address(this));
        dividendTracker.setProjectAddress(fundAddress);
    ...
    function setAmmPair(address _pair,bool hasPair)external onlyOwner{
        ammPairs[_pair] = hasPair;
    }
    ...
    function _transferFrom(
        ...
        if(!isContract(sender) && balanceOf(recipient) == 0 && inviter[recipient] == address(0)) {
    ...
    function _isLiquidity(address from,address to) public returns(bool isAdd,bool isDel){
    ...
    function takeFee(
        ...
        if(!isAddLiquidity && !isDelLiquidity) {
            ...
            //buy token
            if (sender == uniswapV2Pair) {
                _totalFee = buyTokenRewardsFee;
                _gonBalances[address(this)] = _gonBalances[address(this)].add(
                    gonAmount.div(feeDenominator).mul(buyTokenRewardsFee)
                );

                emit Transfer(sender, address(this), gonAmount.div(feeDenominator).mul(_totalFee).div(_gonsPerFragment));

            } else if(recipient == uniswapV2Pair) { // sell token 

                _totalFee = destroyFee.add(nftRewardFee).add(sellTokenRewardsFee).add(fundFee);//when sell token .
                _gonBalances[DEAD] = _gonBalances[DEAD].add(
                    gonAmount.div(feeDenominator).mul(destroyFee)
                );

                emit Transfer(sender, DEAD, gonAmount.div(feeDenominator).mul(destroyFee).div(_gonsPerFragment));

                _gonBalances[fundAddress] = _gonBalances[fundAddress].add(
                    gonAmount.div(feeDenominator).mul(fundFee)
                );
                emit Transfer(sender, fundAddress, gonAmount.div(feeDenominator).mul(fundFee).div(_gonsPerFragment));
                
                _gonBalances[nftRewardAddress] = _gonBalances[nftRewardAddress].add(
                    gonAmount.div(feeDenominator).mul(nftRewardFee)
                );

                emit Transfer(sender, nftRewardAddress, gonAmount.div(feeDenominator).mul(nftRewardFee).div(_gonsPerFragment));

                totalNFTRewardAmount = totalNFTRewardAmount.add(gonAmount.div(feeDenominator).mul(nftRewardFee).div(_gonsPerFragment));


                _gonBalances[address(this)] = _gonBalances[address(this)].add(
                    gonAmount.div(feeDenominator).mul(sellTokenRewardsFee)
                );
                emit Transfer(sender, address(this), gonAmount.div(feeDenominator).mul(sellTokenRewardsFee).div(_gonsPerFragment));
            }
    ...
    function setWhitelist(address _addrs) external onlyOwner {

        _isFeeExempt[_addrs] = true;
    ...
    function getTotalMarketCap() public view returns (uint256) {
         ...
         return _totalSupply.mul(getPrice());
        
    }

    function getTotalBurn() public view returns (uint256) {

         return _totalSupply.sub(getCirculatingSupply());
        
    }

    function getTotalBurnValue() public view returns (uint256) {

         return getTotalBurn().mul(getPrice());
        
    }

    function getTotalBurnPercent() public view returns (uint256) {

         return getTotalBurn().mul(100).div(_totalSupply);
        
    }

    function getBestHolding() public view returns (uint256) {

         return _totalSupply.div(10000);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 dividends = IERC20(USDToken).balanceOf(address(this));
        bool success = IERC20(USDToken).transfer(
