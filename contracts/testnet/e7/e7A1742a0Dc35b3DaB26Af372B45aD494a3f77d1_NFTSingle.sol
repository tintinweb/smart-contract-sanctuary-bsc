...
// ERC721
// ERC721A
contract NFTSingle is INFT, ERC721, Caller, Fallback {
    ...
    constructor(
        ...
        _init(config_, baseURI_, name__, symbol__);
    }
    ...
    function bytesToUint256(bytes memory b) external pure returns (uint256) {
        uint256 number;
    ...
    function batchMint(
        ...
        require(maxId.add(1, "1162") == startTokenId, "1153");
        require(1 == property_.length, "1087");
        uint256 amount = this.bytesToUint256(property_[0]);
        require(0 < amount, "1160");
        uint256 id = maxId;
        uint256[] memory tokenId = new uint256[](amount);
        ...
        if (address(0) != address(token)) {
            ...
            address openBox = config.getAddressByUint256(10010);
            require(address(0) != openBox, "1191");
            INFTOpenBox(openBox).addToken(
                address(token),
                address(this),
                tokenId
            );
    ...
    function mint(
        ...
        emit MintSingleEvent(maxId, new bytes(0), to, key);
    }
