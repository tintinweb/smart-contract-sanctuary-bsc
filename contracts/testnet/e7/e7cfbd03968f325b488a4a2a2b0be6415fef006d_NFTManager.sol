...
interface IItemFactory {
    function upsertItemSize(uint256 itemId_, uint256 size_)
        external
        view
        returns (uint256);

    function decreaseItemSize(uint256 itemId_) external;

    function getRandomItem(uint256 randomness)
        external
        view
    ...
    ItemFactoryManager
{
    ...
    mapping(uint256 => uint256) private _nftToType;
        ...
        ItemFactoryManager(itemFactory_)
    {
        ...
        _supportedNftTypes.add(3); // voucher
    ...
    function _mint(address to_, uint256 nftType_) private returns (uint256) {
        ...
        _nftToType[_tokenId] = nftType_;
        return _tokenId;
    }

    function mintBox(address to_)
        public
    ...
    {
        uint256 _tokenId1 = _mint(_msgSender(), 2);
        (bytes32 _dna, uint256 _artifacts) = nftCore.tokenMetaData(_tokenId1);
        _artifacts = _addArtifactValue(_artifacts, 16, 8, 4);
        nftCore.updateTokenMetaData(_tokenId1, _artifacts, _dna);
        
        uint256 _tokenId2 = _mint(_msgSender(), 2);
        randomGenerator.requestRandomNumber(_tokenId2);
        nftCore.burn(tokenId_);
    }
        ...
        override(RandomConsumerBase)
    {
        ...
        uint256 _itemId = itemFactory.getRandomItem(randomness_);
        itemFactory.decreaseItemSize(_itemId);
        uint256 _sex = (randomness_ % 2) + 1;
        _artifacts = _addArtifactValue(_artifacts, 16, 8, _itemId);
        _artifacts = _addArtifactValue(_artifacts, 24, 16, _sex);
