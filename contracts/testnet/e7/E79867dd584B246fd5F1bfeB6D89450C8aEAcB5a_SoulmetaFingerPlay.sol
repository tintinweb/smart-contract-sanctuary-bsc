...
contract SoulmetaFingerPlay is DivestorUpgradeable {
    ...
    event EnterRoom(uint indexed roomId, address indexed player, uint indexed enterType, uint amount);
    ...
    function setMeta(address serverAddr_, address soulToken_, address fishPondBonus_, address timeBank_, bool isOpen_) public onlyOwner {
        serverAddr = serverAddr_;
        soulTokenAddr = ISoulToken(soulToken_);
        aFishPondBonus = fishPondBonus_;
        aTimeBank = timeBank_;
    ...
    function enterRoom(uint roomId_, address player_, uint amount_) public {
        require(isOpen, "not open");
        require(_msgSender() == serverAddr, "no permission");
        require(roomId_ >= 1 && roomId_ <= MAX_ROOM_COUNT, "room id error");

        RoomInfo storage info = roomIdToUserInfo[roomId_];
        require(info.addr1 == address(0) || info.addr2 == address(0), "fill up");

        uint price;
        if (info.addr1 == address(0)) {
            info.addr1 = player_;
            info.amount = amount_;
            price = amount_;
        } else if (info.addr1 != address(0)) {
            require(info.addr1 != player_, "same player");
            info.addr2 = player_;
            price = info.amount;
        }

        soulTokenAddr.transferFrom(player_, address(this), price);
        emit EnterRoom(roomId_, player_, info.addr2 == address(0) ? 1 : 2, price);
    }
    ...
    function settleGame(uint roomId_, address winner_) public {
        ...
        uint burnSoulCount = totalSoulCount * BURN_COUNT_WEIGHT[0] / 100; // 销毁
        uint bonusSoulCount = totalSoulCount * BURN_COUNT_WEIGHT[1] / 100; // 鱼塘
        uint timeBankSoulCount = totalSoulCount * BURN_COUNT_WEIGHT[2] / 100; // 时间银行

        delete roomIdToUserInfo[roomId_];

        soulTokenAddr.burn(burnSoulCount);
        soulTokenAddr.transfer(aFishPondBonus, bonusSoulCount);
        soulTokenAddr.transfer(aTimeBank, timeBankSoulCount);
        soulTokenAddr.transfer(winner_, totalSoulCount - burnSoulCount - bonusSoulCount - timeBankSoulCount);

        emit SettleGame(roomId_, winner_, totalSoulCount / 2);
    }
}
