...
contract DexGo is Ownable,IERC721Receiver {
    ...
    constructor(uint256 networkId, address _storageContract) {
        storageContract =_storageContract;
    }
    ...
    event AddPath(uint256 indexed tokenId);
    function addPath(uint256 tokenId) public nonReentrant {
        ...
        balanceOf++;
        emit AddPath(tokenId);
    }

    event ReturnPath(uint256 indexed tokenId);
    function returnPath(uint256 tokenId) public nonReentrant {
        ...
        balanceOf--;
        delete pathsOwners[tokenId];
        emit ReturnPath(tokenId);
    }
