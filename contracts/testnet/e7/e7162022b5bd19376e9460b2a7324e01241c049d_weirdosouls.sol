...
contract weirdosouls is Claimable, UserBonus, ReentrancyGuard, IERC721Receiver {
    ...
    function Quitarunlock(address add) public{
        Player storage player = players[add];

        player.unlockedsoul =  player.unlockedsoul -1;
        //player.souls[player.souls.length] = player.souls[player.souls.length] - 28;
    }
...
*/
   /* function unstake(uint256 _amount) external {
    ...
    function WithdrawNft(uint256[] calldata tokenId, address add) public nonReentrant returns (bool) {

         for (uint256 i = 0; i < tokenId.length; i++) {
           _WithdrawNft(tokenId[i], add);
        }
      ...
      function _WithdrawNft(uint256 tokenId, address add) internal onlyStaker(tokenId)  returns (bool) {
