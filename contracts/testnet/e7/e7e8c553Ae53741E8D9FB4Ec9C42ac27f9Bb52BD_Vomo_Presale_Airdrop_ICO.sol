...
contract Vomo_Presale_Airdrop_ICO {
        ...
        onlyInState(State.Running)
    {
        ...
        if (block.timestamp <= Presale_End_Countdown) {
            uint256 PrebuyerTokens = _amount*_numToken;
            if (PrebuyerTokens > 38461538000000000000000) {
                ...
                uint256 actual_Presale_Price = _numToken -
                    presaleBuy_Transactionfee;
                uint256 actual_PrebuyerTokens = _amount * actual_Presale_Price;
                uint256 reftokensVal = _amount * _numToken;
                // tranfer to Referal if buyer 20USD or more
                if(Referral != 0x0000000000000000000000000000000000000000){
                if (_amount >= 13152000000000000) {
                    uint256 refToken = (reftokensVal / 100) * 4;
        ...
        //SoftCap
            ...
            uint256 SoftbuyerTokens = _amount*_numToken;
            if (SoftbuyerTokens > 38461538000000000000000) {
                ...
                uint256 actual_Softcap_Price = Softcap_Price -
                    softcapBuy_Transactionfee;
                uint256 actual_SoftbuyerTokens = _amount *
        ...
        //HardCap
            ...
            uint256 hardbuyerTokens = _amount * _numToken;
            if (hardbuyerTokens > 38461538000000000000000) {
                ...
                uint256 actual_Hardcap_Price = _numToken -
                    hardcapBuy_Transactionfee;
                uint256 actual_hardbuyerTokens = _amount *
        ...
        //Listing Price
            ...
            uint256 listbuyerTokens = _amount * _numToken;
            if (listbuyerTokens > 38461538000000000000000) {
                ...
                uint256 actual_Listing_Price = _numToken -
                    listingbuy_Transactionfee;
                uint256 actual_listbuyerTokens = _amount *
        ...
        // uint256 Balance_funder1 = (_amount / 100) * 15;
        // uint256 Balance_funder2 = (_amount / 100) * 5;
        // uint256 Balance_funder3 = (_amount / 100) * 5;
        // uint256 Balance_Development = (_amount / 100) * 35;
        // uint256 Balance_Marketing = (_amount / 100) * 25;
        // uint256 Balance_Community = (_amount / 100) * 5;
        // uint256 Balance_TokenStability = (_amount / 100) * 10;

        //Transfer crypto Eth to 7 wallets address
        // if (address(this).balance > 0) {
        //     payable(funder1).transfer((_amount / 100) * 15);
        //     payable(funder2).transfer((_amount / 100) * 5);
        //     payable(funder3).transfer((_amount / 100) * 5);
        //     payable(Development).transfer((_amount / 100) * 35);
        //     payable(Marketing).transfer((_amount / 100) * 25);
        //     payable(Community).transfer((_amount / 100) * 5);
        //     payable(TokenStability).transfer((_amount / 100) * 10);
        // }
    }
