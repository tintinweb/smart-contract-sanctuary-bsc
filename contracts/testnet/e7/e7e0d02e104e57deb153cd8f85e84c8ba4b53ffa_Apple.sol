...
contract Apple {
    ...
    function refreshDynamicHashRateByBuy(address user, uint256 blockHeight) internal {
        ...
        for (uint i = 0; i < maxCanGetGenD; i++) {
            ...
            // 此上级地址没有算力变更，跳过
            if (totalDynamicHashRate == getAddressCurrentDynamicHashRate(parent)) continue;
            // 每个上级的新增算力
            (bool sub, uint256 value) = checkSubOrAdd(getAddressCurrentDynamicHashRate(parent), totalDynamicHashRate);
            // 上级地址更新总动态算力
            setAddressCurrentDynamicHashRate(parent, totalDynamicHashRate);
            // 上级地址的此高度的动态算力刷新
    ...
    function refreshDynamicHashRateByInvite(address user, uint256 blockHeight) internal {
        ...
        // 重新设置此地址的动态算力
        setAddressCurrentDynamicHashRate(user, totalDynamicHashRate);
        // 此地址在此高度的动态算力
    ...
    // 地址的指定代数的子地址个数
    function getAddressSpecificSubCount(address user, uint256 i) internal view returns (uint256) {
    ...
    // 获取地址当前总动态算力
    function getAddressCurrentDynamicHashRate(address user) public view returns (uint256) {
        return _addressTotalDynamicHashRateMap[user];
    }

    // 设置地址当前总动态算力
    function setAddressCurrentDynamicHashRate(address user, uint256 hash) internal {
        _addressTotalDynamicHashRateMap[user] = hash;
    }
    ...
    // 根据区块高度得到产出量
    function getTotalGenByHeight(uint256 blockHeight) public pure returns (uint256) {
        uint256 d = blockHeight.div(90);
        if (d > 10) d = 10;
        return uint256(5200).sub(d.mul(310));
    }
