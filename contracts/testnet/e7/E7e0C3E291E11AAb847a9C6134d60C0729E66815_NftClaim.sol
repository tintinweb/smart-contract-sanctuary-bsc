...
contract NftClaim is Ownable, ReentrancyGuard {
    ...
    function setFeeAmount(uint256 _amount) external onlyAdmin nonReentrant {
        require(_amount > 0, "NONEMPTY_AMOUNT");
    ...
    function setPlatformAddress(address _address) public onlyAdmin nonReentrant {
    ...
    event Claim(address _user, address _payToken, uint256 _payPrice, uint256 nftNum, address _token, uint256[] resultTokenId, uint256 _at);
    ...
    function claim(address _token) public payable nonReentrant {
        require(checkNftIsAllowed(_token), "NFT_NOT_ALLOWED");
        uint256 nftNum = nftBalanceList[_token][msg.sender];
        require(nftNum > 0, 'NFT_BALANCE_ERROR');
        require(msg.value == feeAmount, "PAY_AMOUNT_ERROR");

        nftBalanceList[_token][msg.sender] = nftBalanceList[_token][msg.sender].sub(nftNum);

        address payable _platformAddress = address(uint160(platformAddress));
        _platformAddress.transfer(feeAmount);
        ...
        for (uint256 _i = 0; _i < nftNum; _i++) {
            if (nftTokenType[_token] > 0) {
                delete transferAddressAndTokenIdIndex[_token];
            }
        }

        emit Claim(msg.sender, BASE_ADDRESS, feeAmount, nftNum, _token, resultTokenId, block.timestamp);
    }
}