...
import { IGovernanceHelper } from "../interfaces/IGovernanceHelper.sol";
import {SafeMath} from "@openzeppelin/contracts/math/SafeMath.sol";
import { IERC20 } from "../interfaces/IERC20Snapshots.sol";

/**
 * @title Governance helper contract
 * @dev Allows to easily read data from OceanDriveGovernance contract
 * - List of proposals with state
 * - List of votes per proposal and voters
 * @author Aave
 **/
contract GovernanceHelper is IGovernanceHelper {
  using SafeMath for uint256;
  uint256 public constant ONE_HUNDRED_WITH_PRECISION = 10000;

  function getProposal(uint256 id, IOceanDriveGovernance governance)
    public
    view
    override
    returns (ProposalStats memory proposalStats)
  {
    IOceanDriveGovernance.ProposalWithoutVotes memory proposal = governance
      .getProposalById(id);
    IGovernanceStrategy strategy = IGovernanceStrategy(proposal.strategy);
    uint256 votingSupply = proposal.startBlock > block.number
      ? IERC20(strategy.OCDR()).totalSupply()
      : strategy.getTotalVotingSupplyAt(proposal.startBlock);

    return
      ProposalStats({
        totalVotingSupply: votingSupply,
        minimumQuorum: IProposalValidator(address(proposal.executor))
          .MINIMUM_QUORUM(),
        minimumDiff: IProposalValidator(address(proposal.executor))
          .VOTE_DIFFERENTIAL(),
        executionTimeWithGracePeriod: proposal.executionTime > 0
          ? IExecutorWithTimelock(proposal.executor).GRACE_PERIOD().add(
    ...
    returns (Power[] memory power)
  {
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      IGovernancePowerDelegationToken delegation = IGovernancePowerDelegationToken(
          tokens[i]
        );
...
interface IGovernanceHelper {
    ...
    struct ProposalStats {
        ...
        address strategy;
        bytes32 ipfsHash;
        IOceanDriveGovernance.ProposalState proposalState;
    }
 ...
 */
interface IERC20Snapshots is IERC20 {
