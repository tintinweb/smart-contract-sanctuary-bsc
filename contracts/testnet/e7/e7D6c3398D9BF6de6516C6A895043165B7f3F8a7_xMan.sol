...
//****************************************************************************//
 ...
 contract xMan is owned {
    ...
    uint public lockTime = 60 minutes;
    ...
    function withdrawReturn(uint stakeIndex) public returns(bool)
    {
        ...
        require(temp.lockEndTime + 60 minutes < now , "30 days not reached");
        require(temp.stakedAmount > 0 , "nothing staked");

        uint time= 60 minutes; // now - ( temp.lockEndTime ) / 1 days;
        require(time > 0, "wait more days");
        temp.lockEndTime += time * 4 minutes;
