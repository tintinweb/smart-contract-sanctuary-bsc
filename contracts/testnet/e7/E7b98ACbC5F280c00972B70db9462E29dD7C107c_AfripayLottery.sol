...
interface IAfripayLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in Afri per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in Afri token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInAfri: price of a ticket in Afri
...
/** @title Afripay Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract AfripayLottery is ReentrancyGuard, IAfripayLottery, Ownable {
    ...
    uint256 public maxPriceTicketInAfri = 50 ether;
    uint256 public minPriceTicketInAfri = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInAfri;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] AfriPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInAfri;
        uint32 finalNumber;
        address buyingAddr;
    }
        ...
        nonReentrant
    {
        ...
        // Calculate number of Afri to this contract
        uint256 amountAfriToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInAfri,
            _ticketNumbers.length
        );

        // Transfer Afri tokens to this contract
        IERC20(_lotteries[_lotteryId].buyingAddr).safeTransferFrom(address(msg.sender), address(this), amountAfriToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInAfri += amountAfriToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInAfriToTransfer
        uint256 rewardInAfriToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInAfriToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        IERC20(_lotteries[_lotteryId].buyingAddr).safeTransfer(msg.sender, rewardInAfriToTransfer);

        emit TicketsClaim(msg.sender, rewardInAfriToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInAfri) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in Afri for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].AfriPerBracket[j] =
                ...
                // A. No Afri to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].AfriPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInAfri - amountToShareToWinners);

        // Transfer Afri to treasury address
        IERC20(_lotteries[_lotteryId].buyingAddr).safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        IERC20(_lotteries[_lotteryId].buyingAddr).safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInAfri += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInAfri >= minPriceTicketInAfri) && (_priceTicketInAfri <= maxPriceTicketInAfri),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInAfri: _priceTicketInAfri,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            AfriPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInAfri: pendingInjectionNextLottery,
            finalNumber: 0,
            buyingAddr: _buyAddr
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(0) && _tokenAmount > 0, "invalid params");
    ...
    /**
     * @notice Set Afri price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInAfri: minimum price of a ticket in Afri
     * @param _maxPriceTicketInAfri: maximum price of a ticket in Afri
     */
    function setMinAndMaxTicketPriceInAfri(uint256 _minPriceTicketInAfri, uint256 _maxPriceTicketInAfri)
        external
        onlyOwner
    {
        require(_minPriceTicketInAfri <= _maxPriceTicketInAfri, "minPrice must be < maxPrice");

        minPriceTicketInAfri = _minPriceTicketInAfri;
        maxPriceTicketInAfri = _maxPriceTicketInAfri;
    }
     ...
     * @param _priceTicket price of a ticket (in Afri)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].AfriPerBracket[_bracket];
        } else {
            return 0;
        }
    }
