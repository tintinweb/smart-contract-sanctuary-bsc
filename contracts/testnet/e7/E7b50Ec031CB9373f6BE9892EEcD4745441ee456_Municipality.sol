...
contract Municipality is OwnableUpgradeable, ReentrancyGuardUpgradeable {
        ...
        uint256 _referrerId) external onlySaleActive notZeroAddress
    {
        ...
        uint256[] memory minerIds = new uint256[](minerIdPair0 + minerIdPair1);
        uint256 index = 0;
    ...
    function _getPriceForParcels(Parcel[] memory parcels) private view returns (uint256, uint256) {
        ...
        uint256 percentage;
        if(parcels.length >= 240) {
            percentage = 18;
        } else if(parcels.length >= 140) {
            percentage = 16;
        } else if(parcels.length >= 80) {
            percentage = 12;
        } else if(parcels.length >= 40) {
            percentage = 10;
        } else if(parcels.length >= 10) {
            percentage = 8;
        } else if(parcels.length >= 4) {
            percentage = 5;
        }

        uint256 finalPrice = _discountPrice(priceBefore + price, percentage);
