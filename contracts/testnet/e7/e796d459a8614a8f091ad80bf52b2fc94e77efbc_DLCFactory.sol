...
contract DLCFactory is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    ...
    function createDLC() external onlyAdmin returns (address _DLCAddress) {
        ...
        bytes32 salt = keccak256(abi.encodePacked(msg.sender, keccak256(abi.encodePacked(type(DLC).creationCode)), block.timestamp));
        DLC DLCArr = new DLC{salt: salt}(msg.sender);
