...
contract CrocoMain is Context, Ownable, Pausable
{
   ...
   {
      require((percent_ > 0) && (percent_ <= 100), "CrocoMain: depoBodyPercent must be > 0 and <= 100");
      depoBodyPercent = percent_;
   }

   function setBonusDayPercent(uint32 percent_) external onlyOwner
   {
      require((percent_ > 0) && (percent_ <= 100), "CrocoMain: bonusDayPercent must be > 0 and <= 100");
      bonusDayPercent = percent_;
   }
