...
contract MSC is Context, IBEP20, Ownable {
    ...
    uint256 public rwHoldAmount = 1000 * 10**_decimals;
    ...
    function setRwHoldAmount(uint256 num) public onlyOwner {
        rwHoldAmount = num;
        totalRwAmount = 0;
        uint256 length = rwProviders.length();
        for (uint256 i = 0; i < length; i++) {
            setHold(rwProviders.at(i));
        }
    }
    ...
    function resetRwHoldAmount(uint256 num) public onlyOwner {
        ...
        uint256 length = rwProviders.length();
        for (uint256 i = 0; i < length; i++) {
            rwProviders.remove(rwProviders.at(i));
        }
    }
    ...
    function setHold(address holder) private {
        if (_updatedRw[holder]) {
            if (balanceOf(holder) < rwHoldAmount) quitHold(holder);
        ...
        if (balanceOf(holder) < rwHoldAmount) return;
