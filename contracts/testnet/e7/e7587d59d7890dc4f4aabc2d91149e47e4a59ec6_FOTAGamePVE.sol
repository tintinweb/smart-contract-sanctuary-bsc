...
contract FOTAGamePVE is Auth, EIP712Upgradeable {
  ...
  event MissionAdded(uint id, uint mainReward, uint subReward, uint32 experience, address landLord, uint timestamp);
  event MissionUpdated(uint id, uint mainReward, uint subReward, uint32 experience, address landLord, uint timestamp);
  ...
  event GameFinished(address indexed user, uint mission, uint userReward, uint currentEnergy, bool dailyQuestCompleted);
  ...
  function getLandLord(uint _mission) external view returns (address) {
    return missions[_mission].landLord;
  }
  ...
  function finishGame(uint _mission, uint[] calldata _heroIds, uint[] calldata _itemIds, bytes memory _signature, uint _timestamp) external {
    ...
    emit GameFinished(msg.sender, _mission, userReward, energy, dailyQuestCompleted);
  }
  ...
  function _initData() private {
    ...
    for(uint i = 1; i <= totalMissions; i++) {
      missions[i] = Mission(i, i * 1e18, i * 1e18 / 2, uint32(i), mainAdmin);
  ...
  function addMission(uint _mainReward, uint _subReward, uint32 _experience, address _landLord) external onlyMainAdmin {
    totalMissions++;
    missions[totalMissions] = Mission(totalMissions, _mainReward, _subReward, _experience, _landLord);
    landLordManager.syncLandLord(totalMissions);
    emit MissionAdded(totalMissions, _mainReward, _subReward, _experience, _landLord, block.timestamp);
  }

  function updateMission(uint _id, uint _mainReward, uint _subReward, uint32 _experience, address _landLord) external onlyMainAdmin {
    missions[_id].mainReward = _mainReward;
    missions[_id].subReward = _subReward;
    missions[_id].experience = _experience;
    missions[_id].landLord = _landLord;
    landLordManager.syncLandLord(_id);
    emit MissionUpdated(_id, _mainReward, _subReward, _experience, _landLord, block.timestamp);
  }
  ...
  function setContracts(address _heroNft, address _itemNft, address _energyManager, address _rewardManager, address _marketPlace, address _landLordManager) external onlyMainAdmin {
