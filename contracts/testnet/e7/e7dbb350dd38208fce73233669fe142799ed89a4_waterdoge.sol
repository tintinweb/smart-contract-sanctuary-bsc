...
contract waterdoge is ERC20, Ownable {
    ...
    // exlcude from fees,  max transaction amount
    ...
    bool public AntiBot = true; //boolean used to turn antibot on and off
    uint256 private gasPriceLimit = 7 * 1 gwei; 
    mapping(address => uint256) public _holderLastTransferBlock; // for 1 tx per block
    mapping(address => uint256) public _holderLastTransferTimestamp; // for sell cooldown timer
    uint256 public launchblock;
    uint256 public cooldowntimer = 60; //default cooldown 60seconds
    ...
    event ExcludeFromMaxTxEvt(address indexed account, bool isExcluded);
    ...
    constructor() ERC20("WaterDoge", "WTDoge") {
        ...
        uint256 _buyDeadFees = 0;
        uint256 _buyMarketingFees = 6;
    ...
    function checkTxLimit(address from, uint256 amount) internal view {
        require(amount <= maxTransactionAmount || _isExcludedFromMaxTx[from], "MaxTX Limit Exceeded");
    }

    // exclude from max TX
    function excludeFromMaxTx(address account, bool excluded) public onlyOwner {
        _isExcludedFromMaxTx[account] = excluded;
        emit ExcludeFromMaxTxEvt(account, excluded);
    }

    // Include MaxTX function
    function includeInMaxTx(address account, bool excluded) public onlyOwner {
        _isExcludedFromMaxTx[account] = excluded;
        emit ExcludeFromMaxTxEvt(account, excluded);
    }
    ...
    // set cooldown timer, can only be between 60 and 864000 seconds (10 days max)
    function setcooldowntimer(uint256 value) external onlyOwner {
        require(value >= 60, "cooldown timer cannot be lower than 1 minute");
        require(value <= 864000, "cooldown timer cannot exceed 10 days");
        cooldowntimer = value;
    }

    // set max wallet, can not be lower than 0.01% of supply & higher than 20%
    function setmaxWallet(uint256 value) external onlyOwner {
        value = value * (10**18);
        require(value >= _totalSupply / 10000, "max wallet cannot be set to less than 0.01%");
        require(value <= _totalSupply / 5, "max wallet cannot be higher than 20%");
        maxWallet = value;
    }

    // set max Tx, can not be lower than 0.005% of supply & higher than 50%
    function setmaxTX(uint256 value) external onlyOwner {
        ...
        require(value <= _totalSupply / 2, "max TX cannot be set higher than 50%");
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _rewardFee, uint256 _liquidityFee, uint256 _deadFee, uint256 _devFee) external onlyOwner {
        ...
        buyDevFee = _devFee;
        totalBuyFees = buyMarketingFee + buyRewardsFee + buyLiquidityFee + buyDeadFee + buyDevFee;
        require(totalBuyFees <= 30, "You can only set buy fees at 30% or less");
    }
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _rewardFee, uint256 _liquidityFee, uint256 _deadFee, uint256 _devFee) external onlyOwner {
        ...
        sellDevFee = _devFee;
        totalSellFees = sellMarketingFee + sellRewardsFee + sellLiquidityFee + sellDeadFee + sellDevFee;
        require(totalSellFees <= 30, "You can only set buy fees at 30% or less");
    }
