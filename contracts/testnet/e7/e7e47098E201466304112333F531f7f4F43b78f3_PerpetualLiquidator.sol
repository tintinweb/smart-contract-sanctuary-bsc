...
contract PerpetualLiquidator is PerpetualRebalanceFunctions, IFunctionList, IPerpetualLiquidator {
    ...
    function liquidateByAMM(
        ...
        address _liquidatorAddr,
        address _traderAddr
    ) external virtual override updateFundingAndPrices(_getPoolIdFromPerpetual(_perpetualIndex)) returns (int128 fLiquidatedAmount) {
        ...
        int128 fPrice = _getPerpetualMarkPrice(perpetual);
        int128 fTraderPos = marginAccounts[perpetual.id][_traderAddr].fPositionBC;
        // partially or fully close the trader's position:
        int128 fRealizedPnL = _getTradeLogic().executeTrade(perpetual.id, _traderAddr, fTraderPos, fLiquidatedAmount, fPrice, true);
        ...
        int128 fRemainingMargin = _getMarginViewLogic().getAvailableMargin(perpetual.id, _traderAddr, false);
        ...
        int128 fPenaltyCC;
        {
            int128 fB2C = _getBaseToCollateralConversionMultiplier(perpetual, false);
            fPenaltyCC = fLiquidatedAmount.mul(fB2C).abs().mul(perpetual.fLiquidationPenaltyRate);
            fPenaltyCC = fPenaltyCC > fRemainingMargin ? fRemainingMargin : fPenaltyCC;

            // trader pays
            _updateTraderMargin(perpetual, _traderAddr, fPenaltyCC.neg());
            ...
            emit DistributeFees(liqPool.id, perpetual.id, _traderAddr, fHalfPenaltyCC, 0);
        }
        ...
        if (fRemainingMargin.sub(fPenaltyCC) > 0) {
            // regular fees
            fRealizedPnL = fRealizedPnL.sub(_getTradeLogic().distributeFees(perpetual.id, _traderAddr, address(0), fLiquidatedAmount, false));
        }
    ...
    function _getPositionAmountToLiquidate(PerpetualData storage _perpetual, address _traderAddr) internal view returns (int128) {
        ...
        if (fTraderPos.abs().sub(delta_bc.abs()) < lot) {
            return fTraderPos;
        }
    ...
    function _getBaseToQuoteConversionMultiplier(PerpetualData storage _perpetual, bool _isMarkPriceRequest) internal view returns (int128) {
        if (_isMarkPriceRequest) {
            ...
            return _perpetual.state != PerpetualState.NORMAL ? _perpetual.settlementS2PriceData.fPrice : oraclePriceData[_perpetual.oracleS2Addr].fPrice;
        }
    }
