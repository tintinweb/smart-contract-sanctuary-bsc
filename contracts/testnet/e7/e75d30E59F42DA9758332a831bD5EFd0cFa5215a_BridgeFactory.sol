...
contract BridgeFactory {
    ...
    uint256[] public projectInstances;

    event CompanyCreated(address wallet, address admin);
    event ProjectCreated(Bridge proj, Company owner, uint256 projectId);
    event FeeWalletUpdated(address oldWallet, address newWallet);
    event CompanyDetailUpdated(uint256 _companyId, PARAMETER _parameter, address value);
    event AddedAdmin(address newAdmin);
    event RemovedAdmin(address oldAdmin);

    mapping(uint256 => Company) public companies;
    mapping(address => uint256) public companyIds;
    mapping(address => uint256) public projectOwner;
    ...
    function createProject(
        ...
        require(_mintingDistributionRecipients.length == _recipientsMintingPercentage.length, "INVALID DISTRIBUTION INPUTS");
        uint256 totalPercentage;
        for(uint8 i = 0; i<_recipientsMintingPercentage.length; i++){
            totalPercentage += _recipientsMintingPercentage[i];
        }
        require(totalPercentage == 100, "INVALID DISTRIBUTION PERCENTAGES");
        Company storage owner = companies[_companyId];
        ...
        owner.companyProjects.push(projectNumber);
        projects[projectNumber] = address(project);
        projectOwner[address(project)] = _companyId;
        projectNumber++;

        emit ProjectCreated(project, owner, projectNumber);
    }

    function createCompany(address _wallet, address _admin) public isBridgeAdmin {
        require(companyIds[_admin] == 0, "WALLET REGISTERED A COMPANY");
        require(_wallet != address(0), "INVALID ADDRESS");
        require(_admin != address(0), "INVALID ADDRESS");

        companyIds[_admin] = companyNumber;
        companies[companyNumber] = Company({wallet: _wallet, admin: _admin, companyProjects: projectInstances, totalNFTsSold: 0, totalRevenue: 0});
        companyNumber++;

        emit CompanyCreated(_wallet, _admin);
    }
    ...
    function updateCompanyDetail(
        ...
        require(msg.sender == companies[_companyId].admin, "Not company admin");
        require(_companyId < companyNumber, "COMPANY NON EXISTANT");
        require(value != address(0), "INVALID ADDRESS");

        Company storage company = companies[_companyId];
        require(msg.sender == company.admin, "NOT COMPANY'S ADMIN");
    ...
    function addCompanyNFTsSold(uint256 _companyID, uint256 _quantity, uint256 _projectId) external {
        require(msg.sender == projects[_projectId], "Not correct contract");
        companies[_companyID].totalNFTsSold += _quantity;
    }

    function addTotalRevueToCompany(uint256 _companyID, uint256 _price, uint256 _projectId) external {
        require(msg.sender == projects[_projectId], "Not correct contract");
        companies[_companyID].totalRevenue += _price;
    }

    function getProjectAddress(uint256 _Id) public view returns (address) {
        require (_Id > 0, "UNEXISTANT ID");
        require(_Id <= projectNumber, "COMPANY NON EXISTANT");
        return projects[_Id];
    }

    function getCompany(uint256 _companyId) public view returns (Company memory) {
        require (_companyId > 0, "UNEXISTANT ID");
        require(_companyId < companyNumber, "COMPANY NON EXISTANT");
        Company memory company = companies[_companyId];
        return company;
    }

    function getProjectOwner(address _address) public view returns (Company memory) {
        uint256 _companyId = projectOwner[_address];
        Company memory company = companies[_companyId];
        return company;
    }

    function getCompanyProjects(uint256 _companyId) public view returns (uint256[] memory) {
        require (_companyId > 0, "UNEXISTANT ID");
        require(_companyId < companyNumber, "COMPANY NON EXISTANT");
        Company memory company = companies[_companyId];
