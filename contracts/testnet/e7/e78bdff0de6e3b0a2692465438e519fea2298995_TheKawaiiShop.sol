...
contract TheKawaiiShop is Ownable, ERC721AQueryable, PaymentSplitter {
    ...
    constructor(address[] memory _team, uint[] memory _teamShares) ERC721A("Kawaii Shop", "KAWAIISHOP") PaymentSplitter(_team, _teamShares){
        ...
        setItem(3, "Dague", "Stabby Stabby.", 25, "Common", "Weapon", "ipfs://dague", 0, 0, 0, 15, 5);
    ...
    function wlRewards(address owner) external onlyAllowedContract {
        ...
        _mint(owner, 1);
        // Give small knife --- TO CHECK
        tokenToItemIds[_nextTokenId()] = 3;
        _mint(owner, 1);
    }

    function getRandomRune() external payable{
        if(msg.value < randomRunePrice) revert("Not enough funds");
        tokenToItemIds[_nextTokenId()] = randomRuneID;
        _mint(msg.sender, 1);
    }

    function getMagicCookie() external payable{
        if(msg.value < magicCookiePrice) revert("Not enough funds");
        tokenToItemIds[_nextTokenId()] = magicCookieID;
        _mint(msg.sender, 1);
    }

    function getResurrectionScroll() external payable{
