...
import "./IPrice.sol";
import "./ISaleMITI.sol";

contract NFTO is ERC721A, Ownable,INFTO  {
...
string private URL;
uint256 public Max_NFTO= 349250;
uint256 public Total_NFTO;
uint256 public Max_mint= 100;

mapping (uint256 => uint256) public Life;
uint256 Life_Num=5;

constructor() ERC721A("Ocean","NFTO",Max_mint,Max_NFTO){
    ...
    Mint_NFT(msg.sender,2);
    //update();
    //update_URL();
}

function Mint_NFT (address adr , uint256 amount) public onlyOwner{
    //address sender = msg.sender;
    uint256 Num=amount;
    require(Total_NFTO+amount<= Max_NFTO,"Max supply");
    require(amount<=Max_mint,"Max mint 30 amount");
    //dis.MITItrans(sender, amount, ID_gnft);

    while(Num!=0){
      uint256 ID = Total_NFTO+1;
      Life[ID]=Life_Num;
      _safeMint(adr, 1);
      Num--;
      Total_NFTO++;
  ...
  function get_Max_NFTO () public virtual override view  returns (uint256){
    return Max_NFTO;
  }

  function get_Total_NFTO () public virtual override view  returns (uint256){
    return Total_NFTO;
  }

  function get_Life_Num() public virtual override view returns (uint256) {
    return Life_Num;
  }

  function set_Life_Num (uint256 new_Life_Num) public onlyOwner {
    Life_Num=new_Life_Num;
  }
  function get_LIFE_ID (uint256 ID_NFTO) public virtual override view returns (uint256){
    return Life[ID_NFTO];
  }

  function set_LIFE_ID (uint256 ID_NFTO) public virtual override {
    require(msg.sender==Contr.getMining());
    Life[ID_NFTO]--;
  }

  function Buy_Life(uint256 ID_NFTO,uint256 amount) public{
    address sender = msg.sender;  
    require(ownerOf(ID_NFTO)==sender);
    uint256 Price_ = Price.get_Price_USDT_LifeNFTO();

    require(USDT.balanceOf(sender)>=Price_*amount);
    Sale.TransferUSDT(Price_, amount, sender);
    Life[ID_NFTO]=Life[ID_NFTO]+amount;
  }

  function  update()  public virtual override{
    Basic=IBasic_info(Contr.getBasic_info());
    MITI = IERC20(address(Contr.getMiticoin()));
    dis = Idis(address(Contr.getdis()));
    Price=IPrice(address(Contr.getPrice()));
    Sale=ISaleMITI(address(Contr.getSaleMITI()));
  } 
}
...
interface INFTO {
    function get_Max_NFTO () external view  returns (uint256);
    function get_Total_NFTO () external view  returns (uint256);
    function update_URL()  external;
    function  update() external;
    function get_Life_Num() external view returns (uint256);
    function get_LIFE_ID (uint256 ID_NFTO) external view returns (uint256);
    function set_LIFE_ID (uint256 ID_NFTO)  external;
}
...
interface ISaleMITI { 
    function TransferUSDT (uint256 Price_,uint256 amount,address adr) external;
    function getTotal_Ori () external view returns (uint256);
    function getMax_Ori () external view returns (uint256);
}
