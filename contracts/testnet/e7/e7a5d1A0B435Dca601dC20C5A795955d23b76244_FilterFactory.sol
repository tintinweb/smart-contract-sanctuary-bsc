...
interface IFilterPair is IFilterERC20 {
    ...
    function initialize(address, address, IFilterManager) external;
}
...
interface IFilterRouter {
    function isLiquidityLocked(address liquidityProviderAddress, address pairAddress) external returns (bool);
}
...
contract FilterERC20 is IFilterERC20 {
    ...
    function transfer(address to, uint value) external returns (bool) {
        require(!IFilterRouter(routerAddress).isLiquidityLocked(msg.sender, address(this)), "FilterPair: LP_TRANSFER_LOCKED");
        _transfer(msg.sender, to, value);
        return true;
    }

    function transferFrom(address from, address to, uint value) external returns (bool) {
        require(!IFilterRouter(routerAddress).isLiquidityLocked(from, address(this)), "FilterPair: LP_TRANSFER_LOCKED");
...
contract FilterPair is IFilterPair, FilterERC20 {
    ...
    function initialize(address _token0, address _token1, IFilterManager _filterManager) external override {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        feeOn = filterManager.feeToAddress() != address(0);
        uint _kLast = kLast;
        if (feeOn) {
            if (_kLast != 0) {
                ...
                if (rootK > rootKLast) {
                    ...
                    if (liquidity > 0) {
                        _mint(filterManager.feeToAddress(), liquidity);
                    }
                }
            }
        } else if (_kLast != 0) {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        require(!filterManager.isFlaggedAsScam(token0), "FilterPair: SWAP_BLOCKED");
        require(!filterManager.isFlaggedAsScam(token1), "FilterPair: SWAP_BLOCKED");
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves();
...

        {
            ...
            if (data.length > 0) {
                IFilterCallee(to).filterCall(msg.sender, amount0Out, amount1Out, data);
            }
    ...
    function transferReserves() external {
        require(msg.sender == filterManager.strainerAddress(), "FilterPair: FORBIDDEN");
        IERC20(token0).transfer(filterManager.strainerAddress(), IERC20(token0).balanceOf(address(this)));
        IERC20(token1).transfer(filterManager.strainerAddress(), IERC20(token1).balanceOf(address(this)));
    }
}
...
contract FilterFactory is IFilterFactory {
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(filterManager.isVerifiedSafe(tokenA) || filterManager.isVerifiedSafe(tokenB), "FilterFactory: UNVERIFIED_BASE_TOKEN");
        ...
        IFilterPair(pair).initialize(token0, token1, filterManager);
