...
contract TOAX_NFT is BEP721, BEP721Enumerable, BEP721URIStorage, Pausable, Ownable {
    ...
    uint256 public tokenMintFee;
    address public feeWallet;
    bool public isBNBMinting;
    IBEP20 public WheelsToken;
    event SetMintFee(address indexed owner, uint256 mintFee, uint256 tokenMintFee);
    event SetFeeWallet(address indexed owner, address newFeeWallet);

    constructor(uint256 _mintFee, uint256 _tokenMintFee, address _feeWallet, address _wheels) BEP721("WHEELS", "WHEELS") {
        mintFee = _mintFee;
        feeWallet = _feeWallet;
        tokenMintFee = _tokenMintFee;
        WheelsToken = IBEP20(_wheels);
        isBNBMinting = true;
    }
    ...
    function updateMintFee(uint256 _mintFee, uint256 _tokenMintFee) public onlyOwner{
        mintFee = _mintFee;
        tokenMintFee = _tokenMintFee;
        emit SetMintFee(msg.sender, _mintFee, _tokenMintFee);
    }

    function setNFTmintingType(bool _isBNBMinting) external onlyOwner{
        require(isBNBMinting != _isBNBMinting,"already in this type of trading");
        isBNBMinting = _isBNBMinting;
    }
        ...
        whenNotPaused
    {
        if(msg.sender != owner()){
            if(isBNBMinting){
                ...
                require(msg.value == 0 ,"Invalid MintFee");
                require(WheelsToken.balanceOf(msg.sender) >= tokenMintFee ,"Invalid MintFee");
                WheelsToken.transferFrom(msg.sender,feeWallet,tokenMintFee);
            }
        }
