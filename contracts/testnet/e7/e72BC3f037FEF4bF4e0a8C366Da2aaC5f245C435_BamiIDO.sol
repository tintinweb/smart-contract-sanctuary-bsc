...
contract BamiIDO is Ownable, ReentrancyGuard {
  ...
  uint256 public tokenHold = 10 * generalDec; // 10 BAMI
  ...
  // Using TimeStamp
  function setTimeStake(uint256 _startStakeTime, uint256 _endStakeTime) public onlyOwner {
  ...
  function setUserWhitelist(address[] calldata userList) public onlyOwner {
    for (uint256 index = 0; index < userList.length; index++) {
  ...
  function removeUserWhitelist(address[] calldata userList) public onlyOwner {
    for (uint256 index = 0; index < userList.length; index++) {
      whitelistMap[msg.sender] = false;
      if (totalOfUserStaked > 0) {
        totalOfUserStaked--;
      }
    }
  }
  ...
  function buyIdo() public validBuyTime nonReentrant {
    ...
    uint256 bamiInWallet = bamiToken.balanceOf(msg.sender);
    require(bamiInWallet >= tokenHold, "Your BAMI in wallet must be greater 10 tokens.");
