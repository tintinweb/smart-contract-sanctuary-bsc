...
library PancakeLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'da9cb31baf144326a748c7774a1bea6e93221a70046bb4d2a121afcbc521f0b9' // init code hash
            ))));
    }
...
contract PancakeRouter is IPancakeRouter02 {
    ...
    function swapExactTokensForTokens(
        ...
        amounts = PancakeLibrary.getAmountsOut(factory, amountInX, path);
    ...
    function swapTokensForExactTokens(
        ...
        _takeTokenFees(path[0] , fees , msg.sender);
        amounts = PancakeLibrary.getAmountsIn(factory, amountOutX, path);
        require(amounts[0] <= amountInMaxX, 'PancakeRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]
        );
        _swap(amounts, path, to);
    ...
    {
        require(path[0] == WETH, 'PancakeRouter: INVALID_PATH');
        (uint fees , uint amountInX , uint amountOutMinX  )  = countETHFees(msg.value, msg.sender, to, path, amountOut);
        _takeFees(fees);
        amounts = PancakeLibrary.getAmountsIn(factory, amountOutMinX, path);
        require(amounts[0] <= amountInX, 'PancakeRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
        // refund dust eth, if any
        if (amountInX > amounts[0]) TransferHelper.safeTransferETH(msg.sender, amountInX - amounts[0]);
    }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        _takeTokenFees(path[0] , fees , msg.sender);
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, PancakeLibrary.pairFor(factory, path[0], path[1]), amountInX
        );
        ...
        ensure(deadline)
    {
        ...
        uint amountIn = msg.value;
         (uint fees , uint amountInX , uint amountOutMinX  )  = countETHFees(amountIn, msg.sender, to, path, amountOutMin);
        _takeFees(fees);
        IWETH(WETH).deposit{value: amountInX}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, path[0], path[1]), amountInX));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMinX,
