...
contract GPlantsDesign is AccessControlUpgradeable, UUPSUpgradeable, IGDesign {
  using GPlantsLibrary for GPlantsLibrary.Details;

  bytes32 public constant UPGRADER_ROLE = keccak256("UPGRADER_ROLE");
  bytes32 public constant DESIGNER_ROLE = keccak256("DESIGNER_ROLE");
  uint256 private constant SKIN_COMMON_COUNT      = 9; // Indicas
  uint256 private constant SKIN_RARE_COUNT        = 9; // Sativas
  uint256 private constant SKIN_SUPER_RARE_COUNT  = 9; // Rudelaris
  ...
  function initialize() public initializer {
    ...
    // common - sativas
    rarityStats[0] = Stats(MinMax(50, 65), MinMax(1, 9), 0, 0);

    // rare - indicas
    rarityStats[1] = Stats(MinMax(50, 65), MinMax(10, 19), 0, 0);

    // super rare - rudelaris
    rarityStats[2] = Stats(MinMax(50, 65), MinMax(20, 29), 0, 0);
  ...
  function createRandomToken(uint256 seed, uint256 id, uint256 rarity) external view override returns (uint256 nextSeed, uint256 encodedDetails) {
    GPlantsLibrary.Details memory details;
    details.id = id;
    ...
    if(details.rarity == 0) {
      (seed, details.skin) = Utils.randomRangeInclusive(seed, 1, SKIN_COMMON_COUNT);
    }
    else if (details.rarity == 1){
      (seed, details.skin) = Utils.randomRangeInclusive(seed, 1, SKIN_RARE_COUNT);
    }
    else if (details.rarity >= 2) {
      details.rarity = 2;
      (seed, details.skin) = Utils.randomRangeInclusive(seed, 1, SKIN_SUPER_RARE_COUNT);
    }
    Stats storage stats = rarityStats[details.rarity];
    (seed, details.harvestDays) = Utils.randomRangeInclusive(seed, stats.harvestDays.min, stats.harvestDays.max);
    details.commitDate = 0;
    details.claimDate = 0;
    // status 0 = not planted / seed
    // status 1 = planted
    // status 2 = dead / expired
    details.status = 0;
...
library GPlantsLibrary {
  ...
  function decode(uint256 details) internal pure returns (Details memory result) {
    ...
    result.claimDate = decodeClaimDate(details);
    result.status = decodeStatus(details);
...
library Utils {
  ...
  /// Weighted random.
  function weightedRandom(uint256 seed, uint256[] memory weights) internal
    view
