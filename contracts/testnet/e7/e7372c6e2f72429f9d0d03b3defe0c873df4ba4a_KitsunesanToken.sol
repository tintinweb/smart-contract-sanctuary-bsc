...
contract KitsunesanToken is BEP20, Ownable {
    ...
    Kitsunesan_Dividend_Tracker public dividendTracker;
    ...
    constructor(address team_, address marketing_, address liquidity_) BEP20("KITSUNESAN", "KIST") {
        
        teamAddress = team_;
        marketingAddress = marketing_;
        liquidityWallet = liquidity_;

    	dividendTracker = new Kitsunesan_Dividend_Tracker();
    ...
    function enableTrading() public onlyOwner {
        require(!tradingIsEnabled, "KIST: Trading already enabled..");
    ...
    function addOnBlackList(address botAddress) public onlyOwner {
        require(isContract(botAddress), "KIST: You can blacklit only bot not an user..");
    ...
    function burn(uint256 amount) public {
        require(amount > 0, "KIST: amount must be greater than 0");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "KIST: The dividend tracker already has that address");

        Kitsunesan_Dividend_Tracker newDividendTracker = Kitsunesan_Dividend_Tracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "KIST: The new dividend tracker must be owned by the KIST token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "KIST: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "KIST: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "KIST: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "KIST: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "KIST: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "KIST: Cannot update gasForProcessing to same value");
    ...
    function _transfer(
        ...
        require(!isBlacklisted[from] && !isBlacklisted[to], "KIST: You are blacklisted...");

        // only whitelisted addresses can make transfers after the fixed-sale has started
        // and before the public presale is over
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "KIST: This account cannot send tokens until trading is enabled");
        }
    ...
    function transferAnyBEP20Tokens(address tokenAddress, address recipient, uint256 amount) public onlyOwner {
        require(amount > 0, "KIST: amount must be greater than 0");
        require(recipient != address(0), "KIST: recipient is the zero address");
        require(tokenAddress != address(this), "KIST: Not possible to transfer KIST");
    ...
    function transferAnyBEP20TokensFromDividend(address tokenAddress, address _dividend, address receipent, uint256 amount) public onlyOwner {
        require(amount > 0, "KIST: amount must be greater than 0");
        require(receipent != address(0), "KIST: recipient is the zero address");
...
contract Kitsunesan_Dividend_Tracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    ...
    constructor() DividendPayingToken("Kitsunesan_Dividend_Tracker", "KISTDT") {
    	claimWait = 3600;
        minimumTokenBalanceForDividends = 1; //must hold 1+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Kitsunesan_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "Kitsunesan_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Kitsunesan contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "Kitsunesan_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "Kitsunesan_Dividend_Tracker: Cannot update claimWait to same value");
    ...
    // function to allow admin to transfer *any* BEP20 tokens from this contract..
    function transferAnyBEP20Tokens(address tokenAddress, address recipient, uint256 amount) public onlyOwner {
        require(amount > 0, "Kitsunesan: amount must be greater than 0");
        require(recipient != address(0), "Kitsunesan: recipient is the zero address");
        require(tokenAddress != address(this), "Kitsunesan: Not possible to transfer Kitsunesan");
        IContract(tokenAddress).transfer(recipient, amount);
    }
}