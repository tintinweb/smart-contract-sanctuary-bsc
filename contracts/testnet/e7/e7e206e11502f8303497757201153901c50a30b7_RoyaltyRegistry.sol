 ...
 */
contract RoyaltyRegistry is ERC165, Ownable, IRoyaltyRegistry {
    ...
    function overrideAllowed(address tokenAddress) public view override returns(bool) {
        ...
        try Ownable(tokenAddress).owner() returns (address owner) {
            if (owner == _msgSender()) return true;

            if (owner.isContract()) {
              try Ownable(owner).owner() returns (address passThroughOwner) {
                  if (passThroughOwner == _msgSender()) return true;
              } catch {}
            }
        } catch {}

        try IAccessControl(tokenAddress).hasRole(0x00, _msgSender()) returns (bool hasRole) {
