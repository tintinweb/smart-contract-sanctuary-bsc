...
contract Farming is ReentrancyGuard, Ownable {
    ...
    uint8[1000] public happyCowNftBreeds;
    uint[100] public genesisTokenIds;
    ...
    function depositLand(uint _tokenId) public {
        ...
        farms[farmIdx].landTokenIds.push(_tokenId);
    ...
    function setGenesisTokenIds(uint[] calldata _genesisTokenIds) public onlyOwner {
        for(uint idx=0;idx < _genesisTokenIds.length;idx ++) {
            genesisTokenIds[idx] = _genesisTokenIds[idx];
        }
    }

    // Set HappyCow NFTs' Breed attributes.
    function setHappyCowNftBreeds(uint8[] calldata _happyCowNftBreeds) public onlyOwner {
        for(uint idx=0;idx < _happyCowNftBreeds.length;idx ++) {
            happyCowNftBreeds[idx] = _happyCowNftBreeds[idx];
        }
    }
    ...
    function _indexOfFarmByOwner(address _farmer) private view returns (uint) {
        uint idx = 0;
        for(idx = 0;idx < farms.length;idx ++){
