...
contract Exyrium is Context, IERC20, Ownable {
    ...
    address payable public marketingWallet = payable(0xF83ab81f901F3208e550Fa2eE766D94498208F6f);
    ...
    uint256 public _saleBuyBackFee = 3;
    ...
    uint256 public minimumTokensBeforeSwap = 1 * 10**9;
    uint256 public _maxBuyTxAmount = 1000 * 10**9;
    uint256 public _maxSellTxAmount = 1000 * 10**9;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityTokenShare = contractTokenBalance.mul(_saleLiquidityFee).div(_saleLiquidityFee.add(_saleBuyBackFee).add(_saleMarketingFee));
        uint256 marketingTokenShare = contractTokenBalance.mul(_saleMarketingFee).div(_saleLiquidityFee.add(_saleBuyBackFee).add(_saleMarketingFee));
        uint256 buybackTokenShare = contractTokenBalance.sub(liquidityTokenShare.add(marketingTokenShare));
        ...
        // swap for buyback
        swapTokensForEth(buybackTokenShare, address(this));
    ...
    function setFees(uint256 taxFee, uint256 liquidityFee, uint256 marketingFee,uint256 buyBackFee) 
    external onlyOwner() {
    ...
    function SetBuyBackUpperLimit(uint256 _newAmount) external onlyOwner {
  	    buyBackUpperLimit = _newAmount;
  	}
