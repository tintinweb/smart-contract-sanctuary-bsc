...
contract TestToken is ERC20, Ownable, SharedConstants {
    ...
    constructor() ERC20 ("Token", "Token") {
        ...
        TokenDT = dividendManager.addDividendTracker("TokenDividendTracker", "TokenDT", address(this), 86400, 1000 * 10**18, 1, 1);
        ...
        dividendManager.excludeFromAllDividends(TokenDT);
        ...
        isExcludedFromFees[TokenDT] = true;
    ...
    function updateDividendManager (address newAddress) external onlyOwner {
        require (newAddress != address(dividendManager), "Token: The dividend manager already exists");
        DividendManager newDividendManager = DividendManager(newAddress);
        
        require (newDividendManager.owner() == address(this), "Token: The new dividend manager must be owned by the token contract");
        ...
        // newDividendManager.excludeFromAllDividends(TokenDT);
    ...
    function setMarketingWallet (address payable wallet) external onlyOwner {
        require (wallet != address(0), "Token: Can't set marketing wallet to zero address");
    ...
    function setBuyFees (uint8[] memory _rewardsFee, uint8 _liquidityFee, uint8 _marketingFee, uint8 _burnFee) external onlyOwner {
        require (_rewardsFee.length == dividendManager.dividendTrackers(), "Token: Must provide a fee per dividend tracker (array wrong length)");
        ...
        require (totalBuyFees <= 35, "Token: Fees can't be > 35%");
    ...
    function setSellFees (uint8[] memory _rewardsFee, uint8 _liquidityFee, uint8 _marketingFee, uint8 _burnFee) external onlyOwner {
        ...
        require (totalSellFees <= 35, "Token: Fees can't be > 35%");
    ...
    function setAutomatedMarketMakerPair (address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "Token: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function setSwapAmount (uint256 _swapTokensAtAmount) external onlyOwner {
        require (_swapTokensAtAmount >= totalSupply() / 1_000_000 && _swapTokensAtAmount <= totalSupply() / 100, "Token: Swap amount must be between 0.00001% and 1% of total supply");
    ...
    function getRewardTokenPercentages (uint8 feeType) public view returns (uint256 TokenFeePercentage, uint256 BUSDFeePercentage) {
        uint8 BUSDFee = dividendManager.getFee (feeType, BUSD); 
        uint8 TokenFee = dividendManager.getFee (feeType, address(this));
        uint256 sumSells = transactionBuffer.sumPerKey[SELL];
        uint256 sumBuys = transactionBuffer.sumPerKey[BUY];
        
        // Token Fee In Percent  = Sum Sells * Original Token Fee * Total Dividend Fees / (Sum Buys * Original BUSD Fee + Sum Sells * Original Token Fee)
        // increase Token fee on sells, decrease on buys
        TokenFeePercentage = sumSells * TokenFee * dividendManager.getSummedFees (feeType) / (sumSells * TokenFee + sumBuys * BUSDFee);
            
        // BUSD Fee In Percent = Sum Buys * Original BUSD Fee * Total Dividend Fees / (Sum Buys * Original BUSD Fee + Sum Sells * Original Token Fee)
        // increase BUSD fee on buys, decrease on sells
        BUSDFeePercentage = dividendManager.getSummedFees (feeType) - TokenFeePercentage;
    }
    ...
    function getTransferAmounts (uint256 amount, uint8 feeType) private returns (uint256, uint256, uint256) {
        ...
        (uint256 TokenFeePercentage, uint256 BUSDFeePercentage) = getRewardTokenPercentages (feeType);
        uint256 TokenTokens = amount * TokenFeePercentage / 100;
        uint256 BUSDTokens = amount * BUSDFeePercentage / 100;
            
        // Keep track of balances so we can split the address balance
        dividendManager.incrementFeeTokens (TokenTokens, address(this));
        dividendManager.incrementFeeTokens (BUSDTokens, BUSD);
        marketingTokens += _marketingTokens;
        liquidityTokens += _liquidityTokens;
        uint256 fees = _marketingTokens + _liquidityTokens + TokenTokens + BUSDTokens;
    ...
    function _transfer (address from, address to, uint256 amount) internal override {
       ...
       // Need to allow owner to add liquidity, otherwise prevent any snipers from buying for the first few blocks
        if (from != owner() && to != owner() && (block.timestamp <= antibotEndTime || antibotEndTime == 0)) {
            require (to == canStopAntibotMeasures, "Token: Bots can't stop antibot measures");
            if (antibotEndTime == 0)
        ...
        else if (from == TokenDT) { // Don't get caught in circular payout events
        ...
        if (from != owner() && !isExcludedFromFees[to] && to != uniswapV2Pair)
            require (balanceOf(to) + amount <= maxWalletAmount, "Token: Receiver's wallet balance exceeds the max wallet amount");
    ...
    function sellTokensForBNBAndTakeFees (uint256 swapAmount) private {
        ...
        // Send native dividends to Token dividend tracker and distribute
        uint256 TokenDividends = dividendManager.getFeeTokensFromRewardAddress(address(this)) * swapAmount / contractTokenBalance;
        super._transfer (address(this), dividendManager.getTrackerAddress(address(this)), TokenDividends);
        
        try dividendManager.distributeDividends(TokenDividends, address(this)) {
            emit SendDividends (address(this), TokenDividends, TokenDividends);
            // Make sure we zero the count of tokens set aside for Token dividends
            dividendManager.decrementFeeTokens(TokenDividends, address(this));
        } catch {
