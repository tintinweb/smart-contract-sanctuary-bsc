...
ERC721HolderUpgradeable
{
    ...
    mapping(address => Transaction[]) addressTransactions;
    ...
    function setTokenList(address[] memory _tokenList) external onlyRole(ADMIN_ROLE) {
    ...
    function depositToken(uint256 amount, uint256 tokenIndex) external {
        ...
        addressTransactions[_msgSender()].push(transaction);
    ...
    function paymentEth(address[] memory wallets, uint256[] memory amounts) external payable onlyRole(ADMIN_ROLE) {
    ...
    function paymentMultiTokens(address[] memory wallets, uint256[] memory amounts, uint256[] memory tokenIndexs) external onlyRole(ADMIN_ROLE) {
        require(wallets.length == amounts.length && tokenIndexs.length == amounts.length, "INPUT_PARAM_NOT_RIGHT");
        for (uint256 index = 0; index < wallets.length; index++) {
            address wallet = wallets[index];
            uint256 amount = amounts[index];

            address tokenAddress = tokenList[tokenIndexs[index]];
            payToken(wallet, amount, tokenAddress);
        }
    }

    function payToken(address wallets, uint256 amount, address tokenAddress) public onlyRole(ADMIN_ROLE) {
    ...
    function getWalletTransactions(address wallet, uint256 fromIndex, uint256 toIndex)
    public
    view
    returns (Transaction[] memory)
    {
        uint256 length = getTotalWalletDepositTransaction(wallet);
        Transaction[] memory emptyResponse = new Transaction[](0);
        if (length == 0) return emptyResponse;
        if (fromIndex >= length) return emptyResponse;
        ...
        for (uint256 index = fromIndex; index <= normalizedToIndex; index++) {
            result[index - fromIndex] = addressTransactions[wallet][index];
        }
        return result;
    }

    function getTotalWalletDepositTransaction(address wallet) public view returns (uint256) {
        return addressTransactions[wallet].length;
    }
