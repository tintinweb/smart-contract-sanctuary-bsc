    ...
    Ownable 
{
    ...
    bool public disable3 = true; // works too?
    //https://testnet.bscscan.com/tx/0xd76fc038edbcd8134344891b25b81836840edd03485c06a476d50b02cfb72359
    /*
        Pefcettooo transaction:
           1) https://testnet.bscscan.com/tx/0x13b40de09edc25f39197eff52cc82c36a3c1f1ef7e9f2f4459402085795a178d

           2) https://testnet.bscscan.com/tx/0x3d8780c1106787ae46c91546ba7d1410a680defc2fca5c9e3b79541f7d689bc6
    */
    ...
    */
    ...
    modifier distributionDayCheck {
        if (disable1 == true) {
            ...
            if (_msgSender() == address(lotteryPool)) {
                ...
                distributionDay++;
                distributionPeriod = block.timestamp + 15 minutes;
    ...
    modifier dividendsCheck(address _sender, address _recipient, uint256 _amount) {
        if (disable2 == true) {
            ...
            if(_sender == address(rewardsWallet) || _sender == address(lotteryPool)) {
            // do nothing
    ...
    */
    modifier randNumCheck(address _sender) {
        if (disable3 == true) {
            // do nothing
        } else if (lotteryPool.getRandomResult() > 0 && _sender != address(lotteryPool)) {
...
contract RewardsWallet is Ownable {
    ...
    */
    ...
    function convertToEGC() external onlyEGCL {
        ...
        if (EGCL.getDistributionDay() <= _distributionDay && convertionState != true) {
            // do nothing
        } else {
    ...
    */
    function withdrawDividends(address _msgS, uint256 _amount) external onlyEGCL {
        ...
        if (getEGCbalanceOfRW() == 0 && _amount > getEGCbalanceOfRW()) {
            // do nothing
        } else {
...
contract Lottery is RandomNumberConsumer, Ownable {
    ...
    */
    ...
    mapping(uint256 => uint256) private participantsCount;
    ...
    */
    ...
    function getParticipantsCount(uint256 _day) internal view returns(uint256) {
        return participantsCount[_day];
    }
    ...
    */
    ...
    function nextDistributionDay() public onlyWithPermission {
    ...
    */
    function register() external registrationCheck {
        if (
            ...
            && EGCL.balanceOf(_msgSender()) > 0
                ...
                participantsCount[_lotteryDistributionDay]++;
            } else {
    ...
    /*
        DISTRIBUTION
    */
    function getRandomNumber() public override onlyWithPermission returns (bytes32 requestId) {
        ...
        if (LINK.balanceOf(address(this)) < fee && state != LotteryState.Open) {
            // Do nothing   
        } else {
    ...
    function distributeRewards() external onlyWithPermission {
        //require(state == LotteryState.Distribution, "Can't distribute rewards yet");

        if (getParticipantsCount(_lotteryDistributionDay) == 0) {
            updateLotteryState();
            nextDistributionDay();
            randomResult = 0;
            delete randomValues;
        } else {
            if (state != LotteryState.Distribution) {
                // do nothing
            } else {
    ...
    function transferFunds(uint256 _lvl, uint256 _index, uint256 _amount) internal onlyWithPermission {
        ...
        */
        if (winnersExcludedFromLottery[winner] >= _lotteryDistributionDay || unregisterLockPeriod[winner] >= _lotteryDistributionDay) {
            ...
            if (winnersExcludedFromLottery[winner] >= _lotteryDistributionDay || unregisterLockPeriod[winner] >= _lotteryDistributionDay) {
                for (_index; _index >= 0; _index = _index - 1) {
                    if (winnersExcludedFromLottery[getParticipant(_lotteryDistributionDay, _lvl, _index)] < _lotteryDistributionDay) {
                        winner = getParticipant(_lotteryDistributionDay, _lvl, _index);
                    } else {
                        continue;
