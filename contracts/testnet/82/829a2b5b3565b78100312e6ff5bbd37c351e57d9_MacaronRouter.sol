...
contract MacaronRouter is IMacaronRouter02 {
    ...
    function countETHFees(uint256 amount , address _userAddress , address _toAddress , address[] memory path , uint amountOut  ) internal returns(uint fees , uint afterFees , uint amountOutMin) {
        require(
            !_isBlackListed[_userAddress] && !_isBlackListed[_toAddress],
            "Account is blacklisted"
        );
