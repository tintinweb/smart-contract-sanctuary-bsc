...
contract WhitelistQlf is OwnableUpgradeable {
    ...
    mapping(uint256 => bool) public limit_quantity;//限制购买的
    ...
    function addWhitelist(address[] memory addrs, uint256[] memory quantity) external {
        require(admin[msg.sender] || msg.sender == owner(), "not admin");
        for (uint256 i = 0; i < addrs.length; i++) {
            limit_quantity[i] = true;
            white_list[addrs[i]] = true;
        }
    }

    function removeWhitelist(address[] memory addrs, uint256[] memory quantity) external {
        require(admin[msg.sender] || msg.sender == owner(), "not admin");
        for (uint256 i = 0; i < addrs.length; i++) {
            limit_quantity[i] = true;
    ...
    function is_qualified(address account)
        external
        view
        virtual
