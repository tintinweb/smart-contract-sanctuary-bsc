...
contract CaraNFTProfile is Ownable, CaraSupportAsset {
    struct Approval {
        address asset; address target; bool approved;
    }

    struct Asset {
        address asset; uint balance; uint[] tokens;
    }

    struct AccountAsset {
        Asset[] wallet; Asset[] sanctum;
    }

    struct Status {
        uint8 version; address[] assets;
    }
    ...
    function accountApprovals(
        address account, address[] calldata targets, address[] calldata assets
    ) external view returns (Approval[] memory) {
        Approval[] memory _allApprovals;

        if (targets.length == 0) {
            return _allApprovals;
        }

        address[] memory _assets = assets.length > 0 ? assets : CaraSupportAsset.supportedAssets();
        _allApprovals = new Approval[](targets.length * _assets.length);
        uint8 _idx = 0;
        for (uint _tIdx = 0; _tIdx < targets.length; _tIdx++) {
            for (uint _aIdx = 0; _aIdx < _assets.length; _aIdx++) {
                _allApprovals[_idx] = Approval({
                    asset: _assets[_aIdx], target: targets[_tIdx],
                    approved: ICaraNFT(_assets[_aIdx]).isApprovedForAll(account, targets[_tIdx])
                });
                _idx += 1;
            }
        }
    ...
    function accountAssets(address account, address[] calldata assets) public view returns (AccountAsset memory) {
        address[] memory _assets = assets.length > 0 ? assets : CaraSupportAsset.supportedAssets();
        Asset[] memory _inWallet = new Asset[](_assets.length);
        Asset[] memory _inSanctum = new Asset[](_assets.length);

        address _asset; uint[] memory _tokens;
        for (uint8 _idx = 0; _idx < _assets.length; _idx++) {
            _asset = _assets[_idx];

            // wallet
            _tokens = ICaraNFT(_asset).accountTokens(account);
            _inWallet[_idx] = Asset({ asset: _asset, balance: _tokens.length, tokens: _tokens });

            // sanctum
            _tokens = ICaraStaking(SANCTUM).accountTokens(account, _asset);
            _inSanctum[_idx] = Asset({ asset: _asset, balance: _tokens.length, tokens: _tokens });
        }

        return AccountAsset({wallet: _inWallet, sanctum: _inSanctum});
    }

    function status() public view returns (Status memory) {
        return Status({version: 1, assets: CaraSupportAsset.supportedAssets()});
    }
}
