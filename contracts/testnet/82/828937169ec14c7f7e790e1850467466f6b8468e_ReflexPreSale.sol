...
abstract contract Crowdsale {
  ...
  // minimum no.of tokens to buy
  uint256 public minimumBuyLimit;

  // maximum no.of tokens to buy
  ...
  function buyTokens(address _beneficiary) internal {
    ...
    tokensSold += tokens;
    require(tokensSold <= params.hardcap , "Buy Token: Sale reached hardcap.");
    ...
    virtual
  {
    ...
    // require(_weiAmount >= minimumBuyLimit && _weiAmount <= maximumBuyLimit , "Buy Token: BNB amount exceeds minimum and maximum limit.");
  }
...
contract ReflexPreSale is Crowdsale, Pausable, FinalizableCrowdsale {
    ...
    uint256 public feeInBnb = 1;

     // fee(%) in token to reflex owner
    uint256 public tokenContributionFee = 2;
    ...
    event WithdrawContributions(uint256 tokenAmount, uint256 bnbAmount);
    ...
    constructor(
      ...
      uint256 _tokenContributionFee, // token contribution fee(%)
      uint256 _feeInBnb,             // feeInBnb(%)
      ...
      Crowdsale(rate, token)
    {
      ...
      tokenContributionFee = _tokenContributionFee;
      feeInBnb = _feeInBnb;
    }

  function setParams(uint256 _minimumBuyLimit, uint256 _maximumBuyLimit, uint256 _softcap, uint256 _hardcap) external onlyWhileOpen onlyOwner {
    ...
    function finalization() virtual internal override{
      require(tokensSold >= params.softcap, "Finalization: Softcap is not reached");
      dexListing();
    }
  ...
  function dexListing() private {
    ...
    uint256 fee = (bnbBalanceInContract * feeInBnb) / 10**2;
    require(fee > 0,"BEP20: BNB balance Is insufficicent");
    address payable _owner = payable(reflexOwner);   
    _owner.transfer(fee); 
    uint256 bnbForLiquidity = (bnbBalanceInContract * liquidityPercentage) / 10 ** 2;
    uint256 tokenForLiquidity = bnbForLiquidity * listingRate;
    // add liquidity to uniswap
    addLiquidity(tokenForLiquidity, bnbForLiquidity);

    emit DexListing(tokenForLiquidity, bnbForLiquidity);
  ...
  function withdrawContributions() external onlyOwner whenNotPaused {
    require(isFinalized, "Withdraw BNB: Not yet listed on dex"); 
    uint256 balance = token.balanceOf(address(this));
    require(balance > 0, "Withdraw Contributions: Insufficient token balance");
    if(refundType == 1){
    ...
    emit WithdrawContributions(balance, bnbBalance);
  }

  function tokenContributionToPreSale(uint256 amount) external onlyOwner whenNotPaused {
    require(token.balanceOf(owner()) > 0, "Token Contributions: Insufficient token balance");
    uint256 tokensForReflexOwner = (amount * tokenContributionFee) / 10 ** 2;
    uint256 tokensForPresale = amount - tokensForReflexOwner;
    token.transferFrom(owner(), reflexOwner, tokensForReflexOwner);
    token.transferFrom(owner(), address(this), tokensForPresale);
    emit TokenContributionToPreSale(tokensForPresale);
  }
}