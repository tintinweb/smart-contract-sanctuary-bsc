...
contract DeltaAce is Context, IBEP20, Ownable {
    ...
    address payable public MarketingWallet =
        payable(0x291942a0CcDbE9098e3266967f20bd896C996b0E);
    address public DevelopmentWallet =
        0x17eb3D14401fa814c4455D4068AEF13FaBfcE1b1;
    ...
    string private constant _name = "Delta Ace";
    string private constant _symbol = "ACED";
    ...
    uint256 private _DevelopmentFee = 200;
    uint256 private _previousDevelopmentFee = _DevelopmentFee;

    uint256 private _burnFee = 50;
    uint256 private _previousburnFee = _burnFee;

    uint256 public _buyTaxFee = 50;
    ...
    uint256 public _sellDevelopmentFee = 300;
    uint256 public _sellBurnFee = 50;

    uint256 public feeDenominator = 10_000;
    ...
    uint256 public _maxTxAmount = 700 * 10**6 * 10**9;
    uint256 public _maxWalletHoldingLimit = 1 * 10**9 * 10**9;
    ...
    function updateSellLimit24Hours(uint256 updateSellLimit)external onlyOwner(){
    ...
    function UpdateBuyTaxFeePercentage(
        ...
        require(
            taxFee.add(liquidityFee).add(burnFee).add(MarketingFee).add(
                DevelopmentFee
            ) <= (feeDenominator.div(4)),
        ...
        if (_isAtomaticMarketMaker[to] && !_isExcludedFromFee[from]) {
            ...
            if(_24HourTiming[from] < block.timestamp && circuitBreaker) {
                ...
                if(_totalSellIn24Hour[from].add(amount) <= _sellLimit){
                  ...
                  _totalSellIn24Hour[from] = 0;
                  _totalSellIn24Hour[from] = _totalSellIn24Hour[from].add(amount);
                  _24HourTiming[from] = block.timestamp + 10 minutes;
                }else{
                  _taxFee = 250;
                  _liquidityFee = 500;
                  _DevelopmentFee = 500;
            ...
            }else if(_24HourTiming[from] > block.timestamp && circuitBreaker){
                if(_totalSellIn24Hour[from].add(amount) <= _sellLimit){
                _taxFee = _sellTaxFee;
                  _liquidityFee = _sellLiquidityFee;
                  _DevelopmentFee = _sellDevelopmentFee;
