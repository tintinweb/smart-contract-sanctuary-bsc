...
contract LevelXWrapperToken is Initializable, ReentrancyGuard, ERC20
{
	...
	function _compound(address _account, uint256 _i) internal returns (uint256 _rewardAmount)
	{
		...
		if (_rewardAmount > 0) {
			...
			if (_i == 0) {
				...
				_transfer(address(this), 0x2F80922CF7350e06F4924766Cb7EEEC783c1C8ce, _shares);
			} else {
	...
	function _beforeTokenTransfer(address _from, address _to, uint256 _shares) internal override
	{
		...
		if ((msg.sender == masterChef || msg.sender == 0x2F80922CF7350e06F4924766Cb7EEEC783c1C8ce) && (_from == masterChef || _to == masterChef || _from == wrapperTokenBridge || _to == wrapperTokenBridge)) return;
		_claimRewards();
