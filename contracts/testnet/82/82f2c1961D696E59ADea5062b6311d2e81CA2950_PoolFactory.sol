...
contract PoolFactory is OwnableUpgradeable, IPoolFactory {
    ...
    event AssetHandlerChanged(address newAssetHandler);
    ...
    function createPool(
        ...
        PoolManagerLogic(newPoolManagerLogic).initialize(
            address(this),
            msg.sender,
            newPoolLogic,
            _denominationAsset,
        ...
        PoolLogic(newPoolLogic).initialize(address(this), msg.sender, newPoolManagerLogic, _poolName, _poolSymbol);
...
contract PoolLogic is ReentrancyGuardUpgradeable {
    ...
    uint256 public immutable TYPE_INVESTOR = 0;
    uint256 public immutable TYPE_MANAGEMENT = 1;

    address public factory;
    address public poolManagerLogic;
    bytes32 public poolName;
    bytes32 public poolSymbol;

    address public immutable pancakeswapRouter = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    mapping(uint256 => mapping(address => uint256)) private units;
    uint256 private totalUnit; // total amount of unit
    uint256 public sharePriceLatest; // latest share price
    ...
    event PerformanceFee(address manager, uint256 fee);
    ...
    function initialize(
        ...
        require(_poolManagerLogic != address(0), "invalid poolManagerLogic");
        require(_poolName != 0, "invalid poolName");
        require(_poolSymbol != 0, "invalid poolSymbol");
    ...
    /// @dev get balance of asset in pool
    function getBalance(address _pool, address _asset) public view returns (uint256) {
        return IERC20Upgradeable(_asset).balanceOf(_pool);
    }

    /// @dev get total of unit in pool
    ...
    function getUnit(address _investor) public view returns (uint256) {
        return units[TYPE_INVESTOR][_investor];
    }
    ...
    function getAssetHolding(address _addr) public view returns (AssetHolding[] memory) {
        require(_addr != address(0), "invalid address");
        require(indexOfInvestor[_addr] != 0 || _addr == address(this), "no address in pool");
        ...
        for (uint256 i = 0; i < assetCount; i++) {
            uint256 balance = IERC20Upgradeable(assets[i]).balanceOf(_addr);
            if (balance != 0) {
                assetHolding[index] = AssetHolding(assets[i], balance);
                index = index.add(1);
            }
        }
    ...
    function deposit(address _tokenIn, uint256 _amountIn) external {
        ...
        units[TYPE_INVESTOR][msg.sender] = units[TYPE_INVESTOR][msg.sender].add(numberOfUnit);
    ...
    function previewWithdrawal(
        ...
        uint256 numberOfUnit = units[TYPE_INVESTOR][_investor];
    ...
    function withdraw(bool _isStablecoin, uint256 _unitOut) external nonReentrant {
        require(_unitOut > 0, "require number of unit");

        uint256 numberOfUnit = units[TYPE_INVESTOR][msg.sender];
        require(numberOfUnit > 0, "no investment units");
        require(_unitOut <= numberOfUnit, "insufficient balance");
        ...
        if (numberOfUnit == _unitOut) {
            ...
            units[TYPE_INVESTOR][msg.sender] = 0;
        } else {
            // decrease unit
            units[TYPE_INVESTOR][msg.sender] = units[TYPE_INVESTOR][msg.sender].sub(_unitOut);
        }
    ...
    function swap(
        ...
        if (IPoolFactory(factory).isValidAsset(_tokenOut)) {
    ...
    function calculatePerformanceFee() external nonReentrant {
        uint256 performanceFee = IPoolManagerLogic(poolManagerLogic).performanceFee();
        if (performanceFee == 0) {
            return; // no configure performance fee
        }

        uint256 totalShare = totalSupply();
        uint256 totalValueBefore = sharePriceLatest.mul(totalShare);
        uint256 totalValueAfter = IPoolManagerLogic(poolManagerLogic).totalPoolValue();
        ...
        uint256 managerFeeValue = profit.sub(profit.mul(performanceFee).div(100));
        uint256 systemFeeValue = managerFeeValue.sub(managerFeeValue.div(10));
        ...
        uint256 managerFee = (managerFeeValue.sub(systemFeeValue)).mul(10**18).div(sharePriceLatest);
        uint256 systemFee = systemFeeValue.mul(10**18).div(sharePriceLatest);

        totalUnit = totalUnit.add(managerFee).add(systemFee);
        units[TYPE_MANAGEMENT][manager] = units[TYPE_MANAGEMENT][manager].add(managerFee);
        units[TYPE_MANAGEMENT][address(this)] = units[TYPE_MANAGEMENT][address(this)].add(systemFee);

        emit PerformanceFee(manager, managerFee);
    }
    ...
    function _swapForWithdrawal(
        ...
        uint256[] memory amountOutmin = IPancakeRouter02(pancakeswapRouter).getAmountsOut(_amountIn, path);
