...
event referalIncome(address userWoGotIncome,uint256 referalincome, address refealaddress  );
    ...
    //  } 
         ...
         function SetManagerIncome (address[] memory userAddr ,uint256 _managerIncome ) public {
     ...
     function SetSenManagerIncome (address[] memory userAddr ,uint256 _seManagerIncome) public {
        ...
        function SetdailyTopFiveReward (address[] memory userAddr , uint256 _dailyTopFiveReward) public {
             address[] memory  _userAddr = userAddr;
              for (uint8 i = 0; i < _userAddr.length; i++){
           uint256 ProfitValueRemaining = checkProfitForincomeAddition(_userAddr[i]);
            if(ProfitValueRemaining!=0){
