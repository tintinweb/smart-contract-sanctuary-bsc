...
contract FarmContractTest is Initializable, OwnableUpgradeable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address public pancakeRouter02;
    address public usdt;
    ...
    function claimAllReferralReward() public {
        ...
        userTotalClaimedReferralRewards[_msgSender()] = userTotalClaimedReferralRewards[_msgSender()].add(pendingReward);
    }
    ...
    function claimAllNodeReward() public {
        ...
        safeMiningTokenTransfer(_msgSender(), pendingReward);
        userTotalClaimedNodeRewards[_msgSender()] = userTotalClaimedNodeRewards[_msgSender()].add(pendingReward);
    }
    ...
    function getOneStarRequiredLP(uint256 pid) public view returns(uint256) {
        address pair = address(poolInfo[pid].lpToken);
        (uint112 _reserve0, uint112 _reserve1, ) = IPancakePair(pair).getReserves();
        uint256 oneStarRequiredLP = IPancakeRouter02(pancakeRouter02).getAmountOut(oneStarUserpledgeThreshold, _reserve0, _reserve1);
        return oneStarRequiredLP;
    }

    function getTwoStarRequiredLP(uint256 pid) public view returns(uint256) {
        address pair = address(poolInfo[pid].lpToken);
        (uint112 _reserve0, uint112 _reserve1, ) = IPancakePair(pair).getReserves();
        uint256 twoStarRequiredLP = IPancakeRouter02(pancakeRouter02).getAmountOut(twoStarUserpledgeThreshold, _reserve0, _reserve1);
        return twoStarRequiredLP;
    }

    function getThreeStarRequiredLP(uint256 pid) public view returns(uint256) {
        address pair = address(poolInfo[pid].lpToken);
        (uint112 _reserve0, uint112 _reserve1, ) = IPancakePair(pair).getReserves();
        uint256 threeStarRequiredLP = IPancakeRouter02(pancakeRouter02).getAmountOut(threeStarUserpledgeThreshold, _reserve0, _reserve1);
        return threeStarRequiredLP;
    }

    function getLevel(uint256 pid, address user) public view returns(uint256) {
        uint256 oneStarRequiredLP = getOneStarRequiredLP(pid);
        uint256 twoStarRequiredLP = getTwoStarRequiredLP(pid);
        uint256 threeStarRequiredLP = getThreeStarRequiredLP(pid);

        if(depositAmount[pid][user] >= Math.sqrt(threeStarUserpledgeThreshold.mul(threeStarRequiredLP))) {
            return 3;
        } else if(depositAmount[pid][user] >= Math.sqrt(twoStarUserpledgeThreshold.mul(twoStarRequiredLP))) {
            return 2;
        } else if(depositAmount[pid][user] >= Math.sqrt(oneStarUserpledgeThreshold.mul(oneStarRequiredLP))) {
            return 1;
        } else {
            return 0;
        } 
