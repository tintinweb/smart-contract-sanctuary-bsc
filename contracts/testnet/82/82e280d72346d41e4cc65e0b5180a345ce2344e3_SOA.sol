...
contract SOA is Initializable, Context, IERC20, Ownable {
    ...
    function initialize(address owner_ , uint256 _snipeBlockAmt) external initializer {
    ...
    function setNewRouter(address newRouter) public limited() {
    ...
    function removeSniper(address account) external limited() {
        require(_isSniper[account], "Account is not a recorded sniper.");
        _isSniper[account] = false;
    }

    function setSniperProtectionEnabled(bool enabled) external limited() {
        require(enabled != sniperProtection, "Already set.");
        sniperProtection = enabled;
    }
    
    function setBuyTaxes(uint256 liquidityFee, uint256 MarketingFee) external limited() {
    ...
    function setSellTaxes(uint256 liquidityFee, uint256 MarketingFee) external limited() {
        require(liquidityFee <= maxLiquidityFee); // Prevents owner from abusing fees.
        require(MarketingFee <= maxMarketingFee); // Prevents owner from abusing fees.
    ...
    function setMaxTxPercent(uint256 percent, uint256 divisor) external limited() {
    ...
    function setMaxWalletSize(uint256 percent, uint256 divisor) external limited() {
    ...
    function setMarketingWallet(address  newWallet) external limited {
        require(MarketingAddress != newWallet, "Wallet already set!");
        MarketingAddress = (newWallet);
    } 

    function setSwapAndLiquifyEnabled(bool _enabled) public limited {
        swapAndLiquifyEnabled = _enabled;
        emit SwapAndLiquifyEnabledUpdated(_enabled);
    }

    function excludeFromFee(address account) public limited {
        _isExcludedFromFee[account] = true;
    }
    
    function includeInFee(address account) external limited {
    ...
    function excludePresaleAddresses(address router, address presale) external limited {
