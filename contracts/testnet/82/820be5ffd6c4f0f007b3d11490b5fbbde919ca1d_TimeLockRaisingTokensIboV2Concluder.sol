...
contract TimeLockRaisingTokensIboV2Concluder is IIboV2Concluder {
    ...
    function initialize(
        address _sendTo, 
        uint256 _unlockDelta1,
        uint256 _lockedRateBP1,
        uint256 _unlockDelta2,
        uint256 _lockedRateBP2,
        uint256 _unlockDelta3,
        uint256 _lockedRateBP3,
        uint256 _unlockDelta4,
        uint256 _lockedRateBP4,
        uint256 _unlockDelta5,
        uint256 _lockedRateBP5
    ) external {
        require(!_isInitialized, 'Already initialized!');
        require(_sendTo != address(0), '_sendTo cannot be 0x0');        
        require(_lockedRateBP1 > 0, 'error: no lock!');
        
        uint256 totalBPs = _lockedRateBP1+_lockedRateBP2+_lockedRateBP3+_lockedRateBP4+_lockedRateBP5;
        require(totalBPs <= 10000, 'error: Total locked cannot be more than 100%!');
        
        sendTo = _sendTo;
        
        if (_lockedRateBP1 > 0) {
            unlockDeltas.push(_unlockDelta1);
            lockedRateBPs.push(_lockedRateBP1);
        }

        if (_lockedRateBP2 > 0) {
            unlockDeltas.push(_unlockDelta2);
            lockedRateBPs.push(_lockedRateBP2);
        }
        
        if (_lockedRateBP3 > 0) {
            unlockDeltas.push(_unlockDelta3);
            lockedRateBPs.push(_lockedRateBP3);
        }

        if (_lockedRateBP4 > 0) {
            unlockDeltas.push(_unlockDelta4);
            lockedRateBPs.push(_lockedRateBP4);
        }

        if (_lockedRateBP5 > 0) {
            unlockDeltas.push(_unlockDelta5);
            lockedRateBPs.push(_lockedRateBP5);
        }
...
contract TimeLockRaisingTokensIboV2ConcluderTemplate is IIboV2ConcluderTemplate {
    ...
    function createConcluder(
        address ibo, 
        address raisingToken, 
        address offeringToken, 
        address owner, 
        bytes memory callData
    ) external override returns (address concluder) {
        ...
        (bool success, ) = concluder.call(abi.encodePacked(bytes4(keccak256(bytes("initialize(address,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256)"))), callData));
