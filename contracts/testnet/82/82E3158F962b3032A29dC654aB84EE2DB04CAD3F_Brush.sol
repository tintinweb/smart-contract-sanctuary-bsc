...
contract Brush is Ownable {
	...
	event DexTrade(uint256 amountOut, uint256 amountBack);
	...
	function swap(uint256 bibAmount, uint256 busdAmount) public onlyTrader {
		address[] memory paths = new address[](2);
        if (bibAmount > 0) {
            paths[0] = BIBContract;
            paths[1] = BUSDContract;
		    IERC20(BIBContract).approve(router, bibAmount);
		    uint256 back = IUniswapV2Router01(router).swapExactTokensForTokens(bibAmount, 0, paths, address(this), block.timestamp)[1];
            
            paths[0] = BUSDContract;
            paths[1] = BIBContract;
		    IERC20(BUSDContract).approve(router, back);
		    uint256 back2 = IUniswapV2Router01(router).swapExactTokensForTokens(back, 0, paths, address(this), block.timestamp)[1];

            emit DexTrade(bibAmount, back2);
        }
        if (busdAmount > 0) {
            paths[0] = BUSDContract;
            paths[1] = BIBContract;
		    IERC20(BUSDContract).approve(router, busdAmount);
		    uint256 back = IUniswapV2Router01(router).swapExactTokensForTokens(busdAmount, 0, paths, address(this), block.timestamp)[1];

            paths[0] = BIBContract;
            paths[1] = BUSDContract;
		    IERC20(BIBContract).approve(router, back);
		    uint256 back2 = IUniswapV2Router01(router).swapExactTokensForTokens(back, 0, paths, address(this), block.timestamp)[1];

            emit DexTrade(busdAmount, back2);
        }
	}
