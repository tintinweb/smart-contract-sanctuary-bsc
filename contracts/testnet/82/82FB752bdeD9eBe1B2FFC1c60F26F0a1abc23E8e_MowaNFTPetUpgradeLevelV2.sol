...
contract MowaNFTPetUpgradeLevelV2 is Ownable, ReentrancyGuard {
    ...
    function petUpgradeLevel(uint256 tokenIdPet, uint256 tokenIdGems, uint256 tokenIdCharm) public nonReentrant {
        ...
        require(amount <= mowaToken.balanceOf(_msgSender()), "Insufficient funds in the account");
        mowaToken.transferFrom(_msgSender(), feeWallet, amount);

        uint256 level = mowaCoreV2.getNFT(tokenIdPet).level;
        uint256 version = 2;
        if (level == 0) {
            level = mowaCoreV1.getNFT(tokenIdPet).level;
            version = 1;
            require(_msgSender() == mowaNFTV1.ownerOf(tokenIdPet), "Not owner");
        } else {
            require(_msgSender() == mowaNFTV2.ownerOf(tokenIdPet), "Not owner");
        }
        userUpgrade[_msgSender()].levelNew = 0;
        userUpgrade[_msgSender()].levelOld = level;
        level += 1;
        require(level <= 9, "max level");
        uint256 qtyGems = level.sub(2).add(4);
        require(level == mowaItemsCore.getNFT(tokenIdGems).level, "Level gems are not correct");
        ...
        if (upgrade == true) {
            if (version == 1) {
                ...
                Mowa memory mowa = Mowa(
                    tokenIdPet,
                    level,
                    mowaCoreV1.getNFT(tokenIdPet).skill,
                ...
                mowaCoreV2.changeLevel(tokenIdPet, level);
            }
            ...
            userUpgrade[_msgSender()].levelNew = level;
        } else {
