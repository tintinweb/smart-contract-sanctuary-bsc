...
contract ObiDient is ERC20, Ownable, BaseToken {
    ...
    function purchase(uint256 bnbAmount, address ref) internal returns (bool) {
        ...
        uint256 referralTokens = tokensToSend.mul(bReferralFee).div(FEE_DENOMINATOR);
    ...
    function sell(uint256 tokenAmount) public returns (bool) {
        ...
        uint256 rewardAndLiquidityTokens = totalFeesTokens.mul(sTokenRewardsFee.add(sLiquidityFee)).div(
            sTotalFees
        );
        super._transfer(
            msg.sender,
            address(this),
            rewardAndLiquidityTokens
        );
