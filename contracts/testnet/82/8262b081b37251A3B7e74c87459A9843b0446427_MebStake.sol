...
interface IxMebToken {
    function mintByMebDao(address account, uint256 amount) external;

    function burnByMebDao(address account, uint256 amount) external;
}

contract MebStake is Ownable {
    ...
    constructor() {
        ...
        xMeb = 0xF93dE0F6f4fc9F4E59f746485D474b4d1845d6AD;
        meb = 0x1FFCF7F610fc615d0dc2d104B25a8bce452a66F0;
    ...
    function setXMeb(address _addr) public onlyOwner {
        require(xMeb != _addr, "xMeb is already this value");
        xMeb = _addr;
    }

    function setMeb(address _addr) public onlyOwner {
        require(meb != _addr, "Meb token address is already this value");
        meb = _addr;
    }
    ...
    struct PoolInfo {
        uint256 allocPoint;
        uint256 lockSecond;
        uint256 xMebExchangeRate; //需 x  10**18
        uint256 lastRewardTimestamp;
        uint256 accAwardPerShare;
        uint256 totalAmount;
    }

    uint256 public punishMulti = 1 * 1e18; //需 x  10**18
    uint256 public awardPerSecond;
    uint256 public totalAllocPoint = 0;
    PoolInfo[] public poolInfos;

    function setPunishMulti(uint256 _punishMulti) public onlyOwner {
        punishMulti = _punishMulti;
    }
    ...
    function add(
        ...
        poolInfos.push(
            PoolInfo({
                ...
                xMebExchangeRate: _xMebExchangeRate,
    ...
    function set(
        ...
        poolInfos[_pid].xMebExchangeRate = _xMebExchangeRate;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        IERC20(meb).safeTransferFrom(address(msg.sender), address(this), _amount);
        pool.totalAmount = pool.totalAmount.add(_amount);
        IxMebToken(xMeb).mintByMebDao(msg.sender, _amount.mul(poolInfos[_pid].xMebExchangeRate).div(1e18));
        poolInfos[_pid] = pool;
    }
    ...
    function withdraw(
        ...
        PoolInfo memory pool = updatePool(_pid);
        if (pool.lockSecond == 0) {
            IxMebToken(xMeb).burnByMebDao(msg.sender, _amount.mul(poolInfos[_pid].xMebExchangeRate).div(1e18));
            ...
            IERC20(meb).safeTransfer(msg.sender, _amount);
        } else {
            ...
            uint256 xMebAmount;
            if (depositInfo.lockTimestampUtil <= block.timestamp) {
                xMebAmount = _amount.mul(poolInfos[_pid].xMebExchangeRate).div(1e18);
            } else if (_force) {
                xMebAmount = _amount.mul(poolInfos[_pid].xMebExchangeRate).div(1e18).mul(punishMulti).div(1e18);
            } else {
                revert("The deposit is not unlock");
            }
            IxMebToken(xMeb).burnByMebDao(msg.sender, xMebAmount);
    ...
    function harvest(uint256 _pid) public {
        ...
        if (pool.lockSecond == 0) {
            ...
            if (_pending != 0) {
                userCurrentInfos[msg.sender].totalAward = userCurrentInfos[msg.sender].totalAward.add(_pending);
                IERC20(meb).transfer(msg.sender, _pending);
            }
        } else {
            ...
            if (totalPedding != 0) {
                IERC20(meb).transfer(msg.sender, totalPedding);
            }
        }
    }
