...
//    bytes32 keyHash = 0x17cd473250a9a479dc7f234c64332ed4bc8af9e8ded7556aa6e66d83da49f470;
    ...
    function _getRandomNumber(uint _n1, uint _n2, uint _n3, uint _roundId, uint _index, uint _blockNumber, bytes32 _blockHash) internal view returns (uint _number) {
    ...
    function getRandomIndex(uint _maxIndex, uint _quantity) external view returns (uint[] memory _result, uint _blockNumber) {
        ...
        for (uint i = 0; i < _quantity; i++) {
            uint _index = _getRandomNumber(curNumbers[0], curNumbers[1], curNumbers[2], currentRoundId, i, _blockNumber, blockhash(_blockNumber - 128)) % _maxIndex;
    ...
    function getPreviousResult(uint _roundId, uint _maxIndex, uint _quantity, uint _blockNumber, bytes32 _blockHash) external view returns (uint[] memory _result) {
        _result = new uint[](_quantity);
        for (uint i = 0; i < _quantity; i++) {
            uint _index = _getRandomNumber(numbers[_roundId][0], numbers[_roundId][1], numbers[_roundId][2], _roundId, i, _blockNumber, _blockHash) % _maxIndex;
            while (isExist(_result, _index, i)) {
                _index = (_index + 1) % _maxIndex;
            }
            _result[i] = _index;
