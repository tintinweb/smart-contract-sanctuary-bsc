...
contract BusinessParcel is Parcel, IBusinessParcel {

    struct PublicBuildingsBranches {
        uint256 broker;
        uint256 factory;
        uint256 bank;
        uint256 nftMarketplace;
        uint256 occupied;
    }

    /// @notice Record of stakes buildinds on the parcel
    mapping(uint256 => PublicBuildingsBranches) parcelStakes;

    /// @notice Record of assignet buildings type, her building IDs and parcels
    mapping(PublicBuildings => mapping(uint256 => uint256)) buildingTypeAndTokenIdToParcelId;

    /// @notice Amount of busines parcel slots
    uint8 parcelSlotsCount;

    /// @notice Event that is emitted when some token is assigned on parcel
    event Assigned(uint256 _parcelId, uint256 _tokenId, PublicBuildings _publicBuildingType);

    /// @notice Event that is emitted when some token is unassigned on parcel
    event Unassigned(uint256 _parcelId, uint256 _tokenId, PublicBuildings _publicBuildingType);

    /// @notice Event that is emitted when addresses contracts is changed
    event BuildingAddressChanged(address[] _contracts);

    function initialize(
        uint8 _parcelSlotsCount, 
    ...
    {
        parcelSlotsCount = _parcelSlotsCount;
    ...
    /**
     * @dev Function which sets busines parcel slots amount
     *         Only Managers can call this functions
     */
    function setParcelSlotsCount(uint8 _parcelSlotsCount) external onlyBroker {
        parcelSlotsCount = _parcelSlotsCount;
    }

    /**
     * @dev Function which assign building on parcel
     *         Only Managers can call this functions
     * @param _publicBuildingType Type of building
     * @param _x X cordinat of parcel
     * @param _y Y cordinat of parcel
     * @param _tokenId ID of Assign Token
     */
    function assignNFT(
        ...
        require(
            _publicBuildingType <= PublicBuildings.NFTMarketplace, 
            "BussinessParcel: Invalid Public Building type"
        );
        require(_exists(parcelId), "BusinessParcel: Parcel Does not exist");
        require(
            parcelStakes[parcelId].occupied < parcelSlotsCount,
            "BusinessParcel: All parcel slots are occupied"
        );
        if (_publicBuildingType == PublicBuildings.Aesthetic) {
            // Unfinished
        } else if (_publicBuildingType == PublicBuildings.Broker) {
            require(
                parcelStakes[parcelId].broker == 0,
                "BusinessParcel: Slot of the broker  on this parcel"
            );
            parcelStakes[parcelId].broker = _tokenId;
        } else if (_publicBuildingType == PublicBuildings.Factory) {
            require(
                parcelStakes[parcelId].factory == 0,
                "BusinessParcel: Slot of the factory is occupied on this parcel"
            );
            parcelStakes[parcelId].factory = _tokenId;
        } else if (_publicBuildingType == PublicBuildings.Bank) {
            require(
                parcelStakes[parcelId].bank == 0,
                "BusinessParcel: Slot of the bank is occupied on this parcel"
            );
            parcelStakes[parcelId].bank = _tokenId;
        } else {
            require(
                parcelStakes[parcelId].nftMarketplace == 0,
                "BusinessParcel: Slot of the NFT MArketplace is occupied on this parcel"
            );
            parcelStakes[parcelId].nftMarketplace = _tokenId;
        }
        parcelStakes[parcelId].occupied++;
        buildingTypeAndTokenIdToParcelId[_publicBuildingType][_tokenId] = parcelId;
        emit Assigned(parcelId, _tokenId, _publicBuildingType);
    }

    /**
     * @dev Function wh_parcelIdich unassign building on parcel
     *         Only Managers can call this functions
     * @param _publicBuildingType Type of building
     * @param _x X cordinat of parcel
     * @param _y Y cordinat of parcel
    ...
    function unassignNFT(
        ...
        require(_exists(_tokenId), "BusinessParcel: Parcel Does not exist");

        require(
            buildingTypeAndTokenIdToParcelId[_publicBuildingType][_tokenId] == parcelId,
            "BusinessParcel: Buildng  not found"
        );
        if (_publicBuildingType == PublicBuildings.Aesthetic) {
            // Unfinished
        } else if (_publicBuildingType == PublicBuildings.Broker) {
            require(
                parcelStakes[parcelId].broker == _tokenId,
                "BusinessParcel: This broker NFT is not assigned to this parcel"
            );
            parcelStakes[parcelId].broker = 0;
        } else if (_publicBuildingType == PublicBuildings.Factory) {
            require(
                parcelStakes[parcelId].factory == _tokenId,
                "BusinessParcel: This Factory branch is not assigned to this parcel"
            );
            parcelStakes[parcelId].factory = _tokenId;
        } else if (_publicBuildingType == PublicBuildings.Bank) {
            require(
                parcelStakes[parcelId].bank == _tokenId,
                "BusinessParcel: This Bank branch is not assigned to this parcel"
            );
            parcelStakes[parcelId].bank = _tokenId;
        } else if (_publicBuildingType == PublicBuildings.NFTMarketplace) {
            require(
                parcelStakes[parcelId].nftMarketplace == _tokenId,
                "BusinessParcel: This NFT-Marketplace branch is not assigned to this parcel"
            );
            parcelStakes[parcelId].nftMarketplace = 0;
        } else {
            require(
                false,
                "BusinessParcel: Invalid arguments please chack arguments"
            );
        }
        parcelStakes[parcelId].occupied--;
        buildingTypeAndTokenIdToParcelId[_publicBuildingType][_tokenId] = 0;
        emit Unassigned(parcelId, _tokenId, _publicBuildingType);
    }

    /**
     * @notice Function which return all buildings  assagined on parcel
     * @param _x X cordinat of parcel
     * @param _y Y cordinat of parcel
     * @return uint256[]  list of building IDs
     */
    function getParcelBuildings(
        ...
        returns (uint256)
    {
        uint256 parcelTokenId = _getTokenId(_x, _y);
        if(_publicBuildingType == PublicBuildings.Aesthetic) {
            // Unfinished
            return 0;
        } else if(_publicBuildingType == PublicBuildings.Broker) {
            return parcelStakes[parcelTokenId].broker;
        } else if(_publicBuildingType == PublicBuildings.Factory) {
            return parcelStakes[parcelTokenId].factory;
        } else if(_publicBuildingType == PublicBuildings.Bank) {
            return parcelStakes[parcelTokenId].bank;
        } else {
            return parcelStakes[parcelTokenId].nftMarketplace;
        }
    }
        ...
        returns (uint256)
    {
        require(
            _publicBuildingType < PublicBuildings.NFTMarketplace,
            "BusinessParcel: Invalid public building type"
        );

        return (buildingTypeAndTokenIdToParcelId[_publicBuildingType][_tokenId]);
    }
