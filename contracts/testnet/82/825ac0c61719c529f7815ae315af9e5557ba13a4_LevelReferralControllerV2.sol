...
contract LevelReferralControllerV2 is Initializable, OwnableUpgradeable {
    ...
    /// @dev epoch -> referrer -> referredCount
    mapping(uint256 => mapping(address => uint256)) public referredCount;
    ...
    function getUserTier(uint256 _epoch, address _user) public view returns (uint256) {
        ...
        for (; currentTier < tiers.length - 1;) {
            ...
            if (
                referredCount[_epoch][_user] < _nextTierInfo.minTrader
    ...
    function setReferrer(address _referrer) external {
        ...
        referredCount[currentEpoch][_referrer] = referralRegistry.referredCount(_referrer);
    ...
    function updatePoint(address _trader, uint256 _point) external {
        ...
        if (referredCount[currentEpoch][referrer] == 0) {
            referredCount[currentEpoch][referrer] = referralRegistry.referredCount(referrer);
        }
