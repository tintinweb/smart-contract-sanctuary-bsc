...
contract test is IERC20, Context, Ownable, ReentrancyGuard {
    ...
    address public treasuryWallet = 0xDa17D158bC42f9C29E626b836d9231bB173bab06 ;
    ...
    function purchase(uint256 minTokenOut, uint256 deadline) public nonReentrant ensure(deadline) payable returns (bool) {
    ...
    function sell(uint256 tokenAmount, uint256 deadline, uint256 minBNBOut) public nonReentrant ensure(deadline) returns (bool) {
        ...
        uint256 amountBNB = tokensToSwap.div(calculatePrice());
        ...
        taxBalance = taxBalance.add(tokenAmount.div(calculatePrice()).sub(amountBNB));
    ...
    function getValueOfHoldings(address holder) public view returns(uint256) {
        return _balances[holder].div(calculatePrice());
    }

    function changeFees(uint256 newBuyFee, uint256 newSellFee, uint256 newSpreadFee) external nonReentrant onlyOwner {
    ...
    function changeTaxDistribution(uint newTeamTax, uint newTreasuryTax) external nonReentrant onlyOwner {
    ...
    function changeFeeReceivers(address newTeamWallet, address newTreasuryWallet) external nonReentrant onlyOwner {
    ...
    function getTokenAmountOut(uint256 amountBNBIn) external view returns (uint256) {
        return amountBNBIn.mul(calculatePrice()).mul(spreadFee).div(divisor);
    }

    function getBNBAmountOut(uint256 amountIn) external view returns (uint256) {
        return amountIn.div(calculatePrice());
    }
