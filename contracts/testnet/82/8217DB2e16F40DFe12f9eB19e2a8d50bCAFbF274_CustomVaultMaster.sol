...
contract CustomVaultMaster is Initializable, ReentrancyGuard,IEvents{
    ...
    function addCustomVaults(IERC20 _token,address vaultAddress, uint32[] calldata _multiplier) nonReentrant external _isAdmin{
        if(vaultAddress == address(YSLVault)){
            ...
            PoolInfo memory pool = PoolInfo(_token,YSLVault,ISingleVault(YSLVault).receiptToken(),true,false,_multiplier);
            poolInfo.push(pool);
        }
        else{
            if(vaultAddress == address(xYSLVault)){
                // ISingleVault(xYSLVault).initialize(address(Admin),owner);
                PoolInfo memory pool = PoolInfo(_token,xYSLVault,ISingleVault(xYSLVault).receiptToken(),true,false,_multiplier);
                poolInfo.push(pool);
            }
            else{
                if(vaultAddress == address(USDyVault)){
                    // ISingleVault(USDyVault).initialize(address(Admin),owner);
                    PoolInfo memory pool = PoolInfo(_token,USDyVault,ISingleVault(USDyVault).receiptToken(),true,false,_multiplier);
                    poolInfo.push(pool);
                }
                else{
                    if(vaultAddress == address(BShareVault)){
                        // ISingleVault(BShareVault).initialize(address(Admin),owner);
                        PoolInfo memory pool = PoolInfo(_token,BShareVault,ISingleVault(BShareVault).receiptToken(),true,false,_multiplier);
                        poolInfo.push(pool);
                    }
                }
            }
    ...
    function addCustomLPVaults(IERC20 _token,address vaultAddress ,uint32[] calldata _multiplier) nonReentrant external _isAdmin{
        if(vaultAddress == address(YSLBUSDVault)){
            ...
            PoolInfo memory pool = PoolInfo(_token,YSLBUSDVault,ILPVault(YSLBUSDVault).receiptToken(),true,false,_multiplier);
            poolInfo.push(pool);
        }
        else{
            if(vaultAddress == address(xYSLBUSDVault)){
                // ILPVault(xYSLBUSDVault).initialize(address(Admin),owner,address(_token));
                PoolInfo memory pool = PoolInfo(_token,xYSLBUSDVault,ILPVault(xYSLBUSDVault).receiptToken(),true,false,_multiplier);
                poolInfo.push(pool);
            }
            else{
                if(vaultAddress == address(USDyBUSDVault)){
                    // ILPVault(USDyBUSDVault).initialize(address(Admin),owner,address(_token));
                    PoolInfo memory pool = PoolInfo(_token,USDyBUSDVault,ILPVault(USDyBUSDVault).receiptToken(),true,false,_multiplier);
                    poolInfo.push(pool);
                }
                else{
                    if(vaultAddress == address(BshareBUSDVault)){
                        // ILPVault(BshareBUSDVault).initialize(address(Admin),owner,address(_token));
                        PoolInfo memory pool = PoolInfo(_token,BshareBUSDVault,ILPVault(BshareBUSDVault).receiptToken(),true,false,_multiplier);
                        poolInfo.push(pool);
                    }
                }
            }
    ...
    function deposit(address user,address _token,uint32 _level,uint256 _amount, bool isBUSD)
         external 
    ...
    {   
        ...
        ISingleVault(poolInfo[PIDsOfRewardVault[_token]].vault).deposit(user,_amount,_level,isBUSD);
    ...
    function withdraw(address user,address _token,bool isReceipt,uint _recieptAmount, address sendTo)  external  {
        ...
        ISingleVault(poolInfo[PIDsOfRewardVault[_token]].vault).withdraw(user, isReceipt, _recieptAmount, sendTo);
        // userInfo[PIDsOfRewardVault[_token]][user].amount -= _amount;
        if(userInfo[PIDsOfRewardVault[_token]][user].amount == 0){
    ...
    function checkVaultStatus(address vaultAddress) public view returns(bool) {
        require(vaultAddress != address(0), 'Invalid address');
        return poolInfo[PIDsOfRewardVault[vaultAddress]].status;
    }
}
...
pragma solidity ^0.8.7;
 interface ILPVault {
    ...
    function deposit(address user, uint amountLp,uint32 level,bool isBUSD) external;
    function withdraw(address user, bool isReciept,uint _amount,address sendTo) external;
    function receiptToken() external view returns(address);
    function purchase(address user, uint amount, uint minAmount) external returns(uint);
    function sell(address user, uint amount, uint minAmount) external returns(uint);
