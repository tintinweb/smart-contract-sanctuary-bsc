...
contract MilkyWay is Context, Ownable {
    ...
    uint256 private FUEL_TO_LAUNCH_SPACESHIP = 1080000;//for final version should be seconds in a day
    uint256 private PSN = 10000;
    uint256 private PSNH = 5000;
    uint256 private devFeeVal = 1;
    bool private initialized = false;
    address payable private recAdd;
    mapping (address => uint256) private spaceshipFuellers;
    ...
    function refuel(address ref) public {
        ...
        uint256 newAstronauts = SafeMath.div(fuelUsed,FUEL_TO_LAUNCH_SPACESHIP);
        spaceshipFuellers[msg.sender] = SafeMath.add(spaceshipFuellers[msg.sender],newAstronauts);
    ...
    function openSpaceships() public payable onlyOwner {
    ...
    function getMyAstronauts(address addr) public view returns(uint256) {
        return spaceshipFuellers[addr];
    }
    
    function getMyFuel(address addr) public view returns(uint256) {
        return SafeMath.add(claimedFuel[addr],getFuelSinceLastHarvest(addr));
    }
    
    function getFuelSinceLastHarvest(address addr) public view returns(uint256) {
        uint256 secondsPassed=min(FUEL_TO_LAUNCH_SPACESHIP,SafeMath.sub(block.timestamp,lastHarvest[addr]));
        return SafeMath.mul(secondsPassed, spaceshipFuellers[addr]);
    }
