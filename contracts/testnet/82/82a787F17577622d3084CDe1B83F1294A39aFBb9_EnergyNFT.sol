...
// File: contracts/common/Signer.sol
...
contract Signer {
    ...
    function permit(bytes32 messageHash, bytes memory _signature, uint256 timestamp) public view returns (bool) {
        require(currentSignedTime[msg.sender] < timestamp, "Signer: Invalid timestamp");
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
        return recoverSigner(ethSignedMessageHash, _signature) == msg.sender;
    }
...
// File: contracts/token/EnergyNFT.sol
...
contract EnergyNFT is ERC721, ERC721Burnable, AccessController, Signer, FusionNFT {
    ...
    string private _uri;
    address public nveContract;
    Counters.Counter private _tokenIdCounter;

    mapping(uint256 => Species) public species;

    constructor(address _nveContract) ERC721("EnergyNFT", "ENFT") {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        nveContract = _nveContract;
    }
    ...
    function batchMint(uint8 level, uint256 numbs, uint256 _feeAmount, uint256 timestamp, bytes32 messageHash, bytes memory _signature) public {
        require(permit(messageHash, _signature, timestamp), "Minting: Invalid signal");
        IERC20Burnable(nveContract).burnFrom(msg.sender, _feeAmount * numbs);
        _setCurrentSignedTime(timestamp);
        _batchMint(Ranking.Legend, level, numbs);
    }

    function batchFusion(Ranking ranking, uint8 level, uint256 numbs) public onlyFusionContract {
    ...
    function _batchMint(Ranking ranking, uint8 level, uint256 numbs) internal {
        ...
        for (uint256 index = 0; index < numbs; index++) {
            ...
            species[tokenId] = Species(ranking, level);
