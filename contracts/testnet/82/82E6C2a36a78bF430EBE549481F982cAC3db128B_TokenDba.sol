...
contract TokenDba is ERC20{
    ...
    constructor(LibDXba.Init memory _init) ERC20(_init.name,_init.symbol) {
        // __ERC20_init(_init.name,_init.symbol);
        _mint(address(this),_init.total * 1 ether);
        uint day = LibDXba.getDay(block.timestamp, 1646006400);
        // TransferHelper.safeTransfer(address(this), _init.grantsPurse, day * 1399 ether);
        ...
        usdt = _init.usdt;
        pair = IUniswapV2Factory(router.factory()).createPair(address(this),usdt); // Create Pair
        moreFee.poundage = 2;
    ...
    // init
    // function initialize(LibDXba.Init memory _init) public initializer {
    //     __ERC20_init(_init.name,_init.symbol);
    //     _mint(address(this),_init.total * 1 ether);
    //     uint day = LibDXba.getDay(block.timestamp, 1646006400);
    //     // TransferHelper.safeTransfer(address(this), _init.grantsPurse, day * 1399 ether);
    ...
    //     lockMessage.amount = lockAmount;
    // }
    // daily mint 1399（dailyTrigger test open）
    ...
    // # Developer Options
    // function clearLockStatus() public payable _devops {
    //     lockMessage.lockEndTime = 0;
    // }
    // function adminBurn(uint amount) public payable {
...
library LibDXba {
    ...
    struct Init {
        ...
        string symbol;
        uint total; // 总量
        uint dailyMint; // 每日可产出量
        uint institution; // 机构池
        address institutionWallet; // 机构池子钱包地址
        address grantsPurse; // 日产积累钱包（发合约当前日子 - 2022-2-28）天数
        address devops; // 开发者地址
        address outSideFeeWallet; // 场外手续费钱包
        address usdt; // USDT地址
        address swapFeeWallet; // 13% 兑换手续费钱包
        address removeLpFeeWallet; // 6% 移除流动性手续费钱包 
        address dailyRecv1399Wallet; //  日收 1399 接受钱包
        address transferFeeWallet; // 2% 转账钱包
        address nftDbaWallet; // 25%NFT 250w DBA 钱包
    }
    struct fee {
        uint poundage;
    ...
    function getDate(uint timestamp,uint timeZone) internal pure returns(uint[3] memory time){
        timestamp = timestamp + timeZone * 1 hours; // UTC Time Zone
        // leap Year
