...
contract VestingContract is Ownable, ReentrancyGuard {
	...
	uint256 public constant MAX_DAYS = 15;
    ...
    */
	function claim() external nonReentrant {
		...
		require(_now >= startAt, "TGE has not started yet");

		uint256 _next = policies[_beneficiary].next;
		_now = (_now - startAt) / PERIOD;
		require(_now >= _next, "Too early");

		if(_now > MAX_DAYS)
			_now = MAX_DAYS;

		uint256 _totalValue;
		uint256[] memory _amounts =  policies[_beneficiary].amounts;
		for(_next; _next <= _now; _next++) {
			_totalValue = _totalValue + _amounts[_next];
			_amounts[_next] = 0;
		}
		require(_totalValue != 0, "Vesting completed. Please check your policy again");
		...
		releaseTokenTo(_beneficiary, _now, _totalValue);
	}
    ...
    */
	function collect() external onlyOwner {
		...
		require(_balance != 0, "Vested complete");
		releaseTokenTo(_msgSender(), _until, _balance);
	}
