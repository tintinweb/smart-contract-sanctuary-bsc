...
contract testToken is ERC20, Ownable, SafeToken {
    ...
    address payable public lastManStandingWallet;
    ...
    function setFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee, uint256 _lmsFee) public onlyOwner {
        BNBRewardsFee = _bnbRewardFee;
        liquidityFee = _liquidityFee;
        marketingFee = _marketingFee;
        lmsFee = _lmsFee;

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(lmsFee); 
    }
    ...
    function setLmsWallet(address payable _newLmsWallet) public onlyOwner {
        lastManStandingWallet = _newLmsWallet;
    }
    ...
    constructor() ERC20("Test Token", "testToken") {
        ...
        marketingFee = 2;
        lmsFee = 2;
        marketingWallet = payable(0xB94A81F59Cf396F269f9D53aA2927f01C9f85ac9);
        lastManStandingWallet = payable(0xF86E4f8f842FB9ffdB1E4f8308441a83B1e56B81);

        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee).add(lmsFee); // total fee transfer and buy
        ...
        excludeFromFees(lastManStandingWallet, true);
        ...
        _isExcludedFromMaxTx[lastManStandingWallet] = true;
...
//this will be used to exclude from dividends the presale smart contract address
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        marketingWallet.transfer(marketingAmount);
         ...
         uint256 lmsAmount = deltaBalance.sub(bnbToAddLiquidityWith).div(totalFees.sub(liquidityFee).sub(marketingFee)).mul(lmsFee);
        lastManStandingWallet.transfer(lmsAmount);
    ...
    function swapAndSendBNBToBuybak(uint256 tokenAmount) private {
        swapTokensForBnb(tokenAmount, lastManStandingWallet);
    }
