...
contract Payments is SendValueOrEscrow {
     ...
     */
    function calcPercentagePayment(uint256 _amount, uint256 _percentage)
        internal
        pure
        returns (uint256)
    {
...
contract MarketPlace is OwnableUpgradeable, Payments {
    ...
    address public marketplaceFeeWallet = 0xD6D14cc63912E285Fac2Aa266076292ECA03b8c3;
    address public royaltyFeeWallet = 0x02175FE2823287182260044a3692dcF1528b59F9;
    ...
    function SetMarketplaceFeeWallet(address newWallet)
        public
        onlyOwner
    {
        marketplaceFeeWallet = newWallet;
    }

    function setRoyaltyFeeWallet(address newWallet)
        public
        onlyOwner
    {
        royaltyFeeWallet = newWallet;
    }
    ...
    function setSalePrice(
        ...
        tokenPrices[_originContract][_tokenId] = SalePrice(msg.sender, _amount, _royaltyPercentage);
        emit SetSalePrice(_originContract, _amount, _tokenId);
    }
    ...
    function buy(address _originContract, uint256 _tokenId) public payable {
        ...
        uint256 royalty = sp.royaltyPercentage;
        // address creator = IERC721(_originContract).owner();
        ...
        uint256 paymentPayee = (sp.amount).sub(paymentMarketPlace).sub(paymentRoyalty);       //sub 100 to ensure that available balance is always > sending balance
        
        if(paymentMarketPlace > 0){
            
            Payments.simplePayout(
                paymentMarketPlace,
                _makePayable(marketplaceFeeWallet)
            );
        }
        if(paymentRoyalty > 0){
        ...
        emit RoyaltyPaid(_originContract, msg.sender, tokenOwner, sp.amount, _tokenId, royalty);
    }
    ...
    function _resetTokenPrice(address _originContract, uint256 _tokenId)
        internal
    {
        tokenPrices[_originContract][_tokenId] = SalePrice(address(0), 0, 0);
    }
