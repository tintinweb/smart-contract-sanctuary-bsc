...
contract SilverlinePredictionV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public SlnPercentage;

    uint256 public constant MAX_TREASURY_FEE = 1000; // 10%

    address SLNTokenAddress;

    IERC20 SLN = IERC20(SLNTokenAddress);

    mapping(uint256 => mapping(address => BetInfo)) public ledger;
    mapping(uint256 => Round) public rounds;
    mapping(address => uint256[]) public userRounds;
    //mapping user -> epoch -> claimed or not
    mapping(address => mapping(uint256 => bool)) rewardClaimed;
    ...
    event UpdatedRewardRate(uint256 SlnPercentage);
    event SLNClaimed(uint256 amountClaimed);
    ...
    constructor(
        ...
        SLNTokenAddress = _SLNTokenAddress;
    }
    ...
    function setSLNForBNBLost(uint256 _SlnPercentage) external onlyAdmin returns (uint256) {
        SlnPercentage = _SlnPercentage;
        return SlnPercentage;
        emit UpdatedRewardRate(SlnPercentage);
    }

    function claimSLN() external {
        uint256[] memory roundsParticipated = userRounds[msg.sender];
        uint256 totalSLNAmount;
        for (uint256 i = 0; i < roundsParticipated.length; i++) {
            totalSLNAmount = totalSLNAmount + SLNrewardforEpoch(msg.sender, i);
            rewardClaimed[msg.sender][i] = true;
            SLN.transferFrom(operatorAddress, msg.sender, totalSLNAmount);
        }

        emit SLNClaimed(totalSLNAmount);
    }

    function SLNrewardforEpoch(address user, uint256 epoch) public view returns (uint256) {
        BetInfo memory betInfo = ledger[epoch][user];
        Round memory round = rounds[epoch];
        ...
        if (
            ...
            if (rewardClaimed[user][epoch]) {
                rewardSLN = rewardSLN + 0;
            } else {
                rewardSLN = rewardSLN + Amount * SlnPercentage;
            }
        }
        return rewardSLN;
    }

    function SLNrewardforEpochCalculate(address user) external view returns (uint256) {
        uint256[] memory roundsParticipated = userRounds[user];
        uint256 rewardSLN;
        for (uint256 i = 0; i < roundsParticipated.length; i++) {
            BetInfo memory betInfo = ledger[i][user];
