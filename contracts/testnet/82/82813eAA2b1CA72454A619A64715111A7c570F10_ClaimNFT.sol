...
contract ClaimNFT is Ownable {
    ...
    mapping(address => uint256) private claimWallet;
    mapping(address => uint256) private adminlist;
    mapping(address => uint256) private blacklist;
    mapping(uint256 => uint256) private countRarity;
    mapping(uint256 => uint256) private maxRarity;
    mapping(address => uint256) private userClaimNFT;
    mapping(address => uint256) private userClaimTime;

    uint256 randNonce = 0;
    uint256 private status = 1;
    uint256 private totalClaimNFT = 0;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            claimWallet[accounts[i]] = numofNFT[i];
        }
 ...
 */
    ...
    modifier limitClaimPerDay (address account) {
        require(block.timestamp - userClaimTime[account] > limitTime, "Not time to claim");
        _;
    }
    ...
    modifier onlyClaimWallet(address wallet) {
        require(claimWallet[wallet] > 0, "Only Lucky Wallet");
        _;
    }
 ...
 */
    ...
    function claimNFT()
        public
        isNotInBlackList(msg.sender)
        limitClaimPerDay(msg.sender)
        eventGoingOn
        onlyClaimWallet(msg.sender)
        returns (uint256)
    {
        ...
        userClaimTime[recipient] = block.timestamp;
        claimWallet[recipient] -= 1;
        userClaimNFT[recipient]++;
        totalClaimNFT++;
        return res;
    }
 ...
 */
    ...
    function changeLimitTime(uint256 limit) public onlyAdmin {
        limitTime = limit;
    }
...
================================================================
 */
    function addToClaimWallet(address wallet, uint256 numNFT) public onlyAdmin {
        claimWallet[wallet] = numNFT;
        emit ClaimWalletAdded(msg.sender, wallet, numNFT);
    }

    function removeFromClaimWallet(address wallet) public onlyAdmin {
        claimWallet[wallet] = 0;
        emit ClaimWalletRemoved(msg.sender, wallet);
    }
 ...
 */
    ...
    function getTotalClaimNFT() public view returns (uint256) {
        return totalClaimNFT;
    }

    function getUserClaimNFT(address user) public view returns (uint256) {
        return userClaimNFT[user];
    }
 ...
 */
    ...
    event ClaimWalletAdded(address user, address wallet, uint256 numNFT);
    event ClaimWalletRemoved(address user, address wallet);
    event EventStarted(address user, uint256 status);
    event EventStopped(address user, uint256 status);
}