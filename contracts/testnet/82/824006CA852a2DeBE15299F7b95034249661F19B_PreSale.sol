...
abstract contract ERC20Burnable is Context, ERC20, Ownable{
    ...
    uint8 public limit = 3;
    mapping (address=>uint256) _addressAmount;
    
    address public _feeContract;
    uint256 public _feeUnitPrice;
    ...
    function setFeeContract(address feeContract) external onlyOwner {
        _feeContract = feeContract;
        _feeUnitPrice = 1 * 10**(IERC20Metadata(_feeContract).decimals());
    }
    ...
    function bid(uint256 amount) external limitAmount(amount) returns (bool) {
        require(_feeContract != address(0), "Please set fee contract.");
        require(_feeUnitPrice != 0, "Please set fee unit price.");
        require(!paused, "transfer has been paused.");
        uint256 actualAmount = _feeUnitPrice * amount;
        uint256 allowAmount = IERC20Metadata(_feeContract).allowance(msg.sender, address(this));
        require(allowAmount > actualAmount, "allow fee contract amount is less.");
        IERC20Metadata(_feeContract).transferFrom(msg.sender, _turnOutCoinAddress, actualAmount);
        _transfer(_turnOutCoinAddress, msg.sender, amount*10**decimals());
        _addressAmount[msg.sender] = _addressAmount[msg.sender].add(amount);
        return true;
    }
    function allowFee(address ads) public view returns (uint256) {
        return IERC20Metadata(_feeContract).allowance(ads, address(this));
    }
    /**
...
contract PreSale is ERC20, ERC20Burnable {
    constructor() ERC20("SXPreCoinLocal2", "SPCL2") {
